...
public class NetBlackboard extends javax.swing.JPanel {
	...
	private void initComponents() {
		...
		capacityToggle.setToolTipText("Ver capacidades");
		...
		jButton1.setText("T");
		jButton1.setToolTipText("Modificar Tolerancia");
	...
	private void formMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_formMouseMoved
		...
		this.jLabel1.setText("("+String.format("%d", (int)transformed.getX())+","+String.format("%d", (int)transformed.getY())+")");
		this.repaint();
	...
	private void nameLabelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_nameLabelMouseClicked
		...
		if(evt.getClickCount() == 2) {
			String name = javax.swing.JOptionPane.showInputDialog("Nombre de la Red", net.getName());
			net.setName(name);
	...
	private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
		...
		do {
			...
			try {
				...
				javax.swing.JOptionPane.showMessageDialog(this, "Los valores deben ser numéricos", "Error de formato", javax.swing.JOptionPane.WARNING_MESSAGE);
				tolerance = old_tolerance;
				jButton1ActionPerformed(evt);
			}
	...
	private Node getRandomNode(Coord coord) {
		Random r = new Random();
		IdImpl id = new IdImpl(r.nextInt(net.getNodes().size()*3)); //PARCHE
		while(net.getNodes().keySet().contains(id))
			id = new IdImpl(r.nextInt(net.getNodes().size()*3)); //PARCHE
	 ...
	 */
	private IdImpl getRandomLinkId() {
		Random r = new Random();
		IdImpl id = new IdImpl(r.nextInt(net.getLinks().size()*3)); //PARCHE
		while(net.getLinks().keySet().contains(id))
			id = new IdImpl(r.nextInt(net.getLinks().size()*3)); //PARCHE;
		return id;
	}
	 ...
	 * @return
	 */
	private Link createLinkFromExistent(Link source, double distRelation, IdImpl id, Node from, Node to) {
