...
enum RLS19VehicleType implements NoiseVehicleType {
  ...
  /**
   * Lastkraftwagen mit Anhaenger bzw. Sattelkraftfahrzeuge (Zugmaschinen mit Auflieger) mit einer
   * zulaessigen Gesamtmasse ueber 3.5t
   *
   * <p>Lorries with trailers or articulated vehicles (trucks with trailers) with a permissible mass
   * exceeding 3.5t
   */
  ...
  RLS19VehicleType(
      double emissionParameterA,
      double emissionParameterB,
      double emissionParameterC,
      double lowerSpeedBound,
      double upperSpeedBound) {
