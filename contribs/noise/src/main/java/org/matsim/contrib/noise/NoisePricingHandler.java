...
/**
 * This handler calculates agent money events based on the noise damages an agent may cause
 * (NoiseEventCaused).
 *
 * @author ikaddoura
 */
class NoisePricingHandler implements NoiseEventCausedHandler {
  ...
  public void handleEvent(NoiseEventCaused event) {
    ...
    double amount =
        this.noiseContext.getNoiseParams().getNoiseTollFactor() * event.getAmount() * (-1);
    this.amountSum = this.amountSum + amount;

    PersonMoneyEvent moneyEvent =
        new PersonMoneyEvent(
            event.getTime(), event.getCausingAgentId(), amount, "noisePricing", null);
