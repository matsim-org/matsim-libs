...
final class NoiseContextImpl implements NoiseContext {
  ...
  private final Map<Tuple<Integer, Integer>, List<Id<Link>>> zoneTuple2listOfLinkIds =
      new HashMap<>();
  ...
  private final Map<Id<Link>, Map<Id<Vehicle>, Double>> linkId2vehicleId2lastEnterTime =
      new HashMap<>();
  ...
  private final Map<Double, Map<Id<Link>, NoiseLink>> timeInterval2linkId2noiseLinks =
      new HashMap<>();
  ...
  private void checkTunnels() {
    if (noiseParams.getTunnelLinkIdFile() != null) {
      final URL tunnelUrl =
          this.noiseParams.getTunnelLinkIDsFileURL(this.scenario.getConfig().getContext());
      try {
        if (Files.exists(Paths.get(tunnelUrl.toURI()))) {

          if (this.noiseParams.getTunnelLinkIDsSet().size() > 0) {
            log.warn(
                "Loading the tunnel link IDs from a file. Deleting the existing tunnel link IDs that are added manually.");
            ...
            while ((line = br.readLine()) != null) {
              ...
              for (int column = 0; column < columns.length; column++) {
                if (column == 0) {
                  ...
                  if (countWarning < 1) {
                    log.warn(
                        "Expecting the tunnel link Id to be in the first column. Ignoring further columns...");
  ...
  private void checkConsistency() {
    ...
    Collections.addAll(
        consideredActivitiesForDamagesList,
        this.grid.getGridParams().getConsideredActivitiesForDamageCalculationArray());

    Collections.addAll(
        consideredActivitiesForReceiverPointGridList,
        this.grid.getGridParams().getConsideredActivitiesForReceiverPointGridArray());

    if (this.noiseParams.isComputeNoiseDamages()) {

      if (this.scenario.getPopulation() == null
          || this.scenario.getPopulation().getPersons().isEmpty()) {
        throw new RuntimeException(
            "The noise computation requires a loaded population to identify passenger cars."
                + " Please provide a scenario which contains a loaded population. Aborting...");
      }

      if (consideredActivitiesForDamagesList.size() == 0) {
        log.warn(
            "Not considering any activity type for the noise damage computation."
      ...
      if (this.grid.getGridParams().getReceiverPointsGridMaxX() != 0.
        ...
        log.warn(
            "In order to keep track of the agent activities, the grid of receiver points should not be limited to a set of predefined coordinates."
      ...
      if (this.grid.getGridParams().getReceiverPointsGridMinX() == 0.
          && this.grid.getGridParams().getReceiverPointsGridMinY() == 0.
          && this.grid.getGridParams().getReceiverPointsGridMaxX() == 0.
          && this.grid.getGridParams().getReceiverPointsGridMaxY() == 0.) {
        for (String type : consideredActivitiesForDamagesList) {
          if (!consideredActivitiesForReceiverPointGridList.contains(type)) {
            throw new RuntimeException(
                "An activity type which is considered for the damage calculation ("
                    + type
    ...
    for (Link link : scenario.getNetwork().getLinks().values()) {
      ...
      for (Tuple<Integer, Integer> tuple : relevantTuples) {
        List<Id<Link>> linkIds =
            zoneTuple2listOfLinkIds.computeIfAbsent(tuple, k -> new ArrayList<>());
