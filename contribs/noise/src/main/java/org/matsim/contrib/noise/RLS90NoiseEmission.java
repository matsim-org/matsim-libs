...
class RLS90NoiseEmission implements NoiseEmission {
  ...
  private static final Logger log = LogManager.getLogger(RLS90NoiseEmission.class);
  ...
  public void calculateEmission(NoiseLink noiseLink) {
    ...
    // correction for intervals unequal to 3600 seconds (= one hour)
    nPlusOneCarOrHGV =
        (int) (nPlusOneCarOrHGV * (3600. / noiseParams.getTimeBinSizeNoiseComputation()));
    ...
    if (noiseEmissionPlusOneCar < noiseEmission || noiseEmissionPlusOneHgv < noiseEmission) {
      log.warn(
          "vCar: "
              ...
              + " - noise emission: "
              + noiseEmission
              + " - noise emission plus one car: "
              + noiseEmissionPlusOneCar
              + " - noise emission plus one hgv: "
              + noiseEmissionPlusOneHgv
  ...
  static double calculateMittelungspegelLm(int n, double p) {
    ...
    if (p > 1) {
      throw new RuntimeException(
          "p has to be <= 1. For an HGV share of 1%, p should be 0.01. Aborting...");
    }
  ...
  static double calculateGeschwindigkeitskorrekturDv(double vCar, double vHdv, double p) {
    ...
    double d = lHdv - lCar;
    double geschwindigkeitskorrekturDv =
        lCar
            - 37.3
            + 10
                * Math.log10(
                    (100.0 + (Math.pow(10.0, (0.1 * d)) - 1) * pInPercentagePoints)
                        / (100 + 8.23 * pInPercentagePoints));
  ...
  private double getV(NoiseLink noiseLink, NoiseVehicleType type) {
    ...
    if (velocity > freespeedCar) {
      throw new RuntimeException(
          velocity + " > " + freespeedCar + ". This should not be possible. Aborting...");
    }
    ...
    if (!noiseParams.isAllowForSpeedsOutsideTheValidRange()) {
      ...
      if (!validSpeedRange.contains(velocity)) {
        velocity =
            Math.min(
                Math.max(validSpeedRange.lowerEndpoint(), velocity),
                validSpeedRange.upperEndpoint());
      }
    }
    return velocity;
  }
