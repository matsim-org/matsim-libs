...
/**
 * Computes a grid of receiver points and provides some basic spatial functionality, e.g. the
 * nearest receiver point for the coordinates of each 'considered' activity type.
 *
...
final class Grid {
  ...
  private final Map<Id<Person>, List<Coord>> personId2consideredActivityCoords =
      new HashMap<Id<Person>, List<Coord>>();
  ...
  private final Map<Coord, Id<ReceiverPoint>> activityCoord2receiverPointId =
      new HashMap<Coord, Id<ReceiverPoint>>();
  ...
  public Grid(Scenario scenario) {
    ...
    String[] consideredActTypesForDamagesArray =
        noiseParams.getConsideredActivitiesForDamageCalculationArray();
    Collections.addAll(
        this.consideredActivitiesForSpatialFunctionality, consideredActTypesForDamagesArray);

    String[] consideredActTypesForReceiverPointGridArray =
        noiseParams.getConsideredActivitiesForReceiverPointGridArray();
    Collections.addAll(
        this.consideredActivitiesForReceiverPointGrid, consideredActTypesForReceiverPointGridArray);
  ...
  private void initialize() {
    ...
    if (scenario.getScenarioElement(NoiseReceiverPoints.NOISE_RECEIVER_POINTS) != null) {
      ...
      log.info(
          "Loading receiver points based on provided point coordinates in "
    ...
    for (Person person : scenario.getPopulation().getPersons().values()) {
      ...
      for (Activity activity :
          TripStructureUtils.getActivities(
              person.getSelectedPlan(), StageActivityHandling.ExcludeStageActivities)) {
        if (this.consideredActivitiesForSpatialFunctionality.contains(activity.getType())
            || consideredActivityPrefix(
                activity.getType(), this.consideredActivitiesForSpatialFunctionality)) {
          List<Coord> activityCoordinates =
              personId2consideredActivityCoords.computeIfAbsent(
                  person.getId(), value -> new ArrayList<>());
        ...
        if (this.consideredActivitiesForReceiverPointGrid.contains(activity.getType())
            || consideredActivityPrefix(
                activity.getType(), consideredActivitiesForReceiverPointGrid)) {
  ...
  private void loadGrid() {
    ...
    CoordinateTransformation ct =
        TransformationFactory.getCoordinateTransformation(
            this.noiseParams.getReceiverPointsCSVFileCoordinateSystem(),
  ...
  private void loadGridFromScenario() {
    this.receiverPoints =
        (NoiseReceiverPoints)
            scenario.getScenarioElement(NoiseReceiverPoints.NOISE_RECEIVER_POINTS);
    log.info("Total number of receiver points: " + receiverPoints.size());
  ...
  private void createGrid() {
    ...
    if (this.noiseParams.getReceiverPointsGridMinX() == 0.
        && this.noiseParams.getReceiverPointsGridMinY() == 0.
        && this.noiseParams.getReceiverPointsGridMaxX() == 0.
        && this.noiseParams.getReceiverPointsGridMaxY() == 0.) {

      log.info(
          "Creating receiver points for the entire area between the minimum and maximium x and y activity coordinates of all activity locations.");
      ...
      log.info(
          "Creating receiver points for the area between the coordinates ("
              + xCoordMin
              + "/"
              + yCoordMin
              + ") and ("
  ...
  private void createReceiverPoints() {
    ...
    for (double y = yCoordMax + 100.;
        y > yCoordMin - 100. - noiseParams.getReceiverPointGap();
        y = y - noiseParams.getReceiverPointGap()) {

      for (double x = xCoordMin - 100.;
          x < xCoordMax + 100. + noiseParams.getReceiverPointGap();
          x = x + noiseParams.getReceiverPointGap()) {
  ...
  private void setActivityCoord2NearestReceiverPointId() {
    ...
    QuadTree<ReceiverPoint> qTree =
        new QuadTree<>(
            xCoordMin - 15 * gap, yCoordMin - 15 * gap, xCoordMax + 15 * gap, yCoordMax + 15 * gap);
    ...
    while ((line = br.readLine()) != null) {
      ...
      if (line.isEmpty() || line.equals("") || columns.length != headers.length) {
        log.warn(
            "Skipping line "
                + lineCounter
                + ". Line is empty or the columns are inconsistent with the headers: ["
                + line.toString()
                + "]");
        ...
        NoiseReceiverPoint rp =
            new NoiseReceiverPoint(Id.create(id, ReceiverPoint.class), transformedCoord);
