...
public class NoiseDamageCalculation {
  ...
  private static final Logger log = LogManager.getLogger(NoiseDamageCalculation.class);
  ...
  @Inject private NoiseEmission emission;

  @Inject private NoiseImmission immission;

  @Inject private NoiseVehicleIdentifier noiseVehicleIdentifier;

  @Inject private Set<NoiseVehicleType> vehicleTypes;
        ...
        for (PersonActivityInfo actInfo : rp.getPersonId2actInfos().get(personId)) {
          double unitsThisPersonActivityInfo =
              actInfo.getDurationWithinInterval(
                      noiseContext.getCurrentTimeBinEndTime(),
                      noiseContext.getNoiseParams().getTimeBinSizeNoiseComputation())
                  / noiseContext.getNoiseParams().getTimeBinSizeNoiseComputation();
          affectedAgentUnits =
              affectedAgentUnits
                  + (unitsThisPersonActivityInfo * noiseContext.getNoiseParams().getScaleFactor());
        }
      }
    }
    ...
    if (noiseParams.isComputeCausingAgents()) {
      ...
      if (writeOutput()) {
        for (NoiseVehicleType vehicleType : vehicleTypes) {
          NoiseWriter.writeLinkAvgDamagePerVehicleTypeInfoPerHour(
              noiseContext, outputDirectory, vehicleType);
        }
      }
  ...
  private void calculateDamagePerReceiverPoint(NoiseReceiverPoint rp) {
    ...
    double damageCost =
        calculateDamageCosts(
            ...
            annualCostRate,
            timeBinsSize);
    double damageCostPerAffectedAgentUnit =
        calculateDamageCosts(
            noiseImmission, 1., currentTimeBinEndTime, annualCostRate, timeBinsSize);
  ...
  private void throwNoiseEventsAffected() {
    ...
    double timeBinSizeNoiseComputation =
      ...
      if (!(rp.getPersonId2actInfos().isEmpty())) {
        ...
        for (Id<Person> personId : rp.getPersonId2actInfos().keySet()) {
          ...
          for (PersonActivityInfo actInfo : rp.getPersonId2actInfos().get(personId)) {
            ...
            double factor =
                actInfo.getDurationWithinInterval(
                        currentTimeBinEndTime, timeBinSizeNoiseComputation)
                    / timeBinSizeNoiseComputation;
            double amount = factor * rp.getDamageCostsPerAffectedAgentUnit();

            if (amount != 0.) {
              NoiseEventAffected noiseEventAffected =
                  new NoiseEventAffected(
                      eventTime,
                      currentTimeBinEndTime,
                      personId,
                      amount,
                      rp.getId(),
                      actInfo.getActivityType());
  ...
  private void calculateCostSharesPerLinkPerTimeInterval(NoiseReceiverPoint rp) {
    if (rp.getDamageCosts() != 0.) {
      for (Id<Link> linkId : rp.getLinkId2IsolatedImmission().keySet()) {
        ...
        if (!(linkImmission == 0.)) {
          double costShare =
              NoiseEquations.calculateShareOfResultingNoiseImmission(
                  linkImmission, rp.getCurrentImmission());
      ...
      if (noiseLink != null) {
        ...
        if (damageCostSum > 0) {
          ...
          for (NoiseVehicleType type : vehicleTypes) {
            ...
            if (counts[j] > 0) {
              double damageCostPerVehicle =
                  damageCostSumVehicle / (counts[j] * noiseParams.getScaleFactor());
  ...
  //		// ...
  //
  //		// resultingImmission = computeResultingImmission(Immission_linkA(n), Immission_linkB(n),
  // Immission_linkC(n), ...)
  //
  ...
  // ...)
  //
  //		if (printLog) log.info("Computing the marginal damage cost for each link and receiver
  // point...");
  //		calculateMarginalDamageCost();
  //		if (writeOutput()) NoiseWriter.writeLinkMarginalCarDamageInfoPerHour(noiseContext,
  // outputDirectory);
  //		if (writeOutput()) NoiseWriter.writeLinkMarginalHgvDamageInfoPerHour(noiseContext,
  // outputDirectory);
  ...
  private void calculateMarginalDamageCost(NoiseReceiverPoint rp) {
    ...
    if (rp.getAffectedAgentUnits() != 0.) {
      for (NoiseVehicleType type : vehicleTypes) {
        final TObjectDoubleMap<Id<Link>> link2ImmissionPlusOne =
            rp.getLinkId2IsolatedImmissionPlusOneVehicle().get(type);
        for (Id<Link> thisLink : rp.getLinkId2IsolatedImmission().keySet()) {
          ...
          double noiseImmissionPlusOneVehicleThisLink =
              NoiseEquations.calculateResultingNoiseImmissionPlusOneVehicle(
                  rp.getCurrentImmission(), isolated, isolatedPlusOne);
          double damageCostsPlusOneVehicleThisLink =
              calculateDamageCosts(
                  noiseImmissionPlusOneVehicleThisLink,
                  rp.getAffectedAgentUnits(),
                  this.noiseContext.getCurrentTimeBinEndTime(),
                  this.noiseContext.getNoiseParams().getAnnualCostRate(),
                  this.noiseContext.getNoiseParams().getTimeBinSizeNoiseComputation());
          double marginalDamageCostVehicleThisLink =
              (damageCostsPlusOneVehicleThisLink - rp.getDamageCosts())
                  / this.noiseContext.getNoiseParams().getScaleFactor();
          ...
          if (marginalDamageCostVehicleThisLink < 0.0) {
            if (Math.abs(marginalDamageCostVehicleThisLink) < 0.0000000001) {
              ...
              if (cWarn3 == 0) {
                log.warn(
                    "The marginal damage cost per car on link "
                        + thisLink.toString()
                        + " for receiver point "
                        + rp.getId().toString()
                        + " is "
                        + marginalDamageCostVehicleThisLink
                        + ".");
                log.warn(
                    "final immission: "
                        + rp.getCurrentImmission()
                        + " - immission plus one car "
                        + noiseImmissionPlusOneVehicleThisLink
                        + " - marginal damage cost car: "
                        + marginalDamageCostVehicleThisLink);
  ...
  private void throwNoiseEventsCaused() {
    double eventTime = this.noiseContext.getEventTime();
    double currentTimeBinEndTime = this.noiseContext.getCurrentTimeBinEndTime();
    NoiseConfigGroup.NoiseAllocationApproach noiseAllocationApproach =
        this.noiseContext.getNoiseParams().getNoiseAllocationApproach();
      ...
      if (noiseLink != null) {
        if (noiseLink.getEnteringVehicleIds() != null) {
          for (Id<Vehicle> vehicleId : noiseLink.getEnteringVehicleIds()) {
            ...
            final NoiseVehicleType noiseVehicleType =
                noiseVehicleIdentifier.identifyVehicle(vehicleId);
            ...
            } else if (noiseAllocationApproach
            ...
            if (amountVehicle != 0.) {
              ...
              if (this.noiseContext.getNotConsideredTransitVehicleIDs().contains(vehicleId)) {
                ...
                NoiseEventCaused noiseEvent =
                    new NoiseEventCaused(
                        eventTime,
                        currentTimeBinEndTime,
                        this.noiseContext
                            .getLinkId2vehicleId2lastEnterTime()
                            .get(linkId)
                            .get(vehicleId),
  ...
  static double calculateDamageCosts(
      double noiseImmission,
      double affectedAgentUnits,
      double timeInterval,
      double annualCostRate,
      double timeBinSize) {
    ...
    double damageCosts =
        (annualCostRate * laermEinwohnerGleichwert / 365.) * (timeBinSize / (24.0 * 3600));
