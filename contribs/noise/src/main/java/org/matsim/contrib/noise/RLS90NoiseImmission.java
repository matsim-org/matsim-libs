...
class RLS90NoiseImmission implements NoiseImmission {
  ...
  private static final Logger log = LogManager.getLogger(RLS90NoiseImmission.class);
  ...
  public void calculateImmission(NoiseReceiverPoint rp, double currentTimeBinEndTime) {
    ...
    Map<RLS90VehicleType, TObjectDoubleMap<Id<Link>>> linkId2IsolatedImmissionPlusOneVehicle =
        new EnumMap<>(RLS90VehicleType.class);
      ...
      for (Id<Link> linkId : rp.getRelevantLinks()) {
        ...
        if (noiseParams.getTunnelLinkIDsSet().contains(linkId)) {
          linkId2IsolatedImmissionPlusOneVehicle
              .computeIfAbsent(RLS90VehicleType.car, type -> new TObjectDoubleHashMap<>())
              .put(linkId, 0.);
          linkId2IsolatedImmissionPlusOneVehicle
              .computeIfAbsent(RLS90VehicleType.hgv, type -> new TObjectDoubleHashMap<>())
              .put(linkId, 0.);
        } else {
  ...
  private double calculateIsolatedLinkImmissionPlusOneVehicle(
    ...
    if (!(noiseLink.getEmissionPlusOneVehicle(type) == 0.)) {
      double correction = rp.getLinkCorrection(noiseLink.getId());
      plusOne = noiseLink.getEmissionPlusOneVehicle(type) + correction;
    }
    return plusOne;
  }
