  ...
  public BarrierContext(Config config) {
    ...
    if (noiseParams.isConsiderNoiseBarriers()) {
      final Collection<FeatureNoiseBarrierImpl> barriers =
          FeatureNoiseBarriersReader.read(
              noiseParams.getNoiseBarriersFilePath(),
              noiseParams.getNoiseBarriersSourceCRS(),
              config.global().getCoordinateSystem());

      for (NoiseBarrier barrier : barriers) {
        try {
          this.noiseBarriers.insert(
              barrier.getGeometry().getGeometry().getEnvelopeInternal(), barrier);
        } catch (IllegalArgumentException e) {
          logger.warn(
              "Could not add noise barrier " + barrier.getId() + " to quad tree. Ignoring it.");
        }
      }
    }
  }
