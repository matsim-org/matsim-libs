...
final class ShieldingContext {
  ...
  private static final Logger logger = LogManager.getLogger(ShieldingContext.class);

  // STRtree increases performance by ~40% by reducing the amount of potential
  // obstruction candidates. nkuehnel, mar '20
  private static final double GROUND_HEIGHT = 0.5;
  ...
  @Inject
  ShieldingContext(
      Config config, ShieldingCorrection shieldingCorrection, BarrierContext barrierContext) {
  ...
  double determineShieldingValue(Coordinate rpPoint, LineSegment segment) {
    ...
    // GeometryUtils.createGeotoolsPoint(link.getFromNode().getCoord());
    ...
    NavigableMap<Double, Coordinate> edgeCandidates =
        getObstructionEdges(rpPoint, midPoint, projectedLineOfSight);
    ...
    if (!edgeCandidates.isEmpty()) {
      ...
      double lastEdgeToSourceDistance =
          Math.sqrt(
              lastEdgeSourceXYDiff * lastEdgeSourceXYDiff
                  + lastEdgeSourceZDiff * lastEdgeSourceZDiff);

      correctionTermShielding =
          shieldingCorrection.calculateShieldingCorrection(
              rpPoint.distance(midPoint),
              lastEdgeToSourceDistance,
              receiverToFirstEdgeDistance,
              shieldingDepth);
    }
    return correctionTermShielding;
  }
  ...
  private ConcurrentSkipListMap<Double, Coordinate> getObstructionEdges(
      Point receiver,
      Point source,
      LineString directLineOfSight,
      LineString fromLineOfSight,
      LineString toLineOfSight) {
    ...
    for (NoiseBarrier noiseBarrier : candidates) {
      if (isObstructing(
          receiver, source, fromLineOfSight, toLineOfSight, noiseBarrier.getGeometry())) {
        ...
        final Set<Coordinate> intersections =
            intersection(
                (Polygon) noiseBarrier.getGeometry().getGeometry(),
                directLineOfSight.getCoordinates());
  ...
  /** Returns an ordered map of distances and coords to all obstruction edges */
  private ConcurrentSkipListMap<Double, Coordinate> getObstructionEdges(
      Coordinate receiver, Coordinate source, LineString directLineOfSight) {
    final Collection<NoiseBarrier> candidates =
        barrierContext.query(directLineOfSight.getEnvelopeInternal());
  ...
  /**
   * Checks whether a barrier is obstructing the receiver point-emission source relation. The
   * following has to be met: 1) the barrier must not contain the receiver point 2) the direct
   * projected line of sight must intersect the barrier 3) the line of sight from receiver to the
   * from-Node of the link intersects the barrier 4) the line of sight from receiver to the to-Node
   * of the link intersects the barrier
   *
   * <p>Uses prepared geometry for the barrier polygon to cache intersection graphs.
   */
  ...
  private boolean isObstructing(
      Coordinate receiver, Coordinate source, PreparedGeometry barrier, LineString direct) {
  ...
  double determineShieldingValue(ReceiverPoint receiverPoint, Link link, Coord projectedCoord) {
    ...
    if (!edgeCandidates.isEmpty()) {
      ...
      correctionTermShielding =
          shieldingCorrection.calculateShieldingCorrection(
              rpPoint.distance(projectedPoint),
              lastEdgeToSourceDistance,
              receiverToFirstEdgeDistance,
              shieldingDepth);
    }
