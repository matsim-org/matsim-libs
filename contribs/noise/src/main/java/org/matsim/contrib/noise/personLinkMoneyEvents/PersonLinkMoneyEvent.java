...
/**
 * The idea is to have a standard events format to be used by various external cost pricing
 * approaches.
 *
 * <p>The normal {@link PersonMoneyEvent} is not sufficient since we need refer the toll to a
 * specific link and the link information may not be available when the money event is thrown. Also,
 * the routing relevant time may be different from the time when the event is thrown.
 *
 * @author ikaddoura
 */
public class PersonLinkMoneyEvent extends Event implements HasPersonId {
  ...
  public PersonLinkMoneyEvent(
      final double time,
      final Id<Person> agentId,
      final Id<Link> linkId,
      final double amount,
      final double relevantTime,
