...
public final class NoiseComputationModule extends AbstractModule {
  ...
  public void install() {
    ...
    final Multibinder<NoiseVehicleType> noiseVehicleTypeMultibinder =
        Multibinder.newSetBinder(this.binder(), NoiseVehicleType.class);

    NoiseConfigGroup noiseParameters =
        ConfigUtils.addOrGetModule(this.getConfig(), NoiseConfigGroup.class);
    ...
    switch (noiseParameters.getNoiseComputationMethod()) {
      ...
      default:
        throw new IllegalStateException(
            "Unrecognized noise computation method: "
                + noiseParameters.getNoiseComputationMethod());
    }
    ...
    if (noiseParameters.isInternalizeNoiseDamages()) {
      ...
      log.info(
          "Internalizing noise damages. This requires that the default travel disutility is replaced by a travel distuility which accounts for noise tolls.");
    }
