  ...
  public final void run() {
    ...
    if (receiverPointsFile != null) {
      ...
      throw new RuntimeException(
          "Either provide the receiver points file or the network file! Aborting...");
    }
  }
          ...
          for (Id<ReceiverPoint> rp : this.rp2Coord.keySet()) {
            ...
            for (double time = startTime; time <= endTime; time = time + timeBinSize) {
              ...
              String lineToWrite =
                  rp.toString()
                      + ";"
                      + rp2Coord.get(rp).getX()
                      + ";"
                      + rp2Coord.get(rp).getY()
                      + ";"
                      + String.valueOf(time);
    ...
    for (int ll = 0; ll < this.labels.length; ll++) {
      ...
      for (double time = startTime; time <= endTime; time = time + timeBinSize) {
        ...
        try (BufferedReader br = IOUtils.getBufferedReader(fileName)) {
          // this will automagically use the *.gz version if a non-gzipped version does not exist.
          // kai, jan'15
