...
public final class NoiseConfigGroup extends ReflectiveConfigGroup {
  ...
  private static final String CONSIDERED_ACTIVITIES_FOR_DAMAGE_CALCULATION =
      "consideredActivitiesForDamageCalculation";
  private static final String CONSIDERED_ACTIVITIES_FOR_RECEIVER_POINT_GRID =
  ...
  private static final String RECEIVER_POINTS_CSV_FILE_COORDINATE_SYSTEM =
      "receiverPointsCSVFileCoordinateSystem";
  ...
  private static final String ALLOW_FOR_SPEEDS_OUTSIDE_THE_VALID_RANGE =
      "allowForSpeedsOutsideTheValidRange";
  ...
  private static final String COMPUTE_AVG_NOISE_COST_PER_LINK_AND_TIME =
      "computeAvgNoiseCostPerLinkAndTime";
  ...
  private static final String RECEIVER_POINT_GAP_CMT =
      "horizontal and vertical distance between receiver points in x-/y-coordinate units";
  private static final String WRITE_OUTPUT_ITERATION_CMT =
  ...
  private static final String NOISE_BARRIERS_SOURCE_CRS =
      "source coordinate reference system of noise barriers geojson file";
  ...
  public Map<String, String> getComments() {
    ...
    comments.put(
        ...
        "coordinate system; so far only tested for 'TransformationFactory.DHDN_GK4'");
    comments.put(
        CONSIDERED_ACTIVITIES_FOR_DAMAGE_CALCULATION,
        "Specifies the activity types that are considered when computing noise damages (= the activities at which being exposed to noise results in noise damages). A list of the exact activity types, e.g. 'home,work_8hours,work_4hours', the prefixes 'home*,work*' or both, e.g. 'home,work*'.\"");
    comments.put(
        CONSIDERED_ACTIVITIES_FOR_RECEIVER_POINT_GRID,
        "Creates a grid of noise receiver points which contains all agents' activity locations of the specified types. A list of the exact activity types, e.g. 'home,work_8hours,work_4hours', the prefixes 'home*,work*' or both, e.g. 'home,work*'.\"");
    comments.put(
        RECEIVER_POINTS_GRID_MIN_X,
        "Specifies a boundary coordinate min/max x/y value of the receiver point grid. "
    ...
    comments.put(
        RECEIVER_POINTS_GRID_MAX_X,
        "Specifies a boundary coordinate min/max x/y value of the receiver point grid. "
            + "0.0 means the boundary coordinates are ignored and the grid is created based on the agents' activity coordinates of the specified activity types "
            + "(see parameter 'consideredActivitiesForReceiverPointGrid').");
    comments.put(
        RECEIVER_POINTS_GRID_MIN_Y,
        "Specifies a boundary coordinate min/max x/y value of the receiver point grid. "
            + "0.0 means the boundary coordinates are ignored and the grid is created based on the agents' activity coordinates of the specified activity types "
            + "(see parameter 'consideredActivitiesForReceiverPointGrid').");
    comments.put(
        RECEIVER_POINTS_GRID_MAX_Y,
        "Specifies a boundary coordinate min/max x/y value of the receiver point grid. "
            + "0.0 means the boundary coordinates are ignored and the grid is created based on the agents' activity coordinates of the specified activity types "
            + "(see parameter 'consideredActivitiesForReceiverPointGrid').");
    comments.put(
        RECEIVER_POINTS_CSV_FILE,
        "A csv file which provides the ReceiverPoint coordinates (first column: id, second column: x-coordinate, third column: y-coordinate, separator: ',')");
    comments.put(
        RECEIVER_POINTS_CSV_FILE_COORDINATE_SYSTEM,
        "The coordinate reference system of the provided ReceiverPoint csv file.");
    comments.put(
        ANNUAL_COST_RATE,
        "annual noise cost rate [in EUR per exposed pulation unit]; following the German EWS approach");
    comments.put(
        TIME_BIN_SIZE_NOISE_COMPUTATION,
    ...
    comments.put(
        ALLOW_FOR_SPEEDS_OUTSIDE_THE_VALID_RANGE,
    ...
    comments.put(
        NOISE_TOLL_FACTOR,
        "To be used for sensitivity analysis. Default: 1.0 (= the parameter has no effect)");

    comments.put(
        CONSIDER_NOISE_BARRIERS,
        "Set to 'true' if noise barriers / building shielding should be considered. Otherwise set to 'false'.");
    ...
    comments.put(
        USE_DEM,
        "Set to 'true' if a DEM (digital elevation model) should be used for road gradients. Otherwise set to 'false'.");
    comments.put(DEM_FILE, "Path to the geoTiff file of the DEM.");

    comments.put(
        NETWORK_MODES_TO_IGNORE,
        "Specifies the network modes to be excluded from the noise computation, e.g. 'bike'.");

    comments.put(
        NOISE_COMPUTATION_METHOD,
        "Specifies the computation method of different guidelines: "
            + Arrays.toString(NoiseComputationMethod.values()));
  ...
  protected void checkConsistency(Config config) {
    ...
    if (Math.abs(this.getAnnualCostRate() - annualCostRateEws) < 0.001) {
      log.warn(
          "you are using old EWS noise annual cost rates; please go through https://ec.europa"
  ...
  private void checkGridParametersForConsistency() {
    ...
    Collections.addAll(
        consideredActivitiesForDamagesList, consideredActivitiesForDamageCalculation);
    consideredActivitiesForReceiverPointGridList.addAll(
        Arrays.asList(consideredActivitiesForReceiverPointGrid));

    if (this.receiverPointGap == 0.) {
      throw new RuntimeException("The receiver point gap is 0. Aborting...");
    }

    if (consideredActivitiesForReceiverPointGridList.size() == 0
        && this.receiverPointsGridMinX == 0.
        && this.receiverPointsGridMinY == 0.
        && this.receiverPointsGridMaxX == 0.
        && receiverPointsGridMaxY == 0.) {
      throw new RuntimeException(
          "NEITHER providing a considered activity type for the minimum and maximum coordinates of the receiver point grid area "
  ...
  private void checkNoiseParametersForConsistency(Config config) {
    ...
    if (this.useActualSpeedLevel && this.allowForSpeedsOutsideTheValidRange) {
      log.warn(
          "Using the actual vehicle speeds for the noise computation may result in very low speed levels due to congestion."
    ...
    if (this.considerNoiseBarriers) {
      if (this.noiseBarriersFilePath == null || "".equals(this.noiseBarriersFilePath)) {
        log.warn(
            "Cannot consider noise barriers without a specified file path to the geojson file of barriers / buildings.");
  ...
  public void setReceiverPointGap(double receiverPointGap) {
    log.info(
        "setting the horizontal/vertical distance between each receiver point to "
            + receiverPointGap);
  ...
  public void setConsideredActivitiesForReceiverPointGridArray(
  ...
  public void setConsideredActivitiesForDamageCalculationArray(
      String[] consideredActivitiesForSpatialFunctionality) {
  ...
  public void setConsideredActivitiesForReceiverPointGrid(
      String consideredActivitiesForReceiverPointGridString) {
    this.setConsideredActivitiesForReceiverPointGridArray(
        CollectionUtils.stringToArray(consideredActivitiesForReceiverPointGridString));
  }
  ...
  public void setConsideredActivitiesForDamageCalculation(
      String consideredActivitiesForSpatialFunctionalityString) {
    this.setConsideredActivitiesForDamageCalculationArray(
  ...
  public void setTimeBinSizeNoiseComputation(double timeBinSizeNoiseComputation) {
    log.info(
        "setting the time bin size for the computation of noise to " + timeBinSizeNoiseComputation);
  ...
  public void setRelevantRadius(double relevantRadius) {
    log.info(
        "setting the radius of relevant links around each receiver point to " + relevantRadius);
  ...
  public void setAllowForSpeedsOutsideTheValidRange(boolean allowForSpeedsOutsideTheValidRange) {
    log.info(
        "Allowing for speeds above or below the valid range (cars: 30-130 km/h; HGV: 30-80 km/h): "
            + allowForSpeedsOutsideTheValidRange);
  ...
  public void setReceiverPointsCSVFileCoordinateSystem(
      String receiverPointsCSVFileCoordinateSystem) {
