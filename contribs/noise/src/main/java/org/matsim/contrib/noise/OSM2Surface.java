...
public class OSM2Surface {
  ...
  public static void main(String[] args) {
    XmlReader xmlReader =
        new XmlReader(
            new File("D:\\resultStorage\\moia-msm\\final\\surfaces.osm"),
            false,
  ...
  private static class SurfaceSink implements Sink {
            ...
            public void process(WayContainer wayContainer) {
              ...
              final boolean surfaced =
                  entity.getTags().stream()
                      .anyMatch(
                          tag ->
                              "surface".equals(tag.getKey())
                                  && ("cobblestone".equals(tag.getValue())
                                      || "sett".equals(tag.getValue())));
              if (relevantHighway && surfaced) {
                final Optional<Tag> first =
                    entity.getTags().stream()
                        .filter(tag -> "surface".equals(tag.getKey()))
                        .findFirst();
    ...
    public void complete() {
      ...
      new MatsimNetworkReader(network)
          .readFile(
              "D:\\resultStorage\\moia-msm\\realisticModeChoice\\outputMixedCarOnly126\\croppedDenseNetwork.xml.gz");
      ...
      id2Surface.forEachEntry(
          new TLongObjectProcedure<String>() {
            @Override
