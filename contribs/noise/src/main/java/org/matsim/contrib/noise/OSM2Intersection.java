...
public class OSM2Intersection {
  ...
  public static void main(String[] args) {
    XmlReader xmlReader =
        new XmlReader(
            new File("D:\\resultStorage\\moia-msm\\final\\intersections.osm"),
            false,
  ...
  private static class SurfaceSink implements Sink {
      ...
      entityContainer.process(
          new EntityProcessor() {
            ...
            public void process(WayContainer wayContainer) {
              ...
              final boolean relevantHighway =
                  entity.getTags().stream()
                      .anyMatch(
                          tag ->
                              "highway".equals(tag.getKey())
                                  && ("trunk".equals(tag.getValue())
                                      || "primary".equals(tag.getValue())
                                      || "secondary".equals(tag.getValue())
                                      || "tertiary".equals(tag.getValue())
                                      || "residential".equals(tag.getValue())
                                      || "living_street".equals(tag.getValue())
                                      || "trunk_link".equals(tag.getValue())
                                      || "primary_link".equals(tag.getValue())
                                      || "secondary_link".equals(tag.getValue())
                                      || "tertiary_link".equals(tag.getValue())));
              final boolean roundabout =
                  entity.getTags().stream()
                      .anyMatch(
                          tag ->
                              "junction".equals(tag.getKey())
                                  && ("roundabout".equals(tag.getValue())));
    ...
    public void complete() {
      ...
      new MatsimNetworkReader(network)
          .readFile(
              "D:\\resultStorage\\moia-msm\\realisticModeChoice\\outputMixedCarOnly126\\croppedDenseNetwork_surface.xml.gz");
              ...
              for (Link link : links) {
                if (link != null) {
                  ...
                  fromNode
                      .getAttributes()
                      .putAttribute(
                          IntersectionContext.INTERSECTION_TYPE,
                          IntersectionContext.RLS19IntersectionType.roundabout);
                  toNode
                      .getAttributes()
                      .putAttribute(
                          IntersectionContext.INTERSECTION_TYPE,
          ...
          node -> {
            ...
            if (nodeM != null) {
              nodeM
                  ...
                  .putAttribute(
                      IntersectionContext.INTERSECTION_TYPE,
                      IntersectionContext.RLS19IntersectionType.signalized);
            } else {
              System.out.println("could not find node!");
              Coord coord = ct.transform(new Coord(node.getLongitude(), node.getLatitude()));
              final org.matsim.api.core.v01.network.Node nearestNode =
                  NetworkUtils.getNearestNode(network, coord);
              nearestNode
                  .getAttributes()
                  .putAttribute(
                      IntersectionContext.INTERSECTION_TYPE,
      ...
      new NetworkWriter(network)
          .write(
              "D:\\resultStorage\\moia-msm\\realisticModeChoice\\outputMixedCarOnly126\\croppedDenseNetwork_surface_junctions.xml.gz");
    }
