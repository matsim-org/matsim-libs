...
public class RLS19NoiseImmission implements NoiseImmission {
  ...
  private static final Logger log = LogManager.getLogger(RLS19NoiseImmission.class);
  ...
  public void calculateImmission(NoiseReceiverPoint rp, double currentTimeBinEndTime) {
    ...
    Map<RLS19VehicleType, TObjectDoubleMap<Id<Link>>> linkId2IsolatedImmissionPlusOneVehicle =
        new EnumMap<>(RLS19VehicleType.class);
    TObjectDoubleMap<Id<Link>> linkId2IsolatedImmission =
        new TObjectDoubleHashMap<>(rp.getRelevantLinks().size());
    if (!rp.getRelevantLinks().isEmpty()) {
      for (Id<Link> linkId : rp.getRelevantLinks()) {
        ...
        if (noiseParams.getTunnelLinkIDsSet().contains(linkId)) {
          linkId2IsolatedImmissionPlusOneVehicle
              .computeIfAbsent(pkw, type -> new TObjectDoubleHashMap<>())
              .put(linkId, 0.);
          linkId2IsolatedImmissionPlusOneVehicle
              .computeIfAbsent(lkw1, type -> new TObjectDoubleHashMap<>())
              .put(linkId, 0.);
          linkId2IsolatedImmissionPlusOneVehicle
              .computeIfAbsent(lkw2, type -> new TObjectDoubleHashMap<>())
              .put(linkId, 0.);
        } else {
          ...
          if (noiseLink != null) {
            ...
            for (RLS19VehicleType vehicleType : RLS19VehicleType.values()) {
              double immissionPlusOne =
                  calculateIsolatedLinkImmissionPlusOneVehicle(rp, noiseLink, vehicleType);
              ...
              linkId2IsolatedImmissionPlusOneVehicle
                  .computeIfAbsent(
                      vehicleType, type -> new TObjectDoubleHashMap<>(rp.getRelevantLinks().size()))
                  .put(linkId, immissionPlusOne);
            }
          }
        }
        if (noiseImmission > 0.) {
  ...
  public double calculateCorrection(
      double projectedDistance, NoiseReceiverPoint nrp, Link candidateLink) {
  ...
  private double getSubSectionsCorrection(Coordinate nrpCoordinate, LineSegment segment) {
    ...
    if (length <= maxL) {
      final double sectionCorrection =
          10 * Math.log10(length) - calculateCorrection(nrpCoordinate, segment, null);
      correctionTemp += Math.pow(10, 0.1 * sectionCorrection);

      final Set<ReflectionContext.ReflectionTuple> reflectionLinks =
          reflection.getReflections(segment);
      for (ReflectionContext.ReflectionTuple reflection : reflectionLinks) {
        double sectionCorrectionReflection =
            10 * Math.log10(reflection.reflectionLink.getLength())
                - calculateCorrection(nrpCoordinate, reflection.reflectionLink, reflection.facade);
      ...
      final double sectionCorrection =
          10 * Math.log10(central.getLength()) - calculateCorrection(nrpCoordinate, central, null);
      correctionTemp += Math.pow(10, 0.1 * sectionCorrection);

      final Set<ReflectionContext.ReflectionTuple> reflectionLinks =
          reflection.getReflections(central);
      for (ReflectionContext.ReflectionTuple reflection : reflectionLinks) {
        double sectionCorrectionReflection =
            10 * Math.log10(reflection.reflectionLink.getLength())
                - calculateCorrection(nrpCoordinate, reflection.reflectionLink, reflection.facade);
  ...
  private double calculateCorrection(
      Coordinate nrp, LineSegment segment, LineSegment reflectionFacade) {
    ...
    double groundDampening =
        Math.max(4.8 - (AVERAGE_GROUND_HEIGHT / distance) * (34 + 600 / distance), 0);

    if (noiseParams.isConsiderNoiseBarriers()) {
      if (reflectionFacade != null) {
        return geometricDivergence
            + airDampeningFactor
            - intersectionCorrection
            - multipleReflectionCorrection
            + Math.max(
                groundDampening, this.shielding.determineShieldingValue(nrp, reflectionFacade))
            + 0.5;
      } else {
        return geometricDivergence
            + airDampeningFactor
  ...
  private double calculateLinkImmission(NoiseReceiverPoint rp, NoiseLink noiseLink) {
    if (!(noiseLink.getEmission() == 0.)) {
      double noiseImmission =
          Math.pow(10, 0.1 * noiseLink.getEmission()) * rp.getLinkCorrection(noiseLink.getId());
      if (noiseImmission < 0.) {
        noiseImmission = 0.;
      }
  ...
  private double calculateIsolatedLinkImmissionPlusOneVehicle(
      NoiseReceiverPoint rp, NoiseLink noiseLink, NoiseVehicleType type) {
    if (!(noiseLink.getEmission() == 0.)) {
      double noiseImmission =
          10
              * Math.log10(
                  Math.pow(10, 0.1 * noiseLink.getEmissionPlusOneVehicle(type))
                      * rp.getLinkCorrection(noiseLink.getId()));
