...
final class IntersectionContext {
  ...
  private static final Logger logger = LogManager.getLogger(IntersectionContext.class);
  ...
  IntersectionContext(Network network) {
    ...
    this.intersections =
        new QuadTree<>(boundingBox[0], boundingBox[1], boundingBox[2], boundingBox[3]);
      ...
      try {
        ...
        if (intersectionType != null) {
          switch ((IntersectionContext.RLS19IntersectionType) intersectionType) {
            case signalized:
              this.intersections.put(
                  node.getCoord().getX(),
                  node.getCoord().getY(),
                  new Intersection(RLS19IntersectionType.signalized, coordinate));
              break;
            case roundabout:
              this.intersections.put(
                  node.getCoord().getX(),
                  node.getCoord().getY(),
                  new Intersection(RLS19IntersectionType.roundabout, coordinate));
              break;
            case other:
              break;
            default:
  ...
  /**
   * Die Stoerwirkung durch das Anfahren und Bremsen der Fahrzeuge an Knotenpunkten wird in
   * Abhaengigkeit vom Knotenpunkttyp {@link RLS19IntersectionType} und von der Entfernung zum
   * Schnittpunkt von sich kreuzenden oder zusammentreffenden Quellinien bestimmt (=nodes)
   *
   * <p>The disturbance caused by the starting and braking of vehicles at junctions is determined
   * according to the type of junction {@link RLS19IntersectionType} and the distance to the point
   * of intersection of intersecting or converging source lines (=nodes)
   */
