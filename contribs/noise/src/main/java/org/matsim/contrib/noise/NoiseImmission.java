...
public interface NoiseImmission {
  ...
  /**
   * Trigegrs the calculation of noise immission for the current time bin at the given receiver
   * point. The method is not expected to return the immission but should use the methods provided
   * in {@link NoiseReceiverPoint} to set immission values.
   *
   * @param rp The receiver point.
   * @param currentTimeBinEndTime The current time bin end time (usually first minute of an hour,
   *     e.g. 08:00:00), in seconds.
   */
