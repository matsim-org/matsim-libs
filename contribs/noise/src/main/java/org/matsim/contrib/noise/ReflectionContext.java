...
public class ReflectionContext {
  ...
  private static final Logger logger =
      LogManager.getLogger(org.matsim.contrib.noise.ShieldingContext.class);
  ...
  ReflectionContext(
      Collection<FeatureNoiseBarrierImpl> barriers, Config config, BarrierContext barrierContext) {
    this.barrierContext = barrierContext;
  }
  ...
  void setCurrentReceiver(NoiseReceiverPoint nrp) {
    ...
    final Collection<NoiseBarrier> candidates =
        barrierContext.query(
            new GeometryFactory().createPoint(receiver).buffer(200).getEnvelopeInternal());
    ...
    for (NoiseBarrier noiseBarrier : candidates) {
      ...
      final LineString exteriorRing =
          ((Polygon) noiseBarrier.getGeometry().getGeometry()).getExteriorRing();
  ...
  Set<LineSegment> findVisibleEdgesOfPolygon(
      List<LineSegment> polygonEdges, Coordinate coordinate) {
      ...
      if (hit(facade, originalLink)) {
        final AffineTransformation transformation =
            AffineTransformation.reflectionInstance(
                facade.p0.x, facade.p0.y, facade.p1.x, facade.p1.y);

        final Geometry transform = transformation.transform(temp);

        LineSegment mirrored =
            new LineSegment(transform.getCoordinates()[0], transform.getCoordinates()[1]);
        ...
        LineSegment linkSegment =
            new LineSegment(final1.lineIntersection(mirrored), final2.lineIntersection(mirrored));
  ...
  private Coordinate getReflectionSegment(
      Coordinate coordinate, LineSegment segment, double length) {
    ...
    if (intersection != null) {
      final double angle =
          Angle.toDegrees(Angle.angleBetween(segmentRight.p0, intersection, candidate.p1));
