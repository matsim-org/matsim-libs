...
class RLS19NoiseEmission implements NoiseEmission {
  ...
  public void calculateEmission(NoiseLink noiseLink) {
    ...
    int nPkw =
        (int)
            ((noiseLink.getAgentsEntering(pkw) * (noiseParams.getScaleFactor()))
                * (3600. / noiseParams.getTimeBinSizeNoiseComputation()));
    int nLkw1 =
        (int)
            ((noiseLink.getAgentsEntering(lkw1) * (noiseParams.getScaleFactor()))
                * (3600. / noiseParams.getTimeBinSizeNoiseComputation()));
    int nLkw2 =
        (int)
            ((noiseLink.getAgentsEntering(lkw2) * (noiseParams.getScaleFactor()))
                * (3600. / noiseParams.getTimeBinSizeNoiseComputation()));
    ...
    double emissionPlusPkw =
        calculateEmission(noiseLink, vPkw, vLkw1, vLkw2, nPkw + 1, nLkw1, nLkw2);
    double emissionPlusLkw1 =
        calculateEmission(noiseLink, vPkw, vLkw1, vLkw2, nPkw, nLkw1 + 1, nLkw2);
    double emissionPlusLkw2 =
        calculateEmission(noiseLink, vPkw, vLkw1, vLkw2, nPkw, nLkw1, nLkw2 + 1);
  ...
  double calculateEmission(
    ...
    double singlePkwEmission = calculateSingleVehicleEmission(noiseLink, pkw, vPkw);
    double singleLkw1Emission = calculateSingleVehicleEmission(noiseLink, lkw1, vLkw1);
    double singleLkw2Emission = calculateSingleVehicleEmission(noiseLink, lkw2, vLkw2);
  ...
  /**
   * Schallleistungspegel eines Fahrzeuges der Fahrzeuggruppe {@link RLS19VehicleType} mit
   * Geschwindigkeit v
   *
   ...
   * mit Geschwindigkeit v. Beschreibt die Schallemission des Fahrzeuges bei konstanter
   * Geschwindigkeit auf ebener, trockener Fahrbahn.
   *
   * <p>Base sound level of a vehicle of type {@link RLS19VehicleType} with velocity v. Describes
   * sound level of a vehicle driving with constant velocity on a flat and dry surface.
   *
   * @return emission in dB(A)
   */
  double calculateBaseVehicleTypeEmission(RLS19VehicleType vehicleType, double v) {
    double emission =
        vehicleType.getEmissionParameterA()
            + 10
                * Math.log10(
                    1
                        + Math.pow(
                            v / vehicleType.getEmissionParameterB(),
                            vehicleType.getEmissionParameterC()));
    return emission;
  }

  private double calculateSurfaceCorrection(
      RLS19VehicleType type, NoiseLink link, double velocity) {
  ...
  double getV(NoiseLink noiseLink, RLS19VehicleType type) {
    ...
    if (noiseParams.isUseActualSpeedLevel()) {
      if (noiseLink.getTravelTime_sec(type) == 0. || noiseLink.getAgentsLeaving(type) == 0) {
    ...
    if (v > freespeed) {
      throw new RuntimeException(
          v + " > " + freespeed + ". This should not be possible. Aborting...");
    }
