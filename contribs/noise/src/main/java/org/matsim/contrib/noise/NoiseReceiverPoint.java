...
/**
 * Extends the basic information of a receiver point towards data required for the computation of
 * noise.
 *
...
public class NoiseReceiverPoint extends ReceiverPoint {
  ...
  /**
   * Set to true once correction terms are set to re-use points over multiple MATSim runs without
   * the need to recalculate correction terms.
   */
  ...
  private Map<? extends NoiseVehicleType, TObjectDoubleMap<Id<Link>>>
      linkId2IsolatedImmissionPlusOneVehicle = null;
  ...
  void setDamageCostsPerAffectedAgentUnit(double damageCostsPerAffectedAgentUnit) {
  ...
  public String toString() {
    return "NoiseReceiverPoint [personId2actInfos="
        + personId2actInfos
  ...
  /**
   * @return the L_DEN (day (D) - evening (E) - night (N) level (L). A weighted average with added
   *     penalties for night and evening noise levels.
   */
  ...
  void setLinkId2IsolatedImmissionPlusOneVehicle(
      Map<? extends NoiseVehicleType, TObjectDoubleMap<Id<Link>>>
  ...
  Map<? extends NoiseVehicleType, TObjectDoubleMap<Id<Link>>>
      getLinkId2IsolatedImmissionPlusOneVehicle() {
