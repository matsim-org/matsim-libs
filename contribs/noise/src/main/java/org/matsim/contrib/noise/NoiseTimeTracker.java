...
/**
 * A handler which computes noise emissions, immisions, affected agent units and damages for each
 * receiver point and time interval. Throws noise damage events for each affected and causing agent.
 *
 * @author ikaddoura
 */
class NoiseTimeTracker
  ...
  NoiseTimeTracker(
      ...
      NoiseEmission emission,
      NoiseImmission immissionModule,
      NoiseDamageCalculation damageCalculation,
      NoiseVehicleIdentifier vehicleIdentifier,
  ...
  private void setRelevantLinkInfo() {
    ...
    for (NoiseReceiverPoint nrp : noiseContext.getGrid().getReceiverPoints().values()) {
      if (!nrp.isInitialized()) {
        ...
        potentialLinks.parallelStream()
            .forEach(
                linkId -> {
                  if (!(relevantLinkIds.contains(linkId))) {
                    Link candidateLink =
                        noiseContext.getScenario().getNetwork().getLinks().get(linkId);
                    double projectedDistance =
                        CoordUtils.distancePointLinesegment(
                            candidateLink.getFromNode().getCoord(),
                            candidateLink.getToNode().getCoord(),
                            nrp.getCoord());
                    if (projectedDistance < noiseParams.getRelevantRadius()) {
                      relevantLinkIds.add(linkId);
                      double correction =
                          immissionModule.calculateCorrection(
                              projectedDistance, nrp, candidateLink);
  ...
  private void processTimeBin() {
    ...
    if (printLog) {
      log.info(
          "# Computing noise for time interval "
              + Time.writeTime(this.noiseContext.getCurrentTimeBinEndTime(), Time.TIMEFORMAT_HHMMSS)
              + " #");
    }
    if (printLog) {
      log.info("##############################################");
    ...
    // Compute noise emissions, immissions, affected agent units and damages for the current time
    // interval.
    computeNoiseForCurrentTimeInterval();

    log.info(
        "# Finished time interval "
            + Time.writeTime(this.noiseContext.getCurrentTimeBinEndTime(), Time.TIMEFORMAT_HHMMSS)
            + " #");

    // Set the current time bin to the next one ( current time bin = current time bin + time bin
    // size ).
  ...
  private void updateActivityInformation() {
    ...
    for (NoiseReceiverPoint rp : this.noiseContext.getReceiverPoints().values()) {
      for (Id<Person> personId : rp.getPersonId2actInfos().keySet()) {
        rp.getPersonId2actInfos()
            .get(personId)
            .removeIf(personActivityInfo -> personActivityInfo.getEndTime() < (timeBinEnd));
      }
    }
  }
  ...
  private void computeNoiseForCurrentTimeInterval() {
    ...
    if (printLog) {
      log.info(
          "Calculating noise immissions, the number of affected agent units and noise damage costs.....");
    }
    calculateNoiseImmissionsAndDamages();

    if (writeOutput()) {
      log.info("Writing to files...");
      NoiseWriter.writeNoiseEmissionStatsPerHour(
          this.noiseContext, outputDirectory, useCompression, vehicleTypes);
      ...
      if (this.noiseContext.getNoiseParams().isComputeCausingAgents()) {
        for (NoiseVehicleType type : vehicleTypes) {
          NoiseWriter.writeLinkMarginalVehicleDamageInfoPerHour(
              noiseContext, outputDirectory, type);
        }
      }
    }
  }
  ...
  private void calculateNoiseImmissionsAndDamages() {
    ...
    this.noiseContext.getReceiverPoints().values().parallelStream()
        .forEach(
            rp -> {
  ...
  void computeFinalTimeIntervals() {
    while (this.noiseContext.getCurrentTimeBinEndTime()
        <= Math.max(
            24. * 3600.,
            this.noiseContext.getScenario().getConfig().qsim().getEndTime().orElse(0))) {
      processTimeBin();
    }
  }
  ...
  public void handleEvent(LinkEnterEvent event) {
    ...
    if (this.noiseContext.getIgnoredNetworkModeVehicleIDs().contains(event.getVehicleId())
        || this.noiseContext.getNotConsideredTransitVehicleIDs().contains(event.getVehicleId())) {
      ...
      if (this.noiseContext.getLinkId2vehicleId2lastEnterTime().get(event.getLinkId()) != null) {
        this.noiseContext
            ...
            .get(event.getLinkId())
            .put(event.getVehicleId(), event.getTime());
        ...
        this.noiseContext
            .getLinkId2vehicleId2lastEnterTime()
            .put(event.getLinkId(), vehicleId2enterTime);
      }
  ...
  public void handleEvent(TransitDriverStartsEvent event) {
    ...
    if (this.noiseContext.getNoiseParams().getBusIdIdentifierSet() == null
        || this.noiseContext.getNoiseParams().getBusIdIdentifierSet().size() == 0) {
      if (cWarn1 == 0) {
        log.warn(
            "Simulated public transit detected. "
      ...
      if (isBus) {
        ...
        if (cWarn2 == 0) {
          log.warn(
              "This noise computation approach does not account for transit vehicles other than road vehicles. "
                  + "Vehicle "
                  + event.getVehicleId()
                  + " belonging to transit line "
