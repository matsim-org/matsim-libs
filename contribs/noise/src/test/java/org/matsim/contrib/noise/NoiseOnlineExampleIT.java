...
public class NoiseOnlineExampleIT {
  ...
  public final void testOnTheFlyAggregationTerms() {
    ...
    NoiseReceiverPoint nrp1 =
        new NoiseReceiverPoint(Id.create("nrp1", ReceiverPoint.class), new Coord(0, 0));
    NoiseReceiverPoint nrp2 =
        new NoiseReceiverPoint(Id.create("nrp2", ReceiverPoint.class), new Coord(1000, 0));
    NoiseReceiverPoint nrp3 =
        new NoiseReceiverPoint(Id.create("nrp3", ReceiverPoint.class), new Coord(4000, 0));
    NoiseReceiverPoint nrp4 =
        new NoiseReceiverPoint(Id.create("nrp4", ReceiverPoint.class), new Coord(5000, 0));
    NoiseReceiverPoint nrp5 =
        new NoiseReceiverPoint(Id.create("nrp5", ReceiverPoint.class), new Coord(5000, -5000));
    NoiseReceiverPoint nrp6 =
        new NoiseReceiverPoint(Id.create("nrp6", ReceiverPoint.class), new Coord(0, -5000));

    NoiseReceiverPoint nrp7 =
        new NoiseReceiverPoint(Id.create("nrp7", ReceiverPoint.class), new Coord(1000, 1000));
    NoiseReceiverPoint nrp8 =
        new NoiseReceiverPoint(Id.create("nrp8", ReceiverPoint.class), new Coord(2000, 1));
    NoiseReceiverPoint nrp9 =
        new NoiseReceiverPoint(Id.create("nrp9", ReceiverPoint.class), new Coord(3000, 1000));
    NoiseReceiverPoint nrp10 =
        new NoiseReceiverPoint(Id.create("nrp10", ReceiverPoint.class), new Coord(4000, 1000));
    ...
    String pathToImmissionsFile =
        controler.getConfig().controller().getOutputDirectory()
            + "/ITERS/it."
            + controler.getConfig().controller().getLastIteration()
            + "/immissions/immission_processed.csv";
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        ldenByRp.put(
            ...
            Double.parseDouble(keys[idxLden]));
        l69ByRp.put(
            Id.create(keys[idxReceiverPointId], ReceiverPoint.class),
            Double.parseDouble(keys[idxL69]));
        l1619ByRp.put(
            Id.create(keys[idxReceiverPointId], ReceiverPoint.class),
            Double.parseDouble(keys[idxL1619]));
      }
    } catch (IOException e) {
      e.printStackTrace();
    }
    ...
    for (Map.Entry<Id<ReceiverPoint>, Double> entry : ldenByRp.entrySet()) {
      Assert.assertEquals(
          entry.getValue(), nrps.get(entry.getKey()).getLden(), MatsimTestUtils.EPSILON);
    }
    for (Map.Entry<Id<ReceiverPoint>, Double> entry : l69ByRp.entrySet()) {
      Assert.assertEquals(
          entry.getValue(), nrps.get(entry.getKey()).getL69(), MatsimTestUtils.EPSILON);
    }
    for (Map.Entry<Id<ReceiverPoint>, Double> entry : l1619ByRp.entrySet()) {
      Assert.assertEquals(
          entry.getValue(), nrps.get(entry.getKey()).getL1619(), MatsimTestUtils.EPSILON);
    }
  }
  ...
  public final void testNoiseListener() {
    ...
    Config config =
        ConfigUtils.loadConfig(
            testUtils.getPackageInputDirectory() + "config.xml", new NoiseConfigGroup());
    config.controller().setLastIteration(1);
    config.controller().setOutputDirectory(testUtils.getOutputDirectory());
    ...
    controler.addOverridingModule(
        new AbstractModule() {
          @Override
          public void install() {
            Multibinder<NoiseModule.NoiseListener> binder =
                Multibinder.newSetBinder(this.binder(), NoiseModule.NoiseListener.class);
            binder
                .addBinding()
                .toInstance(
                    new NoiseModule.NoiseListener() {
                      @Override
                      public void newRecord(XYTRecord record) {
                        log.warn(record);
                      }
                    });
          }
    ...
    ProcessNoiseImmissions processNoiseImmissions =
        new ProcessNoiseImmissions(
            workingDirectory, receiverPointsFile, noiseParameters.getReceiverPointGap());
    processNoiseImmissions.addListener(
        new NoiseModule.NoiseListener() {
          @Override
          public void newRecord(XYTRecord record) {
            log.warn(record);
