...
public class NoiseRLS19IT {
  ...
  public void testShielding() {
    ...
    Assert.assertEquals(
        "Wrong shielding value z!",
        34.463933081239965,
        shieldingCorrection.calculateShieldingCorrection(10, 15, 15, 15),
    ...
    FeatureNoiseBarrierImpl barrier =
        new FeatureNoiseBarrierImpl(Id.create("1", NoiseBarrier.class), geom, 10);
    ...
    Assert.assertEquals(
        "Wrong shielding correction!", 30.78517993490919, v, MatsimTestUtils.EPSILON);
  }
  ...
  public void testEmission() {
    ...
    final Link link =
        NetworkUtils.createAndAddLink(network, Id.createLinkId("1"), from, to, 400, 40 / 3.6, 0, 0);

    NoiseLink noiseLink = new NoiseLink(link.getId());

    RLS19NoiseEmission emission =
        new RLS19NoiseEmission(
            scenario, new RoadSurfaceContext(network), new DEMContextImpl(scenario.getConfig()));

    final double basePkwEmission =
        emission.calculateBaseVehicleTypeEmission(RLS19VehicleType.pkw, 40);
    Assert.assertEquals(
        "Wrong base pkw emission!", 97.70334139531323, basePkwEmission, MatsimTestUtils.EPSILON);

    double singleVehicleEmission =
        emission.calculateSingleVehicleEmission(noiseLink, RLS19VehicleType.pkw, 40);
    Assert.assertEquals(
        "Wrong single pkw emission!",
        97.70334139531323,
        singleVehicleEmission,
        MatsimTestUtils.EPSILON);

    final double vehicleTypePart = emission.calculateVehicleTypeNoise(1, 40, singleVehicleEmission);
    Assert.assertEquals(
        "Wrong pkw emission sum part!",
        1.4732421924637252E8,
        vehicleTypePart,
        MatsimTestUtils.EPSILON);

    final double noiseLinkEmission = emission.calculateEmission(noiseLink, 40, 40, 40, 1800, 0, 0);
    Assert.assertEquals(
        "Wrong noise link emission!",
        84.23546653306667,
        noiseLinkEmission,
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Wrong final noise link emission!",
        84.23546653306667,
        noiseLink.getEmission(),
        MatsimTestUtils.EPSILON);
  }
  ...
  public void testImmission() {
    ...
    final Link link =
        NetworkUtils.createAndAddLink(
            network, Id.createLinkId("link"), from, to, 10, 40 / 3.6, 0, 0);
    final Link link2 =
        NetworkUtils.createAndAddLink(
            network, Id.createLinkId("link2"), from, to, 10, 40 / 3.6, 0, 0);
    ...
    NoiseReceiverPoint rp =
        new NoiseReceiverPoint(Id.create("a", ReceiverPoint.class), new Coord(0, 0));

    RLS19NoiseImmission immission =
        new RLS19NoiseImmission(
            noiseContext, shieldingContext, new IntersectionContext(network), reflectionContext);
    ...
    Assert.assertEquals(
        "Wrong immission!", 73.77467715144601, currentImmission, MatsimTestUtils.EPSILON);
  }
}
