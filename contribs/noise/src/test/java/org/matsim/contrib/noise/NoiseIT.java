...
public class NoiseIT {
  ...
  public final void test1() {
    ...
    NoiseConfigGroup noiseParameters =
        (NoiseConfigGroup) scenario.getConfig().getModules().get(NoiseConfigGroup.GROUP_NAME);
    ...
    Assert.assertEquals(
        "wrong number of receiver points",
        16,
        noiseContext.getReceiverPoints().size(),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong coord for receiver point Id '10'",
        new Coord((double) 500, (double) 100).toString(),
        noiseContext
            .getReceiverPoints()
            .get(Id.create(10, ReceiverPoint.class))
            .getCoord()
            .toString());

    // test the allocation of activity coordinates to the nearest receiver point
    Assert.assertEquals(
        "wrong nearest receiver point Id for coord 300/300 (x/y)",
        "5",
        noiseContext
            .getGrid()
            .getActivityCoord2receiverPointId()
            .get(new Coord((double) 300, (double) 300))
            .toString());
    Assert.assertEquals(
        "wrong nearest receiver point Id for coord 150/150 (x/y)",
        "9",
        noiseContext
            .getGrid()
            .getActivityCoord2receiverPointId()
            .get(new Coord((double) 150, (double) 150))
            .toString());
    Assert.assertEquals(
        "wrong nearest receiver point Id for coord 100/100 (x/y)",
        "8",
        noiseContext
            .getGrid()
            .getActivityCoord2receiverPointId()
            .get(new Coord((double) 100, (double) 100))
            .toString());
    Assert.assertEquals(
        "wrong nearest receiver point Id for coord 500/500 (x/y)",
        "2",
        noiseContext
            .getGrid()
            .getActivityCoord2receiverPointId()
            .get(new Coord((double) 500, (double) 500))
            .toString());

    // test the allocation of relevant links to the receiver point
    Assert.assertEquals(
        "wrong relevant link for receiver point Id '15'",
        3,
        noiseContext
            .getReceiverPoints()
            .get(Id.create("15", Link.class))
            .getRelevantLinks()
            .size());
    //		Assert.assertEquals("wrong relevant link for receiver point Id '15'", 3,
    // noiseContext.getReceiverPoints().get(Id.create("15",
    // Link.class)).getLinkId2angleCorrection().size());

    // test the distance correction term
    //		Assert.assertEquals("wrong distance between receiver point Id '8' and link Id '1'",
    // 8.749854822140838, noiseContext.getReceiverPoints().get(Id.create("8",
    // ReceiverPoint.class)).getLinkId2distanceCorrection().get(Id.create("link0", Link.class)),
    // MatsimTestUtils.EPSILON);

    // test the angle correction term
    //		Assert.assertEquals("wrong immission angle correction for receiver point 14 and link1",
    // -0.8913405699036482, noiseContext.getReceiverPoints().get(Id.create("14",
    // ReceiverPoint.class)).getLinkId2angleCorrection().get(Id.create("link1", Link.class)),
    // MatsimTestUtils.EPSILON);

    double angle0 = 180.;
    double immissionCorrection0 = 10 * Math.log10((angle0) / (180));
    //		Assert.assertEquals("wrong immission angle correction for receiver point 12 and link5",
    // immissionCorrection0, noiseContext.getReceiverPoints().get(Id.create("12",
    // ReceiverPoint.class)).getLinkId2angleCorrection().get(Id.create("link5", Link.class)),
    // MatsimTestUtils.EPSILON);

    double angle = 65.39222026185993;
    double immissionCorrection = 10 * Math.log10((angle) / (180));
    //		Assert.assertEquals("wrong immission angle correction for receiver point 9 and link5",
    // immissionCorrection, noiseContext.getReceiverPoints().get(Id.create("9",
    // ReceiverPoint.class)).getLinkId2angleCorrection().get(Id.create("link5", Link.class)),
    // MatsimTestUtils.EPSILON);
    ...
    //		Assert.assertEquals("wrong immission angle correction for receiver point 8 and link5",
    // immissionCorrection2, noiseContext.getReceiverPoints().get(Id.create("8",
    // ReceiverPoint.class)).getLinkId2angleCorrection().get(Id.create("link5", Link.class)),
    // MatsimTestUtils.EPSILON);

    double angle3 = 84.28940686250034;
    double immissionCorrection3 = 10 * Math.log10((angle3) / (180));
    //		Assert.assertEquals("wrong immission angle correction for receiver point 8 and link1",
    // immissionCorrection3, noiseContext.getReceiverPoints().get(Id.create("8",
    // ReceiverPoint.class)).getLinkId2angleCorrection().get(Id.create("link1", Link.class)),
    // MatsimTestUtils.EPSILON);

    double angle4 = 180;
    double immissionCorrection4 = 10 * Math.log10((angle4) / (180));
    //		Assert.assertEquals("wrong immission angle correction for receiver point 8 and link0",
    // immissionCorrection4, noiseContext.getReceiverPoints().get(Id.create("8",
    // ReceiverPoint.class)).getLinkId2angleCorrection().get(Id.create("link0", Link.class)),
    // MatsimTestUtils.EPSILON);
  }

  // tests the noise emissions, immissions, considered agent units, damages (receiver points),
  // damages (per link), damages (per vehicle) based on the generated *.csv output
  ...
  private static void runTest2a(Config runConfig) {
    ...
    NoiseConfigGroup noiseParameters =
        (NoiseConfigGroup) config.getModules().get(NoiseConfigGroup.GROUP_NAME);
    ...
    Map<Id<ReceiverPoint>, List<Double>> consideredAgentsPerReceiverPoint =
        new HashMap<Id<ReceiverPoint>, List<Double>>();
    Map<String, Integer> idxFromKey = new ConcurrentHashMap<>();
    BufferedReader br;

    for (double currentTimeSlot : timeSlots) {

      pathToConsideredAgentUnitsFile =
          runDirectory
              + "noise-analysis/consideredAgentUnits/consideredAgentUnits_"
              + Double.toString(currentTimeSlot)
              + ".csv";
      ...
      try {
        ...
        int idxConsideredAgentUnits =
            idxFromKey.get(
                "Considered Agent Units "
                    + Time.writeTime(currentTimeSlot, Time.TIMEFORMAT_HHMMSS));

        while ((line = br.readLine()) != null) {

          keys = line.split(separator);
          if (!consideredAgentsPerReceiverPoint.containsKey(
              Id.create(keys[idxReceiverPointId], ReceiverPoint.class))) {
            consideredAgentsPerReceiverPoint.put(
                Id.create(keys[idxReceiverPointId], ReceiverPoint.class), new ArrayList<Double>());
          }

          consideredAgentsPerReceiverPoint
              .get(Id.create(keys[idxReceiverPointId], ReceiverPoint.class))
              .add(Double.parseDouble(keys[idxConsideredAgentUnits]));
        }
    ...
    for (double currentTimeSlot : timeSlots) {
      final Map<Id<ReceiverPoint>, Double> affectedPersonsPerReceiverPointTest =
          new HashMap<Id<ReceiverPoint>, Double>();
      ...
      for (Id<Person> personId : scenario.getPopulation().getPersons().keySet()) {
        ...
        for (Event e : eventsPerPersonId.get(personId)) {
          ...
          if (activityEnded) {
            ...
            double timeIntervalStart =
                currentTimeSlot - noiseParameters.getTimeBinSizeNoiseComputation();

            if ((actInfo.getStartTime() < currentTimeSlot)
                && (actInfo.getEndTime() >= timeIntervalStart)) {

              if ((actInfo.getStartTime() <= timeIntervalStart)
                  && actInfo.getEndTime() >= currentTimeSlot) {

                durationInThisInterval = noiseParameters.getTimeBinSizeNoiseComputation();

              } else if (actInfo.getStartTime() <= timeIntervalStart
                  && actInfo.getEndTime() <= currentTimeSlot) {
            ...
            double durationInThisIntervalMethod =
                actInfo.getDurationWithinInterval(
                    currentTimeSlot, noiseParameters.getTimeBinSizeNoiseComputation());

            Assert.assertEquals(
                "Durations of activities do not match!",
                durationInThisIntervalMethod,
                durationInThisInterval,
                MatsimTestUtils.EPSILON);

            double unitsThisPersonActivityInfo =
                durationInThisInterval / noiseParameters.getTimeBinSizeNoiseComputation();
            ...
            Coord coord =
                actInfo.getActivityType().equals("home")
                    ? ((Activity)
                            scenario
                                .getPopulation()
                                .getPersons()
      ...
      if (currentTimeSlot == endTime) {
        ...
        if (!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
          Assert.assertEquals(
              "Wrong number of affected persons at receiver point 16",
              2.3447222222222224,
              affectedPersonsPerReceiverPointTest.get(Id.create("16", ReceiverPoint.class)),
              MatsimTestUtils.EPSILON);
          // result changed after changing where agents are walking to in access/egress (July 20)

          Assert.assertEquals(
              "Wrong number of affected persons at receiver point 0",
              0.479722222222222,
              affectedPersonsPerReceiverPointTest.get(Id.create("0", ReceiverPoint.class)),
              MatsimTestUtils.EPSILON);
        } else {
          Assert.assertEquals(
              "Wrong number of affected persons at receiver point 16",
      ...
      for (Id<ReceiverPoint> receiverPointId : affectedPersonsPerReceiverPointTest.keySet()) {
        ...
        if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
          Assert.assertEquals(
              "Wrong number of affected persons", expected, actual, MatsimTestUtils.EPSILON);
        } else {
          Assert.assertEquals(
              "Wrong number of affected persons", expected, actual, MatsimTestUtils.EPSILON);
        }
    ...
    String pathToEmissionsFile =
        runDirectory + "noise-analysis/emissions/emission_" + Double.toString(endTime) + ".csv";
    ...
    try {
      ...
      int idxNoiseEmission =
          idxFromKey.get("Noise Emission " + Time.writeTime(endTime, Time.TIMEFORMAT_HHMMSS));

      while ((line = br.readLine()) != null) {

        keys = line.split(separator);
        emissionsPerLink.put(
            Id.create(keys[idxLinkId], Link.class), Double.parseDouble(keys[idxNoiseEmission]));
      }
    ...
    for (NoiseEventCaused event :
        noiseCalculation.getTimeTracker().getDamageCalculation().getNoiseEventsCaused()) {
    ...
    for (Id<Link> linkId : amountOfVehiclesPerLink.keySet()) {
      ...
      Assert.assertEquals(
          "Wrong amount of emission!",
          noiseEmission,
          emissionsPerLink.get(linkId),
          MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Wrong amount of emission!",
        56.4418948379387,
        noiseEmissionsPerLink.get(Id.create("link2", Link.class)),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong amount of emission!",
        86.4302864851097,
        noiseEmissionsPerLink.get(Id.create("linkA5", Link.class)),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong amount of emission!",
        0.,
        noiseEmissionsPerLink.get(Id.create("link4", Link.class)),
        MatsimTestUtils.EPSILON);
    ...
    String pathToImmissionsFile =
        runDirectory + "noise-analysis/immissions/immission_" + Double.toString(endTime) + ".csv";

    Map<Id<ReceiverPoint>, Double> immissionPerReceiverPointId =
        new HashMap<Id<ReceiverPoint>, Double>();
    ...
    try {
      ...
      int idxReceiverPointId = idxFromKey.get("Receiver Point Id");
      int idxImmission =
          idxFromKey.get("Immission " + Time.writeTime(endTime, Time.TIMEFORMAT_HHMMSS));

      while ((line = br.readLine()) != null) {

        keys = line.split(separator);
        immissionPerReceiverPointId.put(
            Id.create(keys[idxReceiverPointId], ReceiverPoint.class),
            Double.parseDouble(keys[idxImmission]));
      }
    ...
    final RLS90NoiseImmission rls90NoiseImmission =
        new RLS90NoiseImmission(noiseCalculation.getNoiseContext(), null);
    ...
    for (NoiseReceiverPoint rp : noiseCalculation.getNoiseContext().getReceiverPoints().values()) {
      ...
      Assert.assertEquals(
          "Wrong amount of immission!",
          rls90NoiseImmission.calculateResultingNoiseImmission(linkId2IsolatedImmission.values()),
          immissionPerReceiverPointId.get(rp.getId()),
          MatsimTestUtils.EPSILON);
    }
    ...
    Assert.assertEquals(
        "Wrong amount of immission!",
        77.25915342419277,
        immissionPerReceiverPointId.get(Id.create("15", ReceiverPoint.class)),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong amount of immission!",
        67.9561670074151,
        immissionPerReceiverPointId.get(Id.create("31", ReceiverPoint.class)),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong amount of immission!",
        0.,
        immissionPerReceiverPointId.get(Id.create("0", ReceiverPoint.class)),
        MatsimTestUtils.EPSILON);
    ...
    String pathToDamagesFile =
        runDirectory
            + "noise-analysis/damages_receiverPoint/damages_receiverPoint_"
            + Double.toString(endTime)
            + ".csv";

    Map<Id<ReceiverPoint>, Double> damagesPerReceiverPointId =
        new HashMap<Id<ReceiverPoint>, Double>();
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        keys = line.split(separator);
        damagesPerReceiverPointId.put(
            Id.create(keys[idxReceiverPointId], ReceiverPoint.class),
            Double.parseDouble(keys[idxDamages]));
      }
    ...
    for (ReceiverPoint rp : noiseCalculation.getNoiseContext().getReceiverPoints().values()) {
      ...
      Assert.assertEquals(
          ...
          NoiseDamageCalculation.calculateDamageCosts(
              ...
              affectedAgentUnits,
              endTime,
              noiseParameters.getAnnualCostRate(),
              noiseParameters.getTimeBinSizeNoiseComputation()),
          damagesPerReceiverPointId.get(rp.getId()),
          MatsimTestUtils.EPSILON);
    }
    ...
    if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
      Assert.assertEquals(
          "Wrong damage!",
          0.0664164095284536,
          damagesPerReceiverPointId.get(Id.create("16", ReceiverPoint.class)),
          MatsimTestUtils.EPSILON);
    } else {
      Assert.assertEquals(
          "Wrong damage!",
    ...
    Assert.assertEquals(
        "Wrong damage!",
        0.,
        damagesPerReceiverPointId.get(Id.create("0", ReceiverPoint.class)),
        MatsimTestUtils.EPSILON);
    ...
    // relevant link IDs
    // link2: noise contribution: 50.45464287410944 -->  share: 0.01202333 --> damage costs:
    // 0.00079855
    // linkA5: noise contribution: 69.60186152606298 ---> share: 0.98797667 --> damage costs:
    // 0.06561786
    // linkB5: noise contribution: 0

    line = null;

    String pathToDamageLinkFile =
        runDirectory
            + "noise-analysis/average_damages_link/average_damages_link_"
            + Double.toString(endTime)
            + ".csv";
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        keys = line.split(separator);
        damagesPerlinkId.put(
            Id.create(keys[idxLinkId], Link.class), Double.parseDouble(keys[idxDamages]));
      }
    ...
    if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
      Assert.assertEquals(
          "Wrong link's damage contribution!",
          0.00079854651258,
          damagesPerlinkId.get(Id.create("link2", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong link's damage contribution!",
          0.06561786301587,
          damagesPerlinkId.get(Id.create("linkA5", Link.class)),
          MatsimTestUtils.EPSILON);
    } else {
      Assert.assertEquals(
          "Wrong link's damage contribution!",
          7.957184286235844E-4,
          damagesPerlinkId.get(Id.create("link2", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong link's damage contribution!",
    ...
    Assert.assertEquals(
        "Wrong link's damage contribution!",
        0.,
        damagesPerlinkId.get(Id.create("linkB5", Link.class)),
        MatsimTestUtils.EPSILON);
    ...
    String pathToDamageLinkCar =
        runDirectory
            + "noise-analysis/average_damages_link_car/average_damages_link_car_"
            + Double.toString(endTime)
            + ".csv";
    ...
    try {
      ...
      int idxDamages =
          idxFromKey.get(
              "Average damages per car " + Time.writeTime(endTime, Time.TIMEFORMAT_HHMMSS));
    ...
    if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
      Assert.assertEquals(
          "Wrong damage per car per link!",
          0.00079854651258 / 2.0,
          damagesPerCar.get(Id.create("link2", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong damage per car per link!",
          0.06561786301587 / 2.0,
          damagesPerCar.get(Id.create("linkA5", Link.class)),
          MatsimTestUtils.EPSILON);
    } else {
      Assert.assertEquals(
          "Wrong damage per car per link!",
          3.978592143117922E-4,
          damagesPerCar.get(Id.create("link2", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong damage per car per link!",
    ...
    Assert.assertEquals(
        "Wrong damage per car per link!",
        0.,
        damagesPerCar.get(Id.create("linkB5", Link.class)),
        MatsimTestUtils.EPSILON);

    line = null;

    String pathToMarginalDamageLinkCar =
        runDirectory
            + "noise-analysis/marginal_damages_link_car/marginal_damages_link_car_"
            + Double.toString(endTime)
            + ".csv";
    ...
    try {
      ...
      int idxDamages =
          idxFromKey.get(
              "Marginal damages per car " + Time.writeTime(endTime, Time.TIMEFORMAT_HHMMSS));

      while ((line = br.readLine()) != null) {

        keys = line.split(separator);
        marginaldamagesPerCar.put(
            Id.create(keys[idxlinkId], Link.class), Double.parseDouble(keys[idxDamages]));
      }
    ...
    if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
      Assert.assertEquals(
          "Wrong damage per car per link!",
          0.00011994155845965193,
          marginaldamagesPerCar.get(Id.create("link2", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong damage per car per link!",
          0.008531432493391652,
          marginaldamagesPerCar.get(Id.create("linkA5", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong damage per car per link!",
          3.440988380343235E-8,
          marginaldamagesPerCar.get(Id.create("linkB5", Link.class)),
          MatsimTestUtils.EPSILON);
    } else {
      Assert.assertEquals(
          "Wrong damage per car per link!",
          1.1951678071236982E-4,
          marginaldamagesPerCar.get(Id.create("link2", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong damage per car per link!",
          0.008501218474617364,
          marginaldamagesPerCar.get(Id.create("linkA5", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong damage per car per link!",
          3.428802136662412E-8,
          marginaldamagesPerCar.get(Id.create("linkB5", Link.class)),
          MatsimTestUtils.EPSILON);
    }
    ...
    for (NoiseEventCaused event :
      ...
      if (event.getTimeBinEndTime() == 11 * 3600.
          && event.getLinkId().toString().equals(Id.create("linkA5", Link.class).toString())
          && event
              .getCausingVehicleId()
              .toString()
              .equals((Id.create("person_car_test1", Vehicle.class).toString()))) {
        if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
          Assert.assertEquals(
              "wrong cost per car for the given link and time interval",
              0.0328089315079348,
              event.getAmount(),
              MatsimTestUtils.EPSILON);
        } else {
        ...
        counter++;
      } else if (event.getTimeBinEndTime() == 11 * 3600.
          && event.getLinkId().toString().equals(Id.create("linkA5", Link.class).toString())
          && event
              .getCausingVehicleId()
              .toString()
              .equals((Id.create("person_car_test2", Vehicle.class).toString()))) {
        if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
          Assert.assertEquals(
              "wrong cost per car for the given link and time interval",
              0.0328089315079348,
        ...
        counter++;
          ...
          && event.getLinkId().toString().equals(Id.create("link2", Link.class).toString())
          && event
              .getCausingVehicleId()
              .toString()
              .equals((Id.create("person_car_test1", Vehicle.class).toString()))) {
        ...
        Assert.assertEquals(
            "There should either be no further events, or the amount should be zero.",
            0.,
            event.getAmount(),
            MatsimTestUtils.EPSILON);
      }
    ...
    int counter2 = 0;
    for (NoiseEventAffected event :
        noiseCalculation.getTimeTracker().getDamageCalculation().getNoiseEventsAffected()) {
      tested2 = true;
      ...
      if (event.getTimeBinEndTime() == 11 * 3600.
          && event
              .getrReceiverPointId()
              .toString()
              .equals(Id.create("16", ReceiverPoint.class).toString())
          && event
              .getAffectedAgentId()
              .toString()
              .equals((Id.create("person_car_test1", Person.class).toString()))
          && event.getActType().equals("work")) {
        if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
          Assert.assertEquals(
              "wrong cost per car for the given link and time interval",
              0.020745817449213576,
              event.getAmount(),
              MatsimTestUtils.EPSILON);
        } else {
          Assert.assertEquals(
        ...
        counter2++;
          ...
          && event
              .getAffectedAgentId()
              .toString()
              .equals((Id.create("person_car_test2", Person.class).toString()))
          && event.getActType().equals("work")) {
        if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
          Assert.assertEquals(
              "wrong cost per car for the given link and time interval",
              0.017444990107520864,
              event.getAmount(),
              MatsimTestUtils.EPSILON);
        } else {
          Assert.assertEquals(
              "wrong cost per car for the given link and time interval",
              0.017327383429596242,
              event.getAmount(),
              MatsimTestUtils.EPSILON);
        }
        counter2++;
          ...
          && event
              .getAffectedAgentId()
              .toString()
              .equals((Id.create("person_car_test3", Person.class).toString()))
          && event.getActType().equals("home")) {
        if (!!runConfig.routing().getAccessEgressType().equals(AccessEgressType.none)) {
          Assert.assertEquals(
              "wrong cost per car for the given link and time interval",
              0.028225601971719153,
              event.getAmount(),
              MatsimTestUtils.EPSILON);
        } else {
          Assert.assertEquals(
  ...
  public final void test2b() {
    ...
    int lastIteration = -1;
    {
      ...
      // I made test2a test both versions, but I don't really want to do that work again myself.
      // kai, feb'16
    ...
    noiseParameters.setScaleFactor(1.);
    noiseParameters.setNoiseAllocationApproach(
        NoiseConfigGroup.NoiseAllocationApproach.MarginalCost);
    noiseParameters.setUseActualSpeedLevel(false);
    noiseParameters.setAllowForSpeedsOutsideTheValidRange(true);
    Scenario scenario = ScenarioUtils.loadScenario(config);
  ...
  public final void test2c() {
    ...
    try {
      ...
      Assert.assertEquals(
          "Wrong amount of emission!",
          56.4418948379387,
          emissionsPerLink.get(Id.create("link2", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong amount of emission!",
          77.3994680630406,
          emissionsPerLink.get(Id.create("linkA5", Link.class)),
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Wrong amount of emission!",
          0.,
          emissionsPerLink.get(Id.create("link4", Link.class)),
          MatsimTestUtils.EPSILON);
  ...
  public final void test3() {
    ...
    Assert.assertEquals(
        "Error in deviation term for speed correction (car)",
        expectedEcar,
        eCar,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Error in deviation term for speed correction (car)",
        expectedEcar,
        RLS90NoiseEmission.calculateLCar(vCar),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Error in deviation term for speed correction (hgv)",
        expectedEhgv,
        eHgv,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Error in deviation term for speed correction (hgv)",
        expectedEhgv,
        RLS90NoiseEmission.calculateLHdv(vHgv),
        MatsimTestUtils.EPSILON);
    ...
    for (double nHgvs = 0; nHgvs < 3; nHgvs++) {
      ...
      for (double nCars = 0; nCars < 3; nCars++) {
        ...
        Assert.assertEquals(
            "Error while calculating Mittelungspegel for "
                + nCars
                + " car(s) and "
                + nHgvs
                + " hgv(s)!",
            expectedMittelungspegel,
            mittelungspegel,
            MatsimTestUtils.EPSILON);
        Assert.assertEquals(
            "Error while calculating Mittelungspegel for "
                ...
                + " hgv(s)!",
            expectedMittelungspegel,
            RLS90NoiseEmission.calculateMittelungspegelLm(n, p),
            MatsimTestUtils.EPSILON);
        ...
        double speedCorrection =
            expectedEcar
                ...
                + 10
                    * Math.log10(
                        (100 + (Math.pow(10, 0.1 * (expectedEhgv - expectedEcar)) - 1) * pInPercent)
                            / (100 + 8.23 * pInPercent));
        ...
        Assert.assertEquals(
            "Error while calculating speed correction term for p = " + p + "!",
            expectedSpeedCorrection,
            speedCorrection,
            MatsimTestUtils.EPSILON);
        Assert.assertEquals(
            "Error while calculating speed correction term for p = " + p + "!",
            expectedSpeedCorrection,
            RLS90NoiseEmission.calculateGeschwindigkeitskorrekturDv(vCar, vHgv, p),
            MatsimTestUtils.EPSILON);
      }
    }
    ...
    for (double distance = 5.; distance <= 140; distance += 45.) {
      ...
      double distanceCorrection =
          15.8 - 10 * Math.log10(distance) - 0.0142 * Math.pow(distance, 0.9);
      ...
      Assert.assertEquals(
          "Error while calculating distance correction term!",
          expectedDistanceCorrection,
          distanceCorrection,
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Error while calculating distance correction term!",
          expectedDistanceCorrection,
          RLS90NoiseImmission.calculateDistanceCorrection(distance),
          MatsimTestUtils.EPSILON);
    }
    ...
    for (double angle = 45; angle <= 360; angle += 45) {
      ...
      Assert.assertEquals(
          "Error while calculating angle correction term!",
          expectedAngleCorrection,
          angleCorrection,
          MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "Error while calculating angle correction term!",
          expectedAngleCorrection,
          RLS90NoiseImmission.calculateAngleCorrection(angle),
          MatsimTestUtils.EPSILON);
    }
    ...
    double distanceCorrection1 =
        15.8 - 10 * Math.log10(distance1) - 0.0142 * Math.pow(distance1, 0.9);
    double angleCorrection1 = 10 * Math.log10(angle1 / 180);
    double distanceCorrection2 =
        15.8 - 10 * Math.log10(distance2) - 0.0142 * Math.pow(distance2, 0.9);
    double angleCorrection2 = 10 * Math.log10(angle2 / 180);
    double distanceCorrection3 =
        15.8 - 10 * Math.log10(distance3) - 0.0142 * Math.pow(distance3, 0.9);
    ...
    Assert.assertEquals(
        "Error in noise immission calculation!",
        expectedResultingNoiseImmission,
        resultingNoiseImmission,
        MatsimTestUtils.EPSILON);
    ...
    double costsDay =
        resultingNoiseImmission > thresholdDay
            ? annualCostRate
                * 3600
                / (365 * 24)
                * nPersons
                / 3600
                * Math.pow(2, 0.1 * (resultingNoiseImmission - thresholdDay))
            : 0.;
    double costsEvening =
        resultingNoiseImmission > thresholdEvening
            ? annualCostRate
                * 3600
                / (365 * 24)
                * nPersons
                / 3600
                * Math.pow(2, 0.1 * (resultingNoiseImmission - thresholdEvening))
            : 0.;
    double costsNight =
        resultingNoiseImmission > thresholdNight
            ? annualCostRate
                * 3600
                / (365 * 24)
                * nPersons
                / 3600
                * Math.pow(2, 0.1 * (resultingNoiseImmission - thresholdNight))
            : 0.;
    ...
    Assert.assertEquals(
        "Error in damage calculation!", expectedCostsDay, costsDay, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Error in damage calculation!",
        expectedCostsDay,
        NoiseDamageCalculation.calculateDamageCosts(
            resultingNoiseImmission, nPersons, 7. * 3600, annualCostRate, 3600.),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Error in damage calculation!",
        expectedCostsEvening,
        costsEvening,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Error in damage calculation!",
        expectedCostsEvening,
        NoiseDamageCalculation.calculateDamageCosts(
            resultingNoiseImmission, nPersons, 19. * 3600, annualCostRate, 3600.),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Error in damage calculation!", expectedCostsNight, costsNight, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Error in damage calculation!",
        expectedCostsNight,
        NoiseDamageCalculation.calculateDamageCosts(
            resultingNoiseImmission, nPersons, 23. * 3600, annualCostRate, 3600.),
        MatsimTestUtils.EPSILON);
  }
  ...
  public final void test4() {
    ...
    double mittelungspegel = RLS90NoiseEmission.calculateMittelungspegelLm(n, p);
    Assert.assertEquals(
        "Wrong mittelungspegel for n=" + n + " and p=" + p + "!",
        69.22567453336540,
        mittelungspegel,
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Wrong LCar for vCar=" + vCar + "!", 27.70000000425900, lCar, MatsimTestUtils.EPSILON);

    double lHGV = RLS90NoiseEmission.calculateLHdv(vHGV);
    Assert.assertEquals(
        "Wrong LHGV for vHGV=" + vHGV + "!", 6.60145932205085, lHGV, MatsimTestUtils.EPSILON);
    ...
    double mittelungspegelPlusOneCar =
        RLS90NoiseEmission.calculateMittelungspegelLm(nPlusOneCar, pPlusOneCar);
    double dVPlusOneCar =
        RLS90NoiseEmission.calculateGeschwindigkeitskorrekturDv(vCar, vHGV, pPlusOneCar);
    double emissionPlusOneCar = mittelungspegelPlusOneCar + dVPlusOneCar;
    Assert.assertEquals(
        "Wrong emission!", 58.4896140186478, emissionPlusOneCar, MatsimTestUtils.EPSILON);

    // plus one HGV

    int nPlusOneHGV = (nCar + nHGV + 1) * 10;
    double pPlusOneHGV = ((double) (nHGV + 1) / (double) (nCar + (nHGV + 1)));
    double mittelungspegelPlusOneHGV =
        RLS90NoiseEmission.calculateMittelungspegelLm(nPlusOneHGV, pPlusOneHGV);
    double dVPlusOneHGV =
        RLS90NoiseEmission.calculateGeschwindigkeitskorrekturDv(vCar, vHGV, pPlusOneHGV);
    double emissionPlusOneHGV = mittelungspegelPlusOneHGV + dVPlusOneHGV;
    Assert.assertEquals(
        "Wrong emission!", 58.4529399949061, emissionPlusOneHGV, MatsimTestUtils.EPSILON);
  }
  ...
  public final void test5() {
    ...
    double lCar = RLS90NoiseEmission.calculateLCar(vCar);
    Assert.assertEquals(
        "Wrong LCar for vCar=" + vCar + "!", 28.54933574936720, lCar, MatsimTestUtils.EPSILON);

    double lHGV = RLS90NoiseEmission.calculateLHdv(vHGV);
    Assert.assertEquals(
        "Wrong LHGV for vHGV=" + vHGV + "!", 41.56401568399580, lHGV, MatsimTestUtils.EPSILON);
    ...
    double emissionPlusOneCar = mittelungspegelPlusOneCar + dVPlusOneCar;
    Assert.assertEquals(
        "Wrong emission!", 61.5580658162266, emissionPlusOneCar, MatsimTestUtils.EPSILON);
    ...
    double emissionPlusOneHGV = mittelungspegelPlusOneHGV + dVPlusOneHGV;
    Assert.assertEquals(
        "Wrong emission!", 61.9518310976080, emissionPlusOneHGV, MatsimTestUtils.EPSILON);
  }
}
