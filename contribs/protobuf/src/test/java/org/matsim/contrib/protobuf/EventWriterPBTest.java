...
public class EventWriterPBTest {
  ...
  public void writer() throws IOException {
    ...
    // (one could argue that reading should be a separate test.  On the other hand, write-read tests
    // and read-write tests are quite normal and quite
    // plausible.  This one here, however, is not complete; it should check if we can reconstruct
    // the original material.)

    final FileInputStream fileInputStream =
        new FileInputStream(utils.getOutputDirectory() + "/events.pb");
    {
      PBFileHeader header = PBFileHeader.parseDelimitedFrom(fileInputStream);
      for (Map.Entry<Descriptors.FieldDescriptor, Object> entry :
          header.getAllFields().entrySet()) {
    ...
    // the following does not read any additional material.  I am, in fact, a bit surprised that it
    // does not cause an error.  Looks like it is
    // possible to read non-existing containers as "empty" containers.  However, reading the header
    // twice (as above) causes this one here to fail.  --???
    //        {
    //            ProtoEvents.EventBatch batch = ProtoEvents.EventBatch.parseDelimitedFrom(
    // fileInputStream );
    //            for( ProtoEvents.Event event : batch.getEventsList() ){
    //                System.out.println( "event=" + event );
    //            }
  ...
  public void convertEvent() {
    ...
    assertThat(EventWriterPB.convertEvent(event))
        ...
        .has(
            new Condition<>(
                m -> !m.containsKey(Event.ATTRIBUTE_TIME) && !m.containsKey(Event.ATTRIBUTE_X),
                "does not contain common attributes"));

    assertThat(
            EventWriterPB.convertEvent(
                new PersonDepartureEvent(
                    15.0, Id.createPersonId(15), Id.createLinkId(20), "test", "test")))
        ...
        .has(
            new Condition<>(
                e -> e.getPersonId().getId().equals("15") && e.getLinkId().getId().equals("20"),
                "Ids correct"));
  }
  ...
  public void convertId() {
    ...
    ProtoEvents.LinkEnterEvent msg =
        ProtoEvents.LinkEnterEvent.newBuilder().setLinkId(EventWriterPB.convertId(id)).build();

    Assertions.assertThat(msg.hasLinkId());

    msg = ProtoEvents.LinkEnterEvent.newBuilder().setLinkId(EventWriterPB.convertId(null)).build();

    Assertions.assertThat(!msg.hasLinkId());

    // check that empty string is a valid id
    msg = ProtoEvents.LinkEnterEvent.newBuilder().setLinkId(ProtoId.newBuilder().setId("")).build();
