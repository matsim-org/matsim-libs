...
public class EventWriterPB implements EventWriter, BasicEventHandler {
  ...
  public void closeFile() {
    try {
      if (batch.getEventsCount() > 0) writeBatch();
  ...
  public void handleEvent(Event event) {
    try {
      ...
      if (batch.getEventsCount() == BATCH_SIZE) writeBatch();
    } catch (IOException e) {
      throw new UncheckedIOException(e);
    }
  }
  ...
  public static ProtoEvents.Event convertEvent(Event event) {
    ...
    ProtoEvents.Event.Builder builder = ProtoEvents.Event.newBuilder().setTime(event.getTime());
    ...
    if (event instanceof ActivityEndEvent) {
      ...
      builder
          .getVehicleEntersTrafficBuilder()
          ...
          .setRelativePositionOnLink(
              ((VehicleEntersTrafficEvent) event).getRelativePositionOnLink());
    } else if (event instanceof VehicleLeavesTrafficEvent) {
      builder
          .getVehicleLeavesTrafficBuilder()
          ...
          .setRelativePositionOnLink(
              ((VehicleLeavesTrafficEvent) event).getRelativePositionOnLink());
    } else {
      ...
      attrs
          ...
          .removeIf(
              key ->
                  key == Event.ATTRIBUTE_X
                      || key == Event.ATTRIBUTE_Y
                      || key == Event.ATTRIBUTE_TIME
                      || key == Event.ATTRIBUTE_TYPE);

      // covers generic and all unknown events
      builder.getGenericBuilder().setType(event.getEventType()).putAllAttrs(attrs);
    }
