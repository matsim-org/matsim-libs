...
public final class MasterController implements ControlerI, MatsimServices, AllowsConfiguration {
	...
	private List<AbstractModule> modules = Collections.singletonList(new MasterDefaultsModule());
...
//			Logger.getRootLogger().info("Usage: Controler config-file [dtd-file]");
			...
			final MasterController controler = new MasterController(args);
			controler.run();
		}
//		System.exit(0);
	}
	 ...
	 */
	public MasterController(final String[] args) {
		...
		if (this.injector != null) {
			...
			return new EventsManager() {
				...
				public void processEvent(Event event) {
					MasterController.this.injector.getInstance(EventsManager.class).processEvent(event);
				}
	...
	@Override
	public final MasterController addOverridingModule(AbstractModule abstractModule ) {
		if (this.injectorCreated) {
	...
	@Override
	public final MasterController addOverridingQSimModule(AbstractQSimModule qsimModule ) {
		if (this.injectorCreated) {
			throw new RuntimeException("Too late for configuring the Controler. This can only be done before calling run.");
		}
	...
	@Override
	public final MasterController addQSimModule(AbstractQSimModule qsimModule) {
		this.addOverridingModule(new AbstractModule() {
			@Override
			public void install() {
     ...
     */
	@Override
	public final MasterController configureQSimComponents(QSimComponentsConfigurator configurator) {
		this.addOverridingModule(new AbstractModule() {
			@Override
			public void install() {
