...
public final class WorkerController implements ControlerI, MatsimServices, AllowsConfiguration {
	...
	private List<AbstractModule> modules = Collections.singletonList(new WorkerDefaultsModule());
	...
	public static void main(final String[] args) {
		if ((args == null) || (args.length == 0)) {
			Logger.getRootLogger().info("No argument given!");
			Logger.getRootLogger().info("Usage: Controler config-file [dtd-file]");
		} else {
			final WorkerController controler = new WorkerController(args);
			controler.run();
		}
//		System.exit(0);
	}
	 ...
	 */
	public WorkerController(final String[] args) {
	...
	/**
	 * Gives access to a {@link TripRouter} instance.
	 ...
	 *
	 * See {@link TripRouter} for more information and pointers to examples.
	 */
	@Override
		...
		if (this.injector != null) {
			...
			return new EventsManager() {
				...
				public void processEvent(Event event) {
					WorkerController.this.injector.getInstance(EventsManager.class).processEvent(event);
				}
	...
	/**
	 * @return Returns the {@link StrategyManager}
	...
	@Override
	public final WorkerController addOverridingModule(AbstractModule abstractModule ) {
		if (this.injectorCreated) {
	...
	@Override
	public final WorkerController addOverridingQSimModule(AbstractQSimModule qsimModule ) {
		if (this.injectorCreated) {
			throw new RuntimeException("Too late for configuring the Controler. This can only be done before calling run.");
		}
	...
	@Override
	public final WorkerController addQSimModule(AbstractQSimModule qsimModule) {
		this.addOverridingModule(new AbstractModule() {
			@Override
			public void install() {
     ...
     */
	@Override
	public final WorkerController configureQSimComponents(QSimComponentsConfigurator configurator) {
		this.addOverridingModule(new AbstractModule() {
			@Override
			public void install() {
