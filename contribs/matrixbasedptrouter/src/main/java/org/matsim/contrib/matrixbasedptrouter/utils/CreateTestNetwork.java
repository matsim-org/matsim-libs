...
public final class CreateTestNetwork {
  ...
  /**
   * This method creates 4 pt stops for the test network from createTestNetwork(). The information
   * about the coordinates will be written to a csv file. The 4 pt stops are located as a square in
   * the coordinate plane with a side length of 180 meter (see the sketch below).
   *
  ...
  /**
   * This methods creates a csv file with informations about pt travel times and pt distances for
   * the test network from createTestNetwork(). We set the pt travel time between all pairs of pt
   * stops to 100 seconds, except pairs of same pt stops where the travel time is 0 seconds. We set
   * the pt distance between all pairs of pt stops to 100 meter, except pairs of same pt stops where
   * the distance is 0 meter. Because the data in the csv file does not need an entity, you can use
   * the same csv file for both informations.
   *
      ...
      for (int origin = 1; origin <= 4; origin++) {
        for (int destination = 1; destination <= 4; destination++) {
          if (origin == destination) {
            ...
            // set a dummy travel time/distance of 100s or 100m between different origin and
            // destination pt stops
  ...
  /**
   * This method creates 4 facilities for the test network from createTestNetwork(). The distance
   * between each facility and the nearest pt stop is 50 meter (see the sketch below).
   *
  ...
  /**
   * creating a test network the path 1,2,4 has a total length of 1000m with a free speed travel
   * time of 10m/s the second path 1,3,4 has a total length of 100m but only a free speed travel
   * time of 0.1m/s
   */
  public static Network createTriangularNetwork() {
    ...
    Node node2 =
        NetworkUtils.createAndAddNode(
            network, Id.create(2, Node.class), new Coord((double) 50, (double) 100));
    Node node3 =
        NetworkUtils.createAndAddNode(
            network, Id.create(3, Node.class), new Coord((double) 50, (double) 0));
    Node node4 =
        NetworkUtils.createAndAddNode(
    ...
    // add links
    NetworkUtils.createAndAddLink(
        network, Id.create(1, Link.class), fromNode, toNode, 500.0, 10.0, 3600.0, (double) 1);
    final Node fromNode1 = node2;
    final Node toNode1 = node4;
    NetworkUtils.createAndAddLink(
        network, Id.create(2, Link.class), fromNode1, toNode1, 500.0, 10.0, 3600.0, (double) 1);
    final Node fromNode2 = node1;
    final Node toNode2 = node3;
    NetworkUtils.createAndAddLink(
        network, Id.create(3, Link.class), fromNode2, toNode2, 50.0, 0.1, 3600.0, (double) 1);
    final Node fromNode3 = node3;
    final Node toNode3 = node4;
    NetworkUtils.createAndAddLink(
        network, Id.create(4, Link.class), fromNode3, toNode3, 50.0, 0.1, 3600.0, (double) 1);
