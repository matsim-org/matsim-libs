...
public final class MatrixBasedPtRouterConfigGroup extends ReflectiveConfigGroup {
  ...
  public static final String PT_STOPS =
      "ptStopsFile"; // tnicolai: originally, this was named "ptStops" in the matsim config. old
  // config files need to be adjusted
  public static final String USING_PT_STOPS = "usingPtStops";
  public static final String PT_TRAVEL_TIMES =
      "ptTravelTimesFile"; // tnicolai: originally, this was named "ptTravelTimes" in the matsim
  // config. old config files need to be adjusted
  public static final String PT_TRAVEL_DISTANCES =
      "ptTravelDistancesFile"; // tnicolai: originally, this was named "ptTravelDistances" in the
  // matsim config. old config files need to be adjusted
  public static final String PT_TRAVEL_TIMES_AND_DISTANCES_SWITCH =
      "usingTravelTimesAndDistances"; // tnicolai: originally, this was named
  // "useTravelTimesAndDistances" in the matsim config. old
  // config files need to be adjusted
  ...
  protected void checkConsistency(Config config) {
    ...
    if (isUsingPtStops()) {
      log.info(
          MatrixBasedPtRouterConfigGroup.USING_PT_STOPS
              + " switch is set to true. Trying to find pt stops file ...");
      ...
      if (getPtStopsInputFile() != null) {
        ...
        if (ptStopsFile.exists()) {
          ...
          log.error(
              "Pt stops file "
                  + getPtStopsInputFile()
                  + " not found although switch is set to true! Will abort ...");
        }

        // checking for other input files
        if (isUsingTravelTimesAndDistances()) {
          log.info(
              MatrixBasedPtRouterConfigGroup.PT_TRAVEL_TIMES_AND_DISTANCES_SWITCH
                  + " switch is set to true. Trying to find travel times and distances files ...");

          if (new File(getPtTravelTimesInputFile()).exists()
              && new File(getPtTravelDistancesInputFile()).exists()) {
            ...
            problem = true;
            log.error(
                "Travel times and/or travel distances input files not found!  Will abort ...");
          ...
          log.info(
              MatrixBasedPtRouterConfigGroup.PT_TRAVEL_TIMES_AND_DISTANCES_SWITCH
                  + " switch is set to false. Additional travel times and distances files will not be read!");
        }
      ...
      log.info(
          MatrixBasedPtRouterConfigGroup.USING_PT_STOPS
              + " switch is set to false. Matrix based pt router will not be initialized.");
    }
    if (problem) {
      throw new RuntimeException(
          "found fatal problem in matrix based pt router initialization; aborting ...");
    }
  }
}
