 ...
 *
 * <ul>
 *   <li>yyyy The module includes walk times and walk distances. But rather than putting them into a
 *       separate leg, they are included "silently" in the pt leg, which thus pretends to go door to
 *       door. kai, jul'13
 * </ul>
 *
 * @author thomas
...
public final class MatrixBasedPtRoutingModule implements RoutingModule {
  ...
  // 2) read a file with travel times and general costs from every stop to every other stop
  // ("matrix")
  // 3) route:
  ...
  public List<? extends PlanElement> calcRoute(RoutingRequest request) {
    ...
    double travelTime =
        this.ptMatrix.getTotalTravelTime_seconds(fromFacility.getCoord(), toFacility.getCoord());
    newLeg.setTravelTime(travelTime);

    final Route route = genericRouteFactory.createRoute(startLinkId, endLinkId);
    double distance =
        this.ptMatrix.getTotalTravelDistance_meter(fromFacility.getCoord(), toFacility.getCoord());
