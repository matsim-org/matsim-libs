...
public class MatrixBasedPtRouterIT {
  ...
  /**
   * This method tests the travel time computation with pseudo pt. The scenario contains a simple
   * network (9 nodes and 9 links, rangeX: 0,...,200, rangeY: 0,...,200) and a single person who
   * lives at node 1 (0,0) and works at node 3 (0,200) and uses pt. A single MATSim run is executed.
   * Afterwards, the travel time of the person's executed plan and the travel time computed by the
   * PtMatrix are compared (should be equal).
   */
  @Test
  public void testIntegration() throws IOException {
    ...
    Population population =
        CreateTestPopulation.createTestPtPopulation(
            1, new Coord((double) 0, (double) 0), new Coord((double) 0, (double) 200));
    new PopulationWriter(population, network).write(path + "plans.xml");

    // dummy csv files for pt stops, travel times and travel distances fitting into the dummy
    // network are created
    String stopsLocation =
        CreateTestNetwork.createTestPtStationCSVFile(folder.newFile("ptStops.csv"));
    String timesLocation =
        CreateTestNetwork.createTestPtTravelTimesAndDistancesCSVFile(
            folder.newFile("ptTravelInfo.csv"));
    ...
    final PtMatrix ptMatrix =
        PtMatrix.createPtMatrix(
            ...
            ConfigUtils.addOrGetModule(
                config,
                MatrixBasedPtRouterConfigGroup.GROUP_NAME,
                MatrixBasedPtRouterConfigGroup.class));
    ...
    double ttime =
        ptMatrix.getTotalTravelTime_seconds(
            new Coord((double) 0, (double) 0), new Coord((double) 0, (double) 200));
    ...
    double actualTtime =
        ((Leg) person.getSelectedPlan().getPlanElements().get(1)).getTravelTime().seconds();
