...
/**
 * This is the default mode chain generator. It can be used right away or as a template for more
 * specialized implementatons.
 *
 * <p>It works as follows: Since the available modes are known and the number of trips in the
 * requested mode chain it is possible to calculate the total amount of chains that can be created:
 * modes ^ trips. Therefore, each distinct chain can be encoded as an integer. What happens then is
 * that a counter is increased every time a new mode chain is requested and a new chain
 * corresponding to that integer is created. This way this generator has a very low memory
 * footprint.
 *
 * @author sebhoerl
 */
public class DefaultModeChainGenerator implements ModeChainGenerator {
  ...
  public static class Factory implements ModeChainGeneratorFactory {
    @Override
    public ModeChainGenerator createModeChainGenerator(
        Collection<String> modes, Person person, List<DiscreteModeChoiceTrip> trips) {
