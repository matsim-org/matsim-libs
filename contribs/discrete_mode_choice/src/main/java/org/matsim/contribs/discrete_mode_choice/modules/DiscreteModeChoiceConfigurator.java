...
/**
 * Utility class that makes it possible to set up various was of using the Discrete Mode Choice
 * extension with MATSim.
 *
 * @author sebhoerl
 */
public final class DiscreteModeChoiceConfigurator {
  ...
  public static void configureAsSubtourModeChoiceReplacement(Config config) {
    ...
    DiscreteModeChoiceConfigGroup dmcConfig =
        (DiscreteModeChoiceConfigGroup)
            config.getModules().get(DiscreteModeChoiceConfigGroup.GROUP_NAME);
    ...
    if (smcConfig.getProbaForRandomSingleTripMode() > 0.0) {
      dmcConfig
          .getSubtourConstraintConfig()
          .setConstrainedModes(Arrays.asList(smcConfig.getChainBasedModes()));
    } else {
      dmcConfig
          .getSubtourConstraintConfig()
          .setConstrainedModes(Arrays.asList(smcConfig.getModes()));
    }
    // ...setConstrainedModes used to ignore its arguments due to a typo.  This is now corrected,
    // but results are no longer backwards compatible.  kai, jan'23
    ...
    dmcConfig
        .getVehicleTourConstraintConfig()
        .setRestrictedModes(Arrays.asList(smcConfig.getChainBasedModes()));

    if (smcConfig.considerCarAvailability()) {
      dmcConfig.setModeAvailability(ModeAvailabilityModule.CAR);
      dmcConfig
          .getCarModeAvailabilityConfig()
          .setAvailableModes(Arrays.asList(smcConfig.getModes()));
    } else {
      dmcConfig.setModeAvailability(ModeAvailabilityModule.DEFAULT);
      dmcConfig
          .getDefaultModeAvailabilityConfig()
          .setAvailableModes(Arrays.asList(smcConfig.getModes()));
    }
  }
  ...
  private static final double DEFAULT_REPLANNING_RATE = 0.20;
  ...
  public static void configureAsModeChoiceInTheLoop(Config config, double replanningRate) {
