...
/**
 * This trip estimator tries to resemble the MATSim scoring functions as closely as possible. The
 * utility parameters are taken directly from the config file.
 *
 * @author sebhoerl
 */
public class MATSimTripScoringEstimator extends AbstractTripRouterEstimator {
  ...
  public MATSimTripScoringEstimator(
  ...
  protected TripCandidate estimateTripCandidate(
    ...
    if (modeParameters == null) {
      throw new IllegalStateException(
          "Encountered mode for which no scoring parameters are defined: " + mode);
    }
  ...
  private double computeLegUtility(
      ScoringParameters parameters, String mode, double travelTime, double travelDistance) {
    ...
    utility +=
        parameters.marginalUtilityOfMoney * modeParams.monetaryDistanceCostRate * travelDistance;
    return utility;
  }

  private ComputationResult computeStandardTrip(
      ScoringParameters parameters, List<? extends PlanElement> elements) {
    double utility = 0.0;
  ...
  private ComputationResult computePtTrip(
      ScoringParameters parameters, List<? extends PlanElement> elements, double departureTime) {
