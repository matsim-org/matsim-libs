...
public final class TripListConverter {
  /**
   * Convert a MATSim plan into a list of DiscreteModeChoiceTrip and extract the respective legs. It
   * is expected that the plan is already flattened (i.e. there are no interaction activities).
   */
    ...
    for (Trip initialTrip : initialTrips) {
      ...
      trips.add(
          new DiscreteModeChoiceTrip(
              ...
              destinationActivity,
              routingMode,
              initialTrip.getTripElements(),
              plan.getPerson().hashCode(),
              index,
              index,
              initialTrip.getTripAttributes()));
