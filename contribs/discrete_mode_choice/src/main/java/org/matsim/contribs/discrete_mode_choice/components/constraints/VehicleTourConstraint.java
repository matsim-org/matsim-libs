...
/**
 * This constraint makes sure that trips are continuous in the sense that vehicles get not dumped
 * somewhere in the network:
 *
 * <ul>
 *   <li>Vehicles can only be used where they have been moved to before.
 *   <li>Within one tour, vehicles must depart first from the home location.
 *   <li>Within one tour, vehicles must be brought back to the home location.
 * </ul>
 *
 * For that, it needs to be decided where "home" is. Currently, there are two options: Either the
 * location of the first activity is used (as it is for SubtourModeChoice), or the location of first
 * activity with a certain type (default is "home") is used.
 *
 * <p>If a home location cannot be found in the tour, a mode must start and end at the first and
 * last location in the tour.
 *
 * @author sebhoerl
 */
public class VehicleTourConstraint implements TourConstraint {
  ...
  public VehicleTourConstraint(
      Collection<String> restrictedModes, Id<? extends BasicLocation> homeLocationId) {
  ...
  public boolean validateBeforeEstimation(
      List<DiscreteModeChoiceTrip> tour, List<String> modes, List<List<String>> previousModes) {
    for (String restrictedMode : restrictedModes) {
      if (modes.contains(restrictedMode)) {
        ...
        if (homeLocationId != null) {
          Id<? extends BasicLocation> startLocationId =
              LocationUtils.getLocationId(tour.get(firstIndex).getOriginActivity());
          Id<? extends BasicLocation> endLocationId =
              LocationUtils.getLocationId(tour.get(lastIndex).getDestinationActivity());
        ...
        Id<? extends BasicLocation> currentLocationId =
            LocationUtils.getLocationId(tour.get(firstIndex).getDestinationActivity());
