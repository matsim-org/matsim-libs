...
/**
 * This replanning algorithm uses a predefined discrete mode choice model to perform mode decisions
 * for a given plan.
 *
 * @author sebhoerl
 */
public class DiscreteModeChoiceAlgorithm implements PlanAlgorithm {
  ...
  public DiscreteModeChoiceAlgorithm(
      Random random,
  ...
  @Override
  /**
   * Performs mode choice on a plan. We assume that TripsToLegs has been called before, hence the
   * code is working diretly on legs.
   */
  public void run(Plan plan) {
    ...
    try {
      ...
      List<TripCandidate> chosenCandidates =
          modeChoiceModel.chooseModes(plan.getPerson(), trips, random);
      ...
      for (int i = 0; i < trips.size(); i++) {
        ...
        TripRouter.insertTrip(
            plan, trip.getOriginActivity(), insertElements, trip.getDestinationActivity());
      }
