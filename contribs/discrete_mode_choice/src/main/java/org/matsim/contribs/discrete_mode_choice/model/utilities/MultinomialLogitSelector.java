 ...
 * on according to the multinomial logit model. For each candidate (i) a utility is calculated as:
 * <code>P(i) = exp( Ui ) / Sum( U1 + U2 + ... + Un )</code> For large utilities the exponential
 * terms can exceed the value range of a double. Therefore, the selector has a cutoff value, which
 * is a maximum of 700.0 by default. If this value is reached a warning will be shown.
 *
 * @author sebhoerl
 */
public class MultinomialLogitSelector implements UtilitySelector {
  private static final Logger logger = LogManager.getLogger(MultinomialLogitSelector.class);
  ...
  /**
   * Creates a MultinomialSelector. The utility cutoff value defines the maximum utility possible.
   */
  public MultinomialLogitSelector(
      double maximumUtility, double minimumUtility, boolean considerMinimumUtility) {
