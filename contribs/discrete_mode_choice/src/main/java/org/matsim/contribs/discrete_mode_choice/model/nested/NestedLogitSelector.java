...
public class NestedLogitSelector implements UtilitySelector {
  ...
  public NestedLogitSelector(
      NestStructure structure, double minimumUtility, double maximumUtility) {
  ...
  public Optional<UtilityCandidate> select(Random random) {
    ...
    for (NestedUtilityCandidate candidate : candidates) {
      ...
      if (!Double.isFinite(probability)) {
        probability =
            0.0; // Revise this, maybe it is better to cosntruct the process so we don't have
        // them at all. TODO.
      }
