...
/**
 * Defines a constraint that applies trip-based constraint on the tour level. This means that the
 * trip constraint must be fulfilled for each trip in the tour for the tour to be feasible.
 *
 * @author sebhoerl
 */
public class TourFromTripConstraint implements TourConstraint {
  ...
  public boolean validateBeforeEstimation(
      List<DiscreteModeChoiceTrip> currentTourTrips,
    ...
    for (int i = 0; i < currentTourModes.size(); i++) {
      if (!constraint.validateBeforeEstimation(
          currentTourTrips.get(i), currentTourModes.get(i), previousTripModes)) {
        return false;
      }
  ...
  public boolean validateAfterEstimation(
    ...
    previousTourCandidates.stream()
        .map(TourCandidate::getTripCandidates)
        .forEach(previousTripCandidates::addAll);
    ...
    for (int i = 0; i < currentTourCandidate.getTripCandidates().size(); i++) {
      ...
      if (!constraint.validateAfterEstimation(
          currentTourTrips.get(i), currentTripCandidate, previousTripCandidates)) {
        return false;
      }
