...
/**
 * This defines the general choice strategy for the discrete mode choice extension. It consists of
 * three replanning modules:
 *
 * <ul>
 *   <li>TripsToLegs, which collapses all multi-stage trips into one leg
 *   <li>DiscreteModeChoiceReplanningModule, which creates a new discrete choice model and performs
 *       the choices <(li>
 *   <li>Optionally, ReRoute if is is configured in the config, otherwise a check is added that all
 *       routes are present after the mode choice
 * </ul>
 *
 * @author sebhoerl
 */
public class DiscreteModeChoiceStrategyProvider implements Provider<PlanStrategy> {
  ...
  @Inject
  DiscreteModeChoiceStrategyProvider(
  ...
  public PlanStrategy get() {
    ...
    builder.addStrategyModule(
        new DiscreteModeChoiceReplanningModule(
            globalConfigGroup,
    ...
    if (dmcConfig.getPerformReroute()) {
      builder.addStrategyModule(
          new ReRoute(
              activityFacilities, tripRouterProvider, globalConfigGroup, timeInterpretation));
    } else {
