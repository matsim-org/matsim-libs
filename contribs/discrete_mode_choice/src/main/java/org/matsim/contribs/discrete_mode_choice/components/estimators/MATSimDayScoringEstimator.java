...
/**
 * This tour estimator tries to resemble the MATSim scoring functions as closely as possible. The
 * utility parameters are taken directly from the config file.
 *
 * <p>Right now, we do not score activities, but everything for that is prepared. We know, for
 * instance, the travel times of all the trips so we could easily shift activities around and check
 * how much of activity time is lost. The calculation of utility for activities is not trivial
 * though, so it is ommitted for now. Feel free to add this in another DayEstimator.
 *
 * @author sebhoerl
 */
public class MATSimDayScoringEstimator implements TourEstimator {
  ...
  public MATSimDayScoringEstimator(
