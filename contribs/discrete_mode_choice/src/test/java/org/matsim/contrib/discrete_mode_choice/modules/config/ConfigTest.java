...
public class ConfigTest {
  ...
  public void testReadWriteConfig() {
    ...
    assertEquals(
        new HashSet<>(dmcConfig.getCarModeAvailabilityConfig().getAvailableModes()),
  ...
  public void testReadWriteConfigMultipleTimes() throws IOException {
    ...
    Config config2 =
        ConfigUtils.loadConfig(
            utils.getOutputDirectory() + "/test_config1.xml", new DiscreteModeChoiceConfigGroup());
    new ConfigWriter(config2).write(utils.getOutputDirectory() + "/test_config2.xml");

    Config config3 =
        ConfigUtils.loadConfig(
            utils.getOutputDirectory() + "/test_config2.xml", new DiscreteModeChoiceConfigGroup());
    new ConfigWriter(config3).write(utils.getOutputDirectory() + "/test_config3.xml");

    BufferedReader reader =
        new BufferedReader(
            new InputStreamReader(
                new FileInputStream(new File(utils.getOutputDirectory() + "/test_config3.xml"))));
