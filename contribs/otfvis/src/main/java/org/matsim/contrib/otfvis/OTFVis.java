...
public class OTFVis {
  ...
  private static void printUsage() {
    ...
    System.out.println(
        "usage 4: OTFVis -(-)convert event-file network-file mvi-file [snapshot-period]");
    System.out.println();
    System.out.println("Usages 1-3: Starts the Visualizer");
    System.out.println(
        "mvi-file:      A MATSim visualizer file that contains a pre-recorder state");
    System.out.println("               to be visualized (*.mvi).");
    System.out.println(
        "config-file:   A complete MATSim config file to run a simulation. In that case,");
    System.out.println(
        "               a QueueSimulation will be started and visualized in real-time, ");
    ...
    System.out.println(
        "snapshot-period:  Optional. Specify how often a snapshot should be taken when");
  ...
  public static OnTheFlyServer startServerAndRegisterWithQSim(
    ...
    OnTheFlyServer server =
        OnTheFlyServer.createInstance(scenario, events, qSim, nonPlanAgentQueryHelper);
    ...
    if (config.transit().isUseTransit()) {
      ...
      for (AgentTracker agentTracker : qSim.getAgentTrackers()) {
        FacilityDrawer.Writer facilityWriter =
            new FacilityDrawer.Writer(network, transitSchedule, agentTracker, positionInfoBuilder);
