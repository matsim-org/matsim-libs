...
public class BlockingStopDemo {
  ...
  private void createNetwork() {
    ...
    for (int i = 0; i <= nOfLinks; i++) {
      ...
      nodes[i + nOfLinks + 1] =
          network
              .getFactory()
              .createNode(Id.create(i + nOfLinks + 1, Node.class), new Coord(i * 500, 500));
    ...
    for (int i = 0; i < nOfLinks; i++) {
      ...
      link =
          network
              .getFactory()
              .createLink(
                  Id.create(i + nOfLinks, Link.class),
                  nodes[i + nOfLinks + 1],
                  nodes[i + nOfLinks + 2]);
      link.setLength(500.0);
      link.setFreespeed(10.0);
  ...
  private void createTransitSchedule() {
    ...
    for (int i = 0; i < nOfStops; i++) {
      stops[i] =
          builder.createTransitStopFacility(
              Id.create(i, TransitStopFacility.class), new Coord(1000 + i * 500, 0), false);
    ...
    for (int i = 0; i < nOfStops; i++) {
      stops[i + nOfStops] =
          builder.createTransitStopFacility(
              Id.create(i + nOfStops, TransitStopFacility.class),
              new Coord(1000 + i * 500, 500),
              true);
    ...
    TransitRoute tRoute2 =
        builder.createTransitRoute(Id.create(2, TransitRoute.class), networkRoute, stopList, "bus");
  ...
  private void createPopulation() {
    ...
    for (int i = 1; i < nOfStops; i++) {
      ...
      leg.setRoute(
          new DefaultTransitPassengerRoute(
              schedule.getFacilities().get(Id.create(i - 1, TransitStopFacility.class)),
              tLine1,
              tRoute1,
              schedule.getFacilities().get(Id.create(nOfStops - 1, TransitStopFacility.class))));
    ...
    for (int i = 1; i < nOfStops; i++) {
      ...
      leg.setRoute(
          new DefaultTransitPassengerRoute(
              schedule.getFacilities().get(Id.create(nOfStops + i - 1, TransitStopFacility.class)),
              tLine2,
              tRoute2,
              schedule
                  .getFacilities()
                  .get(Id.create(2 * nOfStops - 1, TransitStopFacility.class))));
    ...
    // car-drivers
    NetworkRoute carRoute1 =
        population
            .getFactory()
            .getRouteFactories()
            .createRoute(
                NetworkRoute.class, Id.create(0, Link.class), Id.create(nOfLinks - 1, Link.class));
    NetworkRoute carRoute2 =
        population
            ...
            .createRoute(
                NetworkRoute.class,
                Id.create(nOfLinks, Link.class),
                Id.create(2 * nOfLinks - 1, Link.class));
    ...
    carRoute2.setLinkIds(
        Id.create(nOfLinks, Link.class), linkIds2, Id.create(2 * nOfLinks - 1, Link.class));
  ...
  private void runSim() {
    ...
    TransitRouteAccessEgressAnalysis analysis =
        new TransitRouteAccessEgressAnalysis(
            this.scenario
                .getTransitSchedule()
                .getTransitLines()
        ...
        System.out.print(
            "\t"
                + ttc.getLinkTravelTimes()
                    .getLinkTravelTime(
                        network.getLinks().get(Id.create(i, Link.class)), time, null, null));
      }
      System.out.println();
    }
    System.out.println();
