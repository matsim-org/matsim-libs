...
public class OTFClientLive {
    ...
    SwingUtilities.invokeLater(
        new Runnable() {
          ...
          public void run() {
            ...
            // I think that this essentially just connects the quad tree ... so that not all links
            // are used, but only those
            ...
            connectionManager.connectWriterToReader(
                OTFAgentsListHandler.Writer.class, OTFAgentsListHandler.class);
            // I think that this only works if at least one corresponding OTFDataWriter is added via
            // OnTheFlyServer.addAdditionalElement(...).

            // Can we say something like
            //
            // connectionManager.connectLinkToWriter(OTFLinkAgentsHandler.WriteToProtocolBuffers.class);
            // ???  But why would we set this on the client side?  Maybe the client has to tell the
            // server what to send?  kai, jun'16

            if (config.transit().isUseTransit()) {
              connectionManager.connectWriterToReader(
                  FacilityDrawer.Writer.class, FacilityDrawer.Reader.class);
              connectionManager.connectReaderToReceiver(
                  FacilityDrawer.Reader.class, FacilityDrawer.DataDrawer.class);
              connectionManager.connectReceiverToLayer(
                  FacilityDrawer.DataDrawer.class, SimpleSceneLayer.class);
            }
            ...
            OTFClientControl.getInstance()
                .setOTFVisConfig(
                    visconf); // has to be set before OTFClientQuadTree.getConstData() is invoked!
            OTFServerQuadTree serverQuadTree = server.getQuad(connectionManager);
            OTFClientQuadTree clientQuadTree =
                serverQuadTree.convertToClient(server, connectionManager);

            OTFOGLDrawer mainDrawer =
                new OTFOGLDrawer(clientQuadTree, visconf, canvas, hostControl);
            OTFControlBar hostControlBar = new OTFControlBar(server, hostControl, mainDrawer);
            OTFVisFrame otfVisFrame =
                new OTFVisFrame(canvas, server, hostControlBar, mainDrawer, saver);

            OTFQueryControl queryControl = new OTFQueryControl(server, visconf);
            OTFQueryControlToolBar queryControlBar =
                new OTFQueryControlToolBar(queryControl, visconf);
  ...
  private static void assertZoomLevel17(Config config) {
    if (ConfigUtils.addOrGetModule(config, OTFVisConfigGroup.GROUP_NAME, OTFVisConfigGroup.class)
            .getMaximumZoom()
        != 17) {
      throw new RuntimeException(
          "The OSM layer only works with maximumZoomLevel = 17. Please adjust your config.");
    }
  }
  ...
  private static class OTFVisWMSTileFactory extends DefaultTileFactory {
    public OTFVisWMSTileFactory(final WMSService wms, final int maxZoom) {
      super(
          new TileFactoryInfo(
              0, maxZoom, maxZoom, 256, true, true, // tile size and x/y orientation is r2l & t2b
              "", "x", "y", "zoom") {
            @Override
            public String getTileUrl(int x, int y, int zoom) {
              int zz = maxZoom - zoom;
