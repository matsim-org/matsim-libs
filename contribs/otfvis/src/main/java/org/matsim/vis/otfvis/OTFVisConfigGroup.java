...
public class OTFVisConfigGroup extends ConfigGroup {
  ...
  /**
   * this is here so that the entry can be communicated from the network (which seems to be known
   * only by the otfvis server) to the otfvis client. I am not sure if this is really a hack; might
   * also just make this configurable in the sense that it tries to find a useful value either from
   * network or from config or from the saved config. this is, however, not implemented. kai, jan'11
   */
  ...
  public static enum ColoringScheme {
    standard,
    bvg,
    bvg2,
    byId,
  ...
  public String getValue(final String key) {
    // for variables that have getters, this is not needed (and should probably be avoided).  kai,
    // jan'11
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        AGENT_SIZE,
        "The (initial) size of the agents.  Only a range of numbers is allowed, otherwise otfvis aborts"
    ...
    map.put(
        LINK_WIDTH_IS_PROPORTIONAL_TO,
        "Link width is proportional to `" + NUMBER_OF_LANES + "' or to `" + CAPACITY + "'.");
    ...
    map.put(
        COLORING,
        "coloring scheme for otfvis.  Currently (2012) allowed values:" + allowedColorings);
    ...
    map.put(
        DRAW_NON_MOVING_ITEMS,
        "If non-moving items (e.g. agents at activities, at bus stops, etc.) should be showed.  "
  ...
  public OTFVisConfigGroup setNetworkColor(final Color networkColor) {
    setModified();
    this.networkColor =
        new Color(networkColor.getRed(), networkColor.getGreen(), networkColor.getBlue(), 128);
    return this;
  }
