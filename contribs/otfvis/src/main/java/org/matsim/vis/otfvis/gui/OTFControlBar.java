...
public final class OTFControlBar extends JToolBar {
  ...
  public OTFControlBar(
      OTFServer server, final OTFHostControl hostControl, OTFOGLDrawer mainDrawer) {
    this.server = server;
    ...
    if (!server.isLive()) {
      ...
      multiStepsBackwardButton.addActionListener(
          e -> {
    ...
    multiStepForwardButton.addActionListener(
        e -> {
          int bigStep = OTFClientControl.getInstance().getOTFVisConfig().getBigTimeStep();
          hostControl.requestTimeStep(OTFControlBar.this.hostControl.getSimTime() + bigStep);
        });
    ...
    playOrPauseButton.addActionListener(
        e -> {
  ...
  private void updateScaleField() {
    scaleField.setText(
        String.valueOf((double) ((float) (Math.round(drawer.getScale() * 100)) / 100)));
  }
}
