...
/**
 * OGLAgentPointLayer is responsible for drawing the agents/vehicles as point sprites. It is a very
 * fast way to draw massive (100ks) of agents in realtime. It does not run too well on ATI cards,
 * though.
 *
 * @author dstrippgen
 */
public class OGLAgentPointLayer extends OTFGLAbstractDrawable implements SceneLayer {
  ...
  public OGLAgentPointLayer() {
    if (OTFClientControl.getInstance().getOTFVisConfig().getColoringScheme()
        == OTFVisConfigGroup.ColoringScheme.infection) {
      redToGreenColorizer =
          new FastColorizer(
              new double[] {0.0, 30., 50.},
  ...
  public void onDraw(GL2 gl) {
    ...
    // texture = null;
    // setting the texture to null means that agents are painted using (software-rendered?) squares.
    //  I have made speed
    // tests and found on my computer (mac powerbook, with "slow" graphics settings) no difference
    // at all between "null"
    ...
    for (int i = 0; i < this.posBuffers.size(); i++) {
      ...
      int remain =
          i == this.posBuffers.size() - 1
              ? this.count % OGLAgentPointLayer.BUFFERSIZE
              : OGLAgentPointLayer.BUFFERSIZE;
      colors.position(0);
      vertex.position(0);
