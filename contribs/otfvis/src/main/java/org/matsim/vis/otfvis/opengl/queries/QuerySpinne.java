...
/**
 * QuerySpinne shows a relationship network for a given link based on several options. The network
 * shown is that of the routes that that agents take over the course of their day/trip.
 *
 * @author dstrippgen
 */
  ...
  public static class Result implements OTFQueryResult {
    ...
    public void draw(OTFOGLDrawer drawer) {
      ...
      if (this.calcOffset) {
        ...
        colorizer3 =
            new FastColorizer(new double[] {0.0, maxCount}, new Color[] {Color.WHITE, Color.BLUE});
    ...
    private void drawCaption(OTFOGLDrawer drawer) {
      ...
      InfoText text1 =
          new InfoText("Count: 0", (float) (minX + (b + 1) * verOf), (float) (minY + c * horOf));
      text1.draw(
          textRenderer,
          OTFClientControl.getInstance().getMainOTFDrawer().getCanvas(),
          drawer.getViewBoundsAsQuadTreeRect());
      ...
      InfoText text3 =
          new InfoText(
              "Count: " + (maxCount), (float) (minX + (b + 1) * verOf), (float) (minY + c * horOf));
      text3.draw(
          textRenderer,
          OTFClientControl.getInstance().getMainOTFDrawer().getCanvas(),
          drawer.getViewBoundsAsQuadTreeRect());
    ...
    for (Plan plan : actPersons) {
      for (PlanElement pe : plan.getPlanElements()) {
        if (pe instanceof Leg) {
          ...
          if (route
              instanceof NetworkRoute) { // added in jun09, see below in "collectLinks". kai, jun09
