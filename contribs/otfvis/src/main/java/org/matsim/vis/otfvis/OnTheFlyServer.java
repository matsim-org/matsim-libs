...
/**
 * OnTheFlyServer is the live server of the OTFVis. it is injected into a running QueueSimulation by
 * either overloading the simulation's afterSimStep() method or via the external trigger events. The
 * simulation should call on every tick the method updateStatus(). Then the server can stop the
 * simulation run whenever wanted. The interface to the client is given by the OTFLiveServerRemote
 * interface.
 *
 * @author dstrippgen
 */
public class OnTheFlyServer implements OTFLiveServer {
  ...
  public static OnTheFlyServer createInstance(
      Scenario scenario, EventsManager events, VisMobsim qSim) {
  ...
  public OTFServerQuadTree getQuad(OTFConnectionManager connect) {
    if (quad != null) {
      ...
      if (ConfigUtils.addOrGetModule(config, OTFVisConfigGroup.GROUP_NAME, OTFVisConfigGroup.class)
          .isMapOverlayMode()) {
        // Transform everything from network coordinates first to WGS84 and then to standard
        // mercator projection as used by OSM tiles and Geoserver.
        ...
        final CoordinateTransformation scenario2wgs84 =
            TransformationFactory.getCoordinateTransformation(
                scenarioCRS, TransformationFactory.WGS84);
        ...
        OTFServerQuadTree.setTransformation(
  ...
  public OTFVisConfigGroup getOTFVisConfig() {
    OTFVisConfigGroup otfVisConfig =
        ConfigUtils.addOrGetModule(
            this.scenario.getConfig(), OTFVisConfigGroup.GROUP_NAME, OTFVisConfigGroup.class);
