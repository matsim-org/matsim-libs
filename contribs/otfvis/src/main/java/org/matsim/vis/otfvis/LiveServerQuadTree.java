...
class LiveServerQuadTree extends OTFServerQuadTree {
  ...
  private static final Logger log = LogManager.getLogger(LiveServerQuadTree.class);
  ...
  private static <T> List<OTFWriterFactory<T>> instanciateFactories(
      Collection<Class<OTFWriterFactory<T>>> nodeFactories) {
  ...
  private void installLinkWriterFactories(
    ...
    for (VisLink link : this.net.getVisLinks().values()) {
      ...
      for (OTFWriterFactory<VisLink> fac : linkWriterFactoryObjects) {
        ...
        // (The unterlying QuadTree documentation says that it will accept the same object multiple
        // times as
        // long as at different coordinates.  So I am trying this out here.  The result will depend
        // on the fact if
        // the collections are implemented as sets (in which case duplicate links will just be
        // ignored) or standard
        // collections, in which case the plotting will supposedly happen twice, this reducing
        // plotting speed by a
