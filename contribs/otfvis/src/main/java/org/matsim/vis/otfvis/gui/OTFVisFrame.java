...
public final class OTFVisFrame extends JFrame {
  ...
  /**
   * This method statically installs a custom Swing RepaintManager which ties the map component to
   * the JPanel in which it is layered under the agent drawer. Otherwise the map would repaint
   * itself when it has finished loading a tile, and the agent drawer would not notice and would be
   * painted over.
   *
   * <p>This looks dirty and probably does not scale to the case where many components would do
   * this, but it is the only way I have found, short of patching the JXMapViewer.
   */
  private static void installCustomRepaintManager(
      final JPanel compositePanel, final JXMapViewer jMapViewer) {
    RepaintManager myManager =
        new RepaintManager() {
          public void addDirtyRegion(JComponent c, int x, int y, int w, int h) {
            // I had the feeling I should call the *previous* RepaintManager here instead of the
            // supertype, but that does not work.
  ...
  public OTFVisFrame(
      Component canvas,
      OTFServer server,
      OTFControlBar controlBar,
      OTFOGLDrawer mainDrawer,
      SettingsSaver saver) {
    super("MATSim OTFVis");
    ...
    log.warn(
        "See https://github.com/matsim-org/matsim-libs/pull/2940 and/or https://jogamp.org/bugzilla/show_bug.cgi?id=1317#c21 .");
        ...
        new AbstractAction() {
          ...
          public void actionPerformed(final ActionEvent e) {
            PreferencesDialog preferencesDialog =
                new PreferencesDialog(OTFClientControl.getInstance().getOTFVisConfig());
  ...
  public void addMapViewer(TileFactory tf) {
    ...
    final CoordinateTransformation coordinateTransformation =
        new WGS84ToMercator.Deproject(otfVisConfig.getMaximumZoom());
    mainDrawer.addChangeListener(
        new ChangeListener() {

          @Override
          public void stateChanged(ChangeEvent e) {
            double x =
                mainDrawer.getViewBoundsAsQuadTreeRect().centerX + mainDrawer.getQuad().offsetEast;
            double y =
                mainDrawer.getViewBoundsAsQuadTreeRect().centerY + mainDrawer.getQuad().offsetNorth;
  ...
  private class PreferencesDialog extends JDialog {
    ...
    public PreferencesDialog(OTFVisConfigGroup config) {
      ...
      // Mouse Buttons
      {
        ...
        {
          ComboBoxModel<String> leftMFuncModel =
              new DefaultComboBoxModel<>(new String[] {"Zoom", "Pan", "Select"});
        ...
        }
        {
          ComboBoxModel<String> jComboBox2Model =
              new DefaultComboBoxModel<>(new String[] {"Menu", "Zoom", "Pan", "Select"});
                ...
                public void actionPerformed(ActionEvent e) {
                  ...
                  Color c =
                      JColorChooser.showDialog(
                          frame, "Choose the background color", visConfig.getBackgroundColor());
                  if (c != null) {
          ...
          jButton.addActionListener(
              new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                  ...
                  Color c =
                      JColorChooser.showDialog(
                          frame, "Choose the network color", visConfig.getNetworkColor());
                  if (c != null) {
      ...
      // Agent size
      {
        ...
        SpinnerNumberModel model =
            new SpinnerNumberModel((int) visConfig.getAgentSize(), 0.1, Double.MAX_VALUE, 10);
        agentSizeSpinner.setModel(model);
        agentSizeSpinner.setBounds(90, 145, 153, 31);
        agentSizeSpinner.addChangeListener(
            new ChangeListener() {
              @Override
              public void stateChanged(ChangeEvent e) {
      ...
      // Link Width
      {
        ...
        SpinnerNumberModel model2 =
            new SpinnerNumberModel((int) visConfig.getLinkWidth(), 0.1, Double.MAX_VALUE, 10);
        linkWidthSpinner.setModel(model2);
        linkWidthSpinner.setBounds(90, 195, 153, 31);
        linkWidthSpinner.addChangeListener(
            new ChangeListener() {
              @Override
              public void stateChanged(ChangeEvent e) {
      ...
      // Delay ms
      {
        ...
        SpinnerNumberModel model2 =
            new SpinnerNumberModel(visConfig.getDelay_ms(), 0, Double.MAX_VALUE, 10);
        delaySpinner.setModel(model2);
        delaySpinner.setBounds(90, 245, 153, 31);
        delaySpinner.addChangeListener(
            new ChangeListener() {
              @Override
              public void stateChanged(ChangeEvent e) {
