...
/**
 * A Thread implementation responsible for showing the "MobSim running to ..." Dialog when the user
 * selects a new time in the input field for the time.
 *
 * @author dstrippgen
 */
public class OTFAbortGoto extends Thread {
  ...
  public void run() {
    ...
    ProgressMonitor progressMonitor =
        new ProgressMonitor(
            null, "Running Simulation forward to " + Time.writeTime(toTime), "hat", from, to);
    ...
    while (!terminate) {
      try {
        ...
        int progress =
            (pastMidnight > 0
                ? 24 * 3600 + (int) (5 * 3600 * (pastMidnight / (pastMidnight + 1)))
                : actTime);
