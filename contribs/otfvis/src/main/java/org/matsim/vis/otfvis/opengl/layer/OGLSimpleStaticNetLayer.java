 ...
 *
 * <p>It uses an OpenGL "Display List" to speed up drawing. The Display List essentially collects
 * all draw commands from the link drawers ("items") and caches them for extremely fast redrawing.
 *
 * @author dstrippgen
 */
public class OGLSimpleStaticNetLayer implements GLEventListener {
  ...
  public void display(GLAutoDrawable glAutoDrawable) {
    ...
    float[] backgroundColor =
        OTFClientControl.getInstance()
            .getOTFVisConfig()
            .getBackgroundColor()
            .getColorComponents(new float[4]);
  ...
  private void checkNetList(final GLAutoDrawable glAutoDrawable) {
    ...
    final Rect rect =
        OTFClientControl.getInstance().getMainOTFDrawer().getViewBoundsAsQuadTreeRect();

    if (oldLinkWidth != config.getLinkWidth()
        || !config.getNetworkColor().equals(oldColor)
        || !rect.equals(oldRect)) {
      // If display options have changed or if the screen area has changed, we need to recreate the
      // display list.
    ...
    if (netDisplList < 0) {
      ...
      gl.glColor4d(
          ...
          components[1],
          components[2],
          config.getNetworkColor().getAlpha() / 255.0f);
      OTFClientControl.getInstance()
          .getMainOTFDrawer()
          .getQuad()
          .execute(
              rect,
              new QuadTree.Executor<OTFDataReader>() {
                @Override
  ...
  private boolean isZoomBigEnoughForLabels(OTFVisConfigGroup config) {
    ...
    return (size.getX() * pixelsizeStreet < cellWidth)
        && (size.getX() * pixelsizeStreet < cellWidth);
  }

  private void displayLinkIds(
      Map<Coord, String> linkIds, GLAutoDrawable glAutoDrawable, Rect rect) {
