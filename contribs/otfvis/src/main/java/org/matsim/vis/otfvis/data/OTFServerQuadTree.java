...
/**
 * OTFServerQuad is the quad representation of all elements of the network on the server side. This
 * QuadTree is mirrored on the client side by OTFClientQuad.
 *
 * @author dstrippgen
 */
public abstract class OTFServerQuadTree extends QuadTree<OTFDataWriter> {
  ...
  protected void setBoundingBoxFromNetwork(Network n) {
    ...
    //		log.debug("offsetEast: " + offsetEast + " offsetNorth: " + offsetNorth + " " +
    // transformation);
  ...
  /**
   * Sets a new top node in case the extremities from the c'tor are not good anymore, it also clear
   * the QuadTree
   *
  ...
  public OTFClientQuadTree convertToClient(
      final OTFServer host, final OTFConnectionManager connect) {
    final OTFClientQuadTree client =
        new OTFClientQuadTree(host, 0., 0., this.easting, this.northing);
    ...
    for (OTFDataWriter<?> element : this.additionalElements) {
      Collection<Class<OTFDataReader>> readerClasses =
          connect.getReadersForWriter(element.getClass());
      for (Class<? extends OTFDataReader> readerClass : readerClasses) {
        try {
          ...
          log.info(
              "Connected additional element writer "
  ...
  // Internally we hold the coordinates from 0,0 to max -min .. to optimize use of float in
  // visualizer
  @Override
