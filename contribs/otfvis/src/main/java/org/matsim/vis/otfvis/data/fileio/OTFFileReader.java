...
/**
 * The OTF has a file Reader and a file Writer part. The Reader is the the mvi playing OTFServer.
 *
 * @author dstrippgen
 * @author dgrether
 * @author michaz
...
public final class OTFFileReader implements OTFServer {
  ...
  public byte[] getQuadDynStateBuffer(final Rect bounds) {
    if (bounds == null) {
      log.warn(
          "Bounds are ignored but set! [[I don't understand what this means here.  kai, feb'11]]");
    }
  ...
  private byte[] readTimeStep(final double time_s) throws IOException {
    ...
    DataInputStream inFile =
        new DataInputStream(new BufferedInputStream(zipFile.getInputStream(entry), 1000000));
    readStateBuffer(inFile, buffer);
    zipFile.close();
    return buffer;
  }
    ...
    try {
      ...
      if (offset != size) {
        throw new RuntimeException(
            "READ SIZE did not fit! File corrupted! in second " + timenextTime);
      }
