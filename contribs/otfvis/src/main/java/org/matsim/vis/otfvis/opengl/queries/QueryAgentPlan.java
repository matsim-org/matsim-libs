...
public class QueryAgentPlan extends AbstractQuery implements OTFQueryOptions, ItemListener {
  ...
  private static final Logger log = LogManager.getLogger(QueryAgentPlan.class);
  ...
  public OTFQueryResult query() {
    ...
    if (plan != null) {
      for (PlanElement e : plan.getPlanElements()) {
        if (e instanceof Activity) {
          ...
          if (StageActivityTypeIdentifier.isStageActivity(act.getType())
              && OTFClientControl.getInstance().getOTFVisConfig().getColoringScheme()
                  == ColoringScheme.infection) {

            continue; // skip
          }
          Coord c2 = getCoord(act);
          ActivityInfo activityInfo =
              new ActivityInfo((float) c2.getX(), (float) c2.getY(), getSubstring(act));
          if (StageActivityTypeIdentifier.isStageActivity(act.getType())) {
            activityInfo =
                new ActivityInfo(
                    (float) c2.getX(),
                    (float) c2.getY(),
                    act.getType().replace("interaction", "i"));
          }
      ...
      if (includeRoutes) {
        result.buildRoute(
            plan, agentId, simulationView.getNetwork(), Level.ROUTES, simulationView.getScenario());
      } else {
        result.buildRoute(
            plan,
            agentId,
            simulationView.getNetwork(),
            Level.PLANELEMENTS,
            simulationView.getScenario());
      }
    } else {
    ...
    if (act != null) {
      ...
      if (act.getStartTime().isDefined()
          && simulationView.getTime() > act.getStartTime().seconds()
          && act.getEndTime().isDefined()
          && simulationView.getTime() <= act.getEndTime().seconds()) {
        ActivityInfo activityInfo =
            new ActivityInfo((float) c2.getX(), (float) c2.getY(), getSubstring(act));
        activityInfo.finished =
            (simulationView.getTime() - act.getStartTime().seconds())
                / (act.getEndTime().seconds() - act.getStartTime().seconds());
        result.acts.add(activityInfo);
      }
    }
    return result;
  ...
  private static final SnapshotLinkWidthCalculator linkWidthCalculator =
      new SnapshotLinkWidthCalculator();
  private static final PositionInfo.LinkBasedBuilder builder =
  ...
  public static class Result implements OTFQueryResult {
    ...
    private void buildRoute(
        Plan plan, Id<Person> agentId, Network net, Level level, Scenario scenario) {
      ...
      for (PlanElement planElement : planElements) {
        if (planElement instanceof Activity) {
          ...
          if (coord == null) {
            ...
            // AgentSnapshotInfo pi = snapshotInfoFactory.createAgentSnapshotInfo(agentId, link,
            // 0.9*link.getLength(), 0);
    ...
    public void draw(OTFOGLDrawer drawer) {
      ...
      drawActivityTexts();
      Point2D.Double agentCoords =
          drawer
              .getCurrentSceneGraph()
              .getAgentPointLayer()
              .getAgentCoords(this.agentId.toCharArray());
      if (agentCoords != null) {
    ...
    private void drawTextBox(
        GLAutoDrawable drawable, TextRenderer textRenderer, ActivityInfo activityEntry) {
      float scale = (float) OTFClientControl.getInstance().getMainOTFDrawer().getScale();
