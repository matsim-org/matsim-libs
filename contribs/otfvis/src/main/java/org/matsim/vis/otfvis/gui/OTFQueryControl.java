...
/**
 * This class is only used with the "live" OTFVis. It represents th Query issuing GUI element on the
 * lower part of the screen. Alls queries are initiated and managed here.
 *
 * @author dstrippgen
 */
public class OTFQueryControl implements GLEventListener {
  ...
  private final Vector<QueryEntry> queries =
      new Vector<>(
          Arrays.asList(
              new QueryEntry("agentPlan", "show the actual plan of an agent", QueryAgentPlan.class),
              new QueryEntry(
                  "agentEvents", "show the actual events of an agent", QueryAgentEvents.class),
              new QueryEntry(
                  "agentPTBus", "highlight all buses of a given line", QueryAgentPTBus.class),
              new QueryEntry("linkSpinneALL", "show Spinne of ALL traffic", QuerySpinne.class),
              new QueryEntry(
                  "linkById", "show link(s) by comma separated link id", QueryLinkById.class),
              new QueryEntry(
                  "nodeById", "show node(s) by comma separated node id", QueryNodeById.class)));
  ...
  public void handleClick(Rectangle2D.Double origRect, int mouseButton) {
    if (mouseButton == 3) {
      ...
      if (typeOfQuery == OTFQuery.Type.AGENT) {
        ...
        QueryLinkId.Result linkIdQuery =
            (QueryLinkId.Result) createQuery(new QueryLinkId(origRect));
  ...
  public synchronized void updateQueries() {
  ...
  AbstractQuery createQuery(String className) {
    try {
      Class<? extends AbstractQuery> classDefinition =
          (Class<? extends AbstractQuery>) Class.forName(className);
  ...
  private List<String> resolveId(Double origRect) {
    QueryAgentId.Result agentIdQuery =
        (QueryAgentId.Result) createQuery(new QueryAgentId(origRect));
