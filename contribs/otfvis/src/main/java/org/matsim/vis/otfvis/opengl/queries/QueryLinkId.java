...
/**
 * QueryLinkId find the most likely link (or links) to a given coordinate (or rectangle). The
 * meaning of "nearest" is that of the smallest distance between the middle point of the link and
 * the given coordinate. TODO This might lead to unexpected behavior with long links.
 *
 * @author dstrippgen
 */
public class QueryLinkId extends AbstractQuery {
        ...
        if (nearestOnly) {
          ...
          if (dist <= minDist) {
            ...
            Point2D.Float anchor =
                OTFLinkAgentsHandler.calcOrtho(fromX, fromY, middleX, middleY, cellWidth / 2.);
            result.linkIds.put(
                new Coord(middleX + anchor.x, middleY + anchor.y), link.getId().toString());
          }
  ...
  public void installQuery(SimulationViewForQueries simulationView) {
    ...
    if (width == 0) {
      rect =
          new QuadTree.Rect(
              sx - regionWidth, sy - regionHeight, sx + regionWidth, sy + regionHeight);
    } else {
