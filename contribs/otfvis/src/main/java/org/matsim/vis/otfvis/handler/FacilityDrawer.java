...
public class FacilityDrawer {
  private static final Logger log = LogManager.getLogger(FacilityDrawer.class);
  ...
  public static class Writer extends OTFDataWriter<Void> {
    ...
    public Writer(
        ...
        PositionInfo.LinkBasedBuilder positionInfoBuilder) {
    ...
    public void writeConstData(ByteBuffer out) {
      ...
      for (TransitStopFacility facility : this.schedule.getFacilities().values()) {
        ...
        if (facility.getLinkId() != null) {
          // yyyy would most probably make sense to have something that generates coordinates for
          // facilities
          ...
          if (link == null) {
            ...
            Point2D.Double point =
                OTFServerQuadTree.transform(new Coord(ps.getEasting(), ps.getNorthing()));
            out.putDouble(point.getX());
            out.putDouble(point.getY());
          }
        } else {
    ...
    public void onDraw(GL2 gl) {
      if (OTFClientControl.getInstance().getOTFVisConfig().isDrawTransitFacilities()) {
        ...
        for (VisBusStop stop : this.stops) {
          if (stop.linkId != null) {
            InfoText stopText =
                new InfoText(stop.buildText(), (float) stop.x - 100.0f, (float) stop.y + 50.0f);
  ...
  private static class VisBusStop {
    ...
    private String buildText() {
      ...
      else return Integer.toString(getnOfPeople());
    }
