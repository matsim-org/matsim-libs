...
/**
 * OTFTimeLine is the time line toolbar. It is only used in case of playing a mvi file.
 *
 * @author dstrippgen
 */
public class OTFTimeLine extends JToolBar implements ActionListener, ChangeListener {
  ...
  private void addSlider() {
    ...
    labelTable.put(
        Integer.valueOf(min), new JLabel(Time.writeTime(dsteps[0], Time.TIMEFORMAT_HHMM)));
    ...
    for (int i = n; i < dsteps.length - 1; i += n) {
      labelTable.put(
          Integer.valueOf(dsteps[i].intValue()),
