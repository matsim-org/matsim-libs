...
/**
 * QueryAgentId is an internal query. For a given coordinate it tries to find the most likely agent.
 * It does that by first finding the most likely link to the clicked coordinate and then the agent
 * nearest to the coordinate on that link. TODO This might yield unexpected results. It would
 * probably be a better solution to collect the agents of all surrounding link and then chose the
 * nearest one.
 *
 * @author dstrippgen
 */
public class QueryAgentId extends AbstractQuery {
  ...
  public void installQuery(SimulationViewForQueries simulationView) {
    ...
    for (AgentSnapshotInfo info : positions) {
      if ((info.getAgentState() == AgentState.PERSON_AT_ACTIVITY)
          && !OTFLinkAgentsHandler.showParked) continue;
      java.awt.geom.Point2D.Double xy =
          OTFServerQuadTree.transform(new Coord(info.getEasting(), info.getNorthing()));
      ...
      if (this.width == 0) {
        ...
        dist =
            Math.sqrt(
                xDist * xDist
                    + yDist
                        * yDist); // "dist_2 = xDist*xDist + yDist*yDist" will work here (no need
        // for "sqrt"), michalm
        if (dist < minDist) {
          minDist = dist;
