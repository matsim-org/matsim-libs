package org.matsim.simwrapper.scenariocomp;
...
//			.addDashboard(new EmissionsDashboard(config.global().getCoordinateSystem()))
			...
			// random measure that makes no sense
			for (Link link : scenario.getNetwork().getLinks().values()) {
				if (link.getCapacity() > 800) {
				link.setNumberOfLanes(link.getNumberOfLanes() + 1);
				link.setFreespeed(link.getFreespeed() + 30);
				}
