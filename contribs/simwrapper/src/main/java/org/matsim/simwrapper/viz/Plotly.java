...
public final class Plotly extends Viz {
  ...
  /**
   * Empty placeholder array, which may be used as input for constructing traces. Actual data is
   * loaded later from files and not used directly in the specification file.
   */
  ...
  @Nullable @JsonIgnore public Layout layout;
  ...
  /**
   * Merge two column as index. Column name as key will be merged with the column name value. This
   * allows to build level multi indices for certain plot types.
   */
  public Map<String, String> multiIndex;

  @JsonIgnore private List<Trace> traces = new ArrayList<>();
  @JsonIgnore private List<DataSet> data = new ArrayList<>();
  @JsonIgnore private List<DataMapping> mappings = new ArrayList<>();
  ...
  public DataSet addDataset(String path) {
    ...
    String name =
        data.size() == 0
            ? "dataset"
            : FilenameUtils.removeExtension(FilenameUtils.getName(path)).replace("_", "");
  ...
  private Map<String, Object> convertTrace(Trace t, DataMapping d, int i) {
    ...
    for (Field field : fields) {
      ...
      try {
        ...
        if (name.equals("opacity") && Objects.equals(o, 1.0)) continue;
        if (name.equals("visible") && Objects.equals(o, AbstractTrace.Visibility.TRUE)) continue;
        if (name.equals("xAxis") && Objects.equals(o, "x")) continue;
        if (name.equals("yAxis") && Objects.equals(o, "y")) continue;

        if (name.equals("type")) type = (String) o;
  ...
  private Map<String, Object> clean(Map<String, Object> object) {
    ...
    while (it.hasNext()) {
      ...
      if (e.getValue() instanceof Map) clean((Map<String, Object>) e.getValue());
  ...
  public static final class DataSet {
    ...
    private Object toJSON() {
      if (pivot == null && constant == null && aggregate == null) return file;
    ...
    /**
     * Indicates that each column beside the already mapped ones contain one data group. This is
     * also known as 'wide-format'. Defining this will map each new column to the specified target
     * array.
     *
    ...
    public DataSet pivot(List<String> exclude, String namesTo, String valuesTo) {
      ...
      if (exclude.isEmpty()) throw new IllegalArgumentException("Exclude list can not be empty");
    ...
    public DataSet constant(String columnName, Object constant) {
      if (this.constant == null) this.constant = new LinkedHashMap<>();
    ...
    /**
     * Aggregate data within each trace on the {@code targetColumn}. Uses all unmapped columns or
     * only the given columns if not empty.
     */
  ...
  public static final class DataMapping {
    ...
    private void insert(Map<String, Object> obj) {
      ...
      if (obj.containsKey("name")) obj.put("original_name", obj.get("name"));
  ...
  /**
   * Utility class that holds some, but not all available color schemes. See {@link <a
   * href="https://github.com/d3/d3-scale-chromatic/tree/main">here</a>}
   */
