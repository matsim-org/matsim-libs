...
public final class Data {
  ...
  public String resource(String name) {
    ...
    Path baseDir;
    if (currentContext.getName().isBlank()) baseDir = this.path.resolve("resources");
    else baseDir = this.path.resolve("resources-" + currentContext.getName());
    ...
    try {
      if (resources.containsKey(resolved)
          && !resources.get(resolved).toURI().equals(resource.toURI()))
        throw new IllegalArgumentException(
            String.format(
                "Resource '%s' was already mapped to resource '%s'. ",
                name, resources.get(resolved)));
  ...
  /**
   * Copies an input file (which can be local or an url) to the output directory. This method is
   * intended to copy input for analysis classes.
   */
  public String copyInput(URL file, String baseFolder) {
    ...
    Path baseDir;
    if (currentContext.getName().isBlank()) baseDir = this.path.resolve(baseFolder);
    else baseDir = this.path.resolve(baseFolder + "-" + currentContext.getName());
    ...
    try {
      if (resources.containsKey(resolved) && !resources.get(resolved).toURI().equals(file.toURI()))
        throw new IllegalArgumentException(
            String.format(
                "File '%s' was already mapped to url '%s'. ", file, resources.get(resolved)));
  ...
  /**
   * Returns the unix path. Otherwise, paths might be Windows paths, which are not compatible to
   * simwrapper.
   */
