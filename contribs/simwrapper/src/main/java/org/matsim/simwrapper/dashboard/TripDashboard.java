...
public class TripDashboard implements Dashboard {
  ...
  @Nullable private final String modeShareRefCsv;
  @Nullable private final String modeShareDistRefCsv;
  @Nullable private final String modeUsersRefCsv;
  ...
  /**
   * Create a dashboard containing reference data. If any of the reference data is not available it
   * can also be null Data format needs to be the same as produced by the analysis. Please refer to
   * the dashboard output. All given argument must be resources in the classpath.
   *
   * @param modeShareRefCsv resource containing the mode share per distance group and mode, summing
   *     to a total of one
   ...
   */
  public TripDashboard(
      @Nullable String modeShareRefCsv,
      @Nullable String modeShareDistRefCsv,
      @Nullable String modeUsersRefCsv) {
  ...
  public void configure(Header header, Layout layout) {
        ...
        (viz, data) -> {
          ...
          viz.addTrace(
              BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT)
                  ...
                  .build(),
              ds.mapping().name("main_mode").y("source").x("share"));
        });
    ...
    first.el(
        ...
        (viz, data) -> {
          ...
          viz.addTrace(
              BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT).name("Simulated").build(),
              viz.addDataset(data.compute(TripAnalysis.class, "mode_share.csv", args))
          ...
          if (modeShareRefCsv != null) {
            viz.addTrace(
                BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT).name("Reference").build(),
                viz.addDataset(data.resource(modeShareRefCsv))
                    .aggregate(List.of("dist_group"), "share", Plotly.AggrFunc.SUM)
                    .mapping()
                    .x("dist_group")
    ...
    layout
        ...
        .el(
            ...
            (viz, data) -> {
              ...
              Plotly.DataSet sim =
                  viz.addDataset(data.compute(TripAnalysis.class, "mode_share_per_dist.csv"))
                      .constant("source", "Sim");

              viz.addTrace(
                  BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT).build(),
                  sim.mapping().name("main_mode").x("dist_group").y("share"));

              if (modeShareDistRefCsv != null) {

                Plotly.DataSet ref =
                    viz.addDataset(data.resource(modeShareDistRefCsv)).constant("source", "Ref");
    ...
    layout
        ...
        .el(
            ...
            (viz, data) -> {
              ...
              Plotly.DataSet ds =
                  viz.addDataset(data.compute(TripAnalysis.class, "mode_users.csv"));
              viz.addTrace(
                  BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT).build(),
                  ds.mapping().x("main_mode").y("user").name("main_mode"));

              if (modeUsersRefCsv != null) {
                ds.constant("source", "sim");

                viz.addDataset(data.resource(modeUsersRefCsv)).constant("source", "ref");
    ...
    layout
        ...
        .el(
            ...
            (viz, data) -> {
              ...
              viz.addTrace(
                  BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT).build(),
                  viz.addDataset(data.compute(TripAnalysis.class, "trip_purposes_by_hour.csv"))
                      .mapping()
