...
/**
 * @author jbischoff This is an example how to set different flow capacity consumptions for
 *     different vehicles. Two groups of agents, one equipped with AVs (having an improved flow of
 *     factor 2), the other one using ordinary cars are traveling on two different routes in a grid
 *     network , highlighting the difference between vehicles. Network flow capacities are the same
 *     on all links. All agents try to depart at the same time. The queue is emptied twice as fast
 *     for the agents using an AV.
 */
public class RunAvExample {
  public void run(URL configUrl, boolean otfvis) {
    Scenario scenario =
        ScenarioUtils.loadScenario(ConfigUtils.loadConfig(configUrl, new OTFVisConfigGroup()));
