...
public class VariableAccessTransitRouterImplTest {
	...
	public void testCalcRoute() throws MalformedURLException {
		...
		TransitRouterNetworkTravelTimeAndDisutility transitRouterNetworkTravelTimeAndDisutility = new TransitRouterNetworkTravelTimeAndDisutility(
				trConfig, preparedTransitSchedule);
		...
		FixedDistanceBasedVariableAccessModule variableAccessModule = new FixedDistanceBasedVariableAccessModule(
		...
		VariableAccessTransitRouterImpl router = new VariableAccessTransitRouterImpl(pcsConfig, trConfig,
				preparedTransitSchedule, ptRouterNetwork, travelTime, travelDisutility, variableAccessModule,
				scenario.getNetwork());
		...
		List<Leg> legsAtPtSideOfDecisionPoint = router.calcRoute(homeXCoord2050, workAtPtSideOfDecisionPoint,
				8 * 60 * 60, personCarNeverAvailable);
		...
		List<Leg> legsAtDecisionPoint = router.calcRoute(homeXCoord2050, workAtDecisionPoint, 8 * 60 * 60,
				personCarNeverAvailable);
		...
		List<Leg> legsAtDirectWalkSideOfDecisionPoint = router.calcRoute(homeXCoord2050,
				workAtDirectWalkSideOfDecisionPoint, 8 * 60 * 60, personCarNeverAvailable);
		...
		Assert.assertEquals(Id.create("2122", Link.class),
				legsAtPtSideOfDecisionPoint.get(0).getRoute().getStartLinkId());
		Assert.assertEquals(Id.create("11", Link.class), legsAtPtSideOfDecisionPoint.get(0).getRoute().getEndLinkId());
		Assert.assertEquals(Id.create("11", Link.class),
				legsAtPtSideOfDecisionPoint.get(1).getRoute().getStartLinkId());
		Assert.assertEquals(Id.create("33", Link.class), legsAtPtSideOfDecisionPoint.get(1).getRoute().getEndLinkId());
		Assert.assertEquals(Id.create("33", Link.class),
				legsAtPtSideOfDecisionPoint.get(2).getRoute().getStartLinkId());
		Assert.assertEquals(Id.create("23", Link.class),
				legsAtPtSideOfDecisionPoint.get(2).getRoute().getEndLinkId());  // on a train link!

		Assert.assertEquals(Id.create("2122", Link.class),
				legsAtDirectWalkSideOfDecisionPoint.get(0).getRoute().getStartLinkId());
		Assert.assertEquals(Id.create("23", Link.class),
				legsAtDirectWalkSideOfDecisionPoint.get(0).getRoute().getEndLinkId());
		...
		Assert.assertEquals(1822.0 / 0.641025641025641, legsAtDirectWalkSideOfDecisionPoint.get(0).getTravelTime(),
				0.001);
		...
		List<Leg> legsAtDirectTripSideOfDecisionPoint = router.calcRoute(homeAtDirectTripSideOfDecisionPoint,
