...
public class ReceiversReaderTest {
  ...
  /**
   * This is just a simplified version of {@link #testV2()} with the main difference being that the
   * input file for this test was actually generated from the {@link ReceiversWriterTest#testV2()}
   * class whereas the more complete test below was inherited (and manually adapted) from the
   * original DFG code.
   */
  @Test
  public void testBasicV2() {
    ...
    try {
      new ReceiversReader(receivers)
          .readFile(utils.getClassInputDirectory() + "receivers_v2_basic.xml");
    } catch (Exception e) {
      e.printStackTrace();
    ...
    Assert.assertTrue(
        "Wrong ProductType description", p1.getDescription().equalsIgnoreCase("Product 1"));
  ...
  public void testV2() {
    ...
    try {
      new ReceiversReader(receivers)
          .readFile(utils.getClassInputDirectory() + "receivers_v2_full.xml");
    } catch (Exception e) {
      e.printStackTrace();
      Assert.fail("Should read without exception.");
    }
    /* Receivers */
    Assert.assertTrue(
        "Wrong description.", "Chessboard".equalsIgnoreCase(receivers.getDescription()));
    Object r = receivers.getAttributes().getAttribute("date");
    Assert.assertNotNull("No attribute", r);

    /* Product types */
    Assert.assertEquals(
        "Wrong number of product types.", 2L, receivers.getAllProductTypes().size());
    ...
    Assert.assertNotNull(
        "Could not find ProductType \"P2\"",
        receivers.getProductType(Id.create("P2", ProductType.class)));

    Assert.assertTrue(
        "Wrong ProductType description", pt1.getDescription().equalsIgnoreCase("Product 1"));
    ...
    Assert.assertNotNull(
        "Should find attribute '" + CollaborationUtils.ATTR_GRANDCOALITION_MEMBER + "'",
        r1.getAttributes().getAttribute(CollaborationUtils.ATTR_GRANDCOALITION_MEMBER));
    Assert.assertNotNull(
        "Should find attribute '" + CollaborationUtils.ATTR_COLLABORATION_STATUS + "'",
        r1.getAttributes().getAttribute(CollaborationUtils.ATTR_COLLABORATION_STATUS));
    Assert.assertNotNull(
        "Should find attribute '" + ReceiverUtils.ATTR_RECEIVER_SCORE + "'",
        r1.getAttributes().getAttribute(ReceiverUtils.ATTR_RECEIVER_SCORE));

    /* Time window */
    Assert.assertEquals(
        "Wrong number of time windows.", 2, r1.getSelectedPlan().getTimeWindows().size());
    TimeWindow t1 = r1.getSelectedPlan().getTimeWindows().get(0);
    Assert.assertEquals(
        "Wrong time window start time.",
        Time.parseTime("06:00:00"),
        t1.getStart(),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong time window end time.",
    ...
    Assert.assertEquals(
        "Wrong time window start time.",
        Time.parseTime("15:00:00"),
        t2.getStart(),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong time window end time.",
        Time.parseTime("18:00:00"),
        t2.getEnd(),
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Wrong order quantity.", 5000, o.getOrderQuantity(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong product type.",
        o.getProduct().getProductType().getId(),
        Id.create("P1", ProductType.class));
    Assert.assertEquals("Wrong receiver.", o.getReceiver().getId(), Id.create("1", Receiver.class));
    Assert.assertEquals(
        "Wrong service duration.",
        Time.parseTime("03:00:00"),
        o.getServiceDuration(),
        MatsimTestUtils.EPSILON);
  }
}
