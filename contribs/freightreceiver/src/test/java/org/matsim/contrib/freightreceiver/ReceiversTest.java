  ...
  public void createAndAddProductType() {
    ...
    ProductType test =
        ReceiverUtils.createAndGetProductType(
            receivers, Id.create("test", ProductType.class), Id.createLinkId("j(1,7)"));
    Assert.assertEquals("Wrong number of product types.", 3, receivers.getAllProductTypes().size());
    Assert.assertTrue(
        "Should contain new product types", receivers.getAllProductTypes().contains(test));
  }
  ...
  private Receivers setupReceivers() {
    Scenario scenario =
        ReceiverChessboardScenario.createChessboardScenario(
            1234L, 5, utils.getOutputDirectory(), false);
