...
public class ReceiverChessboardScenario {
  private static final Logger LOG = LogManager.getLogger(ReceiverChessboardScenario.class);

  /** Build the entire chessboard example. */
  public static Scenario createChessboardScenario(
      long seed, int numberOfReceivers, String outputFolder, boolean write) {
    ...
    ConfigUtils.addOrGetModule(sc.getConfig(), ReceiverConfigGroup.class)
        .setReceiverReplanningInterval(5);
    ...
    // needs to be done in iterations startup listener, where it is also done during the iterations.
    //  kai, jan'19
    ...
    CollaborationUtils.linkReceiverOrdersToCarriers(
        ReceiverUtils.getReceivers(sc), CarriersUtils.getCarriers(sc));
  ...
  public static void writeFreightScenario(Scenario sc) {
    ...
    new CarrierVehicleTypeWriter(CarrierVehicleTypes.getVehicleTypes(CarriersUtils.getCarriers(sc)))
        .write(outputFolder + "carrierVehicleTypes.xml");
  }

  /**
   * Creates the product orders for the receiver agents in the simulation. Currently (28/08/2018)
   * all the receivers have the same orders for experiments, but this must be adapted in the future
   * to accept other parameters as inputs to enable different orders per receiver.
   */
  private static void createReceiverOrders(Scenario sc) {
     ...
     */
    Iterator<CarrierVehicle> vehicles =
        carrierOne.getCarrierCapabilities().getCarrierVehicles().values().iterator();
    if (!vehicles.hasNext()) {
    ...
    ProductType productTypeOne =
        ReceiverUtils.createAndGetProductType(
            receivers, Id.create("P1", ProductType.class), carrierOriginLinkId);
    productTypeOne.setDescription("Product 1");
    productTypeOne.setRequiredCapacity(1);

    ProductType productTypeTwo =
        ReceiverUtils.createAndGetProductType(
            receivers, Id.create("P2", ProductType.class), carrierOriginLinkId);
    ...
    for (int r = 1; r < ReceiverUtils.getReceivers(sc).getReceivers().size() + 1; r++) {
      ...
      Receiver receiver =
          receivers.getReceivers().get(Id.create(Integer.toString(r), Receiver.class));
      ...
      Collection<Order> rOrders = new ArrayList<>();
      {
        Order rOrder1 =
            createProductOrder(
                Id.create("Order" + r + "1", Order.class),
                receiver,
                receiverProductOne,
                Time.parseTime(serdur));
      ...
      }
      {
        Order rOrder2 =
            createProductOrder(
                Id.create("Order" + r + "2", Order.class),
                receiver,
                receiverProductTwo,
                Time.parseTime(serdur));
      ...
      ReceiverOrder receiverOrder =
          new ReceiverOrder(receiver.getId(), rOrders, carrierOne.getId());
      ReceiverPlan receiverPlan =
          ReceiverPlan.Builder.newInstance(receiver, true)
              .addReceiverOrder(receiverOrder)
              //					// setting the time window start time of ALL receivers to 08:00 for time window
              // experiments.
  ...
  public static void convertReceiverOrdersToInitialCarrierShipments(
      Carriers carriers, ReceiverOrder receiverOrder, ReceiverPlan receiverPlan) {
    for (Order order : receiverOrder.getReceiverProductOrders()) {

      CarrierShipment.Builder shpBuilder =
      ...
      carriers
          .getCarriers()
          .get(receiverOrder.getCarrierId())
          .getShipments()
  ...
  /**
   * Creates and adds the receivers that are part of the grand coalition. These receivers are
   * allowed to replan their orders as well as decided to join or leave the coalition.
   */
  static void createAndAddChessboardReceivers(Scenario sc, int numberOfReceivers) {
    ...
    for (int r = 1; r < numberOfReceivers + 1; r++) {
      ...
      Receiver receiver =
          ReceiverUtils.newInstance(Id.create(Integer.toString(r), Receiver.class))
  ...
  private static void createChessboardCarriersAndAddToScenario(Scenario sc) {
    ...
    /* Heavy vehicle. */
    CarrierVehicleType.Builder typeBuilderHeavy =
        CarrierVehicleType.Builder.newInstance(Id.create("heavy", VehicleType.class));
    VehicleType typeHeavy =
        typeBuilderHeavy
            .setCapacity(14000)
            .setFixCost(2604)
            ...
            .build();
    CarrierVehicle.Builder carrierHVehicleBuilder =
        CarrierVehicle.Builder.newInstance(Id.createVehicleId("heavy"), carrierLocation, typeHeavy);
    CarrierVehicle heavy =
        carrierHVehicleBuilder
            .setEarliestStart(Time.parseTime("06:00:00"))
            .setLatestEnd(Time.parseTime("18:00:00"))
            .build();

    /* Light vehicle. */
    CarrierVehicleType.Builder typeBuilderLight =
        CarrierVehicleType.Builder.newInstance(Id.create("light", VehicleType.class));
    VehicleType typeLight =
        typeBuilderLight
            .setCapacity(3000)
            .setFixCost(1168)
            ...
            .build();
    CarrierVehicle.Builder carrierLVehicleBuilder =
        CarrierVehicle.Builder.newInstance(Id.createVehicleId("light"), carrierLocation, typeLight);
    CarrierVehicle light =
        carrierLVehicleBuilder
            .setEarliestStart(Time.parseTime("06:00:00"))
            .setLatestEnd(Time.parseTime("18:00:00"))
            .build();
   ...
   * policy.
   *
   * <p>TODO This must be made more generic so that multiple policies can be considered. Currently
   * (2018/04 this is hard-coded to be a min-max (s,S) reordering policy.
   */
  private static ReceiverProduct createReceiverProduct(
      ProductType productType, int minLevel, int maxLevel) {
  ...
  private static Order createProductOrder(
      Id<Order> number, Receiver receiver, ReceiverProduct receiverProduct, double serviceTime) {
    Order.Builder builder = Order.Builder.newInstance(number, receiver, receiverProduct);

    return builder.calculateOrderQuantity().setServiceTime(serviceTime).build();
  }
  ...
  static TimeWindow selectRandomTimeStart(int tw) {
    ...
    Random randomTime =
        MatsimRandom.getLocalInstance(); // overkill, but easiest to retrofit.  kai, jan'19
    int randomStart = (min + randomTime.nextInt(max - tw - min + 1));
