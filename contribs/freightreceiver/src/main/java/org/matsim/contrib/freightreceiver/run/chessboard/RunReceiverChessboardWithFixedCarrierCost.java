...
public class RunReceiverChessboardWithFixedCarrierCost {
  private static final Logger LOG =
      LogManager.getLogger(RunReceiverChessboardWithFixedCarrierCost.class);
  private static final int FIRST_RUN = 1;
  private static final int LAST_RUN = 2;

  public static void main(String[] args) {
    if (args.length > 0) {
      throw new IllegalArgumentException(
          "This class should be self-contained and run without any arguments.");
    }
  ...
  static void run(int run) {
    ...
    String folder =
        String.format(Locale.US, "%s/run_%03d/", ReceiverChessboardParameters.OUTPUT_FOLDER, run);
    ...
    ReceiverModule receiverModule =
        new ReceiverModule(ReceiverUtils.createFixedReceiverCostAllocation(100.0));
  ...
  /**
   * This method ensures that the {@link Carriers}' scores are also updated in the output. TODO This
   * is less elegant than the embedded score stats for the receiver.
   */
  static void prepareFreightOutputDataAndStats(MatsimServices controler) {
    CarrierScoreStats scoreStats =
        new CarrierScoreStats(
            CarriersUtils.getCarriers(controler.getScenario()),
            controler.getScenario().getConfig().controller().getOutputDirectory()
                + "/carrier_scores",
            true);
