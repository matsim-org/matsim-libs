...
/**
 * Generates score stats for receivers. This is adapted from
 * "org.matsim.contrib.freight.usecases.analysis.CarrierScoreStats" by mrieser.
 *
 * @author wlbean
 */
final class ReceiverScoreStats implements StartupListener, IterationEndsListener, ShutdownListener {
  ...
  private static final Logger log = LogManager.getLogger(ReceiverScoreStats.class);
  ...
  public void notifyStartup(final StartupEvent event) {
    String fileName =
        sc.getConfig().controller().getOutputDirectory() + ReceiverUtils.FILENAME_RECEIVER_SCORES;
  ...
  public void notifyIterationEnds(final IterationEndsEvent event) {
    ...
    CarriersUtils.writeCarriers(
        CarriersUtils.getCarriers(sc),
        dir + File.separator + event.getIteration() + CARRIER_PLANS_XML);
    new ReceiversWriter(ReceiverUtils.getReceivers(sc))
        .write(dir + File.separator + event.getIteration() + RECEIVER_PLANS_XML);
  }
  ...
  private void writeIterationScores(IterationEndsEvent event) {
    ...
    log.info(
        "       avg. score of the worst plan of each agent: " + (sumScoreWorst / nofScoreWorst));
  ...
  static void writeHeadings(Scenario sc) {
    try (BufferedWriter bw =
        IOUtils.getBufferedWriter(
            sc.getConfig().controller().getOutputDirectory() + RECEIVER_STATS_CSV)) {
      bw.write(
          String.format(
