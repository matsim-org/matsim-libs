...
/**
 * This class implements a receiver reorder strategy that changes the delivery time window of its
 * orders.
 *
 * @author wlbean
 */
class TimeWindowStrategyManagerFactory implements Provider<ReceiverStrategyManager> {
  ...
  public ReceiverStrategyManager get() {
    ...
    {
      GenericPlanStrategyImpl<ReceiverPlan, Receiver> strategy =
          new GenericPlanStrategyImpl<>(
              (new ExpBetaPlanChanger.Factory<ReceiverPlan, Receiver>())
                  .setBetaValue(10.0)
                  .build());
     ...
     */
    {
      GenericPlanStrategyImpl<ReceiverPlan, Receiver> strategy =
          new GenericPlanStrategyImpl<>(
              new ExpBetaPlanChanger.Factory<ReceiverPlan, Receiver>().setBetaValue(10.).build());
      ...
      strategyManager.addChangeRequest(
          (int) (sc.getConfig().controller().getLastIteration() * 0.9), strategy, null, 0.0);
    }
    ...
    /* Replanning for grand coalition receivers. TODO Ignored for now */
    {
      ...
      //			GenericPlanStrategyImpl<ReceiverPlan, Receiver> strategy = new
      // GenericPlanStrategyImpl<>(new ExpBetaPlanChanger<>(10.));
      //			strategy.addStrategyModule(new CollaborationStatusMutator());
      //			strategyManager.addStrategy(strategy, null, 0.2);
      //			strategyManager.addChangeRequest((int)
      // Math.round((sc.getConfig().controler().getLastIteration()) * 0.9), strategy, null, 0.0);
    }
