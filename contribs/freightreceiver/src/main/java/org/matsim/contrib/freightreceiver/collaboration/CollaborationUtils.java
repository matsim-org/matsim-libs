...
public class CollaborationUtils {
  ...
  /**
   * Ensures that each {@link Receiver}'s {@link ReceiverOrder}s are linked to a {@link Carrier},
   * and does not simply have a pointer to the {@link Carrier}'s {@link Id}.
   */
        ...
        for (ReceiverOrder receiverOrder : plan.getReceiverOrders()) {
          ...
          if (!carriers.getCarriers().containsKey(receiverOrder.getCarrierId())) {
            throw new RuntimeException(
                "Cannot find carrier \""
                    + receiverOrder.getCarrierId().toString()
                    + "\" for receiver \""
  ...
  public static void setCoalitionFromReceiverAttributes(Scenario scenario, Coalition coalition) {
    ...
    LOG.info(
        "Current number of receiver coalition members: "
            + coalition.getReceiverCoalitionMembers().size());
    LOG.info(
        "Current number of carrier coalition members: "
            + coalition.getCarrierCoalitionMembers().size());
    LOG.info(
        "Total number of receiver agents: "
            + ReceiverUtils.getReceivers(scenario).getReceivers().size());
  }
}
