...
/**
 * A cost allocation model where the {@link Carrier} distributes its cost equally among the {@link
 * Receiver}s it services.
 */
class ReceiverCostAllocationEqualProportion implements ReceiverCostAllocation {
  ...
  private List<Id<Receiver>> findAllReceiversServicedByThisCarrier(Id<Carrier> carrierId) {
    ...
    for (Receiver receiver : receivers.getReceivers().values()) {
      Iterator<ReceiverOrder> orderIterator =
          receiver.getSelectedPlan().getReceiverOrders().iterator();
      boolean found = false;
