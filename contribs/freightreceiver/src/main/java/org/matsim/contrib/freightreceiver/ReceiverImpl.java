...
class ReceiverImpl implements Receiver {
  ...
  public Receiver addProduct(ReceiverProduct product) {
    if (products.contains(product)) {
      throw new IllegalArgumentException(
          "receiver already has product " + product + "; not adding it a second time.");
    }
  ...
  /**
   * Returns the link from which the receiver is accessed. TODO One may consider changing this so
   * that it is a list of links.
   */
  @Override
  ...
  public ReceiverProduct getProduct(Id<ProductType> productType) {
    ...
    if (product == null) {
      log.warn(
          "Receiver \""
              + this.id.toString()
              + "\" does not have the requested product type \""
