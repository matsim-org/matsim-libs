...
class ReceiverStrategyManagerImpl implements ReceiverStrategyManager {
  final GenericStrategyManager<ReceiverPlan, Receiver> delegate =
      new GenericStrategyManagerImpl<>();

  @Override
  public void addStrategy(
      GenericPlanStrategy<ReceiverPlan, Receiver> strategy, String subpopulation, double weight) {
    delegate.addStrategy(strategy, subpopulation, weight);
  }

  @Override
  public void run(
      Iterable<? extends HasPlansAndId<ReceiverPlan, Receiver>> persons,
      int iteration,
