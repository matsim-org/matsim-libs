...
class ReceiverTriggersCarrierReplanningListener implements IterationStartsListener {
  ...
  public void notifyIterationStarts(IterationStartsEvent event) {
    ...
    if (event.getIteration() > 0
        && (event.getIteration() + 1)
                % ConfigUtils.addOrGetModule(sc.getConfig(), ReceiverConfigGroup.class)
                    .getReceiverReplanningInterval()
            != 0) {
      return;
    }
    LogManager.getLogger(ReceiverTriggersCarrierReplanningListener.class)
        .info("--> Receiver triggering carrier to replan.");
    ...
    for (Receiver receiver : receivers.values()) {
      ...
      for (ReceiverOrder receiverOrder : receiverPlan.getReceiverOrders()) {
        for (Order order : receiverOrder.getReceiverProductOrders()) {
          nn++;
          CarrierShipment.Builder builder =
              CarrierShipment.Builder.newInstance(
                  Id.create(
                      "Order" + receiverPlan.getReceiver().getId().toString() + nn,
          ...
          CarrierShipment newShipment =
              builder
                  ...
                  // TODO This only looks at the FIRST time window. This may need revision once we
                  // handle multiple
                  // time windows.
                  .build();
    ...
    for (Carrier carrier : carriers.values()) {
      ...
      VehicleRoutingProblem.Builder vrpBuilder =
          MatsimJspritFactory.createRoutingProblemBuilder(carrier, sc.getNetwork());

      NetworkBasedTransportCosts netBasedCosts =
          NetworkBasedTransportCosts.Builder.newInstance(
                  sc.getNetwork(), carrier.getCarrierCapabilities().getVehicleTypes())
              .build();
      ...
      URL algoConfigFileName =
          IOUtils.extendUrl(sc.getConfig().getContext(), "initialPlanAlgorithm.xml");
      VehicleRoutingAlgorithm vra =
          VehicleRoutingAlgorithms.readAndCreateAlgorithm(vrp, algoConfigFileName);
    ...
    //        new CarrierPlanWriter(CarrierControlerUtils.getCarriers(sc)).write(outputdirectory +
    // ReceiverConfigGroup.CARRIERS_FILE);
    new CarrierPlanWriter(CarriersUtils.getCarriers(sc))
        .write(outputdirectory + receiverConfig.getCarriersFile());
    //        new ReceiversWriter( ReceiverUtils.getReceivers( sc ) ).write(outputdirectory +
    // ReceiverConfigGroup.RECEIVERS_FILE);
    new ReceiversWriter(ReceiverUtils.getReceivers(sc))
        .write(outputdirectory + receiverConfig.getReceiversFile());
  }
}
