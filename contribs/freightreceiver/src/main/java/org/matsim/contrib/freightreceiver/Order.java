...
/**
 * A concrete assignment of a receiver product, order quantity, delivery time windows, delivery
 * service time and delivery location.
 *
 * <p>Default value for delivery service time is 1 hour and daily order quantity is 1000 units (5000
 * units per week, 5 deliveries per week).
 *
 * @author wlbean
 *     <p>FIXME Receiver is both at Order and ReceiverOrder level. Necessary?
 */
public final class Order {
  private final Logger log = LogManager.getLogger(Order.class);
  ...
  /**
   * Returns the number of weekly deliveries of a receiver. The default is set to 5 since MATSim
   * runs only for a single day, we assume there is a delivery of the specified Order quantity each
   * day.
   *
   * @return
   */
   ...
   *
   * <p>FIXME I (jwj) removed the time window since it is no longer a single value, but rather a
  ...
  public static class Builder {
    ...
    public static Builder newInstance(
        Id<Order> orderId, Receiver receiver, ReceiverProduct receiverProduct) {
    ...
    private Builder(
        final Id<Order> orderId, final Receiver receiver, final ReceiverProduct receiverProduct) {
      this.orderId = orderId;
      this.receiver = receiver;
      this.receiverProduct = receiverProduct;
    }
    ...
    /**
     * Determines the order quantity in units based on the receivers min and max inventory levels
     * (in units) and create a new order.
     *
     * <p>This should be expanded later on when including demand rate for products.
     */
