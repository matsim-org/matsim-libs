...
/**
 * Like a natural {@link Person} a plan contains the intention of a {@link Receiver} agent. In
 * consequence, all information is <i>expected</i>. This container describes a {@link Receiver}'s
 * behaviour in terms of how orders are placed with different {@link Carrier}s.
 *
...
public final class ReceiverPlan implements BasicPlan, Attributable {
  ...
  public String toString() {
    ...
    strb.append("; collabStatus=")
        .append(this.attributes.getAttribute(CollaborationUtils.ATTR_COLLABORATION_STATUS));
   ...
   *
   * @return true if the time is within at least one of the set time window(s), or <i>if no time
   *     windows are set</i>.
   */
  ...
  public ReceiverPlan createCopy() {
    Builder builder =
        Builder.newInstance(
            receiver,
            (boolean) attributes.getAttribute(CollaborationUtils.ATTR_COLLABORATION_STATUS));
  ...
  /**
   * The constructor mechanism for creating a {@link ReceiverPlan}. Once built the only thing one
   * will be able to change is the score.
   *
   * @author jwjoubert
   */
