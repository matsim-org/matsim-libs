...
class ReceiverAgent {
  ...
  private final Logger log = LogManager.getLogger(ReceiverAgent.class);
  ...
  /**
   * Score the receiver agent's selected order. This score reflects the receiver cost and is
   * currently determined as the carrier's delivery cost to that receiver (based on the proportion
   * of this receiver's orders in all the orders delivered by the carrier). This is not really
   * realistic, and will be changed in the future.
   *
   * <p>FIXME: JWJ (23/6/2018): I'm not quite sure what the purpose of this method is. I've updated
   * it so that the plan's cost is simply the sum of all individual order's costs.
   *
  ...
  public void scoreSelectedPlan(Scenario scenario, ReceiverCostAllocation costAllocation) {
    ...
    receiver
        .getAttributes()
        .putAttribute(ReceiverUtils.ATTR_RECEIVER_SCORE, scoringFunction.getScore());
  }
