...
public class ReceiverUtils {
  ...
  // Now using the MATSim-infrastructure to avoid problems if this element is named differently
  // beetween (MATSim) CarrierControlerUtils and here.
  ...
  public static ProductType createAndGetProductType(
      Receivers receivers, Id<ProductType> typeId, Id<Link> originLinkId) {
  ...
  /**
   * A cost allocation model where the {@link Carrier}(s) charge a fixed amount to the receivers.
   * One cannot expect to much behavioral response in this scenario.
   *
   * @param cost the amount charged by the {@link Carrier}. It is assumed that this is a
   *     <b><i>cost</i></b>, so a positive value will have a negative utility on the receiver.
   */
  ...
  /**
   * A cost allocation model where each {@link Carrier} charges an equal amount to each of the
   * receivers it services, irrespective of the number, size or value of the {@link Receiver}'s
   * order.
   */
