...
/**
 * This class implements a receiver reorder strategy that changes the delivery service time of its
 * orders.
 *
 * @author wlbean
 */
final class ServiceTimeStrategyManagerProvider implements Provider<ReceiverStrategyManager> {
  ...
  public ReceiverStrategyManager get() {
...

    {
      ...
      GenericPlanStrategy<ReceiverPlan, Receiver> strategy =
          new GenericPlanStrategyImpl<>(
              new ExpBetaPlanChanger.Factory<ReceiverPlan, Receiver>().build());
...

    {
      ...
      GenericPlanStrategyImpl<ReceiverPlan, Receiver> increaseStrategy =
          new GenericPlanStrategyImpl<>(
              new ExpBetaPlanChanger.Factory<ReceiverPlan, Receiver>().build());
      increaseStrategy.addStrategyModule(
          new ServiceTimeMutator(Time.parseTime("0:15:00"), Time.parseTime("04:00:00"), true));
      strategyManager.addStrategy(increaseStrategy, null, 0.15);
      strategyManager.addChangeRequest(
          (int) (sc.getConfig().controller().getLastIteration() * 0.9),
          increaseStrategy,
          null,
          0.0);
    }

    {
       ...
       * The strategy is witched off after 90% of the number of iterations. */
      GenericPlanStrategyImpl<ReceiverPlan, Receiver> decreaseStrategy =
          new GenericPlanStrategyImpl<>(
              new ExpBetaPlanChanger.Factory<ReceiverPlan, Receiver>().build());
      decreaseStrategy.addStrategyModule(
          new ServiceTimeMutator(Time.parseTime("00:15:00"), Time.parseTime("01:00:00"), false));
    ...
    //			GenericPlanStrategyImpl<ReceiverPlan, Receiver> strategy = new GenericPlanStrategyImpl<>(
    // new ExpBetaPlanSelector<>( 10. ) );
