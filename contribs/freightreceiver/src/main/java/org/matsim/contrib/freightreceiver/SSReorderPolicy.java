...
/**
 * Basic implementation of an (s,S) reordering policy. That is, as soon as the current inventory
 * level reaches a value below 's', the product is replenished by a quantity that will get the
 * inventory back up to 'S'.
 *
 * @author jwjoubert
 */
final class SSReorderPolicy implements ReorderPolicy {
  final String policyName = "(s,S)";
  private final String attributeMinimumLevel = "s";
  private final String attributeMaximumLevel = "S";
  private final Attributes attributes = new AttributesImpl();

  /**
   * This class should be instantiated through {@link ReceiverUtils#createSSReorderPolicy(double,
   * double)}.
   */
  SSReorderPolicy(double minLevel, double maxLevel) {
    ...
    // (these need to be attributes since they are written to file and read back in.  Yet, they are
    // policy specific, so
  ...
  public double getMinimumStockLevel() {
    return Double.parseDouble(
        Objects.requireNonNull(attributes.getAttribute(attributeMinimumLevel)).toString());
  }

  /** Get the upper limit 'S' of the (s,S) stock ordering policy. */
  public double getMaximumStockLevel() {
    return Double.parseDouble(
        Objects.requireNonNull(attributes.getAttribute(attributeMaximumLevel)).toString());
  }
