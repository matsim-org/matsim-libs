...
/**
 * This controller ensures that each receiver receives a cost (score) per order at the end of each
 * iteration and replans its orders based on the cost of the previous iteration and past iterations.
 *
 * @author wlbean, jwjoubert
 */
class ReceiverControlerListener
  ...
  private static final Logger LOG = LogManager.getLogger(ReceiverControlerListener.class);
  ...
  @Inject
  private ReceiverControlerListener(
      ReceiverStrategyManager strategyManager,
  ...
  public void notifyReplanning(final ReplanningEvent event) {
    ...
    for (Receiver receiver : ReceiverUtils.getReceivers(sc).getReceivers().values()) {
      if ((event.getIteration() - 1)
              % ConfigUtils.addOrGetModule(sc.getConfig(), ReceiverConfigGroup.class)
                  .getReceiverReplanningInterval()
          == 0) {
       ...
       */
      if ((boolean)
          receiver.getAttributes().getAttribute(CollaborationUtils.ATTR_GRANDCOALITION_MEMBER)) {
      ...
      //		GenericPlanStrategyImpl<ReceiverPlan, Receiver> strategy = new
      // GenericPlanStrategyImpl<>(new KeepSelected<ReceiverPlan, Receiver>());
      //		strategy.addStrategyModule(new CollaborationStatusMutator());
      //		collaborationStratMan.addStrategy(strategy, null, 0.2);
      //		collaborationStratMan.addChangeRequest((int)
      // Math.round((fsc.getScenario().getConfig().controler().getLastIteration())*0.8), strategy,
      // null, 0.0);
      ...
      strategyManager.run(
          receiverControlCollection, event.getIteration(), event.getReplanningContext());
  ...
  public void notifyScoring(ScoringEvent event) {
    if (event.getIteration() == 0) {
      ...
      for (Receiver receiver : ReceiverUtils.getReceivers(sc).getReceivers().values()) {
        double score =
            (double) receiver.getAttributes().getAttribute(ReceiverUtils.ATTR_RECEIVER_SCORE);
  ...
  private void linkReceiverTimeWindowToCarrierTourPosition() throws IOException {
    ...
    for (Receiver receiver : receivers.getReceivers().values()) {
      ...
      //                LOG.error("There are multiple receivers at the same link Id. Fix how this is
      // dealt with.");
    ...
    try (BufferedWriter bw =
        IOUtils.getBufferedWriter(
            this.sc.getConfig().controller().getOutputDirectory()
                + "output_receiverInTourPlacement.csv.gz")) {
      bw.write(
          "receiverId,twStart,twEnd,twDuration,positionInTour,product,deliveryStart,deliveryEnd");
      bw.newLine();
          ...
          for (int i = 0; i < tour.getTour().getTourElements().size(); i++) {
            ...
            if (element instanceof Tour.ShipmentBasedActivity act) {
              ...
              if (act.getActivityType().equalsIgnoreCase("delivery")) {
                ...
                String twStart =
                    Time.writeTime(
                        thisReceiver.getSelectedPlan().getTimeWindows().get(0).getStart());
                String twEnd =
                    Time.writeTime(thisReceiver.getSelectedPlan().getTimeWindows().get(0).getEnd());
