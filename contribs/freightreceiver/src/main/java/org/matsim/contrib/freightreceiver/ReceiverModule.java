...
public final class ReceiverModule extends AbstractModule {
  private static final Logger LOG = LogManager.getLogger(ReceiverModule.class);
   ...
   *
   * @param costAllocation an instance of {@link ReceiverCostAllocation} that specifies how the
   *     {@link Carrier} costs are to be allocated to the different {@link Receiver}s.
   */
  ...
  public void install() {
    ...
    configGroup =
        ConfigUtils.addOrGetModule(
            this.getConfig(), ReceiverConfigGroup.NAME, ReceiverConfigGroup.class);
    ...
    bind(ReceiverScoringFunctionFactory.class)
        .toInstance(new ReceiverScoringFunctionFactoryMoneyOnly());
    ...
    if (this.replanningType != null) {
      ...
      if (currentType != this.replanningType) {
        LOG.warn(
            "   Overwriting the receiver replanning: was '"
                + currentType
                + "'; now '"
                + this.replanningType
                + "'");
    ...
    switch (configGroup.getReplanningType()) {
      case timeWindow -> bind(ReceiverStrategyManager.class)
          .toProvider(
              ReceiverReplanningUtils.createStrategyManager(ReceiverReplanningType.timeWindow));
      case serviceTime -> bind(ReceiverStrategyManager.class)
          .toProvider(
              ReceiverReplanningUtils.createStrategyManager(ReceiverReplanningType.serviceTime));
      case orderFrequency -> bind(ReceiverStrategyManager.class)
          .toProvider(
              ReceiverReplanningUtils.createStrategyManager(ReceiverReplanningType.orderFrequency));
      default -> throw new RuntimeException(
          "Strategy manager for '" + configGroup.getReplanningType() + "' not implemented yet!!");
    }
    ...
    //        CarrierScoreStats scoreStats = new CarrierScoreStats( ReceiverUtils.getCarriers(
    // controler.getScenario() ),
    // controler.getScenario().getConfig().controler().getOutputDirectory() + "/carrier_scores",
    // this.createPNG);
