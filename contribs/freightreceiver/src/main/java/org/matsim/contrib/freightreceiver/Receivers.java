...
public final class Receivers implements Attributable {
  ...
  /**
   * Returns the receivers in the container. The design choice (May'21) was that this should be
   * unmodifiable. If you want to edit a {@link Receiver}, you cannot access it via this map, but
   * should rather use {@link Receivers#getReceiver(Id)}.
   */
  ...
  /**
   * Creates and adds a product type. Each product type is a unique stock keeping unit (SKU). When
   * the product type is created, it is registered here inside the receivers container to ensure all
   * unique SKUs are used consistently.
   */
  ProductType createAndAddProductType(final Id<ProductType> typeId, final Id<Link> originLinkId) {
    if (this.productTypeMap.containsKey(typeId)) {
      log.warn(
          "Product type '" + typeId.toString() + "' already exists. Returning the existing value.");
  ...
  /**
   * Gets a registered {@link ProductType} that represents a unique SKU. If the product type has not
   * been registered, the code will crash with a {@link RuntimeException}.
   */
