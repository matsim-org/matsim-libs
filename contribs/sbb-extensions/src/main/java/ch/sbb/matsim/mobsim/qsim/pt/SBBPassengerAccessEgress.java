 ...
 * some methods with small adaptations, other directly copied. The aforementioned class is
 * package-protected and thus not visible from this package, making it impossible to inherit from it
 * and re-use parts of the code.
 *
 * @author mrieser / SBB
 */
public class SBBPassengerAccessEgress implements PassengerAccessEgress {
  ...
  /**
   * Allows passengers to leave and/or board a vehicle according to the vehicle's accessTime,
   * egressTime and doorOperation mode.
   *
   * <p>The code is to a large part a copy of
   * PassengerAccessEgressImpl.calculateStopTimeAndTriggerBoarding. It could not be directly used as
   * the class itself is package-protected and not visible in our package.
   *
   * @return 0.0 (no more agents currently to board or leave), or 1.0 (there were passenger actions
   *     this time step, need to recheck next time step again)
   */
  double handlePassengersWithPhysicalLimits(
      TransitStopFacility stop,
      TransitVehicle vehicle,
      TransitLine line,
      TransitRoute route,
      List<TransitRouteStop> upcomingStops,
      double now) {
    ArrayList<PTPassengerAgent> passengersLeaving = findPassengersLeaving(vehicle, stop);
    int freeCapacity =
        vehicle.getPassengerCapacity() - vehicle.getPassengers().size() + passengersLeaving.size();
    List<PTPassengerAgent> passengersEntering =
        findPassengersEntering(route, line, vehicle, stop, upcomingStops, freeCapacity, now);

    TransitStopHandler stopHandler = vehicle.getStopHandler();
    double stopTime =
        stopHandler.handleTransitStop(
            stop, now, passengersLeaving, passengersEntering, this, vehicle);
    if (stopTime == 0.0) { // (de-)boarding is complete when the additional stopTime is 0.0
      if (this.isGeneratingDeniedBoardingEvents) {
        List<PTPassengerAgent> stillWaiting =
            findAllPassengersWaiting(route, line, vehicle, stop, upcomingStops, now);
  ...
  /**
   * Allows all passengers wanting to leave the vehicle to do so immediately in the current time
   * step, not taking constraints like number of doors and their passenger capacity into account.
   * Allows all passengers wanting to board the vehicle to do so immediately, given there is still
   * some free capacity left in the vehicle.
   */
  double handleAllPassengersImmediately(
      TransitStopFacility stop,
      TransitVehicle vehicle,
      TransitLine line,
      TransitRoute route,
    ...
    List<PTPassengerAgent> boardingPassengers =
        findPassengersEntering(route, line, vehicle, stop, upcomingStops, freeCapacity, now);
  ...
  public boolean handlePassengerLeaving(
      PTPassengerAgent passenger, MobsimVehicle vehicle, Id<Link> toLinkId, double time) {
  ...
  private ArrayList<PTPassengerAgent> findPassengersLeaving(
      TransitVehicle vehicle, final TransitStopFacility stop) {
  ...
  private List<PTPassengerAgent> findAllPassengersWaiting(
      TransitRoute transitRoute,
      TransitLine transitLine,
      TransitVehicle vehicle,
      final TransitStopFacility stop,
  ...
  private void fireBoardingDeniedEvents(
      TransitVehicle vehicle, double now, List<PTPassengerAgent> agents) {
