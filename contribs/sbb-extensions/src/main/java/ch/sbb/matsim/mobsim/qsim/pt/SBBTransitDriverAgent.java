...
public class SBBTransitDriverAgent extends TransitDriverAgentImpl {
  ...
  SBBTransitDriverAgent(
    ...
    this.accessEgress =
        new SBBPassengerAccessEgress(
            internalInterface,
            agentTracker,
            internalInterface.getMobsim().getScenario(),
            this.eventsManager);
    checkCurrentRoute();
  }
  ...
  public double handleTransitStop(TransitStopFacility stop, double now) {
    ...
    double stopTime =
        this.accessEgress.handlePassengersWithPhysicalLimits(
            stop,
            this.getVehicle(),
            this.getTransitLine(),
            this.currentTransitRoute,
            this.remainingRouteStops,
            now);

    if (stopTime <= 0.0) {
      // figure out if it's already time to depart or not
      double departureOffset =
          this.currentStop.getDepartureOffset().or(this.currentStop.getArrivalOffset()).seconds();
  ...
  private void assertExpectedStop(final TransitStopFacility stop) {
    ...
    if (stop != this.nextStop.getStopFacility() || this.currentStop != null) {
      throw new RuntimeException(
          "Expected stop " + this.nextStop.getStopFacility().getId() + ", got " + stop.getId());
    }
  }
  ...
  private void processVehicleArrival(final TransitStopFacility stop, final double now) {
    if (this.currentStop == null) {
      ...
      this.eventsManager.processEvent(
          new VehicleArrivesAtFacilityEvent(now, this.getVehicle().getId(), stop.getId(), 0.0));
      this.remainingRouteStops.removeFirst();
      this.nextStop =
          this.remainingRouteStops.isEmpty() ? null : this.remainingRouteStops.getFirst();
    }
  }
  ...
  private void processVehicleDeparture(final TransitStopFacility stop, final double now) {
    if (this.currentStop != null) {
      ...
      this.eventsManager.processEvent(
          new VehicleDepartsAtFacilityEvent(now, this.getVehicle().getId(), stop.getId(), 0.0));
    }
  }
}
