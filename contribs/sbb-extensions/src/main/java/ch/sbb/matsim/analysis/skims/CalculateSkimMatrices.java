...
/**
 * Main class to calculate skim matrices. Provides a main-method to be directly started from the
 * command line, but the main-method also acts as a template for custom code using the skims
 * calculation.
 *
 * <p>All calculated matrices are written to files with fixed names (see constants in this class) in
 * an output directory.
 *
 * @author mrieser / SBB
 */
public class CalculateSkimMatrices {
  ...
  public static void main(String[] args) throws IOException {
    ...
    skims.calculateSamplingPointsPerZoneFromFacilities(
        facilitiesFilename,
        numberOfPointsPerZone,
        zonesShapeFilename,
    ...
    // skims.calculateSamplingPointsPerZoneFromNetwork(networkFilename, numberOfPointsPerZone,
    // zonesShapeFilename, zonesIdAttributeName, r);
    ...
    if (modes.contains(TransportMode.car)) {
      skims.calculateAndWriteNetworkMatrices(
          networkFilename, eventsFilename, timesCar, config, "", l -> true);
    }

    if (modes.contains(TransportMode.pt)) {
      skims.calculateAndWritePTMatrices(
          networkFilename,
  ...
  public final void calculateSamplingPointsPerZoneFromFacilities(
      String facilitiesFilename,
      int numberOfPointsPerZone,
      String zonesShapeFilename,
      String zonesIdAttributeName,
      Random r,
      ToDoubleFunction<ActivityFacility> weightFunction)
      throws IOException {
    // load facilities
    ...
    new MatsimFacilitiesReader(
            null,
            null,
            new StreamingFacilities(
                f -> {
    ...
    selectSamplingPoints(
        facilities, numberOfPointsPerZone, zonesShapeFilename, zonesIdAttributeName, r);
  }

  public final void calculateSamplingPointsPerZoneFromNetwork(
    ...
    selectSamplingPoints(
        weightedNodes, numberOfPointsPerZone, zonesShapeFilename, zonesIdAttributeName, r);
  }

  public final void selectSamplingPoints(
      List<WeightedCoord> locations,
      int numberOfPointsPerZone,
      String zonesShapeFilename,
      String zonesIdAttributeName,
      Random r)
    ...
    Collection<SimpleFeature> zones =
        new ShapeFileReader().readFileAndInitialize(zonesShapeFilename);
    ...
    try (BufferedReader reader = IOUtils.getBufferedReader(filename)) {
      ...
      if (!expectedHeader.equals(header)) {
        throw new RuntimeException(
            "Bad header, expected '" + expectedHeader + "', got: '" + header + "'.");
      }
      String line;
      int maxIdx = 0;
  ...
  public final void calculateAndWriteBeelineMatrix() throws IOException {
    ...
    FloatMatrix<String> beelineMatrix =
        BeelineDistanceMatrix.calculateBeelineDistanceMatrix(
            this.coordsPerZone.keySet(), coordsPerZone, numberOfThreads);
  ...
  public final FloatMatrix<String> calculateBeelineMatrix() {
    log.info("calc beeline distance matrix");
    return BeelineDistanceMatrix.calculateBeelineDistanceMatrix(
        this.coordsPerZone.keySet(), coordsPerZone, numberOfThreads);
  }

  public final void calculateAndWriteNetworkMatrices(
      String networkFilename,
    ...
    var netIndicators =
        calculateNetworkMatrices(networkFilename, eventsFilename, times, config, xy2linksPredicate);
    log.info(
        "write CAR matrices to "
            + outputDirectory
            + (prefix.isEmpty() ? "" : (" with prefix " + prefix)));
    FloatMatrixIO.writeAsCSV(
        netIndicators.travelTimeMatrix, outputDirectory + "/" + prefix + CAR_TRAVELTIMES_FILENAME);
    FloatMatrixIO.writeAsCSV(
        netIndicators.distanceMatrix, outputDirectory + "/" + prefix + CAR_DISTANCES_FILENAME);
  }

  public final NetworkIndicators<String> calculateNetworkMatrices(
      String networkFilename,
      String eventsFilename,
      double[] times,
      Config config,
    ...
    if (eventsFilename != null) {
      ...
      TravelTimeCalculator ttc =
          TravelTimeCalculator.create(scenario.getNetwork(), config.travelTimeCalculator());
      ...
      log.info(
          "No events specified. Travel Times will be calculated with free speed travel times.");
    }
    ...
    new TransportModeNetworkFilter(scenario.getNetwork())
        .filter(carNetwork, Collections.singleton(TransportMode.car));
    ...
    NetworkIndicators<String> netIndicators =
        NetworkSkimMatrices.calculateSkimMatrices(
    ...
    if (tt instanceof FreeSpeedTravelTime) {
      ...
      for (int i = 1; i < times.length; i++) {
        ...
        NetworkIndicators<String> indicators2 =
            NetworkSkimMatrices.calculateSkimMatrices(
  ...
  private Network extractXy2LinksNetwork(
      Network network, Predicate<Link> xy2linksPredicate, Config config) {
  ...
  public final void calculateAndWritePTMatrices(
      String networkFilename,
    ...
    var matrices =
        calculatePTMatrices(
            networkFilename, transitScheduleFilename, startTime, endTime, config, trainDetector);
    writePTMatricesAsCSV(matrices, outputPrefix);
  }

  public final void writePTMatricesAsCSV(
      PTSkimMatrices.PtIndicators<String> matrices, String outputPrefix) throws IOException {
    String prefix = outputPrefix == null ? "" : outputPrefix;

    log.info(
        "write PT matrices to "
            + outputDirectory
            + (prefix.isEmpty() ? "" : (" with prefix " + prefix)));
    FloatMatrixIO.writeAsCSV(
        matrices.adaptionTimeMatrix, outputDirectory + "/" + prefix + PT_ADAPTIONTIMES_FILENAME);
    FloatMatrixIO.writeAsCSV(
        matrices.frequencyMatrix, outputDirectory + "/" + prefix + PT_FREQUENCIES_FILENAME);
    FloatMatrixIO.writeAsCSV(
        matrices.distanceMatrix, outputDirectory + "/" + prefix + PT_DISTANCES_FILENAME);
    FloatMatrixIO.writeAsCSV(
        matrices.travelTimeMatrix, outputDirectory + "/" + prefix + PT_TRAVELTIMES_FILENAME);
    FloatMatrixIO.writeAsCSV(
        matrices.accessTimeMatrix, outputDirectory + "/" + prefix + PT_ACCESSTIMES_FILENAME);
    FloatMatrixIO.writeAsCSV(
        matrices.egressTimeMatrix, outputDirectory + "/" + prefix + PT_EGRESSTIMES_FILENAME);
    FloatMatrixIO.writeAsCSV(
        matrices.transferCountMatrix, outputDirectory + "/" + prefix + PT_TRANSFERCOUNTS_FILENAME);
    FloatMatrixIO.writeAsCSV(
        matrices.trainTravelTimeShareMatrix,
        outputDirectory + "/" + prefix + PT_TRAINSHARE_BYTIME_FILENAME);
    FloatMatrixIO.writeAsCSV(
        matrices.trainDistanceShareMatrix,
        outputDirectory + "/" + prefix + PT_TRAINSHARE_BYDISTANCE_FILENAME);
  }

  public final PTSkimMatrices.PtIndicators<String> calculatePTMatrices(
      String networkFilename,
      String transitScheduleFilename,
      double startTime,
      double endTime,
    ...
    PTSkimMatrices.PtIndicators<String> matrices =
        PTSkimMatrices.calculateSkimMatrices(
            raptorData,
            this.coordsPerZone,
            startTime,
            endTime,
