...
/**
 * Creates a huge csv file with the data of all the matrices combined. Zones are identified by their
 * identifier as well as one random coordinate per zone. Given the from-/to-coordinates for each
 * relation, the data can be easily visualized with Via's OD-Aggregator.
 *
 * <p>Please note that this process might require substantial memory, as all the data first needs to
 * be loaded into memory before it is written out again.
 *
 * @author mrieser
 */
public class MatricesToXY {

  private static final Logger log = LogManager.getLogger(MatricesToXY.class);

  public static void main(String[] args) throws IOException {
    String zonesShapeFilename = args[0]; // path to a shape-file, e.g. /path/to/my-zones.shp
    String zonesIdAttributeName =
        args[1]; // name of the zone attribute to be used as identifier, e.g. "ZONE_ID"
    String matricesDirectory =
        args[2]; // path to the directory containing all the skim-matrices, e.g. /path/to/skims/
    String xyCsvOutputFilename =
        args[3]; // path to the csv-file to be written, e.g. /path/to/skim-data.csv
    ...
    FloatMatrixIO.readAsCSV(
        carTravelTimes,
        new File(matricesDirectory, CalculateSkimMatrices.CAR_TRAVELTIMES_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        carDistances,
        new File(matricesDirectory, CalculateSkimMatrices.CAR_DISTANCES_FILENAME).getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptAdaptionTimes,
        new File(matricesDirectory, CalculateSkimMatrices.PT_ADAPTIONTIMES_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptFrequencies,
        new File(matricesDirectory, CalculateSkimMatrices.PT_FREQUENCIES_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptDistances,
        new File(matricesDirectory, CalculateSkimMatrices.PT_DISTANCES_FILENAME).getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptTravelTimes,
        new File(matricesDirectory, CalculateSkimMatrices.PT_TRAVELTIMES_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptAccessTimes,
        new File(matricesDirectory, CalculateSkimMatrices.PT_ACCESSTIMES_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptEgressTimes,
        new File(matricesDirectory, CalculateSkimMatrices.PT_EGRESSTIMES_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptTransferCounts,
        new File(matricesDirectory, CalculateSkimMatrices.PT_TRANSFERCOUNTS_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptRailShareDistances,
        new File(matricesDirectory, CalculateSkimMatrices.PT_TRAINSHARE_BYDISTANCE_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        ptRailShareTimes,
        new File(matricesDirectory, CalculateSkimMatrices.PT_TRAINSHARE_BYTIME_FILENAME)
            .getAbsolutePath(),
        id -> id);
    ...
    FloatMatrixIO.readAsCSV(
        beelineDistances,
        new File(matricesDirectory, CalculateSkimMatrices.BEELINE_DISTANCE_FILENAME)
            .getAbsolutePath(),
        id -> id);
