...
/**
 * Calculates zone-to-zone matrices containing a number of performance indicators related to modes
 * routed on a network.
 *
 * <p>Inspired by
 * https://github.com/moeckel/silo/blob/siloMatsim/silo/src/main/java/edu/umd/ncsg/transportModel/Zone2ZoneTravelTimeListener.java.
 *
 * <p>Idea of the algorithm: - given n points per zone - find the nearest link and thereof the
 * to-node for each point - this results in n nodes per zone (where some nodes can appear multiple
 * times, this is wanted as it acts as a weight/probability) - for each zone-to-zone combination,
 * calculate the travel times for each node to node combination. - this results in n x n travel
 * times per zone-to-zone combination. - average the n x n travel times and store this value as the
...
public final class NetworkSkimMatrices {
  ...
  public static <T> NetworkIndicators<T> calculateSkimMatrices(
      Network xy2lNetwork,
    ...
    Counter counter =
        new Counter(
            "CAR-TravelTimeMatrix-" + Time.writeTime(departureTime) + " zone ",
            " / " + coordsPerZone.size());
    ...
    for (int i = 0; i < numberOfThreads; i++) {
      RowWorker<T> worker =
          new RowWorker<>(
              ...
              counter);
      threads[i] =
          new Thread(worker, "CAR-TravelTimeMatrix-" + Time.writeTime(departureTime) + "-" + i);
      threads[i].start();
    }
  ...
  private static class RowWorker<T> implements Runnable {
    ...
    RowWorker(
        ...
        SpeedyGraph graph,
        Map<T, Node[]> nodesPerZone,
        NetworkIndicators<T> networkIndicators,
        double departureTime,
        TravelTime travelTime,
        TravelDisutility travelDisutility,
        Counter counter) {
    ...
    public void run() {
      LeastCostPathTree lcpTree =
          new LeastCostPathTree(this.graph, this.travelTime, this.travelDisutility);
      while (true) {
            ...
            for (T toZoneId : this.destinationZones) {
              ...
              if (toNodes != null) {
                for (Node toNode : toNodes) {
                  ...
                  double currTime =
                      currOptionalTime.orElseThrow(() -> new RuntimeException("Undefined Time"));
                ...
                this.networkIndicators.travelTimeMatrix.set(
                    fromZoneId, toZoneId, Float.POSITIVE_INFINITY);
                this.networkIndicators.distanceMatrix.set(
                    fromZoneId, toZoneId, Float.POSITIVE_INFINITY);
              }
            }
          }
