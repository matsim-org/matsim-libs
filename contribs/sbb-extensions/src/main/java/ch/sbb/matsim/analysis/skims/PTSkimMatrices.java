...
/**
 * Calculates zone-to-zone matrices containing a number of performance indicators related to public
 * transport.
 *
 * <p>Amongst the performance indicators are: - travel time (measured from the first departure to
 * the last arrival, without access/egress time) - travel distance (measured from first departure to
 * last arrival, without access/egress time) - access time - egress time - perceived frequency -
 * share of travel time within trains - share of travel distance within trains
 *
 * <p>The main idea to calculate the frequencies follows the rooftop-algorithm from Niek Guis (ca.
 * 2015).
 *
 * <p>Idea of the algorithm for a single origin-destination (OD) pair: - Given a time window (e.g.
 * 07:00 - 08:00), - find all possible (useful) connections between O and D - for each minute in the
 * time window, calculate the required adaption time to catch the next best connection (can be x
 * minutes earlier or later) - average the minutely adaption times over the time window - based on
 * the average adaption time, the service frequency can be calculated
 *
 * <p>Idea of the algorithm for a full zone-to-zone matrix: - given n points per zone - for each
 * point, find the possible stops to be used as departure or arrival stops. - for each zone-to-zone
 * combination, calculate the average adaption time to travel from each point to each other point in
 * the destination zone. - this results in n x n average adaption travel times per zone-to-zone
 * combination. - average the n x n adaption times and store this value as the zone-to-zone adaption
 * time.
 *
 * <p>A basic implementation for calculating the travel times between m zones would result in m^2 *
 * n^2 pt route calculations, which could take a very long time. The actual algorithm makes use of
 * LeastCostPathTrees, reducing the computational effort down to the calculation of m*n
 * LeastCostPathTrees. In addition, it supports running the calculation in parallel to reduce the
 * time required to compute one matrix.
 *
 * <p>If no connection can be found between two zones (can happen when there is no transit stop in a
 * zone), the corresponding matrix cells contain the value "0" for the perceived frequency, and
...
public class PTSkimMatrices {
  ...
  public static <T> PTSkimMatrices.PtIndicators<T> calculateSkimMatrices(
    ...
    Counter counter =
        new Counter(
            "PT-FrequencyMatrix-"
                + Time.writeTime(minDepartureTime)
                + "-"
                + Time.writeTime(maxDepartureTime)
                + " zone ",
            " / " + coordsPerZone.size());
  ...
  static class RowWorker<T> implements Runnable {
    ...
    RowWorker(
        ConcurrentLinkedQueue<T> originZones,
        Set<T> destinationZones,
        Map<T, Coord[]> coordsPerZone,
        PtIndicators<T> pti,
    ...
    private static Collection<TransitStopFacility> findStopCandidates(
        Coord coord, SwissRailRaptor raptor, RaptorParameters parameters) {
      Collection<TransitStopFacility> stops =
          raptor
              .getUnderlyingData()
              .findNearbyStops(coord.getX(), coord.getY(), parameters.getSearchRadius());
      if (stops.isEmpty()) {
        TransitStopFacility nearest =
            raptor.getUnderlyingData().findNearestStop(coord.getX(), coord.getY());
        ...
        stops =
            raptor
                ...
                .findNearbyStops(
                    coord.getX(),
                    coord.getY(),
                    nearestStopDistance + parameters.getExtensionRadius());
      }
      return stops;
    }
    ...
    private void calcForRow(T fromZoneId, Coord fromCoord) {
      ...
      Collection<TransitStopFacility> fromStops =
          findStopCandidates(fromCoord, this.raptor, this.parameters);
      ...
      for (double time = this.minDepartureTime - timeWindow;
          time < endTime;
          time += this.stepSize) {
        Map<Id<TransitStopFacility>, TravelInfo> tree =
            this.raptor.calcTree(fromStops, time, this.parameters, null);
        trees.add(tree);
      }
    ...
    private void calcForOD(
        T fromZoneId,
        T toZoneId,
        Coord toCoord,
        Map<Id<TransitStopFacility>, Double> accessTimes,
        List<Map<Id<TransitStopFacility>, TravelInfo>> trees) {
      double walkSpeed = this.parameters.getBeelineWalkSpeed();

      Collection<TransitStopFacility> toStops =
          findStopCandidates(toCoord, this.raptor, this.parameters);
      ...
      double avgAdaptionTime =
          RooftopUtils.calcAverageAdaptionTime(connections, minDepartureTime, maxDepartureTime);

      this.pti.adaptionTimeMatrix.add(fromZoneId, toZoneId, (float) avgAdaptionTime);

      Map<ODConnection, Double> connectionShares =
          RooftopUtils.calcConnectionShares(connections, minDepartureTime, maxDepartureTime);
    ...
    private List<ODConnection> buildODConnections(
        List<Map<Id<TransitStopFacility>, TravelInfo>> trees,
        Map<Id<TransitStopFacility>, Double> accessTimes,
  ...
  public static class PtIndicators<T> {
    ...
    public final FloatMatrix<T>
        dataCountMatrix; // how many values/routes were taken into account to calculate the averages
