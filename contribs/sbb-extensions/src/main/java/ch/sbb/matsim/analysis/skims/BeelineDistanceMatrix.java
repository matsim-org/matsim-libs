 ...
 *
 * <p>Idea of the algorithm: - select n random points per zone - for each zone-to-zone combination,
 * calculate the beeline distance from each point to each other point in the destination zone. -
 * this results in n x n distances per zone-to-zone combination. - average the n x n distances and
 * store this value as the zone-to-zone distance.
 *
 * @author mrieser / SBB
 */
public final class BeelineDistanceMatrix {
  ...
  public static <T> FloatMatrix<T> calculateBeelineDistanceMatrix(
      Set<T> zoneIds, Map<T, Coord[]> coordsPerZone, int numberOfThreads) {
    // prepare calculation
    ...
    for (int i = 0; i < numberOfThreads; i++) {
      BeelineDistanceMatrix.RowWorker<T> worker =
          new BeelineDistanceMatrix.RowWorker<>(
              originZones, zoneIds, coordsPerZone, matrix, counter);
