...
/**
 * Helper methods to write and read matrices as CSV files (well, actually semi-colon separated
 * files).
 *
 * @author mrieser / SBB
 */
public final class FloatMatrixIO {
  ...
  private static final String HEADER = "FROM" + SEP + "TO" + SEP + "VALUE";
  ...
  private static <T> void writeCSV(FloatMatrix<T> matrix, BufferedWriter writer)
      throws IOException {
    writer.write(HEADER);
    writer.write(NL);
  ...
  public static <T> void readAsCSV(
      FloatMatrix<T> matrix, String filename, IdConverter<T> idConverter) throws IOException {
  ...
  public static <T> void readAsCSV(
      FloatMatrix<T> matrix, InputStream stream, IdConverter<T> idConverter) throws IOException {
    BufferedReader reader = new BufferedReader(new InputStreamReader(stream));
    readCSV(matrix, reader, idConverter);
  }

  private static <T> void readCSV(
      FloatMatrix<T> matrix, BufferedReader reader, IdConverter<T> idConverter) throws IOException {
