...
/**
 * Various static methods and a data structure to calculate average adaption times, connection
 * frequency and connection shares based on the rooftop-algorithm from Niek Guis (ca. 2015).
 *
 * <p>For more details, see {@link PTSkimMatrices}.
 *
 * @author mrieser / Simunto
 */
public class RooftopUtils {

  public static List<ODConnection> sortAndFilterConnections(
      List<ODConnection> connections, double maxDepartureTime) {
    connections.sort(
        (c1, c2) ->
            Double.compare((c1.departureTime - c1.accessTime), (c2.departureTime - c2.accessTime)));

    // step forward through all connections and figure out which can be ignore because the earlier
    // one is better
    ...
    for (ODConnection connection : connections) {
      if (earlierConnection == null) {
        ...
        double timeDiff =
            (connection.departureTime - connection.accessTime)
                - (earlierConnection.departureTime - earlierConnection.accessTime);
        if (earlierConnection.totalTravelTime() + timeDiff
            > connection.totalTravelTime()
                + 0.5) { // +0.5 to catch numerical instabilities in double calculations
    ...
    // now step backwards through the remaining connections and figure out which can be ignored
    // because the later one is better
    ...
    for (int i = filteredConnections1.size() - 1; i >= 0; i--) {
      ...
      if (laterConnection == null) {
        ...
        double timeDiff =
            (laterConnection.departureTime - laterConnection.accessTime)
                - (connection.departureTime - connection.accessTime);
        if (laterConnection.totalTravelTime() + timeDiff
            > connection.totalTravelTime()
                + 0.5) { // +0.5 to catch numerical instabilities in double calculations
  ...
  public static double calcAverageAdaptionTime(
      List<ODConnection> connections, double minDepartureTime, double maxDepartureTime) {
  ...
  public static Map<ODConnection, Double> calcConnectionShares(
      List<ODConnection> connections, double minDepartureTime, double maxDepartureTime) {
    ...
    for (ODConnection connection : connections) {
      if (prevConnection != null) {
        ...
        if (depTime2 > minDepartureTime && depTime1 < maxDepartureTime) {
          ...
          final double fShare1 =
              share1; // variables must be final to be used in lambda expression below
          final double fShare2 = share2;

          shares.compute(
              prevConnection, (c, oldVal) -> (oldVal == null ? fShare1 : (oldVal + fShare1)));
          shares.compute(
              connection, (c, oldVal) -> (oldVal == null ? fShare2 : (oldVal + fShare2)));
        }
      } else {
        // there is no previous connection
        double depTime = connection.departureTime - connection.accessTime;
