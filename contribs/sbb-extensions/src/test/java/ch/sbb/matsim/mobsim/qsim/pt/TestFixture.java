...
/**
 * Creates a test fixture with a simple TransitSchedule consisting of 1 TransitLine with 1
 * TransitRoute with 1 Departure and 5 Stops
 *
 * @author mrieser / SBB
 */
class TestFixture {
  ...
  TestFixture() {
    ...
    this.stopA =
        f.createTransitStopFacility(
            Id.create("A", TransitStopFacility.class), node1.getCoord(), false);
    this.stopB =
        f.createTransitStopFacility(
            Id.create("B", TransitStopFacility.class), node2.getCoord(), false);
    this.stopC =
        f.createTransitStopFacility(
            Id.create("C", TransitStopFacility.class), node3.getCoord(), false);
    this.stopD =
        f.createTransitStopFacility(
            Id.create("D", TransitStopFacility.class), node4.getCoord(), false);
    this.stopE =
        f.createTransitStopFacility(
            Id.create("E", TransitStopFacility.class), node5.getCoord(), false);
    ...
    NetworkRoute networkRoute =
        RouteUtils.createLinkNetworkRouteImpl(link1.getId(), linkIdList, link4.getId());
    ...
    this.route1 =
        f.createTransitRoute(Id.create("A2E", TransitRoute.class), networkRoute, stops, "train");
  ...
  void addTripleTransitDemand() {
    ...
    for (int i = 1; i <= 3; i++) {
      ...
      Leg leg = pf.createLeg("pt");
      Route route =
          new DefaultTransitPassengerRoute(this.stopB, this.line1, this.route1, this.stopD);
      leg.setRoute(route);
      Activity act2 = pf.createActivityFromLinkId("work", Id.create(3, Link.class));
  ...
  void addSesselbahn(boolean removeExistingLines, boolean malformed) {
    ...
    NetworkRoute netRoute =
        RouteUtils.createLinkNetworkRouteImpl(
            this.stopB.getLinkId(), Collections.emptyList(), this.stopC.getLinkId());
    List<TransitRouteStop> stops = new ArrayList<>();
    stops.add(f.createTransitRouteStopBuilder(this.stopB).departureOffset(0).build());
    stops.add(
        f.createTransitRouteStopBuilder(this.stopC).arrivalOffset(malformed ? 0 : 120).build());
    TransitRoute route =
        f.createTransitRoute(Id.create("SB1", TransitRoute.class), netRoute, stops, "train");
  ...
  void addLoopyRoute(boolean removeExistingLines) {
    ...
    TransitStopFacility stopBLoop =
        f.createTransitStopFacility(
            Id.create("BLoop", TransitStopFacility.class), node2.getCoord(), false);
    TransitStopFacility stopCLoop =
        f.createTransitStopFacility(
            Id.create("CLoop", TransitStopFacility.class), node3.getCoord(), false);
    ...
    NetworkRoute networkRoute =
        RouteUtils.createLinkNetworkRouteImpl(link1.getId(), linkIdList, link3.getId());
    ...
    TransitRoute route =
        f.createTransitRoute(Id.create("A2D", TransitRoute.class), networkRoute, stops, "train");

    Vehicle veh1 =
        this.scenario.getTransitVehicles().getVehicles().get(Id.create("train1", Vehicle.class));
  ...
  void delayDepartureAtFirstStop() {
    ...
    TransitRouteStop stop =
        f.createTransitRouteStopBuilder(oldStop.getStopFacility()).departureOffset(20.0).build();
    stops.set(0, stop);
