...
public class SBBTransitQSimEngineTest {
  ...
  private static void assertEqualEvent(
      Class<? extends Event> eventClass, double time, Event event) {
  ...
  public void testDriver() {
    ...
    SBBTransitQSimEngine trEngine =
        new SBBTransitQSimEngine(
            qSim,
            null,
  ...
  private void assertNextStop(
      SBBTransitDriverAgent driver, TransitRouteStop stop, double routeDepTime) {
  ...
  /**
   * This test also checks that the passenger boarding/leaving times are correctly taken into
   * account
   */
  @Test
   ...
   * trying to figure out the driver of the vehicle driving around. This tests reproduces this
   * situation in order to check that the code can correctly cope with such a situation.
   */
  @Test
  ...
  /**
   * During first tests, some events were out-of-order in time, leading to an exception, due to the
   * first stop on a route having a departure offset > 0. This tests reproduces this situation in
   * order to check that the code can correctly cope with such a situation.
   */
  @Test
  ...
  /**
   * During tests, the travel time between two stops was not correctly calculated when the two stops
   * were connected with a single direct link, and each of the stops is on a zero-length loop link.
   */
  @Test
  ...
  public void testCreateEventsInterval() {
    ...
    for (int iteration = 0; iteration <= 10; iteration++) {
      ...
      Assert.assertEquals(
          "wrong number of events in iteration " + iteration,
          expectedEventsCount,
