...
/**
 * The deterministic transit simulation ({@link SBBTransitQSimEngine}) requires access to the
 * current iteration number to determine if link-events should be created or not. This iteration
 * number is accessible using {@link ReplanningContext}. In order to run the tests, we have a
 * special test module that registers an instance of a ReplanningContext in order to run the tests
 * successfully. This module does exactly that: it registers a special ReplanningContext for the
 * tests.
 *
 * @author mrieser / SBB
 */
