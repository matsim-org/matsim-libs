...
public class RooftopUtilsTest {
  ...
  public void testSortAndFilterConnections() {
    ...
    // randomize the list. instead of randomizing, we sort the connections by transferCount which we
    // misused to specify an order
  ...
  public void testCalcAverageAdaptionTime_1() {
    ...
    double adaptionTime =
        RooftopUtils.calcAverageAdaptionTime(
            connections, Time.parseTime("08:00:00"), Time.parseTime("09:00:00"));
    // there is a departure every 900 seconds, max adaption time would be 450, average of that would
    // be 225.0.
  ...
  public void testCalcAverageAdaptionTime_noEarlierDeparture() {
    ...
    double adaptionTime =
        RooftopUtils.calcAverageAdaptionTime(
            connections, Time.parseTime("07:00:00"), Time.parseTime("08:00:00"));
  ...
  public void testCalcAverageAdaptionTime_noLaterDeparture() {
    ...
    // average adaption time should be: 25min between 7:00 and 7:10, 15min between 7:10 and 7:40,
    // 10min between 7:40 and 8:00
  ...
  public void testCalcConnectionShares() {
    ...
    Map<ODConnection, Double> shares =
        RooftopUtils.calcConnectionShares(
            connections, Time.parseTime("08:00:00"), Time.parseTime("09:00:00"));
    // there is a departure every 900 seconds, max adaption time would be 450, average of that would
    // be 225.0.
    // every connection should have the same share, i.e. 1/4th = 0.25, but the first and the last
    // only cover part of the time, so they have less
  ...
  public void testCalculationShares_noEarlierDeparture2() {
    ...
    // access time is 5 min, so actual departure time is 7:10 and 8:10, and the zenith at 7:40,
    // which results in the share 40:20 = 2:1
  ...
  public void testCalculationShares_noLaterDeparture() {
    ...
    // access time is 5 min, so actual departure time is 6:40 and 7:40, and the zenith at 7:10,
    // resulting in the share 10:50 = 1:5
  ...
  public void testCalculationShares_noDepartureInRange() {
    ...
    // access time is 5 min, so actual departure time is 6:40 and 8:40, and the zenith at 7:40,
    // which results in the share 40:20 = 2:1
