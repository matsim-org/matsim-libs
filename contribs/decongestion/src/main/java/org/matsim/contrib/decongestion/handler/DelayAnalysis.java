 ...
 *
 * <p>WARNING: Link-based analysis. Ignores the travel time and delay on the first link of each
 * trip, i.e. the link on which the trip has started.
 *
 * @author ikaddoura
 */
public class DelayAnalysis
  ...
  public void handleEvent(LinkLeaveEvent event) {
    ...
    if (this.vehicleId2enterTime.get(event.getVehicleId()) != null) {
      ...
      double traveltimeThisAgent =
          event.getTime() - this.vehicleId2enterTime.get(event.getVehicleId());
      ...
      if (delayThisAgent < -1.) {
        if (warnCnt2 <= 5) {
          log.warn(
              "The delay is negative! Delay:"
                  + delayThisAgent
                  + " | traveltime: "
                  + traveltimeThisAgent
                  + " | freespeed traveltime: "
                  + freespeedTravelTime
                  + " | link: "
                  + event.getLinkId());
        ...
        if (warnCnt == 0) {
          ...
          log.info(
              "Delay is "
                  + delayThisAgent
                  + ". A negative delay of down to -1 sec may result from rounding errors. Therefore it is ignored and set to "
                  + "zero.");
  ...
  public void handleEvent(LinkEnterEvent event) {
    if (this.decongestionInfo == null
        || !this.decongestionInfo.getTransitVehicleIDs().contains(event.getVehicleId())) {
