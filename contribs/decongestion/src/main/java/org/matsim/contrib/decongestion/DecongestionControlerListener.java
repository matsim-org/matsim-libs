 ...
 *
 * <p>(1) Identify congested links and time intervals and set an initial toll for these links and
 * time intervals. (2) Let the demand adjust for x iterations. (3) Adjust the tolls (different
 * implementations of {@link DecongestionTollSetting}) (4) GOTO (2)
 *
 * <p>All relevant parameters are specified in {@link DecongestionInfo}.
 *
 * @author ikaddoura
 */
public class DecongestionControlerListener
  ...
  public void notifyStartup(StartupEvent event) {
    ...
    this.outputDirectory =
        this.congestionInfo.getScenario().getConfig().controller().getOutputDirectory();
  ...
  public void notifyAfterMobsim(AfterMobsimEvent event) {
    ...
    if (event.getIteration()
                % this.congestionInfo.getDecongestionConfigGroup().getWriteOutputIteration()
            == 0.
        || event.getIteration()
                % this.congestionInfo.getDecongestionConfigGroup().getUpdatePriceInterval()
            == 0.) {
      computeDelays(event);
    }

    if (event.getIteration()
        == this.congestionInfo.getScenario().getConfig().controller().getFirstIteration()) {
      ...
      int totalNumberOfIterations =
          this.congestionInfo.getScenario().getConfig().controller().getLastIteration()
              - this.congestionInfo.getScenario().getConfig().controller().getFirstIteration();
      ...
      if (iterationCounter
              < this.congestionInfo
                      ...
                      .getFractionOfIterationsToEndPriceAdjustment()
                  * totalNumberOfIterations
          && iterationCounter
              > this.congestionInfo
                      .getDecongestionConfigGroup()
                      .getFractionOfIterationsToStartPriceAdjustment()
                  * totalNumberOfIterations) {

        if (tollComputation != null) {
          log.info(
              "+++ Iteration " + event.getIteration() + ". Update tolls per link and time bin.");
    ...
    if (event.getIteration()
      ...
      CongestionInfoWriter.writeDelays(
          congestionInfo,
          event.getIteration(),
          this.outputDirectory + "ITERS/it." + event.getIteration() + "/",
  ...
  private void computeDelays(AfterMobsimEvent event) {
    ...
    for (Link link : this.congestionInfo.getScenario().getNetwork().getLinks().values()) {
      ...
      for (double endTime = timeBinSize;
        ...
        final double congestedTravelTime =
            travelTime.getLinkTravelTime(link, probedTime, null, null);
        double avgDelay = congestedTravelTime - freespeedTravelTime;

        if (linkHasAtLeastOneTimeBinWithNonZeroAvgDelay == false
            && avgDelay
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    if (this.delayComputation != null) {
      ...
      if (this.intervalBasedTolling != null)
        totalPayments = this.intervalBasedTolling.getTotalTollPayments();
      ...
      for (Person person :
          this.congestionInfo.getScenario().getPopulation().getPersons().values()) {
      ...
      XYLineChart chart1 =
          new XYLineChart("Total travel time and total delay", "Iteration", "Hours");
      ...
      for (int i = this.congestionInfo.getScenario().getConfig().controller().getFirstIteration();
      ...
      chart1.saveAsPng(
          outputDirectory
              + this.congestionInfo.getScenario().getConfig().controller().getRunId()
              + ".decongestion_travelTime_delay.png",
          800,
          600);

      XYLineChart chart2 =
          new XYLineChart("user benefits and toll revenues", "Iteration", "Monetary units");
      ...
      chart2.saveAsPng(
          outputDirectory
              + this.congestionInfo.getScenario().getConfig().controller().getRunId()
              + ".decongestion_userBenefits_tolls.png",
          800,
          600);
    }
  }
      ...
      if (event.getIteration()
        ...
        this.nextDisableInnovativeStrategiesIteration =
            (int)
                (congestionInfo
                    ...
                    * congestionInfo.getDecongestionConfigGroup().getUpdatePriceInterval());
        log.info(
        ...
        if (this.nextDisableInnovativeStrategiesIteration != 0) {
          this.nextEnableInnovativeStrategiesIteration =
              (int) (congestionInfo.getDecongestionConfigGroup().getUpdatePriceInterval() + 1);
        }
        log.info(
        ...
        if (event.getIteration() == this.nextDisableInnovativeStrategiesIteration) {
          ...
          if (event.getIteration()
              >= congestionInfo
                      ...
                      .replanning()
                      .getFractionOfIterationsToDisableInnovation()
                  * (congestionInfo.getScenario().getConfig().controller().getLastIteration()
                      - congestionInfo
                          .getScenario()
                          .getConfig()
                          .controller()
