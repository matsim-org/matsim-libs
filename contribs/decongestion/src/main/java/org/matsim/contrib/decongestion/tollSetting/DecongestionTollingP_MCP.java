...
public class DecongestionTollingP_MCP implements DecongestionTollSetting, LinkLeaveEventHandler {
  ...
  @Inject private DecongestionInfo congestionInfo;
  ...
  public void updateTolls() {
    ...
    final double vtts =
        (this.congestionInfo.getScenario().getConfig().scoring().getPerforming_utils_hr()
                - this.congestionInfo
                    .getScenario()
                    .getConfig()
                    .scoring()
    ...
    final double toleratedAvgDelay =
        this.congestionInfo.getDecongestionConfigGroup().getToleratedAverageDelaySec();
    final boolean msa = this.congestionInfo.getDecongestionConfigGroup().isMsa();
    final double blendFactorFromConfig =
        this.congestionInfo.getDecongestionConfigGroup().getTollBlendFactor();
    ...
    for (Id<Link> linkId : this.congestionInfo.getlinkInfos().keySet()) {
      ...
      for (Integer intervalNr : linkInfo.getTime2avgDelay().keySet()) {
        ...
        if (this.linkId2time2leavingAgents.get(linkId) != null
            && this.linkId2time2leavingAgents.get(linkId).get(intervalNr) != null) {
    ...
    for (Map.Entry<Id<Link>, LinkInfo> entry : this.congestionInfo.getlinkInfos().entrySet()) {
      ...
      linkId2infoPreviousTollComputation.put(
          linkInfo.getLink().getId(), linkInfoPreviousTollComputation);
    }
  }
  ...
  private int getIntervalNr(double time) {
    double timeBinSize =
        congestionInfo.getScenario().getConfig().travelTimeCalculator().getTraveltimeBinSize();
