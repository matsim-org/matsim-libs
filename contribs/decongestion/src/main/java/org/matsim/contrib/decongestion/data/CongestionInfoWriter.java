...
public class CongestionInfoWriter {
  ...
  public static void writeDelays(
      DecongestionInfo congestionInfo, int iteration, String outputPath, String runId) {
    ...
    String fileName =
        outputPathCongestionInfo
            + runId
            + "."
            + iteration
            + ".decongestion_delays_perLinkAndTimeBin.csv";
    File file = new File(fileName);

    try {
      BufferedWriter bw = new BufferedWriter(new FileWriter(file));

      int totalNumberOfTimeBins =
          (int)
              (congestionInfo.getScenario().getConfig().travelTimeCalculator().getMaxTime()
                  / congestionInfo
                      .getScenario()
                      .getConfig()
      ...
      for (Id<Link> linkId : congestionInfo.getScenario().getNetwork().getLinks().keySet()) {
        ...
        for (int i = 0; i < totalNumberOfTimeBins; i++) {
          ...
          if (congestionInfo.getlinkInfos().get(linkId) != null
              && congestionInfo.getlinkInfos().get(linkId).getTime2avgDelay().get(i) != null) {
    ...
    if (congestionInfo.getDecongestionConfigGroup().isWriteLinkInfoCharts()) {
      ...
      XYLineChart chart =
          new XYLineChart(
              "Iteration " + iteration, "Time of day [hours]", "Average delay [seconds]");
  ...
  private static void writeTolls4Excel(
    ...
    String fileName2 =
        outputPathCongestionInfo
            ...
            + "."
            + iteration
            + ".decongestion_toll_perLinkAndTimeBin.csv";
    ...
    try {
      ...
      for (Id<Link> linkId : congestionInfo.getScenario().getNetwork().getLinks().keySet()) {
        ...
        for (int i = 0; i < totalNumberOfTimeBins; i++) {
          ...
          double toll = 0.;
          if (congestionInfo.getlinkInfos().get(linkId) != null
              && congestionInfo.getlinkInfos().get(linkId).getTime2toll().get(i) != null) {
    ...
    if (congestionInfo.getDecongestionConfigGroup().isWriteLinkInfoCharts()) {
      ...
      XYLineChart chart =
          new XYLineChart("Iteration " + iteration, "Time of day [hours]", "Toll [monetary units]");
      ...
      chart.saveAsPng(
          outputPathCongestionInfo
              ...
              + "."
              + iteration
              + ".decongestion_toll_perLinkAndTimeBin.png",
          800,
          600);
    }
  }

  private static void writeFile4Via(
      DecongestionInfo congestionInfo, int iteration, String outputPath, String runId) {
    ...
    String fileName2 =
        outputPathCongestionInfo + runId + "." + iteration + ".decongestion_info_via.csv.gz";
    ...
    try {
      ...
      for (int timeBin = 0; timeBin < totalNumberOfTimeBins; timeBin++) {
        double timeInterval =
            timeBin
                * congestionInfo
                    ...
                    .travelTimeCalculator()
                    .getTraveltimeBinSize();
        // (in via (and most other softwares), I need the beginning of the interval in the data, not
        // the end.  Thus, not "(timeBin+1)*...".  kai, may'18
        ...
        // replaced by the following because congestionInfo.getLinkInfos() may no longer contain all
        // network links and we want to avoid delay=NaN
  ...
  public static void writeIterationStats(
    ...
    try {
      ...
      bw.write(
          "Iteration ; Total delay [hours] ; Total congestion toll payments [monetary units] ; Total travel time [hours]; Total user benefits [monetary units]; System welfare (only considering congestion toll revenues, other revenues are neglected [monetary units]");
      bw.newLine();
