...
public class IntervalBasedTollingAll implements LinkLeaveEventHandler, IntervalBasedTolling {
  ...
  @Inject private DecongestionInfo decongestionInfo;
  ...
  public void handleEvent(LinkLeaveEvent event) {
    if (!decongestionInfo.getTransitVehicleIDs().contains(event.getVehicleId())
        && decongestionInfo.getlinkInfos().get(event.getLinkId()) != null) {
      ...
      if (decongestionInfo.getlinkInfos().get(event.getLinkId()).getTime2toll().get(currentTimeBin)
          != null) {
        double toll =
            decongestionInfo
                .getlinkInfos()
        ...
        this.eventsManager.processEvent(
            new PersonMoneyEvent(
                event.getTime(),
                this.decongestionInfo.getVehicleId2personId().get(event.getVehicleId()),
                -1. * toll,
                "congestionPricing",
                null));
        //				this.eventsManager.processEvent(new PersonLinkMoneyEvent(event.getTime(),
        // this.decongestionInfo.getVehicleId2personId().get(event.getVehicleId()),
        // event.getLinkId(), -1. * toll, event.getTime(), "congestion"));
