...
public class DecongestionTollingPID implements DecongestionTollSetting, LinkLeaveEventHandler {
  ...
  private final Map<Id<Link>, Map<Integer, Double>> linkId2time2totalDelayAllIterations =
      new HashMap<>();

  private final Map<Id<Link>, Map<Integer, Double>> linkId2time2avgDelayAllIterations =
      new HashMap<>();
  ...
  public void updateTolls() {
    ...
    final String integralApproach =
        congestionInfo.getDecongestionConfigGroup().getIntegralApproach().toString();
    ...
    final double capacityPeriod =
        this.congestionInfo.getScenario().getNetwork().getCapacityPeriod();
    final double flowCapacityFactor =
        this.congestionInfo.getScenario().getConfig().qsim().getFlowCapFactor();
    ...
    for (Id<Link> linkId : this.congestionInfo.getlinkInfos().keySet()) {
      ...
      for (Integer intervalNr : linkInfo.getTime2avgDelay().keySet()) {
        ...
        // DONE (alpha = 1.0)
        // --> Mittel über die letzten positven averageDelays (exponential smoothing probably ok:
        // lastAverageDelay = (1-alpha)*lastAvDelay + alpha*averageDelay ; ) --> DONE
        // --> \propto * (1/flow - 1/cap, so etwas wie die "headway reserve" oder "unused time
        // headway") --> DONE
        ...
        if (K_i != 0.) {
          ...
          if (integralApproach.equals(IntegralApproach.Average.toString())) {
            if (averageDelay
                > congestionInfo.getDecongestionConfigGroup().getToleratedAverageDelaySec()) {
                  ...
                  avgDelayAllIterations =
                      (1
                                  - congestionInfo
                                      ...
                                      .getIntegralApproachAverageAlpha())
                              * this.linkId2time2avgDelayAllIterations.get(linkId).get(intervalNr)
                          + congestionInfo
                                  .getDecongestionConfigGroup()
                                  .getIntegralApproachAverageAlpha()
                              * averageDelay;
              ...
              this.linkId2time2avgDelayAllIterations
                  .get(linkId)
                  .put(intervalNr, avgDelayAllIterations);
            }
          }
          ...
          if (integralApproach.equals(IntegralApproach.UnusedHeadway.toString())) {
            ...
            if (this.linkId2time2leavingAgents.get(linkId) != null
              ...
              flowHeadwaySec =
                  timeBinSize / this.linkId2time2leavingAgents.get(linkId).get(intervalNr);
            }
          ...
          if (linkId2time2totalDelayAllIterations.get(linkId) == null) {
            ...
            if (this.linkId2time2totalDelayAllIterations.get(linkId).get(intervalNr) == null) {
              ...
              if (averageDelay
                ...
                if (integralApproach.equals(IntegralApproach.Average.toString())) {
                  totalDelayAllIterations =
                      this.linkId2time2totalDelayAllIterations.get(linkId).get(intervalNr)
                          - avgDelayAllIterations;
                  ...
                  totalDelayAllIterations =
                      this.linkId2time2totalDelayAllIterations.get(linkId).get(intervalNr)
                          - (congestionInfo
                                  .getDecongestionConfigGroup()
                                  .getIntegralApproachUnusedHeadwayFactor()
                              * unusedHeadway);
          ...
          this.linkId2time2totalDelayAllIterations
              .get(linkId)
              .put(intervalNr, totalDelayAllIterations);
        ...
        if (K_d != 0.) {
          ...
          if (this.linkId2infoPreviousTollComputation.get(linkId) != null
              && this.linkId2infoPreviousTollComputation
                      .get(linkId)
                      .getTime2avgDelay()
                      .get(intervalNr)
                  != null) {
