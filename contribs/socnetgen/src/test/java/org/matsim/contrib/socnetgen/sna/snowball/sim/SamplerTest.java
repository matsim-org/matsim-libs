...
public class SamplerTest extends MatsimTestCase {
	...
	public void test() throws IOException {
		...
		Graph graph = reader.readGraph(getPackageInputDirectory() + "test.graphml.gz");
		...
		for (int it = 0; it <= sampler.getIteration(); it++) {
			...
			String reference = String.format("%1$s/k.%2$s.txt", getPackageInputDirectory(), it);
			String tmp = String.format("%1$s/k.%2$s.txt", getPackageInputDirectory(), it);
