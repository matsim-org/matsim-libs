		...
		for(TransitRouterNetworkWW.TransitRouterNetworkLink link:routerNetwork.getLinks().values())
			if(link.route!=null) {
				...
				for(int slot = 0; slot<numSlots; slot++) {
					...
					occs[slot] = vehicleOccupancy.getVehicleOccupancy(((TransitRouterNetworkWW.TransitRouterNetworkNode)link.getFromNode()).stop.getStopFacility().getId(), link.toNode.line.getId(), link.toNode.route.getId(), startTime+slot*timeSlot);
				}
