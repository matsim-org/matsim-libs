...
public class LinkProperties {
  ...
  /**
   * Assume for links with max speed lower than 51km/h to be in urban areas. The free speed is
   * reduced by this factor to account for traffic lights, ROW, etc.
   *
  ...
  public static double getLaneCapacity(
      double linkLength, LinkProperties properties, double adjustCapacityLength) {
  ...
  /**
   * For links with unknown max speed we assume that links with a length of less than 300m are urban
   * links. For urban links with a length of 0m the speed is 10km/h. For links with a length of 300m
   * the speed is the default freespeed property for that highway type. For links with a length
   * between 0 and 300m the speed is interpolated linearly. (2.778m/s ~ 10km/h)
   *
   * <p>All links longer than 300m the default freesped property is assumed
   */
  public static double calculateSpeedIfNoSpeedTag(double linkLength, LinkProperties properties) {
    if (properties.hierarchyLevel > LinkProperties.LEVEL_MOTORWAY
        && properties.hierarchyLevel <= LinkProperties.LEVEL_TERTIARY
        && linkLength <= 300) {
