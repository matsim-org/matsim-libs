...
public class PbfParser implements OsmHandler {
  ...
  private PbfParser(
      Consumer<OsmNode> nodeHandler,
      Consumer<OsmWay> waysHandler,
      Consumer<OsmRelation> relationHandler,
  ...
  private ParsingResult parse(BlobHeader header, Fileformat.Blob blob) throws IOException {
    ...
    if (header.getType().equals(Constants.BLOCK_TYPE_DATA)) {
      Osmformat.PrimitiveBlock primBlock =
          Osmformat.PrimitiveBlock.parseFrom(blockData.getBlobData());
  ...
  private ParsingResult parse(Osmformat.PrimitiveBlock block) {
    ...
    for (Osmformat.PrimitiveGroup primitiveGroup : block.getPrimitivegroupList()) {
      ...
      } else if (primitiveGroup.getWaysCount() > 0
          && waysHandler == null
  ...
  // have these more or less identical methods, since I currently don't know how to handle
  ...
  private void startParseRelationsTask(
      Osmformat.PrimitiveBlock block, Osmformat.PrimitiveGroup group) {
    ...
    public PbfParser build() {
      ...
      if (nodeHandler == null && waysHandler == null && relationHandler == null)
        throw new RuntimeException(
            "No handler was provided. This way, the file is parsed without extracting information.");
