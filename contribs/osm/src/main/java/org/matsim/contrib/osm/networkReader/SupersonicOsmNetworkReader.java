...
/**
 * Class for converting osm-networks into matsim-networks. This class uses the binary osm.pbf format
 * as an input. Suitable input files can be found at https://download.geofabrik.de
 *
 * <p>Examples on how to use the reader can be found in {@link org.matsim.contrib.osm.examples}
 *
 * <p>For the most common highway tags the {@link LinkProperties} class contains default properties
 * for the corresponding links in the matsim-network (e.g. speed, number of lanes). Those default
 * properties may be overridden with custom link properties using the {@link
 * SupersonicOsmNetworkReader.Builder#addOverridingLinkProperties(String, LinkProperties)} method of
 * the Builder.
 */
public class SupersonicOsmNetworkReader {
  ...
  static AfterLinkCreated adjustFreespeed(final double factor) {
    return (link, osmTags, direction) -> {
      if (osmTags.containsKey(OsmTags.MAXSPEED)) {
        if (link.getFreespeed() < 51 / 3.6) link.setFreespeed(link.getFreespeed() * factor);
      }
    };
  }
  ...
  Collection<WaySegment> createWaySegments(ProcessedOsmWay way) {
    ...
    for (int i = 1; i < way.getNodeIds().size(); i++) {
      ...
      segmentLength +=
          CoordUtils.calcEuclideanDistance(fromOsmNode.getCoord(), toOsmNode.getCoord());
      ...
      // if we have an intersection or the end of the way
        ...
        // get all the node ids for the way segment. TLongList.subList(fromIndex, toIndex) has
        // toIndex exclusive, therefore i + 1
        ...
        var waySegment =
            new WaySegment(
                ...
                way.getTags(),
                way.getId(),
                // if the way id is 1234 we will get a link id like 12340001, this is necessary
                // because we need to generate unique
                // ids. The osm wiki says ways have no more than 2000 nodes which means that i will
                // never be greater 1999.
                way.getId() * 10000 + i - 1);
  ...
  private boolean isCreateSegment(ProcessedOsmNode from, ProcessedOsmNode to, ProcessedOsmWay way) {
    ...
    // if the user wants to have this node, then he should get it no matter what the other
    // conditions are.
  ...
  private Collection<WaySegment> handleLoop(
      Map<Long, ProcessedOsmNode> nodes,
      ProcessedOsmNode node,
      ProcessedOsmWay way,
      int toNodeIndex) {

    // we need an extra test whether the loop is within the link filter
    if (!includeLinkAtCoordWithHierarchy.test(
        node.getCoord(), way.getLinkProperties().hierarchyLevel)) return Collections.emptyList();

    List<WaySegment> result = new ArrayList<>();
    ProcessedOsmNode toSegmentNode = node;

    // iterate backwards and keep all elements of the loop. Don't do thinning since this is an edge
    // case
    for (int i = toNodeIndex - 1; i > 0; i--) {
      ...
      result.add(
          new WaySegment(
              fromSegmentNode,
              toSegmentNode,
              new TLongArrayList(new long[] {fromSegmentNode.getId(), toSegmentNode.getId()}),
              CoordUtils.calcEuclideanDistance(
                  fromSegmentNode.getCoord(), toSegmentNode.getCoord()),
              way.getLinkProperties(),
              way.getTags(),
              way.getId(),
              way.getId() * 10000 + i));
  ...
  Link createLink(Node fromNode, Node toNode, WaySegment segment, Direction direction) {
    ...
    String linkId =
        direction == Direction.Forward
            ? segment.getSegmentId() + "f"
            : segment.getSegmentId() + "r";
    ...
    link.setCapacity(
        LinkProperties.getLaneCapacity(link.getLength(), properties, adjustCapacityLength)
    ...
    link.getAttributes()
        .putAttribute(
            ...
            getMaxSpeed(segment.getTags(), link.getLength(), properties));
    if (storeOriginalGeometry && segment.getNodesForSegment().size() > 2)
      // this is optional functionality. Also, the first and last nodeforsegment are the from and to
      // node of the simplified link
      // it makes only sense to store more geometry information if there are intermediate nodes.
      link.getAttributes()
          .putAttribute(
              NetworkUtils.ORIG_GEOM, getOriginalGeometry(segment.getNodesForSegment(), direction));
  ...
  private double getFreespeed(
      Map<String, String> tags, double linkLength, LinkProperties properties) {
  ...
  private double parseSpeedTag(String tag, LinkProperties properties) {
    ...
    try {
      ...
      else return Double.parseDouble(tag) / 3.6;
  ...
  private double getNumberOfLanes(
      Map<String, String> tags, Direction direction, LinkProperties properties) {

    try {
      if (tags.containsKey(OsmTags.LANES)) {
        String directionKey =
            direction == Direction.Reverse ? OsmTags.LANES_BACKWARD : OsmTags.LANES_FORWARD;
        ...
        // lanes tag specifies lanes into both directions of a way, so cut it in half if it is not a
        // oneway street
  ...
  private String getOriginalGeometry(TLongList nodeIds, Direction direction) {
    ...
    // the first and last nodes are the from- and to node of a link. We don't have to include them
    // here
    // if forward link, put nodes in the same order as original osm way, otherwise the other way
    // around
  ...
  public abstract static class AbstractBuilder<T> {
    ...
    /**
     * Replace all Link-Properties at once. Link properties describe how an osm-highway-tag is
     * translated into a matsim-link. E.g. which freespeed is set on the link, whether it is
     * one-way, etc.
     *
     * <p>The reader only reads osm-ways which have a corresponding entry in the linkProperties map.
     *
     * @param linkProperties The link properties to be included in the matsim-network.
     */
    public AbstractBuilder<T> setLinkProperties(
        ConcurrentMap<String, LinkProperties> linkProperties) {
      this.linkProperties = linkProperties;
      return this;
    }

    /**
     * The reader has a default set of osm-highway-tags which are parsed. The following are included
     * by default: motorway, motorway_link, trunk, trunk_link, primary, primary_link, secondary,
     * secondary_link, tertiary, tertiary_link, unclassified, residential, living_street
     *
     * <p>By invoking this method one can override the assigned {@link LinkProperties} by supplying
     * the desired highway tag with a new {@link LinkProperties} object.
     *
     ...
     */
    public AbstractBuilder<T> addOverridingLinkProperties(
        String highwayType, LinkProperties properties) {
      linkProperties.put(highwayType, properties);
      return this;
    }

    /**
     * This sets a filter to in- or exclude links depending on its hierarchy level and location. The
     * filter is invoked for each to and from node of a link.
     *
     * <p>The hierarchy levels reach from {@link LinkProperties#LEVEL_MOTORWAY} = 1 to {@link
     * LinkProperties#LEVEL_LIVING_STREET} = 8
     *
     * <p>Note: The supplied function is invoked concurrently
     *
     * @param includeLinkAtCoordWithHierarchy Bi-Predicate which takes a node's coordinate and its
     *     hierarchy level
     */
    public AbstractBuilder<T> setIncludeLinkAtCoordWithHierarchy(
        BiPredicate<Coord, Integer> includeLinkAtCoordWithHierarchy) {
      this.includeLinkAtCoordWithHierarchy = includeLinkAtCoordWithHierarchy;
      return this;
    }
     ...
     *
     * <p>The reader tries to simplify the network as much as possible. If it is essential to keep
     * certain nodes of the original osm-network within the output matsim-network for e.g. counts
     * the filter can prevent the removal of the node with the supplied id
     *
     * <p>Setting the filter to '() -> true' will omit network simplification entirely and is
     * equivalent to the 'keepPath' option of the previous OsmNetworkReader
     *
     * <p>Note: The supplied function is invoked concurrently
     *
     * @param preserveNodeWithId Predicate which returns true if the node corresponding to the
     *     supplied id must not be removed.
     */
     ...
     * <p>Note: The supplied function is invoked concurrently
     *
     * @param afterLinkCreated Basically a tri-consumer which accepts the created link, the original
     *     osm-tags as a map and the direction enum which describes whether it is the forward or
     *     backward link for an osm-way
     */
    ...
    /**
     * Coordinate transformation to transform spherical-osm-coordinates into euclidean-coordinates
     * suited for matsim simulations
     *
     * @param coordinateTransformation The supplied transformation should have {@link
     *     org.matsim.core.utils.geometry.transformations.TransformationFactory#WGS84} as input
     *     coordinate-system. And something like "EPSG:25832" as output sytem
     */
    public AbstractBuilder<T> setCoordinateTransformation(
        CoordinateTransformation coordinateTransformation) {
     ...
     * {@link org.matsim.core.network.NetworkUtils#ORIG_GEOM} A collection of nodes can be fetched
     * with {@link org.matsim.core.network.NetworkUtils#getOriginalGeometry(Link)}
     *
    ...
    SupersonicOsmNetworkReader createInstance() {
      ...
      if (coordinateTransformation == null) {
        throw new IllegalArgumentException(
            "Target coordinate transformation is required parameter!");
      }
  ...
  static class WaySegment {
    ...
    public WaySegment(
        ProcessedOsmNode fromNode,
        ProcessedOsmNode toNode,
        TLongList nodesForSegment,
        double length,
