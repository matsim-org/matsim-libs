...
/**
 * Network reader which adds bicycle specific properties to the generated matsim-network. These
 * include: - {@link TransportMode#bike} is added as allowed transport mode - Surface, smoothness
 * are stored as link attributes - Cycleway and restriction attributes are set as link attributes as
 * well - sets bicycleInfrastructureSpeedFactor to 0.5 on each link which has allowed mode {@link
 * TransportMode#bike}
 *
 * <p>Additionally the osm-highway-tags cycleway, service, track, footway, pedestrian, path, steps
 * are parsed from the original osm-network.
 */
public final class OsmBicycleReader extends SupersonicOsmNetworkReader {

  private static final double BIKE_PCU = 0.25;
  private static final Set<String> bicycleNotAllowed =
      new HashSet<>(
          Arrays.asList(
              OsmTags.MOTORWAY, OsmTags.MOTORWAY_LINK, OsmTags.TRUNK, OsmTags.TRUNK_LINK));
  private static final Set<String> onlyBicycleAllowed =
      new HashSet<>(
          Arrays.asList(
              OsmTags.TRACK,
              OsmTags.CYCLEWAY,
  ...
  public OsmBicycleReader(
      OsmNetworkParser parser,
      ...
      double adjustCapacityLength,
      boolean storeOriginalGeometry) {
    super(
        parser,
        preserveNodeWithId,
        includeLinkAtCoordWithHierarchy,
        (link, tags, direction) -> handleLink(link, tags, direction, afterLinkCreated),
        freeSpeedFactor,
        adjustCapacityLength,
  ...
  private static void handleLink(
      ...
      SupersonicOsmNetworkReader.Direction direction,
      AfterLinkCreated outfacingCallback) {
  ...
  private static void setAllowedModes(Link link, String highwayType) {
    ...
    if (!bicycleNotAllowed.contains(highwayType)) allowedModes.add(TransportMode.bike);
    if (onlyBicycleAllowed.contains(highwayType)) allowedModes.remove(TransportMode.car);
  ...
  private static void setSurface(Link link, Map<String, String> tags, String highwayType) {
    ...
    } else if (highwayType.equals(OsmTags.PRIMARY)
  ...
  Collection<Link> createLinks(WaySegment segment) {
    ...
    if (links.size() == 1 && isReverseCycleWay(segment.getTags())) {
      ...
      Direction direction =
          reverseLink.getFromNode().getId().equals(Id.createNodeId(segment.getFromNode().getId()))
              ? Direction.Forward
              : Direction.Reverse;
  ...
  private Link createReverseBicycleLink(Link forwardLink) {
    ...
    Link result =
        getNetworkFactory()
            .createLink(
                Id.createLinkId(linkId), forwardLink.getToNode(), forwardLink.getFromNode());
  ...
  private boolean isReverseCycleWay(Map<String, String> tags) {
    ...
    if (tags.containsKey(OsmTags.CYCLEWAY)) {
      ...
      return (tag.equals("opposite")
          || tag.equals("opposite_track")
          || tag.equals("opposite_lane"));
    }
    return false;
  }
  ...
  public static class Builder extends AbstractBuilder<OsmBicycleReader> {
    ...
    public Builder() {
      addOverridingLinkProperties(
          OsmTags.TRACK, new LinkProperties(9, 1, 30 / 3.6, 1500 * BIKE_PCU, false));
      addOverridingLinkProperties(
          OsmTags.CYCLEWAY, new LinkProperties(9, 1, 30 / 3.6, 1500 * BIKE_PCU, false));
      addOverridingLinkProperties(
          OsmTags.SERVICE, new LinkProperties(9, 1, 10 / 3.6, 100 * BIKE_PCU, false));
      addOverridingLinkProperties(
          OsmTags.FOOTWAY, new LinkProperties(10, 1, 10 / 3.6, 600 * BIKE_PCU, false));
      addOverridingLinkProperties(
          OsmTags.PEDESTRIAN, new LinkProperties(10, 1, 10 / 3.6, 600 * BIKE_PCU, false));
      addOverridingLinkProperties(
          OsmTags.PATH, new LinkProperties(10, 1, 20 / 3.6, 600 * BIKE_PCU, false));
      addOverridingLinkProperties(
          OsmTags.STEPS, new LinkProperties(11, 1, 1 / 3.6, 50 * BIKE_PCU, false));
    }

    @Override
    OsmBicycleReader createInstance() {
      OsmNetworkParser parser =
          new OsmNetworkParser(
              coordinateTransformation,
              linkProperties,
              includeLinkAtCoordWithHierarchy,
              Executors.newWorkStealingPool());
      return new OsmBicycleReader(
          parser,
          preserveNodeWithId,
          includeLinkAtCoordWithHierarchy,
          afterLinkCreated,
