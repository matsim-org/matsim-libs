...
class OsmSignalsParser extends OsmNetworkParser {
  ...
  OsmSignalsParser(
      ...
      Map<String, LinkProperties> linkProperties,
      BiPredicate<Coord, Integer> linkFilter,
  ...
  private void handleRelation(OsmRelation osmRelation) {
    ...
    if (osmRelation.getNumberOfMembers() == 3) {
      ...
      if (tags.containsKey(OsmTags.TYPE)
          && tags.get(OsmTags.TYPE).equals(OsmTags.RESTRICTION)
          && tags.containsKey(OsmTags.RESTRICTION)) {

        String restrictionTag = tags.get(OsmTags.RESTRICTION);
        ProcessedRelation.Type type =
            restrictionTag.startsWith("only")
                ? ProcessedRelation.Type.IMPERATIVE
                : ProcessedRelation.Type.PROHIBITIVE;
        ...
        if (via != null && from != null && to != null) {
          nodeRestrictions
              .computeIfAbsent(via.getId(), id -> Collections.synchronizedSet(new HashSet<>()))
