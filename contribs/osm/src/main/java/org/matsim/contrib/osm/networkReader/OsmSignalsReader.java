...
public class OsmSignalsReader extends SupersonicOsmNetworkReader {
  ...
  OsmSignalsReader(
    ...
    throw new RuntimeException(
        "The signals reader is not yet implemented. Please don't use it yet.");
  }
  ...
  private static void doAfterLinkCreated(
      Link link,
  ...
  public Network read(Path inputFile) {
    ...
    for (Node node : network.getNodes().values()) {
      ...
      searchable
          .getNodeQuadTree()
          .getRectangle(
              coord.getX() - 30, coord.getY() - 30, coord.getX() + 30, coord.getY() + 30, out);
  ...
  public static class Builder extends AbstractBuilder<OsmSignalsReader> {
    ...
    OsmSignalsReader createInstance() {
      ...
      OsmSignalsParser parser =
          new OsmSignalsParser(
              coordinateTransformation,
              linkProperties,
              includeLinkAtCoordWithHierarchy,
              Executors.newWorkStealingPool());

      return new OsmSignalsReader(
          parser,
          preserveNodeWithId,
          includeLinkAtCoordWithHierarchy,
          afterLinkCreated,
