...
public class OsmNetworkParserTest {
  ...
  @Rule public MatsimTestUtils matsimUtils = new MatsimTestUtils();
  ...
  public void parse_singleLink() {
    ...
    OsmNetworkParser parser =
        new OsmNetworkParser(
            transformation, LinkProperties.createLinkProperties(), (coord, id) -> true, executor);
    parser.parse(file);
  ...
  public void parse_twoIntersectingWays() {
    ...
    for (var node : nodes.values()) {
      ...
      else if (node.getId() == 9L
          || node.getId() == 1L
          || node.getId() == 10L
          || node.getId() == 17L) // the end nodes
  ...
  public void parse_intersectingLinksOneDoesNotMatchFilter() {
    ...
    Path file =
        Paths.get(matsimUtils.getOutputDirectory(), "parser_intersecting-links-with-filtering.pbf");
    Utils.writeOsmData(twoIntersectingLinks.getNodes(), twoIntersectingLinks.getWays(), file);
    ...
    OsmNetworkParser parser =
        new OsmNetworkParser(
            ...
            (coord, level) ->
                level == LinkProperties.LEVEL_MOTORWAY, // just take the motorway link,
            executor);
    parser.parse(file);
    ...
    for (ProcessedOsmNode node : nodes.values()) {
      if (node.getId() == 1 || node.getId() == 5 || node.getId() == 9) {
        assertEquals(1, node.getFilteredReferencedWays().size());
        assertEquals(
            twoIntersectingLinks.getWays().get(0).getId(),
            node.getFilteredReferencedWays().get(0).getId());
      } else {
        assertEquals(0, node.getFilteredReferencedWays().size());
      }
    }
  ...
  public void parse_singleLink_withTransformation() {
    ...
    Path file =
        Paths.get(matsimUtils.getOutputDirectory(), "parser_single-link-with-transformation.pbf");
    Utils.writeOsmData(singleLink.getNodes(), singleLink.getWays(), file);

    OsmNetworkParser parser =
        new OsmNetworkParser(
            atlantis, LinkProperties.createLinkProperties(), (coord, id) -> true, executor);
    parser.parse(file);
    ...
    Coord transformedNode1 =
        atlantis.transform(new Coord(node1.getLatitude(), node1.getLongitude()));
