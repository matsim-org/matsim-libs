...
public class Utils {
  ...
  static OsmData createSingleLink(List<OsmTag> tags) {
    ...
    TLongArrayList nodeReference =
        new TLongArrayList(new long[] {node1.getId(), node2.getId(), node3.getId(), node4.getId()});
  ...
  static OsmData createTwoIntersectingLinksWithDifferentLevels() {
    ...
    TLongArrayList nodeReferenceForWay1 =
        new TLongArrayList(
            new long[] {
              node1.getId(),
              node2.getId(),
    ...
    TLongArrayList nodeReferenceForWay2 =
        new TLongArrayList(
            new long[] {
              node10.getId(),
              node11.getId(),
    ...
    Way way1 =
        new Way(
            1,
            nodeReferenceForWay1,
            Collections.singletonList(new Tag(OsmTags.HIGHWAY, Utils.MOTORWAY)));
    Way way2 =
        new Way(
            2,
            nodeReferenceForWay2,
            Collections.singletonList(new Tag(OsmTags.HIGHWAY, Utils.TERTIARY)));
  ...
  static OsmData createGridWithDifferentLevels() {
    ...
    List<OsmWay> waysList =
        Arrays.asList(
            new Way(
                1,
                new TLongArrayList(new long[] {3, 4, 5, 6}),
                Collections.singletonList(new Tag("highway", MOTORWAY))),
            new Way(
                2,
                new TLongArrayList(new long[] {7, 8, 9, 10}),
                Collections.singletonList(new Tag("highway", MOTORWAY))),
            new Way(
                3,
                new TLongArrayList(new long[] {1, 4, 8, 11}),
                Collections.singletonList(new Tag("highway", TERTIARY))),
            new Way(
                4,
                new TLongArrayList(new long[] {2, 5, 9, 12}),
                Collections.singletonList(new Tag("highway", TERTIARY))));
  ...
  private static void testNodesAreEqual(
      org.matsim.api.core.v01.network.Node expected, org.matsim.api.core.v01.network.Node actual) {
    ...
    expected
        .getOutLinks()
        .forEach(
            (id, link) -> Assert.assertEquals(link.getId(), actual.getOutLinks().get(id).getId()));
    expected
        .getInLinks()
        .forEach(
            (id, link) -> Assert.assertEquals(link.getId(), actual.getInLinks().get(id).getId()));
  }
