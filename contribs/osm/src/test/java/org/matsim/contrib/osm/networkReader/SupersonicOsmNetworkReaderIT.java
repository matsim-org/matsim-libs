  ...
  public void test_andorra() {
    ...
    Network expectedResult =
        NetworkUtils.readNetwork(
            Paths.get(utils.getInputDirectory()).resolve("expected-result.xml.gz").toString());

    log.info(
        "expected result contains: "
            + expectedResult.getLinks().size()
            + " links and "
            + expectedResult.getNodes().size()
            + " nodes");
    log.info(
        "result contains: "
    ...
    Network alternative =
        new SupersonicOsmNetworkReader.Builder()
            ...
            .setAfterLinkCreated(
                SupersonicOsmNetworkReader.adjustFreespeed(LinkProperties.DEFAULT_FREESPEED_FACTOR))
            .build()
            .read(Paths.get(utils.getPackageInputDirectory()).resolve("andorra-latest.osm.pbf"));
