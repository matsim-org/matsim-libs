...
public class SupersonicOsmNetworkReaderTest {
  ...
  public void singleLink() {
    ...
    double expectedLengthPart1 =
        CoordUtils.calcEuclideanDistance(
            new Coord(node1.getLongitude(), node1.getLatitude()),
            new Coord(node2.getLongitude(), node2.getLatitude()));
    double expectedLengthPart2 =
        CoordUtils.calcEuclideanDistance(
            new Coord(node2.getLongitude(), node2.getLatitude()),
            new Coord(node3.getLongitude(), node3.getLatitude()));
    double expectedLengthPart3 =
        CoordUtils.calcEuclideanDistance(
            new Coord(node3.getLongitude(), node3.getLatitude()),
            new Coord(node4.getLongitude(), node4.getLatitude()));
    assertEquals(
        expectedLengthPart1 + expectedLengthPart2 + expectedLengthPart3, link.getLength(), 0);

    LinkProperties linkProperties = LinkProperties.createMotorway();
    assertEquals(linkProperties.freespeed, link.getFreespeed(), 0);
    assertEquals(
        linkProperties.laneCapacity * linkProperties.lanesPerDirection, link.getCapacity(), 0);
  ...
  public void singleLink_withMaxSpeedTag() {
    ...
    Way wayWithMaxSpeed =
        new Way(
            1,
            nodeReference,
            Arrays.asList(new Tag(OsmTags.HIGHWAY, TERTIARY), new Tag(OsmTags.MAXSPEED, "60")));
    ...
    assertEquals(
        link.getFreespeed(),
        (double) link.getAttributes().getAttribute(NetworkUtils.ALLOWED_SPEED),
        0);
  }
  ...
  public void singleLink_withMaxSpeedTag_milesPerHour() {
    ...
    Node node2 = new Node(2, 10, 10);
    TLongArrayList nodeReference = new TLongArrayList(new long[] {node1.getId(), node2.getId()});
    Way wayWithMaxSpeedMph =
        new Way(
            1,
            nodeReference,
            Arrays.asList(new Tag(OsmTags.HIGHWAY, TERTIARY), new Tag(OsmTags.MAXSPEED, "60 mph")));

    Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "single-link-with-max-speed-in-mph.pbf");
  ...
  public void singleLink_withMaxSpeedTag_urbanLink() {
    ...
    Way wayWithMaxSpeedUrban =
        new Way(
            1,
            nodeReference,
            Arrays.asList(new Tag(OsmTags.HIGHWAY, TERTIARY), new Tag(OsmTags.MAXSPEED, "50")));

    Path file =
        Paths.get(
            matsimTestUtils.getOutputDirectory(), "single-link-with-max-speed-urban-link.pbf");
    writeOsmData(
        Arrays.asList(node1, node2), Collections.singletonList(wayWithMaxSpeedUrban), file);
    ...
    assertEquals(
        50 / 3.6, (double) link.getAttributes().getAttribute(NetworkUtils.ALLOWED_SPEED), 0);
  }
  ...
  public void singleLink_withMaxSpeedTag_cantParseMaxSpeed() {
    ...
    Way wayWithInvalidMaxSpeed =
        new Way(
            1,
            nodeReference,
            Arrays.asList(
                new Tag(OsmTags.HIGHWAY, MOTORWAY), new Tag(OsmTags.MAXSPEED, "not a number")));

    Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "single-link-with-unknown-max-speed.pbf");
    writeOsmData(
        Arrays.asList(node1, node2), Collections.singletonList(wayWithInvalidMaxSpeed), file);
  ...
  public void singleLink_noMaxSpeedTag_ruralLink() {
    ...
    Way wayWithoutMaxSpeed =
        new Way(1, nodeReference, Collections.singletonList(new Tag(OsmTags.HIGHWAY, TERTIARY)));

    Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "single-link-no-max-speed-rural-link.pbf");
  ...
  public void singleLink_noMaxSpeedTag_urbanLink() {
    ...
    Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "single-link-no-max-speed-urban-link.pbf");
    writeOsmData(Arrays.asList(node1, node2), Collections.singletonList(wayWithoutMaxSpeed), file);
    ...
    // the freespeed for 'urban' links (links without a speed tag and shorter than 300m) freespeed
    // is reduced depending on the length of the link
  ...
  public void singleLink_noLanesTag() {
    ...
    Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "single-link-with-no-lanes-tag.pbf");
  ...
  public void singleLink_withLanesTag() {
    ...
    Way way =
        new Way(
            1,
            nodeReference,
            Arrays.asList(new Tag(OsmTags.HIGHWAY, TERTIARY), new Tag("lanes", "4")));
  ...
  public void singleLink_lanesTagOneWay() {
    ...
    Way way =
        new Way(
            ...
            nodeReference,
            Arrays.asList(
                new Tag(OsmTags.HIGHWAY, TERTIARY),
                new Tag("lanes", "4"),
                new Tag("oneway", "true")));

    Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "single-oneway-link-with-lanes-tag.pbf");
    writeOsmData(Arrays.asList(node1, node2), Collections.singletonList(way), file);
    ...
    Way way =
        new Way(
            ...
            Arrays.asList(
                new Tag(OsmTags.HIGHWAY, TERTIARY),
                new Tag("lanes", "4"),
                new Tag("lanes:forward", "4"),
                new Tag("lanes:backward", "1")));

    Path file =
        Paths.get(
            matsimTestUtils.getOutputDirectory(),
            "single-link-with-lanes-forward-and-backward-tag.pbf");
    writeOsmData(Arrays.asList(node1, node2), Collections.singletonList(way), file);
  ...
  public void singleLink_capacityLongLink() {
    ...
    Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "single-link-capacity-for-long-link.pbf");
    writeOsmData(Arrays.asList(node1, node2), Collections.singletonList(way), file);
  ...
  public void singleLink_capacityShortLink() {
    ...
    Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "single-link-capacity-for-short-link.pbf");
    writeOsmData(Arrays.asList(node1, node2), Collections.singletonList(way), file);
  ...
  public void singleLink_overridingLinkProperties() {
    ...
    Way way =
        new Way(
            1, nodeReference, Collections.singletonList(new Tag(OsmTags.HIGHWAY, linkCategory)));

    Path file =
        Paths.get(
            matsimTestUtils.getOutputDirectory(), "single-link-overriding-link-properties.pbf");
    writeOsmData(Arrays.asList(node1, node2), Collections.singletonList(way), file);
  ...
  public void twoIntersectingLinks() {
    ...
    assertEquals(
        CoordUtils.calcEuclideanDistance(
            link1.getFromNode().getCoord(), link1.getToNode().getCoord()),
        link1.getLength(),
        0);
    ...
    assertEquals(
        CoordUtils.calcEuclideanDistance(
            link3.getFromNode().getCoord(), link3.getToNode().getCoord()),
        link3.getLength(),
        0);
    ...
    assertEquals(
        CoordUtils.calcEuclideanDistance(
            link4.getFromNode().getCoord(), link4.getToNode().getCoord()),
        link4.getLength(),
        0);
    ...
    assertEquals(
        CoordUtils.calcEuclideanDistance(
            link5.getFromNode().getCoord(), link5.getToNode().getCoord()),
        link5.getLength(),
        0);
    ...
    assertEquals(
        CoordUtils.calcEuclideanDistance(
            link6.getFromNode().getCoord(), link6.getToNode().getCoord()),
        link6.getLength(),
        0);
  }
  ...
  public void twoIntersectingLinks_withAfterLinkCreatedHook() {
    ...
    final List<OsmWay> ways =
        Arrays.asList(
            new Way(1, new TLongArrayList(new long[] {1, 2, 3}), tags),
    ...
    HashSet<String> allowedModes =
        new HashSet<>(Arrays.asList(TransportMode.car, TransportMode.airplane));
    Network network =
        new SupersonicOsmNetworkReader.Builder()
            .setCoordinateTransformation(transformation)
  ...
  public void twoIntersectingLinks_oneShouldBeSimplified() {
    ...
    final List<OsmWay> ways =
        Arrays.asList(
            new Way(1, new TLongArrayList(new long[] {1, 2, 3, 6}), tags),
            new Way(2, new TLongArrayList(new long[] {4, 2, 5}), tags));
    final Path file = Paths.get(matsimTestUtils.getOutputDirectory(), "two-intersecting-links.pbf");
    writeOsmData(nodes, ways, file);
    ...
    Network network =
        new SupersonicOsmNetworkReader.Builder()
            ...
            .setIncludeLinkAtCoordWithHierarchy(
                (coord, level) -> !(level == LinkProperties.LEVEL_TERTIARY && coord.getX() > 100))
            .build()
            .read(file);
  ...
  public void twoIntersectingLinks_oneWithLoop() {
    ...
    final List<OsmWay> ways =
        Arrays.asList(
            new Way(1, new TLongArrayList(new long[] {1, 2, 6, 7, 8, 6, 3}), tags),
            new Way(2, new TLongArrayList(new long[] {4, 2, 5}), tags));
    final Path file =
        Paths.get(matsimTestUtils.getOutputDirectory(), "two-intersecting-links-with-loop.pbf");
    writeOsmData(nodes, ways, file);
  ...
  private void assertOriginalGeometryOnLink(Link link, List<OsmNode> osmNodes, boolean isForward) {
    ...
    for (var i = 0; i != osmNodes.size(); i++) {
      ...
      // if forward link traverse forward, otherwise the nodes should be reversed compared to
      // original geometry
      var actualNode =
          isForward
              ? originalGeometry.get(i)
              : originalGeometry.get(originalGeometry.size() - 1 - i);
