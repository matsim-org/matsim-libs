...
import org.matsim.contrib.matsim4opus.utils.helperObjects.MeasurementObject;
	...
	protected Population readUrbansimPersons(ReadFromUrbansimParcelModel readFromUrbansim, ActivityFacilitiesImpl facilities, NetworkImpl network){
		...
		if(measurements != null){
			try {
				...
				newPopulation = readFromUrbansim.readPersons( oldPopulation, facilities, network, Double.parseDouble( scenario.getConfig().getParam(Constants.MATSIM_4_URBANSIM_PARAM, Constants.SAMPLING_RATE) ) ) ;
				...
				try {
					...
					for ( ActivityFacility fromZone : zones.getFacilities().values() ) {
						...
						for ( ActivityFacility toZone : zones.getFacilities().values() ) {
							...
							double arrivalTime = lcpt.getTree().get(toNode.getId()).getTime();
