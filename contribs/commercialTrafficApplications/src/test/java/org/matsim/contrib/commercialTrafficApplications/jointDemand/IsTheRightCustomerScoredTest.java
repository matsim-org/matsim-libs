...
public class IsTheRightCustomerScoredTest {
  ...
  public void setUp() {
    ...
    CarriersUtils.getCarriers(scenario)
        .getCarriers()
        .get(Id.create("salamiPizza", Carrier.class))
  ...
  private void preparePopulation(Scenario scenario) {
    ...
    Person customerOrderingForParty =
        factory.createPerson(Id.createPersonId("customerOrderingForParty"));
    Person customerOrderingJustForItself =
        factory.createPerson(Id.createPersonId("customerOrderingJustForItself"));
    Person nonCustomer = factory.createPerson(Id.createPersonId("nonCustomer"));

    Plan planNonCustomer = factory.createPlan();
    Activity homeStarving =
        factory.createActivityFromLinkId("homeStarving", Id.createLinkId("116"));
    ...
    JointDemandUtils.addCustomerCommercialJobAttribute(
        pizzaParty, pizzaCarrier, 6, 6 * 3600, 7 * 3600, 180);
    ...
    JointDemandUtils.addCustomerCommercialJobAttribute(
        pizzaAlone, pizzaCarrier, 2, 6 * 3600, 7 * 3600, 180);
  ...
  public void testIfTheRightPersonIsScoredForReceivingAJob() {
    Plan partyPizzaPlan =
        scenario
            .getPopulation()
            .getPersons()
            .get(Id.createPersonId("customerOrderingForParty"))
            .getSelectedPlan();
    Plan lonelyPizzaPlan =
        scenario
            .getPopulation()
            .getPersons()
            .get(Id.createPersonId("customerOrderingJustForItself"))
            .getSelectedPlan();
    Plan nonCustomerPlan =
        scenario
            .getPopulation()
            .getPersons()
            .get(Id.createPersonId("nonCustomer"))
            .getSelectedPlan();

    // derive the service activity from the carrier plan and compare the service id (which should
    // contain the customer id) with the person id of the expected customer
    Carrier pizzaCarrier =
        CarriersUtils.getCarriers(scenario)
            .getCarriers()
            .get(Id.create("salamiPizza", Carrier.class));
    ScheduledTour tour =
        (ScheduledTour) pizzaCarrier.getSelectedPlan().getScheduledTours().toArray()[0];
    Id<CarrierService> serviceActivity =
        tour.getTour().getTourElements().stream()
            ...
            .findFirst()
            .orElseThrow(() -> new RuntimeException("no service activity found in scheduledTours"));

    Assert.assertEquals(
        "the person that is delivered pizza should be customerOrderingForParty",
        partyPizzaPlan.getPerson().getId().toString(),
        serviceActivity.toString().split("_")[0]);

    // compare scores
    Assert.assertTrue(
        "the plan of the customer receiving a job should get a higher score than the plan of the non customer ",
        partyPizzaPlan.getScore() > nonCustomerPlan.getScore());
    Assert.assertTrue(
        "the plan of the customer receiving a job should get a higher score than the plan of the customer not receiving one ",
        partyPizzaPlan.getScore() > lonelyPizzaPlan.getScore());
    Assert.assertTrue(
        "the difference of receiving a job in time and not receiving it at all should be the maxJobScore="
            + MAX_JOB_SCORE
            + " as job is performed in time",
        partyPizzaPlan.getScore() - lonelyPizzaPlan.getScore() == MAX_JOB_SCORE);
    Assert.assertEquals(
        "not receiving a job at all should be scored with zero",
        lonelyPizzaPlan.getScore(),
        0.0,
