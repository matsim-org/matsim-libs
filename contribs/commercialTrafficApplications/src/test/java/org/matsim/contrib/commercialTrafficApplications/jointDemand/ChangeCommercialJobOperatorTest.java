...
public class ChangeCommercialJobOperatorTest {
  ...
  public void getPlanAlgoInstance() {
    ...
    ChangeCommercialJobOperator changeCommercialJobOperator =
        new ChangeCommercialJobOperator(scenario.getConfig().global(), carriers);

    Plan testPlan =
        scenario.getPopulation().getPersons().get(Id.createPersonId(1)).getSelectedPlan();
    ...
    Assert.assertEquals(
        "the person should expect a pizza",
        "pizza",
        JointDemandUtils.getCarrierMarket(carriers.getCarriers().get(carrierId)));
    Assert.assertTrue(
        "the person should expect a pizza from the italian place",
        carrierId.toString().contains("italian"));
    ...
    Assert.assertTrue(
        "the person should expect a pizza from the american place",
        carrierId.toString().contains("american"));
