...
class ScoreCommercialJobs implements ActivityStartEventHandler, ActivityEndEventHandler {
  ...
  ScoreCommercialJobs(
      ...
      CommercialJobScoreCalculator scoreCalculator,
      ...
      freightPlan.getPlanElements().stream()
          ...
          .filter(
              act ->
                  ((Activity) act)
                      .getType()
                      .startsWith(CommercialJobGenerator.COMMERCIALJOB_ACTIVITYTYPE_PREFIX))
          .forEach(
              act -> {
                Id<CarrierService> serviceId =
                    Id.create(
                        (String)
                            act.getAttributes()
                                .getAttribute(CommercialJobGenerator.SERVICEID_ATTRIBUTE_NAME),
  ...
  private void handleFreightActivityStart(ActivityStartEvent event) {
    if (event.getActType().equals(CarrierConstants.END)) {
      ...
      CarrierService job =
          carrier.getServices().get(freightAgent2Jobs.get(event.getPersonId()).poll());

      Id<Person> customerAboutToBeServed =
          Id.createPersonId(
              (String)
                  job.getAttributes().getAttribute(CommercialJobGenerator.CUSTOMER_ATTRIBUTE_NAME));
      ...
      eventsManager.processEvent(
          new PersonScoreEvent(
              event.getTime(), customerAboutToBeServed, score, "jobStart_" + job.getId()));
      logEntries.add(
          new DeliveryLogEntry(
              customerAboutToBeServed,
              carrier.getId(),
              event.getTime(),
              score,
              event.getLinkId(),
  ...
  private double calcDifference(CarrierService service, double time) {
    if (time < service.getServiceStartTimeWindow().getStart())
      return (service.getServiceStartTimeWindow().getStart() - time);
    else if (time >= service.getServiceStartTimeWindow().getStart()
