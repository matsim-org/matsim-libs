...
class CommercialTrafficAnalysisListener implements IterationEndsListener {
  ...
  @Inject private MatsimServices services;

  @Inject private Carriers carriers;

  @Inject private ScoreCommercialJobs scoreCommercialJobs;

  @Inject private TourLengthAnalyzer tourLengthAnalyzer;

  private boolean firstIteration = true;
  private String sep = ";";
  private Map<String, Map<Id<Carrier>, Map<Integer, Double>>> carrierShareHistories =
      new HashMap<>();
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    writeJobStats(
        services
            .getControlerIO()
            .getIterationFilename(event.getIteration(), "commercialJobStats.csv"));
  ...
  private void analyzeCarrierMarketShares(int iteration) {
    ...
    Map<String, Set<Id<Carrier>>> carriersSplitByMarket =
        JointDemandUtils.splitCarriersByMarket(carriers);

    for (Map.Entry<String, Set<Id<Carrier>>> entry : carriersSplitByMarket.entrySet()) {
      Map<Id<Carrier>, Map<Integer, Double>> marketShareHistory =
          carrierShareHistories.getOrDefault(entry.getKey(), new HashMap<>());
      ...
      for (Id<Carrier> carrierId : entry.getValue()) {
        carrierDeliveries.put(
            carrierId,
            scoreCommercialJobs.getLogEntries().stream()
      ...
      try (CSVPrinter csvPrinter =
          new CSVPrinter(
              Files.newBufferedWriter(
                  Paths.get(
                      services
                          .getControlerIO()
                          .getOutputFilename("output_marketshare_" + entry.getKey() + ".csv")),
                  StandardOpenOption.CREATE,
                  StandardOpenOption.WRITE,
                  StandardOpenOption.APPEND),
              CSVFormat.DEFAULT.withDelimiter(sep.charAt(0)))) {
        if (firstIteration) {
        ...
        for (Map.Entry<Id<Carrier>, Long> del : carrierDeliveries.entrySet()) {
          Map<Integer, Double> history =
              marketShareHistory.getOrDefault(del.getKey(), new HashMap<>());
      ...
      XYLineChart chart =
          new XYLineChart(
              entry.getKey() + " Carrier market Share Statistics",
              "iteration",
              "share of jobs in market " + entry.getKey());
      for (Map.Entry<Id<Carrier>, Map<Integer, Double>> idMapEntry :
          marketShareHistory.entrySet()) {
      ...
      chart.saveAsPng(
          services
              .getControlerIO()
              .getOutputFilename("output_marketshare_" + entry.getKey() + ".png"),
          800,
          600);
    }
  }
  ...
  private void writeJobStats(String filename) {
    Collections.sort(
        scoreCommercialJobs.getLogEntries(),
        Comparator.comparing(ScoreCommercialJobs.DeliveryLogEntry::getTime));
    try (CSVPrinter csvPrinter =
        new CSVPrinter(
  ...
  private void writeIterationCarrierStats(IterationEndsEvent event) {
    for (Carrier carrier : carriers.getCarriers().values()) {
      DoubleSummaryStatistics distances =
          tourLengthAnalyzer.getDeliveryAgentDistances().entrySet().stream()
              .filter(
                  entry ->
                      JointDemandUtils.getCarrierIdFromDriver(entry.getKey())
                          .equals(carrier.getId()))
              .mapToDouble(Map.Entry::getValue)
              .summaryStatistics();
      DoubleSummaryStatistics scores =
          scoreCommercialJobs.getLogEntries().stream()
              ...
              .summaryStatistics();
      DoubleSummaryStatistics timeDerivations =
          scoreCommercialJobs.getLogEntries().stream()
              .filter(deliveryLogEntry -> deliveryLogEntry.getCarrierId().equals(carrier.getId()))
          ...
          new CSVPrinter(
              Files.newBufferedWriter(
                  Paths.get(
                      services
                          .getControlerIO()
                          .getOutputFilename("carrierStats." + carrier.getId() + ".csv")),
                  StandardOpenOption.CREATE,
                  StandardOpenOption.WRITE,
                  StandardOpenOption.APPEND),
              CSVFormat.DEFAULT.withDelimiter(sep.charAt(0)))) {
