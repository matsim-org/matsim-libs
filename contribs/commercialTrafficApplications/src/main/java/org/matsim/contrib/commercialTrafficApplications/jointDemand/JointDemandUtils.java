...
public class JointDemandUtils {
  ...
  //    attribute name="commercialJob[NUMBER]"
  // class="java.util.Collection">["OPERATOR","CAPACITYDEMAND","EARLIESTSTART","LATESTSTART","DURATION"]</attribute>
  ...
  public static Id<Carrier> getCurrentlySelectedCarrierForJob(
      Activity activity, int commercialJobIndex) {
    Collection<String> commercialJobProperties = getCommercialJob(activity, commercialJobIndex);
    return Id.create(
        (String) commercialJobProperties.toArray()[COMMERCIALJOB_ATTRIBUTE_CARRIER_IDX],
        Carrier.class);
  }
  ...
  public static Set<Id<Carrier>> getExistingOperatorsForMarket(Carriers carriers, String market) {
    return carriers.getCarriers().values().stream()
        .filter(
            carrier ->
                carrier.getAttributes().getAttribute(CARRIER_MARKET_ATTRIBUTE_NAME).equals(market))
        .map(Carrier::getId)
        .collect(Collectors.toSet());
  }

  public static Id<Person> generateDriverId(
      Carrier carrier, CarrierVehicle carrierVehicle, int nextId) {
    return Id.createPersonId(
        FREIGHT_DRIVER_PREFIX
            + "_"
            + carrier.getId()
            + "_veh_"
              ...
              if (key.startsWith(COMMERCIALJOB_ATTRIBUTE_NAME)) {
                if (((Collection) value).size() != 5)
                  throw new IllegalArgumentException(
                      "wrong length of commercialJob attribute for activity=" + activity);
  ...
  public static Map<String, Set<Id<Carrier>>> splitCarriersByMarket(Carriers carriers) {
    ...
    for (Id<Carrier> carrierId : carriers.getCarriers().keySet()) {
      ...
      Set<Id<Carrier>> carriersForMarket =
          carriersSplitByMarket.getOrDefault(market, new HashSet<>());
  ...
  public static Collection<String> getCommercialJob(Activity activity, int jobIndex) {
    return (Collection<String>)
        activity.getAttributes().getAttribute(COMMERCIALJOB_ATTRIBUTE_NAME + jobIndex);
  }
  ...
  private static String convertPropertiesArrayToAttributeValue(String[] jobProperties) {
    ...
    return propertiesString.substring(
        0, propertiesString.length() - 1); // cut off the last semicolon
  }

  public static void addCustomerCommercialJobAttribute(
      Activity activity,
      Id<Carrier> carrier,
      int amount,
    ...
    List<String> jobProperties =
        List.of(
            ...
            String.valueOf(earliestStart),
            String.valueOf(latestStart),
            String.valueOf(duration));
    if (activity
        .getAttributes()
        .getAsMap()
        .containsKey(COMMERCIALJOB_ATTRIBUTE_NAME + commercialJobIndex))
      throw new RuntimeException("job attribute nr " + commercialJobIndex + " already exists");
    activity
        .getAttributes()
        .putAttribute(COMMERCIALJOB_ATTRIBUTE_NAME + commercialJobIndex, jobProperties);
  }
}
