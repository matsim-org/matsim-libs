...
class TourPlanning {
  ...
  static void runTourPlanningForCarriersWithNetBasedCosts(
      Carriers carriers, Scenario scenario, int jSpritTimeSliceWidth, TravelTime travelTime)
    ...
    NetworkBasedTransportCosts.Builder netBuilder =
        NetworkBasedTransportCosts.Builder.newInstance(scenario.getNetwork(), vehicleTypes);
  ...
  static void runTourPlanningForCarriers(
      Carriers carriers, Scenario scenario, VRPTransportCosts transportCosts)
    ...
    HashMap<Id<Carrier>, Integer> sortedMap =
        carrierServiceCounterMap.entrySet().stream()
                ...
                tempList.parallelStream()
                    .forEach(
                        carrierId -> {
                          ...
                          if (serviceCount
                              > 1000) { // if problem is huge, take only half the share for
                            // replanning
                            radialShare = 0.15;
                            randomShare = 0.25;
                          }

                          int radialServicesReplanned =
                              Math.max(1, (int) (serviceCount * radialShare));
                          int randomServicesReplanned =
                              Math.max(1, (int) (serviceCount * randomShare));

                          // use this in order to set a 'hard' constraint on time windows
                          StateManager stateManager = new StateManager(problem);
                          ConstraintManager constraintManager =
                              new ConstraintManager(problem, stateManager);
                          constraintManager.addConstraint(
                              new ServiceDeliveriesFirstConstraint(),
                              ConstraintManager.Priority.CRITICAL);
                          constraintManager.addConstraint(
                              new VehicleDependentTimeWindowConstraints(
                                  stateManager,
                                  problem.getTransportCosts(),
                                  problem.getActivityCosts()),
                          ...
                          VehicleRoutingAlgorithm algorithm =
                              Jsprit.Builder.newInstance(problem)
                                  ...
                                  .setProperty(
                                      Jsprit.Parameter.THREADS, String.valueOf(jspritThreads))
                                  .setProperty(
                                      Jsprit.Parameter.RADIAL_MIN_SHARE,
                                      String.valueOf(radialServicesReplanned))
                                  .setProperty(
                                      Jsprit.Parameter.RADIAL_MAX_SHARE,
                                      String.valueOf(radialServicesReplanned))
                                  .setProperty(
                                      Jsprit.Parameter.RANDOM_BEST_MIN_SHARE,
                                      String.valueOf(randomServicesReplanned))
                                  .setProperty(
                          ...
                          // variationCoefficient = stdDeviation/mean. so i set the threshold rather
                          // soft
                          // algorithm.addTerminationCriterion(new
                          // VariationCoefficientTermination(5,
                          ...
                          CarrierPlan carrierPlan =
                              MatsimJspritFactory.createPlan(carrier, bestSolution);
