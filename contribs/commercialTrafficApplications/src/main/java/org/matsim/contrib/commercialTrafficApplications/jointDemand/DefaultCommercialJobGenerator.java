...
class DefaultCommercialJobGenerator implements CommercialJobGenerator {
  ...
  private static final Logger log = LogManager.getLogger(DefaultCommercialJobGenerator.class);
  ...
  /**
   * Converts Jsprit tours to MATSim freight agents and adjusts departure times of leg and end times
   * of activities
   */
  ...
  private void buildVehicleAndDriver(
      Carrier carrier, Person driverPerson, CarrierVehicle carrierVehicle) {
    ...
    VehicleUtils.insertVehicleIdsIntoAttributes(
        driverPerson, Map.of(CarriersUtils.getCarrierMode(carrier), vid));
    scenario
        .getVehicles()
        .addVehicle(
            scenario.getVehicles().getFactory().createVehicle(vid, carrierVehicle.getType()));
    freightVehicles.add(vid);
    freightDrivers.add(driverPerson.getId());
  }

  /**
   * Jsprit creates tours that begin with the opening time of the carrier. As a consequence, the
   * tour vehicle arrives may arrive too early at the first customer. Moreover, we need to enforce
   * that a service does not end too early (before the of it's TimeWindow), otherwise following
   * services would start too early. This method adjusts the (service) activity endTimes and leg
   * departure times to avoid too early starts of the services. The firsttourTraveltimeBuffer tries
   * to ensure that the carrier departures early enough (leg travel time x
   * firsttourTraveltimeBuffer) to reach his first destination (service).
   */
    ...
    for (int i = 0; i < planElements.size(); i++) {
      ...
      if (planElement instanceof Activity) {
        ...
        if (!currentActivity.getType().equals(CarrierConstants.START)) {
          ...
          if (!prevAct.getType().equals(CarrierConstants.START)) {
            ...
            if (nextLeg == null) {
              ...
              double expectedArrivalTime =
                  nextLeg.getDepartureTime().seconds()
                      - Double.parseDouble(
                          Objects.requireNonNull(
                                  currentActivity
                                      .getAttributes()
                                      .getAttribute(SERVICE_DURATION_NAME))
                              .toString());
              ...
              double earliestPrevActEndTime =
                  expectedArrivalTime - prevLeg.getTravelTime().seconds();
          ...
          double departureTimeAtFirstJob =
              ((Leg) planElements.get(i + 3)).getDepartureTime().seconds();
          double jobServiceDuration =
              Double.parseDouble(
                  Objects.requireNonNull(
                          planElements
                              .get(i + 2)
                              ...
                              .getAttribute(SERVICE_DURATION_NAME))
                      .toString());
          double initialLegDepartureTime =
              departureTimeAtFirstJob
                  - jobServiceDuration
                  - travelTimeToFirstJob * this.firstTourTraveltimeBuffer;
          double expectedArrivalTimeAtFirstJob =
              departureTimeAtFirstJob - travelTimeToFirstJob * this.firstTourTraveltimeBuffer;

          // Set optimal endTimes to avoid a too early arrival at first job
          planElements
              .get(i + 2)
              .getAttributes()
              .putAttribute(EXPECTED_ARRIVALTIME_NAME, expectedArrivalTimeAtFirstJob);
  ...
  private Plan createPlainPlanFromTour(Carrier carrier, ScheduledTour scheduledTour) {
    ...
    Activity startActivity =
        PopulationUtils.createActivityFromLinkId(
            CarrierConstants.START, scheduledTour.getVehicle().getLinkId());
    ...
    for (Tour.TourElement tourElement : scheduledTour.getTour().getTourElements()) {
      if (tourElement instanceof Tour.Leg) {
        ...
        Assert.isTrue(
            tourLeg.getRoute() != null, "Missing route for carrier: " + carrier.getId().toString());
        ...
        if (drtModes.contains(carrierMode)) {
          ...
          double routeDistance =
              RouteUtils.calcDistance((NetworkRoute) route, 1.0, 1.0, scenario.getNetwork());
        ...
        Activity defaultActivity =
            PopulationUtils.createActivityFromLinkId(actType, tourActivity.getLocation());
        defaultActivity.getAttributes().putAttribute(CUSTOMER_ATTRIBUTE_NAME, customer);
        defaultActivity
            .getAttributes()
            .putAttribute(SERVICEID_ATTRIBUTE_NAME, service.getId().toString());
    ...
    Activity endActivity =
        PopulationUtils.createActivityFromLinkId(
            CarrierConstants.END, scheduledTour.getVehicle().getLinkId());
  ...
  public void notifyBeforeMobsim(BeforeMobsimEvent event) {
    ...
    event
        .getServices()
        .getInjector()
        .getInstance(ScoreCommercialJobs.class)
        .prepareTourArrivalsForDay();
    String dir =
        event.getServices().getConfig().controller().getOutputDirectory()
            + "/ITERS/it."
            + event.getIteration()
            + "/";
  ...
  public void generateIterationServices(Carriers carriers, Population population) {
    ...
    population
        ...
        .forEach(
            p -> {
              Set<Activity> activitiesWithServices =
                  JointDemandUtils.getCustomerActivitiesExpectingJobs(p.getSelectedPlan());
              if (!activitiesWithServices.isEmpty())
                customer2ActsWithJobs.put(p, activitiesWithServices);
            });
    for (Person customer : customer2ActsWithJobs.keySet()) {
      for (Activity activity : customer2ActsWithJobs.get(customer)) {
        Map<String, Object> commercialJobAttributes =
            JointDemandUtils.getCommercialJobAttributes(activity);
        for (String commercialJobAttributeKey : commercialJobAttributes.keySet()) {
          List<String> commercialJobProperties =
              new ArrayList<>(
                  (Collection<? extends String>)
                      commercialJobAttributes.get(commercialJobAttributeKey));

          int jobIdx =
              Integer.parseInt(
                  commercialJobAttributeKey.substring(
                      JointDemandUtils.COMMERCIALJOB_ATTRIBUTE_NAME.length()));
          Id<CarrierService> serviceId = createCarrierServiceIdXForCustomer(customer, jobIdx);

          double earliestStart =
              Double.parseDouble(commercialJobProperties.get(COMMERCIALJOB_ATTRIBUTE_START_IDX));
          double latestStart =
              Double.parseDouble(commercialJobProperties.get(COMMERCIALJOB_ATTRIBUTE_END_IDX));

          CarrierService.Builder serviceBuilder =
              CarrierService.Builder.newInstance(
                  serviceId, PopulationUtils.decideOnLinkIdForActivity(activity, scenario));
          serviceBuilder.setCapacityDemand(
              Integer.parseInt(commercialJobProperties.get(COMMERCIALJOB_ATTRIBUTE_AMOUNT_IDX)));
          serviceBuilder.setServiceDuration(
              Double.parseDouble(
                  commercialJobProperties.get(COMMERCIALJOB_ATTRIBUTE_DURATION_IDX)));
          serviceBuilder.setServiceStartTimeWindow(
              TimeWindow.newInstance(earliestStart, latestStart));

          Id<Carrier> carrierId =
              JointDemandUtils.getCurrentlySelectedCarrierForJob(activity, jobIdx);
          if (carriers.getCarriers().containsKey(carrierId)) {
            ...
            service
                .getAttributes()
                .putAttribute(CUSTOMER_ATTRIBUTE_NAME, customer.getId().toString());
            carrier.getServices().put(serviceId, service);
          } else {
            throw new RuntimeException(
                "Carrier Id does not exist: "
                    + carrierId.toString()
                    + ". There is a wrong reference in activity attribute of person "
                    + customer);
          }
        }
      }
  ...
  private void buildTours() throws InterruptedException, ExecutionException {
    TourPlanning.runTourPlanningForCarriers(
        carriers, scenario, vrpTransportCostsFactory.createVRPTransportCosts());
  }
  ...
  private void cleanCarriers() {
    CarrierVehicleTypes.getVehicleTypes(carriers)
        .getVehicleTypes()
        .keySet()
  ...
  private void toggleChangeCommercialJobOperatorStrategy(int currentIteration) {
    ...
    if (this.changeOperatorInterval != 0) {
      ...
      Collection<StrategySettings> allStrategies =
          this.scenario.getConfig().replanning().getStrategySettings();
      ...
      for (StrategySettings strategy : allStrategies) {
        if (strategy.getStrategyName().equals(ChangeCommercialJobOperator.SELECTOR_NAME)) {
          ...
          for (GenericPlanStrategy<Plan, Person> currentStrategy :
              this.strategyManager.getStrategies(null)) {
  ...
  private boolean isChangeCommercialJobOperatorStrategy(
