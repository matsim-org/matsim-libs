...
public class ProfileWriter implements IterationEndsListener {
  ...
  public ProfileWriter(
      MatsimServices matsimServices, String mode, ProfileView profileView, String fileName) {
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    try (CompactCSVWriter writer =
      ...
      for (int i = 0; i < times.length; i++) {
        var line =
            new CSVLineBuilder()
                .add(Time.writeTime(times[i], timeFormat))
                .addAll(cells(profiles, i));
  ...
  private void generateImage(
      DefaultTableXYDataset xyDataset, TimeProfileCharts.ChartType chartType) {
