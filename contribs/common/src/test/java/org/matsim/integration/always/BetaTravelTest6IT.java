...
/**
 * This TestCase should ensure the correct behavior of agents when different values for
 * beta_travel-parameters are used, it does so by basically implementing Vickrey's bottleneck model
 * (see "Economics of a bottleneck" by Arnott, De Palma and Lindsey, 1987). The agents drive on the
 * equil-net, but can only do departure-time adaptation, no rerouting. This leads to all agents
 * driving through the one bottleneck in the network. A special time-adaption module is used
 * (TimeAllocationMutatorBottleneck, inside this class), that only mutates the endtime of the very
 * first activity, but not the duration of other activities. This is to ensure that the duration of
 * the activities do not have any influence on the score, but only the travel-time (and with that,
 * eventually shortened activity durations) has.
 *
 * @author mrieser
 */
public class BetaTravelTest6IT {
  ...
  public void testBetaTravel_6() {
    ...
    ConfigUtils.loadConfig(
        config, utils.getInputDirectory() + "config.xml"); // specific setting for this test
    config.controller().setWritePlansInterval(0);
    config
        .plans()
        .setActivityDurationInterpretation(ActivityDurationInterpretation.tryEndTimeThenDuration);
    /*
  ...
  /**
   * Collects some statistics on a specific link. Used to automatically verify the TestCase still
   * works as intended.
   *
   * @author mrieser
   */
  ...
  private static class MyStrategyManagerProvider implements Provider<StrategyManager> {
    ...
    public StrategyManager get() {
      ...
      PlanStrategyImpl strategy1 =
          new PlanStrategyImpl(new ExpBetaPlanSelector<Plan, Person>(config.scoring()));
      ...
      strategy2.addStrategyModule(
          new TimeAllocationMutatorBottleneck(config.global().getNumberOfThreads()));

      // Trying to replace this by the standard mutator ...
      //			double mutationRange = 1800. ;
      //			boolean affectingDuration = false ;
      //			strategy2.addStrategyModule( new TimeAllocationMutator(config, mutationRange,
      // affectingDuration));
  ...
  /**
   * Responsible for the verification of the tests. It adds a few event handlers and checks their
   * result in a specific iteration.
   *
   * @author mrieser
   */
  private static class TestControlerListener
    ...
    public void notifyStartup(final StartupEvent event) {
      ...
      if ((beta_travel != -6.0) && (beta_travel != -66.0)) {
        throw new IllegalArgumentException(
            "Unexpected value for beta_travel. Expected -6.0 or -66.0, actual value is "
                + beta_travel);
      }
      ...
      if (lastIter < 100) {
        throw new IllegalArgumentException(
            "Controler.lastIteration must be at least 100. Current value is " + lastIter);
      }
      if (lastIter > 100) {
        System.err.println(
            "Controler.lastIteration is currently set to "
                + lastIter
                + ". Only the first 100 iterations will be analyzed.");
      }
      this.ttAnalyzer =
          new BottleneckTravelTimeAnalyzer(
    ...
    public void notifyIterationEnds(final IterationEndsEvent event) {
      ...
      if (iteration % 50 == 0) {
        this.ttAnalyzer.plot(
            event
                .getServices()
                .getControlerIO()
                .getIterationFilename(event.getIteration(), "bottleneck_times.png"));
      ...
      if (iteration == 100) {
        ...
        if (beta_travel == -6.0) {
          ...
          System.out.println(
              "firstCarEnter="
                  + this.la.firstCarEnter
                  + "; lastCarEnter="
                  + this.la.lastCarEnter
                  + "; firstCarLeave="
                  + this.la.firstCarLeave
                  + "; lastCarLeave="
                  + this.la.lastCarLeave
                  + "; maxCarsOnLink="
                  + this.la.maxCarsOnLink
                  + "; maxCarsOnLinkTime="
                  + this.la.maxCarsOnLinkTime);
          ...
          // The results changed after Michał's change of the QueueWithBuffer slow capacity update.
          // See 4dda004a8db98d9a2e757b1896cd3250d4e84ba5 .
      ...
      for (PlanElement pe : plan.getPlanElements()) {
        ...
        if (pe instanceof Activity) {
          ...
          if (i == 0) {
            ...
            act.setEndTime(
                mutateTime(act.getEndTime())); // mutate the end time of the first activity
            act.setMaximumDuration(
                act.getEndTime().seconds()
                    - act.getStartTime().seconds()); // calculate resulting duration
            now += act.getEndTime().seconds(); // move now pointer
          } else if (i < (max - 1)) {
            // handle middle activities
            act.setStartTime(
                now); // assume that there will be no delay between arrival time and activity start
            // time
            act.setMaximumDuration(
                6 * 3600); // <-- This line differs from the original PlanMutateTimeAllocation,
            // use a fix time to minimize effect of act-duration on score
            act.setEndTimeUndefined(); // <-- This line differs from the original
  ...
  /**
   * Collects the departure and arrival times of the first leg of each agent, and plots them in
   * XY-Scatter-Plot for manual verification in case this test case fails.
   *
   * @author mrieser
   */
  private static class BottleneckTravelTimeAnalyzer
    ...
    protected void plot(final String filename) {
      XYScatterChart graph =
          new XYScatterChart("Bottleneck Analysis", "departure time", "arrival time");
