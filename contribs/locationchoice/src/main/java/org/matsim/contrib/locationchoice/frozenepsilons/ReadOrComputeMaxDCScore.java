...
class ReadOrComputeMaxDCScore {
  ...
  private static final Logger log = LogManager.getLogger(ReadOrComputeMaxDCScore.class);
  ...
  public ReadOrComputeMaxDCScore(DestinationChoiceContext lcContext) {
    ...
    this.dccg =
        (FrozenTastesConfigGroup)
            scenario.getConfig().getModule(FrozenTastesConfigGroup.GROUP_NAME);
  ...
  public void readOrCreateMaxDCScore(boolean arekValsRead) {
    ...
    if (maxEpsValuesFileName != null && arekValsRead) {
      try {
        ObjectAttributesXmlReader maxEpsReader =
            new ObjectAttributesXmlReader(this.personsMaxDCScoreUnscaled);
        maxEpsReader.readFile(maxEpsValuesFileName);
        for (Person p : this.scenario.getPopulation().getPersons().values()) {
          for (String flexibleType : this.flexibleTypes) {
            double maxType =
                (Double) personsMaxDCScoreUnscaled.getAttribute(p.getId().toString(), flexibleType);
            if (maxType != 0) {
        ...
        log.error(
            "unsuccessful reading of maxDCScore from file!\nThe values are now computed"
  ...
  private void computeMaxDCScore() {
    ...
    for (String actType : this.scaleEpsilon.getFlexibleTypes()) {
      ...
      ComputeMaxDCScoreMultiThreatedModule maxEpsilonComputer =
          new ComputeMaxDCScoreMultiThreatedModule(actType, this.lcContext, sampler);
