 ...
 *       Default: null <br>
 *       Description: This parameter globally (for all agents) specifies the activity types for
 *       which location choice is performed. For demand v1 instead of x0.5 ... x24, simply x has to
 *       be given.
 ...
 *       Type and range: String <br>
 *       Default: null<br>
 *       Description: If the destinations have to be chosen from an additionally restrained region a
 *       center node and a radius can be set. This is only useful for diluted scenarios
 ...
 *       Type and range: String <br>
 *       Default: null <br>
 *       Description: specifies the location of the persons k values file (created during previous
 *       pre-processing)
 *   <li><strong>fkValuesFile</strong><br>
 *       Type and range: String <br>
 *       Default: null <br>
 *       Description: specifies the location of the facilities k values file (created during
 *       previous pre-processing)
 *   <li><strong>prefsFile</strong><br>
 *       Type and range: String <br>
 *       Default: null <br>
 *       Description: Specifies the agents' earliestEndTime, latestStartTime, minimalDuration and
 *       typicalDuration. If no prefs file is found, the module tries to get the typicalDuration
 *       from the desires and the rest from the config. If also no desires are available, then
 *       everything is tried to be taken from the config. And finally, if also in the config not
 *       everything is there, you need to help yourself ;) This should sooner or later go somewhere
 *       else, more into the core.
 *   <li><strong>pBetasFileName</strong><br>
 *       Type and range: String <br>
 *       Default: null <br>
 *       Description: Specifies the beta parameters for additional destination attributes specified
 *       in fAttributesFileName below
 ...
 *       Default: 0 <br>
 *       Description: 0: no approximation, routing for every OD-pair (for real-world scenarios not
 *       applicable as it is much too slow)<br>
 *       1: Dijkstra trees forward and backwards. See working paper Horni, Nagel, Axhausen 2011,
 *       page 13<br>
 *       2: based on travel distances
 *   <li><strong>maxDistanceDCScore</strong><br>
 *       Type and range: float (-1 or > 0.0) <br>
 *       Default: -1 <br>
 *       Description: Maximum search space radius [m]. "-1" means no explicit fixed search space
 *       limitation.
 *   <li><strong>probChoiceSetSize</strong><br>
 *       Type and range: int > 0 <br>
 *       Default: 5 <br>
 *       Description: Number of alternatives taken into account for the probabilistic choice. See
 *       working paper Horni, Nagel, Axhausen 2011, page 13
 *   <li><strong>maxDCScoreFile</strong><br>
 *       Type and range: String <br>
 *       Default: null <br>
 *       Description: specifies the location of the unscaled max dc score file (created during
 *       pre-processing). If the k values have not changed from one run to another, the (unscaled)
 *       max dc score values can be reused and the pre-processing can be skipped.
 ...
 *       But here used to exclude all agents from destination choice and analysis with id >
 *       analysisIdExclusion. (e.g., border crossers)
 *   <li><strong>travelSpeed_car</strong><br>
 *       Type and range: double > 0.0<br>
 *       Default: 8.5 m/s<br>
 *       Description: approximated assumed car travel speed to convert time into distance for search
 *       space construction.
 *   <li><strong>travelSpeed_pt</strong><br>
 *       Type and range: double > 0.0<br>
 *       Default: 5 m/s<br>
 *       Description: approximated assumed pt travel speed to convert time into distance for search
 *       space construction.
 *   <li><strong>destinationSamplePercent</strong><br>
 *       Type and range: double, 3 possible values: 100.0, 10.0, and 1.0<br>
 *       Default: 100.0<br>
 *       Description: specifies how many percent of the destinations in the choice set should be
 *       used for the final choice
 * </ul>
 *
 * <h3>Analysis</h3>
 *
