...
public class FrozenTastesConfigGroup extends ReflectiveConfigGroup
    implements DestinationChoiceConfigGroupI {
  private static final Logger log = LogManager.getLogger(FrozenTastesConfigGroup.class);
  ...
  /**
   * penalty roughly is restr_factor * #persons^restr_exp. Only needed when facility load penalties
   * are used
   */
  ...
  /**
   * I think that this is how much to scale the epsilons for each activity type. comma-separated
   * list, corresponding to comma-separated list of flexible activity types.
   */
  ...
  @Deprecated // TODO replace by replaceable implementation behind interface (and move to analysis
  // contrib)
  ...
  /**
   * Some distance cut-off for {@link DistanceStats}. But with a more flexible data structure there
