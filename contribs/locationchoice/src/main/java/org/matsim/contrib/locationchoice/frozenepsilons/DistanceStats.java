...
class DistanceStats implements IterationEndsListener {
  ...
  public DistanceStats(Config config, String bestOrSelected, String type, String mode) {
    ...
    this.mode = mode;
    this.bins =
        new Bins(this.dccg.getAnalysisBinSize(), analysisBoundary, type + "_" + mode + "_distance");
  }
  ...
  public void notifyIterationEnds(final IterationEndsEvent event) {
    ...
    for (Person p : event.getServices().getScenario().getPopulation().getPersons().values()) {
      ...
      if (this.dccg.getIdExclusion() == null
          || !this.isLong(p.getId().toString())
          ...
          if (((Activity) pe).getType().equals(type)
            ...
            double distance =
                CoordUtils.calcEuclideanDistance(
                    ...
                    PopulationUtils.getPreviousActivity(
                            plan, PopulationUtils.getPreviousLeg(plan, (Activity) pe))
                        .getCoord());
    ...
    // Actually, path is not the full file name - inside the plotBinnedDistribution some other stuff
    // is added.
    String path =
        event
            .getServices()
            .getControlerIO()
            .getIterationFilename(event.getIteration(), "plan=" + this.bestOrSelected + "_");
