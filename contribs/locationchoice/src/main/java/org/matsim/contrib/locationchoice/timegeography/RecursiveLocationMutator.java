...
class RecursiveLocationMutator extends AbstractLocationMutator {
  ...
  public RecursiveLocationMutator(
  ...
  private int handleSubChain(Person person, SubChain subChain, double speed, int trialNr) {
    if (trialNr > this.maxRecursions) {
      ...
      while (act_it.hasNext()) {
        ...
        this.modifyLocation(
            (Activity) act, subChain.getStartCoord(), subChain.getEndCoord(), Double.MAX_VALUE, 0);
      }
      return 0;
    }
  ...
  private boolean modifyLocation(
      Activity act, Coord startCoord, Coord endCoord, double radius, int trialNr) {

    ArrayList<ActivityFacility> choiceSet =
        this.computeChoiceSetCircle(startCoord, endCoord, radius, act.getType());
  ...
  private List<SubChain> calcActChainsDefinedFixedTypes(final Plan plan) {
    ...
    for (int j = 0; j < actslegs.size(); j = j + 2) {
      ...
      if (super.getDefineFlexibleActivities()
          .getFlexibleTypes()
          .contains(act.getType())) { // found secondary activity
  ...
  private ArrayList<ActivityFacility> computeChoiceSetCircle(
      Coord coordStart, Coord coordEnd, double radius, String type) {
    double midPointX = (coordStart.getX() + coordEnd.getX()) / 2.0;
    double midPointY = (coordStart.getY() + coordEnd.getY()) / 2.0;
    return (ArrayList<ActivityFacility>)
        this.getQuadTreesOfType().get(type).getDisk(midPointX, midPointY, radius);
  }
