...
/**
 * Optimized data structure used in the location choice contribution. There, a huge amount of plans
 * are created and evaluated, which is time consuming and increases the workload for the garbage
 * collector.
 *
 * <p>This class uses an optimized data structure designed especially for that application. There is
 * no guarantee that it can be utilized for other applications!
 *
 * @author cdobler
 */
class LCPlan implements Plan {
  ...
  public static void copyFrom1(Plan srcPlan, LCPlan destPlan) {
    ...
    for (PlanElement planElement : srcPlan.getPlanElements()) {
      if (planElement instanceof Activity) activityCount++;
        ...
        throw new RuntimeException(
            "Found unexpected PlanElement type: "
                + planElement.getClass().toString()
                + ". Aborting!");
    }
    ...
    for (PlanElement planElement : srcPlan.getPlanElements()) {
      if (planElement instanceof Activity) {
        ...
        Leg r = ((Leg) leg);
        destPlan.arrTimes[legCount] =
            OptionalTime.defined(r.getDepartureTime().seconds() + r.getTravelTime().seconds());
  ...
  public static void copyFrom(Plan srcPlan, LCPlan destPlan) {
    if (srcPlan instanceof Plan) copyFrom1((Plan) srcPlan, destPlan);
      ...
      throw new RuntimeException(
          "Found unexpected source plan type: " + srcPlan.getClass().toString() + ". Aborting!");
  }
