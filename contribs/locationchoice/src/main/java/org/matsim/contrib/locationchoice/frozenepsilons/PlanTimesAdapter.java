...
class PlanTimesAdapter {
  ...
  /* package */ double scorePlan(
      final Plan planTmp, final ScoringFunctionFactory scoringFunctionFactory, Person person) {
    // We need a copy of the plan since for scoring other fields need to be filled out than in the
    // original plan (e.g. activityEndTime).  The plan is afterwards thrown
    ...
    for (PlanElement pe : planTmp.getPlanElements()) {
      if (pe instanceof Activity) {
        ...
        // the scoring needs, for "middle" acts, actStart/EndTime filled out, even if the mobsim
        // does not need that:
        if (firstAct) {
          firstAct = false;
        } else {
        ...
        now =
            timeInterpretation
                .decideOnActivityEndTimeAlongPlan(
                    Objects.requireNonNull(rememberedActivity), planTmp)
                .seconds();
    ...
    Activity lastAct =
        (Activity) planTmp.getPlanElements().get(planTmp.getPlanElements().size() - 1);
    lastAct.setStartTime(now);
    scoringFunction.handleActivity(lastAct);
    // the following is hedging against the newer tripscoring; not clear if this will work
    // out-of-sequence.
