...
final class BestReplyLocationChoicePlanAlgorithm implements PlanAlgorithm {
  private static final Logger log =
      LogManager.getLogger(BestReplyLocationChoicePlanAlgorithm.class);
  ...
  public BestReplyLocationChoicePlanAlgorithm(
      ...
      int iteration,
      Map<Id<ActivityFacility>, Id<Link>> nearestLinks,
      ...
      if (this.scaleEpsilon.isFlexibleType(activity.getType())) {
        ...
        if (actLegIndex < 1) {
          throw new RuntimeException(
              "can't get the previous activity from the first activity in the plan");
        }
        final Activity actPre = activities.get(actLegIndex - 1);
        if (activities.size() <= actLegIndex + 1) {
          throw new RuntimeException(
              "can't get the post activity from the last activity in the plan");
        }
        ...
        double maximumDistance =
            this.convertEpsilonIntoDistance(plan.getPerson(), actToMove.getType());
        ...
        ChoiceSet cs =
            createChoiceSetFromCircle(
                plan,
                personIndex,
                this.dccg.getTravelTimeApproximationLevel(),
                actToMove,
                maxRadius,
                center);

        // === this is where the work is done:
        final Id<ActivityFacility> choice =
            cs.getWeightedRandomChoice(
                actLegIndex,
                this.scoringFunctionFactory,
                plan,
                this.tripRouter,
                this.lcContext.getPersonsKValuesArray()[personIndex],
                this.forwardMultiNodeDijkstra,
                this.backwardMultiNodeDijkstra,
                this.iteration);
        // yy This looks like method envy, i.e. the method should rather be in this class here.
        // kai, mar'19
        // ===
  ...
  private ChoiceSet createChoiceSetFromCircle(
    ...
    Collection<ActivityFacilityWithIndex> list =
        quadTree.getDisk(center.getX(), center.getY(), maxRadius);
    ...
    for (ActivityFacilityWithIndex facility : list) {
      ...
      if (this.sampler.sample(facilityIndex, personIndex)) {
        ...
        // TODO: solve this generic. for that we need info from the config, which modes are actually
        // teleported.
        if (this.lcContext
                .getScenario()
                .getNetwork()
  ...
  private void setLocationOfActivityToFacilityId(Activity act2, Id<ActivityFacility> facilityId) {
    ...
    if (facility != null) {
      // (null can happen when the activity initially was located without a facility, and it did not
      // find an alternative facility in the choice
      // set. kai, jul'19)
  ...
  private double convertEpsilonIntoDistance(Person person, String type) {
    ...
    //		if (
    // Boolean.getBoolean(this.scenario.getConfig().vspExperimental().getValue(VspExperimentalConfigKey.isUsingOpportunityCostOfTimeForLocationChoice)) ) {
    if (this.scenario
        .getConfig()
        .vspExperimental()
    ...
    if (betaTime != 0.0) {
      if (betaTime >= 0.) {
        throw new RuntimeException(
            "betaTime >= 0 in location choice; method not designed for this; aborting ...");
      }
      maxTravelTime =
          Math.abs(
              maxDCScore
                  / (-1.0 * betaTime)
                  * 3600.0); // [s] // abs used for the case when somebody defines beta > 0
      // yy maybe one can still used it with betaTime>0 (i.e. traveling brings utility), but taking
      // the "abs" of it is almost certainly not the
    ...
    // yyyy it is quite stupid to do the following at the end of this method and not at the
    // beginning.  Can probably be moved up, but would need to be tested.  kai, jul'23
