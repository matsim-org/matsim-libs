...
class ChoiceSet {
  ...
  ChoiceSet(
      FrozenTastesConfigGroup.ApproximationLevel approximationLevel,
      Scenario scenario,
    ...
    this.dccg =
        (FrozenTastesConfigGroup)
            this.scenario.getConfig().getModule(FrozenTastesConfigGroup.GROUP_NAME);
  ...
  Id<ActivityFacility> getWeightedRandomChoice(
      int actlegIndex,
    ...
    if (this.destinations.size() > 0) {
      ...
      list =
          this.createReducedChoiceSetWithPseudoScores(
              actlegIndex, this.facilities, scoringFunction, planTmp, tripRouter);
      // ===
    } else {
      // if we have no destinations defined so far, we can shorten this
      // currently handled activity which should be re-located
      Activity act =
          TripStructureUtils.getActivities(plan, ExcludeStageActivities).get(actlegIndex);
  ...
  /**
   * the following two lines seem to be outdated: The "score", which is behind the "Double" in the
   * TreeMap, is some pseudo score 0.6, 0.84, ..., see
   * {ChoiceSet#generateReducedChoiceSet(ArrayList)}. Well, no, not any more, just setting all of
   * them to 0.2.
   *
   * <p>because of the return-typ from the constructPath method from the MultiNodeDijkstra and
   * BackwardFastMultiNodeDijkstra class, we don't overwrite the whole path between activity we want
   * to change and the activity before/after that. That means the score includes the new path and
   * the old path, because it is the same in all destinations and the score is not used in the plan
   * file, we decided to keep the information on the basis of a cleaner code
   */
  private List<ScoredAlternative> createReducedChoiceSetWithPseudoScores(
    ...
    if (this.approximationLevel == FrozenTastesConfigGroup.ApproximationLevel.localRouting) {
      ...
      // (2) the (backwards) Dijkstra tree from all these destinations to the activity following the
      // given activity.
      // The two trees will then be passed into the (preliminary) scoring of these destinations,
      // where they will be evaluated for the
      ...
      for (Id<ActivityFacility> destinationId : this.destinations) {
        ActivityFacility destinationFacility =
            this.scenario.getActivityFacilities().getFacilities().get(destinationId);
      ...
      // (1) forward tree
      {
        ...
        Node nextActNode =
            this.network
                .getLinks()
                .get(PopulationUtils.decideOnLinkIdForActivity(previousActivity, scenario))
                .getToNode();

        forwardMultiNodeDijkstra.setSearchAllEndNodes(true);
        forwardMultiNodeDijkstra.calcLeastCostPath(
            nextActNode,
            destinationNode,
            timeInterpretation
                .decideOnActivityEndTimeAlongPlan(previousActivity, planTmp)
                .seconds(),
            planTmp.getPerson(),
            null);
      }
      ...
      // (2) backward tree
      {
        ...
        Node nextActNode =
            this.network
                .getLinks()
                .get(PopulationUtils.decideOnLinkIdForActivity(nextActivity, scenario))
                .getToNode();

        backwardMultiNodeDijkstra.setSearchAllEndNodes(true);
        backwardMultiNodeDijkstra.calcLeastCostPath(
            nextActNode,
            destinationNode,
            timeInterpretation
                .decideOnActivityEndTimeAlongPlan(activityToRelocate, planTmp)
                .seconds(),
            planTmp.getPerson(),
            null);
        // yy it is not clear to me how the dp time is interpreted for the backwards Dijkstra.  kai,
        // mar'19
      }
      // ---
    }
        ...
        case localRouting:
          {
            Node movedActNode;
            {
              Id<Link> movedActLinkId =
                  PopulationUtils.decideOnLinkIdForActivity(activityToRelocate, scenario);
            ...
            }
            {
...

              {
                Activity previousActivity = activities.get(actlegIndex - 1);
                Id<Link> linkId =
                    PopulationUtils.decideOnLinkIdForActivity(previousActivity, scenario);
              ...
              LeastCostPathCalculator.Path result =
                  this.forwardMultiNodeDijkstra.constructPath(
                      link.getToNode(), movedActNode, startTime);
            ...
            }
            {
              ...
              Leg nextLeg = PopulationUtils.getNextLeg(planTmp, activityToRelocate);
              {
                ...
                Id<Link> linkId =
                    PopulationUtils.decideOnLinkIdForActivity(
                        Objects.requireNonNull(nextAct), scenario);
                link = scenario.getNetwork().getLinks().get(linkId);
              }
              double startTime =
                  timeInterpretation
              ...
              LeastCostPathCalculator.Path result =
                  this.backwardMultiNodeDijkstra.constructPath(
                      link.getToNode(), movedActNode, startTime);
              NetworkRoute linkNetworkRouteImpl = getNetworkRoute(activityToRelocate, link, result);
    ...
    if (!list.isEmpty()) {
      ...
      // I don't think that this can happen.  But it was in the code before, and better safe than
      // sorry.  kai, mar'19
      return Collections.singletonList(
          new ScoredAlternative(largestValue, facilityIdWithLargestScore));
    }
  }

  private NetworkRoute getNetworkRoute(
      Activity activityToRelocate, Link link, LeastCostPathCalculator.Path result) {
    NetworkRoute linkNetworkRouteImpl =
        RouteUtils.createLinkNetworkRouteImpl(activityToRelocate.getLinkId(), link.getId());
    double distance = 0;
