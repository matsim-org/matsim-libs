	...
	private List<ScoredAlternative> createReducedChoiceSetWithPseudoScores(
		...
		if (this.approximationLevel == FrozenTastesConfigGroup.ApproximationLevel.localRouting )
		{
			...
			// (1) forward tree
			{
				...
				forwardMultiNodeDijkstra.calcLeastCostPath( nextActNode, destinationNode,
						previousActivity.getEndTime().seconds(), planTmp.getPerson(), null );
			}
			...
			// (2) backward tree
			{
				...
				backwardMultiNodeDijkstra.calcLeastCostPath( nextActNode, destinationNode,
						activityToRelocate.getEndTime().seconds(), planTmp.getPerson(), null );
