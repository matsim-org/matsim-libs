...
class DCActivityScoringFunction
    extends org.matsim.deprecated.scoring.functions.CharyparNagelActivityScoring {
  ...
  public DCActivityScoringFunction(Plan plan, DestinationChoiceContext dcContext) {
    ...
    this.dccg =
        (FrozenTastesConfigGroup)
            this.dcContext.getScenario().getConfig().getModule(FrozenTastesConfigGroup.GROUP_NAME);
  ...
  @Deprecated // needs to be re-designed with delegation instead of inheritance. kai, oct'14
  protected final double calcActScore(
    ...
    final OptionalTime latestStartTime =
        (OptionalTime)
            this.prefs.getAttribute(
                this.plan.getPerson().getId().toString(), "latestStartTime_" + act.getType());

    // double latestStartTime = attribute;
    if (latestStartTime.isDefined()
        && (latestStartTime.seconds() >= 0)
        && (activityStart > latestStartTime.seconds())) {
      tmpScore +=
          this.params.marginalUtilityOfLateArrival_s * (activityStart - latestStartTime.seconds());
    }
    ...
    OptionalTime typicalDuration =
        (OptionalTime)
            this.prefs.getAttribute(
                this.plan.getPerson().getId().toString(), "typicalDuration_" + act.getType());
    // initialize zero utility durations here for better code readability, because we only need them
    // here
    ...
    if (this.zeroUtilityDurations.containsKey(act.getType())) {
      ...
      zeroUtilityDuration =
          (typicalDuration.seconds() / 3600.0)
              * Math.exp(
                  -10.0
                      / (typicalDuration.seconds() / 3600.0)
                      / DCActivityScoringFunction.DEFAULT_PRIORITY);
    ...
    OptionalTime earliestEndTime =
        (OptionalTime)
            this.prefs.getAttribute(
                this.plan.getPerson().getId().toString(), "earliestEndTime_" + act.getType());
    if (earliestEndTime.isDefined()
        && (earliestEndTime.seconds() >= 0)
        && (activityEnd < earliestEndTime.seconds())) {
      tmpScore +=
          this.params.marginalUtilityOfEarlyDeparture_s * (earliestEndTime.seconds() - activityEnd);
    }
    ...
    OptionalTime minimalDuration =
        (OptionalTime)
            this.prefs.getAttribute(
                this.plan.getPerson().getId().toString(), "minimalDuration_" + act.getType());
    if (minimalDuration.isDefined()
        && (minimalDuration.seconds() >= 0)
        && (duration < minimalDuration.seconds())) {
      tmpScore +=
      ...
      if (this.dcContext.getScaleEpsilon().isFlexibleType(act.getType())
        ...
        this.penalty.add(
            new ScoringPenalty(
                activityStart,
                activityEnd,
                this.dcContext.getFacilityPenalties().get(act.getFacilityId()),
                tmpScore));
        // ---------------------------------------------------------------------------
      }
    } else {
  ...
  protected OptionalTime[] getOpeningInterval(Activity act) {
    ...
    if (act.getType().contains("interaction")
        || // yyyy might be too loose. kai, feb'16
    ...
    if (!foundAct) {
      throw new RuntimeException(
          "No suitable facility activity type found for activity "
              + act.getType()
              + " and facility "
              + facility.getId()
              + ". Aborting...");
