...
class ComputeMaxDCScorePlanAlgo implements PlanAlgorithm {
  ...
  public ComputeMaxDCScorePlanAlgo(
      final String type,
      final ActivityFacilityWithIndex[] typedFacilities,
      final DestinationScoring scorer,
      final DestinationSampler sampler,
      final DestinationChoiceContext lcContext) {
    this.type = type;
      ...
      if (pe instanceof Activity) {
        ...
        if (((Activity) pe).getType().equals(type)) {
          ...
          for (ActivityFacilityWithIndex f : this.typedFacilities) {
            ...
            //						double dcScore = scorer.getDestinationScore(act, epsilonScaleFactor,
            // activityIndex, p.getId());
            double dcScore =
                scorer.getDestinationScore(
                    this.dummyActivity, epsilonScaleFactor, activityIndex, p.getId());
  ...
  private static final class DummyActivity implements Activity {
    ...
    public DummyActivity(Id<Link> linkId) {
      this.linkId = linkId;
    }
