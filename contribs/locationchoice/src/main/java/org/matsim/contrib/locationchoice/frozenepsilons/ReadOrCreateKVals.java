...
class ReadOrCreateKVals {
    ...
    if (pkValuesFileName != null && fkValuesFileName != null && maxEpsValuesFileName != null) {
      ...
      try {
        ...
        for (Person p : this.scenario.getPopulation().getPersons().values()) {
          p.getAttributes()
              .putAttribute("k", personsKValues.getAttribute(p.getId().toString(), "k"));
        }
        for (ActivityFacility facility :
            this.scenario.getActivityFacilities().getFacilities().values()) {
          facility
              .getAttributes()
              .putAttribute("k", facilitiesKValues.getAttribute(facility.getId().toString(), "k"));
        }
  ...
  private boolean existingKValues() {
    ...
    for (ActivityFacility activityFacility :
