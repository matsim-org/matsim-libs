...
/**
 * This implementation of a backwards routing Dijkstra is based on an inverted network, i.e. a
 * network were the links' from and to nodes are switched.
 *
 * @author cdobler
 */
public class BackwardFastMultiNodeDijkstra extends FastMultiNodeDijkstra
    implements BackwardMultiNodePathCalculator {
  ...
  protected boolean addToPendingNodes(
    ...
    if (currTime < 0) {
      ...
      travelCost =
          this.costFunction.getLinkTravelDisutility(l, currTime, getPerson(), getVehicle());
    }
    //		double travelTime = this.timeFunction.getLinkTravelTime(l, currTime, person, vehicle);
    //		double travelCost = this.costFunction.getLinkTravelDisutility(l, currTime, this.person,
    // this.vehicle);
  ...
  //		else {
  //			if (!(fromNode instanceof RoutingNetworkNode)) fromNode =
  // this.routingNetwork.getNodes().get(fromNode.getId());
  //			if (!(toNode instanceof RoutingNetworkNode)) toNode =
  // this.routingNetwork.getNodes().get(toNode.getId());
  //
