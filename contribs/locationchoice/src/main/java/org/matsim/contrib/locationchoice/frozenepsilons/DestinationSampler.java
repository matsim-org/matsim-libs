 ...
 *
 * <p>At the moment it is only working for powers of 10 in the range [0..100] The sample must remain
 * stable over the course of iterations. Thus, person as well as facility k values are used.
 *
 * <p>(Conditional) probability (in %) of a facility k-value matching the given person k-value is
 * this.samplePercent example: this.samplePercent = 10; person i has a personValue k between 0 and 9
...
class DestinationSampler {
  ...
  public DestinationSampler(
      double[] personsKValues, double[] facilitiesKValues, FrozenTastesConfigGroup module) {
  ...
  public boolean sample(int facilityIndex, int personIndex) {
    ...
    int facilityValue =
        (int) Math.floor(100.0 / samplePercent * this.facilitiesKValues[facilityIndex]);

    // return true if the facility has the same number as the person. This will happen for
    // samplePercent of all facilities:
