...
/**
 * An extended implementation of the Dijkstra algorithm that supports multiple start- and/or end
 * nodes.
 *
 * <p>To do so, ImaginaryNodes and IntialNodes are introduced which is required for backwards
 * compatibility with the default Dijkstra implementation which expects only single nodes as
 * arguments.
 *
 * <p>Therefore, ImaginaryNodes are introduced. They contain a collection of InitialNodes that
 * represents the start or end nodes to be used by the algorithm. Each InitialNode represents a node
 * from the network. In addition, initial time and costs can be defined, i.e. the time respectively
 * costs to reach that node.
 *
 * <p>
 *
 * <p>By default, only the cheapest path between is calculated, i.e. the routing algorithm will
 * terminate before all end nodes have been reached. This behaviour can be changed by setting the
 * searchAllEndNodes parameter to true.
 *
...
public class MultiNodeDijkstra extends Dijkstra implements MultiNodePathCalculator {
  ...
  private static final Logger log = LogManager.getLogger(MultiNodeDijkstra.class);
  ...
  public static ImaginaryNode createImaginaryNode(
      Collection<? extends InitialNode> nodes, Coord coord) {
  ...
  protected void relaxImaginaryNode(
      final ImaginaryNode outNode,
      final RouterPriorityQueue<Node> pendingNodes,
    ...
    for (InitialNode initialNode : outNode.initialNodes) {
      ...
      visitNode(
          initialNode.node, data, pendingNodes, currTime + travelTime, currCost + travelCost, l);
    }
  }
   ...
   * After that, the paths between the multiple start and/or end nodes can be constructed using this
   * method.
   *
   * <p>Is there a way to check whether this method is called as intended?? cdobler, oct'13
   */
