...
/**
 * Performance optimized version of the MultiNodeDijkstra least cost path router which uses its own
 * network to route within.
 *
...
public class FastMultiNodeDijkstra extends MultiNodeDijkstra {
   ...
   */
  protected FastMultiNodeDijkstra(
    ...
    this.fastRouter =
        fastRouterFactory.createFastRouterDelegate(
            this, new DijkstraNodeDataFactory(), routingNetwork);
  ...
  public Path calcLeastCostPath(
    ...
    if (fromNode instanceof ImaginaryNode) {
      ...
      for (InitialNode initialNode : initialNodes)
        initialNode.node = routingNetwork.getNodes().get(initialNode.node.getId());
    ...
    return super.calcLeastCostPath(
        routingNetworkFromNode, routingNetworkToNode, startTime, person, vehicle);
  }
  ...
  protected Path constructPath(Node fromNode, Node toNode, double startTime, double arrivalTime) {
    ...
    if (imaginaryNode != null && path != null && path.nodes.size() > 0) {
      ...
      return new Path(
          path.nodes, path.links, path.travelTime - initialTime, path.travelCost - initialCost);
    }
