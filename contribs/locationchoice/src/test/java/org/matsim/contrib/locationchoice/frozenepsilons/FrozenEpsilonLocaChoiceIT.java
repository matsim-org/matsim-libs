...
public class FrozenEpsilonLocaChoiceIT {
  ...
  public void testLocationChoiceJan2013() {
    //	CONFIG:
    final Config config =
        localCreateConfig(this.utils.getPackageInputDirectory() + "../config2.xml");
    ...
    config.controller().setOverwriteFileSetting(OverwriteFileSetting.deleteDirectoryIfExists);
    {
      ...
      // yy I don't think that this is honoured anywhere in the way this is plugged together here.
      // kai, mar'19
    ...
    // makes all kinds of stuff available.  also reads or creates the k values both for the persons
    // and the facilities.
    ...
    controler.addOverridingModule(
        new AbstractModule() {
          @Override
          public void install() {
            addPlanStrategyBinding("MyLocationChoice")
                .to(BestReplyLocationChoicePlanStrategy.class);
          }
        });
    ...
    assertTrue(
        !newWork
            ...
            .equals(
                Id.create(
                    1, ActivityFacility.class))); // should be different from facility number 1 !!
    //		assertEquals( Id.create(63, ActivityFacility.class), newWork.getFacilityId() ); // as I have
    // changed the scoring (act is included) I also changed the test here: 27->92
    assertEquals(
        Id.create(64, ActivityFacility.class),
        newWork
            .getFacilityId()); // as I have changed the scoring (act is included) I also changed the
    // test here: 27->92
  }
  ...
  public void testLocationChoiceFeb2013NegativeScores() {
    ...
    final FrozenTastesConfigGroup dccg =
        ConfigUtils.addOrGetModule(config, FrozenTastesConfigGroup.class);
    ...
    DCScoringFunctionFactory scoringFunctionFactory =
        new DCScoringFunctionFactory(controler.getScenario(), lcContext);
    ...
    System.err.println(" newWork: " + newWork);
    System.err.println(" facilityId: " + newWork.getFacilityId());
    //		assertTrue( !newWork.getFacilityId().equals(Id.create(1) ) ) ; // should be different from
    // facility number 1 !!
  ...
  @Test
  public void testFacilitiesAlongALine() {
    ...
    //		dccg.setInternalPlanDataStructure(
    // DestinationChoiceConfigGroup.InternalPlanDataStructure.lcPlan );
    ...
    // 0 --(0-1)-- 1 --(2-1)-- 2 -- ...
    // with a facility of same ID attached to each link.  Have home towards the left, and then
    // select a shop facility, with frozen epsilons.
    ...
    for (int jj = 0; jj < 1000; jj++) {
      Person person = pf.createPerson(Id.createPersonId(jj));
      {
        ...
        }
        {
          ...
          // shop.setMaximumDuration( 3600. ); // does not work for locachoice: time computation is
          // not able to deal with it.  yyyy replace by
                      ...
                      public void notifyShutdown(ShutdownEvent event) {
                        ...
                        for (Person person : population.getPersons().values()) {
                          ...
                          for (Trip trip : trips) {
                            ...
                            double tripBeelineDistance =
                                CoordUtils.calcEuclideanDistance(
                                    facFrom.getCoord(), facTo.getCoord());
                        ...
                        // Note that the following "check" method is deliberately a bit imprecise
                        // (see implementation), since we are only interested in the
    ...
    // yyyy todo then make other test with other epsilon to show that average distance depends on
    // this

  }

  public static void addLinkAndFacility(
      Scenario scenario,
      NetworkFactory nf,
  ...
  private static void createExampleNetwork(
      final Scenario scenario, final double scale, final double speed) {
    Network network = scenario.getNetwork();

    final double x = -scale;
    Node node0 =
        network.getFactory().createNode(Id.create(0, Node.class), new Coord(x, (double) 0));
    network.addNode(node0);

    Node node1 =
        network
            .getFactory()
            .createNode(Id.create(1, Node.class), new Coord((double) 10, (double) 0));
    ...
    for (int ii = 2; ii < nNodes + 2; ii++) {
      ...
      ActivityFacility facility =
          scenario
              ...
              .getFactory()
              .createActivityFacility(Id.create(ii, ActivityFacility.class), coord);
    ...
    ActivityFacility facility1 =
        scenario
            ...
            .createActivityFacility(
                Id.create(1, ActivityFacility.class), new Coord(scale, (double) 0));
    scenario.getActivityFacilities().addActivityFacility(facility1);
    facility1.addActivityOption(new ActivityOptionImpl("work"));
    // (as soon as you set a scoring function that looks if activity types match opportunities at
    // facilities, you can only use
