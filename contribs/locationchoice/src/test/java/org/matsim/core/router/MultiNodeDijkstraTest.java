...
public class MultiNodeDijkstraTest {
  ...
  private static final Logger log = LogManager.getLogger(MultiNodeDijkstraTest.class);

  private MultiNodeDijkstra makeMultiNodeDikstra(
      Network network,
      TravelDisutility travelDisutility,
      TravelTime travelTime,
      boolean fastRouter) {
    if (fastRouter) {
      return (MultiNodeDijkstra)
  ...
  /**
   * Both nodes 1 and 4 are part of the start set. Even if the path from 1 to the target leads over
   * node 4, it may be faster, due to the intial cost values. Test that the route does not cut at
   * node 4 as the first node backwards from the start set.
   */
  ...
  /**
   * Tests that a path is found if some links are in the set of start as well as in the set of end
   * nodes and the path only containing of this node is the cheapest.
   */
  ...
  /**
   * Tests that a path is found if some links are in the set of start as well as in the set of end
   * nodes, but the path only containing of this node is the not the cheapest.
   */
  ...
  public void testSomeEndNodesNotReachable(boolean fastRouter) {
    ...
    toNodes.add(
        new InitialNode(
            f.network.getNodes().get(Id.create(3, Node.class)), 3.0, 3.0)); // cannot be reached!
  ...
  public void testSomeStartNodesNotUseable(boolean fastRouter) {
    ...
    fromNodes.add(
        new InitialNode(
            f.network.getNodes().get(Id.create(4, Node.class)), 3.0, 3.0)); // cannot lead to 5 or 6
    toNodes.add(new InitialNode(f.network.getNodes().get(Id.create(5, Node.class)), 1.0, 1.0));
  ...
  public void testInitialValuesCorrection(boolean fastRouter) {
    ...
    fromNodes.add(
        new InitialNode(f.network.getNodes().get(Id.create(1, Node.class)), 10000.0, 10000.0));
    toNodes.add(
        new InitialNode(f.network.getNodes().get(Id.create(6, Node.class)), 20000.0, 20000.0));
    ...
    public Fixture() {
      ...
      Node node1 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create(1, Node.class), new Coord((double) 1000, (double) 0));
      Node node2 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create(2, Node.class), new Coord((double) 500, (double) 0));
      Node node3 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create(3, Node.class), new Coord((double) 0, (double) 0));
      Node node4 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create(4, Node.class), new Coord((double) 1000, (double) 2000));
      Node node5 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create(5, Node.class), new Coord((double) 500, (double) 2000));
      Node node6 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create(6, Node.class), new Coord((double) 0, (double) 2000));
      Node node7 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create(7, Node.class), new Coord((double) 500, (double) 500));
      Node node8 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create(8, Node.class), new Coord((double) 500, (double) 1500));
  ...
  /*package*/ static class TestTimeCost implements TravelTime, TravelDisutility {
    ...
    public double getLinkTravelTime(
        final Link link, final double time, Person person, Vehicle vehicle) {
