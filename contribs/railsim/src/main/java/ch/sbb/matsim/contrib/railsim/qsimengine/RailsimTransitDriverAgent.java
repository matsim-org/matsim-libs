...
public final class RailsimTransitDriverAgent extends TransitDriverAgentImpl {
  ...
  public RailsimTransitDriverAgent(
      Map<Id<TransitStopArea>, List<TransitStopFacility>> stopAreas,
      Umlauf umlauf,
  ...
  public TransitStopFacility addDetour(List<RailLink> original, List<RailLink> detour) {
    ...
    if (nextStop != null) {
      ...
      if (adjust) {
        ...
        log.warn(
            "Could not re-route vehicle {} to a replacement transit stop", getVehicle().getId());
      }
    }
