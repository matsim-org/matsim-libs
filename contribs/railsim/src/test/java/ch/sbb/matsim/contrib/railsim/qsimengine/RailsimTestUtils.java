...
public class RailsimTestUtils {
  ...
  static {
    ...
    vehicles.put(
        TestVehicle.Sprinter, veh.getVehicleTypes().get(Id.create("Sprinter", VehicleType.class)));
    vehicles.put(
        TestVehicle.Express, veh.getVehicleTypes().get(Id.create("Express", VehicleType.class)));
    vehicles.put(
        TestVehicle.Regio, veh.getVehicleTypes().get(Id.create("Regio", VehicleType.class)));
    vehicles.put(
        TestVehicle.Cargo, veh.getVehicleTypes().get(Id.create("Cargo", VehicleType.class)));
  }

  /** Create a departure within the engine. Route will be determined automatically. */
  public static void createDeparture(
      Holder test, TestVehicle type, String veh, double time, String from, String to) {

    DijkstraFactory f = new DijkstraFactory();
    LeastCostPathCalculator lcp =
        f.createPathCalculator(
            test.network(),
    ...
    LeastCostPathCalculator.Path path =
        lcp.calcLeastCostPath(fromLink.getFromNode(), toLink.getToNode(), 0, null, null);
    NetworkRoute route =
        RouteUtils.createNetworkRoute(path.links.stream().map(Link::getId).toList());
    ...
    public void debugFiles(EventCollector collector, String out) {
      RailsimCsvWriter.writeTrainStatesCsv(
          collector.events.stream()
              .filter(ev -> ev instanceof RailsimTrainStateEvent)
      ...
      RailsimCsvWriter.writeLinkStatesCsv(
          collector.events.stream()
              .filter(ev -> ev instanceof RailsimLinkStateChangeEvent)
