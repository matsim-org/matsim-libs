...
public class RailsimIntegrationTest {
  ...
  public void testMesoUniDirectionalVaryingCapacities() {
    EventsCollector collector =
        runSimulation(
            new File(utils.getPackageInputDirectory(), "mesoUniDirectionalVaryingCapacities"));
    ...
    // train starts in the station, accelerates to station speed and continues until the train has
    // left the station link
    ...
    double cruiseDistance2 =
        distanceTravelled(stationSpeed, 0, cruiseTime2); // should be = trainLength - accDistance1
    currentTime += cruiseTime2;
    assertTrainState(
        currentTime, stationSpeed, stationSpeed, 0, accDistance1 + cruiseDistance2, train1events);
    ...
    assertTrainState(
        ...
        linkSpeed,
        0,
        accDistance1 + cruiseDistance2 + accDistance3,
        train1events);
    ...
    double cruiseDistance4 =
        linkLength - accDistance1 - cruiseDistance2 - accDistance3 - decDistance5;
    ...
    assertTrainState(
        currentTime, linkSpeed, linkSpeed, 0, trainLength + accDistance7, train1events);
  ...
  public void testScenarioMesoGenfBernOneTrain() {
    ...
    Consumer<Scenario> filter =
        scenario -> {
          Set<Id<Vehicle>> remove =
              scenario.getTransitVehicles().getVehicles().values().stream()
                  .filter(v -> !v.getId().toString().equals("Bummelzug_GE_BE_train_0"))
                  .map(Vehicle::getId)
  ...
  public void testMicroTrainFollowingConstantSpeed() {
    EventsCollector collector =
        runSimulation(
            new File(utils.getPackageInputDirectory(), "microTrainFollowingConstantSpeed"));
  }

  // This test is similar to testMicroTrainFollowingConstantSpeed but with varying speed levels
  // along the corridor.
  @Test
  public void testMicroTrainFollowingVaryingSpeed() {
    EventsCollector collector =
        runSimulation(
            new File(utils.getPackageInputDirectory(), "microTrainFollowingVaryingSpeed"));
  }
  ...
  public void testMicroJunctionCross() {
    EventsCollector collector =
        runSimulation(new File(utils.getPackageInputDirectory(), "microJunctionCross"));
  }
  ...
  public void testMicroSimpleUniDirectionalTrack() {
    EventsCollector collector =
        runSimulation(new File(utils.getPackageInputDirectory(), "microSimpleUniDirectionalTrack"));
    ...
    for (Event event : collector.getEvents()) {
      if (event.getEventType().equals(VehicleArrivesAtFacilityEvent.EVENT_TYPE)) {
        ...
        if (vehicleArrivesEvent.getVehicleId().toString().equals("train1")
          ...
          Assert.assertEquals(
              "The arrival time of train1 at stop_3-4 has changed.",
              29594.,
              event.getTime(),
  ...
  public void testMicroStationRerouting() {
    EventsCollector collector =
        runSimulation(new File(utils.getPackageInputDirectory(), "microStationRerouting"));
  ...
  private void assertTrainState(
    ...
    for (RailsimTrainStateEvent event : events) {
      ...
      if (event.getTime() > Math.ceil(time)) {
        Assert.fail(
            String.format(
                "No matching event found for time %f, speed %f pos %f, Closest event is%s",
                time, speed, headPosition, prev));
      }
  ...
  private List<RailsimTrainStateEvent> filterTrainEvents(EventsCollector collector, String train) {
    return collector.getEvents().stream()
        .filter(event -> event instanceof RailsimTrainStateEvent)
        .map(event -> (RailsimTrainStateEvent) event)
        .filter(event -> event.getVehicleId().toString().equals(train))
        .toList();
  }
}
