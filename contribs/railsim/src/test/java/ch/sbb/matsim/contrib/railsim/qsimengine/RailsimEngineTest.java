...
public class RailsimEngineTest {
  ...
  private RailsimTestUtils.Holder getTestEngine(String network, @Nullable Consumer<Link> f) {
    Network net =
        NetworkUtils.readNetwork(new File(utils.getPackageInputDirectory(), network).toString());
    ...
    return new RailsimTestUtils.Holder(
        new RailsimEngine(eventsManager, config, res, new SimpleDisposition(res, router)), net);
  }
  ...
  public void testCongestedWithHeadway() {
    ...
    RailsimTestUtils.Holder test =
        getTestEngine("networkMicroBi.xml", l -> RailsimUtils.setMinimumHeadwayTime(l, 60));
  ...
  public void testVaryingSpeedOne() {
    ...
    RailsimTestUtils.createDeparture(
        test, TestVehicle.Regio, "regio", 0, "t1_IN-t1_OUT", "t3_IN-t3_OUT");
  ...
  public void testVaryingSpeedMany() {
    ...
    for (int i = 0; i < 10; i++) {
      RailsimTestUtils.createDeparture(
          test, TestVehicle.Regio, "regio" + i, 60 * i, "t1_IN-t1_OUT", "t3_IN-t3_OUT");
    }
