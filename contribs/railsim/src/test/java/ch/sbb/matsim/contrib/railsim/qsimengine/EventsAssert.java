...
class EventsAssert
    extends AbstractCollectionAssert<
        EventsAssert, Collection<? extends Event>, Event, EventAssert> {
  ...
  public EventsAssert hasTrainState(String veh, double time, double headPosition, double speed) {
    return haveExactly(
        ...
        new Condition<>(
            ...
            String.format(
                "event with veh %s time %.0f headPosition: %.2f speed: %.2f",
                veh, time, headPosition, speed)));
  }
  ...
  protected EventsAssert newAbstractIterableAssert(Iterable<? extends Event> iterable) {
    return new EventsAssert(
        StreamSupport.stream(iterable.spliterator(), false).toList(), EventsAssert.class);
  }
}
