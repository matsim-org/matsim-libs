...
public class TaxiHistogramsWriter {
  ...
  private void writePassengerWaitTime(CompactCSVWriter writer) {
    ...
    for (TaxiStats s : taxiStats) {
      writeHistogramValues(
          writer, s.id, UniformHistogram.create(2.5 * 60, 25, s.passengerWaitTime.getValues()));
    }
  ...
  private void writeVehicleEmptyDriveRatio(CompactCSVWriter writer) {
    ...
    for (TaxiStats s : taxiStats) {
      writeHistogramValues(
          writer, s.id, UniformHistogram.create(0.05, 20, s.vehicleEmptyDriveRatio.getValues()));
    }
    writer.writeNextEmpty();
  }
  ...
  private void writeVehicleStayRatio(CompactCSVWriter writer) {
    ...
    for (TaxiStats s : taxiStats) {
      writeHistogramValues(
          writer, s.id, UniformHistogram.create(0.05, 20, s.vehicleStayRatio.getValues()));
    }
    writer.writeNextEmpty();
  }
  ...
  private void writeHourlyWaitRatioCounters(CompactCSVWriter writer) {
    ...
    for (TaxiStats s : taxiStats) {
      writeHistogramValues(
          writer, s.id, BoundedHistogram.create(bounds, s.vehicleStayRatio.getValues()));
    }
    writer.writeNextEmpty();
  }

  private void writeHistogramHeader(
      CompactCSVWriter writer, String header, Histogram<?> histogram) {
