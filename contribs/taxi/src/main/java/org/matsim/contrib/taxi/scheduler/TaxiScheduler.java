...
public class TaxiScheduler implements MobsimBeforeCleanupListener {
  ...
  // Pre-computing paths (occupied drive tasks) when they are not needed immediately
  // (destinationKnown is false):
  // In some configurations, the taxi optimiser may not know the destination until the passenger is
  // picked up.
  // But we can obtain this information already on request submission and use it to pre-compute the
  // origin-destination path in the background.
  // If sufficiently many threads are used, all such paths are found before the pickup simulation
  // ends.
  // Consequently, QSim is not (directly) slowed down by computing these paths.
  private final ConcurrentMap<Id<Request>, Future<VrpPathWithTravelData>> pathFutures =
  ...
  public TaxiScheduler(
      ...
      Fleet fleet,
      TaxiScheduleInquiry taxiScheduleInquiry,
    ...
    executorService =
        taxiCfg.destinationKnown
            ? null
            : new ExecutorServiceWithResource<>(
                IntStream.range(0, taxiCfg.numberOfThreads)
  ...
  private void initFleet() {
    for (DvrpVehicle veh : fleet.getVehicles().values()) {
      veh.getSchedule()
          .addTask(
              new TaxiStayTask(
                  veh.getServiceBeginTime(), veh.getServiceEndTime(), veh.getStartLink()));
    }
  }
  ...
  public void scheduleRequest(
      DvrpVehicle vehicle, DrtRequest request, VrpPathWithTravelData vrpPath) {
    Schedule schedule = vehicle.getSchedule();
    divertOrAppendDrive(schedule, vrpPath, TaxiEmptyDriveTask.TYPE);

    double pickupEndTime =
        Math.max(vrpPath.getArrivalTime(), request.getEarliestStartTime()) + taxiCfg.pickupDuration;
    ...
    if (taxiCfg.destinationKnown) {
      // TODO use an estimate to set up the occupied drive task and then start computing the actual
      ...
      // pre-compute path for occupied drive; the occupied drive and subsequent tasks will be added
      // after the pickup
      var pathFuture =
          executorService.submitCallable(
              router ->
                  VrpPaths.calcAndCreatePath(
                      reqFromLink, reqToLink, pickupEndTime, router, travelTime));
  ...
  protected void divertOrAppendDrive(
      Schedule schedule, VrpPathWithTravelData vrpPath, TaxiTaskType taskType) {
  ...
  protected void scheduleDrive(
      Schedule schedule,
      TaxiStayTask lastTask,
  ...
  /**
   * If diversion is enabled, this method must be called after scheduling in order to make sure that
   * no vehicle is moving aimlessly.
   *
   * <p>The reason: the destination/goal had been removed before scheduling (e.g. by calling the
  ...
  public void stopVehicle(DvrpVehicle vehicle) {
    ...
    tracker.divertPath(
        new VrpPathWithTravelDataImpl(
            stopPoint.time, 0, new Link[] {stopPoint.link}, new double[] {0}));
  ...
  public void updateBeforeNextTask(DvrpVehicle vehicle) {
    ...
    if (!taxiCfg.destinationKnown) {
      ...
      if (PICKUP.isBaseTypeOf(currentTask)) {
        ...
        var path =
            Futures.getUnchecked(pathFutures.remove(req.getId()))
  ...
  protected void appendOccupiedDriveAndDropoff(
      Schedule schedule, DrtRequest req, VrpPathWithTravelData path) {
