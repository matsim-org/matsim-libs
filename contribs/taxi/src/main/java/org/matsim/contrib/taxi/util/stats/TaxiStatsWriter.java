...
public class TaxiStatsWriter {
  ...
  private void writePassengerWaitTimeStats(CompactCSVWriter writer) {
    ...
    for (TaxiStats s : taxiStats) {
      CSVLineBuilder lineBuilder =
          new CSVLineBuilder().add(s.id).add(s.passengerWaitTime.getN() + "");
  ...
  private String[] getStatsSubheader(String header2) {
    return new String[] {
      "hour", header2, "avg", "sd", null, //
      "min", "2%ile", "5%ile", "25%ile", "50%ile", "75%ile", "95%ile", "98%ile", "max"
    };
  }

  private void addStats(
      CSVLineBuilder lineBuilder, String format1, String format2, DescriptiveStatistics stats) {
    lineBuilder
        .addf(format1, stats.getMean())
  ...
  private ImmutableList<Task.TaskType> gtAllCollectedTaskTypes() {
    ...
    return ImmutableList.<Task.TaskType>builder()
