...
class AssignmentChargerPlugData {
  ...
  static AssignmentDestinationData<ChargerPlug> create(
      double currentTime, Iterable<? extends Charger> chargers) {
    ...
    for (Charger c : chargers) {
      ...
      double assignedWorkload =
          ChargingEstimations.estimateTotalTimeToCharge(
              logic.getChargingStrategy(),
              Streams.concat(
                  logic.getPluggedVehicles().stream(),
                  logic.getQueuedVehicles().stream(),
