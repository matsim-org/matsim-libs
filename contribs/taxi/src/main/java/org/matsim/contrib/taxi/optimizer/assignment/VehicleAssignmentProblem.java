...
public class VehicleAssignmentProblem<D> {
  ...
  public VehicleAssignmentProblem(
      Network network, TravelTime travelTime, TravelDisutility travelDisutility) {
  ...
  public VehicleAssignmentProblem(
    ...
    pathSearch =
        OneToManyPathSearch.createSearch(
            new SpeedyGraph(network), nodeMap, travelTime, travelDisutility, false);

    // TODO this kNN is slow
    destinationFinder =
        nearestDestinationLimit < 0
            ? null
            : new StraightLineKnnFinder<>(
                nearestDestinationLimit,
                veh -> veh.link.getToNode().getCoord(),
                dest -> dest.link.getFromNode().getCoord());
    vehicleFinder =
        nearestVehicleLimit < 0
            ? null
            : new StraightLineKnnFinder<>(
                nearestVehicleLimit,
                dest -> dest.link.getFromNode().getCoord(),
  ...
  public List<Dispatch<D>> findAssignments(
      VehicleData vData, AssignmentDestinationData<D> dData, AssignmentCost<D> assignmentCost) {
    this.vData = vData;
    this.dData = dData;
  ...
  private void calcPathsForVehicles(PathData[][] pathDataMatrix) {
    for (int v = 0; v < vData.getSize(); v++) {
      ...
      List<DestEntry<D>> filteredDests =
          destinationFinder == null
              ? dData.getEntries()
      ...
      PathData[] paths =
          pathSearch.calcPathDataArray(departure.link, toLinks, departure.time, true);
  ...
  private void calcPathsForDestinations(PathData[][] pathDataMatrix) {
    for (int d = 0; d < dData.getSize(); d++) {
      ...
      List<VehicleData.Entry> filteredVehs =
          vehicleFinder == null
              ? vData.getEntries()
  ...
  private List<Dispatch<D>> createDispatches(
      int[] assignments, PathData[][] pathDataMatrix, TravelTime travelTime) {
      ...
      VrpPathWithTravelData vrpPath =
          pathData == null
              ? VrpPaths.calcAndCreatePath(
                  departure.link, dest.link, departure.time, router, travelTime)
              : VrpPaths.createPath(
                  departure.link, dest.link, departure.time, pathData, travelTime);
