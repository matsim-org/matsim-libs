...
public class DurationStats {
  ...
  public static SortedMap<Integer, Map<Task.TaskType, Double>> taskDurationByTimeBinAndType(
    ...
    return tasks
        .flatMap(task -> TimeBinSamples.taskSamples(task, binSize))
        .collect(
            groupingBy(
                TimeBinSample::timeBin,
                TreeMap::new,
                groupingBy(
                    sample -> sample.value().taskType,
  ...
  private static double taskDurationInTimeBin(TimeBinSample<ExecutedTask> taskSample, int binSize) {
    return durationInTimeBin(
        taskSample.value().beginTime, taskSample.value().endTime, taskSample.timeBin(), binSize);
  }

  private static <V> double stateDurationInTimeBin(
      TimeBinSample<State<V>> stateSample, int binSize) {
    return durationInTimeBin(
        stateSample.value().beginTime, stateSample.value().endTime, stateSample.timeBin(), binSize);
  }

  private static double durationInTimeBin(
      double beginTime, double endTime, int timeBin, int binSize) {
