...
//	private int sharedRides = 0;
	public SharedTaxiDispatchFinder(TaxibusOptimizerContext optimContext, double maximumDetourFactor) {
	 ...
	 // this one is basically a copy from the taxi package. I'm not entirely happy with this solution, but well.
	 {
	        ...
	        LinkTimePair bestDeparture = scheduler
	...
	public TaxibusDispatch findBestVehicleForRequest(TaxibusRequest req, Set<Vehicle> busyVehicles,  Set<Vehicle> idleVehicles){
		...
		for (Vehicle veh : busyVehicles){
			...
			Path secondPickupToFirstDest = router.calcLeastCostPath(req.getFromLink().getToNode(), firstRequest.getToLink().getFromNode(), currTime + firstToSecondPickup.travelTime, null,null);
			Path firstDestToSecondDest = router.calcLeastCostPath(firstRequest.getToLink().getToNode(), req.getToLink().getFromNode(), currTime+secondPickupToFirstDest.travelTime + firstToSecondPickup.travelTime, null,null);
			Path secondDestToFirstDest = router.calcLeastCostPath(req.getToLink().getToNode(), firstRequest.getToLink().getFromNode(), currTime+currentDirectPath.travelTime + firstToSecondPickup.travelTime, null,null);
			...
			if (way212>=way221){
				double ttFirstCustomer = firstToSecondPickup.travelTime +  secondPickupToFirstDest.travelTime + optimContext.tbcg.getPickupDuration();
				double ttSecondCustomer = way212 + optimContext.tbcg.getDropoffDuration();
				if ((ttFirstCustomer<=maximumDetourFactor*firstDirectPath.travelTime)&&(ttSecondCustomer<=maximumDetourFactor*(currentDirectPath.travelTime+bestNewVehDispatchTime))){
				...
				double ttFirstCustomer = firstToSecondPickup.travelTime +  way221 + optimContext.tbcg.getDropoffDuration();
				double ttSecondCustomer = currentDirectPath.travelTime;
				if ((ttFirstCustomer<=maximumDetourFactor*firstDirectPath.travelTime)&&(ttSecondCustomer<=maximumDetourFactor*currentDirectPath.travelTime)){
