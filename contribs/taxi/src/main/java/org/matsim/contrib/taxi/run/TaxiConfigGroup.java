...
public final class TaxiConfigGroup extends ReflectiveConfigGroupWithConfigurableParameterSets
    implements Modal {
  ...
  /**
   * @param config
   * @return single-mode taxi config (only if there is exactly 1 taxi config group in {@link
   *     MultiModeTaxiConfigGroup}. Otherwise will fail.
   */
  public static TaxiConfigGroup getSingleModeTaxiConfig(Config config) {
    Collection<TaxiConfigGroup> taxiConfigGroups =
        MultiModeTaxiConfigGroup.get(config).getModalElements();
    Preconditions.checkArgument(
        taxiConfigGroups.size() == 1,
        "Supported for only 1 taxi mode in the config. Number of taxi modes: %s",
        taxiConfigGroups.size());
  ...
  @Parameter
  @Comment(
      "An XML file specifying the taxi fleet."
  ...
  @NotNull private AbstractTaxiOptimizerParams taxiOptimizerParams;

  @Nullable private TaxiFareParams taxiFareParams;
  ...
  private void initSingletonParameterSets() {
    ...
    addOptimizerParamsDefinition(
        AssignmentTaxiOptimizerParams.SET_NAME, AssignmentTaxiOptimizerParams::new);

    addOptimizerParamsDefinition(FifoTaxiOptimizerParams.SET_NAME, FifoTaxiOptimizerParams::new);

    addOptimizerParamsDefinition(
        RuleBasedTaxiOptimizerParams.SET_NAME, RuleBasedTaxiOptimizerParams::new);

    addOptimizerParamsDefinition(ZonalTaxiOptimizerParams.SET_NAME, ZonalTaxiOptimizerParams::new);

    // taxi fare
    addDefinition(
        TaxiFareParams.SET_NAME,
        TaxiFareParams::new,
        () -> taxiFareParams,
        params -> taxiFareParams = (TaxiFareParams) params);
  }

  public void addOptimizerParamsDefinition(
      String name, Supplier<AbstractTaxiOptimizerParams> creator) {
    addDefinition(
        name,
        creator,
        () -> taxiOptimizerParams,
  ...
  protected void checkConsistency(Config config) {
    ...
    Verify.verify(
        !vehicleDiversion || onlineVehicleTracker,
        "vehicle diversion requires online vehicle tracker");
