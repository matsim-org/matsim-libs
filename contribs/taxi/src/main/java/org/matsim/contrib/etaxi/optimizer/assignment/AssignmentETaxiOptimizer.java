 ...
 *   <li>since the destination remains unknown till the end of pickup, all schedules end with STAY
 *       or PICKUP tasks
 * </ul>
 */
public class AssignmentETaxiOptimizer extends DefaultTaxiOptimizer {
  ...
  private final IdMap<DvrpVehicle, DvrpVehicle> scheduledForCharging =
      new IdMap<>(DvrpVehicle.class);

  public AssignmentETaxiOptimizer(
    ...
    super(
        ...
        new AssignmentRequestInserter(
            ...
            travelDisutility,
            eScheduler,
            ((AssignmentETaxiOptimizerParams) taxiCfg.getTaxiOptimizerParams())
                .getAssignmentTaxiOptimizerParams(),
            router));
  ...
  protected void scheduleCharging() {
    AssignmentDestinationData<ChargerPlug> pData =
        AssignmentChargerPlugData.create(
            timer.getTimeOfDay(), chargingInfrastructure.getChargers().values());
    if (pData.getSize() == 0) {
      return;
    }
    ...
    List<Dispatch<ChargerPlug>> assignments =
        eAssignmentProblem.findAssignments(vData, pData, cost);
  ...
  private VehicleData initVehicleDataForCharging(AssignmentDestinationData<ChargerPlug> pData) {
    ...
    Stream<DvrpVehicle> vehiclesBelowMinSocLevel =
        fleet.getVehicles().values().stream()
            .filter(
                v ->
                    isChargingSchedulable(
                        (EvDvrpVehicle) v, eScheduler.getScheduleInquiry(), maxDepartureTime));

    // filter least charged vehicles
    // assumption: all b.capacities are equal
    List<DvrpVehicle> leastChargedVehicles =
        PartialSort.kSmallestElements(
            pData.getSize(),
            vehiclesBelowMinSocLevel,
            Comparator.comparingDouble(
                v -> ((EvDvrpVehicle) v).getElectricVehicle().getBattery().getCharge()));

    return new VehicleData(
        timer.getTimeOfDay(), eScheduler.getScheduleInquiry(), leastChargedVehicles.stream());
  }

  // TODO MIN_SOC should depend on %idle
  private boolean isChargingSchedulable(
      EvDvrpVehicle eTaxi, TaxiScheduleInquiry scheduleInquiry, double maxDepartureTime) {
    ...
    return departure != null
        && departure.time <= maxDepartureTime; // schedulable within the time horizon
  }
}
