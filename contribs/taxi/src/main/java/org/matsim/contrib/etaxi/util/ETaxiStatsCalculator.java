...
public class ETaxiStatsCalculator {
  ...
  public ETaxiStatsCalculator(
      List<ChargingSequence> chargingSequences, FleetSpecification fleetSpecification) {
    for (ChargingSequence sequence : chargingSequences) {
      ...
      stateDurationByTimeBinAndState(getStateSampleStream(sequence, 3600), 3600)
          .forEach(
              (hour, stateDurations) -> updateStateDurations(getHourlyStats(hour), stateDurations));

      Map<ETaxiState, Double> dailyStateDuration =
          DurationStats.stateDurationByTimeBinAndState(
  ...
  public ImmutableList<ETaxiStats> getETaxiStats() {
    return ImmutableList.<ETaxiStats>builder()
        .addAll(hourlyEStats.values())
        .add(dailyEStats)
        .build();
  }
  ...
  private Stream<TimeBinSample<State<ETaxiState>>> getStateSampleStream(
      ChargingSequence seq, int binSize) {
    var pluggedState =
        new State<>(
            ETaxiState.PLUGGED, seq.getChargingStart().getTime(), seq.getChargingEnd().getTime());
  ...
  private static void updateStateDurations(
      ETaxiStats stats, Map<ETaxiState, Double> stateDurations) {
    stateDurations.forEach(
        (state, duration) -> stats.stateDurations.merge(state, duration, Double::sum));
  }
}
