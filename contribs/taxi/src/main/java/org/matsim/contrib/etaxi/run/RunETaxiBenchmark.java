 ...
 *
 * <p>To model the impact of traffic, we can use a time-variant network, where we specify different
...
public class RunETaxiBenchmark {
  ...
  public static Controler createControler(Config config, int runs) {
    ...
    controler.addOverridingModule(
        new AbstractDvrpModeModule(mode) {
          @Override
          public void install() {
            bindModal(TaxiBenchmarkStats.class)
                .toProvider(
                    modalProvider(
                        getter ->
                            new TaxiBenchmarkStats(
    ...
    controler.addOverridingModule(
        new AbstractDvrpModeModule(mode) {
          @Override
          public void install() {
            bindModal(ETaxiBenchmarkStats.class)
                .toProvider(
                    modalProvider(
                        getter ->
                            new ETaxiBenchmarkStats(
                                getter.get(OutputDirectoryHierarchy.class),
