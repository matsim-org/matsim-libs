...
public class BestDispatchFinder {
  ...
  public BestDispatchFinder(
    ...
    multiSourceALT =
        new SpeedyMultiSourceALTFactory()
            .createPathCalculator(network, travelDisutility, travelTime);
  }

  // for immediate requests only
  // minimize TW
  public Dispatch<DrtRequest> findBestVehicleForRequest(
      DrtRequest req, Stream<? extends DvrpVehicle> vehicles) {
  ...
  public <D> Dispatch<D> findBestVehicle(
      D destination, Stream<? extends DvrpVehicle> vehicles, Function<D, Link> destinationToLink) {
    ...
    Map<Id<Node>, DvrpVehicle> nodeToVehicle =
        Maps.newHashMapWithExpectedSize(EXPECTED_NEIGHBOURHOOD_SIZE);
    Map<Id<Node>, StartNode> vehicleNodes =
        Maps.newHashMapWithExpectedSize(EXPECTED_NEIGHBOURHOOD_SIZE);
    vehicles.forEach(
        veh -> {
          ...
          if (departure != null) {
            ...
            if (departure.link == destinationLink) {
              // hack: we are basically there (on the same link), so let's pretend vehNode ==
              // destinationNode
              ...
              delay +=
                  1 + destinationLink.getLength() / destinationLink.getFreespeed(departure.time);
            }
    ...
    Path path =
        multiSourceALT.calcLeastCostPath(vehicleNodes.values(), destinationNode, null, null, false);
    ...
    LinkTimePair bestDeparture =
        scheduleInquiry.getImmediateDiversionOrEarliestIdleness(bestVehicle);

    VrpPathWithTravelData vrpPath =
        VrpPaths.createPath(
            bestDeparture.link, destinationLink, bestDeparture.time, path, travelTime);
    return new Dispatch<>(bestVehicle, destination, vrpPath);
  }

  // for immediate requests only
  // minimize TP
  public Dispatch<DrtRequest> findBestRequestForVehicle(
      DvrpVehicle veh, Stream<DrtRequest> unplannedRequests) {
  ...
  public <D> Dispatch<D> findBestDestination(
      DvrpVehicle veh, Stream<D> destinations, Function<D, Link> destinationToLink) {
    LinkTimePair departure = scheduleInquiry.getImmediateDiversionOrEarliestIdleness(veh);
    Node vehicleNode = departure.link.getToNode();

    Map<Id<Node>, D> nodeToDestination =
        Maps.newHashMapWithExpectedSize(EXPECTED_NEIGHBOURHOOD_SIZE);
    Map<Id<Node>, StartNode> destinationNodes =
        Maps.newHashMapWithExpectedSize(EXPECTED_NEIGHBOURHOOD_SIZE);
    for (D loc : (Iterable<D>) destinations::iterator) {
      ...
      if (departure.link == link) {
        return new Dispatch<>(
            veh, loc, VrpPaths.createZeroLengthPath(departure.link, departure.time, false));
      }
      ...
      if (!destinationNodes.containsKey(locNodeId)) {
        ...
        StartNode newDestinationNode =
            new StartNode(link.getFromNode(), delayAtLastLink, expectedArrivalTime);
    ...
    Path path =
        multiSourceALT.calcLeastCostPath(destinationNodes.values(), vehicleNode, null, null, true);
    ...
    VrpPathWithTravelData vrpPath =
        VrpPaths.createPath(
            departure.link,
            destinationToLink.apply(bestDestination),
            departure.time,
            path,
            travelTime);
