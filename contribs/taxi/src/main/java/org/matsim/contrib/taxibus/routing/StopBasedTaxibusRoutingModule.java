 ...
 *
 */
public class StopBasedTaxibusRoutingModule implements RoutingModule {

	private final StageActivityTypes drtStageActivityType = new TaxibusStageActivityType();
	private final RoutingModule walkRouter;
	private final Map<Id<TransitStopFacility>, TransitStopFacility> stops;
	private final TaxibusConfigGroup taxibusconfig;
	 ...
	 */
	@Inject
	public StopBasedTaxibusRoutingModule(@Named(TransportMode.walk) RoutingModule walkRouter,
			@Named("taxibus") TransitSchedule transitSchedule, Scenario scenario) {
		transitSchedule.getFacilities();
		this.walkRouter = walkRouter;
		this.stops = transitSchedule.getFacilities();
		this.taxibusconfig = (TaxibusConfigGroup)scenario.getConfig().getModules().get(TaxibusConfigGroup.GROUP_NAME);
	...
	public List<? extends PlanElement> calcRoute(Facility<?> fromFacility, Facility<?> toFacility, double departureTime,
		...
		Activity drtInt1 = scenario.getPopulation().getFactory()
				.createActivityFromCoord(TaxibusStageActivityType.TAXIBUSTAGEACTIVITY, accessFacility.getCoord());
		...
		Leg drtLeg = PopulationUtils.createLeg("taxibus");
		...
		Activity drtInt2 = scenario.getPopulation().getFactory()
				.createActivityFromCoord(TaxibusStageActivityType.TAXIBUSTAGEACTIVITY, egressFacility.getCoord());
	...
	private Set<TransitStopFacility> findStopCoordinates(Coord coord) {
		...
		for (TransitStopFacility stop : this.stops.values()) {
			...
			if (distance <= taxibusconfig.getMaxWalkDistance()) {
