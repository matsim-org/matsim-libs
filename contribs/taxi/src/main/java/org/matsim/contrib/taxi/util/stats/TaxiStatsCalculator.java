...
public class TaxiStatsCalculator {
  ...
  public TaxiStatsCalculator(
      Collection<ExecutedSchedule> executedSchedules,
    ...
    for (ExecutedSchedule schedule : executedSchedules) {
      DurationStats.taskDurationByTimeBinAndType(schedule.getExecutedTasks().stream(), 3600)
          .forEach(
              (hour, taskTypeDurations) ->
                  updateTaxiStats(getHourlyStats(hour), taskTypeDurations));

      Map<TaskType, Double> dailyTaskTypeDuration =
          DurationStats.taskDurationByTimeBinAndType(
                  schedule.getExecutedTasks().stream(), Integer.MAX_VALUE)
              .entrySet()
              .iterator()
              .next()
              .getValue();
  ...
  static OptionalDouble calculateEmptyDriveRatio(Map<TaskType, Double> taskTypeDurations) {
    ...
    return (empty != 0 || occupied != 0)
        ? OptionalDouble.of(empty / (empty + occupied))
  ...
  private static double sumBaseTaskTypeDurations(
      Map<TaskType, Double> taskTypeDurations, TaxiTaskBaseType baseType) {
  ...
  private static void updateTaskDurations(
      TaxiStats stats, Map<TaskType, Double> taskTypeDurations) {
