...
public class ETaxiOptimizerProvider implements Provider<TaxiOptimizer> {
    ...
    if (type.equals(RuleBasedETaxiOptimizerParams.SET_NAME)) {
      ZonalRegisters zonalRegisters =
          createZonalRegisters(
              ((RuleBasedETaxiOptimizerParams) taxiCfg.getTaxiOptimizerParams())
                  .getRuleBasedTaxiOptimizerParams());
      BestDispatchFinder dispatchFinder =
          new BestDispatchFinder(
              eScheduler.getScheduleInquiry(), network, timer, travelTime, travelDisutility);
      RuleBasedRequestInserter requestInserter =
      ...
      return new RuleBasedETaxiOptimizer(
          eventsManager,
          taxiCfg,
          fleet,
          eScheduler,
      ...
      return new AssignmentETaxiOptimizer(
          eventsManager,
          taxiCfg,
          fleet,
          timer,
  ...
  private ZonalRegisters createZonalRegisters(RuleBasedTaxiOptimizerParams params) {
    ...
    IdleTaxiZonalRegistry idleTaxiRegistry =
        new IdleTaxiZonalRegistry(zonalSystem, eScheduler.getScheduleInquiry());
