...
public class TrafficVolumeGeneration {
  ...
  /**
   * Creates the traffic volume (start) for each zone separated in the modesORvehTypes and the
   * purposes.
   *
  ...
  static HashMap<TrafficVolumeKey, Object2DoubleMap<Integer>> createTrafficVolume_start(
    ...
    calculateTrafficVolumePerZone(
        trafficVolume_start, resultingDataPerZone, "start", sample, modesORvehTypes);
    String sampleName = SmallScaleCommercialTrafficUtils.getSampleNameOfOutputFolder(sample);
    Path outputFileStart =
        output
            .resolve("calculatedData")
            .resolve(
                "TrafficVolume_" + trafficType + "_" + "startPerZone_" + sampleName + "pt.csv");
  ...
  /**
   * Creates the traffic volume (stop) for each zone separated in the modesORvehTypes and the
   * purposes.
   *
   * @param resultingDataPerZone employee date for each zone
   * @param output output path
  ...
  static HashMap<TrafficVolumeKey, Object2DoubleMap<Integer>> createTrafficVolume_stop(
    ...
    calculateTrafficVolumePerZone(
        trafficVolume_stop, resultingDataPerZone, "stop", sample, modesORvehTypes);
    String sampleName = SmallScaleCommercialTrafficUtils.getSampleNameOfOutputFolder(sample);
    Path outputFileStop =
        output
            .resolve("calculatedData")
  ...
  private static void calculateTrafficVolumePerZone(
    ...
    } else throw new RuntimeException("No generation and commitment rates selected. Please check!");
        ...
        for (Integer purpose : generationRates.keySet()) {
          ...
          if (resultingDataPerZone.get(zoneId).isEmpty())
            ...
            for (String category : resultingDataPerZone.get(zoneId).keySet()) {
              ...
              if (modeORvehType.equals("total")) commitmentFactor = 1;
              else
                commitmentFactor =
                    commitmentRates
   ...
   *
   * @param smallScaleCommercialTrafficType used smallScaleCommercialTrafficType (freight or
   *     business traffic)
   */
   ...
   * @param smallScaleCommercialTrafficType used trafficType (commercialPersonTraffic or
   *     goodsTraffic)
   ...
   */
  static void reduceDemandBasedOnExistingCarriers(
      Scenario scenario,
    ...
    for (Carrier carrier : CarriersUtils.addOrGetCarriers(scenario).getCarriers().values()) {
      if (!carrier.getAttributes().getAsMap().containsKey("subpopulation")
          || !carrier
              .getAttributes()
              .getAttribute("subpopulation")
              .equals(smallScaleCommercialTrafficType)) continue;
      ...
      if (carrier.getSelectedPlan() != null) {
        for (ScheduledTour tour : carrier.getSelectedPlan().getScheduledTours()) {
          ...
          for (TourElement tourElement : tour.getTour().getTourElements()) {
            if (tourElement instanceof ServiceActivity service) {
              String stopZone =
                  SmallScaleCommercialTrafficUtils.findZoneOfLink(
                      service.getLocation(), regionLinksMap);
              try {
                reduceVolumeForThisExistingJobElement(
            ...
            if (tourElement instanceof Pickup pickup) {
              startZone =
                  SmallScaleCommercialTrafficUtils.findZoneOfLink(
                      pickup.getShipment().getFrom(), regionLinksMap);
              String stopZone =
                  SmallScaleCommercialTrafficUtils.findZoneOfLink(
                      pickup.getShipment().getTo(), regionLinksMap);
              try {
                reduceVolumeForThisExistingJobElement(
                    trafficVolumePerTypeAndZone_start,
                    trafficVolumePerTypeAndZone_stop,
        ...
        if (carrier.getServices().size() != 0) {
          ...
          for (CarrierVehicle vehicle :
            ...
            possibleStartAreas.add(
                SmallScaleCommercialTrafficUtils.findZoneOfLink(
                    vehicle.getLinkId(), regionLinksMap));
          }
          for (CarrierService service : carrier.getServices().values()) {
            String startZone =
                (String)
                    possibleStartAreas
                        .toArray()[MatsimRandom.getRandom().nextInt(possibleStartAreas.size())];
            String stopZone =
                SmallScaleCommercialTrafficUtils.findZoneOfLink(
                    service.getLocationLinkId(), regionLinksMap);
            try {
              reduceVolumeForThisExistingJobElement(
                  trafficVolumePerTypeAndZone_start,
                  trafficVolumePerTypeAndZone_stop,
          ...
          for (CarrierShipment shipment : carrier.getShipments().values()) {
            String startZone =
                SmallScaleCommercialTrafficUtils.findZoneOfLink(shipment.getFrom(), regionLinksMap);
            String stopZone =
                SmallScaleCommercialTrafficUtils.findZoneOfLink(shipment.getTo(), regionLinksMap);
            try {
              reduceVolumeForThisExistingJobElement(
                  trafficVolumePerTypeAndZone_start,
                  trafficVolumePerTypeAndZone_stop,
  ...
  private static void reduceVolumeForThisExistingJobElement(
    ...
    if (startZone != null && stopZone != null) {
      ...
      if (trafficVolumePerTypeAndZone_start.get(trafficVolumeKey_start).getDouble(purpose) == 0)
        reduceVolumeForOtherArea(
            trafficVolumePerTypeAndZone_start,
            modeORvehType,
            purpose,
            "Start",
            trafficVolumeKey_start.getZone());
      else
        trafficVolumePerTypeAndZone_start
            .get(trafficVolumeKey_start)
            .mergeDouble(purpose, -1, Double::sum);
      if (trafficVolumePerTypeAndZone_stop.get(trafficVolumeKey_stop).getDouble(purpose) == 0)
        reduceVolumeForOtherArea(
            ...
            purpose,
            "Stop",
            trafficVolumeKey_stop.getZone());
      else
        trafficVolumePerTypeAndZone_stop
            .get(trafficVolumeKey_stop)
            .mergeDouble(purpose, -1, Double::sum);
    } else {
  ...
  private static void reduceVolumeForOtherArea(
      HashMap<TrafficVolumeKey, Object2DoubleMap<Integer>> trafficVolumePerTypeAndZone,
      String modeORvehType,
      Integer purpose,
      String volumeType,
      String originalZone) {
    ArrayList<TrafficVolumeKey> shuffledKeys =
        new ArrayList<>(trafficVolumePerTypeAndZone.keySet());
      ...
      if (trafficVolumeKey.getModeORvehType().equals(modeORvehType)
        ...
        log.warn(
            volumeType
                ...
                + "', purpose '"
                + purpose
                + "') was reduced because the volume for the zone "
                + originalZone
   ...
   */
  private static HashMap<Integer, HashMap<String, Double>> setGenerationRates(
      String smallScaleCommercialTrafficType, String generationType) {
  ...
  /**
   * Sets the commitment rates based on the IVV 2005 for the goodsTraffic. The commitment rate for
   * the commercialPersonTraffic is 1, because mode choice will be done in MATSim.
   *
   * @param smallScaleCommercialTrafficType used trafficType (freight or business traffic)
   * @param commitmentType start or stop parameter
   */
  private static HashMap<String, HashMap<String, Double>> setCommitmentRates(
      String smallScaleCommercialTrafficType, String commitmentType) {
