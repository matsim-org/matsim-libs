...
public class TripDistributionMatrix {
  ...
  private static class TripDistributionMatrixKey {
    ...
    public TripDistributionMatrixKey(
        String fromZone,
        String toZone,
    ...
    public int hashCode() {
      ...
      result =
          prime * result
              + ((smallScaleCommercialTrafficType == null)
                  ? 0
                  : smallScaleCommercialTrafficType.hashCode());
      return result;
    }
  ...
  private final ConcurrentHashMap<TripDistributionMatrixKey, Integer> matrixCache =
      new ConcurrentHashMap<>();
  private final ConcurrentHashMap<ResistanceFunktionKey, Double> resistanceFunktionCache =
      new ConcurrentHashMap<>();
  private final ConcurrentHashMap<GravityConstantKey, Double> gravityConstantACache =
      new ConcurrentHashMap<>();
  //	private final ConcurrentHashMap<String, Double> gravityConstantBCache = new
  // ConcurrentHashMap<String, Double>();
  private final ConcurrentHashMap<String, Object2DoubleMap<String>> roundingError =
      new ConcurrentHashMap<>();
  private NetworkBasedTransportCosts netBasedCosts = null;

  /**
   * Calculates the traffic volume between two zones for a specific modeORvehType and purpose.
   *
  ...
  void setTripDistributionValue(
    ...
    double volumeStart =
        trafficVolume_start
            .get(TrafficVolumeGeneration.makeTrafficVolumeKey(startZone, modeORvehType))
            .getDouble(purpose);
    double volumeStop =
        trafficVolume_stop
            .get(TrafficVolumeGeneration.makeTrafficVolumeKey(stopZone, modeORvehType))
            .getDouble(purpose);
    int roundedVolume;
    if (volumeStart != 0 && volumeStop != 0) {

      double resistanceValue =
          getResistanceFunktionValue(
              startZone, stopZone, network, regionLinksMap, resistanceFactor);
      double gravityConstantA =
          getGravityConstant(
              stopZone,
              trafficVolume_start,
              modeORvehType,
      ...
      roundingError
          .get(stopZone)
          .merge((modeORvehType + "_" + purpose), certainRoundingError, Double::sum);
      ...
      if (!listOfZones.contains(startZone)) listOfZones.add(startZone);
      if (!listOfZones.contains(stopZone)) listOfZones.add(stopZone);
    } else roundedVolume = 0;
    TripDistributionMatrixKey matrixKey =
        makeKey(startZone, stopZone, modeORvehType, purpose, smallScaleCommercialTrafficType);
   ...
   * @param smallScaleCommercialTrafficType goodsTraffic or commercialPersonTraffic
   */
  Integer getTripDistributionValue(
      String startZone,
      String stopZone,
      String modeORvehType,
      Integer purpose,
   ...
   */
  private Double getResistanceFunktionValue(
      String startZone,
      String stopZone,
      Network network,
      Map<String, HashMap<Id<Link>, Link>> regionLinksMap,
    ...
    if (netBasedCosts == null) {
      VehicleType vehicleType =
          VehicleUtils.createVehicleType(Id.create("vwCaddy", VehicleType.class));
      vehicleType
          .getCostInformation()
          .setCostsPerMeter(0.00017)
          .setCostsPerSecond(0.00948)
          .setFixedCost(22.73);
      NetworkBasedTransportCosts.Builder netBuilder =
          NetworkBasedTransportCosts.Builder.newInstance(network, List.of(vehicleType));
    ...
    if (!resistanceFunktionCache.containsKey(makeResistanceFunktionKey(startZone, stopZone)))
      for (SimpleFeature startZoneFeature : zonesFeatures) {
        ...
        for (SimpleFeature stopZoneFeature : zonesFeatures) {
            ...
            if (useNetworkRoutesForResistanceFunction) {
              Location startLocation =
                  Location.newInstance(
                      regionLinksMap.get(startZone).keySet().iterator().next().toString());
              Location stopLocation =
                  Location.newInstance(
                      regionLinksMap.get(stopZone).keySet().iterator().next().toString());
              Vehicle exampleVehicle = getExampleVehicle(startLocation);
              //							distance = netBasedCosts.getDistance(startLocation, stopLocation, 21600.,
              // exampleVehicle);
              travelCosts =
                  netBasedCosts.getTransportCost(
                      startLocation, stopLocation, 21600., null, exampleVehicle);
            } else {
              Point geometryStartZone =
                  ((Geometry) startZoneFeature.getDefaultGeometry()).getCentroid();
              Point geometryStopZone =
                  ((Geometry) stopZoneFeature.getDefaultGeometry()).getCentroid();
          ...
          else resistanceFunktionResult = Math.exp(-distance);
          resistanceFunktionCache.put(
              makeResistanceFunktionKey(zone1, zone2), resistanceFunktionResult);
          resistanceFunktionCache.put(
              makeResistanceFunktionKey(zone2, zone1), resistanceFunktionResult);
        }
      }
        ...
        for (Integer purpose : getListOfPurposes()) {
          double trafficVolume =
              trafficVolume_stop
                  .get(TrafficVolumeGeneration.makeTrafficVolumeKey(stopZone, modeORvehType))
                  .getDouble(purpose);
          int generatedTrafficVolume =
              getSumOfServicesForStopZone(
          ...
          if (trafficVolume > generatedTrafficVolume) {
            if (generatedTrafficVolume == 0) {
              TripDistributionMatrixKey matrixKey =
                  makeKey(
                      stopZone, stopZone, modeORvehType, purpose, smallScaleCommercialTrafficType);
              matrixCache.replace(matrixKey, matrixCache.get(matrixKey) + 1);
              generatedTrafficVolume =
                  getSumOfServicesForStopZone(
                      stopZone, modeORvehType, purpose, smallScaleCommercialTrafficType);
            } else {
  ...
  private VehicleImpl getExampleVehicle(Location fromId) {
    return VehicleImpl.Builder.newInstance("vwCaddy")
        .setType(
   ...
   */
  private double getGravityConstant(
      String baseZone,
   ...
   */
  private TripDistributionMatrixKey makeKey(
      String fromZone,
      String toZone,
      String modeORvehType,
      int purpose,
      String smallScaleCommercialTrafficType) {
    return new TripDistributionMatrixKey(
        fromZone, toZone, modeORvehType, purpose, smallScaleCommercialTrafficType);
  }
  ...
  ArrayList<Integer> getListOfPurposes() {
    if (listOfPurposes.isEmpty()) {
      for (TripDistributionMatrixKey key : matrixCache.keySet()) {
        if (!listOfPurposes.contains(key.getPurpose())) listOfPurposes.add(key.getPurpose());
      }
    }
    return listOfPurposes;
  }
   ...
   */
  int getSumOfServicesForStartZone(
      String startZone, String modeORvehType, int purpose, String smallScaleCommercialTrafficType) {
    int numberOfTrips = 0;
   ...
   * @return numberOfTrips
   */
  int getSumOfServicesForStopZone(
      String stopZone, String modeORvehType, int purpose, String smallScaleCommercialTrafficType) {
    int numberOfTrips = 0;
    ArrayList<String> zones = getListOfZones();
  ...
  void writeODMatrices(Path output, String smallScaleCommercialTrafficType)
    ...
    for (String modeORvehType : usedModesORvehTypes) {
      for (int purpose : usedPurposes) {
        ...
        BufferedWriter writer =
            IOUtils.getBufferedWriter(outputFolder.toUri().toURL(), StandardCharsets.UTF_8, true);
        try {
          ...
          for (String startZone : usedZones) {
            ...
            for (String stopZone : usedZones) {
              if (getTripDistributionValue(
                      startZone, stopZone, modeORvehType, purpose, smallScaleCommercialTrafficType)
                  == null)
                throw new RuntimeException(
                    "OD pair is missing; start: "
                        + startZone
                        + "; stop: "
                        + stopZone
                        + "; modeORvehType: "
