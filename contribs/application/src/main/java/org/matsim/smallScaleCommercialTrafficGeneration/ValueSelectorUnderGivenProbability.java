...
public class ValueSelectorUnderGivenProbability {
  ...
  public ValueSelectorUnderGivenProbability(
      List<ProbabilityForValue> ProbabilityDistribution, Random rnd) {
    this.anIntAsSum = 0;
  ...
  public ProbabilityForValue getNextValueUnderGivenProbability() {
    ...
    while (selectedValue == null) {
      ProbabilityForValue probabilityForValue =
          ProbabilityDistribution.get(rnd.nextInt(ProbabilityDistribution.size()));
  ...
  private void weightedProbability() {
    ...
    // After 'testCount' loops, print out the number of times each value was selected and the
    ...
    //				+ "(" + String.format("%.2f", (probabilityForValue.getExpectedCount() *
    // Math.pow(anIntAsSum,
    //				-1)) * 100) + " %); prob: " + ((double)Math.round(probabilityForValue.getProbability() *
    // 1000)/10) + "%");
    //		}
  }
  ...
  public void writeResults() {
    for (ProbabilityForValue probabilityForValue : ProbabilityDistribution) {
      System.out.println(
          probabilityForValue.getValue()
              + " -> expected: "
              + probabilityForValue.getExpectedCount()
              + "("
              + String.format(
                  "%.2f", (probabilityForValue.getExpectedCount() * Math.pow(anIntAsSum, -1)) * 100)
              + " %); prob: "
              + ((double) Math.round(probabilityForValue.getProbability() * 1000) / 10)
              + "%");
    }
  }
