...
public final class ExtractHomeCoordinates implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--output",
      description = "Write population with home coordinates as attributes")
  private Path output;

  @CommandLine.Mixin private CsvOptions options = new CsvOptions();
  ...
  public Integer call() throws Exception {
    ...
    for (Person person : population.getPersons().values()) {
      ...
      if (coord != null) coords.put(person, coord);
    }
    ...
    if (csv != null) {
      ...
      try (CSVPrinter printer = options.createPrinter(csv)) {
        ...
        for (Map.Entry<Person, Coord> e : coords.entrySet()) {
          printer.printRecord(
              e.getKey().getId().toString(), e.getValue().getX(), e.getValue().getY());
        }
      }
    }
