			...
			for (Table t : compTables) {
				for (String mode : new HashSet<>(t.columnNames())) {
					if (!mode.equals("Info")) {
						...
						// based on new naming scheme from TripAnalysis.class
						t.column(mode).setName("Group");
					}
				}
				i++;
			}

			if (simTable.columnNames().contains("Info")) {
				simTable.column("Info").setName("Group");
			}
...
//			compTripFiles.add(new File(new File(file).getAbsoluteFile() + "/analysis/population/trip_stats.csv"));
		...
		if (compTripFiles.isEmpty() || !simTripsFile.exists()) {
			...
			JoinAndWriteCsv(simDataTable, compDataTables, compNames, "trip_stats_comparison.csv", "Group",true);
		}
		...
		if (compPopTripFiles.isEmpty() || !simPopTripsFile.exists()) {
			...
			JoinAndWriteCsv(simDataTable, compDataTables, compNames,"population_trip_stats_comparison.csv", "Group",true);
		}
