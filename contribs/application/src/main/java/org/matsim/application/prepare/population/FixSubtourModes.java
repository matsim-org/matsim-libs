...
@CommandLine.Command(
    name = "fix-subtour-modes",
    description =
        "Fix modes for subtours that contain chain and non-chain based modes, by choosing one of the found modes randomly.",
...
public class FixSubtourModes implements MATSimAppCommand, PersonAlgorithm {
  ...
  @CommandLine.Option(
      names = "--all-plans",
      description = "Whether to fix all plans, or only selected",
      defaultValue = "false")
  private boolean allPlans;

  @CommandLine.Option(
      names = "--mass-conservation",
      description = "Whether to check for the mass conservation of the whole plan",
      defaultValue = "true",
      negatable = true)
  ...
  public Integer call() throws Exception {
    ...
    log.info(
        "Processed {} out of {} persons. Modified {} subtours",
        processed,
  ...
  public void run(Person person) {
    ...
    for (Plan plan : plans) {
      ...
      try {
        ...
        log.warn(
            "Exception occurred when handling person {}: {}. Whole plan will be set to walk.",
            person.getId(),
            e);
  ...
  private void selectMode(Person p, TripStructureUtils.Subtour st) {
    ...
    List<String> modes =
        st.getTrips().stream()
            .map(t -> TripStructureUtils.identifyMainMode(t.getTripElements()))
            .collect(Collectors.toList());

    if (!PersonUtils.canUseCar(p) && modes.contains("car"))
      throw new IllegalStateException(
          "Person " + p.getId() + " uses car as mode, which should not be allowed.");
