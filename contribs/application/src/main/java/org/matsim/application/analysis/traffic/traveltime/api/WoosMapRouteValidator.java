...
public class WoosMapRouteValidator extends AbstractRouteValidator {
  ...
  public Result retrieve(Coord from, Coord to, int hour) {
    ...
    ClassicHttpRequest req =
        ClassicRequestBuilder.get(URL)
            .addParameter("key", apiKey)
            .addParameter("origin", String.format(Locale.US, "%.6f,%.6f", from.getY(), from.getX()))
            .addParameter(
                "destination", String.format(Locale.US, "%.6f,%.6f", to.getY(), to.getX()))
            .addParameter(
                "departure_time",
    ...
    try {
      ...
      return new Result(
          hour,
          route.get("duration").get("value").asInt(),
          route.get("distance").get("value").asInt());
    } catch (IOException e) {
