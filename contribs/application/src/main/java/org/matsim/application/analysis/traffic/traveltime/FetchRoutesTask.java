...
final class FetchRoutesTask implements Runnable {
  ...
  FetchRoutesTask(
      RouteAPI api,
      String apiKey,
      List<SampleValidationRoutes.Route> routes,
      List<Integer> hours,
      Path out) {
    this.api = api;
    this.apiKey = apiKey;
  ...
  private void fetch() throws Exception {
    ...
    } else open = StandardOpenOption.CREATE_NEW;

    try (RouteValidator val =
        switch (api) {
        ...
        for (SampleValidationRoutes.Route route : routes) {
          for (int h : hours) {
            ...
            try {
              ...
              csv.printRecord(
                  route.fromNode(), route.toNode(), val.name(), h, res.dist(), res.travelTime());
              ...
              csv.flush();
              log.error(
                  "{}: stopping because API indicated key is not valid or quota exhausted", api);
              return;
  ...
  private RouteValidator.Result fetchWithBackoff(
      RouteValidator val, SampleValidationRoutes.Route route, int h, int i)
    ...
    try {
      ...
      if (i < 3) {
        ...
        log.warn(
            "Failed to fetch result for {} {}: {} (retrying after {}s)",
            api,
            route,
            e.getMessage(),
            backoff / 1000);
