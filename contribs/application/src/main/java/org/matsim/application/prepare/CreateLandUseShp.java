...
@CommandLine.Command(name = "create-landuse-shp", description = "Create a shapefile of landuse")
public class CreateLandUseShp implements MATSimAppCommand {
  ...
  private static final Set<String> INCLUDE =
      Set.of(
          "commercial",
          "construction",
          "industrial",
  ...
  @CommandLine.Parameters(
      paramLabel = "INPUT",
      arity = "1..*",
      description = "Path to input shapes")
  private List<Path> inputs;

  @CommandLine.Option(
      names = "--layer",
      arity = "0..*",
      description = "Name of the shape file if input is a zip")
  private List<String> layer =
      List.of("gis_osm_landuse_a_free_1.shp", "gis_osm_buildings_a_free_1.shp");

  @CommandLine.Option(
      names = "--attr",
      description = "Name of attribute to match",
      defaultValue = "fclass")
  private String attr;

  @CommandLine.Option(
      names = "--output",
      description = "Output path for shape file",
      required = true)
  private Path output;
  ...
  public Integer call() throws Exception {
    ...
    DataStore out =
        factory.createNewDataStore(Collections.singletonMap("url", output.toUri().toURL()));
    ...
    for (Path input : inputs) {
      ...
      if (input.toString().endsWith("zip")) {
        ...
        paths.add(input);
      } else throw new IllegalArgumentException("Unknown file format: " + input);
      ...
      for (Path path : paths) {
            ...
            if (INCLUDE.contains(type)) {
              ...
              if (built) {
                ...
                for (MultiPolygon p : result) {
                  if (p.contains(geom)) continue outer;
                }

              } else index.insert(geom.getEnvelopeInternal(), geom);
        ...
        if (schema == null) {
          ...
          if (crs.getTargetCRS() != null) {
            ...
            schema =
                SimpleFeatureTypeBuilder.retype(ds.getSchema(), CRS.decode(crs.getTargetCRS()));
          }
        ...
        if (crs.getTargetCRS() == null) {
          ...
          ReprojectingFeatureCollection rfc =
              new ReprojectingFeatureCollection(list, schema.getCoordinateReferenceSystem());
