...
@CommandLine.Command(
    name = "air-pollution",
    description = "General air pollution analysis.",
...
@CommandSpec(
    ...
    produces = {
      "emissions_total.csv",
      "emissions_grid_per_day.xyt.csv",
      "emissions_per_link.csv",
...
public class AirPollutionAnalysis implements MATSimAppCommand {
  ...
  @CommandLine.Mixin private SampleOptions sample;

  @CommandLine.Option(
      names = "--grid-size",
      description = "Grid size in meter",
      defaultValue = "100")
  ...
  public Integer call() throws Exception {
    ...
    if (shp.isDefined()) {
      ...
      NetworkFilterManager manager =
          new NetworkFilterManager(scenario.getNetwork(), config.network());
      manager.addLinkFilter(l -> index.contains(l.getCoord()));

      filteredNetwork = manager.applyFilters();
    } else filteredNetwork = scenario.getNetwork();
  ...
  private Config prepareConfig() {
    Config config =
        ConfigUtils.loadConfig(
            ApplicationUtils.matchInput("config.xml", input.getRunDirectory())
                .toAbsolutePath()
                .toString());
    ...
    config
        .vehicles()
        .setVehiclesFile(
            ApplicationUtils.matchInput("vehicles", input.getRunDirectory())
                .toAbsolutePath()
                .toString());
    config
        .network()
        .setInputFile(
            ApplicationUtils.matchInput("network", input.getRunDirectory())
                .toAbsolutePath()
                .toString());
    config
        .transit()
        .setTransitScheduleFile(
            ApplicationUtils.matchInput("transitSchedule", input.getRunDirectory())
                .toAbsolutePath()
                .toString());
    config
        .transit()
        .setVehiclesFile(
            ApplicationUtils.matchInput("transitVehicles", input.getRunDirectory())
                .toAbsolutePath()
                .toString());
  ...
  private void writeOutput(Network network, EmissionsOnLinkEventHandler emissionsEventHandler)
      throws IOException {
    ...
    CSVPrinter absolute =
        new CSVPrinter(
            Files.newBufferedWriter(output.getPath("emissions_per_link.csv")), CSVFormat.DEFAULT);
    CSVPrinter perMeter =
        new CSVPrinter(
            Files.newBufferedWriter(output.getPath("emissions_per_link_per_m.csv")),
            CSVFormat.DEFAULT);
    ...
    Map<Id<Link>, Map<Pollutant, Double>> link2pollutants =
        emissionsEventHandler.getLink2pollutants();

    for (Id<Link> linkId : link2pollutants.keySet()) {

      // Link might be filtered
      if (!network.getLinks().containsKey(linkId)) continue;
  ...
  private void writeTotal(Network network, EmissionsOnLinkEventHandler emissionsEventHandler) {
    ...
    DecimalFormat scientific =
        new DecimalFormat("0.###E0", DecimalFormatSymbols.getInstance(Locale.ENGLISH));

    for (Map.Entry<Id<Link>, Map<Pollutant, Double>> e :
        emissionsEventHandler.getLink2pollutants().entrySet()) {

      if (!network.getLinks().containsKey(e.getKey())) continue;
      for (Map.Entry<Pollutant, Double> p : e.getValue().entrySet()) {
        sum.mergeDouble(p.getKey(), p.getValue(), Double::sum);
      }
    }

    try (CSVPrinter total =
        new CSVPrinter(
            Files.newBufferedWriter(output.getPath("emissions_total.csv")), CSVFormat.DEFAULT)) {

      total.printRecord("Pollutant", "kg");
      for (Pollutant p : Pollutant.values()) {
        double val = (sum.getDouble(p) / sample.getSample()) / 1000;
        total.printRecord(
            p, val < 100_000 && val > 100 ? simple.format(val) : scientific.format(val));
      }
  ...
  /**
   * Creates the data for the XY-Time plot. The time is fixed and the data is summarized over the
   * run. Currently only the CO2_Total Values is printed because Simwrapper can handle only one
   * value.
   */
  private void writeRaster(
      Network network, Config config, EmissionsOnLinkEventHandler emissionsEventHandler) {

    Map<Pollutant, Raster> rasterMap =
        FastEmissionGridAnalyzer.processHandlerEmissions(
            emissionsEventHandler.getLink2pollutants(), network, gridSize, 20);
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            Files.newBufferedWriter(output.getPath("emissions_grid_per_day.xyt.csv")),
            CSVFormat.DEFAULT.builder().setCommentMarker('#').build())) {

      String crs = ProjectionUtils.getCRS(network);
      if (crs == null) crs = config.network().getInputCRS();
      if (crs == null) crs = config.global().getCoordinateSystem();
  ...
  private void writeTimeDependentRaster(
      Network network, Config config, EmissionsOnLinkEventHandler emissionsEventHandler) {

    TimeBinMap<Map<Pollutant, Raster>> timeBinMap =
        FastEmissionGridAnalyzer.processHandlerEmissionsPerTimeBin(
            emissionsEventHandler.getTimeBins(), network, gridSize, 20);
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(output.getPath("emissions_grid_per_hour.xyt.csv").toString()),
            CSVFormat.DEFAULT.builder().setCommentMarker('#').build())) {
