...
@CommandSpec(
    requireEvents = true,
    produces = {
      "stuck_agents_per_hour.csv",
      "stuck_agents_per_mode.csv",
      "stuck_agents_per_link.csv",
      "stuck_agents.csv"
    })
public class StuckAgentAnalysis
    implements MATSimAppCommand, PersonStuckEventHandler, ActivityStartEventHandler {
  ...
  public Integer call() throws Exception {
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(output.getPath("stuck_agents.csv").toString()),
            CSVFormat.DEFAULT)) {
      ...
      printer.printRecord(
          "Proportion of stuck agents",
          new DecimalFormat("#.0#", DecimalFormatSymbols.getInstance(Locale.US))
                  .format(
                      ((Math.round(
                              (100.0 / allAgents.size() * allStuckLinks.keySet().size()) * 100))
                          / 100.0))
              + '%',
          "chart-pie");
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(output.getPath("stuck_agents_per_hour.csv").toString()),
            CSVFormat.DEFAULT)) {
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(output.getPath("stuck_agents_per_link.csv").toString()),
            CSVFormat.DEFAULT)) {

      // Sort Map
      List<String> sorted = new ArrayList<>(allStuckLinks.keySet());
      sorted.sort(
          (o1, o2) -> -Double.compare(allStuckLinks.getDouble(o1), allStuckLinks.getDouble(o2)));
      ...
      for (int i = 0; i < 20; i++) {
        ...
        for (int j = 2; j < header.size(); j++) {
          result[j] =
              String.valueOf(stuckAgentsPerLink.get(header.get(j)).getDouble(sorted.get(i)));
        }
        printer.printRecord((Object[]) result);
      }
    } catch (IOException ex) {
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(output.getPath("stuck_agents_per_mode.csv").toString()),
            CSVFormat.DEFAULT)) {
  ...
  private double summarizeStuckAgentsPerLink(String stuckedLink) {
    double sum = 0.;
    for (Map.Entry<String, Object2DoubleOpenHashMap<String>> entry :
        stuckAgentsPerLink.entrySet()) {
  ...
  public void handleEvent(PersonStuckEvent event) {
    // Pie chart
    if (!stuckAgentsPerMode.containsKey(event.getLegMode()))
      stuckAgentsPerMode.put(event.getLegMode(), 1);
    else
      stuckAgentsPerMode.put(event.getLegMode(), stuckAgentsPerMode.getInt(event.getLegMode()) + 1);

    // Stuck Agents per Hour
    Int2DoubleMap perHour =
        stuckAgentsPerHour.computeIfAbsent(event.getLegMode(), (k) -> new Int2DoubleOpenHashMap());
    int hour = (int) event.getTime() / 3600;
    if (hour > maxHour) maxHour = hour;
    perHour.mergeDouble(hour, 1, Double::sum);

    // Stuck Agents per Link
    Object2DoubleMap<String> perLink =
        stuckAgentsPerLink.computeIfAbsent(
            event.getLegMode(), (k) -> new Object2DoubleOpenHashMap<>());

    Id<Link> link =
        Objects.requireNonNullElseGet(event.getLinkId(), () -> Id.createLinkId("unknown"));
