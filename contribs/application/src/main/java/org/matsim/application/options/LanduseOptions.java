...
public class LanduseOptions {
  ...
  @CommandLine.Option(
      names = "--landuse",
      description = "Optional path to shape-file to distribute coordinates according to landuse",
      required = false)
  private Path landuse;

  @CommandLine.Option(
      names = "--landuse-iters",
      description = "Maximum number of points to generate trying to fit into landuse",
      defaultValue = "500")
  private int iters;

  @CommandLine.Option(
      names = "--landuse-filter",
      arity = "0..*",
      description = "Filter landuse features by certain category",
      required = false)
  private Set<String> filter;

  @CommandLine.Option(
      names = "--landuse-filter-attr",
      description = "Attribute name for the filter",
      defaultValue = "fclass")
  private String attr;
  ...
  public synchronized ShpOptions.Index getIndex(String queryCRS) {
    ...
    if (index != null || landuse == null) return index;
  ...
  /**
   * Tries to select a point that lies within one of the geometries of the index. Will try at least
   * {@link #iters} times, after which the last point is returned even if not within a geometry.
