...
public class ApplicationUtils {
  ...
  /**
   * Extends a context (usually config location) with an relative filename. If the results is a
   * local file, the path will be returned. Otherwise, it will be an url. The results can be used as
   * input for command line parameter or {@link IOUtils#resolveFileOrResource(String)}.
   *
  ...
  public static String globFile(Path path, String runId, String name) {
    ...
    String file =
        globWithParent(path, runId + ".*" + name + ".*")
            .orElseThrow(() -> new IllegalStateException("No " + name + "file found."))
            .toString();
  ...
  public static Scenario loadScenario(String runId, Path runDirectory, CrsOptions crs) {
    ...
    Path populationFile =
        globWithParent(runDirectory, runId + ".*plans.*")
            .orElseThrow(() -> new IllegalStateException("No plans file found."));
    ...
    Path networkFile =
        globWithParent(runDirectory, runId + ".*network.*")
            .orElseThrow(() -> new IllegalStateException("No network file found."));
    log.info("Using network {}", networkFile);

    String facilitiesFile =
        globWithParent(runDirectory, runId + ".*facilities.*").map(Path::toString).orElse(null);
  ...
  public static void checkCommand(Class<? extends MATSimAppCommand> command) {
    ...
    if (command.getAnnotation(CommandSpec.class) == null) {
      throw new IllegalArgumentException(
          String.format("The command %s has no @CommandSpec annotation.", command));
    }
    ...
    for (Field field : fields) {
      if (field.getType().equals(InputOptions.class)) input = true;

      if (field.getType().equals(OutputOptions.class)) output = true;
    }

    if (!input) {
      throw new IllegalArgumentException(
          String.format("The command %s has no field with InputOptions.", command));
    }

    if (!output) {
      throw new IllegalArgumentException(
          String.format("The command %s has no field with OutputOptions.", command));
    }
  }

  /** Whether this command accepts a specific class as options. */
  public static boolean acceptsOptions(
      Class<? extends MATSimAppCommand> command, Class<?> options) {
  ...
  /**
   * Tries to match input file {@code name} with files from the input directory {@code dir}. Please
   * check docs in the code for the conventions. Different naming conventions are tried in a
   * specific order.
   *
  ...
  public static Path matchInput(String name, Path dir) {
    ...
    if (Files.exists(possibility)) return possibility;
