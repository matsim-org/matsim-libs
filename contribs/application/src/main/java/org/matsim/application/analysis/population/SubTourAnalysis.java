...
@CommandLine.Command(name = "subtours", description = "Analyze subtours of a population.")
public class SubTourAnalysis implements MATSimAppCommand {
  ...
  @CommandLine.Parameters(
      ...
      description = "Path to population")
  private Path input;
  ...
  @CommandLine.Option(
      names = "--behaviour",
      description = "Subtour mode-choice behaviour",
      defaultValue = "betweenAllAndFewerConstraints")
  private SubtourModeChoice.Behavior behavior;

  @CommandLine.Option(
      names = "--st-proba",
      description = "Probability for single trip mode-choice",
      defaultValue = "0.5")
  private double singleTrip;

  @CommandLine.Option(
      names = "--coord-dist",
      description = "Use coordinate distance for subtours if greater 0",
      defaultValue = "0")
  private double coordDist;

  @CommandLine.Option(
      names = "--iter",
      description = "Iterate strategy to output choice sets",
      defaultValue = "1")
  private int iter;
  ...
  public Integer call() throws Exception {
    ...
    for (Person agent : population.getPersons().values()) {
      ...
      if (person != null && !agent.getId().toString().equals(person)) continue;
    ...
    for (TripStructureUtils.Subtour st : subtours) {
      if (st.isClosed()) closed++;

      if (strategy.isMassConserving(st)) massConserving++;
    }
    ...
    log.info(
        "Subtours: {} | closed: {}% | massConserving: {}%",
        subtours.size(), 100d * closed / subtours.size(), 100d * massConserving / subtours.size());
    ...
    for (int i = 0; i < iter; i++) {
      ...
      log.info(
          "Total choice sets: {}, persons with no choices: {}, (avg: {}, q90: {}), person with uncovered trips in choice-set: {}",
          c.size(),
          empty,
          avg.orElse(0),
          Quantiles.percentiles().index(90).compute(nChoices),
          tripMissing);
    }
