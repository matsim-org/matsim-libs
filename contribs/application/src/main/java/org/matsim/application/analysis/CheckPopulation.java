...
public class CheckPopulation implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--attribute",
      arity = "0..*",
      description = "Print full distribution for selected attributes")
  ...
  public Integer call() throws Exception {
    ...
    for (Person agent : population.getPersons().values()) {
      ...
      queryAttr.forEach(
          attr ->
              instances.get(attr).mergeInt(map.getOrDefault(attr, "UNDEFINED"), 1, Integer::sum));
    }
    ...
    if (!queryAttr.isEmpty()) {
      ...
      for (Map.Entry<String, Object2IntMap<Object>> e : instances.entrySet()) {
        ...
        for (Object2IntMap.Entry<Object> oe : e.getValue().object2IntEntrySet()) {
          ...
          log.info(
              "\t\t{}: {} ({}%)", oe.getKey(), oe.getIntValue(), oe.getIntValue() * 100d / total);
        }
      }
    }
    ...
    List<? extends Person> agents =
        population.getPersons().values().stream().filter(filter).collect(Collectors.toList());

    // agents with trips
    List<? extends Person> mobileAgents =
        agents.stream()
    ...
    List<TripStructureUtils.Trip> trips =
        agents.stream()
            .flatMap(
                agent ->
                    TripStructureUtils.getTrips(agent.getSelectedPlan().getPlanElements()).stream())
            .collect(Collectors.toList());
    ...
    counts.forEach(
        (k, v) ->
            log.info(
                "\t{}-m: {} ({}%)", k.intValue(), v, Math.round((v * 1000d) / trips.size()) / 10d));
    ...
    haveActivity.forEach(
        (k, v) -> log.info("\t{}: {} ({}%)", k, v, Math.round((v * 1000d) / agents.size()) / 10d));
    ...
    if (subtours.size() > 0)
      log.info(
          "Closed subtours estimate: \t{}%", Math.round((closed * 1000d) / subtours.size()) / 10d);
    else log.info("No info about subtours (link or facilities ids missing)");
