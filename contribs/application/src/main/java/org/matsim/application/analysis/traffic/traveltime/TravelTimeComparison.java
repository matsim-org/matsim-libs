...
public class TravelTimeComparison implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--input-ref",
      description = "File with reference data",
      required = true)
  private String apiFile;

  @CommandLine.Option(
      names = "--modes",
      description = "Network modes to analyze",
  ...
  public Integer call() throws Exception {
    ...
    LeastCostPathCalculator congestedRouter =
        new SpeedyALTFactory().createPathCalculator(network, util, tt);
    LeastCostPathCalculator freeflowRouter =
        new SpeedyALTFactory()
            .createPathCalculator(network, new OnlyTimeDependentTravelDisutility(fs), fs);
    ...
    List<String> columns =
        List.of("min", "max", "mean", "std", "simulated", "free_flow", "bias", "abs_error");

    Table aggr = data.summarize(columns, mean).by("hour");

    for (Column<?> column : aggr.columns()) {
      String name = column.name();
      if (name.startsWith("Mean")) column.setName(name.substring(6, name.length() - 1));
    }
  ...
  private LeastCostPathCalculator.Path computePath(
      Network network, LeastCostPathCalculator router, Row row) {
