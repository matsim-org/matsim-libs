...
public class ResolveGridCoordinates implements MATSimAppCommand {
  ...
  /**
   * It is important to <b>use a car-network</b> (i.e. a network where car is permitted on each
   * link), as otherwise one probably runs into problems. In scenarios that use access/egress
   * routing, things might still work, but older scenarios will most likely break (during qsim).
   */
  @CommandLine.Parameters(paramLabel = "INPUT", arity = "1", description = "Path to population")
  private Path input;

  @CommandLine.Option(
      names = "--grid-resolution",
      description = "Original grid resolution in meter")
  private double gridResolution;

  @CommandLine.Option(
      names = "--network",
      description = "Match to closest link using given network",
      required = false)
      ...
      for (Plan plan : p.getPlans()) {
        ...
        for (PlanElement el : plan.getPlanElements()) {
          if (el instanceof Activity) {
            ...
            if (geom != null && !geom.contains(MGC.coord2Point(coord))) continue;
                  ...
                  CoordUtils.round(
                      landuse.select(
                          ...
                          () -> {
                            ...
                            else return new Coord(coord.getX() + x, coord.getY() + y);
                          }));
            ...
            if (network != null) {
              ...
              if (!link.getAllowedModes().contains(TransportMode.car)) {
                throw new IllegalArgumentException(
                    "About to set linkId for activity"
                        + act
                        + "for person "
                        + p
                        + "to "
