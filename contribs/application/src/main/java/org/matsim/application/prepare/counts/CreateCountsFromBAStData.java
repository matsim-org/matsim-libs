...
/**
 * Prepare command to generate Counts from the BASt traffic count data. If you want to use manual
 * matched counts, you have to follow a name convention. Unfortunately one count station counts
 * vehicles in both directions For example: The station id is 001. The count direction 1 (Column
 * 'HiRi1') is E (east) and the count direction 2 (Column 'HiRi2') is W (west) If you want to match
 * the count values of the east-lane to matsim link 'my_link_1' add a entry in the .csv like this:
 * row1 : 001_1; my_link_1
 *
 * <p>If you want to ignore stations just paste the station id into the .csv. Both count directions
 * of the station will be ignored.
 *
 * @author hzoerner
 */
@CommandLine.Command(
    name = "counts-from-bast",
    description = "Creates MATSim from BASt Stundenwerte.txt")
public class CreateCountsFromBAStData implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--road-types",
      description = "Define on which roads counts are created")
  private final List<String> roadTypes = List.of("motorway", "primary", "trunk");

  @CommandLine.Option(
      names = "--primary-data",
      description = "path to BASt Bundesstraßen-'Stundenwerte'-.txt file",
      required = true)
  private Path primaryData;

  @CommandLine.Option(
      names = "--motorway-data",
      description = "path to BASt Bundesautobahnen-'Stundenwerte'-.txt file",
      required = true)
  private Path motorwayData;

  @CommandLine.Option(
      names = "--station-data",
      description = "path to default BASt count station .csv",
      required = true)
  private Path stationData;

  @CommandLine.Option(
      names = "--search-range",
      description = "range for the buffer around count stations, in which links are queried",
      defaultValue = "50")
  ...
  @CommandLine.Option(
      names = "--car-output",
      description = "output car counts path",
      defaultValue = "car-counts-from-bast.xml.gz")
  private Path carOutput;

  @CommandLine.Option(
      names = "--freight-output",
      description = "output freight counts path",
      defaultValue = "freight-counts-from-bast.xml.gz")
  private Path freightOutput;

  @CommandLine.Mixin private final ShpOptions shp = new ShpOptions();

  @CommandLine.Mixin private final CountsOption counts = new CountsOption();

  @CommandLine.Mixin private final CrsOptions crs = new CrsOptions("EPSG:25832");
  ...
  private void mapTrafficVolumeToCount(
      BAStCountStation station, Counts<Link> miv, Counts<Link> freight) {

    if (!station.hasMatchedLink()) return;
    ...
    Count<Link> mivCount =
        miv.createAndAddCount(
            station.getMatchedLink().getId(), station.getName() + "_" + station.getDirection());
    Count<Link> freightCount =
        freight.createAndAddCount(
            station.getMatchedLink().getId(), station.getName() + "_" + station.getDirection());
  ...
  private void readHourlyTrafficVolume(
      Path pathToDisaggregatedData, Map<String, BAStCountStation> stations) {
    ...
    if (input.getFileName().toString().endsWith(".zip")) {
      try {
        ...
        try (Stream<Path> stream = Files.walk(fs.getPath("/"))) {
          ...
          if (opt.isPresent()) input = opt.get();
    ...
    try {
      ...
      Set<String> stationIds =
          preFilteredRecords.stream().map(row -> row.get("Zst")).collect(Collectors.toSet());
      ...
      for (String number : stationIds) {
        ...
        // Use station name from 'direction1' because names of 'direction1' and 'direction2' are the
        // same.
        log.info("Process data for count station {}", direction1.getName());

        List<CSVRecord> allEntriesOfStation =
            preFilteredRecords.stream().filter(row -> row.get("Zst").equals(number)).toList();

        for (String hour : hours) {
          var hourlyTrafficVolumes =
              allEntriesOfStation.stream()
                  .filter(row -> row.get("Stunde").replace("\"", "").equals(hour))
                  .toList();
  ...
  private void match(
      Network network,
      NetworkIndex<BAStCountStation> index,
      BAStCountStation station,
      CountsOption counts) {
    ...
    if (manuallyMatched != null) {
      ...
      if (!network.getLinks().containsKey(manuallyMatched))
        throw new RuntimeException(
            "Manual matched station link " + manuallyMatched + " is not in the network!");
  ...
  private void matchBAStWithNetwork(
      String pathToNetwork,
      Map<String, BAStCountStation> stations,
      CountsOption countsOption,
      CrsOptions crs) {

    if (crs.getTargetCRS() != null)
      throw new RuntimeException(
          "Please don't specify --target-crs. Only use --input-crs to determinate the network crs!");
...

    {
      ...
      filter.addLinkFilter(
          link -> roadTypeFilter.stream().anyMatch(predicate -> predicate.test(link)));

      filteredNetwork = filter.applyFilters();
    }

    CoordinateTransformation coordinateTransformation =
        TransformationFactory.getCoordinateTransformation("EPSG:25832", crs.getInputCRS());
    NetworkIndex<BAStCountStation> index =
        new NetworkIndex<>(
            filteredNetwork,
            searchRange,
    ...
    for (var station : stations.values()) match(filteredNetwork, index, station, countsOption);
  }

  private Map<String, BAStCountStation> readBAStCountStations(
      Path pathToAggregatedData, ShpOptions shp, CountsOption counts) {

    List<BAStCountStation> stations = new ArrayList<>();

    // The original bast file has windows encoding
    try (BufferedReader reader =
        Files.newBufferedReader(pathToAggregatedData, StandardCharsets.ISO_8859_1)) {
      ...
      for (CSVRecord row : records) {
        ...
        if (counts.isIgnored(id) || counts.isIgnored(name)) continue;
    ...
    final Predicate<BAStCountStation> optFilter =
        station -> !ignored.contains(station.getId().replaceAll("_1", "").replaceAll("_2", ""));
    ...
    if (shp.getShapeFile() != null) {
      ...
      CoordinateTransformation transformation =
          TransformationFactory.getCoordinateTransformation("EPSG:25832", shp.getShapeCrs());
      shpFilter = station -> index.contains(transformation.transform(station.getCoord()));
    } else shpFilter = (station) -> true;

    // Return filtered map with id as key and station as value
    return stations.stream()
        .filter(optFilter.and(shpFilter))
        .collect(Collectors.toMap(BAStCountStation::getId, Function.identity()));
  }
}
