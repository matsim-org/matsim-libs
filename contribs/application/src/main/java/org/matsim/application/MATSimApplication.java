...
/**
 * A helper class to define and execute MATSim scenarios, including a pipeline of prepare and
 * analysis scripts. This class provides a common scenario setup procedure and command line parsing.
 * Scenarios simply need to extend it and overwrite the *prepare methods if needed.
 *
 * <p>To run your application use: <code>
 * MATSimApplication.run(RunScenario.class, args);
 * </code>
 *
 * <p>There are also other variants of this method, e.g. to run the scenario within code.
 *
 * <p>This class also automatically registers classes from the {@link Prepare} and {@link Analysis}
 * annotations as subcommands. These can be used to build a pipeline with command needed for
...
@CommandLine.Command(
    ...
    description = {
      "",
      "Use the \"run\" command to execute the scenario, or any of the other available commands."
    },
    ...
    footerHeading = "\n",
    footer =
        "@|cyan If you would like to contribute or report an issue please go to https://github.com/matsim-org.|@",
    usageHelpWidth = 120,
...
public abstract class MATSimApplication
    implements Callable<Integer>, CommandLine.IDefaultValueProvider {
  ...
  @CommandLine.Option(
      names = "--config",
      description = "Path to config file used for the run.",
      order = 0)
  protected File configPath;

  @CommandLine.Option(
      names = "--yaml",
      description = "Path to yaml file with config params to overwrite.",
      required = false)
  protected Path specs;

  @CommandLine.Option(
      names = "--iterations",
      description = "Overwrite number of iterations (if greater than -1).",
      defaultValue = "-1")
  ...
  @CommandLine.Option(
      names = "--output",
      description = "Overwrite output folder defined by the application")
  protected Path output;

  @CommandLine.Option(
      names = "--post-processing",
      description = "Option for post-processing",
      defaultValue = "enabled")
  protected PostProcessOption post;

  /**
   * This Map will never contain anything, because this argument is not parsed correctly, but
   * instead will go into remainingArgs field.
   *
   * @see #remainingArgs
   */
  @CommandLine.Option(
      names = {"-c:", "--config:"},
      arity = "0..*",
      description = "Overwrite config values (e.g. --config:controler.runId=123)")
  private Map<String, String> configValues;

  @CommandLine.Unmatched private List<String> remainingArgs;

  /** Path to the default scenario config, if applicable. */
  @Nullable private final String defaultScenario;
  ...
  public Integer call() throws Exception {
    ...
    if (config == null) {
      config =
          loadConfig(
              Objects.requireNonNull(configPath, "No default scenario location given")
                  .getAbsoluteFile()
                  .toString());
    } else {
    ...
    if (specs != null) applySpecs(config, specs);

    if (remainingArgs != null) {
      String[] args =
          remainingArgs.stream().map(s -> s.replace("-c:", "--config:")).toArray(String[]::new);
      ConfigUtils.applyCommandline(config, args);
    }

    if (iterations > -1) config.controller().setLastIteration(iterations);

    if (output != null) config.controller().setOutputDirectory(output.toString());

    if (runId != null) config.controller().setRunId(runId);
    ...
    if (post != PostProcessOption.post_process_only) controler.run();

    if (post != PostProcessOption.disabled) {

      List<MATSimAppCommand> commands =
          preparePostProcessing(
              Path.of(config.controller().getOutputDirectory()), config.controller().getRunId());
      ...
      if (field.getValue().isArray()) {
        ...
        if (field.getValue().size() == 1 && params.size() == 1) {
          ...
          for (JsonNode item : field.getValue()) {
            ...
            Optional<? extends ConfigGroup> m =
                params.stream()
                    .filter(
                        p -> p.getParams().get(first.getKey()).equals(first.getValue().textValue()))
                    .findFirst();
            if (m.isEmpty())
        ...
        if (!field.getValue().isValueNode())
          throw new IllegalArgumentException(
              "Received complex value type instead of primitive: " + field.getValue());

        if (field.getValue().isTextual())
          group.addParam(field.getKey(), field.getValue().textValue());
        else group.addParam(field.getKey(), field.getValue().toString());
      }
    }
  }
  ...
  protected List<ConfigGroup> getConfigurableModules() {
    return Lists.newArrayList(
        new ControllerConfigGroup(), new GlobalConfigGroup(), new QSimConfigGroup());
  }
  ...
  /**
   * Preparation of {@link MATSimAppCommand} to run after the simulation has finished. The instances
   * have to be fully constructed in this method no further arguments are passed down to them.
   *
  ...
  protected final void addRunOption(Config config, String option, Object value) {
    ...
    String postfix;
    if ("".equals(value)) postfix = "-" + option;
    else postfix = "-" + option + "_" + value;

    String outputDir = config.controller().getOutputDirectory();
    if (outputDir.endsWith("/")) {
      config
          .controller()
          .setOutputDirectory(outputDir.substring(0, outputDir.length() - 1) + postfix + "/");
    } else config.controller().setOutputDirectory(outputDir + postfix);
  ...
  public String defaultValue(CommandLine.Model.ArgSpec argSpec) throws Exception {
    ...
    if (obj instanceof Field field) {
      ...
      if (field.getName().equals("configPath")
          && field.getDeclaringClass().equals(MATSimApplication.class)) {
  ...
  /**
   * Run the application class and terminates when done. This should never be used in tests and only
   * in main methods.
   */
  ...
  public static int execute(
      Class<? extends MATSimApplication> clazz, Config config, String... args) {
    ...
    if (code > 0) {
      ...
      else throw new RuntimeException("Application exited with error", e);
    }
  ...
  /**
   * Prepare and return controller without running the scenario. This allows to configure the
   * controller after setup has been run.
   */
  public static Controler prepare(MATSimApplication app, Config config, String... args) {
    ...
    if (parseResult.errors().size() > 0) throw new RuntimeException(parseResult.errors().get(0));
    ...
    if (app.remainingArgs != null) {
      String[] extraArgs =
          app.remainingArgs.stream().map(s -> s.replace("-c:", "--config:")).toArray(String[]::new);
  ...
  /**
   * Prepare and return controller without running the scenario. This allows to configure the
   * controller after setup has been run. This method tries to use one of the constructors of the
   * given class automatically.
   *
   * @see #prepare(MATSimApplication, Config, String...)
   */
  public static Controler prepare(
      Class<? extends MATSimApplication> clazz, Config config, String... args) {
  ...
  private static List<String> unmatched(CommandLine.ParseResult parseResult) {
    ...
    while (it.hasNext()) {
      ...
      if (current.startsWith("-c:") || current.startsWith("--config:")) it.remove();

      // separate arguments should also be valid, e.g --c:global.threads 5
      if (!current.startsWith("--")
          && prev != null
          && (prev.startsWith("-c:") || prev.startsWith("--config:"))) it.remove();
  ...
  private static MATSimApplication newInstance(
      Class<? extends MATSimApplication> clazz, Config config) {
  ...
  private static void setupConfig(CommandLine cli, List<ConfigGroup> modules) {
    ...
    for (ConfigGroup module : modules) {
      ...
      for (Map.Entry<String, String> param : module.getParams().entrySet()) {
        ...
        group.addArg(
            CommandLine.Model.OptionSpec.builder("--" + module.getName() + "-" + param.getKey())
  ...
  @CommandLine.Command(
      name = "prepare",
      description =
          "Contains all commands for preparing the scenario. (See prepare help; \n"
              + "  needs to be ... \"prepare\", \"help\" ) if run from Java.)",
      // (This used to be "help prepare", which works as well.  However, "prepare help <subcommand>"
      // then also works while "help
      // prepare <subcommand>" does not.  So I think that "prepare help" saves some time in
      // understanding help options.  kai, nov'22)
  ...
  @CommandLine.Command(
      name = "analysis",
      description = "Contains all commands for analysing the scenario. (See help analysis)",
      subcommands = CommandLine.HelpCommand.class)
