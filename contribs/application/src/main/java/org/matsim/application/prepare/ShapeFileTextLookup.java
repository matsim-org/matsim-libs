...
public class ShapeFileTextLookup implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--csv-column",
      description = "Name of the csv column to match",
      required = true)
  private String csvColumn;

  @CommandLine.Option(
      names = "--shp-column",
      description = "Name of the shp feature to match",
      required = true)
  private String shpColumn;

  @CommandLine.Mixin private ShpOptions shp = new ShpOptions();

  @CommandLine.Mixin private CsvOptions csvOptions = new CsvOptions();
  ...
  public Integer call() throws Exception {
    ...
    for (SimpleFeature ft : features) {
      ...
      if (map.containsKey(name)) log.warn("Entry {} is duplicated", name);
    ...
    for (AttributeDescriptor attr : features.get(0).getType().getAttributeDescriptors()) {
      ...
      if (name.contains("geom")) continue;

      attributes.add(attr.getLocalName());
    }

    try (CSVParser reader = csvOptions.createParser(input);
        CSVPrinter writer = csvOptions.createPrinter(output)) {
