...
@CommandLine.Command(
    name = "congestion",
    description = "Calculates congestion indices and relative travel times.")
@CommandSpec(
    requireEvents = true,
    requireNetwork = true,
    produces = {
      "traffic_stats_by_link_daily.csv",
      "traffic_stats_by_link_and_hour.csv",
      "traffic_stats_by_road_type_daily.csv",
...
public class TrafficAnalysis implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--transport-modes",
      description = "transport modes to analyze",
      defaultValue = "",
      split = ",")
  ...
  public Integer call() throws Exception {
    ...
    TrafficStatsCalculator calc =
        new TrafficStatsCalculator(network, travelTimes.getLinkTravelTimes(), 900);
    ...
    List<String> sums =
        ds.columnNames().stream()
            .filter(s -> s.startsWith("vol_") || s.endsWith("_volume"))
            .toList();
    ...
    for (int hour = 0; hour < 24; hour++) {
      ...
      for (String roadType : roadTypes) {
        ...
        double congestionIndex =
            calc.getNetworkCongestionIndex(
                hour * 3600, (hour + 1) * 3600, roadType.equals("all") ? null : roadType);
    ...
    perRoadTypeAndHour
        ...
        .write()
        .csv(output.getPath("traffic_stats_by_road_type_and_hour.csv").toFile());

    Table dailyCongestionIndex =
        Table.create(StringColumn.create("road_type"), DoubleColumn.create("congestion_index"));

    for (String roadType : roadTypes) {

      double congestionIndex =
          calc.getNetworkCongestionIndex(0, 86400, roadType.equals("all") ? null : roadType);
    ...
    Table perRoadType =
        dailyCongestionIndex
            .joinOn("road_type")
            .leftOuter(
                weightedMeanBy(ds, means, "road_type")
                    .rejectColumns("speed_performance_index", "congestion_index"));

    DoubleColumn meanLaneKm =
        perRoadType
            .doubleColumn("lane_km")
            .divide(24)
            .multiply(1000)
            .round()
            .divide(1000)
    ...
    perRoadType
        .sortOn("Road Type")
        .write()
        .csv(output.getPath("traffic_stats_by_road_type_daily.csv").toFile());
  ...
  private Table createDataset(
      Network network, TrafficStatsCalculator calc, VolumesAnalyzer volumes) {
    ...
    for (Link link : network.getLinks().values()) {
      ...
      for (int h = 0; h < 24; h += 1) {
        ...
        row.setDouble(
            "speed_performance_index", calc.getSpeedPerformanceIndex(link, startTime, endTime));
        ...
        for (String mode : modes) {
          row.setDouble(
              "vol_" + mode,
              volumes.getVolumesPerHourForLink(link.getId(), mode)[h] / sample.getSample());
        }
      }
    }
