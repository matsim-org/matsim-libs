...
/**
 * This script utilizes GTFS2MATSim and creates a pseudo network and vehicles using MATSim standard
 * API functionality.
 *
 * @author rakow
 */
@CommandLine.Command(
    ...
    description =
        "Create transit schedule and vehicles from GTFS data and merge pt network into existing network",
...
public class CreateTransitScheduleFromGtfs implements MATSimAppCommand {
  ...
  @CommandLine.Parameters(
      arity = "1..*",
      paramLabel = "INPUT",
      description = "Input GTFS zip files")
  ...
  @CommandLine.Option(
      names = "--network",
      description = "Base network that will be merged with pt network.",
      required = true)
  ...
  @CommandLine.Option(
      names = "--date",
      arity = "1..*",
      description = "The day for which the schedules will be extracted, can also be multiple",
      required = true)
  ...
  @CommandLine.Option(
      names = "--copy-late-early",
      description = "Copy late/early departures to have at complete schedule")
  private boolean copyLateEarlyDepartures;

  @CommandLine.Option(
      names = "--validate",
      description = "Validate the resulting network and schedule",
      defaultValue = "true",
      negatable = true)
  ...
  @CommandLine.Option(
      names = "--include-stops",
      description = "Fully qualified class name to a Predicate<Stop> for filtering certain stops")
  private Class<?> includeStops;

  @CommandLine.Option(
      names = "--merge-stops",
      description = "Whether stops should be merged by coordinate")
  private boolean mergeStops;

  @CommandLine.Option(
      names = "--prefix",
      description =
          "Prefixes to add to the gtfs ids. Required if multiple inputs are used and ids are not unique.",
      split = ",")
  ...
  @CommandLine.Option(
      names = "--shp",
      description =
          "Path to shape file for filtering stops. Can be multiple if different filters on each input file should be used.",
      arity = "0..*")
  private List<Path> shpFiles;

  @CommandLine.Option(
      names = "--shp-crs",
      description = "Overwrite coordinate system of the shape file")
  ...
  public Integer call() throws Exception {
    ...
    for (Path gtfsFile : gtfsFiles) {
      ...
      GtfsConverter.Builder converter =
    ...
    if (copyLateEarlyDepartures) {
      ...
      TransitSchedulePostProcessTools.copyLateDeparturesToStartOfDay(
          scenario.getTransitSchedule(), 24 * 3600, "copied", false);
      TransitSchedulePostProcessTools.copyEarlyDeparturesToFollowingNight(
          scenario.getTransitSchedule(), 6 * 3600, "copied");
    }

    Network network = NetworkUtils.readNetwork(networkFile);

    Scenario ptScenario =
        getScenarioWithPseudoPtNetworkAndTransitVehicles(
            network, scenario.getTransitSchedule(), "pt_");

    if (validate) {
      // Check schedule and network
      TransitScheduleValidator.ValidationResult checkResult =
          TransitScheduleValidator.validateAll(
              ptScenario.getTransitSchedule(), ptScenario.getNetwork());
    ...
    new TransitScheduleWriter(ptScenario.getTransitSchedule())
        .writeFile(scheduleFile.getAbsolutePath());
    ...
    new MatsimVehicleWriter(ptScenario.getTransitVehicles())
        .writeFile(transitVehiclesFile.getAbsolutePath());
  ...
  private Predicate<Stop> createFilter(int i) throws Exception {
    ...
    if (shpFiles != null && !shpFiles.isEmpty()) {
      ShpOptions shp =
          new ShpOptions(shpFiles.size() == 1 ? shpFiles.get(0) : shpFiles.get(i), shpCrs, null);
  ...
  private static Scenario getScenarioWithPseudoPtNetworkAndTransitVehicles(
      Network network, TransitSchedule schedule, String ptNetworkIdentifier) {
    ScenarioUtils.ScenarioBuilder builder =
        new ScenarioUtils.ScenarioBuilder(ConfigUtils.createConfig());
    ...
    new CreatePseudoNetwork(
            scenario.getTransitSchedule(), scenario.getNetwork(), "pt_", 0.1, 100000.0)
        .createNetwork();

    // create TransitVehicle types
    // see https://svn.vsp.tu-berlin.de/repos/public-svn/publications/vspwp/2014/14-24/ for veh
    // capacities
    // the values set here are at the upper end of the typical capacity range, so on lines with high
    // capacity vehicles the
    // capacity of the matsim vehicle equals roughly the real vehicles capacity and on other lines
    // the Matsim vehicle
    // capacity is higher than the real used vehicle's capacity (gtfs provides no information on
    ...
    VehicleType reRbVehicleType =
        vehicleFactory.createVehicleType(Id.create("RE_RB_veh_type", VehicleType.class));
    {
      ...
      VehicleUtils.setAccessTime(
          reRbVehicleType, 1.0 / 10.0); // 1s per boarding agent, distributed on 10 doors
      VehicleUtils.setEgressTime(
          reRbVehicleType, 1.0 / 10.0); // 1s per alighting agent, distributed on 10 doors
      scenario.getTransitVehicles().addVehicleType(reRbVehicleType);
    }
    VehicleType sBahnVehicleType =
        vehicleFactory.createVehicleType(Id.create("S-Bahn_veh_type", VehicleType.class));
    {
      ...
      VehicleUtils.setDoorOperationMode(
          ...
          VehicleType.DoorOperationMode.serial); // first finish boarding, then start alighting
      VehicleUtils.setAccessTime(
          sBahnVehicleType, 1.0 / 24.0); // 1s per boarding agent, distributed on 8*3 doors
      VehicleUtils.setEgressTime(
          sBahnVehicleType, 1.0 / 24.0); // 1s per alighting agent, distributed on 8*3 doors
      scenario.getTransitVehicles().addVehicleType(sBahnVehicleType);
    }
    VehicleType uBahnVehicleType =
        vehicleFactory.createVehicleType(Id.create("U-Bahn_veh_type", VehicleType.class));
    {
      ...
      VehicleUtils.setDoorOperationMode(
          ...
          VehicleType.DoorOperationMode.serial); // first finish boarding, then start alighting
      VehicleUtils.setAccessTime(
          uBahnVehicleType, 1.0 / 18.0); // 1s per boarding agent, distributed on 6*3 doors
      VehicleUtils.setEgressTime(
          uBahnVehicleType, 1.0 / 18.0); // 1s per alighting agent, distributed on 6*3 doors
      scenario.getTransitVehicles().addVehicleType(uBahnVehicleType);
    }
    VehicleType tramVehicleType =
        vehicleFactory.createVehicleType(Id.create("Tram_veh_type", VehicleType.class));
    {
      ...
      VehicleUtils.setDoorOperationMode(
          ...
          VehicleType.DoorOperationMode.serial); // first finish boarding, then start alighting
      VehicleUtils.setAccessTime(
          tramVehicleType, 1.0 / 5.0); // 1s per boarding agent, distributed on 5 doors
      VehicleUtils.setEgressTime(
          tramVehicleType, 1.0 / 5.0); // 1s per alighting agent, distributed on 5 doors
    ...
    VehicleType ferryVehicleType =
        vehicleFactory.createVehicleType(Id.create("Ferry_veh_type", VehicleType.class));
    {
      ...
      VehicleUtils.setDoorOperationMode(
          ...
          VehicleType.DoorOperationMode.serial); // first finish boarding, then start alighting
      VehicleUtils.setAccessTime(
          ferryVehicleType, 1.0 / 1.0); // 1s per boarding agent, distributed on 1 door
      VehicleUtils.setEgressTime(
          ferryVehicleType, 1.0 / 1.0); // 1s per alighting agent, distributed on 1 door
      scenario.getTransitVehicles().addVehicleType(ferryVehicleType);
    }

    VehicleType ptVehicleType =
        vehicleFactory.createVehicleType(Id.create("Pt_veh_type", VehicleType.class));
    {
      ...
      VehicleUtils.setDoorOperationMode(
          ...
          VehicleType.DoorOperationMode.serial); // first finish boarding, then start alighting
      VehicleUtils.setAccessTime(
          ptVehicleType, 1.0 / 1.0); // 1s per boarding agent, distributed on 1 door
      VehicleUtils.setEgressTime(
          ptVehicleType, 1.0 / 1.0); // 1s per alighting agent, distributed on 1 door
    ...
    for (TransitLine line : scenario.getTransitSchedule().getTransitLines().values()) {
      ...
      // identify veh type / mode using gtfs route type (3-digit code, also found at the end of the
      // line id (gtfs: route_id))
      int gtfsTransitType;
      try {
        gtfsTransitType =
            Integer.parseInt((String) line.getAttributes().getAttribute("gtfs_route_type"));
      } catch (NumberFormatException e) {
        log.error(
            "unknown transit mode! Line id was "
                + line.getId().toString()
                + "; gtfs route type was "
                + line.getAttributes().getAttribute("gtfs_route_type"));
        throw new RuntimeException("unknown transit mode");
      }

      switch (gtfsTransitType) {
          // the vbb gtfs file generally uses the new gtfs route types, but some lines use the old
          // enum in the range 0 to 7
      ...
      for (TransitRoute route : line.getRoutes().values()) {
        ...
        if (routeStops.size() < 2) {
          log.warn(
              "TransitRoute with less than 2 stops found: line {}, route {}",
              line.getId(),
              route.getId());
          toRemove.add(route);
          continue;
        ...
        for (Departure departure : route.getDepartures().values()) {
          Vehicle veh =
              vehicleFactory.createVehicle(
                  Id.create(
                      "pt_" + route.getId().toString() + "_" + Long.toString(routeVehId++),
                      Vehicle.class),
                  lineVehicleType);
        ...
        // min. time spend at a stop, useful especially for stops whose arrival and departure offset
        // is identical,
        ...
        for (int i = 1; i < routeStops.size(); i++) {
          ...
          double stopDuration =
              routeStop.getDepartureOffset().isDefined()
                  ? routeStop.getDepartureOffset().seconds()
                      - routeStop.getArrivalOffset().seconds()
                  : minStopTime;
          // ensure arrival at next stop early enough to allow for 30s stop duration -> time for
          // passengers to board / alight
          // if link freespeed had been set such that the pt veh arrives exactly on time, but
          // departure tiome is identical
