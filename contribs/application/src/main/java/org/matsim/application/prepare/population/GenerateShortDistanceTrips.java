...
public class GenerateShortDistanceTrips implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--act-type",
      description = "Name of the generated activity type",
      defaultValue = "other")
  ...
  public Integer call() throws Exception {
    ...
    if (shp.getShapeFile() != null) {
      ...
      HomeLocationFilter homeLocationFilter =
          new HomeLocationFilter(shp, crs.getInputCRS(), population);
      ...
      log.info(
          "The population size inside the city for this simulation setup is "
              + homeLocationFilter.size());

    } else personsInCityBoundary.addAll(population.getPersons().keySet());

    log.info("Persons in boundary: {}", personsInCityBoundary.size());

    Predicate<Id<Person>> condition = personsInCityBoundary::contains;

    List<TripStructureUtils.Trip> originalTrips =
        population.getPersons().values().stream()
            .filter(person -> condition.test(person.getId()))
    ...
    if (generate.numOfMissingTrips > 0) {
      ...
      double shortDistance =
          originalTrips.stream()
              .mapToDouble(
                  t ->
                      CoordUtils.calcEuclideanDistance(
                          t.getOriginActivity().getCoord(), t.getDestinationActivity().getCoord()))
              .filter(d -> d <= range)
              .count();
      ...
      log.info(
          "There are {} short distance trips, share is {} and should be {}",
          shortDistance,
          actualShare,
          generate.tripShare);
      ...
      numTrips =
          (int)
              (Math.round(shortDistance - originalTrips.size() * generate.tripShare)
                  / (generate.tripShare - 1));
    }

    log.info("Generating {} trips", numTrips);

    if (numTrips <= 0) return 2;

    run(condition, numTrips);

    if (output == null) output = Path.of(input.toString().replace(".xml", "-with-trips.xml"));
  ...
  private void run(Predicate<Id<Person>> addingCondition, int numOfMissingTrips) {
    ...
    if (probability > 1)
      log.warn(
          "Probability is too high, there are not enough activities in the input too enhance them");
    ...
    for (Person person : population.getPersons().values()) {
      if (addingCondition.test(person.getId())) {
        ...
        List<Activity> markedActivities =
            filterActivities.stream()
                .filter(x -> rnd.nextDouble() < probability)
                .collect(Collectors.toList());
        ...
        if (markedActivities.size() > 0) {
          ...
          for (TripStructureUtils.Trip trip : TripStructureUtils.getTrips(plan.getPlanElements())) {
            ...
            if (markedActivities.contains(originActivity)) {
              ...
              double maxDurationForShortDistanceTrips =
                  Math.max(
                      originActivity.getEndTime().seconds()
                          - originActivity.getStartTime().seconds()
                          - walkTime,
                      1);
              double duration =
                  Math.min(this.maxDuration, rnd.nextDouble() * maxDurationForShortDistanceTrips);
              ...
              Activity shortDistanceRangeActivity =
                  population
                      .getFactory()
                      .createActivityFromCoord(
                          actType,
                          getShortDistanceCoordinate(trip.getOriginActivity().getCoord(), range));
  ...
  private double updateEndTime(Activity originActivity, double duration, double v) {
    double actDuration =
        originActivity.getEndTime().seconds() - originActivity.getStartTime().seconds();
  ...
  private double computeAddingProbability(
      int numOfMissingTrips, Predicate<Id<Person>> addingCondition) {
    int numOfAct = 0;
    int numOfTrips = 0;
    for (Person person : population.getPersons().values()) {
  ...
  private List<Activity> activityFilter(List<Activity> activities) {
    ...
    if (activities.size() > 2) {
      for (int i = 1; i < activities.size() - 2; i++) {
        ...
        if ((endTime - startTime) > range * 2 * 1.5) filterAct.add(activities.get(i));
      }
    }
    return filterAct;
  }
  ...
  static class Generate {
    ...
    @CommandLine.Option(
        names = "--num-trips",
        description = "Number of trips to generate",
        required = true)
    private int numOfMissingTrips = -1;

    @CommandLine.Option(
        names = "--trip-share",
        description = "Generate as many trips needed to match the share",
        required = true)
