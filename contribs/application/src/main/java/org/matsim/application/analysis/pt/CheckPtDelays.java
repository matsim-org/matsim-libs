...
public class CheckPtDelays implements MATSimAppCommand {
  ...
  private final Map<Id<TransitRoute>, Id<TransitLine>> transitRouteId2TransitLineId =
      new HashMap<>();
  ...
  // it seems that VehicleArrivesAtFacility and VehicleDepartsAtFacility evnt types are only used
  // for transit vehicles,
  // because both have TransitStopFacility
  private class VehicleDelayEventHandler
    ...
    private void handleDelay(
        ...
        double delayTime,
        Map<Id<Vehicle>, DelayRecord> veh2maxDelay,
        Map<Id<Vehicle>, DelayRecord> veh2minDelay) {
  ...
  /**
   * This assumes that each TransitVehicle is used on exactly one TransitLine, never on multiple
   * lines
   */
