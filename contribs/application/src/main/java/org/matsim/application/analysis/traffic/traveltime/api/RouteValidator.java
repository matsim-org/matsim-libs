...
public interface RouteValidator extends AutoCloseable {
  ...
  static ZonedDateTime createDateTime(int hour) {
    LocalDate date =
        LocalDate.now(ZoneId.systemDefault()).with(TemporalAdjusters.next(DayOfWeek.WEDNESDAY));
  ...
  /**
   * Exception that can be thrown, if an API key is not valid and processing should stop
   * immediately.
   */
