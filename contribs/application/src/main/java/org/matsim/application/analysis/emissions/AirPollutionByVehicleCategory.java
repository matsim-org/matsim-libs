...
public class AirPollutionByVehicleCategory implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--runId",
      description = "Pattern to match runId.",
      defaultValue = "*")
  ...
  @CommandLine.Option(
      names = "--hbefa-lookup",
      description = "Emission detailedVsAverageLookupBehavior",
      defaultValue = "directlyTryAverageTable")
  private DetailedVsAverageLookupBehavior lookupBehavior;

  @CommandLine.Option(
      names = "-p",
      description = "Password for encrypted hbefa files",
      interactive = true,
      required = false)
  ...
  @CommandLine.Option(
      names = "--vehicle-type",
      description = "Map vehicle type to Hbefa category",
      defaultValue = "defaultVehicleType=PASSENGER_CAR")
  private Map<String, HbefaVehicleCategory> vehicleCategories;

  @CommandLine.Option(
      names = "--use-default-road-types",
      description = "Add default hbefa_road_type link attributes to the network",
      defaultValue = "false")
  ...
  public AirPollutionByVehicleCategory(
      Path runDirectory, String runId, Path hbefaFileWarm, Path hbefaFileCold, Path output) {
  ...
  public Integer call() throws Exception {
    ...
    for (VehicleType type :
      ...
      HbefaVehicleCategory cat =
          vehicleCategories.computeIfAbsent(
              type.getId().toString(),
              (k) -> {
                log.warn(
                    "Vehicle type {} not mapped to a category, using {}",
                    k,
                    HbefaVehicleCategory.NON_HBEFA_VEHICLE);
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            Files.newBufferedWriter(
                Path.of(output.toString().replace(".xml", ".footprint.csv").replace(".gz", ""))),
            CSVFormat.DEFAULT)) {
