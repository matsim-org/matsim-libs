...
@CommandSpec(
    ...
    produces = {
      "mode_share.csv",
      "mode_share_per_dist.csv",
      "mode_users.csv",
      "trip_stats.csv",
      "population_trip_stats.csv",
      "trip_purposes_by_hour.csv"
    })
public class TripAnalysis implements MATSimAppCommand {
  ...
  @CommandLine.Mixin private InputOptions input = InputOptions.ofCommand(TripAnalysis.class);
  @CommandLine.Mixin private OutputOptions output = OutputOptions.ofCommand(TripAnalysis.class);
  ...
  @CommandLine.Option(
      names = "--dist-groups",
      split = ",",
      description = "List of distances for binning",
      defaultValue = "0,1000,2000,5000,10000,20000")
  private List<Long> distGroups;

  @CommandLine.Option(
      names = "--modes",
      split = ",",
      description = "List of considered modes, if not set all will be used")
  private List<String> modeOrder;

  @CommandLine.Option(
      names = "--shp-filter",
      description = "Define how the shp file filtering should work",
      defaultValue = "home")
  ...
  private static String cut(long dist, List<Long> distGroups, List<String> labels) {
    ...
    if (idx >= 0) return labels.get(idx);
  ...
  public Integer call() throws Exception {
    ...
    if (shp.isDefined() && filter == LocationFilter.trip_start_and_end) {
      ...
      for (int i = 0; i < trips.rowCount(); i++) {
        Row row = trips.row(i);
        Point start =
            f.createPoint(new Coordinate(row.getDouble("start_x"), row.getDouble("start_y")));
    ...
    trips
        .stringColumn("main_mode")
        .set(
            trips.stringColumn("main_mode").isMissing(),
    ...
    StringColumn dist_group =
        joined
            .longColumn("traveled_distance")
            .map(dist -> cut(dist, distGroups, labels), ColumnType.STRING::create)
            .setName("dist_group");
  ...
  private void writeModeShare(Table trips, List<String> labels) {
    ...
    Comparator<Row> cmp =
        Comparator.comparingInt(row -> labels.indexOf(row.getString("dist_group")));
  ...
  private void writeTripStats(Table trips) throws IOException {
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            Files.newBufferedWriter(output.getPath("trip_stats.csv")), CSVFormat.DEFAULT)) {
  ...
  private void writePopulationStats(Table persons, Table trips) throws IOException {
    ...
    for (Row person : persons) {
      ...
      if (!tripsPerPerson.containsKey(id)) tripsPerPerson.put(id, 0);
    }
    ...
    Table table =
        Table.create(
            TextColumn.create("main_mode", usedModes.size()),
            DoubleColumn.create("user", usedModes.size()));
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            Files.newBufferedWriter(output.getPath("population_trip_stats.csv")),
            CSVFormat.DEFAULT)) {
      ...
      printer.printRecord(
          "Mobile persons [%]",
          new BigDecimal(100 * totalMobile / tripsPerPerson.size())
              .setScale(2, RoundingMode.HALF_UP));
      printer.printRecord("Avg. trips", new BigDecimal(avgTrips).setScale(2, RoundingMode.HALF_UP));
      printer.printRecord(
          "Avg. trip per mobile persons",
          new BigDecimal(avgTripsMobile).setScale(2, RoundingMode.HALF_UP));
    }
  }
  ...
  private void writeTripPurposes(Table trips) {
    ...
    for (Row t : trips) {
      ...
      if (depTime[2] >= 60) depTime[1]++;

      depTime[1] += travTimes[1];
      if (depTime[1] >= 60) depTime[0]++;
    ...
    DoubleColumn share =
        tArrival.numberColumn(2).divide(tArrival.numberColumn(2).sum()).setName("arrival");
    ...
    share =
        tDeparture.numberColumn(2).divide(tDeparture.numberColumn(2).sum()).setName("departure");
