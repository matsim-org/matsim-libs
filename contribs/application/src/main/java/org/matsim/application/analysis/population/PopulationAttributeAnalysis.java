...
@CommandSpec(
    ...
    produces = {
      "amount_per_age_group.csv",
      "amount_per_sex_group.csv",
      "total_agents.csv",
      "average_income_per_age_group.csv"
    })
public class PopulationAttributeAnalysis implements MATSimAppCommand {
  ...
  private final HashMap<Integer, List<Double>> averageIncomeOverAge =
      new HashMap<>(); // <Age, <Number, Total Income>>
  ...
  public Integer call() throws Exception {
    ...
    for (Person person : population.getPersons().values()) {
      ...
      if (person.getAttributes().getAsMap().containsKey("age")
          && person.getAttributes().getAsMap().containsKey("income")
          && person.getAttributes().getAsMap().containsKey("sex")) {
        this.splitIncomeOverAgeAndSex(
            (int) person.getAttributes().getAttribute("age"),
            (double) person.getAttributes().getAttribute("income"));
      }
      ...
      for (Map.Entry<String, Object> entry : map.entrySet()) {
        ...
        if (entry.getKey().equals("vehicles"))
          System.out.println(((PersonVehicles) entry.getValue()).getModeVehicles().toString());
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(output.getPath("amount_per_age_group.csv").toString()),
            CSVFormat.DEFAULT)) {
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(
                output.getPath("average_income_per_age_group.csv").toString()),
            CSVFormat.DEFAULT)) {
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(output.getPath("total_agents.csv").toString()),
            CSVFormat.DEFAULT)) {
      printer.printRecord("Total Agents", totalAgents, "user-group");
      printer.printRecord(
          "Average Age",
          new DecimalFormat("#.0#").format(this.calculateMeanFromIntegerArray(allAges)));
      printer.printRecord(
          "Average Income",
          new DecimalFormat("#.0#").format(this.calculateMeanFromDoubleArray(allIncomes)),
          "money-check-dollar");
    } catch (IOException ex) {
      log.error(ex);
    }
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(output.getPath("amount_per_sex_group.csv").toString()),
            CSVFormat.DEFAULT)) {
  ...
  private void splitAgentsIntoSex(String sex) {
    ...
    if (amountPerSexGroup.containsKey(sex))
      amountPerSexGroup.put(sex, amountPerSexGroup.get(sex) + 1);
  ...
  private void splitAgentsIntoAgeGroup(Integer age) {
    // Rounded Age: 9 -> 10; 10 -> 10; 11 -> 20
    int roundedAge = Math.round(((age - 1) / 10) + 1) * 10;
    if (amountPerAgeGroup.keySet().contains(roundedAge))
      amountPerAgeGroup.put(roundedAge, amountPerAgeGroup.get(roundedAge) + 1);
