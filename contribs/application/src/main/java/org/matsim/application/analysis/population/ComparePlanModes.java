...
@CommandLine.Command(
    name = "compare-plan-modes",
    description = "Compare plans between two populations")
public class ComparePlanModes implements MATSimAppCommand {

  @CommandLine.Parameters(
      arity = "2",
      paramLabel = "POPULATION",
      description = "Path to populations")
  ...
  public Integer call() throws Exception {
    ...
    if (!subpopulation.isEmpty())
      persons.removeIf(
          p -> {
            String subpop =
                PopulationUtils.getSubpopulation(populations.get(0).getPersons().get(p));
    ...
    try (CSVPrinter printer = csv.createPrinter(output)) {
      ...
      printer.printRecord(
          "person", "trips_to_subtours", "p1_score", "p2_score", "p1_modes", "p2_modes");
  ...
  private String getModes(Plan p) {
    return TripStructureUtils.getTrips(p).stream()
  ...
  private String getSubtourString(Plan p) {
    ...
    for (TripStructureUtils.Trip t : trips) {
      ...
      for (TripStructureUtils.Subtour st : subtours) {
        if (st.getTrips().contains(t)) contains.add(String.valueOf(i));
