...
public final class InputOptions {
  ...
  public static String argName(String require) {
    ...
    // Remove the suffix
    String s = split.length > 1 ? StringUtils.join(split, "-", 0, split.length - 1) : require;

    // replace special chars
    String result = s.replaceAll("_|/\"|\\\\|:|/", "-");

    // crs is a reserved name from other options
    if (result.equals("crs")) result = "crs-file";
  ...
  public String getPath() {
    if (spec.requires().length == 0)
      throw new IllegalArgumentException(
          "Spec input definition is empty. Input can not be accessed.");
  ...
  public String getPath(String name) {
    if (!inputs.containsKey(name))
      throw new IllegalArgumentException(
          String.format("The input '%s' is not registered as required in @CommandSpec.", name));
  ...
  public Network getNetwork() {
    if (!spec.requireNetwork())
      throw new IllegalArgumentException(
          "Network can not be accessed unless, requireNetwork=true.");
  ...
  public Population getPopulation() {
    if (!spec.requirePopulation())
      throw new IllegalArgumentException(
          "Population can not be accessed unless, requirePopulation=true.");
  ...
  public Path getRunDirectory() {
    if (!spec.requireRunDirectory())
      throw new IllegalArgumentException(
          "Run directory can not be accessed unless, requireRunDirectory=true.");
  ...
  void setSpec(CommandLine.Model.CommandSpec command) {
    ...
    if (!command.userObject().getClass().equals(clazz)) {
      ...
      throw new IllegalArgumentException(
          String.format(
              "InputOptions in the implementation of '%s' is using class '%s' as reference. These two must be the same",
    ...
    for (String require : spec.requires()) {
      ...
      command.add(
          createArg(
              flag,
              "--input-" + argName(require),
              "Path to input " + require + ".",
    ...
    if (spec.requirePopulation()) {
      command.add(
          createArg(
              flag,
              "--population",
              "Path to input plans / population.",
              new CommandLine.Model.ISetter() {
                @Override
                public <T> T set(T value) {
  ...
  private CommandLine.Model.OptionSpec createArg(
      AtomicBoolean flag, String name, String description, CommandLine.Model.ISetter setter) {
    CommandLine.Model.OptionSpec.Builder arg =
        CommandLine.Model.OptionSpec.builder(argNames(flag, "--input", name))
            .initialValue("")
            .type(String.class)
            .setter(setter)
