...
public class ScenarioOptions {
  ...
  @CommandLine.Option(
      names = "--config",
      description = "Path (or URI) to scenario config",
      required = true)
  ...
  @CommandLine.Option(
      names = "--scenario",
      description =
          "Full qualified classname of the MATSim application scenario class. The IMC modules must be specified there.",
      required = true)
  ...
  public Config getConfig() {
    if (config == null) {
      ...
      if (populationPath != null) config.plans().setInputFile(populationPath.toString());
    }
  ...
  public Controler createControler() {
    ...
    else return MATSimApplication.prepare(scenario, getConfig(), scenarioArgs.split(" "));
  }
}
