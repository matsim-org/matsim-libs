...
public class FreightAgentGenerator {
  ...
  public FreightAgentGenerator(
      Network network,
      Path shpPath,
      LanduseOptions landUse,
      double averageLoad,
      int workingDays,
      double sample)
      throws IOException {
    ...
    this.numOfTripsCalculator =
        new DefaultNumberOfTripsCalculator(averageLoad, workingDays, sample);
  ...
  public List<Person> generateFreightAgents(TripRelation tripRelation, String tripRelationId) {
    ...
    if (!preRunMode.equals("2") && !mainRunMode.equals("2") && !postRunMode.equals("2")) {
      return freightAgents; // This trip relation is irrelevant as it does not contain any freight
      // by road
    }

    int numOfTrips =
        numOfTripsCalculator.calculateNumberOfTrips(
            tripRelation.getTonsPerYear(), tripRelation.getGoodsType());
    for (int i = 0; i < numOfTrips; i++) {
      // pre-run
      if (preRunMode.equals("2")) {
        Person person =
            populationFactory.createPerson(
                Id.createPersonId("freight_" + tripRelationId + "_" + i + "_pre"));
        Plan plan = populationFactory.createPlan();
        double departureTime = departureTimeCalculator.getDepartureTime();

        Id<Link> startLinkId =
            locationCalculator.getLocationOnNetwork(tripRelation.getOriginalCell());
        Activity startAct =
            populationFactory.createActivityFromLinkId("freight_start", startLinkId);
        ...
        Id<Link> endLinkId =
            locationCalculator.getLocationOnNetwork(tripRelation.getOriginalCellMainRun());
      ...
      if (mainRunMode.equals("2")) {
        Person person =
            populationFactory.createPerson(
                Id.createPersonId("freight_" + tripRelationId + "_" + i + "_main"));
        Plan plan = populationFactory.createPlan();
        double departureTime = departureTimeCalculator.getDepartureTime();
        ...
        Id<Link> endLinkId =
            locationCalculator.getLocationOnNetwork(tripRelation.getDestinationCellMainRun());
        Activity endAct = populationFactory.createActivityFromLinkId("freight_end", endLinkId);
        endAct.setCoord(network.getLinks().get(endLinkId).getToNode().getCoord());
        plan.addActivity(endAct);
      ...
      if (postRunMode.equals("2")) {
        Person person =
            populationFactory.createPerson(
                Id.createPersonId("freight_" + tripRelationId + "_" + i + "_post"));
        Plan plan = populationFactory.createPlan();
        double departureTime = departureTimeCalculator.getDepartureTime();
        ...
        Id<Link> endLinkId =
            locationCalculator.getLocationOnNetwork(tripRelation.getDestinationCell());
        Activity endAct = populationFactory.createActivityFromLinkId("freight_end", endLinkId);
        endAct.setCoord(network.getLinks().get(endLinkId).getToNode().getCoord());
        plan.addActivity(endAct);
  ...
  private void writeCommonAttributes(
      Person person, TripRelation tripRelation, String tripRelationId) {
    ...
    person
        .getAttributes()
        .putAttribute("origin_cell_main_run", tripRelation.getDestinationCellMainRun());
    person
        .getAttributes()
        .putAttribute("destination_cell_main_run", tripRelation.getDestinationCellMainRun());
    person
        .getAttributes()
        .putAttribute("final_destination_cell", tripRelation.getDestinationCell());
