...
/** Type description //TODO // TODO Typo 2021--> 2016 */
public class DefaultLocationCalculator implements FreightAgentGenerator.LocationCalculator {
  private static final Logger logger = LogManager.getLogger(DefaultLocationCalculator.class);
  ...
  private static final String lookUpTablePath =
      "https://svn.vsp.tu-berlin.de/repos/public-svn/matsim/"
          + "scenarios/countries/de/german-wide-freight/v2/processed-data/complete-lookup-table.csv"; // This one is now fixed

  public DefaultLocationCalculator(Network network, Path shpFilePath, LanduseOptions landUse)
      throws IOException {
    this.shp = new ShpOptions(shpFilePath, "EPSG:4326", StandardCharsets.ISO_8859_1);
    // Reading shapefile from URL may not work properly, therefore users may need to download the
    // shape file to the local directory
  ...
  private void prepareMapping() throws IOException {
    ...
    ShpOptions.Index index =
        shp.createIndex("EPSG:25832", "NUTS_ID", relevantNutsIds); // network CRS: EPSG:25832
    ...
    // When filtered links list is not empty, then we use filtered links list. Otherwise, we use the
    // full link lists in the NUTS region.
    ...
    try (CSVParser parser =
      ...
      for (CSVRecord record : parser) {
        ...
        if (!nuts2021.equals("") && nutsToLinksMapping.get(nuts2021) != null) {
          mapping.put(
              verkehrszelle,
              nutsToLinksMapping.get(nuts2021).stream()
