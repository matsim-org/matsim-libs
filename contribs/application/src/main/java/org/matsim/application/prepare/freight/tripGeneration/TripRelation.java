...
/**
 * The object TripRelation corresponds to each entry (row) in the ketten-2010 table. The information
 * will be readed from the table and stored as the TripRelation. The data can then be used by
 * different tools to generate MATSim plans file.
 */
public class TripRelation {
  ...
  /**
   * Mode encoding: 0 --> unknown/not applicable; 1 --> railway; 2 --> road; 3 --> container ship;
   * Therefore "2" is what we are mainly looking at
   */
  ...
  // private final String originalTerminal; // Starting terminal for the main run (also the
  // destination for the pre-run)
  // private final String destinationTerminal; // Destination terminal for main run (also the
  // starting terminal for the post-run)
  ...
  public static List<TripRelation> readTripRelations(String pathToKettenData) throws IOException {
    ...
    try (CSVParser parser =
        CSVParser.parse(
            URI.create(pathToKettenData).toURL(),
      ...
      for (CSVRecord record : parser) {
        ...
        // Read locations
        builder
            .originalCell(record.get(0))
            .originalCellMainRun(record.get(2))
            .destinationCellMainRun(record.get(3))
            .destinationCell(record.get(1));
        // Read trips
