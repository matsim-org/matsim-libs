...
public final class CommandRunner {
  ...
  public void run(Path input) {
    ...
    Graph<Class<? extends MATSimAppCommand>, DefaultEdge> graph =
    ...
    for (Map.Entry<Class<? extends MATSimAppCommand>, String[]> e : args.entrySet()) {
      ...
      if (depends.length == 0) start.add(clazz);
    }

    BreadthFirstIterator<Class<? extends MATSimAppCommand>, DefaultEdge> it =
  ...
  private void runCommand(Class<? extends MATSimAppCommand> clazz, Path input)
  ...
  private Path buildPath(CommandSpec spec, Class<? extends MATSimAppCommand> command) {
    ...
    if (name != null && !name.isBlank()) context += "-" + name;

    return output.resolve(context);
  }

  private String[] createArgs(
      Class<? extends MATSimAppCommand> command, String[] existingArgs, Path input) {
    ...
    if (ApplicationUtils.acceptsOptions(command, ShpOptions.class)
        && !ArrayUtils.contains(existingArgs, "--shp")) {
    ...
    if (ApplicationUtils.acceptsOptions(command, CrsOptions.class)
        && !ArrayUtils.contains(existingArgs, "--input-crs")) {
      if (defaultCrs != null) {
        args.add("--input-crs");
        args.add(defaultCrs);
      }
    ...
    if (ApplicationUtils.acceptsOptions(command, SampleOptions.class)
        && !ArrayUtils.contains(existingArgs, "--sample-size")) {
  ...
  /**
   * Returns the output path of a command. Will throw an exception if this command does not declare
   * it as an output.
   */
  public Path getRequiredPath(Class<? extends MATSimAppCommand> command, String file) {
    ...
    if (!ArrayUtils.contains(spec.produces(), file))
      throw new IllegalArgumentException(
          String.format("Command %s does not declare output %s", command, file));
  ...
  public void add(Class<? extends MATSimAppCommand> command, String... args) {
    ...
    if (args.length != 0) {
      ...
      if (existing != null && existing.length > 0 && !Arrays.equals(existing, args)) {
        throw new IllegalArgumentException(
            String.format(
                "Command %s already registered with args %s, can not define different args as %s",
    ...
    for (Class<? extends MATSimAppCommand> depends : spec.dependsOn()) {
      if (!this.args.containsKey(depends)) add(depends);
    }
  }
