...
@CommandLine.Command(name = "downsample-population", description = "Down sample a population")
public class DownSamplePopulation implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--samples",
      description = "Desired down-sampled sizes in (0, 1]",
      arity = "1..*",
      required = true)
    ...
    for (Double sample : samples) {
      ...
      if (input.toString().contains(orig))
        ...
        path =
            input
                .toString()
                .replace(".xml", String.format("-%dpct.xml", Math.round(sampleSize * 100)));
