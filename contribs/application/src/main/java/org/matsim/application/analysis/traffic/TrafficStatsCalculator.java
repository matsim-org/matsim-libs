...
/**
 * Class to calculate the traffic congestion index based on the paper "A Traffic Congestion
 * Assessment Method for Urban Road Networks Based on Speed Performance Index" by Feifei He, Xuedong
 * Yan*, Yang Liu, Lu Ma.
 */
public final class TrafficStatsCalculator {
  ...
  /**
   * Calculates the speed performance index, which is the ratio of actual travel time and free speed
   * travel time.
   */
  ...
  public double getSpeedPerformanceIndex(Link link, int startTime, int endTime) {
    ...
    for (int time = startTime; time < endTime; time += timeSlice)
      indices.add(this.getSpeedPerformanceIndex(link, time));

    return indices.doubleStream().average().orElse(-1);
  }

  /**
   * Calculates the congestion index based on the ratio of actual travel time and free speed travel
   * time.
   */
  public double getLinkCongestionIndex(Link link, int startTime, int endTime) {
    ...
    for (int time = startTime; time < endTime; time += timeSlice) {
      ...
      if (speedPerformanceIndex <= 0.5) congestedPeriodCounter++;
  ...
  /**
   * Calculates the network congestion index for a given time period. Can be done for a certain road
   * type.
   */
  public double getNetworkCongestionIndex(int startTime, int endTime, @Nullable String roadType) {
    ...
    for (Map.Entry<Id<Link>, ? extends Link> entry : this.network.getLinks().entrySet()) {
      ...
      if (roadType != null && !type.equals(roadType)) continue;
