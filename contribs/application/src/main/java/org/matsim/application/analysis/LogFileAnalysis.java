...
@CommandLine.Command(
    name = "log-file",
    description = "Analyses MATSim log files to gather run information.")
@CommandSpec(
...
public class LogFileAnalysis implements MATSimAppCommand {
  ...
  @CommandLine.Mixin private InputOptions input = InputOptions.ofCommand(LogFileAnalysis.class);
  @CommandLine.Mixin private OutputOptions output = OutputOptions.ofCommand(LogFileAnalysis.class);

  @CommandLine.Mixin private CsvOptions csv = new CsvOptions(CSVFormat.Predefined.Default);
  ...
  public Integer call() throws Exception {
    ...
    Pattern mem =
        Pattern.compile(
            ".+MemoryObserver:\\d+ used RAM: (\\d+) MB\\s+free: (\\d+) MB\\s+total: (\\d+) MB");
    Pattern warn =
        Pattern.compile(".+(WARN|ERROR) (\\S+(ConfigGroup|ConsistencyCheck).*):[0-9]+ (.+)");
    ...
    if (first != null) {
      ...
      info.put(
          ...
          DurationFormatUtils.formatDurationWords(
              Duration.between(start, end).toMillis(), true, true));
    }
  ...
  private void renderWarnings(BufferedWriter writer, Set<Warning> warnings) throws IOException {
    ...
    if (warnings.isEmpty()) {
      ...
      Map<String, List<Warning>> grouped =
          warnings.stream().collect(Collectors.groupingBy(w -> w.module, Collectors.toList()));
      writer.write(
          String.format(
              "<h3 class=\"found-warnings\">Warnings found in %d module%s ❌</h3>\n\n",
