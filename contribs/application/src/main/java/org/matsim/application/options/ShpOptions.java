...
public final class ShpOptions {
  ...
  @CommandLine.Option(
      names = "--shp",
      description = "Optional path to shape file used for filtering",
      required = false)
  private Path shp;
  ...
  @CommandLine.Option(
      names = "--shp-charset",
      description = "Charset used to read the shape file",
  ...
  public static ShapefileDataStore openDataStore(Path shp) throws IOException {
    ...
    if (shp.toString().endsWith(".shp"))
      ...
      Optional<Path> match =
          Files.walk(fs.getPath("/")).filter(p -> p.toString().endsWith(".shp")).findFirst();

      if (match.isEmpty()) throw new IllegalArgumentException("No .shp file found in the zip.");
  ...
  public Index createIndex(String queryCRS, String attr, Set<String> filter) {
    ...
    if (!isDefined()) throw new IllegalStateException("Shape file path not specified");
    if (!Files.exists(shp))
      throw new IllegalStateException(String.format("Shape file %s does not exists", shp));
    if (queryCRS == null) throw new IllegalArgumentException("Query crs must not be null!");

    CoordinateTransformation ct =
        TransformationFactory.getCoordinateTransformation(queryCRS, detectCRS());
  ...
  /**
   * Create a coordinate transformation to the shape file crs. Tries to autodetect the crs of the
   * shape file.
   */
    ...
    if (shpCrs == null) {
      try {
        ...
        throw new IllegalStateException(
            "Could not determine crs of the shape file. Try to specify it manually using --shp-crs.",
            e);
      }
    }
  ...
  /**
   * Create an inverse coordinate transformation from the shape file crs. Tries to autodetect the
   * crs of the shape file.
   */
  ...
  public final class Index {
    ...
    Index(CoordinateTransformation ct, String attr, @Nullable Set<String> filter)
      ...
      while (it.hasNext()) {
        ...
        if (filter != null && !filter.contains(ft.getAttribute(attr))) continue;
    ...
    public String query(Coord coord) {
      // Because we can not easily transform the feature geometry with MATSim we have to do it the
      // other way around...
      ...
      for (SimpleFeature ft : result) {
        Geometry geom = (Geometry) ft.getDefaultGeometry();
        if (geom.contains(MGC.coordinate2Point(p))) return (String) ft.getAttribute(attr);
      }

      return null;
      // throw new NoSuchElementException(String.format("No matching entry found for x:%f y:%f %s",
      // x, y, p));
    }
    ...
    public boolean contains(Coord coord) {
      ...
      for (SimpleFeature ft : result) {
        Geometry geom = (Geometry) ft.getDefaultGeometry();
        if (geom.contains(MGC.coordinate2Point(p))) return true;
      }
