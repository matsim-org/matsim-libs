...
@CommandLine.Command(
    ...
    description =
        "Ensure activity to nearest link distance outside shape file is similar to within shape file.")
public class AdjustActivityToLinkDistances implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--scale",
      description = "Additional scale for the length",
      defaultValue = "1")
  ...
  public Integer call() throws Exception {
    ...
    DoubleList dist =
        computeLinkDistances(population, network, act -> index.contains(act.getCoord()));
    ...
    double std =
        Math.sqrt(dist.doubleStream().map(x -> (x - mean) * (x - mean)).sum() / dist.size());
    ...
    double lstd =
        Math.sqrt(
            dist.doubleStream().map(Math::log).map(x -> (x - lmean) * (x - lmean)).sum()
                / dist.size());
    ...
    for (Person p : population.getPersons().values()) {
      ...
      for (Activity act :
        ...
        if (!index.contains(act.getCoord())) {
          ...
          Coord v;
          if (mapping.containsKey(act.getCoord())) v = mapping.get(act.getCoord());
          else {
    ...
    double std2 =
        Math.sqrt(dist.doubleStream().map(x -> (x - mean2) * (x - mean2)).sum() / dist.size());

    log.info(
        "Distance to nearest link distribution outside shape had mean: {}, std: {}", mean2, std2);

    dist = computeLinkDistances(population, network, act -> !index.contains(act.getCoord()));

    double mean3 = dist.doubleStream().sum() / dist.size();
    double std3 =
        Math.sqrt(dist.doubleStream().map(x -> (x - mean3) * (x - mean3)).sum() / dist.size());

    log.info(
        "Distance to nearest link distribution outside shape after adjusting has mean: {}, std: {}",
        mean3,
        std3);
  ...
  private DoubleList computeLinkDistances(
      Population population, Network network, Predicate<Activity> f) {
