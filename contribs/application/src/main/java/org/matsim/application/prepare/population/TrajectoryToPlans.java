...
public class TrajectoryToPlans implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--sample-size",
      description = "Sample size of the given input data in (0, 1]",
      required = true)
  ...
  @CommandLine.Option(
      names = "--samples",
      description =
          "Desired down-sampled sizes in (0, 1]. Deprecated: Use the separate down-sampling instead.",
      arity = "1..*")
  private List<Double> samples;

  @CommandLine.Option(
      names = "--population",
      description = "Input original population file",
      required = true)
  private Path population;

  @CommandLine.Mixin private final CrsOptions crs = new CrsOptions();
  ...
  @CommandLine.Option(
      ...
      description =
          "Activity types are extended so that they belong to a typical duration. This parameter influences the number of typical duration classes. The default is 600s")
  private int activityBinSize = 600;

  @CommandLine.Option(
      names = {"--max-typical-duration", "--mtd"},
      description =
          "Max duration of activities for which a typical activity duration type is created in seconds. Default is 86400s (24h)")
  private int maxTypicalDuration = 86400;

  @CommandLine.Option(
      names = "--output",
      description = "Output folder",
      defaultValue = "scenarios/input")
  private Path output;
  ...
  public Integer call() throws Exception {
    ...
    PopulationUtils.writePopulation(
        scenario.getPopulation(),
        output
            .resolve(String.format("%s-%dpct.plans.xml.gz", name, Math.round(sampleSize * 100)))
            .toString());
    ...
    for (Person p : population.getPersons().values()) {
      ...
      for (Plan plan : p.getPlans()) {
        for (PlanElement el : plan.getPlanElements()) {
          ...
          if (!(el instanceof Activity)) continue;
  ...
  private void mergeOvernightActivities(Plan plan) {
    ...
    if (plan.getPlanElements().size() > 1) {
      ...
      Activity lastActivity =
          (Activity) plan.getPlanElements().get(plan.getPlanElements().size() - 1);
      ...
      if (firstBaseActivity.equals(lastBaseActivity)) {
        double mergedDuration =
            Double.parseDouble(firstActivity.getType().split("_")[1])
                + Double.parseDouble(lastActivity.getType().split("_")[1]);
