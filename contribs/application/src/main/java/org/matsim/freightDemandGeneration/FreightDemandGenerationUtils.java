...
public class FreightDemandGenerationUtils {
  ...
  static void preparePopulation(
    ...
    for (Id<Person> id : personsToRemove) population.removePerson(id);
  }
  ...
  static void createDemandLocationsFile(Controler controler) {
    ...
    File file =
        new File(
            controler.getConfig().controller().getOutputDirectory() + "/outputFacilitiesFile.tsv");
    try (FileWriter writer = new FileWriter(file, true)) {
      writer.write("id	x	y	type	ServiceLocation	pickupLocation	deliveryLocation\n");

      for (Carrier thisCarrier :
          CarriersUtils.getCarriers(controler.getScenario()).getCarriers().values()) {
        for (CarrierService thisService : thisCarrier.getServices().values()) {
          Coord coord =
              FreightDemandGenerationUtils.getCoordOfMiddlePointOfLink(
                  network.getLinks().get(thisService.getLocationLinkId()));
          writer.write(
              thisCarrier.getId().toString()
                  + thisService.getId().toString()
                  + "	"
                  + coord.getX()
                  + "	"
                  + coord.getY()
        ...
        for (CarrierShipment thisShipment : thisCarrier.getShipments().values()) {
          Coord coordFrom =
              FreightDemandGenerationUtils.getCoordOfMiddlePointOfLink(
                  network.getLinks().get(thisShipment.getFrom()));
          Coord coordTo =
              FreightDemandGenerationUtils.getCoordOfMiddlePointOfLink(
                  network.getLinks().get(thisShipment.getTo()));

          writer.write(
              thisCarrier.getId().toString()
                  + thisShipment.getId().toString()
                  + "	"
                  + coordFrom.getX()
                  + "	"
                  + coordFrom.getY()
  ...
  static void reducePopulationToShapeArea(Population population, ShpOptions.Index index) {
    ...
    for (Person person : population.getPersons().values()) {
      ...
      if (!index.contains(new Coord(x, y))) personsToRemove.add(person.getId());
    }
   ...
   * @return
   */
  static boolean checkPositionInShape(
      Link link,
      Point point,
    ...
    if (polygonsInShape == null) return true;
    ...
    if (link != null && point == null) {
      if (crsTransformationNetworkAndShape != null)
        p =
            MGC.coord2Point(
                crsTransformationNetworkAndShape.transform(getCoordOfMiddlePointOfLink(link)));
      else p = MGC.coord2Point(getCoordOfMiddlePointOfLink(link));
    } else if (link == null && point != null) p = point;
  ...
  static Coord getCoordOfMiddlePointOfLink(Link link) {
    ...
    if (xCoordFrom > xCoordTo) x = xCoordFrom - ((xCoordFrom - xCoordTo) / 2);
    else x = xCoordTo - ((xCoordTo - xCoordFrom) / 2);
    if (yCoordFrom > yCoordTo) y = yCoordFrom - ((yCoordFrom - yCoordTo) / 2);
    else y = yCoordTo - ((yCoordTo - yCoordFrom) / 2);
