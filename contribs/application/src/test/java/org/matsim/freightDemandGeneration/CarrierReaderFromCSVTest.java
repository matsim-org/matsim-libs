...
public class CarrierReaderFromCSVTest {
  ...
  public void carrierCreation() throws IOException {
    ...
    freightCarriersConfigGroup.setCarriersVehicleTypesFile(
        utils.getPackageInputDirectory() + "testVehicleTypes.xml");
    ...
    Set<CarrierInformationElement> allNewCarrierInformation =
        CarrierReaderFromCSV.readCarrierInformation(carrierCSVLocation);
    String shapeCategory = "Ortsteil";
    CarrierReaderFromCSV.checkNewCarrier(
        allNewCarrierInformation,
        freightCarriersConfigGroup,
        scenario,
        polygonsInShape,
        shapeCategory);
    CarrierReaderFromCSV.createNewCarrierAndAddVehicleTypes(
        scenario, allNewCarrierInformation, freightCarriersConfigGroup, polygonsInShape, 1, null);
    Assert.assertEquals(3, CarriersUtils.getCarriers(scenario).getCarriers().size());
    Assert.assertTrue(
        CarriersUtils.getCarriers(scenario)
            .getCarriers()
            .containsKey(Id.create("testCarrier1", Carrier.class)));
    Assert.assertTrue(
        CarriersUtils.getCarriers(scenario)
            .getCarriers()
            .containsKey(Id.create("testCarrier2", Carrier.class)));
    Assert.assertTrue(
        CarriersUtils.getCarriers(scenario)
            .getCarriers()
            .containsKey(Id.create("testCarrier3", Carrier.class)));

    // check carrier 1
    Carrier testCarrier1 =
        CarriersUtils.getCarriers(scenario)
            .getCarriers()
    ...
    for (CarrierVehicle carrierVehicle :
        testCarrier1.getCarrierCapabilities().getCarrierVehicles().values()) {
      typesPerDepot
          .computeIfAbsent(carrierVehicle.getLinkId().toString(), (k) -> new ArrayList<>())
    ...
    for (CarrierVehicle carrierVehicle :
        testCarrier2.getCarrierCapabilities().getCarrierVehicles().values()) {
      typesPerDepot
          .computeIfAbsent(carrierVehicle.getLinkId().toString(), (k) -> new ArrayList<>())
          .add(carrierVehicle.getVehicleTypeId().toString());
      depotSums.merge(carrierVehicle.getLinkId().toString(), 1, Integer::sum);
    ...
    typesPerDepot = new HashMap<>();
    for (CarrierVehicle carrierVehicle :
        testCarrier3.getCarrierCapabilities().getCarrierVehicles().values()) {
      typesPerDepot
          .computeIfAbsent(carrierVehicle.getLinkId().toString(), (k) -> new ArrayList<>())
          .add(carrierVehicle.getVehicleTypeId().toString());
      depotSums.merge(carrierVehicle.getLinkId().toString(), 1, Integer::sum);
      ...
      if (!depot.equals("j(2,6)R")) {
        ...
        Assert.assertTrue(
            FreightDemandGenerationUtils.checkPositionInShape(
                link, null, polygonsInShape, null, null));
        Assert.assertTrue(
            FreightDemandGenerationUtils.checkPositionInShape(
                link, null, polygonsInShape, new String[] {"area1"}, null));
        Assert.assertFalse(
            FreightDemandGenerationUtils.checkPositionInShape(
                link, null, polygonsInShape, new String[] {"area2"}, null));
      }
    }
    Assert.assertEquals(2, typesPerDepot.size());
  ...
  public void csvCarrierReader() throws IOException {
    ...
    for (CarrierInformationElement carrierInformationElement : allNewCarrierInformation) {
      if (carrierInformationElement.getName().equals("testCarrier1")) {
        ...
        Assert.assertEquals(
            0, carrierInformationElement.getFixedNumberOfVehiclePerTypeAndLocation());
        ...
        Assert.assertTrue(
            carrierInformationElement.getVehicleDepots().contains("i(2,0)")
        ...
        Assert.assertTrue(
            carrierInformationElement.getVehicleTypes()[0].equals("testVehicle1")
                || carrierInformationElement.getVehicleTypes()[0].equals("testVehicle2"));
        Assert.assertNotEquals(
            carrierInformationElement.getVehicleTypes()[0],
            carrierInformationElement.getVehicleTypes()[1]);
        ...
        Assert.assertNull(carrierInformationElement.getAreaOfAdditionalDepots());
        Assert.assertEquals(
            3, carrierInformationElement.getFixedNumberOfVehiclePerTypeAndLocation());
