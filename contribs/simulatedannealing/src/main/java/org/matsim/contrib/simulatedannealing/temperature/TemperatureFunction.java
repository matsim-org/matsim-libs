...
/**
 * The temperature function calculates the simulated annealing temperature that influences the
 * acceptance probability of a new solution in the default setup.
 *
 * <p>Contains multiple default implementations.
 *
  ...
  enum DefaultFunctions implements TemperatureFunction {
        ...
        return finalTemperature
            + (initialTemperature - finalTemperature)
                * (1
                    / (1
                        + Math.exp(
                            (2 * Math.log(initialTemperature - finalTemperature) / cycles)
                                    * iteration
                                - 0.5 * cycles)));
      }
    },
    linearAdditive {
      ...
      public double getTemperature(
          ...
          double bestCost) {
        return finalTemperature
            + (initialTemperature - finalTemperature) * ((double) (cycles - iteration) / cycles);
      }
    },
    quadraticAdditive {
      @Override
      public double getTemperature(
        ...
        return finalTemperature
            + (initialTemperature - finalTemperature)
                * Math.pow(((double) (cycles - iteration) / cycles), 2);
      }
    },
    trigonometricAdditive {
      ...
      public double getTemperature(
        ...
        return finalTemperature
            + 0.5
                * (initialTemperature - finalTemperature)
                * (1 + Math.cos((iteration * Math.PI) / cycles));
      }
    }
  }
}
