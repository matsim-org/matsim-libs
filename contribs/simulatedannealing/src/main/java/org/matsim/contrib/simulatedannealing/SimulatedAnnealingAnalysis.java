...
public final class SimulatedAnnealingAnalysis<T> implements IterationStartsListener {
  ...
  public SimulatedAnnealingAnalysis(
      Config config, MatsimServices matsimServices, SimulatedAnnealing<T> simulatedAnnealing) {
  ...
  public void notifyIterationStarts(IterationStartsEvent event) {
    if (simulatedAnnealing.getCurrentState().isPresent()) {
      SimulatedAnnealing.SimulatedAnnealingIteration<T> lastIteration =
          simulatedAnnealing.getCurrentState().get();
      String summarizeAnnealing =
  ...
  private void writeIterationAnnealingStats(String summarizeAnnealing, int it) {
    try (var bw =
        getAppendingBufferedWriter(
            "simulatedAnnealing_" + simulatedAnnealing.get().getClass().getSimpleName(), ".csv")) {
      if (!headerWritten) {
        headerWritten = true;
        bw.write(
            line("runId", "iteration", "acceptedCost", "currentCost", "bestCost", "temperature"));
      }
