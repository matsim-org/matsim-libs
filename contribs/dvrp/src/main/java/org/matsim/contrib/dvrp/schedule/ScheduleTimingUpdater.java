...
public class ScheduleTimingUpdater {
  ...
  public static final double REMOVE_STAY_TASK = Double.NEGATIVE_INFINITY;

  public ScheduleTimingUpdater(
      MobsimTimer timer, StayTaskEndTimeCalculator stayTaskEndTimeCalculator) {
    this.timer = timer;
    this.stayTaskEndTimeCalculator = stayTaskEndTimeCalculator;
  }

  /**
   * This method should be called inside {@code VrpOptimizer.nextTask()} before anything else is
   * done. Check and decide if the schedule should be updated due to if vehicle is Update timings
   * (i.e. beginTime and endTime) of all tasks in the schedule.
   */
  ...
  public void updateTimings(DvrpVehicle vehicle) {
    ...
    double predictedEndTime =
        TaskTrackers.predictEndTime(schedule.getCurrentTask(), timer.getTimeOfDay());
  ...
  public void updateTimingsStartingFromTaskIdx(
      DvrpVehicle vehicle, int startIdx, double newBeginTime) {
    Schedule schedule = vehicle.getSchedule();
