...
public class GroupPassengerEngine implements PassengerEngine, PassengerRequestRejectedEventHandler {
  ...
  private final Queue<PassengerRequestRejectedEvent> rejectedRequestsEvents =
  ...
  GroupPassengerEngine(
      ...
      PassengerGroupIdentifier groupIdentifier) {
    this.mode = mode;
  ...
  public void doSimStep(double time) {
    ...
    while (!rejectedRequestsEvents.isEmpty()) {
      List<MobsimPassengerAgent> passengers =
          activePassengers.remove(rejectedRequestsEvents.poll().getRequestId());
    ...
    for (List<MobsimPassengerAgent> group : agentGroups.values()) {
      ...
      PassengerRequest request =
          requestCreator.createRequest(
              internalPassengerHandling.createRequestId(),
              group.stream().map(Identifiable::getId).toList(),
              route,
              getLink(firstAgent.getCurrentLinkId()),
              getLink(toLinkId),
              now,
              now);
      ...
      eventsManager.processEvent(
          new PassengerWaitingEvent(
              now, mode, request.getId(), group.stream().map(Identifiable::getId).toList()));
  ...
  private void validateAndSubmitRequest(
      List<MobsimPassengerAgent> passengers, PassengerRequest request, double now) {
   ...
   *
   * <p>- (1) The stop activity tries to pick up a passenger and receives whether the pickup
   * succeeded or not (see tryPickUpPassenger). In the classic implementation, the vehicle only
   * calls tryPickUpPassenger at the time when it actually wants to pick up the person (at the end
   * of the activity). It may happen that the person is not present yet. In that case, the pickup
   * request is saved and notifyPassengerReady is called on the stop activity upen departure of the
   * agent.
   *
   * <p>- (2) If pickup and dropoff times are handled more flexibly by the stop activity, it might
   * want to detect whether an agent is ready to be picked up, then start an "interaction time" and
   * only after perform the actual pickup. For that purpose, we have queryPickUpPassenger, which
   * indicates whether the agent is already there, and, if not, makes sure that the stop activity is
   * notified once the agent arrives for departure.
   */
  @Override
  public boolean notifyWaitForPassengers(
  ...
  @Override
  public boolean tryPickUpPassengers(
      PassengerPickupActivity pickupActivity,
      MobsimDriverAgent driver,
      Id<Request> requestId,
      double now) {
    boolean pickedUp =
        internalPassengerHandling.tryPickUpPassengers(
            driver, activePassengers.get(requestId), requestId, now);
  ...
  public void dropOffPassengers(MobsimDriverAgent driver, Id<Request> requestId, double now) {
    internalPassengerHandling.dropOffPassengers(
        driver, activePassengers.remove(requestId), requestId, now);
  }
