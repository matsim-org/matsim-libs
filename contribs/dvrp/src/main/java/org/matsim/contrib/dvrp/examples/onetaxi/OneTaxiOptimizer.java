...
public final class OneTaxiOptimizer implements VrpOptimizer {
  ...
  @Inject
  public OneTaxiOptimizer(
      EventsManager eventsManager,
      @DvrpMode(TransportMode.taxi) Network network,
      @DvrpMode(TransportMode.taxi) Fleet fleet,
      MobsimTimer timer) {
  ...
  public void requestSubmitted(Request request) {
    ...
    double t0 =
        schedule.getStatus() == ScheduleStatus.UNPLANNED
            ? Math.max(vehicle.getServiceBeginTime(), currentTime)
            : Schedules.getLastTask(schedule).getEndTime();

    VrpPathWithTravelData pathToCustomer =
        VrpPaths.calcAndCreatePath(lastTask.getLink(), fromLink, t0, router, travelTime);
    ...
    VrpPathWithTravelData pathWithCustomer =
        VrpPaths.calcAndCreatePath(fromLink, toLink, t2, router, travelTime);
  ...
  /**
   * Simplified version. For something more advanced, see {@link
   * org.matsim.contrib.taxi.scheduler(DvrpVehicle)} in the taxi contrib
   */
