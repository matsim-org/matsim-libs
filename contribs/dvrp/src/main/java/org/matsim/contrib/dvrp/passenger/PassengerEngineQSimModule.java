...
/**
 * dvrp needs {@link PassengerHandler} bound to something. It is first bound to a {@link
 * PassengerEngine} so it can also do stuff on its own. Then, {@link PassengerEngine} is bound to
  ...
  protected void configureQSim() {
    ...
    switch (type) {
      ...
      case WITH_PREBOOKING -> {
        addModalComponent(
            PassengerEngine.class, PassengerEngineWithPrebooking.createProvider(getMode()));
        return;
      }
      case TELEPORTING -> {
        addModalComponent(
            PassengerEngine.class, TeleportingPassengerEngine.createProvider(getMode()));
        return;
      }
      case WITH_GROUPS -> {
