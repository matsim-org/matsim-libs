...
public class OneToManyPathSearch {
  public static OneToManyPathSearch createSearch(
      SpeedyGraph graph,
      IdMap<Node, Node> nodeMap,
      TravelTime travelTime,
      TravelDisutility travelDisutility,
      boolean lazyPathCreation) {
    return new OneToManyPathSearch(
        nodeMap,
        new LeastCostPathTree(graph, travelTime, travelDisutility),
        travelTime,
        lazyPathCreation);
  }
  ...
  public static class PathData {
    ...
    public static final PathData INFEASIBLE =
        new PathData(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
    ...
    public PathData(Path path, double firstAndLastLinkTT) {
      ...
      this.path =
          new Path(
              path.nodes != null ? ImmutableList.copyOf(path.nodes) : null,
              ImmutableList.copyOf(path.links),
              path.travelTime,
              path.travelCost);
  ...
  public PathData[] calcPathDataArray(
      Link fromLink, List<Link> toLinks, double startTime, boolean forward) {
    return calcPathDataArray(fromLink, toLinks, startTime, forward, Double.POSITIVE_INFINITY);
  }

  public PathData[] calcPathDataArray(
      Link fromLink, List<Link> toLinks, double startTime, boolean forward, double maxTravelTime) {
    OneToManyPathCalculator pathConstructor =
        new OneToManyPathCalculator(
            nodeMap, dijkstraTree, travelTime, forward, fromLink, startTime);
    pathConstructor.calculateDijkstraTree(toLinks, maxTravelTime);
    return createPathDataArray(toLinks, pathConstructor);
  }

  public Map<Link, PathData> calcPathDataMap(
      Link fromLink, Collection<Link> toLinks, double startTime, boolean forward) {
    return calcPathDataMap(fromLink, toLinks, startTime, forward, Double.POSITIVE_INFINITY);
  }

  public Map<Link, PathData> calcPathDataMap(
      Link fromLink,
      Collection<Link> toLinks,
      double startTime,
      boolean forward,
      double maxTravelTime) {
    OneToManyPathCalculator pathCalculator =
        new OneToManyPathCalculator(
            nodeMap, dijkstraTree, travelTime, forward, fromLink, startTime);
  ...
  private PathData[] createPathDataArray(
      List<Link> toLinks, OneToManyPathCalculator pathCalculator) {
  ...
  private Map<Link, PathData> createPathDataMap(
      Collection<Link> toLinks, OneToManyPathCalculator pathCalculator) {
