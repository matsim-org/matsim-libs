...
public class FleetModule extends AbstractDvrpModeModule {
  ...
  public FleetModule(
      String mode, URL fleetSpecificationUrl, boolean updateVehicleStartLinkToLastLink) {
    super(mode);
  ...
  public void install() {
    // 3 options:
    // - vehicle specifications provided in a separate XML file
    // (http://matsim.org/files/dtd/dvrp_vehicles_v1.dtd)
    // - vehicle specifications derived from the "standard" matsim vehicles (only if they are read
    // from a file,
          ...
          .toProvider(
              new Provider<>() {
                ...
                public FleetSpecification get() {
                  return DvrpVehicleSpecificationWithMatsimVehicle
                      .createFleetSpecificationFromMatsimVehicles(getMode(), vehicles);
                }
              })
          .asEagerSingleton();
    }
          ...
          protected void configureQSim() {
            bindModal(Fleet.class)
                .toProvider(
                    modalProvider(
                        getter ->
                            Fleets.createDefaultFleet(
                                getter.getModal(FleetSpecification.class),
                                getter.getModal(Network.class).getLinks()::get)))
                .asEagerSingleton();
          }
        });

    if (updateVehicleStartLinkToLastLink) {
      bindModal(VehicleStartLinkToLastLinkUpdater.class)
          .toProvider(
              modalProvider(
                  getter ->
                      new VehicleStartLinkToLastLinkUpdater(
                          getter.getModal(FleetSpecification.class),
    ...
    bindModal(FleetControlerListener.class)
        .toProvider(
            modalProvider(
                getter ->
                    new FleetControlerListener(
                        getMode(),
