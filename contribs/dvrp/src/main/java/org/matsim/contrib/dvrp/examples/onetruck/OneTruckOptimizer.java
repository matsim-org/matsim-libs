...
final class OneTruckOptimizer implements VrpOptimizer {
  ...
  @Inject
  public OneTruckOptimizer(
      @DvrpMode(TransportMode.truck) Network network,
      @DvrpMode(TransportMode.truck) Fleet fleet,
      MobsimTimer timer) {
    this.timer = timer;
   ...
   * org.matsim.contrib.taxi.scheduler.TaxiScheduler#updateBeforeNextTask(DvrpVehicle)} in the taxi
   * contrib
   */
