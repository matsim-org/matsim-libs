...
public class DvrpConfigGroup extends ReflectiveConfigGroupWithConfigurableParameterSets {
  ...
  @Comment(
      "Mode of which the network will be used for throwing events and hence calculating travel times. "
          + "Default is car.")
  @NotBlank
  public String mobsimMode =
      TransportMode.car; // used for events throwing and thus calculating travel times, etc.
  ...
  // In DVRP 'time < currentTime' may only happen for backward path search, a adding proper search
  // termination
  ...
  @Nullable private DvrpTravelTimeMatrixParams travelTimeMatrixParams;
  ...
  private void initSingletonParameterSets() {
    ...
    addDefinition(
        DvrpTravelTimeMatrixParams.SET_NAME,
        DvrpTravelTimeMatrixParams::new,
        () -> travelTimeMatrixParams,
        params -> travelTimeMatrixParams = (DvrpTravelTimeMatrixParams) params);
  }
  ...
  protected void checkConsistency(Config config) {
    ...
    if (!config.qsim().isInsertingWaitingVehiclesBeforeDrivingVehicles()) {
      ...
      // The latter expectation is assumes that departing vehicles must be inserted before driving
      // ones
      // (though that still does not guarantee 1-second stay since the vehicle may need to wait if
      // the next
      // link is fully congested)
      log.warn(
          " 'QSim.insertingWaitingVehiclesBeforeDrivingVehicles' should be true in order to get"
