...
public final class SparseMatrix {
    ...
    private Bucket(List<NodeAndTime> nodeAndTimes) {
      ...
      for (int i = 0; i < nodeAndTimeArray.length; i++) {
        ...
        Preconditions.checkArgument(
            Double.isFinite(e.time) && e.time >= 0 && e.time < MAX_UNSIGNED_SHORT);
  ...
  static final class SparseRow {
    // 64-128 seemed to work best when micro-benchmarking the berlin network from the robo-taxi
    // papers
    // for a bigger neighbourhood (maxNeighborDistance = 4000). On average, there is 620
    // neighbouring nodes
    // (min 2; max 1941).
    // Interestingly, the old version (not using buckets) was minimally worse (the tests included
    // all nodes,
    // so in most of the cases from-to nodes were not neighbours and get filtered out by the bit
    // set)
  ...
  int get(int fromNode, int toNode) {
    ...
    return row != null
        ? row.get(toNode) // get the value from the selected row
