...
public class DvrpBenchmarkConfigConsistencyChecker implements ConfigConsistencyChecker {
  private static final Logger log =
      LogManager.getLogger(DvrpBenchmarkConfigConsistencyChecker.class);
  ...
  public void checkConsistency(Config config) {
    ...
    if (config.qsim().getFlowCapFactor() < 100) {
      log.warn(
          "FlowCapFactor should be large enough (e.g. 100) to obtain deterministic (fixed) travel times");
    }

    if (config.qsim().getStorageCapFactor() < 100) {
      log.warn(
          "StorageCapFactor should be large enough (e.g. 100) to obtain deterministic (fixed) travel times");
    }

    if (config.network().isTimeVariantNetwork()
        && !DvrpConfigGroup.get(config).networkModes.isEmpty()) {
      throw new RuntimeException(
          "The current version of RunTaxiBenchmark does not support this case: "
