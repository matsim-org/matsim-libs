...
public final class DynAgent implements MobsimDriverPassengerAgent {
  ...
  public DynAgent(
      Id<Person> id, Id<Link> startLinkId, EventsManager events, DynAgentLogic agentLogic) {
    this.id = id;
  ...
  private void computeNextAction(DynAction oldDynAction, double now) {
    ...
    if (nextDynAction instanceof DynActivity) {
      ...
      state = MobsimAgent.State.ACTIVITY;
      events.processEvent(
          new ActivityStartEvent(now, id, currentLinkId, null, dynActivity.getActivityType()));
    } else {
  ...
  public boolean getEnterTransitRoute(
    ...
    return ((PTPassengerDynLeg) dynLeg)
