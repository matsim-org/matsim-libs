...
public final class OneTruckRequestCreator implements MobsimAfterSimStepListener {
  ...
  private final PriorityQueue<OneTruckRequest> requests =
      new PriorityQueue<>(10, Comparator.comparing(Request::getSubmissionTime));

  @Inject
  public OneTruckRequestCreator(
      @DvrpMode(TransportMode.truck) VrpOptimizer optimizer,
  ...
  private OneTruckRequest createRequest(
      String requestId, String fromLinkId, String toLinkId, double time, Network network) {
    return new OneTruckRequest(
        Id.create(requestId, Request.class),
        network.getLinks().get(Id.createLinkId(fromLinkId)),
        network.getLinks().get(Id.createLinkId(toLinkId)),
        time);
  }
