...
public class TripPrebookingManager implements MobsimInitializedListener, MobsimBeforeSimStepListener {
	private static class PrebookingEntry {
		...
		private PrebookingEntry(double submissionTime, MobsimPassengerAgent passenger, Id<Link> fromLinkId,
	...
	{
		PrebookingEntry prebooking = new PrebookingEntry(submissionTime, passenger, fromLinkId, toLinkId,
				departureTime);
	...
	private void prebookTrips(double now) {
		while (prebookingQueue.peek().submissionTime <= now) {
			...
			passengerEngine.prebookTrip(now, pe.passenger, pe.fromLinkId, pe.toLinkId, pe.departureTime);
		}
	}
}
