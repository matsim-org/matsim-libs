...
public class TeleportingPassengerEngine implements PassengerEngine, VisData {
  ...
  private final Queue<Pair<Double, PassengerRequest>> teleportedRequests =
      new PriorityQueue<>(Comparator.comparingDouble(Pair::getLeft));
  ...
  TeleportingPassengerEngine(
      ...
      TeleportedRouteCalculator teleportedRouteCalculator,
      Network network,
  ...
  public boolean handleDeparture(double now, MobsimAgent agent, Id<Link> fromLinkId) {
    ...
    eventsManager.processEvent(
        new PassengerWaitingEvent(now, mode, request.getId(), request.getPassengerIds()));
    ...
    if (internalPassengerHandling.validateRequest(request, requestValidator, now)) {
      ...
      eventsManager.processEvent(
          new PassengerPickedUpEvent(now, mode, request.getId(), passenger.getId(), null));
      teleportationEngine.handleDeparture(now, passenger, fromLinkId);
      teleportedRequests.add(
          ImmutablePair.of(now + teleportedRoute.getTravelTime().seconds(), request));
    } else {
  ...
  private Route adaptLegRouteForTeleportation(
    ...
    for (MobsimPassengerAgent passenger : passengers) {
      Leg leg =
          (Leg)
              WithinDayAgentUtils.getCurrentPlanElement(
                  passenger); // side effect: makes the plan modifiable
    ...
    eventsManager.processEvent(
        new PassengerRequestScheduledEvent(
            ...
            null,
            now,
            now + teleportedRoute.getTravelTime().seconds()));
  ...
  public static Provider<PassengerEngine> createProvider(String mode) {
    return new ModalProviders.AbstractProvider<>(mode, DvrpModes::mode) {
      ...
      public TeleportingPassengerEngine get() {
        return new TeleportingPassengerEngine(
            getMode(),
            eventsManager,
            mobsimTimer,
