 ...
 *
 * <p>Handles one-of-many cases where a parameter set has different implementation classes (e.g.
 * taxi/drt config groups contain exactly one optimiser-specific parameter set, but that set can be
 * a subclass of a common parent class)
 *
 * @author Michal Maciejewski (michalm)
 */
public abstract class ReflectiveConfigGroupWithConfigurableParameterSets
  ...
  private static class Definition<T extends ConfigGroup> {
    ...
    public Definition(
        Supplier<T> creator, Supplier<ConfigGroup> getter, Consumer<ConfigGroup> setter) {
  ...
  protected <T extends ConfigGroup> void addDefinition(
      String type,
      Supplier<T> creator,
      Supplier<ConfigGroup> getter,
      Consumer<ConfigGroup> setter) {
    var oldDefinition = definitions.put(type, new Definition<>(creator, getter, setter));
    Preconditions.checkState(
        oldDefinition == null, "Parameter set for type (%s) already defined", type);
  }
  ...
  public final ConfigGroup createParameterSet(String type) {
    ...
    Verify.verify(
        params.getName().equals(type),
        "The created parameter set has type (%s) instead of (%s)",
        params.getName(),
        type);
    return params;
  }
  ...
  public final void addParameterSet(ConfigGroup set) {
    ...
    if (existingParams != null) {
      throw new IllegalStateException(
          String.format(
              "Remove the existing parameter set of type (%s) before adding a new one of type (%s)",
  ...
  private Definition<?> definition(String type) {
    return Preconditions.checkNotNull(
        definitions.get(type), "Unsupported parameter set type (%s)", type);
  }
}
