...
public final class TravelTimeMatrices {
  ...
  public static Matrix calculateTravelTimeMatrix(
      RoutingParams params, Map<Zone, Node> centralNodes, double departureTime) {
    Matrix travelTimeMatrix = new Matrix(centralNodes.keySet());
    Calculation<Zone> calculation =
        (lcpTree, z) ->
            computeForDepartureZone(z, centralNodes, departureTime, travelTimeMatrix, lcpTree);
    calculate(params, centralNodes.keySet(), calculation, "DVRP free-speed TT matrix: zone ");
    return travelTimeMatrix;
  }

  private static void computeForDepartureZone(
      Zone fromZone,
      Map<Zone, Node> centralNodes,
  ...
  public static Optional<SparseMatrix> calculateTravelTimeSparseMatrix(
      RoutingParams params, double maxDistance, double maxTravelTime, double departureTime) {
    ...
    Calculation<Node> calculation =
        (lcpTree, n) ->
            computeForDepartureNode(
                n, nodes, departureTime, travelTimeMatrix, lcpTree, maxDistance, maxTravelTime);
    calculate(params, nodes, calculation, counter);
    return Optional.of(travelTimeMatrix);
  }

  private static void computeForDepartureNode(
      Node fromNode,
    ...
    lcpTree.calculate(
        fromNode.getId().index(),
        departureTime,
        null,
        null,
        (nodeIndex, arrivalTime, travelCost, distance, departTime) ->
            distance >= maxDistance && arrivalTime >= departTime + maxTravelTime);
    ...
    for (Node toNode : nodes) {
      ...
      // these values may not the actual minimum, because many nodes may not get fully relaxed (due
  ...
  private static <E> void calculate(
      RoutingParams params,
      Collection<? extends E> elements,
      Calculation<E> calculation,
      String counterPrefix) {
    var trees =
        IntStream.range(0, params.numberOfThreads)
            .mapToObj(
                i ->
                    new LeastCostPathTree(
                        new SpeedyGraph(params.routingNetwork),
                        params.travelTime,
                        params.travelDisutility))
            .toList();
