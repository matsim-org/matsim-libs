 ...
 * DvrpOfflineTravelTimeEstimator. The beta coefficient is provided in seconds and should be either
 * 0 (no online estimation) or positive (mixed online-offline estimation). If beta is sufficiently
 * large, 'beta >> 0', only the currently observed TT is used
 *
 * @author michalm
 */
public class DvrpOnlineTravelTimeEstimator
  ...
  @Inject
  public DvrpOnlineTravelTimeEstimator(
      WithinDayTravelTime withinDayTT,
      DvrpOfflineTravelTimeEstimator offlineTTEstimator,
  ...
  public double getLinkTravelTime(Link link, double time, Person person, Vehicle vehicle) {
    ...
    // XXX Alternatively:
    // double currentOfflineTT = offlineTTEstimator.getLinkTravelTime(link, currentTime, person,
    // vehicle);
