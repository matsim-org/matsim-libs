...
public class AdaptiveTravelTimeMatrixImpl implements AdaptiveTravelTimeMatrix {
  ...
  public AdaptiveTravelTimeMatrixImpl(
      double maxTime,
      Network dvrpNetwork,
      DvrpTravelTimeMatrixParams params,
      TravelTimeMatrix freeSpeedMatrix,
      double alpha) {
    this.alpha = alpha;
    this.numberOfBins = numberOfBins(maxTime);
    this.gridSystem = new SquareGridSystem(dvrpNetwork.getNodes().values(), params.cellSize);
    this.centralNodes =
        ZonalSystems.computeMostCentralNodes(dvrpNetwork.getNodes().values(), this.gridSystem);
    this.timeDependentMatrix =
        IntStream.range(0, numberOfBins).mapToObj(i -> new Matrix(centralNodes.keySet())).toList();
    this.params = params;
          ...
          .forEach(
              originZoneEntry -> {
                for (Entry<Zone, Node> destinationZoneEntry : centralNodes.entrySet()) {
                  ...
                  if (DistanceUtils.calculateSquaredDistance(
                          originNode.getCoord(), destinationNode.getCoord())
                      < (params.maxNeighborDistance * params.maxNeighborDistance)) {
                    SparseTravelTimeKey key =
                        getSparseTravelTimeKey(originNode, destinationNode, bin);
                    double freeSpeedTravelTime =
                        freeSpeedMatrix.getTravelTime(originNode, destinationNode, Double.NaN);
              ...
              originZoneEntry -> {
                for (Entry<Zone, Node> destinationZoneEntry : centralNodes.entrySet()) {
                  ...
                  this.timeDependentMatrix
                      .get(bin)
                      .set(
                          originZoneEntry.getKey(),
                          destinationZoneEntry.getKey(),
                          freeSpeedTravelTime);
                }
              });
    }
  ...
  public double getTravelTime(Node fromNode, Node toNode, double departureTime) {
    ...
    Double sparseValue =
        this.sparseTravelTimeCache.get(this.getSparseTravelTimeKey(fromNode, toNode, bin));
    if (sparseValue != null) {
      return sparseValue;
    }
    return this.timeDependentMatrix
        .get(bin)
        .get(this.gridSystem.getZone(fromNode), this.gridSystem.getZone(toNode));
  }
  ...
  public void setTravelTime(
      Node fromNode, Node toNode, double routeEstimate, double departureTime) {
    int bin = this.getBin(departureTime);
    ...
    if (sparseValue != null) {
      ...
      this.timeDependentMatrix
          .get(bin)
          .set(this.gridSystem.getZone(fromNode), this.gridSystem.getZone(toNode), value);
    }
  }
