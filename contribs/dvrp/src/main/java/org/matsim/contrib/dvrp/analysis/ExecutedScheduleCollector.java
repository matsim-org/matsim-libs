...
public class ExecutedScheduleCollector implements TaskStartedEventHandler, TaskEndedEventHandler {
  ...
  public final void handleEvent(TaskStartedEvent startEvent) {
    ...
    Preconditions.checkState(
        previousEvent == null,
        "No end event reported for previous task start event: (%s).",
        previousEvent);
  }
  ...
  public final void handleEvent(TaskEndedEvent endEvent) {
    ...
    Preconditions.checkNotNull(
        startEvent, "No start event reported for task end event: (%s).", startEvent);
    Preconditions.checkArgument(
        startEvent.getTaskType().equals(endEvent.getTaskType()),
        "Start event (%s) and end event (%s) refer to tasks of different types",
        startEvent,
        endEvent);

    executedSchedules
        .computeIfAbsent(endEvent.getDvrpVehicleId(), ExecutedSchedule::new) //
        .executedTasks
        .add(createTask(startEvent, endEvent));
  }

  /**
   * This method is meant for overriding in order to enrich created ExecutedTasks with some
   * additional info.
   *
   * <p>In order to do so, we often need to listen to other events. Given Matsim's default
   * implementation of parallel event handling, there is no guarantee that two distinct event
   * handler objects are processing events in the same thread, therefore overriding is simpler than
   * delegation.
   */
  protected ExecutedTask createTask(TaskStartedEvent taskStarted, TaskEndedEvent taskEnded) {
    return new ExecutedTask(
        taskStarted.getTaskType(),
        taskStarted.getTime(),
        taskEnded.getTime(),
        taskStarted.getLinkId(),
        taskEnded.getLinkId());
  }
