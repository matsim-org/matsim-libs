...
public class VrpPaths {
  ...
  public static VrpPathWithTravelData calcAndCreatePath(
      Link fromLink,
      Link toLink,
    ...
    Path path =
        fromLink == toLink
            ...
            : router.calcLeastCostPath(
                fromLink.getToNode(),
                toLink.getFromNode(),
                departureTime + FIRST_LINK_TT,
                null,
                null);
    return VrpPaths.createPath(fromLink, toLink, departureTime, path, travelTime);
  }

  public static VrpPathWithTravelData calcAndCreatePathForDiversion(
      LinkTimePair diversionPoint,
      Link toLink,
      LeastCostPathCalculator router,
      TravelTime travelTime) {
    Path path =
        diversionPoint.link == toLink
            ? null
            : router.calcLeastCostPath(
                diversionPoint.link.getToNode(),
                toLink.getFromNode(),
                diversionPoint.time,
                null,
                null);
  ...
  public static VrpPathWithTravelData createZeroLengthPath(
      Link fromTolink, double departureTime, boolean calculateForDiversion) {
    ...
    return new VrpPathWithTravelDataImpl(
        departureTime, travelTime, new Link[] {fromTolink}, new double[] {0});
  }

  public static VrpPathWithTravelData createZeroLengthPathForDiversion(
      LinkTimePair diversionPoint) {
    return createZeroLengthPath(diversionPoint.link, diversionPoint.time, true);
  }

  public static VrpPathWithTravelData createPath(
      Link fromLink, Link toLink, double departureTime, PathData pathData, TravelTime travelTime) {
    return createPath(fromLink, toLink, departureTime, pathData.getPath(), travelTime);
  }

  public static VrpPathWithTravelData createPathForDiversion(
      LinkTimePair diversionPoint, Link toLink, PathData pathData, TravelTime travelTime) {
    return createPathForDiversion(diversionPoint, toLink, pathData.getPath(), travelTime);
  }

  public static VrpPathWithTravelData createPath(
      Link fromLink, Link toLink, double departureTime, Path path, TravelTime travelTime) {
    return createPath(fromLink, toLink, departureTime, path, travelTime, true, false);
  }

  public static VrpPathWithTravelData createPathForDiversion(
      LinkTimePair diversionPoint, Link toLink, Path path, TravelTime travelTime) {
    return createPath(
        diversionPoint.link, toLink, diversionPoint.time, path, travelTime, true, true);
  }
  ...
  public static VrpPathWithTravelData createPath(
      ...
      Path path,
      TravelTime travelTime,
      boolean considerZeroLengthPath,
    ...
    if (count > 0) {
      Preconditions.checkArgument(
          fromLink.getToNode() == path.links.get(0).getFromNode(),
          "fromLink and path are not connected. From link: %s. First link in path: %s",
          fromLink,
          path.links.get(0));
      Preconditions.checkArgument(
          path.links.get(count - 1).getToNode() == toLink.getFromNode(),
          "path and toLink are not connected. To link: %s. Last link in path: %s",
          toLink,
