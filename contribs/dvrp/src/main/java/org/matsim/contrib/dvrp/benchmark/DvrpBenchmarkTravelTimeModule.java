...
public class DvrpBenchmarkTravelTimeModule extends AbstractModule {
  ...
  public void install() {
    if (dvrpCfg.initialTravelTimesFile != null) {
      addTravelTimeBinding(DvrpTravelTimeModule.DVRP_ESTIMATED)
          .toProvider(
              () -> {
                URL url =
                    ConfigGroup.getInputFileURL(
                        getConfig().getContext(), dvrpCfg.initialTravelTimesFile);
                var timeDiscretizer = new TimeDiscretizer(getConfig().travelTimeCalculator());
                var linkTravelTimes =
                    DvrpOfflineTravelTimes.loadLinkTravelTimes(timeDiscretizer, url);
                return DvrpOfflineTravelTimes.asTravelTime(timeDiscretizer, linkTravelTimes);
              })
          .asEagerSingleton();
    } else {
      addTravelTimeBinding(DvrpTravelTimeModule.DVRP_ESTIMATED)
          .to(QSimFreeSpeedTravelTime.class)
          .asEagerSingleton();
    }

    // Because TravelTimeCalculatorModule is not installed for benchmarking, we need to add a
    // binding
    // for the car mode
    addTravelTimeBinding(TransportMode.car)
