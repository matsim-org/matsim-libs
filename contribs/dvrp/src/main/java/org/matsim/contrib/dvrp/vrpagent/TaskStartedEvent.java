...
public class TaskStartedEvent extends AbstractTaskEvent {
  ...
  public TaskStartedEvent(
      double time, String dvrpMode, Id<DvrpVehicle> dvrpVehicleId, Id<Person> driverId, Task task) {
    this(
        time,
        dvrpMode,
        dvrpVehicleId,
  ...
  public static TaskStartedEvent convert(
      GenericEvent event, Function<String, Task.TaskType> taskTypeFunction) {
    Map<String, String> attributes = event.getAttributes();
    double time = Double.parseDouble(attributes.get(ATTRIBUTE_TIME));
    String mode = Objects.requireNonNull(attributes.get(ATTRIBUTE_DVRP_MODE));
    Id<DvrpVehicle> vehicleId =
        Id.create(attributes.get(ATTRIBUTE_DVRP_VEHICLE), DvrpVehicle.class);
    Id<Person> driverId = Id.createPersonId(attributes.get(ATTRIBUTE_PERSON));
    Task.TaskType taskType =
        Objects.requireNonNull(taskTypeFunction.apply(attributes.get(ATTRIBUTE_TASK_TYPE)));
