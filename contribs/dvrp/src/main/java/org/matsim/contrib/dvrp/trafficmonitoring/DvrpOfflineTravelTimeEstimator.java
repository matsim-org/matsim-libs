...
/**
 * Used for offline estimation of travel times for VrpOptimizer by means of the exponential moving
 * average. The weighting decrease, alpha, must be in (0,1]. We suggest small values of alpha, e.g.
 * 0.05.
 *
 * <p>The averaging starts from the initial travel time estimates. If not provided, the free-speed
 * TTs is used as the initial estimates
 *
 * @author michalm
 */
public class DvrpOfflineTravelTimeEstimator
  ...
  public DvrpOfflineTravelTimeEstimator(
      @Named(DvrpTravelTimeModule.DVRP_INITIAL) TravelTime initialTT,
    ...
    this(
        ...
        observedTT,
        network,
        new TimeDiscretizer(ttCalcConfig),
        dvrpConfig.travelTimeEstimationAlpha,
        outputDirectoryHierarchy);
  }

  public DvrpOfflineTravelTimeEstimator(
      TravelTime initialTT,
      TravelTime observedTT,
      Network network,
    ...
    linkTravelTimes =
        DvrpOfflineTravelTimes.convertToLinkTravelTimeMatrix(
            initialTT, network.getLinks().values(), timeDiscretizer);
  }
  ...
  public void notifyAfterMobsim(AfterMobsimEvent event) {
    DvrpOfflineTravelTimes.saveLinkTravelTimes(
        ...
        outputDirectoryHierarchy.getIterationFilename(
            event.getIteration(), "dvrp_travel_times.csv.gz"));
  }
