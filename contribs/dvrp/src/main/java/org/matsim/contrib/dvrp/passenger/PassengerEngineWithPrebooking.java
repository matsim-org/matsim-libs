...
public final class PassengerEngineWithPrebooking
  ...
  PassengerEngineWithPrebooking(
      String mode,
      EventsManager eventsManager,
      MobsimTimer mobsimTimer,
      PreplanningEngine preplanningEngine,
  ...
  public final List<TripInfo> getTripInfos(TripInfo.Request tripInfoRequest) {
    // idea is to be able to return multiple trip options, cf. public transit router.  In the case
    // here, we will need only one.  I.e. goals of this method are:

    // (1) fill out TripInfo

    // (2) possibly keep handle so that passenger can accept, or not-confirmed request is eventually
    // deleted again.  Also see
    // doSimStep(...).  (In reality, it seems that real trips are not planned until the offer is
    // accepted, which means that one does not
    // need to keep a handle.)

    Gbl.assertIf(
        tripInfoRequest.getTimeInterpretation() == TripInfo.Request.TimeInterpretation.departure);
    ...
    // for the time being: generating TripInfo object that will be returned to the potential
    // passenger:
    return ImmutableList.of(
        new DvrpTripInfo(
            mode, pickupLink, dropoffLink, tripInfoRequest.getTime(), now, tripInfoRequest, this));
  }
  ...
  public void bookTrip(MobsimPassengerAgent passenger, TripInfoWithRequiredBooking tripInfo) {
    // this is the handle by which the passenger can accept.  This would, we think, easiest go to a
    // container that keeps track of unconfirmed
    // offers.  We cannot say if advanceRequestStorage is the correct container for this, probably
    // not and you will need yet another one.
    ...
    PassengerRequest request =
        requestCreator.createRequest(
            ...
            tripInfo.getOriginalRequest().getPlannedRoute(),
            getLink(tripInfo.getPickupLocation().getLinkId()),
            getLink(tripInfo.getDropoffLocation().getLinkId()),
            tripInfo.getExpectedBoardingTime(),
            now);
  ...
  public boolean handleDeparture(double now, MobsimAgent agent, Id<Link> fromLinkId) {
    ...
    List<PassengerRequest> prebookedRequests =
        removeRequests(passenger.getId(), fromLinkId, passenger.getDestinationLinkId());

    Preconditions.checkState(
        prebookedRequests.size() == 1,
        "Currently only one request is allowed for the same from-to link pair");
    ...
    eventsManager.processEvent(
        new PassengerWaitingEvent(
            now, mode, prebookedRequest.getId(), prebookedRequest.getPassengerIds()));
  ...
  private List<PassengerRequest> removeRequests(
      Id<Person> passengerId, Id<Link> fromLinkId, Id<Link> toLinkId) {
    Collection<PassengerRequest> allRequestsForThisPassenger = advanceRequests.get(passengerId);
    List<PassengerRequest> filteredRequests =
        advanceRequests.get(passengerId).stream()
            .filter(
                r ->
                    r.getFromLink().getId().equals(fromLinkId)
                        && r.getToLink().getId().equals(toLinkId))
  ...
  // TODO have not decided yet how VrpOptimizer determines if request is prebooked, maybe 'boolean
  // prebooked'??
  ...
  public boolean tryPickUpPassengers(
    ...
    if (!internalPassengerHandling.tryPickUpPassengers(
        driver, List.of(passenger), requestId, now)) {
  ...
  public void dropOffPassengers(MobsimDriverAgent driver, Id<Request> requestId, double now) {
    internalPassengerHandling.dropOffPassengers(
        driver, List.of(activeRequests.remove(requestId).passenger), requestId, now);
  }

  // ================ REJECTED/SCHEDULED EVENTS

  private final Queue<PassengerRequestRejectedEvent> rejectedEvents = new ConcurrentLinkedQueue<>();
  private final Queue<PassengerRequestScheduledEvent> scheduledEvents =
  ...
  private void processRequestRejectedEvents(PassengerRequestRejectedEvent event) {
    ...
    advanceRequests
        .get(requestEntry.passenger.getId())
        .removeIf(req -> req.getId().equals(event.getRequestId()));
  ...
  private void processRequestScheduledEvents(PassengerRequestScheduledEvent event) {
    ...
    if (requestEntry != null) {
      ...
      preplanningEngine.notifyChangedTripInformation(
          requestEntry.passenger,
          Optional.of(
              new DvrpTripInfo(
                  mode,
                  request.getFromLink(),
  ...
  public static Provider<PassengerEngine> createProvider(String mode) {
    return new ModalProviders.AbstractProvider<>(mode, DvrpModes::mode) {
      ...
      @Inject private PreplanningEngine preplanningEngine;

      @Override
      public PassengerEngineWithPrebooking get() {
        return new PassengerEngineWithPrebooking(
            getMode(),
            eventsManager,
            mobsimTimer,
            preplanningEngine,
