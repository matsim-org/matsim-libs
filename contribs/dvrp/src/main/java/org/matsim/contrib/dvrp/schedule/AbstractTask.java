...
public abstract class AbstractTask implements Task {
  ...
  AbstractTask(TaskType taskType, double beginTime, double endTime) {
    Preconditions.checkArgument(
        beginTime <= endTime, "beginTime=%s; endTime=%s", beginTime, endTime);
  ...
  public final void setBeginTime(double beginTime) {
    Preconditions.checkState(
        status != TaskStatus.STARTED && status != TaskStatus.PERFORMED,
  ...
  public final void setEndTime(double endTime) {
    Preconditions.checkState(
        status != TaskStatus.PERFORMED, "It is too late to change the endTime");
    this.endTime = endTime;
  }
