...
public class TravelTimeMatricesTest {
  ...
  public void travelTimeMatrix() {
    ...
    var matrix =
        TravelTimeMatrices.calculateTravelTimeMatrix(routingParams(network), centralNodes, 0);
  ...
  public void travelTimeSparseMatrix_maxDistance() {
    ...
    Node nodeB = NetworkUtils.createAndAddNode(network, Id.createNodeId("B"), new Coord(150, 150));
    Node nodeC =
        NetworkUtils.createAndAddNode(network, Id.createNodeId("C"), new Coord(-150, -150));
    NetworkUtils.createAndAddLink(network, Id.createLinkId("AB"), nodeA, nodeB, 150, 15, 20, 1);
    NetworkUtils.createAndAddLink(network, Id.createLinkId("BA"), nodeB, nodeA, 300, 15, 40, 1);
    ...
    var matrix =
        TravelTimeMatrices.calculateTravelTimeSparseMatrix(
                routingParams(network), maxDistance, 0, 0)
            .orElseThrow();
  ...
  public void travelTimeSparseMatrix_maxTravelTime() {
    ...
    // 20 s (max TT) corresponds to 300 m (max distance) in another test (see:
    // travelTimeSparseMatrix_maxDistance())
    double maxTravelTime = 20; // B->A->C and C->A->B are pruned by  the limit
    var matrix =
        TravelTimeMatrices.calculateTravelTimeSparseMatrix(
                routingParams(network), 0, maxTravelTime, 0)
            .orElseThrow();
  ...
  private static TravelTimeMatrices.RoutingParams routingParams(Network network) {
    return new TravelTimeMatrices.RoutingParams(
        network,
