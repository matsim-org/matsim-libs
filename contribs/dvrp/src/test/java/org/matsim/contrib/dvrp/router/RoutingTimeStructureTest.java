...
public class RoutingTimeStructureTest {
	...
	public void testTimingWithSimpleAccess() {
		TimeInterpreter.Factory timeInterpreterFactory = TimeInterpreterModule.createFromConfig(ConfigUtils.createConfig());
		...
		when(stopFinder.findFacilities(fromFacility, toFacility)).thenReturn(Optional.of(Pair.of(accessFacility, egressFacility)));
		...
		DvrpRoutingModule routingModule = new DvrpRoutingModule(mainRouter, accessRouter, egressRouter, stopFinder, "drt", timeInterpreterFactory);
		List<? extends PlanElement> result = routingModule.calcRoute(DefaultRoutingRequest.withoutAttributes(fromFacility, toFacility, 500.0, null));
		...
		assertThat(((Leg) result.get(2)).getDepartureTime().seconds()).isEqualTo(500.0 + 120.0 + 1.0);
		...
		assertThat(((Leg) result.get(4)).getDepartureTime().seconds()).isEqualTo(500.0 + 120.0 + 1.0 + 800.0 + 1.0);
	}
	...
	public void testTimingWithComplexAccess() {
		...
		assertThat(((Activity) result.get(3)).getMaximumDuration().seconds()).isEqualTo(0.0);
		assertThat(((Activity) result.get(3)).getEndTime().isUndefined()).isTrue();
		assertThat(((Leg) result.get(4)).getDepartureTime().seconds()).isEqualTo(500.0 + 120.0 + 400.0 + 1.0);
		...
		assertThat(((Leg) result.get(6)).getDepartureTime().seconds()).isEqualTo(500.0 + 120.0 + 400.0 + 1.0 + 800.0 + 1.0);
	}
}
