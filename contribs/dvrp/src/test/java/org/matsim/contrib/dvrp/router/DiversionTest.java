...
public class DiversionTest {
  ...
  public static final int DIVERSION_FREQUENCY = 10;

  @Test
  /**
   * This is a relatively complex test to the the diversion behaviour. The experiment is constructed
   * as folows: We have one DVRP vehicle which moves along 8 links (l0 to l8) with a distance of
   * 1000m each and a odd freespeed to that the QSim will perform rounding.
   *
   * <p>To do so, before the simulation starts, the route from l0 to l8 is calculated and assigned
   * to a drive task, which is added to the vehicle. Here, we already have an estimated for the
   * arrival time, which should predict correctly the final arrival time of the vehicle at l8. This
   * is the first fact to check.
   *
   * <p>After, we divert the vehicle every N seconds. To do so, we obtain the next possible
   * diversion point from the drive task tracker and perform a routing from there to l8. Again, any
   * time we do that, we should obtain the correct arrival time if prediction is correct.
   *
   * <p>State 1 Nov 2021 is the following: In this scenario:
   *
   * <ul>
   *   <li>the vehicle arrives at the end of l8 at 657s (reference value).
   *   <li>the initial routing predicts an arrival time of 656s (wrong, too quick)
   *   <li>at every diversion the new routing gives an arrival time of 658s (wrong, too slow)
   *   <li>once the vehicle enters l8 all remaining diversions give 657s (correct)
   * </ul>
   *
   * Hypotheses for the time being:
   *
   * <ul>
   *   <li>Different than stated in QSimFreeSpeedTravelTime, departing from the current activity
   *       takes 2s according to the events. One for "enters traffic", one for "leaves link".
   *   <li>The change of behaviour for the diversion when entering the final link comes from the
   *       fallback in createPath.createPath with considerZeroLengthPath == true
   *   <li>The difference between the initial routing and the diversion comes from the fact that
   *       when departing from an activity, we have the slack described above (agent needs to
   *       depart), but we do *not* have that when diverting. In that case, the agent will simply
   *       enter the next link after traversing the current one.
   * </ul>
   */
  public void testRepeatedSameDestinationDiversions() {
    ...
    controller.addOverridingQSimModule(
        new AbstractDvrpModeQSimModule(MODE) {
          ...
          protected void configureQSim() {
            ...
            bind(TestOptimizerSameDestination.class)
                .toProvider(
                    modalProvider(
                        getter -> {
                          ...
                          return new TestOptimizerSameDestination(
                              timer, network, fleet, getConfig().qsim(), testTracker);
                        }))
                .in(Singleton.class);
          }
        });
    ...
    System.out.println(
        "Arrival in simulation acc. to events: " + testTracker.eventBasedArrivalTime);
    System.out.println(
        "Predicted arrival time when preparing drive task: " + testTracker.initialArrivalTime);
    System.out.println(
        "Predicted arrival time for diversions: " + testTracker.diversionArrivalTimes);
  ...
  private static class TestModeModule extends AbstractDvrpModeModule {
  ...
  private static class TestModeQSimModule extends AbstractDvrpModeQSimModule {
    ...
    protected void configureQSim() {
      ...
      bindModal(Fleet.class)
          .toProvider(
              modalProvider(
                  getter -> {
                    return Fleets.createDefaultFleet(
                        fleetSpecification, getter.getModal(Network.class).getLinks()::get);
                  }))
          .in(Singleton.class);
              ...
              modalProvider(
                  getter -> {
                    return (DynAgent dynAgent, DvrpVehicle vehicle, double now) -> {
                      return VrpLegFactory.createWithOnlineTracker(
                          TransportMode.car,
                          vehicle,
  ...
  private static class TestOptimizerSameDestination
    ...
    public TestOptimizerSameDestination(
        MobsimTimer timer,
        Network network,
        Fleet fleet,
      ...
      this.router =
          new DijkstraFactory().createPathCalculator(network, travelDisutility, travelTime);
    ...
    private void insertDriveTask() {
      ...
      VrpPathWithTravelData path =
          VrpPaths.calcAndCreatePath(startLink, endLink, timer.getTimeOfDay(), router, travelTime);
    ...
    @Override
    public void notifyMobsimBeforeSimStep(
        @SuppressWarnings("rawtypes") MobsimBeforeSimStepEvent e) {
      if (vehicle.getSchedule().getStatus().equals(ScheduleStatus.STARTED)) {
        if (timer.getTimeOfDay() % DIVERSION_FREQUENCY == 0) {
          ...
          if (currentTask instanceof DefaultDriveTask) {
            ...
            VrpPathWithTravelData path =
                VrpPaths.calcAndCreatePathForDiversion(diversionPoint, endLink, router, travelTime);
            ...
            this.testTracker.taskEndTimes.add(
                TaskTrackers.predictEndTime(currentTask, timer.getTimeOfDay()));
          }
        }
      }
    }
  ...
  private static class ArrivalHandler implements PersonArrivalEventHandler {
    private final TestTracker testTracker;
  ...
  private static class TestTracker {
  ...
  @Test
  /**
   * This test is similar as the one above. However, there is another catch when diverting, which is
   * not covered on top. In fact, following the QSim and according to QSimFreeSpeedTravelTime, the
   * traversal time for a link is floor(length / speed + 1, i.e. always one time step (second)
   * longer than predicted by distance and speed. This additional second is needed to traverse the
   * node. HOWEVER, this additional second is not relevant on the last link of the route as the
   * vehicle will just arrive on the last link. This is why there is a special implementation for
   * the calculation of the last link's travel time in VrpPaths.
   *
   * <p>Now, when diverting from the last link, the vehicle *will* pass the node at the end of that
   * link, so we need to add back one second when calculating the diversion point.
   *
   * <p>In this test, we let the vehicle traverse L0 to L8 as before, however the network has an
   * additional L9. In every diversion step, we calculate the arrival time at L9 if we would divert,
   * but we *do not* implement the diversion. Without any code fixes, we will get a certain
   * prediction for the arrival time at L9 until we reach L8, and after having entered L8, we will
    ...
    controller.addOverridingQSimModule(
        new AbstractDvrpModeQSimModule(MODE) {
          ...
          protected void configureQSim() {
            ...
            bind(TestOptimizerDifferentDestinationFromLastLink.class)
                .toProvider(
                    modalProvider(
                        getter -> {
                          Network network = getter.getModal(Network.class);
                          ...
                          return new TestOptimizerDifferentDestinationFromLastLink(
                              timer, network, fleet, getConfig().qsim(), testTracker);
                        }))
                .in(Singleton.class);
          }
  ...
  private static class TestOptimizerDifferentDestinationFromLastLink
      implements VrpOptimizer, MobsimBeforeSimStepListener {
    private final MobsimTimer timer;
    ...
    public TestOptimizerDifferentDestinationFromLastLink(
        MobsimTimer timer,
        Network network,
        Fleet fleet,
        QSimConfigGroup qsimConfig,
    ...
    private void insertDriveTask() {
      ...
      VrpPathWithTravelData path =
          VrpPaths.calcAndCreatePath(
              startLink, initialEndLink, timer.getTimeOfDay(), router, travelTime);
      ...
      if (vehicle.getSchedule().getStatus().equals(ScheduleStatus.STARTED)) {
        if (timer.getTimeOfDay() % DIVERSION_FREQUENCY == 0) {
          ...
          if (currentTask instanceof DriveTask) {
            ...
            // (2) Perform rerouting from the diversion point
            VrpPathWithTravelData path =
                VrpPaths.calcAndCreatePathForDiversion(
                    diversionPoint, updatedEndLink, router, travelTime);
