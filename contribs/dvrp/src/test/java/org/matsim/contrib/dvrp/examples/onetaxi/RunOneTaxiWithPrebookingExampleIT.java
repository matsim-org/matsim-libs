...
public class RunOneTaxiWithPrebookingExampleIT {
  ...
  public void testRun() {
    ...
    // PopulationUtils.writePopulation(scenario.getPopulation(), utils.getOutputDirectory() +
    // "/../pop.xml");
    ...
    controler.addOverridingModule(
        new AbstractModule() {
          ...
          public void install() {
            ...
            install(
                new OneTaxiModule(
                    ConfigGroup.getInputFileURL(config.getContext(), "one_taxi_vehicles.xml"),
                    ...
                    (BasicEventHandler)
                        event -> {
                          if (event instanceof AgentWakeupEvent) {
                            wakeupEvents.put(
                                ((AgentWakeupEvent) event).getPersonId(), (AgentWakeupEvent) event);
                          } else if (event instanceof PassengerRequestScheduledEvent) {
                            requestScheduledEvents.put(
                                ((PassengerRequestScheduledEvent) event)
                                    .getPersonIds().stream().findFirst().orElseThrow(),
                                (PassengerRequestScheduledEvent) event);
                          } else if (event instanceof ActivityEndEvent
                              && ((ActivityEndEvent) event).getActType().equals("dummy")) {
                            activityEndEvents.put(
                                ((ActivityEndEvent) event).getPersonId(), (ActivityEndEvent) event);
                          }
    ...
    // check results
    eventsByPassenger.keySet().stream()
        .sorted()
    ...
    // assertRequestScheduledEvent(requestScheduledEvents, "passenger_3", /*1402.66*/ 1401.66,
    // "taxi_3");
    // (exchanging the sequence of binding of PreplanningEngineQSimModule and DynActivityEngine
    // changes the results :-(. kai, jan'20 (*))
    ...
    // assertRequestScheduledEvent(requestScheduledEvents, "passenger_4", /*1977.8*/ 1976.8,
    // "taxi_4"); // see (*)
    ...
    assertRequestScheduledEvent(
        requestScheduledEvents, "passenger_5", 2503.46, "taxi_5"); // see (*)
    // assertRequestScheduledEvent(requestScheduledEvents, "passenger_5", /*2503.46*/ 2502.46,
    // "taxi_5"); // see (*)
    ...
    assertRequestScheduledEvent(
        requestScheduledEvents, "passenger_6", 2932.46, "taxi_6"); // see (*)
    // assertRequestScheduledEvent(requestScheduledEvents, "passenger_6", /*2932.46*/ 2931.46,
    // "taxi_6"); // see (*)
    ...
    assertRequestScheduledEvent(
        requestScheduledEvents, "passenger_7", 3317.46, "taxi_7"); // see (*)
    // assertRequestScheduledEvent(requestScheduledEvents, "passenger_7", /*3317.46*/ 3316.46,
    // "taxi_7"); // see (*)
    ...
    assertRequestScheduledEvent(
        requestScheduledEvents, "passenger_8", 3944.86, "taxi_8"); // see (*)
    // assertRequestScheduledEvent(requestScheduledEvents, "passenger_8", /*3944.86*/ 3943.86,
    // "taxi_8"); // see (*)
    ...
    assertRequestScheduledEvent(
        requestScheduledEvents, "passenger_9", 4333.53, "taxi_9"); // see (*)
    // assertRequestScheduledEvent(requestScheduledEvents, "passenger_9", /*4333.53*/ 4332.53,
    // "taxi_9"); // see (*)
    assertActivityEndEvent(activityEndEvents, 3434, "passenger_9");
  }

  private static void assertWakeupEvent(
      Map<Id<Person>, AgentWakeupEvent> events, double time, String personId) {
  ...
  private static void assertRequestScheduledEvent(
      Map<Id<Person>, PassengerRequestScheduledEvent> events,
      String personId,
      double pickupTime,
      String requestId) {
  ...
  private static void assertActivityEndEvent(
      Map<Id<Person>, ActivityEndEvent> events, double time, String personId) {
