...
public class OneToManyPathCalculatorTest {
  ...
  private final LeastCostPathTree dijkstraTree =
      new LeastCostPathTree(
          new SpeedyGraph(network), travelTime, new TimeAsTravelDisutility(travelTime));
  ...
  public void forward_fromNodeB_toNodeB() {
    ...
    var pathCalculator =
        new OneToManyPathCalculator(nodeMap, dijkstraTree, travelTime, true, linkAB, 0);

    // search until node B is reached
    pathCalculator.calculateDijkstraTree(List.of(linkBC));

    assertThat(pathCalculator.createPath(nodeB))
        .isEqualToComparingFieldByField(new Path(List.of(nodeB), List.of(), 0, 0));
  ...
  public void forward_fromNodeB_toNodesBD() {
    ...
    assertThat(pathCalculator.createPath(nodeD))
  ...
  public void backward_fromNodeD_toNodeD() {
    ...
    var pathCalculator =
        new OneToManyPathCalculator(nodeMap, dijkstraTree, travelTime, false, linkDE, 0);

    // search until node D is reached
    pathCalculator.calculateDijkstraTree(List.of(linkCD));

    assertThat(pathCalculator.createPath(nodeD))
        .isEqualToComparingFieldByField(new Path(List.of(nodeD), List.of(), 0, 0));
  ...
  public void equalFromLinkAndToLink() {
    ...
    for (boolean forward : List.of(true, false)) {
      var pathCalculator =
          new OneToManyPathCalculator(nodeMap, mockedTree, travelTime, forward, linkAB, 0);
  ...
  public void pathData_forward() {
    ...
    // path: B -> C -> D
    assertPathData(
        pathCalculator.createPathDataLazily(linkDE),
        new Path(ImmutableList.of(nodeB, nodeC, nodeD), ImmutableList.of(linkBC, linkCD), 20, 20),
        1 + 10);
  }
  ...
  public void pathData_backward() {
    ...
    // path: B -> C -> D
    assertPathData(
        pathCalculator.createPathDataLazily(linkAB),
        new Path(ImmutableList.of(nodeB, nodeC, nodeD), ImmutableList.of(linkBC, linkCD), 20, 20),
        1 + 10);
  }
  ...
  public void pathData_fromLinkEqualsToLink() {
    for (boolean forward : List.of(true, false)) {
      var pathCalculator =
          new OneToManyPathCalculator(nodeMap, dijkstraTree, travelTime, forward, linkAB, 0);
      pathCalculator.calculateDijkstraTree(List.of(linkAB));
  ...
  private Link createAndAddLink(
      String id, Node fromNode, Node toNode, double length, double freespeed) {
    return NetworkUtils.createAndAddLink(
        network, Id.createLinkId(id), fromNode, toNode, length, freespeed, length / 7.5, 1);
  }
}
