/* *********************************************************************** *
 * project: org.matsim.*
 * GTFS2MATSimTransitSchedule.java
 *                                                                         *
 * *********************************************************************** *
 *                                                                         *
...
public class GTFS2MATSimTransitSchedule {
	 ...
	 * Log
	 */
	private final static Logger log = Logger.getLogger(GTFS2MATSimTransitSchedule.class);
	/**
	...
	private File[] roots;
	/**
	 * Network modes for each public transport system
	 */
	...
	private void calculateUnknownInformation() throws IOException {
		...
		for(byte publicSystemNumber=0; publicSystemNumber<roots.length; publicSystemNumber++) {
			RoutesPathsGenerator routesPathsGenerator = new RoutesPathsGenerator(network, roots[publicSystemNumber], modes[publicSystemNumber], routes[publicSystemNumber], stops[publicSystemNumber]);
	...
	private void splitStopLinks(byte publicTransportSystem) {
		...
		for(Stop stop:stops[publicTransportSystem].values()) {
			if(stop.getLinkId()!=null) {
				...
				if(linkStops == null) {
					linkStops = new LinkStops(network.getLinks().get(Id.create(stop.getLinkId(), Link.class)));
		...
		for(LinkStops linkStops:linksStops.values()) {
			for(int i=0; i<linkStops.getNumStops()-1; i++) {
				Link link = linkStops.split(i,network, coordinateTransformation);
				if(link!=null)
					changeTrips(linkStops.getLink(), link, false, publicTransportSystem);
			}
			if(linkStops.getLastDistance()>MAX_DISTANCE_STOP_LINK) {
				Link link  = linkStops.split(linkStops.getNumStops()-1,network, coordinateTransformation);
				if(link!=null)
			...
			for(int publicSystemNumber=0; publicSystemNumber<roots.length; publicSystemNumber++)
				for(Entry<String, Stop> stop: stops[publicSystemNumber].entrySet())
					if(stop.getValue().getLinkId()!=null) {
						...
						transitStopFacility.setLinkId(Id.create(stop.getValue().getLinkId(), Link.class));
	...
	public static void main(String[] args) throws Exception {
		...
		GTFS2MATSimTransitSchedule g2m = new GTFS2MATSimTransitSchedule(new File[]{new File("./data/gtfs/buses"),new File("./data/gtfs/trains")}, new String[]{"road","rail"}, network, new String[]{"weekday","weeksatday","daily"}, TransformationFactory.WGS84_SVY21);
		//Convert
