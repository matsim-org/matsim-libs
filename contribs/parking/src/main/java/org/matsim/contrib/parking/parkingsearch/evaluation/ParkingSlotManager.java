...
public class ParkingSlotManager {
  ...
  private final Map<Id<Vehicle>, Tuple<Coord, Double>> occupiedSlots =
      new HashMap<Id<Vehicle>, Tuple<Coord, Double>>();
  ...
  public Tuple<Coord, Double> processParking(double timeOfParking, Id<Vehicle> vehID) {
    if (this.freeSlots.isEmpty())
      throw new RuntimeException(
          "all slots at link "
              + linkID.toString()
              + " already occupied. cannot display another occupied slot ("
              + vehID.toString()
              + ")");
