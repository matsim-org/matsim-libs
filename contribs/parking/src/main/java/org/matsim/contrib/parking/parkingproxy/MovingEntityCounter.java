...
/**
 * Counts departures and arrivals of not strictly defined entities that move through spacetime
 * (persons, vehicles, ...) based on a spatial and temporal grid.
 *
...
class MovingEntityCounter implements PenaltyGenerator {
   ...
   * @param endTime The endtime of the last "real" timebin in seconds. After this there is an
   *     overflow timebin without end time. Linear memory impact, constant performance impact
   * @param spatialGridSize The size of one gridcell in the spatial grid in CRS units. Reasonable
   *     values are between 100 and 1000. Pretty surely less-than-quadratic memory and performance
   *     impact.
   */
  public MovingEntityCounter(
      Collection<Tuple<Coord, Integer>> initialPositions,
      int timeBinSize,
      int endTime,
      int spatialGridSize) {
    ...
    for (Tuple<Coord, Integer> carTuple : initialPositions) {
      this.initialLoad.adjustOrPutValue(
          this.hectareMapper.getKey(carTuple.getFirst()),
          carTuple.getSecond(),
          carTuple.getSecond());
    }
  ...
  /**
   * Handles the arrival of an entity at a given time at a given spatial coordinate. The weight
   * parameter allows to account for samples that should represent the whole ensemble, i.e. one
   * entity with weight 10 has the same effect as 10 entities with weight 1.
   *
   ...
   * @return The number of "real world" arrivals in this space-time-bin after handling the current
   *     arrival. Negative arrivals are equivalent to departures. Therefore a return value of 0 not
   *     necessarily means no arrivals, but only that there are as many arrivals as departures
   *     (which still could be 0)
   */
  public int handleArrival(int time, double x, double y, int weight) {
    return this.carArrivals[getTimeBin(time)].adjustOrPutValue(
        this.hectareMapper.getKey(x, y), weight, weight);
  }
  ...
  /**
   * Handles the departure of an entity at a given time at a given spatial coordinate. The weight
   * parameter allows to account for samples that should represent the whole ensemble, i.e. one
   * entity with weight 10 has the same effect as 10 entities with weight 1.
   *
   * @param time The time (in seconds) since the start of the simulation
   ...
   * @return The number of "real world" arrivals in this space-time-bin after handling the current
   *     arrival. Negative arrivals are equivalent to departures. Therefore a return value of 0 not
   *     necessarily means no no departures, but only that there are as many arrivals as departures
   *     (which still could be 0)
   */
  ...
  /**
   * Calculates for each space-time-bin the number of specimen based on the initial load received in
   * the constructor and the departures and arrivals collected with {@linkplain #handleArrival(int,
   * double, double, int)} and {@linkplain #handleDeparture(int, double, double, int)} and then
   * generates a {@linkplain PenaltyCalculator} based on the result.
   */
  @Override
