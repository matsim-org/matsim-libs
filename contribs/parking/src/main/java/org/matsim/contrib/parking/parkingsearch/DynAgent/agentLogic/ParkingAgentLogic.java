...
public class ParkingAgentLogic implements DynAgentLogic {
  ...
  public enum LastParkActionState {
    ...
    ACTIVITY, // ordinary activity: get next Leg, if car: go to car, otherwise add ordinary leg by
    // other mode
  ...
  protected DynAction nextStateAfterUnParkActivity(DynAction oldAction, double now) {
    ...
    NetworkRoute actualRoute =
        this.parkingRouter.getRouteFromParkingToDestination(
            plannedRoute.getEndLinkId(), now, agent.getCurrentLinkId());
    if ((this.parkingManager.unParkVehicleHere(
            currentlyAssignedVehicleId, agent.getCurrentLinkId(), now))
        || (isInitialLocation)) {
      this.lastParkActionState = LastParkActionState.CARTRIP;
      isInitialLocation = false;
      Leg currentLeg = (Leg) this.currentPlanElement;
      // this could be Car, Carsharing, Motorcylce, or whatever else mode we have, so we want our
      // leg to reflect this.
      return new ParkingDynLeg(
          currentLeg.getMode(),
          actualRoute,
          parkingLogic,
          parkingManager,
  ...
  protected DynAction nextStateAfterWalkFromPark(DynAction oldAction, double now) {
    // walkleg complete, time to get the next activity from the plan Elements and start it, this is
    // basically the same as arriving on any other mode
  ...
  protected DynAction nextStateAfterParkActivity(DynAction oldAction, double now) {
    ...
    Facility fromFacility =
        new LinkWrapperFacility(network.getLinks().get(agent.getCurrentLinkId()));
    Facility toFacility =
        new LinkWrapperFacility(
            network.getLinks().get(currentPlannedLeg.getRoute().getEndLinkId()));
    List<? extends PlanElement> walkTrip =
        walkRouter.calcRoute(
            DefaultRoutingRequest.withoutAttributes(
                fromFacility, toFacility, now, plan.getPerson()));
    if (walkTrip.size() != 1 || !(walkTrip.get(0) instanceof Leg walkLeg)) {
      String message =
          "walkRouter returned something else than a single Leg, e.g. it routes walk on the network with non_network_walk to access the network. Not implemented in parking yet!";
      log.error(message);
  ...
  protected DynAction nextStateAfterCarTrip(DynAction oldAction, double now) {
    // car trip is complete, we have found a parking space (not part of the logic), block it and
    // start to park
    if (this.parkingManager.parkVehicleHere(
        Id.create(this.agent.getId(), Vehicle.class), agent.getCurrentLinkId(), now)) {
  ...
  protected DynAction nextStateAfterActivity(DynAction oldAction, double now) {
    ...
    if (plan.getPlanElements().size() >= planIndex + 1) {
      ...
      if (currentLeg.getMode().equals(TransportMode.car)) {
        ...
        if (parkLink == null) {
          // this is the first activity of a day and our parking manager does not provide
          // informations about initial stages. We suppose the car is parked where we are
          parkLink = agent.getCurrentLinkId();
        }

        Facility fromFacility =
            new LinkWrapperFacility(network.getLinks().get(agent.getCurrentLinkId()));
        Id<Link> teleportedParkLink =
            this.teleportationLogic.getVehicleLocation(
                agent.getCurrentLinkId(), vehicleId, parkLink, now, currentLeg.getMode());
