...
public class ParkingAgentFactory implements AgentFactory {
  ...
  /** */
  @Inject
  @Named(value = TransportMode.walk)
  RoutingModule walkRouter;

  @Inject ParkingSearchManager parkingManager;

  @Inject EventsManager events;
  @Inject ParkingRouter parkingRouter;
  @Inject Network network;
  @Inject VehicleTeleportationLogic teleportationLogic;
  QSim qsim;
  ...
  public MobsimAgent createMobsimAgentFromPerson(Person p) {
    ...
    switch (psConfigGroup.getParkingSearchStrategy()) {
      ...
      case DistanceMemory -> {
        ...
        agentLogic =
            new MemoryBasedParkingAgentLogic(
                p.getSelectedPlan(),
                parkingManager,
                walkRouter,
                network,
      ...
      case NearestParkingSpot -> {
        ...
        int numberReserved =
            (int)
                Math.round(psConfigGroup.getFractionCanReserveParkingInAdvanced() * numberOfAgents);
        int numberCapacityCheck =
            (int)
                Math.round(
                    psConfigGroup.getFractionCanCheckFreeCapacitiesInAdvanced() * numberOfAgents);

        if (currentAgentNumber == 1) {
          LogManager.getLogger(getClass())
              .info(
                  "Number of agents, who can reserve a parking slot in advanced: "
                      + numberReserved);
          LogManager.getLogger(getClass())
              .info(
                  "Number of agents, who can check a free parking slot in advanced: "
                      + numberCapacityCheck);
          LogManager.getLogger(getClass())
              .info(
                  "Number of agents, who have no technical support to find a parking slot: "
                      + (numberOfAgents - numberReserved - numberCapacityCheck));
        }
        if (numberReserved >= currentAgentNumber) {
          parkingLogic =
              new NearestParkingSpotSearchLogic(network, parkingRouter, parkingManager, true, true);
        } else if (numberCapacityCheck + numberReserved >= currentAgentNumber) {
          parkingLogic =
              new NearestParkingSpotSearchLogic(
                  network, parkingRouter, parkingManager, false, true);
        } else
          parkingLogic =
              new NearestParkingSpotSearchLogic(
                  network, parkingRouter, parkingManager, false, false);
        agentLogic =
            new NearestParkingSpotAgentLogic(
                p.getSelectedPlan(),
                parkingManager,
                walkRouter,
                network,
