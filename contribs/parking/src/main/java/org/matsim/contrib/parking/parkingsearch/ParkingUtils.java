...
public class ParkingUtils {
  ...
  public static final String ParkingStageInteractionType = "parking";
  public static final String ParkingActivityType = "parking_activity";
  public static final String WaitingForParkingActivityType = "waitingForParkingSpace_activity";
  public static final int NO_OF_LINKS_TO_GET_ON_ROUTE = 5;
  ...
  public static List<Link> getOutgoingLinksForMode(Link link, String mode) {
    ...
    if (outGoingModeLinks.size() == 0) {
      throw new RuntimeException(
          "could not find an outgoing link for mode "
              + mode
              + " from link "
              + link
              + ". Consequences are not checked. Please check the network. \n Aborting...");
    }
  ...
  public static boolean checkIfActivityHasNoParking(Activity followingActivity) {
    return followingActivity.getAttributes().getAsMap().containsKey("parking")
        && followingActivity.getAttributes().getAttribute("parking").equals("noParking");
  }
  ...
  /**
   * This activity has a passenger interaction. This would mean that the location is fixed, and can
   * not be changed.
   *
   * @param activity
   */
  ...
  /**
   * Checks if the activity has a passanger interaction. This would mean that the location is fixed,
  ...
  public static boolean checkIfActivityHasPassengerInteraction(Activity activity) {
    return activity.getAttributes().getAsMap().containsKey("parking")
