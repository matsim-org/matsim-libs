 ...
 *
 * <p>The key is composed in such a way that the x-number (Integer) of the gridcell (starting at
 * 0,0) is stored in the first half of the bits of the Long-key and the y-number (Integer) in the
 * second half. This happens in binary, i.e. you can not "by eye" reconstruct the original
 * coordinate from looking at the key.
 *
 * <p><b>Important note:</b> This class works with int and long values and is therefore best used
...
class HectareMapper {
  ...
  public Coord getCenter(long key) {
    ...
    return new Coord(
        xCell * this.gridsize + this.gridsize / 2, yCell * this.gridsize + this.gridsize / 2);
  }
}
