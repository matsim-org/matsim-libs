...
public class GeneralParkingModule implements StartupListener, BeforeMobsimListener {
  ...
  public void notifyStartup(StartupEvent event) {
    parkingSimulation =
        new ParkingChoiceSimulation(controler.getScenario(), parkingInfrastructureManager);
  ...
  public final void setParkingInfrastructurManager(
      ParkingInfrastructure parkingInfrastructureManager) {
    this.parkingInfrastructureManager = parkingInfrastructureManager;
  }

  @Deprecated
  // lower level objects may keep back pointers to higher level objects if they have to, but we
  // prefer that they do not provide them
