...
public class FacilityBasedParkingManager implements ParkingSearchManager {
  ...
  protected Map<Id<ActivityFacility>, MutableLong> numberOfStaysFromGetOffUntilGetIn =
      new HashMap<>();
  ...
  public FacilityBasedParkingManager(Scenario scenario) {
    psConfigGroup =
        (ParkingSearchConfigGroup)
            scenario.getConfig().getModules().get(ParkingSearchConfigGroup.GROUP_NAME);
   ...
   * @return
   */
  public boolean canParkAtThisFacilityUntilEnd(
      Id<Link> linkId,
      double stopDuration,
    ...
    if (facilities != null) {
      ...
      for (Id<ActivityFacility> facility : facilities) {
        ...
        if (this.parkingFacilities
            .get(facility)
            .getAttributes()
            .getAsMap()
            .containsKey("maxParkingDurationInHours"))
          maxParkingDurationAtFacilityInHours =
              3600
                  * (double)
                      this.parkingFacilities
                          ...
                          .getAttributes()
                          .getAsMap()
                          .get("maxParkingDurationInHours");
        if (maxParkingDurationAtFacilityInHours > totalNeededParkingDuration) {
          ActivityOption parkingOptions =
              this.parkingFacilities.get(facility).getActivityOptions().get("parking");
          if (!parkingOptions.getOpeningTimes().isEmpty()) {
            if ((parkingOptions.getOpeningTimes().first().getStartTime() == 0
                && parkingOptions.getOpeningTimes().first().getEndTime() == 24 * 3600))
              if (parkingOptions.getOpeningTimes().first().getStartTime() <= now
                  && parkingOptions.getOpeningTimes().first().getEndTime()
                      >= now + totalNeededParkingDuration) return true;
          } else return true;
        }
      }
    }
    ...
    if (parkingFacilitiesAtLink == null) {
      ...
      if (fac != null) {
        ...
        throw new RuntimeException(
            "no parking reservation found for vehicle "
                + vehicleId.toString()
                + " arrival on link "
                + linkId
    ...
    for (Entry<Id<ActivityFacility>, MutableLong> e : this.occupation.entrySet()) {
      ...
      String s =
          linkId.toString()
              ...
              + e.getValue().toString()
              + ";"
              + this.reservationsRequests.get(e.getKey()).toString()
              + ";"
              + this.numberOfParkedVehicles.get(e.getKey()).toString()
              + ";"
              + this.rejectedParkingRequest.get(e.getKey()).toString()
              + ";"
              + this.numberOfWaitingActivities.get(e.getKey()).toString()
              + ";"
              + this.numberOfStaysFromGetOffUntilGetIn.get(e.getKey()).intValue()
              + ";"
              + this.numberOfParkingBeforeGetIn.get(e.getKey()).intValue();
      stats.add(s);
    }
    return stats;
  }
  ...
  public List<String> produceTimestepsStatistics() {
    ...
    for (int time : rejectedReservationsByTime.keySet()) {
      ...
      String s =
          Time.writeTime(time, Time.TIMEFORMAT_HHMM)
              + ";"
              + rejectedReservationsByTime.get(time)
              + ";"
              + foundParkingByTime.get(time)
              + ";"
              + unparkByTime.get(time);
      stats.add(s);
    }
    return stats;
  }
  ...
  public void addVehicleForWaitingForParking(Id<Link> linkId, Id<Vehicle> vehicleId, double now) {
    //		System.out.println(now + ": vehicle " +vehicleId.toString() + " starts waiting here: " +
    // linkId.toString());
          ...
          int cap =
              (int)
                  this.parkingFacilities
                      ...
                      .get(ParkingUtils.ParkingStageInteractionType)
                      .getCapacity();
          while (this.occupation.get(fac).intValue() < cap
              && !waitingVehicles.get(linkId).isEmpty()) {
            double startWaitingTime = waitingVehicles.get(linkId).firstKey();
            if (startWaitingTime > now) break;
            Id<Vehicle> vehcileId = waitingVehicles.get(linkId).remove(startWaitingTime);
            DynAgent agent =
                (DynAgent) qSim.getAgents().get(Id.createPersonId(vehcileId.toString()));
