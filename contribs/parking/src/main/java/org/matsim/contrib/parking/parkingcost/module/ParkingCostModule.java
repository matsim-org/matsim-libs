...
public class ParkingCostModule extends AbstractModule {
  ...
  public void install() {
    ParkingCostConfigGroup parkingCostConfigGroup =
        ConfigUtils.addOrGetModule(getConfig(), ParkingCostConfigGroup.class);
    if (parkingCostConfigGroup.useParkingCost) {
      Collection<String> mainModes =
          switch (getConfig().controller().getMobsim()) {
            case "qsim" -> getConfig().qsim().getMainModes();
            case "hermes" -> getConfig().hermes().getMainModes();
            default -> throw new RuntimeException(
                "ParkingCosts are currently supported for Qsim and Hermes");
          };
      for (String mode : parkingCostConfigGroup.getModesWithParkingCosts()) {
        if (mainModes.contains(mode)) {
          addEventHandlerBinding()
              .toInstance(new MainModeParkingCostVehicleTracker(mode, parkingCostConfigGroup));
        } else {
          addEventHandlerBinding()
              .toInstance(new TeleportedModeParkingCostTracker(mode, parkingCostConfigGroup));
        }
      }
    }
  }
