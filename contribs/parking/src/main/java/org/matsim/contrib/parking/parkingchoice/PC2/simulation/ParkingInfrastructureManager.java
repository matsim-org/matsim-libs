...
// TODO: make abstract and create algorithm in Zuerich case -> provide protected helper methods
// already here.
public final class ParkingInfrastructureManager implements ParkingInfrastructure {
  ...
  private LinkedListValueHashMap<Id<ActivityFacility>, PPRestrictedToFacilities>
      privateParkingsRestrictedToFacilities;
  ...
  public ParkingInfrastructureManager(
      ParkingScore parkingScoreManager, EventsManager eventsManager) {
  ...
  public synchronized void setPublicParkings(LinkedList<PublicParking> publicParkings) {
    ...
    this.publicParkingsQuadTree =
        (new QuadTreeInitializer<PC2Parking>()).getQuadTree(allPublicParkingRect);

    for (String groupNames : groupRects.keySet()) {
      publicParkingGroupQuadTrees.put(
          groupNames,
  ...
  @Override
  public synchronized PC2Parking parkAtClosestPublicParkingNonPersonalVehicle(
      Coord destCoordinate, String groupName) {
    ...
    if (groupName == null) {
      ...
      if (parking == null) {
        throw new Error(
            "system is in inconsistent state: "
                + "not enough parking available for parkingGroupName:"
                + groupName);
      }
    }
    parkVehicle(parking);
  ...
  public synchronized PC2Parking parkAtClosestPublicParkingNonPersonalVehicle(
    ...
    double walkScore =
        parkingScoreManager.calcWalkScore(
            destCoordinate, parking, personId, parkingDurationInSeconds);
  ...
  @Override
  public synchronized PC2Parking parkVehicle(
    ...
    for (PPRestrictedToFacilities pp :
        privateParkingsRestrictedToFacilities.get(parkingOperationRequestAttributes.facilityId)) {
    ...
    double distance = 300;
    if (!parkingFound) {
      Collection<PC2Parking> collection =
          getFilteredCollection(parkingOperationRequestAttributes, distance);
      ...
      PriorityQueue<SortableMapObject<PC2Parking>> queue =
          new PriorityQueue<SortableMapObject<PC2Parking>>();
      for (PC2Parking parking : collection) {
        double score =
            parkingScoreManager.calcScore(
                ...
                parkingOperationRequestAttributes.legIndex,
                false);
        queue.add(
            new SortableMapObject<PC2Parking>(
                parking,
                -1.0 * score)); // score made positive, so that priority queue works properly
      }
      ...
      if (rentablePrivateParking.containsKey(parkingOperationRequestAttributes.personId)) {
        RentableParking rentableParking =
            rentablePrivateParking.get(parkingOperationRequestAttributes.personId);
  ...
  private synchronized void parkVehicle(PC2Parking parking) {
    ...
    if (parking.getAvailableParkingCapacity() == 0) {
      ...
      wasRemoved =
          publicParkingsQuadTree.remove(
              parking.getCoordinate().getX(), parking.getCoordinate().getY(), parking);

      if (!wasRemoved) {
        throw new Error("system is in inconsistent state: " + parking.getId().toString());
      }

      wasRemoved =
          publicParkingGroupQuadTrees
              .get(parking.getGroupName())
              .remove(parking.getCoordinate().getX(), parking.getCoordinate().getY(), parking);
      ...
      Collection<PC2Parking> collection =
          publicParkingsQuadTree.getDisk(
              parking.getCoordinate().getX(), parking.getCoordinate().getY(), 1);
  ...
  @Override
  public synchronized void scoreParkingOperation(
  ...
  public synchronized void unParkVehicle(
      PC2Parking parking, double departureTime, Id<Person> personId) {
