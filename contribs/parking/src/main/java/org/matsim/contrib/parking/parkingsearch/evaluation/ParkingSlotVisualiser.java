...
public class ParkingSlotVisualiser
  ...
  protected Map<Id<Link>, ParkingSlotManager> slotsOnLink =
      new HashMap<Id<Link>, ParkingSlotManager>();
  ...
  public ParkingSlotVisualiser(
      Network network, Map<Id<ActivityFacility>, ActivityFacility> parkingFacilities) {
  ...
  private void initialize(Map<Id<ActivityFacility>, ActivityFacility> parkingFacilities) {
    ...
    for (Id<Link> linkID : nrOfSlotsPerLink.keySet()) {
      //			LogManager.getLogger(getClass()).info("initialize parking visualisation for link " +
      // linkID);
      this.slotsOnLink.put(
          linkID,
          new ParkingSlotManager(
              network.getLinks().get(linkID), nrOfSlotsPerLink.get(linkID).intValue()));
    }
  }
  ...
  public void handleEvent(VehicleLeavesTrafficEvent event) {
    if (this.slotsOnLink.containsKey(event.getLinkId())) {
      this.vehiclesResponsibleManager.put(
          event.getVehicleId(), this.slotsOnLink.get(event.getLinkId()));
    }
  }
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    ...
    if (manager != null) {
      Tuple<Coord, Double> parkingTuple =
          manager.processParking(event.getTime(), event.getVehicleId());
      this.parkings.add(
          manager.getLinkId()
              + ";"
              + parkingTuple.getSecond()
              + ";"
              + event.getTime()
              + ";"
              + parkingTuple.getFirst().getX()
              + ";"
              + parkingTuple.getFirst().getY()
              + ";"
              + "free");
  ...
  public void handleEvent(PersonEntersVehicleEvent event) {
    if (this.parkedVehicles.containsKey(event.getVehicleId())) {
      ParkingSlotManager manager =
          this.slotsOnLink.get(this.parkedVehicles.get(event.getVehicleId()));
      Tuple<Coord, Double> parkingTuple =
          manager.processUnParking(event.getTime(), event.getVehicleId());
      this.parkings.add(
          manager.getLinkId()
              + ";"
              + parkingTuple.getSecond()
  ...
  public void plotSlotOccupation(String filename) {
    ...
    LogManager.getLogger(getClass())
        .info("FINISHED WRITING PARKING SLOT VISUALISATION FILE TO: " + filename);
  }
    ...
    String path =
        event
            ...
            .getIterationFilename(
                event.getIteration(), "ParkingSlots_it" + event.getIteration() + ".csv");
    this.finishDay();
