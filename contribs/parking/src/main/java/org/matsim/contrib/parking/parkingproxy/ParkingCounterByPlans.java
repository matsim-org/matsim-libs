 ...
 *
 * <p>Tracks how many vehicles are in a given area at any time based on the selected Plans of the
 * Persons in the Population.</br>
 *
...
class ParkingCounterByPlans implements IterationStartsListener, PenaltyGenerator {
  ...
  /**
   * Gets the current population from the event services and calls {@link
   * #calculateByPopulation(Population)}.
   */
  @Override
  public void notifyIterationStarts(IterationStartsEvent event) {
    calculateByPopulation(
        event.getServices().getScenario().getPopulation(),
        event.getServices().getScenario().getNetwork());
  }

  /**
   * Iterates over all plans and calls the {@linkplain MovingEntityCounter#handleArrival(int,
   * double, double, int)} and {@linkplain MovingEntityCounter#handleDeparture(int, double, double,
   * int)} functions whenever an arrival or departure is detected. More precisely, the functions are
   * called whenever an egress leg after a car leg starts and whenever an access leg followed by a
   * car leg ends. The Coordinate used is the to-coord of the link the activity is at which is
   * consistent with how {@linkplain ParkingVehiclesCountEventHandler} currently does things.
   */
    ...
    for (Person p : pop.getPersons().values()) {
      ...
      for (LegActPair walkActPair : this.egressFinder.findAccessWalks(p.getSelectedPlan())) {
        carCounter.handleDeparture(
            (int)
                (walkActPair.leg.getDepartureTime().seconds()
                    + walkActPair.leg.getTravelTime().seconds()),
            network.getLinks().get(walkActPair.act.getLinkId()).getToNode().getCoord(),
            carWeight);
      }
    }
