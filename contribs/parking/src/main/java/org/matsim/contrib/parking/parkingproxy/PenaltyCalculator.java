...
/**
 * Calculates time penalties based on the number of certain entities (cars, persons, ...) in a
 * certain space-time-gridcell. The underlying input data can not be changed after creation, however
 * a different function to translate the number of entities into a numerical penalty may be changed
 * at any time.
 *
 * <p>Instances of this class typically will only be provided by {@linkplain PenaltyGenerator}
 * implementations. This is a deliberate decision to underline the immutability of this object.
 *
...
class PenaltyCalculator {
  ...
  /*package*/ PenaltyCalculator(
      TLongIntMap[] numberOfEntities, int timeBinSize, HectareMapper hectareMapper) {
  ...
  /**
   * Sets the penalty function that translates the number of entities in a space-time-bin into a
   * time-penalty
   *
   * @param function
   */
  ...
  /**
   * Dumps the penalties for all space-time-bins with a non-zero number of entities (there may be
   * some with zero entities in the dump however) as a csv file with the header "{@code
   * x;y;t;penalty}".
   *
  ...
  /**
   * Creates a dummy calculator with one timebin and a spatial binsize of 100km. It returns 3600s
   * (1h) on every request by using {@linkplain PenaltyCalculator.DummyPenaltyFunction} as function.
   *
  ...
  /**
   * The default Penalty function which assumes 2.5s of penalty per entity in the space-time-bin but
   * never more than 900s (15min).
   */
