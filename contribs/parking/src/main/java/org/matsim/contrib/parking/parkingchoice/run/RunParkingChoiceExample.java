...
/**
 * @author jbischoff Sample main file for setting up an Matsim run with parking. Check the output
 *     events for Parking events and the scores.
 */
public class RunParkingChoiceExample {
  ...
  public static void run(Config config) {
    ...
    ParkingScore parkingScoreManager =
        new ParkingScoreManager(new WalkTravelTime(controler.getConfig().routing()), scenario);
    parkingScoreManager.setParkingScoreScalingFactor(1);
    parkingScoreManager.setParkingBetas(new ParkingBetaExample());

    // ---

    ParkingInfrastructure parkingInfrastructureManager =
        new ParkingInfrastructureManager(parkingScoreManager, controler.getEvents());
    {
      ...
      publicParkings.add(
          new PublicParking(
              ...
              98,
              new Coord((double) 10000, (double) 0),
