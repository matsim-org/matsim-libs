...
public class NearestParkingSpotSearchLogic implements ParkingSearchLogic {
  ...
  public NearestParkingSpotSearchLogic(
      ...
      boolean canReserveParkingSlot,
      boolean canCheckParkingCapacitiesInAdvanced) {
  ...
  public Id<Link> getNextLink(
      ...
      boolean passangerInteractionAtParkingFacilityAtEndOfLeg,
      Coord coordOfAttraction) {
    double maxAdditionalDistanceToAttraction = Double.MAX_VALUE;
    // if a passenger interaction take place at a facility the walking distance to the attraction
    // should be not extended by the value of maxAdditionalDistanceToAttraction
    ...
    if (actualRoute == null) {
      actualRoute =
          findRouteToNearestParkingFacility(
              baseLinkId,
              currentLinkId,
              canCheckParkingCapacitiesInAdvanced,
              now,
              maxParkingDuration,
              passangerInteractionAtParkingFacilityAtEndOfLeg,
              maxAdditionalDistanceToAttraction,
              coordOfAttraction);
      if (!passangerInteractionAtParkingFacilityAtEndOfLeg)
        checkIfDrivingToNextParkingLocationIsPossible(
            currentLinkId, baseLinkId, now, nextPickupTime);
      triedParking.clear();
    ...
    // if no possible parking was found. The vehicle takes a random next link. Background
    // assumption: parking only at given parking slots
    if (actualRoute == null) {
      List<Link> outGoingLinks =
          ParkingUtils.getOutgoingLinksForMode(network.getLinks().get(currentLinkId), mode);
   ...
   */
  private void findDistanceBetweenBaseLinkAndAttraction(
      Id<Link> baseLinkId, Coord coordOfAttraction) {
    for (ActivityFacility activityFacility : activityFacilities.values()) {
      if (activityFacility.getLinkId().equals(baseLinkId))
        distanceFromBaseToAttraction =
            NetworkUtils.getEuclideanDistance(activityFacility.getCoord(), coordOfAttraction);
    }
  }

  /**
   * Checks if it is possible to drive to the new parking facility and to drive back to the base
   * without extending the startTime of the following activity. If the resulting parking time at the
   * new facility is less then 5 minutes the vehicle will drive directly to the next activity
   * location.
   *
   * @param currentLinkId
   * @param baseLinkId
   ...
   */
  private void checkIfDrivingToNextParkingLocationIsPossible(
      Id<Link> currentLinkId, Id<Link> baseLinkId, double now, double nextPickupTime) {
    double expectedTravelTimeFromParkingToBase;

    if (actualRoute == null)
      expectedTravelTimeFromParkingToBase =
          this.parkingRouter
              .getRouteFromParkingToDestination(baseLinkId, now, currentLinkId)
              .getTravelTime()
              .seconds(); // TODO better: use the nextLink for the check
    else
      expectedTravelTimeFromParkingToBase =
          this.parkingRouter
              .getRouteFromParkingToDestination(baseLinkId, now, actualRoute.getEndLinkId())
              .getTravelTime()
              .seconds();
    double minimumExpectedParkingDuration = 5 * 60;
    double travelTimeNextPart;
    if (actualRoute == null) travelTimeNextPart = 0.;
    else travelTimeNextPart = actualRoute.getTravelTime().seconds();

    if ((nextPickupTime - now - travelTimeNextPart - expectedTravelTimeFromParkingToBase)
        < minimumExpectedParkingDuration) {
      actualRoute =
          this.parkingRouter.getRouteFromParkingToDestination(baseLinkId, now, currentLinkId);
      skipParkingActivity = true;
    }
  }

  public Id<Link> getNextParkingLocation() {
    if (actualRoute == null) return null;
    return actualRoute.getEndLinkId();
  }

  /**
   * If the next parking activity is skipped because the given constraints are not fulfilled, it
   * returns true.
   *
   * @return
   */
  ...
  private NetworkRoute findRouteToNearestParkingFacility(
      Id<Link> baseLinkId,
      Id<Link> currentLinkId,
      boolean canCheckParkingCapacitiesInAdvanced,
      double now,
    ...
    for (ActivityFacility activityFacility : activityFacilities.values()) {
      if (triedParking.size() == activityFacilities.size()) triedParking.clear();
      if (!passangerInteractionAtParkingFacilityAtEndOfLeg
          && triedParking.contains(activityFacility.getId())) continue;
      if (canCheckParkingCapacitiesInAdvanced) {
        if (((FacilityBasedParkingManager) parkingManager)
                .getNrOfFreeParkingSpacesOnLink(activityFacility.getLinkId())
            < 1) continue;
      }
      double latestEndOfParking = now + maxParkingDuration;
      // checks if parking slot is now open and still open when the parking will finish; if no
      // openingTime is set, we assume that it is open the hole day
      ActivityOption parkingOptions = activityFacility.getActivityOptions().get("parking");
      if (!parkingOptions.getOpeningTimes().isEmpty()) {
        if (parkingOptions.getOpeningTimes().first().getStartTime() > now
            && parkingOptions.getOpeningTimes().first().getEndTime() < latestEndOfParking) continue;
      }
      // check if approx. the max parking time at facility will not exceed, assumption: "parking
      // duration - 30 minutes" is parking Time.
      if (activityFacility
          .getAttributes()
          .getAsMap()
          .containsKey("maxParkingDurationInHours")) { // TODO vielleicht etwas sparsamer machen
        double maxParkingDurationAtFacility =
            3600
                * (double)
                    activityFacility.getAttributes().getAsMap().get("maxParkingDurationInHours");
        if (maxParkingDuration - 30 * 60 > maxParkingDurationAtFacility) continue;
      }

      // TODO beschreiben was passiert
      if (passangerInteractionAtParkingFacilityAtEndOfLeg) {
        double distanceBetweenThisParkingFacilityAndTheAttraction =
            NetworkUtils.getEuclideanDistance(activityFacility.getCoord(), coordOfAttraction);
        if (distanceBetweenThisParkingFacilityAndTheAttraction - distanceFromBaseToAttraction
            > maxDistanceFromBase) continue;
      }
      // create Euclidean distances to the parking activities to find routes only to the nearest
      // facilities in the next step
      ...
      double distanceBaseAndFacility =
          NetworkUtils.getEuclideanDistance(activityFacility.getCoord(), coordBaseLink);
      double distanceCurrentAndFacility;
      if (passangerInteractionAtParkingFacilityAtEndOfLeg) distanceCurrentAndFacility = 0.;
      else
        distanceCurrentAndFacility =
            NetworkUtils.getEuclideanDistance(activityFacility.getCoord(), coordCurrentLink);
    ...
    // selects the parking facility with the minimum travel time; only investigates the nearest
    // facilities
    for (ActivityFacility activityFacility : euclideanDistanceToParkingFacilities.values()) {
        ...
        double maxParkingDurationAtFacility =
            3600
                * (double)
                    activityFacility.getAttributes().getAsMap().get("maxParkingDurationInHours");
        double expectedTravelTimeFromParkingToBase =
            getExpectedTravelTime(baseLinkId, now, activityFacility.getLinkId());
        double expectedTravelTimeFromCurrentToParking =
            getExpectedTravelTime(activityFacility.getLinkId(), now, currentLinkId);
        double expectedParkingTime =
            maxParkingDuration
                - expectedTravelTimeFromCurrentToParking
                - expectedTravelTimeFromParkingToBase;
        if (expectedParkingTime > maxParkingDurationAtFacility) continue;
      }
      counter++;
      if (passangerInteractionAtParkingFacilityAtEndOfLeg
          && euclideanDistanceToParkingFacilities.size() > triedParking.size())
        if (triedParking.contains(activityFacility.getId())) continue;
      if (passangerInteractionAtParkingFacilityAtEndOfLeg
          && euclideanDistanceToParkingFacilities.size() == triedParking.size())
        triedParking.clear();
      NetworkRoute possibleRoute =
          this.parkingRouter.getRouteFromParkingToDestination(
              activityFacility.getLinkId(), now, currentLinkId);
      // reason is that we expect that the driver will always take the nearest possible getOff point
      // to reduce the walk distance for the guests
      if (passangerInteractionAtParkingFacilityAtEndOfLeg) {
      ...
      if (!baseLinkId.equals(currentLinkId)) {
        NetworkRoute routeFromParkingToBase =
            this.parkingRouter.getRouteFromParkingToDestination(
                baseLinkId, now, activityFacility.getLinkId());
      ...
      if (counter == numberOfCheckedRoutes) break;
    }
  ...
  public double getExpectedTravelTime(
      Id<Link> destinationLinkId, double now, Id<Link> currentLinkId) {
    NetworkRoute possibleRoute =
        this.parkingRouter.getRouteFromParkingToDestination(destinationLinkId, now, currentLinkId);
