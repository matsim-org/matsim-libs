...
/**
 * @author schlenther
 *     <p>Benenson et al defined 3 phases of parking search OBSERVING: observation of parking
 *     situation while driving towards destination SEARCH_WHILE_APPROACH: estimating the amount of
 *     free parking lots on the way to destination and if applicable parking before arriving
 *     SEARCH_FOR_NEXT: taking the next free parking space if it isn't too far away from destination
 */
enum ParkingMode {
  DRIVING,
  OBSERVING,
  SEARCH_WHILE_APPROACH,
  SEARCH_FOR_NEXT
}
...
public class BenensonDynLeg extends ParkingDynLeg {
  ...
  public void movedOverNode(Id<Link> newLinkId) {
    ...
    if (this.legStage == ParkingMode.DRIVING) {
      ...
      if (((BenensonParkingSearchLogic) this.logic)
          .transitionToObservingBehaviour(currentLinkId, this.route.getEndLinkId())) {
        ...
        if (logForDebug)
          logger.error(
              "vehicle " + this.vehicleId + " goes into observing on link " + this.currentLinkId);
      }
    }
    if (this.legStage == ParkingMode.OBSERVING) {
      ...
      if (((BenensonParkingSearchLogic) this.logic)
          .transitionToParkingBehaviour(currentLinkId, this.route.getEndLinkId())) {
        this.legStage = ParkingMode.SEARCH_WHILE_APPROACH;
        if (logForDebug)
          logger.error(
              "vehicle " + this.vehicleId + " goes into parking on link " + this.currentLinkId);
      }
    }
    if (this.legStage == ParkingMode.SEARCH_WHILE_APPROACH) {
      if (currentLinkId.equals(route.getEndLinkId())) {
        ...
        if (memorizeParkingSituationAndIsSomethingFree()) {
          ...
          if (((BenensonParkingSearchLogic) this.logic)
              .wantToParkHere(pUnoccupied, currentLinkId, route.getEndLinkId())) {
            if (logForDebug)
              logger.error(
                  "vehicle "
                      + this.vehicleId
                      + " would like to park on link"
                      + currentLinkId
                      + "\n \t pUnoccupied = "
                      + pUnoccupied
                      + "\n\t totalObservedParkingSpaces = "
                      + totalObservedParkingSpaces
                      + "\n\t observedFreeSpaces = "
                      + this.observedFreeParkingSpaces);
            hasFoundParking =
    ...
    if (this.legStage == ParkingMode.SEARCH_FOR_NEXT) {
      ...
      // if( ((BenensonParkingSearchLogic)this.logic).isDriverInAcceptableDistance(currentLinkId,
      // route.getEndLinkId(), this.firstDestLinkEnterTimer, timer.getTimeOfDay()) ){
      ...
      if (logForDebug)
        logger.error(
            "vehicle "
                + this.vehicleId
                + " tries in PHASE3 to park on link "
                + this.currentLinkId
                + ", "
                + (int) (timer.getTimeOfDay() - this.firstDestinationLinkEnterTime) / 60
                + ":"
                + (int) (timer.getTimeOfDay() - this.firstDestinationLinkEnterTime) % 60
                + " min after passing destination. Result: "
                + hasFoundParking);
      // }
    }
  }
  ...
  private boolean memorizeParkingSituationAndIsSomethingFree() {
    this.totalObservedParkingSpaces +=
        ((FacilityBasedParkingManager) this.parkingManager)
            .getNrOfAllParkingSpacesOnLink(currentLinkId);
    double freespaces =
        ((FacilityBasedParkingManager) this.parkingManager)
            .getNrOfFreeParkingSpacesOnLink(currentLinkId);
      ...
      if (hasFoundParking) {
        if (logForDebug)
          logger.error(
              "vehicle "
                  + this.vehicleId
                  + " has found a parking on link "
                  + this.currentLinkId
                  + " after passing "
                  + Math.abs((this.route.getLinkIds().size() - this.currentLinkIdx - 3))
                  + " links");
        return null;
      } else {
        ...
        if (this.legStage == ParkingMode.SEARCH_FOR_NEXT) {
          nextLinkId =
              ((BenensonParkingSearchLogic) this.logic)
                  .getNextLinkRandomInAcceptableDistance(
                      currentLinkId,
                      this.route.getEndLinkId(),
                      vehicleId,
                      firstDestinationLinkEnterTime,
                      this.timer.getTimeOfDay(),
                      mode);
        } else {
          nextLinkId =
              ((BenensonParkingSearchLogic) (this.logic))
                  .getNextLinkBenensonRouting(currentLinkId, route.getEndLinkId(), mode);
        }
