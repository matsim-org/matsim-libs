...
public class ParkingListener
    implements IterationEndsListener, MobsimBeforeSimStepListener, MobsimInitializedListener {

  @Inject ParkingSearchManager manager;
  @Inject OutputDirectoryHierarchy output;
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    writeStatsByTimesteps(
        ((FacilityBasedParkingManager) manager).produceTimestepsStatistics(), event.getIteration());
  ...
  private void writeStatsByTimesteps(List<String> produceBeneStatistics, int iteration) {
    BufferedWriter bw =
        IOUtils.getBufferedWriter(
            output.getIterationFilename(iteration, "parkingStatsPerTimeSteps.csv"));
    try {
  ...
  private void writeStats(List<String> produceStatistics, int iteration) {
    BufferedWriter bw =
        IOUtils.getBufferedWriter(output.getIterationFilename(iteration, "parkingStats.csv"));
    try {
  ...
  public void notifyMobsimBeforeSimStep(MobsimBeforeSimStepEvent event) {
    ((FacilityBasedParkingManager) manager)
        .checkFreeCapacitiesForWaitingVehicles(
            (QSim) event.getQueueSimulation(), event.getSimulationTime());
  }
