...
public class ParkingSearchConfigGroup extends ReflectiveConfigGroup {
  ...
  public enum ParkingSearchManagerType {
  ...
  private ParkingSearchManagerType parkingSearchManagerType =
      ParkingSearchManagerType.FacilityBasedParkingManager;

  private static final String FRACTION_CAN_CHECK_FREE_CAPACITIES_IN_ADVANCED =
      "fractionCanCheckFreeCapacitiesInAdvanced";
  private double fractionCanCheckFreeCapacitiesInAdvanced = 0.;

  private static final String FRACTION_CAN_RESERVE_PARKING_IN_ADVANCED =
      "fractionCanReserveParkingInAdvanced";
  ...
  public void setFractionCanCheckFreeCapacitiesInAdvanced(
      double fractionCanCheckFreeCapacitiesInAdvanced) {
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        PARKINGSEARCH_STRATEGY,
        "The strategy to find a parking slot. Possible strategies: "
            + Arrays.toString(ParkingSearchStrategy.values()));
    map.put(
        PARKINGSEARCH_MANAGER,
        "The type of the ParkingManager, may have the values: "
            + Arrays.toString(ParkingSearchManagerType.values()));
    map.put(
    ...
    map.put(
        FRACTION_CAN_RESERVE_PARKING_IN_ADVANCED,
  ...
  protected void checkConsistency(Config config) {
    ...
    if (getFractionCanCheckFreeCapacitiesInAdvanced() != 0.
        && !getParkingSearchManagerType()
            .equals(ParkingSearchManagerType.FacilityBasedParkingManager))
      log.warn(
          "Fraction of agents who can check free capacities in advanced has no impact on your selected ParkingSearchManagerType, because it is only implemented for the FacilityBasedParkingManager.");

    if (getFractionCanCheckFreeCapacitiesInAdvanced() + getFractionCanReserveParkingInAdvanced()
        > 1.0)
