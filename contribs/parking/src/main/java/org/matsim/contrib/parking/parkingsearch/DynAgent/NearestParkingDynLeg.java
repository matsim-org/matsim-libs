...
public class NearestParkingDynLeg extends ParkingDynLeg {
  ...
  public NearestParkingDynLeg(
      Leg currentPlannedLeg,
      NetworkRoute route,
      Plan plan,
      int planIndexNextActivity,
    ...
    if (ParkingUtils.checkIfActivityHasNoParking(followingActivity)) parkingAtEndOfLeg = false;
    ...
    if (!parkingMode) {
      if (currentLinkId.equals(this.getDestinationLinkId())) {
        if (!parkingAtEndOfLeg) {
          ...
          hasFoundParking =
              parkingManager.reserveSpaceIfVehicleCanParkHere(vehicleId, currentLinkId);
          if (hasFoundParking) {
            this.events.processEvent(
                new ReserveParkingLocationEvent(
                    timer.getTimeOfDay(), vehicleId, currentLinkId, currentLinkId));
            nextSelectedParkingLink = currentLinkId;
          } else
            ((FacilityBasedParkingManager) parkingManager)
                .registerRejectedReservation(timer.getTimeOfDay());
        }
      }
    } else if (followingActivity.getLinkId().equals(newLinkId)) {
      if (alreadyReservedParking) hasFoundParking = true;
      else {
  ...
  public Id<Link> getNextLinkId() {
    ...
    if (!parkingMode && !reachedDestinationWithoutParking && !driveToBaseWithoutParking) {
      ...
      if (passangerInteractionAtParkingFacilityAtEndOfLeg
          && !hasFoundParking
          && followingActivity.getLinkId().equals(currentLinkId)) {
        createWaitingActivityUntilPassengerInteractionIsPossible(
            currentLinkId, vehicleId, timer.getTimeOfDay());
        this.events.processEvent(
            new StartWaitingForParkingEvent(timer.getTimeOfDay(), vehicleId, currentLinkId));
      }
      if (hasFoundParking || reachedDestinationWithoutParking) {
        ...
        if (hasFoundParking && !passangerInteractionAtParkingFacilityAtEndOfLeg) {
          ...
          double drivingDurationFromGetOff =
              timer.getTimeOfDay() - currentPlannedLeg.getDepartureTime().seconds();

          if (nextSelectedParkingLink.equals(currentLinkId)) {
            expectedDrivingDurationToPickup =
                ((NearestParkingSpotSearchLogic) this.logic)
            ...
            expectedDrivingDurationToPickup =
                ((NearestParkingSpotSearchLogic) this.logic)
                    .getExpectedTravelTime(
                        currentPlannedLeg.getRoute().getStartLinkId(),
                        timer.getTimeOfDay(),
                        currentLinkId);
          }
          parkingDuration =
              followingActivity.getMaximumDuration().seconds()
                  - drivingDurationFromGetOff
                  - expectedDrivingDurationToPickup
        ...
        if (passangerInteractionAtParkingFacilityAtEndOfLeg) {
          ...
          nextPickupTime =
              currentPlannedLeg.getDepartureTime().seconds()
                  + followingActivity.getMaximumDuration().seconds();
        ...
        if (((NearestParkingSpotSearchLogic) this.logic).isNextParkingActivitySkipped()
            && parkingAtEndOfLeg) {
        ...
        if (!driveToBaseWithoutParking
            && !((NearestParkingSpotSearchLogic) this.logic).isUseRandomLinkChoice()) {
          Id<Link> nextPlanedParkingLink =
              ((NearestParkingSpotSearchLogic) this.logic).getNextParkingLocation();
          if (nextSelectedParkingLink == null
              || !nextSelectedParkingLink.equals(nextPlanedParkingLink)) {
            nextSelectedParkingLink = nextPlanedParkingLink;
            if (((NearestParkingSpotSearchLogic) this.logic).canReserveParkingSlot()) {
              alreadyReservedParking =
                  parkingManager.reserveSpaceIfVehicleCanParkHere(
                      vehicleId, nextSelectedParkingLink);
              if (alreadyReservedParking)
                this.events.processEvent(
                    new ReserveParkingLocationEvent(
                        timer.getTimeOfDay(), vehicleId, currentLinkId, nextSelectedParkingLink));
              else
                ((FacilityBasedParkingManager) parkingManager)
                    .registerRejectedReservation(timer.getTimeOfDay());
            } else {
              this.events.processEvent(
                  new SelectNewParkingLocationEvent(
                      timer.getTimeOfDay(), vehicleId, currentLinkId, nextSelectedParkingLink));
            }
  ...
  private void createWaitingActivityUntilPassengerInteractionIsPossible(
      Id<Link> newLinkId, Id<Vehicle> vehicleId, double now) {
    Activity waitingActivity =
        PopulationUtils.createActivityFromLinkId(
            ParkingUtils.WaitingForParkingActivityType, newLinkId);
    ...
    ((FacilityBasedParkingManager) parkingManager)
        .addVehicleForWaitingForParking(newLinkId, vehicleId, now);
  }
  ...
  private void removeNextActivityAndFollowingLeg() {
    ...
    plan.getPlanElements().remove(planIndexNextActivity);
    //		log.info(
    //			plan.getPerson().getId().toString() + ": Parking activity after getOff point '" +
    // ((Activity)plan.getPlanElements().get(planIndexNextActivity - 2)).getType() + "' is removed,
    // because no parking facility was found.");
  }
