...
/**
 * Before the mobsim, gets the current {@linkplain PenaltyCalculator} from the provided {@linkplain
 * PenaltyGenerator} and resets that latter so it can - theoretically - gather information during
 * the new iteration. However, it is your own responsibility to feed it with data. The penalty for
 * each space-time-gridcell with non-zero penalty is dumped for each iteration. In the zeroth
 * iteration, {@linkplain PenaltyCalculator#getDummyHourCalculator()} is used to calculate the
 * penalties.
 *
...
class CarEgressWalkObserver implements BeforeMobsimListener {
  ...
  /**
   * Sets the class up with the {@linkplain PenaltyCalculator.DefaultPenaltyFunction} and the
   * specified {@linkplain PenaltyGenerator}.
   *
   * @param penaltyGenerator
   */
  public CarEgressWalkObserver(
      PenaltyGenerator penaltyGenerator,
      PenaltyFunction penaltyFunction,
      PenaltyCalculator iter0PenaltyCalculator) {
  ...
  public void notifyBeforeMobsim(BeforeMobsimEvent event) {
    ...
    String file =
        event
            .getServices()
            .getControlerIO()
            .getIterationFilename(event.getIteration(), OUTFILE_PENALTIES);
