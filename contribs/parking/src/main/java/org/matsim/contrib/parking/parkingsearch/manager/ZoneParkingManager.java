...
public class ZoneParkingManager extends FacilityBasedParkingManager {
  ...
  /**
   * reads in a tabular file that declares which link id's are in the monitored zone the part
   * between the last '/' and the file type extension in the given path is considered to be the zone
   * name
   *
  ...
  void readZone(String pathToZoneFile) {
    String zone =
        pathToZoneFile.substring(
            pathToZoneFile.lastIndexOf("/") + 1, pathToZoneFile.lastIndexOf("."));
        ...
        if (linksOfZone.get(zone).contains(linkId) && this.facilitiesPerLink.containsKey(linkId)) {
          ...
          if (this.totalCapOfZone.get(zone) < newOcc) {
            ...
            String u =
                "Capacity: "
                    + this.parkingFacilities
                        .get(this.parkingLocations.get(vehicleId))
                        .getActivityOptions()
            ...
            throw new RuntimeException(
                "occupancy of zone "
                    + zone
                    + " is higher than 100%. Capacity= "
                    + this.totalCapOfZone.get(zone)
                    + "  occupancy="
                    + newOcc
                    + "time = "
                    + time
    ...
    if (!this.parkingLocations.containsKey(vehicleId)) {
      ...
      for (String zone : this.linksOfZone.keySet()) {
        if (linksOfZone.get(zone).contains(parkingLink)) {
          double newOcc = this.occupationOfZone.get(zone) - 1;
          if (newOcc < 0) {
            // in iteration 0 agents can "leave parking spaces" (get into traffic), but the manager
            // didn't record them to be parked
            newOcc = 0;
          }
          this.occupationOfZone.put(zone, newOcc);
        }
  ...
  public double getOccupancyRatioOfZone(String zone) {
    if (!(this.linksOfZone.keySet().contains(zone)))
      throw new RuntimeException(
          "zone " + zone + " was not defined. thus, could'nt calculate occupancy ratio.");
