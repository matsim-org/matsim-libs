...
/**
 * @author schlenther
 *     <p>the matsim version of the parking strategy used in PARKAGENT. see the following paper for
 *     more information: doi: 10.1016/j.compenvurbsys.2008.09.011
 */
public class BenensonParkingSearchLogic implements ParkingSearchLogic {
  ...
  public boolean transitionToObservingBehaviour(Id<Link> currLinkId, Id<Link> endLinkId) {
    double distToDest =
        NetworkUtils.getEuclideanDistance(
            network.getLinks().get(currLinkId).getCoord(),
   ...
   * @return
   */
  public Id<Link> getNextLinkBenensonRouting(
      Id<Link> currentLinkId, Id<Link> destinationLinkId, String mode) {
  ...
  public Id<Link> getNextLinkRandomInAcceptableDistance(
      Id<Link> currentLinkId,
      Id<Link> endLinkId,
      Id<Vehicle> vehicleId,
      double firstDestLinkEnterTime,
      double timeOfDay,
      String mode) {
    ...
    for (int i = 1; i <= nrOfOutGoingLinks; i++) {
      ...
      if (isDriverInAcceptableDistance(
          nextLink.getId(), endLinkId, firstDestLinkEnterTime, timeOfDay)) return nextLink.getId();
      outGoingLinks.remove(nextLink);
    }
    logger.error(
        "vehicle "
            + vehicleId
            + " finds no outlink in acceptable distance going out from link "
            + currentLinkId
            + ". it just takes a random next link");
  ...
  /**
   * estimate amount of free parking spaces on the way to destination Link and decide whether to
   * park on currentLink
   *
   * @param pUnoccupied
   * @param currentLinkId
  ...
  public boolean wantToParkHere(double pUnoccupied, Id<Link> currentLinkId, Id<Link> endLinkId) {
    ...
    // if pUnoccupied = 0, no free slot has been detected, so it is realistic to accept the very
    // next free slot
    double distToDest =
        NetworkUtils.getEuclideanDistance(
            network.getLinks().get(currentLinkId).getToNode().getCoord(),
            network.getLinks().get(endLinkId).getToNode().getCoord());
    ...
    if (logForDebug)
      logger.error(
          "\n current link: "
              ...
              + "\n expected slots: "
              + expectedFreeSlots
              + "\n probabilty to continue driving: "
              + getProbabilityOfContinuingToDrive(expectedFreeSlots)
              + "\n rnd: "
              + rnd);
  ...
  private double getProbabilityOfContinuingToDrive(double expectedFreeSlots) {
    ...
    return (expectedFreeSlots - MIN_THRESHOLD_PROB_FUNCTION)
        / (MAX_THRESHOLD_PROB_FUNCTION - MIN_THRESHOLD_PROB_FUNCTION);
  }
   ...
   * @return
   */
  private boolean isDriverInAcceptableDistance(
      Id<Link> currentLinkId, Id<Link> endLinkId, double firstDestLinkEnterTime, double timeOfDay) {
    ...
    double distToDest =
        NetworkUtils.getEuclideanDistance(
            network.getLinks().get(currentLinkId).getCoord(),
            network.getLinks().get(endLinkId).getCoord());
    double timeSpent = timeOfDay - firstDestLinkEnterTime;
    double acceptedDistance =
        ACCEPTED_DISTANCE_START + ACCEPTED_DISTANCE_INCREASING_RATE_PER_MIN * (timeSpent / 60);
