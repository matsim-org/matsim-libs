 ...
 * @author tschlenther
 */
public class ZoneParkingOccupationListener
  ...
  /** */
  @Inject
  public ZoneParkingOccupationListener(
      ParkingSearchManager manager, MatsimServices services, Config config) {
  ...
  public void notifyMobsimAfterSimStep(MobsimAfterSimStepEvent e) {
    if ((e.getSimulationTime() % 900) == 0 && !(e.getSimulationTime() == 0)) {
      ...
      if (!(this.allMonitoredTimeStamps.contains(timeStamp)))
        this.allMonitoredTimeStamps.add(timeStamp);
  ...
  public void notifyMobsimBeforeCleanup(MobsimBeforeCleanupEvent e) {
    String fileName =
        services.getControlerIO().getIterationFilename(iteration, "OccupationStats.csv");
    writeStatsVertical(fileName);
    double lastOcc;
    for (String zone : zoneManager.getZones()) {
      lastOcc = this.zoneManager.getOccupancyRatioOfZone(zone);
      //			lastOcc =
      // this.zoneOccupationPerHour.get(zone)[this.zoneOccupationPerHour.get(zone).length-1];
        ...
        for (int i = 0; i < zoneSize; i++) {
          ...
          for (int zz = 0; zz < timeSize; zz++) {
            ...
            if (time < ff.getTime()) {
              ...
              String timeString =
                  "This is what the TreeSet looks like:\n"
                      + timeSet.toString()
    ...
    for (String zone : this.zoneManager.getZones()) {
      ...
      for (int i = 0; i < numberOfSlots; i++) {
        ...
        if (slotTimes[i] == tuple.getTime()) {
          ...
          throw new RuntimeException(
              "timeSlot number is higher than recorded time. timeSlot: "
                  + slotTimes[i]
                  + "\t time: "
                  + tuple.getTime());
        }
