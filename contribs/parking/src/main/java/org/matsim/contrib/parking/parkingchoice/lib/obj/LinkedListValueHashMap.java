...
public class LinkedListValueHashMap<KeyClass, ValueClass> {
  ...
  HashMap<KeyClass, LinkedList<ValueClass>> hashMap =
      new HashMap<KeyClass, LinkedList<ValueClass>>();
  ...
  public void putAndSetBackPointer(KeyClass key, ValueClass value) {
    ...
    if (hashMapBackPointer.containsKey(value)) {
      throw new Error(
          "The backpointer concept only works for one-to-one mappings between key and value!");
    }
