...
/**
 * @author tschlenther
 *     <p>Agents drive to destination first. Knowledge about surrounding streets is assumed. If no
 *     parking slot is available, they always look for a slot on the one outgoing link that has the
 *     shortest distance to their destination and is unknown to them so far. If every outlink is
 *     known they choose the next link to search on randomly.
 */
public class DistanceMemoryParkingSearchLogic implements ParkingSearchLogic {
  ...
  public Id<Link> getNextLink(
      Id<Link> currentLinkId, Id<Link> destLinkId, Id<Vehicle> vehicleId, String mode) {
      ...
      if (this.knownLinks.contains(outLinkId)) nrKnownLinks++;
        ...
        double distToDest =
            NetworkUtils.getEuclideanDistance(
                outLink.getCoord(), network.getLinks().get(destLinkId).getCoord());
        if (distToDest < shortestDistance) {
          ...
          if (Math.random() > 0.5) nextLink = outLinkId;
          ...
          if (doLogging) {
            logger.info(
                "link "
                    + outLinkId
                    + " was not chosen because it is "
                    + distToDest
                    + "m away whereas shortest distance is "
                    + shortestDistance);
          }
        }
      }
