...
public class NearestParkingSpotAgentLogic extends ParkingAgentLogic {
  public NearestParkingSpotAgentLogic(
      Plan plan,
  ...
  public DynAction computeNextAction(DynAction oldAction, double now) {
    ...
    if (lastParkActionState.equals(LastParkActionState.CARTRIP)
        && ((NearestParkingDynLeg) oldAction).driveToBaseWithoutParking())
  ...
  protected DynAction nextStateAfterUnParkActivity(DynAction oldAction, double now) {
    ...
    if ((this.parkingManager.unParkVehicleHere(
      ...
      // leg to reflect this.
      return new NearestParkingDynLeg(
          currentPlannedLeg,
          actualRoute,
          plan,
          planIndexNextActivity,
          parkingLogic,
          parkingManager,
  ...
  protected DynAction nextStateAfterParkActivity(DynAction oldAction, double now) {
    ...
    else return nextStateAfterWalkFromPark(oldAction, now);
  }
  ...
  protected DynAction nextStateAfterActivity(DynAction oldAction, double now) {
    ...
    if (plan.getPlanElements().size() >= planIndex + 1) {
      if (plan.getPlanElements().get(planIndex) instanceof Activity
          && ((Activity) plan.getPlanElements().get(planIndex))
              .getType()
              .equals(ParkingUtils.WaitingForParkingActivityType)) {
        // now the waiting activity has finished and we can park now
        this.parkingManager.parkVehicleHere(
      ...
      if (plan.getPlanElements().get(planIndex + 1) instanceof Activity)
        return nextStateAfterNonCarTrip(oldAction, now);
      if (plan.getPlanElements().get(planIndex) instanceof Activity
          && ((Activity) plan.getPlanElements().get(planIndex)).getType().contains("_GetOff")) {
        ((Activity) plan.getPlanElements().get(planIndex)).setEndTime(now);
        ((Activity) plan.getPlanElements().get(planIndex + 4))
            .setStartTime(
                now
                    + ((Activity) plan.getPlanElements().get(planIndex + 2))
                        .getMaximumDuration()
                        .seconds());
        // checks if it is possible to stay from getOff until getIn
        boolean possibleToStay =
            checkIfParkingIsPossibleUntilNextActivities(this.planIndex, this.planIndex + 2);
        if (possibleToStay) return nextStateAfterNonCarTrip(oldAction, now);
      }
      planIndex++;
      ...
      if (currentLeg.getMode().equals(TransportMode.car)) {
        ...
        if (parkLink == null) {
          ...
          // information about initial stages. We suppose the car is parked where we are
  ...
  protected DynAction nextStateAfterCarTrip(DynAction oldAction, double now) {
    if (this.plan.getPlanElements().get(planIndex + 1) instanceof Activity
        && ((Activity) this.plan.getPlanElements().get(planIndex + 1))
            .getType()
            .equals(ParkingUtils.WaitingForParkingActivityType)) {
  ...
  protected DynAction nextStateAfterNonCarTrip(DynAction oldAction, double now) {
    ...
    if (nextPlannedActivity.getType().equals(ParkingUtils.ParkingActivityType)
        && plan.getPlanElements().get(planIndex + 2) instanceof Leg) {
  ...
  private boolean checkIfParkingIsPossibleUntilNextActivities(
    ...
    if (indexOfFollowingActivity == indexOfParkingActivity) {
      ...
      if (agent.getCurrentLinkId().equals(activityAfterFollowing.getLinkId())
          && !ParkingUtils.checkIfActivityHasNoParking((Activity) currentPlanElement)) {
        boolean canParkAtFacilityUntilGetIn =
            ((FacilityBasedParkingManager) parkingManager)
                .canParkAtThisFacilityUntilEnd(
                    agent.getCurrentLinkId(),
                    followingActivity.getMaximumDuration().seconds(),
                    currentActivity.getMaximumDuration().seconds(),
        ...
        if (canParkAtFacilityUntilGetIn) {
          ...
          ((FacilityBasedParkingManager) parkingManager)
              .registerStayFromGetOffUntilGetIn(this.agent.getVehicle().getId());
          return true;
        }
      }
    }
    // checks if the now started parking activity can extend until the end of the following GetIn
    // activity
      ...
      if (agent.getCurrentLinkId().equals(followingActivity.getLinkId())
          && !ParkingUtils.checkIfActivityHasNoParking(followingActivity)) {
        boolean canParkAtFacilityUntilGetIn =
            ((FacilityBasedParkingManager) parkingManager)
                .canParkAtThisFacilityUntilEnd(
                    agent.getCurrentLinkId(),
        ...
        if (canParkAtFacilityUntilGetIn) {
          ...
          ((FacilityBasedParkingManager) parkingManager)
              .registerParkingBeforeGetIn(this.agent.getVehicle().getId());
          return true;
        }
      }
    }
