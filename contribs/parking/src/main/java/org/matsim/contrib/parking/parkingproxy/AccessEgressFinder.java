...
class AccessEgressFinder {
  ...
  /**
   * Simple container class holding a leg and an act. Could be replaced by a Tuple, but I think the
   * explicit field names result in easier to read code.
   */
  ...
  private static enum EgressStage {
    leg,
    walk,
    activity
  }

  private static enum AccessStage {
    activity,
    walk,
    leg
  }
  ...
  /**
   * Finds all egress walks corresponding to the legmode and returns them together with the activity
   * <b>following</b> that walk.
   *
   * @param plan The plan in which to search for egress walks
   * @return A chronological list of {@linkplain LegActPair}s each containing the egress Leg and the
   *     following Activity.
   */
  public List<LegActPair> findEgressWalks(Plan plan) {
    ...
    EgressStage nextStage =
        EgressStage.leg; // first state is a leg, because we never start with an egress_walk.
    ...
    for (PlanElement element : plan.getPlanElements()) {
      if (element instanceof Activity) {
        ...
        if (nextStage == EgressStage.leg && leg.getMode().equals(this.legmode)) {
          ...
          // this is our egress walk (we wouldn't be here after another mode interaction). Save this
          // leg (we'll modify it soon).
  ...
  /**
   * Finds all access walks corresponding to the legmode and returns them together with the activity
   * <b>before</b> that walk.
   *
   * @param plan The plan in which to search for access walks
   * @return A chronological list of {@linkplain LegActPair}s each containing the access Leg and the
   *     previous Activity.
   */
  public List<LegActPair> findAccessWalks(Plan plan) {
    ...
    AccessStage nextStage =
        AccessStage
            .activity; // first state is an act, because we don't have access walks from nowhere.
    LegActPair pair = new LegActPair();
    for (PlanElement element : plan.getPlanElements()) {
      if (element instanceof Activity) {
        Activity act = (Activity) element;
        if (nextStage == AccessStage.activity) {
          // we preemptively store all activities because we don't know anything about how we leave
          // from here.
        ...
        if (nextStage == AccessStage.walk) {
          if (leg.getMode().equals(TransportMode.walk)) {
            // this may be our access walk. It's an access walk to somewhere, we'll find out about
            // the mode soon enough.
            pair.leg = leg;
