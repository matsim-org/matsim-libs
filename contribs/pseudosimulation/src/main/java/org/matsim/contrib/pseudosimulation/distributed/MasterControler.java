...
public class MasterControler
    implements AfterMobsimListener, ShutdownListener, StartupListener, IterationStartsListener {
  ...
  /**
   * value between 0 and 1; increasing it increases the dampening effect of preventing large
   * transfers of persons during load balance iterations
   */
  private static final double loadBalanceDampeningFactor = 0.4;

  private int currentIteration = -1;

  public MasterControler(String configFile)
      throws NumberFormatException, IOException, InterruptedException {
    config = ConfigUtils.loadConfig(configFile);
    final DistributedSimConfigGroup distributedSimConfigGroup =
        ConfigUtils.addOrGetModule(
            this.config, DistributedSimConfigGroup.GROUP_NAME, DistributedSimConfigGroup.class);
    ...
    SelectedSimulationMode =
        distributedSimConfigGroup.isSlavesRunInParallelToMaster()
            ? SimulationMode.PARALLEL
            : SimulationMode.SERIAL;
    slaveIterationsPerMasterIteration =
        distributedSimConfigGroup.getSlaveIterationsPerMasterIteration();
    fullTransitPerformanceTransmission =
        distributedSimConfigGroup.isFullTransitPerformanceTransmission();
    ...
    for (int i = 0; i < initialNumberOfSlaves; i++) {
      ...
      System.out.println(
          "Slave " + (i + 1) + " out of an initial " + initialNumberOfSlaves + " accepted.\n");
    ...
    writeServer.close();
    masterInitialLogString.append(
        "MASTER accepted minimum number of incoming connections. All further slaves will be registered on the Hydra.\n");
    hydra = new Hydra();
    ...
    List<Person>[] personSplit =
        (List<Person>[])
            CollectionUtils.split(scenario.getPopulation().getPersons().values(), initialWeights);
    j = 0;
    for (int i : slaveHandlerTreeMap.keySet()) {
    ...
    if (this.config.transit().isUseTransit()) {
      //            waitTimeCalculator = new
      // WaitTimeCalculatorSerializable(matsimControler.getScenario().getTransitSchedule(),
      // this.config.travelTimeCalculator().getTraveltimeBinSize(),
      //                    (int) (this.config.qsim().getEndTime().seconds() -
      // this.config.qsim().getStartTime().seconds()));
      //            matsimControler.getEvents().addHandler(waitTimeCalculator);
      //            stopStopTimeCalculator = new
      // StopStopTimeCalculatorSerializable(matsimControler.getScenario().getTransitSchedule(),
      //                    this.config.travelTimeCalculator().getTraveltimeBinSize(), (int)
      // (this.config.qsim()
      ...
      if (fullTransitPerformanceTransmission) {
        transitPerformanceRecorder =
            new TransitPerformanceRecorder(scenario, matsimControler.getEvents());
      }
    }
    ...
    matsimControler.addOverridingModule(
        new AbstractModule() {
          @Override
          public void install() {
            addPlanStrategyBinding("ReplacePlanFromSlave")
                .toProvider(new ReplacePlanFromSlaveFactory(newPlans));
          }
        });
   ...
   * @param borrowingRate
   */
  private void setReplanningWeights(
      Config config, double masterMutationRate, double borrowingRate) {
    ...
    for (int i = 0; i < strategySettings.size(); i++) {
      ...
      if (DistributedPlanStrategyTranslationAndRegistration.SupportedSelectors.keySet()
          .contains(setting.getStrategyName())) selectors.put(i, setting.getWeight());
      else {
        mutators.put(i, setting.getWeight());
        maximumIterationForMutationDisabling =
            Math.max(setting.getDisableAfter(), maximumIterationForMutationDisabling);
      }
    }
    ...
    for (Entry<Integer, Double> entry : selectors.entrySet()) {
      strategySettings
          .get(entry.getKey())
          .setWeight((1 - masterMutationRate - borrowingRate) * entry.getValue() / selectorSum);
    }
    for (Entry<Integer, Double> entry : mutators.entrySet()) {
      strategySettings
          .get(entry.getKey())
          .setWeight(masterMutationRate * entry.getValue() / mutatorSum);
    }
  ...
  public void startSlaveHandlersInMode(CommunicationsMode mode) {
    if (numThreads.get() > 0)
      masterLogger.warn(
          "All slaveHandlers have not finished previous operation but they are being asked to "
              + mode.toString());
  ...
  public void waitForSlaveThreads() {
    masterLogger.warn("Waiting for " + numThreads.get() + " slaveHandlers");
  ...
  public void notifyStartup(StartupEvent event) {
    ...
    if (initialRoutingOnSlaves) {
      ...
      if (slaveHandlerTreeMap.size() > 1
          || slavesHaveRequestedShutdown()
          || hydra.hydraSlaves.size() > 0) loadBalance();
  ...
  public void notifyIterationStarts(IterationStartsEvent event) {
    ...
    if (innovationEndsAtIter > 0 && event.getIteration() > innovationEndsAtIter) return;
  ...
  public void notifyAfterMobsim(AfterMobsimEvent event) {
    ...
    boolean isLoadBalanceIteration =
    ...
    if (isLoadBalanceIteration) loadBalance();
    isLoadBalanceIteration = false;
    waitForSlaveThreads();
    linkTravelTimes =
        new SerializableLinkTravelTimes(
            matsimControler.getLinkTravelTimes(),
  ...
  private boolean slavesHaveRequestedShutdown() {
    for (SlaveHandler slaveHandler : slaveHandlerTreeMap.values()) {
      if (!slaveHandler.isOkForNextIter) return true;
    }
    return false;
  }
  ...
  private void loadBalance() {
    ...
    if (slaveHandlerTreeMap.size() < 2) return;
    ...
    if (getTotalNumberOfPlansFromSlaves() > 0) {
      ...
      bytesPerPerson =
          getTotalSlavePopulationMemoryUse() / scenario.getPopulation().getPersons().size();
    }
  ...
  private static Map<Integer, Integer> getOptimalNumbers(
      int popSize, double[] timesPerPlan, Set<Integer> validSlaveIndices) {
    ...
    while (total > popSize) {
      for (int i : validSlaveIndices) {
        ...
        total--;
        if (total == popSize) break;
      }
    }
    return output;
  }

  public static int[] getSlaveTargetPopulationSizes(
    ...
    for (int i = 0; i < numSlaves; i++) {
      if (personsPerSlave[i] > 0 && totalIterationTime[i] > 0) {
        ...
        if (timesPerPlan[i] < fastestTimePerPlan) fastestTimePerPlan = timesPerPlan[i];
      } else newSlaves.add(i);
    ...
    fastestTimePerPlan =
        fastestTimePerPlan > 0
                && !Double.valueOf(fastestTimePerPlan).equals(Double.POSITIVE_INFINITY)
            ? fastestTimePerPlan
            : 1;
    for (int i : newSlaves) timesPerPlan[i] = fastestTimePerPlan;
    ...
    while (remainder > 0 && validSlaveIndices.size() > 0) {
      ...
      while (!isAllTrue(fullyAllocated)) {
        for (int i : valid) {
              ...
              (long)
                  (maxMemory[i]
                      - (planAllocationLimiter
                          * (optimalNumberPerSlave.get(i) + allocation[i])
                          * bytesPerPlan));
          ...
          if (optimalNumberPerSlave.get(i) > maxPersonAllocation) {
            ...
            if (optimalNumberPerSlave.get(i) > personsPerSlave[i]
                && optimalNumberPerSlave.get(i) > 10) {
              int dampenedNumber =
                  (int)
                      (((1 - dampeningFactor) * (double) optimalNumberPerSlave.get(i))
                          + (dampeningFactor
                              * (double) Math.min(personsPerSlave[i], maxPersonAllocation)));
      ...
      for (int i : valid) {
        ...
        if (allocation[i] <= 0) {
          masterLogger.error(
              "Something went wrong during loadBalancing (allocation <=0). Continuing as-is for now...");
          return personsPerSlave;
        }
      }
      if (validSlaveIndices.size() == 0 && remainder > 0) {
        masterLogger.error(
            "All slaves are nearing their maximum memory capacity!! Probably not a sustainable situation...");
        planAllocationLimiter--;
    ...
    lines =
        new String[] {
          "slave", "time per plan", "pax per slave", "allocation", "memUSED_MB", "memAVAIL_MB"
        };
    sb.append("\n");
    for (int j = 0; j < 6; j++) {
  ...
  private class SlaveHandler implements Runnable {
    ...
    public void transmitPlans() throws IOException, ClassNotFoundException {
      ...
      slaveLogger.warn(
          String.format(
              "Plan signature: M%03dP%03dT%03d ",
              currentIteration + 1, slaveIteration, timesIteration));
      slaveLogger.warn(
          "(M = iteration for execution on master,P = PSim iteration when plan came from on slave, T = travel time iteration from master used to generate plan on slave)");
      Map<String, PlanSerializable> serialPlans =
          (Map<String, PlanSerializable>) reader.readObject();
      ...
      for (Entry<String, PlanSerializable> entry : serialPlans.entrySet()) {
        plans.put(
            entry.getKey(),
            entry.getValue().getPlan(matsimControler.getScenario().getPopulation()));
      }
    ...
    public void poolPersons() throws IOException, ClassNotFoundException {
      ...
      slaveLogger.warn(
          "Currently has " + currentPopulationSize + " persons, target is " + targetPopulationSize);
    ...
    public void run() {
      try {
        slaveLogger.warn(
            "SlaveHandler " + myNumber + " entering comms mode: " + communicationsMode.toString());
      ...
      numThreads.decrementAndGet();
      slaveLogger.warn(
          "SlaveHandler " + myNumber + " leaving comms mode: " + communicationsMode.toString());
    }

    private void transmitScores() throws IOException, ClassNotFoundException {
      slaveScoreStats.insertEntry(
          currentIteration,
  ...
  private void initializeSlave(SlaveHandler slaveHandler, int i, boolean initialRoutingOnSlaves)
      throws IOException {
