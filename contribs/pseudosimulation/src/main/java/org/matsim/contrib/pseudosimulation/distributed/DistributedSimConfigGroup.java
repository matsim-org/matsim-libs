...
public class DistributedSimConfigGroup extends ReflectiveConfigGroup {
  ...
  /**
   * The number of slaves the master will distribute to from the start. Defaults to 2, each with one
   * thread for replanning. You can still add more afterwards and load balancing should take care of
   * it.
   *
   * <p>Override the default number of threads on the slave with a
   */
  ...
  /**
   * Overall rate of innovation on slaves per slave iteration. Total number of plans produced per
   * master iteration across all slaves is slaveMutationRate * slaveIterationsPerMasterIteration.
   */
  ...
  /**
   * if false, randomised routing takes place. Should be modified to allow injection of custom
   * routers...
   */
  ...
  /**
   * If true, a data structure recording each vehicle's arrivals and departures at each stop in the
   * master simulation is transmitted to the slaves, instead of an average StopStopTime that gives
   * stop-to-stop travel time for time bins between each pair of stops.
   */
  ...
  /**
   * If in parallel, the slaves run while the Master is sunning QSim. That means they operate on
   * information that is 2 iterations older than the master's current iteration. If not in parallel,
   * slaves wait or the latest travel times from the master. The master, in turn, waits for all
   * slaves to complete the requisite number of slaveIterationsPerMasterIteration. Slaves then
   * operate on information one iteration older than the master's current iteration, i.e. similar to
   * default MATSim.
   */
