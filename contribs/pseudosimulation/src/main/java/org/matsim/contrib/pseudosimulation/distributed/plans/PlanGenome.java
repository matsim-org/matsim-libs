 ...
 *
 * <p>Plan class that records the various manipulations it has undergone (for diagnostic purposes),
 * as well as its score in PSim
 */
public class PlanGenome implements Plan {
  private static final Logger log = LogManager.getLogger(PlanGenome.class);
  ...
  @Deprecated // use scenario.getPopulation().getFactory().createActivity(...) instead, and add it
  // yourself
  ...
  @Deprecated // use scenario.getPopulation().getFactory().createLeg(...) instead, and add it
  // yourself
   ...
   *
   * <ul>
   *   <li>first act: removes the act and the following leg
   *   <li>last act: removes the act and the previous leg
   *   <li>in-between act: removes the act, removes the previous leg's route, and removes the
   *       following leg.
   * </ul>
   *
   * @param index
   ...
   *
   * <ul>
   *   <li>In my intuition, this is really a terrible method: (1) Plan is a data object, not a
   *       behavioral object, and thus it should be accessed from static, interface-based methods
   *       only. (2) It is not clear about the fact if it is doing a deep or a shallow copy. The
   *       only excuse is that this is one of the oldest parts of matsim. kai, jan'13
   * </ul>
   *
        ...
        if (pe instanceof Leg) {
          ...
          l2.setTravelTime(
              r.getDepartureTime().seconds()
                  + r.getTravelTime().seconds()
                  - l2.getDepartureTime().seconds());
        }
        if (l.getRoute() != null) {
   ...
   */
  public void insertLegAct(final int pos, final Leg leg, final Activity act)
