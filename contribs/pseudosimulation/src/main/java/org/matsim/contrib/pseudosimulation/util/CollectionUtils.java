...
/**
 * @author illenberger
 *     <p>Used to divide tasks between threads in {@link PSim}. Copied here to avoid dependence.
 */
public class CollectionUtils {
   ...
   * @param weights is an array of weights that will be normalized, so sum can be larger than one,
   *     but all should be positive
   * @return an array of lists each containing a number of objects from the collection, the number
   *     determined by the weight relative to total weight
   */
  public static <T> List<T>[] split(Collection<T> set, double[] weights) {
    if (set.size() >= weights.length) {
      ...
      for (int i = 0; i < weights.length; i++) relativeWeight[i] = weights[i] / totalweight;
      int[] listSizes = new int[weights.length];
      for (int i = 0; i < weights.length; i++)
        listSizes[i] = (int) (weights[i] / totalweight * (double) set.size());
      int allocated = 0;
      for (int ls : listSizes) allocated += ls;
