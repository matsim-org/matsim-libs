...
public class TransitPerformance implements Serializable {
  ...
  public void addVehicleDwellEventAtStop(
      Id<TransitLine> line,
  ...
  public Tuple<Double, Double> getRouteTravelTime(
      Id<TransitLine> line,
      Id<TransitRoute> route,
      Id<TransitStopFacility> originStop,
      Id<TransitStopFacility> destinationStop,
      double time) {
    try {
      return linesToStopDwellEvents
          .get(line.toString())
          .getRouteTravelTime(route, originStop, destinationStop, time);
  ...
  private class DwellEventsAtStop implements Serializable {
    ...
    public Tuple<Double, Double> getTravelTime(
        Id<TransitStopFacility> destinationStop, double time) {
      ...
      for (DwellEvent dwellEvent : dwellEvents) {
        lastInvehicleTime.add(
            dwellEvent.getVehicle().getInVehicleTime(dwellEvent, destinationStop));
        ...
        if (dwellEvent.getArrivalTime() >= time) {
          ...
          //                        inVehicleTime = new NormalDistributionImpl(mean,
          // Math.sqrt(Descriptive.sampleVariance(doubleArrayList,mean))).inverseCumulativeProbability(MatsimRandom.getRandom().nextDouble());
          ...
          //                    inVehicleTime = dwellEvent.getVehicle().getInVehicleTime(dwellEvent,
          // destinationStop);
        }
  ...
  private class DwellEventsForLine implements Serializable {
    ...
    private void addVehicleDwellEventAtStop(
        Id<TransitRoute> route, Id<TransitStopFacility> stopId, DwellEvent dwellEvent) {
    ...
    public Tuple<Double, Double> getRouteTravelTime(
        ...
        Id<TransitStopFacility> destinationStop,
        double time) {
      return routesToDwellEvents
          .get(route.toString())
          .getTravelTime(originStop, destinationStop, time);
    }
  }
}
