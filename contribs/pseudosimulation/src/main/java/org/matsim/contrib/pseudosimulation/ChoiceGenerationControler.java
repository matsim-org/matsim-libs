...
/**
 * Created by pieterfourie on 4/20/15. A class for artemc to where you load the travel time
 * structures with events, and psim repeatedly executes using those fixed times. so produces the
 * plans that are optimal if each agent were the only one to adapt to the system, i.e. if their
 * change did not affect other traffic.
 */
public class ChoiceGenerationControler implements BeforeMobsimListener {
  ...
  public ChoiceGenerationControler(String[] args) {
    ...
    //            (int) (controler.getConfig().qsim().getEndTime().seconds() -
    // controler.getConfig().qsim().getStartTime().seconds()));
    ...
    //        @Override
    //        public void install() {
    //            bind(TransitRouter.class).toProvider(new
    // TransitRouterEventsWSFactory(controler.getScenario(),
    ...
    //            new
    // CharyparNagelOpenTimesScoringFunctionFactory(controler.getConfig().planCalcScore(),
            ...
            bindMobsim()
                .toProvider(
                    new Provider<Mobsim>() {
                      @Override
                      public Mobsim get() {
                        return pSimProvider.createMobsim(
