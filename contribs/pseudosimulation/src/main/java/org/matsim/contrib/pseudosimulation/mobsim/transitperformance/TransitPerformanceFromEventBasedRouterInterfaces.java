...
/**
 * An attempt to carve out the WaitTime/StopStopTime based transit emulation. Largely cut & paste
 * from PSim. Not tested with an actual model.
 *
...
public class TransitPerformanceFromEventBasedRouterInterfaces implements TransitEmulator {
  ...
  @Inject
  public TransitPerformanceFromEventBasedRouterInterfaces(
      /*WaitTime waitTimes, StopStopTime stopStopTimes,*/
  ...
  public Trip findTrip(Leg prevLeg, double earliestDepartureTime_s) {
    ...
    final double accessTime_s =
        earliestDepartureTime_s /*+ this.waitTimes.getRouteStopWaitTime(route.getLineId(),
  ...
  private double findTransitTravelTime(TransitPassengerRoute route, double prevEndTime) {
    ...
    TransitRouteImpl transitRoute =
        (TransitRouteImpl) transitLines.get(route.getLineId()).getRoutes().get(route.getRouteId());
