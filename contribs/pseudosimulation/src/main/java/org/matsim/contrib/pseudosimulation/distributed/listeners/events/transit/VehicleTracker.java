...
class VehicleTracker implements Serializable {
  ...
  private final transient Id<Person> driverId;
  ...
  public void ridershipIncrement(PersonEntersVehicleEvent event) {
    if (!event.getPersonId().equals(driverId)) ridership++;
  }
  ...
  public DwellEvent registerArrival(VehicleArrivesAtFacilityEvent event) {
    DwellEvent dwellEvent =
        new DwellEvent(
            event.getTime(), event.getFacilityId().toString(), this, stopsVisited.size());
