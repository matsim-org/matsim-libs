...
/**
 * @author pieterfourie
 *     <p>A controler that alternates between the QSim and PSim for the mobility simulation. Run
 *     this class with no arguments to get printed help listing current command line options.
 */
public class RunPSim {
  ...
  public RunPSim(Config config, PSimConfigGroup pSimConfigGroup) {
    ...
    // The following line will make the controler use the events manager that doesn't check for
    // event order.
    // This is essential for pseudo-simulation as the PSim module generates events on a
    // person-basis,
    // not a system basis
    ...
    matsimControler.addOverridingModule(
        new AbstractModule() {
          ...
          public void install() {
            ...
            // I made TravelTimeCalculator final, so PSimTravelTimeCalculator can no longer inherit
            // from it.  The following statement binds PSimTravelTimeCalculator
            // directly.  I am not even sure if that is necessary.  Pls let me know if you get stuck
            // and I will try to help.  kai, feb'19
            ...
            bind(PSimProvider.class)
                .toInstance(new PSimProvider(scenario, matsimControler.getEvents()));
