...
public class SlaveScoreWriter implements IterationEndsListener, ShutdownListener, StartupListener {
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    XYLineChart chart = new XYLineChart("SlaveHandler Score Statistics", "iteration", "score");
    ...
    for (int s = 0; s < series.length; s++) {
      System.arraycopy(history[series[s]], 1, fullhist, 0, fullhist.length);
      int valuecounter = 0;
      for (int i = 0; i < idx; i++) {
    ...
    chart.addMatsimLogo();
    chart.saveAsPng(
        controler.getMATSimControler().getControlerIO().getOutputPath() + "/slaveScoreStats.png",
        1200,
        800);
  }
  ...
  public void notifyStartup(StartupEvent event) {
    String fileName =
        controler.getMATSimControler().getControlerIO().getOutputPath() + "/slaveScoreStats.txt";
