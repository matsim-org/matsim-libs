...
public class PSim implements Mobsim {
	...
	private FifoTransitPerformance fifoTransitPerformance = null;
	...
	public PSim(Scenario sc, EventsManager eventsManager, Collection<Plan> plans, TravelTime carLinkTravelTimes) {
		...
		this.beelineDistanceFactor = pcrConfig.getModeRoutingParams().get(TransportMode.walk)
	...
	public PSim(Scenario sc, EventsManager eventsManager, Collection<Plan> plans, TravelTime carLinkTravelTimes,
			WaitTime waitTimes, StopStopTime stopStopTimes, TransitPerformance transitPerformance) {
	...
	public class SimThread implements Runnable {
		...
		@Override
		public void run() {
			PLANS: for (Plan plan : threadPlans) {
					...
					if (idx > 0) {
						...
						if (prevLeg.getMode().equals(TransportMode.car)) {
							try {
								...
								eventQueue.add(new VehicleEntersTrafficEvent(prevEndTime, personId,
										prevLeg.getRoute().getStartLinkId(), personId, TransportMode.car, 1.0));
								NetworkRoute croute = (NetworkRoute) prevLeg.getRoute();

								travelTime = calcRouteTravelTime(croute, prevEndTime, carLinkTravelTimes, network,
										eventQueue, personId);
								eventQueue.add(new VehicleLeavesTrafficEvent(prevEndTime + travelTime, personId,
										prevLeg.getRoute().getEndLinkId(), personId, TransportMode.car, 1.0));
								eventQueue.add(
										new PersonLeavesVehicleEvent(prevEndTime + travelTime, personId, personId));
							...
							travelTime = tnd.time;
							eventQueue
									.add(new TeleportationArrivalEvent(prevEndTime + tnd.time, personId, tnd.distance));
							...
							if (isUseTransit) {
								...
								// >>>>> TODO NEW >>>>>
								if (fifoTransitPerformance != null) {
									floetteroed.utilities.Tuple<Departure, Double> accessDepartureAndTime_s = fifoTransitPerformance
											.getNextDepartureAndTime_s(line.getId(), transitRoute, accessStopId,
													prevEndTime + travelTime);
									if (accessDepartureAndTime_s == null) {
										travelTime = Double.POSITIVE_INFINITY;
									} else {
										eventQueue.add(new PersonEntersVehicleEvent(accessDepartureAndTime_s.getB(),
												personId, dummyVehicleId));
										double egressTime_s = accessDepartureAndTime_s.getA().getDepartureTime()
												+ transitRoute.getStop(stopFacilities.get(egressStopId))
														.getArrivalOffset();
										eventQueue.add(
												new PersonLeavesVehicleEvent(egressTime_s, personId, dummyVehicleId));
										travelTime = egressTime_s - prevEndTime;
									}
								} else
								// <<<<< TODO NEW <<<<<
								if (transitPerformance == null) {
									travelTime += waitTimes.getRouteStopWaitTime(line.getId(), transitRoute.getId(),
									...
									Tuple<Double, Double> routeTravelTime = transitPerformance.getRouteTravelTime(
											line.getId(), route.getRouteId(), accessStopId, egressStopId, prevEndTime);
							...
							try {
								...
								eventQueue.add(
										new TeleportationArrivalEvent(prevEndTime + travelTime, personId, Double.NaN));
						...
						/*
						 * Make sure that the activity does not end before the agent arrives.
						 */
		...
		private double calcRouteTravelTime(NetworkRoute route, double startTime, TravelTime travelTime, Network network,
			...
			if (route.getStartLinkId() != route.getEndLinkId()) {
				...
				for (Id<Link> routeLinkId : routeLinkIds) {
					...
					double linkTime = travelTime.getLinkTravelTime(network.getLinks().get(routeLinkId), linkEnterTime,
							null, null);
			...
			eventQueue.add(linkEnterEvent);
			return tt + travelTime.getLinkTravelTime(network.getLinks().get(route.getEndLinkId()), tt + startTime, null,
					null);
		}
