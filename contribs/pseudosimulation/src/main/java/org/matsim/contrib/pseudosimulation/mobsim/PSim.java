...
/**
 * @author illenberger
 *     <p>Original car mode code and multi-threading by Johannes Illenberger for his social network
 *     research.
 * @author fouriep
 *     <p>Extended to produce link events, making it compatible with road pricing.
 * @author fouriep, sergioo
 *     <p>Extended for transit simulation.
 */
public class PSim implements Mobsim {
  ...
  public PSim(
    ...
    for (int i = 0; i < numThreads; i++) threads[i] = new SimThread();
  ...
  public PSim(
    ...
    this.transitModes =
        ConfigUtils.addOrGetModule(sc.getConfig(), TransitConfigGroup.class).getTransitModes();
  }
  ...
  public void run() {
    ...
    LogManager.getLogger(this.getClass())
        .error("Executing " + plans.size() + " plans in pseudosimulation.");
  ...
  public class SimThread implements Runnable {
    ...
    public void run() {
      ...
      for (Plan plan : threadPlans) {
        ...
        Id<Vehicle> personVehicleId =
            Id.createVehicleId(personId.toString()); // TODO: find cleaner access to vehicle id
        ...
        for (int idx = 0; idx < elements.size(); idx += 2) {
          ...
          if (idx > 0) {
            ...
            if (prevLeg.getMode().equals(TransportMode.car)) {
              try {
                eventQueue.add(
                    new PersonEntersVehicleEvent(prevEndTime, personId, personVehicleId));
                eventQueue.add(
                    new VehicleEntersTrafficEvent(
                        prevEndTime,
                        personId,
                        prevLeg.getRoute().getStartLinkId(),
                        personVehicleId,
                        TransportMode.car,
                        1.0));
                ...
                eventQueue.add(
                    new VehicleLeavesTrafficEvent(
                        ...
                        personId,
                        prevLeg.getRoute().getEndLinkId(),
                        personVehicleId,
                        TransportMode.car,
                        1.0));
                eventQueue.add(
                    new PersonLeavesVehicleEvent(
                        prevEndTime + travelTime, personId, personVehicleId));
              ...
              if (trip != null) {
                ...
                eventQueue.add(
                    new PersonEntersVehicleEvent(
                        trip.accessTime_s(), personId, vehicleId)); // dummyVehicleId));
                eventQueue.add(
                    new PersonLeavesVehicleEvent(
                        trip.egressTime_s(), personId, vehicleId)); // dummyVehicleId));
              ...
              eventQueue.add(
                  new TeleportationArrivalEvent(
                      prevEndTime + travelTime, personId, route.getDistance(), prevLeg.getMode()));
            }
             ...
             */
            PersonArrivalEvent arrivalEvent =
                new PersonArrivalEvent(arrivalTime, personId, act.getLinkId(), prevLeg.getMode());
            eventQueue.add(arrivalEvent);
            ActivityStartEvent startEvent =
                new ActivityStartEvent(
                    arrivalTime, personId, act.getLinkId(), act.getFacilityId(), act.getType());
          ...
          if (idx < elements.size() - 1) {
            ...
            ActivityEndEvent endEvent =
                new ActivityEndEvent(
                    actEndTime, personId, act.getLinkId(), act.getFacilityId(), act.getType());
            ...
            PersonDepartureEvent departureEvent =
                new PersonDepartureEvent(
                    ...
                    act.getLinkId(),
                    nextLeg.getMode(),
                    TripStructureUtils.getRoutingMode(nextLeg));
    ...
    private double calcRouteTravelTime(
        NetworkRoute route,
        double startTime,
      ...
      if (route.getStartLinkId() != route.getEndLinkId()) {
        ...
        LinkLeaveEvent linkLeaveEvent =
            new LinkLeaveEvent(++linkEnterTime, personVehicleId, startLink);
        ...
        for (Id<Link> routeLinkId : routeLinkIds) {
          ...
          double linkTime =
              travelTime.getLinkTravelTime(
                  network.getLinks().get(routeLinkId), linkEnterTime, null, null);
      ...
      LinkEnterEvent linkEnterEvent =
          new LinkEnterEvent(startTime + tt, personVehicleId, route.getEndLinkId());
      eventQueue.add(linkEnterEvent);
      return tt
          + travelTime.getLinkTravelTime(
              network.getLinks().get(route.getEndLinkId()), tt + startTime, null, null);
    }
  }
}
