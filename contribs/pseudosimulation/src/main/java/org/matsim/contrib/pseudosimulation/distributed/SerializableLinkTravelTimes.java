...
public class SerializableLinkTravelTimes implements Serializable, TravelTime {
  ...
  public SerializableLinkTravelTimes(
      TravelTime linkTravelTimes,
      double traveltimeBinSize,
      int endTime,
    ...
    for (int i = 0; i < times.length; i++) {
      ...
      for (int j = 0; j < times[i].length; j++)
        times[i][j] = linkTravelTimes.getLinkTravelTime(link, traveltimeBinSize * j, null, null);
    }
  }
  ...
  public double getLinkTravelTime(Link link, double time, Person person, Vehicle vehicle) {
    time = time % 86400;
    try {
      return times[indices.get(link.getId().toString())][
          TimeBinUtils.getTimeBinIndex(time, travelTimeBinSize, endTime)];
