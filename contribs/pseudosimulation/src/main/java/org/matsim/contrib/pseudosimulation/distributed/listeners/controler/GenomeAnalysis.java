...
public class GenomeAnalysis implements IterationEndsListener {
  ...
  public GenomeAnalysis(
      boolean writePlanScores, boolean writeFullGenomeStats, boolean writeScoreComponents) {
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    Map<Id<Person>, ? extends Person> persons =
        event.getServices().getScenario().getPopulation().getPersons();
    boolean append =
        event.getIteration() != event.getServices().getConfig().controller().getFirstIteration();
    try {
      ...
      if (writeScoreComponents) {
        scoreComponentWriter =
            getWriter(
                false, "scoreComponents.csv", "iter,personid,simtype,component,description,score");
      }
        ...
        if (writePlanScores) {
          writer.println(
              String.format(
                  ...
                  event.getIteration(),
                  person.getId(),
                  planGenome.getGenome(),
                  planGenome.getScore(),
                  planGenome.getpSimScore()));
        }
        if (writeScoreComponents) {
        ...
        for (String key : keys) {
          writer.println(
              String.format(
                  ...
                  event.getIteration(),
                  key,
                  fullGeneCount.get(key),
                  fullGeneScore.get(key),
                  fullGeneAltScore.get(key)));
        }
        writer.flush();
        writer.close();
        //            Set<SimpleLink> edges = new HashSet<>();
        //            for(String key:keys){
        //                edges.add(new SimpleLink(key,
        // fullGeneCount.get(key),fullGeneScore.get(key),fullGeneAltScore.get(key)));
        //            }
        //            writer = new PrintWriter(new BufferedWriter(new
        // FileWriter(event.getServices().getControlerIO().getOutputPath() + "/planEvoEdges.csv",
        // false)));
        //
        // writer.println(String.format("Id,Source,Target,Type,Label,Count,Score,altscore"));
        //            for (SimpleLink edge : edges) {
        //                writer.println(String.format("%s,%s,%s,Undirected,%s,%d,%.3f,%.3f",
        // edge.to,edge.from,edge.to,edge.type, edge.count,edge.score,edge.altscore));
        //            }
        //            writer.flush();
        //            writer.close();
      } catch (IOException e) {
    ...
    for (String s : fullGeneCount.keySet()) {
      ...
      if (geneLengths.get(geneLength) != null) {
        ...
        geneScoresbyLength.put(
            geneLength,
            (geneScoresbyLength.get(geneLength) * geneLengths.get(geneLength)
                    + fullGeneScore.get(s) * fullGeneCount.get(s))
                / (geneLengths.get(geneLength) + fullGeneCount.get(s)));
      } else {
      ...
      if (charSplit.length == 1) {
        ...
        for (int i = 2; i < charSplit.length; i += 2) {
          ...
          try {
            ...
            score =
                (score * (double) count + fullGeneScore.get(s) * (double) fullGeneCount.get(s))
                    / (count + fullGeneCount.get(s));
            simpleGeneScore.put(simpleGene, score);
            score = simpleGeneAltScore.get(simpleGene);
            score =
                (score * (double) count + fullGeneAltScore.get(s) * (double) fullGeneCount.get(s))
                    / (count + fullGeneCount.get(s));
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        writer.println(
            String.format("%d,%s,%d", event.getIteration(), next.getKey(), next.getValue()));
      }
      writer.flush();
      writer.close();
      ...
      writer =
          getWriter(
              append,
              "mutationAtIterSurvivorCounts.csv",
              String.format(
                  "%s,%s,%s,%s,%s,%s",
                  "iter", "mutator", "mutationIter", "survivors", "avg.score", "avg.alt.score"));
      for (String key : keys) {
        writer.println(
            String.format(
                "%s,%s,%d,%d,%.3f,%.3f",
                ...
                simpleGeneCount.get(key),
                simpleGeneScore.get(key),
                simpleGeneAltScore.get(key)));
      }
      writer.flush();
      writer.close();
      ...
      writer =
          getWriter(
              append,
              "geneLengths.csv",
              String.format("%s,%s,%s,%s", "iter", "len", "num", "avgscor"));
      for (int gl : geneLengths.keySet()) {
        writer.println(
            String.format(
                "%d,%d,%d,%.3f",
                event.getIteration(), gl, geneLengths.get(gl), geneScoresbyLength.get(gl)));
      }
      writer.flush();
      writer.close();
    } catch (IOException e) {
