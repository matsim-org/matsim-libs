...
public class RunPSimTest {
  ...
  public void testA() {
    ...
    ((Controler) runPSim.getMatsimControler())
        .addOverridingModule(
            new AbstractModule() {
              @Override
              public void install() {
    ...
    Assert.assertEquals(
        "RunPsim score changed.", 138.86084460860525, psimScore, MatsimTestUtils.EPSILON);
  }

  /**
   * For comparison run 2 normal qsim iterations. Psim score should be slightly higher than default
   * Controler score.
   *
   * <p>Prior to implementing routing mode RunPSimTest tested only that psimScore outperformed
   * default Controler on this test for executed score by a margin > 1%. In the last commit in
   * matsim master where the test ran, the psim score in testA() was 134.52369453719413 and qsim
   * score in testB was 131.84309487251033).
   */
  @Test
  public void testB() {
    ...
    //		Assert.assertEquals("Default controler score changed.", 131.84309487251033d, qsimScore,
    // MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Default controler score changed.", 131.8303325803256, qsimScore, MatsimTestUtils.EPSILON);
  }
    ...
    public void notifyShutdown(ShutdownEvent event) {
      executedScore =
          controler
              .getScoreStats()
              .getScoreHistory()
              .get(ScoreStatsControlerListener.ScoreItem.executed)
              .get(controler.getConfig().controller().getLastIteration());
    }
  }
}
