...
public abstract class JOGLSetup {
	...
	private static final String JOGL_BASE = "/libs/jogl-1.1.1";
	...
	public static void configureJOGL() {
		...
		if (os == OS.Windows) {
			if (is64) {
				 try {
					 	checkJOGL(pwd + JOGL_BASE + "/jogl-1.1.1-windows-amd64/lib");
				 ...
				 try {
					 checkJOGL(pwd + JOGL_BASE + "/jogl-1.1.1-windows-i586/lib");
			 ...
			 try {
				checkJOGL(pwd + JOGL_BASE + "/jogl-1.1.1-macosx-universal/lib");
			...
			if (is64) {
				 try {
					 	checkJOGL(pwd + JOGL_BASE + "/jogl-1.1.1-linux-amd64/lib");
						addJNILibDir(pwd + JOGL_BASE + "/jogl-1.1.1-linux-amd64/lib");
					} catch (IOException e) {
						e.printStackTrace();
					}
			} else {
				 try {
					    checkJOGL(pwd + JOGL_BASE + "/jogl-1.1.1-linux-i586/lib");
	...
	private static void checkJOGL(String string) {
		File f = new File(string);
		if (!f.exists()) {
			log.error("Can not find JOGL native library at: " + string + ". Fix it an try again!");
			System.exit(-1);
		}
