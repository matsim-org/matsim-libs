...
public class RunExample {
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    Injector mobsimProviderInjector =
    ...
    final Provider<Mobsim> mobsimProvider =
        mobsimProviderInjector.getInstance(HybridMobsimProvider.class);
  ...
  private static void createNetwork(Scenario sc) {
    ...
    for (Link l : net.getLinks().values()) {
      ...
      l.setLength(
          CoordUtils.calcEuclideanDistance(l.getFromNode().getCoord(), l.getToNode().getCoord()));
    }
    Set<String> ext = new HashSet<>();
    ext.add("car");
    ext.add("ext");
    ext.add("2ext");
    lJPS0.setAllowedModes(ext);
    lJPS0r.setAllowedModes(ext);
