...
public class ExternalEngine
    implements MobsimEngine { // , MATSimInterfaceServiceGrpc.MATSimInterfaceService {
  ...
  public void registerAdapter(QLinkInternalIAdapter external2qAdapterLink) {
    this.adapters.put(external2qAdapterLink.getLink().getId(), external2qAdapterLink);
  }
  ...
  public void doSimStep(double time) {
    double to = time + 1;
    HybridSimProto.LeftClosedRightOpenTimeInterval req =
        HybridSimProto.LeftClosedRightOpenTimeInterval.newBuilder()
            .setFromTimeIncluding(time)
            .setToTimeExcluding(to)
            .build();
    ...
    for (HybridSimProto.Agent agent : rtrvs.getAgentsList()) {
      ...
      if (!ql.isAcceptingFromUpstream()) {
        throw new RuntimeException(
            "DS link is full, spill-back to external is not yet implemented!");
      }
  ...
  public void onPrepareSim() {
    ...
    HybridSimProto.Scenario hsc =
        (HybridSimProto.Scenario) sc.getScenarioElement("hybrid_scenario");
  ...
  public void addFromUpstream(QVehicle veh) {
    ...
    for (Id<Link> linkId : linkIds) {
      ...
      if (!extRd && linkId == currentLinkId) {
        ...
        ab.setEnterLocation(
            HybridSimProto.Coordinate.newBuilder()
                .setX(l.getFromNode().getCoord().getX())
                .setY(l.getFromNode().getCoord().getY()));
        extRd = true;
      }
      if (extRd) {
        Link l = this.net.getLinks().get(linkId);
        ...
        ab.setLeaveLocation(
            HybridSimProto.Coordinate.newBuilder()
                .setX(l.getToNode().getCoord().getX())
                .setY(l.getToNode().getCoord().getY()));
      }
    }
    ...
    //		Interfacedef.MATSim2ExternPutAgent.Agent.Builder ab =
    // Interfacedef.MATSim2ExternPutAgent.Agent.newBuilder();
    //
    //	ab.setEnterNode(enterId.toString()).setId(driverId.toString()).setLeaveNode(leaveId.toString());
    //		Interfacedef.MATSim2ExternPutAgent addReq =
    // Interfacedef.MATSim2ExternPutAgent.newBuilder().setAgent(ab).build();
    //		Interfacedef.MATSim2ExternPutAgentConfirmed addReqResp =
    // this.client.getBlockingStub().reqMATSim2ExternPutAgent(addReq);
  }
}
