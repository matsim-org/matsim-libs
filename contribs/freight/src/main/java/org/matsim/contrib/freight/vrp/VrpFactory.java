...
class VrpFactory {
	...
	VrpFactory(Matsim2VrpMap matsim2vrpMap, VehicleRoutingCosts vehicleRoutingCosts) {
		super();
	...
	public VehicleRoutingProblem createVrp(){
		verify();
		logProblem();
		VehicleRoutingProblemImpl vrp = new VehicleRoutingProblemImpl(matsim2vrpMap.getShipments(), matsim2vrpMap.getVehicles(), costs);
		return vrp;
	}

	private void logProblem() {
		logger.info("Vehicle Routing Problem #jobs: " + matsim2vrpMap.getShipments().size() + " #vehicles: " + matsim2vrpMap.getVehicles().size());
		Collection<Type> types = getTypes();
		logger.info("Available VehicleTypes #types: " + types.size());
		for(Type type : types){
			logger.info("Type id=" + type.typeId + " fixCost=" + type.vehicleCostParams.fix + " costPerDistUnit=" + type.vehicleCostParams.perDistanceUnit + " costPerTimeUnit=" + type.vehicleCostParams.perTimeUnit);
		}
