...
public class CVRPTWSolver implements VehicleRoutingProblemSolver{
	...
	public VehicleRoutingProblemSolution solve() {
		...
		RuinRadial radialRuin = new RuinRadial(vrp, new JobDistanceAvgCosts(vrp.getCosts()));
	...
	private void checkWhetherProblemIsOfCorrectType() {
		...
		for(Vehicle v : vrp.getVehicles()){
			if(location == null){
				...
				throw new IllegalStateException("if you use this solver " + this.getClass().toString() + "), all vehicles must have the same depot-location. vehicle " + v.getId() + " has not.");
			}
		}
