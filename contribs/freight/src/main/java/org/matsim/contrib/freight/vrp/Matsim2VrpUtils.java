...
class Matsim2VrpUtils {
	...
	static Collection<ScheduledTour> createTours(
			VehicleRoutingProblemSolution vrpSolution,
			Matsim2VrpMap matsim2vrpMap) {
		...
		for (VehicleRoute route : vrpSolution.getRoutes()) {
			...
			for (TourActivity act : tour.getActivities()) {
				if (act instanceof Pickup) {
					...
					CarrierShipment carrierShipment = matsim2vrpMap
							.getCarrierShipment(shipment);
					...
					tourBuilder.scheduleStart(makeId(act.getLocationId()),
							act.getEarliestOperationStartTime(),
							act.getLatestOperationStartTime());
				} else if (act instanceof End) {
					tourBuilder.addLeg(new Leg());
					tourBuilder.scheduleEnd(makeId(act.getLocationId()));
				} else {
					throw new IllegalStateException(
							"unknown tourActivity occurred. this cannot be");
				}
			}
			org.matsim.contrib.freight.carrier.Tour vehicleTour = tourBuilder
					.build();
			ScheduledTour scheduledTour = new ScheduledTour(vehicleTour,
					matsim2vrpMap.getCarrierVehicle(route.getVehicle()),
					vehicleTour.getEarliestDeparture());
