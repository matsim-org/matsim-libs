...
public class MatsimJspritFactory {
	...
	static CarrierShipment createCarrierShipmentFromService(Service service, String depotLink) {
		CarrierShipment carrierShipment = CarrierShipment.Builder.newInstance(Id.create(service.getId(), CarrierShipment.class), Id.create(depotLink, Link.class),
	...
	static CarrierShipment createCarrierShipment(Shipment shipment) {
		CarrierShipment carrierShipment = CarrierShipment.Builder.newInstance(Id.create(shipment.getId(), CarrierShipment.class), Id.createLinkId(shipment.getPickupLocation().getId()),
		...
		if (vehicleRoutingAlgortihmFile != null && !vehicleRoutingAlgortihmFile.equals("")) {
			...
			switch (freightConfig.getUseDistanceConstraintForTourPlanning()) {
				...
				case basedOnEnergyConsumption:
					...
					algorithm = VehicleRoutingAlgorithms.readAndCreateAlgorithm(problem, algorithmConfig, 0, null, stateManager, constraintManager, false);
					break;
				default:
					throw new IllegalStateException("Unexpected value: " + freightConfig.getUseDistanceConstraintForTourPlanning());
			}
