...
public class CarrierVehicleTypeReader implements MatsimReader{
	...
	public void readFile( String filename ){
		try {
			...
			log.warn("### Exception found while trying to read Carrier Vehicle Type: Message: " + e.getMessage() + " ; cause: " + e.getCause() + " ; " +
					     "class " + e.getClass());
...
//			log.debug("Reading start tag. name: " + name + " , attributes: " + atts.toString() + " , context: " + context);
			if ( "vehicleTypes".equalsIgnoreCase( name ) ) {
				...
				// yyyyyy somehow, the above does not work yet.  Note that if it is a v1 file, it starts with
