...
public class TransportCostCalculator implements VehicleRoutingCosts, TourCost{
	...
	private static Logger logger = Logger.getLogger(TransportCostCalculator.class);
	...
	public double getTransportCost(String fromId, String toId, double time, Driver driver, Vehicle vehicle) {
		...
		MatsimVehicleAdapter matsimVehicle = new MatsimVehicleAdapter(vrpVehicleAdapter);
		MatsimPersonAdapter matsimPerson = new MatsimPersonAdapter(driver);
		Path path = router.calcLeastCostPath(fromLink.getToNode(), toLink.getFromNode(), time, matsimPerson, matsimVehicle);
	...
	public double getTransportTime(String fromId, String toId, double time, Driver driver, Vehicle vehicle) {
		...
		int timeSlice = getTimeSlice(time);
		TransportDataKey transportDataKey = makeKey(fromId,toId,timeSlice,vehicle.getType().typeId);
		if(costCache.containsKey(transportDataKey)){
	...
	public double getBackwardTransportCost(String fromId, String toId, double arrivalTime, Driver driver, Vehicle vehicle) {
		...
		double freeFlowTime = getTransportTime(fromId, toId, 0.0, driver, vehicle);
		return getTransportCost(fromId, toId, arrivalTime-freeFlowTime, driver, vehicle);
	}
	...
	public double getBackwardTransportTime(String fromId, String toId, double arrivalTime, Driver driver, Vehicle vehicle) { 
		...
		double freeFlowTime = getTransportTime(fromId, toId, 0.0, driver, vehicle);
		return getTransportTime(fromId, toId, arrivalTime-freeFlowTime, driver, vehicle);
	}

	private void memorize(TransportDataKey costKey, Path path) {
		costCache.put(costKey, new TransportData(path.travelCost,path.travelTime));
