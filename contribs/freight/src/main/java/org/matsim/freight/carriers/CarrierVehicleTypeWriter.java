...
public class CarrierVehicleTypeWriter implements MatsimWriter {
  ...
  public CarrierVehicleTypeWriter(CarrierVehicleTypes types) {
    // note: for reading, we do the automatic version handling.  for writing, we just always write
    // the newest version; the older writer handlers are
    // left around if someone insists on writing the old version.  Since the carrier vehicle type
    // format is just a subset of the vehicle definitions,
