...
/*package-private*/ class CarrierPlanXmlWriterV2_1 extends MatsimXmlWriter {
  ...
  private void writeRootElement() throws UncheckedIOException, IOException {
    ...
    atts.add(
        createTuple(
            "xsi:schemaLocation",
            MATSIM_NAMESPACE + " " + DEFAULT_DTD_LOCATION + "carriersDefinitions_v2.1.xsd"));
    this.writeStartTag(CARRIERS, atts);
  }

  private void startCarrier(Carrier carrier, BufferedWriter writer) throws IOException {
    this.writeStartTag(CARRIER, List.of(createTuple(ID, carrier.getId().toString())), false, true);
    ...
    for (CarrierVehicle v : carrier.getCarrierCapabilities().getCarrierVehicles().values()) {
      ...
      this.writeStartTag(
          ...
          List.of(
              ...
              createTuple(LATEST_END, getTime(v.getLatestEndTime()))),
          true);
    }
  ...
  private void writeShipment(
      CarrierShipment s, Id<CarrierShipment> shipmentId, boolean closeElement, boolean lineBreak) {
    this.writeStartTag(
        SHIPMENT,
        List.of(
            ...
            createTuple(DELIVERY_SERVICE_TIME, getTime(s.getDeliveryServiceTime()))),
        closeElement,
        lineBreak);
  }
  ...
  private void writeService(CarrierService s, boolean closeElement, boolean lineBreak) {
    this.writeStartTag(
        ...
        List.of(
            ...
            createTuple(SERVICE_DURATION, getTime(s.getServiceDuration()))),
        closeElement,
        lineBreak);
  }
  ...
  private void writePlans(Carrier carrier, BufferedWriter writer) throws IOException {
    ...
    for (CarrierPlan plan : carrier.getPlans()) {
      if (plan.getScore() != null) {
        if (carrier.getSelectedPlan() != null && plan == carrier.getSelectedPlan()) {
          this.writeStartTag(
              PLAN, List.of(createTuple(SCORE, plan.getScore()), createTuple(SELECTED, "true")));
        } else if (carrier.getSelectedPlan() != null && plan != carrier.getSelectedPlan()) {
          this.writeStartTag(
              PLAN, List.of(createTuple(SCORE, plan.getScore()), createTuple(SELECTED, "false")));
        } else {
          this.writeStartTag(
              PLAN, List.of(createTuple(SCORE, plan.getScore()), createTuple(SELECTED, "false")));
        }
        ...
        if (carrier.getSelectedPlan() != null && plan == carrier.getSelectedPlan()) {
          this.writeStartTag(PLAN, List.of(createTuple(SELECTED, "true")));
        } else if (carrier.getSelectedPlan() != null && plan != carrier.getSelectedPlan()) {
          this.writeStartTag(PLAN, List.of(createTuple(SELECTED, "false")));
        } else {
          this.writeStartTag(PLAN, List.of(createTuple(SELECTED, "false")));
        }
      }
      ...
      for (ScheduledTour tour : plan.getScheduledTours()) {
        ...
        this.writeStartTag(
            ...
            List.of(
                ...
                createTuple(END_TIME, Time.writeTime(tour.getDeparture()))),
            true);
          ...
          if (tourElement instanceof Tour.Leg leg) {
            this.writeStartTag(
                ...
                List.of(
                    ...
                    createTuple(
                        EXPECTED_TRANSP_TIME, Time.writeTime(leg.getExpectedTransportTime()))));
            ...
            this.writeStartTag(
                ...
                List.of(
                    ...
                    createTuple(SHIPMENT_ID, act.getShipment().getId().toString())),
                true);
            if (!carrier.getShipments().containsKey(act.getShipment().getId())) {
              logger.error(
                  "Shipment with id "
                      + act.getShipment().getId().toString()
                      + " is contained in the carriers plan, "
                      + "but not available in the list of shipments. Carrier with carrierId: "
                      + carrier.getId());
            }
            ...
            this.writeStartTag(
                ...
                List.of(
                    createTuple(TYPE, act.getActivityType()),
                    createTuple(SERVICE_ID, act.getService().getId().toString())),
                true);
            if (!carrier.getServices().containsKey(act.getService().getId())) {
              logger.error(
                  "service with id "
                      + act.getService().getId().toString()
                      + " is contained in the carriers plan, "
                      + "but not available in the list of services. Carrier with carrierId: "
                      + carrier.getId());
            }
          }
        }
        this.writeStartTag(ACTIVITY, List.of(createTuple(TYPE, CarrierConstants.END)), true);
