...
final class CarrierAgent implements Identifiable<Carrier> {
  ...
  private int nextId =
      1; // used to generate unique driver Ids. First agent should have the 1 and not 0.
   ...
   *
   * <p>A carrier plan consists usually of many tours (activity chains). Each plan in the returned
   * list represents a carrier tour.
   *
  ...
  List<Plan> createFreightDriverPlans() {
    ...
    for (ScheduledTour scheduledTour : carrier.getSelectedPlan().getScheduledTours()) {
      ...
      CarrierDriverAgent carrierDriverAgent =
          new CarrierDriverAgent(
              driverId, scheduledTour, scoringFunction, carrier, events, carrierEventCreators);
      ...
      for (TourElement tourElement : scheduledTour.getTour().getTourElements()) {
        if (tourElement instanceof Tour.Leg tourLeg) {
          ...
          // yy At least in EquilWithoutCarrierWithPassIT and routes removed, it runs through even
          // without the above
          // line ... but it looks like the simulation is not generating kilometers.  Presumably,
          ...
          leg.setTravelTime(
              tourLeg.getExpectedDepartureTime()
                  + tourLeg.getExpectedTransportTime()
          ...
          Activity tourElementActivity =
              PopulationUtils.createActivityFromLinkId(act.getActivityType(), act.getLocation());
          double duration = act.getDuration();
          tourElementActivity.setMaximumDuration(
              duration); // "maximum" has become a bit of a misnomer ...
  ...
  private Vehicle createVehicle(Person driverPerson, CarrierVehicle carrierVehicle) {
    ...
    return VehicleUtils.getFactory()
        .createVehicle(Id.create(driverPerson.getId(), Vehicle.class), carrierVehicle.getType());
  }
  ...
  private Id<Person> createDriverId(CarrierVehicle carrierVehicle) {
    Id<Person> id =
        Id.create(
            "freight_" + carrier.getId() + "_veh_" + carrierVehicle.getId() + "_" + nextId,
            Person.class);
    driverIds.add(id);
    ++nextId;
    return id;
  ...
  void handleEvent(Event event, Id<Person> driverId) {
    // the event comes to here from CarrierAgentTracker only for those drivers that belong to this
    // carrier.  The driver IDs are also
