...
class CarrierPlanXmlParserV2_1 extends MatsimXmlParser {
  ...
  public void startTag(String name, Attributes atts, Stack<String> context) {
    switch (name) {
      ...
      case SERVICE:
        {
          ...
          if (serviceTimeString != null)
            serviceBuilder.setServiceDuration(parseTimeToDouble(serviceTimeString));
      ...
      case SHIPMENT:
        {
          ...
          CarrierShipment.Builder shipmentBuilder =
              CarrierShipment.Builder.newInstance(
                  id, Id.create(from, Link.class), Id.create(to, Link.class), size);
          ...
          if (startPickup != null && endPickup != null)
            shipmentBuilder.setPickupTimeWindow(
                TimeWindow.newInstance(
                    parseTimeToDouble(startPickup), parseTimeToDouble(endPickup)));
      ...
      case VEHICLE:
        ...
        if (depotLinkId == null)
          throw new IllegalStateException("depotLinkId of vehicle is missing.");

        String typeId = atts.getValue("typeId");
        if (typeId == null) throw new IllegalStateException("vehicleTypeId is missing.");
        VehicleType vehicleType =
            this.carrierVehicleTypes.getVehicleTypes().get(Id.create(typeId, VehicleType.class));
        if (vehicleType == null) {
        ...
        CarrierVehicle.Builder vehicleBuilder =
            CarrierVehicle.Builder.newInstance(
                Id.create(vId, Vehicle.class), Id.create(depotLinkId, Link.class), vehicleType);
      ...
      case "leg":
        ...
        if (transpTime == null)
          throw new IllegalStateException("leg.expected_transp_time is missing.");
  ...
  public void endTag(String name, String content, Stack<String> context) {
    switch (name) {
      ...
      case "vehicleType" -> throw new RuntimeException(
          "I am confused now if, for carriers, vehicleType is in the plans file, or in a separate file.");
