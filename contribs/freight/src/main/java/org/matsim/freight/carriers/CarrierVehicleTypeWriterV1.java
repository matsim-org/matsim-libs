  ...
  private void writeTypes(BufferedWriter writer) throws IOException {
    ...
    for (VehicleType type : vehicleTypes.getVehicleTypes().values()) {
      ...
      if (engineInformation != null && !engineInformation.getAttributes().isEmpty()) {
        writer.write(
            "\t\t\t<engineInformation fuelType=\""
                + engineInformation.getFuelType().toString()
                + "\" gasConsumption=\""
                + engineInformation.getFuelConsumption()
                + "\"/>\n");
      }
      ...
      if (vehicleCostInformation == null)
        throw new IllegalStateException("vehicleCostInformation is missing.");
      writer.write(
          "\t\t\t<costInformation fix=\""
              + vehicleCostInformation.getFixedCosts()
              + "\" perMeter=\""
              + vehicleCostInformation.getCostsPerMeter()
              + "\" perSecond=\""
              + vehicleCostInformation.getCostsPerSecond()
              + "\"/>\n");
