...
public class CarriersUtils {
  ...
  /**
   * From the outside, rather use {@link CarriersUtils#getCarriers(Scenario)} . This string constant
   * will eventually become private.
   */
  ...
  public static void addCarrierVehicle(Carrier carrier, CarrierVehicle carrierVehicle) {
    carrier
        ...
        .getCarrierVehicles()
        .put(carrierVehicle.getId(), carrierVehicle);
  }
  ...
  public static CarrierVehicle getCarrierVehicle(Carrier carrier, Id<Vehicle> vehicleId) {
    ...
    log.error(
        ...
        new IllegalStateException(
            "vehicle with id " + vehicleId + " is missing in Carrier: " + carrier.getId()));
    return null;
  }
  ...
  public static CarrierService getService(Carrier carrier, Id<CarrierService> serviceId) {
    ...
    log.error(
        "Service with Id does not exists",
        new IllegalStateException(
            "Service with id " + serviceId + " is missing in Carrier: " + carrier.getId()));
    return null;
  }
  ...
  public static CarrierShipment getShipment(Carrier carrier, Id<CarrierShipment> serviceId) {
    ...
    log.error(
        "Shipment with Id does not exists",
        new IllegalStateException(
            "Shipment with id " + serviceId + " is missing in Carrier: " + carrier.getId()));
    return null;
  }
  ...
  public static int getJspritIterations(Carrier carrier) {
    ...
    if (result == null) {
      log.error(
          "Requested attribute jspritIterations does not exists. Will return " + Integer.MIN_VALUE);
  ...
  /**
   * Runs jsprit and so solves the VehicleRoutingProblem (VRP) for all {@link Carriers}, doing the
   * following steps: - creating NetbasedCosts based on the network - building and solving the VRP
   * for all carriers using jsprit - take the (best) solution, route and add it as {@link
   * CarrierPlan} to the {@link Carrier}.
   *
   * <p>
   *
   * @param scenario
  ...
  public static void runJsprit(Scenario scenario) throws ExecutionException, InterruptedException {
    ...
    final NetworkBasedTransportCosts netBasedCosts =
    ...
    // Fill carrierActivityCounterMap -> basis for sorting the carriers by number of activities
    // before solving in parallel
    for (Carrier carrier : carriers.getCarriers().values()) {
      carrierActivityCounterMap.put(
          carrier.getId(),
          carrierActivityCounterMap.getOrDefault(carrier.getId(), 0)
    ...
    HashMap<Id<Carrier>, Integer> sortedMap =
        carrierActivityCounterMap.entrySet().stream()
                    ...
                    .forEach(
                        carrierId -> {
                          ...
                          VehicleRoutingAlgorithm algorithm =
                              MatsimJspritFactory.loadOrCreateVehicleRoutingAlgorithm(
                                  scenario, freightCarriersConfigGroup, netBasedCosts, problem);

                          algorithm
                              .getAlgorithmListeners()
                              .addListener(
                                  new StopWatch(), VehicleRoutingAlgorithmListeners.Priority.HIGH);
                          ...
                          VehicleRoutingProblemSolution solution =
                              Solutions.bestOf(algorithm.searchSolutions());

                          log.info(
                              "tour planning for carrier "
                                  + carrier.getId()
                                  + " took "
                          ...
                          // yy In principle, the carrier should know the vehicle types that it can
                          // deploy.
                          ...
                          log.info(
                              "routing for carrier "
                                  + carrier.getId()
                                  + " finished. Tour planning plus routing took "
                                  + (System.currentTimeMillis() - start) / 1000
                                  + " seconds.");
  ...
  /**
   * Creates a new {@link Carriers} container only with {@link CarrierShipment}s for creating a new
   * VRP. As consequence of the transformation of {@link CarrierService}s to {@link
   * CarrierShipment}s the solution of the VRP can have tours with vehicles returning to the depot
   * and load for another tour instead of creating another vehicle with additional (fix) costs. <br>
   * The method is meant for multi-depot problems. Here, the original "services" input does not have
   * an assignment of services to depots. The solution to the problem, however, does. So the
   * assignment is taken from that solution, and each returned {@link Carrier} has that depot as
   * pickup location in each shipment.
   *
  ...
  public static Carriers createShipmentVRPCarrierFromServiceVRPSolution(Carriers carriers) {
    ...
    for (Carrier carrier : carriers.getCarriers().values()) {
      ...
      carrierWS.setCarrierCapabilities(
          carrier.getCarrierCapabilities()); // vehicles and other carrierCapabilities
  ...
  /**
   * @deprecated -- please inline. Reason: move syntax closer to how it is done in {@link
   *     ConfigUtils}.
   */
  @Deprecated
  ...
  public static Carriers getCarriers(Scenario scenario) {
    // I have separated getOrCreateCarriers and getCarriers, since when the controler is started, it
    // is better to fail if the carriers are
  ...
  public static CarrierVehicleTypes getCarrierVehicleTypes(Scenario scenario) {
    CarrierVehicleTypes types =
        (CarrierVehicleTypes) scenario.getScenarioElement(CARRIER_VEHICLE_TYPES);
    if (types == null) {
  ...
  public static void loadCarriersAccordingToFreightConfig(Scenario scenario) {
    ...
    new CarrierVehicleTypeReader(vehTypes)
        .readURL(
            IOUtils.extendUrl(
                scenario.getConfig().getContext(),
                freightCarriersConfigGroup.getCarriersVehicleTypesFile()));

    Carriers carriers = addOrGetCarriers(scenario); // also registers with scenario
    new CarrierPlanXmlReader(carriers, vehTypes)
        .readURL(
            IOUtils.extendUrl(
                scenario.getConfig().getContext(), freightCarriersConfigGroup.getCarriersFile()));
  ...
  /**
   * Transform all services from the existing carrier to the new carrier with shipments. The
   * location of the depot from which the "old" carrier starts the tour to the service is used as
   * fromLocation for the new Shipment.
   *
   * @param carrierWS the "new" carrier with Shipments
   * @param carrier the already existing carrier
   */
    ...
    for (CarrierService carrierService : carrier.getServices().values()) {
      ...
      CarrierShipment carrierShipment =
          CarrierShipment.Builder.newInstance(
                  Id.create(carrierService.getId().toString(), CarrierShipment.class),
              ...
              // Limited to end of delivery timeWindow (pickup later than the latest delivery is not
              // useful).
              .setPickupTimeWindow(
                  TimeWindow.newInstance(0.0, carrierService.getServiceStartTimeWindow().getEnd()))
              .build();
      addShipment(carrierWS, carrierShipment);
    }
  }
   ...
   * @return <code>true</code> if the skill is in the skill set; or <code>false</code> if the skill
   *     is not in the skill set, or there is no skill set available/set.
   */
  ...
  /**
   * Sets the given skills as an attribute. If prior skills were set, they will be overwritten.
   *
  ...
  /**
   * Get all the skills in the argument's {@link Attributes}. You should not try and modify the
   * returned list. If you want to add a skill, use the method {@link #addSkill(VehicleType,
   * String)}.
   *
  ...
  /**
   * Get all the skills in the argument's {@link Attributes}. You should not try and modify the
   * returned list. If you want to add a skill, use the method {@link #addSkill(CarrierShipment,
   * String)}.
   *
  ...
  /**
   * Get all the skills in the argument's {@link Attributes}. You should not try and modify the
   * returned list. If you want to add a skill, use the method {@link #addSkill(CarrierService,
   * String)}.
   *
   ...
   *
   * @param attributes the {@link Attributes} container that is checked for the skill(s) to be
   *     converted.
  ...
  private static List<String> convertSkillsAttributeToList(Attributes attributes) {
    if (attributes.getAttribute(ATTR_SKILLS) == null) {
      ...
      return Arrays.asList(
          Objects.requireNonNull(attributes.getAttribute(ATTR_SKILLS)).toString().split(","));
    }
  }
