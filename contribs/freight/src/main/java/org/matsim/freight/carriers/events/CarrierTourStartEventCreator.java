...
/*package-private*/ final class CarrierTourStartEventCreator implements CarrierEventCreator {
  ...
  public Event createEvent(
    ...
    // temporarily store some information, because ActivityEndEvent does not have any information
    // about the person's vehicle.
    if ((event instanceof ActivityEndEvent endEvent)
        && CarrierConstants.START.equals(endEvent.getActType())) {
      final Id<Person> personId = endEvent.getPersonId();
      endEventMap.put(personId, endEvent);

      // it is unclear in which order the events arrive in the events stream, when they have the
      // same time step ->Check if TourStartsEvent should be thrown now.
    ...
    if (event
        ...
        personEntersVehicleEvent) { // now we have the persons vehicle
   ...
   */
  private CarrierTourStartEvent createFreightTourStartsEvent(
      Id<Person> personId, Carrier carrier, ScheduledTour scheduledTour) {
    ...
    if (endEvent.getTime() == entersVehicleEvent.getTime()) {
      ...
      return new CarrierTourStartEvent(
          endEvent.getTime(),
          carrier.getId(),
          scheduledTour.getTour().getStartLinkId(),
          entersVehicleEvent.getVehicleId(),
          scheduledTour.getTour().getId());
    }
    return null;
  }
}
