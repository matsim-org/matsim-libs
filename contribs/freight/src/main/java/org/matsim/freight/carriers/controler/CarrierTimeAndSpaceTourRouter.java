...
public class CarrierTimeAndSpaceTourRouter {
   ...
   */
  public CarrierTimeAndSpaceTourRouter(
      LeastCostPathCalculator router, Network network, TravelTime travelTime) {
    super();
    this.router = router;
   ...
   *
   * <p>Uses a leastCostPathCalculator to calculate a route/path from one activity to another. It
   * starts at the departureTime of the scheduledTour and determines activity arrival and departure
   * times considering activities time-windows.
   *
   * @param tour
   */
  ...
  private void route(Leg prevLeg, Id<Link> fromLinkId, Id<Link> toLinkId, Vehicle vehicle) {
    ...
    Path path =
        router.calcLeastCostPath(
            ...
            network.getLinks().get(toLinkId).getFromNode(),
            prevLeg.getExpectedDepartureTime(),
            null,
            vehicle);
     ...
     */
    double toLinkTravelTime =
        this.travelTime.getLinkTravelTime(
            network.getLinks().get(toLinkId),
            prevLeg.getExpectedDepartureTime() + travelTime,
            null,
            vehicle);
