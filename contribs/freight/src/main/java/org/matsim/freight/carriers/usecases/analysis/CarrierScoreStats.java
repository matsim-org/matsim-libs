...
/**
 * As you can see, it is basically a copy of {@link
 * org.matsim.analysis.ScoreStatsControlerListener}. However, it is modified to score {@link
 * Carrier}s rather than Persons. (Oct'13, schroeder)
 *
 * <p>
 *
...
public class CarrierScoreStats implements StartupListener, IterationEndsListener, ShutdownListener {
  ...
  private static final Logger log = LogManager.getLogger(CarrierScoreStats.class);

  /**
   * Creates a new ScoreStats instance.
   *
   * @param filename including the path, excluding the file type extension
   * @param createPNG true if in every iteration, the scorestats should be visualized in a graph and
   *     written to disk.
   * @throws UncheckedIOException
   */
  public CarrierScoreStats(Carriers carriers, final String filename, final boolean createPNG)
      throws UncheckedIOException {
