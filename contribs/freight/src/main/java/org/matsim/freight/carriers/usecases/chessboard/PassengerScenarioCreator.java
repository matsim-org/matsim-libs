...
final class PassengerScenarioCreator {
  ...
  public static void main(String[] args) {
    ...
    new PopulationWriter(population, scenario.getNetwork())
        .write("input/usecases/chessboard/passenger/passengerPlansV2.xml");
  }

  private static List<Person> createPersons(Id<Link> homeId, Id<Link> workId, Scenario scenario) {
    LeastCostPathCalculator lcpa =
        new DijkstraFactory()
            .createPathCalculator(
                scenario.getNetwork(),
                new FreespeedTravelTimeAndDisutility(-1.0, -1.0, -1.0),
                new FreespeedTravelTimeAndDisutility(-1.0, -1.0, -1.0));
    ...
    for (int agent = 0; agent < nuOfAgentsPerHomeLink; agent++) {
      ...
      Path path1 =
          lcpa.calcLeastCostPath(
              scenario.getNetwork().getLinks().get(homeId).getToNode(),
              scenario.getNetwork().getLinks().get(workId).getFromNode(),
              act1.getEndTime().seconds(),
              person,
              null);
      NetworkRoute linkNetworkRoute =
          RouteUtils.createLinkNetworkRouteImpl(homeId, getLinkIds(path1), workId);
      ...
      Path path2 =
          lcpa.calcLeastCostPath(
              scenario.getNetwork().getLinks().get(workId).getToNode(),
              scenario.getNetwork().getLinks().get(homeId).getFromNode(),
              act1.getEndTime().seconds(),
              person,
              null);
      NetworkRoute linkNetworkRoute2 =
          RouteUtils.createLinkNetworkRouteImpl(workId, getLinkIds(path2), homeId);
