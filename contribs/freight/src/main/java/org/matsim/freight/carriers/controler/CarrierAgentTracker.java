...
public final class CarrierAgentTracker implements BasicEventHandler {
  ...
  // need to use this via injection, since LSP is using it from another package, and thus has to be
  // public.  With injection, can at least
  // protect the constructor.  With injection, can either use this with global scope, or with mobsim
  // scope.  Mobsim scope is too narrow since
  // this class also handles scoring.  This only leaves global scope.  In consequence, needs to be
  // moved from its original design where the
  // tracker was destroyed and recreated in every iteration, to something that is persistent.
  // Indeed, original matsim design always was like
  // that (so that observers could collect information over multiple iterations without additional
  // programming).  kai, jul'22
  ...
  public void handleEvent(Event event) {
    ...
    if (event instanceof HasPersonId) {
      ...
      // (we could say that we rather use the specific handleEvent methods.  But this here hedges
      // against someone make Link
      // Enter/Leave Event implement HasPersonId.  kai, jul'22)
      driverId =
          vehicle2DriverEventHandler.getDriverOfVehicle(((HasVehicleId) event).getVehicleId());
    } else {
      return;
    }
