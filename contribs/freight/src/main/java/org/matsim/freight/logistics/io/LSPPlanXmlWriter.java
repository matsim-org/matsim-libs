...
public class LSPPlanXmlWriter extends MatsimXmlWriter {
	...
	private void writePlans(LSP lsp) {
		...
		for (LSPPlan plan : lsp.getPlans()) {
			if (plan.getScore() != null && lsp.getSelectedPlan() != null) {
				if (plan == lsp.getSelectedPlan()) {
					this.writeStartTag(LSP_PLAN, List.of(createTuple(SCORE, plan.getScore()), createTuple(SELECTED, "true")));
				} else {
					this.writeStartTag(LSP_PLAN, List.of(createTuple(SCORE, plan.getScore()), createTuple(SELECTED, "false")));
				}
			} else {
