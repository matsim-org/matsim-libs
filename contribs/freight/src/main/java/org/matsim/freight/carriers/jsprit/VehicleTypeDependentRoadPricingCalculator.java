...
public class VehicleTypeDependentRoadPricingCalculator {
  ...
  private final Map<Id<org.matsim.vehicles.VehicleType>, Collection<RoadPricingScheme>> schemes =
      new HashMap<>();
   ...
   */
  public Collection<RoadPricingScheme> getPricingSchemes(
      Id<org.matsim.vehicles.VehicleType> vehicleType) {
    Collection<RoadPricingScheme> collection = schemes.get(vehicleType);
    return Collections.unmodifiableCollection(
        Objects.requireNonNullElse(collection, Collections.emptyList()));
  }

  /**
   * Gets the toll-amount for vehicleType on {@link Link} link at time 'time'.
   *
   * <p>In case of <code>RoadPricingScheme.TOLL_TYPE_CORDON</code> the toll amount on a link is
   * equivalent to <code>rps.getLinkCostInfo(...).amount</code>.
   *
   * <p>In case of <code>RoadPricingScheme.TOLL_TYPE_DISTANCE</code> the toll amount on a link is
   * equivalent to <code>rps.getLinkCostInfo(...).amount*link.getLength()</code>.
   *
  ...
  public double getTollAmount(
      Id<org.matsim.vehicles.VehicleType> vehicleType, Link link, double time) {
  ...
  public void addPricingScheme(String vehicleTypeId, RoadPricingScheme pricingScheme) {
    Id<org.matsim.vehicles.VehicleType> typeId =
        Id.create(vehicleTypeId, org.matsim.vehicles.VehicleType.class);
  ...
  public void addPricingScheme(
      Id<org.matsim.vehicles.VehicleType> vehicleTypeId, RoadPricingScheme pricingScheme) {
    Collection<RoadPricingScheme> list =
        schemes.computeIfAbsent(vehicleTypeId, k -> new ArrayList<>());
    list.add(pricingScheme);
    if (pricingScheme.getType().equals(RoadPricingScheme.TOLL_TYPE_CORDON)) {
      throw new RuntimeException(
          "the matsim cordon toll implementation no longer exists; link pricing is probably what you want");
