...
/**
 * This calculates transport-times, transport-costs and the distance to cover the distance from one
 * location to another. It calculates these values based on a {@link Network} to serve as {@link
 * VehicleRoutingTransportCosts} in the {@link VehicleRoutingProblem}. The distance includes the
 * links of the path and the fromLink and not the toLink.
 *
 * <p>It can be used with multiple threads. Note that each thread gets its own
 * leastCostPathCalculator. It is created only once and cached afterwards. Thus, it requires a
 * threadSafe leastCostPathCalculatorFactory (the calculator itself does not need to be
 * thread-safe).
 *
 * <p>If the client of this class requests cost-information to get from from-location to to-location
 * at a certain time and with a certain vehicle, it looks up whether there is already an entry in
 * the cache. If so, it returns the cached value, if not it calculates new values with a
 * leastCostPathCalculator defined in here. It looks up the cached values according to the {@link
 * TransportDataKey}.
 *
 * <p>Two TransportDataKeys are equal if the following data are equal</br> - from-locations</br> -
 * to-locations</br> - time and</br> - vehicleTypes
 *
 * <p>Keep in mind that if you have many locations, small time-bins and many vehicleTypes,
 * calculations get very time- and memory-consuming.
 *
...
public class NetworkBasedTransportCosts implements VRPTransportCosts {
  ...
  /**
   * This creates a matsim-vehicle {@link org.matsim.vehicles.Vehicle} from a matsim-freight-vehicle
   * {@link CarrierVehicle} and jsprit-vehicle .
   *
   * @author stefan schröder
   */
  public static class MatsimVehicleWrapper implements org.matsim.vehicles.Vehicle {
    ...
    public MatsimVehicleWrapper(com.graphhopper.jsprit.core.problem.vehicle.Vehicle vehicle) {
      ...
      this.vehicleType =
          makeType(vehicle.getType().getTypeId(), vehicle.getType().getMaxVelocity());
    }
    ...
    private org.matsim.vehicles.VehicleType makeType(String typeId, double maxVelocity) {
      org.matsim.vehicles.VehicleType vehicleTypeImpl =
   ...
   * to-locations</br> - time and</br> - vehicleTypes
   *
   * <p>The time-value is usually a representative value for a certain time-bin. If the time-bin's
   * width is 1, it calculates and caches each and every transport-data which might be a) very
   * time-consuming and b) very memory-consuming.
   *
   * @author stefan schröder
   */
   ...
   *
   * <p>Note that it uses <code>vehicleType.getCostInformation().perDistanceUnit</code> and <code>
   * vehicleType.getCostInformation().perTimeUnit</code> to calculate time and distance related
   * transport costs.
   *
   * @author stefan
   */
  static class BaseVehicleTransportCosts implements TravelDisutility {
    ...
    private BaseVehicleTransportCosts(
        Map<String, VehicleTypeVarCosts> typeSpecificCosts, TravelTime travelTime) {
  ...
  static class VehicleTransportCostsIncludingToll implements TravelDisutility {
    ...
    // LogManager.getLogger(VehicleTransportCostsIncludingToll.class);
    ...
    public VehicleTransportCostsIncludingToll(
        TravelDisutility baseTransportDisutility,
  ...
  public static class Builder {
    ...
    /**
     * By default, it takes <code>link.getFreespeed(time);</code> to calculate the travelTime over
     * that link.
     */
    private TravelTime travelTime =
        (link, time, person, vehicle) -> {
          double velocity =
              Math.min(vehicle.getType().getMaximumVelocity(), link.getFreespeed(time));
          if (velocity <= 0.0) {
    ...
    private LeastCostPathCalculatorFactory leastCostPathCalculatorFactory =
        (network, travelCosts, travelTimes) ->
            new SpeedyALTFactory().createPathCalculator(network, travelCosts, travelTime);

    private VehicleTypeDependentRoadPricingCalculator roadPricingCalculator =
     ...
     *
     * @param network
     * @param vehicleTypes must be all vehicleTypes and their assigned costInformation in the
     *     system.
     */
    ...
    private void retrieveTypeSpecificCosts(Collection<VehicleType> vehicleTypes) {
      for (VehicleType type : vehicleTypes) {
        typeSpecificCosts.put(
            type.getId().toString(),
            new VehicleTypeVarCosts(
                type.getCostInformation().getCostsPerMeter(),
                type.getCostInformation().getCostsPerSecond()));
      }
    ...
    public Builder setBaseTravelTimeAndDisutility(
     ...
     *
     * <p><i>Note that this needs to be set to some plausible value to enable any kind of
     * time-dependent network!!!</i> In particular if you are using the matsim time dependent
     * network option.
     */
    ...
    /**
     * Builds the network-based transport costs which are the basis for solving the {@link
     ...
     *   <li>yyyy These free speed travel times do <i>not</i> take the time-dependent network into
     *       account. kai, jan'14
    ...
    public NetworkBasedTransportCosts build() {
      ...
      if (withToll) {
        finalDisutility =
            new VehicleTransportCostsIncludingToll(baseDisutility, roadPricingCalculator);
     ...
     * @param perMeter
     */
    public void addVehicleTypeSpecificCosts(
        String typeId, double fix, double perSecond, double perMeter) {
  ...
  /**
   * cost-cache to cache transport-costs and transport-times (see {@link TransportData}) according
   * to {@link TransportDataKey}
   */
  private final ConcurrentHashMap<TransportDataKey, TransportData> costCache =
      new ConcurrentHashMap<>();

  /** caches leastCostPathCalculators according to <code>Thread.currentThread().getId()</code> */
  private final ConcurrentHashMap<Long, LeastCostPathCalculator> routerCache =
      new ConcurrentHashMap<>();
   ...
   *
   * <p>If <code>fromId.equals(toId)</code> it returns 0.0. Otherwise, it looks up in the cache
   * whether the transport-time has already been computed (see {@link TransportDataKey}, {@link
   * TransportData}). If so, it returns the cached travel-time. If not, it computes and caches new
   * values with the leastCostPathCalc defined in here. @Throws {@link IllegalStateException} if
   * vehicle is null
   */
  @Override
  public double getTransportTime(
      Location fromId, Location toId, double departureTime, Driver driver, Vehicle vehicle) {
    ...
    if (data != null) {
      ...
      Path path =
          router.calcLeastCostPath(
              fromLink.getToNode(), toLink.getFromNode(), departureTime, null, matsimVehicle);
      //			if(path == null) return Double.MAX_VALUE;
      double additionalCostTo =
          travelDisutility.getLinkTravelDisutility(
              toLink, departureTime + path.travelTime, null, matsimVehicle);
      double additionalTimeTo =
      ...
      TransportData newData =
          new TransportData(
              path.travelCost + additionalCostTo,
  ...
  private VehicleImpl getDefaultVehicle(Location fromId) {
    return VehicleImpl.Builder.newInstance("default")
        .setType(
            com.graphhopper.jsprit.core.problem.vehicle.VehicleTypeImpl.Builder.newInstance(
                    defaultTypeId)
                .build())
   ...
   *
   * <p>If <code>fromId.equals(toId)</code> it returns 0.0. Otherwise, it looks up in the cache
   * whether the transport-costs have already been computed (see {@link TransportDataKey}, {@link
   * TransportData}). If so, it returns the cached travel-cost value. If not, it computes and caches
   * new values with the leastCostPathCalc defined in here. @Throws {@link IllegalStateException} if
   * vehicle is null
   */
  @Override
   ...
   *
   * <p>If <code>fromId.equals(toId)</code> it returns 0.0. Otherwise, it looks up in the cache
   * whether the transport-distance has already been computed (see {@link TransportDataKey}, {@link
   * TransportData}). If so, it returns the cached distance. If not, it computes and caches new
   * values with the leastCostPathCalc defined in here. @Throws {@link IllegalStateException} if
   * vehicle is null
   */
  @Override
  ...
  /**
   * Backward transport-costs are approximated by calculating <code>
   * getTransportCost(fromId, toId, arrivalTime, driver, vehicle);</code>.
   *
   * <p>This is a rather bad approximation. If you require this, you should implement another {@link
   * VehicleRoutingTransportCosts} @Throws {@link IllegalStateException} if vehicle is null
   */
  @Override
  public double getBackwardTransportCost(
      Location fromId, Location toId, double arrivalTime, Driver driver, Vehicle vehicle) {
    return getTransportCost(fromId, toId, arrivalTime, driver, vehicle);
  }

  /**
   * Backward transport-time are approximated by calculating <code>
   * getTransportTime(fromId, toId, arrivalTime, driver, vehicle);</code>.
   *
   * <p>This is a rather bad approximation. If you require this, you should implement another {@link
   * VehicleRoutingTransportCosts}. @Throws {@link IllegalStateException} if vehicle is null
   */
  ...
  private LeastCostPathCalculator createLeastCostPathCalculator() {
    ...
    if (router == null) {
      LeastCostPathCalculator newRouter =
