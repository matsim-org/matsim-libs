...
public class CarrierVehicleTypeReader implements MatsimReader {
  ...
  private static final String MSG =
      "With early carrier vehicle type file formats, there will be an expected exception in the following.";
  ...
  public void readFile(String filename) {
    ...
    try {
      ...
      if (e.getCause()
        ...
        log.warn(
            "read with validation = true failed. Try it again without validation. filename: "
                + filename);
  ...
  public void readURL(URL url) {
    ...
    try {
      ...
      if (e.getCause()
          .getMessage()
          .contains(
              "cvc-elt.1.a")) { // "Cannot find the declaration of element" -> exception comes most
        // probably because no validation information was found
        log.warn(
  ...
  public void readStream(InputStream inputStream) {
    ...
    try {
      ...
      log.warn(
          "### Exception found while trying to read Carrier Vehicle Type: Message: "
              + e.getMessage()
              + " ; cause: "
              + e.getCause()
              + " ; class "
  ...
  private static final class CarrierVehicleTypeParser extends MatsimXmlParser {
    ...
    public void startTag(
        final String name, final Attributes attributes, final Stack<String> context) {
      log.debug(
          "Reading start tag. name: "
              + name
              + " , attributes: "
              + attributes.toString()
              + " , context: "
    ...
    public void endDocument() {
      ...
      if (vehicles != null) {
        for (Map.Entry<Id<VehicleType>, VehicleType> entry :
            vehicles.getVehicleTypes().entrySet()) {
