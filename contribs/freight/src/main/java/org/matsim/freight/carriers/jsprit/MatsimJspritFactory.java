...
/**
 * A factory that creates matsim-object from jsprit (<a
 * href="https://github.com/jsprit/jsprit">...</a>) and vice versa.
 *
 * @author sschroeder
 *     <p>Extend the functionality for the use of shipments
 * @author kturner
 */
public final class MatsimJspritFactory {
  ...
  static CarrierShipment createCarrierShipment(Shipment jspritShipment) {
    CarrierShipment carrierShipment =
        CarrierShipment.Builder.newInstance(
                Id.create(jspritShipment.getId(), CarrierShipment.class),
                Id.createLinkId(jspritShipment.getPickupLocation().getId()),
                Id.createLinkId(jspritShipment.getDeliveryLocation().getId()),
                jspritShipment.getSize().get(0))
            .setDeliveryServiceTime(jspritShipment.getDeliveryServiceTime())
            .setDeliveryTimeWindow(
                org.matsim.freight.carriers.TimeWindow.newInstance(
                    jspritShipment.getDeliveryTimeWindow().getStart(),
                    jspritShipment.getDeliveryTimeWindow().getEnd()))
            .setPickupServiceTime(jspritShipment.getPickupServiceTime())
            .setPickupTimeWindow(
                org.matsim.freight.carriers.TimeWindow.newInstance(
                    jspritShipment.getPickupTimeWindow().getStart(),
  ...
  static Shipment createJspritShipment(CarrierShipment carrierShipment) {
    Shipment.Builder shipmentBuilder =
        Shipment.Builder.newInstance(carrierShipment.getId().toString())
            ...
            .setDeliveryTimeWindow(
                com.graphhopper.jsprit.core.problem.solution.route.activity.TimeWindow.newInstance(
                    carrierShipment.getDeliveryTimeWindow().getStart(),
  ...
  static Shipment createJspritShipment(
      CarrierShipment carrierShipment, Coord fromCoord, Coord toCoord) {
  ...
  static Service createJspritService(CarrierService carrierService, Coord locationCoord) {
    ...
    if (locationCoord != null) {
      locationBuilder.setCoordinate(
          Coordinate.newInstance(locationCoord.getX(), locationCoord.getY()));
    }
    ...
    serviceBuilder
        ...
        .setServiceTime(carrierService.getServiceDuration())
        .setTimeWindow(
            com.graphhopper.jsprit.core.problem.solution.route.activity.TimeWindow.newInstance(
  ...
  static CarrierService createCarrierService(Service jspritService) {
    CarrierService.Builder serviceBuilder =
        CarrierService.Builder.newInstance(
    ...
    serviceBuilder.setServiceStartTimeWindow(
        org.matsim.freight.carriers.TimeWindow.newInstance(
            jspritService.getTimeWindow().getStart(), jspritService.getTimeWindow().getEnd()));
   ...
   */
  static com.graphhopper.jsprit.core.problem.vehicle.Vehicle createJspritVehicle(
      CarrierVehicle carrierVehicle, Coord locationCoord) {
    ...
    if (locationCoord != null) {
      vehicleLocationBuilder.setCoordinate(
          Coordinate.newInstance(locationCoord.getX(), locationCoord.getY()));
    }
    Location vehicleLocation = vehicleLocationBuilder.build();
    com.graphhopper.jsprit.core.problem.vehicle.VehicleType vehicleType =
        createJspritVehicleType(carrierVehicle.getType());
    VehicleImpl.Builder vehicleBuilder =
        VehicleImpl.Builder.newInstance(carrierVehicle.getId().toString());
    vehicleBuilder
   ...
   */
  static CarrierVehicle createCarrierVehicle(
    ...
    if (jspritVehicle.getType().getUserData() != null) {
      log.info(
          "Use the (MATSim) vehicleType that was stored inside the (jsprit) vehicleType during the jsprit run but never interacted with jsprit. ");
      matsimVehicleType =
          (VehicleType)
              jspritVehicle
                  .getType()
                  .getUserData(); // Read in store MATSimVehicleType... Attention: This will not
      // take care for any changes during the jsprit run
    } else {
      log.info(
          "There was no (MATSim) vehicleType stored inside the (jsprit) vehicleType. -> create one from the available data of the (jsprit) vehicle type");
      matsimVehicleType = createMatsimVehicleType(jspritVehicle.getType());
    }
    // yyyy

    String vehicleId = jspritVehicle.getId();
    CarrierVehicle.Builder carrierVehicleBuilder =
    ...
    //		carrierVehicleBuilder.setType(matsimVehicleType); //Not needed any more, because is now part
    // of the constructor. KMT jan22
  ...
  /**
   * Creates a MATSim {@link VehicleType} from a jsprit {@link
   * com.graphhopper.jsprit.core.problem.vehicle.VehicleType}.
   *
   * <p>No description and engineInformation can be set here. Do it by calling
   * setEngineInformation(engineInfo) from the returned object.
   *
   * @param jspritVehType to be transformed
   * @return MatsimVehicleType
   */
  static VehicleType createMatsimVehicleType(
      com.graphhopper.jsprit.core.problem.vehicle.VehicleType jspritVehType) {
    VehicleType matsimVehicleType =
    ...
    matsimVehicleType
        .getCostInformation()
        .setCostsPerMeter(jspritVehType.getVehicleCostParams().perDistanceUnit);
    matsimVehicleType
        .getCostInformation()
        .setCostsPerSecond(jspritVehType.getVehicleCostParams().perTransportTimeUnit);
    matsimVehicleType.getCostInformation().setFixedCost(jspritVehType.getVehicleCostParams().fix);
    VehicleUtils.setCostsPerSecondInService(
        matsimVehicleType.getCostInformation(),
        jspritVehType.getVehicleCostParams().perServiceTimeUnit);
    VehicleUtils.setCostsPerSecondWaiting(
        matsimVehicleType.getCostInformation(),
        jspritVehType.getVehicleCostParams().perWaitingTimeUnit);
   ...
   */
  static com.graphhopper.jsprit.core.problem.vehicle.VehicleType createJspritVehicleType(
      VehicleType matsimVehicleType) {
    if (matsimVehicleType == null) throw new IllegalStateException("carrierVehicleType is null");
    VehicleTypeImpl.Builder jspritVehTypeBuilder =
        VehicleTypeImpl.Builder.newInstance(matsimVehicleType.getId().toString());
    ...
    if (vehicleCapacity - vehicleCapacityInt > 0) {
      log.warn(
          "vehicle capacity truncated to int: before="
              + vehicleCapacity
              + "; after="
              + vehicleCapacityInt);
    ...
    jspritVehTypeBuilder.setCostPerDistance(
        matsimVehicleType.getCostInformation().getCostsPerMeter());

    jspritVehTypeBuilder.setCostPerTransportTime(
        matsimVehicleType.getCostInformation().getCostsPerSecond());
    if (VehicleUtils.getCostsPerSecondInService(matsimVehicleType.getCostInformation()) != null) {
      jspritVehTypeBuilder.setCostPerServiceTime(
          VehicleUtils.getCostsPerSecondInService(matsimVehicleType.getCostInformation()));
    } else {
      log.info(
          "'costsPerSecondInService' is not set in VehicleType attributes. Will use the value of 'costsPerSecond' instead. VehicleTypeId: "
              + matsimVehicleType.getId());
      jspritVehTypeBuilder.setCostPerServiceTime(
          matsimVehicleType.getCostInformation().getCostsPerSecond());
    }
    if (VehicleUtils.getCostsPerSecondWaiting(matsimVehicleType.getCostInformation()) != null) {
      jspritVehTypeBuilder.setCostPerWaitingTime(
          VehicleUtils.getCostsPerSecondWaiting(matsimVehicleType.getCostInformation()));
    } else {
      log.info(
          "'costsPerSecondWaiting' is not set in VehicleType attributes. Will use the value of 'costsPerSecond' instead. VehicleTypeId: "
              + matsimVehicleType.getId());
      jspritVehTypeBuilder.setCostPerWaitingTime(
          matsimVehicleType.getCostInformation().getCostsPerSecond());
    }
  ...
  static ScheduledTour createScheduledTour(VehicleRoute jspritRoute, Id<Tour> tourId) {
    // have made this non-public for the time being since it is nowhere used within the freight
    // contrib, and we might want to add the
    // vehicle types as argument.  If this is publicly used, please move back to public.  kai,
    // jan'22

    if (jspritRoute.getDepartureTime() != jspritRoute.getStart().getEndTime())
      throw new AssertionError(
          "at this point route.getDepartureTime and route.getStart().getEndTime() must be equal");
    ...
    matsimFreightTourBuilder.scheduleStart(
        Id.create(jspritRoute.getStart().getLocation().getId(), Link.class));
    for (TourActivity act : tour.getActivities()) {
      if (act instanceof ServiceActivity || act instanceof PickupService) {
        log.debug(
            "Found ServiceActivity or PickupService : "
                + act.getName()
                + " at location "
      ...
      } else throw new IllegalStateException("unknown tourActivity occurred. this cannot be");
    }
    matsimFreightTourBuilder.addLeg(new Tour.Leg());
    matsimFreightTourBuilder.scheduleEnd(
        Id.create(jspritRoute.getEnd().getLocation().getId(), Link.class));
   ...
   * @return VehicleRoute
   */
  public static VehicleRoute createRoute(
      ScheduledTour scheduledTour, VehicleRoutingProblem vehicleRoutingProblem) {
    ...
    for (Tour.TourElement e : tour.getTourElements()) {
      if (e instanceof Tour.TourActivity) {
        if (e instanceof Tour.ServiceActivity) {
          ...
          Service service =
              (Service) vehicleRoutingProblem.getJobs().get(carrierService.getId().toString());
          if (service == null)
            throw new IllegalStateException(
                "service to id=" + carrierService.getId() + " is missing");
    ...
    log.debug(
        "start-location: "
            + jspritRoute.getStart().getLocation()
            + " endTime: "
            + jspritRoute.getDepartureTime()
            + "("
   ...
   *
   * <p>For creation, it takes only the information needed to set up the problem (not the solution,
   * i.e. predefined plans are ignored).
   *
   * <p>The network is required to retrieve coordinates of locations.
   *
   * <p>Note that currently only services ({@link Service}) and Shipments ({@link Shipment}) are
   * supported.
   *
   * <p>Pickups and deliveries can be defined as shipments with only one location (toLocation for
   * delivery and fromLocation for pickup). Implementation follows
   */
  public static VehicleRoutingProblem createRoutingProblem(
      Carrier carrier,
      Network network,
    ...
    for (CarrierVehicle carrierVehicle : carrierCapabilities.getCarrierVehicles().values()) {
      ...
      if (network != null) {
        ...
        if (link == null)
          throw new IllegalStateException(
              "vehicle.locationId cannot be found in network [vehicleId="
                  + carrierVehicle.getId()
                  + "][locationId="
                  + carrierVehicle.getLinkId()
                  + "]");
        coordinate = link.getCoord();
      } else log.warn("cannot find linkId " + carrierVehicle.getId());
    ...
    if (shipmentInVrp) {
      for (CarrierShipment carrierShipment : carrier.getShipments().values()) {
        ...
        if (network != null) {
          ...
          if (fromLink != null
              && toLink != null) { // Shipment to be delivered from specified location to
            // specified location
            ...
            throw new IllegalStateException(
                "cannot create shipment since neither fromLinkId "
  ...
  /**
   * Creates {@link VehicleRoutingProblem.Builder} from {@link Carrier} for later building of the
   * {@link VehicleRoutingProblem}. This is required if you need to add stuff to the problem later,
   * e.g. because it cannot solely be retrieved from network and carrier such as {@link
   ...
   * delivery and fromLocation for pickup). Implementation follows
   */
  public static VehicleRoutingProblem.Builder createRoutingProblemBuilder(
      Carrier carrier, Network network) {
    VehicleRoutingProblem.Builder vrpBuilder = VehicleRoutingProblem.Builder.newInstance();
    boolean serviceInVrp = !carrier.getServices().isEmpty();
    boolean shipmentInVrp = !carrier.getShipments().isEmpty();
    ...
    if (serviceInVrp) {
      for (CarrierService service : carrier.getServices().values()) {
        ...
        if (network != null) {
          ...
          if (link == null) {
            throw new IllegalStateException(
                "cannot create service since linkId "
                    + service.getLocationLinkId()
          ...
          if (fromLink != null
            ...
            throw new IllegalStateException(
                "cannot create shipment "
                    + carrierShipment.getId().toString()
                    + " since either fromLinkId "
   ...
   *
   * <p>To retrieve coordinates the {@link Network} is required. </br>
   */
  public static VehicleRoutingProblemSolution createSolution(
      CarrierPlan plan, VehicleRoutingProblem vehicleRoutingProblem) {
  ...
  /**
   * Creates a {@link Carrier} from {@link VehicleRoutingProblem}. The carrier is initialized with
   * the carrierId, i.e. <code>carrier.getId()</code> returns carrierId. </br>
   */
   ...
   *
   * <p>Note that the costs of the solution are multiplied by -1 to represent the corresponding
   * score of a carrierPlan.
   *
   * <p>The input parameter {@link Carrier} is just required to initialize the plan. </br>
   */
  ...
  /**
   * Creates or load the VehicleRoutingAlgorithm. If an algorithmFile is given the algorithm will be
   * load. If there is no algorithm given a new algorithm will be created. In both situation the
   * distanceConstraint will be used if he is set in the freightConfigGroup.
   */
  public static VehicleRoutingAlgorithm loadOrCreateVehicleRoutingAlgorithm(
      Scenario scenario,
    ...
    if (vehicleRoutingAlgorithmFile != null && !vehicleRoutingAlgorithmFile.equals("")) {
      ...
      switch (freightConfig.getUseDistanceConstraintForTourPlanning()) {
        case noDistanceConstraint -> algorithm =
            VehicleRoutingAlgorithms.readAndCreateAlgorithm(problem, vraURL);
        case basedOnEnergyConsumption -> {
          ...
          stateManager.addStateUpdater(
              new DistanceUpdater(distanceStateId, stateManager, netBasedCosts));
          ConstraintManager constraintManager = new ConstraintManager(problem, stateManager);
          constraintManager.addConstraint(
              new DistanceConstraint(CarriersUtils.getCarrierVehicleTypes(scenario), netBasedCosts),
              ConstraintManager.Priority.CRITICAL);
          AlgorithmConfig algorithmConfig = new AlgorithmConfig();
          AlgorithmConfigXmlReader xmlReader = new AlgorithmConfigXmlReader(algorithmConfig);
          xmlReader.read(vraURL);
          algorithm =
              VehicleRoutingAlgorithms.readAndCreateAlgorithm(
                  problem, algorithmConfig, 0, null, stateManager, constraintManager, true);
        }
