...
public class CarrierPlanXmlReader implements MatsimReader {
  ...
  private static final String MSG =
      "With early carrier plans file formats, there will be an expected exception in the following.";
  ...
  public void readFile(String filename) {
    ...
    try {
      ...
      log.warn(
          "### Exception found while trying to read CarrierPlan: Message: "
              + e.getMessage()
              + " ; cause: "
              + e.getCause()
              + " ; class "
  ...
  private static final class CarriersPlanReader extends MatsimXmlParser {
    ...
    public void startTag(
      ...
      if (CARRIERS.equalsIgnoreCase(name)) {
        ...
        if (str == null) {
          log.warn(
              "Carrier plans file does not contain a valid xsd header. Using CarrierPlanReaderV2.");
