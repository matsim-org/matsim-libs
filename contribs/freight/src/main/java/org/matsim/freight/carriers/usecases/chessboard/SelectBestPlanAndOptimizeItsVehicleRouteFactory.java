...
final class SelectBestPlanAndOptimizeItsVehicleRouteFactory {
  ...
  public SelectBestPlanAndOptimizeItsVehicleRouteFactory(
      Network network, CarrierVehicleTypes vehicleTypes, TravelTime travelTimes) {
    super();
  ...
  public GenericPlanStrategy<CarrierPlan, Carrier> createStrategy() {
    //		CarrierReplanningStrategy replanningStrat = new CarrierReplanningStrategy(new
    // SelectBestPlan());
    GenericPlanStrategyImpl<CarrierPlan, Carrier> replanningStrat =
        new GenericPlanStrategyImpl<>(new BestPlanSelector<>());

    GenericPlanStrategyModule<CarrierPlan> vraModule =
        new GenericPlanStrategyModule<>() {
          ...
          public void handlePlan(CarrierPlan carrierPlan) {
            ...
            // by default travelTimes are calculated with freeSpeed and vehicleType.maxVelocity on
            // the network
            ...
            VehicleRoutingActivityCosts activitycosts =
                new VehicleRoutingActivityCosts() {
                  ...
                  public double getActivityCost(
                    ...
                    //						double waiting = Math.max(0,
                    // act.getTheoreticalEarliestOperationStartTime() - arrivalTime);
                    //						double service =
                    // act.getOperationTime()*vehicle.getType().getVehicleCostParams().perTimeUnit;
                    //						return penalty4missedTws*tooLate +
                    // vehicle.getType().getVehicleCostParams().perTimeUnit*waiting + service;
                    //						return penalty4missedTws*tooLate;
                    return 0.0;
                  }
            ...
            VehicleRoutingAlgorithm vra =
                VehicleRoutingAlgorithms.readAndCreateAlgorithm(
                    vrp, IOUtils.extendUrl(url, "algorithm.xml"));
