...
/**
 * @author rewert
 *     <p>Includes all classes and methods for the distance constraint of every vehicle with an
 *     energyCapacity. The base for calculating the consumption is only the driven distance and not
 *     the transported weight or other influences. But is possible to integrate it.
 *     <p>!! No recharging or refueling is integrated. Vehicles are totally full at the beginning.
 *     <p>Creates the distance constraint.
 */
/* package-private */ class DistanceConstraint implements HardActivityConstraint {
  ...
  /**
   * When adding a TourActivity to the tour and the new vehicle has an energyCapacity (fuel or
   * electricity etc.) the algorithm always checks the fulfilled method if all conditions
   * (constraints) are fulfilled or not. Because every activity is added separately and the pickup
   * before the delivery of a shipment, it will investigate which additional distance is necessary
   * for the pickup and which minimal additional distance of the associated Delivery is needed. This
   * is also important for the fulfilled decision of this function. At the end the conditions checks
   * if the consumption of the tour including the additional shipment is possible with the possible
   * energyCapacity.
   */
  // TODO add the time dependencies of the distance calculation because the route choice can be
  ...
  public ConstraintsStatus fulfilled(
    ...
    VehicleType vehicleTypeOfNewVehicle =
        vehicleTypes
            .getVehicleTypes()
    ...
    if (VehicleUtils.getEnergyCapacity(vehicleTypeOfNewVehicle.getEngineInformation()) != null) {
      ...
      Double energyCapacityInKWhOrLiters =
          VehicleUtils.getEnergyCapacity(vehicleTypeOfNewVehicle.getEngineInformation());
      Double consumptionPerMeter;
      if (VehicleUtils.getHbefaTechnology(vehicleTypeOfNewVehicle.getEngineInformation())
          .equals("electricity"))
        consumptionPerMeter =
            VehicleUtils.getEnergyConsumptionKWhPerMeter(
                vehicleTypeOfNewVehicle.getEngineInformation());
      else consumptionPerMeter = VehicleUtils.getFuelConsumption(vehicleTypeOfNewVehicle);
      ...
      if (newAct instanceof PickupShipment) {
        // calculates the additional distance for adding a pickup and checks the shortest
        // possibility for adding the associated delivery
        additionalDistance =
            getDistance(prevAct, newAct, newVehicle)
                + getDistance(newAct, nextAct, newVehicle)
                - getDistance(prevAct, nextAct, newVehicle)
                + findMinimalAdditionalDistance(context, newAct, newAct);
  ...
  /**
   * Calculates the distance based on the route-based distances between every tour activities. The
   * method also integrates the associated pickup in the tour.
   */
  private double calculateRouteDistanceWithAssociatedPickup(JobInsertionContext context) {
    ...
    if (positionOfRelatedPickup == 0 && context.getRoute().getActivities().isEmpty()) {
      ...
      routeDistance =
          getDistance(
              context.getAssociatedActivities().get(0),
              context.getRoute().getEnd(),
              context.getNewVehicle(),
              context.getNewDepTime());
      return routeDistance;
    } else if (positionOfRelatedPickup == 0 && !context.getRoute().getActivities().isEmpty()) {
      routeDistance =
          getDistance(
              context.getAssociatedActivities().get(0),
              context.getRoute().getActivities().get(0),
              context.getNewVehicle(),
              context.getNewDepTime());
    } else {
      routeDistance =
          getDistance(
              context.getRoute().getStart(),
              context.getRoute().getActivities().get(0),
              context.getNewVehicle(),
              context.getNewDepTime());
    }
    ...
    while (context.getRoute().getTourActivities().getActivities().size()
        > (nextRouteActivity + 1)) {
      if (positionOfRelatedPickup == (nextRouteActivity + 1) && positionOfRelatedPickup != 0) {
        ...
        routeDistance =
            routeDistance
                + getDistance(
                    context.getRoute().getActivities().get(nextRouteActivity),
                    context.getRoute().getActivities().get(nextRouteActivity + 1),
                    context.getNewVehicle());
      }
      nextRouteActivity++;
    }
  ...
  /** Calculates the distance based on the route-based distances between every tour activities. */
  private double calculateRouteDistance(JobInsertionContext context, Vehicle newVehicle) {
    double realRouteDistance = 0;
    if (context.getRoute().getTourActivities().getActivities().isEmpty()) return realRouteDistance;
    int n = 0;
    realRouteDistance =
        getDistance(
            context.getRoute().getStart(),
            context.getRoute().getTourActivities().getActivities().get(0),
            newVehicle);
    while (context.getRoute().getTourActivities().getActivities().size() > (n + 1)) {
      realRouteDistance =
          realRouteDistance
              + getDistance(
                  context.getRoute().getTourActivities().getActivities().get(n),
                  context.getRoute().getTourActivities().getActivities().get(n + 1),
                  newVehicle);
      n++;
    }
    realRouteDistance =
  ...
  /**
   * Finds a minimal additional distance for the tour, when a pickup is added to the plan. The
   * AssociatedActivities contains both activities of a job which should be added to the existing
   * tour. The TourActivities which are already in the tour are found in
   * context.getRoute().getTourActivities. In this method the position of the new pickup is fixed
   * and three options of the location of the delivery activity will be checked: delivery between
   * every other activity after the pickup, delivery as the last activity before the end and
   * delivery directly behind the new pickup. This method gives back the minimal distance of this
   * three options.
   *
   * @param context
   ...
   */
  private double findMinimalAdditionalDistance(
      JobInsertionContext context, TourActivity newInvestigatedPickup, TourActivity nextAct) {
    ...
    if (context.getAssociatedActivities().get(1) instanceof DeliverShipment) {
      ...
      for (TourActivity tourActivity : route.getTourActivities().getActivities()) {
        if (tourActivity.getIndex() == indexNextActivity) {
          while (countIndex < route.getTourActivities().getActivities().size()) {
            if (route.getTourActivities().getActivities().get(countIndex).getIndex()
                == indexNextActivity) {
      ...
      while ((tourPositionOfActivityBehindNewPickup + 1)
          < route.getTourActivities().getActivities().size()) {
        TourActivity activityBefore =
            route.getTourActivities().getActivities().get(tourPositionOfActivityBehindNewPickup);
        TourActivity activityAfter =
            route
                .getTourActivities()
                .getActivities()
                .get(tourPositionOfActivityBehindNewPickup + 1);
        double possibleAdditionalDistance =
            getDistance(activityBefore, assignedDelivery, newVehicle)
                + getDistance(assignedDelivery, activityAfter, newVehicle)
                - getDistance(activityBefore, activityAfter, newVehicle);
        minimalAdditionalDistance =
            findMinimalDistance(minimalAdditionalDistance, possibleAdditionalDistance);
      ...
      if (route.getTourActivities().getActivities().size() > 0) {
        TourActivity activityLastDelivery =
            route
                .getTourActivities()
                .getActivities()
        ...
        double possibleAdditionalDistance =
            getDistance(activityLastDelivery, assignedDelivery, newVehicle)
        ...
        TourActivity activityAfter =
            route.getTourActivities().getActivities().get(tourPositionOfActivityBehindNewPickup);
        possibleAdditionalDistance =
            getDistance(newInvestigatedPickup, assignedDelivery, newVehicle)
                + getDistance(assignedDelivery, activityAfter, newVehicle)
   ...
   */
  private double findMinimalDistance(
      double minimalAdditionalDistance, double possibleAdditionalDistance) {
    if (minimalAdditionalDistance == 0) minimalAdditionalDistance = possibleAdditionalDistance;
  ...
  private double getDistance(TourActivity from, TourActivity to, Vehicle vehicle) {
    double distance =
        netBasedCosts.getDistance(from.getLocation(), to.getLocation(), from.getEndTime(), vehicle);
    if (!(distance >= 0.))
  ...
  private double getDistance(
      TourActivity from, TourActivity to, Vehicle vehicle, double departureTime) {
    double distance =
        netBasedCosts.getDistance(from.getLocation(), to.getLocation(), departureTime, vehicle);
    if (!(distance >= 0.))
      throw new AssertionError(
          "Distance must not be negative! From, to" + from + ", " + to + " distance " + distance);
    return distance;
