...
final class ExampleTwoEchelonGrid_shipmentBasedDistribution {
	...
	public static void main(String[] args) {
		...
		new LSPPlanXmlWriter(LSPUtils.getLSPs(controller.getScenario())).write(controller.getConfig().controller().getOutputDirectory() + "/lsps.xml");
		new LSPPlanXmlReader(LSPUtils.getLSPs(controller.getScenario()), CarriersUtils.getCarriers(controller.getScenario()));
	...
	private static Collection<LspShipment> createInitialLSPShipments(Network network) {
		...
		switch (demandSetting) {
			...
			case tenCustomers -> {
				...
				List<String> zoneLinkList = Arrays.asList(
	...
	private static List<LSPResource> createResourcesListFromLSPPlans(List<LSPPlan> lspPlans) {
		...
		List<LSPResource> resourcesList = new ArrayList<>(); // TODO: Mache daraus ein Set, damit jede Resource nur einmal drin ist?
		// kmt Feb22
