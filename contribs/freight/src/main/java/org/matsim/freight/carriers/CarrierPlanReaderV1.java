...
class CarrierPlanReaderV1 extends MatsimXmlParser {
    ...
    switch (name) {
      ...
      case SHIPMENT:
        {
          ...
          CarrierShipment.Builder shipmentBuilder =
              CarrierShipment.Builder.newInstance(
                  Id.create(id, CarrierShipment.class),
                  Id.create(from, Link.class),
                  Id.create(to, Link.class),
                  size);
          if (startPickup == null) {
            shipmentBuilder
                .setPickupTimeWindow(TimeWindow.newInstance(0.0, Integer.MAX_VALUE))
                .setDeliveryTimeWindow(TimeWindow.newInstance(0.0, Integer.MAX_VALUE));
          } else {
            shipmentBuilder
                .setPickupTimeWindow(
                    TimeWindow.newInstance(getDouble(startPickup), getDouble(endPickup)))
                .setDeliveryTimeWindow(
                    TimeWindow.newInstance(getDouble(startDelivery), getDouble(endDelivery)));
          }
          if (pickupServiceTime != null)
            shipmentBuilder.setPickupServiceTime(getDouble(pickupServiceTime));
          if (deliveryServiceTime != null)
            shipmentBuilder.setDeliveryServiceTime(getDouble(deliveryServiceTime));
      ...
      case VEHICLE:
        {
          ...
          if (typeId == null) {
            logger.warn(
                "no vehicle type. set type='default' -> defaultVehicleType (see CarrierVehicleTypeImpl)");
            typeId = "default";
          }

          VehicleType vehicleType =
              carrierVehicleTypes.getVehicleTypes().get(Id.create(typeId, VehicleType.class));
          if (vehicleType == null) {
          ...
          CarrierVehicle.Builder vehicleBuilder =
              CarrierVehicle.Builder.newInstance(
                  Id.create(vId, Vehicle.class), Id.create(linkId, Link.class), vehicleType);
      ...
      case ACTIVITY:
        {
          switch (attributes.getValue(TYPE)) {
            case "start" -> {
              ...
              currentTourBuilder.scheduleStart(
                  currentVehicle.getLinkId(),
                  TimeWindow.newInstance(
                      currentVehicle.getEarliestStartTime(), currentVehicle.getLatestEndTime()));
            }
            case "pickup" -> {
      ...
      default:
        throw new RuntimeException(
            "encountered xml tag=" + name + " which cannot be processed; aborting ...");
  ...
  private void finishLeg(Id<Link> toLocation) {
    ...
    currentTourBuilder.addLeg(
        currentTourBuilder.createLeg(route, currentLegDepTime, currentLegTransTime));
  ...
  public void endTag(String name, String content, Stack<String> context) {
    ...
    if (name.equals("tour")) {
      ScheduledTour sTour =
          ScheduledTour.newInstance(currentTourBuilder.build(), currentVehicle, currentStartTime);
