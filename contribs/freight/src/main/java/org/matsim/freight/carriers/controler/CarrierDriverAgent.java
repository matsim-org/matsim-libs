...
/**
 * This keeps track of a scheduledTour during simulation and can thus be seen as the driver of the
 * vehicle that runs the tour.
 *
 * <p>In addition, the driver knows which planElement is associated to a shipment and service,
 * respectively.
 *
...
final class CarrierDriverAgent {
  ...
  CarrierDriverAgent(
      ...
      Collection<CarrierEventCreator> carrierEventCreators) {
  ...
  private void handleEvent(PersonArrivalEvent event) {
    ...
    double travelTime =
        currentLeg.getDepartureTime().seconds()
            + currentLeg.getTravelTime().seconds()
            - currentLeg.getDepartureTime().seconds();
    ...
    if (event.getActType().equals(CarrierConstants.END)) {
      ...
      if (!activity.getLinkId().toString().equals(tourActivity.getLocation().toString()))
        throw new AssertionError(
            "linkId of activity is not equal to linkId of tourActivity. This must not be.");
      currentActivity = new FreightActivity(activity, tourActivity.getTimeWindow());
    }
    createAdditionalEvents(event, currentActivity, scheduledTour, driverId, planElementCounter);
    // yyyyyy uses the previous activity, not the current (end) activity.  Bug or feature?  Only
    // used by LSP, not by carrier.  kai, jul'22
  }

  private void createAdditionalEvents(
      Event event,
      Activity activity,
    ...
    // Reason why this here is needed is that the more informative objects such as ScheduledTour
    // cannot be
    // filled from just listening to events.  kai, jul'22
    for (CarrierEventCreator carrierEventCreator : carrierEventCreators) {
      Event freightEvent =
          carrierEventCreator.createEvent(
              event, carrier, activity, scheduledTour, activityCounter, vehicleId);
  ...
  private Tour.TourActivity getTourActivity() {
    return (Tour.TourActivity)
        this.scheduledTour.getTour().getTourElements().get(planElementCounter);
  }
  ...
  /**
   * Basic event handler that collects the relation between vehicles and drivers. Necessary since
   * link enter and leave events do not contain the driver anymore.
   *
   * <p>This is the vice-versa implementation of {@link
   * org.matsim.core.events.algorithms.Vehicle2DriverEventHandler}.
   *
   * <p>In a first step only used internally. When needed more often, I have nothing against putting
   * it more central.
   *
   * @author kturner
   */
  private final class Driver2VehicleEventHandler
    ...
    // maybe change, that it is removed when a ActivityStartEvent- as last point of a leg - is
    // thrown.
    // kmt sep'22
    @Override
