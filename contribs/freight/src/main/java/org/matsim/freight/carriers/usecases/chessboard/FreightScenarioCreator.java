...
final class FreightScenarioCreator {
  ...
  public static void main(String[] args) {
    ...
    new CarrierPlanWriter(carriers)
        .write(
            "input/usecases/chessboard/freight/multipleCarriers_withoutTW_withDepots_withoutPlan.xml");
  }
    ...
    for (int i = 0; i < 20; i++) {
      CarrierService.Builder serviceBuilder =
          CarrierService.Builder.newInstance(
              Id.create((i + 1), CarrierService.class),
              drawLocationLinkId(innerCityLinks, outerCityLinks));
  ...
  private static Id<Link> drawLocationLinkId(
      List<Id<Link>> innerCityLinks, List<Id<Link>> outerCityLinks) {
  ...
  private static CarrierVehicle createLightVehicle(Id<?> id, Id<Link> homeId, String depot) {
    CarrierVehicle.Builder vBuilder =
        CarrierVehicle.Builder.newInstance(
            Id.create(("carrier_" + id.toString() + "_lightVehicle_" + depot), Vehicle.class),
            homeId,
            createLightType());
  ...
  private static VehicleType createLightType() {
    VehicleType typeBuilder =
        VehicleUtils.getFactory().createVehicleType(Id.create("small", VehicleType.class));
  ...
  private static CarrierVehicle createHeavyVehicle(Id<?> id, Id<Link> homeId, String depot) {
    CarrierVehicle.Builder vBuilder =
        CarrierVehicle.Builder.newInstance(
            Id.create("carrier_" + id.toString() + "_heavyVehicle_" + depot, Vehicle.class),
            homeId,
            createHeavyType());
    vBuilder.setEarliestStart(6 * 60 * 60);
    vBuilder.setLatestEnd(16 * 60 * 60);
  ...
  private static VehicleType createHeavyType() {
    VehicleType typeBuilder =
        VehicleUtils.getFactory().createVehicleType(Id.create("heavy", VehicleType.class));
