...
public class CarrierControlerUtilsIT {
  ...
  public void numberOfToursIsCorrect() {
    ...
    Assert.assertEquals(
        1, carrierWShipmentsOnlyFromCarrierWServices.getSelectedPlan().getScheduledTours().size());
    Assert.assertEquals(
        1, carrierWShipmentsOnlyFromCarrierWShipments.getSelectedPlan().getScheduledTours().size());
  }

  /**
   * TODO Calculation of tour distance and duration are commented out, because ...tour.getEnd() has
   * no values -> need for fixing in NetworkRouter or somewhere else kmt/okt18
   */
  @Test
  public void toursInitialCarrierWServicesIsCorrect() {
    Assert.assertEquals(
        -270.462,
        carrierWServices.getSelectedPlan().getJspritScore(),
        MatsimTestUtils
            .EPSILON); // Note: In score waiting and serviceDurationTime are not includes by now ->
    // May fail, when fixed. KMT Okt/18
    //		double tourDurationSum = 0;
    //		for (ScheduledTour scheduledTour: carrierWServices.getSelectedPlan().getScheduledTours()){
    //			tourDurationSum += scheduledTour.getTour().getEnd().getExpectedArrival() -
  ...
  public void toursInitialCarrierWShipmentsIsCorrect() {
    Assert.assertEquals(
        -136.87,
        carrierWShipments.getSelectedPlan().getJspritScore(),
        MatsimTestUtils
            .EPSILON); // Note: In score waiting and serviceDurationTime are not includes by now ->
    // May fail, when fixed. KMT Okt/18
  ...
  public void toursCarrierWShipmentsOnlyFromCarrierWServicesIsCorrect() {
    Assert.assertEquals(
        -140.462,
        carrierWShipmentsOnlyFromCarrierWServices.getSelectedPlan().getJspritScore(),
        MatsimTestUtils
            .EPSILON); // Note: In score waiting and serviceDurationTime are not includes by now ->
    // May fail, when fixed. KMT Okt/18

    //		double tourDurationSum = 0;
    //		for (ScheduledTour scheduledTour:
    // carrierWShipmentsOnlyFromCarrierWServices.getSelectedPlan().getScheduledTours()){
    //			tourDurationSum += scheduledTour.getTour().getEnd().getExpectedArrival() -
    // scheduledTour.getDeparture();
    //		}
    ...
    //					tourLengthSum += ((Leg) te).getRoute().getDistance();
    ////					System.out.println("Added:" + ((Leg) te).getRoute().getDistance() + " ; Sum Tour
    // Length: " + tourLengthSum);
    //				}
    //			}
    //		}
    //		Assert.assertEquals(52000, tourLengthSum, 0);
  ...
  public void toursCarrierWShipmentsOnlyFromCarrierWShipmentsIsCorrect() {
    Assert.assertEquals(
        -136.87,
        carrierWShipmentsOnlyFromCarrierWShipments.getSelectedPlan().getJspritScore(),
        MatsimTestUtils
            .EPSILON); // Note: In score waiting and serviceDurationTime are not includes by now ->
    // May fail, when fixed. KMT Okt/18
    ...
    //		double tourDurationSum = 0;
    //		for (ScheduledTour scheduledTour:
    // carrierWShipmentsOnlyFromCarrierWShipments.getSelectedPlan().getScheduledTours()){
    //			tourDurationSum += scheduledTour.getTour().getEnd().getExpectedArrival() -
    // scheduledTour.getDeparture();
    //		}
    //		Assert.assertEquals(5260.0 , tourDurationSum, 0);
