...
public class SkillsIT {
  ...
  public void testJspritWithDifferentSkillsRequired() {
    ...
    Assert.assertEquals(
        "Wrong number of vehicles.", 2L, solutionWithDifferentSkills.getRoutes().size());
    Assert.assertEquals(
        "Wrong carrier score.",
        2086.9971014492755,
        solutionWithDifferentSkills.getCost(),
  ...
  public void testJspritWithSameSkillsRequired() {
    ...
    Assert.assertEquals(
        "Wrong carrier score.",
        1044.0985507246377,
        solutionWithSameSkills.getCost(),
        MatsimTestUtils.EPSILON);
  }
  ...
  private VehicleRoutingProblemSolution generateCarrierPlans(Scenario scenario) {
    ...
    NetworkBasedTransportCosts networkBasedTransportCosts =
    ...
    //		new
    // CarrierPlanXmlWriterV3(CarrierControlerUtils.getCarriers(scenario)).write(utils.getOutputDirectory() + "carriers.xml");
    //		Scenario scNew = ScenarioUtils.createScenario(ConfigUtils.createConfig());
    //		new
    // CarrierPlanXmlReaderV3(CarrierControlerUtils.getCarriers(scNew)).readFile(utils.getOutputDirectory() + "carriers.xml");
    return solution;
  }
  ...
  private Scenario setupTestScenario() {
    ...
    new MatsimNetworkReader(scenario.getNetwork())
        .readFile(utils.getClassInputDirectory() + "grid-network.xml");
      ...
      Carrier carrier = CarriersUtils.createCarrier(Id.create("TestCarrier", Carrier.class));
      {
        ...
        VehicleType typeOne =
            scenario
                .getVehicles()
                .getFactory()
                .createVehicleType(Id.create("Type 1", VehicleType.class));
        //				typeOne.setCostInformation(costInformation1);
        typeOne
            .getCostInformation()
            .setFixedCost(1000.0)
            .setCostsPerMeter(0.001)
            .setCostsPerSecond(0.001);
        ...
        CarrierVehicle vehicleOne =
            CarrierVehicle.Builder.newInstance(Id.createVehicleId("1"), carrierLocation, typeOne)
        ...
        CarrierVehicle vehicleTwo =
            CarrierVehicle.Builder.newInstance(Id.createVehicleId("2"), carrierLocation, typeTwo)
                .setEarliestStart(0.0)
                .setLatestEnd(Time.parseTime("24:00:00"))
                .build();
