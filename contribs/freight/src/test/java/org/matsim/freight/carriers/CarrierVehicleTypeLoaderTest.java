...
public class CarrierVehicleTypeLoaderTest {
  ...
  public void setUp() throws Exception {
    ...
    new CarrierPlanXmlReader(carriers, types)
        .readFile(utils.getClassInputDirectory() + "carrierPlansEquils.xml");
  }
  ...
  public void test_whenLoadingTypes_allAssignmentsInLightVehicleAreCorrectly() {
    ...
    Assert.assertEquals(
        20, vehicleTypeLoaded.getCostInformation().getFixedCosts(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        0.35, vehicleTypeLoaded.getCostInformation().getCostsPerMeter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        30, vehicleTypeLoaded.getCostInformation().getCostsPerSecond(), MatsimTestUtils.EPSILON);

    Assert.assertEquals(
        "gasoline", vehicleTypeLoaded.getEngineInformation().getFuelType().toString());
    Assert.assertEquals(
        0.02, VehicleUtils.getFuelConsumption(vehicleTypeLoaded), MatsimTestUtils.EPSILON);
  }
  ...
  public void test_whenLoadingTypes_allAssignmentsInMediumVehicleAreCorrectly() {
    ...
    Assert.assertEquals(
        50, vehicleTypeLoaded.getCostInformation().getFixedCosts(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        0.4, vehicleTypeLoaded.getCostInformation().getCostsPerMeter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        30, vehicleTypeLoaded.getCostInformation().getCostsPerSecond(), MatsimTestUtils.EPSILON);
