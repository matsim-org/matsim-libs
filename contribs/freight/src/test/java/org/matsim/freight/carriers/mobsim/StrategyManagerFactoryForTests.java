...
public class StrategyManagerFactoryForTests implements Provider<CarrierStrategyManager> {
  ...
  public CarrierStrategyManager get() {
    ...
    final LeastCostPathCalculator router =
        new SpeedyALTFactory()
            .createPathCalculator(
                network,
                new MyTravelCosts(travelTimes.get(TransportMode.car)),
    ...
    planStrat_reRoutePlan.addStrategyModule(
        new CarrierReRouteVehicles.Factory(router, network, travelTimes.get(TransportMode.car))
            .build());

    CarrierStrategyManager stratManager =
