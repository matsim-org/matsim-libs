...
public class EquilWithCarrierWithPersonsIT {
  ...
  public void setUp() {
    ...
    config
        .replanning()
        .addStrategySettings(new StrategySettings().setStrategyName("BestScore").setWeight(1.0));
    config
        .replanning()
        .addStrategySettings(
            new StrategySettings().setStrategyName("ReRoute").setWeight(0.0).setDisableAfter(300));
  ...
  static Scenario commonScenario(Config config, MatsimTestUtils testUtils) {
    ...
    new CarrierPlanXmlReader(carriers, carrierVehicleTypes)
        .readFile(testUtils.getClassInputDirectory() + "carrierPlansEquils.xml");
    return scenario;
  }
