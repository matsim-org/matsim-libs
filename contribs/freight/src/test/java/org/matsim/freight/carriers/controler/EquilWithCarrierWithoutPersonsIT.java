...
public class EquilWithCarrierWithoutPersonsIT {
        ...
        new AbstractModule() {
          ...
          public void install() {
            bind(CarrierStrategyManager.class)
                .toProvider(StrategyManagerFactoryForTests.class)
                .asEagerSingleton();
            bind(CarrierScoringFunctionFactory.class)
                .to(DistanceScoringFunctionFactoryForTests.class)
                .asEagerSingleton();
          }
        });
    controler.run();
  ...
  public void testWithoutCarrierRoutes() {
    ...
    // this fails in CarrierAgent#createDriverPlans(...).  Could be made pass there, but then does
    // not seem to drive on network.  Would
    // need carrier equivalent to PersonPrepareForSim.  Could then adapt this test accordingly. kai,
    // jul'22
    controler.run();
  }
  ...
  public void testScoringInMeters() {
    ...
    Carrier carrier1 =
        CarriersUtils.getCarriers(controler.getScenario())
            .getCarriers()
            .get(Id.create("carrier1", Carrier.class));
    Assert.assertEquals(-170000.0, carrier1.getSelectedPlan().getScore(), 0.0);

    Carrier carrier2 =
        CarriersUtils.getCarriers(controler.getScenario())
            .getCarriers()
            .get(Id.create("carrier2", Carrier.class));
  ...
  public void testScoringInSecondsWoTimeWindowEnforcement() {
    ...
    if (false) {
      ...
      freightCarriersConfigGroup.setTimeWindowHandling(
          FreightCarriersConfigGroup.TimeWindowHandling.ignore);
    }
    controler.addOverridingModule(new CarrierModule());
    controler.addOverridingModule(
        new AbstractModule() {
          ...
          public void install() {
            bind(CarrierStrategyManager.class)
                ...
                .asEagerSingleton();
            bind(CarrierScoringFunctionFactory.class)
                .to(TimeScoringFunctionFactoryForTests.class)
                .asEagerSingleton();
          }
        });
    controler.run();
