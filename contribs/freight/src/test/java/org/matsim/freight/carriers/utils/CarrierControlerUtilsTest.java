...
public class CarrierControlerUtilsTest {
  ...
  public void setUp() {
    ...
    //		VehicleType carrierVehType = CarriersUtils.CarrierVehicleTypeBuilder.newInstance(
    // vehicleTypeId )
    ...
    CarrierVehicle carrierVehicle =
        CarrierVehicle.Builder.newInstance(
                Id.create("gridVehicle", org.matsim.vehicles.Vehicle.class),
                Id.createLinkId("i(6,0)"),
                carrierVehType)
            .setEarliestStart(0.0)
            .setLatestEnd(36000.0)
            .build();
    CarrierCapabilities.Builder ccBuilder =
    ...
    new MatsimNetworkReader(network)
        .readFile(testUtils.getPackageInputDirectory() + "grid-network.xml");
    Builder netBuilder =
    ...
    // Build jsprit, solve and route VRP for carrierService only -> need solution to convert
    // Services to Shipments
    VehicleRoutingProblem.Builder vrpBuilder =
        MatsimJspritFactory.createRoutingProblemBuilder(carrierWServices, network);
    ...
    CarrierPlan carrierPlanServicesAndShipments =
        MatsimJspritFactory.createPlan(carrierWServices, bestSolution);
    ...
    Carriers carriersWithShipmentsOnly =
        CarriersUtils.createShipmentVRPCarrierFromServiceVRPSolution(
            carriersWithServicesAndShpiments);
    carrierWShipmentsOnlyFromCarrierWServices =
        carriersWithShipmentsOnly.getCarriers().get(CARRIER_SERVICES_ID); // with converted Service
    carrierWShipmentsOnlyFromCarrierWShipments =
        carriersWithShipmentsOnly.getCarriers().get(CARRIER_SHIPMENTS_ID); // with copied Shipments
  ...
  public void numberOfShipmentsFromCopiedShipmentsIsCorrect() {
    ...
    int demandShipments = 0;
    for (CarrierShipment carrierShipment :
        carrierWShipmentsOnlyFromCarrierWServices.getShipments().values()) {
      demandShipments += carrierShipment.getSize();
    }
  ...
  public void fleetAvailableAfterConvertingIsCorrect() {
    Assert.assertEquals(
        FleetSize.INFINITE,
        carrierWShipmentsOnlyFromCarrierWServices.getCarrierCapabilities().getFleetSize());
    Assert.assertEquals(
        1,
        carrierWShipmentsOnlyFromCarrierWServices
            ...
            .getVehicleTypes()
            .size());
    for (VehicleType carrierVehicleType :
        carrierWShipmentsOnlyFromCarrierWServices.getCarrierCapabilities().getVehicleTypes()) {
      ...
      Assert.assertEquals(
          "diesel", VehicleUtils.getHbefaTechnology(carrierVehicleType.getEngineInformation()));
      Assert.assertEquals(0.015, VehicleUtils.getFuelConsumption(carrierVehicleType), 0.0);
    }

    Assert.assertEquals(
        FleetSize.INFINITE,
        carrierWShipmentsOnlyFromCarrierWShipments.getCarrierCapabilities().getFleetSize());
    Assert.assertEquals(
        1,
        carrierWShipmentsOnlyFromCarrierWShipments
            .getCarrierCapabilities()
            ...
            .size());
    for (VehicleType carrierVehicleType :
        carrierWShipmentsOnlyFromCarrierWShipments.getCarrierCapabilities().getVehicleTypes()) {
      Assert.assertEquals(3., carrierVehicleType.getCapacity().getOther(), Double.MIN_VALUE);
      Assert.assertEquals(130, carrierVehicleType.getCostInformation().getFixedCosts(), 0.0);
      Assert.assertEquals(0.0001, carrierVehicleType.getCostInformation().getCostsPerMeter(), 0.0);
      Assert.assertEquals(0.001, carrierVehicleType.getCostInformation().getCostsPerSecond(), 0.0);
  ...
  public void copiingOfShipmentsIsDoneCorrectly() {
    ...
    CarrierShipment carrierShipment1 =
        CarriersUtils.getShipment(
            carrierWShipmentsOnlyFromCarrierWShipments,
    ...
    CarrierShipment carrierShipment2 =
        CarriersUtils.getShipment(
            carrierWShipmentsOnlyFromCarrierWShipments,
  ...
  public void convertionOfServicesIsDoneCorrectly() {
    ...
    CarrierShipment carrierShipment1 =
        CarriersUtils.getShipment(
            carrierWShipmentsOnlyFromCarrierWServices,
            Id.create("Service1", CarrierShipment.class));
    assert carrierShipment1 != null;
    ...
    CarrierShipment carrierShipment2 =
        CarriersUtils.getShipment(
            carrierWShipmentsOnlyFromCarrierWServices,
            Id.create("Service2", CarrierShipment.class));
    assert carrierShipment2 != null;
  ...
  public void exceptionIsThrownWhenUsingMixedShipmentsAndServices() {
    Carrier carrierMixedWServicesAndShipments =
        CarriersUtils.createCarrier(Id.create("CarrierMixed", Carrier.class));
    CarrierService service1 = createMatsimService("Service1", "i(3,9)", 2);
  ...
  private static CarrierService createMatsimService(String id, String to, int size) {
    return CarrierService.Builder.newInstance(
            Id.create(id, CarrierService.class), Id.create(to, Link.class))
  ...
  public void testAddVehicleTypeSkill() {
    VehiclesFactory factory =
        ScenarioUtils.createScenario(ConfigUtils.createConfig()).getVehicles().getFactory();
    ...
    Assert.assertTrue(
        "Should have skill 'testSkillOne'.", CarriersUtils.hasSkill(type, "testSkillOne"));

    CarriersUtils.addSkill(type, "testSkillTwo");
    Assert.assertTrue(
        "Should have skill 'testSkillOne'.", CarriersUtils.hasSkill(type, "testSkillOne"));
    Assert.assertTrue(
        "Should have skill 'testSkillTwo'.", CarriersUtils.hasSkill(type, "testSkillTwo"));
  }
  ...
  public void testAddShipmentSkill() {
    CarrierShipment shipment =
        CarrierShipment.Builder.newInstance(
                Id.create("testShipment", CarrierShipment.class),
                Id.createLinkId("1"),
                Id.createLinkId("2"),
                1)
            .build();
    Assert.assertFalse("Should not have skill.", CarriersUtils.hasSkill(shipment, "testSkill"));

    CarriersUtils.addSkill(shipment, "testSkillOne");
    Assert.assertTrue(
        "Should have skill 'testSkillOne'.", CarriersUtils.hasSkill(shipment, "testSkillOne"));

    CarriersUtils.addSkill(shipment, "testSkillTwo");
    Assert.assertTrue(
        "Should have skill 'testSkillOne'.", CarriersUtils.hasSkill(shipment, "testSkillOne"));
    Assert.assertTrue(
        "Should have skill 'testSkillTwo'.", CarriersUtils.hasSkill(shipment, "testSkillTwo"));
  }
  ...
  public void testAddServiceSkill() {
    CarrierService service =
        CarrierService.Builder.newInstance(
            ...
            .build();
    Assert.assertFalse("Should not have skill.", CarriersUtils.hasSkill(service, "testSkill"));

    CarriersUtils.addSkill(service, "testSkillOne");
    Assert.assertTrue(
        "Should have skill 'testSkillOne'.", CarriersUtils.hasSkill(service, "testSkillOne"));

    CarriersUtils.addSkill(service, "testSkillTwo");
    Assert.assertTrue(
        "Should have skill 'testSkillOne'.", CarriersUtils.hasSkill(service, "testSkillOne"));
    Assert.assertTrue(
        "Should have skill 'testSkillTwo'.", CarriersUtils.hasSkill(service, "testSkillTwo"));
  }
  ...
  public void testRunJsprit_allInformationGiven() {
    ...
    Assert.assertEquals(
        vraFile,
        ConfigUtils.addOrGetModule(controler.getConfig(), FreightCarriersConfigGroup.class)
            .getVehicleRoutingAlgorithmFile());
  }

  /**
   * This test should lead to an exception, because the NumberOfJspritIterations is not set for
   * carriers.
   */
  @Test(expected = java.util.concurrent.ExecutionException.class)
  public void testRunJsprit_NoOfJspritIterationsMissing()
  ...
  private Config prepareConfig() {
    ...
    freightCarriersConfigGroup.setCarriersFile(
        IOUtils.extendUrl(scenarioUrl, "singleCarrierFiveActivitiesWithoutRoutes.xml").toString());
    freightCarriersConfigGroup.setCarriersVehicleTypesFile(
        IOUtils.extendUrl(scenarioUrl, "vehicleTypes.xml").toString());
