...
public class GeoserverUpdater implements FacilityDataExchangeInterface {
  ...
  public void finish() {
    ...
    DefaultFeatureCollection featureCollection =
        createFeatureCollection(geometryFactory, featureType);
  ...
  private DefaultFeatureCollection createFeatureCollection(
      GeometryFactory geometryFactory, SimpleFeatureType featureType) {
    ...
    CoordinateTransformation transformation =
        TransformationFactory.getCoordinateTransformation(this.crs, TransformationFactory.WGS84);

    for (Entry<Tuple<ActivityFacility, Double>, Map<String, Double>> entry :
        accessibilitiesMap.entrySet()) {
      ...
      for (String currentAdditionalFacInfo : additionalFacInfo) {
        Double additionalFacInfoValue =
            Double.parseDouble(
                entry
                    .getKey()
                    .getFirst()
                    .getAttributes()
                    .getAttribute(currentAdditionalFacInfo)
                    .toString());
  ...
  private void updateOnGeoserver(
      SimpleFeatureType featureType, DefaultFeatureCollection featureCollection) {
    ...
    try {
      ...
      // There have been errors with the data store if the dependency "gt-jdbc-postgis", version
      // 13.0 was missing!
    ...
    // Re-publish layer using the REST api (of geoserver; the above is the postgis db) if we want to
    // automatically recompute the bounding box.  mz & kai, nov'15
  }
}
