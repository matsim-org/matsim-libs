 ...
 */
class OSMCoordUtils {
  private static final Logger LOG = LogManager.getLogger(OSMCoordUtils.class);
  ...
  /** Calculate the centre of the way as the centroid of the bounding box of the facility */
  private static Coord getWayCentroid(
      Way way, CoordinateTransformation ct, Map<Long, NodeContainer> nodeMap) {
  ...
  /** Calculate the centre of the relation as the centroid of the bounding box of the facility */
  private static Coord getRelationCentroid(
      Relation relation,
      CoordinateTransformation ct,
      Map<Long, NodeContainer> nodeMap,
      Map<Long, WayContainer> wayMap,
  ...
  public static Coord[] getAllWayCoords(
      Way way, CoordinateTransformation ct, Map<Long, NodeContainer> nodeMap) {
    List<Coord> list = new ArrayList<Coord>();
    ...
    for (WayNode wayNode : way.getWayNodes()) {
      ...
      if (nc == null) {
        LOG.error("Some way coords are missing.");
  ...
  private static List<Coord> getRelationBox(
      Relation relation,
      Map<Long, NodeContainer> nodeMap,
      Map<Long, WayContainer> wayMap,
      Map<Long, RelationContainer> relationMap) {
    ...
    for (RelationMember rm : relation.getMembers()) {
      if (rm.getMemberType() == EntityType.Node) {
        ...
        if (wayMap.containsKey(rm.getMemberId())) {
          ...
          List<Coord> box = getWayBox(way, nodeMap);
          if (box.get(0).getX() < xmin) {
            xmin = box.get(0).getX();
          }
          if (box.get(0).getY() < ymin) {
            ymin = box.get(0).getY();
          }
          if (box.get(1).getX() > xmax) {
            xmax = box.get(1).getX();
          }
          if (box.get(1).getY() > ymax) {
            ymax = box.get(1).getY();
          }
        } else {
        ...
        try {
          if (relationMap.containsKey(rm.getMemberId())) {
            ...
            LOG.warn(
                "Relation "
                    + rm.getMemberId()
                    + " was not found in relationMap, and will be ignored.");
          }
