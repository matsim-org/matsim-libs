...
/**
 * Merges two networks into one, by copying all nodes and links from the two given networks to a
 * third one.
 *
 * @author mrieser
 */
public class MergeNetworks {
  // TODO copied here like several others did before. potentially find a central place where this
  // can go

  /**
   * Merges two networks into one, by copying all nodes and links from the addNetworks to the
   * baseNetwork.
   *
   * @param baseNetwork
   * @param addNetwork
  ...
  public static void merge(
      final Network baseNetwork, final String addPrefix, final Network addNetwork) {
    ...
    for (Node node : addNetwork.getNodes().values()) {
      Node node2 =
          (Node)
              factory.createNode(
                  Id.create(addPrefix + node.getId().toString(), Node.class), node.getCoord());
      baseNetwork.addNode(node2);
    }
    for (Link link : addNetwork.getLinks().values()) {
      Id<Node> fromNodeId =
          Id.create(addPrefix + link.getFromNode().getId().toString(), Node.class);
      ...
      Link link2 =
          factory.createLink(
              Id.create(addPrefix + link.getId().toString(), Link.class), fromNode, toNode);
   ...
   */
  public static void merge(
      final Network networkA,
      final String prefixA,
      final Network networkB,
    ...
    for (Node node : networkA.getNodes().values()) {
      Node node2 =
          (Node)
              factory.createNode(
                  Id.create(prefixA + node.getId().toString(), Node.class), node.getCoord());
    ...
    for (Link link : networkA.getLinks().values()) {
      ...
      Link link2 =
          factory.createLink(
              Id.create(prefixA + link.getId().toString(), Link.class), fromNode, toNode);
      link2.setAllowedModes(link.getAllowedModes());
      link2.setCapacity(link.getCapacity() * capacityFactor);
      link2.setFreespeed(link.getFreespeed());
      link2.setLength(link.getLength());
    ...
    for (Node node : networkB.getNodes().values()) {
      Node node2 =
          (Node)
              factory.createNode(
                  Id.create(prefixB + node.getId().toString(), Node.class), node.getCoord());
      mergedNetwork.addNode(node2);
    }
    for (Link link : networkB.getLinks().values()) {
      ...
      Link link2 =
          factory.createLink(
              Id.create(prefixB + link.getId().toString(), Link.class), fromNode, toNode);
      link2.setAllowedModes(link.getAllowedModes());
      link2.setCapacity(link.getCapacity() * capacityFactor);
      link2.setFreespeed(link.getFreespeed());
      link2.setLength(link.getLength());
