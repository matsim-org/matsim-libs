 ...
 */
final class LeastCostPathCalculatorAccessibilityContributionCalculator
  ...
  public LeastCostPathCalculatorAccessibilityContributionCalculator(
  ...
  public void initialize(ActivityFacilities measuringPoints, ActivityFacilities opportunities) {
    this.aggregatedMeasurePoints =
        AccessibilityUtils.aggregateMeasurePointsWithSameNearestNode(
            measuringPoints, scenario.getNetwork());
    this.aggregatedOpportunities =
        AccessibilityUtils.aggregateOpportunitiesWithSameNearestNode(
            opportunities, scenario.getNetwork(), scenario.getConfig());
  }
    ...
    for (final AggregationObject destination : aggregatedOpportunities.values()) {
      LeastCostPathCalculator.Path path =
          leastCostPathCalculator.calcLeastCostPath(
              fromNode, (Node) destination.getNearestBasicLocation(), departureTime, null, null);
      expSum +=
          destination.getSum() * Math.exp(scoringConfigGroup.getBrainExpBeta() * path.travelCost);
    }
    return expSum;
  }
  ...
  public LeastCostPathCalculatorAccessibilityContributionCalculator duplicate() {
    LeastCostPathCalculatorAccessibilityContributionCalculator
        leastCostPathCalculatorAccessibilityContributionCalculator =
            new LeastCostPathCalculatorAccessibilityContributionCalculator(
                this.scoringConfigGroup, this.leastCostPathCalculator, this.scenario);
