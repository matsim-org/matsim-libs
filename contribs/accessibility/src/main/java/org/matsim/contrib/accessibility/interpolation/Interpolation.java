...
/**
 * Wrapper class for interpolation. Interpolates the value at one point from grid data as
 * SpatialGrid. Offers bilinear interpolation, bicubic spline interpolation and inverse distance
 * weighting.
 *
 * <p>Bilinear interpolation: Uses linear spline interpolation with separation (first horizontal
 * then vertical). Own implementation.
 *
 * <p>Bicubic spline interpolation: Uses BicubicSplineInterpolator from apache
 ...
 *
 * <p>Inverse distance weighting: Uses inverse distance weighting for the 4 nearest known values.
 * Own implementation. Peaks and valleys occur. For more information see e.g.:
...
public final class Interpolation {
   ...
   *
   * <p>If inverse distance weighting is chosen, the exponent for weights will be the default value
   * 1.
   *
   ...
   * @param sg the SpatialGrid to interpolate
   * @param method the interpolation method
   * @param exp the exponent for weights. only considered if interpolation method is inverse
   *     distance weighting. standard values are one or two.
   */
  ...
  public double interpolate(double x, double y) {
    ...
    switch (this.interpolationMethod) {
      case 0:
        return this.biLinearInterpolator.biLinearInterpolation(x, y);
      case 1:
        return this.biCubicInterpolator.biCubicInterpolation(x, y);
      case 2:
        return this.inverseDistanceWeighting.inverseDistanceWeighting(x, y, this.exp);
    }
    return Double.NaN;
  }
