...
public final class AccessibilityFromEvents {
  ...
  private AccessibilityFromEvents(
      Scenario scenario, String eventsFile, List<FacilityDataExchangeInterface> dataListeners) {
    this.scenario = scenario;
    this.eventsFile = eventsFile;
  ...
  public void run() {
    ...
    for (String mode : scenario.getConfig().routing().getNetworkModes()) {
      TravelTimeCalculator.Builder builder =
          new TravelTimeCalculator.Builder(scenario.getNetwork());
    ...
    // The following is more awkward than it should be because of Java type erasure; essentially, it
    // says ...getInstance( Set<ControlerListener>.class ):
    Set<ControlerListener> result =
        injector.getInstance(Key.get(new TypeLiteral<Set<ControlerListener>>() {}));

    // We do not have named ControlerListeners, so we have to go through all of them (if the
    // AccessibilityListener had a class name, then one might
