...
/**
 * Interpolates data on a SpatialGrid with bicubic spline interpolation from apache
 * (http://commons.apache.org).
 *
 * <p>Requires values on a SpatialGrid.
 *
 * <p>Problem: Wave effects may occur.
 *
 * @author tthunig
 */
class BiCubicInterpolator {
  ...
  /**
   * Prepares bicubic spline interpolation: Generates an interpolation function with
   * PiecewiseBicubicSplineInterpolator from apache
   *
    ...
    for (double y = this.sg.getYmin(); y <= this.sg.getYmax(); y += this.sg.getResolution()) {
      for (double x = this.sg.getXmin(); x <= this.sg.getXmax(); x += this.sg.getResolution()) {
        if (Double.isNaN(this.sg.getValue(x, y))) {
          log.error(
              "Bicubic spline interpolation doesn't work with NaN entries. "
