...
public final class ConstantSpeedAccessibilityExpContributionCalculator implements AccessibilityContributionCalculator {
	...
	private double modeBetaTT;	// in MATSim this is [utils/h]: cnScoringGroup.getTravelingBike_utils_hr() - cnScoringGroup.getPerforming_utils_hr()
	private double modeBetaTD;	// in MATSim this is 0 !!! since getMonetaryDistanceCostRateBike doesn't exist:
	private double modeConstant;
	private double modeSpeed_m_h = -1;
	
	private final double walkBetaTT;
	private final double walkBetaTD;
	...
	public ConstantSpeedAccessibilityExpContributionCalculator(final String mode, Config config, Network network) {
		...
		modeBetaTT = modeParams.getMarginalUtilityOfTraveling() - planCalcScoreConfigGroup.getPerforming_utils_hr();
		modeBetaTD = modeParams.getMarginalUtilityOfDistance();
		modeConstant = modeParams.getConstant();

		walkBetaTT = planCalcScoreConfigGroup.getModes().get(TransportMode.walk).getMarginalUtilityOfTraveling() - planCalcScoreConfigGroup.getPerforming_utils_hr();
		walkBetaTD = planCalcScoreConfigGroup.getModes().get(TransportMode.walk).getMarginalUtilityOfDistance();
	...
	public double computeContributionOfOpportunity(ActivityFacility origin, AggregationObject destination, Double departureTime) {
		...
		double utilityMeasuringPoint2Road = (distancePoint2Intersection_m / this.walkSpeed_m_h * walkBetaTT)	+ (distancePoint2Intersection_m * walkBetaTD);
		
		// Utility on the network to first node
		double distanceIntersection2Node_m = distances.getDistanceIntersection2Node();
		double utilityRoad2Node = (distanceIntersection2Node_m / modeSpeed_m_h * modeBetaTT)	+ (distanceIntersection2Node_m * modeBetaTD); // toll or money ???
		...
		double utility = ((travelDistance_m / modeSpeed_m_h * modeBetaTT) + (travelDistance_m * modeBetaTD)); // toll or money ???
		...
		return Math.exp(logitScaleParameter * (modeConstant + utilityMeasuringPoint2Road + utilityRoad2Node + utility)) * sumExpVjkWalk;
	}
}