...
/**
 * Implements inverse distance weighting for interpolation. Own implementation (no suitable
 * implementation found).
 *
 * <p>Requires values on a SpatialGrid.
 *
 * <p>Problem: Peaks and valleys occur.
 *
 * <p>For more information see e.g.:
...
class InverseDistanceWeighting {
  ...
  /**
   * Interpolates the value on a arbitrary point with inverse distance weighting. Considers only
   * four neighboring values because this method needs less time for calculation than considering
   * all known values and the result is even more suitable for accessibility interpolation.
   *
  ...
  /**
   * Interpolates a value at the given point (xCoord, yCoord) with the inverse distance weighting
   * with variable power of weights. Considers only four neighboring values.
   *
  ...
  static double fourNeighborsIDW(SpatialGrid sg, double xCoord, double yCoord, double exp) {
    ...
    return (sg.getValue(x1, y1) / d11
            + sg.getValue(x1, y2) / d12
            + sg.getValue(x2, y1) / d21
            + sg.getValue(x2, y2) / d22)
   ...
   *
   * <p>Interpolates a value at the given point (xCoord, yCoord) with the inverse distance weighting
   * with variable power of weights: z(u_0)= Sum((1/d_i^exp)*z(u_i)) / Sum (1/d_i^exp). Needs more
   * time for calculation than fourNeighborsIDW and the result is even less suitable for
   * accessibility interpolation.
   *
   * @param sg the SpatialGrid with the known values
   * @param xCoord
   * @param yCoord
