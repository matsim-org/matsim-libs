...
public final class AccessibilityModule extends AbstractModule {
    ...
    addControlerListenerBinding()
        .toProvider(
            new Provider<ControlerListener>() {
              ...
              //			@Inject (optional = true) PtMatrix ptMatrix = null; // Downstream code knows how
              // to handle a null PtMatrix
              // not consistent with guice-grapher, but also a result of garbled design: There
              // should rather be a routing-mode that uses PtMatrix.
              ...
              public ControlerListener get() {
                AccessibilityConfigGroup acg =
                    ConfigUtils.addOrGetModule(
                        scenario.getConfig(), AccessibilityConfigGroup.class);
                ActivityFacilities opportunities =
                    AccessibilityUtils.collectActivityFacilitiesWithOptionOfType(
                        scenario, activityType);
                final BoundingBox boundingBox;

                int tileSize_m = acg.getTileSize();
                if (tileSize_m <= 0) {
                  LOG.error("Tile Size must be assigned a value greater than zero.");
                }

                if (acg.getAreaOfAccessibilityComputation()
                    == AreaOfAccesssibilityComputation.fromShapeFile) {
                  Geometry boundary =
                      GridUtils.getBoundary(acg.getShapeFileCellBasedAccessibility());
                  ...
                  boundingBox =
                      BoundingBox.createBoundingBox(
                          ...
                          envelope.getMaxY());
                  measuringPoints =
                      GridUtils.createGridLayerByGridSizeByShapeFileV2(boundary, tileSize_m);
                  LOG.info("Using shape file to determine the area for accessibility computation.");

                } else if (acg.getAreaOfAccessibilityComputation()
                    == AreaOfAccesssibilityComputation.fromBoundingBox) {
                  boundingBox =
                      BoundingBox.createBoundingBox(
                          acg.getBoundingBoxLeft(),
                          acg.getBoundingBoxBottom(),
                          acg.getBoundingBoxRight(),
                          acg.getBoundingBoxTop());
                  measuringPoints =
                      GridUtils.createGridLayerByGridSizeByBoundingBoxV2(boundingBox, tileSize_m);
                  LOG.info(
                      "Using custom bounding box to determine the area for accessibility computation, which is resolved in squares.");

                } else if (acg.getAreaOfAccessibilityComputation()
                    == AreaOfAccesssibilityComputation.fromBoundingBoxHexagons) {
                  boundingBox =
                      BoundingBox.createBoundingBox(
                          acg.getBoundingBoxLeft(),
                          acg.getBoundingBoxBottom(),
                  ...
                  LOG.info(
                      "Using custom bounding box to determine the area for accessibility computation, whichs is resolved in hexagons.");

                } else if (acg.getAreaOfAccessibilityComputation()
                    == AreaOfAccesssibilityComputation.fromFacilitiesFile) {
                  boundingBox =
                      BoundingBox.createBoundingBox(
                          acg.getBoundingBoxLeft(),
                          acg.getBoundingBoxBottom(),
                  ...
                  measuringPoints =
                      ConfigUtils.addOrGetModule(config, AccessibilityConfigGroup.class)
                          .getMeasuringPointsFacilities();
                  LOG.warn("Number of measuringPoints = " + measuringPoints.getFacilities().size());
                  if (measuringPoints == null) {
                    throw new RuntimeException(
                        "Measuring points should have been set direclty if from-facilities-object mode is used.");
                  }
                  LOG.info("Using measuring points from facilities object.");

                } else { // This covers also the "fromNetwork" case
                  LOG.info(
                      "Using the boundary of the network file to determine the area for accessibility computation.");
                  LOG.warn(
                      "This can lead to memory issues when the network is large and/or the cell size is too fine!");
                ...
                if (acg.getMeasurePointGeometryProvision()
                    == MeasurePointGeometryProvision.fromShapeFile) {
                  measurePointGeometryMap = acg.getMeasurePointGeometryMap();
                } else {
                  measurePointGeometryMap =
                      VoronoiGeometryUtils.buildMeasurePointGeometryMap(
                          measuringPoints, boundingBox, tileSize_m);
                }
                AccessibilityUtils.assignAdditionalFacilitiesDataToMeasurePoint(
                    measuringPoints, measurePointGeometryMap, additionalFacs);

                String outputDirectory =
                    scenario.getConfig().controller().getOutputDirectory() + "/" + activityType;
                AccessibilityComputationShutdownListener accessibilityShutdownListener =
                    new AccessibilityComputationShutdownListener(
                        scenario, measuringPoints, opportunities, outputDirectory);
                ...
                for (String mode : acg.getModes()) {
                  ...
                  if (Modes4Accessibility.freespeed.name().equals(mode)) {
                    ...
                    calculator =
                        new NetworkModeAccessibilityExpContributionCalculator(
                            mode, new FreeSpeedTravelTime(), travelDisutilityFactory, scenario);
                    ...
                    final TravelDisutilityFactory nwModeTravelDisutility =
                        travelDisutilityFactories.get(mode);
                    Gbl.assertNotNull(nwModeTravelDisutility);
                    calculator =
                        new NetworkModeAccessibilityExpContributionCalculator(
                            mode, nwModeTravelTime, nwModeTravelDisutility, scenario);
                  } else if (TransportMode.pt.equals(mode)) {
                    calculator =
                        new SwissRailRaptorAccessibilityContributionCalculator(
                            mode, config.scoring(), scenario);
                  } else if (Modes4Accessibility.matrixBasedPt.name().equals(mode)) {
                    throw new RuntimeException(
                        "currently not supported because implementation not consistent with guice grapher.  kai, sep'19");
                    //						calculator = new
                    // LeastCostPathCalculatorAccessibilityContributionCalculator(
                    //								config.planCalcScore(),
                    //	ptMatrix.asPathCalculator(config.planCalcScore()), scenario);
                  } else if (TransportMode.walk.equals(mode) || TransportMode.bike.equals(mode)) {
                    // special case(s), since often in the simulation this is not treated as network
                    // route
                    calculator =
                        new ConstantSpeedAccessibilityExpContributionCalculator(mode, scenario);
                  } else {
                    ...
                    if (travelDisutilityFactory == null) {
                      throw new RuntimeException(
                          "mode=" + mode + "; travelDisutilityFactory is null!");
                    }
                    calculator =
                  ...
                  if (calculator != null) {
                    accessibilityShutdownListener.putAccessibilityContributionCalculator(
                        mode, calculator);
                  } else {
                    LOG.warn(
                        "accessibility contribution calculator for mode="
                            + mode
                            + " was null.  Will not compute accessibility for "
                            + "that mode.");
                  }
                }
                ...
                if (pushing2Geoserver || createQGisOutput) {
                  if (measurePointGeometryMap == null) {
                    throw new IllegalArgumentException(
                        "measure-point-to-geometry map must not be null if push to Geoserver is intended.");
                  }
                  Set<String> additionalFacInfo = additionalFacs.keySet();
                  accessibilityShutdownListener.addFacilityDataExchangeListener(
                      new GeoserverUpdater(
                          acg.getOutputCrs(),
                          config.controller().getRunId() + "_" + activityType,
                          measurePointGeometryMap,
                          additionalFacInfo,
                          outputDirectory,
                          pushing2Geoserver,
  ...
  /**
   * Add additional facility data that will generate an additional column for each (x,y,t)-Entry.
   * The facilities are aggregated to the measurement points in downstream code.
   */
  public void addAdditionalFacilityData(
      ActivityFacilities facilities) { // TDO cleanu up this method
    if (facilities.getName() == null || facilities.getName().equals("")) {
      throw new RuntimeException(
          "Cannot add unnamed facility containers here. A key is required to identify them.");
    }
    for (ActivityFacilities existingFacilities : this.additionalFacs.values()) {
      if (existingFacilities.getName().equals(facilities.getName())) {
        throw new RuntimeException(
            "Additional facilities under the name of + "
                + facilities.getName()
