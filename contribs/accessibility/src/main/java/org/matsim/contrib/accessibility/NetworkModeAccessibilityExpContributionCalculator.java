 ...
 */
final class NetworkModeAccessibilityExpContributionCalculator
    implements AccessibilityContributionCalculator {
  private static final Logger LOG =
      LogManager.getLogger(NetworkModeAccessibilityExpContributionCalculator.class);
  ...
  public NetworkModeAccessibilityExpContributionCalculator(
    ...
    // FastMultiNodeDijkstraFactory fastMultiNodeDijkstraFactory = new
    // FastMultiNodeDijkstraFactory(true);
    // this.multiNodePathCalculator = (MultiNodePathCalculator)
    // fastMultiNodeDijkstraFactory.createPathCalculator(network, travelDisutility, travelTime);
  ...
  public void notifyNewOriginNode(Id<? extends BasicLocation> fromNodeId, Double departureTime) {
    ...
    // multiNodePathCalculator.calcLeastCostPath(fromNode, aggregatedToNodes, departureTime, null,
    // null);
  }
  ...
  public double computeContributionOfOpportunity(
    ...
    // NEW AV MODE
    //		double waitingTime_h = (Double) origin.getAttributes().getAttribute("waitingTime_s") /
    // 3600.;
    //		double walkUtilityMeasuringPoint2Road = ((walkTravelTimeMeasuringPoint2Road_h +
    // waitingTime_h) * betaWalkTT)
    ...
    for (final AggregationObject destination : aggregatedOpportunities.values()) {
      ...
      double congestedCarUtility =
          -lcpt.getTree().get(((Node) destination.getNearestBasicLocation()).getId()).getCost();
      // double congestedCarUtility = -
      // dijkstraTree.getLeastCostPath(destination.getNearestNode()).travelCost;
      // double congestedCarUtility = - multiNodePathCalculator.constructPath(fromNode,
      // destination.getNearestNode(), departureTime).travelCost;
  ...
  public NetworkModeAccessibilityExpContributionCalculator duplicate() {
    ...
    NetworkModeAccessibilityExpContributionCalculator
        networkModeAccessibilityExpContributionCalculator =
            new NetworkModeAccessibilityExpContributionCalculator(
                this.mode, this.travelTime, this.travelDisutilityFactory, this.scenario);
    networkModeAccessibilityExpContributionCalculator.subNetwork = this.subNetwork;
    networkModeAccessibilityExpContributionCalculator.aggregatedMeasurePoints =
        this.aggregatedMeasurePoints;
    networkModeAccessibilityExpContributionCalculator.aggregatedOpportunities =
        this.aggregatedOpportunities;
