...
class LandUseBuildingSink implements Sink {
  ...
  private void processBuildings(
    ...
    for (long entityKey : entityMap.keySet()) {
      ...
      // Entities that possess an "amenity" tag need to be excluded here, because they are already
      // considered via the AmenityReader
      ...
      if (buildingType != null && ignoreBecauseOfTag == false) {
        ...
        if (activityType != null) {
          ...
          ActivityOption aactivityOption =
              activityFacilityFactory.createActivityOption(activityType);
  ...
  private String getActivityTypeFromLandUseArea(
      GeometryFactory geometryFactory, String buildingType, Coord coord) {
