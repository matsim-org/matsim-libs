...
class VoronoiGeometryUtils {
  private static final Logger LOG = LogManager.getLogger(VoronoiGeometryUtils.class);

  private static GeometryFactory geometryFactory = new GeometryFactory();

  public static Map<Id<ActivityFacility>, Geometry> buildMeasurePointGeometryMap(
      ActivityFacilities measuringPoints, BoundingBox boundingBox, int tileSize_m) {
    ...
    for (ActivityFacility measurePoint : measuringPoints.getFacilities().values()) {
      ...
      for (Geometry geometry : geometries) {
        if (geometry.covers(
            geometryFactory.createPoint(CoordUtils.createGeotoolsCoordinate(coord)))) {
      ...
      if (!polygonFound) {
        throw new RuntimeException(
            "No polygon found for measure point "
                + measurePointId
                + " with coord = "
                + coord
                + ".");
    ...
    Map<Id<ActivityFacility>, Geometry> revisedMeasurePointPolygons =
        reduceEdgePolygons(measuringPoints, tileSize_m, measurePointPolygons);

    LOG.warn("Finished building measure-point-to-geometry map.");
    return revisedMeasurePointPolygons;
  }

  private static Map<Id<ActivityFacility>, Geometry> reduceEdgePolygons(
      ActivityFacilities measuringPoints,
      int tileSize_m,
      Map<Id<ActivityFacility>, Geometry> measurePointPolygons) {
  ...
  static Collection<Geometry> determineVoronoiShapes(
      ActivityFacilities measuringPoints, BoundingBox box) {
    ...
    for (ActivityFacility measuringPoint : measuringPoints.getFacilities().values()) {
      Coordinate coordinate =
          new Coordinate(measuringPoint.getCoord().getX(), measuringPoint.getCoord().getY());
    ...
    List<Polygon> polygons =
        voronoiDiagramBuilder.getSubdivision().getVoronoiCellPolygons(geometryFactory);
  ...
  static Polygon createBoundingPolygon(BoundingBox box) {
    ...
    Coordinate[] boundingBoxCoordinates =
        new Coordinate[] {
          boundingBoxSW, boundingBoxSE, boundingBoxNE, boundingBoxNW, boundingBoxSW
        };
  ...
  static Collection<Geometry> cutPolygonsByBoundary(
      Collection<Polygon> polygons, Polygon boundingPolygon) {
