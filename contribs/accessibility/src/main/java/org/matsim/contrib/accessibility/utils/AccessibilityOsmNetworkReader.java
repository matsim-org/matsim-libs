...
public class AccessibilityOsmNetworkReader {
  private static final Logger LOG = LogManager.getLogger(AccessibilityOsmNetworkReader.class);
  ...
  private boolean onlyMotorwayToTertiary =
      false; // "Thinner" network down to tertiary; do not use this together with
  // "includeLowHierarchyWays"
  private boolean onlyMotorwayToSecondary =
      false; // "Thinner" network down to tertiary; do not use this together with
  // "includeLowHierarchyWays"
  ...
  public static void main(String[] args) {
    ...
    AccessibilityOsmNetworkReader osmNetworkCreatorDZ =
        new AccessibilityOsmNetworkReader(osmFileName, outputCRS);
  ...
  public void createNetwork() {
    ...
    // Setting this to true means that pure geometry-describing nodes (i.e. nodes which are not
    // intersections etc.) are kept.
    ...
    if (includeLowHierarchyWays == true) {
      ...
      // By defaults set for motorway, motorway_link, trunk, trunk_link, primary, primary_link,
      // secondary, secondary_link,
      // tertiary, tertiary_link, minor, unclassified, residential, living_street; minor does not
      // exist on the website anymore
    ...
    // For a coarser network ("onlyMotorwayToTertiaryMotorways" leads to a network only a 14th as
    // big (77.8MB vs. 1.04GB) for Maryland)
    if (onlyMotorwayToSecondary == true || onlyMotorwayToTertiary == true) {
      LOG.info("Only bigger roads are included.");
      if (includeLowHierarchyWays == true) {
        throw new RuntimeException(
            "It does not make sense to set both \"includeLowHierarchyWays\""
    ...
    if (osmFileName != null && osmInputStream == null) {
      ...
      throw new IllegalArgumentException(
          "Either the input stream OR the input file should be defined, but not both.");
    }
  ...
  /**
   * The input CRS for data from OSM should in almost all cases be EPSG:4326 = WGS84, which is used
   * here as default.
   */
