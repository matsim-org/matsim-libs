...
class TripRouterAccessibilityContributionCalculator implements AccessibilityContributionCalculator {
  private static final Logger LOG =
      LogManager.getLogger(TripRouterAccessibilityContributionCalculator.class);
  ...
  public TripRouterAccessibilityContributionCalculator(
      String mode,
    ...
    LOG.warn(
        "This is currently heavliy oriented on the need of car-based computatations. Revise beofre using for other modes!");
    this.mode = mode;
    ...
    this.walkSpeed_m_s =
        scenario.getConfig().routing().getTeleportedModeSpeeds().get(TransportMode.walk);
  ...
  public void initialize(ActivityFacilities measuringPoints, ActivityFacilities opportunities) {
    ...
    subNetwork =
        AccessibilityUtils.createModeSpecificSubNetwork(
            scenario.getNetwork(), mode, networkConfigGroup);

    this.aggregatedMeasurePoints =
        AccessibilityUtils.aggregateMeasurePointsWithSameNearestNode(measuringPoints, subNetwork);
    this.aggregatedOpportunities =
        AccessibilityUtils.aggregateOpportunitiesWithSameNearestNode(
            opportunities, subNetwork, scenario.getConfig());
  }
  ...
  public double computeContributionOfOpportunity(
    ...
    for (final AggregationObject destination : aggregatedOpportunities.values()) {
      ...
      ((ActivityFacilityImpl) origin)
          .setLinkId(
              nearestLink
                  .getId()); // Set nearest link to origin so that router really starts fomr here

      // Orthogonal walk to nearest link
      Distances distance =
          NetworkUtil.getDistances2NodeViaGivenLink(origin.getCoord(), nearestLink, fromNode);
      double walkTravelTimeMeasuringPoint2Road_h =
          distance.getDistancePoint2Intersection() / (this.walkSpeed_m_s * 3600);
      ...
      double congestedCarUtilityRoad2Node =
          -travelDisutility.getLinkTravelDisutility(nearestLink, departureTime, null, null)
              * distanceFraction;
      ...
      ActivityFacility destinationFacility =
          activityFacilitiesFactory.createActivityFacility(
              null, destination.getNearestBasicLocation().getCoord());

      Gbl.assertNotNull(tripRouter);
      List<? extends PlanElement> plan =
          tripRouter.calcRoute(
              mode, origin, destinationFacility, departureTime, null, new AttributesImpl());
      ...
      for (Leg leg : legs) {
        ...
        double estimatedEndLinkTT =
            endLinkLength
                / endLink.getFreespeed(); // This is an assumption as it is only the freespeed

        // Note: The following computation where the end link length is added is only correct once
        // the end link is removed from the route
        // in the NetworkRoutingModule (route.setDistance(RouteUtils.calcDistance(route, 1.0, 0.0,
        // this.network));)
        if (this.scoringConfigGroup.getModes().get(leg.getMode()).getMarginalUtilityOfDistance()
            != 0.) {
          LOG.warn(
              "A computation including a marginal utility of distance will only be correct if the route time/distance"
                  + "inconsistency in the NetworkRoutingModule is solved.");
        }
        utility +=
            (leg.getRoute().getDistance() + endLinkLength)
                * this.scoringConfigGroup
                    .getModes()
                    .get(leg.getMode())
                    .getMarginalUtilityOfDistance();
        utility +=
            (leg.getRoute().getTravelTime().seconds() + estimatedEndLinkTT)
                * this.scoringConfigGroup
                    .getModes()
                    .get(leg.getMode())
        ...
        utility +=
            -(leg.getRoute().getTravelTime().seconds() + estimatedEndLinkTT)
                * this.scoringConfigGroup.getPerforming_utils_hr()
                / 3600.;
      }
      ...
      expSum +=
          Math.exp(
                  this.scoringConfigGroup.getBrainExpBeta()
                      * (utility
                          + modeSpecificConstant
                          + walkUtilityMeasuringPoint2Road
                          + congestedCarUtilityRoad2Node))
              * sumExpVjkWalk;
    }
    return expSum;
  }
  ...
  public TripRouterAccessibilityContributionCalculator duplicate() {
    TripRouterAccessibilityContributionCalculator tripRouterAccessibilityContributionCalculator =
        new TripRouterAccessibilityContributionCalculator(
            this.mode,
            this.tripRouter,
            this.scoringConfigGroup,
            this.scenario,
    ...
    tripRouterAccessibilityContributionCalculator.aggregatedMeasurePoints =
        this.aggregatedMeasurePoints;
    tripRouterAccessibilityContributionCalculator.aggregatedOpportunities =
        this.aggregatedOpportunities;
  ...
  private Link getNearestLinkInCorrectDirection(
      ActivityFacility origin, Network network, Node nodeInWhichLinkHasToEnd) {
    Link nearestLink = NetworkUtils.getNearestLinkExactly(network, origin.getCoord());
    Link reverseLink =
        NetworkUtils.getConnectingLink(nearestLink.getToNode(), nearestLink.getFromNode());
    if (nearestLink.getToNode().getId() != nodeInWhichLinkHasToEnd.getId()) {
      LOG.info(
          "Link does not have nearest node "
              + nodeInWhichLinkHasToEnd.getId()
              + " of origin as toNode. ToNode is "
              + nearestLink.getToNode().getId());
      if (reverseLink.getToNode().getId() == nodeInWhichLinkHasToEnd.getId()) {
        LOG.info(
            "Use reverse direction of link "
                + nearestLink.getId()
                + ", i.e. use link "
                + reverseLink.getId());
