...
/**
 * The spatial grid saves the data values of a study area in a matrix. Intern the matrix is mirrored
 * horizontal. The methods getValue and setValue compensate the mirroring process (so altogether the
 * class works consistent). In contrast to the methods above the method getMirroredValue(int row,
 * int col) returns the intern mirrored value.
 *
 * @author illenberger
 * @author thomas
 * @author tthunig
...
public final class SpatialGrid {
  // yy this class was set to deprecated by DZ.  Quite possibly, we decided this together.  Still, I
  // can't remember the reason.  Could you please
   ...
   */
  public SpatialGrid(
      double xmin, double ymin, double xmax, double ymax, double resolution, double initialValue) {
    minX = xmin;
    minY = ymin;
    maxX = xmax;
    maxY = ymax;
    ...
    for (int x = 0; x < numXBins; x++)
      for (int y = 0; y < numYBins; y++) matrix[y][x] = initialValue;
  }
  ...
  /**
   * independent of the intern representation (mirrored) this method returns the initial value at
   * the given point
   *
   * @param point
  ...
  public double getValue(Point point) {
    if (isInBounds(point)) return getValue(point.getX(), point.getY());

    // log.warn("This point lies outside the boundary!!!");
    // log.warn("Boundary: xmin:"+this.minX+", ymin:"+ this.minY+", xmax:"+this.maxX+",
    // maxy:"+this.maxY);
  ...
  public double getValue(double x, double y) {
    if (isInBounds(x, y)) return matrix[getRow(y)][getColumn(x)];
  ...
  /**
   * independent of the intern representation (mirrored) this method sets the given value at the
   * initial point if it lies in the bounds of the study area
   *
   * @param value
   * @param point
  ...
  /**
   * independent of the intern representation (mirrored) this method returns the initial data as a
   * matrix (remirrored) for example the value at (xmin, ymin) will be at the bottom left corner of
   * the returned matrix hence it fits with the coordinate system if you plot the matrix
   *
  ...
  /**
   * determines the boundary and resolution of the study area than this initializes and returns a
   * Spatial Grid with these settings
   *
   * @param filename
   * @return SpatialGrid
   * @throws IOException
  ...
  private static SpatialGrid initSpatialGrid(String filename) throws IOException {
    ...
    if (header.length > 1) {
      ...
      while ((line = reader.readLine()) != null) {
        ytmp = Double.parseDouble(line.split(SpatialGridTableWriter.separator)[0]);
        if (firstLine) {
          ymin = ytmp;
          firstLine = false;
        }
      ...
      System.out.println(xmin + "," + ymin + "," + xmax + "," + ymax + "," + res);
  ...
  public static void main(String args[]) {
    ...
    SpatialGrid test =
        SpatialGrid.readFromFile(
            "/Users/thomas/Development/opus_home/data/seattle_parcel/results/interpolationQuickTest/results/400.0travel_time_accessibility.txt"); // SpatialGrid.readFromFile("/Users/thomas/Development/opus_home/matsim4opus/tmp/carAccessibility_cellsize_100.0SF.txt");
    test.writeToFile(
        "/Users/thomas/Development/opus_home/data/seattle_parcel/results/interpolationQuickTest/results/400.0travel_time_accessibilityTEST.txt");
  }
}
