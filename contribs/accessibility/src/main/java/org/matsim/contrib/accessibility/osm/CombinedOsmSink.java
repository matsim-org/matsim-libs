...
class CombinedOsmSink implements Sink {
  ...
  public CombinedOsmSink(
      ...
      List<String> unmannedEntitiesList,
  ...
  private void processLandUseAreas(
      ActivityFacilitiesFactory aff, Map<Long, ? extends EntityContainer> entityMap) {

    CoordinateTransformation ct =
        TransformationFactory.getCoordinateTransformation("WGS84", this.outputCRS);
  ...
  // creates a feature in form of a polygon that is defined by its vertices, which are defined by
  // its coordinates
  ...
  private void processEntity(
    ...
    for (long entityKey : entityMap.keySet()) {
      ...
      Coord centroidCoord =
          OSMCoordUtils.getCentroidCoord(entity, ct, this.nodeMap, this.wayMap, this.relationMap);
      ...
      // entities with shop tag are converted into a facility with shopping and work activity
      // options
      if (shopType != null) {
      ...
      // activity option is added, since most entities also have a leisure tag -- especially the
      // manned facilities
      if (sportType != null) {
      ...
      if (buildingType != null && !(entity instanceof Relation) && !(entity instanceof Node)) {
        ...
        // do this step first to be able to "continue" in loop if feature for building cannot be
        // created
        ...
        if (activityType == null) {
          activityType =
              getActivityTypeFromLandUseArea(geometryFactory, buildingType, centroidCoord);
        }
  ...
  private void createFacility(
      ActivityFacilitiesFactory activityFacilityFactory,
      Entity entity,
      String name,
      Coord coord,
      String activityType) {
  ...
  private String getActivityTypeFromLandUseArea(
      GeometryFactory geometryFactory, String buildingType, Coord buildingCoord) {
  ...
  private Integer getBuildingLevels(long entityKey, String buildingLevelsAsString) {
    ...
    if (buildingLevelsAsString != null) {
      ...
      if (!Character.isDigit(buildingLevelsAsString.charAt(0))) {
        ...
        buildingLevels =
            (Integer.parseInt(subString[0].trim()) + Integer.parseInt(subString[1].trim())) / 2;
        log.warn(
            "Simplified level number of building " + entityKey + " by averaging different values.");
        ...
        log.warn(
            "Simplified level number of building " + entityKey + " by truncating decimal places.");
  ...
  /**
   * Sets a prefix that will be used for each id. For example, useful when parsing from osm (as one
   * of many sources) to use the prefix "osm_" so that the resulting facilities can be identified
   * accordingly.
   *
   * @param prefix
   */
