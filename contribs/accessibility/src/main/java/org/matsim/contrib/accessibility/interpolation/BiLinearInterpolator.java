...
/**
 * Implements bilinear interpolation. Uses linear spline interpolation with separation: first
 * horizontal then vertical. Own implementation (no suitable implementation found).
 *
 * <p>Requires values on a SpatialGrid.
 *
 * @author tthunig
 */
class BiLinearInterpolator {
  ...
  /**
   * Interpolates the value on a arbitrary point with bilinear interpolation. Requires values on a
   * grid as SpatialGrid.
   *
  ...
  static double biLinearValueInterpolation(SpatialGrid sg, double xCoord, double yCoord) {
    ...
    // case differentiation important for boundary data of shapefiles, because of neighboring NaN
    // values
    if (xDif == 0) {
      if (yDif == 0) {
        // known value
