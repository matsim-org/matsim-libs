...
final class FeatureSHP {
  ...
  /**
   * Calls {@link FeatureSHP#readFeatures(String, String)} with <tt>null</tt> as feature type
   * indicating to read all features.
   *
   ...
   *
   * @param filename the path to the shape file.
   * @param type the feature type to read in, or <tt>null</tt> if all available feature types should
   *     be read. (I have the feeling that this does not work reliable. joh15/01/10)
   * @return a set of features
   * @throws IOException
   */
  ...
  private static void addFeatures(SimpleFeatureSource source, Set<SimpleFeature> features)
      throws IOException {
