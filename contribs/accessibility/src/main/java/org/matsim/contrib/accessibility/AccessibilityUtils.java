...
public class AccessibilityUtils {
  ...
  /**
   * Aggregates disutilities Vjk to get from node j to all k that are attached to j and assign
   * sum(Vjk) is to node j.
   *
   * <p>j---k1 |\ | \ k2 k3
   */
  public static final Map<Id<? extends BasicLocation>, AggregationObject>
      aggregateOpportunitiesWithSameNearestNode(
    ...
    // for now, we decided not to add "capacities" as it is not needed for current projects. dz,
    // feb'16

    double walkSpeed_m_h =
        config.routing().getTeleportedModeSpeeds().get(TransportMode.walk) * 3600.;
    AccessibilityConfigGroup acg =
        ConfigUtils.addOrGetModule(
            config, AccessibilityConfigGroup.GROUP_NAME, AccessibilityConfigGroup.class);

    LOG.info(
        "Aggregating "
            + opportunities.getFacilities().size()
            + " opportunities with same nearest node...");
    Map<Id<? extends BasicLocation>, AggregationObject> opportunityClusterMap =
    ...
    for (ActivityFacility opportunity : opportunities.getFacilities().values()) {
      ...
      double distance_m =
          NetworkUtils.getEuclideanDistance(opportunity.getCoord(), nearestNode.getCoord());

      // in MATSim this is [utils/h]: cnScoringGroup.getTravelingWalk_utils_hr() -
      // cnScoringGroup.getPerforming_utils_hr()
      double walkBetaTT_utils_h =
          config.scoring().getModes().get(TransportMode.walk).getMarginalUtilityOfTraveling()
    ...
    LOG.info(
        "Aggregated "
            + opportunities.getFacilities().size()
            + " opportunities to "
            + opportunityClusterMap.size()
            + " nodes.");
    return opportunityClusterMap;
  }

  public static Map<Id<? extends BasicLocation>, ArrayList<ActivityFacility>>
      aggregateMeasurePointsWithSameNearestNode(
          ActivityFacilities measuringPoints, Network network) {
    ...
    for (ActivityFacility measuringPoint : measuringPoints.getFacilities().values()) {
      ...
      Node nearestNode =
          NetworkUtils.getCloserNodeOnLink(
              measuringPoint.getCoord(),
              NetworkUtils.getNearestLinkExactly(network, measuringPoint.getCoord()));
  ...
  public static Network createModeSpecificSubNetwork(
      Network network, String mode, NetworkConfigGroup networkConfigGroup) {
  ...
  public static double getModeSpecificConstantForAccessibilities(
      String mode, ScoringConfigGroup scoringConfigGroup) {
  ...
  public static ActivityFacilities collectActivityFacilitiesWithOptionOfType(
      Scenario scenario, String activityOptionType) {
    ActivityFacilities activityFacilities =
        FacilitiesUtils.createActivityFacilities(activityOptionType);
    for (ActivityFacility facility : scenario.getActivityFacilities().getFacilities().values()) {
      if (activityOptionType
          == null) { // no activity option type for facility given, use all of them
  ...
  public static List<String> collectAllFacilityOptionTypes(Scenario scenario) {
    ...
    LOG.warn(
        "The following activity option types where found within the activity facilities: "
            + activityOptionTypes);
    return activityOptionTypes;
  }

  public static void combineDifferentActivityOptionTypes(
      final Scenario scenario,
      String combinedType,
      final List<String> activityOptionsToBeIncluded) {
    ...
    for (ActivityFacility facility : scenario.getActivityFacilities().getFacilities().values()) {
      for (ActivityOption option : facility.getActivityOptions().values()) {
        if (activityOptionsToBeIncluded.contains(option.getType())) {
          ...
          // !option.getType().equals(FacilityTypes.WORK) && !option.getType().equals("minor")) {
  ...
  public static final ActivityFacilities createFacilityForEachLink(
      String facilityContainerName, Network network) {
  ...
  public static final ActivityFacilities createFacilityFromBuildingShapefile(
      String shapeFileName, String identifierCaption, String numberOfHouseholdsCaption) {
    ...
    for (SimpleFeature feature : features) {
      ...
      Integer numberOfHouseholds =
          Integer.parseInt((String) feature.getAttribute(numberOfHouseholdsCaption));
      Geometry geometry = (Geometry) feature.getDefaultGeometry();
      Coord coord =
          CoordUtils.createCoord(geometry.getCentroid().getX(), geometry.getCentroid().getY());

      for (int i = 0; i < numberOfHouseholds; i++) {
        ActivityFacility facility =
            aff.createActivityFacility(
                Id.create(featureId + "_" + i, ActivityFacility.class), coord);
        facilities.addActivityFacility(facility);
      }
    }
    return facilities;
  ...
  public static ActivityFacilities createMeasuringPointsFromNetworkBounds(
      Network network, int cellSize) {
    ...
    ActivityFacilities measuringPoints =
        GridUtils.createGridLayerByGridSizeByBoundingBoxV2(xMin, yMin, xMax, yMax, cellSize);
  ...
  /**
   * Calculates Gini coefficient of the values of a given values. The Gini Coefficient is equals to
   * the half of the relative mean absolute difference (RMD).
   *
   * @see <a href="https://en.wikipedia.org/wiki/Gini_coefficient">
   * @see <a
   *     href="https://en.wikipedia.org/wiki/Mean_absolute_difference#Relative_mean_absolute_difference">
   * @param valueList
  ...
  public static double calculateGiniCoefficient(List<Double> valueList) {
    ...
    double giniCoefficient =
        sumOfAbsoluteDifferences / (2 * Math.pow(numberOfValues, 2) * arithmeticMean);
    return giniCoefficient;
  }

  /**
   * Creates facilities from plans. Note that a new additional facility is created for each
   * activity.
   *
   * @param population
   * @return
      ...
      for (Plan plan : person.getPlans()) {
        ...
        for (PlanElement planElement : plan.getPlanElements()) {
          if (planElement instanceof Activity) {
            ...
            // In case an agent visits the same activity location twice, create another activity
            // facility with a modified ID
            Integer i = 1;
            Id<ActivityFacility> facilityId =
                Id.create(
                    activityType + "_" + personId.toString() + "_" + i.toString(),
            ...
            facilities.addActivityFacility(facility);
            //						log.info("Created activity with option of type " + activityType + " and ID " +
            // facilityId + ".");
          }
        }
      }
  ...
  public static void assignAdditionalFacilitiesDataToMeasurePoint(
      ActivityFacilities measurePoints,
    ...
    for (ActivityFacilities additionalDataFacilities :
        additionalFacilityData.values()) { // Iterate over all additional data collections
      ...
      for (Id<ActivityFacility> measurePointId :
          measurePoints.getFacilities().keySet()) { // Iterate over all measure points
        ...
        for (ActivityFacility facility :
            additionalDataFacilities
                .getFacilities()
                .values()) { // Iterate over additional-data facilities
          Point point =
              geometryFactory.createPoint(
                  new Coordinate(facility.getCoord().getX(), facility.getCoord().getY()));
          if (geometry.contains(point)) {
            measurePoint
                .getAttributes()
                .putAttribute(
                    additionalDataName,
                    (int) measurePoint.getAttributes().getAttribute(additionalDataName) + 1);
      ...
      LOG.warn(
          additionalDataFacilitiesToAssign
              + " additional data facilies have not been assigned to a measure point geometry.");
    }
