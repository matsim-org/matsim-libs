...
public final class GnuplotScriptWriter {
	...
	//				}
					...
					try {
						BufferedWriter writer = IOUtils.getBufferedWriter( config.controller().getOutputDirectory() + "/" + actType
								+ "/" + mode + "/t.gpl" ) ;
					...
					String stdoutFileName = config.controller().getOutputDirectory() + actType + "/" + mode + "/gnuplot.log";
					...
					ExeRunner.run(cmd, stdoutFileName, timeout, config.controller().getOutputDirectory() + actType + "/" + mode);
				}
			}
		}
