...
public final class GridUtils {
  ...
  public static Geometry getBoundary(String shapeFileName) {
    ...
    try {
      ...
      List<Geometry> featureSet =
          ShpGeometryUtils.loadGeometries(IOUtils.resolveFileOrResource(shapeFileName));
      // yyyy I find this quite terrible to have the reader hidden in here.  Now I cannot pass a
      // shape file which
      ...
      if (featureSet.size() > 1) {
        ...
        LOG.warn(
            "This means you have to provide a shape file that only contains the border of the study area without any further features, i.e. zones or fazes, are allowed!");
        LOG.warn(
            "Replace the shape file in your UrbanSim configuration provided at \"travel_model_configuration/matsim4urbansim/controler_parameter/shape_file\"");
        LOG.warn(
            "If the shape file contains features accessibilities will be computes for only feature, i.e. for one zone or faz.");
      }
      return boundary;
   ...
   */
  static ActivityFacilitiesImpl createGridLayerByGridSizeByShapeFileV2(
      Geometry boundary, double gridSize) {
    ...
    ActivityFacilitiesImpl measuringPoints =
        (ActivityFacilitiesImpl)
            FacilitiesUtils.createActivityFacilities("Accessibility measuring points");
    ...
    for (double x = env.getMinX(); x < env.getMaxX(); x += gridSize) {
      ...
      for (double y = env.getMinY(); y < env.getMaxY(); y += gridSize) {
        ...
        if (boundary.contains(centroid)) {
          ...
          measuringPoints.createAndAddFacility(
              Id.create(setPoints, ActivityFacility.class), center);
          setPoints++;
        } else skippedPoints++;
      }
    }
    ...
    LOG.info(
        "Having "
            + setPoints
            + " inside the shape file boundary (and "
            + skippedPoints
            + " outside).");
  ...
  static ActivityFacilitiesImpl createGridLayerByGridSizeByBoundingBoxV2(
      BoundingBox box, int gridSize) {
    return createGridLayerByGridSizeByBoundingBoxV2(
        box.getXMin(), box.getYMin(), box.getXMax(), box.getYMax(), gridSize);
  }
   ...
   * @return ActivityFacilitiesImpl containing the coordinates for the measuring points
   */
  public static ActivityFacilitiesImpl createGridLayerByGridSizeByBoundingBoxV2(
      double minX, double minY, double maxX, double maxY, int gridSize) {
  ...
  static ActivityFacilities createHexagonLayer(BoundingBox box, int maxDiameter_m) {
    return createHexagonLayer(
        box.getXMin(), box.getYMin(), box.getXMax(), box.getYMax(), maxDiameter_m);
  }

  private static ActivityFacilities createHexagonLayer(
      double minX, double minY, double maxX, double maxY, int maxDiameter_m) {
  ...
  public static final void writeSpatialGrids(List<SpatialGrid> spatialGrids, String path) {
    // seems that we cannot re-use the accessibility write method since it plays games with the
    // modes. kai, mar'14
    ...
    for (double y = spatialGrid.getYmin();
        y <= spatialGrid.getYmax();
        y += spatialGrid.getResolution()) {
      for (double x = spatialGrid.getXmin();
          x <= spatialGrid.getXmax();
          x += spatialGrid.getResolution()) {
        writer.writeField(x);
        writer.writeField(y);
  ...
  public static void aggregateFacilitiesIntoSpatialGrid(
      ActivityFacilities facilities,
      SpatialGrid spatialGridSum,
      SpatialGrid spatialGridCnt,
    ...
    for (ActivityFacility fac : facilities.getFacilities().values()) {
      ...
      //				double value = fac.getActivityOptions().get("h").getCapacity() ; // infinity if
      // undefined!!!
