...
/**
 * This runs the normal LeastCostPathTree, with whatever TravelDisutility object it is using (e.g.
 * including distance and/or toll). However, since the normal LeastCostPathTree does not return its
 * contributing cost dimensions, this class here is used to also accumulate distance and toll
 * separately along the route.
 *
 * <p>To re-iterate: This class does <i>not</i> lead to a different tree than the base class.
 *
 * @author thomas
 */
public final class LeastCostPathTreeExtended extends LeastCostPathTree {

  protected static final Logger log = LogManager.getLogger(LeastCostPathTreeExtended.class);
   ...
   */
  public LeastCostPathTreeExtended(
      final TravelTime tt, final TravelDisutility td, final RoadPricingScheme scheme) {
    super(tt, td);
    this.scheme = scheme;
  }
  ...
  public final void calculateExtended(final Network network, final Node origin, final double time) {
    ...
    this.nodeDataExt =
        new ConcurrentHashMap<Id<Node>, NodeDataExtended>(
            (int) (network.getNodes().size() * 1.1), 0.95f);
  ...
  public static void main(String args[]) {
    ...
    ControllerConfigGroup controlerCG =
        (ControllerConfigGroup) config.getModule(ControllerConfigGroup.GROUP_NAME);
    ...
    controler.run();
    // init lcpte
    LeastCostPathTreeExtended lcpte =
        new LeastCostPathTreeExtended(
            controler.getLinkTravelTimes(),
            controler.createTravelDisutilityCalculator(),
            (RoadPricingSchemeImpl)
                controler.getScenario().getScenarioElement(RoadPricingScheme.ELEMENT_NAME));
  ...
  private static Network createTriangularNetwork() {
    ...
    Node node2 =
        NetworkUtils.createAndAddNode(network, Id.create(2, Node.class), new Coord(50, 100));
    Node node3 = NetworkUtils.createAndAddNode(network, Id.create(3, Node.class), new Coord(50, 0));
    Node node4 =
        NetworkUtils.createAndAddNode(network, Id.create(4, Node.class), new Coord(100, 0));
