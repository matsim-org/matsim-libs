...
public final class AccessibilityConfigGroup extends ReflectiveConfigGroup {
  ...
  public static enum AccessibilityMeasureType {
    logSum,
    rawSum,
    gravity
  }
  ...
  //	private static final String ACCESSIBILITY_DESTINATION_SAMPLING_RATE =
  ...
  public static enum MeasurePointGeometryProvision {
    autoCreate,
    fromShapeFile
  }

  private MeasurePointGeometryProvision measurePointGeometryProvision =
      MeasurePointGeometryProvision.autoCreate;
  ...
  public static enum AreaOfAccesssibilityComputation {
    fromNetwork,
    fromBoundingBox,
    fromBoundingBoxHexagons,
    fromShapeFile,
    fromFacilitiesFile,
    fromFacilitiesObject
  }

  private AreaOfAccesssibilityComputation areaOfAccessibilityComputation =
      AreaOfAccesssibilityComputation.fromNetwork;
  ...
  public Map<String, String> getComments() {
    ...
    //		map.put(ACCESSIBILITY_DESTINATION_SAMPLING_RATE, "if only a sample of destinations should be
    // used " +
    //				"(reduces accuracy -- not recommended except when necessary for computational speed
    // reasons)");

    map.put(ACCESSIBILITY_MEASURE_TYPE, "defines type of measure for accessibility computation.");

    map.put(
        USING_CUSTOM_BOUNDING_BOX,
        "true if custom bounding box should be used for accessibility computation (otherwise e.g. extent of network will be used)");
    map.put(
        BOUNDING_BOX_BOTTOM,
    ...
    map.put(
        AREA_OF_ACC_COMP,
        "method to determine the area for which the accessibility will be computed; possible values: "
            + stb);

    map.put(
        MEASURING_POINTS_FILE,
        "if the accibility is computed using the `fromFile` option, "
  ...
  // NOTE: It seems ok to have the string constants immediately here since having them separately
  ...
  /**
   * Set the size of the tiles (in meters if a metric CRS is used). If a square grid is created,
   * this value will be used as the side length of each square. If a hexagon pattern is created,
   * this value will be used as the maximum diameter of every hexagon.
   */
  ...
  public void setTileSize_m(int value) {
    ...
    if (value < 100) {
      LOG.warn(
          "Tile size = "
              + value
              + ". This is a comparatively small value, which may lead to computational problems.");
    }
  ...
  public void setMeasurePointGeometryProvision(
      MeasurePointGeometryProvision measurePointGeometryProvision) {
  ...
  public void setAreaOfAccessibilityComputation(
      AreaOfAccesssibilityComputation areaOfAccessibilityComputation) {
  ...
  public void setMeasurePointGeometryMap(
      Map<Id<ActivityFacility>, Geometry> measurePointGeometryMap) {
    if (measuringPointsFacilities.getFacilities() == null) {
      throw new RuntimeException(
          "Setting geometries of measure points does not make sense if measure points are not yet set!");
    }
