...
public class OsmPoiReader {
  private static final Logger LOG = LogManager.getLogger(OsmPoiReader.class);
  ...
  public OsmPoiReader(String osmInputFile, CoordinateTransformation ct)
  ...
  /**
   * Parses a given <i>OpenStreetMap</i> file for data in it that can be converted into MATSim
   * facilities.
   */
  ...
  public void parseOsmFileAndAddFacilities(
      Map<String, String> osmToMatsimTypeMap, String osmKey, CompressionMethod compressionMethod) {
    OsmPoiSink sink =
        new OsmPoiSink(this.ct, osmToMatsimTypeMap, osmKey, useGeneralTypeIsSpecificTypeUnknown);
  ...
  public class OsmPoiSink implements Sink {
    ...
    public OsmPoiSink(
        ...
        Map<String, String> osmToMatsimType,
        String osmKey,
        boolean useGeneralTypeIsSpecificTypeUnknown) {
      this.ct = ct;
