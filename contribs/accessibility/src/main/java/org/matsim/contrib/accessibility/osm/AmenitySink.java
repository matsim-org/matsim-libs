...
class AmenitySink implements Sink {
  ...
  public AmenitySink(
      CoordinateTransformation ct, Map<String, String> osmToMatsimType, boolean skipDesc) {
    this.ct = ct;
  ...
  private void processFacilities(
    ...
    for (long n : entityMap.keySet()) {
      ...
      if (matsimType != null) {
        ...
        Coord coord =
            OSMCoordUtils.getCentroidCoord(
                entity, this.ct, this.nodeMap, this.wayMap, this.relationMap);
  ...
  //						if(box.get(1).getY() > ymax){ ymax = box.get(1).getY(); }
  //					} else{
  //						log.warn("Relation " + rm.getMemberId() + " was not found in relationMap, and will be
  // ignored.");
  //					}
