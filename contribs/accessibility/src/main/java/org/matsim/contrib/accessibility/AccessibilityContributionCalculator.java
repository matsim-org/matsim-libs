...
/**
 * Interface to provide a means to compute the utility of a given origin destination, for a given
 * mode. This includes travel time estimation and utility formulation.
 *
...
interface AccessibilityContributionCalculator {
  ...
  /**
   * Provided for performance purpose. The accessibility listener(s) aggregate first all "measuring
   * points" according to the node they are associated to on the network, allowing to perform some
   * preprocessing (for instance pre-computing a least-cost path tree).
   *
  ...
  /**
   * estimates the contribution of a given opportunity to the accessibility metric, defined as an
   * origin point (provided as a facility) and a group of facilities. "Contribution" is understood
   * here as what is summed, that is, for the logsum, the exponential of the utility, scaled by the
   * logit scale parameter.
   *
   * @param origin the origin point //@param destination the opportunities at the destination
   * @param departureTime TODO
   * @return the utility of the OD pair, to be included in the logsum
   */
  double computeContributionOfOpportunity(
