...
public class LeastCostPathTreeTest {
  ...
  public void testRouteChoiceTestSpanningTree() {
    this.scenario =
        new ScenarioBuilder(ConfigUtils.createConfig())
            .setNetwork(CreateTestNetwork.createTriangularNetwork())
            .build();
    compareRouteChoices();
  }
  ...
  private void compareRouteChoices() {
    ...
    double travelTime =
        lcptTime.getTree().get(destination.getId()).getCost(); // here cost = traveled time
    ...
    double travelDistance =
        lcptDistance.getTree().get(destination.getId()).getCost(); // here cost = traveled distance
    printResults(arrivalTimeTD, travelDistance); // travel time = 1000s, cost = (50m+50m) = 100m
    // check route ( visited nodes should be 1,3,4)
    List<Id<Node>> spDistenceVisitedNodes =
        getVisitedNodes(lcptDistance, destination, "Travel Distance");
  ...
  private List<Id<Node>> getVisitedNodes(
      LeastCostPathTree lcpt, Node destination, String costType) {
  ...
  private boolean containsNode(List<Id<Node>> list, Id<Node> nodeId) {
    ...
    while (nodes.hasNext()) {
      ...
      if (node.compareTo(nodeId) == 0) return true;
    }
    return false;
  }
}
