...
public class CompareLogsumFormulasTest {
  ...
  /** underlying network cjk1 / / i--------------j--cjk2 \ \ cjk3 */
  @Test
  public void testLogsumFormulas() {
    ...
    double Ai =
        computeLogsum(betaWalkTT, betaWalkTD, cik1TT, cik2TT, cik3TT, cik1TD, cik2TD, cik3TD);
    double Ai2 =
        computeTransformedLogsum(
            betaWalkTT, betaWalkTD, cijTT, cjk1TT, cjk2TT, cjk3TT, cijTD, cjk1TD, cjk2TD, cjk3TD);

    Assert.assertTrue(Ai == Ai2);
  }

  /**
   * Now the logsum is computed in 2 steps: 1) sum the disutilities of traveling (e^(Vjk)) from the
   * destination node j to all opportunities k (that are attached to j) 2) compute the disutility of
   * traveling to get from the origin i to the destination node j (e^(Vij)) 3) multiply 1) and 2)
   ...
   * @param cjk3TD
   */
  private double computeTransformedLogsum(
      double betaWalkTT,
      double betaWalkTD,
      double cijTT,
      double cjk1TT,
  ...
  /**
   * Calculates the logsum by taking log sum of the disutilities of traveling from the origin i to
   * each opportunity k
   *
   * @param betaWalkTT
   * @param betaWalkTD
