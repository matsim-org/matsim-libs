...
public class MultiModalModule extends AbstractModule {
  ...
  public void install() {
    ...
    // * Factories for network-based routing can now be provided by config, so no need to provide
    // them here.
    // * The multi-modal contrib now essentially is (I think) a "parallel" network for each mode.
    // In consequence, the corresponding network-based router
    ...
    MultiModalConfigGroup multiModalConfigGroup =
        (MultiModalConfigGroup) getConfig().getModule(MultiModalConfigGroup.GROUP_NAME);
    ...
    for (String mode : simulatedModes) {
      switch (mode) {
        case TransportMode.walk:
          {
            Provider<TravelTime> factory =
                new WalkTravelTimeFactory(routingConfigGroup, linkSlopes);
            addTravelTimeBinding(mode).toProvider(factory);
            addTravelDisutilityFactoryBinding(mode)
                .toInstance(new RandomizingTimeDistanceTravelDisutilityFactory(mode, getConfig()));
            addRoutingModuleBinding(mode).toProvider(new NetworkRoutingProvider(mode));
            break;
          }
        case TransportMode.transit_walk:
          //                Provider<TravelTime> factory = new
          // TransitWalkTravelTimeFactory(plansCalcRouteConfigGroup, linkSlopes);
          //                addTravelTimeBinding(mode).toProvider(factory);
          //                addTravelDisutilityFactoryBinding(mode).toInstance( new
          // RandomizingTimeDistanceTravelDisutility.Builder( mode ) );
        ...
        case TransportMode.bike:
          {
            Provider<TravelTime> factory =
                new BikeTravelTimeFactory(routingConfigGroup, linkSlopes);
            addTravelTimeBinding(mode).toProvider(factory);
            addTravelDisutilityFactoryBinding(mode)
                .toInstance(new RandomizingTimeDistanceTravelDisutilityFactory(mode, getConfig()));
            addRoutingModuleBinding(mode).toProvider(new NetworkRoutingProvider(mode));
