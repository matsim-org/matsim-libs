...
class MultiModalSimEngine implements MobsimEngine {
  ...
  private static final Logger log = LogManager.getLogger(MultiModalSimEngine.class);
  ...
  /*package*/ MultiModalSimEngine(
      Map<String, TravelTime> multiModalTravelTimes, MultiModalConfigGroup multiModalConfigGroup) {
    ...
    if (this.numOfThreads > 1)
      log.info(
          "Using "
              + multiModalConfigGroup.getNumberOfThreads()
              + " threads for MultiModalSimEngine.");
  }
  ...
  public void onPrepareSim() {
    ...
    for (Link link : simulatedLinks) {
      ...
      MultiModalQLinkExtension extension =
          new MultiModalQLinkExtension(link, this, getMultiModalQNodeExtension(toNodeId));
    ...
    for (Node node : simulatedNodes) {
      ...
      for (Link inLink : node.getInLinks().values()) {
        if (simulatedLinks.contains(inLink))
          inLinks.add(this.getMultiModalQLinkExtension(inLink.getId()));
      }
  ...
  /*package*/ void printSimLog(double time) {
    if (time >= this.infoTime) {
      ...
      log.info(
          "SIMULATION (MultiModalSimEngine) AT "
              + Time.writeTime(time)
              + " #links="
              + nofActiveLinks
              + " #nodes="
  ...
  public void afterSim() {
    // Calling the afterSim Method of the MultiModalSimEngineRunners will set their
  ...
  private void initMultiModalSimEngineRunners() {
    ...
    for (int i = 0; i < numOfThreads; i++) {
      MultiModalSimEngineRunner engine =
          new MultiModalSimEngineRunner(this.startBarrier, separationBarrier, this.endBarrier);
  ...
  private void assignSimEngines() {
    ...
    for (Node node : scenario.getNetwork().getNodes().values()) {
      MultiModalQNodeExtension multiModalQNodeExtension =
          this.getMultiModalQNodeExtension(node.getId());
      ...
      if (multiModalQNodeExtension != null) {
        ...
        for (Link link : node.getOutLinks().values()) {
          MultiModalQLinkExtension multiModalQLinkExtension =
              this.getMultiModalQLinkExtension(link.getId());
    ...
    for (int i = 0; i < this.runners.length; i++) {
      log.info(
          "Assigned "
              ...
              + " nodes and "
              + links[i]
              + " links to MultiModalSimEngineRunner #"
              + i);
    }
  }
}
