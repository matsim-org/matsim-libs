...
/**
 * Data to calculate a person's travel time on a link is taken from: Weidmann, Ulrich (1992)
 * Transporttechnik der Fussgänger - Transporttechnische Eigenschaften des Fussgängerverkehrs,
 * Literaturauswertung, Schriftenreihe des IVT (90), Institut für Verkehrsplanung und
 * Transportsysteme, Zürich.
 *
 * <p>Age data from 0..3 and from 80..100 is extrapolated.
 */
public class WalkTravelTime implements TravelTime {
  ...
  /*package*/ double getSlopeFactor(double slope) {
    ...
    if (slope > 80.0) {
      if (slopeRangeWarnCount.get() < 10) {
        incSlopeRangeWarnCount(
            "Slope is out of expected range (-40% .. -80%). Found slope of "
                + slope
                + ". Use 80.0 instead.");
      }
      slope = 80.0;
  ...
  /*package*/ final double getSlope(Link link) {
    ...
    if (slope == null) {
      incSlopeNotFoundWarnCount(
          "No slope information for link "
              + link.getId().toString()
  ...
  /*package*/ void setPerson(Person person) {
    ...
    double scatterSpeed =
        random.nextGaussian() * weidmannScatterStandardDeviation + weidmannReferenceWalkSpeed;
    ...
    if (person instanceof Person) {
      ...
      if (PersonUtils.getSex(p) == null) {
        if (genderWarnCount.get() < 10) {
          incGenderWarnCount(
              "Person's gender is not defined. Ignoring gender dependent walk speed factor.");
        }
      ...
      if (age == null) {
        ...
        if (ageWarnCount.get() < 10) {
          incAgeWarnCount(
              "Person's age is out of expected range (0 .. 100). Founde age of "
                  + age
                  + ". Use 0 instead.");
        }
