...
public class MultiModalTravelTimeFactory implements MatsimFactory {
  ...
  public MultiModalTravelTimeFactory(
      Config config,
      Map<Id<Link>, Double> linkSlopes,
      Map<String, Provider<TravelTime>> additionalFactories) {
    ...
    if (this.linkSlopes == null) {
      log.warn(
          "No slope information for the links available - travel time will only take agents age and gender into account!");
    }
  ...
  private void initMultiModalTravelTimeFactories(Config config) {
    ...
    MultiModalConfigGroup multiModalConfigGroup =
        (MultiModalConfigGroup) config.getModule(MultiModalConfigGroup.GROUP_NAME);
    ...
    for (String mode : simulatedModes) {
      if (mode.equals(TransportMode.walk)) {
        ...
        Provider<TravelTime> factory =
            new TransitWalkTravelTimeFactory(routingConfigGroup, linkSlopes);
        this.factories.put(mode, factory);
        ...
        if (factory == null) {
          ...
          log.info(
              "Found additional travel time factory from type "
