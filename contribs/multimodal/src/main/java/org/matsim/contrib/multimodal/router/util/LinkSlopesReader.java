...
public class LinkSlopesReader {
  ...
  private static final Logger log = LogManager.getLogger(LinkSlopesReader.class);

  private static final String ATTRIBUTE_NAME = "slope";

  /**
   * If height information for a network is available, the links' slopes is returned in a
   * Map<LinkId, Slope in %>.
   *
   * <p>It is assumed that the heightInformation ObjectAttributes contain a field "slope" for each
   * link Id in the network.
   *
  ...
  /**
   * Reads height information from the file specified in the config group and returns them as
   * ObjectAttributes.
   */
