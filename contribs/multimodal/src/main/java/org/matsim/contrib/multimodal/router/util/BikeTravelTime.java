...
/**
 * Data to calculate a person's travel time on a link is taken from: John Parkin and Jonathon
 * Rotheram (2010): Design speeds and acceleration characteristics of bicycle traffic for use in
 * planning and appraisal.
 *
 * <p>The default downhill speed is limited to 35 km/h. The value is scaled for each person based on
 * age, gender and scatter.
 *
 * <p>On steep links, a person is faster by walking than by biking. For such links, the walk travel
 * time is returned.
 */
public class BikeTravelTime extends WalkTravelTime {
  ...
  public BikeTravelTime(RoutingConfigGroup plansCalcGroup, Map<Id<Link>, Double> linkSlopes) {
  ...
  public BikeTravelTime(RoutingConfigGroup plansCalcGroup) {
  ...
  public double getLinkTravelTime(Link link, double time, Person person, Vehicle vehicle) {
    ...
    if (linkSpeed > this.maxPersonBikeSpeedCache.get())
      linkSpeed = this.maxPersonBikeSpeedCache.get();
  ...
  void setPerson(Person person) {
    ...
    if (value != null) {
      ...
      this.personUphillFactorCache.set(
          uphillFactor * personFactor * referenceBikeSpeed / defaultReferenceSpeed);
      this.personDownhillFactorCache.set(
          downhillFactor * personFactor * referenceBikeSpeed / defaultReferenceSpeed);
    ...
    //		this.personUphillFactor = uphillFactor * personFactor * referenceBikeSpeed /
    // defaultReferenceSpeed;
    //		this.personDownhillFactor = downhillFactor * personFactor * referenceBikeSpeed /
    // defaultReferenceSpeed;
