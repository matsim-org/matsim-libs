...
class MultiModalQNodeExtension {
  ...
  public boolean moveNode(final double now) {
    // Check all incoming links for WaitingToLeaveAgents. If waiting Agents are found, we activate
    // the Node.
  ...
  boolean moveAgentOverNode(final MobsimAgent mobsimAgent, final double now) {
    ...
    if (nextLinkId != null) {
      ...
      if (this.checkNextLinkSemantics(currentLink, nextLink, mobsimAgent) == false) {
        ...
        this.simEngine
            .getMultiModalQLinkExtension(nextLinkId)
            .addAgentFromIntersection(mobsimAgent, now);
      }
    }
    // --> nextLink == null
    else {
  ...
  private void moveToAbort(
      final MobsimAgent mobsimAgent, final double now, Link currentLink, Link nextLink) {
    log.error(
        "Agent has no or wrong route! agentId="
            ...
            + " nextLink="
            + (nextLink != null ? nextLink.getId() : "null")
            + " currentLinkToNode="
            + currentLink.getToNode().getId()
            + " nextLinkFromNode="
            + (nextLink != null ? nextLink.getFromNode().getId() : "null")
