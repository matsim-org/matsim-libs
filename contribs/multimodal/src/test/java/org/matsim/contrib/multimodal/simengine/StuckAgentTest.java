...
public class StuckAgentTest {
  ...
  public void testStuckEvents() {
    ...
    Node node1 =
        scenario
            .getNetwork()
            .getFactory()
            .createNode(Id.create("n1", Node.class), new Coord(1.0, 0.0));
    Node node2 =
        scenario
            .getNetwork()
            .getFactory()
            .createNode(Id.create("n2", Node.class), new Coord(2.0, 0.0));
    Node node3 =
        scenario
            .getNetwork()
            .getFactory()
            .createNode(Id.create("n3", Node.class), new Coord(3.0, 0.0));
    Node node4 =
        scenario
            .getNetwork()
            .getFactory()
            .createNode(Id.create("n4", Node.class), new Coord(4.0, 0.0));

    Link link0 =
        scenario.getNetwork().getFactory().createLink(Id.create("l0", Link.class), node0, node1);
    Link link1 =
        scenario.getNetwork().getFactory().createLink(Id.create("l1", Link.class), node1, node2);
    Link link2 =
        scenario.getNetwork().getFactory().createLink(Id.create("l2", Link.class), node2, node3);
    Link link3 =
        scenario.getNetwork().getFactory().createLink(Id.create("l3", Link.class), node3, node4);
    ...
    Route route0 =
        routeFactory.createRoute(
            Id.create("l0", Link.class), Id.create("l3", Link.class)); // missing l1 & l2
    Route route1 =
        routeFactory.createRoute(
            Id.create("l0", Link.class), Id.create("l3", Link.class)); // missing l2
    List<Id<Link>> linkIds = new ArrayList<>();
    linkIds.add(Id.create("l0", Link.class));
    ((NetworkRoute) route1)
        .setLinkIds(Id.create("l0", Link.class), linkIds, Id.create("l3", Link.class));

    scenario
        .getPopulation()
        .addPerson(createPerson(scenario, "p0", "bike", route0, 6.5 * 3600)); // stuck
    scenario
        .getPopulation()
        .addPerson(createPerson(scenario, "p1", "walk", route1, 7.5 * 3600)); // stuck
    scenario
        .getPopulation()
        .addPerson(createPerson(scenario, "p2", "walk", null, 8.5 * 3600)); // regular
    scenario
        .getPopulation()
        .addPerson(
            createPerson(
                scenario, "p3", "walk", null, 23.5 * 3600)); // en-route when simulation ends
    scenario
        .getPopulation()
        .addPerson(
            createPerson(
                scenario, "p4", "walk", null, 24.5 * 3600)); // departs after simulation has ended
  ...
  private Person createPerson(
      Scenario scenario, String id, String mode, Route route, double departureTime) {
    ...
    Activity to =
        scenario
            .getPopulation()
            .getFactory()
            .createActivityFromLinkId("home", Id.create("l3", Link.class));
