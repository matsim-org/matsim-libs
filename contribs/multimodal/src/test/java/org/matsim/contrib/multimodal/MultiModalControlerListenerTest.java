  ...
  void runBerlinScenario(int numberOfThreads) {
    ...
    Config config =
        ConfigUtils.loadConfig(
            IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("berlin"), "config.xml"));
    ...
    // added by me to fix the test.  If you normally run with the default setting (now
    // tryEndTimeThenDuration), I would suggest to remove
    ...
    Assert.assertEquals(
        "unexpected number of link leave events for mode car with number of threads "
            + numberOfThreads,
        //				513445, carCount);
        692259,
        carCount);
    Assert.assertEquals(
        "unexpected number of link leave events for mode bike with number of threads "
            + numberOfThreads,
        4577,
        bikeCount);
    Assert.assertEquals(
        "unexpected number of link leave events for mode walk with number of threads "
            + numberOfThreads,
        //				5834, walkCount);
        7970,
        walkCount);
