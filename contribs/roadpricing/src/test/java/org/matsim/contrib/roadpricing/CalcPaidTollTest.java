...
/**
 * Tests that {@link RoadPricingTollCalculator} calculates the correct tolls and adds them to the
 * scores of the executed plans.
 *
 * @author mrieser
 */
public class CalcPaidTollTest {
  ...
  private static final Logger log = LogManager.getLogger(CalcPaidTollTest.class);
  ...
  public void testDistanceToll() {
    ...
    Map<Id<Person>, ? extends Person> referencePopulation =
        RoadPricingTestUtils.createReferencePopulation1(config).getPersons();
    Map<Id<Person>, ? extends Person> population =
        runTollSimulation(tollFile, "distance", config).getPersons();
  ...
  public void testAreaToll() {
    ...
    Map<Id<Person>, ? extends Person> referencePopulation =
        RoadPricingTestUtils.createReferencePopulation1(config).getPersons();
    Map<Id<Person>, ? extends Person> population =
        runTollSimulation(tollFile, "area", config).getPersons();
  ...
  public void testCordonToll() {
    ...
    Map<Id<Person>, ? extends Person> referencePopulation =
        RoadPricingTestUtils.createReferencePopulation1(config).getPersons();
    Map<Id<Person>, ? extends Person> population =
        runTollSimulation(tollFile, "link", config).getPersons();
    ...
    //				referencePopulation.get(id5).getPlans().get(0).getScore(),
    //				population.get(id5).getPlans().get(0).getScore(),
    //				1.00); // this agent only pays when entering in the second area, as it starts in the first
    // where it should not be tolled.
    //		compareScores(
    //				referencePopulation.get(id7).getPlans().get(0).getScore(),
    //				population.get(id7).getPlans().get(0).getScore(),
    ...
    // The above test cases have to do with that somewhat weird cording pricing implementation where
    // the toll was only paid on the first tolled link that was encountered.
    // (The assumption being that one has an area toll file and wants to use it for cording
    // pricing.)  That execution path is now deleted.  kai/kai, feb'21

  }

  private void compareScores(
      final double scoreWithoutToll, final double scoreWithToll, final double expectedToll) {
   ...
   */
  private Population runTollSimulation(
      final String tollFile, final String tollType, final Config config) {
  ...
  private void runTollSimulation(final Scenario scenario, final RoadPricingScheme toll) {
    ...
    RoadPricingTollCalculator paidToll =
        new RoadPricingTollCalculator(scenario.getNetwork(), toll, eventsManager);
    EventsToScore scoring =
        EventsToScore.createWithScoreUpdating(
            scenario, new CharyparNagelScoringFunctionFactory(scenario), eventsManager);
