...
public class PlansCalcRouteWithTollOrNotTest {
  ...
  public void testBestAlternatives() {
    ...
    RoadPricingTestUtils.compareRoutes(
        "2 3 4 6", (NetworkRoute) getLeg1(config, population, id1).getRoute());
    RoadPricingTestUtils.compareRoutes(
        "8 11 12", (NetworkRoute) getLeg3(config, population, id1).getRoute());
    ...
    log.warn("access/egress? " + config.routing().getAccessEgressType());
    RoadPricingTestUtils.compareRoutes(
        "2 5 6", (NetworkRoute) getLeg1(config, population, id1).getRoute());
    RoadPricingTestUtils.compareRoutes(
        "8 11 12", (NetworkRoute) getLeg3(config, population, id1).getRoute());
  ...
  private static Leg getLeg1(Config config, Population population, Id<Person> id1) {
    final List<PlanElement> planElements =
        population.getPersons().get(id1).getPlans().get(0).getPlanElements();
  ...
  private PlansCalcRouteWithTollOrNot testee(
    ...
    return new PlansCalcRouteWithTollOrNot(
        toll, tripRouterProvider, TimeInterpretation.create(scenario.getConfig()));
    // yy might be more plausible to get the full class out of the injector, but that ain't that
    // easy ...  kai, oct'19
  }
  ...
  public void testTolledActLink() {
    ...
    RoadPricingTestUtils.compareRoutes(
        "2 5 6",
        (NetworkRoute)
            getLeg1(config, population, id1).getRoute()); // agent should take shortest route
    RoadPricingTestUtils.compareRoutes(
        "8 11 12", (NetworkRoute) getLeg3(config, population, id1).getRoute());
  }
  ...
  public void testOutsideTollTime() {
    ...
    RoadPricingTestUtils.compareRoutes(
        "2 5 6",
        (NetworkRoute)
            leg1.getRoute()); // agent should take shortest route, as tolls are not active at that
    // time
