...
/**
 * Tests the correct working of {@link TravelDisutilityIncludingToll} by using it to calculate some
 * routes with {@link PlansCalcRoute}.
 *
 * @author mrieser
 */
public class TollTravelCostCalculatorTest {
  ...
  public void testDisutilityResults() {
    ...
    RoadPricingSchemeImpl scheme =
        RoadPricingUtils.addOrGetMutableRoadPricingScheme(
    ...
    final TravelDisutilityFactory defaultDisutilityFactory =
    ...
    RoadPricingTravelDisutilityFactory travelDisutilityFactory =
        new RoadPricingTravelDisutilityFactory(defaultDisutilityFactory, scheme, margUtlOfMoney);
    //        travelDisutilityFactory.setSigma( 0. ) ;
    TravelDisutility travelDisutility =
        travelDisutilityFactory.createTravelDisutility(timeCalculator);
    ...
    for (int ii = 0; ii < results.length; ii++) {
...

      {
        double value =
            travelDisutility.getLinkTravelDisutility(
                net.getLinks().get(link5Id), 10., person, null);
      ...
      }
      {
        double value =
            travelDisutility.getLinkTravelDisutility(
                net.getLinks().get(link11Id), 10, person, null);
        System.out.println("value=" + value);
        //				Assert.assertEquals( results[ii], value, delta);
      }
    }
  ...
  public void testDistanceTollRouter() {
    ...
    // yy note: this returns a combined TravelTime and TravelDisutility object.  The
    // TravelDisutility object is used in the next three lines to be wrapped,
    // and then never again.  Would be nice to be able to get them separately ...  kai, oct'13

    TravelDisutility costCalc =
        new TravelDisutilityIncludingToll(
            (TravelDisutility) timeCalc, toll, config); // we use freespeedTravelCosts as base costs
    ...
    routePopulation(scenario, new DijkstraFactory(), timeCalc, costCalc);
    RoadPricingTestUtils.compareRoutes(
        "2 5 6",
        (NetworkRoute)
            ((Leg) (person1.getPlans().get(0).getPlanElements().get(carLegIndex))).getRoute());
    // also test it with A*-Landmarks
    clearRoutes(population);
    assertNull(
        ((Leg) (person1.getPlans().get(0).getPlanElements().get(1)))
            .getRoute()); // make sure the cleaning worked. we do this only once, then we believe
    // it.
    routePopulation(scenario, aStarLandmarksFactory, timeCalc, costCalc);
    RoadPricingTestUtils.compareRoutes(
        "2 5 6",
        (NetworkRoute)
            ((Leg) (person1.getPlans().get(0).getPlanElements().get(carLegIndex))).getRoute());
    ...
    Cost morningCost =
        toll.createAndAddCost(
            6 * 3600, 10 * 3600,
            0.0006); // 0.0006 * link_length(100m) = 0.06, which is slightly below the threshold of
    // 0.0666
    ...
    toll.createAndAddCost(
        6 * 3600, 10 * 3600,
        0.0007); // new morning toll, this should be slightly over the threshold
    clearRoutes(population);
    routePopulation(scenario, new DijkstraFactory(), timeCalc, costCalc);
    RoadPricingTestUtils.compareRoutes(
        "2 3 4 6",
  ...
  public void testLinkTollRouter() {
    ...
    TravelDisutility costCalc =
        new TravelDisutilityIncludingToll(
            timeCostCalc, toll, config); // we use freespeedTravelCosts as base costs
    ...
    Person person1 = population.getPersons().get(Id.create("1", Person.class));
    // 1st case: without toll, agent chooses shortest path
    routePopulation(scenario, new DijkstraFactory(), timeCostCalc, costCalc);
    ...
    RoadPricingTestUtils.compareRoutes(
        "2 5 6", (NetworkRoute) ((Leg) (planElements.get(carLegIndex))).getRoute());

    // also test it with A*-Landmarks
    clearRoutes(population);
    //		assertNull(((LegImpl)
    // (person1.getPlans().get(0).getPlanElements().get(carLegIndex))).getRoute()); // make sure the
    // cleaning worked. we do this only once, then we believe it.
    routePopulation(scenario, routerFactory, timeCostCalc, costCalc);
    ...
    RoadPricingTestUtils.compareRoutes(
        "2 5 6", (NetworkRoute) ((Leg) (planElements2.get(carLegIndex))).getRoute());

    Cost morningCost =
        toll.createAndAddCost(
            6 * 3600, 10 * 3600, 0.06); // 0.06, which is slightly below the threshold of 0.0666
    // 2nd case: with a low toll, agent still chooses shortest path
    clearRoutes(population);
    routePopulation(scenario, new DijkstraFactory(), timeCostCalc, costCalc);
    ...
    RoadPricingTestUtils.compareRoutes(
        "2 5 6", (NetworkRoute) ((Leg) (planElements3.get(carLegIndex))).getRoute());
    // also test it with A*-Landmarks
    clearRoutes(population);
    routePopulation(scenario, routerFactory, timeCostCalc, costCalc);
    RoadPricingTestUtils.compareRoutes(
    ...
    toll.removeCost(morningCost);
    toll.createAndAddCost(
        6 * 3600, 10 * 3600, 0.07); // new morning toll, this should be slightly over the threshold
    clearRoutes(population);
    routePopulation(scenario, new DijkstraFactory(), timeCostCalc, costCalc);
    RoadPricingTestUtils.compareRoutes(
        "2 3 4 6",
  ...
  /**
   * Clears all routes from all legs of all persons in the given population to make sure they are
   * calculated from new.
   *
   * @param population
   */
  ...
  private static void routePopulation(
    ...
    final PersonAlgorithm router =
        new PlanRouter(tripRouter, TimeInterpretation.create(scenario.getConfig()));
