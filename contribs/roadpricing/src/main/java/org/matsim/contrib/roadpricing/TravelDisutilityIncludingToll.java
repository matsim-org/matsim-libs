...
/**
 * Calculates the travel disutility for links, including tolls. Currently supports distance, cordon
 * and area tolls.
 *
 * @author mrieser
 */
class TravelDisutilityIncludingToll implements TravelDisutility {
  ...
  TravelDisutilityIncludingToll(
      final TravelDisutility normalTravelDisutility,
  ...
  TravelDisutilityIncludingToll(
    ...
    if (RoadPricingScheme.TOLL_TYPE_DISTANCE.equals(scheme.getType())) {
      ...
      LogManager.getLogger(this.getClass())
          .warn(
              "area pricing is more brittle than the other toll schemes; "
      ...
      throw new RuntimeException(
          "Use LINK toll type for cordon pricing (charge toll on links traversing cordon)");
    } else if (scheme.getType().equals(RoadPricingScheme.TOLL_TYPE_LINK)) {
      this.tollCostHandler = new LinkTollCostBehaviour();
    } else {
      throw new IllegalArgumentException(
          "RoadPricingScheme of type \"" + scheme.getType() + "\" is not supported.");
    }
    ...
    if (utlOfMoneyWrnCnt < 1 && this.marginalUtilityOfMoney != 1.) {
      ...
      LogManager.getLogger(this.getClass())
          .warn(
              "There are no test cases for marginalUtilityOfMoney != 1.  Please write one "
  ...
  public double getLinkTravelDisutility(
    ...
    double normalTravelDisutilityForLink =
        this.normalTravelDisutility.getLinkTravelDisutility(link, time, person, vehicle);
    ...
    // sign convention: these are all costs (= disutilities), so they are all normally positive.
    // tollCost is positive, marginalUtilityOfMoney as well.
  }
  ...
  /*package*/ class AreaTollCostBehaviour implements TollRouterBehaviour {
    ...
    public double getTypicalTollCost(final Link link, final double time) {
      ...
      if (wrnCnt2 < 1) {
        ...
        LogManager.getLogger(this.getClass())
            .warn(
                "at least here, the area toll does not use the true toll value. "
