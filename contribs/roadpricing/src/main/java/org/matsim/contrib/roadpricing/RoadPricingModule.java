...
public final class RoadPricingModule extends AbstractModule {
  private static final Logger LOG = LogManager.getLogger(RoadPricingModule.class);
  ...
  public void install() {
    ...
    // also add RoadPricingScheme as ScenarioElement.  yyyy TODO might try to get rid of this;
    // binding it is safer
    // (My personal preference is actually to have it as scenario element ... since then it can be
    // set before controler is even called.  Which
    ...
    addTravelDisutilityFactoryBinding(TransportMode.car)
        .toProvider(TravelDisutilityIncludingTollFactoryProvider.class);
    ...
    addTravelDisutilityFactoryBinding(PlansCalcRouteWithTollOrNot.CAR_WITH_PAYED_AREA_TOLL)
        .toInstance(
            new RandomizingTimeDistanceTravelDisutilityFactory(TransportMode.car, getConfig()));
    addRoutingModuleBinding(PlansCalcRouteWithTollOrNot.CAR_WITH_PAYED_AREA_TOLL)
        .toProvider(new RoadPricingNetworkRouting());

    // yyyy TODO It might be possible that the area stuff is adequately resolved by the randomizing
    // approach.  Would need to try
    // that out.  kai, sep'16
