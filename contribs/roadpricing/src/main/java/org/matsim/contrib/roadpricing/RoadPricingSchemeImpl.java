...
/**
 * A road pricing scheme (sometimes also called toll scheme) contains the type of the toll, a list
 * of the tolled links and the (time-dependent) toll amount agents have to pay.
 *
 * @author mrieser
 */
public final class RoadPricingSchemeImpl implements RoadPricingScheme {
  ...
  private void warnAboutNoToll(double startTime, double endTime) {
    if (startTime == 0. && endTime == 24. * 3600.) {
      if (wrnCnt < 1) {
        wrnCnt++;
        log.warn(
            "startTime=0:00 and endTime=24:00 means NO toll after 24h (no wrap-around); make sure this is what you want");
        if (wrnCnt == 1) {
  ...
  public Cost getLinkCostInfo(
    ...
    // Now also added vehicleId as an argument, which is also ignored at the default level. kai,
    // apr'14
