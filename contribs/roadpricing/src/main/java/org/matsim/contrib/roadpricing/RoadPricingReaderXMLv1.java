...
/**
 * Reads XML files containing a {@link RoadPricingSchemeImpl} according to <code>roadpricing_v1.dtd
 * </code>.
 *
 * @author mrieser
 */
public final class RoadPricingReaderXMLv1 extends MatsimXmlParser {
  ...
  public void startTag(final String name, final Attributes atts, final Stack<String> context) {
    if (TAG_ROADPRICING.equals(name)) {
      ...
      double startTime =
          Time.parseOptionalTime(atts.getValue(ATTR_START_TIME)).orElse(Double.NEGATIVE_INFINITY);
      // an undefined toll end time == +Inf
      double endTime =
          Time.parseOptionalTime(atts.getValue(ATTR_END_TIME)).orElse(Double.POSITIVE_INFINITY);
