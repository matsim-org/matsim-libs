	...
	class DistanceTollBehaviour implements TollBehaviourI {
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			if (cost != null) {
				...
				info.toll += newToll;
				events.processEvent(new PersonMoneyEvent(event.getTime(),driverId,-newToll,"toll",""));
	...
	class LinkTollBehaviour implements TollBehaviourI {
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			if (cost != null) {
				...
				events.processEvent(new PersonMoneyEvent(event.getTime(),driverId,-cost.amount,"toll",""));
			}
		}
	}
