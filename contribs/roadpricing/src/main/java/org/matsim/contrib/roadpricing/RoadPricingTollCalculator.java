...
/**
 * Calculates the toll agents pay during a simulation by analyzing events. Add an instance of this
 * class as an EventHandler.
 *
 * <p>Users of this class can get to the amounts which have to be paid by the agents by using the
 * getter methods and/or by asking for a stream of AgentMoneyEvent instances.
 *
 * @author mrieser
 */
public final class RoadPricingTollCalculator
  ...
  /**
   * Much of this is no longer needed since we now throw money events immediately when links are
   * left. It is, however, still needed for area toll, and for the specific implementation of cordon
   * toll here. kai, jan'21
   */
  ...
  RoadPricingTollCalculator(
      final Network network, final RoadPricingScheme scheme, EventsManager events) {
    super();
    ...
    if (RoadPricingScheme.TOLL_TYPE_DISTANCE.equals(scheme.getType())) {
      ...
      log.warn(
          "area toll does not work if you have different toll amounts on different "
  ...
  public void handleEvent(final VehicleEntersTrafficEvent event) {
    ...
    if (handler instanceof DistanceTollBehaviour || handler instanceof LinkTollBehaviour) {
      ...
      this.handler.handleEvent(
          new LinkEnterEvent(event.getTime(), event.getVehicleId(), event.getLinkId()), link);
    }
  }
  ...
  //	 * @param events the {@link EventsManager} collection, the generated events are sent to for
  // processing
  //	 */
  ...
  /**
   * Handles the calculation of the distance toll. If an agent enters a link at a time the toll has
   * to be paid, the toll amount is added to the agent. The agent does not have to pay the toll for
   * a link if it starts on the link, as it may have paid already when arriving on the link.
   */
  private class DistanceTollBehaviour implements TollBehaviourI {
    ...
    public void handleEvent(final LinkEnterEvent event, final Link link) {
      ...
      Cost cost =
          RoadPricingTollCalculator.this.scheme.getLinkCostInfo(
              link.getId(), event.getTime(), driverId, event.getVehicleId());
      if (cost != null) {
        double newToll = link.getLength() * cost.amount;
        events.processEvent(
            new PersonMoneyEvent(event.getTime(), driverId, -newToll, "toll", null));

        AgentTollInfo info =
            RoadPricingTollCalculator.this.agents.computeIfAbsent(
                driverId, (key) -> new AgentTollInfo());
        info.toll += newToll;
      }
    }
  }
  ...
  private class LinkTollBehaviour implements TollBehaviourI {
    ...
    public void handleEvent(final LinkEnterEvent event, final Link link) {
      ...
      if (cost != null) {
        events.processEvent(
            new PersonMoneyEvent(event.getTime(), driverId, -cost.amount, "toll", null));
  ...
  /**
   * Handles the calculation of the area toll. Whenever the agent is seen on one of the tolled link,
   * the constant toll amount has to be paid once. <br>
   * Design comments:
   *
   * <ul>
   *   <li>This implementation becomes a problem if someone tries to implement more than one area
   *       which do not share the same flat fee. kai, mar'12
   * </ul>
   */
