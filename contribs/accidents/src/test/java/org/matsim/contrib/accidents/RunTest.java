...
public class RunTest {
  ...
  public void test1() {
    ...
    String configFile =
        utils.getPackageInputDirectory() + "/trial_scenario/trial_scenario_config.xml";
    ...
    for (Link link : scenario.getNetwork().getLinks().values()) {
      link.getAttributes()
          .putAttribute(
              accidentsSettings.getAccidentsComputationMethodAttributeName(),
              AccidentsComputationMethod.BVWP.toString());
      ...
      if (link.getFreespeed() > 16.) {
        link.getAttributes()
            .putAttribute(
                AccidentsConfigGroup.BVWP_ROAD_TYPE_ATTRIBUTE_NAME, "1,0," + numberOfLanesBVWP);
      } else {
        link.getAttributes()
            .putAttribute(
                AccidentsConfigGroup.BVWP_ROAD_TYPE_ATTRIBUTE_NAME, "1,2," + numberOfLanesBVWP);
      }
    }
    ...
    BufferedReader br =
        IOUtils.getBufferedReader(outputDirectory + "ITERS/it.0/run1.0.accidentCosts_BVWP.csv");
      ...
      while ((line = br.readLine()) != null) {
        ...
        for (int column = 0; column < columns.length; column++) {
          ...
          if (lineCounter == 0 && column == 25) {
            ...
            Assert.assertEquals(
                "wrong accident costs", 10.38, accidentCosts, MatsimTestUtils.EPSILON);
          }

          if (lineCounter == 1 && column == 25) {
            double accidentCosts = Double.valueOf(columns[column]);
            Assert.assertEquals(
                "wrong accident costs", 16.68, accidentCosts, MatsimTestUtils.EPSILON);
          }
        }
