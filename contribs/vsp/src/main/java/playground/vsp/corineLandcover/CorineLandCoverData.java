...
public class CorineLandCoverData {
  ...
  private final Map<LandCoverUtils.LandCoverActivityType, Geometry>
      activityType2CombinedLandcoverZone = new HashMap<>();
  Map<LandCoverUtils.LandCoverActivityType, List<Geometry>> activityTypes2ListOfLandCoverZones =
      new HashMap<>();
  ...
  public CorineLandCoverData(
      String corineLandCoverShapeFile, boolean simplifyGeometries, boolean combiningGeom) {

    LOGGER.info("Reading CORINE landcover shape file . . .");
    Collection<SimpleFeature> landCoverFeatures =
        ShapeFileReader.getAllFeatures(corineLandCoverShapeFile);

    this.simplifyGeometries = simplifyGeometries;
    if (this.simplifyGeometries)
      LOGGER.warn(
          "Geometries will be simplified such that number of vertices in each geometry is less than 1000. "
              + "This is likely to speed up the process.");

    for (SimpleFeature landCoverZone : landCoverFeatures) {
      int landCoverId =
          Integer.valueOf(
              (String) landCoverZone.getAttribute(LandCoverUtils.CORINE_LANDCOVER_TAG_ID));
      List<LandCoverUtils.LandCoverActivityType> acts =
          landCoverUtils.getActivityTypesFromZone(landCoverId);
    ...
    if (this.combiningGeom) {
      LOGGER.warn(
          "Combining geoms is most expensive operation. See http://docs.geotools.org/latest/userguide/library/jts/combine.html");
      // combine geoms of the same activity types
      // Not sure, Out of two options --simplify and merge OR merge and simplify-- I think, the
      // former would be somewhat better. Amit Oct'17
      for (LandCoverUtils.LandCoverActivityType activityTypeFromLandCover :
          activityTypes2ListOfLandCoverZones.keySet()) {
        LOGGER.info(
            "Merging the geometries of the activity types " + activityTypeFromLandCover + ".");
        Geometry combinedGeom =
            GeometryUtils.combine(
                activityTypes2ListOfLandCoverZones.get(activityTypeFromLandCover));
  ...
  public static void main(String[] args) {
    String landcoverFile =
        "../../repos/shared-svn/projects/nemo_mercator/30_Scenario/cemdap_input/shapeFiles/CORINE_landcover_nrw/corine_nrw_src_clc12.shp";
    String zoneFile =
        "../../repos/shared-svn/projects/nemo_mercator/30_Scenario/cemdap_input/shapeFiles/sourceShape_NRW/dvg2gem_nw.shp";
    CorineLandCoverData landCoverInformer = new CorineLandCoverData(landcoverFile, true, false);
    //        landCoverInformer.getRandomPoint(...);
  }

  /**
   * @param thresholdForPointInsideLandUseGeoms number of times the check is performed (if the point
   *     is inside 'landUseGeoms'); afterwards a random point is returned.
   */
   ...
   * @return a random point such that it is inside the @param feature as well as inside the zones
   *     corresponding to the given activity type.
   */
  public Point getRandomPoint(
      final SimpleFeature feature, final LandCoverUtils.LandCoverActivityType activityType) {
   ...
   */
  public Point getRandomPoint(
      final Geometry geometry, final LandCoverUtils.LandCoverActivityType activityType) {
    Geometry zoneGeom;
    if (this.simplifyGeometries) {
    ...
    if (activityType.equals(LandCoverUtils.LandCoverActivityType.home)) {
      ...
      if (!activityType.equals(LandCoverUtils.LandCoverActivityType.other) && warnCnt < 1) {
        LOGGER.warn(
            "A random point is desired for activity type "
                + activityType
                + ". However, the CORINE landcover data is categorized only for 'home' and 'other' activity types.");
        LOGGER.warn(Gbl.ONLYONCE);
        warnCnt++;
      }

      landUseGeom =
          this.activityType2CombinedLandcoverZone.get(LandCoverUtils.LandCoverActivityType.other);
      landUseGeoms =
          this.activityTypes2ListOfLandCoverZones.get(LandCoverUtils.LandCoverActivityType.other);
    }

    if (this.combiningGeom) return GeometryUtils.getPointInteriorToGeometry(landUseGeom, zoneGeom);
    else
      return GeometryUtils.getPointInteriorToGeometriesWithFallback(
          landUseGeoms, zoneGeom, this.thresholdForPointInsideLandUseGeoms);
  }
   ...
   */
  public boolean isPointInsideLandCover(
      LandCoverUtils.LandCoverActivityType activityType, Point point) {
    if (combiningGeom) {
      Geometry geometry = this.activityType2CombinedLandcoverZone.get(activityType);
      return geometry.contains(point);
    } else {
      return GeometryUtils.isPointInsideGeometries(
          this.activityTypes2ListOfLandCoverZones.get(activityType), point);
    }
  }
}
