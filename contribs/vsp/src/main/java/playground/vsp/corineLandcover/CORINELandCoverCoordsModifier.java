 ...
 * <li>- takes matsim plans, CORINE shape file and zonal shape file as input
 * <li>- check for every activity location, if they are inside the correct zone based on CORINE
 *     landcover activity type (home/other) - if not, reassign the coordinate.
 *
 *     <p>There are some assumptions about the activity types:
 * <li>-first activity is always home
 * <li>-all home activities are located at the same coord if sameHomeActivity is set to true.
 */
public class CORINELandCoverCoordsModifier {
  ...
  public CORINELandCoverCoordsModifier(
      String matsimPlans,
      Map<String, String> shapeFileToFeatureKey,
      String CORINELandCoverFile,
      boolean simplifyGeoms,
      boolean combiningGeoms,
      boolean sameHomeActivity,
      String homeActivityPrefix) {
    this.corineLandCoverData =
        new CorineLandCoverData(CORINELandCoverFile, simplifyGeoms, combiningGeoms);
    ...
    for (String shapeFile : shapeFileToFeatureKey.keySet()) {
      ...
      for (SimpleFeature feature : features) {
        ...
        String shapeId =
            Cemdap2MatsimUtils.removeLeadingZeroFromString((String) feature.getAttribute(key));

        if (zoneFeatures.get(shapeId) != null) { // union geoms corresponding to same zone id.
          zoneFeatures.put(
              shapeId, GeometryUtils.combine(Arrays.asList(geometry, zoneFeatures.get(shapeId))));
        } else {
    ...
    if (this.sameHomeActivity)
      LOG.info("Home activities for a person will be at the same location.");
  ...
  public static void main(String[] args) {
    ...
    String corineLandCoverFile =
        "/Users/amit/Documents/gitlab/mercator-nemo/data/original_files/shapeFiles/CORINE_landcover_nrw/corine_nrw_src_clc12.shp"; // <- e.g. Berlin brandenburg file, NRW
    String zoneFile =
        "/Users/amit/Documents/gitlab/mercator-nemo/data/original_files/shapeFiles/sourceShape_NRW/dvg2gem_nw.shp";
    String zoneIdTag = "KN";

    String spatialRefinementShapeFile =
        "/Users/amit/Documents/gitlab/mercator-nemo/data/original_files/shapeFiles/plzBasedPopulation/plz-gebiete_Ruhrgebiet/plz-gebiete_Ruhrgebiet_withPopulation.shp";
    String featureKeySpatialRefinement = "plz";

    String matsimPlans =
        "/Users/amit/Documents/gitlab/mercator-nemo/data/matsim_input/zz_archive/plans/2018_jan_24/mergedPlansFiles/plans_1pct_fullChoiceSet.xml.gz";
    String outPlans =
        "/Users/amit/Documents/gitlab/mercator-nemo/data/matsim_input/zz_archive/plans/2018_jan_24/mergedPlansFiles/plans_1pct_fullChoiceSet_coordsAssigned.xml.gz";
      ...
      for (Plan plan : person.getPlans()) {
        for (PlanElement planElement : plan.getPlanElements()) {
          if (planElement instanceof Activity) {
            ...
            // could be home_10587 or home_1h
            String activityType = activity.getType(); // could be home or home_1h

            // during matsim plans generation, for home activities following fake coordinate is
            // assigned.
            if (coord != null && coord.equals(fakeCoord)) {
              ...
              if (homeLocationCoord == null) {
                ...
                if (activityType.startsWith(this.homeActivityPrefix)) {
                  if (!corineLandCoverData.isPointInsideLandCover(
                      LandCoverUtils.LandCoverActivityType.home, point)) {
                  ...
                  if (!corineLandCoverData.isPointInsideLandCover(
                      LandCoverUtils.LandCoverActivityType.other, point)) {
  ...
  private Coord getRandomCoord(LandCoverUtils.LandCoverActivityType activityType, String zoneId) {
    Geometry zone =
        this.zoneFeatures.keySet().stream()
   ...
   * @param point
   * @param activityType
   * @return a random coord if there was already a coordinate assigned to activity, i.e. already
   *     assigned coord is inside the zone (municipality/plz/lor) but not in the CORINE landcover
   *     zone for given activity type.
   */
  private Coord reassignCoord(Point point, LandCoverUtils.LandCoverActivityType activityType) {
    Geometry zone =
        this.zoneFeatures.values().stream()
    ...
    if (zone == null) {
      LOG.warn(
          point.toString()
              + " / "
              + activityType
              + " not reassigned (Activity coordinates are outside given shape zone.");
