...
public class GeometryUtils {
  ...
  /**
   * @param geom
   * @return a simplified geometry by increasing tolerance until number of vertices are less than
   *     1000.
   */
   ...
   */
  public static Point getPointInteriorToGeometry(
      final Geometry landUseGeom, final Geometry zoneGeom) {
    ...
    do {
      // assuming that zoneGeom is a subset of landUseGeom, it would be better to first find a point
      // in a subset and then look if it's inside landUseGeom
   ...
   */
  public static Point getPointInteriorToGeometries(
      final Collection<Geometry> landUseGeoms, final Geometry zoneGeom) {
  ...
  /**
   * @param landUseGeoms
   * @param threshold number of times the check is performed (if the point is inside
   *     'landUseGeoms'); afterwards a random point is returned.
   * @return a random point which is covered by list of landUseGeom as well as zoneGeom
   */
  public static Point getPointInteriorToGeometriesWithFallback(
      final Collection<Geometry> landUseGeoms, final Geometry zoneGeom, final int threshold) {
    if (landUseGeoms.isEmpty() || zoneGeom.isEmpty()) throw new RuntimeException("No geometries.");
    ...
    do {
      // assuming that zoneGeom is a subset of landUseGeom, it would be better to first find a point
      // in a subset and then look if it's inside landUseGeom
      // this can create infinite loops, I'm changing it to fallback to a random point in case we
      // can't find anything .
      counter++;
      Coordinate coordinate = getRandomInteriorPoints(zoneGeom, 1)[0];
      commonPoint = geometryFactory.createPoint(coordinate);
  ...
  /**
   * Return a random Coordinate in the geometry or null if ({@link Geometry#isEmpty()} || !(g
   * instanceof {@linkPolygonal})).
   *
   * @param g
   * @return
   */
  public static final Coordinate[] getRandomInteriorPoints(Geometry g, int numPoints) {
    if (!(g instanceof Polygonal) || g.isEmpty())
      throw new RuntimeException("Given geometry is not an instance of polygon or is empty.");
   ...
   */
  public static boolean isPointInsideGeometries(
      final Collection<Geometry> geometries, final Point point) {
