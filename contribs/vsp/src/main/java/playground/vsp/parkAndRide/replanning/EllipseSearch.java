...
public class EllipseSearch {
  ...
  public List<PRWeight> getPrWeights(
      int nrPrFacilities,
      Network net,
    ...
    if (nrPrFacilities == 0) {
      ...
      if (nrPrFacilities > id2prFacility.size()) {
        throw new RuntimeException(
            "Number of park-and-ride facilities to calculate the weight for is higher than number of available park-and-ride facilities.");
      }

      log.info(
          "Getting weights for " + nrPrFacilities + " randomly chosen park-and-ride facilities...");
      ...
      for (int n = 0; n < nrPrFacilities; ) {
        ...
        if (insertedPrIds.contains(pr.getId())) {
          log.info(
              "Weight for ParkAndRide Facility "
                  + pr.getId().toString()
                  + " already calculated. Chosing again... ");
        } else {
  ...
  private double calculateWeight(Coord homeCoord, Coord workCoord, Coord prCoord, double gravity) {
    ...
    //		System.out.println("home-->P+R-->work: " + homeCoord.toString() + " --> " +
    // prCoord.toString() + " --> " + workCoord.toString() + " = distance [km]: " + r);
  ...
  public Link getRndPrLink(
      Network net, Map<Id<PRFacility>, PRFacility> id2prFacility, List<PRWeight> prWeights) {
    if (prWeights.isEmpty()) {
      throw new RuntimeException(
          "For no park-and-ride facility a weight is calculated. Aborting...");
    }
    ...
    //		for (PrWeight prWeight : prWeights) {
    //			System.out.println("id / weight / probability: " + prWeight.getId() + " / " +
    // prWeight.getWeight() + " / " + prWeight.getWeight() / weightSum);
    //		}
