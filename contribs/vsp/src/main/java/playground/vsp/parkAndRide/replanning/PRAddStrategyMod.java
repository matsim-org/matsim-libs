...
public class PRAddStrategyMod implements PlanStrategyModule {
  ...
  public void handlePlan(Plan plan) {
    ...
    if (hasHomeActivity == true && hasWorkActivity == true) {
      ...
      final int rndWork =
          (int) (MatsimRandom.getRandom().nextDouble() * planAnalyzer.getWorkActs().size());
      log.info(
          "PlanElements of possible Work Activities: "
              + planAnalyzer.getWorkActs()
              + ". Creating Park'n'Ride before and after Work Activity "
              + planAnalyzer.getWorkActs().get(rndWork)
              + "...");
      ...
      if (indicesPRact.size() > 2)
        throw new RuntimeException(
            "More than two ParkAndRide Activities in this Home-Work-Home Sequence. Aborting...");

      if (indicesPRact.isEmpty()) {

        log.info(
            "Home-Work-Home sequence doesn't contain Park'n'Ride. Adding Park'n'Ride to this Home-Work-Home sequence...");

        // create ParkAndRideActivity
        planAnalyzer.setIndices();
        Activity parkAndRide =
            createParkAndRideActivity(planAnalyzer, planAnalyzer.getWorkActs().get(rndWork), plan);
        ...
        //						System.out.println("Leg Index to be transformed into P+R (1): " +
        // transformIntoPRLeg1);
        ...
        //						System.out.println("Leg Index to be transformed into P+R (2): " +
        // transformIntoPRLeg2);
        ...
        log.info(
            "Home-Work-Home sequence contains a ParkAndRide Activity. Not modifying the plan...");
      }
