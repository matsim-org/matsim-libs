...
public class PRFactory {
  ...
  public PRFactory(
      MutableScenario scenario, double extensionRadius, int maxSearchSteps, String outputPath) {
  ...
  public void setId2prCarLinkToNode() {
    ...
    if (this.useScheduleFile == true) {
      ...
      Map<Id<TransitStopFacility>, TransitStopFacility> id2transitStopFacility =
          this.scenario.getTransitSchedule().getFacilities();
      this.setId2prCarLinkToNode_fromScheduleFile(id2transitStopFacility);

    } else {
      throw new RuntimeException(
          "Not specified how to create park-and-ride facilities. Aborting...");
    }
  }
      ...
      if (!nodeFound) {
        if (link.getAllowedModes().contains(TransportMode.car)) {
          ...
          if (hyp <= searchRadius) {
            ...
            //						System.out.println("Der Link "+link.getId()+" liegt NICHT im Suchradius um die
            // TransitStopCoordinates "+coord.toString()+".");
          }
        } else {
          // not a carLink
        }
  ...
  private void setId2prCarLinkToNode_fromInputFile(
      Map<Id<PRFacility>, PRInputData> id2prInputData) {
  ...
  private void setId2prCarLinkToNode_fromScheduleFile(
      Map<Id<TransitStopFacility>, TransitStopFacility> id2transitStopFacility) {
      ...
      if (stop.getId() == null || stop.getLinkId() == null) {
        ...
        if (stop.getLinkId() == null) {
          log.info(
              "Transit stop "
                  + name
                  + " has no reference link Id. Using the coordinates of the transit stop...");
          coordinates = stop.getCoord();
        } else {
          log.info(
              "Transit stop "
                  + name
                  + " has a reference link Id. Using the coordinates of the toNode...");
  ...
  public void createPRLinks(PRFacilityCreator prFacilityCreator) {
    if (this.useInputFile == true) {
      ...
      for (Id id : this.id2nextCarLinkToNode.keySet()) {
        ...
        prFacilityCreator.createPRFacility(
            prFacilitiyId,
            this.id2nextCarLinkToNode.get(id),
            this.id2prInputData.get(id).getStopName(),
            this.id2prInputData.get(id).getCapacity());
        i++;
      }
    }
    ...
    if (this.useScheduleFile == true) {
      ...
      for (Id id : this.id2nextCarLinkToNode.keySet()) {
        ...
        prFacilityCreator.createPRFacility(
            prFacilitiyId, this.id2nextCarLinkToNode.get(id), name, constantPRcapacity);
        i++;
      }
    }
  }
