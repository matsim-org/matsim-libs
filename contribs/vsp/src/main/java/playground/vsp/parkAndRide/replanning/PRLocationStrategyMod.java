...
/**
 * This strategy module randomly chooses a home-work-home sequence and replaces the park-and-ride
 * location by a new one.
 *
 * @author ikaddoura
 */
public class PRLocationStrategyMod implements PlanStrategyModule {
  private static final Logger log = LogManager.getLogger(PRLocationStrategyMod.class);

  private MutableScenario sc;
  private Network net;
  private Map<Id<PRFacility>, PRFacility> id2prFacility = new HashMap<>();
  private int nrOfPrFacilitiesForReplanning =
      0; // 0 means all P+R-Facilities are used for replanning
   ...
   */
  public PRLocationStrategyMod(
      MatsimServices controler,
      Map<Id<PRFacility>, PRFacility> id2prFacility,
      double gravity,
  ...
  public void handlePlan(Plan plan) {
    ...
    if (hasHomeActivity == true && hasWorkActivity == true) {
      ...
      final int rndWork =
          (int) (MatsimRandom.getRandom().nextDouble() * planIndices.getWorkActs().size());
      log.info(
          "PlanElements of possible Work Activities: "
              + planIndices.getWorkActs()
              + ". Creating Park'n'Ride before and after Work Activity "
              + planIndices.getWorkActs().get(rndWork)
              + "...");
      ...
      if (prIndicesToChange.size() == 2) {
        ...
        log.info(
            "Home-Work-Home sequence contains two ParkAndRide Activities. Changing the Park'n'Ride Location...");

        // create a new ParkAndRideActivity
        Activity parkAndRide =
            createParkAndRideActivity(planIndices, planIndices.getWorkActs().get(rndWork), plan);
        ...
        log.warn(
            "More than two ParkAndRide Activities in this Home-Work-Home Sequence. This should not be possible. Not modifiying the plan...");
      } else if (prIndicesToChange.isEmpty()) {
        log.info("Home-Work-Home sequence doesn't contain Park'n'Ride. Not modifying the plan...");
      } else if (prIndicesToChange.size() < 2) {
        log.info(
            "Home-Work-Home sequence contains only one ParkAndRide Activity. Not modifying the plan...");
      }
  ...
  private Activity createParkAndRideActivity(
      PlanIndicesAnalyzer planIndices, Integer workIndex, Plan plan) {
    List<PRWeight> prWeights;
    EllipseSearch ellipseSearch = new EllipseSearch();

    Activity firstHomeAct =
        (Activity)
            plan.getPlanElements()
                .get(
                    planIndices
                        .getHomeActs()
                        .get(0)); // assuming that if an agent has more than one home activity, it
    // always have the same coordinates...
    Activity workAct = (Activity) plan.getPlanElements().get(workIndex);
    log.info(
        "Create Park'n'Ride Activity for planElements (homeIndex: "
            + planIndices.getHomeActs().get(0)
            + " / workIndex: "
            + workIndex
            + "): "
            + firstHomeAct.getCoord()
            + " / "
            + workAct.getCoord());
    log.info("Calculating Weights...");
    prWeights =
        ellipseSearch.getPrWeights(
            this.nrOfPrFacilitiesForReplanning,
            this.net,
            this.id2prFacility,
            firstHomeAct.getCoord(),
            workAct.getCoord(),
    ...
    Activity parkAndRide =
        PopulationUtils.createActivityFromCoordAndLinkId(
            PRConstants.PARKANDRIDE_ACTIVITY_TYPE,
            rndPrLink.getToNode().getCoord(),
            rndPrLink.getId());
