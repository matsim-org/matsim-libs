...
/**
 * An activity scoring function which extends the ordinary CharyparNagelActivityScoring. For each
 * park-and-ride activity the intermodal transfer penalty is added to the score of a plan.
 *
 * @author ikaddoura
 */
public class PRActivityScoringFunction
  ...
  protected double calcActScore(double arrivalTime, double departureTime, Activity act) {
    if (act.getType().equals(PtConstants.TRANSIT_ACTIVITY_TYPE)) {
      ...
      log.info(
          "Adding the intermodal transfer penalty to the plan score: "
