...
public class PtAccessMapShapeWriter {
  ...
  public static void writeAccessMap(
      Map<String, Map<String, MultiPolygon>> cluster2mode2area,
    ...
    HashMap<Integer, HashMap<String, Geometry>> distance2mode2buffer =
        new HashMap<Integer, HashMap<String, Geometry>>();
    ...
    for (Integer distance : distancesSmallestFirst) {
      if (mode2buffer == null) {
        ...
        for (Entry<String, MultiPolygon> multipolygonEntry :
            cluster2mode2area.get(distancesSmallestFirst.get(0).toString()).entrySet()) {
          mode2buffer.put(
              multipolygonEntry.getKey(),
              multipolygonEntry.getValue().buffer(0.0, quadrantSegments));
        ...
        for (Entry<String, Geometry> bufferEntry : mode2buffer.entrySet()) {
          tempBuffers.put(
              bufferEntry.getKey(),
              bufferEntry.getValue().buffer(distance.intValue() - lastDistance));
        }
    ...
    writeGeometries(
        outputFolder + PtStopMap.FILESUFFIX + "_buffer",
        distance2mode2buffer,
    ...
    HashMap<Integer, HashMap<String, Geometry>> distance2mode2diffBuffer =
        new HashMap<Integer, HashMap<String, Geometry>>();
    ...
    for (Integer distance : distancesLargestFirst) {
      ...
      if (lastMode2Buffer == null) {
        ...
        for (String mode : distance2mode2buffer.get(distance).keySet()) {
          Geometry diffBuffer =
              lastMode2Buffer.get(mode).difference(distance2mode2buffer.get(distance).get(mode));
    ...
    for (Entry<String, Geometry> mode2BufferEntry : lastMode2Buffer.entrySet()) {
      distance2mode2diffBuffer
          .get(lastDist)
          .put(mode2BufferEntry.getKey(), mode2BufferEntry.getValue());
    }

    writeGeometries(
        outputFolder + PtStopMap.FILESUFFIX + "_diffBuffer",
        distance2mode2diffBuffer,
        targetCoordinateSystem);
  }

  private static void writeGeometries(
      String outputFolderAndFileName,
      HashMap<Integer, HashMap<String, Geometry>> distance2mode2buffer,
      String targetCoordinateSystem) {
    // write all to file
    ...
    for (Entry<Integer, HashMap<String, Geometry>> distance2mode2bufferEntry :
        distance2mode2buffer.entrySet()) {
      ...
      try {
        ShapeFileWriter.writeGeometries(
            bufferFeatures,
            outputFolderAndFileName + "_" + distance2mode2bufferEntry.getKey() + ".shp");
