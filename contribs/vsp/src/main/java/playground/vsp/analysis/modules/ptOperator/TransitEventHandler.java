 ...
 * @author ikaddoura
 */
public class TransitEventHandler
  ...
  public void handleEvent(LinkLeaveEvent event) {
    ...
    if (this.ptDriverIdAnalyzer.isPtDriver(personId)) {
      try {
        this.vehicleKm =
            this.vehicleKm + network.getLinks().get(event.getLinkId()).getLength() / 1000;
  ...
  public double getVehicleHours() {
    ...
    for (Id id : this.personID2firstDepartureTime.keySet()) {
      vehicleSeconds =
          vehicleSeconds
              + ((this.personID2lastArrivalTime.get(id)
                  - this.personID2firstDepartureTime.get(id)));
    }
