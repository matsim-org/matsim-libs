 ...
 * @author droeder
 */
public class TransitVehicleVolumeHandler
    implements TransitDriverStartsEventHandler, LinkEnterEventHandler {

  @SuppressWarnings("unused")
  private static final Logger log = LogManager.getLogger(TransitVehicleVolumeHandler.class);
  ...
  public void handleEvent(LinkEnterEvent event) {
    ...
    if (this.vehId2mode.containsKey(event.getVehicleId())) {
      ...
      Count count =
          this.mode2Counts
              .get(vehId2mode.get(event.getVehicleId()))
              .createAndAddCount(event.getLinkId(), event.getLinkId().toString());
      if (count == null) {
        // or get the old one if there is one
        count =
            this.mode2Counts.get(vehId2mode.get(event.getVehicleId())).getCount(event.getLinkId());
      } else {
