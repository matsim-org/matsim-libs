...
/**
 * Counts number of vehicles, number of passengers, and capacity of transit vehicles per link. In
 * addition provides Paxkm and capacitykm per link.
 *
 * @authors aneumann
 */
public class TaxiTravelStatsHandler
  ...
  public void handleEvent(LinkEnterEvent event) {
    ...
    if (this.taxiId2CurrentPaxCountMap.containsKey(event.getVehicleId().toString())) {
      ...
      Count<Link> countVehicles =
          this.countsVehicles.createAndAddCount(event.getLinkId(), event.getLinkId().toString());
      Count<Link> countCapacity =
          this.countsCapacity.createAndAddCount(event.getLinkId(), event.getLinkId().toString());
      Count<Link> countCapacity_m =
          this.countsCapacity_m.createAndAddCount(event.getLinkId(), event.getLinkId().toString());
      Count<Link> countPax =
          this.countsPax.createAndAddCount(event.getLinkId(), event.getLinkId().toString());
      Count<Link> countPax_m =
          this.countsPax_m.createAndAddCount(event.getLinkId(), event.getLinkId().toString());
      ...
      double vehCapacity_m =
          vehCapacity * this.scenario.getNetwork().getLinks().get(event.getLinkId()).getLength();
  ...
  public void handleEvent(PersonEntersVehicleEvent event) {
    ...
    if (this.taxiId2CurrentPaxCountMap.keySet().contains(event.getVehicleId().toString())) {
      ...
      if (!event.getVehicleId().toString().equalsIgnoreCase(event.getPersonId().toString())) {
        ...
        this.taxiId2CurrentPaxCountMap.put(
            event.getVehicleId().toString(),
            this.taxiId2CurrentPaxCountMap.get(event.getVehicleId().toString()) + 1);
      }
    }
  }
