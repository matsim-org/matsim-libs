...
public class CreateRscript {
  ...
  public static void main(String[] args) {
    ...
    createScript(
        sc.getTransitSchedule().getTransitLines(), dir + "PtRoutes2PaxAnalysis\\", 60 * 60.0, 24);
  }
  ...
  public static void createScript(
      Map<Id<TransitLine>, TransitLine> lines, String outDir, double interval, int maxSlices) {
  ...
  public static void createScriptFromTransitLines2PaxCounts(
      Map<Id, TransitLines2PaxCounts> tl2c, String outDir, double interval, int maxSlices) {
    readScript(outDir, interval, maxSlices);
    heatmapScript(outDir, interval, maxSlices);
    /* b for the plotAll.bat, b2 added by gleich for the alternative plotAll.R */
    StringBuffer b = new StringBuffer();
  ...
  private static void createRouteScript(Id lineId, Id routeId, String outDir) {
    BufferedWriter w =
        IOUtils.getBufferedWriter(outDir + lineId.toString() + "--" + routeId.toString() + ".R");
    try {
      w.write("rm(list = ls(all = TRUE))\n");
      w.write("library(gplots)\n");
      w.write("pdf(\"" + lineId.toString() + "--" + routeId.toString() + ".pdf\")\n\n");
      w.write(
          readAndPlotSingleFile(outDir, lineId.toString() + "--" + routeId.toString(), "boarding"));
      w.write(
          readAndPlotSingleFile(
              outDir, lineId.toString() + "--" + routeId.toString(), "alighting"));
      w.write(
          readAndPlotSingleFile(outDir, lineId.toString() + "--" + routeId.toString(), "totalPax"));
      w.write(
          readAndPlotSingleFile(
              outDir, lineId.toString() + "--" + routeId.toString(), "occupancy"));
      w.write(
          readAndPlotSingleFile(outDir, lineId.toString() + "--" + routeId.toString(), "capacity"));
      w.write("dev.off()");
      w.flush();
      w.close();
    } catch (IOException e) {
    ...
    try {
      w.write(
          "data <- read.table(file, sep=\";\", header=T, na.strings=\"--\", fileEncoding=\"UTF8\")\n"
              +
              // maxSlices + 1 to cater for measurements after the last slice (time >
              // maxSlices*interval)
              "values <- array(0, dim=c(length(data$index),"
              + (maxSlices + 1)
              + "))\n");
  ...
  private static void heatmapScript(String outdir, double interval, int maxSlices) {
    ...
    for (int i = (int) interval; i < (maxSlices * (int) interval) + 1; i += (int) interval) {
      labCol.append(
          "\""
              + Time.writeTime(i - (int) interval, Time.TIMEFORMAT_HHMM)
              + "-"
              + Time.writeTime(i, Time.TIMEFORMAT_HHMM)
              + "\",\n\t\t");
    }
    labCol.append(
        "\"> " + Time.writeTime(maxSlices * (int) interval, Time.TIMEFORMAT_HHMM) + "\")");
    try {
      w.write(
          "library(RColorBrewer)  \n"
              ...
              + "sepcolor=\"white\",\n\t\t"
              + "tracecol=\"white\", \n\t\t"
              + "sepwidth=c(1/length(data$index),1/"
              + (maxSlices + 1)
              + "),\n\t\t"
              + "dendrogram=\"none\",\n\t\t"
              + "Colv=\"none\",\n\t\t"
