...
public class PtLines2PaxAnalysis extends AbstractAnalysisModule {
  ...
  private static final Logger log = LogManager.getLogger(PtLines2PaxAnalysis.class);
  ...
  public void writeResults(String outputFolder) {
    ...
    CreateRscript.createScriptFromTransitLines2PaxCounts(
        this.handler.getLinesPaxCounts(), dir, interval, maxSlices);
  }

  private void writeCounts2File(
      List<TransitRouteStop> transitRouteStops,
      int maxSlice,
      Counts counts,
    ...
    try {
      ...
      for (int noStops = 0; noStops < transitRouteStops.size(); noStops++) {
        ...
        w.write(
            String.valueOf(noStops)
                + ";"
                + count.getCsLabel()
                + ";"
                + transitRouteStops.get(noStops).getStopFacility().getName());
  ...
  private void writeLineFiles(String dir, TransitLines2PaxCounts tl2c) {
    for (int i = 0; i < tl2c.getRouteList().size(); i++) {
      ...
      String filePrefix =
          dir + tl2c.getId() + this.SEP + tl2c.getRouteList().get(i).getId().toString();
      writeCounts2File(
          transitRouteStops,
          maxSlice,
          tl2c.getAlighting(),
          filePrefix + this.SEP + "alighting.csv");
      writeCounts2File(
          transitRouteStops, maxSlice, tl2c.getBoarding(), filePrefix + this.SEP + "boarding.csv");
      writeCounts2File(
          transitRouteStops, maxSlice, tl2c.getCapacity(), filePrefix + this.SEP + "capacity.csv");
      writeCounts2File(
          transitRouteStops,
          maxSlice,
          tl2c.getOccupancy(),
          filePrefix + this.SEP + "occupancy.csv");
      writeCounts2File(
          transitRouteStops, maxSlice, tl2c.getTotalPax(), filePrefix + this.SEP + "totalPax.csv");
    }
  }
  ...
  public static void main(String[] args) {
    ...
    //		new TransitScheduleReader(sc).readFile(dir + "tut_10min_0.0.transitSchedule_1.xml");	//for
    // testing
    new MatsimVehicleReader(((MutableScenario) sc).getTransitVehicles())
        .readFile(dir + "r5-w1000.output_transitVehicles.xml.gz");
    PtLines2PaxAnalysis ptLinesPax =
        new PtLines2PaxAnalysis(
