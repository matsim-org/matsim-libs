...
public class TransitSchedule2Shp extends AbstractAnalysisModule {
  ...
  public void writeResults(String outputFolder) {
    ...
    for (TransitLine transitLine : this.schedule.getTransitLines().values()) {
      if (transitLine.getRoutes().isEmpty()) {
        log.warn(
            "can not create a shapefile for transitline "
                + transitLine.getId()
                + ", because the line contains no routes...");
        continue;
      }
      Collection<SimpleFeature> temp =
          getTransitLineFeatures(transitLine, this.targetCoordinateSystem);
      features.addAll(temp);
      try {
        ShapeFileWriter.writeGeometries(
            temp, outputFolder + transitLine.getId().toString() + ".shp");
    ...
    if (features.isEmpty()) {
      log.error(
          "the transitschedule seems to be empty. No features are created, thus no shapefile will be written...");
      return;
    }
    try {
  ...
  private Collection<SimpleFeature> getTransitLineFeatures(
      TransitLine transitLine, String targetCoordinateSystem) {
    ...
    for (TransitRoute transitRoute : transitLine.getRoutes().values()) {
      routeFeatureAttributes =
          getRouteFeatureAttribs(transitRoute, transitLine.getId(), new Object[17]);
      try {
  ...
  private Object[] getRouteFeatureAttribs(
      TransitRoute transitRoute, Id<TransitLine> lineId, Object[] routeFeatureAttributes) {
    ...
    Coord toNode =
        this.network
            .getLinks()
            .get(transitRoute.getRoute().getStartLinkId())
            .getToNode()
            .getCoord();
    ...
    // add the endlink
    toNode =
        this.network.getLinks().get(transitRoute.getRoute().getEndLinkId()).getToNode().getCoord();
    coords.add(new Coordinate(toNode.getX(), toNode.getY(), 0.));
    ...
    LineString lineString =
        new GeometryFactory().createLineString(new CoordinateArraySequence(coord));
    ...
    routeFeatureAttributes[8] =
        Time.writeTime(transitRouteData.getTravelTime(), Time.TIMEFORMAT_HHMMSS);
    routeFeatureAttributes[9] = transitRouteData.getNVehicles();
    routeFeatureAttributes[10] =
        Time.writeTime(transitRouteData.getHeadway(), Time.TIMEFORMAT_HHMMSS);
    ...
    routeFeatureAttributes[14] =
        Time.writeTime(transitRouteData.getFirstDeparture(), Time.TIMEFORMAT_HHMMSS);
    routeFeatureAttributes[15] =
        Time.writeTime(transitRouteData.getLastDeparture(), Time.TIMEFORMAT_HHMMSS);
    routeFeatureAttributes[16] =
        Time.writeTime(transitRouteData.getOperatingDuration(), Time.TIMEFORMAT_HHMMSS);
