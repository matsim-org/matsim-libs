...
public class TransitLines2PaxCounts {
  ...
  private static final Logger log = LogManager.getLogger(TransitLines2PaxCounts.class);
  ...
  private void createRouteSegments() {
    ...
    for (int ii = 0; ii < numberOfRoutes; ii++) {
      ...
      for (int jj = numberOfRoutes - 1; jj >= 0; jj--) {
        ...
        // tr noch nicht abgearbeitet ist und tr2 noch nicht als Duplikat einer anderen Route
        // erkannt wurde
        if (!comparedRoutes.contains(tr2.getId())
            && !dupeRoutes.contains(tr2)
            && !tr.getId().equals(tr2.getId())) {
          if (tr2Length > trLength) {
            log.error(
                "TransitRoute tr2 to be compared is longer than base TransitRoute tr, this should not happen. \n "
                    + "tr ID: "
                    + tr.getId()
                    + " with length: "
                    + trLength
          ...
          // Unterscheidung nötig: Routensegment am Anfang, Routensegment am Ende, Routensegment in
          // der Mitte
          ...
          if (tr.getStops()
              ...
              .getStopFacility()
              .getId()
              .equals(tr2.getStops().get(0).getStopFacility().getId())) {
            ...
            for (int rr = 0; rr < tr2Length; rr++) {
              ...
              if (tr.getStops()
                  ...
                  .getStopFacility()
                  .getId()
                  .equals(tr2.getStops().get(rr).getStopFacility().getId())) {
              ...
              //
                ...
                if (rr == tr2Length - 1) {
                  Id<TransitRoute> fragmentId =
                      Id.create(
                          tr2.getId().toString() + "_" + routeFragments.size(), TransitRoute.class);
                  TransitRoute frag =
                      this.tsf.createTransitRoute(
                          fragmentId,
                          tr2.getRoute(),
                          routeFragmentStopList,
                          tr2.getTransportMode());
          ...
          // letzter Stop identisch
              ...
              .getStopFacility()
              .getId()
              .equals(tr2.getStops().get(tr2Length - 1).getStopFacility().getId())) {
            // siehe oben, rückwärts
            for (int rr = tr2Length - 1; rr >= 0; rr--) {
              if (!createFragment
                  && tr.getStops()
                      .get(trLength - 1 - (tr2Length - 1 - rr))
                      .getStopFacility()
                      .getId()
                      .equals(tr2.getStops().get(rr).getStopFacility().getId())) {
          ...
          // weder erster noch letzter identisch
            ...
            for (int rr = 0; rr < trLength; rr++) {
              if (tr2.getStops()
                  ...
                  .getStopFacility()
                  .getId()
                  .equals(tr.getStops().get(rr).getStopFacility().getId())) {
                for (int ss = 0; ss < tr2Length; ss++) {
                  if ((rr + ss) <= trLength - 1) {
                    if (tr2.getStops()
                        ...
                        .getStopFacility()
                        .getId()
                        .equals(tr.getStops().get(rr + ss).getStopFacility().getId())) {
                      if (ss == tr2Length - 1) {
                        if (!dupeRoutes.contains(tr2)) dupeRoutes.add(tr2);
                      }
                    }
            ...
            log.info(
                "Does not fit into current route scheme: "
                    + "tr ID: "
                    + tr.getId()
                    + " with length: "
                    + trLength
  ...
  //	this might not be the best way of removing duplicate routes, it seems to work properly
  // nontheless
    ...
    for (Integer ns : stops2routes.keySet()) {
      for (int jj = this.routeList.size(); jj > 1; jj--) {
        ...
        for (int kk = 0; kk < stops2routes.get(ns).size(); kk++) {
          ...
          if ((!dupeRoutes.contains(tr))
              && (tr.getId() != tr2.getId())
              && (tr2.getStops().containsAll(tr.getStops()))) {
            dupeRoutes.add(tr);
          }
        }
      }
  ...
  public void vehicleDeparts(
    ...
    set(
        ...
        stopFacilityId,
        time,
        this.totalPax.getCount(convertedId).getVolume(slice).getValue()
            / this.capacity.getCount(convertedId).getVolume(slice).getValue());
  }

  private void increase(
      Counts counts, Id<TransitStopFacility> stopFacilityId, Double time, double increaseBy) {
  ...
  private void set(
      Counts counts, Id<TransitStopFacility> stopFacilityId, Double time, double value) {
    Count count = counts.getCount(Id.create(stopFacilityId, Link.class));
    Integer slice = getTimeSlice(time);
    Volume v;
    if (count.getVolumes().containsKey(slice)) {
