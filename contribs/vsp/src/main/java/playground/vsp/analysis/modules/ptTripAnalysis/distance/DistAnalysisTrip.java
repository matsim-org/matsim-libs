...
public class DistAnalysisTrip extends AbstractAnalysisTrip implements DistAnalysisTripI {
  ...
  private void analyzeElements(ArrayList<PlanElement> elements) {
    ...
    if (!(((Activity) elements.get(0)).getCoord() == null)
        && !(((Activity) elements.get(elements.size() - 1)).getCoord() == null)) {
      super.setStart(
          new Coordinate(
              ((Activity) elements.get(0)).getCoord().getX(),
              ((Activity) elements.get(0)).getCoord().getY()));
      super.setEnd(
          new Coordinate(
              ((Activity) elements.get(elements.size() - 1)).getCoord().getX(),
  ...
  public void processAgentEvent(Event e) {
    ...
    if (e instanceof PersonDepartureEvent) {
      ...
      if (!((PersonArrivalEvent) e).getLegMode().equals(TransportMode.pt)
          && !((PersonArrivalEvent) e).getLegMode().equals(TransportMode.car)) {
  ...
  private void finish() {
    if (super.getMode().equals(TransportMode.pt)) {
      if (this.legStartEnd.size() == 2) {
        ...
        this.egressWalkCnt++;
        this.accesWalkDist =
            GeoCalculator.distanceBetween2Points(
                this.legStartEnd.get(0).getFirst(), this.legStartEnd.get(0).getSecond());
        this.egressWalkDist =
            GeoCalculator.distanceBetween2Points(
                this.legStartEnd.get(1).getFirst(), this.legStartEnd.get(1).getSecond());
        //				System.out.println(this.accesWalkCnt + " " + this.accesWalkDist + "; " +
        // this.egressWalkCnt + " " + this.egressWalkDist);
        ...
        this.egressWalkDist =
            GeoCalculator.distanceBetween2Points(
                this.legStartEnd.get(this.legStartEnd.size() - 1).getFirst(),
                this.legStartEnd.get(this.legStartEnd.size() - 1).getSecond());

        for (int i = 1; i < this.legStartEnd.size() - 1; i++) {
          this.switchWalkCnt++;
          this.switchWalkDist +=
              GeoCalculator.distanceBetween2Points(
                  this.legStartEnd.get(i).getFirst(), this.legStartEnd.get(i).getSecond());
        }
        //				System.out.println(this.accesWalkCnt + " " + this.accesWalkDist + "; " +
        // this.egressWalkCnt + " " + this.egressWalkDist + "; " + this.switchWalkCnt + " " +
        // this.switchWalkDist);
      } else {
        log.error("a pt-trip need's at least an acces and an egressWalk!");
      }
      this.tripDist =
          this.accesWalkDist + this.inPtDist + this.switchWalkDist + this.egressWalkDist;
