...
public class UserBenefitsCalculator {
  ...
  private final Map<Id<Person>, Double> personId2MonetizedUtility =
  ...
  public UserBenefitsCalculator(Config config, WelfareMeasure wm, boolean considerAllPlans) {
    ...
    logger.warn(
        "There is no list of person IDs provided to be excluded from the user benefit calculation.");
    this.stuckingAgents = null;

    if (considerAllPlans) {
      logger.warn(
          "All plans are considered for the calculation of user benefits. For an economic interpretation invalid plans (score <= 0.0 or score == null) should not be considered.");
    } else {
      logger.warn(
          "Invalid plans that will not be considered for the calculation of user benefits are defined to have either a null score or a negative score.");
    }
  }

  public UserBenefitsCalculator(Config config, Set<Id<Person>> stuckingAgents) {
    logger.info(
        "Providing the IDs of agents that are stucking in the final iteration (selected plans).");
    ...
    logger.info(
        "A list of person IDs of stucking agents is provided. These persons will be excluded from the user benefit calculation."
        ...
        if (personsWithoutValidPlanScore <= maxWarnCnt) {
          logger.warn(
              "Person "
                  + person.getId()
                  + " has no valid plans. "
                  + "This person's utility is "
                  + utilityOfPerson_utils);
        ...
        if (this.stuckingAgents.contains(personId)) {
          ...
          if (minusScore <= maxWarnCnt) {
            logger.warn(
                "Person "
                    + personId
                    + " is stucking and therefore cannot be used for utility calculation.");
            if (minusScore == maxWarnCnt) logger.warn(Gbl.FUTURE_SUPPRESSED + "\n");
          }
  ...
  public int getPersonsWithoutValidPlanCnt() {
    if (this.considerAllPlans) {
      logger.warn(
          "All plans are forced to be considered. Asking for the number of persons without a valid plan doesn't make sense.");
  ...
  public int getInvalidPlans() {
    if (this.considerAllPlans) {
      logger.warn(
          "All plans are forced to be considered. Asking for the number of invalid plans doesn't make sense.");
      return Integer.MIN_VALUE;
    } else {
