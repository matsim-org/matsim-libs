...
/**
 * This module calculates the total car distance for each person as well as the avg. distance per
 * car user and the avg. distance per trip.
 *
 * @author ikaddoura
 */
public class CarDistanceAnalyzer extends AbstractAnalysisModule {
  private static final Logger log = LogManager.getLogger(CarDistanceAnalyzer.class);
  ...
  public void init(MutableScenario scenario) {
    ...
    this.carDistanceEventHandler =
        new CarDistanceEventHandler(scenario.getNetwork(), this.ptDriverIdAnalyzer);
  }
