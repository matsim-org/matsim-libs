...
/**
 * If someone starts with transit_walk leg, and do not use pt before starting a regular act
 * (home/work/leis/shop); it is walk else it is pt mode.
 *
 * @author amit
 */
public class ModalShareEventHandler
  ...
  // agents who first departs with transitWalk and their subsequent modes are stored here until it
  // starts a regular act (home/work/leis/shop)
  ...
  public void handleEvent(PersonDepartureEvent event) {
    ...
    // at this point, it could be main leg (e.g. car/bike) or start of a stage activity (e.g. car/pt
    // interaction)
  ...
  public void handleEvent(PersonStuckEvent event) {
    if (person2Modes.containsKey(event.getPersonId())) {
      // since mode for transit users is determined at activity start, so storing mode for stuck
      // agents so that, these can be handeled later.
  ...
  private void handleRemainingTransitUsers() {
    LogManager.getLogger(ModalShareEventHandler.class)
        .warn("A few transit users are not handle due to stuckAndAbort. Handling them now.");
  ...
  private String getMainMode(List<String> modes) {
    if (modes.size() == 1)
      return modes.get(0).equals(TransportMode.transit_walk) ? TransportMode.walk : modes.get(0);
    ...
    if (modes.contains(TransportMode.transit_walk)
        || modes.contains(TransportMode.access_walk)
        || modes.contains(TransportMode.egress_walk)) {
