...
/**
 * This class creates a bounding polygon around a given network. The method starts with an initial
 * bounding box. Coordinates are placed in a certain step size on the edge of the box. Then, the
 * nearest node of each coordinate is searched to get the corners of the resulting polygon.
 *
 * @author dhosse
 */
public class BoundingPolygon {
  ...
  public Geometry returnPolygon() {
    ...
    for (int i = 0; i < ring.length; i++) ring[i] = this.coordinates.get(i);
  ...
  private void createPolygon() {
    ...
    if (this.network != null && this.stepSize > 0) {
      ...
      for (Coordinate coord : coordinates) {
        ...
        Node node =
            NetworkUtils.getNearestNode(((Network) this.network), MGC.coordinate2Coord(coord));
