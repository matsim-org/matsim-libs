...
public class TransitRouteContainer {
  ...
  private static final Logger log = LogManager.getLogger(TransitRouteContainer.class);
  ...
  /**
   * The number of Pax boarding/alighting at different stopFacilities (served by the route) is
   * counted per interval/timeslice. Furthermore the number of "inVehiclePeople" when vehicles
   * depart and the capacity of departing vehicles is evaluated.
   *
   * <p>Assuming the countsinterval is 3,600s
   *
   * @param r
   * @param countsInterval, the length of an interval/timeslice ins second
   * @param maxSlice, the maximum number of timeslices. In the last timeslice all numbers of later
   *     e.g. boardings are summed up...
   */
  ...
  public void vehicleDeparts(
      double time, double vehCapacity, double nrSeatsInUse, Id<Link> stopIndexId) {
    ...
    set(
        ...
        stopIndexId,
        time,
        this.totalPax.getCount(stopIndexId).getVolume(slice).getValue()
  ...
  private void create(
      Counts counts,
      Id<Link> stopIndexId,
      Id<TransitStopFacility> stationName,
      Double time,
      double value) {
    // create a new count
