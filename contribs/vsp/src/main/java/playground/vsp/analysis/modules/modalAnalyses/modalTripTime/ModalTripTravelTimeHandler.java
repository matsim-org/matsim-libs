...
/**
 * (1) Handles departure and arrival events and store total travel time of a person and travel time
 * of each trip of a person segregated by leg mode (2) See followings transit_walk - transit_walk
 * --> walk & transit_walk - pt - transit_walk --> pt & transit_walk - pt - transit_walk - pt -
 * transit_walk --> pt
 *
 * @author amit
 */
public class ModalTripTravelTimeHandler
  ...
  public void handleEvent(PersonStuckEvent event) {
    ...
    if (this.warnCount <= MAX_STUCK_AND_ABORT_WARNINGS) {
      LOGGER.warn(
          "'StuckAndAbort' event is thrown for person "
              + event.getPersonId()
              + " on link "
              + event.getLinkId()
              + " at time "
  ...
  private void storeData(final Id<Person> personId, final String legMode, final double travelTime) {
    Map<Id<Person>, List<Double>> personId2TravelTimes =
        this.mode2PersonId2TravelTimes.getOrDefault(legMode, new HashMap<>());
