...
public class HeatMap {
  ...
  public void createHeatMap() {
    ...
    for (Tuple<Coord, Double> v : this.values) {
      ...
      Point p =
          new GeometryFactory()
              .createPoint(new Coordinate(v.getFirst().getX(), v.getFirst().getY(), 0.));
      if (!t.getGeometry().contains(p)) {
        // should never happen
        log.warn(
            "check the created heatMap. A point will be added to a tile, but is not part of it!");
  ...
  public static void writeHeatMapShape(
      String name, HeatMap heatmap, String file, String targetCoordinateSystem) {
