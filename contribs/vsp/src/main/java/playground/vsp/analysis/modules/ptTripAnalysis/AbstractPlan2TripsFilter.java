...
public abstract class AbstractPlan2TripsFilter extends AbstractPersonFilter {
  ...
  protected Map<Id, LinkedList<AbstractAnalysisTrip>> id2Trips =
      new HashMap<Id, LinkedList<AbstractAnalysisTrip>>();
  ...
  public boolean judge(Person p) {
    ...
    for (PlanElement pe : p.getSelectedPlan().getPlanElements()) {
      ...
      if (pe instanceof Leg) {
        ...
        // an activity which is not the first and not a pt interaction is the last activity of a
        // trip
        else {
          temp.add(pe);
          ...
          list.add(trip);
          // the following Trip starts with the same activity. If it was the last activity, temp
          // will not be stored
  ...
  public Map<Id, LinkedList<AbstractAnalysisTrip>> getTrips() {
    if (this.unprocessedAgents.size() > 0) {
      log.warn(
          this.unprocessedAgents.size()
              + " agents not processed, because they have a plan consisting of one or less elements! call getUnproccessedAgents() to get their Id's!");
    }
    return this.id2Trips;
  }

  /**
   * return a <code>String</code> of Id's of all Agents which are not processed, because they have
   * only one activity
   *
   * @return
   */
  public String getUnprocessedAgents() {
    ...
    b.append(
        "following Agents are not processed, because their plan consists only of one Element\n");
