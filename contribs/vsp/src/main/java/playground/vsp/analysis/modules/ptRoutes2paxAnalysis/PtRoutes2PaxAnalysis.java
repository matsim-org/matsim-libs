...
public class PtRoutes2PaxAnalysis extends AbstractAnalysisModule {
  ...
  public PtRoutes2PaxAnalysis(
      Map<Id<TransitLine>, TransitLine> lines, Vehicles vehicles, double interval, int maxSlices) {
  ...
  private void writeRouteFiles(String dir, TransitRouteContainer rc, TransitRoute transitRoute) {
    writeCounts2File(
        ...
        rc.getMaxSlice(),
        rc.getAlighting(),
        dir + rc.getId().toString() + "--alighting.csv");
    writeCounts2File(
        transitRoute,
        rc.getMaxSlice(),
        rc.getBoarding(),
        dir + rc.getId().toString() + "--boarding.csv");
    writeCounts2File(
        transitRoute,
        rc.getMaxSlice(),
        rc.getCapacity(),
        dir + rc.getId().toString() + "--capacity.csv");
    writeCounts2File(
        transitRoute,
        rc.getMaxSlice(),
        rc.getOccupancy(),
        dir + rc.getId().toString() + "--occupancy.csv");
    writeCounts2File(
        transitRoute,
        rc.getMaxSlice(),
        rc.getTotalPax(),
        dir + rc.getId().toString() + "--totalPax.csv");
  }
   ...
   * @param string
   */
  private void writeCounts2File(
      TransitRoute transitRoute, Integer maxSlice, Counts counts, String file) {
    ...
    try {
      ...
      for (int i = 0; i < maxSlices; i++) {
        ...
        String beginString =
            "H" + rFormat.format(begin / (60 * 60)) + "M" + rFormat.format((begin / 60) % 60);
        int end = (int) ((i + 1) * interval);
        // Interval end
        String endString =
            "H" + rFormat.format(end / (60 * 60)) + "M" + rFormat.format((end / 60) % 60);
      ...
      for (int i = 0; i < transitRoute.getStops().size(); i++) {
        ...
        w.write(
            String.valueOf(i)
                ...
                + c.getCsLabel()
                + ";"
                + transitRoute.getStops().get(i).getStopFacility().getName());
  ...
  public static void main(String[] args) {
    ...
    VspAnalyzer analyzer =
        new VspAnalyzer(dir, dir + "ITERS\\it.300\\testReRoute3Old.300.events.xml.gz");

    Scenario sc = ScenarioUtils.createScenario(ConfigUtils.createConfig());
    sc.getConfig().transit().setUseTransit(true);
    new TransitScheduleReader(sc)
        .readFile(dir + "ITERS\\it.299\\testReRoute3Old.299.transitSchedule.xml.gz");
    new MatsimVehicleReader(((MutableScenario) sc).getTransitVehicles())
        .readFile(dir + "ITERS\\it.299\\testReRoute3Old.299.vehicles.xml.gz");
    PtRoutes2PaxAnalysis ptRoutesPax =
        new PtRoutes2PaxAnalysis(
            sc.getTransitSchedule().getTransitLines(),
            ((MutableScenario) sc).getTransitVehicles(),
            3600,
            24);
