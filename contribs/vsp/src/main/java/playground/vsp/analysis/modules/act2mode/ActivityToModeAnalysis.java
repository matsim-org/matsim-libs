...
/**
 * @author droeder
 *     <p>writeResultsPlanCoords() added by gleich
 */
public class ActivityToModeAnalysis extends AbstractAnalysisModule {
  ...
  /**
   * a class to create shapefiles per timeSlot for activities mapped to the first mainmode
   * after/before the activity
   *
   ...
   *     <p>output using Activity Coordinates from the plans instead of departure and arrival links
   *     with writeResultsPlanCoords()
   */
  public ActivityToModeAnalysis(
      Scenario sc, Set<Id> personsOfInterest, int slotSize, String targetCoordinateSystem) {
   ...
   * @param featureType
   */
  private void createFeatureAndAdd(
      ActivityToMode atm,
      HashMap<Integer, Set<SimpleFeature>> slotFeatures,
      PointFeatureFactory factory) {
    ...
    featureAttribs[0] =
        new GeometryFactory()
            .createPoint(new Coordinate(atm.getCoord().getX(), atm.getCoord().getY(), 0.));
    featureAttribs[1] = atm.getActType();
    featureAttribs[2] = atm.getMode();
    SimpleFeature f =
        factory.createPoint(atm.getCoord(), new Object[] {atm.getActType(), atm.getMode()}, null);
    temp.add(f);
  }
  ...
  public void writeResults(String outputFolder) {
    for (Entry<Integer, Set<SimpleFeature>> e : this.departureSlotFeatures.entrySet()) {
      ShapeFileWriter.writeGeometries(
          e.getValue(), outputFolder + "departure_" + e.getKey().toString() + ".shp");
    }
    ...
    for (Entry<Integer, Set<SimpleFeature>> e : this.arrivalSlotFeatures.entrySet()) {
      ShapeFileWriter.writeGeometries(
          e.getValue(), outputFolder + "arrival_" + e.getKey().toString() + ".shp");
    }
  }
