...
/**
 * Counts number of vehicles, number of passengers, and capacity of transit vehicles per mode and
 * link. In addition provides Paxkm and capacitykm per mode and link.
 *
 * @authors aneumann, fuerbas, droeder
 */
public class TravelStatsHandler
    implements LinkEnterEventHandler,
        TransitDriverStartsEventHandler,
        PersonEntersVehicleEventHandler,
        PersonLeavesVehicleEventHandler {

  private static final Logger log = LogManager.getLogger(TravelStatsHandler.class);
  ...
  public void handleEvent(TransitDriverStartsEvent event) {
    ...
    if (!this.scenario
        .getTransitSchedule()
        .getTransitLines()
        .containsKey(event.getTransitLineId())) {
      log.debug(
          "The transit line "
              + event.getTransitLineId()
              + " does not exist in the transit schedule of the provided scenario.");
      return;
    }

    TransitLine line =
        this.scenario.getTransitSchedule().getTransitLines().get(event.getTransitLineId());
    if (line == null) {
    ...
    if (route == null) {
      log.debug(
          "The route "
              ...
              + " does not exist for transit line "
              + event.getTransitLineId());
      return;
    }
  ...
  public void handleEvent(PersonEntersVehicleEvent event) {
    ...
    if (this.transitVehicleId2CurrentPaxCountMap.keySet().contains(event.getVehicleId())) {
      if (!this.transitVehicleDriverIds.contains(event.getPersonId())) {
        ...
        this.transitVehicleId2CurrentPaxCountMap.put(
            event.getVehicleId(),
            this.transitVehicleId2CurrentPaxCountMap.get(event.getVehicleId()) + 1);
      }
    }
  }
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    // subtract a passenger to the vehicle counts data, but ignore every non pt-vehicle and every
    // driver
    if (this.transitVehicleId2CurrentPaxCountMap.keySet().contains(event.getVehicleId())) {
      if (!this.transitVehicleDriverIds.contains(event.getPersonId())) {
