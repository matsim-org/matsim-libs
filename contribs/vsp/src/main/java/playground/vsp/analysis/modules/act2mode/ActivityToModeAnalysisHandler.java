 ...
 * @author droeder
 */
public class ActivityToModeAnalysisHandler
  ...
  private static final Logger log = LogManager.getLogger(ActivityToModeAnalysisHandler.class);
  ...
  public void handleEvent(PersonDepartureEvent event) {
    ...
    if (!event.getLegMode().equalsIgnoreCase(TransportMode.transit_walk)) {
      ...
      if (!(e == null)) {
        ...
        this.departures.add(
            new ActivityToMode(
                e.getActType(), event.getLegMode(), e.getTime(), link.getToNode().getCoord()));
      }
    }
  }
    ...
    if (!event.getActType().equalsIgnoreCase(PtConstants.TRANSIT_ACTIVITY_TYPE)) {
      if (this.person2ArrMode.containsKey(event.getPersonId())) {
        ...
        this.arrivals.add(
            new ActivityToMode(
                event.getActType(),
                this.person2ArrMode.get(event.getPersonId()),
                event.getTime(),
