...
public class TransitVehicleVolumeAnalyzer extends AbstractAnalysisModule {
  ...
  /**
   * creates a shapefiles (per pt-mode) containing the links used by pt-vehicles. Every single
   * Geometry provides fields containing informations about the number of vehicles per interval and
   * in total.
   *
  ...
  public void writeResults(String outputFolder) {
    for (Entry<String, Counts<Link>> e : this.handler.getMode2Counts().entrySet()) {
      writeModeShape(
          e.getKey(),
          e.getValue(),
          this.mode2Link2Total.get(e.getKey()),
          outputFolder + e.getKey() + ".shp",
          this.targetCoordinateSystem);
    }
  }

  private void writeModeShape(
      String name,
      Counts<Link> counts,
      Map<Id, Double> mode2Total,
      String file,
    ...
    for (Count c : counts.getCounts().values()) {
      ...
      coord[0] =
          new Coordinate(l.getFromNode().getCoord().getX(), l.getFromNode().getCoord().getY(), 0.);
      coord[1] =
          new Coordinate(l.getToNode().getCoord().getX(), l.getToNode().getCoord().getY(), 0.);
