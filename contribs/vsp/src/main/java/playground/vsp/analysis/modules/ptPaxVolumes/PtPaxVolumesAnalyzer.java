...
public class PtPaxVolumesAnalyzer extends AbstractAnalysisModule {
  ...
  public void postProcessData() {
    ...
    this.lineId2features.put(
        Id.create("all", TransitLine.class), getAll(this.targetCoordinateSystem));
  }
  ...
  private Collection<SimpleFeature> getTransitLineFeatures(
      TransitLine l, String targetCoordinateSystem) {
    SimpleFeatureTypeBuilder b = new SimpleFeatureTypeBuilder();
    b.setCRS(MGC.getCRS(targetCoordinateSystem));
    b.setName("the_geom");
    ...
    for (Link link : this.sc.getNetwork().getLinks().values()) {
      featureAttribs =
          getLinkFeatureAttribs(link, l.getId(), new Object[4 + this.handler.getMaxInterval()]);
      // skip links without passengers
      if (featureAttribs == null) continue;
      try {
    ...
    for (Entry<Id<TransitLine>, Collection<SimpleFeature>> ee : this.lineId2features.entrySet()) {
      try {
        ...
        ShapeFileWriter.writeGeometries(
            ee.getValue(), outputFolder + ee.getKey().toString() + ".shp");
