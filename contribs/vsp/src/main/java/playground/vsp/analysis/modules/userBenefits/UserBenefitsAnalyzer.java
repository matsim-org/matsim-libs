...
/**
 * This module calculates the logsum for each user and the sum of all user logsums in monetary
 * units. Furthermore, it analyzes users with no valid plan, that are not considered for the logsum
 * calculation.
 *
...
public class UserBenefitsAnalyzer extends AbstractAnalysisModule {
  ...
  public void init(MutableScenario scenario) {
    ...
    this.userWelfareCalculator =
        new UserBenefitsCalculator(this.scenario.getConfig(), WelfareMeasure.LOGSUM, false);
  ...
  public void preProcessData() {
    this.allUsersLogSum =
        this.userWelfareCalculator.calculateUtility_money(this.scenario.getPopulation());
    this.personWithNoValidPlanCnt = this.userWelfareCalculator.getPersonsWithoutValidPlanCnt();
    log.warn(
        "users with no valid plan (all scores ``== null'' or ``<= 0.0''): "
            + personWithNoValidPlanCnt);
