 ...
 * @author gleich
 */
public class PtDistanceHandler
  ...
  private Map<Id, Double> distanceSinceLastDeparture =
      new HashMap<Id, Double>(); // distance travelled by the pt vehicle since the last stop
  ...
  private Map<Id, List<Double>> ptDistancesBetweenActivities =
      new HashMap<Id, List<Double>>(); // including transitWalk to, from and between pt stops
  private Map<Id, List<Double>> transitWalkDistancesBetweenActivities =
      new HashMap<Id, List<Double>>(); // transit walk only
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    ...
    if ((!transitDrivers.contains(event.getPersonId()))
        && (legMode.get(event.getPersonId()).equalsIgnoreCase("pt")
            || legMode.get(event.getPersonId()).equalsIgnoreCase("transit_walk"))) {
      lastTransitWalkBeginCoord.put(
          event.getPersonId(), currentPtFacilityCoord.get(event.getVehicleId()));
      ...
      for (Double d :
          distancePerArrivalAndVehicle
              .get(event.getVehicleId())
              .subMap(lastVehicleEnterEvent.get(event.getPersonId()), event.getTime())
              .values()) {
        distance += d;
      }
  ...
  public void handleEvent(VehicleArrivesAtFacilityEvent event) {
    currentPtFacilityCoord.put(
        event.getVehicleId(),
        scenario.getTransitSchedule().getFacilities().get(event.getFacilityId()).getCoord());
    distancePerArrivalAndVehicle
        .get(event.getVehicleId())
        .put(event.getTime(), distanceSinceLastDeparture.get(event.getVehicleId()));
