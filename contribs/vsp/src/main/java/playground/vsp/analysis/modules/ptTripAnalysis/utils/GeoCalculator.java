...
public class GeoCalculator {
  ...
  public static double angleBeetween2Straights(Tuple<Coord, Coord> one, Tuple<Coord, Coord> two) {
    ...
    Vector2D o =
        new Vector2D(
            one.getSecond().getX() - one.getFirst().getX(),
            one.getSecond().getY() - one.getFirst().getY());
    Vector2D t =
        new Vector2D(
            two.getSecond().getX() - two.getFirst().getX(),
  ...
  public static double angleStraight2e1(Tuple<Coord, Coord> one) {
    if (one.getSecond().getY() < one.getFirst().getY()) {
      return ((2 * Math.PI)
          - angleBeetween2Straights(
              one,
              new Tuple<Coord, Coord>(
                  new Coord((double) 0, (double) 0), new Coord((double) 1, (double) 0))));
    } else {
      return angleBeetween2Straights(
          one,
          new Tuple<Coord, Coord>(
              new Coord((double) 0, (double) 0), new Coord((double) 1, (double) 0)));
    }
  ...
  //	public static double angleBeetween2StraightsDeg(Tuple<Coord, Coord> one, Tuple<Coord, Coord>
  // two){
  //		return (angleBeetween2Straights(one, two)/(Math.PI * 2) * 360);
  //	}

  @Deprecated
  public static double averageStraightDistance(
      Tuple<Coord, Coord> straightOne, Tuple<Coord, Coord> straightTwo) {
