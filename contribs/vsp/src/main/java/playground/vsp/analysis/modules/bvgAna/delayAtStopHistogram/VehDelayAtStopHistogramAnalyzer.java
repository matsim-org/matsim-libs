...
/**
 * Evaluates the delay a vehicle reports at a stop when arriving or departing. Writes a histogram to
 * file. A negative delay is counted as no delay (0s).
 *
 * @author ikaddoura, aneumann
 */
public class VehDelayAtStopHistogramAnalyzer extends AbstractAnalysisModule {
  private static final Logger log = LogManager.getLogger(VehDelayAtStopHistogramAnalyzer.class);
  ...
  /**
   * @param numberOfDetailedSlots Number of separate slots. Each slot aggregates the delay within
   *     that minute, e.g. slot 1 delay from 1s to 59s. Delays <= 0s are counted in the first slot,
   *     delays >= <code>numberOfDetailedSlots</code> in the last slot.
   */
  ...
  public void writeResults(String outputFolder) {
    ...
    try {
      ...
      writer.write("Dumping arrival delay histogram...");
      writer.newLine();
      writer.write("delay; count");
      writer.newLine();
      for (int i = 0; i < this.arrivalDelay.length; i++) {
        writer.write(i + "; " + this.arrivalDelay[i]);
        writer.newLine();
      }

      writer.write("Dumping departure delay histogram...");
      writer.newLine();
      writer.write("delay; count");
      writer.newLine();
      for (int i = 0; i < this.departureDelay.length; i++) {
        writer.write(i + "; " + this.departureDelay[i]);
        writer.newLine();
      }
      writer.flush();
      writer.close();
