 ...
 * @author droeder
 */
public class TTtripEventsHandler
  ...
  private static final Logger log = LogManager.getLogger(TTtripEventsHandler.class);
  ...
  public void handleEvent(PersonStuckEvent e) {
    if (!stuck) {
      log.warn(
          "Found StuckEvent for Agent "
              + e.getPersonId()
              + "! Probably not all Trips from PlansFile are processed! Message thrown only once");
      stuck = true;
    }
  ...
  public void addZones(Map<String, Geometry> zones) {
    ...
    for (Entry<String, Geometry> e : zones.entrySet()) {
      this.zone2tripSet.put(
          e.getKey(), new AnalysisTripSetStorage(false, e.getValue(), this.ptModes));
    }
  }
  ...
  /**
   * returns a csv-<code>String</code> of all agents with a not corresponding number of PlanElements
   * and Events
   *
   * @return
   */
  public String getUncompletedPlans() {
    ...
    b.append(
        "check following agents, because nr of PlanElements and Events does not match, so not all trips are processed\n");
  ...
  public Map<String, AnalysisTripSetStorage> getZone2Tripset() {
    log.info(
        this.nrOfprocessedTrips
            + " of "
            + this.possibleTrips
            + " trips from plansfile are processed");
