...
public class TransitScheduleAnalyserToCSVandTEX {
  ...
  public static void transitScheduleAnalyser(
      TransitSchedule ts, Network net, String outputDirectory) {

    Date date = new Date();
    String filenameTEX =
        outputDirectory
            + "\\transitScheduleAnalyser_"
            + date.getHours()
            + "_"
            + date.getMinutes()
            + ".tex";
    ...
    List<TransitScheduleAnalyserElement> allInformations =
        new ArrayList<TransitScheduleAnalyserElement>();
    ...
    for (Entry<Id<TransitLine>, TransitLine> lineEntry : transitLinesMap.entrySet()) {
      ...
      String routeId;
      Map<Id<TransitRoute>, TransitRoute> transitRoutesMap =
          transitLinesMap.get(lineEntry.getKey()).getRoutes();
      for (Entry<Id<TransitRoute>, TransitRoute> routeEntry : transitRoutesMap.entrySet()) {
        ...
        Double tourlengthTimeDouble =
            transitstopsList.get(transitstopsList.size() - 1).getArrivalOffset().seconds()
                - transitstopsList.get(0).getDepartureOffset().seconds();
        ...
        for (int i = 0; i < transitstopsList.size(); i++) {
          routeLength =
              routeLength
                  + net.getLinks()
                      .get(transitstopsList.get(i).getStopFacility().getLinkId())
                      .getLength();
        }
        ...
        TransitScheduleAnalyserElement temp =
            new TransitScheduleAnalyserElement(
                lineId,
                routeId,
                firstDeparture,
                lastDeparture,
    ...
    try {
      ...
      writer.write(
          "# lineID; routeID; first Departure; last Departure; tourlength time; tourlength distance; average headway; number of vehicles");
      writer.newLine();
      for (int i = 0; i < allInformations.size(); i++) {
        writer.write(
            allInformations.get(i).lineId
                + "; "
                + allInformations.get(i).routeId
                + "; "
                + allInformations.get(i).firstDeparture
                + "; "
                + allInformations.get(i).lastDeparture
                + "; "
                + allInformations.get(i).tourLengthTime
                + "; "
                + allInformations.get(i).tourLengthDistance
                + "; "
                + allInformations.get(i).averageHeadway
                + "; "
                + allInformations.get(i).numberOfVehicles);
        writer.newLine();
      }
    ...
    try {
      ...
      writer.newLine();
      writer.write(
          "\\begin{longtable}{>{\\raggedleft}p{15mm} >{\\raggedleft}p{15mm} >{\\raggedleft}p{18mm} >{\\raggedleft}p{18mm} >{\\raggedleft}p{18mm} >{\\raggedleft}p{10mm} >{\\raggedleft}p{15mm} >{\\raggedleft}p{10mm}}");
      writer.newLine();
      writer.write(
          "\\caption{\\label{tab:LABEL}Results of Transitschedule Analyser} \\tabularnewline \\addlinespace[1.0em]");
      writer.newLine();
      writer.write("\\toprule");
      writer.newLine();
      writer.write(
          "lineID & routeID & first Departure &  last Departure &  tourlength time & tourlength distance &  average headway &  number of vehicles \\tabularnewline");
      writer.newLine();
    ...
    try {
      BufferedWriter writer =
          new BufferedWriter(new FileWriter(new File(filenameTexForAutomaticAnalyzer), false));
      writer.write(
          "\\begin{longtable}{>{\\raggedleft}p{15mm} >{\\raggedleft}p{15mm} >{\\raggedleft}p{18mm} >{\\raggedleft}p{18mm} >{\\raggedleft}p{18mm} >{\\raggedleft}p{10mm} >{\\raggedleft}p{15mm} >{\\raggedleft}p{10mm}}");
      writer.newLine();
      writer.write(
  ...
  public static void main(String[] args) {
    ...
    TransitScheduleAnalyserToCSVandTEX.transitScheduleAnalyser(
        sc.getTransitSchedule(), sc.getNetwork(), "F:/temp");
  }
}
