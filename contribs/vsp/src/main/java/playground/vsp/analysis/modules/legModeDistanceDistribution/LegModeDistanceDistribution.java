 ...
 *
 * <p>WARNING: The tool depends on plans only, no events are processed! Trip distances are beeline
 * distances between activity locations. The actual (driven, teleported, ...) distances may differ
 * significantly.
 *
 * @author aneumann, benjamin, ihab
 */
public class LegModeDistanceDistribution extends AbstractAnalysisModule {
  private static final Logger log = LogManager.getLogger(LegModeDistanceDistribution.class);
  ...
  public void preProcessData() {
    ...
    log.info(
        "Checking if the plans file that will be analyzed is based on a run with simulated public transport.");
    log.info("Transit activities and belonging transit walk legs will be removed from the plan.");
    log.info(
        "However, there might be some transit walk legs remaining without previous transit activity.");
      ...
      for (int i = 0, n = planElements.size(); i < n; i++) {
        ...
        if (pe instanceof Activity) {
          ...
          if (PtConstants.TRANSIT_ACTIVITY_TYPE.equals(act.getType())) {
            ...
            final int index = i;
            PopulationUtils.removeActivity(
                ((Plan) selectedPlan), index); // also removes the following leg
            n -= 2;
            i--;
          }
        }
        ...
        if (pe instanceof Leg) {
          ...
          if (legMode.equals(TransportMode.transit_walk)) {
            ...
            if (transitWalkInterpretation < 1) {
              log.info(
                  "Transit walk leg without previous transit activity detected. It will be interpreted as walk leg.");
    ...
    log.info(
        transitWalkInterpretation
            + " transit walk legs without previous transit activity were interpreted as walk legs.");
  }

  @Override
  public void postProcessData() {
    this.mode2DistanceClass2LegCount =
        calculateMode2DistanceClass2LegCount(this.scenario.getPopulation());
  ...
  public void writeResults(String outputFolder) {
    ...
    try {
      ...
      for (int i = 0; i < this.distanceClasses.size() - 1; i++) {
        //	Integer middleOfDistanceClass = ((this.distanceClasses.get(i) +
        // this.distanceClasses.get(i + 1)) / 2);
        ...
        for (String mode : this.usedModes) {
          ...
          modeLegs =
              this.mode2DistanceClass2LegCount.get(mode).get(this.distanceClasses.get(i + 1));
    ...
    //		log.info("Dumping population for control in case legs were cut out (simulated public
    // transport)...");
  ...
  private SortedMap<String, Map<Integer, Integer>> calculateMode2DistanceClass2LegCount(
      Population pop) {
    SortedMap<String, Map<Integer, Integer>> mode2DistanceClassNoOfLegs =
        new TreeMap<String, Map<Integer, Integer>>();
          ...
          for (PlanElement pe : planElements) {
            if (pe instanceof Leg) {
              ...
              if (legMode.equals(mode)) {
                if (legBeelineDist > this.distanceClasses.get(i)
                    && legBeelineDist <= this.distanceClasses.get(i + 1)) {
                  noOfLegs++;
                } else {
  ...
  private double getLongestBeelineDistance(Population pop) {
    ...
    log.info(
        "The longest beeline distance between two activity locations is found to be: "
            + longestBeelineDistance);
