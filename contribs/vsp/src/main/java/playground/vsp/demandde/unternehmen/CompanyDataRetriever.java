 ...
 *     <p>This class searches for company data on the bundesanzeiger-verlag.de website and writes
...
public class CompanyDataRetriever {
  ...
  private static final String COMPANY_URL =
      "/betrifft-unternehmen/unternehmensdaten/deutsche-unternehmensdaten/"
  ...
  private static final String PAGE_URL =
      "/betrifft-unternehmen/unternehmensdaten/deutsche-unternehmensdaten/suche"
  ...
  private static String outputDirectory =
      "../../../../Workspace/data/accessibility/berlin/companyData/";
  ...
  public static void main(String[] args) {
    ...
    if (webPageIndex != 1 && (webPageIndex - 1) % numberOfWebpagesPerFile != 0) {
      throw new RuntimeException(
          "For consecutive files (i.e. files that do not start with "
    ...
    do {
      ...
      log.info(
          "Number of companies to be added to file " + outputFile + " is " + companyData.size());
  ...
  private static void writeCompanyData(CSVFileWriter writer, CompanyData companyData) {
        ...
        try {
          ...
          try {
            while ((line = bufferedReader.readLine()) != null) {
              if (line.contains("href=\"" + COMPANY_URL)
                  && line.contains("InternerLinkHighlight")) {
                ...
                if (line.contains("<td>Berlin</td>")) {
                  ...
                  CompanyData currentCompanyData =
                      getCompanyDataFromIdAndPageIndex(id, webPageIndex);
                  currentCompanyData.setCorporateForm(removeTapsAndSpacesBeforeLine(corporateForm));
                  companyData.add(currentCompanyData);
                }
              }
              if (webPageIndex >= endInt
                  || line.contains("Ihre Suche ergab leider keine Treffer.")) {
        ...
        try {
          while ((line = bufferedReader.readLine()) != null) {
            if (line.contains("s4a_freekmu_single_content")) {
              line = bufferedReader.readLine();
              data.setCompanyName(
                  removeTapsAndSpacesBeforeLine(removeTagFromContent(formatContent(line), "h1")));
            }
            if (line.contains("<h5>Ort</h5>")) {
              line = bufferedReader.readLine();
              String[] site =
                  removeTagFromContent(removeTapsAndSpacesBeforeLine(line), "p").split(" ");
            ...
            if (line.contains("<h5>Register</h5>")) {
              line = bufferedReader.readLine();
              data.setCommercialRegisterNumber(
                  removeTapsAndSpacesBeforeLine(
            ...
            if (line.contains("<h5>Branche(n):</h5>")
              ...
              while (!(line = bufferedReader.readLine()).contains("</ul>")) {
                if (line.contains("<li>")) {
                  sector.add(
                      removeTapsAndSpacesBeforeLine(
                          removeTagFromContent(formatContent(line), "li")));
                }
              }
  ...
  private static String formatContent(String content) {
    return removeTagFromContent(
        removeTapsAndSpacesBeforeLine(content).replace("&nbsp;", " ").replace(";", ""), "strong");
  }
