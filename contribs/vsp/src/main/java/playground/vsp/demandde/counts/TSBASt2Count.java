...
public class TSBASt2Count {
  ...
  private static final String bastInputFile = "C:/Users/Tille/WORK/BASt/2013_A_S.txt";

  // note: should end with .gz since it is probably quite big
  private static final String OUTPUTDIR = "C:/Users/Tille/WORK/BASt/2013_A_S_TUE_THU_WKEND.xml.gz";

  private static final int BEGINNING_WEEKDAY = 2;
  private static final int ENDING_WEEKDAY = 4;

  private static final boolean CALC_WEEKENDS = true;
  ...
  public static void main(String[] args) throws IOException, ParseException, FactoryException {
    ...
    while ((line = reader.readLine()) != null) {
      ...
      boolean isConsideredWeekDay =
          (tokens[indexPurpose].equals("w")
              && (weekDay >= BEGINNING_WEEKDAY && weekDay <= ENDING_WEEKDAY));
      boolean isConsideredWeekEnd =
          (CALC_WEEKENDS && (tokens[indexPurpose].equals("s") || weekDay == 6));
      ...
      if (isConsideredWeekDay || isConsideredWeekEnd) {
        ...
        switch (tokens[indexValidityOfD1]) {
          ...
          case "s":
          case "k":
            data.computeAndSetVolume(
                true, hour, format.parse(tokens[indexVolumeD1].trim()).doubleValue());
        }
        switch (tokens[indexValidityOfD2]) {
          ...
          case "k":
            data.computeAndSetVolume(
                false, hour, format.parse(tokens[indexVolumeD2].trim()).doubleValue());
        }
        ...
        for (int i = 1; i < 25; i++) {
          ...
          if (value == null) {
            logger.error(
                "station "
                    + str
                    + " has no valid entry for hour "
                    + i
                    + ". Please check this. Instead, a negative volume for this hour is written into the counts file.");
            value = -1.0;
          }
