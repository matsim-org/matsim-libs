...
/**
 * This class provides the basic functionality to calculate congestion effects which may be used for
 * internalization. One of the main tasks is to keep track of the queues on each link.
 *
 * @author ikaddoura
 */
class CongestionHandlerBaseImpl implements CongestionHandler {
  ...
  private static final Logger log = LogManager.getLogger(CongestionHandlerBaseImpl.class);
  ...
  CongestionHandlerBaseImpl(EventsManager events, Scenario scenario) {
    ...
    if (this.scenario.getConfig().transit().isUseTransit()) {
      log.warn(
          "Mixed traffic (simulated public transport) is not tested. Vehicles may have different effective cell sizes than 7.5 meters.");
    }
  }
  ...
  public final void handleEvent(PersonStuckEvent event) {
    //		log.warn("An agent is stucking. No garantee for right calculation of external congestion
    // effects: " + event.toString());
  }
  ...
  public final void handleEvent(PersonDepartureEvent event) {
    if (event.getLegMode().toString().equals(TransportMode.car.toString())) { // car!
      ...
      LinkCongestionInfo linkInfo =
          CongestionUtils.getOrCreateLinkInfo(event.getLinkId(), linkId2congestionInfo, scenario);

      AgentOnLinkInfo agentInfo =
          new AgentOnLinkInfo.Builder()
              .setAgentId(event.getPersonId())
              .setLinkId(event.getLinkId())
              .setEnterTime(event.getTime())
              .setFreeSpeedLeaveTime(event.getTime() + 1.)
              .build();
      ...
      if (this.carPersonIDs.contains(driverId)) {
        ...
        AgentOnLinkInfo agentInfo =
            new AgentOnLinkInfo.Builder()
                ...
                .setLinkId(event.getLinkId())
                .setEnterTime(event.getTime())
                .setFreeSpeedLeaveTime(event.getTime() + linkInfo.getFreeTravelTime() + 1.)
                .build();
  ...
  public final void handleEvent(LinkLeaveEvent event) {
    // yy My preference would be if we found a solution where the basic bookkeeping (e.g. update
    // flow and
    // delay queues) is done here.  However, delegation does not allow to have custom code in
    // between
    // standard code (as was the case before with calculateCongestion).  We need to consider if it
    // is possible to
      ...
      if (this.carPersonIDs.contains(personId)) {
        ...
        LinkCongestionInfo linkInfo =
            CongestionUtils.getOrCreateLinkInfo(
                event.getLinkId(), this.getLinkId2congestionInfo(), scenario);

        AgentOnLinkInfo agentInfo = linkInfo.getAgentsOnLink().get(personId);

        DelayInfo delayInfo =
            new DelayInfo.Builder(agentInfo).setLinkLeaveTime(event.getTime()).build();
  ...
  public static final void updateFlowAndDelayQueues(
      double time, DelayInfo delayInfo, LinkCongestionInfo linkInfo) {
    ...
    if (linkInfo.getFlowQueue().isEmpty()) {
      ...
      // seems like this should never be called when (*) is used, but in practice it happens, i.e.
      // tests fail when the above if condition
      // is commented out.  maybe initialization?  kai, apr'16
    } else {
      double earliestLeaveTimeAfterVehicleAhead =
          linkInfo.getLastLeaveEvent().getTime() + linkInfo.getMarginalDelayPerLeavingVehicle_sec();
      if (time > earliestLeaveTimeAfterVehicleAhead + 1.) {
        // Vehicle is delayed by more than 1/cap.  That is, we must be spill-back delayed.

        double freeSpeedLeaveTimeGap =
            delayInfo.freeSpeedLeaveTime - linkInfo.getFlowQueue().getLast().freeSpeedLeaveTime;
        ...
        // yy this is what I think we would have to accept if we wanted to bring V3 and V4 under the
        // same umbrella.
        // It would, as I discussed via skype with Ihab, probably change the old result, hopefully
        // only slightly.
        // On second thought, however, this is not so obvious.  The Amit case (first flow then
        // storage delayed)
        // is important, but the above condition would also move cases into the flow queue where the
        // origin
   ...
   * @param affectedAgentDelayInfo delayInfo of affected agent to get free speed link leave time and
   *     person id
   * @param remainingDelay at this step, it is delay of the affected agent
   * @return the remaining uncharged delay
   *     <p>Charging the agents that are in the flow queue.
   */
  final double computeFlowCongestionAndReturnStorageDelay(
      double now, Id<Link> linkId, DelayInfo affectedAgentDelayInfo, double remainingDelay) {

    LinkCongestionInfo linkInfo = this.linkId2congestionInfo.get(linkId);

    for (Iterator<DelayInfo> it = linkInfo.getFlowQueue().descendingIterator();
    ...
    if (remainingDelay > 0. && remainingDelay <= 1) {
      // The remaining delay of up to 1 sec may result from rounding errors. The delay caused by the
      // flow capacity sometimes varies by 1 sec.
