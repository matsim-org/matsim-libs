 ...
 */
public class CongestionAnalysisEventHandler
    implements PersonMoneyEventHandler,
        ...
        CongestionEventHandler {
  private static final Logger log = LogManager.getLogger(CongestionAnalysisEventHandler.class);
  ...
  private Map<Id<Person>, Map<Integer, String>> personId2tripNumber2legMode =
      new HashMap<Id<Person>, Map<Integer, String>>();

  private Map<Id<Person>, Map<Integer, Double>> personId2tripNumber2departureTime =
      new HashMap<Id<Person>, Map<Integer, Double>>();
  private Map<Id<Person>, Map<Integer, Double>> personId2tripNumber2tripDistance =
      new HashMap<Id<Person>, Map<Integer, Double>>();
  private Map<Id<Person>, Map<Integer, Double>> personId2tripNumber2amount =
      new HashMap<Id<Person>, Map<Integer, Double>>();
  ...
  public CongestionAnalysisEventHandler(Scenario scenario, boolean useMoneyEvents) {
    ...
    if (useMoneyEvents) {
      log.warn(
          "Money events may be thrown later than the congestion events... May result in a wrong interpretation of the results. Better use directly the congestion events for analysis.");
    }
    this.vtts_car =
        (this.scenario
                    .getConfig()
    ...
    log.info(
        "Anlayzing the congestion events during the simulation. Assuming the following VTTS (equal for all agents): "
            + vtts_car);
  }
  ...
  public void handleEvent(CongestionEvent event) {
    ...
    if (useMoneyEvents == false) {
      ...
      Map<Integer, Double> tripNumber2departureTime =
          personId2tripNumber2departureTime.get(event.getCausingAgentId());
      ...
      double amountBefore =
          personId2tripNumber2amount.get(event.getCausingAgentId()).get(tripNumber);
      double updatedAmount = amountBefore + amount;
      Map<Integer, Double> tripNumber2amount =
          personId2tripNumber2amount.get(event.getCausingAgentId());
  ...
  public Map<Id<Person>, List<Double>> getPersonId2listOfDepartureTimes(String mode) {
    Map<Id<Person>, List<Double>> personId2listOfDepartureTimes =
        new HashMap<Id<Person>, List<Double>>();
  ...
  public Map<Id<Person>, List<Double>> getPersonId2listOfDistances(String mode) {
    Map<Id<Person>, List<Double>> personId2listOfDistances =
        new HashMap<Id<Person>, List<Double>>();
  ...
  public Map<Double, Double> getAvgAmountPerTripDepartureTime(String mode) {
    ...
    for (Double time : tripDepTime2fares.keySet()) {
      for (int counter : counter2allDepartureTimesAndAmounts.keySet()) {
        if (counter2allDepartureTimesAndAmounts.get(counter)[0] < time
            && counter2allDepartureTimesAndAmounts.get(counter)[0] >= (time - periodLength)) {
  ...
  public Map<Double, Double> getAvgAmountPerTripDistance(String mode) {
    ...
    for (Double dist : tripDistance2amount.keySet()) {
      for (int counter : counter2allDistancesAndAmounts.keySet()) {
        if (counter2allDistancesAndAmounts.get(counter)[0] < dist
            && counter2allDistancesAndAmounts.get(counter)[0] >= (dist - groupsize)) {
  ...
  public void handleEvent(ActivityEndEvent event) {
    ...
    if (ptDrivers.contains(event.getPersonId())) {
      throw new RuntimeException(
          "ActivityEndEvent by a transit-driver! The code has to be adapted.");
    }
    if (event.getActType().toString().equals("pt interaction")) {
      ...
      if (personId2actualTripNumber.containsKey(event.getPersonId())) {
        ...
        personId2actualTripNumber.put(
            event.getPersonId(), personId2actualTripNumber.get(event.getPersonId()) + 1);
        Map<Integer, Double> tripNumber2departureTime =
            personId2tripNumber2departureTime.get(event.getPersonId());
        tripNumber2departureTime.put(
            personId2actualTripNumber.get(event.getPersonId()), event.getTime());
  ...
  public Map<Id<Person>, Double> getCausingAgentId2amountSumAllAgents() {
    ...
    if (this.scenario.getPopulation().getPersons().isEmpty()) {
      log.warn(
          "Scenario does not contain a Population. Using the person IDs from the events file for the person-based analysis (total: "
              + personIds.size()
              + ").");
      personIds.addAll(this.persons);
    } else {
      log.info(
          "Scenario contains a Population. Using the person IDs from the population for the person-based analysis.");
  ...
  public Map<Id<Person>, Double> getAffectedAgentId2amountSumAllAgents() {
    ...
    if (this.affectedAgentId2amountSum.isEmpty()) {
      log.warn(
          "There is no info re the affected agents. This info is only avalable when analyzing congestion events and not the money events.");
