 ...
 *
 * <p>If the delay is > 0, in this implementation, the headway to the previous agent is allocated to
 * ALL agents ahead in the flow queue. Each agent has to pay for 1 / c_flow.
 *
...
public final class CongestionHandlerImplV10 implements CongestionHandler {
  ...
  private static final Logger log = LogManager.getLogger(CongestionHandlerImplV10.class);
  ...
  public void calculateCongestion(LinkLeaveEvent event, DelayInfo delayInfo) {
    ...
    if (delayOnThisLink < 0.) {
      ...
      for (Iterator<DelayInfo> it = linkInfo.getFlowQueue().iterator(); it.hasNext(); ) {
        // Get the agents 'ahead' from the flow queue. The agents 'ahead' are considered as causing
        // agents.
