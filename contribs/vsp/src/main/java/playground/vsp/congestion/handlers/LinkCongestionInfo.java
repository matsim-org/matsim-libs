...
/**
 * Collects all link specific informations which are required for the calculation of external
 * congestion effects.
 *
 * @author ikaddoura
 */
public final class LinkCongestionInfo {
  public static class Builder {
    ...
    public Builder setFreeTravelTime(double val) {
      this.freeTravelTime = val;
      return this;
    }

    public Builder setMarginalDelayPerLeavingVehicle_sec(double val) {
      this.marginalDelayPerLeavingVehicle_sec = val;
      return this;
    }

    public Builder setStorageCapacityCars(double val) {
      this.storageCapacityCars = val;
      return this;
    }

    public LinkCongestionInfo build() {
      return new LinkCongestionInfo(
          this.linkId,
          this.freeTravelTime,
          this.marginalDelayPerLeavingVehicle_sec,
          this.storageCapacityCars);
    }
  }

  private LinkCongestionInfo(
      Id<Link> linkId,
      double freeTravelTime,
      double marginalDelayPerLeavingVehicle_sec,
      double storageCapacityCars) {
    this.linkId = linkId;
  ...
  /**
   * Vehicles that have left the link in previous time steps, while the bottleneck was "active". The
   * flow queue is in consequence interrupted when the bottleneck is not active, which is when time
   * headway > 1/cap + eps
   */
