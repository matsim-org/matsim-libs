...
/// **
// * @author amit
// * Another version of congestion handler, if a person is delayed, it will charge everything to the
// person who just left before if link is constrained by flow capacity else
// * it will identify the bottleneck link (spill back causing link) and charge the person who just
// entered on that link.
// */
//
...
//		double delay = event.getTime() -
// linkInfo.getPersonId2freeSpeedLeaveTime().get(event.getDriverId());
//
//		if(delay==0) return;
//
...
//			/*
//			 * Instead of the above if statement, I should use the commented if statement (isLinkFree). So
// that,
//			 * if an agent is first delayed by flowCapacity and then storageCapacity, it charges correct
// agent.
...
//			List<Id<Person>> personsEnteredOnCausingLink = new
// ArrayList<Id<Person>>(this.getLinkId2congestionInfo().get(causingLink).getPersonId2linkEnterTime().keySet());
...
//					return;
//				}else {
//					throw new RuntimeException("Delay for person "+event.getDriverId()+" is "+ delay+" sec. But
// causing agent could not be located. This happened during event "+event.toString()+"
// Aborting...");
//				}
//			}
//
...
//		}
//
//		CongestionEvent congestionEvent = new CongestionEvent(event.getTime(), congestionType,
// causingAgent,
//				event.getDriverId(), delay, causingLink,
// this.getLinkId2congestionInfo().get(causingLink).getPersonId2linkEnterTime().get(causingAgent));
...
//		double freeSpeedLeaveTimeOfLastLeftAgent =
// linkInfo.getPersonId2freeSpeedLeaveTime().get(linkInfo.getLastLeaveEvent());
//		double freeSpeedLeaveTimeOfNowAgent =
// linkInfo.getPersonId2freeSpeedLeaveTime().get(event.getDriverId());
//
...
//		/*
//		 *  TODO : yet to fix this, at the moment (sep 15), lastLeaveTime will throw NullPointException
// because
...
//
//		/*
//		 * the following check ensures that the time gap between the current agent and last left agent
// is more than MarginalDelayPerLeavingVehicle_sec
//		 * i.e. agent is now delayed due to storage capacity
//		 */
//
//		double earliestLeaveTime = Math.floor(lastLeaveTime+
// linkInfo.getMarginalDelayPerLeavingVehicle_sec()) +1;
...
// scenario.getPopulation().getPersons().get(personId).getSelectedPlan().getPlanElements();
