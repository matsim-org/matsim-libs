 ...
 *
 * <p>In this implementation, the delay is partially allocated to ALL agents ahead in the flow
 * queue. Each agent has to pay for 1 / c_flow.
 *
 * <p>Spill-back effects are not taken into account.
 *
 * @author ikaddoura
 */
public final class CongestionHandlerImplV8 implements CongestionHandler {

  private static final Logger log = LogManager.getLogger(CongestionHandlerImplV8.class);
  ...
  public void calculateCongestion(LinkLeaveEvent event, DelayInfo delayInfo) {
    ...
    if (delayOnThisLink < 0.) {
      ...
      for (Iterator<DelayInfo> it = linkInfo.getFlowQueue().descendingIterator(); it.hasNext(); ) {
        // Get the agent 'ahead' from the flow queue. The agents 'ahead' are considered as causing
        // agents.
        ...
        CongestionEvent congestionEvent =
            new CongestionEvent(
                event.getTime(),
                "version8",
                causingAgentDelayInfo.personId,
                delayInfo.personId,
                allocatedDelay,
                event.getLinkId(),
                causingAgentDelayInfo.linkEnterTime);
