...
/**
 * This handler calculates delays (caused by the flow and storage capacity), identifies the causing
 * agent(s) and throws marginal congestion events. Marginal congestion events can be used for
 * internalization. 1) At link Leave event, delay is calculated which is the difference of actual
 * leaving time and the leaving time according to free speed. 2) Persons leaving link are identified
 * and these are charged until delay =0; if the time headway between free speed time of two
 * subsequent vehicle is more than minimum time headway (1/c_flow), then (=spill back delays), spill
 * back causing link is stored 3) Subsequently spill back delays are processed by identifying spill
 * back causing link(s) and by charging entering agents (i.e. persons currently on the link) and
 * leaving agents alternatively until delay=0;
 *
 * @author amit
 */
public final class CongestionHandlerImplV4 implements CongestionHandler {

  private static final Logger log = LogManager.getLogger(CongestionHandlerImplV4.class);
  ...
  public final void handleEvent(LinkEnterEvent event) {
    ...
    Id<Person> driverId =
        delegate.getVehicle2DriverEventHandler().getDriverOfVehicle(event.getVehicleId());
  ...
  public final void handleEvent(LinkLeaveEvent event) {
    ...
    if (this.delegate.getPtVehicleIDs().contains(event.getVehicleId())) {
      ...
      LinkCongestionInfo linkInfo =
          CongestionUtils.getOrCreateLinkInfo(
              event.getLinkId(), delegate.getLinkId2congestionInfo(), scenario);

      AgentOnLinkInfo agentInfo = linkInfo.getAgentsOnLink().get(personId);

      DelayInfo delayInfo =
          new DelayInfo.Builder()
              .setPersonId(personId)
              .setLinkEnterTime(agentInfo.getEnterTime())
              .setFreeSpeedLeaveTime(agentInfo.getFreeSpeedLeaveTime())
              .setLinkLeaveTime(event.getTime())
              .build();
  ...
  public final void calculateCongestion(LinkLeaveEvent event, DelayInfo affectedAgentDelayInfo) {
    ...
    double remainingDelay =
        event.getTime()
            - linkInfo
                .getAgentsOnLink()
                .get(affectedAgentDelayInfo.personId)
                .getFreeSpeedLeaveTime();

    if (remainingDelay == 0.) return;

    if (linkInfo.getFlowQueue().isEmpty() && remainingDelay > 0.0) {
      // (flow queue contains only those agents where time headway approx 1/cap. So we get here only
      // if we are spillback delayed,
    ...
    remainingDelay =
        this.delegate.computeFlowCongestionAndReturnStorageDelay(
            event.getTime(), event.getLinkId(), affectedAgentDelayInfo, remainingDelay);

    if (remainingDelay > 0) {

      // for combined delay due to flow and storage capacities on the same link, the spill back
      // causing link it not captured
      // because flowQueue is not empty any more with new (updateFlowQueue in delegate, 16-sep-15)
      // logic. amit sep'15
      if (this.linkId2SpillBackCausingLinks.get(event.getLinkId()) == null) {
        Id<Person> driverId =
            this.delegate.getVehicle2DriverEventHandler().getDriverOfVehicle(event.getVehicleId());
        memorizeSpillBackCausingLinkForCurrentLink(
            event.getLinkId(), getDownstreamLinkInRoute(driverId));
      }

      // !! calling the following method is the big difference to V3 !!!
      remainingDelay =
          allocateStorageDelayToDownstreamLinks(
              remainingDelay, event.getLinkId(), event, affectedAgentDelayInfo);
      ...
      if (remainingDelay > 0.) {
        throw new RuntimeException(
            "time="
                ...
                + "; "
                + remainingDelay
                + " sec delay is not internalized. Aborting...");
      }
    }
  }

  private void memorizeSpillBackCausingLinkForCurrentLink(
      Id<Link> currentLink, Id<Link> spillBackCausingLink) {
    if (this.linkId2SpillBackCausingLinks.containsKey(currentLink)) {
      ...
      this.linkId2SpillBackCausingLinks.put(
          currentLink, new LinkedList<Id<Link>>(Arrays.asList(spillBackCausingLink)));
    }
  }

  private double allocateStorageDelayToDownstreamLinks(
      double remainingDelay,
      Id<Link> linkId,
      LinkLeaveEvent event,
      DelayInfo affectedAgentDelayInfo) {
    ...
    for (Iterator<Id<Link>> it = spillBackCausingLinks.descendingIterator();
        it.hasNext() && remainingDelay > 0.; ) {
      Id<Link> spillBackCausingLink = it.next();

      remainingDelay =
          processSpillbackDelays(
              remainingDelay, event, spillBackCausingLink, affectedAgentDelayInfo);
      ...
      if (remainingDelay <= 0) {
        ...
        remainingDelay =
            allocateStorageDelayToDownstreamLinks(
                remainingDelay, spillBackCausingLink, event, affectedAgentDelayInfo);
        // yy this looks like depth-first allocation to me; should probably also try breadth-first
        // allocation. kai, sep'15
      }
    }
    return remainingDelay;
  }

  private double processSpillbackDelays(
      double remainingDelay,
      LinkLeaveEvent event,
      Id<Link> spillbackCausingLink,
      DelayInfo affectedAgentDelayInfo) {
    Id<Person> affectedPersonId =
        delegate.getVehicle2DriverEventHandler().getDriverOfVehicle(event.getVehicleId());

    // first charge for agents present on the link or in other words agents entered on the link
    LinkCongestionInfo spillbackLinkCongestionInfo =
        this.delegate.getLinkId2congestionInfo().get(spillbackCausingLink);

    final LinkedList<AgentOnLinkInfo> agentsOnLinksAsDeque =
        new LinkedList<AgentOnLinkInfo>(spillbackLinkCongestionInfo.getAgentsOnLink().values());
    for (Iterator<AgentOnLinkInfo> it = agentsOnLinksAsDeque.descendingIterator();
        remainingDelay > 0.0 && it.hasNext(); ) {
      // (didn't find an easier way both to be able to remove by Id and to iterate from back.
      // sep'15, kai)
      AgentOnLinkInfo agentInfo = it.next();
      Id<Person> causingPersonId = agentInfo.getPersonId();
      double agentDelay =
          Math.min(
              spillbackLinkCongestionInfo.getMarginalDelayPerLeavingVehicle_sec(), remainingDelay);
      ...
      CongestionEvent congestionEvent =
          new CongestionEvent(
              ...
              affectedPersonId,
              agentDelay,
              spillbackCausingLink,
              spillbackLinkCongestionInfo.getAgentsOnLink().get(causingPersonId).getEnterTime());
    ...
    if (remainingDelay > 0.) {
      remainingDelay =
          this.delegate.computeFlowCongestionAndReturnStorageDelay(
              event.getTime(), spillbackCausingLink, affectedAgentDelayInfo, remainingDelay);
    }
