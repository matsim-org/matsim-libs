...
public class CemdapStopsParser {
  ...
  private static final Logger LOG = LogManager.getLogger(CemdapStopsParser.class);
  ...
  public final void parse(
      String cemdapStopsFile,
      int planNumber,
      Population population,
      ObjectAttributes personZoneAttributes,
    ...
    try {
      ...
      while ((currentLine = bufferedReader.readLine()) != null) {
        ...
        if (lineCount % 1000000 == 0) {
          LOG.info(
              "Line " + lineCount); // + ": " + population.getPersons().size() + " persons stored so
          // far.");
        ...
        if (person == null) {
          if (planNumber != 0) {
            throw new RuntimeException(
                "Person "
                    + personId.toString()
                    + " must not be null here since even home-staying personsn should have been added by now.");
          }
        ...
        // Create a new plan if plan with current plan number does not already exist and add a home
        // activity
        ...
        if (plan.getPlanElements().isEmpty()) {
          ...
          Activity firstActivity =
              population.getFactory().createActivityFromCoord(activityType, DEFAULT_COORD);
        ...
        double endTime =
            departureTime + Integer.parseInt(entries[TRAVEL_TIME_TO_STOP]) * 60 + stopDuration;
        activity.getAttributes().putAttribute(CEMDAP_STOP_DURATION_S_ATTRIBUTE_NAME, stopDuration);
        if (endTime
            < 97200.) { // Set end time only if it is not the last activity of the day; 97200s =
          // 24h+3h
  ...
  // see:
  // shared-svn/projects/cemdapMatsimCadyts/cemdap_software/cemdap-11-2014/Activity_Mapping_Nov13.xlsx
  ...
  private String transformActType(int activityTypeNumber) {
    switch (activityTypeNumber) {
      ...
      case 0:
        return ActivityTypes.SHOPPING; // 	Shopping	0
      case 1:
        return ActivityTypes.OTHER; // 	Social Activity	1
      case 2:
        return ActivityTypes.OTHER; // 	Others	2
      case 3:
        return ActivityTypes.LEISURE; // 	Eating Out	3
      case 4:
        return ActivityTypes.OTHER; // 	Serving Passengers	4
      case 5:
        return ActivityTypes.LEISURE; // 	Entertainment	5
      case 6:
        return ActivityTypes.LEISURE; // 	Active Recreation	6
      case 7:
        return ActivityTypes.OTHER; // 	Visit friends/family	7
      case 8:
        return ActivityTypes.WORK; // 	Work Related	8
      case 9:
        return ActivityTypes.OTHER; // 	Maintainence	9
      case 10:
        return ActivityTypes.OTHER; // 	Drop off to school	10
      case 11:
        return ActivityTypes.OTHER; // 	Pick up from school	11
      case 12:
        return ActivityTypes.HOME; // 	Go home adult	12
      case 13:
        return ActivityTypes.WORK; // 	Go work adult	13
      case 14:
        return ActivityTypes.HOME; // 	Go home child	14
      case 15:
        return ActivityTypes.EDUCATION; // 	Go school child	15
      case 16:
        return ActivityTypes.LEISURE; // 	Independent discretionary activity	16
      case 17:
        return ActivityTypes.WORK; // 	Work	17
      case 18:
        return ActivityTypes.HOME; // 	Home	18
      case 19:
        return ActivityTypes.EDUCATION; // 	School	19
      case 20:
        return ActivityTypes.LEISURE; // 	Joint discretionary with child	20
      case 21:
        return ActivityTypes.LEISURE; // 	Joint discretionary with parents	21
      default:
        LOG.error(
            new IllegalArgumentException("actTypeNo=" + activityTypeNumber + " not allowed."));
        return null;
    }
  }
  ...
  private void cleanUp(Population population, int planNumber) {
    ...
    for (Person person : population.getPersons().values()) {
      Activity firstActivity =
          (Activity) person.getPlans().get(planNumber).getPlanElements().get(0);
      if (firstActivity.getEndTime().seconds() < 0.0) {
        idsOfPersonsToRemove.add(person.getId());
        LOG.info(
            "Person with ID "
                + person.getId()
                + ": first departure before 00:00:00. Will be removed from the population");
      }
    }
