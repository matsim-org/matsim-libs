...
public class CemdapOutputAnalyzer {
  ...
  public void logOutput() {
    ...
    for (String activityType : activityTypeToCount.keySet()) {
      ...
      double averageNumberOfCurrentActivities =
          activityTypeToCount.get(activityType) / numberOfAgents;
      double averageDurationOfCurrentActivities =
          activityTypeToDuration.get(activityType) / (double) activityTypeToCount.get(activityType);
      LOG.info(
          "Average number of " + activityType + "-activities: " + averageNumberOfCurrentActivities);
      LOG.info(
          "Average duration of "
              + activityType
              + "-activities: "
              + convertTimeFromSeconds((int) averageDurationOfCurrentActivities));
    }
  }
    ...
    try {
      ...
      for (String activityType : activityTypeToCount.keySet()) {
        ...
        double averageNumberOfCurrentActivities =
            activityTypeToCount.get(activityType) / numberOfAgents;
        double averageDurationOfCurrentActivities =
            activityTypeToDuration.get(activityType)
                / (double) activityTypeToCount.get(activityType);
        fileStream.println(
            "Average number of "
                + activityType
                + "-activities: "
                + averageNumberOfCurrentActivities);
        fileStream.println(
            "Average duration of "
                + activityType
                + "-activities: "
