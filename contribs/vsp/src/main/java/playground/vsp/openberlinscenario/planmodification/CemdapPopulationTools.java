...
public class CemdapPopulationTools {
  ...
  public void setActivityTypesAccordingToDurationAndMergeOvernightActivities(
      Population population, double timeCategorySize, double dayStartTime) {
    log.info(
        "First, setting activity types according to duration (time bin size: "
            + timeCategorySize
            + ")");
  ...
  private static void setActivityTypesAccordingToDuration(
      Plan plan, double timeCategorySize, double dayStartTime) {
    ...
    for (PlanElement pE : plan.getPlanElements()) {
      if (pE instanceof Activity) {
        ...
        if (firstActivity) {
          ...
          } else if (act.getMaximumDuration().seconds() > 0.
              && act.getMaximumDuration().seconds() <= 24. * 3600) {
          ...
          if (act.getAttributes()
                  .getAttribute(CemdapStopsParser.CEMDAP_STOP_DURATION_S_ATTRIBUTE_NAME)
              == null) {
            log.warn(plan.toString());
            log.warn(plan.getPlanElements().toString());
            throw new RuntimeException(
                "All activities (except for the first one) should have a cemdapStopDuration_s attribute. Aborting...");
          } else {
            int cemdapStopDuration =
                (int)
                    act.getAttributes()
                        .getAttribute(CemdapStopsParser.CEMDAP_STOP_DURATION_S_ATTRIBUTE_NAME);
