...
public class Feature2Coord {
  private static final Logger LOG = LogManager.getLogger(Feature2Coord.class);

  public Feature2Coord() {}

  public final void assignCoords(
      Population population,
      int planNumber,
      ObjectAttributes personZoneAttributes,
      Map<String, Geometry> zones,
      Map<Id<Person>, Coord> homeZones,
      boolean allowVariousWorkAndEducationLocations,
      CorineLandCoverData corineLandCoverData) {
    int counter = 0;
    ...
    for (Person person : population.getPersons().values()) {
      ...
      for (PlanElement planElement : person.getPlans().get(planNumber).getPlanElements()) {
        if (planElement instanceof Activity) {
          ...
          String zoneId =
              (String)
                  personZoneAttributes.getAttribute(
                      person.getId().toString(), CemdapStopsParser.ZONE + activityIndex);
          if (zoneId == null) {
            LOG.error(
                "Person with ID "
                    + person.getId()
                    + ": Object attribute '"
                    + CemdapStopsParser.ZONE
                    + activityIndex
                    + "' not found.");
          }
  ...
  private Coord getCoord(
      CorineLandCoverData corineLandCoverData, Geometry geometry, String activityType) {
    Coord coord;
    if (corineLandCoverData == null) {
      coord = Cemdap2MatsimUtils.getRandomCoordinate(geometry);
    } else {
      coord =
          corineLandCoverData.getRandomCoord(
              geometry,
              activityType.equals("home")
                  ? LandCoverUtils.LandCoverActivityType.home
                  : LandCoverUtils.LandCoverActivityType.other);
    }
    return coord;
  }

  public final void assignHomeCoords(
      Population population,
      ObjectAttributes personZoneAttributes,
      Map<String, Geometry> zones,
      Map<Id<Person>, Coord> homeZones,
