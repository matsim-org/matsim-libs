...
/**
 * This class creates a children-only population of a study region (in Germany) based on the Zensus
 * and the Pendlerstatistik. Only meant as as supplement ot an already existing adults-only
 * population.
 *
 * @author dziemke
 */
public class SynPopCreatorChildren {
  ...
  public static void main(String[] args) {
    ...
    String outputBase =
        "../../shared-svn/studies/countries/de/open_berlin_scenario/be_5/cemdap_input/500/";

    // Parameters
    List<String> idsOfFederalStatesIncluded =
        Arrays.asList("11", "12"); // 11=Berlin, 12=Brandenburg

    SynPopCreatorChildren demandGeneratorCensus =
        new SynPopCreatorChildren(censusFile, outputBase, idsOfFederalStatesIncluded);
  ...
  public SynPopCreatorChildren(
      String censusFile, String outputBase, List<String> idsOfFederalStatesIncluded) {
    ...
    for (Person person : inputPopulation.getPersons().values()) {
      ...
      clonedPerson
          ...
          .putAttribute(
              ...
              person.getAttributes().getAttribute(CEMDAPPersonAttributes.gender.toString()));
      clonedPerson
          .getAttributes()
          .putAttribute(
              CEMDAPPersonAttributes.age.toString(),
              person.getAttributes().getAttribute(CEMDAPPersonAttributes.age.toString()));
  ...
  private void createChildren(
    ...
    for (int i = 0; i < numberOfPersons; i++) {
      ...
      Id<Person> personId =
          Id.create(
              counter + i + "01", Person.class); // TODO Currently only singel-person households
