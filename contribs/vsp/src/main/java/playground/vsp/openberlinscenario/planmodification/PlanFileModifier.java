...
/**
 * @author dziemke
 *     <p>Reads in a plans file and copies persons with their plans into a new plans file according
 *     to configurable parameters. Then writes new plans file to a given location.
 */
public class PlanFileModifier {
  private static final Logger LOG = LogManager.getLogger(PlanFileModifier.class);
  ...
  private boolean
      onlyConsiderPeopleAlwaysGoingByCar; // TODO A leftover from an early, quite specific case;
  ...
  public static void main(String[] args) {
    ...
    // "../../upretoria/data/capetown/scenario_2017/original/population.xml.gz";
    //		String outputPlansFile =
    // "../../upretoria/data/capetown/scenario_2017/population_32734.xml.gz";
    ...
    String outputPlansFile =
        "../../runs-svn/open_berlin_scenario/b5_22/b5_22.output_plans_no_links.xml.gz";

    //		String inputPlansFile =
    // "../../shared-svn/studies/countries/de/open_berlin_scenario/be_5/cemdap_input/502/plans1.xml.gz";
    //		String outputPlansFile =
    // "../../shared-svn/studies/countries/de/open_berlin_scenario/be_6/population/plans_10000.xml.gz";
    ...
    PlanFileModifier planFileModifier =
        new PlanFileModifier(
            inputPlansFile,
            outputPlansFile,
            selectionProbability,
            onlyTransferSelectedPlan,
            considerHomeStayingAgents,
            includeNonSelectedStayHomePlans,
            onlyConsiderPeopleAlwaysGoingByCar,
            maxNumberOfAgentsConsidered,
            removeLinksAndRoutes,
            ct);

    planFileModifier.modifyPlans();
  }

  public PlanFileModifier(
      String inputPlansFile,
      String outputPlansFile,
      double selectionProbability,
      boolean onlyTransferSelectedPlan,
      boolean considerHomeStayingAgents,
      boolean includeNonSelectedStayHomePlans,
      boolean onlyConsiderPeopleAlwaysGoingByCar,
      int maxNumberOfAgentsConsidered,
      boolean removeLinksAndRoutes,
      CoordinateTransformation ct) {
  ...
  public void modifyPlans() {
    ...
    for (Person person : population.getPersons().values()) {
      if (agentCounter < maxNumberOfAgentsConsidered) {
        ...
        // If selected according to all criteria, create a copy of the person and add it to new
        // population
        if (copyPerson) {
  ...
  private void createPersonAndAddToPopulation(
      Population population, Person person, Plan selectedPlan) {
    ...
    for (String attributeKey : person.getAttributes().getAsMap().keySet()) {
      person2
          .getAttributes()
          .putAttribute(attributeKey, person.getAttributes().getAttribute(attributeKey));
    }
