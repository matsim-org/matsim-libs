...
public class CemdapOutput2MatsimPlansConverter {
  ...
  public static void main(String[] args) throws IOException {
    // Local use
    //		String cemdapDataRoot =
    // "../../shared-svn/studies/countries/de/open_berlin_scenario/cemdap_output/";
    String cemdapDataRoot =
    ...
    // "../../shared-svn/studies/countries/de/open_berlin_scenario/matsim_initial/" +
    // numberOfPlansFile + "/";
    String outputDirectory =
        "../../shared-svn/projects\\nemo_mercator\\data\\cemdap_output\\2018-01-19\\matsim_input\\";
    String zonalShapeFile =
        "../../shared-svn/studies/countries/de/open_berlin_scenario/input/shapefiles/2013/gemeindenLOR_DHDN_GK4.shp";
    //		String zonalShapeFile =
    ...
    boolean assignCoordinatesToActivities =
        true; // if set to false, the zone id will be attached to activity types and a fake
    ...
    if (args.length != 0) {
      ...
      useLandCoverData =
          Boolean.parseBoolean(
              args[7]); // I think, it is not necessary, one can provide landCoverFile or not. Amit
      // Oct'17
      // Not providing the landCoverFile, i.e. only assigning "", does cause trouble to the counting
      // of the arguments -> IndexOutOfBoundException
      // So, having the switched based on a filename that is there or not there, does not seem to be
      // a good option either, dz, apr'18
    ...
    convert(
        cemdapDataRoot,
        numberOfFirstCemdapOutputFile,
        numberOfPlans,
        outputDirectory,
        shapeFileToFeatureKey,
        allowVariousWorkAndEducationLocations,
        addStayHomePlan,
        useLandCoverData,
        landCoverFile,
        stopFile,
  ...
  public static void convert(
      String cemdapDataRoot,
      int numberOfFirstCemdapOutputFile,
      int numberOfPlans,
      String outputDirectory,
      Map<String, String>
          shapeFileToFeatureKey, // ensures, unique shape files with same or different featureKey.
      // Amit Nov'17
      boolean allowVariousWorkAndEducationLocations,
      boolean addStayHomePlan,
      boolean useLandCoverData,
      ...
      boolean assignCoordinatesToActivities,
      int activityDurationThreshold_s)
      throws IOException {
    ...
    if (landCoverFile != null && useLandCoverData) {
      corineLandCoverData =
          new CorineLandCoverData(landCoverFile, simplifyGeometries, combiningGeoms);
    }
    ...
    for (Map.Entry<String, String> entry : shapeFileToFeatureKey.entrySet()) {
      for (SimpleFeature feature : ShapeFileReader.getAllFeatures(entry.getKey())) {
        ...
        String shapeId =
            Cemdap2MatsimUtils.removeLeadingZeroFromString(
                (String) feature.getAttribute(entry.getValue()));
        // TODO check if removal of leading zero is always valid
        // It was necessary since code may interpret number incorrectly if leading zero present; dz,
        // jul'17
    ...
    // Additional information from files read in below (e.g. Persons) would be required to take
    // person-specific information like age
    // along for later analyses; dz, jul'17
    //		CemdapPersonParser cemdapPersonParser = new CemdapPersonParser();
    //		cemdapPersonParser.parse(cemdapDataRoot + numberOfFirstCemdapOutputFile + "/" +
    // cemdapAdultsFilename, personsIds);
    //		cemdapPersonParser.parse(cemdapDataRoot + numberOfFirstCemdapOutputFile + "/" +
    // cemdapChildrenFilename, personsIds);
    CemdapActivityParser cemdapActivityParser = new CemdapActivityParser();
    cemdapActivityParser.parse(
        cemdapDataRoot + numberOfFirstCemdapOutputFile + "/" + activityFile, personHomeMap);
    ...
    for (int planNumber = 0; planNumber < numberOfPlans; planNumber++) {
      ...
      cemdapStopsParser.parse(
          cemdapStopsFilesMap.get(planNumber),
          planNumber,
          population,
          personZoneAttributesMap.get(planNumber),
          outputDirectory);

      // Commenting this for the time being; it does not do anything if the activity file is not
      // considered on top of the stops file, dz,aa, sep'17
      // Add a stay-home plan for those people who have no stops (i.e. no travel) in current stop
      // file
      // Following is required to add persons who just stays at home. Such persons does not appear
      // in the Stops.out file. dz, aa Oct'17
      ...
      for (Id<Person> personId : personHomeMap.keySet()) {
        ...
        if (person.getPlans().size() <= planNumber) {
          ...
          stayHomePlan.addActivity(
              population
                  .getFactory()
                  .createActivityFromCoord(
                      ActivityTypes.HOME, new Coord(-1.0, -1.0))); // TODO maybe improve later
          person.addPlan(stayHomePlan);
          personZoneAttributesMap
              .get(planNumber)
              .putAttribute(
                  personId.toString(),
                  "zone" + "0",
                  personHomeMap.get(personId)); // TODO maybe improve later
          counter++;
        }
      }
      LOG.info(
          "For "
              + counter
              + " persons who were not assigned with an activity-travel by CEMDAP (i.e. persons not contained in stops.out), "
              + "a stay-home plans was added. Plan number is "
              + planNumber
              + ".");
    }
    ...
    if (assignCoordinatesToActivities) {
      ...
      feature2Coord.assignHomeCoords(
          population, personZoneAttributesMap.get(0), zones, homeZones, corineLandCoverData);

      // Assign coordinates to all other activities
      for (int planNumber = 0; planNumber < numberOfPlans; planNumber++) {
        feature2Coord.assignCoords(
            population,
            planNumber,
            personZoneAttributesMap.get(planNumber),
            zones,
      ...
      //			LOG.warn("No coordinate will be assigned to activities. The zone id for each activity
      // will be concatenated at the end of activityType seperated by '_'.");
      LOG.warn(
          "No coordinate will be assigned to activities. The zone id for each activity will be stored with activity attributes.");
      // for EXTRA LARGE scenario, one can assign coordinates after sampling which would be at least
      // '1/sampleSize' times faster. Amit Oct'17
    ...
    if (addStayHomePlan) {
      ...
      for (Person person : population.getPersons().values()) {
        ...
        Activity2.getAttributes()
            .putAttribute(
                activityZoneId_attributeKey,
                firstActivity.getAttributes().getAttribute(activityZoneId_attributeKey));
