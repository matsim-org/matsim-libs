...
/**
 * Every person in sampled plans file has exactly one plan. This class reads rest plans files (100%
 * scenario) and extract the plans for sampled plans. Eventually, this writes out a sampled plans
 * file with full choice set for each person.
 *
 * <p>Created by amit on 24.10.17.
 */
public class SampledPlansMerger {

  public static void main(String[] args) {
    String plansBaseDir =
        "../../shared-svn/studies/countries/de/open_berlin_scenario/be_5/population/500";
    ...
    //      String sampledPlans = plansBaseDir + "/" + plansFolderPrefix + numberOfFirstPlansFile +
    // plansFolderSuffix + "/plans_10pct.xml.gz"; // hardcoded until 2018-10-24
    String sampledPlans =
        plansBaseDir
            + "/"
            + plansFolderPrefix
            + numberOfFirstPlansFile
            + plansFolderSuffix
            + "/"
            + firstPlanFileName;
    ...
    for (int planNumber = 1; planNumber < numberOfPlans; planNumber++) {
      //          String unsampledPlans = plansBaseDir + "/" + plansFolderPrefix +
      // (numberOfFirstPlansFile + planNumber) + plansFolderSuffix + "/plans.xml.gz"; // hardcoded
      // until 2018-10-24
      String unsampledPlans =
          plansBaseDir
              + "/"
              + plansFolderPrefix
              + (numberOfFirstPlansFile + planNumber)
              + plansFolderSuffix
              + "/"
              + otherPlansFileNames;
      ...
      for (Person sampledPerson : sampledPop.getPersons().values()) {
        ...
        if (person == null) {
          throw new RuntimeException(
              "Sampled person "
                  + sampledPerson.getId()
                  + " is not found in unsampled plans "
                  + unsampledPlans
                  + ".");
          ...
          throw new RuntimeException(
              "Sampled person "
                  + sampledPerson.getId()
                  + " does not have any plan in his choice set.");
          ...
          sampledPerson.addPlan(
              firstPlan); // The first plan must be added (It can be a "normal" plan or a stay-home
          // plan.)

          int lengthOfPlanElement = person.getPlans().get(1).getPlanElements().size();
          if (lengthOfPlanElement
              == 1) { // The second plan in a given input population must be a stay-home plan if
            // this class is applied correctly.
          } else {
            throw new RuntimeException(
                "The second plan of the unsampled person "
                    + person.getId()
                    + " must be stay-home plan. The number of plan elements, however, is "
                    + lengthOfPlanElement
                    + ".");
          }
        } else {
          throw new RuntimeException(
              "Unsampled person "
                  + sampledPerson.getId()
                  + " should have less than 3 plans in choice set. It has "
      ...
      for (Person person : sampledPop.getPersons().values()) {
        ...
        Activity firstActivity =
            (Activity)
                firstPlan
                    .getPlanElements()
                    .get(0); // Get first (i.e. presumably "home") activity from agent's first plan

        Plan stayHomePlan = sampledPop.getFactory().createPlan();
        // Create new activity with type and coordinates (but without end time) and add it to
        // stay-home plan
        Activity Activity2 =
            sampledPop
                .getFactory()
                .createActivityFromCoord(firstActivity.getType(), firstActivity.getCoord());
        Activity2.getAttributes()
            .putAttribute(
                CemdapOutput2MatsimPlansConverter.activityZoneId_attributeKey,
                firstActivity
                    .getAttributes()
                    .getAttribute(CemdapOutput2MatsimPlansConverter.activityZoneId_attributeKey));
