...
public class RunEquil {
  ...
  public static void main(String[] args) {
    ...
    final LinkActivityOccupancyCounter atNight =
        new LinkActivityOccupancyCounter(scenario.getPopulation(), 0, 6 * 60 * 60);
    controler.getEvents().addHandler(atNight);

    // This counter looks at a time interval where everyone is travelling, so occupancy should be 0
    // everywhere
    final LinkActivityOccupancyCounter duringMorningCommute =
        new LinkActivityOccupancyCounter(
            scenario.getPopulation(), 6 * 60 * 60 + 1, 6 * 60 * 60 + 10);
    ...
    final LinkActivityOccupancyCounter atWork =
        new LinkActivityOccupancyCounter(
            scenario.getPopulation(), 6 * 60 * 60 + 1, 6 * 60 * 60 + 30 * 60);
    controler.getEvents().addHandler(atWork);

    // This one briefly listens in when everyone is already at home again
    final LinkActivityOccupancyCounter nextNight =
        new LinkActivityOccupancyCounter(
            scenario.getPopulation(), 13 * 60 * 60 + 45 * 60, 13 * 60 * 60 + 47 * 60);
    controler.getEvents().addHandler(nextNight);

    // During this window, most people are already home, a few are still working, and one person is
    // actually seen both at work and at home.
    final LinkActivityOccupancyCounter middle =
        new LinkActivityOccupancyCounter(scenario.getPopulation(), 27810, 31440);
    controler.getEvents().addHandler(middle);

    // This one listens all day
    final LinkActivityOccupancyCounter allDay =
        new LinkActivityOccupancyCounter(scenario.getPopulation());
