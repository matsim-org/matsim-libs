...
class BuildingEnergyAnalyzerMain {
  ...
  public static void main(String[] args) {
    ...
    if (args.length < 13) {
      throw new IllegalArgumentException(
          "expecting min 12 arguments {inputpath, outputPath, timeSliceSize, tmax, "
    ...
    OutputDirectoryLogging.initLogging(
        new OutputDirectoryHierarchy(
            outputPath,
            BuildingEnergyAnalyzer.class.getSimpleName(),
    ...
    // run
    BuildingEnergyAnalyzer analyzer =
        new BuildingEnergyAnalyzer(
            inputPath, outputPath, td, tmax, baseRun, runs, homeType, workType, factory);
  ...
  private static class OfficeEnergyConsumptionRuleImpl implements BuildingEnergyConsumptionRule {
     ...
     */
    OfficeEnergyConsumptionRuleImpl(
        double td,
        double baseLoadPerPerson,
        double additionalLoadPerPerson,
        double someCoefficient) {
      this.td = td;
    ...
    public double getEnergyConsumption_kWh(double maxSize, double currentOccupancy) {
      if (currentOccupancy > maxSize)
        throw new RuntimeException("more persons on the link than expected");
      ...
      return (td
          ...
          * (baseload
              + additionalLoad
                  * (1 - Math.exp(-1.0 * currentOccupancy / maxSize * someCoefficient))));
    }
  }
  ...
  private static class HomeEnergyConsumptionRuleImpl implements BuildingEnergyConsumptionRule {
     ...
     */
    HomeEnergyConsumptionRuleImpl(
        double td, double baseLoadPerPerson, double additionalLoadPerPerson) {
      this.td = td;
      this.baseLoad = baseLoadPerPerson;
      this.additional = additionalLoadPerPerson;
    }
