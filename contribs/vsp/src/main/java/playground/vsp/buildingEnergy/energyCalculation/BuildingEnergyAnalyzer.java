...
/**
 * Analyzes the base-run and all other runs for the agents energy consumption. The class assumes
 * they all use the same network! Only agents that perform both, home and work-activities, will be
 * analyzed.
 *
 * @author droeder
 */
public class BuildingEnergyAnalyzer {
  ...
  public BuildingEnergyAnalyzer(
    ...
    log.warn(
        "you may define more activity-types than home and work. Note, the analysis of probabilities and "
  ...
  public void run() {
    if (berlin) {
      log.warn(
          "Plans and events will be modified. All ``not specified'' activities and events will be replaced "
              + "with "
              + homeType
              + "-activities. This will work for Berlin-Scenario only! Make sure this is what you want!");
    }
    initTimeBins();
    runRawAnalysis();
    writeRawData();
  ...
  private void runRawAnalysis() {
    ...
    this.run2PopulationStats =
        new HashMap<String, BuildingEnergyMATSimDataReader.PopulationStats>();
  ...
  private void analyseSingleRunRaw(String runId, int iter) {
    ...
    BuildingEnergyMATSimDataReader reader =
        new BuildingEnergyMATSimDataReader(timeBins, iter, tMax, actTypes);
  ...
  private void writeRawData() {
    new BuildingEnergyRawDataWriter(
            baseRunId, run2type2RawOccupancy, run2PopulationStats, timeBins, links)
        .write(outputPath);
  }
  ...
  private void createAggregatedAnalysis() {
    ...
    BuildingEnergyAggregatedEnergyConsumptionCalculator calculator =
        new BuildingEnergyAggregatedEnergyConsumptionCalculator(rules, links, timeBins, maxPerLink);
    this.energyConsumption = calculator.run(run2type2RawOccupancy);
    this.probabilities =
        new BuildingEnergyActivityProbabilityCalculator(links)
            .run(run2type2RawOccupancy, run2PopulationStats);
  }
