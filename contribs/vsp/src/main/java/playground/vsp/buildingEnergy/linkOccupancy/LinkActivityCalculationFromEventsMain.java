...
/**
 * Simple main-routine that parse an eventsfile, using {@link LinkActivityOccupancyCounter}. The
 * generated data is written to csv-tables.
 *
 * @author droeder
 */
public final class LinkActivityCalculationFromEventsMain {
  ...
  private static final Logger log =
      LogManager.getLogger(LinkActivityCalculationFromEventsMain.class);
  ...
  public static void main(String[] args) {
    ...
    if (args.length != 7) {
      throw new IllegalArgumentException(
          "expecting 6 arguments {networkFile, plansFile, eventsFile, outputPath, timeSliceSize, tmax, runId");
    }
    ...
    Map<String, LinkActivityOccupancyCounter> home =
        initOccupancyCounter("home", td, tmax, sc.getPopulation());
    Map<String, LinkActivityOccupancyCounter> work =
        initOccupancyCounter("work", td, tmax, sc.getPopulation());
  ...
  private static Map<String, LinkActivityOccupancyCounter> initOccupancyCounter(
    ...
    Map<String, LinkActivityOccupancyCounter> map =
        new HashMap<String, LinkActivityOccupancyCounter>();
    for (int i = 0; i < tmax; i += td) {
      map.put(String.valueOf(i), new LinkActivityOccupancyCounter(p, i, i + td, string));
    }
    map.put(
        ">" + String.valueOf(tmax),
        new LinkActivityOccupancyCounter(p, tmax, Integer.MAX_VALUE, string));
  ...
  private static void dump(
      String outputPath,
      String name,
      Map<String, LinkActivityOccupancyCounter> values,
      List<Id> linkIds,
      int tmax,
      int td,
      String runId) {
    ...
    BufferedWriter w =
        IOUtils.getBufferedWriter(outputPath + runId + "." + PREFIX + "." + name + ".csv.gz");
    try {
      // write header
      w.write("linkId;");
      for (int i = 0; i < tmax; i += td) {
