...
/**
 * Calculates the probabilities of performing a certain activity for a certain {@link
 * LinkActivityOccupancyCounter}
 *
 * @author droeder
 */
class BuildingEnergyActivityProbabilityCalculator {
  ...
  private static final Logger log =
      LogManager.getLogger(BuildingEnergyActivityProbabilityCalculator.class);
  ...
  /**
   * @return the probabilities of performing a certain activity for a certain {@link
   *     LinkActivityOccupancyCounter}
   */
  private Map<String, Double> calcActivityProbability(
      double maxOfActivityType, Map<String, LinkActivityOccupancyCounter> counter) {
  ...
  public ActivityProbabilities run(
    ...
    Map<String, Map<String, Map<String, Double>>> runs =
    ...
    for (Entry<String, Map<String, LinkOccupancyStats>> e : run2Type2RawOccupancy.entrySet()) {
      ...
      for (Entry<String, LinkOccupancyStats> ee : e.getValue().entrySet()) {
        types.put(
            ee.getKey(),
            calcActivityProbability(
                run2PopulationStats.get(e.getKey()).getHomeAndWorkCnt(), ee.getValue().getStats()));
      }
