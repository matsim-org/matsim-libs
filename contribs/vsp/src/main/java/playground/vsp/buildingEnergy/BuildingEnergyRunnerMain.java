...
public abstract class BuildingEnergyRunnerMain {
  ...
  private static final Logger log = LogManager.getLogger(BuildingEnergyRunnerMain.class);
  ...
  public static void main(String[] args) {
    ...
    if (args.length == 0) {
      args =
          new String[] {
            "E:\\VSP\\svn\\shared-svn\\studies\\droeder\\buildingEnergy\\baseConfig.xml",
  ...
  // static class which plug the analysis to the services and generates some more output (distance-
  // & tt-shares)
  private static class MyControlerListener
    ...
    public void notifyIterationEnds(IterationEndsEvent event) {
      if (event.getIteration() == event.getServices().getConfig().controller().getLastIteration()) {
        ...
        calcWriteDistanceDistribution(
            analyzer.getTraveller(), path + prefix + "distanceShare.csv.gz");
