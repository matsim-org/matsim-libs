...
/**
 * This class aims to increase the diversity of plans in an agent's choice set. This is achieved by
 * removing a plan once it is found to be <i>similar</i> to another plan of the choice set. If no
 * plan is considered <i>similar</i> to another plan the standard MATSim behavior is preserved, i.e.
 * as of May 2014 the worst scored plan will be removed from the choice set. As first tests
 * indicate, this effectively prevents agents from over-adapting, i.e. arriving at a stop only
 * seconds before the bus departs.
 *
 * <p>Details:<br>
 * A plan is considered <i>similar</i> to another plan if all of the similarity checkers consider
 * the plan <i>similar</i>. For example, two plans with the same activity end times are
 * <i>similar</i> if they also use the same mode of transport. If one of the similarity checkers
 * fails, the plans are considered <i>dissimilar</i>. Note that currently only activity end times
 * are checked for. See {@link DiversityGeneratingPlansRemoverANIK#similarity(Plan, Plan,
 * StageActivityTypes, double)} for information on adding further similarity checkers. From two
 * plans considered as <i>similar</i> the older one is preserved and the newer one will be deleted.
 * The comparison stops at this point and further plans are not checked for. Note that this class
 * can only delete one plan at a time. Multiple iterations are required in case more than two plans
 * would be considered as <i>similar</i>. Note that the similarity checks depend on the plans of the
 * choice set and may thus yield different results once a plan is removed. Further note that the
 * order in which plans are compared to each other depends on the List implementation in which the
 * plans are stored. As of May 2014 this is an ArrayList ({@link Person#plans}).
 *
 * @author aneumann
 * @author ikaddoura
 */
public final class DiversityGeneratingPlansRemoverANIK extends AbstractPlanSelector {
  ...
  /**
   * @return Map giving a weight for each plan. Higher weights have a higher probability of being
   *     selected.
   */
  @Override
  protected Map<Plan, Double> calcWeights(List<? extends Plan> plans) {
    ...
    for (Plan plan1 : plans) {
      for (Plan plan2 : plans) {
        ...
        if (plan1 == plan2) {
          ...
          // check two plans for similarity TODO Should be a kind of builder passed further down
          // that configures all similarity checkers
          if (similarity(plan1, plan2, this.similarTimeInterval)) {
            // This one is similar. Tag it as to be removed and return. We only can remove one plan.
            // So we remove the newer one.
            map.put(plan2, 1.);
            return map;
          } else {
    ...
    // In case there is no similar plan, fall back to the standard behavior of MATSim (as of May,
    // 2014). Remove worst plan from choice set.
  ...
  /**
   * Compare two plans for similarity. A plan is considered similar to another plan, if all
   * similarity checkers consider it being similar.<br>
   * TODO The checkers should be implemented as individual classes being configured once with a
   * builder passed to them.
   *
   * @param plan1 First plan to be compared.
   * @param plan2 Second plan to be compared.
   * @param stageActivities Type of activities that should be ignored, e.g. {@link
   *     PtConstants#TRANSIT_ACTIVITY_TYPE}.
   * @param similarTimeInterval The interval in which two activities are considered as being the
   *     same. TODO Builder implemenation
  ...
  /**
   * Compare two plans for similarity. A plan is considered similar to another plan, if all
   * activities are within the specified interval.
   *
   * @param plan1 First plan to be compared.
   * @param plan2 Second plan to be compared.
   * @param stageActivities Type of activities that should be ignored, e.g. {@link
