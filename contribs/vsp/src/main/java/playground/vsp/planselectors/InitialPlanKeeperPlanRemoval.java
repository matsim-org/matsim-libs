...
public class InitialPlanKeeperPlanRemoval implements PlanSelector<Plan, Person> {
  ...
  @Inject
  public InitialPlanKeeperPlanRemoval(
      ReplanningConfigGroup replanningConfigGroup, Population population) {
    this.delegate = new WorstPlanForRemovalSelector();
    if (replanningConfigGroup.getMaxAgentPlanMemorySize() < 12) {
      LogManager.getLogger(InitialPlanKeeperPlanRemoval.class)
          .warn(
              "A plans remover is used which keeps the initial plans or at least their copy \n "
                  + "and maximum number of plans in the choice set is limited to "
                  + replanningConfigGroup.getMaxAgentPlanMemorySize()
    ...
    for (Person person : population.getPersons().values()) {
      for (int index = 0; index < person.getPlans().size(); index++) {
        ...
        plan.getAttributes()
            .putAttribute(
                InitialPlanKeeperPlanRemoval.plan_attribute_name,
                InitialPlanKeeperPlanRemoval.plan_attribute_prefix + index);
      }
    }
  }
  ...
  public Plan selectPlan(HasPlansAndId<Plan, Person> member) {
    ...
    // Imp: putting plans back may change the positions of the plans in the choice set, assuming
    // that it will not change the behaviour somewhere else.
    while (true) {
      ...
      if (value == null)
        throw new RuntimeException(
            "The value for the attribute key "
                + plan_attribute_name
                + " does not exist in the plan for person "
      ...
      int occurrences =
          (int)
              member.getPlans().stream()
                  .filter(
                      pl ->
                          Objects.equals(
                              pl.getAttributes().getAttribute(plan_attribute_name), value))
                  .count();
