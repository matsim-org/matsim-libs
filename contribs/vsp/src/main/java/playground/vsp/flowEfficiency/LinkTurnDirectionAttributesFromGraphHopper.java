...
/**
 * This class sets link attributes such that information on turning direction from each link to each
 * possible outlink can be obtained. This is done by converting Graphhopper routing instructions.
 *
...
public class LinkTurnDirectionAttributesFromGraphHopper implements Callable<Integer> {
  ...
  private static final Logger log =
      LogManager.getLogger(LinkTurnDirectionAttributesFromGraphHopper.class);
  ...
  private Set<Id<Link>> debug = Set.of(Id.createLinkId("debugLink"));
  ...
  private LinkTurnDirectionAttributesFromGraphHopper(
      String osmFilePath, String matsimNetworkFilePath, String networkCrs) {
    this.osm = Path.of(osmFilePath);
  ...
  public static void main(String[] args) throws Exception {
    LinkTurnDirectionAttributesFromGraphHopper main =
        new LinkTurnDirectionAttributesFromGraphHopper(args[0], args[1], args[2]);
    main.call();
  }
    ...
    network.getLinks().values().parallelStream()
        .forEach(
            link -> {
              ...
              if (!map.isEmpty()) link.getAttributes().putAttribute("turns", map);
  ...
  private static GraphHopper createGraphHopperInstance(String ghLoc) {
    ...
    // now this can take minutes if it imports or a few seconds for loading of course this is
    // dependent on the area you import
    hopper.importOrLoad();
    return hopper;
  }
  ...
  private TurnDirection routing(Link fromLink, Link toLink) {
    ...
    if (debug.contains(fromLink.getId())) {
      ...
      System.out.println(
          "### TO " + toLink.getId() + " = " + guessDirection(path.getInstructions()));
      System.out.println("\\/\\/\\/\\/\\/\\/\\/");
  ...
  /**
   * Retrieve coordinate used for routing given a link. The coordinate is scaled between from and to
   * node and given factor.
   */
  ...
  private TurnDirection guessDirection(InstructionList inst) {
    ...
    if (inst.isEmpty()) return TurnDirection.STRAIGHT;

    boolean hasLeft =
        inst.stream()
            .mapToInt(Instruction::getSign)
            .anyMatch(s -> s == Instruction.TURN_LEFT || s == Instruction.TURN_SHARP_LEFT);
    boolean hasRight =
        inst.stream()
            .mapToInt(Instruction::getSign)
            .anyMatch(s -> s == Instruction.TURN_RIGHT || s == Instruction.TURN_SHARP_RIGHT);
    boolean hasRoundabout =
        inst.stream()
            .mapToInt(Instruction::getSign)
            .anyMatch(s -> s == Instruction.LEAVE_ROUNDABOUT || s == Instruction.USE_ROUNDABOUT);

    // we do deliberately not check for u-turns as the deviation in precision of the MATSim and the
    // OSM network is too big. Most U-Turn instructions consist
    // of one or more turn instructions anyways, that way we should at least end up and not declare
    // them as STRAIGHT.

    // conflicting directions
    if (hasLeft && hasRight) return TurnDirection.UNKNOWN;
    else if (hasLeft) return TurnDirection.LEFT;
    else if (hasRight || hasRoundabout) return TurnDirection.RIGHT;
