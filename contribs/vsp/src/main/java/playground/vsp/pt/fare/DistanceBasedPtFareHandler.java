...
public class DistanceBasedPtFareHandler implements ActivityStartEventHandler {
  ...
  public void handleEvent(ActivityStartEvent event) {
    if (event.getActType().equals(PtConstants.TRANSIT_ACTIVITY_TYPE)) {
      personDepartureCoordMap.computeIfAbsent(
          event.getPersonId(),
          c ->
              event
                  .getCoord()); // The departure place is fixed to the place of first pt interaction
      // an agent has in the whole leg
      personArrivalCoordMap.put(
          event.getPersonId(),
          event.getCoord()); // The arrival stop will keep updating until the agent start a real
      ...
      if (personDepartureCoordMap.containsKey(personId)) {
        ...
        events.processEvent(
            new PersonMoneyEvent(
                ...
                event.getPersonId(),
                -fare,
                PtFareConfigGroup.PT_FARE,
                DistanceBasedPtFareParams.PT_FARE_DISTANCE_BASED,
  ...
  public static double computeFare(
      double distance,
      double longTripThreshold,
      double minFare,
      double shortTripIntercept,
      double shortTripSlope,
      double longTripIntercept,
      double longTripSlope) {
