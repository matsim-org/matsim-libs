...
public class PtFareConfigGroup extends ReflectiveConfigGroup {
  ...
  public enum PtFareCalculationModels {
    distanceBased
  } // More to come (e.g. zone based, hybrid...)

  private PtFareCalculationModels ptFareCalculation =
      PtFareCalculationModels.distanceBased; // Use distance based calculation by default
  private boolean applyUpperBound = true;
  @PositiveOrZero private double upperBoundFactor = 1.5;
  ...
  public Map<String, String> getComments() {
    ...
    map.put(
        PT_FARE_CALCULATION,
        "PT fare calculation scheme. Current implementation: distanceBased (more to come...)");
    map.put(
        APPLY_UPPER_BOUND,
        "Enable the upper bound for daily PT fare to count for ticket subscription. Input value: true or false");
    map.put(
        UPPER_BOUND_FACTOR,
        "When upper bound is applied, upperBound  = upperBoundFactor * max Fare of the day. "
