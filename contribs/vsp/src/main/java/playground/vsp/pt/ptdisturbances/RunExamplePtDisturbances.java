...
public class RunExamplePtDisturbances {
  ...
  private static final URL configURL =
      IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("ptdisturbances"), "config.xml");
  ...
  public static Controler prepareControler(Scenario scenario) {
    ...
    {
      QSimComponentsConfigGroup qsimComponentsConfig =
          ConfigUtils.addOrGetModule(controler.getConfig(), QSimComponentsConfigGroup.class);
  ...
  static void run(Config config) {
    ...
    networkChangeEvent1.setFreespeedChange(
        new ChangeValue(ChangeType.ABSOLUTE_IN_SI_UNITS, link.getLength() / 3600));
    ...
    networkChangeEvent2.setFreespeedChange(
        new ChangeValue(ChangeType.ABSOLUTE_IN_SI_UNITS, 30. / 3.6));
                    ...
                    new BasicEventHandler() {
                      ...
                      public void handleEvent(Event event) {
                        if (event instanceof HasPersonId
                            && ((HasPersonId) event).getPersonId().toString().equals("b1810066")) {
  ...
  private static class DisturbanceAndReplanningEngine implements MobsimEngine {
    ...
    public void doSimStep(double now) {
      ...
      // replan after an affected bus has already departed -> pax on the bus are replanned to get
      // off earlier
      ...
      if ((int) now == replanTime) {
        ...
        TransitLine disturbedLine =
            scenario.getTransitSchedule().getTransitLines().get(disturbedLineId);
        Gbl.assertNotNull(disturbedLine);

        TransitRoute disturbedRoute =
            disturbedLine.getRoutes().get(Id.create("345", TransitRoute.class));
        ...
        for (Departure departure : disturbedRoute.getDepartures().values()) {
          if (departure.getDepartureTime() >= 7.5 * 3600.
              && departure.getDepartureTime() < 8.5 * 3600.) {
  ...
  static void replanPtPassengers(
      double now,
      final Id<TransitLine> disturbedLineId,
    ...
    for (MobsimAgent agent : (qsim).getAgents().values()) {
        ...
        if (pe instanceof Activity) {
          if (StageActivityTypeIdentifier.isStageActivity(((Activity) pe).getType())) {
            log.warn(
                "agent with ID="
                    + agent.getId()
                    + " is at stage activity of type="
                    + ((Activity) pe).getType());
          } else {
            log.warn(
                "agent with ID="
                    + agent.getId()
                    + " is at real activity of type="
                    + ((Activity) pe).getType());
          }
        } else if (pe instanceof Leg) {
          log.warn(
              "agent with ID=" + agent.getId() + " is at leg with mode=" + ((Leg) pe).getMode());
        }
      }
      ...
      Activity nextRealActivity =
          null; // would be nicer to use TripStructureUtils to find trips, but how can we get back
      // to the original plan to modify it?
      ...
      for (int ii = currentPlanElementIndex; ii < plan.getPlanElements().size(); ii++) {
        ...
        // Replanning the same trip twice could happen e.g. if first replanCurrentTrip is called and
        // keeps or re-inserts
        // a leg with the disturbed line. So on a later plan element (higher ii) of the same trip
        // replanCurrentTrip or
...

      {
        ...
        if (pe instanceof Activity) {
          if (StageActivityTypeIdentifier.isStageActivity(((Activity) pe).getType())) {
            ...
            internalInterface.unregisterAdditionalAgentOnLink(
                agent.getId(), agent.getCurrentLinkId());
          }
        }
