...
public class RunTransitRouteTrimmerBerlinExample {
  ...
  private static void designateBusStopsAsHubs(
      String outputPath, int bufferRadius, int hubReach, Scenario scenario) throws IOException {
    ...
    List<TransitStopFacility> railStations =
        allStations.stream()
    ...
    List<Id<TransitStopFacility>> railStationIds =
        railStations.stream().map(x -> x.getId()).collect(Collectors.toList());
    ...
    for (Coordinate coord : coordinates) {
      busStopsInBufferIds.addAll(
          StopId2CoordMap.entrySet().stream()
              .filter(x -> x.getValue().equals(coord))
              .map(x -> x.getKey())
              .collect(Collectors.toList()));
    }
    ...
    for (Id<TransitStopFacility> id : busStopsInBufferIds) {
      scenario
          ...
          .get(id)
          .getAttributes()
          .putAttribute("hub-reach", hubReach);
    }

    { // Write shape files for rail stops, buffers around rail stops, and bus stops within buffer
      writeGeometryCollection2ShapeFile(
          outputPath + "railStopGeo-300", ShapeType.POINT, (GeometryCollection) railStopsGeo);

      writeGeometryCollection2ShapeFile(
          outputPath + "railBufferGeo-300", ShapeType.POLYGON, (GeometryCollection) railBufferGeo);

      writeGeometryCollection2ShapeFile(
          outputPath + "busStopsInBufferGeo-300",
          ShapeType.POINT,
          (GeometryCollection) busStopsInBuffer);
    }
  }

  public static void writeGeometryCollection2ShapeFile(
      String fileName, ShapeType shapeType, GeometryCollection geometryCollection)
      throws IOException {
