...
/**
 * @author Chengqi Lu (luchengqi7) The parameters for the distance-based PT trip fare calculation.
 *     The default values are set based on the fitting results of the trip and fare data collected
 *     on September 2021 The values are based on the standard unit of meter (m) and Euro (EUR)
 */
public class DistanceBasedPtFareParams extends ReflectiveConfigGroup {
  ...
  @PositiveOrZero private double minFare = 2.0;
  @PositiveOrZero private double normalTripIntercept = 1.6;
  @PositiveOrZero private double normalTripSlope = 0.00017;
  @PositiveOrZero private double longDistanceTripThreshold = 50000.0;
  @PositiveOrZero private double longDistanceTripIntercept = 30.0;
  @PositiveOrZero private double longDistanceTripSlope = 0.00025;
  ...
  public Map<String, String> getComments() {
    ...
    map.put(
        MIN_FARE,
        "Minimum fare for a PT trip "
            + "(e.g. Kurzstrecke/short distance ticket in cities, ticket for 1 zone in rural areas)");
    map.put(
        NORMAL_TRIP_SLOPE,
        "Linear model y = ax + b: the value of a, for normal trips (e.g. within the city or region)");
    map.put(NORMAL_TRIP_INTERCEPT, "Linear model y = ax + b: the value of b, for normal trips");
    map.put(
        LONG_DISTANCE_TRIP_SLOPE,
        "Linear model y = ax + b: the value of a, for long distance trips (e.g. intercity trips)");
    map.put(
        LONG_DISTANCE_TRIP_INTERCEPT, "Linear model y = ax + b: the value of b, for long trips");
    map.put(
        LONG_DISTANCE_TRIP_THRESHOLD,
        "Threshold of the long trips in meters. Below this value, "
