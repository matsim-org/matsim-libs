...
public class TransitRouteTrimmer {
  ...
  public static Pair<TransitSchedule, Vehicles> deleteRoutesEntirelyInsideZone(
      TransitSchedule transitScheduleOld,
      Vehicles vehiclesOld,
    ...
    for (TransitLine line : transitScheduleOld.getTransitLines().values()) {
      ...
      if (lineNew.getRoutes().size() == 0 && removeEmptyLines) {
        log.info(
            lineNew.getId() + " does not contain routes. It will NOT be added to the schedule");
        continue;
      }
    ...
    log.info(
        "Old schedule contained "
            + transitScheduleOld.getTransitLines().values().size()
            + " lines.");
    log.info(
        "New schedule contains "
            + transitScheduleNew.getTransitLines().values().size()
            + " lines.");
  ...
  public static Pair<TransitSchedule, Vehicles> trimEnds(
      TransitSchedule transitScheduleOld,
      Vehicles vehiclesOld,
      Set<Id<TransitStopFacility>> stopsInZone,
      Set<Id<TransitLine>> linesToModify,
    ...
    for (TransitLine line : transitScheduleOld.getTransitLines().values()) {
      ...
      for (TransitRoute route : line.getRoutes().values()) {
        ...
        routeNew =
            modifyRouteTrimEnds(route, minimumRouteLength, stopsInZone, includeFirstStopWithinZone);
  ...
  public static Pair<TransitSchedule, Vehicles> skipStops(
      TransitSchedule transitScheduleOld,
      Vehicles vehiclesOld,
      Set<Id<TransitStopFacility>> stopsInZone,
      Set<Id<TransitLine>> linesToModify,
    ...
    for (TransitLine line : transitScheduleOld.getTransitLines().values()) {
      ...
      for (TransitRoute route : line.getRoutes().values()) {
        ...
        routeNew =
            modifyRouteSkipStopsWithinZone(
                route, minimumRouteLength, stopsInZone, includeFirstStopWithinZone);
  ...
  public static Pair<TransitSchedule, Vehicles> splitRoute(
      TransitSchedule transitScheduleOld,
      Vehicles vehiclesOld,
      Set<Id<TransitStopFacility>> stopsInZone,
      Set<Id<TransitLine>> linesToModify,
    ...
    for (TransitLine line : transitScheduleOld.getTransitLines().values()) {
      ...
      for (TransitRoute route : line.getRoutes().values()) {
        ...
        ArrayList<TransitRoute> routesNew =
            modifyRouteSplitRoute(
                route,
                stopsInZone,
                includeFirstStopWithinZone,
                allowHubsWithinZone,
                includeFirstHubInZone,
                allowableStopsWithinZone,
                vehiclesNew);
  ...
  // This will skip stops within zone. If beginning or end of route is within zone, it will cut
  // those ends off.
  private static TransitRoute modifyRouteSkipStopsWithinZone(
      TransitRoute routeOld,
      int minimumRouteLength,
      Set<Id<TransitStopFacility>> stopsInZone,
      boolean includeFirstStopWithinZone) {
  ...
  private static TransitRoute modifyRouteTrimEnds(
      TransitRoute routeOld,
      int minimumRouteLength,
      Set<Id<TransitStopFacility>> stopsInZone,
      boolean includeFirstStopWithinZone) {
  ...
  private static ArrayList<TransitRoute> modifyRouteSplitRoute(
      TransitRoute routeOld,
      Set<Id<TransitStopFacility>> stopsInZone,
      boolean includeFirstStopWithinZone,
      boolean allowHubsWithinZone,
    ...
    // Get list of hubs: each hub is represented [location, reach] where location is the index along
    // the route and
    // reach is the number of stops away the hub can be from the edge of the zone to still be
    // included.
    ...
    for (Integer[] pair : routeIndices) {
      ...
      if (allowHubsWithinZone && !hubLocationAndReach.isEmpty()) {
        ...
        // if there are hubs in a particular direction, but none are included in route because their
        // reaches
      ...
      // add first stop within zone if hub hasn't already extended the route in the respective
      // direction
      if (includeFirstStopWithinZone) {
    ...
    for (Integer[] pair : routeIndices2) {
      resultRoutes.add(
          createNewRouteModifiedId(routeOld, pair[0], pair[1], newRouteCnt, vehiclesNew));
      newRouteCnt++;
    }
  ...
  private static Vehicles removeExtraVehicles(
      TransitSchedule transitScheduleNew, Vehicles vehicles) {

    Set<Id<Vehicle>> vehiclesUsedInTransitSchedule =
        transitScheduleNew.getTransitLines().values().stream()
            .flatMap(
                x ->
                    x.getRoutes().values().stream()
                        .flatMap(
                            y -> y.getDepartures().values().stream().map(Departure::getVehicleId)))
  ...
  private static List<Integer[]> findStartEndIndicesForAllRoutes(boolean[] stops2Keep) {
    ...
    for (int i = 0; i < stops2Keep.length; i++) {
      ...
      // the route is complete when we have a start and end index --> add route indicies to
      // routeIndicies
  ...
  private static TransitRoute createNewRouteSameId(
      TransitRoute routeOld, List<TransitRouteStop> stopsInNewRoute) {
    ...
    TransitRoute routeNew =
        tsf.createTransitRoute(
            routeOld.getId(), networkRouteNew, clonedStops, routeOld.getTransportMode());
    ...
    for (Departure departure : routeOld.getDepartures().values()) {
      Departure departureNew =
          tsf.createDeparture(departure.getId(), departure.getDepartureTime() + deltaSeconds);
  ...
  private static TransitRoute createNewRouteModifiedId(
      TransitRoute routeOld,
      Integer startIndex,
      Integer endIndex,
      int splitNumber,
    ...
    Id<TransitRoute> routeIdNew =
        Id.create(routeIdOld.toString() + "_split" + splitNumber, TransitRoute.class);

    TransitRoute routeNew =
        tsf.createTransitRoute(
            routeIdNew, networkRouteNew, clonedStops, routeOld.getTransportMode());
    routeNew.setDescription(routeOld.getDescription());
    ...
    for (Departure departure : routeOld.getDepartures().values()) {
      ...
      Departure departureNew =
          tsf.createDeparture(
              Id.create(depIdOld + "_split" + splitNumber, Departure.class),
  ...
  private static NetworkRoute buildNetworkRoute(
      TransitRoute routeOld, List<TransitRouteStop> stopsInNewRoute) {
    ...
    Id<Link> endLinkNew =
        stopsInNewRoute.get(stopsInNewRoute.size() - 1).getStopFacility().getLinkId();
  ...
  private static List<TransitRouteStop> copyStopsAndModifyDepArrOffsets(
      List<TransitRouteStop> stopsOld, double offsetDelta) {
    ...
    for (TransitRouteStop oldStop : stopsOld) {
      ...
      TransitRouteStop.Builder<?> transitRouteStopBuilder =
          tsf.createTransitRouteStopBuilder(stopFacility);
