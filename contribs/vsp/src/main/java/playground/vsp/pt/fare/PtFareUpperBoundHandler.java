...
/**
 * This class will calculate the potential refund a PT user will get because of the upper bound of
 * the daily fare ( e.g. ticket subscription). If the upper bound is reached, refund will be issued
 * at the end of the iteration (i.e. after Mobsim). In that case, we assume that PT user is a
 * frequent traveller and will use subscription instead of single tickets.
 *
...
public class PtFareUpperBoundHandler implements PersonMoneyEventHandler, AfterMobsimListener {
      ...
      if (refund > 0) {
        ...
        events.processEvent(
            new PersonMoneyEvent(
                getOrCalcCompensationTime(),
                personId,
                refund,
                PT_REFUND,
                TransportMode.pt,
                "Refund for person" + personId.toString()));
      }
    }
  }
  ...
  private double getOrCalcCompensationTime() {
    if (Double.isNaN(this.mobsimEndTime)) {
      this.mobsimEndTime =
          (Double.isFinite(qSimConfigGroup.getEndTime().seconds())
                  && qSimConfigGroup.getEndTime().seconds() > 0)
