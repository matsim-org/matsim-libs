...
/**
 * This class estimates the air pollution exposure costs of all pollutants except CO2 and flat costs
 * of CO2 if considering CO2 costs at all.
 *
 * @author benjamin
 */
public class EmissionResponsibilityCostModule {
  ...
  @Inject
  EmissionResponsibilityCostModule(
      EmissionsConfigGroup emissionsConfigGroup, ResponsibilityGridTools rgt) {
    //		this.emissionCostMultiplicationFactor =
    // emissionsConfigGroup.getEmissionCostMultiplicationFactor();
    //		this.considerCO2Costs = emissionsConfigGroup.isConsideringCO2Costs();

    logger.info(
        "Emission costs from Maibach et al. (2008) are multiplied by a factor of "
            + this.emissionCostMultiplicationFactor);

    if (this.considerCO2Costs) {
      logger.info("CO2 emission costs will be calculated... ");
      logger.warn(
          "The first iteration will include only flat CO2 emission costs because, "
  ...
  public double calculateWarmEmissionCosts(
      Map<Pollutant, Double> warmEmissions, Id<Link> linkId, double time) {
    ...
    for (Pollutant wp : warmEmissions.keySet()) {
      ...
      if (wp != Pollutant.CO2_TOTAL) {
        ...
        //				if ( true ) {
        //					throw new RuntimeException("pollutants are no longer enums; need to hedge against
        // header changes in upstream input file") ;
        //				}
  ...
  public double calculateColdEmissionCosts(
      Map<Pollutant, Double> coldEmissions, Id<Link> linkId, double time) {
    ...
    // relative density = person minutes of resp. cell and time bin / average person minutes of all
    // cells from this time bin
    Double relativeDensity = responsibilityGridTools.getFactorForLink(linkId, time);
