...
public class ResponsibilityGridTools {
  ...
  private static final Logger LOGGER = LogManager.getLogger(ResponsibilityGridTools.class);
  ...
  public Double getFactorForLink(Id<Link> linkId, double time) {
    ...
    if (!isDurationsStored && warnCount++ <= maxWarnCount) {
      LOGGER.warn(
          "Make sure, you have stored activity durations by using "
              + IntervalHandler.class.getSimpleName()
              + " along with normal events and pass it to the "
              + ResponsibilityGridTools.class.getSimpleName());
    }
  ...
  public void resetAndcaluculateRelativeDurationFactors(SortedMap<Double, Double[][]> duration) {
    ...
    for (Double timeBin : duration.keySet()) {
      ...
      Double averageOfCurrentTimeBin =
          sumOfCurrentTimeBin / currentDurations.length / currentDurations[0].length;
      ...
      for (Id<Link> linkId : links2xCells.keySet()) {
        if (links2yCells.containsKey(linkId)) { // only if in research are
          Double relativeFactorForCurrentLink =
              getRelativeFactorForCurrentLink(averageOfCurrentTimeBin, currentDurations, linkId);
  ...
  private Double getRelativeFactorForCurrentLink(
      Double averageOfCurrentTimeBin, Double[][] currentDurations, Id<Link> linkId) {
    ...
    if (links2xCells.get(linkId) != null && links2yCells.get(linkId) != null) {
      ...
      for (int distance = 0; distance <= 3; distance++) {
        List<Cell> distancedCells =
            cellOfLink.getCellsWithExactDistance(noOfXCells, noOfYCells, distance);
