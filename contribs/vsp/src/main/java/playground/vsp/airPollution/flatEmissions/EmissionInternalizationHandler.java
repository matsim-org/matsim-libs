 ...
 * @author benjamin
 */
public class EmissionInternalizationHandler
  ...
  public EmissionInternalizationHandler(
      MatsimServices controler, EmissionCostModule emissionCostModule, Set<Id<Link>> hotspotLinks) {
  ...
  private void calculateColdEmissionCostsAndThrowEvent(ColdEmissionEvent event) {
    ...
    double coldEmissionCosts =
        emissionCostModule.calculateColdEmissionCosts(event.getColdEmissions());
    ...
    PersonLinkMoneyEvent moneyLinkEvent =
        new PersonLinkMoneyEvent(
            time, personId, event.getLinkId(), amount2Pay, time, "airPollution");
  ...
  private void calculateWarmEmissionCostsAndThrowEvent(WarmEmissionEvent event) {
    Id<Person> personId = Id.create(event.getVehicleId(), Person.class);
    double time = event.getTime();
    double warmEmissionCosts =
        emissionCostModule.calculateWarmEmissionCosts(event.getWarmEmissions());
