...
public class EmissionResponsibilityTollTimeDistanceTravelDisutility implements TravelDisutility {
  ...
  private static final Logger log =
      LogManager.getLogger(EmissionResponsibilityTollTimeDistanceTravelDisutility.class);
  ...
  public EmissionResponsibilityTollTimeDistanceTravelDisutility(
  ...
  public double getLinkTravelDisutility(
    ...
    double randomizedTimeDistanceDisutilityForLink =
        this.randomizedTimeDistanceTravelDisutility.getLinkTravelDisutility(link, time, person, v);
    ...
    if (emissionVehicle == null) {
      ...
      if (person == null) {
        ...
        log.warn(
            "No person and no vehicle is given to calculate the link travel disutility. The default vehicle type is used to estimate emission disutility.");
        emissionVehicle =
            VehicleUtils.getFactory()
                .createVehicle(
                    Id.createVehicleId("defaultVehicle"), VehicleUtils.getDefaultVehicleType());
      } else {
        // a person is given -> use the vehicle for that person given in emissionModule
        emissionVehicle =
            this.emissionVehicles.getVehicles().get(Id.createVehicleId(person.getId()));
      }
    }
    ...
    if (hotspotLinks == null) {
      ...
      linkExpectedEmissionDisutility =
          calculateExpectedEmissionDisutility(
              emissionVehicle, link, link.getLength(), linkTravelTime, time);
    } else {
      // pricing applies for the current link
      if (hotspotLinks.contains(link.getId()))
        linkExpectedEmissionDisutility =
            calculateExpectedEmissionDisutility(
                emissionVehicle, link, link.getLength(), linkTravelTime, time);
  ...
  private double calculateExpectedEmissionDisutility(
      Vehicle vehicle, Link link, double distance, double linkTravelTime, double time) {
    double linkExpectedEmissionDisutility;
    ...
    LinkEmissionsCalculator warmEmissionAnalysisModule =
        this.emissionModule.getWarmEmissionAnalysisModule();
    Map<Pollutant, Double> expectedWarmEmissions =
        warmEmissionAnalysisModule.checkVehicleInfoAndCalculateWarmEmissions(
            vehicle,
            ...
            link,
            linkTravelTime);
    double expectedEmissionCosts =
        this.emissionResponsibilityCostModule.calculateWarmEmissionCosts(
            expectedWarmEmissions, link.getId(), time);
