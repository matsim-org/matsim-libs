...
/**
 * @author amit These values are taken from Maibach et al. (2008)
...
public enum EmissionCostFactors {
  ...
  NO2(0.), // NO2 is included in NOx; we are not separately computing possible additional damages of
  // NO2.  kai/ihab, feb'19
  HC(0.), // not included in Maibach et al; they only have NMVOC, which should be equivalent to our
  // NMHC, see below.
  ...
  // (It is not clear if HBEFA reports PM10 or PM2.5.  It seems more plausible that they report all
  // PM.  It also seems that there is no PM above size 10, so this would
  // be equivalent to PM10.  Meaning that we should use the PM10 cost factor from Maibach, not the
  // PM2.5 factor as currently used.)
  ...
  /**
   * This is (since feb/2019) deliberately programmed such that it throws an exception when the
