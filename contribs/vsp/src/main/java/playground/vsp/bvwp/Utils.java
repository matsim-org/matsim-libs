 ...
 */
class Utils {
  static final String FMT_STRING =
      "%16s || %16.2f | %16.1f || %16.2f | %16.1f || %16.2f | %16.1f || %16.2f | %12.1e  mio||\n";
  ...
  static void writeOperatorProfit(Map<Mode, Double> operatorProfitGains, Html html) {
    for (Entry<Mode, Double> profit : operatorProfitGains.entrySet()) {
      html.beginTableMulticolumnRow();
      html.write(
          "Operator profit gain; "
              + profit.getKey().toString()
              + ": "
              + Utils.convertToMillions(profit.getValue()));
      html.endTableRow();
    }
  }
  ...
  static void writeSum(Html html, double utils) {
    ...
    //		html.bvwpTableRow("", "", "", "", "", "", "", "", "", "<strong>" + convertToMillions(utils)
    // + "</strong>" ) ;
    html.beginTableMulticolumnRow();
    html.beginDivRightAlign();
    html.write("<strong>" + convertToMillions(utils) + "</strong>");
  ...
  static void initializeOutputTables(Html html) {
    ...
    html.beginTableRow();
    html.write("<strong>Attribut</strong>");
    html.nextTableEntry();
    html.write("<strong>Attribut Nullfall</strong>");
    html.nextTableEntry();
    html.write("<strong>... mal Menge</strong>");
    html.nextTableEntry();
    html.write("<strong>Attribut Planfall</strong>");
    html.nextTableEntry();
    html.write("<strong>... mal Menge</strong>");
    html.nextTableEntry();
    html.write("<strong>Attribut Diff</strong>");
    html.nextTableEntry();
    html.write("<strong>... mal Menge</strong>");
    html.nextTableEntry();
    html.write("<strong>Nutzen Diff</strong>");
    html.nextTableEntry();
    html.write("<strong>... mal Menge</strong>");
    html.nextTableEntry();
    html.write(
  ...
  static void writeRoh(
      Html html,
      double utilsUserFromRoHOldUsers,
      double utilsUserFromRoHNewUsers,
      Map<Mode, Double> operatorProfitGains) {
    ...
    System.out.printf(
        "RoH: utl gain old users: %16.1e ; utl gain new users: %16.1e ; operator profit gain: %16.1e ; sum: %16.1e\n",
        utilsUserFromRoHOldUsers,
        utilsUserFromRoHNewUsers,
        operatorProfit,
        utilsUserFromRoHOldUsers + utilsUserFromRoHNewUsers + operatorProfit);
    ...
    html.beginTableMulticolumnRow();
    html.write(
        "RoH: utl gain old users: "
            + Utils.convertToMillions(utilsUserFromRoHOldUsers)
            + "RoH: utl gain new users: "
            + Utils.convertToMillions(utilsUserFromRoHNewUsers)
            + "; operator profit gain: "
            + Utils.convertToMillions(operatorProfit)
            + "; sum: "
  ...
  static void writeSubHeaderWechselnd(
      Html html, Id id, DemandSegment segm, Mode mode, final double deltaAmounts) {

    System.out.printf(
        "====================%16s; %16s; %16s; wechselnder & induzierter Verkehr: %16.1f Personen/Tonnen ====================\n",
    ...
    html.write(
        "<strong>"
            ...
            + "; "
            + mode
            + "; wechselnder & induzierter Verkehr: "
            + deltaAmounts
            + " Personen/Tonnen"
            + "</strong>");
    html.endTableRow();
  ...
  static void writeSubHeaderVerlagert(
      Html html, String id, DemandSegment segm, Mode mode, final double deltaAmounts) {

    System.out.printf(
        "====================%16s; %16s; %16s; verlagerter Verkehr: %16.1f Personen/Tonnen ====================\n",
        id, mode, segm, deltaAmounts);
    html.beginTableMulticolumnRow();
    html.write(
        "<strong>"
  ...
  static void writeSubHeaderInduziert(
      Html html, String id, DemandSegment segm, Mode mode, final double deltaAmounts) {

    System.out.printf(
        "====================%16s; %16s; %16s;  induzierter Verkehr: %16.1f Personen/Tonnen ====================\n",
        id, mode, segm, deltaAmounts);
    html.beginTableMulticolumnRow();
    html.write(
        "<strong>"
  ...
  static void writeSubHeaderVerbleibend(
      Html html, String id, DemandSegment segm, Mode mode, double amountAltnutzer) {
    System.out.printf(
        "====================%16s; %16s; %16s;             verbleibender Verkehr: %16.1f Personen/Tonnen ====================\n",
  ...
  static void writeImplicitUtl(
      Html html,
      final double implicitUtlPerItem,
      final double implicitUtlOverall,
      final String str) {
    if (implicitUtlPerItem != 0.) {
      ...
      System.out.printf(
          ...
          0.,
          implicitUtlPerItem,
          implicitUtlOverall / 1000. / 1000.);
      html.bvwpTableRow(
          str, 0., 0., 0., 0., 0., 0., implicitUtlPerItem, convertToMillions(implicitUtlOverall));
    }
  }

  static void writeAltnutzerRow(
      ...
      final double utlChangePerItem,
      final double utlChange) {
    System.out.printf(
        Utils.FMT_STRING,
        attribute,
        ...
        deltaQuantities,
        deltaQuantities * amountAltnutzer,
        utlChangePerItem,
        utlChange / 1000. / 1000.);
    html.bvwpTableRow(
        attribute.toString(),
  ...
  static void writeAufnehmendRow(
      Html html,
      final double deltaAmounts,
      Attribute attribute,
      final double attributeValuePlanfall,
    ...
    System.out.printf(
        ...
        attributeValuePlanfall * deltaAmounts,
        attributeValuePlanfall,
        attributeValuePlanfall * deltaAmounts,
        utlChangesPerItem.utl,
        utlChange / 1000. / 1000.);
    html.bvwpTableRow(
        attribute.toString() + " aufn.",
        0.,
        0.,
        attributeValuePlanfall,
        attributeValuePlanfall * deltaAmounts,
  ...
  static void writeAbgebendRow(
      Html html,
      final double deltaAmounts,
      Attribute attribute,
      final double attributeValueNullfall,
      final double attributeValuePlanfall,
      UtlChangesData utlChangesPerItem,
      final double utlChange) {
    System.out.printf(
        ...
        attributeValueNullfall * deltaAmounts,
        0.,
        0.,
        -attributeValueNullfall,
        attributeValueNullfall * deltaAmounts,
        // (selbst wenn sich das abgebende System ändert, so ist unser Gewinn dennoch basierend auf
        // dem
        // Nullfall)
        utlChangesPerItem.utl,
        utlChange / 1000. / 1000.);
    html.bvwpTableRow(
        attribute.toString() + " abgeb.",
        attributeValueNullfall,
        attributeValueNullfall * deltaAmounts,
        attributeValuePlanfall,
        0., // this uses Planfall just for completeness
        -attributeValueNullfall,
        attributeValueNullfall * deltaAmounts,
        // (selbst wenn sich das abgebende System ändert, so ist unser Gewinn dennoch basierend auf
        // dem
  ...
  static void writeVerlagertSum(Html html, HashMap<Mode, Double> utilsVerl) {
    ...
    //		html.endTableRow() ;
    //
    //		html.bvwpTableRow("Summe", "", "", "", "", "", "", "", "", "<strong>" + Double.toString(
    // ((long)(utilsVerl/100./1000.))/10.   ) + " mio</strong>" ) ;
  }
  ...
  static void writeInduziertSum(Html html, HashMap<Mode, Double> utilsInduz) {
    ...
    //
    //		html.bvwpTableRow("Summe", "", "", "", "", "", "", "", "", "<strong>" + Double.toString(
    // ((long)(utilsInduz/100./1000.))/10.   ) + " mio</strong>" ) ;
  }

  static void writeImplVerlagertSum(Html html, HashMap<Mode, Double> modularImplVerlagertUtils) {

    System.out.printf("%188s\n", "----------------");
    for (Entry<Mode, Double> e : modularImplVerlagertUtils.entrySet()) {
      System.out.println(
          "Fuer Mode " + e.getKey() + " Summe IMPLIZITER Nutzen verlagert: " + e.getValue());
    }
  ...
  static void writeImplInduziertSum(Html html, HashMap<Mode, Double> utilsImplInduz) {
    ...
    for (Entry<Mode, Double> e : utilsImplInduz.entrySet()) {
      System.out.println(
          "Fuer Mode " + e.getKey() + " Summe IMPLIZITER Nutzen induziert: " + e.getValue());
    }
  ...
  static void addUtlToMap(
      Map<Mode, Map<Attribute, Double>> map, Mode mode, Attribute attribute, double utl) {
  ...
  static double writeOverallOutputTable(
      Html totalHtml,
    ...
    for (Mode mode : relModes) {
      ...
      line.add(
          Math.round(verlagertNMAuf.get(mode)[1] / Math.abs(verlagertNMAuf.get(mode)[0])) + "km");
      totalHtml.tableRowFromList(line, false);
      line.clear();
      ...
      line.add(
          Math.round(Math.abs(verlagertNMAuf.get(mode)[5] * 100 / verlagertNMAuf.get(mode)[2]))
              + " c/Pkw-km");
      totalHtml.tableRowFromList(line, false);
      line.clear();
    ...
    //				writePartialSum(totalHtml, "Nutzenaenderung bei Verlagerung insgesamt (alle
    // Verkehrstraeger)", verlSum);
