      ...
      {
        {
          Mode mode = Mode.Bahn;
          {
            ...
            // (we need both the base case and the measure case here to be able to compute the
            // "half" improvement!!!)
            nullfallForInduced.put(makeKey(mode, segm, Attribute.Nutzerkosten_Eu), distance * 0.1);
            nullfallForInduced.put(
    ...
    // verlagert:
    {
      ...
      }
      {
        Mode mode = Mode.Strasse;
        {
          ...
          nullfallForSwitched.put(makeKey(mode, segm, Attribute.Nutzerkosten_Eu), distance * 0.1);
          nullfallForSwitched.put(
              makeKey(mode, segm, Attribute.Produktionskosten_Eu), distance * 0.1);
        }
        {
          DemandSegment segm = DemandSegment.PV_NON_COMMERCIAL;
          final double distance = 210.;
  ...
  static ScenarioForEvalData createPlanfallBahnausbau(ScenarioForEvalData nullfall) {
...

    {
      ...
      Objects.requireNonNull(planfallValuesForOD);
      {
        ...
        // (we need both the base case and the measure case here to be able to compute the "half"
        // improvement!!!)
