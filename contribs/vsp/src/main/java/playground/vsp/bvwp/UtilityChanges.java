...
abstract class UtilityChanges {
  ...
  Map<Mode, Map<Attribute, Double>> verbleibendRV =
      new HashMap<MultiDimensionalArray.Mode, Map<Attribute, Double>>();
  ...
  // verlagertNMAb/Auf: 0 - personen 1- personenKM 2 - pkw-KM 3 - personen-h 4 - pkw-h 5 -
  // nutzerkosten
  ...
  final void computeAndPrintResults(
      Values economicValues, ScenarioForEvalData nullfall, ScenarioForEvalData planfall) {
    Html html = new Html();
    html.beginHtml();
    html.beginBody();
    html.beginTable();
  ...
  final double computeAndPrintResults(
      Values economicValues,
      ScenarioForEvalData nullfall,
      ScenarioForEvalData planfall,
      String outFileName) {
  ...
  final double computeAndPrintResults(
    ...
    for (String id : nullfall.getAllRelations()) { // for all OD relations
      ...
      for (DemandSegment segm : DemandSegment.values()) { // for all types (e.g. PV or GV)
        ...
        Mode improvedMode =
            autodetectImprovingMode(nullfallForODRelation, planfallForODRelation, segm);

        if (improvedMode == null) {
          continue; // means that in the demand segment, nothing has changed; goto next demand
          // segment
        }

        Attributes econValuesReceiving = economicValues.getAttributes(improvedMode, segm);
        Attributes attributesNullfallReceiving =
            nullfallForODRelation.getAttributes(improvedMode, segm);
        Attributes attributesPlanfallReceiving =
            planfallForODRelation.getAttributes(improvedMode, segm);
        ...
        for (Mode mode : Mode.values()) {
          ...
          if (mode.equals(improvedMode)) {
            ...
            utils +=
                computeAndPrintValuesForAltnutzer(
                    econValues,
                    attributesNullfall,
                    mode,
                    attributesPlanfall,
          ...
          if (mode != improvedMode) {
            ...
            utils +=
                computeAndPrintGivingOrReceiving(
                    econValuesReceiving,
                    attributesNullfallReceiving,
                    attributesPlanfallReceiving,
                    econValues,
                    attributesNullfall,
                    attributesPlanfall,
                    mode,
            ...
            if (utils != utilsBefore) {
              Utils.writePartialSum(
                  html,
                  "Nutzen&auml;nderung aus &Auml;nderung Ressourcenverzehr bei Verlagerung:",
                  utils - utilsBefore);
            }

            final double utilsImp =
                computeAndPrintImplicitUtl(
                    econValuesReceiving,
                    attributesNullfallReceiving,
                    attributesPlanfallReceiving,
                    econValues,
            ...
            //						if ( utils != utilsBefore ) {
            Utils.writePartialSum(
                html, "Nutzen&auml;nderung bei Verlagerung gesamt:", utils - utilsBefore);
            //						}

          }

          // roh etc. stuff (for comparison):
          utilsUserFromRoHOldUsers +=
              computeUserBenefitsOldUsers(
                  econValues, attributesNullfall, attributesPlanfall, amountNullfall);
          utilsUserFromRoHNewUsers +=
              computeUserBenefitsNewUsers(
                  econValues,
                  attributesNullfall,
                  attributesPlanfall,
                  amountNullfall,
        ...
        try {
          final double amountNullfallRcv =
              nullfallForODRelation.get(makeKey(improvedMode, segm, Attribute.XX));
          final double amountPlanfallRcv =
              planfallForODRelation.get(makeKey(improvedMode, segm, Attribute.XX));
        ...
        induziertNM[2] += pkwKm;
        final double pH =
            amountInduced * attributesPlanfallReceiving.getByEntry(Attribute.Reisezeit_h);
        induziertNM[3] += pH;
        final double pkwH = pH / besetzungsgrad;
        induziertNM[4] += pkwH;

        induziertNM[5] +=
            attributesPlanfallReceiving.getByEntry(Attribute.Nutzerkosten_Eu) * amountInduced;
        ...
        for (Attribute attribute : Attribute.values()) { // for all entries (e.g. km or hrs)
          if (attribute != Attribute.XX && attribute != Attribute.Nutzerkosten_Eu) {
            // aufnehmende Seite:
            final double attributeValuePlanfallReceiving =
                attributesPlanfallReceiving.getByEntry(attribute);
            final double attributeValueNullfallReceiving =
                attributesNullfallReceiving.getByEntry(attribute);

            UtlChangesData utlChangesPerItem =
                utlChangePerEntry(
                    attribute,
                    amountInduced,
                    attributeValueNullfallReceiving,
                    attributeValuePlanfallReceiving,
                    econValuesReceiving.getByEntry(attribute));
        ...
        final double implUtlInducedPerItem =
            this.computeImplicitUtilityPerItem(
                econValuesReceiving, attributesNullfallReceiving, attributesPlanfallReceiving);
  ...
  private static double computeUserBenefitsOldUsers(
    ...
    for (Attribute attribute : Attribute.values()) {
      if (attribute != Attribute.XX && attribute != Attribute.Produktionskosten_Eu) {
        final double improvementOfAttribute =
            attributesPlanfall.getByEntry(attribute) - attributesNullfall.getByEntry(attribute);
  ...
  private static double computeUserBenefitsNewUsers(
      Attributes econValues,
      Attributes attributesNullfall,
      Attributes attributesPlanfall,
      final double amountNullfall,
      ...
      for (Attribute attribute : Attribute.values()) {
        if (attribute != Attribute.XX && attribute != Attribute.Produktionskosten_Eu) {
          final double improvementOfAttribute =
              attributesPlanfall.getByEntry(attribute) - attributesNullfall.getByEntry(attribute);
          utilsUserFromRoH +=
              improvementOfAttribute * averageOfXX * econValues.getByEntry(attribute);
        }
      }
    }
  ...
  private double computeOperatorProfit(
      double operatorProfit,
      Attributes attributesNullfall,
      Attributes attributesPlanfall,
      final double amountNullfall,
      final double amountPlanfall) {
    {
      ...
      final double revenueNullfall =
          attributesNullfall.getByEntry(Attribute.Nutzerkosten_Eu) * amountNullfall;
      final double revenuePlanfall =
          attributesPlanfall.getByEntry(Attribute.Nutzerkosten_Eu) * amountPlanfall;
      final double operatorCostNullfall =
          attributesNullfall.getByEntry(Attribute.Produktionskosten_Eu) * amountNullfall;
      final double operatorCostPlanfall =
          attributesPlanfall.getByEntry(Attribute.Produktionskosten_Eu) * amountPlanfall;
      operatorProfit +=
          -(revenueNullfall - operatorCostNullfall) + (revenuePlanfall - operatorCostPlanfall);
    }
    return operatorProfit;
  }

  private double computeAndPrintGivingOrReceiving(
      Attributes econValuesReceiving,
      Attributes attributesNullfallReceiving,
      Attributes attributesPlanfallReceiving,
      Attributes econValues,
      Attributes attributesNullfall,
    ...
    final double deltaAmounts =
        attributesPlanfall.getByEntry(Attribute.XX) - attributesNullfall.getByEntry(Attribute.XX);
    ...
    if (deltaAmounts != 0.) {
      ...
      // aufnehmend
      {
        ...
        final double nutzerkosten =
            attributesPlanfallReceiving.getByEntry(Attribute.Nutzerkosten_Eu);
      ...
      if (attribute != Attribute.XX && attribute != Attribute.Nutzerkosten_Eu) {
        {
          ...
          UtlChangesData utlChangesPerItem =
              utlChangePerEntry(
                  attribute,
                  deltaAmounts,
                  attributeValueNullfall,
                  attributeValuePlanfall,
                  econValues.getByEntry(attribute));
          final double utlChange = utlChangesPerItem.utl * Math.abs(deltaAmounts);
          partialUtils += utlChange;

          if (utlChange != 0.) {
            Utils.writeAbgebendRow(
                html,
                deltaAmounts,
                attribute,
                attributeValueNullfall,
  ...
  private double computeAndPrintImplicitUtl(
      Attributes econValuesReceiving,
      Attributes attributesNullfallReceiving,
      Attributes attributesPlanfallReceiving,
      Attributes econValues,
    ...
    final double implicitUtlPerItem =
        this.computeImplicitUtilityPerItem(econValues, attributesNullfall, attributesPlanfall);
    // probably positive
    ...
    final double implicitUtlPerItemReceiving =
        this.computeImplicitUtilityPerItem(
            econValuesReceiving, attributesNullfallReceiving, attributesPlanfallReceiving);
    // probably positive
    ...
    if (implicitUtlOverallReceiving != 0.) {
      Utils.writeImplicitUtl(
          html, implicitUtlPerItemReceiving, implicitUtlOverallReceiving, "Impl. Nutz. aufn.");
  ...
  private static Mode autodetectImprovingMode(
      Values nullfallForODRelation, Values planfallForODRelation, DemandSegment segm) {
  ...
  private double computeAndPrintValuesForAltnutzer(
      Attributes econValues,
    ...
    for (Attribute attribute : Attribute.values()) { // for all entries (e.g. km or hrs)
      if (attribute != Attribute.XX && attribute != Attribute.Nutzerkosten_Eu) {
        ...
        // Aber ist das nicht richtig: Eine Preisänderung bewirkt nur eine Veränderung der
        // Einnahme-Aufteilung; roh geht (z.B.) hoch,
        // producer surplus geht runter, vw'lich keine Änderung.  kai, oct'13

        double deltaQuantities =
            quantitiesPlanfall.getByEntry(attribute) - quantitiesNullfall.getByEntry(attribute);
  ...
  abstract UtlChangesData utlChangePerEntry(
      Attribute attribute,
      double deltaAmount,
  ...
  abstract double computeImplicitUtilityPerItem(
