...
class ScenarioZielnetzBahn2 {
    ...
    nullfall.setValuesForODRelation("BC", nullfallForOD);
    {
      System.out.println(" distance: " + distance);
      {
        ...
        Mode mode = Mode.Strasse;
        {
          ...
          nullfallForOD.put(
              ...
              (0.1 - 0.02) * distance); // price minus taxes
      ...
      }
      {
        ...
        Mode mode = Mode.Bahn;
        {
          ...
          nullfallForOD.put(
              makeKey(mode, segm, Attribute.Produktionskosten_Eu),
              0.); // assume (marginal) production cost of rail is zero
        }
        {
          // commercial passenger traffic:
          DemandSegment segm = DemandSegment.PV_GESCHAEFT;
  ...
  static ScenarioForEvalData createPlanfallBahnausbau(ScenarioForEvalData nullfall) {
    ...
    Objects.requireNonNull(planfallValuesForOD);
    {
      ...
      planfallValuesForOD.inc(
          makeKey(Mode.Bahn, segm, Attribute.XX), verlagert_bahn_von_strasse + induziert_bahn);
      planfallValuesForOD.inc(
          makeKey(Mode.Strasse, segm, Attribute.XX), -verlagert_bahn_von_strasse);
    }
    //		{
    //			// modify the travel times for the rail mode:
    ...
    //
    //			// modify some demand (presumably as a result):
    //			planfallValuesForOD.inc( makeKey( Mode.RAIL, segm, Attribute.XX),
    // verlagert_bahn_von_strasse_comm + induziert_bahn_comm) ;
    //			planfallValuesForOD.inc( makeKey( Mode.ROAD, segm, Attribute.XX),
    // -verlagert_bahn_von_strasse_comm ) ;
    //
    //		}
    return planfall;
  }
  ...
  static ScenarioForEvalData createPlanfallStrassenausbau(ScenarioForEvalData nullfall) {
    ...
    Objects.requireNonNull(planfallValuesForOD);
    {
      ...
      DemandSegment segm = DemandSegment.PV_NON_COMMERCIAL;
      planfallValuesForOD.inc(
          makeKey(Mode.Strasse, segm, Attribute.Reisezeit_h), -0.13); // ausgedacht

      // modify some demand (presumably as a result):
      planfallValuesForOD.inc(
          makeKey(Mode.Strasse, segm, Attribute.XX),
          verlagert_strasse_von_bahn + induziert_strasse);
    ...
    }
    {
      ...
      planfallValuesForOD.inc(
          makeKey(Mode.Strasse, segm, Attribute.XX),
          verlagert_strasse_von_bahn_comm + induziert_strasse_comm);
      planfallValuesForOD.inc(
          makeKey(Mode.Bahn, segm, Attribute.XX), -verlagert_strasse_von_bahn_comm);
    }
    return planfall;
  }
}
