...
public class IVVReaderV2 {
  ...
  private static final Logger log = LogManager.getLogger(IVVReaderV2.class);
  ...
  void runBVWP2015(ScenarioForEvalData nullfall, ScenarioForEvalData planfall, String outname) {
    ...
    double diff =
        utilityChanges.computeAndPrintResults(economicValues, nullfall, planfall, outname);
  ...
  void read() {
    ...
    log.info(
        "Creating Index using Production & Operations' file : " + config.getImpedanceMatrixFile());
    ...
    //	    read(config.getRemainingDemandMatrixFile(), new DemandRemainingHandler(odRelations,
    // planfallData));
    ...
    log.info(
        "Reading Cost Of Production & Operations' file (10-11,12-13,14-15 Sammelfile Nullfall&Planfall ): "
            + config.getImpedanceMatrixFile());
    ...
    read(
        config.getDroppedDemandMatrixFile(),
        new DemandNewOrDroppedHandler(odRelations, planfallData));

    log.info("Reading Verlagert (16-17): " + config.getImpedanceShiftedMatrixFile());
    read(
        config.getImpedanceShiftedMatrixFile(),
        new ImpedanceShiftedHandler(odRelations, planfallData, nullfallData));
    ...
    if (nullFallDataByFromId.size() != planFallDataByFromId.size()) {
      System.err.println(
          "Null und Planfall haben verschiedene Anzahl an Von-Ids. Das kann nicht sein");
      throw new IllegalStateException();
    }
    for (String fromId : nullFallDataByFromId.keySet()) {
      runBVWP2015(
          nullFallDataByFromId.get(fromId), planFallDataByFromId.get(fromId), fromId.toString());
    }
  ...
  private void analyse(ScenarioForEvalData nullfallData, ScenarioForEvalData planfallData) {
    ...
    for (String odId : planfallData.getAllRelations()) {
      ...
      for (DemandSegment ds : DemandSegment.values()) {
        double xxPlan =
            planfallData
                .getByODRelation(odId)
                .getAttributes(Mode.Strasse, ds)
                .getByEntry(Attribute.XX);
        double xxNull =
            nullfallData
                .getByODRelation(odId)
                .getAttributes(Mode.Strasse, ds)
      ...
      if (diffXX == 0.0) {
        ...
        double dPlan =
            planfallData
                .getByODRelation(odId)
                .getAttributes(Mode.Strasse, DemandSegment.PV_ARBEIT)
                .getByEntry(Attribute.Distanz_km);
        double dNull =
            nullfallData
                .getByODRelation(odId)
                .getAttributes(Mode.Strasse, DemandSegment.PV_ARBEIT)
        ...
        double tPlan =
            planfallData
                .getByODRelation(odId)
                .getAttributes(Mode.Strasse, DemandSegment.PV_ARBEIT)
                .getByEntry(Attribute.Reisezeit_h);
        double tNull =
            nullfallData
                .getByODRelation(odId)
                .getAttributes(Mode.Strasse, DemandSegment.PV_ARBEIT)
  ...
  void checkCosts(ScenarioForEvalData data) {
    ...
    for (DemandSegment seg : DemandSegment.values()) {
      ...
      for (String id : data.getAllRelations()) {
        double pu =
            data.getByODRelation(id)
                .getAttributes(Mode.Strasse, seg)
                .getByEntry(Attribute.Nutzerkosten_Eu);
        double cop =
            data.getByODRelation(id)
                .getAttributes(Mode.Strasse, seg)
                .getByEntry(Attribute.Produktionskosten_Eu);
        if (pu < cop) usercostsTooLow++;
      }

      System.out.println(
          "Out of "
              + data.getAllRelations().size()
              + " Relations, user costs were lower than CoP in "
              + usercostsTooLow
              + " cases for DemandSegment "
              + seg);
    }
  }
  ...
  private static class DemandHandler implements TabularFileHandler {
    ...
    public void startRow(String[] row) {
      ...
      if (this.odRelations.contains(odId)) {
        ...
        setValuesForODRelation(
            odId,
            Key.makeKey(mode, DemandSegment.PV_ARBEIT, Attribute.XX),
            Double.parseDouble(row[2].trim()),
            data);
        setValuesForODRelation(
            odId,
            Key.makeKey(mode, DemandSegment.PV_AUSBILDUNG, Attribute.XX),
            Double.parseDouble(row[3].trim()),
            data);
        setValuesForODRelation(
            odId,
            Key.makeKey(mode, DemandSegment.PV_EINKAUF, Attribute.XX),
            Double.parseDouble(row[4].trim()),
            data);
        setValuesForODRelation(
            odId,
            Key.makeKey(mode, DemandSegment.PV_GESCHAEFT, Attribute.XX),
            Double.parseDouble(row[5].trim()),
            data);
        setValuesForODRelation(
            odId,
            Key.makeKey(mode, DemandSegment.PV_URLAUB, Attribute.XX),
            Double.parseDouble(row[6].trim()),
            data);
        setValuesForODRelation(
            odId,
            Key.makeKey(mode, DemandSegment.PV_SONST, Attribute.XX),
        ...
        setValuesForODRelation(
            odId,
            Key.makeKey(mode, DemandSegment.PV_URLAUB, Attribute.XX),
            Double.parseDouble(row[12].trim()),
            data);
        setValuesForODRelation(
            odId,
            Key.makeKey(mode, DemandSegment.PV_SONST, Attribute.XX),
            Double.parseDouble(row[13].trim()),
            data);
      }
    }
  }
  ...
  private static class DemandNewOrDroppedHandler implements TabularFileHandler {
    ...
    public void startRow(String[] row) {
      ...
      String odid = getODId(from, to);
      if (this.odRelations.contains(odid)) {
        data.getByODRelation(odid)
            .inc(
                Key.makeKey(Mode.Strasse, DemandSegment.PV_ARBEIT, Attribute.XX),
                Double.parseDouble(row[2].trim()));
        data.getByODRelation(odid)
            .inc(
                Key.makeKey(Mode.Strasse, DemandSegment.PV_AUSBILDUNG, Attribute.XX),
                Double.parseDouble(row[3].trim()));
        data.getByODRelation(odid)
            .inc(
                Key.makeKey(Mode.Strasse, DemandSegment.PV_EINKAUF, Attribute.XX),
                Double.parseDouble(row[4].trim()));
        data.getByODRelation(odid)
            .inc(
                Key.makeKey(Mode.Strasse, DemandSegment.PV_URLAUB, Attribute.XX),
                Double.parseDouble(row[5].trim()));
        data.getByODRelation(odid)
            .inc(
                Key.makeKey(Mode.Strasse, DemandSegment.PV_SONST, Attribute.XX),
                Double.parseDouble(row[6].trim()));
      }
    }
  }
  ...
  private static class CostHandler implements TabularFileHandler {
    ...
    double getBesetzungsgrad(ScenarioForEvalData data, DemandSegment segment, String odId) {
      ...
      double distance =
          data.getByODRelation(odId)
              .getAttributes(Mode.Strasse, segment)
              .getByEntry(Attribute.Distanz_km);
    ...
    public void startRow(String[] row) {
      ...
      for (DemandSegment dds : DemandSegment.values()) {
        ...
        setValuesForODRelation(
            odId,
            Key.makeKey(Mode.Strasse, dds, Attribute.Produktionskosten_Eu),
            Double.parseDouble(row[6]) / bsNullfall,
            nullfalldata);
        setValuesForODRelation(
            odId,
            Key.makeKey(Mode.Strasse, dds, Attribute.Nutzerkosten_Eu),
            Double.parseDouble(row[this.tableLookUp.get(dds)]),
            nullfalldata);

        setValuesForODRelation(
            odId,
            Key.makeKey(Mode.Strasse, dds, Attribute.Produktionskosten_Eu),
            Double.parseDouble(row[7]) / bsPlanfall,
            planfalldata);
        setValuesForODRelation(
            odId,
            Key.makeKey(Mode.Strasse, dds, Attribute.Nutzerkosten_Eu),
            Double.parseDouble(row[this.tableLookUp.get(dds) + 6]),
            planfalldata);
      }
      ...
      ////              double bsNullfall = getBesetzungsgrad(nullfalldata, dds, odId);
      ////              double bsPlanfall = getBesetzungsgrad(planfalldata, dds, odId);
      //              setValuesForODRelation(odId,Key.makeKey(Mode.Strasse, dds,
      // Attribute.Produktionskosten_Eu), Double.parseDouble(row[this.tableLookUp.get(dds)]),
      // nullfalldata);
      //              setValuesForODRelation(odId,Key.makeKey(Mode.Strasse, dds,
      // Attribute.Nutzerkosten_Eu), Double.parseDouble(row[this.tableLookUp.get(dds)]),
      // nullfalldata);
      //
      //              setValuesForODRelation(odId,Key.makeKey(Mode.Strasse, dds,
      // Attribute.Produktionskosten_Eu), Double.parseDouble(row[this.tableLookUp.get(dds)+6]),
      // planfalldata);
      //              setValuesForODRelation(odId,Key.makeKey(Mode.Strasse, dds,
      // Attribute.Nutzerkosten_Eu), Double.parseDouble(row[this.tableLookUp.get(dds)+6]),
      // planfalldata);
      //
      //
      //
        ...
        for (DemandSegment ds : DemandSegment.values()) {
          ...
          log.info(
              ds
                  ...
                  + this.nullfalldata
                      .getByODRelation(odId)
                      .get(Key.makeKey(Mode.Strasse, ds, Attribute.Produktionskosten_Eu))
                  + " NK: "
                  + this.nullfalldata
                      .getByODRelation(odId)
                      .get(Key.makeKey(Mode.Strasse, ds, Attribute.Nutzerkosten_Eu)));
  ...
  private static class ImpedanceShiftedHandler implements TabularFileHandler {
    ...
    public ImpedanceShiftedHandler(
        List<String> odRelations,
        ScenarioForEvalData planfalldata,
        ScenarioForEvalData nullfalldata) {
      this.planfalldata = planfalldata;
      this.nullfalldata = nullfalldata;
      this.odRelations = odRelations;
    }
    ...
    public void startRow(String[] row) {
      ...
      String odId = getODId(from, to);
      // Annahme: Wegezwecke fuer Verlagerungen sind identisch mit Wegezwecken im Bahn fuer
      // OD-Relation im Nullfall
      ...
      if (this.odRelations.contains(odId)) {
        ...
        for (DemandSegment segment : DemandSegment.values()) {
          ...
          Double bahnhere =
              nullfalldata
                  .getByODRelation(odId)
                  .getAttributes(Mode.Bahn, segment)
                  .getByEntry(Attribute.XX);

          bahnZwecke.put(segment, bahnhere);
          totalBahn += bahnhere;
          if (odId.equals(getODId("1500301", "1509001")))
            System.out.println(segment + " : " + bahnhere);
        }
        if (odId.equals(getODId("1500301", "1509001")))
          System.out.println("Total Bahn : " + totalBahn);
        ...
        for (DemandSegment segment : DemandSegment.values()) {
          ...
          Double verl =
              Double.parseDouble(row[11].trim())
                  * -1.0
                  * WERKTAGEPROJAHR
                  * bahnZwecke.get(segment)
                  / totalBahn;
          planfalldata
              .getByODRelation(odId)
              .inc(Key.makeKey(Mode.Bahn, segment, Attribute.XX), Math.round(verl));
          planfalldata
              .getByODRelation(odId)
              .inc(Key.makeKey(Mode.Strasse, segment, Attribute.XX), -1 * Math.round(verl));
          planfalldata
              .getByODRelation(odId)
              .put(
                  Key.makeKey(Mode.Bahn, segment, Attribute.Reisezeit_h),
                  Double.parseDouble(row[5].trim()) / 60.);
          nullfalldata
              .getByODRelation(odId)
              .put(
                  Key.makeKey(Mode.Bahn, segment, Attribute.Reisezeit_h),
  ...
  private static class TravelTimeHandler implements TabularFileHandler {
     ...
     * @param odRelations
     */
    public TravelTimeHandler(
        ScenarioForEvalData data, List<String> odRelations, boolean includeRail) {
      this.data = data;
    ...
    public void startRow(String[] row) {
      ...
      if ((this.odRelations.contains(odId) || this.odRelations.contains(reverseOd))) {
        for (DemandSegment ds : DemandSegment.values()) {
          ...
          setValuesForODRelation(
              odId,
              Key.makeKey(Mode.Strasse, ds, Attribute.Distanz_km),
              Double.parseDouble(myRow[4].trim()),
              data);
          setValuesForODRelation(
              odId,
              Key.makeKey(Mode.Strasse, ds, Attribute.Reisezeit_h),
              Double.parseDouble(myRow[5].trim()) / 60.,
              data);
          setValuesForODRelation(
              reverseOd,
              Key.makeKey(Mode.Strasse, ds, Attribute.Distanz_km),
          ...
          if (includeRail) {
            setValuesForODRelation(
                odId,
                Key.makeKey(Mode.Bahn, ds, Attribute.Distanz_km),
                Double.parseDouble(myRow[4].trim()),
                data);
            setValuesForODRelation(
                odId,
                Key.makeKey(Mode.Bahn, ds, Attribute.Nutzerkosten_Eu),
                Double.parseDouble(myRow[4].trim()) * BAHNPREISPROKM,
                data);
            setValuesForODRelation(
                odId, Key.makeKey(Mode.Bahn, ds, Attribute.Produktionskosten_Eu), 0., data);
            ...
            setValuesForODRelation(
                ...
                data);
            setValuesForODRelation(
                reverseOd, Key.makeKey(Mode.Bahn, ds, Attribute.Produktionskosten_Eu), 0., data);
          }
        }
        count++;
      }
   ...
   * @param data2
   */
  private static void setValuesForODRelation(
      String odId, Key key, Double value, ScenarioForEvalData data2) {
  ...
  public static void main(String[] args) {
    ...
    //		String dir =
    // "/Users/nagel/shared-svn/projects/bvwp/fe2-bewertung/testrechnungen_strasse/IVV_NeubauA14/MD_only/" ;
