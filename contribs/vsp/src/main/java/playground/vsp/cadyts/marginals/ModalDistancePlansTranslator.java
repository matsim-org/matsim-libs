...
class ModalDistancePlansTranslator
    implements PlansTranslator<Id<DistanceDistribution.DistanceBin>> {
  ...
  public cadyts.demand.Plan<Id<DistanceDistribution.DistanceBin>> getCadytsPlan(Plan plan) {
    ...
    if (plan.getCustomAttributes().containsKey(BUILDER_KEY)) {
      return ((PlanBuilder<Id<DistanceDistribution.DistanceBin>>)
              plan.getCustomAttributes().get(BUILDER_KEY))
          .getResult();
    }
    return null;
  }
  ...
  void addTurn(Plan forPlan, Id<DistanceDistribution.DistanceBin> turn, int iteration) {
    ...
    // but the count-cadyts implementation does it too. Therefore we do it here as well for
    // consistency
    Integer lastIteration = (Integer) forPlan.getCustomAttributes().get(ITERATION_KEY);
    PlanBuilder<Id<DistanceDistribution.DistanceBin>> builder =
        (PlanBuilder<Id<DistanceDistribution.DistanceBin>>)
            forPlan.getCustomAttributes().get(BUILDER_KEY);
