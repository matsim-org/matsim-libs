...
class TripEventHandler
  ...
  public void handleEvent(ActivityEndEvent event) {
    if (StageActivityTypeIdentifier.isStageActivity(event.getActType())
        || drivers.contains(event.getPersonId())
        || !agentFilter.includeAgent(event.getPersonId())) return;
    ...
    // we have to put in the trip here, since the activity end lets us know whether we have a main
    // activity or a
    // staging acitivity
    Trip trip = new Trip();
  ...
  public void handleEvent(ActivityStartEvent event) {
    ...
    if (StageActivityTypeIdentifier.isStageActivity(event.getActType())
        || !tripToPerson.containsKey(event.getPersonId())) return;
