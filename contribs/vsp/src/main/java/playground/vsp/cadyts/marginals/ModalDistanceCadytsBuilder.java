...
class ModalDistanceCadytsBuilder {
  ...
  ModalDistanceCadytsBuilder setExpectedDistanceDistribution(
  ...
  AnalyticalCalibrator<Id<DistanceDistribution.DistanceBin>> build() {
    ...
    AnalyticalCalibrator<Id<DistanceDistribution.DistanceBin>> calibrator =
    ...
    calibrator.setMinStddev(
        cadytsConfig.getMinFlowStddev_vehPerHour(), SingleLinkMeasurement.TYPE.FLOW_VEH_H);
    calibrator.setMinStddev(
        cadytsConfig.getMinFlowStddev_vehPerHour(), SingleLinkMeasurement.TYPE.COUNT_VEH);
    ...
    calibrator.setStatisticsFile(
        config.controller().getOutputDirectory() + "/calibration-stats" + MARGINALS + ".txt");
    ...
    for (DistanceDistribution.DistanceBin distanceBin :
        expectedDistanceDistribution.getDistanceBins()) {
      calibrator.addMeasurement(
          distanceBin.getId(),
          0,
          86400,
          distanceBin.getValue(),
          distanceBin.getStandardDeviation(),
