...
public class DistanceDistribution {
  ...
  public void add(
      String mode, double lowerLimit, double upperLimit, double standardDeviation, double value) {
            ...
            .orElseThrow(
                () ->
                    new RuntimeException(
                        "Could not find distance bin for: " + mode + "->" + distance));
  ...
  public DistanceDistribution copyWithEmptyBins() {
    ...
    for (DistanceBin bin : distanceBins.values()) {
      result.add(
          bin.getMode(),
          bin.getDistanceRange().getLowerLimit(),
          bin.getDistanceRange().getUpperLimit(),
          bin.getStandardDeviation(),
          0);
    }
    return result;
  }
