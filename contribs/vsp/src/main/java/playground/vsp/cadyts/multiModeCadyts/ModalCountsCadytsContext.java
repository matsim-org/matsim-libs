...
/**
 * {@link PlanStrategy Plan Strategy} used for replanning in MATSim which uses Cadyts to select
 * plans that better match to given occupancy counts.
 */
public class ModalCountsCadytsContext
    implements CadytsContextI<ModalCountsLinkIdentifier>,
        StartupListener,
        IterationEndsListener,
        BeforeMobsimListener {

  private static final Logger log = LogManager.getLogger(ModalCountsCadytsContext.class);
  ...
  @Inject
  private ModalCountsCadytsContext(
      Config config,
      Scenario scenario,
      @Named("calibration") Counts<ModalCountsLinkIdentifier> calibrationCounts,
    ...
    // dont set calibrated items back to cadytsConfig because by default, count stations for car
    // mode (from config file) are used.
    // As far as I see, this is a problem is using more than one cadyts in parallel and only one
    // cadyts config group. Amit May'18
    //
  ...
  public void notifyStartup(StartupEvent event) {
    ...
    this.plansTranslator =
        new ModalCountsPlansTranslatorBasedOnEvents(scenario, modalLinkContainer);
    this.eventsManager.addHandler(plansTranslator);

    this.calibrator =
        CadytsBuilderImpl.buildCalibratorAndAddMeasurements(
            scenario.getConfig(),
            this.calibrationCounts,
            new ModalCountsLinkLookUp(modalLinkContainer) /*, cadytsConfig.getTimeBinSize()*/,
            ModalCountsLinkIdentifier.class);
  }
  ...
  public void notifyBeforeMobsim(BeforeMobsimEvent event) {
    ...
    // mz 2015
    for (Person person :
        scenario.getPopulation().getPersons().values()) { // this is wrong. adding plan
  ...
  public void notifyIterationEnds(final IterationEndsEvent event) {
    if (this.writeAnalysisFile) {
      ...
      if (isActiveInThisIteration(event.getIteration(), scenario.getConfig())) {
        analysisFilepath =
            controlerIO.getIterationFilename(event.getIteration(), FLOWANALYSIS_FILENAME);
      }
    ...
    try {
      new CadytsCostOffsetsXMLFileIO<>(
              new ModalCountsLinkLookUp(modalLinkContainer), ModalCountsLinkIdentifier.class)
