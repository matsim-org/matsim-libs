...
public class ModalDistanceCadytsContext
    implements CadytsContextI<Id<DistanceDistribution.DistanceBin>>,
        StartupListener,
        BeforeMobsimListener,
        IterationEndsListener {

  private static final String LINKOFFSET_FILENAME =
      "linkCostOffsets" + ModalDistanceCadytsBuilder.MARGINALS + ".xml";
  private static final String FLOWANALYSIS_FILENAME =
      "flowAnalysis" + ModalDistanceCadytsBuilder.MARGINALS + ".txt";
  ...
  @Inject private DistanceDistribution expectedDistanceDistribution;
  ...
  public void notifyStartup(StartupEvent event) {
    ...
    this.calibrator =
        new ModalDistanceCadytsBuilder()
            .setConfig(config)
  ...
  public void notifyBeforeMobsim(BeforeMobsimEvent event) {
    ...
    for (Person person : scenario.getPopulation().getPersons().values()) {
      cadyts.demand.Plan<Id<DistanceDistribution.DistanceBin>> demand =
          plansTranslator.getCadytsPlan(person.getSelectedPlan());
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    DistanceDistribution simulatedDistanceDistribution =
        expectedDistanceDistribution.copyWithEmptyBins();

    for (Map.Entry<Id<Person>, List<TripEventHandler.Trip>> entry :
        tripEventHandler.getTrips().entrySet()) {
      for (TripEventHandler.Trip trip : entry.getValue()) {
        ...
        DistanceDistribution.DistanceBin bin =
            simulatedDistanceDistribution.getBin(trip.getMainMode(), distance);
        Plan selectedPlan =
            scenario.getPopulation().getPersons().get(entry.getKey()).getSelectedPlan();
    ...
    if (cadytsConfig.isWriteAnalysisFile()
        && shouldWriteAnalysisFile(
            event.getIteration(), config.counts().getWriteCountsInterval())) {
      String filename =
          outputDirectoryHierarchy.getIterationFilename(
              event.getIteration(), FLOWANALYSIS_FILENAME);
      this.calibrator.setFlowAnalysisFile(filename);
    }

    // afterwards call cadyts
    // as a parameter it takes a SimResults function. As our result we just return the count value
    // of the distance
    // bin defined by the bin id. This safes us at least three classes
    this.calibrator.afterNetworkLoading(
        (binId, lower, upper, type) ->
    ...
    // ... getValue() * scalingCorrection.getFactor() where scaling correction is an injected
    // instance

    String offsetFilename =
        outputDirectoryHierarchy.getIterationFilename(event.getIteration(), LINKOFFSET_FILENAME);
    try {
  ...
  private double calculateBeelineDistance(TripEventHandler.Trip trip) {
    ...
    if (scenario.getActivityFacilities().getFacilities().containsKey(trip.getDepartureFacility())
        || scenario
            ...
            .getFacilities()
            .containsKey(trip.getArrivalFacility())) {
      ActivityFacility departureFacility =
          scenario.getActivityFacilities().getFacilities().get(trip.getDepartureFacility());
      ActivityFacility arrivalFacility =
          scenario.getActivityFacilities().getFacilities().get(trip.getArrivalFacility());
      return CoordUtils.calcEuclideanDistance(
          departureFacility.getCoord(), arrivalFacility.getCoord());
    } else {
      Link departureLink = network.getLinks().get(trip.getDepartureLink());
      Link arrivalLink = network.getLinks().get(trip.getArrivalLink());
      return CoordUtils.calcEuclideanDistance(
          departureLink.getToNode().getCoord(), arrivalLink.getToNode().getCoord());
    }
  }

  private boolean shouldWriteAnalysisFile(
      final int currentIteration, final int writeCountsInterval) {
