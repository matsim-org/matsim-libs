...
/*package*/ class ModalCountsSimResultsContainerImpl
    implements SimResults<ModalCountsLinkIdentifier> {

  private static final long serialVersionUID = 1L;
  private final VolumesAnalyzer volumesAnalyzer;
  private final double countsScaleFactor;

  ModalCountsSimResultsContainerImpl(
      final VolumesAnalyzer volumesAnalyzer, final double countsScaleFactor) {
  ...
  public double getSimValue(
      final ModalCountsLinkIdentifier modalLinkCounter,
      final int startTime_s,
      final int endTime_s,
      final TYPE type) { // stopFacility or link

    double[] values =
        volumesAnalyzer.getVolumesPerHourForLink(
            modalLinkCounter.getLinkId(), modalLinkCounter.getMode());
    ...
    // (The javadoc specifies that endTime_s should be _exclusive_.  However, in practice I find
    // 7199 instead of 7200.  So
