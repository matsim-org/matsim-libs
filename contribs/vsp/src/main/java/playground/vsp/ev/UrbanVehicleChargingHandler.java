...
/**
 * This is an events based approach to trigger vehicle charging. Vehicles will be charged as soon as
 * a person begins a PLUGIN_INTERACTION activity. Charging will end as soon as the person performs a
 * PLUGOUT_INTERACTION activity.
 *
 * <p>Do not use this class for charging DVRP vehicles (DynAgents). In that case, vehicle charging
 * is simulated with ChargingActivity (DynActivity)
 *
 * <p>This class is a modified version of {@link VehicleChargingHandler}
 *
 * @author tschlenther
 */
class UrbanVehicleChargingHandler
  ...
  private Map<Id<Link>, Map<Id<Person>, Tuple<Id<Vehicle>, Id<Charger>>>> chargingProcedures =
      new HashMap<>();

  @Inject
  UrbanVehicleChargingHandler(
    ...
    if (event.getActType().endsWith(UrbanEVModule.PLUGIN_INTERACTION)) {
      ...
      if (vehicleId != null) {
        ...
        if (electricFleet.getElectricVehicles().containsKey(evId)) {
          ...
          Map<Id<Person>, Tuple<Id<Vehicle>, Id<Charger>>> proceduresOnLink =
              this.chargingProcedures.get(event.getLinkId());
          if (proceduresOnLink != null && proceduresOnLink.containsKey(event.getPersonId())) {
            throw new RuntimeException(
                "person "
                    + event.getPersonId()
                    + " tries to charge 2 vehicles at the same time on link "
                    + event.getLinkId()
          ...
          throw new IllegalStateException(
              "can not plug in non-registered ev " + evId + " of person " + event.getPersonId());
        }
      } else {
        throw new IllegalStateException(
            "last used vehicle  of person " + event.getPersonId() + "is null. should not happen");
      }
    }
  }
  ...
  public void handleEvent(ActivityEndEvent event) {
    if (event.getActType().endsWith(UrbanEVModule.PLUGOUT_INTERACTION)) {
      Tuple<Id<Vehicle>, Id<Charger>> tuple =
          chargingProcedures.get(event.getLinkId()).remove(event.getPersonId());
      if (tuple != null) {
        Id<Vehicle> evId = Id.create(tuple.getFirst(), Vehicle.class);
        if (vehiclesAtChargers.remove(evId)
            != null) { // if null, vehicle is fully charged and de-plugged already (see
          // handleEvent(ChargingEndedEvent) )
          Id<Charger> chargerId = tuple.getSecond();
          Charger c = chargingInfrastructure.getChargers().get(chargerId);
          c.getLogic()
              .removeVehicle(electricFleet.getElectricVehicles().get(evId), event.getTime());
        }
      } else {
        throw new RuntimeException(
            "there is something wrong with the charging procedure of person="
                + event.getPersonId()
                + " on link= "
                + event.getLinkId());
      }
