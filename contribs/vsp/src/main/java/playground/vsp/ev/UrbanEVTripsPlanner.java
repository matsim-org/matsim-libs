...
class UrbanEVTripsPlanner implements MobsimInitializedListener {
  ...
  @Inject private ChargingInfrastructureSpecification chargingInfrastructureSpecification;
  ...
  @Inject ActivityWhileChargingFinder activityWhileChargingFinder;
  ...
  public void notifyMobsimInitialized(MobsimInitializedEvent e) {
    if (!(e.getQueueSimulation() instanceof QSim)) {
      throw new IllegalStateException(
          UrbanEVTripsPlanner.class + " only works with a mobsim of type " + QSim.class);
    }
    // collect all selected plans that contain ev legs and map them to the set of ev used
    Map<Plan, Set<Id<Vehicle>>> selectedEVPlans =
        StreamEx.of(scenario.getPopulation().getPersons().values())
  ...
  private boolean isEV(Id<Vehicle> vehicleId) {
    return this.electricFleetSpecification
  ...
  private void processPlans(Map<Plan, Set<Id<Vehicle>>> selectedEVPlans) {
    ...
    UrbanEVConfigGroup configGroup =
        (UrbanEVConfigGroup) config.getModules().get(UrbanEVConfigGroup.GROUP_NAME);
    ...
    for (Plan plan : selectedEVPlans.keySet()) {
      ...
      for (Id<Vehicle> ev : selectedEVPlans.get(plan)) {
        // only replan cnt times per vehicle and person. otherwise, there might be a leg which is
        // just too long and we end up in an infinity loop...
        ...
        ElectricVehicleSpecification evSpec =
            electricFleetSpecification.getVehicleSpecifications().get(Id.create(ev, Vehicle.class));
        Leg legWithCriticalSOC;
        ElectricVehicle pseudoVehicle =
            ElectricFleetUtils.create(
                evSpec, driveConsumptionFactory, auxConsumptionFactory, chargingPowerFactory);
        // TODO: erase hardcoding of car mode!
        List<Leg> evCarLegs =
            TripStructureUtils.getLegs(modifiablePlan).stream()
                .filter(leg -> leg.getMode().equals(TransportMode.car))
                .collect(toList());
        boolean pluginBeforeStart = configGroup.getPluginBeforeStartingThePlan();

        if (pluginBeforeStart
            && hasHomeCharger(
                mobsimagent,
                modifiablePlan,
                evCarLegs,
                pseudoVehicle)) { // TODO potentially check for activity duration and/or SoC
          // TODO this does not work !! the planning stuff right here works. However, in the qsim,
          // the initial plugin trip is not reflected and thus the physical ev will not be charged!
          // This then leads to (large) inconsistency of the planning and the qsim!
          // Thus, set pluginBeforeStart to false for the time being!!
          // tschlenther, july '22

          Leg firstEvLeg = evCarLegs.get(0);
          //					Activity originalActWhileCharging = EditPlans.findRealActBefore(mobsimagent,
          // modifiablePlan.getPlanElements().indexOf(firstEvLeg));
          //					Activity lastAct = EditPlans.findRealActBefore(mobsimagent,
          // modifiablePlan.getPlanElements().indexOf(firstEvLeg));
          Activity actWhileCharging = (Activity) modifiablePlan.getPlanElements().get(0);
          Network modeNetwork =
              this.singleModeNetworksCache.getSingleModeNetworksCache().get(firstEvLeg.getMode());
          ...
          Activity newFirstAct =
              factory.createActivityFromLinkId(
                  actWhileCharging.getType(), actWhileCharging.getLinkId());
          ...
          //	planPluginTripFromHomeToCharger(modifiablePlan, routingMode, evSpec, actWhileCharging,
          // chargingLink, tripRouter);

          Leg plugoutLeg = firstEvLeg;
          Activity plugoutTripOrigin =
              findRealOrChargingActBefore(
                  mobsimagent, modifiablePlan.getPlanElements().indexOf(plugoutLeg));
          Activity plugoutTripDestination =
              findRealOrChargingActAfter(
                  mobsimagent, modifiablePlan.getPlanElements().indexOf(plugoutLeg));
          planPlugoutTrip(
              modifiablePlan,
              routingMode,
              evSpec,
              plugoutTripOrigin,
              plugoutTripDestination,
              chargingLink,
              tripRouter,
              timeInterpretation
                  .decideOnActivityEndTimeAlongPlan(plugoutTripOrigin, modifiablePlan)
                  .seconds());

          throw new RuntimeException(
              "currently, plugging in at qsim start does not work. needs debugging. was trying tat for agent "
                  + mobsimagent.getId());
        }
        ...
        do {
          ...
          if (legWithCriticalSOC != null) {
            ...
            if (evLegs.get(0).equals(legWithCriticalSOC)) {
              ...
              AgentsOutOfEnergyData agentsOutOfEnergyData =
                  new AgentsOutOfEnergyData(
                      mobsimagent.getId(),
                      "is running beyond capacity threshold during the first leg of the day.");
              UrbanEVTripsPlanner.agentsOutOfEnergyDataSets.add(agentsOutOfEnergyData);

              // TODO: maybe find nearest charger for next act and let vehicle run empty on first
              // leg but then be charged!?
              break;
              ...
              // trip leads to location of the first (ev-leg-preceding-) activity in the plan and
              // there is a charger and so we can charge at home do not search for opportunity
              // charge before
              // if SoC == 0, we should search for an earlier charge

              Activity actBeforeCharging =
                  EditPlans.findRealActBefore(
                      mobsimagent, modifiablePlan.getPlanElements().indexOf(legWithCriticalSOC));
              Activity lastAct =
                  EditPlans.findRealActAfter(
                      mobsimagent, modifiablePlan.getPlanElements().indexOf(legWithCriticalSOC));
              Network modeNetwork =
                  this.singleModeNetworksCache
                      .getSingleModeNetworksCache()
                      .get(legWithCriticalSOC.getMode());
              Link chargingLink =
                  modeNetwork
                      .getLinks()
                      .get(lastAct.getLinkId()); // TODO possibly let the agent walk to a nearby
              // charger!
              ...
              AgentsOutOfEnergyData agentsOutOfEnergyData =
                  new AgentsOutOfEnergyData(mobsimagent.getId(), "is charging at home.");
              UrbanEVTripsPlanner.agentsOutOfEnergyDataSets.add(agentsOutOfEnergyData);
              break;
              ...
              // TODO: plan for the next day! that means, check at least if the first leg can be
              // done!
              cnt = 0;

            } else {
              // critical leg is somewhere in the middle of the plan
              if (replanPrecedentAndCurrentEVLegs(
                  mobsimagent, modifiablePlan, evSpec, legWithCriticalSOC)) {
                log.info(mobsimagent.getId() + " is charging on the route.");
                AgentsOutOfEnergyData agentsOutOfEnergyData =
                    new AgentsOutOfEnergyData(mobsimagent.getId(), "is charging on the route.");
                UrbanEVTripsPlanner.agentsOutOfEnergyDataSets.add(agentsOutOfEnergyData);
                cnt--;
              } else {
                log.warn(
                    "could not insert plugin activity in plan of agent "
                        + mobsimagent.getId()
   ...
   * @return
   */
  private Leg getCriticalOrLastEvLeg(
      Plan modifiablePlan, ElectricVehicle pseudoVehicle, Id<Vehicle> originalVehicleId) {
    UrbanEVConfigGroup configGroup =
        (UrbanEVConfigGroup) config.getModules().get(UrbanEVConfigGroup.GROUP_NAME);
    ...
    for (PlanElement planElement : modifiablePlan.getPlanElements()) {
      if (planElement instanceof Leg) {
        ...
        if (modesWithVehicles.contains(leg.getMode())
            && VehicleUtils.getVehicleId(modifiablePlan.getPerson(), leg.getMode())
                .equals(originalVehicleId)) {
          lastLegWithVehicle = leg;
          emulateVehicleDischarging(pseudoVehicle, leg);
          if (pseudoVehicle.getBattery().getSoc()
              <= configGroup.getCriticalSOC()) { // TODO randomize?
            return leg;
          }
        }
        ...
        if (((Activity) planElement).getType().contains(UrbanEVModule.PLUGIN_INTERACTION)) {
          Leg legToCharger =
              (Leg)
                  modifiablePlan
                      .getPlanElements()
                      .get(modifiablePlan.getPlanElements().indexOf(planElement) - 1);
          chargingBegin =
              legToCharger.getDepartureTime().seconds() + legToCharger.getTravelTime().seconds();
          ...
          ChargerSpecification chargerSpecification =
              chargingInfrastructureSpecification.getChargerSpecifications().values().stream()
                  .filter(
                      charger -> charger.getLinkId().equals(((Activity) planElement).getLinkId()))
                  .filter(
                      charger -> pseudoVehicle.getChargerTypes().contains(charger.getChargerType()))
                  .findAny()
                  .orElseThrow();

          double energy =
              pseudoVehicle.getChargingPower().calcChargingPower(chargerSpecification)
                  * chargingDuration;
          double newCharge =
              Math.min(
                  pseudoVehicle.getBattery().getCharge() + energy,
                  pseudoVehicle.getBattery().getCapacity());
          pseudoVehicle.getBattery().setCharge(newCharge);
        }
    ...
    if (lastLegWithVehicle == null) {
      throw new RuntimeException(
          "found no leg with vehicle "
              + originalVehicleId
              + ". Should not happen. Is the corresponding mode a network mode?");
    } else {
   ...
   * @param leg
   */
  private boolean replanPrecedentAndCurrentEVLegs(
      MobsimAgent mobsimagent,
      Plan modifiablePlan,
      ElectricVehicleSpecification electricVehicleSpecification,
      Leg leg) {
    Network modeNetwork =
        this.singleModeNetworksCache.getSingleModeNetworksCache().get(leg.getMode());

    String routingMode = TripStructureUtils.getRoutingMode(leg);
    List<Leg> evLegs =
        TripStructureUtils.getLegs(modifiablePlan).stream()
            .filter(evleg -> evleg.getMode().equals(routingMode))
            .collect(toList());
    ...
    do {
      actWhileCharging =
          activityWhileChargingFinder.findActivityWhileChargingBeforeLeg(
              mobsimagent,
              modifiablePlan,
      ...
      selectedCharger =
          selectChargerNearToLink(
              actWhileCharging.getLinkId(), electricVehicleSpecification, modeNetwork);

      if (selectedCharger == null) {
        log.warn(
            "no charger within max distance could be found for link "
    ...
    //		Activity pluginTripOrigin = EditPlans.findRealActBefore(mobsimagent,
    // modifiablePlan.getPlanElements().indexOf(actWhileCharging));
    Activity pluginTripOrigin =
        findRealOrChargingActBefore(
            mobsimagent, modifiablePlan.getPlanElements().indexOf(actWhileCharging));

    Leg plugoutLeg =
        activityWhileChargingFinder.getNextLegOfRoutingModeAfterActivity(
            ImmutableList.copyOf(modifiablePlan.getPlanElements()), actWhileCharging, routingMode);
    Activity plugoutTripOrigin =
        findRealOrChargingActBefore(
            mobsimagent, modifiablePlan.getPlanElements().indexOf(plugoutLeg));
    Activity plugoutTripDestination =
    ...
    { // some consistency checks.. //TODO consider to put in a JUnit test..
      int offset =
          config.routing().getAccessEgressType().equals(RoutingConfigGroup.AccessEgressType.none)
              ? 1
              : 3;

      Preconditions.checkNotNull(
          pluginTripOrigin, "pluginTripOrigin is null. should never happen..");
      Preconditions.checkState(
          !pluginTripOrigin.equals(actWhileCharging),
          "pluginTripOrigin is equal to actWhileCharging. should never happen..");

      PlanElement legToBeReplaced =
          modifiablePlan
              .getPlanElements()
              .get(modifiablePlan.getPlanElements().indexOf(pluginTripOrigin) + offset);
      Preconditions.checkState(legToBeReplaced instanceof Leg);
      Preconditions.checkState(
          TripStructureUtils.getRoutingMode((Leg) legToBeReplaced).equals(routingMode),
          "leg after pluginTripOrigin has the wrong routing mode. should not happen..");

      legToBeReplaced =
          modifiablePlan
              .getPlanElements()
              .get(modifiablePlan.getPlanElements().indexOf(actWhileCharging) - offset);
      Preconditions.checkState(legToBeReplaced instanceof Leg);
      Preconditions.checkState(
          TripStructureUtils.getRoutingMode((Leg) legToBeReplaced).equals(routingMode),
      ...
      Preconditions.checkState(
          !plugoutTripDestination.equals(actWhileCharging),
          "plugoutTripDestination is equal to actWhileCharging. should never happen..");

      Preconditions.checkState(
          modifiablePlan.getPlanElements().indexOf(pluginTripOrigin)
              < modifiablePlan.getPlanElements().indexOf(actWhileCharging));
      Preconditions.checkState(
          modifiablePlan.getPlanElements().indexOf(actWhileCharging)
              <= modifiablePlan.getPlanElements().indexOf(plugoutTripOrigin));
      Preconditions.checkState(
          modifiablePlan.getPlanElements().indexOf(plugoutTripOrigin)
              < modifiablePlan.getPlanElements().indexOf(plugoutTripDestination));

      legToBeReplaced =
          modifiablePlan
              .getPlanElements()
              .get(modifiablePlan.getPlanElements().indexOf(plugoutTripOrigin) + offset);
      Preconditions.checkState(legToBeReplaced instanceof Leg);
      Preconditions.checkState(
          TripStructureUtils.getRoutingMode((Leg) legToBeReplaced).equals(routingMode),
      ...
      legToBeReplaced =
          modifiablePlan
              .getPlanElements()
              .get(modifiablePlan.getPlanElements().indexOf(plugoutTripDestination) - offset);
      Preconditions.checkState(legToBeReplaced instanceof Leg);
      Preconditions.checkState(
          TripStructureUtils.getRoutingMode((Leg) legToBeReplaced).equals(routingMode),
  ...
  private void planPlugoutTrip(
    ...
    // actually destination can not be null based on how we determine the actWhileCharging = origin
    // at the moment...
    ...
    Activity plugOutAct =
        PopulationUtils.createStageActivityFromCoordLinkIdAndModePrefix(
            chargingLink.getCoord(),
            chargingLink.getId(),
            routingMode + UrbanEVModule.PLUGOUT_IDENTIFIER);
    plugOutAct =
        PopulationUtils.createActivity(
            plugOutAct); // createStageActivity... creates a InteractionActivity where duration
    // cannot be set.
    trip.add(plugOutAct);
    now = timeInterpretation.decideOnElementEndTime(accessLeg, now).seconds();
    ...
    routedSegment =
        tripRouter.calcRoute(
            routingMode, chargerFacility, toFacility, now, plan.getPerson(), new AttributesImpl());
    ...
    if (!plan.getPlanElements().get(plan.getPlanElements().size() - 1).equals(destination)) {
      destination.setEndTime(
          timeInterpretation.decideOnActivityEndTime(destination, now).seconds());
    }
  }

  private void planPluginTrip(
      Plan plan,
      String routingMode,
      ElectricVehicleSpecification electricVehicleSpecification,
      Activity actBeforeCharging,
    ...
    Facility fromFacility =
        FacilitiesUtils.toFacility(actBeforeCharging, scenario.getActivityFacilities());
    Facility chargerFacility = new LinkWrapperFacility(chargingLink);
    Facility toFacility =
        FacilitiesUtils.toFacility(actWhileCharging, scenario.getActivityFacilities());
    ...
    List<? extends PlanElement> routedSegment =
        tripRouter.calcRoute(
            ...
            fromFacility,
            chargerFacility,
            timeInterpretation.decideOnActivityEndTimeAlongPlan(actBeforeCharging, plan).seconds(),
            plan.getPerson(),
            new AttributesImpl());
    ...
    double now =
        lastLeg.getDepartureTime().seconds() + lastLeg.getRoute().getTravelTime().seconds();
    trip.addAll(routedSegment);
    ...
    pluginAct =
        PopulationUtils.createActivity(
            pluginAct); // createStageActivity... creates a InteractionActivity where duration
    // cannot be set.
    trip.add(pluginAct);
    ...
    if (!plan.getPlanElements().get(plan.getPlanElements().size() - 1).equals(actWhileCharging)) {
      actWhileCharging.setEndTime(
          timeInterpretation.decideOnActivityEndTime(actWhileCharging, now).seconds());
    }
  }

  // possibly put behind interface
  @Nullable
  private ChargerSpecification selectChargerNearToLink(
      Id<Link> linkId, ElectricVehicleSpecification vehicleSpecification, Network network) {

    UrbanEVConfigGroup configGroup =
        (UrbanEVConfigGroup) config.getModules().get(UrbanEVConfigGroup.GROUP_NAME);
    double maxDistanceToAct = configGroup.getMaxDistanceBetweenActAndCharger_m();

    List<ChargerSpecification> chargerList =
        chargingInfrastructureSpecification.getChargerSpecifications().values().stream()
            .filter(
                charger ->
                    vehicleSpecification.getChargerTypes().contains(charger.getChargerType()))
            .collect(Collectors.toList());

    StraightLineKnnFinder<Link, ChargerSpecification> straightLineKnnFinder =
        new StraightLineKnnFinder<>(
            1,
            l -> l.getToNode().getCoord(),
            s ->
                network
                    .getLinks()
                    .get(s.getLinkId())
                    .getToNode()
                    .getCoord()); // TODO get closest X chargers and choose randomly?
    List<ChargerSpecification> nearestChargers =
        straightLineKnnFinder.findNearest(network.getLinks().get(linkId), chargerList.stream());
    if (nearestChargers.isEmpty()) {
      throw new RuntimeException("no charger could be found for vehicle: " + vehicleSpecification);
    }
    ChargerSpecification chosenCharger =
        nearestChargers.get(0); // currently we chose the closest one
    double distanceFromActToCharger =
  ...
  /**
   * this method has the side effect that the soc of the ev is altered by estimated energy
   * consumption of the leg
   *
   * @param ev
   * @param leg
   */
  private void emulateVehicleDischarging(ElectricVehicle ev, Leg leg) {
    ...
    for (Link l : links) {
      double travelT =
          travelTime.getLinkTravelTime(l, leg.getDepartureTime().seconds(), null, null);

      double driveConsumption =
          driveEnergyConsumption.calcEnergyConsumption(l, travelT, linkEnterTime);
      double auxConsumption =
          auxEnergyConsumption.calcEnergyConsumption(
              leg.getDepartureTime().seconds(), travelT, l.getId());
      //			double consumption = driveEnergyConsumption.calcEnergyConsumption(l, travelT,
      // linkEnterTime)
      //					+ auxEnergyConsumption.calcEnergyConsumption(leg.getDepartureTime().seconds(), travelT,
      // l.getId());
      ...
      ev.getBattery()
          .dischargeEnergy(
              ...
              missingEnergy -> {
                throw new RuntimeException(
                    "Energy consumed greater than the current charge. Missing energy: "
                        + missingEnergy);
              });
  ...
  //	the following methods are modified versions of EditPlans.findRealActBefore() and
  // EditPlans.findRealActAfter()
    ...
    for (int ii = 0; ii < index; ii++) {
      if (planElements.get(ii) instanceof Activity) {
        ...
        if (!StageActivityTypeIdentifier.isStageActivity(act.getType())
            || act.getType().contains(UrbanEVModule.PLUGIN_INTERACTION)
            || act.getType().contains(UrbanEVModule.PLUGOUT_INTERACTION)) {
          prevAct = act;
        }
      }
    }
  ...
  private Boolean isHomeChargingTrip(
      MobsimAgent mobsimAgent, Plan modifiablePlan, List<Leg> evLegs, ElectricVehicle ev) {
    ...
    boolean isHomeTrip =
        EditPlans.findRealActAfter(
                mobsimAgent,
                modifiablePlan.getPlanElements().indexOf(evLegs.get(evLegs.size() - 1)))
            .getLinkId()
            .equals(homeLink);
        ...
        chargingInfrastructureSpecification.getChargerSpecifications().values().stream()
            .filter(
                chargerSpecification ->
                    ev.getChargerTypes().contains(chargerSpecification.getChargerType()))
              ...
              Files.newBufferedWriter(
                  Paths.get(
                      controlerIO.getIterationFilename(
                          iterationCounter.getIterationNumber(), "agentsOutOfEnergy.csv"))),
              CSVFormat.DEFAULT.withDelimiter(';').withHeader("PersonID", "Reason"));
      {
        for (AgentsOutOfEnergyData agentsOutOfEnergyData :
            UrbanEVTripsPlanner.agentsOutOfEnergyDataSets) {
