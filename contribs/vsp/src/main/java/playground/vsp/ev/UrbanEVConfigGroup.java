...
public class UrbanEVConfigGroup extends ReflectiveConfigGroup {
  ...
  // TODO should we rename the entire package from UrbanEV to EVChargingPreplanning or something
  // similar?
  ...
  /**
   * determines how often an agent can charge per day at max. During the pre-planning process, the
   * agent has to go through it's entire plan n + 1 times where n is the number of charging
   * procedures he ends up with.
   */
  ...
  private static final String MIN_WHILE_CHARGING_ACT_DURATION_s =
      "minWhileChargingActivityDuration_s";

  /**
   * determines the minimum duration for activities to be determined suitable for charging the
   * vehicle during the performance of the activity. In seconds.
   */
  ...
  // TODO: the setting of this to true currently leads to inconsistency between the planning and the
  // qsim, due to the non-reflection of plugin trip at the start of the day. needs further
  // investigation!
  private static boolean pluginBeforeStartingThePlan = false;

  /**
   * determines the plug in act before the start of the plan to simulate charging between last act
   * of the plan and first
   */
