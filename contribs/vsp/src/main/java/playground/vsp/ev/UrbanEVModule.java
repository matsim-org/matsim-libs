...
public class UrbanEVModule extends AbstractModule {
  ...
  public static final String PLUGIN_INTERACTION =
      ScoringConfigGroup.createStageActivityType(PLUGIN_IDENTIFIER);
  static final String PLUGOUT_IDENTIFIER = " plugout";
  public static final String PLUGOUT_INTERACTION =
      ScoringConfigGroup.createStageActivityType(PLUGOUT_IDENTIFIER);
  @Inject private Config config;

  @Override
  public void install() {
    QSimComponentsConfigGroup qsimComponentsConfig =
        ConfigUtils.addOrGetModule(config, QSimComponentsConfigGroup.class);
    qsimComponentsConfig.addActiveComponent(EvModule.EV_COMPONENT);

    //		UrbanEVConfigGroup urbanEVConfig = ConfigUtils.addOrGetModule( config,
    // UrbanEVConfigGroup.class );

    //		if (urbanEVConfig == null)
    //			throw new IllegalArgumentException(
    //					"no config group of type " + UrbanEVConfigGroup.GROUP_NAME + " was specified in the
    // config");
    // was this meaningful?  I.e. do we want the code to fail if there is no such config group?
    // kai, apr'23
    ...
    // (this takes the matsim modal vehicles for each leg and gives them to the
    // ElectricFleetSpecification.  Don't know why it has to be in
    ...
    installQSimModule(
        new AbstractQSimModule() {
          ...
          protected void configureQSim() {
            // this is responsible for charging vehicles according to person activity start and end
            // events..
            //				bind(UrbanVehicleChargingHandler.class);
            addMobsimScopeEventHandlerBinding().to(UrbanVehicleChargingHandler.class);
            // (I think that this takes the plugin/plugout activities, and actually initiates the
            // charging.  kai, apr'23)
          }
        });
    ...
    // (I think that this inserts the charging activities just before the mobsim starts (i.e. it is
    // not in the plans).  kai, apr'23)

    // TODO find a better solution for this yyyy yes.  We do not want automagic.  kai, apr'23  done.
    //  kai, apr'23
    //		Collection<String> whileChargingActTypes =
    // urbanEVConfig.getWhileChargingActivityTypes().isEmpty() ?
    //				config.planCalcScore().getActivityTypes() :
    //				urbanEVConfig.getWhileChargingActivityTypes();

    //		bind(ActivityWhileChargingFinder.class).toInstance(new
    // ActivityWhileChargingFinder(whileChargingActTypes,
