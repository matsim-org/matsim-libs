...
/**
 * this is an example of how to run MATSim with the UrbanEV module which inserts charging activities
 * for all legs which use a EV. By default, {@link ElectricFleetUpdater} is used, which declares any
 * vehicle as an EV that has a vehicle type with HbefaTechnology set to 'electricity'. At the
 * beginning of each iteration, the consumption is estimated. Charging is planned to take place
 * during the latest possible activity in the agent's plan that fits certain criteria (ActivityType
 * and minimum duration) and takes place before the estimated SOC drops below a defined threshold.
 */
public class RunUrbanEVExample {
  ...
  private static Config prepareConfig(String[] args) {
    ...
    config
        .scoring()
        .addActivityParams(
            new ActivityParams(TransportMode.car + UrbanEVModule.PLUGOUT_INTERACTION)
                .setScoringThisActivityAtAll(false));
    config
        .scoring()
        .addActivityParams(
            new ActivityParams(TransportMode.car + UrbanEVModule.PLUGIN_INTERACTION)
                .setScoringThisActivityAtAll(false));
    return config;
  }
  ...
  static void createAndRegisterPersonalCarAndBikeVehicles(Scenario scenario) {
    ...
    for (Person person : scenario.getPopulation().getPersons().values()) {
      ...
      VehicleType carVehicleType =
          vehicleFactory.createVehicleType(
              Id.create(
      ...
      VehicleUtils.setEnergyCapacity(
          carVehicleType.getEngineInformation(), CAR_BATTERY_CAPACITY_kWh);
      ElectricFleetUtils.setChargerTypes(
          carVehicleType.getEngineInformation(), Arrays.asList("a", "b", "default"));
      scenario.getVehicles().addVehicleType(carVehicleType);
      Vehicle carVehicle =
          vehicleFactory.createVehicle(
              VehicleUtils.createVehicleId(person, TransportMode.car), carVehicleType);
      ...
      Vehicle bikeVehicle =
          vehicleFactory.createVehicle(
              VehicleUtils.createVehicleId(person, TransportMode.bike), bikeVehicleType);
