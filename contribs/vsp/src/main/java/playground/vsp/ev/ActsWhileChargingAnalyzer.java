...
class ActsWhileChargingAnalyzer
  ...
  @Inject
  public ActsWhileChargingAnalyzer(
      ChargingInfrastructureSpecification chargingInfrastructureSpecification, Scenario scenario) {
  ...
  public void handleEvent(ActivityStartEvent event) {
    if (!TripStructureUtils.isStageActivityType(event.getActType())) {
      ...
      containers.stream()
          ...
          .get()
          .acts
          .add(chargingActAndTime);

      PersonContainer personContainer =
          new PersonContainer(
              event.getPersonId().toString(), chargingActAndTime, event.getTime(), null);
              ...
              Files.newBufferedWriter(
                  Paths.get(
                      controlerIO.getIterationFilename(
                          iterationCounter.getIterationNumber(), "actsPerCharger.csv"))),
              CSVFormat.DEFAULT
                  .withDelimiter(';')
      ...
      for (PersonContainer personContainer : personContainers) {
        ...
        if (plan.get(plan.indexOf(personContainer.chargingAct)) != plan.get(plan.size() - 1)) {
          csvPrinter.printRecord(
              personContainer.personId,
              personContainer.chargerId,
              plan.get(plan.indexOf(personContainer.chargingAct) + 1),
              Time.writeTime(personContainer.time));
        }
      }
