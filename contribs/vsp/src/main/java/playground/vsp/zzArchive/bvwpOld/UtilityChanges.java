...
abstract class UtilityChanges {
  private static final String FMT_STRING =
      "%16s || %16.2f | %16.1f || %16.2f | %16.1f || %16.2f | %16.1f || %16.2f | %16.1f ||\n";
  ...
  final void computeAndPrintResults(
    ...
    for (String id : nullfall.getAllRelations()) { // for all OD relations
      ...
      for (DemandSegment demandSegment : DemandSegment.values()) { // for all types (e.g. PV or GV)
        ...
        Mode improvedMode =
            autodetectImprovingMode(nullfallForODRelation, planfallForODRelation, demandSegment);
        ...
        Attributes econValuesImprovedMode =
            economicValues.getByMode(improvedMode).getByDemandSegment(demandSegment);
        Attributes attributesNullfallImprovedMode =
            nullfallForODRelation.getByMode(improvedMode).getByDemandSegment(demandSegment);
        Attributes attributesPlanfallImprovedMode =
            planfallForODRelation.getByMode(improvedMode).getByDemandSegment(demandSegment);
        ...
        html.write("Attribut");
        html.nextTableEntry();
        html.write("Attribut Nullfall");
        html.nextTableEntry();
        html.write("... mal Menge");
        html.nextTableEntry();
        html.write("Attribut Planfall");
        html.nextTableEntry();
        html.write("... mal Menge");
        html.nextTableEntry();
        html.write("Attribut Diff");
        html.nextTableEntry();
        html.write("... mal Menge");
        html.nextTableEntry();
        html.write("Nutzen Diff");
        html.nextTableEntry();
        html.write("... mal Menge");
        html.endTableRow();
        ...
        for (Mode mode : Mode.values()) { // for all modes
          ...
          Attributes attributesNullfall =
              nullfallForODRelation.getByMode(mode).getByDemandSegment(demandSegment);
          Attributes attributesPlanfall =
              planfallForODRelation.getByMode(mode).getByDemandSegment(demandSegment);
          ...
          if (amountPlanfall != 0. || amountNullfall != 0.) {
            ...
            if (mode == improvedMode) {
              ...
              System.out.printf(
                  "%16s; %16s; %16s; wechselnder Verkehr: %16.1f Personen/Tonnen\n",
              ...
              utils =
                  computeAndPrintGivingOrReceiving(
                      utils,
                      econValuesImprovedMode,
                      attributesNullfallImprovedMode,
                      attributesPlanfallImprovedMode,
                      -deltaAmounts,
                      html);
            ...
            for (Attribute attribute : Attribute.values()) {
              if (attribute != Attribute.XX && attribute != Attribute.costOfProduction) {
                final double improvementOfAttribute =
                    attributesPlanfall.getByEntry(attribute)
                        - attributesNullfall.getByEntry(attribute);
                utilsUserFromRoH +=
          ...
          final double revenueNullfall =
              attributesNullfall.getByEntry(Attribute.priceUser) * amountNullfall;
          final double revenuePlanfall =
              attributesPlanfall.getByEntry(Attribute.priceUser) * amountPlanfall;
          final double operatorCostNullfall =
              attributesNullfall.getByEntry(Attribute.costOfProduction) * amountNullfall;
          final double operatorCostPlanfall =
              attributesPlanfall.getByEntry(Attribute.costOfProduction) * amountPlanfall;
          operatorProfit +=
    ...
    System.out.printf(
        "RoH: utl gain users: %16.1f ; operator profit gain: %16.1f ; sum: %16.1f\n",
    ...
    html.write(
        "RoH: utl gain users: "
            ...
            + operatorProfit
            + "; sum: "
            + (utilsUserFromRoH + operatorProfit));
    html.endTableRow();
      ...
      if (attribute != Attribute.XX && attribute != Attribute.priceUser) {
        ...
        if (deltaAmounts > 0) {
          ...
          System.out.printf(
              ...
              -attributeValueNullfall,
              attributeValueNullfall * deltaAmounts,
              // (selbst wenn sich das abgebende System ändert, so ist unser Gewinn dennoch
              // basierend auf dem
              // Nullfall)
              utlChangesPerItem.utl,
              utlChange);
  ...
  private double computeAndPrintImplicitUtl(
      ...
      Html html) {
    // implicit utility:
    final double implicitUtlImprovedMode =
        this.computeImplicitUtility(
            econValuesImprovedMode, attributesNullfallImprovedMode, attributesPlanfallImprovedMode);

    final double implicitUtl =
        this.computeImplicitUtility(econValues, attributesNullfall, attributesPlanfall);
    ...
    if (implicitUtl != 0. || implicitUtlImprovedMode != 0.) {
      ...
      System.out.printf(
          ...
          0.,
          0.,
          implicitUtlImprovedMode,
          -implicitUtlImprovedMode * deltaAmounts);
      System.out.printf(
          FMT_STRING, "implicit utl", 0., 0., 0., 0., 0., 0., implicitUtlSum, implicitUtlOverall);
      html.bvwpTableRow("implicit utl", 0., 0., 0., 0., 0., 0., implicitUtlSum, implicitUtlOverall);
  ...
  private static Mode autodetectImprovingMode(
      Values nullfallForODRelation, Values planfallForODRelation, DemandSegment demandSegment) {
  ...
  private double computeAndPrintValuesForAltnutzer(
    ...
    for (Attribute attribute : Attribute.values()) { // for all entries (e.g. km or hrs)
      if (attribute != Attribute.XX && attribute != Attribute.priceUser) {
        // yyyy not so great: if policy measure = price change, then RoH and resource consumption
        // are
  ...
  abstract double computeImplicitUtility(
