 ...
 * For marginalUtilityOfMoney an attribute {@link PersonUtils#getIncome(Person)} for persons that have a specific
 * income is used. Persons in the population, that have no attribute {@link PersonUtils#getIncome(Person)} will use the
 ...
 * where
 * AVERAGE_INCOME is computed as the average of all values of the attribute {@link PersonUtils#getIncome(Person)} that are contained in the population.
 * <p>
 ...
 */
public class PersonSpecificScoringParameters implements ScoringParametersForPerson {
	private static final Logger log = LogManager.getLogger( PersonSpecificScoringParameters.class );
    ...
    @Inject PersonSpecificScoringParameters( Population population, ScoringConfigGroup scoringConfigGroup, ScenarioConfigGroup scenarioConfigGroup, TransitConfigGroup transitConfigGroup ) {
    ...
    public ScoringParameters getScoringParameters(Person person) {
        ...
        if (scoringParametersForThisPerson == null) {
			// yy we need the mode params from somewhere, probably by subpop
			// yy we need to retrofit for persons that do not have the relevant attributes, probably with a delegate

			Map<String,Double> startTimes = (Map<String, Double>) person.getAttributes().getAttribute( "startTimesFromData" );
			Map<String,Double> durations = (Map<String, Double>) person.getAttributes().getAttribute( "durationsFromData" );

			for( Map.Entry<String, Double> entry : startTimes.entrySet() ){
				String actType = entry.getKey();
				double startTime = entry.getValue();
				double duration = durations.get( actType );
				ScoringConfigGroup.ActivityParams actParams = new ScoringConfigGroup.ActivityParams( actType );
				actParams.setTypicalDuration( duration );
			}
			// looks like we do not even need the start time for this most simple version.  ????
