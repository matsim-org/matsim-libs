...
/**
 * @author tschlenther
 *     <p>This class is an adoption of {@link
 *     org.matsim.core.scoring.functions.SubpopulationScoringParameters}. It additionaly allows for
 *     person-specific marginalUtilityOfMoney. For marginalUtilityOfMoney an attribute {@link
 *     org.matsim.core.population.PersonUtils#getIncome(Person)} for persons that have a specific
 *     income is used. Persons in the population, that have no attribute {@link
 *     org.matsim.core.population.PersonUtils#getIncome(Person)} will use the default marginal
 *     utility set in their subpopulation's scoring parameters.
 *     <p>The person specific marginal utility is computed by subpopulationMgnUtilityOfMoney *
 *     AVERAGE_INCOME / PERSONAL_INCOME where AVERAGE_INCOME is computed as the average of all
 *     values of the attribute {@link org.matsim.core.population.PersonUtils#getIncome(Person)} that
 *     are contained in the population.
 *     <p>If you want to distinguish between 'rich' areas and 'poor' areas, make use of the
 *     subpopulation feature and set subpopulation-specific mgnUtilityOfMoney in the
 *     #PlanCalcScoreConfigGroup
 */
public class IncomeDependentUtilityOfMoneyPersonScoringParameters
  ...
  private final Map<String, Map<String, ActivityUtilityParameters>> activityParamsPerSubpopulation =
      new ConcurrentHashMap<>();

  @Inject
  IncomeDependentUtilityOfMoneyPersonScoringParameters(
  ...
  private double computeAvgIncome(Population population) {
    log.info(
        "reading income attribute using "
            + PersonUtils.class
    ...
    OptionalDouble averageIncome =
        population.getPersons().values().stream()
            .filter(
                person ->
                    PersonUtils.getIncome(person)
                        != null) // consider only agents that have a specific income provided
    ...
    if (averageIncome.isEmpty()) {
      throw new RuntimeException(
          "you are using "
              + this.getClass()
              + " but there is not a single income attribute in the population! "
  ...
  public ScoringParameters getScoringParameters(Person person) {
    // (as so often, this functionality is not attached to the person (i.e. person. get...), but
    // "wrapped around" it (i.e. get...(person)). kai, apr'22)
    ...
    if (scoringParametersForThisPerson == null) {
      final String subpopulation =
          PopulationUtils.getSubpopulation(person) == null
              ? "default"
              : PopulationUtils.getSubpopulation(person);
      // (weird, but so be it.  kai, apr'22)

      // the following is a comment that was orinally put into SubpopulationScoringParams, which is
      ...
      ScoringConfigGroup.ScoringParameterSet subpopulationScoringParams =
          this.config.getScoringParameters(subpopulation);
      // (we can set scoring params per subpopulation, so retrieve them as starting point.  kai,
      // apr'22)

      // save the activityParams of the subpopulation so we need to build them only once.
      this.activityParamsPerSubpopulation.computeIfAbsent(
          subpopulation,
          k -> {
            Map<String, ActivityUtilityParameters> activityParams = new TreeMap<>();
            for (ScoringConfigGroup.ActivityParams params :
                subpopulationScoringParams.getActivityParams()) {
              ActivityUtilityParameters.Builder factory =
                  new ActivityUtilityParameters.Builder(params);
              activityParams.put(params.getActivityType(), factory.build());
            }
            return activityParams;
          });
      // (I think that this is just an adapter class, converting the representation in the config to
      // some internal representation.  kai, apr'22)
      ...
      ScoringParameters.Builder builder =
          new ScoringParameters.Builder(
              this.config,
              subpopulationScoringParams,
              this.activityParamsPerSubpopulation.get(subpopulation),
              scConfig);
      // (odd that this now needs both the version from the config and the version from the adapter
      // class.  kai, apr'22)
      ...
      if (PersonUtils.getIncome(person) != null) {
        ...
        // (getIncome returns Double, i.e. "null" is an option.  Not sure what happens if this is
        // now converted to the primitive type.  kai, apr'22)

        if (personalIncome > 0) {
          builder.setMarginalUtilityOfMoney(
              subpopulationScoringParams.getMarginalUtilityOfMoney()
                  * globalAvgIncome
                  / personalIncome);
        } else {
          // (not sure what this means.  "null" would have been an option, but is made impossible,
          // see above.  An income of 0 may be possible, but with the 1/y that we often use it
          // should be avoided.)
          log.warn(
              "you have set income to "
                  + personalIncome
                  + " for person "
                  + person
                  + ". This is invalid and gets ignored."
      ...
      this.params.put(person.getId(), scoringParametersForThisPerson);
    }
