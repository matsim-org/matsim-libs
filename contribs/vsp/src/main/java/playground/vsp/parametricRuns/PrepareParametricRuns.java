...
/**
 * A class to create a job script, write it on remote and then run the job based on the given
 * parameters.
 *
 * <p>(a) create a public-private key pair as (it does not work if password is set). 'ssh-keygen -t
 * rsa -b 4096 -f $HOME/.ssh/nameOfTheKey' (b) copy public key ('nameOfTheKey.pub') to cluster as
 * 'ssh-copy-id -i nameOfTheKey.pub agarwal@cluster-i.math.tu-berlin.de'
 *
 * <p>Created by amit on 04.10.17.
 */
public class PrepareParametricRuns {
  ...
  public PrepareParametricRuns(String pathToKnownHosts, String pathToPrivateKey, String userName) {
    ...
    try {
      ...
      config.put(
          ...
          "no"); // so that no question asked, and script run without any problem
    ...
    System.out.println(
        "Session is connected. Please note that, a few default parameters (see "
            + JobScriptWriter.class.getSimpleName()
            + " for default params) are used in the job script."
  ...
  private void executeCommand(final String command, final StringBuilder output) {
    try {
      ...
      BufferedReader bufferedReader =
          new BufferedReader(new InputStreamReader(stdout, Charsets.UTF_8));
      channel.connect();
  ...
  private String[] prepareCommands(
      final String[] additionalLines, final String baseDir, final String jobName) {
    ...
    if (!isExists) {
      try {
        ...
        throw new RuntimeException(
            "Data is not written/read. Reason : "
                + e
                + ". Check if the parent directory exists on remote. \n"
