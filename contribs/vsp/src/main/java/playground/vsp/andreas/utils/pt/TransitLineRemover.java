...
public class TransitLineRemover {
  ...
  public static void main(String[] args) {
    ...
    final String NO_TXL_SCHEDULE_FILE =
        "e:/_shared-svn/andreas/paratransit/input/trb_2012/transitSchedules/transitSchedule_noTxlBusLines.xml.gz";
    final String TXL_BUS_LINES =
        "e:/_shared-svn/andreas/paratransit/input/trb_2012/transitSchedules/transitSchedule_onlyTxlBusLines.xml.gz";
    final String NO_BVG_BUSES =
        "e:/_shared-svn/andreas/paratransit/input/trb_2012/transitSchedules/transitSchedule_noBvgBusLines.xml.gz";
    final String ONLY_BVG_BUSES =
        "e:/_shared-svn/andreas/paratransit/input/trb_2012/transitSchedules/transitSchedule_onlyBvgBusLines.xml.gz";
    ...
    new TransitScheduleReaderV1(baseCaseTransitSchedule, new RouteFactories())
    ...
    //				log.info("Moving stop facility " + stop.getId() + " " + stop.getName() + " from link " +
    // stop.getLinkId() + "...");
    ...
    Set<Id<TransitStopFacility>> stopsInArea =
        TransitLineRemover.getStopIdsWithinArea(baseCaseTransitSchedule, minCoord, maxCoord);
    Set<Id<TransitLine>> linesToRemove =
        TransitLineRemover.getLinesServingTheseStops(baseCaseTransitSchedule, stopsInArea);

    TransitSchedule noTxlTransitSchedule =
        TransitLineRemover.removeTransitLinesFromTransitSchedule(
            baseCaseTransitSchedule, linesToRemove);
    ...
    TransitSchedule txlBusLinesTransitSchedule =
        TransitLineRemover.removeTransitLinesFromTransitSchedule(
            baseCaseTransitSchedule, linesToKeep);
    ...
    TransitSchedule noBvgBusLinesSchedule =
        TransitLineRemover.removeTransitLinesFromTransitSchedule(baseCaseTransitSchedule, buslines);
    ...
    TransitSchedule onlyBvgBusLinesSchedule =
        TransitLineRemover.removeTransitLinesFromTransitSchedule(
            baseCaseTransitSchedule, noBuslines);
    new TransitScheduleWriterV1(onlyBvgBusLinesSchedule).write(ONLY_BVG_BUSES);
  }

  public static Set<Id<TransitLine>> getLinesServingTheseStops(
      TransitSchedule transitSchedule, Set<Id<TransitStopFacility>> stopIds) {
    ...
    log.info(
        "Found the following "
            + linesServingOneOfThoseStops.size()
            + " lines: "
            + linesServingOneOfThoseStops);
    return linesServingOneOfThoseStops;
  }

  public static Set<Id<TransitStopFacility>> getStopIdsWithinArea(
      TransitSchedule transitSchedule, Coord minCoord, Coord maxCoord) {
    log.info(
        "Searching for stops within the area of "
            + minCoord.toString()
            + " - "
            + maxCoord.toString());
  ...
  public static TransitSchedule removeTransitLinesFromTransitSchedule(
      TransitSchedule transitSchedule, Set<Id<TransitLine>> linesToRemove) {
    ...
    log.info(
        "Old schedule contained " + transitSchedule.getTransitLines().values().size() + " lines.");
