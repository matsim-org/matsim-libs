 ...
 *
 * <ul>
 *   <li>cartesian: x^4+(x^2)*(y^2)+y^4=x*(x^2+y^2)
 *   <li>parametrically: X = a*cos(t), Y = b*sin(t)
 *   <li>area: A = 1.058049*a*b
 * </ul>
 *
 * In space, a Bean curve is described by the following 5 parameters:
 *
 * <ul>
 *   <li>x, y: the Coordinates of the center of the bean
 *   <li>a, b: the length of the two orthogonal axes
 *   <li>theta: the rotation of the main axis a
 * </ul>
 *
 * In our case, we use a 4-dimensional parameter space with the following 4 axes and their ranges:
 *
 * <ul>
 *   <li>x : ]-INF .. +INF[
 *   <li>y : ]-INF .. +INF[
 ...
 * </ul>
 *
 * The effective size of the bean curve is given implicitely by a minimum cover-percentage. The bean
 * curve will be enlarged until it contains at least the specified percentage of all points.
 */
public class BeanObjective implements Objective {
  ...
  private static final Logger log = LogManager.getLogger(BeanObjective.class);

  //////////////////////////////////////////////////////////////////////
  // member variables
  //////////////////////////////////////////////////////////////////////

  private final Collection<Coord>
      coords; // list of points which the calculated bean curve region should cover
   ...
   * @return smallest bean-axis parameter a so that the bean covers at least a predefined percentage
   *     of points
   */
  ...
  private final boolean isPointWithin(Coord coord, ParamPoint p, double a) {
    ...
    //		System.out.println(a + " " + b + " " + x0 + " " + y0 + " " + x + " " + y + " " +
    // p.getValue(RATIO_idx) + " " + x1_ + " " + y1_ + " " + x1 + " " + y1 + " " + x_ + " " + y_);

    //		double dist =  b*( Math.sqrt( (x_/a) * ( (1-(x_/a)) + Math.sqrt(1+(2-3*(x_/a))*(x_/a)) ) )
    // )/(Math.sqrt(2.00));
    //      double dist = Math.pow(x1,4)+ Math.pow(y1,4)+ Math.pow(x1,2)* Math.pow(y1,2)- x *
    // (Math.pow(x1,2)+ Math.pow(y1,2));
    double dist =
        Math.pow(y_, 4)
            - Math.pow(y_, 2) * b * b * (x_ / a) * (1 - (x_ / a))
            + Math.pow((x_ / a), 4) * Math.pow(b, 4)
            - Math.pow(b, 4) * Math.pow((x_ / a), 3);
  ...
  public final ParamPoint getInitialParamPoint(final int index) {
    if (index > DIMENSION) {
      log.warn(
          "Initial paramPoint "
              + index
              + " was requested, but we only have 4 Dimensions. Returning Initial paramPoint 0.");
  ...
  public final boolean isValidParamPoint(ParamPoint p) {
    ...
    // so the bean can be transformed over to itself by switching the ratio and rotating it by 90
    // degrees = pi/2
