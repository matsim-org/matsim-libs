 ...
 * PersonEntersVehicleEventHandler</code> for a given set of agent ids.
 *
 * @author aneumann
 */
public class AgentId2DepartureDelayAtStopMap
  ...
  private TreeMap<Id<Person>, AgentId2DepartureDelayAtStopMapData> stopId2DelayAtStopMap =
      new TreeMap<>();
    ...
    if (this.agentIds.contains(event.getPersonId())) {
      ...
      if (event.getLegMode() == TransportMode.pt) {
        ...
        if (this.stopId2DelayAtStopMap.get(event.getPersonId()) == null) {
          ...
          this.stopId2DelayAtStopMap.put(
              event.getPersonId(), new AgentId2DepartureDelayAtStopMapData(event.getPersonId()));
        }
