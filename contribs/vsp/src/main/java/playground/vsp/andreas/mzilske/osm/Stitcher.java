...
public class Stitcher {
        ...
        if (!networkForThisRoute.getNodes().containsKey(nodeId)) {
          Node nearestNode =
              NetworkUtils.getNearestNode(
                  (networkForThisRoute), network.getNodes().get(nodeId).getCoord());
  ...
  public void addBoth(Way way) {
    System.out.println(
        "WayBoth: "
            + way.getWayNodes().get(0)
            + " "
            + way.getWayNodes().get(way.getWayNodes().size() - 1));
    addForwardLinks(way);
  ...
  private List<Id<Link>> route(
      List<Id<Node>> stopNodes, List<Id<Link>> outStopLinks, List<Double> outTravelTimes) {
    ...
    //		LeastCostPathCalculator router = new
    // DijkstraFactory().createPathCalculator(networkForThisRoute, cost, cost);
    LeastCostPathCalculator router =
        new DijkstraFactory().createPathCalculator(network, cost, cost);
    ...
    while (i.hasNext()) {
      ...
      // This may have been a bug.  The old router would use the network that was connected to the
      // origin node, _not_ the one that was given by the constructor.
      // This evidently started being a problem with mode-specific sub-networks.  Christoph found
      // this at some point and put in a test.  Now (many years later) the
      // code failed that test.  I first tried to repair the nodes, but it turns out that I should
      // not pass the networkForThisRoute but instead the full network to
  ...
  private void addLink(Link link) {
    if (!networkForThisRoute.getLinks().containsKey(link.getId())) {
      networkForThisRoute.addLink(
          networkForThisRoute
              .getFactory()
              .createLink(link.getId(), link.getFromNode(), link.getToNode()));
    }
  }

  private void addNode(Node fromNode) {
    if (!networkForThisRoute.getNodes().containsKey(fromNode.getId())) {
      networkForThisRoute.addNode(
          networkForThisRoute.getFactory().createNode(fromNode.getId(), fromNode.getCoord()));
    }
  }
