...
import playground.vsp.andreas.fcd.Fcd;
import playground.vsp.andreas.mzilske.osm.OsmPrepare;
import playground.vsp.andreas.osmBB.osm2counts.Osm2Counts;
...
public class PTCountsOsm2Matsim {
  ...
  public static void main(String[] args) {
    ...
    //		String[] streetFilter = new
    // String[]{"motorway","motorway_link","trunk","trunk_link","primary","primary_link","secondary","tertiary","minor","unclassified","residential","living_street"};
    String[] streetFilter =
        new String[] {
          "motorway",
          "motorway_link",
    ...
    //		String[] transitFilter = new String[]{"ferry", "subway", "light_rail", "tram", "train",
    // "bus", "trolleybus"};
    String[] transitFilter = new String[] {"fsfsfgsg"};

    OsmPrepare osmPrepare =
        new OsmPrepare(osmRepository + osmFile, filteredOsmFile, streetFilter, transitFilter);
    ...
    OsmTransitMain osmTransitMain =
        new OsmTransitMain(
            filteredOsmFile,
            TransformationFactory.WGS84,
            TransformationFactory.DHDN_GK4,
            outDir + outName + "_network.xml",
            outDir + outName + "_schedule.xml",
            outDir + outName + "_vehicles.xml");
    osmTransitMain.convertOsm2Matsim(transitFilter);
    osmTransitMain = null;

    //		ResizeLinksByCount4 r = new ResizeLinksByCount4(outDir + outName + "_network.xml", counts,
    // shortNameMap, 1.0);
    ...
    Set<String> linksBlocked =
        Fcd.readFcdReturningLinkIdsUsed(
            fcdNetInFile,
            fcdEventsInFile,
            outDir,
            outDir + outName + "_network_resized.xml",
            500.0);

    PTCountsNetworkSimplifier ptCountNetSimplifier =
        new PTCountsNetworkSimplifier(
            outDir + outName + "_network_resized.xml",
            outDir + outName + "_schedule.xml",
            outDir + outName + "_network_merged.xml",
            outDir + outName + "_schedule_merged.xml",
            shortNameMap,
            countsFile,
            countsOutFile,
            outDir + "transitVehicles.xml.gz",
            linksBlocked);
