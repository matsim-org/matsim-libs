 ...
 *
 * <ul>
 *   <li>cartesian: |x/a|^r + |y/b|^r = 1
 *   <li>parametrically: X = a*[(cos(t))^(2/r)], Y = b*[(sin(t))^(2/r)]
 *   <li>area: A = {4^(1-1/r)*a*b*(??)*?(1+1/r)}/{(1/2+1/r)} In space, a superellipse is described
 *       by the following 6 parameters:
 *       <ul>
 ...
 *       </ul>
 *       In our case, we use a 5-dimensional parameter space with the following 5 axes and their
 *       ranges:
 *       <ul>
 *         <li>x : ]-INF .. +INF[
 *         <li>y : ]-INF .. +INF[
 ...
 *         <li>ratio : ]0 .. +INF[
 *       </ul>
 *       The effective size of the superellipse is given implicitely by a minimum cover-percentage.
 *       The superellipse will be enlarged until it contains at least the specified percentage of
 *       all points.
 */
public class SuperEllipseObjective implements Objective {
  ...
  private static final Logger log = LogManager.getLogger(SuperEllipseObjective.class);
  ...
  private final Collection<Coord>
      coords; // list of points which the calculated superelliptic region should cover
  ...
  public double getResponse(ParamPoint p) {
    ...
    return a
        * a
        * p.getValue(RATIO_idx)
        * Math.sqrt(Math.PI)
        * (Math.pow(4.0, (1.0 - (1 / (p.getValue(R_idx)))))
            * dgamma(1.0 + (1 / (p.getValue(R_idx))))
            / dgamma((1.0 / 2.0) + (1 / (p.getValue(R_idx)))));
  }
   ...
   * @return smallest superellipse-axis parameter a so that the ellipse covers at least a predefined
  ...
  //	This routine calculates the value of Gamma function which is a prerequesite for area of
  // superellipse.
  double dgamma(double x) {
    ...
    w = x - (n + 2);
    y =
        (((((((((((((-1.99542863674e-7 * w + 1.337767384067e-6) * w - 2.591225267689e-6) * w
                                                                                            - 1.7545539395205e-5)
                                                                                        * w
                                                                                    + 1.45596568617526e-4)
                                                                                * w
