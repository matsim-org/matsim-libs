...
/**
 * Implementation of the Simplex Optimization Algorithm. Works in N-dimensional parameter spaces
 * with a convex objective function.
 *
 * <p>http://mathworld.wolfram.com/SimplexMethod.html
 */
public final class SimplexOptimization {
  ...
  public static ParamPoint getBestParams(final Objective objective) {
    ...
    while (improvement > EPSILON || cnt < 100) {
      ...
      if (objective.isValidParamPoint(r)) {
        ...
        if (rR < rW && rR > rB) { // rR is better than rW, but worse than rB
          ...
          // if the direction we take seems okay, we go a step further in the same direction and
          // check again
          // this can be seen as an optimization to the algorithm to find the past param-values
          // faster
          ...
          ParamPoint c =
              ParamPoint.add(worst, ParamPoint.multiply(diff, 0.5)); // contracted point C
          points.remove(worst);
          points.add(c);
        }
      } else {
    ...
    // store the result
    // `best' contains now the ParamPoint best matching the objective function / activity space
  ...
  /**
   * A Comparator based on response-values of an objective function. Note that lower responses are
   * considered better, so after sorting an array the best parameter point is at index 0.
   */
