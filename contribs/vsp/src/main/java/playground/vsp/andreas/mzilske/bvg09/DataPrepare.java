...
public class DataPrepare {
  ...
  private static String InNetworkFile =
      "/Volumes/Data/VSP/svn/shared-svn/studies/schweiz-ivtch/baseCase/network/ivtch-osm.xml";
  private static String InInputPlansFileWithXY2Links =
      "/Volumes/Data/VSP/coding/eclipse35/thesis-data/application/plans.census2000ivtch1pct.dilZh30km.sample.xml.gz";

  // INTERMEDIARY FILES
  private static String IntermediateTransitNetworkFile =
      "../berlin-bvg09/pt/alles_mit_umlaeufen/test/transit_network.xml";
  private static String IntermediateTransitScheduleWithoutNetworkFile =
      "/Users/michaelzilske/Desktop/transit_schedule_without_network.xml";

  // OUTPUT FILES
  private static String OutTransitScheduleWithNetworkFile =
      "../berlin-bvg09/pt/alles_mit_umlaeufen/test/transitSchedule.networkOevModellBln.xml";
  private static String OutVehicleFile =
      "../berlin-bvg09/pt/alles_mit_umlaeufen/test/vehicles.oevModellBln.xml";
  private static String OutMultimodalNetworkFile =
      "../berlin-bvg09/pt/alles_mit_umlaeufen/test/network.multimodal.mini.xml";
  private static String OutRoutedPlanFile =
      "../berlin-bvg09/pt/alles_mit_umlaeufen/test/plan.routedOevModell.BVB344.moreLegPlan_Agent.xml";
  ...
  protected void convertSchedule() {
    ...
    Visum2TransitSchedule converter =
        new Visum2TransitSchedule(
            vNetwork, this.scenario.getTransitSchedule(), this.scenario.getTransitVehicles());
  ...
  private void writeScheduleAndVehicles() {
    ...
    new TransitScheduleWriterV1(this.scenario.getTransitSchedule())
        .write(IntermediateTransitScheduleWithoutNetworkFile);
  ...
  protected void mergeNetworks() {
    ...
    MergeNetworks.merge(
        streetNetwork, "", transitNetwork, "", (Network) this.scenario.getNetwork());
  ...
  protected void routePopulation() {
    ...
    FreespeedTravelTimeAndDisutility timeCostCalculator =
        new FreespeedTravelTimeAndDisutility(this.scenario.getConfig().scoring());
    TransitConfigGroup transitConfig = new TransitConfigGroup();

    TransitRouterConfig transitRouterConfig =
        new TransitRouterConfig(
            this.scenario.getConfig().scoring(),
            this.scenario.getConfig().routing(),
            this.scenario.getConfig().transitRouter(),
    ...
    final PersonAlgorithm router = new PlanRouter(factory.get(), TimeInterpretation.create(config));
    // PlansCalcTransitRoute router = new
    // PlansCalcTransitRoute(this.scenario.getConfig().plansCalcRoute(),
    //		this.scenario.getNetwork(), timeCostCalculator, timeCostCalculator, dijkstraFactory, new
    // ModeRouteFactory(),
    //		transitConfig, new TransitRouterImpl(transitRouterConfig, this.scenario.getTransitSchedule()
  ...
  protected void visualizeRouterNetwork() {
    ...
    /* I commented this code out, as it directly refers to the old and inefficient TransitRouterImpl
      which got removed at the Code Sprint 2023 in Berlin. // mrieser, 2023oct09
    */

    //		TransitRouterConfig transitRouterConfig = new
    // TransitRouterConfig(this.scenario.getConfig().planCalcScore()
    //				, this.scenario.getConfig().plansCalcRoute(), this.scenario.getConfig().transitRouter(),
    //				this.scenario.getConfig().vspExperimental());
    //		TransitRouterImpl router = new TransitRouterImpl(transitRouterConfig,
    // this.scenario.getTransitSchedule() );
    //		Network routerNet = router.getTransitRouterNetwork();
    //
    //		log.info("create vis network");
    //		MutableScenario visScenario = (MutableScenario)
    ...
    // visNet.getNodes().get(link.getFromNode().getId()),
    // visNet.getNodes().get(link.getToNode().getId()));
    ...
    //		QSim otfVisQSim = new QSimBuilder(visScenario.getConfig()).useDefaults().build(visScenario,
    // events);
    //		OnTheFlyServer server = OTFVis.startServerAndRegisterWithQSim(visScenario.getConfig(),
    // visScenario, events, otfVisQSim);
    //		OTFClientLive.run(visScenario.getConfig(), server);
    //		otfVisQSim.run();
  }

  private void buildUmlaeufe() {
    Collection<TransitLine> transitLines =
        this.scenario.getTransitSchedule().getTransitLines().values();
    GreedyUmlaufBuilderImpl greedyUmlaufBuilder =
        new GreedyUmlaufBuilderImpl(
            new UmlaufInterpolator(this.scenario.getNetwork(), this.scenario.getConfig().scoring()),
            transitLines);
    ...
    this.scenario.getTransitVehicles().addVehicleType(vehicleType);

    long vehId = 0;
    for (Umlauf umlauf : umlaeufe) {
      Vehicle veh =
          vb.createVehicle(Id.create("veh_" + Long.toString(vehId++), Vehicle.class), vehicleType);
  ...
  public static void run(
      ...
      String inInputPlansFile,
      String interTransitNetworkFile,
      String interTransitScheduleWithoutNetworkFile,
      String outTransitScheduleWithNetworkFile,
      String outVehicleFile,
      String outMultimodalNetworkFile,
    ...
    DataPrepare.IntermediateTransitScheduleWithoutNetworkFile =
        interTransitScheduleWithoutNetworkFile;
