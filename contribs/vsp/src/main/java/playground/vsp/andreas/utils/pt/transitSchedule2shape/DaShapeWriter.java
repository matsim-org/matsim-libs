...
public class DaShapeWriter {
  ...
  public static void writeLinks2Shape(
      String fileName,
      Map<Id, ? extends Link> links,
      Map<Id, SortedMap<String, Object>> attributes,
  ...
  public static void writeNodes2Shape(
      String fileName, Map<Id, ? extends Node> nodes, String targetCoordinateSystem) {
    initPointFeatureType("nodes", null, targetCoordinateSystem);
    write(createNodeFeatures(nodes), fileName);
  }

  public static void writeDefaultPoints2Shape(
      String fileName,
      String name,
      Map<String, Coord> points,
   ...
   * @param lines2write
   */
  public static void writeTransitLines2Shape(
      String shapeFileOutName,
      TransitSchedule transitSchedule,
      Collection<Id> lines2write,
      Map<Id, SortedMap<String, Object>> attributes,
      String targetCoordinateSystem) {
   ...
   * @param stops2write
   */
  public static void writeRouteStops2Shape(
      String fileName,
      Map<Id, TransitStopFacility> stops,
      Collection<Id> stops2write,
      String targetCoordinateSystem) {
    initPointFeatureType("TransitRouteStops", null, targetCoordinateSystem);
    write(createStopFeatures(stops, stops2write), fileName);
  }

  public static void writePointDist2Shape(
      String fileName,
      Map<String, Tuple<Coord, Coord>> points,
      Map<String, SortedMap<String, Object>> attributes,
      String targetCoordinateSystem) {
    for (SortedMap<String, Object> m : attributes.values()) {
   ...
   * @param attributes
   */
  public static void writeDefaultLineString2Shape(
      String fileName,
      String name,
  ...
  public static void writeDefaultLineStrings2Shape(
      String fileName,
      String name,
      Map<String, List<Coord>> lineStrings,
      String targetCoordinateSystem) {
  ...
  private static void initLineFeatureType(
      String name, SortedMap<String, Object> attributes, String targetCoordinateSystem) {
  ...
  private static void initPointFeatureType(
      String name, SortedMap<String, Object> attributes, String targetCoordinateSystem) {
    SimpleFeatureTypeBuilder b = new SimpleFeatureTypeBuilder();
    b.setName(name);
    b.setCRS(MGC.getCRS(targetCoordinateSystem));
  ...
  private static Collection<SimpleFeature> createRouteFeatures(
          ...
          if (attributes == null) {
            feature =
                getLineStringFeature(
                    new CoordinateArraySequence(coord),
                    line.getId().toString() + "_" + route.getId().toString(),
                    null);
          } else {
            feature =
                getLineStringFeature(
  ...
  private static Collection<SimpleFeature> createLinkFeatures(
      Map<Id, ? extends Link> links, Map<Id, SortedMap<String, Object>> attributes) {
    Collection<SimpleFeature> features = new ArrayList<SimpleFeature>();
    SimpleFeature feature;
    Coordinate[] coord;
    ...
    for (Link l : links.values()) {
      ...
      if (attributes == null) {
        ...
        feature =
            getLineStringFeature(new CoordinateArraySequence(coord), l.getId().toString(), attr);
      } else {
        feature =
            getLineStringFeature(
                new CoordinateArraySequence(coord),
                l.getId().toString(),
                attributes.get(l.getId()));
      }
      features.add(feature);
    }
    return features;
  ...
  private static Collection<SimpleFeature> createPointDistanceFeatures(
      Map<String, Tuple<Coord, Coord>> points, Map<String, SortedMap<String, Object>> attributes) {
    Collection<SimpleFeature> features = new ArrayList<SimpleFeature>();
    SimpleFeature feature;
    Coordinate[] coord;
    ...
    for (Entry<String, Tuple<Coord, Coord>> e : points.entrySet()) {
      ...
      feature =
          getLineStringFeature(
              new CoordinateArraySequence(coord), e.getKey(), attributes.get(e.getKey()));
      features.add(feature);
    }
  ...
  private static Collection<SimpleFeature> createStopFeatures(
      Map<Id, TransitStopFacility> stops, Collection<Id> stops2write) {
    Collection<SimpleFeature> features = new ArrayList<SimpleFeature>();
    SimpleFeature feature;
  ...
  private static Collection<SimpleFeature> createDefaultPointFeature(
      Map<String, Coord> points, Map<String, SortedMap<String, Object>> attributes) {
    Collection<SimpleFeature> features = new ArrayList<SimpleFeature>();
    SimpleFeature feature;
  ...
  private static Collection<SimpleFeature> createDefaultLineStringFeature(
      Map<String, SortedMap<Integer, Coord>> lineStrings,
      Map<String, SortedMap<String, Object>> attributes) {
    Collection<SimpleFeature> features = new ArrayList<SimpleFeature>();
    SimpleFeature feature;
    ...
    for (Entry<String, SortedMap<Integer, Coord>> points : lineStrings.entrySet()) {
      if (points.getValue().size() < 2) {
        log.error(
            points.getKey() + ": not enough points for a lineString. Need at least 2 points!");
      } else {
        int i = 0;
  ...
  private static SimpleFeature getLineStringFeature(
      CoordinateArraySequence c, String name, SortedMap<String, Object> attributes) {
  ...
  private static SimpleFeature getPointFeature(
      Coord coord, String id, SortedMap<String, Object> attributes) {
