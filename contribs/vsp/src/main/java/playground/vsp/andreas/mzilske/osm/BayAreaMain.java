...
public class BayAreaMain {
  ...
  public static void main(String[] args) {
    for (int hierarchyLevel = 1; hierarchyLevel <= 6; hierarchyLevel++) {
      Scenario scenario =
          createScenario(
              "/Users/zilske/Documents/osm-bayarea/bayarea-" + hierarchyLevel + ".osm.xml");
      new NetworkCleaner().run(scenario.getNetwork());
      new NetworkWriter(scenario.getNetwork())
          .write("/Users/zilske/Documents/osm-bayarea/network-" + hierarchyLevel + ".xml");
    }
  }
  ...
  private static Scenario createScenario(String networkFilename) {
    ...
    FastXmlReader reader =
        new FastXmlReader(new File(networkFilename), true, CompressionMethod.None);

    SimplifyTask simplify = new SimplifyTask(IdTrackerType.BitSet);

    CoordinateTransformation coordinateTransformation =
        TransformationFactory.getCoordinateTransformation("WGS84", "EPSG:32710");
