...
public class Plans2Gexf extends MatsimJaxbXmlWriter {
  ...
  private HashMap<GridNode, HashMap<GridNode, GridEdge>> fromNode2toNode2EdgeMap =
      new HashMap<GridNode, HashMap<GridNode, GridEdge>>();
        ...
        if (pE instanceof Activity) {
          ...
          if (lastNode == null) {
            ...
            if (this.fromNode2toNode2EdgeMap.get(lastNode).get(currentNode) == null) {
              this.fromNode2toNode2EdgeMap
                  .get(lastNode)
                  .put(currentNode, new GridEdge(lastNode, currentNode));
            }
    ...
    for (HashMap<GridNode, GridEdge> toNode2EdgeMap : this.fromNode2toNode2EdgeMap.values()) {
      for (GridEdge edge : toNode2EdgeMap.values()) {
        ...
        if (edge.getFromNode()
            .getId()
            .toString()
            .equalsIgnoreCase(edge.getToNode().getId().toString())) {
          log.debug(
              "Omitting link "
                  + edge.getId().toString()
