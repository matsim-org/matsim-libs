...
public class JOSMTolerantFastXMLParser {
   ...
   *
   * @param sink The sink receiving all output data.
   * @param reader The input xml reader.
   * @param enableDateParsing If true, parsing of dates in the xml will be enabled, otherwise the
  ...
  private Tag readTag() throws Exception {
    Tag tag =
        new Tag(
            reader.getAttributeValue(null, ATTRIBUTE_NAME_KEY),
  ...
  public void readOsm() {
    ...
    try {
      ...
      if (reader.nextTag() == XMLStreamConstants.START_ELEMENT
          && reader.getLocalName().equals("osm")) {
