...
public class PTCountsNetworkSimplifier {
  ...
  public PTCountsNetworkSimplifier(
      String netInFile,
      String scheduleInFile,
      String netOutFile,
      String scheduleOutFile,
      HashMap<String, String> shortNameMap,
      String inCounts,
      String countsOutFile,
      String vehiclesInFile,
      Set<String> linksBlocked) {
  ...
  public static void main(String[] args) {
    PTCountsNetworkSimplifier simplifier =
        new PTCountsNetworkSimplifier(
            "f:/simplifyTest/net.xml",
            "f:/simplifyTest/transitSchedule.xml",
            "f:/simplifyTest/_out/net_simplified_merged.xml",
            "f:/simplifyTest/_out/transitSchedule_subway_merged.xml",
            null,
            null,
            null,
            "f:/simplifyTest/vehicles.xml",
            new TreeSet<String>());
  ...
  public void simplifyPTNetwork() {
    ...
    if (this.usePT) {
      ...
      TransitScheduleCleaner.removeAllRoutesWithMissingLinksFromSchedule(
          osmScenario.getTransitSchedule(), this.network);
      ...
      this.network =
          TransitScheduleCleaner.tagTransitLinksInNetwork(
              osmScenario.getTransitSchedule(), this.network);
    }
          ...
          for (Link oL : oLinks) {
            ...
            if (inLink != null && outLink != null) {
              ...
              if (!outLink.getToNode().equals(inLink.getFromNode())) {
                ...
                if (!this.linksBlockedByFacility.contains(inLink.getId().toString())
                    && !this.linksBlockedByFacility.contains(outLink.getId().toString())) {
                  ...
                  if (link != null) {
                    if (inLink.getAllowedModes().contains(TransportMode.pt)
                        && outLink.getAllowedModes().contains(TransportMode.pt)) {
                    ...
                    } else if (!inLink.getAllowedModes().contains(TransportMode.pt)
                        && !outLink.getAllowedModes().contains(TransportMode.pt)) {
                  ...
                  //									log.info("inLink " + inLink.getId().toString() + " and outLink " +
                  // outLink.getId().toString() + " will not be touched");
                }
              }
            }
          }
  ...
  private void registerLinksBlockedByTransitStopFacility() {
    ...
    log.info(
        "Marked " + this.linksBlockedByFacility.size() + " links as blocked by transit stops.");
  }

  /**
   * Check whether the node is marked as count station. If that is the case, migrate its count data
   * to outCounts and mark it as non modifiable
   *
  ...
  private void checkNodeIsMarkedAsCountStation(Node node) {
    ...
    if (this.shortNameMap.keySet().contains(node.getId().toString())) {
      ...
      if (node.getInLinks().size() == 1 && node.getOutLinks().size() == 1) {
        // ok, node has one in and one outLink, so put the count station on the shorter one and
        // block it
        ...
        Id<Link> shortNameId =
            Id.create(this.shortNameMap.get(node.getId().toString()), Link.class);
        ...
        log.warn(
            "Count station "
                + this.shortNameMap.get(node.getId().toString())
                + " is registerd to node "
                + node.getId().toString()
                + " which has "
                + node.getInLinks().size()
                + " inLinks and "
                + node.getOutLinks().size()
                + " outLinks. Can only map one to one. Removing count station from counts data.");
      }
    }
  ...
  private boolean removeLinksFromTransitSchedule(Link link, Link inLink, Link outLink) {
    // Link can only be merged, if all routes contain a) both links in direct following order and b)
    // no route contain only one of those links
    for (TransitLine transitLine : this.transitSchedule.getTransitLines().values()) {
      for (TransitRoute transitRoute : transitLine.getRoutes().values()) {
        ...
        if (transitRoute.getRoute().getLinkIds().contains(inLink.getId())
          ...
          // b) Check if only one Link is part of the route, so the route would be turned invalid,
          // if merged
          if (transitRoute.getRoute().getLinkIds().contains(inLink.getId())
              || transitRoute.getRoute().getLinkIds().contains(outLink.getId())) {
    ...
    for (TransitLine transitLine : this.transitSchedule.getTransitLines().values()) {
      for (TransitRoute transitRoute : transitLine.getRoutes().values()) {
        ...
        // Only merge if both links are part of the route. Cases were one link is part of it, should
        // already be filtered
        if (transitRoute.getRoute().getLinkIds().contains(inLink.getId())
            && transitRoute.getRoute().getLinkIds().contains(outLink.getId())) {
  ...
  /**
   * @param mergeLinkStats If set true, links will be merged despite their different attributes. If
   *     set false, only links with the same attributes will be merged, thus preserving as much
   *     information as possible. Default is set false.
   */
