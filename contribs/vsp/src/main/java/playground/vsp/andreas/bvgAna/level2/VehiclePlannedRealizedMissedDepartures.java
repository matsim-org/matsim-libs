...
/**
 * Searches the next planned and next realized departures for a given stop and time. Calculates the
...
public class VehiclePlannedRealizedMissedDepartures {
  ...
  /**
   * Tries to approximate the number of vehicles an agent has missed due to its delay. The results
   * may get confused in case vehicles of one specific route<br>
   * a) suffer from severe delay and <br>
   * b) are allowed to overtake each other.<br>
   * e.g. an agent has a positive delay (is 10 minutes late) but due to the vehicles running late as
   * well, the agent can enter a vehicle running ahead of the scheduled one.
   *
   ...
   * @param lineId
   * @param routeId
   * @return The number of missed vehicles, if positive. Negative number, if the agent could get
   *     some vehicles earlier. Zero, if the agent took the vehicle as scheduled.
   */
  public int getNumberOfMissedVehicles(
      Id stopId, double plannedDepartureTime, double realizedDepartureTime, Id lineId, Id routeId) {
    Tuple<OptionalTime, Integer> plannedDeparture =
        this.getNextDepartureTime(stopId, plannedDepartureTime, lineId, routeId, this.planned);
    Tuple<OptionalTime, Integer> realizedDeparture =
        this.getNextDepartureTime(stopId, realizedDepartureTime, lineId, routeId, this.realized);
    ...
    if (log.getLevel() == Level.DEBUG) {
      ...
      if (numberOfMissedVehicles < 0 && delay > 0) {
        this.log.debug(
            "Positive delay despite the fact that the agent could get a vehicle earlier. This could be right in case vehicles suffer from severe delay, please check.");
      }
      if (numberOfMissedVehicles > 0 && delay < 0) {
        this.log.debug(
            "Negative delay despite the fact that the agent missed its vehicle. This could be right in case vehicles are allowed to run ahead of schedule, please check.");
      }
    }
  ...
  public double getNextPlannedDepartureTime(Id stopId, double time, Id lineId, Id routeId) {
    return this.getNextDepartureTime(stopId, time, lineId, routeId, this.planned)
        .getFirst()
        .seconds();
  }
  ...
  public double getNextRealizedDepartureTime(Id stopId, double time, Id lineId, Id routeId) {
    return this.getNextDepartureTime(stopId, time, lineId, routeId, this.realized)
        .getFirst()
        .seconds();
  }

  private Tuple<OptionalTime, Integer> getNextDepartureTime(
      Id stopId, double time, Id lineId, Id routeId, String type) {
    this.log.debug(
        "TransitAgent.getEnterTransitRoute only checks for line id and stops to come ignoring route id. Agent probably too fast in simulation compared to plan.");
    TreeMap<Id, StopId2RouteId2DelayAtStopMapData> possibleRoutes =
        this.stopId2Route2DelayAtStopMap.get(stopId);
    ...
    Tuple<OptionalTime, Integer> closestDepartureTimeAndSlot =
        new Tuple<>(OptionalTime.defined(Double.POSITIVE_INFINITY), Integer.MIN_VALUE);

    for (StopId2RouteId2DelayAtStopMapData delayContainer : possibleRoutes.values()) {
      if (delayContainer.getLineId().toString().equalsIgnoreCase(lineId.toString())
          && delayContainer.getRouteId().toString().equalsIgnoreCase(routeId.toString())) {
        ...
        if (type.equalsIgnoreCase(this.planned)) {
          ...
          this.log.error(
              "Unknown type " + type + ". Don't know what to do. Returning positive Inf");
          return null;
        }
        for (int i = 0; i < departures.size(); i++) {
          if (departures.get(i) > time) {
            if (departures.get(i) < closestDepartureTimeAndSlot.getFirst().seconds()) {
              closestDepartureTimeAndSlot =
                  new Tuple<OptionalTime, Integer>(OptionalTime.defined(departures.get(i)), i);
            }
          }
        }
      }
