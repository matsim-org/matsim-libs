...
public final class GreedyUmlaufBuilderImpl implements UmlaufBuilder {
  ...
  private static final Comparator<UmlaufKey> umlaufKeyComparator =
      new Comparator<UmlaufKey>() {
  ...
  private final SortedMap<UmlaufKey, Umlauf> umlaeufe =
      new TreeMap<UmlaufKey, Umlauf>(umlaufKeyComparator);
  private ArrayList<UmlaufStueck> umlaufStuecke;

  private final UmlaufInterpolator interpolator;

  public GreedyUmlaufBuilderImpl(
      UmlaufInterpolator interpolator, Collection<TransitLine> transitLines) {
  ...
  private UmlaufKey getKey(Umlauf umlauf) {
    return new UmlaufKey(
        umlauf.getLineId(),
        Id.create(getLastStopPostAreaId(umlauf), TransitStopFacility.class),
        getLastArrivalTime(umlauf),
        umlauf.getId());
  }

  private Umlauf findFittingUmlauf(UmlaufStueck umlaufStueck) {
    String firstStopPostAreaId =
        umlaufStueck.getRoute().getStops().get(0).getStopFacility().getStopAreaId().toString();
    Id<TransitLine> lineId = umlaufStueck.getLine().getId();
    UmlaufKey earliestAtPoint =
        new UmlaufKey(
            lineId,
            Id.create(firstStopPostAreaId, TransitStopFacility.class),
            0.0,
            Id.create(0, Umlauf.class));
    UmlaufKey latestAtPoint =
        new UmlaufKey(
            lineId,
            Id.create(firstStopPostAreaId, TransitStopFacility.class),
            umlaufStueck.getDeparture().getDepartureTime(),
            Id.create(0, Umlauf.class));
