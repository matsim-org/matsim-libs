 ...
 *
 * <ul>
 *   <li>x, y: the Coordinates of the center of the ellipse
 ...
 * </ul>
 *
 * The effective size of the ellipse is given implicitely by a minimum cover-percentage. The ellipse
 * will be enlarged until it contains at least the specified percentage of all points.
 */
public class EllipseObjective implements Objective {
  ...
  private final Collection<Coord>
      coords; // list of points which the calculated elliptic region should cover
  ...
  private static final Logger log = LogManager.getLogger(EllipseObjective.class);
  ...
  public double getResponse(ParamPoint p) {
    ...
    return Math.PI
        * a
        * a
        * p.getValue(RATIO_idx); // area = pi*a*b = pi*a*b*(a/a) = pi*a*a*(b/a) = pi*a^2*ratio
  }
   ...
   * @return smallest ellipse-axis parameter a so that the ellipse covers at least a predefined
  ...
  public final boolean isValidParamPoint(ParamPoint p) {
    ...
    // so the ellipse can be transformed over to itself by switching the ratio and rotating it by 90
    // degrees = pi/2
