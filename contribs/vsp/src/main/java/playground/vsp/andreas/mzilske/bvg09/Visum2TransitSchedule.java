...
public class Visum2TransitSchedule {
  ...
  public Visum2TransitSchedule(
      final VisumNetwork visum, final TransitSchedule schedule, final Vehicles vehicles) {
    this.visum = visum;
      ...
      if (tLine.getRoutes().size() > 0) {
        ...
        if (keepPlan) {
          ...
          log.warn(
              "The line "
                  + tLine.getId()
                  + " was not added to the transit schedule because it does not serve stops within m44/344 area.");
        }
      } else {
        log.warn(
            "The line "
                + tLine.getId()
                + " was not added to the transit schedule because it does not contain any routes.");
      }
    }
  ...
  private VehicleType determineVehicleType(
    ...
    VehicleType vehType;
    VisumNetwork.VehicleCombination vehCombination =
        this.visum.vehicleCombinations.get(timeProfile.vehCombNr);
    if (vehCombination == null) {
      vehCombination = this.visum.vehicleCombinations.get(line.vehCombNo);
    }
    if (vehCombination == null) {
      log.warn(
          "Could not find vehicle combination with id="
              + timeProfile.vehCombNr
              + " used in line "
              + line.id.toString()
              + ". Using default type "
              + line.tCode
      ...
      if (vehType == null) {
        ...
        log.warn(
            "Could not find default vehicle for line type "
                + line.tCode
                + " used in line "
                + line.id.toString()
                + ". Using the first vehicle type available!");
      }

    } else {
      vehType =
          this.vehicles.getVehicleTypes().get(Id.create(vehCombination.id, VehicleType.class));
    }
    return vehType;
  }

  private void convertDepartures(
      VisumNetwork.TransitLine line,
      VisumNetwork.TimeProfile timeProfile,
      VehicleType vehType,
      TransitRoute tRoute) {
    for (VisumNetwork.Departure d : this.visum.departures.values()) {
      if (d.lineName.equals(line.id.toString())
          && d.lineRouteName.equals(timeProfile.lineRouteName.toString())
          && d.TRI.equals(timeProfile.index.toString())) {
        Departure departure =
            builder.createDeparture(Id.create(d.index, Departure.class), Time.parseTime(d.dep));
        Vehicle veh =
            this.vehicles
                .getFactory()
                .createVehicle(Id.create("tr_" + nextVehicleId++, Vehicle.class), vehType);
  ...
  private TransitRoute convertRouteProfile(
      VisumNetwork.TransitLine line, VisumNetwork.TimeProfile timeProfile) {
    List<TransitRouteStop> stops = new ArrayList<TransitRouteStop>();
    for (VisumNetwork.TimeProfileItem tpi : this.visum.timeProfileItems.values()) {
      if (tpi.lineName.equals(line.id.toString())
          && tpi.lineRouteName.equals(timeProfile.lineRouteName.toString())
          && tpi.timeProfileName.equals(timeProfile.index.toString())
          && tpi.dirCode.equals(timeProfile.dirCode.toString())) {
        String lineRouteItemKey =
            line.id.toString()
                + "/"
                + timeProfile.lineRouteName.toString()
                + "/"
                + tpi.lRIIndex.toString()
                + "/"
                + tpi.dirCode;
        LineRouteItem lineRouteItem = this.visum.lineRouteItems.get(lineRouteItemKey);
        Id<TransitStopFacility> stopFacilityId =
            Id.create(lineRouteItem.stopPointNo, TransitStopFacility.class);
        if (stopFacilityId != null) {
          ...
          TransitRouteStop s =
              builder.createTransitRouteStop(
                  stopFacility, Time.parseTime(tpi.arr), Time.parseTime(tpi.dep));
    ...
    TransitRouteImpl tRoute =
        (TransitRouteImpl)
            builder.createTransitRoute(
                Id.create(
                    timeProfile.lineName.toString()
                        + "."
                        + timeProfile.lineRouteName.toString()
                        + "."
                        + timeProfile.index.toString()
                        + "."
                        + timeProfile.dirCode.toString(),
                    TransitRoute.class),
                null,
                stops,
                mode);
  ...
  private Map<Id<TransitStopFacility>, TransitStopFacility> convertStopPoints(
    ...
    for (VisumNetwork.StopPoint stopPoint : this.visum.stopPoints.values()) {
      Coord coord =
          this.coordinateTransformation.transform(
              this.visum.stops.get(this.visum.stopAreas.get(stopPoint.stopAreaId).StopId).coord);
      TransitStopFacility stop =
          builder.createTransitStopFacility(
              Id.create(stopPoint.id, TransitStopFacility.class), coord, false);
  ...
  private void convertVehicleTypes() {
    ...
    for (VehicleCombination vehComb : this.visum.vehicleCombinations.values()) {
      ...
      capacity.setStandingRoom(
          Integer.valueOf(vehComb.numOfVehicles * (vu.passengerCapacity - vu.seats)));
