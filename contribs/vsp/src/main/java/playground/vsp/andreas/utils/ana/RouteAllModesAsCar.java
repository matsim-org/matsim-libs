...
/**
 * Routes all modes of transport (except for transit_walk) as car modes and counts the number of
 * trips per link. Output as Shape
 *
 * @author aneumann
 */
public class RouteAllModesAsCar extends AbstractPersonFilter {

  private static final Logger log = LogManager.getLogger(RouteAllModesAsCar.class);

  private final Scenario sc;
  private final LeastCostPathCalculator routingAlgo;

  private HashMap<Link, Integer> link2totals = new HashMap<Link, Integer>();
  private HashMap<String, HashMap<Link, Integer>> mode2link2totals =
      new HashMap<String, HashMap<Link, Integer>>();
  ...
  private void writeAsShape(String outputDir, String targetCoordinateSystem) {
    ...
    for (Link link : this.sc.getNetwork().getLinks().values()) {
      ...
      featureAttribs =
          createLinkFeatureAttribs(link, new Object[3 + this.mode2link2totals.keySet().size()]);
  ...
  private Object[] createLinkFeatureAttribs(Link link, Object[] objects) {
    ...
    coord[0] =
        new Coordinate(
            link.getFromNode().getCoord().getX(), link.getFromNode().getCoord().getY(), 0.);
    coord[1] =
        new Coordinate(link.getToNode().getCoord().getX(), link.getToNode().getCoord().getY(), 0.);
        ...
        if (lastAct == null) {
          ...
          Path path =
              this.routingAlgo.calcLeastCostPath(
                  lastLink.getToNode(), currentLink.getFromNode(), 0.0, null, null);
