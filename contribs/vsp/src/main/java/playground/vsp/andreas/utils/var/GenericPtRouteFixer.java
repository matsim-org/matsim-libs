...
/**
 * Old routed plans may contain pt routes of type generic. This fix substitutes them by the proper
 * {@link TransitPassengerRoute} route type. Note that the current route type cannot be read from
 * {@link TransitPassengerRoute}. Hence there are no automatic updates.
 *
 * @author aneumann
 */
public class GenericPtRouteFixer {

  private static final Logger log = LogManager.getLogger(GenericPtRouteFixer.class);
    ...
    if (inFile.equalsIgnoreCase(outFile)) {
      ...
      log.info(
          "Substituting all occurences of \""
              + GenericPtRouteFixer.oldRouteType
              + "\" by \""
              + GenericPtRouteFixer.newRouteType
              + "\" in lines containing the pattern \""
              + GenericPtRouteFixer.searchPattern
              + "\".");
