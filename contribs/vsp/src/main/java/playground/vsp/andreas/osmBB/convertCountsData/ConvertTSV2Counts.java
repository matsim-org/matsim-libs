...
public class ConvertTSV2Counts {
  ...
  public static void main(String[] args) {
    ...
    List<CountStationDataBox> countStations =
        ReadCountStations.readCountStations(countStationsFileName);

    log.info("Building count station map by reading " + countStations.size() + " stations");
    HashMap<String, CountStationDataBox> countStationsMap =
        new HashMap<String, CountStationDataBox>();
    ...
    for (CountStationDataBox countStation : countStationsMap.values()) {
      counts.createAndAddCount(
          Id.create(countStation.getShortName(), Link.class), countStation.getShortName());
      counts
          .getCount(Id.create(countStation.getShortName(), Link.class))
          .setCoord(countStation.getCoord());
      String filename = workingDir + "Wochenübersicht_" + countStation.getShortName() + ".tsv";
      ReadCountDataForWeek.readCountDataForWeek(
          filename, counts.getCount(Id.create(countStation.getShortName(), Link.class)), startTag);
    }
