...
public class TransitNetworkSink implements Sink {
  ...
  static final Logger log = LogManager.getLogger(TransitNetworkSink.class);
  ...
  public TransitNetworkSink(
    ...
    stopNodeStore =
        new IndexedObjectStore<NodeContainer>(
            new SingleClassObjectSerializationFactory(NodeContainer.class), "stops");
    routeSegmentStore =
        new IndexedObjectStore<WayContainer>(
            new SingleClassObjectSerializationFactory(WayContainer.class), "routesegments");
    allWays =
    ...
    entityContainer.process(
        new EntityProcessor() {
          ...
          public void process(NodeContainer container) {
            ...
            // debug
            count++;
            if (count % 50000 == 0) log.info(count + " nodes processed so far");
          }
          ...
          public void process(RelationContainer relationContainer) {
            ...
            if ("route".equals(tags.get("type"))) {
              if (tags.get("route") == null) {
                ...
                if (getTransitModes()
                    .contains(tags.get("route"))) { // && "tram".equals(tags.get("route"))) {
  ...
  public void complete() {
    ...
    while (transitLineIterator.hasNext()) {
      ...
      System.out.println(
          networkOperator + " // " + route + " // " + ref + " // " + operator + " // " + name);
      TransitLine line =
          transitSchedule
              .getFactory()
              .createTransitLine(
                  Id.create(
                      operator + "-" + route + "-" + ref + "-" + name + "-" + relation.getId(),
                      TransitLine.class));
          ...
          if (allNodesTracker.get(relationMember.getMemberId())) {
            ...
            Coord coordinate =
                coordinateTransformation.transform(
                    new Coord(node.getLongitude(), node.getLatitude()));
            String role = relationMember.getMemberRole();
      ...
      if (linkIdsH.size() >= 2) {
        ...
        List<TransitRouteStop> stops =
            enterStopLinkIds(
                ...
                forwardTravelTimes,
                line.getId() + "H",
                network.getLinks().get(linkIdsH.get(0)).getFromNode());
        TransitRoute routeH =
            transitSchedule
                .getFactory()
                .createTransitRoute(
                    Id.create("H", TransitRoute.class), networkRouteH, stops, TransportMode.pt);
      ...
      if (linkIdsR.size() >= 2) {
        ...
        List<TransitRouteStop> stops =
            enterStopLinkIds(
                ...
                backwardTravelTimes,
                line.getId() + "R",
                network.getLinks().get(linkIdsR.get(0)).getFromNode());
        TransitRoute routeR =
            transitSchedule
                .getFactory()
                .createTransitRoute(
                    Id.create("R", TransitRoute.class), networkRouteR, stops, TransportMode.pt);
  ...
  private List<TransitRouteStop> enterStopLinkIds(
      List<Node> stopNodes,
      List<Id<Link>> stopLinkIdsH,
    ...
    Coord firstCoordinate =
        coordinateTransformation.transform(
            new Coord(firstStopNode.getLongitude(), firstStopNode.getLatitude()));
    ...
    Link entryLink =
        network
            .getFactory()
            .createLink(Id.create(routeRef + firstStopName + "_ENTRY", Link.class), node, node);
    network.addLink(entryLink);
    TransitStopFacility firstFacility =
        transitSchedule
            .getFactory()
            .createTransitStopFacility(
                createTransitStopId(routeRef, firstStopName, stopNo), firstCoordinate, false);
    transitSchedule.addStopFacility(firstFacility);
    stopNo++;
    double time = 0;
    transitRouteStops.add(
        transitSchedule.getFactory().createTransitRouteStop(firstFacility, time, time));
    ...
    while (i.hasNext()) {
      ...
      Coord coordinate =
          coordinateTransformation.transform(
              new Coord(stopNode.getLongitude(), stopNode.getLatitude()));
      ...
      TransitStopFacility facility =
          transitSchedule
              .getFactory()
              .createTransitStopFacility(
                  createTransitStopId(routeRef, stopName, stopNo), coordinate, false);
      stopNo++;
      transitSchedule.addStopFacility(facility);
      time += k.next();
      transitRouteStops.add(
          transitSchedule.getFactory().createTransitRouteStop(facility, time, time));
  ...
  private NetworkRoute createNetworkRoute(List<Id<Link>> plinkIds) {
    ...
    NetworkRoute networkRouteH =
        RouteUtils.createLinkNetworkRouteImpl(linkIds.getFirst(), linkIds.getLast());
