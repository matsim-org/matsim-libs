...
/**
 * Converts a given transit schedule to shape. Extracts mode of transport. Allows to restrict the
 * partransit lines to be converted by the iteration they are found.
 *
 * @author aneumann
 */
public class TransitSchedule2Shape {
  ...
  public static void main(String[] args) {
    ...
    final String ALLLINESSHAPEOUTFILE =
        "f:/p_runs/txl/"
            + runId
            + "/it."
            + iteration
            ...
            + iteration
            + ".transitSchedule.shp";
    final String PARAINBUSINESSSHAPEOUTFILE =
        "f:/p_runs/txl/"
            + runId
            + "/it."
            + iteration
            ...
            + "."
            + iteration
            + ".transitSchedule_para_in_business.shp";
    final int removeAllParatransitLinesYoungerThanIteration = 2729;
    final String pIdentifier = "para_";

    //		final String SCHEDULEFILE =
    // "e:/_shared-svn/andreas/paratransit/input/trb_2012/transitSchedules/onlySpandauBusLines.xml.gz";
    //		final String SCHEDULEFILE =
    // "e:/_shared-svn/andreas/paratransit/input/trb_2012/transitSchedules/transitSchedule_basecase.xml.gz";
    //		final String SHAPEOUTFILE =
    // "e:/_shared-svn/andreas/paratransit/input/trb_2012/transitSchedules/onlySpandauBusLines.shp";

    final String NETWORKFILE =
        "e:/_shared-svn/andreas/paratransit/input/trb_2012/network.final.xml.gz";
    ...
    Map<Id, SortedMap<String, Object>> lineAttributesMap =
        TransitSchedule2Shape.getAttributesForLines(transitSchedule, pIdentifier);
    Collection<Id> linesToConvert =
        TransitSchedule2Shape.getIdsFromAllLinesButParatransitYoungerThanIterationGiven(
            transitSchedule, pIdentifier, removeAllParatransitLinesYoungerThanIteration);
    DaShapeWriter.writeTransitLines2Shape(
        ALLLINESSHAPEOUTFILE,
        transitSchedule,
        linesToConvert,
        lineAttributesMap,
    ...
    linesToConvert =
        TransitSchedule2Shape.getIdsFromCoopLoggerInBusinessOnly(transitSchedule, COOPLOGGERFILE);
    DaShapeWriter.writeTransitLines2Shape(
        PARAINBUSINESSSHAPEOUTFILE,
        transitSchedule,
        linesToConvert,
        lineAttributesMap,
        targetCoordinateSystem);
  ...
  public static Map<Id, SortedMap<String, Object>> getAttributesForLines(
      TransitSchedule transitSchedule, String pIdentifier) {
    Map<Id, SortedMap<String, Object>> lineAttributesMap =
        new HashMap<Id, SortedMap<String, Object>>();
  ...
  private static Collection<Id> getIdsFromAllLinesButParatransitYoungerThanIterationGiven(
      TransitSchedule transitSchedule,
      String pIdentifier,
      int removeAllParatransitLinesYoungerThanIteration) {
    ...
    for (TransitLine transitLine : transitSchedule.getTransitLines().values()) {
      if (transitLine.getId().toString().contains(pIdentifier)) {
        try {
          int iterationTheParatransitLineWasFound =
              Integer.valueOf(transitLine.getId().toString().split("_")[1]);
          if (iterationTheParatransitLineWasFound
              <= removeAllParatransitLinesYoungerThanIteration) {
  ...
  private static Collection<Id> getIdsFromCoopLoggerInBusinessOnly(
      TransitSchedule transitSchedule, String coopLoggerFile) {
    Set<String> coopsToKeep =
        ReadCoopLoggerFileAndReturnCoopsInBusiness.readCoopLoggerFileAndReturnCoopsInBusiness(
            coopLoggerFile);
    Collection<Id> linesToConvert = new TreeSet<Id>();
