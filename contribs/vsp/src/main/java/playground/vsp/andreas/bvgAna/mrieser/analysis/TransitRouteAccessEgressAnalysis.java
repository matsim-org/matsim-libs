...
/**
 * Keeps track of the number of passengers entering and leaving each single departure along a given
 * route.
 *
 * @author mrieser
 */
public class TransitRouteAccessEgressAnalysis
  ...
  private final Map<Departure, Map<Id<TransitStopFacility>, Integer>> accessCounters =
      new LinkedHashMap<>();
  private final Map<Departure, Map<Id<TransitStopFacility>, Integer>> egressCounters =
      new LinkedHashMap<>();
  private final VehicleTracker vehTracker;

  public TransitRouteAccessEgressAnalysis(
      final TransitRoute transitRoute, final VehicleTracker vehicleTracker) {
  ...
  public void printStats() {
    List<Id<TransitStopFacility>> stopFacilityIds =
        new ArrayList<>(this.transitRoute.getStops().size());
