 ...
 *
 * <ul>
 *   <li>cartesian: [(x-a)^2 + y^2][(x+a)^2 + y^2] = b^4
 *   <li>polar equation: (r^4)+(a^4)-2(a^2)*(r^2)*cos(2@) = b^4
 *   <li>area: A = 1/2*(r)^2*d@ = a^2+b^2*E[(a^2)/(b^2)] (where E(x)is the complete elliptic
 ...
 *
 * <ul>
 *   <li>x, y: the Coordinates of the center of the Cassini
 ...
 * </ul>
 *
 * The effective size of the Cassini is given implicitely by a minimum cover-percentage. The Cassini
...
public class CassiniObjective implements Objective {
  ...
  private static final Logger log = LogManager.getLogger(CassiniObjective.class);
  ...
  public static double rSquare(double aa, double bb, double ang) {
    return aa
        * aa
        * (Math.cos(2 * ang) + Math.sqrt(Math.pow((bb / aa), 4) - Math.pow(Math.sin(2 * ang), 2)));
  }
   ...
   * @return smallest cassini oval-axis parameter a so that the cassini oval covers at least a
   *     predefined percentage of points
   */
  ...
  public final boolean isValidParamPoint(ParamPoint p) {
    ...
    // so the Cassini can be transformed over to itself by switching the ratio and rotating it by 90
    // degrees = pi/2
