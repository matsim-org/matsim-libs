...
/**
 * Collects all <code>PersonEntersVehicleEvent</code> and <code>PersonLeavesVehicleEvent</code> with
 * their corresponding <code>ActivityEndEvent</code> and <code>ActivityStartEvent</code> for a given
 * set of agent ids ignoring <code>pt interaction</code> events.
 *
 * @author aneumann
 */
public class PersonEnterLeaveVehicle2ActivityHandler
  ...
  private TreeMap<Id, List<PersonLeavesVehicleEvent>> agentId2LeaveVehEvent =
      new TreeMap<Id, List<PersonLeavesVehicleEvent>>();

  private HashMap<PersonEntersVehicleEvent, ActivityEndEvent>
      personsEntersVehicleEvent2ActivityEndEvent =
          new HashMap<PersonEntersVehicleEvent, ActivityEndEvent>();
  private HashMap<PersonLeavesVehicleEvent, ActivityStartEvent>
      personLeavesVehicleEvent2ActivityStartEvent =
          new HashMap<PersonLeavesVehicleEvent, ActivityStartEvent>();
  ...
  /**
   * @return Returns a map containing the preceding <code>ActivityEndEvent</code> for a given <code>
   *     PersonEntersVehicleEvent</code>
   */
  public Map<PersonEntersVehicleEvent, ActivityEndEvent>
      getPersonEntersVehicleEvent2ActivityEndEvent() {
    return this.personsEntersVehicleEvent2ActivityEndEvent;
  }

  /**
   * @return Returns a map containing the following <code>ActivityStartEvent</code> for a given
   *     <code>PersonLeavesVehicleEvent</code>
   */
  public Map<PersonLeavesVehicleEvent, ActivityStartEvent>
      getPersonLeavesVehicleEvent2ActivityStartEvent() {
  ...
  public void handleEvent(ActivityStartEvent event) {
    if (!event.getActType().equalsIgnoreCase(PtConstants.TRANSIT_ACTIVITY_TYPE)) {
      ...
      if (this.agentIds.contains(personId)) {
        ...
        List<PersonLeavesVehicleEvent> events =
            this.agentId2LeaveVehEvent.remove(event.getPersonId());
        if (events != null) {
