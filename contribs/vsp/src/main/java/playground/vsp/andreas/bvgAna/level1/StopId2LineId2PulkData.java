...
/**
 * Collects the <code>VehicleDepartsAtFacilityEvent</code> of one vehicle and the event of its
 * following vehicle to calculate their planned and realized headway.
 *
 * @author aneumann
 */
public class StopId2LineId2PulkData {
  ...
  protected StopId2LineId2PulkData(
      Id lineId, VehicleDepartsAtFacilityEvent oldEvent, VehicleDepartsAtFacilityEvent newEvent) {
    this.lineId = lineId;
    ...
    this.headwaySOLL =
        (newEvent.getTime() - newEvent.getDelay()) - (oldEvent.getTime() - oldEvent.getDelay());
  }
