...
public class SetPersonCoordsToBoundingBox extends NewPopulation {
  ...
  public SetPersonCoordsToBoundingBox(
      Network network, Population plans, String filename, Coord minXY, Coord maxXY) {
    ...
    if (person.getPlans().size() != 1) {
      ...
      for (PlanElement planElement : plan.getPlanElements()) {
        if (planElement instanceof Activity) {
          ...
          if (act.getCoord().getX() >= this.minXY.getX()
              && act.getCoord().getX() <= this.maxXY.getX()) {
            x = act.getCoord().getX();
          }
          if (act.getCoord().getY() >= this.minXY.getY()
              && act.getCoord().getY() <= this.maxXY.getY()) {
            y = act.getCoord().getY();
          }
  ...
  public static void main(final String[] args) {
    ...
    SetPersonCoordsToBoundingBox dp =
        new SetPersonCoordsToBoundingBox(net, inPop, outPlansFile, minXY, maxXY);
    dp.run(inPop);
    System.out.println(
