...
import playground.vsp.andreas.bvgAna.level2.StopId2RemainSeatedDataMapData;
import playground.vsp.andreas.bvgAna.level3.AgentId2StopDifferenceMap;

public class RunAnalyses {

  private static final Logger log = LogManager.getLogger(RunAnalyses.class);

  private static final String networkFilename =
      "/Volumes/Data/projects/bvg2010/runs/2010-11-21-run01/output_network.xml.gz";
  private static final String plansFilename =
      "/Volumes/Data/projects/bvg2010/runs/2010-11-21-run01/output_plans.xml.gz";
  private static final String eventsFilename =
      "/Volumes/Data/projects/bvg2010/runs/2010-11-21-run01/250.events.xml.gz";
  private static final String transitScheduleFilename =
      "/Volumes/Data/projects/bvg2010/runs/2010-11-21-run01/transitSchedule.oevnet.xml.gz";
  private static final String transitVehiclesFilename =
      "/Volumes/Data/projects/bvg2010/runs/2010-11-21-run01/transitVehicles_bvg_2005.xml.gz";
  ...
  public void createPersonAttributeTable(
      final String attributesFilename, final String idsFilename) {
  ...
  public void createRemainSeatedStats() {
    ...
    Map<Id, List<StopId2RemainSeatedDataMapData>> remainSeatedMap =
        remainSeated.getStopId2RemainSeatedDataMap();
    ...
    Map<Id, List<StopId2RemainSeatedDataMapData>> aggregatedMap =
    ...
    for (Map.Entry<Id, List<StopId2RemainSeatedDataMapData>> e : aggregatedMap.entrySet()) {
      Collections.sort(
          e.getValue(),
          new Comparator<StopId2RemainSeatedDataMapData>() {
            @Override
            public int compare(
                StopId2RemainSeatedDataMapData o1, StopId2RemainSeatedDataMapData o2) {
              return Double.compare(
                  o1.getVehicleArrivesAtFacilityEvent().getTime(),
                  o2.getVehicleArrivesAtFacilityEvent().getTime());
            }
          });
    }
    ...
    for (Map.Entry<Id, List<StopId2RemainSeatedDataMapData>> e : aggregatedMap.entrySet()) {
      ...
      System.out.print(
          stop.getCoord().getX() + "\t" + stop.getCoord().getY() + "\t" + stop.getName());
    ...
    for (Map.Entry<Id, List<StopId2RemainSeatedDataMapData>> e : aggregatedMap.entrySet()) {
      ...
      for (int slot = 0; slot < entering.length; slot++) {
        ...
        if (arriving[slot] == 0) {
          ...
          System.out.print(
              "\t" + ((double) (arriving[slot] - leaving[slot])) / ((double) arriving[slot]));
        }
      }
      System.out.println();
    }
  ...
  public void createMissedConnectionStats(Set<Id<Person>> agentIds) {
    AgentId2StopDifferenceMap missedConnections =
        new AgentId2StopDifferenceMap(this.scenario.getPopulation(), agentIds);

    EventsManager em = EventsUtils.createEventsManager();
    em.addHandler(missedConnections);
    new MatsimEventsReader(em).readFile(eventsFilename);

    Map<Id, List<Tuple<Id, Integer>>> missedVehiclesMap =
        missedConnections.getNumberOfMissedVehiclesMap();
  ...
  public void createCatchmentAreaStats(
      Set<Id<Person>> agentIds, Id<TransitLine> lineId, Id<TransitRoute>[] routeIds) {
    TransitSchedule ts = ((MutableScenario) this.scenario).getTransitSchedule();
    ...
    StopId2PersonEnterLeaveVehicleHandler enterLeave =
        new StopId2PersonEnterLeaveVehicleHandler(stopIds);
    PersonEnterLeaveVehicle2ActivityHandler enterLeave2Act =
        new PersonEnterLeaveVehicle2ActivityHandler(agentIds);
    EventsManager em = EventsUtils.createEventsManager();
    ...
    //		for (List<PersonEntersVehicleEvent> events :
    // enterLeave.getStopId2PersonEnterEventMap().values()) {
    //			for (PersonEntersVehicleEvent event : events) {
    //				agentIds.add(event.getDriverId());
    //			}
    //		}

    Map<Id, List<PersonEntersVehicleEvent>> enterEventsPerPerson =
        new HashMap<Id, List<PersonEntersVehicleEvent>>();
    Map<Id, List<PersonLeavesVehicleEvent>> leaveEventsPerPerson =
        new HashMap<Id, List<PersonLeavesVehicleEvent>>();
    ...
    for (List<PersonLeavesVehicleEvent> events :
        enterLeave.getStopId2PersonLeaveEventMap().values()) {
    ...
    Map<PersonEntersVehicleEvent, ActivityEndEvent> activityEnds =
        enterLeave2Act.getPersonEntersVehicleEvent2ActivityEndEvent();
    Map<PersonLeavesVehicleEvent, ActivityStartEvent> activityStarts =
        enterLeave2Act.getPersonLeavesVehicleEvent2ActivityStartEvent();
    ...
    for (Id personId : enterEventsPerPerson.keySet()) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (startEvent == null) {
          log.warn(
              "No ActivityStartEvent found for PersonLeavesVehicleEvent: time="
                  + leaveEvent.getTime()
                  + " person="
                  + leaveEvent.getPersonId()
                  + " vehicle="
                  + leaveEvent.getVehicleId());
        } else {
    ...
    for (List<AgentTripData> list : agentData.values()) {
      for (AgentTripData data : list) {
          ...
          if (pe instanceof Activity) {
            ...
            if (prevActivity != null) {
              if (data.toActType == null) {
                ...
                if (prevActivity.getType().equals(data.fromActType)
                    && prevActivity.getLinkId().equals(data.fromLinkId)) {
                ...
                if (prevActivity.getType().equals(data.fromActType)
                    && a.getType().equals(data.toActType)
                    && prevActivity.getLinkId().equals(data.fromLinkId)
                    && a.getLinkId().equals(data.toLinkId)) {
                  data.fromActCoord = prevActivity.getCoord();
                  data.toActCoord = a.getCoord();
                  break;
                }
        ...
        if (data.fromActCoord == null) {
          log.error(
              "No from.coord found for person="
                  + data.agentId
                  + " from.act="
                  + data.fromActType
                  + " from.link="
        ...
        if (data.fromActCoord == null) {
          ...
          System.out.println(
              data.agentId
                  ...
                  + data.fromActType
                  + "\t"
                  + data.fromActCoord.getX()
                  + "\t"
                  + data.fromActCoord.getY()
                  + "\t"
                  + data.toActType
                  + "\t\t");
        } else {
    ...
    Map<Tuple<Integer, Integer>, Integer> fromBlockCnt =
        new HashMap<Tuple<Integer, Integer>, Integer>();
    Map<Tuple<Integer, Integer>, Integer> toBlockCnt =
        new HashMap<Tuple<Integer, Integer>, Integer>();
    for (List<AgentTripData> list : agentData.values()) {
      for (AgentTripData data : list) {
        if (data.fromActCoord != null) {
          Tuple<Integer, Integer> blockId =
              new Tuple<Integer, Integer>(
                  (int) data.fromActCoord.getX() / aggregateSize,
                  (int) data.fromActCoord.getY() / aggregateSize);
        ...
        if (data.toActCoord != null) {
          Tuple<Integer, Integer> blockId =
              new Tuple<Integer, Integer>(
                  (int) data.toActCoord.getX() / aggregateSize,
                  (int) data.toActCoord.getY() / aggregateSize);
    ...
    Set<Tuple<Integer, Integer>> keySet =
        new HashSet<Tuple<Integer, Integer>>(fromBlockCnt.keySet());
    ...
    for (Tuple<Integer, Integer> t : keySet) {
      System.out.print(
          t.getFirst() * aggregateSize
              + aggregateSize * 0.5
              + "\t"
              + (t.getSecond() * aggregateSize + aggregateSize * 0.5)
              + "\t");
  ...
  public static void main(String[] args) {
    ...
    app.createCatchmentAreaStats(
        allPersonIds,
        Id.create("BVB----145", TransitLine.class),
        new Id[] {
          Id.create("BVB----145.10.GEN12.R", TransitRoute.class),
          Id.create("BVB----145.10.GEN5.R", TransitRoute.class),
          Id.create("BVB----145.10.GEN8.R", TransitRoute.class)
        });
  }
}
