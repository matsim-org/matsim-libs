...
package playground.vsp.andreas.bvgAna.level4;
...
public class StopId2MissedVehMap
  ...
  private final Logger log = LogManager.getLogger(StopId2MissedVehMap.class);
  ...
  private void calculateStopId2AverageDelayMap() {
    Map<Id, List<Tuple<Id, Double>>> delayMap =
        this.agentId2StopDifferenceMap.getAgentId2StopDifferenceMap();
    ...
    for (List<Tuple<Id, Double>> delayList : delayMap.values()) {
      for (Tuple<Id, Double> delayTuple : delayList) {
        if (this.stopId2StopId2MissedVehMapDataMap.get(delayTuple.getFirst()) == null) {
          this.stopId2StopId2MissedVehMapDataMap.put(
              delayTuple.getFirst(), new StopId2MissedVehMapData(delayTuple.getFirst()));
        }

        this.stopId2StopId2MissedVehMapDataMap
            .get(delayTuple.getFirst())
            .addDelay(delayTuple.getSecond());
      }
    }
  }

  private void calculateStopId2MissedVehMap() {
    Map<Id, List<Tuple<Id, Integer>>> numberOfMissedVehiclesMap =
        this.agentId2StopDifferenceMap.getNumberOfMissedVehiclesMap();
    ...
    for (List<Tuple<Id, Integer>> stopId2MissedVehiclesList : numberOfMissedVehiclesMap.values()) {
      for (Tuple<Id, Integer> stopId2MissedVehicleTuple : stopId2MissedVehiclesList) {
        if (this.stopId2StopId2MissedVehMapDataMap.get(stopId2MissedVehicleTuple.getFirst())
            == null) {
          this.stopId2StopId2MissedVehMapDataMap.put(
              stopId2MissedVehicleTuple.getFirst(),
              new StopId2MissedVehMapData(stopId2MissedVehicleTuple.getFirst()));
        }

        this.stopId2StopId2MissedVehMapDataMap
            .get(stopId2MissedVehicleTuple.getFirst())
            .addMissedVehicle(stopId2MissedVehicleTuple.getSecond());
      }
    }
  }
  ...
  public void writeResultsToFile(String filename) {
    try {
      ...
      for (StopId2MissedVehMapData data : results.values()) {
        writer.write(data.getFullStatistics());
        writer.newLine();
      }
      writer.flush();
      writer.close();
