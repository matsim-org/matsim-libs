...
public class Fcd {
  ...
  private static final Logger log = LogManager.getLogger(Fcd.class);
  ...
  public static Set<String> readFcdReturningLinkIdsUsed(
      String fcdNetInFile,
      String fcdEventsInFile,
      String outDir,
      String matsimNetwork,
      double minDistanceBetweenTwoActs) {
      ...
      if (lastEvent == null) {
        ...
        currentPerson =
            PopulationUtils.getFactory()
                .createPerson(
                    Id.create(
                        numberOfPlans + "-" + currentEvent.getVehId().toString(), Person.class));
  ...
  private void writeComplexPlansFromEvents(String plansOutFile, Network net) {
    ...
    for (Iterator<FcdEvent> iterator = this.fcdEventsList.iterator(); iterator.hasNext(); ) {
      ...
      if (lastLink != null) {
        if (CoordUtils.calcEuclideanDistance(link.getCoord(), lastLink.getCoord())
            < this.minDistanceBetweenTwoActs) {
          lastEvent = currentEvent;
          continue;
        }
      }
      ...
      if (lastEvent == null) {
        ...
        currentPerson
            .getSelectedPlan()
            .addActivity(createActivityWithLinkFromFcdEvent(currentEvent, link));
  ...
  private void writeNetworkFromEvents(String netOutFile) {
      ...
      if (lastEvent.getVehId().toString().equalsIgnoreCase(currentEvent.getVehId().toString())) {
        ...
        if (net.getNodes().get(currentEvent.getLinkId()) == null) {
          NetworkUtils.createAndAddNode(
              net,
              (Id<Node>) currentEvent.getLinkId(),
              this.coordTransform.transform(
                  this.networkMap.get(currentEvent.getLinkId()).getCoord()));
        }
        if (net.getNodes().get(lastEvent.getLinkId()) == null) {
          NetworkUtils.createAndAddNode(
              net,
              (Id<Node>) lastEvent.getLinkId(),
              this.coordTransform.transform(this.networkMap.get(lastEvent.getLinkId()).getCoord()));
        }

        Id<Link> newLinkId =
            Id.create(
                lastEvent.getLinkId().toString() + "-" + currentEvent.getLinkId().toString(),
                Link.class);
        if (net.getLinks().get(newLinkId) == null) {
          ...
          NetworkUtils.createAndAddLink(
              net,
              id,
              net.getNodes().get(lastEvent.getLinkId()),
              net.getNodes().get(currentEvent.getLinkId()),
              999.9,
              9.9,
              9999.9,
              9.9);
    ...
    FeatureGeneratorBuilder builder =
        new FeatureGeneratorBuilder() {
          @Override
          public FeatureGenerator createFeatureGenerator() {
            FeatureGenerator fg =
                new LineStringBasedFeatureGenerator(wc, MGC.getCRS(TransformationFactory.WGS84));
            return fg;
          }
        };
  ...
  private Activity createActivityFromFcdEvent(FcdEvent fcdEvent) {
    Activity act =
        PopulationUtils.createActivityFromCoord(
            "fcd",
            this.coordTransform.transform(this.networkMap.get(fcdEvent.getLinkId()).getCoord()));
  ...
  public static Link getNearestLinkWithRightDirection(Network net, Coord coord, double direction) {
    ...
    for (Link tempLink : link.getToNode().getOutLinks().values()) {
      if (tempLink
          ...
          .getId()
          .toString()
          .equalsIgnoreCase(link.getFromNode().getId().toString())) {
