...
public class PTNetworkSimplifier {
  ...
  public PTNetworkSimplifier(
      String netInFile, String scheduleInFile, String netOutFile, String scheduleOutFile) {
  ...
  public void run(final Network net, final TransitSchedule tranSched) {
    ...
    this.network =
        TransitScheduleCleaner.tagTransitLinksInNetwork(this.transitSchedule, this.network);
            ...
            if (inLink != null && outLink != null) {
              ...
              if (!outLink.getToNode().equals(inLink.getFromNode())) {
                ...
                if (!linkNeededByTransitStop(inLink, outLink)) {
                  ...
                  if (link != null) {
                    if (!nodesConnectedToTransitStop.contains(node.getId().toString())) {
                      if (!nodesConnectedToTransitStop.contains(
    ...
    TransitScheduleCleaner.removeAllRoutesWithMissingLinksFromSchedule(
        this.transitSchedule, this.network);
    TransitScheduleCleaner.removeEmptyLines(this.transitSchedule);
    TransitScheduleCleaner.removeStopsNotUsed(this.transitSchedule);
  }

  private boolean removeLinksFromTransitSchedule(Link link, Link inLink, Link outLink) {
    // first test - links must not be changed if, only one link is part of a route, but the other
    // one not
    for (TransitLine transitLine : this.transitSchedule.getTransitLines().values()) {
      for (TransitRoute transitRoute : transitLine.getRoutes().values()) {
        ...
        if (transitRoute.getRoute().getLinkIds().contains(inLink.getId())
            || transitRoute.getRoute().getLinkIds().contains(inLink.getId())) {
  ...
  private boolean linkNeededByTransitStop(Link inLink, Link outLink) {
    ...
    if (this.linksNeededByTransitSchedule == null) {
      ...
      for (TransitStopFacility transitStopFacility :
          this.transitSchedule.getFacilities().values()) {
  ...
  public static void main(String[] args) {
    PTNetworkSimplifier simplifier =
        new PTNetworkSimplifier(
            "e:/_out/osm/transit-network_bb_subway.xml",
            "e:/_out/osm/osm_transitSchedule_subway.xml",
            "e:/_out/osm/transit-network_bb_subway_simplified_merged.xml",
            "e:/_out/osm/osm_transitSchedule_subway_merged.xml");
