...
public class CountVehOnLinksPolygonBasedFeatureGenerator implements FeatureGenerator {
  ...
  public CountVehOnLinksPolygonBasedFeatureGenerator(
    ...
    this.compareResultMap =
        CountVehOnLinks.compareEventFiles(
            EventsCompareConfig.eventsFileOne, EventsCompareConfig.eventsFileTwo);
  }
  ...
  public SimpleFeature getFeature(final Link link) {
    ...
    return this.factory.createPolygon(
        new Coordinate[] {from, to, to2, from2, from}, attributes, link.getId().toString());
  }
}
