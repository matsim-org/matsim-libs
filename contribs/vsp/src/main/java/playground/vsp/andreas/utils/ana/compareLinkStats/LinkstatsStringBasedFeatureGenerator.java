...
public class LinkstatsStringBasedFeatureGenerator implements FeatureGenerator {
  ...
  public LinkstatsStringBasedFeatureGenerator(
      final WidthCalculator widthCalculator, final CoordinateReferenceSystem crs) {
    this.widthCalculator = widthCalculator;
    this.crs = crs;
    this.factory = initFeatureFactory();

    this.compareResultMap =
        EvaluateLinkstats.compareLinkstatFiles(
            LinkStatsCompareConfig.linkStatsFileOne, LinkStatsCompareConfig.linkStatsFileTwo);
  }
  ...
  public SimpleFeature getFeature(final Link link) {
    ...
    if (this.compareResultMap.get(link.getId().toString()) != null) {
      ...
      for (int i = 0; i < 8; i++) {
        attributes.put(
            "HRS" + (i + i * 2) + "-" + (i + i * 2 + 3) + "avg",
            Double.valueOf(
                (tempArray.get(i + i * 2).doubleValue()
                        + tempArray.get(i + 1 + i * 2).doubleValue()
                        + tempArray.get(i + 2 + i * 2).doubleValue())
                    / 3));
      }
    }
