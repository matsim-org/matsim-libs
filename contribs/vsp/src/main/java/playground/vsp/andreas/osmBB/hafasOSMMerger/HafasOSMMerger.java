...
public class HafasOSMMerger {
  ...
  private void mergeSchedules() {
    ...
    TransitScheduleImpl hafasSchedule =
        (TransitScheduleImpl) this.hafasScenario.getTransitSchedule();
    ...
    for (Entry<Id<TransitLine>, TransitLine> osm : osmSchedule.getTransitLines().entrySet()) {
      ...
      if (osm.getKey().toString().contains(this.lineToConvert)) {
        ...
        for (TransitRoute osmRoute : osm.getValue().getRoutes().values()) {
          ...
          if (hafasLine != null) {
            TransitRoute hafasRouteBestMatched =
                findRouteBestMatching(osmRoute, hafasLine.getRoutes());
          } else {
  ...
  private TransitRoute findRouteBestMatching(
      TransitRoute route, Map<Id<TransitRoute>, TransitRoute> candidateRoutes) {
    ...
    for (Entry<Id<TransitRoute>, TransitRoute> candidateRoute : candidateRoutes.entrySet()) {
      ...
      for (int i = 0;
          i < Math.min(route.getStops().size(), candidateRoute.getValue().getStops().size());
          i++) {
        stopDistanceSum +=
            calculateDistanceBetweenCoords(
                route.getStops().get(i).getStopFacility().getCoord(),
                candidateRoute.getValue().getStops().get(i).getStopFacility().getCoord());
      }

      // add something for every stop missing or too much
      //			stopDistanceSum += 1000 * (Math.max(route.getStops().size(),
      // candidateRoute.getValue().getStops().size()) - Math.min(route.getStops().size(),
      // candidateRoute.getValue().getStops().size()));

      if (bestRoute == null) {
        bestRoute = candidateRoute.getValue();
      } else if (bestTotalScore / bestRoute.getStops().size()
          > stopDistanceSum / candidateRoute.getValue().getStops().size()) {
