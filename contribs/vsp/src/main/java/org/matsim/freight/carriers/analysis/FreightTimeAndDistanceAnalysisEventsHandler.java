...
public class FreightTimeAndDistanceAnalysisEventsHandler implements BasicEventHandler {
  ...
  private static final Logger log =
      LogManager.getLogger(FreightTimeAndDistanceAnalysisEventsHandler.class);
  ...
  private void handleEvent(LinkEnterEvent event) {
    ...
    final Id<VehicleType> vehTypeId =
  ...
  void writeTravelTimeAndDistance(String analysisOutputDirectory, Scenario scenario)
      throws IOException {
    ...
    // Write headline:
    bw1.write(
        "vehicleId \t carrierId \t vehicleTypeId \t tourId \t tourDuration[s] \t travelDistance[m] \t "
  ...
  void writeTravelTimeAndDistancePerVehicleType(String analysisOutputDirectory, Scenario scenario)
    ...
    // ----- All VehicleTypes in CarriervehicleTypes container. Used so that even unused vehTypes
    // appear in the output

    TreeMap<Id<VehicleType>, VehicleType> vehicleTypesMap =
        new TreeMap<>(CarriersUtils.getCarrierVehicleTypes(scenario).getVehicleTypes());
    ...
    // Write headline:
    bw1.write(
        "vehicleTypeId \t nuOfVehicles \t SumOfTourDuration[s] \t SumOfTravelDistances[m] \t "
    ...
    for (VehicleType vehicleType : vehicleTypesMap.values()) {
      ...
      long nuOfVehicles =
          vehicleId2VehicleType.values().stream()
              .filter(vehType -> vehType.getId() == vehicleType.getId())
              .count();
      ...
      final Double sumOfDurationInSeconds =
          vehicleTypeId2SumOfDuration.getOrDefault(vehicleType.getId(), 0.);
      final Double sumOfDistanceInMeters =
          vehicleTypeId2Mileage.getOrDefault(vehicleType.getId(), 0.);
