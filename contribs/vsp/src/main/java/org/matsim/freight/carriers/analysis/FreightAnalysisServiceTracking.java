...
/**
 * @deprecated We have new event types now, allowing us to use a more straight forward analysis
 *     without guessing. I will let this here for some time so we can have a look, what else should
 *     be moved over, but in the end, We will remove this here. (kmt apr'23)
...
class FreightAnalysisServiceTracking {
  ...
  private final LinkedHashMap<Id<Carrier>, ServiceTracker.CarrierServiceTracker>
      carrierServiceTrackers = new LinkedHashMap<>();

  public LinkedHashMap<Id<Carrier>, ServiceTracker.CarrierServiceTracker>
      getCarrierServiceTrackers() {
    return carrierServiceTrackers;
  }
  ...
  // handle activityStartEvents to track the start of a service activity in case LSP Events are not
  // thrown
      ...
      for (ServiceTracker service : cst.serviceTrackers.values()) {
        if (service.service.getLocationLinkId().equals(activityStartEvent.getLinkId())) {
          if (service.driverId == null) {
            // if there is no driver, but there is a service which is to be performed at the moment
            // at this place, we guess this could be the event for it.
            // (Does not work well obviously as soon as there are multiple services at a location
            // that have generous time windows, like e.g. at stores).
            if (service.service.getServiceStartTimeWindow().getStart()
                    <= activityStartEvent.getTime()
                && activityStartEvent.getTime()
                    <= service.service.getServiceStartTimeWindow().getEnd()) {
              service.driverIdGuess = activityStartEvent.getPersonId();
              service.arrivalTimeGuess = activityStartEvent.getTime();
            }
            // if (unlikely) the driver is known for the service and this event is thrown at the
            // location of that service, we assume this event is meant for this particular service.
            // (doesn't work well either, because the driver is likely not known without LSP events,
            // and then we don't have to make guesses anyway.)
          } else if (service
              .driverId
              .toString()
              .equals(activityStartEvent.getPersonId().toString())) {
  ...
  public void setExpectedArrival(
      Id<Carrier> carrierId, Id<CarrierService> serviceId, double expectedArrival) {
    if (carrierServiceTrackers.containsKey(carrierId)) {
      if (carrierServiceTrackers.get(carrierId).serviceTrackers.containsKey(serviceId)) {
        carrierServiceTrackers.get(carrierId).serviceTrackers.get(serviceId).expectedArrival =
            expectedArrival;
      }
    }
  }

  public void setCalculatedArrival(
      Id<Carrier> carrierId, Id<CarrierService> serviceId, Double calculatedArrival) {
    if (carrierServiceTrackers.containsKey(carrierId)) {
      if (carrierServiceTrackers.get(carrierId).serviceTrackers.containsKey(serviceId)) {
        carrierServiceTrackers.get(carrierId).serviceTrackers.get(serviceId).calculatedArrival =
            calculatedArrival;
      }
    }
  }

  // UNTESTED handling of LSP Service events that provided reliable info about driver and
  // timestamps.
  public void handleStartEvent(CarrierServiceStartEvent event) {
    if (carrierServiceTrackers.containsKey(event.getCarrierId())) {
      if (carrierServiceTrackers
          .get(event.getCarrierId())
          .serviceTrackers
          .containsKey(event.getServiceId())) {
        ServiceTracker service =
            carrierServiceTrackers
                .get(event.getCarrierId())
                .serviceTrackers
                .get(event.getServiceId());
  ...
  // Method that estimates Arrival Times based on the expected leg departure and travel times to
  // measure punctuality.
        ...
        for (Tour.TourElement tourElement : tour.getTour().getTourElements()) {
          if (tourElement instanceof Tour.Leg) {
            calculatedArrivalTime =
                ((Tour.Leg) tourElement).getExpectedDepartureTime()
                    + ((Tour.Leg) tourElement).getExpectedTransportTime();
          }
