...
class FreightAnalysisShipmentTracking {
  ...
  private final LinkedHashMap<Id<CarrierShipment>, ShipmentTracker> shipments =
      new LinkedHashMap<>();
  ...
  // tracking Shipments based on Guesses the same way as Services are tracked, and also with the
  // same issues.
    ...
    for (ShipmentTracker shipment : shipments.values()) {
      if (shipment.to == activityStartEvent.getLinkId()) {
        if (shipment.driverId == null) {
          if (shipment.shipment.getDeliveryTimeWindow().getStart() <= activityStartEvent.getTime()
              && activityStartEvent.getTime()
                  <= shipment.shipment.getDeliveryTimeWindow().getEnd()) {
  ...
  // for improving the guess, we track the pickup activities aswell to narrow down the selection of
  // drivers on those that could have picked up the shipment when we later on try to match the
  // delivery activity.
    ...
    for (ShipmentTracker shipmentTracker : shipments.values()) {
      if (shipmentTracker.from == activityStartEvent.getLinkId()) {
        if (shipmentTracker.driverId == null) {
          if (shipmentTracker.shipment.getPickupTimeWindow().getStart()
                  <= activityStartEvent.getTime()
              && activityStartEvent.getTime()
                  <= shipmentTracker.shipment.getPickupTimeWindow().getEnd()) {
