...
class FreightAnalysisEventHandler
  ...
  private static final Logger log = LogManager.getLogger(FreightAnalysisEventHandler.class);
  ...
  private void init() {
    // the EventHandler tracks all vehicles containing "freight" by default which is as of now
    // (02/21) the easiest way to do so, but not a pretty one.
    ...
    for (Vehicle vehicle : vehicles.getVehicles().values()) {
      ...
      if (vehicle.getId().toString().contains("freight")) {
        ...
        // The vehicleId is based on a scheme like "freight_" + <carrierId> + "_veh" +
        // <vehicleNumber>
        // we try to extract the CarrierId based on this scheme and see if it is indeed a carrier.
        // TODO: Where do we check this? KMT Mai22
  ...
  // evaluating the ActivityEvents to track vehicle usage and guess what happens with services and
  // shipments
  @Override
  ...
  public void handleEvent(CarrierShipmentPickupEndEvent event) {
    ...
    // as we know the driver of the shipment now, we can assign the shipment's carrier to the
    // driver's vehicle.
    // FIXME: We do not have the driver in the events anymore. Need to collect them from other
    // places (if we still need them)
    log.error("Need a fix, since we are not having the drivers in here any more");
    //		if (shipmentTracking.getShipments().containsKey(event.getShipment().getId()) &&
    // vehicleTracking.getDriver2VehicleId(event.getDriverId())!=null){
    //
    //	vehicleTracking.addCarrier2Vehicle(vehicleTracking.getDriver2VehicleId(event.getDriverId()),
    // shipmentTracking.getShipments().get(event.getShipment()).carrierId);
    //		}
  }
  ...
  public void handleEvent(CarrierServiceStartEvent event) {
    ...
    // as we know the driver of a service now, we can assign the shipment's carrier to the driver's
    // vehicle.
    // FIXME: We do not have the driver in the events anymore. Need to collect them from other
    // places (if we still need them)
    log.error("Need a fix, since we are not having the drivers in here any more");
    //		if (serviceTracking.getCarrierServiceTrackers().containsKey(event.getServiceId()) &&
    // vehicleTracking.getDriver2VehicleId(event.getDriverId())!=null){
    //
    //	vehicleTracking.addCarrier2Vehicle(vehicleTracking.getDriver2VehicleId(event.getDriverId()),
    // serviceTracking.getCarrierServiceTrackers().get(event.getServiceId()).carrierId);
    //		}
  }
  ...
  public void exportVehicleInfo(String path, Boolean exportGuesses) {
    ...
    try {
      ...
      out.write(
          "vehicleId	vehicleType	carrierId	driverID	usageTime	roadTime	travelDistance	vehicleCost	tripCount");
      out.newLine();
      ...
      for (Id<Vehicle> vehId : trackers.keySet()) {
        ...
        String carrierIdString =
            (tracker.carrierId == null && exportGuesses)
                ? "##" + id2String(tracker.carrierIdGuess)
                : id2String(tracker.carrierId);
        ...
        String vehicleInfoString =
            vehId.toString()
                + "	"
                + tracker.vehicleType.getId().toString()
                + "	"
                + carrierIdString
                + "	"
                + lastDriverIdString
                + "	"
                + tracker.usageTime.toString()
                + "	"
                + tracker.roadTime.toString()
                + "	"
                + tracker.travelDistance.toString()
                + "	"
                + tracker.cost.toString()
                + "	"
                + tracker.tripHistory.size();
      ...
      for (String carrierIdString : carrierVehicleStatistics.keySet()) {
        BufferedWriter carrierOutFile =
            new BufferedWriter(
                new FileWriter(path + "/carrier_" + carrierIdString + "_vehicleStats.tsv"));
        carrierOutFile.write(
            "vehicleId	vehicleType	carrierId	driverID	usageTime	roadTime	travelDistance	vehicleCost	tripCount");
  ...
  public void exportVehicleTripInfo(String path, Boolean exportGuesses) {
    ...
    try {
      BufferedWriter out =
          new BufferedWriter(new FileWriter(path + "/freightVehicleTripStats.tsv"));
      out.write(
          "vehicleId	vehicleType	tripNumber	carrierId	driverId	tripRoadTime	tripDistance	tripVehicleCost");
      out.newLine();
      LinkedHashMap<Id<Vehicle>, VehicleTracker> trackers = vehicleTracking.getTrackers();
      ...
      for (String carrierIdString : carrierTripStatistics.keySet()) {
        BufferedWriter carrierOutFile =
            new BufferedWriter(
                new FileWriter(path + "/carrier_" + carrierIdString + "_tripStats.tsv"));
        carrierOutFile.write(
            "vehicleId	vehicleType	tripNumber	carrierId	driverId	tripRoadTime	tripDistance	tripVehicleCost");
        carrierOutFile.newLine();
  ...
  //		//there are no guesses to be exported as of now, still having this method to keep the export
  // calls consistent
  ...
  public void exportVehicleTypeStats(String path, Boolean exportGuesses) {
    ...
    try {
      ...
      singleFile.write(
          "carrierId	vehicleType	vehicleCount	totalDistance	totalServiceTime	totalRoadTime	totalCost");
      singleFile.newLine();
      for (Carrier carrier : carriers.getCarriers().values()) {
        ...
        BufferedWriter out =
            new BufferedWriter(
                new FileWriter(
                    path + "/carrier_" + carrier.getId().toString() + "_VehicleTypeStats.tsv"));
        for (VehicleTracker tracker : vehicleTracking.getTrackers().values()) {
          // if desired get carrierIdString, in which case the vehicleType gets the "##" prefix to
          // separate guessed vehicle connections from non-guessed ones, even if they are of the
          // same vehicle type
          String carrierIdString =
              tracker.carrierId == null && exportGuesses
                  ? id2String(tracker.carrierIdGuess)
        ...
        for (String vt : vehicleTypeStatsMap.keySet()) {
          ...
          out.write(
              carrier.getId().toString()
                  ...
                  + vt
                  + "	"
                  + vts.vehicleCount.toString()
                  + "	"
                  + vts.totalDistance.toString()
                  + "	"
                  + vts.totalServiceTime
                  + "	"
                  + vts.totalRoadTime
                  + "	"
                  + vts.totalCost);
  ...
  public void exportServiceInfo(String path, Boolean exportGuesses) {
    ...
    try {
      ...
      out.newLine();
      LinkedHashMap<Id<Carrier>, ServiceTracker.CarrierServiceTracker> carrierServiceTrackers =
          serviceTracking.getCarrierServiceTrackers();
      for (ServiceTracker.CarrierServiceTracker carrierServiceTracker :
          carrierServiceTrackers.values()) {
        String carrierIdString = id2String(carrierServiceTracker.carrierId);
        BufferedWriter out_carrier =
            new BufferedWriter(
                new FileWriter(path + "/carrier_" + carrierIdString + "_ServiceStats.tsv"));
        ...
        for (ServiceTracker serviceTracker : carrierServiceTracker.serviceTrackers.values()) {
          ...
          String driverIdString =
              (exportGuesses && serviceTracker.driverId == null)
                  ? "##" + id2String(serviceTracker.driverIdGuess)
                  : id2String(serviceTracker.driverId);
          String vehicleIdString =
              (vehicleTracking.getDriver2VehicleId(serviceTracker.driverId) == null
                      && exportGuesses)
                  ? "##"
                      + id2String(vehicleTracking.getDriver2VehicleId(serviceTracker.driverIdGuess))
                  : id2String(vehicleTracking.getDriver2VehicleId(serviceTracker.driverId));
          String arrivalTime =
              (exportGuesses && serviceTracker.startTime == 0.0)
                  ? "?" + serviceTracker.arrivalTimeGuess.toString()
                  : serviceTracker.startTime.toString();
          out.write(
              carrierIdString
                  + "	"
                  + serviceIdString
                  ...
                  + vehicleIdString
                  + "	"
                  + serviceTracker.expectedArrival
                  + "	"
                  + serviceTracker.calculatedArrival
                  + "	"
                  + arrivalTime);
          out_carrier.write(
              carrierIdString
  ...
  public void exportShipmentInfo(String path, Boolean exportGuesses) {
    ...
    try {
      ...
      singleFile.write(
          "carrierId	shipmentId	driverId	vehicleId	pickupTime	deliveryTime	deliveryDuration	beelineDistance");
      singleFile.newLine();
      for (Carrier carrier : carriers.getCarriers().values()) {
        BufferedWriter out =
            new BufferedWriter(
                new FileWriter(
                    path + "/carrier_" + carrier.getId().toString() + "_ShipmentStats.tsv"));
        out.write(
            "carrierId	shipmentId	driverId	vehicleId	pickupTime	deliveryTime	deliveryDuration	beelineDistance");
        out.newLine();
        for (CarrierShipment shipment : carrier.getShipments().values()) {
          ...
          String driverIdString =
              (shipmentTracker.driverId == null && exportGuesses)
                  ? "##" + id2String(shipmentTracker.driverIdGuess)
                  : id2String(shipmentTracker.driverId);
          String vehicleIdString =
              (vehicleTracking.getDriver2VehicleId(shipmentTracker.driverId) == null
                      && exportGuesses)
                  ? "##"
                      + id2String(
                          vehicleTracking.getDriver2VehicleId(shipmentTracker.driverIdGuess))
                  : id2String(vehicleTracking.getDriver2VehicleId(shipmentTracker.driverId));
          // calculate euclidean Distance between from and to for comparison
          double dist =
              NetworkUtils.getEuclideanDistance(
                  network.getLinks().get(from).getCoord(),
                  network.getLinks().get(toLink).getCoord());
          out.write(
              carrierIdString
                  + "	"
                  ...
                  + vehicleIdString
                  + "	"
                  + shipmentTracker.pickUpTime.toString()
                  + "	"
                  + shipmentTracker.deliveryTime.toString()
                  + "	"
                  + shipmentTracker.deliveryDuration.toString()
                  + "	"
                  + dist);
          out.newLine();
          singleFile.write(
  ...
  //		String path = parentDirectory + "/freight-analysis-it_" + iterationCount.toString(); // create
  // one subfolder per iteration
  //		try {
  ...
  private String id2String(
      Id id) { // Failsafe Id to String - Converter, because Id.toString() throws Exception if the
    // Id is null.
    return id == null
        ? " "
        : id.toString(); // return space because instead of empty string because TSV files get
    // confused otherwise
  }
