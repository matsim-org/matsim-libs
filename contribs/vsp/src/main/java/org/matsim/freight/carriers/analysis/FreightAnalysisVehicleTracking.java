...
class FreightAnalysisVehicleTracking {
  ...
  public void trackLinkLeaveEvent(LinkLeaveEvent linkLeaveEvent, double length) {
    if (trackers.containsKey(linkLeaveEvent.getVehicleId())) {
      if (vehiclesOnLink.containsKey(linkLeaveEvent.getVehicleId())) {
        Double onLinkTime =
            vehiclesOnLink.get(linkLeaveEvent.getVehicleId()) - linkLeaveEvent.getTime();
  ...
  public void addLeg(Id<Vehicle> vehId, Double travelTime, Double travelDistance, Boolean isEmpty) {
    ...
    tracker.cost = tracker.cost + calculateCost(tracker.vehicleType, travelDistance, travelTime);
  ...
  public void addDriver2Vehicle(Id<Person> personId, Id<Vehicle> vehicleId, double time) {
    if (trackers.containsKey(vehicleId)) {
      if (trackers.get(vehicleId).currentDriverId
          != personId) { // In Case the Person wasn't using the vehicle before, a new tour of the
        // vehicle is started.
        if (null != trackers.get(vehicleId).currentDriverId) {
          log.warn(
              "Warning that driver changed unexpectedly as this wrongs the service duration because the end of the previous service is obviously not known.");
        }
  ...
  // when a person that could've been a driver throws a "end" Event, it is assumed that they have
  // stopped using the vehicle. The service time is updated, the currentDriver reset.
  ...
  // when a person leaves a vehicle, a trip/leg (depending on view) has ended, so it is registered
  // with length, duration and cost, also counters are reset.
  public void registerVehicleLeave(PersonLeavesVehicleEvent event) {
    if (trackers.containsKey(event.getVehicleId())) {
      ...
      tracker.tripHistory.add(
          new VehicleTracker.VehicleTrip(
              tracker.currentDriverId,
              tracker.currentTripDistance,
              tracker.currentTripDuration,
              calculateCost(
                  tracker.vehicleType, tracker.currentTripDistance, tracker.currentTripDuration)));
