...
/**
 * A first approach for some analysis based on the freight events introduced in 2022/23. This class
 * comes from teaching SimGV in the winter term 2022/23.
 *
 * <p>This class should get extended and prepared as a standardized analysis for freight output.
 * This should also get aligned with the current development in Simwrapper. Todo: Add some tests.
 *
...
public class RunFreightAnalysisEventBased {
   ...
   * @param globalCrs
   */
  public RunFreightAnalysisEventBased(
      String simOutputPath, String analysisOutputPath, String globalCrs) {
  ...
  public void runAnalysis() throws IOException {
    ...
    freightCarriersConfigGroup.setCarriersVehicleTypesFile(
        SIM_OUTPUT_PATH + "output_allVehicles.xml.gz");
    ...
    // CarrierPlanAnalysis
    CarrierPlanAnalysis carrierPlanAnalysis =
        new CarrierPlanAnalysis(CarriersUtils.getCarriers(scenario));
    ...
    FreightTimeAndDistanceAnalysisEventsHandler freightTimeAndDistanceAnalysisEventsHandler =
        new FreightTimeAndDistanceAnalysisEventsHandler(scenario);
    eventsManager.addHandler(freightTimeAndDistanceAnalysisEventsHandler);

    CarrierLoadAnalysis carrierLoadAnalysis =
        new CarrierLoadAnalysis(CarriersUtils.getCarriers(scenario));
    ...
    freightTimeAndDistanceAnalysisEventsHandler.writeTravelTimeAndDistance(
        analysisOutputDirectory, scenario);
    freightTimeAndDistanceAnalysisEventsHandler.writeTravelTimeAndDistancePerVehicleType(
        analysisOutputDirectory, scenario);
