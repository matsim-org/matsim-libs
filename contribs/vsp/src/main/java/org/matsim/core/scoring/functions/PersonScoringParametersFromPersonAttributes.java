 ...
 *     <p>This class is an extension of {@link
 *     playground.vsp.scoring.IncomeDependentUtilityOfMoneyPersonScoringParameters} which is an
 *     adoption of {@link org.matsim.core.scoring.functions.SubpopulationScoringParameters}. This
 *     class additionaly allows for person-specific mode scoring parameters (for now ASC only) and
 *     marginalUtilityOfMoney. In order to use this, you need to provide the respective attributes
 *     (otherwise default values for the subpopulation are used). For mode scoring parameters use
 *     .... TODO For marginalUtilityOfMoney an attribute {@link
...
public class PersonScoringParametersFromPersonAttributes implements ScoringParametersForPerson {
  ...
  @Inject
  PersonScoringParametersFromPersonAttributes(
    ...
    if (scoringParametersForThisPerson == null) {
      ...
      if (personalScoringModeConstants != null) {
        for (Map.Entry<String, String> entry : personalScoringModeConstants.entrySet()) {
          ModeUtilityParameters.Builder modeUtilityParamsBuilder =
              new ModeUtilityParameters.Builder();
          try {
            modeUtilityParamsBuilder.setConstant(Double.parseDouble(entry.getValue()));
          } catch (NumberFormatException e) {
            log.error(
                "PersonalScoringModeConstants from person attribute could not be parsed for person "
          ...
          ScoringConfigGroup.ModeParams subpopulationModeParams =
              subpopulationScoringParams.getModes().get(entry.getKey());
          modeUtilityParamsBuilder.setMarginalUtilityOfTraveling_s(
              subpopulationModeParams.getMarginalUtilityOfTraveling());
          modeUtilityParamsBuilder.setMarginalUtilityOfDistance_m(
              subpopulationModeParams.getMarginalUtilityOfDistance());
          modeUtilityParamsBuilder.setMonetaryDistanceRate(
              subpopulationModeParams.getMonetaryDistanceRate());
          modeUtilityParamsBuilder.setDailyMoneyConstant(
              subpopulationModeParams.getDailyMonetaryConstant());
          modeUtilityParamsBuilder.setDailyUtilityConstant(
              subpopulationModeParams.getDailyUtilityConstant());
