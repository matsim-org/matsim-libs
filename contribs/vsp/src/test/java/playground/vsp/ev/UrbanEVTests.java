...
/**
 * <b>für (jeden) Agenten:</b> </b> (*) wann wird geladen? <br>
 * (*) wo wird geladen? <br>
 * (*) wird geladen? <br>
 ...
 * ? gleichzeitiges Laden: werden die Fahrzeuge in der richtigen Reihenfolge ein- und ausgestöpselt?
 * (chargingStart und chargingEndEvents) <br>
 * ? nicht Lader <br>
...
public class UrbanEVTests {
  ...
  public static void run() {
    ...
    scenario
        .getConfig()
        .controller()
        .setOutputDirectory("test/output/playground/vsp/ev/UrbanEVTests/");

    scenario.getConfig().controller().setLastIteration(0);

    // modify population
    overridePopulation(scenario);
    plannedActivitiesPerPerson =
    ...
    scenario
        ...
        .forEach(
            vehicleId -> {
              Id<VehicleType> type =
                  scenario.getVehicles().getVehicles().get(vehicleId).getType().getId();
    ...
    //		EVUtils.setInitialEnergy(scenario.getVehicles().getVehicleTypes().get(Id.create("Not enough
    // time so charging early", VehicleType.class)).getEngineInformation(),
    //				5.0);
  ...
  public void testAgentsExecuteSameNumberOfActsAsPlanned() {
    ...
    for (Map.Entry<Id<Person>, List<Activity>> person2Acts :
        plannedActivitiesPerPerson.entrySet()) {

      List<ActivityStartEvent> executedActs = handler.normalActStarts.get(person2Acts.getKey());
      if (executedActs.size()
          != person2Acts.getValue().size()
              - 1) { // first act of the day is not started but only ended in qsim
        fail = true;
    ...
    Assert.assertFalse(
        "the following persons do not execute the same amount of activities as they plan to:"
            + personsWithDifferingActCount,
        fail);
  }
  ...
  public void testCarAndBikeAgent() {
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Charge during leisure + bike"), List.of());
    ...
    // agent travels 5 links between precedent work activity which end at 11. each link takes 99
    // seconds
    Assert.assertEquals(
        "wrong charging start time",
        11 * 3600 + 5 * 99,
        pluginActStart2.getTime(),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging start location", "90", pluginActStart2.getLinkId().toString());

    List<ActivityEndEvent> plugouts =
        this.handler.plugOutCntPerPerson.getOrDefault(
            Id.createPersonId("Charge during leisure + bike"), List.of());
    Assert.assertEquals(1, plugouts.size(), 0);

    ActivityEndEvent plugoutActStart = plugouts.get(0);
    Assert.assertEquals(
        "wrong charging end time", 14 * 3600, plugoutActStart.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging end location", "90", plugoutActStart.getLinkId().toString());
  }
  ...
  public void testTripleCharger() {
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Triple Charger"), List.of());
    Assert.assertEquals(plugins.size(), 3., 0);
    ActivityStartEvent pluginActStart = plugins.get(0);
    Assert.assertEquals(
        "wrong charging start time", 1490d, pluginActStart.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging start location", "90", pluginActStart.getLinkId().toString());

    ActivityStartEvent pluginActStart2 = plugins.get(1);
    Assert.assertEquals(
        "wrong charging start time", 25580d, pluginActStart2.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging start location", "90", pluginActStart2.getLinkId().toString());

    ActivityStartEvent pluginActStart3 = plugins.get(2);
    Assert.assertEquals(
        "wrong charging start time", 45724d, pluginActStart3.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging start location", "95", pluginActStart3.getLinkId().toString());
    ...
    ActivityEndEvent plugoutActStart = plugouts.get(0);
    Assert.assertEquals(
        "wrong charging end time", 3179d, plugoutActStart.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging end location", "90", plugoutActStart.getLinkId().toString());

    ActivityEndEvent plugoutActStart2 = plugouts.get(1);
    Assert.assertEquals(
        "wrong charging end time", 26608d, plugoutActStart2.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging end location", "90", plugoutActStart2.getLinkId().toString());
  }
  ...
  public void testChargerSelectionShopping() {
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Charging during shopping"), List.of());
    Assert.assertEquals(1, plugins.size(), 0);
    ...
    Assert.assertEquals(
        "wrong charging start time",
        10 * 3600 + 8 * 99,
        pluginActStart.getTime(),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging start location", "172", pluginActStart.getLinkId().toString());
    ...
    ActivityEndEvent plugoutActStart = plugouts.get(0);
    Assert.assertEquals(
        "wrong charging end time",
        11 * 3600 + 23 * 60 + 27,
        plugoutActStart.getTime(),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging end location", "172", plugoutActStart.getLinkId().toString());
  }
  ...
  public void testLongDistance() {
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Charger Selection long distance leg"), List.of());
    Assert.assertEquals(1, plugins.size(), 0);
    ActivityStartEvent pluginActStart = plugins.get(0);
    ...
    Assert.assertEquals(
        ...
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging start location", "89", pluginActStart.getLinkId().toString());
    ...
    Assert.assertEquals(
        ...
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging end location", "89", plugoutActStart.getLinkId().toString());
  }
  ...
  public void testTwin() {
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Charger Selection long distance twin"), List.of());
    Assert.assertEquals(1, plugins.size(), 0);
    ActivityStartEvent pluginActStart = plugins.get(0);
    ...
    ActivityEndEvent plugoutActStart = plugouts.get(0);
    Assert.assertEquals(
        "wrong charging end time", 10 * 3600, plugoutActStart.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging end location", "89", plugoutActStart.getLinkId().toString());
  }
  ...
  public void testDoubleCharger() {
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Double Charger"), List.of());
    Assert.assertEquals(1, plugins.size(), 0);
    ActivityStartEvent pluginActStart = plugins.get(0);
    ...
    ActivityEndEvent plugoutActStart = plugouts.get(0);
    Assert.assertEquals(
        "wrong charging end time", 28783d, plugoutActStart.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging end location", "90", plugoutActStart.getLinkId().toString());
  }
  ...
  public void testNotEnoughTimeCharger() {
    ...
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Not enough time so no charge"), List.of());
  ...
  public void testEarlyCharger() {
    // this guy starts with more energy than the others, exceeds the threshold at the 3rd leg but
    // can only charge during first non-home-act. charge is lasting long enough so no additional
    // charge is needed

    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Not enough time so charging early"), List.of());
    Assert.assertEquals(1, plugins.size(), 0);
    ActivityStartEvent pluginActStart = plugins.get(0);
  ...
  public void testHomeCharger() {
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(Id.createPersonId("Home Charger"), List.of());
    Assert.assertEquals(1, plugins.size(), 0);
    ActivityStartEvent pluginActStart = plugins.get(0);
    ...
    Assert.assertEquals(
        ...
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging start location", pluginActStart.getLinkId().toString(), "95");

    List<ActivityEndEvent> plugouts =
        this.handler.plugOutCntPerPerson.getOrDefault(Id.createPersonId("Home Charger"), List.of());
  ...
  public void testNoRoundTripSoNoHomeCharge() {
    ...
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("No Round Trip So No Home Charge"), List.of());
  ...
  public void testDoubleChargerHomeCharger() {
    List<ActivityStartEvent> plugins =
        this.handler.plugInCntPerPerson.getOrDefault(
            Id.createPersonId("Double Charger Home Charger"), List.of());
    ...
    Assert.assertEquals(
        ...
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging start location", "95", pluginActStart.getLinkId().toString());
    ...
    ActivityEndEvent plugoutActStart = plugouts.get(0);
    Assert.assertEquals(
        "wrong charging end time", 33242d, plugoutActStart.getTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong charging end location", "95", plugoutActStart.getLinkId().toString());
    Assert.assertEquals(
        "Should plug out exactly once (as second plugin is for home charge)", 1, plugouts.size());
  }
  ...
  private static void overridePopulation(Scenario scenario) {
    ...
    {
      Person person3 =
          factory.createPerson(Id.createPersonId("Charger Selection long distance leg"));
    ...
    {
      Person person4 =
          factory.createPerson(Id.createPersonId("Charger Selection long distance twin"));
    ...
    {
      Person personWithOpenSubtour =
          factory.createPerson(Id.createPersonId("No Round Trip So No Home Charge"));
      Plan plan8 = factory.createPlan();
      ...
      Activity home8 =
          factory.createActivityFromLinkId(
              "home",
              Id.createLinkId(
                  "94")); // will return home to link 95 later (so no round trip, i.e. subtour, so
      // no charge
  ...
  private static class UrbanEVTestHandler
    ...
    public void reset(int iteration) {
      if (iteration > 0) {
        ...
        // TODO move the following assert statements out of the simulation loop? Or do we want to
        // explicitly check these _every_ iteration?

        //				Assert.assertEquals("there should be 9 people plugging in in this test", 9,
        // plugInCntPerPerson.size(), 0);
        //				Assert.assertEquals("there should be 8 people plugging out this test", 9,
        // plugOutCntPerPerson.size(), 0);
        //	The number of plug in and outs is not equal anymore since we added homecharging
        //				Assert.assertEquals( plugInCntPerPerson.size(), plugOutCntPerPerson.size()); //not
        // necessary
    ...
    public void handleEvent(ChargingEndEvent event) {
      ...
      ChargingStartEvent correspondingStart =
          this.chargingStarts
              .get(event.getVehicleId())
              .get(this.chargingStarts.get(event.getVehicleId()).size() - 1);
      Assert.assertEquals(
          "chargingEnd and chargingStart do not seem not to take place at the same charger. event="
              + event,
          correspondingStart.getChargerId(),
          event.getChargerId());
    }

    @Override
    public void handleEvent(ChargingStartEvent event) {
      this.chargingStarts.compute(
          event.getVehicleId(),
          (person, list) -> {
            if (list == null) list = new ArrayList<>();
            list.add(event);
  ...
  private static void compute(
      Map<Id<Person>, List<ActivityStartEvent>> map, ActivityStartEvent event) {
    map.compute(
        event.getPersonId(),
        (person, list) -> {
          if (list == null) list = new ArrayList<>();
