...
public class TransferFinalSocToNextIterTest {
  ...
  public void test() {
    ...
    scenario
        .getConfig()
        .controller()
        .setOutputDirectory("test/output/playground/vsp/ev/FinalSoc2VehicleTypeTest/");

    var vehicle1 =
        scenario.getVehicles().getVehicles().get(Id.create("Triple Charger_car", Vehicle.class));
    ...
    // agent has driven the car so SOC should have changed and should be dumped into the vehicle
    // type
    var vehicle =
        scenario.getVehicles().getVehicles().get(Id.create("Triple Charger_car", Vehicle.class));
    ...
    Assert.assertEquals(
        0.7273605127621898,
        evSpec.getInitialSoc(),
        MatsimTestUtils.EPSILON); // should not be fully charged
