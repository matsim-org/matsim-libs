...
public class TestWarmEmissionHandler {
  ...
  public final void testEmissionPerPersonWarmEventHandler() {
    ...
    //		if ( true ) {
    //			throw new RuntimeException("code below will probably no longer work since keys are now
    ...
    if (wepp.get(Id.create("v1", Person.class)).containsKey(Pollutant.PM)) {
      ...
      //		return key;
      Assert.assertNull(
          "PM of vehicle 1 should be null.",
          wepp.get(Id.create("v1", Person.class)).get(Pollutant.PM));
    }
    // CO vehicle 2
    //		return key;
    if (wepp.get(Id.create("v2", Person.class)).containsKey(Pollutant.CO)) {
      ...
      Assert.assertEquals(
          "CO of vehicle 2 should be 23.9", 23.9, actualCO2, MatsimTestUtils.EPSILON);
    } else {
    ...
    if (wepp.get(Id.create("v2", Person.class)).containsKey(Pollutant.PM)) {
      ...
      Assert.assertEquals(
          "PM of vehicle 2 should be 18.1", 18.1, actualPM2, MatsimTestUtils.EPSILON);
    } else {
