 ...
 *
 * <p>create and handle some events for different link and vehicle id assert that all used
 * pollutants are found in the corresponding maps but nothing else
 *
 * @author julia
 */
public class TestColdEmissionHandler {
  ...
  public final void testEmissionPerPersonColdEventHandler() {
    ...
    //
    //		if ( true ) {
    //			throw new RuntimeException("code below will possibly no longer work, since keys are now
    // strings not enums.  kai, dec'18") ;
    //		}
    ...
    Assert.assertEquals(
        "CO of vehicle 1 should be 17.1 but was " + actualCO1,
        new Double(17.1),
        actualCO1,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NOx of vehicle 1 should be 44.1 but was " + actualNOx1,
        new Double(44.1),
        actualNOx1,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "PM of vehicle 1 should be 0 but was " + actualPM1,
        new Double(0.0),
        actualPM1,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "CO of vehicle 2 should be 23.9 but was " + actualCO2,
        new Double(23.9),
        actualCO2,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NOx of vehicle 2 should be 0 but was " + actualNOx2,
        new Double(0.0),
        actualNOx2,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "PM of vehicle 2 should be 18.1 but was " + actualPM2,
        new Double(18.1),
        actualPM2,
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "There should be two types of emissions in this map but "
            + "there were "
            + cepp.get(Id.create("v1", Person.class)).size()
            + ".",
        2,
        cepp.get(Id.create("v1", Person.class)).keySet().size());
    Assert.assertEquals(
        "There should be two types of emissions in this map but "
            + "there were "
            + cepp.get(Id.create("v2", Person.class)).size()
            + ".",
        2,
        cepp.get(Id.create("v2", Person.class)).keySet().size());
  }
}
