...
/**
 * Simple scenario: - The bottleneck on link 3 (1 car every 10 seconds) is activated by the first
 * agent. - Then, the storage capacity on link 3 is reached. - Finally, the last agent is delayed.
 *
...
public class MarginalCongestionHandlerFlowSpillbackQueueQsimTest {
  ...
  public final void testFlowAndStorageCongestion_3agents() {
    ...
    for (CongestionEvent event : congestionEvents) {
      ...
      if (event.getCausingAgentId().toString().equals(this.testAgent1.toString())
          && event.getAffectedAgentId().toString().equals(this.testAgent2.toString())) {
        Assert.assertEquals("wrong delay.", 10.0, event.getDelay(), MatsimTestUtils.EPSILON);
      } else if ((event.getCausingAgentId().toString().equals(this.testAgent2.toString()))
          && (event.getAffectedAgentId().toString().equals(this.testAgent3.toString()))
          && (event.getTime() == 116.0)) {
        Assert.assertEquals("wrong delay.", 10.0, event.getDelay(), MatsimTestUtils.EPSILON);
      } else if ((event.getCausingAgentId().toString().equals(this.testAgent1.toString()))
          && (event.getAffectedAgentId().toString().equals(this.testAgent3.toString()))
          && (event.getTime() == 126.0)) {
  ...
  // 3 agents enter the relevant link at 8:14 (in the time-bin 8:00-8:15), the second and third
  // agent  leave the link after 8:15:00 (hence in the next time-bin 8:15-8:30)
  ...
  // in both cases the router should consider the time-bin of the LinkEnterEvent (not that one of
  // the Link-LeaveEvent)
  ...
  public final void testRouting() {
    ...
    String configFile =
        testUtils.getPackageInputDirectory()
            + "MarginalCongestionHandlerV3QsimTest/configTestRouting.xml";
                ...
                .toProvider(
                    new Provider<ControlerListener>() {
                      ...
                      public ControlerListener get() {
                        return new MarginalCongestionPricingContolerListener(
                            scenario,
                            tollHandler,
                            new CongestionHandlerImplV3(eventsManager, scenario));
                      }
                    });
    ...
    controler.addControlerListener(
        new IterationStartsListener() {
          ...
          public void notifyIterationStarts(IterationStartsEvent event) {
            ...
            if (((event.getServices().getConfig().controller().getLastIteration())
                    - (event.getIteration()))
                == 1) {
            ...
            // last iteration
            else if (((event.getServices().getConfig().controller().getLastIteration())
                    - (event.getIteration()))
                == 0) {
    ...
    { // controlling and showing the conditions of the test:
      Assert.assertEquals(
          "entering agents in timeBin1.", 3.0, enterCounter.get(timeBin1), MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "entering agents in timeBin2.", 3.0, enterCounter.get(timeBin2), MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "leaving agents in timeBin1.", 1.0, leaveCounter.get(timeBin1), MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          "leaving agents in timeBin1.", 5.0, leaveCounter.get(timeBin2), MatsimTestUtils.EPSILON);
    }
    ...
    Assert.assertEquals(
        "avgOldValue1 == avgOldValue2", avgOldValue1, avgOldValue2, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "avgOldValue3 == avgOldValue3", avgOldValue3, avgOldValue4, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "avgOldValue1 == avgOldValue3", avgOldValue1, avgOldValue3, MatsimTestUtils.EPSILON);

    Assert.assertEquals(
        "avgValue1 == avgOldValue1", avgValue1, avgOldValue1, MatsimTestUtils.EPSILON);
  }
  ...
  public final void testInsertingWaitingVehicles_01() {
    ...
    Assert.assertEquals(
        "numberOfCongestionEvents", congestionEvents.size(), 3, MatsimTestUtils.EPSILON);

    for (CongestionEvent mce : congestionEvents) {
      if ((mce.getCausingAgentId().equals(testAgent1))
          && (mce.getAffectedAgentId().equals(testAgent2))) {
        Assert.assertEquals("delay", 10., mce.getDelay(), MatsimTestUtils.EPSILON);
      } else if ((mce.getCausingAgentId().equals(testAgent2))
          && (mce.getAffectedAgentId().equals(testAgent3))) {
        Assert.assertEquals("delay", 10., mce.getDelay(), MatsimTestUtils.EPSILON);
      } else if ((mce.getCausingAgentId().equals(testAgent1))
          && (mce.getAffectedAgentId().equals(testAgent3))) {
  ...
  public final void testInsertingWaitingVehicles_02() {
    ...
    for (CongestionEvent mce : congestionEvents) {
      if ((mce.getCausingAgentId().equals(testAgent1))
          && (mce.getAffectedAgentId().equals(testAgent3))) {
        Assert.assertEquals("delay", 4., mce.getDelay(), MatsimTestUtils.EPSILON);
      } else if ((mce.getCausingAgentId().equals(testAgent3))
          && (mce.getAffectedAgentId().equals(testAgent2))) {
        Assert.assertEquals("delay", 10., mce.getDelay(), MatsimTestUtils.EPSILON);
      } else if ((mce.getCausingAgentId().equals(testAgent1))
          && (mce.getAffectedAgentId().equals(testAgent2))) {
