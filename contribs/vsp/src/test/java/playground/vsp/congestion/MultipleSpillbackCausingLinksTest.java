...
public class MultipleSpillbackCausingLinksTest {
   ...
   * therefore, need to iterate through all spill back causing links. (2) an agent may have two
   * different links as next link in the route and therefore correct one should be adopted.
   */
  @Test
  public final void multipleSpillBackTest() {
    ...
    for (CongestionEvent e : congestionEvents) {
      if (e.getAffectedAgentId().equals(Id.createPersonId("2"))
          && e.getLinkId().equals(Id.createLinkId("2"))
          && index == 1) {
        // first next link in the route is link 2
        Assert.assertEquals(
            "Delay on first next link in the route is not correct.",
            9.0,
            e.getDelay(),
            MatsimTestUtils.EPSILON);
        //				Assert.assertEquals("Delay on first next link in the route is not correct.", 10.0,
        // e.getDelay(), MatsimTestUtils.EPSILON); //1 sec change (1 time on link2) is caused due to
        // the change in the flow accumulation approach (can go to -ive). Amit Jan 17
        eventTimes.add(e.getTime());
        index++;
      }

      if (e.getAffectedAgentId().equals(Id.createPersonId("2"))
          && e.getLinkId().equals(Id.createLinkId("6"))
          && (index == 2 || index == 3)) {
        ...
        if (e.getCausingAgentId().equals(Id.createPersonId("3"))) {
          Assert.assertEquals(
              "Delay on second next link in the route is not correct.",
              10.0,
              e.getDelay(),
              MatsimTestUtils.EPSILON);
      ...
      if (e.getAffectedAgentId().equals(Id.createPersonId("2"))
        ...
        Assert.assertEquals(
            "Delay on first next link in the route due to multiple spill back is not correct.",
            4.0,
            e.getDelay(),
            MatsimTestUtils.EPSILON);
        //				Assert.assertEquals("Delay on first next link in the route due to multiple spill back
        // is not correct.", 6.0, e.getDelay(), MatsimTestUtils.EPSILON); //1 sec change (two times
        // on link2 and link6) is caused due to the change in the flow accumulation approach (can go
        // to -ive). Amit Jan 17
        eventTimes.add(e.getTime());
        index++;
      }
    ...
    // now check, for agent 2, first link in the route (trip1) is occur first and rest later time
    // i.e. during index ==1.
    Assert.assertFalse(
        "Congestion event for first next link in the route should occur first.",
        eventTimes.get(0) > eventTimes.get(1));

    // all other congestion event for agent 2 while leaving link 1 should occur at the same time.
    Assert.assertFalse(
        "Congestion event for multiple spill back causing links should occur at the same time.",
        eventTimes.get(1) == eventTimes.get(2) && eventTimes.get(2) == eventTimes.get(3));
  }
  ...
  private class createPseudoInputs {
    ...
    private void createNetwork() {
      ...
      Node node2 =
          NetworkUtils.createAndAddNode(
              network, Id.createNodeId("2"), new Coord((double) 0, (double) 100));
      Node node3 =
          NetworkUtils.createAndAddNode(
              network, Id.createNodeId("3"), new Coord((double) 500, (double) 150));
      Node node4 =
          NetworkUtils.createAndAddNode(
              network, Id.createNodeId("4"), new Coord((double) 1000, (double) 100));
      Node node5 =
          NetworkUtils.createAndAddNode(
              network, Id.createNodeId("5"), new Coord((double) 1000, (double) 0));
      Node node6 =
          NetworkUtils.createAndAddNode(
              network, Id.createNodeId("6"), new Coord((double) 500, (double) 50));
