...
public class CorridorNetworkTest {
  ...
  public void v3Test() {
    ...
    Assert.assertEquals(
        "wrong number of congestion events", 6, v3_events.size(), MatsimTestUtils.EPSILON);
    ...
    for (CongestionEvent event : v3_events) {
      ...
      if (event
          ...
          .equals(
              Id.createPersonId(
                  2))) { // agent 2 is delayed on link 2 (bottleneck link) due to agent 1
        ...
        Assert.assertEquals("wrong delay", 3, event.getDelay(), MatsimTestUtils.EPSILON);
          ...
          .getAffectedAgentId()
          .equals(Id.createPersonId(3))) { // agent 3 is delayed on link 2 due to agent 2, 1
        ...
        if (event.getCausingAgentId().equals(Id.createPersonId(2))) {
          ...
          .equals(
              Id.createPersonId(
                  4))) { // agent 4 is first delayed due to spill back on link 1 (3 sec) and then on
        // link 2 (6sec)
  ...
  public void v4Test() {
    ...
    Assert.assertEquals(
        "wrong number of congestion events", 6, v4_events.size(), MatsimTestUtils.EPSILON);
      ...
      if (event
        ...
        if (event.getCausingAgentId().equals(Id.createPersonId(3))) {
          if (event.getTime() == 10.0) {
            ...
            Assert.assertEquals(
                "wrong congestion event time", 18.0, event.getTime(), MatsimTestUtils.EPSILON);
            Assert.assertEquals("wrong delay", 4, event.getDelay(), MatsimTestUtils.EPSILON);
          }
  ...
  private List<CongestionEvent> getCongestionEvents(String congestionPricingImpl, Scenario sc) {
    ...
    } else if (congestionPricingImpl.equalsIgnoreCase("v4"))
  ...
  private class CorridorNetworkAndPlans {
    ...
    void createNetwork() {
      ...
      Node node2 =
          NetworkUtils.createAndAddNode(
              network, Id.createNodeId("2"), new Coord((double) 100, (double) 10));
      double y = -10;
      Node node3 =
          NetworkUtils.createAndAddNode(network, Id.createNodeId("3"), new Coord((double) 300, y));
      Node node4 =
          NetworkUtils.createAndAddNode(
              network, Id.createNodeId("4"), new Coord((double) 500, (double) 20));
      Node node5 =
          NetworkUtils.createAndAddNode(
              network, Id.createNodeId("5"), new Coord((double) 700, (double) 0));
      ...
      link0 =
          NetworkUtils.createAndAddLink(
              network,
              Id.createLinkId(String.valueOf("0")),
              fromNode,
              toNode,
              1000.0,
              20.0,
