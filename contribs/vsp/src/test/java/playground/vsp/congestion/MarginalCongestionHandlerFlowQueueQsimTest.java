...
/**
 * Simple scenario setup: Three agents moving along a corridor with unlimited storage capacity;
 * arriving simultaneously at the bottleneck.
 *
...
public class MarginalCongestionHandlerFlowQueueQsimTest {
  ...
  public final void testFlowCongestion_3agents_V3() {
    ...
    for (CongestionEvent event : congestionEvents) {
      Assert.assertEquals(
          "here the delay should be equal to the inverse of the flow capacity",
          3.,
          event.getDelay(),
    ...
    Assert.assertEquals(
        "wrong total delay", 9., congestionHandler.getTotalDelay(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "wrong total internalized delay",
        9.,
        congestionHandler.getTotalInternalizedDelay(),
        MatsimTestUtils.EPSILON);
  }
  ...
  public final void testFlowCongestion_3agents_V7() {
    ...
    for (CongestionEvent event : congestionEvents) {
      if (event.getCausingAgentId().toString().equals("agentC")
          && event.getAffectedAgentId().toString().equals("agentB")) {
        Assert.assertEquals("wrong delay", 3., event.getDelay(), MatsimTestUtils.EPSILON);
      }

      if (event.getCausingAgentId().toString().equals("agentC")
          && event.getAffectedAgentId().toString().equals("agentA")) {
        Assert.assertEquals("wrong delay", 6., event.getDelay(), MatsimTestUtils.EPSILON);
      }

      if (event.getCausingAgentId().toString().equals("agentB")
          && event.getAffectedAgentId().toString().equals("agentA")) {
        Assert.assertEquals("wrong delay", 6., event.getDelay(), MatsimTestUtils.EPSILON);
      }
    }
