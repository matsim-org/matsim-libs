 ...
 *
 * <p>It checks whether the person specific income is read from the person attributes. The
 * marginalUtilityOfMoney should be calculated as averageIncome/personSpecificIncome and not taken
 * from the subpopulation-specific scoring params. To check whether the remaining scoring params are
 * subpopulation-specific, this class tests the person's marginalUtilityOfWaitingPt_s accordingly.
 */
public class IncomeDependentUtilityOfMoneyPersonScoringParametersNoSubpopulationTest {

  @Rule public MatsimTestUtils utils;
  ...
  public void setUp() {
    ...
    ScoringConfigGroup.ScoringParameterSet defaultParams =
        scoringConfigGroup.getOrCreateScoringParameters(null);
    ...
    personScoringParams =
        new IncomeDependentUtilityOfMoneyPersonScoringParameters(
            population, scoringConfigGroup, scenarioConfigGroup, transitConfigGroup);
  }
  ...
  public void testPersonWithNegativeIncome() {
    ...
    ScoringParameters params =
        personScoringParams.getScoringParameters(population.getPersons().get(id));
    // person's attribute says it has negative income which is considered invalid and therefore the
    // subpopulation's mgnUtilityOfMoney is taken (which is 1)
  ...
  public void testPersonWithNoIncome() {
    ...
    ScoringParameters params =
        personScoringParams.getScoringParameters(population.getPersons().get(id));
    // person's attribute says it has 0 income which is considered invalid and therefore the
    // subpopulation's mgnUtilityOfMoney is taken (which is 1)
    makeAssert(params, 1d, 0.5d);
  }
  ...
  public void testMoneyScore() {
    ScoringParameters paramsRich =
        personScoringParams.getScoringParameters(
            population.getPersons().get(Id.createPersonId("highIncome")));
    CharyparNagelMoneyScoring moneyScoringRich = new CharyparNagelMoneyScoring(paramsRich);
    moneyScoringRich.addMoney(100);
    Assert.assertEquals(
        "for the rich person, 100 money units should be equal to a score of ",
        20 * 1. / 1.5 * 100,
        moneyScoringRich.getScore(),
        MatsimTestUtils.EPSILON);

    ScoringParameters paramsPoor =
        personScoringParams.getScoringParameters(
            population.getPersons().get(Id.createPersonId("lowIncome")));
    CharyparNagelMoneyScoring moneyScoringPoor = new CharyparNagelMoneyScoring(paramsPoor);
    moneyScoringPoor.addMoney(100);
    Assert.assertEquals(
        "for the poor person, 100 money units should be equal to a score of ",
        20 * 1. / 0.5 * 100,
        moneyScoringPoor.getScore(),
        MatsimTestUtils.EPSILON);

    Assert.assertTrue(
        "100 money units should worth more for a poor person than for a rich person",
        moneyScoringPoor.getScore() > moneyScoringRich.getScore());
  }

  private void makeAssert(
      ScoringParameters params, double income, double marginalUtilityOfWaitingPt_s) {
    Assert.assertEquals(
        "marginalUtilityOfMoney is wrong", 20 * 1 / income, params.marginalUtilityOfMoney, 0.);
    Assert.assertEquals(
        "marginalUtilityOfWaitingPt_s is wrong",
        marginalUtilityOfWaitingPt_s,
        params.marginalUtilityOfWaitingPt_s,
        0.);
  }
}
