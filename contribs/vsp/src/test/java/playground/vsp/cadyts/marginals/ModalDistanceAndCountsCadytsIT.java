...
public class ModalDistanceAndCountsCadytsIT {
  ...
  private static void createPopulation(Population population, Network network) {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      person.addPlan(
          createPlan(TransportMode.bike, "long-end-link", network, population.getFactory()));
      person.addPlan(
          createPlan(TransportMode.car, "long-end-link", network, population.getFactory()));
      person.addPlan(
          createPlan(TransportMode.bike, "short-end-link", network, population.getFactory()));
      person.addPlan(
          createPlan(TransportMode.car, "counts-end-link", network, population.getFactory()));
      Plan carPlan =
          createPlan(TransportMode.car, "short-end-link", network, population.getFactory());
  ...
  private static Plan createPlan(
      String mode, String endLink, Network network, PopulationFactory factory) {
    ...
    // work
    Activity w = factory.createActivityFromLinkId("work", Id.createLinkId(endLink));
  ...
  public void test() {
    ...
    controler.setScoringFunctionFactory(
        new ScoringFunctionFactory() {
          ...
          public ScoringFunction createNewScoringFunction(Person person) {
            ...
            // add counts cadyts
            final CadytsScoring<Link> scoringFunctionCounts =
    ...
    if (this.modalDistanceWeight > 0 && this.countsWeight == 0) {
      ...
      assertTrue(
          "expected more than 500 car trips on the long route but the number of trips was "
              + modalDistanceCount.get("car_2250.0"),
          modalDistanceCount.get("car_2250.0")
              > 500); // don't know. one would assume a stronger impact when only running the cadyts
      // count correction but there isn't
    } else if (this.modalDistanceWeight > 0 && this.countsWeight > 0) {
