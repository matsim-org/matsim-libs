...
public class TripEventHandlerTest {
  ...
  /**
   * This test takes the pt-tutorial from the scenarios module and performs one iteration.
   * Afterwards it is tested whether every agents has performed trips. For two agents the trips are
   * investigated in detail.
   *
   * <p>Also one agent is excluded from the recording, to test, whether the agent filter works
   * correctly.
   *
  ...
  public void test() throws MalformedURLException {
    ...
    URL ptTutorial =
        URI.create(ExamplesUtils.getTestScenarioURL("pt-tutorial").toString() + "0.config.xml")
            .toURL();
