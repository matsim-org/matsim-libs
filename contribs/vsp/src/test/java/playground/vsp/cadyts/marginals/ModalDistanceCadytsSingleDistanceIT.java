...
public class ModalDistanceCadytsSingleDistanceIT {
  ...
  /**
   * This test runs a population of 1000 agents which have the same home and work place. All agents
   * start with two plans. One with mode car and one with mode bike. The selected plan is the car
   * plan. Now, the desired distance distribution is set to have an equal share of car and bike
   * users. The accepted error in the test is 5%, due to stochastic fuzziness
   */
  @Test
  public void test() {
    ...
    controler.setScoringFunctionFactory(
        new ScoringFunctionFactory() {
          ...
          @Inject private ModalDistanceCadytsContext modalDistanceCadytsContext;
          ...
          public ScoringFunction createNewScoringFunction(Person person) {
            ...
            final CadytsScoring<Id<DistanceDistribution.DistanceBin>> scoringFunctionMarginals =
                new CadytsScoring<>(person.getSelectedPlan(), config, modalDistanceCadytsContext);
        ...
        scenario.getPopulation().getPersons().values().stream()
            ...
            .map(
                activity ->
                    activity
                        .getType()
                        .substring(0, activity.getType().length() - " interaction".length()))
  ...
  private Config createConfig() {
    ...
    // have random selection of plans to generate heterogenity in the beginning, so that cadyts can
    // calibrate its correction
    ReplanningConfigGroup.StrategySettings selectRandom =
    ...
    config
        .changeMode()
        .setBehavior(ChangeModeConfigGroup.Behavior.fromSpecifiedModesToSpecifiedModes);
  ...
  private static Scenario cretaeScenario(Config config) {
    ...
    VehicleType bike =
        scenario
            .getVehicles()
            .getFactory()
            .createVehicleType(Id.create(TransportMode.bike, VehicleType.class));
  ...
  private static Link createLink(
      Node from, Node to, String id, double length, NetworkFactory factory) {
  ...
  private static Plan createPlan(String mode, Network network, PopulationFactory factory) {
    ...
    Activity h =
        factory.createActivityFromCoord(
            "home", network.getLinks().get(Id.createLinkId("start-link")).getFromNode().getCoord());
    ...
    // work
    Activity w = factory.createActivityFromLinkId("work", Id.createLinkId("end-link"));
