...
/**
 * TODO: reduce size of input files: either cut files to only include relevant TransitLines or use
 * different scenario Maybe the latter would be better such that we can include those files on this
 * repository / write them as Fixture in Java.
 *
 * @author jakobrehmann
 */
@Ignore // is ignored :-(
public class TransitRouteTrimmerTest {
  ...
  final String inZoneShpFile =
      "https://svn.vsp.tu-berlin.de/repos/public-svn/matsim/scenarios/countries/de/berlin/projects/avoev/shp-files/shp-berlin-hundekopf-areas/berlin_hundekopf.shp";
  final String inScheduleFile =
      "https://svn.vsp.tu-berlin.de/repos/public-svn/matsim/scenarios/countries/de/berlin/berlin-v5.5-10pct/input/berlin-v5.5-transit-schedule.xml.gz";
  final String inVehiclesFile =
      "https://svn.vsp.tu-berlin.de/repos/public-svn/matsim/scenarios/countries/de/berlin/berlin-v5.5-10pct/input/berlin-v5.5-transit-vehicles.xml.gz";
  final String inNetworkFile =
  ...
  /**
   * This test class examines three different types of routes: allIn: all stops of transitRoute are
   * within zone halfIn: one end of route is within the zone, one outside of the zone middleIn: both
   * ends of route are outside of zone, but middle in within zone
   *
   * <p>The following enum stores the transitRoute and transitLine Ids for each type
   */
  public enum routeType {
    allIn(
        Id.create("265---17372_700", TransitLine.class),
        Id.create("265---17372_700_0", TransitRoute.class)),
    halfIn(
        Id.create("184---17340_700", TransitLine.class),
        Id.create("184---17340_700_15", TransitRoute.class)),
    middleIn(
        Id.create("161---17326_700", TransitLine.class),
  ...
  public void test_AllIn() {
    ...
    Scenario scenario =
        provideCopyOfScenario(this.inScheduleFile, this.inNetworkFile, this.inVehiclesFile);
    Set<Id<TransitStopFacility>> stopsInZone =
        getStopsInZone(scenario.getTransitSchedule(), inZoneShpFile);
  ...
  /**
   * In the halfIn scenario, the transitRoute in question begins outside of the zone and ends within
   * the zone.
   */
  @Ignore
  @Test
  public void test_HalfIn() {
    ...
    Id<TransitStopFacility> lastStopId =
        transitRoute.getStops().get(sizeOld - 1).getStopFacility().getId();
  ...
  /**
   * trimming method: DeleteRoutesEntirelyInsideZone. route scenario: AllIn The testRoute should be
   * deleted since all stops are within the zone.
   */
  @Ignore
  @Test
  public void testDeleteRoutesEntirelyInsideZone_AllIn() {
    ...
    Pair<TransitSchedule, Vehicles> results =
        TransitRouteTrimmer.deleteRoutesEntirelyInsideZone(
    ...
    assertEquals(
        "transitLine should no longer contain any routes",
        transitScheduleNew.getTransitLines().get(transitLineId).getRoutes().size(),
        0);

    // Test vehicles
    Set<Id<Vehicle>> vehiclesUsedInTransitSchedule =
        getVehiclesUsedInTransitSchedule(transitScheduleNew);
  ...
  /**
   * trimming method: DeleteRoutesEntirelyInsideZone. route scenario: HalfIn The testRoute should be
   * retained and left unmodified, since some stops are outside the zone.
   */
  @Ignore
  @Test
  public void testDeleteRoutesEntirelyInsideZone_HalfIn() {
    ...
    assertEquals(
        "transitRoute should contain same number of stops as before modification",
        stopCntOld,
        stopCntNew);
  ...
  /**
   * trimming method: DeleteRoutesEntirelyInsideZone. route scenario: MiddleIn The testRoute should
   * be retained and left unmodified, since some stops are outside the zone.
   */
  @Ignore
  @Test
  ...
  /**
   * trimming method: TrimEnds. route scenario: AllIn The testRoute should be deleted since all
   * stops are within the zone.
   */
  @Ignore
  @Test
  ...
  /**
   * trimming method: TrimEnds. route scenario: HalfIn The second half of the route is outside the
   * zone and should be trimmed
   */
  @Ignore
  @Test
  public void testTrimEnds_HalfIn() {
    ...
    TransitRoute transitRouteOld =
        scenario
            .getTransitSchedule()
            .getTransitLines()
            .get(Id.create("184---17340_700", TransitLine.class))
            .getRoutes()
            .get(Id.create("184---17340_700_15", TransitRoute.class));
    int sizeOld = transitRouteOld.getStops().size();
    int outCntOld = countStopsOutsideZone(transitRouteOld, stopsInZone);
    Id<TransitStopFacility> firstStopOld =
        transitRouteOld.getStops().get(0).getStopFacility().getId();
    Id<TransitStopFacility> lastStopOld =
        transitRouteOld.getStops().get(sizeOld - 1).getStopFacility().getId();
    ...
    // After trim
    TransitSchedule transitScheduleNew = results.getKey();
    TransitRoute transitRouteNew =
        transitScheduleNew.getTransitLines().get(transitLineId).getRoutes().get(transitRouteId);
    ...
    Id<TransitStopFacility> firstStopNew =
        transitRouteNew.getStops().get(0).getStopFacility().getId();
    Id<TransitStopFacility> lastStopNew =
        transitRouteNew.getStops().get(sizeNew - 1).getStopFacility().getId();

    Assert.assertTrue("modified route should have less stops as original route", sizeOld > sizeNew);
    assertEquals("there should be no stops within the zone", 0, inCntNew);
    assertEquals("number of stops outside of zone should remain same", outCntOld, outCntNew);
    assertEquals("first stop of old and new route should be same", firstStopOld, firstStopNew);
    assertNotEquals("last stop of old and new route should be different", lastStopOld, lastStopNew);
  ...
  /**
   * trimming method: TrimEnds. route scenario: MiddleIn Since the ends are both outside of zone,
   * route should not be modified
   */
  @Ignore
  @Test
  public void testTrimEnds_MiddleIn() {
    ...
    Assert.assertTrue(
        "line should still exist", transitScheduleNew.getTransitLines().containsKey(transitLineId));
    Assert.assertEquals(
        "new route should contain same number of stops as old one", numStopsOld, numStopsNew);
    Assert.assertEquals(
        "new route should contain same number of links as old one", numLinksOld, numLinksNew);
  ...
  /** trimming method: SkipStops. route scenario: AllIn New route should be empty */
  @Ignore
  @Test
  public void testSkipStops_AllIn() {
    ...
    TransitSchedule transitScheduleNew = results.getKey();
    assertTrue(
        "sched should include empty transit line",
        transitScheduleNew.getTransitLines().containsKey(transitLineId));
    assertEquals(
        "transitLine should not longer contain any routes",
        transitScheduleNew.getTransitLines().get(transitLineId).getRoutes().size(),
        0);
  ...
  /** trimming method: SkipStops. route scenario: HalfIn Stops outside zone should be skipped */
  @Ignore
  @Test
  public void testSkipStops_HalfIn() {
    ...
    Assert.assertTrue("there should be less stops after the modification", sizeOld > sizeNew);
    assertTrue("new route should have less links than old route", numLinksNew < numLinksOld);
    assertEquals("there should only be one stop within the zone", 1, inCntNew);
    assertEquals("the number of stops outside of zone should remain same", outCntOld, outCntNew);
  ...
  /**
   * trimming method: SkipStops. route scenario: MiddleIn New route should have less stops than old
   * route, but same amount of links
   */
  @Ignore
  @Test
  public void testSkipStops_MiddleIn() {
    ...
    Assert.assertNotEquals(
        "new route should NOT contain same number of stops as old one", numStopsOld, numStopsNew);
    Assert.assertEquals(
        "new route should contain same number of links as old one", numLinksOld, numLinksNew);
    Assert.assertEquals(
        "new route should only have two stops within zone, one per zone entrance/exit",
        2,
        inCntNew);
  ...
  /** trimming method: SplitRoutes. route scenario: AllIn route will be deleted */
  @Ignore
  @Test
  public void testSplitRoutes_AllIn() {
    ...
    Set<Id<TransitLine>> linesToModify = Collections.singleton(transitLineId);
    Pair<TransitSchedule, Vehicles> results =
        TransitRouteTrimmer.splitRoute(
            scenario.getTransitSchedule(),
            scenario.getTransitVehicles(),
            stopsInZone,
            linesToModify,
  ...
  /**
   * trimming method: SplitRoutes. route scenario: HalfIn New route should have less stops than old
   * route
   */
  @Ignore
  @Test
  public void testSplitRoutes_HalfIn() {
    ...
    assertTrue(
        transitLine
            .getRoutes()
            .containsKey(Id.create("184---17340_700_15_split1", TransitRoute.class)));
    ...
    TransitRoute transitRouteNew =
        transitScheduleNew
            ...
            .get(Id.create("184---17340_700", TransitLine.class))
            .getRoutes()
            .get(Id.create("184---17340_700_15_split1", TransitRoute.class));
    int sizeNew = transitRouteNew.getStops().size();
    int inCntNew = countStopsInZone(transitRouteNew, stopsInZone);
    int outCntNew = countStopsOutsideZone(transitRouteNew, stopsInZone);

    assertTrue("new route should have less stops than old route", sizeOld > sizeNew);
    assertEquals("there should only be one stop within the zone", 1, inCntNew);
    assertEquals("# of stops outside of zone should remain same", outCntOld, outCntNew);
  ...
  /**
   * trimming method: SplitRoutes. route scenario: MiddleIn Two routes should be created, each with
   * only one stop within zone
   */
  @Ignore
  @Test
  public void testSplitRoutes_MiddleIn() {
    ...
    assertTrue(
        transitLineNew
            .getRoutes()
            .containsKey(Id.create("161---17326_700_22_split1", TransitRoute.class)));
    assertTrue(
        transitLineNew
            .getRoutes()
            .containsKey(Id.create("161---17326_700_22_split2", TransitRoute.class)));
    assertFalse(
        transitLineNew
            .getRoutes()
            .containsKey(Id.create("161---17326_700_22_split0", TransitRoute.class)));
    assertFalse(
        transitLineNew
            .getRoutes()
            .containsKey(Id.create("161---17326_700_22_split3", TransitRoute.class)));
    assertFalse(
        transitLineNew
            .getRoutes()
            .containsKey(Id.create("161---17326_700_22", TransitRoute.class)));

    TransitRoute transitRouteNew1 =
        transitLineNew.getRoutes().get(Id.create("161---17326_700_22_split1", TransitRoute.class));
    TransitRoute transitRouteNew2 =
        transitLineNew.getRoutes().get(Id.create("161---17326_700_22_split2", TransitRoute.class));

    assertNotEquals(
        transitRouteOld.getStops().size(),
        transitRouteNew1.getStops().size() + transitRouteNew2.getStops().size());
  ...
  /**
   * Test Hub functionality trimming method: SplitRoutes. route scenario: MiddleIn tests reach of
   * hubs. Left hub should be included in route 1, while right hub should not be included in route
   * 2, due to lacking reach
   */
  @Ignore
  @Test
  public void testSplitRoutes_MiddleIn_Hub_ValidateReach() {
    ...
    scenario
        ...
        .get(facIdLeft)
        .getAttributes()
        .putAttribute("hub-reach", 3);

    // Stop 070101006207 is a hub with reach of 3. This stop is therefore just out of range for
    // route 2
  ...
  /**
   * Test Hub functionality trimming method: SplitRoutes. route scenario: MiddleIn tests parameter
   * to include first nearest hub, even if reach is insufficient. Right hub should be included, even
   * though reach is too low.
   */
  @Ignore
  @Test
  public void testSplitRoutes_MiddleIn_Hub_IncludeFirstHubInZone() {
    ...
    assertEquals("new route #1 should have three stops within zone", 3, inCntNew1);
    assertEquals("new route #2 should have four stops within zone", 4, inCntNew2);
    Id<TransitStopFacility> idRoute1 =
        transitRouteNew1
            .getStops()
            .get(transitRouteNew1.getStops().size() - 1)
            .getStopFacility()
            .getId();
    assertEquals("last stop of route #1 should be the left hub", facIdLeft, idRoute1);

    Id<TransitStopFacility> idRoute2 = transitRouteNew2.getStops().get(0).getStopFacility().getId();
    assertEquals("first stop of route #2 should be the right hub", facIdRight, idRoute2);
  ...
  /**
   * Test Hub functionality trimming method: SplitRoutes. route scenario: MiddleIn if multiple hubs
   * are within reach of route, the route should go to further one
   */
  @Ignore
  @Test
  public void testSplitRoutes_MiddleIn_Hub_MultipleHubs() {
    ...
    assertFalse(
        stopsInZone.contains(
            transitRouteOld.getStops().get(numStopsOld - 1).getStopFacility().getId()));
  ...
  /**
   * Test Hub functionality trimming method: SplitRoutes. route scenario: MiddleIn if two new routes
   * overlap (because they both reach to same hub) then they should be combined into one route
   */
  @Ignore
  @Test
  ...
  /**
   * Test parameter allowableStopsWithinZone trimming method: SplitRoutes. route scenario: MiddleIn
  ...
  public void testDeparturesAndOffsetsAndDescription() {
    ...
    Assert.assertEquals(
        routeOld2End.getDepartureOffset().seconds() - deltaSeconds,
        routeNew2End.getDepartureOffset().seconds(),
        0.);
    Assert.assertEquals(
        routeOld2End.getArrivalOffset().seconds() - deltaSeconds,
        routeNew2End.getArrivalOffset().seconds(),
        0.);

    // Check Departures
    Assert.assertEquals(
        transitRouteOld.getDepartures().size(), transitRouteNew1.getDepartures().size());
    Assert.assertEquals(
        transitRouteOld.getDepartures().size(), transitRouteNew2.getDepartures().size());
    for (Departure departureOld : transitRouteOld.getDepartures().values()) {
      ...
      Assert.assertEquals(
          vehicleIdOld.toString() + "_split1", departureNew1.getVehicleId().toString());
      ...
      Assert.assertEquals(
          vehicleIdOld.toString() + "_split2", departureNew2.getVehicleId().toString());
  ...
  private Set<Id<TransitStopFacility>> getStopsInZone(
      TransitSchedule transitSchedule, String zoneShpFile) {
   ...
   */
  private Scenario provideCopyOfScenario(
      String scheduleFile, String networkFile, String vehiclesFile) {
      ...
      copiedScenario
          ...
          .addNode(
              copiedScenario
                  ...
                  .createNode(
                      node.getId(),
                      CoordUtils.createCoord(node.getCoord().getX(), node.getCoord().getY())));
    }
    ...
    for (TransitStopFacility transitStopFacility :
        inputScenario.getTransitSchedule().getFacilities().values()) {
      TransitStopFacility copiedTransitStopFacility =
          copiedScenario
              .getTransitSchedule()
              .getFactory()
      ...
      for (Map.Entry<String, Object> entry :
          transitStopFacility.getAttributes().getAsMap().entrySet()) {
    ...
    for (TransitLine transitLine : inputScenario.getTransitSchedule().getTransitLines().values()) {
      TransitLine copiedTransitLine =
          copiedScenario.getTransitSchedule().getFactory().createTransitLine(transitLine.getId());
      ...
      for (TransitRoute transitRoute : transitLine.getRoutes().values()) {
              ...
              copiedScenario
                  ...
                  .createTransitRouteStop(
                      copiedScenario
                          .getTransitSchedule()
                          .getFacilities()
                          .get(transitRouteStop.getStopFacility().getId()),
                      transitRouteStop.getArrivalOffset(),
                      transitRouteStop.getDepartureOffset()));
        }
        TransitRoute copiedTransitRoute =
            copiedScenario
                .getTransitSchedule()
                .getFactory()
                .createTransitRoute(
                    transitRoute.getId(),
                    transitRoute.getRoute().clone(),
        ...
        for (Departure departure : transitRoute.getDepartures().values()) {
          Departure copiedDeparture =
              copiedScenario
                  .getTransitSchedule()
                  .getFactory()
                  .createDeparture(departure.getId(), departure.getDepartureTime());
    ...
    for (VehicleType vehicleType : inputScenario.getTransitVehicles().getVehicleTypes().values()) {
      VehicleType copiedVehicleType =
          copiedScenario.getTransitVehicles().getFactory().createVehicleType(vehicleType.getId());
      ...
      VehicleUtils.setDoorOperationMode(
          copiedVehicleType, VehicleUtils.getDoorOperationMode(vehicleType));
  ...
  private Set<Id<Vehicle>> getVehiclesUsedInTransitSchedule(TransitSchedule transitScheduleNew) {
    return transitScheduleNew.getTransitLines().values().stream()
        .flatMap(
            x ->
                x.getRoutes().values().stream()
                    .flatMap(y -> y.getDepartures().values().stream().map(Departure::getVehicleId)))
        .collect(Collectors.toSet());
  }

  private int countStopsInZone(
      TransitRoute transitRoute, Set<Id<TransitStopFacility>> stopsInZone) {
    int inCnt = 0;
    for (TransitRouteStop stop : transitRoute.getStops()) {
      if (stopsInZone.contains(stop.getStopFacility().getId())) {
        inCnt++;
  ...
  private int countStopsOutsideZone(
      TransitRoute transitRoute, Set<Id<TransitStopFacility>> stopsInZone) {
    int outCnt = 0;
    for (TransitRouteStop stop : transitRoute.getStops()) {
      if (!stopsInZone.contains(stop.getStopFacility().getId())) {
