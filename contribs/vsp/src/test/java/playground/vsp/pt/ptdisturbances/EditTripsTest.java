 ...
 *     <p>TODO: Add test that first edits one trip of an agent and then later on edits another one
 *     -> notice issues with undefined travel times and similar.
 */
public class EditTripsTest {
  ...
  /**
   * Case 1.1.2 Looks awkward in otfvis, because after replanning there suddenly are 2 testAgents at
   * different places, but events are fine :-/
   */
  @Test
  ...
  /**
   * Case 2.1 Does what the name suggests although waiting at the first stop for the first
   * undisturbed departure leads to boarding the very same bus. This is due to the utility params
   * for walk, pt_wait and pt being equal/indifferent.
   */
  @Test
  ...
  /**
   * Case 2.2 Same as {@link #testAgentIsAtTeleportLegAndLeavesStop()} but marginal utility of
   * travelling worse for walk than for pt_wait and pt. Strangely only works with walk being
   * significantly worse, does not work with small differences.
   */
  @Test
  ...
  /**
   * Case 3 Simulates 900 agents so this case should include every possible state of an agent.
   * Current and future trips are replanned.
   */
  @Test
  public void testOneAgentEveryFourSeconds() {
    ...
    for (Id<?> personId : arrivalTimes.keySet()) {
      ...
      assertTrue(
          "Number of used lines ist not plausible",
          numberOfUsedLines == 1 || numberOfUsedLines == 2);
    }
  ...
  public static Person buildPerson(Scenario scenario, double endTime) {
    ...
    Activity act0 =
        populationFactory.createActivityFromLinkId("dummy", Id.createLinkId("car_17bOut"));
  ...
  void run(
      Scenario scenario,
      boolean openOTFVis,
      HashMap<Id<Person>, List<String>> trips,
      HashMap<Id<Person>, Double> arrivalTimes) {
  ...
  private SignalSystemsConfigGroup adaptConfigForSignals(Config config) {
    ...
    config
        .qsim()
        .setUsingFastCapacityUpdate(
            false); // otherwise inject error message "Fast flow capacity update does not support
    // signals"
  ...
  private void buildSignals(Scenario scenario, SignalSystemsConfigGroup signalSystemsConfigGroup) {
    ...
    SignalSystemControllerData controller =
        control.getFactory().createSignalSystemControllerData(idSignalSystem);
    ...
    SignalPlanData plan1 =
        control.getFactory().createSignalPlanData(Id.create("1", SignalPlan.class));
    ...
    // cycle time, start time and offset interact, wrong combinations lead to the signal remaining
    // switched off. Better don't touch - gl jul'19
    int startTime =
        7 * 3600 + 30 * 60
            + 120; // let 7:30 departure pass the signal for testAgentLeavesVehicleAtNextStop()
    ...
    SignalGroupSettingsData settings1 =
        control.getFactory().createSignalGroupSettingsData(Id.create("1", SignalGroup.class));
      ...
      if ((int) now == replanTime) {
        ...
        for (Departure departure : disturbedRoute.getDepartures().values()) {
          if (departure.getDepartureTime() >= 7.5 * 3600.
              && departure.getDepartureTime() < 8. * 3600.) {
        ...
        RunExamplePtDisturbances.replanPtPassengers(
            now, disturbedLineId, tripRouterProvider, scenario, internalInterface);
      }
    }
  ...
  private static class HandlerForTests
    ...
    public HandlerForTests(
        HashMap<Id<Person>, List<String>> trips, HashMap<Id<Person>, Double> arrivalTimes) {
      this.trips = trips;
