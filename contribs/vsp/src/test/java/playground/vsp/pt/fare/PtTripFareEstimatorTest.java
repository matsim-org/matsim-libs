...
public class PtTripFareEstimatorTest {
  ...
  @Inject private EstimateRouter router;
  @Inject private ScoringParametersForPerson params;
  @Inject private Map<String, TripEstimator<?>> tripEstimator;
  ...
  public void setUp() throws Exception {
    ...
    Map<String, ScoringConfigGroup.ModeParams> modes =
        config.scoring().getScoringParameters("person").getModes();
    ...
    DistanceBasedPtFareParams distanceFare =
        ConfigUtils.addOrGetModule(config, DistanceBasedPtFareParams.class);
  ...
  private List<MinMaxEstimate> estimateAgent(Id<Person> personId) {
    ...
    EstimatorContext context =
        new EstimatorContext(plan.getPerson(), params.getScoringParameters(plan.getPerson()));

    PlanModel model = PlanModel.newInstance(plan);

    router.routeModes(
        model, Set.of(TransportMode.pt, TransportMode.walk, TransportMode.bike, TransportMode.car));
    ...
    for (int i = 0; i < model.trips(); i++) {
      ...
      MinMaxEstimate est =
          estimator.estimate(context, TransportMode.pt, model, trip, ModeAvailability.YES);
  ...
  public void fare() {
    ...
    assertThat(est)
        ...
        .first()
        .extracting(MinMaxEstimate::getMin, InstanceOfAssertFactories.DOUBLE)
  ...
  public void all() {
    ...
    for (Id<Person> agent : TestScenario.Agents) {
      ...
      assertThat(est).allMatch(e -> e.getMin() <= e.getMax(), "Min smaller max");
    }
  }
  ...
  public void planEstimate() {
    ...
    Person person =
        controler.getScenario().getPopulation().getPersons().get(TestScenario.Agents.get(2));
    Plan plan = person.getSelectedPlan();
    ...
    double estimate =
        estimator.estimate(
            context,
            TransportMode.pt,
            new String[] {"pt", "car", "pt", "pt", "pt"},
            model,
            ModeAvailability.YES);
    ...
    estimate =
        estimator.estimate(
            context,
            TransportMode.pt,
            new String[] {"pt", "car", "car", "car", "car"},
            model,
            ModeAvailability.YES);
    assertThat(estimate).isCloseTo(singleTrips.get(0).getMin(), Offset.offset(0.1));
  }
}
