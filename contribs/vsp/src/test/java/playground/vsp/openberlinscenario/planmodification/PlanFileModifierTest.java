...
public class PlanFileModifierTest {
  private static final Logger LOG = LogManager.getLogger(PlanFileModifierTest.class);

  @Rule public MatsimTestUtils utils = new MatsimTestUtils();

  private static final double SELECTION_PROBABILITY = 0.70;
  ...
  public void initializeTestPopulations() {
    ...
    PlanFileModifier planFileModifier2 =
        new PlanFileModifier(
            formatedInputPlansFile,
            outputPlansFile2,
            SELECTION_PROBABILITY,
  ...
  public void testSelectionProbability() {
    ...
    LOG.info(
        "real selection probability: "
            + ((double) modifiedPopulationCase1.getPersons().size()
                / originalPopulationCase.getPersons().size()));
    Assert.assertEquals(
        "Selection probability was not correctly applied",
        11,
        modifiedPopulationCase1.getPersons().size(),
  ...
  public void testEveryPersonCopiedExistsInOriginal() {
    ...
    for (Person copy : modifiedPopulationCase1.getPersons().values()) {
      ...
      Assert.assertTrue(
          "Person " + copy.getId() + " does not exist in the original file", original != null);
    }
  }
  ...
  public void testNotOnlyTransferSelectedPlan() {
    ...
    for (Person copy : modifiedPopulationCase1.getPersons().values()) {
      Person original = originalPopulationCase.getPersons().get(copy.getId());
      Assert.assertEquals(
          "Not the same amount of plans", original.getPlans().size(), copy.getPlans().size());
  ...
  public void testConsiderHomeStayingAgents() {
    ...
    for (Person copy : modifiedPopulationCase2.getPersons().values()) {
      if (copy.getSelectedPlan().getPlanElements().size() == 1) atLeastOneHomeStayingPerson = true;
    }
  ...
  public void testNotConsiderHomeStayingAgents() {
    ...
    for (Person copy : modifiedPopulationCase1.getPersons().values()) {
      Assert.assertTrue(
          "No home staying agents allowed", copy.getSelectedPlan().getPlanElements().size() > 1);
    }
  }
  ...
  public void testIncludeStayHomePlans() {
    ...
    for (Person copy : modifiedPopulationCase1.getPersons().values()) {
      for (Plan plan : copy.getPlans())
        if (plan.getPlanElements().size() <= 1) atLeastOneHomeStayingPlan = true;
    }
    ...
    for (Person copy : modifiedPopulationCase2.getPersons().values()) {
      for (Plan plan : copy.getPlans())
        if (!plan.equals(copy.getSelectedPlan()))
          Assert.assertTrue("No home staying plans allowed", plan.getPlanElements().size() > 1);
    }
  }
    ...
    for (Person copy : modifiedPopulationCase2.getPersons().values()) {
      for (Plan plan : copy.getPlans())
        for (PlanElement planElement : plan.getPlanElements()) {
          if (planElement instanceof Leg) {
            Assert.assertTrue(
                "There should not be a route left", (((Leg) planElement).getRoute() == null));
          }
        }
    }
  }
  ...
  private void comparePlans(Plan original, Plan copy) {
    ...
    for (int i = 0; i < original.getPlanElements().size(); i++) {
      Assert.assertEquals(
          ...
          original.getPlanElements().get(i).toString(),
