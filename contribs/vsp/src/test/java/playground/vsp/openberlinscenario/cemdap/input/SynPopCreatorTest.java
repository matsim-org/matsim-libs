...
public class SynPopCreatorTest {
  ...
  public void TestGenerateDemand() {
    ...
    SynPopCreator demandGeneratorCensus =
        new SynPopCreator(
            ...
            defaultEmployeesToCommutersRatio);

    demandGeneratorCensus.setShapeFileForSpatialRefinement(
        utils.getInputDirectory() + "Bezirksregion_EPSG_25833.shp");
    ...
    ArrayList<String> possibleLocationsOfWork =
        readPossibleLocationsOfWork(commuterFileOutgoingTest, municipal);
    ...
    for (Person person : pop.getPersons().values()) {
      ...
      Gender gender =
          Gender.valueOf(
              (String) person.getAttributes().getAttribute("gender")); // assumes that female = 1

      // assert
      //			Assert.assertEquals("Wrong municipality", "12060034",
      // householdId.toString().substring(0,8));
      if (!employed) {
  ...
  private ArrayList<String> readPossibleLocationsOfWork(
      String commuterFileOutgoingTest, String municipal) {
