...
public class AgentId2DepartureDelayAtStopMapTest {
  ...
  public void testAgentId2DepartureDelayAtStopMap() {
    ...
    PersonDepartureEvent event3 =
        new PersonDepartureEvent(2.9 * 3600, persId1, linkId3, TransportMode.pt, TransportMode.pt);
    handler.handleEvent(event3);
    PersonDepartureEvent event4 =
        new PersonDepartureEvent(2.1 * 3600, persId2, linkId1, TransportMode.pt, TransportMode.pt);
    ...
    Assert.assertEquals(
        event3.getTime(),
        handler.getStopId2DelayAtStopMap().get(persId1).getAgentEntersVehicle().get(0),
        0);

    Assert.assertTrue(handler.getStopId2DelayAtStopMap().containsKey(persId2));
    Assert.assertEquals(
        event4.getTime(),
        handler.getStopId2DelayAtStopMap().get(persId2).getAgentEntersVehicle().get(0),
        0);
  }
}
