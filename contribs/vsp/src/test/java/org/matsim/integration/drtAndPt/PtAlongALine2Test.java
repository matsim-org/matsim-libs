...
public class PtAlongALine2Test {
  ...
  public void testPtAlongALineWithRaptorAndDrtServiceArea() {
    ...
    // * In many situations, a good solution is that drt drives to some transit stop, and from there
    // directly to the destination.  The swiss rail
    // raptor will return a cost "infinity" of such a solution, in which case the calling method
    // falls back onto transit_walk.
    // * If "walk" is defined as intermodal access, then swiss rail raptor will call the correct
    // RoutingModule, but afterwards change the mode of all
    ...
    if (drtMode == DrtMode.teleportBeeline) { // (configure teleportation router)
      config
          ...
          .addModeRoutingParams(
              new RoutingConfigGroup.TeleportedModeParams()
                  .setMode(TransportMode.drt)
                  .setTeleportedModeSpeed(100. / 3.6));
      if (drt2) {
        config
            .routing()
            .addModeRoutingParams(
    ...
    // === RAPTOR: ===
    {
      ...
      // we'll set up SwissRailRaptor ourself, so disable it here to prevent conflicts
      ...
      if (drtMode != DrtMode.none) {
        ...
        //					paramSetXxx.setMode( TransportMode.walk ); // this does not work because sbb raptor
        // treats it in a special way
        configRaptor.addIntermodalAccessEgress(
            new IntermodalAccessEgressParameterSet()
                .setMode("walk2")
                .setMaxRadius(1000000)
                .setInitialSearchRadius(1000000)
                .setSearchExtensionRadius(10000));
        // (in principle, walk as alternative to drt will not work, since drt is always faster.
        // Need to give the ASC to the router!  However, with
    ...
    double margUtlTravPt =
        config.scoring().getModes().get(TransportMode.pt).getMarginalUtilityOfTraveling();
    if (drtMode != DrtMode.none) {
      // (scoring parameters for drt modes)
      config
          .scoring()
          .addModeParams(
              new ModeParams(TransportMode.drt).setMarginalUtilityOfTraveling(margUtlTravPt));
      if (drt2) {
        config
            .scoring()
    ...
    config
        .scoring()
        .addModeParams(new ModeParams("walk2").setMarginalUtilityOfTraveling(margUtlTravPt));
    ...
    if (drtMode == DrtMode.full || drtMode == DrtMode.withPrebooking) {
      ...
      dvrpConfig.networkModes =
          ImmutableSet.copyOf(Arrays.asList(TransportMode.drt, "drt2", "drt3"));

      MultiModeDrtConfigGroup mm =
    ...
    List<String> testAgents =
        Arrays.asList("2-3", "50-51", "300-301", "550-551", "690-691", "800-801");
    for (String str : testAgents) {
      ...
      Activity shop =
          pf.createActivityFromActivityFacilityId(
              "dummy", Id.create("999-1000", ActivityFacility.class));
    ...
    PtAlongALineTest.addModeToAllLinksBtwnGivenNodes(
        scenario.getNetwork(), 0, 400, TransportMode.drt);
    if (drt2) {
    ...
    if (drt2) {
      scenario
          .getVehicles()
          .addVehicleType(
              vf.createVehicleType(Id.create("drt2", VehicleType.class))
    ...
    if (drt3) {
      scenario
          .getVehicles()
          .addVehicleType(
              vf.createVehicleType(Id.create("drt3", VehicleType.class))
                  .setMaximumVelocity(25. / 3.6));
    }
    scenario
        .getVehicles()
        .addVehicleType(
            vf.createVehicleType(Id.create(TransportMode.drt, VehicleType.class))
                .setMaximumVelocity(25. / 3.6));
    ...
    scenario
        .getVehicles()
        .addVehicleType(
            vf.createVehicleType(Id.create(TransportMode.car, VehicleType.class))
                .setMaximumVelocity(25. / 3.6));
    ...
    if (drtMode == DrtMode.full || drtMode == DrtMode.withPrebooking) {
      ...
      if (drt2 && drt3) {
        controler.configureQSimComponents(
            DvrpQSimComponents.activateModes(TransportMode.drt, "drt2", "drt3"));
      } else if (drt2) {
        controler.configureQSimComponents(
            DvrpQSimComponents.activateModes(TransportMode.drt, "drt2"));
      } else if (drt3) {
        controler.configureQSimComponents(
            DvrpQSimComponents.activateModes(TransportMode.drt, "drt3"));
      } else {
    ...
    controler.addOverridingModule(
        PtAlongALineTest.createGeneratedFleetSpecificationModule(
            TransportMode.drt, "DRT-", 30, Id.createLinkId("0-1"), 1));
    if (drt2) {
      controler.addOverridingModule(
          PtAlongALineTest.createGeneratedFleetSpecificationModule(
              "drt2", "DRT2-", 10, Id.createLinkId("999-1000"), 4));
    }
    if (drt3) {
      controler.addOverridingModule(
          PtAlongALineTest.createGeneratedFleetSpecificationModule(
              "drt3", "DRT3-", 10, Id.createLinkId("500-501"), 4));
    }
    ...
    /**
     * Case 1: The agent starts on link 2-3. While the agent is in the drt service area, s/he is so
     * close to the pt stop that it doesn't make sense for him/her to use drt as an intermodal
     * access to get the pt. Therefore the agent will just walk to the pt stop.
     */
    List<PlanElement> planFullCase1 =
        controler
            .getScenario()
            .getPopulation()
            .getPersons()
            ...
            .getSelectedPlan()
            .getPlanElements();
    List<Leg> planLegCase1 =
        planFullCase1.stream()
    ...
    /**
     * Case 2a: Agent starts at Link "50-51". This is within the drt service area. Agent is expected
     * use drt as intermodal access to the left pt stop. The plan should be walk -> drt -> walk ->
     * pt -> walk
     */
    List<PlanElement> planFullCase2a =
        controler
            .getScenario()
            .getPopulation()
            .getPersons()
            ...
            .getSelectedPlan()
            .getPlanElements();
    List<Leg> planLegCase2a =
        planFullCase2a.stream()
            .filter(pe -> pe instanceof Leg)
            .map(pe -> (Leg) pe)
            .collect(toList());
    ...
    /**
     * Case 2b: Agent starts at Link "300-301". This is within the drt service area. Agent is
     * expected use drt as intermodal access to the middle pt stop. The plan should be walk -> drt
     * -> walk -> pt -> walk
     */
    List<PlanElement> planFullCase2b =
        controler
            .getScenario()
            .getPopulation()
            .getPersons()
            ...
            .getSelectedPlan()
            .getPlanElements();
    List<Leg> planLegCase2b =
        planFullCase2b.stream()
            .filter(pe -> pe instanceof Leg)
            .map(pe -> (Leg) pe)
            .collect(toList());
    ...
    /**
     * Case 2c: Agent starts at Link "550-551". This is within the drt3 service area. Agent is
     * expected use drt3 as intermodal access to the middle pt stop. The plan should be walk -> drt3
     * -> walk -> pt -> walk
     */
    List<PlanElement> planFullCase2c =
        controler
            .getScenario()
            .getPopulation()
            .getPersons()
            ...
            .getSelectedPlan()
            .getPlanElements();
    List<Leg> planLegCase2c =
        planFullCase2c.stream()
            .filter(pe -> pe instanceof Leg)
            .map(pe -> (Leg) pe)
            .collect(toList());
    ...
    /**
     * Case 3a: Agent starts at Link "690-691". This is not within any drt service areas. Agent is
     * expected to use transit_walk to get to his/her destination.
     */
    List<PlanElement> planFullCase3a =
        controler
            .getScenario()
            .getPopulation()
            .getPersons()
            ...
            .getSelectedPlan()
            .getPlanElements();
    List<Leg> planLegCase3a =
        planFullCase3a.stream()
            .filter(pe -> pe instanceof Leg)
            .map(pe -> (Leg) pe)
            .collect(toList());
    ...
    /**
     * Case 3b: Agent starts at Link "800-801". This is within the drt2 service area. Agent is NOT
     * expected to utilize drt2, since s/he is on a pt trip, and can only use drt as access/egress
     * to a pt stop. Therefore, the agent is agent is expected to use transit_walk to get to his/her
     * destination.
     */
    List<PlanElement> planFullCase3b =
        controler
            .getScenario()
            .getPopulation()
            .getPersons()
            ...
            .getSelectedPlan()
            .getPlanElements();
    List<Leg> planLegCase3b =
        planFullCase3b.stream()
            .filter(pe -> pe instanceof Leg)
            .map(pe -> (Leg) pe)
            .collect(toList());
  ...
  public void intermodalAccessEgressPicksWrongVariant() {
    ...
    //            // allow transfering from the initial stop to another one if we have intermodal
    // access,
    //            // as not all stops might be intermodal
    //            handleTransfers(true, parameters);
    //        }
    // makes it pass.  I have no idea why, or if this would be a good direction to go for a fix.
    // kai, jul'19

    // does now work with these lines of code in SwissRailRaptorCore (which solve problems in other
    // tests) gleich, aug'19
    ...
    config
        .routing()
        .addModeRoutingParams(
            new RoutingConfigGroup.TeleportedModeParams("walk2").setTeleportedModeSpeed(5. / 3.6));

    config
        .routing()
        .addModeRoutingParams(
            new RoutingConfigGroup.TeleportedModeParams(TransportMode.walk)
                .setTeleportedModeSpeed(0.));
    // (when specifying "walk2", all default routing params are cleared.  However, swiss rail raptor
    // needs "walk" to function. kai, feb'20)
    ...
    // === RAPTOR: ===
    {
      ...
      configRaptor.addIntermodalAccessEgress(
          new IntermodalAccessEgressParameterSet()
              ...
              .setInitialSearchRadius(1000000)
              .setSearchExtensionRadius(10000));
      // (in principle, walk as alternative to drt will not work, since drt is always faster.  Need
      // to give the ASC to the router!  However, with
      // the reduced drt network we should be able to see differentiation.)

      // when we constructed this test, this did not work with "walk" since sbb raptor treats the
      // walk mode in some special way.
    ...
    scenario
        ...
        .getPersons()
        .values()
        .removeIf(person -> !person.getId().toString().equals("3"));
  ...
  public void networkWalkDoesNotWorkWithRaptor() {
    ...
    PtAlongALineTest.addModeToAllLinksBtwnGivenNodes(
        scenario.getNetwork(), 0, 1000, TransportMode.walk);
