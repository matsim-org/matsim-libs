 ...
 *
 * <p>It checks whether the person specific income and mode constant (asc) is read from the person
 * attributes. The marginalUtilityOfMoney should be calculated as averageIncome/personSpecificIncome
 * and not taken from the subpopulation-specific scoring params. To check whether the remaining
 * scoring params are subpopulation-specific, this class tests the person's
 * marginalUtilityOfWaitingPt_s accordingly.
 */
public class PersonScoringParametersFromPersonAttributesTest {
  ...
  public void setUp() {
    ...
    ScoringConfigGroup.ScoringParameterSet personParams =
    ...
    ScoringConfigGroup.ModeParams modeParamsCar =
        new ScoringConfigGroup.ModeParams(TransportMode.car);
    ...
    ScoringConfigGroup.ModeParams modeParamsBike =
        new ScoringConfigGroup.ModeParams(TransportMode.bike);
    ...
    ScoringConfigGroup.ScoringParameterSet freightParams =
    ...
    { // fill population
      ...
      Person mediumIncomeHighCarAsc =
          factory.createPerson(Id.createPersonId("mediumIncomeHighCarAsc"));
    ...
    personScoringParams =
        new PersonScoringParametersFromPersonAttributes(
  ...
  public void testPersonWithLowIncomeLowCarAsc() {
    ...
    Assert.assertEquals(
        -0.1d, params.modeParams.get(TransportMode.car).constant, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        -0.001d,
  ...
  public void testPersonWithMediumIncomeHighCarAsc() {
    ...
    makeAssertMarginalUtilityOfMoneyAndPtWait(params, 1d, 0.5d);
    Assert.assertEquals(
        -2.1d, params.modeParams.get(TransportMode.car).constant, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        -50.0d, params.modeParams.get(TransportMode.bike).constant, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        -0.001d,
        params.modeParams.get(TransportMode.car).marginalUtilityOfTraveling_s,
        MatsimTestUtils.EPSILON);
  ...
  public void testPersonFreight() {
    ...
    ScoringParameters params =
        personScoringParams.getScoringParameters(population.getPersons().get(id));
    // freight agent has no income attribute set, so it should use the marginal utility of money
    // that is set in its subpopulation scoring parameters!
  ...
  public void testFreightWithIncome() {
    ...
    ScoringParameters params2 =
        personScoringParams.getScoringParameters(population.getPersons().get(id2));
  ...
  public void testMoneyScore() {
    ScoringParameters paramsRich =
        personScoringParams.getScoringParameters(
            population.getPersons().get(Id.createPersonId("highIncomeLowCarAsc")));
    CharyparNagelMoneyScoring moneyScoringRich = new CharyparNagelMoneyScoring(paramsRich);
    moneyScoringRich.addMoney(100);
    Assert.assertEquals(
        "for the rich person, 100 money units should be equal to a score of 66.66",
        1. / 1.5 * 100,
    ...
    ScoringParameters paramsPoor =
        personScoringParams.getScoringParameters(
            population.getPersons().get(Id.createPersonId("lowIncomeLowCarAsc")));
    CharyparNagelMoneyScoring moneyScoringPoor = new CharyparNagelMoneyScoring(paramsPoor);
    moneyScoringPoor.addMoney(100);
    Assert.assertEquals(
        "for the poor person, 100 money units should be equal to a score of 200.00",
        1. / 0.5 * 100,
  ...
  public void testPersonSpecificAscScoring() {
    ScoringParameters paramsRich =
        personScoringParams.getScoringParameters(
            population.getPersons().get(Id.createPersonId("highIncomeLowCarAsc")));
    CharyparNagelLegScoring legScoringRichCarLeg =
        new CharyparNagelLegScoring(
            paramsRich, NetworkUtils.createNetwork(), Set.of(TransportMode.pt));
    ...
    Assert.assertEquals(
        "for the rich person with low car asc, a 0 meter and 10s car trip should be equal to a score of ",
        -0.1d - 0.001d * 10 - 7.5 * 1. / 1.5 - 0.3,
        legScoringRichCarLeg.getScore(),
        MatsimTestUtils.EPSILON);

    ScoringParameters paramsMediumIncomeHighCarAsc =
        personScoringParams.getScoringParameters(
            population.getPersons().get(Id.createPersonId("mediumIncomeHighCarAsc")));
    CharyparNagelLegScoring legScoringMediumIncomeHighCarAsc =
        new CharyparNagelLegScoring(
            paramsMediumIncomeHighCarAsc, NetworkUtils.createNetwork(), Set.of(TransportMode.pt));
    legScoringMediumIncomeHighCarAsc.handleLeg(carLegZeroDistanceTenSeconds);
    Assert.assertEquals(
        "for the medium person with high car asc, a 0 meter and 10s car trip should be equal to a score of ",
        -2.1d - 0.001d * 10 - 7.5 * 1. / 1.0 - 0.3,
        legScoringMediumIncomeHighCarAsc.getScore(),
        MatsimTestUtils.EPSILON);

    // bike has no person specific asc for high income person and is not affected
    CharyparNagelLegScoring legScoringRichBikeLeg =
        new CharyparNagelLegScoring(
            paramsRich, NetworkUtils.createNetwork(), Set.of(TransportMode.pt));
    ...
    Assert.assertEquals(
        "for the rich person with low car asc, a 0 meter and 0s bike trip should be equal to a score of ",
        -0.55d,
        legScoringRichBikeLeg.getScore(),
        MatsimTestUtils.EPSILON);

    // bike has a person specific asc for the medium income person
    CharyparNagelLegScoring legScoringMediumIncomeBikeLeg =
        new CharyparNagelLegScoring(
            paramsMediumIncomeHighCarAsc, NetworkUtils.createNetwork(), Set.of(TransportMode.pt));
    legScoringMediumIncomeBikeLeg.handleLeg(bikeLegZeroDistanceZeroSeconds);
    Assert.assertEquals(
        "for the medium income person with high car asc, a 0 meter and 0s bike trip should be equal to a score of ",
        -50.0d,
        legScoringMediumIncomeBikeLeg.getScore(),
        MatsimTestUtils.EPSILON);
  }
  ...
  private static Leg createLeg(String mode, double distance, double travelTime) {
    ...
    Route carRouteZeroDistance =
        RouteUtils.createGenericRouteImpl(
            Id.createLinkId("dummyStart"), Id.createLinkId("dummyEnd"));
  ...
  private void makeAssertMarginalUtilityOfMoneyAndPtWait(
      ScoringParameters params, double income, double marginalUtilityOfWaitingPt_s) {
    Assert.assertEquals(
        "marginalUtilityOfMoney is wrong", 1 / income, params.marginalUtilityOfMoney, 0.);
    Assert.assertEquals(
