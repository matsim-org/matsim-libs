...
public class RunFreightAnalysisWithShipmentTest {
  ...
  public void runShipmentTrackerTest() {
    ...
    MyShipmentTrackerEventHandler eventHandler =
        new MyShipmentTrackerEventHandler(vehicles, network, carriers);
    ...
    LinkedHashMap<Id<CarrierShipment>, ShipmentTracker> shipments =
        eventHandler.getShipmentTracker().getShipments();
    ...
    while (shipmentKeys.hasNext()) {
      ...
      double dist =
          NetworkUtils.getEuclideanDistance(
              network.getLinks().get(fromLinkId).getCoord(),
              network.getLinks().get(toLinkId).getCoord());
      beelineDistance.put(shipmentId, dist);
      System.out.println("from " + fromLinkId + " to " + toLinkId + " distance is " + dist);
    }

    Assert.assertEquals(
        "Beeline distance is not as expected for shipment 1",
        Double.valueOf(10816.653826391968),
        beelineDistance.get(Id.create("1", CarrierShipment.class)));
    Assert.assertEquals(
        "Beeline distance is not as expected for shipment 11",
        Double.valueOf(6000.0),
        beelineDistance.get(Id.create("11", CarrierShipment.class)));
    Assert.assertEquals(
        "Beeline distance is not as expected for shipment 12",
        Double.valueOf(7106.335201775948),
        beelineDistance.get(Id.create("12", CarrierShipment.class)));
    Assert.assertEquals(
        "Beeline distance is not as expected for shipment 13",
        Double.valueOf(4123.105625617661),
        beelineDistance.get(Id.create("13", CarrierShipment.class)));
    Assert.assertEquals(
        "Beeline distance is not as expected for shipment 14",
        Double.valueOf(9055.385138137417),
        beelineDistance.get(Id.create("14", CarrierShipment.class)));
    Assert.assertEquals(
        "Beeline distance is not as expected for shipment 15",
        Double.valueOf(6964.19413859206),
        beelineDistance.get(Id.create("15", CarrierShipment.class)));
    Assert.assertEquals(
        "Beeline distance is not as expected for shipment 16",
        Double.valueOf(9192.388155425118),
        beelineDistance.get(Id.create("16", CarrierShipment.class)));
  }
}
