...
public final class DSim implements Netsim {
	...
	public void run() {
		...
		double time = config.qsim().getStartTime().orElse(0);
		...
		while (time < config.qsim().getEndTime().orElse(86400)) {
		...
		double rtr = config.qsim().getEndTime().orElse(86400) * 1000 / runtime;
