...
/**
 * Add this module if you want to simulate signals. It does not work without signals. By default, it
 * works with without signals and with signal implementations of fixed-time signals,
 * traffic-actuated signals called SYLVIA and traffic-adaptive signals based on Laemmer. If you want
 * to add other signal controllers, you can add a respective factory by calling the method
 * addSignalControllerFactory. It is also possible to use different control schemes in one scenario
 * at different intersections (i.e. signal systems).
 *
 * @author tthunig
 */
class SignalsModule extends AbstractModule {
  // This is no longer public since there is now also material that needs to be injected at the QSim
  // level (see
  // Signals.configure(...)), and making SignalsModule nonpublic seems the best way of forcibly
  // notifying users.  kai, nov'18
  ...
  private Map<String, Class<? extends SignalControllerFactory>> signalControllerFactoryClassNames =
      new HashMap<>();

  SignalsModule() {
    // specify default signal controller. you can add your own by calling addSignalControllerFactory
    // (see method java-doc below)
    signalControllerFactoryClassNames.put(
        DefaultPlanbasedSignalSystemController.IDENTIFIER,
        DefaultPlanbasedSignalSystemController.FixedTimeFactory.class);
    signalControllerFactoryClassNames.put(
        SylviaSignalController.IDENTIFIER, SylviaSignalController.SylviaFactory.class);
    signalControllerFactoryClassNames.put(
        LaemmerSignalController.IDENTIFIER, LaemmerSignalController.LaemmerFactory.class);
  }
  ...
  public void install() {
    ...
    log.warn(
        "setting travelTimeCalculatur.setSeparateModes to false since otherwise link2link routing does not work");
    // yyyy move to individual config files???  kai, feb'19

    this.signalControllerFactoryMultibinder =
        MapBinder.newMapBinder(
            binder(), new TypeLiteral<String>() {}, new TypeLiteral<SignalControllerFactory>() {});

    if ((boolean)
        ConfigUtils.addOrGetModule(
                getConfig(), SignalSystemsConfigGroup.GROUP_NAME, SignalSystemsConfigGroup.class)
            .isUseSignalSystems()) {
      ...
      for (String identifier : signalControllerFactoryClassNames.keySet()) {
        ...
        signalControllerFactoryMultibinder
            .addBinding(identifier)
            .to(signalControllerFactoryClassNames.get(identifier));
      }
  ...
  /**
   * Call this method when you want to add your own SignalController. E.g. via
   ...
   */
  final void addSignalControllerFactory(
      String key, Class<? extends SignalControllerFactory> signalControllerFactoryClassName) {
