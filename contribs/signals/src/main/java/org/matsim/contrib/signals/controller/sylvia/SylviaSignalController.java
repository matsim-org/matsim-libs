 ...
 */
public final class SylviaSignalController extends AbstractSignalController
    implements SignalController {

  private static final Logger log = LogManager.getLogger(SylviaSignalController.class);

  public static final String IDENTIFIER = "SylviaSignalControl";

  private static int sylviaPlanDumpCount = 0;

  public static final class SylviaFactory implements SignalControllerFactory {
    ...
    public SignalController createSignalSystemController(SignalSystem signalSystem) {
      SignalController controller =
          new SylviaSignalController(scenario, sensorManager, downstreamSensor);
  ...
  private SylviaSignalController(
      Scenario scenario, LinkSensorManager sensorManager, DownstreamSensor downstreamSensor) {
  ...
  /**
   * Decide how to control the signals in this second. Possibilities: (1) continue with the sylvia
   * base plan. (2) extend a signal phase when more vehicles are arriving. (3) extend the last
   * signal phase when fixed time cycle time is not reached yet (forced extension).
   */
  @Override
  public void updateState(double currentTime) {
    ...
    if (this.forcedExtensionActive) {
      ...
      // TODO what happens when first sim sim event is not at second 0? it does not extend the phase
      // then?! theresa apr'17
    ...
    if (this.secondInSylviaCycle == this.activeSylviaPlan.getCycleTime() - 1) {
      // the base plan cycle including all extensions is processed. init data structure for the next
      // cycle.
      this.initCycle();
    }
  }
  ...
  /**
   * Checks whether there is time left to extend phases. If the fixed time cycle time is used as
   * maximal extension time, this method checks whether it is already reached. If not, extension is
   * always allowed.
   */
   ...
   *
   * @return false if maximal green time is reached or if signal has not been switched on (first
   *     second of the simulation); true if signal can be extended based on the number of current
   *     green time seconds.
   */
  ...
  /**
   * Checks whether the phase should be forced to extend. This is the case if there is cycle time
   * left.
   *
  ...
  /**
   * Checks 1. whether there is green time left to extend the signal groups green phase and, if yes,
   * 2. whether there are cars arriving, i.e. there is need to extend the signal groups green time.
   *
   * @param currentTime
   * @param extensionPoint
   * @return true, if the signal should be extended. false, if not, i.e. if there is no time left to
   *     extend the signal or if there is no need to extend the signal.
   */
  private boolean checkExtensionCondition(double currentTime, SylviaExtensionPoint extensionPoint) {
    if (this.isExtensionTimeLeft()) {
      ...
      for (Id<SignalGroup> signalGroupId : extensionPoint.getSignalGroupIds()) {
        if (!this.isGreenTimeLeft(
            currentTime, signalGroupId, extensionPoint.getMaxGreenTime(signalGroupId))) {
          return false;
        }
      }
   ...
   * @param currentTime
   * @param extensionPoint
   * @return true, if there is a car at an incoming lane (or, if no lanes are used, when there is a
   *     car on an incoming link within some distance (specified in sylviaConfig - default is 10))
  ...
  private boolean checkTrafficConditions(double currentTime, SylviaExtensionPoint extensionPoint) {
    if (sylviaConfig.isCheckDownstream()) {
      ...
      for (Id<SignalGroup> greenGroup : extensionPoint.getSignalGroupIds()) {
        if (!downstreamSensor.allDownstreamLinksEmpty(system.getId(), greenGroup)) return false;
      }
    }
      ...
      if (signal.getLaneIds() == null || signal.getLaneIds().isEmpty()) {
        ...
        int noCars =
            this.sensorManager.getNumberOfCarsInDistance(
                signal.getLinkId(), this.sylviaConfig.getSensorDistanceMeter(), currentTime);
        if (noCars > 0) return true;
      } else {
        // link has lanes
        for (Id<Lane> laneId : signal.getLaneIds()) {
          int noCars =
              this.sensorManager.getNumberOfCarsInDistanceOnLane(
                  signal.getLinkId(),
                  laneId,
                  this.sylviaConfig.getSensorDistanceMeter(),
                  currentTime);
          if (noCars > 0) return true;
        }
      }
  ...
  /**
   * Is called when mobsim is initialized. The parameter simStartTimeSeconds is not used.
   *
   * <p>This method initializes all elements needed for the sylvia controller: it collects relevant
   * (sylvia and fixed time) signal plans, determines possible extension points (in time) and
   * prepares sensors (event handlers) to count vehicles in front of signals.
   */
  @Override
  ...
  /**
   * Collects sylvia and fixed time signal plans of this signal system from the signal plan
   * container.
   *
  ...
  private Tuple<SignalPlan, SylviaSignalPlan> searchActivePlans() {
    ...
    if (this.signalPlans.size() > 2) {
      throw new IllegalStateException(
          "The current implementation of Sylvia does not work with multiple signal plans for different times of the day.");
    }
    ...
    if (sylviaPlan == null && fixedTimePlan == null) {
      throw new IllegalStateException(
          "No suitable plans found for controller of signal system: " + this.system.getId());
    }
    return new Tuple<SignalPlan, SylviaSignalPlan>(fixedTimePlan, sylviaPlan);
  }

  /**
   * This method determines points in time when extension is possible (extension points). It
   * calculates maximal green times based on maximal cycle time and maximal green time scale. The
   * last extension point in a cycle is defined as a forced extension point.
   *
   * @param plans
   */
  private void calculateExtensionPoints(Tuple<SignalPlan, SylviaSignalPlan> plans) {
    ...
    for (SignalGroupSettingsData settings :
        sylvia.getPlanData().getSignalGroupSettingsDataByGroupId().values()) {
      // set the extension moment to the second of the dropping (but respecting offset and cycle
      // time)
      ...
      SignalGroupSettingsData fixedTimeSettings =
          ((DatabasedSignalPlan) fixedTime)
              .getPlanData()
              .getSignalGroupSettingsDataByGroupId()
              .get(settings.getSignalGroupId());
      int fixedTimeGreen =
          SignalUtils.calculateGreenTimeSeconds(fixedTimeSettings, fixedTime.getCycleTime());
      ...
      //			// comment this out because isExtensionTimeLeft() already checks whether cycle time is
      // exceeded. tt, oct'16
    ...
    if (this.sylviaConfig.isUseFixedTimeCycleAsMaximalExtension()) {
      // convert last extension point in cycle into a forced extension point (has to be extended
      // until fixed time cycle time is reached).
      forcedExtensionPointMap.put(
          lastExtensionMoment, extensionPointMap.remove(lastExtensionMoment));
    }
  }
  ...
  private void dumpSylviaPlan() {
    log.debug(
        "Signal System: "
            + this.system.getId()
            + " Plan: "
            + this.activeSylviaPlan.getPlanData().getId());
   ...
   * to count vehicles in a specific distance (when no lanes are used) or on the last lane (if lanes
   * are used).
   *
   * <p>Prepare also downstream sensors if checkDownstream is enabled in sylviaConfig
   */
  private void initializeSensoring() {
    for (SylviaExtensionPoint extPoint : this.extensionPointMap.values()) {
      ...
      for (Signal signal : extPointSignals) {
        if (signal.getLaneIds() == null || signal.getLaneIds().isEmpty()) {
          this.sensorManager.registerNumberOfCarsInDistanceMonitoring(
              signal.getLinkId(), this.sylviaConfig.getSensorDistanceMeter());
        } else {
          for (Id<Lane> laneId : signal.getLaneIds()) {
            this.sensorManager.registerNumberOfCarsOnLaneInDistanceMonitoring(
                signal.getLinkId(), laneId, this.sylviaConfig.getSensorDistanceMeter());
          }
        }
      }
    }
