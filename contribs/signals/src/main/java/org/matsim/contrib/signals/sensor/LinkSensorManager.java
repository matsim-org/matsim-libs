 ...
 */
@Singleton
public final class LinkSensorManager
    ...
    if (!this.linkIdSensorMap.containsKey(linkId)) {
      ...
      if (link == null) {
        throw new IllegalStateException(
            "Link with Id " + linkId + " is not in the network, can't register sensor");
      }
  ...
  public void registerNumberOfCarsOnLaneInDistanceMonitoring(
      Id<Link> linkId, Id<Lane> laneId, Double distanceMeter) {
    Link link = this.network.getLinks().get(linkId);
    if (link == null) {
      throw new IllegalStateException(
          "Link with Id " + linkId + " is not in the network, can't register sensor");
    ...
    if (this.laneDefinitions == null
        || this.laneDefinitions.getLanesToLinkAssignments().get(linkId) == null
        || this.laneDefinitions.getLanesToLinkAssignments().get(linkId).getLanes().get(laneId)
            == null) {
      throw new IllegalStateException(
          "No data found for lane  "
              + laneId
              + " on link  "
              + linkId
              + " is not in the network, can't register sensor");
    }
  ...
  public void registerAverageNumberOfCarsPerSecondMonitoringOnLane(
      Id<Link> linkId, Id<Lane> laneId) {
    registerAverageNumberOfCarsPerSecondMonitoringOnLane(
        linkId, laneId, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
  }

  public void registerAverageNumberOfCarsPerSecondMonitoringOnLane(
      Id<Link> linkId, Id<Lane> laneId, double lookBackTime, double timeBucketCollectionDuration) {
    Link link = this.network.getLinks().get(linkId);
    ...
    linkIdLaneIdSensorMap
        .get(linkId)
        .get(laneId)
        .registerAverageVehiclesPerSecondToMonitor(lookBackTime, timeBucketCollectionDuration);
  }

  public void registerAverageNumberOfCarsPerSecondMonitoring(Id<Link> linkId) {
    registerAverageNumberOfCarsPerSecondMonitoring(
        linkId, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
  }

  public void registerAverageNumberOfCarsPerSecondMonitoring(
      Id<Link> linkId, double lookBackTime, double timeBucketCollectionDuration) {
    if (!this.linkIdSensorMap.containsKey(linkId)) {
      Link link = this.network.getLinks().get(linkId);
      if (link == null) {
        throw new IllegalStateException(
  ...
  public int getNumberOfCarsOnLink(Id<Link> linkId) {
    if (!this.linkIdSensorMap.containsKey(linkId)) {
      throw new IllegalStateException(
          "No sensor on link "
              + linkId
              + "! Register measurement for this link by calling one of the 'register...' methods of this class first.");
    }
    ...
    if (map == null || !map.containsKey(laneId)) {
      throw new IllegalStateException(
          "No sensor on lane "
              ...
              + " of link "
              + linkId
              + "! Register measurement for this link lane pair!");
    }
  ...
  public int getNumberOfCarsInDistanceOnLane(
      Id<Link> linkId, Id<Lane> laneId, Double distanceMeter, double timeSeconds) {
    if (!this.linkIdLaneIdSensorMap.containsKey(linkId)) {
      throw new IllegalStateException(
          "No sensor on link "
              + linkId
