...
/**
 * Class to calculate the intergreen times from a (fixed) signal control plan and write them into a
 * xml file. Intergreens are necessary to create and check (adaptive) signal control plans.
 *
 * @author tthunig
 */
public final class CalculateSimplifiedIntergreens {
  ...
  public void calculateIntergreens(
      String signalControlInputFile, String intergreensOutputFile, boolean simplifyPhases) {
    log.info("Calculate intergreens of the signal control from file " + signalControlInputFile);
    if (simplifyPhases)
      log.info(
          "Simplify signal phases: a phase ends with the last setting starting at the phase starting time, "
    ...
    for (SignalSystemControllerData signalSystemData :
        signalControl.getSignalSystemControllerDataBySystemId().values()) {

      // create a container for the intergreens of this signal system
      IntergreensForSignalSystemData systemIntergreens =
          new IntergreensForSignalSystemDataImpl(signalSystemData.getSignalSystemId());
      ...
      log.info(
          "intergreen calculation for signal system "
  ...
  private void preprocessSystemsSignalGroups(
    ...
    if (simplifyPhases) {
      Map<Integer, List<SignalGroupSettingsData>> droppingSettingsMap =
          this.createSameDroppingMapping();
      this.createSimplifiedPhases(sortedGroupSettingsList, droppingSettingsMap);
    } else {
      Map<Id<SignalGroup>, Set<SignalGroupSettingsData>> overlappingSettingsMap =
          this.createOverlappingSettingsMap();
    ...
    for (SignalGroupSettingsData selectedSignal : groupSettingsList) {
      ...
      for (SignalGroupSettingsData comparativeSignal : groupSettingsList) {
        ...
        if (comparativeSignal.getOnset() < selectedSignal.getDropping()
            && comparativeSignal.getDropping() > selectedSignal.getOnset()) {
  ...
  private void createPhases(
    ...
    for (SignalGroupSettingsData selectedSignal : sortedGroupSettingsList) {
      ...
      do { // while phase dropping changes
        ...
        lastDroppingSignal =
            extendPhase(lastDroppingSignal, overlappingSettingsMap, phaseSignals, handledSignals);
  ...
  private SignalGroupSettingsData extendPhase(
    ...
    for (SignalGroupSettingsData overlappingSignal :
        overlappingSettingsMap.get(lastDroppingSignal.getSignalGroupId())) {
  ...
  private void handleSamePhase(
      IntergreensForSignalSystemData systemIntergreens, SignalGroupSettingsData selectedSignal) {

    // calculate intergreens to later signals in the same phase
    FixedTimeSignalPhase signalPhase =
        signalGroupIdToPhaseMapping.get(selectedSignal.getSignalGroupId());
    for (SignalGroupSettingsData samePhaseSignals :
        signalPhase.getSignalGroupSettingsByGroupId().values()) {
      if (samePhaseSignals.getOnset() >= selectedSignal.getDropping()) {
        systemIntergreens.setIntergreenTime(
            samePhaseSignals.getOnset() - selectedSignal.getDropping(),
  ...
  private void handleNextPhase(
    ...
    FixedTimeSignalPhase selectedPhase =
        this.signalGroupIdToPhaseMapping.get(selectedSignal.getSignalGroupId());
    ...
    for (SignalGroupSettingsData nextPhaseSignals :
        nextPhase.getSignalGroupSettingsByGroupId().values()) {
      ...
      systemIntergreens.setIntergreenTime(
          intergreen, selectedSignal.getSignalGroupId(), nextPhaseSignals.getSignalGroupId());
    }
  }
   ...
   *
   * <p>Intergreen times are only calculated to signals in the next phase. So different definitions
   * of phases treat in different numbers of calculated intergreens.
   *
   * <p>A phase is a time period of a (fixed time) signal control, where the end is defined by the
   * dropping of the largest setting starting at the phase starting time. So different such phases
   * may overlap each other. If you choose "simplifyPhases" as true, overlapping phases will be
   * simplified in the sense that they get zero intergreen times.
   *
   * <p>If you choose "simplifyPhases" as false, all these simplified phases will be handled as one
   * phase. They will get no intergreen times because they are allowed to show green at the same
   * time. In this case you will get more intergreen times, because all intergreens to signals in
   * the next (now bigger) phase will be calculated.
   */
  public static void main(String[] args) {

    if (args == null || args.length < 3) {
      throw new UnsupportedOperationException(
          "Please call this class with three arguments: 1. path to the signal control file; "
    ...
    if (simplifyPhases) intergreensOutFile += "signal_control_simplifiedIntergreens.xml";
    else intergreensOutFile += "signal_control_unsimplifiedIntergreens.xml";

    new CalculateSimplifiedIntergreens()
        .calculateIntergreens(signalControlFile, intergreensOutFile, simplifyPhases);
  }
}
