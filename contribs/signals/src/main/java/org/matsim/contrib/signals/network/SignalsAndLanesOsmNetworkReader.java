...
/**
 * Osm reader that extends the basic OSMNetworkReader and in addition reads in signals and lanes
 * information. This tool is based on the master thesis of Nils Schirrmacher from 2017 at VSP and
 * intensively debugged, simplified and cleaned up by Soehnke Braun and Theresa Ziemke (Thunig)
 *
 * @author tthunig, nschirrmacher, sbraun
 */
public class SignalsAndLanesOsmNetworkReader extends OsmNetworkReader {

  private static final Logger LOG = LogManager.getLogger(SignalsAndLanesOsmNetworkReader.class);

  private static final String TAG_RESTRICTION = "restriction";
  private static final String TAG_TURNLANES = "turn:lanes";
  private static final String TAG_TURNLANESFORW = "turn:lanes:forward";
  private static final String TAG_TURNLANESBACK = "turn:lanes:backward";

  private static final int DEFAULT_LANE_OFFSET = 35;
  private static final int INTERGREENTIME = 5;
  private static final int MIN_GREENTIME = 10;
  private static final double SIGNAL_MERGE_DISTANCE = 30; // changed from 40m sb 25.03.2020
  private final double MERGE_DISTANCE_JUNCTIONS = 30;

  private static final double SIGNAL_LANES_CAPACITY = 2000.0;
  private static final double THROUGHLINK_ANGLE_TOLERANCE = 0.1666667;
  private static final double DEGREE_TOLERANCE_ROUNDABOUTS = 1.;
  private static final int PEDESTRIAN_CROSSING_TIME = 20;
  private static final int CYCLE_TIME = 90;
  private final int minimalTimeForPair = 2 * INTERGREENTIME + 2 * MIN_GREENTIME;

  private static final String ORIG_ID = "origid";
  private static final String TYPE = "type";
  private static final String TO_LINKS_ANGLES = "toLinksAngles";
  ...
  public SignalsAndLanesOsmNetworkReader(
      Network network,
  ...
  public SignalsAndLanesOsmNetworkReader(
      Network network,
      CoordinateTransformation transformation,
      boolean useHighwayDefaults,
      final SignalsData signalsData,
      final Lanes lanes) {
    super(network, transformation, useHighwayDefaults);

    super.addWayTags(
        Arrays.asList(TAG_TURNLANES, TAG_TURNLANESFORW, TAG_TURNLANESBACK, TAG_RESTRICTION));
  ...
  private void stats() {
    LOG.info(
        "MATSim: # links with lanes created: " + this.lanes.getLanesToLinkAssignments().size());
    ...
    if (north > south && east > west) {
      ...
      if (!(north > south)) {
        throw new RuntimeException(
            "Double value of north coordinate should be larger than south value but was: N:"
                + north
                + " <= S:"
                + south);
      } else {
        throw new RuntimeException(
            "Double value of east coordinate should be larger than west value but was: E:"
                + east
                + " <= W:"
                + west);
      }
  ...
  /**
   * Extends the super class method: Signal data (which is often located at upstream nodes) is
   * pushed into intersection nodes, such that signalized intersections can more easily be
   * simplified during network simplification
   */
  @Override
  protected void preprocessOsmData() {
    super.preprocessOsmData();
    // The logic to merge small roundabouts is depreciated as the original reader is doing something
    // similar anyway.
    ...
    // BoundingBox validation check - check if all if the BoundingBox is set within the range of
    // available Nodes:
    if (this.bbox != null) {
      ...
      if (!(minX <= this.bbox.west)
          || !(maxX >= this.bbox.east)
          || !(minY <= this.bbox.south)
          || !(maxY >= this.bbox.north)) {
        throw new RuntimeException(
            "The bounding box is not within the range of parsed coordinates. Please verify the box correspond to OSM data.");
      }
  ...
  /**
   * Extends the super class method: Intersections with more than two ways (i.e. more than one
   * intersection node) are simplified into one intersection node.
   */
  @Override
  ...
  protected void createMatsimData() {
    ...
    // lanes were already created (via setOrModifyLinkAttributes()) but without toLinks. add toLinks
    // now:
      ...
      if (link.getToNode().getOutLinks().size() >= 1) {
        if (link.getNumberOfLanes() > 1) {
          ...
          if (restrictions != null
              ...
              && (this.bbox == null || this.bbox.contains(nodes.get(toNodeId).coord))) {
    ...
    for (Link link : network.getLinks().values()) {
      ...
      Id<SignalSystem> systemId =
          Id.create("System" + link.getToNode().getId(), SignalSystem.class);
      if (this.systems.getSignalSystemData().containsKey(systemId)) {
        if (lanes.getLanesToLinkAssignments().containsKey(link.getId())) {
          for (Lane lane :
              lanes.getLanesToLinkAssignments().get(link.getId()).getLanes().values()) {
            ...
            if (!end.equals("ol")) {
              SignalData signal =
                  this.systems
                      .getFactory()
                      .createSignalData(
                          Id.create("Signal" + link.getId() + "." + end, Signal.class));
    ...
    for (Node node : network.getNodes().values()) {
      Id<SignalSystem> systemId =
          Id.create("System" + Long.valueOf(node.getId().toString()), SignalSystem.class);
      if (this.systems.getSignalSystemData().containsKey(systemId)) {
        ...
        if (node.getInLinks().size() == 2) {
          ...
          if (this.systems.getSignalSystemData().containsKey(systemId)) setInLinksCapacities(node);
        }
        ...
        if (node.getInLinks().size() == 3) {
          ...
          for (int i = 0; i < inLinks.size(); i++) {
            if (!inLinks.get(i).equals(pair.getFirst())
                && !inLinks.get(i).equals(pair.getSecond())) {
        ...
        if (node.getInLinks().size() == 4) {
          ...
          for (LinkVector inLink : inLinks) {
            if (first == null) {
              if (!inLink.equals(firstPair.getFirst()) && !inLink.equals(firstPair.getSecond())) {
                first = inLink;
              }
            } else {
              if (!inLink.equals(firstPair.getFirst()) && !inLink.equals(firstPair.getSecond())) {
          ...
          Tuple<LinkVector, LinkVector> secondPair =
              new Tuple<LinkVector, LinkVector>(first, second);
        ...
        if (node.getInLinks().size() > 4) {
          if (this.acceptFourPlusCrossings) {
            createPlansForOneWayJunction(signalSystem, node);
            LOG.warn(
                "Signal system with more than four in-links detected @ Node "
                    + node.getId().toString()
    ...
    //		LOG.warn("Found "+badSignalSystemData.size()+" incomplete or incorrect SignalSystemData ->
    ...
    for (Id<SignalSystem> systemId : this.systems.getSignalSystemData().keySet()) {
      for (SignalData signaldata :
          this.systems.getSignalSystemData().get(systemId).getSignalData().values()) {
        ...
        Long temp2 =
            new Long(
                network.getLinks().get(signaldata.getLinkId()).getFromNode().getId().toString());
  ...
  private void mergeOnewaySignalSystems(List<OsmNode> addingNodes, List<OsmNode> checkedNodes) {
    ...
    for (Long nodeID : nodesInBB) {
      ...
      if (signalizedOsmNodes.contains(nodeID)
          && isNodeAtJunction(this.nodes.get(nodeID))
          && !oldToMergedJunctionNodeMap.containsKey(nodeID)
          && hasNodeOneway(this.nodes.get(nodeID))) {
        junctionNodes.add(nodeID);
        for (Long otherNodeID : nodesInBB) {

          if (signalizedOsmNodes.contains(otherNodeID)
              && isNodeAtJunction(this.nodes.get(otherNodeID))
              && NetworkUtils.getEuclideanDistance(
                      this.nodes.get(nodeID).coord.getX(),
                      this.nodes.get(nodeID).coord.getY(),
                      this.nodes.get(otherNodeID).coord.getX(),
                      this.nodes.get(otherNodeID).coord.getY())
                  < SIGNAL_MERGE_DISTANCE
      ...
      // TODO sbraun30092020 This is maybe redundant? Closeby nodes are already found by method
      // above??
      if (junctionNodes.size() > 1) {
        ...
        for (Long jnID : junctionNodes) {
          ...
          if (repXmin == 0 || tempNode.coord.getX() < repXmin) repXmin = tempNode.coord.getX();
          if (repXmax == 0 || tempNode.coord.getX() > repXmax) repXmax = tempNode.coord.getX();
          if (repYmin == 0 || tempNode.coord.getY() < repYmin) repYmin = tempNode.coord.getY();
          if (repYmax == 0 || tempNode.coord.getY() > repYmax) repYmax = tempNode.coord.getY();
        }
        ...
        for (Long betweenNodeID : nodesInBB) {
          if (box.contains(this.nodes.get(betweenNodeID).coord)) junctionNodes.add(betweenNodeID);
        }
      ...
      if (this.lanes.getLanesToLinkAssignments().containsKey(lvec.getLink().getId())) {
        ...
        for (Lane lane :
            this.lanes
                ...
                .get(lvec.getLink().getId())
                .getLanes()
                .values()) {
          if (!lane.getId().toString().endsWith("ol")) {
            lane.setCapacityVehiclesPerHour(
                SIGNAL_LANES_CAPACITY * lane.getNumberOfRepresentedLanes());
  ...
  private void findCloseJunctionNodesWithSignals(
      OsmNode firstNode,
      OsmNode node,
      List<OsmNode> junctionNodes,
      List<OsmNode> checkedNodes,
      double distance,
      boolean getAll) {
    // Loop over all ways of given node
    for (OsmWay way : node.ways.values()) {
      String oneway = way.tags.get(TAG_ONEWAY);
      // if (oneway != null) { // && (oneway.equals("yes") || oneway.equals("true") ||
      // oneway.equals("1"))   //sbraun20200204 check why this was commented out??
      if ((oneway != null)
          && (oneway.equals("yes") || oneway.equals("true") || oneway.equals("1"))) { //
        ...
        for (int i = way.nodes.indexOf(node.id) + 1; i < way.nodes.size(); i++) {
          ...
          if (otherNode.used
              && !checkedNodes.contains(otherNode)
              && !junctionNodes.contains(otherNode)) {
            // if smaller than 30m
            if (NetworkUtils.getEuclideanDistance(
                    node.coord.getX(),
                    node.coord.getY(),
                    otherNode.coord.getX(),
                    otherNode.coord.getY())
                < distance) {
              if (otherNode.id == firstNode.id) {
                ...
                junctionNodes.add(otherNode);
                findCloseJunctionNodesWithSignals(
                    firstNode, otherNode, junctionNodes, checkedNodes, distance, getAll);
      ...
      if (junctionNodes.contains(firstNode) && !getAll) break;
    }
  }
      ...
      if (!checkedNodes.contains(node) && node.used && node.ways.size() > 1) {
        ...
        if (junctionNodes.size() > 1) {
          ...
          for (OsmNode tempNode : junctionNodes) {
            ...
            if (repYmin == 0 || tempNode.coord.getY() < repYmin) repYmin = tempNode.coord.getY();
            if (repYmax == 0 || tempNode.coord.getY() > repYmax) repYmax = tempNode.coord.getY();
            if (signalizedOsmNodes.contains(tempNode.id)) signalized = true;
          }
          repX = repXmin + (repXmax - repXmin) / 2;
          repY = repYmin + (repYmax - repYmin) / 2;
          OsmNode junctionNode = new OsmNode(this.id, new Coord(repX, repY));
          if (signalized) signalizedOsmNodes.add(junctionNode.id);
          junctionNode.used = true;
          for (OsmNode tempNode : junctionNodes) {
    ...
    for (OsmNode node : this.nodes.values()) {
      if (!checkedNodes.contains(node) && node.used && isNodeAtJunction(node)) {
        ...
        if (suit) {
          ...
          for (OsmWay way : node.ways.values()) {
            String oneway = way.tags.get(TAG_ONEWAY);
            // sbraun 05032020 alles was kein Onewaytag hat wird übersprungen -> änder das -> evtl.
            // die ganz Logik rausnehmen
            // -> Fixed Error in Junction System 1420523170 in Benchmark Network
            if (oneway == null || !oneway.equals("no")) continue; // changed to continue
            for (int i = 0; i < way.nodes.size(); i++) {
              if (otherSuit) continue; // changed to continue
              ...
              // Only checks one way out of node: There are sometimes Nodes in the mid of a
              // junction; try to fix in else statement
              if (NetworkUtils.getEuclideanDistance(
                          node.coord.getX(),
                          node.coord.getY(),
                          otherNode.coord.getX(),
                ...
                while (counter < 5
                    && (otherNode.ways.size() <= 2
                        && distance < SIGNAL_MERGE_DISTANCE
                        && !isNodeAtJunction(otherNode))) {
                  // counter++;
                  if (!checkedNodes.contains(otherNode)) {
                    for (OsmWay tempWay : otherNode.ways.values()) {
                      ...
                      if (!passedWays.contains(tempWay.id)) {
                        ...
                        // check if there are better candiates on the same way if not take the last
                        // node
                        ...
                        // sbraun 05032020 wenn kleine Mittelwege keine Onewways sind ist die
                        // Reihenfolge manchmal anders rum
                    ...
                    if (otherNodeSignalized == nodeSignalized
                        && isNodeAtJunction(otherNode)
                        && !node.equals(otherNode)) {
                      break;
                    }
  ...
  private void findingFourNodeJunctions(List<OsmNode> addingNodes, List<OsmNode> checkedNodes) {
    for (OsmNode node : this.nodes.values()) {
      if (!checkedNodes.contains(node)
          && node.used
          && signalizedOsmNodes.contains(node.id)
          && node.ways.size() > 1) {
        List<OsmNode> junctionNodes = new ArrayList<>();

        findCloseJunctionNodesWithSignals(
            node, node, junctionNodes, checkedNodes, MERGE_DISTANCE_JUNCTIONS, false);
  ...
  private void pushSignalsOverShortWays() {
    ...
    //					pushSignalOverShortWay(way, this.nodes.get(way.nodes.get(0)),
    // this.nodes.get(way.nodes.get(1)));
    //					pushSignalOverShortWay(way, this.nodes.get(way.nodes.get(1)),
    // this.nodes.get(way.nodes.get(0)));
    //				}
    ...
    //			}
    //		}
    // sbraun20200305 new logic -> irgendwie nicht schoen und code Wiederholung von
    // #pushIntoNearbyJunction
    for (Long id : nodes.keySet()) {
      ...
      if (shortWays.size() == 1) {
        ...
        pushSignalOverShortWay(
            way, this.nodes.get(way.nodes.get(0)), this.nodes.get(way.nodes.get(1)));
        pushSignalOverShortWay(
            way, this.nodes.get(way.nodes.get(1)), this.nodes.get(way.nodes.get(0)));
      }
      ...
      if (shortWays.size() > 1) {
        ...
        pushSignalOverShortWay(
            bestWay, this.nodes.get(bestWay.nodes.get(0)), this.nodes.get(bestWay.nodes.get(1)));
        pushSignalOverShortWay(
            bestWay, this.nodes.get(bestWay.nodes.get(1)), this.nodes.get(bestWay.nodes.get(0)));
      }
    }
  ...
  private void pushSignalOverShortWay(OsmWay shortWay, OsmNode fromNode, OsmNode toNode) {
    if (NetworkUtils.getEuclideanDistance(
            fromNode.coord.getX(), fromNode.coord.getY(), toNode.coord.getX(), toNode.coord.getY())
        < SIGNAL_MERGE_DISTANCE) {
      if (fromNode.ways.size() == 2
          && toNode.ways.size() > 2
      ...
      for (int i = 1; i < way.nodes.size() - 1; i++) {
        ...
        if (signalizedOsmNodes.contains(signalNode.id) && !isNodeAtJunction(signalNode)) {
          ...
          if ((oneWay != null && !oneWay.equals("-1") && !oneWay.equals("no")) || oneWay == null) {
            ...
            if (signalizedOsmNodes.contains(endPoint.id)
                && isNodeAtJunction(endPoint)
                && NetworkUtils.getEuclideanDistance(
                        signalNode.coord.getX(),
                        signalNode.coord.getY(),
                        endPoint.coord.getX(),
                        endPoint.coord.getY())
                    < SIGNAL_MERGE_DISTANCE) signalizedOsmNodes.remove(signalNode.id);
          }
  ...
  // TODO this method already moves signals over crossings to endpoints of ways. do we still need
  // pushingSingnalsIntoEndpoints?
  // or, if we need it because it does additional stuff, can we remove the case for
  // crossings/endpoints here in this method??
      ...
      for (int i = 1; i < way.nodes.size() - 1; i++) {
        ...
        if (signalizedOsmNodes.contains(signalNode.id) && !isNodeAtJunction(signalNode)) {
          ...
          if ((oneway != null && !oneway.equals("-1")) || oneway == null || oneway.equals("no")) {
            ...
            // TODO 20200124 sbraun: loops over all nodes of this way to find the closest junction.
            // The problem seems
            // that once he found that node he continue to loop over the nodes of the way, so once
            // junction is found it continues to look
            ...
            if (junctionNodePosDir != null && junctionNodeNegDir != null) {
              ...
              // sbraun 19032020
              // rather complicated but it compares firstly the hierarchies of the ways the junction
              // node is on
            ...
            // TODO sbraun20200130 This is only used once in Cottbus and it will only rest above
            // junctionNode to Null again.
            if (i < way.nodes.size() - 2) {
              ...
              if (crossingOsmNodes.contains(nextNode.id) && secondNextNode.ways.size() > 1) {
                // next node = pedestrian crossing. and the second next node is either a junction or
                // the end of the way
          ...
          if (junctionNode != null
              && NetworkUtils.getEuclideanDistance(
                      signalNode.coord.getX(),
                      signalNode.coord.getY(),
                      junctionNode.coord.getX(),
                      junctionNode.coord.getY())
                  < SIGNAL_MERGE_DISTANCE) {
            signalizedOsmNodes.remove(signalNode.id);
            signalizedOsmNodes.add(junctionNode.id);

            // TODO sbraun20200128: This could have been checked further up, would be much clearer
            // -> dont have code above double in this method
  ...
  private void createPlansForFourWayJunction(
      Node node,
      SignalSystemData signalSystem,
      Tuple<LinkVector, LinkVector> firstPair,
      Tuple<LinkVector, LinkVector> secondPair) {
    int groupNumber = 1;
    int cycle = CYCLE_TIME;
    double lanesFirst =
        (firstPair.getFirst().getLink().getNumberOfLanes()
                + firstPair.getSecond().getLink().getNumberOfLanes())
            / 2;
    double lanesSecond =
        (secondPair.getFirst().getLink().getNumberOfLanes()
                + secondPair.getSecond().getLink().getNumberOfLanes())
            / 2;
    int changeTime = (int) ((lanesFirst) / (lanesFirst + lanesSecond) * cycle);
    if (changeTime < this.minimalTimeForPair) changeTime = this.minimalTimeForPair;
    if (changeTime > cycle - this.minimalTimeForPair) changeTime = cycle - this.minimalTimeForPair;
  ...
  private void createPlansforThreeWayJunction(
      Node node,
      SignalSystemData signalSystem,
      Tuple<LinkVector, LinkVector> pair,
      LinkVector thirdArm) {
    int groupNumber = 1;
    int cycle = CYCLE_TIME;
    double lanesPair =
        (pair.getFirst().getLink().getNumberOfLanes()
                + pair.getSecond().getLink().getNumberOfLanes())
            / 2;
    int changeTime =
        (int) ((lanesPair) / (lanesPair + thirdArm.getLink().getNumberOfLanes()) * cycle);
    if (changeTime < this.minimalTimeForPair) changeTime = this.minimalTimeForPair;
    if (changeTime > cycle - this.minimalTimeForPair) changeTime = cycle - this.minimalTimeForPair;
    boolean firstIsCritical = false;
    List<Lane> criticalSignalLanes = new ArrayList<Lane>();
    if (pair.getFirst().getRotationToOtherInLink(thirdArm) > Math.PI) firstIsCritical = true;
    if (firstIsCritical
        && lanes.getLanesToLinkAssignments().containsKey(pair.getFirst().getLink().getId())) {
      for (Lane lane :
          lanes
              .getLanesToLinkAssignments()
              .get(pair.getFirst().getLink().getId())
              .getLanes()
              .values()) {
        if (lane.getAlignment() == 2) criticalSignalLanes.add(lane);
      }
    } else if (lanes.getLanesToLinkAssignments().containsKey(pair.getSecond().getLink().getId())) {
      for (Lane lane :
          lanes
              .getLanesToLinkAssignments()
              .get(pair.getSecond().getLink().getId())
              .getLanes()
              .values()) {
        if (lane.getAlignment() == 2) criticalSignalLanes.add(lane);
      }
  ...
  private void createPlansforTwoWayJunction(Node node, SignalSystemData signalSystem) {
    ...
    if (inLinksAngle > (3. / 4. * Math.PI) && inLinksAngle < (5. / 4. * Math.PI)) {
      if (!this.makePedestrianSignals) {
        ...
        LOG.info(
            "Remove Signal-"
                ...
                + " as the InLinks angle indicates that this is is Pedestrian Crossing.");
        return;
      } else {
        SignalGroupData group =
            this.groups
                .getFactory()
                .createSignalGroupData(
                    signalSystem.getId(),
        ...
        SignalGroupSettingsData settings =
            createSetting(
                0, cycle - PEDESTRIAN_CROSSING_TIME + INTERGREENTIME, node, group.getId());
      ...
      SignalGroupSettingsData settingsFirst =
          createSetting(0, 45 - INTERGREENTIME, node, groupOne.getId());
      ...
      controller.addSignalPlanData(plan);
      SignalGroupSettingsData settingsSecond =
          createSetting(45, 90 - INTERGREENTIME, node, groupTwo.getId());
      plan.addSignalGroupSettings(settingsSecond);
      groups.addSignalGroupData(groupTwo);
    }
  }

  private void createTwoPhase(
      int groupNumber,
      SignalSystemData signalSystem,
      ...
      if (signal.getLinkId().equals(pair.getFirst().getLink().getId())
        ...
        for (int i = 0; i < criticalSignalLanes.size(); i++) {
          if (signal.getLaneIds() != null
              && signal.getLaneIds().contains(criticalSignalLanes.get(i).getId()))
            firstPhase = false;
        }
        if (firstPhase) groupOne.addSignalId(signal.getId());
      }
    }
    ...
    if (first)
      settingsFirst =
          createSetting(
              0, changeTime - (2 * INTERGREENTIME + MIN_GREENTIME), node, groupOne.getId());
    else
      settingsFirst =
          createSetting(
              changeTime, cycle - (2 * INTERGREENTIME + MIN_GREENTIME), node, groupOne.getId());
    plan.addSignalGroupSettings(settingsFirst);
    groups.addSignalGroupData(groupOne);
    groupNumber++;
    ...
    if (first)
      settingsSecond =
          createSetting(
              changeTime - (INTERGREENTIME + MIN_GREENTIME),
              changeTime - INTERGREENTIME,
              node,
              groupTwo.getId());
    else
      settingsSecond =
          createSetting(
              cycle - (INTERGREENTIME + MIN_GREENTIME),
              cycle - INTERGREENTIME,
              node,
              groupTwo.getId());
    plan.addSignalGroupSettings(settingsSecond);
    groups.addSignalGroupData(groupTwo);
  ...
  private void fillConflictingLanesData(
      Tuple<LinkVector, LinkVector> pair, List<Lane> criticalSignalLanes) {
    Link firstLink = pair.getFirst().getLink();
    Link secondLink = null;
    if (pair.getSecond() != null) secondLink = pair.getSecond().getLink();
  ...
  private void setConflictingAndNonConflictingLanesToLanes(
      Link firstLink, Link secondLink, List<Lane> criticalSignalLanes) {
    ...
    if (otherl2l != null) {
      for (Lane lane : otherl2l.getLanes().values()) {
        if (!lane.getId().toString().endsWith("ol")) secondLanes.add(lane);
      }
    }
    ...
    for (Lane lane : firstLanes) {
      ...
      for (Lane otherLane : firstLanes) {
        if (!otherLane.equals(lane)) nonCritLanes.add(otherLane.getId());
      }
      for (Lane otherLane : secondLanes) {
        ...
        else nonCritLanes.add(otherLane.getId());
      }
  ...
  private void createOnePhase(
    ...
    if (pair.getSecond() != null) secondLinkId = pair.getSecond().getLink().getId();
    for (SignalData signal : signalSystem.getSignalData().values()) {
    ...
    if (first) settings = createSetting(0, changeTime - INTERGREENTIME, node, group.getId());
    else settings = createSetting(changeTime, cycle - INTERGREENTIME, node, group.getId());
    plan.addSignalGroupSettings(settings);
    groups.addSignalGroupData(group);
    groupNumber++;
  }
  ...
  private SignalGroupSettingsData createSetting(
      int onset, int dropping, Node node, Id<SignalGroup> id) {
  ...
  private SignalPlanData createPlan(Node node, int cycle) {
    SignalPlanData plan =
        this.control.getFactory().createSignalPlanData(Id.create(node.getId(), SignalPlan.class));
  ...
  private SignalSystemControllerData createController(SignalSystemData signalSystem) {
    SignalSystemControllerData controller =
        this.control.getFactory().createSignalSystemControllerData(signalSystem.getId());
    this.control.addSignalSystemControllerData(controller);
    controller.setControllerIdentifier(DefaultPlanbasedSignalSystemController.IDENTIFIER);
    return controller;
  }

  private SignalGroupData createSignalGroup(
      int groupNumber, SignalSystemData signalSystem, Node node) {
    SignalGroupData group =
        this.groups
            .getFactory()
            .createSignalGroupData(
  ...
  private void findTwoPhaseSignalLanes(
      Tuple<LinkVector, LinkVector> pair, List<Lane> criticalSignalLanes) {
  ...
  private void createLanes(final Link l, final Lanes lanes, final double nofLanes) {
    OsmHighwayDefaults defaults =
        this.highwayDefaults.get(l.getAttributes().getAttribute(TYPE).toString());
    LanesFactory factory = lanes.getFactory();
    LanesToLinkAssignment lanesForLink =
        factory.createLanesToLinkAssignment(Id.create(l.getId(), Link.class));
    ...
    for (int i = 1; i <= nofLanes; i++) {
      Lane lane =
          lanes.getFactory().createLane(Id.create("Lane" + l.getId() + "." + i, Lane.class));
  ...
  private void simplifyLanesAndAddOrigLane(Link link) {
    ...
    Lane origLane =
        lanes.getFactory().createLane(Id.create("Lane" + link.getId() + ".ol", Lane.class));
    ...
    for (int indexLane1 = 1;
        indexLane1 <= lanes.getLanesToLinkAssignments().get(link.getId()).getLanes().size();
        indexLane1++) {
      Lane lane1 =
          lanes
              .getLanesToLinkAssignments()
              .get(link.getId())
              .getLanes()
              .get(Id.create("Lane" + link.getId() + "." + indexLane1, Lane.class));
      ...
      if (!lanesToBeRemoved.contains(lane1.getId())) {
        ...
        mergeCheck:
        for (int indexLane2 = indexLane1 + 1;
            indexLane2 <= lanes.getLanesToLinkAssignments().get(link.getId()).getLanes().size();
            indexLane2++) {
          Lane lane2 =
              lanes
                  .getLanesToLinkAssignments()
                  .get(link.getId())
                  .getLanes()
                  .get(Id.create("Lane" + link.getId() + "." + indexLane2, Lane.class));
   ...
   */
  private void createLaneStack(
      String turnLanes, Stack<Stack<Integer>> turnLaneStack, double nofLanes, Id<Link> id) {
    ...
    // number of Lanes
    Stack<Integer> tempLane = new Stack<Integer>();
    if (turnLaneStack.size() < nofLanes)
      LOG.warn(
          "Number of Lanes does not match the number of tags at link"
              + id.toString()
              + "\n Tags:"
              + turnLaneStack.size()
              + "\n Lanes: "
  ...
  private List<LinkVector> constructInLinkVectors(Node node) {
    ...
    for (int i = 0; i < inLinks.size(); i++) {
      ...
      if (linkToOrigToNodeCoord.containsKey(inLinks.get(i).getId())
          || linkToOrigFromNodeCoord.containsKey(inLinks.get(i).getId())) {
  ...
  private void fillLanesAndCheckRestrictions(Link link) {
    ...
    if (laneStack != null && !laneStack.isEmpty()) {
      if (laneStack.size() != (int) link.getNumberOfLanes()) {
        throw new RuntimeException(
            "the turn:lanes tag has a different number of lanes than the lanes tag");
      }
      for (int i = (int) link.getNumberOfLanes(); i > 0; i--) {
        ...
        setToLinksForLaneWithTurnLanes(
            lane, laneStack.pop(), linkVectors, (laneStack.size() == 1), link.getId());
      }
    } else {
  ...
  private void setToLinksForLanesDefault(Link link, List<LinkVector> toLinks) {
    try {
      ...
      for (int i = 1; i < toLinks.size(); i++) {
        if (Math.abs(toLinks.get(i).getRotation() - Math.PI)
            < Math.abs(toLinks.get(straightLink).getRotation() - Math.PI)) straightLink = i;
        if (Math.abs(toLinks.get(i).getRotation() - Math.PI)
            > Math.abs(toLinks.get(reverseLink).getRotation() - Math.PI)) reverseLink = i;
      }
      if (toLinks.get(straightLink).getRotation() < (1. - THROUGHLINK_ANGLE_TOLERANCE) * Math.PI
          || toLinks.get(straightLink).getRotation()
              > (1. + THROUGHLINK_ANGLE_TOLERANCE) * Math.PI) {
      ...
      // sbraun08082020 the case above leads to wrong results -> find reverse link with node
      // coordinates:
      if (toLinks.size() == 2) {
        for (int i = 1; i < toLinks.size(); i++) {
      ...
      if (lanes.getLanesToLinkAssignments().containsKey(link.getId()) && toLinks.size() > 1) {
        {
          ...
          Lane lane =
              lanes
                  ...
                  .get(
                      Id.create(
                          "Lane" + link.getId() + "." + ((int) link.getNumberOfLanes()),
                          Lane.class));
          if (reverseLink != 0) lane.addToLinkId(toLinks.get(0).getLink().getId());
          else lane.addToLinkId(toLinks.get(1).getLink().getId());
          lane.setAlignment(-2);
          ...
          if (reverseLink != -1) lane.addToLinkId(toLinks.get(reverseLink).getLink().getId());
          if (reverseLink == toLinks.size() - 1)
            lane.addToLinkId(toLinks.get(toLinks.size() - 2).getLink().getId());
          else lane.addToLinkId(toLinks.get(toLinks.size() - 1).getLink().getId());
          lane.setAlignment(2);
          // lane.getAttributes().putAttribute(TO_LINK_REFERENCE, OUTER_LANE_TYPE);
        }
        ...
        if (link.getNumberOfLanes() > 2) {
          ...
          Lane laneToPutTo =
              lanes
                  ...
                  .get(
                      Id.create(
                          "Lane" + link.getId() + "." + ((int) link.getNumberOfLanes() - 1),
                          Lane.class));
  ...
  private void setToLinksForLaneWithTurnLanes(
      Lane lane,
      Stack<Integer> laneStack,
      List<LinkVector> toLinks,
    ...
    if (reverseLink.getRotation() < (2. - THROUGHLINK_ANGLE_TOLERANCE) * Math.PI
        && reverseLink.getRotation() > THROUGHLINK_ANGLE_TOLERANCE * Math.PI) reverseLink = null;
    int it = 1;
    ...
    while (!laneStack.isEmpty()) {
      ...
      if (tempDir == null) { // no direction for lane available
        ...
        LOG.warn(
            "Did not not understand direction of lane "
                + lane.getId().toString()
      ...
      if (tempDir < 0 && tempDir > -5 || tempDir == 99) { // all right directions (right,
        ...
        for (LinkVector lvec : toLinks) {
          if (lvec.dirTheta < (1. - THROUGHLINK_ANGLE_TOLERANCE) * Math.PI) tempLinks.add(lvec);
        }
            ...
            if (!this.allowUTurnAtLeftLaneOnly && insertedUTurnOnLeftLane) {
              ...
              if (this.saveTurnLanes == true) {
                ...
                if (lane.getAttributes().getAttribute(OSM_TURN_INFO) == null) {
                  ...
                  lane.getAttributes()
                      .putAttribute(OSM_TURN_INFO, newTurn + "|" + toLinkId + ":reverse");
                }
              }
            }
          }
        ...
        if (tempLinks.size() == 1) { // if there is just one "right"
          ...
          if (tempDir == -1
              || tempDir
                  == 99) { // lane direction: "right" //TODO sbraun 31072020 add right direction for
            // 99??
            for (LinkVector lvec : tempLinks) {
              if (reverseLink != null
                  && lvec.getLink().getId().equals(reverseLink.getLink().getId())) {
                continue;
              }
              lane.addToLinkId(lvec.getLink().getId());
              writeOutLinkInfo(lane, lvec.getLink().getId(), "right");

              //                            lane.getAttributes().putAttribute(OSM_TURN_INFO,
              // "right");
            }
          }
      ...
      if (tempDir > 0 && tempDir < 4) { // all "left" directions (left,
        ...
        if (alignmentAnte == 0 && it == 1) alignmentAnte = -10;
        for (LinkVector lvec : toLinks) {
          if (lvec.dirTheta > (1. + THROUGHLINK_ANGLE_TOLERANCE) * Math.PI) tempLinks.add(lvec);
        }
        ...
        if (tempLinks.size() == 1) { // if there is just one "left"
          ...
          // add Reverse Turn here and flag "insertedUTurnOnLeftLane" it (TODO maybe useful later
          // other wise redundant)
          ...
          if (tempDir == 1) { // lane direction: "left"
            for (LinkVector lvec : tempLinks) {
              // add link as to-link, if it is not the reverse link OR the link has only one lane OR
              // u-turn is allowed at any lane
              if (!lvec.equals(reverseLink)
                  || singleLaneOfTheLink
                  || !this.allowUTurnAtLeftLaneOnly) {
                lane.addToLinkId(lvec.getLink().getId());
          ...
          if (tempDir == 2) { // lane direction: "slight_left"
            if (tempLinks.get(1).dirTheta > 3. * Math.PI / 2.
                || !tempLinks.get(1).equals(reverseLink)) {
              lane.addToLinkId(tempLinks.get(0).getLink().getId());
        ...
        if (alignmentAnte == 0) lane.setAlignment(1);
        else lane.setAlignment(2);
      }
      if (tempDir == 0
          || tempDir == 4
          || tempDir == -5
          || tempDir == 99) { // lane directions that have to lead to a forward link
        // (through, merge_to_left,merge_to_right)
        // sbraun24072020: added 99 -> a direction which has been not understood previously
        alignmentAnte =
            lane.getAlignment(); // look for the most "forward" link (closest to 180° or pi) and
        // take it
        ...
        if (toLinks.size() == 2) {
          if (Math.abs(toLinks.get(0).dirTheta - Math.PI) < DEGREE_TOLERANCE_ROUNDABOUTS
              ...
              && !(toLinks.get(0).getLink().equals(reverseLink)
                  || toLinks.get(1).getLink().equals(reverseLink))) {
            unclearThrough = true;
          }
        }
        if (unclearThrough) {
          LOG.warn(
              "Detected unclear through direction at Lane "
                  + lane.getId()
        ...
        if (this.saveTurnLanes) {
          ...
          if (unclearThrough) {
            toLinkId =
                toLinks.get(0).getLink().getId().toString()
                    + ","
                    + toLinks.get(1).getLink().getId().toString();
          } else {
        ...
        if (alignmentAnte == -2) lane.setAlignment(-1);
      }
      // sbraun08082020 added this to ensure that some special links are connected e.g.
      // Baenschstr./Berlin
      if (tempDir == 5
          || network.getLinks().get(linkId).getNumberOfLanes() == 1) { // lane direction: "reverse"
        ...
        if (alignmentAnte == 0
            && lane.getToLinkIds() != null) { // || lane.getToLinkIds().isEmpty())) //sbraun26052020
          if (lane.getToLinkIds().isEmpty()) alignmentAnte = -10;
        }
        if (reverseLink != null) {
          lane.addToLinkId(reverseLink.getLink().getId());
          writeOutLinkInfo(lane, reverseLink.getLink().getId(), "reverse");
  ...
  private List<LinkVector> orderToLinks(Link link, List<Link> toLinks) {
    ...
    if (linkToOrigToNodeCoord.containsKey(link.getId())
        || linkToOrigFromNodeCoord.containsKey(link.getId())) {
    ...
    for (Link value : toLinks) {
      ...
      if (linkToOrigToNodeCoord.containsKey(value.getId())
          || linkToOrigFromNodeCoord.containsKey(value.getId())) {
  ...
  private void removeRestrictedLinks(Link fromLink, List<LinkVector> toLinks) {
    ...
    if (toLinks.size() == 1) {
      LOG.warn(
          "The Node between ToLink "
              + toLinks.get(0).getLink().getId().toString()
              + " and FromLink "
              + fromLink.getId().toString()
    ...
    for (OsmRelation restriction : osmNodeRestrictions.get(toNode.id)) {
      if (Long.valueOf(fromLink.getAttributes().getAttribute(ORIG_ID).toString())
        ...
        if (restriction.restrictionValue == false) {
          ...
          for (LinkVector linkVector : toLinks) {
            ...
            if (linkVector.getLink().getToNode().getId().equals(fromLink.getFromNode().getId())
                ...
                && toLinks.size() == 2) {
              LOG.warn(
                  "Don't delete reverse Link from Link "
                      + fromLink.getId().toString()
  ...
  protected void setOrModifyLinkAttributes(Link l, OsmWay way, boolean forwardDirection) {
    ...
    if (oldToMergedJunctionNodeMap.containsKey(toNodeOsmId)) {
      ...
      l.setLength(
          NetworkUtils.getEuclideanDistance(l.getFromNode().getCoord(), l.getToNode().getCoord()));
    }
  ...
  public boolean isNodeAtJunction(OsmNode node) {
    if (node.endPoint && node.ways.size() > 2) return true;
    if (!node.endPoint && node.ways.size() > 1) return true;
        ...
        for (int i = 0; i < way.nodes.size(); i++) {
          if (node.id == (way.nodes.get(i))) {
            if (i != 0 && i != way.nodes.size() - 1)
              // the node is an endpoint of one way and no endpoint of the other way -> three arm
              // intersection
              return true;
          }
        }
  ...
  private boolean hasNodeOneway(OsmNode node) {
    ...
    for (OsmWay way : node.ways.values()) {
      String oneway = way.tags.get(TAG_ONEWAY);
      if (oneway != null && !oneway.equals("no")) hasOneway = true;
    }
    return hasOneway;
  }
  ...
  private void writeOutLinkInfo(Lane lane, Id<Link> linkId, String dir) {
    if (this.saveTurnLanes) {
      if (lane.getAttributes().getAttribute(OSM_TURN_INFO) == null) {
        ...
        String newTurn = lane.getAttributes().getAttribute(OSM_TURN_INFO).toString();
        lane.getAttributes()
            .putAttribute(OSM_TURN_INFO, newTurn + "|" + linkId.toString() + ":" + dir);
      }
    }
  }
  ...
  private final class SignalLanesOsmXmlParser extends OsmXmlParser {
    ...
    public SignalLanesOsmXmlParser(
    ...
    public void enableOptimization(int step) {
      throw new UnsupportedOperationException(
          "slow but low memory is not supported for the signals and lanes osm reader");
    }
      ...
      if ("relation".equals(name)) {
        ...
        if (this.currentRelation != null) {
          ...
          if ("node".equals(type)) {
            ...
            if ("from".equals(role)) {
              this.currentRelation.fromRestricted =
                  this.ways.get(Long.parseLong(atts.getValue("ref")));
            } else if ("to".equals(role)) {
              this.currentRelation.toRestricted =
                  this.ways.get(Long.parseLong(atts.getValue("ref")));
            }
          }
        }
  ...
  private static final class LinkVector implements Comparable<LinkVector> {
    ...
    public void calculateRotation(LinkVector linkVector) {
      if (this.theta <= Math.PI) this.dirTheta = this.theta - linkVector.getAlpha() + Math.PI;
      else this.dirTheta = this.theta - linkVector.getAlpha() - Math.PI;
      if (this.dirTheta < 0) {
    ...
    public int compareTo(LinkVector lv) {
      ...
      if (this.dirTheta == otherDirAlpha) return 0;
      if (this.dirTheta > otherDirAlpha) return 1;
      else return -1;
    }
  }
  ...
  private static final class BoundingBox {
    ...
    public boolean contains(Coord coord) {
      if ((coord.getX() < this.east && coord.getX() > this.west)
          && (coord.getY() < this.north && coord.getY() > this.south)) return true;
      else return false;
    }
  }
}
