...
public final class IntergreensLogicImpl implements IntergreensLogic {
  ...
  private final Map<Id<SignalSystem>, Map<Id<SignalGroup>, Double>> signalSystemIdSignalGroupMap =
      new HashMap<>();

  public IntergreensLogicImpl(
      IntergreenTimesData intergreenTimesData, SignalSystemsConfigGroup signalSystemsConfigGroup) {
  ...
  private void handleRedOrSimilarStateChange(SignalGroupStateChangedEvent event) {
    Map<Id<SignalGroup>, Double> signalGroupOffTimeMap =
        this.signalSystemIdSignalGroupMap.get(event.getSignalSystemId());
    if (signalGroupOffTimeMap == null) {
      this.signalSystemIdSignalGroupMap.put(
          event.getSignalSystemId(), new HashMap<Id<SignalGroup>, Double>());
    }
    this.signalSystemIdSignalGroupMap
        .get(event.getSignalSystemId())
        .put(event.getSignalGroupId(), event.getTime());
  }

  private void checkAndHandleGreenAllowed(SignalGroupStateChangedEvent event) {
    IntergreensForSignalSystemData greensData4System =
        this.intergreensData.getIntergreensForSignalSystemDataMap().get(event.getSignalSystemId());
    if (greensData4System != null) {
      Map<Id<SignalGroup>, Integer> endGroupTimes4BeginningGroup =
          greensData4System.getEndSignalGroupTimesForBeginningGroup((event.getSignalGroupId()));
      if (endGroupTimes4BeginningGroup != null) {
        for (Entry<Id<SignalGroup>, Integer> intergreens4BeginningGroup :
            endGroupTimes4BeginningGroup.entrySet()) {
          int time = (int) event.getTime();
          Map<Id<SignalGroup>, Double> droppingTimes4System =
              this.signalSystemIdSignalGroupMap.get(event.getSignalSystemId());
          if (droppingTimes4System != null) {
            ...
            if (lastDropTime != null) {
              ...
              if (intergreens4BeginningGroup.getValue() > realIntergreen) {
                ...
                intergreenViolation.append(" seconds.");
                if (this.signalsConfig
                    .getActionOnIntergreenViolation()
                    .equals(SignalSystemsConfigGroup.ActionOnSignalSpecsViolation.WARN)) {
                  log.warn(intergreenViolation.toString());
                } else if (this.signalsConfig
                    .getActionOnIntergreenViolation()
                    .equals(SignalSystemsConfigGroup.ActionOnSignalSpecsViolation.EXCEPTION)) {
                  log.warn(intergreenViolation.toString());
