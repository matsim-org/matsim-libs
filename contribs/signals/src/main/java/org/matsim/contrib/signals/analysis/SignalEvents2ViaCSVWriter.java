 ...
 * @author tthunig
 */
public class SignalEvents2ViaCSVWriter
    implements SignalGroupStateChangedEventHandler, IterationEndsListener, IterationStartsListener {
  ...
  /**
   * distance between signal (the coordinate that will be drawn) and link in orthogonal direction to
   * the link
   */
  ...
  public void notifyIterationStarts(IterationStartsEvent event) {
    ...
    if (event.isLastIteration()
        || (scenario.getConfig().controller().getWriteEventsInterval() != 0
            && event.getIteration() % scenario.getConfig().controller().getWriteEventsInterval()
                == 0)) {
      writeThisIteration = true;
      String signalCSVFilename =
          scenario.getConfig().controller().getOutputDirectory()
              + "/ITERS/it."
              + event.getIteration()
              + "/signalEvents2Via.csv";

      // log.info("Initializing SignalsCSVWriter ...");
      signalsCSVWriter = IOUtils.getBufferedWriter(signalCSVFilename);
      log.info(
          "Writing signal events of iteration "
              + event.getIteration()
              + " as csv file for via to "
              + signalCSVFilename
              + " ...");

      // create header
      try {
        signalsCSVWriter.write(
            SIGNAL_ID + ";" + X_COORD + ";" + Y_COORD + ";" + TIME + ";" + SIGNAL_STATE);
  ...
  public void handleEvent(SignalGroupStateChangedEvent event) {
    if (writeThisIteration) {
      if (signalsData == null) {
        if (countWarnings > 9) {
          log.warn(
              "You are using the SignalsModule without Controler. No output is written out. (This warning is only given 10 times.)");
          countWarnings++;
        }
        return;
      }
      SignalGroupData signalGroupData =
          signalsData
              .getSignalGroupsData()
              .getSignalGroupDataBySystemId(event.getSignalSystemId())
              .get(event.getSignalGroupId());
      ...
      for (Id<Signal> signalId : signalGroupData.getSignalIds()) {
        ...
        try {
          ...
          e.printStackTrace();
          log.error(
              "Something went wrong while adding a line for the signal state switch of signal "
                  + signalId);
        }
      }
    }
  ...
  private Coord calculateSignalCoordinate(SignalData signalData) {
    // log.info("Calculating coordinate for signal " + signalId + " of signal system " +
    // signalSystemId + " ...");
    ...
    if (signalData.getLaneIds() != null) {
      // if the signal belongs to lanes, different signals may exist for one link -> determine step
      // number for visualization
      Lanes lanes = scenario.getLanes();
      Id<Lane> firstSignalLaneId = (Id<Lane>) signalData.getLaneIds().toArray()[0];
      Lane firstSignalLane =
          lanes.getLanesToLinkAssignments().get(signalLinkId).getLanes().get(firstSignalLaneId);
    ...
    if (toNodeCoord.getX() == fromNodeCoord.getX()) {
      ...
      double m =
          (toNodeCoord.getY() - fromNodeCoord.getY()) / (toNodeCoord.getX() - fromNodeCoord.getX());
    ...
    double x =
        toNodeCoord.getX()
            + SIGNAL_COORD_NODE_OFFSET * deltaX
            + stepNumber * SIGNAL_COORD_LINK_OFFSET * deltaY;
    double y =
        toNodeCoord.getY()
            + SIGNAL_COORD_NODE_OFFSET * deltaY
            - stepNumber * SIGNAL_COORD_LINK_OFFSET * deltaX;
