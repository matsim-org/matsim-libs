...
public final class SignalUtils {
  ...
  /**
   * Creates a signal group for each single signal of the system and adds it to the given
   * SignalGroupsData container. Each created signal group will get the same ID as the signal
   * itself.
   *
   * @param groups container to that the signal groups are added
   * @param system contains the signals for which signal groups are created
   */
  public static void createAndAddSignalGroups4Signals(
    ...
    for (SignalData signal : system.getSignalData().values()) {
      SignalGroupData group4signal =
          groups
              .getFactory()
              .createSignalGroupData(system.getId(), Id.create(signal.getId(), SignalGroup.class));
  ...
  /**
   * Convenience method to create a signal with the given Id on the link with the given Id on the
   * lanes with the given Ids. The signal is added to the SignalSystemData.
   */
  public static void createAndAddSignal(
      SignalSystemData sys,
      SignalSystemsDataFactory factory,
      Id<Signal> signalId,
      Id<Link> linkId,
      List<Id<Lane>> laneIds) {
  ...
  /**
   * Creates a signal plan with the given cycle time and offset. The plan gets the default signal
   * plan id 1.
   *
   * @param fac
   * @param cycleTime
   ...
   */
  public static SignalPlanData createSignalPlan(
      SignalControlDataFactory fac, int cycleTime, int offset) {
   ...
   */
  public static SignalPlanData createSignalPlan(
      SignalControlDataFactory fac, int cycleTime, int offset, Id<SignalPlan> signalPlanId) {
  ...
  /**
   * Creates and returns a signal group setting for the given signal group id with the given onset
   * and dropping time.
   *
   * @param fac
   * @param signalGroupId
  ...
  public static SignalGroupSettingsData createSetting4SignalGroup(
      SignalControlDataFactory fac, Id<SignalGroup> signalGroupId, int onset, int dropping) {

    SignalGroupSettingsData signalGroupSettings = fac.createSignalGroupSettingsData(signalGroupId);
  ...
  /**
   * Clones the SignalControlData given as argument and returns a new instance of SignalControlData
  ...
  public static SignalControlData copySignalControlData(SignalControlData oldSignalControlData) {
    ...
    for (SignalSystemControllerData oldSystemControl :
        oldSignalControlData.getSignalSystemControllerDataBySystemId().values()) {
      SignalSystemControllerData newSystemControl =
          new SignalSystemControllerDataImpl(oldSystemControl.getSignalSystemId());
  ...
  /**
   * Clones the SignalPlanData given as argument and returns a new instance of SignalPlanData with
   * the same content.
   */
  public static SignalPlanData copySignalPlanData(
      final SignalPlanData oldPlan, final Id<SignalPlan> newPlanId) {
    ...
    for (SignalGroupSettingsData oldGroupSetting :
        oldPlan.getSignalGroupSettingsDataByGroupId().values()) {
      SignalGroupSettingsData newGroupSettings =
          new SignalGroupSettingsDataImpl(oldGroupSetting.getSignalGroupId());
  ...
  /**
   * Clones the SignalGroupSettingsData given as argument and returns a new instance of
   * SignalGroupSettingsData with the same content.
   */
  public static SignalGroupSettingsData copySignalGroupSettingsData(
      final SignalGroupSettingsData signalGroupSettings, final SignalControlDataFactory fac) {
    SignalGroupSettingsData newSettings =
        fac.createSignalGroupSettingsData(signalGroupSettings.getSignalGroupId());
    newSettings.setOnset(signalGroupSettings.getOnset());
    newSettings.setDropping(signalGroupSettings.getDropping());
    return newSettings;
  }

  public static int calculateGreenTimeSeconds(
      SignalGroupSettingsData settings, int cylceTimeSeconds) {
  ...
  public static void fillIntersectionDirectionsForSingleCrossingScenario(
      IntersectionDirections directionsForTheIntersection,
    ...
    for (int i = 0; i < dirNS.length; i++) {
      ...
      Direction dirItself =
          directionsForTheIntersection.getDirections().get(Id.create(dirNS[i], Direction.class));
