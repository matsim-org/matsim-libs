...
final class LinkSensor {
  ...
  /**
   * Calculate the average number of vehicles per second accourding to the number of vehicles which
   * passed the link from the beginning of time. Average is calculated from the first time a vehicle
   * entered the link on.
   *
   * @param link
   * @author dgrether
  ...
  public void registerAverageVehiclesPerSecondToMonitor(
      double lookBackTime, double timeBucketCollectionDuration) {
    if (!doAverageVehiclesPerSecondMonitoring) {
      ...
      this.numOfBucketsNeededForLookback =
          (int) Math.ceil(lookBackTime / timeBucketCollectionDuration);
    }
  }
  ...
  public int getNumberOfCarsInDistance(Double distanceMeter, double now) {
    Map<Id<Vehicle>, CarLocator> distSpecificCarLocators =
        this.distanceMeterCarLocatorMap.get(distanceMeter);
    int count = 0;
    ...
    if (now > monitoringStartTime) {
      if (lookBackTime == Double.POSITIVE_INFINITY) {
        ...
        if (timeBuckets.size() > 0) {
          // if we have less buckets collected than needed for lookback, we calculate the average
          // only with the buckets we already have.
          avgVehPerSecond =
              timeBuckets.stream().mapToInt(AtomicInteger::intValue).sum()
                  / (timeBuckets.size() * this.timeBucketSize);
        }
  ...
  /**
   * check if: - the current bucket should be closed and a new one should be created and set as
   * currentBucket - there are empty buckets, which we need to add to the list, because there wasn't
   * any vehicles in their collection period
   */
  ...
  /**
   * Queues a bucket to the queue and removes an old one if already enough buckets for desired
   * lookBackTime
   *
  ...
  public void handleEvent(LinkEnterEvent event) {
    ...
    if (this.doDistanceMonitoring) {
      for (Double distance : this.distanceMeterCarLocatorMap.keySet()) {
        Map<Id<Vehicle>, CarLocator> carLocatorPerVehicleId =
            this.distanceMeterCarLocatorMap.get(distance);
        carLocatorPerVehicleId.put(
            event.getVehicleId(), new CarLocator(this.link, event.getTime(), distance));
      }
    }
  }
  ...
  public void handleEvent(PersonEntersVehicleEvent event) {
    ...
    if (this.doDistanceMonitoring) {
      for (Double distance : this.distanceMeterCarLocatorMap.keySet()) {
        Map<Id<Vehicle>, CarLocator> carLocatorPerVehicleId =
            this.distanceMeterCarLocatorMap.get(distance);
        // as vehicles enter links at their downstream end, assume enter time was freespeed travel
        // time ago
        double fs_tt = this.link.getLength() / this.link.getFreespeed();
        carLocatorPerVehicleId.put(
            event.getVehicleId(), new CarLocator(this.link, event.getTime() - fs_tt, distance));
      }
    }
  }
