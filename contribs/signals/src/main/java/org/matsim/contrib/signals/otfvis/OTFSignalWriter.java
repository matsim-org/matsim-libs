...
final class OTFSignalWriter extends OTFLaneWriter {
  ...
  OTFSignalWriter(
      ...
      Lanes lanes,
      Config config,
      SignalSystemsData signalSystemsData,
      SignalGroupsData signalGroupsData,
      SignalGroupStateChangeTracker signalTracker) {
  ...
  private void writeSignalSystems(ByteBuffer out) {
    ...
    for (Id<SignalSystem> systemId :
        this.signalGroups.getSignalGroupDataBySignalSystemId().keySet()) {
      ByteBufferUtils.putString(out, systemId.toString());
      Map<Id<SignalGroup>, SignalGroupData> groups =
          this.signalGroups.getSignalGroupDataBySystemId(systemId);
      ...
      for (SignalGroupData group : groups.values()) {
        ...
        for (Id<Signal> signalId : group.getSignalIds()) {
          ...
          SignalData signal =
              signalSystems.getSignalSystemData().get(systemId).getSignalData().get(signalId);
