 ...
 */
public final class DefaultPlanbasedSignalSystemController extends AbstractSignalController
    implements SignalController {

  private static final Logger log =
      LogManager.getLogger(DefaultPlanbasedSignalSystemController.class);
  ...
  public static final class FixedTimeFactory implements SignalControllerFactory {
    @Override
  ...
  /**
   * Method to be called when a check time is reached. Checks whether the next signal plan has to
   * start or if not whether the active signal plan has to end, though. It also sets next check
   * times.
   *
   * <p>This method may assume some important facts: 1. there is always a next plan (because
   * activated plans are put to the end of the queue again for the next day, see
   * startFirstPlanInQueue) 2. start and end times of signal plans always exist (see
   * validateSignalPlans) 3. the start time of the next plan is always >= the end time of the active
   * plan (or at the next day) (see validateSignalPlans)
   *
  ...
  private void checkActivePlan(double now) {
    ...
    if (nextPlan.getStartTime() == now % (3600 * 24)) {
      ...
      //			if (nextPlan.getStartTime() > (now % (3600*24) +
      // SignalSystemImpl.SWITCH_OFF_SEQUENCE_LENGTH)){
    ...
    } else { // This case is just for completeness. It should not happen when check times are set
      // correctly.
  ...
  /**
   * Correct next start or end time of a signal plan. Needed in case a plan goes over midnight or
   * simulation reaches the next day.
   *
   ...
   * @return the next start or end time of the signal plan in seconds corresponding to the current
   *     day
   */
  ...
  /**
   * Activate the signal plan that is first in the queue. Moves plan from the queues first position
   * to the last position. (Necessary to keep simulating signals when plans go over midnight and/or
   * the simulation lasts for more than 24h.) Determine the time when to check active plans next.
   *
   * @param now current time
   */
  ...
  /**
   * Reset active plan - no plan is active. Determine the time when to check active plans next.
   *
   * @param now current time
   */
  ...
  /**
   * This method checks whether signal plans overlap each other. It also throws an exception if
   * start or end times of signal plans are not specified.
   *
  ...
  /**
   * Assume that no plan covers the hole day. Than there are 4 valid cases how plan 1 and 2 can be
   * ordered within the day such that they do not overlap: 1. [--|1**|--|2**|--] <=> start1 <= end1
   * <= start2 <= end2 2. [*|---|1**|--|2**] <=> end2 <= start1 <= end1 <= start2 3.
   * [--|2**|---|1**|-] <=> start2 <= end2 <= start1 <= end1 4. [**|--|2**|---|1*] <=> end1 <=
   * start2 <= end2 <= start1 This means: If no plan covers the hole day it is 3 of the 4
   * inequalities are fulfilled <=> plans do not overlap.
   *
   * <p>The method throws an UnsupportedOperationException when less than 3 inequalities are
   * fulfilled, i.e. when the plans overlap.
   *
   * @param plan1
   * @param plan2
   */
  private void checkOverlapping(SignalPlan plan1, SignalPlan plan2) {
    if (plan1.getStartTime() % (24 * 3600) == plan1.getEndTime() % (24 * 3600)
        || plan2.getStartTime() % (24 * 3600) == plan2.getEndTime() % (24 * 3600)) {
      throw new UnsupportedOperationException(
          "Signal system "
              + system.getId()
              + " has multiple plans but at least one of them covers the hole day. "
              + "If multiple signal plans are used, they are not allowed to overlap.");
    }

    int counter = 0;
    if (plan1.getStartTime() <= plan1.getEndTime()) counter++;
    if (plan1.getEndTime() <= plan2.getStartTime()) counter++;
    if (plan2.getStartTime() <= plan2.getEndTime()) counter++;
    if (plan2.getEndTime() <= plan1.getStartTime()) counter++;
    if (counter < 3) {
      throw new UnsupportedOperationException(
          "Signal plans "
              + plan1.getId()
  ...
  /**
   * Sorts Signal Plans according to their end times. The plan with the end time coming first after
   * (not at!) simulation start gets the first position. The simulation start time is given in the
   * constructor.
   */
