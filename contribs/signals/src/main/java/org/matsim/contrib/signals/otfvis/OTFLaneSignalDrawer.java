...
final class OTFLaneSignalDrawer extends OTFGLAbstractDrawableReceiver {
  ...
  public void onDraw(GL2 gl) {
    if (this.currentLinkWidthCorrection
        != OTFClientControl.getInstance().getOTFVisConfig().getLinkWidth()) {
      this.currentLinkWidthCorrection =
          OTFClientControl.getInstance().getOTFVisConfig().getLinkWidth();
  ...
  private void drawSignals(
      ...
      Point2D.Double ortho,
      List<VisLinkWLanes> toLinks) {
    double dist = signals.size() - 1;
    Point2D.Double startPoint =
        this.laneModelBuilder.calcPoint(point, ortho, (quadSizeLinkEnd * -dist));
    int i = 0;
  ...
  private void recalculatePositions() {
    ...
    linkWidthCalculator.setLaneWidth(
        OTFClientControl.getInstance().getOTFVisConfig().getEffectiveLaneWidth());
  ...
  private void drawVertex(
      GL2 gl, Point2D.Double startPoint, Point2D.Double endPoint, float lineWidth) {
