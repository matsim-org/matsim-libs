 ...
 */
@Singleton
public class SignalAnalysisTool
  ...
  public void handleEvent(ActivityEndEvent event) {
    if (firstActEndTime == null) firstActEndTime = event.getTime();
  }
  ...
  public void handleEvent(SignalGroupStateChangedEvent event) {
    ...
    // assumption: there is an SignalGroupStateChangedEvent for every signal group at the first
  ...
  public void notifyAfterMobsim(AfterMobsimEvent event) {
    ...
    for (Id<SignalGroup> signalGroupId : lastSwitchesToGreen.keySet()) {
      calculateLastGreenTimeOfTheGroupAndAddToTotalGreen(
          signalGroupId, simEndTime, lastSwitchesToGreen.get(signalGroupId));
      fillBygoneGreenTimeMapForEverySecondSinceLastSwitch(
          signalGroupId, simEndTime, lastSwitchesToGreen.get(signalGroupId), 1);
    }
    lastSwitchesToGreen.clear();
    for (Id<SignalGroup> signalGroupId : lastSwitchesToRed.keySet()) {
      fillBygoneGreenTimeMapForEverySecondSinceLastSwitch(
          signalGroupId, simEndTime, lastSwitchesToRed.get(signalGroupId), 0);
    }
  ...
  private void doBygoneGreenTimeAnalysis(SignalGroupStateChangedEvent event, Double lastSwitch) {
    if (lastSwitch == null) {
      ...
      if (event.getNewState().equals(SignalGroupState.RED)) {
        ...
        calculateLastGreenTimeOfTheGroupAndAddToTotalGreen(
            event.getSignalGroupId(), event.getTime(), lastSwitch);
      }
      fillBygoneGreenTimeMapForEverySecondSinceLastSwitch(
          event.getSignalGroupId(), event.getTime(), lastSwitch, increment);
    }
  }

  private void fillBygoneGreenTimeMapForEverySecondSinceLastSwitch(
      Id<SignalGroup> signalGroupId, double thisSwitch, double lastSwitch, int increment) {
    double lastBygoneSignalGreenTimeInsideMap =
        summedBygoneSignalGreenTimesPerSecond.get(lastSwitch).get(signalGroupId);
    ...
    while (time <= thisSwitch) {
      ...
      summedBygoneSignalGreenTimesPerSecond
          .get(time)
          .put(signalGroupId, lastBygoneSignalGreenTimeInsideMap);
      time++;
    }
  }

  private void calculateLastGreenTimeOfTheGroupAndAddToTotalGreen(
      Id<SignalGroup> signalGroupId, double redSwitch, double lastGreenSwitch) {
  ...
  private void doCycleAnalysis(SignalGroupStateChangedEvent event) {
    // TODO so far, this only works for fixed signals. instead: collect green switches and 'start' a
    // new cycle, when any group switches again to green (empty the list at this moment and start
    // collecting again)
    ...
    if (event
        .getSignalGroupId()
        .equals(firstSignalGroupOfSignalSystem.get(event.getSignalSystemId()))) {
      // increase counter if first signal group of the system gets green
      numberOfCyclesPerSystem.put(
          event.getSignalSystemId(), numberOfCyclesPerSystem.get(event.getSignalSystemId()) + 1);
  ...
  private void addLastSystemCycleTime(Id<SignalSystem> signalSystemId, double cycleStartTime) {
    ...
    sumOfSystemCycleTimes.put(
        signalSystemId, sumOfSystemCycleTimes.get(signalSystemId) + lastCycleTime);
  }
  ...
  public Map<Id<SignalGroup>, Double> calculateAvgSignalGreenTimePerFlexibleCycle() {
    ...
    for (Id<SignalGroup> signalGroupId : totalSignalGreenTime.keySet()) {
      ...
      double avgSignalGreenTime =
          totalSignalGreenTime.get(signalGroupId) / numberOfCyclesPerSystem.get(signalSystemId);
  ...
  public Map<Id<SignalSystem>, Double> calculateAvgFlexibleCycleTimePerSignalSystem() {
    ...
    for (Id<SignalSystem> signalSystemId : sumOfSystemCycleTimes.keySet()) {
      double avgSystemCylceTime =
          sumOfSystemCycleTimes.get(signalSystemId) / numberOfCyclesPerSystem.get(signalSystemId);
  ...
  /**
   * can be used for fixed cycle times with repeating signal groups per cycle too (e.g. for
   * downstream signal)
   *
   * @return
   */
  public Map<Id<SignalGroup>, Double> calculateSignalGreenTimeRatios() {
    ...
    for (Id<SignalGroup> signalGroupId : totalSignalGreenTime.keySet()) {
      double avgSignalGreenTime =
          totalSignalGreenTime.get(signalGroupId) / (this.lastActStartTime - this.firstActEndTime);
