...
public final class SylviaPreprocessData {
  ...
  private static void convertFixedTimePlansToSylviaBasePlans(
      String signalControlInputFile, String signalControlOutputFile)
  ...
  /**
   * Convert old, fixed time signal control data into new sylvia signal control data. The sylvia
   * signal control data will then contain the old fixed time signal control plan and the new
   * shortened sylvia signal control plan.
   *
   * @param controlData old, fixed-time signal control
   * @param sylviaSignalControlData signal control object, where the sylvia signal control should be
   *     stored in
   */
  public static void convertSignalControlData(
      final SignalControlData controlData, SignalControlData sylviaSignalControlData) {
    for (SignalSystemControllerData controllerData :
        controlData.getSignalSystemControllerDataBySystemId().values()) {
      SignalSystemControllerData newControllerData =
          sylviaSignalControlData
              .getFactory()
              .createSignalSystemControllerData(controllerData.getSignalSystemId());
      ...
      for (SignalPlanData signalPlan : controllerData.getSignalPlanData().values()) {
        ...
        newControllerData.addSignalPlanData(
            SignalUtils.copySignalPlanData(
                signalPlan,
  ...
  private static SignalPlanData convertSignalPlanData(final SignalPlanData fixedTimePlan) {
    SignalPlanData newPlan =
        SignalUtils.copySignalPlanData(
            fixedTimePlan,
            Id.create(SYLVIA_PREFIX + fixedTimePlan.getId().toString(), SignalPlan.class));
    ...
    Collections.sort(
        settingsSortedByDropping,
        new Comparator<SignalGroupSettingsData>() {
    ...
    SignalGroupSettingsData nextDropping =
        droppingIterator.hasNext() ? droppingIterator.next() : null;
    while (nextDropping != null) {
      // look for the next dropping pair with a minimum of MIN_GREEN_SECONDS seconds inbetween
      while (nextDropping != null
          && thisDropping.getDropping() - nextDropping.getDropping() <= MIN_GREEN_SECONDS) {
      ...
      // look for the next onset that is before the current dropping (note: there always is one,
      // because no dropping is scheduled at second 0 but at least one offset is scheduled for
      // second 0)
      ...
      int shrinkStart =
          Math.max(nextOnset.getOnset(), nextDropping == null ? 0 : nextDropping.getDropping())
              + MIN_GREEN_SECONDS;
      ...
      log.info(
          "Shrink plan "
              ...
              + " seconds at dropping of signal group "
              + thisDropping.getSignalGroupId()
              + ". Shift all settings behind "
              + shrinkStart
              + " accordingly. The next dropping is signal group "
              + (nextDropping == null ? "null" : nextDropping.getSignalGroupId())
              + ". The next onset is signal group "
              + nextOnset.getSignalGroupId());
  ...
  /**
   * shift plan such that longest green time starts at second zero. adapt offset accordingly. check
   * that no dropping happens at second 0 but cycleTime
   */
  private static SignalPlanData shiftPlan(SignalPlanData signalPlan) {
    ...
    for (SignalGroupSettingsData setting :
        signalPlan.getSignalGroupSettingsDataByGroupId().values()) {
    ...
    int shiftBy =
        signalPlan.getSignalGroupSettingsDataByGroupId().get(longestGreenGroup).getOnset();
    log.info(
        "The setting with the longest green time is signal group "
            + longestGreenGroup
            + " with a green time of "
            + longestGreenTime
            + " seconds. Shift start to second "
            + shiftBy);
    if (shiftBy == 0) {
      return signalPlan;
    }
    signalPlan.setOffset(
        (signalPlan.getOffset() - shiftBy + signalPlan.getCycleTime()) % signalPlan.getCycleTime());
    for (SignalGroupSettingsData setting :
        signalPlan.getSignalGroupSettingsDataByGroupId().values()) {
  ...
  /**
   * move all droppings and onsets behind shrinkStart by secondsToBeRemoved seconds to the left,
   * i.e. shrink the whole signal plan by secondsToBeRemoved seconds beginning at second
   * shrinkStart. Move the offset accordingly.
   */
  private static void shrinkSignalPlan(
      SignalPlanData signalPlan, int shrinkStart, int secondsToBeRemoved) {
  ...
  public static void main(String[] args)
    ...
    SylviaPreprocessData.convertFixedTimePlansToSylviaBasePlans(
        signalControlFile, signalControlOutFile);
  }
}
