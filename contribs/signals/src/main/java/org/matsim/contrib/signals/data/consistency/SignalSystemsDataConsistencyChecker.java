...
public final class SignalSystemsDataConsistencyChecker {
  ...
  public SignalSystemsDataConsistencyChecker(
      Network network, Lanes lanes, SignalsData signalsData) {
  ...
  private void checkWhetherTwoSignalsAreIdentical() {
    ...
    for (SignalSystemData system : signalSystems.getSignalSystemData().values()) {
      ...
      // TODO note, that if a signal gets removed from the signalsData, it also has to be removed
      // from the groups and from the control.
        ...
        if (linkToSignalMatching.get(linkId).size() > 1) {
          ...
          if (!lanes.getLanesToLinkAssignments().containsKey(linkId)) {
            log.warn(
                "Link "
                    + linkId
                    + " does not have lanes but more then one signal. This does not make sense since all vehicles will queue in one queue anyway.");
            ...
            for (SignalData signal : linkToSignalMatching.get(linkId)) {
              if (signal.getLaneIds() == null || signal.getLaneIds().isEmpty()) {
                log.warn(
                    "Link "
                        + linkId
                        + " has lanes and multiple signals but signal "
                        + signal.getId()
                        + " controls the whole link.");
                    ...
                    if (signal.getLaneIds().equals(otherSignal.getLaneIds())) {
                      log.warn(
                          "Signal "
                              ...
                              + " on link "
                              + linkId
                              + " controls exactly the same lanes as signal "
                              + otherSignal.getId()
  ...
  private void removeMalformedSignalSystems() {
    ...
    for (Tuple<Id<Signal>, Id<SignalSystem>> tuple : malformedSignals) {
      signalsData
          ...
          .get(tuple.getSecond())
          .getSignalData()
          .remove(tuple.getFirst());
      for (SignalGroupData sigGroup :
          this.signalsData
              .getSignalGroupsData()
  ...
  private void removeEmptyGroups() {
    ...
    for (Map<Id<SignalGroup>, SignalGroupData> sigGroups :
        this.signalsData.getSignalGroupsData().getSignalGroupDataBySignalSystemId().values()) {
    ...
    for (Tuple<Id<SignalGroup>, Id<SignalSystem>> groupSystemTuple : emptyGroups) {
      signalsData
          .getSignalGroupsData()
          .getSignalGroupDataBySignalSystemId()
          .get(groupSystemTuple.getSecond())
          .remove(groupSystemTuple.getFirst());
      for (SignalPlanData sigPlanData :
          signalsData
  ...
  private void removeEmptyControlSettings() {
    ...
    for (SignalSystemControllerData controller :
      ...
      Map<Id<SignalGroup>, SignalGroupData> signalGroups =
          this.signalsData
              .getSignalGroupsData()
              .getSignalGroupDataBySystemId(controller.getSignalSystemId());
      Iterator<Entry<Id<SignalPlan>, SignalPlanData>> planIterator =
          controller.getSignalPlanData().entrySet().iterator();
      while (planIterator.hasNext()) {
        SignalPlanData plan = planIterator.next().getValue();
        if (plan.getSignalGroupSettingsDataByGroupId() != null) {
          Iterator<Entry<Id<SignalGroup>, SignalGroupSettingsData>> planSettingIterator =
              plan.getSignalGroupSettingsDataByGroupId().entrySet().iterator();
        ...
        if (plan.getSignalGroupSettingsDataByGroupId() == null) {
          // this check against null is again necessary, because by the step above plan could have
          // become empty
          // remove this plan
          planIterator.remove();
        }
      ...
      for (SignalData signal : system.getSignalData().values()) {
        if (!this.network.getLinks().containsKey(signal.getLinkId())) {
          ...
          log.error(
              "\t\tSignalData Id: "
                  + signal.getId()
                  + " of SignalSystemData Id: "
                  + system.getId()
                  + " is located at Link Id: "
                  + signal.getLinkId()
                  + " but this link is not existing in the network!");
          ...
          if (!this.lanes.getLanesToLinkAssignments().containsKey(signal.getLinkId())) {
            ...
            log.error(
                "\t\tSignalData Id: "
                    ...
                    + " of SignalSystemData Id: "
                    + system.getId()
                    + " is located at some lanes of Link Id: "
                    + signal.getLinkId()
                    + " but there is no LanesToLinkAssignemt existing in the LaneDefinitions.");
            malformedSignals.add(new Tuple<>(signal.getId(), system.getId()));
          } else {
            LanesToLinkAssignment l2l =
                this.lanes.getLanesToLinkAssignments().get(signal.getLinkId());
