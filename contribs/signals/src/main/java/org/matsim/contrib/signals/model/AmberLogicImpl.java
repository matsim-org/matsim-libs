...
/**
 * AmberLogic for a simulation with amber times (i.e. a simulation where isUseAmbertimes in the
 * signals module is enabled). It uses amber times defined in the amber times file (signal specific
 * or global defaults, respectively).
 *
 * @author dgrether
 */
public final class AmberLogicImpl implements AmberLogic {
  ...
  public Set<SignalGroupStateChangeRequest> processDropping(
    ...
    if (amber != null) {
      ...
      SignalGroupStateChangeRequest amberRequest =
          new SignalGroupStateChangeRequestImpl(signalGroupId, SignalGroupState.YELLOW, now);
      ret.add(amberRequest);
      SignalGroupStateChangeRequest redRequest =
          new SignalGroupStateChangeRequestImpl(signalGroupId, SignalGroupState.RED, now + amber);
      ret.add(redRequest);
  ...
  public Set<SignalGroupStateChangeRequest> processOnsets(
    ...
    if (redAmber != null) {
      ...
      SignalGroupStateChangeRequest amberRequest =
          new SignalGroupStateChangeRequestImpl(signalGroupId, SignalGroupState.REDYELLOW, now);
      ret.add(amberRequest);
      SignalGroupStateChangeRequest greenRequest =
          new SignalGroupStateChangeRequestImpl(
              signalGroupId, SignalGroupState.GREEN, now + redAmber);
