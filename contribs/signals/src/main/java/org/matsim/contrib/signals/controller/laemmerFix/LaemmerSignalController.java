 ...
 * @author nkuehnel
 */
public final class LaemmerSignalController extends AbstractSignalController
  ...
  public static final class LaemmerFactory implements SignalControllerFactory {
    ...
    public SignalController createSignalSystemController(SignalSystem signalSystem) {
      SignalController controller =
          new LaemmerSignalController(scenario, sensorManager, downstreamSensor);
      controller.setSignalSystem(signalSystem);
      return controller;
    }
  }

  private LaemmerSignalController(
  ...
  public void updateState(double now) {
    ...
    if (activeRequest != null
        && activeRequest.signal.group.getState().equals(SignalGroupState.GREEN)) {
  ...
  /**
   * checks whether the active regulation (stabilization) has to be stopped (i.e. removed from the
   * regulation queue)
   */
  private void updateActiveRegulation(double now) {
    if (activeRequest != null
        && !regulationQueue.isEmpty()
        && regulationQueue.peek().equals(activeRequest.signal)) {
        ...
        for (LaemmerSignal signal : laemmerSignals) {
          if (signal.index > index) {
            ...
            if (!laemmerConfig.isCheckDownstream()
                || downstreamSensor.allDownstreamLinksEmpty(system.getId(), signal.group.getId())) {
              max = signal;
              index = signal.index;
            }
          }
  ...
  /**
   * updates the parameters needed for calculation: 1. determines the current representative
   * driveway of each signal 2. sums up the outflow capacity of all signals of the system 3.
   * calculates the remaining cycle time tIdle
   */
  private void updateRepresentativeDriveways(double now) {
    ...
    for (LaemmerSignal signal : laemmerSignals) {
      ...
      tIdle -=
          Math.max(
              signal.determiningLoad * laemmerConfig.getDesiredCycleTime()
                  + laemmerConfig.getIntergreenTime(),
  ...
  private void initializeSensoring() {
    for (SignalGroup group : this.system.getSignalGroups().values()) {
      for (Signal signal : group.getSignals().values()) {
        if (signal.getLaneIds() != null && !(signal.getLaneIds().isEmpty())) {
          for (Id<Lane> laneId : signal.getLaneIds()) {
            this.sensorManager.registerNumberOfCarsOnLaneInDistanceMonitoring(
                signal.getLinkId(), laneId, 0.);
            this.sensorManager.registerAverageNumberOfCarsPerSecondMonitoringOnLane(
                signal.getLinkId(),
                laneId,
                laemmerConfig.getLookBackTime(),
                laemmerConfig.getTimeBucketSize());
          }
        }
        ...
        this.sensorManager.registerAverageNumberOfCarsPerSecondMonitoring(
            signal.getLinkId(), laemmerConfig.getLookBackTime(), laemmerConfig.getTimeBucketSize());
      }
    }
    if (laemmerConfig.isCheckDownstream()) {
  ...
  class LaemmerSignal {
    ...
    LaemmerSignal(SignalGroup signalGroup) {
      ...
      // calculate outflow capacity once (in reality one also has to initialize this value and
      // cannot update it every second)
      for (Signal signal : group.getSignals().values()) {
        if (signal.getLaneIds() != null
            && !signal.getLaneIds().isEmpty()
          ...
          double linkOutflowPerS =
              network.getLinks().get(signal.getLinkId()).getCapacity()
                  * config.qsim().getFlowCapFactor()
                  / 3600;
    ...
    private void determineRepresentativeDriveway(double now) {
      ...
      for (Signal signal : group.getSignals().values()) {
        if (signal.getLaneIds() != null
          ...
          /* bound load by 1 (i.e. 100%) as calculation of nCrit etc depend on that. tt, dec'18 */
          double tempLoad =
              Math.min(1, arrivalRate / linkOutflowCapacitiesPerS.get(signal.getLinkId()));
          if (tempLoad >= this.determiningLoad) {
            this.determiningLoad = tempLoad;
            this.determiningArrivalRate = arrivalRate;
            this.determiningLane = null;
      ...
      if (activeRequest != null && this.equals(activeRequest.signal)) {
        ...
        for (double i = remainingInBetweenTime; i < laemmerConfig.getIntergreenTime(); i++) {
          for (Signal signal : group.getSignals().values()) {
            if (signal.getLaneIds() != null
                && !signal.getLaneIds().isEmpty()
                && lanes.getLanesToLinkAssignments().get(signal.getLinkId()).getLanes().size()
                    > 1) {
              for (Id<Lane> laneId : signal.getLaneIds()) {
                waitingTimeSum +=
        ...
        for (Signal signal : group.getSignals().values()) {
          if (signal.getLaneIds() != null
            ...
            for (Id<Lane> laneId : signal.getLaneIds()) {
              n +=
                  getNumberOfExpectedVehiclesOnLane(
                      now + laemmerConfig.getIntergreenTime(), signal.getLinkId(), laneId);
            }
          } else {
            n +=
                getNumberOfExpectedVehiclesOnLink(
                    now + laemmerConfig.getIntergreenTime(), signal.getLinkId());
          }
        }
        if (n > 0) {
        ...
        for (Signal signal : this.group.getSignals().values()) {
          if (signal.getLaneIds() != null
            ...
            for (Id<Lane> laneId : signal.getLaneIds()) {
              double nTemp =
                  getNumberOfExpectedVehiclesOnLane(
                      now + laemmerConfig.getIntergreenTime() + laemmerConfig.getMinGreenTime(),
                      signal.getLinkId(),
                      laneId);
              nExpected += nTemp;
              double tempGreenTime = nTemp / laneOutflowCapacitiesPerS.get(laneId);
    ...
    private void updateStabilization(double now) {
      ...
      double nCrit =
          determiningArrivalRate
              * laemmerConfig.getDesiredCycleTime()
      ...
      if (n > 0 && n >= nCrit) {
        ...
        if (!laemmerConfig.isCheckDownstream()
            || downstreamSensor.allDownstreamLinksEmpty(system.getId(), group.getId())) {
          regulationQueue.add(this);
          this.regulationTime =
              Math.max(
                  Math.rint(
                      determiningLoad * laemmerConfig.getDesiredCycleTime()
                          + (signalOutflowCapacityPerS / systemOutflowCapacity) * tIdle),
                  laemmerConfig.getMinGreenTime());
          this.stabilize = true;
        }
      }
    ...
    public void getStepStats(StringBuilder builder, double now) {
      ...
      builder
          .append(this.group.getState().name() + ";")
          .append(this.index + ";")
