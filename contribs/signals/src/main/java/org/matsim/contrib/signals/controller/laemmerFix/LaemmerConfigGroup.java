...
public final class LaemmerConfigGroup extends ReflectiveConfigGroup {
  ...
  private static final String DESIRED_CYCLE_TIME_CMT =
      "Cycle time that is aimed by the stabilizing regime.";
  private static final String MAX_CYCLE_TIME_CMT =
      "Maximal cycle time that is allowed by the stabilizing regime.";
  private static final String INTERGREEN_TIME_CMT =
      "Red time that has to be ensured between the green phases of two conflicting directions (exluding amber time).";
  private static final String MIN_GREEN_TIME_CMT =
      "Minimal time that a signal has to show green once it is switched to green.";
  ...
  public Map<String, String> getComments() {
     ...
     */
    final String ACTIVE_REGIME_CMT =
        "Regime that is used for the laemmer control. "
            + "The options are: "
            + Arrays.stream(Regime.values())
                .map(regime -> " " + regime.toString())
  ...
  public enum StabilizationStrategy {
    USE_MAX_LANECOUNT,
    PRIORIZE_HIGHER_POSITIONS,
    COMBINE_SIMILAR_REGULATIONTIME,
    HEURISTIC
  };
  ...
  /**
   * size of timeBuckets in seconds for live arrival rates in LaneSensor and LinkSensor. Setting
   * this only has an effect if lookBackTime is different to Double.POSITIVE_INFINITY.
   */
  private double timeBucketSize = 30.;

  /**
   * lookBackTime in seconds for live arrival rates in LaneSensor and LinkSensor. If you use
   * Double.POSITIVE_INFINITY, the sensor will start monitoring the arrival rates newly each time
   * the link is empty; as long as the link is not empty, it simply averages all past arrivals.
   */
  ...
  /**
   * tIdle is determined by the highest determinedLoad of the current regulationPhase and the
   * highest determinedLoad of the top n flows not in the regulationPhase, where n is the number of
   * estimatedPhases minus 1. By default the flows are grouped by their signals beforehand, keeping
   * only the flow with the highest load per signal. This prevents that similar flows on two lanes
   * are counted twice, since you can assume that they will be signalized together. However, in some
   * cases a flow can be signalized by two signals and you can probably not trust on the
   * prerequisite, that they will be signalized together. An example for such a case would be
   * right-turning flows, which can be signalized for some time alone and for some other time
   * together with the straight flow. If your network includes such cases, you need set this to
   * false. As a side-effect from setting it to false will be a under-estimate of tIdle in some
   * cases.
   */
  private boolean determineMaxLoadForTIdleGroupedBySignals = true;

  /**
   * if a signals showed green during intergreenTime, stabilization time will be reduced after
   * intergreenTime
   */
  ...
  /**
   * If true subphases will be removed from the set of phases. Phase A is a subphase of phase B it
   * the set of SignalGroups of A if fully covered by the set of SignalGroups of B.
   *
  ...
  /**
   * Configure the sensor for live arrival rates. Live arrival Rate can be calculated from the time,
   * the first car entered the Link/Lane until now or by getting an average from lookBackTime. If
   * using last option, lookBackTime will be splitted in buckets with duration timeBucketsSize. Only
   * finished buckets will be used to calculate the average.
   *
   * @param lookBackTime For which duration of passed time the average should be calculated. Set to
   *     Double.POSITIVE_INFINITY to calculate from time the first car enters the link on.
   * @param timeBucketSize Resolution of lookBackTime. Average is calculated only with full time
   *     buckets. Set to Double.POSITIVE_INFINITY to calculate from time the first car enters the
   *     link on.
   */
  public void setTimeBucketParametersForLiveArrivalRates(
      double lookBackTime, double timeBucketSize) {
    this.lookBackTime = lookBackTime;
    this.timeBucketSize = timeBucketSize;
  }

  /**
   * If signals showed green during the intergreenTime, there regulation time can be shorten after
   * intergreenTime
   *
