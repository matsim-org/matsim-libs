 ...
 *
 * <p>It updates the fixed signal control depending on the average delay every i-th iteration,
 * whereby i is defined in the variable INTERVAL.
 *
 * @author tthunig
 */
public class SimpleResponsiveSignal extends AbstractSignalController
  ...
  public static final class SimpleResponsiveSignalFactory implements SignalControllerFactory {
  ...
  public void notifyAfterMobsim(AfterMobsimEvent event) {
    ...
    if (event.getIteration() % INTERVAL == 0
        && event.getIteration() != scenario.getConfig().controller().getFirstIteration()) {
      computeDelays(event);
  ...
  private void updateSignals() {
    ...
    double delaySignalGroup1 =
        link2avgDelay.get(Id.createLinkId("2_3")) + link2avgDelay.get(Id.createLinkId("4_3"));
    double delaySignalGroup2 =
        link2avgDelay.get(Id.createLinkId("7_3")) + link2avgDelay.get(Id.createLinkId("8_3"));
    ...
    if (greenTimeShift != 0
        && group1Setting.getDropping() + greenTimeShift > 0
        && group2Setting.getOnset() + greenTimeShift < 55) {
  ...
  public void setSignalSystem(SignalSystem signalSystem) {
    ...
    delegatePlanbasedSignalController =
        new FixedTimeFactory().createSignalSystemController(signalSystem);
  }
