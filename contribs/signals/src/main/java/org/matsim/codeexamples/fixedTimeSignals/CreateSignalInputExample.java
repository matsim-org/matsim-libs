 ...
 *
 * <p>See also the picture of the scenario in the same package as the input files
...
public class CreateSignalInputExample {
   ...
   * Furthermore groups for the signals are created that specify which signals will always have the
   * same control.
   *
   * @param systems the so far empty object for information about signalized intersections
   * @param groups the so far empty object for information about signals that are controlled
   *     together as groups
   */
  private void createSignalSystemsAndGroups(SignalSystemsData systems, SignalGroupsData groups) {
    ...
    SignalSystemData sys =
        systems.getFactory().createSignalSystemData(Id.create("3", SignalSystem.class));
  ...
  /**
   * Create a fixed time traffic signal control for all signal groups in the scenario, i.e. specify
   * when their signals show green or red.
   *
   * <p>Each signal system (signalized intersection) is equipped with a control, namely each with
   * the same. The control contains the following information. - A cylce time of 120 seconds. - An
   * offset (for green waves) of 0 seconds. - Each direction gets green for second 0 to 55 within
   * the cycle.
   *
  ...
  private void createSignalControl(SignalControlData control) {
    ...
    for (Id<SignalSystem> id : ids) {
      ...
      SignalGroupSettingsData settings2 =
          control.getFactory().createSignalGroupSettingsData(Id.create("2", SignalGroup.class));
      ...
      SignalPlanData plan2 =
          control.getFactory().createSignalPlanData(Id.create("2", SignalPlan.class));
  ...
  /**
   * Set up the config and scenario, create signal information and write them to file as input for
   * further simulations.
   *
   * @throws IOException
   */
  public void run(String outputDir) throws IOException {
     ...
     */
    this.createSignalSystemsAndGroups(
        signalsData.getSignalSystemsData(), signalsData.getSignalGroupsData());
