...
/**
 * This example starts the simulation of a single crossing scenario where the intersection is
 * signalized by traffic-adaptive signals. The logic of the traffic-adaptive signals is based on the
 * basic version of Stefan Laemmer from his phd from 2007 and can be found in
 * LaemmerSignalController.
 *
 * <p>If you want to use other implementations of traffic-adaptive signals you have to adjust the
 * identifiers in the signal control file and add a factory class for your specific implementation
 * to the SignalsModule (see comment below).
 *
 * @author tthunig
 */
public class RunAdaptiveSignalsExample {
  // do not change name of class; EWGT-paper (ThunigKuehnelNagel2018...) refers to it.  theresa,
  // aug'18
  ...
  public static void main(String[] args) {
    ...
    if (args != null && args.length != 0) {
      log.info(
          "Your config file "
              + args[0]
              + " will be loaded and run with signals and otfvis-visualization.");
      configFileName = args[0];
    }
    run(
        configFileName,
        "output/runAdaptiveSignalsExampleOutput/",
        true); // The run method is extracted so that a test can operate on it.
  }
  ...
  public static void run(String configFileName, String outputDir, boolean visualize) {
    ...
    LaemmerConfigGroup laemmerConfigGroup =
        ConfigUtils.addOrGetModule(config, LaemmerConfigGroup.GROUP_NAME, LaemmerConfigGroup.class);
