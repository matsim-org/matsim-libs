...
public class CreateSignalInputWithLanesExample {
  ...
  private static final String INPUT_DIR =
      "./examples/tutorial/example90TrafficLights/createSignalInput/";
  ...
  private void createGroupsAndSystem2(
      Scenario scenario, SignalSystemsData systems, SignalGroupsData groups) {
    SignalSystemData sys =
        systems.getFactory().createSignalSystemData(Id.create("2", SignalSystem.class));
    systems.addSignalSystemData(sys);
    SignalSystemsDataFactory factory = systems.getFactory();

    SignalUtils.createAndAddSignal(
        sys,
        factory,
        Id.create("1", Signal.class),
        Id.createLinkId("12"),
        Arrays.asList(Id.create("12.l", Lane.class)));
    SignalUtils.createAndAddSignal(
        sys,
        factory,
        Id.create("2", Signal.class),
        Id.createLinkId("12"),
        Arrays.asList(Id.create("12.r", Lane.class)));
    SignalUtils.createAndAddSignal(
        sys, factory, Id.create("3", Signal.class), Id.createLinkId("32"), null);
    SignalUtils.createAndAddSignal(
        sys, factory, Id.create("4", Signal.class), Id.createLinkId("72"), null);

    // create a signal group for every signal
    SignalUtils.createAndAddSignalGroups4Signals(groups, sys);
  }

  private void createGroupsAndSystem5(
      Scenario scenario, SignalSystemsData systems, SignalGroupsData groups) {
    SignalSystemData sys =
        systems.getFactory().createSignalSystemData(Id.create("5", SignalSystem.class));
    systems.addSignalSystemData(sys);
    SignalSystemsDataFactory factory = systems.getFactory();
    ...
    SignalUtils.createAndAddSignal(
        sys, factory, Id.create("3", Signal.class), Id.createLinkId("45"), null);
    SignalUtils.createAndAddSignal(
        sys, factory, Id.create("4", Signal.class), Id.createLinkId("85"), null);
  ...
  private void createSystemControl(
    ...
    plan.addSignalGroupSettings(
        SignalUtils.createSetting4SignalGroup(
            fac, Id.create("1", SignalGroup.class), onset1, dropping1));
    plan.addSignalGroupSettings(
        SignalUtils.createSetting4SignalGroup(
            fac, Id.create("2", SignalGroup.class), onset1, dropping1));
    plan.addSignalGroupSettings(
        SignalUtils.createSetting4SignalGroup(
            fac, Id.create("3", SignalGroup.class), onset2, dropping2));
    plan.addSignalGroupSettings(
        SignalUtils.createSetting4SignalGroup(
            fac, Id.create("4", SignalGroup.class), onset2, dropping2));
  }
  ...
  private void createLanes(Lanes lanes) {
    ...
    LanesToLinkAssignment lanesForLink12 =
        factory.createLanesToLinkAssignment(Id.createLinkId("12"));
    lanes.addLanesToLinkAssignment(lanesForLink12);

    // original lane, i.e. lane that starts at the link from node and leads to all other lanes of
    // the link
    LanesUtils.createAndAddLane(
        lanesForLink12,
        factory,
        Id.create("12.ol", Lane.class),
        LANE_CAPACITY,
        LINK_LENGTH,
        0,
        NO_LANES,
        null,
        Arrays.asList(Id.create("12.l", Lane.class), Id.create("12.r", Lane.class)));

    // left turning lane (alignment 1)
    LanesUtils.createAndAddLane(
        lanesForLink12,
        factory,
        Id.create("12.l", Lane.class),
        LANE_CAPACITY,
        LANE_LENGTH,
        1,
        NO_LANES,
        Collections.singletonList(Id.create("23", Link.class)),
        null);
    ...
    LanesUtils.createAndAddLane(
        lanesForLink12,
        factory,
        Id.create("12.r", Lane.class),
        LANE_CAPACITY,
        LANE_LENGTH,
        -1,
        NO_LANES,
        Collections.singletonList(Id.create("27", Link.class)),
        null);

    // create lanes for link 65
    LanesToLinkAssignment lanesForLink65 =
        factory.createLanesToLinkAssignment(Id.create("65", Link.class));
    ...
    LanesUtils.createAndAddLane(
        lanesForLink65,
        factory,
        Id.create("65.r", Lane.class),
        LANE_CAPACITY,
        LANE_LENGTH,
        -1,
        NO_LANES,
        Collections.singletonList(Id.create("54", Link.class)),
        null);
    ...
    LanesUtils.createAndAddLane(
        lanesForLink65,
        factory,
        Id.create("65.l", Lane.class),
        LANE_CAPACITY,
        LANE_LENGTH,
        1,
        NO_LANES,
        Collections.singletonList(Id.create("58", Link.class)),
        null);
  }
  ...
  public void run(String outputDir) throws IOException {
    ...
    this.createGroupsAndSystem2(
        scenario, signalsData.getSignalSystemsData(), signalsData.getSignalGroupsData());
    this.createGroupsAndSystem5(
        scenario, signalsData.getSignalSystemsData(), signalsData.getSignalGroupsData());
    this.createSystemControl(
        signalsData.getSignalControlData(),
    ...
    log.info(
        "Visualize scenario by calling VisTrafficSignalScenarioWithLanes in the same package.");
  }
