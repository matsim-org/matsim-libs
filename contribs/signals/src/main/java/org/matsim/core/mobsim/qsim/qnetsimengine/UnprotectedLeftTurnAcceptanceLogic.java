...
/**
 * This class extends the SignalTurnAcceptanceLogic by also checking for oncoming traffic: If a turn
 * is physically possible and the corresponding signal shows green, additionally, a vehicle is only
 * accepted for the turn, if no prioritized oncoming traffic is crossing the intersection at the
 * same time. Otherwise, the vehicle has to wait (AcceptTurn.WAIT is returned).
 *
 * @author tthunig
 */
final class UnprotectedLeftTurnAcceptanceLogic implements TurnAcceptanceLogic {
  ...
  public AcceptTurn isAcceptingTurn(
    ...
    if (conflicts.getConflictsPerNode().containsKey(currentLink.getToNode().getId())) {
       ...
       */
      IntersectionDirections intersectionConflicts =
          conflicts.getConflictsPerNode().get(currentLink.getToNode().getId());
      ...
      for (Id<Direction> rightOfWayDirId : thisDirection.getDirectionsWithRightOfWay()) {
        ...
        for (QLaneI rightOfWayQLane : rightOfWayLink.getOfferingQLanes()) {
          // this gives us all lanes on the link from which the direction starts -> if multiple
          // lanes exist, check whether the lane leads into the correct direction
          if (lanes != null
              && lanes.getLanesToLinkAssignments().containsKey(rightOfWayDir.getFromLink())) {
            // lanes for this link exist
            Lane rightOfWayLane =
                lanes
                    .getLanesToLinkAssignments()
                    .get(rightOfWayDir.getFromLink())
                    .getLanes()
                    .get(rightOfWayQLane.getId());
          ...
          if (rightOfWayQLane instanceof SignalizeableItem
              ...
              && firstVehOnLink != null
              && firstVehOnLink.getEarliestLinkExitTime() <= now) {
