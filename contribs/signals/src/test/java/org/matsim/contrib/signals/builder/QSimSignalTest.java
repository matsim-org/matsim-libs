...
/**
 * Simple test case for the Controler and or QSim and the default signal system implementation. One
 * agent drives one round a simple test network.
 *
 * @author dgrether
 */
public class QSimSignalTest
    implements LinkEnterEventHandler,
        SignalGroupStateChangedEventHandler,
        LaneEnterEventHandler,
        LaneLeaveEventHandler {

  private static final Logger log = LogManager.getLogger(QSimSignalTest.class);
  ...
  public void testSignalSystems1AgentGreenAtSec100() {
    ...
    SignalGroupSettingsData groupData =
        planData.getSignalGroupSettingsDataByGroupId().get(fixture.signalGroupId100);
  ...
  /**
   * Tests the setup with a traffic light that shows red less than the specified intergreen time of
   * five seconds.
   */
  ...
  /**
   * Tests the setup with a traffic light which red time corresponds to the specified intergreen
  ...
  public void testConflictingDirectionsNoAbortOneAgentDriving() {
    ...
    Scenario scenario = fixture.createAndLoadTestScenarioTwoSignals(true);
    SignalsData signalsData = (SignalsData) scenario.getScenarioElement(SignalsData.ELEMENT_NAME);
    SignalGroupSettingsData group100setting =
        signalsData
            .getSignalControlData()
            .getSignalSystemControllerDataBySystemId()
            .get(fixture.signalSystemId2)
            .getSignalPlanData()
            .get(fixture.signalPlanId2)
            .getSignalGroupSettingsDataByGroupId()
            .get(fixture.signalGroupId100);
    group100setting.setOnset(15);

    runQSimWithSignals(scenario, false);
  }

  private void runQSimWithSignals(final Scenario scenario, boolean handleEvents)
