...
public class DefaultPlanbasedSignalSystemControllerIT {
  ...
  private static final Logger log =
      LogManager.getLogger(DefaultPlanbasedSignalSystemControllerIT.class);
  ...
  public void test2SequentialPlansCompleteDay() {
    ...
    Assert.assertEquals(
        "First signal state event unexpected.",
        0.0,
        signalAnalyzer.getFirstSignalEventTime(),
        MatsimTestUtils.EPSILON);
    //		Assert.assertEquals("Number of plan start events is wrong.", 2,
    // signalAnalyzer.getNumberOfPlanStartEvents());
    //		Assert.assertEquals("Time when last plan starts is wrong.", 3600*1,
    // signalAnalyzer.getLastPlanStartEventTime(), MatsimTestUtils.EPSILON);
    Assert.assertNull(
        "There was an unexpected event that switches off signals.",
        signalAnalyzer.getLastSignalOffEventTime());
    Assert.assertEquals(
        "Number of signal off events is wrong.", 0, signalAnalyzer.getNumberOfOffEvents());
    // test if signal plans are both running
    Assert.assertEquals(
        "Cycle time of first signal plan wrong.",
        120,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(0),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Cycle time of second signal plan wrong.",
        60,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(1),
        MatsimTestUtils.EPSILON);
  }

  @Test
  public void test2SequentialPlansUncompleteDayEnd() {
    SignalEventAnalyzer signalAnalyzer =
        (new ScenarioRunner(0.0, 3600 * 1.0, 3600 * 1.0, 3600 * 2.0)).run();
    ...
    log.info(
        "Number of signal events after 2am " + signalAnalyzer.getNumberOfSignalEventsInHour(2));
    // test time when signal plans are switched on and off
    Assert.assertEquals(
        "First signal state event unexpected.",
        0.0,
    ...
    // signalAnalyzer.getLastPlanStartEventTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Time when signals are finally switched off is wrong.",
        3600 * 2,
        signalAnalyzer.getLastSignalOffEventTime(),
        5 + MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Number of signal off events is wrong.", 1, signalAnalyzer.getNumberOfOffEvents());
    Assert.assertEquals(
        "Signals where unexpectedly switched on after 2am.",
        0,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(2),
        MatsimTestUtils.EPSILON);
    Assert.assertTrue(
        "Signals where unexpectedly switched on after 2am.",
        3 > signalAnalyzer.getNumberOfSignalEventsInHour(2));
  ...
  public void test2SequentialPlansUncompleteDayStart() {
    SignalEventAnalyzer signalAnalyzer =
        (new ScenarioRunner(3600 * 1.0, 3600 * 2.0, 3600 * 2.0, 3600 * 24.0)).run();
    ...
    log.info("First cycle time after 2am " + signalAnalyzer.getCycleTimeOfFirstCycleInHour(2));
    log.info(
        "Number of signal events between 0am and 1am "
            + signalAnalyzer.getNumberOfSignalEventsInHour(0));
    // test time when signal plans are switched on and off
    Assert.assertEquals(
        "First signal state event unexpected.",
        3600 * 1,
    ...
    //		Assert.assertEquals("Number of plan start events is wrong.", 2,
    // signalAnalyzer.getNumberOfPlanStartEvents());
    //		Assert.assertEquals("Time when last plan starts is wrong.", 3600*2,
    // signalAnalyzer.getLastPlanStartEventTime(), MatsimTestUtils.EPSILON);
    Assert.assertNull(
        "There was an unexpected event that switches off signals.",
        signalAnalyzer.getLastSignalOffEventTime());
    ...
    Assert.assertEquals(
        "Signals where unexpectedly switched on between 0am and 1am.",
        0,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(0),
        MatsimTestUtils.EPSILON);
    Assert.assertTrue(
        "Signals where unexpectedly switched on between 0am and 1am.",
        1 > signalAnalyzer.getNumberOfSignalEventsInHour(0));
  ...
  public void test2SequentialPlans1SecGap() {
    ...
    //		Assert.assertEquals("Number of plan start events is wrong.", 2,
    // signalAnalyzer.getNumberOfPlanStartEvents());
    //		Assert.assertEquals("Time when last plan starts is wrong.", 3600*1+1,
    // signalAnalyzer.getLastPlanStartEventTime(), MatsimTestUtils.EPSILON);
    Assert.assertNull(
        "There was an unexpected event that switches off signals.",
        signalAnalyzer.getLastSignalOffEventTime());
  ...
  public void test2SequentialPlans1HourGap() {
    SignalEventAnalyzer signalAnalyzer =
        (new ScenarioRunner(3600 * 0.0, 3600 * 1.0, 3600 * 2.0, 3600 * 24.0)).run();
    ...
    log.info("First cycle time after 2am " + signalAnalyzer.getCycleTimeOfFirstCycleInHour(2));
    log.info(
        "Number of signal events between 1am and 2am "
            + signalAnalyzer.getNumberOfSignalEventsInHour(1));
    // test time when signal plans are switched on and off
    Assert.assertEquals(
        "First signal state event unexpected.",
        0.0,
    ...
    Assert.assertEquals(
        "Signals where unexpectedly switched on between the signal plans.",
        0,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(1),
        MatsimTestUtils.EPSILON);
    Assert.assertTrue(
        "Signals where unexpectedly switched on between the signal plans.",
        3 > signalAnalyzer.getNumberOfSignalEventsInHour(1));
  ...
  public void test2SequentialPlans1HourGap2TimesOff() {
    ...
    log.info(
        "Number of signal events between 1am and 2am "
            + signalAnalyzer.getNumberOfSignalEventsInHour(1));
    log.info(
        "Number of signal events after 3am " + signalAnalyzer.getNumberOfSignalEventsInHour(3));
    // test time when signal plans are switched on and off
    Assert.assertEquals(
        "First signal state event unexpected.",
        0.0,
    ...
    /* "5 + " because there is SignalSystemImpl.SWITCH_OFF_SEQUENCE_LENGTH of 5 seconds that is added to each signal plans end time as buffer */
    Assert.assertEquals(
        "Number of signal off events is wrong.", 2, signalAnalyzer.getNumberOfOffEvents());
    Assert.assertEquals(
        "Signals where unexpectedly switched on between the signal plans.",
        0,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(1),
    ...
    /* "3 >" because last signal switches of the first plan are allowed at 1am and switch off is only after 5 seconds (in 1:00:05 am) */
    Assert.assertEquals(
        "Signals where unexpectedly switched on after the last signal plan.",
        0,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(3),
        MatsimTestUtils.EPSILON);
    Assert.assertTrue(
        "Signals where unexpectedly switched on after 3am.",
        3 > signalAnalyzer.getNumberOfSignalEventsInHour(3));
  ...
  public void test2SequentialPlansOverMidnight() {
    SignalEventAnalyzer signalAnalyzer =
        (new ScenarioRunner(3600 * 22.0, 3600 * 1.0, 3600 * 1.0, 3600 * 22.0)).run();
  ...
  public void test1SignalPlanUncompleteDay() {
    SignalEventAnalyzer signalAnalyzer =
        (new ScenarioRunner(3600 * 1.0, 3600 * 2.0, null, null)).run();
    ...
    Assert.assertEquals(
        ...
        signalAnalyzer.getFirstSignalEventTime(),
        MatsimTestUtils.EPSILON);
    //		Assert.assertEquals("Number of plan start events is wrong.", 1,
    // signalAnalyzer.getNumberOfPlanStartEvents());
    //		Assert.assertEquals("Time when last plan starts is wrong.", 3600*1,
    // signalAnalyzer.getLastPlanStartEventTime(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
    ...
    Assert.assertEquals(
        "Number of signal off events is wrong.", 1, signalAnalyzer.getNumberOfOffEvents());
    Assert.assertEquals(
        "Signals where unexpectedly switched on before 1am.",
        0,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(0),
        MatsimTestUtils.EPSILON);
    Assert.assertTrue(
    ...
    /* "1 > " because the first signal event should be at 1am, i.e. outside (after) this count interval */
    Assert.assertEquals(
        "Signal plan is not active between 1am and 2am.",
        120,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(1),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
  ...
  /**
   * test 1. one signal plan with default start and end times. should give all day signal plan 2.
   * all day signal plan starts again/ is still active, when simulation time exceeds 12pm
   */
  @Test
  public void test1AllDaySignalPlanOverMidnightLateStart() {
    ScenarioRunner runner =
        new ScenarioRunner(
            null, null, null, null); // i.e. new ScenarioRunner(0.0, 0.0, null, null);
    ...
    log.info(
        "First cycle time after 0am next day " + signalAnalyzer.getCycleTimeOfFirstCycleInHour(24));
    // test time when signal plan is switched on and off
    Assert.assertEquals(
        "First signal state event unexpected.",
        3600 * 23,
    ...
    //		Assert.assertEquals("Number of plan start events is wrong.", 2,
    // signalAnalyzer.getNumberOfPlanStartEvents());
    //		Assert.assertEquals("Time when last plan starts is wrong.", 3600*24,
    // signalAnalyzer.getLastPlanStartEventTime(), MatsimTestUtils.EPSILON);
    Assert.assertNull(
        "There was an unexpected event that switches off signals.",
        signalAnalyzer.getLastSignalOffEventTime());
    ...
    Assert.assertEquals(
        "Signal plan is not active between 11pmam and 12pm.",
        120,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(23),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Signal plan is not active anymore after 0am next day.",
        120,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(24),
        MatsimTestUtils.EPSILON);
  }

  /**
   * test 1. one signal plan with default start and end times. should give all day signal plan 2.
   * should directly start at time 0.0
   */
  @Test
  public void test1AllDaySignalPlanMidnightStart() {
    SignalEventAnalyzer signalAnalyzer =
        (new ScenarioRunner(null, null, null, null))
            .run(); // i.e. new ScenarioRunner(0.0, 0.0, null, null);
    ...
    //		Assert.assertEquals("Number of plan start events is wrong.", 1,
    // signalAnalyzer.getNumberOfPlanStartEvents());
    //		Assert.assertEquals("Time when last plan starts is wrong.", 0.0,
    // signalAnalyzer.getLastPlanStartEventTime(), MatsimTestUtils.EPSILON);
    Assert.assertNull(
        "There was an unexpected event that switches off signals.",
        signalAnalyzer.getLastSignalOffEventTime());
    ...
    // test if signal plan is running for more than 24h
    Assert.assertEquals(
        "Signal plan is not active between 0am and 1am.",
        120,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(0),
        MatsimTestUtils.EPSILON);
  }
  ...
  public void test2SignalPlanFor25h() {
    ...
    Assert.assertEquals(
        ...
        signalAnalyzer.getFirstSignalEventTime(),
        MatsimTestUtils.EPSILON);
    //		Assert.assertEquals("Number of plan start events is wrong.", 3,
    // signalAnalyzer.getNumberOfPlanStartEvents());
    //		Assert.assertEquals("Time when last plan starts is wrong.", 3600*24,
    // signalAnalyzer.getLastPlanStartEventTime(), MatsimTestUtils.EPSILON);
    Assert.assertNull(
    ...
    Assert.assertEquals(
        "First signal plan is not active between 0am and 1am.",
        120,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(0),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Second signal plan is not active between 12am and 1pm.",
        60,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(12),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "First signal plan is not active again after 0am next day.",
        120,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(24),
        MatsimTestUtils.EPSILON);
  }
  ...
  public void testSimStartAfterFirstDayPlan() {
    ...
    log.info(
        "First cycle time after 0am next day " + signalAnalyzer.getCycleTimeOfFirstCycleInHour(24));
    log.info(
        "First cycle time after 1am next day " + signalAnalyzer.getCycleTimeOfFirstCycleInHour(25));
    log.info(
        "Number of signal events before 11pm " + signalAnalyzer.getNumberOfSignalEventsInHour(22));
    log.info(
        "Number of signal events after 1am next day "
            + signalAnalyzer.getNumberOfSignalEventsInHour(25));
    // test time when signal plans are switched on and off
    Assert.assertEquals(
        "First signal state event unexpected.",
        23 * 3600,
    ...
    Assert.assertEquals(
        "Number of signal off events is wrong.", 1, signalAnalyzer.getNumberOfOffEvents());
    Assert.assertEquals(
        "Signals where unexpectedly switched on before 11pm.",
        0,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(22),
        MatsimTestUtils.EPSILON);
    Assert.assertTrue(
        "Signals where unexpectedly switched on before 11pm.",
        1 > signalAnalyzer.getNumberOfSignalEventsInHour(22));
    Assert.assertEquals(
        "Signals where unexpectedly switched on after 1am next day.",
        0,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(25),
        MatsimTestUtils.EPSILON);
    Assert.assertTrue(
        "Signals where unexpectedly switched on after 1am next day.",
        3 > signalAnalyzer.getNumberOfSignalEventsInHour(25));
    // test if signal plans are correctly running
    Assert.assertEquals(
        "Second signal plan is not active between 11pm and 12pm.",
        60,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(23),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "First signal plan is not active between 0am and 1pm next day.",
        120,
        signalAnalyzer.getCycleTimeOfFirstCycleInHour(24),
        MatsimTestUtils.EPSILON);
  }

  /** tests: 1. overlapping signal plans (uncomplete day) result in an exception */
  @Test
  public void test2PlansSameTimesUncompleteDay() {
    final String exceptionMessageOverlapping21 =
        "Signal plans SignalPlan2 and SignalPlan1 of signal system SignalSystem-3 overlap.";

    try {
      (new ScenarioRunner(0.0, 1.0, 0.0, 1.0)).run();
      Assert.fail(
          "The simulation has not stopped with an exception although the signal plans overlap.");
    } catch (UnsupportedOperationException e) {
      log.info("Exception message: " + e.getMessage());
      Assert.assertEquals(
          "Wrong exception message.", exceptionMessageOverlapping21, e.getMessage());
    }
  }

  /** tests: 1. overlapping signal plans (complete day) result in an exception */
  @Test
  public void test2PlansSameTimesCompleteDay() {
    final String exceptionMessageHoleDay =
        "Signal system SignalSystem-3 has multiple plans but at least one of them covers the hole day. "
    ...
    try {
      ...
      Assert.fail(
          "The simulation has not stopped with an exception although multiple signal plans exist and at least one of them covers the hole day (i.e. they overlap).");
    } catch (UnsupportedOperationException e) {
      log.info("Exception message: " + e.getMessage());
  ...
  public void test2OverlappingPlans() {
    final String exceptionMessageOverlapping12 =
        "Signal plans SignalPlan1 and SignalPlan2 of signal system SignalSystem-3 overlap.";
    ...
    try {
      ...
      log.info("Exception message: " + e.getMessage());
      Assert.assertEquals(
          "Wrong exception message.", exceptionMessageOverlapping12, e.getMessage());
    }
  }
  ...
  public void testNegativeOffset() {
    ...
    log.info(
        "Offset "
            + offset1
            + " leads to the first signal event at second: "
            + signalAnalyzer.getFirstSignalEventTime());
    Assert.assertEquals(
        "The first signal event should be at the first second after simulation start corresponding to offset, "
  ...
  public void testTwoPlansWithNegativeOffsets() {
    ...
    log.info(
        "Offsets "
            ...
            + " and "
            + offset2
            + " with a simulation start at second "
            + simStart_s
            + " lead to the first signal event at second: "
            + signalAnalyzer.getFirstSignalEventTime());
    Assert.assertEquals(
        "The first signal event should be at the first second after simulation start corresponding to offset, "
            + "cycle time and plan start time. Also if the offset is negative!",
        simStart_s + 30 + offset2,
        signalAnalyzer.getFirstSignalEventTime(),
        MatsimTestUtils.EPSILON);
  }
  ...
  private class ScenarioRunner {
    ...
    /**
     * Create a scenario with two signal plans with given start end end times. If start and end
     * times of second signal plan are null, only one signal plan is created.
     */
    /* package */ ScenarioRunner(
        Double plan1StartTime, Double plan1EndTime, Double plan2StartTime, Double plan2EndTime) {
    ...
    /** Creates a dummy population going from left to right with one agent starting every second. */
    private void createPopulation() {
      ...
      for (int i = simStart_h * 3600; i <= simStart_h * 3600 + noSimHours * 3600; i += 1800) {
        ...
        Activity startAct =
            pop.getFactory().createActivityFromLinkId("dummy", Id.createLinkId("1_2"));
        // one agent starts every second
        startAct.setEndTime(i);
        plan.addActivity(startAct);
        plan.addLeg(pop.getFactory().createLeg(TransportMode.car));
        Activity drainAct =
            pop.getFactory().createActivityFromLinkId("dummy", Id.createLinkId("4_5"));
    ...
    private void createSignals() {
      ...
      SignalGroupData signalGroup1 =
          signalGroups.getFactory().createSignalGroupData(signalSystemId, signalGroupId1);
      ...
      SignalSystemControllerData signalSystemControl =
          conFac.createSignalSystemControllerData(signalSystemId);
      signalSystemControl.setControllerIdentifier(
      ...
      SignalPlanData signalPlan1 =
          SignalUtils.createSignalPlan(
              conFac, 120, offsetPlan1, Id.create("SignalPlan1", SignalPlan.class));
      ...
      signalPlan1.addSignalGroupSettings(
          SignalUtils.createSetting4SignalGroup(conFac, signalGroupId1, 0, 60));

      if (plan2StartTime != null && plan2EndTime != null) {
        // create a second plan for the signal system (with cycle time 60) if start and end times
        // are not null
        SignalPlanData signalPlan2 =
            SignalUtils.createSignalPlan(
                conFac, 60, offsetPlan2, Id.create("SignalPlan2", SignalPlan.class));
        ...
        signalPlan2.addSignalGroupSettings(
            SignalUtils.createSetting4SignalGroup(conFac, signalGroupId1, 0, 30));
      }
    }
    ...
    private Config defineConfig() {
      ...
      SignalSystemsConfigGroup signalConfigGroup =
