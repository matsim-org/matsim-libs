...
public class LaneSensorTest {
  ...
  public void testGetAvgVehiclesPerSecondAfterBucketCollection() {
    ...
    for (int time = 0; time <= 3600; time++) {
      ...
      if (time % 3 == 0) sensor.handleEvent(new LaneEnterEvent(time, null, null, null));
      // after enough buckets are collected the average should be 0,333 per second.
      if (time > 60) assertEquals(1.0 / 3.0, sensor.getAvgVehiclesPerSecond(time), 0.04);
    }
  }
  ...
  public void testGetAvgVehiclesPerSecondDuringBucketCollection() {
    ...
    for (int time = 0; time <= 3600; time++) {
      ...
      if (time % 3 == 0) sensor.handleEvent(new LaneEnterEvent(time, null, null, null));
      // after enough buckets are collected the average should be 0,333 per second.
      if (time > 15) assertEquals(1.0 / 3.0, sensor.getAvgVehiclesPerSecond(time), 0.04);
    }
  }
  ...
  public void testGetAvgVehiclesPerSecondWithNoTrafficForTwoBucket() {
    ...
    for (int time = 0; time <= 3600; time++) {
      ...
      // after a bucket with vehicles is colleted now the avg should be 0,25 since there is still an
      // empty bucket
      ...
      if (time > 210) assertEquals((1.0 / 3.0), sensor.getAvgVehiclesPerSecond(time), 0.02);
    }
  }
  ...
  public void testGetAvgVehiclesPerSecondWithNoTrafficForTwoBucketWhileHavingNotEnoughBuckets() {
    ...
    for (int time = 0; time <= 3600; time++) {
      ...
      // after generating two empty bucket the avg should be 0,1111 for two bucket collection times
      // since there's only one full bucket (second 0 to 15) of total three
      if (time > 45 && time < 60)
      ...
      if (time > 60 && time < 90)
        assertEquals((1.0 / 3.0) / (4.0 / 2.0), sensor.getAvgVehiclesPerSecond(time), 0.02);
      // after the first empty bucket is removed and avg should be 0,25 since there is still one
      // empty bucket
      if (time > 90 && time < 105)
        assertEquals((1.0 / 3.0) / (4.0 / 3.0), sensor.getAvgVehiclesPerSecond(time), 0.02);
      // after four buckets are collected again (45+60)the average should be 0,333 per second.
      if (time > 105) assertEquals((1.0 / 3.0), sensor.getAvgVehiclesPerSecond(time), 0.02);
    }
  }
  ...
  public void testClassicBehaviour() {
    ...
    for (int time = 0; time <= 3600; time++) {
      ...
      if (time <= 30) assertEquals(0.0, sensor.getAvgVehiclesPerSecond(time), 0.02);
      if (time == 32) assertEquals(1.0, sensor.getAvgVehiclesPerSecond(time), 0.02);
      if (time > 50) assertTrue(sensor.getAvgVehiclesPerSecond(time) < 1.0);
      if (time == 1000) assertTrue(sensor.getAvgVehiclesPerSecond(time) > 0.96);
    }
  }
}
