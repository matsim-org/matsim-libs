...
public class TravelTimeFourWaysTest {
  ...
  private Scenario createTestScenario() {
    ...
    scenario.addScenarioElement(
        SignalsData.ELEMENT_NAME, new SignalsDataLoader(scenario.getConfig()).loadSignalsData());
    return scenario;
  }
  ...
  private void runQSimWithSignals(final Scenario scenario) {
    ...
    //	    Assert.assertEquals("different events files",
    // EventsFileComparator.compareAndReturnInt(this.testUtils.getInputDirectory() + EVENTSFILE,
    // eventsOut), 0);
    Assert.assertEquals(
        Result.FILES_ARE_EQUAL,
        new EventsFileComparator()
            .setIgnoringCoordinates(true)
            .runComparison(this.testUtils.getInputDirectory() + EVENTSFILE, eventsOut));
  }
}
