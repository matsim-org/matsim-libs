...
/**
 * Test sylvia logic at an intersection with four incoming links and one signal each. No lanes are
 * used.
 *
 * <p>It is tested whether sylvia expands signal phases correctly when more vehicles want to pass
...
public class SylviaIT {
  ...
  /**
   * Test sylvia with two conflicting streams at a single intersection. A fixed cycle time (of 60
   * seconds) and no maximal extension time per setting are used. The two approaches have equal
   * demand but different priority in the Sylvia signal algorithm. The priority is given by the
   * order in the signal plan. In this test, signal group 1 has priority over 2.
   *
   * <p>note: signal settings of the fixed time plan are unbalanced with 5 seconds for signal group
   * 1 vs. 45 seconds for signal group 2. but signal settings of the sylvia plan are balanced,
   * because each setting is shortend to 5 seconds green time. because of the setup in this test
   * (with a fixed cycle time and no maximal extension time per setting), the first setting of the
   * plan has priority over the second.
   */
  @Test
  public void testDemandABPrioA() {
    ...
    Map<Id<SignalGroup>, Double> totalSignalGreenTimes =
        signalAnalyzer
            .getTotalSignalGreenTime(); // should be more or less equal (OW direction is always
    // favored as the first phase)
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycle =
        signalAnalyzer
            .calculateAvgSignalGreenTimePerFlexibleCycle(); // should be more or less equal and
    // around 25
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystem =
        signalAnalyzer.calculateAvgFlexibleCycleTimePerSignalSystem(); // should be 60
    ...
    log.info(
        "total signal green times: "
            + totalSignalGreenTimes.get(signalGroupId1)
            + ", "
            + totalSignalGreenTimes.get(signalGroupId2));
    log.info(
        "avg signal green times per cycle: "
            + avgSignalGreenTimePerCycle.get(signalGroupId1)
            + ", "
            + avgSignalGreenTimePerCycle.get(signalGroupId2));
    log.info("avg cycle time per system: " + avgCycleTimePerSystem.get(signalSystemId));
    Assert.assertEquals(
        "total signal green time of signal group 1 is wrong",
        2900,
        totalSignalGreenTimes.get(signalGroupId1),
        50);
    Assert.assertEquals(
        "total signal green time of signal group 2 is wrong",
        2000,
        totalSignalGreenTimes.get(signalGroupId2),
        50);
    Assert.assertEquals(
        "avg green time per cycle of signal group 1 is wrong",
        30,
        avgSignalGreenTimePerCycle.get(signalGroupId1),
        1);
    Assert.assertEquals(
        "avg green time per cycle of signal group 2 is wrong",
        20,
        avgSignalGreenTimePerCycle.get(signalGroupId2),
        1);
    // can differ from the fixed cycle length because the analysis is quit after the last activity
    // start event
    Assert.assertEquals(
        "avg cycle time of the system is wrong", 60, avgCycleTimePerSystem.get(signalSystemId), 1);
  }
   ...
   * seconds) and no maximal extension time per setting are used. The two approaches have equal
   * demand but different priority in the Sylvia signal algorithm. The priority is given by the
   * order in the signal plan. In this test, signal group 2 has priority over 1.
   *
   * <p>note: signal settings of the fixed time plan are unbalanced with 5 seconds for signal group
   * 1 vs. 45 seconds for signal group 2. but signal settings of the sylvia plan are balanced,
   * because each setting is shortend to 5 seconds green time. because of the setup in this test
  ...
  public void testDemandA() {
    ...
    Map<Id<SignalGroup>, Double> totalSignalGreenTimes =
        signalAnalyzer
            .getTotalSignalGreenTime(); // group 1 should have more total green time than group 2
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycle =
        signalAnalyzer
            .calculateAvgSignalGreenTimePerFlexibleCycle(); // should be 45 vs 5 (excluding initial
    // phase)
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystem =
        signalAnalyzer.calculateAvgFlexibleCycleTimePerSignalSystem(); // should be 60
    Id<SignalGroup> signalGroupId1 = Id.create("SignalGroup1", SignalGroup.class);
    ...
    log.info("avg cycle time per system: " + avgCycleTimePerSystem.get(signalSystemId));
    Assert.assertTrue(
        "total signal green time of group 1 is not bigger than of group 2",
        totalSignalGreenTimes.get(signalGroupId1) > totalSignalGreenTimes.get(signalGroupId2));
    Assert.assertEquals(
        "avg green time per cycle of signal group 1 is wrong",
        45,
        avgSignalGreenTimePerCycle.get(signalGroupId1),
  ...
  private static void createNetwork(Network net) {
    ...
    String[] links = {
      "1_2", "2_1", "2_3", "3_2", "3_4", "4_3", "4_5", "5_4", "6_7", "7_6", "7_3", "3_7", "3_8",
      "8_3", "8_9", "9_8"
    };
  ...
  private static void createPopulation(Population population, double[] noPersons) {
    ...
    for (String od : odRelations) {
      ...
      for (int i = 0; i < noPersons[odIndex]; i++) {
        ...
        Activity startAct =
            population.getFactory().createActivityFromLinkId("dummy", Id.createLinkId(fromLinkId));
        ...
        Activity drainAct =
            population.getFactory().createActivityFromLinkId("dummy", Id.createLinkId(toLinkId));
  ...
  private void createSignals(Scenario scenario, int offset) {
    ...
    for (Id<Link> inLinkId :
        scenario.getNetwork().getNodes().get(Id.createNodeId(3)).getInLinks().keySet()) {
    ...
    Id<SignalGroup> signalGroupId2 = Id.create("SignalGroup2", SignalGroup.class);
    SignalGroupData signalGroup2 =
        signalGroups.getFactory().createSignalGroupData(signalSystemId, signalGroupId2);
    ...
    SignalPlanData signalPlan =
        SignalUtils.createSignalPlan(
            conFac, 60, offset, Id.create("SignalPlan1", SignalPlan.class));
    signalSystemControl.addSignalPlanData(signalPlan);

    // specify signal group settings for both signal groups
    signalPlan.addSignalGroupSettings(
        SignalUtils.createSetting4SignalGroup(conFac, signalGroupId1, 0, 5));
    signalPlan.addSignalGroupSettings(
        SignalUtils.createSetting4SignalGroup(conFac, signalGroupId2, 10, 55));
