...
public class LaemmerIT {
  ...
  /**
   * single intersection with demand (equals flow capacity) only in NS-direction. signals should
   * show green only for the NS-direction.
   */
  @Test
  public void testSingleCrossingScenarioDemandNS() {
    ...
    Map<Id<SignalGroup>, Double> totalSignalGreenTimes =
        signalAnalyzer.getTotalSignalGreenTime(); // NS should show a lot more green than WE
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycle =
        signalAnalyzer.calculateAvgSignalGreenTimePerFlexibleCycle(); // WE should be almost 0
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystem =
        signalAnalyzer
            .calculateAvgFlexibleCycleTimePerSignalSystem(); // should be around 60, at most 90
    ...
    Assert.assertNull(
        "signal group 1 should show no green", avgSignalGreenTimePerCycle.get(signalGroupId1));
    Assert.assertNotNull(
        "signal group 2 should show green", avgSignalGreenTimePerCycle.get(signalGroupId2));
    Assert.assertEquals(
        "avg delay at NS-direction should be zero", 0.0, avgDelayNS, MatsimTestUtils.EPSILON);
  }

  /**
   * Test minimum green time: single intersection with high demand in WE-direction, very low demand
   * in NS-direction but minimum green time. I.e. the NS-signal should show green for exactly this 5
   * seconds per cycle.
   */
  @Test
  public void testSingleCrossingScenarioLowVsHighDemandWithMinG() {
    ...
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycle =
        signalAnalyzer.calculateAvgSignalGreenTimePerFlexibleCycle();
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystem =
        signalAnalyzer.calculateAvgFlexibleCycleTimePerSignalSystem();
    Map<Id<Link>, Double> avgDelayPerLink = generalAnalyzer.getAvgDelayPerLink();
    Double avgDelayWE = avgDelayPerLink.get(Id.createLinkId("2_3"));
    Double avgDelayNS = avgDelayPerLink.get(Id.createLinkId("7_3"));
    ...
    Assert.assertTrue(
        ...
        avgSignalGreenTimePerCycle.get(signalGroupId1)
            ...
            > 0);
    Assert.assertEquals(
        "avg signal green time per cycle of NS-direction should be the minimum green time of 5 seconds",
        5.0,
        avgSignalGreenTimePerCycle.get(signalGroupId2),
        MatsimTestUtils.EPSILON);
    Assert.assertTrue(
        "cycle time should stay below 90 seconds", avgCycleTimePerSystem.get(signalSystemId) <= 90);
    Assert.assertTrue(
        "avg delay per vehicle on WS-direction should be less than on NS-direction",
        avgDelayWE < avgDelayNS);
  }

  /**
   * counterpart to minimum green time test above. single intersection with high demand in
   * WE-direction, very low demand in NS-direction. No minimum green time! I.e. the NS-signal should
   * show green for less than 5 seconds per cycle.
   */
  @Test
  public void testSingleCrossingScenarioLowVsHighDemandWoMinG() {
    ...
    Assert.assertTrue(
        ...
        avgSignalGreenTimePerCycle.get(signalGroupId1)
            ...
            > 0);
    Assert.assertTrue(
        "avg signal green time per cycle of NS-direction should be less than 5 seconds",
        avgSignalGreenTimePerCycle.get(signalGroupId2) < 5.0);
    Assert.assertTrue(
        "cycle time should stay below 90 seconds", avgCycleTimePerSystem.get(signalSystemId) <= 90);
    Assert.assertTrue(
        "avg delay per vehicle on WS-direction should be less than on NS-direction",
  ...
  public void testSingleCrossingScenarioEqualDemandCapacityRatio() {
    ...
    Assert.assertEquals(
        "total signal green times should not differ more than 1%",
        1,
        totalSignalGreenTimes.get(signalGroupId1) / totalSignalGreenTimes.get(signalGroupId2),
        0.01);
    Assert.assertEquals(
        "avg signal green times per cycle should not differ more than 1%",
        1,
        avgSignalGreenTimePerCycle.get(signalGroupId1)
            / avgSignalGreenTimePerCycle.get(signalGroupId2),
        0.01);
    //		Assert.assertEquals("avg delay per vehicle per link should not differ more than 5%", 1,
    // avgDelayWE/avgDelayNS, 0.05);
  ...
  /**
   * compare different regimes for laemmer: stabilizing, optimizing and combination of both
   * (combined) for low demand, i.e. an occupancy rate of 0.5 in the example of nico kuehnel's
   * master thesis, the optimizing regime should be better than the stabilizing regime.
   */
  @Test
  public void testSingleCrossingScenarioStabilizingVsOptimizingRegimeLowDemand() {
    ...
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycleStab =
        signalAnalyzerStab.calculateAvgSignalGreenTimePerFlexibleCycle();
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystemStab =
        signalAnalyzerStab.calculateAvgFlexibleCycleTimePerSignalSystem();
    ...
    log.info(
        "avg signal green times per cycle: "
            + avgSignalGreenTimePerCycleStab.get(signalGroupId1)
            + ", "
    ...
    log.info(
        "avg delay per link: "
            + avgDelayPerLinkStab.get(Id.createLinkId("2_3"))
            + ", "
            + avgDelayPerLinkStab.get(Id.createLinkId("7_3")));
    log.info("Total delay: " + generalAnalyzerStab.getTotalDelay());

    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycleOpt =
        signalAnalyzerOpt.calculateAvgSignalGreenTimePerFlexibleCycle();
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystemOpt =
        signalAnalyzerOpt.calculateAvgFlexibleCycleTimePerSignalSystem();
    ...
    log.info(
        "avg signal green times per cycle: "
            + avgSignalGreenTimePerCycleOpt.get(signalGroupId1)
            + ", "
    ...
    log.info(
        "avg delay per link: "
            + avgDelayPerLinkOpt.get(Id.createLinkId("2_3"))
            + ", "
            + avgDelayPerLinkOpt.get(Id.createLinkId("7_3")));
    log.info("Total delay: " + generalAnalyzerOpt.getTotalDelay());

    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycleComb =
        signalAnalyzerComb.calculateAvgSignalGreenTimePerFlexibleCycle();
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystemComb =
        signalAnalyzerComb.calculateAvgFlexibleCycleTimePerSignalSystem();
    ...
    log.info(
        "avg signal green times per cycle: "
            + avgSignalGreenTimePerCycleComb.get(signalGroupId1)
            + ", "
    ...
    log.info(
        "avg delay per link: "
            + avgDelayPerLinkComb.get(Id.createLinkId("2_3"))
            + ", "
            + avgDelayPerLinkComb.get(Id.createLinkId("7_3")));
    ...
    for (Id<Link> linkId : avgDelayPerLinkStab.keySet()) {
      Assert.assertTrue(
          "stab: avg delay per link should be below a threshold (i.e. still stable)",
          avgDelayPerLinkStab.get(linkId) < maxCycleTime);
    }
    Assert.assertTrue(
        "stab: total delay should be higher than for the other regimes",
        generalAnalyzerStab.getTotalDelay() > generalAnalyzerOpt.getTotalDelay());
    Assert.assertTrue(
        "stab: total delay should be higher than for the other regimes",
        generalAnalyzerStab.getTotalDelay() > generalAnalyzerComb.getTotalDelay());
    Assert.assertTrue(
        "the stabilizing regime should satisfy the maximum cycle time",
        avgCycleTimePerSystemStab.get(signalSystemId) < maxCycleTime);
    // stabilizing regime only shows green when number of vehicles beyond a critical number, i.e.
    // some of the cycle time is given away (all signals show red)
    Assert.assertTrue(
    ...
    for (Id<Link> linkId : avgDelayPerLinkOpt.keySet()) {
      Assert.assertTrue(
          "opt: avg delay per link should be below a threshold (i.e. still stable)",
          avgDelayPerLinkOpt.get(linkId) < maxCycleTime);
    }
    Assert.assertEquals(
    ...
    for (Id<Link> linkId : avgDelayPerLinkComb.keySet()) {
      Assert.assertTrue(
          "avg delay per link should be below a threshold (i.e. still stable)",
          avgDelayPerLinkComb.get(linkId) < maxCycleTime);
    }
    Assert.assertEquals(
        ...
        2);
    Assert.assertTrue(
        "the combined regime should satisfy the maximum cycle time",
        avgCycleTimePerSystemComb.get(signalSystemId) < maxCycleTime);
    Assert.assertTrue(
        "total delay with the combined regime should be the lowest",
        generalAnalyzerOpt.getTotalDelay() > generalAnalyzerComb.getTotalDelay());
  }

  /**
   * compare different regimes for laemmer: stabilizing, optimizing and combination of both
   * (combined) for high demand, i.e. an occupancy rate of 0.6 in the example of nico kuehnel's
   * master thesis, the optimizing regime should no longer be stable (high delays, high travel time,
   * no standard cycle pattern). The stabilizing regime should still be stable, the combined regime
   * should be the best.
   */
  @Test
  public void testSingleCrossingScenarioStabilizingVsOptimizingRegimeHighDemand() {
    ...
    // Test Optimizing Regime:
    Assert.assertTrue(
        "avg delay for NS-direction should be very high for the optimizing regime with high demand",
        avgDelayPerLinkOpt.get(Id.createLinkId("7_3")) > maxCycleTime);
    Assert.assertTrue(
        "total delay of optimizing regime should be the highest",
        generalAnalyzerStab.getTotalDelay() < generalAnalyzerOpt.getTotalDelay());
    Assert.assertTrue(
  ...
  /**
   * test laemmer for different flow capacity factors: a factor of 0.5 should result in the same
   * signal settings as a factor of 1.0 when the demand is exactly doubled (same departure times).
   */
  @Test
  public void testSingleCrossingScenarioWithDifferentFlowCapacityFactors() {
    ...
    Map<Id<SignalGroup>, Double> totalSignalGreenTimesFlowCap2 =
        signalAnalyzerFlowCap2.getTotalSignalGreenTime();
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycleFlowCap2 =
        signalAnalyzerFlowCap2.calculateAvgSignalGreenTimePerFlexibleCycle();
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystemFlowCap2 =
    ...
    log.info(
        "total signal green times: "
            + totalSignalGreenTimesFlowCap2.get(signalGroupId1)
            + ", "
            + totalSignalGreenTimesFlowCap2.get(signalGroupId2));
    log.info(
        "avg signal green times per cycle: "
            + avgSignalGreenTimePerCycleFlowCap2.get(signalGroupId1)
            + ", "
    ...
    log.info(
        "avg delay per link: "
            + avgDelayPerLinkFlowCap2.get(Id.createLinkId("2_3"))
            + ", "
            + avgDelayPerLinkFlowCap2.get(Id.createLinkId("7_3")));
    log.info("Total delay: " + generalAnalyzerFlowCap2.getTotalDelay());

    Map<Id<SignalGroup>, Double> totalSignalGreenTimesFlowCap1 =
        signalAnalyzerFlowCap1.getTotalSignalGreenTime();
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycleFlowCap1 =
        signalAnalyzerFlowCap1.calculateAvgSignalGreenTimePerFlexibleCycle();
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystemFlowCap1 =
    ...
    log.info(
        "total signal green times: "
            + totalSignalGreenTimesFlowCap1.get(signalGroupId1)
            + ", "
            + totalSignalGreenTimesFlowCap1.get(signalGroupId2));
    log.info(
        "avg signal green times per cycle: "
            + avgSignalGreenTimePerCycleFlowCap1.get(signalGroupId1)
            + ", "
    ...
    log.info(
        "avg delay per link: "
            + avgDelayPerLinkFlowCap1.get(Id.createLinkId("2_3"))
            + ", "
            + avgDelayPerLinkFlowCap1.get(Id.createLinkId("7_3")));
    log.info("Total delay: " + generalAnalyzerFlowCap1.getTotalDelay());

    Assert.assertEquals(
        "total signal green times should not differ",
        1,
        totalSignalGreenTimesFlowCap1.get(signalGroupId1)
            / totalSignalGreenTimesFlowCap2.get(signalGroupId1),
        0.01);
    Assert.assertEquals(
        ...
        0.01);
    Assert.assertEquals(
        "avg signal green times per cycle should not differ",
        avgSignalGreenTimePerCycleFlowCap1.get(signalGroupId1),
        avgSignalGreenTimePerCycleFlowCap2.get(signalGroupId1),
        0.1);
    Assert.assertEquals(
        ...
        0.1);
    Assert.assertEquals(
        "avg cycle time should not differ",
        avgCycleTimePerSystemFlowCap1.get(signalSystemId),
        avgCycleTimePerSystemFlowCap2.get(signalSystemId),
        0.1);
    Assert.assertEquals(
        "avg delay per vehicle per link should not differ",
        avgDelayPerLinkFlowCap1.get(Id.createLinkId("2_3")),
        avgDelayPerLinkFlowCap2.get(Id.createLinkId("2_3")),
        0.1);
    Assert.assertEquals(
        ...
        2);
    Assert.assertEquals(
        "total delay for doubled demand should be doubled",
        2,
        generalAnalyzerFlowCap2.getTotalDelay() / generalAnalyzerFlowCap1.getTotalDelay(),
        0.1);
  }
  ...
  public void testMultipleIterations() {
    ...
    Map<Id<SignalGroup>, Double> totalSignalGreenTimes0It =
        signalAnalyzer0It.getTotalSignalGreenTime();
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycle0It =
        signalAnalyzer0It.calculateAvgSignalGreenTimePerFlexibleCycle();
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystem0It =
        signalAnalyzer0It.calculateAvgFlexibleCycleTimePerSignalSystem();
    ...
    log.info(
        "total signal green times in iteration 0: "
            + totalSignalGreenTimes0It.get(signalGroupId1)
            + ", "
            + totalSignalGreenTimes0It.get(signalGroupId1l)
            + ", "
            + totalSignalGreenTimes0It.get(signalGroupId2));
    log.info(
        "avg signal green times per cycle in itertion 0: "
            + avgSignalGreenTimePerCycle0It.get(signalGroupId1)
            + ", "
            + avgSignalGreenTimePerCycle0It.get(signalGroupId1l)
            + ", "
            + avgSignalGreenTimePerCycle0It.get(signalGroupId2));
    log.info(
        "avg cycle time per system in itertion 0: " + avgCycleTimePerSystem0It.get(signalSystemId));
    ...
    Map<Id<SignalGroup>, Double> totalSignalGreenTimes1It =
        signalAnalyzer1It.getTotalSignalGreenTime();
    Map<Id<SignalGroup>, Double> avgSignalGreenTimePerCycle1It =
        signalAnalyzer1It.calculateAvgSignalGreenTimePerFlexibleCycle();
    Map<Id<SignalSystem>, Double> avgCycleTimePerSystem1It =
        signalAnalyzer1It.calculateAvgFlexibleCycleTimePerSignalSystem();
    ...
    log.info(
        "total signal green times in itertion 1: "
            + totalSignalGreenTimes1It.get(signalGroupId1)
            + ", "
            + totalSignalGreenTimes1It.get(signalGroupId1l)
            + ", "
            + totalSignalGreenTimes1It.get(signalGroupId2));
    log.info(
        "avg signal green times per cycle in itertion 1: "
            + avgSignalGreenTimePerCycle1It.get(signalGroupId1)
            + ", "
            + avgSignalGreenTimePerCycle1It.get(signalGroupId1l)
            + ", "
            + avgSignalGreenTimePerCycle1It.get(signalGroupId2));
    log.info(
        "avg cycle time per system in itertion 1: " + avgCycleTimePerSystem1It.get(signalSystemId));
    log.info("avg delay per link in itertion 1: " + avgDelayWE1It + ", " + avgDelayNS1It);

    Assert.assertEquals(
        "total green time of signal group 1 should be the same as in the first iteration",
        totalSignalGreenTimes0It.get(signalGroupId1),
        totalSignalGreenTimes1It.get(signalGroupId1),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "total green time of signal group 1l should be the same as in the first iteration",
        totalSignalGreenTimes0It.get(signalGroupId1l),
        totalSignalGreenTimes1It.get(signalGroupId1l),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "total green time of signal group 2 should be the same as in the first iteration",
        totalSignalGreenTimes0It.get(signalGroupId2),
        totalSignalGreenTimes1It.get(signalGroupId2),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "avg green time of signal group 1 should be the same as in the first iteration",
        avgSignalGreenTimePerCycle0It.get(signalGroupId1),
        avgSignalGreenTimePerCycle1It.get(signalGroupId1),
        .01);
    Assert.assertEquals(
        "avg green time of signal group 1l should be the same as in the first iteration",
        avgSignalGreenTimePerCycle0It.get(signalGroupId1l),
        avgSignalGreenTimePerCycle1It.get(signalGroupId1l),
        .01);
    Assert.assertEquals(
        "avg green time of signal group 2 should be the same as in the first iteration",
        avgSignalGreenTimePerCycle0It.get(signalGroupId2),
        avgSignalGreenTimePerCycle1It.get(signalGroupId2),
        .01);
    Assert.assertEquals(
        "avg cycle time should be the same as in the first iteration",
        avgCycleTimePerSystem0It.get(signalSystemId),
        avgCycleTimePerSystem1It.get(signalSystemId),
        .01);
    Assert.assertEquals(
        "avg delay in direction WE should be the same as in the first iteration",
        avgDelayWE0It,
        avgDelayWE1It,
        .01);
    Assert.assertEquals(
        "avg delay in direction NS should be the same as in the first iteration",
        avgDelayNS0It,
        avgDelayNS1It,
        .01);
    ...
    long checksum_it0 =
        CRCChecksum.getCRCFromFile(
            testUtils.getOutputDirectory() + "ITERS/it.0/signalEvents2Via.csv");
    long checksum_itLast =
        CRCChecksum.getCRCFromFile(
            testUtils.getOutputDirectory() + "ITERS/it." + lastIt + "/signalEvents2Via.csv");
  ...
  // TODO test temporarily overcrowded situations (no exeption; signal is able to resolve
  // congestion; like fixed-time schedule; cycle times get longer when overload continues)
  // TODO test liveArrivalRate (with/without time buckets) vs. exact data (the second results in
  // more precise green times?!; liveArrivalRates are determined correctly)
  // TODO test grouping
  // TODO test lanes
  // ...
  ...
  private class Fixture {
    ...
    Fixture(double flowNS, double flowWE, double minG, Regime regime) {
      ...
      // create Config
      Config config =
          ConfigUtils.loadConfig("./examples/tutorial/singleCrossingScenario/config.xml");
      config.plans().setInputFile(null);
      config.controller().setOutputDirectory(testUtils.getOutputDirectory());

      LaemmerConfigGroup laemmerConfigGroup =
          ConfigUtils.addOrGetModule(
              config, LaemmerConfigGroup.GROUP_NAME, LaemmerConfigGroup.class);
      ...
      // modify lanes (such that only one lane leads straight with a capacity of 3600 veh. this make
      // especially the flow capacity test more clear.)
      LanesToLinkAssignment lanesOfLink23 =
          scenario.getLanes().getLanesToLinkAssignments().get(Id.createLinkId("2_3"));
      LanesToLinkAssignment lanesOfLink43 =
          scenario.getLanes().getLanesToLinkAssignments().get(Id.createLinkId("4_3"));
      lanesOfLink23
          .getLanes()
          .get(Id.create("2_3.r", Lane.class))
          .getToLinkIds()
          .remove(Id.createLinkId("3_4"));
      ...
      lanesOfLink43
          .getLanes()
          .get(Id.create("4_3.r", Lane.class))
          .getToLinkIds()
          .remove(Id.createLinkId("3_2"));
    ...
    private void createSimplePopulationWithoutLeftTurns(
        Population pop, double flowNS, double flowWE, String idPostFix) {
    ...
    private void createPopulationForRelation(
        double flow, Population population, String[] links, String idPostFix) {
      if (flow == 0) {
        return;
      }
      ...
      for (String od : links) {
        ...
        for (double i = 0; i < 5400; i += nthSecond) {
          createPerson(
              population, fromLinkId, toLinkId, i, Id.createPersonId(od + "_" + i + idPostFix));
        }
      }
    }

    private void createPerson(
        Population population, String fromLinkId, String toLinkId, double time, Id<Person> id) {
