...
public class SignalConflictDataReaderWriterTest {
  ...
  private void compare(ConflictData conflictData1, ConflictData conflictData2) {
    Assert.assertEquals(
        "not the same number of intersections",
        conflictData1.getConflictsPerNode().size(),
        conflictData2.getConflictsPerNode().size());
    for (IntersectionDirections intersection1 :
        conflictData1.getConflictsPerSignalSystem().values()) {
      IntersectionDirections intersection2 =
          conflictData2.getConflictsPerSignalSystem().get(intersection1.getSignalSystemId());
      Assert.assertNotNull(
          "no conflict data exists for signal system " + intersection1.getSignalSystemId(),
          intersection2);
      Assert.assertEquals(
          "not the same node, but the same signal system " + intersection1.getSignalSystemId(),
          intersection1.getNodeId(),
          intersection2.getNodeId());
      Assert.assertEquals(
          "not the same number of direction at node " + intersection1.getNodeId(),
          intersection1.getDirections().size(),
          intersection2.getDirections().size());
      for (Direction dir1 : intersection1.getDirections().values()) {
        ...
        Assert.assertEquals(
            "direction " + dir1.getId() + " has not the same from link",
            dir1.getFromLink(),
            dir2.getFromLink());
        Assert.assertEquals(
            "direction " + dir1.getId() + " has not the same to link",
            dir1.getToLink(),
            dir2.getToLink());
        Assert.assertEquals(
            "not the same number of conflicting directions for direction " + dir1.getId(),
            dir1.getConflictingDirections().size(),
            dir2.getConflictingDirections().size());
        Assert.assertEquals(
            "not the same number of directions with right of way for direction " + dir1.getId(),
            dir1.getDirectionsWithRightOfWay().size(),
            dir2.getDirectionsWithRightOfWay().size());
        Assert.assertEquals(
            "not the same number of directions which must yield for direction " + dir1.getId(),
            dir1.getDirectionsWhichMustYield().size(),
            dir2.getDirectionsWhichMustYield().size());
        Assert.assertEquals(
            "not the same number of non-conflicting directions for direction " + dir1.getId(),
            dir1.getNonConflictingDirections().size(),
            dir2.getNonConflictingDirections().size());
        for (Id<Direction> conflDir1 : dir1.getConflictingDirections()) {
          Assert.assertTrue(
              "direction " + conflDir1 + " is not a conflicting direction for " + dir1.getId(),
              dir2.getConflictingDirections().contains(conflDir1));
        }
        for (Id<Direction> conflDir1 : dir1.getDirectionsWithRightOfWay()) {
          Assert.assertTrue(
              "direction "
                  + conflDir1
                  + " is not a direction with right of way for "
                  + dir1.getId(),
              dir2.getDirectionsWithRightOfWay().contains(conflDir1));
        }
        for (Id<Direction> conflDir1 : dir1.getDirectionsWhichMustYield()) {
          Assert.assertTrue(
              "direction " + conflDir1 + " is not a direction which must yield for " + dir1.getId(),
              dir2.getDirectionsWhichMustYield().contains(conflDir1));
        }
        for (Id<Direction> conflDir1 : dir1.getNonConflictingDirections()) {
          Assert.assertTrue(
              "direction " + conflDir1 + " is not a non-conflicting direction for " + dir1.getId(),
              dir2.getNonConflictingDirections().contains(conflDir1));
        }
      }
    }
  }
  ...
  private ConflictData createConflictDataForTestCase() {
    ...
    IntersectionDirections intersection1 =
        conflictData
            .getFactory()
            .createConflictingDirectionsContainerForIntersection(signalSystemId1, nodeId1);
    conflictData.addConflictingDirectionsForIntersection(signalSystemId1, nodeId1, intersection1);

    Direction dir13 =
        conflictData
            .getFactory()
            .createDirection(signalSystemId1, nodeId1, linkId1, linkId3, dirId13);
    ...
    Direction dir31 =
        conflictData
            .getFactory()
            .createDirection(signalSystemId1, nodeId1, linkId3, linkId1, dirId31);
    ...
    Direction dir12 =
        conflictData
            .getFactory()
            .createDirection(signalSystemId1, nodeId1, linkId1, linkId2, dirId12);
    ...
    Direction dir23 =
        conflictData
            .getFactory()
            .createDirection(signalSystemId1, nodeId1, linkId2, linkId3, dirId23);
    ...
    Direction dir34 =
        conflictData
            .getFactory()
            .createDirection(signalSystemId2, nodeId2, linkId3, linkId4, dirId34);
    ...
    Direction dir43 =
        conflictData
            .getFactory()
            .createDirection(signalSystemId2, nodeId2, linkId4, linkId3, dirId43);
