...
public class UnprotectedLeftTurnLogicTest {
  ...
  public void testSingleIntersectionScenarioWithLeftTurns() {
    ...
    AnalyzeSingleIntersectionLeftTurnDelays restrictedLeftTurns =
        runSimulation(IntersectionLogic.CONFLICTING_DIRECTIONS_AND_TURN_RESTRICTIONS);
    AnalyzeSingleIntersectionLeftTurnDelays unrestrictedLeftTurns =
        runSimulation(IntersectionLogic.CONFLICTING_DIRECTIONS_NO_TURN_RESTRICTIONS);
    ...
    Assert.assertTrue(
        "Delay without restriction should be less than with restricted left turns.",
        2 * leftTurnDelayWoTurnRestriction < leftTurnDelayWTurnRestriction);
    Assert.assertEquals(
        "Delay without turn restriction should be equal to the case without conflicting data.",
        leftTurnDelayWoTurnRestriction,
        leftTurnDelayWithoutLogic,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Delay value for the case without turn restrictions is not as expected!",
        21120,
        leftTurnDelayWoTurnRestriction,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Delay value for the case with turn restrictions is not as expected!",
        80845,
        leftTurnDelayWTurnRestriction,
        MatsimTestUtils.EPSILON);
  }

  private AnalyzeSingleIntersectionLeftTurnDelays runSimulation(
    ...
    config
        .controller()
        .setOutputDirectory(testUtils.getOutputDirectory() + intersectionLogic + "/");
