...
/**
 * @author tschlenther this class tests the functionality of TtTotalDelay in
 *     playground.dgrether.koehlerstrehlersignal.analysis which calculates the total delay of all
 *     agents in a network
 *     <p>having only one agent in a network, the total delay should be 0
 *     (testGetTotalDelayOnePerson) calculation of the delay of all persons in a network should be
 *     made in awareness of matsim's step logic
 *     <p>the network generated in this tests basically consists of 4 links of 1000m with a free
 *     speed of 201 m/s output is optionally written to outputDirectory of this testClass -> set
 *     field writeOutput to do so
 *     <p>the number of persons to be set in the network for the test can be modified they will get
 *     an insertion delay due to the capacity of the first link
 */
public class DelayAnalysisToolTest {
  ...
  public void testGetTotalDelayOnePerson() {
    ...
    Assert.assertEquals(
        "Total Delay of one agent is not correct",
        0.0,
        handler.getTotalDelay(),
  ...
  public void testGetTotalDelaySeveralPerson() {
    ...
    Assert.assertEquals(
        "Total Delay for " + NUMBER_OF_PERSONS + " persons is not correct.",
        expectedDelay,
        handler.getTotalDelay(),
        MatsimTestUtils.EPSILON);
  }
