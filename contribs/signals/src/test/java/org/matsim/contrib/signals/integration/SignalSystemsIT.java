...
public class SignalSystemsIT {
  ...
  private static final String CONFIG_FILE_NAME = "signalSystemsIntegrationConfig.xml";
    ...
    String inputDirectory = testUtils.getInputDirectory();
    {
      ...
      Assert.assertEquals(
          ...
          new EventsFileComparator()
              ...
              .runComparison(
                  inputDirectory + "0.events.xml.gz", iterationOutput + "0.events.xml.gz"));

      Scenario expectedPopulation = ScenarioUtils.createScenario(c.getConfig());
      new MatsimNetworkReader(expectedPopulation.getNetwork())
          .parse(c.getConfig().network().getInputFileURL(c.getConfig().getContext()));
      ...
      boolean works =
          PopulationUtils.equalPopulation(
              expectedPopulation.getPopulation(), actualPopulation.getPopulation());

      if (!works) {
        new org.matsim.api.core.v01.population.PopulationWriter(expectedPopulation.getPopulation())
            .write(testUtils.getOutputDirectory() + "/expected_plans_it0.xml.gz");
        new org.matsim.api.core.v01.population.PopulationWriter(actualPopulation.getPopulation())
            .write(testUtils.getOutputDirectory() + "/actual_plans_it0.xml.gz");
      }
    ...
    }
    {
      ...
      Assert.assertEquals(
          ...
          new EventsFileComparator()
              .setIgnoringCoordinates(true)
              .runComparison(
                  inputDirectory + "10.events.xml.gz", iterationOutput + "10.events.xml.gz"));
      ...
      if (!works) {
        new org.matsim.api.core.v01.population.PopulationWriter(expectedPopulation.getPopulation())
            .write(testUtils.getOutputDirectory() + "/expected_plans_it10.xml.gz");
        new org.matsim.api.core.v01.population.PopulationWriter(actualPopulation.getPopulation())
            .write(testUtils.getOutputDirectory() + "/actual_plans_it10.xml.gz");
      }
  ...
  public void testSignalSystemsWTryEndTimeThenDuration() {
    ...
    String inputDirectory = testUtils.getInputDirectory();
    {
      ...
      new MatsimNetworkReader(expectedPopulation.getNetwork())
          .parse(c.getConfig().network().getInputFileURL(c.getConfig().getContext()));
      new PopulationReader(expectedPopulation)
          .readFile(testUtils.getInputDirectory() + "0.plans.xml.gz");
