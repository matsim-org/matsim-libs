...
public class SignalsAndLanesOsmNetworkReaderTest {
  ...
  @Parameterized.Parameter public boolean setMergeOnewaySignalSystems;
  ...
  public OsmData constructSignalisedJunction() {
    ...
    TLongArrayList nodeWay1 =
        new TLongArrayList(new long[] {node11.getId(), signal1.getId(), node13.getId()});
    TLongArrayList nodeWay2 =
        new TLongArrayList(new long[] {node21.getId(), signal3.getId(), node13.getId()});

    TLongArrayList nodeWay3 =
        new TLongArrayList(new long[] {node13.getId(), signal4.getId(), node25.getId()});
    TLongArrayList nodeWay4 =
        new TLongArrayList(new long[] {node13.getId(), signal2.getId(), node15.getId()});
    ...
    OsmData osmData =
        new OsmData(
            Arrays.asList(
                node11, node13, node15, node21, node25, signal1, signal2, signal3, signal4),
  ...
  public void singleJunction() {
    ...
    SignalsAndLanesOsmNetworkReader signalReader =
    ...
    Assert.assertEquals(
        ...
        1,
        signalsData.getSignalSystemsData().getSignalSystemData().size());

    Coord junctionCoord =
        new Coord(osmData.nodes.get(1).getLatitude(), osmData.nodes.get(1).getLongitude());
    ...
    for (Id<Link> link : scenario.getLanes().getLanesToLinkAssignments().keySet()) {
      ...
      boolean shouldHave4Lanes =
          network.getLinks().get(link).getToNode().getId().toString().equals("13")
      ...
      if (shouldHave4Lanes) {
        ...
        Assert.assertEquals(
            "Number of Lanes on InLink of SignalisedJunction incorrect",
            4,
            scenario.getLanes().getLanesToLinkAssignments().get(link).getLanes().size());
      } else {
        Assert.assertEquals(
            "Number of Lanes on InLink of SignalisedJunction incorrect",
            2,
    ...
    Id<SignalSystem> systemId =
        signalsData.getSignalSystemsData().getSignalSystemData().keySet().iterator().next();
    int groups =
        signalsData.getSignalGroupsData().getSignalGroupDataBySignalSystemId().get(systemId).size();
  ...
  public void berlinSnippet() {
    ...
    if (setMakePedestrianSignals && setAllowUTurnAtLeftLaneOnly && setMergeOnewaySignalSystems) {
      ...
      Assert.assertEquals(
          "Assert no of links with lanes",
          128,
          scenario.getLanes().getLanesToLinkAssignments().size());
    }
