...
public class CalculateAngleTest {
  ...
  public void testGetLeftLane() {
    ...
    new MatsimNetworkReader(scenario.getNetwork())
        .parse(conf.network().getInputFileURL(conf.getContext()));

    Assert.assertEquals(
        "Has to be 'null', since there is no other way back but Link 11.",
        null,
        NetworkUtils.getLeftmostTurnExcludingU(
            scenario.getNetwork().getLinks().get(Id.create("1", Link.class))));

    Assert.assertEquals(
        scenario.getNetwork().getLinks().get(Id.create("2", Link.class)),
        NetworkUtils.getLeftmostTurnExcludingU(
            scenario.getNetwork().getLinks().get(Id.create("11", Link.class))));

    Assert.assertEquals(
        scenario.getNetwork().getLinks().get(Id.create("3", Link.class)),
        NetworkUtils.getLeftmostTurnExcludingU(
            scenario.getNetwork().getLinks().get(Id.create("22", Link.class))));

    Assert.assertEquals(
        scenario.getNetwork().getLinks().get(Id.create("4", Link.class)),
        NetworkUtils.getLeftmostTurnExcludingU(
            scenario.getNetwork().getLinks().get(Id.create("33", Link.class))));

    Assert.assertEquals(
        scenario.getNetwork().getLinks().get(Id.create("1", Link.class)),
        NetworkUtils.getLeftmostTurnExcludingU(
            scenario.getNetwork().getLinks().get(Id.create("44", Link.class))));

    Assert.assertEquals(
        scenario.getNetwork().getLinks().get(Id.create("5", Link.class)),
        NetworkUtils.getLeftmostTurnExcludingU(
            scenario.getNetwork().getLinks().get(Id.create("3", Link.class))));
  }
  ...
  public void testGetOutLinksSortedByAngle() {
    ...
    for (double angle = 0.0; angle < twicePi; angle = angle + piStep) {
      ...
      TreeMap<Double, Link> m =
          NetworkUtils.getOutLinksSortedClockwiseByAngle(
              net.getLinks().get(Id.create(1, Link.class)));
      Entry<Double, Link> entry = m.firstEntry();
      Assert.assertEquals(
          "For angle " + angle + "CalculateAngle returns not the correct order of outlinks",
      ...
      Link leftLane =
          NetworkUtils.getLeftmostTurnExcludingU(net.getLinks().get(Id.create(1, Link.class)));
      Assert.assertEquals(Id.create(2, Link.class), leftLane.getId());
    }
  }

  /**
   * creates a network with node 5 at 0,0 the other nodes build a cross spanning 200 m coordinates:
   * 3 | 2--5--4 | 1
   *
   * @param sc
   * @param ids
