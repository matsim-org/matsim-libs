...
class DefaultColorRange {
  ...
  private Color getColorBetween(Color lowerColor, Color upperColor, double power) {
    float[] lowerHSB =
        Color.RGBtoHSB(lowerColor.getRed(), lowerColor.getGreen(), lowerColor.getBlue(), null);
    float[] upperHSB =
        Color.RGBtoHSB(upperColor.getRed(), upperColor.getGreen(), upperColor.getBlue(), null);
