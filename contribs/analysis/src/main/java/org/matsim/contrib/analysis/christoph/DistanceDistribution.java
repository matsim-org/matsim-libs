...
/**
 * Add filters for: - mode - activity combination
 *
 * @author cdobler
 */
public class DistanceDistribution implements IterationEndsListener {

  private static final Logger log = LogManager.getLogger(DistanceDistribution.class);
  ...
  private void analyzePlan(Plan plan) {
    ...
    for (Trip trip : trips) {
      ...
      Tuple<String, String> tuple =
          new Tuple<String, String>(originActivityType, destinationActivityType);
      for (DistributionClass distributionClass : classes) {
        boolean containsMainMode = distributionClass.mainModes.contains(mainMode);
        boolean containsActivityTypePair =
            distributionClass.activityTypesFromTo.contains(tuple)
                || distributionClass.activityTypesFromTo.size() == 0;
              ...
              if (route instanceof NetworkRoute) {
                distance =
                    RouteUtils.calcDistanceExcludingStartEndLink(
                        (NetworkRoute) route, this.network);
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    for (DistributionClass distributionClass : classes) {
      writeDistanceDistributionClass(
          distributionClass, event.getIteration(), event.getServices().getControlerIO());
    }
  }

  private void writeDistanceDistributionClass(
    ...
    String fileName =
        outputDirectoryHierarchy.getIterationFilename(
            iteration, "DistanceDistribution_" + distributionClass.name);
  ...
  public void createAndAddDistanceBin(
      DistributionClass distributionClass, double low, double high, double referenceShare) {
    DistanceBin distanceBin = new DistanceBin(low, high, referenceShare);
    distributionClass.distributionBins.add(distanceBin);
  }

  public void addActivityCombination(
      DistributionClass distributionClass, String fromActivity, String toActivity) {
