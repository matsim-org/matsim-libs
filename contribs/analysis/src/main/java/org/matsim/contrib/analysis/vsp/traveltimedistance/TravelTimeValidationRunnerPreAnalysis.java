...
public class TravelTimeValidationRunnerPreAnalysis {
  ...
  /**
   * Proportion of trips will be chosen that cross the border of the key area (when key area shape
   * file is provided)
   */
  ...
  public TravelTimeValidationRunnerPreAnalysis(
      Network network,
      int trips,
      String outputFolder,
      String mode,
      TravelTimeDistanceValidator validator,
      Geometry keyAreaGeometry) {
  ...
  /**
   * Set the proportion of the inside trips to analyze. Note that insideTripProportion +
   * crossBorderTripProportion should be less than or equal to 1 The rest proportion will be outside
   * trips
   */
  ...
  /**
   * Set the proportion of the cross border trips to analyze. Note that insideTripProportion +
   * crossBorderTripProportion should be less than or equal to 1 The rest proportion will be outside
   * trips
   */
  ...
  public void run() throws IOException, InterruptedException {
    ...
    OnlyTimeDependentTravelDisutilityFactory disutilityFactory =
    ...
    CSVPrinter tsvWriter =
        new CSVPrinter(new FileWriter(outputFolder + "/pre-analysis-results.csv"), CSVFormat.TDF);
    ...
    while (counter < trips) {
      ...
      if (!fromLink.getToNode().getId().equals(toLink.getToNode().getId())) {
        ...
        double validatedTravelTime =
            validator.getTravelTime(fromCorrd, toCoord, 1, detailedFile).getFirst();
        counter++;
        Thread.sleep(100);
        if (validatedTravelTime < 60) {
          continue;
        }
        double simulatedTravelTime =
            router.calcLeastCostPath(fromLink.getToNode(), toLink.getToNode(), 0, null, null)
                .travelTime;
        tsvWriter.printRecord(
            Integer.toString(counter),
            tripType,
            Double.toString(fromCorrd.getX()),
            Double.toString(fromCorrd.getY()),
            Double.toString(toCoord.getX()),
            Double.toString(toCoord.getY()),
            Double.toString(simulatedTravelTime),
