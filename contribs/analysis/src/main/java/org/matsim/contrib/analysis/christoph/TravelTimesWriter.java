...
/**
 * Analyzes the average link travel times and writes them to files (absolute and relative values;
 * txt and shp files).
 *
 * <p>When added as ControlerListener, car travel times are collected and written based on the
 * TravelTimeCalculator provided by the services.
 *
 * <p>For other modes, first call collectTravelTimes(...) and then the writer methods.
 *
 * @author cdobler
 */
public class TravelTimesWriter implements IterationEndsListener {
  ...
  private void collectTravelTimes(
      TravelTime travelTime, Network network, double timeSlice, int numSlots) {
  ...
  public void writeAbsoluteSHPTravelTimes(
      String file, CoordinateReferenceSystem crs, boolean ignoreExitLinks) {
    try {
  ...
  public void writeRelativeSHPTravelTimes(
      String file, CoordinateReferenceSystem crs, boolean ignoreExitLinks) {
    try {
  ...
  private Collection<SimpleFeature> generateSHPFileData(
      CoordinateReferenceSystem crs, Network network, boolean absolute, boolean ignoreExitLinks)
      throws Exception {

    Collection<SimpleFeature> features = new ArrayList<SimpleFeature>();

    PolylineFeatureFactory.Builder builder =
    ...
    for (Link link : network.getLinks().values()) {
      ...
      Coordinate[] coordArray =
          new Coordinate[] {
            coord2Coordinate(link.getFromNode().getCoord()),
            coord2Coordinate(link.getCoord()),
            coord2Coordinate(link.getToNode().getCoord())
          };
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    if (writeSHPFiles) {
      String absoluteSHPTravelTimesFile =
          controlerIO.getIterationFilename(0, TravelTimesWriter.travelTimesAbsoluteSHPFile);
      String relativeSHPTravelTimesFile =
          controlerIO.getIterationFilename(0, TravelTimesWriter.travelTimesRelativeSHPFile);
