...
public class QGisFileWriter {
  ...
  public void writeHeaderAndStartElement(BufferedWriter out) throws IOException {
    ...
    out.write(
        "<qgis projectname=\""
            + this.writer.getProjectname()
            + "\" version=\""
            + QGisConstants.currentVersion
            + "\">\n");
  }
  ...
  private void writeLayerTreeLayer(BufferedWriter out, QGisLayer layer) throws IOException {
    ...
    out.write(
        "\t\t<layer-tree-layer expanded=\"1\" checked=\"Qt::Checked\" id=\""
  ...
  private void writeDestinationSrs(BufferedWriter out) throws IOException {
    ...
    out.write(
        "\t\t\t\t<projectionacronym>"
            + this.writer.getSRS().getProjectionacronym()
            + "</projectionacronym>\n");
    out.write(
        "\t\t\t\t<ellipsoidacronym>"
            + this.writer.getSRS().getEllipsoidacronym()
            + "</ellipsoidacronym>\n");
  ...
  private void writeVectorLayer(BufferedWriter out, QGisLayer layer) throws IOException {
    ...
    if (vlayer.getGeometryType().equals(QGisConstants.geometryType.No_geometry)) {
      ...
      out.write(
          "\t\t<maplayer minimumScale=\"0\" maximumScale=\"1e+08\" geometry=\""
              + vlayer.getGeometryType().toString().replace("_", " ")
              + "\" type=\""
              + vlayer.getType()
      ...
      out.write(
          "\t\t<maplayer minimumScale=\"0\" maximumScale=\"1e+08\" simplifyDrawingHints=\"1\" minLabelScale=\"0\" maxLabelScale=\"1e+08\" simplifyDrawingTol=\"1\" geometry=\""
              + vlayer.getGeometryType().toString()
    ...
    String relP =
        Paths.get(this.writer.getWorkingDir()).relativize(Paths.get(layer.getPath())).toString();
    ...
    if (isCsvDataLayer(vlayer)) {
      ...
      datasource +=
          "quote='&amp;escape='&amp;skipEmptyField=Yes&amp;xField="
              + vlayer.getXField()
              + "&amp;yField="
              + vlayer.getYField()
    ...
    out.write(
        "\t\t\t\t\t<projectionacronym>" + srs.getProjectionacronym() + "</projectionacronym>\n");
      ...
      for (VectorJoin vj : vlayer.getVectorJoins()) {
        ...
        out.write(
            "\t\t\t\t<join joinFieldName=\""
                + vj.getJoinFieldName()
                + "\" targetFieldName=\""
                + vj.getTargetFieldName()
                + "\""
                + " memoryCache=\"1\" joinLayerId=\""
                + vj.getJoinLayerId().toString()
                + "\"/>\n");
      }
    ...
    if (vlayer.getRenderer() != null) {
      ...
      out.write(
          "\t\t\t<layerTransparency>"
              + Integer.toString(vlayer.getLayerTransparency())
              + "</layerTransparency>\n");
    }
  ...
  private boolean isCsvDataLayer(VectorLayer vlayer) {
    ...
    return (vlayer.getInputType().equals(QGisConstants.inputType.csv)
        && !vlayer.getGeometryType().equals(QGisConstants.geometryType.No_geometry));
  }
  ...
  private void writeRasterLayer(BufferedWriter out, QGisLayer layer) throws IOException {
    ...
    out.write(
        "\t\t<maplayer minimumScale=\"0\" maximumScale=\"1e+08\" type=\""
            + rlayer.getType()
            + "\" hasScaleBasedVisibilityFlag=\"0\">\n");
    ...
    out.write(
        "\t\t\t\t<rasterrenderer opacity=\""
            + renderer.getOpacity()
            + "\" alphaBand=\""
            + renderer.getAlphaBand()
            + "\" blueBand=\""
            + renderer.getBlueBand()
            + "\" greenBand=\""
            + renderer.getGreenBand()
            + "\" type=\""
            + renderer.getType()
            + "\" redBand=\""
            + renderer.getRedBand()
  ...
  private void writeGeometryLayer(BufferedWriter out, VectorLayer layer) throws IOException {
    ...
    if (qRenderer.getRenderingType().equals(QGisConstants.renderingType.categorizedSymbol)) {
      ...
      out.write(
          "\t\t\t<renderer-v2 attr=\""
              + renderer.getRenderingAttribute()
              + "\" symbollevels=\"0\" type=\""
              + renderer.getRenderingType().toString()
              + "\">\n");
      ...
      for (int i = 0; i < renderer.getRanges().length; i++) {
        ...
        if (currentRange.isLowerBoundSet()) out.write("\" lower=\"" + currentRange.getLowerBound());
        if (currentRange.isUpperBoundSet()) out.write("\" upper=\"" + currentRange.getUpperBound());
      ...
      out.write(
          "\t\t\t<renderer-v2 attr=\""
              + renderer.getRenderingAttribute()
              + "\" symbollevels=\"0\" type=\""
              + QGisConstants.renderingType.RuleRenderer.toString()
              + "\">\n");
      ...
      out.write(
          "\t\t\t<renderer-v2 symbollevels=\"0\" type=\""
              + qRenderer.getRenderingType().toString()
              + "\">\n");
    }
  ...
  private void writePointLayer(BufferedWriter out, QGisLayer layer, int idx) throws IOException {
    ...
    QGisPointSymbolLayer psl =
        (QGisPointSymbolLayer) layer.getRenderer().getSymbolLayers().get(idx);

    String color =
        Integer.toString(psl.getColor().getRed())
            + ","
    ...
    String colorBorder =
        Integer.toString(psl.getColorBorder().getRed())
            + ","
    ...
    String offsetMapUnitScale =
        Double.toString(psl.getOffsetMapUnitScale()[0])
            + ","
            + Double.toString(psl.getOffsetMapUnitScale()[1]);

    out.write(
        "\t\t\t\t\t<symbol alpha=\""
            + psl.getLayerTransparency()
            + "\" type=\""
            + psl.getSymbolType().toString().toLowerCase()
            + "\" name=\""
            + idx
            + "\">\n");
    ...
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"name\" v=\"" + psl.getPointLayerSymbol().toString() + "\"/>\n");
    out.write("\t\t\t\t\t\t\t<prop k=\"offset\" v=\"" + offset + "\"/>\n");
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"offset_map_unit_scale\" v=\"" + offsetMapUnitScale + "\"/>\n");
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"offset_unit\" v=\"" + psl.getSizeUnits().toString() + "\"/>\n");
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"outline_style\" v=\"" + psl.getPenStyle().toString() + "\"/>\n");
    out.write("\t\t\t\t\t\t\t<prop k=\"outline_width\" v=\"0\"/>\n");
    out.write("\t\t\t\t\t\t\t<prop k=\"outline_width_map_unit_scale\" v=\"0,0\"/>\n");
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"outline_width_unit\" v=\""
            + psl.getSizeUnits().toString()
            + "\"/>\n");
    ...
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"size_unit\" v=\"" + psl.getSizeUnits().toString() + "\"/>\n");
  ...
  private void writeLineLayer(BufferedWriter out, QGisLayer layer, int idx) throws IOException {
    ...
    String color =
        Integer.toString(lsl.getColor().getRed())
            + ","
    ...
    out.write(
        "\t\t\t\t\t<symbol alpha=\"1\" type=\""
            + lsl.getSymbolType().toString().toLowerCase()
            + "\" name=\""
            + idx
            + "\">\n");
    ...
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"customdash_unit\" v=\""
            + lsl.getSizeUnits().toString()
            + "\"/>\n");
    ...
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"offset_unit\" v=\"" + lsl.getSizeUnits().toString() + "\"/>\n");
    ...
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"width_unit\" v=\"" + lsl.getSizeUnits().toString() + "\"/>\n");
  ...
  private void writePolygonLayer(BufferedWriter out, QGisLayer layer, int idx) throws IOException {
    ...
    QGisPolygonSymbolLayer psl =
        (QGisPolygonSymbolLayer) layer.getRenderer().getSymbolLayers().get(idx);
    ...
    String outlineColor =
        Integer.toString(psl.getOutlineColor().getRed())
            + ","
    ...
    out.write(
        "\t\t\t\t\t\t\t<prop k=\"outline_style\" v=\""
            + psl.getPenStyleBorder().toString()
            + "\"/>\n");
