...
public class TravelTimeValidationRunner {
  ...
  public TravelTimeValidationRunner(
      Network network,
      Set<Id<Person>> populationIds,
      String eventsFile,
      String outputFolder,
      TravelTimeDistanceValidator travelTimeValidator,
    ...
    this(
        ...
        TransportMode.car,
        travelTimeValidator,
        numberOfTripsToValidate,
        new Tuple<>((double) 0, (double) 3600 * 30),
        null);
  }
  ...
  public TravelTimeValidationRunner(
      ...
      int numberOfTripsToValidate,
      Tuple<Double, Double> timeWindow,
      Predicate<NetworkTrip> tripFilter) {
  ...
  public void run() throws InterruptedException, IOException {
    ...
    for (NetworkTrip trip : trips) {
      ...
      if (tripFilter != null && !tripFilter.test(trip)) continue;

      if (trip.getDepartureTime() >= timeWindow.getFirst()
          && trip.getDepartureTime() <= timeWindow.getSecond()) {
  ...
  private void writeTravelTimeValidation(String folder, List<NetworkTrip> trips)
    ...
    CSVPrinter tsvWriter =
        new CSVPrinter(new FileWriter(folder + "/validated_trips.tsv"), CSVFormat.TDF);
    ...
    try {
      ...
      for (NetworkTrip trip : trips) {
        if (trip.getValidatedTravelTime() != null) {
          tsvWriter.printRecord(trip.getTripData());
      ...
      final JFreeChart chart2 =
          ChartFactory.createScatterPlot(
              "Travel Times", "Simulated travel time [s]", "Validated travel time [s]", times);
      final JFreeChart chart =
          ChartFactory.createScatterPlot(
              "Travel Distances",
              "Simulated travel distance [m]",
              "Validated travel distance [m]",
              distances);
      ...
      XYAnnotation diagonald =
          new XYLineAnnotation(
              xAxisd.getRange().getLowerBound(),
              yAxisd.getRange().getLowerBound(),
              xAxisd.getRange().getUpperBound(),
              yAxisd.getRange().getUpperBound());
      ((XYPlot) chart.getPlot()).addAnnotation(diagonald);

      ChartUtils.writeChartAsPNG(
          new FileOutputStream(folder + "/validated_traveltimes.png"), chart2, 1500, 1500);
      ChartUtils.writeChartAsPNG(
          new FileOutputStream(folder + "/validated_traveldistances.png"), chart, 1500, 1500);
    } catch (IOException e) {
