...
/**
 * Data structure that allows to register data boundaries, and then to count data in the thereby
 * defined categories. The data structure allows multiple keys, so it is, for example, possible to
 * count different trip purposes into the same categories.
 *
 * @author nagel
 * @param <K>
 */
public final class Databins<K> {
  ...
  public int getIndex(double dblVal) {
    ...
    for (; ii >= 0; ii--) {
      if (dataBoundaries[ii] <= dblVal) return ii;
    }
    log.warn(
        "statistics contains value that smaller than the smallest category; adding it to smallest category");
