...
/**
 * Maps values to time bins. Time bins have a fixed size and are relative to the chosen start time.
 * If no start time is configured, 0 is assumed as start time of the first bin. New bins are created
 * as they are requested by {@link org.matsim.contrib.analysis.time.TimeBinMap#getTimeBin(double)}.
 * Note: only requested time bins are created, so that there is no guarantee that time bins are
 * consecutive.
 *
 * @param <T>
 */
public class TimeBinMap<T> {
  ...
  /**
   * Get a time bin for the given time. Returns an existing time bin or creates a new one if no bin
   * is present yet.
   *
  ...
  public TimeBin<T> getTimeBin(double forTime) {
    ...
    if (forTime < startTime)
      throw new IllegalArgumentException(
          "only times greater than startTime of: " + startTime + " allowed");
  ...
  /**
   * Retrieve all time bins. Note: There is no guarantee that time bins are consecutive. Since only
   * requested bins are created, it is possible that there are 'gaps' in the time series.
   *
