...
public class NetworkRouteValidator implements TravelTimeDistanceValidator {
  ...
  public NetworkRouteValidator(Network network, @Nullable String mode) {
    ...
    if (mode != null) {
      for (Link link : links) {
        if (!link.getAllowedModes().contains(mode)) network.removeLink(link.getId());
      }
    ...
    this.router =
        factory.createPathCalculator(network, new OnlyTimeDependentTravelDisutility(tt), tt);
    this.network = (SearchableNetwork) network;
  }

  @Override
  public Tuple<Double, Double> getTravelTime(
      Coord fromCoord, Coord toCoord, double departureTime, String tripId) {

    LeastCostPathCalculator.Path path =
        router.calcLeastCostPath(
            network.getNearestNode(fromCoord),
            network.getNearestNode(toCoord),
            departureTime,
            null,
            null);
