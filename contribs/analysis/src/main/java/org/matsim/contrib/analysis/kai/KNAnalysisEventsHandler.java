 ...
 * @author mrieser
 */
public class KNAnalysisEventsHandler
  ...
  private static final Logger log = LogManager.getLogger(KNAnalysisEventsHandler.class);
  ...
  // using this one here since presumably a fair number of the transit_walk trips in the survey in
  // fact were pt trips.  kai, sep'16
  ...
  // general trip counter.  Would, in theory, not necessary to do this per StatType, but I find it
  // too brittle
  ...
  /* deliberately package */ KNAnalysisEventsHandler(final Scenario scenario) {
    // this does not need to be injectable, since it is typically called from KaiAnalysisListener.
    // kai, may'18
    this.scenario = scenario;

    final String tollLinksFileName =
    ...
    for (StatType type : StatType.values()) {
      ...
      switch (type) {
        case legBeelineDistances:
          {
            double[] dataBoundariesTmp = {
              0., 100., 200., 500., 1000., 2000., 5000., 10000., 20000., 50000., 100000.
            };
        ...
        case tripBeelineDistancesCumulative:
          {
            ...
            double[] dataBoundariesTmp = {
              0., 100., 200., 400., 800., 1600., 3200., 6400., 12800., 25600., 51200., 102400.,
        ...
        case legDistances:
          {
            double[] dataBoundariesTmp = {
              0., 1000, 3000, 10000, 30000, 10000, 300000, 1000. * 1000.
            };
            Databins<String> databins = new Databins<>(type.name(), dataBoundariesTmp);
            this.statsContainer.put(type, databins);
            break;
        ...
        default:
          throw new RuntimeException(
              "statistics container for type " + type.toString() + " not initialized.");
      }
    }
    ...
    if (depTime != null && person != null) {
      ...
      for (StatType statType : StatType.values()) {
        ...
        switch (statType) {
            ...
            if (leg.getRoute() instanceof NetworkRoute) {
              item =
                  RouteUtils.calcDistanceExcludingStartEndLink(
                      ((NetworkRoute) leg.getRoute()), this.scenario.getNetwork());
          ...
          default:
            throw new RuntimeException(
                "`item' for statistics type not defined; statistics type: " + statType);
        }
        if (item != null) {
  ...
  public void reset(final int iteration) {
    ...
    for (Person person : this.scenario.getPopulation().getPersons().values()) {
      ...
      // yy I must have written this myself, but I don't know why there is the null check first.
      // kai, may'19
  ...
  public void handleEvent(PersonMoneyEvent event) {
    ...
    // (this is not additive by person, but it is additive by legType.  So if a person has multiple
    // money events, they
  ...
  public void writeStats(final String filenameTmp) {
    ...
    for (Person person : pop.getPersons().values()) {
      ...
      Gbl.assertNotNull(selectedPlan);
      {
        ...
        if (selectedPlan.getScore() != null) {
          // hapens in au-flood-evac, where new plan selection is at iteration end (maybe change
          // there). kai, may'18
          this.addItemToAllRegisteredTypes(
              categories, StatType.personScores, selectedPlan.getScore());
        }
      }
      {
        for (Trip trip : TripStructureUtils.getTrips(selectedPlan)) {
          ...
          double item =
              calcBeelineDistance(trip.getOriginActivity(), trip.getDestinationActivity());

          // this defines to which categories this person should belong for the statistical
          // averaging:
          List<String> categories = new ArrayList<>();
          categories.add(this.getSubpopName(person));
          ...
          this.addItemToAllRegisteredTypes(
              categories, StatType.tripBeelineDistancesCumulative, item);
          this.addItemToAllRegisteredTypes(
              categories, StatType.tripBeelineDistancesOtherBins, item);
        }
      }
    }
    ...
    for (Entry<String, double[]> entry :
        this.statsContainer.get(StatType.tripBeelineDistancesCumulative).entrySet()) {
      double sum = 0.;
    ...
    new ObjectAttributesXmlWriter(attribs)
        .writeFile(filenameTmp + "extendedPersonAttributes.xml.gz");
    ...
    for (Entry<Id<Link>, Double> entry : this.linkCnts.entrySet()) {
      ...
      linkAttribs.putAttribute(
          linkId.toString(), TTIME_SUM, this.linkTtimesSums.get(linkId).toString());
    }
    new ObjectAttributesXmlWriter(this.linkAttribs)
        .writeFile(filenameTmp + "networkAttributes.xml.gz");
        ...
        for (Entry<Id<Vehicle>, Double> entry : this.vehicleGantryCounts.entrySet()) {
          writer.write(
              entry.getKey()
                  + "\t"
                  + entry.getValue()
                  + "\t 1 "); // the "1" makes automatic processing a bit easier. kai, mar'14
        }
        writer.close();
      } catch (IOException e) {
        e.printStackTrace();
  ...
  private void writeStatsHorizontal(StatType statType, final java.io.Writer out)
    ...
    try {
      ...
      switch (statType) {
        ...
        case legDurationsOtherBins:
          out.write(
              "control statistics: average ttime = "
                  + (controlStatisticsSum / controlStatisticsCnt));
          out.write("\n");
          out.write("\n");
          break;
        case legBeelineDistances:
    ...
    if (this.otherTolledLinkIds.contains(event.getLinkId())) {
      final Person person =
          this.scenario
              .getPopulation()
              .getPersons()
              .get(delegate.getDriverOfVehicle(event.getVehicleId()));
  ...
  public void handleEvent(VehicleLeavesTrafficEvent event) {
    ...
    if (result == null) {
      if (wrnCnt == 0) {
        wrnCnt++;
        //				throw new RuntimeException("vehicle arrival for vehicle that never entered link.
        // teleportation?") ;
        LogManager.getLogger(this.getClass())
            .warn(
                "vehicle arrival for vehicle that never entered link.  I think this can happen with departures "
