...
public class VectorLayer extends QGisLayer {
  ...
  /**
   * Instantiates a new qgis layer that contains vector data (can still be geometry OR data). After
   * calling the constructor you have to: # Case 1: shp input file (and in general) 1) Specify a
   * renderer (how a layer is drawn) # Case 2: csv input and geometry 2) set an x and y field (to
   * tell QGis where the geometry information is stored) Optionally: set character that delimits
   * text fields (but causes problems if your lines end on a delimiter) If the csv file contains
   * data used for a vector join, no x and y fields need to be specified
   *
  ...
  public VectorLayer(
      String name,
      String path,
      org.matsim.contrib.analysis.vsp.qgis.QGisConstants.geometryType geometryType,
      boolean useHeader) {
  ...
  private void setHeader() {
    ...
    if (file.exists()) {
      ...
      throw new RuntimeException(
          "File " + this.getPath() + " cannot be found! Please check input path!");
    }
  }
  ...
  /**
   * Sets the x field to the specified column index. This method is used if no file header exists.
   *
  ...
  public void setXField(int columnIndex) {
    ...
    if (!this.useHeader) {
      ...
      throw new RuntimeException(
          "The file "
              + this.getPath()
              + " has a header! Use the method \"setXField(String xField)\" instead!");
    }
  }

  /**
   * Sets the x field to the specified column name. This method is used if a file header exists.
   *
   * @param xField the header field name of the column that contains the x coordinates of the
   *     geometries
   */
  public void setXField(String xField) {
    ...
    if (this.useHeader) {
      ...
      throw new RuntimeException(
          "The file "
              + this.getPath()
              + " has no header! Use the method \"setXField(int xField)\" instead!");
    }
  }

  /**
   * Sets the y field to the specified column index. This method is used if no file header exists.
   *
  ...
  /**
   * Sets the y field to the specified column name. This method is used if a file header exists.
   *
   * @param yField the header field name of the column that contains the x coordinates of the
   *     geometries
   */
  public void setYField(int columnIndex) {
    ...
    if (!this.useHeader) {
      ...
      throw new RuntimeException(
          "The file "
              + this.getPath()
              + " has a header! Use the method \"setYField(String yField)\" instead!");
    }
  }
  ...
  public void setYField(String yField) {
    ...
    if (this.useHeader) {
      ...
      throw new RuntimeException(
          "The file "
              + this.getPath()
              + " has no header! Use the method \"setYField(int yField)\" instead!");
    }
  }
  ...
  /**
   * Creates a vector join between a geometry layer (e.g. points) and a data layer (e.g.
   * accessibility data, immissions,...). This method must be called by the geometry layer because
   * the vector joins are stored there. You need to specify the name of the data layer and the field
   * names of the geometry and the data layer so QGis can merge objects with same field contents
   * (e.g. same Id). The field names of the layers don't have to be equal.
   *
   ...
   * @param zField
   */
  public void addVectorJoin(
      QGisLayer layer, String joinFieldName, String targetFieldName, String zField) {
