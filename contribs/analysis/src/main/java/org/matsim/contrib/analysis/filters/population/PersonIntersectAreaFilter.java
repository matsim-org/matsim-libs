...
/**
 * Filters all persons out whose routes do not lead at least once through one of the links within
 * the <code>areaOfInterest</code>, passing only persons to the next algorithm that travel at least
 * once on a link of the <code>areaOfInterest</code>. For the case a leg is missing route
 * information, and thus missing the information which links are used, an {@linkplain
 * #setAlternativeAOI(Coord, double) alternative area of interest can be specified}, given by its
 * center and a radius. In that case it is tested whether the bee-line connecting the start and end
...
public class PersonIntersectAreaFilter extends AbstractPersonFilter {
  ...
  public PersonIntersectAreaFilter(
      final PersonAlgorithm nextAlgorithm,
      final Map<Id<Link>, Link> areaOfInterest,
  ...
  /**
   * Sets an alternative area of interest, given by its center and a radius. This alternative aoi is
   * used when a route is missing in a leg and thus no links are available to decide. In that case
   * it is checked if the bee-line between from and to activity cuts the circle with radius <code>
   * aoiRadius</code> around <code>aoiCenter</code>.
   *
   * @param aoiCenter
   * @param aoiRadius
   */
