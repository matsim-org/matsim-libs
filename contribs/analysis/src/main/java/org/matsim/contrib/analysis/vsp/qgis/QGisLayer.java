...
public abstract class QGisLayer {
  ...
  /**
   * Creates a new instance of a qgis layer. For each geometry layer, a {@code QGisRenderer} must be
   * created (for use cases look at package layerTemplates). If the layer input is a csv file, you
   * also need to specify a delimiter (e.g. , or ;) and the header names of the columns that contain
   * the x and y coordinates (by setXField and setYField).
   *
  ...
  public QGisLayer(String name, String path) {
    ...
    this.path = path;
    this.id =
        Id.create(name + new SimpleDateFormat("yyyyMMdd").format(new Date()), QGisLayer.class);
