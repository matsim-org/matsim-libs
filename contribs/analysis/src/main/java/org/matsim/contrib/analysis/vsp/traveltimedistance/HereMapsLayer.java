...
public final class HereMapsLayer {
  ...
  public Result fetchAll(Envelope bbox, FC maxFC) {
    ...
    for (FC fc : FC.values()) {
      ...
      if (fc.compareTo(maxFC) > 0) return result;
      ...
      for (int i = tileX1; i <= tileX2; i++) {
        for (int j = tileY1; j <= tileY2; j++) {
          ...
          if (result == null) result = fetch;
          else result.add(fetch);
  ...
  public static final class Result {
    ...
    public String toString() {
      return "Result{" + "header=" + header + ", records=" + records.size() + '}';
    }
  }
  ...
  public enum VehicleType {
    ...
    public static Set<VehicleType> parse(int value) {
      ...
      for (VehicleType t : VehicleType.values()) {
        if ((value & t.bit) == t.bit) set.add(t);
      }
