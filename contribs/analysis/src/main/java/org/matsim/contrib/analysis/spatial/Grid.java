...
public abstract class Grid<T> {
  ...
  public Grid(
      final double horizontalCentroidDistance,
      final Supplier<T> initialValueSupplier,
      final Geometry bounds) {
    this(
        horizontalCentroidDistance,
        initialValueSupplier,
        new PreparedGeometryFactory().create(bounds));
  }
  ...
  public Collection<Cell<T>> getCells(Geometry bounds) {
    ...
    return quadTree.getRectangle(
        bounds.getEnvelopeInternal().getMinX(),
        bounds.getEnvelopeInternal().getMinY(),
        bounds.getEnvelopeInternal().getMaxX(),
        bounds.getEnvelopeInternal().getMaxY(),
        new ArrayList<>());
  }
  ...
  private void generateAllRows(
      final Supplier<T> initialValueSupplier, final PreparedGeometry bounds) {
  ...
  private void generateRow(
      final double y, final Supplier<T> initialValueSupplier, final PreparedGeometry bounds) {
