...
public final class HexagonalGrid<T> extends Grid<T> {
  ...
  public HexagonalGrid(
      double horizontalCentroidDistance, Supplier<T> initialValueSupplier, Geometry bounds) {
   ...
   */
  public HexagonalGrid(
      double horizontalCentroidDistance,
      Supplier<T> initialValueSupplier,
      PreparedGeometry bounds) {
    super(horizontalCentroidDistance, initialValueSupplier, bounds);
  }
  ...
  double getMinX(double forY) {
    ...
    if ((factor % 2) == 0) return quadTree.getMinEasting();
    else return quadTree.getMinEasting() + horizontalCentroidDistance / 2;
  }
