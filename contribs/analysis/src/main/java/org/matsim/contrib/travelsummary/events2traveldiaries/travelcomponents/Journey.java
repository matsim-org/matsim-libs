...
public class Journey extends TravelComponent {
  ...
  public double getInVehDistance() {
    if (getMainMode().equals("walk")) return 0;
    if (!isCarJourney()) {
      double distance = 0;
      for (Trip t : getTrips()) {
  ...
  double getWalkDistance() {
    if (getMainMode().equals("walk")) return walkSpeed * getDuration();
    if (!isCarJourney()) {
      double distance = 0;
      for (Walk w : getWalks()) {
  ...
  public double getEgressWalkDistance() {
    try {
      for (Walk w : getWalks()) {
        if (w.isEgressWalk()) return w.getDistance();
      }
    } catch (Exception e) {
      return 0;
    }
  ...
  public double getEgressWalkTime() {
    try {
      for (Walk w : getWalks()) {
        if (w.isEgressWalk()) return w.getDuration();
      }
    } catch (Exception e) {
      return 0;
    }
