...
/**
 * Renderer that draws graduated symbols. This type of renderer needs a rendering attribute (to
 * classify the symbols), ranges (from which to which value a symbol is drawn in a specific way) and
 * at least two symbol layers.
 *
...
public class GraduatedSymbolRenderer extends QGisRenderer {
  ...
  public GraduatedSymbolRenderer(
      VectorLayer layer, Double lowerBound, Double upperBound, Integer range, int symbolSize) {
  ...
  public void setRenderingAttribute(String attr) {
    if (this.useHeader) {
      ...
      throw new RuntimeException(
          "The input file for this renderer has no header. Use method \"setRenderingAttribute(int columnIndex)\" instead!");
    }
  }
  ...
  void setRenderingAttribute(int columnIndex) {
    if (!this.useHeader) {
      ...
      throw new RuntimeException(
          "The input file for this renderer has a header. Use method \"setRenderingAttribute(String attr)\" instead!");
    }
  }
    ...
    for (int i = 0; i < range - 2; i++) {
      ...
      this.ranges[i + 1] =
          new Range(
              lowerBoundary,
              upperBoundary,
              lowerBoundary.toString() + " - " + upperBoundary.toString());
    }

    if (range > 1) this.ranges[this.range - 1] = createLastRange();
