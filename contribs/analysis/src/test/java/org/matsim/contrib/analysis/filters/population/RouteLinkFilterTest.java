...
public class RouteLinkFilterTest extends MatsimTestCase {
	...
	private Population getTestPopulation() {
		...
		ActivityImpl a = plan.createAndAddActivityFromLinkId("h", link1.getId());
		...
		plan = PersonUtils.createAndAddPlan(person, true);
		ActivityImpl a2 = plan.createAndAddActivityFromLinkId("h", link1.getId());
		...
		plan = PersonUtils.createAndAddPlan(person, true);
		ActivityImpl a3 = plan.createAndAddActivityFromLinkId("h", link1.getId());
