...
//			printGlobalConfigGroupSettings();
			...
			Utils.printAccessibilityParameterSettings( log, getAccessibilityParameterConfig() );
...
<<<<<<< HEAD
	 ...
	 */
	private void initMATSim4UrbanSimModule(ConfigType matsim4urbansimConfigPart1) throws UncheckedIOException {
...
//	}
	...
	private void initGlobalSettings(){
		...
		Utils.printGlobalConfigGroupSettings(log, config);
		log.info("...done!");
	}
...
//			}
		...
		if(useMATSimCarParameter){
			...
			throw new RuntimeException("This MATSim4UrbanSim version does not support custom beta parameters such as \"betaBikeTravelTime\" etc. anymore (both in the UrbanSim GUI (car and walk) and the external MATSim config file (bike and pt)). Please let us know if this causes serious problems." +
...
//			betaPtLnTMC		= getValueAsDouble(BETA_PT_LN_TRAVEL_MONETARY_COST);
	...
	private void initNetwork(ConfigType matsimParameter){
		...
		Utils.printNetworkConfigGroupSettings(log, config);
		log.info("...done!");
	}
	...
	private void setPlansFile(String plansFile) {
		...
		Utils.printPlansConfigGroupSettings(log, config);
		log.info("...done!");
	}
	...
	private void initControler(ConfigType matsimParameter){
		...
		Utils.printControlerConfigGroupSetting(log, config);
		log.info("...done!");
	}
	...
	private void initPlanCalcScore(ConfigType matsimParameter){
		...
		Utils.printPlanCalcScoreSettings(log, config);
		log.info("...done!");
	}
	...
	private void initQSim(){
		...
		double popSampling = this.matsim4urbansimConfig.getMatsim4Urbansim().getUrbansimParameter().getPopulationSamplingRate();
		...
		Utils.printQSimConfigGroupSettings(log, config);
		log.info("...done!");
	}
	 ...
	 * setting strategy
	 */
	private void initStrategy(ConfigType matsim4urbansimConfig){
		...
		Utils.printStrategyConfigGroupSettings(log, config);
		log.info("...done!");
	}
	...
	private void initPlanCalcRoute(){
		...
		Utils.printPlanCalcRouteGroupSettings(log, config);
		log.info("...done!");
	}
	...
	private boolean useCustomMarginalUtilitiesBike(){
		if(matsim4UrbanSimModule != null) {
			boolean used = (matsim4UrbanSimModule.getValue(BETA_BIKE_TRAVEL_TIME) != null ||
	...
	private boolean useCustomMarginalUtilitiesPt(){
		if(matsim4UrbanSimModule != null) {
			boolean used = (matsim4UrbanSimModule.getValue(BETA_PT_TRAVEL_TIME) != null ||
