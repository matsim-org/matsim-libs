...
public class CreateTestNetwork {
	...
	public static String createTestPtStationCSVFile(){
		...
		try{
			bw.write("id,x,y" + org.matsim.contrib.matsim4urbansim.utils.InternalConstants.NEW_LINE); 	// header
			bw.write("1,10,10" + org.matsim.contrib.matsim4urbansim.utils.InternalConstants.NEW_LINE);	// pt stop next to node (3)
			bw.write("2,10, 190" + org.matsim.contrib.matsim4urbansim.utils.InternalConstants.NEW_LINE); // pt stop next to node (2)
			bw.write("3,190,190" + org.matsim.contrib.matsim4urbansim.utils.InternalConstants.NEW_LINE); // pt stop next to node (8)
			bw.write("4,190,10" + org.matsim.contrib.matsim4urbansim.utils.InternalConstants.NEW_LINE);  // pt stop next to node (9)
			bw.flush();
			bw.close();
		}
		catch(Exception e){
			...
			for (int origin = 1; origin <= 4; origin++){
				for (int destination = 1; destination <= 4; destination++){
					if (origin == destination)
						...
						bw.write(origin + " " + destination + " 0" + org.matsim.contrib.matsim4urbansim.utils.InternalConstants.NEW_LINE);
					else
						// set a dummy travel time/distance of 100s or 100m between different origin and destination pt stops
						bw.write(origin + " " + destination + " 100" + org.matsim.contrib.matsim4urbansim.utils.InternalConstants.NEW_LINE); 
				}
			}
			bw.flush();
			bw.close();
