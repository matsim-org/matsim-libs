...
public class SumoNetworkHandler extends DefaultHandler {
  ...
  static SumoNetworkHandler read(File file)
  ...
  /**
   * Merges another sumo network into this one. To work properly, this requires that edge und
   * junction ids are the same in both networks. This function does not clean left over edges, when
   * using this, a network cleaner should be user afterwards.
   *
  ...
  void merge(SumoNetworkHandler other, CoordinateTransformation ct) {
    ...
    Set<String> notDeadEnd =
        other.junctions.entrySet().stream()
    ...
    for (Map.Entry<String, Edge> e : other.edges.entrySet()) {
      if (edges.containsKey(e.getKey())) {
        for (int i = 0;
            i < Math.min(e.getValue().lanes.size(), edges.get(e.getKey()).lanes.size());
            i++) {
    ...
    other.junctions.forEach(
        (k, v) -> {
          if (notDeadEnd.contains(k)) junctions.put(k, v);
          else junctions.putIfAbsent(k, v);
        });
    ...
    for (Map.Entry<String, List<Connection>> e : other.connections.entrySet()) {
      ...
      } else connections.put(e.getKey(), e.getValue());
    }
    ...
    switch (qName) {
      ...
      case "type":
        ...
        types.put(
            ...
            new Type(
                typeId,
                attributes.getValue("allow"),
                attributes.getValue("disallow"),
      ...
      case "edge":
        ...
        if ("internal".equals(attributes.getValue("function"))) break;
  ...
  static final class Edge {
    ...
    @Nullable String origFrom;

    @Nullable String origTo;
  ...
  static final class Type {
    ...
    Type(String id, String allow, String disallow, double speed) {
      this.id = id;
      this.speed = speed;
      if (allow != null) Collections.addAll(this.allow, allow.split(" "));

      if (disallow != null) Collections.addAll(this.disallow, disallow.split(" "));
