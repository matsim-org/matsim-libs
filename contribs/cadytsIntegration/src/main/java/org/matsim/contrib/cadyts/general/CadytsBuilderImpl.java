...
public final class CadytsBuilderImpl {
  ...
  public static <T> AnalyticalCalibrator<T> buildCalibratorAndAddMeasurements(
      final Config config,
    ...
    int multiple =
        cadytsConfig.getTimeBinSize() / 3600; // e.g. "3" when timeBinSize_s = 3*3600 = 10800

    // If I remember correctly, the following is trying to get around the fact that the counts time
    // bins are fixed at hourly, but we want to
    // be able to be more flexible.  As a first step, time bins which are multiples from 1 hour are
    // allowed, say "3".  In order to get a somewhat
    // meaningful connection to the counts format, it will add up all entries from the corresponding
    // 3 hours before giving this to cadyts, i.e.
    // you may keep a higher resolution counts file but tell cadyts to work on longer time
    // intervals. kai, dec'13

    // yyyy However, it seems that some of this did not work: We are using "hourly" counts, and
    // dividing the multi-hour values by
    // the number of hours. ???????

    // yyyyyy I am currently of the opinion that the multi-hour version should be decoupled from the
    // counts format.  There is a
    // cadyts file format which allows setting mult-hour measurements, and that seems a lot more
    // direct than trying to use a file
    ...
    for (Map.Entry<Id<T>, Count<T>> entry : occupCounts.getCounts().entrySet()) {
      ...
      for (Volume volume : entry.getValue().getVolumes().values()) {
        ...
        if (!((timeBinIndex % multiple) == (multiple - 1))) {
          log.warn(
              " NOT adding measurement: timeBinIndex: " + timeBinIndex + "; multiple: " + multiple);
        } else {
          // (i.e. last timeBinIndex belonging to given bin)

          int endTimeOfBin_s = volume.getHourOfDayStartingWithOne() * 3600 - 1;
          if (!(cadytsConfig.getStartTime() <= startTimeOfBin_s
              && endTimeOfBin_s <= cadytsConfig.getEndTime())) {
            log.warn(
                " NOT adding measurement: cadytsConfigStartTime: "
                    + cadytsConfig.getStartTime()
                    + "; startTimeOfBin_s: "
                    + startTimeOfBin_s
                    + "; endTimeOfBin_s: "
                    + endTimeOfBin_s
            ...
            //						matsimCalibrator.addMeasurement(item, startTimeOfBin_s, endTimeOfBin_s,
            // count/multiple, SingleLinkMeasurement.TYPE.FLOW_VEH_H);
            ...
            // changed this from FLOW_VEH_H to COUNT_VEH on 30/jul/2012 since this is no longer
            // "hourly".
            // kai/manuel, jul'12
            // Despite the above comment, I am finding this with FLOW_VEH_H.  Why?  kai, feb'13
            // yyyyyy For the test case, this seems to produce weird results.  The expected counts
            // are 1 and 5, the expected result is 0 and 4.
            // Possibly, it divides 1 and 5 by 2 and then has 0.5 and 2.5 for the given hour, in
            // which case 1 and 3 would be best?????
            // kai, feb'13
          }
        }
        timeBinIndex++;
  ...
  public static <T> AnalyticalCalibrator<T> buildCalibrator(final Config config) {
    ...
    matsimCalibrator.setStatisticsFile(
        config.controller().getOutputDirectory() + "/calibration-stats.txt");
    return matsimCalibrator;
  }
}
