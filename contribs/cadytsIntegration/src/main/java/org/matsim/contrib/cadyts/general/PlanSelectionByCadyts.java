 ...
 * @author nagel
 */
@Deprecated // please test before using.  If it works, remove deprecated tag.  Optimally, write test
// case. kai, dec'13
public final class PlanSelectionByCadyts<T> implements PlanSelector<Plan, Person> {
  ...
  public Plan selectPlan(HasPlansAndId<Plan, Person> person) {
    ...
    for (Plan plan : person.getPlans()) {
      ...
      choiceProbs.add(
          pos,
          ExpBetaPlanSelector.getSelectionProbability(
              new ExpBetaPlanSelector(beta), plan.getPerson(), plan));
      // I guess these are supposed to be the prior probabilities. If so, than the above shoudl be
      // correct (albeit a bit expensive).
      // I would have expected that these are internally ignored when brute force is switched on,
      // but they are not.
      // I think that what it does is to use them when plans are equal according to cadyts. kai,
      // dec'13
