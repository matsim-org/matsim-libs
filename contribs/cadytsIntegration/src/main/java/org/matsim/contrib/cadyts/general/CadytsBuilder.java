...
//		if (occupCounts.getCounts().size() == 0) {
		...
		int numberOfAddedMeasurements = 0 ;
//		for (Map.Entry<Id<Link>, Count> entry : occupCounts.getCounts().entrySet()) {
		for (Map.Entry<Integer, Integer> entry : measurementsMap.entrySet()) {
			// (loop over all counting "items" (usually locations/stations)
...
//					
//					int endTimeOfBin_s   = volume.getHourOfDayStartingWithOne()*3600 - 1 ;
					int endTimeOfBin_s =  timeBinSize_s;
//					if ( !( cadytsConfig.getStartTime() <= startTimeOfBin_s && endTimeOfBin_s <= cadytsConfig.getEndTime()) ) {
//						log.warn( " NOT adding measurement: cadytsConfigStartTime: " + cadytsConfig.getStartTime() + "; startTimeOfBin_s: " + startTimeOfBin_s +
//								"; endTimeOfBin_s: " + endTimeOfBin_s + "; cadytsConfigEndTime: " + cadytsConfig.getEndTime() );
//					} else { //add volumes for each bin to calibrator
						numberOfAddedMeasurements++ ;
						System.out.println("count = " + count); // TODO delete later, for testing
						matsimCalibrator.addMeasurement(item, startTimeOfBin_s, endTimeOfBin_s, count, SingleLinkMeasurement.TYPE.COUNT_VEH );
//						matsimCalibrator.addMeasurement(value, startTimeOfBin_s, endTimeOfBin_s, count, SingleLinkMeasurement.TYPE.COUNT_VEH );
...
//			}
	...
	// -----------------------------------------------------------------------------------------------------------
//	public static <T> AnalyticalCalibrator<T> buildCalibrator_V2(final Config config, final Counts occupCounts, LookUp<T> lookUp, Class<T> idType ) {
	public static <T> AnalyticalCalibrator<T> buildCalibrator_V2(final Config config, final Measurements measurements, LookUp<T> lookUp, Class<T> idType ) {
...
//			log.warn("Counts container is empty.");
			log.warn("Measurments are empty.");
		}
		...
		int numberOfAddedMeasurements = 0 ;
//		for (Map.Entry<Id<Link>, Count> entry : occupCounts.getCounts().entrySet()) {
		for (Map.Entry<Id<Measurement>, Measurement> entry : measurements.getMeasurements().entrySet()) {
			// (loop over all counting "items" (usually locations/stations)
			...
			double count = -1 ;
//			for (Volume volume : entry.getValue().getVolumes().values()){
			for (org.matsim.contrib.cadyts.measurement.Volume volume : entry.getValue().getVolumes().values()) {
				// (loop over the different time slots)
