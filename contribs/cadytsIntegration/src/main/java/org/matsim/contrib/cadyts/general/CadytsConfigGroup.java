...
public class CadytsConfigGroup extends ReflectiveConfigGroup {
  ...
  private int preparatoryIterations =
      MATSimUtilityModificationCalibrator.DEFAULT_PREPARATORY_ITERATIONS;
  ...
  //			setStartTime((int)Time.parseTime(value));   //The "hh:mm:ss" format is converted here to
  // seconds after midnight
  //		} else if (END_TIME.equals(paramName)) {
  //			setEndTime((int)Time.parseTime(value));		//The "hh:mm:ss" format is converted here to seconds
  // after midnight
  //		} else if ( TIME_BIN_SIZE.equals(paramName)) {
  //			setTimeBinSize(Integer.parseInt(value)) ;
  //		} else if (CALIBRATED_LINKS.equals(paramName) || CALIBRATED_LINES.equals(paramName) ||
  // CALIBRATED_ITEMS.equals(paramName) ) {
  //			this.calibratedItems.clear();
  //			for (String linkId : CollectionUtils.stringToArray(value)) {
  //				this.calibratedItems.add( linkId );
  //			}
  //		} else {
  //			throw new IllegalArgumentException("Parameter '" + paramName + "' is not supported by config
  // group '" + GROUP_NAME + "'.");
  //		}
  //	}
  ...
  public final Map<String, String> getComments() {
    ...
    //		comments.put(CALIBRATED_ITEMS, "Comma-separated list of items with counts  to be
    // calibrated.");
    comments.put(
        START_TIME, "The first second of the day to be used for calibration.  hh:mm:ss format");
    comments.put(
        END_TIME, "The last second of the day to be used for calibration.  hh:mm:ss format");
    comments.put(
        TIME_BIN_SIZE,
  ...
  //	private String getCalibratedItemsAsString() { return CollectionUtils.setToString(
  // this.calibratedItems ); }
  //	@StringSetter( CALIBRATED_ITEMS )
  //	private void setCalibratedItemsAsString( String string ) { setCalibratedItems(
  // CollectionUtils.stringToSet( string ) ); }
  //	// ---
  ...
  private void setCalibratedLinesAsString(String string) {
    setCalibratedLines(CollectionUtils.stringToSet(string));
  }
  ...
  private void setCalibratedLinksAsString(String string) {
    setCalibratedLinks(CollectionUtils.stringToSet(string));
  }
