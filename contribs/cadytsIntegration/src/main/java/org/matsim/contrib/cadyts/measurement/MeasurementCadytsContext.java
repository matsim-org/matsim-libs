...
public class MeasurementCadytsContext
    implements CadytsContextI<Measurement>,
        StartupListener,
        IterationEndsListener,
        BeforeMobsimListener {

  static final Logger log = LogManager.getLogger(MeasurementCadytsContext.class);
  ...
  public void notifyStartup(StartupEvent event) {
    // yy could probably move all of this method into the constructor, and then get rid of this
    // method.  But should
    ...
    this.calibrator =
        buildCalibratorAndAddMeasurements(config, this.counts, measurements, Measurement.class);
  ...
  public void notifyBeforeMobsim(BeforeMobsimEvent event) {
    ...
    for (Person person : event.getServices().getScenario().getPopulation().getPersons().values()) {
      Plan<Measurement> planSteps =
          this.measurementListener.getCadytsPlan(person.getSelectedPlan());
  ...
  public void notifyIterationEnds(final IterationEndsEvent event) {
    if (this.writeAnalysisFile) {
      String analysisFilepath =
          event
              .getServices()
              .getControlerIO()
              .getIterationFilename(event.getIteration(), ANALYSIS_FILENAME);
    ...
    String filename =
        event
            .getServices()
            .getControlerIO()
            .getIterationFilename(event.getIteration(), COSTOFFSET_FILENAME);
    try {
