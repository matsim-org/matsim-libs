...
public class GenericCadytsConfigGroup<T> extends ConfigGroup {
...
//	private final Set<Id<Link>> calibratedItems = new HashSet<>();
	private final Set<Id<T>> calibratedItems = new HashSet<>();

	public GenericCadytsConfigGroup(Class<T> type) {
		super(GROUP_NAME);
		this.type = type;
	}
...
//			for (String linkId : CollectionUtils.stringToArray(value)) {
			for (String itemId : CollectionUtils.stringToArray(value)) {
//				this.calibratedItems.add(Id.create(linkId, Link.class));
				
//				this.calibratedItems.add(Id.create(linkId, type));
				this.calibratedItems.add(Id.create(itemId, this.type));
			}
		} else {
...
//	public final Set<Id<Link>> getCalibratedItems() {
	public final Set<Id<T>> getCalibratedItems() {
		return Collections.unmodifiableSet(this.calibratedItems);
	}

//	public final void setCalibratedItems(final Set<Id<Link>> links) {
	public final void setCalibratedItems(final Set<Id<T>> calibratedItems) {
		this.calibratedItems.clear();
		this.calibratedItems.addAll(calibratedItems);
	}
