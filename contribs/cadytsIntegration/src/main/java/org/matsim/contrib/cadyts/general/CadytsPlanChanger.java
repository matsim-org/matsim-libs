...
public class CadytsPlanChanger<T> implements PlanSelector<Plan, Person> {
  ...
  public Plan selectPlan(final HasPlansAndId<Plan, Person> person) {
    ...
    cadyts.demand.Plan<T> currentPlanSteps =
        this.cadytsContext.getPlansTranslator().getCadytsPlan(currentPlan);
    double currentPlanCadytsCorrection =
        this.cadytsContext.getCalibrator().calcLinearPlanEffect(currentPlanSteps) / this.beta;
    double currentScore = currentPlan.getScore() + cadytsWeight * currentPlanCadytsCorrection;

    cadyts.demand.Plan<T> otherPlanSteps =
        this.cadytsContext.getPlansTranslator().getCadytsPlan(otherPlan);
    double otherPlanCadytsCorrection =
        this.cadytsContext.getCalibrator().calcLinearPlanEffect(otherPlanSteps) / this.beta;
