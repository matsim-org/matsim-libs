 ...
 */
public class CadytsContext
    implements CadytsContextI<Link>, StartupListener, IterationEndsListener, BeforeMobsimListener {

  private static final Logger log = LogManager.getLogger(CadytsContext.class);
  ...
  CadytsContext(
      Config config,
      Scenario scenario,
      @Named(CadytsCarModule.CALIBRATION) Counts<Link> calibrationCounts,
    ...
    if (cadytsConfig.getCalibratedLinks().isEmpty()) {
      // found this without the above condition, i.e. it would always set the calibrated links to
      // all links for which there exist counts.
      // However, the logic for ptCounts is different, and there it would probably make sense to
      // keep the functionality that only some
      // lines are calibrated.  So, for symmetry, would make sense to also make it functional here.
      // ????  kai, feb'20
  ...
  public void notifyStartup(StartupEvent event) {
    ...
    this.calibrator =
        buildCalibratorAndAddMeasurements(
            ...
            new LinkLookUp(scenario) /*, cadytsConfig.getTimeBinSize()*/,
            Link.class);
  }
