...
public class CadytsCarModule extends AbstractModule {
  ...
  public void install() {
    if (calibrationCounts != null) {
      bind(Key.get(new TypeLiteral<Counts<Link>>() {}, Names.named(CALIBRATION)))
          .toInstance(calibrationCounts);
    } else {
      bind(Key.get(new TypeLiteral<Counts<Link>>() {}, Names.named(CALIBRATION)))
          .toProvider(CalibrationCountsProvider.class)
          .in(Singleton.class);
    }
    // In principle this is bind(Counts<Link>).to...  But it wants to keep the option of multiple
    // counts, under different names, open.
    // I think.  kai, jan'16
  ...
  private static class CalibrationCountsProvider implements Provider<Counts<Link>> {
    ...
    public Counts<Link> get() {
      ...
      new MatsimCountsReader(calibrationCounts)
          .parse(config.getCountsFileURL(matsimConfig.getContext()));
