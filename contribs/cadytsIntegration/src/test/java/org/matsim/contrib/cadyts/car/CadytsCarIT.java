...
/**
 * This is a modified copy of CadytsIntegrationTest (which is used for the cadyts pt integration) in
 * order to establish an according test for the cadyts car integration. At this stage all original
 * pt code is still included here, but outcommeted, to make the adaptations from pt to car well
 * traceable in case of any errors.
 */
public class CadytsCarIT {
  ...
  public final void testInitialization() {
              ...
              public void install() {
                ...
                install(
                    AbstractModule.override(
                        Collections.singleton(new ControlerDefaultsModule()),
                        new AbstractModule() {
                          @Override
                          public void install() {
                ...
                install(
                    new AbstractModule() {
                      @Override
                      public void install() {
                        addPlanStrategyBinding(CADYTS_STRATEGY_NAME)
                            .toProvider(
                                new jakarta.inject.Provider<PlanStrategy>() {
                                  @Inject Scenario scenario;
                                  @Inject CadytsContext cadytsContext;

                                  @Override
                                  public PlanStrategy get() {
                                    return new PlanStrategyImpl(
                                        new CadytsPlanChanger(scenario, cadytsContext));
                                  }
                                });
                      }
                    });
    ...
    Assert.assertEquals(
        8.0,
        context.getCalibrator().getMinStddev(SingleLinkMeasurement.TYPE.FLOW_VEH_H),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(1, context.getCalibrator().getPreparatoryIterations());
    Assert.assertEquals(
        0.95, context.getCalibrator().getRegressionInertia(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(1.0, context.getCalibrator().getVarianceScale(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        3600.0, context.getCalibrator().getTimeBinSize_s(), MatsimTestUtils.EPSILON);
  }
  ...
  public final void testCalibrationAsScoring() throws IOException {
    ...
    config
        .replanning()
        .addStrategySettings(
            new StrategySettings().setStrategyName(DefaultSelector.ChangeExpBeta).setWeight(1.0));
                                ...
                                .toInstance(
                                    new ScoringFunctionFactory() {
                                      ...
                                      public ScoringFunction createNewScoringFunction(
                                        ...
                                        SumScoringFunction scoringFunctionAccumulator =
                                            new SumScoringFunction();
                                        ...
                                        scoringFunction.setWeightOfCadytsCorrection(
                                            cadytsScoringWeight);
                                        scoringFunctionAccumulator.addScoringFunction(
                                            scoringFunction);
    ...
    Assert.assertEquals(
        "Different number of links in network.", scenario.getNetwork().getLinks().size(), 23);
    Assert.assertEquals(
        "Different number of nodes in network.", scenario.getNetwork().getNodes().size(), 15);

    Assert.assertNotNull("Population is null.", scenario.getPopulation());

    Assert.assertEquals(
        "Num. of persons in population is wrong.", scenario.getPopulation().getPersons().size(), 5);
    Assert.assertEquals(
        "Scale factor is wrong.",
        scenario.getConfig().counts().getCountsScaleFactor(),
        1.0,
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Count file is wrong.",
        scenario.getConfig().counts().getCountsFileName(),
        inputDir + "counts5.xml");
    ...
    Assert.assertEquals(
        "Occupancy counts description is wrong",
        occupCounts.getDescription(),
        "counts values for equil net");
    Assert.assertEquals("CsId is wrong.", count.getCsLabel(), "link_19");
    Assert.assertEquals(
        "Volume of hour 6 is wrong", count.getVolume(7).getValue(), 5.0, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Max count volume is wrong.",
        count.getMaxVolume().getValue(),
        5.0,
        MatsimTestUtils.EPSILON);

    String outCounts =
        outputDir + "ITERS/it." + lastIteration + "/" + lastIteration + ".countscompare.txt";
    ...
    CalibrationStatReader.StatisticsData outStatData =
        calibrationStatReader.getCalStatMap().get(lastIteration);
    // Assert.assertEquals("different Count_ll", "-0.046875", outStatData.getCount_ll() );
    // Assert.assertEquals("different Count_ll_pred_err",  "0.01836234363152515" ,
    // outStatData.getCount_ll_pred_err() );
    //			Assert.assertEquals("different Link_lambda_avg", "-2.2604922388914356E-10",
    // outStatData.getLink_lambda_avg() );
    Assert.assertEquals(
        "different Link_lambda_avg", "3.2261421242498865E-5", outStatData.getLink_lambda_avg());
    //			Assert.assertEquals("different Link_lambda_max", "0.0" , outStatData.getLink_lambda_max()
    // );
    //			Assert.assertEquals("different Link_lambda_min", "-7.233575164452593E-9",
    // outStatData.getLink_lambda_min() );
    //			Assert.assertEquals("different Link_lambda_stddev", "1.261054219517188E-9",
    // outStatData.getLink_lambda_stddev());
    //			Assert.assertEquals("different P2p_ll", "--" , outStatData.getP2p_ll());
    //			Assert.assertEquals("different Plan_lambda_avg", "-7.233575164452594E-9",
    // outStatData.getPlan_lambda_avg() );
    //			Assert.assertEquals("different Plan_lambda_max", "-7.233575164452593E-9" ,
    // outStatData.getPlan_lambda_max() );
    //			Assert.assertEquals("different Plan_lambda_min", "-7.233575164452593E-9" ,
    // outStatData.getPlan_lambda_min() );
    //			Assert.assertEquals("different Plan_lambda_stddev", "0.0" ,
    // outStatData.getPlan_lambda_stddev());
    ...
    String linkOffsetFile =
        outputDir + "ITERS/it." + lastIteration + "/" + lastIteration + ".linkCostOffsets.xml";

    CadytsCostOffsetsXMLFileIO<Link> offsetReader =
        new CadytsCostOffsetsXMLFileIO<Link>(new LinkLookUp(network), Link.class);
    ...
    // find first offset value different from null to compare. Useful to test with different time
    // bin sizes
    int binIndex = -1;
    boolean isZero;
    do {
      binIndex++;
      isZero =
          (Math.abs(linkOffsets.getBinValue(link19, binIndex) - 0.0) < MatsimTestUtils.EPSILON);
    } while (isZero && binIndex < 86400);

    Assert.assertEquals("Wrong bin index for first link offset", 6, binIndex);

    Assert.assertEquals(
        "Wrong link offset of link 11",
        0.0,
        linkOffsets.getBinValue(link11, binIndex),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong link offset of link 19",
        0.0014707121641471912,
        linkOffsets.getBinValue(link19, binIndex),
        MatsimTestUtils.EPSILON);
  }
