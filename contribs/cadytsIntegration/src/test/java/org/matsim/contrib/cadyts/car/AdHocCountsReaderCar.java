...
/**
 * Parses a output text file containing counts comparisons for car This is a modified copy of
 * CountsReader (which is used for the cadyts pt integration) in order to realize the according
 * functionality for the cadyts car integration. At this stage all original pt code is still
 * included here, but outcommeted, to make the adaptions from pt to car well traceable in case of
 * any errors.
 */
class AdHocCountsReaderCar {

  private static final Logger log = LogManager.getLogger(AdHocCountsReaderCar.class);
      ...
      while (row != null) {
        ...
        if (row != null && row != "") {
          ...
          this.count
              ...
              .put(
                  values[2],
                  // new double[] { Double.parseDouble(values[1]), Double.parseDouble(values[2]),
