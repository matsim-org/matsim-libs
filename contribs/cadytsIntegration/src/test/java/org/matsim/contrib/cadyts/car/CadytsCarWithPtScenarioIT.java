  ...
  public void testCadytsWithPtVehicles() {
    final Config config =
        ConfigUtils.loadConfig(
            IOUtils.extendUrl(
                ExamplesUtils.getTestScenarioURL("siouxfalls-2014"), "config_default.xml"));
    config
        .controller()
    ...
    controler
        ...
        .addHandler(
            (LinkLeaveEventHandler)
                event -> {
                  if (event.getLinkId().equals(testLink)
                      && event.getVehicleId().toString().startsWith("bus")) {
    ...
    assertTrue(
        "This test runs to the end, meaning cadyts doesn't throw an exception with pt", true);
  }
}
