...
public class SharingTeleportedRentalsHandler
    implements SharingPickupEventHandler,
  ...
  public SharingTeleportedRentalsHandler(
      EventsManager eventsManager, SharingServiceConfigGroup serviceParams) {
  ...
  public void handleEvent(TeleportationArrivalEvent event) {
    if (pickups.containsKey(event.getPersonId())) {
      distance.compute(
          event.getPersonId(), (k, v) -> v == null ? event.getDistance() : v + event.getDistance());
    }
  }
  ...
  public void handleEvent(SharingDropoffEvent event) {
    ...
    if (event.getServiceId().toString().equals(serviceParams.getId())) {
      ...
      // distance fare
      double sharedDistanceFare =
          this.distance.get(event.getPersonId()) * this.serviceParams.getDistanceFare();
      ...
      double sharedFare =
          Math.max(minimumFare, sharedBaseFare + sharedDistanceFare + sharedTimeFare);
      eventsManager.processEvent(
          new PersonMoneyEvent(
              event.getTime(),
              event.getPersonId(),
              -sharedFare,
              PERSON_MONEY_EVENT_PURPOSE_SHARING_FARE,
