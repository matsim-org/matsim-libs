...
public class SharingLogic {
  ...
  public boolean tryBookVehicle(double now, MobsimAgent agent) {
    ...
    if (closestVehicleInteraction.isPresent()) {
      ...
      if (dropoffActivity.getLinkId().equals(vehicleLinkId)) return false;
      ...
      List<? extends PlanElement> accessElements =
          routeAccessEgressStage(bookingActivity.getLinkId(), vehicleLinkId, now, agent);
      ...
      List<? extends PlanElement> mainElements =
          routeMainStage(vehicleLinkId, dropoffActivity.getLinkId(), now, agent);
      ...
      eventsManager.processEvent(
          new SharingReservingEvent(
              currentTime,
              service.getId(),
              agent.getId(),
              bookingActivity.getLinkId(),
              closestVehicleInteraction.get().getVehicle().getId(),
              closestVehicleInteraction.get().getStationId(),
              dropoffActivity.getLinkId()));
   ...
   *
   * <p>If it returns false, agent needs to abort!
   *
   * @param agent
   */
  public boolean tryPickupVehicle(double now, MobsimAgent agent) {
    ...
    if (selectedVehicleInteraction.isPresent()) {
      ...
      if (vehicleLinkId.equals(pickupActivity.getLinkId())) {
        ...
        // 1) Leg to pickup activity
        List<? extends PlanElement> accessElements =
            routeAccessEgressStage(pickupActivity.getLinkId(), vehicleLinkId, now, agent);
        updatedElements.addAll(accessElements);
        now = timeInterpretation.decideOnElementsEndTime(accessElements, now).seconds();

        // 2) Pickup activity
        Activity updatedPickupActivity =
            createPickupActivity(now, selectedVehicleInteraction.get());
        updatedElements.add(updatedPickupActivity);
        now = timeInterpretation.decideOnElementEndTime(updatedPickupActivity, now).seconds();
  ...
  private void setVehicle(
      Plan plan, List<? extends PlanElement> planElements, SharingVehicle vehicle) {
    ...
    for (PlanElement pe : planElements) {
      ...
      if (vehicleIds.size() > 1) {
        throw new IllegalStateException(
            "Tried to modify more than one vehicleId. Current code supports only non-vehicular access and egress modes");
      }
  ...
  public void tryDropoffVehicle(double now, MobsimAgent agent) {
    ...
    if (closestDropoffInteraction.getLinkId().equals(dropoffActivity.getLinkId())) {
      ...
      eventsManager.processEvent(
          new SharingDropoffEvent(
              now,
              service.getId(),
              agent.getId(),
              closestDropoffInteraction.getLinkId(),
              vehicle.getId(),
      ...
      Activity destinationActivity =
          (Activity) plan.getPlanElements().get(destinationActivityIndex);
      ...
      List<? extends PlanElement> mainElements =
          routeMainStage(
              dropoffActivity.getLinkId(), closestDropoffInteraction.getLinkId(), now, agent);
      updatedElements.addAll(mainElements);
      ...
      List<? extends PlanElement> accessElements =
          routeAccessEgressStage(
              closestDropoffInteraction.getLinkId(), destinationActivity.getLinkId(), now, agent);
      updatedElements.addAll(accessElements);
  ...
  private Activity createPickupActivity(double now, InteractionPoint interaction) {
    Activity activity =
        populationFactory.createActivityFromLinkId(
            SharingUtils.PICKUP_ACTIVITY, interaction.getLinkId());
  ...
  private Activity createDropoffActivity(double now, InteractionPoint interaction) {
    Activity activity =
        populationFactory.createActivityFromLinkId(
            SharingUtils.DROPOFF_ACTIVITY, interaction.getLinkId());
    activity.setStartTime(now);
    activity.setMaximumDuration(SharingUtils.INTERACTION_DURATION);
    SharingUtils.setServiceId(activity, service.getId());
  ...
  private List<? extends PlanElement> routeAccessEgressStage(
      Id<Link> originId, Id<Link> destinationId, double departureTime, MobsimAgent agent) {
    ...
    return accessEgressRoutingModule.calcRoute(
        DefaultRoutingRequest.of(
            ...
            getCurrentTripAttributes(agent)));
  }

  private List<? extends PlanElement> routeMainStage(
      Id<Link> originId, Id<Link> destinationId, double departureTime, MobsimAgent agent) {
    Facility originFacility = new LinkWrapperFacility(network.getLinks().get(originId));
    Facility destinationFacility = new LinkWrapperFacility(network.getLinks().get(destinationId));
