...
public class SharingRoutingModule implements RoutingModule {
  ...
  public List<? extends PlanElement> calcRoute(RoutingRequest request) {
    ...
    List<? extends PlanElement> exitElements =
    ...
    List<? extends PlanElement> pickupElements =
        routeAccessEgressStage(
            fromFacility.getLinkId(),
            pickupInteraction.get().getLinkId(),
            departureTime,
            person,
            request.getAttributes());
    allElements.addAll(pickupElements);
    departureTime =
        timeInterpretation.decideOnElementsEndTime(pickupElements, departureTime).seconds();
    ...
    departureTime =
        timeInterpretation.decideOnElementEndTime(pickupActivity, departureTime).seconds();

    // Route main stage

    List<? extends PlanElement> mainElements =
        routeMainStage(
            pickupActivity.getLinkId(),
            dropoffInteraction.get().getLinkId(),
            departureTime,
            person,
            request.getAttributes());
    ...
    departureTime =
        timeInterpretation.decideOnElementEndTime(dropoffActivity, departureTime).seconds();

    // Route dropoff stage

    List<? extends PlanElement> dropoffElements =
        routeAccessEgressStage(
  ...
  private List<? extends PlanElement> routeAccessEgressStage(
    ...
    return accessEgressRoutingModule.calcRoute(
        DefaultRoutingRequest.of(
            originFacility, destinationFacility, departureTime, person, tripAttributes));
  }
  ...
  private Activity createBookingActivity(double now, Id<Link> linkId) {
    Activity activity =
        populationFactory.createActivityFromLinkId(SharingUtils.BOOKING_ACTIVITY, linkId);
