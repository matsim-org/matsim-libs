...
public class SharingAnalysisModule extends AbstractModule {
  ...
  @Inject SharingConfigGroup sharingConfig;
  ...
  public void install() {
    ...
    for (SharingServiceConfigGroup serviceConfig : sharingConfig.getServices()) {
      install(
          new AbstractModalModule<SharingMode>(
              SharingUtils.getServiceMode(serviceConfig), SharingModes::mode) {
            @Override
            public void install() {
              bindModal(SharingVehicleStatusTimeProfileCollectorProvider.class)
                  .toProvider(
                      modalProvider(
                          getter -> {
                            MatsimServices matsimServices = getter.get(MatsimServices.class);
                            VehicleStateCollector vehicleStateCollector =
                                getter.getModal(VehicleStateCollector.class);
                            return new SharingVehicleStatusTimeProfileCollectorProvider(
                                vehicleStateCollector,
                                matsimServices,
                                SharingUtils.getServiceMode(serviceConfig));
                          }));
              addMobsimListenerBinding()
                  .toProvider(modalKey(SharingVehicleStatusTimeProfileCollectorProvider.class));
            }
          });
    }
  }
