...
/**
 * This class allows to read GBFS and convert it to the input file needed by the sharing contrib.
 * Currently this is only applicable to the station-based services.
 */
public class ReadGBFS {
  public static void main(String[] args) throws IOException, ConfigurationException {
    CommandLine cmd =
        new CommandLine.Builder(args) //
    ...
    Set<String> modes =
        Arrays.asList(cmd.getOptionStrict("network-modes").split(",")).stream()
            .map(String::trim)
...

    {
      ...
      if (stationsNode.isArray()) {
        for (JsonNode stationNode : stationsNode) {
          ...
          Coord coord =
              new Coord(
                  stationNode.findValue("lon").asDouble(), stationNode.findValue("lat").asDouble());
          coord = transformation.transform(coord);

          Link link = NetworkUtils.getNearestLink(network, coord);

          service.addStation(
              ImmutableSharingStationSpecification.newBuilder() //
      ...
      if (stationsNode.isArray()) {
        for (JsonNode stationNode : stationsNode) {
          ...
          for (int k = 0; k < numberOfBikes; k++) {
            service.addVehicle(
                ImmutableSharingVehicleSpecification.newBuilder() //
