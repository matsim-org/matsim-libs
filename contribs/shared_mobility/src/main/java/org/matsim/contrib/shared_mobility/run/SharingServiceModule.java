...
public class SharingServiceModule extends AbstractModalModule<SharingMode> {
  ...
  public void install() {
    ...
    bindModal(SharingServiceSpecification.class)
        .toProvider(
            modalProvider(
                getter -> {
                  SharingServiceSpecification specification =
                      new DefaultSharingServiceSpecification();
                  new SharingServiceReader(specification)
                      .readURL(
                          ConfigGroup.getInputFileURL(
                              getConfig().getContext(), serviceConfig.getServiceInputFile()));
                  return specification;
                }))
        .in(Singleton.class);

    bindModal(SharingRoutingModule.class)
        .toProvider(
            modalProvider(
                getter -> {
                  Scenario scenario = getter.get(Scenario.class);
                  RoutingModule accessEgressRoutingModule =
                      getter.getNamed(RoutingModule.class, TransportMode.walk);
                  RoutingModule mainModeRoutingModule =
                  ...
                  return new SharingRoutingModule(
                      scenario,
                      accessEgressRoutingModule,
                      mainModeRoutingModule,
                      interactionFinder,
                      Id.create(serviceConfig.getId(), SharingService.class),
                      timeInterpretation);
                }));

    addRoutingModuleBinding(getMode()).to(modalKey(SharingRoutingModule.class));

    bindModal(FreefloatingInteractionFinder.class)
        .toProvider(
            modalProvider(
                getter -> {
    ...
    bindModal(StationBasedInteractionFinder.class)
        .toProvider(
            modalProvider(
                getter -> {
                  Network network = getter.get(Network.class);
                  SharingServiceSpecification specification =
                      getter.getModal(SharingServiceSpecification.class);

                  return new StationBasedInteractionFinder(
                      network, specification, serviceConfig.getMaximumAccessEgressDistance());
                }));

    bindModal(OutputWriter.class)
        .toProvider(
            modalProvider(
                getter -> {
                  SharingServiceSpecification specification =
    ...
    bindModal(FreefloatingServiceValidator.class)
        .toProvider(
            modalProvider(
                getter -> {
                  return new FreefloatingServiceValidator(
                      Id.create(serviceConfig.getId(), SharingService.class));
                }))
        .in(Singleton.class);

    bindModal(StationBasedServiceValidator.class)
        .toProvider(
            modalProvider(
                getter -> {
                  return new StationBasedServiceValidator(
                      Id.create(serviceConfig.getId(), SharingService.class));
                }))
        .in(Singleton.class);

    bindModal(ValidationListener.class)
        .toProvider(
            modalProvider(
                getter -> {
                  SharingServiceSpecification specification =
                      getter.getModal(SharingServiceSpecification.class);
                  SharingServiceValidator validator =
                      getter.getModal(SharingServiceValidator.class);
    ...
    bindModal(VehicleStateCollector.class)
        .toProvider(
            modalProvider(
                getter -> {
                  SharingServiceSpecification specification =
    ...
    if (((QSimConfigGroup) getConfig().getModules().get(QSimConfigGroup.GROUP_NAME))
        .getMainModes()
