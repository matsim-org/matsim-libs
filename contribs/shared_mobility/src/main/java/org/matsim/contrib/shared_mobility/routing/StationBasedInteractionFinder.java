...
public class StationBasedInteractionFinder implements InteractionFinder {
  ...
  public StationBasedInteractionFinder(
      Network network, SharingServiceSpecification specification, double maximumDistance) {
    this.network = network;
    this.maxmimumDistance = maximumDistance;

    this.stations =
        QuadTrees.createQuadTree(
            specification.getStations(),
            spec -> network.getLinks().get(spec.getLinkId()).getCoord(),
            1000.0);
  }
  ...
  private Optional<InteractionPoint> findStation(Facility nearbyFacility) {
    ...
    if (nearbyLink != null) {
      SharingStationSpecification station =
          stations.getClosest(nearbyLink.getCoord().getX(), nearbyLink.getCoord().getY());
      Link stationLink = network.getLinks().get(station.getLinkId());

      if (CoordUtils.calcEuclideanDistance(stationLink.getCoord(), nearbyLink.getCoord())
          <= maxmimumDistance) {
