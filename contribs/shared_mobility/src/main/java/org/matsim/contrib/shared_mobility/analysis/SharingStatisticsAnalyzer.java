...
public class SharingStatisticsAnalyzer implements IterationEndsListener {
  ...
  @Inject
  SharingStatisticsAnalyzer(
      Scenario scenario, SharingLegCollector sharingLegCollector, MatsimServices matsimServices) {
  ...
  void writeAllLeg(String filename) {
    ...
    Map<Id<SharingService>, Collection<SharingLeg>> legsPerService =
        sharingLegCollector.getSharingLegs();
  ...
  private String getFilename(IterationEndsEvent event, String prefix, String extension) {
    return matsimServices
        .getControlerIO()
        .getIterationFilename(event.getIteration(), prefix + "_" + extension);
  }
