...
public class StationBasedService implements SharingService {
  ...
  public StationBasedService(
    ...
    vehicles
        ...
        .forEach(
            v -> {
              availableVehicles.put(
                  v.getLink().getCoord().getX(), v.getLink().getCoord().getY(), v);
            });

    this.availableStations = new QuadTree<>(bounds[0], bounds[1], bounds[2], bounds[3]);
    stations.values().stream()
        .filter(s -> s.getFreeCapacity() > 0)
        .forEach(
            s -> {
              availableStations.put(
                  s.getLink().getCoord().getX(), s.getLink().getCoord().getY(), s);
            });
  }
  ...
  public Optional<VehicleInteractionPoint> findClosestVehicle(MobsimAgent agent) {
    ...
    SharingVehicle vehicle =
        availableVehicles.getClosest(currentLink.getCoord().getX(), currentLink.getCoord().getY());

    if (CoordUtils.calcEuclideanDistance(vehicle.getLink().getCoord(), currentLink.getCoord())
        > maximumDistance) {
      return Optional.empty();
    }
  ...
  public InteractionPoint findClosestDropoffLocation(SharingVehicle vehicle, MobsimAgent agent) {
    ...
    return InteractionPoint.of(
        availableStations.getClosest(currentLink.getCoord().getX(), currentLink.getCoord().getY()));
  }
  ...
  public void reserveVehicle(MobsimAgent agent, SharingVehicle vehicle) {
    Verify.verify(
        !reservations.values().contains(vehicle)); // This vehicle should be reserved only once
    Coord coord = vehicle.getLink().getCoord();
    availableVehicles.remove(coord.getX(), coord.getY(), vehicle);
    reservations.put(agent.getId(), vehicle);
    LOG.debug(
        "Reserve vehicle {} from agent {}", vehicle.getId().toString(), agent.getId().toString());
  }
