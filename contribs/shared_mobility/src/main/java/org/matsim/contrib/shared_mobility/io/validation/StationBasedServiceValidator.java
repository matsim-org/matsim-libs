...
public class StationBasedServiceValidator implements SharingServiceValidator {
  ...
  public void validate(SharingServiceSpecification specification) {
    ...
    for (SharingStationSpecification station : specification.getStations()) {
      Verify.verify(
          !stationIds.contains(station.getId()),
          "Service %s has duplicate station %s",
          serviceId.toString(),
    ...
    for (SharingVehicleSpecification vehicle : specification.getVehicles()) {
      Verify.verify(
          !vehicleIds.contains(vehicle.getId()),
          "Service %s has duplicate vehicle %s",
          serviceId.toString(),
          vehicle.getId().toString());

      Verify.verify(
          vehicle.getStartStationId().isPresent(),
          "Vehicle %s of service %s needs start link",
          vehicle.getId().toString(),
          serviceId.toString());

      Verify.verify(
          stationIds.contains(vehicle.getStartStationId().get()),
          "Station %s for vehicle %s does not exist in service %s",
          vehicle.getStartStationId().get().toString(),
          vehicle.getId().toString(),
          serviceId.toString());
