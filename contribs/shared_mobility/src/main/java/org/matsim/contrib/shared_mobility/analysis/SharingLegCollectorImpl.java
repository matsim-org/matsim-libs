 ...
 * @author steffenaxer
 */
public class SharingLegCollectorImpl
    implements TeleportationArrivalEventHandler,
        LinkLeaveEventHandler,
        PersonEntersVehicleEventHandler,
        PersonLeavesVehicleEventHandler,
        SharingLegCollector,
        SharingVehicleEventHandler,
        SharingPickupEventHandler,
        SharingDropoffEventHandler,
  ...
  private final IdMap<Person, SharingLegBuilder> person2CurrentSharingLegBuilder =
      new IdMap<>(Person.class);
  private final IdMap<SharingService, Collection<SharingLeg>> sharingService2SharingLegs =
      new IdMap<>(SharingService.class);
  ...
  public void handleEvent(SharingDropoffEvent event) {
    ...
    SharingLegBuilder sharingLegBuilder =
        this.person2CurrentSharingLegBuilder.remove(event.getPersonId());
    sharingLegBuilder.setArrivalTime(event.getTime());
    sharingLegBuilder.setToCoord(
        scenario.getNetwork().getLinks().get(event.getLinkId()).getCoord());
  ...
  public void handleEvent(SharingPickupEvent event) {
    ...
    SharingLegBuilder currentSharingLegBuilder =
        this.person2CurrentSharingLegBuilder.get(event.getPersonId());

    if (currentSharingLegBuilder != null) {
      throw new IllegalStateException(
          "Adding new SharingLegBuilder without finishing previous instance");
    }
    ...
    sharingLegBuilder.setFromCoord(
        scenario.getNetwork().getLinks().get(event.getLinkId()).getCoord());
