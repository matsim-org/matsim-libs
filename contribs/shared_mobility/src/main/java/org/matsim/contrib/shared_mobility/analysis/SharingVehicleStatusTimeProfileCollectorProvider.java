...
public class SharingVehicleStatusTimeProfileCollectorProvider implements Provider<MobsimListener> {
	...
	@Inject
	public SharingVehicleStatusTimeProfileCollectorProvider(VehicleStateCollector vehicleStateCollector, MatsimServices matsimServices,
			String serviceMode) {
	...
	public MobsimListener get() {
		var header = ImmutableList.of(SHARING_VEHICLE_STATES.RESERVED.toString(), SHARING_VEHICLE_STATES.BOOKED.toString(),
		...
		TimeProfileCollector collector = new TimeProfileCollector(header, calculator, 120, this.serviceMode + "_vehicle_status_time_profiles",
				matsimServices);
