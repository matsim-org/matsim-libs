...
/**
 * Tests {@link RouteDesignScoringManager}, {@link AreaBtwStopsVsTerminiBeelinePenalty}, {@link
 * Stop2StopVsTerminiBeelinePenalty}.
 *
 * <p>The latter two are tested here, because they are also used as two example
...
public class RouteDesignScoringManagerTest {
  ...
  public void testRectangularLine() {
    ...
    NetworkRoute route =
        RouteUtils.createLinkNetworkRouteImpl(
            Id.createLinkId("dummy1.1"), Id.createLinkId("dummy1.2"));

    PPlan pPlan1 =
        new PPlan(
            Id.create("PPlan1", PPlan.class), "creator1", Id.create("PPlanParent1", PPlan.class));
    ...
    TransitRoute route1 =
        factory.createTransitRoute(
            Id.create("TransitRoute1", TransitRoute.class), route, stops, "bus");
    ...
    stop2stopVsBeeline.setRouteDesignScoreFunction(
        RouteDesignScoreFunctionName.stop2StopVsBeelinePenalty);
    ...
    areaVsBeeline.setRouteDesignScoreFunction(
        RouteDesignScoreFunctionName.areaBtwStopsVsBeelinePenalty);
    ...
    expected =
        -1 * ((10 * 10 / (10 * Math.sqrt(2))) - 1) + (-1) * ((4 * 10 / (10 * Math.sqrt(2))) - 1);
    Assert.assertEquals(expected, actual, 0.001);
    ...
    route =
        RouteUtils.createLinkNetworkRouteImpl(
            Id.createLinkId("dummy2.1"), Id.createLinkId("dummy2.2"));

    PPlan pPlan2 =
        new PPlan(
            Id.create("PPlan2", PPlan.class), "creator2", Id.create("PPlanParent2", PPlan.class));
    pPlan2.setStopsToBeServed(stopsToBeServed);
    TransitLine line2 = factory.createTransitLine(Id.create("line2", TransitLine.class));
    TransitRoute route2 =
        factory.createTransitRoute(
            Id.create("TransitRoute2", TransitRoute.class), route, stops, "bus");
    ...
    // high valueToStartScoring -> all scores below would be positive, check that they are capped at
    // 0
    ...
    PPlan pPlan3 =
        new PPlan(
            Id.create("PPlan3", PPlan.class), "creator3", Id.create("PPlanParent3", PPlan.class));
    pPlan3.setStopsToBeServed(stopsToBeServed);
    TransitLine line3 = factory.createTransitLine(Id.create("line3", TransitLine.class));
    TransitRoute route3 =
        factory.createTransitRoute(
            Id.create("TransitRoute3", TransitRoute.class), route, stops, "bus");
  ...
  private TransitStopFacility getOrCreateStopAtCoord(int x, int y) {
    ...
    if (scenario.getTransitSchedule().getFacilities().containsKey(stopId)) {
      ...
      return factory.createTransitStopFacility(stopId, CoordUtils.createCoord(x, y), false);
    }
  }
