...
public class ComplexCircleScheduleProviderTest {
  ...
  public final void testCreateTransitLineLikeSimpleCircleScheduleProvider() {
    ...
    TransitSchedule tS =
        CreateStopsForAllCarLinks.createStopsForAllCarLinks(scenario.getNetwork(), pC);
    ...
    TransitStopFacility startStop =
        tS.getFacilities().get(Id.create(pC.getPIdentifier() + "1424", TransitStopFacility.class));
    TransitStopFacility endStop =
        tS.getFacilities().get(Id.create(pC.getPIdentifier() + "4434", TransitStopFacility.class));
    ...
    refIds.add(Id.create("2434", Link.class));
    refIds.add(Id.create("3444", Link.class));
    refIds.add(Id.create("4434", Link.class));
    refIds.add(Id.create("3424", Link.class));
    refIds.add(Id.create("2414", Link.class));
    refIds.add(Id.create("1424", Link.class));

    TransitLine line = prov.createTransitLineFromOperatorPlan(lineId, plan);

    Assert.assertEquals(
        "Transit line ids have to be the same", Id.create(lineId, TransitLine.class), line.getId());

    for (TransitRoute route : line.getRoutes().values()) {
      Assert.assertEquals(
          "Route id have to be the same",
          Id.create(lineId + "-" + routeId, TransitRoute.class),
          route.getId());
      Assert.assertEquals(
          "Number of departures", 14.0, route.getDepartures().size(), MatsimTestUtils.EPSILON);

      // check links
      Assert.assertEquals(
          "Start link id has to be the same", refIds.get(0), route.getRoute().getStartLinkId());
      ...
      Assert.assertEquals(
          "End link id has to be the same",
          refIds.get(refIds.size() - 1),
          route.getRoute().getEndLinkId());

      // check stops
      i = 0;
      for (TransitRouteStop stop : route.getStops()) {
        Assert.assertEquals(
            "Route stop ids have to be the same",
            Id.create(pC.getPIdentifier() + refIds.get(i), TransitStopFacility.class),
  ...
  public final void testCreateTransitLineWithMoreStops() {
    ...
    TransitStopFacility stop2 =
        tS.getFacilities().get(Id.create(pC.getPIdentifier() + "2423", TransitStopFacility.class));
    TransitStopFacility stop3 =
        tS.getFacilities().get(Id.create(pC.getPIdentifier() + "2333", TransitStopFacility.class));
    TransitStopFacility stop4 =
        tS.getFacilities().get(Id.create(pC.getPIdentifier() + "3433", TransitStopFacility.class));
    TransitStopFacility stop5 =
        tS.getFacilities().get(Id.create(pC.getPIdentifier() + "3334", TransitStopFacility.class));
    ArrayList<TransitStopFacility> stopsToBeServed = new ArrayList<>();
    ...
    ArrayList<Id<Link>> refIds = new ArrayList<>();
    refIds.add(Id.create("1424", Link.class));
    refIds.add(Id.create("2423", Link.class));
    refIds.add(Id.create("2333", Link.class));
    refIds.add(Id.create("3334", Link.class));
    refIds.add(Id.create("3433", Link.class));
    refIds.add(Id.create("3334", Link.class));
    refIds.add(Id.create("3424", Link.class));
    refIds.add(Id.create("2414", Link.class));
    refIds.add(Id.create("1424", Link.class));
    ...
    for (TransitRoute route : line.getRoutes().values()) {
      ...
      Assert.assertEquals(
          "Number of departures", 11.0, route.getDepartures().size(), MatsimTestUtils.EPSILON);
    }
  }
  ...
  public final void testGetRandomTransitStop() {
    ...
    RandomStopProvider randomStopProvider =
        new RandomStopProvider(pC, scenario.getPopulation(), scenario.getTransitSchedule(), null);

    SimpleCircleScheduleProvider prov =
        new SimpleCircleScheduleProvider(
            pC.getPIdentifier(),
