...
/**
 * Calculates the average trip distance per ptModes specified. A trip starts by entering a vehicle
 * and end by leaving one.
 *
 * @author aneumann
 */
final class AverageTripDistanceMeterPerMode extends AbstractPAnalyisModule
    implements TransitDriverStartsEventHandler,
        PersonEntersVehicleEventHandler,
        PersonLeavesVehicleEventHandler,
        LinkEnterEventHandler {

  private static final Logger log = LogManager.getLogger(AverageTripDistanceMeterPerMode.class);
  ...
  private HashMap<Id<Vehicle>, HashMap<Id<Person>, Double>>
      vehId2AgentId2DistanceTravelledInMeterMap = new HashMap<>();
  ...
  public String getResult() {
    ...
    for (String ptMode : this.ptModes) {
      strB.append(
          ", "
              + (this.ptMode2MeterTravelledMap.get(ptMode) / this.ptMode2TripCountMap.get(ptMode)));
    }
  ...
  public void handleEvent(PersonEntersVehicleEvent event) {
    if (this.vehId2AgentId2DistanceTravelledInMeterMap.get(event.getVehicleId()) == null) {
      this.vehId2AgentId2DistanceTravelledInMeterMap.put(
          event.getVehicleId(), new HashMap<Id<Person>, Double>());
    }

    if (!super.ptDriverIds.contains(event.getPersonId())) {
      this.vehId2AgentId2DistanceTravelledInMeterMap
          .get(event.getVehicleId())
          .put(event.getPersonId(), 0.0);
    }
  }
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    if (!super.ptDriverIds.contains(event.getPersonId())) {
      ...
      this.ptMode2MeterTravelledMap.put(
          ptMode,
          this.ptMode2MeterTravelledMap.get(ptMode)
              + this.vehId2AgentId2DistanceTravelledInMeterMap
                  .get(event.getVehicleId())
  ...
  public void handleEvent(LinkEnterEvent event) {
    ...
    for (Id<Person> agentId :
        this.vehId2AgentId2DistanceTravelledInMeterMap.get(event.getVehicleId()).keySet()) {
      double oldValue =
          this.vehId2AgentId2DistanceTravelledInMeterMap.get(event.getVehicleId()).get(agentId);
      this.vehId2AgentId2DistanceTravelledInMeterMap
          .get(event.getVehicleId())
          .put(agentId, oldValue + newValue);
    }
  }
}
