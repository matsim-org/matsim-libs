 ...
 * @author aneumann
 */
final class AverageLoadPerDeparturePerMode extends AbstractPAnalyisModule
    implements TransitDriverStartsEventHandler,
        PersonEntersVehicleEventHandler,
        PersonLeavesVehicleEventHandler,
        VehicleDepartsAtFacilityEventHandler {

  private static final Logger log = LogManager.getLogger(AverageLoadPerDeparturePerMode.class);
  ...
  public void updateVehicles(Vehicles vehicles) {
    ...
    for (Vehicle veh : vehicles.getVehicles().values()) {
      ...
      this.vehId2VehicleCapacity.put(
    ...
    if (this.vehId2PaxMap.get(event.getVehicleId()) != null) {
      if (this.vehId2PaxMap.get(event.getVehicleId()) != 0) {
        log.warn(
            event.getVehicleId()
                + " has still "
                + this.vehId2PaxMap.get(event.getVehicleId())
                + " passengers onboard. Should be zero by now.");
      }
    }
  }
