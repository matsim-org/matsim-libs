...
final class OperatorInitializer {
  ...
  private static final Logger log = LogManager.getLogger(OperatorInitializer.class);
  ...
  OperatorInitializer(
    ...
    this.routeProvider =
        PRouteProviderFactory.createRouteProvider(
  ...
  public LinkedList<Operator> createOperatorsFromSchedule(TransitSchedule originalSchedule) {
    return new CreateOperatorFromTransitSchedule(
            this.operatorFactory, this.routeProvider, this.pConfig, originalSchedule)
        .run();
  }
   ...
   * @return
   */
  public LinkedList<Operator> createAdditionalOperators(
      PStrategyManager pStrategyManager, int iteration, int numberOfNewOperators) {
    LinkedList<Operator> emptyOperators = new LinkedList<>();
    for (int i = 0; i < numberOfNewOperators; i++) {
      Operator operator =
          this.operatorFactory.createNewOperator(this.createNewIdForOperator(iteration));
    ...
    for (Operator operator : emptyOperators) {
      boolean initComplete =
          operator.init(
              this.routeProvider, this.initialStrategy, iteration, this.pConfig.getInitialBudget());
      if (initComplete) {
    ...
    if (numberOfOperatorsFailedToBeInitialized > 0) {
      log.warn(
          numberOfOperatorsFailedToBeInitialized
              ...
              + " operators could no be initialized. Proceeding with "
              + initializedOperator.size()
              + " new operators.");
    }
  ...
  private Id<Operator> createNewIdForOperator(int iteration) {
    this.counter++;
    return Id.create(
        this.pConfig.getPIdentifier() + iteration + "_" + this.counter, Operator.class);
  }
}
