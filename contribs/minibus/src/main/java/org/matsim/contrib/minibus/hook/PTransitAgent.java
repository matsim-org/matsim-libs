...
/**
 * Own implementation of boarding behavior. Note, that this behavior may be (more) inconsistent with
 * the transit router as the default implementation.
 *
 * @author aneumann
 */
class PTransitAgent extends PersonDriverAgentImpl implements MobsimDriverPassengerAgent {
  private static final Logger log = LogManager.getLogger(PTransitAgent.class);

  private final TransitSchedule transitSchedule;

  public static PTransitAgent createTransitAgent(
      Person p, Netsim simulation, TimeInterpretation timeInterpretation) {
    return new PTransitAgent(p, simulation, timeInterpretation);
  }

  private PTransitAgent(
  ...
  public boolean getEnterTransitRoute(
    ...
    if (containsId(stopsToCome, route.getEgressStopId())) {
      if (route.getRouteId().toString().equalsIgnoreCase(transitRoute.getId().toString())) {
        ...
        for (TransitRouteStop stop : tempStopsToCome) {
          if (route.getEgressStopId().equals(stop.getStopFacility().getId())) {
            ...
            if (egressStopFound) {
              // egress stop found - so the agent will be able to reach its destination before the
              // vehicle returns to this stop
              // boarding now should be faster
              return true;
            } else {
              // egress stop not found - the vehicle will return before reaching the agent's
              // destination
              // boarding now or the next the vehicle passes by will not change the arrival time
              // although people tend to board the first vehicle arriving, lines looping may impose
              // extra costs, e.g. increased ticket costs due to more kilometer or hours traveled
      ...
      TransitRoute transitRoutePlanned =
      ...
      TransitRoute transitRouteOffered =
          this.transitSchedule
              .getTransitLines()
              .get(line.getId())
              .getRoutes()
              .get(transitRoute.getId());

      double travelTimePlanned =
          getTravelTimeFromAccessStopToEgressStop(
              transitRoutePlanned, route.getAccessStopId(), route.getEgressStopId());
      double travelTimeOffered =
          getTravelTimeFromAccessStopToEgressStop(
  ...
  private double getTravelTimeFromAccessStopToEgressStop(
    ...
    for (TransitRouteStop routeStop : transitRoute.getStops()) {
      if (accessStopId.equals(routeStop.getStopFacility().getId())) {
        // this is overwritten if the vehicle passes multiple times the access stop before stopping
        // at the
        // egress stop. The code in getEnterTransitRoute claims to exclude the case that the agent
        // boards a
        // vehicle if it passes the access stop another time before reaching the next time the
        // egress stop.
      ...
      } else if (lastAccessStopDepartureOffset > -10000.0
          && egressStopId.equals(routeStop.getStopFacility().getId())) {
        return routeStop.getArrivalOffset().seconds() - lastAccessStopDepartureOffset;
      }
    }
    log.error(
        "Sequence access stop "
            + accessStopId.toString()
            + " -> egress stop "
            + egressStopId.toString()
            + " not found in TransitRoute "
