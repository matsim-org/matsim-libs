...
/**
 * This operator has multiple plans. Each is weighted by the number of vehicles associated with. The
 * number of vehicles depends on the score per vehicle and plan. In the end, each plan should have
 * approximately the same score. In contrast to {@link MultiPlanOperator}, this operator accepts all
 * plans as blueprints for replanning, again picking one is a weighted random draw. Vehicle are
 * shifted away from plans instantly, i.e. if a plan scored negative at least one vehicle is
 * removed. Vehicles are distributed to positive plans and new plans.
 *
 * @author aneumann
 */
public final class CarefulMultiPlanOperator extends AbstractOperator {
  ...
  public PPlan getBestPlan() {
    if (this.bestPlan == null) {
      ...
      // will not return the best plan, but one random plan selected from all plans weighted by the
      // number of vehicles of each plan
  ...
  public void replan(PStrategyManager pStrategyManager, int iteration) {
    ...
    // Fifth, add new plans for all vehicles in reserve, but half the number of vehicles already in
    // service as max
    ...
    while (this.numberOfVehiclesInReserve > 0
        && maxVehiclesToDistribute > 0
        && remainingNumberOfTryouts > 0) {
  ...
  private void removeAllPlansWithZeroVehicles() {
    ...
    if (plansToKeep.size() == 0) {
      log.info(
          "All plans have no vehicles. This may happen, if replanning fails to return a valid new plan AND all remaining plans scored negative while having only one vehicle.");
  ...
  private int removeVehiclesFromAllPlansWithNegativeScore(List<PPlan> plansToCheck) {
    ...
    for (PPlan pPlan : plansToCheck) {
      if (pPlan.getScore() < 0.0) {
        ...
        // remove as many vehicles as necessary to compensate the impact on the budget plus one to
        // hopefully get that plan positively scored in the next iteration
  ...
  /**
   * Find plan with the worst score per vehicle. Removes one vehicle. Removes the whole plan, if no
   * vehicle is left.
   *
   * @return
   */
