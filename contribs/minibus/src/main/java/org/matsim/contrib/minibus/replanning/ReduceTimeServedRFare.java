...
/**
 * Restricts the time of operation to temporal relations higher than a certain threshold. Threshold
 * is standard deviation of number of trips or collected fare of all relations twice a scaling
 * factor.
 *
 * @author aneumann
 */
public final class ReduceTimeServedRFare extends AbstractPStrategyModule
    implements StageContainerHandler {

  private static final Logger log = LogManager.getLogger(ReduceTimeServedRFare.class);
  ...
  private LinkedHashMap<Id<TransitRoute>, LinkedHashMap<Integer, LinkedHashMap<Integer, Double>>>
      route2StartTimeSlot2EndTimeSlot2WeightMap = new LinkedHashMap<>();
  ...
  public ReduceTimeServedRFare(ArrayList<String> parameter) {
    ...
    if (parameter.size() != 4) {
      ...
      log.error(
          "Parameter 4: true=allow for picking one of the demand segments, false=enforce coverage of all demand segments");
    }
  ...
  public PPlan run(Operator operator) {
    ...
    Tuple<Double, Double> timeToBeServed =
        getTimeToBeServed(
            this.route2StartTimeSlot2EndTimeSlot2WeightMap.get(routeToOptimize.getId()));
  ...
  private Tuple<Double, Double> getTimeToBeServed(
      LinkedHashMap<Integer, LinkedHashMap<Integer, Double>> startSlot2EndSlot2TripsMap) {
    ...
    for (Entry<Integer, LinkedHashMap<Integer, Double>> endSlot2TripsMapEntry :
        startSlot2EndSlot2TripsMap.entrySet()) {
  ...
  public void handleFareContainer(StageContainer stageContainer) {
    ...
    if (this.route2StartTimeSlot2EndTimeSlot2WeightMap.get(routeId) == null) {
      this.route2StartTimeSlot2EndTimeSlot2WeightMap.put(
          routeId, new LinkedHashMap<Integer, LinkedHashMap<Integer, Double>>());
    }

    if (this.route2StartTimeSlot2EndTimeSlot2WeightMap.get(routeId).get(startTimeSlot) == null) {
      this.route2StartTimeSlot2EndTimeSlot2WeightMap
          .get(routeId)
          .put(startTimeSlot, new LinkedHashMap<Integer, Double>());
    }

    if (this.route2StartTimeSlot2EndTimeSlot2WeightMap
            .get(routeId)
            .get(startTimeSlot)
            .get(endTimeSlot)
        == null) {
    ...
    this.route2StartTimeSlot2EndTimeSlot2WeightMap
        .get(routeId)
        .get(startTimeSlot)
        .put(endTimeSlot, oldWeight + additionalWeight);
  }
