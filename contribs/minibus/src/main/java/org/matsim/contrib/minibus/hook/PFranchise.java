...
/**
 * Simple Franchise system rejecting all routes already operated with respect to stops served and
 * time of operation
 *
 * @author aneumann
 */
public final class PFranchise {
  ...
  private static final Logger log = LogManager.getLogger(PFranchise.class);
          ...
          if (this.routeHashes.contains(routeHash)) {
            if (routesHashesOfOperator.contains(routeHash)) {
              ...
              log.warn(
                  "Operator "
                      + operator.getId()
                      + " offers the same plan twice. Plan: "
                      + plan.getId()
                      + " managed to circumvent the franchise system with route "
                      + routeHash);
            } else {
  ...
  private String generateRouteHash(PPlan plan) {
    ...
    return generateRouteHashWithGridNodes(
        plan.getStartTime(), plan.getEndTime(), plan.getStopsToBeServed());
  }
  ...
  private String generateRouteHash(
      double startTime, double endTime, ArrayList<TransitStopFacility> stopsToBeServed) {
  ...
  private String generateRouteHashWithGridNodes(
      double startTime, double endTime, ArrayList<TransitStopFacility> stopsToBeServed) {
    StringBuffer sB = new StringBuffer();
    sB.append(startTime);
    sB.append("-" + endTime);
    ...
    for (TransitStopFacility transitStopFacility : stopsToBeServed) {
      String gridNodeId =
          GridNode.getGridNodeIdForCoord(transitStopFacility.getCoord(), this.gridSize);
