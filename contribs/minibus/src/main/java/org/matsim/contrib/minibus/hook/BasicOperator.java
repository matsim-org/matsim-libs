  ...
  public void replan(PStrategyManager pStrategyManager, int iteration) {
    ...
    if (this.testPlan != null) {
      ...
      if (this.getScore() > this.getScoreLastIteration()) {
        // testPlan improves the plan, apply its modification to bestPlan, transfer the vehicle from
        // the testPlan to the bestPlan
    ...
    if (this.budget < 0) {
      ...
      //			log.info("Sold " + numberOfVehiclesToSell + " vehicle from line " + this.id + " - new
      // budget is " + this.budget);
    }

    // THIS IS A COPY FROM
    // org.matsim.contrib.minibus.replanning.modules.deprecated.AggressiveIncreaseNumberOfVehicles;
    // First buy vehicles
    int vehicleBought = 0;
    ...
    PPlan plan =
        new PPlan(
            this.getNewPlanId(),
            "Copied code from AggressiveIncreaseNumberOfVehicles",
            oldPlan.getId());
    ...
    this.bestPlan.setLine(
        this.routeProvider.createTransitLineFromOperatorPlan(this.id, this.bestPlan));
