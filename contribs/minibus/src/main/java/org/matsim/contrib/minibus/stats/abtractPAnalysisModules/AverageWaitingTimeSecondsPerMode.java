...
/**
 * Calculates the average waiting time per trip and per ptModes specified. This is from departure
 * event to PersonEntersVehicleEvent.
 *
 * @author aneumann
 */
final class AverageWaitingTimeSecondsPerMode extends AbstractPAnalyisModule
    implements PersonDepartureEventHandler,
        TransitDriverStartsEventHandler,
        PersonEntersVehicleEventHandler {

  private static final Logger log = LogManager.getLogger(AverageWaitingTimeSecondsPerMode.class);
  ...
  public String getResult() {
    ...
    for (String ptMode : this.ptModes) {
      strB.append(
          ", "
              + (this.ptMode2SecondsTravelledMap.get(ptMode)
                  / this.ptMode2TripCountMap.get(ptMode)));
    }
  ...
  public void handleEvent(PersonEntersVehicleEvent event) {
    if (!super.ptDriverIds.contains(event.getPersonId())) {
      ...
      this.ptMode2SecondsTravelledMap.put(
          ptMode,
          this.ptMode2SecondsTravelledMap.get(ptMode)
              + (event.getTime() - this.agentId2AgentDepartureEventTime.get(event.getPersonId())));
