...
/**
 * Holds all information needed to calculate the cost for this vehicle. Note, this container does
 * not know, whether there are multiple instances of the container for a single vehicle or note.
 * Consider this, when adding up all containers, especially the fixed costs per day.
 *
 * <p>There is one container for each stage of a vehicle indicated by a {@link
...
final class OperatorCostContainer {
  ...
  private static final Logger log = LogManager.getLogger(OperatorCostContainer.class);
  ...
  OperatorCostContainer(
      double costPerVehicleAndDay, double expensesPerMeter, double expensesPerSecond) {
  ...
  /**
   * Adds any additional monetary transaction that the vehicle may incur, for example toll or fines.
   * This is specifically not called 'expense' since it may be an income, like a driver tip. A
   * positive value indicates an income, and a negative value indicates an expense.
   */
  ...
  /**
   * This terminates the stage if the driver is stuck. If the driver stucks there should not be any
   * PersonLeavesVehicleEvent since the last TransitDriverStartsEvent, so there is either a
   * PersonLeavesVehicleEvent or a PersonStuckEvent.
   */
  ...
  public String toString() {
    return "costPerVehicleAndDay "
        + costPerVehicleAndDay
        + "; "
        + "expensesPerMeter "
        + expensesPerMeter
        + "; "
        ...
        + expensesPerSecond
        + "; "
        + "transitDriverStartsE "
        + transitDriverStartsE
        + "; "
        + "transitDriverAlightsE "
        + transitDriverAlightsE
        + "; "
        + "vehicleAbortE "
        + vehicleAbortE
        ...
        + "; "
        + "additionalMoney "
        + otherMonetaryTransactionsIncurred;
  }
}
