...
public final class PConfigGroup extends ConfigGroup {
  ...
  public static enum StopLocationSelector {
    allCarLinks("allCarLinks"),
    junctionApproachesAndBetweenJunctions("junctionApproachesAndBetweenJunctions");
    public final String name;

    StopLocationSelector(String name) {
      this.name = name;
    }
  }

  public static enum LogRouteDesignVsTotalScore {
    no,
    onlyNonZeroRouteDesignScore
  }
  ...
  private static final String NUMBER_OF_ITERATIONS_FOR_PROSPECTING =
      "numberOfIterationsForProspecting";
  ...
  private static final String EARNINGS_PER_KILOMETER_AND_PASSENGER =
      "earningsPerKilometerAndPassenger";
  ...
  private static final String DISABLE_CREATION_OF_NEW_OPERATORS_IN_ITERATION =
      "disableCreationOfNewOperatorsInIteration";
  ...
  // Strategies
  private final LinkedHashMap<Id<PStrategySettings>, PStrategySettings> strategies =
      new LinkedHashMap<>();
  ...
  public void addParam(final String key, final String value) {
    ...
    if (P_IDENTIFIER.equals(key)) {
      ...
      if (value.equals(StopLocationSelector.allCarLinks.name)) {
        this.stopLocationSelector = StopLocationSelector.allCarLinks;
      } else if (value.equals(StopLocationSelector.junctionApproachesAndBetweenJunctions.name)) {
        this.stopLocationSelector = StopLocationSelector.junctionApproachesAndBetweenJunctions;
      } else {
        log.error("unknown parameter value: " + key + ": " + value);
      }
      ...
      PStrategySettings settings =
          getStrategySettings(
              Id.create(key.substring(PMODULE.length()), PStrategySettings.class), true);
      settings.setModuleName(value);
    } else if (key != null && key.startsWith(PMODULE_PROBABILITY)) {
      PStrategySettings settings =
          getStrategySettings(
              Id.create(key.substring(PMODULE_PROBABILITY.length()), PStrategySettings.class),
              true);
      ...
      PStrategySettings settings =
          getStrategySettings(
              Id.create(
                  key.substring(PMODULE_DISABLEINITERATION.length()), PStrategySettings.class),
              true);
      settings.setDisableInIteration(Integer.parseInt(value));
    } else if (key != null && key.startsWith(PMODULE_PARAMETER)) {
      PStrategySettings settings =
          getStrategySettings(
              Id.create(key.substring(PMODULE_PARAMETER.length()), PStrategySettings.class), true);
  ...
  public TreeMap<String, String> getParams() {
    ...
    map.put(
        NUMBER_OF_ITERATIONS_FOR_PROSPECTING,
        Integer.toString(this.numberOfIterationsForProspecting));
    ...
    map.put(
        EARNINGS_PER_KILOMETER_AND_PASSENGER,
        Double.toString(this.earningsPerKilometerAndPassenger));
    ...
    map.put(
        DISABLE_CREATION_OF_NEW_OPERATORS_IN_ITERATION,
        Integer.toString(this.disableCreationOfNewOperatorsInIteration));
    ...
    for (Entry<Id<PStrategySettings>, PStrategySettings> entry : this.strategies.entrySet()) {
      ...
      map.put(
          PMODULE_PROBABILITY + entry.getKey().toString(),
          Double.toString(entry.getValue().getProbability()));
      map.put(
          PMODULE_DISABLEINITERATION + entry.getKey().toString(),
          Integer.toString(entry.getValue().getDisableInIteration()));
      map.put(
          PMODULE_PARAMETER + entry.getKey().toString(), entry.getValue().getParametersAsString());
    }
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        EARNINGS_PER_BOARDING_PASSENGER,
    ...
    map.put(
        LOG_ROUTE_DESIGN_VS_TOTAL_SCORE,
        "will log total score before and after adding route design score. Values: "
            + LogRouteDesignVsTotalScore.no
            + " and "
    ...
    map.put(
        TRANSIT_SCHEDULE_TO_START_WITH,
    ...
    map.put(
        STOP_LOCATION_SELECTOR_PARAMETER,
  ...
  private PStrategySettings getStrategySettings(
      final Id<PStrategySettings> strategyId, final boolean createIfMissing) {
  ...
  public static class RouteDesignScoreParams extends ReflectiveConfigGroup
    ...
    public enum StopListToEvaluate {
      transitRouteAllStops,
      pPlanStopsToBeServed
    }
    ...
    public final Map<String, String> getComments() {
      ...
      map.put(
          ROUTE_DESIGN_SCORE_FUNCTION,
          "name of route design score function to be applied. Possible default names: "
              + defaultRouteDesignScoreFunctions
              + ".");
      map.put(
          COST_FACTOR,
      ...
      map.put(
          STOP_LIST_TO_EVALUATE,
          "which stops shall be evaluated. Possible values "
              + StopListToEvaluate.pPlanStopsToBeServed
              + " and "
              + StopListToEvaluate.transitRouteAllStops);
      map.put(
          VALUE_T0_START_SCORING,
    ...
    public void setRouteDesignScoreFunction(final String routeDesignScoreFunction) {
      setRouteDesignScoreFunction(
          RouteDesignScoringManager.RouteDesignScoreFunctionName.valueOf(routeDesignScoreFunction));
    }

    public void setRouteDesignScoreFunction(
        final RouteDesignScoringManager.RouteDesignScoreFunctionName routeDesignScoreFunction) {
  ...
  public void removeRouteDesignScoreParams(
      RouteDesignScoringManager.RouteDesignScoreFunctionName key) {
    testForLocked();
    for (ConfigGroup pars : getParameterSets(RouteDesignScoreParams.SET_TYPE)) {
      final RouteDesignScoringManager.RouteDesignScoreFunctionName routeDesignScoreFunction =
          ((RouteDesignScoreParams) pars).getRouteDesignScoreFunction();
  ...
  public Map<RouteDesignScoringManager.RouteDesignScoreFunctionName, RouteDesignScoreParams>
    ...
    final Map<RouteDesignScoringManager.RouteDesignScoreFunctionName, RouteDesignScoreParams> map =
        new LinkedHashMap<>();
    ...
    for (ConfigGroup pars : getParameterSets(RouteDesignScoreParams.SET_TYPE)) {
      ...
      final RouteDesignScoringManager.RouteDesignScoreFunctionName routeDesignScoreFunction =
          scoreParams.getRouteDesignScoreFunction();
      final RouteDesignScoreParams old = map.put(routeDesignScoreFunction, scoreParams);
      if (old != null)
        throw new IllegalStateException(
            "several parameter sets for routeDesignScoreFunction " + routeDesignScoreFunction);
    }

    return map;
  }

  public RouteDesignScoreParams getOrCreateRouteDesignScoreParams(
      final RouteDesignScoringManager.RouteDesignScoreFunctionName routeDesignScoreFunction) {
  ...
  protected void checkConsistency(Config config) {
    ...
    if (this.mergeTransitLine) {
      log.info(
          "All routes of a minibus transit line with the same stop sequence will be merged into one single transit route. Note that the transit schedules written to the output directory do not contain all minibus routes anymore.");

      if (!logOperators) {
        log.warn(
            "Transit lines will be merged. Activate the operator logger to retrieve more detailed information on particular routes.");
      }
    }
  }
}
