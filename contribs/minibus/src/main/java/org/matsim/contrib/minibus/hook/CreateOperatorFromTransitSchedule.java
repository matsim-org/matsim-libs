...
public final class CreateOperatorFromTransitSchedule implements PStrategy {
  ...
  private static final Logger log = LogManager.getLogger(CreateOperatorFromTransitSchedule.class);
  ...
  public CreateOperatorFromTransitSchedule(
      OperatorFactory operatorFactory,
      PRouteProvider routeProvider,
      PConfigGroup pConfig,
      TransitSchedule originalSchedule) {
  ...
  public LinkedList<Operator> run() {
    ...
    if (pConfig.getTransitScheduleToStartWith() != null) {
      ...
      for (Entry<Id<TransitLine>, TransitLine> lineEntry : tS.getTransitLines().entrySet()) {
        Operator operator =
            this.operatorFactory.createNewOperator(
                Id.create(this.pConfig.getPIdentifier() + lineEntry.getKey(), Operator.class));
    ...
    log.info(
        "Returning "
            + operatorsToReturn.size()
            + " operators created from transit schedule file "
            + pConfig.getTransitScheduleToStartWith());
  ...
  private PPlan createPlan(TransitLine line) {
    ...
    if (longestRouteH != null) {
      ...
      double lastStopOffset =
          lastStop.getDepartureOffset().isDefined()
              ? lastStop.getDepartureOffset().seconds()
              : lastStop.getArrivalOffset().seconds();
    ...
    if (longestRouteH != null) {
      for (TransitRouteStop routeStop : longestRouteH.getStops()) {
        if (this.checkStopInServiceArea(routeStop.getStopFacility(), this.pConfig)) {
          TransitStopFacility originalStopFacility =
              this.originalStops.get(routeStop.getStopFacility().getId().toString());
  ...
  private boolean checkStopInServiceArea(TransitStopFacility stop, PConfigGroup pC) {
    ...
    if (stop.getCoord().getX() < pC.getMinX()) {
      isInServiceArea = false;
    }
    if (stop.getCoord().getX() > pC.getMaxX()) {
      isInServiceArea = false;
    }

    if (stop.getCoord().getY() < pC.getMinY()) {
      isInServiceArea = false;
    }
    if (stop.getCoord().getY() > pC.getMaxY()) {
      isInServiceArea = false;
    }
