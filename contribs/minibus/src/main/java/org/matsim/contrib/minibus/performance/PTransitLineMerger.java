...
/**
 * Merges all routes of a transit line that have the same sequence of stops. Does not respect the
 * time profile of the routes.
 *
 * @author aneumann
 */
public class PTransitLineMerger {
  ...
  private static TransitLine mergeTransitLine(
      TransitScheduleFactory transitScheduleFactory, TransitLine oldLine) {
    ...
    for (TransitRoute transitRoute : oldLine.getRoutes().values()) {
      ...
      if (routeHash2TransitRoute.keySet().contains(routeHash)) {
        ...
        for (Departure oldDeparture : transitRoute.getDepartures().values()) {
          ...
          Id<Departure> newDepartureId =
              Id.create("new_" + mergeDestination.getDepartures().values().size(), Departure.class);
          Departure newDeparture =
              transitScheduleFactory.createDeparture(
                  newDepartureId, oldDeparture.getDepartureTime());
