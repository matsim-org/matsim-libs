...
/**
 * Uses a {@link CountPPaxHandler} to count passengers per paratransit vehicle and link, {@link
 * CountPOperatorHandler} to count operators and their ids and writes them to a gexf network as
 * dynamic link attributes. In addition, writes one column per link and operator with its number of
 * passengers served.
 *
 * @author aneumann
 */
final class GexfPStat extends MatsimJaxbXmlWriter
  ...
  private static final String FILENAME = "pStat.gexf.gz";
  ...
  public GexfPStat(boolean writeOperatorInDetail, PConfigGroup pConfig) {
    ...
    if (this.getWriteGexfStatsInterval > 0) {
      ...
      this.gexfContainer
          .getGraph()
          .getAttributesOrNodesOrEdges()
          .add(edgeAttributeContentsContainer);
    }
  }
        ...
        if (writeOperatorInDetail) {
          this.write(
              event
                  .getServices()
                  .getControlerIO()
                  .getIterationFilename(event.getIteration(), "pStat_detail.gexf.gz"));
        } else {
          this.write(
              event
                  .getServices()
                  .getControlerIO()
                  .getIterationFilename(event.getIteration(), GexfPStat.FILENAME));
        }
      }
    }
  }
  ...
  private void addValuesToGexf(
      int iteration, CountPPaxHandler paxHandler, CountPOperatorHandler operatorHandler) {
    for (Entry<Id<Link>, XMLAttvaluesContent> linkEntry :
        this.linkAttributeValueContentMap.entrySet()) {
    ...
    for (Entry<Id<Link>, XMLAttvaluesContent> linkEntry :
      ...
      Set<Id<Operator>> operatorsForLink =
          operatorHandler.getOperatorIdsForLinkId(linkEntry.getKey());

      // Test, if something changed
      if (this.linkId2OperatorIdsFromLastIteration.get(linkEntry.getKey()) != null) {
        // There is already an entry
        if (this.linkId2OperatorIdsFromLastIteration
            .get(linkEntry.getKey())
            .equals(operatorsForLink)) {
          // same as last iteration - ignore
          continue;
        }
      }
    ...
    if (writeOperatorInDetail) {
        ...
        if (!currentLineIds.contains(lineId)) {
          ...
          for (Entry<Id<Link>, XMLAttvaluesContent> linkEntry :
            ...
            if (this.linkId2LineId2CountsFromLastIteration.get(linkEntry.getKey()) != null) {
              if (this.linkId2LineId2CountsFromLastIteration.get(linkEntry.getKey()).get(lineId)
                  != null) {
                // There is already an entry
                if (this.linkId2LineId2CountsFromLastIteration.get(linkEntry.getKey()).get(lineId)
                    == 0) {
      ...
      for (Entry<Id<Link>, XMLAttvaluesContent> linkEntry :
        ...
        for (String lineId : currentLineIds) {
          ...
          if (this.linkId2LineId2CountsFromLastIteration.get(linkEntry.getKey()) == null) {
            this.linkId2LineId2CountsFromLastIteration.put(
                linkEntry.getKey(), new HashMap<String, Integer>());
          }
          this.linkId2LineId2CountsFromLastIteration
              .get(linkEntry.getKey())
              .put(lineId, countForLinkAndLineId);
        }
      }
