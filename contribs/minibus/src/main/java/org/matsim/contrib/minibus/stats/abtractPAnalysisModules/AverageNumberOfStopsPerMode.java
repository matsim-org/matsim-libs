 ...
 * @author aneumann
 */
final class AverageNumberOfStopsPerMode extends AbstractPAnalyisModule
    implements TransitDriverStartsEventHandler,
        PersonEntersVehicleEventHandler,
        PersonLeavesVehicleEventHandler,
        VehicleArrivesAtFacilityEventHandler {

  private static final Logger log = LogManager.getLogger(AverageNumberOfStopsPerMode.class);
  ...
  private HashMap<Id<Vehicle>, HashMap<Id<Person>, Integer>> vehId2AgentId2StopCountMap =
      new HashMap<>();
  ...
  public String getResult() {
    ...
    for (String ptMode : this.ptModes) {
      strB.append(
          ", "
              + (this.ptMode2NumberOfStopsTravelledMap.get(ptMode).doubleValue()
                  / this.ptMode2TripCountMap.get(ptMode).doubleValue()));
    }
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    if (!super.ptDriverIds.contains(event.getPersonId())) {
      ...
      this.ptMode2NumberOfStopsTravelledMap.put(
          ptMode,
          this.ptMode2NumberOfStopsTravelledMap.get(ptMode)
              + this.vehId2AgentId2StopCountMap.get(event.getVehicleId()).get(event.getPersonId()));
