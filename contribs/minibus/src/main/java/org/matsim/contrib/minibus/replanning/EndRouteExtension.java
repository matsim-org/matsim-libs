...
/**
 * Takes the route transformed into a lineString to calculate a buffer around it. Excludes the part
 * of the buffer going parallel/along the actual route, so only the end caps remain. Chooses then
 * randomly a new stop within the buffer and inserts it after the nearest existing stop.
 *
 * @author aneumann
 */
public final class EndRouteExtension extends AbstractPStrategyModule {

  private static final Logger log = LogManager.getLogger(EndRouteExtension.class);
  ...
  public EndRouteExtension(ArrayList<String> parameter) {
    ...
    if (parameter.size() != 2) {
      ...
      log.error(
          "Parameter 2: Ratio bufferSize to route's beeline length. If set to something very small, e.g. 0.01, the calculated buffer size may be smaller than the one specified in parameter 1. Parameter 1 will then be taken as minimal buffer size.");
    }
  ...
  public PPlan run(Operator operator) {
    ...
    double bufferSizeBasedOnRatio =
        CoordUtils.calcEuclideanDistance(
                baseStop.getCoord(), currentStopsToBeServed.get(remoteStopIndex).getCoord())
            * this.ratio;

    List<Geometry> lineStrings =
        this.createGeometryFromStops(currentStopsToBeServed, remoteStopIndex);
    Geometry bufferWithoutEndCaps =
        this.createBuffer(lineStrings, Math.max(this.bufferSize, bufferSizeBasedOnRatio), true);
    Geometry bufferWithEndCaps =
        this.createBuffer(lineStrings, Math.max(this.bufferSize, bufferSizeBasedOnRatio), false);
    Geometry buffer = bufferWithEndCaps.difference(bufferWithoutEndCaps);

    Set<Id<TransitStopFacility>> stopsUsed =
        this.getStopsUsed(oldPlan.getLine().getRoutes().values());
    TransitStopFacility newStop =
        this.drawRandomStop(buffer, operator.getRouteProvider(), stopsUsed);
    ...
    ArrayList<TransitStopFacility> newStopsToBeServed =
        this.addStopToExistingStops(
            baseStop, currentStopsToBeServed.get(remoteStopIndex), currentStopsToBeServed, newStop);
  ...
  private ArrayList<TransitStopFacility> addStopToExistingStops(
      TransitStopFacility baseStop,
      TransitStopFacility remoteStop,
      ArrayList<TransitStopFacility> currentStopsToBeServed,
      TransitStopFacility newStop) {
    ArrayList<TransitStopFacility> newStopsToBeServed = new ArrayList<>(currentStopsToBeServed);

    // decide which stop is closer
    if (CoordUtils.calcEuclideanDistance(baseStop.getCoord(), newStop.getCoord())
        < CoordUtils.calcEuclideanDistance(remoteStop.getCoord(), newStop.getCoord())) {
  ...
  private TransitStopFacility drawRandomStop(
      Geometry buffer, PRouteProvider pRouteProvider, Set<Id<TransitStopFacility>> stopsUsed) {
