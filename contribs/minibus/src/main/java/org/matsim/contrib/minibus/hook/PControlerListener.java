 ...
 * @author aneumann
 */
final class PControlerListener
    implements IterationStartsListener, StartupListener, ScoringListener {

  private static final Logger log = LogManager.getLogger(PControlerListener.class);
  ...
  @Inject
  PControlerListener(Config config, POperators operators) {
  ...
  public void notifyStartup(StartupEvent event) {
    ...
    addPTransitScheduleToOriginalOne(
        event.getServices().getScenario().getTransitSchedule(), pBox.getpTransitSchedule());
    addPVehiclesToOriginalOnes(
        event.getServices().getScenario().getTransitVehicles(),
        this.pVehiclesFactory.createVehicles(pBox.getpTransitSchedule()));
  ...
  public void notifyIterationStarts(IterationStartsEvent event) {
    ...
    if (event.getIteration() == controler.getConfig().controller().getFirstIteration()) {
      ...
      removePreviousPTransitScheduleFromOriginalOne(
        ...
        ParallelPersonAlgorithmUtils.run(
            ...
            new ParallelPersonAlgorithmUtils.PersonAlgorithmProvider() {
              ...
              public AbstractPersonAlgorithm getPersonAlgorithm() {
                return stuckFactory.getReRouteStuck(
                    new PlanRouter(
                        controler.getTripRouterProvider().get(),
                        controler.getScenario().getActivityFacilities(),
                        timeInterpretation),
                    ((MutableScenario) controler.getScenario()),
                    agentsStuckHandler.getAgentsStuck());
              }
            });
      }
    }
  ...
  private void addPTransitScheduleToOriginalOne(
      TransitSchedule baseSchedule, TransitSchedule pSchedule) {
    if (pSchedule == null) {
  ...
  private void dumpTransitScheduleAndVehicles(MatsimServices controler, int iteration) {
    TransitScheduleWriter writer =
        new TransitScheduleWriter(controler.getScenario().getTransitSchedule());
    MatsimVehicleWriter writer2 =
        new MatsimVehicleWriter(controler.getScenario().getTransitVehicles());
    writer.writeFile(
        controler.getControlerIO().getIterationFilename(iteration, "transitSchedule.xml.gz"));
    writer2.writeFile(
        controler.getControlerIO().getIterationFilename(iteration, "transitVehicles.xml.gz"));
  }
}
