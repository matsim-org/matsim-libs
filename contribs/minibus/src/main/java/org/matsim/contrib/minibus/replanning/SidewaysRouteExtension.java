...
/**
 * Takes the route transformed into a lineString to calculate a buffer around it. Chooses then
 * randomly a new stop within the buffer and inserts it in both directions of the line. This is done
 * assuming the most distant stops to be the termini.
 *
 * @author aneumann
 */
public final class SidewaysRouteExtension extends AbstractPStrategyModule {

  private static final Logger log = LogManager.getLogger(SidewaysRouteExtension.class);
  ...
  public PPlan run(Operator operator) {
    ...
    ArrayList<TransitStopFacility> newStopsToBeServed =
        this.addStopToExistingStops(0, remoteStopIndex, currentStopsToBeServed, newStop);
  ...
  private ArrayList<TransitStopFacility> addStopToExistingStops(
    ...
    List<ArrayList<TransitStopFacility>> subRoutes =
        this.cutRouteIntoSubRoutes(markerStopIndexes, currentStopsToBeServed);
  ...
  private void addNewStopBetweenTwoNearestExistingStops(
    ...
    for (TransitStopFacility transitStopFacility : stops) {
      double currentDistance =
          CoordUtils.calcEuclideanDistance(stop.getCoord(), transitStopFacility.getCoord());
  ...
  private List<ArrayList<TransitStopFacility>> cutRouteIntoSubRoutes(
      List<Integer> markerStopIndexes, ArrayList<TransitStopFacility> stops) {
