 ...
 * @author aneumann
 */
final class CountTripsPerPtModeCombination extends AbstractPAnalyisModule
    implements TransitDriverStartsEventHandler,
        PersonEntersVehicleEventHandler,
        ActivityStartEventHandler {

  private static final Logger log = LogManager.getLogger(CountTripsPerPtModeCombination.class);
  ...
  public void handleEvent(PersonEntersVehicleEvent event) {
    if (!super.ptDriverIds.contains(event.getPersonId())) {
      ...
      if (this.agentId2TripCombination.get(event.getPersonId()) == null) {
        ...
        String tripCombination =
            this.agentId2TripCombination.get(event.getPersonId()) + "-" + ptMode;
    ...
    if (!super.ptDriverIds.contains(event.getPersonId())) {
      if (!event.getActType().equalsIgnoreCase(PtConstants.TRANSIT_ACTIVITY_TYPE)) {
        ...
        if (tripCombination != null) {
          ...
          this.ptModeCombination2TripCountMap.put(
              tripCombination, this.ptModeCombination2TripCountMap.get(tripCombination) + 1);
        }
