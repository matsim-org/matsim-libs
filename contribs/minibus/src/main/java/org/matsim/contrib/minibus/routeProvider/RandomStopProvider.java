...
/**
 * Provides random draws of transit stops facilities. The draw is weighted by the number of
 * activities within the stops proximity.
 *
 * @author aneumann
 */
final class RandomStopProvider {

  private static final Logger log = LogManager.getLogger(RandomStopProvider.class);
  ...
  public RandomStopProvider(
      PConfigGroup pConfig, Population population, TransitSchedule pStopsOnly, String outputDir) {
  ...
  private void updateWeights() {
    ...
    for (Entry<String, List<TransitStopFacility>> stopsEntry : gridNodeId2StopsMap.entrySet()) {
      ...
      // divide count by the number of associated stops, thus neglecting a higher probability for
      // stops located in a dense network area
    ...
    log.info(
        "Initialized with "
            + this.stops2Weight.size()
            + " of "
            + this.pStopsOnly.getFacilities().values().size()
            + " stops covering "
            + (this.totalWeight / numberOfActsInPlans)
      ...
      if (this.lastIteration == -1) {
        ...
        this.outputDir =
            this.outputDir
                ...
                + RandomStopProvider.class.getSimpleName()
                + "/";
        new File(this.outputDir).mkdir();
      }

      BufferedWriter writer =
          IOUtils.getBufferedWriter(outputDir + currentIteration + ".stopId2stopWeight.txt.gz");
      writer.write("# stop id; x; y; weight");
      writer.newLine();
      for (Entry<TransitStopFacility, Double> stopEntry : this.stops2Weight.entrySet()) {
        writer.write(
            stopEntry.getKey().getId().toString()
                + "; "
                + stopEntry.getKey().getCoord().getX()
                + "; "
                + stopEntry.getKey().getCoord().getY()
                + "; "
                + stopEntry.getValue().toString());
        writer.newLine();
      }
      writer.flush();
      writer.close();
