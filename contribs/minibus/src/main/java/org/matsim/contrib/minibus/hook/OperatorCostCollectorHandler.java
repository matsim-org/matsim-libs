 ...
 * @author aneumann
 */
final class OperatorCostCollectorHandler
  ...
  private static final Logger log = LogManager.getLogger(OperatorCostCollectorHandler.class);
  ...
  private final List<OperatorCostContainerHandler> operatorCostContainerHandlerList =
      new LinkedList<>();
  HashMap<Id<Vehicle>, OperatorCostContainer> vehId2OperatorCostContainer = new HashMap<>();

  OperatorCostCollectorHandler(
      String pIdentifier,
      double costPerVehicleAndDay,
      double expensesPerMeter,
      double expensesPerSecond) {
  ...
  public void reset(int iteration) {
    ...
    for (OperatorCostContainerHandler operatorCostContainerHandler :
        this.operatorCostContainerHandlerList) {
  ...
  public void handleEvent(TransitDriverStartsEvent event) {
    this.vehId2OperatorCostContainer.put(
        event.getVehicleId(),
        new OperatorCostContainer(
            this.costPerVehicleAndDay, this.expensesPerMeter, this.expensesPerSecond));
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    if (event.getVehicleId().toString().startsWith(this.pIdentifier)) {
      ...
      if (event.getPersonId().toString().contains(this.pIdentifier)) {
        // it's the driver
        OperatorCostContainer operatorCostContainer =
            this.vehId2OperatorCostContainer.remove(event.getVehicleId());
   ...
   * PersonStuckEvent instead as a replacement to avoid NullPointerExceptions and calculate an
   * approximate running time cost.
   */
  @Override
    ...
    if (entriesToProcess.size() > 0) {
      log.warn(
          "There are "
              ...
              + " incomplete entries. Will forward them anyway...");
  ...
  public void handleEvent(PersonMoneyEvent event) {
    // yyyy what comes below is not so beautiful since it depends on conventions: If some string
    // contains something of some other string,
    // then ... It looks like this was the original design of the minibus contrib, so in principle
    // it should be operated out at some
