...
/**
 * Calculates the average in vehicle trip travel time per ptModes specified. A trip starts by
 * entering a vehicle and end by leaving one.
 *
 * @author aneumann
 */
final class AverageInVehicleTripTravelTimeSecondsPerMode extends AbstractPAnalyisModule
    implements TransitDriverStartsEventHandler,
        PersonEntersVehicleEventHandler,
        PersonLeavesVehicleEventHandler {

  private static final Logger log =
      LogManager.getLogger(AverageInVehicleTripTravelTimeSecondsPerMode.class);
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    if (!super.ptDriverIds.contains(event.getPersonId())) {
      ...
      this.ptMode2SecondsTravelledMap.put(
          ...
          this.ptMode2SecondsTravelledMap.get(ptMode)
              + (event.getTime()
                  - this.agentId2PersonEntersVehicleEventTime.get(event.getPersonId())));
