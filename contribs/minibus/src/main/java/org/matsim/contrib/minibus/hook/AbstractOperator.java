...
abstract class AbstractOperator implements Operator {
  ...
  static final Logger log = LogManager.getLogger(AbstractOperator.class);
  ...
  @Override
  public void score(
      Map<Id<Vehicle>, PScoreContainer> pScores,
      SubsidyI subsidy,
    ...
    for (PPlan plan : this.getAllPlans()) {
      ...
      if (subsidy != null) {
        Id<PPlan> pplanId =
            Id.create(
                plan.getLine().getId().toString() + "-" + plan.getId().toString(), PPlan.class);
  ...
  void processScore() {
    ...
    if (this.budget < 0) {
      ...
      int numberOfVehiclesToSell =
          -1 * Math.min(-1, (int) Math.floor(this.budget / this.costPerVehicleSell));
  ...
  @Override
  public abstract void replan(PStrategyManager pStrategyManager, int iteration);
  ...
  private double capAndAddRouteDesignScore(
      PPlan plan,
      double originaltotalLineScore,
      RouteDesignScoringManager routeDesignScoringManager) {
    ...
    if (routeDesignScoringManager.isActive()) {
      ...
      if (routeDesignScore != 0) {
        ...
        if (originaltotalLineScore >= minScoreToSurvive) {
          ...
          totalLineScore =
              Math.max(
                  originaltotalLineScore + routeDesignScore * plan.getNVehicles(),
        ...
        if (logRouteDesignVsTotalScore.equals(
            LogRouteDesignVsTotalScore.onlyNonZeroRouteDesignScore)) {
          log.info(
              "operator "
                  + id
                  + ". TransitRoute(s) "
                  + plan.getLine().getRoutes().values().iterator().next().getId()
                  + ". original totalLineScore: "
                  + originaltotalLineScore
                  + ". totalLineScore after: "
                  + totalLineScore
                  + ". routeDesignScore: "
                  + routeDesignScore
                  + ". vehicles "
