...
/**
 * Calculate penalty score for circuitous TransitRoutes (TransitRoute of the PPlan) which overlap
 * themselves, i.e. the same stop is served several times by the same TransitRoute. Score is
 * calculated as number of stops on the route divided by the number of unique stop facilities
 * served.
 *
 * @author gleich
 */
class StopServedMultipleTimesPenalty implements RouteDesignScoringFunction {
  ...
  public double getScore(PPlan pPlan, TransitRoute route) {
    ...
    double score =
        ((double) stopListToEvaluate.size()) / stopIdServed.size()
            - params.getValueToStartScoring();
    if (score > 0) {
    ...
    if (params.getLogScore().equals(LogRouteDesignScore.onlyNonZeroScore) && score != 0) {
      log.info(
          "Transit Route "
              ...
              + score
              + " (total "
              + stopListToEvaluate.size()
              + " stops served; unique TransitStopFacilities "
              + stopIdServed.size());
    }
    return score;
  }
