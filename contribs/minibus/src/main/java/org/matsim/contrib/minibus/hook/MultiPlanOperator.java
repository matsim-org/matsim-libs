...
public final class MultiPlanOperator extends AbstractOperator {
  ...
  public PPlan getBestPlan() {
    if (this.bestPlan == null) {
      ...
      // will not return the best plan, but one random plan selected from all plans with at least
      // two vehicles
  ...
  public void replan(PStrategyManager pStrategyManager, int iteration) {
    ...
    if (plansToKeep.size() == 0) {
      log.error(
          "Removed too many plans. There was at least one plan with no vehicles associated with.");
  ...
  /**
   * Find worst negative plan (worst score per vehicle). Removes one vehicle. Removes the whole
   * plan, if no vehicle is left.
   *
