...
public class TerminusStopFinder {
   ...
   *
   * <p>Returns stop index instead of the stop, in order to cater for stops which are served
   * multiple times
   *
   * @param stops
   * @return index of the stop which is half way on the route from start stop over all stops back to
   *     the start stop
   * @author gleich
   */
  public static final int findSecondTerminusStop(ArrayList<TransitStopFacility> stops) {
    ...
    for (int i = 1; i < stops.size(); i++) {
      if (distFromStart2StopIndex.get(i) >= totalDistance / 2) {
        if (Math.abs(totalDistance / 2 - distFromStart2StopIndex.get(i - 1))
            > Math.abs(totalDistance / 2 - distFromStart2StopIndex.get(i))) {
