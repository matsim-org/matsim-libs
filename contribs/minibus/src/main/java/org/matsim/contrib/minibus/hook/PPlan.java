...
public final class PPlan implements Comparable<PPlan> {
  ...
  private static final Logger log = LogManager.getLogger(PPlan.class);
  ...
  public String toString() {
    ...
    sB.append(
        "Plan "
            ...
            + this.score
            + ", score/veh: "
            + this.getScorePerVehicle()
            + ", trips: "
            + this.tripsServed
            + ", vehicles: "
            + this.vehicleIds.size()
            + ", Operation time: "
  ...
  public boolean isSameButVehSize(PPlan testPlan) {
    ...
    for (int i = 0; i < this.stopsToBeServed.size(); i++) {
      if (!this.stopsToBeServed
          .get(i)
          .getId()
          .toString()
          .equalsIgnoreCase(testPlan.getStopsToBeServed().get(i).getId().toString())) {
        return false;
      }
    }
