...
public final class PStrategyManager {
  ...
  private static final Logger log = LogManager.getLogger(PStrategyManager.class);
  ...
  public void init(
      ...
      StageContainerCreator stageContainerCreator,
      TicketMachineI ticketMachine,
    ...
    for (PStrategySettings settings : pConfig.getStrategySettings()) {
      ...
      PStrategy strategy =
          loadStrategy(classname, settings, stageContainerCreator, ticketMachine, timeProvider);
  ...
  private PStrategy loadStrategy(
      final String name,
      final PStrategySettings settings,
      StageContainerCreator stageContainerCreator,
      TicketMachineI ticketMachine,
      TimeProvider timeProvider) {
    ...
    switch (name) {
      ...
      case ReduceTimeServedRFare.STRATEGY_NAME:
        {
          ReduceTimeServedRFare strat =
              new ReduceTimeServedRFare(settings.getParametersAsArrayList());
      ...
      case ReduceStopsToBeServedRFare.STRATEGY_NAME:
        {
          ReduceStopsToBeServedRFare strat =
              new ReduceStopsToBeServedRFare(settings.getParametersAsArrayList());
          strat.setTicketMachine(ticketMachine);
          stageContainerCreator.addStageContainerHandler(strat);
          strategy = strat;
          break;
      ...
      case WeightedStartTimeExtension.STRATEGY_NAME:
        {
          WeightedStartTimeExtension strat =
              new WeightedStartTimeExtension(settings.getParametersAsArrayList());
      ...
      case WeightedEndTimeExtension.STRATEGY_NAME:
        {
          WeightedEndTimeExtension strat =
              new WeightedEndTimeExtension(settings.getParametersAsArrayList());
          strat.setTimeProvider(timeProvider);
          strategy = strat;
          break;
        }
  ...
  /**
   * Changes the weights of each strategy to zero and removes it from the choice set if it needs to
   * be disabled
   */
  ...
  /**
   * Picks randomly one strategy from the set of available strategies. Strategies with a higher
   * weight have a higher probability of being picked.
   *
  ...
  public String toString() {
    ...
    strBuilder.append(this.strategies.get(0).getStrategyName());
    strBuilder.append(" (");
    strBuilder.append(this.weights.get(0));
    strBuilder.append(")");

    for (int i = 1; i < this.strategies.size(); i++) {
      strBuilder.append(", ");
      strBuilder.append(this.strategies.get(i).getStrategyName());
      strBuilder.append(" (");
      strBuilder.append(this.weights.get(i));
      strBuilder.append(")");
    }
