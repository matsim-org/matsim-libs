...
public final class TimeProvider implements ActivityStartEventHandler, ActivityEndEventHandler {
  ...
  private static final Logger log = LogManager.getLogger(TimeProvider.class);
  ...
  public TimeProvider(PConfigGroup pConfig, String outputDir) {
    ...
    if (numberOfSlots == 0) {
      log.warn(
          "Calculated number of slots is zero. MaxTime: "
              + maxTime
              + ", timeSlotSize: "
              + this.timeSlotSize);
      numberOfSlots = 1;
    ...
    this.writer =
        IOUtils.getBufferedWriter(
            outputDir + PConstants.statsOutputFolder + "timeSlots2weight.txt");
    ...
    try {
      this.writer.write("# iteration" + strB.toString());
      this.writer.newLine();
    } catch (IOException e) {
  ...
  public double getRandomTimeInInterval(double startTime, double endTime) {
    ...
    log.warn(
        "Could not find any time slot. This should not happen. Check time slot size in config. Will return the start slot time");
