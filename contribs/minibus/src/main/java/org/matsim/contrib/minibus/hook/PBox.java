...
public final class PBox implements POperators {
  ...
  private static final Logger log = LogManager.getLogger(PBox.class);
  ...
  /**
   * Constructor that allows to set the ticketMachine. Deliberately in constructor and not as setter
   * to keep the variable final. Might be replaced by a builder and/or guice at some later point in
   * time. But stay with "direct" injection for the time being. kai, jan'17
   */
  @Inject
  PBox(PConfigGroup pConfig, TicketMachineI ticketMachine) {
    ...
    this.operatorCostCollectorHandler =
        new OperatorCostCollectorHandler(
            this.pConfig.getPIdentifier(),
            this.pConfig.getCostPerVehicleAndDay(),
            this.pConfig.getCostPerKilometer() / 1000.0,
            this.pConfig.getCostPerHour() / 3600.0);
  ...
  void notifyStartup(StartupEvent event) {
    ...
    TimeProvider timeProvider =
        new TimeProvider(this.pConfig, event.getServices().getControlerIO().getOutputPath());
    event.getServices().getEvents().addHandler(timeProvider);

    // initialize strategy manager
    this.strategyManager.init(
        this.pConfig, this.stageCollectorHandler, this.ticketMachine, timeProvider);
    ...
    StageContainer2AgentMoneyEvent fare2AgentMoney =
        new StageContainer2AgentMoneyEvent(event.getServices(), this.ticketMachine);
    this.stageCollectorHandler.addStageContainerHandler(fare2AgentMoney);

    // init possible paratransit stops
    this.pStopsOnly =
        PStopsFactory.createPStops(
            event.getServices().getScenario().getNetwork(),
            this.pConfig,
            event.getServices().getScenario().getTransitSchedule(),
            event.getServices().getScenario().getConfig().network());

    this.operators = new LinkedList<>();
    this.operatorInitializer =
        new OperatorInitializer(
            this.pConfig, this.franchise, this.pStopsOnly, event.getServices(), timeProvider);

    // init additional operators from a given transit schedule file
    LinkedList<Operator> operatorsFromSchedule =
        this.operatorInitializer.createOperatorsFromSchedule(
            event.getServices().getScenario().getTransitSchedule());
    this.operators.addAll(operatorsFromSchedule);

    // init initial set of operators - reduced by the number of preset operators
    LinkedList<Operator> initialOperators =
        this.operatorInitializer.createAdditionalOperators(
            this.strategyManager,
            event.getServices().getConfig().controller().getFirstIteration(),
            (this.pConfig.getNumberOfOperators() - operatorsFromSchedule.size()));
  ...
  void notifyScoring(ScoringEvent event) {
    ...
    Map<Id<Vehicle>, PScoreContainer> driverId2ScoreMap =
        this.scorePlansHandler.getDriverId2ScoreMap();
    for (Operator operator : this.operators) {
  ...
  private void handleBankruptOperators(int iteration) {
    ...
    if (this.pConfig.getUseAdaptiveNumberOfOperators()) {
      ...
      numberOfNewOperators =
          (int)
                  (operatorsInBusiness * (1.0 / this.pConfig.getShareOfOperatorsWithProfit() - 1.0)
                      + 0.0000000000001)
              - operatorsProspecting;
    }
    ...
    if (this.pConfig.getDisableCreationOfNewOperatorsInIteration() > iteration) {
      // recreate all other
      LinkedList<Operator> newOperators1 =
          this.operatorInitializer.createAdditionalOperators(
              this.strategyManager, iteration, numberOfNewOperators);
      this.operators.addAll(newOperators1);

      // too few operators in play, increase to the minimum specified in the config
      LinkedList<Operator> newOperators2 =
          this.operatorInitializer.createAdditionalOperators(
              this.strategyManager,
              iteration,
              (this.pConfig.getNumberOfOperators() - this.operators.size()));
      ...
      if (this.operators.size() == operatorsInBusiness) {
        LinkedList<Operator> newOperators3 =
            this.operatorInitializer.createAdditionalOperators(this.strategyManager, iteration, 1);
