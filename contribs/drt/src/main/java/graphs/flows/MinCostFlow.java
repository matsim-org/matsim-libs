...
public class MinCostFlow {
  ...
  public static int[] minCostFlow(
      List<Edge>[] graph, int s, int t, int maxf, boolean runBellmanFord) {
    int n = graph.length;
    int[] prio = new int[n];
    ...
    if (runBellmanFord) {
      bellmanFord(
          graph, s, pot); // bellmanFord invocation can be skipped if edges costs are non-negative
    }
    ...
    while (flow < maxf) {
      ...
      if (prio[t] == Integer.MAX_VALUE) break;
