...
public class VehicleDataEntryFactoryImpl implements VehicleEntry.EntryFactory {
  public VehicleEntry create(DvrpVehicle vehicle, double currentTime) {
      ...
      start =
          switch (getBaseTypeOrElseThrow(startTask)) {
            case DRIVE -> {
              ...
              yield diversionPoint != null
                  ? diversionPoint
                  : // diversion possible
                  new LinkTimePair(
                      driveTask.getPath().getToLink(),
                      driveTask.getEndTime()); // too late for diversion
            }
            case STOP -> new LinkTimePair(
                ((DrtStopTask) startTask).getLink(), startTask.getEndTime());
    ...
    Waypoint.Stop startStop =
        startTask != null && STOP.isBaseTypeOf(startTask)
    ...
    return new VehicleEntry(
        vehicle,
        new Waypoint.Start(startTask, start.link, start.time, outgoingOccupancy),
  ...
  static double[] computeSlackTimes(
      DvrpVehicle vehicle,
      double now,
      Waypoint.Stop[] stops,
      Waypoint.Stop start,
