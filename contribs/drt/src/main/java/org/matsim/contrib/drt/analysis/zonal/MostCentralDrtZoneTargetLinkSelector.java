...
public class MostCentralDrtZoneTargetLinkSelector implements DrtZoneTargetLinkSelector {
  ...
  public MostCentralDrtZoneTargetLinkSelector(DrtZonalSystem drtZonalSystem) {
    targetLinks =
        drtZonalSystem.getZones().values().stream()
            .collect(
                toMap(
                    zone -> zone,
                    zone ->
                        zone.getLinks().stream()
                            .min(
                                // 1. choose links with the most central toNode (there may be
                                // several "most central" nodes)
                                // 2. if there is more than one such link (which is usually the
                                // case),
                                //   choose one with the most central fromNode
                                Comparator.<Link>comparingDouble(
                                        link -> squaredDistance(zone, link.getToNode())) //
                                    .thenComparing(
                                        link -> squaredDistance(zone, link.getFromNode())))
                            .orElseThrow()));
  }
