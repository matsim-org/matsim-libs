...
/**
 * This calculator is a more flexible implementation of CorrectedStopTimeCalculator. Instead of
 * imposing a fixed stop duration, it makes use of a StopDurationProvider. Adding a pickup or a
 * dropoff may extend the stop duration to accomodate for a request that needs more time to enter
...
public class ParallelStopTimeCalculator implements StopTimeCalculator {
  ...
  @Override
  public double updateEndTimeForPickup(
    ...
    return Math.max(
        stop.getEndTime(),
        insertionTime + stopDurationProvider.calcPickupDuration(vehicle, request));
  }
  ...
  @Override
  public double updateEndTimeForDropoff(
      DvrpVehicle vehicle, DrtStopTask stop, double insertionTime, DrtRequest request) {
    // adding a dropoff may extend the stop duration
    return Math.max(
        stop.getEndTime(),
        insertionTime + stopDurationProvider.calcDropoffDuration(vehicle, request));
  }
