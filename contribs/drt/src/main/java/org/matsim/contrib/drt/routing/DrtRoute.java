 ...
 *
 * <ul>
 *   <li>{@code maxWaitTime} is the maximum wait time
 *   <li>{@code directRideTime} is the time of an unshared ride
 *   <li>{@code travelTime} is the maximum travel (wait+ride) time (excluding walk to/from the stop)
 * </ul>
 *
...
public class DrtRoute extends AbstractRoute {
  ...
  public static final String ROUTE_TYPE = TransportMode.drt;
  ...
  public double getMaxTravelTime() {
    return getTravelTime()
        .seconds(); // currently DrtRoute.travelTime is set to the max allowed travel time
  }
  ...
  public void setRouteDescription(String routeDescription) {
    ...
    if (pat.matcher(routeDescription).find()) {
      ...
      this.routeDescription.setDirectRideTime(
          requiresZeroOrPositive(Double.parseDouble(values[1])));
  ...
  private double requiresZeroOrPositive(double value) {
    checkArgument(
        value >= 0 || value <= Double.MAX_VALUE, "Value: (%s) must be zero or positive", value);
    return value;
  }
  ...
  public static class RouteDescription {
    ...
    public double getDirectRideTime() {
      return directRideTime.isUndefined()
          ? OptionalTime.undefined().seconds()
          : directRideTime.seconds();
    }
