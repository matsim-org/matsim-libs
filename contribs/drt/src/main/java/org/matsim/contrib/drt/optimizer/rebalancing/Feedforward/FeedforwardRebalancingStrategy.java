...
/**
 * This strategy is created based on the Feedforward Fluidic rebalancing algorithm in AMoDeus. The
 * algorithm send rebalancing vehicles based on the DRT demand flow of previous iteration. This
 * strategy is comparable to the MinCostFlowRebalancing Strategy with Previous Iteration Zonal DRT
 * Demand Aggregator. But, instead of setting a rebalance target for each zone, the concept of flow
 * is used.
 *
 * <p>Important: At least 2 iterations are needed in order to make this strategy function properly.
 *
 * @author Chengqi Lu
 */
public class FeedforwardRebalancingStrategy implements RebalancingStrategy {
  ...
  public FeedforwardRebalancingStrategy(
    ...
    scaling =
        strategySpecificParams.feedforwardSignalStrength
            * (double) generalParams.interval
            / timeBinSize;
    log.info(
        "The feedforward signal strength is: " + strategySpecificParams.feedforwardSignalStrength);
  ...
  public List<Relocation> calcRelocations(
    ...
    Map<DrtZone, List<DvrpVehicle>> rebalancableVehiclesPerZone =
        RebalancingUtils.groupRebalancableVehicles(
            zonalSystem, generalParams, rebalancableVehicles, time);
    ...
    if (feedbackSwitch) {
      ...
      Map<DrtZone, List<DvrpVehicle>> soonRebalancableVehiclesPerZone =
          RebalancingUtils.groupSoonIdleVehicles(zonalSystem, generalParams, fleet, time);
      ...
      relocationList.addAll(
          fastHeuristicRelocationCalculator.calcRelocations(
              vehicleSurplusList, rebalancableVehiclesPerZone));
      // Connection between feedback and feedforward part
      Set<DvrpVehicle> relocatedVehicles =
          relocationList.stream().map(relocation -> relocation.vehicle).collect(toSet());
      for (DrtZone zone : rebalancableVehiclesPerZone.keySet()) {
        actualRebalancableVehiclesPerZone.put(
            zone,
            rebalancableVehiclesPerZone.get(zone).stream()
    ...
    if (feedforwardSignal.containsKey(timeBin)) {
      ...
      for (Flow<DrtZone, DrtZone> rebalanceInfo : feedforwardSignal.get(timeBin)) {
        ...
        List<DvrpVehicle> rebalancableVehiclesInDepartureZone =
            actualRebalancableVehiclesPerZone.get(departureZone);
        ...
        for (int i = 0; i < vehicleToSend; i++) {
          ...
          relocationList.add(
              new Relocation(
                  rebalancableVehiclesInDepartureZone.get(numAvailableVehiclesInZone - 1),
                  destinationLink));
