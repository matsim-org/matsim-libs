...
public class InsertionDetourTimeCalculator {
  ...
  // If the detour data uses approximated detour times (e.g. beeline or matrix-based), provide the
  // same estimator for
  // replaced drives to avoid systematic errors
  // When null, the replaced drive duration is derived from the schedule
  @Nullable private final DetourTimeEstimator replacedDriveTimeEstimator;

  public InsertionDetourTimeCalculator(
  ...
  public DetourTimeInfo calculateDetourTimeInfo(
      Insertion insertion, InsertionDetourData detourData, DrtRequest drtRequest) {
    ...
    double toDropoffTT =
        followedByDropoff ? fromPickupTT : detourData.detourToDropoff.getTravelTime();
    double fromDropoffTT = detourData.detourFromDropoff.getTravelTime();
    var dropoffDetourInfo =
        calcDropoffDetourInfo(insertion, toDropoffTT, fromDropoffTT, pickupDetourInfo, drtRequest);

    return new DetourTimeInfo(pickupDetourInfo, dropoffDetourInfo);
  }

  public PickupDetourInfo calcPickupDetourInfo(
      VehicleEntry vEntry,
    ...
    if (followedByDropoff) {
      ...
      return calcPickupDetourInfoIfPickupToDropoffDetour(
          vEntry, pickup, toPickupTT, fromPickupTT, drtRequest);
    }
    ...
    if (pickup.newWaypoint.getLink() == pickup.previousWaypoint.getLink()) {
      ...
      var times =
          calculatePickupIfSameLink(vEntry, pickup.index, toPickupDepartureTime, drtRequest);
    ...
    double departureTime =
        stopTimeCalculator.initEndTimeForPickup(vEntry.vehicle, arrivalTime, drtRequest);
    double stopDuration = departureTime - arrivalTime;
    double replacedDriveTT =
        calculateReplacedDriveDuration(vEntry, pickup.index, toPickupDepartureTime);
  ...
  public DropoffDetourInfo calcDropoffDetourInfo(
      Insertion insertion,
      double toDropoffTT,
    ...
    if (insertion.pickup.index == insertion.dropoff.index) {
      // handle the pickup->dropoff case separately
      return calcDropoffDetourInfoIfPickupToDropoffDetour(
          toDropoffTT, fromDropoffTT, pickupDetourInfo, insertion.vehicleEntry, drtRequest);
    }
    ...
    if (dropoff.newWaypoint.getLink() == dropoff.previousWaypoint.getLink()) {
      double remainingPickupTimeLoss =
          calculateRemainingPickupTimeLossAtDropoff(insertion, pickupDetourInfo);
      ...
      double departureTime =
          stopTimeCalculator.updateEndTimeForDropoff(
              vEntry.vehicle, stopTask, arrivalTime, drtRequest);
    ...
    double departureTime =
        stopTimeCalculator.initEndTimeForDropoff(vEntry.vehicle, arrivalTime, drtRequest);
    double stopDuration = departureTime - arrivalTime;
    double replacedDriveTT =
        calculateReplacedDriveDuration(vEntry, dropoff.index, toDropoffDepartureTime);
  ...
  private PickupDetourInfo calcPickupDetourInfoIfPickupToDropoffDetour(
      VehicleEntry vEntry,
      InsertionPoint pickup,
      double toPickupTT,
      double fromPickupTT,
    ...
    if (pickup.newWaypoint.getLink() == pickup.previousWaypoint.getLink()) {
      ...
      departureTime =
          stopTimeCalculator.initEndTimeForPickup(
              vEntry.vehicle, toPickupDepartureTime + toPickupTT, drtRequest);
  ...
  private DropoffDetourInfo calcDropoffDetourInfoIfPickupToDropoffDetour(
      double fromPickupToDropoffTT,
      double fromDropoffTT,
      PickupDetourInfo pickupDetourInfo,
      VehicleEntry vEntry,
      DrtRequest drtRequest) {
  ...
  private PickupTimeInfo calculatePickupIfSameLink(
      VehicleEntry vEntry, int pickupIndex, double toPickupDepartureTime, DrtRequest request) {
    ...
    if (stopTask == null) {
      ...
      double departureTime =
          stopTimeCalculator.initEndTimeForPickup(vEntry.vehicle, toPickupDepartureTime, request);
      ...
      double departureTime =
          stopTimeCalculator.updateEndTimeForPickup(
              vEntry.vehicle, stopTask, insertionTime, request);
      ...
      double departureTime =
          stopTimeCalculator.updateEndTimeForPickup(
              vEntry.vehicle, stopTask, vEntry.createTime, request);
      double additionalStopDuration = departureTime - stopTask.getEndTime();
      return new PickupTimeInfo(departureTime, additionalStopDuration);
    }
  }
  ...
  private double calculateReplacedDriveDuration(
      VehicleEntry vEntry, int insertionIdx, double detourStartTime) {
    if (vEntry.isAfterLastStop(insertionIdx)) {
      return 0; // end of route - bus would wait there
    }

    if (replacedDriveTimeEstimator != null) {
      // use the approximated drive times instead of deriving (presumably more accurate) times from
      // the schedule
      return replacedDriveTimeEstimator.estimateTime(
          vEntry.getWaypoint(insertionIdx).getLink(),
    ...
    return replacedDriveEndTime
        - replacedDriveStartTime
        - vEntry.getPrecedingStayTime(insertionIdx);
  }
   ...
   */
  public static double calculateRemainingPickupTimeLossAtDropoff(
      Insertion insertion, PickupDetourInfo pickupDetourInfo) {
    VehicleEntry vEntry = insertion.vehicleEntry;
    ...
    for (int i = insertion.pickup.index + 1; i < insertion.dropoff.index; i++) {
      remainingPickupTimeLoss =
          Math.max(remainingPickupTimeLoss - vEntry.getPrecedingStayTime(i), 0.0);
    }
  ...
  public static class DetourTimeInfo {
    ...
    /**
     * @return TOTAL time delay of each stop placed after the dropoff insertion point (this is the
     *     amount of extra time the vehicle will operate if this insertion is applied)
     */
