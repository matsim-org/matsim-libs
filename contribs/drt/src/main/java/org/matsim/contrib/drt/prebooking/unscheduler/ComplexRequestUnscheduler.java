...
/**
 * This RequestUnscheduler searches for a request in a vehicle's schedule and removes the request
 * from the relevant stop tasks. Furthermore, the stops are removed if they don't carry any other
 * pickups or dropoffs. Accordingly, the schedule will also be rerouted.
 *
...
public class ComplexRequestUnscheduler implements RequestUnscheduler {
  ...
  public ComplexRequestUnscheduler(
      DvrpVehicleLookup vehicleLookup,
  ...
  public void unscheduleRequest(double now, Id<DvrpVehicle> vehicleId, Id<Request> requestId) {
    ...
    boolean removePickup =
        pickupStopTask.getPickupRequests().size() == 0
            && pickupStopTask.getDropoffRequests().size() == 0;
    boolean removeDropoff =
        dropoffStopTask.getPickupRequests().size() == 0
    ...
    if (pickupReplacement != null && dropoffReplacement != null) {
      if (pickupReplacement.endTask.getTaskIdx() >= dropoffReplacement.startTask.getTaskIdx()) {
        ...
        pickupReplacement =
            new Replacement(
                pickupReplacement.startTask, dropoffReplacement.endTask, vehicle.getSchedule());
  ...
  private void unschedule(double now, VehicleEntry vEntry, Replacement replacement) {
    ...
    if (removeUntilEnd) {
      ...
      schedule.addTask(
          taskFactory.createStayTask(
              vEntry.vehicle,
              replacement.startTask.getEndTime(),
              Math.max(replacement.startTask.getEndTime(), initialEndTime),
              stayLink));
    ...
    if (replacement.startTask instanceof DriveTask) { // special case: start task is driving
      ...
      VrpPathWithTravelData vrpPath =
          VrpPaths.calcAndCreatePathForDiversion(diversion, endLink, router, travelTime);
      ...
      if (vrpPath.getArrivalTime() < endArrivalTime && scheduleWaitBeforeDrive) {
        ...
        lastInsertedTask =
            insertDriveWithWait(vEntry.vehicle, replacement.startTask, vrpPath, endArrivalTime);
      } else {
      ...
      if (startLink == endLink) { // no need to move, maybe just wait
        ...
        VrpPathWithTravelData vrpPath =
            VrpPaths.calcAndCreatePath(
                startLink, endLink, startStayTask.getEndTime(), router, travelTime);

        lastInsertedTask =
            insertDriveWithWait(vEntry.vehicle, startStayTask, vrpPath, endArrivalTime);
      }
    }

    timingUpdater.updateTimingsStartingFromTaskIdx(
        vEntry.vehicle, lastInsertedTask.getTaskIdx() + 1, lastInsertedTask.getEndTime());
  }
  ...
  private class Replacement {
    ...
    Replacement(Task startTask, Task endTask, Schedule schedule) {
      ...
      boolean endIsLastStay =
          endTask instanceof DrtStayTask && Schedules.getLastTask(schedule) == endTask;
