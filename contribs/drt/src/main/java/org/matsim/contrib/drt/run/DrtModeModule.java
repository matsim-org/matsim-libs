...
public final class DrtModeModule extends AbstractDvrpModeModule {
  ...
  public void install() {
    ...
    bindModal(DefaultStopTimeCalculator.class)
        .toProvider(
            modalProvider(
                getter -> {
    ...
    if (drtCfg.getPrebookingParams().isEmpty()) {
      ...
      bindModal(StopTimeCalculator.class)
          .toProvider(
              modalProvider(
                  getter -> {
                    PassengerStopDurationProvider provider =
                        getter.getModal(PassengerStopDurationProvider.class);
                    return new MinimumStopDurationAdapter(
                        new PrebookingStopTimeCalculator(provider), drtCfg.stopDuration);
                  }));
