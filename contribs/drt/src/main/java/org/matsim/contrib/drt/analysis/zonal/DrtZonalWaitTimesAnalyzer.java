...
public final class DrtZonalWaitTimesAnalyzer implements IterationEndsListener, ShutdownListener {
  ...
  public DrtZonalWaitTimesAnalyzer(
      DrtConfigGroup configGroup, DrtEventSequenceCollector requestAnalyzer, DrtZonalSystem zones) {
    ...
    String fileName =
        event
            ...
            .getIterationFilename(
                event.getIteration(), "waitStats" + "_" + drtCfg.getMode() + "_zonal.csv");
    write(fileName);
  }
  ...
  public void write(String fileName) {
    ...
    try {
      ...
      bw.append(
          "zone;centerX;centerY;nRequests;sumWaitTime;meanWaitTime;min;max;p95;p90;p80;p75;p50");
      // sorted output
      ...
      for (String zoneId : zoneIdsAndOutside) {
        ...
        String centerX =
            drtZone != null ? String.valueOf(drtZone.getCentroid().getX()) : notAvailableString;
        String centerY =
            drtZone != null ? String.valueOf(drtZone.getCentroid().getY()) : notAvailableString;
  ...
  private Map<String, DescriptiveStatistics> createZonalStats() {
    ...
    for (EventSequence seq : requestAnalyzer.getPerformedRequestSequences().values()) {
      for (Map.Entry<Id<Person>, EventSequence.PersonEvents> entry :
          seq.getPersonEvents().entrySet()) {
  ...
  private Collection<SimpleFeature> convertGeometriesToSimpleFeatures(
    ...
    for (DrtZone zone : zones.getZones().values()) {
      ...
      Geometry geometry =
          zone.getPreparedGeometry() != null ? zone.getPreparedGeometry().getGeometry() : null;
