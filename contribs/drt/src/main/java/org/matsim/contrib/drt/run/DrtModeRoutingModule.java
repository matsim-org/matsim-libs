...
public class DrtModeRoutingModule extends AbstractDvrpModeModule {
  ...
  public void install() {
    ...
    bindModal(DefaultMainLegRouter.RouteCreator.class)
        .toProvider(new DrtRouteCreatorProvider(drtCfg)); // not singleton

    bindModal(DrtStopNetwork.class)
        .toProvider(new DrtStopNetworkProvider(getConfig(), drtCfg))
        .asEagerSingleton();
    ...
    bindModal(DrtRouteUpdater.class)
        .toProvider(
            new ModalProviders.AbstractProvider<>(getMode(), DvrpModes::mode) {
              ...
              public DefaultDrtRouteUpdater get() {
                ...
                var routeCreatorProvider =
                    getModalProvider(DefaultMainLegRouter.RouteCreator.class);
                return new DefaultDrtRouteUpdater(
                    drtCfg, network, population, config, routeCreatorProvider::get);
              }
            })
        .asEagerSingleton();
  ...
  private static class DrtRouteCreatorProvider
      extends ModalProviders.AbstractProvider<DvrpMode, DrtRouteCreator> {
  ...
  private static class DrtStopNetworkProvider
      extends ModalProviders.AbstractProvider<DvrpMode, DrtStopNetwork> {
    ...
    public DrtStopNetwork get() {
      switch (drtCfg.operationalScheme) {
        ...
        case serviceAreaBased:
          return createDrtStopNetworkFromServiceArea(
              config, drtCfg, getModalInstance(Network.class));
        default:
  ...
  private static DrtStopNetwork createDrtStopNetworkFromServiceArea(
      Config config, DrtConfigGroup drtCfg, Network drtNetwork) {
    ...
    ImmutableMap<Id<DrtStopFacility>, DrtStopFacility> drtStops =
        drtNetwork.getLinks().values().stream()
            .filter(
                link ->
                    ShpGeometryUtils.isCoordInPreparedGeometries(
                        link.getToNode().getCoord(), preparedGeometries))
            .map(DrtStopFacilityImpl::createFromLink)
            .collect(ImmutableMap.toImmutableMap(DrtStopFacility::getId, f -> f));
    return () -> drtStops;
  }

  private static DrtStopNetwork createDrtStopNetworkFromTransitSchedule(
      Config config, DrtConfigGroup drtCfg) {
