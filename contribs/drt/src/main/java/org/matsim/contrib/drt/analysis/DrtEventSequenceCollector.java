...
/**
 * Creates PerformedRequestEventSequence (for scheduled requests) and RejectedRequestEventSequence
 * (for rejected requests). Almost all data for request/leg analysis is there (except info on actual
 * paths), so should be quite reusable.
 *
 * <p>Without prebooking, the order of sequences is always the same: First the agent *departs* then
 * the request is *submitted*, then it is *rejected* or picked up. With prebooking, the order of
 * departure and submission can be reversed: An agent first submits the request, and only later
 * departs on the leg. Since *departure* is core MATSim, the respective event has no information
 * about the request identifier. It could now happen that two submission with same characteristics
 * have been submitted (person id, origin id). Then it is not clear which request belongs to the
 * current departure. For that purpose the PassengerWaiting event has been introduced which is fired
 * right after a departure has been processed by a DRT-related PassengerEngine. This events allows
 * to link the latest departure of an agent to a request id.
 *
 * @author jbischoff
...
public class DrtEventSequenceCollector
    implements PassengerRequestRejectedEventHandler,
        PassengerRequestScheduledEventHandler,
        DrtRequestSubmittedEventHandler,
        PassengerWaitingEventHandler,
  ...
  public static class EventSequence {
    ...
    public static class PersonEvents {
      ...
      @Nullable private PersonDepartureEvent departure;
    ...
    @Nullable private PassengerRequestRejectedEvent rejected;

    private final Map<Id<Person>, PersonEvents> personEvents = new HashMap<>();

    @Nullable private List<PersonMoneyEvent> drtFares = new LinkedList<>();
  ...
  public Map<Id<Request>, EventSequence> getPerformedRequestSequences() {
    return sequences.entrySet().stream() //
        ...
        .filter(
            e ->
                e.getValue().personEvents.values().stream()
                    .allMatch(pe -> pe.departure != null)) //
        .filter(
            e ->
                e.getValue().personEvents.values().stream().allMatch(pe -> pe.pickedUp != null)) //
        .collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
  }
  ...
  public void handleEvent(PersonDepartureEvent event) {
    if (event.getLegMode().equals(mode)) {
      ...
      Preconditions.checkState(
          !latestDepartures.containsKey(event.getPersonId()),
          "Attempt to register a departure event for "
              + mode
              + " and person "
              + event.getPersonId()
    ...
    if (event.getMode().equals(mode)) {
      ...
      for (Id<Person> personId : event.getPersonIds()) {
        ...
        PersonDepartureEvent departureEvent =
            Objects.requireNonNull(latestDepartures.remove(personId));

        if (sequence != null) {
          // prebooked request, we already have the submission
          Verify.verifyNotNull(sequence.submitted);
          sequence.personEvents.computeIfAbsent(personId, p -> new EventSequence.PersonEvents())
                  .departure =
              departureEvent;
        } else {
          // immediate request, submission event should come soon
          waitingForSubmission
              .computeIfAbsent(event.getRequestId(), requestId -> new ArrayList<>())
              .add(departureEvent);
        }
      }
    }
  ...
  public void handleEvent(PersonMoneyEvent event) {
    if (mode.equals(event.getTransactionPartner())
        && DrtFareHandler.PERSON_MONEY_EVENT_PURPOSE_DRT_FARE.equals(event.getPurpose())) {
      ...
      EventSequence sequence = sequences.get(Id.create(event.getReference(), Request.class));
      if (sequence != null) {
