...
public class DrtConfigGroup extends ReflectiveConfigGroupWithConfigurableParameterSets
    implements Modal {
  ...
  public static DrtConfigGroup getSingleModeDrtConfig(Config config) {
    Collection<DrtConfigGroup> drtConfigGroups =
        MultiModeDrtConfigGroup.get(config).getModalElements();
    Preconditions.checkArgument(
        drtConfigGroups.size() == 1,
        "Supported for only 1 DRT mode in the config. Number of DRT modes: %s",
        drtConfigGroups.size());
  ...
  @Parameter
  @Comment(
      "Specifies the duration (seconds) a vehicle needs to be idle in order to get send back to the depot."
  ...
  @Parameter
  @Comment(
      "Specifies the time interval (seconds) a vehicle gets evaluated to be send back to depot.")
  ...
  // TODO consider renaming maxWalkDistance to max access/egress distance (or even have 2 separate
  // params)
  @Parameter
  @Comment(
  ...
  @Parameter
  @Comment(
      "An XML file specifying the vehicle fleet."
  ...
  public boolean storeUnsharedPath =
      false; // If true, the planned unshared path is stored and exported in plans

  @NotNull private DrtInsertionSearchParams drtInsertionSearchParams;

  @Nullable private DrtZonalSystemParams zonalSystemParams;

  @Nullable private RebalancingParams rebalancingParams;

  @Nullable private DrtFareParams drtFareParams;

  @Nullable private DrtSpeedUpParams drtSpeedUpParams;

  @Nullable private PrebookingParams prebookingParams;

  @Nullable private DrtRequestInsertionRetryParams drtRequestInsertionRetryParams;
  ...
  private void initSingletonParameterSets() {
    ...
    addDefinition(
        RebalancingParams.SET_NAME,
        RebalancingParams::new,
        () -> rebalancingParams,
        params -> rebalancingParams = (RebalancingParams) params);

    // zonal system (optional)
    addDefinition(
        DrtZonalSystemParams.SET_NAME,
        DrtZonalSystemParams::new,
        () -> zonalSystemParams,
    ...
    addDefinition(
        ExtensiveInsertionSearchParams.SET_NAME,
        ExtensiveInsertionSearchParams::new,
        () -> drtInsertionSearchParams,
        params -> drtInsertionSearchParams = (ExtensiveInsertionSearchParams) params);
    addDefinition(
        SelectiveInsertionSearchParams.SET_NAME,
        SelectiveInsertionSearchParams::new,
        () -> drtInsertionSearchParams,
        params -> drtInsertionSearchParams = (SelectiveInsertionSearchParams) params);
    addDefinition(
        RepeatedSelectiveInsertionSearchParams.SET_NAME,
        RepeatedSelectiveInsertionSearchParams::new,
        () -> drtInsertionSearchParams,
  ...
  protected void checkConsistency(Config config) {
    ...
    if (config.qsim().getEndTime().isUndefined()
      ...
      // Not an issue if all request rejections are immediate (i.e. happen during request
      // submission)
      log.warn(
          "qsim.endTime should be specified and qsim.simEndtimeInterpretation should be 'onlyUseEndtime'"
    ...
    Verify.verify(
        operationalScheme != OperationalScheme.stopbased || transitStopFile != null,
        "transitStopFile must not be null when operationalScheme is "
            + OperationalScheme.stopbased);

    Verify.verify(
        operationalScheme != OperationalScheme.serviceAreaBased || drtServiceAreaShapeFile != null,
    ...
    if (config.global().getNumberOfThreads() < numberOfThreads) {
      log.warn(
          "Consider increasing global.numberOfThreads to at least the value of drt.numberOfThreads"
              + " in order to speed up the DRT route update during the replanning phase.");
    }

    if (this.idleVehiclesReturnToDepots
        && this.returnToDepotTimeout < this.returnToDepotEvaluationInterval) {
      log.warn(
          "idleVehiclesReturnToDepots is active and returnToDepotTimeout < returnToDepotEvaluationInterval. "
              + "Vehicles will be send back to depot after {} seconds",
          returnToDepotEvaluationInterval);
    }

    Verify.verify(
        getParameterSets(MinCostFlowRebalancingStrategyParams.SET_NAME).size() <= 1,
  ...
  /**
   * Convenience method that brings syntax closer to syntax in, e.g., {@link RoutingConfigGroup} or
   * {@link ScoringConfigGroup}
   */
