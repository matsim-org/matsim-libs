...
class SelectiveInsertionProvider {
  public static SelectiveInsertionProvider create(
  ...
  @VisibleForTesting
  SelectiveInsertionProvider(
      BestInsertionFinder initialInsertionFinder,
  ...
  Optional<InsertionWithDetourData> getInsertion(
    ...
    return forkJoinPool
        .submit(
            ...
            () ->
                initialInsertionFinder.findBestInsertion(
                    drtRequest,
                    // for each vehicle entry
                    vehicleEntries.parallelStream()
                        // generate feasible insertions (wrt occupancy limits) with restrictive
                        // detour times
                        .flatMap(
                            e -> insertionGenerator.generateInsertions(drtRequest, e).stream())))
        .join();
  }
}
