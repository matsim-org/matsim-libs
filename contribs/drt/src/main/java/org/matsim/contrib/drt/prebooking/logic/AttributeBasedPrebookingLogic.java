 ...
 * "prebooking:submissionTime:[mode]" attribute in the origin activity of a trip. If a DRT leg is
 * found in the trip, the submission time for this request will be read from the attribute. In order
 * to make use of prebooked requests, you need to define these attributes before the start of the
 * QSim iteration.
 *
 * <p>To indicate the expected departure time (for which the vehicle is requested), you can also
 * define the "prebooking:plannedDepartureTime:[mode]" attribute. If it is not specified, the logic
 * will try to figure out the departure time by traversing the activities and legs according to the
 * configured time interpretation.
 *
 * @author Sebastian Hörl (sebhoerl), IRT SystemX
 */
public class AttributeBasedPrebookingLogic implements PrebookingLogic, MobsimInitializedListener {
  private static final String SUBMISSION_TIME_ATTRIBUTE_PREFIX = "prebooking:submissionTime";
  private static final String PLANNED_DEPARTURE_ATTRIBUTE_PREFIX =
      "prebooking:plannedDepartureTime";

  public static String getSubmissionAttribute(String mode) {
    return SUBMISSION_TIME_ATTRIBUTE_PREFIX + mode;
  }

  public static String getPlannedDepartureAttribute(String mode) {
    return PLANNED_DEPARTURE_ATTRIBUTE_PREFIX + mode;
  }

  public static Optional<Double> getSubmissionTime(String mode, Trip trip) {
    return Optional.ofNullable(
        (Double) trip.getTripAttributes().getAttribute(getSubmissionAttribute(mode)));
  }

  public static Optional<Double> getPlannedDepartureTime(String mode, Trip trip) {
    return Optional.ofNullable(
        (Double) trip.getTripAttributes().getAttribute(getPlannedDepartureAttribute(mode)));
  }
  ...
  private AttributeBasedPrebookingLogic(
      String mode,
            ...
            if (mode.equals(leg.getMode())) {
              ...
              if (submissionTime.isPresent()) {
                if (plannedDepartureTime.isPresent()) {
                  Preconditions.checkState(
                      plannedDepartureTime.get() > submissionTime.get(),
  ...
  public static AbstractDvrpModeQSimModule createModule(DrtConfigGroup drtConfig) {
    return new AbstractDvrpModeQSimModule(drtConfig.getMode()) {
      @Override
      protected void configureQSim() {
        bindModal(AttributeBasedPrebookingLogic.class)
            .toProvider(
                modalProvider(
                    getter -> {
                      Preconditions.checkState(drtConfig.getPrebookingParams().isPresent());

                      return new AttributeBasedPrebookingLogic(
                          drtConfig.getMode(),
  ...
  public static void install(Controler controller, DrtConfigGroup drtConfig) {
