...
public class DefaultRequestInsertionScheduler implements RequestInsertionScheduler {
  ...
  public DefaultRequestInsertionScheduler(
      Fleet fleet,
      MobsimTimer timer,
      ...
      veh.getSchedule()
          .addTask(
              taskFactory.createStayTask(
                  veh, veh.getServiceBeginTime(), veh.getServiceEndTime(), veh.getStartLink()));
    }
  }

  @Override
  public PickupDropoffTaskPair scheduleRequest(
      AcceptedDrtRequest request, InsertionWithDetourData insertion) {
    var pickupTask = insertPickup(request, insertion);
    verifyTimes(
        "Inconsistent pickup departure time",
    ...
    verifyTimes(
        "Inconsistent dropoff arrival time",
  ...
  private void verifyStructure(Schedule schedule) {
    ...
    int startIndex =
        schedule.getStatus().equals(ScheduleStatus.STARTED)
            ? schedule.getCurrentTask().getTaskIdx()
            : 0;
  ...
  private void verifyTimes(
      String messageStart, double timeFromInsertionData, double timeFromScheduler) {
    // The source of discrepancies is the first link travel time (as it should be taken into
    // consideration
    // when a vehicle enters the traffic (a new drive task), but not when a vehicle is diverted (an
    // ongoing drive task)
    ...
    // Verify.verify(Math.abs(timeFromInsertionData - timeFromScheduler) <= VrpPaths.FIRST_LINK_TT +
    // 1e-10,
    // 		"%s: %s (insertion data) vs %s (scheduler)", messageStart, timeFromInsertionData,
    // 		timeFromScheduler);
  }

  private DrtStopTask insertPickup(
      AcceptedDrtRequest request, InsertionWithDetourData insertionWithDetourData) {
    ...
    if (pickupIdx == 0
        ...
        && DRIVE.isBaseTypeOf(currentTask)) {
      LinkTimePair diversion =
          ((OnlineDriveTaskTracker) currentTask.getTaskTracker()).getDiversionPoint();
      if (diversion != null) { // divert vehicle
        ...
        VrpPathWithTravelData vrpPath =
            VrpPaths.createPath(
                ...
                detourData.detourToPickup,
                travelTime);

        if (vrpPath.getArrivalTime() < request.getEarliestStartTime() && scheduleWaitBeforeDrive) {
          // prebooking case: we need to wait right now
          ((OnlineDriveTaskTracker) beforePickupTask.getTaskTracker())
              .divertPath(VrpPaths.createZeroLengthPathForDiversion(diversion));
          beforePickupTask =
              insertDriveWithWait(
                  vehicleEntry.vehicle, currentTask, vrpPath, request.getEarliestStartTime());
        } else {
          ...
          beforePickupTask =
              insertWait(vehicleEntry.vehicle, beforePickupTask, request.getEarliestStartTime());
        }
      ...
      if (pickupIdx == 0) {
        if (scheduleStatus == ScheduleStatus.PLANNED) { // PLANNED schedule
          ...
          stayTask.setEndTime(
              stayTask.getBeginTime()); // could get later removed with ScheduleTimingUpdater
        } else if (STAY.isBaseTypeOf(currentTask)) {
          stayTask = (StayTask) currentTask; // ongoing stay task
          if (stayTask.getEndTime()
              > now) { // stop stay task; a new stop/drive task can be inserted now
      ...
      boolean canMergePickup =
          stopTask != null
              && request.getFromLink() == stopTask.getLink()
      ...
      if (canMergePickup) { // no detour; no new stop task
        ...
        stopTask.setEndTime(
            stopTimeCalculator.updateEndTimeForPickup(
                vehicleEntry.vehicle, stopTask, now, request.getRequest()));
        ...
        if (pickupIdx == dropoffIdx) {
          ...
          VrpPathWithTravelData vrpPath =
              VrpPaths.createPath(
                  ...
                  toLink,
                  stopTask.getEndTime(),
                  detourData.detourFromPickup,
                  travelTime);
          Task driveFromPickupTask =
              taskFactory.createDriveTask(
                  vehicleEntry.vehicle, vrpPath, DrtDriveTask.TYPE); // immediate drive to dropoff
          ...
          scheduleTimingUpdater.updateTimingsStartingFromTaskIdx(
              vehicleEntry.vehicle, stopTask.getTaskIdx() + 2, driveFromPickupTask.getEndTime());
          ///////
        } else {
          scheduleTimingUpdater.updateTimingsStartingFromTaskIdx(
              vehicleEntry.vehicle, stopTask.getTaskIdx() + 1, stopTask.getEndTime());
        }
        ...
        if (request.getFromLink() == stayOrStopTask.getLink()) {
          ...
          VrpPathWithTravelData vrpPath =
              VrpPaths.createPath(
                  stayOrStopTask.getLink(),
                  request.getFromLink(),
                  stayOrStopTask.getEndTime(),
                  detourData.detourToPickup,
                  travelTime);
          // we may want to wait before or after driving
          beforePickupTask =
              insertDriveWithWait(
                  vehicleEntry.vehicle, stayOrStopTask, vrpPath, request.getEarliestStartTime());
        }
      }
    }
    ...
    double stopEndTime =
        stopTimeCalculator.initEndTimeForPickup(
            vehicleEntry.vehicle, beforePickupTask.getEndTime(), request.getRequest());
    DrtStopTask pickupStopTask =
        taskFactory.createStopTask(
            vehicleEntry.vehicle,
            beforePickupTask.getEndTime(),
            stopEndTime,
            request.getFromLink());
    ...
    Link toLink =
        pickupIdx == dropoffIdx
            ? request.getToLink() // pickup->dropoff
            : stops.get(pickupIdx).task.getLink(); // pickup->i+1

    double nextBeginTime =
        pickupIdx == dropoffIdx
            ? //
            pickupStopTask.getEndTime()
            : // asap
            stops.get(pickupIdx).task.getBeginTime(); // as planned

    if (request.getFromLink() == toLink) {
      // prebooking case when we are already at the stop location, but next stop task happens in the
      // future
      Task afterPickupTask = insertWait(vehicleEntry.vehicle, pickupStopTask, nextBeginTime);

      scheduleTimingUpdater.updateTimingsStartingFromTaskIdx(
          vehicleEntry.vehicle, afterPickupTask.getTaskIdx() + 1, afterPickupTask.getEndTime());
    } else {
      VrpPathWithTravelData vrpPath =
          VrpPaths.createPath(
              request.getFromLink(),
      ...
      Task afterPickupTask =
          insertDriveWithWait(vehicleEntry.vehicle, pickupStopTask, vrpPath, nextBeginTime);
  ...
  private DrtStopTask insertDropoff(
      AcceptedDrtRequest request,
      InsertionWithDetourData insertionWithDetourData,
    ...
    if (pickupIdx == dropoffIdx) { // no drive to dropoff
      ...
      if (request.getToLink() == stopTask.getLink()) { // no detour; no new stop task
        // add dropoff request to stop task, and extend the stop task (when incremental stop task
        // duration is used)
        stopTask.addDropoffRequest(request);

        // potentially extend task
        stopTask.setEndTime(
            stopTimeCalculator.updateEndTimeForDropoff(
                vehicleEntry.vehicle, stopTask, now, request.getRequest()));
        scheduleTimingUpdater.updateTimingsStartingFromTaskIdx(
            vehicleEntry.vehicle, stopTask.getTaskIdx() + 1, stopTask.getEndTime());
        ...
        VrpPathWithTravelData vrpPath =
            VrpPaths.createPath(
                ...
                request.getToLink(),
                stopTask.getEndTime(),
                detourData.detourToDropoff,
                travelTime);
        // direct drive to dropoff (no waiting)
        driveToDropoffTask =
            taskFactory.createDriveTask(vehicleEntry.vehicle, vrpPath, DrtDriveTask.TYPE);
    ...
    double stopEndTime =
        stopTimeCalculator.initEndTimeForDropoff(
            vehicleEntry.vehicle, startTime, request.getRequest());
    DrtStopTask dropoffStopTask =
        taskFactory.createStopTask(
            vehicleEntry.vehicle, startTime, stopEndTime, request.getToLink());
    ...
    if (dropoffIdx == stops.size()) { // bus stays at dropoff
      ...
      if (taskIdx + 1 == schedule.getTaskCount()) {
        ...
        double endTime =
            Math.max(dropoffStopTask.getEndTime(), vehicleEntry.vehicle.getServiceEndTime());
        schedule.addTask(
            taskFactory.createStayTask(
                vehicleEntry.vehicle,
                dropoffStopTask.getEndTime(),
                endTime,
      ...
      VrpPathWithTravelData vrpPath =
          VrpPaths.createPath(
              ...
              toLink,
              dropoffStopTask.getEndTime(),
              detourData.detourFromDropoff,
              travelTime);

      if (toLink == request.getToLink()) {
        // prebooking case: we stay, but may add some wait time until the next stop
        Task afterDropoffTask =
            insertWait(
                vehicleEntry.vehicle, dropoffStopTask, stops.get(dropoffIdx).task.getBeginTime());

        scheduleTimingUpdater.updateTimingsStartingFromTaskIdx(
            vehicleEntry.vehicle, afterDropoffTask.getTaskIdx() + 1, afterDropoffTask.getEndTime());
      } else {
  ...
  private Task insertWait(DvrpVehicle vehicle, Task departureTask, double earliestNextStartTime) {
    ...
    if (departureTask.getEndTime() < earliestNextStartTime) {
      DrtStayTask waitTask =
          taskFactory.createStayTask(
              vehicle, departureTask.getEndTime(), earliestNextStartTime, waitLink);
  ...
  private Task insertDriveWithWait(
    ...
    if (scheduleWaitBeforeDrive) {
      ...
      if (driveDepartureTime > departureTask.getEndTime()) {
        ...
        DrtStayTask waitTask =
            taskFactory.createStayTask(
                vehicle, departureTask.getEndTime(), driveDepartureTime, path.getFromLink());
        schedule.addTask(departureTask.getTaskIdx() + 1, waitTask);
    ...
    if (driveTask.getEndTime() < latestArrivalTime) {
      DrtStayTask waitTask =
          taskFactory.createStayTask(
              vehicle, driveTask.getEndTime(), latestArrivalTime, path.getToLink());
