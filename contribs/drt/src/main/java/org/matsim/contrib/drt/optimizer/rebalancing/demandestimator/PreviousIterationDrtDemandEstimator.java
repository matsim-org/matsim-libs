...
/**
 * Aggregates PersonDepartureEvents per iteration for the given mode and returns the numbers from
 * the previous iteration as expected demand for the current iteration.
 *
 * @author jbischoff
 * @author michalm
 */
public final class PreviousIterationDrtDemandEstimator
  ...
  private static final Logger logger =
      LogManager.getLogger(PreviousIterationDrtDemandEstimator.class);
  ...
  public PreviousIterationDrtDemandEstimator(
      DrtZonalSystem zonalSystem, DrtConfigGroup drtCfg, int demandEstimationPeriod) {
  ...
  public void handleEvent(PersonDepartureEvent event) {
    if (event.getLegMode().equals(mode)) {
      ...
      if (zone == null) {
        // might be that somebody walks into the service area or that service area is
        // larger/different than DrtZonalSystem...
  ...
  public ToDoubleFunction<DrtZone> getExpectedDemand(double fromTime, double estimationPeriod) {
    ...
    Map<DrtZone, MutableInt> expectedDemandForTimeBin =
        previousIterationDepartures.getOrDefault(timeBin, Map.of());
