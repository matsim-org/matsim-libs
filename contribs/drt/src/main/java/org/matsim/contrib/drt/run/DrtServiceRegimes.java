...
/**
 * Runtime view on the time-dependent service regimes of one DRT service: which regime is active at a given time
 ...
 * ({@code DrtServiceRegimeRequestValidator}).
 *
 * @author nkuehnel / MOIA
 */
public final class DrtServiceRegimes {

	/**
	 * One service regime with its stops resolved. The time window is half-open, i.e. {@code [startTime,
	 * endTime)}.
	 *
	...
	public DrtServiceRegimes(DrtServiceRegimesParams params, DrtStopNetwork stopNetwork) {
		this.regimes = params.getServiceRegimes()
				.stream()
				.map(serviceRegime -> createRegime(serviceRegime, stopNetwork))
				.toList();
	}

	private static Regime createRegime(DrtServiceRegimeParams serviceRegime, DrtStopNetwork stopNetwork) {
		Collection<DrtStopFacility> stops = stopNetwork.getDrtStops()
				.values()
				.stream()
				.filter(stop -> serviceRegime.getStopNetwork() == null
						|| AttributeBasedStopFinder.parseStopNetworks(stop)
						.contains(serviceRegime.getStopNetwork()))
				.collect(ImmutableList.toImmutableList());
		if (serviceRegime.getStopNetwork() != null) {
			// otherwise the service regime would silently serve nobody
			Verify.verify(!stops.isEmpty(),
					"No stop belongs to the stop network '%s' of %s '%s'. Check the '%s' attribute of the stops or the"
							+ " service areas they are derived from.", serviceRegime.getStopNetwork(),
		...
		return new Regime(serviceRegime.getServiceRegimeName(), serviceRegime.getStartTime(),
				serviceRegime.getEndTime(), stops, linkIds);
	}

	/**
	 * @return the service regime whose half-open time window covers the given time, or an empty
	 * {@link Optional} if the service is not available then. The windows are pairwise disjoint (checked in
	 * {@link DrtServiceRegimesParams#checkConsistency}), so there is at most one match.
	 */
