...
public class DrtAnalysisControlerListener implements IterationEndsListener, ShutdownListener {
  ...
  DrtAnalysisControlerListener(
      ...
      DrtVehicleDistanceStats drtVehicleStats,
  ...
  private static List<DrtLeg> newDrtLegs(
      EventSequence sequence, Function<Id<Link>, ? extends Link> linkProvider) {
    ...
    // PersonMoneyEvent has negative amount because the agent's money is reduced -> for the operator
    // that is a positive amount
    ...
    for (Id<Person> person : submittedEvent.getPersonIds()) {
      ...
      legs.add(
          new DrtLeg(
              ...
              toCoord,
              waitTime,
              unsharedDistanceEstimate_m,
              unsharedTimeEstimate_m,
              arrivalTime,
              fare,
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    writeAndPlotWaitTimeEstimateComparison(
        drtEventSequenceCollector.getPerformedRequestSequences().values(),
        filename(event, "waitTimeComparison", ".png"),
    ...
    List<PassengerRequestRejectedEvent> rejectionEvents =
        drtEventSequenceCollector.getRejectedRequestSequences().values().stream()
            .map(eventSequence -> eventSequence.getRejected().get())
            .sorted(Comparator.comparing(rejectionEvent -> rejectionEvent.getTime()))
            .collect(toList());

    collection2Text(
        drtEventSequenceCollector.getRejectedRequestSequences().values(),
        ...
        seq -> {
          ...
          Coord fromCoord =
              network.getLinks().get(submission.getFromLinkId()).getToNode().getCoord();
          ...
          return String.join(
              delimiter,
              submission.getTime() + "", //
              submission.getPersonIds().stream()
                      .map(Object::toString)
    ...
    double rejectionRate =
        (double) drtEventSequenceCollector.getRejectedRequestSequences().size()
            / drtEventSequenceCollector.getRequestSubmissions().size();
    String legsSummarize =
        summarizeLegs(
            legs,
            drtVehicleStats.getTravelDistances(),
    ...
    writeIterationPassengerStats(
        legsSummarize
            ...
            + delimiter
            + format.format(rejectionRate),
        event.getIteration());
    double l_d =
        getTotalDistance(drtVehicleStats.getVehicleStates()) / (legs.size() * directDistanceMean);

    MinCountAndShareIdleVehiclesOverDay minCountAndShareIdleVehiclesOverDay =
        getMinCountAndShareIdleVehiclesOverDay();
    String vehStats =
        summarizeVehicles(drtVehicleStats.getVehicleStates(), delimiter)
            + delimiter
            + format.format(l_d)
            + delimiter
    ...
    String occStats =
        summarizeDetailedOccupancyStats(drtVehicleStats.getVehicleStates(), delimiter, maxcap);
    ...
    writeVehicleDistances(
        drtVehicleStats.getVehicleStates(),
    ...
    analyzeBoardingsAndDeboardings(
        legs,
        delimiter,
        qSimCfg.getStartTime().orElse(0),
        endTime,
        3600,
        filename(event, "drt_boardings", ".csv"),
        filename(event, "drt_alightments", ".csv"),
        network);
  }

  private static double getTotalDistance(
      Map<Id<Vehicle>, DrtVehicleDistanceStats.VehicleState> vehicleDistances) {
  ...
  private record MinCountAndShareIdleVehiclesOverDay(
      double minCountIdleVehiclesOverDay, double minShareIdleVehiclesOverDay) {}
  ...
  private MinCountAndShareIdleVehiclesOverDay getMinCountAndShareIdleVehiclesOverDay() {
    ...
    double[] stayTaskProfile =
        vehicleOccupancyProfileCalculator
            .getNonPassengerServingTaskProfiles()
            .get(DrtStayTask.TYPE);
    double[] numberOfVehiclesInServiceProfile =
        vehicleOccupancyProfileCalculator.getNumberOfVehiclesInServiceProfile();
    if (stayTaskProfile != null) {
      for (int i = 0; i < numberOfVehiclesInServiceProfile.length; i++) {
        if (numberOfVehiclesInServiceProfile[i] > 0) {
          // only consider time intervals in which vehicles were in operation. Otherwise, any time
          // period without a vehicle in operation will make for count 0.
          minCountIdleVehiclesOverDay = Math.min(minCountIdleVehiclesOverDay, stayTaskProfile[i]);
          minShareIdleVehiclesOverDay =
              Math.min(
                  minShareIdleVehiclesOverDay,
                  stayTaskProfile[i] / numberOfVehiclesInServiceProfile[i]);
        }
      }
    }
      ...
      if (!headerWritten) {
        ...
        bw.write(
            line(
                ...
                "wait_p75",
                "wait_median",
                "percentage_WT_below_10",
                "percentage_WT_below_15",
                "inVehicleTravelTime_mean",
                "distance_m_mean",
                "directDistance_m_mean",
                "totalTravelTime_mean",
                "fareAllReferences_mean",
                "rejections",
                "rejectionRate"));
      }
      ...
      if (!vheaderWritten) {
        bw.write(
            line(
                ...
                "averagePassengerDistanceTraveled",
                "d_p/d_t",
                "l_det",
                "minShareIdleVehicles",
                "minCountIdleVehicles"));
      }
  ...
  private void writeAndPlotWaitTimeEstimateComparison(
      Collection<EventSequence> performedRequestEventSequences,
      String plotFileName,
      String textFileName,
      boolean createChart) {
    try (var bw = IOUtils.getBufferedWriter(textFileName)) {
          ...
          if (seq.getPersonEvents().containsKey(person)) {
            ...
            if (personEvents.getPickedUp().isPresent() && personEvents.getDeparture().isPresent()) {
              double actualWaitTime =
                  personEvents.getPickedUp().get().getTime()
                      - personEvents.getDeparture().get().getTime();
              double estimatedWaitTime =
                  seq.getScheduled().get().getPickupTime()
                      - seq.getSubmitted().getEarliestDepartureTime();
              bw.append(
                  line(
                      seq.getSubmitted().getRequestId(),
                      actualWaitTime,
                      estimatedWaitTime,
      ...
      if (createChart) {
        final JFreeChart chart2 =
            DensityScatterPlots.createPlot(
                "Wait times",
                "Actual wait time [s]",
                "Initially planned wait time [s]",
                times,
                Pair.of(0., drtCfg.maxWaitTime));
  ...
  private BufferedWriter getAppendingBufferedWriter(String prefix, String extension) {
    return IOUtils.getAppendingBufferedWriter(
        matsimServices
            .getControlerIO()
            .getOutputFilename(prefix + "_" + drtCfg.getMode() + extension));
  }
  ...
  public void notifyShutdown(ShutdownEvent event) {
    ...
    dumpOutput(
        event.getIteration(),
        "occupancy_time_profiles" + "_" + TimeProfileCharts.ChartType.Line,
        ".png");
    dumpOutput(
        event.getIteration(),
        "occupancy_time_profiles" + "_" + TimeProfileCharts.ChartType.StackedArea,
        ".png");
    dumpOutput(event.getIteration(), "task_time_profiles", ".txt");
    dumpOutput(
        event.getIteration(),
        "task_time_profiles" + "_" + TimeProfileCharts.ChartType.Line,
        ".png");
    dumpOutput(
        event.getIteration(),
        "task_time_profiles" + "_" + TimeProfileCharts.ChartType.StackedArea,
        ".png");
  }
  ...
  private static Map<Double, List<DrtLeg>> splitLegsIntoBins(
      Collection<DrtLeg> legs, int startTime, int endTime, int binSize_s) {
    LinkedList<DrtLeg> allLegs = new LinkedList<>(legs);
    DrtLeg currentLeg = allLegs.pollFirst();
    if (currentLeg.departureTime > endTime) {
      LogManager.getLogger(DrtAnalysisControlerListener.class)
          .error("wrong end / start Times for analysis");
    }
  ...
  private static Map<Double, List<PassengerRequestRejectedEvent>> splitEventsIntoBins(
      List<PassengerRequestRejectedEvent> rejectionEvents, int binSize_s) {
    ...
    int endTime =
        ((int) (rejectionEvents.get(rejectionEvents.size() - 1).getTime() / binSize_s) + 1)
            * binSize_s;
  ...
  private static void analyzeBoardingsAndDeboardings(
      List<DrtLeg> legs,
      String delimiter,
      double startTime,
      double endTime,
      double timeBinSize,
      String boardingsFile,
      String deboardingsFile,
      Network network) {
  ...
  private static void writeBoardings(
      String filename,
      Network network,
      Map<Id<Link>, int[]> boardings,
      double startTime,
      double timeBinSize,
      int bins,
      String delimiter) {
  ...
  private static String summarizeLegs(
      List<DrtLeg> legs,
      Map<Id<Request>, Double> travelDistances,
      List<PersonMoneyEvent> drtFarePersonMoneyEvents,
      String delimiter) {
    ...
    return String.join(
        delimiter,
        format.format(waitStats.getValues().length) + "", //
        ...
        // PersonMoneyEvent has negative amount because the agent's money is reduced -> for the
        // operator that is a positive amount
        format.format(
            -drtFarePersonMoneyEvents.stream().mapToDouble(PersonMoneyEvent::getAmount).sum()
  ...
  private static void analyseDetours(
      Network network,
      List<DrtLeg> legs,
      Map<Id<Request>, Double> travelDistances,
      DrtConfigGroup drtCfg,
    ...
    for (DrtLeg leg : legs) {
      ...
      if (createGraphs) {
        ...
        rideTimes.add(
            leg.arrivalTime - leg.departureTime - leg.waitTime, leg.unsharedTimeEstimate_m);
      }
  ...
  private static void analyseWaitTimes(
      String fileName, List<DrtLeg> legs, int binsize_s, boolean createGraphs, String delimiter) {
    if (legs.size() == 0) return;
    int startTime = ((int) (legs.get(0).departureTime / binsize_s)) * binsize_s;
    int endTime =
        ((int) (legs.get(legs.size() - 1).departureTime / binsize_s) + binsize_s) * binsize_s;
    ...
    try {
      ...
      if (createGraphs) {
        ...
        JFreeChart chart =
            chartProfile(splitLegs.size(), dataset, "Waiting times", "Wait time (s)");
        JFreeChart chart2 =
            chartProfile(
                splitLegs.size(),
                datasetrequ,
  ...
  private static void analyseRejections(
      String fileName,
      List<PassengerRequestRejectedEvent> rejectionEvents,
      int binsize_s,
      boolean createGraphs,
      String delimiter) {
    if (rejectionEvents.size() == 0) return;

    Map<Double, List<PassengerRequestRejectedEvent>> splitEvents =
        splitEventsIntoBins(rejectionEvents, binsize_s);
    ...
    try {
      ...
      if (createGraphs) {
        ...
        JFreeChart chart =
            chartProfile(splitEvents.size(), dataset, "Number of rejections", "Number");
        ChartSaveUtils.saveAsPNG(chart, fileName, 1500, 1000);
      }
  ...
  private static JFreeChart chartProfile(
      int length, TimeSeriesCollection dataset, String descriptor, String yax) {
  ...
  private static <T> void collection2Text(
      Collection<T> c, String filename, String header, Function<T, String> toStringFunction) {
    BufferedWriter bw = IOUtils.getBufferedWriter(filename);
    try {
      if (header != null) {
        bw.write(header);
   ...
   */
  private static void writeVehicleDistances(
      Map<Id<Vehicle>, DrtVehicleDistanceStats.VehicleState> vehicleDistances,
    ...
    try {
      bw.write(header);
      bw.newLine();
      for (Map.Entry<Id<Vehicle>, DrtVehicleDistanceStats.VehicleState> e :
          vehicleDistances.entrySet()) {
   ...
   * @return
   */
  private static String summarizeVehicles(
      Map<Id<Vehicle>, DrtVehicleDistanceStats.VehicleState> vehicleDistances, String del) {
    DecimalFormat format = new DecimalFormat();
    format.setDecimalFormatSymbols(new DecimalFormatSymbols(Locale.US));
    format.setMinimumIntegerDigits(1);
    format.setMaximumFractionDigits(2);
  ...
  static int findMaxVehicleCapacity(FleetSpecification fleet) {
    return fleet.getVehicleSpecifications().values().stream()
  ...
  private static void analyseConstraints(String fileName, List<DrtLeg> legs, boolean createGraphs) {
    ...
    final JFreeChart chart =
        DensityScatterPlots.createPlot(
            "Maximum wait time", "Maximum wait time [s]", "Actual wait time [s]", waitingTimes);
    ChartSaveUtils.saveAsPNG(chart, fileName + "_waiting_time", 1500, 1500);

    final JFreeChart chart2 =
        DensityScatterPlots.createPlot(
            "Maximum travel time",
            "Maximum travel time [s]",
            "Actual travel time [s]",
            travelTimes);
