...
class ExtensiveInsertionProvider {
  static ExtensiveInsertionProvider create(
    ...
    var admissibleTimeEstimator =
        DetourTimeEstimator.createMatrixBasedEstimator(
            insertionParams.admissibleBeelineSpeedFactor, travelTimeMatrix, travelTime);
    return new ExtensiveInsertionProvider(
        (ExtensiveInsertionSearchParams) drtCfg.getDrtInsertionSearchParams(),
        insertionCostCalculator,
        new InsertionGenerator(stopTimeCalculator, admissibleTimeEstimator),
        forkJoinPool);
  }
  ...
  @VisibleForTesting
  ExtensiveInsertionProvider(
      ExtensiveInsertionSearchParams insertionParams,
      InsertionCostCalculator admissibleCostCalculator,
  ...
  List<Insertion> getInsertions(DrtRequest drtRequest, Collection<VehicleEntry> vehicleEntries) {
    ...
    List<InsertionWithDetourData> preFilteredInsertions =
        forkJoinPool
            .submit(
                () ->
                    vehicleEntries.parallelStream()
                        // generate feasible insertions (wrt occupancy limits) with admissible
                        // detour times
                        .flatMap(e -> insertionGenerator.generateInsertions(drtRequest, e).stream())
                        // optimistic pre-filtering wrt admissible cost function
                        .filter(
                            i ->
                                admissibleCostCalculator.calculate(
                                        drtRequest, i.insertion, i.detourTimeInfo)
    ...
    return KNearestInsertionsAtEndFilter.filterInsertionsAtEnd(
        insertionParams.nearestInsertionsAtEndLimit, preFilteredInsertions);
  }
}
