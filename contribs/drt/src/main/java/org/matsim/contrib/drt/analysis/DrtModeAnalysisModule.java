...
public class DrtModeAnalysisModule extends AbstractDvrpModeModule {
  ...
  private ImmutableSet<Task.TaskType> passengerServingTaskTypes =
      ImmutableSet.of(DrtDriveTask.TYPE, DefaultDrtStopTask.TYPE);
  ...
  private static final Map<Task.TaskType, Paint> taskTypePaints =
      ImmutableMap.of(DrtStayTask.TYPE, Color.LIGHT_GRAY);
  ...
  public DrtModeAnalysisModule(
      DrtConfigGroup drtCfg, ImmutableSet<Task.TaskType> passengerServingTaskTypes) {
    super(drtCfg.getMode());
    this.drtCfg = drtCfg;
  ...
  public void install() {
    ...
    bindModal(DrtVehicleDistanceStats.class)
        .toProvider(
            modalProvider(
                getter ->
                    new DrtVehicleDistanceStats(
    ...
    bindModal(DrtEventSequenceCollector.class)
        .toProvider(modalProvider(getter -> new DrtEventSequenceCollector(drtCfg.getMode())))
        .asEagerSingleton();
            ...
            modalProvider(
                getter -> {
                  ...
                  return new ProfileWriter(
                      matsimServices, mode, profileView, "occupancy_time_profiles");
                }));
    ...
    addControlerListenerBinding()
        .toProvider(
            modalProvider(
                getter ->
                    new DrtAnalysisControlerListener(
