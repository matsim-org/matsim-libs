...
public class NetDepartureReplenishDemandEstimator
  ...
  public NetDepartureReplenishDemandEstimator(
  ...
  public void handleEvent(DrtRequestSubmittedEvent event) {
    if (event.getMode().equals(mode)) {
      ...
      potentialDrtTripsMap.put(
          event.getRequestId(), new Trip(timeBin, departureZoneId, arrivalZoneId));
    }
  }
  ...
  public void handleEvent(PassengerRequestScheduledEvent event) {
    if (event.getMode().equals(mode)) {
      ...
      var zoneNetDepartureMapSlice =
          currentZoneNetDepartureMap.computeIfAbsent(trip.timeBin, t -> new HashMap<>());
  ...
  public ToDoubleFunction<DrtZone> getExpectedDemandForTimeBin(int timeBin) {
    var expectedDemandForTimeBin =
        previousZoneNetDepartureMap.getOrDefault(timeBin, Collections.emptyMap());
