...
public class DrtGridUtils {
  ...
  public static Map<String, PreparedGeometry> createGridFromNetwork(
      Network network, double cellsize) {
  ...
  /**
   * First creates a grid based on the network bounding box. Then removes all zones that do not
   * intersect the service area. Result may contain zones that are barely included in the service
   * area. But as passengers may walk into the service area, it seems appropriate that the
   * DrtZonalSystem, which is used for demand estimation, is larger than the service area. The
   * {@code cellsize} indirectly determines, how much larger the DrtZonalSystem may get.
   *
   * @param network
   * @param cellsize
   ...
   * @return
   */
  public static Map<String, PreparedGeometry> createGridFromNetworkWithinServiceArea(
    ...
    Map<String, PreparedGeometry> zonesWithinServiceArea =
        EntryStream.of(grid)
            .peekKeys(id -> counter.incCounter())
            .filterValues(
                cell ->
                    serviceAreaGeoms.stream()
