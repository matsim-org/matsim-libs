...
/**
 * This class proposes a first implementation of the {@link CapacityReconfigurationLogic}
 * The logic behind this implementation is as follows:
 * - The capacities compatible with requests are tracked in the previous iteration. Building A list containing the number of requests per capacity per time slot.
 * - The Capacity changes of the fleet are planned such as the distribution of vehicles with each capacity during the day matches the distribution of capacities required by requests.
 * - If configured to do so, the logic starts by overriding the default vehicle capacities at the beginning of the day before inserting capacity changes
 * - Moreover, to select the capacity change locations, we use the {@link DefaultCapacityReconfigurationLogic.CapacityChangeLinkSelection} parameter to choose either a random selection or the location closest to the vehicle's initial position
 * @author Tarek Chouaki (tkchouaki)
 */
public class DefaultCapacityReconfigurationLogic implements CapacityReconfigurationLogic, DrtRequestSubmittedEventHandler, PassengerRequestRejectedEventHandler {
	...
	public DefaultCapacityReconfigurationLogic(FleetSpecification fleetSpecification, Set<DvrpLoad> possibleCapacities, DvrpLoadFromDrtPassengers dvrpLoadFromDrtPassengers, Network network, Collection<? extends Link> links, int reconfigurationInterval) {
	...
	public DefaultCapacityReconfigurationLogic(FleetSpecification fleetSpecification, Set<DvrpLoad> possibleCapacities, DvrpLoadFromDrtPassengers dvrpLoadFromDrtPassengers, Network network, Collection<? extends Link> links, int reconfigurationInterval, boolean allowCapacityChangeBeforeDayStarts, CapacityChangeLinkSelection capacityChangeLinkSelection) {
		...
		if(this.capacityChangeLinkSelection.equals(CapacityChangeLinkSelection.ClosestToVehicleStartLocation)) {
			...
			this.fleetSpecification.getVehicleSpecifications()
				.forEach((id, specification) -> {
					Link startingLink = Objects.requireNonNull(network.getLinks().get(specification.getStartLinkId()));
		...
		for(DvrpVehicleSpecification vehicleSpecification: fleetSpecification.getVehicleSpecifications().values()) {
			...
			for(int slotIndex=timeToSlotIndex(vehicleSpecification.getServiceBeginTime()); slotIndex < timeToSlotIndex(vehicleSpecification.getServiceEndTime()); slotIndex++) {
				this.vehiclesActivePerTimeSlot.computeIfAbsent(slotIndex, key -> new ArrayList<>()).add(vehicleSpecification.getId());
			}
	...
	private DvrpLoad getFittingCapacity(DvrpLoad load) {
		...
		throw new IllegalArgumentException("Found a request that cannot be handled by one of the registered capacities");
	}
	...
	public void handleEvent(DrtRequestSubmittedEvent event) {
		...
		if(timeSlot < this.totalRequestsPerTimeSlot.length) {
			...
			requestsPerTimeSlotPerDvrpLoad[timeSlot].put(load, requestsPerTimeSlotPerDvrpLoad[timeSlot].getOrDefault(load, 0)+1);
	...
	private void updateCapacityPlanning() {
		...
		this.fleetSpecification.getVehicleSpecifications().keySet().forEach(vehicleId -> capacityChangesPerVehicle.put(vehicleId, 0));
		...
		for(int timeSlotIndex=0; timeSlotIndex<this.totalRequestsPerTimeSlot.length; timeSlotIndex++) {
			if(totalRequestsPerTimeSlot[timeSlotIndex] > 0) {
				...
				for(Map.Entry<DvrpLoad, Integer> entry: this.requestsPerTimeSlotPerDvrpLoad[timeSlotIndex].entrySet()) {
					DvrpLoad load = entry.getKey();
					double requests = entry.getValue();
					target.put(load, (int) (requests / totalRequestsPerTimeSlot[timeSlotIndex] * fleetSpecification.getVehicleSpecifications().size()));
				...
				for(DvrpLoad capacity: this.possibleCapacities) {
					...
					while(delta.getOrDefault(capacity, 0) > 0) {
						...
						if(vehicleToTransfer != null) {
							...
							capacityChangesPerVehicle.put(vehicleToTransfer, capacityChangesPerVehicle.getOrDefault(vehicleToTransfer, 0)+1);
	...
	public IdMap<DvrpVehicle, DvrpLoad> getOverriddenStartingCapacities() {
		return this.overriddenStartingCapacities;
	}

	@Override
	public List<CapacityChangeItem> getPreScheduledCapacityChanges(DvrpVehicle dvrpVehicle) {
		List<CapacityChangeItem> capacityChangeItems = new ArrayList<>();
		for(int timeSlotIndex = 0; timeSlotIndex<this.capacityReconfigurations.length; timeSlotIndex++) {
			...
			if(currentReconfigurations.containsKey(dvrpVehicle.getId())) {
				capacityChangeItems.add(new CapacityChangeItem(timeSlotIndexToTime(timeSlotIndex), getCapacityChangeLinkId(dvrpVehicle), currentReconfigurations.get(dvrpVehicle.getId())));
			}
		}
