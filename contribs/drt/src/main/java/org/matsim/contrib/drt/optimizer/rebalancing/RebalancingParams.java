...
public final class RebalancingParams extends ReflectiveConfigGroupWithConfigurableParameterSets {
  ...
  @Parameter
  @Comment(
      "Specifies how often empty vehicle rebalancing is executed."
  ...
  @PositiveOrZero
  public double maxTimeBeforeIdle =
      0.5 * interval; // [s], if 0 then soon-idle vehicle will not be considered

  public interface RebalancingStrategyParams {}

  @NotNull private RebalancingStrategyParams rebalancingStrategyParams;
  ...
  private void initSingletonParameterSets() {
    ...
    addDefinition(
        MinCostFlowRebalancingStrategyParams.SET_NAME,
        MinCostFlowRebalancingStrategyParams::new,
        () -> (ConfigGroup) rebalancingStrategyParams,
        params -> rebalancingStrategyParams = (RebalancingStrategyParams) params);
    addDefinition(
        FeedforwardRebalancingStrategyParams.SET_NAME,
        FeedforwardRebalancingStrategyParams::new,
        () -> (ConfigGroup) rebalancingStrategyParams,
        params -> rebalancingStrategyParams = (RebalancingStrategyParams) params);
    addDefinition(
        PlusOneRebalancingStrategyParams.SET_NAME,
        PlusOneRebalancingStrategyParams::new,
        () -> (ConfigGroup) rebalancingStrategyParams,
        params -> rebalancingStrategyParams = (RebalancingStrategyParams) params);
    addDefinition(
        CustomRebalancingStrategyParams.SET_NAME,
        CustomRebalancingStrategyParams::new,
        () -> (ConfigGroup) rebalancingStrategyParams,
        params -> rebalancingStrategyParams = (RebalancingStrategyParams) params);
  }
  ...
  protected void checkConsistency(Config config) {
    ...
    Preconditions.checkArgument(
        minServiceTime > maxTimeBeforeIdle,
