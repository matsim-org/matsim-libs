...
/**
 * This is a prebooking logic that detects a person-level attribute which indicates whether requests
 * to a certain DRT mode for that person are prebooked. It can be installed by calling
 *
 * <p>PersonBasedPrebookingLogic.install
 *
 * <p>on the controller with the respective DRT mode and a value indicating the prebooking slack,
 * i.e., the time between the expected departure and when the request is submitted.
 */
public class PersonBasedPrebookingLogic implements PrebookingLogic, MobsimInitializedListener {
  private static final String ATTRIBUTE_PREFIX = "prebooking:";

  public static String getPersonAttribute(String mode) {
    return ATTRIBUTE_PREFIX + mode;
  }

  public static String getPersonAttribute(DrtConfigGroup drtConfig) {
    return getPersonAttribute(drtConfig.getMode());
  }

  public static boolean isPrebooked(String mode, Person person) {
  ...
  private PersonBasedPrebookingLogic(
      String mode,
      PrebookingQueue prebookingQueue,
      PopulationIteratorFactory populationIteratorFactory,
        ...
        for (PlanElement element : personItem.plan().getPlanElements()) {
          if (element instanceof Leg) {
            ...
            if (leg.getMode().equals(mode)) {
              ...
              prebookingQueue.schedule(
                  submissionTime, personItem.agent(), leg, earliestDepartureTime);
            }
          }
  ...
  public static AbstractDvrpModeQSimModule createModule(
      DrtConfigGroup drtConfig, double prebookingSlack) {
    return new AbstractDvrpModeQSimModule(drtConfig.getMode()) {
      @Override
      protected void configureQSim() {
        bindModal(PersonBasedPrebookingLogic.class)
            .toProvider(
                modalProvider(
                    getter -> {
                      Preconditions.checkState(drtConfig.getPrebookingParams().isPresent());

                      return new PersonBasedPrebookingLogic(
                          drtConfig.getMode(),
                          getter.getModal(PrebookingQueue.class),
                          getter.getModal(PopulationIteratorFactory.class),
  ...
  public static void install(
      Controler controller, DrtConfigGroup drtConfig, double prebookingSlack) {
