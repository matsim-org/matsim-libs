...
public class DefaultUnplannedRequestInserter implements UnplannedRequestInserter {
  ...
  public DefaultUnplannedRequestInserter(
  ...
  @VisibleForTesting
  DefaultUnplannedRequestInserter(
      String mode,
      Fleet fleet,
  ...
  private void scheduleUnplannedRequest(
      DrtRequest req, Map<Id<DvrpVehicle>, VehicleEntry> vehicleEntries, double now) {
    Optional<InsertionWithDetourData> best =
        insertionSearch.findBestInsertion(
            req, Collections.unmodifiableCollection(vehicleEntries.values()));
    if (best.isEmpty()) {
      if (!insertionRetryQueue.tryAddFailedRequest(req, now)) {
        eventsManager.processEvent(
            new PassengerRequestRejectedEvent(
                now, mode, req.getId(), req.getPassengerIds(), NO_INSERTION_FOUND_CAUSE));
        log.debug(
            "No insertion found for drt request "
                + req
                + " with passenger ids="
                + req.getPassengerIds().stream()
                    .map(Object::toString)
      ...
      var acceptedRequest =
          drtOfferAcceptor.acceptDrtOffer(
              req,
      ...
      var pickupDropoffTaskPair =
          insertionScheduler.scheduleRequest(acceptedRequest.get(), insertion);
      ...
      expectedPickupTime =
          Math.max(expectedPickupTime, acceptedRequest.get().getEarliestStartTime());
