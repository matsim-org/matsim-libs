...
/**
 * This strategy is based on the Plus One Rebalancing Algorithm in AMoDeus. At each rebalancing
 * period, the algorithm will send idling vehicles to the departure places of the request departed
 * during the time period
 *
 * @author Chengqi Lu
 */
public class PlusOneRebalancingStrategy
    implements RebalancingStrategy,
  ...
  public PlusOneRebalancingStrategy(
      String mode, Network network, LinkBasedRelocationCalculator linkBasedRelocationCalculator) {
    this.mode = mode;
  ...
  public List<Relocation> calcRelocations(
    ...
    // Because events can be received at the same time we calculate relocations, we want to only
    // consider target links from events that are older
    ...
    log.debug(
        "There are in total " + targetLinks.size() + " rebalance targets at this time period");
