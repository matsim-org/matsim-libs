...
public class StoppingTaskRecorder
    implements TaskEndedEventHandler,
  ...
  public void handleEvent(TaskStartedEvent taskStartedEvent) {
    ...
    if (DrtTaskBaseType.STAY.isBaseTypeOf(taskType)
        || DrtTaskBaseType.STOP.isBaseTypeOf(taskType)
      ...
      int occupancy =
          vehicleOccupancyTracker.computeIfAbsent(vehicleId, v -> new MutableInt()).intValue();

      startedTasks.put(
          vehicleId, new DrtTaskInformation(taskType.name(), linkId, time, vehicleId, occupancy));
    }
  }
