...
/**
 * This service helps to define a PrebookingLogic where at some point in time (usually at the
 * beginning of the simulaton), it is known in advance that a request will happen at a specific
 * time. Once we know that the request will be sent, we can use the present class to put it in a
 * queue, making sure the request will be *booked* the the time we want.
 *
...
public class PrebookingQueue implements MobsimBeforeSimStepListener {
  ...
  public void notifyMobsimBeforeSimStep(
      @SuppressWarnings("rawtypes") MobsimBeforeSimStepEvent event) {
   ...
   * MobsimInitializedEvent, otherwise this is called automatically per time step to see if there
   * are requests queued that need to be submitted.
   */
  ...
  private record ScheduledSubmission(
      double submissionTime, MobsimAgent agent, Leg leg, double departuretime, int sequence)
      implements Comparable<ScheduledSubmission> {
    @Override
