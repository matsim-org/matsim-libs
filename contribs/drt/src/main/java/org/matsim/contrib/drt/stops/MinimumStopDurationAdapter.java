...
public class MinimumStopDurationAdapter implements StopTimeCalculator {
  ...
  public double initEndTimeForPickup(DvrpVehicle vehicle, double beginTime, DrtRequest request) {
    return Math.max(
        beginTime + minimumStopDuration,
        delegate.initEndTimeForPickup(vehicle, beginTime, request));
  }
  ...
  public double updateEndTimeForPickup(
    ...
    return Math.max(
        stop.getBeginTime() + minimumStopDuration,
  ...
  public double initEndTimeForDropoff(DvrpVehicle vehicle, double beginTime, DrtRequest request) {
    return Math.max(
        beginTime + minimumStopDuration,
        delegate.initEndTimeForDropoff(vehicle, beginTime, request));
  }
  ...
  public double updateEndTimeForDropoff(
      DvrpVehicle vehicle, DrtStopTask stop, double insertionTime, DrtRequest request) {
    return Math.max(
        insertionTime + minimumStopDuration,
  ...
  public double shiftEndTime(DvrpVehicle vehicle, DrtStopTask stop, double beginTime) {
    return Math.max(
        beginTime + minimumStopDuration, delegate.shiftEndTime(vehicle, stop, beginTime));
  }
}
