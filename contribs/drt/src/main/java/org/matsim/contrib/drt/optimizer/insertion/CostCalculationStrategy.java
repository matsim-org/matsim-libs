...
public interface CostCalculationStrategy {
   ...
   */
  double calcCost(
      DrtRequest request, InsertionGenerator.Insertion insertion, DetourTimeInfo detourTimeInfo);
  ...
  class RejectSoftConstraintViolations implements CostCalculationStrategy {
    ...
    public double calcCost(
      ...
      if (detourTimeInfo.pickupDetourInfo.departureTime > request.getLatestStartTime()
        ...
        // no extra time is lost => do not check if the current slack time is long enough (can be
        // even negative)
  ...
  class DiscourageSoftConstraintViolations implements CostCalculationStrategy {
    ...
    static final double MAX_WAIT_TIME_VIOLATION_PENALTY =
        1; // 1 second of penalty per 1 second of late departure
    static final double MAX_TRAVEL_TIME_VIOLATION_PENALTY =
        10; // 10 seconds of penalty per 1 second of late arrival
    ...
    public double calcCost(
      ...
      double waitTimeViolation =
          Math.max(0, detourTimeInfo.pickupDetourInfo.departureTime - request.getLatestStartTime());
      // (if drt vehicle picks up too late) (max wait time (often 600 sec) after submission)

      double travelTimeViolation =
          Math.max(
              0, detourTimeInfo.dropoffDetourInfo.arrivalTime - request.getLatestArrivalTime());
