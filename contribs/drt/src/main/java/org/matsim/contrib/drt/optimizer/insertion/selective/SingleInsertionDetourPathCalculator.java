...
public class SingleInsertionDetourPathCalculator implements MobsimBeforeCleanupListener {
  ...
  public SingleInsertionDetourPathCalculator(
      Network network,
  ...
  @VisibleForTesting
  SingleInsertionDetourPathCalculator(
      Network network,
      TravelTime travelTime,
      TravelDisutility travelDisutility,
      int numberOfThreads,
    ...
    fromPickupPathSearch =
        pathCalculatorFactory.createPathCalculator(network, travelDisutility, travelTime);
    toDropoffPathSearch =
        pathCalculatorFactory.createPathCalculator(network, travelDisutility, travelTime);
    fromDropoffPathSearch =
        pathCalculatorFactory.createPathCalculator(network, travelDisutility, travelTime);
  ...
  public InsertionDetourData calculatePaths(DrtRequest drtRequest, Insertion insertion) {
    ...
    // TODO use times from InsertionWithDetourData<Double> as approximate departure times for
    // Dijkstra (will require
    //  passing it as an argument, instead of Insertion)

    Future<PathData> toPickupFuture =
        executorService.submit(
            () ->
                calcPathData(
                    toPickupPathSearch,
                    insertion.pickup.previousWaypoint.getLink(),
                    pickup,
                    earliestPickupTime));

    Future<PathData> fromPickupFuture =
        executorService.submit(
            () ->
                calcPathData(
                    fromPickupPathSearch,
                    pickup,
                    insertion.pickup.nextWaypoint.getLink(),
                    earliestPickupTime));

    Future<PathData> toDropoffFuture =
        insertion.dropoff.previousWaypoint instanceof Waypoint.Pickup
            ? Futures.immediateFuture(null)
            : executorService.submit(
                () ->
                    calcPathData(
                        toDropoffPathSearch,
                        insertion.dropoff.previousWaypoint.getLink(),
                        dropoff,
                        latestDropoffTime));

    Future<PathData> fromDropoffFuture =
        insertion.dropoff.nextWaypoint instanceof Waypoint.End
    ...
    try {
      return new InsertionDetourData(
          toPickupFuture.get(),
          fromPickupFuture.get(),
          toDropoffFuture.get(),
  ...
  private PathData calcPathData(
      LeastCostPathCalculator router, Link fromLink, Link toLink, double departureTime) {
    if (fromLink == toLink) {
    ...
    Path path =
        router.calcLeastCostPath(
            fromLink.getToNode(), toLink.getFromNode(), departureTime + FIRST_LINK_TT, null, null);
    double firstAndLastLinkTT =
        FIRST_LINK_TT
            + VrpPaths.getLastLinkTT(
                travelTime, toLink, departureTime + FIRST_LINK_TT + path.travelTime);
