...
/**
 * For prebooking, we implement an alternative DynActivity that handles entering / exiting
 * passengers more flexibly than the standard DrtStopActivity.
 *
 * <p>Specifically, we track when a person is ready for departure and then add the expected duration
 * of the interaction into a queue. The agent only actually enters the vehicle after this time has
 * elapsed.
 *
...
public class PrebookingActionCreator implements VrpAgentLogic.DynActionCreator {
  ...
  public PrebookingActionCreator(
  ...
  public DynAction createAction(DynAgent dynAgent, DvrpVehicle vehicle, double now) {
    ...
    if (getBaseTypeOrElseThrow(task).equals(DrtTaskBaseType.STOP)) {
      ...
      return new PrebookingStopActivity(
          passengerHandler,
          dynAgent,
          stopTask,
          stopTask.getDropoffRequests(),
          stopTask.getPickupRequests(),
          DrtActionCreator.DRT_STOP_NAME,
          () -> stopTask.getEndTime(),
