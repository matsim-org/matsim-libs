...
/**
 * Generates all possible pickup and dropoff insertion point pairs that do not violate the vehicle
 * capacity. In order to generate insertion points for a given vehicle, a {@code VehicleEntry} must
 * be prepared. It contains the information about the vehicle current state (location, time and
 * occupancy; additionally, the vehicle can be queried about the current task etc. ) and the
 * sequence of planned stops (of length {@code N}.
 *
 * <p>Pickup insertion points are indexed in the following way:
 *
 * <ul>
 *   <li>{@code pickupIdx == 0} - pickup inserted at/after the current position (e.g. ongoing stop,
 *       stay or drive, the latter resulting in immediate diversion)
 *   <li>{@code 0 < pickupIdx <= stops.size()} - pickup inserted at/after stop {@code pickupIdx - 1}
 * </ul>
 *
 * <p>Dropoff insertion points are indexed in the following way:
 *
 * <ul>
 *   <li>{@code dropoffIdx == pickupIdx} - dropoff inserted directly after pickup
 *   <li>{@code pickupIdx < dropoffIdx <= stops.size()} - dropoff inserted at/after stop {@code
 *       dropoffIdx - 1}
 * </ul>
 *
 * <p>A pickup/dropoff is inserted at (i.e. included into) a given stop if they are on the same
 * link. Otherwise, it is inserted after that stop. In the latter case, the sequence of stops is
 * changed from: <br>
 * {@code ... --> stop i --> stop i + 1 --> ...} <br>
 * to: <br>
 * {@code ... --> stop i --> new stop --> stop i + 1 --> ...}.
 *
 * <p>If a pickup/dropoff is inserted at stop {@code i} (pickup/dropoff is on the same link as the
 * stop), insertion after stop {@code i-1} will not be generated (as that would be equivalent
 * to/duplicate of the former one).
 *
 * @author michalm
 */
public class InsertionGenerator {
  public static class InsertionPoint {
    ...
    public InsertionPoint(
        int index, Waypoint previousWaypoint, Waypoint newWaypoint, Waypoint nextWaypoint) {
      this.index = index;
  ...
  private static InsertionPoint createPickupInsertion(
      DrtRequest request, VehicleEntry vehicleEntry, int pickupIdx, boolean followedByDropoff) {
    var pickupWaypoint = new Waypoint.Pickup(request);
    var pickupPreviousWaypoint = vehicleEntry.getWaypoint(pickupIdx);
    var pickupNextLink =
        followedByDropoff ? new Waypoint.Dropoff(request) : vehicleEntry.getWaypoint(pickupIdx + 1);
    return new InsertionPoint(pickupIdx, pickupPreviousWaypoint, pickupWaypoint, pickupNextLink);
  }

  private static InsertionPoint createDropoffInsertion(
      DrtRequest request, VehicleEntry vehicleEntry, InsertionPoint pickup, int dropoffIdx) {
  ...
  public InsertionGenerator(
      StopTimeCalculator stopTimeCalculator, DetourTimeEstimator detourTimeEstimator) {
    this.detourTimeEstimator = detourTimeEstimator;
    detourTimeCalculator =
        new InsertionDetourTimeCalculator(stopTimeCalculator, detourTimeEstimator);
  }

  public List<InsertionWithDetourData> generateInsertions(
      DrtRequest drtRequest, VehicleEntry vEntry) {
  ...
  private void generateDropoffInsertions(
      DrtRequest request, VehicleEntry vEntry, int i, List<InsertionWithDetourData> insertions) {
    ...
    double toPickupTT =
        detourTimeEstimator.estimateTime(
            pickupInsertion.previousWaypoint.getLink(),
            request.getFromLink(),
            toPickupDepartureTime);
    double earliestPickupStartTime =
        Math.max(toPickupDepartureTime + toPickupTT, request.getEarliestStartTime());
    double fromPickupTT =
        detourTimeEstimator.estimateTime(
            request.getFromLink(),
            pickupInsertion.nextWaypoint.getLink(),
            earliestPickupStartTime); // TODO stopDuration not included
    var pickupDetourInfo =
        detourTimeCalculator.calcPickupDetourInfo(
            vEntry, pickupInsertion, toPickupTT, fromPickupTT, true, request);
    ...
    if (vEntry.getSlackTime(i) >= pickupDetourInfo.pickupTimeLoss) {
      ...
      if (i == stopCount || request.getToLink() != nextStop(vEntry, j).task.getLink()) {
        ...
        // otherwise, do not evaluate insertion _before_stop j, evaluate only insertion _after_ stop
        // j
        addInsertion(
            insertions,
            createInsertionWithDetourData(
                request, vEntry, pickupInsertion, fromPickupTT, pickupDetourInfo, j));
      }
    }
    ...
    pickupDetourInfo =
        detourTimeCalculator.calcPickupDetourInfo(
            vEntry, pickupInsertion, toPickupTT, fromPickupTT, false, request);
    ...
    for (int j = i + 1; j < stopCount; j++) { // insertions up to before last stop
      ...
      if (currentStop.outgoingOccupancy + request.getPassengerCount()
          > vEntry.vehicle.getCapacity()) {
      ...
      if (request.getToLink()
          != nextStop(vEntry, j).task.getLink()) { // next stop at different link
    ...
    addInsertion(
        insertions,
        createInsertionWithDetourData(
            request, vEntry, pickupInsertion, fromPickupTT, pickupDetourInfo, stopCount));
  }

  private void addInsertion(
      List<InsertionWithDetourData> insertions, InsertionWithDetourData insertion) {
    if (insertion != null) {
  ...
  private boolean checkStartSlack(
      VehicleEntry vEntry, DrtRequest request, PickupDetourInfo pickupDetourInfo) {
  ...
  private InsertionWithDetourData createInsertionWithDetourData(
      DrtRequest request,
      VehicleEntry vehicleEntry,
      InsertionPoint pickupInsertion,
      double fromPickupTT,
      PickupDetourInfo pickupDetourInfo,
      int dropoffIdx) {
    var dropoffInsertion =
        createDropoffInsertion(request, vehicleEntry, pickupInsertion, dropoffIdx);
    var insertion = new Insertion(vehicleEntry, pickupInsertion, dropoffInsertion);

    double toDropoffDepartureTime =
        pickupInsertion.index == dropoffIdx
            ? pickupDetourInfo.departureTime
            : dropoffInsertion.previousWaypoint.getDepartureTime()
                + pickupDetourInfo.pickupTimeLoss;
    double toDropoffTT =
        pickupInsertion.index == dropoffIdx
            ? fromPickupTT
            : detourTimeEstimator.estimateTime(
                dropoffInsertion.previousWaypoint.getLink(),
                request.getToLink(),
                toDropoffDepartureTime);
    double fromDropoffTT =
        dropoffIdx == vehicleEntry.stops.size()
            ? 0
            : detourTimeEstimator.estimateTime(
                request.getToLink(),
                dropoffInsertion.nextWaypoint.getLink(),
                toDropoffDepartureTime + toDropoffTT); // TODO stopDuration not included

    var dropoffDetourInfo =
        detourTimeCalculator.calcDropoffDetourInfo(
            insertion, toDropoffTT, fromDropoffTT, pickupDetourInfo, request);
    ...
    return new InsertionWithDetourData(
        insertion, null, new DetourTimeInfo(pickupDetourInfo, dropoffDetourInfo));
  }
}
