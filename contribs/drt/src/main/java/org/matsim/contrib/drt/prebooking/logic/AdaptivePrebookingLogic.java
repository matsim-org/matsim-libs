...
/**
 * This is a prebooking logic that, whenever an agent starts an activity, checks whether there is a
 * DRT leg coming up before the next main (non-stage) activity. If so, the upcoming leg is prebooked
 * in advance with the submissionSlack parameter defining how much in advance.
 *
 * @author Sebastian Hörl (sebhoerl), IRT SystemX
 */
public class AdaptivePrebookingLogic
    implements PrebookingLogic, ActivityStartEventHandler, MobsimScopeEventHandler {
  ...
  private AdaptivePrebookingLogic(
      String mode,
      QSim qsim,
  ...
  public static AbstractDvrpModeQSimModule createModule(
      DrtConfigGroup drtConfig, double subissionSlack) {
    return new AbstractDvrpModeQSimModule(drtConfig.getMode()) {
      @Override
      protected void configureQSim() {
        bindModal(AdaptivePrebookingLogic.class)
            .toProvider(
                modalProvider(
                    getter -> {
                      Preconditions.checkState(drtConfig.getPrebookingParams().isPresent());

                      return new AdaptivePrebookingLogic(
                          drtConfig.getMode(),
                          getter.get(QSim.class),
  ...
  public static void install(
      Controler controller, DrtConfigGroup drtConfig, double subissionSlack) {
