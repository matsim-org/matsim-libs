...
public class DrtZone {
  ...
  @Nullable private final PreparedGeometry preparedGeometry; // null for virtual/dummy zones
  private final List<Link> links;
  private final Coord centroid;

  public DrtZone(String id, PreparedGeometry preparedGeometry, List<Link> links) {
    this(
        id, preparedGeometry, links, MGC.point2Coord(preparedGeometry.getGeometry().getCentroid()));
  }

  private DrtZone(
      String id, @Nullable PreparedGeometry preparedGeometry, List<Link> links, Coord centroid) {
    this.id = id;
