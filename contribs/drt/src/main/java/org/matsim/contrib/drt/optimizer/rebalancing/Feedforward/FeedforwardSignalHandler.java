...
public class FeedforwardSignalHandler implements IterationStartsListener {
  ...
  /** Constructor */
  public FeedforwardSignalHandler(
  ...
  private void calculateFeedforwardSignal() {
    ...
    for (int t = 0; t < numOfTimeBin; t++) {
      ToDoubleFunction<DrtZone> netDepartureInputFunction =
          netDepartureReplenishDemandEstimator.getExpectedDemandForTimeBin(t);
      List<DrtZoneVehicleSurplus> vehicleSurpluses =
          zonalSystem.getZones().values().stream()
              .map(
                  z ->
                      new DrtZoneVehicleSurplus(
                          z, (int) netDepartureInputFunction.applyAsDouble(z) * -1))
              .collect(toList());
      ...
      log.debug(
          "Calculating: "
              + (double) progressCounter * timeBinSize / simulationEndTime / 36
              + "% complete");
    }
