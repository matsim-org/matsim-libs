...
/**
 * "Insertion at end" means appending both pickup and dropoff at the end of the schedule, which
 * means the ride is not shared (like a normal taxi). In this case, the best insertion-at-end is the
 * one that is closest in time, so we just select the nearest (in straight-line) for the
 * MultiNodeDijkstra (OneToManyPathSearch)
 *
 * @author michalm
 */
class KNearestInsertionsAtEndFilter {
  static List<Insertion> filterInsertionsAtEnd(int k, List<InsertionWithDetourData> insertions) {
    var nearestInsertionsAtEnd =
        new PartialSort<>(k, BestInsertionFinder.INSERTION_WITH_COST_COMPARATOR);
    ...
    for (var i : insertions) {
      ...
      if (!vEntry.isAfterLastStop(pickup.index)) {
        ...
        nearestInsertionsAtEnd.add(
            new InsertionWithCost(i, i.detourTimeInfo.pickupDetourInfo.departureTime));
      }
    }

    nearestInsertionsAtEnd
        .kSmallestElements()
