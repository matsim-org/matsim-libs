...
/**
 * Modified version of DrtStopActivity which handles parallel mounting and alighting of passengers
 * according to individual pickup and dropoff times.
 *
 * @author Sebastian Hörl, IRT SystemX (sebhoerl)
 */
public class PrebookingStopActivity extends FirstLastSimStepDynActivity
  ...
  private void initDropoffRequests(double now) {
    for (var request : dropoffRequests.values()) {
      double leaveTime =
          now + stopDurationProvider.calcDropoffDuration(vehicle, request.getRequest());
