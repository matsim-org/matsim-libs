...
public class DrtModeZonalSystemModule extends AbstractDvrpModeModule {
  ...
  public void install() {
    ...
    bindModal(DrtZonalSystem.class)
        .toProvider(
            modalProvider(
                getter -> {
                  ...
                  switch (params.zonesGeneration) {
                    case ShapeFile:
                      final List<PreparedGeometry> preparedGeometries =
                          loadPreparedGeometries(
                              ConfigGroup.getInputFileURL(
                                  getConfig().getContext(), params.zonesShapeFile));
                      return DrtZonalSystem.createFromPreparedGeometries(
                          network,
                    ...
                    case GridFromNetwork:
                      ...
                      var gridZones =
                          drtCfg.operationalScheme == OperationalScheme.serviceAreaBased
                              ? createGridFromNetworkWithinServiceArea(
                                  network,
                                  params.cellSize,
                                  loadPreparedGeometries(
                                      ConfigGroup.getInputFileURL(
    ...
    bindModal(DrtZoneTargetLinkSelector.class)
        .toProvider(
            modalProvider(
                getter -> {
                  switch (params.targetLinkSelection) {
                    case mostCentral:
                      return new MostCentralDrtZoneTargetLinkSelector(
    ...
    bindModal(ZonalIdleVehicleXYVisualiser.class)
        .toProvider(
            modalProvider(
                getter ->
                    new ZonalIdleVehicleXYVisualiser(
