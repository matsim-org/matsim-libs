...
public class RebalancingUtils {
  public static Map<DrtZone, List<DvrpVehicle>> groupRebalancableVehicles(
    ...
    rebalancableVehicles
        .filter(v -> v.getServiceEndTime() > time + params.minServiceTime)
        .forEach(
            v -> {
              ...
              if (zone == null) {
                zone =
                    DrtZone.createDummyZone(
                        "single-vehicle-zone-" + v.getId(),
                        List.of(link),
  ...
  public static Map<DrtZone, List<DvrpVehicle>> groupSoonIdleVehicles(
      DrtZonalSystem zonalSystem, RebalancingParams params, Fleet fleet, double time) {
