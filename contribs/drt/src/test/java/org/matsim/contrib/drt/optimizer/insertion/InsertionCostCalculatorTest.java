...
public class InsertionCostCalculatorTest {
  ...
  public void testCalculate() {
    ...
    assertCalculate(
        insertion,
        new DetourTimeInfo(new PickupDetourInfo(0, 11), new DropoffDetourInfo(0, 22)),
        11 + 22);

    // feasible solution - longest possible pickup and dropoff time losses
    assertCalculate(
        insertion,
        new DetourTimeInfo(new PickupDetourInfo(0, 20), new DropoffDetourInfo(0, 30)),
        20 + 30);

    // infeasible solution - time constraints at stop 0
    assertCalculate(
        insertion,
        new DetourTimeInfo(new PickupDetourInfo(0, 21), new DropoffDetourInfo(0, 29)),
        INFEASIBLE_SOLUTION_COST);

    // infeasible solution - vehicle time constraints
    assertCalculate(
        insertion,
        new DetourTimeInfo(new PickupDetourInfo(0, 20), new DropoffDetourInfo(0, 31)),
        INFEASIBLE_SOLUTION_COST);
  }

  private void assertCalculate(
      Insertion insertion, DetourTimeInfo detourTimeInfo, double expectedCost) {
    var insertionCostCalculator =
