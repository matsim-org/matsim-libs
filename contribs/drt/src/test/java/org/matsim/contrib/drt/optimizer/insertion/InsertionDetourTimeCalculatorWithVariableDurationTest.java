...
public class InsertionDetourTimeCalculatorWithVariableDurationTest {
  private static final Link fromLink = link("from");
  private static final Link toLink = link("to");

  private static final DrtRequest drtRequestInitial =
      DrtRequest.newBuilder().fromLink(fromLink).toLink(toLink).build();
  private static final DrtRequest drtRequestAdded =
      DrtRequest.newBuilder().fromLink(fromLink).toLink(toLink).build();

  private static final int STOP_DURATION_INITIAL = 10;
  private static final int STOP_DURATION_ADDED = 5;

  public static final PassengerStopDurationProvider STOP_DURATION_PROVIDER =
  ...
  public void detourTimeLoss_start_pickup_dropoff() {
    ...
    assertDetourTimeInfo(
        insertion,
        new DetourTimeInfo(
            new InsertionDetourTimeCalculator.PickupDetourInfo(departureTime, pickupTimeLoss),
  ...
  public void detourTimeLoss_ongoingStopAsStart_pickup_dropoff() {
    // similar to detourTmeLoss_start_pickup_dropoff(), but the pickup is appended to the ongoing
    // STOP task
    ...
    // sh 03/08/23: Updated this test, according to VehicleDataEntryFactoryImpl start time should be
    // task end time
  ...
  public void detourTimeLoss_start_pickup_dropoff_stop() {
    ...
    double pickupTimeLoss =
        detour.toPickup + STOP_DURATION_ADDED + detour.fromPickup - timeBetween(start, stop0);
    double arrivalTime = departureTime + detour.fromPickup;
  ...
  public void calculatePickupDetourTimeLoss_start_pickup_stop_dropoff_stop() {
    ...
    double pickupTimeLoss =
        detour.toPickup + STOP_DURATION_ADDED + detour.fromPickup - timeBetween(start, stop0);
    double arrivalTime = stop0.getDepartureTime() + pickupTimeLoss + detour.toDropoff;
    double dropoffTimeLoss =
        detour.toDropoff + STOP_DURATION_ADDED + detour.fromDropoff - timeBetween(stop0, stop1);
    assertDetourTimeInfo(
        insertion,
        new DetourTimeInfo(
            new InsertionDetourTimeCalculator.PickupDetourInfo(departureTime, pickupTimeLoss),
  ...
  public void replacedDriveTimeEstimator() {
    ...
    var replacedDriveTimeEstimates =
        ImmutableTable.<Link, Link, Double>builder() //
    ...
    var detourTimeCalculator =
        new InsertionDetourTimeCalculator(
            STOP_TIME_CALCULATOR,
    ...
    DetourTimeInfo detourTimeInfo =
        new DetourTimeInfo(
            new InsertionDetourTimeCalculator.PickupDetourInfo(departureTime, pickupTimeLoss),
            new InsertionDetourTimeCalculator.DropoffDetourInfo(arrivalTime, dropoffTimeLoss));
    assertThat(actualDetourTimeInfo.pickupDetourInfo)
        .isEqualToComparingFieldByField(detourTimeInfo.pickupDetourInfo);
    assertThat(actualDetourTimeInfo.dropoffDetourInfo)
        .isEqualToComparingFieldByField(detourTimeInfo.dropoffDetourInfo);
  }
  ...
  private void assertDetourTimeInfo(InsertionWithDetourData insertion, DetourTimeInfo expected) {
    ...
    DetourTimeInfo detourTimeInfo =
        detourTimeCalculator.calculateDetourTimeInfo(
            insertion.insertion, insertion.detourData, drtRequestAdded);
    assertThat(detourTimeInfo.pickupDetourInfo)
        .isEqualToComparingFieldByField(expected.pickupDetourInfo);
    assertThat(detourTimeInfo.dropoffDetourInfo)
        .isEqualToComparingFieldByField(expected.dropoffDetourInfo);
  }
  ...
  private Waypoint.Stop stop(double beginTime, Link link) {
    DrtStopTask stopTask =
        new DefaultDrtStopTask(beginTime, beginTime + STOP_DURATION_INITIAL, link);
  ...
  private InsertionWithDetourData insertion(
      VehicleEntry entry, int pickupIdx, int dropoffIdx, Detour detour) {
    Insertion insertion = new Insertion(drtRequestInitial, entry, pickupIdx, dropoffIdx);
    InsertionWithDetourData.InsertionDetourData insertionDetourData =
        new InsertionWithDetourData.InsertionDetourData(
            getPath(detour.toPickup),
            getPath(detour.fromPickup),
            getPath(detour.toDropoff),
            getPath(detour.fromDropoff));

    DetourTimeInfo detourTimeInfo =
        new DetourTimeInfo(
            new InsertionDetourTimeCalculator.PickupDetourInfo(
                0, detour.fromPickup + detour.toPickup),
            new InsertionDetourTimeCalculator.DropoffDetourInfo(
                0, detour.fromDropoff + detour.toDropoff));
  ...
  private OneToManyPathSearch.PathData getPath(double detour) {
    return new OneToManyPathSearch.PathData(
        new LeastCostPathCalculator.Path(null, Collections.EMPTY_LIST, detour, detour), 0);
  }
