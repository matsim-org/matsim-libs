...
public class DetourPathDataCacheTest {
  ...
  private final DrtRequest request =
      DrtRequest.newBuilder().fromLink(pickupLink).toLink(dropoffLink).build();
  private final VehicleEntry entry = entry(startLink, stop0Link, stop1Link);

  private final ImmutableMap<Link, PathData> pathToPickupMap =
      ImmutableMap.of(startLink, start_pickup, stop0Link, stop0_pickup, stop1Link, stop1_pickup);

  private final ImmutableMap<Link, PathData> pathFromPickupMap =
      ImmutableMap.of(
          stop0Link, pickup_stop0, stop1Link, pickup_stop1, dropoffLink, pickup_dropoff);

  private final ImmutableMap<Link, PathData> pathToDropoffMap =
      ImmutableMap.of(
          pickupLink, pickup_dropoff, stop0Link, stop0_dropoff, stop1Link, stop1_dropoff);

  private final ImmutableMap<Link, PathData> pathFromDropoffMap =
      ImmutableMap.of(stop0Link, dropoff_stop0, stop1Link, dropoff_stop1);

  private static final PathData ZERO_DETOUR = mock(PathData.class);
  private final DetourPathDataCache detourPathDataCache =
      new DetourPathDataCache(
          pathToPickupMap, pathFromPickupMap, pathToDropoffMap, pathFromDropoffMap, ZERO_DETOUR);
  ...
  private void assertInsertion(
    ...
    var expectedInsertionDetourData =
        new InsertionDetourData(
            detourToPickup, detourFromPickup, detourToDropoff, detourFromDropoff);
  ...
  private VehicleEntry entry(Link startLink, Link... stopLinks) {
    return new VehicleEntry(
        null,
        new Waypoint.Start(null, startLink, 0, 0),
