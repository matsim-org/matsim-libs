...
public class SelectiveInsertionProviderTest {
  ...
  public void getInsertions_noInsertionsGenerated() {
    var insertionProvider =
        new SelectiveInsertionProvider(
            initialInsertionFinder,
  ...
  private void getInsertions_twoInsertionsGenerated(boolean oneSelected) {
    ...
    var insertionWithDetourData =
        new InsertionWithDetourData(
            new Insertion(vehicleEntry, null, null),
    ...
    when(insertionGenerator.generateInsertions(eq(request), eq(vehicleEntry)))
        .thenReturn(List.of(insertionWithDetourData));

    // mock initialInsertionFinder
    var selectedInsertion =
        oneSelected
            ? Optional.of(insertionWithDetourData)
            : Optional.<InsertionWithDetourData>empty();
    when(initialInsertionFinder.findBestInsertion(
            eq(request),
            argThat(
                argument -> argument.collect(toSet()).equals(Set.of(insertionWithDetourData))))) //
        .thenReturn(selectedInsertion);

    // test insertionProvider
    var insertionProvider =
        new SelectiveInsertionProvider(
            initialInsertionFinder, insertionGenerator, rule.forkJoinPool);
    assertThat(insertionProvider.getInsertion(request, List.of(vehicleEntry)))
        .isEqualTo(selectedInsertion);
  }
}
