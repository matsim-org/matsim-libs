...
public class MultiInsertionDetourPathCalculatorTest {
  ...
  private final MultiInsertionDetourPathCalculator detourPathCalculator =
      new MultiInsertionDetourPathCalculator(pathSearch, pathSearch, pathSearch, pathSearch, 1);
  ...
  public void calculatePaths() {
    var pathToPickup =
        mockCalcPathData(pickupLink, beforePickupLink, request.getEarliestStartTime(), false, 11);
    var pathFromPickup =
        mockCalcPathData(pickupLink, afterPickupLink, request.getEarliestStartTime(), true, 22);
    var pathToDropoff =
        mockCalcPathData(dropoffLink, beforeDropoffLink, request.getLatestArrivalTime(), false, 33);
    var pathFromDropoff =
        mockCalcPathData(dropoffLink, afterDropoffLink, request.getLatestArrivalTime(), true, 44);
  ...
  public void calculatePaths_dropoffAfterPickup_dropoffAtEnd() {
    ...
    var pathToPickup =
        mockCalcPathData(pickupLink, beforePickupLink, request.getEarliestStartTime(), false, 11);
    var pathFromPickup =
        mockCalcPathData(pickupLink, dropoffLink, request.getEarliestStartTime(), true, 22);

    // use specific class of waypoint to allow for detecting the special case
    var pickup =
        insertionPoint(waypoint(beforePickupLink), waypoint(dropoffLink, Waypoint.Dropoff.class));
    var dropoff =
        insertionPoint(
            waypoint(pickupLink, Waypoint.Pickup.class), waypoint(dropoffLink, Waypoint.End.class));
    var insertion = new InsertionGenerator.Insertion(null, pickup, dropoff);
  ...
  public void calculatePaths_noDetours() {
    ...
    when(pathSearch.calcPathDataMap(
            eq(pickupLink),
            eqSingleLinkCollection(pickupLink),
            eq(request.getEarliestStartTime()),
            anyBoolean()))
        .thenReturn(Map.of(pickupLink, PathData.EMPTY));
    when(pathSearch.calcPathDataMap(
            eq(dropoffLink),
            eqSingleLinkCollection(dropoffLink),
            eq(request.getLatestArrivalTime()),
            anyBoolean()))
        .thenReturn(Map.of(dropoffLink, PathData.EMPTY));
  ...
  private PathData mockCalcPathData(
      Link fromLink, Link toLink, double startTimeArg, boolean forward, double pathTravelTime) {
    var fromNode = fromLink.getToNode();
    var toNode = toLink.getFromNode();
    var path =
        new Path(List.of(fromNode, toNode), List.of(), pathTravelTime, pathTravelTime + 1000);
    var pathData = new PathData(path, 99);
    when(pathSearch.calcPathDataMap(
            eq(fromLink), eqSingleLinkCollection(toLink), eq(startTimeArg), eq(forward)))
        .thenReturn(Map.of(toLink, pathData));
    return pathData;
  }
  ...
  private InsertionGenerator.InsertionPoint insertionPoint(
      Waypoint beforeWaypoint, Waypoint afterWaypoint) {
