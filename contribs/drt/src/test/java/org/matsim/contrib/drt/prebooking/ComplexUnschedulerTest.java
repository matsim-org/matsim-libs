...
public class ComplexUnschedulerTest {
  ...
  public void testDirectDropoffAfterPickup() {
    ...
    ComplexRequestUnscheduler unscheduler =
        new ComplexRequestUnscheduler(
            fixture.lookup,
            fixture.entryFactory,
            fixture.taskFactory,
            fixture.router,
            fixture.travelTime,
            fixture.timingUpdater,
            false);
  ...
  public void testRemoveAtBeginningWithDriveDiversion() {
    ...
    LinkTimePair diversionPoint =
        new LinkTimePair(fixture.network.getLinks().get(Id.createLinkId("f5")), 20.0);
  ...
  private static void assertTasks(Schedule schedule, List<ReferenceTask> references) {
    for (int i = 0; i < references.size(); i++) {
      ...
      if (i > 0) {
        assertEquals(
            "wrong transition from " + (i - 1) + " to " + i,
            schedule.getTasks().get(i).getBeginTime(),
            schedule.getTasks().get(i - 1).getEndTime(),
            1e-3);
      }
  ...
  private Network createNetwork() {
    ...
    for (int i = 0; i < 99; i++) {
      Link forwardLink =
          networkFactory.createLink(Id.createLinkId("f" + i), nodes.get(i), nodes.get(i + 1));
      network.addLink(forwardLink);

      Link backwardLink =
          networkFactory.createLink(Id.createLinkId("b" + i), nodes.get(i + 1), nodes.get(i));
  ...
  private class Fixture {
    ...
    Fixture() {
      ...
      DvrpVehicleSpecification vehicleSpecification =
    ...
    DrtStayTask addWait(double duration) {
      DrtStayTask task =
          taskFactory.createStayTask(vehicle, currentTime, currentTime + duration, currentLink);
    ...
    DrtStayTask addFinalStay(double until) {
      DrtStayTask task =
          taskFactory.createStayTask(
              vehicle, currentTime, Math.max(currentTime, until), currentLink);
      schedule.addTask(task);
    ...
    DrtStopTask addStop(double duration) {
      DrtStopTask task =
          taskFactory.createStopTask(vehicle, currentTime, currentTime + duration, currentLink);
      schedule.addTask(task);
      currentTime += duration;
      return task;
    }
    ...
    DrtDriveTask addDrive(String destinationLinkId) {
      ...
      VrpPathWithTravelData path =
          VrpPaths.calcAndCreatePath(currentLink, destinationLink, currentTime, router, travelTime);
