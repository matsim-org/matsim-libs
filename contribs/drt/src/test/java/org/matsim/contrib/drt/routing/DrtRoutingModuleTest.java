...
public class DrtRoutingModuleTest {
  ...
  public void testCottbusClosestAccessEgressStopFinder() {
    ...
    TeleportationRoutingModule walkRouter =
        new TeleportationRoutingModule(
            TransportMode.walk, scenario, networkTravelSpeed, beelineFactor);
    ...
    ImmutableMap<Id<DrtStopFacility>, DrtStopFacility> drtStops =
    ...
    AccessEgressFacilityFinder stopFinder =
        new ClosestAccessEgressFacilityFinder(
            drtCfg.maxWalkDistance,
            scenario.getNetwork(),
            QuadTrees.createQuadTree(drtStops.values()));
    DrtRouteCreator drtRouteCreator =
        new DrtRouteCreator(
            drtCfg,
            scenario.getNetwork(),
            new SpeedyDijkstraFactory(),
            new FreeSpeedTravelTime(),
            TimeAsTravelDisutility::new);
    DefaultMainLegRouter mainRouter =
        new DefaultMainLegRouter(
            drtMode, scenario.getNetwork(), scenario.getPopulation().getFactory(), drtRouteCreator);
    DvrpRoutingModule dvrpRoutingModule =
    ...
    List<? extends PlanElement> routedList =
        dvrpRoutingModule.calcRoute(DefaultRoutingRequest.withoutAttributes(hf, wf, 8 * 3600, p1));
    ...
    // drt boarding should be at link id 2183 or 2184, not totally clear which one of these (from
    // and to nodes inverted)
    // drt alighting should be at link id 5866 or 5867, not totally clear which one of these (from
    // and to nodes inverted)
     ...
     */
    Assert.assertTrue(
        realDrtLegP1Route.getEndLinkId().equals(Id.createLinkId(5866))
            || realDrtLegP1Route.getEndLinkId().equals(Id.createLinkId(5867)));
    Id<Link> endLink = realDrtLegP1Route.getEndLinkId();
    // Check of other, more drt-specific attributes of the DrtRoute is missing, maybe these should
    // be tested in DrtRoutingModule instead
    ...
    // case 2: origin and destination outside max walking distance from next stop (>2000m vs. max
    // 200m)
    ...
    List<? extends PlanElement> routedList2 =
        dvrpRoutingModule.calcRoute(
            DefaultRoutingRequest.withoutAttributes(hf2, wf2, 8 * 3600, p2));

    Assert.assertNull(routedList2);

    // case 3: origin and destination at the same coordinate, > 2000 m walking distance from next
    // stop
    ...
    List<? extends PlanElement> routedList3 =
        dvrpRoutingModule.calcRoute(
            DefaultRoutingRequest.withoutAttributes(hf3, wf3, 8 * 3600, p3));

    Assert.assertNull(routedList3);

    // case 4: origin and destination at the same coordinate, in 200 m walking distance from next
    // stop
    ...
    List<? extends PlanElement> routedList4 =
        dvrpRoutingModule.calcRoute(
            DefaultRoutingRequest.withoutAttributes(hf4, wf4, 8 * 3600, p4));

    Assert.assertNull(routedList4);

    // case 5: origin within 200 m walking distance from next stop, but destination outside walking
    // distance
    ...
    List<? extends PlanElement> routedList5 =
        dvrpRoutingModule.calcRoute(
            DefaultRoutingRequest.withoutAttributes(hf5, wf5, 8 * 3600, p5));

    // TODO: Asserts are prepared for interpreting maxWalkingDistance as a real maximum, but routing
    // still works wrongly
    Assert.assertNull(routedList5);

    // case 6: destination within 200 m walking distance from next stop, but origin outside walking
    // distance
    ...
    List<? extends PlanElement> routedList6 =
        dvrpRoutingModule.calcRoute(
  ...
  public void testRouteDescriptionHandling() {
    ...
    String newRouteFormat =
        "{\"maxWaitTime\":600.0,\"directRideTime\":400.0,\"unsharedPath\":[\"a\",\"b\",\"c\"]}";
  ...
  private Scenario createTestScenario() {
    ...
    new MatsimNetworkReader(scenario.getNetwork())
        .readFile(utils.getClassInputDirectory() + "testCottbus/network.xml.gz");
  ...
  private void createSomeAgents(Scenario scenario) {
    ...
    p1.addPlan(plan1);
    Activity home =
        pf.createActivityFromCoord("home", new Coord(451931.406932525, 5733832.50176344));
    home.setLinkId(Id.createLinkId(3699));
    home.setEndTime(8 * 3600);
    plan1.addActivity(home);
    plan1.addLeg(pf.createLeg("drt"));
    Activity work =
        pf.createActivityFromCoord("work", new Coord(453595.36658007314, 5734504.7695318265));
    ...
    p2.addPlan(plan2);
    Activity home2 =
        pf.createActivityFromCoord("home", new Coord(460077.7116017367, 5740133.3409971865));
    home2.setLinkId(Id.createLinkId(9541));
    home2.setEndTime(8 * 3600);
    plan2.addActivity(home2);
    plan2.addLeg(pf.createLeg("drt"));
    Activity work2 =
        pf.createActivityFromCoord("work", new Coord(461757.56027226395, 5742929.001039858));
    work2.setLinkId(Id.createLinkId(7717));
    plan2.addActivity(work2);
    scenario.getPopulation().addPerson(p2);

    // origin and destination at the same coordinate, different links! (should we test link inside
    // walking distance but coordinate outside etc.?), > 2000 m walking distance from next stop
    ...
    Activity work3 =
        pf.createActivityFromCoord("work", new Coord(460077.7116017367, 5740133.3409971865));
    ...
    Activity work4 =
        pf.createActivityFromCoord("work", new Coord(451931.406932525, 5733832.50176344));
