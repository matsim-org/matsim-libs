...
public class ExtensiveInsertionProviderTest {
  ...
  public void getInsertions_noInsertionsGenerated() {
    var insertionProvider =
        new ExtensiveInsertionProvider(
            null,
            null,
            new InsertionGenerator(new DefaultStopTimeCalculator(120), null),
            rule.forkJoinPool);
  ...
  private void getInsertions_twoInsertionsGenerated(int nearestInsertionsAtEndLimit) {
    ...
    when(insertionGenerator.generateInsertions(eq(request), eq(vehicleEntry))) //
        .thenReturn(
            List.of(
                insertionWithDetourData(feasibleInsertion),
    ...
    when(admissibleCostCalculator.calculate(
            eq(request), argThat(argument -> argument == feasibleInsertion), any()))
        .thenReturn(1.);
    when(admissibleCostCalculator.calculate(
            eq(request), argThat(argument -> argument == infeasibleInsertion), any()))
    ...
    var insertionProvider =
        new ExtensiveInsertionProvider(
            params, admissibleCostCalculator, insertionGenerator, rule.forkJoinPool);
    assertThat(insertionProvider.getInsertions(request, List.of(vehicleEntry)))
        .isEqualTo(nearestInsertionsAtEndLimit == 0 ? List.of() : List.of(feasibleInsertion));
  }

  private InsertionGenerator.InsertionPoint insertionPoint() {
    return new InsertionGenerator.InsertionPoint(
        -1, mock(Waypoint.class), null, mock(Waypoint.class));
  }

  private InsertionWithDetourData insertionWithDetourData(Insertion insertion) {
    return new InsertionWithDetourData(
        insertion,
        new InsertionDetourData(null, null, null, null),
