...
public class DrtSpeedUpTest {
  ...
  private final Node nodeA =
      NetworkUtils.createAndAddNode(network, Id.createNodeId("A"), new Coord(0, 0));
  private final Node nodeB =
      NetworkUtils.createAndAddNode(network, Id.createNodeId("B"), new Coord(100, 0));
  private final Node nodeC =
      NetworkUtils.createAndAddNode(network, Id.createNodeId("C"), new Coord(200, 0));
  private final Link linkAB =
      NetworkUtils.createAndAddLink(network, Id.createLinkId("AB"), nodeA, nodeB, 100, 15, 20, 1);
  private final Link linkBC =
      NetworkUtils.createAndAddLink(network, Id.createLinkId("BA"), nodeB, nodeC, 100, 15, 20, 1);
  ...
  public void test_useOnlyInitialEstimates_noRegression() {
    ...
    DrtSpeedUp drtSpeedUp =
        new DrtSpeedUp(
            MODE, drtSpeedUpParams, controlerConfig, network, fleetSpecification, requestAnalyzer);
  ...
  public void test_linearRegression() {
    ...
    // linear regression
    drtSpeedUpParams.waitingTimeUpdateDuringSpeedUp =
    ...
    // teleported iteration 3 (0.25 rides/vehicle => estimated wait time as 25 s - estimation is run
    // after iteration)
  ...
  private void updateRequestAnalyser(EventSequence... eventSequences) {
    var sequences =
        Arrays.stream(eventSequences)
            .collect(
                ImmutableMap.toImmutableMap(seq -> seq.getSubmitted().getRequestId(), seq -> seq));
    when(requestAnalyzer.getPerformedRequestSequences()).thenReturn(sequences);
  }

  private EventSequence eventSequence(
      String id, double submittedTime, double waitTime, double inVehicleSpeed) {
    ...
    var submittedEvent =
        new DrtRequestSubmittedEvent(
            ...
            Double.NaN);
    var pickupEvent =
        new PassengerPickedUpEvent(submittedTime + waitTime, MODE, requestId, null, null);
    double rideTime = DistanceUtils.calculateDistance(linkBC, linkAB) / inVehicleSpeed;
    var dropoffEvent =
        new PassengerDroppedOffEvent(
            submittedTime + waitTime + rideTime, MODE, requestId, null, null);
    var drtFare =
    ...
    return new EventSequence(
        Id.createPersonId("r1"),
        departureEvent,
        submittedEvent,
        mock(PassengerRequestScheduledEvent.class),
        pickupEvent,
        dropoffEvent,
        List.of(drtFare));
  }
  ...
  private void assertAverages(
      DrtSpeedUp drtSpeedUp, double waitTime, double inVehicleBeelineSpeed) {
