...
public class DrtRequestInsertionRetryQueueTest {
  ...
  public void requestMaxAgeNotExceeded_retry() {
    ...
    assertThat(queue.getRequestsToRetryNow(now))
        .usingRecursiveFieldByFieldElementComparator()
        .containsExactly(
            DrtRequest.newBuilder(request)
  ...
  public void requestMaxAgeNotExceeded_lateRetry() {
    ...
    // retry
    double now =
        999999; // no guarantee the method is called every second, so let's make a very late call
    assertThat(queue.getRequestsToRetryNow(now))
        .usingRecursiveFieldByFieldElementComparator()
        .containsExactly(
            DrtRequest.newBuilder(request)
  ...
  public void queueOrderMaintained() {
    var queue = new DrtRequestInsertionRetryQueue(params(2, 10));
    assertThat(
            queue.tryAddFailedRequest(
                DrtRequest.newBuilder(request).id(Id.create("a", Request.class)).build(),
                SUBMISSION_TIME))
        .isTrue();
    assertThat(
            queue.tryAddFailedRequest(
                DrtRequest.newBuilder(request).id(Id.create("b", Request.class)).build(),
                SUBMISSION_TIME))
        .isTrue();
    assertThat(
            queue.tryAddFailedRequest(
                DrtRequest.newBuilder(request).id(Id.create("c", Request.class)).build(),
                SUBMISSION_TIME))
        .isTrue();
    assertThat(
            queue.tryAddFailedRequest(
                DrtRequest.newBuilder(request).id(Id.create("d", Request.class)).build(),
                SUBMISSION_TIME))
        .isTrue();
    ...
    // order is maintained
    assertThat(queue.getRequestsToRetryNow(SUBMISSION_TIME + 2))
        .extracting(req -> req.getId().toString())
