...
public class InsertionDetourTimeCalculatorTest {
  ...
  public void detourTimeLoss_start_pickup_dropoff() {
    ...
    double departureTime =
        start.getDepartureTime() + detour.detourToPickup.getTravelTime() + STOP_DURATION;
    double pickupTimeLoss =
        detour.detourToPickup.getTravelTime()
            + STOP_DURATION
    ...
    assertDetourTimeInfo(
        insertion,
        new DetourTimeInfo(
            new PickupDetourInfo(departureTime, pickupTimeLoss),
  ...
  public void detourTimeLoss_ongoingStopAsStart_pickup_dropoff() {
    ...
    // STOP task
    // sh 03/08/23: Changed this test, according to VehicleDataEntryFactoryImpl the start time
    // should be end time of stop task
    Waypoint.Start start =
        start(
            new DefaultDrtStopTask(20, 20 + STOP_DURATION, fromLink), 20 + STOP_DURATION, fromLink);
    VehicleEntry entry = entry(start);
  ...
  public void calculatePickupDetourTimeLoss_start_pickup_stop_dropoff() {
    ...
    double pickupTimeLoss =
        detour.detourToPickup.getTravelTime()
            ...
            - timeBetween(start, stop0);
    double arrivalTime =
        stop0.getDepartureTime() + pickupTimeLoss + detour.detourToDropoff.getTravelTime();
  ...
  public void calculatePickupDetourTimeLoss_start_pickup_stop_dropoff_stop() {
    ...
    double dropoffTimeLoss =
        detour.detourToDropoff.getTravelTime()
            + STOP_DURATION
            + detour.detourFromDropoff.getTravelTime()
            - timeBetween(stop0, stop1);
    assertDetourTimeInfo(
        insertion,
        new DetourTimeInfo(
            new PickupDetourInfo(departureTime, pickupTimeLoss),
  ...
  public void replacedDriveTimeEstimator() {
    ...
    var actualDetourTimeInfo =
        detourTimeCalculator.calculateDetourTimeInfo(
            insertion.insertion, insertion.detourData, drtRequest);
    ...
    assertThat(actualDetourTimeInfo)
  ...
  private void assertDetourTimeInfo(InsertionWithDetourData insertion, DetourTimeInfo expected) {
    var detourTimeCalculator =
        new InsertionDetourTimeCalculator(new DefaultStopTimeCalculator(STOP_DURATION), null);
    var detourTimeInfo =
        detourTimeCalculator.calculateDetourTimeInfo(
            insertion.insertion, insertion.detourData, drtRequest);
  ...
  private InsertionDetourData detourData(
      double toPickupTT, double fromPickupTT, double toDropoffTT, double fromDropoffTT) {
    ...
    return new InsertionDetourData(
        toPickupDetour, fromPickupDetour, toDropoffDetour, fromDropoffDetour);
  }

  private InsertionWithDetourData insertion(
      VehicleEntry entry, int pickupIdx, int dropoffIdx, InsertionDetourData detour) {
    return new InsertionWithDetourData(
        new Insertion(drtRequest, entry, pickupIdx, dropoffIdx), detour, null);
  }
