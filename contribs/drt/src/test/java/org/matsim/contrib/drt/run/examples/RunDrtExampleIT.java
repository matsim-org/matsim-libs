...
public class RunDrtExampleIT {
  ...
  public void testRunDrtStopbasedExample() {
    ...
    URL configUrl =
        IOUtils.extendUrl(
            ExamplesUtils.getTestScenarioURL("mielec"), "mielec_stop_based_drt_config.xml");
    Config config =
        ConfigUtils.loadConfig(
            configUrl,
            new MultiModeDrtConfigGroup(),
  ...
  public void testRunDrtStopbasedExampleWithFlexibleStopDuration() {
    ...
    controller.addOverridingModule(
        new AbstractDvrpModeModule("drt") {
          @Override
          public void install() {
  ...
  public void testRunServiceAreabasedExampleWithSpeedUp() {
    ...
    URL configUrl =
        IOUtils.extendUrl(
            ExamplesUtils.getTestScenarioURL("mielec"), "mielec_serviceArea_based_drt_config.xml");
    Config config =
        ConfigUtils.loadConfig(
            configUrl,
            new MultiModeDrtConfigGroup(),
        ...
        new AbstractDvrpModeModule("drt") {
          ...
          public void install() {
            PassengerStopDurationProvider stopDurationProvider =
                StaticPassengerStopDurationProvider.of(60.0, 5.0);
            StopTimeCalculator stopTimeCalculator =
                new CumulativeStopTimeCalculator(stopDurationProvider);
  ...
  /**
   * Early warning system: if customer stats vary more than the defined percentage above or below
   * the expected values then the following unit tests will fail. This is meant to serve as a red
   * flag. The following customer parameter checked are: rejectionRate, rejections, waitAverage,
   * inVehicleTravelTimeMean, & totalTravelTimeMean
   */
  private void verifyDrtCustomerStatsCloseToExpectedStats(
      String outputDirectory, Stats expectedStats) {
  ...
  private static class PrebookingTracker
