...
/**
 * This set of tests examines how the pooling (car-sharing) behavior varies based on parameters in
 * the drt config group: maxWaitTime, maxTravelTimeAlpha and maxTravelTimeBeta.
 *
 * <p>In this case, rejectRequestIfMaxWaitOrTravelTimeViolated=true (default value), which means
 * that maxTravelTime and maxWaitTime are seen as hard constraints; if those conditions aren't met,
 * the request is rejected.
 *
 * <p>The mielec scenario is used for this purpose. There are only four agents present in this case,
 * which matches the vehicle capacity of the drt vehicles. All four agents have their home location
 * in a small area (links are adjacent to one another). The same is true of their work locations.
 * The purpose of this is to encourage pooling.
 *
 * <p>While there are 10 DRT vehicles overall, there are only two that are closeby: drt_veh_1_1 and
 * drt_veh_1_2. These two vehicles should be used for all of the test cases.
 *
 * <p>I attempted to calculate specific thresholds for the various parameters using my own
 * calculations. This was not successful, in part, because during my work the calculation process to
 * find the detourTime changed: within the class DetourTimesProvider.java, the method
 * createNodeToNodeBeelineTimeEstimator was replaced by createFreeSpeedZonalTimeEstimator.
 * Therefore, the following task: TODO: Calculate specific thresholds for parameters and demonstrate
 * those calculations ^The beginning of this process is commented at the end of this class
 *
 * <p>What happens if rejectRequestIfMaxWaitOrTravelTimeViolated = false? --> TODO: verify how soft
 * constraints work. The stopDuration is set to 1s --> potential TODO: reset to default of 60s TODO:
 * write tests for maxTravelTimeAlpha
 */
public class DrtPoolingParameterTest {
  ...
  /**
   * With a maxWaitTime of 121s, the two closest drt vehicles should be able to reach 2 agents, but
   * be unable to then pickup the other 2 agents without surpassing the maxWaitTime. All other DRT
   * Vehicles too far away to reach those agents.
   */
  @Test
  public void testMaxWaitTimeTwoVehiclesForTwoAgents() {
    ...
    Assert.assertEquals(
        "drt_veh_1_1 should be requested exactly once",
        1,
        handler.getVehRequestCount().get(drt_veh_1_1),
        0);
    Assert.assertTrue(
        "drt_veh_1_2 should be requested in general",
        handler.getVehRequestCount().containsKey(drt_veh_1_2));
    Assert.assertEquals(
        "drt_veh_1_2 should be requested exactly once",
        1,
        handler.getVehRequestCount().get(drt_veh_1_2),
        0);
  }
  ...
  public void testMaxWaitTimeTwoVehiclesForFourAgents() {
    ...
    Assert.assertTrue(
        ...
        handler.getVehRequestCount().containsKey(drt_veh_1_1));
    Assert.assertEquals(
        "drt_veh_1_1 should be requested exactly twice",
        2,
        handler.getVehRequestCount().get(drt_veh_1_1),
        0);
    Assert.assertTrue(
        ...
        handler.getVehRequestCount().containsKey(drt_veh_1_2));
    Assert.assertEquals(
        "drt_veh_1_2 should be requested exactly twice",
        2,
        handler.getVehRequestCount().get(drt_veh_1_2),
        0);
  }
  ...
  /**
   * With a high maxWaitTime of 500s, a single DRT vehicle should be able to pick up all four
   * agents.
   */
  @Test
  public void testMaxWaitTimeOneVehicleForFourAgents() {
    ...
    Assert.assertEquals(
        "There should only be one vehicle used", 1, handler.getVehRequestCount().size());
    Id<DvrpVehicle> drt_veh_1_1 = Id.create("drt_veh_1_1", DvrpVehicle.class);
    Assert.assertTrue(
        "drt_veh_1_1 should be requested in general",
        handler.getVehRequestCount().containsKey(drt_veh_1_1));
    Assert.assertEquals(
        "drt_veh_1_1 should be requested exactly four times",
        4,
        handler.getVehRequestCount().get(drt_veh_1_1),
        0);
  }
  ...
  */
  ...
  public void testBetaNoVehicles() {
    ...
    Assert.assertEquals(
        "There should only be zero vehicles used", 0, handler.getVehRequestCount().size());
  }
  ...
  /**
   * With a Beta value of 250s, two DRT vehicles should have enough time to pick up two passengers
   * each.
   */
  @Test
  ...
  private PersonEnterDrtVehicleEventHandler setupAndRunScenario(
      double maxWaitTime, double maxTravelTimeAlpha, double maxTravelTimeBeta) {
    URL configUrl =
    ...
    ActivityFacility homeFac1 =
        ff.createActivityFacility(Id.create("hf1", ActivityFacility.class), Id.createLinkId("359"));
    ActivityFacility homeFac2 =
        ff.createActivityFacility(Id.create("hf2", ActivityFacility.class), Id.createLinkId("364"));
    ActivityFacility homeFac3 =
        ff.createActivityFacility(Id.create("hf3", ActivityFacility.class), Id.createLinkId("373"));
    ActivityFacility homeFac4 =
        ff.createActivityFacility(Id.create("hf4", ActivityFacility.class), Id.createLinkId("353"));

    ActivityFacility workFac1 =
        ff.createActivityFacility(Id.create("wf1", ActivityFacility.class), Id.createLinkId("185"));
    ActivityFacility workFac2 =
        ff.createActivityFacility(Id.create("wf2", ActivityFacility.class), Id.createLinkId("245"));
    ActivityFacility workFac3 =
        ff.createActivityFacility(Id.create("wf3", ActivityFacility.class), Id.createLinkId("237"));
    ActivityFacility workFac4 =
        ff.createActivityFacility(Id.create("wf4", ActivityFacility.class), Id.createLinkId("240"));
  ...
  // scenario.getActivityFacilities().getFacilities();
  //
  ...
  //		ActivityFacility vehLoc =
  // scenario.getActivityFacilities().getFactory().createActivityFacility(Id.create("vehLoc",
  // ActivityFacility.class), Id.createLinkId(385));
  //
  ...
  //	}
  //
  //	private double findBeelineTime(ActivityFacility actFac1, ActivityFacility actFac2,
  // DrtConfigGroup drtCfg) {
  //		ExtensiveInsertionSearchParams drtInsertionSearchParams = (ExtensiveInsertionSearchParams)
  // drtCfg.getDrtInsertionSearchParams();
  ...
  //		Coord fromCoord =
  // controler.getScenario().getNetwork().getLinks().get(actFac1.getLinkId()).getCoord();
  //		Coord toCoord =
  // controler.getScenario().getNetwork().getLinks().get(actFac2.getLinkId()).getCoord();
  //		double beelineDistance = CoordUtils.calcEuclideanDistance(fromCoord, toCoord);
  //
  //		return beelineDistance / admissibleBeelineSpeed;
  //
  //	}
  //	private Double getDirectTT(ActivityFacility homeFac1, ActivityFacility workFac1, Person p1,
  // TripRouter router) {
  //
  //		List<? extends PlanElement> tripP1 = router.getRoutingModule("drt").calcRoute(homeFac1,
  // workFac1, 0., p1);
  ...
  //		return tt;
  //	}
  //	private Double getDirectTT_OLD(ActivityFacility homeFac1, ActivityFacility workFac1, Person p1,
  // TripRouter router) {
  //
  //		List<? extends PlanElement> planElements = router.calcRoute("car", homeFac1, workFac1, 8 *
  // 3600 + 0., p1);
  //
