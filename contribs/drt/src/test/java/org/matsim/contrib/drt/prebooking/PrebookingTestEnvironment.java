...
public class PrebookingTestEnvironment {
  ...
  public PrebookingTestEnvironment configure(
      double maximumWaitTime, double detourRelative, double detourAbsolute, double stopDuration) {
  ...
  public PrebookingTestEnvironment addRequest(
      String personId,
      int originX,
      int originY,
      int destinationX,
    ...
    for (Vehicle vehicle : vehicles) {
      fleetSpecification.addVehicleSpecification(
          ImmutableDvrpVehicleSpecification.newBuilder() //
              ...
              .startLinkId(
                  createLinkId(
                      createNodeId(vehicle.depot.getLeft(), vehicle.depot.getRight()),
  ...
  private void buildController(Controler controller) {
    ...
    MultiModeDrtConfigGroup multiModeDrtConfigGroup =
        MultiModeDrtConfigGroup.get(controller.getConfig());
    controller.configureQSimComponents(
        DvrpQSimComponents.activateAllModes(multiModeDrtConfigGroup));
  }
  ...
  private void buildPopulation(Scenario scenario) {
    ...
    for (Request request : requests) {
      ...
      Id<Link> originLinkId =
          createLinkId(
              createNodeId(request.origin.getLeft(), request.origin.getRight()),
      ...
      if (!Double.isNaN(request.submissionTime)) {
        firstActivity
            .getAttributes()
            .putAttribute(
                AttributeBasedPrebookingLogic.getSubmissionAttribute("drt"),
  ...
  private void buildNetwork(Scenario scenario) {
    ...
    for (int i = 0; i < width; i++) {
      for (int j = 0; j < height; j++) {
        nodes[i][j] =
            factory.createNode(createNodeId(i, j), new Coord(i * edgeLength, j * edgeLength));
  ...
  private class RequestListener
      implements DrtRequestSubmittedEventHandler,
    ...
    public void handleEvent(DrtRequestSubmittedEvent event) {
      String ids =
          event.getPersonIds().stream().map(Object::toString).collect(Collectors.joining("-"));
      requestInfo.computeIfAbsent(ids, id -> new RequestInfo()).submissionTime = event.getTime();
      requestInfo
          .computeIfAbsent(ids, id -> new RequestInfo())
          .submissionTimes
          .add(event.getTime());
    }
    ...
    public void handleEvent(PassengerRequestRejectedEvent event) {
      requestInfo.computeIfAbsent(
                  event.getPersonIds().stream()
                      .map(Object::toString)
    ...
    public void handleEvent(PassengerPickedUpEvent event) {
      requestInfo.computeIfAbsent(event.getPersonId().toString(), id -> new RequestInfo())
              .pickupTime =
          event.getTime();
    }
  ...
  private class VehicleListener implements TaskStartedEventHandler, TaskEndedEventHandler {
    ...
    public void handleEvent(TaskStartedEvent event) {
      taskInfo
          .computeIfAbsent(event.getDvrpVehicleId().toString(), id -> new LinkedList<>())
          .add(new TaskInfo());
