...
public class InsertionGeneratorTest {
	...
	private ListAssert<Insertion> assertThatInsertions(DrtRequest drtRequest, VehicleData.Entry entry) {
		...
		int endOccupancy = stopCount > 0 ? entry.stops.get(stopCount - 1).outgoingOccupancy : entry.startOccupancy;
	...
	private VehicleData.Entry entry(LinkTimePair start, int startOccupancy, VehicleData.Stop... stops) {
		return new VehicleData.Entry(vehicle, start, startOccupancy, ImmutableList.copyOf(stops));
	}
}
