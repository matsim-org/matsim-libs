...
public class InsertionGeneratorTest {
  ...
  private final DrtRequest drtRequest2Pax =
      DrtRequest.newBuilder()
          .fromLink(fromLink)
          .toLink(toLink)
          .passengerIds(List.of(Id.createPersonId("person1"), Id.createPersonId("person2")))
          .build();

  private final DrtRequest drtRequest5Pax =
      DrtRequest.newBuilder()
          .fromLink(fromLink)
          .toLink(toLink)
          .passengerIds(
          ...
          .build();
  private final DrtRequest prebookedRequest =
      DrtRequest.newBuilder().fromLink(fromLink).toLink(toLink).earliestStartTime(100).build();
  ...
  public void startEmpty_noStops() {
    Waypoint.Start start =
        new Waypoint.Start(null, link("start"), 0, 0); // no stops => must be empty
  ...
  public void startNotFull_oneStop() {
    ...
    Waypoint.Stop stop0 =
        stop(start.time + TIME_REPLACED_DRIVE, link("stop0"), 0); // drop off 1 pax
    ...
    { // 00
      ...
      var dropoff =
          new DropoffDetourInfo(
              pickup.departureTime + TIME_FROM_PICKUP, STOP_DURATION + TIME_FROM_DROPOFF);
      insertions.add(insertion(insertion, pickup, dropoff));
    }
    { // 01
    ...
    { // 11
      ...
      var pickup =
          new PickupDetourInfo(
              ...
              TIME_TO_PICKUP + STOP_DURATION + TIME_FROM_PICKUP);
      var dropoff =
          new DropoffDetourInfo(stop0.getDepartureTime() + pickup.pickupTimeLoss, STOP_DURATION);
      insertions.add(insertion(insertion, pickup, dropoff));
    }
    assertInsertionsWithDetour(drtRequest, entry, insertions);
  }
  ...
  public void startFull_oneStop() {
    ...
    Waypoint.Stop stop0 =
        stop(start.time + TIME_REPLACED_DRIVE, link("stop0"), 0); // drop off 4 pax
    VehicleEntry entry = entry(start, stop0);
  ...
  public void startEmpty_twoStops_notFullBetweenStops() {
    ...
    Waypoint.Stop stop1 =
        stop(stop0.getDepartureTime() + TIME_REPLACED_DRIVE, link("stop1"), 0); // drop off 1 pax
    ...
    { // 22
      ...
      var pickup =
          new PickupDetourInfo(
              ...
              TIME_TO_PICKUP + STOP_DURATION + TIME_FROM_PICKUP);
      var dropoff =
          new DropoffDetourInfo(stop1.getDepartureTime() + pickup.pickupTimeLoss, STOP_DURATION);
      insertions.add(insertion(insertion, pickup, dropoff));
    }
    assertInsertionsWithDetour(drtRequest, entry, insertions);
  }
  ...
  public void startEmpty_twoStops_notFullBetweenStops_tightSlackTimes() {
    ...
    double[] slackTimes = {
      0, 0, // impossible insertions: 00, 01, 02 (pickup at 0 is not possible)
    ...
    VehicleEntry entry =
        new VehicleEntry(
            vehicle, start, ImmutableList.of(stop0, stop1), slackTimes, precedingStayTimes, 0);
  ...
  public void noDetourForDropoff_vehicleOutgoingFullAfterDropoff_insertionPossible() {
    // a special case where we allow inserting the dropoff after a stop despite outgoingOccupancy ==
    // maxCapacity
  ...
  public void startEmpty_prebookedRequest() {
    ...
    VehicleEntry entry = entry(start);
    assertInsertionsOnly(prebookedRequest, entry, new Insertion(prebookedRequest, entry, 0, 0));
  }
  ...
  public void startEmpty_prebookedRequest_afterAlreadyPrebookedOtherRequest() {
    ...
    VehicleEntry entry = entry(start, stop0, stop1);
    assertInsertionsOnly(prebookedRequest, entry, new Insertion(prebookedRequest, entry, 2, 2));
  }
  ...
  private InsertionWithDetourData insertion(
      Insertion insertion, PickupDetourInfo pickupDetourInfo, DropoffDetourInfo dropoffDetourInfo) {
    return new InsertionWithDetourData(
        insertion, null, new DetourTimeInfo(pickupDetourInfo, dropoffDetourInfo));
  }

  private void assertInsertionsWithDetour(
      DrtRequest drtRequest, VehicleEntry entry, List<InsertionWithDetourData> expectedInsertions) {
    int stopCount = entry.stops.size();
    int endOccupancy =
        stopCount > 0 ? entry.stops.get(stopCount - 1).outgoingOccupancy : entry.start.occupancy;
    Preconditions.checkArgument(endOccupancy == 0); // make sure the input is valid

    DetourTimeEstimator timeEstimator =
    ...
    var actualInsertions =
        new InsertionGenerator(new DefaultStopTimeCalculator(STOP_DURATION), timeEstimator)
            .generateInsertions(drtRequest, entry);
    assertThat(actualInsertions)
        .usingRecursiveFieldByFieldElementComparator()
        .containsExactlyElementsOf(expectedInsertions);
  }

  private void assertInsertionsOnly(
      DrtRequest drtRequest, VehicleEntry entry, Insertion... expectedInsertions) {
    int stopCount = entry.stops.size();
    int endOccupancy =
        stopCount > 0 ? entry.stops.get(stopCount - 1).outgoingOccupancy : entry.start.occupancy;
    Preconditions.checkArgument(endOccupancy == 0); // make sure the input is valid
    ...
    var actualInsertions =
        new InsertionGenerator(new DefaultStopTimeCalculator(STOP_DURATION), timeEstimator)
            .generateInsertions(drtRequest, entry);
    assertThat(actualInsertions.stream().map(i -> i.insertion))
        .usingRecursiveFieldByFieldElementComparator()
        .containsExactly(expectedInsertions);
  }

  private Waypoint.Stop stop(double beginTime, Link link, int outgoingOccupancy) {
    return new Waypoint.Stop(
        new DefaultDrtStopTask(beginTime, beginTime + STOP_DURATION, link), outgoingOccupancy);
  }
  ...
  private VehicleEntry entry(Waypoint.Start start, Waypoint.Stop... stops) {
    ...
    return new VehicleEntry(
        vehicle, start, ImmutableList.copyOf(stops), slackTimes, precedingStayTimes, 0);
  }
}
