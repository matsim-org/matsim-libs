...
public class SingleInsertionDetourPathCalculatorTest {
  ...
  private final SingleInsertionDetourPathCalculator detourPathCalculator =
      new SingleInsertionDetourPathCalculator(
          null,
          new FreeSpeedTravelTime(),
          null,
          1,
          (network, travelCosts, travelTimes) -> pathCalculator);
  ...
  public void calculatePaths() {
    var pathToPickup =
        mockCalcLeastCostPath(beforePickupLink, pickupLink, request.getEarliestStartTime(), 11);
    var pathFromPickup =
        mockCalcLeastCostPath(pickupLink, afterPickupLink, request.getEarliestStartTime(), 22);
    var pathToDropoff =
        mockCalcLeastCostPath(beforeDropoffLink, dropoffLink, request.getLatestArrivalTime(), 33);
    var pathFromDropoff =
        mockCalcLeastCostPath(dropoffLink, afterDropoffLink, request.getLatestArrivalTime(), 44);
  ...
  public void calculatePaths_dropoffAfterPickup_dropoffAtEnd() {
    ...
    var pathToPickup =
        mockCalcLeastCostPath(beforePickupLink, pickupLink, request.getEarliestStartTime(), 11);
    var pathFromPickup =
        mockCalcLeastCostPath(pickupLink, dropoffLink, request.getEarliestStartTime(), 22);
    ...
    var dropoff =
        insertionPoint(waypoint(pickupLink, Pickup.class), waypoint(dropoffLink, End.class));
    var insertion = new Insertion(null, pickup, dropoff);
  ...
  private Path mockCalcLeastCostPath(
      Link fromLink, Link toLink, double startTimeArg, double pathTravelTime) {
    ...
    when(pathCalculator.calcLeastCostPath(
            eq(fromNode), eq(toNode), eq(startTimeArg + FIRST_LINK_TT), isNull(), isNull()))
        .thenReturn(path);
    return path;
  }

  private void assertPathData(PathData pathData, Path inputPath, Link toLink) {
    assertThat(pathData.getTravelTime())
        .isEqualTo(
            inputPath.travelTime
                + FIRST_LINK_TT
                + getLastLinkTT(new FreeSpeedTravelTime(), toLink, Double.NaN));
  }
