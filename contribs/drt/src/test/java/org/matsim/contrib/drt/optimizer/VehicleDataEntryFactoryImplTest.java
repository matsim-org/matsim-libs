...
public class VehicleDataEntryFactoryImplTest {
  ...
  public void computeSlackTimes_withStops() {
    ...
    assertThat(
            computeSlackTimes(
                vehicle(500, 450), 100, new Stop[] {stop0, stop1}, null, precedingStayTimes))
        .containsExactly(20, 20, 30, 50);

    // final stay task not started - vehicle slack time is 25 and limits the slack times at stop1
    assertThat(
            computeSlackTimes(
                vehicle(500, 475), 100, new Stop[] {stop0, stop1}, null, precedingStayTimes))
        .containsExactly(20, 20, 25, 25);

    // final stay task not started - vehicle slack time is 10 and limits the slack times at all
    // stops
    assertThat(
            computeSlackTimes(
                vehicle(500, 490), 100, new Stop[] {stop0, stop1}, null, precedingStayTimes))
  ...
  public void computeSlackTimes_withoutStops() {
    ...
    assertThat(computeSlackTimes(vehicle(500, 490), 485, new Stop[] {}, null, precedingStayTimes))
        .containsExactly(10, 10);

    // final stay task just started - vehicle slack time is 10
    assertThat(computeSlackTimes(vehicle(500, 490), 490, new Stop[] {}, null, precedingStayTimes))
        .containsExactly(10, 10);

    // final stay task half completed - vehicle slack time is 5
    assertThat(computeSlackTimes(vehicle(500, 490), 495, new Stop[] {}, null, precedingStayTimes))
        .containsExactly(5, 5);

    // final stay task just completed - vehicle slack time is 0
    assertThat(computeSlackTimes(vehicle(500, 490), 500, new Stop[] {}, null, precedingStayTimes))
        .containsExactly(0, 0);

    // final stay task started, but delayed - vehicle slack time is 0
    assertThat(computeSlackTimes(vehicle(500, 510), 510, new Stop[] {}, null, precedingStayTimes))
        .containsExactly(0, 0);

    // final stay task planned after vehicle end time - vehicle slack time is 0s
    assertThat(computeSlackTimes(vehicle(500, 510), 300, new Stop[] {}, null, precedingStayTimes))
        .containsExactly(0, 0);
  }
  ...
  public void computeSlackTimes_withStart() {
    ...
    // start without stop
    assertThat(
            computeSlackTimes(vehicle(500, 450), 100, new Stop[] {}, stop0, noPrecedingStayTimes))
        .containsExactly(30, 50);

    // start without stop
    assertThat(
            computeSlackTimes(vehicle(500, 450), 100, new Stop[] {}, stop1, noPrecedingStayTimes))
        .containsExactly(30, 50);

    // start with stop
    assertThat(
            computeSlackTimes(
                vehicle(500, 450), 100, new Stop[] {stop1}, stop0, onePrecedingStayTime))
        .containsExactly(30, 30, 50);
  }

  @Test
  public void computeSlackTimes_withPrecedingStayTimes() {
    final List<Double> precedingStayTimes =
        Arrays.asList( //
            0.0, //
            33.0 // second stop is a prebooked pickup, so slack for insertion after first stop is
            // longer
            );
    ...
    assertThat(
            computeSlackTimes(
                vehicle(500, 450), 100, new Stop[] {stop0, stop1}, null, precedingStayTimes))
        .containsExactly(20, 20, 63, 50);

    // final stay task not started - vehicle slack time is 25 and limits the slack times at stop1
    assertThat(
            computeSlackTimes(
                vehicle(500, 475), 100, new Stop[] {stop0, stop1}, null, precedingStayTimes))
        .containsExactly(20, 20, 58, 25);
    ...
    assertThat(
            computeSlackTimes(
                vehicle(500, 490), 100, new Stop[] {stop0, stop1}, null, precedingStayTimes))
        .containsExactly(20, 20, 43, 10);
  }

  private Stop stop(
      double beginTime, double latestArrivalTime, double endTime, double latestDepartureTime) {
    return new Stop(
        new DefaultDrtStopTask(beginTime, endTime, null),
        latestArrivalTime,
        latestDepartureTime,
        0);
  }
  ...
  private DvrpVehicle vehicle(double vehicleEndTime, double lastStayTaskBeginTime) {
    ...
    vehicle
        ...
        .addTask(
            new DrtStayTask(
                lastStayTaskBeginTime, Math.max(lastStayTaskBeginTime, vehicleEndTime), depot));
    return vehicle;
  }
}
