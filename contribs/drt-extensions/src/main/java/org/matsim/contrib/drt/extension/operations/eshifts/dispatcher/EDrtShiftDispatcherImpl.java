...
public class EDrtShiftDispatcherImpl implements DrtShiftDispatcher {
  ...
  private void checkChargingAtHub(double timeStep) {
    for (OperationFacility operationFacility :
        operationFacilities.getDrtOperationFacilities().values()) {
      ...
      for (Id<DvrpVehicle> vehicle : operationFacility.getRegisteredVehicles()) {
        ...
        if (dvrpVehicle instanceof EvShiftDvrpVehicle eShiftVehicle) {
          if (eShiftVehicle.getSchedule().getStatus() == Schedule.ScheduleStatus.STARTED) {
            ...
            if (electricVehicle.getBattery().getCharge()
                    / electricVehicle.getBattery().getCapacity()
                < drtShiftParams.chargeAtHubThreshold) {
              ...
              if (currentTask instanceof EDrtWaitForShiftStayTask
                ...
                Optional<Charger> selectedCharger =
                    chargerIds.stream()
                        ...
                        .filter(
                            charger ->
                                drtShiftParams.outOfShiftChargerType.equals(
                ...
                if (selectedCharger.isPresent()) {
                  ...
                  ChargingStrategy chargingStrategy =
                      selectedChargerImpl.getLogic().getChargingStrategy();
                  if (!chargingStrategy.isChargingCompleted(electricVehicle)) {
                    final double waitTime =
                        ChargingEstimations.estimateMaxWaitTimeForNextVehicle(selectedChargerImpl);
                    final double chargingTime =
                        chargingStrategy.calcRemainingTimeToCharge(electricVehicle);
                    double energy = -chargingStrategy.calcRemainingEnergyToCharge(electricVehicle);
                    final double endTime = timeStep + waitTime + chargingTime;
                    if (endTime < currentTask.getEndTime()) {
                      shiftTaskScheduler.chargeAtHub(
