...
public class EDrtModeOptimizerQSimModule extends AbstractDvrpModeQSimModule {
  ...
  protected void configureQSim() {
    addModalComponent(
        ...
        modalProvider(
            getter ->
                new EDrtOptimizer(
                    drtCfg,
                    getter.getModal(DefaultDrtOptimizer.class),
                    getter.getModal(EmptyVehicleChargingScheduler.class))));

    bindModal(DefaultDrtOptimizer.class)
        .toProvider(
            modalProvider(
                getter -> {
    ...
    bindModal(ChargingInfrastructure.class)
        .toProvider(
            modalProvider(
                getter ->
                    ChargingInfrastructureUtils.createModalNetworkChargers(
                        getter.get(ChargingInfrastructure.class),
    ...
    bindModal(DepotFinder.class)
        .toProvider(
            modalProvider(
                getter -> new NearestChargerAsDepot(getter.getModal(ChargingInfrastructure.class))))
        .asEagerSingleton();

    bindModal(EmptyVehicleChargingScheduler.class)
        .toProvider(
            new ModalProviders.AbstractProvider<>(drtCfg.getMode(), DvrpModes::mode) {
              @Inject private MobsimTimer timer;
              ...
              public EmptyVehicleChargingScheduler get() {
                ...
                return new EmptyVehicleChargingScheduler(
                    timer, taskFactory, chargingInfrastructure);
              }
            })
        .asEagerSingleton();

    bindModal(DrtRequestInsertionRetryQueue.class)
        .toInstance(
            new DrtRequestInsertionRetryQueue(
                drtCfg
                    .getDrtRequestInsertionRetryParams()
                    .orElse(new DrtRequestInsertionRetryParams())));

    addModalComponent(
        QSimScopeForkJoinPoolHolder.class,
    ...
    bindModal(VehicleEntry.EntryFactory.class)
        .toProvider(EDrtVehicleDataEntryFactory.EDrtVehicleDataEntryFactoryProvider.class)
        .asEagerSingleton();
    ...
    bindModal(CostCalculationStrategy.class)
        .to(
            drtCfg.rejectRequestIfMaxWaitOrTravelTimeViolated
                ...
                : CostCalculationStrategy.DiscourageSoftConstraintViolations.class)
        .asEagerSingleton();

    bindModal(DrtTaskFactory.class).toInstance(new EDrtTaskFactoryImpl());

    bindModal(EmptyVehicleRelocator.class)
        .toProvider(
            new ModalProviders.AbstractProvider<>(drtCfg.getMode(), DvrpModes::mode) {
              @Inject private MobsimTimer timer;
              ...
              public EmptyVehicleRelocator get() {
                ...
                return new EmptyVehicleRelocator(
                    network, travelTime, travelDisutility, timer, taskFactory);
              }
            })
        .asEagerSingleton();
    ...
    boolean scheduleWaitBeforeDrive =
        drtCfg.getPrebookingParams().map(p -> p.scheduleWaitBeforeDrive).orElse(false);
    bindModal(RequestInsertionScheduler.class)
        .toProvider(
            modalProvider(
                getter ->
                    new DefaultRequestInsertionScheduler(
                        getter.getModal(Fleet.class),
                        getter.get(MobsimTimer.class),
            ...
            modalProvider(
                getter ->
                    new ScheduleTimingUpdater(
                        getter.get(MobsimTimer.class),
                        new EDrtStayTaskEndTimeCalculator(
                            getter.getModal(StopTimeCalculator.class)))))
        .asEagerSingleton();
    ...
    bindModal(EDrtActionCreator.class)
        .toProvider(
            modalProvider(
                getter -> {
