...
public class EDrtOptimizer implements DrtOptimizer {
  ...
  public EDrtOptimizer(
      DrtConfigGroup drtConfigGroup,
      DefaultDrtOptimizer optimizer,
      EmptyVehicleChargingScheduler chargingScheduler) {
    Verify.verify(
        drtConfigGroup.idleVehiclesReturnToDepots,
        "DrtConfigGroup(mode=%s).idleVehiclesReturnToDepots is false."
            + " Recharging is possible only if idle vehicles return to depots",
  ...
  public void nextTask(DvrpVehicle vehicle) {
    ...
    if (schedule.getStatus() == ScheduleStatus.STARTED // only active vehicles
      ...
      // XXX if all vehicles are at depots at the very beginning, they will all start charging
      // (incl. queueing for charging)
      // since charging is uninterruptible, all not fully charged vehicles (could be the whole
      // fleet) would get frozen
      // to prevent this -- do not allow opportunistic charging when switching to the first (STAY)
      // task
