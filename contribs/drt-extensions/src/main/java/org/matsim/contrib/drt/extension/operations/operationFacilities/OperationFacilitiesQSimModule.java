...
public class OperationFacilitiesQSimModule extends AbstractDvrpModeQSimModule {
  ...
  protected void configureQSim() {
    ...
    bindModal(OperationFacilityFinder.class)
        .toProvider(
            modalProvider(
                getter ->
                    new NearestOperationFacilityWithCapacityFinder(
                        getter.getModal(OperationFacilities.class))))
        .asEagerSingleton();

    bindModal(OperationFacilities.class)
        .toProvider(
            new ModalProviders.AbstractProvider<>(getMode(), DvrpModes::mode) {

              @Override
              public OperationFacilities get() {
                OperationFacilitiesSpecification operationFacilitiesSpecification =
                    getModalInstance(OperationFacilitiesSpecification.class);
                ImmutableMap<Id<OperationFacility>, OperationFacility> operationFacilities =
                    operationFacilitiesSpecification
    ...
    addModalQSimComponentBinding()
        .toProvider(
            modalProvider(
                dvrpModeInstanceGetter -> {
                  List<OperationFacility> facilitiesList =
                      new ArrayList<>(
                          dvrpModeInstanceGetter
                              .getModal(OperationFacilities.class)
                              .getDrtOperationFacilities()
                              .values());
                  var header =
                      facilitiesList.stream().map(f -> f.getId() + "").collect(toImmutableList());
                  ProfileCalculator profileCalculator =
                      () ->
                          facilitiesList.stream()
                              .collect(
                                  toImmutableMap(
                                      f -> f.getId() + "",
                                      f -> (double) f.getRegisteredVehicles().size()));
                  return new TimeProfileCollector(
                      header,
                      profileCalculator,
                      300,
