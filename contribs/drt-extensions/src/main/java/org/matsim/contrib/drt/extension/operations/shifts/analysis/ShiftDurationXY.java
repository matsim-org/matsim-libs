...
public class ShiftDurationXY
  ...
  private final Map<Id<DrtShift>, Tuple<Double, Double>> shift2plannedVsActualDuration =
      new HashMap<>();
  private final Map<Id<DrtShift>, Tuple<Double, Double>> shift2plannedVsActualBreakDuration =
      new HashMap<>();
  ...
  public void handleEvent(final DrtShiftEndedEvent event) {
    ...
    final DrtShiftSpecification drtShift =
        shifts.get().getShiftSpecifications().get(event.getShiftId());
  ...
  public void handleEvent(DrtShiftBreakEndedEvent event) {
    ...
    double duration = event.getTime() - start;
    final DrtShiftBreakSpecification drtShift =
        shifts.get().getShiftSpecifications().get(event.getShiftId()).getBreak().orElseThrow();
    double plannedDuration = drtShift.getDuration();
    shift2plannedVsActualBreakDuration.put(
        event.getShiftId(), new Tuple<>(plannedDuration, duration));
  }
