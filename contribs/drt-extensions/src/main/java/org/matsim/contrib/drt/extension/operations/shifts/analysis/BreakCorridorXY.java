 ...
 */
public class BreakCorridorXY
    implements DrtShiftBreakStartedEventHandler, DrtShiftBreakEndedEventHandler {

  private final Provider<DrtShiftsSpecification> shifts;
  private final Map<Id<DrtShift>, Tuple<Double, Double>> shift2plannedVsActualBreakStart =
      new HashMap<>();
  private final Map<Id<DrtShift>, Tuple<Double, Double>> shift2plannedVsActualBreakEnd =
      new HashMap<>();
  ...
  public void handleEvent(DrtShiftBreakStartedEvent event) {
    final DrtShiftBreakSpecification drtShiftBreak =
        shifts.get().getShiftSpecifications().get(event.getShiftId()).getBreak().orElseThrow();
    final double earliestBreakStartTime = drtShiftBreak.getEarliestBreakStartTime();
    shift2plannedVsActualBreakStart.put(
        event.getShiftId(), new Tuple<>(earliestBreakStartTime, event.getTime()));
  }
  ...
  public void handleEvent(DrtShiftBreakEndedEvent event) {
    ...
    shift2plannedVsActualBreakEnd.put(
        event.getShiftId(), new Tuple<>(latestBreakEndTime, event.getTime()));
  }
  ...
  public void write(final PrintStream stream) {
    stream.print(
        "break of shift\tearliest break start\tlatest break end\tactual start\tactual end");
    stream.print("\n");
