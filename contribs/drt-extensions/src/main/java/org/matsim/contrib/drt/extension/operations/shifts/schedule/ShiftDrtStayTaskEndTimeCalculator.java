 ...
 */
public class ShiftDrtStayTaskEndTimeCalculator
    implements ScheduleTimingUpdater.StayTaskEndTimeCalculator {

  public static final Logger logger = LogManager.getLogger(ShiftDrtStayTaskEndTimeCalculator.class);

  private final ShiftsParams drtShiftParams;
  private final DrtStayTaskEndTimeCalculator delegate;

  public ShiftDrtStayTaskEndTimeCalculator(
      ShiftsParams drtShiftParams, DrtStayTaskEndTimeCalculator delegate) {
  ...
  public double calcNewEndTime(DvrpVehicle vehicle, StayTask task, double newBeginTime) {
    if (task instanceof ShiftBreakTask) {
      ...
      return Math.max(newBeginTime, ((ShiftChangeOverTask) task).getShift().getEndTime())
          + drtShiftParams.changeoverDuration;
