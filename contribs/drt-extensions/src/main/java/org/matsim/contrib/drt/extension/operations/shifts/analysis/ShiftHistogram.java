...
public class ShiftHistogram
  ...
  public void write(final PrintStream stream) {
    stream.print(
        "time\ttime\tshift_start\tshift_end\tshift_active\tshift_break_start\tshift_break_end\tshift_break_active");
    stream.print("\n");
    int active = 0;
    int activeBreaks = 0;
    ...
    for (int i = 0; i < allModesData.countsStart.length; i++) {
      ...
      activeBreaks =
          activeBreaks + allModesData.countsBreaksStart[i] - allModesData.countsBreaksEnd[i];
      stream.print(Time.writeTime(i * this.binSize) + "\t" + i * this.binSize);
      stream.print(
          "\t" + allModesData.countsStart[i] + "\t" + allModesData.countsEnd[i] + "\t" + active);
      stream.print(
          "\t"
              + allModesData.countsBreaksStart[i]
              + "\t"
              + allModesData.countsBreaksEnd[i]
              + "\t"
              + activeBreaks);
      // new line
      stream.print("\n");
