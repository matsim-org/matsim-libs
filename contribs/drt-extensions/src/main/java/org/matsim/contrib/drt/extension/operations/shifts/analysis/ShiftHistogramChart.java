...
public class ShiftHistogramChart {
  static JFreeChart getGraphic(final ShiftHistogram.DataFrame dataFrame, int iteration) {
    ...
    for (int i = 0; i < dataFrame.countsStart.length; i++) {
      ...
      active =
          active
              + dataFrame.countsStart[i]
              - dataFrame.countsEnd[i]
              - dataFrame.countsBreaksStart[i]
              + dataFrame.countsBreaksEnd[i];
  ...
  /**
   * Writes a graphic showing the number of shift starts, ends and active shifts to the specified
   * file.
   *
  ...
  public static void writeGraphic(ShiftHistogram shiftHistogram, final String filename) {
    try {
      ChartUtils.saveChartAsPNG(
          new File(filename),
          getGraphic(shiftHistogram.getData(), shiftHistogram.getIteration()),
          1024,
          768);
    } catch (IOException e) {
