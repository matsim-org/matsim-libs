...
public class ShiftTaskSchedulerImpl implements ShiftTaskScheduler {
  ...
  private static final Logger logger = LogManager.getLogger(ShiftTaskSchedulerImpl.class);
    ...
    if (currentTask instanceof DriveTask
      ...
      if (currentLink != link) {
        ...
        final double endTime =
            Math.min(startTime + drtShiftParams.changeoverDuration, vehicle.getServiceEndTime());
        if (path.getArrivalTime() > shift.getEndTime()) {
          logger.warn(
              "Shift changeover of shift "
                  + shift.getId()
  ...
  private void appendShiftChange(
    ...
    schedule.addTask(
        taskFactory.createWaitForShiftStayTask(
            vehicle, endTime, Math.max(endTime, vehicle.getServiceEndTime()), link, breakFacility));
  }
  ...
  private void updateShiftChangeImpl(
    ...
    final double endTime =
        Math.max(shift.getEndTime(), vrpPath.getArrivalTime()) + drtShiftParams.changeoverDuration;
    ShiftChangeOverTask dropoffStopTask =
        taskFactory.createShiftChangeoverTask(
            vehicle,
