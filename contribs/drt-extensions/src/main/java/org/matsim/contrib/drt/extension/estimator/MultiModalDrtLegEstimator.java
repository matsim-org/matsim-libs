...
/**
 * Aggregate class for informed-mode-choice that makes sure to invoke the correct estimator for each
 * drt mode.
 */
public class MultiModalDrtLegEstimator implements LegEstimator<ModeAvailability> {
  ...
  public double estimate(EstimatorContext context, String mode, Leg leg, ModeAvailability option) {
    ...
    DrtEstimator estimator =
        Objects.requireNonNull(
            estimators.get(mode),
            String.format("No drt estimator found for mode %s. Check warnings for errors.", mode));
