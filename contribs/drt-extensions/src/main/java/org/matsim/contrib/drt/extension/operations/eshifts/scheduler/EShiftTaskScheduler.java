...
public class EShiftTaskScheduler implements ShiftTaskScheduler {
  ...
  private static final Logger logger = LogManager.getLogger(EShiftTaskScheduler.class);
  ...
  public void relocateForBreak(
      ShiftDvrpVehicle vehicle, OperationFacility breakFacility, DrtShift shift) {
    ...
    if (currentTask instanceof DriveTask
      ...
      if (start != null) {
        ...
        start =
            new LinkTimePair(
                ((DriveTask) currentTask).getPath().getToLink(), currentTask.getEndTime());
        path = VrpPaths.calcAndCreatePath(start.link, toLink, start.time, router, travelTime);
        ...
        schedule.addTask(
            taskFactory.createDriveTask(
                vehicle, path, RELOCATE_VEHICLE_SHIFT_CHANGEOVER_TASK_TYPE)); // add RELOCATE
      }
      ...
      relocateForBreakImpl(
          vehicle, startTime, endTime, latestDetourArrival, toLink, shift, breakFacility);
      ...
      if (lastLink.getId() != breakFacility.getLinkId()) {
        ...
        VrpPathWithTravelData path =
            VrpPaths.calcAndCreatePath(lastLink, toLink, departureTime, router, travelTime);
        if (path.getArrivalTime() < vehicle.getServiceEndTime()) {
          ...
          schedule.addTask(
              taskFactory.createDriveTask(
                  vehicle, path, RELOCATE_VEHICLE_SHIFT_BREAK_TASK_TYPE)); // add RELOCATE
          double startTime = path.getArrivalTime();
          double endTime = startTime + shift.getBreak().orElseThrow().getDuration();
          double latestDetourArrival = path.getDepartureTime() + path.getTravelTime() * 1.5;
  ...
  private void relocateForBreakImpl(
      ShiftDvrpVehicle vehicle,
      double startTime,
      double endTime,
      double latestDetourArrival,
      Link link,
      DrtShift shift,
    ...
    if (charger.isPresent()) {
      ...
      final double waitTime = ChargingEstimations.estimateMaxWaitTimeForNextVehicle(chargerImpl);

      if (ev.getBattery().getCharge() / ev.getBattery().getCapacity()
              > shiftsParams.chargeDuringBreakThreshold
          || waitTime > 0) {
        dropoffStopTask =
            taskFactory.createShiftBreakTask(
                vehicle, startTime, endTime, link, shiftBreak, breakFacility);
      } else {
        double energyCharge =
            ((BatteryCharging) ev.getChargingPower())
                .calcEnergyCharged(chargerImpl.getSpecification(), endTime - startTime);
        double totalEnergy = -energyCharge;
        ((ChargingWithAssignmentLogic) chargerImpl.getLogic()).assignVehicle(ev);
        dropoffStopTask =
            ((ShiftEDrtTaskFactoryImpl) taskFactory)
                .createChargingShiftBreakTask(
                    vehicle,
                    startTime,
                    endTime,
                    link,
    ...
    final double latestTimeConstraintArrival =
  ...
  private Optional<Charger> charge(
      OperationFacility breakFacility, ElectricVehicle electricVehicle) {
    if (chargingInfrastructure != null) {
      List<Id<Charger>> chargerIds = breakFacility.getChargers();
      if (!chargerIds.isEmpty()) {
        Optional<Charger> selectedCharger =
            chargerIds.stream()
                ...
                .min(
                    (c1, c2) -> {
                      final double waitTime =
                          ChargingEstimations.estimateMaxWaitTimeForNextVehicle(c1);
                      final double waitTime2 =
                          ChargingEstimations.estimateMaxWaitTimeForNextVehicle(c2);
        ...
        if (selectedCharger.isPresent()) {
          if (selectedCharger
              ...
              .getLogic()
              .getChargingStrategy()
              .isChargingCompleted(electricVehicle)) {
  ...
  public void relocateForShiftChange(
      DvrpVehicle vehicle, Link link, DrtShift shift, OperationFacility breakFacility) {
    final Schedule schedule = vehicle.getSchedule();
    ...
    if (currentTask instanceof DriveTask
      ...
      if (path.getArrivalTime() > shift.getEndTime()) {
        logger.warn(
            "Shift changeover of shift "
                + shift.getId()
                + " will probably be delayed by "
                + (path.getArrivalTime() - shift.getEndTime())
                + " seconds.");
      }
      schedule.addTask(
          taskFactory.createStayTask(vehicle, path.getArrivalTime(), startTime, path.getToLink()));
      ...
      if (currentLink != link) {
        ...
        VrpPathWithTravelData path =
            VrpPaths.calcAndCreatePath(currentLink, link, departureTime, router, travelTime);
        ...
        schedule.addTask(
            taskFactory.createStayTask(
                vehicle, path.getArrivalTime(), startTime, path.getToLink()));
        appendShiftChange(vehicle, shift, breakFacility, startTime, endTime, link);
      } else {
  ...
  private void appendShiftChange(
      DvrpVehicle vehicle,
      DrtShift shift,
      OperationFacility breakFacility,
      double startTime,
    ...
    if (charger.isPresent()) {
      ...
      final double waitTime = ChargingEstimations.estimateMaxWaitTimeForNextVehicle(chargingImpl);

      if (ev.getBattery().getCharge() / ev.getBattery().getCapacity()
              < shiftsParams.chargeDuringBreakThreshold
          || ((ChargingWithAssignmentLogic) chargingImpl.getLogic())
              .getAssignedVehicles()
              .contains(ev)
          || waitTime > 0) {
        dropoffStopTask =
            taskFactory.createShiftChangeoverTask(
                vehicle, startTime, endTime, link, shift, breakFacility);
      } else {
        double energyCharge =
            ((BatteryCharging) ev.getChargingPower())
                .calcEnergyCharged(chargingImpl.getSpecification(), endTime - startTime);
        double totalEnergy = -energyCharge;
        ((ChargingWithAssignmentLogic) chargingImpl.getLogic()).assignVehicle(ev);
        dropoffStopTask =
            ((ShiftEDrtTaskFactoryImpl) taskFactory)
                .createChargingShiftChangeoverTask(
                    vehicle,
                    startTime,
                    endTime,
                    link,
    ...
    schedule.addTask(dropoffStopTask);
    schedule.addTask(
        taskFactory.createWaitForShiftStayTask(
            vehicle, endTime, vehicle.getServiceEndTime(), link, breakFacility));
  }
  ...
  public void startShift(ShiftDvrpVehicle vehicle, double now, DrtShift shift) {
    ...
    if (stayTask instanceof WaitForShiftStayTask) {
      ...
      schedule.addTask(
          taskFactory.createStayTask(vehicle, now, shift.getEndTime(), stayTask.getLink()));
    } else {
      throw new IllegalStateException(
          "Vehicle cannot start shift during task:" + stayTask.getTaskType().name());
    }
  }

  public boolean updateShiftChange(
      ShiftDvrpVehicle vehicle,
      Link link,
      DrtShift shift,
      LinkTimePair start,
    ...
    if (start.link != link) {
      ...
      VrpPathWithTravelData path =
          VrpPaths.calcAndCreatePath(
              start.link, link, Math.max(start.time, timer.getTimeOfDay()), router, travelTime);
      // if (path.getArrivalTime() <= shift.getEndTime()) {
  ...
  private void updateShiftChangeImpl(
      DvrpVehicle vehicle,
    ...
    if (oldChangeOver.isPresent() && oldChangeOver.get() instanceof EDrtShiftChangeoverTaskImpl) {
      if (((EDrtShiftChangeoverTaskImpl) oldChangeOver.get()).getChargingTask() != null) {
        ElectricVehicle ev = ((EvDvrpVehicle) vehicle).getElectricVehicle();
        ((EDrtShiftChangeoverTaskImpl) oldChangeOver.get())
            .getChargingTask()
            .getChargingLogic()
            .unassignVehicle(ev);
      }
    ...
    List<Task> copy =
        new ArrayList<>(
            schedule.getTasks().subList(lastTask.getTaskIdx() + 1, schedule.getTasks().size()));
    for (Task task : copy) {
    ...
    if (DrtTaskBaseType.getBaseTypeOrElseThrow(lastTask).equals(DRIVE)) {
      ...
      schedule.addTask(
          taskFactory.createDriveTask(
              vehicle, vrpPath, RELOCATE_VEHICLE_SHIFT_CHANGEOVER_TASK_TYPE)); // add RELOCATE
    }

    if (vrpPath.getArrivalTime() < shift.getEndTime()) {
      schedule.addTask(
          taskFactory.createStayTask(
              vehicle, vrpPath.getArrivalTime(), shift.getEndTime(), vrpPath.getToLink()));
    }
    // append SHIFT_CHANGEOVER task
    final double endTime =
        Math.max(shift.getEndTime(), vrpPath.getArrivalTime()) + shiftsParams.changeoverDuration;
    ShiftChangeOverTask dropoffStopTask =
        taskFactory.createShiftChangeoverTask(
            vehicle,
            Math.max(shift.getEndTime(), vrpPath.getArrivalTime()),
            endTime,
            vrpPath.getToLink(),
            shift,
            facility);
    ...
    schedule.addTask(
        taskFactory.createWaitForShiftStayTask(
            vehicle, endTime, vehicle.getServiceEndTime(), vrpPath.getToLink(), facility));
  }

  public void chargeAtHub(
      WaitForShiftStayTask currentTask,
      ShiftDvrpVehicle vehicle,
    ...
    final WaitForShiftStayTask chargingWaitForShiftStayTask =
        ((ShiftEDrtTaskFactoryImpl) taskFactory)
            .createChargingWaitForShiftStayTask(
                vehicle,
                beginTime,
                endTime,
                currentTask.getLink(),
                currentTask.getFacility(),
                energy,
                charger);

    final WaitForShiftStayTask waitForShiftStayTask =
        taskFactory.createWaitForShiftStayTask(
            vehicle, endTime, initialEndTime, currentTask.getLink(), currentTask.getFacility());
