...
public class ShiftRequestInsertionScheduler implements RequestInsertionScheduler {
  ...
  private DrtStopTask insertPickup(
    ...
    Task currentTask =
        scheduleStatus == Schedule.ScheduleStatus.PLANNED ? null : schedule.getCurrentTask();
    Task beforePickupTask;

    if (pickupIdx == 0
        && scheduleStatus != Schedule.ScheduleStatus.PLANNED
      ...
      if (stopTask != null
          && request.getFromLink() == stopTask.getLink()) { // no detour; no new stop task
        ...
        if (stopTask instanceof OperationalStop) {
          ...
          stopTask.setEndTime(
              stopTimeCalculator.updateEndTimeForPickup(
                  vehicleEntry.vehicle, stopTask, insertionTime, request.getRequest()));
        }
        ...
        if (pickupIdx == dropoffIdx) {
          ...
          if (pickupIdx < stops.size()) { // there is at least one following stop
            ...
            if (nextStopTask instanceof ShiftChangeOverTask) {
              if (stopTask.getTaskIdx() + 3
                  != nextStopTask.getTaskIdx()) { // there must a drive and stay task in
                // between
                throw new RuntimeException();
              }
            } else if (stopTask.getTaskIdx() + 2
                != nextStopTask.getTaskIdx()) { // there must a drive task in
              // between
              throw new RuntimeException();
            }
            if (nextStopTask instanceof ShiftChangeOverTask) {
              if (stopTask.getTaskIdx() + 3
                  == nextStopTask.getTaskIdx()) { // there must a drive task and stay in
                // between
        ...
        if (pickupIdx < stops.size()) { // there is at least one following stop
          ...
          if (nextStopTask instanceof ShiftChangeOverTask) {
            ...
            if (stayOrStopTask.getTaskIdx() + 2 != nextStopTask.getTaskIdx() //
                && stayTask != null
                && stayTask.getTaskIdx() + 1 != nextStopTask.getTaskIdx()) {
              throw new RuntimeException();
            }
            if (stayOrStopTask.getTaskIdx() + 2 == nextStopTask.getTaskIdx()) {
              // removing the drive task that is in between
    ...
    DrtStopTask pickupStopTask =
        taskFactory.createStopTask(
            vehicleEntry.vehicle, startTime, stopEndTime, request.getFromLink());
    ...
    // TODO should be enough to update the timeline only till dropoffIdx...
    scheduleTimingUpdater.updateTimingsStartingFromTaskIdx(
        vehicleEntry.vehicle, taskIdx + 2, driveFromPickupTask.getEndTime());
    ...
    if (dropoffIdx == stops.size()) { // bus stays at dropoff
      ...
      if (taskIdx + 1 == schedule.getTaskCount()) {
        ...
        double endTime =
            Math.max(
                beginTime,
                ((ShiftDvrpVehicle) vehicleEntry.vehicle).getShifts().peek().getEndTime());
        schedule.addTask(
            taskFactory.createStayTask(
                vehicleEntry.vehicle, beginTime, endTime, dropoffStopTask.getLink()));
      } else {
        throw new RuntimeException();
      }
    } else {
      ...
      if (nextStopTask instanceof ShiftChangeOverTask) {
        ...
        if (arrivalTime <= shiftEndTime) {
          DrtStayTask stayWaitShiftEndTask =
              taskFactory.createStayTask(
                  vehicleEntry.vehicle, arrivalTime, shiftEndTime, nextStopTask.getLink());
      ...
      // update timings
      scheduleTimingUpdater.updateTimingsStartingFromTaskIdx(
          vehicleEntry.vehicle, taskIdx + 2, driveFromDropoffTask.getEndTime());
    }
