...
public class ShiftInsertionCostCalculator implements InsertionCostCalculator {
  ...
  public ShiftInsertionCostCalculator(MobsimTimer timer, InsertionCostCalculator delegate) {
    this.timer = timer;
  ...
  public double calculate(
    ...
    if (!checkShiftTimeConstraintsForScheduledRequests(
        insertion,
        detourTimeInfo.pickupDetourInfo.pickupTimeLoss,
  ...
  boolean checkShiftTimeConstraintsForScheduledRequests(
      Insertion insertion, double pickupDetourTimeLoss, double totalTimeLoss) {
    ...
    // each existing stop has 2 time constraints: latestArrivalTime and latestDepartureTime (see:
    // Waypoint.Stop)
    // we are looking only at the time constraints of the scheduled requests (the new request is
    // checked separately)

    // all stops after the new (potential) pickup but before the new dropoff are delayed by
    ...
    for (int s = pickupIdx; s < dropoffIdx; s++) {
      ...
      if (stop.task instanceof ShiftBreakTask) {
        ...
        if (shiftBreak != null) {
          if (stop.task.getBeginTime() + pickupDetourTimeLoss
              > shiftBreak.getScheduledLatestArrival()) {
            return false;
          }
        }
