...
import org.matsim.contrib.dvrp.passenger.PassengerGroupIdentifier;
 ...
 final class DrtCompanionRideGenerator implements BeforeMobsimListener, AfterMobsimListener {
	 ...
	 private final DrtCompanionParams drtCompanionParams;
	 ...
	 DrtCompanionRideGenerator(final String drtMode,
		 ...
		 drtCompanionParams = drtWithExtensionsConfigGroup.getDrtCompanionParams().orElseThrow();
	 }
	 ...
	 private int limitCompanionsBySeats(int additionalCompanions, int maxCapacity) {
		 return drtCompanionParams.getLimitCompanionsBySeats()
			 ? Math.min(additionalCompanions, maxCapacity - 1) : additionalCompanions;
	 }
	 ...
	 private void addCompanionAgents() {
		 ...
		 for (Person person : this.scenario.getPopulation().getPersons().values()) {
			...
			for (TripStructureUtils.Trip trip : TripStructureUtils.getTrips(person.getSelectedPlan())) {
				int additionalCompanions = this.limitCompanionsBySeats(sampler.select(), maxCapacity);
