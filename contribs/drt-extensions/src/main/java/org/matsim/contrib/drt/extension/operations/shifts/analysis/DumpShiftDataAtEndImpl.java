...
/**
 * Dumps DRT shift related data at end. Based on {@link
 * org.matsim.core.controler.corelisteners.DumpDataAtEndImpl}
 *
 * @author nkuehnel / MOIA
 */
public final class DumpShiftDataAtEndImpl implements ShutdownListener {
  ...
  public DumpShiftDataAtEndImpl(
      Provider<DrtShiftsSpecification> shifts,
      OperationFacilitiesSpecification operationFacilities,
  ...
  private void dumpShiftPans() {
    try {
      if (this.shifts != null) {
        new DrtShiftsWriter(shifts.get())
            .writeFile(this.controlerIO.getOutputFilename("output_shifts.xml.gz"));
      }
    } catch (Exception ee) {
  ...
  private void dumpOperationFacilities() {
    try {
      if (this.operationFacilities != null) {
        new OperationFacilitiesWriter(operationFacilities)
            .writeFile(this.controlerIO.getOutputFilename("output_operationFacilities.xml.gz"));
      }
    } catch (Exception ee) {
