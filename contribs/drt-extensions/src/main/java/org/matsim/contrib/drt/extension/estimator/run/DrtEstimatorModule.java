...
public class DrtEstimatorModule extends AbstractModule {
  ...
  public void install() {
    ...
    MultiModeDrtEstimatorConfigGroup configs =
        ConfigUtils.addOrGetModule(getConfig(), MultiModeDrtEstimatorConfigGroup.class);
    ...
    for (DrtConfigGroup cfg : drtConfigs.getModalElements()) {
      ...
      estCfg.ifPresent(
          drtEstimatorConfigGroup -> install(new ModeModule(cfg, drtEstimatorConfigGroup)));
    }
  }
  ...
  static final class ModeModule extends AbstractDvrpModeModule {
    ...
    public void install() {
      ...
      if (group.estimator == DrtEstimatorConfigGroup.EstimatorType.BASIC) {
        bindModal(DrtEstimator.class)
            .toProvider(
                modalProvider(
                    getter ->
                        new BasicDrtEstimator(
                            getter.getModal(DrtEventSequenceCollector.class), group, cfg)))
            .in(Singleton.class);
      }
      ...
      bindModal(DrtEstimateAnalyzer.class)
          .toProvider(
              modalProvider(
                  getter ->
                      new DrtEstimateAnalyzer(
                          getter.getModal(DrtEstimator.class),
