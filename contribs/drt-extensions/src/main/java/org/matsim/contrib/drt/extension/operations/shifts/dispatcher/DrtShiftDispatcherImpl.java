...
public class DrtShiftDispatcherImpl implements DrtShiftDispatcher {
  ...
  private static final Logger logger = LogManager.getLogger(DrtShiftDispatcherImpl.class);
  ...
  public DrtShiftDispatcherImpl(
      DrtShifts shifts,
      Fleet fleet,
      MobsimTimer timer,
      OperationFacilities operationFacilities,
      OperationFacilityFinder breakFacilityFinder,
      ShiftTaskScheduler shiftTaskScheduler,
      Network network,
      EventsManager eventsManager,
      ShiftsParams drtShiftParams,
      ShiftStartLogic shiftStartLogic,
  ...
  private void createQueues() {
    ...
    for (OperationFacility facility : operationFacilities.getDrtOperationFacilities().values()) {
      PriorityQueue<ShiftDvrpVehicle> queue =
          new PriorityQueue<>(
              (v1, v2) ->
                  String.CASE_INSENSITIVE_ORDER.compare(
                      v1.getId().toString(), v2.getId().toString()));
      ...
      idleVehiclesQueues.put(facility.getId(), queue);
    }
  ...
  public void dispatch(double timeStep) {
    if (timeStep % drtShiftParams.loggingInterval == 0) {
      logger.info(
          String.format(
              "Active shifts: %s | Assigned shifts: %s | Unscheduled shifts: %s",
              activeShifts.size(), assignedShifts.size(), unscheduledShifts.size()));
      for (Map.Entry<Id<OperationFacility>, Queue<ShiftDvrpVehicle>> queueEntry :
          idleVehiclesQueues.entrySet()) {
        logger.info(
            String.format(
                "Idle vehicles at facility %s: %d",
                queueEntry.getKey().toString(), queueEntry.getValue().size()));
      }
    }
    endShifts(timeStep);
      ...
      if (shift != null && shift.isStarted()) {
        ...
        if (breakFacility != null) {
          ...
          eventsManager.processEvent(
              new DrtShiftBreakScheduledEvent(
                  timer.getTimeOfDay(),
                  shift.getId(),
                  activeShift.vehicle.getId(),
                  breakFacility.getLinkId(),
  ...
  private void startShifts(double timeStep) {
    ...
    while (iterator.hasNext()) {
      ...
      if (shiftStartLogic.shiftStarts(next)) {
        ...
        eventsManager.processEvent(
            new DrtShiftStartedEvent(
                timeStep, next.shift.getId(), next.vehicle.getId(), currentTask.getLink().getId()));
      }
    }
  }
  ...
  private void assignShifts(double timeStep) {
    ...
    while (!this.unscheduledShifts.isEmpty()
        && isSchedulable(this.unscheduledShifts.peek(), timeStep)) {
      ...
      for (ShiftEntry active : activeShifts) {
        ...
        if (shift.getOperationFacilityId().isPresent()) {
          ...
          if (active.shift.getOperationFacilityId().isPresent()) {
            if (!active
                .shift
                .getOperationFacilityId()
                .get()
                .equals(shift.getOperationFacilityId().get())) {
              continue;
            }
          } else {
            Optional<ShiftChangeOverTask> nextShiftChangeover =
                ShiftSchedules.getNextShiftChangeover(active.vehicle.getSchedule());
            if (nextShiftChangeover.isPresent()) {
              Verify.verify(nextShiftChangeover.get().getShift().equals(active.shift));
              if (!nextShiftChangeover
                  .get()
                  .getFacility()
                  .getId()
                  .equals(shift.getOperationFacilityId().get())) {
  ...
  private void assignShiftToVehicle(DrtShift shift, ShiftDvrpVehicle vehicle) {
    ...
    eventsManager.processEvent(
        new DrtShiftAssignedEvent(timer.getTimeOfDay(), shift.getId(), vehicle.getId()));
  }
  ...
  private void endShifts(double timeStep) {
    ...
    while (iterator.hasNext()) {
      ...
      logger.debug(
          "Scheduling shift end for shift "
              + next.shift.getId()
              + " of vehicle "
              + next.vehicle.getId());
  ...
  private void updateShiftEnd(ShiftEntry next) {
    ...
    if (start == null) {
      ...
      switch (getBaseTypeOrElseThrow(currentTask)) {
        case DRIVE:
          ...
          LinkTimePair diversionPoint =
              ((OnlineDriveTaskTracker) driveTask.getTaskTracker()).getDiversionPoint();
          // diversion possible
          start =
              diversionPoint != null
                  ? diversionPoint
    ...
    if (drtShiftParams.allowInFieldChangeover) {
      ...
      shiftChangeFacility =
          breakFacilityFinder.findFacilityOfType(start.link.getCoord(), OperationFacilityType.hub);
    }
    if (shiftChangeFacility != null
        && changeOverTask != null
        && !(shiftChangeFacility.getId().equals(changeOverTask.getFacility().getId()))) {
      if (shiftChangeFacility.hasCapacity()) {
        if (shiftTaskScheduler.updateShiftChange(
            next.vehicle,
            network.getLinks().get(shiftChangeFacility.getLinkId()),
            next.shift,
            start,
            shiftChangeFacility,
            lastTask)) {
          ...
          eventsManager.processEvent(
              new ShiftFacilityRegistrationEvent(
                  timer.getTimeOfDay(), next.vehicle.getId(), shiftChangeFacility.getId()));
        }
      }
    }
  }
  ...
  private void scheduleShiftEnd(ShiftEntry endingShift) {
    ...
    if (currentTask instanceof DriveTask
      ...
      lastLink = ((DrtStayTask) schedule.getTasks().get(schedule.getTaskCount() - 1)).getLink();
    }
    ...
    if (shiftChangeoverFacility == null) {
      shiftChangeoverFacility =
          breakFacilityFinder.findFacilityOfType(coord, OperationFacilityType.hub);
    }

    if (shiftChangeoverFacility != null
        && shiftChangeoverFacility.register(endingShift.vehicle.getId())) {
      shiftTaskScheduler.relocateForShiftChange(
          endingShift.vehicle,
          network.getLinks().get(shiftChangeoverFacility.getLinkId()),
          endingShift.shift,
          shiftChangeoverFacility);
      eventsManager.processEvent(
          new ShiftFacilityRegistrationEvent(
              timer.getTimeOfDay(), endingShift.vehicle.getId(), shiftChangeoverFacility.getId()));
    } else {
  ...
  private OperationFacility decideOnBreak(ShiftEntry activeShift) {
    if (activeShift.shift != null) {
      if (shiftNeedsBreak(activeShift.shift, timer.getTimeOfDay())) {
        ...
        final OperationFacility shiftBreakFacility =
            breakFacilityFinder.findFacility(lastLink.getCoord());
        ...
        if (shiftBreakFacility.register(activeShift.vehicle.getId())) {
          eventsManager.processEvent(
              new ShiftFacilityRegistrationEvent(
                  timer.getTimeOfDay(), activeShift.vehicle.getId(), shiftBreakFacility.getId()));
  ...
  public void endShift(
      ShiftDvrpVehicle vehicle, Id<Link> id, Id<OperationFacility> operationFacilityId) {
    final DrtShift shift = vehicle.getShifts().poll();
    logger.debug("Ended shift " + shift.getId());
    shift.end();
    eventsManager.processEvent(
        new DrtShiftEndedEvent(
            timer.getTimeOfDay(), shift.getId(), vehicle.getId(), id, operationFacilityId));
  ...
  public void endBreak(ShiftDvrpVehicle vehicle, ShiftBreakTask previousTask) {
    ...
    eventsManager.processEvent(
        new DrtShiftBreakEndedEvent(
            timer.getTimeOfDay(),
            vehicle.getShifts().peek().getId(),
            vehicle.getId(),
            previousTask.getFacility().getLinkId()));
  }

  public void startBreak(ShiftDvrpVehicle vehicle, Id<Link> linkId) {
    eventsManager.processEvent(
        new DrtShiftBreakStartedEvent(
            timer.getTimeOfDay(), vehicle.getShifts().peek().getId(), vehicle.getId(), linkId));
  }

  private boolean isSchedulable(DrtShift shift, double timeStep) {
    return shift.getStartTime()
        <= timeStep + drtShiftParams.shiftScheduleLookAhead; // && shift.getEndTime() > timeStep;
  }

  private boolean shiftNeedsBreak(DrtShift shift, double timeStep) {
    return shift.getBreak().isPresent()
        && shift.getBreak().get().getEarliestBreakStartTime() == timeStep
