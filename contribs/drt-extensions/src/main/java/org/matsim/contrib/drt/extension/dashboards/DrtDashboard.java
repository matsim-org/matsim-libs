...
public class DrtDashboard implements Dashboard {
  ...
  public DrtDashboard(
      DrtConfigGroup drtConfigGroup, URL matsimConfigContext, String crs, int lastIteration) {
  ...
  private String postProcess(Data data, String file) {
    List<String> args =
        new ArrayList<>(List.of("--drt-mode", drtConfigGroup.getMode(), "--input-crs", crs));

    // it might be, that a serviceArea file is provided in the config, but the drt service is
    // configured to be stopBased, nevertheless
    switch (drtConfigGroup.operationalScheme) {
      ...
      args.addAll(
          List.of(
              "--stops-file",
              ConfigGroup.getInputFileURL(matsimConfigContext, drtConfigGroup.transitStopFile)
                  .toString()));
      case door2door -> {
        // TODO potentially show the entire drt network (all drt links have stops)
      }
      case serviceAreaBased ->
      //				drtConfigGroup.drtServiceAreaShapeFile can not be null, otherwise simulation crashed,
      // earlier
      // this copies the input shape file into the output directory. might not be ideal. but the
      // input file might be anywhere (web, different local partition, ...) and simwrapper might not
      // have access....
      args.addAll(
          List.of(
              "--area-file",
              ConfigGroup.getInputFileURL(
                      matsimConfigContext, drtConfigGroup.drtServiceAreaShapeFile)
                  .toString()));
    }
  ...
  public void configure(Header header, Layout layout) {
    ...
    layout
        .row("Spatial demand distribution")
        .el(
            Hexagons.class,
            (viz, data) -> {
        ...
        .el(
            ...
            (viz, data) -> {
              ...
              viz.file =
                  data.output(
                      "ITERS/it." + lastIteration + "/*rejections_" + drtConfigGroup.mode + ".csv");
              viz.addAggregation(
                  "rejections", "origins", "fromX", "fromY", "destinations", "toX", "toY");
    ...
    // Final Demand stats
    layout
        .row("Final Demand And Wait Time Statistics")
        .el(
            Plotly.class,
            (viz, data) -> {
              viz.title = "Final Demand and Wait Stats over day time";
              viz.description =
                  "Number of rides (customers) is displayed in bars, wait statistics in lines";

              Plotly.DataSet waitStats =
                  viz.addDataset(data.output("*_waitStats_" + drtConfigGroup.mode + ".csv"));
              Plotly.DataSet rejections =
                  viz.addDataset(
                      data.output("*drt_rejections_perTimeBin_" + drtConfigGroup.mode + ".csv"));
              ...
              viz.addTrace(
                  ScatterTrace.builder(Plotly.INPUT, Plotly.INPUT)
                      ...
                      .name("Average")
                      .build(),
                  waitStats.mapping().x("timebin").y("average_wait"));
              ...
              viz.addTrace(
                  ScatterTrace.builder(Plotly.INPUT, Plotly.INPUT)
                      ...
                      .name("P5")
                      .build(),
                  waitStats.mapping().x("timebin").y("p_5"));
              ...
              viz.addTrace(
                  ScatterTrace.builder(Plotly.INPUT, Plotly.INPUT)
                      ...
                      .name("P95")
                      .build(),
                  waitStats.mapping().x("timebin").y("p_95"));
              ...
              viz.addTrace(
                  BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT)
                      ...
                      .name("Rides")
                      .build(),
                  waitStats.mapping().x("timebin").y("legs"));
              ...
              viz.addTrace(
                  BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT)
                      ...
                      .name("Rejections")
                      .build(),
                  rejections.mapping().x("timebin").y("rejections"));
            })
        .el(
            Area.class,
            (viz, data) -> {
    ...
    if (lastIteration >= 3) {
      ...
      layout
          .row("Demand and Wait Time Statistics per iteration")
          .el(
              Plotly.class,
              (viz, data) -> {
                viz.title = "Rides and rejections per iteration";
                viz.description =
                    "Number of rides (customers) and rejections over the course of the simulation";

                Plotly.DataSet dataset =
                    viz.addDataset(data.output("*_customer_stats_" + drtConfigGroup.mode + ".csv"));
                ...
                viz.addTrace(
                    BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT).name("Rejections").build(),
                    dataset.mapping().x("iteration").y("rejections")
                    //						.color(Plotly.ColorScheme.RdBu)
                    );

                viz.addTrace(
                    BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT).name("Rides").build(),
                    dataset.mapping().x("iteration").y("rides")
                    //						.color(Plotly.ColorScheme.RdBu)
                    );
              })
          .el(
      ...
      layout
          .row("Demand And Travel Time Statistics per iteration")
          .el(
              Plotly.class,
              (viz, data) -> {
                viz.title = "Travel time components per iteration";
                viz.description =
                    "Comparing mean wait vs. mean in-vehicle travel time per customer";
                ...
                viz.addTrace(
                    BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT)
                        ...
                        .name("In Vehicle")
                        .build(),
                    dataset.mapping().x("iteration").y("inVehicleTravelTime_mean")
                    //						.color(Plotly.ColorScheme.RdBu)
                    );
          ...
          .el(
              ...
              (viz, data) -> {
                ...
                viz.description =
                    "Number of "
                        + drtConfigGroup.mode
                        + " vehicles (customers) is displayed as bars, distance stats as lines";

                Plotly.DataSet dataset =
                    viz.addDataset(data.output("*_vehicle_stats_" + drtConfigGroup.mode + ".csv"));
                ...
                viz.addTrace(
                    ScatterTrace.builder(Plotly.INPUT, Plotly.INPUT)
                        ...
                        .name("Pax Dist.")
                        .build(),
                    dataset.mapping().x("iteration").y("totalPassengerDistanceTraveled"));
                ...
                viz.addTrace(
                    ScatterTrace.builder(Plotly.INPUT, Plotly.INPUT)
                        ...
                        .build(),
                    dataset.mapping().x("iteration").y("totalDistance"));
                ...
                viz.addTrace(
                    ScatterTrace.builder(Plotly.INPUT, Plotly.INPUT)
                        ...
                        .name("Empty mileage")
                        .build(),
                    dataset.mapping().x("iteration").y("totalEmptyDistance"));
                ...
                viz.addTrace(
                    BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT)
                        ...
                        .name("Vehicles")
                        .build(),
                    dataset.mapping().x("iteration").y("vehicles")
                    //						.color(Plotly.ColorScheme.RdBu)
                    );
              })
          .el(
              ...
              (viz, data) -> {
                ...
                viz.description =
                    "Pooling ratio (Pax distance / Vehicle mileage), Detour ratio, and Empty Ratio";
                viz.x = "iteration";
