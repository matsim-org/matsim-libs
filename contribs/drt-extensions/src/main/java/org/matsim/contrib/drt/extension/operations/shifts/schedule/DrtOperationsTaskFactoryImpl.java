 ...
 */
public class DrtOperationsTaskFactoryImpl implements DrtOperationsTaskFactory {
    ...
    public DrtOperationsTaskFactoryImpl(DrtTaskFactory delegate, OperationFacilities operationFacilities,
    ...
    @Override
    public IncidentHoldTask createIncidentHoldTask(DvrpVehicle vehicle, double beginTime, double endTime, Link link) {
        // A held vehicle is stationary: no drive energy, only time-dependent auxiliary consumption. For an electric
        // vehicle, compute that aux energy over the hold from its aux consumption model (mirrors how EDrtStopTask does
        // it); for a non-electric vehicle it is simply zero. Note the hold's end may still be extended while the
        // incident queues, so this reflects the aux energy for the hold's currently-known duration.
        double consumedEnergy = 0.0;
        if (vehicle instanceof EvDvrpVehicle evVehicle) {
            consumedEnergy = evVehicle.getElectricVehicle().getAuxEnergyConsumption()
                    .calcEnergyConsumption(beginTime, endTime - beginTime, link.getId());
        }
        return new IncidentHoldTask(beginTime, endTime, link, consumedEnergy);
    }
