...
import org.matsim.contrib.drt.extension.operations.operationFacilities.OperationFacilitySpecification;
...
public class OperationFacilitiesWriter extends MatsimXmlWriter {
  ...
  private static final Logger log = LogManager.getLogger(OperationFacilitiesWriter.class);

  private static final String ROOT = "facilities";
  ...
  private void writeShifts(Map<Id<OperationFacility>, OperationFacilitySpecification> facilities)
      throws UncheckedIOException, IOException {
    List<OperationFacilitySpecification> sortedFacilities =
    ...
    for (OperationFacilitySpecification facility : sortedFacilities) {
      ...
      if (!facility.getChargers().isEmpty()) {
        ...
        for (Id<Charger> charger : facility.getChargers()) {
          this.writeStartTag(
              CHARGER, Lists.newArrayList(new Tuple<>(CHARGER_ID, charger.toString())), true);
        }
