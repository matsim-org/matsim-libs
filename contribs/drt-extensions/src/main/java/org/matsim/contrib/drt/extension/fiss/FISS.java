 ...
 *
 * <p>The aim is to only assign a specified fraction of vehicular agents and teleport the rest. This
 * is achieved by making use of a VehicularDepartureHandler (so far not enforced, as the
 * implementation is not exposed, so in theory could be any DepartureHandler) and a
 * TeleportationEngine. Both used as delegates.
 *
 * <p>Transit driver agents are always assigned. In addition does not handle DynAgents (e.g., DVRP
 * agents)
 *
 * <p>Also implements MobsimEngine to delegate required teleportation steps (such as arrivals).
 *
...
public class FISS implements DepartureHandler, MobsimEngine {
  ...
  public boolean handleDeparture(double now, MobsimAgent agent, Id<Link> linkId) {
    if (this.fissConfigGroup.sampledModes.contains(agent.getMode())) {
      if (random.nextDouble() < fissConfigGroup.sampleFactor
          || agent instanceof TransitDriverAgent
          || this.switchOffFISS()) {
        ...
        if (!(agent instanceof DynAgent)) {
            ...
            for (Id<Link> routeLinkId : networkRoute.getLinkIds()) {
              newTravelTime +=
                  this.travelTime.getLinkTravelTime(
                      network.getLinks().get(routeLinkId), now + newTravelTime, null, null);
            }
            LOG.debug(
                "New travelTime: {}, was {}",
                newTravelTime,
          ...
          if (removedVehicle != null) {
            ...
            QLinkI qLinkDest =
                (QLinkI) this.qNetsimEngine.getNetsimNetwork().getNetsimLink(destinationLinkId);
    ...
    for (String sampledQsimModes : fissConfigGroup.sampledModes) {
      VehicleType vehicleType =
          scenario
              .getVehicles()
              .getVehicleTypes()
              .get(Id.create(sampledQsimModes, VehicleType.class));
  ...
  private boolean switchOffFISS() {
    return (this.fissConfigGroup.switchOffFISSLastIteration
        && this.matsimServices.getConfig().controller().getLastIteration()
            == this.matsimServices.getIterationNumber());
  }
