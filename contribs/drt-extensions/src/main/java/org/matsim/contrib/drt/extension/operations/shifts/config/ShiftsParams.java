...
public class ShiftsParams extends ReflectiveConfigGroupWithConfigurableParameterSets {
  ...
  @Parameter
  @Comment(
      "Time of shift assignment (i.e. which vehicle carries out a specific shift) before start of shift in [seconds]")
  public double shiftScheduleLookAhead = 1800;

  @Parameter
  @Comment(
      "Time of shift end scheduling (i.e. plan shift end location) before end of shift in [seconds]")
  public double shiftEndLookAhead = 3600;

  @Parameter
  @Comment(
      "Time of shift end rescheduling  (i.e. check whether shift should end"
          + " at a different facillity) before end of shift in [seconds]")
  public double shiftEndRescheduleLookAhead = 1800;

  @Parameter
  @Comment(
      "Time interval for periodic check of shift end re-scheduling for shifts ending within the "
          + "'shiftEndRescheduleLookAhead' in [seconds]")
  public double updateShiftEndInterval = 60 * 3;

  @Parameter
  @Comment(
      "set to true if shifts can start and end at in field operational facilities,"
  ...
  @Parameter
  @Comment(
      "defines the battery state of charge threshold at which vehicles will start charging"
  ...
  @Parameter
  @Comment(
      "defines the interval at which idle vehicles at operation facilities are checked for whether"
          + "they can and should start charging. In [seconds]")
  public double chargeAtHubInterval = 3 * 60;

  @Parameter
  @Comment(
      "defines the minimum battery state of charge threshold at which vehicles are available "
          + " for shift assignment. values between [0,1)")
  public double shiftAssignmentBatteryThreshold = 0.6;

  @Parameter
  @Comment(
      "defines the charger type that should be chosen when charging during shift break or changeover. "
          + "Defaults to '"
          + ChargerSpecification.DEFAULT_CHARGER_TYPE
          + "'")
  public String breakChargerType = ChargerSpecification.DEFAULT_CHARGER_TYPE;

  @Parameter
  @Comment(
      "defines the charger type that should be chosen when charging inactive vehicles outside of shifts. "
          + "Defaults to '"
          + ChargerSpecification.DEFAULT_CHARGER_TYPE
          + "'")
