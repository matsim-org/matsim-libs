...
public final class ShiftEfficiencyAnalysisControlerListener implements IterationEndsListener {
  ...
  @Inject
  public ShiftEfficiencyAnalysisControlerListener(
  ...
  private void writeAndPlotShiftEfficiency(
    ...
    try (var bw = IOUtils.getBufferedWriter(csvFile)) {
      ...
      for (Map.Entry<Id<DrtShift>, Double> revenuePerShiftEntry : revenuePerShift.entrySet()) {
        DrtShiftSpecification drtShift =
            drtShiftsSpecification
                .get()
                .getShiftSpecifications()
                .get(revenuePerShiftEntry.getKey());
        int nRequests =
            requestsPerShift
                .getOrDefault(revenuePerShiftEntry.getKey(), Collections.EMPTY_LIST)
                .size();
        ...
        if (dvrpVehicleId != null) {
          bw.append(
              line(
                  drtShift.getId().toString(),
                  drtShift.getStartTime(),
                  drtShift.getEndTime(),
                  dvrpVehicleId.toString(),
                  nRequests,
                  revenuePerShiftEntry.getValue(),
                  ridesPerVRH,
                  revenuePerVRH));
      ...
      if (createGraphs) {
        ...
        JFreeChart chartRides =
            ChartFactory.createBoxAndWhiskerChart(
                "Rides per VRH distribution", "", "Rides", ridesPerVRHDataset, false);
        JFreeChart chartRevenue =
            ChartFactory.createBoxAndWhiskerChart(
                "Revenue per VRH distribution", "", "Revenue", revenuePerVRHDataset, false);

        ((BoxAndWhiskerRenderer) chartRides.getCategoryPlot().getRenderer()).setMeanVisible(false);
        ((BoxAndWhiskerRenderer) chartRevenue.getCategoryPlot().getRenderer())
