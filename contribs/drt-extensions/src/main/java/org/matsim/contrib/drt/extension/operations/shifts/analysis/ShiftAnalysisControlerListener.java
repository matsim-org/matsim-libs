...
public class ShiftAnalysisControlerListener implements IterationEndsListener {
  ...
  private static final Logger logger = LogManager.getLogger(ShiftAnalysisControlerListener.class);
  ...
  @Inject
  public ShiftAnalysisControlerListener(
      Config config,
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    writeAndPlotShiftDurationComparison(
        shiftDurationXY.getShift2plannedVsActualDuration(),
        filename(event, "shiftDurationComparison", ".png"),
        filename(event, "shiftDurationComparison", ".csv"),
        createGraphs);
    writeAndPlotBreakDurationComparison(
        shiftDurationXY.getShift2plannedVsActualBreakDuration(),
        filename(event, "shiftBreakDurationComparison", ".png"),
        filename(event, "shiftBreakDurationComparison", ".csv"),
        createGraphs);
    writeAndPlotBreakTimesComparison(
        breakCorridorXY.getShift2plannedVsActualBreakStart(),
  ...
  private void writeAndPlotBreakTimesComparison(
      Map<Id<DrtShift>, Tuple<Double, Double>> shift2plannedVsActualBreakStart,
    ...
    try (var bw = IOUtils.getBufferedWriter(csvFile)) {
      ...
      bw.append(
          line("ShiftId", "earliestBreakStart", "latestBreakEnd", "actualStart", "actualEnd"));
      for (Map.Entry<Id<DrtShift>, Tuple<Double, Double>> entry :
          shift2plannedVsActualBreakStart.entrySet()) {
        final Tuple<Double, Double> breakEndTuple =
            shift2plannedVsActualBreakEnd.get(entry.getKey());
        if (breakEndTuple != null) {
          bw.append(
              line(
                  entry.getKey(),
                  entry.getValue().getFirst(),
                  breakEndTuple.getFirst(),
                  entry.getValue().getSecond(),
                  breakEndTuple.getSecond()));
      ...
      if (createGraphs) {
        final JFreeChart chart2 =
            DensityScatterPlots.createPlot(
                "Break end times", "Planned break end time", "Actual break end time", times, null);
  ...
  private void writeAndPlotShiftDurationComparison(
      Map<Id<DrtShift>, Tuple<Double, Double>> shift2plannedVsActualDuration,
      String plotFilename,
      String csvFileName,
      boolean createGraphs) {
    try (var bw = IOUtils.getBufferedWriter(csvFileName)) {
      ...
      for (Map.Entry<Id<DrtShift>, Tuple<Double, Double>> entry :
          shift2plannedVsActualDuration.entrySet()) {

        double actualDuration = entry.getValue().getSecond();
        double plannedDuration = entry.getValue().getFirst();
        bw.append(
            line(
                entry.getKey(), actualDuration, plannedDuration, actualDuration - plannedDuration));
      ...
      if (createGraphs) {
        final JFreeChart chart2 =
            DensityScatterPlots.createPlot(
                "Shift durations",
                "Actual shift duration [s]",
                "Initially planned shift duration [s]",
                times,
                null);
  ...
  private void writeAndPlotBreakDurationComparison(
      Map<Id<DrtShift>, Tuple<Double, Double>> shift2plannedVsActualBreakDuration,
      String plotFilename,
      String csvFileName,
      boolean createGraphs) {
    try (var bw = IOUtils.getBufferedWriter(csvFileName)) {
      ...
      bw.append(line("ShiftId", "actualDuration", "plannedDuration", "deviate"));
      for (Map.Entry<Id<DrtShift>, Tuple<Double, Double>> entry :
          shift2plannedVsActualBreakDuration.entrySet()) {
      ...
      if (createGraphs) {
        final JFreeChart chart2 =
            DensityScatterPlots.createPlot(
                "Break durations",
                "Actual break duration [s]",
                "Initially planned break duration [s]",
                times,
                null);
        ChartUtils.writeChartAsPNG(new FileOutputStream(plotFilename), chart2, 1500, 1500);
      }
