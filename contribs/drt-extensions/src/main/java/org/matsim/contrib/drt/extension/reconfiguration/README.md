...
## How to use ?
We describe below the steps to follow to implement two example tests. If you want to look at code examples, they are present in `CapacityReconfigurationTest`
...
5. Bind the proposed `DefaultCapacityReconfigurationLogic` or bind your own `CapacityReconfigurationLogic` implementation, an example is shown in `CapacityReconfigurationTest.SimpleCapacityConfiguration` that configures all vehicles to switch capacities at 12pm.
