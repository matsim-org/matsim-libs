...
public class RunFissDrtScenarioIT {
  ...
  public void test() {
    ...
    MultiModeDrtConfigGroup multiModeDrtConfigGroup =
        new MultiModeDrtConfigGroup(DrtWithOperationsConfigGroup::new);
    ...
    DrtWithOperationsConfigGroup drtWithShiftsConfigGroup =
        (DrtWithOperationsConfigGroup) multiModeDrtConfigGroup.createParameterSet("drt");
    ...
    MinCostFlowRebalancingStrategyParams strategyParams =
        new MinCostFlowRebalancingStrategyParams();
    ...
    final Config config = ConfigUtils.createConfig(multiModeDrtConfigGroup, new DvrpConfigGroup());
    ...
    final ScoringConfigGroup.ActivityParams education =
        new ScoringConfigGroup.ActivityParams("education");
    education.setTypicalDuration(6 * 3600);
    final ScoringConfigGroup.ActivityParams shopping =
        new ScoringConfigGroup.ActivityParams("shopping");
    ...
    final ReplanningConfigGroup.StrategySettings stratSets =
    ...
    DrtOperationsParams operationsParams =
        (DrtOperationsParams)
            drtWithShiftsConfigGroup.createParameterSet(DrtOperationsParams.SET_NAME);
    ShiftsParams shiftsParams =
        (ShiftsParams) operationsParams.createParameterSet(ShiftsParams.SET_NAME);
    OperationFacilitiesParams operationFacilitiesParams =
        (OperationFacilitiesParams)
            operationsParams.createParameterSet(OperationFacilitiesParams.SET_NAME);
    ...
    LinkCounter linkCounter = new LinkCounter();
    {
      ...
      QSimComponentsConfigurator qSimComponentsConfigurator =
          FISSConfigurator.activateModes(
              List.of(), MultiModeDrtConfigGroup.get(config).modes().collect(Collectors.toList()));

      run.configureQSimComponents(qSimComponentsConfigurator);

      run.addOverridingModule(
          new AbstractModule() {
            @Override
            public void install() {
