...
public class ShiftEfficiencyTest {
  ...
  public void testDrtShiftEfficiency() {
    ...
    Id<OperationFacility> operationFacility1 =
        Id.create("operationFacility1", OperationFacility.class);
    ...
    events.processEvent(
        new PassengerDroppedOffEvent(11 * 3600, "drt", request1, person1, vehicle1));
    Assert.assertTrue(
    ...
    events.processEvent(
        new PersonMoneyEvent(
            ...
            person1,
            -FARE,
            DrtFareHandler.PERSON_MONEY_EVENT_PURPOSE_DRT_FARE,
            "drt",
            request1.toString()));
    Assert.assertEquals(
        FARE,
        shiftEfficiencyTracker.getCurrentRecord().getRevenueByShift().get(shift1),
        MatsimTestUtils.EPSILON);

    Assert.assertFalse(
        shiftEfficiencyTracker.getCurrentRecord().getFinishedShifts().containsKey(shift1));
    events.processEvent(
        new DrtShiftEndedEvent(20 * 3600, shift1, vehicle1, link1, operationFacility1));
    Assert.assertTrue(
        shiftEfficiencyTracker.getCurrentRecord().getFinishedShifts().containsKey(shift1));
  }
}
