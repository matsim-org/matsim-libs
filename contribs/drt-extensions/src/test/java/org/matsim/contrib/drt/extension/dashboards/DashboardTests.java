...
public class DashboardTests {
  ...
  private void run() {
    ...
    // we have 2 operators ('av' + 'drt'), configure one of them to be areaBased (the other remains
    // stopBased)
  ...
  public void drtDefaults() {
    ...
    Path drtOutputPath = Path.of(utils.getOutputDirectory(), "analysis", "drt");
    {
      ...
      Assertions.assertThat(drtOutputPath).isDirectoryContaining("glob:**supply_kpi.csv");
      Assertions.assertThat(drtOutputPath).isDirectoryContaining("glob:**demand_kpi.csv");
    }
    {
      // test output files specific for stopBased services
      Assertions.assertThat(drtOutputPath).isDirectoryContaining("glob:**stops.shp");
      Assertions.assertThat(drtOutputPath).isDirectoryContaining("glob:**trips_per_stop.csv");
      Assertions.assertThat(drtOutputPath).isDirectoryContaining("glob:**trips_per_stop.csv");
    }
    ...
    {
      // test general output files
      Assertions.assertThat(avOutputPath).isDirectoryContaining("glob:**supply_kpi.csv");
      Assertions.assertThat(avOutputPath).isDirectoryContaining("glob:**demand_kpi.csv");
    }

    {
      // test output files specific for stopBased services
      Assertions.assertThat(avOutputPath).isDirectoryContaining("glob:**serviceArea.shp");
    }
  }
}
