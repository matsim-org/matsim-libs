...
public class ChargingWithQueueingLogic implements ChargingLogic {
  ...
  public ChargingWithQueueingLogic(
  ...
  public void chargeVehicles(double chargePeriod, double now) {
    ...
    while (evIter.hasNext()) {
      ...
      eventsManager.processEvent(
          new EnergyChargedEvent(
              now, charger.getId(), ev.getId(), newCharge - oldCharge, newCharge));
    ...
    int queuedToPluggedCount =
        Math.min(queuedVehicles.size(), charger.getPlugCount() - pluggedVehicles.size());
  ...
  public void removeVehicle(ElectricVehicle ev, double now) {
    if (pluggedVehicles.remove(ev.getId()) != null) { // successfully removed
      ...
      Preconditions.checkState(
          queuedVehicles.remove(ev),
          "Vehicle (%s) is neither queued nor plugged at charger (%s)",
          ev.getId(),
          charger.getId());
