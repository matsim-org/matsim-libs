...
/**
 * This network Routing module adds stages for re-charging into the Route. This wraps a "computer
 * science" {@link LeastCostPathCalculator}, which routes from a node to another node, into
 * something that routes from a {@link Facility} to another {@link Facility}, as we need in MATSim.
 *
 * @author jfbischoff
 */
final class EvNetworkRoutingModule implements RoutingModule {
  ...
  public List<? extends PlanElement> calcRoute(RoutingRequest request) {
    ...
    if (!electricFleet.getVehicleSpecifications().containsKey(evId)) {
      ...
      double estimatedOverallConsumption =
          estimatedEnergyConsumption.values().stream().mapToDouble(Number::doubleValue).sum();
      ...
      if (numberOfStops < 1) {
        ...
        for (Link stopLocation : stopLocations) {
          ...
          StraightLineKnnFinder<Link, ChargerSpecification> straightLineKnnFinder =
              new StraightLineKnnFinder<>(
                  2, Link::getCoord, s -> network.getLinks().get(s.getLinkId()).getCoord());
          List<ChargerSpecification> nearestChargers =
              straightLineKnnFinder.findNearest(
                  stopLocation,
          ...
          List<? extends PlanElement> routeSegment =
              delegate.calcRoute(
                  DefaultRoutingRequest.of(
                      lastFrom, nexttoFacility, lastArrivaltime, person, request.getAttributes()));
          Leg lastLeg = (Leg) routeSegment.get(0);
          lastArrivaltime =
              lastLeg.getDepartureTime().seconds() + lastLeg.getTravelTime().seconds();
          stagedRoute.add(lastLeg);
          Activity chargeAct =
              PopulationUtils.createStageActivityFromCoordLinkIdAndModePrefix(
                  selectedChargerLink.getCoord(),
                  selectedChargerLink.getId(),
                  stageActivityModePrefix);
          ...
          // assume that the battery is compatible with a power that allows for full charge within
          // one hour (cf. FixedSpeedCharging)
          double maxPowerEstimate =
              Math.min(selectedCharger.getPlugPower(), ev.getBatteryCapacity() / 3600);
          double estimatedChargingTime = (ev.getBatteryCapacity() * 1.5) / maxPowerEstimate;
          chargeAct.setMaximumDuration(
              Math.max(evConfigGroup.minimumChargeTime, estimatedChargingTime));
        ...
        stagedRoute.addAll(
            delegate.calcRoute(
                DefaultRoutingRequest.of(
                    lastFrom, toFacility, lastArrivaltime, person, request.getAttributes())));
  ...
  private Map<Link, Double> estimateConsumption(ElectricVehicleSpecification ev, Leg basicLeg) {
    ...
    for (Link l : links) {
      double travelT =
          travelTime.getLinkTravelTime(l, basicLeg.getDepartureTime().seconds(), null, null);

      double consumption =
          driveEnergyConsumption.calcEnergyConsumption(l, travelT, linkEnterTime)
              + auxEnergyConsumption.calcEnergyConsumption(
                  basicLeg.getDepartureTime().seconds(), travelT, l.getId());
