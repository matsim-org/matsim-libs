...
public class SocHistogramTimeProfileCollectorProvider implements Provider<MobsimListener> {
	...
	@Inject
	public SocHistogramTimeProfileCollectorProvider(ElectricFleet evFleet, Vehicles vehicles, MatsimServices matsimServices) {
	...
	public MobsimListener get() {
		...
		ProfileCalculator calculator = () -> {
			...
			for (var vehicle : vehicles.getVehicles().values()) {
				if (evFleet.hasVehicle(vehicle.getId())) {
					var ev = evFleet.getVehicle(vehicle.getId());
