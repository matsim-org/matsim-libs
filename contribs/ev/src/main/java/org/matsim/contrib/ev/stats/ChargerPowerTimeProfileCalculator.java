...
public final class ChargerPowerTimeProfileCalculator
  ...
  /**
   * Calculation of average power output for each charging station for each charging event. Charging
   * stations without any charging events will not be present in the output file. Implementation
   * does only work when the Qsim end time is defined in the config, i.e., will not work for
  ...
  ChargerPowerTimeProfileCalculator(Config config) {
    ...
    qsimEndTime =
        ConfigUtils.addOrGetModule(config, QSimConfigGroup.class).getEndTime().orElse(0.0);
  ...
  public void handleEvent(ChargingEndEvent event) {
    double chargingTimeIn_h =
        (event.getTime() - chargingStartTimeMap.get(event.getVehicleId())) / 3600.0;
    double averagePowerIn_kW =
        (EvUnits.J_to_kWh(event.getCharge()) - chargingStartEnergyMap.get(event.getVehicleId()))
            / chargingTimeIn_h;
    increment(
        averagePowerIn_kW,
        event.getChargerId(),
        chargingStartTimeMap.get(event.getVehicleId()),
        event.getTime());
  }
  ...
  private void increment(
      ...
      double chargingEndTime) {

    // If Qsim end time is undefined in config, qsimEndTime will be 0.0 and will therefore not
    // proceed in calculating the power curves.
    if (chargingStartTime == chargingEndTime
        || chargingStartTime >= qsimEndTime
        || qsimEndTime == 0.0) {
      return;
    }
    ...
    for (int i = fromIdx; i < toIdx; i++) {
      double[] chargingVector =
          chargerProfiles.computeIfAbsent(
              chargerId, c -> new double[timeDiscretizer.getIntervalCount()]);
