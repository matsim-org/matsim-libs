...
public class VehicleTypeAggregatedChargeTimeProfileCollectorProvider
    implements Provider<MobsimListener> {
  private final ElectricFleet evFleet;
  private final MatsimServices matsimServices;

  @Inject
  public VehicleTypeAggregatedChargeTimeProfileCollectorProvider(
      ElectricFleet evFleet, MatsimServices matsimServices) {
  ...
  public MobsimListener get() {
    ...
    var vehicleTypes =
        evFleet.getElectricVehicles().values().stream()
    ...
    ProfileCalculator calculator =
        () -> {
          Map<String, Double> averageSocByType =
              evFleet.getElectricVehicles().values().stream()
                  .collect(
                      groupingBy(
                          ev ->
                              ev.getVehicleSpecification().getMatsimVehicle().getType().getId()
                                  + "",
                          mapping(
    ...
    return new TimeProfileCollector(
        header, calculator, 300, "average_charge_time_profiles", matsimServices);
  }
}
