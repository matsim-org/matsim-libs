...
public final class LTHDriveEnergyConsumption implements DriveEnergyConsumption {
  ...
  public static class Factory implements DriveEnergyConsumption.Factory {
    ...
    public Factory(
        double[] speeds,
        double[] slopes,
        double[][] consumptionPerSpeedAndSlope,
    ...
    public DriveEnergyConsumption create(ElectricVehicle electricVehicle) {
      return new LTHDriveEnergyConsumption(
          speeds, slopes, consumptionPerSpeedAndSlope, crashIfOutOfBoundValue);
    }
  }

  private LTHDriveEnergyConsumption(
      double[] speeds,
      double[] slopes,
      double[][] consumptionPerSpeedAndSlope,
      boolean crashIfOutOfBoundValue) {
    this.function =
        new PiecewiseBicubicSplineInterpolator()
            .interpolate(speeds, slopes, consumptionPerSpeedAndSlope);
  ...
  public double calcEnergyConsumption(Link link, double travelTime, double linkEnterTime) {
    ...
    if (speed > maxSpeed) {
      if (crashIfOutOfBoundValue) {
        throw new IllegalArgumentException(
            "Speed greater than the supported maxSpeed; speed =" + speed);
      } else {
    ...
    if (speed < minSpeed) {
      if (crashIfOutOfBoundValue) {
        throw new IllegalArgumentException(
            "Speed less than the supported minSpeed; speed =" + speed);
      } else {
  ...
  private double checkSlope(double currentSlope) {
    if (currentSlope < minSlope) {
      if (crashIfOutOfBoundValue) {
        throw new IllegalArgumentException(
            "Slope less than the supported minSlope; slope =" + currentSlope);
      } else {
      ...
      if (crashIfOutOfBoundValue) {
        throw new IllegalArgumentException(
            "Slope greater than the supported maxSlope; slope =" + currentSlope);
      } else {
