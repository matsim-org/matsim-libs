...
public class VariableSpeedCharging implements ChargingPower { // TODO upgrade to BatteryCharging
  ...
  public VariableSpeedCharging(
      ElectricVehicle electricVehicle, Point pointA, Point pointB, Point pointC, Point pointD) {
    ...
    Preconditions.checkArgument(pointB.soc < pointC.soc, "PointB.soc must be less than PointC.soc");
    Preconditions.checkArgument(
        pointA.relativePower <= pointB.relativePower,
        "PointA.relativePower must not be greater than PointB.relativePower");
    Preconditions.checkArgument(
        pointD.relativePower <= pointC.relativePower,
  ...
  private Point adjustPointIfSlowerCharging(
      double relativeChargerPower, Point lowerPoint, Point higherPoint) {
    if (relativeChargerPower >= higherPoint.relativePower) {
      return higherPoint;
    }

    double a =
        (relativeChargerPower - lowerPoint.relativePower)
            / (higherPoint.relativePower - lowerPoint.relativePower);
