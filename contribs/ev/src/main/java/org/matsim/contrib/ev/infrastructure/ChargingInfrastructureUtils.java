...
public class ChargingInfrastructureUtils {
  ...
  public static ChargingInfrastructure createChargingInfrastructure(
    ...
    var chargers =
        infrastructureSpecification.getChargerSpecifications().values().stream()
            .map(
                s ->
                    new ChargerDefaultImpl(
                        s, linkProvider.apply(s.getLinkId()), chargingLogicFactory.create(s)))
            .collect(ImmutableMap.toImmutableMap(Charger::getId, c -> (Charger) c));
    return () -> chargers;
  }

  public static ImmutableListMultimap<Id<Link>, Charger> getChargersAtLinks(
      ChargingInfrastructure infrastructure) {
    return infrastructure.getChargers().values().stream()
        .collect(ImmutableListMultimap.toImmutableListMultimap(c -> c.getLink().getId(), c -> c));
  }

  public static ChargingInfrastructure filterChargers(
      ChargingInfrastructure infrastructure, Predicate<Charger> filter) {
    var filteredChargers =
        infrastructure.getChargers().values().stream()
            .filter(filter)
            .collect(ImmutableMap.toImmutableMap(Charger::getId, c -> c));
    return () -> filteredChargers;
  }

  public static ChargingInfrastructure createModalNetworkChargers(
      ChargingInfrastructure infrastructure, Network network, String mode) {
        ...
        infrastructure.getChargers().values().stream()
            .map(
                c -> {
                  ...
                  return link == null
                      ? null
                      : new ChargerDefaultImpl(c.getSpecification(), link, c.getLogic());
                })
