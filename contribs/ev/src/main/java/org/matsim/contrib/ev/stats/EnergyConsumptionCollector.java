...
public class EnergyConsumptionCollector
    implements DrivingEnergyConsumptionEventHandler,
  ...
  @Inject
  EnergyConsumptionCollector() {} // so that class can only be instantiated via guice.  kai, oct'23
  ...
  public void notifyMobsimBeforeCleanup(MobsimBeforeCleanupEvent event) {
    try (CSVPrinter csvPrinter2 =
        new CSVPrinter(
            Files.newBufferedWriter(
                Paths.get(
                    controlerIO.getIterationFilename(
                        iterationCounter.getIterationNumber(), "evConsumptionPerLink.csv"))),
            CSVFormat.DEFAULT
                .withDelimiter(';')
                .withHeader("Link", "TotalConsumptionPerKm", "TotalConsumption"))) {
      for (Map.Entry<Id<Link>, Double> e : energyConsumptionPerLink.entrySet()) {
        csvPrinter2.printRecord(
            e.getKey(),
            (EvUnits.J_to_kWh(e.getValue()))
                / (network.getLinks().get(e.getKey()).getLength() / 1000.0),
            EvUnits.J_to_kWh(e.getValue()));
      }
    } catch (IOException e) {
