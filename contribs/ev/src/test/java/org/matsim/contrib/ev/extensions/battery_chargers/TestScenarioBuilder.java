...
public class TestScenarioBuilder {
	...
	public Controller build() {
		...
		for (VehicleRecord record : vehicleRecords) {
			...
			Activity charging = populationFactory
				.createActivityFromLinkId("car" + ChargingActivityEngine.CHARGING_INTERACTION, link.getId());
	...
	private static class SocListener implements IterationStartsListener, IterationEndsListener, EnergyChargedEventHandler {
		...
		public void notifyIterationStarts(IterationStartsEvent event) {
			try {
				...
				writer.write(String.join(";", "time", "vehicle_id", "charge_kWh") + "\n");
			} catch (IOException _) {
			}
		}
		...
		public void handleEvent(EnergyChargedEvent event) {
			try {
				writer.write(String.join(";", String.valueOf(event.getTime()),
