...
@Singleton
public class WholeDaySlotProvider implements ChargingSlotProvider {
	@Inject
	...
	public List<ChargingSlot> findSlots(Person person, Plan plan, ElectricVehicle vehicle) {
		...
		Activity startActivity = (Activity) elements.get(0);
		Activity endActivity = (Activity) elements.get(elements.size() - 1);

		Charger charger = infrastructure.getChargers().values().iterator().next();
		return Collections.singletonList(new ChargingSlot(startActivity, endActivity, null, 0.0, charger));
	}
}
