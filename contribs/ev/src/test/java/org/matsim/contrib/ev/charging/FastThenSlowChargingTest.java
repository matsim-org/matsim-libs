...
public class FastThenSlowChargingTest {
  ...
  private void assertCalcChargingPower(
      double capacity_kWh,
      double charge_kWh,
      double chargerPower_kW,
  ...
  private void assertCalcChargingTime(
      double capacity_kWh,
      double charge_kWh,
      double energy_kWh,
      double chargerPower_kW,
    ...
    Assertions.assertThat(
            ((FastThenSlowCharging) electricVehicle.getChargingPower())
                .calcChargingTime(charger, EvUnits.kWh_to_J(energy_kWh)))
        .isCloseTo(expectedChargingTime_s, Percentage.withPercentage(1e-13));
  }
  ...
  private ElectricVehicle createElectricVehicle(double capacity_kWh, double charge_kWh) {
    // this record is a bit hacky implementation of ElectricVehicleSpecification just meant for
    // tests
    record TestEvSpecification(
        Id<Vehicle> getId,
    ...
    var specification =
        new TestEvSpecification(
            Id.create("ev_id", Vehicle.class),
            null,
            "vt",
            ImmutableList.of("ct"),
            EvUnits.kWh_to_J(capacity_kWh),
            charge_kWh / capacity_kWh);

    return ElectricFleetUtils.create(
        specification,
        ev ->
            (link, travelTime, linkEnterTime) -> {
  ...
  private void assertCalcEnergyCharge(
      double capacity_kWh,
      double charge_kWh,
      double chargerPower_kW,
    ...
    double energy =
        ((FastThenSlowCharging) electricVehicle.getChargingPower())
            .calcEnergyCharged(charger, chargingPeriod);
  ...
  private void assertEnergyAndDurationCalcCompliance(
      double capacity_kWh, double charge_kWh, double chargerPower_kW, double chargingPeriod) {
    ChargerSpecification charger = createCharger(chargerPower_kW);
    ElectricVehicle electricVehicle = createElectricVehicle(capacity_kWh, charge_kWh);
    double energyCharged_J =
        ((FastThenSlowCharging) electricVehicle.getChargingPower())
            .calcEnergyCharged(charger, chargingPeriod);
    double chargingTime =
        ((FastThenSlowCharging) electricVehicle.getChargingPower())
            .calcChargingTime(charger, energyCharged_J);
