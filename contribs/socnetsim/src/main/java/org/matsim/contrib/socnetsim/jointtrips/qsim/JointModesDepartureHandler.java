...
public class JointModesDepartureHandler implements DepartureHandler, MobsimEngine {
  private static final Logger log = LogManager.getLogger(JointModesDepartureHandler.class);

  private final QVehicleProvider vehicleProvider;
  private final DepartureHandler departureHandler;

  private final PassengersWaitingPerDriver passengersWaitingPerDriver =
      new PassengersWaitingPerDriver();
  // map driverId -> driver info
  private final Map<Id, WaitingDriver> waitingDrivers = new LinkedHashMap<Id, WaitingDriver>();

  public JointModesDepartureHandler(final QNetsimEngineI netsimEngine) {
    this(new NetsimWrappingQVehicleProvider(netsimEngine), netsimEngine.getDepartureHandler());
  }

  public JointModesDepartureHandler(
      final QVehicleProvider vehicleProvider, final DepartureHandler departureHandler) {
  ...
  // /////////////////////////////////////////////////////////////////////////
  @Override
  public boolean handleDeparture(final double now, final MobsimAgent agent, final Id linkId) {
  ...
  private void handleDriverDeparture(final double now, final MobsimAgent agent, final Id linkId) {
    ...
    final Map<Id, PassengerAgent> passengersWaiting =
        passengersWaitingPerDriver.getPassengersWaitingDriverAtLink(driverId, linkId);
    ...
    if (presentPassengers.containsAll(passengerIds)) {
      ...
      final boolean handled = departureHandler.handleDeparture(now, agent, linkId);
      ...
      if (log.isTraceEnabled()) {
        log.trace(
            "NOT all passengers " + passengerIds + " in present passengers " + presentPassengers);
  ...
  private static Id getVehicleId(final MobsimAgent agent) {
    if (!(agent instanceof VehicleUsingAgent))
      throw new RuntimeException(agent.getClass().toString());
  ...
  private void handlePassengerDeparture(
      final double now, final MobsimAgent agent, final Id linkId) {
    ...
    if (wDriver != null && wDriver.linkId.equals(linkId)) {
      if (log.isTraceEnabled()) {
        log.trace(
            "driver "
                + driverId
                + " found waiting: ask him to depart for passenger "
                + agent.getId());
      }
      handleDriverDeparture(now, wDriver.driverAgent, linkId);
    } else if (log.isTraceEnabled()) {
      log.trace(
          "driver " + driverId + " NOT found waiting: DO NOTHING for passenger " + agent.getId());
  ...
  private final class WaitingDriver {
    ...
    public WaitingDriver(final MobsimAgent agent, final Id link) {
  ...
  private final class PassengersWaitingPerDriver {
    ...
    public Map<Id, PassengerAgent> getPassengersWaitingDriverAtLink(
        final Id driverId, final Id linkId) {
  ...
  private final class PassengersWaitingForDriver {
    private final Map<Id, Map<Id, PassengerAgent>> agentsAtLink =
        new LinkedHashMap<Id, Map<Id, PassengerAgent>>();
