...
public class RandomGroupLevelSelector implements GroupLevelPlanSelector {
  ...
  public RandomGroupLevelSelector(
      final Random random, final IncompatiblePlansIdentifierFactory incompFactory) {
    this.random = random;
  ...
  public final GroupPlans selectPlans(final JointPlans jointPlans, final ReplanningGroup group) {
    ...
    final GroupPlans plans =
        searchForRandomCombination(incompatibleRecords, new ArrayList<>(personRecords.values()));
    ...
    assert plans.getAllIndividualPlans().size() == group.getPersons().size()
        : plans.getAllIndividualPlans() + " != " + group.getPersons();
  ...
  private static Map<Id, PersonRecord> getPersonRecords(
    ...
    for (Person person : group.getPersons()) {
      ...
      for (Plan plan : person.getPlans()) {
        ...
        final PlanRecord r = new PlanRecord(plan, jp, 0);
        plans.add(r);
        if (jp != null) {
  ...
  private GroupPlans searchForRandomCombination(
      final IncompatiblePlanRecords incompatibleRecords, final List<PersonRecord> persons) {
      ...
      if (currentPlan.record.isStillFeasible) {
        ...
        if (!actuallyRemainingPersons.isEmpty()) {
          ...
          SelectorUtils.tagIncompatiblePlansAsInfeasible(
              currentPlan.record, incompatibleRecords, feasibilityChanger);
          ...
          for (PlanRecord r : shuffle(nextPerson.plans)) {
            ...
            assert !containsSamePersonTwice(newNode)
                : "plans=" + plansStack + "\n       answer=" + newNode;
  ...
  private void markJointPlansAsInfeasible(
      final PlanRecord currentRecord, final FeasibilityChanger feasibilityChanger) {
  ...
  private void markPlansOfCotravelersAsInfeasible(
      final AnswerNode currentPlan, final FeasibilityChanger feasibilityChanger) {
