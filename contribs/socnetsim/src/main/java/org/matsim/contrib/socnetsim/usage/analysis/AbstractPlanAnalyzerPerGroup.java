 ...
 * @author thibautd
 */
public abstract class AbstractPlanAnalyzerPerGroup
    implements IterationEndsListener, ShutdownListener {
  private static final Logger log = LogManager.getLogger(AbstractPlanAnalyzerPerGroup.class);

  /**
   * THe plan sizes are plotted per group: this class aims at saying which agent pertains to each
   * group (eg "agents in an household of 3 persons which have a car available") <br>
  ...
  private void dropDataFile(final Id groupId, final History history) {
  ...
  private void dropChart(final Id groupId, final History history) {
    final XYLineChart chart =
        new XYLineChart(getStatName() + " Statistics for " + groupId, "iteration", getStatName());

    chart.addSeries(
        "avg. minimum " + getStatName(), history.getIterations(), history.getAvgOfMinimums());
    chart.addSeries(
        "avg. max " + getStatName(), history.getIterations(), history.getAvgOfMaximums());
    chart.addSeries(
        "avg. of plans' average " + getStatName(),
        history.getIterations(),
        history.getAvgOfAverages());
    chart.addSeries(
        "avg. executed " + getStatName(), history.getIterations(), history.getAvgOfExecuted());
    chart.addMatsimLogo();
    chart.saveAsPng(this.fileName + "_" + groupId + ".png", 800, 600);
  }

  private void fillStatsForPerson(final History history, final Person person) {
    int count = 0;
  ...
  private static final class HistoryPerGroup {
    private final Map<Id, History> historyPerGroup = new LinkedHashMap<Id, History>();
