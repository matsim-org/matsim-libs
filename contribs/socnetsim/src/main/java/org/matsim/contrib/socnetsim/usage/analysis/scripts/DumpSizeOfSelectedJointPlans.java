...
public class DumpSizeOfSelectedJointPlans {
    ...
    new MatsimXmlParser(MatsimXmlParser.ValidationType.DTD_ONLY) {
      ...
      public void startTag(final String name, final Attributes atts, final Stack<String> context) {
        ...
        if (name.equals("individualPlan")) {
          size++;
          final Id id = Id.create(atts.getValue("personId").trim(), Person.class);
          final Integer selectedIndex = selectedIndices.get(id);
          persons.add(id);
          if (selectedIndex.equals(Integer.valueOf(atts.getValue("planNr")))) {
  ...
  private static Map<Id, Integer> parseSelectedIndices(final String plansFile) {
