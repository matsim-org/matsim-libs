...
public class CountTripsPerTypeOfOriginAndDestination {
  ...
  private static void writeCounts(final Iterable<Record> counts, final String outFile)
      throws IOException {
    final BufferedWriter writer = IOUtils.getBufferedWriter(outFile);
    writer.write("origType\tdestType\tmode\tcount");
    for (Record r : counts) {
      writer.newLine();
      writer.write(r.originType + "\t" + r.destinationType + "\t" + r.mode + "\t" + r.count);
    }
    writer.close();
  }
