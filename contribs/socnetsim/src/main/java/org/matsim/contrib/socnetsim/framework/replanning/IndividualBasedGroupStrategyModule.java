...
public class IndividualBasedGroupStrategyModule implements GenericStrategyModule<GroupPlans> {
  private static final Logger log = LogManager.getLogger(IndividualBasedGroupStrategyModule.class);

  private final boolean actOnPlansInJointPlans;
  private final PlanStrategyModule delegate;

  public IndividualBasedGroupStrategyModule(final PlanStrategyModule module) {
    this(true, module);
  }

  public IndividualBasedGroupStrategyModule(
      final boolean actOnPlansInJointPlans, final PlanStrategyModule module) {
  ...
  public String name() {
    ...
    if (delegateName.length() == 0 && delegate instanceof AbstractMultithreadedModule) {
      ...
      try {
        delegateName =
            ((AbstractMultithreadedModule) delegate)
                .getPlanAlgoInstance()
                .getClass()
                .getSimpleName();
      } catch (Exception e) {
