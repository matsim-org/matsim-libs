...
/**
 * Helps working with quad trees, by re-building a new quad tree if necessary (avoids having to care
 * about bounds).
 *
 * @author thibautd
 */
public class QuadTreeRebuilder<T> {
  ...
  private void buildQuadTree() {
    ...
    this.quadTree = new QuadTree<T>(minX - EPSILON, minY - EPSILON, maxX + EPSILON, maxY + EPSILON);

    for (Tuple<Coord, T> element : elements) {
      try {
        quadTree.put(element.getFirst().getX(), element.getFirst().getY(), element.getSecond());
      } catch (Exception e) {
        // if insertion fail, give more informative error message.
        throw new RuntimeException(
            "problem at inserting element "
