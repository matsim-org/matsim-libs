...
public class FixedGroupsIdentifierFileParser {
  ...
  protected FixedGroupsIdentifier parse(final XMLStreamReader streamReader)
    ...
    while (streamReader.hasNext()) {
      ...
      if (streamReader.getLocalName().equals("clique") && currentGroup.size() > 0) {
        counter.incCounter();
  ...
  private static <T> Id<T> parseId(final XMLStreamReader streamReader, final Class<T> idType) {
    ...
    if (!streamReader.getAttributeLocalName(0).equals("id")) {
      throw new ParsingException(
          "unexpected attribute name " + streamReader.getAttributeLocalName(0));
    }
