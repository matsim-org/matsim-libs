...
public class HouseholdBasedVehicleRessources implements VehicleRessources {
  ...
  public HouseholdBasedVehicleRessources(final Households hhs) {
    this.households = hhs;
  ...
  public Set<Id<Vehicle>> identifyVehiclesUsableForAgent(final Id<Person> person) {
    ...
    if (!hh.getMemberIds().contains(person))
      throw new RuntimeException("household " + hhId + " does not contain " + person);
