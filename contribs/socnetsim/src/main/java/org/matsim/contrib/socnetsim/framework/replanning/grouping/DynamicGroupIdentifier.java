...
public class DynamicGroupIdentifier implements GroupIdentifier {
  private static final Logger log = LogManager.getLogger(DynamicGroupIdentifier.class);
  ...
  public Collection<ReplanningGroup> identifyGroups(final Population population) {
    ...
    final List<ReplanningGroup> mergedGroups = randomlyMergeGroups(jointPlansGroups, socialNetwork);

    return mergedGroups;
  }

  private List<ReplanningGroup> randomlyMergeGroups(
      final Queue<TaggedReplanningGroup> groupsToMerge, final SocialNetwork socialNetwork) {
  ...
  private static List<Id> getGroupAlters(
      final ReplanningGroup g, final SocialNetwork socialNetwork, final Set<Id> allowedPersonIds) {
  ...
  private Queue<TaggedReplanningGroup> identifyGroupsBasedOnJointPlans(
    ...
    while (!remainingPersons.isEmpty()) {
      ...
      addPersonsLinkedByJointPlans(group, person, remainingPersons, jointPlans);
    }
      ...
      for (Plan planInJp : jp.getIndividualPlans().values()) {
        ...
        if (!remainingPersons.remove(alter)) {
          ...
          throw new RuntimeException(
              "alter "
                  + alter
                  + " was not available and not in group "
                  + group
                  + " for person "
                  + person);
        }
        ...
        addPersonsLinkedByJointPlans(group, alter, remainingPersons, jointPlans);
      }
    }
  }
