...
/**
 * sets the mode of all trips in full tours to the same mode. Useful after operations which may
 * change the tour structure (for instance location choice or sequence mutation)
 *
 * @author thibautd
 */
public class TourModeUnifierAlgorithm implements PlanAlgorithm {
  ...
  public TourModeUnifierAlgorithm(
      final Predicate<String> isStageActivity, final MainModeIdentifier modeIdentifier) {
    this(isStageActivity, new SubtourFirstModeIdentifier(modeIdentifier));
  }

  public TourModeUnifierAlgorithm(
      final Predicate<String> isStageActivity, final SubtourModeIdentifier modeIdentifier) {
  ...
  public static final class SubtourFirstModeIdentifier implements SubtourModeIdentifier {
    ...
    public SubtourFirstModeIdentifier(final MainModeIdentifier modeIdentifier) {
      this.modeIdentifier = modeIdentifier;
    }

    @Override
    public String identifyMode(final Subtour subtour) {
      return modeIdentifier.identifyMainMode(subtour.getTrips().get(0).getTripElements());
    }
  }
}
