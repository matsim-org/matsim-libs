...
public class CliquesModule extends AbstractModule {
  ...
  public void install() {
    bind(GroupIdentifier.class)
        ...
        .in(Scopes.SINGLETON);
    bind(SocialNetwork.class).toProvider(SocialNetworkProvider.class).asEagerSingleton();
  }
  ...
  private static class SocialNetworkProvider implements Provider<SocialNetwork> {
    ...
    @Inject
    public SocialNetworkProvider(final Scenario sc, final GroupIdentifier groupIdentifier) {
      this.sc = sc;
