...
public class PrismicLocationChoiceAlgorithm implements GenericPlanAlgorithm<GroupPlans> {
  ...
  public PrismicLocationChoiceAlgorithm(
    ...
    this(config, getChooser(config), facilities, socialNetwork, stageActivityTypes);
  }

  private static LocationChooser getChooser(final PrismicLocationChoiceConfigGroup config) {
  ...
  private static void changeLocation(
      final Collection<Subchain> subchains, final ActivityFacility facility) {
    for (Subchain subchain : subchains) {
      ((Activity) subchain.getToMove()).setFacilityId(facility.getId());
      ((Activity) subchain.getToMove()).setLinkId(facility.getLinkId());
      ((Activity) subchain.getToMove()).setCoord(facility.getCoord());
    }
  }

  private List<ActivityFacility> identifyPotentialLocations(final Collection<Subchain> subchains) {
    ...
    // TODO: handle especially case where one agent is far away (ie do choice only for other
    // agents?)
    ...
    do {
      ...
      for (Subchain subchain : subchains) {
        final ActivityFacility start =
            facilities.getFacilities().get(subchain.getStart().getFacilityId());
        if (start == null)
          throw new RuntimeException(
              "no facility "
                  + subchain.getStart().getFacilityId()
                  + " for activity "
                  + subchain.getStart());
        final ActivityFacility end =
            facilities.getFacilities().get(subchain.getEnd().getFacilityId());
        if (end == null)
          throw new RuntimeException(
              "no facility "
                  + subchain.getEnd().getFacilityId()
                  + " for activity "
                  + subchain.getEnd());

        final double minDistance =
            CoordUtils.calcEuclideanDistance(start.getCoord(), end.getCoord()) + 1E-9;
        final Collection<ActivityFacility> prism =
            approximatePrism(Math.max(minDistance, distanceBudget), quadTree, start, end);
    ...
    Collections.sort(
        ...
        new Comparator<ActivityFacility>() {
          @Override
          public int compare(final ActivityFacility o1, final ActivityFacility o2) {
  ...
  private Collection<Collection<Subchain>> selectActivityGroups(final GroupPlans plan) {
    final Collection<Collection<Plan>> planGroups =
        GroupingUtils.randomlyGroup(config, random, plan, socialNetwork);
  ...
  private Subchain getRandomSubchain(final Plan plan, final Collection<String> types) {
    ...
    for (Trip trip : TripStructureUtils.getTrips(plan, stageActivityTypes::contains)) {
      if (accessTrip != null) {
        assert accessTrip.getDestinationActivity() == trip.getOriginActivity()
            : accessTrip.getDestinationActivity() + " != " + trip.getOriginActivity();
      ...
      accessTrip = types.contains(trip.getDestinationActivity().getType()) ? trip : null;
    }

    return potentialSubchains.isEmpty()
        ? null
  ...
  public static class Subchain {
    ...
    public Subchain(final Activity start, final Activity toMove, final Activity end) {
      this.start = start;
      this.toMove = toMove;
      this.end = end;
    }
  ...
  public interface LocationChooser {
    ActivityFacility choose(Collection<Subchain> subchains, List<ActivityFacility> choiceSet);
  }
  ...
  public static class RandomLocationChooser implements LocationChooser {
    ...
    public ActivityFacility choose(
        final Collection<Subchain> subchains, final List<ActivityFacility> choiceSet) {
  ...
  public static class MaxDistanceProportionalLocationChooser implements LocationChooser {
    ...
    public ActivityFacility choose(
      ...
      for (int i = 0; i < maxDists.length; i++) {
        ...
        for (Subchain subchain : subchains) {
          final double dist =
              CoordUtils.calcEuclideanDistance(subchain.getStart().getCoord(), fac.getCoord())
                  + CoordUtils.calcEuclideanDistance(subchain.getEnd().getCoord(), fac.getCoord());
  ...
  public static class MaxDistanceLogitLocationChooser implements LocationChooser {
    ...
    public MaxDistanceLogitLocationChooser(final double beta) {
      if (beta > 0)
        throw new IllegalArgumentException("beta must be negative to make sense. Got " + beta);
      this.beta = beta;
    }
