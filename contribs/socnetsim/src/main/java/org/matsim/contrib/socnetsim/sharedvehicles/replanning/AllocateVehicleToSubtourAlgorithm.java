...
/**
 * Allocates a random vehicle to a random tour ( "root subtour" ) containing a given <u>main</u>
 * mode. The vehicle is allocated to all NetworkRoutes in the trips with the given main mode in the
 * choosen tour.
 *
 * @author thibautd
 */
public class AllocateVehicleToSubtourAlgorithm implements PlanAlgorithm {
  ...
  public void run(final Plan plan) {
    final List<Id> vehs =
        new ArrayList<Id>(
            vehicleRessources.identifyVehiclesUsableForAgent(plan.getPerson().getId()));
  ...
  private boolean containsMode(final Subtour s) {
    for (Trip t : s.getTrips()) {
      if (mainModeIdentifier.identifyMainMode(t.getTripElements()).equals(mode)) {
        return true;
      }
    }
    return false;
