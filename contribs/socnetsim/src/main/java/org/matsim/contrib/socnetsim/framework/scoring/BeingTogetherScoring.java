...
public class BeingTogetherScoring {
  ...
  private final Map<Id, IntervalsAtLocation> intervalsPerAlter =
      new HashMap<Id, IntervalsAtLocation>();
    ...
    for (Map.Entry<Location, WrappedAroundIntervalSequence> e : intervalsForEgo.map.entrySet()) {
      ...
      for (Map.Entry<Id, IntervalsAtLocation> e2 : intervalsPerAlter.entrySet()) {
        ...
        MapUtils.addToDouble(
            ...
            0,
            calcOverlap(activeTimeWindow, openingIntervals, egoIntervals, alterIntervals));
      }
    }
  ...
  private List<Interval> getOpeningIntervals(final Location location) {
  ...
  private static Interval intersect(final Interval i1, final Interval i2) {
  ...
  private static double measureOverlap(final Interval i1, final Interval i2) {
    final double startOverlap = Math.max(i1.start, i2.start);
    final double endOverlap = Math.min(i1.end, i2.end);
    ...
    final IntervalsAtLocation intervals =
        event.getPersonId().equals(ego)
            ...
            : MapUtils.getArbitraryObject(
                event.getPersonId(), intervalsPerAlter, locatedIntervalsFactory);
  ...
  private static class IntervalsAtLocation {
    ...
    public void startInterval(final Location location, final double time) {
      final WrappedAroundIntervalSequence seq =
          MapUtils.getArbitraryObject(location, map, seqFactory);
      seq.startInterval(time);
    }

    public void endInterval(final Location location, final double time) {
      final WrappedAroundIntervalSequence seq =
          MapUtils.getArbitraryObject(location, map, seqFactory);
  ...
  private static class Location {
    ...
    public Location(final Id linkId, final Id facilityId, final String actType) {
      this.vehId = null;
      this.linkId = linkId;
    ...
    private final boolean areEquals(final Object o1, final Object o2) {
  ...
  private static class WrappedAroundIntervalSequence {
    ...
    public void startInterval(double time) {
      if (last != null)
        throw new IllegalStateException("must close interval before starting new one");
      last = new Interval();
      last.start = time;
    }
  ...
  public interface PersonOverlapScorer {
    public double getScore(Id alter, double totalTimePassedTogether);
  }
  ...
  public static class LogOverlapScorer implements PersonOverlapScorer {
    ...
    public LogOverlapScorer(
        final double marginalUtility, final double typicalDuration, final double zeroDuration) {
      this.marginalUtility = marginalUtility;
    ...
    public double getScore(final Id alter, final double totalTimePassedTogether) {
      ...
      final double log =
          marginalUtility * typicalDuration * Math.log(totalTimePassedTogether / zeroDuration);
