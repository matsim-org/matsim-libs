...
/**
 * An algorithm which attempts to synchronize the plans of passengers with the plans of drivers. It
 * relies on the travel times stored in the legs for its correctness, and assumes departure times
 * correspond to activity end times.
 *
 * @author thibautd
 */
public class SynchronizeCoTravelerPlansAlgorithm implements GenericPlanAlgorithm<JointPlan> {
  private static final Logger log = LogManager.getLogger(SynchronizeCoTravelerPlansAlgorithm.class);
  ...
  public void run(final JointPlan plan) {
    final JointTravelStructure jointTravelStructure = JointTravelUtils.analyseJointTravel(plan);

    for (JointTrip jt : jointTravelStructure.getJointTrips()) {
      final double pickUpTime = inferPickUpTime(jt, plan);
      if (pickUpTime < 0) {
        throw new RuntimeException(
            "got negative PU time for joint trip " + jt + " in plan " + plan);
      }
  ...
  private void setPassengerDepartureTime(
      final JointTrip jt, final JointPlan plan, final double pickUpTime) {
  ...
  private double inferPickUpTime(final JointTrip jt, final JointPlan plan) {
    final Leg firstDriverLeg = jt.getDriverLegs().get(0);
    final List<PlanElement> driverPlanElements =
        plan.getIndividualPlan(jt.getDriverId()).getPlanElements();
