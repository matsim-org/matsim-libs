 ...
 */
@Deprecated
public class MutateActivityLocationsToLocationsOfOthersAlgorithm
  ...
  public MutateActivityLocationsToLocationsOfOthersAlgorithm(
      final ChoiceSet choiceSet, final Random random) {
    this.random = random;
  ...
  private static class BasicFacility implements Facility, Identifiable {
    ...
    public BasicFacility(final Activity act) {
  ...
  public static class ChoiceSet {
    ...
    // TODO: This looks awkward. Not clear whether we can really assume that this in practice really
    // identified only activity
    // types which end on "interaction" or whether we now miss some. In socnetsim all stage activity
    // types are renamed to end
    // on "interaction"
    ...
    public ChoiceSet(final Population population, final String type) {
      this.type = type;
        ...
        for (Plan plan : person.getPlans()) {
          for (Activity act :
              TripStructureUtils.getActivities(
                  plan, StageActivityHandling.ExcludeStageActivities)) {
            assert act.getType().equals(type);
            facilities.add(pool.getPooledInstance(new BasicFacility(act)));
          }
        }
      }
