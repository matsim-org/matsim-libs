...
/**
 * Groups methods too specific to go in the "frameworky" part of the code, but which still needs to
 * be called from various application-specific scripts.
 *
 * <p>Ideally, scripts should consist mainly of calls to those methods, with a few lines specific to
 * the application.
 *
 * @author thibautd
 */
public class RunUtils {
  static final Logger log = LogManager.getLogger(RunUtils.class);
  ...
  //
  //	controller.getRegistry().getScenario().getConfig().plansCalcRoute().getModeRoutingParams().get(
  // TransportMode.transit_walk ).getBeelineDistanceFactor(),
  ...
  //
  //	controller.getRegistry().getScenario().getConfig().plansCalcRoute().getModeRoutingParams().get(
  // TransportMode.walk ).getBeelineDistanceFactor(),
  //					controller.getRegistry().getScenario().getNetwork() ) );
  //
  //		algo.putEstimator(
  //				TransportMode.pt,
  ...
  //						for ( Person person :
  // controller.getRegistry().getScenario().getPopulation().getPersons().values() ) {
  ...
  //					(KtiLikeScoringConfigGroup) scenario.getConfig().getModule(
  // KtiLikeScoringConfigGroup.GROUP_NAME ),
  ...
  //			new SocialNetworkReader( elementName , scenario ).parse(
  // scoringFunctionConf.getInternalizationNetworkFile() );
  ...
  //			final ReplanningStatsDumper replanningStats = new ReplanningStatsDumper(
  // controller.getControlerIO().getOutputFilename( "replanningGroups.dat" ) );
  ...
  //			final ReplanningAllocationDumper replanningStats = new ReplanningAllocationDumper(
  // controller.getControlerIO().getOutputFilename( "replanningAllocations.dat" ) );
  //			repl.getStrategyManager().addListener( replanningStats );
  //			controller.addControlerListener( replanningStats );
  //		}
  //
  ...
  public static void enrichScenario(final Scenario scenario) {
    ...
    if (config.households().getInputFile() != null && weights.getUseLimitedVehicles()) {
      scenario.addScenarioElement(
          ...
          new HouseholdBasedVehicleRessources(scenario.getHouseholds()));
    }
      ...
      for (Plan plan : person.getPlans()) {
        for (Activity act :
            TripStructureUtils.getActivities(
                plan, StageActivityHandling.StagesAsNormalActivities)) {
  ...
  public static void connectLocations(final Scenario scenario) {
    if (scenario.getActivityFacilities() != null) {
      new WorldConnectLocations(scenario.getConfig())
          .connectFacilitiesWithLinks(scenario.getActivityFacilities(), scenario.getNetwork());
    }
  }
}
