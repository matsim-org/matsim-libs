...
/**
 * Stores plans for a group. For plans pertaining to a joint plan, the joint plan is stored; for
 * individual plans, the individual plan is stored. <br>
 * The collections returned are the interal references. GroupStrategyModules may modify them if
 * needed.
 *
 * @author thibautd
 */
public class GroupPlans {
  ...
  public GroupPlans() {
    this(Collections.<JointPlan>emptyList(), Collections.<Plan>emptyList());
  }

  public GroupPlans(
      final Collection<JointPlan> jointPlans, final Collection<Plan> individualPlans) {
  ...
  public static GroupPlans copyPlans(
      final JointPlanFactory jointPlanFactory, final GroupPlans plans) {
    ...
    for (Plan p : plans.getIndividualPlans()) {
      ...
      // I think that the above cast will now fail.  It probably worked originally, since JointPlan
      // was an extension of PlanImpl, which is
      ...
      // please let me know if you want to dicuss.
      // kai, nov'15
