...
public final class GroupStrategyRegistry {
  private static final MapUtils.Factory<SubpopulationRegistry> registryFactory =
  ...
  private final Map<String, SubpopulationRegistry> populationRegistries =
      new HashMap<String, SubpopulationRegistry>();
  ...
  public final void addStrategy(
    ...
    final SubpopulationRegistry registry =
        MapUtils.getArbitraryObject(subpopulation, populationRegistries, registryFactory);
    registry.addStrategy(strategy, weight, lastIteration);
  }

  public GroupPlanStrategy chooseStrategy(
      final int iteration, final String subpopulation, final double randomDraw) {
    final SubpopulationRegistry registry =
        MapUtils.getArbitraryObject(subpopulation, populationRegistries, registryFactory);
    return registry.chooseStrategy(iteration, randomDraw);
  }
  ...
  public void setExtraPlanRemover(final ExtraPlanRemover remover) {
    if (this.remover != null) {
...
class SubpopulationRegistry {
  ...
  public final void addStrategy(
      final GroupPlanStrategy strategy, final double weight, final int lastIteration) {
    if (weight <= 0.0) {
      log.info(
          "strategy "
              + strategy
              + " with weight "
              + weight
              + " will not be added: weight negative or null.");
      return;
    }
  ...
  public GroupPlanStrategy chooseStrategy(final int iteration, final double randomDraw) {
    ...
    throw new RuntimeException(
        "choice=" + randomDraw + " not found in " + calcSumOfWeights(iteration));
  }
