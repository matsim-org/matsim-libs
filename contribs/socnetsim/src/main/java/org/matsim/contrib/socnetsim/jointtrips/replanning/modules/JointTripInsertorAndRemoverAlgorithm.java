...
public class JointTripInsertorAndRemoverAlgorithm implements GenericPlanAlgorithm<JointPlan> {
    ...
    this.insertor =
        new JointTripInsertorAlgorithm(
            ...
            (JointTripInsertorConfigGroup)
                scenario.getConfig().getModule(JointTripInsertorConfigGroup.GROUP_NAME),
            mainModeIdentifier);
    this.remover = new JointTripRemoverAlgorithm(random, mainModeIdentifier);
  ...
  public void run(final JointPlan plan) {
    ...
    do {
      ...
      assert !agentsToIgnore.contains(actedUpon.getDriverId())
          : actedUpon + " <- " + agentsToIgnore;
      agentsToIgnore.add(actedUpon.getDriverId());

      assert !agentsToIgnore.contains(actedUpon.getPassengerId())
          : actedUpon + " <- " + agentsToIgnore;
      agentsToIgnore.add(actedUpon.getPassengerId());
    } while (iterative);
  }

  private double getProbRemoval(final JointPlan plan, final List<Id<Person>> agentsToIgnore) {
    ...
    for (Plan indivPlan : plan.getIndividualPlans().values()) {
      ...
      // trips by the router!
      for (Trip trip :
          TripStructureUtils.getTrips(indivPlan, JointActingTypes.JOINT_STAGE_ACTS::contains)) {
        if (tripContainsOneOfThoseModes(trip, Collections.singleton(JointActingTypes.PASSENGER)))
          countPassengers++;
  ...
  private static boolean tripContainsOneOfThoseModes(
      final Trip trip, final Collection<String> modes) {
