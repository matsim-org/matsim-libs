...
public class CourtesyHistogramListener implements IterationEndsListener, IterationStartsListener {
  ...
  @Inject
  CourtesyHistogramListener(
      Config config, CourtesyHistogram histogram, OutputDirectoryHierarchy controlerIO) {
  ...
  public void notifyIterationEnds(final IterationEndsEvent event) {
    this.histogram.write(
        controlerIO.getIterationFilename(event.getIteration(), "courtesyHistogram.txt"));
    if (this.outputGraph) {
      for (String type : histogram.getDataFrames().keySet()) {
        writeGraphic(
            ...
            controlerIO.getIterationFilename(
                event.getIteration(), "courtesyHistogram_" + type + ".png"));
      }
    }
  }

  static JFreeChart getGraphic(
      final CourtesyHistogram.DataFrame dataFrame, int iteration, String actType) {
    ...
    final JFreeChart chart =
        ChartFactory.createXYStepChart(
            "Courtesy Statistics," + "actType " + actType + " it." + iteration,
            "time",
            "# persons",
            xyData,
  ...
  public static void writeGraphic(
      CourtesyHistogram courtesyHistogram, final String actType, final String filename) {
    try {
