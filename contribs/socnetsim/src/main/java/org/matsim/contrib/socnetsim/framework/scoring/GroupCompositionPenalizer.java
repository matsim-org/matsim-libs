...
/**
 * Scoring element that computes a penalty for "joint activities" if they do not fulfill the desired
 * number of participants. Each agent has preferences on the number of social contacts with whom it
 * wants to perform a joint activity with. A penalty of time, dependent on the number of
...
public class GroupCompositionPenalizer implements ArbitraryEventScoring {
  ...
  public GroupCompositionPenalizer(
      final String activityType, final UtilityOfTimeCalculator utilityCalculator) {
  ...
  private void updateScore(final double time) {
    ...
    this.score +=
        utilityCalculator.getUtilityOfTime(currentNCoparticipants)
            * (time - lastChangeInNCoparticipants);

    if (log.isTraceEnabled()) {
      log.trace(
          "update score "
              + Time.writeTime(lastChangeInNCoparticipants)
              + "->"
              + Time.writeTime(time)
              + ": currently "
              + currentNCoparticipants
  ...
  public static class MinGroupSizeLinearUtilityOfTime implements UtilityOfTimeCalculator {
    ...
    public MinGroupSizeLinearUtilityOfTime(
        final int minGroupSize, final double utilOfMissingContact) {
      this.minGroupSize = minGroupSize;

      if (utilOfMissingContact > 0)
        throw new IllegalArgumentException(
            "util of missing contact expected to be negative. Was " + utilOfMissingContact);
