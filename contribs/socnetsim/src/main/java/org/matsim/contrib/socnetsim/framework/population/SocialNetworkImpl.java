...
public class SocialNetworkImpl implements SocialNetwork {
  private static final Logger log = LogManager.getLogger(SocialNetworkImpl.class);
  ...
  /**
   * @param isReflective indicates whether social ties are directed. tagging a social network as
   *     reflective allows to reduce the size of the output by half.
   */
  ...
  public void addMonodirectionalTie(final Id ego, final Id alter) {
    if (isReflective) {
      throw new IllegalStateException(
          "cannot add a monodirectional tie to a reflective social network.");
    }
    addTieInternal(ego, alter);
  }

  public void removeMonodirectionalTie(final Id ego, final Id alter) {
    if (isReflective) {
      throw new IllegalStateException(
          "cannot remove a monodirectional tie to a reflective social network.");
    }
    removeTieInternal(ego, alter);
  }

  private void addTieInternal(final Id ego, final Id alter) {
  ...
  private void removeTieInternal(final Id ego, final Id alter) {
    final Set<Id<Person>> alters = map.get(ego);
    if (alters == null) throw new IllegalArgumentException("ego " + ego + " unknown");
  ...
  /* (non-Javadoc)
   * @see org.matsim.contrib.socnetsim.jointtrips.population.SocialNetworkI#isReflective()
   */
  @Override
  ...
  public void addMetadata(final String attribute, final String value) {
    ...
    if (old != null)
      log.warn(
          "replacing metadata \"" + attribute + "\" from \"" + old + "\" to \"" + value + "\"");
  }

  public void removeEgo(final Id ego) {
    if (!isReflective())
      throw new IllegalStateException("Cannot remove an ego in a non reflective network.");
