...
public final class VehicleAllocationConsistencyChecker
    implements IterationEndsListener, IterationStartsListener {
  private static final Logger log = LogManager.getLogger(VehicleAllocationConsistencyChecker.class);
      ...
      final Set<Id> vehsOfPlan =
          jp != null && knownJointPlans.add(jp)
              ? SharedVehicleUtils.getVehiclesInJointPlan(
                  jp, SharedVehicleUtils.DEFAULT_VEHICULAR_MODES)
              : (jp == null
                  ? SharedVehicleUtils.getVehiclesInPlan(
                      plan, SharedVehicleUtils.DEFAULT_VEHICULAR_MODES)
      ...
      for (Id v : vehsOfPlan) {
        if (v == null) {
          ...
          if (!knownVehicles.add(v)) {
            log.error(
                "inconsistent allocation of vehicle "
                    + v
                    + " (found in several distinct joint plans)");
            gotError = true;
          }
          hadNonNull = true;
        }
