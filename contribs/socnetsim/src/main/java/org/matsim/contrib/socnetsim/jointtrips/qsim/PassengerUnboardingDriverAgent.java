...
public class PassengerUnboardingDriverAgent
    implements MobsimDriverAgent, PlanAgent, PassengerAgent, PTPassengerAgent, HasPerson {
  ...
  public void endLegAndComputeNextState(final double now) {
    ...
    if (mode.equals(JointActingTypes.DRIVER)) {
      ...
      for (PassengerAgent p : passengersToUnboard) {
        ...
        events.processEvent(new PersonLeavesVehicleEvent(now, p.getId(), vehicle.getId()));
  ...
  private void boardPassengers() {
    ...
    final MobsimVehicle vehicle =
        getVehicle(); // vehicleProvider.getVehicle( delegate.getPlannedVehicleId() );
    final EventsManager events = ((QSim) internalInterface.getMobsim()).getEventsManager();
    for (PassengerAgent passenger : passengersToBoard) {
      assert passenger.getCurrentLinkId().equals(getCurrentLinkId())
          : passenger
              + " is at "
              + passenger.getCurrentLinkId()
              + " instead of "
              + getCurrentLinkId()
              + " for driver "
              + this;
      assert ((Leg) getCurrentPlanElement()).getMode().equals(JointActingTypes.DRIVER)
          : getCurrentPlanElement();
      assert ((DriverRoute) ((Leg) getCurrentPlanElement()).getRoute())
              .getPassengersIds()
              .contains(passenger.getId())
          : passenger
              + " not in "
              + ((DriverRoute) ((Leg) getCurrentPlanElement()).getRoute()).getPassengersIds()
              + " for driver "
              + this;
    ...
    assert vehicle.getPassengers().size() == dr.getPassengersIds().size()
            && IdentifiableCollectionsUtils.containsAll(
                dr.getPassengersIds(), vehicle.getPassengers())
  ...
  public String toString() {
    return "["
        ...
        + getCurrentPlanElement()
        + "; positionInPlan="
        + getCurrentPlan().getPlanElements().indexOf(getCurrentPlanElement())
        + " / "
  ...
  public boolean getEnterTransitRoute(
    ...
    if (ptDelegate == null)
      throw new UnsupportedOperationException(
          delegate.getClass().getName() + " do not provide PTPassengerAgent");
