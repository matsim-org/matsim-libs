...
/**
 * Stores links between individual plans, as defined by joint plans. <br>
 * Using the {@link JointPlanFactory} to create instances of individual plans will allow to cache
 * the JointPlan in the Plan instance, avoiding expensive Map lookups. <br>
...
public class JointPlans implements MatsimToplevelContainer {
    ...
    synchronized (jointPlan) {
      for (Plan indivPlan : jointPlan.getIndividualPlans().values()) {
        if (indivPlan instanceof PlanWithCachedJointPlan) {
          ...
          if (withCache.getJointPlan(instanceId) != jointPlan)
            throw new PlanLinkException(
                withCache.getJointPlan(instanceId) + " differs from " + indivPlan);
        ...
        if (removed != jointPlan)
          throw new PlanLinkException(removed + " differs from " + indivPlan + " for " + jointPlan);
      }
    }
  }
    ...
    synchronized (jointPlan) {
      for (Plan indivPlan : jointPlan.getIndividualPlans().values()) {
        if (indivPlan instanceof PlanWithCachedJointPlan) {
          ...
          if (withCache.getJointPlan(instanceId) != null
              && withCache.getJointPlan(instanceId) != jointPlan) {
            throw new PlanLinkException(
                withCache.getJointPlan(instanceId)
                    + " was associated to "
                    + indivPlan
  ...
  public Collection<JointPlan> getJointPlansOfSameComposition(final JointPlan jointPlan) {
    ...
    for (Plan p : person.getPlans()) {
      ...
      if (jp != null
          && jp.getIndividualPlans().keySet().equals(jointPlan.getIndividualPlans().keySet())) {
        jps.add(jp);
      }
    }
    return jps;
