...
public class JointTravelUtils {
  ...
  private static List<JointTrip> reconstructJointTrips(
      final List<DriverTrip> driverTrips, final JointPlan plan) {
    ...
    for (final DriverTrip driverTrip : driverTrips) {
      for (final Map.Entry<Id, Id> e : driverTrip.passengerOrigins.entrySet()) {
        ...
        jointTrips.add(
            getNextPassengerTrip(iterators, driverTrip, passengerId, originId, destinationId));
      }
    }
  ...
  private static JointTrip getNextPassengerTrip(
    ...
    final Iterator<PlanElement> it =
        iterators.getIterator(driverTrip.driverId, passengerId, originId, destinationId);
    while (it.hasNext()) {
      ...
      if (pe instanceof Leg && ((Leg) pe).getMode().equals(JointActingTypes.PASSENGER)) {
        ...
        if (route.getDriverId().equals(driverTrip.driverId)
          ...
          return new JointTrip(
              driverTrip.driverId,
              extractDriverSubTrip(originId, destinationId, driverTrip.driverTrip),
              passengerId,
              (Leg) pe);
        }
      }
    ...
    throw new RuntimeException(
        "no valid trip found for passenger " + passengerId + " in driver trip " + driverTrip);
  }

  private static List<Leg> extractDriverSubTrip(
      final Id originId, final Id destinationId, final List<Leg> driverTrip) {
  ...
  /**
   * Parses the plans and get information about driver trips, ie who drives whom from where to
   * where. Package protected to be callable from tests
   */
  static final List<DriverTrip> parseDriverTrips(final JointPlan plan) {
    ...
    for (final Plan indivPlan : plan.getIndividualPlans().values()) {
      ...
      for (final PlanElement pe : indivPlan.getPlanElements()) {
        if (pe instanceof Leg && JointActingTypes.DRIVER.equals(((Leg) pe).getMode())) {
          ...
          for (final Id passengerId : passengerIds) {
            if (!currentPassengers.contains(passengerId)) {
              currentDriverTrip.passengerOrigins.put(passengerId, origin);
          ...
          while (currPassengersIter.hasNext()) {
            ...
            if (!passengerIds.contains(p)) {
              ...
              // update destination
              currentDriverTrip.passengerDestinations.put(p, destination);
            }
          }
        } else if (pe instanceof Leg
            || !(JointActingTypes.JOINT_STAGE_ACTS.contains(((Activity) pe).getType())
  ...
  // Data structures
  /**
   * Represents the "joint travel structure" of the plan, that is, who travels with whom from where
   * to where, with references to the relevant plan elements.
   */
  public static final class JointTravelStructure {
    private final List<JointTrip> jointTrips;
    private final Map<Couple, List<JointTrip>> jointTripsPerCouple =
        new HashMap<Couple, List<JointTrip>>();

    public JointTravelStructure(final List<JointTrip> jointTrips) {
      this.jointTrips = jointTrips;

      for (final JointTrip jt : jointTrips) {
        final List<JointTrip> trips =
            MapUtils.getList(
                new Couple(jt.getDriverId(), jt.getPassengerId()), jointTripsPerCouple);
        trips.add(jt);
      }
    }
    ...
    public List<JointTrip> getJointTripsForCotravelers(final Id id1, final Id id2) {
      final List<JointTrip> trips = jointTripsPerCouple.get(new Couple(id1, id2));
  ...
  /**
   * Gathers information releted to one joint trip, that is, one (and <b>only one</b>) passenger
   * being driven. Note that driver legs may pertain to several joint trips, if the driver drives
   * several passengers at the same time.
   */
...
class PlanElementIterators {
  ...
  private final Map<TripCharacteristics, Iterator<PlanElement>> iterators =
      new HashMap<TripCharacteristics, Iterator<PlanElement>>();
  ...
  public Iterator<PlanElement> getIterator(
      final Id driver, final Id passenger, final Id origin, final Id destination) {
    ...
    if (it == null) {
      ...
      if (plan == null)
        throw new RuntimeException(
            "no plan for passenger " + passenger + " in joint plan " + jointPlan);
