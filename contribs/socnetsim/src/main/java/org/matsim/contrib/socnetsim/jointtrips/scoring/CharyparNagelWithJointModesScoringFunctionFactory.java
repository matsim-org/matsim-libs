...
public class CharyparNagelWithJointModesScoringFunctionFactory implements ScoringFunctionFactory {
  ...
  private final Map<String, LegScoringParameters> parametersPerMode =
      new HashMap<String, LegScoringParameters>();
  ...
  // /////////////////////////////////////////////////////////////////////////
  @Inject
  public CharyparNagelWithJointModesScoringFunctionFactory(final Scenario sc) {
    this(JointActingTypes.JOINT_STAGE_ACTS, sc);
  }

  public CharyparNagelWithJointModesScoringFunctionFactory(
      final Set<String> typesNotToScore, final Scenario scenario) {
    log.warn(
        "initializing the \"CharyparNagel\" scoring function: this has no special parameters for joint modes!");
    this.params =
    ...
    // standard modes
    parametersPerMode.put(TransportMode.car, LegScoringParameters.createForCar(params));
    parametersPerMode.put(TransportMode.pt, LegScoringParameters.createForPt(params));
    parametersPerMode.put(TransportMode.walk, LegScoringParameters.createForWalk(params));
    parametersPerMode.put(TransportMode.bike, LegScoringParameters.createForBike(params));
    parametersPerMode.put(TransportMode.transit_walk, LegScoringParameters.createForWalk(params));

    // joint modes
    parametersPerMode.put(JointActingTypes.DRIVER, LegScoringParameters.createForCar(params));
