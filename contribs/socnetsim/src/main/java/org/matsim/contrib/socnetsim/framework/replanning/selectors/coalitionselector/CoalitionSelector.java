...
/**
 * Selects a combination of plans without "blocking coalition", that is, there is no group of agents
 * which can improve their utility by trading together.
 *
 * @author thibautd
 */
public class CoalitionSelector implements GroupLevelPlanSelector {
  ...
  public CoalitionSelector(final WeightCalculator weight, final ConflictSolver conflictSolver) {
    this.weight = weight;
  ...
  public GroupPlans selectPlans(final JointPlans jointPlans, final ReplanningGroup group) {
    try {
      ...
      for (Person person : group.getPersons()) {
        final PointingAgent agent = new PointingAgent(person, group, weight);
      ...
      while (!agents.isEmpty()) {
        ...
        try {
          doIteration(jointPlans, agents, recordsPerJointPlan, groupPlans);
        } catch (Throwable e) {
      ...
      if (log.isTraceEnabled())
        log.trace(
            "did "
                + counter.getCounter()
                + " iterations ("
                + (((double) counter.getCounter()) / group.getPersons().size())
                + " per person)");
  ...
  private void doIteration(
    ...
    // need to check that agents are not empty, as there might be a last iteration just to remove
    // "switched off" agents,
    // that might cause problems with some conflict solvers (as there is no conflict to solve)
    if (!didSomething && !agents.isEmpty()) {
      conflictSolver.attemptToSolveConflicts(recordsPerJointPlan);
    }
  }
  ...
  private static boolean areAllPlansPointed(final Collection<PlanRecord> records) {
  ...
  public static class RecordsOfJointPlan {
    ...
    public void addRecord(final PlanRecord r) {
      ...
      if (jp != null) {
        MapUtils.getCollection(jp, recordsPerJointPlan).add(r);
    ...
    public String toString() {
      return "RecordsOfJointPlan{"
          + "recordsPerJointPlan ("
          + recordsPerJointPlan.size()
          + ")="
          + recordsPerJointPlan
          + ", recordsPerIndividualPlan ("
          + recordsPerIndividualPlan.size()
          + ")="
          + recordsPerIndividualPlan
          + '}';
    }
