...
/**
 * Creates "courtesy" events: agents say "hello" and "goodbye" to their social contacts located in
 * the same facility as them.
 *
 * <p>This is needed for scoring, because scoring functions cannot listen to the events of social
 * contacts.
 *
 * @author thibautd
 */
public class CourtesyEventsGenerator implements ActivityStartEventHandler, ActivityEndEventHandler {
  ...
  private final Map<Id<ActivityFacility>, Map<String, Set<Id<Person>>>> personsAtFacility =
      new HashMap<>();

  @Inject
  public CourtesyEventsGenerator(final EventsManager events, final Scenario sc) {
    this.events = events;
    this.socialNetwork = (SocialNetwork) sc.getScenarioElement(SocialNetwork.ELEMENT_NAME);
  }

  public CourtesyEventsGenerator(final EventsManager events, final SocialNetwork socialNetwork) {
    this.events = events;
  ...
  private void handleEvent(
    ...
    for (Id<Person> present : getPersonsAtFacilityForType(facility, actType)) {
      if (alters.contains(present)) {
        events.processEvent(new CourtesyEvent(time, actType, ego, present, type));
        events.processEvent(new CourtesyEvent(time, actType, present, ego, type));
      }
    }
  }

  private Set<Id<Person>> getPersonsAtFacilityForType(
      Id<ActivityFacility> facility, String actType) {
    return MapUtils.getSet(actType, MapUtils.getMap(facility, personsAtFacility));
  }
}
