 ...
 * @author thibautd
 */
public class PrepareVehicleAllocationForSimAlgorithm
    implements GenericPlanAlgorithm<ReplanningGroup> {
  ...
  public PrepareVehicleAllocationForSimAlgorithm(
    ...
    this.allocateVehiclesAlgo =
        new AllocateVehicleToPlansInGroupPlanAlgorithm(
            random, vehicles, SharedVehicleUtils.DEFAULT_VEHICULAR_MODES, true, true);

    this.recomposeJointPlansAlgo =
        planLinkIdentifier == null
            ? null
            : new RecomposeJointPlanAlgorithm(jointPlans.getFactory(), planLinkIdentifier);
  }
