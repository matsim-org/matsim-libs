...
public class JointPlansXmlReader extends MatsimXmlParser {
  ...
  public JointPlansXmlReader(final Scenario scenario) {
  ...
  public void startTag(final String name, final Attributes atts, final Stack<String> context) {
    ...
    if (name.equals(JointPlansXmlSchemaNames.PLAN_TAG)) {
      final Id<Person> id =
          Id.create(atts.getValue(JointPlansXmlSchemaNames.PERSON_ATT), Person.class);

      final int planIndex = Integer.parseInt(atts.getValue(JointPlansXmlSchemaNames.PLAN_NR_ATT));
      ...
      if (old != null)
        throw new RuntimeException(
            "joint plan contained " + old + " and " + plan + " for agent " + id);
    }
  }
  ...
  public void endTag(final String name, final String content, final Stack<String> context) {
    if (name.equals(JointPlansXmlSchemaNames.JOINT_PLAN_TAG)) {
      jointPlans.addJointPlan(jointPlans.getFactory().createJointPlan(currentJointPlan));
    }
  }
}
