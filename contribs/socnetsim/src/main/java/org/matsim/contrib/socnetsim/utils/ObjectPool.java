...
/**
 * A simple object to avoid multiplying identical instances of immutable objects. It uses a
 * WeakHashMap internally, so that no unnecessary instances are remembered. It helps achieving nice
 * memory consumption reduction when using with coordinates, for instance. <br>
 * Do not use with mutable objects!
 *
 * @author thibautd
 */
public class ObjectPool<T extends Object> {
  private static final Logger log = LogManager.getLogger(ObjectPool.class);
  ...
  /**
   * @param weaklyReference if true, pooled instances can be garbage collected if not referenced
   *     anywhere anymore (default)
   */
  ...
  /**
   * Get the pooled instance that is equal to the object passed as a parameter, or pool the object.
   *
   * @param object the object for which an instance is searched
   * @return the pooled instance equal to the object, if any, or the object itself, after being
   *     pooled.
   */
  ...
  public void printStats(final String poolName) {
    ...
    log.info(
        "["
            + poolName
            + "]: => "
            + (queries - unknown) * 100 / ((double) queries)
            + "% of the queries returned pooled instances");
  }
}
