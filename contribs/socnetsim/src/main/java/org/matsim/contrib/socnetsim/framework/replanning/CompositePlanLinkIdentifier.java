...
/**
 * Allows to define complex rules about which plans should be associated in a joint plan. It accepts
 * two kind of rules: "AND" and "OR" rules. For two plans to be linked, they need to be linked by
 * all "AND" rules and by at least one "OR" rule. <b> Note that this implies that a coumpond without
 * "OR" rule will never link any pair of plans! </b>
 *
 * @author thibautd
 */
public final class CompositePlanLinkIdentifier implements PlanLinkIdentifier {
    ...
    if (locked)
      throw new IllegalStateException(
          "cannot modify a "
              ...
              + " after its areLinked() method has been called");
    this.orDelegates.add(delegate);
  }

  /**
   * if one "and" component returns false, the plans are considered not being linked. Can be used
   * for instance to forbid linking plans of persons not linked by a social tie.
   */
  ...
  public String toString() {
    return "CompositePlanLinkIdentifier{" + "ANDS=" + andDelegates + ", ORS=" + orDelegates + '}';
  }
}
