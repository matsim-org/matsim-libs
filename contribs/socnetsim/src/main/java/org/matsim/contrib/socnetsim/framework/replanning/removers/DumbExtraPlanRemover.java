...
public class DumbExtraPlanRemover implements ExtraPlanRemover {
  ...
  public DumbExtraPlanRemover(
      final GroupLevelPlanSelector selectorForRemoval, final int maxPlanPerAgent) {
    this.selectorForRemoval = selectorForRemoval;
    this.maxPlanPerAgent = maxPlanPerAgent;
  }

  @Override
  public boolean removePlansInGroup(final JointPlans jointPlans, final ReplanningGroup group) {
    int c = 0;
  ...
  private final boolean removeOneExtraPlan(
    ...
    while (!personsToHandle.isEmpty()) {
      ...
      if (toRemove == null) {
        ...
        if (toRemove == null) {
          throw new RuntimeException(
              "The selector for removal returned no plan " + "for group " + group);
        }
      }
      ...
      for (Plan plan : toRemove(jointPlans, person, toRemove)) {
        ...
        if (personToHandle != person) {
          ...
          if (!removed)
            throw new RuntimeException(
                "person "
                    + personToHandle
                    + " is not part of the persons to handle when processing group "
                    + group);
        }
        ...
        if (!removed)
          throw new RuntimeException(
              "could not remove plan " + plan + " of person " + personToHandle);
  ...
  private static final Collection<Plan> toRemove(
      final JointPlans jointPlans, final Person person, final GroupPlans toRemove) {
