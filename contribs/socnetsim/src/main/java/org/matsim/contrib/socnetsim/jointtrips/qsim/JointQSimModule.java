...
public class JointQSimModule extends AbstractQSimModule {
  public static final String JOINT_MODES_DEPARTURE_HANDLER = "JointModesDepartureHandler";
  public static final String JOINT_PASSENGER_UNBOARDING = "JoinPassengerUnboarding";
  public static final String AGENTS_SOURCE_WITH_VEHICLES = "AgentSourceWithVehicles";
  public static final String REPLACEMENT_QNETSIM_ENGINE = "ReplacementQNetsimEngine";
  ...
  protected void configureQSim() {
    ...
    addQSimComponentBinding(JOINT_MODES_DEPARTURE_HANDLER).to(JointModesDepartureHandler.class);
    addQSimComponentBinding(AGENTS_SOURCE_WITH_VEHICLES)
        .to(PopulationAgentSourceWithVehicles.class);
  ...
  @Provides
  @Singleton
  PopulationAgentSourceWithVehicles providePopulationAgentSourceWithVehicles(
      Population population, PassengerUnboardingAgentFactory passAgentFactory, QSim qsim) {
    return new PopulationAgentSourceWithVehicles(population, passAgentFactory, qsim);
  }

  @Provides
  @Singleton
  PassengerUnboardingAgentFactory providePassengerUnboardingAgentFactory(
      Config config,
      QSim qsim,
      QNetsimEngineI netsimEngine,
      TimeInterpretation timeInterpretation) {
    return new PassengerUnboardingAgentFactory(
        config.transit().isUseTransit()
            ? new TransitAgentFactory(qsim, timeInterpretation)
            : new DefaultAgentFactory(qsim, timeInterpretation),
