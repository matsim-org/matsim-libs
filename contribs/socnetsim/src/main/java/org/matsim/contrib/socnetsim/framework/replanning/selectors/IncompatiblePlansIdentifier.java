...
/**
 * Allows to define incomaptibility relations between plans. Two incompatible plans cannot be
 * selected at the same time. This is useful for instance when using vehicle ressources, to enforce
 * that only one joint plan related to one given vehicle is selected at the same time.
 *
 * @author thibautd
 */
public abstract class IncompatiblePlansIdentifier<T> {
   ...
   * @return a collection containing ids of "incompatibility groups". Two plans are considered
   *     incompatible if they have at least one group in common.
   */
  ...
  public Set<Id<T>> identifyIncompatibilityGroups(final JointPlans jointPlans, final Plan plan) {
    ...
    return jp == null ? identifyIncompatibilityGroups(plan) : identifyIncompatibilityGroups(jp);
  }
}
