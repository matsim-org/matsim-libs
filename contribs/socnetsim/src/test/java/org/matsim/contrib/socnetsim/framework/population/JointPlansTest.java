...
public class JointPlansTest {
  ...
  private static void testExceptionAdd(final boolean withCache) throws Exception {
    Plan p1 =
        createPlan(PopulationUtils.getFactory().createPerson(Id.createPersonId(1)), withCache);
    Plan p2 =
        createPlan(PopulationUtils.getFactory().createPerson(Id.createPersonId(2)), withCache);
    Plan p3 =
        createPlan(PopulationUtils.getFactory().createPerson(Id.createPersonId(3)), withCache);
    ...
    Assert.assertTrue(
        "got no exception when associating two joint plans to one individual plan", gotException);
  }
  ...
  private static void testExceptionRemove(final boolean withCache) throws Exception {
    ...
    JointPlan wrongInstance = new JointPlan(jp1);
  ...
  private static void testAddAndGetSeveralInstances(final boolean withCache) {
    ...
    Assert.assertSame("wrong joint plan 1 for person 1", jointPlan1, jointPlans1.getJointPlan(p1));
    Assert.assertSame("wrong joint plan 1 for person 2", jointPlan1, jointPlans1.getJointPlan(p2));

    Assert.assertSame("wrong joint plan 2 for person 1", jointPlan2, jointPlans2.getJointPlan(p1));
    Assert.assertSame("wrong joint plan 2 for person 2", jointPlan2, jointPlans2.getJointPlan(p2));

    Assert.assertSame("wrong joint plan 3 for person 1", jointPlan3, jointPlans3.getJointPlan(p1));
    Assert.assertSame("wrong joint plan 3 for person 2", jointPlan3, jointPlans3.getJointPlan(p2));

    Assert.assertSame("wrong joint plan 4 for person 1", jointPlan4, jointPlans4.getJointPlan(p1));
    Assert.assertSame("wrong joint plan 4 for person 2", jointPlan4, jointPlans4.getJointPlan(p2));

    Assert.assertSame("wrong joint plan 5 for person 1", jointPlan5, jointPlans5.getJointPlan(p1));
    Assert.assertSame("wrong joint plan 5 for person 2", jointPlan5, jointPlans5.getJointPlan(p2));
  }
  ...
  private static void testClear(final boolean withCache) {
    ...
    Assert.assertNotNull("joint plan was not added???", jointPlans.getJointPlan(p1));
    jointPlans.clear();

    Assert.assertNull("still a joint plan after clear...", jointPlans.getJointPlan(p1));
  }
