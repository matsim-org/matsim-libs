...
public class DynamicGroupIdentifierTest {
    ...
    for (int i = 0; i < nPersons; i++) {
      final Person p =
          scenario
              .getPopulation()
              .getFactory()
              .createPerson(Id.create("person-" + i, Person.class));
      ...
      for (int j = 0; j < nPersonsPerGroup; j++) {
        final Person p =
            scenario
                .getPopulation()
                .getFactory()
                .createPerson(Id.createPersonId("person-" + i + "." + j));
        persons[j] = p;
        scenario.getPopulation().addPerson(p);
      }
  ...
  private void test(final Fixture fixture) {
    ...
    final Collection<ReplanningGroup> groups =
        testee.identifyGroups(fixture.scenario.getPopulation());
    Assert.assertEquals("unexpected number of groups", fixture.expectedNGroups, groups.size());
  ...
  private static class Fixture {
    ...
    public Fixture(final Scenario scenario, final int expectedNGroups) {
