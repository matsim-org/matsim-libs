...
public class TourModeUnifierAlgorithmTest {
  ...
  public void testPlanWithOneSingleTour() throws Exception {
    final Plan plan =
        PopulationUtils.createPlan(
            PopulationUtils.getFactory().createPerson(Id.create("jojo", Person.class)));
    ...
    plan.addActivity(
        PopulationUtils.createActivityFromLinkId(
            "type-" + random.nextLong(), Id.create(random.nextLong(), Link.class)));
    ...
    plan.addActivity(
        PopulationUtils.createActivityFromLinkId(
            stageType, Id.create(random.nextLong(), Link.class)));
    plan.addLeg(PopulationUtils.createLeg("mode-" + random.nextLong()));
    plan.addActivity(
        PopulationUtils.createActivityFromLinkId(
            stageType, Id.create(random.nextLong(), Link.class)));
    ...
    final int nActs =
        TripStructureUtils.getActivities(plan, StageActivityHandling.ExcludeStageActivities).size();

    final PlanAlgorithm testee =
        new TourModeUnifierAlgorithm(
            TripStructureUtils::isStageActivityType, new MainModeIdentifierImpl());
    testee.run(plan);

    Assert.assertEquals("unexpected plan size", 2 * nActs - 1, plan.getPlanElements().size());
    for (Trip trip : TripStructureUtils.getTrips(plan, TripStructureUtils::isStageActivityType)) {
      Assert.assertEquals("unexpected size of trip " + trip, 1, trip.getTripElements().size());

      Assert.assertEquals(
          "unexpected mode of trip " + trip, mode, ((Leg) trip.getTripElements().get(0)).getMode());
    }
  }
  ...
  public void testPlanWithTwoToursOnOpenTour() throws Exception {
    ...
    Assert.assertEquals("unexpected plan size", 31, plan.getPlanElements().size());

    final List<Trip> trips =
        TripStructureUtils.getTrips(plan, TripStructureUtils::isStageActivityType);

    Assert.assertEquals("unexpected number of trips", 13, trips.size());
    ...
    for (int tripNr : new int[] {2, 3, 3, 4, 5}) {
      Assert.assertEquals(
          "unexpected length for trip " + tripNr, 1, trips.get(tripNr).getLegsOnly().size());
  ...
  public void testPlanWithTwoHomeBasedTours() throws Exception {
    ...
    Assert.assertEquals("unexpected number of trips", 8, trips.size());
