...
public class RecomposeJointPlanAlgorithmTest {
  private static class Fixture {
    ...
    public Fixture(
        final GroupPlans groupPlans, final Collection<Set<Id<Person>>> expectedJointPlanStructure) {
      this(
          groupPlans,
  ...
  private Fixture createRandomFixtureWithIndividualPlans(final Random random) {
    ...
    return new Fixture(
        new GroupPlans(Collections.<JointPlan>emptyList(), plans), jointPlansToExpect);
  }
  ...
  private Fixture createRandomFixtureWithOneBigJointPlan(final Random random) {
    ...
    return new Fixture(
        new GroupPlans(
            Collections.singleton(new JointPlanFactory().createJointPlan(jointPlan)), plans),
        jointPlansToExpect);
  }
  ...
  private Fixture createRandomFixtureWithJointAndIndividualPlans(final Random random) {
    ...
    return new Fixture(new GroupPlans(jointPlans, plans), jointPlansToExpect);
  }
  ...
  private Fixture createRandomFixtureWithIncompleteLinks(final Random random) {
    ...
    return new Fixture(
        new GroupPlans(jointPlans, plans),
        jointPlansToExpect,
        new PlanLinkIdentifier() {
          @Override
          public boolean areLinked(final Plan p1, final Plan p2) {
  ...
  private static void test(final Fixture fixture) {
    final RecomposeJointPlanAlgorithm algo =
        new RecomposeJointPlanAlgorithm(new JointPlanFactory(), fixture.identifier);
