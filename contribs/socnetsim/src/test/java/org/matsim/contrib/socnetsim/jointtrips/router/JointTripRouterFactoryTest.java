...
public class JointTripRouterFactoryTest {
  private static final Logger log = LogManager.getLogger(JointTripRouterFactoryTest.class);
  ...
  private static Scenario createScenario() {
    ...
    NetworkUtils.createAndAddLink(
        net, id2, fromNode, toNode, (double) 1, (double) 1, (double) 1, (double) 1);
    final Id<Link> id3 = link2;
    final Node fromNode1 = node2inst;
    final Node toNode1 = node3inst;
    NetworkUtils.createAndAddLink(
        net, id3, fromNode1, toNode1, (double) 1, (double) 1, (double) 1, (double) 1);
    final Id<Link> id4 = link3;
    final Node fromNode2 = node3inst;
    final Node toNode2 = node4inst;
    NetworkUtils.createAndAddLink(
        net, id4, fromNode2, toNode2, (double) 1, (double) 1, (double) 1, (double) 1);
    ...
    PopulationUtils.createAndAddActivityFromLinkId(plan, JointActingTypes.INTERACTION, linkId2)
        .setMaximumDuration(0);
                    ...
                    new AbstractModule() {
                      ...
                      public void install() {
                        bind(EventsManager.class)
                            .toInstance(EventsUtils.createEventsManager(scenario.getConfig()));
  ...
  public void testPassengerRoute() throws Exception {
    final PlanAlgorithm planRouter =
        new JointPlanRouterFactory(
                (ActivityFacilities) null, TimeInterpretation.create(ConfigUtils.createConfig()))
            .createPlanRoutingAlgorithm(factory.get());
      ...
      if (toRoute) {
        ...
        for (PlanElement pe : plan.getPlanElements()) {
          if (pe instanceof Leg && ((Leg) pe).getMode().equals(JointActingTypes.PASSENGER)) {
            ...
            Assert.assertEquals("wrong driver Id", driver, actualDriver);
          }
        }
      }
    }
  ...
  public void testDriverRoute() throws Exception {
    ...
    for (Person pers : scenario.getPopulation().getPersons().values()) {
      ...
      if (toRoute) {
        ...
        for (PlanElement pe : plan.getPlanElements()) {
          if (pe instanceof Leg && ((Leg) pe).getMode().equals(JointActingTypes.DRIVER)) {
            final Collection<Id<Person>> actualPassengers =
                ((DriverRoute) ((Leg) pe).getRoute()).getPassengersIds();

            Assert.assertEquals(
                "wrong number of passengers", passengerIds.size(), actualPassengers.size());
