...
public class LexicographicRemoverTest {
	...
	public void testOnlyIndividualPlans() {
		...
		for ( int i=0; i < 4; i++ ) {
			final Person person = PopulationUtils.getDefaultFactory().createPerson(Id.create(i, Person.class));
	...
	public void testOnlyOneComposition() {
		...
		for ( int i=0; i < 4; i++ ) {
			group.addPerson(PopulationUtils.getDefaultFactory().createPerson(Id.create(i, Person.class)));
		}
