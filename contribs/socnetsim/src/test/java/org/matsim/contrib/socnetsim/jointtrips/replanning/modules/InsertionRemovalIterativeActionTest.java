...
public class InsertionRemovalIterativeActionTest {
  private static final int N_COUPLES = 100;
    ...
    for (Plan individualPlan : jointPlan.getIndividualPlans().values()) {
      for (PlanElement pe : individualPlan.getPlanElements()) {
        ...
        if (JointActingTypes.DRIVER.equals(mode)) {
          Assert.assertNotNull("route must not be null", l.getRoute());
          Assert.assertTrue(
  ...
  private JointPlan createPlanWithJointTrips() {
    ...
    for (int i = 0; i < N_COUPLES; i++) {
      ...
      PopulationUtils.createAndAddActivityFromLinkId(
              plan, "first_act_d" + i, Id.create("some_link", Link.class))
          .setEndTime(10);
      Leg leg5 = PopulationUtils.createAndAddLeg(plan, TransportMode.car);
      final Id<Link> linkId = puLink;
      PopulationUtils.createAndAddActivityFromLinkId(plan, JointActingTypes.INTERACTION, linkId)
          .setMaximumDuration(0);
      Leg leg4 = PopulationUtils.createAndAddLeg(plan, JointActingTypes.DRIVER);
      Leg driverLeg1 = leg4;
      final Id<Link> linkId1 = doLink;
      PopulationUtils.createAndAddActivityFromLinkId(plan, JointActingTypes.INTERACTION, linkId1)
          .setMaximumDuration(0);
      Leg leg3 = PopulationUtils.createAndAddLeg(plan, TransportMode.car);
      PopulationUtils.createAndAddActivityFromLinkId(
          plan, "second_act_d" + i, Id.create("nowhere", Link.class));
      ...
      PopulationUtils.createAndAddActivityFromLinkId(
              plan, "first_act_p" + i, Id.create("earth", Link.class))
          .setEndTime(10);
      Leg leg2 = PopulationUtils.createAndAddLeg(plan, TransportMode.walk);
      PopulationUtils.createAndAddActivityFromLinkId(plan, JointActingTypes.INTERACTION, puLink)
          .setMaximumDuration(0);
      Leg leg1 = PopulationUtils.createAndAddLeg(plan, JointActingTypes.PASSENGER);
      Leg passengerLeg1 = leg1;
      PopulationUtils.createAndAddActivityFromLinkId(plan, JointActingTypes.INTERACTION, doLink)
          .setMaximumDuration(0);
      Leg leg = PopulationUtils.createAndAddLeg(plan, TransportMode.walk);
      PopulationUtils.createAndAddActivityFromLinkId(
          plan, "second_act_p" + i, Id.create("space", Link.class));
