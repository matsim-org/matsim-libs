...
public class JointTravelingSimulationIntegrationTest {
  ...
  private void testAgentsArriveTogether(final Fixture fixture) throws Exception {
    ...
    for (int i = 0; i < N_RANDOM_SCENARIOS; i++) {
      ...
      final Scenario sc = createTestScenario(fixture, random);
      final EventsManager events = EventsUtils.createEventsManager();

      final EventWriter eventsWriter =
          new EventWriterXML(utils.getOutputDirectory() + "/events.xml.gz");
      if (DUMP_EVENTS) {
      ...
      events.addHandler(
          new PersonArrivalEventHandler() {
            ...
            public void handleEvent(final PersonArrivalEvent event) {
              ...
              log.info(
                  "agent " + event.getPersonId() + ": " + mode + " arrival at " + event.getTime());
      ...
      Assert.assertEquals(
          "run " + i + ": unexpected number of joint arrivals", N_LAPS * 3, arrCount.get());
  ...
  private void testNumberOfEnterLeaveVehicle(final RouteType routeType) {
    ...
    for (int i = 0; i < N_RANDOM_SCENARIOS; i++) {
      ...
      events.addHandler(
          new PersonEntersVehicleEventHandler() {
            @Override
      ...
      events.addHandler(
          new PersonLeavesVehicleEventHandler() {
            @Override
            public void reset(int iteration) {}
      ...
      Assert.assertEquals(
          "not as many leave events as enter events", enterCount.get(), leaveCount.get());
    }
  }

  private Fixture createFixture(final boolean insertDummyActivities, final RouteType routeType) {
    switch (routeType) {
      case normal:
        return new Fixture(
      ...
      case puAtDo:
        return new Fixture(
            ...
            Arrays.asList(TRAVEL_LINK_1, TRAVEL_LINK_2, DO_LINK, TO_DESTINATION_LINK),
            Arrays.asList(RETURN_LINK));
  ...
  private Scenario createTestScenario(final Fixture fixture, final Random random) {
      ...
      for (int i = 0; i < N_LAPS; i++) {
        ...
        l.setRoute(
            RouteUtils.createLinkNetworkRouteImpl(
                fixture.originLink, fixture.toPuRoute, fixture.puLink));
        driverPlan.addLeg(l);

        if (fixture.insertDummyActivities) {
          act = factory.createActivityFromLinkId(JointActingTypes.INTERACTION, fixture.puLink);
          driverPlan.addActivity(act);
          act.setMaximumDuration(0);
        }

        l = factory.createLeg(JointActingTypes.DRIVER);
        DriverRoute dRoute = new DriverRoute(fixture.puLink, fixture.doLink);
        dRoute.setLinkIds(fixture.puLink, fixture.puToDoRoute, fixture.doLink);
        ...
        if (fixture.insertDummyActivities) {
          act = factory.createActivityFromLinkId(JointActingTypes.INTERACTION, fixture.doLink);
          driverPlan.addActivity(act);
          act.setMaximumDuration(0);
        }
        ...
        l.setRoute(
            RouteUtils.createLinkNetworkRouteImpl(
                fixture.doLink, fixture.doToDestRoute, fixture.destinationLink));
        driverPlan.addLeg(l);

        final Activity work =
            factory.createActivityFromLinkId(DESTINATION_ACT, fixture.destinationLink);
        ...
        l.setRoute(
            RouteUtils.createLinkNetworkRouteImpl(
                fixture.destinationLink, fixture.destToOrigRoute, fixture.originLink));
        driverPlan.addLeg(l);
      }
    ...
    for (Id passengerId : new Id[] {passengerId1, passengerId2}) {
      ...
      for (int i = 0; i < N_LAPS; i++) {
        ...
        PassengerRoute pRoute = new PassengerRoute(fixture.puLink, fixture.doLink);
  ...
  private void createNetwork(final Network network) {
    ...
    final Node firstNode =
        network.getFactory().createNode(Id.create(c++, Node.class), new Coord((double) 0, y2));
    Node node1 = firstNode;
    final double y1 = d++;
    Node node2 =
        network.getFactory().createNode(Id.create(c++, Node.class), new Coord((double) 0, y1));
  ...
  private static class Fixture {
    private final boolean insertDummyActivities;
    private final Id<Link> originLink;
    private final Id<Link> puLink;
    private final Id<Link> doLink;
    private final Id<Link> destinationLink;
    private final List<Id<Link>> toPuRoute;
    private final List<Id<Link>> puToDoRoute;
    private final List<Id<Link>> doToDestRoute;
    private final List<Id<Link>> destToOrigRoute;
