...
public class GroupPlansTest {
  ...
  public void initPlanWithoutIndividualPlans() {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      for (int j = 0; j < 5; j++) {
        ...
        planMap.put(id, PopulationUtils.createPlan(PopulationUtils.getFactory().createPerson(id1)));
      }
  ...
  public void testGetters() throws Exception {
    ...
    assertEquals(
        "wrong number of individual plans", indivPlans.size(), testee.getIndividualPlans().size());

    assertEquals("wrong number of joint plans", jointPlans.size(), testee.getJointPlans().size());
  ...
  public void testCopyIsNotSame() throws Exception {
    for (GroupPlans plans : testPlans) {
      ...
      assertNotSame("copy is the same instance", plans, copy);

      for (Plan p : plans.getIndividualPlans()) {
        assertFalse(
            "the copy contains references from the copied", copy.getIndividualPlans().contains(p));
      }
  ...
  private JointPlan getCopy(final JointPlan copiedJointPlan, final GroupPlans copy) {
