  ...
  public void testOnePlanSelectedForEachAgent() throws Exception {
    final WhoIsTheBossSelector testee =
        new WhoIsTheBossSelector(
            new Random(9087), new EmptyIncompatiblePlansIdentifierFactory(), new ScoreWeight());
    ...
    for (int i = 0; i < 100; i++) {
      counter.incCounter();
      final ReplanningGroup group = createNextTestClique(ids, jointPlans, random);
      ...
      if (selected == null) {
        ...
        Assert.assertEquals("unexpected agent ids in selected plan", groupIds, selectedIds);
      }
    }
