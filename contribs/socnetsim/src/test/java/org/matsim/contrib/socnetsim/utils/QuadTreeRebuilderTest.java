...
public class QuadTreeRebuilderTest {
  ...
  public void testGrowingQuadTree() {
    ...
    Assert.assertEquals("unexpected quadtree size", 0, rebuilder.getQuadTree().size());

    rebuilder.put(new Coord(0, 0), new Object());

    Assert.assertEquals("unexpected quadtree size", 1, rebuilder.getQuadTree().size());

    rebuilder.put(new Coord(100, 100), new Object());

    Assert.assertEquals("unexpected quadtree size", 2, rebuilder.getQuadTree().size());
