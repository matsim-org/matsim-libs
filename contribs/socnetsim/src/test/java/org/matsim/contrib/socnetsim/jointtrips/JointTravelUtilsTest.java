...
public class JointTravelUtilsTest {
  ...
  public void initOnePassengerFixture() {
    final Person driver =
        PopulationUtils.getFactory().createPerson(Id.createPersonId("Alain Prost"));
    final Person passenger1 =
        PopulationUtils.getFactory().createPerson(Id.createPersonId("Tintin"));
  ...
  public void initTwoPassengerTwoOdFixture() {
    final Person driver =
        PopulationUtils.getFactory().createPerson(Id.create("Alain Prost", Person.class));
    final Person passenger1 =
        PopulationUtils.getFactory().createPerson(Id.create("Tintin", Person.class));
    final Person passenger2 =
        PopulationUtils.getFactory().createPerson(Id.create("Milou", Person.class));
    ...
    driverLeg2.setRoute(
        new DriverRoute(
            ...
            Arrays.asList(passenger1.getId(), passenger2.getId())));
  ...
  public void initOnePassengerTwoTripsWithDifferentDriversFixture() {
    final Person driver1 =
        PopulationUtils.getFactory().createPerson(Id.createPersonId("Alain Prost"));
    final Person driver2 =
        PopulationUtils.getFactory().createPerson(Id.createPersonId("Michel Vaillant"));
    final Person passenger1 =
        PopulationUtils.getFactory().createPerson(Id.createPersonId("Tintin"));
  ...
  /**
   * Link the first trip of the driver with the second of the passenger, and vice-versa. This is
   * obbviously infeasible, but may be considered as valid, the same way going to the bus station
   * when no bus is running is a valid, though stupid, plan.
   */
  @Before
  ...
  public void testExtractJointTrips() throws Exception {
    for (Fixture f : fixtures) {
      ...
      Assert.assertEquals(
          "wrong structure for fixture "
              + f.name
              + " of size "
              + f.structure.getJointTrips().size()
              + " compared to result of size "
              + struct.getJointTrips().size(),
          f.structure,
          struct);
    }
  }
