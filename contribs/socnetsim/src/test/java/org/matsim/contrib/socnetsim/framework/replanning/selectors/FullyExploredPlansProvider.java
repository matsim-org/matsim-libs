...
import org.matsim.contrib.socnetsim.framework.replanning.grouping.FixedGroupsIdentifierFileParser;
import org.matsim.contrib.socnetsim.framework.replanning.grouping.GroupIdentifier;
import org.matsim.contrib.socnetsim.framework.replanning.grouping.GroupPlans;
import org.matsim.contrib.socnetsim.framework.replanning.grouping.ReplanningGroup;
import org.matsim.contrib.socnetsim.framework.replanning.selectors.highestweightselection.FullExplorationSelector;
...
public class FullyExploredPlansProvider {
  private static final Logger log = LogManager.getLogger(FullyExploredPlansProvider.class);

  private static final String CACHE_DIRECTORY =
      "test/output/cache/" + FullyExploredPlansProvider.class.getName() + "/";

  private FullyExploredPlansProvider() {}
  ;

  public static SelectedInformation getGroupsAndSelected(
      final IncompatiblePlansIdentifierFactory incompFactory, final boolean isBlocking) {
  ...
  private static SelectedInformation readGroupsAndSelected(
    ...
    try {
      ...
      final GroupIdentifier cliques =
          FixedGroupsIdentifierFileParser.readCliquesFile(paths.cliquesFilePath);
      ...
      new JointPlansXmlReader(scenario).readFile(paths.jointPlansFilePath);
      ...
      for (ReplanningGroup clique : cliques.identifyGroups(scenario.getPopulation())) {
        ...
        final GroupPlans gps = new GroupPlans(selectedJointPlans, selectedPlans);
        final int nPlans = gps.getAllIndividualPlans().size();

        assert nPlans == 0 || nPlans == clique.getPersons().size()
            : nPlans + " != " + clique.getPersons().size();

        information.addInformation(clique, nPlans == 0 ? null : gps);
      }
  ...
  private static StoragePaths getStoragePaths(
    ...
    final String baseDir =
        CACHE_DIRECTORY + "/" + incompFactory.getClass().getName() + "/" + isBlocking + "/";
  ...
  private static SelectedInformation createGroupsAndSelected(
    ...
    for (int i = 0; i < nTries; i++) {
      counter.incCounter();
      final ReplanningGroup clique = createNextTestClique(ids, selecteds.getJointPlans(), random);
      final GroupPlans fullResult = fullSelector.selectPlans(selecteds.getJointPlans(), clique);
      selecteds.addInformation(clique, fullResult);
    }
  ...
  private static ReplanningGroup createNextTestClique(
      final Iterator<Id<Person>> idsIterator, final JointPlans jointPlans, final Random random) {
  ...
  public static class SelectedInformation {
    ...
    private void addInformation(final ReplanningGroup group, final GroupPlans selected) {
      plans.add(new Tuple<ReplanningGroup, GroupPlans>(group, selected));
    }
  }

  private static SelectedInformation generateInputData(
      final IncompatiblePlansIdentifierFactory factory, final boolean blocking) {
    ...
    for (Tuple<ReplanningGroup, GroupPlans> info : toDump.getGroupInfos()) {
      cliquesWriter.writeClique(Id.create(id++, Clique.class), info.getFirst().getPersons());
      ...
      assert info.getSecond() == null
          || consistentSelectionStatus(info.getSecond().getJointPlans());
      assert info.getSecond() == null
          || !inJointPlans(info.getSecond().getIndividualPlans(), toDump.getJointPlans());
    }
    cliquesWriter.finishAndCloseFile();

    new PopulationWriter(scenario.getPopulation(), scenario.getNetwork())
        .write(paths.plansFilePath);

    JointPlansXmlWriter.write(
        scenario.getPopulation(), toDump.getJointPlans(), paths.jointPlansFilePath);
  ...
  private static boolean inJointPlans(
      final Collection<Plan> individualPlans, final JointPlans jointPlans) {
...
class StoragePaths {
  ...
  public StoragePaths(final String directoryPath) {
