...
public class RandomSelectorsTest {
  ...
  public void createPartiallyJointPlansMessOfJointPlans() {
    ...
    jointPlans.addJointPlan(jointPlans.getFactory().createJointPlan(jp5));
    jointPlans.addJointPlan(jointPlans.getFactory().createJointPlan(jp6));
    jointPlans.addJointPlan(jointPlans.getFactory().createJointPlan(jp7));
    jointPlans.addJointPlan(jointPlans.getFactory().createJointPlan(jp8));
  }
  ...
  public void createRandomFixtures() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      while (!planLists.isEmpty()) {
        ...
        jointPlans.addJointPlan(jointPlans.getFactory().createJointPlan(jp));
      }
    }
  }
      ...
      for (int i = 0; i < 100; i++) {
        ...
        final GroupLevelPlanSelector selector =
            new RandomGroupLevelSelector(
                new Random(seed), new EmptyIncompatiblePlansIdentifierFactory());

        final GroupPlans selected = selector.selectPlans(jointPlans, group);
        if (previous != null) {
          assertEquals("different results with the same random seed", previous, selected);
        }

        if (selected == null)
          throw new NullPointerException("test is useless if the selector returns null");
        previous = selected;
      }
    }
    count.printCounter();
  ...
  public void testNoFailuresWithVariousSeeds() throws Exception {
    final RandomGroupLevelSelector selector =
        new RandomGroupLevelSelector(
            new Random(123), new EmptyIncompatiblePlansIdentifierFactory());
