...
public class JointScenarioUtilsTest {
  ...
  public void testJointTripsImport() throws Exception {
    ...
    for (Person person : loadedPopulation.getPersons().values()) {
      ...
      if (dumpedLeg.getMode().equals(JointActingTypes.DRIVER)) {
        assertEquals("wrong route class", DriverRoute.class, loadedLeg.getRoute().getClass());

        final Collection<Id<Person>> dumpedPassengers =
            ((DriverRoute) dumpedLeg.getRoute()).getPassengersIds();
        final Collection<Id<Person>> loadedPassengers =
            ((DriverRoute) loadedLeg.getRoute()).getPassengersIds();

        assertEquals("unexpected passenger ids", dumpedPassengers, loadedPassengers);
      } else {
        assertEquals("wrong route class", PassengerRoute.class, loadedLeg.getRoute().getClass());
        ...
        assertEquals("unexpected passenger ids", dumpedDriver, loadedDriver);
      }
    }
  }
  ...
  private static Population createPopulation() {
    ...
    driverPlan.addActivity(
        population.getFactory().createActivityFromLinkId("h", Id.create(1, Link.class)));
    ...
    final Person passenger =
        population.getFactory().createPerson(Id.create("passenger", Person.class));
