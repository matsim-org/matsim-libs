...
public class FullExplorationVsCuttoffTest {
  private static final Logger log = LogManager.getLogger(FullExplorationVsCuttoffTest.class);
  ...
  public void testFullExplorationVsCuttoffIncompatibility() throws Exception {
    ...
    testFullExplorationVsCuttoff(new FewGroupsIncompatibilityFactory(), false);
  }
  ...
  public void testFullExplorationVsCuttoffIncompatibilityBlocking() throws Exception {
    ...
    testFullExplorationVsCuttoff(new FewGroupsIncompatibilityFactory(), true);
  }

  private void testFullExplorationVsCuttoff(
      final IncompatiblePlansIdentifierFactory incompFactory, final boolean blocking)
      throws Exception {
    final HighestScoreSumSelector fastSelector =
        new HighestScoreSumSelector(incompFactory, blocking);

    final Counter count = new Counter("test plan # ");
    final SelectedInformation selecteds =
        FullyExploredPlansProvider.getGroupsAndSelected(incompFactory, blocking);
    int countNull = 0;
    for (Tuple<ReplanningGroup, GroupPlans> groupInfo : selecteds.getGroupInfos()) {
      ...
      count.incCounter();
      final GroupPlans fastResult = fastSelector.selectPlans(selecteds.getJointPlans(), clique);
