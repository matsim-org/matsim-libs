...
public class SocialNetworkTest {
  ...
  public void testMonodirectionalTie() {
    ...
    Assert.assertEquals(
        "unexpected egos", new HashSet<Id<Person>>(Arrays.asList(ego, alter)), sn.getEgos());

    Assert.assertEquals(
        "unexpected alters of ego", Collections.singleton(alter), sn.getAlters(ego));

    Assert.assertEquals(
        "unexpected alters of alter", Collections.<Id<Person>>emptySet(), sn.getAlters(alter));
  }
  ...
  public void testBidirectionalTie() {
    ...
    Assert.assertEquals(
        "unexpected alters of alter", Collections.singleton(ego), sn.getAlters(alter));
  }
  ...
  public void testRemoveEgo() {
    ...
    sn.removeEgo(ego);
    Assert.assertFalse("ego still in social network", sn.getEgos().contains(ego));

    for (Id<Person> alter : new Id[] {a1, a2, a3, a4, a5, a6, a7, a8, a9, a10}) {
      Assert.assertFalse(
          "ego still in network of agent " + alter, sn.getAlters(alter).contains(ego));
    }

    for (Id<Person> alter : new Id[] {a1, a2, a3, a4, a5}) {
      Assert.assertEquals("wrong network size for " + alter, 0, sn.getAlters(alter).size());
    }

    for (Id<Person> alter : new Id[] {a7, a8, a9, a10}) {
      Assert.assertEquals("wrong network size for " + alter, 1, sn.getAlters(alter).size());
    }

    Assert.assertEquals("wrong network size for " + a6, 4, sn.getAlters(a6).size());
  }
}
