...
public class InsertionRemovalIgnoranceBehaviorTest {
  ...
  public void testRemoverIgnorance() throws Exception {
    final JointTripRemoverAlgorithm algo =
        new JointTripRemoverAlgorithm(random, new MainModeIdentifierImpl());

    JointPlan jointPlan = createPlanWithJointTrips();

    assertNull(
        "unexpected removed trips", algo.run(jointPlan, jointPlan.getIndividualPlans().keySet()));
  }
    ...
    final JointTripInsertorAlgorithm algo =
        new JointTripInsertorAlgorithm(
            ...
            (JointTripInsertorConfigGroup)
                config.getModule(JointTripInsertorConfigGroup.GROUP_NAME),
  ...
  private JointPlan createPlanWithJointTrips() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      PopulationUtils.createAndAddActivityFromLinkId(plan, JointActingTypes.INTERACTION, linkId3)
          .setMaximumDuration(0);
      PopulationUtils.createAndAddLeg(plan, TransportMode.walk);
      PopulationUtils.createAndAddActivityFromLinkId(
          plan, "second_act_p" + i, Id.create("space", Link.class));
