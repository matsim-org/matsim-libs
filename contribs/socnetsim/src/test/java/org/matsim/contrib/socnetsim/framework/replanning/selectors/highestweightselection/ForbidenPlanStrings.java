...
public class ForbidenPlanStrings {
  ...
  private static boolean forbids(final GroupPlans forbidden, final PlanString string) {
    ...
    while (tail != null) {
      ...
      if (head.jointPlan != null && !forbidden.getJointPlans().contains(head.jointPlan)) {
        return false;
      }

      if (head.jointPlan == null && !plans.remove(head.plan)) {
        assert !forbidden.getIndividualPlans().contains(head.plan)
            : "planString contains duplicates";
        return false;
      }
    }
