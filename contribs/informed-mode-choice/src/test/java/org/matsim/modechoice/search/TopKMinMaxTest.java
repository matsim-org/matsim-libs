...
public class TopKMinMaxTest {
  ...
  @Mock private TripRouter router;

  @Mock private ControlerListenerManager cl;

  @Mock private EventsManager em;
  ...
  public void setUp() throws Exception {
    ...
    InformedModeChoiceConfigGroup config =
        ConfigUtils.addOrGetModule(testModule.config, InformedModeChoiceConfigGroup.class);
  ...
  public void minmax() {
    ...
    assertThat(it.next())
        ...
        .extracting(PlanCandidate::getUtility)
  ...
  public void subset() {
    ...
    Collection<PlanCandidate> candidates =
        generator.generate(model, null, new boolean[] {true, false});
  ...
  private class TestModule extends AbstractModule {
    ...
    protected void configure() {
      ...
      when(router.calcRoute(any(), any(), any(), anyDouble(), any(), any()))
          .then(
              new Answer<List<PlanElement>>() {
                @Override
                public List<PlanElement> answer(InvocationOnMock invocationOnMock)
                    throws Throwable {
      ...
      MapBinder<String, ModeOptions<?>> optionBinder =
      ...
      MapBinder<String, FixedCostsEstimator<?>> fcBinder =
          MapBinder.newMapBinder(binder(), new TypeLiteral<>() {}, new TypeLiteral<>() {});

      fcBinder.addBinding(TransportMode.car).toInstance((context, mode, option) -> -1);

      MapBinder<String, LegEstimator<?>> legBinder =
          MapBinder.newMapBinder(binder(), new TypeLiteral<>() {}, new TypeLiteral<>() {});

      legBinder.addBinding(TransportMode.walk).toInstance((context, mode, leg, option) -> -0.5);

      MapBinder<String, TripEstimator<?>> tripBinder =
          MapBinder.newMapBinder(binder(), new TypeLiteral<>() {}, new TypeLiteral<>() {});

      tripBinder.addBinding(TransportMode.car).toInstance(new CarTripEstimator());

      ScoringParameters.Builder scoring =
          new ScoringParameters.Builder(
              config.scoring(),
              config.scoring().getScoringParameters("person"),
              Map.of(),
              config.scenario());

      bind(ScoringParametersForPerson.class).toInstance(person -> scoring.build());
      bind(InformedModeChoiceConfigGroup.class)
          .toInstance(ConfigUtils.addOrGetModule(config, InformedModeChoiceConfigGroup.class));
