...
public class SingleTripChoicesGeneratorTest extends ScenarioTest {
  ...
  public void choices() {
    ...
    Collection<PlanCandidate> candidates =
        generator.generate(model, null, new boolean[] {true, false, false, false});

    assertThat(candidates).first().matches(c -> c.getMode(0).equals(TransportMode.car));

    assertThat(candidates).last().matches(c -> c.getMode(0).equals(TransportMode.ride));
  }
  ...
  public void unavailable() {
    ...
    Collection<PlanCandidate> candidates =
        generator.generate(
            model, null, new boolean[] {false, false, true, false, false, false, false});

    System.out.println(candidates);

    assertThat(candidates).noneMatch(c -> c.getMode(2).equals(TransportMode.pt));
  }
  ...
  public void subset() {
    ...
    Collection<PlanCandidate> candidates =
        generator.generate(
            model,
            Set.of(TransportMode.car, TransportMode.walk),
            new boolean[] {true, false, false, false});

    assertThat(candidates).hasSize(2).first().matches(c -> c.getMode(0).equals(TransportMode.car));
  }
}
