...
public class DefaultActivityEstimatorTest extends ScenarioTest {
  ...
  public void person() {
    ...
    DefaultActivityEstimator est =
        new DefaultActivityEstimator(injector.getInstance(TimeInterpretation.class));
    ...
    assertThat(est.estimate(context, 6 * 3600, act.get(1))).isEqualTo(7.15, Offset.offset(0.1));

    assertThat(est.estimate(context, 3 * 3600, act.get(1))).isEqualTo(7.15, Offset.offset(0.1));

    assertThat(est.estimate(context, 8 * 3600, act.get(1))).isEqualTo(-4, Offset.offset(0.1));

    assertThat(est.estimate(context, 12 * 3600, act.get(1))).isEqualTo(-4, Offset.offset(0.1));
  }
}
