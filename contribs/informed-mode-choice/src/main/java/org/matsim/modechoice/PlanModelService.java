...
public final class PlanModelService implements StartupListener {
  ...
  @Inject private Map<String, LegEstimator<?>> legEstimators;

  @Inject private Map<String, TripEstimator<?>> tripEstimator;

  @Inject private Set<TripConstraint<?>> constraints;

  @Inject private ActivityEstimator actEstimator;

  @Inject private TimeInterpretation timeInterpretation;

  @Inject private EventsManager eventsManager;

  @Inject private ControlerListenerManager controlerListenerManager;

  private final InformedModeChoiceConfigGroup config;
  private final Map<String, ModeOptions<?>> options;

  @Inject
  private PlanModelService(
      InformedModeChoiceConfigGroup config, Map<String, ModeOptions<?>> options) {
    this.config = config;
    ...
    for (String mode : config.getModes()) {
      ...
      if (!options.containsKey(mode))
        throw new IllegalArgumentException(
            String.format("No estimators configured for mode %s", mode));
    }
  }
  ...
  /**
   * Initialized {@link ModeEstimate} for all available options in the {@link PlanModel}. No routing
   * or estimation is performed yet.
   */
  ...
  public List<String> modesForEstimator(LegEstimator<?> est) {
    return legEstimators.entrySet().stream()
        .filter(e -> e.getValue().equals(est))
  ...
  /**
   * Calculate the estimates for all options. Note that plan model has to be routed before computing
   * estimates.
   */
    ...
    for (Map.Entry<String, List<ModeEstimate>> e : planModel.getEstimates().entrySet()) {
      ...
      for (ModeEstimate c : e.getValue()) {
        ...
        for (int i = 0; i < planModel.trips(); i++) {
          ...
          double estimate = 0;
          if (tripEst != null) {
            MinMaxEstimate minMax =
                tripEst.estimate(context, c.getMode(), planModel, legs, c.getOption());
            double tripEstimate = c.isMin() ? minMax.getMin() : minMax.getMax();

            // Only store if required
            if (tValues != null) tValues[i] = tripEstimate;
          ...
          for (Leg leg : legs) {
            ...
            if (tripEst != null && legMode.equals(c.getMode())) continue;
          ...
          // early or late arrival can also have an effect on the activity scores which is
          // potentially considered here
          estimate +=
              actEstimator.estimate(
                  context, planModel.getStartTimes()[i] + tt, trip.getDestinationActivity());
  ...
  public boolean isValidOption(PlanModel model, String[] modes) {
    ...
    for (TripConstraint<?> c : this.constraints) {
      ConstraintHolder<?> h =
          new ConstraintHolder<>((TripConstraint<Object>) c, c.getContext(context, model));

      if (!h.test(modes)) return false;
    }
  ...
  public record ConstraintHolder<T>(TripConstraint<T> constraint, T context)
