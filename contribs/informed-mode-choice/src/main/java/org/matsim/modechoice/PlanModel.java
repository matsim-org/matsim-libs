...
public final class PlanModel implements Iterable<TripStructureUtils.Trip>, HasPerson {
  ...
  /**
   * Stored trip start times. Depends on the current modes. This will only be re-estimated during
   * routing.
   */
  ...
  private void setCurrentModes(List<TripStructureUtils.Trip> trips) {
    ...
    for (int i = 0; i < trips.size(); i++) {
      ...
      if (routingMode == null) routingMode = trips.get(i).getLegsOnly().get(0).getMode();
  ...
  public static String guessPlanType(Plan plan, Collection<String> modes) {
    ...
    for (int i = 0; i < trips.size(); i++) {
      ...
      if (!modes.contains(routingMode)) routingMode = null;

      b.append(routingMode);
      if (i != trips.size() - 1) b.append("-");
    }
  ...
  public double distance() {
    ...
    for (TripStructureUtils.Trip trip : trips) {
      if (trip.getOriginActivity().getCoord() == null
          || trip.getDestinationActivity().getCoord() == null) continue;
  ...
  public double distance(int idx) {
    ...
    if (trip.getOriginActivity().getCoord() == null
        || trip.getDestinationActivity().getCoord() == null) return 0;
    ...
    if (existing != null) {
      ...
      if (legs.length != existing.length)
        throw new IllegalArgumentException(
            String.format(
                "Existing legs have different length than the newly provided: %d vs. %d",
                existing.length, legs.length));

      // Copy existing non-null legs
      for (int i = 0; i < legs.length; i++) {
        List<Leg> l = legs[i];
        if (l != null) existing[i] = l;
      }
    }
  }
  ...
  public Set<String> filterModes(Predicate<? super ModeEstimate> predicate) {
    ...
    for (Map.Entry<String, List<ModeEstimate>> e : estimates.entrySet()) {
      if (e.getValue().stream().anyMatch(predicate)) modes.add(e.getKey());
    }
  ...
  /**
   * Return the legs of using a mode for ith trip. This can explicitly be null if a mode can not be
   * used for this trip.
   */
  @Nullable
