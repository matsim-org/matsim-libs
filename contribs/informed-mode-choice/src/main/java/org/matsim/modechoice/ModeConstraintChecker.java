...
public class ModeConstraintChecker implements IterationEndsListener, PersonAlgorithm {
  ...
  @Inject private Provider<GeneratorContext> provider;

  @Inject private PlanModelService service;
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    ...
    if (event.getIteration() == 0
        && service.hasConstraints()
        && config.getConstraintCheck() != InformedModeChoiceConfigGroup.ConstraintCheck.none) {

      generator = ThreadLocal.withInitial(() -> provider.get());
      violations.clear();

      ParallelPersonAlgorithmUtils.run(
          population, event.getServices().getConfig().global().getNumberOfThreads(), this);
    ...
    if (violations.size() > 0) {
      ...
      log.error(
          "Violating persons are: {}",
          violations.stream().map(Person::getId).collect(Collectors.toList()));

      if (config.getConstraintCheck() == InformedModeChoiceConfigGroup.ConstraintCheck.abort)
        throw new IllegalStateException(
            String.format(
                "Aborting due to %d constraint violations in input plans. Set constraintCheck to 'warn' or 'repair' to avoid this error.",
                violations.size()));
    }
  }
  ...
  public void run(Person person) {
    ...
    for (Plan plan : plans) {
      ...
      if (valid) hasValidPlan = true;
      else {
        hasViolation = true;
        violations.add(person);

        // remove all invalid plans, but keep at least one
        if (config.getConstraintCheck() == InformedModeChoiceConfigGroup.ConstraintCheck.repair
            && person.getPlans().size() > 1) {
    ...
    if (!hasValidPlan
        && config.getConstraintCheck() == InformedModeChoiceConfigGroup.ConstraintCheck.repair) {

      GeneratorContext ctx = generator.get();

      Collection<PlanCandidate> candidates =
          ctx.generator.generate(
              PlanModel.newInstance(first), null, null, 1, Double.NaN, Double.NaN);
