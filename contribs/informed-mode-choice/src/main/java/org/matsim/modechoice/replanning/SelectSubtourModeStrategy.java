...
public class SelectSubtourModeStrategy extends AbstractMultithreadedModule {
  ...
  public SelectSubtourModeStrategy(
      Config config, Scenario scenario, Provider<GeneratorContext> generator) {
    super(config.global());
    ...
    this.nonChainBasedModes =
        this.config.getModes().stream()
  ...
  public PlanAlgorithm getPlanAlgoInstance() {
    ...
    return new Algorithm(
        context,
        SelectSingleTripModeStrategy.newAlgorithm(
   ...
   */
  public static boolean hasSingleTripChoice(
      PlanModel model, Collection<String> nonChainBasedModes) {

    for (int i = 0; i < model.trips(); i++) {
      if (nonChainBasedModes.contains(model.getTripMode(i))) return true;
    }
  ...
  private final class Algorithm implements PlanAlgorithm {
    ...
    public void run(Plan plan) {
      ...
      if (model.trips() == 0) return;

      // Force re-estimation
      if (rnd.nextDouble() < config.getProbaEstimate()) model.reset();

      // Do change single trip on non-chain based modes with certain probability
      if (rnd.nextDouble() < smc.getProbaForRandomSingleTripMode()
          && hasSingleTripChoice(model, nonChainBasedModes)) {
      ...
      List<TripStructureUtils.Subtour> subtours =
          new ArrayList<>(TripStructureUtils.getSubtours(plan, smc.getCoordDistance()));
      ...
      while (!subtours.isEmpty()) {
        ...
        for (int i = 0; i < model.trips(); i++) {
          if (st.getTrips().contains(model.getTrip(i))
              && switchModes.contains(model.getTripMode(i))) mask[i] = true;
        }
        ...
        for (String m : config.getModes()) {
          ...
          for (int i = 0; i < mask.length; i++) {
            if (mask[i]) option[i] = m;
          }

          // Current option is not added twice
          if (!Arrays.equals(model.getCurrentModesMutable(), option)) options.add(option);
        }
        ...
        // this works in conjunction with subtour constraints, with other constraints it might be
        // too strict
        ...
        // best k generator applied pruning already, but the single trip options need to be checked
        // again
        if (ctx.pruner != null) {
          double threshold = ctx.pruner.planThreshold(model);
          if (!Double.isNaN(threshold) && threshold > 0) {
            candidates.removeIf(
                c -> c.getUtility() < singleModeCandidates.get(0).getUtility() - threshold);
          }
        }
