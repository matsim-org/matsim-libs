...
@CommandLine.Command(name = "analyze-choices", description = "Write plan and choice output to tsv.")
public class AnalyzeChoices implements MATSimAppCommand {
  ...
  @CommandLine.Option(
      names = "--modes",
      description = "Modes to include for distance estimate",
  ...
  public Integer call() throws Exception {
    ...
    try (CSVPrinter printer =
        new CSVPrinter(Files.newBufferedWriter(output), CSVFormat.MONGODB_TSV)) {

      List<String> header =
          ObjectArrayList.of("person", "k", "selected", "score", "type", "estimate");
      ...
      for (Person person : population.getPersons().values()) {
        ...
        if (!subpopulation.isEmpty()
            && !subpopulation.equals(PopulationUtils.getSubpopulation(person))) continue;
            ...
            person.getPlans().stream()
                ...
                .sorted(
                    Comparator.comparingDouble(
                        p -> -(double) p.getAttributes().getAttribute(PlanCandidate.ESTIMATE_ATTR)))
        ...
        for (Plan plan : plans) {
          ...
          if (person.getSelectedPlan() == plan) {
            ...
            } else types.put(plan.getType(), plan);
          } else {
            if (types.containsKey(plan.getType())) toRemove.add(plan);
            else types.put(plan.getType(), plan);
          }
        }
        ...
        for (int k = 0; k < plans.size(); k++) {
          ...
          for (String m : modes) {
            ...
            for (int i = 0; i < planModes.length; i++) {
              if (m.equals(planModes[i])) dist += model.distance(i);
            }
            dists.put(m, dist);
          }

          ObjectArrayList<Object> record =
              ObjectArrayList.of(
                  person.getId(),
                  k,
                  person.getSelectedPlan() == plan ? 1 : 0,
                  plan.getScore(),
                  plan.getType(),
                  plan.getAttributes().getAttribute(PlanCandidate.ESTIMATE_ATTR));
