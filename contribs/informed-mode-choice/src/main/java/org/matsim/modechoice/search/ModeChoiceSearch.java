...
/**
 * This class finds the best solutions by doing an exhaustive search over the best possible
...
final class ModeChoiceSearch {
  ...
  /**
   * Iterate over all solutions, starting with the best possible combination. {@link
   * #addEstimates(String, double[])} needs to be called for every search.
   *
   * @param result mode assignment will be written into this array, no memory is allocated during
   *     search.
  ...
  public DoubleIterator iter(String[] result) {
    ...
    for (int i = 0; i < result.length; i++) {
      ...
      if (max != Double.NEGATIVE_INFINITY) total += max;
    }
  ...
  public void addEstimates(String mode, double[] values, boolean[] mask) {
    ...
    for (int i = 0; i < values.length; i++) {
      if (mask == null || mask[i]) estimates[i][idx] = values[i];
    }
  }
  ...
  public String toString() {
    ...
    for (int i = 0; i < estimates.length; i++) {
      ...
      for (byte j = 0; j < estimate.length; j++) {
        ...
        if (j != estimate.length - 1) b.append(", ");
      }

      if (i != estimate.length - 1) b.append(" || ");
    }
  ...
  final class Iterator implements DoubleIterator {
    ...
    // Because the graph structure is fixed, it does not need to be as complicated as other k
    ...
    // This is the case in Yens algorithms and other and will consume a lot of memory, when many top
    // k paths are generated and a lot are thrown away.
