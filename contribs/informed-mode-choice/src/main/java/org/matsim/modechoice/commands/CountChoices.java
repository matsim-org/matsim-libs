...
public class CountChoices implements MATSimAppCommand, PersonAlgorithm {
  ...
  @CommandLine.Option(
      names = "--population",
      description = "Path to input population",
      required = true)
  ...
  @CommandLine.Option(
      names = "--modes",
      description = "Modes to include in estimation",
      defaultValue = "car,walk,bike,pt,ride",
      split = ",")
  ...
  @CommandLine.Option(
      names = "--max-choices",
      description = "Maximum number of choices to analyze",
  ...
  public Integer call() throws Exception {
    ...
    stConfig.setChainBasedModes(
        chainBasedModes.stream().map(String::intern).toArray(String[]::new));
  ...
  public void run(Person person) {
    ...
    if (subpopulation != null && !subpop.equals(subpopulation)) return;

    PlanModel trips = PlanModel.newInstance(person.getSelectedPlan());

    List<String> usableModes = new ArrayList<>(modes);
    if (!PersonUtils.canUseCar(person)) usableModes.remove(TransportMode.car);
    ...
    do {
      ...
      if (st.isValid(stContext, modes)) st_choices++;

      if (mc.isValid(mcContext, modes)) mc_choices++;
  ...
  private static boolean increment(int[] indices, int idx, int length) {
    ...
    if (indices.length == idx) return false;
