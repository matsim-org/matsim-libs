...
public class GenerateChoiceSet implements MATSimAppCommand, PersonAlgorithm {
  ...
  @CommandLine.Mixin private ScenarioOptions scenario;
  ...
  @CommandLine.Option(
      names = "--output-dist",
      description = "Write estimation distribution to output. Filename is derived from output",
      defaultValue = "false")
  ...
  public Integer call() throws Exception {
    ...
    generatorCache =
        ThreadLocal.withInitial(() -> injector.getInstance(TopKChoicesGenerator.class));
    ...
    for (Person person : controler.getScenario().getPopulation().getPersons().values()) {
      ...
      if (subpopulation != null && !subpop.equals(subpopulation)) continue;
    ...
    ParallelPersonAlgorithmUtils.run(
        controler.getScenario().getPopulation(), config.global().getNumberOfThreads(), this);

    PopulationUtils.writePopulation(controler.getScenario().getPopulation(), output.toString());

    if (distWriter != null) distWriter.close();

    log.info(
        "Generated {} plans for {} persons, (avg. {})",
        counter.get(),
        persons,
        counter.get() / (double) persons);
  ...
  public void run(Person person) {
    ...
    Plan plan =
        person.getPlans().stream()
            .filter(p -> "source".equals(p.getType()))
            .findFirst()
            .orElseThrow();
    ...
    for (PlanCandidate c : candidates) {
      ...
      if (plan == null) plan = person.createCopyOfSelectedPlanAndMakeSelected();
