...
/**
 * Calculate the allowed threshold based on a constant plus distance factor depending on the current
 * mode.
 */
public class ModeDistanceBasedPruner implements CandidatePruner {
  ...
  public double tripThreshold(PlanModel planModel, int idx) {
    return c
        + dist.getDouble(planModel.getCurrentModesMutable()[idx]) * planModel.distance(idx) / 1000;
  }
}
