  ...
  private final class Algorithm implements PlanAlgorithm {
    ...
    public void run(Plan plan) {
      ...
      if (rnd.nextDouble() < smc.getProbaForRandomSingleTripMode()
          && SelectSubtourModeStrategy.hasSingleTripChoice(model, nonChainBasedModes)) {
        ...
        if (!options.isEmpty()) {
          ...
          if (config.isRequireDifferentModes()) candidates.remove(current[idx]);
      ...
      while (!subtours.isEmpty()) {
        ...
        for (String m : modes) {
          ...
          if (!config.isRequireDifferentModes()
              || !Arrays.equals(model.getCurrentModesMutable(), option))
