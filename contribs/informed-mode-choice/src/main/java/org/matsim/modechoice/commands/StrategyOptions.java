...
public final class StrategyOptions {
   ...
   *
   * @param log usually method reference to {@link
   *     org.matsim.application.MATSimApplication#addRunOption(Config, String, Object)}
   */
  public void applyConfig(Config config, TriConsumer<Config, String, Object> log) {
    ...
    if (group.actEst) log.accept(config, "act-est", "");

    if (group.timeMutation != 0.05) log.accept(config, "tm", group.timeMutation);

    if (!group.innovateModes) log.accept(config, "no-inv-modes", "");

    if (group.anneal != InformedModeChoiceConfigGroup.Schedule.quadratic)
      log.accept(config, "anneal", group.anneal);

    if (group.invBeta != 2.5) log.accept(config, "invBeta", group.invBeta);

    if (group.forceInnovation != 10) log.accept(config, "f-inv", group.forceInnovation);

    if (group.weight != 0.1) log.accept(config, "weight", group.weight);
    ...
    if (getModeChoice() == ModeChoice.none)
      config
          ...
          .setMaxAgentPlanMemorySize(
              Math.max(config.replanning().getMaxAgentPlanMemorySize(), group.k) + 5);
  }
   ...
   */
  public Module applyModule(
      Binder binder, Config config, Consumer<InformedModeChoiceModule.Builder> setup) {
    ...
    List<ReplanningConfigGroup.StrategySettings> strategies =
        config.replanning().getStrategySettings().stream()
            .filter(
                s ->
                    !filtered.contains(s.getStrategyName())
                        || !Objects.equals(s.getSubpopulation(), defaultSubpopulation))
    ...
    if (group.forceInnovation > 0)
      binder
          ...
          .toInstance(
              new ForceInnovationStrategyChooser<>(
                  group.forceInnovation, ForceInnovationStrategyChooser.Permute.yes));

    InformedModeChoiceModule.Builder builder = InformedModeChoiceModule.newBuilder();

    setup.accept(builder);

    if (group.massConservation) builder.withConstraint(RelaxedMassConservationConstraint.class);

    if (!group.actEst) builder.withActivityEstimator(ActivityEstimator.None.class);
  ...
  public static final class Group {
    ...
    @CommandLine.Option(
        names = {"--mode-choice", "--mc"},
        description = "Mode choice strategy: ${COMPLETION-CANDIDATES}")
    private ModeChoice modeChoice;

    @CommandLine.Option(
        names = "--weight",
        defaultValue = "0.10",
        description = "Mode-choice strategy weight")
    private double weight;

    @CommandLine.Option(
        names = "--top-k",
        defaultValue = "5",
        description = "Top k options for some of the strategies")
    private int k;

    @CommandLine.Option(
        names = "--avoid-k",
        defaultValue = "10",
        description = "Avoid using recent mode types again")
    private int avoidK;

    @CommandLine.Option(
        names = "--time-mutation",
        defaultValue = "0.05",
        description = "Set time mutation strategy weight")
    private double timeMutation;

    @CommandLine.Option(
        names = "--mass-conservation",
        defaultValue = "false",
        description = "Enable mass conservation constraint",
        negatable = true)
    ...
    @CommandLine.Option(
        names = "--act-est",
        defaultValue = "false",
        description = "Enable activity estimation",
        negatable = true)
    private boolean actEst;

    @CommandLine.Option(
        names = "--force-innovation",
        defaultValue = "10",
        description = "Force innovative strategy with %% of agents")
    ...
    @CommandLine.Option(
        names = "--no-innovate-modes",
        defaultValue = "true",
        description = "Require that new plan modes are always different from the current one.",
        negatable = true)
    ...
    @CommandLine.Option(
        ...
        description = "Parameter annealing: ${COMPLETION-CANDIDATES}")
