...
/**
 * Interface to determine which mode options are available to an agent and considered when computing
 * best options.
 *
 * <p>This interface also contains the default implementations.
 *
...
public interface ModeOptions<T extends Enum<?>> {
  ...
  /**
   * Return whether an option allows to use the mode. Normally only one of the option should forbid
   * using the mode at all.
   */
  ...
  final class ConsiderYesAndNo implements ModeOptions<ModeAvailability> {
    ...
    private static final List<ModeAvailability> BOTH =
        List.of(ModeAvailability.YES, ModeAvailability.NO);
  ...
  final class ConsiderIfCarAvailable implements ModeOptions<ModeAvailability> {
    ...
    public List<ModeAvailability> get(Person person) {
      ...
      if (PersonUtils.canUseCar(person)) return ConsiderYesAndNo.BOTH;
