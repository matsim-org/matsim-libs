...
public class SingleTripChoicesGenerator extends AbstractCandidateGenerator {
  ...
  public Collection<PlanCandidate> generate(
    ...
    if (mask == null) throw new IllegalArgumentException("Mask must be provided");

    int idx = ArrayUtils.indexOf(mask, true);

    if (idx == -1 || ArrayUtils.lastIndexOf(mask, true) != idx) {
      throw new IllegalArgumentException(
          "Mask must contain exactly one true value:" + Arrays.toString(mask));
    }

    if (mask.length != planModel.trips())
      throw new IllegalArgumentException(
          "Mask needs length " + planModel.trips() + ": " + Arrays.toString(mask));
    ...
    Predicate<ModeEstimate> considerMode =
        m -> consideredModes == null || consideredModes.contains(m.getMode());

    router.routeSingleTrip(
        planModel, planModel.filterModes(considerMode.and(ModeEstimate::isUsable)), idx);
    ...
    for (List<ModeEstimate> value : planModel.getEstimates().values()) {
      ...
      if (estimate == Double.NEGATIVE_INFINITY) continue;
