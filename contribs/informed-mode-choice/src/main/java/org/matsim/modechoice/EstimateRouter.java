...
public final class EstimateRouter {
  ...
  @Inject
  public EstimateRouter(
      TripRouter tripRouter, ActivityFacilities facilities, TimeInterpretation timeInterpretation) {
  ...
  public void routeModes(PlanModel model, Collection<String> modes) {
    ...
    for (String mode : modes) {
      ...
      for (TripStructureUtils.Trip oldTrip : model) {
        ...
        // this one special case is handled here, it is unclear if similar behaviour might be
        // present in other modes
        if (mode.equals(TransportMode.pt)
            && ll.stream().noneMatch(l -> l.getMode().equals(TransportMode.pt))) {
          legs[i++] = null;
          continue;
        }
  ...
  private void calcStartTimes(
      PlanModel model, TimeTracker timeTracker, double[] startTimes, int until) {
  ...
  private double advanceTimetracker(
      TimeTracker timeTracker, TripStructureUtils.Trip oldTrip, PlanModel plan) {
    List<Leg> oldLegs = oldTrip.getLegsOnly();
    boolean undefined =
        oldLegs.stream().anyMatch(l -> timeInterpretation.decideOnLegTravelTime(l).isUndefined());
    ...
    if (undefined) {
      ...
      List<? extends PlanElement> legs =
          routeTrip(
              oldTrip,
              plan,
              routingMode != null ? routingMode : oldLegs.get(0).getMode(),
              timeTracker);
      timeTracker.addElements(legs);

    } else timeTracker.addElements(oldLegs);
  ...
  private List<? extends PlanElement> routeTrip(
      TripStructureUtils.Trip trip, PlanModel plan, String routingMode, TimeTracker timeTracker) {
