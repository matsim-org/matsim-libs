...
/**
 * Mass conservation constraint. Only one chain-based mode can be used per subtour. Only one type of
 * chain-based vehicle can be used, if it was not brought back to its original location. Note that
 * this might not be fully mass conserving in a traditional sense where all agents have closed plans
 * and start/end at home. Overall strictly more restrictive than {@link RelaxedSubtourConstraint}.
 */
public final class RelaxedMassConservationConstraint
    implements TripConstraint<RelaxedMassConservationConstraint.Context> {
  ...
  public RelaxedMassConservationConstraint(SubtourModeChoiceConfigGroup config) {
    ...
    chainBasedModes.addAll(
        Arrays.stream(config.getChainBasedModes()).map(String::intern).collect(Collectors.toSet()));
  ...
  public Context getContext(EstimatorContext context, PlanModel model) {
    ...
    for (int i = 0; i < model.trips(); i++) {
      ...
      for (TripStructureUtils.Subtour st : subtours) {
        if (st.getTrips().contains(model.getTrip(i))) res.subtours[i] |= (1 << j);

        j++;
      }

      res.origins[i] =
          facilities.computeIfAbsent(
              getLocation(model.getTrip(i).getOriginActivity()), k -> facilities.size());
      res.destinations[i] =
          facilities.computeIfAbsent(
              getLocation(model.getTrip(i).getDestinationActivity()), k -> facilities.size());
    }
  ...
  private static Object getLocation(Activity act) {
    if (act.getFacilityId() != null) return act.getFacilityId();
    else if (act.getLinkId() != null) return act.getLinkId();
    ...
    for (int i = 0; i < modes.length; i++) {
      ...
      if (chainBasedModes.contains(mode)) {
        ...
        if (context.destinations[i] == tookLocation) {
          tookVehicle = null;
          tookLocation = -1;
        } else currentLocation = context.destinations[i];
      }
