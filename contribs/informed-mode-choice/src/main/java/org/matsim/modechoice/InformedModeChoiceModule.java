...
public final class InformedModeChoiceModule extends AbstractModule {
  ...
  public void install() {
    ...
    Multibinder<TripConstraint<?>> tcBinder =
    ...
    MapBinder<String, CandidatePruner> pBinder =
        MapBinder.newMapBinder(binder(), String.class, CandidatePruner.class);
    for (Map.Entry<String, CandidatePruner> e : builder.pruner.entrySet()) {
      pBinder.addBinding(e.getKey()).toInstance(e.getValue());
    }

    addPlanStrategyBinding(SELECT_BEST_K_PLAN_MODES_STRATEGY)
        .toProvider(SelectBestKPlanModesStrategyProvider.class);
    addPlanStrategyBinding(SELECT_SINGLE_TRIP_MODE_STRATEGY)
        .toProvider(SelectSingleTripModeStrategyProvider.class);
    addPlanStrategyBinding(SELECT_SUBTOUR_MODE_STRATEGY)
        .toProvider(SelectSubtourModeStrategyProvider.class);
    addPlanStrategyBinding(RANDOM_SUBTOUR_MODE_STRATEGY)
        .toProvider(RandomSubtourModeStrategyProvider.class);
  ...
  @Provides
  public CandidatePruner pruner(
      Map<String, CandidatePruner> pruners, InformedModeChoiceConfigGroup config) {

    if (config.getPruning() == null) return null;
    ...
    if (pruner == null)
      throw new IllegalStateException(
          String.format(
              "Requested pruner %s in config, but it is not bound in the module",
              config.getPruning()));
  ...
  @Provides
  public PlanRouter planRouter(
      Provider<TripRouter> tripRouter, ActivityFacilities facilities, TimeInterpretation time) {
  ...
  private <T> void bindAllModes(Map<String, Class<? extends T>> map, TypeLiteral<T> value) {
    ...
    MapBinder<String, T> mapBinder =
        MapBinder.newMapBinder(binder(), new TypeLiteral<>() {}, value);
  ...
  public static final class Builder {
    ...
    public <T extends Enum<?>> Builder withFixedCosts(
        Class<? extends FixedCostsEstimator<T>> estimator, String... modes) {
    ...
    public <T extends Enum<T>> Builder withLegEstimator(
        Class<? extends LegEstimator<T>> estimator,
        Class<? extends ModeOptions<T>> option,
        String... modes) {
      ...
      for (String mode : modes) {
        ...
        if (tripEstimators.containsKey(mode))
          throw new IllegalArgumentException(
              String.format(
                  "Mode %s already has a plan estimator. Only one of either can be used", mode));
    ...
    public <T extends Enum<?>> Builder withTripEstimator(
        Class<? extends TripEstimator<T>> estimator,
        Class<? extends ModeOptions<T>> option,
        String... modes) {
    ...
    /**
     * Add candidate pruner with specific name. Has to be set in the config in order to be
     * activated.
     */
