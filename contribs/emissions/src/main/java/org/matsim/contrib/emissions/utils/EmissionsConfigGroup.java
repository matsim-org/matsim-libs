...
public final class EmissionsConfigGroup extends ReflectiveConfigGroup {
  ...
  private static final String EMISSION_FACTORS_WARM_FILE_AVERAGE =
      "averageFleetWarmEmissionFactorsFile";
  private String averageFleetWarmEmissionFactorsFile = null;

  private static final String EMISSION_FACTORS_COLD_FILE_AVERAGE =
      "averageFleetColdEmissionFactorsFile";
  private String averageFleetColdEmissionFactorsFile = null;

  private static final String EMISSION_FACTORS_WARM_FILE_DETAILED =
      "detailedWarmEmissionFactorsFile";
  private String detailedWarmEmissionFactorsFile = null;

  private static final String EMISSION_FACTORS_COLD_FILE_DETAILED =
  ...
  private EmissionsComputationMethod emissionsComputationMethod =
      EmissionsComputationMethod.AverageSpeed;
  ...
  private static final String DETAILED_VS_AVERAGE_LOOKUP_BEHAVIOR =
  ...
  // This is the first quick fix for the issue
  // https://github.com/matsim-org/matsim-libs/issues/1226.
  // Maybe other (smarter) strategies will be added later. kturner nov'20
  public enum HbefaTableConsistencyCheckingLevel {
    allCombinations,
    consistent,
    none
  }

  private static final String HBEFA_TABLE_CONSISTENCY_CHECKING_LEVEL =
      "hbefaTableConsistencyCheckingLevel";
  private HbefaTableConsistencyCheckingLevel hbefaTableConsistencyCheckingLevel =
      HbefaTableConsistencyCheckingLevel.allCombinations;

  private static final String EMISSION_FACTORS_WARM_FILE_AVERAGE_CMT =
      "file with HBEFA vehicle type specific fleet average warm emission factors";
  private static final String EMISSION_FACTORS_COLD_FILE_AVERAGE_CMT =
      "file with HBEFA vehicle type specific fleet average cold emission factors";
  private static final String DETAILED_VS_AVERAGE_LOOKUP_BEHAVIOR_CMT =
      "Should the calculation bases on average or detailed emission factors? "
          + "\n\t\t\t"
          + DetailedVsAverageLookupBehavior.onlyTryDetailedElseAbort.name()
          + " : try detailed values. Abort if values are not found. Requires DETAILED"
          + " emission factors. \n\t\t\t"
          + DetailedVsAverageLookupBehavior.tryDetailedThenTechnologyAverageElseAbort.name()
          + " : try detailed values first, if not found try to use "
          ...
          + DetailedVsAverageLookupBehavior.tryDetailedThenTechnologyAverageThenAverageTable.name()
          + "try detailed values first, if not found try to "
          ...
          + DetailedVsAverageLookupBehavior.directlyTryAverageTable.name()
          + "only calculate from average table. Requires AVERAGE emission factors. "
          + "Default is "
          + DetailedVsAverageLookupBehavior.onlyTryDetailedElseAbort.name();
  private static final String EMISSION_FACTORS_WARM_FILE_DETAILED_CMT =
      "file with HBEFA detailed warm emission factors";
  private static final String EMISSION_FACTORS_COLD_FILE_DETAILED_CMT =
      "file with HBEFA detailed cold emission factors";
  private static final String HBEFA_TABLE_CONSISTENCY_CHECKING_LEVEL_CMT =
      "Define on which level the entries in the provided hbefa tables are checked for consistency"
          + "\n\t\t\t"
          + HbefaTableConsistencyCheckingLevel.allCombinations.name()
          + " : check if entries for all combinations of HbefaTrafficSituation, HbefaVehicleCategory, HbefaVehicleAttributes, HbefaComponent. "
          ...
          + HbefaTableConsistencyCheckingLevel.consistent.name()
          + " : check if the entries for the two HbefaTrafficSituations 'StopAndGo' and 'FreeFlow' (nov 2020, may be subject to change) are consistently available in the table. \n\t\t\t"
          + // TODO
          HbefaTableConsistencyCheckingLevel.none.name()
          + " : There is no consistency check. This option is NOT recommended and only for backward capability to inputs from before spring 2020 . \n\t\t\t"
          + "Default is "
          + HbefaTableConsistencyCheckingLevel.allCombinations.name();

  private static final String WRITING_EMISSIONS_EVENTS_CMT =
      "if false, emission events will not appear in the events file.";

  private static final String HANDLE_HIGH_AVERAGE_SPEEDS_CMT =
      "if true, don't fail when average speed is higher than the link freespeed, but cap it instead.";

  private static final String NON_SCENARIO_VEHICLES_CMT =
      "Specifies the handling of non-scenario vehicles.  The options are: "
          //			+ Arrays.stream(NonScenarioVehicles.values()).map(handling -> " " +
          // handling.toString()).collect(Collectors.joining()) +"."
          //
  ...
  private static final String EMISSIONS_COMPUTATION_METHOD_CMT =
  ...
  private static final String messageTrue =
      "Please use <param name=\"detailedVsAverageLookupBehavior\" value=\"tryDetailedThenTechnologyAverageThenAverageTable\" />";
  private static final String messageFalse =
      "Please use <param name=\"detailedVsAverageLookupBehavior\" value=\"directlyTryAverageTable\" />";
  ...
  public void setDetailedVsAverageLookupBehavior(
      DetailedVsAverageLookupBehavior detailedVsAverageLookupBehavior) {
  ...
  public void setHbefaTableConsistencyCheckingLevel(
      HbefaTableConsistencyCheckingLevel hbefaTableConsistencyCheckingLevel) {
  ...
  private static final String HBEFA_VEHICLE_DESCRIPTION_SOURCE_CMT =
  ...
  // I am now thinking that it would be more expressive to keep this setting, because it makes users
  // aware of the fact that there needs to be something
  // in the vehicles file.  kai, dec'19
  @StringSetter(HBEFA_VEHICLE_DESCRIPTION_SOURCE)
  public void setHbefaVehicleDescriptionSource(
      HbefaVehicleDescriptionSource hbefaVehicleDescriptionSource) {
  ...
  protected final void checkConsistency(Config config) {
    switch (this.emissionsComputationMethod) {
      case StopAndGoFraction -> log.info(
          "Please note that with setting of emissionsComputationMethod "
              ...
              + "therefore following the fallback behaviour set in "
              + DETAILED_VS_AVERAGE_LOOKUP_BEHAVIOR
      ...
      case AverageSpeed -> log.warn(
          "This setting of emissionsComputationMethod. "
              + EmissionsComputationMethod.AverageSpeed
              + " is not covered by many test cases.");
      default -> throw new IllegalStateException(
          "Unexpected value: " + this.emissionsComputationMethod);
    }
  }
}
