...
/**
 * This class provides functions for blurring emissions. It does the same thing as {@link
 * EmissionGridAnalyzer} but is much faster The most convenient way to use it is the {@link
 * FastEmissionGridAnalyzer#processEventsFile(String, Network, double, int)} method. If an emissions
 * Events file is already parsed, one may also use the {@link
 * FastEmissionGridAnalyzer#processLinkEmissions(Map, Network, double, int)} method
 */
public abstract class FastEmissionGridAnalyzer {
   ...
   *
   * <p>1. All emissions are summed up by link id if the link id was found within the supplied
   * network. 2. The aggregated emissions for each link are rastered onto all the raster-cells
   * covered by the link. 3. In the smoothing step the emissions are blurred onto the surrounding
   * raster-cells.
   *
   * <p>The blurring algorithm is a gaussian blur https://en.wikipedia.org/wiki/Gaussian_blur
   *
   * <p>If only a certain area of the scenario is of interest for the analysis. The supplied network
   * must be filtered beforehand. The resulting raster's size depends on the bounding box of the
   * supplied network.
   *
   * <p>Note: The algorithm is not accurate at the edges of the raster. The kernel is cut of a the
   * edges meaning that emissions are underestimated at the edges of the raster. I didn't bother to
   * implement this correctly. Otherwise the overall amount of emissions doesn't change.
   *
   * @param eventsFile The events file which contains the emission events
   * @param network The network those emissions occurred on. The size of the resulting rater depends
   *     on the bounding box of the network
   * @param cellSize size of a cell. This determines how many 'pixels' the resulting raster will
   *     have. Smaller cellSize means higher pixel-density
   * @param radius smoothing radius which determines the strength of the blur. The radius describes
   *     onto how many cells the emissions of a single cell are distributed in one direction. A
   *     radius of 0 means no blurring. A radius of ~20 is probably a good guess for real-world
   *     scenarios. The resulting smoothing kernel will have radius * 2 + 1 entries.
   * @return A raster containing emission values for each (x,y)-cell within the bounds of the
   *     network
   */
  public static Map<Pollutant, Raster> processEventsFile(
      final String eventsFile, final Network network, final double cellSize, final int radius) {
    ...
    new RawEmissionEventsReader(
            (time, linkId, vehicleId, pollutant, value) -> {
              ...
              if (network.getLinks().containsKey(id)) {
                ...
                var linkMap =
                    linkEmissionsByPollutant.computeIfAbsent(
                        pollutant, key -> new TObjectDoubleHashMap<>());
    ...
    return linkEmissionsByPollutant.entrySet().stream()
        .map(
            entry -> {
              ...
              return Tuple.of(
                  entry.getKey(),
                  processLinkEmissions(entry.getValue(), network, cellSize, radius));
            })
        .collect(Collectors.toMap(Tuple::getFirst, Tuple::getSecond));
  }

  /** Processes emissions that have been read by the {@link EmissionsOnLinkEventHandler}. */
  public static Map<Pollutant, Raster> processHandlerEmissions(
    ...
    for (Map.Entry<Id<Link>, Map<Pollutant, Double>> perLink : link2pollutants.entrySet()) {
      for (Map.Entry<Pollutant, Double> e : perLink.getValue().entrySet()) {
        var linkMap =
            linkEmissionsByPollutant.computeIfAbsent(
                e.getKey(), key -> new TObjectDoubleHashMap<>());
  ...
  /** Processes emissions that have been read by the {@link EmissionsOnLinkEventHandler}. */
  public static TimeBinMap<Map<Pollutant, Raster>> processHandlerEmissionsPerTimeBin(
      TimeBinMap<Map<Id<Link>, EmissionsByPollutant>> timeBinMap,
      Network network,
      double cellSize,
      int radius) {

    Map<Double, Map<Pollutant, TObjectDoubleHashMap<Id<Link>>>> linkEmissionsByPollutantAndTime =
        new HashMap<>();

    for (TimeBinMap.TimeBin<Map<Id<Link>, EmissionsByPollutant>> perLink :
        timeBinMap.getTimeBins()) {
      Double time = perLink.getStartTime();
      Map<Pollutant, TObjectDoubleHashMap<Id<Link>>> emissions = new HashMap<>();

      for (Map.Entry<Id<Link>, EmissionsByPollutant> emissionsByPollutantEntry :
          perLink.getValue().entrySet()) {
        // Added linkID if not exists
        Id<Link> linkId = emissionsByPollutantEntry.getKey();
        for (Map.Entry<Pollutant, Double> e :
            emissionsByPollutantEntry.getValue().getEmissions().entrySet()) {
    ...
    // Transpose the map
    for (Map.Entry<Double, Map<Pollutant, TObjectDoubleHashMap<Id<Link>>>> timeSlice :
        linkEmissionsByPollutantAndTime.entrySet()) {
  ...
  /**
   * Works as {@link FastEmissionGridAnalyzer#processEventsFile(String, Network, double, int)} but
   * without events parsing The emissions per link have to be supplied.
   */
  public static Raster processLinkEmissions(
      final TObjectDoubleMap<Id<Link>> emissions,
   ...
   */
  public static Raster processLinkEmissions(
      final Map<Id<Link>, Double> emissions,
      final Network network,
      final double cellSize,
      final int radius) {
  ...
  static Raster blur(Raster raster, int radius) {
    ...
    // smooth horizontally
    firstPassRaster.setValueForEachIndex(
        (x, y) ->
            calculateBlurredValue(
                y,
                x,
                firstPassRaster.getXLength(),
                kernel,
                (yf, xv) -> raster.getValueByIndex(xv, yf)));

    // smooth vertically
    result.setValueForEachIndex(
        (x, y) ->
            calculateBlurredValue(
                x, y, result.getYLength(), kernel, firstPassRaster::getValueByIndex));

    return result;
  }

  private static double calculateBlurredValue(
      int fixedIndex, int volatileIndex, int volatileLength, double[] kernel, GetValue getValue) {

    var halfKernelLength = kernel.length / 2;
    var value = 0.;
    var startIndex = (volatileIndex - halfKernelLength < 0) ? halfKernelLength - volatileIndex : 0;
    var endIndex =
        (volatileIndex + halfKernelLength >= volatileLength)
            ? volatileLength - 1 - volatileIndex + halfKernelLength
            : kernel.length;
  ...
  static Raster rasterizeNetwork(
      final Network network, final TObjectDoubleMap<Id<Link>> emissions, final double cellSize) {
  ...
  static Raster rasterizeNetwork(
      Network network, Map<Id<Link>, Double> emissions, double cellSize) {
  ...
  /**
   * Rasterizes links into squares. Uses Bresenham's line drawing algorithm, which is supposed to be
   * fast Maybe the result is too chunky, but it'll do as a first try
   *
  ...
  private static int rasterizeLink(Link link, double value, Raster raster) {
    ...
    do {
      ...
      // have this condition in separate method because we want to get one more cell than the
      // original algorithm
