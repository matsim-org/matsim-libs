 ...
 *
 * <p>handled OSM road types: motorway,trunk,primary,secondary, tertiary,
 * unclassified,residential,service motorway_link, trunk_link,primary_link, secondary_link
 * tertiary_link, living_street, pedestrian,track,road
 *
 * <p>Hbefa categories and respective speeds URB/MW-Nat./80 - 130 URB/MW-City/60 - 110
 * URB/Trunk-Nat./70 - 110 URB/Trunk-City/50 - 90 URB/Distr/50 - 80 URB/Local/50 - 60 URB/Access/30
 * - 50
 *
 * <p>Conversions from OSM to hbefa types motorway;MW primary;Trunk secondary;Distr tertiary;Local
 * residential;Access living;Access
 */
public class OsmHbefaMapping extends HbefaRoadTypeMapping {
  ...
  public String determineHebfaType(Link link) {
    ...
    double allowedSpeed =
        allowedSpeedAttribute != null ? (double) allowedSpeedAttribute : link.getFreespeed();
  ...
  private String getHEBFAtype(String type, double speed) {
    ...
    // sometimes link type is smth like 'motorway_link' or 'living_street' or
    // 'primary|railway.tram'. We only care about the first part, here
    int idx =
        Arrays.asList(type.indexOf('.'), type.indexOf('|'), type.indexOf('_'), type.indexOf(','))
            .stream()
