 ...
 * @author benjamin
 */
final class ColdEmissionHandler
  ...
  /*package-private*/ ColdEmissionHandler(
      Scenario scenario,
      ...
      Set<Pollutant> coldPollutants,
      EventsManager eventsManager) {

    this.coldEmissionAnalysisModule =
        new ColdEmissionAnalysisModule(
            avgHbefaColdTable,
    ...
    if (previousDistance != null) {
      ...
      if (vehicle == null) {
        ...
        if ((distance / 1000) > 1.0) {
          ...
          coldEmissionAnalysisModule.throwColdEmissionEvent(
              vehicle.getId(), linkId, event.getTime(), coldEmissions);
        ...
        // I now think that this has to do with the fact that the cold emissions are smeared out
        // over the initial distance.  Don't know the details, though.  kai, dec'22
      }
    }
  }

  static void handleNullVehicleECG(
      Id<Vehicle> vehicleId, EmissionsConfigGroup emissionsConfigGroup) {
    switch (emissionsConfigGroup.getNonScenarioVehicles()) {
      case abort:
        throw new RuntimeException(
            "No vehicle defined for id "
                + vehicleId
                + ". "
                + "Please make sure that requirements for emission vehicles in "
                + EmissionsConfigGroup.GROUP_NAME
                + " config group are met."
                + " Or set the parameter + 'nonScenarioVehicles' to 'ignore' in order to skip such vehicles."
                + " Aborting...");
      case ignore:
        if (noVehWarnCnt < 10) {
          logger.warn("No vehicle defined for id " + vehicleId + ". The vehicle will be ignored.");
          noVehWarnCnt++;
          if (noVehWarnCnt == 10) logger.warn(Gbl.FUTURE_SUPPRESSED);
        }
        break;
        // yyyy I found the above without the "break".  There is
        // https://github.com/matsim-org/matsim-code-examples/issues/910, according to which it
        // seems that this is a bug.  There should be a testcase, but I guess there ain't.  kai,
        // apr'22
      default:
  ...
  public void handleEvent(VehicleEntersTrafficEvent event) {
    ...
    if (vehicle == null) {
      ...
      coldEmissionAnalysisModule.throwColdEmissionEvent(
          vehicleId, linkId, startEngineTime, coldEmissions);
    ...
    if (linkLength == 0.) {
      if (zeroLinkLengthWarnCnt == 0) {
        logger.warn(
            "Length of the link "
                + linkId
                + " is zero. No emissions will be estimated for this link. Make sure, this is intentional.");
        logger.warn(Gbl.ONLYONCE);
