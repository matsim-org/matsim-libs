...
final class HbefaVehicleAttributes {
    ...
    public void setHbefaSizeClass(String hbefaSizeClass) {
        /* JWJ May'21: we have to check if the passed argument is null. If it is,
          which is perfectly fine for HBEFA4.1 (amd later?), then it should
          retain the default "average" value and NOT override it to 'null'. */
        if (hbefaSizeClass != null) {
    ...
    public int hashCode() {
        ...
        if (hbefaSizeClass == null) {
            Logger.getLogger(HbefaVehicleAttributes.class).error("Found you!!");
        }
