 ...
 *
 * <ul>
 *   <li>HBEFA 3.1 does not provide further distance categories for cold start emission factors when
 *       average amient temperature is assumed <br>
 *   <li>HBEFA 3.1 does not provide cold start emission factors for Heavy Goods Vehicles; thus, HGV
 *       are assumed to produce the same cold start emission factors as passenger cars <br>
 *   <li>In the current implementation, vehicles emit one part of their cold start emissions when
 *       the engine is started (distance class 0 - 1 km); after reaching 1 km, the rest of their
 *       cold start emissions is emitted (difference between distance class 1 - 2 km and distance
 *       class 0 - 1 km)
 * </ul>
 *
 * @author benjamin
...
final class ColdEmissionAnalysisModule {
  ...
  /**
   * yy I do not know what the "W" in the method name means. Normally, it means "With". But that
   * does not make sense here. kai, dec'22
   */
  ...
  /*package-private*/ void throwColdEmissionEvent(
      Id<Vehicle> vehicleId,
      Id<Link> coldEmissionEventLinkId,
      double eventTime,
      Map<Pollutant, Double> coldEmissions) {
    Event coldEmissionEvent =
        new ColdEmissionEvent(eventTime, coldEmissionEventLinkId, vehicleId, coldEmissions);
    this.eventsManager.processEvent(coldEmissionEvent);
  }

  private static int cnt = 10;

  private Map<Pollutant, Double> calculateColdEmissions(
      Id<Vehicle> vehicleId,
      double parkingDuration,
      Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> vehicleInformationTuple,
      int distance_km) {
    ...
    // HBEFA 3 provides cold start emissions for "pass. car" and Light_Commercial_Vehicles (LCV)
    // only.
    // HBEFA 4.1 provides cold start emissions for "pass. car" and Light_Commercial_Vehicles (LCV)
    // only.
    // see https://www.hbefa.net/e/documents/HBEFA41_Development_Report.pdf (WP 4 , page 23)
    // kturner, may'20
    // Mapping everything except "motorcycle" to "pass.car", since this was done in the last years
    // for HGV.
    ...
    if (vehicleInformationTuple.getFirst().equals(HbefaVehicleCategory.HEAVY_GOODS_VEHICLE)) {
      changeVehCategory(
          key, HbefaVehicleCategory.HEAVY_GOODS_VEHICLE, HbefaVehicleCategory.PASSENGER_CAR);
    }
    ...
    for (Pollutant coldPollutant : coldPollutants) {
      ...
      if (distance_km == 1) {
        generatedEmissions =
            getEmissionsFactor(vehicleInformationTuple, 1, key, coldPollutant).getFactor();
      } else {
        generatedEmissions =
            getEmissionsFactor(vehicleInformationTuple, 2, key, coldPollutant).getFactor()
                - getEmissionsFactor(vehicleInformationTuple, 1, key, coldPollutant).getFactor();
      }
  ...
  /**
   * Replace the vehicleCategory with HbefaVehicleCategory.PASSENGER_CAR This is the old behaviour
   * as it was until Aug 21. (Aug'21, KMT) This does not help, since the emConcepts are not the
   * same. So it is _not_ usable if using some kind of detailed values.
   *
   * @param key
   * @param originVehCat
   * @param targetvehCat
   */
  // TODO Mabe make the behaviour settable by an enum? -> keep some kind of backwards capability or
  // just return a 0.0 as it is for motorcycle?
  private void changeVehCategory(
      HbefaColdEmissionFactorKey key,
      HbefaVehicleCategory originVehCat,
      HbefaVehicleCategory targetvehCat) {
    ...
    if (vehInfoWarnHDVCnt < maxWarnCnt) {
      vehInfoWarnHDVCnt++;
      logger.warn(
          "Automagic changing of VehCategory is disabled. Please make sure that your table contains the "
  ...
  private HbefaColdEmissionFactor getEmissionsFactor(
      Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> vehicleInformationTuple,
      int distance_km,
      HbefaColdEmissionFactorKey efkey,
      Pollutant coldPollutant) {
    ...
    switch (ecg.getDetailedVsAverageLookupBehavior()) {
      ...
      case tryDetailedThenTechnologyAverageThenAverageTable:
        ...
        try {
          ...
          e.printStackTrace();
          logger.error(
              "Cannot find an emissions factor. One possible cause might be "
    ...
    throw new RuntimeException(
        "Was not able to lookup emissions factor. Maybe you wanted to look up detailed values and did not specify this in "
            ...
            + "values ar missing in your emissions table(s) either average or detailed OR... ? efkey: "
            + efkey.toString());
  }

  static HbefaVehicleAttributes createHbefaVehicleAttributes(
      final String hbefaTechnology, final String hbefaSizeClass, final String hbefaEmConcept) {
