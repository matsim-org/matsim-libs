...
public abstract class HbefaTables {
    ...
    return load(
        ...
        record ->
            new HbefaWarmEmissionFactor(
                Double.parseDouble(record.get("EFA")), Double.parseDouble(record.get("V"))));
  }
  ...
  private static <K extends HbefaEmissionFactorKey, V extends HbefaEmissionFactor> Map<K, V> load(
      URL file, Function<CSVRecord, K> createKey, Function<CSVRecord, V> createValue) {
  ...
  private static <K extends HbefaEmissionFactorKey> void setCommonParametersOnKey(
      K key, CSVRecord record) {
  ...
  private static <K extends HbefaEmissionFactorKey> void setCommonDetailedParametersOnKey(
      K key, CSVRecord record) {
    ...
    if (string.endsWith("Freeflow")) return HbefaTrafficSituation.FREEFLOW;
      ...
      logger.warn(
          "Could not map String "
              + string
              + " to any HbefaTrafficSituation; please check syntax in hbefa input file.");
