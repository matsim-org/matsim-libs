...
/**
 * This class may be used to collect emission events of some events file and assign those emissions
 * to a grid structure. Additionally those emissions are divided into time bins
 */
public class EmissionGridAnalyzer {
  ...
  /**
   * Processes the events file of the given path. The events file must already contain emission
   * events. The emission events will be divided into time bins, as configured by the binSize
   * parameter in the constructor. Within each time bin all the emission events for each link are
   * collected and their emissions are summed up by pollutant. After processing the events the
   * impact of the emissions per link onto each grid cell is calculated by using a gaussian blur.
   *
  ...
  public TimeBinMap<Grid<Map<Pollutant, Double>>> process(String eventsFile) {
    ...
    TimeBinMap<Map<Id<Link>, EmissionsByPollutant>> timeBinsWithEmissions =
        processEventsFile(eventsFile);
  ...
  public void processTimeBinsWithEmissions(String eventsFile) {
    ...
    TimeBinMap<Map<Id<Link>, EmissionsByPollutant>> emissionsByPollutant =
        processEventsFile(eventsFile);
  ...
  /**
   * Generate the emissions grid data for the next time bin. Requires that the events file has
   * already been processed by calling processTimeBinsWithEmissions(). This method should be called
   * in a loop to exhaust the time bin iterator.
   *
   * @return tuple containing the start time of the next time bin and the emissions grid for that
   *     time bin. Returns null if there are no further time bins.
   */
  ...
  /**
   * Works like {@link org.matsim.contrib.emissions.analysis.EmissionGridAnalyzer#process(String)}
   * but writes the result into a Json-String
   *
   * @param eventsFile Path to the events file e.g. '/path/to/events.xml.gz
  ...
  /**
   * Works like {@link org.matsim.contrib.emissions.analysis.EmissionGridAnalyzer#process(String)}
   * but writes the result into a JSON-File
   *
   * @param eventsFile Path to the events file e.g. '/path/to/events.xml.gz
  ...
  private Grid<Map<Pollutant, Double>> writeAllLinksToGrid(
      Map<Id<Link>, EmissionsByPollutant> linksWithEmissions) {
    ...
    linksWithEmissions.entrySet().parallelStream()
        .forEach(
            entry -> {
              ...
              if (count % 10000 == 0)
                logger.info(
                    "processing: " + count * 100 / linksWithEmissions.keySet().size() + "% done");

              if (network.getLinks().containsKey(entry.getKey())
                  && isWithinBounds(network.getLinks().get(entry.getKey()))) {
                processLink(network.getLinks().get(entry.getKey()), entry.getValue(), grid);
              }
            });

    grid.getCells().parallelStream().forEach(cell -> removeTinyValuesFromResults(cell.getValue()));

    return grid;
  }

  private void processLink(
      Link link, EmissionsByPollutant emissions, Grid<Map<Pollutant, Double>> grid) {
    ...
    Geometry clip =
        factory
            ...
            .buffer(smoothingRadius * 5);

    for (var cell : grid.getCells(clip)) {
      double normalizationFactor =
          grid.getCellArea() / (Math.PI * smoothingRadius * smoothingRadius);
      double weight =
          SpatialInterpolation.calculateWeightFromLine(
              transformToCoordinate(link.getFromNode()),
              transformToCoordinate(link.getToNode()),
              cell.getCoordinate(),
              smoothingRadius);
  ...
  private void processCell(
      Grid.Cell<Map<Pollutant, Double>> cell, EmissionsByPollutant emissions, double weight) {
  ...
  private Grid<Map<Pollutant, Double>> createGrid() {
    ...
    else return new SquareGrid<>(gridSize, ConcurrentHashMap::new, bounds);
  }
  ...
  /**
   * Builder to configure a new {@link org.matsim.contrib.emissions.analysis.EmissionGridAnalyzer}
   * instance
   */
  public static class Builder {
     ...
     *
     * @param size duration of a time bin. Usually MATSim uses seconds as time unit but the
     *     implementation doesn't really care.
     ...
     *
     * @param size The horizontal distance between grid cells. Should conform to the units used by
     ...
     * @throws java.lang.IllegalArgumentException if binSize, gridSize, smoothingRadius are <= 0,
     *     and if network was not set
     */
    public EmissionGridAnalyzer build() {

      if (!isValidParameters())
        throw new IllegalArgumentException(
            "binSize, gridSize, smoothingRadius must be set and greater 0, Also network must be set");

      if (!isValidSmoothingRadiusToGridSizeRatio())
        throw new IllegalArgumentException(
            "A smoothing radius smaller than the grid size may lead to artifacts.In fact: Smoothing radius should be much bigger than grid size!");

      if (bounds == null) bounds = new PreparedGeometryFactory().create(createBounds());
      return new EmissionGridAnalyzer(
          binSize, gridSize, smoothingRadius, countScaleFactor, gridType, network, bounds);
    }
