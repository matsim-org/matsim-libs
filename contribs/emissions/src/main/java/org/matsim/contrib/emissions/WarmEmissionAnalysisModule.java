...
public final class WarmEmissionAnalysisModule implements LinkEmissionsCalculator {
  ...
  private final Map<HbefaRoadVehicleCategoryKey, Map<HbefaTrafficSituation, Double>>
    ...
    if (detailedHbefaWarmTable != null) {
      switch (ecg.getHbefaTableConsistencyCheckingLevel()) {
        case allCombinations:
          // The following tests if the detailed table is consistent, i.e. if there exist all
          // combinations of entries.  There used to be some test
          // cases where this was deliberately not the case, implying that this was assumed as
          // plausible also for studies.  This is now forbidding it.
          // If this causes too many problems, we could insert a switch (or attach it to the
          // fallback behavior switch).  kai, feb'20
          // Eventually vehicle category and vehicle attribute should be alligned in order to make
          // the allCombinations setting useful
          // see discussion in  https://github.com/matsim-org/matsim-libs/issues/1226 kturner,
          // nov'20
          Set<String> roadCategories = new HashSet<>();
          Set<HbefaTrafficSituation> trafficSituations =
              EnumSet.noneOf(HbefaTrafficSituation.class);
        ...
        case consistent:
          // yy The above consistency check might actually be too restrictive.  The code only needs
          // that both freeflow and stopgo traffic
          // conditions exist for a certain lookup.  So we could still have some road categories,
          // vehicle categories or vehicle attributes
          // where some detailed values exist and others don't.  So the thing to check would be if
          // for each existing
          ...
          for (String syntheticKey : freeflowSet) {
            if (!stopgoSet.contains(syntheticKey)) {
              throw new RuntimeException(
                  "inconsistent Freeflow and Stop&Go entries in detailed HBEFA table - Stop&Go entry missing ");
            }
          }
          for (String syntheticKey : stopgoSet) {
            if (!freeflowSet.contains(syntheticKey)) {
              throw new RuntimeException(
                  "inconsistent Freeflow and Stop&Go entries in detailed HBEFA table - Freeflow entry missing");
            }
          }
          break;
        case none:
        ...
        default:
          throw new IllegalStateException(
              "Unexpected value: " + ecg.getHbefaTableConsistencyCheckingLevel());
      }
    }
  }
  ...
  @Override
  public Map<Pollutant, Double> checkVehicleInfoAndCalculateWarmEmissions(
      Vehicle vehicle, Link link, double travelTime) {
    return checkVehicleInfoAndCalculateWarmEmissions(
        vehicle.getType(), vehicle.getId(), link, travelTime);
  }

  private static int cnt = 10;

  /*package-private*/ Map<Pollutant, Double> checkVehicleInfoAndCalculateWarmEmissions(
      VehicleType vehicleType, Id<Vehicle> vehicleId, Link link, double travelTime) {
    {
      String hbefaVehicleTypeDescription =
          EmissionUtils.getHbefaVehicleDescription(vehicleType, this.ecg);
    ...
    Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> vehicleInformationTuple =
        EmissionUtils.convertVehicleDescription2VehicleInformationTuple(vehicleType);
    Gbl.assertNotNull(vehicleInformationTuple);

    if (vehicleInformationTuple.getFirst() == null) {
      throw new RuntimeException(
          "Vehicle category for vehicle "
              + vehicleType
              + " is not valid. "
              + "Please make sure that requirements for emission vehicles in "
              + EmissionsConfigGroup.GROUP_NAME
              + " config group are met. Aborting...");
    }
  ...
  Map<Pollutant, Double> calculateWarmEmissions(
      double travelTime_sec,
      String roadType,
      double freeVelocity_ms,
      double linkLength_m,
      Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> vehicleInformationTuple) {
    ...
    if (vehicleInformationTuple.getFirst() == HbefaVehicleCategory.NON_HBEFA_VEHICLE) {
      ...
      if (cnt > 0) {
        logger.warn(
            "Just encountered non hbefa vehicle; currently, this code is setting the emissions of such vehicles to zero.  "
    ...
    // translate vehicle information type into factor key.  yyyy maybe combine these two? kai,
    // jan'20
    ...
    if (this.detailedHbefaWarmTable != null) {
      ...
      hbefaVehicleAttributes.setHbefaTechnology(
          vehicleInformationTuple.getSecond().getHbefaTechnology());
      hbefaVehicleAttributes.setHbefaSizeClass(
          vehicleInformationTuple.getSecond().getHbefaSizeClass());
      hbefaVehicleAttributes.setHbefaEmConcept(
          vehicleInformationTuple.getSecond().getHbefaEmConcept());
    ...
    if (averageSpeed_kmh <= 0.0) {
      throw new RuntimeException(
          "Average speed has been calculated to 0.0 or a negative value. Aborting...");
    }
    if ((averageSpeed_kmh - freeVelocity_ms * 3.6) > 1.0) {
      if (ecg.getHandleHighAverageSpeeds()) {
        logger.warn(
            "averageSpeed was capped from " + averageSpeed_kmh + " to" + freeVelocity_ms * 3.6);
        averageSpeed_kmh = freeVelocity_ms * 3.6;
      } else {
        throw new RuntimeException(
            "Average speed has been calculated to be greater than free flow speed; this might produce negative warm emissions. Aborting...");
      }
    }
    ...
    if (ecg.getEmissionsComputationMethod() == AverageSpeed) {
      final HbefaTrafficSituation trafficSituation =
          getTrafficSituation(efkey, averageSpeed_kmh, freeVelocity_ms * 3.6);
    ...
    for (Pollutant warmPollutant : warmPollutants) {
      ...
      if (ecg.getEmissionsComputationMethod() == StopAndGoFraction) {
        ...
        fractionStopGo =
            getFractionStopAndGo(
                freeVelocity_ms * 3.6, averageSpeed_kmh, vehicleInformationTuple, efkey);
    ...
    // update counters:
    // yy I don't now what this is good for; I would base downstream analysis rather on events.
    // kai, jan'20
    if (ecg.getEmissionsComputationMethod() == StopAndGoFraction) {
  ...
  private double getFractionStopAndGo(
    ...
    if ((averageSpeed_kmh - freeFlowSpeed_kmh)
        >= -1.0) { // both speeds are assumed to be not very different > only freeFlow on link
      fractionStopGo = 0.0;
    } else if ((averageSpeed_kmh - stopGoSpeedFromTable_kmh)
        <= 0.0) { // averageSpeed is less than stopGoSpeed > only stop&go on link
      fractionStopGo = 1.0;
    } else {
      fractionStopGo =
          stopGoSpeedFromTable_kmh
              * (freeFlowSpeed_kmh - averageSpeed_kmh)
              / (averageSpeed_kmh * (freeFlowSpeed_kmh - stopGoSpeedFromTable_kmh));
    }

    return fractionStopGo;
  }

  private HbefaWarmEmissionFactor getEf(
      Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> vehicleInformationTuple,
      HbefaWarmEmissionFactorKey efkey) {
    ...
    switch (ecg.getDetailedVsAverageLookupBehavior()) {
      case onlyTryDetailedElseAbort -> {
        ...
        if (this.detailedHbefaWarmTable.get(efkey) != null) {
          ...
          HbefaVehicleAttributes attribs2 =
              EmissionUtils.tryRewriteHbefa3toHbefa4(vehicleInformationTuple);
      ...
      case tryDetailedThenTechnologyAverageElseAbort -> {
        ...
        if (this.detailedHbefaWarmTable.get(efkey) != null) {
          ...
          if (ecg.getDetailedVsAverageLookupBehavior()
            ...
            // lookups of type "<technology>; average; average" should, I think, just be entered as
            // such. kai, feb'20
      ...
      case tryDetailedThenTechnologyAverageThenAverageTable -> {
        ...
        if (detailedFallbackAverageTableWarnCnt <= 1) {
          ...
          logger.warn(
              "Now trying with setting to vehicle attributes to \"average; average; average\" and try it with the average table");
          logger.warn(Gbl.ONLYONCE);
          logger.warn(Gbl.FUTURE_SUPPRESSED);
      ...
      default -> throw new IllegalStateException(
          "Unexpected value: " + ecg.getDetailedVsAverageLookupBehavior());
    }

    throw new RuntimeException(
        "Was not able to lookup emissions factor. Maybe you wanted to look up detailed values and did not specify this in "
            + "the config OR "
            + "you should use another fallback setting when using detailed calculation OR values ar missing in your emissions table(s) either average or detailed OR... ? efkey: "
            + efkey.toString());
  }

  // TODO: this is based on looking at the speeds in the HBEFA files, using an MFP, maybe from
  // A.Loder would be nicer, jm  oct'18
  private HbefaTrafficSituation getTrafficSituation(
      HbefaWarmEmissionFactorKey efkey, double averageSpeed_kmh, double freeFlowSpeed_kmh) {
    // TODO: should this be generated only once much earlier?
    HbefaRoadVehicleCategoryKey hbefaRoadVehicleCategoryKey =
        new HbefaRoadVehicleCategoryKey(efkey);
    Map<HbefaTrafficSituation, Double> trafficSpeeds =
        this.hbefaRoadTrafficSpeeds.get(hbefaRoadVehicleCategoryKey);

    // TODO: Hier die Berechunung einfügen, die die trafficSpeedTabelle entsprechend aus den Werten
    // erstellt?
    // Frage Laufzeit: Einmal berechnen ha

    if (trafficSpeeds == null || !trafficSpeeds.containsKey(FREEFLOW)) {
      throw new RuntimeException(
          "At least the FREEFLOW condition must be specified for all emission factor keys. "
    ...
    if (trafficSpeeds.containsKey(STOPANDGO) && averageSpeed_kmh <= trafficSpeeds.get(STOPANDGO)) {
      if (averageSpeed_kmh != trafficSpeeds.get(FREEFLOW)) { // handle case
        // testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent6
    ...
    //		if (trafficSpeeds.containsKey(STOPANDGO_HEAVY) && averageSpeed_kmh <=
    // trafficSpeeds.get(STOPANDGO_HEAVY)) {
    //			if (averageSpeed_kmh != trafficSpeeds.get(FREEFLOW)) { //handle case
    // testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent6
  ...
  private void incrementCountersAverage(
      HbefaTrafficSituation hbefaTrafficSituation, double linkLength_km) {
    kmCounter = kmCounter + linkLength_km;
    emissionEventCounter++;

    switch (hbefaTrafficSituation) { // both speeds are assumed to be not very different > only
        // freeFlow on link
      case FREEFLOW -> {
        freeFlowCounter++;
  ...
  /*package-private*/ int getHeavyStopGoCounter() {
    return heavyStopGoCounter;
  }
