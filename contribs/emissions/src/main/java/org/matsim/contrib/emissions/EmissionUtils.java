...
public abstract class EmissionUtils {
  ...
  public static Map<Pollutant, Double> sumUpEmissions(
      Map<Pollutant, Double> warmEmissions, Map<Pollutant, Double> coldEmissions) {
  ...
  public static <T> Map<Id<T>, Map<Pollutant, Double>> sumUpEmissionsPerId(
    ...
    if (warmEmissions == null) return coldEmissions;
    if (coldEmissions == null) return warmEmissions;

    return warmEmissions.entrySet().stream()
        .map(
            entry -> {
  ...
  public static Map<Id<Person>, SortedMap<Pollutant, Double>>
      setNonCalculatedEmissionsForPopulation(
          Population population,
          Map<Id<Person>, SortedMap<Pollutant, Double>> totalEmissions,
          Set<Pollutant> pollutants) {
  ...
  private static Set<String> getAllPollutants(
      Map<Id<Person>, SortedMap<String, Double>> totalEmissions) {
    return totalEmissions.values().stream()
        .flatMap(m -> m.keySet().stream())
        .collect(Collectors.toSet());
  }

  public static Map<Id<Link>, SortedMap<Pollutant, Double>> setNonCalculatedEmissionsForNetwork(
      Network network,
      Map<Id<Link>, SortedMap<Pollutant, Double>> totalEmissions,
      Set<Pollutant> pollutants) {
  ...
  public static <T> SortedMap<Pollutant, Double> getTotalEmissions(
      Map<Id<T>, SortedMap<Pollutant, Double>> person2TotalEmissions) {
  ...
  public static void setHbefaVehicleDescription(
      final VehicleType vehicleType, final String hbefaVehicleDescription) {
    ...
    Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> result =
        convertVehicleDescription2VehicleInformationTuple(vehicleType);

    EngineInformation engineInformation = vehicleType.getEngineInformation();
    VehicleUtils.setHbefaEmissionsConcept(
        engineInformation, result.getSecond().getHbefaEmConcept());
  ...
  static Tuple<HbefaVehicleCategory, HbefaVehicleAttributes>
      convertVehicleDescription2VehicleInformationTuple(VehicleType vehicleType) {
    // yyyy what is the advantage of having this as a tuple over just using a class with four
    // entries?  kai, oct'18
    ...
    HbefaVehicleCategory hbefaVehicleCategory =
        mapString2HbefaVehicleCategory(
            VehicleUtils.getHbefaVehicleCategory(vehicleType.getEngineInformation()));

    HbefaVehicleAttributes hbefaVehicleAttributes = new HbefaVehicleAttributes();

    final String hbefaTechnology =
        VehicleUtils.getHbefaTechnology(vehicleType.getEngineInformation());
    if (hbefaTechnology != null) {
      hbefaVehicleAttributes.setHbefaTechnology(hbefaTechnology);
      hbefaVehicleAttributes.setHbefaSizeClass(
          VehicleUtils.getHbefaSizeClass(vehicleType.getEngineInformation()));
      hbefaVehicleAttributes.setHbefaEmConcept(
          VehicleUtils.getHbefaEmissionsConcept(vehicleType.getEngineInformation()));
    }
    // yyyy we are as of now not catching the case where the information is not there. kai/kai,
    // sep'19
  ...
  static Map<HbefaRoadVehicleCategoryKey, Map<HbefaTrafficSituation, Double>>
    ...
    avgHbefaWarmTable.forEach(
        (warmEmissionFactorKey, emissionFactor) -> {
          HbefaRoadVehicleCategoryKey roadVehicleCategoryKey =
              new HbefaRoadVehicleCategoryKey(warmEmissionFactorKey);
  ...
  /*package-private*/ static String getHbefaVehicleDescription(
      VehicleType vehicleType, EmissionsConfigGroup emissionsConfigGroup) {
    if (vehicleType == null) {
      throw new RuntimeException("vehicleType is null; not possible for emissions contrib.");
    }

    EngineInformation engineInformation;
    // get information from where it used to be in previous versions and move to where it should be
    // now:
    logger.debug(
        "emissionsConfigGroup.getHbefaVehicleDescriptionSource="
            + emissionsConfigGroup.getHbefaVehicleDescriptionSource());
    switch (emissionsConfigGroup.getHbefaVehicleDescriptionSource()) {
      case usingVehicleTypeId:
        ...
        // VehicleTypeId can contain ; to provide more specific information, e.g. for
        // HbefaTechnology, HbefaSizeClass, HbefaEmissionsConcept
        if (vehicleType.getId().toString().contains(";")) {
          ...
          VehicleUtils.setHbefaVehicleCategory(
              vehicleType.getEngineInformation(), vehicleType.getId().toString());
        }
      ...
      case fromVehicleTypeDescription:
        ...
        if (VehicleUtils.getHbefaTechnology(vehicleType.getEngineInformation()) != null) {
          ...
          break;
          // yy Note: If the information is already at the new location (as engine attributes), then
          // the following code will silently take
          // it from there, and not complain that the config setting says it should be in the
          // "description".  kai/kai, sep'19
        }

        // v2, hbefa vehicle description is in vehicle type description.  Move to where it is
        // expected now:

        // copy to new location:
        if (vehicleType.getDescription() == null) {
          throw new RuntimeException(
              "vehicleType.getDescription() is null; not possible for selected config setting");
        }
        int startIndex =
            vehicleType
                    .getDescription()
                    .indexOf(EmissionSpecificationMarker.BEGIN_EMISSIONS.toString())
                + EmissionSpecificationMarker.BEGIN_EMISSIONS.toString().length();
        int endIndex =
            vehicleType
                .getDescription()
                .lastIndexOf(EmissionSpecificationMarker.END_EMISSIONS.toString());
        ...
        String oldString =
            EmissionSpecificationMarker.BEGIN_EMISSIONS
                + substring
                + EmissionSpecificationMarker.END_EMISSIONS;
      ...
      default:
        throw new IllegalStateException(
            "Unexpected value: " + emissionsConfigGroup.getHbefaVehicleDescriptionSource());
    }
  ...
  public static HbefaVehicleCategory mapString2HbefaVehicleCategory(String string) {
    ...
    if (string.contains("pass. car")) hbefaVehicleCategory = HbefaVehicleCategory.PASSENGER_CAR;
    else if (string.contains("HGV"))
      hbefaVehicleCategory = HbefaVehicleCategory.HEAVY_GOODS_VEHICLE;
    else if (string.contains("LCV"))
      hbefaVehicleCategory = HbefaVehicleCategory.LIGHT_COMMERCIAL_VEHICLE;
      ...
      try {
        ...
        logger.warn(
            "Could not map String "
                + string
                + " to any HbefaVehicleCategory; please check syntax in hbefa input file.");
        throw new RuntimeException();
      }
    }
  ...
  static Pollutant getPollutant(String pollutantString) {
    // for the time being, we just manually add alternative spellings here, and map them all to the
    // established enums.  One option to make this
    ...
    switch (pollutantString) {
      ...
      default:
        ...
        // the Pollutant.valueOf(...) should fail if the incoming key is not consistent with what is
        // available in the enum.  Two possibilities:
        // (1) it is a new pollutant.  In that case, just add to the enum.
        // (2) It is a different spelling of an already existing pollutant.  In that case, see
        // above.
        // kai, jan'20
    }
    return pollutant;
  ...
  /*package-private*/
  static HbefaVehicleAttributes tryRewriteHbefa3toHbefa4(
      Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> vehicleInformationTuple) {
    // try to re-write the key from hbefa3.x to hbefa4.x:
