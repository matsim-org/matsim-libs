...
//		logger.info(vehicleType.getEngineInformation().getAttributes().toString());
    ...
    /*package-private*/
    static String getHbefaVehicleDescription(VehicleType vehicleType, EmissionsConfigGroup emissionsConfigGroup) {
        if (vehicleType == null) {
    ...
    static HbefaVehicleAttributes tryRewriteHbefa3toHbefa4(Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> vehicleInformationTuple) {
        ...
        // JWJ (May'21): this is only true for passenger vehicles. Heavy vehicles do have size classes.
        //     So, rather check if it is null (not set), THEN only hard-code the overriding statement.
        //     Otherwise, use whatever is given because that is what came from the HBEFA database.
//		attribs2.setHbefaSizeClass("not specified");
        String sizeClass = vehicleInformationTuple.getSecond().getHbefaSizeClass();
        if (sizeClass == null) {
            attribs2.setHbefaSizeClass("not specified");
        } else{
            attribs2.setHbefaSizeClass(sizeClass);
        }
        ...
        switch (vehicleInformationTuple.getSecond().getHbefaEmConcept()) {
            ...
            case "PC-D-Euro-6":
                ...
                break;
            default:
                String emConcept = vehicleInformationTuple.getSecond().getHbefaEmConcept();
                if (emConcept != null) {
                    attribs2.setHbefaEmConcept(emConcept);
                }
                break;
        }
        return attribs2;
