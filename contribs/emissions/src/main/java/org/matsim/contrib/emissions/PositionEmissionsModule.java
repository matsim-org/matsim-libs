...
public class PositionEmissionsModule extends AbstractModule {
  ...
  private static void checkConsistency(Config config) {
    ...
    if (!config
        .controller()
        .getSnapshotFormat()
        .contains(ControllerConfigGroup.SnapshotFormat.positionevents)) {
    ...
    if (isNotCorrectSnapshotStyle(config.qsim().getSnapshotStyle())) {
      throw new RuntimeException(
          "I think generating emissions only makes sense if config.qsim.snapshotstyle == queue or == kinematicWaves");
    }
  }

  private static boolean isNotCorrectSnapshotStyle(QSimConfigGroup.SnapshotStyle style) {
    return !(style.equals(QSimConfigGroup.SnapshotStyle.queue)
        || style.equals(QSimConfigGroup.SnapshotStyle.kinematicWaves));
  }
  ...
  static class EmissionCalculator {
    ...
    @Inject private EmissionsConfigGroup emissionsConfigGroup;
    @Inject private EmissionModule emissionModule;

    Map<Pollutant, Double> calculateWarmEmissions(
        Vehicle vehicle, Link link, double distance, double time) {
    ...
    Map<Pollutant, Double> calculateColdEmissions(
        Vehicle vehicle, double parkingDuration, int distance) {
      ...
      return emissionModule
          ...
          .checkVehicleInfoAndCalculateWColdEmissions(
              vehicle.getType(), vehicle.getId(), null, -1, parkingDuration, distance);
    }

    private Tuple<HbefaVehicleCategory, HbefaVehicleAttributes> getVehicleAttributes(
        Vehicle vehicle) {
      // the following block fixes the vehicle types's emission information whenusing an  old
      // vehicle type format
      // the unit test I found uses an old format, so have it here.
      {
        String hbefaVehicleTypeDescription =
            EmissionUtils.getHbefaVehicleDescription(vehicle.getType(), emissionsConfigGroup);
        // (this will, importantly, repair the hbefa description in the vehicle type. kai/kai,
        // jan'20)
    ...
    private Map<Pollutant, Double> getEmptyColdEmissions() {
      return emissionModule.getColdPollutants().stream()
          .collect(Collectors.toMap(p -> p, p -> 0.0));
    }

    private Map<Pollutant, Double> getEmptyWarmEmissions() {
      return emissionModule.getWarmPollutants().stream()
          .collect(Collectors.toMap(p -> p, p -> 0.0));
    }
  }
  ...
  static class Handler implements BasicEventHandler {
    ...
    @Inject private EmissionCalculator emissionCalculator;
    ...
    private void handlePositionEvent(PositionEvent event) {
      ...
      if (!vehiclesInTraffic.containsKey(event.getVehicleId()))
        return; // only collect positions if vehicle has entered traffic (if vehicle is wait2link
      // its position is calculated but it hasn't entered traffic yet.

      if (trajectories.containsKey(event.getVehicleId())) {
        computeCombinedEmissionEvent(event);
      }

      // remember all the positions. It is important to do it here, so that the current event is not
      // yet in the
    ...
    private Map<Pollutant, Double> computeColdEmissions(
        PositionEvent event, Vehicle vehicle, double distanceToLastPosition) {

      // we remember the vehicles which are currently emmitting cold emissions if not stored here
      // return nothing
      ...
      // this model assumes vehicles to emmit cold emissions for the first 2000m of a trip remove a
      // vehicle from
      ...
      // HBEFA assumes a constantly decreasing ammount of cold emissions depending on the distance
      // travelled
      // the underlying emission module simplifies this into two steps. Between 0-1km and 1-2km. We
      // use the same
      // classes here because we don't want to rewrite all the stuff. The cold emission module
      // computes emissions
      // for 1000m. We take these as is and muliply with distanceToLastPosition / 1000. This way we
      // have the fraction
      // of cold emissions for the distance travelled during the last time step janek oct' 2021
      int distanceClass = distance <= 1000 ? 1 : 2;

      var coldEmissionsFor1km =
          emissionCalculator.calculateColdEmissions(
              vehicle, parkingDurations.get(vehicle.getId()), distanceClass);
      return coldEmissionsFor1km.entrySet().stream()
          .collect(
              Collectors.toMap(
                  Map.Entry::getKey, entry -> entry.getValue() * distanceToLastPosition / 1000));
    }
    ...
    private void computeCombinedEmissionEvent(PositionEvent event) {
      ...
      var distanceToLastPosition =
          CoordUtils.calcEuclideanDistance(event.getCoord(), previousPosition.getCoord());
      ...
      if (distanceToLastPosition > 0) {
        ...
        // add a rounding error to the compared freespeed. The warm emission module has a tolerance
        // of 1km/h
        if (speed <= link.getFreespeed() + 0.01) {
          ...
          var warmEmissions =
              emissionCalculator.calculateWarmEmissions(
                  vehicle, link, distanceToLastPosition, travelTime);
          var combinedEmissions =
              Stream.concat(coldEmissions.entrySet().stream(), warmEmissions.entrySet().stream())
                  .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, Double::sum));

          eventsManager.processEvent(
              new PositionEmissionEvent(event, combinedEmissions, "combined"));
          ...
          log.warn(
              "speed was too fast: "
                  ...
                  + event.getTime()
                  + " prev time: "
                  + previousPosition.getTime()
                  + " current linkId: "
                  + event.getLinkId()
                  + " prev linkId: "
                  + previousPosition.getLinkId()
                  + " agentId: "
                  + event.getPersonId());
        }
      } else {
        // if the vehicle hasn't moved, issue an event with 0 emissions. This way there is an event
        // for every timestep
        // we want that for the palm integration. The more appropriate fix would be to not issue an
        // event here
        // but generate the 0 emission positions in the necdf module in the palm project but this is
        // much easier
        // and will do for now
        // janek july'21
  ...
  public static class PositionEmissionEvent extends Event {
    ...
    public PositionEmissionEvent(
        PositionEvent positionEvent, Map<Pollutant, Double> emissions, String emissionType) {
    ...
    public static MatsimEventsReader.CustomEventMapper getEventMapper() {
      return event -> {
        var position =
            new PositionInfo.DirectBuilder()
                .setAgentState(
                    AgentSnapshotInfo.AgentState.valueOf(event.getAttributes().get("state")))
                ...
                .setPersonId(
                    Id.createPersonId(event.getAttributes().get(HasPersonId.ATTRIBUTE_PERSON)))
            ...
            Arrays.stream(Pollutant.values())
                ...
                .map(
                    pollutant ->
                        Tuple.of(
                            pollutant,
                            Double.parseDouble(event.getAttributes().get(pollutant.toString()))))
