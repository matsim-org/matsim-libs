...
public class TestWarmEmissionsFallbackBehaviour {
  ...
  public void testWarmDetailedValueOnlyDetailed() {
    EmissionModule emissionModule =
        setUpScenario(
            EmissionsConfigGroup.DetailedVsAverageLookupBehavior.onlyTryDetailedElseAbort);

    double travelTimeOnLink =
        21; // sec. approx freeSpeed of link12 is : (200 m) / (9.72.. m/s) approx 20.57 s
    ...
    Assert.assertEquals(
        expectedValue, warmEmissions.get(Pollutant.CO2_TOTAL), MatsimTestUtils.EPSILON);
  }
  ...
  public void testWarm_DetailedElseAbort_ShouldAbort1() {
    ...
    emissionModule
        ...
        .checkVehicleInfoAndCalculateWarmEmissions(
            vehicleFallbackToTechnologyAverage, link, travelTimeOnLink);
  }
  ...
  public void testWarm_DetailedElseAbort_ShouldAbort2() {
    ...
    emissionModule
        .getWarmEmissionAnalysisModule()
        .checkVehicleInfoAndCalculateWarmEmissions(
            vehicleFallbackToAverageTable, link, travelTimeOnLink);
  }
   ...
   */
  private EmissionModule setUpScenario(
      EmissionsConfigGroup.DetailedVsAverageLookupBehavior lookupBehavior) {
    //		Config config = ConfigUtils.createConfig();
    Config config =
        ConfigUtils.loadConfig(
            IOUtils.extendUrl(
                    ExamplesUtils.getTestScenarioURL("emissions-sampleScenario"),
                    "config_empty.xml")
                .toString());
    //		Config config = RunDetailedEmissionToolOnlineExample.prepareConfig( new
    // String[]{"./scenarios/sampleScenario/testv2_Vehv2/config_detailed.xml"} ) ;
