 ...
 *         <p>
 *         ColdEmissionAnalysisModule (ceam) public methods and corresponding
 *         tests: ceam module parameter - implicitly tested in
 *         calculateColdEmissionAndThrowEventTest, rescaleColdEmissionTest ceam
 *         - constructor, nothing to test reset - nothing to test calculate cold
 *         emissions and throw event - calculateColdEmissionsAndThrowEventTest
 *         <p>
 *         private methods and corresponding tests: rescale cold emissions -
 *         rescaleColdEmissionsTest calculate cold emissions - implicitly tested
 *         in calculateColdEmissionAndThrowEventTest, rescaleColdEmissionTest
 *         convert string to tuple - implicitly tested in
...
public class TestColdEmissionAnalysisModuleCase1 {
	...
	private static final Set<Pollutant> pollutants = new HashSet<>(
			Arrays.asList(CO, CO2_TOTAL, FC, HC, NMHC, NOx, NO2, PM, SO2));
	private final int numberOfColdEmissions = pollutants.size();
	// strings for test cases

	// The material below was confused in the way that strings like "petrol" or
	// "diesel" were given for the
	// size classes, and "<1,4L" or ">=2L" for the emissions concept. Tried to make
	// it consistent,
	// but I don't know if it is still testing the original functionality. kai,
	// jul'18
	...
	// third case: complete data corresponding entries in average and detailed table
	// - use detailed
	...
	private static void fillDetailedTable(
...

		{
			...
			HbefaVehicleAttributes vehAtt = ColdEmissionAnalysisModule.createHbefaVehicleAttributes(petrol_technology2,
					leq14l_sizeClass, PC_P_Euro_1_emConcept);

			putIntoHbefaColdTable(detailedHbefaColdTable, vehAtt, new HbefaColdEmissionFactor(detailedPetrolFactor),
					PASSENGER_CAR);
		}
		{
			// add passenger car entry "diesel;>=2L;PC-D-Euro-3":
			HbefaVehicleAttributes vehAtt = ColdEmissionAnalysisModule.createHbefaVehicleAttributes(diesel_technology,
					geq2l_sizeClass, PC_D_Euro_3_emConcept);

			putIntoHbefaColdTable(detailedHbefaColdTable, vehAtt, new HbefaColdEmissionFactor(detailedDieselFactor),
					PASSENGER_CAR);
		}
		{
			// add heavy goods vehicle entry "petrol;none;none":
			// (pre-existing comment: HEAVY_GOODS_VEHICLE;PC petrol;petrol;none should not
			// be used --???)
			HbefaVehicleAttributes vehAtt = ColdEmissionAnalysisModule.createHbefaVehicleAttributes(petrol_technology,
					none_sizeClass, none_emConcept);

			putIntoHbefaColdTable(detailedHbefaColdTable, vehAtt, new HbefaColdEmissionFactor(fakeFactor),
					HEAVY_GOODS_VEHICLE);
		}
		...
		// technology average
		{
			...
			putIntoHbefaColdTable(detailedHbefaColdTable, vehAtt, new HbefaColdEmissionFactor(averagePetrolFactor),
					PASSENGER_CAR);
		}
	...
	private static void fillAveragesTable(Map<HbefaColdEmissionFactorKey, HbefaColdEmissionFactor> avgHbefaColdTable) {
...

		{
			...
			// this should not be used but is needed to assure that the detailed table is
			// tried before the average table
			HbefaVehicleAttributes vehAtt = ColdEmissionAnalysisModule.createHbefaVehicleAttributes(diesel_technology,
					geq2l_sizeClass, PC_D_Euro_3_emConcept);
		...
		}
		{
			...
			// (pre-existing comment: HEAVY_GOODS_VEHICLE;PC petrol;petrol;none should not
			// be used --???)
			final HbefaVehicleAttributes vehAtt = ColdEmissionAnalysisModule
					.createHbefaVehicleAttributes(petrol_technology, none_sizeClass, none_emConcept);

			putIntoHbefaColdTable(avgHbefaColdTable, vehAtt, new HbefaColdEmissionFactor(fakeFactor),
					HEAVY_GOODS_VEHICLE);
		}
	}
	...
	private static void putIntoHbefaColdTable(
			...
			final HbefaVehicleAttributes vehAtt, final HbefaColdEmissionFactor detColdFactor,
