...
/**
 * @author joe
 *     <p>/* test for playground.vsp.emissions.WarmEmissionAnalysisModule
 */
...
public class TestWarmEmissionAnalysisModuleTrafficSituations {
  ...
  private final HbefaTrafficSituation[] trafficSituations = {
  ...
  public TestWarmEmissionAnalysisModuleTrafficSituations(
      EmissionsComputationMethod emissionsComputationMethod) {
  ...
  public void setUp() {
    ...
    ecg.setDetailedVsAverageLookupBehavior(
        EmissionsConfigGroup.DetailedVsAverageLookupBehavior
            .onlyTryDetailedElseAbort); // declare using detailed values

    weam =
        new WarmEmissionAnalysisModule(
            avgHbefaWarmTable,
  ...
  // Test to check that vehicles not found in the detailed table revert back to average table - ie
  // detailed (petrol, 1,2,3), average (petrol), search pet 4
  @Test
  public void testFallBackToAverageTable() {
    ...
    Id<VehicleType> vehicleTypeId =
        Id.create(
            passengercar + ";" + petrolTechnology + ";" + petrolSizeClass + ";" + petrolConcept,
            VehicleType.class);
    ...
    weam.getEcg()
        .setDetailedVsAverageLookupBehavior(
            EmissionsConfigGroup.DetailedVsAverageLookupBehavior
                .tryDetailedThenTechnologyAverageThenAverageTable);
    warmEmissions =
        weam.checkVehicleInfoAndCalculateWarmEmissions(vehicle, pcLink, travelTime * 3.6);
    Assert.assertEquals(
        detailedPetrolFactor[FF_INDEX] * (linkLength / 1000.),
        warmEmissions.get(NOx),
        MatsimTestUtils.EPSILON);

    vehicleTypeId =
        Id.create(
            passengercar + ";" + pcTechnology + ";" + pcSizeClass + ";" + pcConcept,
            VehicleType.class);
    vehicle = vehFac.createVehicle(vehicleId, vehFac.createVehicleType(vehicleTypeId));
    warmEmissions =
        weam.checkVehicleInfoAndCalculateWarmEmissions(vehicle, pcLink, travelTime * 3.6);
    Assert.assertEquals(
        avgPetrolFactor[FF_INDEX] * (linkLength / 1000.),
        warmEmissions.get(NOx),
        MatsimTestUtils.EPSILON);
  }

  // using the different computation methods, the NOx warm Emissions are calculated for the diffrent
  // trafic Situations (FF;HEAVY;SAT;SG)
  @Test
  public void testTrafficSituations() {
    ...
    switch (emissionsComputationMethod) {
      ...
      case AverageSpeed:
        Assert.assertEquals(
            detailedPetrolFactor[HEAVY_INDEX] * (linkLength / 1000.),
            warmEmissions.get(NOx),
            MatsimTestUtils.EPSILON);
        break;
      default:
    ...
    switch (emissionsComputationMethod) {
      ...
      case AverageSpeed:
        Assert.assertEquals(
            detailedPetrolFactor[SAT_INDEX] * (linkLength / 1000.),
            warmEmissions.get(NOx),
            MatsimTestUtils.EPSILON);
        break;
      default:
    ...
    warmEmissions =
        weam.checkVehicleInfoAndCalculateWarmEmissions(vehicle, pcLink, travelTime * 3.6);
    Assert.assertEquals(
        detailedPetrolFactor[SG_INDEX] * (linkLength / 1000.),
        warmEmissions.get(NOx),
        MatsimTestUtils.EPSILON);
...

    {
      ...
      // results should be equal here, because in both cases only the s&g value is relevant (0%
      // freeflow, 100% stop&go).
  ...
  private static Link createMockLink(double linkLength, double ffspeed) {
    ...
    Link l =
        NetworkUtils.createLink(
            Id.createLinkId("link"), mockNode1, mockNode2, null, linkLength, ffspeed, 1800, 1);
  ...
  private void addDetailedRecordsToTestSpeedsTable(
      Map<HbefaRoadVehicleCategoryKey, Map<HbefaTrafficSituation, Double>> hbefaRoadTrafficSpeeds,
      Map<HbefaWarmEmissionFactorKey, HbefaWarmEmissionFactor> detailedHbefaWarmTable) {
    detailedHbefaWarmTable.forEach(
