...
public class TestWarmEmissionAnalysisModuleCase1 {
  ...
  private static final HandlerToTestEmissionAnalysisModules emissionEventManager =
  ...
  public TestWarmEmissionAnalysisModuleCase1(
  ...
  // this test method creates a mock link and mock vehicle with a complete vehicleTypId --> detailed
  // values are used
  // the CO2_TOTAL warm Emissions are compared to a given value --> computed by using detailed
  // Petrol and traffic state freeflow
  ...
  public void testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent1() {
    ...
    Map<Pollutant, Double> warmEmissions =
        warmEmissionAnalysisModule.checkVehicleInfoAndCalculateWarmEmissions(
            vehicle, mockLink, linkLength / PETROL_SPEED_FF * 3.6);
    Assert.assertEquals(
        DETAILED_PETROL_FACTOR_FF * linkLength / 1000.,
        warmEmissions.get(CO2_TOTAL),
        MatsimTestUtils.EPSILON);

    emissionEventManager.reset();

    warmEmissionAnalysisModule.throwWarmEmissionEvent(
        leaveTime, mockLink.getId(), vehicleId, warmEmissions);
    Assert.assertEquals(
        pollutants.size() * DETAILED_PETROL_FACTOR_FF * linkLength / 1000.,
  ...
  public void testCounters1() {
    ...
    warmEmissionAnalysisModule.checkVehicleInfoAndCalculateWarmEmissions(
        vehicle, mockLink, travelTime * 3.6);
    Assert.assertEquals(
        0, warmEmissionAnalysisModule.getFractionOccurences(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        0., warmEmissionAnalysisModule.getFreeFlowKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(0, warmEmissionAnalysisModule.getFreeFlowOccurences());
    Assert.assertEquals(
        linkLength / 1000, warmEmissionAnalysisModule.getKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        linkLength / 1000,
        warmEmissionAnalysisModule.getStopGoKmCounter(),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(1, warmEmissionAnalysisModule.getStopGoOccurences());
    Assert.assertEquals(1, warmEmissionAnalysisModule.getWarmEmissionEventCounter());
    warmEmissionAnalysisModule.reset();

    // = s&g speed
    warmEmissionAnalysisModule.checkVehicleInfoAndCalculateWarmEmissions(
        vehicle, mockLink, linkLength / PETROL_SPEED_SG * 3.6);
    Assert.assertEquals(0, warmEmissionAnalysisModule.getFractionOccurences());
    Assert.assertEquals(
        0., warmEmissionAnalysisModule.getFreeFlowKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(0, warmEmissionAnalysisModule.getFreeFlowOccurences());
    Assert.assertEquals(
    ...
    // speed in km/h
    travelTime =
        .5 * linkLength / PETROL_SPEED_FF * 3.6
            + .5 * (linkLength / PETROL_SPEED_SG) * 3.6; // 540 seconds
    warmEmissionAnalysisModule.checkVehicleInfoAndCalculateWarmEmissions(
        vehicle, mockLink, travelTime);
    switch (emissionsComputationMethod) {
        // the following may be different for the wrong reasons, see comments in the ...Event2
        // method above.  kai, jan'20
      case StopAndGoFraction:
        Assert.assertEquals(
            1., warmEmissionAnalysisModule.getFreeFlowKmCounter(), MatsimTestUtils.EPSILON);
        Assert.assertEquals(0, warmEmissionAnalysisModule.getFreeFlowOccurences());
        Assert.assertEquals(
            1., warmEmissionAnalysisModule.getStopGoKmCounter(), MatsimTestUtils.EPSILON);
        break;
      case AverageSpeed:
        Assert.assertEquals(
            2., warmEmissionAnalysisModule.getFreeFlowKmCounter(), MatsimTestUtils.EPSILON);
        Assert.assertEquals(1, warmEmissionAnalysisModule.getFreeFlowOccurences());
        Assert.assertEquals(
            0., warmEmissionAnalysisModule.getStopGoKmCounter(), MatsimTestUtils.EPSILON);
        break;
      default:
    ...
    Assert.assertEquals(
        linkLength / 1000, warmEmissionAnalysisModule.getKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(0, warmEmissionAnalysisModule.getStopGoOccurences());
    Assert.assertEquals(1, warmEmissionAnalysisModule.getWarmEmissionEventCounter());
    Assert.assertEquals(
        warmEmissionAnalysisModule.getKmCounter(),
        (warmEmissionAnalysisModule.getStopGoKmCounter()
            + warmEmissionAnalysisModule.getFreeFlowKmCounter()),
        MatsimTestUtils.EPSILON);
    warmEmissionAnalysisModule.reset();
    ...
    try {
      warmEmissionAnalysisModule.checkVehicleInfoAndCalculateWarmEmissions(
          vehicle, mockLink, 0.4 * linkLength / PETROL_SPEED_FF * 3.6);
  ...
  public void testCounters5() {
    ...
    Id<VehicleType> inconffVehicleTypeId =
        Id.create(
            PASSENGER_CAR
                + ";"
                + PETROL_TECHNOLOGY
            ...
            VehicleType.class);
    VehiclesFactory vehFac = VehicleUtils.getFactory();
    Vehicle inconffVehicle =
        vehFac.createVehicle(inconffVehicleId, vehFac.createVehicleType(inconffVehicleTypeId));
    ...
    emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
        inconffVehicle, inconLink, inconff / PETROL_SPEED_FF * 3.6);
    ...
    emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
        inconffVehicle, inconLink, inconff / inconffavgSpeed * 3.6);
    Assert.assertEquals(1, emissionsModule.getFreeFlowOccurences());
    Assert.assertEquals(inconff / 1000, emissionsModule.getKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(0, emissionsModule.getStopGoOccurences());
    Assert.assertEquals(1, emissionsModule.getWarmEmissionEventCounter());
    ...
    emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
        inconffVehicle, inconLink, 2 * inconff / (PETROL_SPEED_FF + PETROL_SPEED_SG) * 3.6);
  }
  ...
  public void testCounters1fractional() {
    ...
    Assert.assertEquals(
        linkLength / 1000, emissionsModule.getStopGoKmCounter(), MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(1, emissionsModule.getWarmEmissionEventCounter());
    Assert.assertEquals(
        emissionsModule.getKmCounter(),
        (emissionsModule.getStopGoKmCounter() + emissionsModule.getFreeFlowKmCounter()),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        travelTime,
        3600 * emissionsModule.getFreeFlowKmCounter() / PETROL_SPEED_FF
            + 3600 * emissionsModule.getStopGoKmCounter() / PETROL_SPEED_SG,
        MatsimTestUtils.EPSILON);
    emissionsModule.reset();
    ...
    Assert.assertEquals(0, emissionsModule.getFractionOccurences());
    Assert.assertEquals(
        linkLength / 1000, emissionsModule.getFreeFlowKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(1, emissionsModule.getFreeFlowOccurences());
    Assert.assertEquals(linkLength / 1000, emissionsModule.getKmCounter(), MatsimTestUtils.EPSILON);
  ...
  public void testCounters8() {
    ...
    // = s&g speed
    emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
        vehicle, pcLink, linkLength / PETROL_SPEED_SG * 3.6);
    // > s&g speed, <ff speed
    travelTime =
        .5 * linkLength / PETROL_SPEED_FF * 3.6
            + .5 * (linkLength / PETROL_SPEED_SG) * 3.6; // 540 seconds
    ...
    // = ff speed
    emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
        vehicle, pcLink, linkLength / PETROL_SPEED_FF * 3.6);
  ...
  private static void fillDetailedTable(
    ...
    // petrol free-flow:
    {
      ...
      HbefaWarmEmissionFactor detWarmFactor =
          new HbefaWarmEmissionFactor(DETAILED_PETROL_FACTOR_FF, PETROL_SPEED_FF);
