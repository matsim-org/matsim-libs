...
/**
 * @author julia /
 *     <p>/* Case 2 - free flow entry in both tables, stop go entry in average table -> use average
...
public class TestWarmEmissionAnalysisModuleCase2 {
  // This used to be one large test class, which had separate table entries for each test, but put
  // them all into the same table.  The result was
  // difficult if not impossible to debug, and the resulting detailed table was inconsistent in the
  // sense that it did not contain all combinations of
  // entries. -- I have now pulled this apart into 6 different test classes, this one here plus
  // "Case1" to "Case5".  Things look ok, but given that the
  // single class before was so large that I could not fully comprehend it, there may now be errors
  // in the ripped-apart classes.  Hopefully, over time,
  // this will help to sort things out.  kai, feb'20

  private final HandlerToTestEmissionAnalysisModules emissionEventManager =
      new HandlerToTestEmissionAnalysisModules();
  ...
  // case 2 - free flow entry in both tables, stop go entry in average table -> use average (now
  // fail)
  ...
  public TestWarmEmissionAnalysisModuleCase2(
  ...
  public void testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent2() {
    ...
    Link pclink =
        TestWarmEmissionAnalysisModule.createMockLink(
            "link 2", pclinkLength, PC_FREE_VELOCITY_KMH / 3.6);

    // create a vehicle:
    Id<Vehicle> pcVehicleId = Id.create("veh 2", Vehicle.class);
    Id<VehicleType> pcVehicleTypeId =
        Id.create(
            PASSENGER_CAR + ";" + PC_TECHNOLOGY + ";" + PC_SIZE_CLASS + ";" + PC_CONCEPT,
            VehicleType.class);
    VehiclesFactory vehFac = VehicleUtils.getFactory();
    Vehicle pcVehicle =
        vehFac.createVehicle(pcVehicleId, vehFac.createVehicleType(pcVehicleTypeId));

    // sub case avg speed = free flow speed
    {
      // compute warm emissions with travel time coming from free flow:
      warmEmissions =
          emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
              pcVehicle, pclink, pclinkLength / PC_FREE_VELOCITY_KMH * 3.6);
      // KMT Feb'20: Trying to understand the result:
      // - StopAndGo fraction is computed as 0 --> ONLY freeFlow values needed and looked up
      // - DETAILED FreeFlow value is available in the table (1.0E-4 g/km);  StopAndGo value is NOT
      // available in the table
      // - AVERAGE  FreeFlow value is available in the table (1.0 g/km) ;    StopAndGo value is NOT
      // available in the table ( 10.0 g/km)
      // --> It seems like it was intended (or only implemented) in a way, that if one of the
      // detailed values is missing (FreeFlow or StopGo) there is a fallback to average. So the
      // result of this would be 1.0 g/km * 0.1 km = 0.1 g/km
      // --> Now, after implementing the new fallback behaviour, it is looking up both values
      // (FreeFlow or StopGo) ways independently from each other. Therefore the result comes from
      // the detailed table (1.0E-4 g/km) * * 0.1 km = 1.0E-5 g/km
      // -----> We need a decision here, if we want allow that inconsistent(?) lookup of FreeFlow
      // and Detailed values with different grade of detail or not.
      // After discussion with Kai N. we decided to let it as it is for the time being. I will add a
      // log.info in the consistency checker.  KMT Jul'20

      // results should be equal here, because in both cases only the freeflow value is relevant
      // (100% freeflow, 0% stop&go).
      ...
      Assert.assertEquals(
          2.3,
          emissionEventManager.getSum(),
          MatsimTestUtils
              .EPSILON); // seems to be (0.1 (g/km -- see expected values a few lines above(*#) *
      // number of entries in enum Pollutant)
    ...
    // sub case avg speed = stop go speed
    {
      warmEmissions =
          emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
              pcVehicle, pclink, pclinkLength / PCSG_VELOCITY_KMH * 3.6);
      Assert.assertEquals(
          AVG_PC_FACTOR_SG * pclinkLength / 1000.,
          warmEmissions.get(NMHC),
          MatsimTestUtils.EPSILON);

      emissionsModule.throwWarmEmissionEvent(leaveTime, pclink.getId(), pcVehicleId, warmEmissions);
      Assert.assertEquals(
          pollutants.size() * AVG_PC_FACTOR_SG * pclinkLength / 1000.,
          emissionEventManager.getSum(),
          MatsimTestUtils.EPSILON);
  ...
  public void testCounters3() {
    ...
    Assert.assertEquals(
        pclinkLength / 1000, emissionsModule.getFreeFlowKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(1, emissionsModule.getFreeFlowOccurences());
    Assert.assertEquals(
        pclinkLength / 1000, emissionsModule.getKmCounter(), MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        pclinkLength / 1000, emissionsModule.getKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        pclinkLength / 1000, emissionsModule.getStopGoKmCounter(), MatsimTestUtils.EPSILON);
  ...
  private WarmEmissionAnalysisModule setUp() {
    ...
    Map<HbefaRoadVehicleCategoryKey, Map<HbefaTrafficSituation, Double>> hbefaRoadTrafficSpeeds =
        EmissionUtils.createHBEFASpeedsTable(avgHbefaWarmTable);
    TestWarmEmissionAnalysisModule.addDetailedRecordsToTestSpeedsTable(
        hbefaRoadTrafficSpeeds, detailedHbefaWarmTable);

    EventsManager emissionEventManager = this.emissionEventManager;
    EmissionsConfigGroup ecg = new EmissionsConfigGroup();
    ecg.setHbefaVehicleDescriptionSource(
        EmissionsConfigGroup.HbefaVehicleDescriptionSource.usingVehicleTypeId);
    ...
    return new WarmEmissionAnalysisModule(
        avgHbefaWarmTable,
        detailedHbefaWarmTable,
        hbefaRoadTrafficSpeeds,
        pollutants,
  ...
  private void fillDetailedTable(
    ...
    // entry for second test case "pc" -- should not be used
    {
      ...
      HbefaWarmEmissionFactor detWarmFactor =
          new HbefaWarmEmissionFactor(DETAILED_PC_FACTOR_FF, PC_FREE_VELOCITY_KMH);
    ...
    // entry for ffOnlyTestcase
    {
      ...
      HbefaWarmEmissionFactor detWarmFactor =
          new HbefaWarmEmissionFactor(detailedFfOnlyFactorFf, ffOnlyffSpeed);
    ...
    // entry for sgOnlyTestcase
    {
      ...
      HbefaWarmEmissionFactor detWarmFactor =
          new HbefaWarmEmissionFactor(detailedSgOnlyFactorSg, sgOnlysgSpeed);
