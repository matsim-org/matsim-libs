...
public class EmissionUtilsTest {
  ...
  private Set<Pollutant> pollsFromEU =
      new HashSet<>(Arrays.asList(CO, CO2_TOTAL, FC, HC, NMHC, NOx, NO2, PM, SO2));
  ...
  public final void testSumUpEmissions() {
    ...
    Assert.assertEquals(
        "Value of CO should be " + (wcov + ccov), cov, wcov + ccov, MatsimTestUtils.EPSILON);
    Assert.assertEquals("Value of CO2_TOTAL should be " + wc2v, c2v, wc2v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Value of FC should be " + (wfcv + cfcv), fcv, wfcv + cfcv, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Value of HC should be " + (whcv + chcv), hcv, whcv + chcv, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Value of NMHC should be " + (wnmv + cnmv), nmv, wnmv + cnmv, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Value of NO2 should be " + (wn2v + cn2v), n2v, wn2v + cn2v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Value of NOx should be " + (wnxv + cnxv), nxv, wnxv + cnxv, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Value of PM should be ." + (wpmv + cpmv), pmv, wpmv + cpmv, MatsimTestUtils.EPSILON);
  ...
  public final void testSumUpEmissionsPerId() {
    ...
    Map<Id<Person>, Map<Pollutant, Double>> sums =
        EmissionUtils.sumUpEmissionsPerId(warmEmissions, coldEmissions);
    ...
    Assert.assertEquals(
        "CO value of person 1 should be" + e1co + "but is ", e1co, a1co, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "CO2 value of person 1 should be" + e1c2 + "but is ", e1c2, a1c2, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "FC value of person 1 should be" + e1fc + "but is ", e1fc, a1fc, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "HC value of person 1 should be" + e1hc + "but is ", e1hc, a1hc, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NMHC value of person 1 should be" + e1nm + "but is ", e1nm, a1nm, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NO2 value of person 1 should be" + e1n2 + "but is ", e1n2, a1n2, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NOx value of person 1 should be" + e1nx + "but is ", e1nx, a1nx, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "PM value of person 1 should be" + e1pm + "but is ", e1pm, a1pm, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "SO value of person 1 should be" + e1so + "but is ", e1so, a1so, MatsimTestUtils.EPSILON);

    Assert.assertEquals(
        "CO value of person 2 should be" + e2co + "but is ", e2co, a2co, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "CO2 value of person 2 should be" + e2c2 + "but is ", e2c2, a2c2, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "FC value of person 2 should be" + e2fc + "but is ", e2fc, a2fc, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "HC value of person 2 should be" + e2hc + "but is ", e2hc, a2hc, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NMHC value of person 2 should be" + e2nm + "but is ", e2nm, a2nm, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NO2 value of person 2 should be" + e2n2 + "but is ", e2n2, a2n2, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NOx value of person 2 should be" + e2nx + "but is ", e2nx, a2nx, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "PM value of person 2 should be" + e2pm + "but is ", e2pm, a2pm, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "SO value of person 2 should be" + e2so + "but is ", e2so, a2so, MatsimTestUtils.EPSILON);
  }
  ...
  public final void testGetTotalEmissions_completeData() {
    ...
    Assert.assertEquals(
        CO + " values are not correct",
        p1co + p2co + p3co,
        totalEmissions.get(CO),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        CO2_TOTAL + " values are not correct",
        p1c2 + p2c2 + p3c2,
        totalEmissions.get(CO2_TOTAL),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        FC + " values are not correct",
        p1fc + p2fc + p3fc,
        totalEmissions.get(FC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        HC + " values are not correct",
        p1hc + p2hc + p3hc,
        totalEmissions.get(HC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        NMHC + " values are not correct",
        p1nm + p2nm + p3nm,
        totalEmissions.get(NMHC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        NO2 + " values are not correct",
        p1n2 + p2n2 + p3n2,
        totalEmissions.get(NO2),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        NOx + " values are not correct",
        p1nx + p2nx + p3nx,
        totalEmissions.get(NOx),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        PM + " values are not correct",
        p1pm + p2pm + p3pm,
        totalEmissions.get(PM),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        SO2 + " values are not correct",
        p1so + p2so + p3so,
        totalEmissions.get(SO2),
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "this list should be as long as number of pollutants",
        totalEmissions.keySet().size(),
        pollsFromEU.size());
  }

  @Test
  public final void testSetNonCalculatedEmissionsForPopulation_completeData() {
    // test setNonCalculatedEmissionsForPopulation for three persons with complete lists of
    // emissions
    // check values
    ...
    Map<Id<Person>, SortedMap<Pollutant, Double>> finalMap =
        EmissionUtils.setNonCalculatedEmissionsForPopulation(pop, totalEmissions, pollsFromEU);
    ...
    Assert.assertEquals(
        "the calculated map should contain two persons but contains "
            + finalMap.size()
            + "persons.",
        pop.getPersons().keySet().size(),
        finalMap.size());
    ...
    for (Object id : finalMap.keySet()) {
      ...
      Assert.assertEquals(
          "the number of pullutants is "
              + finalMap.get(id).keySet().size()
              + " but should be"
              + numOfPolls,
          numOfPolls,
          finalMap.get(id).keySet().size());
    ...
    Assert.assertEquals(
        "CO value for person 1 is not correct",
        cov1,
        finalMap.get(idp1).get(CO),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "CO2 value for person 1 is not correct",
        c2v1,
        finalMap.get(idp1).get(CO2_TOTAL),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "FC value for person 1 is not correct",
        fcv1,
        finalMap.get(idp1).get(FC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "HC value for person 1 is not correct",
        hcv1,
        finalMap.get(idp1).get(HC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NMHC value for person 1 is not correct",
        nmv1,
        finalMap.get(idp1).get(NMHC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NO2 value for person 1 is not correct",
        n2v1,
        finalMap.get(idp1).get(NO2),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NOx value for person 1 is not correct",
        nxv1,
        finalMap.get(idp1).get(NOx),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "PM value for person 1 is not correct",
        pmv1,
        finalMap.get(idp1).get(PM),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "SO value for person 1 is not correct",
        sov1,
        finalMap.get(idp1).get(SO2),
        MatsimTestUtils.EPSILON);

    // check: values for all emissions are correct -person 2
    Assert.assertEquals(
        "CO value for person 2 is not correct",
        cov2,
        finalMap.get(idp2).get(CO),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "CO2 value for person 2 is not correct",
        c2v2,
        finalMap.get(idp2).get(CO2_TOTAL),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "FC value for person 2 is not correct",
        fcv2,
        finalMap.get(idp2).get(FC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "HC value for person 2 is not correct",
        hcv2,
        finalMap.get(idp2).get(HC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NMHC value for person 2 is not correct",
        nmv2,
        finalMap.get(idp2).get(NMHC),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NO2 value for person 2 is not correct",
        n2v2,
        finalMap.get(idp2).get(NO2),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "NOx value for person 2 is not correct",
        nxv2,
        finalMap.get(idp2).get(NOx),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "PM value for person 2 is not correct",
        pmv2,
        finalMap.get(idp2).get(PM),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "SO value for person 2 is not correct",
        sov2,
        finalMap.get(idp2).get(SO2),
        MatsimTestUtils.EPSILON);
  }
  ...
  public final void testSetNonCalculatedEmissionsForPopulation_missingMap() {
    ...
    // nothing else in the finalMap
    message =
        "the calculated map should contain "
            + pop.getPersons().size()
            + " person(s) but contains "
            + finalMap.keySet().size()
            + "person(s).";
  ...
  public final void testSetNonCalculatedEmissionsForPopulation_missingPerson() {
    ...
    // check: all persons added to the population are contained in the finalMap
    Assert.assertFalse(
        "the calculated map should not contain person 4", finalMap.containsKey(idp4));
    // nothing else in the finalMap
    message =
        "the calculated map should contain "
            + pop.getPersons().size()
  ...
  public final void testSetNonCalculatedEmissionsForPopulation_nullEmissions() {
    ...
    try {
      // empty emissions map
      Map<Id<Person>, SortedMap<Pollutant, Double>> finalMap =
          EmissionUtils.setNonCalculatedEmissionsForPopulation(pop, null, pollsFromEU);
  ...
  public final void testSetNonCalculatedEmissionsForPopulation_emptyEmissionMap() {
    ...
    for (Id<Person> id : finalMap.keySet()) {
      for (Object pollutant : finalMap.get(id).values()) {
        Assert.assertSame(pollutant.getClass(), Double.class);
        Assert.assertEquals(
            "map of pollutants was missing. Therefore all values should be set to zero.",
            0.0,
            (Double) pollutant,
            MatsimTestUtils.EPSILON);
        Assert.assertNotNull(pollutant);
  ...
  public final void testSetNonCalculatedEmissionsForNetwork() {
    ...
    Map<Id<Link>, SortedMap<Pollutant, Double>> totalEmissionsFilled =
        EmissionUtils.setNonCalculatedEmissionsForNetwork(network, totalEmissions, pollsFromEU);
    ...
    // link 12 and 13
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(CO2_TOTAL), c2link12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(CO), colink12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(FC), fclink12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(HC), hclink12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(NMHC), nmlink12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(NO2), n2link12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(NOx), nxlink12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(PM), pmlink12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link12id).get(SO2), solink12v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(CO2_TOTAL), c2link13v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(CO), colink13v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(FC), fclink13v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(HC), hclink13v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(NMHC), nmlink13v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(NO2), n2link13v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(NOx), nxlink13v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(PM), pmlink13v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link13id).get(SO2), solink13v, MatsimTestUtils.EPSILON);

    // link 14 and 34
    for (Pollutant pollutant : pollsFromEU) {
      Assert.assertEquals(
          totalEmissionsFilled.get(link14id).get(pollutant), .0, MatsimTestUtils.EPSILON);
      Assert.assertEquals(
          totalEmissionsFilled.get(link34id).get(pollutant), .0, MatsimTestUtils.EPSILON);
    }

    // link 23 - partial
    Assert.assertEquals(
        totalEmissionsFilled.get(link23id).get(CO2_TOTAL), .0, MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        totalEmissionsFilled.get(link23id).get(NOx), nxlink23v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link23id).get(PM), pmlink23v, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        totalEmissionsFilled.get(link23id).get(SO2), solink23v, MatsimTestUtils.EPSILON);

    // link 24 - empty
    Assert.assertEquals(
        totalEmissionsFilled.get(link24id).get(CO2_TOTAL), .0, MatsimTestUtils.EPSILON);
  ...
  private void addLinksToNetwork(Scenario sc) {
    ...
    Network network = sc.getNetwork();
    Node node1 =
        NetworkUtils.createAndAddNode(network, Id.create("node1", Node.class), new Coord(.0, .0));
    Node node2 =
        NetworkUtils.createAndAddNode(
            network, Id.create("node2", Node.class), new Coord(.0, 1000.));
    Node node3 =
        NetworkUtils.createAndAddNode(
            network, Id.create("node3", Node.class), new Coord(1000., .0));
    Node node4 =
        NetworkUtils.createAndAddNode(
            network, Id.create("node4", Node.class), new Coord(1000., 1000.));
    NetworkUtils.createAndAddLink(
        network, Id.create("link12", Link.class), node1, node2, 1000., 20., 3600, 2);
    NetworkUtils.createAndAddLink(
        network, Id.create("link13", Link.class), node1, node3, 1000., 20., 3600, 2);
    NetworkUtils.createAndAddLink(
        network, Id.create("link14", Link.class), node1, node4, 1000., 20., 3600, 2);
    NetworkUtils.createAndAddLink(
        network, Id.create("link23", Link.class), node2, node3, 1000., 20., 3600, 2);
    NetworkUtils.createAndAddLink(
        network, Id.create("link24", Link.class), node2, node4, 1000., 20., 3600, 2);
    NetworkUtils.createAndAddLink(
        network,
        Id.create("link34", Link.class),
        node3,
        node4,
        1000.,
        20.,
        3600,
        2); // w/o orig id and type
  }
}
