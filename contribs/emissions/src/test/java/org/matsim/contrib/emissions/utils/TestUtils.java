...
public class TestUtils {
  ...
  public static void writeWarmEventsToFile(
      Path eventsFile,
      Network network,
      Pollutant pollutant,
      double pollutionPerEvent,
      int fromTime,
      int toTime) {
            ...
            link -> {
              for (int i = fromTime; i <= toTime; i++) {
                eventsManager.processEvent(
                    createWarmEmissionEvent(i, link, pollutant, pollutionPerEvent));
              }
            });
  ...
  public static WarmEmissionEvent createWarmEmissionEvent(
      double time, Link link, Pollutant pollutant, double pollutionPerEvent) {
    Map<Pollutant, Double> emissions = new HashMap<>();
    emissions.put(pollutant, pollutionPerEvent);
    return new WarmEmissionEvent(
        time, link.getId(), Id.createVehicleId(UUID.randomUUID().toString()), emissions);
  }

  public static void writeColdEventsToFile(
      Path eventsFile,
      Network network,
      Pollutant pollutant,
      double pollutionPerEvent,
    ...
    network
        ...
        .forEach(
            link -> {
              for (int i = fromTime; i <= toTime; i++) {
                eventsManager.processEvent(
                    createColdEmissionEvent(i, link, pollutant, pollutionPerEvent));
              }
            });
    eventsManager.finishProcessing();
    writer.closeFile();
  ...
  public static ColdEmissionEvent createColdEmissionEvent(
      double time, Link link, Pollutant pollutant, double pollutionPerEvent) {
    Map<Pollutant, Double> emissions = new HashMap<>();
    emissions.put(pollutant, pollutionPerEvent);
    return new ColdEmissionEvent(
        time, link.getId(), Id.createVehicleId(UUID.randomUUID().toString()), emissions);
  }
