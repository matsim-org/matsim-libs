...
public class TestWarmEmissionAnalysisModuleCase3 {
  ...
  // case 3 - stop go entry in both tables, free flow entry in average table -> use average (now
  // fail)
  ...
  public TestWarmEmissionAnalysisModuleCase3(
  ...
  public void testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent3() {
    ...
    Link diesellink =
        TestWarmEmissionAnalysisModule.createMockLink(
            "link 3",
            dieselLinkLength,
            TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_FF_KMH / 3.6);

    Id<VehicleType> dieselVehicleTypeId =
        Id.create(
            PASSENGER_CAR + ";" + dieselTechnology + ";" + dieselSizeClass + ";" + dieselConcept,
            VehicleType.class);
    VehiclesFactory vehFac = VehicleUtils.getFactory();
    Vehicle dieselVehicle =
        vehFac.createVehicle(dieselVehicleId, vehFac.createVehicleType(dieselVehicleTypeId));
    ...
    warmEmissions =
        emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
            dieselVehicle,
            diesellink,
            dieselLinkLength / TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_FF_KMH * 3.6);
    Assert.assertEquals(
        AVG_PC_FACTOR_FF * dieselLinkLength / 1000.,
        warmEmissions.get(PM),
        MatsimTestUtils.EPSILON);
    emissionEventManager.reset();
    emissionsModule.throwWarmEmissionEvent(
        leaveTime, diesellink.getId(), dieselVehicleId, warmEmissions);
    Assert.assertEquals(
        pollutants.size() * AVG_PC_FACTOR_FF * dieselLinkLength / 1000.,
    ...
    warmEmissions =
        emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
            dieselVehicle,
            diesellink,
            dieselLinkLength / TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_SG_KMH * 3.6);
    Assert.assertEquals(
        AVG_PC_FACTOR_SG * dieselLinkLength / 1000.,
        warmEmissions.get(PM),
        MatsimTestUtils.EPSILON);
    emissionEventManager.reset();
    emissionsModule.throwWarmEmissionEvent(
        leaveTime, diesellink.getId(), dieselVehicleId, warmEmissions);
    Assert.assertEquals(
        pollutants.size() * AVG_PC_FACTOR_SG * dieselLinkLength / 1000.,
        emissionEventManager.getSum(),
        MatsimTestUtils.EPSILON);
    emissionEventManager.reset();
    warmEmissions.clear();
  ...
  public void testCounters4() {
    ...
    Assert.assertEquals(
        dieselLinkLength / 1000., emissionsModule.getFreeFlowKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(1, emissionsModule.getFreeFlowOccurences());
    Assert.assertEquals(
        dieselLinkLength / 1000, emissionsModule.getKmCounter(), MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        dieselLinkLength / 1000, emissionsModule.getKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        dieselLinkLength / 1000., emissionsModule.getStopGoKmCounter(), MatsimTestUtils.EPSILON);
  ...
  private void fillDetailedTable(
      Map<HbefaWarmEmissionFactorKey, HbefaWarmEmissionFactor> detailedHbefaWarmTable) {
    // the interesting thing here is that the first table only contains values for freeflow, and the
    // second one only for stop/go.  So they are
...

    {
      ...
      HbefaWarmEmissionFactor detWarmFactorFF =
          new HbefaWarmEmissionFactor(
              .0000001, TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_FF_KMH);
      ...
      HbefaWarmEmissionFactor detWarmFactorStG =
          new HbefaWarmEmissionFactor(
              .000001, TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_SG_KMH);
...

    {
      ...
      HbefaWarmEmissionFactor detWarmFactorFF =
          new HbefaWarmEmissionFactor(
              .000000001, TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_FF_KMH);
      ...
      HbefaWarmEmissionFactor detWarmFactorStG =
          new HbefaWarmEmissionFactor(
              .00000001, TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_SG_KMH);
