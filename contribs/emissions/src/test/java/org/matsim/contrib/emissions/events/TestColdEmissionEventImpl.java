...
public class TestColdEmissionEventImpl {
  ...
  private final Set<Pollutant> coldPollutants =
      new HashSet<>(Arrays.asList(CO, FC, HC, NMHC, NOx, NO2, PM));
  ...
  public final void testGetAttributesForCompleteEmissionMaps() {
    ...
    Assert.assertEquals(
        "the CO value of this cold emission event was "
            + Double.parseDouble(ceg.get(CO.name()))
    ...
    Assert.assertEquals(
        "the FC value of this cold emission event was "
            + Double.parseDouble(ceg.get(FC.name()))
            + "but should have been "
            + fc,
        Double.parseDouble(ceg.get(FC.name())),
        fc,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the HC value of this cold emission event was "
            + Double.parseDouble(ceg.get(HC.name()))
            + "but should have been "
            + hc,
        Double.parseDouble(ceg.get(HC.name())),
        hc,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the NMHC value of this cold emission event was "
            + Double.parseDouble(ceg.get(NMHC.name()))
            + "but should have been "
            + nm,
        Double.parseDouble(ceg.get(NMHC.name())),
        nm,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the NO2 value of this cold emission event was "
            + Double.parseDouble(ceg.get(NO2.name()))
            + "but should have been "
            + n2,
        Double.parseDouble(ceg.get(NO2.name())),
        n2,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the NOx value of this cold emission event was "
            + Double.parseDouble(ceg.get(NOx.name()))
            + "but should have been "
            + nx,
        Double.parseDouble(ceg.get(NOx.name())),
        nx,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the PM value of this cold emission event was "
            + Double.parseDouble(ceg.get(PM.name()))
            + "but should have been "
            + pm,
        Double.parseDouble(ceg.get(PM.name())),
        pm,
  ...
  public final void testGetAttributesForIncompleteMaps() {
    ...
    ColdEmissionEvent valuesNotSetEvent =
        new ColdEmissionEvent(44., linkId, vehicleId, valuesNotSet);
