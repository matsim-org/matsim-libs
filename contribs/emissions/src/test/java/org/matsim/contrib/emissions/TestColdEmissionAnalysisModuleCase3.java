...
public class TestColdEmissionAnalysisModuleCase3 {
	private static final Logger logger = LogManager.getLogger(TestColdEmissionAnalysisModuleCase3.class);
	...
	void calculateColdEmissionsAndThrowEventTest_completeData() {
		...
		String message = "The expected emissions for " + testCase3 + " are " + pollutants.size() * (Double) testCase3.get( 4 ) + " but were " + sumOfEmissions;
		Assertions.assertEquals( pollutants.size() * (Double) testCase3.get( 4 ), sumOfEmissions, MatsimTestUtils.EPSILON, message );
