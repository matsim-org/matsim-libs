...
public class TestWarmEmissionEventImpl {
  ...
  //	private final Set<String> pollutants = new HashSet<>(Arrays.asList(CO, CO2_TOTAL, FC, HC, NMHC,
  // NOx, NO2,PM, SO2));
  ...
  public final void testGetAttributesForCompleteEmissionMaps() {
    ...
    Assert.assertEquals(
        "the CO value of this warm emission event was "
            + Double.parseDouble(weg.get(CO.name()))
    ...
    Assert.assertEquals(
        "the CO2 value of this warm emission event was "
            + Double.parseDouble(weg.get(CO2_TOTAL.name()))
            + "but should have been "
            + c2,
        Double.parseDouble(weg.get(CO2_TOTAL.name())),
        c2,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the FC value of this warm emission event was "
            + Double.parseDouble(weg.get(FC.name()))
            + "but should have been "
            + fc,
        Double.parseDouble(weg.get(FC.name())),
        fc,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the HC value of this warm emission event was "
            + Double.parseDouble(weg.get(HC.name()))
            + "but should have been "
            + hc,
        Double.parseDouble(weg.get(HC.name())),
        hc,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the NMHC value of this warm emission event was "
            + Double.parseDouble(weg.get(NMHC.name()))
            + "but should have been "
            + nm,
        Double.parseDouble(weg.get(NMHC.name())),
        nm,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the NO2 value of this warm emission event was "
            + Double.parseDouble(weg.get(NO2.name()))
            + "but should have been "
            + n2,
        Double.parseDouble(weg.get(NO2.name())),
        n2,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the NOx value of this warm emission event was "
            + Double.parseDouble(weg.get(NOx.name()))
            + "but should have been "
            + nx,
        Double.parseDouble(weg.get(NOx.name())),
        nx,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the PM value of this warm emission event was "
            + Double.parseDouble(weg.get(PM.name()))
            + "but should have been "
            + pm,
        Double.parseDouble(weg.get(PM.name())),
        pm,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "the SO2 value of this warm emission event was "
            + Double.parseDouble(weg.get(SO2.name()))
            + "but should have been "
            + so,
        Double.parseDouble(weg.get(SO2.name())),
        so,
