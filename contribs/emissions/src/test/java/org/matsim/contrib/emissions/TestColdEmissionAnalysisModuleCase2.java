...
public class TestColdEmissionAnalysisModuleCase2 {
  ...
  private static final Logger logger =
      LogManager.getLogger(TestColdEmissionAnalysisModuleCase2.class);
  ...
  public void calculateColdEmissionsAndThrowEventTest_completeData() {
    ...
    Collections.addAll(
        ...
        petrol_technology2,
        leq14l_sizeClass,
        PC_P_Euro_1_emConcept,
        detailedPetrolFactor);
    ...
    String message =
        "The expected emissions for "
            + testCase2.toString()
            + " are "
            + pollutants.size() * (Double) testCase2.get(4)
            + " but were "
            + sumOfEmissions;
    Assert.assertEquals(
        message,
  ...
  private ColdEmissionAnalysisModule setUp() {
    ...
    // This represents the previous behavior, which falls back to the average table, if values are
