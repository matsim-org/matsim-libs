...
public class TestColdEmissionAnalysisModuleCase2 {
	...
	private static final Logger logger = LogManager.getLogger(TestColdEmissionAnalysisModuleCase2.class);
	...
	void calculateColdEmissionsAndThrowEventTest_completeData() {
		...
		Collections.addAll( testCase2, "PASSENGER_CAR", petrol_technology2, leq14l_sizeClass, PC_P_Euro_1_emConcept, detailedPetrolFactor );
		...
		String message = "The expected emissions for " + testCase2 + " are " + pollutants.size() * (Double) testCase2.get( 4 ) + " but were " + sumOfEmissions;
