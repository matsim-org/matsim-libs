...
public class FastEmissionGridAnalyzerTest {
  ...
  public void rasterNetwork_singleLink() {
    ...
    raster.forEachIndex(
        (xi, yi, value) ->
            assertEquals(emissionPerLink / expectedCellNumber / cellArea, value, 0.00000001));
  }
  ...
  public void rasterNetwork_twoLinks() {
    ...
    raster.forEachIndex(
        (xi, yi, value) -> {
          if (xi == 0 && yi == 0) {
            assertEquals(
                (emissionPerLink1 + emissionPerLink2) / (expectedCellNumber * cellArea),
                value,
                0.00001);
          } else if (xi == 0) {
  ...
  public void blur_rasterWithSingleValue() {
    ...
    // raster with a value of one which means only the surrounding pixels of the center receive
    // values
    var blurredRaster = FastEmissionGridAnalyzer.blur(raster, 1);

    blurredRaster.forEachIndex(
        (xi, yi, value) -> {

          // the value should be spread over 9 pixels. The center point should be 1/4 of the
          // original value
          ...
          else if ((xi == 1 || xi == 3) && (yi == 1 || yi == 3))
            assertEquals(0.625, value, EPSILON);
  ...
  public void processEventsFile() {
    ...
    final var networkUrl =
        IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("equil"), "network.xml");
    final var emissionEvents =
        Paths.get(utils.getOutputDirectory()).resolve("emission.events.xml.gz");
    ...
    var rasterMap =
        FastEmissionGridAnalyzer.processEventsFile(emissionEvents.toString(), network, 1000, 1);
