...
/**
 * Most of the other test implicitly test the EmissionModule as well. Still, I guess it makes sense
 * to have this here
 */
public class EmissionModuleTest {
  ...
  public void testWithIncorrectNetwork() {
    ...
    emissionConfig.setAverageColdEmissionFactorsFile(
        IOUtils.extendUrl(scenarioURL, "sample_41_EFA_ColdStart_SubSegm_2020detailed.csv")
            .toString());
    emissionConfig.setAverageWarmEmissionFactorsFile(
        IOUtils.extendUrl(scenarioURL, "sample_41_EFA_HOT_vehcat_2020average.csv").toString());
    ...
    // create a scenario with a random network where every link has a hbefa road type except one
    // link.
    ...
    network
        ...
        .next()
        .getAttributes()
        .removeAttribute(EmissionUtils.HBEFA_ROAD_TYPE);
    ...
    // this call should cause an exception when the consistency check in the constructor of the
    // emission module fails
