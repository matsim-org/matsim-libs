...
public class TestColdEmissionAnalysisModuleCase6 {
  private static final Logger logger =
      LogManager.getLogger(TestColdEmissionAnalysisModuleCase6.class);
  ...
  public void calculateColdEmissionsAndThrowEventTest_completeData() {
    ...
    String message =
        "The expected emissions for "
            + testCase6.toString()
            + " are "
            + pollutants.size() * (Double) testCase6.get(4)
            + " but were "
            + sumOfEmissions;
    Assert.assertEquals(
        message,
  ...
  private static void fillDetailedTable(
    ...
    }
    {
      ...
      // (pre-existing comment: HEAVY_GOODS_VEHICLE;PC petrol;petrol; --> Should be used, since HGV
      // shpuld be supported and not fallback to average any more, kmt apr'20.
      HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes(
      ...
      putIntoHbefaColdTable(
          detailedHbefaColdTable,
          vehAtt,
          new HbefaColdEmissionFactor(heavyGoodsFactor),
          HEAVY_GOODS_VEHICLE);
    }
    {
      ...
      HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes(
              petrol_technology, none_sizeClass, none_emConcept);
      // (this is for a test of what happens when the setter is not explicitly used.  This should go
      // away
    ...
    }
    {
      ...
      HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes(
              diesel_technology, geq2l_sizeClass, PC_D_Euro_3_emConcept);
      putIntoHbefaColdTable(
          avgHbefaColdTable, vehAtt, new HbefaColdEmissionFactor(heavyGoodsFactor), PASSENGER_CAR);
    }
    {
