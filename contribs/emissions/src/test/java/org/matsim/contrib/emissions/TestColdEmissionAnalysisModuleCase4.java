...
public class TestColdEmissionAnalysisModuleCase4 {
	private static final Logger logger = LogManager.getLogger(TestColdEmissionAnalysisModuleCase4.class);
...
//	private static final String nullcase_emConcept = "nullCase";
	...
	void calculateColdEmissionsAndThrowEventTest_completeData() {
		...
		String message = "The expected emissions for " + testCase4.toString() + " are " + pollutants.size() * (Double) testCase4.get( 4 ) + " but were " + sumOfEmissions;
		Assertions.assertEquals( pollutants.size() * (Double) testCase4.get( 4 ), sumOfEmissions, MatsimTestUtils.EPSILON, message );
