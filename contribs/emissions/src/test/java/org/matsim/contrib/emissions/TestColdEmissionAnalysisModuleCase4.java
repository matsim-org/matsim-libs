...
/**
 * @author julia
 *     <p>test for playground.vsp.emissions.ColdEmissionAnalysisModule
 *     <p>ColdEmissionAnalysisModule (ceam) public methods and corresponding tests: ceam module
 *     parameter - implicitly tested in calculateColdEmissionAndThrowEventTest,
 *     rescaleColdEmissionTest ceam - constructor, nothing to test reset - nothing to test calculate
 *     cold emissions and throw event - calculateColdEmissionsAndThrowEventTest
 *     <p>private methods and corresponding tests: rescale cold emissions - rescaleColdEmissionsTest
 *     calculate cold emissions - implicitly tested in calculateColdEmissionAndThrowEventTest,
 *     rescaleColdEmissionTest convert string to tuple - implicitly tested in
 *     calculateColdEmissionAndThrowEventTest, rescaleColdEmissionTest
 *     <p>in all cases the needed tables are created manually by the setUp() method see test methods
 *     for details on the particular test cases
 */
public class TestColdEmissionAnalysisModuleCase4 {
  private static final Logger logger =
      LogManager.getLogger(TestColdEmissionAnalysisModuleCase4.class);
  ...
  // The material below was confused in the way that strings like "petrol" or "diesel" were given
  // for the
  ...
  public void calculateColdEmissionsAndThrowEventTest_completeData() {
    ...
    Vehicle vehicle =
        VehicleUtils.getFactory()
            .createVehicle(vehicleId, VehicleUtils.getFactory().createVehicleType(vehicleTypeId));
    logger.info("VehicleId: " + vehicle.getId().toString());
    logger.info("VehicleTypeId: " + vehicle.getType().getId());

    Map<Pollutant, Double> calculatedPollutants =
        coldEmissionAnalysisModule.checkVehicleInfoAndCalculateWColdEmissions(
            vehicle.getType(), vehicle.getId(), linkId, 0.0, parkingDuration, tableAccDistance);
    double sumOfEmissions =
        calculatedPollutants.values().stream().mapToDouble(Double::doubleValue).sum();

    String message =
        "The expected emissions for "
            + testCase4.toString()
            + " are "
            + pollutants.size() * (Double) testCase4.get(4)
            + " but were "
            + sumOfEmissions;
    Assert.assertEquals(
        message,
  ...
  private ColdEmissionAnalysisModule setUp() {
    ...
    // This represents the previous behavior, which fallbacks to the average table, if values are
    // not found in the detailed table, kmt apr'20
    ...
    return new ColdEmissionAnalysisModule(
        avgHbefaColdTable, detailedHbefaColdTable, ecg, pollutants, emissionEventManager);
  }
  ...
  private static void fillDetailedTable(
...

    {
      ...
      HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes(
              petrol_technology2, leq14l_sizeClass, PC_P_Euro_1_emConcept);
      putIntoHbefaColdTable(
          detailedHbefaColdTable,
          vehAtt,
          new HbefaColdEmissionFactor(detailedPetrolFactor),
          PASSENGER_CAR);
    }
    {
      ...
      HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes(
              diesel_technology, geq2l_sizeClass, PC_D_Euro_3_emConcept);
      putIntoHbefaColdTable(
          detailedHbefaColdTable,
          vehAtt,
          new HbefaColdEmissionFactor(detailedDieselFactor),
          PASSENGER_CAR);
    }
    {
      ...
      HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes(
              petrol_technology, none_sizeClass, none_emConcept);
      putIntoHbefaColdTable(
          detailedHbefaColdTable,
          vehAtt,
          new HbefaColdEmissionFactor(fakeFactor),
          HEAVY_GOODS_VEHICLE);
    }
  }

  private static void fillAveragesTable(
...

    {
      ...
      HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes("average", "average", "average");
      putIntoHbefaColdTable(
          avgHbefaColdTable,
          vehAtt,
          new HbefaColdEmissionFactor(averageAverageFactor),
          PASSENGER_CAR);
    }
    {
      HbefaVehicleAttributes vehAtt =
      ...
      putIntoHbefaColdTable(
          avgHbefaColdTable,
          vehAtt,
          new HbefaColdEmissionFactor(averagePetrolFactor),
          PASSENGER_CAR);
    }
    {
      // duplicate from detailed table, but with different emission factor.
      // this should not be used but is needed to assure that the detailed table is tried before the
      // average table
      HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes(
              diesel_technology, geq2l_sizeClass, PC_D_Euro_3_emConcept);
      putIntoHbefaColdTable(
          avgHbefaColdTable, vehAtt, new HbefaColdEmissionFactor(fakeFactor), PASSENGER_CAR);
    }
    {
      // add HGV entry "petrol;none;none".
      // (pre-existing comment: HEAVY_GOODS_VEHICLE;PC petrol;petrol;none should not be used --???)
      final HbefaVehicleAttributes vehAtt =
          ColdEmissionAnalysisModule.createHbefaVehicleAttributes(
              petrol_technology, none_sizeClass, none_emConcept);
      putIntoHbefaColdTable(
          avgHbefaColdTable, vehAtt, new HbefaColdEmissionFactor(fakeFactor), HEAVY_GOODS_VEHICLE);
    }
  }

  private static void putIntoHbefaColdTable(
      final Map<HbefaColdEmissionFactorKey, HbefaColdEmissionFactor> detailedHbefaColdTable,
      final HbefaVehicleAttributes vehAtt,
      final HbefaColdEmissionFactor detColdFactor,
