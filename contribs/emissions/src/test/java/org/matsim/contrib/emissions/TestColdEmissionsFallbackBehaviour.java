...
/**
 * @author kturner
 *     <p>test the different levels of fallback behaviour -> lookup for detailed or less detailed or
 *     avarage values or abort, depending on @link{DetailedVsAverageLookupBehavior}. Note this test
 *     only focus on one type of emissions and one commutation method. It will NOT test the variety
 *     of commutational methods. For this please look to the other tests.
 */
public class TestColdEmissionsFallbackBehaviour {
  ...
  private final Vehicle vehicleFallbackToTechnologyAverage =
      generateVehicleForFallbackToTechnologyAverage();
  ...
  // This are the expected values and extracted from
  // "./scenarios/sampleScenario/sample_41_EFA_ColdStart_vehcat_2020average.csv" and
  ...
  private final double emissionsFactorInGrammPerKilometer_TechnologyAverage =
      3.27173543; // detailed table
  private final double emissionsFactorInGrammPerKilometer_AverageTable =
      5.043848991; // average table

  // ---------   DetailedVsAverageLookupBehavior.onlyTryDetailedElseAbort)   -----------
  /**
   * vehicles information is complete
   *
   * <p>LookupBehavior: onlyTryDetailedElseAbort
   *
   * <p>-> should calculate value
   */
  @Test
  public void testColdDetailedValueOnlyDetailed() {
    EmissionModule emissionModule =
        setUpScenario(DetailedVsAverageLookupBehavior.onlyTryDetailedElseAbort);

    Map<Pollutant, Double> coldEmissions =
        emissionModule
            .getColdEmissionAnalysisModule()
    ...
    Assert.assertEquals(
        emissionsFactorInGrammPerKilometer_Detailed,
        coldEmissions.get(Pollutant.CO2_TOTAL),
   ...
   *
   * <p>-> should abort --> RuntimeException
   */
  ...
  public void testCold_DetailedElseAbort_ShouldAbort1() {
    ...
    emissionModule
        ...
        .checkVehicleInfoAndCalculateWColdEmissions(
            ...
            vehicleFallbackToTechnologyAverage.getId(),
            link.getId(),
            startTime,
            parkingDuration,
            distance);
  ...
  /**
   * vehicles information is complete but fully specified entry is NOT available in detailed table
   * HbefaTechnology is also not in detailed table -> fall back to technology average is NOT
   * possible as well.
   *
   * <p>LookupBehavior: onlyTryDetailedElseAbort
   *
  ...
  public void testCold_DetailedElseAbort_ShouldAbort2() {
    ...
    emissionModule
        ...
        .checkVehicleInfoAndCalculateWColdEmissions(
            vehicleFallbackToTechnologyAverage.getType(),
            vehicleFallbackToAverageTable.getId(),
            link.getId(),
            startTime,
            parkingDuration,
            distance);
  ...
  // -----------
  /**
   * vehicles information is complete LookupBehavior: tryDetailedThenTechnologyAverageElseAbort
   *
   * <p>-> do NOT fall back to technology average ---> should calculate value from detailed value
   */
  @Test
  public void testCold_DetailedThenTechnologyAverageElseAbort_FallbackNotNeeded() {
    EmissionModule emissionModule =
        setUpScenario(DetailedVsAverageLookupBehavior.tryDetailedThenTechnologyAverageElseAbort);
   ...
   *
   * <p>-> do fall back to technology average ---> should calculate value from technology average
   */
  @Test
  public void testCold_DetailedThenTechnologyAverageElseAbort_FallbackToTechnologyAverage() {
    ...
    Assert.assertEquals(
        emissionsFactorInGrammPerKilometer_TechnologyAverage,
        coldEmissions.get(Pollutant.CO2_TOTAL),
        MatsimTestUtils.EPSILON);
  }
  ...
  public void testCold_DetailedThenTechnologyAverageElseAbort_ShouldAbort() {
    ...
    emissionModule
        .getColdEmissionAnalysisModule()
        .checkVehicleInfoAndCalculateWColdEmissions(
            vehicleFallbackToAverageTable.getType(),
            vehicleFallbackToAverageTable.getId(),
            link.getId(),
            startTime,
            parkingDuration,
   ...
   * vehicles information is complete LookupBehavior: tryDetailedThenTechnologyAverageElseAbort
   *
   * <p>-> do NOT fall back to technology average or average table ---> should calculate value from
   * detailed value
   */
  @Test
   ...
   * LookupBehavior: tryDetailedThenTechnologyAverageElseAbort
   *
   * <p>-> do fall back to technology average; do NOT fall back to average table ---> should
   * calculate value from technology average
   */
  @Test
   ...
   * possible as well.
   *
   * <p>LookupBehavior: tryDetailedThenTechnologyAverageThenAverageTable
   *
   * <p>-> do fall back to average table ---> should calculate value from average table
   */
  @Test
  public void testCold_DetailedThenTechnologyAverageThenAverageTable_FallbackToAverageTable() {
    ...
    Assert.assertEquals(
        emissionsFactorInGrammPerKilometer_AverageTable,
        coldEmissions.get(Pollutant.CO2_TOTAL),
        MatsimTestUtils.EPSILON);
  }
  ...
  private EmissionModule setUpScenario(DetailedVsAverageLookupBehavior lookupBehavior) {
    ...
    //	//Somehow needed even if deprecated, since a null pointer exception ids thrown when not set
    // :( . kmt mar'20
  ...
  private Vehicle generateVehicleForFallbackToTechnologyAverage() {
    ...
    VehicleUtils.setHbefaEmissionsConcept(
        engineInformation, "PC-D-Euro-3_NotInTable"); // <--- this value is not in table
    VehicleUtils.setHbefaSizeClass(engineInformation, ">1,4L");

    return VehicleUtils.createVehicle(
        Id.createVehicleId("dieselCarFallbackToTechnology"), vehicleType);
  }
  ...
  private Vehicle generateVehicleForFallbackToAverageTable() {
    ...
    VehicleUtils.setHbefaTechnology(
        engineInformation, "diesel_NotInTable"); // <--- this value is not in table
    VehicleUtils.setHbefaEmissionsConcept(engineInformation, "PC-D-Euro-3");
    VehicleUtils.setHbefaSizeClass(engineInformation, ">1,4L");

    return VehicleUtils.createVehicle(
        Id.createVehicleId("dieselCarFallbackToAverage"), vehicleType);
  }
}
