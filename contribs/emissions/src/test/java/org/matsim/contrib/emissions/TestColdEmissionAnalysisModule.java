...
public class TestColdEmissionAnalysisModule {
  ...
  private static final Set<Pollutant> pollutants =
    ...
    for (Id<VehicleType> vehicleTypeId : testCasesExceptions) {
      String message =
          "'"
              + vehicleTypeId
              + "'"
              + " was used to calculate cold emissions and generate an emissions event."
              + "It should instead throw an exception because it is not a valid vehicle information string.";
      try {
        Id<Link> linkId =
            Id.create("linkId" + testCasesExceptions.indexOf(vehicleTypeId), Link.class);
        Id<Vehicle> vehicleId =
            Id.create("vehicleId" + testCasesExceptions.indexOf(vehicleTypeId), Vehicle.class);
        Vehicle vehicle =
            VehicleUtils.getFactory()
                .createVehicle(
                    vehicleId, VehicleUtils.getFactory().createVehicleType(vehicleTypeId));
  ...
  public void calculateColdEmissionsAndThrowEventTest_minimalVehicleInformation() {
    ...
    Vehicle vehicle =
        VehicleUtils.getFactory()
            .createVehicle(vehicleId7, VehicleUtils.getFactory().createVehicleType(vehInfo11));
    ...
    String message =
        "The expected emissions for an emissions event with vehicle information string '"
            + vehInfo11
            + "' are "
            + numberOfColdEmissions * averageAverageFactor
            + " but were "
            + sumOfEmissions;
    Assert.assertEquals(
        message,
