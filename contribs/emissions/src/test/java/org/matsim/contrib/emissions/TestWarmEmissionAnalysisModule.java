 ...
 *     <p>WarmEmissionAnalysisModule (weam) public methods and corresponding tests: weamParameter -
 *     testWarmEmissionAnalysisParameter throw warm EmissionEvent -
 *     testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent*,
 *     testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent_Exceptions check
 *     vehicle info and calculate warm emissions
 *     -testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent*,
 *     testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent_Exceptions get free
 *     flow occurences - testCounters*() get fraction occurences - testCounters*() get stop go
 *     occurences - testCounters*() get km counter - testCounters*() get free flow km counter -
 *     testCounters*() get top go km couter - testCounters*() get warm emission event counter -
 *     testCounters*()
 *     <p>private methods and corresponding tests: rescale warm emissions -
 *     rescaleWarmEmissionsTest() calculate warm emissions - implicitly tested convert string 2
 *     tuple - implicitly tested
...
public class TestWarmEmissionAnalysisModule {
  ...
  // (These must be in kmh, since at some later point they are divided by 3.6.  However, this makes
  // them awfully slow for practical purposes ...  kai, jan'20)
  ...
  private static final Double PETROL_SPEED_FF =
      TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_FF_KMH;
  private static final Double PETROL_SPEED_SG =
      TestWarmEmissionAnalysisModule.AVG_PASSENGER_CAR_SPEED_SG_KMH;
  ...
  public TestWarmEmissionAnalysisModule(
  ...
  public void testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent6() {
    ...
    Id<VehicleType> sgffVehicleTypeId =
        Id.create(
            PASSENGER_CAR + ";" + sgffTechnology + ";" + sgffSizeClass + ";" + sgffConcept,
            VehicleType.class);
    VehiclesFactory vehFac = VehicleUtils.getFactory();
    Vehicle sgffVehicle =
        vehFac.createVehicle(sgffVehicleId, vehFac.createVehicleType(sgffVehicleTypeId));

    // avg>ff
    boolean exceptionThrown = false;
    try {
      emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
          sgffVehicle, sgflink, .5 * sgffLinklength / AVG_PASSENGER_CAR_SPEED_FF_KMH * 3.6);
    } catch (RuntimeException re) {
      exceptionThrown = true;
    }
    Assert.assertTrue(
        "An average speed higher than the free flow speed should throw a runtime exception",
        exceptionThrown);

    { // avg=ff=sg -> use ff factors
      Map<Pollutant, Double> warmEmissions =
          emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
              sgffVehicle, sgflink, sgffLinklength / AVG_PASSENGER_CAR_SPEED_FF_KMH * 3.6);
      Assert.assertEquals(
          DETAILED_SGFF_FACTOR_FF * sgffLinklength / 1000.,
          warmEmissions.get(NO2),
    ...
    { // avg<sg -> use sg factors
      Map<Pollutant, Double> warmEmissions =
          emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
              sgffVehicle, sgflink, 2 * sgffLinklength / AVG_PASSENGER_CAR_SPEED_FF_KMH * 3.6);
      Assert.assertEquals(
          DETAILED_SGFF_FACTOR_SG * sgffLinklength / 1000.,
          warmEmissions.get(NO2),
          MatsimTestUtils.EPSILON);
    }
  }
  ...
  @Test
  public void
      testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent_Exceptions1() {
    // -- set up tables, event handler, parameters, module
    setUp();

    // case 5 - no entry in any table - must be different to other test case's strings
    // With the bug fix to handle missing values - this test should no longer throw an error - jm
    // oct'18
    ...
    Id<VehicleType> noeVehicleTypeId =
        Id.create(
            PASSENGER_CAR + ";" + noeTechnology + ";" + noeSizeClass + ";" + noeConcept,
            VehicleType.class);
    VehiclesFactory vehFac = VehicleUtils.getFactory();
    Vehicle noeVehicle =
        vehFac.createVehicle(noeVehicleId, vehFac.createVehicleType(noeVehicleTypeId));
  ...
  @Test
  public void
      testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent_Exceptions3() {
    // -- set up tables, event handler, parameters, module
    setUp();
  ...
  @Test
  public void
      testCheckVehicleInfoAndCalculateWarmEmissions_and_throwWarmEmissionEvent_Exceptions4() {
    // -- set up tables, event handler, parameters, module
    setUp();
  ...
  public void testCounters7() {
    ...
    Id<VehicleType> tableVehicleTypeId =
        Id.create(
            PASSENGER_CAR + ";" + "petrol (4S)" + ";" + "<1,4L" + ";" + "PC-P-Euro-0",
            VehicleType.class);
    VehiclesFactory vehFac = VehicleUtils.getFactory();
    Vehicle tableVehicle =
        vehFac.createVehicle(tableVehicleId, vehFac.createVehicleType(tableVehicleTypeId));
    Link tableLink =
        createMockLink("link table", tableLinkLength, AVG_PASSENGER_CAR_SPEED_FF_KMH / 3.6);
    ...
    Assert.assertEquals(
        tableLinkLength / 1000., emissionsModule.getFreeFlowKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(1, emissionsModule.getFreeFlowOccurences());
    Assert.assertEquals(
        tableLinkLength / 1000, emissionsModule.getKmCounter(), MatsimTestUtils.EPSILON);
    ...
    emissionsModule.checkVehicleInfoAndCalculateWarmEmissions(
        tableVehicle, tableLink, 2 * tableLinkLength / (AVG_PASSENGER_CAR_SPEED_FF_KMH) * 3.6);
    Assert.assertEquals(0, emissionsModule.getFractionOccurences());
    ...
    Assert.assertEquals(
        tableLinkLength / 1000, emissionsModule.getKmCounter(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        tableLinkLength / 1000, emissionsModule.getStopGoKmCounter(), MatsimTestUtils.EPSILON);
  ...
  static Link createMockLink(String linkId, double linkLength, double ffspeed) {
    ...
    Link l =
        NetworkUtils.createLink(
            mockLinkId, mockNode1, mockNode2, null, linkLength, ffspeed, 1800, 1);
  ...
  private void fillDetailedTable(
    ...
    // entries for case sg speed = ff speed
    {
      ...
      HbefaWarmEmissionFactor detWarmFactor =
          new HbefaWarmEmissionFactor(DETAILED_SGFF_FACTOR_FF, AVG_PASSENGER_CAR_SPEED_FF_KMH);
    ...
    }
    {
      ...
      HbefaWarmEmissionFactor detWarmFactor =
          new HbefaWarmEmissionFactor(DETAILED_SGFF_FACTOR_SG, AVG_PASSENGER_CAR_SPEED_SG_KMH);
  ...
  static void fillAverageTable(
      Map<HbefaWarmEmissionFactorKey, HbefaWarmEmissionFactor> avgHbefaWarmTable) {

    // entries for first case "petrol" should not be used since there are entries in the detailed
    // table
    ...
    {
      HbefaWarmEmissionFactor detWarmFactor =
          new HbefaWarmEmissionFactor(AVG_PC_FACTOR_FF, PETROL_SPEED_FF);
    ...
    {
      HbefaWarmEmissionFactor detWarmFactor =
          new HbefaWarmEmissionFactor(AVG_PC_FACTOR_SG, PETROL_SPEED_SG);
      for (Pollutant wp : pollutants) {
        HbefaWarmEmissionFactorKey detWarmKey = new HbefaWarmEmissionFactorKey();
        detWarmKey.setComponent(wp);
        detWarmKey.setRoadCategory(HBEFA_ROAD_CATEGORY);
  ...
  static void addDetailedRecordsToTestSpeedsTable(
    ...
    for (Map.Entry<HbefaWarmEmissionFactorKey, HbefaWarmEmissionFactor> entry :
        detailedHbefaWarmTable.entrySet()) {
      ...
      // for that road-vehicle category key, add traffic situation and speed value from current
      // emissions factor:
      ...
      if (hbefaRoadTrafficSpeeds
              .get(roadVehicleCategoryKey)
              .get(warmEmissionFactorKey.getTrafficSituation())
          != null) {
        if (hbefaRoadTrafficSpeeds
                .get(roadVehicleCategoryKey)
                .get(warmEmissionFactorKey.getTrafficSituation())
            != emissionFactor.getSpeed()) {

          throw new RuntimeException(
              "Try to override speeds from average table with speed from detailed table. This may lead to wrong emission calculations. KMT/GR Aug'20");
        }
      }

      hbefaRoadTrafficSpeeds
          .get(roadVehicleCategoryKey)
          .put(warmEmissionFactorKey.getTrafficSituation(), emissionFactor.getSpeed());
    }
  }
}
