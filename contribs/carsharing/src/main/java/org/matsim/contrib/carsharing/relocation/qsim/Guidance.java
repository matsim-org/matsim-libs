...
public class Guidance {
  ...
  public synchronized Id<Link> getBestOutgoingLink(
      Link startLink, Link destinationLink, double now) {
    ...
    List<? extends PlanElement> trip =
        router.calcRoute(
            ...
            new AttributesImpl());
    Path path =
        lcpc.calcLeastCostPath(
            startLink.getToNode(), destinationLink.getFromNode(), now, person, null);
    if (path.links.size() == 0) return destinationLink.getId();
    else return path.links.get(0).getId();
  }

  public synchronized OptionalTime getExpectedTravelTime(
      Link startLink, Link destinationLink, double departureTime, String mode, Person person) {
    Facility startFacility = new LinkWrapperFacility(startLink);
    Facility destinationFacility = new LinkWrapperFacility(destinationLink);
    List<? extends PlanElement> trip =
        router.calcRoute(
            mode, startFacility, destinationFacility, departureTime, person, new AttributesImpl());
    Route route = ((Leg) trip.get(0)).getRoute();

    return route != null ? route.getTravelTime() : OptionalTime.undefined();
  }

  public synchronized double getExpectedTravelDistance(
      Link startLink, Link destinationLink, double departureTime, String mode, Person person) {
    Facility startFacility = new LinkWrapperFacility(startLink);
    Facility destinationFacility = new LinkWrapperFacility(destinationLink);
    List<? extends PlanElement> trip =
