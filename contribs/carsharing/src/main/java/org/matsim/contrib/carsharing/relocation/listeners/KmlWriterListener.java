...
public class KmlWriterListener implements IterationStartsListener, IterationEndsListener {
  ...
  public void notifyIterationEnds(IterationEndsEvent event) {
    if (event.getIteration() % this.frequency == 0) {
      ...
      for (Entry<String, List<RelocationZone>> relocationZoneEntry :
        ...
        for (Entry<Double, Map<Id<RelocationZone>, Map<String, Double>>> relocationZoneStatiEntry :
            this.carsharingVehicleRelocation.getStatus().get(companyId).entrySet()) {
          Double time = relocationZoneStatiEntry.getKey();
          Map<Id<RelocationZone>, Map<String, Double>> relocationZoneStati =
              relocationZoneStatiEntry.getValue();
          String filename =
              this.outputDirectoryHierarchy.getIterationFilename(
                  event.getIteration(), companyId + "." + time + ".relocation_zones.xml");

          Map<Id<RelocationZone>, Map<String, Object>> relocationZoneStatiData =
              new TreeMap<Id<RelocationZone>, Map<String, Object>>();

          for (Entry<Id<RelocationZone>, Map<String, Double>> relocationZoneStatusEntry :
              relocationZoneStati.entrySet()) {
            ...
            String content =
                "ID: "
                    ...
                    + " vehicles: "
                    + decimalFormat.format(numVehicles)
                    + " requests: "
                    + decimalFormat.format(numRequests)
                    + " (expected: "
                    + decimalFormat.format(numRequestsExpected)
                    + ")"
                    + " returns: "
                    + decimalFormat.format(numReturns)
                    + " (expected: "
                    + decimalFormat.format(numReturnsExpected)
                    + ")";
      ...
      // log relocations
      final BufferedWriter outRelocations =
          IOUtils.getBufferedWriter(
              this.outputDirectoryHierarchy.getIterationFilename(
                  event.getIteration(), "relocations.txt"));
      try {
        for (Entry<String, List<RelocationInfo>> companyEntry :
            this.carsharingVehicleRelocation.getRelocations().entrySet()) {
          List<RelocationInfo> companyRelocations = companyEntry.getValue();

          outRelocations.write(
              "timeSlot	startZone	endZone	startTime	endTime	startLink	endLink	companyID	vehicleID	agentID");
      ...
      for (Entry<String, Map<Double, Matrices>> companyEntry :
          this.demandDistributionHandler.getODMatrices().entrySet()) {
        ...
        for (Entry<Double, Matrices> timeEntry : companyODMatrices.entrySet()) {
          ...
          String filename =
              this.outputDirectoryHierarchy.getIterationFilename(
                  event.getIteration(), "CS-OD-Matrix." + companyId + "." + start + ".txt");
