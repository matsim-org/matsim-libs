...
public final class ChooseRandomTripMode implements PlanAlgorithm {
  ...
  public void run(Plan plan) {
    ...
    if (personMemmbership != null) {
      if (personMemmbership.getMembershipsPerCSType().containsKey("freefloating")) ffcard = true;
      if (personMemmbership.getMembershipsPerCSType().containsKey("oneway")) owcard = true;
    }
    // in case the person does not hold neither ff nor ow membership
    // do not do anything
    if (!ffcard && !owcard) return;
    ...
    if (trips.get(rndIdx).getOriginActivity().getFacilityId() != null) {
      if (!trips
          ...
          .getFacilityId()
          .toString()
          .equals(trips.get(rndIdx).getDestinationActivity().getFacilityId().toString()))
      ...
      if (!trips
          ...
          .getLinkId()
          .toString()
          .equals(trips.get(rndIdx).getDestinationActivity().getLinkId().toString()))
        setRandomTripMode(trips.get(rndIdx), plan, ffcard, owcard);
    }
  }
  ...
  private void setRandomTripMode(final Trip trip, final Plan plan, boolean ffcard, boolean owcard) {
    ...
    if (possibleModes.length == 2) {
      ...
      if ((possibleModes[0].equals("freefloating") && ffcard)
          || (possibleModes[0].equals("oneway") && owcard))
        TripRouter.insertTrip(
            plan,
            trip.getOriginActivity(),
