...
public class TwoWayContainer implements VehiclesContainer {
  ...
  public TwoWayContainer(
      QuadTree<CarsharingStation> twvehicleLocationQuadTree,
      Map<String, CarsharingStation> twowaycarsharingstationsMap,
      Map<CSVehicle, Link> twvehiclesMap) {
  ...
  public CSVehicle findClosestAvailableVehicle(
    ...
    synchronized (twvehicleLocationQuadTree) {
      Collection<CarsharingStation> location =
          this.twvehicleLocationQuadTree.getDisk(
      ...
      if (closest != null) {
        CSVehicle vehicleToBeUsed =
            ((TwoWayCarsharingStation) closest).getVehicles(typeOfVehicle).get(0);
