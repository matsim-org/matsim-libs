...
public class CarSharingDemandTracker implements IterationStartsListener, IterationEndsListener {
  ...
  private Map<Integer, Map<Id<Person>, AgentRentals>> agentRentalsMapHistory =
      new HashMap<Integer, Map<Id<Person>, AgentRentals>>();

  private TreeMap<Integer, Map<String, Map<Double, Matrices>>> ODMatricesHistory =
      new TreeMap<Integer, Map<String, Map<Double, Matrices>>>();

  @Override
  public void notifyIterationEnds(IterationEndsEvent event) {
    this.agentRentalsMapHistory.put(event.getIteration(), this.demandHandler.getAgentRentalsMap());

    this.ODMatricesHistory.put(
        event.getIteration(), this.demandDistributionHandler.getODMatrices());
  }
