...
public class ChooseTheCompanyPriceBased implements ChooseTheCompany {
  ...
  public String pickACompany(Plan plan, Leg leg, double now, String vehicleType) {
    ...
    Set<String> availableCompanies =
        this.memberships
            .getPerPersonMemberships()
            .get(personId)
            .getMembershipsPerCSType()
            .get(mode);
    ...
    for (String companyName : availableCompanies) {
      ...
      CSVehicle vehicle =
          this.carsharingSupply.findClosestAvailableVehicle(
              network.getLinks().get(leg.getRoute().getStartLinkId()),
              mode,
              vehicleType,
              companyName,
              750.0);
      ...
      if (vehicle != null) {
        ...
        double x = random.nextBoolean() ? -0.0001 : +0.0001;
        if (price == -1.0
            || costCalculator.getCost(companyName, carsharingType, rentalInfo) + x < price) {
  ...
  private double estimatetravelTime(Leg leg, Link vehicleLocation, Person person, double now) {
    ...
    LeastCostPathCalculator pathCalculator =
        pathCalculatorFactory.createPathCalculator(
    ...
    Path path =
        pathCalculator.calcLeastCostPath(
            vehicleLocation.getToNode(), endLink.getFromNode(), now, person, vehicle);
