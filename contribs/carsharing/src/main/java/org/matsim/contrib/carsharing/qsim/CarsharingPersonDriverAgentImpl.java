...
public class CarsharingPersonDriverAgentImpl
    implements MobsimDriverAgent, MobsimPassengerAgent, HasPerson, PlanAgent, PTPassengerAgent {
  ...
  public CarsharingPersonDriverAgentImpl(
      final Plan plan,
    ...
    this.basicAgentDelegate =
        new BasicPlanAgentImpl(
            plan,
            scenario,
            simulation.getEventsManager(),
            simulation.getSimTimer(),
            timeInterpretation);
    this.transitAgentDelegate =
        new TransitAgentImpl(
            this.basicAgentDelegate, TransitConfigGroup.BoardingAcceptance.checkLineAndStop);
    this.driverAgentDelegate = new PlanBasedDriverAgentImpl(this.basicAgentDelegate);
    this.originalPlan =
        this.scenario
            .getPopulation()
            .getPersons()
            .get(this.basicAgentDelegate.getId())
            .getSelectedPlan();
    // this should work with multiple qsim threads now since different instances of router are used
    // sep '17 mb
    // if ( scenario.getConfig().qsim().getNumberOfThreads() != 1 ) {
    //	throw new RuntimeException("does not work with multiple qsim threads (will use same instance
    // of router)") ;
    // }
  }
  ...
  public final void endActivityAndComputeNextState(final double now) {
    ...
    if (carsharingLeg(pe)) {
      ...
      for (PlanElement pe2 : originalPlan.getPlanElements()) {
        if (pe2 instanceof Leg && carsharingLeg(pe2)) countCSLegs++;
        if (countCSLegs > this.carsharingTrips) {
          newTrip =
              carsharingManager.reserveAndrouteCarsharingTrip(
                  this.originalPlan, legToBerouted.getMode(), (Leg) (pe2), now);
          carsharingTrips++;
          break;
        }
      }
      ...
      if (newTrip == null) {
        ...
        // Replace InteractionActivity with ActivityImply since the Mobsim needs to be able to set
        // the start time.
        newTrip =
            newTrip.stream()
                .map(e -> e instanceof Activity ? PopulationUtils.createActivity((Activity) e) : e)
  ...
  private boolean carsharingLeg(PlanElement pe) {
    ...
    if (mode.equals("freefloating") || mode.equals("oneway") || mode.equals("twoway")) return true;
  ...
  public final Id<Vehicle> getPlannedVehicleId() {
    ...
    NetworkRoute route =
        (NetworkRoute) ((Leg) currentPlanElement).getRoute(); // if casts fail: illegal state.

    if (route.getVehicleId() != null) return route.getVehicleId();
    else
      return Id.create(
          this.getId(),
          Vehicle.class); // we still assume the vehicleId is the agentId if no vehicleId is given.
  }
  ...
  public boolean getEnterTransitRoute(
    ...
    return this.transitAgentDelegate.getEnterTransitRoute(
        line, transitRoute, stopsToCome, transitVehicle);
  }
