 ...
 *
 * <p>MobsimDriverAgent that - gets a person Id, start and destination link Id - computes best path
 * to it at every turn, using a Guidance instance
 */
public class RelocationAgent implements MobsimDriverAgent {
  ...
  public RelocationAgent(
      Id<Person> id, String companyId, Id<Link> homeLinkId, Scenario scenario, Network network) {
    this.id = id;
  ...
  /**
   * - add RelocationInfo to relocations - reserve car sharing vehicle, aka remove it from storage
   * structure
   */
  ...
  protected void startLeg(String transportMode) {
    ...
    leg.setRoute(
        RouteUtils.createLinkNetworkRouteImpl(
            this.getCurrentLinkId(), this.getDestinationLinkId()));
  ...
  protected void addLinkId(Id<Link> linkId) {
    try {
      ...
      Route newRoute =
          RouteUtils.createLinkNetworkRouteImpl(
              route.getStartLinkId(), linkIds, route.getEndLinkId());
  ...
  public OptionalTime getExpectedTravelTime() {
    if (this.transportMode.equals("bike")) return BIKE_EXPECTED_TRAVEL_TIME;
    else
  ...
  private void deliverCarSharingVehicle() {
    CompanyContainer companyContainer = this.carsharingSupply.getCompany(this.companyId);
    CSVehicle vehicle =
        this.carsharingSupply.getVehicleWithId(this.relocations.get(0).getVehicleId());
  ...
  public Id<Vehicle> getPlannedVehicleId() {
    return (this.relocations.get(0) != null)
        ? Id.create(this.relocations.get(0).getVehicleId(), Vehicle.class)
        : null;
  }
