...
public class OneWayCarsharingStation implements CarsharingStation {
  ...
  private Map<String, ArrayList<CSVehicle>> vehiclesPerType =
      new HashMap<String, ArrayList<CSVehicle>>();
  private String stationId;
  private int avaialbleParkingSpots;
  private Link link;

  public OneWayCarsharingStation(
      String stationId,
      Link link,
      Map<String, Integer> numberOfvehiclesPerType,
  ...
  public ArrayList<CSVehicle> getVehicles(String type) {
    if (this.vehiclesPerType.containsKey(type)) return this.vehiclesPerType.get(type);
    else return new ArrayList<CSVehicle>();
  }
  ...
  public void addCar(String type, CSVehicle vehicle) {
    ...
    if (currentVehicles == null) currentVehicles = new ArrayList<CSVehicle>();

    currentVehicles.add(vehicle);
    this.vehiclesPerType.put(type, currentVehicles);
    if (this.numberOfvehiclesPerType.get(type) == null) this.numberOfvehiclesPerType.put(type, 1);
    else {
      int currentNumberOfVehicles = this.numberOfvehiclesPerType.get(type);
      currentNumberOfVehicles++;
      this.numberOfvehiclesPerType.put(type, currentNumberOfVehicles);
