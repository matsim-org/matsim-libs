...
public class TwoWayEventsHandler implements PersonLeavesVehicleEventHandler, 
PersonEntersVehicleEventHandler, LinkLeaveEventHandler, StartRentalEventHandler, EndRentalEventHandler{
	private Map<String, RentalInfoTW> statsPerVehicle = new HashMap<String, RentalInfoTW>();
	
	private ArrayList<RentalInfoTW> arr = new ArrayList<RentalInfoTW>();
	...
	public void reset(int iteration) {
		arr = new ArrayList<RentalInfoTW>();

	}
	
	@Override
	public void handleEvent(StartRentalEvent event) {
		if (event.getvehicleId().startsWith("TW")){
			RentalInfoTW info = new RentalInfoTW();
	...
	public void handleEvent(EndRentalEvent event) {
		if (event.getvehicleId().startsWith("TW")){
			...
			RentalInfoTW info = this.statsPerVehicle.get(event.getvehicleId());
	...
	public void handleEvent(PersonLeavesVehicleEvent event) {
		...
		if (event.getVehicleId().toString().startsWith("TW")){
			...
			RentalInfoTW info = this.statsPerVehicle.get(event.getVehicleId().toString());
	...
	public ArrayList<RentalInfoTW> rentals() {
		
		return arr;
	}
	
	public class RentalInfoTW {
