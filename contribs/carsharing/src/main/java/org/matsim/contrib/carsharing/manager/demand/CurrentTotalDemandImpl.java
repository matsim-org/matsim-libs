...
public class CurrentTotalDemandImpl implements CurrentTotalDemand {
  ...
  private Map<Id<Person>, CarsharingCurrentRentalsInfo> currentDemand =
      new HashMap<Id<Person>, CarsharingCurrentRentalsInfo>();
  ...
  public boolean hasVehicleOnLink(Id<Person> personId, Link link, String type) {
    if (!currentDemand.containsKey(personId)) return false;
    else return currentDemand.get(personId).hasVehicleOnLink(link, type);
  }

  @Override
  public CSVehicle getVehicleOnLink(Id<Person> personId, Link link, String type) {

    if (!currentDemand.containsKey(personId)) return null;
    else return currentDemand.get(personId).getVehicleOnLink(link, type);
  }
  ...
  public boolean addVehicle(Id<Person> personId, Link link, CSVehicle vehicle, String type) {
    if (!currentDemand.containsKey(personId)) {
      ...
      CarsharingCurrentRentalsInfo carsharingCurrentRentalsInfo =
          new CarsharingCurrentRentalsInfo(network);
