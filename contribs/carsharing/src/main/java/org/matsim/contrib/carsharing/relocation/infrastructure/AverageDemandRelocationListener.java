...
public class AverageDemandRelocationListener
    implements IterationStartsListener, DispatchRelocationsEventHandler {
  ...
  public void notifyIterationStarts(IterationStartsEvent event) {
    ...
    if (this.iteration >= this.carsharingVehicleRelocation.moduleEnableAfterIteration()) {
      List<Map<String, Map<Double, Matrices>>> previousODMatricesList =
          new ArrayList<Map<String, Map<Double, Matrices>>>();
      ...
      for (int i = 1; i <= demandEstimateIterations; i++) {
        Map<String, Map<Double, Matrices>> previousODMatrices =
            this.demandTracker.getODMatrices(this.iteration - 1);
  ...
  public void handleEvent(DispatchRelocationsEvent event) {
    ...
    List<RelocationZone> companyRelocationZones =
        this.carsharingVehicleRelocation.getRelocationZones(companyId);
    ...
    if (null != companyODMatrices) {
      for (RelocationZone relocationZone : companyRelocationZones) {
        ...
        List<Entry> noVehicleFromLocEntries =
            noVehicle.getFromLocEntries(relocationZoneId.toString());
      ...
      for (RelocationInfo info :
          this.calcOptimizedDistanceRelocations(start, end, companyId, companyRelocationZones)) {
        this.carsharingVehicleRelocation.addRelocation(companyId, info);
        log.info(
            "AverageDemandRelocationListener suggests we move vehicle "
                + info.getVehicleId()
                + " from link "
                + info.getStartLinkId()
                + " to "
  ...
  protected ArrayList<RelocationInfo> calculateRelocations(
      Double start, Double end, String companyId, List<RelocationZone> relocationZones) {
    ...
    for (RelocationZone relocationZone : relocationZones) {
      ...
      if (numberOfVehicles >= 1) {
        vehicleSurplusZones.put(
            new Integer(relocationZones.indexOf(relocationZone)), Math.floor(numberOfVehicles));
      } else if (numberOfVehicles <= -1) {
        vehicleDemandZones.put(
            new Integer(relocationZones.indexOf(relocationZone)),
            Math.floor(Math.abs(numberOfVehicles)));
      }
    }

    List<Map.Entry<Integer, Double>> vehicleSurplusList =
        new LinkedList<Map.Entry<Integer, Double>>(vehicleSurplusZones.entrySet());
    List<Map.Entry<Integer, Double>> vehicleDemandList =
        new LinkedList<Map.Entry<Integer, Double>>(vehicleDemandZones.entrySet());
      ...
      while (vehicleDemandEntry.getValue() > 0) {
        ...
        Link destinationLink =
            NetworkUtils.getNearestLink(
                this.carsharingVehicleRelocation.getNetwork(), destinationZone.getCenter());
        ...
        try {
          ...
          LinkedList<Map.Entry<Link, ArrayList<String>>> vehiclesList =
              new LinkedList<Map.Entry<Link, ArrayList<String>>>(
          ...
          originZone.removeVehicles(
              originLink, new ArrayList<String>(Arrays.asList(new String[] {vehicleId})));
        ...
        relocations.add(
            new RelocationInfo(
                ...
                originZone.getId().toString(),
                destinationZone.getId().toString()));
  ...
  private ArrayList<RelocationInfo> calcOptimizedRelocations(
      Double start, Double end, String companyId, List<RelocationZone> relocationZones) {
    ...
    for (RelocationZone relocationZone : relocationZones) {
      ...
      if (numberOfVehicles >= 1) {
        ...
        if ((relocationZone.getNumberOfVehicles()
                    + relocationZone.getNumberOfExpectedReturns() / 2.0)
                - relocationZone.getNumberOfExpectedRequests()
            > 0)
          vehicleSurplusZones.put(
              new Integer(relocationZones.indexOf(relocationZone)),
              Math.floor(
                  (relocationZone.getNumberOfVehicles()
    ...
    while (!vehicleDemandList.isEmpty()
        && vehicleDemandList.get(0).getValue() > 0
        && !vehicleSurplusList.isEmpty()
      ...
      LinkedList<Map.Entry<Link, ArrayList<String>>> vehiclesList =
          new LinkedList<Map.Entry<Link, ArrayList<String>>>(originZone.getVehicles().entrySet());
  ...
  private ArrayList<RelocationInfo> calcOptimizedDistanceRelocations(
      Double start, Double end, String companyId, List<RelocationZone> relocationZones) {
    ...
    for (RelocationZone relocationZone : relocationZones) {
      ...
      if (numberOfVehicles >= 1) {
        ...
        if (relocationZone.getNumberOfExpectedRequests() < relocationZone.getNumberOfVehicles()) {
          ...
          qtSurplus.put(
              relocationZone.getCenter().getX(), relocationZone.getCenter().getY(), relocationZone);
        }
    ...
    while (!vehicleDemandList.isEmpty()
      ...
      RelocationZone originZone =
          qtSurplus.getClosest(
              destinationZone.getCenter().getX(), destinationZone.getCenter().getY());
      ...
      // if (CoordUtils.calcEuclideanDistance(originLink.getCoord(), destinationLink.getCoord()) >=
      // 1000.0)
      relocations.add(
          new RelocationInfo(
              timeSlot,
  ...
  public Matrix getAvgODMatrix(String companyId, Double time, String eventType) {
    ...
    if ((null != companyODMatrices)
        && (companyODMatrices.getMatrices().keySet().contains(eventType))) {
  ...
  protected Map<String, Map<Double, Matrices>> addODMatrices(
      Map<String, Map<Double, Matrices>> ODMatrices1,
      Map<String, Map<Double, Matrices>> ODMatrices2) {
    if (ODMatrices1.isEmpty()) {
      return ODMatrices2;
    }

    for (java.util.Map.Entry<String, Map<Double, Matrices>> companyODMatricesEntry :
        ODMatrices1.entrySet()) {
      ...
      if (null != companyODMatrices2) {
        for (java.util.Map.Entry<Double, Matrices> intervalODMatrices1Entry :
            companyODMatrices1.entrySet()) {
          ...
          if (null != intervalODMatrices2) {
            ...
            for (ArrayList<Entry> fromLocEntries :
                intervalODMatrices2.getMatrix("rentals").getFromLocations().values()) {
            ...
            for (ArrayList<Entry> fromLocEntries :
                intervalODMatrices2.getMatrix("no_vehicle").getFromLocations().values()) {
              for (Entry fromLocEntry : fromLocEntries) {
                String originId = fromLocEntry.getFromLocation();
                String destinationId = fromLocEntry.getToLocation();
  ...
  protected Map<String, Map<Double, Matrices>> divideODMatrices(
      Map<String, Map<Double, Matrices>> ODMatrices, double divisor) {
  ...
  protected RelocationAgent getRelocationAgent(String companyId) {
    Map<Id<Person>, RelocationAgent> relocationAgents =
        this.carsharingVehicleRelocation.getRelocationAgents(companyId);
  ...
  protected void writeAvgODMatrices() {
    for (java.util.Map.Entry<String, Map<Double, Matrices>> companyEntry :
        this.avgODMatrices.entrySet()) {
      ...
      for (java.util.Map.Entry<Double, Matrices> timeEntry : companyODMatrices.entrySet()) {
        ...
        String filename =
            this.outputDirectoryHierarchy.getIterationFilename(
                this.iteration, "CS-OD-AVG." + companyId + "." + start + ".txt");
  ...
  protected void writeAvgODMatricesMap() {
    ...
    for (java.util.Map.Entry<String, List<RelocationZone>> relocationZoneEntry :
        this.carsharingVehicleRelocation.getRelocationZones().entrySet()) {
      String companyId = relocationZoneEntry.getKey();
      Map<Id<RelocationZone>, MultiPolygon> polygons =
          new HashMap<Id<RelocationZone>, MultiPolygon>();

      for (RelocationZone relocationZone : relocationZoneEntry.getValue()) {
        polygons.put(
            relocationZone.getId(),
            (MultiPolygon) relocationZone.getPolygon().getAttribute("the_geom"));
      }

      writer.setPolygons(polygons);

      for (java.util.Map.Entry<Double, Matrices> avgODMatricesEntry :
          this.getAvgODMatrices(companyId).entrySet()) {
        Double time = avgODMatricesEntry.getKey();
        Matrices avgODMatrices = avgODMatricesEntry.getValue();
        String filename =
            this.outputDirectoryHierarchy.getIterationFilename(
                this.iteration, companyId + "." + time + ".relocation_zones_avg.xml");

        Map<Id<RelocationZone>, Map<String, Object>> relocationZoneStatiData =
            new TreeMap<Id<RelocationZone>, Map<String, Object>>();
        ...
        if (null != avgODMatrices) {
          for (RelocationZone relocationZone : relocationZoneEntry.getValue()) {
            ...
            List<Entry> rentalsFromLocEntries =
                rentals.getFromLocEntries(relocationZoneId.toString());
            if (null != rentalsFromLocEntries) {
              for (Entry rentalOriginEntry : rentalsFromLocEntries) {
                numRequests += rentalOriginEntry.getValue();
              }
            ...
            String content =
                "ID: "
                    + relocationZoneId.toString()
                    + " requests: "
                    + decimalFormat.format(numRequests)
                    + " returns: "
                    + decimalFormat.format(numReturns);
