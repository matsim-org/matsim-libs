...
public class CarsharingManagerNew implements CarsharingManagerInterface, IterationStartsListener {
  ...
  @Override
  public List<PlanElement> reserveAndrouteCarsharingTrip(
      final Plan plan, String carsharingType, Leg legToBeRouted, Double time) {
    ...
    boolean willHaveATripFromLocation =
        willUseTheVehicleLaterFromLocation(destinationLink.getId(), plan, legToBeRouted);
    ...
    if (((CarsharingRoute) legToBeRouted.getRoute()).isOldRoute())
      ...
      keepTheCar =
          keepTheCarModel.keepTheCarDuringNextActivity(
              durationOfNextActivity, plan.getPerson(), carsharingType);
    ...
    if (vehicle != null) {
      ...
      if ((willHaveATripFromLocation && keepTheCar)
        ...
        return this.routerProvider.routeCarsharingTrip(
            plan,
            time,
            legToBeRouted,
            carsharingType,
        ...
        if (carsharingType.equals("oneway")) {
          ...
          CompanyContainer companyContainer =
              this.carsharingSupplyContainer.getCompany(vehicle.getCompanyId());
          VehiclesContainer vehiclesContainer =
              companyContainer.getVehicleContainer(carsharingType);
          Link parkingLocation =
              vehiclesContainer.findClosestAvailableParkingLocation(
                  destinationLink, searchDistance);
          ...
          CarsharingStation parkingStation =
              ((TwoWayContainer)
                      this.carsharingSupplyContainer
                          .getCompany(vehicle.getCompanyId())
                          .getVehicleContainer(carsharingType))
      ...
      for (CompanyAgent companyAgent : this.carsharingSupplyContainer.getCompanyAgents().values()) {
        ...
        CSVehicle offeredVehicle =
            companyAgent.vehicleRequest(
                person.getId(),
                startLink,
                destinationLink,
                carsharingType,
                typeOfVehicle,
                vehicleSearchDistance);

        if (offeredVehicle != null) offeredVehicles.add(offeredVehicle);
      }
      ...
      CSVehicle chosenVehicle =
          vehicleChoiceAgent.chooseVehicle(
              offeredVehicles, startLink, legToBeRouted, time, plan.getPerson());
      ...
      if (chosenVehicle == null) {
        ...
        CompanyContainer companyContainer =
            this.carsharingSupplyContainer.getCompany(chosenVehicle.getCompanyId());
        ...
        if (false == companyContainer.reserveVehicle(chosenVehicle)) {
          eventsManager.processEvent(
              new NoVehicleCarSharingEvent(
                  time, carsharingType, chosenVehicle.getCompanyId(), startLink, destinationLink));

          return null;
        }

        eventsManager.processEvent(
            new StartRentalEvent(
                time,
                carsharingType,
                chosenVehicle.getCompanyId(),
                startLink,
                stationLink,
                destinationLink,
                person.getId(),
                chosenVehicle.getVehicleId()));
        ...
        if ((willHaveATripFromLocation && keepTheCar)
          ...
          if ((carsharingType.equals("oneway")) || (carsharingType.equals("freefloating"))) {
            Link parkingStationLink =
                this.carsharingSupplyContainer.findClosestAvailableParkingSpace(
                    destinationLink, carsharingType, chosenVehicle.getCompanyId(), searchDistance);
            if (parkingStationLink == null) return null;
      ...
      case "oneway":
        return Double.parseDouble(
            scenario
                .getConfig()
                .getModules()
                .get(OneWayCarsharingConfigGroup.GROUP_NAME)
                .getParams()
      ...
      case "freefloating":
        return Double.parseDouble(
            scenario
                ...
                .getParams()
                .get("searchDistanceFreefloating"));
      case "twoway":
        return Double.parseDouble(
            scenario
                .getConfig()
