...
public class CarsharingVehicleRelocationContainer {
  ...
  private Map<String, Map<Double, Map<Id<RelocationZone>, Map<String, Double>>>> status =
      new HashMap<String, Map<Double, Map<Id<RelocationZone>, Map<String, Double>>>>();
  ...
  public CarsharingVehicleRelocationContainer(Scenario sc, Network network) {
    ...
    final CarsharingVehicleRelocationConfigGroup confGroup =
        (CarsharingVehicleRelocationConfigGroup)
  ...
  public final void readRelocationAgents() throws IOException {
    ...
    for (Entry<String, Map<String, Map<String, Double>>> companyEntry :
        reader.getRelocationAgentBases().entrySet()) {
      ...
      for (Entry<String, Map<String, Double>> baseEntry : companyEntry.getValue().entrySet()) {
        ...
        while (counter < agentBaseData.get("number")) {
          Id<Person> id =
              Id.createPersonId("RelocationAgent" + "_" + companyId + "_" + baseId + "_" + counter);
          RelocationAgent agent =
              this.relocationAgentFactory.createRelocationAgent(id, companyId, link.getId());
  ...
  public void resetRelocationAgents() {
    for (Entry<String, Map<Id<Person>, RelocationAgent>> companyEntry :
        this.getRelocationAgents().entrySet()) {
  ...
  public void storeStatus(String companyId, double now) {
    Map<Id<RelocationZone>, Map<String, Double>> relocationZonesStatus =
        new HashMap<Id<RelocationZone>, Map<String, Double>>();
    ...
    if (this.status.get(companyId) == null) {
      this.status.put(
          companyId, new HashMap<Double, Map<Id<RelocationZone>, Map<String, Double>>>());
    }

    this.status.get(companyId).put(now, relocationZonesStatus);
  }

  protected Collection<RelocationZone> getAdjacentZones(
      String companyId, RelocationZone currentZone) {
    Collection<RelocationZone> relocationZones =
        new ArrayList<RelocationZone>(this.getRelocationZones().get(companyId));
