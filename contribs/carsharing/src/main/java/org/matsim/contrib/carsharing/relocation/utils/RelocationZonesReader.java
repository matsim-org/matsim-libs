  ...
  public void endTag(final String name, final String content, final Stack<String> context) {
    if (name.equals("relocationZone")) {
      ...
      RelocationZone relocationZone =
          new RelocationZone(Id.create(this.idString, RelocationZone.class), relocationZonePolygon);
      this.relocationZones.get(this.companyId).add(relocationZone);
    }

    if (name.equals("company")) {
      Collections.sort(
          this.relocationZones.get(this.companyId),
          new Comparator<RelocationZone>() {
            @Override
