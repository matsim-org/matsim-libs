...
public class CarsharingSupplyContainer implements CarsharingSupplyInterface {
  ...
  @Override
  public Link findClosestAvailableParkingSpace(
      Link destinationLink, String carsharingType, String companyId, double searchDistance) {
    CompanyContainer companyContainer = this.companies.get(companyId);
    VehiclesContainer vehiclesContainer = companyContainer.getVehicleContainer(carsharingType);
  ...
  public void populateSupply() {
    ...
    final FreeFloatingConfigGroup ffConfigGroup =
        (FreeFloatingConfigGroup)
    ...
    if (areasFile != null) {
      ...
      ffAreasReader.parse(
          ConfigGroup.getInputFileURL(scenario.getConfig().getContext(), areasFile));
