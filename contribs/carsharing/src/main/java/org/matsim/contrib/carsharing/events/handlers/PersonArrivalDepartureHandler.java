 ...
 * @author balac
 */
public class PersonArrivalDepartureHandler
  ...
  public void handleEvent(PersonDepartureEvent event) {
    ...
    if (legMode.equals("egress_walk_ff")) {
      ...
      this.currentDemand.removeVehicle(
          event.getPersonId(), link, carsharingSupply.getAllVehicles().get(vehId), "freefloating");
      eventsManager.processEvent(
          new EndRentalEvent(event.getTime(), linkId, event.getPersonId(), vehId));
      ...
      Link link = network.getLinks().get(linkId);
      this.currentDemand.removeVehicle(
          event.getPersonId(), link, carsharingSupply.getAllVehicles().get(vehId), "oneway");
      eventsManager.processEvent(
          new EndRentalEvent(event.getTime(), linkId, event.getPersonId(), vehId));
      ...
      Link link = network.getLinks().get(linkId);
      this.currentDemand.removeVehicle(
          event.getPersonId(), link, carsharingSupply.getAllVehicles().get(vehId), "twoway");
      eventsManager.processEvent(
          new EndRentalEvent(event.getTime(), linkId, event.getPersonId(), vehId));
    }
    ...
    if (arrivalMode != null) {
      if (vehId.startsWith("OW") && !arrivalMode.equals("access_walk_ow")) {
        ...
        this.currentDemand.removeVehicle(
            event.getPersonId(),
            link,
            carsharingSupply.getAllVehicles().get(vehId),
            "freefloating");
