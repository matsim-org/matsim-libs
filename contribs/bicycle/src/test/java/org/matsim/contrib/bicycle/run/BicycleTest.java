...
public class BicycleTest {
  ...
  public void testNormal() {
    ...
    assertEquals(
        ...
        new EventsFileComparator()
            ...
            .runComparison(eventsFilenameReference, eventsFilenameNew));
    ...
    new PopulationReader(scenarioReference)
        .readFile(utils.getInputDirectory() + "output_plans.xml.gz");
    new PopulationReader(scenarioCurrent)
        .readFile(utils.getOutputDirectory() + "output_plans.xml.gz");
    for (Id<Person> personId : scenarioReference.getPopulation().getPersons().keySet()) {
      double scoreReference =
          scenarioReference.getPopulation().getPersons().get(personId).getSelectedPlan().getScore();
      double scoreCurrent =
          scenarioCurrent.getPopulation().getPersons().get(personId).getSelectedPlan().getScore();
      Assert.assertEquals(
          "Scores of persons " + personId + " are different",
          scoreReference,
          scoreCurrent,
    ...
    assertTrue(
        ...
        PopulationUtils.equalPopulation(
            scenarioReference.getPopulation(), scenarioCurrent.getPopulation()));
  }
  ...
  @Test
  public void testLinkBasedScoring() {
    //		{
    //			Config config = createConfig( 0 );
    //			BicycleConfigGroup bicycleConfigGroup = (BicycleConfigGroup) config.getModules().get(
    // "bicycle" );
        ...
        .readFile(utils.getInputDirectory() + "output_plans.xml.gz");
    {
      ...
      BicycleConfigGroup bicycleConfigGroup2 =
          (BicycleConfigGroup) config2.getModules().get("bicycle");
    ...
    //				new EventsFileComparator().setIgnoringCoordinates( true
    // ).runComparison(eventsFilenameReference, eventsFilenameNew));
    ...
    //		assertTrue("Populations are different",
    // PopulationUtils.equalPopulation(scenarioReference.getPopulation(),
    // scenarioCurrent.getPopulation()));
  }

  @Test
  public void testLinkVsLegMotorizedScoring() {
    // --- withOUT additional car traffic:
    //		{
    //			Config config2 = createConfig( 0 );
    //			BicycleConfigGroup bicycleConfigGroup2 = ConfigUtils.addOrGetModule( config2,
    ...
    // --- WITH additional car traffic:
    {
      ...
      // the following comes from inlining RunBicycleExample, which we need since we need to modify
      // scenario data:
      ...
      for (Link link : scenario.getNetwork().getLinks().values()) {
        link.setAllowedModes(
            CollectionUtils.stringArrayToSet(new String[] {bicycleMode, TransportMode.car}));
      }
      ...
      scenario
          .getConfig()
          .qsim()
          .setVehiclesSource(VehiclesSource.modeVehicleTypesFromVehiclesData);
    ...
    for (Id<Person> personId : scenarioReference.getPopulation().getPersons().keySet()) {
      ...
      double scoreCurrent =
          scenarioCurrent.getPopulation().getPersons().get(personId).getSelectedPlan().getScore();
      Assert.assertEquals(
          "Scores of person=" + personId + " are different",
          scoreReference,
          scoreCurrent,
          MatsimTestUtils.EPSILON);
    }
  ...
  //		BicycleConfigGroup bicycleConfigGroup = (BicycleConfigGroup)
  // config.getModules().get("bicycle");
  ...
  //		Scenario scenarioReference = ScenarioUtils.createScenario(ConfigUtils.createConfig());
  //		Scenario scenarioCurrent = ScenarioUtils.createScenario(ConfigUtils.createConfig());
  //		new PopulationReader(scenarioReference).readFile(utils.getInputDirectory() +
  // "output_plans.xml.gz");
  //		new PopulationReader(scenarioCurrent).readFile(utils.getOutputDirectory() +
  // "output_plans.xml.gz");
  //		for (Id<Person> personId : scenarioReference.getPopulation().getPersons().keySet()) {
  //			double scoreReference =
  // scenarioReference.getPopulation().getPersons().get(personId).getSelectedPlan().getScore();
  //			double scoreCurrent =
  // scenarioCurrent.getPopulation().getPersons().get(personId).getSelectedPlan().getScore();
  //			Assert.assertEquals("Scores of persons " + personId + " are different", scoreReference,
  // scoreCurrent, MatsimTestUtils.EPSILON);
  //		}
  //		assertTrue("Populations are different",
  // PopulationUtils.equalPopulation(scenarioReference.getPopulation(),
  // scenarioCurrent.getPopulation()));
  ...
  public void testInfrastructureSpeedFactor() {
    ...
    scenario
        .getVehicles()
        .addVehicleType(
            vf.createVehicleType(Id.create(bicycleMode, VehicleType.class))
                .setNetworkMode(bicycleMode)
                .setMaximumVelocity(25.0 / 3.6)
                .setPcuEquivalents(0.25));
    ...
    Assert.assertEquals(
        "All bicycle users should use the longest but fastest route where the bicycle infrastructur speed factor is set to 1.0",
        3,
        linkHandler.getLinkId2demand().get(Id.createLinkId("2")),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Only the car user should use the shortest route",
        1,
        linkHandler.getLinkId2demand().get(Id.createLinkId("6")),
        MatsimTestUtils.EPSILON);

    Assert.assertEquals(
        "Wrong travel time (bicycle user)",
        1.0 + Math.ceil(13000 / (25.0 / 3.6)),
        linkHandler.getLinkId2travelTimes().get(Id.createLinkId("2")).get(0),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong travel time (bicycle user)",
        1.0 + Math.ceil(13000 / (25.0 / 3.6)),
        linkHandler.getLinkId2travelTimes().get(Id.createLinkId("2")).get(1),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong travel time (bicycle user)",
        1.0 + Math.ceil(13000 / (25.0 / 3.6)),
        linkHandler.getLinkId2travelTimes().get(Id.createLinkId("2")).get(2),
        MatsimTestUtils.EPSILON);

    Assert.assertEquals(
        "Wrong travel time (car user)",
        Math.ceil(10000 / (13.88)),
        linkHandler.getLinkId2travelTimes().get(Id.createLinkId("6")).get(0),
        MatsimTestUtils.EPSILON);
  }
  ...
  public void testInfrastructureSpeedFactorDistanceMoreRelevantThanTravelTime() {
    ...
    scenario
        ...
        .addVehicleType(
            vf.createVehicleType(Id.create("bicycle", VehicleType.class))
                .setMaximumVelocity(25.0 / 3.6)
                .setPcuEquivalents(0.25)
                .setNetworkMode(bicycleConfigGroup.getBicycleMode()));
    ...
    Assert.assertEquals(
        "All bicycle users should use the shortest route even though the bicycle infrastructur speed factor is set to 0.1",
        4,
        linkHandler.getLinkId2demand().get(Id.createLinkId("6")),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
    ...
    Assert.assertEquals(
        "Wrong travel time (bicycle user)",
        Math.ceil(10000 / (25. * 0.1 / 3.6)),
        linkHandler.getLinkId2travelTimes().get(Id.createLinkId("6")).get(1),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong travel time (bicycle user)",
        Math.ceil(10000 / (25. * 0.1 / 3.6)),
        linkHandler.getLinkId2travelTimes().get(Id.createLinkId("6")).get(2),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong travel time (bicycle user)",
        Math.ceil(10000 / (25. * 0.1 / 3.6)),
        linkHandler.getLinkId2travelTimes().get(Id.createLinkId("6")).get(3),
        MatsimTestUtils.EPSILON);
  }
