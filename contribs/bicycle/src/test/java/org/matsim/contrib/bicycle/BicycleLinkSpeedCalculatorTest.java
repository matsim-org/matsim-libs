...
public class BicycleLinkSpeedCalculatorTest {
  ...
  // The  more general parts of the test (testing different car behavior) were moved to
  // SpeedCalculatorTest in the core, because there it was
  ...
  public void getMaximumVelocity_bike() {
    ...
    QVehicle vehicle =
        new QVehicleImpl(
            createVehicle(link.getFreespeed() * 0.5, 1)); // higher speed than the default
    BicycleLinkSpeedCalculatorDefaultImpl calculator =
        new BicycleLinkSpeedCalculatorDefaultImpl(config);
  ...
  private Link createLink(
      double elevation, String surfaceType, String wayType, double speedFactor) {
    ...
    link.getAttributes()
        .putAttribute(BicycleUtils.BICYCLE_INFRASTRUCTURE_SPEED_FACTOR, speedFactor);
