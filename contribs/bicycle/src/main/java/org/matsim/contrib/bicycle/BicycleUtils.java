...
public final class BicycleUtils {
  ...
  public static final String BICYCLE_INFRASTRUCTURE_SPEED_FACTOR =
  ...
  public static String getUserDefinedNetworkAttribute(
      Link link, String nameOfUserDefinedNetworkAttribute) {
  ...
  /* package */ static double getComfortFactor(String surface) {
    ...
    // This method included another if/els branch with some conditions on road types which could
    // never be reached. The following comment was
    // written above this branch. Deleting it, because I don't know what it was supposed to do.
    // janek may '23
    //
    // For many primary and secondary roads, no surface is specified because they are by default
    // assumed to be is asphalt.
  ...
  /* package */ static double getInfrastructureFactor(String type, String cyclewaytype) {
    ...
    // The method was unreadable before, so I hope I got the logic right. basically this
    // differentiates between explicit cycleway tags, where the
    // road type has an influence on the factor, i.e. cycling along a primary road without cycle
    // lane is less attractive compared to a tertiary road.
    // On the other hand if cycleways are present the factor is always 0.95, exept the cycle tracks
    // has steps (horrible) or the road type is a
  ...
  /* package */ static double getUserDefinedNetworkAttributeFactor(
      String userDefinedNetworkAttributeString, double userDefinedNetworkAttributeDefaultValue) {
