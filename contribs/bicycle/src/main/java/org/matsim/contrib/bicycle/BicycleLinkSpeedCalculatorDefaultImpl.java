...
public final class BicycleLinkSpeedCalculatorDefaultImpl implements BicycleLinkSpeedCalculator {
  private static final Logger log =
      LogManager.getLogger(BicycleLinkSpeedCalculatorDefaultImpl.class);
  ...
  public double getMaximumVelocity(QVehicle qVehicle, Link link, double time) {
    if (isBike(qVehicle)) {
      ...
      return Double.NaN;
      // (this now works because the link speed calculator returns the default for all combinations
      // of (vehicle, link, time) that
  ...
  public double getMaximumVelocityForLink(Link link, Vehicle vehicle) {
    ...
    // prior to matsim 12.0 routers would not pass a vehicle. This is why we have a fallback for a
    // default value from the config
    double maxBicycleSpeed =
        vehicle == null
            ? bicycleConfigGroup.getMaxBicycleSpeedForRouting()
  ...
  /**
   * Based on "Flügel et al. -- Empirical speed models for cycling in the Oslo road network" (not
   * yet published!) Positive gradients (uphill): Roughly linear decrease in speed with increasing
   * gradient At 9% gradient, cyclists are 42.7% slower Negative gradients (downhill): Not linear;
   * highest speeds at 5% or 6% gradient; at gradients higher than 6% braking
   */
  ...
  private double computeInfrastructureFactor(Link link) {
    var speedFactor =
        link.getAttributes().getAttribute(BicycleUtils.BICYCLE_INFRASTRUCTURE_SPEED_FACTOR);
  ...
  private boolean isBike(QVehicle qVehicle) {
    ...
    // the above is what I found.  With mode vehicles, the vehicle ID is indeed abused for the
    // model.  But we should not rely on this.
    // Unfortunately, backwards compatibility may now fail ... I have seen mode vehicles different
    // from car but having car as network
    // mode.  kai, jun'23
    ...
    //		if ( qSimConfigGroup.getVehiclesSource()==
    ...
    //				throw new RuntimeException( "You are using mode vehicles but the network mode of the
    // vehicle type is wrong: vehType.id=" + vehicleType.getId()
    ...
    if (qVehicle
            ...
            .getId()
            .toString()
            .equals(bicycleConfigGroup.getBicycleMode())
        && !vehicleType.getNetworkMode().equals(bicycleConfigGroup.getBicycleMode())) {
      throw new RuntimeException(
          "You are using mode vehicles but the network mode of the vehicle type is wrong: vehType.id="
              + vehicleType.getId()
              + "; vehType.mode="
              + vehicleType.getNetworkMode());
    }
