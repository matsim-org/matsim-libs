...
/**
 * @author smetzler, dziemke based on RandomizingTimeDistanceTravelDisutility and adding more
 *     components
 */
class BicycleTravelDisutility implements TravelDisutility {
  ...
  BicycleTravelDisutility(
    ...
    final ScoringConfigGroup.ModeParams bicycleParams =
        cnScoringGroup.getModes().get(bicycleConfigGroup.getBicycleMode());
    if (bicycleParams == null) {
      throw new NullPointerException(
          "Mode "
              + bicycleConfigGroup.getBicycleMode()
              + " is not part of the valid mode parameters "
              + cnScoringGroup.getModes().keySet());
    }

    this.marginalCostOfDistance_m =
        -(bicycleParams.getMonetaryDistanceRate() * cnScoringGroup.getMarginalUtilityOfMoney())
            - bicycleParams.getMarginalUtilityOfDistance();
    this.marginalCostOfTime_s =
        -(bicycleParams.getMarginalUtilityOfTraveling() / 3600.0)
            + cnScoringGroup.getPerforming_utils_hr() / 3600.0;

    this.marginalCostOfInfrastructure_m =
  ...
  public double getLinkTravelDisutility(Link link, double time, Person person, Vehicle vehicle) {
    ...
    double infrastructureDisutility =
        marginalCostOfInfrastructure_m * (1. - infrastructureFactor) * distance;
    ...
    if (nameOfUserDefinedNetworkAttribute != null) {
      String userDefinedNetworkAttributeString =
          BicycleUtils.getUserDefinedNetworkAttribute(link, nameOfUserDefinedNetworkAttribute);
      double userDefinedNetworkAttributeFactor =
          BicycleUtils.getUserDefinedNetworkAttributeFactor(
              userDefinedNetworkAttributeString, this.userDefinedNetworkAttributeDefaultValue);
      userDefinedNetworkAttritubeDisutility =
          marginalCostOfUserDefinedNetworkAttribute_m
              * (1. - userDefinedNetworkAttributeFactor)
              * distance;
    }

    //		LOG.warn("link = " + link.getId() + "-- travelTime = " + travelTime + " -- distance = " +
    // distance + " -- comfortFactor = "
    //				+ comfortFactor	+ " -- infraFactor = "+ infrastructureFactor + " -- gradient = " +
    // gradientFactor);
    ...
    if (sigma != 0.) {
      if (person == null) {
        throw new RuntimeException(
            "you cannot use the randomzing travel disutility without person.  If you need this without a person, set"
                + "sigma to zero.");
      }
      //			normalRndLink = 0.05 * random.nextGaussian();
      // are we sure that this is a good approach?  In high resolution networks, this leads to
      // quirky detours ...  kai, sep'19
      ...
      if (person != prevPerson) {
        ...
        // are we sure that this is a good approach?  In high resolution networks, this leads to
        // quirky detours ...  kai, sep'19
        // --> addressed with moving it to down here, i.e. into the person.  Also caused race
        // conditions. kai, jul'23
        ...
        // this should be a log-normal distribution with sigma as the "width" parameter.   Instead
        // of figuring out the "location"
    ...
    //		LOG.warn("Person = " + person.getId() + " / link = " + link.getId() + " / ttD = " +
    // travelTimeDisutility	+ " / dD = "+ distanceDisutility
    //				+ " / infD = " + infrastructureDisutility + " / comfD = " + comfortDisutility + " / gradD
    // = " + gradientDisutility + " / rnd = " + normalRndLink
    //				+ " / rndDist = " + logNormalRndDist + " / rndInf = "	+ logNormalRndInf + " / rndComf = "
    // + logNormalRndComf + " / rndGrad = " + logNormalRndGrad);
    double disutility =
        (1 + normalRndLink) * travelTimeDisutility
            + logNormalRndDist * distanceDisutility
            + logNormalRndInf * infrastructureDisutility
            + logNormalRndComf * comfortDisutility
            + logNormalRndGrad * gradientDisutility
            + logNormalRndUserDef * userDefinedNetworkAttritubeDisutility;
    // note that "normalRndLink" follows a Gaussian distribution, not a lognormal one as the others
    // do!
    //		double disutility = travelTimeDisutility + logNormalRndDist * distanceDisutility + (1 +
    // normalRndLink) * logNormalRndInf * infrastructureDisutility
    //				+ (1 + normalRndLink) * logNormalRndComf * comfortDisutility + (1 + normalRndLink) *
    // logNormalRndGrad * gradientDisutility;
