...
/**
 * @author dziemke
 * @deprecated This class is replaced by OsmBicycleReader in the osm-contrib. Use {@link
 *     OsmBicycleReader} instead
 */
@Deprecated
public final class BicycleOsmNetworkReaderV2 extends OsmNetworkReader {
  private static final Logger LOG = LogManager.getLogger(BicycleOsmNetworkReaderV2.class);
  ...
  // OSM Reader is using hierarchy until 8, why following hierarchies are not starting with 9? Amit
  // Feb'18
  ...
  public static void main(String[] args) throws Exception {
    ...
    //		String inputOSM =
    // "../../../shared-svn/studies/countries/de/berlin-bike/input/osm/2017-08-29_mitte.osm";
    String inputOSM =
        "../../../shared-svn/studies/countries/de/berlin-bike/input/osm/berlin-latest.osm";
    String tiffFile =
        "../../../shared-svn/studies/countries/de/berlin-bike/input/eu-dem/BerlinEUDEM.tif"; // Berlin EU-DEM
    //		String outputXML =
    // "../../../shared-svn/studies/countries/de/berlin-bike/input/network/2017-08-29_mitte.xml";
    String outputXML =
        "../../../shared-svn/studies/countries/de/berlin-bike/input/network/berlin-latest.xml";

    // adding bicycle way tags by default: Amit Feb'18
    //		List<String> bicycleWayTags = Arrays.asList(
    //				new String[]{BicycleUtils.CYCLEWAY, BicycleUtils.SURFACE, BicycleUtils.SMOOTHNESS,
    // TAG_BICYCLE, TAG_ONEWAYBICYCLE});

    Network network = NetworkUtils.createNetwork();
    CoordinateTransformation ct =
        TransformationFactory.getCoordinateTransformation(inputCRS, outputCRS);

    ElevationDataParser elevationDataParser = new ElevationDataParser(tiffFile, outputCRS);

    BicycleOsmNetworkReaderV2 bicycleNetworkReader =
        new BicycleOsmNetworkReaderV2(network, ct, elevationDataParser);
  ...
  public BicycleOsmNetworkReaderV2(
      final Network network,
      final CoordinateTransformation transformation,
      ElevationDataParser elevationDataParser,
  ...
  public BicycleOsmNetworkReaderV2(
      ...
      ElevationDataParser elevationDataParser,
      final boolean useHighwayDefaults,
      String bicycleAsTransportModeName,
      double bicyclePCU) {
    this(
        network,
        transformation,
  ...
  public BicycleOsmNetworkReaderV2(
    ...
    // If "useHighwayDefaults" is set to true, super sets defaults for all "roads" ("motorway" to
    // "residential", except "service")
    // and all "link roads" and "living_street" (part of "special road types"). Hierachies 1 to 6
    // are used.
    super(network, transformation, useHighwayDefaults, useVspAdjustments);
    List<String> bicycleWayTags =
        Arrays.asList(
            BicycleUtils.CYCLEWAY,
            BicycleUtils.SURFACE,
            BicycleUtils.SMOOTHNESS,
            TAG_BICYCLE,
            TAG_ONEWAYBICYCLE);
    ...
    if (useHighwayDefaults) {
      ...
      // hierarchy , highwayType, lanesPerDirection, freespeed, freespeedFactor,
      ...
      this.setHighwayDefaults(
          BICYCLE_TRACK_HIERARCHY, "track", 1, 30.0 / 3.6, 1.0, 1500 * bicyclePCU);

      // this.setHighwayDefaults(7, "cycleway", 1, 10.0/3.6, 1.0, 50); // Simon's values
      this.setHighwayDefaults(
          BICYCLE_TRACK_HIERARCHY, "cycleway", 1, 30.0 / 3.6, 1.0, 1500 * bicyclePCU);

      // this.setHighwayDefaults(8, "service", 1, 10.0/3.6, 1.0, 50); // Simon's values
      this.setHighwayDefaults(
          BICYCLE_TRACK_HIERARCHY, "service", 1, 10.0 / 3.6, 1.0, 1000 * bicyclePCU);

      // Capacity of a footway/pedestrian/path zone for bicycles analogous to that of a residential
      // road for cars, dz, aug'18
      // this.setHighwayDefaults(8, "footway", 1, 10.0/3.6, 1.0, 50); // Simon's values
      this.setHighwayDefaults(
          FOOTPATH_TRACK_HIERARCHY,
          "footway",
          1,
          10.0 / 3.6,
          1.0,
          600 * bicyclePCU); // Simon's values
      ...
      this.setHighwayDefaults(
          FOOTPATH_TRACK_HIERARCHY, "path", 1, 20.0 / 3.6, 1.0, 600 * bicyclePCU);

      // "bridleway" is exclusively for equestrians; if others are allowed to use it, it should not
      // be classified as bridleway
      ...
      LOG.error(
          "The way this is written assumes that highway defaults are used! This is, however, not done here."
  ...
  private void stats(Network network) {
    LOG.info(
        "Smoothness created: "
            + countSmoothness
            + " which is "
            + ((float) countSmoothness / network.getLinks().size()) * 100
            + "%.");
    LOG.info(
        "Surface (direct) created: "
            + countSurfaceDirect
            + " which is "
            + ((float) countSurfaceDirect / network.getLinks().size()) * 100
            + "%.");
    LOG.info(
        "Surface (inferred) created: "
            + countSurfaceInferred
            + " which is "
            + ((float) countSurfaceInferred / network.getLinks().size()) * 100
            + "%.");
    //		LOG.info("Highway type created: " + countHighwayType + " which is " + ((float)
    // countHighwayType / network.getLinks().size()) * 100 + "%.");
    LOG.info(
        "Cycleway type created: "
            + countCyclewayType
            + " which is "
            + ((float) countCyclewayType / network.getLinks().size()) * 100
            + "%.");
    LOG.info(
        "Bicyle created: "
            + countBicycle
            + " which is "
            + ((float) countBicycle / network.getLinks().size()) * 100
            + "%.");
  }
  ...
  protected void setOrModifyLinkAttributes(Link l, OsmWay way, boolean forwardDirection) {
    ...
    // Allow bicycles on all infrastructures except motorways (hierarchy 1) and trunk roads
    // (hierarchy 2)
    //		if (defaults.hierarchy == 3 || defaults.hierarchy == 4 || defaults.hierarchy == 5 ||
    // defaults.hierarchy == 6
    ...
    // Allow cars if the direction under consideration is open to cars and if we are not on an
    // infrastructure mainly intended
    ...
    if (this.elevationDataParser != null) {
      // Gradient
      //		double gradient = (l.getToNode().getCoord().getZ() - l.getFromNode().getCoord().getZ()) /
      // l.getLength();
      //		l.getAttributes().putAttribute(BicycleUtils.GRADIENT, gradient);

      // Elevation
      double averageElevation =
          (l.getToNode().getCoord().getZ() + l.getFromNode().getCoord().getZ()) / 2;
    ...
    if (surface != null) {
      ...
      int maxWarnCount = 5;
      if (highwayType != null) {
        // it used to be '&&' instead of '||' which will always be false. Most likely, it must be
        // '||'. Amit Feb'18
  ...
  private boolean oppositeDirectionAllowedForBicycles(OsmWay way) {
    ...
    if (onewayTagBicycle != null) {
      if ("no"
          .equals(onewayTagBicycle)) { // This means cyclists are allowed to travel in the opposite
        // direction
    ...
    String cyclewayType = way.tags.get(BicycleUtils.CYCLEWAY);
    if (cyclewayType != null) {
      if ("opposite".equals(cyclewayType)
          || "opposite_track".equals(cyclewayType)
          || "opposite_lane".equals(cyclewayType)) {
        reverseDirectionAllowedForBicycles = true;
      }
    }
