 ...
 * @author dziemke
 */
class BicycleScoreEventsCreator
    implements
  ...
  // yyyy The car interaction is still somewhat primitive -- a vehicle leaving a link gets a penalty
  // that is multiplied with the number of cars that
  // are on the link at that moment.  Evidently, this could be improved, by counting the cars that
  // actually overtake the bicycle.  Not very difficult
  // ...
  ...
  private final Map<String, Map<Id<Link>, Double>> numberOfVehiclesOnLinkByMode =
      new LinkedHashMap<>();
  private final BicycleConfigGroup bicycleConfig;

  @Inject
  BicycleScoreEventsCreator(
      Scenario scenario,
  ...
  public void handleEvent(LinkLeaveEvent event) {
    ...
    if (vehicle2driver.getDriverOfVehicle(event.getVehicleId()) != null) {
      double amount =
          additionalBicycleLinkScore.computeLinkBasedScore(
              network.getLinks().get(event.getLinkId()));

      if (this.bicycleConfig.isMotorizedInteraction()) {
        // yyyy this is the place where instead a data structure would need to be build that counts
        // interaction with every car
      ...
      this.eventsManager.processEvent(
          new PersonScoreEvent(
              event.getTime(), driverOfVehicle, amount, "bicycleAdditionalLinkScore"));
    } else {
      log.warn(
          "no driver found for vehicleId="
              + event.getVehicleId()
              + "; not clear why this could happen");
    }
  }
  ...
  public void handleEvent(VehicleLeavesTrafficEvent event) {
    ...
    if (vehicle2driver.getDriverOfVehicle(event.getVehicleId()) != null) {
      if (!Objects.equals(this.firstLinkIdMap.get(event.getVehicleId()), event.getLinkId())) {
        // what is this good for?  maybe that bicycles that enter and leave on the same link should
        // not receive the additional score?  kai, jul'23

        // yyyy in the link based scoring, it actually uses event.getReleativePositionOnLink.  Good
        // idea!  kai, jul'23
