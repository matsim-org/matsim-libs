...
public class RunBicycleExample {
  ...
  public static void main(String[] args) {
    ...
    if (args.length == 1) {
      ...
      LOG.info(
          "No config.xml file was provided. Using 'standard' example files given in this contrib's resources folder.");
      // Setting the context like this works when the data is stored under
      // "/matsim/contribs/bicycle/src/main/resources/bicycle_example"
      ...
      throw new RuntimeException(
          "More than one argument was provided. There is no procedure for this situation. Thus aborting!"
  ...
  static void fillConfigWithBicycleStandardValues(Config config) {
    ...
    bicycleConfigGroup.setMarginalUtilityOfUserDefinedNetworkAttribute_m(
        -0.0000); // always needs to be negative
    bicycleConfigGroup.setUserDefinedNetworkAttributeName(
        "quietness"); // needs to be defined as a value from 0 to 1, 1 being best, 0 being worst
    bicycleConfigGroup.setUserDefinedNetworkAttributeDefaultValue(
        0.1); // used for those links that do not have a value for the user-defined attribute
    ...
    config
        .replanning()
        .addStrategySettings(
            new StrategySettings().setStrategyName("ChangeExpBeta").setWeight(0.8));
    config
        .replanning()
        .addStrategySettings(new StrategySettings().setStrategyName("ReRoute").setWeight(0.2));
  ...
  private static class MyAdditionalBicycleLinkScore implements AdditionalBicycleLinkScore {
    ...
    @Inject
    MyAdditionalBicycleLinkScore(Scenario scenario) {
    ...
    public double computeLinkBasedScore(Link link) {
      double result =
          (double) link.getAttributes().getAttribute("carFreeStatus"); // from zero to one
