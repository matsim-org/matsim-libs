 ...
 * @author dziemke
 */
public class WOBCommuterFileReaderV2 {
	private static final Logger LOG = Logger.getLogger(WOBCommuterFileReaderV2.class);
	
	private Map<String, Map<String, CommuterRelationV2>> relationsMap = new HashMap<>();
			
	
	public WOBCommuterFileReaderV2(String commuterFileOutgoing, String delimiter) {
	...
	private void readFile(final String filename, String delimiter) {
		...
		new TabularFileParser().parse(tabFileParserConfig, new TabularFileHandler() {
            ...
            public void startRow(String[] row) {
            	if (row.length > 2) {
            		if (row[0].length() == 12) { // new origin
            			origin = row[0];
            			LOG.info("New origin set to: " + origin);
            			return;
            			// Next check for destinations
            		} else if (row[2].length() == 8 || row[2].length() == 7) { // new destiantion
            			destination = row[2];
            			if (row[2].length() == 7){
