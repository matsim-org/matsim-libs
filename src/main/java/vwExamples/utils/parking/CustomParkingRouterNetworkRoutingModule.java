...
/**
 * This wraps a "computer science" {@link LeastCostPathCalculator}, which routes
 * from a node to another node, into something that routes from a
 * {@link Facility} to another {@link Facility}, as we need in MATSim.
 *
...
public final class CustomParkingRouterNetworkRoutingModule implements RoutingModule {
    ...
    private final class AccessEgressStageActivityTypes implements StageActivityTypes {
        ...
        public boolean isStageActivity(String activityType) {
            if (CustomParkingRouterNetworkRoutingModule.this.stageActivityType.equals(activityType)
                    || CustomParkingRouterNetworkRoutingModule.this.parkingStageActivityType.equals(activityType)) {
                return true;
            } else {
                return false;
            }
        ...
        public boolean equals(Object obj) {
            ...
            return other.isStageActivity(CustomParkingRouterNetworkRoutingModule.this.stageActivityType);
        }

        @Override
        public int hashCode() {
            return CustomParkingRouterNetworkRoutingModule.this.stageActivityType.hashCode();// TODO update this too
        }
    }
    ...
    public CustomParkingRouterNetworkRoutingModule(final String mode, final PopulationFactory populationFactory,
                                                   final Network network, final LeastCostPathCalculator routeAlgo, PlansCalcRouteConfigGroup calcRouteConfig,
    ...
    @Override
    public synchronized List<? extends PlanElement> calcRoute(final Facility fromFacility, final Facility toFacility,
                                                              final double departureTime, final Person person) {
        // I need this "synchronized" since I want mobsim agents to be able to call this
        // during the mobsim. So when the
        // mobsim is multi-threaded, multiple agents might call this here at the same
        // time. kai, nov'17
        ...
        if (personToParkLink.containsKey(person.getId())) {
            ...
            // log.warn("Person " + person.getId() + " is walking to " +
            // accessActLink.getId() + " to get the car.");
            // increase the parking capacity counter
            this.zoneToLinks.getParkingZone(accessActLink).updateLinkParkingCapacity(accessActLink, +1);// can be
            // dependent on
            // PCU of the
            // vehicle
        }
        ...
        // === access:
        {
            if (fromFacility.getCoord() != null) { // otherwise the trip starts directly on the link; no need to
                // bushwhack

                Coord accessActCoord = accessActLink.getToNode().getCoord();
                // yyyy think about better solution: this may generate long walks along the
                // link.
                ...
                now += routeBushwhackingLeg(person, accessLeg, fromFacility.getCoord(), accessActCoord, now,
                        accessActLink.getId(), accessActLink.getId(), this.populationFactory);
        ...
        // === compute another network leg (from destination of above link to another
        // nearest link
        {
            Leg newLeg = this.populationFactory.createLeg(this.mode);
            newLeg.setDepartureTime(now);
            ...
            Link parkEndLink = getNClosesedParkLink(egressActLink);
            if (parkEndLink == null) {
                // vehicle is outside the study area or has garage
            } else {
                // log.warn("Person " + person.getId() + " started looking for parking on link "
                // + egressActLink.getId() + " and found parking on link " + parkEndLink
                // .getId());
                ...
                final Activity interactionActivity = PopulationUtils.createActivityFromCoordAndLinkId(
                        this.parkingStageActivityType, egressActLink.getToNode().getCoord(), egressActLink.getId());
                ...
                this.zoneToLinks.getParkingZone(parkEndLink).updateLinkParkingCapacity(parkEndLink, -1);// can be
                // dependent on
                // PCU of the
                // vehicle type
            }
        ...
        // === egress:
        {
            if (toFacility.getCoord() != null) { // otherwise the trip ends directly on the link; no need to bushwhack
                ...
                now += routeBushwhackingLeg(person, egressLeg, egressActCoord, toFacility.getCoord(), now,
                        egressActLink.getId(), decideOnLink(toFacility).getId(), this.populationFactory);
...
//	}
    ...
    private Link getNClosesedParkLink(Link egressActLink) {
        int n = 5;
    ...
    private Link decideOnLink(final Facility fromFacility) {
        ...
        if (fromFacility.getLinkId() != null) {
            ...
            // i.e. if street address is in mode-specific subnetwork, I just use that, and
            // do not search for another (possibly closer)
            // other link.

        }

        if (accessActLink == null) {
            // this is the case where the postal address link is NOT in the subnetwork, i.e.
            // does NOT serve the desired mode,
    ...
    private Activity createInteractionActivity(final Coord interactionCoord, final Id<Link> interactionLink) {
        Activity act = PopulationUtils.createActivityFromCoordAndLinkId(this.stageActivityType, interactionCoord,
                interactionLink);
    ...
    private static double routeBushwhackingLeg(Person person, Leg leg, Coord fromCoord, Coord toCoord, double depTime,
        ...
        // I don't think that it makes sense to use a RoutingModule for this, since that
        // again makes assumptions about how to
        // map facilities, and if you follow through to the teleportation routers one
        // even finds activity wrappers, which is yet another
        // complication which I certainly don't want here. kai, dec'15

        // dpLinkId, arLinkId need to be in Route for lots of code to function. So I am
        // essentially putting in the "street address"
        // for completeness. Note that if we are walking to a parked car, this can be
        // different from the car link id!! kai, dec'15
    ...
    /* package (Tests) */ double routeLeg(Person person, Leg leg, Link fromLink, Link toLink, double depTime) {
        ...
        if (toLink != fromLink) { // (a "true" route)
            ...
            if (path == null)
                throw new RuntimeException(
                        "No route found from node " + startNode.getId() + " to node " + endNode.getId() + ".");

            NetworkRoute route = this.populationFactory.getRouteFactories().createRoute(NetworkRoute.class,
            ...
            // note that we still do a route: someone may drive from one location to another
            // on the link. kai, dec'15
            NetworkRoute route = this.populationFactory.getRouteFactories().createRoute(NetworkRoute.class,
                    fromLink.getId(), toLink.getId());
