...
public class modalSplitTripsWithin {
	...
	static String inFileName = "D:\\Matsim\\Axer\\Hannover\\ZIM\\input\\plans\\w251_inOutDRT.xml.gz";
	static String OutFileName = "D:\\Matsim\\Axer\\Hannover\\ZIM\\input\\plans\\w251_inOutWithDRT.xml.gz";
				...
				public void run(Person person) {
					...
					if (e.getValue().isValidPerson(person)) {
						...
						for (Subtour subTour : TripStructureUtils.getSubtours(plan, new HashSet<>(Arrays.asList(ParkingRouterNetworkRoutingModule.parkingStageActivityType)))) {
							...
							if (desiredModalShiftRatesMap.containsKey(SubtourMode) && subTourInServiceArea) {
