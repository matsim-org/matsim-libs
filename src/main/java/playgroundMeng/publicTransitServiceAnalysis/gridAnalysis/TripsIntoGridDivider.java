...
public class TripsIntoGridDivider {
			...
			if(ptAccessabilityConfig.getConsideredModes() != null) {
				for(String string : ptAccessabilityConfig.getConsideredModes()) {
					if(trip.getModes().contains(string)) {
						addTrip2Grid(gridImp, ptAccessabilityConfig, gf, trip);
					}
				}
			} else {
				addTrip2Grid(gridImp, ptAccessabilityConfig, gf, trip);
	...
	private static void addTrip2Grid(GridImp gridImp, PtAccessabilityConfig ptAccessabilityConfig, GeometryFactory gf,
			Trip trip) {
