...
public class AllActivitiesAnalysis implements ActivitiesAnalysisInterface{
	
	private static final Logger logger = Logger.getLogger(AllActivitiesAnalysis.class);
	Network network;
	AreaSplit areaSplit;
		...
		if(this.areaSplit instanceof DistrictBasedSplit) {
			...
			for(ActivityImp activityImp : activities) {
				for(String string: ((GridBasedSplit) areaSplit).getNum2Polygon().keySet()) {
					...
					geometry = ((GridBasedSplit) areaSplit).getNum2Polygon().get(string);
					boolean bo = geometry.contains(gf.createPoint(new Coordinate(activityImp.getCoord().getX(),activityImp.getCoord().getY())));
					if(bo){
