...
public class EndActivitiesAnalysis implements ActivitiesAnalysisInterface {
	private static final Logger logger = Logger.getLogger(EndActivitiesAnalysis.class);
	Network network;
	AreaSplit areaSplit;
	...
	@Inject
	public EndActivitiesAnalysis(Network network, PtAccessabilityConfig ptAccessabilityConfig, AreaSplit areaSplit){
	...
	public void run() {
		...
		for(Id<Person> personId: personId2Trips.keySet()) {
			for(Trip trip: personId2Trips.get(personId)) {
				activities.add(trip.getActivityEndImp());
			}
		}
