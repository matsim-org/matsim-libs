...
public class MergeInputAttributes {
	...
	public void run(String personsFile, String attributesPersonFile, String outputpath) {
		...
		for (Person p : scenPop.getPopulation().getPersons().values()) {
			...
			if (pAtt != null) {
						...
						for (PlanElement pe : plan.getPlanElements()) {
							if (pe instanceof Activity) {
								...
								if (act.getType().startsWith("work")) {
									...
									// If the student has a work activity, the plan can be substituted with the
									// education activity
									isValidStudent = true;
				...
				if (genderBit == 0)
					PersonUtils.setSex(p, "male");
				else
					PersonUtils.setSex(p, "female");
	...
	public Person makeStudent(Person inputPerson) {
		...
		// Initialize activities, coordinates will be refined in
		// ReassignZOnebyAttractiveness
		...
		startHomeAct.setEndTime(7 * 3600 + r * 0.5 * 3600);
		educationAct.setEndTime(15 * 3600 + r * 2.5 * 3600);
		leisureAct.setEndTime(17.5 * 3600 + r*3*3600);
		otherAct.setEndTime(17.5 * 3600 + r*3*3600);
		shoppingAct.setEndTime(17.5 * 3600 + r*3*3600);
		...
		double selectPlanProb = randomSelection.nextFloat();
		if (selectPlanProb < 0.25) {
			inputPerson.addPlan(eduPlan1);
		} else if (selectPlanProb >= 0.25 && selectPlanProb < 0.5) {
			inputPerson.addPlan(eduPlan2);
		}

		else if (selectPlanProb >= 0.5 && selectPlanProb < 0.75) {
