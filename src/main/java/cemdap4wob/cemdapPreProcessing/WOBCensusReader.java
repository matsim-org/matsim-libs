 ...
 * @author dziemke
 */
public class WOBCensusReader {
	private static final Logger LOG = Logger.getLogger(WOBCensusReader.class);
	
	private ObjectAttributes municipalities = new ObjectAttributes();
	private List<String> municipalitiesList = new ArrayList<>();
			
	
	public WOBCensusReader(String filePath, String delimiter) {
...
//        			municipalities.putAttribute(municipality, "seniorsFemale", seniorsFemale);
        			...
        			if (row.length > 154) { // Note: Length of row has to be considered here
        			...
        			Integer studying;
        			if (row.length > 193) { // Note: Length of row has to be considered here
