...
public class RunDrtScenarioBatchBS_10pct {
...
//		List<Integer> setMaxTravelTimeBetaList = Arrays.asList(500);
		...
		//for (Double demandScenario : demandScenarios){
			...
			final Config config = ConfigUtils.loadConfig("C:\\Temp\\BS_DRT\\input\\config.xml",new DrtConfigGroup(), new DvrpConfigGroup(), new OTFVisConfigGroup());
			...
			drt.setTransitStopFile("../input/virtualstops/stopsGrid_300m.xml");
			...
			config.controler().setOutputDirectory("C:\\Temp\\BS_DRT\\output\\"+runId); //Define dynamically the the output path
