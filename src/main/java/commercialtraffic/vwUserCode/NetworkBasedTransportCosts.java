 ...
 * from one location to another. It calculates these values based on a
 * {@link Network} to serve as {@link VehicleRoutingTransportCosts} in the
...
public class NetworkBasedTransportCosts implements VehicleRoutingTransportCosts {
	 ...
	 * <p>
	 * Note that it uses
	 * <code>vehicleType.getVehicleCostInformation().perDistanceUnit</code> and
	 * <code>vehicleType.getVehicleCostInformation().perTimeUnit</code> to calculate
	 * time and distance related transport costs.
	 * 
	 * @author stefan
	 *
	 */
	...
	static class VehicleTransportCostsIncludingToll implements TravelDisutility {
		...
		// Logger.getLogger(VehicleTransportCostsIncludingToll.class);
		...
		public VehicleTransportCostsIncludingToll(TravelDisutility baseTransportDisutility,
			...
			// System.out.println("huuuuuuuuuuuuuuuuuuuu - initialize transport costs with
			// toll");
		}
	...
	public static class Builder {
		 ...
		 * 
		 * @param {@link
		 * 			{@link LeastCostPathCalculatorFactory}
		 * @return this builder
		 */
	...
	public final Map<String, Geometry> zones;

	public final Map<Id<Link>, String> link2zone;
	...
	private NetworkBasedTransportCosts(Builder builder) {
		...
		network = networkFilter.applyFilters();
	...
	public String getZoneForLinkId(Id<Link> linkId) {
		...
		for (Entry<String, Geometry> e : zones.entrySet()) {
			...
			if (e.getValue().intersects(linkCoord)) {
				...
				// if a link Coord borders two or more cells, the allocation to a cell is
				// random.
	...
	public double getTransportTime(Location fromId, Location toId, double departureTime, Driver driver,
		...
		// Map Links to Zones in order to reduce total number of routings
		Id<Link> toLinkId = Id.create(toId.getId(), Link.class);
		Id<Link> fromLinkId = Id.create(fromId.getId(), Link.class);

		String toZone = this.getZoneForLinkId(toLinkId);
		String fromZone = this.getZoneForLinkId(fromLinkId);

		Coord toCentroid = this.getZoneCentroid(toZone);
		Coord fromCentroid = this.getZoneCentroid(fromZone);

		Link toCentroidLink = NetworkUtils.getNearestLink(network, toCentroid);
		Link fromCentroidLink = NetworkUtils.getNearestLink(network, fromCentroid);
		...
		toLocationBuilder.setCoordinate(
				Coordinate.newInstance(toCentroidLink.getCoord().getX(), toCentroidLink.getCoord().getY()));
		fromLocationBuilder.setCoordinate(
				Coordinate.newInstance(fromCentroidLink.getCoord().getX(), fromCentroidLink.getCoord().getY()));
