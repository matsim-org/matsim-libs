 ...
 */
public class OsmBastNetworkReader implements MatsimSomeReader {

    private final static Logger log = Logger.getLogger(OsmBastNetworkReader.class);

    private final static String KEY_LANES = "lanes";
    private final static String KEY_HIGHWAY = "highway";
    private final static String KEY_MAXSPEED = "maxspeed";
    private final static String KEY_JUNCTION = "junction";
    private final static String KEY_ONEWAY = "oneway";
    private final static String TAG_ACCESS = "access";
    private final static String[] ALL_TAGS_W = new String[]{KEY_LANES, KEY_HIGHWAY, KEY_MAXSPEED, KEY_JUNCTION, KEY_ONEWAY, TAG_ACCESS};

    private final static String KEY_REF_BAST = "ref:bast";
    private final static String KEY_MAN_MADE = "man_made";
    private final static String KEY_MONITORING_TRAFFIC = "monitoring:traffic";
    private final static String KEY_REF_BAST_FORWARD = "ref:bast:forward";
    private final static String KEY_REF_BAST_BACKWARD = "ref:bast:backward";
    private final static String[] ALL_TAGS_N = new String[]{KEY_REF_BAST, KEY_MAN_MADE, KEY_MONITORING_TRAFFIC, KEY_REF_BAST_FORWARD, KEY_REF_BAST_BACKWARD};

    private final Map<Long, OsmNode> nodes = new HashMap<Long, OsmNode>();
    private final Map<Long, OsmWay> ways = new HashMap<Long, OsmWay>();

    private final Map<Long, Integer> nodeId2NodeIndexBast = new HashMap<>();
     ...
     */
    public OsmBastNetworkReader(final Network network, final CoordinateTransformation transformation, Counts counts) {
        this(network, transformation, counts, true);
    }
     ...
     */
    public OsmBastNetworkReader(final Network network, final CoordinateTransformation transformation, Counts counts, final boolean useHighwayDefaults) {
        ...
        if (counts != null) {
            ...
            counts.setYear(Calendar.getInstance().get(Calendar.YEAR));
            counts.setName("BASt");
        }
...
//			this.setHighwayDefaults(6, "living_street", 1,  15.0/3.6, 1.0,  300);
    ...
    private void parse(final String osmFilename, final InputStream stream) throws UncheckedIOException {
        ...
        log.info("MATSim: # counts created: " + this.counts.getCounts().size());
    ...
    private void convert() {
        ...
        for (OsmWay way : this.ways.values()) {
            String highway = way.tags.get(KEY_HIGHWAY);
        ...
        log.info("Mark nodes as used which are refering to a BASt counting station");
        for (OsmWay way : this.ways.values()) {
            String highway = way.tags.get(KEY_HIGHWAY);
            if ((highway != null) && (this.highwayDefaults.containsKey(highway))) {
                OsmNode prevRealNode = this.nodes.get(way.nodes.get(0));
                for (int i = 1; i < way.nodes.size(); i++) {
                    OsmNode node = this.nodes.get(way.nodes.get(i));
                    if (node.used) {
                        prevRealNode = node;
                    }
                    if (node.ways == 1 && node.tags.containsKey(KEY_REF_BAST) && node.tags.containsKey(KEY_MAN_MADE) && node.tags.get(KEY_MAN_MADE).equals("monitoring_station") && node.tags.containsKey(KEY_MONITORING_TRAFFIC) && node.tags.get(KEY_MONITORING_TRAFFIC).equals("yes")) {
                        way.bastFromNode = prevRealNode.id;
                        way.bastNodeTags = node.tags;
                    }
                }
            }
        }
    ...
    private void createLink(final Network network, final OsmWay way, final OsmNode fromNode, final OsmNode toNode,
        ...
        // - check tag "junction"
        if ("roundabout".equals(way.tags.get(KEY_JUNCTION))) {
            // if "junction" is not set in tags, get() returns null and equals() evaluates to false
            oneway = true;
        }

        // check tag "oneway"
        String onewayTag = way.tags.get(KEY_ONEWAY);
        if (onewayTag != null) {
        ...
        String maxspeedTag = way.tags.get(KEY_MAXSPEED);
        if (maxspeedTag != null) {
            try {
        ...
        // check tag "lanes"
        String lanesTag = way.tags.get(KEY_LANES);
        if (lanesTag != null) {
            try {
        ...
        if (network.getNodes().get(fromId) != null && network.getNodes().get(toId) != null) {
            ...
            boolean bastForward = false;
            boolean bastBackward = false;
            String bastRefForward = null;
            String bastRefBackward = null;
            Coord bastStationCoord = null;

            String origId = Long.toString(way.id);


            if (way.bastFromNode == fromNode.id) {

                Map<String, String> tags = way.bastNodeTags;
                if (tags.containsKey(KEY_REF_BAST)) {
                    bastStationCoord = nodes.get(way.bastFromNode).coord;
                    String[] bastRefs = tags.get(KEY_REF_BAST).split(";");
                    if (bastRefs.length > 1) {
                        if (tags.containsKey(KEY_REF_BAST_FORWARD)) {
                            bastForward = true;
                            bastRefForward = tags.get(KEY_REF_BAST_FORWARD);
                        }
                        if (tags.containsKey(KEY_REF_BAST_BACKWARD)) {
                            bastBackward = true;
                            bastRefBackward = tags.get(KEY_REF_BAST_BACKWARD);
                        }
                    } else {
                        bastForward = true;
                        bastRefForward = bastRefs[0];
                    }
                }
            }
            ...
            if (!onewayReverse) {
                ...
                network.addLink(l);
                if (bastForward) {
                    Count count = counts.createAndAddCount(l.getId(), bastRefForward);
                    count.createVolume(1, 0);
                    count.setCoord(bastStationCoord);
                }
                this.id++;
            }
            if (!oneway) {
                ...
                network.addLink(l);
                if (bastBackward) {
                    Count count = counts.createAndAddCount(l.getId(), bastRefBackward);
                    count.createVolume(1, 0);
                    count.setCoord(bastStationCoord);
                }
                this.id++;
    ...
    private static class OsmWay {
        ...
        public long bastFromNode;
        public Map<String, String> bastNodeTags;
    ...
    private class OsmXmlParser extends MatsimXmlParser {
        ...
        public void startTag(final String name, final Attributes atts, final Stack<String> context) {
            if ("node".equals(name)) {
                ...
                currentNode = new OsmNode(id, this.transform.transform(new Coord(lon, lat)));
        ...
        public void endTag(final String name, final String content, final Stack<String> context) {
            if ("way".equals(name)) {
                if (!this.currentWay.nodes.isEmpty()) {
                    boolean used = false;
                    OsmHighwayDefaults osmHighwayDefaults = OsmBastNetworkReader.this.highwayDefaults.get(this.currentWay.tags.get(KEY_HIGHWAY));
                    if (osmHighwayDefaults != null) {
                        this.currentWay.hierarchy = osmHighwayDefaults.hierarchy;
                        if (OsmBastNetworkReader.this.hierarchyLayers.isEmpty()) {
                            used = true;
                        }
                        for (OsmFilter osmFilter : OsmBastNetworkReader.this.hierarchyLayers) {
