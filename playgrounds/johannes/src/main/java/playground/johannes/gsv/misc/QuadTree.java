...
public class QuadTree<T> implements Serializable {
	...
	public Collection<T> get(final double x, final double y, final double minDistance, double maxDistance) {
		return this.top.get(x, y, minDistance, maxDistance, new ArrayList<T>());
	}
	...
	protected static class Node<T> implements Serializable {
		...
		/* default */ Collection<T> get(final double x, final double y, double minDistance, final double maxDistance, final Collection<T> values) {
			if (this.hasChilds) {
				if (this.northwest.bounds.calcDistance(x, y) <= maxDistance) {
					this.northwest.get(x, y, minDistance, maxDistance, values);
				}
				if (this.northeast.bounds.calcDistance(x, y) <= maxDistance) {
					this.northeast.get(x, y, minDistance, maxDistance, values);
				}
				if (this.southeast.bounds.calcDistance(x, y) <= maxDistance) {
					this.southeast.get(x, y, minDistance, maxDistance, values);
				}
				if (this.southwest.bounds.calcDistance(x, y) <= maxDistance) {
					this.southwest.get(x, y, minDistance, maxDistance, values);
				}
				return values;
			}
			// no more childs, so we must contain the closest object
