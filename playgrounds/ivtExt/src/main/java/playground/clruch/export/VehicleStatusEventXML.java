...
class VehicleStatusEventXML extends AbstractEventXML<IdAVStatus> {
        ...
        try {
            ...
            while (e.hasNext()) {
                ...
                for (Double timeVal : status.keySet()) {
                    ...
                    event.setAttribute("edge", status.get(timeVal).idLink.toString());
...
class writeEvent<Type extends HasPersonId> {
    void writeEvent(List<String> attributes, Element node, Event event){
        Type castedEvent = (Type) event;
        for(String attr : attributes){
            if(attr.equals("person")){
                node.setAttribute(new Attribute("person",castedEvent.getPersonId().toString()));
            }
        }
    }
