...
public enum FleetReaderLogUtils {
    ...
    public static void writeRequestTrails(File logFolder, HashSet<List<RequestStatus>> requestTrails) throws IOException {
        String logFile = (logFolder + "/RequestTrails.txt");
        FileWriter fstream = new FileWriter(logFile);
        BufferedWriter out = new BufferedWriter(fstream);
        out.write("RequestTrails Hashmap:\n");
        for (List<RequestStatus> requestTrail : requestTrails) {
            out.write("\n");
            for (int i = 0; i < requestTrail.size(); i++) {
                out.write(requestTrail.get(i).tag());
    ...
    public static void countAllRequests(HashSet<List<RequestStatus>> requestTrails) {
        TreeMap<RequestStatus, Integer> occurenceMap = new TreeMap<>();
        for (RequestStatus requestStatus : RequestStatus.values()) {
            occurenceMap.put(requestStatus, 0);
        }
        for (List<RequestStatus> requestTrail : requestTrails) {
            for (RequestStatus requestStatus : requestTrail) {
                occurenceMap.put(requestStatus, occurenceMap.get(requestStatus) + 1);
            }
        }
        for (RequestStatus requestStatus : occurenceMap.keySet()) {
            System.out.println("Total amount of " + requestStatus.name() + ": " + occurenceMap.get(requestStatus));
        }
    }
