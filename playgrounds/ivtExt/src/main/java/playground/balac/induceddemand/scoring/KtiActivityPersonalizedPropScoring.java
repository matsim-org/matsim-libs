 ...
 * 
 * Added per eprson value of performing activities. balacm
 * 
 * @author thibautd, balacm
 */
public class KtiActivityPersonalizedPropScoring implements ActivityScoring, ScoringFunctionAccumulator.ActivityScoring {
	private static final Logger logger = Logger.getLogger(KtiActivityPersonalizedPropScoring.class);
	...
	public KtiActivityPersonalizedPropScoring(final Plan plan, final ScoringParameters params, final ActivityFacilities facilities) {
		this.params = params;
	...
	protected double getPerformanceScore(final String actType, final double duration) {
		...
		if ( !this.zeroUtilityDurations.containsKey(actType) ) {
			final double zeroUtilityDuration = (typicalDuration / 3600.0) * Math.exp( -10.0 / DEFAULT_PRIORITY);
