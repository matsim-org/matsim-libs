 ...
 * @author polettif
 */
public class PTMapperConstrainedRolloverScoring extends PTMapper {
	 ...
	 * Constructor
	 */
	public PTMapperConstrainedRolloverScoring(TransitSchedule schedule) {
		super(schedule);
	}
	...
	public static void mapFromFiles(String matsimTransitScheduleFile, String networkFile, String outputScheduleFile, String outputNetworkFile) {
		...
		new PTMapperConstrainedRolloverScoring(mainSchedule).mapScheduleToNetwork(mainNetwork);
			...
			for (TransitRoute route : line.getRoutes().values()) {
				...
				if (route.getTransportMode().equals("bus")) {
					...
					for (int i = 0; i < routeStops.size()-1; i++) {
						...
						// route from i to j via k*
						for(int j = i+1;  j <= i + 2 && j < routeStops.size(); j++) {

							List<Link> linkCandidatesNext = allLinkCandidates.get(routeStops.get(j).getStopFacility());

							// add linkCandidates for each via between
							if(j == i+1) {
								for(Link linkCurrent : linkCandidatesCurrent) {
									for(Link linkNext : linkCandidatesNext) {
										...
										double w = 1800 - ptPath.getTravelTime();
										if(w<0)
											w=0;
										MapUtils.addToDouble(linkCurrent.getId(), routeLinkWeights, w, w);
										MapUtils.addToDouble(linkNext.getId(), routeLinkWeights, w, w);
									}
								}
							} else {
								List<Link> linkCandidatesVia = new ArrayList<>(allLinkCandidates.get(routeStops.get(i+1).getStopFacility()));

								for(Link linkCurrent : linkCandidatesCurrent) {
									for(Link linkNext : linkCandidatesNext) {
										for(Link linkVia : linkCandidatesVia) {
											LeastCostPathCalculator.Path leastCostPath = router.calcConstrainedPath(linkCurrent, linkVia, linkNext);
											PTPath ptPath = PTMapperUtils.createPTPath(routeStops.get(i).getStopFacility(), linkCurrent, routeStops.get(j).getStopFacility(), linkNext, leastCostPath);
											ptPath.addViaLink(linkVia);
											linkWeightCalculator.add(ptPath);
											...
											MapUtils.addToDouble(linkCurrent.getId(), routeLinkWeights, w, w);
											MapUtils.addToDouble(linkNext.getId(), routeLinkWeights, w, w);
											MapUtils.addToDouble(linkVia.getId(), routeLinkWeights, w, w);
										}
									}
								}
							}
					 ...
					 */
					Map<Id<Link>, Double> DEBUGrouteLinkWeights = linkWeightCalculator.getSimpleWeights();
