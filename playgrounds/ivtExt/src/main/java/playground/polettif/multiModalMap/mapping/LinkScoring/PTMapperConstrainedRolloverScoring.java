...
public class PTMapperConstrainedRolloverScoring extends PTMapper {
	...
	private Map<Tuple<TransitStopFacility, Id<Link>>, TransitStopFacility> refStopFacility = new HashMap<>();
	private Map<TransitStopFacility, Integer> childFacilityCounter = new HashMap<>();
	...
	public void mapScheduleToNetwork(Network networkParam) {
		...
		for (TransitStopFacility stopFacility : schedule.getFacilities().values()) {
			...
			List<Link> closestLinks = NetworkTools.findNClosestLinks(networkImpl, stopFacility.getCoord(), MAX_N_CLOSEST_LINKS, NODE_SEARCH_RADIUS);
		...
		// initiate router
		PTPathCalculator pathRouter = new PTPathCalculator(this.network);
							...
							if(j == i+1) {
								for(Link linkCurrent : linkCandidatesCurrent) {
									for(Link linkNext : linkCandidatesNext) {
										LeastCostPathCalculator.Path leastCostPath = pathRouter.calcLeastCostPath(linkCurrent.getToNode(), linkNext.getFromNode());
								...
								for(Link linkCurrent : linkCandidatesCurrent) {
									for(Link linkNext : linkCandidatesNext) {
										for(Link linkVia : linkCandidatesVia) {
											LeastCostPathCalculator.Path leastCostPath = pathRouter.calcConstrainedPath(linkCurrent, linkVia, linkNext);
											PTPath ptPath = PTMapperUtils.createPTPath(routeStops.get(i).getStopFacility(), linkCurrent, routeStops.get(j).getStopFacility(), linkNext, leastCostPath);
