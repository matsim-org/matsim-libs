...
public class KtiTravelTimeDistanceCostCalculator implements TravelDisutility {
	...
	public KtiTravelTimeDistanceCostCalculator(
		...
		this.travelCostFactor = (-cnScoringGroup.getModes().get(TransportMode.car).getMarginalUtilityOfTraveling() / 3600.0) + (cnScoringGroup.getPerforming_utils_hr() / 3600.0);

//		this.marginalUtlOfDistance = ktiConfigGroup.getDistanceCostCar()/1000.0 * cnScoringGroup.getMarginalUtlOfDistanceCar();
		this.marginalUtlOfDistance = ktiConfigGroup.getDistanceCostCar()/1000.0 * cnScoringGroup.getModes().get(TransportMode.car).getMonetaryDistanceRate()
