 ...
 *                                                                         *
 * *********************************************************************** */
package playground.agarwalamit.spatial.old;
...
public class SpatialAveragingUtils {
	...
	public SpatialAveragingUtils(double xMin, double xMax, double yMin,	double yMax, int noOfXbins, int noOfYbins, double smoothingRadius_m, String visBoundaryShapeFile, CoordinateReferenceSystem targetCRS) {
		...
		cellArea = ( (this.yMax-this.yMin)*(this.xMax-this.xMin)) / (this.noOfXbins*this.noOfYbins*1000.*1000.);
...
//			}
	...
	double[][] normalizeArray(double[][] array) {
		...
		for(int xIndex = 0; xIndex<noOfXbins; xIndex++){
			for(int yIndex = 0; yIndex<noOfYbins; yIndex++){
				normalizedArray[xIndex][yIndex] =	cellArea* array[xIndex][yIndex] / this.area_in_smoothing_circle_sqkm;
			}
		}
