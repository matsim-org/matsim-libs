...
public class SpatialAveragingDemandEmissionsBK {
	...
	private final static String runDir = "/Users/aagarwal/Desktop/ils/agarwal/siouxFalls/outputMC/run";
	private final static String runBAU = "1/";
	//	private final static String outDir = runDir+runBAU+"NoCheck4VisBoundary";
	private final static String runNumber = "4/";
	private final String netFile1 = runDir+runBAU+"/output_network.xml.gz";//"../../siouxFallsJava/output/run4/output_network.xml.gz" ;//runDirectory1 + runNumber1 + ".output_network.xml.gz";

	private final String siouxFallsShapeFile = "/Users/aagarwal/Desktop/ils4/agarwal/siouxFalls/input/shape/siouxFallsAreaPolygon.shp";//"../../detailedEval/Net/shapeFromVISUM/urbanSuburban/cityArea.shp";

	private static String configFile1 =runDir+runBAU+"/output_config.xml.gz"; //"../../siouxFallsJava/output/run4/output_config.xml";//runDirectory1 + runNumber1 + ".output_config.xml.gz";
	private final String emissionFileBAU = runDir+runBAU+"/ITERS/it.100/100.emission.events.xml.gz";//"../../siouxFallsJava/output/run4/ITERS/it.100/100.emission.events.xml.gz";//runDirectory1 + "ITERS/it." + lastIteration1 + "/" + runNumber1 + "." + lastIteration1 + ".emission.events.xml.gz";
	private final String emissionFile2 = runDir+runNumber+"/ITERS/it.100/100.emission.events.xml.gz";
	...
	//========
	final String pollutant2analyze = WarmPollutant.PM.toString();
	...
	private void run() {
		...
		if(line){
			outPathStub = runDir+runBAU+"/analysis/r/rEmissionLine"+smoothingRadius_m;//runDirectory1 + "analysis/spatialAveraging/" + runNumber1 + "." + lastIteration1;
		} else {
			outPathStub = runDir+runBAU+"/analysis/r/rEmissionPoint"+smoothingRadius_m;//runDirectory1 + "analysis/spatialAveraging/" + runNumber1 + "." + lastIteration1;
		}
		...
		if(compareToBAU){
			...
			if(line){
				outPathStub = runDir+runNumber+"/analysis/r/rEmissionWRTBAULine"+smoothingRadius_m;//runDirectory1 + "analysis/spatialAveraging/" + runNumber1 + "." + lastIteration1;
			} else {
				outPathStub = runDir+runNumber+"/analysis/r/rEmissionWRTBAUPoint"+smoothingRadius_m;//runDirectory1 + "analysis/spatialAveraging/" + runNumber1 + "." + lastIteration1;
			}
	...
	private Map<Double, double[][]> fillWeightedEmissionValues(Map<Double, Map<Id, Map<String, Double>>> time2EmissionsTotalFilledAndFiltered)  {
		...
		if(line)	{
			writer = IOUtils.getBufferedWriter(runDir+runBAU+"/analysis/errorCheckLine.txt");
		} else {
			 writer = IOUtils.getBufferedWriter(runDir+runBAU+"/analysis/errorCheckPoint.txt");
		}
...
//							sumOfAllWeights+=weightOfLinkForCell;
							error4Link=3*weightOfLinkForCell*weightOfLinkForCell-4*weightOfLinkForCell+1;
						} else{
...
//							sumOfAllWeights+=weightOfLinkForCell;
						...
						sumOfAllWeights+=error4Link*scaledValue;
					}
				}
				try {
					writer.write("sum of all normalized weights for link \t"+linkId.toString()+"\t"+endOfTimeInterval+"\t"+sumOfAllWeights+"\n");
					sumOfAllWeights=0;
				} catch (Exception e) {
