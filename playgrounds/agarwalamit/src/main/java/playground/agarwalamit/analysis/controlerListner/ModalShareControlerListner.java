...
public class ModalShareControlerListner implements StartupListener, IterationEndsListener{
	...
	private SortedMap<String, int []> mode2numberofLegs = new TreeMap<>();
	...
	public void notifyIterationEnds(IterationEndsEvent event) {
		...
		if(itNrIndex == 0) {
			for(String mode : mode2legs.keySet()) {
				if ( ! mode2numberofLegs.containsKey(mode)){ // initialize
					int [] legs = new int [this.numberOfIterations];
					legs[0] = mode2legs.get(mode);
		...
		//storeData 
		for(String mode : mode2legs.keySet()) {
			int [] legs = this.mode2numberofLegs.get(mode);
