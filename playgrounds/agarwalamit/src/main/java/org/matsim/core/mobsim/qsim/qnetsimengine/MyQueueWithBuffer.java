 ...
 * @author nagel
 */
final class MyQueueWithBuffer extends QLaneI implements SignalizeableItem {
	...
	private final Queue<MyQueueWithBuffer.Hole> holes = new LinkedList<MyQueueWithBuffer.Hole>();
	...
	static class Builder {
		...
		MyQueueWithBuffer build() {
			...
			return new MyQueueWithBuffer( qLink, vehicleQueue, id, length, effectiveNumberOfLanes, flowCapacity_s ) ;
		}
		/**
	...
	private MyQueueWithBuffer(AbstractQLink qLinkImpl,  final VehicleQ<QVehicle> vehicleQueue, Id<Lane> id, 
		...
		if(MyQueueWithBuffer.HOLES && this.network.simEngine.getMobsim().getScenario().getConfig().getModule("WITH_HOLE")!=null){
	...
	private void calculateStorageCapacity() {
		...
		if (storageCapacity < tempStorageCapacity) {
			if (MyQueueWithBuffer.spaceCapWarningCount <= 10) {
				SeepQLinkImpl.log.warn("Link " + this.id + " too small: enlarge storage capacity from: " + storageCapacity
						+ " Vehicles to: " + tempStorageCapacity + " Vehicles.  This is not fatal, but modifies the traffic flow dynamics.");
				if (MyQueueWithBuffer.spaceCapWarningCount == 10) {
					SeepQLinkImpl.log.warn("Additional warnings of this type are suppressed.");
				}
				MyQueueWithBuffer.spaceCapWarningCount++;
			}
...
//			}
	...
	public boolean doSimStep(final double now ) {
		if(MyQueueWithBuffer.HOLES) this.processArrivalOfHoles( now ) ;
...
//			}
	...
	class VisDataImpl implements VisData {
				...
				double spacing, double freespeedTraveltime, Hole veh)
		{
			...
			double distanceFromFromNode = snapshotInfoBuilder.calculateDistanceOnVectorFromFromNode2(MyQueueWithBuffer.this.length, spacing,
			...
			if (this.upstreamCoord != null){
				snapshotInfoBuilder.positionQItem(positions, this.upstreamCoord, this.downsteamCoord,
						MyQueueWithBuffer.this.length, this.euklideanDistance, veh,
				...
				snapshotInfoBuilder.positionQItem(positions, link.getFromNode().getCoord(), link.getToNode().getCoord(),
						MyQueueWithBuffer.this.length, ((LinkImpl)link).getEuklideanDistance() , veh, 
						distanceFromFromNode, lane, speedValue);
			}
