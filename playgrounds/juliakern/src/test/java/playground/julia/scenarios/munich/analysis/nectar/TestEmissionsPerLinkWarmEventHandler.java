...
public class TestEmissionsPerLinkWarmEventHandler {
	...
	public void handleEventTest(){
		...
		double time = 5.0;
		double endOfTimeInterval = simulationEndTime/noOfTimeBins*1;
		endOfTimeInterval = Math.round(endOfTimeInterval);
		...
		// test event 1 
		Assert.assertEquals(1.0, handler.getTime2linkIdLeaveCount().get(endOfTimeInterval).get(linkId));
		Assert.assertEquals(coValue, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.CO), MatsimTestUtils.EPSILON);
		Assert.assertEquals(c2Value, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.CO2_TOTAL), MatsimTestUtils.EPSILON);
		Assert.assertEquals(fcValue, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.FC), MatsimTestUtils.EPSILON);
		Assert.assertEquals(hcValue, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.HC), MatsimTestUtils.EPSILON);
		Assert.assertEquals(nmValue, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.NMHC), MatsimTestUtils.EPSILON);
		Assert.assertEquals(n2Value, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.NO2), MatsimTestUtils.EPSILON);
		Assert.assertEquals(nxValue, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.NOX), MatsimTestUtils.EPSILON);
		Assert.assertEquals(pmValue, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.PM), MatsimTestUtils.EPSILON);
		Assert.assertEquals(soValue, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.SO2), MatsimTestUtils.EPSILON);
		...
		try{
			handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(new IdImpl("unused link")).get(WarmPollutant.CO2_TOTAL);
		}catch(NullPointerException e){
			excep = true;
		}
		Assert.assertTrue(excep); excep = false;
		Assert.assertNull(handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get("some pollutant"));
		...
		Assert.assertNull(handler.getTime2linkIdLeaveCount().get(endOfTimeInterval).get(new IdImpl("unused link")));
		...
		double timeOfEvent4 = simulationEndTime/noOfTimeBins*14.3;
		...
		for(int i=0; i<noOfTimeBins; i++){
			if(i*timebinsize<timeOfEvent4 && (i+1)*timebinsize>=timeOfEvent4){
				intervalEndOfEvent4 = Math.round((i+1)*timebinsize);
				break;
			}
		}
		...
		//intervalEndOfEvent4 = 100.00000000000001;
		// check time2linkIdLeaveCount
		Assert.assertEquals(2.0, handler.getTime2linkIdLeaveCount().get(endOfTimeInterval).get(linkId));
		Assert.assertEquals(1.0, handler.getTime2linkIdLeaveCount().get(simulationEndTime).get(linkId)); 
		Assert.assertEquals(1.0, handler.getTime2linkIdLeaveCount().get(intervalEndOfEvent4).get(linkId));
		...
		Assert.assertEquals(2*coValue, handler.getWarmEmissionsPerLinkAndTimeInterval().get(endOfTimeInterval).get(linkId).get(WarmPollutant.CO), MatsimTestUtils.EPSILON);
		...
		double intervalEndOfEvent5 = -1.0;
		Id link2Id = new IdImpl("link 2");
		for(int i=0; i<noOfTimeBins; i++){
			if(i*timebinsize<timeOfEvent5 && (i+1)*timebinsize>=timeOfEvent5){
				intervalEndOfEvent5 = (i+1)*timebinsize;
				break;
			}
		}
