...
public class EmissionUtils {
	...
	public SortedMap<String, Double> sumUpEmissions(Map<WarmPollutant, Double> warmEmissions, Map<ColdPollutant, Double> coldEmissions) {
		...
		double sumOfPollutant =0.0;
	
		for(String pollutant : listOfPollutants){
			if(warmPollutant2emissions.containsKey(pollutant)) sumOfPollutant += warmPollutant2emissions.get(pollutant);
			if(coldPollutant2emissions.containsKey(pollutant)) sumOfPollutant += coldPollutant2emissions.get(pollutant);
//			//
