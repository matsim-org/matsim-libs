 ...
 *                                                                         *
 * *********************************************************************** */
package playground.julia.scenarios.munich.analysis;
...
//	private final static String runDirectory2 = "../../runs-svn/detEval/latsis/output/output_policyCase_zone30/";
	...
	private final static Integer lastIteration2 = 1000;
...
//	private final String emissionFile2 = runDirectory2 + "ITERS/it." + lastIteration2 + "/" + runNumber2 + "." + lastIteration2 + ".emission.events.xml.gz";
	...
	final int noOfTimeBins = 2; //was 1
	...
	final boolean calculateRelativeChange = true;
		...
		if(baseCaseOnly){
			...
			if(calculateRelativeChange){
				...
				time2EmissionMapToAnalyze 
					= calcualateRelativeEmissionDifferences(time2EmissionsTotalFilledAndFiltered1, time2EmissionsTotalFilledAndFiltered2);
				time2DemandMapToAnalyze 
					= calculateRelativeDemandDifferences(time2CountsPerLinkFilledAndFiltered1, time2CountsPerLinkFilledAndFiltered2);
				outPathStub = runDirectory1 + "analysis/spatialAveraging/" + runNumber2 + "." + lastIteration2 + "-" + runNumber1 + "." + lastIteration1 + ".relativeDelta";
				// TODO use these maps? maybe write r output?
				System.out.println(outPathStub);
...
//		}
		...
		if(calculateRelativeChange){
			writeOutputForRelativeChanges(time2EmissionMapToAnalyze);
			writeOutputForRelativeChangesDemand(time2DemandMapToAnalyze);
		}
...
//		writeGISoutput(time2Emissions_gPerVkm, outPathStub +  ".GISoutput." + pollutant2analyze.toString() + ".gPerVkm.movie.shp");
	...
	private void writeOutputForRelativeChangesDemand(Map<Double, Map<Id, Double>> time2DemandMapToAnalyze) {
		String[][] demandLinkMatrix = new String[time2DemandMapToAnalyze.keySet().size()+2][network.getLinks().size()];
		//TODO titelzeile
		int iterationCounter = 1;
		for(double endOfTimeInterval : time2DemandMapToAnalyze.keySet()){
			int iterationLinkcouter = 1;
			Map<Id, Double> link2value = time2DemandMapToAnalyze.get(endOfTimeInterval);
			demandLinkMatrix[iterationCounter][0]= Double.toString(endOfTimeInterval);
			for(Id linkId : link2value.keySet()){
				
				demandLinkMatrix [iterationCounter][iterationLinkcouter]= link2value.get(linkId).toString();
				iterationLinkcouter++;
			}
			iterationCounter++;
		}
	...
	private void writeOutputForRelativeChanges(Map<Double, Map<Id, Map<String, Double>>> time2EmissionMapToAnalyze) {
		
		for(double endOfTimeInterval : time2EmissionMapToAnalyze.keySet()){
			System.out.println("end of time interval"+endOfTimeInterval);
			Map<Id, Map<String, Double>> link2emission2value = time2EmissionMapToAnalyze.get(endOfTimeInterval);
			int numberOfEmissions = 10;
			String [][] emissionLinkMatrix = new String[link2emission2value.keySet().size()+1][numberOfEmissions+1];
			//TODO titelzeile
			// first row emissions... dafuer muessten die alle sortiert sein... spaeter
			int iterationCounter =1;
			for(Id linkId : link2emission2value.keySet()){
				Map<String, Double> emission2value = link2emission2value.get(linkId);
				// first column - link id
				emissionLinkMatrix [iterationCounter][0] = linkId.toString();
				
				int iterationEmCounter = 1;
				for(String emission: emission2value.keySet()){
					// fill matrix
					emissionLinkMatrix[iterationCounter][iterationEmCounter] = emission2value.get(emission).toString();
					iterationEmCounter++;
				}
				iterationCounter++;
			}
			String outputPath = outPathStub+endOfTimeInterval;
			System.out.println("should now write output to "+outputPath);
			writeRoutputLinksToEmissions(emissionLinkMatrix, outputPath);
			// write r out for matrix
		}
	}

	private void writeRoutputLinksToEmissions(String[][] emissionLinkMatrix,
			String outputPath) {
		try {
				...
				for(int i = 0; i<emissionLinkMatrix.length; i++){
					String currLine = new String();
					for (int j = 0; j<emissionLinkMatrix[i].length; j++){
						currLine = currLine+emissionLinkMatrix[i][j]+"\t";
					}
					buffW.write(currLine);
					buffW.newLine();
				}
				buffW.close();
				System.out.println("wrote output to "+ outputPath);
			} catch (IOException e) {
				e.printStackTrace();
			}
...
//			}
	...
	private void writeRoutput(double[][] results, String outputPathForR) {
		try {
			...
			FileWriter fileW = new FileWriter(outputPathForR);
