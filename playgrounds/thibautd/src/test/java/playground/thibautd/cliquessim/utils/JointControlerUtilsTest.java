 ...
 * @author thibautd
 */
public class JointControlerUtilsTest {
	@Rule
	...
	public void testJointTripsImport() throws Exception {
		...
		final Config config = JointControlerUtils.createConfig( null );
		config.plans().setInputFile( popFile );
		final Scenario loadedScenario = JointControlerUtils.createScenario( config );
