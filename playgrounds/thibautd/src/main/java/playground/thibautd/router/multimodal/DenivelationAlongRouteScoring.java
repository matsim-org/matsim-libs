...
public class DenivelationAlongRouteScoring implements BasicScoring {
	...
	// TODO: separate from scoring object (used from link disutility as well)
	...
	public double getScore() {
		...
		for ( Leg l : TripStructureUtils.getLegs( plan ) ) {
			...
			for ( Id<Link> linkId : route.getLinkIds() ) {
				score += calcGainUtil( network.getLinks().get( linkId ) );
			}

			score += calcGainUtil( network.getLinks().get( route.getEndLinkId() ) );
		}
