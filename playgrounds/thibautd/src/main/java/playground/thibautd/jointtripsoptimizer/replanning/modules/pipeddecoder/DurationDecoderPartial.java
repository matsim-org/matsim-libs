...
public class DurationDecoderPartial implements JointPlanOptimizerDimensionDecoder {
	 ...
	 */
	public DurationDecoderPartial(
			final JointPlan plan,
			final List<Id> personsToOptimise,
		...
		for (Map.Entry<Id, Plan> planEntry : plan.getIndividualPlans().entrySet()) {
			...
			if (personsToOptimise.contains(currentId)) {
				// construct the list of related genes
				for (PlanElement pe : planElements.subList(0, planElements.size() -1)) {
					...
					if ( !isPickUp(pe) && !isDropOff(pe) ) {
						if (!constrainedActivities.remove(pe)) {
							...
							// negative indices point to the "constrainded" values
							fixedTimeIndex--;
				...
				// mark the shared rides as already accessed
				double now = 0d;
		...
		this.initializeLegEstimators(
				plan,
				personsToOptimise,
				configGroup.getSimLegInterpretation());
	}

	private double[] getFixedEndTimes(
			final List<Double> fixedTimes) {
	 ...
	 */
	private List<Activity> getConstrainedActivities(
			final JointPlan plan,
			final List<Id> personsToOptimise) {
		List<Activity> out = new ArrayList<Activity>();
		Activity lastActivity = null;

		// mark activities followed by a joint trip with at least one participant
		// which is not optimised as "constrained".
		for (Id id : personsToOptimise) {
	...
	private Map<Id, PlanImpl> getPlansNonOptimisedPersons(
			final JointPlan plan,
			final List<Id> personsToOptimise) {
	...
	private Map<JointLeg, JointLeg> getAlwaysPlannedDriverLegs(
			final Map<Id, ? extends Plan> nonOptimisedPlans) {
	...
	public JointPlan decode(
		...
		Map<Id, PlanImpl> constructedIndividualPlans =
			new HashMap<Id, PlanImpl>(this.plansNonOptimisedPersons);
		...
		for (Id id : this.personsToOptimise) {
			...
			individualValuesMap.put(
					...
					new IndividualValuesWrapper(
						inputIndivPlan.getPlanElements(),
						this.genesIndices.get(id)));
		}
	...
	private class IndividualValuesWrapper {
		...
		public IndividualValuesWrapper(
				final List<PlanElement> planElements,
				final List<Integer> indicesInChromosome) {
