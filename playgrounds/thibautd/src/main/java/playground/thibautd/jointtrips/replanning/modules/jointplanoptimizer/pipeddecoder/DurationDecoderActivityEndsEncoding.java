/* *********************************************************************** *
 * project: org.matsim.*
 * DurationDecoderActivityEndsEncoding.java
 *                                                                         *
 * *********************************************************************** *
 *                                                                         *
 ...
 * @author thibautd
 */
public class DurationDecoderActivityEndsEncoding implements JointPlanOptimizerDimensionDecoder {
	private static final Logger log =
		Logger.getLogger(DurationDecoderActivityEndsEncoding.class);
	 ...
	 * @param plan the plan instance which is being optimised by the genetic process
	 * @param configGroup the config parameters for the optimisation
	 * @param legTravelTimeEstimatorFactory the factory to use to create travel time estimators
	 * @param routingAlgorithm the routing object, initialised with proper travel time estimators
	 * @param network the network
	 * @param firstGeneIndex the index of the first double gene encoding for duration
	 * @param individualIds the ids of the individuals, ordered as the double genes
	 * sequences representing their plans. While this is not checked, the collection
	 * must contain each Id one time and only one (this is not a "partial" decoder).
	 */
	public DurationDecoderActivityEndsEncoding(
		...
		int currentDurationGene = firstGeneIndex;
		Integer indexToAdd;

		// construction of the list of relative genes.
		Map<Id, List<PlanElement>> id2planElements = plan.getIndividualPlanElements();
		List<PlanElement> planElements;
		for ( Id currentId : individualIds ) {
			planElements = id2planElements.get( currentId );
			List<Integer> currentGeneIndices = new ArrayList<Integer>();
			this.genesIndices.put( currentId , currentGeneIndices );
			...
			for (PlanElement pe : planElements.subList(0, planElements.size() -1)) {
				...
				if ( !isPickUp(pe) && !isDropOff(pe) ) {
					...
					currentGeneIndices.add( indexToAdd );
				}
			}
			// "mark" the last activity as not being related to a gene
			currentGeneIndices.add( null );
		}
	...
	private final void planNextActivity(
		...
		if (individualValues.getIndexInPlan()==0) {
			// still valid for activity end encoding
	...
	private void planIndividualLegAct(
		...
		individualValues.setNow( endTime );
	...
	private void planSharedLegAct(
			...
			final Integer geneIndex,
			final IChromosome chromosome) {
		// left unchanged when passing from duration encoding to end time
		// encoding: no consideration of gene values
		// --------------------------------------------------------------------
	 ...
	 */
	private static final double getEndTime(
			final IChromosome chromosome,
			final Integer geneIndex,
			final double now,
		...
		if (geneIndex != null) {
			endTime = ((DoubleGene) chromosome.getGene(geneIndex)).doubleValue();

			if (endTime - now - travelTime < MIN_DURATION) {
				endTime = now + travelTime + MIN_DURATION;
			}
		} else {
	...
	private static class IndividualValuesWrapper {
		...
		public void setNow(final double now) {
			this.now = now;
		}
