...
//		panels.add(tabSelectPanel);
	...
	private class ModuleButtonListener implements MouseInputListener, ActionListener
	{
		...
		{
			//check if there are unsaved changes in current module
			if (this.manager.controller.getActiveModuleType()!=null)
			{
				AbstractModule module = this.manager.controller.getModuleByType(this.manager.controller.getActiveModuleType());
				if (module.hasUnsavedChanges())
				{
					int answer = JOptionPane.showOptionDialog(controller.getParentComponent(), controller.getLocale().msgUnsavedChanges(), null, JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE, null, null, null);
					if (answer == 0)
						module.saveChanges();
				}
			}
