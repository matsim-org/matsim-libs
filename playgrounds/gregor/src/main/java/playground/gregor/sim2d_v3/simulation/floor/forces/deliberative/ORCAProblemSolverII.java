...
//		GuiDebugger.addVector(new float []{0, 0, ret[0], ret[1],255,0,0});
	...
	private boolean findNewOptimalSolution(List<ORCALine> constraints, int idx,
		...
		float c1 = ORCALineAgent.dot(wx,wy,c.getDirectionX(),c.getDirectionY());//TODO put static methods to a more central class like Algorithms.java
		float c2 = ORCALineAgent.dot(c.getDirectionX(),c.getDirectionY(),c.getDirectionX(),c.getDirectionY());
		float b = c1 / c2;
		...
		boolean circleCenterValid = c.solutionSatisfiesConstraint(0.f,0.f);
		...
		for (int i = 0; i < idx; i++) {
			...
			float cosDirection = ORCALineAgent.dot(c.getDirectionX(), c.getDirectionY(), tmp.getDirectionX(), tmp.getDirectionY());

			if (Math.abs(cosDirection) >= ALMOST_ONE) { //collinear
				if (cosDirection < 0) { //opposite direction
					if (c.solutionSatisfiesConstraint(tmp.getPointX(), tmp.getPointY())) {
						continue;
					} else {
						//failed
						return false;
					...
					if (tmp.solutionSatisfiesConstraint(c.getPointX(), c.getPointY())) {
						continue;
					} else {
						//failed
						return false;
