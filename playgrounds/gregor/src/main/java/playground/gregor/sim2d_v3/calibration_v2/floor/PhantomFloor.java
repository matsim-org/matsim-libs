package playground.gregor.sim2d_v3.calibration_v2.floor;
...
import playground.gregor.sim2d_v3.calibration_v2.LLCalculator;
import playground.gregor.sim2d_v3.calibration_v2.PhantomAgent2D;
...
public class PhantomFloor extends PhysicalFloor {
	...
	public PhantomFloor(PhantomEvents phantomEvents, Id calibrationAgentId, Collection<? extends Link> collection, Scenario scenario, LLCalculator llCalc, EventsManager em) {
		super(scenario,em,true,true,null,null);
	...
	protected void updateForces(double time) {
		this.getForceUpdater().updateDynamicForces(time);
		
		if (this.calibrationAgent != null) {
			this.getForceUpdater().updateForces(this.calibrationAgent, time);
		}
	}

	private void removeAgent(Id id) {
		AgentInfo pa = this.agentsMap.remove(id);
		this.getAgentsMover().removeAgent(pa.agent);
	...
	private void addAgent(Id id) {
		...
		ai.agent = pa;
		ai.lastUpdate = -1;
		this.getAgentsMover().addAgent(pa);
