...
import playground.gregor.sim2d_v3.simulation.floor.forces.deliberative.velocityobstacle.CircularVelocityObstacle;
import playground.gregor.sim2d_v3.simulation.floor.forces.deliberative.velocityobstacle.ConfigurationSpaceObstacle;
import playground.gregor.sim2d_v3.simulation.floor.forces.deliberative.velocityobstacle.PolygonalVelocityObstacle;
import playground.gregor.sim2d_v3.simulation.floor.forces.deliberative.velocityobstacle.VelocityObstacle;
	...
	private boolean calcOtherAgentsVOs(List<VelocityObstacle> vOs,
		...
		for (Agent2D other : l) {
			...
			if (d <= r0+r1) {
				...
				if (this.debug && (agent.getDelegate().getId().toString().equals("r56") || agent.getDelegate().getId().toString().equals("g48"))) {
					colls++;
