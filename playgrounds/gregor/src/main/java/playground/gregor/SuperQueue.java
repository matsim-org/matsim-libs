...
public class SuperQueue {
	...
	public static void main(String [] args) {
		...
		double lengthInMeter = forwardPedestrianSpeed*timeStepSizeInSeconds*2;
		double flowCapacity = forwardPedestrianSpeed * rhoMaxInPersonsPerMeter /20; 
		...
		for (double timeStep = 0; timeStep < 100; timeStep++) {
			for (int i = 0; i < rho1.length; i++) {
				...
				delta2[i] = Math.min(forwardPedestrianSpeed*rho2[i],flowCapacity);
				delta1[i] = Math.min(forwardPedestrianSpeed*rho1[i],flowCapacity);
				
				
				double space = (rhoMaxInPersonsPerMeter -rho1[i] - rho2[i])* lengthInMeter/(2*timeStepSizeInSeconds);
				sigma1[i] = Math.min(space, Math.max(1e-1 * flowCapacity, rhoMaxInPersonsPerMeter / rho2[i] * (1/conflictDelay - delta2[i])));
				sigma2[i] = Math.min(space, Math.max(1e-1 * flowCapacity, rhoMaxInPersonsPerMeter / rho1[i] * (1/conflictDelay - delta1[i])));
...
//				sigma1[i] = sigma1[i] < 0 ? 0 : sigma1[i];
	//			sigma2[i] = sigma2[i] < 0 ? 0 : sigma2[i];
			...
			for (int i = 0; i < rho1.length-1; i++) {
				double out1 = outflow1[i] * timeStepSizeInSeconds/lengthInMeter;
				rho1[i] -= out1;
				rho1[i+1] += out1;
				
				double out2 = outflow2[i+1] * timeStepSizeInSeconds/lengthInMeter;
				rho2[i+1] -= out2;
				rho2[i] += out2;
			}			
