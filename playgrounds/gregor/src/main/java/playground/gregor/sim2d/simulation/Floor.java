...
public class Floor {
	...
	public Floor(Map<Link,LineString> finishLines, Map<Link,LineString> linksGeos, Map<Link,Coordinate> drivingDirections,  SegmentedStaticForceField sff) {
		...
		//		this.agentsQuad = new QuadTree<Agent2D>(e.getMinX()-400,e.getMinY()-400,e.getMaxX()+400,e.getMaxY()+400); 
...
//			}
	...
	private void updateAgentEnvForce(Agent2D agent, Force force) {
		...
		//		Collection<Force> fs = this.staticForceField.getForcesWithin(agent.getPosition(), 1.);
		//		if (fs.size() > 0) {
		//			for (Force f : fs) {
		//				x += f.getFx();
		//				y += f.getFy();
...
//			double contrariness = getContrariness(force,length,x,y);
			...
			double exp = Math.exp(Sim2DConfig.Bp/length)/length; // * contrariness/length;
			x *= exp;
			y *= exp;
	...
	private double getContrariness(Force force, double length, double x,
			double y) {
		if (length > 0.45) {
			...
			//				log.info("oldFx/Fy:" + force.getOldFx() + "/" + force.getOldFy() + "  x/y:" + x + "/" + y + "  contr:" + contrariness);
			//			}
		} else {
			return 1;
		}
