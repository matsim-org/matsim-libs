...
public class StaticForceFieldGenerator {
	...
	// QuadTree<ForcePoint> tmpTree = new
	// QuadTree<ForcePoint>(e.getMinX(),e.getMinY(),e.getMaxX(),e.getMaxY());
	// QuadTree<Force> q = new
	// QuadTree<Force>(e.getMinX(),e.getMinY(),e.getMaxX(),e.getMaxY());
	// this.ret = new StaticForceField(q);
	// int numOfGeos = 0;
	// for (double x = e.getMinX(); x <= e.getMaxX(); x +=
	// Sim2DConfig.STATIC_FORCE_RESOLUTION) {
	// for (double y = e.getMinY(); y <= e.getMaxY(); y +=
	// Sim2DConfig.STATIC_FORCE_RESOLUTION) {
	// // this.structure.distance(g)
	// DistanceOp op = new DistanceOp(this.structure,geofac.createPoint(new
	// Coordinate(x,y)));
	...
	// LineString ls = this.geofac.createLineString(new Coordinate[]{new
	// Coordinate(fp1.x,fp1.y),new Coordinate(fp2.x,fp2.y)});
