...
public class StaticForceFieldWriter extends MatsimXmlWriter {
	...
	public void write(String file, StaticForceField sff) {
		try {
			...
			for (ForceLocation force : sff.getForces()) {
				writeForce(force);
				// this.writer.write(NL);
				// this.writer.write(NL);
			}
	...
	private void writeForce(ForceLocation force) throws IOException {
		...
		this.writer.write(EQUALS);
		this.writer.write(QUOTE);
		this.writer.write(Double.toString(force.getLocation().x));
		this.writer.write(QUOTE);
		this.writer.write(WHITESPACE);
		this.writer.write(Y_COORD_TAG);
		this.writer.write(EQUALS);
		this.writer.write(QUOTE);
		this.writer.write(Double.toString(force.getLocation().y));
		this.writer.write(QUOTE);
		this.writer.write(WHITESPACE);
		this.writer.write(FORCE_X_TAG);
		this.writer.write(EQUALS);
		this.writer.write(QUOTE);
		this.writer.write(Double.toString(force.getForce().getXComponent()));
		this.writer.write(QUOTE);
		this.writer.write(WHITESPACE);
		this.writer.write(FORCE_Y_TAG);
		this.writer.write(EQUALS);
		this.writer.write(QUOTE);
		this.writer.write(Double.toString(force.getForce().getYComponent()));
		this.writer.write(QUOTE);
