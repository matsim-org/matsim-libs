/* *********************************************************************** *
 * project: org.matsim.*
 * ShelterLocationRePlannerII.java
 *                                                                         *
 * *********************************************************************** *
 *                                                                         *
...
public class ShelterLocationRePlannerII implements IterationStartsListener {
	...
	private final double negDeltaTSum = 0;
	// private double posDeltaTSum = 0;
	// private double posCapSum = 0;

	private final Map<Id, ArrayList<PersonInfo>> asm = new HashMap<Id, ArrayList<PersonInfo>>();
	...
	public ShelterLocationRePlannerII(ScenarioImpl sc, TravelCost tc, TravelTime tt, List<Building> buildings, ShelterCounter shc) { // ,
		// double
		// pshelter)
		// {
		...
		this.dieOutTreshold = (int) (Math.max(5, ((EvacuationConfigGroup) sc.getConfig().getModule("evacuation")).getSampleSize() * this.dieOutTreshold) + .5);
	}
	...
	private void initShelters(List<Building> buildings) {
		for (Building b : buildings) {
			if (b.getShelterSpace() > 0 && b.isQuakeProof() && !b.getId().toString().equals("super_shelter")) {
				...
				// si.reserveCapacity = (int) (Math.max(1,
				// this.sc.getConfig().evacuation().getSampleSize() *
				// RESERVE_CAP)+.5);
