...
//	private TravelCost tc;
	...
	private List<Entry<Id, Shelter>> shelterList = new ArrayList<Entry<Id,Shelter>>();
	
	//simulate annealing related stuff begins here
	double L_k = 0; //number of transitions in current iteration
	double c_k = 0; //current temperature
	
	// 
	private HashMap<Id, Shelter> shelterLinkMapping;

	private Dijkstra router;

	private PopulationFactory popFac;
...
//	private final double L_decrement;
	...
	private final double chi_0 = 0.8;
	private final double epsilon_s = Math.pow(10, -5);
	...
	private List<Double> fTransitions = new ArrayList<Double>();
...
//		this.swNash = true;
	 ...
	 * @throws RuntimeException if more agents in shelter as shelter has space
	 */
	private void validateShelterUtilization() {
		...
		for (Person pers : this.agents) {
			...
			t.count++;
			if (t.count > t.cap) {
				throw new RuntimeException("More agents in shelter than allowed");
			}
		}
	...
	public void notifyIterationStarts(IterationStartsEvent event) {
		if (this.melted ) {
			...
			double denom = 1+ (c_k * Math.log(1+delta))/(3.*sigma_k);
			
			this.c_k = c_k / denom;
			this.L_k = (int) (0.1 * this.agents.size());//TODO does this make sense
			run();
			validateShelterUtilization();
			...
			double chi = (this.m1 + this.m2 * Math.exp(-this.deltaFDecrSum/(this.m2*this.c_k)))/(this.m1+this.m2);
			if (chi >= this.chi_0) {
				this.melted = true;
				log.info("System is melted! Initial control parameter c_0:" + this.c_k);
			} else {
				this.c_k = (this.deltaFDecrSum/this.m2)/Math.log(this.m2/(this.m2*this.chi_0-this.m1*(1-this.chi_0)));
	...
	private void run() {
		...
		for (Person  p : this.agents) {
			this.fBefore += p.getSelectedPlan().getScore();
		}
		...
		for (int i = 0; i < this.L_k; i++) {
			...
			Person p = this.agents.get(idx);
			generateAndTest((PersonImpl) p);
		}
	 ...
	 * @param p
	 */
	private void generateAndTest(PersonImpl p) {
		if (MatsimRandom.getRandom().nextBoolean()) {
			if (!generateAndTestShift(p)){
				//NULL shifts not permitted so we have to switch
				generateAndTestSwitch(p);
			}
	 ...
	 * @param p
	 */
	private void generateAndTestSwitch(PersonImpl p1) {
		...
		double f1_current = plan1.getScore();
		
//		Leg legg = (Leg) plan1.getPlanElements().get(1);
//		
//		double test = legg.getTravelTime()/-600;
//		System.out.println("score: " + f1_current + " time:" + test);
		...
		double f2_current = plan2.getScore();
		...
		double f1_after = path1.travelTime*t/3600.;
		double f2_after = path2.travelTime*t/3600.;
		
		double prob = 0; //probability to switch
		
		double deltaF = 0;
		boolean decrMv = false;
		
		if (this.swNash) {
			//in order to switch according to Nash constraints both agents have to accept
			if (f1_after > f1_current && f2_after > f2_current) {
				prob = 1;
				
			} else {
				double prob1 = Math.exp((f1_after - f1_current)/this.c_k);
				double prob2 = Math.exp((f2_after - f2_current)/this.c_k);
				prob = prob1*prob2;
				decrMv = true;
				
			}
			deltaF = (-f1_current - f2_current) - (-f2_after - f2_current);
		} else {
			//in order to switch according to SO constraints the agents have to accept on average
			double contrib_current = f1_current + f2_current; //current contribution to system costs
			double contrib_after = f1_after + f2_after; //current contribution to system costs
			if (contrib_after > contrib_current) {
				prob = 1;
			} else {
				prob = Math.exp((contrib_after - contrib_current)/this.c_k);
				decrMv = true;
		...
		if (switchAgents) {
			...
			Leg leg1 = this.popFac.createLeg("car");
			leg1.setDepartureTime(actOrigin1.getEndTime());
			...
			leg1.setTravelTime(path1.travelTime);
			newPlan1.addLeg(leg1);
			...
			newPlan1.setScore(f1_after);
			
			p1.addPlan(newPlan1);
			((PersonImpl)p1).setSelectedPlan(newPlan1);
			((PersonImpl)p1).removeUnselectedPlans();			
			
			//person 2
			Plan newPlan2 = this.popFac.createPlan();
			newPlan2.addActivity(actOrigin2);
			
			Leg leg2 = this.popFac.createLeg("car");
			leg2.setDepartureTime(actOrigin2.getEndTime());
			...
			leg2.setTravelTime(path2.travelTime);
			newPlan2.addLeg(leg2);
	...
	private boolean generateAndTestShift(Person p) {
		...
		double f_current = plan.getScore();
		...
		double f_after = path.travelCost*t/3600;
//		System.out.println("f_after:" + f_after + " f_current:" + f_current + " prob:" + Math.exp((f_after - f_current)/this.c_k));
		boolean shiftAgent = false;
		if (f_after >= f_current) {
			shiftAgent = true;
			this.m1++;
		} else if (Math.exp((f_after - f_current)/this.c_k) > MatsimRandom.getRandom().nextDouble()) {
			this.m2++;
		...
		if (shiftAgent) {
			...
			Leg leg = this.popFac.createLeg("car");
