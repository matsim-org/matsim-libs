...
public class CASimExperimentBi3 {
	...
	//	private static double [] WIDTHS = new double[]{.5,.75,.9,1.2,1.6,2,2.1,2.2,2.3,2.4,2.5};//,2.6,2.7,2.8,2.9,3,3.1,3.2,3.3,3.4,3.5,3.6};
	...
	public static void main(String [] args) throws IOException {
		...
		BufferedWriter buf = new BufferedWriter(new FileWriter(new File("/Users/laemmel/devel/bipedca/plot/unbl"+w)));

		for (double bExR = 3; bExR >= 0.5; bExR-=0.1) {	
			for (double bExL = 30; bExL >= 30; bExL-=0.5) {
				for (int i = 0; i < WIDTHS.length; i++){
					System.out.println("bExR:" + bExR + " bExL:" + bExL + " i" + i);
					B_l = WIDTHS[i];
					B_r = w;//WIDTHS_R[i];
					if ((B_l <= bExR*1.1 && B_r <= bExL) || (B_l >= bExR && B_r >= bExL)){// || B_l < bExR/1.1 || B_r < bExL/1.1 || B_l > bExR*1.5 || B_r > bExL * 1.5) {
						//SS or RR regime
						continue;
					}
