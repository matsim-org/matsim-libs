...
public class CAMultiLaneDensityEstimatorSPHII implements
	...
	public double estRho(CAMoveableEntity e) {
		...
		double load = traverse(start, pos, stop, l, e, lane);
		if (dir == 1) {
			load += traverse(start + 1, pos, stop, l, e, lane);
		} else {
			load += traverse(start, pos, stop - 1, l, e, lane);
		}

		return load * AbstractCANetwork.RHO_HAT;
	}

	private double traverse(int current, int pos, int stop, CAMultiLaneLink l,
			CAMoveableEntity e, int lane) {

		double load = 0;
		CAMultiLaneLink last = null;
		CAMultiLaneLink next = null;
		...
		while (current <= stop) {
			...
			if (current == -1) {
				...
				if (last == null) {
					last = (CAMultiLaneLink) e.getLastCANetworkEntity();
				...
				int tmp = dsLast ? last.getNumOfCells() + current + 1
						: -(current + 2);
				if (last.getParticles(lane)[tmp] != null) {
					load += bSplinesKernel(d);
				}
				...
				if (next == null) {
					...
					next = (CAMultiLaneLink) this.net.getCALink(nextId);
				...
				if (next.getParticles(lane)[tmp] != null) {
					load += bSplinesKernel(d);
				}
			} else {
				if (l.getParticles(lane)[current] != null) {
					load += bSplinesKernel(d);
				}
			}
			current++;
