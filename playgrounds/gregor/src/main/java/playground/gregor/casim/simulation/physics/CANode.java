...
public class CANode implements CANetworkEntity, NetsimNode{
	
	private static final Logger log = Logger.getLogger(CANode.class);
	
	
	private double vHat = 1.26; //cells per unit time
	private final double rhoHat = 5.091; //particles per cell
//	private double D = 0.72;
	...
	public CANode(Node node, CANetwork net){
		double cap = 20;//dsl.getCapacity();
		this.D /= cap;
		this.vHat *= cap;
//		this.rhoHat *= cap;
//		this.D /= cap;
