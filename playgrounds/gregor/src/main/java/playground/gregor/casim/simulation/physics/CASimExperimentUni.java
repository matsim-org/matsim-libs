...
public class CASimExperimentUni {
...
//		links2.add(l5);
		...
		for (double dist = 32; dist >= 1; dist--) {
			CANetwork.RHO = 1/dist * 5.091;
			runIt(net,dist,links,monitor,sc);
			monitor.reset(0);
		}
		System.out.println(monitor);
	}

	private static void runIt(Network net, double dist, List<Link> links, CALinkMonitor monitor, Scenario sc) {
		//visualization stuff
...
//		vis.addAdditionalDrawer(new InfoBox(vis, sc));
		...
		for (int j = 0; j  < 5; j++){
			...
			for (int i = 0; i < particles.length; i += dist) {
