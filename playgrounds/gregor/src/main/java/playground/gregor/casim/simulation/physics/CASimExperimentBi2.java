...
public class CASimExperimentBi2 {
	...
	public static void main(String [] args) {
		...
		for (double rho = 0.1; rho <= 1; rho += 0.1) {
			for (int i = 0; i < 100; i++) {
				runIt(net,monitor,links,rho,sc);
				monitor.reset(0);
			}
		}
	...
	private static void runIt(Network net,CALinkMonitor monitor,List<Link>links,double rho, Scenario sc){
		//visualization stuff
...
//		vis.addAdditionalDrawer(new InfoBox(vis, sc));
		...
		for (int j = 0; j  < 5; j++){
			...
			for (int i = 0; i < particles.length; i ++) {
				...
				if (MatsimRandom.getRandom().nextBoolean()) {
					a = new CASimpleAgent(links,j+1,new IdImpl(id++),caLink);
					a.materialize(i, 1);
					CAEvent e = new CAEvent(0, a,caLink, CAEventType.TTA);
					caNet.pushEvent(e);
				} else{
					a = new CASimpleAgent(ll,4-j+1,new IdImpl(-(id++)),caLink);
					a.materialize(i, -1);
		...
		CANetwork.RHO = (double)agents/fields * 5.091;
		caNet.run();
	}
