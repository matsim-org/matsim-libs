...
//				dx /= ll;
//				dy /= ll;
//				this.vis.addTextStatic((c0.getX()+c1.getX())/2+dy, (c0.getY()+c1.getY())/2-dx, l.getId().toString(), 80);
...
//		a=255;
		this.vis.addLine(event.getX(), event.getY(), event.getX()+event.getVX()+dy*al, event.getY()+event.getVY()-dx*al, 0, 0, 0, a, 50);
...
//		}
	...
	public void handleEvent(NeighborsEvent event) {
		...
		for (Sim2DAgent neighbor : n) {
			...
			this.vis.addLine(x0+dx*a.getRadius()*1.5f, y0+dy*a.getRadius()*1.5f, x1-dx*neighbor.getRadius()*1.5f, y1-dy*neighbor.getRadius()*1.5f, r,g,b, 255, 0);
			...
			//			this.vis.addText((x0+x1)/2+offsetX, (y0+y1)/2+offsetY, tmp+"."+fill+tmp2+" m", 80,(float)atan);
