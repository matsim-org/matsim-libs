...
//	public static final String SCHEMA = "http://matsim.org/files/dtd/sim2dEnvironment_v0.2.xsd";
	...
	public void readFile(String filename) throws UncheckedIOException {
		...
		try {
			jc = JAXBContext.newInstance(playground.gregor.sim2d_v4.io.jaxb.sim2denvironment02.ObjectFactory.class);
...
//				System.out.println(crs.getName());
		...
		for (XMLFeatureAssociationType m : xmlcoll.getValue().getFeatureMember()) {
			...
			List<Id<Link>> relatedLinksIds = getRelatedLinkIds(ff);
	...
	private List<Id<Link>> getRelatedLinkIds(XMLSectionPropertyType ff) {
		...
		List<Id<Link>> ret = new ArrayList<>();
		for (int i = 0; i < toks.length; i++) {
			ret.add(Id.create(toks[i], Link.class));
		}
