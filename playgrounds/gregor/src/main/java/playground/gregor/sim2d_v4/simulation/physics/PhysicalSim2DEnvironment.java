...
//		log.warn("not implemented yet!");
		for (PhysicalSim2DSection psec : this.psecs.values()) {
			if (psec.getId().toString().equals("sec1348623433_linksim2d_0_-27216_dep_box_0")) {
	...
	public void createAndAddPhysicalTransitionSections(
		...
		dboxFl.x1 = li.fromOpening.x1; //this is obviously wrong needs to be turned by pi
		dboxFl.x0 = li.fromOpening.x0;
		dboxFl.y1 = li.fromOpening.y1;
		dboxFl.y0 = li.fromOpening.y0;
		...
		boolean cw = CGAlgorithms.isCCW(sec.getPolygon().getExteriorRing().getCoordinates());
		float bx;
		float by;
		if (cw) { // rotate left(currently not supported)
			bx = -dy;
			by = dx;
			throw new RuntimeException("Polygon describing section: " + sec.getId() + " has a clockwise orientation, which currently is not supported!");
		} else { // rotate right 
			bx = dy;
			by = -dy;
		}
		...
		float bottomX = opening.x0;
		float bottomY = opening.y0;
		...
		for (int i = 0; i < nrOfBoxes; i++) {
			...
			float spawnX = bottomX + bx/1.5f + dx/2;
			float spawnY = bottomY + by/1.5f + dy/2;
			PhysicalSim2DSection psecBox = createAndAddPhysicalSection(s);
