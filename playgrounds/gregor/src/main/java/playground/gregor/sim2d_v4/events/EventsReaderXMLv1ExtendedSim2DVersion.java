...
public class EventsReaderXMLv1ExtendedSim2DVersion extends MatsimXmlParser {
	...
	private void startEvent(final Attributes atts) {
		...
		if (XYVxVyEventImpl.EVENT_TYPE.equals(eventType)) {
			...
			this.events.processEvent(new Wait2LinkEvent(time, Id.create(atts.getValue(Wait2LinkEvent.ATTRIBUTE_PERSON), Person.class), Id.create(atts.getValue(Wait2LinkEvent.ATTRIBUTE_LINK), Link.class), atts.getValue(Wait2LinkEvent.ATTRIBUTE_VEHICLE) == null ? null : Id.create(atts.getValue(Wait2LinkEvent.ATTRIBUTE_VEHICLE), Vehicle.class), 1.0));
