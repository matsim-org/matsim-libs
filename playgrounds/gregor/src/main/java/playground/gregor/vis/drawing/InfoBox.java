...
public class InfoBox implements ClockedVisAdditionalDrawer, VisOverlay {
	...
	public void drawText(EventsBasedVisDebugger p) {
		...
		float y = 5 + round + ts;
		p.textSize(ts);
		// PVector cv = p.zoomer.getCoordToDisp(new
		// PVector((float)(li.tx+p.offsetX),(float)-(li.ty+p.offsetY)));
		// p.fill(0,0,0,255);
		...
		// p.rect(5, 5, 5+15+w+round, 5+round+ts+round + ts + ts/2 + ts +
		// ts/2,round);
		...
		p.text(stm, x, y);
		// double sph = this.speedup > .98 ? Math.round(this.speedup) :
		// this.speedup;
