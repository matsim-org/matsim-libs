...
import playground.yu.integration.cadyts.parameterCalibration.withCarCounts.experiment.naive.PCCtlListener;
...
public class Events2Score4PC_mnl extends Events2Score4PC implements
	...
	public Events2Score4PC_mnl(Config config, ScoringFunctionFactory sfFactory,
		...
		String addASCStr = config.findParam("naivePC", "addASC");
		if (addASCStr != null) {
			addASC = Boolean.parseBoolean(addASCStr);
		}
	}
		...
		for (Plan plan : legDurMapCar.keySet()) {
			...
			double util = mnl
					...
					/* s. the attributes order in Events2Score4PC2.attrNameList */.innerProd(
							attrVector)
					+ (addASC ? (Double) plan.getCustomAttributes().get(
							BseStrategyManager.UTILITY_CORRECTION) : 0d)
