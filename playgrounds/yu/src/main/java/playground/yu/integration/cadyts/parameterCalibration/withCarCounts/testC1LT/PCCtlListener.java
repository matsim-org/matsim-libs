...
public class PCCtlListener extends BseParamCalibrationControlerListener
	...
	private void initializeCalibrator(Controler ctl) {
		...
		calibrator = new CPC1<Link>(ctlIO.getOutputFilename("calibration.log"),
				config.global().getRandomSeed(), 3600, paramDim);
	...
	public void notifyIterationEnds(IterationEndsEvent event) {
		...
		// ************************************************
		if (calibrator.getParameterCovariance()
		...
		!= null) {
			writerCV.writeln(iter
					...
					// .toSingleLineString()
					+ "\t"
					+ calibrator.getParameterCovariance().toSingleLineString()
			// + "\t"
		...
		// ###################################################
		Vector avgParams = calibrator.getParameters();
