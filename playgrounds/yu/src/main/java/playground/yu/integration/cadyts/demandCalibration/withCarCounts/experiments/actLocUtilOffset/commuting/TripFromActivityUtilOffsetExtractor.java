 ...
 * 
 */
public class TripFromActivityUtilOffsetExtractor extends
		TripUtilOffsetExtractor {

	private String activityPrefix;

	public TripFromActivityUtilOffsetExtractor(Counts counts, Network net,
	...
	protected void convert2Trip(TripEvents tripEvents) {
		...
		if (tripEvents.getTripBegins().getActType().startsWith(activityPrefix)) {
			convert2TripInternal(tripEvents);
		}
	}
	...
	protected void convert2TripInternal(TripEvents tripEvents) {
		...
		if (enters != null) {
			...
			double firstTime = getTimeStep(enters.get(0).getTime());
			if (firstTime >= caliStartTime && firstTime <= caliEndTime) {
	...
	public static void main(String args[]) {
		...
		, outputFilenameBase = "test/DestinationUtilOffset2/9/1000.originUtiloffset.9."//
		;
		...
		TripUtilOffsetExtractor tuoExtractor = new TripFromActivityUtilOffsetExtractor(
		...
		t2q.writeShapeFile(outputFilenameBase
				+ "tripsWithUtilOffset.fromAllAct.shp"/* "w" */);
	}
