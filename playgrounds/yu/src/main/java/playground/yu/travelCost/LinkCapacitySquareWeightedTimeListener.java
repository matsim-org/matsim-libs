...
public class LinkCapacitySquareWeightedTimeListener implements
	...
	public static class LinkCapacitySquareWeightedTravelTimeCostCalculator
		...
		public LinkCapacitySquareWeightedTravelTimeCostCalculator(
	...
	public void notifyIterationStarts(IterationStartsEvent event) {
		...
		if (event.getIteration() > ctl.getConfig().controler().getFirstIteration()) {
			ctl.addOverridingModule(new AbstractModule() {
				@Override
				public void install() {
					bindTravelDisutilityFactory().toInstance(new LinkCapacitySquareWeightedTravelCostCalculatorFactoryImpl());
				}
			});
		}
	}
