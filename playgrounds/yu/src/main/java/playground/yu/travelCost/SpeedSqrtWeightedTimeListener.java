...
public class SpeedSqrtWeightedTimeListener implements
	...
	public static class SpeedSqrtWeightedTravelTimeCostCalculator implements TravelDisutility {
	...
	public void notifyIterationStarts(IterationStartsEvent event) {
		...
		if (event.getIteration() > ctl.getConfig().controler().getFirstIteration()) {
			ctl.addOverridingModule(new AbstractModule() {
				@Override
				public void install() {
					bindTravelDisutilityFactory().toInstance(new SpeedSqrtWeightedTravelCostCalculatorFactoryImpl());
				}
			});
		}
	}
