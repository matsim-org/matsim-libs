 ...
 *
 */
public class FakeSimTransit implements Mobsim {
	...
	public FakeSimTransit(Scenario sc, EventsManager eventsManager) {
		this.sc = sc;
	...
	public FakeSimTransit(Scenario sc2, EventsManager eventsManager,
	...
	public class SimThread implements Callable {
			...
			for (Plan plan : plans) {
				...
				for (int idx = 0; idx < elements.size(); idx += 2) {
					...
					if (idx > 0) {
						 ...
						 * a leg before.
						 */
						Route route = ((Leg) elements.get(idx - 1)).getRoute();
					...
					if (idx < elements.size() - 1) {
						...
						AgentDepartureEvent departureEvent = new AgentDepartureEventImpl(actEndTime, plan.getPerson()
		...
		private double calcRouteTravelTime(Route route, double startTime, TravelTime travelTime, Network network) {
			double tt = 0;
			try{
