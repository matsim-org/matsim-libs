 ...
 */
public final class PopulationUtilsForPlanGenomes {
	/**
	 * Is a namespace, so don't instantiate:
	 */
	private PopulationUtilsForPlanGenomes() {}
	...
	public static Population createPopulation(Config config, Network network) {
        ...
        return new PopulationForPlanGenomes(new PopulationFactoryForPlanGenomes(routeFactory));
	}
	...
	private static InputStream openPopulationInputStream(final Population s1) {
		try {
			...
			new Thread(new Runnable() {
				@Override
				public void run() {
					final org.matsim.core.population.PopulationWriter writer = new org.matsim.core.population.PopulationWriter(s1);
					try {
						writer.write(out);
