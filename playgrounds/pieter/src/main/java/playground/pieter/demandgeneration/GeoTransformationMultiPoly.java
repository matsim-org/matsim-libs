...
public class GeoTransformationMultiPoly {
...
//	    final CoordinateReferenceSystem targetCRS = CRS.parseWKT( WGS84_UTM35S);
		...
		while (it.hasNext()) {
			...
			if ( geo instanceof MultiPolygon ) {
				...
				multiPoly = geofac.createMultiPolygon(new Polygon[] {(Polygon) geo});
			} else {
				throw new RuntimeException("Feature does not contain a polygon/multipolygon!");
			}
			if(multiPoly.getNumGeometries()>1){
				System.out.println("More than one on feature " + ft.getAttribute(1));
			}
