...
public class CEPASToEvents {
	...
	private class CEPASVehicle {
		class CEPASVehicleDwellEvent implements Comparable<CEPASVehicleDwellEvent> {
			...
			public void findTrueDwellTime() {
				...
				// arrival time)
				if (transactions.size() <= minTransactionClusterSize) {
	...
	private HashMap<String, CEPASVehicle> ptVehicles = new HashMap<String, CEPASToEvents.CEPASVehicle>();
...
//					continue;
				...
				// with
				// it
				if (ptVehicle.cepasLine.lineId.toString().equals("812"))
					continue;
				// String query = String
				// .format("select * "
				// +
