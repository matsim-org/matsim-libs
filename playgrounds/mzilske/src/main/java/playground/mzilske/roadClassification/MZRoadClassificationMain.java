...
//        new ConfigReader(config).parse(BerlinRunUncongested3.class.getResourceAsStream("2kW.15.xml"));
        ...
        new CountsReaderMatsimV1(allCounts).read(sightingsDir + "/all_counts.xml.gz");

        scenario.addScenarioElement(Counts.ELEMENT_NAME, allCounts);

        final ObjectAttributes linkAttributes = new ObjectAttributes();
        new ObjectAttributesXmlReader(linkAttributes).read("/Users/michaelzilske/git/java8-matsim-playground/output/berlin/road-categories/road-categories.xml");
