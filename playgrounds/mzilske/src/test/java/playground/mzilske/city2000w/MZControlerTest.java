...
public class MZControlerTest extends MatsimTestCase {
	...
	public void testTravelTimeCalculation() {
		...
		MZControler controler = new MZControler(f.scenario);
		...
		controler = new MZControler(f.scenario);
	 ...
	 * in the initialization process of the MZControler.
	 *
	 * @author mrieser
	 */
	public void testSetScoringFunctionFactory() {
		...
		final MZControler controler = new MZControler(scenario);
		controler.setWriteEventsInterval(0);
	...
	public void testSetWriteEventsInterval() {
		...
		Scenario scenario = new MultiPopulationScenario(config);
		final MZControler controler = new MZControler(scenario);
		assertFalse("Default for MZControler.writeEventsInterval should be different from the interval we plan to use, otherwise it's hard to decide if it works correctly.",
				3 == controler.getWriteEventsInterval());
		controler.setWriteEventsInterval(3);
		assertEquals(3, controler.getWriteEventsInterval());
		controler.run();

		assertTrue(new File(controler.getControlerIO().getIterationFilename(0, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(1, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(2, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertTrue(new File(controler.getControlerIO().getIterationFilename(3, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(4, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(5, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertTrue(new File(controler.getControlerIO().getIterationFilename(6, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(7, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(8, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertTrue(new File(controler.getControlerIO().getIterationFilename(9, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(10, MZControler.FILENAME_EVENTS_TXT)).exists());
	}
	...
	public void testSetWriteEventsIntervalConfig() {
		...
		assertFalse(new File(controler.getControlerIO().getIterationFilename(1, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(2, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(3, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertTrue(new File(controler.getControlerIO().getIterationFilename(4, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(5, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(6, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(7, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertTrue(new File(controler.getControlerIO().getIterationFilename(8, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(9, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(10, MZControler.FILENAME_EVENTS_TXT)).exists());
	}
	...
	public void testSetWriteEventsNever() {
		...
		assertFalse(new File(controler.getControlerIO().getIterationFilename(0, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(1, MZControler.FILENAME_EVENTS_TXT)).exists());
	}
	...
	public void testSetWriteEventsAlways() {
		...
		assertTrue(new File(controler.getControlerIO().getIterationFilename(0, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertTrue(new File(controler.getControlerIO().getIterationFilename(1, MZControler.FILENAME_EVENTS_TXT)).exists());
	}
	...
	public void testSetWriteEventsTxt() {
		...
		assertTrue(new File(controler.getControlerIO().getIterationFilename(0, MZControler.FILENAME_EVENTS_TXT)).exists());
		assertFalse(new File(controler.getControlerIO().getIterationFilename(0, MZControler.FILENAME_EVENTS_XML)).exists());
	}
	...
	public void testSetWriteEventsXml() {
		...
		assertFalse(new File(controler.getControlerIO().getIterationFilename(0,  MZControler.FILENAME_EVENTS_TXT)).exists());
		assertTrue(new File(controler.getControlerIO().getIterationFilename(0, MZControler.FILENAME_EVENTS_XML)).exists());
	}
