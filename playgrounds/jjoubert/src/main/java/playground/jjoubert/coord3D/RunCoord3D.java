...
//		Scenario sc = elevateEquilNetwork(); 
	...
	private static Scenario elevateEquilNetwork(){
		...
		/* Give elevation details to the nodes. */
		Map<Id<Node>, ? extends Node> nodes = sc.getNetwork().getNodes();
	...
	private static void trySrtmOnPtaSample(String network){
		...
		/* Set up SRTM tile. */
		Coord blc = getBottomLeftCoordinate(sc.getNetwork());
 	...
 	private static Coord getBottomLeftCoordinate(Network network){
	...
	private static void buildElevationGrid(Network network, double delta){
		...
		/* Set up SRTM tile. */
		Coord blc = getBottomLeftCoordinate(network);
		LOG.info("Bottom-left corner: " + blc.toString());
		String tileName = getSrtmTile(blc);
		File srtmFile = new File(path + "data/tiles/" + tileName + ".hgt");
		if(!srtmFile.exists() || !srtmFile.canRead()){
