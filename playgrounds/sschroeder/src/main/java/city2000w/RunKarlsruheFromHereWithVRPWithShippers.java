 ...
 *
 */
public class RunKarlsruheFromHereWithVRPWithShippers implements ScoringListener, ReplanningListener, StartupListener, BeforeMobsimListener, AfterMobsimListener, IterationEndsListener {

	private static final int NUMBEROFCARRIERS = 5;
	
	private static final int NUMBEROFTSPSHIPMENTS = 20;
	
	private static Logger logger = Logger.getLogger(RunKarlsruheFromHereWithVRPWithShippers.class);
	...
	public static void main(String[] args) {
		...
		RunKarlsruheFromHereWithVRPWithShippers runner = new RunKarlsruheFromHereWithVRPWithShippers();
		runner.run();
	}
	...
	private void createShippers() {
		...
		ShipperImpl shipper = ShipperUtils.createShipper("shipper_stefan", "160038764_160038765");
		shippers.getShippers().add(shipper);
	}
	
	private void createShipperContracts(){
...
//			ClarkeAndWrightCarrierPlanBuilder planBuilder = new ClarkeAndWrightCarrierPlanBuilder(scenario.getNetwork());
	...
	private void createTransportServiceProviderWithContracts(int nOfShipments) {
		TransportServiceProviderImpl tsp = new TransportServiceProviderImpl(new IdImpl("tsp_stefan"));
