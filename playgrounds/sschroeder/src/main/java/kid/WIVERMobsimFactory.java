...
public class WIVERMobsimFactory implements MobsimFactory {
	 ...
	 * @param planAgentCreator 
	 * @param controler 
	 */
	public WIVERMobsimFactory(final int nOfThreads, KiDPlanAgentCreator planAgentCreator) {
		this.nOfThreads = nOfThreads;
		this.planAgentCreator = planAgentCreator;
	}
	...
	public Simulation createMobsim(final Scenario scenario, final EventsManager eventsManager) {
		...
		planSim.addAgentSource(planAgentCreator);
		return planSim;
	}

	private Map<Id, Plan> createFreightAgentPlanMap() {
		Map<Id, Plan> result = new HashMap<Id, Plan>();
		for(PlanAgent planAgent : planAgentCreator.getAgents()) {
