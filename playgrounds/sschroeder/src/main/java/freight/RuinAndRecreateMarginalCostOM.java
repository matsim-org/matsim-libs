...
public class RuinAndRecreateMarginalCostOM implements OfferMaker{
	...
	public Offer requestOffer(Id from, Id to, int size, Double startPickup, Double endPickup, Double startDelivery, Double endDelivery, Double memorizedPrice) {
		...
		double transportCost = ruinAndRecreateAlgo.getVrpSolution().getTransportCosts();
		Offer offer = new Offer();
		offer.setCarrierId(carrier.getId());
		if(carrier.getSelectedPlan() != null){
			if(carrier.getSelectedPlan().getScore() != null){
				offer.setPrice(transportCost - carrier.getSelectedPlan().getScore());
			}
			else{
				offer.setPrice(transportCost);
			}
		}
		else{
			offer.setPrice(transportCost);
