...
//				System.out.println(node.getId());
		...
		while (transitLineIterator.hasNext()) {
			...
			String to = tags.get("to");
			String from = tags.get("from");
...
//			TransitLine line = transitSchedule.getFactory().createTransitLine(new IdImpl(operator + "_" + route + "_" + ref + "_" + name + "_" + relation.getId()));
			Id lineId = new IdImpl(operator + "_" + route + "_" + ref);
			TransitLine line;
			...
			if(transitSchedule.getTransitLines().containsKey(lineId)){
				...
				line = transitSchedule.getFactory().createTransitLine(lineId);
			}
...
//			List<Id> linkIdsR = stitcher.getBackwardRoute();
			...
			if (linkIdsH.size() >= 2) {
				...
				List<TransitRouteStop> stops = enterStopLinkIds(stopsH, stopLinkIdsH, forwardTravelTimes, line.getId() + "(" + name + ")", network.getLinks().get(linkIdsH.get(0)).getFromNode());
				TransitRoute routeH = transitSchedule.getFactory().createTransitRoute(new IdImpl(name + "_" + relation.getId()), networkRouteH, stops, TransportMode.pt);
//				if((from==null)||(to == null)){
//					routeH = transitSchedule.getFactory().createTransitRoute(new IdImpl(count++ + "_" + "H"), networkRouteH, stops, TransportMode.pt);
//				}else{
//					routeH = transitSchedule.getFactory().createTransitRoute(new IdImpl(count++ + "_" + to), networkRouteH, stops, TransportMode.pt);
//				}				
...
//				TransitRoute routeR;
//				if((from==null)||(to == null)){
//					routeR = transitSchedule.getFactory().createTransitRoute(new IdImpl(count++ + "_" + "R"), networkRouteR, stops, TransportMode.pt);
//				}else{
//					routeR = transitSchedule.getFactory().createTransitRoute(new IdImpl(count++ + "_" + from), networkRouteR, stops, TransportMode.pt);
//				}		
...
//			}
	...
	private List<TransitRouteStop> enterStopLinkIds(List<Node> stopNodes, List<Id> stopLinkIdsH, List<Double> travelTimes, String routeRef, org.matsim.api.core.v01.network.Node node) {
		...
		while(i.hasNext()) {
			...
			String gtfsId = stopTags.get("gtfs_id");
			if(stopName != null){
			...
			TransitStopFacility facility = transitSchedule.getFactory().createTransitStopFacility(createTransitStopId(routeRef, gtfsId, stopNo), coordinate, false);
			facility.setName(stopName);
			stopNo++;
	...
	private IdImpl createTransitStopId(String ref, String gtfsId, int stopNo) {
		return new IdImpl(ref + "_" + gtfsId + "_" + stopNo);
	}
