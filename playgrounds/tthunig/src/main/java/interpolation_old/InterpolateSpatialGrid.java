...
//		sg_4NeighborsIDW.writeToFile("Z:/WinHome/opus_home_shared/data/seattle_parcel/results/interpolationQuickTest/interpolation/" + directory + "/" + resolution + "_4NeighborsIDW_exp" + exp + ".txt");
	...
	/* alte variante (MyBilinearInterpolator nutzt SpatialGrid nicht; ValueInterpolation)
		...
		for (int k = 0; k < y_new.length; k++) {
			for (int l = 0; l < x_new.length; l++) {
				sg_new.setValue(sg_new.getRow(y_new[k]), sg_new.getColumn(x_new[l]), BiLinearInterpolator.myBiLinearValueInterpolation(sg.getMatrix(), sg_new.getColumn(x_new[l])/2., sg_new.getRow(y_new[k])/2.));
			}
		}
		return sg_new;
	 */
		
	/* alternative 1 (BiLinearInterpolator nutzt SpatialGrid; ValueInterpolation) */
		SpatialGrid sg_new = new SpatialGrid(sg.getXmin(), sg.getYmin(), sg.getXmax(), sg.getYmax(), sg.getResolution() / 2);
		...
		for (int k = 0; k < y_new.length; k++) {
			for (int l = 0; l < x_new.length; l++) {
				sg_new.setValue(sg_new.getRow(y_new[k]), sg_new.getColumn(x_new[l]), BiLinearInterpolator.biLinearValueInterpolation(sg, x_new[l], y_new[k]));
			}
		}
		return sg_new;
	}
	...
	private static SpatialGrid myAllValuesIDW(SpatialGrid sg, double[] x, double[] y, double[] x_new, double[] y_new, double exp) {
		...
		for (int k = 0; k < y_new.length; k++) {
			for (int l = 0; l < x_new.length; l++) {
				sg_new.setValue(sg_new.getRow(y_new[k]), sg_new.getColumn(x_new[l]), InverseDistanceWeighting.allValuesIDW(sg, x_new[l], y_new[k], exp));
			}
		}
		return sg_new;
	}
	...
	private static SpatialGrid my4NeighborsIDW(SpatialGrid sg, double[] x, double[] y, double[] x_new, double[] y_new, double exp) {
		...
		for (int k = 0; k < y_new.length; k++) {
			for (int l = 0; l < x_new.length; l++) {
				sg_new.setValue(sg_new.getRow(y_new[k]), sg_new.getColumn(x_new[l]), InverseDistanceWeighting.fourNeighborsIDW(sg, x_new[l], y_new[k], exp));
			}
		}
		return sg_new;
	}
