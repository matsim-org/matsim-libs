 ...
 *
 */
final class TSParkingSearchAgent implements MobsimDriverAgent, MobsimPassengerAgent, HasPerson, PlanAgent {
	...
	public TSParkingSearchAgent(final Plan plan1, final Netsim simulation) {
	...
	public final Id<Link> chooseNextLinkId() {
		if ( !this.searchMode ) {
			...
			if(nextId.equals(destination)){
				log.error("set agent" + this.getId() + "on searching mode after wanting to drive on link " + nextId);
	...
	public final boolean isWantingToArriveOnCurrentLink() {
		if ( !this.searchMode ) {
			...
			return false ; // true if parking space is available 
		}
	}
