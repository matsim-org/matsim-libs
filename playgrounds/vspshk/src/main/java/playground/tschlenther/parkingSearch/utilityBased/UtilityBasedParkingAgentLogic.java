 ...
 *
 */
public class UtilityBasedParkingAgentLogic extends ParkingAgentLogic {
	 ...
	 */
	public UtilityBasedParkingAgentLogic(Plan plan, ParkingSearchManager parkingManager, WalkLegFactory walkLegFactory,
	...
	protected DynAction nextStateAfterUnParkActivity(DynAction oldAction, double now) {
		...
		if ((this.parkingManager.unParkVehicleHere(currentlyAssignedVehicleId, agent.getCurrentLinkId(), now))||(isinitialLocation)){
			...
			return new UtilityBasedParkingDynLeg(currentLeg.getMode(), actualRoute, parkingLogic, parkingManager, currentlyAssignedVehicleId, timer, events);
		}
