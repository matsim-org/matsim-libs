...
public class ImportTask_v2 extends PleaseWaitRunnable {
	...
	protected void realRun() throws SAXException, IOException,
		...
		for (Link link : scenario.getNetwork().getLinks().values()) {
			...
			way.put("maxspeed", String.valueOf(link.getFreespeed()));
			way.put("capacity", String.valueOf(link.getCapacity()));
			way.put("length", String.valueOf(link.getLength()));
			way.put("lanes", String.valueOf(link.getNumberOfLanes()));

			// as for now, ways that do not have a oneway-tag set to true are
			// considered to represent two links
			way.put("oneway", "true");
