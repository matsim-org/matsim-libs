...
class IllustrationAP200PVScenarioInduced { // Relationsbezogen_mit_generalisierten_Kosten
...
//			nullfallForOD.setValuesForMode( Mode.rail, railValues ) ;
	...
	static ScenarioForEval createPlanfall1(ScenarioForEval nullfall) {
		...
		Values planfallForOD = planfall.getByODRelation(new IdImpl("BC")) ;
		{
			...
			planfall.getByODRelation(new IdImpl("BC")).getByMode(Mode.road).getByType(Type.PV).incByEntry(Entry.XX, -0. ) ;
		}
		return planfall;
	}
