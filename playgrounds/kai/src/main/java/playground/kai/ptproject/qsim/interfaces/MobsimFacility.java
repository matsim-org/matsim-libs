...
public class MobsimFacility implements Updateable {
	...
	private Queue<MobsimAgent> agentsAtActivities = new PriorityQueue<MobsimAgent>(1, new DepartureTimeComparator() ) ;

	// buses somehow via the "feature" --???
	
	/** Plain "add" of a person, normally during initialization */
	void addPerson( MobsimAgent person ) {
	...
	public void update() {
		MobsimAgent person = agentsAtActivities.peek();
		if ( person.getActivityEndTime() <= now() ) {
			agentsAtActivities.remove();
			person.endActivityAndComputeNextState(now()) ;
//			throw new RuntimeException("this is deprecated") ;
	...
	private static class DepartureTimeComparator implements Comparator<MobsimAgent> {
		@Override
		public int compare(MobsimAgent o1, MobsimAgent o2) {
			return 0 ; // dummy
		}
