...
/*	setting these parameters different from each other, will cause problems when converting utils into money terms.
	 ...
	 * @param params
	 * @param network
	 * @param householdIncomePerDay
	 * @param agentId For more helpful error messages.
	 */
	public ScoringFromLeg(final CharyparNagelScoringParameters params, Network network, double householdIncomePerDay, final Id agentId) {
	...
	private double calculateLegDistance(final Leg leg) {
		...
		if (Double.isNaN(dist)){
			throw new IllegalStateException("Route distance is NaN for person: " + this.agentId);
		}
		return dist;
	}
	...
	private double calculateScore(double betaIncome, double distanceCost, double betaTravelTime, double travelTime) {
		...
		if (Double.isNaN(score)){
			throw new IllegalStateException("Leg score is NaN for person: " + this.agentId);
		}
		return score;
	}
