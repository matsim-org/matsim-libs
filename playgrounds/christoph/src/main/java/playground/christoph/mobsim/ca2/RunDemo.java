...
import playground.christoph.mobsim.ca2.events.VXYEventsHandler;
...
public class RunDemo {
	...
	private static void createPopulation(Scenario scenario) {
		...
		for (int i = 0; i < populationSize; i++) {
			...
			Route route = new LinkNetworkRouteImpl(Id.create("l0", Link.class), linkIds, Id.create("l3", Link.class));
			leg.setRoute(route);
			Activity to = populationFactory.createActivityFromLinkId("home", Id.create("l3", Link.class));
...
//		eventsManager.addHandler(new EventsPrinter());
		...
		scenario.getConfig().qsim().setTimeStepSize(timeStep);
		...
		CASimEngine caEngine = new CASimEngineFactory().createCASimEngine(qSim, spatialResolution);
		qSim.addMobsimEngine(caEngine);
        qSim.addDepartureHandler(new CADepartureHandler(caEngine, CollectionUtils.stringToSet(TransportMode.walk)));
