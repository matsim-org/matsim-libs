...
class ParallelFlexibleCellSimEngine extends FlexibleCellSimEngine {
	
	private final int numOfThreads;
	
	private Thread[] threads;
	private FlexibleCellSimEngineRunner[] engines;
	...
	public void setInternalInterface(InternalInterface internalInterface) {
		...
		if (this.engines != null) {
			for (FlexibleCellSimEngineRunner engine : engines) {
	...
	public void doSimStep(final double time) {
		try {
			// set current Time
			for (FlexibleCellSimEngineRunner engine : this.engines) {
	...
	private void initMultiModalSimEngineRunners() {
		...
		this.engines = new FlexibleCellSimEngineRunner[numOfThreads];
		...
		for (int i = 0; i < numOfThreads; i++) {
			FlexibleCellSimEngineRunner engine = new FlexibleCellSimEngineRunner(startBarrier, 
					separationBarrier, endBarrier, this.getMobsim(), this);
	...
	private void assignSimEngines() {
		...
		for (NetsimNode node : this.getMobsim().getNetsimNetwork().getNetsimNodes().values()) {
			FlexibleCellSimEngine simEngine = engines[roundRobin % this.numOfThreads];
			super.getFlexibleCellNode(node.getNode().getId()).setMultiModalSimEngine(simEngine);
			...
			for (Link l : node.getNode().getOutLinks().values()) {
				super.getFlexibleCellLink(l.getId()).setCASimEngine(simEngine);
			}
