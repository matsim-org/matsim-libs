...
import org.matsim.ptproject.qsim.ParallelQSimEngineFactory;
 ...
 */
public class KnowledgeWithinDayQSimFactory implements MobsimFactory {
	  private static final Logger log = Logger.getLogger(KnowledgeWithinDayQSimFactory.class);
	  
	  @Override
	  public KnowledgeWithinDayQSim createMobsim(Scenario sc, EventsManager eventsManager)
	  {  
		  ...
		  if (numOfThreads > 1)
		  {
			  ...
			  KnowledgeWithinDayQSim sim = new KnowledgeWithinDayQSim(sc, em, new ParallelQSimEngineFactory());
		  ...
		  {
			  return new KnowledgeWithinDayQSim(sc, eventsManager); 
		  }
	  }
}
