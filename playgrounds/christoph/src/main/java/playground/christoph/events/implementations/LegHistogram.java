...
import playground.christoph.events.implementations.LegHistogramInstance.ModeData;
 ...
 * based on events.
 * 
 * The parallel implementation is a very close copy of the original implementation.
 * Instead, one could use concurrent data structures to simplify the data synchronization
 * process. However, doing so might result in slightly more complex code (some users
 * might not be familiar with those object types).
 */
public class LegHistogram implements MasterEventHandler {
	...
	private final Set<LegHistogramInstance> instances = new LinkedHashSet<LegHistogramInstance>();
	...
	public EventHandlerInstance createInstance() {
		LegHistogramInstance instance = new LegHistogramInstance(this.binSize, this.nofBins);
