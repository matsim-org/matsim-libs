...
final class EditRoutesKai {

	private static final Logger logger = Logger.getLogger(EditRoutesKai.class);
	
	private EditRoutesKai() {} // only a name space; can't be instantiated
	 ...
	 */
	static boolean replanFutureLegRoute(Plan plan, Leg leg, PlanAlgorithm planAlgorithm) {
	 ...
	 */
	static boolean replanCurrentLegRoute(Person person, List<PlanElement> planElements, Integer planElementIndex, Integer routeLinkIdIndex, 
			PlanAlgorithm routeAlgo, double now) {
		if (planElements == null) return false;
		if (planElementIndex == null) return false;
		if (routeAlgo == null) return false; 

		if ( !( planElements.get(planElementIndex) instanceof Leg ) ) {
			logger.error( "calling replanCurrentLegRoute, but we are not at a leg.  returning ... ") ;
			return false ;
		}
		Leg leg = (Leg) planElements.get(planElementIndex) ;
		Activity fromActivity = (Activity) planElements.get(planElementIndex - 1);
		Activity toActivity = (Activity) planElements.get(planElementIndex + 1);

		if ( !( leg.getRoute() instanceof NetworkRoute ) ) {
			logger.warn( "Route is not NetworkRoute; don't know how to handle this; returning ...") ;
			return false ;
		}
		Route oldRoute = leg.getRoute();
		
		if ( now > -500 ) { // yyyyyy stupid hack to that eclipse compiles the code after the return statement.  kai, oct'10 
			logger.error( "don't know how to continue; aborting ...") ;
			return false ;
		}
		
		int currentNodeIndex=-1 ; // dummy so that this compiles
		...
		newFromActivity.setStartTime(now);
		newFromActivity.setEndTime(now);
