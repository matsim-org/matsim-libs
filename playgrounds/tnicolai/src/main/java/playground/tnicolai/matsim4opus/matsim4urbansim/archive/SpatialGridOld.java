 ...
 * @author thomas
 */
public class SpatialGridOld<T> {
	
	private static final Logger logger = Logger.getLogger(SpatialGridOld.class);
	...
	public SpatialGridOld(double xmin, double ymin, double xmax, double ymax, double resolution) {
		minX = xmin;
		minY = ymin;
		maxX = xmax;
		maxY = ymax;
	...
	public SpatialGridOld(double [] boundingBox, double resolution) {
	...
	public SpatialGridOld(SpatialGridOld<?> grid) {
	...
	public static SpatialGridOld<Double> readFromFile(String filename) {
		return readFromFile(filename, new DoubleSerializer());
	}
	
	public static <V> SpatialGridOld<V> readFromFile(String filename, ObjectSerializer<V> serializer) {
		...
		public void startTag(String name, Attributes atts, Stack<String> context) {
			if(CELL_TAG.equalsIgnoreCase(name)) {
				...
				if(grid != null) {
					...
					grid = new SpatialGridOld<V>(xmin, ymin, xmax, ymax, resolution);
				}
			}
		}
	...
	private static class GridXMLWriter<V> extends MatsimXmlWriter {
		...
		@SuppressWarnings("unchecked")
		public void write(SpatialGridOld<V> grid, String filename) throws IOException {
			openFile(filename);
			writeXmlHead();
	...
	public static void main(String args[]){
		...
		SpatialGridOld<Double> sg = new SpatialGridOld<Double>(0, 0, 100, 100, 1);
