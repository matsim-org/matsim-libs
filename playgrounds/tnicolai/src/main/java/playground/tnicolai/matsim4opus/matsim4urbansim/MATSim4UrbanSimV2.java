 ...
 *
 */
public class MATSim4UrbanSimV2 implements MATSim4UrbanSimInterface{
	...
	MATSim4UrbanSimV2(String args[]){
		...
		// get MATSim4UrbanSim configuration file location
		String matsimConfiFile = (args!= null && args.length==1) ? args[0].trim():null;
		// checks if matsimConfiFile parameter contains a valid path
		Paths.isValidPath(matsimConfiFile);
		
		if( !(connector = new MATSim4UrbanSimConfigurationConverterV4( matsimConfiFile )).init() ){
			log.error("MATSim4UrbanSim can not be initialized. Check the paramerter settings in the travel model configuration section and external MATSim config files ...");
			System.exit(-1);
		}
	...
	void setControlerSettings(ScenarioImpl scenario, String[] args) {
		...
		// set mode 
		this.isParcelMode 				= moduleUrbanSimParameter.isParcelMode();
	}
