...
public class MielecSimpleDemandGeneration {
	public static void main(String[] args) {
		...
		Matrix morningODMatrix = MatrixUtils.createSparseMatrix("morning", zones.keySet(), transposedMatrix);

		double startTime = 6 * 3600;
		dg.generateMultiplePeriods(morningODMatrix, "dummy", "dummy", TransportMode.car, startTime, duration,
				flowCoeffs);

		startTime += 3600 * flowCoeffs.length;
		dg.generateMultiplePeriods(afternoonODMatrix, "dummy", "dummy", TransportMode.car, startTime, duration,
				flowCoeffs);
