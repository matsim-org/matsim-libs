    ...
    private void updateEStatsForVehicle(Vehicle vehicle)
    {
        ...
        for (TaxiTask t : schedule.getTasks()) {
            if (t instanceof ETaxiChargingTask) {
                ...
                dailyEStats.stateTimeSumsByState.add(ETaxiState.QUEUED,
                        chargingStartTime - arrivalTime);
                dailyEStats.stateTimeSumsByState.add(ETaxiState.PLUGGED,
                        chargingEndTime - chargingStartTime);
            }
        }
    }
