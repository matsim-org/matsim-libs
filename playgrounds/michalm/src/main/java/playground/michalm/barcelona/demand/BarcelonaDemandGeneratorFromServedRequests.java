...
public class BarcelonaDemandGeneratorFromServedRequests extends AbstractDemandGeneratorFromServedRequests {
	...
	public void generateDemand(Iterable<BarcelonaServedRequest> requests, double selectionProbability) {
	...
	public static void main(String[] args) {
		...
		requests = ServedRequests.filterWorkDaysPeriods(requests, BarcelonaServedRequests.ZERO_HOUR);
		...
		for (int i = 2; i <= 10; i++) {
			...
			BarcelonaDemandGeneratorFromServedRequests dg = new BarcelonaDemandGeneratorFromServedRequests(scenario);
			double scale = i / 10.;
		...
		// new
		// BarcelonaServedRequestsWriter(requests).writeFile("d:/PP-rad/Barcelona/data/served_requests/tripsInAgglomeration_since_Mar_2011_only_weekdays.csv");
	}
}
