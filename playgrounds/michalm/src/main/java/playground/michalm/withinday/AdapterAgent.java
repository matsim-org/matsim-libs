...
public class AdapterAgent
    implements MobsimDriverPassengerAgent, SimulationBeforeSimStepListener
{
    ...
    TeleportationWorld teleportationWorld = new TeleportationWorld() {
        ...
        public void stop()
        {
            ...
            // yy stopping is supported nearly nowhere, since it does not make sense. An airplane
            // can also not
        ...
        public ActivityPlane getActivityPlane()
        {
            return new ActivityPlane() {
                ...
                public void startDoing(ActivityBehavior activityBehavior)
                {
                    ...
                    // ?? I think this line was missing so I added it (to be checked)!!!
                    AdapterAgent.this.activityEndTime = Double.POSITIVE_INFINITY;

                    eventsManager.processEvent(eventsManager.getFactory().createActivityStartEvent(
                            now, id, currentLinkId, null, activityBehavior.getActivityType()));
        ...
        public RoadNetworkPlane getRoadNetworkPlane()
        {
            return new RoadNetworkPlane() {
                ...
                public void startDriving(DrivingBehavior drivingBehavior)
                {
                    ...
                    drivingBehavior.doSimStep(drivingWorld);// ????????????????? - should be below
                                                            // "arrangeAgentDeparture()"?
                    simulation.arrangeAgentDeparture(AdapterAgent.this);
                }
    ...
    private ActivityWorld activityWorld = new ActivityWorld() {
        ...
        public void stopActivity()
        {
            ...
            eventsManager.processEvent(eventsManager.getFactory().createActivityEndEvent(now, id,
                    currentLinkId, null, activityBehavior.getActivityType()));
            AdapterAgent.this.activityEndTime = now;
            simulation.rescheduleActivityEnd(AdapterAgent.this, Double.POSITIVE_INFINITY, now);

            simulation.getAgentCounter().decLiving();
            // This is necessary because the QSim thinks it must increase the living agents counter
            // in the rescheduling step. mz
            // The intention there was that an agent that is no longer alive has an activity end
            // time of infinity (rather
            // than removing it completely from teh mobsim). The number of
            // alive agents is only modified when an activity end time is changed between a finite
            // time and infinite. kai, jun'11
    ...
    @Override
    public void notifySimulationBeforeSimStep(
            @SuppressWarnings("rawtypes") SimulationBeforeSimStepEvent e)
    {
        if (!living) {
        ...
        if (teleportationBehavior != null) {
            ...
            // I guess you get here in the time step after endLeg/ActAndAssumeControl, since this
            // particular agent does not
            // do anything in those methods. So the agent is in limbo for the time being. Is
            // "being in limbo" consistent with
    ...
    public double getActivityEndTime()
    {
        ...
        //
        // The main reason for this is that the activity end queue is time-sorted, so that not all
        // agents need to be asked
        // if they want to depart. If you take care of agent departure yourself, this could just
        // return infinity.
    ...
    public void endActivityAndAssumeControl(double now)
    {
        ...
        // Or do I not? Then I will tell the Simulation I am having an activity until the next
        // timestep.

        // MM: I would put here firing ActEndEvnt instead of ActivityWorld.stopActivity()!!!
        // eventsManager.processEvent(eventsManager.getFactory().createActivityEndEvent(now, id,
        // currentLinkId, null, activityBehavior.getActivityType()));

        // activityBehavior.activityEnded(activityWorld);
    }
    ...
    public void endLegAndAssumeControl(double now)
    {
        ...
        //
        // In theory, transport-related events should be thrown by the transport part of the
        // simulation, activity-related
        // events should be thrown by the agent. In practice, there are many transport-related
        // modules, such as teleportation,
        // walk, public transit, etc., and many of them are pluggable. It is quite possible that one
        // of them forgets the
        // corresponding arrival event. So it seems easier to ensure consistency in the agent than
        // in the framework.
        // In part also because the user of the agent arrival event is probably the agent
        // programmer, not the framework
        // programmer. Given that this was also the structure that I found, I decided to leave it
        // that way. kai, jun'11

        eventsManager.processEvent(eventsManager.getFactory().createAgentArrivalEvent(now, id,
                currentLinkId, this.mode));

        if (teleportationBehavior != null) {
            teleportationBehavior = null;
        }
        else {
            drivingBehavior.drivingEnded(drivingWorld);
    ...
    {
        // I am told this when the Simulation decides to not move me to my destination over the
        // network, but to teleport me there.
        // This is a little silly - apparently the Simulation thinks that when I'm moved over the
        // network, I can keep track of
        // my current link myself, but when I'm "teleported", I can't do that. But I can! After all,
        // the Simulation just asked me where I
        // want to go. mz
        //
        // Actually no. The teleportation arrival is much later than the teleportation departure.
        // Also, there is no guarantee
        // that the mobsim delivers to where it should deliver. The corresponding notification
        // method for network movement
    ...
    {
        // I am asked this right at the beginning so the Simulation can park my vehicle on that
        // link. mz
        //
        // Many pieces of the code are easier if the agent knows where she is. In most places, this
        // is retrieved over the
    ...
    public Id getDestinationLinkId()
    {
        ...
        return this.currentLinkId; // the agent always has the current link as destination link; it
                                   // actually parks if
    ...
    {
        // The Simulation tells me what vehicle I get to use. Don't know what I should do with that
        // information.
        // I need to remember it so I can give it back when getVehicle is called. mz
        //
        // This is just a back pointer. I would prefer bi-directional pointers (i.e. connectors),
        // but they
        // don't exist in java. As an example: Person may receive a recommendation to change lanes
        // via his/her iPhone.
        // The person would need to be able to do give this info to the steering wheel of the
        // vehicle.
    ...
    {
        // The simulation asks me what vehicle I am using. This is silly. I am an agent! Nobody
        // should use me as a data container.
        // mz
        //
        // agreed. it is used only in two locations; maybe we can get rid of it.
        // (I fact, both places where it is needed have to do with non-physical behavior.) kai,
        // jun'11
        return this.veh;
    }
    ...
    {
        // I think I am told this some time after I was asked about my next link. I think this means
        // that I have entered it now. mz
        //
        // yes, exactly. It may have happened that it was not possible to cross the intersection,
        // and the mobsim may have made you
        // "stuckAndAbort", or a police person may have waved you into a link into which you did not
        // want to go.
        // (It is, however, quite strange that this does not pass on the new link as an argument.)
        // kai, jun'11
        this.nextLinkId = null; // this is set to null because it means the "next" linkId, not the
                                // "current" linkId. kai, jun'11
    ...
    public double getWeight()
    {
        // whatever. mz
        //
        // yyyy well, would be better to set this to "1" since this is, in the end, the space that
        // the agent uses, say in the
        // bus. But it is one of these functions that someone added without thinking it through, and
        // nobody has used so far.
        // kai, jun'11
        return 0;
    }
