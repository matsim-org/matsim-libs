...
/**
 * Because in QSim and JDEQSim vehicles enter and leave traffic at the end of links, we skip the
 * first link when calculating the drive-related energy consumption. However, the time spent on the
 * first link is used by the time-based aux discharge process (see
 * {@link playground.michalm.ev.discharging.AuxDischargingHandler}).
 */
public class DriveDischargingHandler
    implements LinkLeaveEventHandler, VehicleEntersTrafficEventHandler,
    VehicleLeavesTrafficEventHandler
{
    ...
    {
        EVDrive evDrive = dischargeVehicle(event.getVehicleId(), event.getLinkId(),
                event.getTime());
        if (evDrive != null) {
    ...
    private EVDrive dischargeVehicle(Id<Vehicle> vehicleId, Id<Link> linkId, double eventTime)
    {
        ...
        if (evDrive != null && !evDrive.isOnFirstLink()) {// handle only our EVs, except for the first link
            ...
            evDrive.ev.getDriveEnergyConsumption().useEnergy(link, tt);
        }
        return evDrive;
    }
