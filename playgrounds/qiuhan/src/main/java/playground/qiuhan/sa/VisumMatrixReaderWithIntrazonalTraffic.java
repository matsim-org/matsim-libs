 ...
 * @author mrieser
 */
public class VisumMatrixReaderWithIntrazonalTraffic {

	private Matrix matrix = null;

	/* package */static final Logger log = Logger
			.getLogger(VisumMatrixReaderWithIntrazonalTraffic.class);

	public VisumMatrixReaderWithIntrazonalTraffic(final Matrix matrix) {
		this.matrix = matrix;
	}
		...
		try {
			...
			if (header != null) {
				if (header.equals("$VN;Y5")) {
					...
					throw new RuntimeException("Visum file format '" + header
							+ "' is not supported.");
				}
			} else {
		...
		private void parseLine(final String line) {
			...
			if (this.state == STATE_DATA) {
				...
				if (data.length != this.nofZones) {
					throw new RuntimeException("Expected " + this.nofZones
							+ " data items, but found " + data.length
							+ " in line " + this.lineCounter + ".");
				}
				for (int i = 0; i < this.nofZones; i++) {
					this.matrix.setEntry(new IdImpl(
							this.zoneNames[this.zoneCounter]), new IdImpl(
							this.zoneNames[i]), Double.parseDouble(data[i]));
				}
				this.zoneCounter++;
				...
				this.state = STATE_ANZBEZIRKE; // we expect as next the number
												// of zones
				...
				if (this.zoneNames.length != this.nofZones) {
					throw new RuntimeException("Line " + this.lineCounter
							+ ": " + "The actual number of zones ("
							+ this.zoneNames.length + ") does not "
							+ "correspond with the expected number of zones ("
							+ this.nofZones + ").");
				}
				this.zoneCounter = 0;
	...
	/* default */static class SparseMatrixReader {
		 ...
		 * own line together with a from- and to-cell-id, a "sparse matrix"
		 * format, as it is most effective for storing sparse matrices.
		 */
		private final static int STATE_HEADER = 0;
		...
		private void parseLine(final String line) {
			...
			if (this.state == STATE_DATA) {
				...
				if (data.length != 3) {
					throw new RuntimeException("Expected 3 tokens, but found "
							+ data.length + " in line " + this.lineCounter
							+ ".");
				}
				...
				/*
				 * There is no intrazonal traffic in VISUM. However, when there
				 * comes a matrix entry from VISUM  for a tuple of one and the
				 * same zone, store just the value. This does not mean that the
				 * value does not need to be computed otherwise in MATSim.
				 */
				...
				// we ignore the header line
				this.state = STATE_VERKEHRSMITTEL; // we expect as next the
													// Verkehrsmittel-Nr
