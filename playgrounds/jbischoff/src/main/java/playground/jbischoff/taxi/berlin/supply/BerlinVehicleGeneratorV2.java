...
public class BerlinVehicleGeneratorV2 {
	...
	private void createTaxis(int starthr, int endhr){
		...
		for (int i = starthr; i<= endhr; i++ ){
			...
			String filename = DATADIR+"OD/demandperLorHr/OD_201304"+day.toString()+hrstring+"_OD.csv";
	...
	private void removeTaxis(int amount, int hr) {
		for (int i = 0; i<amount; i++ ){
			...
			if (v.t0/3600 > hr) offset = offset + 24*3600;
			v.t1 = hr*3600 + offset;
			vdata.add(v);
	...
	private Map<Integer, Integer> calculateAverageTaxisOverTime(
			Map<Integer, Integer> taxisOverTime2) {
	...
	class TaxiTimeParser implements TabularFileHandler{
	...
	class VData implements Comparable<VData>{
