...
public class CreateSharedTaxiDemand {
	Geometry fromGeo;
	Scenario scenario;
	...
	private void run() {
		...
		fromGeo = JbUtils.readShapeFileAndExtractGeometry(input+"taxibus-data/citycenteronezone.shp", "id").get("center");
		...
		for (int i = 0; i<persons; i++ ){
			...
			Coord to = getRandomPointInFeature(fromGeo);
			
			String type = "train08";
			Id<Person> pid = Id.createPersonId("p_"+i);
			double departureTime = 7.5*3600;
			if (i>persons/2){
				departureTime = 8.5*3600;
				type = "train09";
			}
			scenario.getPopulation().addPerson(createAgent(pid, home, to, departureTime, type));
			
		}
		new PopulationWriter(scenario.getPopulation()).write(input+"sharedtaxi_population_"+persons+".xml");
		
		
	}
	
	Person createAgent(Id<Person> pId, Coord homeCoord, Coord to, double outboundDepartureTime, String destActName){
		...
		plan.addLeg(leg);
		Activity work = fac.createActivityFromCoord(destActName, to);
	...
	public static void main(String[] args) {
		new CreateSharedTaxiDemand().run();
	}
