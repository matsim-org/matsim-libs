...
package playground.jbischoff.wobscenario.braunschweig;
...
public class CreateStatebasedTaxibusDemand {
	Geometry fromGeo;
	Scenario scenario;
	...
	private void run() {
		...
		for (int i = 0; i<persons; i++ ){
			...
			double departureTime = 7.25*3600+locationRandom.nextInt(1800);
		
			scenario.getPopulation().addPerson(createAgent(pid, home, departureTime, type));
			
		}
		new PopulationWriter(scenario.getPopulation()).write(input+"statebasedPop_"+persons+".xml");
	...
	public static void main(String[] args) {
		new CreateStatebasedTaxibusDemand().run();
	}
