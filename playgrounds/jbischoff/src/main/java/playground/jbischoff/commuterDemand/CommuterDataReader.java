		...
		while ((line = br.readLine()) != null) {
			    ...
			    if (!result[0].equals("")) {
			    	...
			    	if (currentTo.equals("")) {
			    		...
			    		continue;
			    	} else if (this.filteredMunicipalities.contains(currentTo) && this.filteredMunicipalities.contains(currentFromCode))
			    	
			    	try{
			    	int commuters = Integer.parseInt(result[4]);
			    	CommuterDataElement current = new CommuterDataElement(currentFromCode,currentTo, commuters);
