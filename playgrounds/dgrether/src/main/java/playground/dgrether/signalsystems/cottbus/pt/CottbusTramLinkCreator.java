...
public class CottbusTramLinkCreator {
	...
	public static void main(String[] args) {
		...
		String cbdir;
		if (args.length == 0){
			cbdir = "\\\\vsp-nas\\jbischoff\\WinHome\\Docs\\cottbus\\cottbus_feb_fix\\";
		}
		else
		cbdir = args[0];
		String netfile = cbdir+"network_wgs84_utm33n.xml.gz";
		...
		FileReader frr;
		try {
			fr = new FileReader(new File (cbdir+"Cottbus-pt/lines/ptadditions.csv"));
			...
			br.close();			
			frr = new FileReader(new File (cbdir+"Cottbus-pt/lines/ptadditions.csv"));	
...
//			manual fixes
			...
			new NetworkWriter(network).write(cbdir+"Cottbus-pt/network_pt.xml"); 
			brr.close();
		}
