...
public class SelectZones {
	...
	public SelectZones(HashMap<String, Zone> zones, String filename) {
		this.zones = zones; 
	...
	private void readLimitingPolygonSHP(String filename) {
		Collection<SimpleFeature> featureCollection = ShapeFileReader.getAllFeatures(filename);
		ArrayList<SimpleFeature> features = new ArrayList<SimpleFeature>(featureCollection);
		...
		if(!features.isEmpty()) {
			...
			try {
				Geometry geometry = wktreader.read(feature.getAttribute("the_geom").toString());
