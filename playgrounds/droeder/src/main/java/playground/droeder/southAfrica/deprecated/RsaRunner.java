...
import playground.droeder.southAfrica.Mode2LineSetterRSA;


/**
 * @deprecated This class uses an overwrites {@link Controler} and it's run-method which is
 * no longer permitted.
 * 
 * @author droeder
 *
...
//		Scenario scenario = ScenarioUtils.loadScenario(ConfigUtils.loadConfig(conf));
		...
		// actually I'm not sure if this is still necessary
		ActivityParams transitActivityParams = new ActivityParams(PtConstants.TRANSIT_ACTIVITY_TYPE);
		transitActivityParams.setTypicalDuration(120.0);
		scenario.getConfig().planCalcScore().addActivityParams(transitActivityParams);
		
		PHook pHook = new PHook(controler, new Mode2LineSetterRSA(), (PTransitRouterFactory) controler.getTransitRouterFactory(), null, PtSubModeTripRouterFactory.class);
