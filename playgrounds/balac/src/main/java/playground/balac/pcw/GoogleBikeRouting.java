...
public class GoogleBikeRouting {
	...
	public static void main(String[] args) throws Exception {
		...
		final BufferedWriter writer = new BufferedWriter(new FileWriter("C:\\LocalDocuments\\Projects\\PCW\\Routing\\clean_weg_correction1_google_bike.csv"));
		reader.readLine();
		String s = reader.readLine();
		GeoApiContext context = new GeoApiContext().setApiKey("AIzaSyBEkVBy4u7UVyvavAfzoeQmczD_CdGw7pA");
		writer.write("tripid;traveltime;distance");
		writer.newLine();
		while (s != null) {
			 String[] arr = s.split(";");
			 if (!arr[1].equals("-99")) {
			...
			double distance = 0.0;
					...
					DirectionsRoute[] route = DirectionsApi.getDirections(context, arr[1] + " " + arr[2] ,
							arr[3] + " " + arr[4]).mode(TravelMode.BICYCLING).departureTime(time).await();
