...
public class GoogleWalkRouting {
	...
	public static void main(String[] args) throws Exception {
		...
		final BufferedWriter writer = new BufferedWriter(new FileWriter("C:\\LocalDocuments\\Projects\\PCW\\Routing\\clean_weg_correction1_google_walk.csv"));
		reader.readLine();
		...
		while (s != null) {
			 ...
			 int h = (int) (m / 60);
			 int min = (int)m - h * 60;
				DateTime time = new DateTime(2016, 12, 22, h, min, DateTimeZone.getDefault());
			...
			double distance = 0.0;
					...
					DirectionsRoute[] route = DirectionsApi.getDirections(context, arr[1] + " " + arr[2] ,
							arr[3] + " " + arr[4]).mode(TravelMode.WALKING).departureTime(time).await();
