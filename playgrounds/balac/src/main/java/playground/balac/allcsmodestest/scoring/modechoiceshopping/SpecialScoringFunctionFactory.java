...
import playground.balac.allcsmodestest.scoring.CarsharingWithTaxiLegScoringFunction;
import playground.balac.allcsmodestest.scoring.DesiresAndOpenTimesActivityScoring;

public class SpecialScoringFunctionFactory extends org.matsim.core.scoring.functions.CharyparNagelScoringFunctionFactory {
	
	private final Config config;
	private final Network network;
	private final Scenario scenario;

	public SpecialScoringFunctionFactory(Config config, Network network, Scenario scenario)
	  {
	...
	@Override
	  public ScoringFunction createNewScoringFunction(Person person)
	  {
	    ...
	    //this is the main difference, since we need a special scoring for carsharing legs
	      ...
	      this.config, 
	      network));
		  scoringFunctionSum.addScoringFunction(new ShoppingByCarScoringFunction(person.getSelectedPlan(), this.scenario));
