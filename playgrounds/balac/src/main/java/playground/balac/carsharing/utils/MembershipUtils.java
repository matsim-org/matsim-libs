...
public class MembershipUtils {
	...
	public static final QuadTree<Person> createPersonQuadTree(MutableScenario scenario) {
	    ...
	    for (Person p : scenario.getPopulation().getPersons().values()) {
	      Coord c = ((ActivityFacility)scenario.getActivityFacilities().getFacilities().get(PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getFacilityId())).getCoord();
