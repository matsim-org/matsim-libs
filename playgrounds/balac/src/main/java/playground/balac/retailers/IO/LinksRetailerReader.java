...
public class LinksRetailerReader
{
  ...
  public LinksRetailerReader(MatsimServices controler, Retailers retailers)
  {
  ...
  public void updateFreeLinks()
  {
    ...
    for (Iterator<LinkRetailersImpl> localIterator1 = this.allLinks.values().iterator(); localIterator1.hasNext(); ) { link = localIterator1.next();
      links.put(link.getId(), link);
    }
    for (Iterator<LinkRetailersImpl> localIterator1 = this.currentLinks.values().iterator(); localIterator1.hasNext(); ) { link = localIterator1.next();
  ...
  private void createFreeLinks() {
    ...
    while (this.freeLinks.size() < newLinksMax.intValue()) {
      ...
      if (this.currentLinks.containsKey(link.getId())) { log.info("On the link " + link.getId() + " there is already a facility");
        ...
        double ratio = (1.0D / 0.0D);
        Collection<ActivityFacility> facilities = Utils.getFacilityQuadTree().getDisk(link.getCoord().getX(), link.getCoord().getY(), 1000.0D);
        double globalCapacity = 0.0D;
        double numberPersons = 0.0D;
        int numberShops = 0;
        for (ActivityFacility facility : facilities) {
        ...
        numberPersons = Utils.getPersonQuadTree().getDisk(link.getCoord().getX(), link.getCoord().getY(), 1000.0D).size();
