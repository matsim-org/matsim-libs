  ...
  private void createFreeLinks() {
    ...
    while (this.freeLinks.size() < newLinksMax.intValue()) {
      ...
      int rd = MatsimRandom.getRandom().nextInt(numberLinks);
        LinkRetailersImpl link = new LinkRetailersImpl((Link) this.controler.getScenario().getNetwork().getLinks().values().toArray()[rd], (NetworkImpl) this.controler.getScenario().getNetwork(), Double.valueOf(0.0D), Double.valueOf(0.0D));
