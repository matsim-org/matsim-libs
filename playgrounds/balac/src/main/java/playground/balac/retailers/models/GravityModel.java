...
public class GravityModel extends RetailerModelImpl
{
  ...
  private ArrayList<Double> computeNormalPotential(ArrayList<Integer> solution)
  {
    ...
    for (ActivityFacility c : this.retailerFacilities.values())
    {
      ...
      for (PersonRetailersImpl pr : this.retailersPersons.values())
      {
        ...
        ActivityFacility firstFacility = this.controlerFacilities.getFacilities().get(PopulationUtils.getFirstActivity( ((PlanImpl)pr.getSelectedPlan()) ).getFacilityId());
        ...
        if (pr.getGlobalShopsUtility() == 0.0D) {
          ...
          for (ActivityFacility s : this.shops.values()) {
            ...
            for (ActivityFacility af : this.retailerFacilities.values()) {
              ...
              else if (CoordUtils.calcEuclideanDistance(s.getCoord(), PopulationUtils.getFirstActivity( ((PlanImpl)pr.getSelectedPlan()) ).getCoord()) == 0.0D) {
                dist = 10.0D;
              }
              else
              {
                dist = CoordUtils.calcEuclideanDistance(s.getCoord(), PopulationUtils.getFirstActivity( ((PlanImpl)pr.getSelectedPlan()) ).getCoord());
              }
              ++count;
            }
  ...
  private void createZonesFromPersonsShops(int n) {
    ...
    for (Person p : this.persons.values()) {
      if (PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getCoord().getX() < minx) minx = PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getCoord().getX();
      if (PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getCoord().getY() < miny) miny = PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getCoord().getY();
      if (PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getCoord().getX() > maxx) maxx = PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getCoord().getX();
      if (PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getCoord().getY() <= maxy) continue; maxy = PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getCoord().getY();
    }
    ...
    while (i < n) {
      ...
      while (j < n) {
        ...
        for (Person p : this.persons.values()) {
          c = ((ActivityFacility)this.controlerFacilities.getFacilities().get(PopulationUtils.getFirstActivity( ((PlanImpl)p.getSelectedPlan()) ).getFacilityId())).getCoord();
