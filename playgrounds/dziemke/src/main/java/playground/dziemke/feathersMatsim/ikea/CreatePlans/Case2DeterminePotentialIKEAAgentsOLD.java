...
public class Case2DeterminePotentialIKEAAgentsOLD {
	...
	// Creates an HashSet containing all Agents fulfilling following criteria:
		...
		while((line=reader.readLine())!=null){
			...
			if(Integer.parseInt(parts[index_activityType])==4
				...
				if(distanceHomeIKEA<=radius&&distancePriorActivityIKEA<=radius){
					...
					double p_agent=
							Math.exp(-0.02*(distanceHomeIKEA/1000));
