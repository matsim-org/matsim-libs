...
public class FileModifierZones {

	// specify in- and output files and number of last column that stays unchanged
	// start counting column with 1 (not with 0).
	private String inputFile = new String("D:/Workspace/container/demand/input/cemdap_samples/zones.dat");
	private String outputFile = new String("D:/Workspace/container/demand/input/cemdap_samples_reduced/zones.dat");
	
	// private int numberOfLastUnchangedColumn = 14; 
	
	
	public static void main(String[] args) {
		FileModifierZones zoneChanger = new FileModifierZones();
		zoneChanger.run();
	}

	
	public void run() {
		read(this.inputFile, this.outputFile);
	}
