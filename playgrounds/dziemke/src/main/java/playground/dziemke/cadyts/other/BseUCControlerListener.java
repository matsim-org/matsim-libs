...
//			try {
//				DynamicData<Link> linkCostOffsets = calibrator.getLinkCostOffsets();
...
//		error somewhere in following two lines; so refactor them to make a bit clearer what they do
//		distanceFilterCenterNodeCoord = net.getNodes().get(
//				new IdImpl(config.counts().getDistanceFilterCenterNode())).getCoord();
		
		String id = config.counts().getDistanceFilterCenterNode();
		IdImpl idForNoIdeaForWhat = new IdImpl(id);
		Node nodeForNoIdeaForWhat = net.getNodes().get(idForNoIdeaForWhat);
		distanceFilterCenterNodeCoord = nodeForNoIdeaForWhat.getCoord();
