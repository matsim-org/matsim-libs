 ...
 *
 */
public class CemdapStops2MatsimPlansConverterOld {

	private final static Logger log = Logger.getLogger(CemdapStops2MatsimPlansConverterOld.class);
	...
	public static void main(String[] args) throws IOException {
		...
		int planNumber = 0;
		new CemdapStopsParserOld().parse(cemdapStopsFile, planNumber, scenario, personObjectAttributes, popFraction);
		log.info("done. (parsing)");
		
		log.info("assigning coordinates to activities...");
		new Feature2CoordOld().assignCoords(scenario, planNumber, personObjectAttributes, cemdapTazFeatures);
		...
		if (multiplePlans == true) {
			...
			planNumber = 1;
			new CemdapStopsParserOld().parse(cemdapStopsFile2, planNumber, scenario, personObjectAttributes2, popFraction);
			log.info("done. (parsing)");
	
			log.info("assigning coordinates to activities...");
			new Feature2CoordOld().assignCoords(scenario, planNumber, personObjectAttributes2, cemdapTazFeatures);
			log.info("done. (assigning)");
			// ----------- END SECOND PLAN ---------------
			...
			planNumber = 2;
			new CemdapStopsParserOld().parse(cemdapStopsFile3, planNumber, scenario, personObjectAttributes3, popFraction);
			log.info("done. (parsing)");
	
			log.info("assigning coordinates to activities...");
			new Feature2CoordOld().assignCoords(scenario, planNumber, personObjectAttributes3, cemdapTazFeatures);
			log.info("done. (assigning)");
			// ----------- END THIRD PLAN ---------------
		}
		...
		if (addStayHomePlan == true) {
			...
			planNumber = 3;
			new CreateStayHomePlanOld().create(cemdapStopsFile, planNumber, scenario, personObjectAttributes, popFraction);
