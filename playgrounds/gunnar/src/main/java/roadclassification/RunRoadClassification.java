...
public class RunRoadClassification {
	...
	public static void optimize(final Scenario scenario) {
		...
		final ObjectiveFunction objectiveFunction = new RoadClassificationObjectiveFunction(counts);
		...
		DecisionVariableRandomizer<RoadClassificationDecisionVariable> randomizer = new DecisionVariableRandomizer<RoadClassificationDecisionVariable>() {
			// @Override
			// public RoadClassificationDecisionVariable
			// newRandomDecisionVariable() {
			// ArrayList<LinkSettings> linkSettingses = new
			// ArrayList<>(almostRealLinkSettings);
			// return new
			// RoadClassificationDecisionVariable(scenario.getNetwork(),
			// linkAttributes, linkSettingses);
			// }

			@Override
			public List<RoadClassificationDecisionVariable> newRandomVariations(
					RoadClassificationDecisionVariable decisionVariable) {
				/*
				 * The algorithm performs best if this function returns two
				 * symmetric variations of the decision variable.
				 */
				RoadClassificationDecisionVariable var1 = new RoadClassificationDecisionVariable(scenario.getNetwork(),
						linkAttributes, new ArrayList<>(decisionVariable.getLinkSettingses()));
				RoadClassificationDecisionVariable var2 = new RoadClassificationDecisionVariable(scenario.getNetwork(),
						linkAttributes, new ArrayList<>(decisionVariable.getLinkSettingses()));
				for (int i = 0; i < decisionVariable.getLinkSettingses().size(); i++) {
					if (MatsimRandom.getRandom().nextDouble() < 2.0 / decisionVariable.getLinkSettingses().size()) {
						...
						var1.getLinkSettingses().set(i,
								new LinkSettings(var1.getLinkSettingses().get(i).getFreespeed(),
										Math.max(var1.getLinkSettingses().get(i).getCapacity() + offset, 0.0),
										var1.getLinkSettingses().get(i).getNofLanes()));
						var2.getLinkSettingses().set(i,
								new LinkSettings(var2.getLinkSettingses().get(i).getFreespeed(),
		...
		// Discretizize the day into 24 one-hour time bins, starting at
		// midnight.
		final TimeDiscretization timeDiscretization = new TimeDiscretization(0, 3600, 24);

		RandomSearch<RoadClassificationDecisionVariable> randomSearch = new RandomSearch<>(
				new MATSimSimulator<RoadClassificationDecisionVariable>(stateFactory, scenario, timeDiscretization),
				// null, null),
				randomizer,
				new RoadClassificationDecisionVariable(scenario.getNetwork(), linkAttributes,
						new ArrayList<>(almostRealLinkSettings)),
				convergenceCriterion,
				// selfTuner,
				maxIterations, maxTransitions, populationSize, MatsimRandom.getRandom(), interpolate, objectiveFunction,
				false);
		...
		// for (DecisionVariable decisionVariable :
		// randomSearch.getBestDecisionVariablesView()) {
		// log.info("--DecisionVariable follows--");
		// RoadClassificationDecisionVariable rcdv =
		// (RoadClassificationDecisionVariable) decisionVariable;
		// for (LinkSettings linkSettings : rcdv.getLinkSettingses()) {
		// log.info(String.format("%d %d %d\n", (int)
		// linkSettings.getCapacity(), (int) linkSettings.getFreespeed(), (int)
		// linkSettings.getNofLanes()));
		// }
		// }
		...
		// final MATSimDecisionVariableSetEvaluator<RoadClassificationState,
		// RoadClassificationDecisionVariable> predictor = new
		// MATSimDecisionVariableSetEvaluator<RoadClassificationState,
		// RoadClassificationDecisionVariable>(
	...
	private static LinkSettings addNoise(LinkSettings linkSettings) {
		...
		return new LinkSettings(linkSettings.getFreespeed(), linkSettings.getCapacity() + noise,
				linkSettings.getNofLanes());
	}
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		countsReader.read(countsFileName);
