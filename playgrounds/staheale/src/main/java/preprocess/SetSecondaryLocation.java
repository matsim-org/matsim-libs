...
public class SetSecondaryLocation  {
	...
	public void run() {
		...
		pw.writeStartPlans("./output/population2010final.xml.gz");
		...
		for (Person p : population.getPersons().values()) {
			...
			for (PlanElement pe : p.getSelectedPlan().getPlanElements()) {
				if (pe instanceof ActivityImpl) {
					...
					else if (L.equals(act.getType()) || S.equals(act.getType())){
						secondaryAct = true;
				}
				}
			}
			...
			//
				...
				for (PlanElement pe : p.getSelectedPlan().getPlanElements()) {
					if (pe instanceof ActivityImpl) {
						ActivityImpl act = (ActivityImpl) pe;
						if (L.equals(act.getType()) || S.equals(act.getType())) {
							...
							ActivityFacilityImpl f = getFacility(coord1,coord2,radius,act.getType());
							act.setCoord(f.getCoord());
							act.setFacilityId(f.getId());
						}
					}
