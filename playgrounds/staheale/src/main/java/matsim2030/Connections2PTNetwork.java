...
public class Connections2PTNetwork {
	...
	public void run() throws Exception {
		...
		NetworkReader.readFile("./input/02-OEV_DWV_2005-HAFAS_adaptedV2.xml");
		...
		ScheduleReader.readFile("./input/02-OEV_DWV_2005-HAFAS_adaptedV2_TransitSchedule.xml");
		...
		VehicleReader.parse("./input/02-OEV_DWV_2005-HAFAS_adaptedV2_PTVehicles.xml"); 
		...
		File file = new File("./input/Bezirk_Knoten_Anbindungen.csv");
		...
		while ((curr_line = bufRdr.readLine()) != null) {
			...
			Node bezirkNode = null;
			TransitStopFacility bezirkStop = null;
			Id bezirkNodeId = sc.createId(bezirkNummer);

			if (bezirkNodeId != bezirkNodeIdVorher) {
				...
				bezirkNode = (NodeImpl) factory.createNode(bezirkNodeId, coords);
				PTnetwork.addNode(bezirkNode);
				
				bezirkList.add(Arrays.asList(bezirkNummer,stadtnummer));	
				
				//log.info("bezirkNode " +bezirkNode.getId()+ " added to network");

				// add district as a stop facility to schedule
				bezirkStop = scheduleFactory.createTransitStopFacility(bezirkNodeId, coords, false);
				PTschedule.addStopFacility(bezirkStop);
				bezirkStop.setName(bezirkName);
				//log.info("stop facility " +bezirkStop.getName()+ " added to schedule");
			}
			else {
				bezirkNode = PTnetwork.getNodes().get(bezirkNodeId);
				bezirkStop = PTschedule.getFacilities().get(bezirkNodeId);
			}
			...
			if (length != 0) {
				...
				PTnetwork.addLink(newLink2);
				//log.info("link2 " +newLink2.getId()+ " added to network");


				// add lines between district and node to schedule
				TransitStopFacility existingStop = PTschedule.getFacilities().get(existingNodeId);
				List<Id> routeLinkIds1 = Arrays.asList(bezirkNodeId, existingNodeId);
				TransitRouteStop route1stop1 = scheduleFactory.createTransitRouteStop(bezirkStop, 0, 0);
				TransitRouteStop route1stop2 = scheduleFactory.createTransitRouteStop(existingStop, 300, 300);			
				...
				PTschedule.addTransitLine(newLine);
				//log.info("new line " +newLine.getId()+ " added to schedule");


				// opposite direction
				List<Id> routeLinkIds2 = Arrays.asList(existingNodeId, bezirkNodeId);
				TransitRouteStop route2stop1 = scheduleFactory.createTransitRouteStop(existingStop, 0, 0);
				...
				PTschedule.addTransitLine(newLine2);
        ...
        for (List<String> arr1 : bezirkList) {
        	...
        	for (List<String> arr2 : bezirkList) {
        		...
        		if (arr2.get(1).equals(arr1.get(1)) && doneList.contains(arr2.get(0)) == false){ //&& currentNodeId1 != currentNodeId2 
                	...
                	//		", stadtnummer1 is: " +stadtNummer1+ ", stadtnummer2 is: " +stadtNummer2);
    				...
    				List<Id> newRouteLinkIds1 = Arrays.asList(currentNodeId1, currentNodeId2);
    				double speed = 20/3.6; // assumption: pt speed is 20 km/h
    				double travelTime = Math.round(dist/speed);
    				TransitRouteStop currentRoute1stop1 = scheduleFactory.createTransitRouteStop(currentStop1, 0, 0);
    				TransitRouteStop currentRoute1stop2 = scheduleFactory.createTransitRouteStop(currentStop2, travelTime, travelTime);			
    				...
    				List<Id> newRouteLinkIds2 = Arrays.asList(currentNodeId2, currentNodeId1);
    				TransitRouteStop currentRoute2stop1 = scheduleFactory.createTransitRouteStop(currentStop2, 0, 0);
    				TransitRouteStop currentRoute2stop2 = scheduleFactory.createTransitRouteStop(currentStop1, travelTime, travelTime);			
		...
		nw.write("./output/02-OEV_DWV_2005-HAFAS_adaptedV2_anbindungen.xml");

		TransitScheduleWriter sw = new TransitScheduleWriter(PTschedule);
		sw.writeFile("./output/02-OEV_DWV_2005-HAFAS_adaptedV2_TransitSchedule_anbindungen.xml");

		VehicleWriterV1 vw = new VehicleWriterV1(PTvehicles);
		vw.writeFile("./output/02-OEV_DWV_2005-HAFAS_adaptedV2_PTVehicles_anbindungen.xml");
	}
}
