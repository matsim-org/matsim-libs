...
public class CbNETollTimeDistanceTravelDisutility implements TravelDisutility {
	...
	public CbNETollTimeDistanceTravelDisutility(TravelDisutility randomizedTimeDistanceTravelDisutility,
			TravelTime timeCalculator,
			EmissionModule emissionModule, EmissionResponsibilityCostModule emissionCostModule,
			NoiseContext noiseContext,
			DecongestionInfo decongestionInfo,
	...
	private double calculateExpectedTollDisutility(final Link link, final double time, final Person person, final Vehicle vehicle) {
		...
		int timeBin = (int) (time / decongestionInfo.getScenario().getConfig().travelTimeCalculator().getTraveltimeBinSize());
		
		double toll = 0.;
		if (decongestionInfo.getlinkInfos().containsKey(link.getId()) && decongestionInfo.getlinkInfos().get(link.getId()).getTime2toll().containsKey(timeBin)) {
			toll = decongestionInfo.getlinkInfos().get(link.getId()).getTime2toll().get(timeBin);
		}		
		double expectedLinkCongestionTollDisutility = -1 * this.marginalUtilityOfMoney * toll;			
