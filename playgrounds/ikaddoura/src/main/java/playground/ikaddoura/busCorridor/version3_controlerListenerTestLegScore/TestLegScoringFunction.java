...
public class TestLegScoringFunction implements LegScoring, BasicScoring {

	private final static Logger log = Logger.getLogger(TestLegScoringFunction.class);
	protected final Plan plan;

	protected double score;
	private double lastTime;
	private int index; // the current position in plan.actslegs
	private double fare = 2.3; // fare muss irgendwie aus dem eventsmanager herausgeholt werden... ?!
	...
	public TestLegScoringFunction(final Plan plan, final CharyparNagelScoringParameters params) {
		this.params = params;
		this.reset();
		this.plan = plan;
	}
	...
	protected double calcLegScore(final double departureTime, final double arrivalTime, final Leg leg) {
		...
		if (TransportMode.car.equals(leg.getMode())) {
			...
			tmpScore += travelTime * this.params.marginalUtilityOfTravelingPT_s + this.params.marginalUtilityOfDistancePt_m * dist + this.params.marginalUtilityOfMoney * fare;
