...
public class MyLegScoringFunction implements MyLegScoring, BasicScoring {
	...
	public MyLegScoringFunction(final Plan plan, final CharyparNagelScoringParameters params, Map<Id, Double> personId2WaitingTime, Map<Id, Double> personId2InVehicleTime) {
		this.params = params;
		this.reset();
		this.plan = plan;
	...
	protected double calcLegScore(final double departureTime, final double arrivalTime, final Leg leg) {
		...
		if (TransportMode.car.equals(leg.getMode())) {
			...
			System.out.println("scoring of: "+plan.getPerson().getId());
			double inVehicleTime = this.personId2InVehicleTime.get(plan.getPerson().getId());
//			double waitingTime = this.personId2WaitingTime.get(plan.getPerson().getId());
//			if ((inVehicleTime + waitingTime) != travelTime){
//				System.out.println("inVehicleTime und waiting time ist ungleich traveltime!!!");
//			}
			if (this.params.marginalUtilityOfDistancePt_m != 0.0) {
				dist = leg.getRoute().getDistance();
			}
//			System.out.println("Waiting Time: "+waitingTime);
//			System.out.println("In-Vehicle Time: "+inVehicleTime);
