...
public class DateTime {
	...
	public static final double parseDateTimeToSeconds( String dateTimeString ) {
		double timeSec = 0.;
		double dateSec = 0.;
		...
		if (datetime.length == 1) {
			if (datetime[0].contains(":")) {
				// only time
				timeSec = Time.parseTimeToSeconds(datetime[0]);
			...
			// date + time
			dateSec = computeDateSec(datetime[0]);
			timeSec = Time.parseTimeToSeconds(datetime[1]);
		} else {
			throw new RuntimeException("Expecting the traffic incidents to have the date+time format YYYY/MM/DD HH:MM:SS. This is something else: " + dateTimeString + " Aborting...");
		}
	...
	private static double computeDateSec(String dateString) {
		...
		if (date.length != 3 || date[0].length() != 4 || date[1].length() != 2 || date[2].length() != 2) {
			throw new RuntimeException("Expecting the traffic incidents to have the date format YYYY/MM/DD. This is something else: " + dateString + " Aborting...");
		}
