...
/**
 * Config group to configure missing scoring parameters of {@link BvgLegScoringFunctionPR} and {@link BvgActivityScoringFunctionPR}
 * 
 * @author aneumann, adjusted for P+R by ikaddoura
 *
 */
public class BvgScoringFunctionConfigGroupPR extends ConfigGroup {
	
	private static final Logger log = Logger.getLogger(BvgScoringFunctionConfigGroupPR.class);
	
	// Tags
	
	public static final String GROUP_NAME = "bvgScoringPR";
	...
	private double interModalTransferPenalty = -0.0;
	...
	private void addParam(Config config){
		...
		this.setInterModalTransferPenalty(Double.parseDouble(config.getParam(GROUP_NAME, INTERMODAL_TRANSFER_PENALTY)));
	}
	...
	public void addParam(final String key, final String value) {
		...
		if (OFFSET_CAR.equals(key)) {
			...
			this.setInterModalTransferPenalty(Double.parseDouble(value));
		}
	...
	public TreeMap<String, String> getParams() {
		...
		map.put(INTERMODAL_TRANSFER_PENALTY, Double.toString(this.interModalTransferPenalty));
	...
	public final Map<String, String> getComments() {
		...
		String transferDefaultMessage = "[utils/transfer] normally negative utility for park-and-ride transfers";
		
		map.put(INTERMODAL_TRANSFER_PENALTY, transferDefaultMessage);
