...
public class ReplannerOldPeople extends WithinDayDuringActivityReplanner {
	...
	public boolean doReplanning(WithinDayAgent withinDayAgent) {
		...
		executedPlan.insertLegAct(executedPlan.getActLegIndex(currentActivity) + 1, legToNewWork, newWorkAct);
		
		// replan the new Legs
		new EditRoutes().replanFutureLegRoute(executedPlan, executedPlan.getActLegIndex(legToNewWork), routeAlgo);
		new EditRoutes().replanFutureLegRoute(executedPlan, executedPlan.getActLegIndex(homeLeg), routeAlgo);
