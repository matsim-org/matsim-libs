...
public class MainDensityAnalysisWithPt {
	...
	public static void main(String[] args) {
		...
		String eventsFile = "H:/thesis/output_no_pricing_v3_1pct/ITERS/it.50/50.events.xml.gz";
		...
		Map<Id, ? extends Link> links = NetworkReadExample.getNetworkLinks(
		...
		HashMap<Id, int[]> linkInFlow = inflowHandler.getLinkInFlow(); // get
																		// the
																		// matrix
	...
	public static HashMap<Id, int[]> deltaFlow(HashMap<Id, int[]> linkInFlow,
		...
		for (Id linkId : linkInFlow.keySet()) {
			...
			for (int i = 0; i < inflowBins.length; i++) {
				...
				if (deltaflowBins == null || inflowBins == null
						|| outflowBins == null) {
					System.out.println();
				}
	...
	public static HashMap<Id, double[]> calculateDensity(
			HashMap<Id, int[]> deltaFlow, Map<Id, ? extends Link> links) {
