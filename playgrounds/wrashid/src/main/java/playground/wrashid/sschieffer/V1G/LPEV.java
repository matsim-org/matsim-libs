...
public class LPEV {
	...
	public Schedule solveLP(Schedule schedule, Id id, double batterySize, double batteryMin, double batteryMax) throws LpSolveException{
		...
		try {
			...
			solver.setOutputfile(playground.wrashid.sschieffer.V1G.Main.outputPath+"\\LP\\EV\\LPEV_V1G_agent"+ personId.toString()+"printLp.txt");
			solver.printLp();
			
			solver.setOutputfile(playground.wrashid.sschieffer.V1G.Main.outputPath+"\\LP\\EV\\LPEV_V1G_agent"+ personId.toString()+"objective.txt");
			solver.printObjective();
			
			solver.setOutputfile(playground.wrashid.sschieffer.V1G.Main.outputPath+"\\LP\\EV\\LPEV_V1G_agent"+ personId.toString()+"tableau.txt");
			solver.printTableau();
		} catch (Exception e) {	    
		}
