...
public class LPPHEV {
	...
	public Schedule solveLP(Schedule schedule, Id id) throws LpSolveException{
		...
		try {
			...
			solver.setOutputfile(playground.wrashid.sschieffer.V1G.Main.outputPath+"\\LP\\PHEV\\LPPHEV_V1G_agent"+ personId.toString()+"printLp.txt");
			solver.printLp();
			
			solver.setOutputfile(playground.wrashid.sschieffer.V1G.Main.outputPath+"\\LP\\PHEV\\LPPHEV_V1G_agent"+ personId.toString()+"objective.txt");
			solver.printObjective();
			
			solver.setOutputfile(playground.wrashid.sschieffer.V1G.Main.outputPath+"\\LP\\PHEV\\LPPHEV_V1G_agent"+ personId.toString()+"tableau.txt");
			solver.printTableau();
		} catch (Exception e) {	    
		}
	...
	public double calcEnergyUsageFromCombustionEngine() throws LpSolveException{
		...
		for(int i=0; i<schedule.getNumberOfEntries(); i++){
			if(schedule.timesInSchedule.get(i).isParking()){
				...
				statusJoule+=((ParkingInterval)schedule.timesInSchedule.get(i)).getChargingSpeed()
							*solution[1+i];
				...
				statusJoule+=(-1)*((DrivingInterval)schedule.timesInSchedule.get(i)).getConsumption();
		...
		double sec2Charge3500= energyFromEngine/3500;
		double min2Charge3500= energyFromEngine/3500/60;
		System.out.println("Approx charging time at 3500kW [s]: "+ sec2Charge3500);
		System.out.println("Approx charging time at 3500kW [min]: "+ min2Charge3500);
