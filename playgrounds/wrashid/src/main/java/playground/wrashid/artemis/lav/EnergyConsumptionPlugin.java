 ...
 * 
 * note: this module is compatible with both the quesim and jdeqsim model. the
 * maxAllowedSpeedInNetworkInKmPerHour parameter to the energyConsumptionModel
 * filters out, if the first or last link have been driven by the vehicle.
 * 
 * @author wrashid
 * 
 */
public class EnergyConsumptionPlugin implements LinkEnterEventHandler, LinkLeaveEventHandler, AgentWait2LinkEventHandler,
	...
	public LinkedListValueHashMap<Id, Double> getEnergyConsumptionOfEachLeg() {
		return energyConsumptionOfLegs; 
		//return energyConsumptionOfEachLeg;
	}
	...
	public EnergyConsumptionPlugin(EnergyConsumptionModelLAV_v1 energyConsumptionModel,
		...
		outputLogEnergyConsumptionPerLink
				.add("averageSpeedDrivenInMetersPerSecond\tMaxSpeedInMetersPerSecond\tenergyConsumptionOnLinkInJoule\tPowerTrain\tFuel\tPower\tWeight\tagentId\tlinkEnterTime\tlinkLengthInMeters");
	...
	private void updateEnergyConsumptionOfLeg(Id personId, double linkLeaveTime, Id linkId) {
		...
		if (timeSpendOnLink == GeneralLib.numberOfSecondsInDay || averageSpeedDrivenInMetersPerSecond > link.getFreespeed()) {
			// do not consider events with "zero" travel time and such, which
			// produce higher travel times than
			// allowed due to artifacts from different simulations, e.g.
			// wait2link, etc.
			return;
		}
		// if (averageSpeedDrivenInMetersPerSecond<3){
		// // avoid too small speeds, which lead to negative number in energy
	...
	private double getEnergyConsumptionAndHandleSoc(Id personId, double timeSpendOnLink, Link link, Double linkEnteranceTime) {
		...
		Double energyConsumptionOnLink = energyConsumptionModel
				.getEnergyConsumptionForLinkInJoule(vehicle, timeSpendOnLink, link);
		...
		if (vehicle.powerTrainClass == LAVLib.getBatteryElectricPowerTrainClass()) {
			...
			if (vehicleSOC.getSocInJoule() < energyConsumptionOnLink) {
				...
				energyConsumptionOnLink = energyConsumptionModel.getEnergyConsumptionForLinkInJoule(vehicle, timeSpendOnLink,
						link);
			} else {
