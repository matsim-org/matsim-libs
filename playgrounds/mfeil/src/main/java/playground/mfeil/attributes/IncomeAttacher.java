	...
	protected void runSimplex (String tableFile){
		...
		for (Iterator<Id> iterator = this.agentsMuns.keySet().iterator(); iterator.hasNext();){
			...
			if (this.education.get(id)==11) {
				...
				index.get(count)[5]+=1; // Maturit�tsschule
				index.get(count)[1]++;
				}
				...
				index.get(count)[7]+=1; // H�here Fach- und Berufsausbildung
				index.get(count)[1]++;
			}
			else if (this.education.get(id)==32) {
				index.get(count)[8]+=1; // H�here Fachschule
				index.get(count)[1]++;
			}
				...
				index.get(count)[10]+=1; // Universit�t
				index.get(count)[1]++;
			}
