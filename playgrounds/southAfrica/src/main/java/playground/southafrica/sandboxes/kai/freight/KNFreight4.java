	...
	private static CarrierScoringFunctionFactory createMyScoringFunction(final Scenario scenario) {
		return new CarrierScoringFunctionFactory() {
			...
			public ScoringFunction createScoringFunction(final Carrier carrier) {
				...
				sum.addScoringFunction(new CharyparNagelLegScoring(CharyparNagelScoringParameters.getBuilder(scenario.getConfig().planCalcScore(), scenario.getConfig().scenario()).create(),
						scenario.getNetwork() ) ) ;
				sum.addScoringFunction( new CharyparNagelMoneyScoring(CharyparNagelScoringParameters.getBuilder(scenario.getConfig().planCalcScore(), scenario.getConfig().scenario()).create()) ) ;
