 ...
 * @author jwjoubert
 */
public class Wp3Accel2dAndHistoricSpeedDigiScorer implements DigiScorer {
	final private static Logger LOG = Logger.getLogger(Wp3Accel2dAndHistoricSpeedDigiScorer.class);
	...
	public Wp3Accel2dAndHistoricSpeedDigiScorer(double scale, List<Double> riskThresholds) {
	...
	public void buildScoringModel(String filename) {
		...
		try{
			...
			while((line = br.readLine()) != null && counter.getCounter() < this.maxLines){
				...
				if(
					...
					/*--------------------------------------------------------*/
					/* Update the speed risk space. In this work package we   */
					/* do this by considering the historic average road speed */
					/* instead of the speed limit. If no historic speed is    */
					/* available, we fall back to the road speed limit. To    */
					/* distinguish between the fall-back strategy and ONLY    */
					/* using records WITH historic speed records, we run this */
					/* same class with different input files.                 */                 
					/*                                                        */
					/* See FilterHistoricSpeedRecords.java                    */
					/*--------------------------------------------------------*/
					double speeding;
					if(historicAvg > 0){
		...
		LOG.info("Report on the use of historice versus speed limit as 'speeding' reference:");
		LOG.info(String.format("      Historic: %.2f%%", ((double)useHistoric)/((double)useHistoric+useSpeedlimit)*100));
		LOG.info(String.format("   Speed limit: %.2f%%", ((double)useSpeedlimit)/((double)useHistoric+useSpeedlimit)*100));
	...
	public static void main(String[] args) {
		Header.printHeader(Wp3Accel2dAndHistoricSpeedDigiScorer.class.toString(), args);
		...
		Wp3Accel2dAndHistoricSpeedDigiScorer wp3AS = new Wp3Accel2dAndHistoricSpeedDigiScorer(scale, riskThresholds);
