...
/**
 * <par>This class implements the DJ-DigicoreCluster density-based clustering approach as published
...
public class DJCluster {
	 ...
	 */
	public DJCluster(List<Coord> pointsToCluster, boolean silent){
		...
		this.clusterList = new ArrayList<DigicoreCluster>();
		this.silent = silent;
	}
			...
			while(pointCounter < listOfPoints.size()){
				...
				if(p.getCluster() == null){
					...
					if(neighbourhood.size() < minimumPoints){
						...
						for (DigicoreCluster DigicoreCluster : localClusters) {
...
//								DigicoreCluster.getPoints().remove(thisClusterList.get(j));
			...
			Map<DigicoreCluster, List<ClusterActivity>> clusterMap = new HashMap<DigicoreCluster, List<ClusterActivity>>();
			...
			for (ClusterActivity ca : listOfPoints) {
				DigicoreCluster theCluster = ca.getCluster();
				if(theCluster != null){
...
//				theCluster.setCenterOfGravity();
	...
	public void writeClustersToFile(String filename){
		...
		try {
			...
			try{
				...
				for (DigicoreCluster c : clusterList) {
	...
	public List<DigicoreCluster> getClusterList() {
		return clusterList;
	}
