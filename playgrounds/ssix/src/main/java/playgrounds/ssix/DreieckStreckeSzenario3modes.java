...
public class DreieckStreckeSzenario3modes {

	/**
	 * @param args
	 */
	private static final Logger log = Logger.getLogger(DreieckStreckeSzenario3modes.class);
	...
	public DreieckStreckeSzenario3modes(int networkCapacity){
	...
	public static void main(String[] args) {
		DreieckStreckeSzenario3modes dreieck = new DreieckStreckeSzenario3modes(NETWORK_CAPACITY);
		...
		for (int number_cars = 0; number_cars < 81; number_cars+=10){//8
			for (int number_bikes = 0; number_bikes < 321 - number_cars; number_bikes+=20){//14
				for (int number_bicycles = 0; number_bicycles < 321 - number_cars - number_bikes; number_bicycles +=20){
					dreieck.runParam(number_bicycles, number_bikes, number_cars);
				}
			}
		}
		dreieck.closeFile();*/
	...
	private void runParam(int number_trucks, int number_med, int number_fast){
		createWantedPopulation(number_trucks,number_med,number_fast,2);
		...
		writer.format("%.2f\t", fundi4.getEndDensity());
		writer.format("%.2f\t", fundi4.getEndDensity_truck());
		writer.format("%.2f\t", fundi4.getEndDensity_med());
		writer.format("%.2f\t\t", fundi4.getEndDensity_fast());
		
		writer.format("%.2f\t", fundi4.getEndFlow());
		writer.format("%.2f\t", fundi4.getEndFlow_truck());
		writer.format("%.2f\t", fundi4.getEndFlow_med());
		writer.format("%.2f\t\t", fundi4.getEndFlow_fast());
		
		writer.format("%.2f\t", fundi4.getEndAverageVelocity());
		writer.format("%.2f\t", fundi4.getEndAverageVelocity_truck());
		writer.format("%.2f\t", fundi4.getEndAverageVelocity_med());
		writer.format("%.2f\n", fundi4.getEndAverageVelocity_fast());
		log.info("Done!");
	}
	...
	private void fillNetworkData(){
		...
		for (int i = 0; i<subdivisionFactor+1; i++){
			...
			if (i>0){
				for (int j=0; j<i; j++){
					x += DreieckStreckeSzenario3modes.length/DreieckStreckeSzenario3modes.subdivisionFactor;
				}
			}
		...
		for (int i = 0; i<DreieckStreckeSzenario3modes.subdivisionFactor; i++){
			double x = DreieckStreckeSzenario3modes.length - (DreieckStreckeSzenario3modes.length/(2*DreieckStreckeSzenario3modes.subdivisionFactor))*(i+1);
			double y = Math.sqrt(3.)*(DreieckStreckeSzenario3modes.length-x);
			Coord coord = scenario.createCoord(x, y);
			Id id = new IdImpl((long)(DreieckStreckeSzenario3modes.subdivisionFactor+i+1));
			
			Node node = scenario.getNetwork().getFactory().createNode(id, coord);
			network.addNode(node);
		}
		//nodes of the triangle left side
		for (int i = 0; i<DreieckStreckeSzenario3modes.subdivisionFactor-1; i++){
			double x = DreieckStreckeSzenario3modes.length/2 - (DreieckStreckeSzenario3modes.length/(2*DreieckStreckeSzenario3modes.subdivisionFactor))*(i+1);
			double y = Math.sqrt(3.)*x;
			Coord coord = scenario.createCoord(x, y);
			Id id = new IdImpl((long)(2*DreieckStreckeSzenario3modes.subdivisionFactor+i+1));
		...
		coord = scenario.createCoord(DreieckStreckeSzenario3modes.length+50.0, 0.0);
		Id endId = new IdImpl(3*DreieckStreckeSzenario3modes.subdivisionFactor);
		Node endNode = scenario.getNetwork().getFactory().createNode(endId, coord);
		network.addNode(endNode);
		
		//LINKS
		//all triangle links
		for (int i = 0; i<3*DreieckStreckeSzenario3modes.subdivisionFactor; i++){
			Id idFrom = new IdImpl((long)i);
			Id idTo;
			if (i != 3*DreieckStreckeSzenario3modes.subdivisionFactor-1)
			...
			link.setFreespeed(DreieckStreckeSzenario3modes.FREESPEED/3.6);
		...
		startLink.setFreespeed(DreieckStreckeSzenario3modes.FREESPEED/3.6);
		startLink.setLength(25.);
		startLink.setNumberOfLanes(2.);
		network.addLink(startLink);
		Link endLink = this.scenario.getNetwork().getFactory().createLink(endId, this.scenario.getNetwork().getNodes().get(new IdImpl(DreieckStreckeSzenario3modes.subdivisionFactor)), endNode);
		endLink.setCapacity(capMax);
		endLink.setFreespeed(DreieckStreckeSzenario3modes.FREESPEED/3.6);
	...
	private void createRandomPopulation(long numberOfPeople, int sekundenFrequenz){
		...
		for (long i = 0; i<numberOfPeople; i++){
			...
			double psum = P_FAST+P_MED+P_TRUCK;
			double p_med = P_MED/psum;
			double p_truck = P_TRUCK/psum;
			if (p<p_truck){
				transportMode = "truck";
			...
			for (long j=0; j<3*subdivisionFactor;j++){
 ...
 travelTimes = new TravelTimeCalculatorFactoryImpl().createTravelTimeCalculator(scenario.getNetwork(), scenario.getConfig().travelTimeCalculator());
	...
	private Activity createWork(){
		Id workLinkId = new IdImpl(3*DreieckStreckeSzenario3modes.subdivisionFactor);
