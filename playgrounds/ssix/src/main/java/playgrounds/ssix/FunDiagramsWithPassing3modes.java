...
public class FunDiagramsWithPassing3modes implements LinkEnterEventHandler{
	...
	private List<Double> speedTable_med;
	...
	private List<Double> flowTable_med;
	...
	public FunDiagramsWithPassing3modes(Scenario sc){
		this.scenario = sc;
		...
		//it is just necessary to let them go home and carry on with their lives instead of running around.
			...
			if (event.getLinkId().equals(studiedMeasuringPointLinkId)){	
				...
				if (permanentRegime){
					...
					//Updating lastXFlows
					for (int j=FunDiagramsWithPassing3modes.NUMBER_OF_MEMORIZED_FLOWS-2; j>=0; j--){
					...
					if (tourNumber >= (this.permanentRegimeTour+2)){//Let the simulation go another turn around to eventually fill data gaps
						...
						double sum=0;
						for (int i=0; i<10; i++){
							sum += this.speedTable.get(i).doubleValue();
						}
						this.permanentAverageVelocity = sum / 10;//m/s
	
						if (almostAbsoluteEqualDoubles(this.lastXFlows.get(0),this.lastXFlows.get(FunDiagramsWithPassing3modes.NUMBER_OF_MEMORIZED_FLOWS-1),1.)){
		...
		if (event.getLinkId().equals(studiedMeasuringPointLinkId)){
			if (this.personTour.containsKey(personId)){
				...
				if (tourNumber>2){
					...
					//Permanent in the mode? Memorizing the first occurrence of close speeds for the different modes...
						...
						if ((almostRelativeEqualDoubles(speed, previousLapSpeed_truck, 0.02)) && (almostRelativeEqualDoubles(previousLapSpeed_truck, theOneBefore_truck, 0.02))){
							...
							if ((permanentRegime_med) && (permanentRegime_fast)){
								if (tourNumber>this.permanentRegimeTour){
									...
									this.cachedPermanentRegimeTour = this.permanentRegimeTour;
									this.permanentRegimeTour = tourNumber;
								} else if (tourNumber>this.cachedPermanentRegimeTour){
									this.cachedPermanentRegimeTour2 = this.cachedPermanentRegimeTour;
									this.cachedPermanentRegimeTour = tourNumber;
								} else {
									this.cachedPermanentRegimeTour2 = tourNumber;
								}
								
							} else if ((permanentRegime_med) || (permanentRegime_fast)){
								if (tourNumber>this.permanentRegimeTour){
									this.cachedPermanentRegimeTour = this.permanentRegimeTour;
									this.permanentRegimeTour = tourNumber;
								} else {
					...
					if (!(permanentRegime)){
						...
						if ((permanentRegime_truck) && (permanentRegime_med) && (permanentRegime_fast)){  //just checking that the modes are effectively stable
							...
							closeSpeeds = true;
								...
								try {
									n_med_prt = tourNumberSpeed_med.get(permanentRegimeTour).getFirst();
								} catch (NullPointerException e) {
									n_med_prt = 0;//means the NumberSpeed_med for this tour hasn't been created => the mode needs to run around a little more
								}
								try {
								...
								if ( (n_truck_prt+n_med_prt+n_fast_prt) == this.scenario.getPopulation().getPersons().size()){
									completedPRT = true;
								}
			...
			if (nowTime == this.flowTime_truck.doubleValue()){//Still measuring the flow of the same second
				...
				this.flowTable_truck.set(0, nowFlow.doubleValue()+DreieckStreckeSzenario3modes.PCU_TRUCK);
			} else {//Need to offset the current data in order to update
				...
				if (timeDifference<3600){
					...
					this.flowTable_truck.set(0, DreieckStreckeSzenario3modes.PCU_TRUCK);
				} else {
	...
	private void handleEvent_med(LinkEnterEvent event){
		...
		if (event.getLinkId().equals(studiedMeasuringPointLinkId)){
			if (this.personTour.containsKey(personId)){
				...
				for (int i=8; i>=0; i--){
					this.speedTable.set(i+1, this.speedTable.get(i).doubleValue());
					this.speedTable_med.set(i+1, this.speedTable_med.get(i).doubleValue());
				}
				this.speedTable.set(0, speed);
				this.speedTable_med.set(0, speed);
				...
				if (tourNumber>2){
					...
					if (!(permanentRegime_med)){
						...
						if ((almostRelativeEqualDoubles(speed, previousLapSpeed_med, 0.02)) && (almostRelativeEqualDoubles(previousLapSpeed_med, theOneBefore_med, 0.02))){
							...
							} else if ((permanentRegime_truck) || (permanentRegime_fast)){
								if (tourNumber>this.permanentRegimeTour){
									this.cachedPermanentRegimeTour = this.permanentRegimeTour;
									this.permanentRegimeTour = tourNumber;
								} else {
			...
			if (nowTime == this.flowTime_med.doubleValue()){//Still measuring the flow of the same second
				Double nowFlow = this.flowTable_med.get(0);
				this.flowTable_med.set(0, nowFlow.doubleValue()+DreieckStreckeSzenario3modes.PCU_MED);
			} else {//Need to offset the current data in order to update
				int timeDifference = (int) (nowTime-this.flowTime_med.doubleValue());
				//log.info("timeDifference is: "+timeDifference);
				if (timeDifference<3600){
					for (int i=3599-timeDifference; i>=0; i--){
						this.flowTable_med.set(i+timeDifference, this.flowTable_med.get(i).doubleValue());
					}
					if (timeDifference > 1){
						for (int i = 1; i<timeDifference; i++){
					...
					this.flowTable_med.set(0, DreieckStreckeSzenario3modes.PCU_MED);
				} else {
					flowTableReset_med();
				}
				this.flowTime_med = new Double(nowTime);
	...
	private void handleEvent_fast(LinkEnterEvent event){
		...
		if (event.getLinkId().equals(studiedMeasuringPointLinkId)){
				...
				if (tourNumber>2){//>=3 because I want to compare actual speed with *2* previous speeds
					...
					if (!(permanentRegime_fast)){
						...
						if ((almostRelativeEqualDoubles(speed, previousLapSpeed_fast, 0.02)) && (almostRelativeEqualDoubles(previousLapSpeed_fast, theOneBefore_fast, 0.02))){
							...
							} else if ((permanentRegime_truck) || (permanentRegime_med)){
								if (tourNumber>this.permanentRegimeTour){
									this.cachedPermanentRegimeTour = this.permanentRegimeTour;
									this.permanentRegimeTour = tourNumber;
								} else {
			...
			if (nowTime == this.flowTime_fast.doubleValue()){//Still measuring the flow of the same second
				...
				this.flowTable_fast.set(0, nowFlow.doubleValue()+DreieckStreckeSzenario3modes.PCU_FAST);
			} else {//Need to offset the current data in order to update
				...
				if (timeDifference<3600){
					...
					this.flowTable_fast.set(0, DreieckStreckeSzenario3modes.PCU_FAST);
				} else {
					flowTableReset_fast();
				}
				this.flowTime_fast = new Double(nowTime);
	...
	private void handlePermanentGroupDependentVariables(int tourNumber){
		double networkLength = 3*DreieckStreckeSzenario3modes.length;
		...
		if (tourNumber >= (this.permanentRegimeTour+3)){//Let the simulation go another turn around to eventually fill data gaps
			...
			this.permanentDensity_truck = N_truck*DreieckStreckeSzenario3modes.PCU_TRUCK/networkLength*1000;
			this.permanentDensity_med = N_med*DreieckStreckeSzenario3modes.PCU_MED/networkLength*1000;
			this.permanentDensity_fast = N_fast*DreieckStreckeSzenario3modes.PCU_FAST/networkLength*1000;
			...
			if (this.tourNumberSpeed_med.size() != 0){
				...
				double sum =0;
				for (int i=0; i<10; i++){
					sum += this.speedTable_med.get(i);
				}
				this.permanentAverageVelocity_med = sum / 10;
			} else {
				this.permanentAverageVelocity_med = 0.;
			}
			if (this.tourNumberSpeed_fast.size() != 0){
