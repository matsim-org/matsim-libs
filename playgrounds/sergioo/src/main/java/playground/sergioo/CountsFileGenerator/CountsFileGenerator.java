...
public class CountsFileGenerator {
	...
	public static void main(final String[] args) throws IOException {
		...
		throw new RuntimeException("commented non-compiling code out, WGS84_SVY21 unknown");
...
//					Coord location = entryLocation.getValue();
//					Window window = new Window(name+"     "+entryLocation.getKey()+"     "+code+"     "+movement, network);
//					for(Coord point:countLocations.values())
//						window.addPoint(point);
//					window.selectPoint(location.getX(), location.getY());
//					for(int i=0; i<10; i++)
//						window.zoomIn(location);
...
//					if(window.isSave() && selected!=null) {
