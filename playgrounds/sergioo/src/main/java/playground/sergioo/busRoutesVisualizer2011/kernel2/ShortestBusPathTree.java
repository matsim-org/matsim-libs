			...
			for(TransitStopFacility stop:scenario.getTransitSchedule().getFacilities().values()) {
				...
				if(legs.size()>0 && legs.size()<=NUM_TRANSFERS+1) {
					...
					for(int j=0; j<legs.size(); j++) {
						...
						Id startId = scenario.getTransitSchedule().getFacilities().get(route.getAccessStopId()).getLinkId();
						Id endId = scenario.getTransitSchedule().getFacilities().get(route.getEgressStopId()).getLinkId();
						networkRoute = networkRoute.getSubRoute(startId, endId);
						for(Id id:networkRoute.getLinkIds())
