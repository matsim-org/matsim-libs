...
public class BSSimpleNetworkWindow extends LayersWindow implements ActionListener {
	...
	//Methods
	public BSSimpleNetworkWindow(String title, Network network) {
		this(title, new NetworkPainter(network));
	}
	
	public BSSimpleNetworkWindow(String title, NetworkPainter networkPainter) {
		setTitle(title);
		setDefaultCloseOperation(HIDE_ON_CLOSE);
		this.setLocation(0,0);
		this.setLayout(new BorderLayout()); 
		layersPanels.put(PanelIds.ONE, new BSNetworkPanel(this, networkPainter));
	...
	public void refreshLabel(playground.sergioo.Visualizer2D.LayersWindow.Labels label) {
		labels[label.ordinal()].setText(((BSNetworkPanel)layersPanels.get(PanelIds.ONE)).getLabelText(label));
	}
	@Override
