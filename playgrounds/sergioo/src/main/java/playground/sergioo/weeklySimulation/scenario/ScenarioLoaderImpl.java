...
public class ScenarioLoaderImpl {
	...
	public void loadNetwork() {
		...
		if ((this.config.network() != null) && (this.config.network().getInputFile() != null)) {
			...
			if ((this.config.network().getChangeEventsInputFileUrl(this.config.getContext()).getFile() != null)
	...
	private void loadLanes() {
		...
		if (filename != null) {
			...
			if (!LaneDefinitionsReader.SCHEMALOCATIONV20.equalsIgnoreCase(fileTypeGuesser.getSystemId())) {
				log.error(
						"Lanes: Wrong file format. With the 0.5 version of matsim the scenario only accepts lane definitions in the "
								+ "file format version 2.0, i.e. " + LaneDefinitionsReader.SCHEMALOCATIONV20
