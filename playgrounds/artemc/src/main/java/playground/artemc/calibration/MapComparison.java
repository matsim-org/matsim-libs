		...
		for(Integer key:dataSet.getDataMap().keySet()){	
			...
			for(int i=0;i<dataSet.getDataMap().get(key).length;i++){
				if(previousKey==null){
					...
					cumSum[i] = (double) cumulativeValuesMap.get(previousKey)[i] + (double) dataSet.getDataMap().get(key)[i]/sum1;
				}
			}
			previousKey=key;	
		...
		for(Integer key:map1.keySet()){
			for(int c=0;c<cumulativeMap2[r].length;c++){
				if(r==0){
					...
					cumulativeMap1[r][c] = (double) cumulativeMap1[r-1][c] + (double) map1.get(key)[c]/sum1;
				}
			}
			r++;
		}
		...
		for(Integer key:map2.keySet()){	
			for(int c=0;c<cumulativeMap2[r].length;c++){
				if(r==0){
					...
					cumulativeMap2[r][c] = (double) cumulativeMap2[r-1][c] + (double) map2.get(key)[c]/sum2;
				}	
			}
			r++;
		}
