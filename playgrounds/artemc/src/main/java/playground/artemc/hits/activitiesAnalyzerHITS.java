...
/**
 * Main class for analysing, checking for consistency and writing out activity information from the HITS database.
 *
 * @author artemc
 */

public class activitiesAnalyzerHITS {
	...
	int consistentActivities=0;
  	...
  	int overnightActivities=0;
	
	int totalWorkLocations=0;
	...
	public static void main(String args[]) throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException, ParseException {
		...
		activitiesAnalyzerHITS actAnalyzer = new activitiesAnalyzerHITS();
	...
	private void printSummary() {
		...
		System.out.println("Consistent PT activities shorter than 1h:: "+shortActivities);
		...
		System.out.println("Total work locations: "+totalWorkLocations);
	...
	private void getTrips() throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException, ParseException{
		...
		BufferedWriter outActivities = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivities_adults_v5_1000.csv"));
		BufferedWriter outActivities80 = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivities_adults_v5_1000_80pct.csv"));
		BufferedWriter outActivities20 = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivities_adults_v5_1000_20pct.csv"));
		BufferedWriter outOnlyWork = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivities_work_hipf_v5_1000.csv"));
	    BufferedWriter outShiftWork = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITS_shiftTimes_v5_1000.csv"));
	    BufferedWriter outActivitiesPT = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivitiesConsistent_v5_1000.csv"));
	    BufferedWriter outAllActivities = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivitiesAll_v5.csv"));
...
//	    		+"StartTime"+"\t"+"Lon"+"\t"+"Lat"+"\t"+"Mode"+"\t"+"PTLine"+"\t"+"AssignedStop"+"\t"+"TripFactor"+"\t"+"PersonID"+"\t"
	    ...
	    outActivities80.write("WorkChoice"+"\t"+"ActivityDuration"+"\t"+"ActivityStartTime24"+"\t"+"StartTimeSin1"+"\t"+"StartTimeSin2"+"\t"+"StartTimeSin3"+"\t"+"StartTimeCos1"+"\t"+"StartTimeCos2"+"\t"+"StartTimeCos3"+"\t"
//	    		+"StartTime"+"\t"+"Lon"+"\t"+"Lat"+"\t"+"Mode"+"\t"+"PTLine"+"\t"+"AssignedStop"+"\t"+"TripFactor"+"\t"+"PersonID"+"\t"
	    		+"AGRI"+"\t"+"BEACH"+"\t"+"BUSI1"+"\t"+"BUSI1W"+"\t"+"BUSI2"+"\t"+"BUSI2W"+"\t"+"BUSIPARKW"+"\t"+"BUSIPARK"+"\t"+"CEMETERY"+"\t"+"CIVIC"+"\t"+"COMM"+"\t"+"COMMFIRST"+"\t"+"COMMRES"
	    		+"\t"+"EDU"+"\t"+"HEALTH"+"\t"+"HOTEL"+"\t"+"OPENSPACE"+"\t"+"PARK"+"\t"+"PORTAIRPORT"+"\t"+"RESIDENTIAL"+"\t"+"RESIDENTIALIN"+"\t"+"RESERVESITE"+"\t"+"SPECIAL"+"\t"+"SPORT"
	    		+"\t"+"TRANSPORT"+"\t"+"UTILITY"+"\t"+"WATERBODY"+"\t"+"WHITE"+"\t"+"WORSHIP"+"\n");
	    outActivities20.write("WorkChoice"+"\t"+"ActivityDuration"+"\t"+"ActivityStartTime24"+"\t"+"StartTimeSin1"+"\t"+"StartTimeSin2"+"\t"+"StartTimeSin3"+"\t"+"StartTimeCos1"+"\t"+"StartTimeCos2"+"\t"+"StartTimeCos3"+"\t"
//	    		+"StartTime"+"\t"+"Lon"+"\t"+"Lat"+"\t"+"Mode"+"\t"+"PTLine"+"\t"+"AssignedStop"+"\t"+"TripFactor"+"\t"+"PersonID"+"\t"
	    		+"AGRI"+"\t"+"BEACH"+"\t"+"BUSI1"+"\t"+"BUSI1W"+"\t"+"BUSI2"+"\t"+"BUSI2W"+"\t"+"BUSIPARKW"+"\t"+"BUSIPARK"+"\t"+"CEMETERY"+"\t"+"CIVIC"+"\t"+"COMM"+"\t"+"COMMFIRST"+"\t"+"COMMRES"
	    		+"\t"+"EDU"+"\t"+"HEALTH"+"\t"+"HOTEL"+"\t"+"OPENSPACE"+"\t"+"PARK"+"\t"+"PORTAIRPORT"+"\t"+"RESIDENTIAL"+"\t"+"RESIDENTIALIN"+"\t"+"RESERVESITE"+"\t"+"SPECIAL"+"\t"+"SPORT"
	    		+"\t"+"TRANSPORT"+"\t"+"UTILITY"+"\t"+"WATERBODY"+"\t"+"WHITE"+"\t"+"WORSHIP"+"\n");
...
//	    		+"\t"+"SPORT"+"\t"+"WORSHIP"+"\t"+"Lat"+"\t"+"Lon"+"\n");    
		...
		ResultSet persons = dba.executeQuery("SELECT DISTINCT pax_idx FROM hitsshort_geo WHERE " +
				...
				"AND p1_age!='04-09' AND p1_age!='10-14' AND p5_econactivity!='student' AND p5_econactivity!='retired' AND h4b_u4yrs!='0-3 yrs ORDER BY RAND()'");
		...
		while(persons.next()){
			...
			analyzeActivities(outActivities,outActivities80,outActivities20,outOnlyWork, outActivitiesPT, outAllActivities);
		...
		outActivities.close();
		outActivities80.close();
		outActivities20.close();
		outOnlyWork.close();
		outShiftWork.close();
  ...
  void analyzeActivities(BufferedWriter outActivities, BufferedWriter outActivities80, BufferedWriter outActivities20, BufferedWriter outOnlyWork, BufferedWriter outActivitiesPT, BufferedWriter outAllActivities) throws ParseException, IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException{
	   ...
	   totalTrips=totalTrips+trips.size();
		...
		for(int i =0; i < (trips.size());i++){
			...
			//Check if it's a single journey
			if(trips.size()==1)
				break firstTripLoop;
			...
			{	
				...
				i_next=0; // Activities without any pt still get counted and checked for overnight activity
				overnightAct =  1;
			}
			else{
				i_next=i+1;
			...
			if(trips.get(i_next).startTime.getTime() >=trips.get(i).endTime.getTime() || trips.get(i_next).endTime.getTime() <= trips.get(i).startTime.getTime()){
				...
				//Check for transfers
				if(trips.get(i).actType.equals("xfer") && actDuration<1){
//					activityType=trips.get(i_next).actType;
				...
				if(distance<=1000.0 &&  trips.get(i).endLon > 0 && trips.get(i).endLat > 0 && !trips.get(i).actType.equals("xfer")){
...
//							outShift.write(sdf.format(lastMainEndTime)+","+sdf.format(startTime)+","+transportMode+"\n");
					...
					//writeAllActivities
					outAllActivities.write(activityType+","+Integer.toString(actStartTime)+","+Double.toString(actDuration)+","+trips.get(i).pax+","+Integer.toString(overnightAct)+"\n");
								...
								if(stageMode.equals("publBus") || stageMode.equals("mrt") || stageMode.equals("lrt")){
									...
									if(distancePT<=1000.0){
										...
										if(trips.get(i).endTime.getTime() > trips.get(j).startTime.getTime()){
											actDuration = actDuration + 24.0;
											if(trips.get(i).actType.equals("work")  && actDuration >= 1)
										...
										if(actDuration >=1){								
											...
											if(!ptconsistency2008_2011){
												...
												if(trips.get(i).lastPTMode.equals("Bus")){
													...
													ResultSet densities = dbaWorkFacilities.executeQuery("SELECT * FROM StopDensities WHERE stop_id ='"+closestStopID+"'");
													while(densities.next()){
														for(int l=0;l<29;l++){
															facilityDensities[l] = densities.getString(l+9);
														}
	...
	//													+facilityDensities[28]+"\n");
												...
												if(numberOfPersons<14030){
												outActivities80.write(actType+"\t"+Double.toString(actDuration)+"\t"+Integer.toString(actStartTime24)+"\t"
														+Double.toString(sin1)+"\t"+Double.toString(sin2)+"\t"+Double.toString(sin3)+"\t"
														+Double.toString(cos1)+"\t"+Double.toString(cos2)+"\t"+Double.toString(cos3)+"\t"	
														+facilityDensities[0]+"\t"+facilityDensities[1]+"\t"+facilityDensities[2]+"\t"+facilityDensities[3]+"\t"
														+facilityDensities[4]+"\t"+facilityDensities[5]+"\t"+facilityDensities[6]+"\t"+facilityDensities[7]+"\t"
												...
												outActivities20.write(actType+"\t"+Double.toString(actDuration)+"\t"+Integer.toString(actStartTime24)+"\t"
														+Double.toString(sin1)+"\t"+Double.toString(sin2)+"\t"+Double.toString(sin3)+"\t"
														+Double.toString(cos1)+"\t"+Double.toString(cos2)+"\t"+Double.toString(cos3)+"\t"	
														+facilityDensities[0]+"\t"+facilityDensities[1]+"\t"+facilityDensities[2]+"\t"+facilityDensities[3]+"\t"
														+facilityDensities[4]+"\t"+facilityDensities[5]+"\t"+facilityDensities[6]+"\t"+facilityDensities[7]+"\t"
...
//												
												...
												outActivitiesPT.write(activityType+","+Integer.toString(actStartTime)+","+Double.toString(actDuration)+","+trips.get(i).pax+","+Integer.toString(overnightAct)+"\n");
											}
										}
										else{
											shortActivities++;
					...
					if(activityType.equals("work") && actDuration >=1){		
						
						sameLocation = false;							
						//Check if there is already same work location in the array list for this worker
						if(!workLocationsLat.isEmpty()){
							for(int j=0;j<workLocationsLat.size();j++){
								double distanceWorkLocation = Math.sqrt((workLocationsLat.get(j) - trips.get(i).endLat)*(workLocationsLat.get(j) - trips.get(i).endLat) + (workLocationsLon.get(j) - trips.get(i).endLon)*(workLocationsLon.get(j) - trips.get(i).endLon));
						...
						{
							workLocationsLat.add(trips.get(i).endLat);
							workLocationsLon.add(trips.get(i).endLon);
					...
					}else if(activityType.equals("home") && actDuration >=1){		
						totalHomeActivities++;
						homePerson=true;
  ...
  private boolean checkForPTconsistency(int i) {
	...
	if(trips.get(i).lastPTLineInformation.equals("999 Don't Know") || trips.get(i).lastPTLineInformation.equals("177") || trips.get(i).lastPTLineInformation.equals("50") 
			...
			|| trips.get(i).lastPTLineInformation.equals("986") || trips.get(i).lastPTLineInformation.equals("256") || trips.get(i).lastPTLineInformation.equals("745")
			|| trips.get(i).lastPTLineInformation.equals("916") || trips.get(i).lastPTLineInformation.equals("824") || trips.get(i).lastPTLineInformation.equals("226")
			|| trips.get(i).lastPTLineInformation.equals("987") || trips.get(i).lastPTLineInformation.equals("541") || trips.get(i).lastPTLineInformation.equals("253")
			|| trips.get(i).lastPTLineInformation.equals("152")){
		return false;
	}
	else{
		return true;
