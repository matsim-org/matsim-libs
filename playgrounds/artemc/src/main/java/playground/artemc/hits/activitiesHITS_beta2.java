...
public class activitiesHITS_beta2 {
	...
	public static void main(String args[]) throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException {
		...
		BufferedWriter outWork = new BufferedWriter( new FileWriter("C:\\Work\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivities_work.csv"));
		...
		while(persons.next()){
			...
			ResultSet agentTrips = dba.executeQuery("SELECT h1_hhid,pax_id,trip_id,stage_id,t10_mode,start_lat,start_lon, end_lat,end_lon, t3_starttime_24h, t4_endtime_24h, t6_purpose, tripfactorsstgfinal FROM hitsshort_geo WHERE  pax_idx='"+pax+"' AND trip_id IS NOT NULL ORDER BY trip_id,stage_id");
...
//								System.out.println(lastMainStartTime+","+lastMainEndTime+","+startTime+","+actDuration+","+pax);																		
							if(actType.equals("work")){
								...
								consistentWork++;
								outWork.write(pax+","+Double.toString(endLon)+","+Double.toString(endLat)+","+transportMode+","+tripfactor+"\n");
							}
...
//							System.out.println(pax);
		...
		System.out.println("Consistent work activities: "+consistentWork);
