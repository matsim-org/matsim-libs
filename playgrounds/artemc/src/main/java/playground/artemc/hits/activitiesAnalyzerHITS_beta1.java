...
public class activitiesAnalyzerHITS_beta1 {
	...
	public static void main(String args[]) throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException, ParseException {
		...
		activitiesAnalyzerHITS_beta1 actAnalyzer = new activitiesAnalyzerHITS_beta1();
	...
	private void printSummary() {
		...
		System.out.println("Considered PT activities:: "+(consistentPtActivities-shortActivities));
	...
	private void getTrips() throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException, ParseException{
		...
		BufferedWriter outActivities = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivities_adults_v3_500.csv"));
		BufferedWriter outOnlyWork = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivities_work_hipf_v3.csv"));
	    BufferedWriter outShiftWork = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITS_shiftTimes_v3.csv"));
	    BufferedWriter outActivityTimes = new BufferedWriter( new FileWriter("C:\\Workspace\\MatsimProjects\\data\\activityAnalysis\\HITSActivities_times_v3.csv"));
...
//	    		+"\t"+"TRANSPORT"+"\t"+"UTILITY"+"\t"+"WATERBODY"+"\t"+"WHITE"+"\t"+"WORSHIP"+"\n");
	    outActivities.write("WorkChoice"+"\t"+"ActivityDuration"+"\t"+"ActivityStartTime24"+"\t"
	    		+"AGRI"+"\t"+"BUSI1"+"\t"+"BUSI1W"+"\t"+"BUSI2"+"\t"+"BUSI2W"+"\t"+"BUSIPARKW"+"\t"+"BUSIPARK"+"\t"+"CEMETERY"+"\t"+"CIVIC"+"\t"+"COMM"+"\t"+"COMMFIRST"+"\t"+"COMMRES"
	    		+"\t"+"EDU"+"\t"+"HEALTH"+"\t"+"HOTEL"+"\t"+"OPENSPACE"+"\t"+"PARK"+"\t"+"PORTAIRPORT"+"\t"+"RESERVESITE"+"\t"+"RESIDENTIAL"+"\t"+"RESIDENTIALIN"+"\t"+"SPECIAL"+"\t"+"SPORT"+"\t"+"WORSHIP"+"\n");
		...
		int numberWorkActivitiesPerson;
		...
		while(persons.next()){
			...
			ResultSet agentTrips = dba.executeQuery("SELECT h1_hhid,pax_id,trip_id,stage_id,t10_mode,start_lat,start_lon, end_lat,end_lon, "+
				"t3_starttime_24h, t4_endtime_24h, t6_purpose, hipf10, p6a_fixedwkpl, t11_boardsvcstn, t12_alightstn FROM hitsshort_geo_hipf "+
			...
			analyzeActivities(outActivities, outOnlyWork, outActivityTimes);
		...
		outOnlyWork.close();
		outShiftWork.close();
		outActivityTimes.close();
	}				
	
  void analyzeActivities(BufferedWriter outActivities, BufferedWriter outOnlyWork, BufferedWriter outoutActivityTimes) throws ParseException, IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException{
	   ...
	   totalTrips=totalTrips+trips.size();
		...
		for(int i =0; i< (trips.size()-1);i++){
			...
			if(trips.get(i+1).startTime.getTime() >=trips.get(i).endTime.getTime() || trips.get(i+1).endTime.getTime() <= trips.get(i).startTime.getTime()){
				double distance = Math.sqrt((trips.get(i+1).startLat - trips.get(i).endLat)*(trips.get(i+1).startLat - trips.get(i).endLat) + (trips.get(i+1).startLon - trips.get(i).endLon)*(trips.get(i+1).startLon - trips.get(i).endLon));							
				totalActivities++;
				//check reporting consistency in HITS
				if(distance<=1000.0){
					consistentActivities++;				
					actDuration = ((double)(trips.get(i+1).startTime.getTime() - trips.get(i).endTime.getTime()))/1000/60/60;
					
					//check for transfers
					if(trips.get(i).actType.equals("xfer")){
						activityType=trips.get(i+1).actType;
						detectedTransfers++;
					}
					else{
						activityType=trips.get(i).actType;
					}
					...
					if(trips.get(i).endTime.getTime() > trips.get(i+1).startTime.getTime()){
...
//							outShift.write(sdf.format(lastMainEndTime)+","+sdf.format(startTime)+","+transportMode+"\n");
					...
					//mode
					if(!trips.get(i).lastPTMode.equals("0"))
						mode=1;
								...
								if(stageMode.equals("publBus") || stageMode.equals("mrt") || stageMode.equals("lrt")){
									...
									if(distancePT<=1000.0){
										...
										if(actDuration >=0.5){
											...
											if(trips.get(i).lastPTLineInformation.equals("999 Don't Know") || trips.get(i).lastPTLineInformation.equals("177") || trips.get(i).lastPTLineInformation.equals("50") 
													|| trips.get(i).lastPTLineInformation.equals("744") || trips.get(i).lastPTLineInformation.equals("926") || trips.get(i).lastPTLineInformation.equals("1")
													|| trips.get(i).lastPTLineInformation.equals("234") || trips.get(i).lastPTLineInformation.equals("715") || trips.get(i).lastPTLineInformation.equals("544 Premier")
													|| trips.get(i).lastPTLineInformation.equals("401") || trips.get(i).lastPTLineInformation.equals("CT 8") || trips.get(i).lastPTLineInformation.equals("573")
													|| trips.get(i).lastPTLineInformation.equals("868") || trips.get(i).lastPTLineInformation.equals("296") || trips.get(i).lastPTLineInformation.equals("623")
													|| trips.get(i).lastPTLineInformation.equals("749") || trips.get(i).lastPTLineInformation.equals("420") || trips.get(i).lastPTLineInformation.equals("967") 
													|| trips.get(i).lastPTLineInformation.equals("621") || trips.get(i).lastPTLineInformation.equals("608") || trips.get(i).lastPTLineInformation.equals("822")
													|| trips.get(i).lastPTLineInformation.equals("CW1") || trips.get(i).lastPTLineInformation.equals("355") || trips.get(i).lastPTLineInformation.equals("522")
													|| trips.get(i).lastPTLineInformation.equals("71") || trips.get(i).lastPTLineInformation.equals("571") || trips.get(i).lastPTLineInformation.equals("625")
													|| trips.get(i).lastPTLineInformation.equals("968") || trips.get(i).lastPTLineInformation.equals("204") || trips.get(i).lastPTLineInformation.equals("572")
													|| trips.get(i).lastPTLineInformation.equals("164") || trips.get(i).lastPTLineInformation.equals("206") || trips.get(i).lastPTLineInformation.equals("216")
													|| trips.get(i).lastPTLineInformation.equals("116") || trips.get(i).lastPTLineInformation.equals("150") || trips.get(i).lastPTLineInformation.equals("864")
													|| trips.get(i).lastPTLineInformation.equals("NR6") || trips.get(i).lastPTLineInformation.equals("CW2") || trips.get(i).lastPTLineInformation.equals("294")
													|| trips.get(i).lastPTLineInformation.equals("986")){
												noPtInformation++;
											}
											else{
												...
												if(trips.get(i).lastPTMode.equals("Bus")){
													...
													for(String stopPrint:stopList){
														System.out.print(stopPrint+",");
													}
													System.out.println();
													...
													while(densities.next()){
														for(int l=0;l<23;l++){
															facilityDensities[l] = densities.getString(l+1);
														}
													}
												}
												else{
													...
													ResultSet densities = dbaWorkFacilities.executeQuery("SELECT * FROM StopDensities WHERE stop_name ='"+closestStop+"'");
													while(densities.next()){
														for(int l=0;l<29;l++){
	...
	//												+facilityDensities[28]+"\n");
												...
												+facilityDensities[12]+"\t"+facilityDensities[13]+"\t"+facilityDensities[14]+"\t"+facilityDensities[15]+"\t"
												+facilityDensities[16]+"\t"+facilityDensities[17]+"\t"+facilityDensities[18]+"\t"+facilityDensities[19]+"\t"
												+facilityDensities[20]+"\t"+facilityDensities[21]+"\t"+facilityDensities[22]+"\n");
												
												//Write Activity Times Description
												outoutActivityTimes.write(activityType+","+Integer.toString(actStartTime)+","+Double.toString(actDuration)+","+trips.get(i).pax+"\n");
											}
										}
										else{
											shortActivities++;
					...
					if(activityType.equals("work") && actDuration >= 0.5){		
						totalWorkActivities++;
						worker=true;
