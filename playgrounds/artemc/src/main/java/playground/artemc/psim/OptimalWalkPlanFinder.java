...
public class OptimalWalkPlanFinder {
	...
	public Plan findOptimalWalkPlan(Plan plan) {
		...
		Coord home = ((Activity) plan.getPlanElements().get(0)).getCoord();
		Coord work = ((Activity) plan.getPlanElements().get(2)).getCoord();
		...
		((Activity) plan.getPlanElements().get(0)).setEndTime(workStartTime - travelTime);

		((Leg) plan.getPlanElements().get(1)).setDepartureTime(((Activity) plan.getPlanElements().get(0)).getEndTime());

		((Activity) plan.getPlanElements().get(2)).setStartTime(workStartTime);
		((Activity) plan.getPlanElements().get(2)).setEndTime(workStartTime + workDuration);

		((Leg) plan.getPlanElements().get(3)).setDepartureTime(((Activity) plan.getPlanElements().get(2)).getEndTime());

		((Activity) plan.getPlanElements().get(4)).setStartTime(((Leg) plan.getPlanElements().get(3)).getDepartureTime() + travelTime);
