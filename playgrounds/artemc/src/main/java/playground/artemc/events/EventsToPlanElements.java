...
import playground.artemc.analysis.postgresql.CSVWriter;
...
class EventsToPlanElements implements TransitDriverStartsEventHandler, PersonEntersVehicleEventHandler, PersonLeavesVehicleEventHandler, PersonDepartureEventHandler, PersonArrivalEventHandler, ActivityStartEventHandler, ActivityEndEventHandler, PersonStuckEventHandler, LinkEnterEventHandler, LinkLeaveEventHandler, TeleportationArrivalEventHandler, VehicleArrivesAtFacilityEventHandler {
	...
	public EventsToPlanElements(TransitSchedule transitSchedule, Network network, Config config, String suffix, String schemaName) {
...
//		transitVehicleIds.addAll(vehicles.getVehicles().keySet());
	...
	public EventsToPlanElements(Network network, Config config, String suffix, String schemaName) {
		this.network = network;
		this.walkSpeed = new TransitRouterConfig(config).getBeelineWalkSpeed();
		TravelComponent.walkSpeed = walkSpeed;
		this.schemaName = schemaName;
	...
	public EventsToPlanElements(TransitSchedule transitSchedule, Network network, Config config, File connectionProperties, String suffix, String schemaName) {
//		this(transitSchedule, network, config, suffix, transitVehiclesfile);
	...
	void writeSimulationResultsToSQL(File connectionProperties,
		...
		String actTableName = "" + schemaName + ".activities" + suffix;
		List<PostgresqlColumnDefinition> columns = new ArrayList<>();
		...
		String journeyTableName = "" + schemaName + ".journeys" + suffix;
		...
		String tripTableName = "" + schemaName + ".trips" + suffix;
		columns = new ArrayList<>();
		...
		String transferTableName = "" + schemaName + ".transfers" + suffix;
		columns = new ArrayList<>();
...
//					System.err.println(act);
	...
	public void writeSimulationResultsToCSV(String path,
		...
		String formattedDate = df.format(new Date());
		// start with activities
		String actTableName = "activities" + suffix;
		List<PostgresqlColumnDefinition> columns = new ArrayList<>();
		columns.add(new PostgresqlColumnDefinition("activity_id",
				PostgresType.INT, "primary key"));
		columns.add(new PostgresqlColumnDefinition("person_id",
		...
		String journeyTableName =  "journeys" + suffix;
		columns = new ArrayList<>();
		columns.add(new PostgresqlColumnDefinition("journey_id",
				PostgresType.INT, "primary key"));
		columns.add(new PostgresqlColumnDefinition("person_id",
		...
		String tripTableName ="trips" + suffix;
		columns = new ArrayList<>();
		columns.add(new PostgresqlColumnDefinition("trip_id", PostgresType.INT));
		columns.add(new PostgresqlColumnDefinition("journey_id",
				PostgresType.INT));
		...
		String transferTableName = "transfers" + suffix;
		columns = new ArrayList<>();
		columns.add(new PostgresqlColumnDefinition("transfer_id",
				PostgresType.INT, "primary key"));
		columns.add(new PostgresqlColumnDefinition("journey_id",
