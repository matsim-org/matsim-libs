...
public class CharyparNagelActivityScoring implements ActivityScoring, org.matsim.core.scoring.SumScoringFunction.ActivityScoring {
	...
	private final PersonalScoringParameters params;
	...
	public CharyparNagelActivityScoring(final PersonalScoringParameters params) {
		this.params = params;
		this.reset();
	}
	...
	private final void handleOvernightActivity(Activity lastActivity) {
		...
		if (lastActivity.getType().equals(this.firstActivity.getType())) {
			...
			if (this.params.scoreActs) {
				...
				// score last activity
				this.score += calcActScore(this.currentActivityStartTime, 24*3600, lastActivity); // Till midnight, independent of scenario duration
			}
		}
	}
