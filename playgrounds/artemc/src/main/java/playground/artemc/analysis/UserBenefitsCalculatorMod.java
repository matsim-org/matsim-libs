 ...
 *
 */
public class UserBenefitsCalculatorMod {
	private static final Logger logger = Logger.getLogger(UserBenefitsCalculatorMod.class);
	...
	private String choiceStrategy = "unknown";


	private final Set<Id<Person>> stuckingAgents;

	public UserBenefitsCalculatorMod(Config config, WelfareMeasure wm, boolean considerAllPlans) {
		...
		for(StrategyConfigGroup.StrategySettings strategySettings:config.strategy().getStrategySettings()){
			if(strategySettings.getStrategyName().equals("SelectExpBeta")){
				choiceStrategy = "SelectExpBeta";
			}
			if(strategySettings.getStrategyName().equals("ChangeExpBeta")){
				choiceStrategy = "ChangeExpBeta";
			}
		}
		if(choiceStrategy.equals("unknown")){
			logger.error("No plan selector strategy found in the config!");
		}
		{
	...
	public UserBenefitsCalculatorMod(Config config, Set<Id<Person>> stuckingAgents) {
		...
		if(this.welfareMeasure.equals(WelfareMeasure.LOGSUM)){
			for(Plan plan : person.getPlans()){
				...
				if(shouldBeConsidered){
					...
					double expScoreOfPlan = 0.0;
					if(choiceStrategy.equals("SelectExpBeta")) {
						expScoreOfPlan = Math.exp(betaLogit * plan.getScore());
					}
					else if(choiceStrategy.equals("ChangeExpBeta")) {
						bestScore = getBestScore(person);
						expScoreOfPlan = Math.exp(betaLogit * (plan.getScore() - bestScore));
					}
					else{
						logger.error("No required plan selector strategy found in the config!");
					}
					sumOfExpScore += expScoreOfPlan;

				} else {
					//logger.warn("Plan with score "+plan.getScore()+" is not considered for person "+plan.getPerson().getId().toString());
				}
			}
