...
public class ScheduleDelayCostHandler implements ActivityStartEventHandler, ActivityEndEventHandler, PersonArrivalEventHandler, PersonDepartureEventHandler, PersonEntersVehicleEventHandler, PersonStuckEventHandler{
	...
	private DisaggregatedCharyparNagelScoringFunctionFactory disScoringFactory = null;
	...
	public void handleEvent(PersonDepartureEvent event) {
		...
		if(!personId.toString().startsWith("pt")){
			...
			PlanElement planElement = controler.getPopulation().getPersons().get(personId).getSelectedPlan().getPlanElements().get(2*numberOfActivities-1);
	...
	public void handleEvent(ActivityEndEvent event) {
		...
		if(disScoringFactory==null){
			disScoringFactory = (DisaggregatedCharyparNagelScoringFunctionFactory) controler.getScoringFunctionFactory();
		}
	...
	public double getFreeSpeedTravelTime(Leg leg) {
		...
		if(leg.getMode().equals(TransportMode.car)){
			...
			for(Id<Link> link:route.getLinkIds()){
				freeSpeedTravelTime += (controler.getNetwork().getLinks().get(link).getLength() / controler.getNetwork().getLinks().get(link).getFreespeed());
			}
		}
