...
public class timesBetweenTrips_ver2 {
...
//		BufferedWriter aggregationErrors = new BufferedWriter( new FileWriter("C:\\Work\\Workspace\\MatsimProjects\\data\\ezLinkAnalysis\\aggregationErrors.csv"));
		...
		while(cardIDs.next()){
			...
			ResultSet agentTrips = dba.executeQuery("SELECT CARD_ID,Journey_Start_Time,Journey_End_Time, start_lat,start_lon, end_lat,end_lon,PassengerType FROM trips_ver1 WHERE CARD_ID="+Long.toString(CARD_ID)+" ORDER BY Journey_Start_Date,Journey_Start_Time");			
