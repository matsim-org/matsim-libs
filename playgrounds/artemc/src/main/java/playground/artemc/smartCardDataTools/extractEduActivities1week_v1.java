...
public class extractEduActivities1week_v1 {
...
//				ResultSet agentTrips = dba.executeQuery("SELECT CARD_ID,Journey_Start_Time,Journey_End_Time, start_lat,start_lon, end_lat,end_lon,PassengerType FROM trips_ver1 WHERE CARD_ID="+Long.toString(CARD_ID)+" AND PassengerType='Adult' ORDER BY Journey_Start_Date, Journey_Start_Time");
				ResultSet agentTrips = dba.executeQuery("SELECT CARD_ID,Ride_Start_Time,Ride_Time, start_lat,start_lon, end_lat,end_lon,PassengerType,Transfer_Number FROM "+dayTable+" WHERE CARD_ID="+Long.toString(CARD_ID)+" ORDER BY Ride_Start_Date, Ride_Start_Time");	
				int count=0;
				int eduActivity=0;		
				while(agentTrips.next()){
					if(count!=0){
						...
						transferNumber = agentTrips.getInt(9);
...
//						if(difference>=7200000 && endTime.getTime() >= 21600000 && endTime.getTime() <= 43200000 && startTime.getTime() <= 64800000 && startTime.getTime() >= 36000000){
						if(startTime.getTime() <= 64800000 && startTime.getTime() >= 36000000 && transferNumber==0){
							longact=longact+1;
//							if (distanceRounded<1000){							
								...
								if(!eduLocationsLat.isEmpty()){
									for(int i=0;i<eduLocationsLat.size();i++){
										double distanceEduLocation = Math.sqrt((eduLocationsLat.get(i) - endLat)*(eduLocationsLat.get(i) - endLat) + (eduLocationsLon.get(i) - endLon)*(eduLocationsLon.get(i) - endLon));
										if(distanceEduLocation<=500.0){
											sameLocation=true;
								...
								if(sameLocation==false)									
								{
									...
									//Assign a school according to exp-probability function dependent on distance
