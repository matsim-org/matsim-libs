...
public class ControlerWithHeteroAndTollFare {

	private static final Logger log = Logger.getLogger(ControlerWithHeteroAndTollFare.class);
...
//		simulationTypes.add("heteroGammaOnly");
		...
		if(args.length>4 && args[4].equals("homo")){
			simulationTypes = new ArrayList<String>();
			simulationTypes.add("homo");
		}
		...
		for(String type:simulationTypes){
			...
			while(fare<10.5){
				...
				if(args.length>1){
					output = args[1] + "_"+simulationType+"_"+heterogeneityFactor+"x"+"_pt"+fare;
				}

				if(simulationType.equals("homo")){
					output = args[1] + "_"+simulationType+"_pt"+fare;
				}

				ControlerWithHeteroAndTollFare runner = new ControlerWithHeteroAndTollFare();
				runner.run();
				fare = fare + 0.5;
...
//			controler.setModules(new ControlerDefaultsModule(), new IncomeHeterogeneityWithoutTravelDisutilityModule(), new RoadPricingWithoutTravelDisutilityModule(),new UpdateSocialCostPricingSchemeModule());
	...
	private static Scenario initScenario() {
		...
		ConfigUtils.addOrGetModule(config, RoadPricingConfigGroup.GROUP_NAME, RoadPricingConfigGroup.class).setTollLinksFile(input + "roadpricing.xml");
