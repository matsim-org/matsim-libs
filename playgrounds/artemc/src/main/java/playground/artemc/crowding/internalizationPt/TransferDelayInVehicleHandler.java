...
public class TransferDelayInVehicleHandler implements PersonEntersVehicleEventHandler, PersonLeavesVehicleEventHandler, TransitDriverStartsEventHandler, VehicleArrivesAtFacilityEventHandler, VehicleDepartsAtFacilityEventHandler {
	...
	private final Map<Id, List<Id>> vehId2passengersId = new HashMap<Id, List<Id>>();
	private final Map<Id, Id> passengers2facility = new HashMap<Id, Id>();
	...
	public void reset(int iteration) {
		...
		this.passengers2facility.clear();
		this.vehId2passengersId.clear();
	...
	public void handleEvent(PersonEntersVehicleEvent event) {
		...
		// "normal" passenger, not the driver
		if (!ptDriverIDs.contains(event.getPersonId()) && ptVehicleIDs.contains(event.getVehicleId())){
			...
			Id facilityId = this.vehId2facilityId.get(event.getVehicleId());		
			//Id facilityId = 	this.passengers2facility.get(event.getPersonId());
			...
			TransferDelayInVehicleEvent delayInVehicleEvent = new TransferDelayInVehicleEvent(event.getPersonId(), event.getVehicleId(), facilityId, event.getTime(), delayedPassengers_inVeh, delay);
			//System.out.println("PersonId : " + event.getPersonId().toString() + ", VehicleId : " + event.getVehicleId().toString() + ", Facility : " + facilityId.toString() + " " + event.getTime() + " " + delayedPassengers_inVeh + " " + delay);
			...
			// add the boarding agent in the vehicle's passengers list
//			if (this.vehId2passengersId.containsKey(event.getVehicleId())){
//				this.vehId2passengersId.get(event.getVehicleId()).add(event.getPersonId());
//			} else {
//				List<Id> passengersIdInVeh = new ArrayList<Id>();
//				passengersIdInVeh.add(event.getPersonId());
//				this.vehId2passengersId.put(event.getVehicleId(), passengersIdInVeh);
//			}
	...
	public void handleEvent(PersonLeavesVehicleEvent event) {
		...
		if (!ptDriverIDs.contains(event.getPersonId()) && ptVehicleIDs.contains(event.getVehicleId())){		
			...
			// remove passenger in vehicle
//			this.vehId2passengersId.get(event.getVehicleId()).remove(event.getPersonId());
...
//		if(this.vehId2passengersId.containsKey(event.getVehicleId())){
//			for(Id agent:this.vehId2passengersId.get(event.getVehicleId())) {
//				this.passengers2facility.put(agent,event.getFacilityId());
//			}
//		}
		...
		if (!(this.vehId2agentsBoardingAtThisStop.get(event.getVehicleId()) == null)){
			...
			if (!(agentsBoardingAtThisStop.size() == 0)) {
				...
				for (Id personId : agentsBoardingAtThisStop){
					TransferDelayInVehicleEvent delayInVehicleEvent = new TransferDelayInVehicleEvent(personId, event.getVehicleId(), event.getFacilityId(), event.getTime(), affectedAgents, delayPerPerson);
					this.events.processEvent(delayInVehicleEvent);
				}
			}
		}
