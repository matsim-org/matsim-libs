...
public class FakeAgent implements MobsimDriverAgent, PTPassengerAgent {
  ...
  // as long as all instance variables are final, the "resetCaches" method can remain empty.  kai,
  // oct'10

  /**
   * Creates a new fake Agent. If enterStop or exitStop are <code>null</code>, the leg will have no
   * route.
   *
  ...
  public FakeAgent(final TransitStopFacility enterStop, final TransitStopFacility exitStop) {
    ...
    if ((enterStop != null) && (exitStop != null)) {
      ...
      route.setRouteDescription(
          "{"
              + "\"accessFacilityId\":\""
              + enterStop.getId().toString()
              + "\","
              + "\"egressFacilityId\":\""
              + exitStop.getId().toString()
              + "\","
              + "\"transitLineId\":\"T1\""
              + "}");
  ...
  public OptionalTime getExpectedTravelTime() {
    // since the class does not tell what it is supposed to do I do not know what is a reasonable
    // answer here.  kai, jun'11
  ...
  public Id<Vehicle> getPlannedVehicleId() {
    return ((NetworkRoute) this.dummyLeg.getRoute())
        .getVehicleId(); // not sure if this is very clever.  kai, jun'11
  }
