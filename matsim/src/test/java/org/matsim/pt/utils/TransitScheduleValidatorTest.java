...
public class TransitScheduleValidatorTest {
  ...
  public void testPtTutorialWithError() {
    ...
    transitLine
        ...
        .getRoute()
        .setLinkIds(
            Id.createLinkId("33"), Collections.<Id<Link>>emptyList(), Id.createLinkId("11"));
    TransitScheduleValidator.ValidationResult validationResult =
        TransitScheduleValidator.validateAll(scenario.getTransitSchedule(), scenario.getNetwork());
    System.out.println(validationResult);

    Assertions.assertThat(validationResult.getIssues())
        .usingRecursiveFieldByFieldElementComparator()
        .containsExactly(
            new TransitScheduleValidator.ValidationResult.ValidationIssue<>(
  ...
  public void testValidator_Transfers_implausibleTime() {
    ...
    schedule.addStopFacility(
        factory.createTransitStopFacility(id1, new Coord(10000, 10000), false));
    schedule.addStopFacility(
        factory.createTransitStopFacility(id2, new Coord(20000, 10000), false));

    schedule.getMinimalTransferTimes().set(id1, id2, 120);
    TransitScheduleValidator.ValidationResult result =
        TransitScheduleValidator.validateTransfers(schedule);
    ...
    Assert.assertEquals(
        "Should warn against implausible transfer time.", 1, result.getIssues().size());
  }
  ...
  public void testValidator_Transfers_missingStop() {
    ...
    Assert.assertTrue(
        "Message should contain hint about stop3 being missing. "
            + result.getIssues().get(0).getMessage(),
        result.getIssues().get(0).getMessage().contains("stop3"));
    ...
    Assert.assertTrue(
        "Message should contain hint about stop4 being missing. "
            + result.getIssues().get(0).getMessage(),
        result.getIssues().get(0).getMessage().contains("stop4"));
  }
}
