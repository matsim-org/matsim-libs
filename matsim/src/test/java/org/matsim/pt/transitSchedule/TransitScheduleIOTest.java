...
public class TransitScheduleIOTest {
  ...
  public void testWriteRead_V2() {
    ...
    { // prepare data
      ...
      TransitStopFacility stop1 =
          f.createTransitStopFacility(
              Id.create(1, TransitStopFacility.class), new Coord(123, 234), true);
      TransitStopFacility stop2 =
          f.createTransitStopFacility(
              Id.create(2, TransitStopFacility.class), new Coord(987, 876, 98765), false);
      ...
      TransitRouteStop rStop2 =
          f.createTransitRouteStopBuilder(stop1).arrivalOffset(9999.9).build();
      stops.add(rStop1);
      stops.add(rStop2);
      TransitRoute route1a =
          f.createTransitRoute(
              Id.create("upwards", TransitRoute.class), netRoute, stops, "elevator");
    ...
    TransitStopFacility stop1 =
        schedule2.getFacilities().get(Id.create(1, TransitStopFacility.class));
    Assert.assertTrue(AttributesUtils.isEmpty(stop1.getAttributes()));
    TransitStopFacility stop2 =
        schedule2.getFacilities().get(Id.create(2, TransitStopFacility.class));
    ...
    Assert.assertEquals(
        300, schedule2.getMinimalTransferTimes().get(stop1.getId(), stop2.getId()), 0.0);
    Assert.assertEquals(
        360, schedule2.getMinimalTransferTimes().get(stop2.getId(), stop1.getId()), 0.0);
    Assert.assertEquals(
        Double.NaN, schedule2.getMinimalTransferTimes().get(stop1.getId(), stop1.getId()), 0.0);
