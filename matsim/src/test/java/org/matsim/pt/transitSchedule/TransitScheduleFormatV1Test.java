...
/**
 * Tests the file format <code>transitSchedule_v1.dtd</code> by creating a transit schedule with
 * different attributes, writing it out to a file in the specified file format, reads it back in and
 * compares the actual values with the previously set ones.
 *
 * @author mrieser
 */
public class TransitScheduleFormatV1Test {
  ...
  public void testWriteRead() throws IOException, SAXException, ParserConfigurationException {
    ...
    Node n2 =
        NetworkUtils.createAndAddNode(
            network, Id.create("2", Node.class), new Coord((double) 0, (double) 0));
    Node n3 =
        NetworkUtils.createAndAddNode(
            network, Id.create("3", Node.class), new Coord((double) 0, (double) 0));
    Node n4 =
        NetworkUtils.createAndAddNode(
            network, Id.create("4", Node.class), new Coord((double) 0, (double) 0));
    Node n5 =
        NetworkUtils.createAndAddNode(
    ...
    TransitStopFacility stop4 =
        builder.createTransitStopFacility(
            ...
            new Coord((double) 0, (double) 1000),
            false);
    ...
    TransitRouteStop routeStop3 =
        builder.createTransitRouteStopBuilder(stop3).departureOffset(300).build();
    ...
    TransitRoute route1 =
        builder.createTransitRoute(Id.create(1, TransitRoute.class), null, stops, "bus");
    ...
    TransitRoute route2 =
        builder.createTransitRoute(Id.create(2, TransitRoute.class), route, stops, "bus");
  ...
  private static void assertEqualSchedules(
      final TransitSchedule expected, final TransitSchedule actual) {

    assertEquals(
        "different number of stopFacilities.",
        expected.getFacilities().size(),
        actual.getFacilities().size());
    ...
    assertEquals(
        "different number of transitLines.",
        expected.getTransitLines().size(),
    ...
    for (TransitLine lineE : expected.getTransitLines().values()) {
      ...
      assertEquals(
          "different number of routes in line.",
          lineE.getRoutes().size(),
          lineA.getRoutes().size());
      for (TransitRoute routeE : lineE.getRoutes().values()) {
        ...
        assertEquals(
            "different route descriptions.", routeE.getDescription(), routeA.getDescription());

        assertEquals(
            "different number of stops.", routeE.getStops().size(), routeA.getStops().size());
        for (int i = 0, n = routeE.getStops().size(); i < n; i++) {
          ...
          assertEquals(
              "different stop facilities.",
              stopE.getStopFacility().getId(),
              stopA.getStopFacility().getId());
          assertEquals(
              "different arrival delay.", stopE.getArrivalOffset(), stopA.getArrivalOffset());
          assertEquals(
              "different departure delay.", stopE.getDepartureOffset(), stopA.getDepartureOffset());
          assertEquals(
              "different awaitDepartureTime.",
              stopE.isAwaitDepartureTime(),
              stopA.isAwaitDepartureTime());
        }
        ...
        assertEquals(
            "different number of departures in route.",
            routeE.getDepartures().size(),
            routeA.getDepartures().size());
        for (Departure departureE : routeE.getDepartures().values()) {
          ...
          assertEquals(
              ...
              departureE.getDepartureTime(),
              departureA.getDepartureTime(),
              MatsimTestUtils.EPSILON);
          assertEquals(
              "different vehicle ids.", departureE.getVehicleId(), departureA.getVehicleId());
        }
      }
    }
  }
