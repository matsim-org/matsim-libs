...
public class CountsReprojectionIOTest {
  ...
  public void testWithControlerAndConfigParameters() {
    ...
    final Counts<Link> internalCounts =
        controler.getInjector().getInstance(Key.get(new TypeLiteral<Counts<Link>>() {}));
  ...
  private void assertCountsAreReprojectedCorrectly(
      Counts<Link> originalCounts, Counts<Link> reprojectedCounts) {
    Assert.assertEquals(
  ...
  private Counts<Link> createDummyCounts() {
    ...
    for (double i = 0; i < 10; i += 1) {
      for (double j = 0; j < 10; j += 1) {
        final Count<Link> c = counts.createAndAddCount(Id.createLinkId(i + "-" + j), i + "-" + j);
