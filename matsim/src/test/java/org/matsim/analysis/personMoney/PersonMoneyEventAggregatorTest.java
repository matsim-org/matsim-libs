...
public class PersonMoneyEventAggregatorTest {
  ...
  public void testPersonMoneyEventCollector() {
    ...
    PersonMoneyEventsAggregator aggregator =
        new PersonMoneyEventsAggregator(ConfigUtils.createConfig().global());
    ...
    try (FileReader aggregatorCsv =
            new FileReader(utils.getOutputDirectory() + "/PersonMoneyEventsAggregator.csv");
        CSVParser parser = CSVParser.parse(aggregatorCsv, format)) {

      List<CSVRecord> csvRecordList = parser.getRecords();

      List<CSVRecord> csvRecordListA =
          csvRecordList.stream()
              .filter(
                  record ->
                      record.get(parser.getHeaderMap().get("transactionPartner")).equals(drt_A)
                          && record.get(parser.getHeaderMap().get("purpose")).equals("drtFare"))
              .collect(Collectors.toList());
      Assert.assertEquals("Either no record or more than one record", 1, csvRecordListA.size());
      Assert.assertEquals(
          "Wrong personMoneyAmountSum for drt_A",
          19.07,
          Double.parseDouble(csvRecordListA.get(0).get(2)),
          MatsimTestUtils.EPSILON);

      List<CSVRecord> csvRecordList0 =
          csvRecordList.stream()
              .filter(
                  record ->
                      record.get(parser.getHeaderMap().get("transactionPartner")).equals("DrtA")
                          && record.get(parser.getHeaderMap().get("purpose")).equals("DrtFare"))
              .collect(Collectors.toList());
      Assert.assertEquals("Either no record or more than one record", 1, csvRecordList0.size());
      Assert.assertEquals(
          "Wrong personMoneyAmountSum for empty purpose",
          4.1,
          Double.parseDouble(csvRecordList0.get(0).get(2)),
          MatsimTestUtils.EPSILON);

      List<CSVRecord> csvRecordListB =
          csvRecordList.stream()
              .filter(
                  record ->
                      record.get(parser.getHeaderMap().get("transactionPartner")).equals(drt_B)
                          && record.get(parser.getHeaderMap().get("purpose")).equals("drtFare"))
              .collect(Collectors.toList());
      Assert.assertEquals("Either no record or more than one record", 1, csvRecordListB.size());
      Assert.assertEquals(
          "Wrong personMoneyAmountSum for drt_B",
          16,
          Double.parseDouble(csvRecordListB.get(0).get(2)),
          MatsimTestUtils.EPSILON);

      List<CSVRecord> csvRecordListC =
          csvRecordList.stream()
              .filter(
                  record ->
                      record.get(parser.getHeaderMap().get("transactionPartner")).equals(drt_C)
                          && record.get(parser.getHeaderMap().get("purpose")).equals("drtFare"))
              .collect(Collectors.toList());
      Assert.assertEquals(
          "Either no record or more than one record for drt_C", 1, csvRecordListC.size());
      Assert.assertEquals(
          "Wrong personMoneyAmountSum",
          2.5,
          Double.parseDouble(csvRecordListC.get(0).get(2)),
          MatsimTestUtils.EPSILON);
