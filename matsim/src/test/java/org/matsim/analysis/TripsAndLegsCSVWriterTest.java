...
public class TripsAndLegsCSVWriterTest {
  ...
  public void testTripsAndLegsCSVWriter() {
    ...
    performTest(
        utils.getOutputDirectory() + "/trip.csv",
        utils.getOutputDirectory() + "/leg.csv",
        map,
    ...
    performTest(
        ...
        utils.getOutputDirectory() + "/leg.csv",
        map,
        new RoutingModeMainModeIdentifier());
  }
  ...
  private void performTest(
    ...
    TripsAndLegsCSVWriter.CustomTimeWriter timeWriter =
        new TripsAndLegsCSVWriter.DefaultTimeWriter();
    TripsAndLegsCSVWriter.CustomTripsWriterExtension customTripsWriterExtension =
        new CustomTripsWriterExtesion();
    TripsAndLegsCSVWriter.CustomLegsWriterExtension customLegsWriterExtension =
        new CustomLegsWriterExtesion();
    TripsAndLegsCSVWriter tripsAndLegsWriter =
        new TripsAndLegsCSVWriter(
            scenario, tripsWriterExtension, legWriterExtension, mainModeIdentifier, timeWriter);
    ...
    TripsAndLegsCSVWriter tripsAndLegsWriterTest =
        new TripsAndLegsCSVWriter(
            scenario,
            customTripsWriterExtension,
   ...
   ***********************************************************/
  private void readTripsFromPlansFile(
      IdMap<Person, Plan> map, AnalysisMainModeIdentifier mainModeIdentifier) {
    // double trav_time = 0;
      ...
      while (tripItr.hasNext()) {
        ...
        for (Leg leg : trip.getLegsOnly()) {
          if (leg.getRoute() instanceof TransitPassengerRoute) {
            ...
            transitStopsVisited =
                expTransitRoute.getAccessStopId().toString()
                    + "-"
                    + expTransitRoute.getEgressStopId().toString()
                    + "-";
          }
        }
      ...
      while (tripItr.hasNext()) {
        ...
        for (Leg leg : legs) {
          ...
          if (start_coord == null) {
            start_coord =
                scenario
                    .getNetwork()
                    .getLinks()
                    .get(leg.getAttributes().getAttribute("link"))
                    .getToNode()
                    .getCoord();
          ...
          Id<Vehicle> vehicleId =
              (Id<Vehicle>)
                  leg.getAttributes().getAttribute(EventsToLegs.VEHICLE_ID_ATTRIBUTE_NAME);
          ...
          if (leg.getMode().equals(TransportMode.walk)
              || leg.getMode().equals("walk_teleportation")) {
            containsWalk = true;
  ...
  private void readAndValidateLegs(ArrayList<Object> legsfromplan, String legFile) {
    ...
    try {
      ...
      while ((line = br.readLine()) != null && legItr.hasNext()) {
        ...
        Assert.assertEquals(
            "dep_time is not as expected",
            String.valueOf(nextleg.get("dep_time")),
            column[dep_time]);
        Assert.assertEquals(
            "trav_time is not as expected",
            String.valueOf(nextleg.get("trav_time")),
            column[trav_time]);
        Assert.assertEquals(
            "wait_time is not as expected",
            String.valueOf(nextleg.get("wait_time")),
            column[wait_time]);
        Assert.assertEquals(
            "Distance is not as expected",
            String.valueOf(nextleg.get("distance")),
            column[distance]);
        Assert.assertEquals(
            "mode is not as expected", String.valueOf(nextleg.get("mode")), column[mode]);
        Assert.assertEquals(
            "start_link is not as expected",
            String.valueOf(nextleg.get("start_link")),
            column[start_link]);
        Assert.assertEquals(
            "start_x is not as expected", String.valueOf(nextleg.get("start_x")), column[start_x]);
        Assert.assertEquals(
            "start_y is not as expected", String.valueOf(nextleg.get("start_y")), column[start_y]);
        Assert.assertEquals(
            "End link is not as expected",
            String.valueOf(nextleg.get("end_link")),
            column[end_link]);
        Assert.assertEquals(
            "end_x is not as expected", String.valueOf(nextleg.get("end_x")), column[end_x]);
        Assert.assertEquals(
            "end_y is not as expected", String.valueOf(nextleg.get("end_y")), column[end_y]);
        Assert.assertEquals(
            "person is not as expected", String.valueOf(nextleg.get("person")), column[person]);
        Assert.assertEquals(
            "trip_id is not as expected", String.valueOf(nextleg.get("trip_id")), column[trip_id]);
        Assert.assertEquals(
            "access_stop_id is not as expected",
            String.valueOf(nextleg.get("access_stop_id")),
            column[access_stop_id] != null ? column[access_stop_id] : "");
        Assert.assertEquals(
            "egress_stop_id is not as expected",
            String.valueOf(nextleg.get("egress_stop_id")),
            column[egress_stop_id] != null ? column[egress_stop_id] : "");
        Assert.assertEquals(
            "transit_line is not as expected",
            String.valueOf(nextleg.get("transit_line")),
            column[transit_line] != null ? column[transit_line] : "");
        Assert.assertEquals(
            "transit_route is not as expected",
            String.valueOf(nextleg.get("transit_route")),
            column[transit_route] != null ? column[transit_route] : "");
        Assert.assertEquals(
            "vehicleId is not as expected",
            String.valueOf(nextleg.get("vehicle_id")),
            column[vehicle_id] != null ? column[vehicle_id] : "");
        if (isIntermodalWalkPt >= 0) {
          // column from CustomLegsWriterExtension is present
          Assert.assertEquals(
              "isIntermodalWalkPt is not as expected",
              String.valueOf(nextleg.get("isIntermodalWalkPt")),
              column[isIntermodalWalkPt]);
        }
      }
    } catch (IOException e) {
  ...
  private void readAndValidateTrips(Map<String, Object> persontrips, String tripFile) {
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        Assert.assertEquals(
            "Departure time is not as expected",
            String.valueOf(tripvalues.get("departureTime")),
            column[dep_time]);
        Assert.assertEquals(
            "Travel time is not as expected",
            String.valueOf(tripvalues.get("travelTime")),
            column[trav_time]);
        Assert.assertEquals(
            "Travel distance is not as expected",
            String.valueOf(tripvalues.get("traveled_distance")),
            column[traveled_distance]);
        Assert.assertEquals(
            "Euclidean distance is not as expected",
            tripvalues.get("euclideanDistance"),
            Integer.parseInt(column[euclidean_distance]));
        Assert.assertEquals(
            "Main mode is not as expected", tripvalues.get("main_mode"), column[main_mode]);
        Assert.assertEquals(
            "Longest distance mode is not as expected",
            tripvalues.get("longest_distance_mode"),
            column[longest_distance_mode]);
        Assert.assertEquals(
            "Modes is not as expected", String.valueOf(tripvalues.get("modes")), column[modes]);
        Assert.assertEquals(
            "Start activity type is not as expected",
            String.valueOf(tripvalues.get("start_activity_type")),
            column[start_activity_type]);
        Assert.assertEquals(
            "End activity type is not as expected",
            String.valueOf(tripvalues.get("end_activity_type")),
            column[end_activity_type]);
        Assert.assertEquals(
            "Start facility id is not as expected",
            String.valueOf(tripvalues.get("start_facility_id")),
            column[start_facility_id]);
        Assert.assertEquals(
            "Start link is not as expected",
            String.valueOf(tripvalues.get("start_link")),
            column[start_link]);
        Assert.assertEquals(
            "Start x is not as expected",
            String.valueOf(tripvalues.get("start_x")),
            column[start_x]);
        Assert.assertEquals(
            "Start y is not as expected",
            String.valueOf(tripvalues.get("start_y")),
            column[start_y]);
        Assert.assertEquals(
            "End facility id is not as expected",
            String.valueOf(tripvalues.get("end_facility_id")),
            column[end_facility_id]);
        Assert.assertEquals(
            "End link is not as expected",
            String.valueOf(tripvalues.get("end_link")),
            column[end_link]);
        Assert.assertEquals(
            "End x is not as expected", String.valueOf(tripvalues.get("end_x")), column[end_x]);
        Assert.assertEquals(
            "End y is not as expected", String.valueOf(tripvalues.get("end_y")), column[end_y]);
        Assert.assertEquals(
            "waiting_time is not as expected",
            String.valueOf(tripvalues.get("waiting_time")),
            column[wait_time]);
        Assert.assertEquals(
            "person is not as expected", String.valueOf(tripvalues.get("person")), column[person]);
        Assert.assertEquals(
            "trip_number is not as expected",
            String.valueOf(tripvalues.get("trip_number")),
            column[trip_number]);
        Assert.assertEquals(
            "trip_id is not as expected",
            String.valueOf(tripvalues.get("trip_id")),
            column[trip_id]);
        if (column.length > 21) {
          Assert.assertEquals(
              "first_pt_boarding_stop is not as expected",
              String.valueOf(tripvalues.get("first_pt_boarding_stop")),
              column[first_pt_boarding_stop]);
          Assert.assertEquals(
              "last_pt_egress_stop is not as expected",
              String.valueOf(tripvalues.get("last_pt_egress_stop")),
              column[last_pt_egress_stop]);
        }
        if (column.length > 23) {
          Assert.assertEquals(
              "transitStopsVisited is not as expected",
              String.valueOf(tripvalues.get("transitStopsVisited")),
              column[transitStopsVisited]);
        }
      }
    } catch (IOException e) {
      // TODO Auto-generated catch block
  ...
  private void createNetwork() {
    ...
    new MatsimNetworkReader(scenario.getNetwork())
        .readFile(utils.getOutputDirectory() + "/network.xml");
  }

  private static class CustomTripsWriterExtesion
      implements TripsAndLegsCSVWriter.CustomTripsWriterExtension {
