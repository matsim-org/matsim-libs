...
public class LegHistogramTest {
  ...
  /**
   * Tests that different modes of transport are recognized and accounted accordingly. Also tests
   * that modes not defined as constants are handled correctly.
   */
  @Test
  public void testDeparturesMiscModes() {
    ...
    Link link =
        NetworkUtils.createAndAddLink(
            network, Id.create(1, Link.class), fromNode, toNode, 1000.0, 100.0, 1.0, (double) 1);
    ...
    histo.handleEvent(
        new PersonDepartureEvent(7 * 3600, person1Id, linkId, leg.getMode(), leg.getMode()));
    histo.handleEvent(
        new PersonDepartureEvent(
            7 * 3600 + 6 * 60, person2Id, linkId, leg.getMode(), leg.getMode()));
    leg.setMode(TransportMode.bike);
    histo.handleEvent(
        new PersonDepartureEvent(
            7 * 3600 + 6 * 60, person1Id, linkId, leg.getMode(), leg.getMode()));
    histo.handleEvent(
        new PersonDepartureEvent(
            7 * 3600 + 10 * 60, person2Id, linkId, leg.getMode(), leg.getMode()));
    leg.setMode("undefined");
    histo.handleEvent(
        new PersonDepartureEvent(
            7 * 3600 + 10 * 60, person1Id, linkId, leg.getMode(), leg.getMode()));
    leg.setMode("undefined");
    histo.handleEvent(
        new PersonDepartureEvent(
            7 * 3600 + 16 * 60, person1Id, linkId, leg.getMode(), leg.getMode()));
  ...
  /**
   * Tests that the constructor-parameter <code>nofBins</code> is correctly taken into account and
   * that times larger than what is covered by the bins do not lead to an exception.
   */
  @Test
  public void testNofBins() {
    ...
    histo.handleEvent(
        new PersonDepartureEvent(2700, person1Id, linkId, leg.getMode(), leg.getMode()));
    histo.handleEvent(new PersonArrivalEvent(2999, person1Id, linkId, leg.getMode()));
    leg.setMode("train");
    histo.handleEvent(
        new PersonDepartureEvent(3000, person1Id, linkId, leg.getMode(), leg.getMode()));
    histo.handleEvent(new PersonArrivalEvent(3001, person1Id, linkId, leg.getMode()));
    leg.setMode("bus");
    histo.handleEvent(
        new PersonDepartureEvent(3600, person1Id, linkId, leg.getMode(), leg.getMode()));
  ...
  public void testReset() {
    ...
    leg.setMode("train");
    histo.handleEvent(
        new PersonDepartureEvent(8 * 3600, person1Id, linkId, leg.getMode(), leg.getMode()));
