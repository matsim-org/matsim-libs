 ...
 *
 * @author jakobrehmann adapted from Aravind "ModeStatsControlerListenerTest"
 */
public class ModeChoiceCoverageControlerListenerTest {
  ...
  public void testChangePlanModes() {
    ...
    OutputDirectoryHierarchy controlerIO =
        new OutputDirectoryHierarchy(
            utils.getOutputDirectory() + "/ModeChoiceCoverageControlerListener",
    ...
    ModeChoiceCoverageControlerListener modeCC =
        new ModeChoiceCoverageControlerListener(
            controllerConfigGroup, population, controlerIO, scoreConfig, transportId);
    ...
    Map<Integer, Map<String, Map<Integer, Double>>> modeChoiceCoverageHistory =
        modeCC.getModeChoiceCoverageHistory();
    Assert.assertEquals(
        "There should only be one mode in the mode modeCCHistory",
        1,
        modeChoiceCoverageHistory.get(1).keySet().size());
    Assert.assertEquals(
        "Since all trips were completed with walk, mcc for walk should be 1.0 (or 100%)",
    ...
    Assert.assertEquals(
        "There should now be two modes in modeCCHistory",
        2,
        modeChoiceCoverageHistory.get(1).keySet().size());
    Assert.assertEquals(
        "Since all trips were completed with walk (in previous iterations), mcc for walk should be 1.0 (or 100%)",
        (Double) 1.0, modeChoiceCoverageHistory.get(1).get(TransportMode.walk).get(1));
    Assert.assertEquals(
        "The mcc for bike from the 0th iteration should be 0.0, since bike only showed up in this iteration",
        (Double) 0.0,
        modeChoiceCoverageHistory.get(1).get(TransportMode.bike).get(0));
    Assert.assertEquals(
        "Since 1 of 2 trips were completed with bike, mcc for bike should be 0.5 (or 50%)",
    ...
    Assert.assertEquals(
        "There should still be two modes in modeCCHistory",
        2,
        modeChoiceCoverageHistory.get(1).keySet().size());
    Assert.assertEquals(
        "Since all trips were completed with walk (in previous iterations), mcc for walk should be 1.0 (or 100%)",
    ...
    Assert.assertEquals(
        "Since all trips were completed with bike (in previous iterations), mcc for bike should be 1.0 (or 100%)",
  ...
  public void testTwoAgents() {
    ...
    Assert.assertEquals(
        ...
        (Double) 1.0, modeChoiceCoverageHistory.get(1).get(TransportMode.walk).get(1));
    Assert.assertEquals(
        "Since 1 of 4 trips were completed with bike, mcc for bike should be 0.25 (or 25%)",
    ...
    Assert.assertEquals(
        ...
        (Double) 1.0, modeChoiceCoverageHistory.get(1).get(TransportMode.walk).get(2));
    Assert.assertEquals(
        "Since 3 of 4 trips were completed with bike (in previous iterations), mcc for bike should be 0.75 (or 75%)",
  ...
  public void testDifferentLevels() {
    ...
    Map<Integer, Map<String, Map<Integer, Double>>> modeChoiceCoverageHistory =
        modeCC.getModeChoiceCoverageHistory();
    Assert.assertEquals(
        "1x threshold should be met after 1 iteration",
        (Double) 1.0,
        modeChoiceCoverageHistory.get(1).get("walk").get(0));
    Assert.assertEquals(
        "5x threshold should NOT be met after 1 iteration",
        (Double) 0.0,
        modeChoiceCoverageHistory.get(5).get("walk").get(0));
    Assert.assertEquals(
        "10x threshold should NOT be met after 1 iteration",
        (Double) 0.0,
        modeChoiceCoverageHistory.get(10).get("walk").get(0));
    ...
    Assert.assertEquals(
        "1x threshold should be met after 5 iterations",
        (Double) 1.0,
        modeChoiceCoverageHistory.get(1).get("walk").get(4));
    Assert.assertEquals(
        "5x threshold should be met after 5 iterations",
        (Double) 1.0,
        modeChoiceCoverageHistory.get(5).get("walk").get(4));
    Assert.assertEquals(
        "10x threshold should NOT be met after 5 iterations",
        (Double) 0.0,
        modeChoiceCoverageHistory.get(10).get("walk").get(4));
    ...
    Assert.assertEquals(
        "1x threshold should be met after 10 iterations",
        (Double) 1.0,
        modeChoiceCoverageHistory.get(1).get("walk").get(9));
    Assert.assertEquals(
        "5x threshold should be met after 10 iterations",
        (Double) 1.0,
        modeChoiceCoverageHistory.get(5).get("walk").get(9));
    Assert.assertEquals(
        "10x threshold should be met after 10 iterations",
        (Double) 1.0,
        modeChoiceCoverageHistory.get(10).get("walk").get(9));
  }
