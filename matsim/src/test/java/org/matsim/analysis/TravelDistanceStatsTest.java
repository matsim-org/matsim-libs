...
public class TravelDistanceStatsTest {
  ...
  private void performTest(IdMap<Person, Plan> map, String outputDirectory) {
    ...
    TravelDistanceStats travelDistanceStats =
        new TravelDistanceStats(controllerConfigGroup, controlerIO, new GlobalConfigGroup());
    travelDistanceStats.addIteration(0, map);
    readAndValidateValues(
        0,
        person1legsum + person2legsum + person3legsum,
        12,
    ...
    readAndValidateValues(
        1, person1legsum + person3legsum, 8, person1TotalNumberOfLegs + person3TotalNumberOfLegs);
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        if (iteration == itr) {
          ...
          Assert.assertEquals(
              "avg. Average Trip distance does not match", (legSum / totalTrip), avgTripvalue, 0);
          Assert.assertEquals(
              "avg. Average Leg distance does not match", (legSum / totalLeg), avgLegvalue, 0);
