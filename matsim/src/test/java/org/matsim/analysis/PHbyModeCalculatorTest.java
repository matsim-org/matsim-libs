...
public class PHbyModeCalculatorTest {
  ...
  private void performTest(IdMap<Person, Plan> map, String outputDirectory) {
    ...
    PHbyModeCalculator phbyModeCalculator =
        new PHbyModeCalculator(controllerConfigGroup, controlerIO, new GlobalConfigGroup());
    ...
    while (pesronItr.hasNext()) {
      ...
      for (PlanElement elem : planelem) {
        if (elem instanceof Leg) {
          ...
          value +=
              (mode != "pt")
                  ? ((Leg) elem).getRoute().getTravelTime().seconds()
                  : ((Leg) elem).getRoute().getTravelTime().seconds()
                      - ((Double)
                              elem.getAttributes()
                                  .getAttribute(EventsToLegs.ENTER_VEHICLE_TIME_ATTRIBUTE_NAME)
                          - ((Leg) elem).getDepartureTime().seconds());
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        if (iteration == itr) {
          ...
          Assert.assertEquals(
              "car_travel hour does not match",
              Math.round(modeValues.get("car") / 3600.0),
              car_travel_value,
              0);
          Assert.assertEquals(
              "pt_travel hour score does not match",
              Math.round(modeValues.get("pt") / 3600.0),
              pt_travel_value,
              0);
          Assert.assertEquals(
              "walk_travel hour does not match",
              Math.round(modeValues.get("walk") / 3600.0),
              walk_travel_value,
              0);
          Assert.assertEquals(
              "pt_wait hour does not match",
              Math.round(modeValues.get("pt_wait") / 3600.0),
              pt_wait_value,
              0);
          Assert.assertEquals(
              "stageActivity_wait hour does not match",
              Math.round(modeValues.get("stageActivity_wait") / 3600.0),
              stageActivity_wait_value,
              0);
