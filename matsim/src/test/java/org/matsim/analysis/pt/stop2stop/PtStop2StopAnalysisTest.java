...
public class PtStop2StopAnalysisTest {
  ...
  public void testPtStop2StopAnalysisSingle() {
    ...
    VehicleType busVehicleType =
        vehicleFactory.createVehicleType(Id.create("Bus_veh_type", VehicleType.class));
    {
      ...
      VehicleUtils.setDoorOperationMode(
          busVehicleType,
          VehicleType.DoorOperationMode.serial); // first finish boarding, then start alighting
      VehicleUtils.setAccessTime(
          busVehicleType, 1.0 / 3.0); // 1s per boarding agent, distributed on 3 doors
      VehicleUtils.setEgressTime(
          busVehicleType, 1.0 / 3.0); // 1s per alighting agent, distributed on 3 doors
      scenario.getTransitVehicles().addVehicleType(busVehicleType);
    }

    VehicleType trainVehicleType =
        vehicleFactory.createVehicleType(Id.create("Train_veh_type", VehicleType.class));
    {
      ...
      VehicleUtils.setDoorOperationMode(
          ...
          VehicleType.DoorOperationMode.serial); // first finish boarding, then start alighting
      VehicleUtils.setAccessTime(
          trainVehicleType, 1.0 / 20.0); // 1s per boarding agent, distributed on 20 doors
      VehicleUtils.setEgressTime(
          trainVehicleType, 1.0 / 20.0); // 1s per alighting agent, distributed on 20 doors
      scenario.getTransitVehicles().addVehicleType(trainVehicleType);
    }

    Vehicle veh_bus1 =
        vehicleFactory.createVehicle(Id.create("pt_bus1", Vehicle.class), busVehicleType);
    Vehicle veh_bus_line1_route1_dep2 =
        vehicleFactory.createVehicle(Id.create("pt_bus2", Vehicle.class), busVehicleType);
    Vehicle veh_train1 =
        vehicleFactory.createVehicle(Id.create("pt_train1", Vehicle.class), trainVehicleType);
    ...
    PtStop2StopAnalysis ptStop2StopAnalysis =
        new PtStop2StopAnalysis(scenario.getTransitVehicles());
    ...
    events.processEvent(
        new TransitDriverStartsEvent(
            ...
            veh_bus1.getId(),
            transitLineId_bus1,
            transitRouteId_bus1_route1,
            departureId_bus1_route1_dep1));
    events.processEvent(
        new PersonDepartureEvent(1.0, driverId_bus1, linkId1, networkMode_bus, networkMode_bus));
    events.processEvent(new PersonEntersVehicleEvent(1.0, driverId_bus1, veh_bus1.getId()));
    events.processEvent(
        new VehicleEntersTrafficEvent(
            1.0, driverId_bus1, linkId1, veh_bus1.getId(), networkMode_bus, 1.0));
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(1.0, veh_bus1.getId(), transitStopFacilityId1, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(3.0, veh_bus1.getId(), transitStopFacilityId1, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(6.0, veh_bus1.getId(), transitStopFacilityId2, -1.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(8.0, veh_bus1.getId(), transitStopFacilityId2, 1.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(11.0, veh_bus1.getId(), transitStopFacilityId3, -1.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(13.0, veh_bus1.getId(), transitStopFacilityId3, 1.0));
    events.processEvent(
        new VehicleLeavesTrafficEvent(
            14.0, driverId_bus1, linkId3, veh_bus1.getId(), networkMode_bus, 1.0));
    ...
    // Tests
    List<PtStop2StopAnalysis.Stop2StopEntry> line1_route1_dep1_stop1 =
        ptStop2StopAnalysis.getStop2StopEntriesByDeparture().stream()
            .filter(
                entry ->
                    entry.transitLineId.equals(transitLineId_bus1)
                        && entry.transitRouteId.equals(transitRouteId_bus1_route1)
                        && entry.departureId.equals(departureId_bus1_route1_dep1)
                        && entry.stopId.equals(transitStopFacilityId1)
                        && entry.stopSequence == 0)
            .collect(Collectors.toList());
    System.out.println(line1_route1_dep1_stop1.size());
    Assert.assertEquals(
        "Either no entry or more than entry for "
            + transitLineId_bus1
            + ", "
            + transitRouteId_bus1_route1
            + ", "
            ...
            + departureId_bus1_route1_dep1
            + ", "
            + transitStopFacilityId1
            + ", 0",
        1,
        line1_route1_dep1_stop1.size());
    Assert.assertNull(
        "There should be no previous stop, but there was",
        line1_route1_dep1_stop1.get(0).stopPreviousId);
    Assert.assertEquals(
        "Wrong arrivalTimeScheduled",
        1.0,
        line1_route1_dep1_stop1.get(0).arrivalTimeScheduled,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong arrivalDelay",
        0.0,
        line1_route1_dep1_stop1.get(0).arrivalDelay,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        3.0, line1_route1_dep1_stop1.get(0).departureTimeScheduled, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        0.0, line1_route1_dep1_stop1.get(0).departureDelay, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        0.0, line1_route1_dep1_stop1.get(0).passengersAtArrival, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        veh_bus1.getType().getCapacity().getSeats()
            + veh_bus1.getType().getCapacity().getStandingRoom(),
        line1_route1_dep1_stop1.get(0).totalVehicleCapacity,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        0.0, line1_route1_dep1_stop1.get(0).passengersAlighting, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        2.0, line1_route1_dep1_stop1.get(0).passengersBoarding, MatsimTestUtils.EPSILON);
    List<Id<Link>> linkList = new ArrayList<>();
    linkList.add(linkId1);
    Assert.assertEquals(
        "Wrong links", linkList, line1_route1_dep1_stop1.get(0).linkIdsSincePreviousStop);

    List<PtStop2StopAnalysis.Stop2StopEntry> line1_route1_dep1_stop3 =
        ptStop2StopAnalysis.getStop2StopEntriesByDeparture().stream()
            .filter(
                entry ->
                    entry.transitLineId.equals(transitLineId_bus1)
                        && entry.transitRouteId.equals(transitRouteId_bus1_route1)
                        && entry.departureId.equals(departureId_bus1_route1_dep1)
                        && entry.stopId.equals(transitStopFacilityId3)
                        && entry.stopSequence == 2)
            .collect(Collectors.toList());
    Assert.assertEquals(
        "Either no entry or more than entry for "
            + transitLineId_bus1
            ...
            + departureId_bus1_route1_dep1
            + ", "
            + transitStopFacilityId3
            + ", 0",
        1,
        line1_route1_dep1_stop3.size());
    Assert.assertEquals(
        "There is no previous stop",
        transitStopFacilityId2,
        line1_route1_dep1_stop3.get(0).stopPreviousId);
    Assert.assertEquals(
        12.0, line1_route1_dep1_stop3.get(0).departureTimeScheduled, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong arrivalTimeScheduled",
        12.0,
        line1_route1_dep1_stop3.get(0).arrivalTimeScheduled,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong arrival delay",
        -1.0,
        line1_route1_dep1_stop3.get(0).arrivalDelay,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong departure delay",
        1.0,
        line1_route1_dep1_stop3.get(0).departureDelay,
        MatsimTestUtils.EPSILON);
  }
  ...
  public void testPtStop2StopAnalysisMulti() {
    ...
    Vehicle veh_bus1_dep1 =
        vehicleFactory.createVehicle(Id.create("pt_bus_1", Vehicle.class), busVehicleType);
    Vehicle veh_bus1_dep2 =
        vehicleFactory.createVehicle(Id.create("pt_bus_2", Vehicle.class), busVehicleType);
    Vehicle veh_train1_dep1 =
        vehicleFactory.createVehicle(Id.create("pt_train_1", Vehicle.class), trainVehicleType);
    ...
    events.processEvent(
        new TransitDriverStartsEvent(
            ...
            transitLineId_bus1,
            transitRouteId_bus1_route1,
            departureId_bus1_dep1));
    events.processEvent(
        new TransitDriverStartsEvent(
            1.0,
            driverId_train1,
            veh_train1_dep1.getId(),
            transitLineId_train1,
            transitRouteId_train1_route1,
            departureId_train1_dep1));
    events.processEvent(
        new PersonDepartureEvent(1.0, driverId_bus1, linkId1, networkMode_bus, networkMode_bus));
    events.processEvent(
        new PersonDepartureEvent(
            1.0, driverId_train1, linkId7, networkMode_train, networkMode_train));
    events.processEvent(new PersonEntersVehicleEvent(1.0, driverId_bus1, veh_bus1_dep1.getId()));
    events.processEvent(
        new PersonEntersVehicleEvent(1.0, driverId_train1, veh_train1_dep1.getId()));
    events.processEvent(
        new VehicleEntersTrafficEvent(
            1.0, driverId_bus1, linkId1, veh_bus1_dep1.getId(), networkMode_bus, 1.0));
    events.processEvent(
        new VehicleEntersTrafficEvent(
            1.0, driverId_train1, linkId7, veh_train1_dep1.getId(), networkMode_train, 1.0));
    //      BUS1 :stopSequence == 0
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(1.0, veh_bus1_dep1.getId(), transitStopFacilityId1, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(3.0, veh_bus1_dep1.getId(), transitStopFacilityId1, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            3.0, veh_train1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            5.0, veh_train1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(6.0, veh_bus1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(8.0, veh_bus1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            8.0, veh_train1_dep1.getId(), transitStopFacilityId6, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            10.0, veh_train1_dep1.getId(), transitStopFacilityId6, 0.0));
    ...
    events.processEvent(
        new TransitDriverStartsEvent(
            ...
            transitLineId_bus1,
            transitRouteId_bus1_route1,
            departureId_bus1_dep2));
    events.processEvent(
        new PersonDepartureEvent(11.0, driverId_bus2, linkId1, networkMode_bus, networkMode_bus));
    events.processEvent(new PersonEntersVehicleEvent(11.0, driverId_bus2, veh_bus1_dep2.getId()));
    events.processEvent(
        new VehicleEntersTrafficEvent(
            11.0, driverId_bus2, linkId1, veh_bus1_dep2.getId(), networkMode_bus, 1.0));
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            11.0, veh_bus1_dep2.getId(), transitStopFacilityId1, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            13.0, veh_bus1_dep2.getId(), transitStopFacilityId1, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            14.0, veh_bus1_dep2.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            16.0, veh_bus1_dep2.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            13.0, veh_bus1_dep1.getId(), transitStopFacilityId4, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            15.0, veh_bus1_dep1.getId(), transitStopFacilityId4, 0.0));
    events.processEvent(
        new VehicleLeavesTrafficEvent(
            15.0, driverId_bus1, linkId4, veh_bus1_dep1.getId(), networkMode_bus, 1.0));
    ...
    events.processEvent(
        new TransitDriverStartsEvent(
            ...
            veh_bus1_dep1.getId(),
            transitLineId_bus1,
            transitRouteId_bus1_route2,
            departureId_bus1_dep1));
    events.processEvent(
        new PersonDepartureEvent(21.0, driverId_bus1, linkId4, networkMode_bus, networkMode_bus));
    events.processEvent(new PersonEntersVehicleEvent(22.0, driverId_bus1, veh_bus1_dep1.getId()));
    events.processEvent(
        new VehicleEntersTrafficEvent(
            22.0, driverId_bus1, linkId4, veh_bus1_dep1.getId(), networkMode_bus, 1.0));
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            23.0, veh_bus1_dep1.getId(), transitStopFacilityId4, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            25.0, veh_bus1_dep1.getId(), transitStopFacilityId4, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            14.0, veh_train1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            16.0, veh_train1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            21.0, veh_bus1_dep2.getId(), transitStopFacilityId4, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            23.0, veh_bus1_dep2.getId(), transitStopFacilityId4, 0.0));
    events.processEvent(
        new VehicleLeavesTrafficEvent(
            23.0, driverId_bus2, linkId4, veh_bus1_dep2.getId(), networkMode_bus, 1.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            19.0, veh_train1_dep1.getId(), transitStopFacilityId6, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            21.0, veh_train1_dep1.getId(), transitStopFacilityId6, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            25.0, veh_train1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            29.0, veh_train1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            28.0, veh_bus1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            29.0, veh_bus1_dep1.getId(), transitStopFacilityId2, 0.0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            29.0, veh_train1_dep1.getId(), transitStopFacilityId6, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            31.0, veh_train1_dep1.getId(), transitStopFacilityId6, 0.0));
    ...
    events.processEvent(
        new VehicleLeavesTrafficEvent(
            34.0, driverId_train1, linkId7, veh_train1_dep1.getId(), networkMode_train, 1.0));
    events.processEvent(
        new PersonLeavesVehicleEvent(34.0, driverId_train1, veh_train1_dep1.getId()));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(
            31.0, veh_bus1_dep1.getId(), transitStopFacilityId1, 0.0));
    ...
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(
            32.0, veh_bus1_dep1.getId(), transitStopFacilityId1, 0.0));
    events.processEvent(
        new VehicleLeavesTrafficEvent(
            32.0, driverId_bus1, linkId1, veh_bus1_dep1.getId(), networkMode_bus, 1.0));
        ...
        ptStop2StopAnalysis.getStop2StopEntriesByDeparture().stream()
            .filter(
                entry ->
                    entry.transitLineId.equals(transitLineId_bus1)
                        && entry.transitRouteId.equals(transitRouteId_bus1_route1)
                        && entry.departureId.equals(departureId_bus1_dep1)
                        && entry.stopId.equals(transitStopFacilityId1)
                        && entry.stopSequence == 0)
            .collect(Collectors.toList());
    Assert.assertEquals(
        "Either no entry or more than entry for "
            ...
            + ", 0",
        1,
        bus1_dep1_stop1.size());
    Assert.assertNull(
        "There should be no previous stop, but there was", bus1_dep1_stop1.get(0).stopPreviousId);
    Assert.assertEquals(
        "Wrong arrivalTimeScheduled",
        1.0,
        bus1_dep1_stop1.get(0).arrivalTimeScheduled,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong arrivalDelay", 0.0, bus1_dep1_stop1.get(0).arrivalDelay, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        3.0, bus1_dep1_stop1.get(0).departureTimeScheduled, MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        veh_bus1_dep1.getType().getCapacity().getSeats()
            + veh_bus1_dep1.getType().getCapacity().getStandingRoom(),
        bus1_dep1_stop1.get(0).totalVehicleCapacity,
        MatsimTestUtils.EPSILON);
    ...
    List<PtStop2StopAnalysis.Stop2StopEntry> bus1_dep1_stop3 =
        ptStop2StopAnalysis.getStop2StopEntriesByDeparture().stream()
            .filter(
                entry ->
                    entry.transitLineId.equals(transitLineId_bus1)
                        && entry.transitRouteId.equals(transitRouteId_bus1_route1)
                        && entry.departureId.equals(departureId_bus1_dep1)
                        && entry.stopId.equals(transitStopFacilityId4)
                        && entry.stopSequence == 2)
            .collect(Collectors.toList());
    ...
    Assert.assertEquals(
        "Either no entry or more than entry for "
            ...
            + departureId_bus1_dep1
            + ", "
            + transitStopFacilityId4
            + ", 0",
        1,
        bus1_dep1_stop3.size());
    Assert.assertEquals(
        "Wrong arrivalDelay", 0.0, bus1_dep1_stop3.get(0).arrivalDelay, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        15.0, bus1_dep1_stop3.get(0).departureTimeScheduled, MatsimTestUtils.EPSILON);
    ...
    List<PtStop2StopAnalysis.Stop2StopEntry> bus1_dep1_stop4 =
        ptStop2StopAnalysis.getStop2StopEntriesByDeparture().stream()
            .filter(
                entry ->
                    entry.transitLineId.equals(transitLineId_bus1)
                        && entry.transitRouteId.equals(transitRouteId_bus1_route2)
                        && entry.departureId.equals(departureId_bus1_dep1)
                        && entry.stopId.equals(transitStopFacilityId2)
                        && entry.stopSequence == 1)
            .collect(Collectors.toList());
    Assert.assertEquals(
        "Either no entry or more than entry for "
            + transitLineId_bus1
            ...
            + ", 0",
        1,
        bus1_dep1_stop4.size());
    Assert.assertEquals(
        "Wrong arrivalTimeScheduled",
        28.0,
        bus1_dep1_stop4.get(0).arrivalTimeScheduled,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong arrivalDelay", 0.0, bus1_dep1_stop4.get(0).arrivalDelay, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        29.0, bus1_dep1_stop4.get(0).departureTimeScheduled, MatsimTestUtils.EPSILON);
    ...
    List<PtStop2StopAnalysis.Stop2StopEntry> train1_dep1_stop4 =
        ptStop2StopAnalysis.getStop2StopEntriesByDeparture().stream()
            .filter(
                entry ->
                    entry.transitLineId.equals(transitLineId_train1)
                        && entry.transitRouteId.equals(transitRouteId_train1_route1)
                        && entry.departureId.equals(departureId_train1_dep1)
                        && entry.stopId.equals(transitStopFacilityId6)
                        && entry.stopSequence == 3)
            .collect(Collectors.toList());
    Assert.assertEquals(
        "Either no entry or more than entry for "
            + transitLineId_train1
            ...
            + departureId_train1_dep1
            + ", "
            + transitStopFacilityId6
            + ", 0",
        1,
        train1_dep1_stop4.size());
    Assert.assertEquals(
        "Wrong arrivalTimeScheduled",
        19.0,
        train1_dep1_stop4.get(0).arrivalTimeScheduled,
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Wrong arrivalDelay", 0.0, train1_dep1_stop4.get(0).arrivalDelay, MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        21.0, train1_dep1_stop4.get(0).departureTimeScheduled, MatsimTestUtils.EPSILON);
