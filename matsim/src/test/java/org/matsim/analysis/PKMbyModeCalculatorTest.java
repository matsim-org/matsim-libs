...
public class PKMbyModeCalculatorTest {
  ...
  public void testPKMbyModeCalculator() {
    ...
    // counting the total distance traveled in each mode
    /****************************************************************************************/
    Double person2CarDist =
        plan2.getPlanElements().stream()
            .filter(Leg.class::isInstance)
            .collect(
                Collectors.summingDouble(
                    l -> {
    ...
    // counting the total distance traveled in each mode
    /****************************************************************************************/
    Double person3CarDist =
        plan3.getPlanElements().stream()
            .filter(Leg.class::isInstance)
            .collect(
                Collectors.summingDouble(
                    l -> {
    ...
    // counting the total distance traveled in each mode
    /****************************************************************************************/
    Double person4PtDist =
        plan4.getPlanElements().stream()
            .filter(Leg.class::isInstance)
            .collect(
                Collectors.summingDouble(
                    l -> {
  ...
  private void performTest(
      IdMap<Person, Plan> map, HashMap<String, Double> modeCalcDist, String outputDirectory) {
    ...
    PKMbyModeCalculator pkmbyModeCalculator =
        new PKMbyModeCalculator(controllerConfigGroup, controlerIO, new GlobalConfigGroup());
    // iteration 0
    pkmbyModeCalculator.addIteration(0, map);
    pkmbyModeCalculator.writeOutput();
    Double totalCarDist =
        modeCalcDist.get("person1CarDist")
            + modeCalcDist.get("person2CarDist")
    ...
    Double totalWalkDist =
        modeCalcDist.get("person1WalkDist")
            + modeCalcDist.get("person2WalkDist")
            + modeCalcDist.get("person3WalkDist")
            + modeCalcDist.get("person4WalkDist");
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        if (iteration == itr) {
          ...
          Assert.assertEquals(
              "Car stats score does not match", Math.round((totalCar / 1000)), carStat, 0);
          Assert.assertEquals(
              "PT stats score does not match", Math.round((totalPt / 1000)), ptStat, 0);
          Assert.assertEquals(
              "Walk stats score does not match", Math.round((totalWalk / 1000)), walkStat, 0);
          break;
        }
        iteration++;
      }
