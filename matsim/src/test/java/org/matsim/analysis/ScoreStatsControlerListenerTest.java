...
public class ScoreStatsControlerListenerTest {
  ...
  public void testScoreStatsControlerListner() throws IOException {
     ...
     ************************************/
    final Plan person1_plan2 = PopulationUtils.createPlan(person1);
    ...
    final Plan person2_plan2 = PopulationUtils.createPlan(person2);
    ...
    final Plan person3_plan1 = PopulationUtils.createPlan(person3);
    ...
    Activity act3_1_4 =
        PopulationUtils.createActivityFromLinkId("shopping", link5); // main mode car
    ...
    final Plan person3_plan2 = PopulationUtils.createPlan(person3);
    ...
    final Plan person4_plan1 = PopulationUtils.createPlan(person4);
    ...
    final Plan person4_plan2 = PopulationUtils.createPlan(person4);
  ...
  private void performTest(String outputDirectory, Population population) throws IOException {
    ...
    OutputDirectoryHierarchy controlerIO =
        new OutputDirectoryHierarchy(
            outputDirectory, OverwriteFileSetting.overwriteExistingFiles, CompressionType.gzip);
    ...
    ScoreStatsControlerListener scoreStatsControlerListener =
        new ScoreStatsControlerListener(
            controllerConfigGroup, population, controlerIO, new GlobalConfigGroup());
  ...
  private void readAndValidateValues(String outDir, int itr, Population population)
      throws IOException {
    ...
    while ((line = br.readLine()) != null) {
      if (iteration == itr) {
        ...
        Assert.assertEquals(
            ...
            (getScore(population, "avgexecuted") / (4 - itr)),
            avgExecuted,
            0);
        Assert.assertEquals(
            "avg. worst score does not match",
            (getScore(population, "avgworst") / (4 - itr)),
            avgWorst,
            0);
        Assert.assertEquals(
            "avg. best score does not match",
            (getScore(population, "avgbest") / (4 - itr)),
            avgBest,
            0);
        Assert.assertEquals(
            "avg average score does not match",
            (getScore(population, "avgaverage") / getNoOfPlans(population)),
            avgAverage,
            0);
      }
      iteration++;
    ...
    Assertions.assertThat(new File(outDir, "scorestats_group1.csv")).isFile();

    Assertions.assertThat(new File(outDir, "scorestats_group2.csv")).isFile();
  }
