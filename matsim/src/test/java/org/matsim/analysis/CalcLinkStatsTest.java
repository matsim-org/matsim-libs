...
public class CalcLinkStatsTest {
   ...
   *
   * <p>Currently, the travel times are only correct if the time bin size in {@link
   * TravelTimeCalculatorConfigGroup} is set to 3600. If the time bin size is set to the default
   * (900, see TODO below), {@link CalcLinkStats} wrongly assumes hourly time bins resulting in
   * wrong travel times.
   *
   * @author ikaddoura
   */
  ...
  public void testAddDataObservedTravelTime() {
    ...
    Assert.assertEquals(
        link2.getLength() / link2.getFreespeed(), cls.getAvgTravelTimes(link2.getId())[3], 1e-8);
