...
public class ActivityFacilitiesSourceTest {
  ...
  // "test/output/"+ActivityFacilitiesSourceTest.class.getCanonicalName().replace('.','/')+"/";

  private final FacilitiesConfigGroup.FacilitiesSource facilitiesSource;
  private final boolean facilitiesWithCoordOnly;

  public ActivityFacilitiesSourceTest(
      FacilitiesConfigGroup.FacilitiesSource facilitiesSource, boolean facilitiesWithCoordOnly) {
  ...
  public static Collection<Object[]> parametersForFacilitiesSourceTest() {
    // it is not clear to me why/how this works.  The documentation of JUnitParamsRunner says that
    // such
    // implicit behavior ("dirty tricks") should no longer be necessary when using it.  kai, jul'18
    return Arrays.asList(
        new Object[] {
          FacilitiesConfigGroup.FacilitiesSource.none, true
        } // true/false doen't matter here
        ,
        new Object[] {
          FacilitiesConfigGroup.FacilitiesSource.fromFile, true
        } // true/false doen't matter here
        ,
        ...
        new Object[] {
          FacilitiesConfigGroup.FacilitiesSource.onePerActivityLinkInPlansFile, true
        } // true/false doen't matter here
        ,
        new Object[] {
          FacilitiesConfigGroup.FacilitiesSource.onePerActivityLocationInPlansFile, true
        } // true/false doen't matter here
        );
  }
  ...
  public void test() {
    ...
    String testOutDir =
        outDir
            + "/"
            + facilitiesSource.toString()
            + "_facilitiesWithCoordOnly_"
            + String.valueOf(facilitiesWithCoordOnly)
            + "/";
    ...
    // (overwriteExistingFiles is needed here for the parameterized test since otherwise all output
    ...
    ActivityFacilities activityFacilities =
        getFacilities(scenario.getConfig().controller().getOutputDirectory());
    switch (this.facilitiesSource) {
      ...
      case setInScenario:
        Assert.assertEquals(
            "wrong number of facilities",
            2,
            activityFacilities.getFacilities().size(),
  ...
  private ActivityFacilities getFacilities(String outputDir) {
    ...
    new FacilitiesReaderMatsimV1(null, null, scenario.getActivityFacilities())
        .readFile(outputDir + "/output_facilities.xml.gz");
  ...
  private Scenario prepareScenario() {
    ...
    if (facilitiesSource.equals(FacilitiesConfigGroup.FacilitiesSource.setInScenario)) {
      ...
      if (facilitiesWithCoordOnly) {
        facilities.addActivityFacility(
            factory.createActivityFacility(
                Id.create("1", ActivityFacility.class), new Coord(-25000.0, 0.), null));
        facilities.addActivityFacility(
            factory.createActivityFacility(
                Id.create("2", ActivityFacility.class), new Coord(10000.0, 0.0), null));
      } else {
        facilities.addActivityFacility(
            factory.createActivityFacility(
                Id.create("1", ActivityFacility.class), null, Id.createLinkId("1")));
        facilities.addActivityFacility(
            factory.createActivityFacility(
                Id.create("2", ActivityFacility.class), null, Id.createLinkId("20")));
      }
    }
    ...
    if (!this.facilitiesSource.equals(
        FacilitiesConfigGroup.FacilitiesSource.onePerActivityLocationInPlansFile)) {
  ...
  private boolean assignFacilityIdToActivity(
      FacilitiesConfigGroup.FacilitiesSource facilitiesSource) {
    return facilitiesSource.equals(FacilitiesConfigGroup.FacilitiesSource.setInScenario)
        || facilitiesSource.equals(FacilitiesConfigGroup.FacilitiesSource.fromFile);
  }
}
