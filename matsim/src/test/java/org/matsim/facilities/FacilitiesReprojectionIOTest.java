...
public class FacilitiesReprojectionIOTest {
  ...
  public void testReprojectionAtImport() {
    ...
    new MatsimFacilitiesReader(originalScenario)
        .parse(IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("chessboard"), "facilities.xml"));
    new MatsimFacilitiesReader(INITIAL_CRS, TARGET_CRS, reprojectedScenario.getActivityFacilities())
        .parse(IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("chessboard"), "facilities.xml"));
  ...
  public void testWithControlerAndObjectAttributes() {
    ...
    new FacilitiesWriter(originalScenario.getActivityFacilities())
        .write(utils.getOutputDirectory() + "/facilities.xml");
    ...
    for (Id<ActivityFacility> id :
        originalScenario.getActivityFacilities().getFacilities().keySet()) {
      final Coord originalCoord =
          originalScenario.getActivityFacilities().getFacilities().get(id).getCoord();
      final Coord internalCoord =
          scenario.getActivityFacilities().getFacilities().get(id).getCoord();
    ...
    new MatsimFacilitiesReader(dumpedScenario)
        .readFile(outputDirectory + "/output_facilities.xml.gz");

    for (Id<ActivityFacility> id : scenario.getActivityFacilities().getFacilities().keySet()) {
      final Coord internalCoord =
          scenario.getActivityFacilities().getFacilities().get(id).getCoord();
      final Coord dumpedCoord =
          dumpedScenario.getActivityFacilities().getFacilities().get(id).getCoord();
  ...
  public void testWithControlerAndConfigParameters() {
    ...
    config
        .facilities()
        .setInputFile(
            IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("chessboard"), "facilities.xml")
                .toString());
  ...
  private void assertScenarioReprojectedCorrectly(
      Scenario originalScenario, Scenario reprojectedScenario) {
  ...
  private void assertReprojectedCorrectly(
      ActivityFacility originalFacility, ActivityFacility reprojectedFacility) {
    final Coord original = originalFacility.getCoord();
    final Coord transformed = reprojectedFacility.getCoord();

    Assert.assertEquals(
        "wrong reprojected coordinate", transformation.transform(original), transformed);
  }
}
