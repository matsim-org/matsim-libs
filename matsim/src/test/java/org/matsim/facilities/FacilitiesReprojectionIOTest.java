...
public class FacilitiesReprojectionIOTest {
	...
	public void testReprojectionAtImport() {
		...
		new MatsimFacilitiesReader( originalScenario ).parse(IOUtils.newUrl(utils.getScenario("chessboard"), "facilities.xml"));
		new MatsimFacilitiesReader( new Transformation() , reprojectedScenario ).parse(IOUtils.newUrl(utils.getScenario("chessboard"), "facilities.xml"));
	...
	public void testWithControlerAndConfigParameters() {
		...
		config.facilities().setInputFile( IOUtils.newUrl(utils.getScenario("chessboard"), "facilities.xml").toString() );
