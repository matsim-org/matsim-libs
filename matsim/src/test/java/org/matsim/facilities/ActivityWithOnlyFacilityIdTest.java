...
public class ActivityWithOnlyFacilityIdTest {
  ...
  public void testSiouxFallsWithOnlyFacilityIds() {
    ...
    Set<Activity> activitiesWithFacilityIds =
        scenario.getPopulation().getPersons().values().stream()
            .flatMap(person -> person.getPlans().stream())
            .flatMap(plan -> plan.getPlanElements().stream())
            .filter(Activity.class::isInstance)
            .map(Activity.class::cast)
            .filter(act -> act.getFacilityId() != null)
            .collect(Collectors.toSet());
    assertTrue(
        "Need at least some activities with facilityIds.", activitiesWithFacilityIds.size() > 0);

    // Remove all (redundant) coords and linkIds from activities with facilityIds.
    activitiesWithFacilityIds.forEach(
        act -> {
          act.setLinkId(null);
          act.setCoord(null);
        });
