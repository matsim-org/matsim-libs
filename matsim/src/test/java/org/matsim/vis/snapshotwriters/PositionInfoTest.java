...
public class PositionInfoTest {
  ...
  /**
   * Test the calculation of a vehicles position if the actual link length is shorter than the
   * euklidean distance.
   *
   * @author mrieser
   */
  ...
  public void testDistanceOnLink_shortLink() {
    ...
    Node node2 =
        NetworkUtils.createAndAddNode(network, Id.create("2", Node.class), new Coord(1000, 1000));
    Link link1 =
        NetworkUtils.createAndAddLink(
            network, Id.create("1", Link.class), node1, node2, 1000, 10, 9999, 1);

    // place the vehicle at one quarter of the link
    SnapshotLinkWidthCalculator linkWidthCalculator = new SnapshotLinkWidthCalculator();

    var posInfo =
        new PositionInfo.LinkBasedBuilder()
    ...
    // AgentSnapshotInfo posInfo = new
    // AgentSnapshotInfoFactory(linkWidthCalculator).createAgentSnapshotInfo(Id.create(1,
    // Person.class), link1, 250, 0);
    assertEquals(260.60660171779824, posInfo.getEasting(), epsilon);
    assertEquals(239.3933982822018, posInfo.getNorthing(), epsilon);
    // These numbers became a little weird when I moved vehicles away from the center of a link.
    // Kai, Dec/08
  ...
  /**
   * Test the calculation of a vehicles position if the actual link length is longer than the
   * euklidean distance.
   *
   * @author mrieser
   */
  ...
  public void testDistanceOnLink_longLink() {
    ...
    Link link1 =
        NetworkUtils.createAndAddLink(
            network, Id.create("1", Link.class), node1, node2, 2000, 10, 9999, 1);
