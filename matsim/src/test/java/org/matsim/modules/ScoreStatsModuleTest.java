...
public class ScoreStatsModuleTest {
  ...
  public ScoreStatsModuleTest(
      boolean isUsingFastCapacityUpdate, boolean isInsertingAccessEgressWalk) {
  ...
  public void testScoreStats() {
    ...
    config
        .routing()
        .setAccessEgressType(
            this.isInsertingAccessEgressWalk
                ? AccessEgressType.accessEgressModeToLink
  ...
  private static class MyControlerListener implements ShutdownListener {
    ...
    public void notifyShutdown(ShutdownEvent event) {
      ...
      // yy the following is retrofitted from an older double[][] data structure and thus messy.
      // Please improve it.  kai, nov'16
      if (config.routing().getAccessEgressType().equals(AccessEgressType.none)) {
        ...
        // yyyy these change with the access/egress car router, but I cannot say if the magnitude of
        // change is plausible. kai, feb'16
        //					if(config.qsim().isUsingFastCapacityUpdate()) {
        {
          Double[] array = result.get(ScoreItem.worst).values().toArray(new Double[0]);
          Assert.assertEquals(
              new double[] {53.18953957492432, 38.73201822323088}[0], array[0], DELTA);
          Assert.assertEquals(
              new double[] {53.18953957492432, 38.73201822323088}[1], array[1], DELTA);
        }
        {
          Double[] array = result.get(ScoreItem.best).values().toArray(new Double[0]);
          Assert.assertEquals(
              new double[] {53.18953957492432, 53.2163372155953}[0], array[0], DELTA);
          Assert.assertEquals(
              new double[] {53.18953957492432, 53.2163372155953}[1], array[1], DELTA);
        }
        {
          Double[] array = result.get(ScoreItem.average).values().toArray(new Double[0]);
          Assert.assertEquals(
              new double[] {53.18953957492432, 45.9741777194131}[0], array[0], DELTA);
          Assert.assertEquals(
              new double[] {53.18953957492432, 45.9741777194131}[1], array[1], DELTA);
        }
        {
          Double[] array = result.get(ScoreItem.executed).values().toArray(new Double[0]);
          Assert.assertEquals(
        ...
        //						{
        //						Double[] array = result.get(ScoreItem.worst).values().toArray(new Double[0]) ;
        //						Assert.assertEquals(new double[]{53.18953957492432, 38.73119275042525}[0], array[0],
        // DELTA);
        //						Assert.assertEquals(new double[]{53.18953957492432, 38.73119275042525}[1], array[1],
        // DELTA);
        //						}{
        //						Double[] array = result.get(ScoreItem.best).values().toArray(new Double[0]) ;
        //						Assert.assertEquals(new double[]{53.18953957492432, 53.2163372155953}[0], array[0],
        // DELTA);
        //						Assert.assertEquals(new double[]{53.18953957492432, 53.2163372155953}[1], array[1],
        // DELTA);
        //						}{
        //						Double[] array = result.get(ScoreItem.average).values().toArray(new Double[0]) ;
        //						Assert.assertEquals(new double[]{53.18953957492432, 45.97376498301028}[0], array[0],
        // DELTA);
        //						Assert.assertEquals(new double[]{53.18953957492432, 45.97376498301028}[1], array[1],
        // DELTA);
        //						}{
        //						Double[] array = result.get(ScoreItem.executed).values().toArray(new Double[0]) ;
        //						Assert.assertEquals(new double[]{53.18953957492432, 38.73119275042525}[0], array[0],
