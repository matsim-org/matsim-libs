...
public class PtScoringTest {
  ...
  @Parameter public TypicalDurationScoreComputation typicalDurationScoreComputation;

  @Parameterized.Parameters
  public static Object[] testParameters() {
    return new Object[] {
      TypicalDurationScoreComputation.relative, TypicalDurationScoreComputation.uniform
    };
  }
  ...
  public void test_PtScoringLineswitch() {
    Config config =
        this.utils.loadConfig(
            IOUtils.extendUrl(
                ExamplesUtils.getTestScenarioURL("pt-simple-lineswitch"), "config.xml"));
    ...
    //		double zeroUtilityDurationHome_s =
    // CharyparNagelScoringUtils.computeZeroUtilityDuration_s(priority, typicalDuration_s);
    ActivityUtilityParameters.Builder builder =
        new ActivityUtilityParameters.Builder(pcs.getActivityParams("home"));
    ...
    double score =
        pcs.getModes().get(TransportMode.walk).getMarginalUtilityOfTraveling()
            * (stop1Arr - homeAct1End)
            / 3600.;
    ...
    score +=
        pcs.getModes().get(TransportMode.pt).getMarginalUtilityOfTraveling()
            * (enterVeh - ptIA1ActEnd)
            / 3600.;
    ...
    score +=
        pcs.getModes().get(TransportMode.walk).getMarginalUtilityOfTraveling()
            * (home2Arr - ptIA2ActEnd)
            / 3600.;
    System.out.println("score after walk: " + score);

    final double duration = homeAct2End - home2Arr;
    double tmpScore =
        (pcs.getPerforming_utils_hr() / 3600.)
            * typicalDuration_s
            * Math.log(duration / zeroUtilityDurationHome_s);
    if (tmpScore < 0) {
      System.out.println("home2score< 0; replacing ... ");
      double slope =
          (pcs.getPerforming_utils_hr() / 3600.) * typicalDuration_s / zeroUtilityDurationHome_s;
    ...
    for (Person pp : pop.getPersons().values()) {
      ...
      if (this.typicalDurationScoreComputation.equals(TypicalDurationScoreComputation.uniform)) {
        Assert.assertEquals(
            -21.280962467387187, pp.getSelectedPlan().getScore(), MatsimTestUtils.EPSILON);
      } else {
        Assert.assertEquals(
            27.468448990195423, pp.getSelectedPlan().getScore(), MatsimTestUtils.EPSILON);
      }
    }
  }
  ...
  public void test_PtScoringLineswitchAndPtConstant() {
    ...
    for (Person pp : pop.getPersons().values()) {
      ...
      if (this.typicalDurationScoreComputation.equals(TypicalDurationScoreComputation.uniform)) {
        //				Assert.assertEquals(89.14608279715044,
        // pp.getSelectedPlan().getScore(),MatsimTestUtils.EPSILON ) ;
        Assert.assertEquals(
            -19.280962467387187, pp.getSelectedPlan().getScore(), MatsimTestUtils.EPSILON);
      } else {
        Assert.assertEquals(
            29.468448990195423, pp.getSelectedPlan().getScore(), MatsimTestUtils.EPSILON);
      }
    }
  }
  ...
  public void test_PtScoring_Wait() {
    Config config =
        this.utils.loadConfig(
            IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("pt-simple"), "config.xml"));
    ScoringConfigGroup pcs = config.scoring();
    ...
    score +=
        (pcs.getPerforming_utils_hr() / 3600.)
            * typicalDuration_s
            * Math.log(timeHome / zeroUtilityDurationHome_s);
    ...
    for (Person pp : pop.getPersons().values()) {
      ...
      if (this.typicalDurationScoreComputation.equals(TypicalDurationScoreComputation.uniform)) {
        Assert.assertEquals(
            89.13108279715044, pp.getSelectedPlan().getScore(), MatsimTestUtils.EPSILON);
      } else {
        Assert.assertEquals(
            137.1310827971504, pp.getSelectedPlan().getScore(), MatsimTestUtils.EPSILON);
      }
    }
  }
  ...
  public void test_PtScoring() {
    ...
    for (Person pp : pop.getPersons().values()) {
      ...
      if (this.typicalDurationScoreComputation.equals(TypicalDurationScoreComputation.uniform)) {
        Assert.assertEquals(
            89.87441613048377, pp.getSelectedPlan().getScore(), MatsimTestUtils.EPSILON);
      } else {
        Assert.assertEquals(
            137.87441613048375, pp.getSelectedPlan().getScore(), MatsimTestUtils.EPSILON);
      }
    }
  }
}
