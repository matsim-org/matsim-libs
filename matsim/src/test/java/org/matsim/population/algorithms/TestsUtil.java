...
public class TestsUtil {
  ...
  static Plan createPlanFromFacilities(
      ActivityFacilitiesImpl layer, Person person, String mode, String facString) {
    Plan plan = PopulationUtils.createPlan(person);
    String[] locationIdSequence = facString.split(" ");
    for (int aa = 0; aa < locationIdSequence.length; aa++) {
      ActivityFacility location =
          layer.getFacilities().get(Id.create(locationIdSequence[aa], ActivityFacility.class));
      Activity act;
    ...
    for (int aa = 0; aa < locationIdSequence.length; aa++) {
      ...
      act =
          PopulationUtils.createAndAddActivityFromLinkId(
              plan, "actOnLink" + locationIdSequence[aa], (Id<Link>) location.getId());
      act.setEndTime(10 * 3600);
      if (aa != (locationIdSequence.length - 1)) {
        PopulationUtils.createAndAddLeg(plan, mode);
      }
  ...
  static boolean equals(PlanElement o1, PlanElement o2) {
    if (o1 instanceof Leg) {
      ...
      if (o2 instanceof Activity) {
        ...
        if (activity1.getEndTime().isDefined()
            && activity1.getEndTime().seconds() != activity2.getEndTime().seconds()) {
          return false;
        }
        if (activity1.getStartTime().isUndefined() ^ activity2.getStartTime().isUndefined()) {
          return false;
        }
        if (activity1.getStartTime().isDefined()
            && activity1.getStartTime().seconds() != activity2.getStartTime().seconds()) {
          return false;
        }
      } else {
        return false;
  ...
  public static boolean equals(List<PlanElement> planElements, List<PlanElement> planElements2) {
