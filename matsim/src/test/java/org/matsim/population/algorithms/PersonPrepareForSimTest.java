...
public class PersonPrepareForSimTest {
  ...
  public void testRun_MultimodalNetwork() {
    ...
    Assert.assertEquals(
        link1id, activity2.getLinkId()); // must also be linked to l1, as l2 has no car mode
  }
  ...
  public void testRun_MultimodalScenario() {
    ...
    Assert.assertEquals(
        link1id, a2.getLinkId()); // must also be linked to l1, as l2 has no car mode
  }
  ...
  /**
   * Fallback modes are outdated with the introduction of routingMode. So, we want the simulation to
   * crash if we encounter them <b>after</b> {@link PrepareForSimImpl} was run (and adapted outdated
   * plans). However, for the time being we do not explicitly check for outdated modes and hope that
   * an exception will be thrown during routing of that single leg trip, because no router should be
   * registered for those modes (and wasn't registered before introducing routingMode, besides
   * "transit_walk" which was also used for access/egress to pt and transfer between pt and
   * therefore is checked explicitly).
   */
  @Test
  public void testSingleFallbackModeLegTrip() {
    ...
    // test outdated fallback mode single leg trip (arbitrary drt mode)
    {
      ...
      Assert.assertEquals(
          "wrong routing mode set", "drt67_fallback", TripStructureUtils.getRoutingMode(leg));
    }
  }
  ...
  public void testCorrectTripsRemainUnchanged() {
    ...
    // test complicated intermodal trip with consistent routing modes passes unchanged
    {
      ...
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg1));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg2));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg3));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg4));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg5));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg6));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg7));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg8));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg9));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg10));
      Assert.assertEquals(
          "wrong routing mode set", TransportMode.pt, TripStructureUtils.getRoutingMode(leg11));
    }
  }
  ...
  public void testReplaceExperimentalTransitRoute() {
    ...
    ExperimentalTransitRoute experimentalTransitRoute =
        new ExperimentalTransitRoute(stopFacility1, stopFacility2, line, route);
    ...
    Assert.assertEquals(
        stopFacility1.getId(), ((DefaultTransitPassengerRoute) leg.getRoute()).getAccessStopId());
    Assert.assertEquals(
        stopFacility2.getId(), ((DefaultTransitPassengerRoute) leg.getRoute()).getEgressStopId());
