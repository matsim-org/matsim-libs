...
public class QSimIntegrationTest {
  ...
  public void testFreespeed() {
    ...
    TestTravelTimeCalculator ttcalc =
        new TestTravelTimeCalculator(person1.getId(), person2.getId(), link2.getId());
    ...
    assertEquals(
        "Person 1 should travel for 11 seconds.",
        10.0 + 1.0,
        ttcalc.person1leaveTime - ttcalc.person1enterTime,
        MatsimTestUtils.EPSILON);
    assertEquals(
        "Person 2 should travel for 6 seconds.",
        5.0 + 1.0,
        ttcalc.person2leaveTime - ttcalc.person2enterTime,
        MatsimTestUtils.EPSILON);
  }

  /**
   * Test the queue simulation for correct behavior if capacity of links is reduced during the run.
   *
   * @author illenberger
   */
  @Test
  public void testCapacity() {
     ...
     */
    assertEquals(
        "Person 1 should travel for 20 seconds.",
        20.0,
        ttcalc.person1leaveTime - ttcalc.person1enterTime,
        MatsimTestUtils.EPSILON);
    assertEquals(
        "Person 2 should travel for 11 seconds.",
        10.0 + 1.0,
        ttcalc.person2leaveTime - ttcalc.person2enterTime,
        MatsimTestUtils.EPSILON);
  }
  ...
  /**
   * Test the queue simulation for correct behavior if capacity of links is reduced to 0.
   *
   * @author dgrether
   */
  @Test
  ...
  /**
   * Creates a network with three links of length 100 m, capacity 3600 veh/h and freespeed 10 m/s.
   *
   * @author illenberger
   */
  private static Network createNetwork(Scenario scenario) {
    ...
    // the network has 4 nodes and 3 links, each link by default 100 long and freespeed = 10 -->
    // freespeed travel time = 10.0
    Node node1 =
  ...
  /**
   * Creates <tt>count</tt> persons with departure time <tt>depTime<tt> + index(person).
   *
    ...
    for (int i = 0; i < count; i++) {
      Person person =
          PopulationUtils.getFactory()
              .createPerson(Id.create(i + (int) departureTime, Person.class));
  ...
  /**
   * A special EventHandler to get the link enter and link leave time for two persons on one
   * specific link.
   *
   * @author mrieser
   */
  private static class TestTravelTimeCalculator
      implements PersonEntersVehicleEventHandler, LinkEnterEventHandler, LinkLeaveEventHandler {
    ...
    protected TestTravelTimeCalculator(
        final Id<Person> personId1, final Id<Person> personId2, final Id<Link> linkId) {
