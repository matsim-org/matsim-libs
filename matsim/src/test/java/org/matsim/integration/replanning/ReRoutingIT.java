...
public class ReRoutingIT {
  ...
  private Scenario loadScenario() {
    ...
    config
        .network()
        .setInputFile(
            IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("berlin"), "network.xml.gz")
                .toString());
    config
        .plans()
        .setInputFile(
            IOUtils.extendUrl(ExamplesUtils.getTestScenarioURL("berlin"), "plans_hwh_1pct.xml.gz")
                .toString());
  ...
  private static final Logger LOG = LogManager.getLogger(ReRoutingIT.class);
  ...
  private void evaluate(String plansFilename) throws MalformedURLException {
    ...
    config
        .plans()
        .setInputFile(
            new File(utils.getOutputDirectory() + "ITERS/it.1/1.plans.xml.gz")
                .toURI()
                .toURL()
                .toString());
    Scenario scenario = ScenarioUtils.loadScenario(config);
    ...
    final boolean isEqual =
        PopulationUtils.equalPopulation(
            referenceScenario.getPopulation(), scenario.getPopulation());
    Gbl.printElapsedTime();
    if (!isEqual) {
      new PopulationWriter(referenceScenario.getPopulation(), scenario.getNetwork())
          .write(utils.getOutputDirectory() + "/reference_population.xml.gz");
      new PopulationWriter(scenario.getPopulation(), scenario.getNetwork())
          .write(utils.getOutputDirectory() + "/output_population.xml.gz");
    }
