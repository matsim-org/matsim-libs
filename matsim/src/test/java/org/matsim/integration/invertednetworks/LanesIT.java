...
public class LanesIT {
        ...
        new AbstractModule() {
          ...
          public void install() {
            ...
            addMobsimListenerBinding()
                .toInstance(
                    new MobsimInitializedListener() {
                      @Override
                      public void notifyMobsimInitialized(MobsimInitializedEvent e) {
                        Assert.assertTrue(e.getQueueSimulation() instanceof QSim);
                        QSim qsim = (QSim) e.getQueueSimulation();
                        NetsimLink link =
                            qsim.getNetsimNetwork().getNetsimLink(Id.create("23", Link.class));
                        ...
                        //				log.error("lane " + lane.getId() + " flow " +
                        // lane.getSimulatedFlowCapacity());
  ...
  private static class TestListener implements IterationEndsListener, ShutdownListener {
    ...
    public void notifyShutdown(ShutdownEvent event) {
      ...
      Assert.assertEquals(
          "lane to link 34 should have approx. 600 veh/h, i.e. 16.6% of the total flow",
          16.6, percent34, 1);
      Assert.assertEquals(
          "lane to link 35 should have approx. 1200 veh/h, i.e. 33.3% of the total flow",
          33.3, percent35, 1);
      Assert.assertEquals(
          "lane to link 36 should have approx. 1800 veh/h, i.e. 50.0% of the total flow",
          50.0, percent36, 1);
    }
  }
