 ...
 * @author mrieser
 */
public class MultimodalQSimIntegrationTest {

	@Rule public MatsimTestUtils util = new MatsimTestUtils();

	@Test
	public void testUseMultimodalSimFactory() {
		...
		final MultimodalMobsimTestController controler = new MultimodalMobsimTestController(config);
		controler.setCreateGraphs(false);
		controler.run();
		Assert.assertEquals(1, controler.callCounter);
	}

	/*package*/ static class MultimodalMobsimTestController extends Controler {
		/*package*/ int callCounter = 0;
		public MultimodalMobsimTestController(Config config) {
			super(config);
		}
		@Override
		protected void runMobSim() {
			Assert.assertTrue(this.getMobsimFactory() instanceof MultiModalMobsimFactory);
			this.callCounter++;
		}
	}
}
