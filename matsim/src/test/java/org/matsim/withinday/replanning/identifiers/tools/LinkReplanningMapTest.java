...
public class LinkReplanningMapTest extends MatsimTestCase {
	...
	private static class MobsimListenerForTests implements SimulationInitializedListener, SimulationBeforeSimStepListener, 
		...
		private LinkReplanningMap lrp;
		private int t1 = 5*3600 + 58*60 + 30;
		private int t2 = 5*3600 + 59*60;
		private int t3 = 5*3600 + 59*60 + 30;
		private int t4 = 6*3600;
		
		public MobsimListenerForTests(LinkReplanningMap lrp) {
			this.lrp = lrp;
		}
		
		@Override
		public void notifySimulationInitialized(SimulationInitializedEvent e) {			
			assertEquals(0, lrp.getLegPerformingAgents().size());	// no agent performs a Leg
			assertEquals(0, lrp.getReplanningAgents(0.0).size());	// no agent needs a replanning
		}
		...
		public void notifySimulationBeforeSimStep(SimulationBeforeSimStepEvent e) {
			...
			if (e.getSimulationTime() == t1) {
				assertEquals(0, lrp.getLegPerformingAgents().size());	// no agent performs a Leg
				assertEquals(0, lrp.getReplanningAgents(e.getSimulationTime()).size());	// no agent needs a replanning
			}

			if (e.getSimulationTime() == t2) {
				assertEquals(1, lrp.getLegPerformingAgents().size());	// one performs a Leg
				assertEquals(0, lrp.getReplanningAgents(e.getSimulationTime()).size());	// no agent needs a replanning
			}
			
			if (e.getSimulationTime() == t3) {
				assertEquals(2, lrp.getLegPerformingAgents().size());	// two agents perform a Leg
				assertEquals(0, lrp.getReplanningAgents(e.getSimulationTime()).size());	// no agent needs a replanning
			}
			
			if (e.getSimulationTime() == t4) {
				assertEquals(3, lrp.getLegPerformingAgents().size());	// three agents perform a Leg
				assertEquals(0, lrp.getReplanningAgents(e.getSimulationTime()).size());	// no agent needs a replanning
			}
		}
		...
		public void notifySimulationAfterSimStep(SimulationAfterSimStepEvent e) {
			if (e.getSimulationTime() == t1) {
				assertEquals(1, lrp.getLegPerformingAgents().size());	// one agent performs a Leg
				assertEquals(1, lrp.getReplanningAgents(e.getSimulationTime()).size());	// one agent has just departed and might do a replanning
			}
			...
			if (e.getSimulationTime() == t4) {
				assertEquals(100, lrp.getLegPerformingAgents().size());	// all agents  perform a Leg
				assertEquals(97, lrp.getReplanningAgents(e.getSimulationTime()).size());	// 97 agents have just departed and might do a replanning
			}
		}
	}
}
