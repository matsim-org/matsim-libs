...
public class ChangeLegModeTest {
  ...
  public void test_behavior_allowSwitchFromListedModesOnly() {
    ...
    config.setParam(
        ChangeModeConfigGroup.CONFIG_MODULE,
        ChangeModeConfigGroup.CONFIG_PARAM_MODES,
        "pt,bike,walk"); // do not include car
    config.setParam(
        ChangeModeConfigGroup.CONFIG_MODULE,
        ChangeModeConfigGroup.MODE_SWITCH_BEHAVIOR,
  ...
  public void testWithConstructor() {
    final ChangeLegMode module =
        new ChangeLegMode(0, new String[] {"car", "pt", "bike", "walk"}, true, false);
    final String[] modes =
        new String[] {TransportMode.car, TransportMode.pt, TransportMode.bike, TransportMode.walk};
    runTest(module, modes);
  }
