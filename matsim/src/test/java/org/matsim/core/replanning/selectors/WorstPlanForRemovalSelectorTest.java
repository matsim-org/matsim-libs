...
public class WorstPlanForRemovalSelectorTest extends AbstractPlanSelectorTest {
  ...
  public void testRemoveWorstPlans_nullType() {
    ...
    assertFalse(
        "test that plan with undefined score was removed.", person.getPlans().contains(plan3));

    person.getPlans().remove(selector.selectPlan(person));
    assertEquals("test that a plan was removed", 3, person.getPlans().size());
    assertFalse(
        "test that the plan with minimal score was removed", person.getPlans().contains(plan4));
  ...
  public void testRemoveWorstPlans_withTypes() {
    ...
    assertFalse(
        "test that the plan with worst score was removed", person.getPlans().contains(plan1));
    assertTrue(
        "test that the now only plan of type a was not removed", person.getPlans().contains(plan5));
    assertFalse(
        "test that the plan with the 2nd-worst score was removed",
        person.getPlans().contains(plan6));

    person.getPlans().remove(selector.selectPlan(person));
    assertEquals("test that one plan was removed", 1, person.getPlans().size());
    assertTrue(
        "test that the plan with highest score of type b was not removed",
        person.getPlans().contains(plan2));
  }
}
