...
/**
 * Tests the functionality of {@link TimeAllocationMutatorModule}, mainly that the correct mutation
 * range is handed over to the underlying {@link MutateActivityTimeAllocation}.
 *
 * @author mrieser
 */
public class TimeAllocationMutatorModuleTest {
  ...
  public void testSimplifiedMutation() {
    ...
    runSimplifiedMutationRangeTest(
        new MutateActivityTimeAllocation(
            750, affectingDuration, MatsimRandom.getLocalInstance(), 24 * 3600, false, 1),
        750);
    runSimplifiedMutationRangeTest(
        new MutateActivityTimeAllocation(
            7200, affectingDuration, MatsimRandom.getLocalInstance(), 24 * 3600, false, 1),
        7200);
  }

  /**
   * Internal helper method to run the real test, but with different setups. Basically, it creates
   * one plan and calls the given TimeAllocationMutator several times with this plans, each time
   * measuring how much the activity durations have changed and thus ensuring, the differences are
   ...
   */
  private void runSimplifiedMutationRangeTest(
      final PlanAlgorithm tripPlanMutateTimeAllocation, final int expectedMutationRange) {
    // setup network
    ...
    Node node2 =
        NetworkUtils.createAndAddNode(network, Id.create("2", Node.class), new Coord(100, 0));
    Node node3 =
        NetworkUtils.createAndAddNode(network, Id.create("3", Node.class), new Coord(200, 0));
    Node node4 =
        NetworkUtils.createAndAddNode(network, Id.create("4", Node.class), new Coord(300, 0));
    final Node fromNode = node1;
    final Node toNode = node2;
    Link link1 =
        NetworkUtils.createAndAddLink(
            network, Id.create("0", Link.class), fromNode, toNode, 100, 5, 100, 1);
    final Node fromNode1 = node2;
    final Node toNode1 = node3;
    NetworkUtils.createAndAddLink(
        network, Id.create("1", Link.class), fromNode1, toNode1, 100, 5, 100, 1);
    final Node fromNode2 = node3;
    final Node toNode2 = node4;
    NetworkUtils.createAndAddLink(
        network, Id.create("2", Link.class), fromNode2, toNode2, 100, 5, 100, 1);
    ...
    assertValueInRange(
        "mutation range out of range (maxDiff1).",
        maxDiff1,
        expectedMutationRange * 0.95,
        expectedMutationRange);
    assertValueInRange(
        "mutation range out of range (minDiff1).",
        minDiff1,
        -expectedMutationRange,
        -expectedMutationRange * 0.95);
    assertValueInRange(
        "mutation range out of range (maxDiff2).",
        maxDiff1,
        expectedMutationRange * 0.95,
        expectedMutationRange);
    assertValueInRange(
        "mutation range out of range (minDiff2).",
        minDiff2,
        -expectedMutationRange,
        -expectedMutationRange * 0.95);
  }

  private static void assertValueInRange(
      final String message, final double actual, final double lowerLimit, final double upperLimit) {
    assertTrue(
        message + " actual: " + actual + ", range: " + lowerLimit + "..." + upperLimit,
        (lowerLimit <= actual) && (actual <= upperLimit));
  }
  ...
  public void testRun() {
    ...
    Activity ptAct1 =
        PopulationUtils.createAndAddActivityFromCoord(
            plan, PtConstants.TRANSIT_ACTIVITY_TYPE, new Coord(0, 100));
    ...
    MutateActivityTimeAllocation mutator =
        new MutateActivityTimeAllocation(
            3600., affectingDuration, new Random(2011), 24 * 3600, false, 1);
    mutator.run(plan);
  ...
  public void testRunLatestEndTime() {
    ...
    MutateActivityTimeAllocation mutator =
        new MutateActivityTimeAllocation(
            3600., affectingDuration, new Random(2011), latestEndTime, false, 1);
  ...
  public void testLegTimesAreSetCorrectly() {
    ...
    Assert.assertEquals(
        firstActEndTime, leg1.getDepartureTime().seconds(), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        firstActEndTime + secondActDuration + leg1.getTravelTime().seconds(),
        leg2.getDepartureTime().seconds(),
