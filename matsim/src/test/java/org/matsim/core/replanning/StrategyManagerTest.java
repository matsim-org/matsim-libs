...
public class StrategyManagerTest {
  ...
  /**
   * This method tests, if adding strategies and strategyRequest get correctly executed and if,
   * after changes are performed, still all required strategies are called according to their
   * weights.
   *
   * @author mrieser
   */
  ...
  /**
   * Tests the removal of strategies. Ensures that after removal, no plan is given to the removed
   * strategy. Also checks that the removal of strategies not known to the StrategyManager doesn't
  ...
  /**
   * This method tests that the StrategyManager uses a so-called "optimistic behavior" when
   * selecting plans for replanning/execution. Optimistic Behavior means that plans with undefined
   * score are chosen before any other plan with defined score.
   *
   * @author mrieser
   */
  @Test
  ...
  public void testSetPlanSelectorForRemoval() {
    ...
    assertTrue(
        "plan should not have been removed.", p.getPlans().contains(plans[plans.length - 3]));
  }
  ...
  /**
   * A simple extension to the PlanStrategy which counts how often it was called.
   *
   * @author mrieser
   */
  ...
  /**
   * A simple PlanSelector that throws an UnsupportedOperationException whenever it should select a
   * plan.
   *
   * @author mrieser
   */
