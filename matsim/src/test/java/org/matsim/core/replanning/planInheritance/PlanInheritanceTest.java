  ...
  public void testPlanInheritanceEnabled() throws IOException {
    ...
    for (Person per : personList) {
      for (Plan p : per.getPlans()) {
        ...
        assert (p.getAttributes()
            .getAsMap()
            .keySet()
            .contains(PlanInheritanceModule.ITERATION_CREATED));
        assert (p.getAttributes().getAsMap().keySet().contains(PlanInheritanceModule.PLAN_ID));
      }
    }

    PlanInheritanceRecordReader reader =
        new PlanInheritanceRecordReader(outputDirectory + "planInheritanceRecords.csv.gz");
    ...
    assert (((PlanInheritanceRecord) records.get(0))
        .getAncestorId()
        .equals(Id.create("NONE", Plan.class)));
    assert (((PlanInheritanceRecord) records.get(0))
        .getMutatedBy()
        .equals(PlanInheritanceModule.INITIAL_PLAN));
    ...
    assert (((PlanInheritanceRecord) records.get(0))
        .getPlanId()
        .equals(Id.create("1", Plan.class)));
    assert (((PlanInheritanceRecord) records.get(0))
        .getIterationsSelected()
        .equals(Arrays.asList(0, 1, 2, 3, 4, 6, 7, 8, 9, 10)));

    assert (((PlanInheritanceRecord) records.get(1)).getAgentId().equals(Id.createPersonId("1")));
    assert (((PlanInheritanceRecord) records.get(1))
        .getAncestorId()
        .equals(Id.create("1", Plan.class)));
    assert (((PlanInheritanceRecord) records.get(1))
        .getMutatedBy()
        .equals("RandomPlanSelector_ReRoute"));
    ...
    for (Person per : personList) {
      for (Plan p : per.getPlans()) {
        assert (!p.getAttributes()
            .getAsMap()
            .keySet()
            .contains(PlanInheritanceModule.PLAN_MUTATOR));
        assert (!p.getAttributes()
            .getAsMap()
            .keySet()
            .contains(PlanInheritanceModule.ITERATION_CREATED));
