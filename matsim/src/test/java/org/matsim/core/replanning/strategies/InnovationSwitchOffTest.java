...
public class InnovationSwitchOffTest {
  ...
  public void testInnovationSwitchOff() {
    ...
    }
    {
      ...
      settings.setStrategyName(
          DefaultPlanStrategiesModule.DefaultStrategy.TimeAllocationMutator.toString());
        ...
        Injector.createInjector(
            ...
            new AbstractModule() {
              ...
              public void install() {
                ...
                final Provider<StrategyManager> strategyManagerProvider =
                    binder().getProvider(StrategyManager.class);
                    ...
                    .toInstance(
                        new BeforeMobsimListener() {
                          ...
                          public void notifyBeforeMobsim(BeforeMobsimEvent event) {
                            ...
                            final StrategyManager sm =
                                strategyManagerProvider
                                    .get(); // move into controler package if access to sm is a
                            // problem. kai, jun'13
                            for (int ii = 0; ii < sm.getStrategies(null).size(); ii++) {
                              log.warn(
                                  "strategy "
                                      + sm.getStrategies(null).get(ii)
                                      + " has weight "
                                      + sm.getWeights(null).get(ii));
                              if (event.getIteration() == 11
                                  && sm.getStrategies(null)
                                      .get(ii)
                                      .toString()
                                      .contains(ReRoute.class.getSimpleName())) {
                                Assert.assertEquals(0.1, sm.getWeights(null).get(ii), 0.000001);
                              }
                              if (event.getIteration() == 12
                                  && sm.getStrategies(null)
                                      .get(ii)
                                      .toString()
                                      .contains(ReRoute.class.getSimpleName())) {
