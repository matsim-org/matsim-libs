...
public class PopulationAttributeConversionTest {
  ...
  public void testDefaultsStream() {
    ...
    testWriteAndReread(
        w -> w.write(IOUtils.getOutputStream(IOUtils.getFileUrl(path), false)),
        w -> w.readFile(path));
  }
  ...
  public void testV7() {
    ...
    testWriteAndReread(
        w -> w.writeV6(IOUtils.getOutputStream(IOUtils.getFileUrl(path), false)),
        w -> w.readFile(path));
  }

  public void testWriteAndReread(
      final Consumer<PopulationWriter> writeMethod, final Consumer<PopulationReader> readMethod) {
