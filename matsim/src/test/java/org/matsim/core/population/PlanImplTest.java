...
public class PlanImplTest {
  ...
  private static final Logger log = LogManager.getLogger(PlanImplTest.class);
  ...
  public void testCopyPlan_NetworkRoute() {
    ...
    Node node3 =
        NetworkUtils.createAndAddNode(network, Id.create(3, Node.class), new Coord(2000, 0));
    ...
    assertEquals(
        "person must not be copied.", Id.create(2, Person.class), plan2.getPerson().getId());
    assertEquals(
        "wrong number of plan elements.",
        plan.getPlanElements().size(),
        plan2.getPlanElements().size());
