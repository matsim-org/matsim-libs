...
public class PopulationReprojectionIOIT {
  ...
  private static final CoordinateTransformation transformation =
      TransformationFactory.getCoordinateTransformation(INITIAL_CRS, TARGET_CRS);
  ...
  public void testInput_V4() {
    final String testFile =
        new File(utils.getOutputDirectory() + "/plans.xml.gz").getAbsolutePath();
    ...
    new MatsimNetworkReader(scenario.getNetwork())
        .parse(IOUtils.extendUrl(config.getContext(), NET_FILE));
  ...
  public void testOutput_V4() {
    ...
    // necessary for v4...
    new MatsimNetworkReader(originalScenario.getNetwork())
        .parse(IOUtils.extendUrl(config.getContext(), NET_FILE));
    ...
    new PopulationWriter(
            transformation, originalScenario.getPopulation(), originalScenario.getNetwork())
        .writeV4(testFile);

    // read converted population
    final Scenario reprojectedScenario = ScenarioUtils.createScenario(config);
    // necessary for v4...
    new MatsimNetworkReader(reprojectedScenario.getNetwork())
        .parse(IOUtils.extendUrl(config.getContext(), NET_FILE));
    new PopulationReader(reprojectedScenario).readFile(testFile);

    assertPopulationCorrectlyTransformed(
  ...
  public void testWithControlerAndAttributes() {
    ...
    // this is used both to compare equality and non-equality, so the more we accept difference
    // between input
    // and output coordinates, the more we require the internally reprojected coordinates to be
    // different.
    ...
    new MatsimNetworkReader(originalScenario.getNetwork())
        .parse(IOUtils.extendUrl(berlin, NET_FILE));
    ...
    new PopulationWriter(originalScenario.getPopulation())
        .write(utils.getOutputDirectory() + BASE_FILE);
    ...
    for (Id<Person> id : originalScenario.getPopulation().getPersons().keySet()) {
      ...
      final List<Activity> originalActivities =
          TripStructureUtils.getActivities(
              originalPerson.getSelectedPlan(), StageActivityHandling.StagesAsNormalActivities);
      final List<Activity> reprojectedActivities =
          TripStructureUtils.getActivities(
              internalPerson.getSelectedPlan(), StageActivityHandling.StagesAsNormalActivities);
      ...
      while (originalIterator.hasNext()) {
        ...
        Assert.assertNotEquals(
            "No coordinates transform performed!", transformation.transform(o.getCoord()), r);
      }
    }
    ...
    for (Id<Person> id : originalScenario.getPopulation().getPersons().keySet()) {
      ...
      final List<Activity> internalActivities =
          TripStructureUtils.getActivities(
              internalPerson.getSelectedPlan(), StageActivityHandling.StagesAsNormalActivities);
      final List<Activity> reprojectedActivities =
          TripStructureUtils.getActivities(
              dumpedPerson.getSelectedPlan(), StageActivityHandling.StagesAsNormalActivities);
  ...
  public void testWithControlerAndConfigParameters() {
    ...
    URL berlin = ExamplesUtils.getTestScenarioURL("berlin");
    final Scenario originalScenario =
        ScenarioUtils.createScenario(ConfigUtils.createConfig(berlin));
    // necessary for v4...
    new MatsimNetworkReader(originalScenario.getNetwork())
        .parse(IOUtils.extendUrl(berlin, NET_FILE));
    new PopulationReader(originalScenario).parse(IOUtils.extendUrl(berlin, BASE_FILE));
  ...
  private void assertPopulationCorrectlyTransformed(
      final Population originalPopulation, final Population reprojectedPopulation) {
    Assert.assertEquals(
    ...
    for (Id<Person> personId : originalPopulation.getPersons().keySet()) {
      ...
      assertPlanCorrectlyTransformed(
          originalPerson.getSelectedPlan(), reprojectedPerson.getSelectedPlan());
    }
  }

  private void assertPlanCorrectlyTransformed(final Plan originalPlan, final Plan reprojectedPlan) {
    final List<Activity> originalActivities =
        TripStructureUtils.getActivities(
            originalPlan, StageActivityHandling.StagesAsNormalActivities);
    final List<Activity> reprojectedActivities =
        TripStructureUtils.getActivities(
            reprojectedPlan, StageActivityHandling.StagesAsNormalActivities);
  ...
  private void assertIsCorrectlyTransformed(final Coord original, final Coord transformed) {
    ...
    Assert.assertEquals("wrong reprojected value", target, transformed);
  }
}
