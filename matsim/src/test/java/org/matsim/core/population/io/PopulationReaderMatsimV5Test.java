...
public class PopulationReaderMatsimV5Test {
  ...
  public void testReadRoute() throws SAXException, ParserConfigurationException, IOException {
    ...
    Assert.assertEquals(
        "different startLink for first leg.", "1", route1a.getStartLinkId().toString());
    Assert.assertEquals(
        "different endLink for first leg.", "20", route1a.getEndLinkId().toString());
    Leg leg1b = (Leg) plan1.getPlanElements().get(3);
    Route route1b = leg1b.getRoute();
    Assert.assertEquals(
        "different startLink for second leg.", "20", route1b.getStartLinkId().toString());
    Assert.assertEquals(
        "different endLink for second leg.", "20", route1b.getEndLinkId().toString());
    Leg leg1c = (Leg) plan1.getPlanElements().get(5);
    Route route1c = leg1c.getRoute();
    Assert.assertEquals(
        "different startLink for third leg.", "20", route1c.getStartLinkId().toString());
    ...
    Assert.assertEquals(
        "different startLink for first leg.", "2", route2a.getStartLinkId().toString());
    Assert.assertEquals(
        "different endLink for first leg.", "20", route2a.getEndLinkId().toString());
    Leg leg2b = (Leg) plan2.getPlanElements().get(3);
    Route route2b = leg2b.getRoute();
    Assert.assertEquals(
        "different startLink for third leg.", "20", route2b.getStartLinkId().toString());
    Assert.assertEquals("different endLink for third leg.", "1", route2b.getEndLinkId().toString());
  }

  @Test
  public void testReadRoute_sameLinkRoute()
    ...
    tester.startTag(
        "act",
        new String[][] {
          {"type", "w"}, {"x", "10000"}, {"y", "0"}, {"link", "1"}, {"dur", "00:10"}
        });
    tester.endTag();
    tester.endTag();
    tester.endTag();
  ...
  @Test
  public void testReadRoute_consequentLinks()
      throws SAXException, ParserConfigurationException, IOException {
    final MutableScenario scenario =
        (MutableScenario) ScenarioUtils.createScenario(ConfigUtils.createConfig());
    final Population population = scenario.getPopulation();
    ...
    tester.startTag(
        "act",
        new String[][] {
          {"type", "w"}, {"x", "10000"}, {"y", "0"}, {"link", "2"}, {"dur", "00:10"}
        });
    tester.endTag();
    tester.endTag();
    tester.endTag();
