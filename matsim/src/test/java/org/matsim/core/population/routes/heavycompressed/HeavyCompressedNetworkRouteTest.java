...
public class HeavyCompressedNetworkRouteTest extends AbstractNetworkRouteTest {
  ...
  @Override
  public NetworkRoute getNetworkRouteInstance(
      final Id<Link> fromLinkId, final Id<Link> toLinkId, final Network network) {
    HeavyCompressedNetworkRouteFactory factory =
        new HeavyCompressedNetworkRouteFactory(network, TransportMode.car);
    return factory.createRoute(fromLinkId, toLinkId);
  }

  /**
   * Tests that setting some links results in the same links returned. This is usually not tested as
   * it is assumed, that just storing some links and returning the same ones should not be
   * error-prone, but it's different in this case where we do not actually store the links.
   */
  @Test
  public void testGetLinks_setLinks() {
    ...
    List<Id<Link>> linkIds =
        List.of(link22.getId(), link12.getId(), link13.getId(), linkM24.getId());
  ...
  public void testGetLinks_onlySubsequentLinks() {
    ...
    NetworkRoute route =
        new HeavyCompressedNetworkRoute(
            link0.getId(),
            link4.getId(),
            HeavyCompressedNetworkRouteFactory.createCompressionData(network, subsequentLinks));
  ...
  /**
   * Tests that {@link HeavyCompressedNetworkRoute#getLinkIds()} doesn't crash or hang when a route
  ...
  public void testClone() {
    ...
    HeavyCompressedNetworkRoute route1 =
        new HeavyCompressedNetworkRoute(
            startLink.getId(),
            endLink.getId(),
            HeavyCompressedNetworkRouteFactory.createCompressionData(network, subsequentLinks));
  ...
  public void testGetLinks_setLinks_alternative() {
    ...
    List<Id<Link>> linkIds =
        List.of(link22.getId(), link12.getId(), linkM23.getId(), link3.getId());
    NetworkRoute route = getNetworkRouteInstance(link1.getId(), link4.getId(), network);
    route.setLinkIds(link1.getId(), linkIds, link4.getId());
  ...
  public void testGetLinks_setLinks_endLoopLink() {
    ...
    NetworkUtils.createAndAddLink(
        network, Id.create("loop5", Link.class), node5, node5, 1000.0, 100.0, 3600.0, 1);
    ...
    List<Id<Link>> linkIds =
        List.of(link22.getId(), link12.getId(), linkM23.getId(), link3.getId(), link4.getId());
  ...
  public void testGetLinks_setLinks_containsLargeLoop() {
    ...
    NetworkUtils.createAndAddLink(
        network, Id.create("-12", Link.class), node13, node12, 1000.0, 100.0, 3600.0, 1);
  ...
  public void testGetLinks_setLinks_containsLargeLoop_alternative() {
    ...
    NetworkUtils.createAndAddLink(
        network, Id.create("-12", Link.class), node13, node12, 1000.0, 100.0, 3600.0, 1);
    NetworkUtils.createAndAddLink(
        network, Id.create("-13", Link.class), node14, node13, 1000.0, 100.0, 3600.0, 1);
  ...
  public void testGetLinks_setLinks_isLargeLoop() {
    ...
    List<Id<Link>> linkIds =
        List.of(link3.getId(), link24.getId(), linkM13.getId(), linkM12.getId(), linkM22.getId());
