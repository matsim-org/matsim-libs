...
public class PopulationReaderMatsimV4Test {
  ...
  public void testReadRoute() throws SAXException, ParserConfigurationException, IOException {
    ...
    tester.startTag(
        "act",
        new String[][] {
          {"type", "h"}, {"x", "-25000"}, {"y", "0"}, {"link", "1"}, {"end_time", "06:00"}
        });
    tester.endTag();
    ...
    tester.startTag(
        "act",
        new String[][] {
          {"type", "w"}, {"x", "10000"}, {"y", "0"}, {"link", "20"}, {"dur", "00:10"}
        });
    tester.endTag();
    tester.startTag("leg", new String[][] {{"mode", "car"}});
    tester.startTag("route");
    ...
    tester.startTag(
        "act",
        new String[][] {
          {"type", "w"}, {"x", "10000"}, {"y", "0"}, {"link", "20"}, {"dur", "03:30"}
        });
    tester.endTag();
    tester.startTag("leg", new String[][] {{"mode", "car"}});
    tester.startTag("route");
    ...
    tester.endTag();
    tester.startTag(
        "act", new String[][] {{"type", "4"}, {"x", "-25000"}, {"y", "0"}, {"link", "1"}});
    tester.endTag();
    tester.endTag();
    tester.endTag();
    ...
    tester.startTag(
        "act",
        new String[][] {
          {"type", "h"}, {"x", "-25000"}, {"y", "0"}, {"link", "2"}, {"end_time", "06:00"}
        });
    tester.endTag();
    tester.startTag("leg", new String[][] {{"mode", "car"}});
    tester.startTag("route");
    ...
    Assert.assertEquals(
        "different startLink for first leg.",
        network.getLinks().get(Id.create("1", Link.class)).getId(),
        route1a.getStartLinkId());
    Assert.assertEquals(
        "different endLink for first leg.",
        network.getLinks().get(Id.create("20", Link.class)).getId(),
        route1a.getEndLinkId());
    Leg leg1b = (Leg) plan1.getPlanElements().get(3);
    Route route1b = leg1b.getRoute();
    Assert.assertEquals(
        "different startLink for second leg.",
        network.getLinks().get(Id.create("20", Link.class)).getId(),
        route1b.getStartLinkId());
    Assert.assertEquals(
        "different endLink for second leg.",
        network.getLinks().get(Id.create("20", Link.class)).getId(),
        route1b.getEndLinkId());
    Leg leg1c = (Leg) plan1.getPlanElements().get(5);
    Route route1c = leg1c.getRoute();
    Assert.assertEquals(
        "different startLink for third leg.",
        network.getLinks().get(Id.create("20", Link.class)).getId(),
        route1c.getStartLinkId());
    Assert.assertEquals(
        "different endLink for third leg.",
        network.getLinks().get(Id.create("1", Link.class)).getId(),
        route1c.getEndLinkId());
    ...
    Assert.assertEquals(
        "different startLink for first leg.",
        network.getLinks().get(Id.create("2", Link.class)).getId(),
        route2a.getStartLinkId());
    Assert.assertEquals(
        "different endLink for first leg.",
        network.getLinks().get(Id.create("20", Link.class)).getId(),
        route2a.getEndLinkId());
    Leg leg2b = (Leg) plan2.getPlanElements().get(3);
    Route route2b = leg2b.getRoute();
    Assert.assertEquals(
        "different startLink for third leg.",
        network.getLinks().get(Id.create("20", Link.class)).getId(),
        route2b.getStartLinkId());
    Assert.assertEquals(
        "different endLink for third leg.",
        network.getLinks().get(Id.create("1", Link.class)).getId(),
        route2b.getEndLinkId());
  }

  /**
   * Tests that reading in plans that contain route-elements, but the activities before or after do
   * not contain any link-information, works. This can be the case if the given routes are part of a
   * teleported transport mode, where links may not be required.
   *
   * @author mrieser
   */
  @Test
  ...
  public void testReadActivity() {
    ...
    Node node2 =
        NetworkUtils.createAndAddNode(network, Id.create("2", Node.class), new Coord(0, 1000));
    ...
    Stack<String> context =
        new Stack<String>(); // not sure the context is ever used in the reader...
    ...
    reader.startTag(
        "act", new AttributesBuilder().add("type", "h").add("link", "3").get(), context);
    reader.endTag("act", "", context);
    reader.startTag("leg", new AttributesBuilder().add("mode", "car").get(), context);
    reader.endTag("leg", "", context);
    reader.startTag(
        "act", new AttributesBuilder().add("type", "h").add("link", "2").get(), context);
    ...
    Assert.assertEquals(
        Id.create("2", Link.class), ((Activity) plan.getPlanElements().get(2)).getLinkId());
  }
