...
public class PopulationWriterHandlerImplV5Test {
  ...
  private NetworkRoute doTestWriteNetworkRoute(
      final String startLinkId,
      final String linkIds,
      final String endLinkId,
      final String expectedRouteSerialization) {
    MutableScenario scenario =
        (MutableScenario) ScenarioUtils.createScenario(this.util.loadConfig((String) null));
    Id<Link> idFrom = Id.create(startLinkId, Link.class);
    Id<Link> idTo = Id.create(endLinkId, Link.class);

    MutableScenario tmpScenario =
    ...
    RouteInterceptingPopulationReader popReader =
        new RouteInterceptingPopulationReader(new PopulationReaderMatsimV5(scenario));
    ...
    Assert.assertTrue(
        "read route is of class " + route2.getClass().getCanonicalName(),
        route2 instanceof NetworkRoute);
    ...
    Assert.assertEquals(
        "wrong number of links in route", routeLinkIds.size(), nr.getLinkIds().size());
    for (int i = 0; i < routeLinkIds.size(); i++) {
      Assert.assertEquals(
          "wrong link in route at position " + i, routeLinkIds.get(i), nr.getLinkIds().get(i));
    }
    return nr;
  }
  ...
  public void testWriteGenericRouteRoute() {
    ...
    Leg leg2 = (Leg) person2.getPlans().get(0).getPlanElements().get(1);
    Route route2 = leg2.getRoute();
    //		Assert.assertTrue("read route is of class " + route2.getClass().getCanonicalName(), route2
    // instanceof GenericRouteImpl);
  ...
  private static final class RouteInterceptingPopulationReader extends MatsimXmlParser
