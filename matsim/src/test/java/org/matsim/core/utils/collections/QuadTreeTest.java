...
public class QuadTreeTest {
  ...
  private static final Logger log = LogManager.getLogger(QuadTreeTest.class);
  ...
  /**
   * Test getting values from a QuadTree using {@link QuadTree#getClosest(double, double)} and
   * {@link QuadTree#getDisk(double, double, double)}.
   */
  @Test
  public void testGet() {
        ...
        for (double x2 : xPositions) {
          for (double y2 : yPositions) {
            ...
            for (double distance : distances) {
              ...
              for (Coord coord : all) {
                if (CoordUtils.calcEuclideanDistance(coord, f1)
                        + CoordUtils.calcEuclideanDistance(coord, f2)
                    <= distance) {
                  expected.add(coord);
                }
              }

              final Collection<Coord> actual = qt.getElliptical(x1, y1, x2, y2, distance);

              // log.info( "testing foci "+f1+" and "+f2+", distance="+distance+",
              // expected="+expected );
              Assert.assertEquals(
                  "unexpected number of elements returned for foci "
                      + f1
                      + " and "
                      + f2
                      + ", distance="
              ...
              Assert.assertTrue(
                  "unexpected elements returned for foci "
                      + f1
                      + " and "
                      + f2
                      + ", distance="
  ...
  public void testRemove() {
    ...
    assertEquals(
        "15.0, 15.0 B", qt.getClosest(15.0, 15.0)); // the other object should still be there...
    ...
    assertEquals(size - 1, qt.size());
    assertEquals(
        "15.0, 15.0", qt.getClosest(15.0, 15.0)); // the other object should still be there...
  ...
  /**
   * Tests that a once obtained values-collection is indeed a live view on the QuadTree, so when the
   * QuadTree changes, the view is updated as well.
   */
  @Test
  ...
  /**
   * Tests that by serializing and de-serializing a QuadTree, all important attributes of the
   * QuadTree are maintained. At the moment, this is essentially the size-attribute.
   *
   * @throws IOException
  ...
  /**
   * A kind of performance test, but not marked as test, as there is no need to run it in every
   * check as there is not assert statement.
   *
   * @author mrieser
   */
  private static void measurePerformance() {
    ...
    for (int i = 0; i < elementCount; i++) {
      Coord coord =
          new Coord(minX + r.nextDouble() * (maxX - minX), minY + r.nextDouble() * (maxY - minY));
