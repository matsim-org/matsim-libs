...
public class ArgumentParserTest {
  ...
  public void testArguments() {
    ArgumentParser parser = new ArgumentParser(new String[] {"a", "bc", "def=g", "hjk=lmn opq"});
  ...
  /**
   * Test if short options (beginning with '-'), which can be written in a collapsed form, are
  ...
  public void testShortOptions() {
    ...
    assertEquals(
        "-=3", iter.next()); // interpret as string argument, as it makes no sense otherwise
    assertEquals(
        "-=4=5",
        iter.next()); /* one could iterprete '=4' as param name and '5' as value to that param,
  ...
  public void testLongOptions() {
    ArgumentParser parser =
        new ArgumentParser(
            new String[] {
              "--a", "--bc", "--def=ghj", "--kl=mn op", "=", "--qr=", "--=", "--=st", "--=uv=wxy z"
            },
            true);
    Iterator<String> iter = parser.iterator();
    assertEquals("--a", iter.next());
    ...
    assertEquals(
        "--=uv=wxy z", iter.next()); // see comment in testShortArguments() for similar case
    assertIteratorAtEnd(iter);
  }
  ...
  public void testMixed() {
    ...
    parser =
        new ArgumentParser(
            new String[] {"-xcf", "=", "=a", "=ab", "-=", "-=a", "--=", "--=a", "--a=b"}, false);
    iter = parser.iterator();
    assertEquals("-xcf", iter.next());
    assertEquals("=", iter.next());
    assertEquals("=a", iter.next());
