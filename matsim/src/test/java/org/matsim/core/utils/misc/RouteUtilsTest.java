...
public class RouteUtilsTest {
  ...
  public void testCalculateCoverage() {
    ...
    NetworkRoute route, route2, route3;
    {
      ...
      route = RouteUtils.createLinkNetworkRouteImpl(startLink.getId(), endLink.getId());
    ...
    }
    {
      ...
      List<Id<Link>> linkIds = new ArrayList<Id<Link>>(4);
      Collections.addAll(linkIds, f.linkIds[1], f.linkIds[2], f.linkIds[3], f.linkIds[4]);
      route2 = RouteUtils.createLinkNetworkRouteImpl(startLink.getId(), endLink.getId());
    ...
    Assert.assertEquals(
        (200. + 400. + 500.) / (200. + 300. + 400. + 500.),
        RouteUtils.calculateCoverage(route, route3, f.network),
        0.0001);
  ...
  public void testGetNodes() {
    ...
    List<Id<Link>> linkIds = new ArrayList<Id<Link>>(4);
    Collections.addAll(linkIds, f.linkIds[1], f.linkIds[2], f.linkIds[3], f.linkIds[4]);
    NetworkRoute route = RouteUtils.createLinkNetworkRouteImpl(startLink.getId(), endLink.getId());
    route.setLinkIds(startLink.getId(), linkIds, endLink.getId());
  ...
  public void testGetNodes_CircularRoute() {
    ...
    f.network.addLink(
        f.network
            ...
            .createLink(
                id99,
                f.network.getNodes().get(f.nodeIds[6]),
                f.network.getNodes().get(f.nodeIds[0])));
    ...
    Collections.addAll(
        linkIds, f.linkIds[4], f.linkIds[5], id99, f.linkIds[0], f.linkIds[1], f.linkIds[2]);
  ...
  public void testGetSubRoute() {
    ...
    NetworkRoute subRoute =
        RouteUtils.getSubRoute(
            route,
            f.network.getNodes().get(f.nodeIds[3]),
            f.network.getNodes().get(f.nodeIds[5]),
            f.network);
  ...
  public void testGetSubRoute_fullRoute() {
    ...
    NetworkRoute subRoute =
        RouteUtils.getSubRoute(
            route,
            f.network.getNodes().get(f.nodeIds[1]),
            f.network.getNodes().get(f.nodeIds[5]),
            f.network);
  ...
  public void testGetSubRoute_emptySubRoute() {
    ...
    NetworkRoute subRoute =
        RouteUtils.getSubRoute(
            route,
            f.network.getNodes().get(f.nodeIds[4]),
            f.network.getNodes().get(f.nodeIds[4]),
            f.network);
  ...
  public void testCalcDistance() {
    ...
    Assert.assertEquals(
        900.0,
        RouteUtils.calcDistanceExcludingStartEndLink(route, f.network),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        1400.0, RouteUtils.calcDistance(route, 1.0, 1.0, f.network), MatsimTestUtils.EPSILON);
    // modify the route
    ...
    Assert.assertEquals(
        ...
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        2000.0, RouteUtils.calcDistance(route, 1.0, 1.0, f.network), MatsimTestUtils.EPSILON);
  }
  ...
  public void testCalcDistance_sameStartEndRoute() {
    ...
    Assert.assertEquals(
        ...
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        0.0, RouteUtils.calcDistance(route, 1.0, 1.0, f.network), MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        400.0, RouteUtils.calcDistance(route, 0.0, 1.0, f.network), MatsimTestUtils.EPSILON);
  }
  ...
  public void testCalcDistance_subsequentStartEndRoute() {
    ...
    Assert.assertEquals(
        ...
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        400.0, RouteUtils.calcDistance(route, 1.0, 1.0, f.network), MatsimTestUtils.EPSILON);
  }
  ...
  public void testCalcDistance_oneLinkRoute() {
    ...
    f.network.getLinks().get(f.linkIds[4]).setLength(500.0);
    f.network.getLinks().get(f.linkIds[5]).setLength(600.0);
    NetworkRoute route = RouteUtils.createLinkNetworkRouteImpl(f.linkIds[2], f.linkIds[4]);
    List<Id<Link>> linkIds = new ArrayList<Id<Link>>();
    ...
    Assert.assertEquals(
        ...
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        900.0, RouteUtils.calcDistance(route, 1.0, 1.0, f.network), MatsimTestUtils.EPSILON);
  }
  ...
  private static class Fixture {
    ...
    protected Fixture() {
      ...
      this.network.addLink(
          nf.createLink(
              this.linkIds[0],
              this.network.getNodes().get(this.nodeIds[0]),
              this.network.getNodes().get(this.nodeIds[1])));
      this.network.addLink(
          nf.createLink(
              this.linkIds[1],
              this.network.getNodes().get(this.nodeIds[1]),
              this.network.getNodes().get(this.nodeIds[2])));
      this.network.addLink(
          nf.createLink(
              this.linkIds[2],
              this.network.getNodes().get(this.nodeIds[2]),
              this.network.getNodes().get(this.nodeIds[3])));
      this.network.addLink(
          nf.createLink(
              this.linkIds[3],
              this.network.getNodes().get(this.nodeIds[3]),
              this.network.getNodes().get(this.nodeIds[4])));
      this.network.addLink(
          nf.createLink(
              this.linkIds[4],
              this.network.getNodes().get(this.nodeIds[4]),
              this.network.getNodes().get(this.nodeIds[5])));
      this.network.addLink(
          nf.createLink(
              this.linkIds[5],
              this.network.getNodes().get(this.nodeIds[5]),
              this.network.getNodes().get(this.nodeIds[6])));
    }
  }
}
