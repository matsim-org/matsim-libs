...
public class NetworkImplTest extends AbstractNetworkTest {
  ...
  /**
   * Tests if the default values of a network instance are the same as the defaults specified in the
   * network_v1.dtd
   */
  @Test
  public void testDefaultValues() {
    ...
    Node node1 =
        NetworkUtils.createNode(Id.create(1, Node.class), new Coord((double) 0, (double) 0));
    Node node2 =
        NetworkUtils.createNode(Id.create(2, Node.class), new Coord((double) 1000, (double) 0));
    net.addNode(node1);
    net.addNode(node2);
  ...
  /**
   * Tests that if a link is added with an id that already exists as link in the network, an
   * exception is thrown. No exception should be thrown when the same link is added a second time.
   */
  @Test
  public void testAddLink_existingId() {
    ...
    Node node2 =
        NetworkUtils.createNode(Id.create(2, Node.class), new Coord((double) 1000, (double) 0));
    Node node3 =
        NetworkUtils.createNode(Id.create(3, Node.class), new Coord((double) 2000, (double) 500));
    final double y = -500;
    ...
    Link link1 =
        NetworkUtils.createLink(
            Id.create(1, Link.class), node1, node2, network, 1000, 100.0, 2000.0, 1.0);
    Link link1b =
        NetworkUtils.createLink(
            Id.create(1, Link.class), node2, node3, network, 1000, 100.0, 2000.0, 1.0);
    Link link2 =
        NetworkUtils.createLink(
            Id.create(2, Link.class), node2, node4, network, 1000, 100.0, 2000.0, 1.0);
  ...
  /**
   * Tests that if a link is added when its associated nodes are not in the network, an exception is
   * thrown. If the node is already in the network, no exception should be thrown.
   */
  @Test
  ...
  public void testAddNode_existingId() {
    ...
    Node node3 =
        NetworkUtils.createNode(Id.create(3, Node.class), new Coord((double) 2000, (double) 500));
    Node node1b =
        NetworkUtils.createNode(Id.create(1, Node.class), new Coord((double) 2000, (double) 0));
    network.addNode(node1);
    network.addNode(node2);
  ...
  public void testAddNode_singleNodeFirstOnly() {
    Network network = new NetworkImpl(new LinkFactoryImpl());
    Node node1 =
        NetworkUtils.createNode(Id.create(1, Node.class), new Coord((double) 500, (double) 400));
    Node node2 =
        NetworkUtils.createNode(Id.create(2, Node.class), new Coord((double) 600, (double) 500));
  ...
  public void testRemoveLink_alsoInQuadTrees() {
    ...
    Link link1 =
        NetworkUtils.createLink(
            Id.create(1, Link.class), node1, node2, network, 800, 13.4, 2000, 1);
    Link link2 =
        NetworkUtils.createLink(
            Id.create(2, Link.class), node2, node3, network, 800, 13.4, 2000, 1);
    Link link3 =
        NetworkUtils.createLink(
            Id.create(3, Link.class), node3, node1, network, 800, 13.4, 2000, 1);
    network.addLink(link1);
    network.addLink(link2);
  ...
  public void testAddLink_alsoInQuadTrees() {
    ...
    Assert.assertEquals(
        ...
        NetworkUtils.getNearestLinkExactly(
            network, new Coord(300, 200))); // this will force the LinkQuadTree to be built
  ...
  public void testAddLink_intoEmptyQuadTree() {
    ...
    Assert.assertNull(
        NetworkUtils.getNearestLink(
            network, new Coord(300, 200))); // this will force the node QuadTree to be built
    Assert.assertNull(
        NetworkUtils.getNearestLinkExactly(
            network, new Coord(300, 200))); // this will force the LinkQuadTree to be built
