...
public class TimeVariantLinkImplTest {
  ...
  private static final double TIME_BEFORE_FIRST_CHANGE_EVENTS =
      -99999; // when  base (default) link properties are used
  ...
  public void testGetFreespeedTravelTime() {
    for (LinkFactory lf : linkFactories(1, 5)) {
      ...
      event.setFreespeedChange(
          new ChangeValue(NetworkChangeEvent.ChangeType.ABSOLUTE_IN_SI_UNITS, 10.));
  ...
  public void testFreespeedChangeAbsolute() {
    for (LinkFactory lf : linkFactories(15 * 60, 30 * 3600)) {
      ...
      // test base values
      assertEquals(
          10.0, link.getFreespeed(TIME_BEFORE_FIRST_CHANGE_EVENTS), MatsimTestUtils.EPSILON);
      assertEquals(
          10.0,
          NetworkUtils.getFreespeedTravelTime(link, TIME_BEFORE_FIRST_CHANGE_EVENTS),
      ...
      assertEquals(
          10.0,
          link.getFreespeed(TIME_BEFORE_FIRST_CHANGE_EVENTS),
          MatsimTestUtils.EPSILON); // before first change event, return base value
      assertEquals(
          10.0,
          link.getFreespeed(7 * 3600.0 - 1.0),
          MatsimTestUtils.EPSILON); // one second before change, still base value
      assertEquals(
          10.0,
          link.getFreespeed(7 * 3600.0 - 0.1),
          MatsimTestUtils.EPSILON); // just before change, still base value
      assertEquals(
          20.0,
          link.getFreespeed(7 * 3600.0),
          MatsimTestUtils.EPSILON); // just on time of change, new value
      assertEquals(
          20.0,
          link.getFreespeed(8 * 3600.0),
          MatsimTestUtils.EPSILON); // some time later, still new value

      // test derived values
      assertEquals(
          10.0,
          NetworkUtils.getFreespeedTravelTime(link, TIME_BEFORE_FIRST_CHANGE_EVENTS),
          MatsimTestUtils.EPSILON); // and now the same tests for the travel time
      assertEquals(
          10.0,
          NetworkUtils.getFreespeedTravelTime(link, 7 * 3600.0 - 1.0),
          MatsimTestUtils.EPSILON);
      assertEquals(
          10.0,
          NetworkUtils.getFreespeedTravelTime(link, 7 * 3600.0 - 0.1),
          MatsimTestUtils.EPSILON);
      assertEquals(
          5.0, NetworkUtils.getFreespeedTravelTime(link, 7 * 3600.0), MatsimTestUtils.EPSILON);
      assertEquals(
          5.0, NetworkUtils.getFreespeedTravelTime(link, 8 * 3600.0), MatsimTestUtils.EPSILON);
      assertEquals(
          5.0,
          NetworkUtils.getFreespeedTravelTime(link, 24 * 3600.0),
          MatsimTestUtils.EPSILON); // also test if it "wraps around" on 24 hours, it shouldn't
      assertEquals(
          5.0, NetworkUtils.getFreespeedTravelTime(link, 30 * 3600.0), MatsimTestUtils.EPSILON);
      assertEquals(
          5.0, NetworkUtils.getFreespeedTravelTime(link, 36 * 3600.0), MatsimTestUtils.EPSILON);
    }
  }
  ...
  public void testFreespeedChangeRelative() {
    for (LinkFactory lf : linkFactories(15 * 60, 30 * 3600)) {
      ...
      assertEquals(
          ...
          MatsimTestUtils.EPSILON);
      assertEquals(
          20.0, NetworkUtils.getFreespeedTravelTime(link, 7 * 3600.0), MatsimTestUtils.EPSILON);
      assertEquals(
          20.0, NetworkUtils.getFreespeedTravelTime(link, 8 * 3600.0), MatsimTestUtils.EPSILON);
      assertEquals(
          20.0,
          NetworkUtils.getFreespeedTravelTime(link, 24 * 3600.0),
          MatsimTestUtils.EPSILON); // also test if it "wraps around" on 24 hours, it shouldn't
      assertEquals(
          20.0, NetworkUtils.getFreespeedTravelTime(link, 30 * 3600.0), MatsimTestUtils.EPSILON);
      assertEquals(
          20.0, NetworkUtils.getFreespeedTravelTime(link, 36 * 3600.0), MatsimTestUtils.EPSILON);
    }
  }
  ...
  public void testFlowCapChangeAbsolute() {
    for (LinkFactory lf : linkFactories(15 * 60, 30 * 3600)) {
      ...
      // test base values
      assertEquals(
          3600.0, link.getCapacity(TIME_BEFORE_FIRST_CHANGE_EVENTS), MatsimTestUtils.EPSILON);
      assertEquals(
          1.0,
          link.getFlowCapacityPerSec(TIME_BEFORE_FIRST_CHANGE_EVENTS),
          MatsimTestUtils.EPSILON);
      ...
      // test derived values
      // TODO test flowcap by sending vehicles through the link, this requires to create a
      // queuenetwork from this time-variant network
    }
  }
  ...
  public void testLanesChangeAbsolute() {
    for (LinkFactory lf : linkFactories(15 * 60, 30 * 3600)) {
      ...
      // test base values
      assertEquals(
          1.0, link.getNumberOfLanes(TIME_BEFORE_FIRST_CHANGE_EVENTS), MatsimTestUtils.EPSILON);
  ...
  // I moved those to core.mobsim.queuesim so that I can further reduce visibility of some queuesimu
  // internals. kai, may'10
