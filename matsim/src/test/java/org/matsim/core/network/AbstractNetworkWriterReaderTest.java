...
/**
 * An abstract class that tests if certain features of networks (e.g. specific settings/attributes
 * for links or nodes) are written to file and afterwards correctly read in. The class is abstract
 * so the tests can be reused with different file formats.
 *
 * @author mrieser
 */
public abstract class AbstractNetworkWriterReaderTest {
  ...
  public void testNodes_withElevation() {
    List<Node> nodes = new ArrayList<>(2);
    Node n1 =
        NetworkUtils.createNode(
            Id.create("1", Node.class), new Coord((double) 0, (double) 0, (double) 0));
    Node n2 =
        NetworkUtils.createNode(
            Id.create("2", Node.class), new Coord((double) 1000, (double) 0, (double) 0));
    nodes.add(n1);
    nodes.add(n2);
    doTestNodes(nodes, utils.getOutputDirectory() + "network.xml");
  }
  ...
  public void testLinks_IdSpecialCharacters() {
    ...
    //		Assert.assertEquals(NetworkUtils.getType(linkA1), NetworkUtils.getType(linkA2)); // type is
    // not supported anymore in v2
    //		Assert.assertEquals(NetworkUtils.getOrigId(linkB1), NetworkUtils.getOrigId(linkB2)); //
    // origId is not supported anymore in v2
  }
  ...
  private void doTestAllowedModes(final Set<String> modes, final String filename) {
    ...
    Node n1 =
        NetworkUtils.createAndAddNode(
            network1, Id.create("1", Node.class), new Coord((double) 0, (double) 0));
    Node n2 =
        NetworkUtils.createAndAddNode(
            network1, Id.create("2", Node.class), new Coord((double) 1000, (double) 0));
    final Node fromNode = n1;
    final Node toNode = n2;
    Link l1 =
        NetworkUtils.createAndAddLink(
            network1, Id.create("1", Link.class), fromNode, toNode, 1000.0, 10.0, 3600.0, 1.0);
  ...
  private void doTestNodes(List<Node> nodes, final String filename) {
    ...
    for (Node n : nodes) {
      assertEquals(
          ...
          n.getCoord(),
          network2.getNodes().get(n.getId()).getCoord());
    }
  }
