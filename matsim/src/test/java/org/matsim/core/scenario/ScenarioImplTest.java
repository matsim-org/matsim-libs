...
public class ScenarioImplTest {
  ...
  public void testAddAndGetScenarioElement() {
    ...
    Assert.assertSame("unexpected scenario element", element1, s.getScenarioElement(name1));
    // just check that it is got, not removed
    Assert.assertSame("unexpected scenario element", element1, s.getScenarioElement(name1));

    Assert.assertSame("unexpected scenario element", element2, s.getScenarioElement(name2));
  }
  ...
  public void testCannotAddAnElementToAnExistingName() {
    ...
    try {
      ...
      Assert.fail(
          "wrong exception thrown when trying to add an element for an existing name "
              + e.getClass().getName());
    }
  ...
  public void testRemoveElement() {
    ...
    Assert.assertSame("unexpected removed element", element, s.removeScenarioElement(name));
    Assert.assertNull("element was not removed", s.getScenarioElement(name));
  }
}
