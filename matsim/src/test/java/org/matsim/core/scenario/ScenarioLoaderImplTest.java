...
public class ScenarioLoaderImplTest {
	...
	public void testLoadScenario_loadTransitData() {
		...
		{
			ScenarioBuilder builder = new ScenarioBuilder(ConfigUtils.loadConfig(IOUtils.newUrl(this.util.getClassInputResourcePath(), "transitConfig.xml")));
		...
		// load directly:
		{
			Scenario scenario = ScenarioUtils.loadScenario(ConfigUtils.loadConfig(IOUtils.newUrl(this.util.getClassInputResourcePath(), "transitConfig.xml")));
			Assert.assertEquals(1, scenario.getTransitSchedule().getTransitLines().size());
			Assert.assertEquals(2, scenario.getTransitSchedule().getFacilities().size());
		}
	}
	...
	public void testLoadScenario_loadPersonAttributes() {
		Config config = ConfigUtils.loadConfig(IOUtils.newUrl(this.util.getClassInputResourcePath(), "personAttributesConfig.xml"));
	...
	public void testLoadScenario_loadTransitLinesAttributes() {
		Config config = ConfigUtils.loadConfig(IOUtils.newUrl(this.util.getClassInputResourcePath(), "transitConfig.xml"));
	...
	public void testLoadScenario_loadFacilitiesAttributes() {
		Config config = ConfigUtils.loadConfig(IOUtils.newUrl(this.util.getClassInputResourcePath(), "facilityAttributesConfig.xml"));
	...
	public void testLoadScenario_loadHouseholdAttributes() {
		Config config = ConfigUtils.loadConfig(IOUtils.newUrl(this.util.getClassInputResourcePath(), "householdAttributesConfig.xml"));
