...
public class TravelTimeCalculatorTest {
  ...
  private static final Logger log = LogManager.getLogger(TravelTimeCalculatorTest.class);
  ...
  @Test
  public final void testTravelTimeCalculator_Array_Optimistic_LinearInterpolation()
      throws IOException {
    ...
    TravelTimeGetter travelTimeGetter =
        new LinearInterpolatingTravelTimeGetter(numSlots, binSize, aggregator);
    doTravelTimeCalculatorTest(
        scenario,
        aggregator,
        binSize,
  ...
  private static void doTravelTimeCalculatorTest(
      final MutableScenario scenario,
      final TimeSlotComputation aggregator,
      final int timeBinSize,
      final int endTime,
      final String compareFile,
      final boolean generateNewData,
    ...
    TravelTimeCalculator ttcalc =
        new TravelTimeCalculator(
            network, timeBinSize, endTime, scenario.getConfig().travelTimeCalculator());
    ...
    if (generateNewData) {
      ...
      try {
        ...
        for (int i = 0; i < numberOfTimeSlotsToTest; i++) {
          double ttime =
              ttcalc.getLinkTravelTimes().getLinkTravelTime(link10, i * timeBinSize, null, null);
    ...
    for (int i = 0; i < numberOfTimeSlotsToTest; i++) {
      double ttime =
          ttcalc.getLinkTravelTimes().getLinkTravelTime(link10, i * timeBinSize, null, null);
      assertEquals(
          Double.parseDouble(compareData[i]),
          ttime,
          1e-3); // traveltimecalculator has a resolution of 0.001 seconds
    }
  }
  ...
  public void testLongTravelTimeInEmptySlot() {
    ...
    final Node fromNode =
        NetworkUtils.createAndAddNode(network, Id.create("1", Node.class), new Coord(0, 0));
    final Node toNode =
        NetworkUtils.createAndAddNode(network, Id.create("2", Node.class), new Coord(1000, 0));
    Link link1 =
        NetworkUtils.createAndAddLink(
            network, Id.create("1", Link.class), fromNode, toNode, 1000.0, 100.0, 3600.0, 1.0);
    ...
    TravelTimeCalculator ttcalc =
        new TravelTimeCalculator(
            network, timeBinSize, 12 * 3600, scenario.getConfig().travelTimeCalculator());
    ...
    ttcalc.handleEvent(
        new LinkLeaveEvent(firstTimeBinStart + linkTravelTime1, vehId, link1.getId()));
    ttcalc.handleEvent(
        new LinkEnterEvent(firstTimeBinStart + 3 * timeBinSize, vehId, link1.getId()));
    ttcalc.handleEvent(
        new LinkLeaveEvent(
            firstTimeBinStart + 3 * timeBinSize + linkTravelTime2, vehId, link1.getId()));
    ...
    assertEquals(
        ...
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + offset, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        linkTravelTime1 - timeBinSize,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 1 * timeBinSize + offset, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        linkTravelTime1 - 2 * timeBinSize,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 2 * timeBinSize + offset, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        linkTravelTime2,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 3 * timeBinSize + offset, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        freeSpeedTT,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 4 * timeBinSize + offset, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        freeSpeedTT,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 5 * timeBinSize + offset, null, null),
        MatsimTestUtils.EPSILON);
  }

  /**
   * This method tests the functionality of the consolidateData-method in TravelTimeCalculator in
   * combination with double time bins
   *
   * @author tthunig
   */
  @Test
  public void testLongTravelTimeInEmptySlotWithDoubleTimeBins() {
    Scenario scenario = ScenarioUtils.createScenario(ConfigUtils.createConfig());
    Network network = scenario.getNetwork();
    network.setCapacityPeriod(3600.0);
    final Node fromNode =
    ...
    //		System.out.println("firstTimeBinStart - offset (" + (firstTimeBinStart - offset) + "): " +
    // ttcalc.getLinkTravelTimes().getLinkTravelTime(link1, firstTimeBinStart - offset, null,
    // null));
    //		System.out.println("firstTimeBinStart + offset (" + (firstTimeBinStart + offset) + "): " +
    // ttcalc.getLinkTravelTimes().getLinkTravelTime(link1, firstTimeBinStart + offset, null,
    // null));
    //		System.out.println("firstTimeBinStart + 1xtimeBinSize + offset (" + (firstTimeBinStart +
    // 1*timeBinSize + offset) + "): " + ttcalc.getLinkTravelTimes().getLinkTravelTime(link1,
    // firstTimeBinStart + 1*timeBinSize + offset, null, null));
    //		System.out.println("firstTimeBinStart + 2xtimeBinSize + offset (" + (firstTimeBinStart +
    // 2*timeBinSize + offset) + "): " + ttcalc.getLinkTravelTimes().getLinkTravelTime(link1,
    // firstTimeBinStart + 2*timeBinSize + offset, null, null));
    //		System.out.println("firstTimeBinStart + 3xtimeBinSize + offset (" + (firstTimeBinStart +
    // 3*timeBinSize + offset) + "): " + ttcalc.getLinkTravelTimes().getLinkTravelTime(link1,
    // firstTimeBinStart + 3*timeBinSize + offset, null, null));
    //		System.out.println("firstTimeBinStart + 4xtimeBinSize + offset (" + (firstTimeBinStart +
    // 4*timeBinSize + offset) + "): " + ttcalc.getLinkTravelTimes().getLinkTravelTime(link1,
    // firstTimeBinStart + 4*timeBinSize + offset, null, null));
    //		System.out.println("firstTimeBinStart + 5xtimeBinSize + offset (" + (firstTimeBinStart +
    // 5*timeBinSize + offset) + "): " + ttcalc.getLinkTravelTimes().getLinkTravelTime(link1,
    // firstTimeBinStart + 5*timeBinSize + offset, null, null));
    //		System.out.println("firstTimeBinStart + linkTravelTime1 + offset (" + (firstTimeBinStart +
    // linkTravelTime1 + offset) + "): " + ttcalc.getLinkTravelTimes().getLinkTravelTime(link1,
    // firstTimeBinStart + linkTravelTime1 + offset, null, null));
    //		System.out.println("firstTimeBinStart + 3xtimeBinSize + linkTravelTime2 + offset (" +
    // (firstTimeBinStart + 3*timeBinSize + linkTravelTime2 + offset) + "): " +
    // ttcalc.getLinkTravelTimes().getLinkTravelTime(link1, firstTimeBinStart + 3*timeBinSize +
    // linkTravelTime2 + offset, null, null));
    assertEquals(
        linkTravelTime1,
        ttcalc
            .getLinkTravelTimes()
  ...
  /**
   * Test linear interpolation of aggregated travel times at different positions of a time bin.
   * (Previous tests only test the midpoint of each time bin.)
   *
   * @author tthunig
   */
  @Test
  public void testInterpolatedTravelTimes() {
    ...
    ttcalc.handleEvent(
        new LinkLeaveEvent(firstTimeBinStart + linkTravelTime1, vehId, link1.getId()));
    ttcalc.handleEvent(
        new LinkEnterEvent(firstTimeBinStart + 2 * timeBinSize, vehId, link1.getId()));
    ttcalc.handleEvent(
        new LinkLeaveEvent(
            firstTimeBinStart + 2 * timeBinSize + linkTravelTime2, vehId, link1.getId()));
    ...
    assertEquals(
        ...
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 0.5 * timeBinSize, null, null),
        MatsimTestUtils.EPSILON);
    ...
    assertEquals(
        ...
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 0.5 * timeBinSize + 1, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        linkTravelTime1 - 1 * timeBinSize + 1,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 1.5 * timeBinSize - 1, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        linkTravelTime1 - 1 * timeBinSize,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 1.5 * timeBinSize, null, null),
        MatsimTestUtils.EPSILON);
    // travel time should increase again by 1/3 seconds per time step between second and third time
    // bin
    assertEquals(
        linkTravelTime1 - 1 * timeBinSize + 1,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 1.5 * timeBinSize + 3, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        linkTravelTime2 - 1,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 2.5 * timeBinSize - 3, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        linkTravelTime2,
        ttcalc
            .getLinkTravelTimes()
            .getLinkTravelTime(link1, firstTimeBinStart + 2.5 * timeBinSize, null, null),
        MatsimTestUtils.EPSILON);
  }

  /**
   * Tests that calculating LinkTravelTimes works also without reading in a complete scenario
   ...
   */
  @Test
  public void testReadFromFile_LargeScenarioCase()
    ...
    EventsManager events =
        EventsUtils
            .createEventsManager(); // DO NOT USE EventsBuilderImpl() here, as we do not have a
    // population!
    ...
    assertEquals(
        "wrong link travel time at 06:00.",
        110.0,
        ttCalc.getLinkTravelTimes().getLinkTravelTime(link10, 6.0 * 3600, null, null),
        MatsimTestUtils.EPSILON);
    assertEquals(
        "wrong link travel time at 06:15.",
        359.9712023038157,
        ttCalc.getLinkTravelTimes().getLinkTravelTime(link10, 6.25 * 3600, null, null),
        1e-3); // traveltimecalculator has a resolution of 0.001 seconds
  }
  ...
  public void testGetLinkTravelTime_ignorePtVehiclesAtStop() {
    ...
    ttc.handleEvent(
        new VehicleArrivesAtFacilityEvent(
            240, ptVehId, Id.create("stop", TransitStopFacility.class), 0));
    ttc.handleEvent(new LinkLeaveEvent(350, ptVehId, link1.getId()));

    Assert.assertEquals(
        "The time of transit vehicles at stop should not be counted",
        100.0,
        ttc.getLinkTravelTimes().getLinkTravelTime(link1, 200, null, null),
        1e-8);
  }
  ...
  /**
   * Enable filtering but set an empty string as modes to analyze. Expect that all link travel times
   * are ignored.
   *
   * @author cdobler
   */
  ...
  public void testGetLinkTravelTime_NoAnalyzedModes() {
    ...
    ttc.handleEvent(
        new VehicleEntersTrafficEvent(100, agId1, link1.getId(), vehId, TransportMode.car, 1.0));
    ...
    Assert.assertEquals(
        "No transport mode has been registered to be analyzed, therefore no vehicle/agent should be counted",
        1000.0,
        ttc.getLinkTravelTimes().getLinkTravelTime(link2, 300, null, null),
        1e-8);
    // 1000.0s is the freespeed travel time (euclidean link length: 1000m, default freespeed: 1m/s)
  }

  /**
   * Enable filtering and analyze only car legs. Expect that walk legs are ignored.
   *
   * @author cdobler
   */
  @Test
  public void testGetLinkTravelTime_CarAnalyzedModes() {
    ...
    ttc.handleEvent(
        new VehicleEntersTrafficEvent(90, agId1, link1.getId(), vehId1, TransportMode.car, 1.0));
    ttc.handleEvent(
        new VehicleEntersTrafficEvent(100, agId2, link1.getId(), vehId2, TransportMode.walk, 1.0));
    ...
    Assert.assertEquals(
        "Only transport mode has been registered to be analyzed, therefore no walk agent should be counted",
        100.0,
        ttc.getLinkTravelTimes().getLinkTravelTime(link2, 200, null, null),
        1e-8);
  }

  /**
   * Disable filtering but also set analyzed modes to an empty string. Expect that still all modes
   * are counted.
   *
   * @author cdobler
   */
  ...
  public void testGetLinkTravelTime_NoFilterModes() {
    ...
    Assert.assertEquals(
        "Filtering analyzed transport modes is disabled, therefore count all modes",
        200.0,
        ttc.getLinkTravelTimes().getLinkTravelTime(link2, 200, null, null),
        1e-8);
  }
  ...
  /**
   * Enable filtering but do not set transport modes which should be counted. Expect that the
   * default value (=car) will be used for the modes to be counted.
   *
   * @author cdobler
   */
  @Test
  public void testGetLinkTravelTime_FilterDefaultModes() {
    ...
    Assert.assertEquals(
        "Filtering analyzed transport modes is enabled, but no modes set. Therefore, use default (=car)",
        100.0,
        ttc.getLinkTravelTimes().getLinkTravelTime(link2, 200, null, null),
        1e-8);
  }
