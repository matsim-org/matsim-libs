...
public class DeparturesOnSameLinkSameTimeTest {
   ...
   *
   * <p>Since, the flow capacity of link l_1 is 1 PCU per sec, both motorbike should be able to
   * leave the link l_1 at the same time whereas cars should leave at a gap of one second.
   */
  @Test
  public void test4LinkEnterTimeOfCarAndBike() {
    ...
    Map<Id<Vehicle>, Map<Id<Link>, Double>> motorbikeLinkLeaveTime =
        getLinkEnterTime("motorbike", 3600);
    Map<Id<Vehicle>, Map<Id<Link>, Double>> carLinkLeaveTime =
        getLinkEnterTime(TransportMode.car, 3600);

    double diff_carAgents_departureLink_LeaveTimes =
        carLinkLeaveTime.get(secondAgent).get(departureLink)
            - carLinkLeaveTime.get(firstAgent).get(departureLink);
    Assert.assertEquals(
        "Both car agents should leave at the gap of 1 sec.",
        1.,
        Math.abs(diff_carAgents_departureLink_LeaveTimes),
        MatsimTestUtils.EPSILON);

    double diff_motorbikeAgents_departureLink_LeaveTimes =
        motorbikeLinkLeaveTime.get(secondAgent).get(departureLink)
            - motorbikeLinkLeaveTime.get(firstAgent).get(departureLink);
    Assert.assertEquals(
        "Both motorbike agents should leave at the same time.",
        0.,
        diff_motorbikeAgents_departureLink_LeaveTimes,
        MatsimTestUtils.EPSILON);
    ...
    diff_carAgents_departureLink_LeaveTimes =
        carLinkLeaveTime.get(secondAgent).get(departureLink)
            - carLinkLeaveTime.get(firstAgent).get(departureLink);
    Assert.assertEquals(
        "Both car agents should leave at the same time",
        0.,
        diff_carAgents_departureLink_LeaveTimes,
        MatsimTestUtils.EPSILON);
  }

  private Map<Id<Vehicle>, Map<Id<Link>, Double>> getLinkEnterTime(
      String travelMode, double departureLinkCapacity) {
  ...
  private class PseudoInputs {
    ...
    private void createNetwork(double departureLinkCapacity) {
      ...
      link2 =
          NetworkUtils.createAndAddLink(
              network, Id.createLinkId("2"), fromNode1, toNode1, 1000.0, 20.0, 3600, 1, null, "7");
    }
