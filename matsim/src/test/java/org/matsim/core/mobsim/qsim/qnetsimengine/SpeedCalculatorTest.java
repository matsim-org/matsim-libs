...
public class SpeedCalculatorTest {
  // This originally comes from the bicycle contrib.  One now needs access to AbstractQLink to
  // properly test this functionality. In contrast, the
  ...
  @Test
  public void limitedByVehicleSpeed() {
    ...
    assertTrue(
        Double.isNaN(new SpecificLinkSpeedCalculator().getMaximumVelocity(vehicle, link, 1)));
  ...
  @Test
  public void limitedByLinkSpeed() {
  ...
  @Test
  public void bikeWithSpecificLinkSpeedCalculator() {
    ...
    assertEquals(
        type.getMaximumVelocity() * 1.5,
        new SpecificLinkSpeedCalculator().getMaximumVelocity(vehicle, link, 1),
        0.0);
  ...
  @Test
  public void bikeLimitedByLinkFreespeed() {
  ...
  private double getSpeedOnLink(Link link, QVehicle vehicle) {
    ...
    QSim qsim =
        new QSimBuilder(config).useDefaults().addQSimModule(module).build(scenario, eventsManager);
    var childInjector = qsim.getChildInjector();

    var qNetworkFactory = childInjector.getInstance(QNetworkFactory.class);
    var qLink =
        (AbstractQLink)
            qNetworkFactory.createNetsimLink(
                link, qNetworkFactory.createNetsimNode(link.getToNode()));
    var speedOnLink =
        qLink.getInternalInterface().getMaximumVelocityFromLinkSpeedCalculator(vehicle, 1.);
    return speedOnLink;
  }
  ...
  private Link createLinkWithNoGradientAndNoSpecialSurface() {
    ...
    Node fromNode =
        NetworkUtils.createAndAddNode(
            unusedNetwork, Id.createNodeId(UUID.randomUUID().toString()), from);
    Node toNode =
        NetworkUtils.createAndAddNode(
            unusedNetwork, Id.createNodeId(UUID.randomUUID().toString()), to);
    return NetworkUtils.createAndAddLink(
        unusedNetwork,
        Id.createLinkId(UUID.randomUUID().toString()),
        fromNode,
        toNode,
        length,
        1000,
  ...
  private static class SpecificLinkSpeedCalculator implements LinkSpeedCalculator {
    ...
    public double getMaximumVelocity(QVehicle qVehicle, Link link, double time) {
      if (isBike(qVehicle)) {
        return Math.min(
            qVehicle.getVehicle().getType().getMaximumVelocity() * 1.5, link.getFreespeed());
      } else {
        return Double.NaN;
        // (this now works because the link speed calculator returns the default for all
        // combinations of (vehicle, link, time) that
