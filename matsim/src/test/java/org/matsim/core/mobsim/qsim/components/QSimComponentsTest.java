...
public class QSimComponentsTest {
  ...
  public void testAddComponentViaString() {
    ...
    List<String> list =
        new ArrayList<>(
            qsimComponentsConfig.getActiveComponents()); // contains the "standard components" (*)
    list.add("abc");
  ...
  /**
   * this tests what happens when we run the same as in {@link #testAddComponentViaString()}, but
   * without requesting the "abc" component by config. "abc" will not be activated (although it is
   * "registered" = added as a qsim module).
   */
  @Test
  public void testAddModuleOnly() {
    ...
    Assert.assertFalse(
        "MockMobsimListener was added to QSim although it should not have been added",
        handler.hasBeenCalled());
  }

  /**
   * this tests what happens when we run the same as in {@link #testAddComponentViaString()}, but
   * without registering the "abc" module. Evidently, it throws an exception.
   */
  ...
  /**
   * this tests what happens when we run the same as in {@link #testAddComponentViaString()}, but
   * request the "abc" component twice.
   */
  @Test
  ...
  public void testGenericAddComponentMethod() {
    ...
    final QSimComponentsConfigurator configurator =
        new QSimComponentsConfigurator() {
          @Override
