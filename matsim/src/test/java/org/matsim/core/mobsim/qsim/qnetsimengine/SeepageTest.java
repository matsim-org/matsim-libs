...
public class SeepageTest {
  private static final Logger log = LogManager.getLogger(SeepageTest.class);
  ...
  /**
   * Two carAgents end act at time 948 and 949 sec and walkAgent ends act at 49 sec. Link length is
   * 1 km and flow capacity 1 PCU/min. Speed of car and walk is 20 mps and 1 mps. First car leave
   * link at 1000 sec thus blocking it for next 59 sec. Second car is in queue at 1000 sec.
   * WalkAgent joins queue at 1050 sec but leave link before second car at 1060 sec and thus
   * blocking link for another 6 sec(flowCap*PCU) Thus, second car leaves link after walkAgent.
   */
  @Test
  public void seepageOfWalkInCongestedRegime() {
    ...
    VehicleType walk =
        VehicleUtils.getFactory()
            .createVehicleType(Id.create(TransportMode.walk, VehicleType.class));
    ...
    for (int i = 0; i < 3; i++) {
      ...
      Vehicle vehicle =
          VehicleUtils.getFactory().createVehicle(vehicleId, modesType.get(leg.getMode()));
  ...
  private static final class SimpleNetwork {
    ...
    public SimpleNetwork() {
      ...
      Node node2 =
          NetworkUtils.createAndAddNode(network, Id.createNodeId("2"), new Coord(0.0, 0.0));
      Node node3 =
          NetworkUtils.createAndAddNode(network, Id.createNodeId("3"), new Coord(0.0, 1000.0));
      Node node4 =
          NetworkUtils.createAndAddNode(network, Id.createNodeId("4"), new Coord(0.0, 1100.0));

      Set<String> allowedModes = new HashSet<String>();
      allowedModes.addAll(Arrays.asList(TransportMode.car, TransportMode.walk));
      final Node fromNode = node1;
      final Node toNode = node2;

      link1 =
          NetworkUtils.createAndAddLink(
              network, Id.createLinkId("1"), fromNode, toNode, 100, 25, 36000, 1, null, "22");
      final Node fromNode1 = node2;
      final Node toNode1 = node3;
      link2 =
          NetworkUtils.createAndAddLink(
              network, Id.createLinkId("2"), fromNode1, toNode1, 1000, 25, 60, 1, null, "22");
      final Node fromNode2 = node3;
      final Node toNode2 = node4; // flow capacity is 1 PCU per min.
      link3 =
          NetworkUtils.createAndAddLink(
              network, Id.createLinkId("3"), fromNode2, toNode2, 100, 25, 36000, 1, null, "22");
  ...
  private static class VehicleLinkTravelTimeEventHandler
    ...
    public VehicleLinkTravelTimeEventHandler(
        final Map<Id<Vehicle>, Map<Id<Link>, Double>> agentTravelTimes) {
