...
/**
 * This test is to show that qsim returns actualTravelTime+1 because of java rounding errors. This
 * is more predominant when flow capacity of link is not multiple of 3600 Veh/h. In such scenarios,
 * flowCapacityFraction is accumulated in every second and that's where problem starts. For e.g.
 * 0.2+0.1 = 0.30000000004 also 0.6+0.1=0.79999999999999999 See, nice article
 * http://floating-point-gui.de/basic/
 *
 * <p>See small numerical test also
 *
 * @author amit
 */
public class JavaRoundingErrorInQsimTest {
  ...
  public void testToCheckTravelTime() {
    ...
    Assert.assertEquals(
        "Wrong travel time for on link 2 for vehicle 2",
        41.0,
        vehicleLinkTravelTime.get(Id.createVehicleId(2)),
        MatsimTestUtils.EPSILON);

    // agent 1 should have 1000/25 +1 + 10 = 51 but, it may be 52 sec sometimes due to rounding
    // errors in java. Rounding errors is eliminated at the moment if accumulating flow to zero
    // instead of one.
    Assert.assertEquals(
        "Wrong travel time for on link 2 for vehicle 1",
        51.0,
        vehicleLinkTravelTime.get(Id.createVehicleId(1)),
        MatsimTestUtils.EPSILON);
    LogManager.getLogger(JavaRoundingErrorInQsimTest.class)
        .warn(
            "Although the test is passing instead of failing for vehicle 1. This is done intentionally in order to keep this in mind for future.");
  }
  ...
  private static class VehicleLinkTravelTimeEventHandler
    ...
    public void handleEvent(LinkLeaveEvent event) {
      if (event.getLinkId().equals(Id.createLinkId(2))) {
        vehicleTravelTime.put(
            event.getVehicleId(), vehicleTravelTime.get(event.getVehicleId()) + event.getTime());
      }
    }
