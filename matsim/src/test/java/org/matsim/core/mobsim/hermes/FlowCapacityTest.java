...
public class FlowCapacityTest {
  ...
  private static final Logger log = LogManager.getLogger(FlowCapacityTest.class);
  ...
  public void testFlowCapacityDrivingFlowEfficiencyFactors() {
    ...
    for (int i = 1; i <= 12000; i++) {
      ...
      Vehicle vehicle =
          i % 2 == 1
              ? VehicleUtils.createVehicle(Id.createVehicleId(person.getId()), av)
              : VehicleUtils.createVehicle(Id.createVehicleId(person.getId()), car);
    ...
    Assert.assertEquals(
        4005,
        volume[
            6]); // we should have half of the maximum flow in this hour * 1.5, because every second
    // vehicle is super flowy
  ...
  public void testFlowCapacityDrivingFlowEfficiencyFactorsWithDownscaling() {
    ...
    Assert.assertEquals(
        401, volume[6],
        1); // we should have half of the maximum flow in this hour * 1.3333, because every second
    // vehicle is super flowy
  ...
  /**
   * Tests flow efficiency factors, e.g. for AVs, are working with downscaling and a value <1 for
   * flow efficiency
   *
   * @author jfbischoff
   */
  ...
  public void testFlowCapacityEfficiencyFactorWithLowValueAndDownscaling() {
    ...
    for (int i = 1; i <= 1200; i++) {
      ...
      Vehicle vehicle =
          i % 2 == 1
              ? VehicleUtils.createVehicle(Id.createVehicleId(person.getId()), tractor)
              : VehicleUtils.createVehicle(Id.createVehicleId(person.getId()), car);
      f.scenario.getVehicles().addVehicle(vehicle);
      VehicleUtils.insertVehicleIdsIntoAttributes(
          person, Map.of(TransportMode.car, vehicle.getId()));
