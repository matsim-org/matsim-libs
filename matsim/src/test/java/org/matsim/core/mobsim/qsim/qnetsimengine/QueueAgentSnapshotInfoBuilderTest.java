...
public class QueueAgentSnapshotInfoBuilderTest {
  ...
  public void positionVehiclesAlongLine_singleVehicleFreeFlow() {
    ...
    var now = 1.0;
    var vehicles =
        createVehicles(setUp.link, 1, setUp.linkEnterTime + setUp.linkLength / setUp.freespeed);
    var builder =
        new QueueAgentSnapshotInfoBuilder(setUp.scenario, new SnapshotLinkWidthCalculator());
    ...
    assertEquals(
        -18.75,
        firstEntry.getNorthing(),
        0.00001); // the calculator assumes an offset to the right of the driving direction ...
  ...
  public void positionVehiclesAlongLine_congestedAboveCapacityLimit() {
    ...
    // we have put more cars onto the link than it can actually fit. The calculator should squeeze
    // all vehicles onto the link now
  ...
  public void positionVehiclesAlongLine_queueAtEnd() {
    ...
    // we expect 1 car to be at the beginning of the queue (2 vehicles * 10 + 10 [its own offset])
    // with a speed of 1.0
  ...
  private static void assertStackedPositions(
      Collection<AgentSnapshotInfo> positions,
      double expectedEasting,
      double expectedFirstNothing,
      AgentSnapshotInfo.AgentState expectedState) {

    // the positions should be at (expectedEasting, -18.75 - 3.75 * waitingListIndex)
    // all should drive a car
    // agentId, vehicleId, linkId should all be 1 if they come out of 'createVehicles' or
    // 'createAgents'
  ...
  public void positionAgentsInActivities() {
    ...
    // act
    var newCount =
        builder.positionAgentsInActivities(outCollection, setUp.link, waitingList, setUp.counter);

    // assert
    assertEquals(setUp.counter + waitingList.size(), newCount);
    assertStackedPositions(
        outCollection,
        setUp.linkLength * 0.9,
        -15,
  ...
  public void positionVehiclesFromWaitingList() {
    ...
    // act
    var newCount =
        builder.positionVehiclesFromWaitingList(
            outCollection, setUp.link, setUp.counter, setUp.waitingList);
  ...
  public void positionVehiclesFromTransitStop() {
    ...
    // act
    var newCount =
        builder.positionVehiclesFromTransitStop(
            outCollection, setUp.link, waitingList, setUp.counter);
    ...
    assertStackedPositions(
        ...
        setUp.linkLength * 0.9,
        -18.75,
        AgentSnapshotInfo.AgentState.PERSON_OTHER_MODE);
  }
  ...
  private static class TestTransitDriverAgent extends TestDriverAgent
  ...
  private static Collection<QVehicle> createVehicles(
      Link link, int size, double exitTimeOfFirstVehicle) {

    VehicleType type =
        VehicleUtils.createVehicleType(Id.create(TransportMode.car, VehicleType.class));
  ...
  private static Queue<QVehicle> createTransitVehicles(
      Link link, int size, double exitTimeOfFirstVehicle) {

    VehicleType type =
        VehicleUtils.createVehicleType(Id.create(TransportMode.pt, VehicleType.class));
    type.setMaximumVelocity(link.getFreespeed() + 10); // faster than link's freespeed
