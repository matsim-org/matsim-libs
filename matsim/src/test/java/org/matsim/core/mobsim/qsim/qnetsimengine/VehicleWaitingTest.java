...
/**
 * Tests the behavior of the qsim with agents waiting for vehicles. <br>
 * This is not really a "unit" test anymore (thus it artificially increases test coverage of the
 * qsim), but at least, it checks that recent bugfixes actually fixed something. td, mar. 2013
 *
 * @author thibautd
 */
public class VehicleWaitingTest {
  ...
  private static void testVehicleWaitingDoesntFail(
      final int nLaps, final boolean insertActivities) {
    ...
    final Node node1 =
        netFact.createNode(Id.create(1, Node.class), new Coord((double) 0, (double) 0));
    final Node node2 =
        netFact.createNode(Id.create(2, Node.class), new Coord((double) 0, (double) 1000));
    final Node node3 =
        netFact.createNode(Id.create(3, Node.class), new Coord((double) 1000, (double) 1000));
      ...
      for (int lap = 0; lap < nLaps; lap++) {
        ...
        final NetworkRoute route =
            RouteUtils.createLinkNetworkRouteImpl(
                link1.getId(), Collections.singletonList(link2.getId()), link3.getId());
        route.setVehicleId(
            Id.create(
                personId1,
                Vehicle.class)); // QSim creates a vehicle per person, with the ids of the persons
        leg.setRoute(route);
        plan.addLeg(leg);
        ...
        final NetworkRoute secondRoute =
            RouteUtils.createLinkNetworkRouteImpl(
                link3.getId(), Collections.<Id<Link>>emptyList(), link1.getId());

        secondRoute.setVehicleId(
            Id.create(
                personId1,
                Vehicle.class)); // QSim creates a vehicle per person, with the ids of the persons
    ...
    events.addHandler(
        new PersonArrivalEventHandler() {
          ...
          public void handleEvent(final PersonArrivalEvent event) {
            ...
            arrivalCounts.put(event.getPersonId(), count == null ? 1 : count + 1);
          }
        });
    ...
    for (Id<Person> id : personIds) {
      Assert.assertNotNull("no arrivals for person " + id, arrivalCounts.get(id));
      Assert.assertEquals(
