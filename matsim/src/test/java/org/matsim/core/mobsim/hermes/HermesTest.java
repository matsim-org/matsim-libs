...
public class HermesTest {
  ...
  private static final Logger log = LogManager.getLogger(HermesTest.class);
  ...
  protected static Hermes createHermes(
      Scenario scenario, EventsManager events, boolean prepareForSim) {
    if (prepareForSim) {
      PrepareForSimUtils.createDefaultPrepareForSim(scenario).run();
    }
    return new HermesBuilder().build(scenario, events);
  ...
  public void testVehicleTeleportationTrue() {
    ...
    l1.setRoute(
        f.scenario
            ...
            .getFactory()
            .getRouteFactories()
            .createRoute(GenericRouteImpl.class, f.link1.getId(), f.link2.getId()));
