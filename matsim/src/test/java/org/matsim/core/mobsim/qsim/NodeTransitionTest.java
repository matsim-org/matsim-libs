 ...
 * EmptyBufferAfterBuffer and blockNodeWhenSingleOutlinkFull = false currently is the MATSim
 * standard.
 *
 * <p>This test checks the throughput of links in two different scenarios: 1. A merge situation,
 * where agents coming from three different links all want to enter the same downstream link with
 * restricted capacity. 2. An intersection, where agents coming from two different links do not use
 * same links but might affect each other when the flag blockNodeWhenSingleOutlinkFull is set to
 * true.
 *
 * <p>While implementing and significantly testing the new node transitions some bugs have been
 * found in QueueWithBuffer regarding half empty buffers and time step sizes < 1. This tests
 * therefore also validates the bug fixes (see e.g. testNodeTransitionWithTimeStepSizeSmallerOne).
 *
 * <p>The results should be the same independently of slow/fast capacity update. That's why the test
 * is run for both (parameterized).
 *
 * @author tthunig
 */
...
public class NodeTransitionTest {
  ...
  public void testMergeSituationWithEmptyBufferAfterBufferRandomDistribution() {
    ...
    scenario
        .getConfig()
        .qsim()
        .setNodeTransitionLogic(
            NodeTransition.emptyBufferAfterBufferRandomDistribution_dontBlockNode);
    ...
    // this is the time before the downstream link (7_8) gets full and with that no inflow capacity
    // is valid
    int timeInterval1Start = 110;
    int timeInterval1End = 180;
    Map<Id<Link>, Double> avgThroughputFreeFlow =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval1Start, timeInterval1End, 1);
    // this is the time when the downstream link (7_8) is already full and only 2 vehicles per
    // second can enter but the third commodity has not started sending vehicles yet
    int timeInterval2Start = 210;
    int timeInterval2End = 340;
    Map<Id<Link>, Double> avgThroughputCongestedTwoLinks =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval2Start, timeInterval2End, 1);
    // this is the time when all three commodities send vehicles and the downstream link (7_8) is
    // still full and only 2 vehicles per second can enter
    int timeInterval3Start = 360;
    int timeInterval3End = 500;
    Map<Id<Link>, Double> avgThroughputCongestedThreeLinks =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval3Start, timeInterval3End, 1);
    for (Id<Link> link : linksOfInterest) {
      System.out.println(
          "avgThroughput link "
              ...
              + timeInterval1End
              + "]: "
              + avgThroughputFreeFlow.get(link)
              + "\t; time interval ["
              + timeInterval2Start
              + ", "
              + timeInterval2End
              + "]: "
              + avgThroughputCongestedTwoLinks.get(link)
              + "\t; time interval ["
              + timeInterval3Start
              + ", "
              + timeInterval3End
              + "]: "
              + avgThroughputCongestedThreeLinks.get(link));
    }
    ...
    Assert.assertEquals(
        "Troughput on link 2_7 is wrong",
        1,
        avgThroughputFreeFlow.get(Id.createLinkId("2_7")),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Troughput on link 4_7 is wrong",
        2,
        avgThroughputFreeFlow.get(Id.createLinkId("4_7")),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Troughput on link 6_7 is wrong",
        0,
        avgThroughputFreeFlow.get(Id.createLinkId("6_7")),
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Troughput on link 2_7 is wrong",
        1. / 3,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("2_7")),
        delta); // 0.25384615384615383
    Assert.assertEquals(
        "Troughput on link 4_7 is wrong",
        5. / 3,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("4_7")),
        delta); // 1.7461538461538462
    Assert.assertEquals(
        "Troughput on link 6_7 is wrong",
        0,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("6_7")),
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Troughput on link 2_7 is wrong",
        1. / 5,
        avgThroughputCongestedThreeLinks.get(Id.createLinkId("2_7")),
        delta); // 0.17857142857142858
    Assert.assertEquals(
        "Troughput on link 4_7 is wrong",
        9. / 10,
        avgThroughputCongestedThreeLinks.get(Id.createLinkId("4_7")),
        delta); // 0.8928571428571429
    Assert.assertEquals(
        "Troughput on link 6_7 is wrong",
        9. / 10,
        avgThroughputCongestedThreeLinks.get(Id.createLinkId("6_7")),
        delta); // 0.9285714285714286
  }
  ...
  public void testMergeSituationWithMoveVehByVehRandomDistribution() {
    ...
    scenario
        .getConfig()
        .qsim()
        .setNodeTransitionLogic(NodeTransition.moveVehByVehRandomDistribution_dontBlockNode);
    scenario.getConfig().qsim().setUsingFastCapacityUpdate(useFastCapUpdate);
    ...
    Assert.assertEquals(
        ...
        5. / 9,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("2_7")),
        delta); // 0.5307692307692308
    Assert.assertEquals(
        "Troughput on link 4_7 is wrong",
        13. / 9,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("4_7")),
        delta); // 1.4692307692307693
    Assert.assertEquals(
        "Troughput on link 6_7 is wrong",
        0,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("6_7")),
  ...
  public void testMergeSituationWithMoveVehByVehDeterministicPriorities() {
    ...
    // note: the deterministic node transition is only implemented for the case when the node is
    // blocked as soon as one outgoing link is full
    scenario.getConfig().qsim().setUsingFastCapacityUpdate(useFastCapUpdate);
    ...
    Assert.assertEquals(
        ...
        1. / 3 * 2,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("2_7")),
        deltaPeriodic); // 0.6692307692307692
    Assert.assertEquals(
        "Troughput on link 4_7 is wrong",
        2. / 3 * 2,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("4_7")),
        deltaPeriodic); // 1.3307692307692307
    Assert.assertEquals(
        "Troughput on link 6_7 is wrong",
        0,
        avgThroughputCongestedTwoLinks.get(Id.createLinkId("6_7")),
  ...
  public void testBlockedNodeSituationWithEmptyBufferAfterBufferRandomDistribution() {
    ...
    //			Map<Id<Link>, Double> avgThroughputThisInterval =
    // throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(start, start+10, 1);
    //			System.out.println("[" + start + ", " + (start+10) + "]. link 2_5: " +
    // avgThroughputThisInterval.get(Id.createLinkId("2_5"))
    //					+ ", link 4_5: " + avgThroughputThisInterval.get(Id.createLinkId("4_5"))
    //					+ ", link 5_8: " + avgThroughputThisInterval.get(Id.createLinkId("5_8")));
    //		}
    //		Map<Id<Link>, Double> avgThroughputUntil210 =
    // throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(0, 210, 1);
    //		System.out.println("absolute throughput [" + 0 + ", " + 210 + "]. link 2_5: " +
    // (avgThroughputUntil210.get(Id.createLinkId("2_5")) * 210 / 1)
    //				+ ", link 4_5: " + (avgThroughputUntil210.get(Id.createLinkId("4_5")) * 210 / 1) );

    // this is the time before the downstream link (5_8) gets full and with that no inflow capacity
    // is valid and no node is blocked
    int timeInterval1Start = 110;
    int timeInterval1End = 200;
    Map<Id<Link>, Double> avgThroughputFreeFlow =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval1Start, timeInterval1End, 1);
    // this is the time when the downstream link (5_8) is full and also no vehicles are leaving this
    // link such that no vehicles can enter and the node stays blocked
    int timeInterval2Start = 210;
    int timeInterval2End = 300;
    Map<Id<Link>, Double> avgThroughputCongestedNodeBlocked =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval2Start, timeInterval2End, 1);
    // this is the time when one vehicle per second is allowed to enter 5_8, i.e. 2_5 is restricted
    // to half of its outflow capacity
    int timeInterval3Start = 310;
    int timeInterval3End = 500;
    Map<Id<Link>, Double> avgThroughputCongestedRestrictFlow =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval3Start, timeInterval3End, 1);
    for (Id<Link> link : linksOfInterest) {
      System.out.println(
          "avgThroughput link "
              ...
              + timeInterval2End
              + "]: "
              + avgThroughputCongestedNodeBlocked.get(link)
              + "\t; time interval ["
              + timeInterval3Start
              + ", "
              + timeInterval3End
              + "]: "
              + avgThroughputCongestedRestrictFlow.get(link));
    }
     ...
     */
    Assert.assertEquals(
        "Troughput on link 2_5 is wrong",
        2,
        avgThroughputFreeFlow.get(Id.createLinkId("2_5")),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Troughput on link 4_5 is wrong",
        1,
        avgThroughputFreeFlow.get(Id.createLinkId("4_5")),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Troughput on link 5_8 is wrong",
        0,
        avgThroughputFreeFlow.get(Id.createLinkId("5_8")),
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Troughput on link 2_5 is wrong",
        0,
        avgThroughputCongestedNodeBlocked.get(Id.createLinkId("2_5")),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Troughput on link 4_5 is wrong",
        1. / 3,
        avgThroughputCongestedNodeBlocked.get(Id.createLinkId("4_5")),
        delta); // 0.36666666666666664
    Assert.assertEquals(
        "Troughput on link 5_8 is wrong",
        0,
        avgThroughputCongestedNodeBlocked.get(Id.createLinkId("5_8")),
        MatsimTestUtils.EPSILON);
    ...
    Assert.assertEquals(
        "Troughput on link 2_5 is wrong",
        1,
        avgThroughputCongestedRestrictFlow.get(Id.createLinkId("2_5")),
        MatsimTestUtils.EPSILON);
    Assert.assertEquals(
        "Troughput on link 4_5 is wrong",
        1. / 3,
        avgThroughputCongestedRestrictFlow.get(Id.createLinkId("4_5")),
        delta); // 0.3263157894736842
    Assert.assertEquals(
        "Troughput on link 5_8 is wrong",
        1,
        avgThroughputCongestedRestrictFlow.get(Id.createLinkId("5_8")),
        MatsimTestUtils.EPSILON);
  }
  ...
  public void testBlockedNodeSituationWithMoveVehByVehRandomDistribution() {
    ...
    scenario
        ...
        .qsim()
        .setNodeTransitionLogic(
            NodeTransition.moveVehByVehRandomDistribution_nodeBlockedWhenSingleOutlinkFull);
    scenario.getConfig().qsim().setUsingFastCapacityUpdate(useFastCapUpdate);
  ...
  public void testBlockedNodeSituationWithMoveVehByVehDeterministicPriorities() {
    ...
    Map<Id<Link>, Double> avgThroughputCongestedNodeBlocked =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval2Start, timeInterval2End, 1);
    // this is the time when one vehicle per second is allowed to enter 5_8, i.e. 2_5 is restricted
    // to half of its outflow capacity, i.e. 4_5 gets also restricted to half of it's capacity
    int timeInterval3Start = 310;
    int timeInterval3End = 500;
    Map<Id<Link>, Double> avgThroughputCongestedRestrictFlow =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
  ...
  /**
   * Test single intersection scenario with old emptyBufferAfterBuffer node transition and
   * blockNode=false. Use time bin size 0.5 seconds. With that the tests validates the following
   * things: 1. correct throughput for a time bin size smaller than 1 (see e.g. former bug in
   * QueueWithBuffer removeFirstVehicle); 2. correct storage capacity bounds for a time bin size
   * smaller than 1 (see e.g. former bug in QueueWithBuffer calculateStorageCapacity); 3. both
   * streams are independently (because blockNode=false).
   */
  @Test
  public void testNodeTransitionWithTimeStepSizeSmallerOne() {
    ...
    //		for (int start = 0; start <= 800; start+=10) {
    //			Map<Id<Link>, Double> avgThroughputThisInterval =
    // throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(start, start+10, 0.5);
    //			System.out.println("[" + start + ", " + (start+10) + "]. link 2_5: " +
    // avgThroughputThisInterval.get(Id.createLinkId("2_5"))
    //					+ ", link 4_5: " + avgThroughputThisInterval.get(Id.createLinkId("4_5"))
    //					+ ", link 5_8: " + avgThroughputThisInterval.get(Id.createLinkId("5_8")));
    //		}
    //		Map<Id<Link>, Double> avgThroughputUntil210 =
    // throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(0, 210, 0.5);
    //		System.out.println("absolute throughput [" + 0 + ", " + 210 + "]. link 2_5: " +
    // (avgThroughputUntil210.get(Id.createLinkId("2_5")) * 210 / 0.5)
    ...
    Map<Id<Link>, Double> avgThroughputFreeFlow =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval1Start, timeInterval1End, 0.5);
    // this is the time when the downstream link (5_8) is full and also no vehicles are leaving this
    // link such that no vehicles can enter and the node stays blocked
    int timeInterval2Start = 210;
    int timeInterval2End = 300;
    Map<Id<Link>, Double> avgThroughputCongestedNodeBlocked =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval2Start, timeInterval2End, 0.5);
    // this is the time when one vehicle per second is allowed to enter 5_8, i.e. 2_5 is restricted
    // to half of its outflow capacity
    int timeInterval3Start = 310;
    int timeInterval3End = 500;
    Map<Id<Link>, Double> avgThroughputCongestedRestrictFlow =
        throughputAnalyzer.calculateAvgThroughputPerTimeStepOfTimeInterval(
            timeInterval3Start, timeInterval3End, 0.5);
    for (Id<Link> link : linksOfInterest) {
      System.out.println(
          "avgThroughput link "
              + link
  ...
  private static final class Fixture {
    ...
    static Scenario createMergeScenario() {
      ...
      Node node1 =
          NetworkUtils.createAndAddNode(network, Id.create("1", Node.class), new Coord(0, 1000));
      Node node2 =
          NetworkUtils.createAndAddNode(network, Id.create("2", Node.class), new Coord(1000, 500));
      Node node3 =
          NetworkUtils.createAndAddNode(network, Id.create("3", Node.class), new Coord(0, 0));
      Node node4 =
          NetworkUtils.createAndAddNode(network, Id.create("4", Node.class), new Coord(1000, 0));
      Node node5 =
          NetworkUtils.createAndAddNode(network, Id.create("5", Node.class), new Coord(0, -1000));
      Node node6 =
          NetworkUtils.createAndAddNode(network, Id.create("6", Node.class), new Coord(1000, -500));
      Node node7 =
          NetworkUtils.createAndAddNode(network, Id.create("7", Node.class), new Coord(2000, 0));
      Node node8 =
          NetworkUtils.createAndAddNode(network, Id.create("8", Node.class), new Coord(3000, 0));
      Node node9 =
          NetworkUtils.createAndAddNode(network, Id.create("9", Node.class), new Coord(4000, 0));
      Link link12 =
          NetworkUtils.createAndAddLink(
              network, Id.create("1_2", Link.class), node1, node2, 1000, 10, 3600, 1);
      Link link27 =
          NetworkUtils.createAndAddLink(
              network, Id.create("2_7", Link.class), node2, node7, 1000, 10, 3600, 1);
      Link link34 =
          NetworkUtils.createAndAddLink(
              network, Id.create("3_4", Link.class), node3, node4, 1000, 10, 7200, 2);
      Link link47 =
          NetworkUtils.createAndAddLink(
              network, Id.create("4_7", Link.class), node4, node7, 1000, 10, 7200, 2);
      Link link56 =
          NetworkUtils.createAndAddLink(
              network, Id.create("5_6", Link.class), node5, node6, 1000, 10, 7200, 2);
      Link link67 =
          NetworkUtils.createAndAddLink(
              network, Id.create("6_7", Link.class), node6, node7, 1000, 10, 7200, 2);
      Link link78 =
          NetworkUtils.createAndAddLink(
              network, Id.create("7_8", Link.class), node7, node8, 1000, 10, 7200, 2);
      Link link89 =
          NetworkUtils.createAndAddLink(
              network, Id.create("8_9", Link.class), node8, node9, 1000, 10, 7200, 2);

      /* build plans */
      fillPopulationWithOneCommodity(
          scenario.getPopulation(), 1, 500, 0, link12.getId(), link89.getId());
      fillPopulationWithOneCommodity(
          scenario.getPopulation(), 2, 500, 0, link34.getId(), link89.getId());
      fillPopulationWithOneCommodity(
          scenario.getPopulation(), 2, 250, 250, link56.getId(), link89.getId());
    ...
    static Scenario createBlockedNodeScenario() {
      ...
      Network network = scenario.getNetwork();
      Node node1 =
          NetworkUtils.createAndAddNode(network, Id.create("1", Node.class), new Coord(0, 2000));
      Node node2 =
          NetworkUtils.createAndAddNode(network, Id.create("2", Node.class), new Coord(1000, 2000));
      Node node3 =
          NetworkUtils.createAndAddNode(network, Id.create("3", Node.class), new Coord(2000, 4000));
      Node node4 =
          NetworkUtils.createAndAddNode(network, Id.create("4", Node.class), new Coord(2000, 3000));
      Node node5 =
          NetworkUtils.createAndAddNode(network, Id.create("5", Node.class), new Coord(2000, 2000));
      Node node6 =
          NetworkUtils.createAndAddNode(network, Id.create("6", Node.class), new Coord(2000, 1000));
      Node node7 =
          NetworkUtils.createAndAddNode(network, Id.create("7", Node.class), new Coord(2000, 0));
      Node node8 =
          NetworkUtils.createAndAddNode(network, Id.create("8", Node.class), new Coord(3000, 2000));
      Node node9 =
          NetworkUtils.createAndAddNode(network, Id.create("9", Node.class), new Coord(4000, 2000));
      Link link12 =
          NetworkUtils.createAndAddLink(
              network, Id.create("1_2", Link.class), node1, node2, 1000, 10, 7200, 2);
      Link link25 =
          NetworkUtils.createAndAddLink(
              network, Id.create("2_5", Link.class), node2, node5, 1000, 10, 7200, 2);
      Link link34 =
          NetworkUtils.createAndAddLink(
              network, Id.create("3_4", Link.class), node3, node4, 1000, 10, 3600, 1);
      Link link45 =
          NetworkUtils.createAndAddLink(
              network, Id.create("4_5", Link.class), node4, node5, 1000, 10, 3600, 1);
      Link link56 =
          NetworkUtils.createAndAddLink(
              network, Id.create("5_6", Link.class), node5, node6, 1000, 10, 3600, 1);
      Link link67 =
          NetworkUtils.createAndAddLink(
              network, Id.create("6_7", Link.class), node6, node7, 1000, 10, 3600, 1);
      Link link58 =
          NetworkUtils.createAndAddLink(
              network, Id.create("5_8", Link.class), node5, node8, 1000, 5, 3600, 1);
      Link link89 =
          NetworkUtils.createAndAddLink(
              network, Id.create("8_9", Link.class), node8, node9, 1000, 10, 7200, 2);
      ...
      /* build plans */
      fillPopulationWithOneCommodity(
          scenario.getPopulation(), 2, 500, 0, link12.getId(), link89.getId());
      fillPopulationWithOneCommodity(
          scenario.getPopulation(), 1, 500, 0, link34.getId(), link67.getId());

      return scenario;
    }

    private static void fillPopulationWithOneCommodity(
      ...
      for (int i = 0; i < agentsPerSec * simulationPeriod; i++) {
        ...
        Person person =
            population
                .getFactory()
                .createPerson(Id.createPersonId("agent-" + sourceLink + "-" + sinkLink + "-" + i));
    ...
    public void handleEvent(LinkLeaveEvent event) {
      for (Id<Link> link : linksOfInterest) {
        if (event.getLinkId().equals(link)) {
          ...
          throughputMapThisLink.put(
              event.getTime(), throughputMapThisLink.get(event.getTime()) + 1);
        }
      }
    }

    public Map<Id<Link>, Double> calculateAvgThroughputPerTimeStepOfTimeInterval(
        int startTime, int endTime, double timeStepSize) {
      ...
      for (double time = startTime; time < endTime; time += timeStepSize) {
        for (Id<Link> link : linksOfInterest) {
          ...
          if (absThroughputThisLink.containsKey(time))
            avgThroughputPerLink.put(
                link, avgThroughputPerLink.get(link) + absThroughputThisLink.get(time));
        }
      }
      // calculate average (divide by the number of time steps)
      for (Id<Link> link : linksOfInterest) {
        avgThroughputPerLink.put(
            link, avgThroughputPerLink.get(link) * timeStepSize / (endTime - startTime));
      }
