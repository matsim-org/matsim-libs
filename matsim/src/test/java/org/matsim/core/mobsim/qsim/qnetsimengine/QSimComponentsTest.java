...
public class QSimComponentsTest {
  private static final String MY_NETSIM_ENGINE = "MyNetsimEngine";
  ...
  public void testRemoveNetsimEngine() {
    ...
    // running MATSim should fail after removing the netsim engine, since some of the routes do not
    // have a travel time, and in consequence cannot
    // be teleported if netsim engine is missing.  Thus, the RuntimeException confirms that removing
    // the module worked.
    ...
    // remove the module:  (There is also syntax at some intermediate level for this, but I prefer
    // the syntax at config level.  kai, oct'22)
    QSimComponentsConfigGroup componentsConfig =
  ...
  public void testReplaceQNetworkFactory() {
    // here we try to replace the QNetworkFactory.  Complains that QNetworkFactory is bound multiple
    // times.
  ...
  public void testOverridingQSimModule() {
    // use the newly implemented install _overriding_ qsim module.  With this, replacing the
    // QNetworkFactory now works as part of AbstractModule.
    ...
    controler.addOverridingModule(
        new AbstractModule() {
          @Override
          public void install() {
            this.installOverridingQSimModule(
                new AbstractQSimModule() {
                  @Override
                  protected void configureQSim() {
                    bind(QNetworkFactory.class).to(MyNetworkFactory.class);
  ...
  private static class MyNetworkFactory implements QNetworkFactory {
    ...
    @Inject
    MyNetworkFactory(EventsManager events, Scenario scenario) {
    ...
    public void initializeFactory(
        ...
        QNetsimEngineI.NetsimInternalInterface simEngine1) {
