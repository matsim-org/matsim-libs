  ...
  public static double getTravelTime(LinkedList<Event> events, int agentId) {
    ...
    for (int i = 0; i < events.size(); i++) {
      if (events.get(i) instanceof PersonDepartureEvent) {
        if (Integer.parseInt(((PersonDepartureEvent) events.get(i)).getPersonId().toString())
            == agentId) {
          startLegTime = events.get(i).getTime();
        }
      } else if (events.get(i) instanceof PersonArrivalEvent) {
        if (Integer.parseInt(((PersonArrivalEvent) events.get(i)).getPersonId().toString())
            == agentId) {
    ...
    for (int i = 0; i < events.size(); i++) {
      ...
      if (currentEvent instanceof PersonDepartureEvent) {
        if (currentEvent.getTime() < 0) {
          ...
          startingTime.put(((PersonDepartureEvent) currentEvent).getPersonId().toString(), 0.0);
        } else {
          startingTime.put(
              ((PersonDepartureEvent) currentEvent).getPersonId().toString(),
              currentEvent.getTime());
        }
      } else if (currentEvent instanceof PersonArrivalEvent) {
        travelTime +=
            currentEvent.getTime()
                - startingTime.get(((PersonArrivalEvent) currentEvent).getPersonId().toString());
      }
    }
    return travelTime;
  }
