...
/**
 * test flow capacity of links with lanes in the simulation, i.e. the number of vehicles that leave
 * the link or lane per hour
 *
 * @author tthunig
 */
public class SimulatedLaneFlowCapacityTest {
  ...
  /**
   * create a lane on link 1 representing the given number of lanes. the lanes capacity is 900 times
   * the number of represented lanes.
   */
  ...
  /**
   * create three lanes on link 1. the first and the third one represent 1 lane each, the second one
   * represents 2 lanes. lane 1 and 3 have a capacity of 900 each; lane 2 has a capacity of 2000,
   * which is higher than the link capacity itself.
   */
  ...
  /**
   * create a population of 5000 persons traveling from link 0 to link 2. they all start 50 minutes
   * after midnight and then queue on link 0 which lets one of them depart every second. for
   * analysis we then use flow values from the second hour.
   */
  private static void initPopulation(Population population) {
    ...
    for (int i = 0; i < 5000; i++) {
      ...
      Activity startAct =
          population.getFactory().createActivityFromLinkId("dummy", Id.createLinkId("0"));
      startAct.setEndTime(50 * 60);
      // create a drain activity at link 2
      Activity drainAct =
          population.getFactory().createActivityFromLinkId("dummy", Id.createLinkId("2"));
      // create a dummy leg
      Leg leg = population.getFactory().createLeg(TransportMode.car);
      leg.setRoute(
          RouteUtils.createLinkNetworkRouteImpl(
              Id.createLinkId("0"),
              Collections.singletonList(Id.createLinkId("1")),
              Id.createLinkId("2")));
  ...
  /**
   * test simulated capacity of link 1 in case without lanes. the capacity should correspond to the
   * given flow capacity of the link
   */
  @Test
  ...
  /**
   * test simulated capacities of link 1 in case of one lane representing one lane. the capacity of
   * the link should correspond to the capacity of the lane, also when it is less than the link
   * capacity given in the network.
   */
  @Test
  public void testCapacityWithOneLaneOneLane() {
    ...
    // check simulated capacity values
    assertEquals(
        simulatedCapacity.getSimulatedLaneCapacity(Id.create("1", Lane.class)),
        simulatedCapacity.getSimulatedLinkCapacity(),
        MatsimTestUtils.EPSILON);
    assertEquals(
        1800,
        simulatedCapacity.getSimulatedLaneCapacity(Id.create("1.ol", Lane.class)),
        MatsimTestUtils.EPSILON);
    assertEquals(
        900,
        simulatedCapacity.getSimulatedLaneCapacity(Id.create("1", Lane.class)),
  ...
  /**
   * test simulated capacities of link 1 in case of one lane representing two lanes. the capacity of
   * the link should correspond to the capacity of the lane, also when it is less than the link
   * capacity given in the network.
   */
  @Test
  ...
  /**
   * test simulated capacities of link 1 in case of three lanes. the simulated capacity of the link
   * should correspond to the sum of capacities of all three lanes, also when it is less than the
   * link capacity given in the network. Interestingly, it also corresponds to this sum, if it is
   * more than the link capacity given in the network. And, finally, it still only uses the lane
   * capacity given in the network, when it is higher than the link capacity (see lane 2 here).
   */
  @Test
  public void testCapacityWithThreeLanes() {
    ...
    double originalLaneCap =
        simulatedCapacity.getSimulatedLaneCapacity(Id.create("1.ol", Lane.class));
    System.out.println(
        "LeaveEvents: " + originalLaneCap + ", " + lane1Cap + ", " + lane2Cap + ", " + lane3Cap);
    assertEquals(
        lane1Cap + lane2Cap + lane3Cap,
        simulatedCapacity.getSimulatedLinkCapacity(),
        MatsimTestUtils.EPSILON);
  ...
  class SimulatedCapacityHandler implements LinkLeaveEventHandler, LaneLeaveEventHandler {
    ...
    public void handleEvent(LaneLeaveEvent event) {
      ...
      if (event.getLinkId().equals(Id.createLinkId(1))
          && event.getTime() >= 3600
          && event.getTime() < 2 * 3600) {
