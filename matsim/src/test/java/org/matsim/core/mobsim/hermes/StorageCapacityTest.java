...
public class StorageCapacityTest {
  ...
  /**
   * Tests that the storage capacity can be reached (but not exceeded) by agents driving over a
   * link.
   *
   * @author jfbischoff
   */
    ...
    for (int i = 1; i <= 600; i++) {
      ...
      NetworkRoute route =
          scenario
              ...
              .getFactory()
              .getRouteFactories()
              .createRoute(NetworkRoute.class, links.get(0).getId(), links.get(4).getId());
      route.setLinkIds(
          links.get(0).getId(),
          List.of(links.get(1).getId(), links.get(2).getId(), links.get(3).getId()),
          links.get(4).getId());
      leg.setRoute(route);
  ...
  public void testStorageCapacityDownscaling() {
    ...
    Assert.assertEquals(
        1,
        counter3
            .currentMax); // the bottleneck link can store 14 vehicles, but one vehicle counts for
    // 10, so only 1 works
    ...
    for (int i = 1; i <= 500; i++) {
      ...
      if (i % 2 == 1) {
        ...
        VehicleUtils.insertVehicleIdsIntoAttributes(
            person, Map.of(TransportMode.car, vehicle.getId()));
      }
      if (i % 2 == 0) {
  ...
  /**
   * Tests that the storage capacity can be reached (but not exceeded) by agents driving over a
   * link, changing vehicles in between. trip is: car -> walk back -> truck for all agents
   *
   * @author jfbischoff
   */
  @Test
  public void testStorageCapacityWithVaryingPCUs() {
    ...
    for (int i = 1; i <= 500; i++) {
      ...
      Route route2 =
          new GenericRouteFactory().createRoute(links.get(4).getId(), links.get(0).getId());
      ...
      Vehicle tractorv =
          VehicleUtils.createVehicle(
              Id.createVehicleId("truck_" + person.getId().toString()), tractor);
      scenario.getVehicles().addVehicle(tractorv);
      VehicleUtils.insertVehicleIdsIntoAttributes(
          person, Map.of(TransportMode.truck, tractorv.getId()));
  ...
  private List<Link> generateNetwork(Network network) {
    ...
    var node3 =
        NetworkUtils.createAndAddNode(network, Id.create("3", Node.class), new Coord(850, 0));
    var node4 =
        NetworkUtils.createAndAddNode(network, Id.create("4", Node.class), new Coord(1600, 0));
    var node5 =
        NetworkUtils.createAndAddNode(network, Id.create("5", Node.class), new Coord(1700, 0));
    var node6 =
        NetworkUtils.createAndAddNode(network, Id.create("6", Node.class), new Coord(1800, 0));

    var link1 =
        NetworkUtils.createAndAddLink(
            network, Id.create("1", Link.class), node1, node2, 105, 100, 3600, 1);
    var link2 =
        NetworkUtils.createAndAddLink(
            network, Id.create("2", Link.class), node2, node3, 750, 100, 3600, 1);
    var link3 =
        NetworkUtils.createAndAddLink(
            network, Id.create("3", Link.class), node3, node4, 750, 100, 3600, 1);
    var link4 =
        NetworkUtils.createAndAddLink(
            network, Id.create("4", Link.class), node4, node5, 105, 100, 360, 1);
    var link5 =
        NetworkUtils.createAndAddLink(
            network, Id.create("5", Link.class), node5, node6, 105, 100, 3600, 1);
