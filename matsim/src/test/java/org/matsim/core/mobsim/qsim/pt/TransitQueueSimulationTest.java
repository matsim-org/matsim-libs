...
public class TransitQueueSimulationTest {
  ...
  public void testCreateAgents() {
    ...
    Node node3 =
        network
            .getFactory()
            .createNode(Id.create("3", Node.class), new Coord((double) 2000, (double) 0));
    ...
    TransitStopFacility stop3 =
        builder.createTransitStopFacility(
            Id.create("stop3", TransitStopFacility.class),
            new Coord((double) 0, (double) 0),
            false);
    TransitStopFacility stop4 =
        builder.createTransitStopFacility(
            Id.create("stop4", TransitStopFacility.class),
            new Coord((double) 0, (double) 0),
            false);
    ...
    { // line 1, 1 route, 2 departures
      ...
      TransitRoute tRoute =
          builder.createTransitRoute(
              Id.create(">", TransitRoute.class), route, stops, TransportMode.pt);
    ...
    { // line 2, 3 routes, each 1 departure
      ...
      { // route 1
        TransitRoute tRoute =
            builder.createTransitRoute(
                Id.create("A", TransitRoute.class), route, stops, TransportMode.pt);
      ...
      { // route 2
        TransitRoute tRoute =
            builder.createTransitRoute(
                Id.create("B", TransitRoute.class), route, stops, TransportMode.pt);
      ...
      { // route 3
        TransitRoute tRoute =
            builder.createTransitRoute(
                Id.create("C", TransitRoute.class), route, stops, TransportMode.pt);
    ...
    Collections.sort(
        agents,
        new Comparator<MobsimAgent>() {
          @Override
          public int compare(MobsimAgent mobsimAgent, MobsimAgent mobsimAgent1) {
            return Double.compare(
                mobsimAgent.getActivityEndTime(), mobsimAgent1.getActivityEndTime());
          }
        });
  ...
  /**
   * Tests that the simulation refuses to let an agent teleport herself by starting a transit leg on
   * a link where she isn't.
   */
  ...
  /**
   * Tests that a vehicle's handleStop() method is correctly called, e.g. it is re-called again when
   * returning a delay > 0, and that is is correctly called when the stop is located on the first
   * link of the network route, on the last link of the network route, or any intermediary link.
   */
  @Test
  public void testHandleStop() {
    ...
    Node node4 =
        network
            .getFactory()
            .createNode(Id.create("4", Node.class), new Coord((double) 3000, (double) 0));
    Node node5 =
        network
            .getFactory()
            .createNode(Id.create("5", Node.class), new Coord((double) 4000, (double) 0));
    Node node6 =
        network
            .getFactory()
            .createNode(Id.create("6", Node.class), new Coord((double) 5000, (double) 0));
    network.addNode(node1);
    network.addNode(node2);
    network.addNode(node3);
    ...
    stop1.setLinkId(
        link1.getId()); // one stop on the first link of network route, as that one may be specially
    // handled
    stop2.setLinkId(link3.getId()); // some stop in the middle of the network route
    stop3.setLinkId(link4.getId());
    stop4.setLinkId(
        link5.getId()); // one stop on the last link of the network route, as that one may be
    // specially handled
    ...
    // run simulation
    EventsManager events = EventsUtils.createEventsManager();
    TestHandleStopSimulation simulation =
        TestHandleStopSimulation.createTestHandleStopSimulation(
            scenario, events, line, tRoute, departure);
    simulation.run();
  ...
  protected static class TestHandleStopSimulation {
    ...
    private TestHandleStopSimulation(
          ...
          new AgentSource() {
            ...
            public void insertAgentsIntoMobsim() {
              TestHandleStopSimulation.this.driver =
                  new SpyDriver(
                      TestHandleStopSimulation.this.line,
                      TestHandleStopSimulation.this.route,
                      TestHandleStopSimulation.this.departure,
                      transitEngine.getAgentTracker(),
                      transitEngine);

              VehicleType vehicleType =
                  VehicleUtils.createVehicleType(
                      Id.create("transitVehicleType", VehicleType.class));
              ...
              TransitQVehicle veh =
                  new TransitQVehicle(
                      VehicleUtils.createVehicle(
                          Id.create(TestHandleStopSimulation.this.driver.getId(), Vehicle.class),
                          vehicleType));
    ...
    protected static TestHandleStopSimulation createTestHandleStopSimulation(
        final MutableScenario scenario,
        final EventsManager events,
        final TransitLine line,
        final TransitRoute route,
  ...
  private static class SpyHandleStopData {
    private static final Logger log =
        LogManager.getLogger(TransitQueueSimulationTest.SpyHandleStopData.class);

    public final TransitStopFacility stopFacility;
    public final double time;
    public final double returnedDelay;

    protected SpyHandleStopData(
        final TransitStopFacility stopFacility, final double time, final double returnedDelay) {
  ...
  public void testStartAndEndTime() {
    ...
    TransitStopFacility stopFacility2 =
        sb.createTransitStopFacility(
            Id.create("2", TransitStopFacility.class), new Coord((double) 2000, (double) 0), false);
    ...
    TransitRouteStop stop1 =
        sb.createTransitRouteStopBuilder(stopFacility1).departureOffset(0.0).build();
    TransitRouteStop stop2 = sb.createTransitRouteStop(stopFacility2, 100.0, 100.0);
    List<TransitRouteStop> stops = new ArrayList<>(2);
    stops.add(stop1);
    stops.add(stop2);
    TransitRoute tRoute =
        sb.createTransitRoute(Id.create("1", TransitRoute.class), route, stops, "bus");
  ...
  public void testEvents() {
    ...
    assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((ActivityStartEvent) allEvents.get(idx)).getActType());
    assertTrue(
        allEvents.get(++idx)
            instanceof
            ActivityEndEvent); // zero activity duration, waiting at stop is considered as leg
    assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((ActivityEndEvent) allEvents.get(idx)).getActType());
    assertTrue(allEvents.get(++idx) instanceof PersonDepartureEvent);
