...
//        vehicleType.setCapacity(capacity);
	...
	void testAddAgentToStop() {
		...
		// use the assert construct to verify the number of agents in the agent tracker. We must test
		// this before the simulation cleans up its state, as the agents in the tracker are deleted
		// during cleanup.
		var module = new AbstractModule() {
			@Override
			public void install() {
				var assertAgents = new AssertAddAgentToStop(stop1.getId(), 1);
				bind(AssertAddAgentToStop.class).toInstance(assertAgents);
				addMobsimListenerBinding().toInstance(assertAgents);
			}
		};
	...
	static class AssertAddAgentToStop implements MobsimBeforeCleanupListener {

		private final int agentsInTracker;
		private final Id<TransitStopFacility> stop;

		@Inject
		private Injector injector;

		AssertAddAgentToStop(Id<TransitStopFacility> stop, int expectedNumAgents) {
			this.stop = stop;
			this.agentsInTracker = expectedNumAgents;
		}

		@Override
		public void notifyMobsimBeforeCleanup(MobsimBeforeCleanupEvent e) {
			var qsim = (QSim) e.getQueueSimulation();
			var engine = qsim.getChildInjector().getInstance(TransitQSimEngine.class);
			assertEquals(agentsInTracker, engine.getAgentTracker().getAgentsAtFacility(stop).size());
		}
	}
