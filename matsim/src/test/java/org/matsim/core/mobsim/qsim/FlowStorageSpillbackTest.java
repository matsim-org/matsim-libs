...
/**
 * Tests the flow capacity for two vehicles that are leaving a link when the storage capacity on the
 * next link (downstream) is reached.
 *
 * @author ikaddoura
 */
...
public class FlowStorageSpillbackTest {
  ...
  public final void testFlowCongestion() {
    ...
    for (LinkLeaveEvent event : linkLeaveEvents) {
      ...
      if (event.getVehicleId().equals(vehicleOfPerson.get(this.testAgent4))
          && event.getLinkId().equals(this.linkId2)) {
        //				if(this.isUsingFastCapacityUpdate) {
        Assert.assertEquals(
            "wrong link leave time.", 169., event.getTime(), MatsimTestUtils.EPSILON);
        //				} else {
        //					Assert.assertEquals("wrong link leave time.", 170., event.getTime(),
