  ...
  public final void testMaterializeAfterReadParameterSets() {
    {
      ...
      TestConfigGroup configGroup = ConfigUtils.addOrGetModule(config, TestConfigGroup.class);
      for (int i = 0; i < 2; i++) {
      ...
      Assert.assertEquals(
          ...
          2,
          configGroup.getParameterSets("blabla").size());
...

    {
      ...
      Config config =
          ConfigUtils.loadConfig(new String[] {utils.getOutputDirectory() + "ad-hoc-config.xml"});
      ...
      for (TestParameterSet set :
          (Iterable<TestParameterSet>) configGroup.getParameterSets("blabla")) {
        Assert.assertEquals(
            "unexpected value for parameter in parameter set",
            "life is wonderful",
            set.getParameter());
      }
    }
