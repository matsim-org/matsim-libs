...
public class ConfigV2IOTest {
  ...
  private void assertTheSame(final Config outConfig, final Config inConfig) {
    Assert.assertEquals(
    ...
    for (String name : outConfig.getModules().keySet()) {
      if (!name.equals(ChangeLegModeConfigGroup.CONFIG_MODULE)) {
        ...
        assertTheSame(outConfig.getModule(name), inConfig.getModule(name));
      }
    }
  }

  private void assertTheSame(final ConfigGroup outModule, final ConfigGroup inModule) {
    Assert.assertEquals("wrong module class", outModule.getClass(), inModule.getClass());

    Assert.assertEquals("different parameters", outModule.getParams(), inModule.getParams());
