...
public class ReplanningConfigGroupTest {
  ...
  public void testParamNames() {
    ...
    assertEquals(
        "unexpected number of strategy settings", 1, configGroup.getStrategySettings().size());
  }

  /**
   * Tests that inconsistent configuration states are recognized, like missing settings or wrong
   * enumeration.
   *
   * @author mrieser
   */
  ...
  private ConfigGroup toUnderscoredModule(final ReplanningConfigGroup initialGroup) {
    // yyyy is this method/this execution path still necessary?  Maybe we need to be able to read
    // config v1, but certainly
