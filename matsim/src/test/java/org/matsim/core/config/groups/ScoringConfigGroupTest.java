...
public class ScoringConfigGroupTest {
  private static final Logger log = LogManager.getLogger(ScoringConfigGroupTest.class);
  ...
  private void testResultsBeforeCheckConsistency(Config config, boolean fullyHierarchical) {
    ...
    if (!fullyHierarchical) {
      ...
      Assert.assertNotNull(
          scoringConfig.getActivityParams(createStageActivityType(TransportMode.pt)));
      Assert.assertNotNull(
          scoringConfig.getActivityParams(createStageActivityType(TransportMode.drt)));
    }
  ...
  public void testFullyHierarchicalVersion() {
    Config config =
        ConfigUtils.loadConfig(utils.getClassInputDirectory() + "config_v2_w_scoringparams.xml");
    ScoringConfigGroup scoringConfig = config.scoring();
  ...
  public void testVersionWoScoringparams() {
    Config config =
        ConfigUtils.loadConfig(utils.getClassInputDirectory() + "config_v2_wo_scoringparams.xml");
    ScoringConfigGroup scoringConfig = config.scoring();
  ...
  private void assertIdentical(
    ...
    for (ActivityParams initialSettings : initialGroup.getActivityParams()) {
      final ActivityParams inputSettings =
          inputConfigGroup.getActivityParams(initialSettings.getActivityType());
      Assert.assertEquals(
          "wrong type " + msg, initialSettings.getActivityType(), inputSettings.getActivityType());
      Assert.assertEquals(
  ...
  private static ConfigGroup toUnderscoredModule(final ScoringConfigGroup initialGroup) {
    ...
    for (ActivityParams settings : initialGroup.getActivityParams()) {
      ...
      settings
          .getClosingTime()
          .ifDefined(t -> module.addParam("activityClosingTime_" + suffix, "" + t));
      settings
          .getEarliestEndTime()
          .ifDefined(t -> module.addParam("activityEarliestEndTime_" + suffix, "" + t));
      settings
          .getLatestStartTime()
          .ifDefined(t -> module.addParam("activityLatestStartTime_" + suffix, "" + t));
      settings
          .getMinimalDuration()
          .ifDefined(t -> module.addParam("activityMinimalDuration_" + suffix, "" + t));
      settings
          .getOpeningTime()
          .ifDefined(t -> module.addParam("activityOpeningTime_" + suffix, "" + t));
      module.addParam("activityPriority_" + suffix, "" + settings.getPriority());
      settings
          .getTypicalDuration()
          .ifDefined(t -> module.addParam("activityTypicalDuration_" + suffix, "" + t));
    }
    ...
    for (ModeParams settings : initialGroup.getModes().values()) {
      ...
      module.addParam(
          "marginalUtlOfDistance_" + mode, "" + settings.getMarginalUtilityOfDistance());
    ...
    for (Map.Entry<String, String> params :
        initialGroup.getScoringParameters(null).getParams().entrySet()) {
