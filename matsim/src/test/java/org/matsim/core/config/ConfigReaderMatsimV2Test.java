...
public class ConfigReaderMatsimV2Test {
  ...
  /**
   * Test that 2 aliases for the same parameter name resolve to different parameter names depending
   * on in which module they are in.
   */
  @Test
  ...
  public void testConditionalParamNameAlias3() {
    ...
    r2.readStream(
        bais); // if the alias were matched, it should produce an exception, as "input" is not known
  ...
  public void testAliasWithParamSets() {
    ...
    Assert.assertEquals(
        -5.6, config.scoring().getModes().get("car").getMarginalUtilityOfTraveling(), 1e-7);
    Assert.assertEquals(
        -8.7, config.scoring().getModes().get("unicycle").getMarginalUtilityOfTraveling(), 1e-7);
  }
}
