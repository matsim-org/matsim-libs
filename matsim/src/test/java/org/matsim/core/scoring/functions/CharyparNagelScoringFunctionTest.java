 ...
 * </blockquote>
 *
 * TODO dg march 09: when walk mode is tested add a walk mode leg and modify at least
 * testMarginalUtilityOfDistance
 *
 * <p>TODO [MR] split this into multiple test classes for the specific parts, according to the
 * newer, more modular scoring function
 *
 * @author mrieser
 */
...
public class CharyparNagelScoringFunctionTest {
  ...
  @Parameter public TypicalDurationScoreComputation typicalDurationComputation;
  ...
  /**
   * The reference implementation to calculate the zero utility duration, the duration of an
   * activity at which its utility is zero.
   *
  ...
  public void testTravelingBikeAndConstantBike() {
    ...
    f.config
        ...
        .setMarginalUtilityOfTraveling(travelingBike);
  ...
  public void testPerforming() {
    ...
    assertEquals(
        perf * 3.0 * Math.log(2.5 / zeroUtilDurW)
            ...
            + perf * 3.0 * Math.log(2.5 / zeroUtilDurW)
            + perf * 15.0 * Math.log(14.75 / zeroUtilDurH),
        calcScore(f),
        EPSILON);
  ...
  public void testOpeningTime() {
    ...
    wParams.setOpeningTime(
        8 * 3600.0); // now the agent arrives 30min early to the FIRST work activity and has to wait
  ...
  public void testClosingTime() {
    ...
    ActivityParams wParams = f.config.scoring().getActivityParams("w");
    wParams.setClosingTime(
        15 * 3600.0); // now the agent stays 1h too long at the LAST work activity
    double score = calcScore(f);
  ...
  /**
   * Test the performing part of the scoring function when an activity has OpeningTime and
   * ClosingTime set.
   */
  @Test
  public void testOpeningClosingTime() {
    ...
    final double zeroUtilityDuration_work_sec =
        3600. * getZeroUtilDuration_hrs(typicalDuration_work_sec / 3600., 1.);
    double slope_work_at_zero_utility_h =
        perf_hrs * typicalDuration_work_sec / zeroUtilityDuration_work_sec;
    ...
    // (note that even _some_ opening time causes zero score, since the minDuration needs to be
    // overcome!)
  ...
  public void testMinimumDuration() {
    ...
    ActivityParams wParams = f.config.scoring().getActivityParams("w");
    wParams.setMinimalDuration(
        3 * 3600.0); // require the agent to be 3 hours at every working activity
  ...
  public void testDistanceCostScoringCar() {
    ...
    f.config
        ...
        .setMonetaryDistanceRate(monetaryDistanceRateCar1);
  ...
  public void testDistanceCostScoringPt() {
    ...
    f.config
        ...
        .setMonetaryDistanceRate(monetaryDistanceRatePt1);
  ...
  public void testAddMoney() {
    ...
    Activity act1a =
        PopulationUtils.createAndAddActivityFromLinkId(
            plan1, "home", (Id<Link>) null); // , 0, 7.0*3600, 7*3600, false);
    act1a.setEndTime(f.secondLegStartTime);
    Leg leg1 =
        PopulationUtils.createAndAddLeg(plan1, TransportMode.car); // , 7*3600, 100, 7*3600+100);
    ...
    Activity act1b =
        PopulationUtils.createAndAddActivityFromLinkId(
            ...
            "work",
            (Id<Link>)
                null); // , 7.0*3600+100, Time.getUndefinedTime(), Time.getUndefinedTime(), false);
    ...
    public Fixture() {
      ...
      Node node1 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("1", Node.class), new Coord(0.0, 0.0));
      Node node2 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("2", Node.class), new Coord(500.0, 0.0));
      Node node3 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("3", Node.class), new Coord(5500.0, 0.0));
      Node node4 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("4", Node.class), new Coord(6000.0, 0.0));
      Node node5 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("5", Node.class), new Coord(11000.0, 0.0));
      Node node6 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("6", Node.class), new Coord(11500.0, 0.0));
      Node node7 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("7", Node.class), new Coord(16500.0, 0.0));
      Node node8 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("8", Node.class), new Coord(17000.0, 0.0));
      Node node9 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("9", Node.class), new Coord(22000.0, 0.0));
      Node node10 =
          NetworkUtils.createAndAddNode(
              this.network, Id.create("10", Node.class), new Coord(22500.0, 0.0));
      ...
      Activity firstActivity =
          PopulationUtils.createAndAddActivityFromLinkId(this.plan, "h", link1.getId());
      ...
      Activity secondActivity =
          PopulationUtils.createAndAddActivityFromLinkId(this.plan, "w", link3.getId());
      ...
      Activity thirdActivity =
          PopulationUtils.createAndAddActivityFromLinkId(this.plan, "w", link5.getId());
      ...
      Activity fourthActivity =
          PopulationUtils.createAndAddActivityFromLinkId(this.plan, "w", link7.getId());
      ...
      Activity fifthActivity =
          PopulationUtils.createAndAddActivityFromLinkId(this.plan, "h", link9.getId());
