...
public class EventsToLegsTest {
  ...
  public void testCreatesLeg() {
    ...
    eventsToLegs.handleEvent(
        new TeleportationArrivalEvent(
            30.0, Id.create("1", Person.class), 50.0, TransportMode.walk));
    eventsToLegs.handleEvent(
        new PersonArrivalEvent(
            30.0, Id.create("1", Person.class), Id.create("l2", Link.class), TransportMode.walk));
  ...
  public void testCreatesLegWithRoute() {
    ...
    eventsToLegs.handleEvent(
        new PersonDepartureEvent(10.0, agentId, Id.createLinkId("l1"), "car", "car"));
    eventsToLegs.handleEvent(new PersonEntersVehicleEvent(10.0, agentId, vehId));
    eventsToLegs.handleEvent(
        new VehicleEntersTrafficEvent(10.0, agentId, Id.createLinkId("l1"), vehId, "car", 1.0));
    ...
    eventsToLegs.handleEvent(
        new VehicleLeavesTrafficEvent(30.0, agentId, Id.createLinkId("l3"), vehId, "car", 1.0));
    eventsToLegs.handleEvent(new PersonArrivalEvent(30.0, agentId, Id.createLinkId("l3"), "car"));
    assertLeg(lh, 10., 20., 550.0, "car");
    Assert.assertEquals(
        EventsToLegs.ENTER_VEHICLE_TIME_ATTRIBUTE_NAME + " missing or incorrect!",
        10.0,
        lh.handledLeg
            .getLeg()
            .getAttributes()
  ...
  public void testCreatesLegWithRoute_jointTrip() {
    ...
    eventsToLegs.handleEvent(
        new PersonDepartureEvent(10.0, agentId1, Id.createLinkId("l1"), "car", "car"));
    ...
    eventsToLegs.handleEvent(
        new PersonDepartureEvent(10.0, agentId2, Id.createLinkId("l1"), "ride", "ride"));
  ...
  public void testCreatesLegWithRoute_withLeavingTrafficOnTheSameLink() {
    ...
    eventsToLegs.handleEvent(
        new PersonDepartureEvent(10.0, agentId1, Id.createLinkId("l1"), "car", "car"));
    eventsToLegs.handleEvent(new PersonEntersVehicleEvent(10.0, agentId1, vehId));
    // driver leaves out vehicle after 10 seconds of driving from one end to the other of the
    // initial link
    eventsToLegs.handleEvent(
  ...
  public void testCreatesTransitPassengerRoute() {
    ...
    Id<TransitStopFacility> accessFacilityId =
        Id.create("accessFacility", TransitStopFacility.class);
    TransitStopFacility accessFacility =
        scheduleFactory.createTransitStopFacility(accessFacilityId, new Coord(0.0, 0.0), false);
    ...
    NetworkRoute networkRoute =
        RouteUtils.createLinkNetworkRouteImpl(accessLinkId, Collections.emptyList(), accessLinkId);
    TransitRoute transitRoute =
        scheduleFactory.createTransitRoute(
            transitRouteId, networkRoute, Collections.emptyList(), "bus");
    ...
    eventsToLegs.handleEvent(
        new TransitDriverStartsEvent(
            0.0, transitDriverId, transitVehiceId, transitLineId, transitRouteId, departureId));
    eventsToLegs.handleEvent(new PersonDepartureEvent(10.0, passengerId, accessLinkId, "pt", "pt"));
    eventsToLegs.handleEvent(
        new VehicleArrivesAtFacilityEvent(50.0, transitVehiceId, accessFacilityId, 0.0));
  ...
  private static Scenario createTriangularNetwork() {
    ...
    Node node1 =
        NetworkUtils.createAndAddNode(
            network, Id.create("n1", Node.class), new Coord((double) 0, (double) 0));
    Node node2 =
        NetworkUtils.createAndAddNode(
            network, Id.create("n2", Node.class), new Coord((double) 50, (double) 100));
    Node node3 =
        NetworkUtils.createAndAddNode(
            network, Id.create("n3", Node.class), new Coord((double) 50, (double) 0));
    Node node4 =
        NetworkUtils.createAndAddNode(
            network, Id.create("n4", Node.class), new Coord((double) 100, (double) 0));
    final Node fromNode = node1;
    final Node toNode = node2;

    // add links
    NetworkUtils.createAndAddLink(
        network, Id.create("l1", Link.class), fromNode, toNode, 500.0, 10.0, 3600.0, (double) 1);
    final Node fromNode1 = node2;
    final Node toNode1 = node3;
    NetworkUtils.createAndAddLink(
        network, Id.create("l2", Link.class), fromNode1, toNode1, 500.0, 10.0, 3600.0, (double) 1);
    final Node fromNode2 = node3;
    final Node toNode2 = node4;
    NetworkUtils.createAndAddLink(
        network, Id.create("l3", Link.class), fromNode2, toNode2, 50.0, 0.1, 3600.0, (double) 1);
    final Node fromNode3 = node4;
    final Node toNode3 = node1;
    NetworkUtils.createAndAddLink(
        network, Id.create("l4", Link.class), fromNode3, toNode3, 50.0, 0.1, 3600.0, (double) 1);

    return scenario;
  }

  private void assertLeg(
      RememberingLegHandler lh,
    ...
    Assert.assertEquals(
        travelTime, lh.handledLeg.getLeg().getRoute().getTravelTime().seconds(), 1e-9);
