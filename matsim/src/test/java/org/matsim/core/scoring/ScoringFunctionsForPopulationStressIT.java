...
public class ScoringFunctionsForPopulationStressIT {
	...
	public void exceptionInScoringFunctionPropagates() {
		...
		events.processEvent(new PersonMoneyEvent(3600.0, personId, 3.4, "tollRefund", "ASFINAG"));
	...
	private void work(Config config) {
		...
		for (int i=0; i<MAX; i++) {
			events.processEvent(new PersonMoneyEvent(i*200, personId, 1.0, "tollRefund", "ASFINAG"));
