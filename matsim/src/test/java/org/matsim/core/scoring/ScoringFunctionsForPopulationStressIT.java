...
public class ScoringFunctionsForPopulationStressIT {
  ...
  public void exceptionInScoringFunctionPropagates() {
    ...
    scenario
        .getPopulation()
        .addPerson(scenario.getPopulation().getFactory().createPerson(personId));
    ...
    events.processEvent(
        new PersonMoneyEvent(3600.0, personId, 3.4, "tollRefund", "motorwayOperator"));
  ...
  private void work(Config config) {
    ...
    for (int i = 0; i < MAX; i++) {
      events.processEvent(
          new PersonMoneyEvent(i * 200, personId, 1.0, "tollRefund", "motorwayOperator", null));
      events.processEvent(
          new ActivityStartEvent(i * 200, personId, Id.createLinkId(0), null, "work", null));
      events.processEvent(
          new ActivityEndEvent(i * 200 + 100, personId, Id.createLinkId(0), null, "work", null));
      events.processEvent(
          new PersonDepartureEvent(i * 200 + 100, personId, Id.createLinkId(0), "car", "car"));
      events.processEvent(
          new PersonArrivalEvent(i * 200 + 190, personId, Id.createLinkId(0), "car"));
    }
    events.finishProcessing();
    scoringFunctionsForPopulation.finishScoringFunctions();

    // assert when TypicalDurationScoreComputation.uniform
    //		assertEquals(60.0 * MAX,
    // scoringFunctionsForPopulation.getScoringFunctionForAgent(personId).getScore(), 1.0);

    // assert when TypicalDurationScoreComputation.relative
    assertEquals(
        1.0 / 6.0 * MAX,
        scoringFunctionsForPopulation.getScoringFunctionForAgent(personId).getScore(),
        1.0);
  }
  ...
  */
  ...
  public void unlikelyTimingOfScoringFunctionStillWorks() {
    ...
    for (int i = 0; i < MAX; i++) {
      events.processEvent(
          new PersonMoneyEvent(i * 200, personId, 1.0, "tollRefund", "motorwayOperator"));
      events.processEvent(
          new ActivityStartEvent(i * 200, personId, Id.createLinkId(0), null, "work"));
      events.processEvent(
          new ActivityEndEvent(i * 200 + 100, personId, Id.createLinkId(0), null, "work"));
      events.processEvent(
          new PersonDepartureEvent(i * 200 + 100, personId, Id.createLinkId(0), "car", "car"));
      events.processEvent(
          new PersonArrivalEvent(i * 200 + 200, personId, Id.createLinkId(0), "car"));
    }
    events.finishProcessing();
    scoringFunctionsForPopulation.finishScoringFunctions();
