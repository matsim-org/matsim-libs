...
public class NetworkRoutingInclAccessEgressModuleTest {
  ...
  private static Scenario createScenario(Config config) {
    ...
    Node n2 =
        factory.createNode(Id.create("2", Node.class), new Coord(HOME.getX() + 50, HOME.getY()));
    Node n3 =
        factory.createNode(
            Id.create("3", Node.class),
            new Coord((WORK.getX() - HOME.getX()) / 2, HOME.getY() - 1000));
    Node n4 =
        factory.createNode(Id.create("4", Node.class), new Coord(WORK.getX() - 50, WORK.getY()));
    ...
    Link directButSlow =
        createLink(SLOW_BUT_DIRECT_LINK, n2, n4, 11000, SLOW_SPEED, modes, factory);
    network.addLink(directButSlow);

    // split the fast link into two pieces to form a triangle. Thus, one could visually examine the
    // separate links
    Link longButFast1 =
        createLink(FAST_BUT_LONGER_LINK + "-1", n2, n3, 6000, FAST_SPEED, modes, factory);
    network.addLink(longButFast1);
    Link longButFast2 =
        createLink(FAST_BUT_LONGER_LINK + "-2", n3, n4, 6000, FAST_SPEED, modes, factory);
  ...
  private Config createConfig() {
    ...
    ReplanningConfigGroup.StrategySettings replanning =
  ...
  private static VehicleType createVehicleType(
      String name, double maxVelocity, VehiclesFactory factory) {
  ...
  public void calcRoute_modeVehiclesFromVehiclesData_differentTypesTakeDifferentRoutes() {
    ...
    ScoringConfigGroup.ModeParams fastParams = new ScoringConfigGroup.ModeParams(FAST_MODE);
    ...
    // create
    VehicleType slowType =
        createVehicleType(SLOW_MODE, SLOW_SPEED, scenario.getVehicles().getFactory());
    VehicleType fastType =
        createVehicleType(FAST_MODE, FAST_SPEED, scenario.getVehicles().getFactory());
    ...
    Person slowPerson =
        createPerson(
            "slow-person", slowType.getId().toString(), scenario.getPopulation().getFactory());
    scenario.getPopulation().addPerson(slowPerson);
    Person fastPerson =
        createPerson(
            "fast-person", fastType.getId().toString(), scenario.getPopulation().getFactory());
    scenario.getPopulation().addPerson(fastPerson);

    // set up a controler with individual main mode identifier at the point of this writing the
    // default one seems broken
  ...
  public void calcRoute_defaultVehicle_defaultVehicleIsAssigned() {
    ...
    for (Id<Vehicle> vehicleId : modeId.values()) {
      ...
      assertEquals(
          ...
          scenario.getVehicles().getVehicles().get(vehicleId).getType());
    }
  }
  ...
  public void useAccessEgressTimeFromLinkAttributes() {
    ...
    config
        .routing()
        .setAccessEgressType(RoutingConfigGroup.AccessEgressType.walkConstantTimeToLink);
    Scenario scenario = createScenario(config);
    NetworkUtils.setLinkAccessTime(
        scenario.getNetwork().getLinks().get(Id.createLinkId(START_LINK)), TransportMode.car, 75);
    NetworkUtils.setLinkEgressTime(
        scenario.getNetwork().getLinks().get(Id.createLinkId(END_LINK)), TransportMode.car, 180);
    // add persons
    Person person =
        createPerson("slow-person", TransportMode.car, scenario.getPopulation().getFactory());
    scenario.getPopulation().addPerson(person);
  ...
  public void useAccessEgressTimeFromConstantAndWalkTime() {
    ...
    config
        .routing()
        .setAccessEgressType(
            RoutingConfigGroup.AccessEgressType.accessEgressModeToLinkPlusTimeConstant);
    Scenario scenario = createScenario(config);
    NetworkUtils.setLinkAccessTime(
        scenario.getNetwork().getLinks().get(Id.createLinkId(START_LINK)), TransportMode.car, 75);
    NetworkUtils.setLinkEgressTime(
        ...
        new AbstractModule() {
          ...
          public void install() {
            addEventHandlerBinding()
                .toInstance(
                    new PersonDepartureEventHandler() {
                      @Override
  ...
  /**
   * returns the mode of the first leg in a plan, which is not a walk (walk is now used as
   * access/egress mode to network modes, except if walk is routed on the network as network mode).
   */
