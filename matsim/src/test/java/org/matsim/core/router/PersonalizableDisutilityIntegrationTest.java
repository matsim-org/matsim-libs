...
public class PersonalizableDisutilityIntegrationTest {
  ...
  public void testPersonAvailableForDisutility_SpeedyALT() {
    ...
    LeastCostPathCalculator router =
        routerFactory.createPathCalculator(f.network, f.costFunction, new FreeSpeedTravelTime());
    router.calcLeastCostPath(
        f.network.getNodes().get(Id.create("2", Node.class)),
        f.network.getNodes().get(Id.create("1", Node.class)),
        07 * 3600,
  ...
  private static class Fixture {
    ...
    public Fixture() {
      ...
      new MatsimNetworkReader(this.scenario.getNetwork())
          .readFile("test/scenarios/equil/network.xml");

      this.person = PopulationUtils.getFactory().createPerson(Id.create(1, Person.class));

      Id<Vehicle> dummyId = Id.createVehicleId("dummy");
      VehicleType dummyType =
          VehicleUtils.createVehicleType(Id.create("dummyVehicleType", VehicleType.class));
