...
public class RandomizingTimeDistanceTravelDisutilityTest {
  ...
  public void testRoutesForDifferentSigmas() {

    {
      ...
      Assert.assertEquals(
          "There should only be a single route in the sigma = 0 case.", 1, routes.size());
    }
  ...
  public NetworkRoute computeRoute(double sigma) {
    ...
    RandomizingTimeDistanceTravelDisutilityFactory factory =
    ...
    LeastCostPathCalculator router =
        TripRouterFactoryBuilderWithDefaults.createDefaultLeastCostPathCalculatorFactory(f.s)
            .createPathCalculator(f.s.getNetwork(), disutility, travelTime);
    ...
    final NetworkRoutingModule routingModule =
        new NetworkRoutingModule(
            TransportMode.car, f.s.getPopulation().getFactory(), f.s.getNetwork(), router);
