...
public class NetworkRoutingModuleTest {
	...
	void testRouteLeg() {
		...
		Assertions.assertEquals(200.0, leg.getTravelTime().seconds(), 1e-8);
	...
	void testRouteLegWithDistance() {
...

		{
			...
			f.s.getConfig().scoring().getModeParams().get(TransportMode.car).setMonetaryDistanceRate(monetaryDistanceRateCar);
