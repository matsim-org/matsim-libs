...
public class InvertertedNetworkRoutingTest {
  ...
  public void testInvertedNetworkLegRouter() {
    ...
    Network invertedNetwork =
        new NetworkInverter(
                f.s.getNetwork(), new NetworkTurnInfoBuilder(f.s).createAllowedTurnInfos())
            .getInvertedNetwork();
    InvertedLeastPathCalculator calc =
        InvertedLeastPathCalculator.create(lcpFactory, tc, f.s.getNetwork(), invertedNetwork, tt);
    LinkToLinkRoutingModule router =
        new LinkToLinkRoutingModule(
            "mode", f.s.getPopulation().getFactory(), f.s.getNetwork(), invertedNetwork, calc);
  ...
  private NetworkRoute calcRoute(
      LinkToLinkRoutingModule router,
    ...
    Leg leg =
        (Leg)
            router
                .calcRoute(
                    DefaultRoutingRequest.withoutAttributes(fromFacility, toFacility, 0.0, person))
                .get(0);
