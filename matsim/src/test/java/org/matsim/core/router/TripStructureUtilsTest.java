...
public class TripStructureUtilsTest {
  ...
  public void createSimpleFixture() {
    ...
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("breakfast", linkId));
    ...
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("snack", linkId));
    ...
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("lunch", linkId));
    ...
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("tea", linkId));
    ...
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("dinner", linkId));

    fixtures.add(new Fixture("strict alternation", plan, nActs, nLegs, nTrips));
  }
  ...
  public void createFixtureWithComplexTrips() {
    ...
    nLegs++;
    plan.addLeg(populationFactory.createLeg("some mode"));
    plan.addActivity(populationFactory.createActivityFromLinkId(dummyType, linkId));
    nLegs++;
    ...
    fixtures.add(new Fixture("complex trips", plan, nActs, nLegs, nTrips));
  }
  ...
  public void createFixtureWithSuccessiveActivities() {
    ...
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("fry eggs", linkId));
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("breakfast", linkId));
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("wash dishes", linkId));
    ...
    plan.addActivity(populationFactory.createActivityFromLinkId("lunch", linkId));
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("coffee", linkId));
    ...
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("read", linkId));
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("dinner", linkId));
    nActs++;
    plan.addActivity(populationFactory.createActivityFromLinkId("sleep", linkId));

    fixtures.add(new Fixture("successive activities", plan, nActs, nLegs, nTrips));
  }
  ...
  public void createFixtureWithAccessEgress() {
    ...
    plan.addActivity(
        populationFactory.createActivityFromLinkId(
            TripStructureUtils.createStageActivityType("access mode"), linkId));
    ...
    plan.addActivity(
        populationFactory.createActivityFromLinkId(
            TripStructureUtils.createStageActivityType("main mode"), linkId));
    ...
    for (Fixture fixture : fixtures) {
      final List<Activity> acts =
          TripStructureUtils.getActivities(
              fixture.plan, StageActivityHandling.ExcludeStageActivities);
  ...
  public void testTrips() throws Exception {
    for (Fixture fixture : fixtures) {
      final List<Trip> trips = TripStructureUtils.getTrips(fixture.plan);
      ...
      for (Trip trip : trips) {
        ...
        final int indexOfStart = fixture.plan.getPlanElements().indexOf(trip.getOriginActivity());
        final int indexOfEnd =
            fixture.plan.getPlanElements().indexOf(trip.getDestinationActivity());
        final List<PlanElement> inPlan =
            fixture.plan.getPlanElements().subList(indexOfStart + 1, indexOfEnd);
  ...
  public void testNPEWhenLocationNullInSubtourAnalysis() {
    ...
    // link ids are null
    plan.addActivity(
        populationFactory.createActivityFromCoord("type", new Coord((double) 0, (double) 0)));
  ...
  public void testSubtourCoords() {
    ...
    // distance between 10,10 and 0,0 is sqrt(200), above this threshold there will be one more
    // subtour
  ...
  public void testFindTripAtPlanElement() {
    ...
    }
    {
      ...
      {
        Trip trip =
            TripStructureUtils.findTripAtPlanElement(
                leg, f0.plan, TripStructureUtils::isStageActivityType);
        log.info("");
        log.info("Trip=");
        for (PlanElement tripElement : trip.getTripElements()) {
          log.info(tripElement);
      ...
      }
      {
        Trip trip =
            TripStructureUtils.findTripAtPlanElement(
                leg, f0.plan, TripStructureUtils.createStageActivityType(leg.getMode())::equals);
        log.info("");
        log.info("Trip=");
        for (PlanElement tripElement : trip.getTripElements()) {
          log.info(tripElement);
