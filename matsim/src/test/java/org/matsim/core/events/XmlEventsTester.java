...
/**
 * An abstract class providing static methods to verify that events are correctly handled with the
 * XML-events-file-format.
 *
 * @author mrieser
 */
public abstract class XmlEventsTester {
  ...
  /**
   * Writes out the given event to the specified file in the XML format, then reads the file again
   * and makes sure the freshly read in event has the same attributes as the original event.
   *
  ...
  public static <T extends Event> T testWriteReadXml(final String eventsFile, final T event) {
    ...
    for (Map.Entry<String, String> attribute : writtenAttributes.entrySet()) {
      assertEquals(
          "attribute '" + attribute.getKey() + "' is different after reading the event.",
          attribute.getValue(),
          readAttributes.get(attribute.getKey()));
    }
