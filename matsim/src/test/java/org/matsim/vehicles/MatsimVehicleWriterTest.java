...
public class MatsimVehicleWriterTest {
  ...
  public void testWriter() throws FileNotFoundException, IOException {
    {
      ...
      VehicleType vehType =
          vehicles.getVehicleTypes().get(Id.create("normal&Car", VehicleType.class));
  ...
  private void checkContent(Vehicles vehdef) {
    ...
    assertEquals(
        23.23, vehTypeDefaultCar.getCapacity().getVolumeInCubicMeters(), MatsimTestUtils.EPSILON);
    assertNotNull(vehTypeDefaultCar.getEngineInformation());
    assertEquals(
        EngineInformation.FuelType.diesel,
    ...
    assertEquals(
        ...
        VehicleUtils.getDoorOperationMode(vehTypeDefaultCar));
