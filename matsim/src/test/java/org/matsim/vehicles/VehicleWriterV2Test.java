...
public class VehicleWriterV2Test {
  ...
  public void test_VehicleTypeValuesAreReadCorrectly_normalCar() {
    ...
    assertEquals(
        23.23, vehTypeNormalCar.getCapacity().getVolumeInCubicMeters(), MatsimTestUtils.EPSILON);
    ...
    assertEquals(
        100, vehTypeNormalCar.getCostInformation().getFixedCosts(), MatsimTestUtils.EPSILON);
    assertEquals(
        0.15, vehTypeNormalCar.getCostInformation().getCostsPerMeter(), MatsimTestUtils.EPSILON);
    assertEquals(
        0.08, vehTypeNormalCar.getCostInformation().getCostsPerSecond(), MatsimTestUtils.EPSILON);
    assertEquals(
        0.06,
        VehicleUtils.getCostsPerSecondWaiting(vehTypeNormalCar.getCostInformation()),
    ...
    assertEquals(
        1.3,
        (double) vehTypeNormalCar.getAttributes().getAttribute("Attribute2"),
        MatsimTestUtils.EPSILON);
    ...
    assertEquals(
        ...
        VehicleUtils.getDoorOperationMode(vehTypeNormalCar));
  }

  @Test
  public void test_VehicleTypeValuesAreReadCorrectly_defaultCar() {
    VehicleType vehTypeDefaultCar =
        vehicleTypes.get(Id.create("defaultValue>Car", VehicleType.class));
    ...
    assertEquals(
        VehicleType.DoorOperationMode.serial, VehicleUtils.getDoorOperationMode(vehTypeDefaultCar));
  ...
  public void test_VehicleTypeValuesAreReadCorrectly_smallTruck() {
    ...
    assertEquals(
        "diesel", VehicleUtils.getHbefaTechnology(vehTypeSmallTruck.getEngineInformation()));
    assertEquals(
        "EURO-6", VehicleUtils.getHbefaEmissionsConcept(vehTypeSmallTruck.getEngineInformation()));
    assertEquals(
        100.0, vehTypeSmallTruck.getCostInformation().getFixedCosts(), MatsimTestUtils.EPSILON);
    assertEquals(
        0.2, vehTypeSmallTruck.getCostInformation().getCostsPerMeter(), MatsimTestUtils.EPSILON);
    assertEquals(
        0.10, vehTypeSmallTruck.getCostInformation().getCostsPerSecond(), MatsimTestUtils.EPSILON);
    assertEquals(
        0.05,
        VehicleUtils.getCostsPerSecondWaiting(vehTypeSmallTruck.getCostInformation()),
        MatsimTestUtils.EPSILON);
    assertEquals(
        0.15,
        VehicleUtils.getCostsPerSecondInService(vehTypeSmallTruck.getCostInformation()),
        MatsimTestUtils.EPSILON);
  }
  ...
  public void test_VehicleTypeToVehiclesAssignmentIsReadCorrectly() {
    ...
    assertEquals(
        Id.create("defaultValue>Car", VehicleType.class), vehicles.get(id42).getType().getId());
