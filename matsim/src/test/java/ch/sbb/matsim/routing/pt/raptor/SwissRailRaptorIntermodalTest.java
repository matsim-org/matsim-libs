...
public class SwissRailRaptorIntermodalTest {
  ...
  public void testIntermodalTrip() {
    ...
    routingModules.put(
        TransportMode.bike, new TeleportationRoutingModule(TransportMode.bike, f.scenario, 3, 1.4));
    ...
    SwissRailRaptorData data =
        SwissRailRaptorData.create(
            ...
            null);
    DefaultRaptorStopFinder stopFinder =
        new DefaultRaptorStopFinder(new DefaultRaptorIntermodalAccessEgress(), routingModules);
    SwissRailRaptor raptor =
        new SwissRailRaptor.Builder(data, f.scenario.getConfig()).with(stopFinder).build();
    ...
    List<? extends PlanElement> legs =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac, toFac, 7 * 3600, f.dummyPerson));
  ...
  public void testIntermodalTrip_TripRouterIntegration() {
    ...
    RoutingModule walkRoutingModule =
        new TeleportationRoutingModule(TransportMode.walk, f.scenario, 1.1, 1.3);
    RoutingModule bikeRoutingModule =
    ...
    TripRouter.Builder tripRouterBuilder =
        new TripRouter.Builder(f.config)
    ...
    RoutingModule ptRoutingModule =
        new SwissRailRaptorRoutingModule(
            raptor, f.scenario.getTransitSchedule(), f.scenario.getNetwork(), walkRoutingModule);
    ...
    List<? extends PlanElement> planElements =
        tripRouter.calcRoute(
            TransportMode.pt, fromFac, toFac, 7 * 3600, f.dummyPerson, new AttributesImpl());
    ...
    Assert.assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((Activity) planElements.get(1)).getType());
    Assert.assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((Activity) planElements.get(3)).getType());
    Assert.assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((Activity) planElements.get(5)).getType());
    Assert.assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((Activity) planElements.get(7)).getType());
  ...
  /**
   * Test that if start and end are close to each other, such that the intermodal access and egress
   * go to/from the same stop, still a direct transit_walk is returned.
   */
  @Test
  ...
  public void testDirectWalkFactor() {
    ...
    bikeAccess.setStopFilterValue("true");
    f.srrConfig.addIntermodalAccessEgress(bikeAccess);
    f.srrConfig.setIntermodalLegOnlyHandling(
  ...
  public void testAccessEgressModeFasterThanPt() {
    ...
    // returnNull
    data.config.setIntermodalLegOnlyHandling(
        SwissRailRaptorConfigGroup.IntermodalLegOnlyHandling.returnNull);
    List<? extends PlanElement> legs =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac, toFac, 7 * 3600, f.dummyPerson));
    Assert.assertNull(
    ...
    legs =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac2, toFac2, 7 * 3600, f.dummyPerson));
    Assert.assertNull(
        "The router should not find a route and return null, but did return something else.", legs);
    ...
    Assert.assertNotNull(
        "The router should find a pt route and not return null, but did return null", legs);
    Assert.assertTrue(
        legs.stream()
            .filter(Leg.class::isInstance)
            .anyMatch(planElement -> ((Leg) planElement).getMode().equals(TransportMode.pt)));

    legs =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac2, toFac2, 7 * 3600, f.dummyPerson));
    Assert.assertNull(
        "The router should not find a pt route and return null, but did return something else",
        legs);
    ...
    // allow
    data.config.setIntermodalLegOnlyHandling(
        SwissRailRaptorConfigGroup.IntermodalLegOnlyHandling.allow);
    legs =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac, toFac, 7 * 3600, f.dummyPerson));
    Assert.assertFalse(
        legs.stream()
            ...
            .anyMatch(planElement -> ((Leg) planElement).getMode().equals(TransportMode.pt)));
    Assert.assertNotNull(
        "The router should find a pt route and not return null, but did return something else.",
        legs);
  ...
  public void testIntermodalTrip_competingAccess() {
    ...
    Facility fromFac =
        new FakeFacility(new Coord(10500, 10050), Id.create("from", Link.class)); // stop 3
    Facility toFac =
        new FakeFacility(new Coord(50000, 10050), Id.create("to", Link.class)); // stop 5
    ...
    // do the test this way to insure it is not accidentally correct due to the accidentally correct
    // order the modes are initialized
    {
      routingModules.put(
          TransportMode.bike,
  ...
  // Checks RandomAccessEgressModeRaptorStopFinder. The desired result is that the StopFinder will
  ...
  public void testIntermodalTrip_RandomAccessEgressModeRaptorStopFinder() {
    ...
    bikeAccess.setSearchExtensionRadius(1);
    f.srrConfig.addIntermodalAccessEgress(bikeAccess);
    f.srrConfig.setIntermodalAccessEgressModeSelection(
        SwissRailRaptorConfigGroup.IntermodalAccessEgressModeSelection
...

    {
      ...
      for (int i = 0; i < 1000; i++) {
        ...
        { // Test 1: Checks whether the amount of legs is correct, whether the legs have the correct
          // modes,
          ...
          Assert.assertTrue(
              (leg.getMode().equals(TransportMode.bike))
                  || (leg.getMode().equals(TransportMode.walk)));
          Assert.assertEquals(Id.create("from", Link.class), leg.getRoute().getStartLinkId());
          Assert.assertEquals(Id.create("pt_3", Link.class), leg.getRoute().getEndLinkId());
          leg = (Leg) legs.get(1);
          Assert.assertEquals(TransportMode.pt, leg.getMode());
        ...
        { // Test 2: Counts all different access/egress mode combinations. The assertions occur
          // later.
          if ((((Leg) (legs.get(0))).getMode().equals(TransportMode.walk))
              && (((Leg) legs.get(2)).getMode().equals(TransportMode.walk))) numWalkWalk++;
          else if ((((Leg) (legs.get(0))).getMode().equals(TransportMode.walk))
              && (((Leg) legs.get(2)).getMode().equals(TransportMode.bike))) numWalkBike++;
          else if ((((Leg) (legs.get(0))).getMode().equals(TransportMode.bike))
              && (((Leg) legs.get(2)).getMode().equals(TransportMode.walk))) numBikeWalk++;
          else if ((((Leg) (legs.get(0))).getMode().equals(TransportMode.bike))
              && (((Leg) legs.get(2)).getMode().equals(TransportMode.bike))) numBikeBike++;
          else numOther++;
        }
      }

      { // Test 2: Tests whether Router chooses all 4 combinations of walk and bike. Also checks
        // that no other
  ...
  /**
   * Tests the following situation: two stops A and B close to each other, A has intermodal access,
   * B not. The route is fastest from B to C, with intermodal access to A and then transferring from
   * A to B. Make sure that in such cases the correct transit_walks are generated around stops A and
   * B for access to pt.
   */
  @Test
  public void testIntermodalTrip_accessTransfer() {
    ...
    Facility fromFac =
        new FakeFacility(new Coord(10000, 500), Id.create("from", Link.class)); // stop B or C
    ...
    SwissRailRaptorData data =
        SwissRailRaptorData.create(
            ...
            null);
    DefaultRaptorStopFinder stopFinder =
        new DefaultRaptorStopFinder(new DefaultRaptorIntermodalAccessEgress(), f.routingModules);
    SwissRailRaptor raptor =
        new SwissRailRaptor.Builder(data, f.scenario.getConfig()).with(stopFinder).build();

    List<? extends PlanElement> legs =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac, toFac, 8 * 3600 - 900, f.dummyPerson));
    for (PlanElement leg : legs) {
      System.out.println(leg);
    }
  ...
  /**
   * When using intermodal access/egress, transfers at the beginning are allowed to be able to
   * transfer from a stop with intermodal access/egress to another stop with better connections to
   * the destination. Earlier versions of SwissRailRaptor had a bug that resulted in only stops
   * where such transfers were possible to be used for route finding, but not stops directly
   * reachable and usable. This test tries to cover this case to make sure, route finding works as
  ...
  public void testIntermodalTrip_singleReachableStop() {
    ...
    f.srrConfig
        ...
        .removeIf(paramset -> paramset.getMode().equals("bike")); // we only want "walk" as mode

    Facility fromFac =
        new FakeFacility(
            new Coord(9800, 400),
            Id.create("from", Link.class)); // stops B or E, B is intermodal and triggered the bug
    Facility toFac =
        new FakeFacility(new Coord(20000, 5100), Id.create("to", Link.class)); // stop F
    ...
    List<? extends PlanElement> legs =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(
                fromFac, toFac, 7.5 * 3600 + 900, f.dummyPerson));
    for (PlanElement leg : legs) {
      System.out.println(leg);
    }
  ...
  public void testIntermodalTrip_egressTransfer() {
    ...
    Facility fromFac =
        new FakeFacility(new Coord(20000, 100), Id.create("from", Link.class)); // stop D
    Facility toFac =
        new FakeFacility(new Coord(10000, 500), Id.create("to", Link.class)); // stop B or C
  ...
  /**
   * If there is no pt stop within the search radius, the Raptor will assign a transit_walk route
   * from the fromFacility to the toFacility. In this case, the search radius is 500, but the
   * fromFacility is 600 away from stop A.
   */
  @Test
  public void testIntermodalTrip_noPtStopsInRadius() {
    ...
    List<? extends PlanElement> legs =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac, toFac, 7.5 * 3600, f.dummyPerson));
  ...
  /**
   * The agent is placed close to stop B, which is bike accessible. The agent is 600 meters away
   * from stop B, which puts it inside the Bike Radius for Access/Egress Mode but not in the Walk
   * radius. This test is meant to verify that the Swiss Rail Raptor will give the agent a
   * transit_walk route if all the applicable intermodal access/egress mode routers return null.
   * This will be shown in part 2 of this test.
   *
   * <p>Part 1 is mainly meant to check that the intermodal access/egress module is working
   * properly. Bike is given a fast speed, the test checks that the agent actually uses bike to get
   * to the pt Stop B.
   *
   * <p>Part 2 replaces the bike router. The bike router will be called, since the agent is within
   * the access/egress radius for mode bike and all the attributes are correct (as shown in part 1).
   * However, the router will return null, since we replaced it with a routing module which always
   * returns null. The expected outcome is that the Swiss Rail Raptor will return a transit_walk
   * between the fromFacility and the toFacility.
   */
  @Test
  public void testIntermodalTrip_accessModeRouterReturnsNull() {

    // Part 1: Bike is very fast. Bike Router will return intermodal route including bike, pt, and
    // walk.
    IntermodalTransferFixture f = new IntermodalTransferFixture();

    Facility fromFac =
        new FakeFacility(new Coord(9980, -600), Id.create("from", Link.class)); // stop B
    ...
    SwissRailRaptorData data2 =
        SwissRailRaptorData.create(
            ...
            f.scenario.getNetwork(),
            null);
    DefaultRaptorStopFinder stopFinder2 =
        new DefaultRaptorStopFinder(new DefaultRaptorIntermodalAccessEgress(), f.routingModules);
    SwissRailRaptor raptor2 =
        new SwissRailRaptor.Builder(data2, f.scenario.getConfig()).with(stopFinder2).build();

    List<? extends PlanElement> legs2 =
        raptor2.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac, toFac, 8 * 3600 - 900, f.dummyPerson));
  ...
  /**
   * The agent has a super-fast bike. So in theory it is faster to travel from stop_3 to the
   * destination. However, with the introduced trip share constraint it will still take the closest
   * stop accessible by the bike. If this constraint is not used the agent will take pt_3 stop as an
    ...
    routingModules.put(
        ...
        new RoutingModule() {
          ...
          public List<? extends PlanElement> calcRoute(RoutingRequest request) {
            ...
            double walkDistance =
                CoordUtils.calcEuclideanDistance(fromFacility.getCoord(), bikeCoord);
            ...
            Activity activity =
                populationFactory.createActivityFromLinkId(
                    "bike interaction", Id.createLinkId("pt_1"));
            ...
            bikeLeg.setDepartureTime(
                departureTime + bikeInteractionDuration + walkDistance / walkSpeed);
  ...
  /**
   * This test tests the intermodal router when access modes have interaction activities and it
   * tests the inclusion of the pt interaction activities by the SwissRailRaptorRoutingModule
   */
  @Test
  public void testIntermodalTrip_activityInteractionAdd() {
    ...
    SwissRailRaptor raptor =
        new SwissRailRaptor.Builder(data, f.scenario.getConfig()).with(stopFinder).build();
    SwissRailRaptorRoutingModule ssrrModule =
        new SwissRailRaptorRoutingModule(
            raptor,
            f.scenario.getTransitSchedule(),
            f.scenario.getNetwork(),
            routingModules.get("walk"));

    Facility fromFac = new FakeFacility(new Coord(9500, 10000), Id.create("from", Link.class));
    Facility toFac = new FakeFacility(new Coord(50000, 10500), Id.create("to", Link.class));

    List<? extends PlanElement> legs =
        ssrrModule.calcRoute(
            DefaultRoutingRequest.withoutAttributes(fromFac, toFac, 7 * 3600, f.dummyPerson));
    for (PlanElement leg : legs) {
      System.out.println(leg);
    }
  ...
  public void testIntermodalTripWithAccessAndEgressTimesAtStops() {
    ...
    f.scenario
        ...
        .forEach(
            stopFacility ->
                TransitScheduleUtils.setSymmetricStopAccessEgressTime(stopFacility, 120.0));
    ScoringConfigGroup.ModeParams walk = new ScoringConfigGroup.ModeParams(TransportMode.walk);
    walk.setMarginalUtilityOfTraveling(0.0);
    f.config.scoring().addModeParams(walk);
    ...
    public IntermodalTransferFixture() {
      ...
      TransitStopFacility stopA =
          sf.createTransitStopFacility(
              Id.create("A", TransitStopFacility.class), nodeA.getCoord(), false);
      TransitStopFacility stopB =
          sf.createTransitStopFacility(
              Id.create("B", TransitStopFacility.class), nodeB.getCoord(), false);
      TransitStopFacility stopC =
          sf.createTransitStopFacility(
              Id.create("C", TransitStopFacility.class), nodeC.getCoord(), false);
      TransitStopFacility stopD =
          sf.createTransitStopFacility(
              Id.create("D", TransitStopFacility.class), nodeD.getCoord(), false);
      TransitStopFacility stopE =
          sf.createTransitStopFacility(
              Id.create("E", TransitStopFacility.class), nodeE.getCoord(), false);
      TransitStopFacility stopF =
          sf.createTransitStopFacility(
              Id.create("F", TransitStopFacility.class), nodeF.getCoord(), false);
      ...
      NetworkRoute networkRouteAB =
          RouteUtils.createLinkNetworkRouteImpl(
              linkAA.getId(), new Id[] {linkAB.getId()}, linkBB.getId());
      ...
      TransitRoute redABRoute =
          sf.createTransitRoute(
              Id.create("redAB", TransitRoute.class), networkRouteAB, stopsRedAB, "train");
      redABRoute.addDeparture(sf.createDeparture(Id.create("1", Departure.class), 7.5 * 3600));
      redLine.addRoute(redABRoute);

      NetworkRoute networkRouteBA =
          RouteUtils.createLinkNetworkRouteImpl(
              linkBB.getId(), new Id[] {linkBA.getId()}, linkAA.getId());
      ...
      TransitRoute redBARoute =
          sf.createTransitRoute(
              Id.create("redBA", TransitRoute.class), networkRouteBA, stopsRedBA, "train");
      ...
      NetworkRoute networkRouteCD =
          RouteUtils.createLinkNetworkRouteImpl(
              linkCC.getId(), new Id[] {linkCD.getId()}, linkDD.getId());
      ...
      TransitRoute blueCDRoute =
          sf.createTransitRoute(
              Id.create("blueCD", TransitRoute.class), networkRouteCD, stopsBlueCD, "train");
      blueCDRoute.addDeparture(sf.createDeparture(Id.create("1", Departure.class), 8 * 3600));
      blueLine.addRoute(blueCDRoute);

      NetworkRoute networkRouteDC =
          RouteUtils.createLinkNetworkRouteImpl(
              linkDD.getId(), new Id[] {linkDC.getId()}, linkCC.getId());
      ...
      TransitRoute blueDCRoute =
          sf.createTransitRoute(
              Id.create("blueDC", TransitRoute.class), networkRouteDC, stopsBlueDC, "train");
      ...
      NetworkRoute networkRouteEF =
          RouteUtils.createLinkNetworkRouteImpl(
              linkEE.getId(), new Id[] {linkEF.getId()}, linkFF.getId());
      ...
      TransitRoute greenEFRoute =
          sf.createTransitRoute(
              Id.create("greenEF", TransitRoute.class), networkRouteEF, stopsGreenEF, "train");
      greenEFRoute.addDeparture(sf.createDeparture(Id.create("1", Departure.class), 8 * 3600));
      greenLine.addRoute(greenEFRoute);

      NetworkRoute networkRouteFE =
          RouteUtils.createLinkNetworkRouteImpl(
              linkDD.getId(), new Id[] {linkDC.getId()}, linkCC.getId());
      ...
      TransitRoute greenFERoute =
          sf.createTransitRoute(
              Id.create("greenFE", TransitRoute.class), networkRouteFE, stopsGreenFE, "train");
      ...
      this.dummyPerson =
          this.scenario.getPopulation().getFactory().createPerson(Id.create("dummy", Person.class));

      // ---

      this.routingModules = new HashMap<>();
      this.routingModules.put(
          TransportMode.walk,
          new TeleportationRoutingModule(TransportMode.walk, this.scenario, 1.1, 1.3));
      this.routingModules.put(
          TransportMode.bike,
          new TeleportationRoutingModule(
              TransportMode.bike, this.scenario, 10, 1.4)); // make bike very fast
