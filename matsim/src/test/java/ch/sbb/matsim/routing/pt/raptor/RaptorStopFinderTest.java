...
public class RaptorStopFinderTest {
  ...
  private final Facility fromFac =
      new FakeFacility(new Coord(0, 0), Id.create("AA", Link.class)); // stop A
  private final Facility toFac =
      new FakeFacility(new Coord(100000, 0), Id.create("XX", Link.class)); // stop X

  /**
   * Empty Initial Search Radius Tests the how the StopFinder reacts when there are no public
   * transit stops within the Initial_Search_Radius. The expected behaviour of the StopFinder is to
   * find the closest public transit stop and set the new search radius as the sum of the distance
   * to the nearest stop and the Search_Extension_Radius. However, if the general radius is smaller
   * than this new search radius, then the StopFinder will only search to the extents of the general
   * Radius.
   *
   * <p>This functionality is tested for the two RaptorStopFinders: 1) DefaultStopFinder and 2)
   * RandomAccessEgressModeRaptorStopFinder For each RaptorStopFinder there is one test where
   * StopFilterAttributes are not used to exlclude stops, and one test where StopFilterAttributes
   * are used.
   */
  @Test
  public void testDefaultStopFinder_EmptyInitialSearchRadius() {
...

    {
      ...
      SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet walkAccess =
      ...
      SwissRailRaptorData data =
          SwissRailRaptorData.create(
              f0.scenario.getTransitSchedule(),
              null,
              RaptorUtils.createStaticConfig(f0.config),
              f0.scenario.getNetwork(),
              null);
      ...
      SwissRailRaptor raptor =
          new SwissRailRaptor.Builder(data, f0.scenario.getConfig()).with(stopFinder).build();

      List<? extends PlanElement> legs =
          raptor.calcRoute(
              DefaultRoutingRequest.withoutAttributes(
                  this.fromFac, this.toFac, 7 * 3600, f0.dummyPerson));
     ...
     */
    {
      ...
      SwissRailRaptorData data =
          SwissRailRaptorData.create(
              f1.scenario.getTransitSchedule(),
              null,
              RaptorUtils.createStaticConfig(f1.config),
              f1.scenario.getNetwork(),
              null);
      DefaultRaptorStopFinder stopFinder =
          new DefaultRaptorStopFinder(new DefaultRaptorIntermodalAccessEgress(), routingModules);
      SwissRailRaptor raptor =
          new SwissRailRaptor.Builder(data, f1.scenario.getConfig()).with(stopFinder).build();

      List<? extends PlanElement> legs =
          raptor.calcRoute(
              DefaultRoutingRequest.withoutAttributes(
                  this.fromFac, this.toFac, 7 * 3600, f1.dummyPerson));
      for (PlanElement leg : legs) {
        System.out.println(leg);
      }
...

    {
      ...
      for (String stop : walkAccessibleStops) {
        f0.scenario
            ...
            .getAttributes()
            .putAttribute("walkAccessible", "true");
      }
  ...
  public void testRandomAccessEgressModeRaptorStopFinder_EmptyInitialSearchRadius() {
     ...
     */
    {
      ...
      walkAccess.setSearchExtensionRadius(1100);
      f1.srrConfig.addIntermodalAccessEgress(walkAccess);
      f1.srrConfig.setIntermodalAccessEgressModeSelection(
          SwissRailRaptorConfigGroup.IntermodalAccessEgressModeSelection
              .RandomSelectOneModePerRoutingRequestAndDirection);
      SwissRailRaptorData data =
          SwissRailRaptorData.create(
  ...
  @Test
  public void
      testRandomAccessEgressModeRaptorStopFinder_EmptyInitialSearchRadius_StopFilterAttributes() {
    /* Initial_Search_Radius and General Radius contain only stop B.
    Stop B is not "walkAccessible"; all other stops are "walkAccessible" Search_Extension_Radius is 0
    Expected: StopFinder will find no stops, since closest accessible stop is outside of general radius. Therefore
    agent will use transit_walk to get from A to X.
  ...
  /**
   * Half Full Initial Search Radius Tests the how the StopFinder reacts when there is only one
   * public transit stop within the Initial_Search_Radius. In this case, the Initial_Search_Radius
   * will always contain stop B, but no other stops. The expected behaviour of the StopFinder define
   * a new extended search radius, which is the sum of the distance to stop B and the
   * Search_Extension_Radius. However, if the general radius is smaller than this new search radius,
   * then the StopFinder will only search to the extents of the general Radius.
   *
   * <p>This functionality is tested for the two RaptorStopFinders: 1) DefaultStopFinder and 2)
   * RandomAccessEgressModeRaptorStopFinder For each RaptorStopFinder there is one test where
   * StopFilterAttributes are not used to exlclude stops, and one test where StopFilterAttributes
  ...
  public void testDefaultStopFinder_HalfFullInitialSearchRadius_StopFilterAttributes() {
     ...
     */
    {
      ...
      walkAccess.setMode(TransportMode.walk);
      walkAccess.setMaxRadius(10000000); // should not be limiting factor
      walkAccess.setInitialSearchRadius(
          1100); // Should include stops C and B (not "walkAccessible")
      walkAccess.setSearchExtensionRadius(0);
      walkAccess.setStopFilterAttribute("walkAccessible");
      walkAccess.setStopFilterValue("true");
      f0.srrConfig.addIntermodalAccessEgress(walkAccess);
  ...
  @Test
  public void
      testRandomAccessEgressModeRaptorStopFinder_HalfFullInitialSearchRadius_StopFilterAttributes() {
    /* Initial_Search_Radius includes B and C. Search_Extension_Radius is 0.
    General_Radius includes B, C, D and E
    Stop B is not "walkAccessible", but all others are.
    Expected: Stop Finder will only find stop C, since the Search_Extension_Radius doesn't encompass more stops.
  ...
  /**
   * Full Initial Search Radius Tests the how the StopFinder reacts when there are at least 2 stop
   * within the Initial_Search_Radius. In the following tests, the Initial_Search_Radius includes
   * stops B and C. The StopFinder should then not find stops outside of Initial_Search_Radius, even
   * if the Search_Extension_Radius is initialized. If the general Radius is smaller than the
   * Initial_Search_Radius, then the StopFinder should only search to the extents of the general
   * radius.
   *
   * <p>This functionality is tested for the two RaptorStopFinders: 1) DefaultStopFinder and 2)
   * RandomAccessEgressModeRaptorStopFinder For each RaptorStopFinder there is one test where
  ...
  @Test
  public void
      testRandomAccessEgressModeRaptorStopFinder_FullInitialSearchRadius_StopFilterAttributes() {
  ...
  public void testDefaultStopFinder_testMultipleModes() {
    ...
    // expected: D, since it is faster and it has correct attribute
    {
      ...
      f0.scenario
          .getTransitSchedule()
          .getFacilities()
          .get(Id.create("C", TransitStopFacility.class))
          .getAttributes()
          .putAttribute("zoomerAccessible", "true");
      f0.scenario
          .getTransitSchedule()
          .getFacilities()
          .get(Id.create("D", TransitStopFacility.class))
          .getAttributes()
          .putAttribute("zoomerAccessible", "true");
      ...
      routingModules.put(
          "zoomer", new TeleportationRoutingModule("zoomer", f0.scenario, 1000., 1.));
      ...
      f0.srrConfig.setUseIntermodalAccessEgress(true);
      SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet zoomerAccess =
          new SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet();
      ...
      SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet nonNetworkAccess =
          new SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet();
    ...
    // C and D have attribute "zoomerAccessible" as "true".
    // Search_Extension_Radius is 600, should include D if it extends based on C (which is nearest
    // stop with correct attribute)
    // General_Radius includes B, C, D, E
    // B and D are faster than C
    // expected: D, since it is faster and it has correct attribute
    {
    ...
    // Search_Extension_Radius includes D and E
    // General_Radius includes B, C, D, E
    // D and E are very fast, but shouldn't be chosen, since Initial_Search_Radius already has 2
    // entries
    // expected: B
    {
      StopFinderFixture f0 = new StopFinderFixture(10 * 60., 20 * 60., 1., 1.);
      ...
      f0.srrConfig.setUseIntermodalAccessEgress(true);
      SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet bikeAccess =
          new SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet();
      ...
      //            leg = legs.get(2);
      //            Assert.assertEquals(TransportMode.bike, leg.getMode());
      //            Assert.assertEquals(Id.create("XX", Link.class),
  ...
  private static class StopFinderFixture {
    ...
    public StopFinderFixture(double offsetB, double offsetC, double offsetD, double offsetE) {
      ...
      TransitStopFacility stopX =
          sf.createTransitStopFacility(
              Id.create("X", TransitStopFacility.class), nodeX.getCoord(), false);
      ...
      NetworkRoute networkRouteBX =
          RouteUtils.createLinkNetworkRouteImpl(
              linkBB.getId(), new Id[] {linkBX.getId()}, linkBB.getId());
      ...
      TransitRoute BXRoute =
          sf.createTransitRoute(
              Id.create("lineBX", TransitRoute.class), networkRouteBX, stopsBX, "train");
      ...
      NetworkRoute networkRouteCX =
          RouteUtils.createLinkNetworkRouteImpl(
              linkCC.getId(), new Id[] {linkCX.getId()}, linkCC.getId());
      ...
      TransitRoute CXRoute =
          sf.createTransitRoute(
              Id.create("lineCX", TransitRoute.class), networkRouteCX, stopsCX, "train");
      ...
      NetworkRoute networkRouteDX =
          RouteUtils.createLinkNetworkRouteImpl(
              linkDD.getId(), new Id[] {linkDX.getId()}, linkDD.getId());
      ...
      TransitRoute DXRoute =
          sf.createTransitRoute(
              Id.create("lineDX", TransitRoute.class), networkRouteDX, stopsDX, "train");
      ...
      NetworkRoute networkRouteEX =
          RouteUtils.createLinkNetworkRouteImpl(
              linkEE.getId(), new Id[] {linkEX.getId()}, linkEE.getId());
      ...
      TransitRoute EXRoute =
          sf.createTransitRoute(
              Id.create("lineEX", TransitRoute.class), networkRouteEX, stopsEX, "train");
