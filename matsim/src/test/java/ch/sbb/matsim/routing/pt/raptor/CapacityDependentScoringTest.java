...
public class CapacityDependentScoringTest {
  ...
  public void testScoring() {
    ...
    // in the capacity dependent case, the vehicle is empty plus the passenger => occupancy = 0.2,
    // thus the cost should only be 0.8 * original cost => -1.2
  ...
  private double calcScore(Fixture f, boolean capacityDependent) {
    ...
    RaptorInVehicleCostCalculator inVehicleCostCalculator =
        capacityDependent
            ? (new CapacityDependentInVehicleCostCalculator(0.4, 0.3, 0.6, 1.8))
            : (new DefaultRaptorInVehicleCostCalculator());
    OccupancyData occData = new OccupancyData();
    OccupancyTracker occTracker =
        new OccupancyTracker(occData, f.scenario, inVehicleCostCalculator, events, parameters);

    ScoringFunctionFactory testSFF =
        new ScoringFunctionFactory() {
          ...
          public ScoringFunction createNewScoringFunction(Person person) {
            ...
            scoringFunctionAccumulator.addScoringFunction(
                new CharyparNagelLegScoring(params, network, config.transit().getTransitModes()));
    ...
    EventsToScore events2Score =
        EventsToScore.createWithoutScoreUpdating(f.scenario, testSFF, events);
    ...
    events.processEvent(
        new TransitDriverStartsEvent(
            ...
            f.fastLineId,
            f.fastRouteId,
            Id.create("f1", Departure.class)));
    events.processEvent(
        new PersonDepartureEvent(7 * 3600 - 100, driver1, Id.create(1, Link.class), "car", "car"));
    ...
    events.processEvent(
        new ActivityEndEvent(
            ...
            new Coord(234., 5.67)));
    events.processEvent(
        new PersonDepartureEvent(7 * 3600, person.getId(), Id.create(1, Link.class), "pt", "pt"));
    events.processEvent(new AgentWaitingForPtEvent(7 * 3600, person.getId(), f.stopAId, f.stopDId));

    events.processEvent(
        new VehicleArrivesAtFacilityEvent(7 * 3600 - 10, veh.getId(), f.stopAId, 0));
    ...
    events.processEvent(
        new VehicleArrivesAtFacilityEvent(7 * 3600 + 5 * 60, veh.getId(), f.stopBId, 0));
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(7 * 3600 + 5 * 60, veh.getId(), f.stopBId, 0));

    events.processEvent(
        new VehicleArrivesAtFacilityEvent(7 * 3600 + 10 * 60, veh.getId(), f.stopCId, 0));
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(7 * 3600 + 10 * 60, veh.getId(), f.stopCId, 0));

    events.processEvent(
        new VehicleArrivesAtFacilityEvent(7 * 3600 + 15 * 60 - 10, veh.getId(), f.stopDId, 0));
    events.processEvent(
        new PersonLeavesVehicleEvent(7 * 3600 + 15 * 60, person.getId(), veh.getId()));
    events.processEvent(
        new PersonArrivalEvent(7 * 3600 + 15 * 60, person.getId(), Id.create(4, Link.class), "pt"));
    events.processEvent(
        new VehicleDepartsAtFacilityEvent(7 * 3600 + 16 * 60, veh.getId(), f.stopDId, 0));
