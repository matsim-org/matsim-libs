...
public class SwissRailRaptorInVehicleCostTest {
  ...
  public void test_capacityDependentInVehicleCost_prefersLowOccupancy_uses_slowRoute() {
    ...
    // from the note above:
    // * fastRoute must get (870 + 420)/870 = 1.483 times more expensive. --> costFactor at least
    // 1.49
    // * slowRoute must get 420/1230 = 0.3414634146 less expensive --> costFactor at most 0.65
    ...
    // Use 1 + 0.49*2 as maximum cost factor, as this applies to 100% occupancy, so it still is 1.49
    // at 80%.
    ...
    runTest(
        f,
        new CapacityDependentInVehicleCostCalculator(1.00, 0.2, 0.6, 1.0 + 2 * 0.48),
        f.fastLineId);
    runTest(
        f,
        new CapacityDependentInVehicleCostCalculator(1.00, 0.2, 0.6, 1.0 + 2 * 0.49),
        f.slowLineId);
  }
  ...
  private void runTest(
    ...
    List<? extends PlanElement> route1 =
        raptor.calcRoute(
            DefaultRoutingRequest.withoutAttributes(
                fromFacility, toFacility, Time.parseTime("07:00:00"), null));
  ...
  private void fillExecutionTracker(Fixture f, OccupancyTracker tracker) {
    ...
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:51:00"), person1, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:51:00"), person1, f.stopAId, f.stopDId));
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:52:00"), person2, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:52:00"), person2, f.stopAId, f.stopDId));
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:53:00"), person3, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:53:00"), person3, f.stopAId, f.stopDId));
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:54:00"), person4, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:54:00"), person4, f.stopAId, f.stopDId));
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:55:00"), person5, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:55:00"), person5, f.stopAId, f.stopDId));
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:56:00"), person6, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:56:00"), person6, f.stopAId, f.stopDId));
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:57:00"), person7, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:57:00"), person7, f.stopAId, f.stopDId));
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:58:00"), person8, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:58:00"), person8, f.stopAId, f.stopDId));
    tracker.handleEvent(
        new PersonDepartureEvent(
            Time.parseTime("06:59:00"), person9, f.stopA.getLinkId(), "pt", "pt"));
    tracker.handleEvent(
        new AgentWaitingForPtEvent(Time.parseTime("06:59:00"), person9, f.stopAId, f.stopDId));
    ...
    tracker.handleEvent(
        new TransitDriverStartsEvent(
            Time.parseTime("06:55:00"), driver1, vehicle1, f.fastLineId, f.fastRouteId, fastDep0));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:00:00"), vehicle1, f.stopAId, 0));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:00:10"), person1, vehicle1));
    tracker.handleEvent(
        new VehicleDepartsAtFacilityEvent(Time.parseTime("07:00:30"), vehicle1, f.stopAId, 0));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:04:30"), vehicle1, f.stopBId, 0));
    tracker.handleEvent(
        new VehicleDepartsAtFacilityEvent(Time.parseTime("07:05:00"), vehicle1, f.stopBId, 0));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:09:30"), vehicle1, f.stopCId, 0));
    tracker.handleEvent(
        new VehicleDepartsAtFacilityEvent(Time.parseTime("07:10:00"), vehicle1, f.stopCId, 0));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:14:30"), vehicle1, f.stopDId, 0));
    tracker.handleEvent(
        new PersonLeavesVehicleEvent(Time.parseTime("07:15:00"), person1, vehicle1));

    // dep at 07:10: person2, person3, person4, person5 --> 4 person in vehicle with capacity 5 -->
    // 80% occupancy
    tracker.handleEvent(
        new TransitDriverStartsEvent(
            Time.parseTime("07:05:00"), driver2, vehicle2, f.fastLineId, f.fastRouteId, fastDep1));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:10:00"), vehicle2, f.stopAId, 0));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:10:05"), person2, vehicle2));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:10:10"), person3, vehicle2));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:10:15"), person4, vehicle2));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:10:20"), person5, vehicle2));
    tracker.handleEvent(
        new VehicleDepartsAtFacilityEvent(Time.parseTime("07:10:30"), vehicle2, f.stopAId, 0));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:14:30"), vehicle2, f.stopBId, 0));
    tracker.handleEvent(
        new VehicleDepartsAtFacilityEvent(Time.parseTime("07:15:00"), vehicle2, f.stopBId, 0));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:19:30"), vehicle2, f.stopCId, 0));
    tracker.handleEvent(
        new VehicleDepartsAtFacilityEvent(Time.parseTime("07:20:00"), vehicle2, f.stopCId, 0));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:24:30"), vehicle2, f.stopDId, 0));
    tracker.handleEvent(
        new PersonLeavesVehicleEvent(Time.parseTime("07:25:00"), person2, vehicle2));
    tracker.handleEvent(
        new PersonLeavesVehicleEvent(Time.parseTime("07:25:00"), person3, vehicle2));
    tracker.handleEvent(
        new PersonLeavesVehicleEvent(Time.parseTime("07:25:00"), person4, vehicle2));
    tracker.handleEvent(
        new PersonLeavesVehicleEvent(Time.parseTime("07:25:00"), person5, vehicle2));

    // dep at 07:20: person6, person7, person8, person9
    tracker.handleEvent(
        new TransitDriverStartsEvent(
            Time.parseTime("07:15:00"), driver3, vehicle3, f.fastLineId, f.fastRouteId, fastDep2));
    tracker.handleEvent(
        new VehicleArrivesAtFacilityEvent(Time.parseTime("07:20:00"), vehicle3, f.stopAId, 0));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:10:20"), person6, vehicle3));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:20:20"), person7, vehicle3));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:20:25"), person8, vehicle3));
    tracker.handleEvent(
        new PersonEntersVehicleEvent(Time.parseTime("07:20:25"), person9, vehicle3));
    tracker.handleEvent(
        new VehicleDepartsAtFacilityEvent(Time.parseTime("07:20:30"), vehicle3, f.stopAId, 0));
  }
    ...
    public Fixture() {
      ...
      Link linkAA =
          NetworkUtils.createLink(
              Id.create("aa", Link.class), nodeA, nodeA, network, 3000, 20, 2000, 1);
      Link linkAB =
          NetworkUtils.createLink(
              Id.create("ab", Link.class), nodeA, nodeB, network, 3000, 20, 2000, 1);
      Link linkBC =
          NetworkUtils.createLink(
              Id.create("bc", Link.class), nodeB, nodeC, network, 3000, 20, 2000, 1);
      Link linkCD =
          NetworkUtils.createLink(
              Id.create("cd", Link.class), nodeC, nodeD, network, 3000, 20, 2000, 1);
      ...
      { // fast line
        ...
        fastStops.add(
            sf.createTransitRouteStopBuilder(this.stopB)
                .arrivalOffset(5 * 60 - 30)
                .departureOffset(5 * 60)
                .build());
        fastStops.add(
            sf.createTransitRouteStopBuilder(this.stopC)
                .arrivalOffset(10 * 60 - 30)
                .departureOffset(10 * 60)
                .build());
        fastStops.add(
            sf.createTransitRouteStopBuilder(this.stopD).arrivalOffset(15 * 60 - 30).build());

        NetworkRoute route =
            RouteUtils.createNetworkRoute(
                List.of(linkAA.getId(), linkAB.getId(), linkBC.getId(), linkCD.getId()), network);
        ...
        for (int i = 0; i < 10; i++) {
          Departure dep =
              sf.createDeparture(Id.create("f" + i, Departure.class), 7 * 3600 + i * 600);
      ...
      { // slow line
        ...
        slowStops.add(
            sf.createTransitRouteStopBuilder(this.stopB)
                .arrivalOffset(7 * 60 - 30)
                .departureOffset(7 * 60)
                .build());
        slowStops.add(
            sf.createTransitRouteStopBuilder(this.stopC)
                .arrivalOffset(14 * 60 - 30)
                .departureOffset(14 * 60)
                .build());
        slowStops.add(
            sf.createTransitRouteStopBuilder(this.stopD).arrivalOffset(21 * 60 - 30).build());
        ...
        for (int i = 0; i < 10; i++) {
          Departure dep =
              sf.createDeparture(Id.create("s" + i, Departure.class), 7 * 3600 + 60 + i * 600);
