...
public class SwissRailRaptorModuleTest {
  ...
  public void testIntermodalIntegration() {
    ...
    Assert.assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((Activity) planElements.get(2)).getType());
    Assert.assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((Activity) planElements.get(4)).getType());
    Assert.assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((Activity) planElements.get(6)).getType());
    Assert.assertEquals(
        PtConstants.TRANSIT_ACTIVITY_TYPE, ((Activity) planElements.get(8)).getType());
    ...
    // MM started filling the times of the swiss rail raptor pt interaction activities with content
    // and so the above (evidently) started failing.  I am
    // fixing it here:
    ...
    // (And that is actually where I should have noticed that something is wrong, since durations
    // need to be zero, and endTimes need to be undefined.  :-(
    // :-( :-(  kai, mar'20)
  ...
  public void testTransitScheduleUpdate() {
    ...
    // Check route: should return one of the added lines although the removed green line would be
    // faster
  ...
  public void testRaptorParametersForPerson() {
    ...
    srrConfig.setScoringParameters(
        ch.sbb.matsim.config.SwissRailRaptorConfigGroup.ScoringParameters.Individual);
    ...
    config
        .scoring()
        .getOrCreateScoringParameters("default")
        .setMarginalUtlOfWaitingPt_utils_hr(-3600.0 * 30.0);
    config
        .scoring()
        .getOrCreateScoringParameters("sub")
        .setMarginalUtlOfWaitingPt_utils_hr(-3600.0 * 10.0);
    ...
    RaptorParametersForPerson parameters =
        controller.getInjector().getInstance(RaptorParametersForPerson.class);

    Assert.assertEquals(
        -80.0, parameters.getRaptorParameters(personA).getMarginalUtilityOfWaitingPt_utl_s(), 1e-3);
    Assert.assertEquals(
        -60.0, parameters.getRaptorParameters(personB).getMarginalUtilityOfWaitingPt_utl_s(), 1e-3);
  }

  private static class ScheduleModifierControlerListener
      implements StartupListener, IterationStartsListener {
    ...
    private void addLineAndStop(ControlerEvent event, int iteration) {
      ...
      TransitStopFacility addedStop =
          scheduleFactory.createTransitStopFacility(
              Id.create("AddedStop" + iteration, TransitStopFacility.class),
              CoordUtils.createCoord(24000 + iteration * 10, 10000),
              false);
      addedStop.setLinkId(Id.create("3", Link.class));
      TransitLine addedLine =
          scheduleFactory.createTransitLine(Id.create("AddedLine" + iteration, TransitLine.class));

      // Set some arbitrary NetworkRoute to obtain a valid TransitRoute
      NetworkRoute netRoute =
          RouteUtils.createLinkNetworkRouteImpl(
              Id.create("15", Link.class), Id.create("3", Link.class));
      List<TransitRouteStop> stops = new ArrayList<>();
      // Same travel time in vehicle as green line from stop 18 to stop 20
      stops.add(
          scheduleFactory.createTransitRouteStop(
              schedule.getFacilities().get(Id.create("5", TransitStopFacility.class)), 0.0, 60.0));
      stops.add(scheduleFactory.createTransitRouteStop(addedStop, 20 * 60.0, 21 * 60));
      TransitRoute addedRoute =
          scheduleFactory.createTransitRoute(
              Id.create("AddedRoute" + iteration, TransitRoute.class),
              netRoute,
              stops,
              TransportMode.pt);

      // new line departs 5 mins after green line in Fixture -> router would return green line if it
      // was not deleted
      Departure addedDep =
          scheduleFactory.createDeparture(
              Id.create("AddedDeparture" + iteration, Departure.class), 7 * 60 * 60 + 5 * 60);
      Id<Vehicle> vehicleId = Id.createVehicleId("AddedVehicle" + iteration);
      transitVehicles.addVehicle(
          vf.createVehicle(
              vehicleId,
              transitVehicles.getVehicleTypes().get(Id.create("train", VehicleType.class))));
    ...
    private void removeGreenLineAndStop(ControlerEvent event) {
      ...
      schedule.removeStopFacility(
          schedule.getFacilities().get(Id.create("13", TransitStopFacility.class)));
    }
  }
}
