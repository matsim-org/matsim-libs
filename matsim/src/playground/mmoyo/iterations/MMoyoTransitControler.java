...
public class MMoyoTransitControler extends Controler {
	...
	private final void init() {
		...
		ActivityParams params = new ActivityParams(MMoyoPlansCalcTransitRoute.TRANSIT_ACTIVITY_TYPE);
...
//		new QueueSimulation(this.scenarioData, this.events).run();
	...
	public static void main(final String[] args) {
		if (args.length > 0) {
			...
			new MMoyoTransitControler(new String[] {"src/playground/marcel/pt/controler/transitConfig.xml"}).run();
		}
	}
