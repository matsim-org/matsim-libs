...
public class JohScoringFunction implements ScoringFunction {
	...
	public void startLeg(final double time, final Leg leg) {
		...
		if (this.index % 2 == 0) {
			//System.out.println("Aufruf startLeg für Person "+plan.getPerson().getId()+" mit time "+time);
	...
	public void endLeg(final double time) {
		//System.out.println("Aufruf endLeg für Person "+plan.getPerson().getId()+" mit time "+time);
		handleLeg(time);
		this.lastTime = time;
	}
	...
	*/
	...
	private final double calcActScore(final double arrivalTime, final double departureTime, final Act act) {
		...
		if (duration > 0) {
			...
			//System.out.println(utilPerf+" für "+utilParams.get(act.getType()).getType()+" und duration = "+duration/3600);
	...
	private void handleLeg(final double time) {
		...
		//System.out.println("Person: "+this.plan.getPerson().getId()+" Score: "+calcLegScore(this.lastTime, time, leg)+", leg: "+leg.getNum()+", travTime: "+leg.getTravTime()+", übergebene lastTime: "+lastTime+", übergebene time: "+time);
		this.index++;
	}
