...
public class TimeModeChoicer1 implements org.matsim.population.algorithms.PlanAlgorithm { 
	...
	public void run (PlanImpl basePlan){
		...
		for (int i=1;i<plan.getPlanElements().size();i=i+2){
			LinkNetworkRoute r = new LinkNetworkRoute(((LegImpl)(plan.getPlanElements().get(i))).getRoute().getStartLink(), ((LegImpl)(plan.getPlanElements().get(i))).getRoute().getEndLink());
		/*	List<Id> l = new ArrayList<Id>();
			...
			List<Id> l = ((NetworkRoute) ((LegImpl)(plan.getPlanElements().get(i))).getRoute()).getLinkIds();
			r.setLinkIds(l);
			routes.add(r);
		}
		this.routes = routes;
	...
	//	stream.println("Selected solution\t"+bestScore);
		...
		for (int i = 0; i<al.size();i++){
			if (i%2==0){
				...
				LinkNetworkRoute r = new LinkNetworkRoute(((LegImpl)(bestSolution.get(i))).getRoute().getStartLink(), ((LegImpl)(bestSolution.get(i))).getRoute().getEndLink());
				List<Id> l = ((NetworkRoute) ((LegImpl)(bestSolution.get(i))).getRoute()).getLinkIds();
				r.setLinkIds(l);
...
//			travelTime = this.estimator.getInterpolation(plan.getPerson().getId(), now, (ActivityImpl)(plan.getPlanElements().get(i-1)), (ActivityImpl)(plan.getPlanElements().get(i+1)), (LegImpl)(plan.getPlanElements().get(i)));
			...
			if (((LegImpl)(plan.getPlanElements().get(i))).getMode()!=TransportMode.car){
				((LegImpl)(plan.getPlanElements().get(i))).setRoute(this.routes.get((int)(i/2)));
			}
