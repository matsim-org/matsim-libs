...
public class PlanomatX implements org.matsim.population.algorithms.PlanAlgorithm { 
	...
	public void run (Plan plan){
		...
		//LinkedList<PlanomatXPlan> tabuList 				= new LinkedList<PlanomatXPlan>();
		...
		for (int currentIteration = 1; currentIteration<MAX_ITERATIONS+1;currentIteration++){
			...
			// Route and score all non-tabu plans and write them into list nonTabuNeighbourhood
			...
			if (currentIteration==MAX_ITERATIONS) {
				...
				for (int initialisationOfNextIteration = 0;initialisationOfNextIteration<NEIGHBOURHOOD_SIZE+1; initialisationOfNextIteration++){
					neighbourhood[initialisationOfNextIteration] = nonTabuNeighbourhood.get(nonTabuNeighbourhood.size()-1);
	...
	public int changeOrder (PlanomatXPlan basePlan, int planBasePos){
		...
		if (actslegs.size()<=5){	//If true the plan has not enough activities to change their order. Do nothing.
			...
			for (int planRunningPos = planBasePos; planRunningPos <= actslegs.size()-4; planRunningPos=planRunningPos+2){ //Go through the "inner" acts only
				...
				//System.out.println("planRunningPos ist "+planRunningPos+", planBasePos ist "+planBasePos);
