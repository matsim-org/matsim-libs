...
public class PuTTripFinderHandlerDistance implements TabularFileHandlerI {
	 ...
	 */
	public PuTTripFinderHandlerDistance(final double searchRadius,
	...
	public void startRow(final String[] row) {
		...
		if (this.line == 1) {
			...
			try {
				this.writer
						.write("PersonID\tTripID\tsegmentId\tSegments\tPutSegments\tRailSegments\tXStartingPoint\tYStartingPoint\tStartingDate\tStartingTime\tXEndingPoint\tYEndingPoint\tEndingDate\tEndingTime\tProbability_Walk\tProbability_Bike\tProbability_Car\tProbability_UrbanPuT\tProbability_Rail\tDistance\tTravelTime\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
			return;
		...
		if (!row[0].equals(this.personId) || !row[1].equals(this.tripId)) {
			...
			if (this.cntPuTSegments > 0 || this.cntRailSegments > 0) {
				// if (this.tripId.equals("53") &&
				// this.personId.equals("45121"))
		...
		this.TravelTime.add(Double.valueOf(row[14]));
		...
		if (this.cntPuTSegments > 0 || this.cntRailSegments > 0) {
			...
			for (int i = 0; i < this.segmentIds.size(); i++)
				this.writer.write(this.personId + "\t" + this.tripId + "\t"
						...
						+ this.Distance.get(i) + "\t" + this.TravelTime.get(i)
						+ "\n");
		}
	}
