...
public class SHPWriter {
	...
	public void write(List<Hectare> hectares)  {
		...
		try {
			if (!features.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)features, "output/hectares.shp");
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void writeNelsonFacilities(List<ZHFacility> facilities)  {
		...
		try {
			if (!featuresCoop.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresCoop, "output/nelsonFacilitiesCoop.shp");
				statistics.write("Nelson Coop: "+ featuresCoop.size() +"\n");
			}
			if (!featuresMigros.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresMigros, "output/nelsonFacilitiesMigros.shp");
				statistics.write("Nelson Mirgos: "+ featuresMigros.size() +"\n");
			}
			if (!featuresDenner.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresDenner, "output/nelsonFacilitiesDenner.shp");
				statistics.write("Nelson Denner: "+ featuresDenner.size() +"\n");
			}
			if (!featuresSpar.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresSpar, "output/nelsonFacilitiesSpar.shp");
				statistics.write("Nelson Spar: "+ featuresSpar.size() +"\n");
			}
			if (!featuresPickPay.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresPickPay, "output/nelsonFacilitiesPickPay.shp");
				statistics.write("Nelson PickPay: "+ featuresPickPay.size() +"\n");
			}
			if (!featuresPrimo.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresPrimo, "output/nelsonFacilitiesPrimo.shp");
				statistics.write("Nelson Primo: "+ featuresPrimo.size() +"\n");
			}
			if (!featuresVolg.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresVolg, "output/nelsonFacilitiesVolg.shp");
				statistics.write("Nelson Volg: "+ featuresVolg.size() +"\n");
			}
			if (!featuresWarehouses.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresWarehouses, "output/nelsonFacilitiesWarehouses.shp");
				statistics.write("Nelson Warehouses: "+ featuresWarehouses.size() +"\n");
			}
			if (!featuresSupermarkets.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresSupermarkets, "output/nelsonFacilitiesSupermarkets.shp");
				statistics.write("Nelson Supermarkets: "+ featuresSupermarkets.size() +"\n");
			}
			if (!featuresDiv.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresDiv, "output/nelsonFacilitiesDiv.shp");
	...
	public void writeDatapulsFacilities(List<ZHFacilityComposed> facilities)  {
		...
		try {
			if (!featuresCoop.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresCoop, "output/datapulsFacilitiesCoop.shp");
				statistics.write("Datapuls Coop: "+ featuresCoop.size() +"\n");
			}
			if (!featuresMigros.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresMigros, "output/datapulsFacilitiesMigros.shp");
				statistics.write("Datapuls Migros: "+ featuresMigros.size() +"\n");
			}
			if (!featuresDenner.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresDenner, "output/datapulsFacilitiesDenner.shp");
				statistics.write("Datapuls Denner: "+ featuresDenner.size() +"\n");
			}
			if (!featuresSpar.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresSpar, "output/datapulsFacilitiesSpar.shp");
				statistics.write("Datapuls Spar: "+ featuresSpar.size() +"\n");
			}
			if (!featuresPickPay.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresPickPay, "output/datapulsFacilitiesPickPay.shp");
				statistics.write("Datapuls PickPay: "+ featuresPickPay.size() +"\n");
			}
			if (!featuresPrimo.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresPrimo, "output/datapulsFacilitiesPrimo.shp");
				statistics.write("Datapuls Primo: "+ featuresPrimo.size() +"\n");
			}
			if (!featuresVolg.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresVolg, "output/datapulsFacilitiesVolg.shp");
				statistics.write("Datapuls Volg: "+ featuresVolg.size() +"\n");
			}
			if (!featuresWarehouses.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresWarehouses, "output/datapulsFacilitiesWarehouses.shp");
				statistics.write("Datapuls Warehouses: "+ featuresWarehouses.size() +"\n");
			}
			if (!featuresSupermarkets.isEmpty()) {
				ShapeFileWriter.writeGeometries((Collection<Feature>)featuresSupermarkets, "output/datapulsFacilitiesSupermarkets.shp");
