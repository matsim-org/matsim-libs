...
public class GrowingCirclesLocationMutator extends AbstractPersonAlgorithm implements PlanAlgorithm {
	...
	public GrowingCirclesLocationMutator(final NetworkLayer network, final ActivityFacilitiesImpl facilities) {
		super();
		this.init(network, facilities);
	}


	private void init(final NetworkLayer network, final ActivityFacilitiesImpl facilities) {
	...
	private void buildShopFacQuadTree() {
		...
		for (final ActivityFacilityImpl f : this.shop_facilities.values()) {
	...
	private void buildLeisFacQuadTree() {
		...
		double maxx = Double.NEGATIVE_INFINITY;
		double maxy = Double.NEGATIVE_INFINITY;
		for (final ActivityFacilityImpl f : this.leisure_facilities.values()) {
			if (f.getCoord().getX() < minx) { minx = f.getCoord().getX(); }
			if (f.getCoord().getY() < miny) { miny = f.getCoord().getY(); }
			if (f.getCoord().getX() > maxx) { maxx = f.getCoord().getX(); }
			if (f.getCoord().getY() > maxy) { maxy = f.getCoord().getY(); }
	...
	private final QuadTree<ActivityFacilityImpl> getFacilities(final String act_type) {
	...
	private final ActivityFacilityImpl getFacility(final Collection<ActivityFacilityImpl> fs, String act_type) {
	...
	private final ActivityFacilityImpl getFacility(final Coord coord, final double radius, final String act_type) {
		final Collection<ActivityFacilityImpl> fs = this.getFacilities(act_type).get(coord.getX(),coord.getY(),radius);
		if (fs.isEmpty()) {
	...
	private final ActivityFacilityImpl getFacility(final CoordImpl coord1, final CoordImpl coord2, final double radius, final String act_type) {
		final Collection<ActivityFacilityImpl> fs = this.getFacilities(act_type).get(coord1.getX(),coord1.getY(),radius);
		...
		if ((prim_coord == null) || (home_coord.equals(prim_coord))) {
			...
			for (PlanElement pe : plan.getPlanElements()) {
				if (pe instanceof ActivityImpl) {
					...
					if ((act.getCoord() == null) || (act.getCoord().equals(ZERO))) {
						final ActivityFacilityImpl f = this.getFacility(home_coord,radius,act.getType());
			...
			for (PlanElement pe : plan.getPlanElements()) {
				if (pe instanceof ActivityImpl) {
					final ActivityImpl act = (ActivityImpl) pe;
					if ((act.getCoord() == null) || (act.getCoord().equals(ZERO))) {
						final ActivityFacilityImpl f = this.getFacility(coord1,coord2,radius,act.getType());
						act.setLink(this.network.getNearestLink(f.getCoord()));
						act.setCoord(f.getCoord());
					}
				}
