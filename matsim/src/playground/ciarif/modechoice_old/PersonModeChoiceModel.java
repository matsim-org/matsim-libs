...
public class PersonModeChoiceModel extends PersonAlgorithm implements PlanAlgorithmI {
	...
	private final void deleteSubTour (Plan plan, TreeMap<Integer, List<Integer>> subtours, int start, int end,int subtour_idx) {
		for (int i=start+2; i<end; i=i+2){
			plan.removeAct(i);
			plan.removeLeg(i-1);//Cambiare!!!! Cos� non funziona sicuramente perch� 
								//non tornanon pi� i numeri di act_leg, la soluzione
								// migliore probabilmente � creare un'altro oggetto
	...
	private final void registerSubTour2 (Plan plan, TreeMap<Integer, List<Integer>> subtours, int start, int end,int subtour_idx) {
		...
		extractSubTours3(plan, start, end, subtour_idx, subtours);
	}
	...
	private final void extractSubTours(Plan plan, int start, int end, int subtour_idx, int sub_order, TreeMap<Integer,Integer> subtours) {
		
		boolean is_leaf = true;
		System.out.println(" start =" + start);
		System.out.println("end = " + end);
		for (int i=start; i<plan.getActsLegs().size()-2; i=i+2){
...
//					else {
					for (int iii=start+1;iii<i; iii=iii+2) { subtours.put(iii,subtour_idx); }
					for (int iii=j+1;iii<end; iii=iii+2) { subtours.put(iii,subtour_idx); }
					this.extractSubTours(plan,j,end,subtour_idx,sub_order,subtours);
					...
					for (int jj=i+2; jj<=j; jj=jj+2) {
						...
						if ((actii.getCoord().getX() == actjj.getCoord().getX()) &&
							...
							this.extractSubTours(plan,ii,jj,subtour_idx,sub_order,subtours);
							ii = jj;
							actii = (Act)plan.getActsLegs().get(ii);
						}
					}
					//subtour_idx=0;
					//this.extractSubTours(plan,j+2,end,subtour_idx,subtours);
					return;
				}	
			}
		}
		if (is_leaf) {
			// mark the legs of the subtour found
			System.out.println("start leaf =" + start);
			System.out.println("end leaf =" + end);
			System.out.println (subtour_idx);
			for (int iii=start+1;iii<end; iii=iii+2) { subtours.put(iii,subtour_idx); }
			// DO NOT HANDLE ANY SUBTOUR YET!!!
			//end = plan.getActsLegs().size()-1;
		}
...
//	private final void extractSubTours2(Plan plan, int start, int end,int subtour_idx) {
//		boolean is_leaf = true;
...
//					// subtour found: start..i & j..end
//					subtour_idx = subtour_idx +1;
//					System.out.println ("subtour =" + subtour_idx); 
//					is_leaf = false;
...
//						if ((actii.getCoord().getX() == actjj.getCoord().getX()) &&
//						    (actii.getCoord().getY() == actjj.getCoord().getY())) {
//							this.extractSubTours2(plan,ii,jj,subtour_idx);
//							ii = jj;
//							actii = (Act)plan.getActsLegs().get(ii);
//						}
//					}
...
//		if (is_leaf) {
//			subtour_idx = subtour_idx +1;
//			System.out.println ("subtour leaf =" + subtour_idx); 
