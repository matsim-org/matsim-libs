 ...
 *
 */
public class DegreeCorrelationStats extends GraphPropertyEstimator {

	private double globalResponseRate;
	
	public DegreeCorrelationStats(String outputDir, double responseRate) {
		super(outputDir);
...
//					&& !v2.getDelegate().isAnonymous()) {
			if (v1.getDelegate().isSampled()
				...
				double w = 1 / (globalResponseRate * (p1 + p2) - (Math.pow(globalResponseRate, 2) * p1 * p2));
