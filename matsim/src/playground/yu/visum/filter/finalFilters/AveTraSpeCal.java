...
public class AveTraSpeCal extends LinkAveCalA {
	 ...
	 */
	@Override
	public void compute(final LinkEnterEnter enter, final double leaveTime) {
		// int linkId = 0;
		try {
			LinkEnterEnter event = rebuildEventLinkEnter(enter);
...
//			System.err.println(e);
	...
	public double atxCal(final String linkID, final String timeBin) {
		...
		ats = (ats != 0) ? ats : ((Link) this.network.getLocation(linkID)).getFreespeed(Time.UNDEFINED_TIME);
		return ats;
	}
}
