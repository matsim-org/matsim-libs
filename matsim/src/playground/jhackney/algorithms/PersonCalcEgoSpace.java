...
//	}
	...
	public void run(Person person) {
		...
		while(e_it.hasNext()){
			activities.add( (ActivityOption) ((Act)(e_it.next().getSelectedPlan().getActsLegs().get(0))).getFacility().getActivityOption("home"));
		}
		...
		if (coords.size() < 1) {
			...
			for (int s=0; s<activity_shapes.length; s++) {
				...
				ActivitySpace act_space = know.createActivitySpace(activity_shape,"home");	// all, home, work, education, shop, leisure
