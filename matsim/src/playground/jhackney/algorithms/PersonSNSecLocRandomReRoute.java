...
import playground.jhackney.SNControllerListener1;
...
//		Pick a type of facility to replace in this plan according to config settings
		...
		if (rand < cum_p_factype[0]) {
			factype = SNControllerListener1.activityTypesForEncounters[0];
		}else if (cum_p_factype[0] <= rand && rand < cum_p_factype[1]) {
			factype = SNControllerListener1.activityTypesForEncounters[1];
		}else if (cum_p_factype[1] <= rand && rand < cum_p_factype[2]) {
			factype = SNControllerListener1.activityTypesForEncounters[2];
		}else if (cum_p_factype[2] <= rand && rand < cum_p_factype[3]) {
			factype = SNControllerListener1.activityTypesForEncounters[3];
		}else {
			factype = SNControllerListener1.activityTypesForEncounters[4];
		}
