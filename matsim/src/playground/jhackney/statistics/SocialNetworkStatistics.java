 ...
 * package on the MatSim social network output.
...
//	    //aout.write("tstep egoid egozone egodeg egoasd egoclust egoaccess lastactivity rseed var\n");
...
//	    Average duration of a social link
	    ...
	    smNum.accumulate(x2);
	    try{
		eout.write(iter+" "+myEdge.getUserDatum("timeLastUsed")+" "+myEdge.getUserDatum("timeMet")+" "+dyadDist+" "+pFrom.getId()+" "+pTo.getId()+" "+myEdge.getUserDatum("type")+" "+myEdge.getUserDatum("visitNum"));
		eout.newLine();
	    }catch (IOException ex){}
	}
	try{
	...
	DoubleArrayList degvals= DegreeDistributions.getDegreeValues(g.getVertices());
	int maxval = max(degvals);
	Histogram h= DegreeDistributions.getIndegreeHistogram(g.getVertices(), 0, maxval, maxval+1);
...
//	    Agent's Plan Type
	    ...
	    String planType=thisPlan.getType();
	    if(planType == null){
		planType="";
...
//	    measures, e.g. in Pajek, replace this 1 with 0
	    clusterCoef = Double.parseDouble(clusterMap.get(myVert).toString());
...
//	    }
	    try {
		aout.write(iter+" "+id+" "+homeId+" "+deg+" "+aSd1+" "+aSd2+" "+aSd3+" "+clusterCoef+" "+planType+" "+myPerson.getKnowledge().map.getNumKnownFacilities());
		aout.newLine();
	    } catch (IOException e) {
...
//	See Watts 1999 book on Small Worlds
	...
	try {
	    gout.write(iter+" "+smAD.average()+" "+ smCC.average()+" "+ clusteringRatio+" "+smASD1.average()+" "+smASD2.average()+" "+smASD3.average());
	    //gout.newLine();//write newLine() after adding variables in Edge statistics
	} catch (IOException e) {
