...
import playground.gregor.snapshots.writers.PositionOnLineStringInfo;
...
public class SnapshotGenerator implements EventHandlerAgentDepartureI, EventHandlerAgentArrivalI, EventHandlerLinkEnterI,
	...
	public SnapshotGenerator(final NetworkLayer network, final double snapshotPeriod, final LineStringTree lst) {
		SnapshotGenerator.lst = lst;
		this.network = network;
		this.eventLinks = new HashMap<String, EventLink>((int)(network.getLinks().size()*1.1), 0.95f);
		this.eventAgents = new HashMap<String, EventAgent>(1000, 0.95f);
		this.snapshotPeriod = snapshotPeriod;
	...
	private void doSnapshot(final double time) {
		...
		if (!this.snapshotWriters.isEmpty()) {
			final Collection<PositionInfo> positions = getVehiclePositions(time);
	...
	private Collection<PositionInfo> getVehiclePositions(final double time) {
		final Collection<PositionInfo> positions = new ArrayList<PositionInfo>();
	...
	private static class EventLink {
		...
		public void getVehiclePositionsQueue(final Collection<PositionInfo> positions, final double time) {
			...
			for (final EventAgent agent : this.buffer) {
				...
				final PositionInfo position = new PositionOnLineStringInfo(agent.id,
			...
			for (final EventAgent agent : this.waitingQueue) {
				final PositionInfo position = new PositionInfo(agent.id,
		...
		public void getVehiclePositionsEquil(final Collection<PositionInfo> positions, final double time) {
			...
			if (cnt > 0) {
				...
				for (final EventAgent agent : this.buffer) {
					...
					final PositionInfo position = new PositionOnLineStringInfo(agent.id, this.link, distFromFromNode, agent.lane, agent.speed, PositionInfo.VehicleState.Driving, null,lst);
					positions.add(position);
			...
			if (cnt > 0) {
				...
				for (final EventAgent agent : this.waitingQueue) {
					...
					agent.linkPosition = distFromFromNode;
					agent.speed = 0.0;
					final PositionInfo position = new PositionOnLineStringInfo(agent.id, this.link, distFromFromNode, agent.lane, agent.speed, PositionInfo.VehicleState.Parking, null,lst);
					positions.add(position);
					distFromFromNode -= cellSize;
				}
			}
	...
	private class NetStateWriter extends DisplayNetStateWriter implements SnapshotWriterI {
		...
		protected double getLinkDisplValue(final BasicLink link, final int index) {
			final EventLink mylink = SnapshotGenerator.this.eventLinks.get(link.getId().toString());
