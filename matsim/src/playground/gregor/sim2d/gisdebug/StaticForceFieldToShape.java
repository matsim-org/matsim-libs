...
public class StaticForceFieldToShape {
	...
	private final QuadTree<Force> forces;


	public StaticForceFieldToShape(QuadTree<Force> forces) {
		this.forces = forces;
	}
	...
	public void createShp() {
		...
		for (Force f : this.forces.values()) {
			
			Coordinate [] coord = {new Coordinate(f.getXCoord(),f.getYCoord()),new Coordinate(f.getXCoord()+f.getFx(),f.getYCoord()+f.getFy()),new Coordinate(f.getXCoord()+f.getFx()+0.01,f.getYCoord()+f.getFy()+0.01)};
			coords.add(coord);
		}
		dump(coords);
