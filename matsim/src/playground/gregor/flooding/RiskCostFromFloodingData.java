...
public class RiskCostFromFloodingData implements RiskCostCalculator {
	...
	public RiskCostFromFloodingData(NetworkLayer net,
			FloodingReader fr) {
		this.network = net;
		this.fr = fr;
		init();
...
//		new NodeCostShapeCreator(this.lis,MGC.getCRS(TransformationFactory.WGS84_UTM47S));
//		log.info("done");
	}
	...
	private Map<Node,NodeInfo> classifyNodes(QuadTree<FloodingInfo> tree) {
		...
		for (Node node : this.network.getNodes().values()) {
			FloodingInfo fi = tree.get(node.getCoord().getX(), node.getCoord().getY());
			double dist = fi.getCoordinate().distance(MGC.coord2Coordinate(node.getCoord())); 
			if (dist > MAX_DIST){
				continue;
			}
	...
	private QuadTree<FloodingInfo> buildQuad() {
		...
		QuadTree<FloodingInfo> tree = new QuadTree<FloodingInfo>(e.getMinX(),e.getMinY(),e.getMaxX(),e.getMaxY()); 
	...
	public void handleEvent(LinkEnterEvent event) {
		...
		AgentMoneyEvent e = new AgentMoneyEvent(event.getTime(),event.getPersonId(),li.cost/-600);
