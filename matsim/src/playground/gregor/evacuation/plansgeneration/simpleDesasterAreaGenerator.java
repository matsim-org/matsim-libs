...
public class simpleDesasterAreaGenerator {
	...
	public static void main(String[] args) {
		...
		Gbl.createWorld();
		Gbl.createConfig(new String[] {"./configs/evacuationConf.xml"});
		...
		QueueLink A  = network.getLinks().get("35718");
		QueueLink B  = network.getLinks().get("50823");
		...
		for (QueueLink link : network.getLinks().values()) {
			...
			EvacuationAreaLink el = new EvacuationAreaLink(link.getId(),3600.0 * 9 + 45*60+60.0 * deadlineOffset );
