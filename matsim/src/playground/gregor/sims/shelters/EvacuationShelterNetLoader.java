...
//		generateShelterLinks();
	...
	public void generateShelterLinks() {
		...
		for (Building building : this.buildings) {
			...
			Link l1 = this.network.createLink(new IdImpl("sl" + count + "a"), from, sn1, getDist(from,sn1) , 1.66, 6, 1); //FIXME find right values flow cap, lanes, ...
			this.buildingsLinkMapping.put(l1.getId(), building);
			Link l2 = this.network.createLink(new IdImpl("sl" + count + "b"), sn1,sn2, 100 , 10000, 10000, 1); //FIXME find right values flow cap, lanes, ...
