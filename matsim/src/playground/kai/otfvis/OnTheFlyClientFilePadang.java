...
public class OnTheFlyClientFilePadang extends OTFClientFile{
	
	private static final String CVSROOT = "../vsp-cvs";
	private static final String BG_IMG_ROOT = CVSROOT + "/studies/padang/imagery/sliced/blabla";
	...
	public static void main2(final String[] args) {
		final String netFileName = CVSROOT + "/studies/padang/padang_net.xml"; 
		...
		final OTFClient client = new OTFClient("tveh:"+vehFileName + "@" + netFileName, connect1);
		client.run();
	}
	...
	public OTFDrawer getLeftDrawerComponent(final JFrame frame) throws RemoteException {
		...
		final OTFClientQuad clientQ = this.hostControl.createNewView(null, connect1);
	...
	private void loadSlicedBackgroundLayer(final int ux, final int uy, final int num_x, final int num_y, int size, final String dir) {
		int x=ux;
		int y=uy;
...
//		OTFOGLDrawer.linkWidth =2;
		...
		final OTFClientFile client = new OnTheFlyClientFilePadang(filename, null, false);
