...
public class Controler {
	...
	static {
		...
		if (url != null) {
			...
			log.error("");
			log.error("Could not find configuration file " + logProperties
					+ " for Log4j in the classpath.");
			log
					.error("A default configuration is used, setting log level to INFO with a ConsoleAppender.");
			log.error("");
			log.error("");
		}
	}
	...
	public Controler(final Config config, final QueueNetworkLayer network,
	...
	private Controler(final String configFileName, final String dtdFileName,
			final Config config) {
		super();
	...
	public void run() {
		if (this.state == ControlerState.Init) {
			...
			log
					.error("Controler in wrong state to call 'run()'. Expected state: <Init> but was <"
							+ this.state + ">");
		}
	}
	...
	private void doIterations() {
		...
		for (iteration = firstIteration; iteration <= lastIteration
				&& this.state == ControlerState.Running; iteration++) {
	...
	protected void shutdown(final boolean unexpected) {
		...
		if (oldState == ControlerState.Running) {
			if (unexpected) {
				log
						.warn("S H U T D O W N   ---   received unexpected shutdown request.");
			} else {
			...
			// dump plans
			new PlansWriter(this.population,
					getOutputFilename("output_plans.xml.gz"), this.config
							.plans().getOutputVersion()).write();

			// dump facilities, if an output file is specified TODO [MR] use
			// fixed path as for plans
			if (this.config.facilities().getOutputFile() != null) {
				new FacilitiesWriter((Facilities) Gbl.getWorld().getLayer(
						Facilities.LAYER_TYPE)).write();
			}

			// dump network, if an output file is specified
			new NetworkWriter(this.network,
					getOutputFilename("output_network.xml.gz")).write();
			...
			if (unexpected) {
				log
						.info("S H U T D O W N   ---   unexpected shutdown request completed.");
			} else {
	...
	protected void setup() {
		...
		if (this.travelTimeCalculator == null) {
			this.travelTimeCalculator = this.config.controler()
					.getTravelTimeCalculator(this.network, (int) endTime);
		}
		if (this.travelCostCalculator == null) {
			this.travelCostCalculator = new TravelTimeDistanceCostCalculator(
		...
		this.writeEvents = this.externalMobsim == null; // do not write events
		// when using an
		// external mobsim
	...
	private LegTravelTimeEstimator initLegTravelTimeEstimator(
			final TravelTimeI linkTravelTimeCalculator) {
		/*
		 * TODO [MR] move this method somewhere else, it should be more general
		 * instead just being here in the Controler. Think of a bigger picture:
		 * this estimator as well as travel time / cost calculators are all kind
		 * of singletons, and all have similar requirements (plans, network,
		 * events). Maybe this could somehow be generalized? -marcel/18jan2008
		 */
		/*
		 * This estimator is currently only used with the planomat, maybe moving
		 * it to the planomat somewhere? -marcel/18jan2008
		 */
		LegTravelTimeEstimator estimator = null;

		int timeBinSize = 900;
		DepartureDelayAverageCalculator tDepDelayCalc = new DepartureDelayAverageCalculator(
				this.network, timeBinSize);
		this.events.addHandler(tDepDelayCalc);

		/*
		 * it would be nice to load the estimator via reflection, but if we just
		 * use make instead of Eclipse (as usual on a remote server) only
		 * classes occurring in the code are compiled, so we do it the classic
		 * way.
		 */
		...
		} else if (estimatorName
				.equalsIgnoreCase("CetinCompatibleLegTravelTimeEstimator")) {
			estimator = new CetinCompatibleLegTravelTimeEstimator(
					linkTravelTimeCalculator, tDepDelayCalc);
		} else if (estimatorName
				.equalsIgnoreCase("CharyparEtAlCompatibleLegTravelTimeEstimator")) {
			estimator = new CharyparEtAlCompatibleLegTravelTimeEstimator(
					linkTravelTimeCalculator, tDepDelayCalc);
		} else {
			Gbl
					.errorMsg("Invalid name of implementation of LegTravelTimeEstimatorI: "
							+ estimatorName);
		}
		return estimator;
	}
		...
		if (outputDir.exists()) {
			...
			if (!outputDir.mkdir()) {
				throw new RuntimeException(
						"The output directory "
								+ outputPath
								+ " could not be created. Does it's parent directory exist?");
			}
		}

		File tmpDir = new File(getTempPath());
		if (!tmpDir.mkdir() && !tmpDir.exists()) {
			throw new RuntimeException("The tmp directory " + getTempPath()
	 ...
	 * not given in the Constructor. <br>
	 ...
	 * because of backward-compatibility with the old Controler class. In the
	 * future, additional data should be loaded by implementing a
	 * {@link org.matsim.controler.listener.StartupListener}.
	 */
	 ...
	 * instance of {@link QueueNetworkLayer} for the standard QueueSimulation.
	 * <br>
	 * <strong>It is highly recommended NOT to overwrite this method!</strong>
	 * This method should be private, but is only protected at the moment
	...
	protected StrategyManager loadStrategyManager() {
		...
		StrategyManagerConfigLoader.load(this.config, manager, this.network,
				this.travelCostCalculator, this.travelTimeCalculator,
	 ...
	 * {@link org.matsim.controler.listener ControlerListener} to provide basic
	 * functionality. <b>Note:</b> Be very careful if you overwrite this
	...
	protected void loadCoreListeners() {
		...
		this.addControlerListener(new LegHistogramListener(this.events,
				this.createGraphs));

		// optional: score stats
		try {
			this.scoreStats = new ScoreStats(this.population,
					getOutputFilename(FILENAME_SCORESTATS), this.createGraphs);
	...
	private void makeIterationPath(final int iteration) {
		...
		if (!dir.mkdir()) {
			if (this.overwriteFiles && dir.exists()) {
				log.info("Iteration directory " + getIterationPath(iteration)
						+ " exists already.");
			} else {
	...
	private void resetRandomNumbers(final int iteration) {
		...
		Gbl.random.nextDouble(); // draw one because of strange
	...
	protected void runMobSim() {
		...
		if (this.externalMobsim == null) {
			Simulation sim = new QueueSimulation(
					(QueueNetworkLayer) this.network, this.population,
					this.events);
			sim.run();
		} else {
			ExternalMobsim sim = new ExternalMobsim(this.population,
					this.events);
			sim.run();
		}
	}
	...
	private void fireControlerStartupEvent() {
		...
		StartupListener[] listener = this.listenerList
				.getListeners(StartupListener.class);
	...
	private void fireControlerShutdownEvent(final boolean unexpected) {
		...
		ShutdownListener[] listener = this.listenerList
	...
	private void fireControlerIterationStartsEvent(final int iteration) {
		...
		IterationStartsListener[] listener = this.listenerList
				.getListeners(IterationStartsListener.class);
		for (int i = 0; i < listener.length; i++) {
	...
	private void fireControlerIterationEndsEvent(final int iteration) {
		...
		IterationEndsListener[] listener = this.listenerList
				.getListeners(IterationEndsListener.class);
		for (int i = 0; i < listener.length; i++) {
	...
	private void fireControlerScoringEvent(final int iteration) {
		...
		ScoringListener[] listener = this.listenerList
				.getListeners(ScoringListener.class);
		for (int i = 0; i < listener.length; i++) {
	...
	private void fireControlerReplanningEvent(final int iteration) {
		...
		ReplanningListener[] listener = this.listenerList
				.getListeners(ReplanningListener.class);
		for (int i = 0; i < listener.length; i++) {
	...
	private void fireControlerBeforeMobsimEvent(final int iteration) {
		...
		BeforeMobsimListener[] listener = this.listenerList
				.getListeners(BeforeMobsimListener.class);
		for (int i = 0; i < listener.length; i++) {
	...
	private void fireControlerAfterMobsimEvent(final int iteration) {
		...
		AfterMobsimListener[] listener = this.listenerList
				.getListeners(AfterMobsimListener.class);
		for (int i = 0; i < listener.length; i++) {
	...
	/**
	 * Sets the size of the time-window over which the travel times are
	 * accumulated and averaged. Changes to this parameter have no effect after
	 * {@link #run()} is called. <br>
	 * Note that smaller values for the binSize increase memory consumption to
	 * store the travel times.
	 * 
	 * @param binSize
	...
	/**
	 * Returns the size of the time-window used to accumulate and average travel
	 * times.
	 * 
	...
	/**
	 * Sets whether the events should be written to a file or not. External
	 * simulation usually write the events on their own, so we don't need to
	 * write them out a second time when reading them.<br />
	 ...
	 * <code>writeEvents(true)</code>. It is however not possible to start
	 * writing events during an iteration if the iteration started without
	 * writing events, as the event writer is only initialized at the beginning
	 * of an iteration if the writing of events is activated at that time.
	 * 
	 * @param writeEvents
	 */
	...
	public final void setTravelCostCalculator(
	...
	/**
	 * Sets a new {@link org.matsim.scoring.ScoringFunctionFactory} to use.
	 ...
	 *         {@link org.matsim.scoring.ScoringFunctionFactory} for scoring
	 *         plans.
	 */
	...
	/**
	 * @return Returns the {@link org.matsim.replanning.StrategyManager} used
	 *         for the replanning of plans.
	 */
	...
	public static final String getIterationPath(final int iteration) {
		return outputPath + "/" + Controler.DIRECTORY_ITERS + "/it."
				+ iteration;
	}
	 ...
	 */
	/* default */class CoreControlerListener implements
			IterationStartsListener, BeforeMobsimListener, AfterMobsimListener {
		...
		public void notifyBeforeMobsim(final BeforeMobsimEvent event) {
			...
			if (Controler.this.writeEvents) {
				if (Controler.this.eventWriter == null) {
					Controler.this.eventWriter = new EventWriterTXT(Controler
							.getIterationFilename(FILENAME_EVENTS));
				} else {
					Controler.this.eventWriter.init(Controler
							.getIterationFilename(FILENAME_EVENTS));
				}
		...
		public void notifyAfterMobsim(final AfterMobsimEvent event) {
			...
			if ((event.getIteration() % 10 == 0 && event.getIteration() > event
					.getControler().getFirstIteration())
					|| (event.getIteration() % 10 >= 6)) {
				Controler.this.linkStats.addData(Controler.this.volumes,
						Controler.this.travelTimeCalculator);
			}

			if (event.getIteration() % 10 == 0
					&& event.getIteration() > event.getControler()
							.getFirstIteration()) {
				Controler.this.events.removeHandler(Controler.this.volumes);
				Controler.this.linkStats
						.writeFile(getIterationFilename(FILENAME_LINKSTATS));
			}
		}
