    ...
    protected void myRendering(Graphics2D display, AffineTransform boxTransform) {
        ...
        if (getVisConfig().showLinkLabels())
            for (Iterator it = network.getLinks().iterator(); it.hasNext();) {
                ...
                double xpos = link.getStartEasting()
                        + (link.getEndEasting() - link.getStartEasting()) * .42;
                double ypos = link.getStartNorthing()
                        + (link.getEndNorthing() - link.getStartNorthing())
                        * .42;
                ...
                AffineTransform linkTransform2 = new AffineTransform(
                        originalTransform);
