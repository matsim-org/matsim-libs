...
public class ControlInputImplSBNoise extends AbstractControlInputImpl implements EventHandlerLinkLeaveI,
		...
		EventHandlerAgentStuckI, EventHandlerAgentDepartureI, EventHandlerAgentArrivalI,
...
//	private double ttFreeSpeedAfterBottleNeckAlternativeRoute;
	...
	public void init() {
		...
		this.routeNodesMainRoute.remove(0); //remove first Node since its inLinks shouldn't be included
		...
		for (Node n : this.routeNodesMainRoute) {
			for (Link inLink : n.getInLinks().values()) {
				if(!this.routeLinksListMainRoute.contains(inLink)){
					double d = getDistanceFromFirstNode(inLink.getToNode(), this.routeLinksMainRoute);
			...
			for (Link outLink : n.getOutLinks().values()) {
				if(!this.routeLinksListMainRoute.contains(outLink)){
					double d = getDistanceFromFirstNode(outLink.getFromNode(), this.routeLinksMainRoute);
		...
		this.routeNodesAlternativeRoute.remove(0); //first node's inLinks shouldn't be included in the model
		this.routeNodesAlternativeRoute.remove(this.routeNodesAlternativeRoute.size()-1);
		this.routeLinksListAlternativeRoute = Arrays.asList(this.routeLinksAlternativeRoute);
		//all distances to in- and outlinks are stored in Maps <link id, distance>
		for (Node n : this.routeNodesAlternativeRoute) {
			for (Link inLink : n.getInLinks().values()) {
				if(!this.routeLinksListAlternativeRoute.contains(inLink)){
					double d = getDistanceFromFirstNode(inLink.getToNode(), this.routeLinksAlternativeRoute);
					this.inLinksAlternativeRoute.add(inLink);
					this.inFlowDistancesAlternativeRoute.put(inLink.getId().toString(), d);
			...
			for (Link outLink : n.getOutLinks().values()) {
				if(!this.routeLinksListAlternativeRoute.contains(outLink)){
					double d = getDistanceFromFirstNode(outLink.getFromNode(), this.routeLinksAlternativeRoute);
					this.outLinksAlternativeRoute.add(outLink);
					this.outFlowDistancesAlternativeRoute.put(outLink.getId().toString(), d);
	...
	private double getDistanceFromFirstNode(final Node node, final Link [] routeLinks) {
		double distance = 0;
		int i=0;
	...
	public double getPredictedNashTime() {
		...
		this.predTTMainRoute = getPredictedTravelTime(this.mainRoute, this.bottleNeckLinkMainRoute);
		this.predTTAlternativeRoute = getPredictedTravelTime(this.alternativeRoute, this.bottleNeckLinkAlternativeRoute);
...
//			ttFreeSpeedAfterBottleNeckAlternativeRoute = ttFreeSpeedAfterBottleNeck;
	...
	private int getAdditionalAgents(final Route route){
		...
		if(route == this.mainRoute){
			...
			distanceToBottleNeck = getDistanceFromFirstNode(this.bottleNeckLinkMainRoute.getToNode(), this.routeLinksMainRoute);
			...
			while(it1.hasNext()){
				...
				if(this.inFlowDistancesMainRoute.get(inLink.getId().toString()) == null
					...
					weightedInFlow = inFlow * this.inFlowDistancesMainRoute.get(inLink.getId().toString());				}
			...
			while(it2.hasNext()){
				...
				if(this.outFlowDistancesMainRoute.get(outLink.getId().toString()) == null
					...
					weightedOutFlow = outFlow * this.outFlowDistancesMainRoute.get(outLink.getId().toString());
				}
...
//			ttToBottleNeck = getPredictedTravelTime(mainRoute, bottleNeckLinkMainRoute) - ttFreeSpeedAfterBottleNeckMainRoute;
		...
		//AlternativeRoute
			...
			distanceToBottleNeck = getDistanceFromFirstNode(this.bottleNeckLinkAlternativeRoute.getToNode(), this.routeLinksAlternativeRoute);
			...
			while(it3.hasNext()){
				...
				double weightedInFlow = 0;
				if(this.inFlowDistancesAlternativeRoute.get(inLink.getId().toString()) == null
						|| this.inFlowDistancesAlternativeRoute.get(inLink.getId().toString()) > distanceToBottleNeck){
					weightedInFlow = 0;
				}
				else{
					weightedInFlow = inFlow * this.inFlowDistancesAlternativeRoute.get(inLink.getId().toString());
				}
				weightedNetFlow += weightedInFlow;
			}
			...
			while(it4.hasNext()){
				...
				double weightedOutFlow = 0;
				if(this.outFlowDistancesAlternativeRoute.get(outLink.getId().toString()) == null
						|| this.outFlowDistancesAlternativeRoute.get(outLink.getId().toString()) > distanceToBottleNeck){
						weightedOutFlow = 0;
				}
				else{
					weightedOutFlow = outFlow * this.outFlowDistancesAlternativeRoute.get(outLink.getId().toString());
				}
				weightedNetFlow -= weightedOutFlow;
			}
			netFlow = (weightedNetFlow / distanceToBottleNeck);
