...
public class TraVolCal extends FinalEventFilterA {
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.matsim.demandmodeling.filters.filter.finalFilters.FinalEventFilterA#UDAexport()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.matsim.demandmodeling.filters.filter.finalFilters.FinalEventFilterA#UDAWexport()
	 */
	@Override
	...
	public Map<String, List<Double>> LmUDAWexport() {
		...
		for (String linkID : this.traVols.keySet()) {
			...
			for (UserDefAtt uda : this.udas) {
				String[] s = uda.getATTID().split("TO");
				double Lm = 37.3 + 10.0 * Math.log10(this.traVols.get(linkID).get(
