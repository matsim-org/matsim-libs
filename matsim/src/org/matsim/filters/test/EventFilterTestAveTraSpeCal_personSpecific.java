...
import org.matsim.filters.filter.PersonIDFilter;
...
public class EventFilterTestAveTraSpeCal_personSpecific {
	...
	public static void testRunAveTraSpeCal() throws IOException {
		...
		PrintStreamUDANET psUdaNet = new PrintStreamUDANET(
				config.getParam("attribut_aveTraSpe", "outputAttNetFile"));
		psUdaNet.output(atsc);
		psUdaNet.close();
		System.out.println("\tdone.");

		System.out.println("\tprinting attributsFile of link...");
		PrintStreamLinkATT psLinkAtt = new PrintStreamLinkATT(
				config.getParam("attribut_aveTraSpe", "outputAttFile"), network);
