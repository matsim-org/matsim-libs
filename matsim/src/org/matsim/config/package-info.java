 ...
 * 
 * Each parameter is part of exactly one {@linkplain org.matsim.config.groups config-group}. Such a group
 * collects similar parameters, or different parameters for one specific part of MATSim. The config-groups
 * defined in the subpackage {@link org.matsim.config.groups} are considered "core" config-groups, which
 ...
 *   &lt;/module>
 * </pre>
 * These settings are then stored in an instance of {@link org.matsim.config.Module}, which just stores
 ...
 * 
 * To write a custom config-group, extend {@link org.matsim.config.Module}. This allows you to convert
 ...
 * methods:
 * <ul>
 * <li>{@link org.matsim.config.Module#addParam(String, String)} to store the values read from a 
 * 		configuration file.</li>
 * <li>{@link org.matsim.config.Module#getParams()} to return a map containing all the parameter names and 
 * 		their values to be stored in a configuration file.</li>
 * <li>{@link org.matsim.config.Module#getValue(String)} should be implemented as well for compatibility
 ...
 * <li>{@link org.matsim.config.Module#getName()} should return the name of your config-group, as it
 *    appears in the configuration file.</li>
 * <li>Optionally: {@link org.matsim.config.Module#checkConsistency()} to check that all the settings make
 *    some sense together after being read from a file.</li> 
 * </ul> 
 * The custom config-group must be 
 * {@linkplain org.matsim.config.Config#addModule(String, Module) added} to the 
 * {@link org.matsim.config.Config}-object before the configuration is read from file, because
 * otherwise a generic {@link org.matsim.config.Module} will be created for these settings. The 
 * {@linkplain org.matsim.controler controler-documenation} has an example of how to use custom
 * config-groups with additional functionality provided by 
 * {@link org.matsim.controler.listener.ControlerListener}s in the "Best Practices" section.
 * 
 * @author mrieser
 */
