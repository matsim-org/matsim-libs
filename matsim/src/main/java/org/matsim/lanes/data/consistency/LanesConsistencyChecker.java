...
public class LanesConsistencyChecker {
  ...
  public void checkConsistency() {
    ...
    log.info(
        "checked consistency. Lanes on "
            + linksWithMalformedLanes.size()
  ...
  private boolean areLanesOnLinkConsistent(LanesToLinkAssignment l2l) {
    ...
    if (!this.network.getLinks().containsKey(l2l.getLinkId())) {
      log.error(
          "No link found for lanesToLinkAssignment on link Id(linkIdRef): " + l2l.getLinkId());
      return false;
    }
    // check length
    else {
      ...
      for (Lane l : l2l.getLanes().values()) {
        if (link.getLength() < l.getStartsAtMeterFromLinkEnd()) {
          log.error(
              "Link Id " + link.getId() + " is shorter than an assigned lane with id " + l.getId());
          return false;
        }
      }
    }
                  ...
                  if (mergedLinks.keySet().contains(toLinkId)) {
                    ...
                    log.info(
                        "Replace ToLinks of Lane Id: "
                            ...
                            + toLinkId.toString()
                            + " with "
                            + mergedLinks.get(toLinkId).toString());
                  }
                }
              }
            }
    ...
    // More precisely: check if every Outlink is reachable (and if not delete all lanes at this
    // junction)
    ...
    for (Id<Node> jn : junctionNodes.keySet()) {
      // At a junction if the size of the set filled above equals the number of outLinks everything
      // is fine
      if (network.getNodes().get(jn).getOutLinks().keySet().size()
          == junctionNodes.get(jn).size()) {
        checkedJn.add(jn);
        continue;
      }
      // If there is one inLink of that junction which has no Lanes (i.e connects every Outlink) -
      // the junction is fine
      ...
      if (lane.getToLaneIds() != null) {
        ...
        while (toLaneIdIterator.hasNext()) {
          ...
          if (!l2l.getLanes().containsKey(toLaneId)) {
            ...
            //						return false; // do not return false because this would remove the whole l2l on
            // this link
      ...
      if (lane.getToLinkIds() != null) {
        ...
        while (toLinkIdIterator.hasNext()) {
          ...
          if (!this.network.getLinks().containsKey(toLinkId)) {
            log.error(
                "No link found in network for toLinkId "
                    + toLinkId
                    + " of laneId "
                    + lane.getId()
                    + " of link id "
            ...
            if (!link.getToNode().getOutLinks().containsKey(toLinkId)) {
              log.error(
                  "The given toLink "
                      + toLinkId
                      + " is not reachable from lane "
                      + lane.getId()
                      + " on link "
                      + link.getId());
              //							return false; // do not return false because this would remove the whole l2l
              // on this link
              // delete this toLink from the lane
              if (this.removeMalformed) {
                toLinkIdIterator.remove();
    ...
    // comment this out, because not every out-link of a node has to be reached by every in-link.
    // theresa, aug'17
    ...
    //				log.error("\t\tThe lanes of link " + link.getId() + " do not lead to all of the outlinks
    // of the links toNode " + link.getToNode().getId() + " . The outlink " + nodeOutLink.getId()
