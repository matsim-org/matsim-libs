...
public final class LanesUtils {
  ...
  /**
   * Convenience method to create a lane with the given Id, the given length, the given capacity,
   * the given number of represented lanes, the given alignment and the given Ids of the downstream
   * links or lanes, respectively, the lane leads to. The lane is added to the LanesToLinkAssignment
   * given as parameter.
   *
   * @param l2l the LanesToLinkAssignment to that the created lane is added
   * @param factory a LaneDefinitionsFactory to create the lane
   * @param laneId
   * @param capacity
   ...
   * @param toLaneIds
   */
  public static void createAndAddLane(
  ...
  /**
   * Replaces the method that converted a lane from format 11 to format 20. Use this when you have
   * not defined an original lane of the link and when you have not set lane capacities yet.
   */
   ...
   */
  public static List<ModelLane> createLanes(
      Link link, LanesToLinkAssignment lanesToLinkAssignment) {
    ...
    sortedLanes.sort(
        Comparator.comparingDouble(Lane::getStartsAtMeterFromLinkEnd)
            .thenComparing(
                (l1, l2) -> {
                  ...
                  if (l1Outgoing && !l2Outgoing) return -1;
                  else if (l2Outgoing && !l1Outgoing) return 1;
                  else return 0;
                }));
   ...
   *
   * <p>Flow of a Lane is given by the flow of the link divided by the number of lanes represented
   * by the link.
   *
   * <p>A Lane may represent one or more lanes in reality. This is given by the attribute
   * numberOfRepresentedLanes of the Lane definition. The flow of a lane is scaled by this number.
   */
  public static void calculateAndSetCapacity(
      Lane lane, boolean isLaneAtLinkEnd, Link link, Network network) {
    if (isLaneAtLinkEnd) {
      double noLanesLink = link.getNumberOfLanes();
      double linkFlowCapPerSecondPerLane =
          link.getCapacity() / network.getCapacityPeriod() / noLanesLink;
      double laneFlowCapPerHour =
          lane.getNumberOfRepresentedLanes() * linkFlowCapPerSecondPerLane * 3600.0;
  ...
  public static void calculateMissingCapacitiesForLanes20(
      String networkInputFilename, String lanes20InputFilename, String lanes20OutputFilename) {
