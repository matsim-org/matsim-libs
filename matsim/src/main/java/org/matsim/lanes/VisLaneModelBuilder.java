...
public final class VisLaneModelBuilder {
  ...
  public void recalculatePositions(
      VisLinkWLanes linkData, SnapshotLinkWidthCalculator linkWidthCalculator) {
    ...
    if (linkData.getLaneData() == null || linkData.getLaneData().isEmpty()) {
      ...
      //			log.error("link " + linkData.getLinkId() + " without lanes starts at " +
      // linkData.getLinkStartCenterPoint() + " and ends at " + x + " " + y + " but " +
      // linkData.getLinkEnd());
    } else {
      double numberOfLinkParts = (2 * linkData.getMaximalAlignment()) + 2;
      //			log.error("link with lanes " + linkData.getLinkId() + " starts at " +
      // linkData.getLinkStart() + " and ends at " + linkData.getLinkEnd());
      for (VisLane lane : linkData.getLaneData().values()) {
        double horizontalFraction = 0.5 - (lane.getAlignment() / numberOfLinkParts);
        Point2D.Double laneStart =
            calculatePointOnLink(linkData, lane.getStartPosition(), horizontalFraction);
        Point2D.Double laneEnd =
        ...
        if (lane.getNumberOfLanes() >= 2.0) {
          ...
          for (int i = 1; i <= noLanesFloor; i++) {
            Point2D.Double drivingLaneStart =
                this.calcPoint(laneStart, linkData.getLinkOrthogonalVector(), laneOffset);
            Point2D.Double drivingLaneEnd =
                this.calcPoint(laneEnd, linkData.getLinkOrthogonalVector(), laneOffset);
  ...
  private Point2D.Double calculatePointOnLink(
      final VisLinkWLanes laneLinkData, final double position, final double horizontalFraction) {
    Point2D.Double lenghtPoint =
        this.calcPoint(
            laneLinkData.getLinkStart(), laneLinkData.getNormalizedLinkVector(), position);
    return this.calcPoint(
        lenghtPoint,
        laneLinkData.getLinkOrthogonalVector(),
        horizontalFraction * laneLinkData.getLinkWidth());
  }
  ...
  private VisLane createVisLane(
      ModelLane qlane, double linkLength, double linkScale, double linkLengthCorrectionFactor) {
    String id = qlane.getLaneData().getId().toString();
    double startPosition =
        (linkLength - qlane.getLaneData().getStartsAtMeterFromLinkEnd())
            * linkScale
            * linkLengthCorrectionFactor;
    double endPosition =
        startPosition + (qlane.getLength() * linkScale * linkLengthCorrectionFactor);
    //		log.error("lane " + qlane.getId() + " starts at: " + startPosition + " and ends at : "
    // +endPosition);
  ...
  public VisLinkWLanes createVisLinkLanes(
    ...
    //		log.debug("  fromNode: " + link.getLink().getFromNode().getId() + " coord: " +
    // link.getLink().getFromNode().getCoord());
    //		log.debug("  toNode: " + link.getLink().getToNode().getId() + " coord: " +
    // link.getLink().getToNode().getCoord());
    ...
    // first calculate the scale of the link based on the node offset, i.e. the link will be
    // shortened at the beginning and the end
    ...
    if (lanes != null) {
      ...
      for (ModelLane lane : lanes) {
        VisLane visLane =
            this.createVisLane(
                lane, link.getLink().getLength(), linkScale, linkLengthCorrectionFactor);
      ...
      for (ModelLane lane : lanes) {
        ...
        if (lane.getToLanes() == null || lane.getToLanes().isEmpty()) {
          ...
          for (ModelLane toLane : lane.getToLanes()) {
            VisLane otfToLane =
                lanesLinkData.getLaneData().get(toLane.getLaneData().getId().toString());
