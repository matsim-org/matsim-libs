...
/**
 * Generates Umlaeufe (= vehicle runs) from the transit schedule. Will do something interesting only
 * if {@link Departure}s in the {@link org.matsim.pt.transitSchedule.api.TransitSchedule} have
 * vehicle ids, <i>and</i> the same vehicle is used for multiple departures.
 *
...
public final class ReconstructingUmlaufBuilder implements UmlaufBuilder {
  ...
  private static final Comparator<UmlaufStueck> departureTimeComparator =
      new Comparator<UmlaufStueck>() {

        @Override
        public int compare(UmlaufStueck o1, UmlaufStueck o2) {
          return Double.compare(
              o1.getDeparture().getDepartureTime(), o2.getDeparture().getDepartureTime());
        }
      };
  ...
  public ReconstructingUmlaufBuilder(Scenario scenario) {
    ...
    this.umlaufInterpolator =
        new UmlaufInterpolator(scenario.getNetwork(), scenario.getConfig().scoring());
  ...
  /**
   * (connect multiple umlautStuecke to one umlauf. will do anything interesting only if same
   * vehicle is shared across multiple departures. vehicle id can be set for departures in transit
   * schedule. but usually isn't)
   */
  private void createUmlaeufe() {
    int cnt = 0;
    for (UmlaufStueck umlaufStueck : umlaufStuecke) {
      Id<Umlauf> umlaufId =
          this.getUmlaufIdForVehicleId(umlaufStueck.getDeparture().getVehicleId());
      if (umlaufId == null) {
  ...
  private Id<Umlauf> createUmlaufIdFromVehicle(Vehicle vehicle) {
    Id<Umlauf> id =
        Id.create(
            vehicle.getId().toString() + "_" + vehicle.getType().getId().toString(), Umlauf.class);
