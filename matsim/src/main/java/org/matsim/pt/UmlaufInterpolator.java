...
public final class UmlaufInterpolator {
  ...
  public UmlaufInterpolator(Network network, final ScoringConfigGroup config) {
    ...
    this.routingAlgo =
        new DijkstraFactory().createPathCalculator(network, travelTimes, travelTimes);
  }
  ...
  public void addUmlaufStueckToUmlauf(UmlaufStueckI umlaufStueck, Umlauf umlauf) {
    ...
    // check if final link of last umlaufStueck and first link of new umlaufStueck are connected;
    // otherwise compute and insert connecting route:
    if (!existingUmlaufStuecke.isEmpty()) {
      UmlaufStueckI previousUmlaufStueck =
          existingUmlaufStuecke.get(existingUmlaufStuecke.size() - 1);
