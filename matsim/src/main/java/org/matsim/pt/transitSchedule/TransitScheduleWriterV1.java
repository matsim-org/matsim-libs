...
/**
 * Writes a transit schedule to a XML file in the format described by <code>transitSchedule_v1.dtd
 * </code>.
 *
 * @author mrieser
 */
public class TransitScheduleWriterV1 extends MatsimXmlWriter implements MatsimSomeWriter {
  ...
  public TransitScheduleWriterV1(
      final CoordinateTransformation coordinateTransformation, final TransitSchedule schedule) {
  ...
  public void write(final String filename) throws UncheckedIOException {
    ...
    this.writeXmlHead();
    this.writeDoctype(
        Constants.TRANSIT_SCHEDULE, "http://www.matsim.org/files/dtd/transitSchedule_v1.dtd");
  ...
  private void writeRouteProfile(final List<TransitRouteStop> stops) throws UncheckedIOException {
    ...
    for (TransitRouteStop stop : stops) {
      ...
      stop.getDepartureOffset()
          .ifDefined(offset -> attributes.add(createTimeTuple(Constants.DEPARTURE_OFFSET, offset)));
      attributes.add(
  ...
  private void writeDepartures(final Map<Id<Departure>, Departure> departures)
