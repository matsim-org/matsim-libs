 ...
 *
 * <ul>
 *   <li>A transfer time defined from stop A to stop B does not define a transfer time from stop B
 *       to stop A.
 *   <li>A transfer time defined from stop A to stop B, and another defined from stop B to stop C,
 *       does not define a transfer time from stop A to stop C.
 * </ul>
 *
 * Both cases, the reverse transfer from stop B to stop A, and the indirect transfer from stop A to
 * stop C via stop B, must explicitly be set in order to be returned in {@link #get(Id, Id)}.
 *
 * @author mrieser / SBB
 */
public interface MinimalTransferTimes {
  ...
  /**
   * Sets the minimal transfer time in seconds needed to transfer from <code>fromStop</code> to
   * <code>toStop</code>.
   *
   * @param fromStop
   * @param toStop
   * @param seconds the minimal transfer time, in seconds
   * @return the minimal transfer time previously assigned between the two stops, <code>Double.NaN
   ...
   * @param fromStop
   * @param toStop
   * @return the minimal transfer time between the two stops if defined, <code>Double.NaN</code> if
   *     none is set.
   */
   ...
   * @return the minimal transfer time between the two stops if defined, <code>defaultSeconds</code>
   *     if none is set.
   */
  double get(
