...
/**
 * Writes a transit schedule to a XML file in the format described by <code>transitSchedule_v2.dtd
 * </code>.
 *
 * @author mrieser
 */
public class TransitScheduleWriterV2 extends MatsimXmlWriter implements MatsimSomeWriter {
  ...
  private void writeData() throws IOException, UncheckedIOException {
    this.writeXmlHead();
    this.writeDoctype(
        Constants.TRANSIT_SCHEDULE, "http://www.matsim.org/files/dtd/transitSchedule_v2.dtd");
  ...
  private void writeMinimalTransferTimes() {
    ...
    MinimalTransferTimes.MinimalTransferTimesIterator iter =
        this.schedule.getMinimalTransferTimes().iterator();
    if (iter.hasNext()) {
  ...
  private void writeTransitRoute(final TransitRoute route)
