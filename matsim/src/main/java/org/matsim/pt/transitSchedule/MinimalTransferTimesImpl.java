...
class MinimalTransferTimesImpl implements MinimalTransferTimes {
  ...
  private Map<Id<TransitStopFacility>, Map<Id<TransitStopFacility>, Double>> minimalTransferTimes =
      new ConcurrentHashMap<>();

  @Override
  public double set(
      Id<TransitStopFacility> fromStop, Id<TransitStopFacility> toStop, double seconds) {
    if (Double.isNaN(seconds)) {
      return remove(fromStop, toStop);
    }
    Map<Id<TransitStopFacility>, Double> innerMap =
        this.minimalTransferTimes.computeIfAbsent(fromStop, key -> new ConcurrentHashMap<>());
  ...
  @Override
  public double get(
      Id<TransitStopFacility> fromStop, Id<TransitStopFacility> toStop, double defaultSeconds) {
  ...
  private static class MinimalTransferTimesIteratorImpl implements MinimalTransferTimesIterator {
    ...
    private Iterator<Map.Entry<Id<TransitStopFacility>, Map<Id<TransitStopFacility>, Double>>>
        outerIterator;
    private Iterator<Map.Entry<Id<TransitStopFacility>, Double>> innerIterator;

    MinimalTransferTimesIteratorImpl(
        Map<Id<TransitStopFacility>, Map<Id<TransitStopFacility>, Double>> values) {
    ...
    public boolean hasNext() {
      ...
      while (this.outerIterator.hasNext()) {
        Map.Entry<Id<TransitStopFacility>, Map<Id<TransitStopFacility>, Double>> outerEntry =
