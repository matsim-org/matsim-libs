...
/**
 * Allows fast queries of a TransitSchedule for the next departure of a given route at a given stop,
 * from a given point in time. If you need further queries of a TransitSchedule, put them here!
 *
 * <p>(I renamed this class and put the TransitSchedule in the constructor to make the purpose
 * clear. michaz '13)
 *
 * <p>Thread-safe.
 *
...
public class PreparedTransitSchedule {
  ...
  static final double MIDNIGHT = 24.0 * 3600;
  ...
  public double getNextDepartureTime(
      final TransitRoute route, final TransitRouteStop stop, final double depTime) {
    ...
    if (earliestDepartureTimeAtTerminus < 0) {
      // this may happen when depTime < departureOffset, e.g. I want to start at 24:03, but the bus
      // departs at 23:55 at terminus
    ...
    if (pos < 0) {
      // (if the departure time is not found _exactly_, binarySearch returns (-(insertion point) -
      // 1).  That is
