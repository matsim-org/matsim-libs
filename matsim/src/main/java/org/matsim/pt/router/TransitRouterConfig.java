 ...
 * Design decisions:
 * <ul>
 * <li>At this point, only those variables that are needed elsewhere (in
 * particular in the scoring) are set from the config. All other variables are
 * considered internal computational variables. kai, feb'11
 * </ul>
 *
 */
public class TransitRouterConfig implements MatsimParameters {
	...
	/**
	 * The distance in meters in which stop facilities should be searched for around
	 ...
	 * {@link #searchRadius}), the nearest stop location is searched for and the
	 * distance from start/end coordinate to this location is extended by the given
	 * amount.<br />
	 * If only one stop facility is found within {@link #searchRadius}, the radius
	 * is also extended in the hope to find more stop facilities (e.g. in the
	 * opposite direction of the already found stop).
	 */
	private double extensionRadius = 200.0;

	/**
	 * The distance in meters that agents can walk to get from one stop to another
	 * stop of a nearby transit line.
	 * <p>
	 * </p>
	 * Is this really needed? If the marg utl of walk is correctly set, this should
	 * come out automagically. kai, feb'11 This value is used to generate the walk
	 * connections between stop facilities. If they are used, depends on the
	 * scoring/cost calculation. But when they are missing, they cannot be used at
	 * all. mrieser, mar'11
	 */
	private double beelineWalkConnectionDistance = 100.0;

	/**
	 * The minimum time needed for a transfer is calculated based on the distance
	 * and the beeline walk speed between two stop facilities. Due to passengers
	 * probably not being able to immediately (=in the same second) leave a transit
	 * vehicle, or a vehicle being delayed by a small amount of time, an additional
	 * "savety" time can be added to transfers when searching for connecting trips.
	 * This could help to find "better" transfer connections such as they can indeed
	 * be realized by the simulation. This value only affects the routing process,
	...
	private final Map<String, UtilityParameters> parametersPerSubpopulation = new HashMap<>();
	...
	public TransitRouterConfig(final PlanCalcScoreConfigGroup pcsConfig, final PlansCalcRouteConfigGroup pcrConfig,
		...
		if (pcsConfig.getScoringParametersPerSubpopulation().size() > 1) {
			Logger.getLogger(getClass())
					.warn("More than one subpopulation is used in plansCalcScore. "
		...
		// yyyyyy the two above need to be moved away from walk since otherwise one is
		// not able to move walk routing to network routing!!!!!! Now trying access_walk
		// ... kai,
		// apr'19

		for (ScoringParameterSet scoringParameters : pcsConfig.getScoringParametersPerSubpopulation().values()) {
			parametersPerSubpopulation.put(
				scoringParameters.getSubpopulation(),
				new UtilityParameters(scoringParameters));
		}
	...
	public UtilityParameters getUtilityParameters(final String subpopulation) {
		return this.parametersPerSubpopulation.get(subpopulation);
	}
	...
	public static class UtilityParameters {
		...
		public UtilityParameters(final PlanCalcScoreConfigGroup.ScoringParameterSet pcsConfig) {
