 ...
 *
 * <ul>
 *   <li>At this point, only those variables that are needed elsewhere (in particular in the
 *       scoring) are set from the config. All other variables are considered internal computational
 *       variables. kai, feb'11
 * </ul>
 */
public class TransitRouterConfig implements MatsimParameters {
  ...
  /**
   * The distance in meters in which stop facilities should be searched for around the start and end
   * coordinate.
   */
  private double searchRadius = 1000.0;

  /**
   * If no stop facility is found around start or end coordinate (see {@link #searchRadius}), the
   * nearest stop location is searched for and the distance from start/end coordinate to this
   * location is extended by the given amount.<br>
   * If only one stop facility is found within {@link #searchRadius}, the radius is also extended in
   * the hope to find more stop facilities (e.g. in the opposite direction of the already found
   * stop).
   */
   ...
   * transit line.
   *
   * <p>Is this really needed? If the marg utl of walk is correctly set, this should come out
   * automagically. kai, feb'11 This value is used to generate the walk connections between stop
   * facilities. If they are used, depends on the scoring/cost calculation. But when they are
   * missing, they cannot be used at all. mrieser, mar'11
   */
  private double beelineWalkConnectionDistance = 100.0;

  /**
   * The minimum time needed for a transfer is calculated based on the distance and the beeline walk
   * speed between two stop facilities. Due to passengers probably not being able to immediately
   * (=in the same second) leave a transit vehicle, or a vehicle being delayed by a small amount of
   * time, an additional "savety" time can be added to transfers when searching for connecting
   * trips. This could help to find "better" transfer connections such as they can indeed be
   * realized by the simulation. This value only affects the routing process, not the simulation
   * itself.
   */
  ...
  public TransitRouterConfig(
      final ScoringConfigGroup pcsConfig,
      final RoutingConfigGroup pcrConfig,
    ...
    if (pcsConfig.getScoringParametersPerSubpopulation().size() > 1) {
      LogManager.getLogger(getClass())
          .warn(
              "More than one subpopulation is used in plansCalcScore. "
    ...
    // yyyyyy the two above need to be moved away from walk since otherwise one is not able to move
    // walk routing to network routing!!!!!! Now trying access_walk ...  kai,
    // apr'19

    this.marginalUtilityOfTravelTimeWalk_utl_s =
        pcsConfig.getModes().get(TransportMode.walk).getMarginalUtilityOfTraveling() / 3600.0
            - pcsConfig.getPerforming_utils_hr() / 3600.;

    this.marginalUtilityOfTravelDistanceWalk_utl_m =
        pcsConfig.getMarginalUtilityOfMoney()
    ...
    // pt:
    this.marginalUtilityOfTravelTimeTransit_utl_s =
        pcsConfig.getModes().get(TransportMode.pt).getMarginalUtilityOfTraveling() / 3600.0
            - pcsConfig.getPerforming_utils_hr() / 3600.;
    ...
    this.marginalUtilityOfWaitingPt_utl_s =
        pcsConfig.getMarginalUtlOfWaitingPt_utils_hr() / 3600.0
            - pcsConfig.getPerforming_utils_hr() / 3600.;
   ...
   *
   * <p>The "_utl" can go as soon as we are confident that there are no more utilities in "Eu". kai,
   * feb'11
   */
  ...
  public void setMarginalUtilityOfTravelTimeWalk_utl_s(
      final double marginalUtilityOfTravelTimeWalk_utl_s) {
  ...
  public void setMarginalUtilityOfTravelTimePt_utl_s(
      final double marginalUtilityOfTravelTimeTransit_utl_s) {
    this.marginalUtilityOfTravelTimeTransit_utl_s = marginalUtilityOfTravelTimeTransit_utl_s;
  }

  public void setMarginalUtilityOfTravelDistanceWalk_utl_m(
      final double marginalUtilityOfTravelDistanceWalk_utl_m) {
  ...
  public void setMarginalUtilityOfTravelDistancePt_utl_m(
      final double marginalUtilityOfTravelDistanceTransit_utl_m) {
    this.marginalUtilityOfTravelDistanceTransit_utl_m =
  ...
  public void setMarginalUtilityOfWaitingPt_utl_s(
      final double effectiveMarginalUtilityOfWaiting_utl_s) {
