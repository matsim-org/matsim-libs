...
/**
 * Builds a network where transit vehicles can drive along and assigns the correct links to the
 * transit stop facilities and routes of transit lines. As each transit stop facility can only be
 * connected to at most one link, the algorithm is forced to duplicated transit stop facilities in
 * certain cases to build the network.
 *
 * @author mrieser
 */
public class CreatePseudoNetwork {
  ...
  private final Map<Tuple<Node, Node>, TransitStopFacility> stopFacilities =
      new HashMap<Tuple<Node, Node>, TransitStopFacility>();
  ...
  public CreatePseudoNetwork(
      final TransitSchedule schedule, final Network network, final String networkIdPrefix) {
  ...
  public void createNetwork() {
    ...
    List<Tuple<TransitLine, TransitRoute>> toBeRemoved =
  ...
  private Link getNetworkLink(final TransitRouteStop fromStop, final TransitRouteStop toStop) {
    TransitStopFacility fromFacility =
        (fromStop == null) ? toStop.getStopFacility() : fromStop.getStopFacility();
    ...
    if (fromNode == null) {
      fromNode =
          this.network
              .getFactory()
              .createNode(
                  Id.create(this.prefix + toFacility.getId(), Node.class), fromFacility.getCoord());
    ...
    if (toNode == null) {
      toNode =
          this.network
              .getFactory()
              .createNode(
                  Id.create(this.prefix + toFacility.getId(), Node.class), toFacility.getCoord());
    ...
    if (link == null) {
      ...
      if (toFacility.getLinkId() == null) {
        ...
        Id<TransitStopFacility> newId =
            Id.create(
                toFacility.getId().toString() + "." + Integer.toString(copies.size() + 1),
                TransitStopFacility.class);
        TransitStopFacility newFacility =
            this.schedule
                .getFactory()
                .createTransitStopFacility(
                    newId, toFacility.getCoord(), toFacility.getIsBlockingLane());
  ...
  private Link createAndAddLink(Node fromNode, Node toNode, Tuple<Node, Node> connection) {
    ...
    link =
        this.network
            .getFactory()
            .createLink(
                Id.create(this.prefix + this.linkIdCounter++, Link.class), fromNode, toNode);
    if (fromNode == toNode) {
      link.setLength(50);
    } else {
  ...
  public Link getLinkBetweenStops(
      final TransitStopFacility fromStop, final TransitStopFacility toStop) {
