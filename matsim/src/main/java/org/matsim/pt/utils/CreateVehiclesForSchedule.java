...
/**
 * Creates a vehicle of type "defaultTransitVehicleType" for each departure. Useful for tests and
 * demos.
 *
 * @author mrieser
 */
public class CreateVehiclesForSchedule {
  ...
  public void run() {
    ...
    VehicleType vehicleType =
        vb.createVehicleType(Id.create("defaultTransitVehicleType", VehicleType.class));
    ...
    for (TransitLine line : this.schedule.getTransitLines().values()) {
      for (TransitRoute route : line.getRoutes().values()) {
        for (Departure departure : route.getDepartures().values()) {
          Vehicle veh =
              vb.createVehicle(
                  Id.create("tr_" + Long.toString(vehId++), Vehicle.class), vehicleType);
