...
/**
 * Calculates the number of passenger that are in a transit vehicle when the vehicle departs a stop
 * location.
 *
 * @author mrieser
 */
public class TransitLoad
  ...
  public List<StopInformation> getDepartureStopInformation(
      final TransitLine line,
      final TransitRoute route,
      final TransitStopFacility stopFacility,
      final Departure departure) {
    return getStopInformation(
        line.getId(), route.getId(), stopFacility.getId(), departure.getId(), false);
  }

  @Override
  public void handleEvent(TransitDriverStartsEvent event) {
    this.vehicleData.put(
        event.getVehicleId(),
        new VehicleData(
            event.getVehicleId(),
    ...
    if (vData != null) {
      List<StopInformation> siList =
          getStopInformation(
              vData.lineId,
              vData.routeId,
              this.vehicleFacilityMap.get(event.getVehicleId()),
              vData.departureId,
              true);
      // Vehicle arrives at facility -> begin of serving the stop (once more) -> create new
      // StopInformation
  ...
  public void handleEvent(final VehicleDepartsAtFacilityEvent event) {
    ...
    if (vData != null) {
      List<StopInformation> siList =
          getStopInformation(vData.lineId, vData.routeId, stopId, vData.departureId, true);
  ...
  private List<StopInformation> getStopInformation(
  ...
  /*package*/ static class RouteData {
    public final Map<Id<TransitStopFacility>, StopData> stopData =
        new HashMap<>(); // use stopFacilityId as key
  }

  /*package*/ static class StopData {
    public final Map<Id<Departure>, List<StopInformation>> departureData =
        new HashMap<>(); // use departure id as key
  }
