...
public class PlansCalcAreaTollRoute extends PlansCalcRoute {
	 ...
	 * @param scheme
	 */
	public PlansCalcAreaTollRoute(PlansCalcRouteConfigGroup configGroup, final Network network, final PersonalizableTravelCost costCalculator, final TravelTime timeCalculator,
	...
	protected void handlePlan(Person person, final Plan plan) {
		...
		for (int i = 2, n = actslegs.size(); i < n; i += 2) {
			...
			if (!isCarLeg[routeIndex]) {
				...
				NetworkRoute tollRoute = (NetworkRoute) ((NetworkLayer) this.network).getFactory().createRoute(TransportMode.car, fromLink.getId(), toLink.getId());
				...
				if (tollRouteInsideTollArea && !agentPaysToll) {
					...
					noTollRoute = (NetworkRoute) ((NetworkLayer) this.network).getFactory().createRoute(TransportMode.car, fromLink.getId(), toLink.getId());
