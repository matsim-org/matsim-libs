...
/**
 * Creates a table - containing the analysis of one or two plans files and a table - containing the
 * average values of the selected plans of all persons
 *
 * @author anhorni
 */
public class CreateSelectedPlansTables {
  ...
  /**
   * @param args array with 2 or 3 entries: - path to network file: required - path to plans file 0:
   *     required - path to plans file 1: optional
   */
  ...
  private void readPlansFile(final String plansfilePath, final Population plans) {
    ...
    final MatsimReader plansReader = new PopulationReader(new PseudoScenario(this.scenario, plans));
  ...
  private void writeSummaryFile() {
    try {
      ...
      for (final Id<Person> person_id : this.plans0.getPersons().keySet()) {
        ...
        this.sumLegTraveldistance[0] +=
            (this.getTravelDist(person) / this.getNumberOfTrips(person));
        ...
        if (this.twoPlans) {
          ...
          this.sumLegTraveltime[1] +=
              (this.getTravelTime(person_comp) / this.getNumberOfTrips(person_comp));
          this.sumLegTraveldistance[1] +=
              (this.getTravelDist(person_comp) / this.getNumberOfTrips(person_comp));
        }
        out.write('\n');
        out.flush();
      }
  ...
  private void run(
      final String networkPath, final String plansfilePath0, final String plansfilePath1) {
  ...
  /**
   * Provides a real scenario, but exchanges the population. Still, network and facilities can be
   * reused that way.
   *
   * @author mrieser
   */
