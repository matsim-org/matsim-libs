...
final class PopulationSampler extends JDialog {
  ...
  private static final Logger log = LogManager.getLogger(PopulationSampler.class);
  ...
  private void setupComponents() {
    this.btnChoose.addActionListener(
        e -> {
    ...
    this.btnCreateSample.addActionListener(
        e -> {
          createSample();
  ...
  private void createSample() {
    ...
    final String namePart =
        srcFilename.substring(0, srcFilename.toLowerCase(Locale.ROOT).lastIndexOf(".xml"));
    ...
    chooser.setSelectedFile(
        new File(srcFile.getParentFile(), namePart + "." + percentage + "pct.xml.gz"));
  ...
  private void doCreateSample(
      File inputPopulationFile, File networkFile, double samplesize, File outputPopulationFile) {
            ...
            () -> {
              ...
              try (FileInputStream fis = new FileInputStream(inputPopulationFile);
                  BufferedInputStream is =
                      getBufferedInputStream(inputPopulationFile.getName(), fis)) {
                ...
                if (e instanceof RuntimeException && networkFile == null) {
                  // just making a guess that providing the corresponding network will solve the
                  // issue
                  SwingUtilities.invokeLater(
                      () -> {
                        JOptionPane.showMessageDialog(
  ...
  private static BufferedInputStream getBufferedInputStream(String filename, FileInputStream fis)
      throws IOException {
