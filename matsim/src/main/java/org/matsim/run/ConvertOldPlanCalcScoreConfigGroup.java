...
final class OldToNewPlanCalcScoreConfigGroup extends ConfigGroup {
  ...
  @Deprecated private static final String MONETARY_DISTANCE_COST_RATE = "monetaryDistanceCostRate";
  ...
  private Double waitingPt =
      null; // if not actively set by user, it will later be set to "travelingPt".

  // ---

  private static final String USING_OLD_SCORING_BELOW_ZERO_UTILITY_DURATION =
  ...
  // ---
  private static final String FRACTION_OF_ITERATIONS_TO_START_SCORE_MSA =
  ...
  public void addParam(final String key, final String value) {
    ...
    if (key.startsWith(ACTIVITY_TYPE)) {
      ActivityParams actParams =
          getActivityTypeByNumber(key.substring(ACTIVITY_TYPE.length()), true);
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        FRACTION_OF_ITERATIONS_TO_START_SCORE_MSA,
    ...
    map.put(
        USING_OLD_SCORING_BELOW_ZERO_UTILITY_DURATION,
    ...
    map.put(
        WRITE_EXPERIENCED_PLANS,
        "write a plans file in each iteration directory which contains what each agent actually did, and the score it received.");

    return map;
  }

  private ActivityParams getActivityTypeByNumber(
      final String number, final boolean createIfMissing) {
  ...
  public Collection<ActivityParams> getActivityParams() {
    ...
    Collection<ActivityParams> collection =
        (Collection<ActivityParams>) getParameterSets(ActivityParams.SET_TYPE);
  ...
  public Map<String, ModeParams> getModes() {
    @SuppressWarnings("unchecked")
    final Collection<ModeParams> modes =
        (Collection<ModeParams>) getParameterSets(ModeParams.SET_TYPE);
  ...
  public void checkConsistency(Config config) {
    ...
    for (ActivityParams actType : this.getActivityParams()) {
      if (actType.isScoringThisActivityAtAll()) {
        ...
        if (actType.getOpeningTime().isDefined() && actType.getClosingTime().isDefined()) {
          ...
          if (actType.getOpeningTime().seconds() == 0.
              && actType.getClosingTime().seconds() > 24. * 3600 - 1) {
            log.error(
                "it looks like you have an activity type with opening time set to 0:00 and closing "
    ...
    if (this.getMarginalUtlOfWaiting_utils_hr() != 0.0) {
      log.warn(
          "marginal utl of wait set to: "
              + this.getMarginalUtlOfWaiting_utils_hr()
   ...
   *     <p>This was discouraged for some time but currently I think this makes sense. kai, mar'12
   */
  ...
  public void addActivityParams(final ActivityParams params) {
    ...
    if (previous != null) {
      if (previous.getActivityType().equals(PtConstants.TRANSIT_ACTIVITY_TYPE)) {
        log.error(
            "ERROR: Activity parameters for activity type "
                + previous.getActivityType()
        ...
        log.info(
            "activity parameters for activity type "
                + previous.getActivityType()
  ...
  public static class ActivityParams extends ReflectiveConfigGroup implements MatsimParameters {
    // in normal pgm execution, code will presumably lock instance of PlanCalcScoreConfigGroup, but
    // not instance of
    // ActivityParams.  I will try to pass the locked setting through the getters. kai, jun'15

    private static final String TYPICAL_DURATION_SCORE_COMPUTATION =
    ...
    public Map<String, String> getComments() {
      ...
      map.put(
          ...
          "method to compute score at typical duration.  Use "
              + TypicalDurationScoreComputation.uniform
  ...
  public void setMarginalUtlOfDistanceOther(double marginalUtlOfDistanceOther) {
    this.getModes()
        .get(TransportMode.other)
        .setMarginalUtilityOfDistance(marginalUtlOfDistanceOther);
  }
      ...
      if ((waiting != 0.) && (setWaitingCnt < 1)) {
        ...
        log.warn(
            "Setting betaWaiting different from zero is discouraged.  It is probably implemented correctly, "
                + "but there is as of now no indication that it makes the results more realistic."
                + Gbl.ONLYONCE);
      }
