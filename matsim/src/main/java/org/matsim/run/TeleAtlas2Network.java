...
public class TeleAtlas2Network {
  ...
  private static final void parseArguments(final String[] args) {
    ...
    while (argIter.hasNext()) {
      ...
      if (arg.equals("--xml")) {
        ...
        ignoreFrcType7onewayN = true;
      } else if (arg.equals("--maxfrc2l")) {
        ensureNextElement(argIter);
        try {
          maxFrcTypeForDoubleLaneLink = Integer.parseInt(argIter.next());
        } catch (Exception e) {
        ...
        ensureNextElement(argIter);
        try {
          minSpeedForNormalCapacity = Integer.parseInt(argIter.next());
        } catch (Exception e) {
        ...
        ensureNextElement(argIter);
        try {
          expansionRadius = Double.parseDouble(argIter.next());
        } catch (Exception e) {
        ...
        ensureNextElement(argIter);
        try {
          linkSeparation = Double.parseDouble(argIter.next());
        } catch (Exception e) {
    ...
    if (maxFrcTypeForDoubleLaneLink != Integer.MIN_VALUE) {
      reader.maxFrcTypeForDoubleLaneLink = maxFrcTypeForDoubleLaneLink;
    }
    if (minSpeedForNormalCapacity != Integer.MAX_VALUE) {
      reader.minSpeedForNormalCapacity = minSpeedForNormalCapacity;
    }
    // init the srModule
    ...
    if (mnShpFileName != null) {
      ...
      if (!Double.isNaN(expansionRadius)) {
        mrModule.expansionRadius = expansionRadius;
      }
      if (!Double.isNaN(linkSeparation)) {
        mrModule.linkSeparation = linkSeparation;
      }
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    //		// example arguments:
    //		// usage: TeleatlasParser [OPTIONS] jcShpFile nwShpFile [srDbfFile] [mnShpFile mpShpFile]
    // [outputDirectory]
    //		String options = "--xml --txt --shp --frc8 --frc7N --maxfrc2l 3 --minsnc 40 --radius
    // 0.000030 --offset 0.000005 --uturn ";
