...
public class Events2Snapshot {
  ...
  private void parseArguments(final String[] args) {
    ...
    while (argIter.hasNext()) {
      ...
      if (arg.equals("-h") || arg.equals("--help")) {
        ...
        if (arg.contains(".xml")) this.configfile = arg;
        else if (arg.contains("events")) this.eventsfile = arg;
        else {
  ...
  public void run(final String[] args) {
    ...
    //		if (((SimulationConfigGroup)
    // this.config.getModule(SimulationConfigGroup.GROUP_NAME)).getSnapshotPeriod() <= 0.0) {
  ...
  /**
   * Starts the conversion of events into snapshot files. Alternative run-method to more easily
   * integrate it into existing code where config, network etc. are already loaded.
   *
   * @param eventsFile
   * @param config
   * @param network
  ...
  private void prepare() {
    ...
    this.visualizer =
        new SnapshotGenerator(
            this.network, this.config.qsim().getSnapshotPeriod(), this.config.qsim());
