...
public class Count<T> implements Identifiable<T> {
   ...
   *
   * @param h indicating the hour-of-day. <b><i>Note: the hours for a counting station must be from
   *     1-24, and <b><i>not</i></b> from 0-23, otherwise the {@link MatsimCountsReader} will throw
   *     an error. </i></b>
  ...
  public final Volume createVolume(final int h, final double val) {
    if (h < 1) {
      throw new RuntimeException(
          "counts start at 1, not at 0.  If you have a use case where you need to go below one, "
  ...
  /**
   * @return Returns the exact coordinate, where this counting station is located, or null if no
   *     exact location is available.
   */
