...
public final class CountsSimRealPerHourGraph extends CountsGraph{
	...
	public CountsSimRealPerHourGraph(final List<CountSimComparison> ccl, final int iteration, final String filename){
		...
		while (l_it.hasNext()) {
			...
			if (cc.getHour()==hour) {
				...
				if (cc.getCountValue()>0.0 && cc.getSimulationValue()>0.0) {
					...
					comps.add(new Comp(realVal, "link"+cc.getId()+".html", "Link "+cc.getId()+"; " +
							"Count: "+ realVal +", Sim: "+ simVal));
				}
				else {
