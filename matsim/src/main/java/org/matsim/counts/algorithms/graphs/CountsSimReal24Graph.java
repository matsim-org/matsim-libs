...
public final class CountsSimReal24Graph extends CountsGraph {
  ...
  public CountsSimReal24Graph(
      final List<CountSimComparison> ccl, final int iteration, final String filename) {
  ...
  public JFreeChart createChart(final int nbr) {
    ...
    // yyyy PtCountsKMLWriterTest.testPtAlightKMLCreation never touches these and then leads to an
    // exception later
    ...
    dataset1.addSeries(
        "f05x", new double[][] {{200.0, maxCountValue}, {100.0, 0.5 * maxCountValue}});
    ...
    XYTextAnnotation annotation0 =
        new XYTextAnnotation("2.0 count", maxCountValue, 2 * maxCountValue);
    ...
    XYTextAnnotation annotation2 =
        new XYTextAnnotation("0.5 count", maxCountValue, 0.5 * maxCountValue);
