...
public class CountsReaderMatsimV1 extends MatsimXmlParser {
  ...
  private void startCount(final Attributes meta) {
    ...
    this.currcount =
        this.counts.createAndAddCount(Id.create(locId, Link.class), meta.getValue("cs_id"));
  ...
  private void startVolume(final Attributes meta) {
    if (this.currcount != null) {
      this.currcount.createVolume(
          Integer.parseInt(meta.getValue("h")), Double.parseDouble(meta.getValue("val")));
    }
  }
}
