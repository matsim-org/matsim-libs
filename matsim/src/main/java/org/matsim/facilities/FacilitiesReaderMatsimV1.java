...
final class FacilitiesReaderMatsimV1 extends MatsimXmlParser {
  ...
  public void startTag(
      final String name, final org.xml.sax.Attributes atts, final Stack<String> context) {
    if (FACILITIES.equals(name)) {
      ...
      currAttributes =
          context.peek().equals(FACILITIES)
              ? this.facilities.getAttributes()
              : this.currfacility.getAttributes();
  ...
  private void startOpentime(final Attributes atts) {
    this.curractivity.addOpeningTime(
