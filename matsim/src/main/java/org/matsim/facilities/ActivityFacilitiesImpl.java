...
public class ActivityFacilitiesImpl implements ActivityFacilities, SearchableActivityFacilities {
  ...
  private final IdMap<ActivityFacility, ActivityFacility> facilities =
      new IdMap<>(ActivityFacility.class); // FIXME potential iteration order change
  ...
  public final ActivityFacilityImpl createAndAddFacility(
      final Id<ActivityFacility> id, final Coord center) {
  ...
  @Override
  public final TreeMap<Id<ActivityFacility>, ActivityFacility> getFacilitiesForActivityType(
  ...
  public final void addActivityFacility(ActivityFacility facility) {
    ...
    if (this.facilities.containsKey(facility.getId())) {
      throw new IllegalArgumentException(
          "Facility with id=" + facility.getId() + " already exists.");
    }
  ...
  public String toString() {
    ...
    stb.append("]\n");
    for (Entry<Id<ActivityFacility>, ? extends ActivityFacility> entry :
        this.facilities.entrySet()) {
   ...
   */
  @Override
  public ActivityFacility getNearestFacility(final Coord coord) {
    if (this.facilitiesQuadTree == null) {
      buildQuadTree();
    }
   ...
   */
  @Override
  public Collection<ActivityFacility> getNearestFacilities(
      final Coord coord, final double distance) {
    if (this.facilitiesQuadTree == null) {
      buildQuadTree();
    }
