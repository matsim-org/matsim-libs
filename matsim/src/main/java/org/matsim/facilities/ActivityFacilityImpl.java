...
public class ActivityFacilityImpl
    implements ActivityFacility, MatsimDataClassImplMarkerInterface, Lockable {
  ...
  // yyyyyy I have to say that I am at this point not so happy.  Better make it final, make it
  // package-protected, make
  // all functionality accessible by interface or from static methods, and then let external users
  // use delegation.
  // People need to get un-used to casting things into the impl to get hold of "special"
  // functionality. kai, jul'16
   ...
   * @param center
   */
  protected ActivityFacilityImpl(
      final Id<ActivityFacility> id, final Coord center, final Id<Link> linkId) {
    this.id = id;
    this.coord = center;
    this.linkId = linkId;
  }
