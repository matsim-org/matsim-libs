...
public class WorldConnectLocations {
  ...
  private static final Logger log = LogManager.getLogger(WorldConnectLocations.class);
  ...
  private final void connectByFile(
      final ActivityFacilities facilities,
      final Network network,
      final String file,
      final Set<Id<ActivityFacility>> remainingFacilities) {
    ...
    log.info(
        "      number of facilities that are still not connected to a link = "
            + remainingFacilities.size());
    log.info(
        "    done. (connecting facilities with links via "
            + CONFIG_F2L_INPUTF2LFile
            + "="
            + file
            + ")");
  }
  ...
  private final void writeF2LFile(final ActivityFacilities facilities, final String file) {
    ...
    log.info(
        "    done. (writing f<-->l connections to  " + CONFIG_F2L_OUTPUTF2LFile + "=" + file + ")");
  }

  public final void connectFacilitiesWithLinks(
      final ActivityFacilities facilities, final Network network) {
    log.info("  connecting facilities with links...");

    Set<Id<ActivityFacility>> remainingFacilities =
        new HashSet<>(facilities.getFacilities().keySet());
    if (this.config != null) {
    ...
    log.info(
        "    connecting remaining facilities with links ("
            + remainingFacilities.size()
            + " remaining)...");
