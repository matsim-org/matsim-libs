...
public class FacilitiesSummary {
  ...
  public void run(ActivityFacilities facilities) {
    ...
    for (ActivityFacility f : facilities.getFacilities().values()) {
      f_cnt++;
      if (f.getCoord().getX() > max_coordX) {
        max_coordX = f.getCoord().getX();
      }
      if (f.getCoord().getY() > max_coordY) {
        max_coordY = f.getCoord().getY();
      }
      if (f.getCoord().getX() < min_coordX) {
        min_coordX = f.getCoord().getX();
      }
      if (f.getCoord().getY() < min_coordY) {
        min_coordY = f.getCoord().getY();
      }
