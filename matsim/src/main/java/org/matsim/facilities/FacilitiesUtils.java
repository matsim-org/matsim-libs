...
public class FacilitiesUtils {
   ...
   */
  public static SortedMap<Id<ActivityFacility>, ActivityFacility> getSortedFacilities(
  ...
  /**
   * Compare to {@link #decideOnLink(Facility, Network)}. Sometimes only the linkId is needed, and
   * often it is cheaper to obtain than the full link. Then call this method.
   *
   * @param facility
   * @param network
   * @return
  ...
  public static Id<Link> decideOnLinkId(final Facility facility, final Network network) {
    ...
    try {
      ...
      // there are implementations that throw an exception here although "null" is, in fact, an
      // interpretable value. kai, oct'18
    }
  ...
  /**
   * @deprecated Please use {@link MultimodalLinkChooser} instead
   */
  @Deprecated
  public static Link decideOnLink(final Facility facility, final Network network) {
    ...
    if (accessActLinkId != null) {
      ...
      // i.e. if street address is in mode-specific subnetwork, I just use that, and do not search
      // for another (possibly closer)
      // other link.

    }

    if (accessActLink == null) {
      // this is the case where the postal address link is NOT in the subnetwork, i.e. does NOT
      // serve the desired mode,
    ...
    // I just found out that there are facilities that insist on links that may not be postal
    // addresses since they cannot be reached by car.
  ...
  /**
   * Preferably use {@link FacilitiesUtils#toFacility(Activity, ActivityFacilities)}. The method
   * here is left in place if one wants to construct a wrapper decidedly without automagic. It
   * deliberately returns the interface.
   */
  ...
  public static Coord decideOnCoord(
      final Facility facility, final Network network, final Config config) {
    return decideOnCoord(facility, network, config.global().getRelativePositionOfEntryExitOnLink());
  }

  /** We have situations where the coordinate field in facility is not filled out. */
  public static Coord decideOnCoord(
      final Facility facility, final Network network, double relativePositionOfEntryExitOnLink) {
    ...
    return new Coord(
        fromCoord.getX() + relativePositionOfEntryExitOnLink * (toCoord.getX() - fromCoord.getX()),
        fromCoord.getY() + relativePositionOfEntryExitOnLink * (toCoord.getY() - fromCoord.getY()));
  }

  // Logic gotten from PopulationUtils, but I am actually a bit unsure about the value of those
  // methods now that
  // attributable is the only way to get attributes... td, aug'19
  // yy I would agree.  They are useful to manage the transition, but can be inlined afterwards.  I
  // would inline for all code we can reach, afterwards
  // resurrect them but mark as deprecated.  kai, nov'19

  public static <F extends Facility & Attributable> Object getFacilityAttribute(
      F facility, String key) {
    return facility.getAttributes().getAttribute(key);
  }

  public static <F extends Facility & Attributable> void putFacilityAttribute(
      F facility, String key, Object value) {
    facility.getAttributes().putAttribute(key, value);
  }

  public static <F extends Facility & Attributable> Object removeFacilityAttribute(
      F facility, String key) {
