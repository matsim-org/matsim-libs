...
/**
 * A reader for facilities-files of MATSim. This reader recognizes the format of the facilities-file
 * and uses the correct reader for the specific facilities-version, without manual setting.
 *
 * @author mrieser
 */
public class MatsimFacilitiesReader extends MatsimXmlParser {
  ...
  private static final String FACILITIES_V2 = "facilities_v2.dtd";

  private static final Logger log = LogManager.getLogger(MatsimFacilitiesReader.class);
  ...
  /**
   * Creates a new reader for MATSim facilities files. Coordinates are not converted
   *
  ...
  /**
   * Creates a new reader for MATSim facilities files. Converts the coordinates to the target CRS,
   * given the CRS information in the container attributes, or, if absent, from the config.
   *
   * @param targetCRS the CRS the coordinates should be expressed in
   * @param scenario The scenario containing the Facilities-object to store the facilities in.
   */
  public MatsimFacilitiesReader(final String targetCRS, final Scenario scenario) {
    this(
        scenario.getConfig().facilities().getInputCRS(),
        targetCRS,
   ...
   *
   * @param externalInputCRS specifies the CRS the coordinates are expressed in. If the CRS is
   *     define in the container attributes, this value is ignored
   * @param targetCRS the CRS the coordinates should be expressed in
   * @param facilities The ActivityFacilities-object to store the facilities in.
   */
  public MatsimFacilitiesReader(
      final String externalInputCRS, final String targetCRS, final ActivityFacilities facilities) {
