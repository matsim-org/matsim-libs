    ...
    private void parseLine(final String line) {
      ...
      if (this.state == STATE_DATA) {
        ...
        for (int i = 0; i < this.nofZones; i++) {
          this.matrix.setEntry(
              this.zoneNames[this.zoneCounter], this.zoneNames[i], Double.parseDouble(data[i]));
        }
        this.zoneCounter++;
  ...
  /*default*/ static class SparseMatrixReader {
    ...
    private void parseLine(final String line) {
      ...
      if (this.state == STATE_DATA) {
        ...
        if (data.length != 3) {
          throw new RuntimeException(
              "Expected 3 tokens, but found " + data.length + " in line " + this.lineCounter + ".");
        }
