...
public class VisumNetwork {
  ...
  public final Map<Id<TransitLine>, TransitLine> lines =
      new TreeMap<Id<TransitLine>, TransitLine>();
  ...
  public final Map<String, TimeProfileItem> timeProfileItems =
  ...
  public final Map<String, VehicleCombination> vehicleCombinations =
  ...
  public void addStopPoint(final StopPoint stopPt) {
    StopPoint oldStopPt =
        this.stopPoints.put(stopPt.stopAreaId.toString() + stopPt.id.toString(), stopPt);
    if (oldStopPt != null) {
      // there was already a stop point with the same id
      // re-do the insertion
  ...
  public void addLineRoute(final TransitLineRoute lr1) {
    TransitLineRoute oldlr =
        this.lineRoutes.put(
            lr1.lineName.toString() + "/" + lr1.id.toString() + "/" + lr1.DCode, lr1);
    if (oldlr != null) {
      // there was already a line route with the same id
      // re-do the insertion
      this.lineRoutes.put(
          oldlr.lineName.toString() + "/" + oldlr.id.toString() + "/" + oldlr.DCode, oldlr);
  ...
  public void addLineRouteItem(final LineRouteItem lri1) {
    LineRouteItem oldlri =
        this.lineRouteItems.put(
            lri1.lineName + "/" + lri1.lineRouteName + "/" + lri1.index + "/" + lri1.DCode, lri1);
    if (oldlri != null) {
      // there was already a Line Route Item with the same id
      // re-do the insertion
      this.lineRouteItems.put(
          oldlri.lineName + "/" + oldlri.lineRouteName + "/" + oldlri.index + "/" + oldlri.DCode,
          oldlri);
      throw new IllegalArgumentException(
          "There is already a route item with the same id."
              + oldlri.lineName
              + "/"
              + oldlri.lineRouteName
              + "/"
              + oldlri.index
              + "/"
              + oldlri.DCode);
  ...
  public void addTimeProfile(final TimeProfile tp1) {
    TimeProfile oldtp =
        this.timeProfiles.put(
            tp1.lineName.toString()
                + "/"
                + tp1.lineRouteName.toString()
                + "/"
                + tp1.dirCode
                + "/"
                + tp1.index.toString(),
    ...
    if (oldtp != null) {
      ...
      // re-do the insertion
      this.timeProfiles.put(
          oldtp.lineName.toString()
              + "/"
              + oldtp.lineRouteName.toString()
              + "/"
              + oldtp.dirCode
              + "/"
              + oldtp.index.toString(),
          oldtp);
      throw new IllegalArgumentException(
          "There is already a time profile with the same id."
              + oldtp.lineRouteName.toString()
              + "/"
              + oldtp.index.toString());
    }
  ...
  public void addTimeProfileItem(final TimeProfileItem tpi1) {
    TimeProfileItem oldtpi =
        this.timeProfileItems.put(
            tpi1.lineName
                + "/"
                + tpi1.lineRouteName
                + "/"
                + tpi1.timeProfileName
                + "/"
                + tpi1.dirCode
                + "/"
                + tpi1.index,
    ...
    if (oldtpi != null) {
      ...
      this.timeProfileItems.put(
          oldtpi.lineName
              + "/"
              + oldtpi.lineRouteName
              + "/"
              + oldtpi.timeProfileName
              + "/"
              + oldtpi.dirCode
              + "/"
              + oldtpi.index,
  ...
  public static class TransitLineRoute {
    ...
    public TransitLineRoute(
        final Id<TransitLineRoute> id, final Id<TransitLine> lineName, final String DCode) {
      this.id = id;
  ...
  public static class VehicleUnit {
    ...
    public VehicleUnit(
        final String id, final String name, final int seats, final int passengerCapacity) {
      this.id = id;
      this.name = name;
      this.seats = seats;
