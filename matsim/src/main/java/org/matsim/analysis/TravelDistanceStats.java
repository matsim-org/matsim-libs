 ...
 *
 * <ul>
 *   <li>average of the average leg distance per plan
 * </ul>
 *
 * Is used by the standard Controler and fed the "really executed" "plans" which are generated from
 * Events during the simulation and which are also used by the scoring. But you can also use it on
 * other kinds of plans from your own code.
 *
...
public class TravelDistanceStats {
  ...
  private static final Logger log = LogManager.getLogger(TravelDistanceStats.class);
  ...
  TravelDistanceStats(
    ...
    this(
        ...
        controlerIO.getOutputFilename("traveldistancestats"),
   ...
   * @param createPNG true if in every iteration, the distance statistics should be visualized in a
   *     graph and written to disk.
   * @throws UncheckedIOException
   */
  public TravelDistanceStats(
      final Config config, final String filename, final boolean createPNG, final String delimiter)
      throws UncheckedIOException {
    this(
        config.controller(), filename, filename + "legs", filename + "trips", createPNG, delimiter);
  }

  private TravelDistanceStats(
      ControllerConfigGroup controllerConfigGroup,
  ...
  public void addIteration(int iteration, IdMap<Person, Plan> map) {
    DoubleSummaryStatistics legStats =
        map
            .values()
            // TODO: This probably doesn't control how many threads parallelStream is using despite
            // the number of threads setting in config
            .parallelStream()
    ...
    DoubleSummaryStatistics tripStats =
        map.values().parallelStream()
            .flatMap(plan -> TripStructureUtils.getTrips(plan).stream())
            .mapToDouble(
                t -> {
                  Trip trip = (Trip) t;
                  return trip.getTripElements().stream()
                      .filter(Leg.class::isInstance)
                      .collect(
                          Collectors.summingDouble(
                              l -> {
                                Leg leg = (Leg) l;
                                // TODO NaN handling of Collectors.summingDouble will lead to many
                                // NaNs... rethink
                                return leg.getRoute() != null
    ...
    log.info(
        "-- average leg distance per plan (executed plans only): "
            + legStats.getAverage()
            + " meters");
    log.info(
        "average leg distance per Person (executed plans only): "
            + legStats.getSum() / map.size()
            + " meters (statistic on all "
            + legStats.getCount()
            + " legs which have a finite distance)");
    log.info(
        "-- average trip distance per plan (executed plans only): "
            + tripStats.getAverage()
            + " meters");
    log.info(
        "average trip distance per Person (executed plans only): "
            + tripStats.getSum() / map.size()
            + " meters (statistic on all "
            + tripStats.getCount()
