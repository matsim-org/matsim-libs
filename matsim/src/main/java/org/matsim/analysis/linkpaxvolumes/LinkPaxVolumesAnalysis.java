...
/**
 * Counts the number of vehicles _entering_ a link and the number of passengers on those vehicles,
 * aggregated into time bins of a specified size. Traditionally other analysis code tended to count
 * vehicles leaving a link, but in practice cars enter and leave the network at the link's end and
 * pt and drt passengers board and alight at the link's end, too. So it seems more precise to count
 * the travel distance starting from the link following the departure link and including the arrival
 * link. RouteUtils.calcDistance() solves this issue in a more precise way, but does not seem
 * applicable here.
 *
 * <p>On top this class counts the number of vehicles (i.e. vehicle ids) per vehicle type which
 * operated on the network and the time vehicles spent on the network.
 *
 * @author vsp-gleich (adapted from VolumesAnalyzer by mrieser, inspired by ikaddoura
 *     DynamicLinkDemandEventHandler in matsim-analysis)
 */
public final class LinkPaxVolumesAnalysis
  ...
  private static final Logger log = LogManager.getLogger(LinkPaxVolumesAnalysis.class);
  ...
  public LinkPaxVolumesAnalysis(
      Vehicles vehicles, Vehicles transitVehicles /* TODO: add some config settings */) {
  ...
  public void handleEvent(PersonEntersVehicleEvent event) {
    ...
    Map<String, Integer> passengersByMode =
        this.vehiclesData.get(event.getVehicleId()).passengerMode2currentPax;
  ...
  public void handleEvent(VehicleEntersTrafficEvent event) {
    ...
    if (!vehicleIdsSeen.contains(event.getVehicleId())) {
      ...
      this.vehicleType2numberSeen.put(
  ...
  public void handleEvent(final LinkEnterEvent event) {
    ...
    if (vehicleVolumesAll == null) {
      vehicleVolumesAll =
          new int[this.maxSlotIndex + 1]; // initialized to 0 by default, according to JVM specs
    ...
    if (this.observeNetworkModes) {
      ...
      Map<String, int[]> modeVehicleVolumes =
          this.linkVehicleVolumesPerNetworkMode.computeIfAbsent(
              event.getLinkId(), k -> new HashMap<>());
      ...
      Map<String, int[]> modePassengerVolumes =
          this.linkPaxVolumesPerNetworkMode.computeIfAbsent(
              event.getLinkId(), k -> new HashMap<>());
    ...
    if (this.observePassengerModes) {
      // There is no clear way how to count a vehicle which is used by passengers of different
      // passenger modes
      // Here we simply count multiple times the same vehicle (once per passenger mode)
      for (Map.Entry<String, Integer> passengerMode2modePax :
          vehicleData.passengerMode2currentPax.entrySet()) {

        Map<String, int[]> modeVehicleVolumes =
            this.linkVehicleVolumesPerPassengerMode.computeIfAbsent(
                event.getLinkId(), k -> new HashMap<>());
        ...
        Map<String, int[]> modePassengerVolumes =
            this.linkPaxVolumesPerPassengerMode.computeIfAbsent(
                event.getLinkId(), k -> new HashMap<>());
    ...
    if (this.observeVehicleTypes) {
      ...
      IdMap<VehicleType, int[]> typeVehicleVolumes =
          this.linkVehicleVolumesPerVehicleType.computeIfAbsent(
              event.getLinkId(), k -> new IdMap<>(VehicleType.class));
      ...
      IdMap<VehicleType, int[]> modePassengerVolumes =
          this.linkPaxVolumesPerVehicleType.computeIfAbsent(
              event.getLinkId(), k -> new IdMap<>(VehicleType.class));
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    ...
    if (vehiclesAboutToLeave.contains(event.getVehicleId())) {
      // drt vehicles have a VehicleLeavesTrafficEvent at each stop, then alights the driver and
      // after that
  ...
  /**
   * @param linkId
   * @return Array containing the number of vehicles entering the link <code>linkId</code> per time
   *     bin, starting with time bin 0 from 0 seconds to (timeBinSize-1)seconds.
   */
   ...
   * @param linkId
   * @param networkMode
   * @return Array containing the number of vehicles using the specified networkMode entering the
   *     link <code>linkId</code> per time bin, starting with time bin 0 from 0 seconds to
   ...
   * @param linkId
   * @param passengerMode
   * @return Array containing the number of vehicles having passengers of the specified
   *     passengerMode entering the link <code>linkId</code> per time bin, starting with time bin 0
   *     from 0 seconds to (timeBinSize-1)seconds.
   */
   ...
   *     <code>linkId</code> per time bin, starting with time bin 0 from 0 seconds to
   *     (timeBinSize-1)seconds.
   */
  ...
  /**
   * @param linkId
   * @return Array containing the number of passengers entering the link <code>linkId</code> per
   *     time bin, starting with time bin 0 from 0 seconds to (timeBinSize-1)seconds.
   */
   ...
   * @param linkId
   * @param networkMode
   * @return Array containing the number of passengers on vehicles using the specified networkMode
   *     entering the link <code>linkId</code> per time bin, starting with time bin 0 from 0 seconds
   *     to (timeBinSize-1)seconds.
   */
   ...
   * @param linkId
   * @param passengerMode
   * @return Array containing the number of passengers of the specified passengerMode entering the
   *     link <code>linkId</code> per time bin, starting with time bin 0 from 0 seconds to
   *     (timeBinSize-1)seconds.
   */
   ...
   * @param linkId
   * @param vehicleType
   * @return Array containing the number of passengers on vehicles of the specified VehicleType
   *     entering the link <code>linkId</code> per time bin, starting with time bin 0 from 0 seconds
   *     to (timeBinSize-1)seconds.
   */
  ...
  /**
   * @return The size of the arrays returned by calls to the {@link #getVehicleVolumesForLink(Id)}
   *     etc. methods.
   */
