...
public class LinkPaxVolumesWriter {
  ...
  public void writeLinkVehicleAndPaxVolumesAllPerDayCsv(String fileName) {
    ...
    try (CSVPrinter printer =
      ...
      for (Id<Link> linkId : linkIdsSorted) {
        ...
        printer.print(
            calcDailyValueOrZero(linkPaxVolumesAnalysis.getVehicleVolumesForLink(linkId)));
    ...
    try (CSVPrinter printer =
      ...
      for (Id<Link> linkId : linkIdsSorted) {
        for (String networkMode : networkModesSorted) {
          int[] hourlyVehicleVolumes =
              calcHourlyValuesOrZero(
                  linkPaxVolumesAnalysis.getVehicleVolumesForLinkPerNetworkMode(
                      linkId, networkMode));
          int[] hourlyPassengerVolumes =
              calcHourlyValuesOrZero(
                  linkPaxVolumesAnalysis.getPaxVolumesForLinkPerNetworkMode(linkId, networkMode));
  ...
  public void writeLinkVehicleAndPaxVolumesPerPassengerModePerHourCsv(String fileName) {
    // have results sorted
    SortedSet<Id<Link>> linkIdsSorted = new TreeSet(network.getLinks().keySet());
    SortedSet<String> passengerModesSorted =
    ...
    try (CSVPrinter printer =
      ...
      for (Id<Link> linkId : linkIdsSorted) {
        for (String passengerMode : passengerModesSorted) {
          int[] hourlyVehicleVolumes =
              calcHourlyValuesOrZero(
                  linkPaxVolumesAnalysis.getVehicleVolumesForLinkPerPassengerMode(
                      linkId, passengerMode));
          int[] hourlyPassengerVolumes =
              calcHourlyValuesOrZero(
                  linkPaxVolumesAnalysis.getPaxVolumesForLinkPerPassengerMode(
                      linkId, passengerMode));
          for (int hour = 0; hour < numberOfHours; hour++) {
            printer.print(linkId);
    ...
    try (CSVPrinter printer =
      ...
      for (Id<Link> linkId : linkIdsSorted) {
        for (Id<VehicleType> vehicleTypeId : vehicleIdsSorted) {
          int[] hourlyVehicleVolumes =
              calcHourlyValuesOrZero(
                  linkPaxVolumesAnalysis.getVehicleVolumesForLinkPerVehicleType(
                      linkId, vehicleTypeId));
          int[] hourlyPassengerVolumes =
              calcHourlyValuesOrZero(
