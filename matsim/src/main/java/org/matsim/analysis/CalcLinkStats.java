...
public class CalcLinkStats {
  ...
  private static final Logger log = LogManager.getLogger(CalcLinkStats.class);
  ...
  public void addData(final VolumesAnalyzer analyzer, final TravelTime ttimes) {
    ...
    for (Id<Link> linkId : this.linkData.keySet()) {
      ...
      // get the destination container for the data from link data (could have gotten this through
      // iterator right away)
      ...
      for (int hour = 0; hour < this.nofHours; hour++) {
        ...
        // the following has something to do with the fact that we are doing this for multiple
        // iterations.  So there are variations.
        // this collects min and max.  There is, however, no good control over how many iterations
        // this is collected.
        if (this.count == 1) {
      ...
      if (this.count == 1) {
        ...
        if (sumVolumes < data.volumes[MIN][this.nofHours])
          data.volumes[MIN][this.nofHours] = sumVolumes;
        data.volumes[SUM][this.nofHours] += sumVolumes;
        if (sumVolumes > data.volumes[MAX][this.nofHours])
          data.volumes[MAX][this.nofHours] = sumVolumes;
      }
    }
  }
  ...
  public void reset() {
    ...
    for (Link link : this.network.getLinks().values()) {
      LinkData data =
          new LinkData(
              new double[NOF_STATS][this.nofHours + 1], new double[NOF_STATS][this.nofHours]);
      ...
      for (Map.Entry<Id<Link>, LinkData> entry : this.linkData.entrySet()) {
        ...
        for (int i = 0; i < this.nofHours; i++) {
          ...
          if (data.volumes[SUM][i] == 0) {
            ...
            // use the value available or the minimum instead (min and max should be the same,
            // =freespeed)
    ...
    try {
      ...
      while (line != null) {
        ...
        if (parts.length == 154) {
          ...
          if (data == null) {
            ...
            for (int i = 0; i < this.nofHours; i++) {
              ...
              if (data.volumes[SUM][i] == 0) {
                ...
                data.ttimes[SUM][i] =
                    Double.parseDouble(parts[baseTTimes + i * 3 + 1]) * data.volumes[SUM][i];
              }
          ...
          System.err.println(
              "CalcLinkStats.readFile(); line cannot be parsed: "
                  + line
                  + " number of colums is: "
                  + parts.length);
          break;
   ...
   * @param linkId
   * @return if no data is available, an array with length 0 is returned.
   *     <p>The method reflects the (wrong) logic of what is done when writing the output and should
   *     eventually be deleted or modified.
   */
  @Deprecated
