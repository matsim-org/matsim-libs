...
/**
 * This class provides a mechanism similar to a stop watch, allowing to measure the duration of
 * operations and remembering time stamps. The class collects all the data and provides a simple
 * analysis of the time stamps and durations for operations for each iteration in the simulation.
 * This analysis can be dumped to console or to a file using the <code>write()</code>-methods.
 *
 * @author mrieser
 */
public final class IterationStopWatch {
   ...
   * {@link #endOperation(String)} and {@link #timestamp(String)} are assigned to the correct
   * iteration for the analysis.
   */
  ...
  public void beginOperation(final String identifier) {
    ...
    if (identifier.equals(OPERATION_OTHER)) {
      throw new RuntimeException(
          "Identifier " + OPERATION_OTHER + " is reserved! Please use another one. Aborting!");
    }
  ...
  /**
   * Make sure the given identifier exists in our collection. If it is missing, insert it at the
   * correct place. "Correct" means that it tries to insert this identifier right after the
   * last-requested identifier. This should help to write the gathered data out in a natural way.
   */
  ...
  /**
   * Formats the time given in milliseconds (e.g. returned by {@link
   * java.lang.System#currentTimeMillis()}) nicely for output.
   *
   * @param millis A time value in milliseconds
   * @return A String containing the formatted time, or an empty String if <code>millis</code> is
   *     null
   */
