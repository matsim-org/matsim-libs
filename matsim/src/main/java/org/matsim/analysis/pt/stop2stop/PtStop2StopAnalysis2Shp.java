...
public class PtStop2StopAnalysis2Shp {
  ...
  private static final String[] headerCsv = {
    "link", "transitLine", "stop", "departures", "passengers", "totalVehicleCapacity"
  };
  ...
  public static void writePtStop2StopAnalysisByTransitLine2ShpFile(
      ...
      final List<PtStop2StopAnalysis.Stop2StopEntry> stop2StopEntriesForEachDeparture,
      String shpFileName,
      String coordinateSystem) {

    // sum per link and transit line
    Map<Id<Link>, Map<Id<TransitLine>, PtStop2StopAnalysis.Stop2StopAggregation>>
        stop2stopByLinkAndTransitLineAggregates =
            aggregatePerLinkAndLine(stop2StopEntriesForEachDeparture);
    ...
    // note: column names may not be longer than 10 characters. Otherwise the name is cut after the
    // 10th character and the avalue is NULL in QGis
    ...
    // simpleFeatureBuilder.add("transitStopIdPrevious", String.class); //not unique, could be
    // multiple
    // simpleFeatureBuilder.add("transitStopIdFollowing", String.class); //not unique, could be
    // multiple
    ...
    SimpleFeatureBuilder linkFeatureBuilder =
        new SimpleFeatureBuilder(simpleFeatureBuilder.buildFeatureType());

    Collection<SimpleFeature> features = new ArrayList<>();
    GeometryFactory geofac = new GeometryFactory();

    for (Map.Entry<Id<Link>, Map<Id<TransitLine>, PtStop2StopAnalysis.Stop2StopAggregation>>
        entryLinkLevel : stop2stopByLinkAndTransitLineAggregates.entrySet()) {
      for (Map.Entry<Id<TransitLine>, PtStop2StopAnalysis.Stop2StopAggregation> entryLineLevel :
          entryLinkLevel.getValue().entrySet()) {
        createLinkFeatureFromStop2StopAggregation(
            scenario,
            linkFeatureBuilder,
            features,
            geofac,
            entryLinkLevel.getKey(),
            entryLineLevel.getKey().toString(),
            entryLineLevel.getValue());
      }
    }

    // sum per link of all transit lines
    Map<Id<Link>, PtStop2StopAnalysis.Stop2StopAggregation> stop2stopByLinkAggregates =
        stop2StopEntriesForEachDeparture.stream()
            .flatMap(
                stop2StopEntry ->
                    stop2StopEntry.linkIdsSincePreviousStop.stream()
                ...
                Collectors.toMap(
                    ...
                    entry ->
                        new PtStop2StopAnalysis.Stop2StopAggregation(
                            1,
                            entry.getValue().passengersAtArrival,
                            entry.getValue().totalVehicleCapacity),
                    aggregateStop2StopAggregations(),
                    HashMap::new));

    for (Map.Entry<Id<Link>, PtStop2StopAnalysis.Stop2StopAggregation> entryLinkLevel :
        stop2stopByLinkAggregates.entrySet()) {
      createLinkFeatureFromStop2StopAggregation(
          scenario,
          linkFeatureBuilder,
          features,
  ...
  public static void writePtStop2StopAnalysisByTransitLine2CsvFile(
    ...
    try (CSVPrinter printer =
        new CSVPrinter(
            IOUtils.getBufferedWriter(fileNameCsv),
  ...
  private static Map<Id<Link>, Map<Id<TransitLine>, PtStop2StopAnalysis.Stop2StopAggregation>>
      aggregatePerLinkAndLine(
          List<PtStop2StopAnalysis.Stop2StopEntry> stop2StopEntriesForEachDeparture) {
  ...
  private static void createLinkFeatureFromStop2StopAggregation(
      Scenario scenario,
      SimpleFeatureBuilder linkFeatureBuilder,
      ...
      PtStop2StopAnalysis.Stop2StopAggregation stop2StopAggregation) {
    Link link = scenario.getNetwork().getLinks().get(linkId);
    LineString ls =
        geofac.createLineString(
            new Coordinate[] {
    ...
    linkFeatureAttributes[6] =
        stop2StopAggregation.getPassengers() / stop2StopAggregation.getTotalVehicleCapacity();
    try {
      features.add(
          linkFeatureBuilder.buildFeature(
              stop2StopAggregation + "_line_" + transitLineId, linkFeatureAttributes));
    } catch (IllegalArgumentException e) {
      e.printStackTrace();
    }
  }
