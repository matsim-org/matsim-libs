...
/**
 * Processes events to create a csv file with passenger volumes, delays and similar information for
 * each stop to stop segment of a pt {@link org.matsim.pt.transitSchedule.api.Departure}.
 *
 * <p>This class is entirely based on events. In case simulated pt should deviate from the {@link
 * org.matsim.pt.transitSchedule.api.TransitSchedule} this class provides data on what the
 * simulation really gave ignoring the TransitSchedule.
 *
 * <p>Currently there are no integrity checks, if the sequence of events is wrong or an event is
 * missing, this class might silently produce invalid output. This class was intended to be run
 * after the last iteration and the simulation has finished and no preconditions were taken for
 * TransitVehicle fleets which might change over the course of iterations (e.g. minibus). So running
 * this class as an EventsListener during the simulation might give wrong results and might need
 * work to get the transit vehicles updated before mobsim in each iteration.
 *
 * @author vsp-gleich
 */
public class PtStop2StopAnalysis
  ...
  private final Map<Id<Vehicle>, PtVehicleData> transitVehicle2temporaryVehicleData =
      new HashMap<>();
  ...
  public void handleEvent(TransitDriverStartsEvent event) {
    transitVehicle2temporaryVehicleData.put(
        event.getVehicleId(),
        new PtVehicleData(
  ...
  /**
   * Vehicle arrives at a stop. This event class is only used by transit vehicles and those should
   * have had a TransitDriverStartsEvent before.
   */
  @Override
  public void handleEvent(VehicleArrivesAtFacilityEvent event) {
    ...
    if (ptVehicleData == null) {
      log.error(
          "Encountered a VehicleArrivesAtFacilityEvent without a previous TransitDriverStartsEvent for vehicle "
              + event.getVehicleId()
              + " at facility "
              + event.getFacilityId()
              + " at time "
              + event.getTime()
              + ". This should not happen, this analysis might fail subsequently.");
    } else {
  ...
  /**
   * Vehicle departs from a stop. This event class is only used by transit vehicles and those should
   * have had a TransitDriverStartsEvent before.
   */
  @Override
  public void handleEvent(VehicleDepartsAtFacilityEvent event) {
    ...
    if (ptVehicleData == null) {
      log.error(
          "Encountered a VehicleDepartsAtFacilityEvent without a previous TransitDriverStartsEvent for vehicle "
              + event.getVehicleId()
              + " at facility "
              + event.getFacilityId()
              + " at time "
      ...
      stop2StopEntriesForEachDeparture.add(
          new Stop2StopEntry(
              ptVehicleData.transitLineId,
              ptVehicleData.transitRouteId,
              ptVehicleData.departureId,
              event.getFacilityId(),
              ptVehicleData.stopSequenceCounter,
              ptVehicleData.lastStopId,
              ptVehicleData.lastVehicleArrivesAtFacilityEvent.getTime()
                  - ptVehicleData.lastVehicleArrivesAtFacilityEvent.getDelay(),
              ptVehicleData.lastVehicleArrivesAtFacilityEvent.getDelay(),
              event.getTime() - event.getDelay(),
              event.getDelay(),
              ptVehicleData.currentPax,
              ptVehicleData.totalVehicleCapacity,
              ptVehicleData.alightings,
              ptVehicleData.boardings,
              ptVehicleData.linksTravelledOnSincePreviousStop));
      // calculate number of passengers at departure
      // (the Stop2StopEntry before needed the number of passengers at arrival so do not move this
      // up!)
      ptVehicleData.currentPax =
          ptVehicleData.currentPax - ptVehicleData.alightings + ptVehicleData.boardings;
      // reset counters
  ...
  public void reset(int iteration) {
    if (transitVehicle2temporaryVehicleData.size() > 0) {
      log.warn(
          transitVehicle2temporaryVehicleData.size()
              + " transit vehicles did not finish service in the last iteration.");
    }
  ...
  private static class PtVehicleData {
    ...
    private final double
        totalVehicleCapacity; // use double for possible future capacity scaling issues
  ...
  static final class Stop2StopEntry {
    final Id<TransitLine>
        transitLineId; // for aggregation -> set null or leave out and use Map lineId ->
    // Stop2StopEntry?
    ...
    Stop2StopEntry(
        ...
        int stopSequence,
        Id<TransitStopFacility> stopPreviousId,
        double arrivalTimeScheduled,
        double arrivalDelay,
        double departureTimeScheduled,
        double departureDelay,
        int passengersAtArrival,
        double totalVehicleCapacity,
        int passengersAlighting,
  ...
  public void writeStop2StopEntriesByDepartureCsv(
      String fileName, String columnSeparator, String listSeparatorInsideColumn) {
    stop2StopEntriesForEachDeparture.sort(
        stop2StopEntryByTransitLineComparator
    ...
    try (CSVPrinter printer =
      ...
      for (Stop2StopEntry entry : stop2StopEntriesForEachDeparture) {
        ...
        printer.print(
            entry.linkIdsSincePreviousStop.stream()
                .map(id -> id.toString())
                .collect(Collectors.joining(listSeparatorInsideColumn)));
        printer.println();
      }
    } catch (IOException e) {
      e.printStackTrace();
  ...
  public static BinaryOperator<Stop2StopAggregation> aggregateStop2StopAggregations() {
    return (entry1, entry2) ->
        new PtStop2StopAnalysis.Stop2StopAggregation(
            entry1.getDepartures() + entry2.getDepartures(),
            entry1.getPassengers() + entry2.getPassengers(),
