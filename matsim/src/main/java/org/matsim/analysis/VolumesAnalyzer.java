...
public class VolumesAnalyzer implements LinkLeaveEventHandler, VehicleEntersTrafficEventHandler {
  ...
  private static final Logger log = LogManager.getLogger(VolumesAnalyzer.class);
  ...
  public VolumesAnalyzer(
      final int timeBinSize, final int maxTime, final Network network, boolean observeModes) {
  ...
  /**
   * @param linkId
   * @return Array containing the number of vehicles leaving the link <code>linkId</code> per time
  ...
  /**
   * @return The size of the arrays returned by calls to the {@link #getVolumesForLink(Id)} and the
   *     {@link #getVolumesForLink(Id, String)} methods.
   */
