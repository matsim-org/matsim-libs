 ...
 *
 * <ul>
 *   <li>average score of the selected plan
 *   <li>average of the score of the worst plan of each agent
 *   <li>average of the score of the best plan of each agent
 *   <li>average of the average score of all plans of each agent
 * </ul>
 *
 * Plans with undefined scores are not included in the statistics. The calculated values are written
 * to a file, each iteration on a separate line.
 *
 * @author mrieser
 */
public class ScoreStatsControlerListener
    implements StartupListener, IterationEndsListener, ShutdownListener, ScoreStats {
  ...
  private static final Logger log = LogManager.getLogger(ScoreStatsControlerListener.class);
      ...
      this.perSubpop.put(
          ...
          new ScoreHist(
              ...
              IOUtils.getBufferedWriter(
                  controllerIO.getOutputFilename("scorestats_" + sub + ".csv"))));
    }
  ...
  private void collectScoreInfo(final IterationEndsEvent event) {
    ...
    for (Person person : this.population.getPersons().values()) {
      ...
      if (subpop != null) perSubpop.get(subpop).update(person);
    }

    log.info(
        "-- avg. score of the executed plan of each agent: "
            + (info.sumExecutedScores / info.nofExecutedScores));
    log.info(
        "-- avg. score of the worst plan of each agent: "
            + (info.sumScoreWorst / info.nofScoreWorst));
    log.info(
        "-- avg. of the avg. plan score per agent: " + (info.sumAvgScores / info.nofAvgScores));
    log.info(
        "-- avg. score of the best plan of each agent: " + (info.sumScoreBest / info.nofScoreBest));
    ...
    this.scoreHistory
        .get(ScoreItem.worst)
        .put(event.getIteration(), info.sumScoreWorst / info.nofScoreWorst);
    this.scoreHistory
        .get(ScoreItem.best)
        .put(event.getIteration(), info.sumScoreBest / info.nofScoreBest);
    this.scoreHistory
        .get(ScoreItem.average)
        .put(event.getIteration(), info.sumAvgScores / info.nofAvgScores);
    this.scoreHistory
        .get(ScoreItem.executed)
        .put(event.getIteration(), info.sumExecutedScores / info.nofExecutedScores);
