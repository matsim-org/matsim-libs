...
class IterationTravelStatsControlerListener implements IterationEndsListener, ShutdownListener {
  ...
  @Inject private PHbyModeCalculator pHbyModeCalculator;

  @Inject private PKMbyModeCalculator pkMbyModeCalculator;
  @Inject OutputDirectoryHierarchy outputDirectoryHierarchy;

  @Inject TripsAndLegsCSVWriter.CustomTripsWriterExtension customTripsWriterExtension;
  @Inject TripsAndLegsCSVWriter.CustomLegsWriterExtension customLegsWriterExtension;
  @Inject TripsAndLegsCSVWriter.CustomTimeWriter customTimeWriter;

  @Inject AnalysisMainModeIdentifier mainModeIdentifier;

  Logger log = LogManager.getLogger(IterationTravelStatsControlerListener.class);

  @Override
  public void notifyIterationEnds(IterationEndsEvent event) {
    travelDistanceStats.addIteration(
        event.getIteration(), experiencedPlansService.getExperiencedPlans());
    pHbyModeCalculator.addIteration(
        event.getIteration(), experiencedPlansService.getExperiencedPlans());
    pkMbyModeCalculator.addIteration(
        event.getIteration(), experiencedPlansService.getExperiencedPlans());
    ...
    final boolean regularWriteEvents =
        writingTripsAtAll
            && ((event.getIteration() > 0
                    && event.getIteration() % config.controller().getWriteTripsInterval() == 0)
    ...
    if (regularWriteEvents || (writingTripsAtAll && event.getIteration() == 0)) {
      new TripsAndLegsCSVWriter(
              ...
              customTimeWriter)
          .write(
              experiencedPlansService.getExperiencedPlans(),
              outputDirectoryHierarchy.getIterationFilename(
                  event.getIteration(), Controler.DefaultFiles.tripscsv),
              outputDirectoryHierarchy.getIterationFilename(
                  event.getIteration(), Controler.DefaultFiles.legscsv));
  ...
  private void writePersonsCSV() {
    LogManager.getLogger(getClass())
        .info("Writing all Person and Attributes to " + Controler.DefaultFiles.personscsv);
    ...
    try (CSVPrinter csvPrinter =
        new CSVPrinter(
            IOUtils.getBufferedWriter(
                outputDirectoryHierarchy.getOutputFilename(Controler.DefaultFiles.personscsv)),
            CSVFormat.DEFAULT
                .withDelimiter(config.global().getDefaultDelimiter().charAt(0))
                .withHeader(header.stream().toArray(String[]::new)))) {
      for (Person p : scenario.getPopulation().getPersons().values()) {
        if (p.getSelectedPlan() == null) {
          log.error(
              "Found person without a selected plan: "
                  + p.getId().toString()
        ...
        line.add(
            p.getSelectedPlan().getScore() == null
                ? "null"
                : p.getSelectedPlan().getScore().toString());
        String x = "";
        String y = "";
        String actType = "";
  ...
  private void writeActivityCSV(int iteration) {
    ...
    List<String> attributes =
        experiencedPlansService.getExperiencedPlans().values().stream()
            .flatMap(
                p ->
                    TripStructureUtils.getActivities(
                        p, TripStructureUtils.StageActivityHandling.ExcludeStageActivities)
                        .stream())
    ...
    try (CSVPrinter csvPrinter =
        new CSVPrinter(
            IOUtils.getBufferedWriter(
                outputDirectoryHierarchy.getIterationFilename(
                    iteration, Controler.DefaultFiles.activitiescsv)),
            CSVFormat.DEFAULT
                .withDelimiter(config.global().getDefaultDelimiter().charAt(0))
                .withHeader(header.stream().toArray(String[]::new)))) {

      for (Map.Entry<Id<Person>, Plan> e :
          experiencedPlansService.getExperiencedPlans().entrySet()) {

        int i = 0;
        for (Activity act :
            TripStructureUtils.getActivities(
                e.getValue(), TripStructureUtils.StageActivityHandling.ExcludeStageActivities)) {
