...
public class Bins {
  ...
  private static final Logger log = LogManager.getLogger(Bins.class);
  ...
  public void plotBinnedDistribution(String path, String xLabel, String xUnit) {
    ...
    String s =
        xLabel
            + " "
            + "[interval = "
            + formatter.format(this.interval)
            + xUnit
            + "]"
            + "[number of entries = "
            + this.entries.size()
            + "]"
            + "[mean = "
            + formatter.format(this.weightedMean(values, weights))
            + xUnit
            + "]"
            + "[median = "
            + formatter.format(this.median(values))
            + xUnit
            + "]"
            + "[max = "
            + formatter.format(this.getMax(values))
            + xUnit
            + "]";

    BarChart chart = new BarChart(desc, s, "#", categories);
  ...
  public double weightedMean(List<Double> values, List<Double> weights) {
    return weightedMean(
        values.toArray(new Double[values.size()]), weights.toArray(new Double[weights.size()]));
  }
