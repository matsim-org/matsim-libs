...
/**
 * Calculates mode choice coverage at the end of each iteration, based on the main mode identifier
 * of a trip chain. Mode choice coverage percentage of trips have used a certain mode at least once
 * (or 5x, 10x, …) in previous iterations.
 *
 * @author jakobrehmann
 */
public class ModeChoiceCoverageControlerListener
  ...
  private final Map<Id<Person>, Map<Integer, Map<String, Integer>>> modesUsedPerPersonTrip =
      new LinkedHashMap<>();
  ...
  @Inject
  ModeChoiceCoverageControlerListener(
  ...
  public void notifyIterationEnds(final IterationEndsEvent event) {
      ...
      for (Map<String, Integer> mapForPersonTrip : mapForPerson.values()) {
        ...
        for (String mode : mapForPersonTrip.keySet()) {
          ...
          for (Integer limit : limits) {
            Map<String, Double> modeCountMap =
                modeCountCurrentIteration.computeIfAbsent(limit, k -> new TreeMap<>());
    ...
    // Calculates mcc share for each mode in current iteration, and updates modeCCHistory
    // accordingly
    for (Integer limit : limits) {
      Map<String, Double> modeCnt = modeCountCurrentIteration.get(limit);
      this.modes.addAll(
          modeCnt.keySet()); // potentially adds new modes to setthat just showed up in current iter
      Map<String, Map<Integer, Double>> modeIterationShareMap =
          modeCCHistory.computeIfAbsent(limit, k -> new HashMap<>());
      for (String mode : modes) {
        ...
        // If this is the first iteration where the mode shows up, add zeros to all previous
  ...
  private void updateModesUsedPerPerson() {
    for (Person person : this.population.getPersons().values()) {
      ...
      Map<Integer, Map<String, Integer>> mapForPerson =
          modesUsedPerPersonTrip.computeIfAbsent(person.getId(), v -> new LinkedHashMap<>());
      ...
      for (Trip trip : TripStructureUtils.getTrips(plan)) {
        ...
        Map<String, Integer> mapForPersonTrip =
  ...
  private void produceGraphs() {
    for (Integer limit : limits) {
      XYLineChart chart =
          new XYLineChart(
              "Mode Choice Coverage (Mode Used >= " + limit + "x per trip)",
              "iteration",
