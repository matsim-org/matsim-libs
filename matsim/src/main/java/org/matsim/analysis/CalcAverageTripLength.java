...
/**
 * Calculates the average trip length of all routes in a plan. The trip length is the sum of the
 * length of all links, including the route's end link, but <em>not</em> including the route's start
 * link.
 *
 * <p>This class works only for NetworkRoutes and will likely fail for legs with other types of
 * Routes.
 *
 * @author mrieser
 */
    ...
    for (PlanElement pe : plan.getPlanElements()) {
      if (pe instanceof Leg) {
        ...
        if (route != null) {
          ...
          double dist =
              RouteUtils.calcDistanceExcludingStartEndLink((NetworkRoute) route, this.network);
