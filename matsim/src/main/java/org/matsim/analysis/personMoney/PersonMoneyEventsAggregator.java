...
public class PersonMoneyEventsAggregator implements PersonMoneyEventHandler {
  ...
  private final Map<String, Map<String, Double>>
      personMoneyAmountSumByPurposeAndTransactionPartner = new HashMap<>();
  ...
  public void handleEvent(PersonMoneyEvent event) {
    ...
    String transactionPartner =
        event.getTransactionPartner() == null ? "null" : event.getTransactionPartner();
    Map<String, Double> transactionPartnerMap =
        personMoneyAmountSumByPurposeAndTransactionPartner.computeIfAbsent(
            purpose, k -> new HashMap<>());
    transactionPartnerMap.put(
        transactionPartner,
        event.getAmount() + transactionPartnerMap.getOrDefault(transactionPartner, 0.0));
  }
  ...
  void writeOutput(String outputFilename) {
    try (CSVPrinter csvPrinter =
      ...
      // sort output
      Map<String, Map<String, Double>> purpose2EntriesSorted =
          new TreeMap<>(personMoneyAmountSumByPurposeAndTransactionPartner);

      for (Map.Entry<String, Map<String, Double>> entryPurposeLevel :
          purpose2EntriesSorted.entrySet()) {
        Map<String, Double> transactionPartner2EntriesSorted =
            new TreeMap<>(entryPurposeLevel.getValue());
        for (Map.Entry<String, Double> entryTransactionPartnerLevel :
            transactionPartner2EntriesSorted.entrySet()) {
          csvPrinter.printRecord(
              entryPurposeLevel.getKey(),
              entryTransactionPartnerLevel.getKey(),
              entryTransactionPartnerLevel.getValue());
        }
      }
    } catch (IOException e) {
