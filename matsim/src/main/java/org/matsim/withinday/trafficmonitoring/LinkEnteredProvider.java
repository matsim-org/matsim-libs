...
/**
 * Returns all agents that have entered a new link in the last time step. Agents who just ended an
 * activity are NOT included, since they...
 *
 * <ul>
 *   <li>do not produce a link entered event.
 *   <li>are limited in their possible replanning operations.
 * </ul>
 *
 * @author cdobler
 */
public class LinkEnteredProvider
  ...
  private Map<Id<Person>, Id<Link>> linkEnteredAgents =
      new ConcurrentHashMap<>(); // <agentId, linkId>
  private Map<Id<Person>, Id<Link>> lastTimeStepLinkEnteredAgents =
      new ConcurrentHashMap<>(); // <agentId, linkId>
  ...
  public void handleEvent(LinkEnterEvent event) {
    this.linkEnteredAgents.put(
        delegate.getDriverOfVehicle(event.getVehicleId()), event.getLinkId());
  }
