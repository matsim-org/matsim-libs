...
/**
 * This Class implements the MobsimEngine interface. If added to a QSim, the QSim's internals ensure
 * that WithinDayEngine.doSimStep(...) is performed before the other MobsimEngines.
 *
 * <p>Also, it lives as long as the Controler, as opposed to all other parts of the QSim, and is
 * added to each new QSim instance in turn.
 *
 * @author cdobler
 */
@Singleton
public class WithinDayEngine implements MobsimEngine, ActivityEndReschedulerProvider {
  ...
  private Map<WithinDayDuringActivityReplannerFactory, Tuple<Double, Double>>
      duringActivityReplannerFactory;
  ...
  public WithinDayEngine(EventsManager eventsManager, GlobalConfigGroup globalConfigGroup) {
    ...
    this.parallelInitialReplanner =
        new ParallelInitialReplanner(globalConfigGroup.getNumberOfThreads(), eventsManager);
    this.parallelDuringActivityReplanner =
        new ParallelDuringActivityReplanner(globalConfigGroup.getNumberOfThreads(), eventsManager);
    this.parallelDuringLegReplanner =
        new ParallelDuringLegReplanner(globalConfigGroup.getNumberOfThreads(), eventsManager);
    ...
    this.duringActivityReplanningModule =
        new DuringActivityReplanningModule(parallelDuringActivityReplanner);
  ...
  public void removeDuringActivityReplannerFactory(
      WithinDayDuringActivityReplannerFactory factory) {
  ...
  public void addTimedDuringActivityReplannerFactory(
      WithinDayDuringActivityReplannerFactory factory,
      double startReplanning,
      double endReplanning) {
  ...
  public void addTimedDuringLegReplannerFactory(
      WithinDayDuringLegReplannerFactory factory, double startReplanning, double endReplanning) {
    Tuple<Double, Double> tuple = new Tuple<Double, Double>(startReplanning, endReplanning);
  ...
  public void doSimStep(double time) {
    ...
    if (!initialReplanningPerformed && isInitialReplanning()) {
      initialReplanningModule.doReplanning(
          Double.NEGATIVE_INFINITY); // -Inf == before any feasible time step
      initialReplanningPerformed = true;
    }

    for (Entry<WithinDayDuringActivityReplannerFactory, Tuple<Double, Double>> entry :
        duringActivityReplannerFactory.entrySet()) {
      if (entry.getValue().getFirst() == time)
        this.parallelDuringActivityReplanner.addWithinDayReplannerFactory(entry.getKey());
      if (entry.getValue().getSecond() == time)
        this.parallelDuringActivityReplanner.removeWithinDayReplannerFactory(entry.getKey());
    }
    for (Entry<WithinDayDuringLegReplannerFactory, Tuple<Double, Double>> entry :
        duringLegReplannerFactory.entrySet()) {
      if (entry.getValue().getFirst() == time)
