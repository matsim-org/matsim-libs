...
public abstract class ReplanningRunnable implements Runnable {
  ...
  private static final Logger log = LogManager.getLogger(ReplanningRunnable.class);
  ...
  protected Map<Id<WithinDayReplanner>, WithinDayReplanner<? extends AgentSelector>>
      withinDayReplanners = new HashMap<>();
  ...
  public final void addReplanningTask(ReplanningTask replanningTask) {
    Queue<ReplanningTask> queue =
        this.replanningTasks.get(replanningTask.getWithinDayReplannerId());
    queue.add(replanningTask);
  }

  public final void addWithinDayReplanner(
      WithinDayReplanner<? extends AgentSelector> withinDayReplanner, Queue<ReplanningTask> queue) {
  ...
  public final void resetReplanners() {
    this.counter.reset();
    for (WithinDayReplanner<? extends AgentSelector> withinDayReplanner :
        this.withinDayReplanners.values()) {
  ...
  private void doReplanning() throws InterruptedException, BrokenBarrierException {
    ...
    for (Entry<Id<WithinDayReplanner>, Queue<ReplanningTask>> entry :
        this.replanningTasks.entrySet()) {

      Id<WithinDayReplanner> withinDayReplannerId = entry.getKey();
      Queue<ReplanningTask> queue = entry.getValue();

      WithinDayReplanner<? extends AgentSelector> withinDayReplanner =
          this.withinDayReplanners.get(withinDayReplannerId);
      ...
      while (true) {
        ...
        if (!replanningSuccessful) {
          log.error(
              "Replanning was not successful! Replanner "
                  + withinDayReplanner.getClass().toString()
                  + ", time "
                  + Time.writeTime(time)
                  + ", agent "
                  + withinDayAgent.getId());
        } else {
          /*
          ...
          if (eventsManager != null) {
            ReplanningEvent replanningEvent =
                new ReplanningEvent(
                    time, withinDayAgent.getId(), withinDayReplanner.getClass().getSimpleName());
