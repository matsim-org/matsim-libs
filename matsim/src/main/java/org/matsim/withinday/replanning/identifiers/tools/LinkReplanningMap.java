...
/**
 * This Module is used by a CurrentLegReplanner. It calculates the time when an agent should do
 * CurrentLegReplanning.
 *
 * <p>The time is estimated as following: When a LinkEnterEvent is thrown the Replanning Time is set
 * to the current time + the FreeSpeed Travel Time. This guarantees that the replanning will be done
 * while the agent is on the Link. After that time, the agent might be already in the outgoing queue
 * of a QLink where not all replanning operations are possible anymore (the agent can e.g. not
 * insert an Activity on its current link anymore).
 *
 * <p>
 *
 * <p>The replanning interval (multiple replannings on the same link when an agent is stuck on a
 * link due to a traffic jam) has been removed since it cannot be guaranteed that all replanning
 * operations are valid anymore.
 *
 * @author cdobler
 */
public class LinkReplanningMap
  ...
  public LinkReplanningMap(
      EarliestLinkExitTimeProvider earliestLinkExitTimeProvider, EventsManager eventsManager) {
  ...
  public Set<Id<Person>> getReplanningAgents(final double time) {
    ...
    Set<Id<Person>> set =
        this.earliestLinkExitTimeProvider.getEarliestLinkExitTimesPerTimeStep(time);
  ...
  /**
   * @param time
   * @return a list of agents who might need an unrestricted replanning and use the given transport
   *     mode
   */
  ...
  /**
   * @param time
   * @return a list of agents who might need a restricted replanning and use the given transport
   *     mode
   */
  ...
  private Set<Id<Person>> filterAgents(final double time, final TimeFilterMode timeMode) {
    ...
    Set<Entry<OptionalTime, Set<Id<Person>>>> entries =
        this.earliestLinkExitTimeProvider.getEarliestLinkExitTimesPerTimeStep().entrySet();
  ...
  /**
   * @return A list of all agents that are currently performing a leg. Note that some of them might
   *     be limited in the available replanning operations!
   */
  public Set<Id<Person>> getLegPerformingAgents() {
    return Collections.unmodifiableSet(
        this.earliestLinkExitTimeProvider.getEarliestLinkExitTimes().keySet());
  }

  /**
   * @return A list of all agents that have just started a leg. Note that they cannot end their leg
   *     on their current link!
   */
