...
public final class EditPlans {
  ...
  public boolean addActivityAtEnd(MobsimAgent agent, Activity activity, String routingMode) {
    ...
    if (!isAtRealActivity(agent)) {
      retVal1 =
          editTrips.replanCurrentTrip(agent, mobsim.getSimTimer().getTimeOfDay(), routingMode);
    }
  ...
  public PlanElement removeActivity(MobsimAgent agent, int index, String mode) {
    ...
    final Trip tripBefore =
        TripStructureUtils.findTripEndingAtActivity((Activity) planElements.get(index), plan);
    final Trip tripAfter =
        TripStructureUtils.findTripStartingAtActivity((Activity) planElements.get(index), plan);
    if (mode == null) {
      final String mainModeBefore =
          TripStructureUtils.identifyMainMode(tripBefore.getTripElements());
    ...
    if (checkIfTripHasAlreadyStarted(agent, tripBefore.getTripElements())) {
      ...
      editTrips.insertEmptyTrip(
          plan, tripBefore.getOriginActivity(), tripAfter.getDestinationActivity(), mode);
    }
    WithinDayAgentUtils.resetCaches(agent);
    this.mobsim.rescheduleActivityEnd(agent);
    return pe;
  ...
  public final void rescheduleActivityEndtime(MobsimAgent agent, int index, double newEndTime) {
    Activity activity =
        (Activity) WithinDayAgentUtils.getModifiablePlan(agent).getPlanElements().get(index);
    activity.setEndTime(newEndTime);
    WithinDayAgentUtils.resetCaches(agent);
    WithinDayAgentUtils.rescheduleActivityEnd(agent, mobsim);
  }

  public final Activity replaceActivity(
      MobsimAgent agent, int index, Activity newAct, String upstreamMode, String downstreamMode) {
    ...
    if (index > 0) {
      ...
      if (checkIfTripHasAlreadyStarted(agent, currentTripElements)) {
        ...
        this.editTrips.replanCurrentTrip(
            agent, this.mobsim.getSimTimer().getTimeOfDay(), currentMode);
      } else {
        // trip has not yet started
        if (upstreamMode == null) {
          upstreamMode = currentMode;
        }
        this.editTrips.insertEmptyTrip(
            plan, tripBeforeAct.getOriginActivity(), newAct, upstreamMode);
      }
    }
    // trip after (if any):
    if (index < planElements.size() - 1) {
      ...
      if (downstreamMode == null) {
        final String currentMainMode =
            TripStructureUtils.identifyMainMode(tripAfterAct.getTripElements());
        EditTrips.insertEmptyTrip(
            plan, newAct, tripAfterAct.getDestinationActivity(), currentMainMode, pf);
      } else {
        EditTrips.insertEmptyTrip(
  ...
  public void insertActivity(
      MobsimAgent agent, int index, Activity activity, String upstreamMode, String downstreamMode) {
    Plan plan = WithinDayAgentUtils.getModifiablePlan(agent);
    List<PlanElement> planElements = plan.getPlanElements();
    ...
    planElements.add(index, activity);
    {
      ...
      if (actBefore != null) {
        if (WithinDayAgentUtils.indexOfPlanElement(agent, actBefore)
            < WithinDayAgentUtils.getCurrentPlanElementIndex(agent)) {
          // we are already under way
          editTrips.replanCurrentTrip(
              agent, this.mobsim.getSimTimer().getTimeOfDay(), upstreamMode);
        } else {
  ...
  public void insertActivity(MobsimAgent agent, int index, Activity activity) {
    String mode =
        TripStructureUtils.identifyMainMode(EditTrips.findCurrentTrip(agent).getTripElements());
  ...
  private static boolean checkIfTripHasAlreadyStarted(
      MobsimAgent agent, final List<PlanElement> currentTripElements) {
  ...
  private static void checkIfSameMode(String upstreamMode, final String currentMode) {
    if (upstreamMode != null && !upstreamMode.equals(currentMode)) {
      throw new ReplanningException(
          "cannot change mode in trip that has already started.  Don't set the mode in the request, "
  ...
  // this.mobsim.getScenario().getActivityFacilities().getFacilities().get( activity.getFacilityId()
  // ) ;
  //			if ( facility != null ) {
  ...
  public String getModeOfCurrentOrNextTrip(MobsimAgent agent) {
    ...
    if (isAtRealActivity(agent)) {
      ...
      trip =
          editTrips.findTripAfterActivity(WithinDayAgentUtils.getModifiablePlan(agent), activity);
    } else {
  ...
  public static List<Leg> findLegsWithModeInFuture(MobsimAgent agent, String mode) {
    ...
    Plan plan = WithinDayAgentUtils.getModifiablePlan(agent);
    for (int ii = WithinDayAgentUtils.getCurrentPlanElementIndex(agent);
        ii < plan.getPlanElements().size();
        ii++) {
