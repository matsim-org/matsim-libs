...
//					log.debug("found trip element") ;
	...
	public final boolean replanCurrentTrip(MobsimAgent agent, double now, String routingMode ) {
		...
		Attributes tripAttributes = trip.getOriginActivity().getAttributes();
		...
		if ( currentPlanElement instanceof Activity ) {
			...
			replanCurrentTripFromStageActivity(trip, tripElementsIndex, routingMode, now, agent, tripAttributes);
		} else {
			// we are on a leg
	...
	private void replanCurrentTripFromLeg(Activity newAct, final PlanElement currentPlanElement, final String routingMode, double now, MobsimAgent agent) {
		...
		Trip currentTrip = findCurrentTrip(agent);
		Attributes tripAttributes = currentTrip.getOriginActivity().getAttributes();
		
		if ( currentLeg.getRoute() instanceof NetworkRoute ) {
			replanCurrentLegWithNetworkRoute(newAct, routingMode, currentLeg, now, agent, tripAttributes);
		} else if ( currentLeg.getRoute() instanceof TransitPassengerRoute ) {
			// public transit leg
			replanCurrentLegWithTransitRoute(newAct, routingMode, currentLeg, now, agent, tripAttributes);
		} else if ( currentLeg.getRoute() instanceof GenericRouteImpl ) {
			// teleported leg
			replanCurrentLegWithGenericRoute(newAct, routingMode, currentLeg, now, agent, tripAttributes);
		} else {
	...
	private void replanCurrentLegWithNetworkRoute(Activity newAct, String mainMode, Leg currentLeg, double now, MobsimAgent agent, Attributes tripAttributes) {
		...
		List<? extends PlanElement> newTripElements = newTripToNewActivity(currentLocationFacility, newAct, mainMode, now, person, tripAttributes );
	...
	private void replanCurrentLegWithTransitRoute(Activity newAct, String routingMode, Leg currentLeg, double now, MobsimAgent agent, Attributes tripAttributes) {
		...
		if (mobsimVehicle == null) {
			...
			newTripElements = newTripToNewActivity(currentOrNextStop, newAct, routingMode, now, person, tripAttributes );
			...
			newTripElements = newTripToNewActivity(currentOrNextStop, newAct, routingMode, reRoutingTime, person, tripAttributes );
	 ...
	 */
	private void replanCurrentLegWithGenericRoute(Activity newAct, String routingMode, Leg currentLeg, double now, MobsimAgent agent, Attributes tripAttributes) {
		...
		if ( StageActivityTypeIdentifier.isStageActivity(nextAct.getType()) ) {
			...
			final List<? extends PlanElement> newTrip = tripRouter.calcRoute(routingMode, fromFacility, toFacility, departureTime, plan.getPerson(), tripAttributes );
	...
	private List<? extends PlanElement> newTripToNewActivity( Facility currentLocationFacility, Activity newAct, String mainMode, double now, Person person, Attributes tripAttributes ) {
		...
		return tripRouter.calcRoute(mainMode, currentLocationFacility, toFacility, now, person, tripAttributes );
	}
	// replan from stage activity:
	private void replanCurrentTripFromStageActivity(Trip trip, int tripElementsIndex,
			String mainMode, double now, MobsimAgent agent, Attributes tripAttributes) {
