...
public class SnapshotLinkWidthCalculator {
  /**
   * Distance (in m) between the two innermost opposing lanes. Setting this to a larger number (e.g.
   * 30) lets you see the direction in which cars are going. (Setting this to a negative number
   * probably gives you "driving on the left", but lanes will be wrong (could be fixed), and, more
   * importantly, the simulation still assumes "driving on the right" when locations (with
   * coordinates) are connected to links.) TODO should be configurable
   */
