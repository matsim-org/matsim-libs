...
public class TransimsSnapshotWriter implements SnapshotWriter {
  ...
  public TransimsSnapshotWriter(String filename) {
    try {
      this.out =
          IOUtils.getBufferedWriter(IOUtils.getFileUrl(filename), IOUtils.CHARSET_UTF8, true);
      String header =
          Labels.VEHICLE
              + "\t"
              + Labels.TIME
  ...
  public void addAgent(AgentSnapshotInfo position) {
    ...
    String buffer =
        position.getId().toString()
            ...
            + "\t0\t0\t1\t0\t"
            + position
                .getColorValueBetweenZeroAndOne() // link(0), from node(0), lane(1), dist(0), speed
            + "\t1\t0\t"
            + position.getId().toString() // vehtype(1), acceleration(0), driver-id
            + "\t0\t"
            + position.getEasting() // # of passengers(0), easting
            + "\t"
