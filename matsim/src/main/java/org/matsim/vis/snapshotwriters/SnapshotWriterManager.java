...
public class SnapshotWriterManager
    implements MobsimBeforeCleanupListener, MobsimAfterSimStepListener, MobsimInitializedListener {
  ...
  public SnapshotWriterManager(
      int snapshotPeriod, QSimConfigGroup.FilterSnapshots filterSnapshots) {
  ...
  public void notifyMobsimInitialized(MobsimInitializedEvent e) {
    ...
    this.snapshotTime =
        Math.floor(mobsim.getSimTimer().getSimStartTime() / this.snapshotPeriod)
            * this.snapshotPeriod;
  ...
  private void doSnapshot(final double time, VisMobsim visMobsim) {
    if (!this.snapshotWriters.isEmpty()) {
      ...
      // This could be parallel since mihal has fixed some concurrency issues in the
      // SnapshotInfoBuilder
      // I think mainly by using separate builders for each generated AgentPositionInfo.
      // I don't have time to test this right now, but if this ever appears to be a bottle neck,
      // this probably
      // can be replaced by a parallel stream. janek: oct' 2021
      var positions =
          visMobsim.getVisNetwork().getVisLinks().values().stream()
              .filter(visLink -> isGenerateSnapshot(visLink.getLink()))
              .flatMap(
                  visLink -> visLink.getVisData().addAgentSnapshotInfo(new HashSet<>()).stream())
    ...
    switch (filterSnapshots) {
      ...
      case withLinkAttributes:
        return Objects.equals(
            link.getAttributes().getAttribute(SnapshotWritersModule.GENERATE_SNAPSHOT_FOR_LINK_KEY),
            true);
      default:
        throw new RuntimeException(
            "Unexpected filter snapshot setting: "
                + filterSnapshots
                + " Possible are: [no, withLinkAttributes]. This can be changed in config.qsim.filterSnapshots");
    }
  }
}
