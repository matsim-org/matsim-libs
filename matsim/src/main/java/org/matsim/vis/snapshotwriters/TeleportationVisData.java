...
public class TeleportationVisData implements AgentSnapshotInfo {
  ...
  public TeleportationVisData(
      double now, Id<Person> personId, Coord fromCoord, Coord toCoord, double travelTime) {
    this.starttime = now;
    ...
    // the following is there to somewhat shift teleported agents.  So that they are not exactly on
    // top of each other when they have
    // exactly the same dp time and destination ... as may happen e.g. for teleported transit walk
    // and illustrative examples. kai, apr'16
