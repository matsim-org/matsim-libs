...
public class SnapshotWritersModule extends AbstractModule {
  ...
  private static class SnapshotWriterManagerProvider implements Provider<MobsimListener> {
    ...
    @Inject
    private SnapshotWriterManagerProvider(
    ...
    public MobsimListener get() {
      if (iterationContext.getIteration() % controllerConfigGroup.getWriteSnapshotsInterval()
          == 0) {
        SnapshotWriterManager manager =
            new SnapshotWriterManager(
                (int) qSimConfigGroup.getSnapshotPeriod(), qSimConfigGroup.getFilterSnapshots());
