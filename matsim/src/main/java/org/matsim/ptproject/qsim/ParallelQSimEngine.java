...
public class ParallelQSimEngine extends QSimEngineImpl{
	...
	private void initQSimEngineThreads(int numOfThreads)
	{
		...
		for (int i = 0; i < numOfThreads; i++)
		{
			QSimEngineThread thread = new QSimEngineThread(simulateAllNodes, simulateAllLinks, this.startBarrier, this.separationBarrier, 
					this.endBarrier, this.getQSim(), this.getAgentSnapshotInfoBuilder(null));
	...
	{
		this.parallelSimLinksLists = new ArrayList<List<QLink>>();

		for (int i = 0; i < this.numOfThreads; i++)
		{
			this.parallelSimLinksLists.add(new ArrayList<QLink>());
		}
