...
public final class VehicleUtils {
  ...
  private static final VehicleType DEFAULT_VEHICLE_TYPE =
      VehicleUtils.getFactory()
          .createVehicleType(Id.create("defaultVehicleType", VehicleType.class));
   ...
   *
   * <p>If config.qsim().getVehicleSource() is "modeVehicleTypesFromVehiclesData", the returned id
   * is a combination of the person's id and the supplied mode. E.g. "person1_car
   *
  ...
  public static void copyFromTo(VehicleType in, VehicleType out) {
    ...
    out.getCostInformation()
        .setCostsPerSecond(cost.getCostsPerSecond())
        .setCostsPerMeter(cost.getCostsPerMeter())
        .setFixedCost(cost.getFixedCosts());
  ...
  public static boolean hasVehicleId(Person person, String mode) {
    PersonVehicles personVehicles =
        (PersonVehicles) person.getAttributes().getAttribute(VehicleUtils.VEHICLE_ATTRIBUTE_KEY);
  ...
  public static Map<String, Id<Vehicle>> getVehicleIds(Person person) {
    ...
    if (personVehicles == null) {
      throw new RuntimeException(
          "Could not retrieve vehicle id from person: "
  ...
  /**
   * Retrieve the optional vehicle types per mode that might be assigned to a person. Returns null
   * if this is not defined.
   */
  public static Map<String, Id<VehicleType>> getVehicleTypes(Person person) {
    var personVehicles =
        (PersonVehicleTypes)
            person.getAttributes().getAttribute(VehicleUtils.VEHICLE_TYPES_ATTRIBUTE_KEY);
   ...
   * @throws RuntimeException In case no vehicleIds were set or in case no vehicleId was set for the
   *     specified mode
   */
  ...
  /**
   * Attaches vehicle ids to a person, so that the router knows which vehicle to use for which mode
   * and person.
   *
   * @param modeToVehicle mode string mapped to vehicle ids. The provided map is copied and stored
   *     as unmodifiable map. If a mode key already exists in the persons's attributes it is
   *     overridden. Otherwise, existing and provided values are merged into one map We use
   *     PersonVehicle Class in order to have a dedicated PersonVehicleAttributeConverter to/from
   *     XML
   */
  public static void insertVehicleIdsIntoAttributes(
      Person person, Map<String, Id<Vehicle>> modeToVehicle) {
  ...
  /**
   * Attaches vehicle types to a person, so that the router knows which vehicle to use for which
   * mode and person.
   *
   * @param modeToVehicleType mode string mapped to vehicle type ids. The provided map is copied and
   *     stored as unmodifiable map.
   */
  public static void insertVehicleTypesIntoAttributes(
      Person person, Map<String, Id<VehicleType>> modeToVehicleType) {
  ...
  public static VehicleType.DoorOperationMode getDoorOperationMode(VehicleType vehicleType) {
    ...
    if (attribute == null) {
      return VehicleType.DoorOperationMode
          .serial; // this was the default value in V1; could also return null instead.
  ...
  public static void setDoorOperationMode(
      VehicleType vehicleType, VehicleType.DoorOperationMode mode) {
  ...
  public static void setHbefaTechnology(
      EngineInformation engineInformation, String hbefaTechnology) {
  ...
  public static void setHbefaVehicleCategory(
      EngineInformation engineInformation, String hbefaVehicleCategory) {
  ...
  public static void setHbefaEmissionsConcept(
      EngineInformation engineInformation, String emissionsConcept) {
  ...
  public static void setEnergyConsumptionKWhPerMeter(
      EngineInformation engineInformation, double energyConsumptionKWhPerMeter) {
  ...
  public static void setEnergyCapacity(
      EngineInformation engineInformation, double energyCapacityInKWhOrLiters) {
  ...
  public static void setCostsPerSecondWaiting(
      CostInformation costInformation, double costsPerSecond) {
  ...
  public static void setCostsPerSecondInService(
      CostInformation costInformation, double costsPerSecond) {
  ...
  @Deprecated
  static void setFuelType(
      EngineInformation engineInformation, EngineInformation.FuelType fuelType) {
    ...
    if (vehicle == null) {
      if (tryStdCnt > 0) {
        ...
        log.info(
            "vehicleId="
                + vehicleId
                + " not in allVehicles; trying standard vehicles container ...");
        if (tryStdCnt == 0) {
    ...
    if (vehicle == null) {
      if (tryTrnCnt > 0) {
        ...
        log.info(
            "vehicleId="
                + vehicleId
                + " not in allVehicles; trying transit vehicles container ...");
        if (tryTrnCnt == 0) {
          log.info(Gbl.FUTURE_SUPPRESSED);
        }
      }
