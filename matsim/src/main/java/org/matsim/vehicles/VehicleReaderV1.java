...
final class VehicleReaderV1 extends MatsimXmlParser {
  ...
  public void endTag(final String name, final String content, final Stack<String> context) {
    if (VehicleSchemaV1Names.DESCRIPTION.equalsIgnoreCase(name) && (content.trim().length() > 0)) {
      ...
      VehicleUtils.setFuelType(
          this.currentVehType.getEngineInformation(),
          EngineInformation.FuelType.valueOf(content.trim()));
    ...
    if (VehicleSchemaV1Names.VEHICLETYPE.equalsIgnoreCase(name)) {
      this.currentVehType =
          this.builder.createVehicleType(
              Id.create(atts.getValue(VehicleSchemaV1Names.ID), VehicleType.class));
      ...
      this.currentVehType
          .getCapacity()
          .setSeats(Integer.valueOf(atts.getValue(VehicleSchemaV1Names.PERSONS)));
    } else if (VehicleSchemaV1Names.STANDINGROOM.equalsIgnoreCase(name)) {
      this.currentVehType
          .getCapacity()
          .setStandingRoom(Integer.valueOf(atts.getValue(VehicleSchemaV1Names.PERSONS)));
      ...
      if (atts.getValue(VehicleSchemaV1Names.CUBICMETERS).contentEquals("INF")) {
        ...
        this.currentVehType
            ...
            .setVolumeInCubicMeters(
                Double.parseDouble(atts.getValue(VehicleSchemaV1Names.CUBICMETERS)));
      }
    } else if (VehicleSchemaV1Names.GASCONSUMPTION.equalsIgnoreCase(name)) {
      VehicleUtils.setFuelConsumption(
          this.currentVehType,
          Double.parseDouble(atts.getValue(VehicleSchemaV1Names.LITERPERMETER)));
    } else if (VehicleSchemaV1Names.VEHICLE.equalsIgnoreCase(name)) {
      Id<VehicleType> typeId =
          Id.create(atts.getValue(VehicleSchemaV1Names.TYPE), VehicleType.class);
      ...
      VehicleUtils.setDoorOperationMode(
          this.currentVehType,
          VehicleType.DoorOperationMode.valueOf((atts.getValue(VehicleSchemaV1Names.MODE))));
    } else if (VehicleSchemaV1Names.PASSENGERCAREQUIVALENTS.equalsIgnoreCase(name)) {
      this.currentVehType.setPcuEquivalents(
          Double.parseDouble(atts.getValue(VehicleSchemaV1Names.PCE)));
    }
  }
}
