...
public final class VehicleType implements Attributable, Identifiable<VehicleType> {
  ...
  // If something like inheritance is needed, please change this class to VehicleTypeImpl, and
  // extract interface under previous name VehicleType.
  // And then use delegation. kai, sep'19

  // yy should maybe the whole type be immutable? I guess that the question is how people use this.
  // If they have a vehicle, get the type, and then keep
  // a reference to the type, then replacing the type means that it will have another reference, and
  // these users will not notice.  ???????

  private double width = 1.0;
  private double maxVelocity =
      Double.POSITIVE_INFINITY; // default: constrained only by the link speed
  ...
  // the following are attributes that did not seem universal enough and thus were relegated to
  // free-form Attributes for the time being.  kai/kai, sep'19
  /**
