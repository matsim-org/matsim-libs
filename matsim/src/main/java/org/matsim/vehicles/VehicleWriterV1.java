...
public final class VehicleWriterV1 extends MatsimXmlWriter {
  ...
  private void writeRootElement() throws UncheckedIOException {
    ...
    atts.add(
        this.createTuple(
            "xsi:schemaLocation",
            MATSIM_NAMESPACE + " " + DEFAULT_DTD_LOCATION + "vehicleDefinitions_v1.0.xsd"));
  ...
  private void writeVehicleTypes(Map<Id<VehicleType>, VehicleType> vts)
    ...
    List<VehicleType> sortedVehicleTypes =
        vts.values().stream().sorted(comparing(VehicleType::getId)).collect(toList());
    for (VehicleType vt : sortedVehicleTypes) {
      ...
      if (!Double.isNaN(vt.getMaximumVelocity()) && !Double.isInfinite(vt.getMaximumVelocity())) {
        ...
        atts.add(
            this.createTuple(
                VehicleSchemaV1Names.METERPERSECOND, Double.toString(vt.getMaximumVelocity())));
      ...
      atts.clear();
      atts.add(
          this.createTuple(VehicleSchemaV1Names.SECONDSPERPERSON, VehicleUtils.getAccessTime(vt)));
      this.writeStartTag(VehicleSchemaV1Names.ACCESSTIME, atts, true);
      atts.clear();
      atts.add(
          this.createTuple(VehicleSchemaV1Names.SECONDSPERPERSON, VehicleUtils.getEgressTime(vt)));
      this.writeStartTag(VehicleSchemaV1Names.EGRESSTIME, atts, true);
      atts.clear();
      atts.add(
          this.createTuple(
              VehicleSchemaV1Names.MODE, VehicleUtils.getDoorOperationMode(vt).toString()));
  ...
  private void writeEngineInformation(EngineInformation ei) throws UncheckedIOException {
    ...
    if ((VehicleUtils.getFuelConsumption(ei) != null)) {
      atts.add(
          this.createTuple(
              VehicleSchemaV1Names.LITERPERMETER, VehicleUtils.getFuelConsumption(ei)));
