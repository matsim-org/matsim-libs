...
final class VehiclesImpl implements Vehicles {
   ...
   *
   * @param v
   * @throws IllegalArgumentException if another {@link Vehicle} with the same {@link Id} already
  ...
  public void addVehicle(final Vehicle v) {
    ...
    if (!this.vehicleTypes.containsKey(v.getType().getId())) {
      throw new IllegalArgumentException(
          "Cannot add Vehicle with type = "
              + v.getType().getId().toString()
   ...
   *
   * @param type
   * @throws IllegalArgumentException if another {@link VehicleType} with the same {@link Id}
  ...
  public void addVehicleType(VehicleType type) {
    ...
    if (this.getVehicleTypes().containsKey(type.getId())) {
      throw new IllegalArgumentException(
          "Vehicle type with id = " + type.getId() + " already exists.");
    }
