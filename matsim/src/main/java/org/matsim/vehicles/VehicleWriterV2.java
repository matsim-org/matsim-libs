...
final class VehicleWriterV2 extends MatsimXmlWriter {
  ...
  private void writeRootElement() throws UncheckedIOException, IOException {
    ...
    atts.add(
        this.createTuple(
            "xsi:schemaLocation",
            MATSIM_NAMESPACE + " " + DEFAULT_DTD_LOCATION + "vehicleDefinitions_v2.0.xsd"));
  ...
  private void writeVehicles(Map<Id<Vehicle>, Vehicle> veh)
      throws UncheckedIOException, IOException {
    List<Vehicle> sortedVehicles =
        veh.values().stream().sorted(comparing(Vehicle::getId)).collect(toList());
  ...
  private void writeVehicleTypes(Map<Id<VehicleType>, VehicleType> vts)
      throws UncheckedIOException, IOException {
    this.writer.write("\n");
    List<VehicleType> sortedVehicleTypes =
        vts.values().stream()
    ...
    for (VehicleType vt : sortedVehicleTypes) {
      ...
      if (vt.getCapacity() != null) {
        ...
        if (vehicleCapacity.getStandingRoom() != null) {
          atts.add(
              this.createTuple(
                  VehicleSchemaV2Names.STANDINGROOM, vehicleCapacity.getStandingRoom()));
        }
        if (vehicleCapacity.getVolumeInCubicMeters() != null
            && !Double.isNaN(vehicleCapacity.getVolumeInCubicMeters())
            && !Double.isInfinite(vehicleCapacity.getVolumeInCubicMeters())) {
          atts.add(
              this.createTuple(
                  VehicleSchemaV2Names.VOLUME, vehicleCapacity.getVolumeInCubicMeters()));
        }
        if (vehicleCapacity.getWeightInTons() != null
            && !Double.isNaN(vehicleCapacity.getWeightInTons())
            && !Double.isInfinite(vehicleCapacity.getWeightInTons())) {
          atts.add(
              this.createTuple(VehicleSchemaV2Names.WEIGHT, vehicleCapacity.getWeightInTons()));
        }
        if (vehicleCapacity.getOther() != null
            && !Double.isNaN(vehicleCapacity.getOther())
            && !Double.isInfinite(vehicleCapacity.getOther())) {
        ...
        attributesWriter.writeAttributes(
            "\t\t\t", this.writer, vehicleCapacity.getAttributes(), false);
      ...
      if (!Double.isNaN(vt.getMaximumVelocity()) && !Double.isInfinite(vt.getMaximumVelocity())) {
        ...
        atts.add(
            this.createTuple(
                VehicleSchemaV2Names.METERPERSECOND, Double.toString(vt.getMaximumVelocity())));
      ...
      if (vt.getEngineInformation() != null
          & !vt.getEngineInformation().getAttributes().isEmpty()) {
        atts.clear();
        this.writeStartTag(VehicleSchemaV2Names.ENGINEINFORMATION, atts);
        this.writer.write("\n");
        attributesWriter.writeAttributes(
            "\t\t\t", this.writer, vt.getEngineInformation().getAttributes(), false);
      ...
      if (vt.getCostInformation() != null) {
        ...
        if (costInformation.getFixedCosts() != null
            && !Double.isNaN(costInformation.getFixedCosts())) {
          atts.add(
              this.createTuple(
                  VehicleSchemaV2Names.FIXEDCOSTSPERDAY, costInformation.getFixedCosts()));
        }
        if (costInformation.getCostsPerMeter() != null
            && !Double.isNaN(costInformation.getCostsPerMeter())) {
          atts.add(
              this.createTuple(
                  VehicleSchemaV2Names.COSTSPERMETER, costInformation.getCostsPerMeter()));
        }
        if (costInformation.getCostsPerSecond() != null
            && !Double.isNaN(costInformation.getCostsPerSecond())) {
          atts.add(
              this.createTuple(
                  VehicleSchemaV2Names.COSTSPERSECOND, costInformation.getCostsPerSecond()));
        }
        this.writeStartTag(VehicleSchemaV2Names.COSTINFORMATION, atts);
        // attributes for costInformation
        this.writer.write("\n");
        attributesWriter.writeAttributes(
            "\t\t\t", this.writer, costInformation.getAttributes(), false);
