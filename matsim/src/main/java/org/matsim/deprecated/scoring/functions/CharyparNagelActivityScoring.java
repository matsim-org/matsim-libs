...
public class CharyparNagelActivityScoring
    implements ActivityScoring, SumScoringFunction.ActivityScoring {
  // yy should be final.  kai, oct'14
  // yyyy "implements SumScoringFunction.ActivityScoring" is needed somewhere in location choice ...
  // where this
    ...
    if (lastActivity.getType().equals(this.firstActivity.getType())) {
      ...
      if (this.params.scoreActs) {
        if (firstLastActWarning <= 10) {
          log.warn(
              "The first and the last activity do not have the same type. "
