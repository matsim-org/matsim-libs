...
/**
 * This is a scoring function which is configurable with sum terms for Activities, Legs, money
 * spent, and becoming stuck. You can create an instance of this class from your own
 * ScoringFunctionFactory and add different sum terms to it, for example depending on a
 * configuration option.
 *
 * <p>The sum terms are instances which implement BasicScoring as well as one of ActivityScoring,
 * LegScoring, MoneyScoring or AgentStuckScoring. They are notified about these events, and after
 * finish() is called on them, they must be able to answer getScore(). Instances of BasicScoring,
 * like ScoringFunctions, must be able to onIterationStarts() themselves between iterations, though
 * this is currently not used (fresh instances are created).
 *
 * <p>Note that the startActivity() and endActivity() as well as starLeg() and endLeg() are
 * redundant. The start time as well as the end time is contained in the Activity and the Leg. This
 * is for historical reasons, when the Activity and the Leg which were passed were instances from
 * the Plan.
 *
 * <p>It is by no means necessary to use this class for a ScoringFunction. You are encouraged to
 * just implement ScoringFunction.
 *
...
public final class ScoringFunctionAccumulator implements ScoringFunction {
  ...
  private ArrayList<AgentStuckScoring> agentStuckScoringFunctions =
      new ArrayList<AgentStuckScoring>();
  private ArrayList<ArbitraryEventScoring> arbtraryEventScoringFunctions =
      new ArrayList<ArbitraryEventScoring>();

  @Deprecated // preferably use SumScoringFunction.  kai, oct'13
  public
  ScoringFunctionAccumulator() {} // empty constructor so I can find where this is called.  kai,
