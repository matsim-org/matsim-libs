...
class BackpackPlan {
	...
	public void handleEvent(Event e) {
		...
		switch (e) {
			...
			case PersonStuckEvent pse -> handleStuck(pse);
	...
	private void handlePersonDeparture(PersonDepartureEvent e) {
		...
		var routeBuilder = providers.get(e.getLegMode()).get();
		currentLeg = new ExperiencedLegBuilder(routeBuilder);
	...
	void handleStuck(PersonStuckEvent pse) {

		if (currentLeg != null) {
			currentLeg.handleEvent(pse);
		}
		if (currentActivity != null) {
			currentActivity.handleEvent(pse);
		}
	...
	record Msg(Plan experiencedPlan, ExperiencedActivityBuilder.Msg actState, ExperiencedLegBuilder.Msg legState) implements Message {
	}
