...
public class HouseholdsImpl implements Households {
  ...
  /**
   * Adds the household to the container. If streaming is set, the household is added, the
   * algorithms are run on the household, and it is subsequently removed. If not, the household is
   * added incrementally to the container.
   *
   * @param household
   * @throws IllegalArgumentException if the container already includes the {@link Id} of the
   *     household being added.
   */
  public final void addHousehold(final Household household) {
    ...
    if (this.getHouseholds().containsKey(household.getId())) {
      throw new IllegalArgumentException(
          "Household with Id " + household.getId() + " already exisits.");
    }
