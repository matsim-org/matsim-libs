...
public class HouseholdsAlgorithmRunner {
  ...
  public void runAlgorithms(Household h) {
    for (HouseholdAlgorithm algorithm : this.householdAlgorithms) {
      log.info(
          "Running algorithm " + algorithm.getClass().getName() + " on household " + h.getId());
      algorithm.run(h);
    }
  }
  ...
  /**
   * Removes the first instance found of the algorithm from the list. It is possible that the same
   * algorithm can appear multiple times in the list.
   *
   * @param algorithm
   * @return
   */
  ...
  /**
   * Add the algorithm to the container. Algorithms will be executed in the same sequence in which
   * they are added.
   *
   * @param algorithm
   */
