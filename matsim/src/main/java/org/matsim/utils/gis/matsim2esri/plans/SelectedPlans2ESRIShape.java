...
/**
 * Simple class to convert MATSim plans to ESRI shape files. Activities will be converted into
 * points and legs will be converted into line strings. Parameters as defined in the population xml
 * file will be added as attributes to the shape files. There are also some parameters to configure
 * this converter, please consider the corresponding setters in this class.
 *
 * @author laemmel
 */
public class SelectedPlans2ESRIShape {
      ...
      for (PlanElement pe : plan.getPlanElements()) {
        if (pe instanceof Leg) {
          ...
          if (leg.getRoute() instanceof NetworkRoute) {
            if (RouteUtils.calcDistanceExcludingStartEndLink(
                    (NetworkRoute) leg.getRoute(), this.network)
                > 0) {
  ...
  private SimpleFeature getActFeature(final String id, final Activity act) {
    ...
    try {
      return this.actBuilder.buildFeature(
          null, new Object[] {MGC.coord2Point(c), id, type, linkId, startTime, endTime});
  ...
  private SimpleFeature getLegFeature(final Leg leg, final String id) {
    ...
    Double dist =
        RouteUtils.calcDistanceExcludingStartEndLink((NetworkRoute) leg.getRoute(), this.network);
    ...
    try {
      return this.legBuilder.buildFeature(
          null, new Object[] {ls, id, mode, depTime, travTime, dist});
    } catch (IllegalArgumentException e) {
      e.printStackTrace();
    }
  ...
  public static void main(final String[] args) {
    ...
    SelectedPlans2ESRIShape sp =
        new SelectedPlans2ESRIShape(
            scenario.getPopulation(), scenario.getNetwork(), crs, outputDir);
