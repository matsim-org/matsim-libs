...
public final class AttributesImpl implements Attributes {
  ...
  //
  // In addition, as lots of classes implement Attributable, there might be a large number of empty
  // attributes,
  // which would result in unnecessary memory overhead if each attribute would use new instances of
  // empty arrays
  // (Which are essentially immutable objects), hence the two "empty" constants (idea from Marcel
  // Rieser, see MATSIM-811)
  ...
  /**
   * Returns a view of the mappings stored by this object as an immutable Map. Behavior is undefined
   * if the mappings are modified after this method was called.
   *
   * <p>It is mostly provided to allow iterating through all attributes for writing to file.
   *
