...
/**
 * Object that converts arbitrary objects to and from strings based on the logic defined by
...
public class ObjectAttributesConverter {
  ...
  public ObjectAttributesConverter() {
    ...
    this.converters.put(
        PersonVehicleTypes.class.getName(), new PersonVehicleTypesAttributeConverter());
  }
  ...
  private AttributeConverter getConverter(String className) {
    ...
    try {
      ...
      if (missingConverters.add(className)) {
        log.warn(
            "No AttributeConverter found for class "
                + className
    ...
    // we pass in a lot of maps here that we can and (maybe) do not want to write
    {
      if (converter instanceof StringStringMapConverter) {
        ...
        if (!map.isEmpty()) {
          ...
          if (!(firstEntry.getKey() instanceof String && firstEntry.getValue() instanceof String))
            return null;
        }
      }
