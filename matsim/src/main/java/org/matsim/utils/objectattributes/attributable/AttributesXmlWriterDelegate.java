...
public class AttributesXmlWriterDelegate {
  ...
  public final void writeAttributes(
      final String indentation, final BufferedWriter writer, final Attributes attributes) {
    writeAttributes(indentation, writer, attributes, true);
  }

  public final void writeAttributes(
      final String indentation, final StringBuilder writer, final Attributes attributes) {
    writeAttributes(indentation, writer, attributes, true);
  }
    ...
    try {
      ...
      for (Map.Entry<String, Object> objAttribute : attributes.getAsMap().entrySet()) {
        Class<?> clazz =
            objAttribute
                .getValue()
                .getClass(); // TODO: Does not work if value is null. Shall we allow for the value
        // being null? - gl-oct'19
        String converted = converter.convertToString(objAttribute.getValue());
        if (converted != null) {
          writer.write(indentation + "\t");
          writer.write(
              "<attribute name=\"" + XmlUtils.encodeAttributeValue(objAttribute.getKey()) + "\" ");
