 ...
 * supported by implementing {@link AttributeConverter} and adding it to the reader with {@link
 * #putAttributeConverter(Class, AttributeConverter)}.
 *
 * @author mrieser
 */
public class ObjectAttributesXmlReader extends MatsimXmlParser {
  private static final Logger log = LogManager.getLogger(ObjectAttributesXmlReader.class);
   ...
   * @param clazz
   * @param converter
   * @return the previously registered converter for this class, or <code>null</code> if none was
   *     set before.
   */
  public AttributeConverter<?> putAttributeConverter(
      final Class<?> clazz, final AttributeConverter<?> converter) {
   ...
   *
   * @param clazz
   * @return the previously registered converter for this class, of <code>null</code> if none was
   *     set.
   */
