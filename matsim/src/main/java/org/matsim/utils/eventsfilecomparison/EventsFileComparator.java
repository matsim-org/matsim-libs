...
/**
 * This class checks if two events files are semantic equivalent. The order of the events does not
 * matter as long as they are chronologically sorted.
 *
 * @author mrieser
 * @author laemmel
 */
public final class EventsFileComparator {
   ...
   * @return <code>Result.FILES_ARE_EQUAL</code> if the events files are equal, or some error code
   *     (see {@link Result}) if not.
   */
      ...
      for (Entry<String, Counter> entry : map1.entrySet()) {
        ...
        if (counter == null) {
          ...
          if (counter.getCount() != entry.getValue().getCount()) {
            log.warn(
                "Wrong event count for: "
                    + entry.getKey()
                    + "\n"
                    + entry.getValue().getCount()
                    + " times in file:"
                    + worker1.getEventsFile()
                    + "\n"
                    + counter.getCount()
                    + " times in file:"
                    + worker2.getEventsFile());
