...
public class MatsimGrapher extends AbstractInjectorGrapher {
  ...
  protected void postProcess() {
    // More or less arbitrarily filter out graph nodes that I think are unhelpful clutter, like
    // config
    // groups, scenario elements, and things that aren't usually overridden.
    Graph<Node, Edge> filteredGraph = filterGraph();

    Graph<Node, Edge> graphComponentReachableFromControler =
        findGraphComponentReachableFromControler(filteredGraph);
  ...
  private Graph<Node, Edge> filterGraph() {
    return new MaskSubgraph<>(
        g,
        node -> {
          if (ConfigGroup.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (Network.class.isAssignableFrom(node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (Population.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (DumpDataAtEnd.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (OutputDirectoryHierarchy.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (MatsimServices.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (Injector.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (PopulationFactory.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (Scenario.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          ...
          if (IterationStopWatch.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (EventsManager.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (ReplanningContext.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (PlansDumping.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (ActivityFacilities.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (EventsHandling.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (TravelTimeCalculator.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (SingleModeNetworksCache.class.isAssignableFrom(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (ExperiencedPlansService.class.equals(
              node.getId().getKey().getTypeLiteral().getRawType())) {
            return true;
          }
          if (node.getId()
              ...
              .getTypeLiteral()
              .toString()
              .contains("ExperiencedPlansServiceImpl")) {
            return true;
          }
          ...
          if (node.getId()
              ...
              .getTypeLiteral()
              .toString()
              .contains("LeastCostPathCalculatorFactory")) {
            return true;
          }
          ...
          if (node.getId()
              .getKey()
              .getTypeLiteral()
              .equals(new TypeLiteral<Set<MobsimListener>>() {})) {
            return true;
          }
          return false;
        },
  ...
  private Graph<Node, Edge> findGraphComponentReachableFromControler(
      Graph<Node, Edge> filteredGraph) {
    ...
    Graphs.addGraph(
        graphComponentReachableFromControler,
        new MaskSubgraph<>(
            g, node -> !ci.connectedSetOf(controlerNode).contains(node), edge -> false));
