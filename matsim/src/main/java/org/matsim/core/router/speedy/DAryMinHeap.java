 ...
 *
 * <p>A d-ary min-heap is a generalization of the binary min-heap, which provides better cpu cache
 * locality and has faster decrease-key operations but slower poll operations. But in Dijkstra's
 * algorithm, decrease-key is more common than poll, so it should still be beneficial.
 *
 * <p>In most literature, the poll-operation is implemented by removing the top element and
 * replacing it with the last entry in the heap, and then let this new root sift down again until
 * the heap-conditions are all met. This requires multiple comparisons on each level (the value must
 * be compared with each of its children).
 *
 * <p>An alternative implementation tries to fix this in the following way: the "whole" at the root
 ...
 *
 * <p>This implementation uses the alternative implementation for additional speedup.
 *
