 ...
 *
 * <p>This class is thread-safe and can safely be used by multiple threads.
 *
 * @author mrieser / Simunto, sponsored by SBB Swiss Federal Railways
 */
class SpeedyALTData {

  private static final Logger LOG = LogManager.getLogger(SpeedyALTData.class);
  ...
  private void findDeadEnds() {
    ...
    for (int nodeIdx = 0; nodeIdx < this.graph.nodeCount; nodeIdx++) {
      ...
      int nIdx = nodeIdx;
      int otherNodeIndex =
          checkNodeForDeadend(this.deadendData, mergedDeadends, nIdx, nodeIdx, outLI, inLI);
  ...
  private int checkNodeForDeadend(
      int[] deadends,
      Map<Integer, Integer> mergedDeadends,
      int nodeIdx,
      int currentDeadend,
      LinkIterator outLI,
      LinkIterator inLI) {
  ...
  private double[] calculateTreeForward(int node) {
    ...
    while (!pq.isEmpty()) {
      ...
      while (outLI.next()) {
        ...
        double newCost =
            currCost
                + this.travelCosts.getLinkMinimumTravelDisutility(
                    this.graph.getLink(outLI.getLinkIndex()));
    ...
    while (!pq.isEmpty()) {
      ...
      while (inLI.next()) {
        ...
        double newCost =
            currCost
                + this.travelCosts.getLinkMinimumTravelDisutility(
                    this.graph.getLink(inLI.getLinkIndex()));
