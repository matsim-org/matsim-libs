...
/**
 * Implementation of <a href="http://en.wikipedia.org/wiki/Dijkstra%27s_algorithm">Dijkstra's
 * shortest-path algorithm</a> on a time-dependent network with arbitrary non-negative cost
 * functions (e.g. negative link cost are not allowed). So 'shortest' in our context actually means
 * 'least-cost'.
 *
 * <p>For every router, there exists a class which computes some preprocessing data and is passed to
 * the router class constructor in order to accelerate the routing procedure. The one used for
 * Dijkstra is {@link org.matsim.core.router.util.PreProcessDijkstra}. <br>
 *
 * <h2>Code example:</h2>
 *
 * <p><code>PreProcessDijkstra preProcessData = new PreProcessDijkstra();<br>
 ...
 *
 * <p>If you don't want to preprocess the network, you can invoke Dijkstra as follows:
 *
 * <p><code> LeastCostPathCalculator routingAlgo = new Dijkstra(network, costFunction);</code>
 *
...
public class Dijkstra implements LeastCostPathCalculator {
  // yyyyyy I don't think that we should make this class publicly inheritable; as we know, will
  // eventually lead
  ...
  /**
   * Provides an unique id (loop number) for each routing request, so we don't have to reset all
   * nodes at the beginning of each re-routing but can use the loop number instead.
   */
  private int iterationID = Integer.MIN_VALUE + 1;

  /**
   * Temporary field that is only used if dead ends are being pruned during routing and is updated
   * each time a new route has to be calculated.
   */
  private Node deadEndEntryNode;

  /**
   * Determines whether we should mark nodes in dead ends during a pre-processing step so they won't
   * be expanded during routing.
   */
   ...
   * Default constructor.
   *
   * @param network The network on which to route.
   * @param costFunction Determines the link cost defining the cheapest route.
   * @param timeFunction Determines the travel time on links.
   */
   ...
   * @param preProcessData The pre processing data used during the routing phase.
   */
  // please use DijkstraFactory when you want to create an instance of this
  protected Dijkstra(
  ...
  /**
   * Calculates the cheapest route from Node 'fromNode' to Node 'toNode' at starting time
   * 'startTime'.
   *
   * @param fromNode The Node at which the route should start.
   * @param toNode The Node at which the route should end.
   * @param startTime The time at which the route should start.
   * @see
   *     org.matsim.core.router.util.LeastCostPathCalculator#calcLeastCostPath(org.matsim.api.core.v01.network.Node,
   *     org.matsim.api.core.v01.network.Node, double, org.matsim.api.core.v01.population.Person,
  ...
  /**
   * Logic that was previously located in the calcLeastCostPath(...) method. Can be overwritten in
   * the MultiModalDijkstra. Returns the last node of the path. By default this is the to-node. The
   * MultiNodeDijkstra returns the cheapest of all given to-nodes.
   */
  /*package*/ Node searchLogic(
      final Node fromNode,
      final Node toNode,
      final RouterPriorityQueue<Node> pendingNodes,
      Person person,
      Vehicle vehicle) {
    // It is a bit overkill to pass Person and Vehicle.  However, since the method is
    // package-private, I think that it is acceptable.  For
   ...
   *
   * @param fromNode The node where the path starts.
   * @param toNode The node where the path ends.
   * @param startTime The time when the trip starts.
   */
   ...
   *
   * @param fromNode The Node to be initialized.
   * @param toNode The Node at which the route should end.
   * @param startTime The time we start routing.
   * @param pendingNodes The pending nodes so far.
   */
  /*package*/ void initFromNode(
      final Node fromNode,
      final Node toNode,
      final double startTime,
      final RouterPriorityQueue<Node> pendingNodes) {
   ...
   *
   * @param l The link from which we came to this Node.
   * @param n The Node to add to the pending nodes.
   * @param pendingNodes The set of pending nodes.
   * @param currTime The time at which we started to traverse l.
   * @param currCost The cost at the time we started to traverse l.
   * @param toNode The target Node of the route.
   * @return true if the node was added to the pending nodes, false otherwise (e.g. when the same
   *     node already has an earlier visiting time).
   */
  protected boolean addToPendingNodes(
    ...
    final double travelTime =
        this.timeFunction.getLinkTravelTime(l, currTime, this.person, this.vehicle);
    final double travelCost =
        this.costFunction.getLinkTravelDisutility(l, currTime, this.person, this.vehicle);
    ...
    if (totalCost < nCost) {
      ...
      // Decide based on the linkId which one to take... just have to common criteria to be
      // deterministic.
  ...
  /**
   * @return <code>true</code> if the link can be passed with respect to a possible mode restriction
   *     set
  ...
  /**
   * Changes the position of the given Node n in the pendingNodes queue and updates its time and
   * cost information.
   *
   * @param n The Node that is revisited.
   * @param data The data for n.
   ...
   * @param time The time of the visit of n.
   * @param cost The accumulated cost at the time of the visit of n.
   * @param outLink The node from which we came visiting n.
   */
  ...
  /**
   * Augments the iterationID and checks whether the visited information in the nodes in the nodes
   * have to be reset.
   */
   ...
   * effective travel cost to sort the nodes in the pending nodes queue during routing.
   */
