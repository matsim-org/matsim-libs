...
/*package*/ class LandmarkerPieSlices {
  ...
  private void putLandmarks(final Collection<? extends Node> nodes, final int landmarkCount) {
    ...
    if (angles.length < landmarkCount) {
      log.info(
          "Reducing number of landmarks from " + landmarkCount + " to " + angles.length + "...");
  ...
  private double[][] fillSectors(
      final ArrayList<ArrayList<Node>> sectors, final Collection<? extends Node> nodes) {
  ...
  private Node getLandmark(final ArrayList<Node> nodes, final double[] angles) {
    ...
    Node landmark = null;
    for (Node node : nodes) {
      if ((node.getOutLinks().size() > 1 && node.getInLinks().size() > 1) || landmark == null) {
        double x = node.getCoord().getX() - this.center.getX();
        double y = node.getCoord().getY() - this.center.getY();
        double angle = Math.atan2(y, x) + Math.PI;
        ...
        // Set the node that is farthest away from the center to be the landmark in the current
        // sector
  ...
  private void refineLandmarks(
      final ArrayList<ArrayList<Node>> sectors, final double[][] sectorAngles) {
    ...
    while (doRefine) {
      ...
      for (int i = 0; i < this.landmarks.length && !sectors.get(i).isEmpty(); i++) {
        ...
        if (angelDiff < minSectorSize * minAngelFactor) {
          ...
          int indexToChange = 0;
          if (CoordUtils.calcEuclideanDistance(this.center, this.landmarks[preInd].getCoord())
              < CoordUtils.calcEuclideanDistance(this.center, this.landmarks[i].getCoord())) {
            // Narrow the sector
          ...
          if (sectors.get(indexToChange).isEmpty()) {
            ...
            this.landmarks[indexToChange] =
                getLandmark(sectors.get(indexToChange), sectorAngles[indexToChange]);
          }
  ...
  private void removeNodesFromSector(final ArrayList<Node> sector, final double[] sectorAngles) {
    int i = 0;
    while (i < sector.size()) {
