...
public final class DefaultRoutingModules {
	// despite the fact that this class is in "old", I actually think that it is ok, providing a public interface to functionality that could be
	// changed underneath. Have to keep in in the "old" package in order to package-protect the infrastructure that is used underneath. kai, mar'15
	...
	public static RoutingModule createPseudoTransitRouter(String mode, PopulationFactory popFac, Network net,
		...
		return new FreespeedFactorRoutingModule(mode, popFac, net, calculatorFactory, travelTime, travelDisutility, params);
	}
	...
	public static RoutingModule createPureNetworkRouter(String mode, PopulationFactory popFact, Network net,
	...
	public static RoutingModule createAccessEgressNetworkRouter(String mode, final LeastCostPathCalculator routeAlgo, Scenario scenario,
			Network filteredNetwork, RoutingModule accessEgressToNetworkRouter, TimeInterpretation timeInterpretation,
			MultimodalLinkChooser multimodalLinkChooser) {

		return new NetworkRoutingInclAccessEgressModule(mode, routeAlgo, scenario, filteredNetwork, null, accessEgressToNetworkRouter,
				accessEgressToNetworkRouter, timeInterpretation, multimodalLinkChooser);
	}
	...
	public static RoutingModule createAccessEgressNetworkRouter(String mode, final LeastCostPathCalculator routeAlgo, Scenario scenario,
		...
		return new NetworkRoutingInclAccessEgressModule(mode, routeAlgo, scenario, filteredNetwork, null, accessToNetworkRouter,
				egressFromNetworkRouter, timeInterpretation, multimodalLinkChooser);
	}

	/**
	 * Creates a new access egress network router.
	 *
	 * @param invertedNetwork if not null, routing will be on the inverted network, in which case routeAlgo needs to be an
	 *        {@link InvertedLeastPathCalculator}
	...
	static RoutingModule createAccessEgressNetworkRouter(String mode, final LeastCostPathCalculator routeAlgo, Scenario scenario,
			Network filteredNetwork, @Nullable Network invertedNetwork, RoutingModule accessToNetworkRouter, RoutingModule egressFromNetworkRouter,
			TimeInterpretation timeInterpretation, MultimodalLinkChooser multimodalLinkChooser) {
		return new NetworkRoutingInclAccessEgressModule(mode, routeAlgo, scenario, filteredNetwork, invertedNetwork, accessToNetworkRouter,
				egressFromNetworkRouter, timeInterpretation, multimodalLinkChooser);
	}
}
