...
/**
 * The (Wrapped)BinaryMinHeap replaces the PseudoRemovePriorityQueue as default PriorityQueue. It
 * offers a decreaseKey method and uses less memory. As a result, the routing performance is
 * increased by ~20%.
 *
 * <p>When an ArrayRoutingNetwork is used, an BinaryMinHeap can be used which uses the
 * getArrayIndex() method of the ArrayRoutingNetworkNodes which further reduces the memory
 * consumption and increases the performance by another ~10%.
 *
 * @author cdobler
 * @author muelleki
...
public class BinaryMinHeap<E extends HasIndex> implements MinHeap<E> {
  ...
  /**
   * Each HeapEntry contains a final integer value that points to a position in the indices array.
   * The value in the indices array points to the position in the data and costs arrays where the
   * HeapEntry is currently located.
   *
   * <p>* data objects, their indices and costs: A(0, 5), B(1, 6), C(2, 7), D(3, 8)
   *
   * <p>[A][B][C][D] indices: [3][0][1][2]
   *
   * <p>data: [B][C][D][A] costs: [6][7][8][5]
   *
   * <p>Example: Index of object A is 0. entry in indices array at position 0 is 3. Therefore, the
   * object is at index 3 in the data array.
   */
  ...
  /**
   * The classic approach of removing the heap's head (poll) is to replace the head with the heap's
   * last entry. Afterwards this entry is sifted downwards until a valid position is reached.
   * However, when doing so, two compare operations have to be performed for each level (comparing
   * with left and right child).
   *
   * <p>In the alternative approach, the head is sifted downwards to the left level in a special
   * way. It replaces always the smaller ones of its children (only they are compared!). After
   * reaching the bottom level, it its replaced with the heap's last entry. Finally, this entry is
   * sifted upwards until a valid position is found. This approach should perform fewer compare
   * operations than the classical approach. Idea: see
   * http://magazin.c-plusplus.de/artikel/Binary%20Heaps
   */
  ...
  private int removeSiftDown(int nodeIndex) {
    while (true) {
      ...
      for (int rightChildIndex = leftChildIndex + 1;
          rightChildIndex < limitChildIndex;
          rightChildIndex++) {
        /*
        ...
        if (leftCosts >= rightCosts
            && (leftCosts > rightCosts
                || this.getIndex(data[leftChildIndex]) > this.getIndex(data[rightChildIndex]))) {
   ...
   * sorted by their priority. However, their order should be deterministic since the underlying
   * array is. Removing elements is not supported via the iterator.
   *
  ...
  private void siftUp(int index, E newEntry, double newCost) {
    while (index > 0) {
      ...
      if (newCost == parentCost && this.getIndex(newEntry) > this.getIndex(data[parentIndex]))
        break;
  ...
  private void siftDown(int nodeIndex) {
    ...
    for (int childIndex = leftChildIndex; childIndex < limitChildIndex; childIndex++) {
      ...
      if (childCosts <= minCosts) {
        if (childCosts < minCosts
            || this.getIndex(data[childIndex]) < this.getIndex(data[minIndex])) {
          minIndex = childIndex;
          minCosts = childCosts;
        }
      }
