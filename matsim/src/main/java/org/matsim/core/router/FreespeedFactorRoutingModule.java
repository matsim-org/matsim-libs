...
public final class FreespeedFactorRoutingModule implements RoutingModule {
  ...
  /* package (for tests) */ final double routeLeg(
      Person person, Leg leg, Activity fromAct, Activity toAct, double depTime) {
    int travTime = 0;
    ...
    if (toLink != fromLink) {
      ...
      // we're still missing the time on the final link, which the agent has to drive on in the java
      // mobsim
      ...
      travTime =
          (int)
              (((int) path.travelTime + travelTimeLastLink)
                  * this.params.getTeleportedModeFreespeedFactor());
      Route route =
          this.populationFactory
              .getRouteFactories()
              .createRoute(Route.class, fromLink.getId(), toLink.getId());
      route.setTravelTime(travTime);

      // yyyyyy the following should actually rather come from the route!  There is a
      // RouteUtils.calcDistance( route ) .  kai, nov'16
      double dist = 0;
