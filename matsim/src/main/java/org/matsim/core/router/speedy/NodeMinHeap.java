...
class NodeMinHeap {
  ...
  private void minHeapify(int i) {
    ...
    if (left <= (this.size - 1)
        && this.costGetter.getCost(this.heap[left]) < this.costGetter.getCost(this.heap[i])) {
      smallest = left;
    }
    if (right <= (this.size - 1)
        && this.costGetter.getCost(this.heap[right])
            < this.costGetter.getCost(this.heap[smallest])) {
      smallest = right;
    }
    if (smallest != i) {
      swap(i, smallest);
