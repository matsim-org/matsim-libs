...
/**
 * A least cost path calculator supposed to work on an inverted network (created with {@link
 * org.matsim.core.network.algorithms.NetworkInverter}). The returned path of this calculator will
 * be inverted again so it is valid on the actual non-inverted network.
 *
 * <p>The nodes in {@link #calcLeastCostPath(Node, Node, double, Person, Vehicle)} have to be pseudo
 * nodes from the inverted network, i.e. link ids.
 */
class InvertedLeastPathCalculator implements LeastCostPathCalculator {
  ...
  static InvertedLeastPathCalculator create(
      LeastCostPathCalculatorFactory costFactory,
      TravelDisutilityFactory travelTimeFactory,
      Network network,
      Network invertedNetwork,
      LinkToLinkTravelTime l2ltravelTimes) {

    // convert l2ltravelTimes into something that can be used by the inverted network router:
    TravelTimesInvertedNetworkProxy invertedTravelTimes =
        new TravelTimesInvertedNetworkProxy(network, l2ltravelTimes);
    // (method that takes a getLinkTravelTime( link , ...) with a link from the inverted network,
    // converts it into links on the
    ...
    return new InvertedLeastPathCalculator(
        network,
        costFactory.createPathCalculator(invertedNetwork, travelCost, invertedTravelTimes));
  }
  ...
  public Path calcLeastCostPath(
    ...
    Path path =
        leastCostPathCalculator.calcLeastCostPath(fromNode, toNode, starttime, person, vehicle);
