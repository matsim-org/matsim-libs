...
/**
 * A factory for a disutility that leads to randomized Pareto search. Starting point is to have
 * something like disutility(link) = alpha * time + beta * money. Evidently, for beta=0 one obtains
 * the time-minimal path, and for alpha=0 the money-minimal path. The current version will randomize
 * the prefactor of the monetary term. It is implemented in a way that the random number is drawn
 * once per routing call, i.e. the route is computed with a constant tradeoff between money and
 * time, but with the next call it will be a different trade-off. <br>
 * The idea is to come up with different routes, with the hope that one of them ends up being a good
 * one for the scoring function. See, e.g., <a
 * href="https://arxiv.org/abs/1002.4330v1">https://arxiv.org/abs/1002.4330v1</a> for how to
 * generate route alternatives (where Pareto routing is one option), and <a
 * href="https://doi.org/10.1016/j.procs.2014.05.488">https://doi.org/10.1016/j.procs.2014.05.488</a>
...
public class RandomizingTimeDistanceTravelDisutilityFactory implements TravelDisutilityFactory {
  private static final Logger log =
      LogManager.getLogger(RandomizingTimeDistanceTravelDisutilityFactory.class);
  ...
  public RandomizingTimeDistanceTravelDisutilityFactory(final String mode, Config config) {
    // NOTE: It is difficult to get rid of this constructor completely, since "mode" needs to be
    // passed in.  One could still get all other
  ...
  public TravelDisutility createTravelDisutility(final TravelTime travelTime) {
    // yyyy This here should honor subpopulations.  It is really not so difficult; something like
    // cnScoringGroup.getScoringParameters( "subpop"
    // ).getMarginalUtilityOfMoney(); That line, or some variant of it, would need to be in the
    // TravelDisutility directly. And I am quite unsure what is the status of
    // the "default" subpopulation anyways ... I seem to recall that Thibaut wanted to get rid of
    // that.  The following method at least outputs
    ...
    if (params == null) {
      throw new NullPointerException(
          mode + " is not part of the valid mode parameters " + cnScoringGroup.getModes().keySet());
    }
    ...
    final double marginalCostOfTime_s =
        (-params.getMarginalUtilityOfTraveling() / 3600.0)
            + (cnScoringGroup.getPerforming_utils_hr() / 3600.0);
    final double marginalCostOfDistance_m =
        -params.getMonetaryDistanceRate() * cnScoringGroup.getMarginalUtilityOfMoney()
    ...
    return new RandomizingTimeDistanceTravelDisutility(
        travelTime, marginalCostOfTime_s, marginalCostOfDistance_m, normalization, sigma);
  }
  ...
  private void logWarningsIfNecessary(final ScoringConfigGroup cnScoringGroup) {
    if (wrnCnt.getAndIncrement() < 1) {
      ...
      if (cnScoringGroup.getModes().get(mode).getMonetaryDistanceRate() == 0. && this.sigma != 0.) {
        log.warn(
            "There will be no routing randomness. The randomization of the travel disutility requires the monetary distance rate "
                + "to be different than zero. Continuing anyway.");
      }

      if ((cnScoringGroup.getModes().get(mode).getMarginalUtilityOfTraveling()
                  + cnScoringGroup.getPerforming_utils_hr())
              == 0.
          && this.sigma != 0.) {
        log.warn(
            "There will be no routing randomness. The randomization of the travel disutility requires the travel time cost rate "
                + "to be different than zero. Continuing anyway.");
      }
    }
  }
