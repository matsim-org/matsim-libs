...
/**
 * Implements a highly optimized data structure for representing a MATSim network. Optimized to use
 * as little memory as possible, and thus to fit as much memory as possible into CPU caches for high
 * performance.
 *
 * <p>Inspired by GraphHopper's Graph data structure as described in
 * https://github.com/graphhopper/graphhopper/blob/master/docs/core/technical.md. GraphHopper uses
 * bi-directional links, while MATSim uses uni-directional links. Thus, instead of having nodeA and
 * nodeB, we always have from- and to-node. This implies that one `node-row` requires two link-ids:
 * one for the first in-link, and one for the first out-link.
 *
 * <p>We use simple int-arrays (int[]) to store the data. This should provide fast and thread-safe
 * read-only access, but limits the number of nodes and links in the network to (Integer.MAX_VALUE/2
 * = 1.073.741.823) nodes and (Integer.MAX_VALUE/6 = 357.913.941) links. I hope that for the
 * foreseeable future, these limits are high enough.
 *
 * <p>This class is thread-safe, allowing a single graph to be used by multiple threads.
 *
 * @author mrieser
 */
public class SpeedyGraph {
  ...
  private abstract static class AbstractLinkIterator implements LinkIterator {
    ...
    @Override
    public final void reset(int nodeIdx) {
    ...
    @Override
    public final int getFromNodeIndex() {
      return this.graph.linkData[this.linkIdx * LINK_SIZE + 2];
    }

    @Override
    public final int getToNodeIndex() {
    ...
    @Override
    public final double getFreespeedTravelTime() {
