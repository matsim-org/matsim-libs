...
/**
 * Pre-processes a given network, gathering information which can be used by a AStarEuclidean when
 * computing least-cost paths between a start and an end node. Specifically, computes the minimal
 * travel cost per length unit over all links, which is used by AStarEuclidean's heuristic function
 * during routing.
 *
 * @author lnicolas
 */
public class PreProcessEuclidean extends PreProcessDijkstra {
  ...
  /**
   * @param costFunction A cost function that returns the minimal possible cost for each link.
   */
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "link "
                  ...
                  + " has length "
                  + linkLength
                  + " which is smaller than the euclidean distance "
                  + eucDist);
        }
        return false;
      }
  ...
  /**
   * @return the minimal travel cost per length unit over all links in the network.
   */
  ...
  /**
   * @return The cost function that was used to calculate the minimal travel cost per length unit
