...
/**
 * CostCalculator and TravelTimeCalculator for Links based on freespeed on links and distance costs
 * if set. It sets the <em> function </em> that is to be used with calls <tt>getLinkTravelTime(
 * link, time)</tt> and <tt>getLinkTravelCost( link, time )</tt>.
 *
 * <p>The unit of "cost" is defined by the input: if the marginal utilities are given in "utils per
 * second", then cost is in "utils"; if the marginal utilities are given in "euros per second", then
 * cost is in "euros". When the CharyparNagelScoringFunction is used, the values come from the
 * config file, where one is also free to interpret the units.
 *
 * @author mrieser
 * @author dgrether
 */
public class FreespeedTravelTimeAndDisutility
    implements TravelDisutility, TravelTime, LinkToLinkTravelTime {
   ...
   */
  public FreespeedTravelTimeAndDisutility(
      double scaledMarginalUtilityOfTraveling,
      double scaledMarginalUtilityOfPerforming,
