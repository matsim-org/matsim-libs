	...
	public void mergeTurnInfoMaps(Map<Id, List<TurnInfo>> allowedInLinkTurnInfoMap,
		...
		for (Id inLinkId : allowedInLinkIds) {
			if (!restrictingTurnInfoMap.containsKey(inLinkId)) {
				...
				List<TurnInfo> allowedTurnInfos = allowedInLinkTurnInfoMap.get(inLinkId);
				List<TurnInfo> restrictingTurnInfos = restrictingTurnInfoMap.get(inLinkId);
				for (TurnInfo allowedForOutlink : allowedTurnInfos) {
					NetworkTurnInfoBuilder networkTurnInfoBuilder = this;
					TurnInfo restrictionForOutlink = networkTurnInfoBuilder.getTurnInfoForOutlinkId(
							restrictingTurnInfos, inLinkId);
					...
					else { // turns are restricted to some modes
						Set<String> commonModes = networkTurnInfoBuilder.calculateCommonModes(
