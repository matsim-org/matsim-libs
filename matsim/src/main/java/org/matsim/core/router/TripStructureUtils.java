...
/**
 * Helps to work on plans with complex trips. It provides convenience methods to get only the
 * non-dummy activities or the information about trips, in a way hopefully clean and useful. <br>
 * The collections returned by this class are immutable. <br>
 * Two versions of the methods are provided, working on {@link Plan}s or lists of {@link
 * PlanElement}s. <br>
 * The methods require {@link StageActivityHandling} as a parameter, which is used to decide whether
 * dummy activities such as pt interaction should be handled like normal activities or ignored.
 *
 * @author thibautd
 */
public final class TripStructureUtils {
  ...
  public static List<Activity> getActivities(
      final Plan plan, final StageActivityHandling stageActivityHandling) {
  ...
  public static List<Trip> getTrips(
      final List<? extends PlanElement> planElements, final Predicate<String> isStageActivity) {
    ...
    for (PlanElement pe : planElements) {
      ...
      //			if (StageActivityTypeIdentifier.isStageActivity( act.getType() ) || isStageActivity.test(
      // act.getType() )) continue;
      // I I don't like the || (= "or").  If I want to identify subtrips, then I want to put in a
      // reduced number of stage
      ...
      if (currentIndex - originActivityIndex > 1) {
        // which means, if I am understanding this right, that two activities without a leg in
        ...
        trips.add(
            new Trip(
                ...
                Collections.unmodifiableList(
                    new ArrayList<>(planElements.subList(originActivityIndex + 1, currentIndex))),
                act));
      }
  ...
  /**
   * Gives access to a list of the subtours in the plan. A subtour is undestood as the smallest
   * sequence of trips starting and ending at the same location (known as anchor point). <br>
   * The subtour structure is a tree: a subtour may have a father subtour, understood as the
   * smallest subtour containing it, and children subtours, which are the "biggest" subtours it
   * contains (ie the subtours it is the father of). Methods are provided to get or all the trips
   * between the two anchor activities, or just the trips not being part of any child subtour. <br>
   * In case of "open" plans (which do not end by a subtour), one of the {@link Subtour} objects
   * contains a non-circular sequence of Trips (ie it is not a subtour according to the definition
   * above). In this case, the {@link Subtour#isClosed()} method returns false. <br>
   * The order of iteration of the returned collection should not be considered as significant, nor
   * even stable facing refactorings. <br>
   * It is able to handle non-strict act/leg sequence and complex trips; in case of successive
   * activities not being located at the same location (that is, if the origin of a trip is not the
   * destination of the preceding trip), an exception will be thrown. <br>
   * Note: We (VSP) are not sure what this code does exactly. The correct definition of what a
   * subtour is in MATSim needs to be found! Theresa, VSP mode choice seminar in jul'22
   *
   * @param coordDistance if larger 0, also consider coordinates to be at same location if smaller
   *     than distance
   * @throws RuntimeException if the Trip sequence has inconsistent location sequence
   */
  public static Collection<Subtour> getSubtours(
      final List<? extends PlanElement> planElements, double coordDistance) {
    return getSubtours(planElements, TripStructureUtils::isStageActivityType, coordDistance);
  }

  /**
   * Returns the top-level tour as {@link Subtour} object even if it is unclosed. This subtour will
   * always contain all trips of a plan. Child tours will not be set.
   *
   * @see Subtour
   */
  ...
  @Deprecated
  public static Collection<Subtour> getSubtours(
      final Plan plan, final Predicate<String> isStageActivity) {
  ...
  public static Collection<Subtour> getSubtours(
    ...
    for (Trip trip : trips) {
      ...
      if (originId == null) {
        throw new NullPointerException(
            "Facility id, link id and coordinates for origin activity "
      ...
      if (destinationId != null && !originId.equals(destinationId)) {
        throw new RuntimeException(
            "unconsistent trip location sequence: " + destinationId + " != " + originId);
      }
      ...
      if (destinationId == null) {
        throw new NullPointerException(
            "Facility id, and link id and coordinates for destination activity "
      ...
      if (lastIdx > -1) {
        ...
        addSubtourAndUpdateParents(
            subtours, new Subtour(subtourStartIndex, subtourEndIndex, subtour, true));
      }
    }
    ...
    if (nonAllocatedTrips.size() != 0) {
      ...
      addSubtourAndUpdateParents(
          subtours, new Subtour(0, trips.size(), new ArrayList<>(trips), false));
    }
  ...
  private static void addSubtourAndUpdateParents(
      final List<Subtour> subtours, final Subtour newSubtour) {
  ...
  /**
   * Represents a trip, that is, the longest sequence of {@link PlanElement}s consisting only of
   * legs and "dummy" activities. <br>
   * It also provides the references of the {@link Activity}s just before and after the trip (origin
  ...
  public static final class Trip {
    ...
    /**
     * Attributes of preceding activity are passed as trip attributes until more explicit encoding
     * is found.
     */
  ...
  public static final class Subtour {
    ...
    private Subtour(
        final int startIndex, final int endIndex, final List<Trip> trips, final boolean isClosed) {
    ...
    private static boolean areChildrenCompatible(
        final List<Subtour> children2, final List<Subtour> children3) {
      return children2.size()
          == children3.size(); // should check more, but risk of infinite recursion...
    }
  ...
  public static Trip findTripAtPlanElement(
      PlanElement currentPlanElement, Plan plan, Predicate<String> isStageActivity) {
    if (currentPlanElement instanceof Activity) {
      //			Gbl.assertIf( StageActivityTypeIdentifier.isStageActivity(
      // ((Activity)currentPlanElement).getType() ) ) ;
  ...
  // if we make the routing mode identifier replaceable via Guice/Inject, we should return that one
  // here or get rid of the method
  ...
  public static String identifyMainMode(final List<? extends PlanElement> tripElements) {
    ...
    // else see if trip has only one leg, if so, use that mode (situation after initial demand
    // generation)
