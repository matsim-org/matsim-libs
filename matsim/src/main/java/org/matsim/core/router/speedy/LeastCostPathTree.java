...
/**
 * Implements a least-cost-path-tree upon a {@link SpeedyGraph} datastructure. Besides using the
 * more efficient Graph datastructure, it also makes use of a custom priority-queue implementation
 * (NodeMinHeap) which operates directly on the least-cost-path-three data for additional
 * performance gains.
 *
 * <p>In some limited tests, this resulted in a speed-up of at least a factor 2.5 compared to
 * MATSim's default LeastCostPathTree.
 *
 * <p>The implementation does not allocate any memory in the {@link #calculate(int, double, Person,
 * Vehicle)} method. All required memory is pre-allocated in the constructor. This makes the
...
public class LeastCostPathTree {
  ...
  public void calculate(int startNode, double startTime, Person person, Vehicle vehicle) {
    this.calculate(
        startNode, startTime, person, vehicle, (node, arrTime, cost, distance, depTime) -> false);
  }
  ...
  public void calculate(
    ...
    while (!this.pq.isEmpty()) {
      ...
      while (this.outLI.next()) {
        ...
        double newCost =
            currCost + this.td.getLinkTravelDisutility(link, currTime, person, vehicle);
  ...
  public void calculateBackwards(
      int arrivalNode, double arrivalTime, Person person, Vehicle vehicle) {
    this.calculate(
        arrivalNode,
        arrivalTime,
        person,
  ...
  public static final class TravelTimeStopCriterion implements StopCriterion {
    ...
    public boolean stop(
      ...
      return Math.abs(arrivalTime - departureTime)
          >= this.limit; // use Math.abs() so it also works in backwards search
    }
  }
