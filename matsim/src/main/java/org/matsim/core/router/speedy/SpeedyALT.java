...
/**
 * A very fast implementation of the ALT algorithm (A*-search with Landmarks and Triangle
 * Inequality).
 *
 * <p>Based on "Computing the Shortest Path: A* Search Meets Graph Theory" by Andrew V. Goldberg and
 * Chris Harrelson, 2005.
 *
 * <p>This implementation always looks at all landmarks and does not filter them, as performance
 * measurements suggest that selecting and re-selecting landmarks regularly actually results in an
 * overhead compared to just calculate the values for each landmark in each step (using a typical
 * value of 16 landmarks). This might be due to the fact that all values for each landmark are just
 * next to each other in the memory, so when accessing the travelcosts to/from one landmark
 * basically already loads the values of all landmarks in the CPU cache, making the calculation for
 * the remaining landmarks very fast.
 *
 * <p>This implementation is not thread-safe. In the case of multi-threading, every thread should
 * use a separate instance. (But the used {@link SpeedyALTData} is thread-safe and can be shared by
 * multiple instances).
 *
...
public class SpeedyALT implements LeastCostPathCalculator {
  ...
  private static final Logger LOG = LogManager.getLogger(SpeedyALT.class);
  ...
  public Path calcLeastCostPath(
      Node startNode, Node endNode, double startTime, Person person, Vehicle vehicle) {
  ...
  private double estimateMinTravelcostToDestinationForLandmark(
      int nodeIdx, int destinationIdx, int landmarkIdx) {
