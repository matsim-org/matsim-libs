...
class LinkToLinkRoutingModule implements RoutingModule {
  ...
  LinkToLinkRoutingModule(
      final String mode,
      final PopulationFactory populationFactory,
      Network network,
      Network invertedNetwork,
      InvertedLeastPathCalculator leastCostPathCalc) {
  ...
  public List<? extends PlanElement> calcRoute(RoutingRequest request) {
    ...
    if (!toFacility.getLinkId().equals(fromFacility.getLinkId())) {
      // (a "true" route)
      Node fromInvNode =
          this.invertedNetwork.getNodes().get(Id.create(fromFacility.getLinkId(), Node.class));
      Node toInvNode =
          this.invertedNetwork.getNodes().get(Id.create(toFacility.getLinkId(), Node.class));

      Path path =
          leastCostPathCalculator.calcLeastCostPath(
              fromInvNode, toInvNode, departureTime, person, null);
      if (path == null) {
        throw new RuntimeException(
            "No route found on inverted network from link "
      ...
      NetworkRoute route =
          this.populationFactory
              .getRouteFactories()
              .createRoute(NetworkRoute.class, fromFacility.getLinkId(), toFacility.getLinkId());
      route.setLinkIds(
          fromFacility.getLinkId(), NetworkUtils.getLinkIds(path.links), toFacility.getLinkId());
