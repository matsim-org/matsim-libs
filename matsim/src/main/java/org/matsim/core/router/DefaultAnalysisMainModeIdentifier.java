...
/**
 * Attempt to provide a default {@link AnalysisMainModeIdentifier} that is justified from the
 * transport engineering perspective, not from the software perspective. If you use other transport
 * modes not included here, please copy this class, add the mode in your copy and bind that copy in
 * your run class.
 *
 * @author vsp-gleich
 */
public final class DefaultAnalysisMainModeIdentifier implements AnalysisMainModeIdentifier {
  ...
  public DefaultAnalysisMainModeIdentifier() {
    ...
    modeHierarchy.add(
        "freight"); // not clear where this should go since it is not passenger traffic, but it is
    // used in many scenarios.

    // NOTE: This hierarchical stuff is not so great: is park-n-ride a car trip or a pt trip?  Could
    // weigh it by distance, or by time spent
    // in respective mode.  Or have combined modes as separate modes.  In any case, can't do it at
    // the leg level, since it does not
    // make sense to have the system calibrate towards something where we have counted the car and
    // the pt part of a multimodal
  ...
  public String identifyMainMode(List<? extends PlanElement> planElements) {
      ...
      if (pe instanceof Leg leg) {
        ...
        if (index < 0) {
          ...
          if (unknownMode != null && !unknownMode.equals(leg.getMode())) {
            log.error(
                "Multiple unknown modes in one trip: "
                    + leg.getMode()
                    + " and "
                    + unknownMode
            ...
            throw new IllegalStateException(
                "unknown modes in AnalysisMainModeIdentifier: "
                    + leg.getMode()
                    + " and "
                    + unknownMode);
          } else {
    ...
    if (unknownMode != null) {
      if (mainModeIndex > modeHierarchy.indexOf(TransportMode.walk)) {
        // another mode besides walk and our unknown mode was found before, we don't know which of
        // them is the main mode
        log.error(
            "Unknown mode "
                + unknownMode
                + " and "
