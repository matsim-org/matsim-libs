 ...
 * updated, even if the previous trip arrival time is after the activity end time.
 *
 * @author thibautd
 */
public final class PlanRouter implements PlanAlgorithm, PersonAlgorithm {
  ...
  public void run(final Plan plan) {
    ...
    for (Trip oldTrip : trips) {
      ...
      TripRouter.insertTrip(
          plan, oldTrip.getOriginActivity(), newTripElements, oldTrip.getDestinationActivity());
  ...
  /**
   * If the old trip had vehicles set in its network routes, and it used a single vehicle, and if
   * the new trip does not come with vehicles set in its network routes, then put the vehicle of the
   * old trip into the network routes of the new trip.
   *
   * @param oldTrip The old trip
   * @param newTrip The new trip
   */
  public static void putVehicleFromOldTripIntoNewTripIfMeaningful(
      Trip oldTrip, List<? extends PlanElement> newTrip) {
  ...
  private static Id<Vehicle> getUniqueVehicleId(Trip trip) {
    ...
    for (Leg leg : trip.getLegsOnly()) {
      if (leg.getRoute() instanceof NetworkRoute) {
        if (vehicleId != null
            && (!vehicleId.equals(((NetworkRoute) leg.getRoute()).getVehicleId()))) {
