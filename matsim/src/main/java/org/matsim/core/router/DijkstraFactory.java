...
public class DijkstraFactory implements LeastCostPathCalculatorFactory {
  ...
  // yy there is no guarantee that "createPathCalculator" is called with the same network as the one
  // that was used for "preProcessData".
  // This can happen for example when LinkToLink routing is switched on.  kai & theresa, feb'15
  // To fix this, we create the PreProcessData when the first LeastCostPathCalculator object is
  // created and store it in a map using
  // the network as key. For the PreProcessDijkstra data this is fine, since it does not take travel
  // times and disutilities into account.
  // For the AStarLandmarks data, we would have to include the other two arguments into the lookup
  // value as well... cdobler, sep'17
  @Override
