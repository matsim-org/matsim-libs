...
/**
 * Pre-processes a given network, gathering information which can be used by a Dijkstra when
 * computing least-cost paths between a start and an end node. Specifically, marks the nodes that
 * are in dead-end, i.e. that are connected to the rest of the network by a single 'entry' node
 * only.
 *
 * @author lnicolas
 */
public class PreProcessDijkstra {
  ...
  /**
   * Marks nodes that are in dead ends (i.e. nodes that are connected through a single node to the
   * rest of the network), starting at all nodes with degree 2.
   *
  ...
  public static class DeadEndData {
    ...
    /**
     * @param inDeadEndCount the inDeadEndCount to set
     */
    ...
    /**
     * @return The node that connects the associated node to the rest of the network (i.e. without
     *     the deadEndEntryNode, the associated node would not be connected to the network) and null
     *     otherwise (i.e. if the associated node is not in a dead end).
     */
