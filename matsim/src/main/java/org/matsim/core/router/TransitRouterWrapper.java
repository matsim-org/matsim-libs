...
public class TransitRouterWrapper implements RoutingModule {
  ...
  public List<? extends PlanElement> calcRoute(RoutingRequest request) {
    ...
    return baseTrip != null ? fillWithActivities(baseTrip, request) : null;
  }

  /**
   * This treats the TransitRouter as a third-party interface, where missing fields must be filled
   * in (distance, travel-time in routes).
   */
    ...
    for (PlanElement pe : baseTrip) {
      ...
      if (i == 0) {
        ...
        if (leg.getRoute() instanceof TransitPassengerRoute) {
          ...
          // it is not an instance of an ExperimentalTransitRoute so it must be a (transit) walk
          // leg.

          // walk legs don't have a coord, use the coord from the last egress point.  yyyy But I
          // don't understand why in one case we take "nextCoord", while in the
          ...
          if (i == baseTrip.size() - 1) {
            ...
            TransitPassengerRoute tRoute =
                (TransitPassengerRoute) ((Leg) baseTrip.get(baseTrip.size() - 2)).getRoute();
            Facility lastFromFacility =
                this.transitSchedule.getFacilities().get(tRoute.getEgressStopId());
          ...
          Activity act =
              PopulationUtils.createStageActivityFromCoordLinkIdAndModePrefix(
                  nextCoord, leg.getRoute().getStartLinkId(), TransportMode.pt);
          trip.add(act);
        }
      }
      trip.add(leg);
  ...
  private Route createWalkRoute(
    ...
    // yyyy I extracted this method to make a bit more transparent that it is used twice.  But I
    // don't know why it is done in this way
    // (take distance from newly computed walk leg, but take travelTime from elsewhere).  Possibly,
    // the problem is that the TransitRouter
    // historically just does not compute the distances.  kai, may'17

    Route route =
        RouteUtils.createGenericRouteImpl(fromFacility.getLinkId(), firstToFacility.getLinkId());
    final List<? extends PlanElement> walkRoute =
        walkRouter.calcRoute(
            DefaultRoutingRequest.of(
                fromFacility, firstToFacility, departureTime, person, routingAttributes));
