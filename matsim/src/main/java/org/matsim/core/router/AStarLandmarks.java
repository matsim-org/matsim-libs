...
/**
 * Implements the A* router algorithm for a given NetworkLayer using some so called landmark nodes
 * that are used to get a better estimation of the remaining travel cost. This way we can accelerate
 * the routing speed, and the routes we find are still guaranteed to be the best ones with respect
 * to the travel cost.
 *
 ...
 * AStarLandmarks is org.matsim.demandmodeling.router.util.PreProcessLandmarks.<br>
 * AStarLandmarks is about double as fast as AStarEuclidean. PreProcessLandmarks.run() takes about 1
 * minute on 400'000 nodes on a AMD Opteron processor 275 with 2.2GHz and requires 2*X double values
 * per node, where X is the number of landmarks. Currently, it is set to 16 (but can be set to
 * another value, as 12 or 8 for example), so with 400'000 nodes we would need 2*8*16*400'000 bytes
 * = about 100MB of additional memory.<br>
...
public class AStarLandmarks extends AStarEuclidean {
   ...
   * @param costFunction
   * @param timeFunction Calculates the travel time on links.
   * @param overdoFactor The factor which is multiplied with the output of the A* heuristic
   *     function. The higher the overdo factor the greedier the router, i.e. it visits less nodes
   *     during routing and is thus faster, but for an overdo factor > 1, it is not guaranteed that
   *     the router returns the least-cost paths. Rather it tends to return distance-minimal paths.
  ...
  /**
   * Inspect all landmarks and determines the actLandmarkCount best ones that will be used for
   * routing (so-called active landmarks).
   *
   * @param fromNode The node for which we estimate the travel time to the toNode in order to rank
   *     the landmarks.
   * @param toNode The node to which we estimate the travel time from the fromNode in order to rank
   *     the landmarks.
   * @param actLandmarkCount The number of active landmarks landmarks to set.
   */
  /*package*/ void initializeActiveLandmarks(
      final Node fromNode, final Node toNode, final int actLandmarkCount) {
  ...
  /**
   * If a landmark has been added to the set of the active landmarks, this function re-evaluates the
   * estimated remaining travel time based on the new set of active landmarks of the nodes contained
   * in pendingNodes. If this estimation improved, the node's position in the pendingNodes queue is
   * updated.
   *
   ...
   */
  /*package*/ void updatePendingNodes(
      final int newLandmarkIndex, final Node toNode, final RouterPriorityQueue<Node> pendingNodes) {
  ...
  /**
   * Checks whether there is a landmark in the set of the non-active landmarks that yields a better
   * estimation than the best active landmark. If there is, this landmark is added to the set of
   * active landmark and its index is returned.
   *
   * @param fromNode The node for which we estimate the remaining travel time to the toNode.
   * @param toNode The target node.
   * @return The index of the landmark that has been added to the set of active landmarks, or -1 if
   *     no landmark was added.
   */
  /*package*/ int checkToAddLandmark(final Node fromNode, final Node toNode) {
    ...
    if (bestIndex != -1) {
      ...
      System.arraycopy(
          this.activeLandmarkIndexes, 0, newActiveLandmarks, 0, this.activeLandmarkIndexes.length);
   ...
   * @param index The index of the landmarks that should be used for the estimation of the travel
   *     cost.
   * @return The travel cost when traveling between the two given nodes.
   */
  protected double estimateRemainingTravelCost(
    ...
    if (tmpTravTime < 0) {
      tmpTravTime =
          toRole.getMinLandmarkTravelTime(index) - fromRole.getMaxLandmarkTravelTime(index);
      if (tmpTravTime <= 0) {
        return 0;
      }
    }
