 ...
 *
 * <p>If the Wait2LinkEvents contain vehicle ids different from null, this ids are added to the
 * LinkEnter- and LinkLeaveEvents. If not, the person id is used as vehicle id in Wait2Link-,
 * LinkLeave- and LinkEnterEvents.
 *
 * <p>Additionally, this class adds VehicleLeavesTrafficEvents if they are not existing.
 *
 * @author tthunig
 */
    ...
    if (EVENT.equals(name)) {
      ...
      switch (eventType) {
        case PersonDepartureEvent.EVENT_TYPE:
          ...
          personToLegMode.put(
              Id.createPersonId(atts.getValue(PersonDepartureEvent.ATTRIBUTE_PERSON)),
              atts.getValue(PersonDepartureEvent.ATTRIBUTE_LEGMODE));
        ...
        case "wait2link":
          ...
          if (atts.getValue(VehicleEntersTrafficEvent.ATTRIBUTE_VEHICLE) == null
              || atts.getValue(VehicleEntersTrafficEvent.ATTRIBUTE_VEHICLE).equals("null")) {
            // use the person id as vehicle id
            vehicleId = Id.create(driverId, Vehicle.class);
          } else {
            vehicleId =
                Id.create(
                    atts.getValue(VehicleEntersTrafficEvent.ATTRIBUTE_VEHICLE), Vehicle.class);
          }
          assert vehicleId != null;

          String networkMode = atts.getValue(VehicleEntersTrafficEvent.ATTRIBUTE_NETWORKMODE);
          // use the (leg) mode from the departure event if this event does not contain a (network)
          // mode
          if (networkMode == null) {
          ...
          this.events.processEvent(
              new VehicleEntersTrafficEvent(
                  time,
                  driverId,
                  Id.createLinkId(atts.getValue(VehicleEntersTrafficEvent.ATTRIBUTE_LINK)),
                  vehicleId,
                  networkMode,
                  1.0));
          break;
        case LinkEnterEvent.EVENT_TYPE:
          if (atts.getValue(LinkEnterEvent.ATTRIBUTE_VEHICLE) == null
              || atts.getValue(LinkEnterEvent.ATTRIBUTE_VEHICLE).equals("null")) {
            // create an link enter event with the correct vehicle id
            this.events.processEvent(
                new LinkEnterEvent(
                    time,
                    driverToVeh.get(Id.createPersonId(atts.getValue(ATTRIBUTE_PERSON))),
        ...
        case LinkLeaveEvent.EVENT_TYPE:
          if (atts.getValue(LinkLeaveEvent.ATTRIBUTE_VEHICLE) == null
              || atts.getValue(LinkLeaveEvent.ATTRIBUTE_VEHICLE).equals("null")) {
        ...
        case PersonLeavesVehicleEvent.EVENT_TYPE:
          if (driverToVeh.containsKey(
                  Id.createPersonId(atts.getValue(PersonLeavesVehicleEvent.ATTRIBUTE_PERSON)))
              && !this.containsVehicleLeavesTrafficEvents) {
        ...
        case PersonArrivalEvent.EVENT_TYPE:
          Id<Person> personId =
              Id.createPersonId(atts.getValue(PersonArrivalEvent.ATTRIBUTE_PERSON));
          ...
          if (driverToVeh.containsKey(personId) && !this.containsVehicleLeavesTrafficEvents) {
            ...
            // remove the person-car-mapping, otherwise every following (even non-vehicle) leg
            // produces leave traffic/vehicle events
            Id<Vehicle> vehicleIdOfDriver = driverToVeh.remove(personId);

            this.events.processEvent(
                new VehicleLeavesTrafficEvent(
                    time, personId, linkId, vehicleIdOfDriver, mode, 1.0));
            this.events.processEvent(
                new PersonLeavesVehicleEvent(time, personId, vehicleIdOfDriver));
          }
          // process the PersonArrivalEvent normally, independently of missing
          // VehicleLeavesTrafficEvents
          this.basicEventsReader.startTag(name, atts, context);
          break;
        default:
          this.basicEventsReader.startTag(name, atts, context);
