...
public final class ParallelEventsManager implements EventsManager {
  ...
  private static final Logger log = LogManager.getLogger(ParallelEventsManager.class);
  ...
  ParallelEventsManager(Config config) {
    this(
        config.eventsManager().getSynchronizeOnSimSteps() != null
            ? config.eventsManager().getSynchronizeOnSimSteps()
            : true,
        config.eventsManager().getEventsQueueSize());
  }
  ...
  public ParallelEventsManager(
      final boolean syncOnTimeSteps, final int numOfThreads, int eventsQueueSize) {
    this(syncOnTimeSteps, false, numOfThreads, eventsQueueSize);
  }

  /*package*/ ParallelEventsManager(
  ...
  private void initialize() {
    int numHandlers =
        oneThreadPerHandler
            ? this.eventsHandlers.size()
            : Math.min(this.numOfThreads, this.eventsHandlers.size());
    ...
    for (int i = 0; i < this.eventsManagers.size(); i++) {
      ...
      ProcessEventsRunnable processEventsRunnable =
          new ProcessEventsRunnable(eventsManager, distributor);
  ...
  public void flush() {
    try {
      ...
      throw new RuntimeException(
          "Exception while waiting on flush... " + e.getMessage(),
          this.uncaughtExceptionHandler.exception);
    }
  }
  ...
  private class Distributor extends Thread {
    ...
    // When set to true, the distributor will process all events until all events in the event
    // manager are processed.
    // This is used when the simulation needs to sync with event processing and make sure there are
    // no unprocessed
    // events in the system.
    private volatile boolean shouldFlush = false;

    public Distributor(
        ArrayList<ProcessEventsRunnable> runnables, BlockingQueue<EventArray> eventQueue) {
    ...
    public final void run() {
      try {
        ...
        while (true) {
          ...
          if (earray == null) {
            synchronized (this) {
              ...
              if (shouldFlush) {
                ...
                for (ProcessEventsRunnable runnable : this.runnables) runnable.flush();
          ...
          // this is an optimization, if we receive a large buffer, avoid copying it and send it
          // directly.
