...
/**
 * An EventsHandler that handles all occurring Events in separate Threads. When a Time Step of the
 * QSim ends, all Events that have been created in that Time Step are processed before the
 * simulation can go on. This is necessary e.g. when using Within-day Replanning.
 *
 * @author cdobler
 */
class SimStepParallelEventsManagerImpl implements EventsManager {

  private static final Logger log = LogManager.getLogger(SimStepParallelEventsManagerImpl.class);
  ...
  public void initProcessing() {
    ...
    ExceptionHandler uncaughtExceptionHandler =
        new ExceptionHandler(
            hadException, waitForEmptyQueuesBarrier, simStepEndBarrier, iterationEndBarrier);

    runnables = new ProcessEventsRunnable[numOfThreads];
    for (int i = 0; i < numOfThreads; i++) {
      ProcessEventsRunnable processEventsRunnable =
          new ProcessEventsRunnable(
              eventsManagers[i],
              processedEventsChecker,
              waitForEmptyQueuesBarrier,
              simStepEndBarrier,
              iterationEndBarrier,
              eventsQueues.get(i),
              eventsQueues.get(i + 1));
  ...
  public synchronized void finishProcessing() {
    ...
    if (hadException.get() == null) {
      try {
        ...
        // prefer storing the original exception and not the follow-up BrokenCyclicBarrier
        // exceptions
        if (hadException.get() == null) {
  ...
  private static class ProcessEventsRunnable implements Runnable {
    ...
    public ProcessEventsRunnable(
        EventsManager eventsManager,
        ProcessedEventsChecker processedEventsChecker,
        CyclicBarrier waitForEmptyQueuesBarrier,
        CyclicBarrier simStepEndBarrier,
        CyclicBarrier iterationEndBarrier,
        Queue<Event> eventsQueue,
        Queue<Event> nextEventsQueue) {
