 ...
 *
 * <ul>
 *   <li>This is deliberately without a handleEvent( Event ev ) so that derived interfaces and
 *       ultimately classes are not forced to implement a handler that deals with <i>all</i> events.
 *       kai, with input from dominik, nov'11
 * </ul>
 */
public interface EventHandler extends MatsimExtensionPoint {
  /**
   * Gives the event handler the possibility to clean up its internal state. Within a
   * Controler-Simulation, this is called before the mobsim starts.
   *
