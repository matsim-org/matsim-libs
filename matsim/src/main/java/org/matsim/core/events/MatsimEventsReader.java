...
/**
 * A reader for events-files of MATSim. This reader recognizes the format of the events-file and
 * uses the correct reader for the specific events-version, without manual setting.
 *
 * @author mrieser
 */
public final class MatsimEventsReader implements MatsimReader {

  private static final Logger log = LogManager.getLogger(MatsimEventsReader.class);
  ...
  public void readFile(final String filename) {
    ...
    if (lcFilename.endsWith(".xml")
        || lcFilename.endsWith(".xml.gz")
        || lcFilename.endsWith(".xml.zst")
        || lcFilename.endsWith(".xml.lz4")) {
      new XmlEventsReader(this.events, this.customEventMappers).readFile(filename);
    } else if (lcFilename.endsWith(".ndjson")
        || lcFilename.endsWith(".ndjson.gz")
        || lcFilename.endsWith(".ndjson.zst")) {
  ...
  public void readStream(
      final InputStream stream, final ControllerConfigGroup.EventsFileFormat format) {
    switch (format) {
      case xml:
        new XmlEventsReader(this.events, this.customEventMappers).parse(stream);
        break;
  ...
  private static class XmlEventsReader extends MatsimXmlParser {
    ...
    private XmlEventsReader(final EventsManager events, Map<String, CustomEventMapper> map) {
      ...
      setDoctype(
          "events_v1.dtd"); // manually set a doctype, otherwise delegate would not be initialized
    }
