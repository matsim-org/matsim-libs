...
public class SnapshotGenerator
  ...
  private static final Logger log = LogManager.getLogger(SnapshotGenerator.class);
  ...
  public SnapshotGenerator(
      final Network network, final double snapshotPeriod, final QSimConfigGroup config) {
  ...
  public void handleEvent(final PersonDepartureEvent event) {
    ...
    this.eventLinks
        .get(event.getLinkId())
        .departure(getEventAgent(event.getPersonId(), event.getTime()));
  }
  ...
  public void handleEvent(final PersonArrivalEvent event) {
    ...
    this.eventLinks
        .get(event.getLinkId())
        .arrival(getEventAgent(event.getPersonId(), event.getTime()));
  }
  ...
  public void handleEvent(final LinkEnterEvent event) {
    ...
    this.eventLinks
        .get(event.getLinkId())
        .enter(getEventAgent(delegate.getDriverOfVehicle(event.getVehicleId()), event.getTime()));
  }
  ...
  public void handleEvent(final LinkLeaveEvent event) {
    ...
    this.eventLinks
        .get(event.getLinkId())
        .leave(getEventAgent(delegate.getDriverOfVehicle(event.getVehicleId()), event.getTime()));
  }
  ...
  public void handleEvent(final VehicleEntersTrafficEvent event) {
    ...
    this.eventLinks
        .get(event.getLinkId())
        .wait2link(getEventAgent(event.getPersonId(), event.getTime()));
  ...
  public void handleEvent(final PersonStuckEvent event) {
    ...
    if (event.getLinkId() != null) { // link id is optional - agent can be teleporting or whatever.
      this.eventLinks
          .get(event.getLinkId())
          .stuck(getEventAgent(event.getPersonId(), event.getTime()));
    }
  }
  ...
  private Collection<AgentSnapshotInfo> getVehiclePositions(final double time) {
    ...
    if (this.snapshotStyle == SnapshotStyle.queue) {
      ...
      log.warn(
          "Cannot generate snapshots offline (e.g., from events) for "
              + this.snapshotStyle
              + ". This snapshot style is supported during simulation only.");
      throw new RuntimeException(
          "The snapshotStyle \"" + this.snapshotStyle + "\" is not supported.");
    }
    return positions;
  }
  ...
  private static class EventLink {
    ...
    private EventLink(
      ...
      this.euklideanDist =
          CoordUtils.calcEuclideanDistance(
              link2.getFromNode().getCoord(), link2.getToNode().getCoord());
      ...
      this.spaceCap =
          (this.link.getLength() * this.link.getNumberOfLanes())
    ...
    private static AgentSnapshotInfo createAgentSnapshotInfo(
        PositionInfo.LinkBasedBuilder builder,
        EventAgent agent,
        Link link,
        double distanceFromNode,
        AgentSnapshotInfo.AgentState agentState) {
      return builder
    ...
    /**
     * Calculates the positions of all vehicles on this link according to the queue-logic: Vehicles
     * are placed on the link according to the ratio between the free-travel time and the time the
     * vehicles are already on the link. If they could have left the link already (based on the
     * time), the vehicles start to build a traffic-jam (queue) at the end of the link.
     *
    ...
    private void getVehiclePositionsQueue(
      ...
      double queueEnd =
          this.link
              .getLength(); // the length of the queue jammed vehicles build at the end of the link
      double vehLen =
          Math.min( // the length of a vehicle in visualization
              this.euklideanDist / this.spaceCap, // all vehicles must have place on the link
              this.effectiveCellSize
                  / this.storageCapFactor); // a vehicle should not be larger than it's actual size
      ...
      for (EventAgent agent : this.buffer) {
        ...
        var position =
            createAgentSnapshotInfo(
                builder, agent, link, queueEnd, AgentSnapshotInfo.AgentState.PERSON_DRIVING_CAR);
      ...
      for (EventAgent agent : this.drivingQueue) {
        ...
        double distanceOnLink =
            (this.freespeedTravelTime == 0.0
                ? 0.0
                : ((travelTime / this.freespeedTravelTime) * this.euklideanDist));
      ...
      for (EventAgent agent : this.waitingQueue) {
        ...
        var position =
            createAgentSnapshotInfo(
                builder, agent, link, queueEnd, AgentSnapshotInfo.AgentState.PERSON_AT_ACTIVITY);
        positions.add(position);
      }
    ...
    /**
     * Calculates the positions of all vehicles on this link so that there is always the same
     * distance between following cars. A single vehicle will be placed at the middle (0.5) of the
     * link, two cars will be placed at positions 0.25 and 0.75, three cars at positions 0.16, 0.50,
     * 0.83, and so on.
     *
     * @param positions A collection where the calculated positions can be stored.
     * @param time The current timestep
