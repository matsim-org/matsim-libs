 ...
 *
 * <ol>
 *   <li>Create a new class MyEventClass extends Event
 *   <li>Create a new interface MyEventHandlerI extends EventHandler
 *   <li>add method public void handleEvent(MyEvent event) to it
 *   <li>ready to go, just implement the interface somewhere and add a HandlerObject with a call to
 *       <code>Events.addHandler(HandlerObject)</code>
 *   <li>(optional) add an appropriate line in callHandlerFast() for speeding up execution!
 * </ol>
 *
 * @author dstrippgen
 * @author mrieser
...
public final class EventsManagerImpl implements EventsManager {
  ...
  private final Map<Class<? extends Event>, HandlerInfo[]> cacheHandlers =
        ...
        try {
          ...
          throw new RuntimeException(
              "problem invoking EventHandler "
                  + info.eventHandler.getClass().getCanonicalName()
                  + " for event-class "
                  + info.eventClass.getCanonicalName(),
              e);
    ...
    do {
      for (Class<?> theInterface : test.getInterfaces()) {
        if (EventHandler.class.isAssignableFrom(theInterface)) {
          Class<? extends EventHandler> eventHandlerInterface =
              (Class<? extends EventHandler>) theInterface;
  ...
  private void addHandlerInterfaces(
      final EventHandler handler, final Class<? extends EventHandler> handlerClass) {
  ...
  private static boolean callHandlerFast(
      final Class<? extends Event> klass, final Event ev, final EventHandler handler) {
