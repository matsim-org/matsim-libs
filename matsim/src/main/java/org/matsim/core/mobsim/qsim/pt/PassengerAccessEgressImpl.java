...
class PassengerAccessEgressImpl implements PassengerAccessEgress {
  ...
  /**
   * @return should be 0.0 or 1.0, values greater than 1.0 may lead to buggy behavior, dependent on
   *     TransitStopHandler used
   */
  /*package*/ double calculateStopTimeAndTriggerBoarding(
    ...
    List<PTPassengerAgent> passengersEntering =
        findPassengersEntering(
            transitRoute, transitLine, vehicle, stop, stopsToCome, freeCapacity, now);
  ...
  public boolean handlePassengerLeaving(
    ...
    if (handled) {
      ...
      // from here on works only if PassengerAgent can be cast into MobsimAgent ... but this is how
      // it was before.
      // kai, sep'12
      ...
      // (cannot set trEngine to TransitQSimEngine because there are tests where this will not work.
      // kai, dec'11)
    }
    return handled;
  }
