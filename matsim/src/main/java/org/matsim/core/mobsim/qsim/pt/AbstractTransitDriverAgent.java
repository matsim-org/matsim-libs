...
public abstract class AbstractTransitDriverAgent implements TransitDriverAgent, PlanAgent {
  ...
  AbstractTransitDriverAgent(
      InternalInterface internalInterface, TransitStopAgentTracker agentTracker2) {
    super();
    ...
    accessEgress =
        new PassengerAccessEgressImpl(
            this.internalInterface, agentTracker2, scenario, eventsManager);
  }
  ...
  public final Id<Link> chooseNextLinkId() {
    ...
    if (this.nextLinkIndex == linkIds.size()) {
      if (linkIds.size() == 0 && netR.getStartLinkId().equals(netR.getEndLinkId())) {
        // unfortunate exception for the unlikely but legal and test-covered case where a whole
        // transit line
        // takes place on a single link.
        // would not need to be here if a route were simply a list of linkIds to traverse, with 1
        // being an allowed length.
        // this used to return endLinkId, which would then require a transit-specific extra case for
        // arriving vehicles
  ...
  /**
   * Overwrite the current link index. May be used by implementing classes, but should be handled
   * with care.
   */
  ...
  public double handleTransitStop(final TransitStopFacility stop, final double now) {
    ...
    List<TransitRouteStop> stopsToCome =
        route.getStops().subList(stopIterator.nextIndex(), route.getStops().size());
    /*
     ...
     */
    double stopTime =
        this.accessEgress.calculateStopTimeAndTriggerBoarding(
            getTransitRoute(), getTransitLine(), this.vehicle, stop, stopsToCome, now);
      ...
      if (getTransitLine() == null) {
        eventsManager.processEvent(
            new TransitDriverStartsEvent(
                now,
                this.dummyPerson.getId(),
                this.vehicle.getId(),
                Id.create("Wenden", TransitLine.class),
                Id.create("Wenden", TransitRoute.class),
   ...
   *
   * <ul>
   *   <li>Keeping this for the time being, since the derived methods somehow need to get the
   *       selected plan. Might keep track of the selected plan directly, but someone would need to
   *       look more into the design. kai, jun'11
   *   <li>For that reason, I made the method package-private. There is, however, probably not much
   *       harm to make it public again as long as it is not part of the PlanDriverAgent interface.
   *       kai, jun'11
   * </ul>
   */
    ...
    if (this.currentStop == null) {
      ...
      eventsManager.processEvent(
          new VehicleArrivesAtFacilityEvent(
              now, this.vehicle.getVehicle().getId(), stop.getId(), delay));
    }
  }
  ...
  protected double longerStopTimeIfWeAreAheadOfSchedule(final double now, final double stopTime) {
    if ((this.nextStop.isAwaitDepartureTime())
        && (this.nextStop.getDepartureOffset().isDefined())) {
    ...
    if (this.nextStop != null) {
      RuntimeException e =
          new RuntimeException(
              "Transit vehicle is not yet at last stop! vehicle-id = "
                  + this.vehicle.getVehicle().getId()
                  + "; next-stop = "
                  + this.nextStop.getStopFacility().getId());
      log.error(e);
      throw e;
    }
  }
  ...
  private void assertVehicleIsEmpty() {
    if (this.vehicle.getPassengers().size() > 0) {
      ...
      log.error(
          "Transit vehicle must be empty after last stop! vehicle-id = "
              + this.vehicle.getVehicle().getId(),
          e);
      for (PassengerAgent agent : this.vehicle.getPassengers()) {
        if (agent instanceof PersonDriverAgentImpl) {
          log.error(
              "Agent is still in transit vehicle: agent-id = "
                  + ((PersonDriverAgentImpl) agent).getPerson().getId());
        }
      }
      throw e;
    }
  ...
  /**
   * A simple wrapper that delegates all get-Methods to another instance, blocks set-methods so this
   * will be read-only, and returns in getVehicleId() a vehicle-Id specific to this driver, and not
   * to the NetworkRoute. This allows to share one NetworkRoute from a TransitRoute with multiple
   * transit drivers, thus saving memory.
   *
   * @author mrieser
   */
  private final class NetworkRouteWrapper implements NetworkRoute, Cloneable {
    ...
    public void setLinkIds(
        final Id<Link> startLinkId, final List<Id<Link>> srcRoute, final Id<Link> endLinkId) {
