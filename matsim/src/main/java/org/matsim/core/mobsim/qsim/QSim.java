 ...
 * The current (dec'2011) approach consists of the following elements (and presumably more,
 * developed by mzilske):
 *
 * <ul>
 *   <li>QSim itself should have all basic functionality to execute a typical agent plan, i.e.
 *       activities and legs. In this basic version, all legs are teleported.
 *   <li>In addition, there are "engines" that plug into QSim. Those are time-step driven, as is
 *       QSim. Many engines move particles around, i.e. they execute the different modes. Others are
 *       responsible for, e.g., time-variant networks or signals.
 *   <li>A special engine is the netsim engine, which is the original "queue" engine. It is invoked
 *       by default, and it carries the "NetsimNetwork" for which there is a getter.
 *   <li>Engines that move particles around need to be able to "end legs". This used to be such that
 *       control went to the agents, which reinserted themselves into QSim. This has now been
 *       changed: The agents compute their next state, but the engines are responsible for
 *       reinsertion into QSim. For this, they obtain an "internal interface" during engine
 *       addition. Naming conventions will be adapted to this in the future.
 *   <li><i>A caveat is that drivers that move around other agents (such as TransitDriver,
 *       TaxicabDriver) need to become "engines".</i> Possibly, something that executes a leg is not
 *       really the same as an "engine", but this is what we have for the time being.
 *   <li>Engines that offer new modes also need to be registered as "DepartureHandler"s. *
 * </ul>
 *
 * Future plans include: pull the agent counter write methods back into QSim (no big deal, I hope);
 * pull the actstart/end, agent departure/arrival back into QSim+engines; somewhat separate the
 * teleportation engine and the activities engine from the framework part of QSim.
 *
 * <p>
 *
 * @author dstrippgen
...
public final class QSim implements VisMobsim, Netsim, ActivityEndRescheduler {
  ...
  private static final Logger log = LogManager.getLogger(QSim.class);
  ...
  /*package (for tests)*/ final InternalInterface internalInterface =
      new InternalInterface() {
        ...
        @Override
        public synchronized MobsimAgent unregisterAdditionalAgentOnLink(
        ...
        //		@Override
        //		@Deprecated // use same method from QSim directly and try to get rid of the handle to
        // internal interface. kai, mar'15
        //		public void rescheduleActivityEnd(MobsimAgent agent) {
        //			// yy my current intuition would be that this could become a public QSim method.  The
        // original idea was that I wanted external
        //			// code only to insert agents into the QSim, and from then on the QSim handles it
        // internally.  However, the main thing that truly seems to be
        //			// done internally is to move the agents between the engines, e.g. around endActivity
        // and endLeg.  In consequence,
        //			// "arrangeNextAgentState" and "(un)registerAgentOnLink" need to be protected.  But not
        // this one.  kai, mar'15
  ...
  /**
   * Constructs an instance of this simulation which does not do anything by itself, but accepts
   * handlers for Activities and Legs. Use this constructor if you want to plug together your very
   * own simulation, i.e. you are writing some of the simulation logic yourself.
   *
   * <p>If you wish to use QSim as a product and run a simulation based on a Config file, rather use
   * QSimFactory as your entry point.
   */
  @Inject
  ...
  public void run() {
    ...
    try {
      ...
      // We really want to perform that. For instance, with QNetsimEngine, threads are cleaned up in
      // this method.
      // Without this finally, in case of a crash, threads are not closed, which lead to process
      // hanging forever
      ...
      try {
        ...
        if (tryBlockWithException) {
          ...
          // no exception thrown in the try block, let's re-throw the exception from the cleanup
          // step
  ...
  /*package*/ void prepareSim() {
    ...
    this.initSimTimer();
    this.infoTime =
        Math.floor(this.simTimer.getSimStartTime() / INFO_PERIOD)
  ...
  public void addParkedVehicle(MobsimVehicle veh, Id<Link> startLinkId) {
    if (this.netEngine != null) {
      ...
      if (wrnCnt2 < 1) {
        log.warn(
            "not able to add parked vehicle since there is no netsim engine.  continuing anyway, but it may "
    ...
    // yy one might want to check if the types/vehicles here are the same as in previous iterations.
    // kai/kai, jan'20
  }
  ...
  private void cleanupSim() {
    ...
    if (analyzeRunTimes) {
      ...
      for (Entry<MobsimEngine, AtomicLong> entry : this.mobsimEngineRunTimes.entrySet()) {
        log.info(
            entry.getKey().getClass().toString() + " cpu time (nanos): " + entry.getValue().get());
      }
      log.info("");
  ...
  /*package*/ boolean doSimStep() {
    ...
    if (this.withindayEngine != null) {
      ...
      if (analyzeRunTimes)
        this.mobsimEngineRunTimes
            .get(this.withindayEngine)
            .addAndGet(System.nanoTime() - this.startClockTime);
    }
    ...
    // this gives after sim step listeners like snapshot generator the opportunity to generate
    // events
  ...
  public void insertAgentIntoMobsim(final MobsimAgent agent) {
    if (this.agents.containsKey(agent.getId())) {
      throw new RuntimeException(
          "Agent with same Id (" + agent.getId().toString() + ") already in mobsim; aborting ... ");
    }
      ...
      case ABORT:
        this.events.processEvent(
            new PersonStuckEvent(
                this.simTimer.getTimeOfDay(),
                agent.getId(),
                agent.getCurrentLinkId(),
                agent.getMode()));

        // NOTE: in the same way as one can register departure handler or activity handler, we could
        // allow to
        // register abort handlers.  If someone ever comes to this place here and needs this.  kai,
        // nov'17
  ...
  /**
   * Informs the simulation that the specified agent wants to depart from its current activity. The
   * simulation can then put the agent onto its vehicle on a link or teleport it to its destination.
   */
  private void arrangeAgentDeparture(final MobsimAgent agent) {
    ...
    events.processEvent(
        new PersonDepartureEvent(now, agent.getId(), linkId, agent.getMode(), routingMode));
    ...
    // yy my intuition is that this should be followed by setting the agent state to abort. kai,
    // nov'14
  ...
  private void initSimTimer() {
    ...
    double simStartTime;
    if (QSimConfigGroup.StarttimeInterpretation.maxOfStarttimeAndEarliestActivityEnd.equals(
    ...
    } else if (QSimConfigGroup.StarttimeInterpretation.onlyUseStarttime.equals(
        qSimConfigGroup.getSimStarttimeInterpretation())) {
  ...
  private void printSimLog(final double time) {
    if (time >= this.infoTime) {
      ...
      log.info(
          "SIMULATION (NEW QSim) AT "
              + Time.writeTime(time)
              + " : #Veh="
              + this.agentCounter.getLiving()
              + " lost="
              + this.agentCounter.getLost()
              + " simT="
              + diffsim
              + "s realT="
              + (diffreal)
  ...
  public void addMobsimEngine(MobsimEngine mobsimEngine) {
    ...
    //				log.warn("pre-existing transitEngine != null; will be overwritten; with the current
    // design, " +
    ...
    // yy note that what follows here somewhat interacts with the QSimProvider, which is doing
    // similar things.  I just fixed a resulting misunderstanding re
  ...
  /**
   * Adds the QueueSimulationListener instance given as parameters as listener to this
   * QueueSimulation instance.
   */
  @Override
  ...
  void addQueueSimulationListeners(Set<MobsimListener> listeners) {
    // I think that "injecting a method" means that the method is called at some point, pulling the
    // method arguments out of injection.  In
    // consequence, it is assumed that a "Set<MobsimListener>" was bound before, and is used here.
    // I think that the results of
    // multibinding will be provided in several ways, one of them as this kind of set.  Thus, the
    // working assumption is that the
    // <MobsimListener> multibinder that is constructed in AbstractModule is retrieved here.  kai,
    // sep'20
  ...
  //	 * just "inline" the factory method of this class to plug together your own QSim, and you've
  // got it!
  ...
  public final void addNetworkChangeEvent(NetworkChangeEvent event) {
    // used (and thus implicitly tested) by bdi-abm-integration project.  A separate core test would
    // be good. kai, feb'18
