...
public final class PlanBasedDriverAgentImpl implements DriverAgent {
  ...
  public final Id<Link> chooseNextLinkId() {
    // To note: there is something really stupid going on here: A vehicle that is at the end of its
    // route and on the destination link will arrive.
    // However, a vehicle that is on the destination link BUT NOT AT THE END OF ITS ROUTE will NOT
    // arrive.  This makes the whole thing
    ...
    // Agreed.  One should also not assume that anything here is the result of one consistent design
    // process.  Rather, many people added
    // and removed material as they needed it for their own studies.  Making the whole code more
    // consistent would be highly
    ...
    if (this.cachedNextLinkId != null && !(this.cachedNextLinkId.equals(this.getCurrentLinkId()))) {
      // cachedNextLinkId used to be set to null when a leg started.  Now the BasicPlanAgentImpl
      // does not longer have access to cached
      // value.  kai, nov'14
    ...
    List<Id<Link>> routeLinkIds =
        ((NetworkRoute) this.basicPlanAgentDelegate.getCurrentLeg().getRoute()).getLinkIds();

    // (3) if route has run dry, we return the destination link (except for one special case, which
    // however may not be necessary any more):
    if (this.basicPlanAgentDelegate.getCurrentLinkIndex() >= routeLinkIds.size()) {
      ...
      if (this.getCurrentLinkId().equals(this.getDestinationLinkId())
        ...
        // the condition for arrival used to be "route has run dry AND destination link not attached
        // to current link".  now with loop links,
        // this condition was not triggered.  So no wonder that for such cases a special condition
        // was needed.  kai, nov'14
  ...
  public final boolean isWantingToArriveOnCurrentLink() {
    ...
    if (!(this.basicPlanAgentDelegate.getCurrentLeg().getRoute() instanceof NetworkRoute)) {
      // non-network links in the past have always returned true (i.e. "null" to the chooseNextLink
      // question). kai, nov'14
      return true;
    }
    ...
    // the standard condition used to be "route has run dry AND destination link not attached to
    // current link":
    // 2nd condition essentially meant "destination link EQUALS current link" but really stupid way
    // of stating this.  Thus
    // changing the second condition to "being at destination".  This breaks old code; I will fix as
    // far as it is covered by tests.  kai, nov'14
    if (this.basicPlanAgentDelegate.getCurrentLinkIndex() >= routeLinkIdsSize
        && this.getCurrentLinkId().equals(this.getDestinationLinkId())) {
      return true;
    }

    // the following are possible consistency checks on which previous code may have relied.
    // Relatively expensive because of hash map lookups. kai, nov'14

    //		Link currentLink = this.getScenario().getNetwork().getLinks().get( this.getCurrentLinkId() )
    // ;
    //		Link destinationLink = this.getScenario().getNetwork().getLinks().get(
    // this.getDestinationLinkId() ) ;
    //
    //		if ( this.currentLinkIndex >= routeLinkIdsSize ) { // route has run dry
    //			if ( currentLink.getToNode() == destinationLink.getFromNode() ) { // will arrive on next
    // link
    //				return false ;
    //			} else { // will not arrive on next link
    //				log.error("route has run dry, and destination link is not attached to next node.  In
    // consequence, vehicle has no chance to continue "
    //						+ "correctly.  Make it arrive here rather than explode it at the intersection.") ;
    //				return true ;
    //			}
    //		}

    //		Link nextLink = this.getScenario().getNetwork().getLinks().get(
    // routeLinkIds.get(this.getCurrentLinkIndex()) ) ;
    //
    //		if ( currentLink.getToNode() != nextLink.getFromNode() ) {
    //			log.error("route is inconsistent.  In consequence, vehicle has no chance to continue
    // correctly." +
  ...
  /* package */ public final void resetCaches() {
    ...
    // keep this where the internals are
    ...
    //		else {
    //			this.basicPlanAgentDelegate.calculateAndSetDepartureTime((Activity)
    // this.basicPlanAgentDelegate.getCurrentPlanElement());
    //		}
  ...
  public String toString() {
    return MoreObjects.toStringHelper(this)
        .add("basicPlanAgentDelegate", basicPlanAgentDelegate)
        .toString();
  }
}
