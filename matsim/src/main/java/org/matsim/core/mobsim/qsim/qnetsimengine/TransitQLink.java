...
final class TransitQLink {
  /**
   * A list containing all transit vehicles that are at a stop but not blocking other traffic on the
   * lane.
   */
  private final Queue<QVehicle> transitVehicleStopQueue =
      new PriorityQueue<>(5, VEHICLE_EXIT_COMPARATOR);

  private final QLaneI road;

  private static final Comparator<QVehicle> VEHICLE_EXIT_COMPARATOR =
    ...
    if (veh.getDriver() instanceof TransitDriverAgent) {
      ...
      while (true) {
        ...
        if ((stop != null) && (stop.getLinkId().equals(linkId))) {
          ...
          if (delay > 0.0) {
            // yy removing this condition makes at least one test fail.  I still think we could
            // discuss doing this. kai, jun'13
  ...
  /**
   * This method moves transit vehicles from the stop queue directly to the front of the "queue" of
   * the QLink. An advantage is that this will observe flow capacity restrictions.
   */
  void handleTransitVehiclesInStopQueue(final double now) {
    ...
    if (departingTransitVehicles != null) {
      ...
      ListIterator<QVehicle> iter =
          departingTransitVehicles.listIterator(departingTransitVehicles.size());
  ...
  HandleTransitStopResult handleTransitStop(
      final double now,
      final QVehicle veh,
    ...
    if ((stop != null) && (stop.getLinkId().equals(linkId))) {
      ...
      if (delay > 0.0) {
        veh.setEarliestLinkExitTime(now + delay);
        // (if the vehicle is not removed from the queue in the following lines, then this will
        // effectively block the lane
  ...
  /**
   * @author dstrippgen
   *     <p>Comparator object, to sort the Vehicle objects in QueueLink.parkingList according to
   *     their departure time
   */
  static class QVehicleEarliestLinkExitTimeComparator
      implements Comparator<QVehicle>, Serializable, MatsimComparator {
