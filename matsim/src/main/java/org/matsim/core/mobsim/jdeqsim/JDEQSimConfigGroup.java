...
public class JDEQSimConfigGroup extends ReflectiveConfigGroup {
  ...
  /**
   * the priorities of the messages. a higher priority comes first in the message queue (when same
   * time) usage: for example a person has a enter road message at the same time as leaving the
   * previous road (need to keep the messages in right order) for events with same time stamp: <br>
  ...
  /**
   * stuckTime is used for deadlock prevention. when a car waits for more than 'stuckTime' for
   * entering next road, it will enter the next. in seconds
   */
