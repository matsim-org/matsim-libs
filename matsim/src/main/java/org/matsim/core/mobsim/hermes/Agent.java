...
class Agent {
  ...
  // <0000> SleepForType    | 4 bits unused | 16 bit event id  | 8 bits unused   | 32 bit sleep for
  // a number of second
  // <0001> SleepUntilType  | 4 bits unused | 16 bit event id  | 8 bits unused   | 32 bit speep
  // until a specific time
  // <0010> LinkType        | 4 bits PCEcategory | 16 bit event id  | 32 bit link id  | 8 bit
  // velocity
  // <0111> WaitType        | 4 bits unused | 16 bit event id  | 8 bits unused   | 16 bit route id |
  // 16 station id
  // <0011> AccessType      | 4 bits unused | 16 bit event id  | 8 bits unused   | 16 bit route id |
  // 16 station id
  // <0100> EgressType      | 4 bits unused | 16 bit event id  | 8 bits unused   | 16 bit route id |
  // 16 station id
  // <0101> StopArriveType  | 4 bits unused | 16 bit event id  | 8 bits unused   | 16 bit route id |
  // 16 station id
  // <1000> StopDelayType   | 20 departure sec                 | 8 bits unused   | 16 bit route id |
  // 16 station id
  // <0110> StopDepartType  | 4 bits unused | 16 bit event id  | 8 bits unused   | 16 bit route id |
  // 16 station id
  ...
  private static final List<Agent> NO_PASSENGERS = Collections.emptyList();
  ...
  public static int getPlanHeader(long plan) {
    return (int) ((plan >> 60) & 0x000000000000000FL);
  }

  public static int getPlanEvent(long plan) {
    return (int) ((plan >> 40) & 0x000000000000FFFFL);
  }

  public static int getDeparture(long plan) {
    return (int) ((plan >> 40) & 0x00000000000FFFFFL);
  }
  ...
  public static double getVelocityPlanEntry(long plan) {
    return decodeVelocityFromLinkEntry((int) (plan & 0x00000000000000FFL));
  }

  public static int getRoutePlanEntry(long plan) {
    return (int) ((plan >> 16) & 0x000000000000FFFFL);
  }

  public static int getStopPlanEntry(long plan) {
    return (int) (plan & 0x000000000000FFFFL);
  }

  public static int getSleepPlanEntry(long plan) {
    return (int) (plan & 0x00000000FFFFFFFFL);
  }
  ...
  public static long prepareLinkEntry(int eventid, int linkid, double velocity, int pcecategory) {
    long l =
        preparePlanEventEntry(
            LinkType, eventid, prepareLinkEntryElement(linkid, velocity, pcecategory));
    return l;
  }
  ...
  private static class UpcomingStops {
    ...
    public void add(int stopId, Agent agent) {
      ...
      throw new RuntimeException(
          "Could not find upcoming stop "
              + Id.get(stopId, TransitStopFacility.class)
              + " along this route.");
    }
