...
/**
 * Coordinates the movement of vehicles on the links and the nodes. Split Up the old {@code
 * QNetsimEngineRunner} which was implementing 2 different approaches parallel.
 *
 ...
 * @author dstrippgen
 */
final class QNetsimEngineWithThreadpool
  ...
  @Inject
  QNetsimEngineWithThreadpool(final QSim sim, QNetworkFactory netsimNetworkFactory) {
  ...
  protected void run(double time) {
    ...
    // message-based experiments a decade ago, it was better to let each runner decide locally when
    // to proceed.  For intuition, imagine that
    // one runner is slowest on the links, and some other runner slowest on the nodes.  With the
    // barriers, this cannot overlap.
    // With message passing, this was achieved by waiting for all necessary messages.  Here, it
    // could (for example) be achieved with runner-local
    // clocks:
    ...
    // (b) Do deliberate domain decomposition rather than round robin (fewer runners to wait for at
    // (*) and (**)).

    // (c) One thread that is much faster than all others is much more efficient than one thread
    // that is much slower than all others.
    // So make sure that no thread sticks out in terms of slowness.  Difficult to achieve, though.
    // A decade back, we used a "typical" run
