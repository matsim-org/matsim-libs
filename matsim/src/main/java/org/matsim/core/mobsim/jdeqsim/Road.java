...
public class Road extends SimUnit {
  ...
  /**
   * this must be initialized before starting the simulation! mapping: key=linkId used to find a
   * road corresponding to a link
   */
  ...
  /** all roads, which are interested in entering the road, but wasn't allowed to do so yet */
  ...
  /**
   * the inverseFlowCapacity is simply the inverse of the respective capacities meaning, and
   * corresponds to the minimal time between two cars entering/leaving the road
   */
  ...
  /**
   * this variable keeps track of the number of cars, which are not on the road, but which have been
   * promised to enter the road (given a time in future, when they can enter the road)
   */
  ...
  /**
   * when trying to enter a road, a deadlock prevention message is put into the queue this allows a
   * car to enter the road, even if no space on it
   */
  ...
  public Road(Scheduler scheduler, Link link) {
    ...
    this.maxNumberOfCarsOnRoad =
        Math.round(
            link.getLength()
                ...
                * config.getStorageCapacityFactor()
                / config.getCarSize());
    ...
    double maxInverseInFlowCapacity =
        3600
            / (config.getMinimumInFlowCapacity()
                * config.getFlowCapacityFactor()
                * NetworkUtils.getNumberOfLanesAsInt(link));

    this.inverseOutFlowCapacity =
        1 / (link.getFlowCapacityPerSec() * config.getFlowCapacityFactor());
  ...
  public void leaveRoad(Vehicle vehicle, double simTime) {
    ...
    if (this.interestedInEnteringRoad.size() > 0) {
      ...
      double nextAvailableTimeForEnteringStreet =
          Math.max(
              this.timeOfLastEnteringVehicle + this.inverseInFlowCapacity,
              simTime + this.gapTravelTime);
    ...
    if (this.carsOnTheRoad.size() > 0) {
      ...
      double nextAvailableTimeForLeavingStreet =
          Math.max(
              this.earliestDepartureTimeOfCar.getFirst(),
  ...
  public void enterRoad(Vehicle vehicle, double simTime) {
    ...
    double nextAvailableTimeForLeavingStreet =
        simTime + this.link.getLength() / this.link.getFreespeed(simTime);
  ...
  public void enterRequest(Vehicle vehicle, double simTime) {
    ...
    if (this.carsOnTheRoad.size() + this.noOfCarsPromisedToEnterRoad < this.maxNumberOfCarsOnRoad) {
      ...
      if (this.deadlockPreventionMessages.size() > 0) {
        this.deadlockPreventionMessages.add(
            vehicle.scheduleDeadlockPreventionMessage(
        ...
        this.deadlockPreventionMessages.add(
            vehicle.scheduleDeadlockPreventionMessage(simTime + config.getSqueezeTime(), this));
      }
