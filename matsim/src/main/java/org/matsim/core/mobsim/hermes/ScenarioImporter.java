...
class ScenarioImporter {
  ...
  private static final Logger log = LogManager.getLogger(Hermes.class);
  ...
  // matsim route id (Id<TransitRoute>.index) of a particular hermes route number. Should be used as
  // follows:
  ...
  private void generateVehicleCategories() {
    ...
    if (vehicleTypes >= HermesConfigGroup.MAX_VEHICLE_PCETYPES) {
      throw new RuntimeException(
          "Too many vehicle types defined. A maximum of "
              + HermesConfigGroup.MAX_VEHICLE_PCETYPES
              + " is supported.");
    }
    ...
    for (VehicleType t : scenario.getVehicles().getVehicleTypes().values()) {
      ...
      flowCapacityPCEs[i] =
          (float)
              (t.getPcuEquivalents()
                  / (t.getFlowEfficiencyFactor()
                      * scenario.getConfig().hermes().getFlowCapacityFactor()));
      storageCapacityPCEs[i] =
          (float)
              (t.getPcuEquivalents() / scenario.getConfig().hermes().getStorageCapacityFactor());
  ...
  public static ScenarioImporter instance(Scenario scenario, EventsManager eventsManager) {
    ...
    if (instance == null
        || !scenario.equals(instance.scenario)
        || !eventsManager.equals(instance.eventsManager)) {
  ...
  public void generate() throws Exception {
    ...
    log.info(
        String.format(
            "Hermes reset took %d ms  (%d agents %d links)",
            System.currentTimeMillis() - time, hermesAgents.length, hermesLinks.length));
  ...
  private void generateLinks() {
    ...
    for (org.matsim.api.core.v01.network.Link matsim_link : matsim_links) {
      ...
      int storageCapacity =
          Math.max(
              1,
              (int) (Math.ceil(matsim_link.getLength() / network.getEffectiveCellSize() * lanes)));
      ...
      hermesLinks[link_id] =
          new HLink(
              ...
              length,
              speed,
              effectiveflowCapacityPerSec,
              scenario.getConfig().hermes().getStuckTime());
    }
  }
    ...
    for (TransitLine line : ts.getTransitLines().values()) {
      for (TransitRoute route : line.getRoutes().values()) {
        ...
        for (int i = 0, stopsCount = stops.size(); i < stopsCount; i++) {
          ...
          IntArrayMap<ArrayDeque<Agent>> linesMap =
              this.agentStops.computeIfAbsent(stopFacility.getId(), k -> new IntArrayMap<>());
      ...
      switch (type) {
        case Agent.LinkType:
          ...
          agent.linkFinishTime =
              (int) Math.round(link.length() / Math.min(velocity, link.velocity()));
        ...
        case Agent.SleepUntilType:
          ...
          realm
              .delayedAgents()
              .get(Math.min(sleep, scenario.getConfig().hermes().getEndTime() + 1))
              .add(agent);
          break;
        default:
          LogManager.getLogger(getClass())
              .error(String.format("ERROR -> unknown plan element type %d", type));
      }
    }
  ...
  private void processPlanActivity(
      Id<Person> id, PlanArray flatplan, EventArray events, Activity act) {
    int time = 0;
    int eventid = 0;
  ...
  private void processPlanNetworkRoute(
    ...
    Id<VehicleType> vtypeid =
        v == null ? VehicleUtils.getDefaultVehicleType().getId() : v.getType().getId();
    int pcuCategory = this.vehicleTypeMapping.get(vtypeid);
    Id<Vehicle> vid = v == null ? Id.createVehicleId("v" + person.getId()) : v.getId();
    double velocity =
        v == null ? HermesConfigGroup.MAX_VEHICLE_VELOCITY : v.getType().getMaximumVelocity();
  ...
  private void processPlanTransitRoute(
      Id<Person> id, PlanArray flatplan, EventArray events, TransitPassengerRoute troute) {
  ...
  private void processPlanElement(
      Person person, PlanArray flatplan, EventArray events, PlanElement element, Agent agent) {
    var id = person.getId();
    if (element instanceof Leg) {
      ...
      if (route instanceof NetworkRoute) {
        ...
        throw new RuntimeException(
            "Route type not supported by Hermes: "
                + route.getRouteType()
                + "\n Person:"
                + id
                + "\n Leg"
  ...
  private void processTeleport(
      Id<Person> id, PlanArray flatplan, EventArray events, Leg element, Route route, String mode) {
  ...
  private void generateAgent(int agentId, int capacity, PlanArray flatplan, EventArray events) {
  ...
  private static class TransitRouteContext {
    ...
    public TransitRouteContext(
      ...
      int earlyDeparture =
          this.trs.get(0).getDepartureOffset().seconds() > 0
              ? 0
              : 3; // start 3 seconds early if we should depart at offset 0.0, otherwise exactly at
      // the specified time
        ...
        while (nextStop != null) {
          if (link.getId().equals(nextStopLinkId)) {
            double offset =
                nextStop
                    .getArrivalOffset()
                    .or(nextStop.getDepartureOffset())
                    .orElseThrow(
                        (() ->
                            new RuntimeException("Stop has neither arrival nor departure offset")));
            ...
            if (speed > HermesConfigGroup.MAX_VEHICLE_VELOCITY) {
              ...
              // make sure vehicle always is at least 1m/s fast to prevent 0 values which result in
              // Infinite values later on
              speed = 1.0;
            }
  ...
  private void generateDeterministicVehicleOnLink(
    ...
    if (stopsToHandle > 0) {
      for (int i = 0; i < stopsToHandle; i++) {
        ...
        if (arrivalTime < c.time) {
          arrivalTime =
              c.time; // there should be at least 1 second between the last stop and this one
        }
        ...
        c.flatplan.add(
            Agent.prepareStopArrivalEntry(c.flatevents.size() - 1, c.routeNo, stopIdIndex));
        ...
        c.flatplan.add(
            Agent.prepareStopDepartureEntry(c.flatevents.size() - 1, c.routeNo, stopIdIndex));
    ...
    if (generateLinkLeaveEvent) {
      if (stopsToHandle > 0) {
        // if we handled a stop, then just assume we leave the link directly after handling the
        // stop.
        c.time++;
      } else {
        ...
        if (generateLinkEnterEvent) {
          ...
          // we did not handle a stop, but also not enter the link. Most likely it's the first link
          // of the route,
      ...
      c.flatplan.add(
          Agent.prepareSleepUntilEntry(
              0, c.time + 1)); // wait finishing the agent until we've driven along all links
    }
  }

  private void generateDeterministicVehicleTrip(
      Agent agent, TransitLine tl, TransitRoute tr, Departure depart) {
    ...
    TransitRouteContext context =
        new TransitRouteContext(agent, tl, tr, routeNo, depart, this.scenario.getNetwork());
    ...
    flatevents.add(
        new PersonDepartureEvent(0, driverid, nr.getStartLinkId(), legmode, routingmode));
    flatevents.add(new PersonEntersVehicleEvent(0, driverid, v.getId()));

    flatevents.add(
        new VehicleEntersTrafficEvent(
            context.time, driverid, nr.getStartLinkId(), v.getId(), legmode, 1));

    // Sleep until the time of departure
    //  the very first flat plan entry does not handle events, so actually add two entries, so the
    // events are correctly handled
    ...
    flatevents.add(
        new VehicleLeavesTrafficEvent(0, driverid, nr.getEndLinkId(), v.getId(), legmode, 1));
  ...
  private void generateNondeterministicVehicleOnLink(
      TransitRouteContext c,
      Id<Link> linkId,
      boolean generateLinkEnterEvent,
      boolean generateLinkLeaveEvent,
    ...
    if (generateLinkEnterEvent) {
      ...
      c.flatplan.add(
          Agent.prepareLinkEntry(c.flatevents.size() - 1, linkId.index(), velocity, pcuCategory));
    }
  ...
  private void generateVehicleTrip(Agent agent, TransitLine tl, TransitRoute tr, Departure depart) {
    ...
    NetworkRoute nr = tr.getRoute();
    int pcuCategory = 0;
    // the PCU category for transit vehicles is never read from plan entry but remains constant over
    // the day
    int velocity =
        (int)
            Math.min(
                Math.round(v.getType().getMaximumVelocity()),
                HermesConfigGroup.MAX_VEHICLE_VELOCITY);
    ...
    flatevents.add(new PersonEntersVehicleEvent(0, driverid, v.getId()));
    flatevents.add(
        new VehicleEntersTrafficEvent(0, driverid, nr.getStartLinkId(), v.getId(), legmode, 1));

    generateNondeterministicVehicleOnLink(
        context, nr.getStartLinkId(), false, true, velocity, pcuCategory);
    ...
    generateNondeterministicVehicleOnLink(
        context, nr.getEndLinkId(), true, false, velocity, pcuCategory);
              ...
              for (TransitRoute tr : tl.getRoutes().values()) {
                for (Departure depart : tr.getDepartures().values()) {
                  ...
                  float storageCapacityPCUE =
                      deterministicPt ? 0.1f : (float) v.getType().getPcuEquivalents();
                  float flowCapacityPCUE =
                      deterministicPt
                          ? 0.1f
                          : (float)
                              (v.getType().getPcuEquivalents()
                                  / v.getType().getFlowEfficiencyFactor());
