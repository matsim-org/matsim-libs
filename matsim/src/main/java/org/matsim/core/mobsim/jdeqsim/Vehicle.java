...
public class Vehicle extends SimUnit {
  ...
  /**
   * based on the current Leg, the previous activity is computed; this could be implemented more
  ...
  /**
   * based on the current Leg, the next activity is computed; this could be implemented more
   * efficiently in future.
   *
   * @return
   */
  ...
  /**
   * updates both the currentLink and link index variables with the next link in the link route of
   * the current leg attention: only applicable, if isCurrentLegFinished==false
   */
  ...
  /**
   * find out, if the vehicle is in endingLegMode this means, that the vehicle is just waiting until
   * it can enter the last link (without entering it) and then ends the leg
   *
   * @return
   */
  ...
  public void scheduleEndLegMessage(double scheduleTime, Road road) {
    sendMessage(
        MessageFactory.getEndLegMessage(road.scheduler, this, timeInterpretation),
        road,
        scheduleTime);
  }
  ...
  public DeadlockPreventionMessage scheduleDeadlockPreventionMessage(
    ...
    DeadlockPreventionMessage dpMessage =
        MessageFactory.getDeadlockPreventionMessage(road.scheduler, this);
