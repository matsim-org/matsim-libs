...
final class VisUtils {
   ...
   *
   * @return All the people in this vehicle. If there is more than one, the first entry is the
   *     driver.
   */
  ...
  public static double calcSpeedValueBetweenZeroAndOne(
      QVehicle veh, double inverseSimulatedFlowCapacity, double now, double freespeed) {
    int cmp = (int) (veh.getEarliestLinkExitTime() + inverseSimulatedFlowCapacity + 2.0);
    // "inverseSimulatedFlowCapacity" is there to keep vehicles green that only wait for capacity
    // (i.e. have no vehicle
