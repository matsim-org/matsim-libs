 ...
 *
 * <ul>
 *   <li>This needs to be public since the ``Q'' version of the vehicle is used by more than one
 *       package. This interfaces should, however, not be used outside the relevant netsimengines.
 *       In particular, the information should not be used for visualization. kai, nov'11
 *   <li>Might be possible to make all methods in this hierarchy protected and just inherit to a
 *       QueueVehicle. kai, nov'11
 * </ul>
 *
 * @author nagel
 */
public class QVehicleImpl implements QVehicle {
  ...
  public QVehicleImpl(final Vehicle basicVehicle) {
    ...
    if (capacity == null) {
      ...
      if (warnCount < 10) {
        log.warn(
            "No VehicleCapacity (= maximum number of passengers) set in Vehicle. "
      ...
      // do *not* subtract one for the driver! Most pt vehicles define the capacity without the
      // driver.
      // for private cars, think about if we should subtract one from the capacity if the driver is
      // set?
   ...
   *
   * <ul>
   *   <li>I am fairly sure that this should not be publicly exposed. As far as I can tell, it is
   *       used in order to figure out of a visualizer should make a vehicle "green" or "red". But
   *       green or red should be related to vehicle speed, and the mobsim should figure that out,
   *       not the visualizer. So something like "getCurrentSpeed" seems to be a more useful option.
   *       kai, nov'11
   *   <li>The problem is not only the speed, but also the positioning of the vehicle in the
   *       "asQueue" plotting method. (Although, when thinking about it: Should be possible to
   *       obtain same result by using "getEarliestLinkExitTime()". But I am not sure if that would
   *       really be a conceptual improvement ... linkEnterTime is, after all, a much more
   ...
   *   <li>removed this while refactoring the visualizer computations, dg jan'12
   * </ul>
   */
   ...
   *
   * <ul>
   *   <li>This has to remain public as long as QVehicle/QVehicleImpl is both used by
   *       QueueSimulation and QSim. At best, we could say that there should also be a MobsimVehicle
   *       interface that does not expose this. kai, nov'11. (This is there now. kai, nov'11)
   *   <li>removed this while refactoring the visualizer computations, dg jan'12
   * </ul>
   */
