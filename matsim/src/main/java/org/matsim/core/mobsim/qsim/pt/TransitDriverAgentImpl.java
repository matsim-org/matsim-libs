...
public class TransitDriverAgentImpl extends AbstractTransitDriverAgent {
  ...
  private static class PlanBuilder {
    ...
    public void addTrip(NetworkRoute networkRoute, String transportMode) {
      ...
      if (!plan.getPlanElements().isEmpty()) {
        ...
        lastActivity =
            PopulationUtils.createActivityFromLinkId(activityType, networkRoute.getStartLinkId());
      ...
      plan.addLeg(leg);
      Activity activity =
          PopulationUtils.createActivityFromLinkId(activityType, networkRoute.getEndLinkId());
        ...
        PopulationUtils.getFactory()
            .createPerson(
                Id.create(
                    "pt_" + umlauf.getId(),
                    Person
                        .class)); // we use the non-wrapped route for efficiency, but the leg has to
    // return the wrapped one.
  ...
  public boolean isWantingToArriveOnCurrentLink() {
    // The following is the old condition: Being at the end of the plan means you arrive anyways, no
    // matter if you are on the right or wrong link.
    // kai, nov'14
  ...
  public Facility getCurrentFacility() {
    ...
    // the above assumes alternating acts/legs.  I start having the suspicion that we should revoke
    // our decision to give that up.
    // If not, one will have to use TripUtils to find the preceeding activity ... but things get
    // more difficult.  Preferably, the
