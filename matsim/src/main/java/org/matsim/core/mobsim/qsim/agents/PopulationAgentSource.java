...
public final class PopulationAgentSource implements AgentSource {
  ...
  PopulationAgentSource(
      ...
      QVehicleFactory qVehicleFactory,
      QSim qsim) {
  ...
  private void insertVehicles(Person person) {
    // this is called in every iteration.  So if a route without a vehicle id is found (e.g. after
    // mode choice),
    ...
    for (Leg leg : TripStructureUtils.getLegs(person.getSelectedPlan())) {
      ...
      final QSimConfigGroup.VehiclesSource vehiclesSource =
          qsim.getScenario().getConfig().qsim().getVehiclesSource();

      if (vehiclesSource != QSimConfigGroup.VehiclesSource.fromVehiclesData) {
        // without this condition, it is surely wrong when the vehicles come from the vehicles data
        // ... because a second vehicle for
        // the same mode is not placed.  kai, nov'19

        // yyyy I am not sure if at this point vehicleId actually can be null ... because the above
        // VehicleUtils.getVehicleId(...,
      ...
      // find the vehicle from the vehicles container.  It should be there, see automatic vehicle
      // creation in PrepareForSim.
      Vehicle vehicle = qsim.getScenario().getVehicles().getVehicles().get(vehicleId);
      if (vehicle == null) {
        String msg =
            "Could not get the requested vehicle with ID="
                + vehicleId
        ...
        switch (vehiclesSource) {
          ...
          case modeVehicleTypesFromVehiclesData:
            msg +=
                "You are using the config switch qsim.vehiclesSource="
                    + vehiclesSource.name()
      ...
      if (result != null) {
        ...
        if (result != vehicleLinkId) {
          throw new RuntimeException(
              "vehicle placement error: vehicleId="
                  + vehicleId
                  + "; previous placement link="
                  + vehicleLinkId
                  + "; current placement link="
                  + result);
        }
  ...
  /**
   * A more careful way to decide where this agent should have its vehicles created than to ask
   * agent.getCurrentLinkId() after creation.
   *
   * @param person TODO
   */
  private Id<Link> findVehicleLink(Person person) {
    ...
    for (PlanElement planElement : person.getSelectedPlan().getPlanElements()) {
      if (planElement instanceof Activity) {
        ...
        final Id<Link> activityLinkId =
            PopulationUtils.computeLinkIdFromActivity(activity, facilities, config);
