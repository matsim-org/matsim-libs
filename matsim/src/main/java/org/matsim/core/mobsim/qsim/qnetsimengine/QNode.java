...
public class QNode implements NetsimNode {
	...
	/*
	 * This needs to be atomic since this allows us to ensure that an node which
	 * is already active is not activated again. This could happen if multiple
	 * thread call activateNode() concurrently. cdobler, sep'14
	 */
	...
	private QNode(final Node n, NetsimEngineContext context, NetsimInternalInterface netsimEngine2) {
		...
		if (this.context.qsimConfig.getNumberOfThreads() > 1) {
			// This could just as well be the "normal" case. The second
			// alternative
			// is just there so some scenarios / test cases stay
			// "event-file-compatible". Consider removing the second
			// alternative.
	...
	/**
	 * Loads the inLinks-array with the corresponding links. Cannot be called in
	 * constructor, as the queueNetwork does not yet know the queueLinks. Should
	 * be called by QueueNetwork, after creating all QueueNodes and QueueLinks.
	 */
	/* package */ void init() {
		int i = 0;
		...
		/*
		 * As the order of links has an influence on the simulation results, the
		 * nodes are sorted to avoid indeterministic simulations. dg[april08]
		 */
	...
	/*
	 * The ParallelQSim replaces the activator with the QSimEngineRunner that
	 * handles this node.
	 */
	...
	/*
	 * This method is called from QueueWithBuffer.addToBuffer(...) which is
	 * triggered at some placed, but always initially by a QLink's
	 * doSomStep(...) method. I.e. QNodes are only activated while
	 * moveNodes(...) is performed. However, multiple threads could try to
	 * activate the same node at a time, therefore this has to be thread-safe.
	 * cdobler, sep'14
	 */
	/* package */ final void activateNode() {
		/*
		 * this.active.compareAndSet(boolean expected, boolean update) We expect
		 * the value to be false, i.e. the node is de-activated. If this is
	 ...
	 * there is no free space on its destination link, the work on this inLink
	 * is finished and the next inLink's buffer is handled (this means, that at
	 * the node, all links have only like one lane, and there are no separate
	 * lanes for the different outLinks. Thus if the front most vehicle cannot
	 * drive further, all other vehicles behind must wait, too, even if their
	 * links would be free).
	 *
	 * @param now
	...
	private static boolean checkNextLinkSemantics(Link currentLink, Id<Link> nextLinkId, QLinkI nextQLink,
			QVehicle veh) {
		// we need nextLinkId and nextQLink, because the link lookup may have
		// failed and then nextQLink is null
		if (nextQLink == null) {
			log.warn("The link id " + nextLinkId + " is not available in the simulation network, but vehicle "
					+ veh.getId() + " plans to travel on that link from link " + veh.getCurrentLink().getId());
			return false;
		}
...
//			 return false;
		...
		// final String message = "The link with id " + nextLinkId + " does not
		// allow the current mode, which is " + veh.getDriver().getMode();
		// throw new RuntimeException( message ) ;
		//// log.warn(message );
		//// return false ;
		// // yyyy is rather nonsensical to get the mode from the driver, not
		// from the vehicle. However, this seems to be
		// // how it currently works: network links are defined for modes, not
		// for vehicle types. kai, may'16
		// }
	...
	// ////////////////////////////////////////////////////////////////////
	/**
	 * @return <code>true</code> if the vehicle was successfully moved over the
	 *         node, <code>false</code> otherwise (e.g. in case where the next
	 *         link is jammed)
	 */
	private boolean moveVehicleOverNode(final QVehicle veh, final QLaneI fromLaneBuffer, final double now) {
		...
		if ((!fromLaneBuffer.hasGreenForToLink(nextLinkId))) {
			...
			// in case of an infinite red time the simulation will not stop
			// automatically because
			// vehicles waiting in front of the red signal will never reach
			// their destination. dg, mar'14
			return false;
		}

		Link currentLink = veh.getCurrentLink();
		if (nextLinkId == null) {
			log.error("Agent has no or wrong route! agentId=" + veh.getDriver().getId() + " currentLink="
					+ currentLink.getId().toString() + ". The agent is removed from the simulation.");
		...
		if (vehicleIsStuck(fromLaneBuffer, now)) {
			 ...
			 * optionally we let them die here, we have a config setting for
			 * that!
			 */
			if (this.context.qsimConfig.isRemoveStuckVehicles()) {
				...
				moveVehicleFromInlinkToOutlink(veh, currentLink.getId(), fromLaneBuffer, nextLinkId, nextQueueLane);
				return true;
				// (yyyy why is this returning `true'? Since this is a fix to
				// avoid gridlock, this should proceed in small steps.
				// kai, feb'12)
			}
		}
	...
	private void moveVehicleFromInlinkToOutlink(final QVehicle veh, Id<Link> currentLinkId, final QLaneI fromLane,
			Id<Link> nextLinkId, QLaneI nextQueueLane) {
		double now = this.context.getSimTimer().getTimeOfDay();

		fromLane.popFirstVehicle();
		// -->
		// network.simEngine.getMobsim().getEventsManager().processEvent(new
		// LaneLeaveEvent(now, veh.getId(), currentLinkId, fromLane.getId()));
		this.context.getEventsManager().processEvent(new LinkLeaveEvent(now, veh.getId(), currentLinkId));
		// <--
