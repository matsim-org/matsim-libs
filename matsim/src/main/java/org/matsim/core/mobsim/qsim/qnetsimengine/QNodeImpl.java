...
final class QNodeImpl extends AbstractQNode {
  ...
  /**
   * Checks if capacity is significant positive. This allows for a small eps unequal to zero to
   * mitigate rounding errors that can happen with non integer capacities.
   */
    ...
    switch (nodeTransitionLogic) {
      ...
      default:
        throw new UnsupportedOperationException(
            "Node transition logic " + nodeTransitionLogic + " is not implemented.");
    }
  ...
  public boolean doSimStep(final double now) {
    ...
    // select vehicles to be moved over the node. The order of vehicles selected depends on the
    // chosen node transition logic.
    switch (nodeTransitionLogic) {
      case emptyBufferAfterBufferRandomDistribution_dontBlockNode:
      case emptyBufferAfterBufferRandomDistribution_nodeBlockedWhenSingleOutlinkFull:
        // randomize based on link capacity: select a link; if next links have enough space all
        // vehicles from the buffer are allowed to pass the node
      ...
      case moveVehByVehRandomDistribution_dontBlockNode:
      case moveVehByVehRandomDistribution_nodeBlockedWhenSingleOutlinkFull:
        // randomize based on link capacity: select the vehicles (one by one) that are allowed to
        // pass the node
        while (capacityLeft(inLinksCapSum)) {
          double rndNum = random.nextDouble() * inLinksCapSum;
          double selCap = 0.0;
          for (int i = 0; i < this.inLinksArrayCache.length; i++) {
            ...
            if (link != null) {
              ...
              if (selCap >= rndNum) {
                ...
                // select the next link (could be the same again) that is allowed to move a vehicle.
                // I.e. start again with the beginning of the while-loop
                break;
              }
            }
          }
        ...
        break;
      case moveVehByVehDeterministicPriorities_nodeBlockedWhenSingleOutlinkFull:
        // deterministically choose the inLinks vehicle by vehicle based on their capacity and also
        // account for decisions made in previous time steps (i.e. update priorities) to approximate
        // the correct distribution over time.
        double prioWithWhichTheLastVehWasSent = 0.;
        while (capacityLeft(inLinksCapSum)) {
          // look for the inLink with minimal priority that has vehicles in the buffer (use first
          // one when equal, because links are sorted by ID)
          ...
          // select the next link (could be the same again) that is allowed to move a vehicle. I.e.
          // start again with the beginning of the while-loop
        }
  ...
  private void updatePriorities(
      final double now, double prioWithWhichTheLastVehWasSent, int linkIndexToBeExcluded) {

    for (int linkIndex = 0; linkIndex < this.inLinkPriorities.length; linkIndex++) {
      // when the node was blocked (see * above) no priority updating is necessary for the blocked
      // links
      if (linkIndex != linkIndexToBeExcluded && tempLinks[linkIndex] == null) {
        // shift priorities of links that where disabled earlier because of other reasons, e.g.
        // because their buffer was empty or a traffic light showed red,
        // to the level of the other priorities such that they are not overprioritized in next time
        // steps
        inLinkPriorities[linkIndex] =
            prioWithWhichTheLastVehWasSent
   ...
   *     <p>This logic is only implemented for the case without lanes. When multiple lanes exist for
   *     a link, an exception is thrown.
   */
  private boolean moveFirstVehicleOnLink(final double now, QLinkI link) {
    if (link.getOfferingQLanes().size() > 1) {
      throw new RuntimeException(
          "The qsim node transition parameter "
              + NodeTransition.moveVehByVehRandomDistribution_dontBlockNode
              + ", "
              ...
              + " are only implemented for the case without lanes. But link "
              + link.getLink().getId()
              + " in your scenario has more than one lane. "
              + "Use the default node transiton "
    ...
    for (QLaneI lane : link.getOfferingQLanes()) {
      if (!lane.isNotOfferingVehicle()) {
        ...
        if (moveVehicleOverNode(veh, link, lane, now)) {
          ...
          // vehicle was not moved, e.g. because the next link is jammed or a traffic light on this
          // link shows red.
          if (this.stopMoveNodeWhenSingleOutlinkFull && this.atLeastOneOutgoingLaneIsJammed) {
  ...
  private boolean moveVehicleOverNode(
      final QVehicle veh, QLinkI fromLink, final QLaneI fromLane, final double now) {
    Id<Link> nextLinkId = veh.getDriver().chooseNextLinkId();
    Link currentLink = fromLink.getLink();

    AcceptTurn turn =
        turnAcceptanceLogic.isAcceptingTurn(
            currentLink, fromLane, nextLinkId, veh, this.netsimEngine.getNetsimNetwork(), now);
    ...
    if (vehicleIsStuck(fromLane, now)) {
      ...
      if (this.context.qsimConfig.isRemoveStuckVehicles()) {
        ...
        moveVehicleFromInlinkToOutlink(
            veh, currentLink.getId(), fromLane, nextLinkId, nextQueueLane);
        return true;
  ...
  private void moveVehicleFromInlinkToAbort(
      final QVehicle veh, final QLaneI fromLane, final double now, Id<Link> currentLinkId) {
    fromLane.popFirstVehicle();
    // -->
    this.context
        .getEventsManager()
        .processEvent(new LinkLeaveEvent(now, veh.getId(), currentLinkId));
    // <--
  ...
  private void moveVehicleFromInlinkToOutlink(
      final QVehicle veh,
      Id<Link> currentLinkId,
      final QLaneI fromLane,
      Id<Link> nextLinkId,
      QLaneI nextQueueLane) {
    double now = this.context.getSimTimer().getTimeOfDay();

    fromLane.popFirstVehicle();
    // -->
    //		network.simEngine.getMobsim().getEventsManager().processEvent(new LaneLeaveEvent(now,
    // veh.getId(), currentLinkId, fromLane.getId()));
    this.context
        .getEventsManager()
        .processEvent(new LinkLeaveEvent(now, veh.getId(), currentLinkId));
    // <--
