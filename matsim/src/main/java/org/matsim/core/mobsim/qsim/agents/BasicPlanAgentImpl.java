...
public final class BasicPlanAgentImpl
    implements MobsimAgent, PlanAgent, HasPerson, VehicleUsingAgent, HasModifiablePlan {
  ...
  /**
   * This notes how far a route is advanced. One could move this into the DriverAgent(Impl). There,
   * however, is no method to know about the start of the route, thus one has to guess when to set
   * it back to zero. Also, IMO there is really some logic to providing this as a service by the
   * entity that holds the plan. Better ideas are welcome. kai, nov'14
   */
  private int currentLinkIndex = 0;

  public BasicPlanAgentImpl(
      Plan plan2,
      Scenario scenario,
      EventsManager events,
      MobsimTimer simTimer,
      TimeInterpretation timeInterpretation) {

    this.plan = PopulationUtils.unmodifiablePlan(plan2);
    // yy MZ suggests, and I agree, to always give the agent a full plan, and consume that plan as
    // the agent goes.  kai, nov'14
    ...
    Preconditions.checkArgument(
        !planElements.isEmpty(), "Plan must consist of at least one activity");
    Activity firstAct = (Activity) planElements.get(0);
    // this.setCurrentLinkId( firstAct.getLinkId() ) ;
    final Id<Link> linkId =
        PopulationUtils.computeLinkIdFromActivity(
            firstAct, scenario.getActivityFacilities(), scenario.getConfig());
  ...
  public final void endLegAndComputeNextState(final double now) {
    this.getEvents()
        .processEvent(
            new PersonArrivalEvent(
                now, this.getId(), this.getDestinationLinkId(), getCurrentLeg().getMode()));
    if ((!(this.getCurrentLinkId() == null && this.getDestinationLinkId() == null))
      ...
      // note that when we are here we don't know if next is another leg, or an activity  Therefore,
      // we go to a general method:
      advancePlan(now);
    }
  ...
  private void initializeActivity(Activity act, double now) {
    ...
    getModifiablePlan(); // this is necessary to make the plan modifiable, so that setting the start
    // time (next line) is actually feasible. kai/mz, oct'16
    ((Activity) getCurrentPlanElement()).setStartTime(now);
  }

  /**
   * If this method is called to update a changed ActivityEndTime please ensure that the
   * ActivityEndsList in the {@link QSim} is also updated.
   */
  private final void calculateAndSetDepartureTime(Activity act) {
    ...
    double departure =
        Math.max(
            now,
            timeInterpretation.decideOnActivityEndTime(act, now).orElse(Double.POSITIVE_INFINITY));
    ...
    if (this.getCurrentPlanElementIndex() == this.getCurrentPlan().getPlanElements().size() - 1) {
      if (finalActHasDpTimeWrnCnt < 1 && departure != Double.POSITIVE_INFINITY) {
        log.error(
            "last activity of person driver agent id "
                + this.getId()
  ...
  public final void endActivityAndComputeNextState(final double now) {
    if (!(this.getCurrentPlanElement() instanceof Activity)) {
      log.warn(
          "trying to end an activity but current plan element is not an activity; agentId="
              + this.getId());
    }
  ...
  /**
   * Returns a modifiable Plan for use by WithinDayAgentUtils in this package. This agent retains
   * the copied plan and forgets the original one. However, the original plan remains in the
   * population file (and will be scored). This is deliberate behavior!
   */
  @Override
  ...
  public final Id<Vehicle> getPlannedVehicleId() {
    NetworkRoute route =
        (NetworkRoute) this.getCurrentLeg().getRoute(); // if casts fail: illegal state.
  ...
  public final String getMode() {
    if (this.getCurrentPlanElementIndex() >= this.getCurrentPlan().getPlanElements().size()) {
      // just having run out of plan elements it not an argument for not being able to answer the
      // "mode?" question,
  ...
  public final Activity getNextActivity() {
    for (int idx = this.currentPlanElementIndex + 1;
        idx < this.plan.getPlanElements().size();
        idx++) {
  ...
  /* default */ final int getCurrentPlanElementIndex() {
    ...
    // Pro: Many programmers urgently seem to need this: They do everything possible to get to this
    // index, including copy/paste of the whole class.
    // Con: I don't think that it is needed in many of those cases with a bit of thinking, and
    // without it it really makes the code more flexible including
  ...
  public final int getCurrentLinkIndex() {
    // Other then getPlanElementIndex, it seems that this one here has to be accessible, since
    // routes may contain loops, in which
  ...
  public Facility getDestinationFacility() {
    ...
    if (pe instanceof Leg) {
      ...
      // the above assumes alternating acts/legs.  I start having the suspicion that we should
      // revoke our decision to give that up.
