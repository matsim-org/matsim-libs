...
class VehicularDepartureHandler implements DepartureHandler {
  ...
  VehicularDepartureHandler(
      QNetsimEngineI qNetsimEngine, VehicleBehavior vehicleBehavior, QSimConfigGroup qsimConfig) {
    ...
    if (vehicle == null) {
      if (vehicleBehavior == VehicleBehavior.teleport) {
        ...
        if (vehicle == null) {
          ...
          final String msg =
              "could not find requested vehicle "
                  + vehicleId
                  + " in simulation for agent "
                  + agent
                  + " with id "
                  + agent.getId()
                  + " on link "
          ...
          log.error(msg);
          log.error(
              "Note that, with AgentSource and if the agent starts on a leg, the "
        ...
        // (since the "teleportVehicle" does not physically move the vehicle, this is finally
        // achieved in the departure
        // logic.  kai, nov'11)
  ...
  private void teleportVehicleTo(QVehicle vehicle, Id<Link> linkId) {
    if (vehicle.getCurrentLink() != null) {
      ...
      QLinkI qlinkOld =
          (QLinkI) qNetsimEngine.getNetsimNetwork().getNetsimLink(vehicle.getCurrentLink().getId());
      QVehicle result = qlinkOld.removeParkedVehicle(vehicle.getId());
      if (result == null) {
        throw new RuntimeException(
            "Could not remove parked vehicle with id "
                + vehicle.getId()
                + " on the link id "
                //						+ linkId
                + vehicle.getCurrentLink().getId()
