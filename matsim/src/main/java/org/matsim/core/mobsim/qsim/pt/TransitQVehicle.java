...
public class TransitQVehicle extends QVehicleImpl implements TransitVehicle {
  ...
  public TransitQVehicle(final Vehicle basicVehicle) {
    ...
    // New default is that vehicle is created with capacity. Initial values are 1 seat for the
    // driver and no standing room {@link org.matsim.vehicles.VehicleCapacity} (sep 19, KMT)
    // PT does not count the driver, so only warn if there is indeed no real capacity (mar 21,
    // mrieser)
