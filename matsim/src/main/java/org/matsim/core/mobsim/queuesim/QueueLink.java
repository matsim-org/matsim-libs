...
class QueueLink implements VisLink, MatsimNetworkObject {
	...
	private void updateBufferCapacity() {
		...
		if (this.buffercap_accumulate < 1.0 && this.bufferIsEmpty()) {
			// (for some reason, the second condition cannot be this.hasBufferSpace(), although that would make more sense
			// (testRouteFactoryIntegration will fail with a non-terminating QueueSim). kai, jun'13)
	...
	class VisDataImpl implements VisData {
		...
		private void getVehiclePositionsEquil(
			...
			if (cnt > 0) {
				...
				for (QueueVehicle veh : QueueLink.this.buffer) {
					...
					AgentSnapshotInfo position = QueueLink.this.queueNetwork.getAgentSnapshotInfoFactory().createAgentSnapshotInfo(veh
		...
		private double positionVehiclesFromBuffer(
			...
			for (QueueVehicle veh : QueueLink.this.buffer) {
				...
				AgentSnapshotInfo position = QueueLink.this.queueNetwork.getAgentSnapshotInfoFactory().createAgentSnapshotInfo(veh.getDriver().getId(), 
						link, queueEnd, lane);
				position.setColorValueBetweenZeroAndOne(speed);
				position.setAgentState(AgentSnapshotInfo.AgentState.PERSON_DRIVING_CAR);
				positions.add(position);
