...
public class QNetwork implements NetsimNetwork {
  ...
  public void initialize(
      QNetsimEngineI simEngine, AgentCounter agentCounter, MobsimTimer simTimer) {
    this.simEngine = simEngine;
    this.queueNetworkFactory.initializeFactory(
        agentCounter, simTimer, simEngine.getNetsimInternalInterface());
    for (Node n : this.network.getNodes().values()) {
      this.nodes.put(n.getId(), this.queueNetworkFactory.createNetsimNode(n));
    }
    for (Link l : this.network.getLinks().values()) {
      final QLinkI qlink =
          this.queueNetworkFactory.createNetsimLink(l, this.nodes.get(l.getToNode().getId()));
