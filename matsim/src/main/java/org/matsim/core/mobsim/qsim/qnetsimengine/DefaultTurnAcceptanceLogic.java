...
/**
 * this class checks whether turning is physically possible, i.e. if the next link exists and if it
 * is connected to the current link
 *
 * @author kainagel
 */
public final class DefaultTurnAcceptanceLogic implements TurnAcceptanceLogic {
  ...
  @Override
  /**
   * We need qNetwork to get the next QLink, because the link lookup may lead to a NullPointer
   * otherwise
   */
  public AcceptTurn isAcceptingTurn(
    ...
    //			final String message = "The link with id " + nextLinkId + " does not allow the current
    // mode, which is " + veh.getDriver().getMode();
    //			throw new RuntimeException( message ) ;
    ////			log.warn(message );
    ////			return acceptTurn.ABORT ;
    //			// yyyy is rather nonsensical to get the mode from the driver, not from the vehicle.
    // However, this seems to be
    //			// how it currently works: network links are defined for modes, not for vehicle types.
    // kai, may'16
    //		}
