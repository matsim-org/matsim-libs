...
class Realm {
  ...
  private void addDelayedLink(HLink link, int until) {
    if (HermesConfigGroup.DEBUG_REALMS)
      log(
          secs,
          String.format(
              "link %d delayed until %d size %d peek agent %d",
              link.id(), until, link.queue().size(), link.queue().peek().id));
  ...
  private void advanceAgent(Agent agent) {
    if (HermesConfigGroup.DEBUG_REALMS) {
      ...
      log(
          secs,
          String.format(
              "agent %d finished %s (prev plan index is %d)",
              agent.id, Agent.toString(centry), agent.planIndex));
    }
    agent.planIndex++;
    if (HermesConfigGroup.DEBUG_REALMS) {
      ...
      log(
          secs,
          String.format(
              "agent %d starting %s (new plan index is %d)",
              agent.id, Agent.toString(nentry), agent.planIndex));
    }
  }
  ...
  protected boolean processAgentLink(Agent agent, long planentry, int currLinkId) {
    ...
    int traveltime =
        (HermesConfigGroup.LINK_ADVANCE_DELAY
            + (int) Math.round(Math.max(1, next.length() / Math.min(velocity, next.velocity()))));
  ...
  protected boolean processAgentWait(Agent agent, long planentry) {
    ...
    try {
      agent_stops.get(accessStop).get(lineid).add(agent);
    } catch (NullPointerException npe) {
      log.error(
          String.format(
              "Hermes NPE agent=%d routeNo=%d accessStop=%d lineid=%d",
              agent.id, routeNo, accessStop, lineid),
          npe);
    }
    return true;
  }
  ...
  public void run() throws Exception {
    ...
    while (secs != HermesConfigGroup.SIM_STEPS) {
      ...
      if (HermesConfigGroup.CONCURRENT_EVENT_PROCESSING
          && secs % 3600 == 0
          && sortedEvents.size() > 0) {
      ...
      for (; agent.eventsIndex <= agentId; agent.eventsIndex++) {
        ...
        if (HermesConfigGroup.DEBUG_REALMS)
          log(
              secs,
              String.format(
                  "agent %d setEventTime (eventsIndex=%d) %s",
                  agent.id, agent.eventsIndex, agentEvents.get(agent.eventsIndex).toString()));
  ...
  ArrayList<ArrayDeque<HLink>> delayedLinks() {
