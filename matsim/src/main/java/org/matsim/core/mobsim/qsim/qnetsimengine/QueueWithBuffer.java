 ...
 *
 * <ul>
 *   <li>In fast capacity update, the flows are not accumulated in every time step, rather updated
 *       only if an agent wants to enter the link or an agent is added to buffer. Improvement of
 *       15-20% in the computational performance is observed. amit feb'16 (I seem to recall that in
 *       the end that statement was not consistently correct. kai, feb'18)
 *   <li>Currently (feb'18), the design is such that (possibly time-dep) flowCap and nEffectiveLanes
 *       are "pushed" into the class, while freeSpeed is "pulled" from the class. This is an attempt
 *       to bridge the diverging design requirements, where flowCap and nEffectiveLanes may, in the
 *       multiple lanes implementation, vary by lane (for the same link), while speed uses {@link
 *       LinkSpeedCalculator}, which does not have the link freeSpeed as a parameter and thus is a
 *       "pull" method.
 * </ul>
 *
 * @author nagel
...
final class QueueWithBuffer implements QLaneI, SignalizeableItem {
  ...
  public final void addFromWait(final QVehicle veh) {
    ...
    // This only could happen for addFromWait(), because it can be called from outside
    // QueueWithBuffer
    if (flowcap_accumulate.getValue() <= 0.0
        && veh.getVehicle().getType().getPcuEquivalents()
            > context.qsimConfig.getPcuThresholdForFlowCapacityEasing()) {
  ...
  /**
   * Stores the accumulated fractional parts of the flow capacity. See also flowCapFraction.
   *
   * <p>I changed this into an internal class as a first step to look into acceleration (not having
   * to keep this link active until this has accumulated to one). There is no need to keep it this
   * way; it just seems to make it easier to keep track of changes. kai, sep'14
   */
  ...
  /**
   * the last time-step the front-most vehicle in the buffer was moved. Used for detecting
   * dead-locks.
   */
  private double bufferLastMovedTime = Double.NEGATIVE_INFINITY;

  /**
   * The list of vehicles that have not yet reached the end of the link according to the free travel
   * speed of the link
   */
  ...
  /**
   * Holds all vehicles that are ready to cross the outgoing intersection and their capacity
   * consumption. <br>
   ...
   * (2) update of the remaining flow capacity (where we account for all vehicles that are still in
   * the buffer since previous time steps) - see {@link
   * #subtractConsumptionOfVehiclesThatAreAlreadyInTheBuffer()} <br>
   */
  ...
  /**
   * I think that it would be good to get rid of the qLink backpointer. Instead keep a reduced
   * QLinkInternalInterface back pointer, and give access only to reduced number of methods (in
   * particular not the full Link information). kai, feb'18 This is now done with the {@link
   * AbstractQLink.QLinkInternalInterface}. kai, feb'18
   */
  ...
  private QueueWithBuffer(
      AbstractQLink.QLinkInternalInterface qlink,
      final VehicleQ<QVehicle> vehicleQueue,
      Id<Lane> laneId,
      double length,
    ...
    // the general idea is to give this object no longer access to "everything".  Objects get back
    // pointers (here qlink), but they
    // do not present the back pointer to the outside.  In consequence, this object can go up to
    // qlink, but not any further. kai, mar'16
    // Now I am even trying to get rid of the full qLink back pointer (since it allows, e.g., going
    // back to Link). kai, feb'18
    ...
    //			throw new IllegalStateException("Double.NaN is not a valid freespeed travel time for a
    // link. Please check the attributes length and freespeed!");
    //		}
  ...
  private void addToBuffer(final QVehicle veh) {
    // yy might make sense to just accumulate to "zero" and go into negative when something is used
    // up.
    // kai/mz/amit, mar'12

    double now = context.getSimTimer().getTimeOfDay();

    double flowConsumption =
        (lastBufferEntry == null)
            ? getFlowCapacityConsumptionInEquivalents(veh, null, null)
            : getFlowCapacityConsumptionInEquivalents(
                veh, lastBufferEntry.getKey(), now - lastBufferEntry.getValue());
    ...
    if (buffer.size() == 1) {
      ...
      // (if there is one vehicle in the buffer now, there were zero vehicles in the buffer before.
      // in consequence,
      // need to reset the lastMovedTime.  If, in contrast, there was already a vehicle in the
      // buffer before, we can
   ...
   * Also removes vehicles from lane if the vehicle arrived at its destination.
   */
    ...
    while ((veh = peekFromVehQueue()) != null) {
      ...
      if (driver instanceof TransitDriverAgent) {
        HandleTransitStopResult handleTransitStop =
            qLink.handleTransitStop(now, veh, (TransitDriverAgent) driver, this.qLink.getId());
  ...
  private void updateFastFlowAccumulation() {
    ...
    if (this.flowcap_accumulate.getTimeStep() < now
      ...
      double timeSteps =
          (now - flowcap_accumulate.getTimeStep()) / context.qsimConfig.getTimeStepSize();
      double accumulateFlowCap = timeSteps * flowCapacityPerTimeStep;
      double newFlowCap =
          Math.min(flowcap_accumulate.getValue() + accumulateFlowCap, remainingFlowCapThisTimeStep);
  ...
  private void updateSlowFlowAccumulation() {
    ...
    if (this.thisTimeStepGreen
      ...
      double newFlowCap =
          Math.min(
              flowcap_accumulate.getValue() + flowCapacityPerTimeStep,
  ...
  private void calculateFlowCapacity() {
    // the following is not looking at time because it simply assumes that the lookups are "now".
    // kai, feb'18
    ...
    flowCapacityPerTimeStep =
        unscaledFlowCapacity_s
            * context.qsimConfig.getTimeStepSize()
            * context.qsimConfig.getFlowCapFactor();
    ...
    switch (context.qsimConfig.getTrafficDynamics()) {
      ...
      case kinematicWaves:
        ...
        // yyyyyy this should possibly be getFreespeed(now). But if that's the case, then
        // maxFlowFromFdiag would
        // also have to be re-computed with each freespeed change. kai, feb'18

        final double maxFlowFromFdiag =
            (this.effectiveNumberOfLanes / context.effectiveCellSize)
                / (1. / (HOLE_SPEED_KM_H / 3.6) + 1 / this.qLink.getFreespeed());
        final double minimumNumberOfLanesFromFdiag =
            this.flowCapacityPerTimeStep
                * context.effectiveCellSize
                * (1. / (HOLE_SPEED_KM_H / 3.6) + 1 / this.qLink.getFreespeed());

        QSimConfigGroup.InflowCapacitySetting inflowCapacitySetting =
            context.qsimConfig.getInflowCapacitySetting();
        ...
        if (inflowCapacitySetting == QSimConfigGroup.InflowCapacitySetting.MAX_CAP_FOR_ONE_LANE) {
          ...
          this.maxInflowUsedInQsim =
              (1 / context.effectiveCellSize)
                  / (1. / (HOLE_SPEED_KM_H / 3.6) + 1 / this.qLink.getFreespeed());
          ...
          qLink
              ...
              .putAttribute(
                  "maxInflowUsedInQsim",
                  3600 * maxInflowUsedInQsim / context.qsimConfig.getTimeStepSize());
          ...
          if (maxFlowFromFdiag < flowCapacityPerTimeStep) { // warnings
            if (wrnCnt < 10) {
              wrnCnt++;
              log.warn(
                  "max flow from fdiag < flow cap in network file; linkId="
                      + qLink.getId()
                      + "; network file flow cap/h="
                      + 3600. * flowCapacityPerTimeStep / context.qsimConfig.getTimeStepSize()
                      + "; max flow from fdiag/h="
                      + 3600 * maxFlowFromFdiag / context.qsimConfig.getTimeStepSize());

              log.warn(
                  "number of lanes from fdiag > number of lanes in network file; linkId="
                      + qLink.getId()
                      + "; number of lanes in network file="
                      + this.effectiveNumberOfLanes
                      + "; number of lanes from fdiag="
                      + minimumNumberOfLanesFromFdiag);
            ...
            } else if (inflowCapacitySetting
                == QSimConfigGroup.InflowCapacitySetting.NR_OF_LANES_FROM_FDIAG) {
              if (wrnCnt < 10) {
                log.warn(
          ...
          if (inflowCapacitySetting == QSimConfigGroup.InflowCapacitySetting.INFLOW_FROM_FDIAG) {
            ...
            qLink
                .getLink()
                .getAttributes()
                .putAttribute("effectiveNumberOfLanesUsedInQsim", effectiveNumberOfLanesUsedInQsim);
          } else {
            throw new RuntimeException(
                "The approach " + inflowCapacitySetting.toString() + " is not implemented yet.");
          }
        }
        break;
      ...
      default:
        throw new RuntimeException(
            "The traffic dynamics "
                + context.qsimConfig.getTrafficDynamics()
                + " is not implemented yet.");
    }
    //		log.debug( "linkId=" + this.qLink.getLink().getId() + "; flowCapPerTimeStep=" +
    // flowCapacityPerTimeStep +
    //						   "; invFlowCapPerTimeStep=" + inverseFlowCapacityPerTimeStep + "; maxFlowFromFdiag=" +
    // maxFlowFromFdiag ) ;
  ...
  private void calculateStorageCapacity() {
    ...
    storageCapacity =
        this.length
            * this.effectiveNumberOfLanesUsedInQsim
            / context.effectiveCellSize
            * context.qsimConfig.getStorageCapFactor();
    //		storageCapacity = this.length * this.qLink.getLink().getNumberOfLanes(now) /
    ...
    // yyyyyy this should possibly be getFreespeed(now). But if that's the case, then storageCap
    // would
    // also have to be re-computed with each freespeed change. kai, feb'18
    if (Double.isNaN(freespeedTravelTime)) {
    ...
    // this assumes that vehicles have the flowEfficiencyFactor of 1.0; the actual flow can be
    // different
    double tempStorageCapacity =
        freespeedTravelTime * unscaledFlowCapacity_s * context.qsimConfig.getFlowCapFactor();
    // yy note: freespeedTravelTime may be Inf.  In this case, storageCapacity will also be set to
    // Inf.  This can still be
    ...
    if (storageCapacity < tempStorageCapacity) {
      if (QueueWithBuffer.spaceCapWarningCount <= 10) {
        log.warn(
            "Link "
                + this.id
                + " too small: enlarge storage capacity from: "
                + storageCapacity
                + " Vehicles to: "
                + tempStorageCapacity
    ...
    switch (context.qsimConfig.getTrafficDynamics()) {
      ...
      case withHoles:
      case kinematicWaves:
        //			final double minStorCapForHoles = 2. * flowCapacityPerTimeStep *
        // context.getSimTimer().getSimTimestepSize();
        final double freeSpeed =
            qLink.getFreespeed(); // yyyyyy not clear why this is not time-dep. kai, feb'18
        final double holeSpeed = HOLE_SPEED_KM_H / 3.6;
        final double minStorCapForHoles =
            length * flowCapacityPerTimeStep * (freeSpeed + holeSpeed) / freeSpeed / holeSpeed;
        //			final double minStorCapForHoles = 2.* length * flowCapacityPerTimeStep * (freeSpeed +
        // holeSpeed) / freeSpeed / holeSpeed ;
        ...
        // yyyyyy (not thought through for TS != 1sec!  (should use flow cap per second) kai,
        // apr'16)
        if (storageCapacity < minStorCapForHoles) {
          if (spaceCapWarningCount <= 10) {
            log.warn(
                "storage capacity not sufficient for holes; increasing from "
                    + storageCapacity
                    + " to "
                    + minStorCapForHoles);
            QueueWithBuffer.spaceCapWarningCount++;
          ...
          qLink
              .getLink()
              .getAttributes()
              .putAttribute("storageCapacityUsedInQsim", storageCapacity);
        }

        remainingHolesStorageCapacity = this.storageCapacity;
        // yyyy how is this.storageCapacity supposed to have a value here?  (It might just be zero,
        // and
  ...
  private double getBufferStorageCapacity() {
    return flowCapacityPerTimeStep; // this assumes that vehicles have the flowEfficiencyFactor of
    // 1.0
  }
  ...
  public final boolean doSimStep() {
    switch (context.qsimConfig.getTrafficDynamics()) {
      ...
      case withHoles:
        ...
        break;
      case kinematicWaves:
        this.accumulatedInflowCap =
            Math.min(accumulatedInflowCap + maxInflowUsedInQsim, maxInflowUsedInQsim);
        this.processArrivalOfHoles();
        break;
      default:
        throw new RuntimeException(
  ...
  public final void addFromUpstream(final QVehicle veh) {
    ...
    if (this.context.qsimConfig.isUseLanes()) {
      if (hasMoreThanOneLane()) {
        this.context
            .getEventsManager()
            .processEvent(new LaneEnterEvent(now, veh.getId(), this.qLink.getId(), this.getId()));
      }
    }
    ...
    if (context.qsimConfig.isSeepModeStorageFree()
        && context
            .qsimConfig
            .getSeepModes()
            .contains(veh.getVehicle().getType().getId().toString())) {
      // do nothing
    } else {
    ...
    //		double linkTravelTime = this.length / this.linkSpeedCalculator.getMaximumVelocity(veh,
    // qLink.getLink(), now);
    double linkTravelTime =
        this.length / this.qLink.getMaximumVelocityFromLinkSpeedCalculator(veh, now);
    linkTravelTime =
        context.qsimConfig.getTimeStepSize()
            * Math.floor(linkTravelTime / context.qsimConfig.getTimeStepSize());

    veh.setEarliestLinkExitTime(now + linkTravelTime);

    // In theory, one could do something like
    //		final double discretizedEarliestLinkExitTime = timeStepSize *
    // Math.ceil(veh.getEarliestLinkExitTime()/timeStepSize);
    //		double effectiveEntryTime = now - ( discretizedEarliestLinkExitTime -
    // veh.getEarliestLinkExitTime() ) ;
    //		double earliestExitTime = effectiveEntryTime + linkTravelTime;
    // We decided against this since this would effectively move the simulation to operating on true
    // floating point time steps.  For example,
    // events could then have arbitrary floating point values (assuming one would use the
    // "effectiveEntryTime" also for the event).
      ...
      case kinematicWaves:
        ...
        double flowConsumption =
            (lastQueueEntry == null)
                ? getFlowCapacityConsumptionInEquivalents(veh, null, null)
                : getFlowCapacityConsumptionInEquivalents(
                    veh, lastQueueEntry.getKey(), now - lastQueueEntry.getValue());
    ...
    if (context.qsimConfig.isUsingFastCapacityUpdate()) {
      return (!this.vehQueue.isEmpty())
          || (!this.isNotOfferingVehicle()
              && context.qsimConfig
                  .isUseLanes()) // if lanes, the buffer needs to be active in order to move
          // vehicles over an internal node
          || (!this.holes.isEmpty());
    } else {
      return (this.flowcap_accumulate.getValue()
              < flowCapacityPerTimeStep) // still accumulating, thus active
  ...
  public void recalcTimeVariantAttributes() {
    ...
    //		log.debug("just entered recalcTimeVariantAttributes; now=" +
  ...
  //		if (Double.isNaN(freespeedTravelTime)) {
  //			throw new IllegalStateException("Double.NaN is not a valid freespeed travel time for a link.
  // Please check the attributes length and freespeed!");
  //		}
  //	}
  ...
  public final QVehicle getVehicle(final Id<Vehicle> vehicleId) {
    ...
    for (Pair<QVehicle, Double> vehEfficiencyPair : this.buffer) {
      if (vehEfficiencyPair.getKey().getId().equals(vehicleId)) return vehEfficiencyPair.getKey();
    }
    return null;
  }
  ...
  public final QVehicle popFirstVehicle() {
    ...
    if (this.context.qsimConfig.isUseLanes()) {
      if (hasMoreThanOneLane()) {
        this.context
            .getEventsManager()
            .processEvent(new LaneLeaveEvent(now, veh.getId(), this.qLink.getId(), this.getId()));
      }
    }
    return veh;
  }
  ...
  private final QVehicle removeFirstVehicle() {
    ...
    bufferLastMovedTime =
        now; // just in case there is another vehicle in the buffer that is now the new front-most
    if (context.qsimConfig.isUsingFastCapacityUpdate()) {
  ...
  @Override
  public final void setSignalStateForTurningMove(
      final SignalGroupState state, final Id<Link> toLinkId) {
    if (!qLink.getToNode().getOutLinks().containsKey(toLinkId)) {
      throw new IllegalArgumentException(
          "ToLink " + toLinkId + " is not reachable from QLink Id " + this.id);
    }
    qSignalizedItem.setSignalStateForTurningMove(state, toLinkId);

    thisTimeStepGreen = qSignalizedItem.hasGreenForAllToLinks();
    // (this is only for capacity accumulation.  As soon as at least one turning relation is green,
  ...
  public final void clearVehicles() {
    // yyyyyy right now it seems to me that one should rather just abort the agents and have the
    // framework take care of the rest. kai, mar'16
    ...
    for (QVehicle veh : vehQueue) {
      context
          .getEventsManager()
          .processEvent(new VehicleAbortsEvent(now, veh.getId(), veh.getCurrentLink().getId()));
      context
          .getEventsManager()
          .processEvent(
  ...
  private double getFlowCapacityConsumptionInEquivalents(
      QVehicle vehicle, QVehicle prevVehicle, Double timeDiff) {
    double flowEfficiency =
        flowEfficiencyCalculator.calculateFlowEfficiency(
            vehicle, prevVehicle, timeDiff, qLink.getLink(), id);
  ...
  private boolean hasMoreThanOneLane() {
    ...
    // this works independent from sorting since if there is only one lane, then it has to be the
    // one to be returned by
    // getOfferingQLanes().get(0), and it is also the same as the accepting QLane.  If, however,
    // "lanes" is used,
    // there are at least two lanes in sequence, so the accepting lane is never the same as any of
    // the offering lanes, and
  ...
  class VisDataImpl implements QLaneI.VisData {
    ...
    @Override
    public final Collection<AgentSnapshotInfo> addAgentSnapshotInfo(
  ...
  static final class Builder implements LaneFactory {
    ...
    public QueueWithBuffer createLane(AbstractQLink qLink) {
      ...
      return new QueueWithBuffer(
          qLink.getInternalInterface(),
          vehicleQueue,
          id,
          length,
