...
/**
 * Calculates the positions of all vehicles on this link according to the queue-logic: Vehicles are
 * placed on the link according to the ratio between the free-travel time and the time the vehicles
 * are already on the link. If they could have left the link already (based on the time), the
 * vehicles start to build a traffic-jam (queue) at the end of the link.
 *
 * @author dgrether
 * @author nagel
 */
class QueueAgentSnapshotInfoBuilder extends AbstractAgentSnapshotInfoBuilder {
  ...
  double calculateVehicleSpacing(
      double curvedLength, double overallStorageCapacity, Collection<? extends VisVehicle> vehs) {
    ...
    return Math.min(
        ...
        curvedLength
            / sum // the link may be more than ``full'' because of forward squeezing of stuck
        // vehicles
        );
  }
  ...
  double calculateOdometerDistanceFromFromNode(
    ...
    // This is a bit weird if prevVehiclesDistance is NAN, this is the first vehicle. I guess this
    // is a little uncommon
    // in Java, but this is how I found things.
    // Place a virtual vehicle at the end of the link + spacing so this vehicle can queue at the
    // link's end
    if (Double.isNaN(prevVehiclesDistance)) {
      prevVehiclesDistance = curvedLength + spacing;
    }
    // if the freeflow position is further along the link as the prev vehicle, this vehicle has to
    // queue behind the
    // previous.
