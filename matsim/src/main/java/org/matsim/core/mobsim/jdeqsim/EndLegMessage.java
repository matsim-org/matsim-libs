...
public class EndLegMessage extends EventMessage {
  ...
  public EndLegMessage(
      final Scheduler scheduler,
      final Vehicle vehicle,
      final TimeInterpretation timeInterpretation) {
    // need the time interpretation info here.  Attaching it to the message feels weird.  The
    // scheduler seems a pure simulation object.
    ...
    if ((actsLegs.size() > this.vehicle.getLegIndex())) {
      ...
      double departureTime =
          Math.max(
              now,
              timeInterpretation
                  .decideOnActivityEndTime(currentAct, now)
  ...
  public void processEvent() {
    ...
    if (vehicle.getCurrentLeg().getMode().equals(TransportMode.car)
        && (vehicle.getCurrentLinkRoute() != null && vehicle.getCurrentLinkRoute().length != 0)) {
      event =
          new LinkEnterEvent(
              this.getMessageArrivalTime(),
              Id.create(
                  vehicle.getOwnerPerson().getId().toString(), org.matsim.vehicles.Vehicle.class),
    ...
    event =
        new VehicleLeavesTrafficEvent(
            ...
            this.vehicle.getCurrentLinkId(),
            vehicleId,
            this.vehicle.getCurrentLeg().getMode(),
            1.0);
    eventsManager.processEvent(event);
    ...
    double actStartEventTime =
        nextAct.getStartTime().isUndefined()
            ? this.getMessageArrivalTime()
    ...
    event =
        new ActivityStartEvent(
            ...
            nextAct.getType(),
            nextAct.getCoord());
    // mobsim needs to know where activity takes place.  jdeqsim does not have access/egress legs;
    // thus using act.getCoord() seems justified.
