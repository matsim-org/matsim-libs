...
/**
 * Includes all agents that have transportation modes unknown to the NetsimEngine (often all !=
 * "car") or have two activities on the same link
 */
public final class DefaultTeleportationEngine implements TeleportationEngine {
              ...
              if (ret == 0) {
                ret =
                    o2.getSecond()
                        ...
                        .compareTo(
                            o1.getSecond()
                                .getId()); // if they're equal, compare the Ids: the one with the
                // larger Id should be first
              }
              return ret;
            }
          });
  ...
  public DefaultTeleportationEngine(
      Scenario scenario, EventsManager eventsManager, boolean withTravelTimeCheck) {
  ...
  public boolean handleDeparture(double now, MobsimAgent agent, Id<Link> linkId) {
    if (agent.getExpectedTravelTime().isUndefined()) {
      ...
      throw new RuntimeException(
          "teleportation does not work when travel time is undefined.  There is also really no magic fix for this,"
      ...
      if (entry.getFirst() <= now) {
        ...
        this.eventsManager.processEvent(
            new TeleportationArrivalEvent(
                now, personAgent.getId(), distance, personAgent.getMode()));
  ...
  private static Double travelTimeCheck(
      Double travelTime, Double speed, Facility dpfac, Facility arfac) {
    if (speed == null) {
    ...
    if (dpfac.getCoord() == null || arfac.getCoord() == null) {
      // yy this is for example the case if activities are initialized at links, without
      // coordinates.  Could use the link coordinate
