...
/**
 * Continuous queue model like transit stop handler using either serial or parallel doors operation
 * mode.
 *
 * @author aneumann
 */
public class ComplexTransitStopHandler implements TransitStopHandler {
  ...
  private static final Logger log = LogManager.getLogger(ComplexTransitStopHandler.class);
  ...
  public double handleTransitStop(
    ...
    if (this.doorOperationMode == VehicleType.DoorOperationMode.parallel) {
      ...
      log.info(
          "Unimplemented door operation mode "
              + this.doorOperationMode
              + " set. Using parralel mode as default.");
      return handleParallelStop(stop, now, leavingPassengers, enteringPassengers, handler, vehicle);
    }
  }

  private double handleSerialStop(
      TransitStopFacility stop,
      double now,
      List<PTPassengerAgent> leavingPassengers,
      List<PTPassengerAgent> enteringPassengers,
        ...
        if (cntEgress > 0) {
          ...
          if (this.passengersLeavingTimeFraction < 1.0) {
            ...
            while (this.passengersLeavingTimeFraction < 1.0) {
              ...
              if (handler.handlePassengerLeaving(
                  leavingPassengers.get(0), vehicle, stop.getLinkId(), now)) {
          ...
          if (cntAccess > 0) {
            ...
            if (this.passengersEnteringTimeFraction < 1.0) {
              ...
              while (this.passengersEnteringTimeFraction < 1.0) {
                ...
                if (handler.handlePassengerEntering(
                    enteringPassengers.get(0), vehicle, stop.getId(), now)) {
  ...
  private double handleParallelStop(
      TransitStopFacility stop,
      double now,
      List<PTPassengerAgent> leavingPassengers,
      List<PTPassengerAgent> enteringPassengers,
