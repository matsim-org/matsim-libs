...
public interface FlowEfficiencyCalculator {
   ...
   * @param previousQVehicle the vehicle previously traveling on this link before {@code qVehicle},
   *     may be null
   * @param timeGapToPreviousVeh time gap to the previous vehicle traveling on this {@code link}.
   *     May be null if {@code qVehicle} is the first to travel on {@code link}.
   * @param link the link the {@code qVehicle} is currently on.
   * @param laneId id of the lane the {@code qVehicle} is traveling on
   */
  double calculateFlowEfficiency(
      QVehicle qVehicle,
