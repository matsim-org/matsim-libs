...
/**
 * It becomes only clear quite indirectly from the material below: An easy way to configure the QSim
 * is
 *
 * <pre>
 ...
 * </pre>
 *
 * The bindings that can be overridden are defined in the default modules referenced near the end of
 * this class.
 *
 * <p>Some people say that one should try to stay away from <i>overriding</i> modules, and rather
 * just set them (leading to an error message if one tries to bind the same thing twice). MATSim has
 * a different history, and thus the global level at this point only allows to override. At the QSim
 * level, there is also
 *
 * <pre>
...
public final class QSimModule extends AbstractModule {
  ...
  public void install() {
    ...
    // this is essentially a structured collection of strings that determines which QSimModules will
    // later be used.  Can be
    // used to configure from config, but maybe easiest just ignore when getting started with this.
    // The default QSimModules
    ...
    bind(Key.get(new TypeLiteral<List<AbstractQSimModule>>() {}, Names.named("overrides")))
        .toInstance(Collections.emptyList());
    // this initializes (I think) the container with the overriding bindings with an empty
    // container.  Recall that "Key" and
    ...
    // I think that the result of this that you can use @Inject Collection<AbstractQSimModule> in
    // addition to @Inject
