...
public final class QLinkImpl extends AbstractQLink implements SignalizeableItem {
  ...
  private static final Logger log = LogManager.getLogger(QLinkImpl.class);
  ...
  public static final class Builder {
    ...
    QLinkImpl build(Link link, QNodeI toNode) {
      ...
      return new QLinkImpl(
          link, toNode, laneFactory, context, netsimEngine, linkSpeedCalculator, vehicleHandler);
    }
  }
  ...
  public interface LaneFactory {
    ...
    /**
     * If this QLinkImpl is passed an instance of this factory upon construction, it will call back
     * this factory within the constructor (!) to obtain a road and pass itself to the creation
     * method.
     */
  ...
  private QLinkImpl(
    ...
    this.visdata =
        this
        .new VisDataImpl(); // instantiating this here and not earlier so we can cache some things
  ...
  public boolean doSimStep() {
    ...
    // That is, do NOT pass the deactivation of the link rather implicitly via returning a false
    // here.
    // kai, mar'16
  }
  ...
  public void recalcTimeVariantAttributes() {
    ...
    //		qlane.changeSpeedMetersPerSecond( getLink().getFreespeed(now) ) ; flowCap & nLanes are
    // "push", freeSpeed is
    // "pull".  This is not completely honest w.r.t. freeSpeed consequences to the fdiag, but also
    // wasn't correctly
  ...
  class VisDataImpl implements VisData {
    ...
    private VisDataImpl() {
      ...
      if (nodeOffset != 0.0) {
        ...
        visLink =
            visModelBuilder.createVisLinkLanes(transformation, QLinkImpl.this, nodeOffset, null);
    ...
    public Collection<AgentSnapshotInfo> addAgentSnapshotInfo(
      ...
      // qnetwork.simEngine.getAgentSnapshotInfoBuilder();

      QLaneI.VisData roadVisData = getAcceptingQLane().getVisData();
      if (visLink != null) {
        ((QueueWithBuffer.VisDataImpl) roadVisData)
            .setVisInfo(visLink.getLinkStartCoord(), visLink.getLinkEndCoord());
        // yyyy not so great but an elegant solution needs more thinking about visualizer structure.
        // kai, jun'13
      }
      ...
      int cnt2 = 10; // a counter according to which non-moving items can be "spread out" in the
      // visualization
      ...
      cnt2 =
          context.snapshotInfoBuilder.positionVehiclesFromWaitingList(
              positions, QLinkImpl.this.getLink(), cnt2, QLinkImpl.this.getWaitingList());

      cnt2 =
          context.snapshotInfoBuilder.positionAgentsInActivities(
              positions,
              QLinkImpl.this.getLink(),
              QLinkImpl.this.getAdditionalAgentsOnLink(),
              cnt2);
