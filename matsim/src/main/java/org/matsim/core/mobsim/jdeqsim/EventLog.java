...
/**
 * This class is used for comparing results with the historical C++ implementation results of
 * DEQSim.
 *
...
public class EventLog {
  ...
  /**
   * the time must be the same (compared up to 4 digits after the floating point) and the link the
   * event type is ignored for the moment, because in the beginning it might be different
   *
   * @param eventLog1
   * @param eventLog2
   * @return
  ...
  /**
   * For each link in the event list, find out how long a car has been on that link. Then compare
   * the usage time of each link for the two different Event logs. Print the average(absolute
   * difference): absSumLink and the sum (absolute difference) in seconds: absAverageLinkDiff
   *
   * @param eventLog1
   * @param eventLog2
   * @return
   */
  public static double absAverageLinkDiff(
      ArrayList<EventLog> eventLog1, ArrayList<EventLog> eventLog2) {
    ...
    assert eventLog1.size() == eventLog2.size()
        : "The size of both eventLogs must be the same!"
            + eventLog1.size()
            + " - "
            + eventLog2.size();
  ...
  public static void filterEvents(
      int linkId, ArrayList<EventLog> eventLog1, ArrayList<EventLog> eventLog2) {
  ...
  /**
   * Get the travel time of one person. This means the time, between starting each leg end its end.
   *
   * @param eventLog1
   * @param vehicleId
   * @return
