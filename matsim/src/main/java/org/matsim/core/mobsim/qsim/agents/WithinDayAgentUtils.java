...
/**
 * This class is an attempt to provide access to the internals of PersonDriverAgentImpl in a way
 * that it can be used for within-day replanning.
 *
 * <p>Moreover, it is an attempt to replace ExperimentalBasicWithindayAgent which extends
 * PersonDriverAgentImpl. This has become a problem since other MATSim modules (such as PT which
 * uses TransitAgents) also extend that class. Instead, this class re-implements the functionality
 * of the PlanBasedWithinDayAgent Interface by accessing the package protected methods from
 * PlanBasedWithinDayAgent (this is possible since it is located in the same package).
 *
 * <p>Up to now, all the methods get a MobsimAgent. Depending on which class implements the
 * interface, the methods will perform their task or throw an exception if the given object does not
 * support that operation. At the moment, only PersonDriverAgentImpl are supported but UmlaufDrivers
 * should also be supported in the future (since UmlaufDriver does not extend PersonDriverAgentImpl
 * the method signatures have been changed). <i>The class is experimental. Use at your own risk, and
 * expect even less support than with other pieces of matsim.</i>
 *
 * @author cdobler
 */
public final class WithinDayAgentUtils {
  ...
  public static Integer getCurrentPlanElementIndex(MobsimAgent agent) {
    ...
    if (agent instanceof PlanAgent) {
      return ((PlanAgent) agent)
          .getCurrentPlan()
          .getPlanElements()
          .indexOf(((PlanAgent) agent).getCurrentPlanElement());
    } else {
  ...
  /**
   * NOTES: () The current link index does not point to where the agent is, but one ahead. () It
   * does that even if there is nothing there in the underlying list. I keep forgetting the
   * convention, but I think that the arrival link is not in the list, and so for the arrival link
   * special treatment is necessary. () Routes may have loops, in which case the "indexOf" approach
   * does not work.
   */
  public static Integer getCurrentRouteLinkIdIndex(MobsimAgent agent) {
    ...
    if (agent instanceof HasModifiablePlan) {
      ...
      //					throw new RuntimeException("agent currently not on network route; asking for link id
      ...
      //				}
      //
      //				// and in the end it points even one further (always points to the _next_ entry, if
      // there is any)
      //				index++ ;
      //
      //				return index ;
  ...
  /**
   * Only the PlanElements are changed - further Steps like updating the Routes of the previous and
   * next Leg have to be done elsewhere.
   *
   * @author cdobler
   */
  public static boolean replaceLegBlindly(Plan plan, Leg oldLeg, Leg newLeg) {
    ...
    // yyyy I can't say how safe this is.  There is no guarantee that the same entry is not used
    // twice in the plan.  This will in
    // particular be a problem if we override the "equals" contract, in the sense that two legs are
    // equal if
   ...
   * @author cdobler
   */
  public static boolean replaceActivityBlindly(
      Plan plan, Activity oldActivity, Activity newActivity) {
    ...
    // yyyy I can't say how safe this is.  There is no guarantee that the same entry is not used
    // twice in the plan.  This will in
    // particular be a problem if we override the "equals" contract, in the sense that two
    // activities are equal if
    // certain (or all) elements are equal.  kai, oct'10
