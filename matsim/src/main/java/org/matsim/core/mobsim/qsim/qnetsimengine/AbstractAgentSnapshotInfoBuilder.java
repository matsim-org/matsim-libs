...
abstract class AbstractAgentSnapshotInfoBuilder {
  ...
  private static double computeHolePositionAndReturnDistance(
      double freespeedTraveltime, Hole hole, double now, double curvedLength) {
    double remainingTravelTime = hole.getEarliestLinkExitTime() - now;
    return remainingTravelTime / freespeedTraveltime * curvedLength;
  }

  /**
   * Put the vehicles from the waiting list in positions. Their actual position doesn't matter,
   * PositionInfo provides a constructor for handling this situation.
   */
  public final int positionVehiclesFromWaitingList(
  ...
  public final int positionAgentsInActivities(
      final Collection<AgentSnapshotInfo> positions,
      Link link,
      Collection<? extends MobsimAgent> agentsInActivities,
      int cnt2) {
  ...
  public final int positionVehiclesFromTransitStop(
      final Collection<AgentSnapshotInfo> positions,
      Link link,
      Queue<QVehicle> transitVehicleStopQueue,
      int cnt2) {
    return positionStack(positions, transitVehicleStopQueue, cnt2);
  }

  public final void positionAgentGivenDistanceFromFNode(
      final Collection<AgentSnapshotInfo> positions,
      Coord startCoord,
      Coord endCoord,
      double lengthOfCurve,
      QVehicle veh,
      double distanceFromFromNode,
      int lane,
      double speedValueBetweenZeroAndOne) {
    // I think that the main reason why this exists as public method is that
    // AssignmentEmulatingQLane wants to use it directly.
    // The reason for this, in return, is that positionVehiclesAlongLine(...) is a service method
    // for queue models only.  kai, apr'16
  ...
  public final Collection<AgentSnapshotInfo> positionVehiclesAlongLine(
      ...
      double now,
      Collection<? extends MobsimVehicle> vehs,
      double curvedLength,
      ...
      case kinematicWaves:
        if (!holes.isEmpty()) {
          ...
          for (Hole hole : holes) {
            ...
            distanceOfHoleFromFromNode =
                computeHolePositionAndReturnDistance(ttimeOfHoles, hole, now, curvedLength);
            ...
            // since hole is already created even if vehicle is in buffer, thus excluding such holes
            // in vehicle position updating
            // probably, don't create hole in visualizer also. amit May 2016
            // }

            if (QSimConfigGroup.SnapshotStyle.withHolesAndShowHoles
                == scenario.getConfig().qsim().getSnapshotStyle()) {
              addHolePosition(
                  positions,
                  distanceOfHoleFromFromNode,
                  hole,
    ...
    for (MobsimVehicle mveh : vehs) {
      ...
      distanceFromFromNode =
          this.calculateOdometerDistanceFromFromNode(
              now,
              curvedLength,
              min(freeSpeed, veh.getMaximumVelocity()),
              vehicleSpacing,
              distanceFromFromNode,
              remainingTravelTime);
      // yyyy if the LinkSpeedCalculator says something that is not free speed, we are out of luck
      // here.  kai, jan'23

      int lane = VisUtils.guessLane(veh, numberOfLanesAsInt);
      double speedValue =
          VisUtils.calcSpeedValueBetweenZeroAndOne(veh, inverseFlowCapPerTS, now, freeSpeed);
      Gbl.assertNotNull(upstreamCoord);
      Gbl.assertNotNull(downstreamCoord);
      this.positionAgentGivenDistanceFromFNode(
          positions,
          upstreamCoord,
          downstreamCoord,
          curvedLength,
      ...
      switch (this.scenario.getConfig().qsim().getTrafficDynamics()) {
        ...
        default:
          throw new RuntimeException(
              "The traffic dynmics "
                  + this.scenario.getConfig().qsim().getTrafficDynamics()
                  + " is not implemented yet.");
      }
    }
  ...
  abstract double calculateVehicleSpacing(
      double linkLength, double overallStorageCapacity, Collection<? extends VisVehicle> vehs);

  abstract double calculateOdometerDistanceFromFromNode(
      double time,
      double linkLength,
      double freespeed,
      double spacing,
  ...
  private void positionPassengers(
      Collection<AgentSnapshotInfo> positions,
      Collection<? extends PassengerAgent> passengers,
  ...
  private AgentState getAgentState(Identifiable<Person> identifiable) {
    ...
    // I don't know whether I have gotten this right, but I think this is tested in every case in
    // every method
    ...
    if (identifiable instanceof MobsimDriverAgent
        && ((MobsimDriverAgent) identifiable).getMode().equals(TransportMode.car))
