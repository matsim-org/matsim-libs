...
final class Hermes implements Mobsim {
      ...
      if (agent != null && !agent.finished() && !agent.isTransitVehicle()) {
        ...
        eventsManager.processEvent(
            new PersonStuckEvent(
                HermesConfigGroup.SIM_STEPS,
                Id.get(matsim_id, Person.class),
                Id.createLinkId("0"),
                "zero"));
      }
    }
  ...
  public void run() {
    ...
    try {
      ...
      importScenario();
      log.info(
          String.format("Hermes importing scenario took %d ms", System.currentTimeMillis() - time));

      eventsManager.initProcessing();

      time = System.currentTimeMillis();
      realm.run();
      log.info(String.format("Hermes took %d ms", System.currentTimeMillis() - time));
