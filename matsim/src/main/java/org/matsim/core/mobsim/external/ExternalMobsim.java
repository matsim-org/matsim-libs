...
public class ExternalMobsim implements Mobsim {
  ...
  private static final Logger log = LogManager.getLogger(ExternalMobsim.class);
  ...
  protected void init() {
    ...
    this.simConfig =
        ConfigUtils.addOrGetModule(
            ...
            ExternalMobimConfigGroup.GROUP_NAME,
            ExternalMobimConfigGroup.class);
  ...
  @Override
  public void run() {
    String iterationPlansFile =
        this.controlerIO.getIterationFilename(this.iterationNumber, this.plansFileName);
    String iterationEventsFile =
        this.controlerIO.getIterationFilename(this.iterationNumber, this.eventsFileName);
    String iterationConfigFile =
        this.controlerIO.getIterationFilename(this.iterationNumber, configFileName);
  ...
  protected void writeConfig(
      final String iterationPlansFile,
      final String iterationEventsFile,
      final String iterationConfigFile)
  ...
  protected void writePlans(final String iterationPlansFile) {
    ...
    log.warn(
        "I don't know if this works; was changed after the streaming api changed, and never tested after that.  Pls let us know. kai, jul'16");
    ...
    //		AbstractPopulationWriterHandler handler = new
    // PopulationWriterHandlerImplV4(this.scenario.getNetwork());
    ...
    //				 *
    //				 * yy could as well copy only the selected plans to a new population.  That would be
    // closer to our
    //				 * programming style over the recent years (sacrifice performance for cleaner code at
  ...
  //	@SuppressWarnings("unused") /* do now show warnings that this method does not throw any
  // exceptions,
  //	as classes inheriting from this class may throw exceptions in their implementation of this
  // method. */
  protected void runExe(final String iterationConfigFile)
      throws FileNotFoundException, IOException {
    String cmd = this.executable + " " + iterationConfigFile;
    log.info("running command: " + cmd);
    Gbl.printMemoryUsage();
    String logfileName =
        this.controlerIO.getIterationFilename(this.getIterationNumber(), "mobsim.log");
    int timeout = this.simConfig.getExternalTimeOut();
    int exitcode = ExeRunner.run(cmd, logfileName, timeout);
    if (exitcode != 0) {
      throw new RuntimeException(
          "There was a problem running the external mobsim. exit code: " + exitcode);
    }
  }
  ...
  //	as classes inheriting from this class may throw exceptions in their implementation of this
  // method. */
  protected void readEvents(final String iterationEventsFile)
      throws FileNotFoundException, IOException {
