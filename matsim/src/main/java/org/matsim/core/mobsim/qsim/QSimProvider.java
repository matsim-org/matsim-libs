...
public class QSimProvider implements Provider<QSim> {
  ...
  QSimProvider(
      ...
      @Named("overridesFromAbstractModule")
  ...
  public QSim get() {
    ...
    AbstractQSimModule qsimModule =
        AbstractQSimModule.overrideQSimModules(modules, Collections.emptyList());

    for (AbstractQSimModule override : overridingModulesFromAbstractModule) {
      qsimModule =
          AbstractQSimModule.overrideQSimModules(
              Collections.singleton(qsimModule), Collections.singletonList(override));
    }
    ...
    for (Object activeComponent : components.getActiveComponents()) {
      ...
      if (activeComponent instanceof Annotation) {
        activeComponentKey =
            Key.get(
                new TypeLiteral<Collection<Provider<QSimComponent>>>() {},
  ...
  /**
   * Historically, some bindings that are used in the QSim were defined in the outer controller
   * scope. This method checks that those bindings are now registered in the QSim scope.
   */
