...
class ActivityEngineDefaultImpl implements ActivityEngine {
  ...
  /**
   * Agents cannot be added directly to the activityEndsList since that would not be thread-safe
   * when within-day replanning is used. There, an agent's activity end time can be modified. As a
   * result, the agent is located at the wrong position in the activityEndsList until it is updated
   * by using rescheduleActivityEnd(...). However, if another agent is added to the list in the mean
   * time, it might be inserted at the wrong position. cdobler, apr'12
   */
  ...
  /**
   * This list needs to be a "blocking" queue since this is needed for thread-safety in the parallel
   * qsim. cdobler, oct'10
   */
  private final Queue<AgentEntry> activityEndsList =
      new PriorityBlockingQueue<>(
          ...
          (e0, e1) -> {
            ...
            if (cmp == 0) {
              ...
              //
              // yy We are not sure what the above comment line is supposed to say.  Presumably, it
              // is supposed
              // to say that the agent with the larger ID should be "smaller" one in the comparison.
              // In practice, it seems
              // that something like "emob_9" is before "emob_8", and something like "emob_10"
              // before "emob_1".
  ...
  public void afterSim() {
    ...
    for (AgentEntry entry : activityEndsList) {
      if (entry.activityEndTime != Double.POSITIVE_INFINITY) {
        // since we are at an activity, it is not plausible to assume that the agents know mode or
        // destination
        // link id.  Thus generating the event with ``null'' in the corresponding entries.  kai,
        // mar'12
  ...
  /**
   * This method is called by QSim to pass in agents which then "live" in the activity layer until
   * they are handed out again through the internalInterface.
   *
   * <p>It is called not before onPrepareSim() and not after afterSim(), but it may be called
   * before, after, or from doSimStep(), and even from itself (i.e. it must be reentrant), since
   * internalInterface.arrangeNextAgentState() may trigger the next Activity.
   */
  @Override
  public boolean handleActivity(MobsimAgent agent) {
    if (agent.getActivityEndTime() == Double.POSITIVE_INFINITY) {
    ...
    } else if (agent.getActivityEndTime()
      ...
      agent.endActivityAndComputeNextState(
          internalInterface.getMobsim().getSimTimer().getTimeOfDay());
      internalInterface.arrangeNextAgentState(agent);
    } else {
    ...
    // - If this class has never had a doSimStep() when this method is called, this means that this
    // Agent is having its
    // "overnight", i.e. first Activity.
    // - This means that this ActivityEngine should not just pass this MobsimAgent along if its
    // activityEndTime has already ended,
    // but queue it in with other such Agents and let them all leave on doSimStep(), because we
    // expect those Agents to leave
    // in the order specified by the activityQueue (no matter if it is a good order or not, see
    // comment there). The order in which new Agents enter
    // the simulation and are passed into this method is a different one, so this matters.
    // - This is safe (Agents will not miss a second), simply because doSimStep for this time step
    // has not yet happened.
    // - It also means that e.g. OTFVis will probably display all Agents while they are in their
    // first Activity before you press play.
    // - On the other hand, agents whose first activity is also their last activity go right to
    // sleep "inside" this engine.
    return true;
  }

  /**
   * For within-day replanning. Tells this engine that the activityEndTime the agent reports may
   * have changed since the agent was added to this engine through handleActivity. May be merged
   * with handleActivity, since this engine can know by itself if it was called the first time or
   * not.
   *
  ...
  public void rescheduleActivityEnd(final MobsimAgent agent) {
    ...
    // The intention in the following is that an agent that is no longer alive has an activity end
    // time of infinity.  The number of
    // alive agents is only modified when an activity end time is changed between a finite time and
    // infinite.  kai, jun'11
    if (oldEntry == null) {
      if (newActivityEndTime == Double.POSITIVE_INFINITY) {
        ...
        internalInterface.registerAdditionalAgentOnLink(agent);
        ((org.matsim.core.mobsim.qsim.AgentCounter) internalInterface.getMobsim().getAgentCounter())
            .incLiving();
      }
