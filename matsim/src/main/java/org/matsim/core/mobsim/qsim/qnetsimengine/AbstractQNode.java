...
/**
 * {@link QNodeI} is the interface; this is an abstract class that contains implementation of
 * non-traffic related "infrastructure", primarily (de)activation.
 */
abstract class AbstractQNode implements QNodeI {
  ...
  /** The ParallelQSim replaces the activator with the QSimEngineRunner that handles this node. */
  /*package*/ void setNetElementActivationRegistry(NetElementActivationRegistry activator) {
    // yyyy I cannot say if this needs to be in QNodeI or not.  The mechanics of this are tricky to
    // implement, so it would
  ...
  /**
   * This method is called from QueueWithBuffer.addToBuffer(...) which is triggered at some placed,
   * but always initially by a QLink's doSomStep(...) method. I.e. QNodes are only activated while
   * moveNodes(...) is performed. However, multiple threads could try to activate the same node at a
   * time, therefore this has to be thread-safe. cdobler, sep'14
   */
