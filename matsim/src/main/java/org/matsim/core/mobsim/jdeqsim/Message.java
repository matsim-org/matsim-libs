...
public abstract class Message implements Comparable<Message> {
  ...
  // yyyy we always seem to have "processEvent()" immediately followed by "handleMessage()", and it
  // is not clear to me why we have both.  kai, feb'19
  // I think that the idea is that in "processEvent()" the normal MATSim event is generated and
  // given to the eventsManager, while in handleMessage, everything else is done.

  /**
   * The comparison is done according to the message arrival Time. If the time is equal of two
   * messages, then the priority of the messages is compared
   */
  @Override
  ...
  //		return sendingUnit;
  //	}
  // this method is only used once, as part of a test case, where it is only used to retreive the
  // scheduler.  Which should, however, be the same if we retreive it from the
  ...
  public void setReceivingUnit(SimUnit receivingUnit) {
    // the receiving unit seems to be the object that one needs when handling the message.  I don't
    // find this totally clear since maybe one would need two objects (such
    // as when they collide) or even more?  Then one needs to somehow find the other objects,
    // indicating that one could find the first object through those methods as
    // well.  kai, feb'18
