 ...
 * @author dstrippgen
 */
abstract class AbstractQNetsimEngine<A extends AbstractQNetsimEngineRunner>
    implements QNetsimEngineI {

  private NetsimInternalInterface ii =
  ...
  AbstractQNetsimEngine(final QSim sim, QNetworkFactory netsimNetworkFactory) {
    ...
    if (netsimNetworkFactory != null) {
      ...
      //			final DefaultQNetworkFactory netsimNetworkFactory2 = new DefaultQNetworkFactory( events,
      // scenario );
  ...
  static AbstractAgentSnapshotInfoBuilder createAgentSnapshotInfoBuilder(
      Scenario scenario, SnapshotLinkWidthCalculator linkWidthCalculator) {
    ...
    switch (snapshotStyle) {
      ...
      case withHolesAndShowHoles:
        // the difference is not in the spacing, thus cannot be differentiated by using different
        // classes.  kai, sep'14
        // ??? kai, nov'15
        return new QueueAgentSnapshotInfoBuilder(scenario, linkWidthCalculator);
      case kinematicWaves:
  ...
  public final void onPrepareSim() {
    this.infoTime =
        Math.floor(internalInterface.getMobsim().getSimTimer().getSimStartTime() / INFO_PERIOD)
            * INFO_PERIOD;
    /*
  ...
  /**
   * called during {@link #doSimStep(double)}. Should perform the simstep-logic in {@link
   * AbstractQNetsimEngineRunner} provided by {@link #getQnetsimEngineRunner()}
   *
   * @param time
   */
  ...
  /**
   * create all necessary {@link AbstractQNetsimEngineRunner}. Will be called during {@link
   * #onPrepareSim()}.
   *
  ...
  public final void addParkedVehicle(MobsimVehicle veh, Id<Link> startLinkId) {
    ...
    if (qlink == null) {
      throw new RuntimeException(
          "requested link with id="
              + startLinkId
              + " does not exist in network. Possible vehicles "
  ...
  public final void registerAdditionalAgentOnLink(final MobsimAgent planAgent) {
    ...
    if (linkId != null) { // may be bushwacking
      ...
      if (qLink == null) {
        throw new RuntimeException(
            "netsim link lookup failed; agentId=" + planAgent.getId() + "; linkId=" + linkId);
      }
  ...
  public final MobsimAgent unregisterAdditionalAgentOnLink(Id<Person> agentId, Id<Link> linkId) {
    if (linkId
        == null) { // seems that this can happen in tests; not sure if it can happen in regular
      // code. kai, jun'15
      return null;
    }
    QLinkI qLink = this.qNetwork.getNetsimLink(linkId);
  ...
  private void letVehicleArrive(QVehicle veh) {
    ...
    this.qsim
        .getEventsManager()
        .processEvent(new PersonLeavesVehicleEvent(now, driver.getId(), veh.getId()));
  ...
  private void assignNetElementActivators() {
    ...
    for (QNodeI node : qNetwork.getNetsimNodes().values()) {
      ...
      for (Link outLink : node.getNode().getOutLinks().values()) {
        ...
        // removing qsim as "person in the middle".  not fully sure if this is the same in the
        // parallel impl.  kai, oct'10
    ...
    for (int i = 0; i < this.engines.size(); i++) {
      log.info(
          "Assigned " + nodes[i] + " nodes and " + links[i] + " links to QSimEngineRunner #" + i);
    }
