...
/**
 * Changes to another plan if that plan is better. Probability to change depends on score
 * difference.
 *
...
public final class ExpBetaPlanChanger<T extends BasicPlan, I> implements PlanSelector<T, I> {
  ...
  /**
   * Changes to another plan with a probability proportional to exp( Delta scores ). Need to think
   * through if this goes to Nash Equilibrium or to SUE !!!
   */
  @Override
  public T selectPlan(final HasPlansAndId<T, I> person) {
    ...
    if ((currentPlan.getScore() == null) || (otherPlan.getScore() == null)) {
      ...
      if (currentPlan.getScore() != null && otherPlan.getScore() == null) {
        if (scoreWrnFlag) {
          log.error(
              "yyyyyy not switching to other plan although it needs to be explored.  "
    ...
    if (MatsimRandom.getRandom().nextDouble()
        < 0.01 * weight) { // as of now, 0.01 is hardcoded (proba to change when both
      // scores are the same)
      return otherPlan;
    }
    return currentPlan;
