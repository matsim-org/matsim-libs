...
public class GenericPlanStrategyImpl<T extends BasicPlan, I> implements GenericPlanStrategy<T, I> {
  ...
  private static final Logger log = LogManager.getLogger(GenericPlanStrategyImpl.class);
  ...
  public void run(final HasPlansAndId<T, I> person) {
    ...
    if (plan != null) {
      ...
      log.error(
          planSelector + " returned no plan: not changing selected plan for person " + person);
    }
  ...
  public void finish() {
    ...
    this.plans.clear();
    log.info(
        "Plan-Strategy finished, " + this.counter + " plans handled. Strategy: " + this.toString());
    this.counter = 0;
  }
