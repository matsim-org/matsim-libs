...
/**
 * Basic wrapper for any call to external "plans-to-plans" modules. As basic handling of such
 * modules is alike for every module: 1.) Write a plans header 2.) dump every person with the
 * selected plan only 3.) close plans file and write a config file based on a config template file
 * 4.) Execute the external program with this config file 5.) Mutate original plans according to
 * re-read plans
 *
 * @author dstrippgen
 * @author mrieser
...
public class ExternalModule implements PlanStrategyModule {
  ...
  public ExternalModule(
    ...
    this.exeRunnerDelegate =
        new ExeRunnerDelegate() {
          @Override
          public boolean invoke() {
            String cmd = exePath + " " + outFileRoot + modulePrefix + ExternalConfigFileName;
            String logfilename =
                controler.getIterationFilename(currentIteration, modulePrefix + "stdout.log");
  ...
  ExternalModule(
      ExeRunnerDelegate exeRunnerDelegate,
      final String moduleId,
      final OutputDirectoryHierarchy controler,
      final Scenario scenario) {
    this.modulePrefix = moduleId + "_";
  ...
  public void handlePlan(final Plan plan) {
    ...
    // I also need to memorize the plans which are passed here, because I am supposed to mutate
    // them.

    final Person personWithOnlySelectedPlan =
        this.exportPopulation.getFactory().createPerson(plan.getPerson().getId());
  ...
  private void prepareExternalExeConfig() {
    ...
    extConfig.setParam(
        SCENARIO,
        SCENARIO_INPUT_PLANS_FILENAME,
        this.outFileRoot + "/" + this.modulePrefix + ExternalInFileName);
    extConfig.setParam(
        SCENARIO,
        SCENARIO_WORKING_PLANS_FILENAME,
        this.outFileRoot + "/" + this.modulePrefix + ExternalOutFileName);
    extConfig.setParam(
        SCENARIO,
        SCENARIO_WORKING_EVENTS_TXT_FILENAME,
        this.controler.getIterationFilename(this.currentIteration - 1, "events.txt"));
