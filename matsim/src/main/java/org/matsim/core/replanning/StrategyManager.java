...
public class StrategyManager implements MatsimManager {
  ...
  // * The PlansReplanning interface is the one that is bound as ReplanningListener, and it calls
  // StrategyManager.
  // * On the other hand, StrategyManager is also independently bound.
  // * This has the consequence that users (possibly via the config) may configure StrategyManager,
  // but it is effectively ignored if the bound
  // implementation of ReplanningListener does not use it.
  // * This is the design that I found.  Not sure if I should change it, but I need something for
  // Carrier and later for LSP.
  // * A possible way out would be to unbind StrategyManager (for someone who implements a
  // ReplanningListener that does not use
  // StrategyManager).  This is not immediately possible, but the internet gives possibilities.
  // Could be added as a utility method, which would
  // (I think) make sense together with the MATSim overall design which overrides defaults instead
  // of asking the user to plug everything together.

  // Not sure if that would work: There is other code that tries to obtain StrategyManager via
  // injection, and all of this would then fail.
  ...
  @Inject
  StrategyManager(
      ReplanningConfigGroup replanningConfigGroup,
      ControllerConfigGroup controllerConfigGroup,
      StrategyChooser<Plan, Person> strategyChooser,
    ...
    int globalInnovationDisableAfter =
        (int)
            ((controllerConfigGroup.getLastIteration() - controllerConfigGroup.getFirstIteration())
                    * replanningConfigGroup.getFractionOfIterationsToDisableInnovation()
                + controllerConfigGroup.getFirstIteration());
    log.info("global innovation switch off after iteration: " + globalInnovationDisableAfter);

    for (Map.Entry<ReplanningConfigGroup.StrategySettings, PlanStrategy> entry :
        planStrategies.entrySet()) {
  ...
  /**
   * Adds a strategy to this manager with the specified weight. This weight compared to the sum of
   * weights of all strategies in this manager defines the probability this strategy will be used
   * for an agent.
   */
  public final void addStrategy(
      final PlanStrategy strategy, final String subpopulation, final double weight) {
   ...
   * @return true if the strategy was successfully removed from this manager, false if the strategy
   *     was not part of this manager and could thus not be removed.
   */
   ...
   *
   * @return true if the strategy is part of this manager and the weight could be changed
  ...
  /**
   * Randomly chooses for each person of the population a strategy and uses that strategy on the
   * person, after adapting the strategies to any pending change requests for the specified
   * iteration.
   *
   * @param iteration the current iteration we're handling
   */
  public final void run(
      final Population population, final int iteration, final ReplanningContext replanningContext) {
  ...
  public GenericPlanStrategy<Plan, Person> chooseStrategy(
      final Person person, final String subpopulation, ReplanningContext replanningContext) {
    return delegate.chooseStrategy(person, subpopulation, replanningContext);
  }

  /**
   * Sets the maximal number of plans an agent can memorize. Setting maxPlansPerAgent to zero means
   * unlimited memory (only limited by RAM). Agents can have up to maxPlansPerAgent plans plus one
   * additional one with the currently modified plan they're trying out.
   */
  ...
  /**
   * Schedules a command for a later iteration. The change will take place before the strategies are
   * applied.
   */
  ...
  /**
   * Sets a plan selector to be used for choosing plans for removal, if they have more plans than
   * the specified maximum. This defaults to {@link WorstPlanForRemovalSelector}.
   *
   * <p>Thoughts about using the logit-type selectors with negative logit model scale parameter:
   *
   * <ul>
   ...
   *       <blockquote>
   *       <i> df(i)/dt = - p(i) * f(i) + 1/n </i>
   *       </blockquote>
   *       where <i>p(i)</i> is from the choice model.
   *   <li>Steady state solution (<i>df/dt=0</i>) <i> f(i) = 1/n * 1/p(i) </i>.
   *   <li>If <i> p(i) = e<sup>-b*U(i)</sup></i>, then <i> f(i) = e<sup>b*U(i)</sup> / n </i>. Or in
   *       words: <i><b> If you use a logit model with a minus in front of the beta for plans
   *       removal, the resulting steady state distribution is the same logit model with normal
   *       beta.</b></i>
   * </ul>
   *
   * The implication seems to be: divide the user-configured beta by two, use one half for choice
   * and the other for plans removal.
   *
   * <p>The path size version still needs to be tested (both for choice and for plans removal).
   *
  ...
  /**
   * @return the weights of the strategies for the given subpopulation, in the same order as the
   *     strategies returned by {@link #getStrategies(java.lang.String)}
   */
