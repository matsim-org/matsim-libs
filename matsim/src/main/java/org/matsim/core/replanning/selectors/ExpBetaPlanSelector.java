...
public class ExpBetaPlanSelector<T extends BasicPlan, I> implements PlanSelector<T, I> {
  ...
  protected double calcPlanWeight(final T plan, final double maxScore) {
    // NOTE: The deduction of "maxScore" from all scores is a numerical trick.  It ensures that the
    // values of exp(...)
    // are in some normal range, instead of close to numerical infinity.  The latter leads to
   ...
   */
  public static <T extends BasicPlan, I> double getSelectionProbability(
      ExpBetaPlanSelector<T, I> expBetaPlanSelector, HasPlansAndId<T, ?> person, final T plan) {
