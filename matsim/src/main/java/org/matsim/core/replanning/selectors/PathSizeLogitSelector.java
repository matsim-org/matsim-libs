...
/**
 * select an existing Plan according to the Path Size Logit (e.g. Frejinger, E. and Bierlaire, M.:
 * Capturing Correlation with subnetworks in route choice models, Transportation Research Part B
 * (2006), doi:10.1016/j.trb.2006.06.003.) If there are unscored plans one of it will be chosen
 * randomly (optimistic strategy).
 *
 * @author laemmel
 */
public final class PathSizeLogitSelector extends AbstractPlanSelector {
  ...
  public PathSizeLogitSelector(
      final double pathSizeLogitExponent, final double logitScaleFactor, final Network network) {
  ...
  protected Map<Plan, Double> calcWeights(final List<? extends Plan> plans) {
    ...
    // (yyyy is it obvious that no two plans can have the same hash code?  what happens if they do?
    // kai, oct'12)
    ...
    // dg, 09-2013: as first and last link are equal for all routes between to activities this is no
    // major issue
    ...
    for (Plan plan : plans) {
        ...
        if (pe instanceof Leg) {
          ...
          for (Id<Link> linkId : route.getLinkIds()) {
            ...
            for (double dbl : linksInTime.get(linkId)) {
              // TODO this is just for testing (those legs where the departure time differs more
              // then 3600 seconds will not compared to each other) - need a
              ...
              if (Math.abs(dbl - currentTime) <= 3600) denominator++;
            }
            // (the meaning seems to be: for each link that the plan uses, it checks how many other
            // times the
      ...
      // tmp is now a number that contains the ``reduced'' travel distance of the plan.  Divide it
      // by the full travel distance
      ...
      if (Double.isInfinite(maxScore)) {
        ...
        // likely that wc.maxScore == -Infinity, and thus plan.getScoreAsPrimitiveType() also ==
        // -Infinity, handle it like any other case where getScore() == maxScore
        // I do not understand the line above.  kai, oct'12

        weight = PSi;
        // (yy I do not understand.  Presumably, wc.maxScore may be -Infinity, in which case ALL
        // plan scores are -Infinity
        // (or NaN or null or something similar).  In this case, plans are simply weighted by their
        // PSi, so that
        ...
        // (this is essentially $PSi * exp( tau * score )$, the "-wc.maxScore" is (presumably) the
        // computational trick
        // to avoid overflow)
      }
      ...
      // the weight is memorized; the sum of all weights in computed.  Choice will be based on those
      // weights
