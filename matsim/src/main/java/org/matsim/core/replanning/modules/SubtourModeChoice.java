...
/**
 * Changes the transportation mode of all legs of one randomly chosen subtour in a plan to a
 * randomly chosen different mode given a list of possible modes.
 *
 * <p>A subtour is a consecutive subset of a plan which starts and ends at the same link.
 *
 * <p>Certain modes are considered only if the choice would not require some resource to appear out
 * of thin air. For example, you can only drive your car back from work if you have previously
 * parked it there. These are called chain-based modes.
 *
 * <p>The assumption is that each chain-based mode requires one resource (car, bike, ...) and that
 * this resource is initially positioned at home. Home is the location of the first activity in the
 * plan.
 *
 * <p>If the plan initially violates this constraint, this module may (!) repair it.
 *
 * @author michaz
 */
public class SubtourModeChoice extends AbstractMultithreadedModule {
  ...
  public SubtourModeChoice(
    ...
    this(
        ...
        subtourModeChoiceConfigGroup.getCoordDistance());
