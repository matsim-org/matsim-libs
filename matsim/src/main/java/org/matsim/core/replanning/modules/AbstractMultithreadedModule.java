...
/**
 * An abstract strategy module for running multiple plan algorithms in parallel. Can be used to
 * easily generate multi-threaded strategy modules, as long as the real functionality of the
 * strategy module is handled by a PlanAlgorithm. Just overwrite getPlanAlgoInstance() to return an
 * instance of your plan algorithm.
 *
 * <p><code>initThreads()</code> creates the threads, but does not yet start them.
 *
 * <p><code>handlePlan(Plan)</code> distributes the plans equally to all threads.
 *
 * <p><code>finishReplanning()</code> finally starts the threads and waits for all threads to be
 * finished.
 *
 * <p>While this approach does not lead to optimal performance gains ("slow threads" vs. "fast
 * threads"), it helps building reproducible runs. Additionally, as the threads are only started
 * after all to-be-handled plans are added, we can use unsynchronized data structures.
 *
 * <p>Design comments/questions:
 *
 * <ul>
 *   <li>As a consequence of the design, the instances that getPlanAlgoInstance() returns, need to
 *       be thread-safe. kai, dec'12 For an example with discussions, see {@link
 *       tutorial.programming.multiThreadedPlanStrategy.RunWithMultithreadedModule}
 * </ul>
 *
 * @author mrieser
 */
public abstract class AbstractMultithreadedModule implements PlanStrategyModule {
  ...
  private static final Logger log = LogManager.getLogger(AbstractMultithreadedModule.class);

  /**
   * Design comments:
   *
   * <ul>
   *   <li>The way I understand this, the instances that this method returns need to be thread-safe
   *       (i.e. independent from each other). They can, for example, not rely on the same instance
   *       of the router. kai, dec'12
   * </ul>
   */
  ...
  protected void beforePrepareReplanningHook(
      @SuppressWarnings("unused") ReplanningContext replanningContextTmp) {
    // left empty for inheritance
  }

  protected void afterPrepareReplanningHook(
      @SuppressWarnings("unused") ReplanningContext replanningContextTmp) {
    // left empty for inheritance
  }
  ...
  private static final class ExceptionHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread t, Throwable e) {
      log.error(
          "Thread " + t.getName() + " died with exception. Will stop after all threads finished.",
          e);
      this.hadException.set(e);
    }
  }

  private static final class PlanAlgoThread implements Runnable {
