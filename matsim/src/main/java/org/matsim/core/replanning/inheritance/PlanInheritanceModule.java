 ...
 */
@Singleton
public class PlanInheritanceModule extends AbstractModule
  ...
  public void notifyStartup(StartupEvent event) {
    ...
    this.planInheritanceRecordWriter =
        new PlanInheritanceRecordWriter(
            event
                .getServices()
                .getControlerIO()
                .getOutputFilename(FILENAME_PLAN_INHERITANCE_RECORDS + ".csv", compressionType));
    this.strategies =
        this.getActiveStrategies(
            event.getServices().getConfig().replanning().getStrategySettings(),
            event.getServices().getStrategyManager());
    this.selectedPlanStrategyShareWriter =
        this.initializeDistributionWriter(
            this.strategies,
            event
                .getServices()
                .getControlerIO()
                .getOutputFilename(FILENAME_PLAN_INHERITANCE_RECORDS + "_shares_selected.csv"));
    this.planStrategyShareWriter =
        this.initializeDistributionWriter(
            this.strategies,
            event
                .getServices()
                .getControlerIO()
                .getOutputFilename(FILENAME_PLAN_INHERITANCE_RECORDS + "_shares.csv"));
  ...
  private ArrayList<String> getActiveStrategies(
      Collection<StrategySettings> strategySettings, StrategyManager strategyManager) {
    ...
    for (String subpopulation : activeSubpopulations) {
      for (GenericPlanStrategy<Plan, Person> planStrategy :
          strategyManager.getStrategies(subpopulation)) {
  ...
  private BufferedWriter initializeDistributionWriter(
      ArrayList<String> strategies, String filename) {
  ...
  public void notifyBeforeMobsim(BeforeMobsimEvent event) {
    // check the plans of the population and all currently stored plan records - do the actual
    // book-keeping
    ...
    for (Person person : event.getServices().getScenario().getPopulation().getPersons().values()) {
      for (Plan plan : person.getPlans()) {
        ...
        if (plan.getIterationCreated() == event.getIteration()) {
          ...
          planInheritanceRecord.setPlanId(
              Id.create(
                  Long.toString(++this.numberOfPlanInheritanceRecordsCreated, 36), Plan.class));
          planInheritanceRecord.setAncestorId(
              plan.getId()); // works because new plan is copy of old selected and attributes are
          // copied -> thus current attribute plan id is old selected plan id
          ...
          this.planId2planInheritanceRecords.put(
              planInheritanceRecord.getPlanId(), planInheritanceRecord);
        }

        if (PersonUtils.isSelected(plan)) {
          this.planId2planInheritanceRecords
              .get(plan.getId())
              .getIterationsSelected()
              .add(event.getIteration());
    ...
    for (Id<Plan> deletedPlanId : deletedPlans) {
      PlanInheritanceRecord deletedPlanInheritanceRecord =
          this.planId2planInheritanceRecords.remove(deletedPlanId);
    ...
    this.calculateAndWriteDistribution(
        event.getIteration(),
        this.strategies,
        this.planId2planInheritanceRecords,
        selectedPlanIds,
    ...
    this.calculateAndWriteDistribution(
        ...
        this.strategies,
        this.planId2planInheritanceRecords,
        this.planId2planInheritanceRecords.keySet(),
        this.planStrategyShareWriter);
  }

  /**
   * Updates the default plan stats - namely the distribution of plan mutators based on the given
   * plan ids.
   */
  private void calculateAndWriteDistribution(
      int currentIteration,
      ArrayList<String> strategies,
      Map<Id<Plan>, PlanInheritanceRecord> planId2planInheritanceRecords,
      Set<Id<Plan>> planIds,
  ...
  public void notifyShutdown(ShutdownEvent event) {
    ...
    for (PlanInheritanceRecord planInheritanceRecord :
        this.planId2planInheritanceRecords.values()) {
  ...
  @Override
  public void install() {
    if (getConfig().planInheritance().getEnabled())
