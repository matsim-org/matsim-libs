...
public class ReplanningAnnealer implements IterationStartsListener, StartupListener {
  ...
  private static boolean isInnovationStrategy(String strategyName) {
    List<String> selectors =
        Arrays.asList(
            ...
            DefaultSelector.ChangeExpBeta,
            DefaultSelector.KeepLastSelected,
            DefaultSelector.SelectExpBeta,
            DefaultSelector.SelectPathSizeLogit,
            DefaultSelector.SelectRandom,
            "selector",
            "expbeta");
    return !(selectors.contains(strategyName)
        || ((strategyName.toLowerCase().contains("selector")
                || strategyName.toLowerCase().contains("expbeta"))
  ...
  public void notifyStartup(StartupEvent event) {
    ...
    for (AnnealingVariable av : this.saConfig.getAnnealingVariables().values()) {
      if (!av.getAnnealType().equals(AnnealOption.disabled)) {
        ...
        if (av.getAnnealParameter().equals(AnnealParameterOption.globalInnovationRate)) {
          header.addAll(
              this.config.replanning().getStrategySettings().stream()
    ...
    try (BufferedWriter bw =
        IOUtils.getBufferedWriter(outputDirectoryHierarchy.getOutputFilename(ANNEAL_FILENAME))) {
    ...
    for (AnnealingVariable av : this.saConfig.getAnnealingVariables().values()) {
      if (this.currentIter > 0) {
        switch (av.getAnnealType()) {
          case geometric:
            this.currentValues.compute(av.getAnnealParameter(), (k, v) -> v * av.getShapeFactor());
            break;
          case exponential:
            int halfLifeIter =
                av.getHalfLife() <= 1.0
                    ? (int) (av.getHalfLife() * this.innovationStop)
                    : (int) av.getHalfLife();
            this.currentValues.compute(
                av.getAnnealParameter(),
                (k, v) -> av.getStartValue() / Math.exp((double) this.currentIter / halfLifeIter));
            break;
          case msa:
            this.currentValues.compute(
                av.getAnnealParameter(),
                (k, v) -> av.getStartValue() / Math.pow(this.currentIter, av.getShapeFactor()));
            break;
          case sigmoid:
            halfLifeIter =
                av.getHalfLife() <= 1.0
            ...
            this.currentValues.compute(
                ...
                (k, v) ->
                    av.getEndValue()
                        + (av.getStartValue() - av.getEndValue())
                            / (1
                                + Math.exp(
                                    av.getShapeFactor() * (this.currentIter - halfLifeIter))));
            break;
          case linear:
            double slope =
                (av.getStartValue() - av.getEndValue())
            ...
            this.currentValues.compute(
                av.getAnnealParameter(), (k, v) -> this.currentIter * slope + av.getStartValue());
            break;
          case disabled:
            return;
          default:
        ...
        log.info(
            "Annealling will be performed on parameter "
                + av.getAnnealParameter()
                + ". Value: "
                + this.currentValues.get(av.getAnnealParameter()));

        this.currentValues.compute(
            av.getAnnealParameter(), (k, v) -> Math.max(v, av.getEndValue()));
      }
      double annealValue = this.currentValues.get(av.getAnnealParameter());
      annealStats.put(
          av.getAnnealParameter().name(), String.format(Locale.US, "%.4f", annealValue));
  ...
  private void writeIterationstats(int currentIter, Map<String, String> annealStats) {
    try (BufferedWriter bw =
        IOUtils.getAppendingBufferedWriter(
            outputDirectoryHierarchy.getOutputFilename(ANNEAL_FILENAME))) {
  ...
  private void anneal(
      IterationStartsEvent event,
      AnnealingVariable av,
      double annealValue,
      Map<String, String> annealStats) {
    switch (av.getAnnealParameter()) {
      ...
      case globalInnovationRate:
        ...
        List<Double> annealValues =
            annealReplanning(
                annealValue,
                event.getServices().getStrategyManager(),
                av.getDefaultSubpopulation());
        int i = 0;
        for (ReplanningConfigGroup.StrategySettings ss :
            this.config.replanning().getStrategySettings()) {
          if (Objects.equals(ss.getSubpopulation(), av.getDefaultSubpopulation())) {
            annealStats.put(
                ss.getStrategyName(), String.format(Locale.US, "%.4f", annealValues.get(i)));
            i++;
          }
        }
        annealStats.put(
            av.getAnnealParameter().name(),
            String.format(
                Locale.US,
                "%.4f", // update value in case of switchoff
                getStrategyWeights(
                    event.getServices().getStrategyManager(),
                    av.getDefaultSubpopulation(),
                    StratType.allInnovation)));
        break;
      default:
        throw new IllegalArgumentException();
    }
  ...
  private List<Double> annealReplanning(
      double globalInnovationValue, StrategyManager stratMan, String subpopulation) {
    List<Double> annealValues = new ArrayList<>();
    double totalInnovationWeights =
        getStrategyWeights(stratMan, subpopulation, StratType.allInnovation);
    double totalSelectorWeights =
        getStrategyWeights(stratMan, subpopulation, StratType.allSelectors);
    ...
    for (GenericPlanStrategy<Plan, Person> strategy : strategies) {
      ...
      if (isInnovationStrategy(strategy.toString())) {
        weight =
            totalInnovationWeights > 0
                ? globalInnovationValue * weight / totalInnovationWeights
                : 0.0;
      } else {
        weight =
            totalSelectorWeights > 0
                ? (1 - globalInnovationValue) * weight / totalSelectorWeights
                : 0.0000001;
      }
  ...
  private double getStrategyWeights(
      StrategyManager stratMan, String subpopulation, StratType stratType) {
  ...
  private double getStrategyWeights(Config config, String subpopulation, StratType stratType) {
    ...
    Collection<ReplanningConfigGroup.StrategySettings> strategies =
  ...
  private int getInnovationStop(Config config) {
    ...
    for (ReplanningConfigGroup.StrategySettings strategy :
        config.replanning().getStrategySettings()) {
  ...
  private void checkAndFixStartValue(
      ReplanningAnnealerConfigGroup.AnnealingVariable av, StartupEvent event) {
    double configValue;
    switch (av.getAnnealParameter()) {
      case BrainExpBeta:
      ...
      case learningRate:
        ...
        break;
      case globalInnovationRate:
        double innovationWeights =
            getStrategyWeights(this.config, av.getDefaultSubpopulation(), StratType.allInnovation);
        double selectorWeights =
            getStrategyWeights(this.config, av.getDefaultSubpopulation(), StratType.allSelectors);
        if (innovationWeights + selectorWeights != 1.0) {
          log.warn("Initial sum of strategy weights different from 1.0. Rescaling.");
          double innovationStartValue =
              av.getStartValue() == null ? innovationWeights : av.getStartValue();
          rescaleStartupWeights(
              innovationStartValue,
              this.config,
              event.getServices().getStrategyManager(),
              av.getDefaultSubpopulation());
        }
  ...
  private void rescaleStartupWeights(
      double innovationStartValue, Config config, StrategyManager stratMan, String subpopulation) {
    ...
    for (GenericPlanStrategy<Plan, Person> strategy : strategies) {
      double weight = stratMan.getWeights(subpopulation).get(strategies.indexOf(strategy));
      if (isInnovationStrategy(strategy.toString())) {
        weight = innovationStartValue > 0 ? weight / innovationStartValue : 0.0;
      } else {
        weight = selectorStartValue > 0 ? weight / selectorStartValue : 0.0;
      }
      stratMan.changeWeightOfStrategy(strategy, subpopulation, weight);
    }
    // adapt also in config for the record
    Collection<ReplanningConfigGroup.StrategySettings> strategiesConfig =
        config.replanning().getStrategySettings();
