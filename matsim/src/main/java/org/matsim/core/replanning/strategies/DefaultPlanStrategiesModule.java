...
public class DefaultPlanStrategiesModule extends AbstractModule {
  ...
  public void install() {
    if (getConfig()
        ...
        .equals(DefaultPlansRemover.WorstPlanSelector.toString())) {
    ...
    if (getConfig()
        .replanning()
        .getPlanSelectorForRemoval()
        .equals(DefaultPlansRemover.SelectRandom.toString())) {
      bindPlanSelectorForRemoval().to(new TypeLiteral<RandomPlanSelector<Plan, Person>>() {});
    }
    if (getConfig()
        .replanning()
        .getPlanSelectorForRemoval()
        .equals(DefaultPlansRemover.SelectExpBetaForRemoval.toString())) {
      bindPlanSelectorForRemoval().toProvider(ExpBetaPlanSelectorForRemoval.class);
    }
    if (getConfig()
        .replanning()
        .getPlanSelectorForRemoval()
        .equals(DefaultPlansRemover.ChangeExpBetaForRemoval.toString())) {
      bindPlanSelectorForRemoval().toProvider(ExpBetaPlanChangerForRemoval.class);
    }
    if (getConfig()
        .replanning()
        .getPlanSelectorForRemoval()
        .equals(DefaultPlansRemover.PathSizeLogitSelectorForRemoval.toString())) {
    ...
    // If you bind your own strategy from your own module, and then don't reference it from the
    // Config,
    // that's fine: The StrategyManager will still only add those strategies to itself which are
    // configured.
    ...
    if (usedStrategyNames.contains(DefaultSelector.SelectPathSizeLogit)) {
      addPlanStrategyBinding(DefaultSelector.SelectPathSizeLogit)
          .toProvider(SelectPathSizeLogit.class);
    }

    // strategy packages that select, copy, and modify.  (The copying is done implicitly as soon as
    // "addStrategyModule" is called
    // at least once).
    ...
    if (usedStrategyNames.contains(DefaultStrategy.TimeAllocationMutator)) {
      addPlanStrategyBinding(DefaultStrategy.TimeAllocationMutator)
          .toProvider(TimeAllocationMutator.class);
    }
    if (usedStrategyNames.contains(DefaultStrategy.TimeAllocationMutator_ReRoute)) {
      addPlanStrategyBinding(DefaultStrategy.TimeAllocationMutator_ReRoute)
          .toProvider(TimeAllocationMutatorReRoute.class);
    }
    ...
    if (usedStrategyNames.contains(DefaultStrategy.ChangeSingleTripMode)) {
      addPlanStrategyBinding(DefaultStrategy.ChangeSingleTripMode)
          .toProvider(ChangeSingleTripMode.class);
    }

    // td, 15 feb 16: removed the "Leg" versions of strategies. Notify the users that they should
    // switch to the
    // "Trip" versions. Should be left in 0.8.XXX releases, and then deleted, along with their name
    // in the enum.
    if (usedStrategyNames.contains(DefaultStrategy.ChangeLegMode)) {
      log.error(
          DefaultStrategy.ChangeLegMode
              + " replanning strategy does not exist anymore. Please use "
              + DefaultStrategy.ChangeTripMode
              + " instead.");
    }
    if (usedStrategyNames.contains(DefaultStrategy.ChangeSingleLegMode)) {
      log.error(
          DefaultStrategy.ChangeSingleLegMode
              + " replanning strategy does not exist anymore. Please use "
              + DefaultStrategy.ChangeSingleTripMode
              + " instead.");
    }
    if (usedStrategyNames.contains(DefaultStrategy.TripSubtourModeChoice)) {
      log.error(
          DefaultStrategy.TripSubtourModeChoice
              + " replanning strategy does not exist anymore. Please use "
              + DefaultStrategy.SubtourModeChoice
              + " instead.");
    }
  }
  ...
  // yyyy Why are the following always implementing Providers of the full implementations, and not
  // just the interface
  ...
  private static class PathSizeLogitSelectorForRemoval implements Provider<PathSizeLogitSelector> {
    ...
    @Inject ScoringConfigGroup config;
