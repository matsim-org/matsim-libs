...
/**
 * This selector is used like the standard WorstPlanForRemovalSelector to reduce plans from an
 * agent's memory. However, adhering to the conflict resolution logic, the selector will make sure
 * that a non-conflicting plan is never selected for removal if it is the last remaining
 * non-conflicting one. This way, we make sure that every agent always keeps one plan that is not
 * conflicting.
 *
...
public class WorstPlanForRemovalSelectorWithConflicts implements PlanSelector<Plan, Person> {
  ...
  public Plan selectPlan(HasPlansAndId<Plan, Person> person) {
    ...
    if (nonConflictingCount == 1
        && !conflictManager.isPotentiallyConflicting(sorter.getFirst().getLeft())) {
