...
/**
 * <par>This class implements the density-based clustering approach as published by Zhou <i>et
 * al</i> (2004).</par>
 *
 * <ul>
 *   <i>``The basic idea of DJ-DigicoreCluster is as follows. For each point, calculate its
 *   <b>neighborhood</b>: the neighborhood consists of points within distance <b>Eps</b>, under
 *   condition that there are at least <b>MinPts</b> of them. If no such neighborhood is found, the
 *   point is labeled noise; otherwise, the points are created as a new DigicoreCluster if no
 *   neighbor is in an existing DigicoreCluster, or joined with an existing DigicoreCluster if any
 *   neighbour is in an existing DigicoreCluster.''</i>
 * </ul>
 *
 * <h4>Reference</h4>
 *
 * Zhou, C., Frankowski, D., Ludford, P.m Shekar, S. and Terveen, L. (2004). Discovering personal
 * gazeteers: An interactive clustering approach. <i> Proceedings of the 12th annual ACM
 * International workshop on Geographic Information Systems</i>, p. 266-273. Washington, DC.
 *
 * <h4></h4>
 *
 * @author jwjoubert
...
public class DensityCluster {
  ...
  private static final Logger log = LogManager.getLogger(DensityCluster.class);
   ...
   *
   * @param radius the radius of the search circle within which other activity points are searched.
   * @param minimumPoints the minimum number of points considered to constitute an independent
   *     {@link Cluster}.
  ...
  public DensityCluster(List<Node> nodesToCluster, boolean silent) {
    ...
    if (nullCounter > 0) {
      log.warn(
          "In DJCluster: of the " + inputPoints.size() + " points, " + nullCounter + " were null.");
    }
   ...
   * procedure of Zhou <i>et al</i> (2004) is followed. If there are no points to cluster, a warning
   * message is logged, and the procedure bypassed.
   */
  public void clusterInput(double radius, int minimumPoints) {
    if (this.inputPoints.size() == 0) {
      ...
      for (int i = 0; i < this.inputPoints.size(); i++) {
        ...
        ClusterActivity cp =
            new ClusterActivity(Id.create(i, Coord.class), inputPoints.get(i), null);
        quadTree.put(x, y, cp);
        listOfPoints.add(cp);
      }
      if (!silent) {
      ...
      while (pointCounter < listOfPoints.size()) {
        ...
        if (p.getCluster() == null) {
          ...
          Collection<ClusterActivity> neighbourhood =
              quadTree.getDisk(p.getCoord().getX(), p.getCoord().getY(), radius);
      ...
      Map<Cluster, List<ClusterActivity>> clusterMap =
          new HashMap<Cluster, List<ClusterActivity>>();
      ...
      for (ClusterActivity ca : listOfPoints) {
        ...
        if (!silent) {
          if (++cpCounter == cpMultiplier) {
            log.info(
                "   ClusterPoints processed: "
                    + cpCounter
                    + " ("
                    + String.format(
                        "%3.2f", ((double) cpCounter / (double) listOfPoints.size()) * 100)
                    + "%)");
            cpMultiplier *= 2;
          }
        }
          ...
          if (++clusterCounter == clusterMultiplier) {
            log.info(
                "   Clusters processed: "
                    ...
                    + String.format(
                        "%3.2f",
                        ((double) clusterCounter / (double) clusterMap.keySet().size()) * 100)
                    + "%)");
            clusterMultiplier *= 2;
          }
        }
  ...
  /**
   * For each DigicoreCluster, this method writes out the DigicoreCluster id, the DigicoreCluster's
   * center of gravity (as a longitude and latitude value), and the order of the DigicoreCluster,
   * i.e. the number of activity points from the input data associated with the DigicoreCluster.
   * Output is a comma-separated flat file, by default, but the delimiter can be set using the class
   * method <code>setDelimiter(String string)</code>.
   *
   * <h5>File format:</h5>
   *
   * <ul>
