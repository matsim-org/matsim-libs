...
/**
 * This algorithm handles double links (two links with same from and to node) by splitting one of
 * the link and adding an additional node. This is necessary, since the routes in MATSim are node
 * based.
 *
 * @author laemmel
 */
public final class NetworkSegmentDoubleLinks implements NetworkRunnable {
  ...
  public void run(final Network network) {
    ...
    log.info(
        "\t"
            + this.trblLinks
            + " trblLinks."); // what does trbl stand for?? please document! TODO [GL] documentation
  ...
  private void splitLink(Link link) {
    ...
    Link tmpLink =
        this.network.getFactory().createLink(link.getId(), link.getFromNode(), medianNode);
