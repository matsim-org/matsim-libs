...
/**
 * Class to generate the concave hull from a set of given points. The algorithm is based on the
 * paper by <br>
 * <br>
 * &nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp Duckham et al. (2008). Efficient generation of
 * simple polygons for characterizing the shape of a set of points in the plane. <i>Pattern
 * Recognition</i>, <b>41</b>, 3224-3236. <br>
 * <br>
 * This code also benefits from insight gained from the code from Eric Grosso, but we've adapted
 * (and some cases simplified and improved) for specific application, originally, to the South
 * African application using <i>Digicore</i> commercial vehicle activity chain data.
 *
 * @author jwjoubert
 */
public class ConcaveHull {
  private static final Logger LOG = LogManager.getLogger(ConcaveHull.class);
  ...
  /**
   * Constructor for the concave hull algorithm. Be default, not output will be written for each
   * iteration of the algorithm's execution. If you require control of the output, rather use the
   * alternative constructor {@link #ConcaveHull(GeometryCollection, double, boolean)}
   *
   * @param points the {@link GeometryCollection} of input points for which the concave hull will be
   *     computed. In the South African application the will typically be {@link Point}s, and not
   *     {@link Coordinate}s as the latter is <i>not</i> a {@link Geometry}.
   * @param threshold the edge length threshold used by the concave hull algorithm. Only border
   *     edges with a length <i>longer</i> than the threshold will be considered for removal.
   */
   ...
   * @param threshold the edge length threshold used by the concave hull algorithm. Only border
   *     edges with a length <i>longer</i> than the threshold will be considered for removal.
   * @param printIterations boolean choice indicating if output (triangles and borders) will be
   *     written to the output folder, or not. This only needs to be true when small-scale single
   *     instances are run, typically for illustration purposes.
   */
  public ConcaveHull(GeometryCollection points, double threshold, boolean printIterations) {
    ...
    if (this.printIterations) {
      ...
      String filename =
          String.format("output/concaveHull/Threshold_%.0f_triangles.csv", this.threshold);
  ...
  /**
   * Returns a {@link Geometry} that represents the concave hull of the input geometry according to
   * the threshold. The returned geometry contains the minimal number of points needed to represent
   * the concave hull. No facility identifier is required. If you require to identify a facility,
   * for example for debugging purposes, rather use the method {@link #getConcaveHull(String)}.
   *
   * @return if the concave hull contains 3 or more points, a {@link Polygon}; 2 points, a {@link
   ...
   * the concave hull.
   *
   * @param facilityIdentifier a {@link String} that uniquely identifies the group of points for
   *     which the hull is created. For example, this can be the facility Id, and is useful for
   *     debugging, especially when a Delaunay triangulation cannot be determined for a facility.
   * @return if the concave hull contains 3 or more points, a {@link Polygon}; 2 points, a {@link
   *     LineString}; 1 point, a {@link Point}; 0 points, an empty {@link GeometryCollection}.
   */
  ...
  /**
   * Just returns the number of (filtered) input points. This method is most likely only going to be
  ...
  /**
   * Reads a flat file containing only the coordinates of points. The first column is assumed to be
   * the longitude, and the second the latitude.
   *
   * <p>NOTE: It is assumed that the coordinate reference system is projected, as distance
   ...
   *   <li>all edges that remain in the triangulation are written to another file.
   * </ul>
   *
  ...
  /**
   * Implementation of the concave hull algorithm. Every iteration, starting with the zero-th
   * iteration, will be written to file. Both the triangles, and the border will be written to file.
   *
   * @param args two arguments required, and in the following order:
   *     <ol>
   *       <li><b>coordinate file</b> the absolute path of the coordinates to be be read in. It is
   *           assumed that the first column of the CSV file will be the longitude, and the second
   *           column the latitude. Also, the coordinate file is assumed to have a header line.
   *       <li><b>threshold</b> the edge length threshold that will be used for the concave hull
   *           algorithm.
   */
