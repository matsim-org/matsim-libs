...
public class IntersectionSimplifier {
  private static final Logger LOG = LogManager.getLogger(IntersectionSimplifier.class);
  ...
  public Network simplify(Network network) {
    ...
    for (Link link : network.getLinks().values()) {
      ...
      Node fromNode =
          NetworkUtils.createNode(link.getFromNode().getId(), link.getFromNode().getCoord());
      ...
      if (newLink.getFromNode().getCoord().equals(newLink.getToNode().getCoord())) {
        ...
        if (!newNetwork.getNodes().containsKey(newLink.getFromNode().getId())) {
          ...
          NetworkUtils.createAndAddNode(
              newNetwork, newLink.getFromNode().getId(), newLink.getFromNode().getCoord());
        }
        if (!newNetwork.getNodes().containsKey(newLink.getToNode().getId())) {
          NetworkUtils.createAndAddNode(
              newNetwork, newLink.getToNode().getId(), newLink.getToNode().getCoord());
        }
  ...
  /**
   * Checking that clustering has been done, and then writes the clusters to file using {@link
   * DensityCluster#writeClustersToFile(String)}.
   *
   * @param file
   */
