...
final class NetworkReaderMatsimV2 extends MatsimXmlParser {
  ...
  private static final Logger log = LogManager.getLogger(NetworkReaderMatsimV2.class);

  NetworkReaderMatsimV2(final String inputCRS, final String targetCRS, final Network network) {
    ...
    switch (name) {
      case ATTRIBUTES:
        if (context.peek().equals(NETWORK)) {
          String inputCRS =
              (String) network.getAttributes().getAttribute(ProjectionUtils.INPUT_CRS_ATT);
  ...
  private void startLinks(final Attributes atts) {
    ...
    if (capperiod != null) {
      ...
      log.warn(
          "capperiod was not defined. Using default value of "
              + Time.writeTime(capacityPeriod)
              + ".");
    }
    ...
    if ((atts.getValue("capPeriod") != null)
        || (atts.getValue("capDivider") != null)
        || (atts.getValue("capdivider") != null)) {
      log.warn(
  ...
  private void startNode(final Attributes atts) {
    final Node node =
        this.network
            .getFactory()
            .createNode(Id.create(atts.getValue("id"), Node.class), parseCoord(atts));
  ...
  private void startLink(final Attributes atts) {
    ...
    Link l =
        this.network
            .getFactory()
            .createLink(Id.create(atts.getValue("id"), Link.class), fromNode, toNode);
