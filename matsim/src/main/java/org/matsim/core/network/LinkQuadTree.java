 ...
 *
 * <p>"The MX-CIF quadtree of Kedem (1981) (see also Abel and Smith 1983) is a region-based
 * representation where each rectangle is associated with the quadtree node corresponding to the
 * smallest block which contains it in its entirety. Subdivision ceases whenever a node's block
 * contains no rectangles. Alternatively, subdivision can also cease once a quadtree block is
 * smaller than a predetermined threshold size." (H. Samet, Hierarchical spatial data structures,
 * www.cs.umd.edu/~hjs/pubs/SametSSD89.pdf) After having looked at the code for 5 min, I cannot,
 * however, tell if this is exactly what the code is doing, and if so, which version. kai, jul'12
 *
 * <p>The implementation splits nodes whenever a node contains more than one link of which at least
 * one can be assigned to a child node. According to the description above, the leaf nodes would
 * always be empty. This implementation splits nodes whenever it is necessary, but not before,
 * leading to leaf nodes that actually may contain elements. marcel, jul'12
 *
...
public final class LinkQuadTree {
  ...
  private static class QuadTreeNode {
    ...
    // I replaced the "int" by an enum since I find it easier to read, and I needed/wanted to
    // understand the code.  If this causes
    ...
    public void put(final LinkWrapper w) {
      if (this.children == null && this.links.isEmpty()) {
        ...
        if (pos == ChildPosition.NO_CHILD) {
          // (i.e. link extends over more than one child node, so the current node is the smallest
          // one that fully contains the link.
          this.links.add(w);
        } else {
     ...
     * @return instance of LinkWrapper that is closest to (x,y) if closer than bestPseudoDistance,
     *     null otherwise. Adjust bestPseudoDistance in first case
     */
    public LinkWrapper getNearest(
        final double x, final double y, final MutableDouble bestDistanceIndicator) {
      ...
      if (this.children != null) {
        ...
        if (childPos != ChildPosition.NO_CHILD) {
          // (not sure we can have children, but none in the correct "direction"; depends on exact
          // behavior of "split")

          // Find child in correct "direction" and do recursive call:
          LinkWrapper tmp =
              this.children[childPos.ordinal()].getNearest(x, y, bestDistanceIndicator);
          if (tmp != null) {
            closest = tmp;
          }
          ...
          for (ChildPosition c : ChildPosition.values()) {
            if (c != childPos && c != ChildPosition.NO_CHILD) {
              ...
              // For those other children, check if their bounding box is so close that we also need
              // to look at them:
    ...
    private void split() {
      ...
      this.children[ChildPosition.CHILD_NW.ordinal()] =
          new QuadTreeNode(this.minX, centerY, centerX, this.maxY);
      this.children[ChildPosition.CHILD_NE.ordinal()] =
          new QuadTreeNode(centerX, centerY, this.maxX, this.maxY);
      this.children[ChildPosition.CHILD_SE.ordinal()] =
          new QuadTreeNode(centerX, this.minY, this.maxX, centerY);
      this.children[ChildPosition.CHILD_SW.ordinal()] =
          new QuadTreeNode(this.minX, this.minY, centerX, centerY);
      ...
      for (LinkWrapper w : this.links) {
        ...
        if (pos == ChildPosition.NO_CHILD) {
          ...
          this.children[pos.ordinal()].put(w);
          // (seems to me that this cannot happen to more than one link since the quad tree node
          // will split
    ...
    private ChildPosition getChildPosition(final LinkWrapper w) {
      ...
      // (happens when bounding box of link overlaps more than one child node.. i.e. in particular
      // with long links)
    }
    ...
    private ChildPosition getChildPosition(final double x, final double y) {
      ...
      return ChildPosition.CHILD_SW;
      //			}
      //			throw new RuntimeException("should never get here since (x,y) has to be _somewhere_ with
      // respect to centerX and centerY") ;
    }

    /**
     * Calculates the distance of a given point to the border of the rectangle. If the point lies
     * within the rectangle, the distance is zero.
     *
  ...
  private static double calcLineSegmentDistanceIndicator(
      final double x, final double y, final Link link) {
  ...
  private static double calcDistanceIndicator(
      final double fromX, final double fromY, final double toX, final double toY) {
  ...
  private static class LinkWrapper {
    ...
    public LinkWrapper(final Link link) {
      ...
      if (fx == tx) {
        ...
        this.minX =
            fx - Math.abs(fx) * 1e-8; // make it adaptive within the number of significant digits
        this.maxX =
            fx + Math.abs(fx) * 1e-8; // make it adaptive within the number of significant digits
      } else {
      ...
      if (fy == ty) {
        this.minY =
            fy - Math.abs(fy) * 1e-8; // make it adaptive within the number of significant digits
        this.maxY =
            fy + Math.abs(fy) * 1e-8; // make it adaptive within the number of significant digits
      } else {
