...
/**
 * Analyzes the network to find for each link the "logical subsequent link", i.e. the link that most
 * people are likely to continue driving on when crossing the next node. This could depend on the
 * geometry (assuming it is more likely to continue straight on instead of making a U-turn), the
 * capacity of links (likely to continue on links with the same or larger capacity instead of using
 * small bumpy roads), the speed limit, other attributes or combinations there of. This code only
 * looks at geometry.
 *
 * @author ychen
 * @author mrieser
...
public final class SubsequentLinksAnalyzer {
  ...
  /**
   * @return an array, giving for each link (link-id-index as array index) the computed subsequent
   *     link (value in map).
   */
  ...
  /**
   * calculates the "default next" link of the current link with respect to geometry and Capacity
   * (depending on BETA) and writes the result in ssLinks.
   */
  private void compute() {
    Comparator<LinkData> linkDataComparator =
        (o1, o2) -> {
          ...
          if (cmp == 0) {
            cmp =
                Double.compare(
                    o2.link.getCapacity(),
                    o1.link.getCapacity()); // prefer the one with larger capacity
          }
          if (cmp == 0) {
    ...
    for (Link l : this.network.getLinks().values()) {
      ...
      String requiredMode =
          modes.size() == 1
              ? modes.iterator().next()
              : (modes.contains(this.preferredMode)
                  ? this.preferredMode
                  : (modes.isEmpty() ? this.preferredMode : modes.iterator().next()));

      Collection<? extends Link> potentialLinks =
          outLinks.stream().filter(link -> link.getAllowedModes().contains(requiredMode)).toList();
      ...
      if (potentialLinks.size() > 1) {
        ...
        // find the best subsequent link: the one with the smallest theta, if multiple have this
        // than the one with the biggest capacity
