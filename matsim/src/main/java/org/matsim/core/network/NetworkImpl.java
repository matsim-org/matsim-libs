 ...
 *
 * <ul>
 *   <li>This class is final, since it is sitting behind an interface, and thus delegation can be
 *       used for implementation modifications. Access to the quad tree might be justified in some
 *       cases, but should then be realized by specific methods and not via inheritance of the field
 *       (I would think).
 * </ul>
 *
 * @author nagel
 * @author mrieser
 */
/*deliberately package*/ final class NetworkImpl
    implements Network, Lockable, TimeDependentNetwork, SearchableNetwork {

  private static final Logger log = LogManager.getLogger(NetworkImpl.class);
  ...
  public void addLink(final Link link) {
    ...
    if (testLink != null) {
      if (testLink == link) {
        log.warn(
            "Trying to add a link a second time to the network. link id = "
                + link.getId().toString());
        return;
      }
      throw new IllegalArgumentException(
          "There exists already a link with id = "
              + link.getId().toString()
              + ".\nExisting link: "
              + testLink
              + "\nLink to be added: "
    ...
    if (!fromNode.getOutLinks().containsKey(link.getId())) fromNode.addOutLink(link);
    if (!toNode.getInLinks().containsKey(link.getId())) toNode.addInLink(link);
    ...
    if (this.linkQuadTree != null) {
      double linkMinX =
          Math.min(link.getFromNode().getCoord().getX(), link.getToNode().getCoord().getX());
      double linkMaxX =
          Math.max(link.getFromNode().getCoord().getX(), link.getToNode().getCoord().getX());
      double linkMinY =
          Math.min(link.getFromNode().getCoord().getY(), link.getToNode().getCoord().getY());
      double linkMaxY =
          Math.max(link.getFromNode().getCoord().getY(), link.getToNode().getCoord().getY());
      ...
      } else if (this.linkQuadTree.getMinEasting() <= linkMinX
          && this.linkQuadTree.getMaxEasting() > linkMaxX
          && this.linkQuadTree.getMinNorthing() <= linkMinY
          && this.linkQuadTree.getMaxNorthing() > linkMaxY) {
  ...
  public void addNode(final Node nn) {
    ...
    if (this.nodeQuadTree != null) {
      ...
      } else if (this.nodeQuadTree.getMinEasting() <= nn.getCoord().getX()
          && this.nodeQuadTree.getMaxEasting() > nn.getCoord().getX()
          && this.nodeQuadTree.getMinNorthing() <= nn.getCoord().getY()
          && this.nodeQuadTree.getMaxNorthing() > nn.getCoord().getY()) {
  ...
  public void setEffectiveLaneWidth(final double effectiveLaneWidth) {
    ...
    if (!Double.isNaN(this.effectiveLaneWidth) && this.effectiveLaneWidth != effectiveLaneWidth) {
      log.warn(
          this
              + "[effectiveLaneWidth="
              + this.effectiveLaneWidth
              + " already set. Will be overwritten with "
              + effectiveLaneWidth
              + "]");
    }
  ...
  /**
   * Sets the network change events and replaces existing events. Before events are applied to their
   * corresponding links, all links are reset to their initial state. Pass an empty event list to
   * reset the complete network.
   *
  ...
  public void setNetworkChangeEvents(final List<NetworkChangeEvent> events) {
    ...
    for (Link link : getLinks().values()) {
      ...
      // Presumably, there is no exception here if this fails because it can be interpreted: maybe
      // only some links are time-dependent
      // and others are not, and it is sufficient if the time-dependent ones can be configured by
      // the addNetworkChangeEvent method.
      // kai, jul'16
    }
  ...
  //		buildQuadTree();
  //	}
  // it is safer if all functionality that could be done here is either done lazily or directly when
  // nodes/links are added.  kai, jul'16
  ...
  private synchronized void buildQuadTree() {
    ...
    maxx += 1.0;
    maxy += 1.0;
    // yy the above four lines are problematic if the coordinate values are much smaller than one.
    // kai, oct'15

    log.info(
        "building QuadTree for nodes: xrange("
            + minx
            + ","
            + maxx
            + "); yrange("
  ...
  private synchronized void buildLinkQuadTree() {
    ...
    log.info(
        "building LinkQuadTree for nodes: xrange("
            + minx
            + ","
            + maxx
            + "); yrange("
