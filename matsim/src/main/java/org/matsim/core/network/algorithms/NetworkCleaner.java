...
/**
 * Ensures that each link in the network can be reached by any other link. Links that cannot be
 * reached by some other links, or links from which it is not possible to reach all other links, are
 * removed from the network. Nodes with no incoming or outgoing links are removed as well from the
 * network.
 *
 * @author mrieser
 * @author balmermi
...
public final class NetworkCleaner implements NetworkRunnable {
  ...
  /**
   * Finds the cluster of nodes of which <code>startNode</code> is part of. The cluster contains all
   * nodes which can be reached starting at <code>startNode</code> and from where it is also
   * possible to return again to <code>startNode</code>.
   *
  ...
  /**
   * Reducing the network so it only contains nodes included in the biggest Cluster. Loop over all
   * nodes and check if they are in the cluster, if not, remove them from the network
   */
  public static void reduceToBiggestCluster(Network network, Map<Id<Node>, Node> biggestCluster) {
    ...
    for (Node node : allNodes2) {
      if (!biggestCluster.containsKey(node.getId())) {
        network.removeNode(
            node.getId()); // removeNode takes care of removing links too in the network
      }
    }
    log.info(
