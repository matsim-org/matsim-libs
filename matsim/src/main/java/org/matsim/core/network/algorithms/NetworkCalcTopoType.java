...
/**
 * See <a
 * href="https://doi.org/10.3929/ethz-b-000023532">https://doi.org/10.3929/ethz-b-000023532</a> for
 * a description of node types. It's the graph matching paper.
 *
 * @author balmermi
 */
public final class NetworkCalcTopoType implements NetworkRunnable {
  ...
  public void run(final Network network) {
    ...
    for (Node n : network.getNodes().values()) {
      ...
      if ((nOfInLinks + nOfOutLinks) == 0) {
        setTopoType(n, EMPTY);
      } else if (nOfInLinks == 0) {
        setTopoType(n, SOURCE);
      } else if (nOfOutLinks == 0) {
        setTopoType(n, SINK);
      } else if (getNOfIncidentNodes(n) == 1) {
        setTopoType(n, DEADEND);
      } else if (getNOfIncidentNodes(n) == 2) {
        if ((nOfOutLinks == 1) && (nOfInLinks == 1)) {
          setTopoType(n, PASS1WAY);
        } else if ((nOfOutLinks == 2) && (nOfInLinks == 2)) {
          setTopoType(n, PASS2WAY);
        } else if ((nOfOutLinks == 2) && (nOfInLinks == 1)) {
          setTopoType(n, START1WAY);
        } else if ((nOfOutLinks == 1) && (nOfInLinks == 2)) {
          setTopoType(n, END1WAY);
        }
        ...
        // connecting the same nodes.
          ...
          // yyyy intellij says that this condition is always true when the code comes to here.
          // kai, dec'17
          setTopoType(n, INTERSECTION);
        } else {
          throw new RuntimeException(
              "Node=" + n.toString() + " cannot be assigned to a topo type!");
        }
