...
/**
 * This class extracts a subnetwork from a given network containing only those links where at least
 * one of the given transport modes are allowed. The resulting network will not contain any links
 * where none of the specified transport modes are allowed. In addition, all links in the resulting
 * network will have at most those modes specified for the extraction, additional modes are removed
 * from the allowed set for each link.<br>
 * This class makes no guarantee that the resulting network is strongly connected, not even when the
 * input network was strongly connected.
 *
 * @author mrieser
 */
public final class TransportModeNetworkFilter {
   ...
   *
   * <p>I had to extend this method in order to keep the nodes in the same order as in the input
   * network. Otherwise, some router tests failed since for some from-to-pairs, multiple routes with
   * the same costs were found. In that case, the outcome depends on the order of the nodes and
   * links in the network. This problem might occur also in other places, therefore, I fixed it
   * here. cdobler, sep'17
   *
   * <p>Time-varying networks did not copy time-dependent information. This functionality is
   * included now. sebhoerl, aug'24
   *
      ...
      if (fullTimeDependentNetwork.getNetworkChangeEvents().size() > 0) {
        if (!(subNetwork instanceof TimeDependentNetwork)) {
          throw new RuntimeException(
              "Filtering time-dependent network with change events into a static network. Information on change events would be lost!");
        } else {
