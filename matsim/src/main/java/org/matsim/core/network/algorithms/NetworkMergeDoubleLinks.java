...
public final class NetworkMergeDoubleLinks implements NetworkRunnable {
  ...
  private static final Logger log = LogManager.getLogger(NetworkMergeDoubleLinks.class);
    ...
    switch (this.mergetype) {
      case REMOVE:
        if (logInfoLevel.equals(LogInfoLevel.MAXIMUM)) {
          log.info(
              "        Link id=" + link2.getId() + " removed because of Link id=" + link1.getId());
        }
      ...
      case ADDITIVE:
        {
          ...
          int lanes =
              NetworkUtils.getNumberOfLanesAsInt(link1) + NetworkUtils.getNumberOfLanesAsInt(link2);
      ...
      case MAXIMUM:
...

        {
          ...
          int lanes =
              Math.max(
                  NetworkUtils.getNumberOfLanesAsInt(link1),
                  NetworkUtils.getNumberOfLanesAsInt(link2));
          double length = Math.max(link1.getLength(), link2.getLength());
