...
/**
 * Collector to transform a {@link Stream<Link>} into a {@link Network}. The collector makes shallow
 * copies of nodes attached to the supplied links.
 *
 * <p>The collector works fine with parallel streams.
 */
public class NetworkCollector implements Collector<Link, Collection<Link>, Network> {
  ...
  private static void addNodeIfNecessary(Network network, Node node) {
    if (!network.getNodes().containsKey(node.getId())) {
      ...
      // nodes keep internal state of in- and out-links. Since we don't know, whether this state is
      // stale at this
      // point we create a shallow copy with empty in-out-links mappings. Simply clearing the
      // mappings would alter the
