...
public final class NetworkWriteAsTable implements NetworkRunnable {
  ...
  private static final Logger log = LogManager.getLogger(NetworkWriteAsTable.class);
  ...
  public void run(Network network) {
    ...
    try {
      ...
      for (Link l : network.getLinks().values()) {
        ...
        offsetVector =
            CoordUtils.scalarMult(offset / CoordUtils.length(offsetVector), offsetVector);
        ...
        out_et.write(
            l.getId() + "\t" + l.getFromNode().getId() + "\t" + l.getToNode().getId() + "\t");
        out_et.write(Math.round(l.getLength()) + "\t" + Math.round(l.getFreespeed() * 3.6) + "\t");
        out_et.write(
            Math.round(l.getCapacity() / capperiod)
                + "\t"
                + NetworkUtils.getNumberOfLanesAsInt(l)
                + "\t");
        out_et.write(
