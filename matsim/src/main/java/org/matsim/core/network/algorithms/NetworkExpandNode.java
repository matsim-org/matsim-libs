...
public final class NetworkExpandNode {
  ...
  /**
   * @param expRadius the expansion radius. If zero, all new nodes have the same coordinate and the
   *     new links with have length equals zero
   */
  ...
  /**
   * Expands the specified {@link Node node} that is part of the {@link Network network} and adds
   * turning restrictions/maneuvers to it.
   *
   ...
   *       <ul>
   *         <li><code>new_nodeId = nodeId+"-"+index; index=[0..#incidentLinks]</code>
   *         <li><code>new_coord</code> with distance <code>r</code> to the given node in direction
   *             of the corresponding incident Link of the given node with offset <code>e</code>.
   *       </ul>
   *       <pre>{@code
   * <-----12------         <----21-------
   *
   ...
   *               v       |
   * }</pre>
   *   <li>connects each incident link of the given node with the corresponding <code>new_node
   *       </code>
   *       <pre>{@code
   * <-----12------ o     o <----21-------
   *                   O
   ...
   *                 v   |
   * }</pre>
   *   <li>inter-connects the <code>new_node</code>s with new links as defined in the <code>turns
   *       </code> list, with:<br>
   *       <ul>
   *         <li><code>new_linkId = fromLinkId+"-"+index; index=[0..#turn-tuples]</code>
   *         <li>length equals the Euclidean distance
   *         <li>freespeed, capacity, permlanes, origId and type are equals to the attributes of the
   *             fromLink.
   *       </ul>
   *       <pre>{@code
   * <-----12------ o <--21-0-- o <----21-------
   ...
   * @return The {@link Tuple} of {@link List lists} containing the newly created {@link Node nodes}
   *     and {@link Link links}.
   */
  public final Tuple<List<Node>, List<Link>> expandNode(
      final Id<Node> nodeId, final List<TurnInfo> turns) {
    ...
    for (TurnInfo turn1 : turns) {
      ...
      if (!node.getInLinks().containsKey(first)) {
        throw new IllegalArgumentException(
            "nodeid=" + nodeId + ", linkid=" + first + ": link not an inlink of given node.");
      }
      ...
      if (!node.getOutLinks().containsKey(second)) {
        throw new IllegalArgumentException(
            "nodeid=" + nodeId + ", linkid=" + second + ": link not an outlink of given node.");
      }
    }
    ...
    if (network.removeNode(node.getId()) == null) {
    ...
    for (Link inlink : inlinks.values()) {
      ...
      Node n =
          network
              .getFactory()
              .createNode(Id.create(node.getId() + "-" + nodeIdCnt, Node.class), new Coord(x, y));
      network.addNode(n);
      newNodes.add(n);
      nodeIdCnt++;
    ...
    for (int i = 0; i < turns.size(); i++) {
      ...
      Link l =
          network
              .getFactory()
              .createLink(
                  Id.create(fromLink.getId() + "-" + i, Link.class),
                  fromLink.getToNode(),
                  toLink.getFromNode());
      double dist =
          CoordUtils.calcEuclideanDistance(
              toLink.getFromNode().getCoord(), fromLink.getToNode().getCoord());
      if (dist < 0.1 * this.expRadius) {
        // mostly the case when nodes are on top of each other
        // use a small length, but not really hard-coded because of different coordinate systems
        // ("1" is not always 1 meter)
  ...
  /**
   * Checks whether it makes sense to expand the node with turning links or not. For example, it
   * might not make sense to expand the node if no real restrictions are given, but just all
   * possible options are listed -- the exact same turning options could be available without
   * expanding the node. Thus, it usually only makes sense to expand a node if {@link
   * #turnsAreSameAsSingleNode(Id, List, boolean)} returns <code>false</code>. The algorithm can
   * optionally ignore u-turns, that means that it is not checked if all u-turns are allowed
   * according to the given turning options.
   *
   * @param nodeId
   ...
   * @return <code>true</code> if the list of explicit turns given results in the same turning
   *     options available as when the node is not expanded, <code>false</code> otherwise.
   */
  public boolean turnsAreSameAsSingleNode(
      final Id<Node> nodeId, final List<TurnInfo> turns, final boolean ignoreUTurns) {
      ...
      if (t2 != null) {
        ...
        if (modes != null) {
          if (ti.getModes() == null) {
            ...
            // because the modes are the intersection of modes from inLink and outLink, we can just
            // clear it
            modes.clear();
          } else {
