...
public final class NetworkUtils {
  ...
  /**
   * Creates a network based on the properties found in network config group. Either returns a
   * default network or a time variable network
   *
  ...
  /**
   * @return The bounding box of all the given nodes as <code>double[] = {minX, minY, maxX, maxY}
   *     </code>
   */
  ...
  /**
   * Splits the given string at whitespace (one or more space, tab, newline) into single pieces,
   * which are interpreted as ids.
   */
  ...
  /**
   * @return formerly, the maximum of 1 and the mathematically rounded number of lanes attribute's
   *     value at time "time" of the link given as parameter now, the number is truncated, but 0 is
   *     never returned. math.round is way, way too slow.
   */
  ...
  public static boolean isMultimodal(final Network network) {
    ...
    for (Link link : network.getLinks().values()) {
      ...
      if (modes.size() > 1) {
        ...
        if (mode == null) {
          if (hasEmptyModes) {
            // i.e. we have a mode restriction on the current link but not mode restriction on some
            // other link. => multi-modal
            return true;
          }
          mode = m2;
          ...
          if (!m2.equals(mode)) {
            // i.e. we have a mode restriction on the current link, and some other mode restriction
            // on some other link. => multi-modal
            return true;
          }
          // position here can be reached with "mode!=null" and "hasEmptyModes==true".  Should
          // return "true" here but does not.
          // Works anyways, since (it seems to me) that that state (mode!=null, hasEmptyModes==true)
          // can never be reached.
          // ???  kai, feb'15
        }
      } else {
        if (mode != null) {
          // i.e. we have no mode restriction on the current link, but a mode restriction on some
          // other link. => multi-modal:
          return true;
        }
        hasEmptyModes = true;
  ...
  /**
   * This method expects the nearest link to a given measure point. It calculates the euclidean
   * distance for both nodes of the link, "fromNode" and "toNode" and returns the node with shorter
   * distance
   */
  public static Node getCloserNodeOnLink(Coord coord, Link link) {
    ...
    if (distanceToNode < distanceFromNode) return toNode;
    return fromNode;
  }
  ...
  /**
   * Finds the (approx.) nearest link to a given point on the map, such that the point lies on the
   * right side of the directed link, if such a link exists.
   *
   * <p>It searches first for the nearest node, and then for the nearest link originating or ending
   * at that node and fulfilling the above constraint.
   *
   * <p><b>Special cases:</b> {@code nodes:o ; links:<-- ; coord:x} <i>No right entry link
   * exists</i>
   *
   * <pre>{@code
   * o<-1--o returning
   ...
   * }</pre>
   *
   * @param coord the coordinate for which the closest link should be found
   * @return the link found closest to <code>coord</code> and oriented such that the point lies on
  ...
  public static Link getNearestRightEntryLink(Network network, final Coord coord) {
    ...
    for (Link link : incidentLinks) {
      ...
      if (dist <= shortestRightDistance) {
        ...
        linkVector[0] = link.getToNode().getCoord().getX() - link.getFromNode().getCoord().getX();
        linkVector[1] = link.getToNode().getCoord().getY() - link.getFromNode().getCoord().getY();
  ...
  /**
   * Finds the (approx.) nearest link to a given point on the map. It searches first for the nearest
   * node, and then for the nearest link originating or ending at that node.
   *
   * @param coord the coordinate for which the closest link should be found
  ...
  public static Link getNearestLink(Network network, final Coord coord) {
    ...
    if (nearestNode.getInLinks().isEmpty() && nearestNode.getOutLinks().isEmpty()) {
      log.warn(
          network
              + "[found nearest node that has no incident links.  Will probably crash eventually ...  Maybe run NetworkCleaner?][node = "
              + nearestNode.getId()
              + "]");
    }
    ...
    // TODO [balmermi] Now it finds the first of the typically two nearest links (same nodes, other
    // direction)
  ...
  /**
   * Calculates the most 'left' outLink for a given inLink (oriented from north to south). That's
   * the link a driver would refer to when turning left (no u-turn), even if there is only one link
   * going to the right.
   *
  ...
  /**
   * Calculates the orientation of outgoing links for a given incoming link beginning from the right
   * if the inLink goes north to south. The most 'left' outLink comes last. The link back to the
   * inLink's upstream Node is ignored. <br>
   * <br>
   * Comments/questions:
   *
   * <ul>
   *   <li>What does the "north to south" part mean? Can't we just sort outLinks right to left no
   *       matter where we come from? kai, aug'16
   *   <li>In fact, a test (NetworkUtilsTest.getOutLinksSortedByAngleTest()) does not confirm the
   *       javadoc. The "north to south" is irrelevant. But instead, it sorts links <i> left to
   *       right </i> instead of right to left. <br>
   * </ul>
   *
   * If someone can confirm this, please comment out all these remarks. kai, aug'16 <br>
   * <br>
   *
   * @param inLink The inLink given
   * @return Collection of outLinks, or an empty collection if there are only outLinks back to the
   *     inLink's fromNode.
   */
  ...
  /**
   * @deprecated -- I don't know why this method exists; it makes reading code harder rather than
   *     easier. Maybe there used to be something more complicated which eventually got refactored
   *     into the current version? kai, feb'20
   */
  ...
  public static String getType(Link link) {
    ...
    //		} else {
    //			throw new RuntimeException( "getType not possible for this implementation of interface
    // Link" ) ;
    //		}
  ...
  /**
   * Returns the road type of a highway link. In OSM highway links contain links for car traffic. If
   * not set this method will return "unclassified".
   */
  public static String getHighwayType(Link link) {

    String type = (String) link.getAttributes().getAttribute(TYPE);

    if (type != null) type = type.replaceFirst("^highway\\.", "");

    if (type == null || type.isBlank()) type = "unclassified";
  ...
  public static double getAllowedSpeed(Link link) {
    ...
    if (speed == null) return link.getFreespeed();

    if (speed instanceof Double s) return s;
  ...
  public static Link createAndAddLink(
      Network network,
      final Id<Link> id,
      final Node fromNode,
      final Node toNode,
    ...
    return createAndAddLink(
        network, id, fromNode, toNode, length, freespeed, capacity, numLanes, null, null);
  }
  ...
  public static Collection<Node> getNearestNodes(
      Network network, final Coord coord, final double distance) {
    if (network instanceof SearchableNetwork) {
   ...
   * @param transformation coordinate transformation as from @{{@link
   *     org.matsim.core.utils.geometry.transformations.TransformationFactory#getCoordinateTransformation(String,
   *     String)}}
  ...
  /**
   * Returns the closest point to on a link from a Point (either its orthogonal projection or the
   * link's to and from node)
   *
  ...
  public static List<Node> getOriginalGeometry(Link link) {
    ...
    if (!StringUtils.isBlank(attr)) {
      ...
      for (String date : data) {
        ...
        if (values.length != 3)
          throw new RuntimeException("expected three values per node but found: " + date);
