...
final class NetworkReaderMatsimV1 extends MatsimXmlParser {
  ...
  private static final Logger log = LogManager.getLogger(NetworkReaderMatsimV1.class);
  ...
  public NetworkReaderMatsimV1(
      final CoordinateTransformation transformation, final Network network) {
  ...
  private void startNode(final Attributes atts) {
    ...
    final Node node =
        this.network.getFactory().createNode(Id.create(atts.getValue("id"), Node.class), c);
