...
public final class NetworkTurnInfoBuilder implements NetworkTurnInfoBuilderI {
    ...
    for (LanesToLinkAssignment l2l : laneDefs.getLanesToLinkAssignments().values()) {
      ...
      for (Lane lane : l2l.getLanes().values()) {
        if (lane.getToLinkIds() != null
            && (lane.getToLaneIds() == null
                || lane.getToLaneIds()
                    .isEmpty())) { // make sure that it is a lane at the end of a link
  ...
  /**
   * Creates a List of TurnInfo objects for every existing link of the network. If the links have
   * mode attributes set, those are considered in TurnInfo creation.
   */
      ...
      for (Link inLink : node.getInLinks().values()) {
        ...
        for (Link outLink : node.getOutLinks().values()) {
          if (!inLink.getAllowedModes().isEmpty() && !outLink.getAllowedModes().isEmpty()) {
            if (inLink.getAllowedModes().contains(mode)
                && outLink.getAllowedModes().contains(mode)) {
  ...
  /**
   * Modifies the first Map containing the allowed turning moves: All turning moves of a fromLink
   * for that the second Map contains (only in this case!) an entry are checked for differences
   * concerning outLinks and modes. If an outLink or mode is not contained in the restriction, the
   * corresponding TurnInfo or mode is removed or modified in the first map.
   */
  public final void mergeTurnInfoMaps(
      Map<Id<Link>, List<TurnInfo>> allowedInLinkTurnInfoMap,
    ...
    for (Map.Entry<Id<Link>, List<TurnInfo>> e : allowedInLinkTurnInfoMap.entrySet()) {
      ...
      if (restrictingTurnInfos != null) { // there are restrictions for the inLink
        ...
        for (TurnInfo allowedForOutlink : allowedTurnInfos) {
          TurnInfo restrictionForOutlink =
              getTurnInfoForOutlinkId(restrictingTurnInfos, allowedForOutlink.getToLinkId());
          if (restrictionForOutlink
              == null) { // there is no turn at all allowed from the inLink to the outLink
            ...
            if (restrictionForOutlink.getModes() != null && allowedForOutlink.getModes() != null) {
              Set<String> commonModes =
                  this.calculateCommonModes(restrictionForOutlink, allowedForOutlink);
