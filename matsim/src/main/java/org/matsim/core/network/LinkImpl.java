...
/*deliberately package*/ class LinkImpl implements Link {
  ...
  private static final Set<String> DEFAULT_ALLOWED_MODES =
      HashSetCache.get(Set.of(TransportMode.car));

  private final Attributes attributes = new AttributesImpl();

  /*deliberately package*/ LinkImpl(
      final Id<Link> id,
      final Node from,
      final Node to,
    ...
    // for the eventual time variant attributes don't call the setter as it must be overwritten in
    // TimeVariantLinkImpl
  ...
  private void checkFreespeedSemantics() {
    if ((this.freespeed <= 0.0) && (fsWarnCnt < maxFsWarnCnt)) {
      fsWarnCnt++;
      log.warn(
          "freespeed=" + this.freespeed + " of link id " + this.getId() + " may cause problems");
      if (fsWarnCnt == maxFsWarnCnt) log.warn(Gbl.FUTURE_SUPPRESSED);
    }
  }
  ...
  private void checkNumberOfLanesSemantics() {
    if ((this.nofLanes < 1) && (plWarnCnt < maxPlWarnCnt)) {
      plWarnCnt++;
      log.warn(
          "permlanes=" + this.nofLanes + " of link id " + this.getId() + " may cause problems");
      if (plWarnCnt == maxPlWarnCnt) log.warn(Gbl.FUTURE_SUPPRESSED);
    }
  }
  ...
  private void checkLengthSemantics() {
    if ((this.getLength() <= 0.0) && (lengthWarnCnt < maxLengthWarnCnt)) {
      ...
      if (lengthWarnCnt == maxLengthWarnCnt) log.warn(Gbl.FUTURE_SUPPRESSED);
    }
  }
  ...
  @Override
  public double getNumberOfLanes(
      final double time) { // not final since needed in TimeVariantLinkImpl
    return this.nofLanes;
  }
