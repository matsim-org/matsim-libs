...
/**
 * Adds additional speed restrictions to a MATSim {@link Network network} created by {@link
 * NetworkReaderTeleatlas}. The input speed restriction DBF file is based on <strong>Tele Atlas
...
public final class NetworkTeleatlasAddSpeedRestrictions implements NetworkRunnable {
  ...
  private static final Logger log =
      LogManager.getLogger(NetworkTeleatlasAddSpeedRestrictions.class);
  ...
  /**
   * Reading and assigning speed restrictions to the {@link Link links} of a {@link Network
   * network}.
   *
   * <p>It uses the following attributes from the Tele Atlas MultiNet speed restriction DBF file:
   *
   * <ul>
   *   <li>{@link #SR_ID_NAME} (Feature Identification)
   *   <li>{@link #SR_SPEED_NAME} (Speed Restriction)
   *   <li>{@link #SR_VALDIR_NAME} (Validity Direction)
   *       <ul>
   *         <li>1: Valid in Both Directions
   *         <li>2: Valid Only in Positive Direction
   *         <li>3: Valid Only in Negative Direction
   *       </ul>
   *   <li>{@link #SR_VERIFIED_NAME} (Verified)
   *       <ul>
   *         <li>0: Not Verified (default)
   *         <li>1: Verified
   *       </ul>
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>speed restrictions that are not verified will be ignored.
   *   <li>speed restrictions will be assigned in given directions ({@link #SR_VALDIR_NAME})
   *   <li>speed restrictions will be ignored if the corresponding {@link Link link} is not found
   *       (produces a trace message).
   *   <li>speed restrictions will not be assigned to the {@link Link link} if it already contains a
   *       speed that is lower then the one from the speed restrictions file.
   * </ul>
   *
   * @param network
        ...
        if (verified == 1) {
          ...
          if (valdir == 1) {
            ...
            if ((ftLink == null) || (tfLink == null)) {
              log.trace(
                  "  linkid="
                      ...
                      + ", valdir="
                      + valdir
                      + ": at least one link not found. Ignoring and proceeding anyway...");
              srIgnoreCnt++;
            } else {
            ...
            if (ftLink == null) {
              log.trace(
                  "  linkid="
                      ...
                      + ", valdir="
                      + valdir
                      + ": link not found. Ignoring and proceeding anyway...");
              srIgnoreCnt++;
            } else {
              double speed = Double.parseDouble(entries[srSpeedNameIndex].toString()) / 3.6;
              // assigning speed restriction only if given freespeed is higher
            ...
            throw new IllegalArgumentException(
                "linkid=" + id + ": valdir=" + valdir + " not known.");
          }
        }
      }
