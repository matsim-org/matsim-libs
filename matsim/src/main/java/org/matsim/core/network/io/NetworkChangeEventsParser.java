...
public final class NetworkChangeEventsParser extends MatsimXmlParser {
  ...
  public void startTag(String name, Attributes atts, Stack<String> context) {
    ...
    if (name.equalsIgnoreCase(NETWORK_CHANGE_EVENT_TAG)) {
      ...
      if (value != null) {
        ...
        if (link != null) this.currentEvent.addLink(link);
        else log.warn(String.format("Link %1$s not found!", value));
      }
      /*
       * flow capacity changes
       */
    } else if (name.equalsIgnoreCase(FLOW_CAPACITY_TAG) && this.currentEvent != null) {
      this.currentEvent.setFlowCapacityChange(
          newNetworkChangeType(atts.getValue(CHANGE_TYPE_TAG), atts.getValue(VALUE_TAG)));
      /*
       * freespeed change
       */
    } else if (name.equalsIgnoreCase(FREESPEED_TAG) && this.currentEvent != null) {
      this.currentEvent.setFreespeedChange(
          newNetworkChangeType(atts.getValue(CHANGE_TYPE_TAG), atts.getValue(VALUE_TAG)));
      /*
       * lanes changes
       */
    } else if (name.equalsIgnoreCase(LANES_TAG) && this.currentEvent != null) {
      this.currentEvent.setLanesChange(
          newNetworkChangeType(atts.getValue(CHANGE_TYPE_TAG), atts.getValue(VALUE_TAG)));
    }
  }
  ...
  // the enum is different from the xml value, since the "in_SI_units" was considered important, but
  // changing it also
