...
/**
 * Converts a network to an inverted network. Inverted nodes are situated at the end of the real
 * link. Inverted link attributes are copied from toLink of the real network, thus every inverted
 * link actually starts at the location of a real node. Each link Id of the real network corresponds
 * to a node Id of the inverted network.
 *
 * @author aneumann
 * @author dgrether
 */
public final class NetworkInverter {
  ...
  private static final Logger log = LogManager.getLogger(NetworkInverter.class);
  ...
  private void invertNetwork() {
    ...
    for (Link link : this.originalNetwork.getLinks().values()) {
      NetworkUtils.createAndAddNode(
          this.invertedNetwork, Id.create(link.getId(), Node.class), link.getToNode().getCoord());
        ...
        for (Link outLink : node.getOutLinks().values()) {
          ...
          if (ti != null) {
            numberOfLinksGenerated =
                this.createInvertedLink(inLink, outLink, numberOfLinksGenerated, ti.getModes());
          }
        }
      }
    }
    ...
    log.info(
        "Generated " + numberOfNodesGenerated + " Nodes and " + numberOfLinksGenerated + " Links");
  ...
  private int createInvertedLink(
      Link inLink, Link outLink, int numberOfLinksGenerated, Set<String> modes) {
    Link link =
        NetworkUtils.createAndAddLink(
            this.invertedNetwork,
            Id.create(numberOfLinksGenerated + 1, Link.class),
            this.invertedNetwork.getNodes().get(Id.create(inLink.getId(), Node.class)),
            this.invertedNetwork.getNodes().get(Id.create(outLink.getId(), Node.class)),
            outLink.getLength(),
            outLink.getFreespeed(),
            outLink.getCapacity(),
            outLink.getNumberOfLanes());
    link.setAllowedModes(modes);
    //		log.error("created inverted link " + link.getId() + " from " + inLink.getId() + " to " +
    // outLink.getId() + " with modes " + modes);
