...
final class VariableIntervalTimeVariantAttribute implements TimeVariantAttribute {
  ...
  public boolean isRecalcRequired() {
    ...
    // yyyy The second condition is a bit weird.  It essentially checks if the cached data structure
    // (aTimes) has
    // as many entries as it should have (given by aEvents).  This does need, however, an honest
    // calling of
    // incChangeEvents().  Why not just compare aTimes.length to changeEvents.length?
    // Counterargument might be that we may not have changeEvents.length available when we call
    // isRecalcRequired().
    // I do think, however, that this does not happen and also cannot happen, because we need the
    // info anyway for recalculation.
    // If going into this direction, then incChangeEvents(), isRecalcRequired(), recalc(...) might
    // not be necessary
  ...
  public void recalc(
    ...
    if (numEvent != this.aEvents - 1) {
      throw new RuntimeException(
          "Expected number of change events ("
              + (this.aEvents - 1)
