...
public final class Injector {
  ...
  public static com.google.inject.Injector createInjector(final Config config, Module... modules) {
        ...
        Guice.createInjector(
            new Module() {
              ...
              public void configure(Binder binder) {
                binder.requireExplicitBindings(); // For now, we are conservative and disable this
                // kind of magic.
    ...
    com.google.inject.Injector realInjector =
        bootstrapInjector.createChildInjector(insertMapBindings(guiceModules));
  ...
  private static Module insertMapBindings(List<Module> guiceModules) {
    com.google.inject.AbstractModule routingModuleBindings =
        createMapBindingsForType(guiceModules, RoutingModule.class);
    com.google.inject.AbstractModule travelTimeBindings =
        createMapBindingsForType(guiceModules, TravelTime.class);
    com.google.inject.AbstractModule travelDisutilityFactoryBindings =
        createMapBindingsForType(guiceModules, TravelDisutilityFactory.class);
    return Modules.combine(
        Modules.combine(guiceModules),
        routingModuleBindings,
        travelTimeBindings,
        travelDisutilityFactoryBindings);
  }

  private static <T> com.google.inject.AbstractModule createMapBindingsForType(
      List<Module> guiceModules, final Class<T> aClass) {
            ...
            public <T> Object visit(Binding<T> binding) {
              if (binding.getKey().getTypeLiteral().getRawType().equals(aClass)) {
                if (binding.getKey().getAnnotation() instanceof com.google.inject.name.Named) {
                  modes.add(
                      ((com.google.inject.name.Named) binding.getKey().getAnnotation()).value());
                }
              }
              return null;
            }
    ...
    return new com.google.inject.AbstractModule() {
      ...
      protected void configure() {
        MapBinder<String, T> routingModuleMultibinder =
            MapBinder.newMapBinder(binder(), String.class, aClass);
  ...
  /**
   * This is (or should be) the minimum bindings that are needed to create a MATSim injector. It
   * can, for example, be used to construct MATSim objects where the standard constructor is
   * deliberately package-private so that the object can only be constructed via injection. It could
   * also be used to create MATSim instances that do not use the "override" syntax. kai, jun'22
   */
  public static com.google.inject.Injector createMinimalMatsimInjector(
      Config config, Scenario scenario, Module... modules) {
