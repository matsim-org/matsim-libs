...
/*package*/ abstract class AbstractController {
  // we already had one case where a method of this was removed, causing downstream failures; better
  // just not
        ...
        new MatsimRuntimeModifications.MyRunnable() {
          ...
          public void run() throws MatsimRuntimeModifications.UnexpectedShutdownException {
            ...
            ControlerUtils.checkConfigConsistencyAndWriteToLog(
                config, "config dump before iterations start");
            prepareForSim();
            doIterations(config);
          }
          ...
          public void shutdown(boolean unexpected) {
            controlerListenerManagerImpl.fireControlerShutdownEvent(
                unexpected, thisIteration == null ? -1 : thisIteration);
          }
        };
  ...
  /**
   * Stopping criterion for iterations. Design thoughts:
   *
   * <ul>
   *   <li>AbstractController only controls process, not content. Stopping iterations controls
   *       process based on content. All such coupling methods are abstract; thus this one has to be
   *       abstract, too.
   *   <li>One can see this confirmed in the KnSimplifiedControler use case, where the function is
   *       delegated to a static method in the SimplifiedControllerUtils class ... as with all other
   *       abstract methods.
   * </ul>
   */
  ...
  private void doIterations(Config config)
  ...
  private void iteration(final Config config, final int iteration, boolean isLastIteration)
      throws MatsimRuntimeModifications.UnexpectedShutdownException {
    ...
    iterationStep(
        ...
        new Runnable() {
          @Override
          public void run() {
            controlerListenerManagerImpl.fireControlerIterationStartsEvent(
                iteration, isLastIteration);
          }
        });
    ...
    iterationStep(
        ...
        new Runnable() {
          ...
          public void run() {
            ...
            controlerListenerManagerImpl.fireControlerIterationEndsEvent(
                iteration, isLastIteration);
          }
        });
    ...
    try {
      this.getStopwatch()
          .writeSeparatedFile(
              this.getControlerIO().getOutputFilename("stopwatch.csv"),
              config.global().getDefaultDelimiter());
    ...
    if (config.controller().isCreateGraphs()) {
      this.getStopwatch()
          .writeGraphFile(
              this.getControlerIO()
                  .getOutputFilename("stopwatch.png", ControllerConfigGroup.CompressionType.none));
    }
    log.info(MARKER + "ITERATION " + iteration + " ENDS");
    log.info(Controler.DIVIDER);
  }

  private void mobsim(final Config config, final int iteration, boolean isLastIteration)
      throws MatsimRuntimeModifications.UnexpectedShutdownException {
    ...
    try {
      iterationStep(
          ...
          new Runnable() {
            @Override
            public void run() {
              controlerListenerManagerImpl.fireControlerBeforeMobsimEvent(
                  iteration, isLastIteration);
            }
          });
      ...
      // manager also resulted in an exception (because the mobsim crashed before initProcessing()
      // was
      // ever called), "hidding" the actual source of the problem.
      // To avoid this, we log anything thrown during mobsim before executing after mobsim
      // listeners.
      // td, oct'14
      ...
      iterationStep(
          ...
          new Runnable() {
            ...
            public void run() {
              ...
              controlerListenerManagerImpl.fireControlerAfterMobsimEvent(
                  iteration, isLastIteration);
            }
          });
    }
  ...
  private void iterationStep(String iterationStepName, Runnable iterationStep)
      throws MatsimRuntimeModifications.UnexpectedShutdownException {
   ...
   *
   * <ul>
   *   <li>This is such that ControlerListenerManager does not need to be exposed. One may decide
   *       otherwise ... kai, jul'12
   * </ul>
   */
