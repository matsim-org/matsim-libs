...
/**
 * "Designed for inheritance." Extend this class, overwrite configure, and use the methods of this
 * class to install your module.
 *
 * <p>See comments in subclasses.
 *
 * <p>AbstractModule is a custom version of com.google.inject.Module, with the following
 * differences:
 *
 * <ul>
 *   <li>AbstractModule is a bit more restrictive.
 *   <li>AbstractModule already has the config (as getConfig() ), which is helpful since the config
 *       contains material that has to be used to configure the services.
 * </ul>
 *
 * @author michaz
 */
public abstract class AbstractModule implements Module {
  ...
  /**
   * Contents retrieved (I think) by injected method QSim#addQueueSimulationListeners(...). Is not
   * public, and therefore cannot be referenced from here. <br>
   * I think that that method will be called every time the mobsim will be constructed. If the
   * injected classes are singletons, they will presumably be re-used, otherwise they will be newly
   * constructed.
   */
  ...
  @Inject com.google.inject.Injector bootstrapInjector;
  ...
  public final void configure(Binder binder) {
    ...
    this.controlerListenerMultibinder =
    ...
    this.qsimOverridingModulesMultibinder =
        Multibinder.newSetBinder(
            this.binder, AbstractQSimModule.class, Names.named("overridesFromAbstractModule"));
    this.install();
  }
   ...
   */
  protected final com.google.inject.binder.LinkedBindingBuilder<PlanSelector<Plan, Person>>
      bindPlanSelectorForRemoval() {
    return bind(new TypeLiteral<PlanSelector<Plan, Person>>() {});
  }

  protected final com.google.inject.binder.LinkedBindingBuilder<PlanStrategy>
  ...
  protected final com.google.inject.binder.LinkedBindingBuilder<ScoringFunctionFactory>
      bindScoringFunctionFactory() {
    return bind(ScoringFunctionFactory.class);
  }

  protected final com.google.inject.binder.LinkedBindingBuilder<MobsimListener>
      addMobsimListenerBinding() {
    return mobsimListenerMultibinder.addBinding();
  }

  protected final com.google.inject.binder.LinkedBindingBuilder<SnapshotWriter>
      addSnapshotWriterBinding() {
    return snapshotWriterMultibinder.addBinding();
  }

  protected final LinkedBindingBuilder<AttributeConverter<?>> addAttributeConverterBinding(
      final Class<?> clazz) {
   ...
   */
  @Deprecated
  protected final com.google.inject.binder.LinkedBindingBuilder<TravelDisutilityFactory>
  ...
  protected final com.google.inject.binder.LinkedBindingBuilder<LeastCostPathCalculatorFactory>
      bindLeastCostPathCalculatorFactory() {
    return bind(LeastCostPathCalculatorFactory.class);
  }

  protected final com.google.inject.binder.LinkedBindingBuilder<TravelTime> addTravelTimeBinding(
      String mode) {
  ...
  protected final LinkedBindingBuilder<PersonPrepareForSimAlgorithm>
  ...
  public static AbstractModule override(
      final Iterable<? extends AbstractModule> modules, final AbstractModule abstractModule) {
