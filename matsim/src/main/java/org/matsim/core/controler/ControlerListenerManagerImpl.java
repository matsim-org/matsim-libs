...
public final class ControlerListenerManagerImpl implements ControlerListenerManager {
  ...
  private static final Logger log = LogManager.getLogger(ControlerListenerManagerImpl.class);
  ...
  /**
   * The swing event listener list to manage ControlerListeners efficiently. First list manages core
   * listeners which are called first when a ControlerEvent is thrown. I.e. this list contains the
   * listeners that are always running in a predefined order to ensure correctness. The second list
   * manages the other listeners, which can be added by calling addControlerListener(...). A normal
   * ControlerListener is not allowed to depend on the execution of other ControlerListeners.
   */
  ...
  public void fireControlerStartupEvent() {
    ...
    for (StartupListener aListener : listener) {
      log.info(
          "calling notifyStartup on "
              + aListener.getClass().getName()
              + " with priority "
              + +aListener.priority());
  ...
  public void fireControlerIterationStartsEvent(final int iteration, boolean isLastIteration) {
    IterationStartsEvent event =
        new IterationStartsEvent(this.controler, iteration, isLastIteration);
    IterationStartsListener[] listener =
        this.coreListenerList.getListeners(IterationStartsListener.class);
    Arrays.sort(listener, Comparator.comparingDouble(ControlerListener::priority).reversed());
    for (IterationStartsListener aListener : listener) {
      log.info(
          "calling notifyIterationStarts on "
              + aListener.getClass().getName()
              + " with priority "
              + +aListener.priority());
  ...
  public void fireControlerIterationEndsEvent(final int iteration, boolean isLastIteration) {
    IterationEndsEvent event = new IterationEndsEvent(this.controler, iteration, isLastIteration);
    {
      IterationEndsListener[] listener =
          this.coreListenerList.getListeners(IterationEndsListener.class);
      Arrays.sort(listener, Comparator.comparingDouble(ControlerListener::priority).reversed());
      for (IterationEndsListener aListener : listener) {
        log.info(
            "calling notifyIterationEnds on "
                + aListener.getClass().getName()
                + " with priority "
                + +aListener.priority());
    ...
    {
      IterationEndsListener[] listener =
          this.listenerList.getListeners(IterationEndsListener.class);
      Arrays.sort(listener, Comparator.comparingDouble(ControlerListener::priority).reversed());
      for (IterationEndsListener aListener : listener) {
        log.info(
            "calling notifyIterationEnds on "
  ...
  public void fireControlerBeforeMobsimEvent(final int iteration, boolean isLastIteration) {
    ...
    BeforeMobsimListener[] listener =
        this.coreListenerList.getListeners(BeforeMobsimListener.class);
    Arrays.sort(listener, Comparator.comparingDouble(ControlerListener::priority).reversed());
    for (BeforeMobsimListener aListener : listener) {
      log.info(
          "calling notifyBeforeMobsim on "
              + aListener.getClass().getName()
              + " with priority "
              + +aListener.priority());
