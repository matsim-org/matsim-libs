...
class MatsimRuntimeModifications {
  ...
  void run() {
    Thread.UncaughtExceptionHandler previousDefaultUncaughtExceptionHandler =
    ...
    try {
      ...
      // Don't let it fall through to the UncaughtExceptionHandler. We want to first log the
      // Exception,
      // then shut down.
