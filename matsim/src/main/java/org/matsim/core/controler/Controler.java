...
public final class Controler implements ControlerI, MatsimServices, AllowsConfiguration {
  ...
  // * Seems to me that we should try to get everything here final.  Flexibility is provided by the
  // ability to set or add factories.  If this is
  ...
  public static final PatternLayout DEFAULTLOG4JLAYOUT =
      PatternLayout.newBuilder().withPattern("%d{ISO8601} %5p %C{1}:%L %m%n").build();
   ...
   *
   * @param args The arguments to initialize the services with. <code>args[0]</code> is expected to
   *     contain the path to a configuration file, <code>args[1]</code>, if set, is expected to
   *     contain the path to a local copy of the DTD file used in the configuration file.
   */
    ...
    if (!injectorCreated) {
      ...
      this.overrides =
          AbstractModule.override(
              Collections.singletonList(this.overrides),
              new AbstractModule() {
                @Override
                public void install() {
                  bind(Key.get(
      ...
      // check config consistency just before creating injector; sometimes, we can provide better
      ...
      final Set<AbstractModule> standardModules =
      ...
      this.injector =
          Injector.createInjector(config, AbstractModule.override(standardModules, overrides));
    }
  }
  ...
  public final void run() {
    ...
    // - that called methods such as setScoringFunctionFactory(), that redirects to
    // addOverridingModule()
  ...
  /**
   * Gives access to a {@link org.matsim.core.router.TripRouter} instance. This is a routing service
   * which you can use to calculate routes, e.g. from your own replanning code or your own
   * within-day replanning agent code. You get a Provider (and not an instance directly) because
   * your code may want to later create more than one instance. A TripRouter is not guaranteed to be
   * thread-safe, so you must get() an instance for each thread if you plan to write multi-threaded
   * code.
   *
   * <p>See {@link org.matsim.core.router.TripRouter} for more information and pointers to examples.
   */
  @Override
  ...
  /**
   * @return Returns the {@link org.matsim.core.replanning.StrategyManager} used for the replanning
   *     of plans.
   * @deprecated -- try to use services.addPlanStrategyFactory or services.addPlanSelectoryFactory.
   *     There are cases when this does not work, which is in particular necessary if you need to
   *     re-configure the StrategyManager during the iterations, <i>and</i> you cannot do this
   *     before the iterations start. In such cases, using this method may be ok. kai/mzilske,
   *     aug'14
   */
  @Override
  ...
  public final Controler addOverridingModule(AbstractModule abstractModule) {
    ...
    this.overrides =
        AbstractModule.override(Collections.singletonList(this.overrides), abstractModule);
    return this;
  }
