...
/**
 * Put MATSim logs into the output directory. This is static, like Loggers. Call initLogging with
 * either an OutputDirectoryHierarchy or simply with a pathname. Don't forget to call
 * closeOutputDirLogging before quitting the JVM. Call catchLogEntries() before either of the
 * initLogging methods to memorize log entries between that call and the init call and put them into
 * the log file. Useful if creating the log directory only happens after some set-up which allready
...
public final class OutputDirectoryLogging {
  ...
  /**
   * This variable is used to store the log4j output before it can be written to a file. This is
   * needed to set the output directory before logging.
   */
  ...
  public static void catchLogEntries() {
    ...
    collectLogMessagesAppender.start();
    {
      ...
      ctx.getConfiguration()
          .getRootLogger()
          .addAppender(collectLogMessagesAppender, Level.ALL, null);
      ctx.updateLoggers();
    }
  }
  ...
  /**
   * Call this method to create 2 log4j logfiles in the output directory specified as parameter. The
   * first logfile contains all messages the second only those above log Level.WARN (Priority.WARN).
   * After the end of the programm run it is strongly recommended to close the file logger by
   * calling the method closeOutputDirLogging().
   *
   ...
   * @author dgrether
   */
  public static void initLoggingWithOutputDirectory(final String outputDirectory)
      throws IOException {
    if (collectLogMessagesAppender != null) {
      final LoggerContext ctx = (LoggerContext) LogManager.getContext(false);
      ctx.getConfiguration().getRootLogger().removeAppender(collectLogMessagesAppender.getName());
  ...
  private static void initLogging(String outputFilename, String warnLogfileName)
      throws IOException {
    // code inspired from
    ...
    { // the "all" logfile
      appender =
          FileAppender.newBuilder()
              ...
              .withAppend(appendToExistingFile)
              .build();
      appender.start();
  ...
  /**
   * Call this method to close the log file streams opened by a call of initOutputDirLogging(). This
   * avoids problems concerning open streams after the termination of the program.
   *
   * @see OutputDirectoryLogging#initLoggingWithOutputDirectory(String)
   */
  public static void closeOutputDirLogging() {
    // might also be sent to the warn logstream but then you end up with a warning even if
    // everything is alright
    String endLoggingInfo =
