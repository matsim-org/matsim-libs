...
public final class ControlerUtils {
   ...
   * Technical reason is that {@link Controler} creates the injector in the run method, and then it
   * is too late to extract material in a direct way.
   *
   * @param config
   * @param scenario
   * @return
   ...
   *
   * <ul>
   *   <li>I extracted this method since it is now called <i>twice</i>: once directly after reading,
   *       and once before the iterations start. The second call seems more important, but I wanted
   *       to leave the first one there in case the program fails before that config dump. Might be
   *       put into the "unexpected shutdown hook" instead. kai, dec'10
   *       <p>Removed the first call for now, because I am now also checking for consistency with
   *       loaded controler modules. If still desired, we can put it in the shutdown hook.. michaz
   *       aug'14
   * </ul>
   *
   * @param config TODO
   ...
   */
  public static final void checkConfigConsistencyAndWriteToLog(
      Config config, final String message) {
    log.info(message);
