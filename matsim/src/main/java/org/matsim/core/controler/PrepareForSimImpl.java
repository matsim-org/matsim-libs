...
public final class PrepareForSimImpl implements PrepareForSim, PrepareForMobsim {
  // I think it is ok to have this public final.  Since one may want to use it as a delegate.  kai,
  // may'18
  ...
  @Nullable @Inject private Set<PersonPrepareForSimAlgorithm> prepareForSimAlgorithms;

  /**
   * backwardCompatibilityMainModeIdentifier should be a separate MainModeidentifier, neither the
   * routing mode identifier from TripStructureUtils, nor the AnalysisMainModeidentifier used for
   * analysis (ModeStats etc.).
   */
  @Inject
  PrepareForSimImpl(
      ...
      FacilitiesConfigGroup facilitiesConfigGroup,
  ...
  public void run() {
    ...
    switch (this.facilitiesConfigGroup.getFacilitiesSource()) {
      ...
      case onePerActivityLinkInPlansFile:
        /* fall-through */
        // switch is inside "FacilitiesFromPopulation" method!
      case onePerActivityLocationInPlansFile:
        //				FacilitiesFromPopulation facilitiesFromPopulation = new
        // FacilitiesFromPopulation(activityFacilities, facilitiesConfigGroup);
        ...
        // Note that location choice, when switched on, should now either use the facilities
        // generated here,
      ...
      default:
        throw new RuntimeException(
            "Facilities source '"
                + this.facilitiesConfigGroup.getFacilitiesSource()
                + "' is not implemented.");
    }
    ...
    if (prepareForSimAlgorithms != null) {
      // This does not nake use of multi-threading because it can not be assumed that these
      // instances are thread-safe
      // thread-safety could be ensured with Providers, but they can not be used automatically in
      // conjunction with set binders.
    ...
    // the above creation of vehicles per agent has to be run before executing the initial routing
    // here. janek, aug'19
    // At least xy2links is needed here, i.e. earlier than PrepareForMobsimImpl.  It could, however,
    // presumably be separated out
    ...
    // (yyyy means that if someone replaces prepareForSim and does not add the above lines, the
    // containers are not locked.  kai, nov'16)
  }
  ...
  private Map<String, VehicleType> getVehicleTypesForAllNetworkAndMainModes() {
    ...
    if (qSimConfigGroup
        .getVehiclesSource()
        .equals(QSimConfigGroup.VehiclesSource.fromVehiclesData)) {
    ...
    for (String mode : modes) {
      ...
      switch (qSimConfigGroup.getVehiclesSource()) {
        ...
        case modeVehicleTypesFromVehiclesData:
          ...
          if (type == null) {
            log.fatal(
                "Could not find requested vehicle type ="
                    + mode
                    + ". With config setting "
                    + modeVehicleTypesFromVehiclesData.toString()
                    + ", you need");
            log.fatal(
            ...
            throw new RuntimeException(
                "Could not find requested vehicle type = " + mode + ". See above.");
          }
          break;
        default:
          throw new RuntimeException(
              qSimConfigGroup.getVehiclesSource().toString() + " is not implemented yet.");
      }
  ...
  private void createAndAddVehicleIfNecessary(Id<Vehicle> vehicleId, VehicleType vehicleType) {
    ...
    if (!scenario.getVehicles().getVehicles().containsKey(vehicleId)) {
      ...
      switch (qSimConfigGroup.getVehiclesSource()) {
        ...
        case modeVehicleTypesFromVehiclesData:
          Vehicle vehicle =
              scenario.getVehicles().getFactory().createVehicle(vehicleId, vehicleType);
          ...
          if (legs.size() >= 1) {
            ...
            for (Leg leg : legs) {
              ...
              if (TripStructureUtils.getRoutingMode(leg) == null) {
                if (routingMode != null) {
                  String errorMessage =
                      "Found a mixed trip having some legs with routingMode set and others without. "
                          + "This is inconsistent. Agent id: "
                ...
                if (routingMode.equals(TripStructureUtils.getRoutingMode(leg))) {
                  ...
                  String errorMessage =
                      "Found a trip whose legs have different routingModes. "
                          + "This is inconsistent. Agent id: "
                          + person.getId().toString()
                          + "\nTrip: "
                          + trip.getTripElements().toString();
            ...
            if (routingMode == null) {
              if (legs.size() == 1) {
                ...
                String oldMainMode =
                    replaceOutdatedFallbackModesAndReturnOldMainMode(legs.get(0), null);
                if (oldMainMode != null) {
                ...
                if (plansConfigGroup
                    .getHandlingOfPlansWithoutRoutingMode()
                    .equals(HandlingOfPlansWithoutRoutingMode.useMainModeIdentifier)) {
                  for (Leg leg : legs) {
                    replaceOutdatedAccessEgressWalkModes(leg, routingMode);
                  }
                  routingMode =
                      getAndAddRoutingModeFromBackwardCompatibilityMainModeIdentifier(person, trip);
                } else {
                  String errorMessage =
                      "Found a trip with multiple legs and no routingMode. "
                          + "Person id "
                          + person.getId().toString()
                          + "\nTrip: "
                          + trip.getTripElements().toString()
  ...
  private String getAndAddRoutingModeFromBackwardCompatibilityMainModeIdentifier(
      Person person, Trip trip) {
    String routingMode;
    ...
    if (routingMode != null) {
      ...
      String errorMessage =
          "Found a trip whose legs had no routingMode. "
  ...
  /**
   * Method to replace outdated TransportModes such as drt_fallback or transit_walk. Some of those
   * were also used as access/egress to pt/drt helper modes.
   *
