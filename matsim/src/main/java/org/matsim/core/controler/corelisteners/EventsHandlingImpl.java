...
final class EventsHandlingImpl
    implements EventsHandling, BeforeMobsimListener, IterationEndsListener, ShutdownListener {

  private static final Logger log = LogManager.getLogger(EventsHandlingImpl.class);
  ...
  public void notifyBeforeMobsim(BeforeMobsimEvent event) {
    ...
    final boolean regularWriteEvents =
        writingEventsAtAll
            && (event.getIteration() > 0 && event.getIteration() % writeEventsInterval == 0);
    // (w/o the "writingEventsAtAll && ..." this is a division by zero when writeEventsInterval=0.
    // kai, apr'18)
    ...
    if (writingEventsAtAll && (regularWriteEvents || earlyIteration || lastIteration)) {
      for (EventsFileFormat format : eventsFileFormats) {
        switch (format) {
          case xml:
            this.eventWriters.add(
                new EventWriterXML(
                    controlerIO.getIterationFilename(
                        event.getIteration(), Controler.DefaultFiles.events)));
            break;
          case pb:
            ...
            URL url =
                IOUtils.getFileUrl(
                    controlerIO.getIterationFilename(
                        event.getIteration(), Controler.DefaultFiles.eventsPb));
            try {
              Class<?> writerClass =
                  ClassLoader.getSystemClassLoader()
                      .loadClass("org.matsim.contrib.protobuf.EventWriterPB");
              Constructor<?> constructor = writerClass.getConstructor(OutputStream.class);
              EventWriter writer =
                  (EventWriter) constructor.newInstance(IOUtils.getOutputStream(url, false));
              this.eventWriters.add(writer);
            } catch (ReflectiveOperationException e) {
              throw new RuntimeException(
                  "Error using the PBWriter. Please make sure protobuf contrib on the classpath, or remove pb output format.",
                  e);
            }
            break;
          case json:
            this.eventWriters.add(
                new EventWriterJson(
                    new File(
                        controlerIO.getIterationFilename(
                            event.getIteration(), Controler.DefaultFiles.eventsJson))));
            break;
          default:
