...
public final class ScenarioUtils {
  ...
  /**
   * Creates an unpopulated scenario. The configuration passed into this method is a) used to
   * determine which containers are required, depending on the options set in the scenario config
   * group, and b) wrapped in the Scenario
   *
   * <p>User code surrenders the config to the scenario. The config should not be externally changed
   * afterwards.
   *
  ...
  /**
   * Initializes a scenario and populates it with data read from the input files which are named in
   * the config. Uses provided {@link AttributeConverter}s when loading the scenario.
   */
  public static Scenario loadScenario(
      final Config config, Map<Class<?>, AttributeConverter<?>> attributeConverters) {
  ...
  /**
   * Populates a scenario with data read from the input files which are named in the config which is
   * wrapped in the scenario. Uses provided {@link AttributeConverter}s when loading the scenario.
   */
  public static void loadScenario(
      final Scenario scenario, Map<Class<?>, AttributeConverter<?>> attributeConverters) {
  ...
  public static final class ScenarioBuilder {
    ...
    public Scenario build() {
      this.scenario
          .setLocked(); // prevents that one can cast to ScenarioImpl and change the containers
      // again. kai, nov'14
