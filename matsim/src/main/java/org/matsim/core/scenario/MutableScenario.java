...
public final class MutableScenario implements Scenario, Lockable {
  ...
  public final Config getConfig() {
    // yy should throw an exception if null. kai, based on
    // https://matsim.atlassian.net/browse/MATSIM-301 , may'15
    return this.config;
  }
  ...
  // That is, in such a situation the null pointer exception is thrown much later than when the null
  // pointer was obtained.
  // He rather wants to have this fail when it is called.
  // I (kn) am a bit sceptic if it makes sense to establish such a convention when most other people
  // use "if ... == null" use as the
  // official dialect to check if something is there (since this would cause an exception with
  // Thibaut's approach). kai, feb'15
  ...
  @Override
  public final void addScenarioElement(final String name, final Object o) {
    ...
    if (former != null) {
      throw new IllegalStateException(
          former + " is already associated with name " + name + " when adding " + o);
    }
  }
