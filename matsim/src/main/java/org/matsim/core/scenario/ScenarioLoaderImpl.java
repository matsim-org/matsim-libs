...
/**
 * Loads elements of Scenario from file. Non standardized elements can also be loaded however they
 * require a specific instance of Scenario. {@link #loadScenario()} reads the complete scenario from
 * files while the other load...() methods only load specific parts of the scenario assuming that
 * required parts are already loaded or created by the user.
 *
 * <p>Design thoughts:
 *
 * <ul>
...
class ScenarioLoaderImpl {
  ...
  /**
   * Loads all mandatory Scenario elements and if activated in config's scenario module/group
   * optional elements.
   *
   * @return the Scenario
   */
    ...
    if ((this.config.network() != null) && (this.config.network().getInputFile() != null)) {
      ...
      MatsimNetworkReader reader =
          new MatsimNetworkReader(
              inputCRS, config.global().getCoordinateSystem(), this.scenario.getNetwork());
      ...
      if ((this.config.network().getChangeEventsInputFile() != null)
        ...
        log.info(
            "loading network change events from "
                + this.config
                    .network()
                    .getChangeEventsInputFileUrl(this.config.getContext())
                    .getFile());
  ...
  private void loadActivityFacilities() {
    if ((this.config.facilities() != null) && (this.config.facilities().getInputFile() != null)) {
      ...
      log.info(
          "loaded "
              + this.scenario.getActivityFacilities().getFacilities().size()
              + " facilities from "
              + facilitiesFileName);
    } else {
      log.info(
    ...
    if ((this.config.facilities() != null)
      ...
      URL facilitiesAttributesURL =
          ConfigGroup.getInputFileURL(
              this.config.getContext(),
              this.config.facilities().getInputFacilitiesAttributesFile());
  ...
  private void loadPopulation() {
    ...
    if ((this.config.plans() != null)
        && (this.config.plans().getInputPersonAttributeFile() != null)) {
      URL personAttributesURL =
          this.config.plans().getInputPersonAttributeFileURL(this.config.getContext());
      ...
      //				// since ScenarioLoader is supposed to only read material,  there are cases where the
      // output directory does not exist at
      //				// this stage. One could maybe write to the "config.getContext()" directory.  However,
      // sometimes this is a URL, and thus also
      //				// non-writeable, and it is even less systematic than writing into the output directory.
      // kai, jun'19
      //
      ...
      //
      //				log.warn(
      //					  "a file with path=" + outFilename + " was just written in order to facilitate the
      // transition to having person attributes inside " +
      //						    "the persons. " );
      //			}
      ...
      if ((fn != null)) {
        if (!this.config.households().isInsistingOnUsingDeprecatedHouseholdsAttributeFile()) {
          throw new RuntimeException(
              HouseholdsConfigGroup.HOUSEHOLD_ATTRIBUTES_DEPRECATION_MESSAGE);
        }
  ...
  private void loadTransit() throws UncheckedIOException {
    ...
    if (this.config.transit().getTransitScheduleFile() != null) {
      URL transitScheduleFile =
          this.config.transit().getTransitScheduleFileURL(this.config.getContext());
    ...
    if (this.config.transit().getTransitLinesAttributesFile() != null) {
      ...
      URL transitLinesAttributesFileName =
          IOUtils.extendUrl(
              this.config.getContext(), this.config.transit().getTransitLinesAttributesFile());
    ...
    if (this.config.transit().getTransitStopsAttributesFile() != null) {
      ...
      URL transitStopsAttributesURL =
          IOUtils.extendUrl(
              this.config.getContext(), this.config.transit().getTransitStopsAttributesFile());
  ...
  private void loadTransitVehicles() throws UncheckedIOException {
    ...
    if (vehiclesFile != null) {
      ...
      new MatsimVehicleReader(this.scenario.getTransitVehicles())
          .readURL(this.config.transit().getVehiclesFileURL(this.config.getContext()));
    } else {
  ...
  private void loadVehicles() throws UncheckedIOException {
    ...
    if (vehiclesFile != null) {
      ...
      new MatsimVehicleReader(this.scenario.getVehicles())
          .readURL(IOUtils.extendUrl(this.config.getContext(), vehiclesFile));
    } else {
  ...
  private <T extends Identifiable<?> & Attributable> void parseObjectAttributesToAttributable(
      URL url, Iterable<T> attributables, String message) {
    ...
    for (T facility : attributables) {
      Collection<String> keys =
          ObjectAttributesUtils.getAllAttributeNames(attributes, facility.getId().toString());
      for (String key : keys) {
    ...
    // (some of the above could also become a static helper method in ObjectAttributesUtils, but
    // this here seems the only
    // place within matsim core where the personAttributes are automatically read so maybe there is
    // no need for this. kai, jun'19)
