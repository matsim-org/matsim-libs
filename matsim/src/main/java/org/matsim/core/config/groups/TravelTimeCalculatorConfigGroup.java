...
public final class TravelTimeCalculatorConfigGroup extends ReflectiveConfigGroup {
  ...
  private double traveltimeBinSize =
      15 * 60; // use a default of 15min time-bins for analyzing the travel times
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        ANALYZEDMODES,
        "(only for backwards compatibility; only used if "
            + SEPARATEMODES
            + "==false && + "
            + FILTERMODES
    ...
    map.put(
        FILTERMODES,
        "(only for backwards compatiblity; only used if "
            + SEPARATEMODES
  ...
  public void setAnalyzedModesAsString(final String analyzedModes) {
    ...
    // lower case is confusing here because at other places (qsimConfigGroup, planCalcRoute), it
    // takes mode string as it is. Amit Aug'17
