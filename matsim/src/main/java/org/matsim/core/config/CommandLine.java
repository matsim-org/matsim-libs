...
/**
 * This class provides functionality to configure (MATSim) applications via the command line. While
 * it can be used as a tool for configuration of any Java application in itself, it provides
 * advanced functionality to directly modify the MATSim {@link Config} object.
 *
 * <h1>General usage</h1>
 *
 * <p>The command line interpreter is set up using the {@link CommandLine.Builder}:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The command line option can be accessed via safe getters, which return {@link Optional}s, or
 * strict getters, which raise exceptions:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>As can be seen, options are always returned as strings. It is the task of the user to convert
 * the arguments to the expected data types. They are given in one of the following ways:
 *
 * <ul>
 *   <li>Value following the option name: <code>--threads 20</code>
 *   <li>With equals sign between: <code>--threads=20</code>
 * </ul>
 *
 * <h2>MATSim usage</h2>
 *
 * <p>In order to configure MATSim with the command line, one needs to tell the interpreter to apply
 * the command line options to the MATSim {@link Config}:
 *
 * <pre>
 * CommandLine cmd = new CommandLine.Builder(args) //
 ...
 * </pre>
 *
 * <p>This will interpret all command line options of the form <code>config:*</code> as options that
 * are supposed to be inserted into the MATSim config. The rules are as follows:
 *
 * <ul>
 *   <li><code>--config:MODULE.PARAM VALUE</code> sets a certain parameter with name <code>PARAM
 *       </code> in the module <code>MODULE</code> to the value <code>VALUE</code>. The way this
 *       passed value is intepreted is up to the MATSim {@link ConfigGroup}, just as if the value
 *       would have come from the configuration file.
 *   <li><code>--config:MODULE.SET_TYPE[ID_PARAM=ID_VALUE].PARAM VALUE</code> sets a value in a
 *       specific parameter set, which is identified using a specific parameter in that set with a
 *       specific selection value.
 *   <li><code>--config:MODULE.SET_TYPE[*=*].PARAM VALUE</code> sets a value in <i>all</i> parameter
 *       sets of SET_TYPE
 * </ul>
 *
 * Some examples:
 *
 * <ul>
 *   <li><code>--config:global.numberOfThreads 48</code>
 *   <li><code>--config:strategy.strategysettings[strategyName=ReRoute].weight 0.0</code>
 *   <li><code>
...
public class CommandLine {
  ...
  public static class Builder {
    private final Set<String> allowedPrefixes = new HashSet<>(Collections.singleton(CONFIG_PREFIX));
    ...
    /**
     * Passing <code>false</code> to this function disables unnamed (positional) command line
     * arguments. The default is to allow them.
     */
    ...
    /**
     * Passing <code>true</code> to this function allows any named command line option to be passed
     * to the application. By default only those which have been defined explicitly are accepted.
     */
    ...
    /**
     * Adds a list of allowed command line options to the application. If not configured otherwise
     * (see {@link #allowAnyOption}), arguments that have not been added explicitly will raise an
     * error.
     */
    ...
    /**
     * Adds a list of required command line options. If those options are not present an error will
     * be raised.
     */
    ...
    /**
     * Allows a list of prefixes, which means that all command line options of the form <code>
     * PREFIX:*</code> can be used for the application. Alternatively, the application can be
     * configured to allow any named command line argument (see {@link #allowAnyOption}).
     */
    ...
    /**
     * Builds the command line interpreter with the given configuration. An {@link
     * ConfigurationException} may be thrown directly in one of the following cases:
     *
     * <ul>
     *   <li>An unnamed (positional) command line argument is passed, although it is forbidden (see
     *       {{@link #allowPositionalArguments(boolean)})
     *   <li>A named command line argument is passed, but it is not allowed (see {@link
     *       #allowOptions(Collection)}. This can be disabled via {@link #allowAnyOption}.
     *   <li>A named command line argument with a prefix is passed, but the prefix is not allowed
     *       (see {@link #allowPrefixes(Collection)}. This can be disabled via {@link
     *       #allowAnyOption}.
     *   <li>Some command line options have been defined as required (see {@link
     *       #requireOptions(Collection)}), but are not present.
     * </ul>
     *
  ...
  /**
   * Returns the positional (unnamed) command line argument with index <code>index</code> if
   * present.
   */
  ...
  /**
   * Returns the positional (unnamed) command line argument with index <code>index</code> if present
   * or raises a {@link ConfigurationException}.
   *
   * @throws ConfigurationException
   */
  ...
  /**
   * Returns a named command line option if it is present, raises a {@link ConfigurationException}
   * otherwise.
   *
   * @throws ConfigurationException
   */
   ...
   *   <li>but makes sure that <code>["param[key=value]"]</code> is preserved
   * </ul>
   *
   * @param args
   * @return
  ...
  private void addPositionalArgument(String value) throws ConfigurationException {
    if (!positionalArgumentsAllowed) {
      throw new ConfigurationException(
          String.format("Positional argument '%s' is not allowed.", value));
    }
   ...
   * CommandLine} for more information on the syntax.
   *
   * @throws ConfigurationException
   */
  public void applyConfiguration(Config config) throws ConfigurationException {
    List<String> configOptions =
        options.keySet().stream()
            .filter(o -> o.startsWith(CONFIG_PREFIX + ":"))
  ...
  /**
   * Reports on the options that have been inserted into the MATSim {@link Config} via the command
   * line.
   */
  private void reportOptions() {
    ...
    logger.info(
        String.format(
            "Received %d command line options with %d prefixes:",
            options.size(), prefixedOptions.size()));
  ...
  private void processConfigOption(Config config, String option, String remainder)
      throws ConfigurationException {
  ...
  private void processParameter(
      String option, String path, ConfigGroup configGroup, String remainder)
      throws ConfigurationException {
    if (remainder.contains("[")) {
      ...
      if (selectorStartIndex > -1 && selectorEndIndex > -1 && equalIndex > -1) {
        if (selectorStartIndex < equalIndex && equalIndex < selectorEndIndex) {
          ...
          if (newRemainder.startsWith(".")) {
            ...
            String newPath =
                String.format(
                    "%s.%s[%s=%s]", path, parameterSetType, selectionParameter, selectionValue);

            Collection<? extends ConfigGroup> parameterSets =
                configGroup.getParameterSets(parameterSetType);

            // change values in *all* parameter sets of parameterSetType
            final boolean changeValueInAllSets =
                selectionParameter.equals("*") && selectionValue.equals("*");

            if (!parameterSets.isEmpty()) {
              for (ConfigGroup parameterSet : parameterSets) {
                if (changeValueInAllSets
                    || parameterSet.getParams().containsKey(selectionParameter)) {
                  ...
                  if (changeValueInAllSets || comparisonValue.equals(selectionValue)) {
                    ...
                    if (!changeValueInAllSets) {
                      // retain current behavior to only change value in first matching parameter
                      // set
                      return;
                    }
                  }
                  ...
                  if (parameterSetType.equals(ScoringParameterSet.SET_TYPE)
                      && selectionParameter.equals("subpopulation")
                      && selectionValue.equals("null")) {
                    processParameter(option, newPath, parameterSet, newRemainder);
                    return;
                  }
                }
      ...
      throw new ConfigurationException(
          String.format(
              "Malformatted parameter set selector: '%s' in %s. Expected %s.SET_TYPE[PARAM=VALUE].*",
              remainder, option, path));
    } else {
      if (configGroup.getParams().containsKey(remainder)) {
        ...
        throw new ConfigurationException(
            String.format("Parameter %s in %s is not available", remainder, path));
      }
    }
  }
