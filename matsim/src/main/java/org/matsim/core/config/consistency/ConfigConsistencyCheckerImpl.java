...
public final class ConfigConsistencyCheckerImpl implements ConfigConsistencyChecker {
  ...
  public ConfigConsistencyCheckerImpl() { // explicit constructor so that I can eclipse-search for
    // instantiation.  kai, may'11
    // nothing to do
  }
  ...
  static boolean checkConsistencyBetweenRouterAndTravelTimeCalculator(final Config config) {
    ...
    //				log.warn("AnalyzedModes in travelTimeCalculator config is not superset of network routing
    // modes.  Will fail later except when defined by other means.");
    //				problem = true ;
    //			}
    //		}
    // not sure if it ever worked like this, but now the analyzedModes are honoured only when
    // separateModes is false and so the check does not make
  ...
  /*package because of test */ static void checkPlanCalcScore(final Config c) {
    ...
    if (ptModeParams != null && ptModeParams.getMarginalUtilityOfTraveling() > 0) {
      log.warn(
          ScoringConfigGroup.GROUP_NAME
              + ".travelingPt is > 0. This values specifies a utility. "
    ...
    if (carModeParams != null && carModeParams.getMarginalUtilityOfTraveling() > 0) {
      log.warn(
          ScoringConfigGroup.GROUP_NAME
              + ".traveling is > 0. This values specifies a utility. "
              + "Typically, this should be a disutility, i.e. have a negative value.");
    }
    ...
    if (bikeModeParams != null && bikeModeParams.getMarginalUtilityOfTraveling() > 0) {
      log.warn(
          ScoringConfigGroup.GROUP_NAME
              + ".travelingBike is > 0. This values specifies a utility. "
              + "Typically, this should be a disutility, i.e. have a negative value.");
    }
    ...
    if (walkModeParams != null && walkModeParams.getMarginalUtilityOfTraveling() > 0) {
      log.warn(
          ScoringConfigGroup.GROUP_NAME
              + ".travelingWalk is > 0. This values specifies a utility. "
              + "Typically, this should be a disutility, i.e. have a negative value.");
    }
    ...
    if (ptAct != null) {
      ...
      //					throw new RuntimeException("setting the pt interaction activity closing time away from
      // 0/undefined is not allowed because it breaks pt scoring." +
      //					" If you need this anyway (for backwards compatibility reasons), you can allow this by
      // a parameter in VspExperimentalConfigGroup.") ;
      //				}
      //			}
      if (ptAct.isScoringThisActivityAtAll()) {
        if (!c.vspExperimental().isAbleToOverwritePtInteractionParams()) {
          throw new RuntimeException(
              "Scoring "
                  + ptAct.getActivityType()
                  + " is not allowed because it breaks pt scoring."
  ...
  private static void checkLaneDefinitionRoutingConfiguration(final Config config) {
    if ((config.qsim().isUseLanes()) && !config.controller().isLinkToLinkRoutingEnabled()) {
      log.warn(
  ...
  private static void checkTransit(final Config config) {
    ...
    //			log.warn("Your are using Transit but have not provided a transit vehicles file. This most
    // likely won't work.");
    //		}
    // The warning will also be logged when the vehicles are defined in code, which is perfectly
    // acceptable.  kai, jun'19
  }
}
