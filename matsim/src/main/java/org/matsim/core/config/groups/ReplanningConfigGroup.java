...
/**
 * Configuration group for specifying the plans-replanning to be used. It can still be specified
 * using the "underscored" way, but can only be written in hierarchical format "v2". Trying to write
 * v1 will result in the strategy settings being silently lost!
 *
 * @author mrieser
 */
public final class ReplanningConfigGroup extends ConfigGroup {
  ...
  // the "right" side is the outside interface, used in the config files, which is left with the old
  ...
  // yy could you please describe why this indirect design was done?  Was ist just because it made
  // refactoring easier, or does it provide
  // an advantage or is even necessary?  Thanks ...  kai, oct'14
  // To maintain backwards compatibility (underscored parameters), one needs to override the
  // "getValue" and "add_param" methods,
  ...
  public static class StrategySettings extends ReflectiveConfigGroup implements MatsimParameters {
    ...
    public StrategySettings(final Id<StrategySettings> id) {
      super(SET_NAME);
      this.id = id;
      //			this.strategyName = id.toString() ; // safety net, can be overridden by (also deprecated)
      // setStrategyName(...).  kai/mz, nov'15
      // putting the above into the code fails at least one test.  We would vote for removing that
      // test ...
    }
    ...
    public final Map<String, String> getComments() {
      ...
      map.put(
          "strategyName",
          "strategyName of strategy.  Possible default names: "
              + sels
              + " (selectors), "
              + strats
    ...
    @Deprecated // not clear if this is only for backwards compatibility (config v1) or should
    // actually be used. kai/mz, nov'15
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        ...
        "strategyName of PlanSelector for plans removal.  "
            + "Possible defaults: "
            + strb.toString()
    ...
    map.put(
        ...
        "the external executable will be called with a config file as argument.  This is the pathname to a possible "
    ...
    map.put(
        ...
        "root directory for temporary files generated by the external executable. Provided as a service; "
  ...
  /**
   * Adds the StrategySettings given as parameter to the map storing the settings for the
   * strategies. An IllegalArgumentException is thrown, if a StrategySEttings instance with the id
   * of the parameter already exists in the map.
   *
   * @param stratSets
   */
  ...
  private static class ReflectiveDelegate extends ReflectiveConfigGroup {
    ...
    static final String ITERATION_FRACTION_TO_DISABLE_INNOVATION =
