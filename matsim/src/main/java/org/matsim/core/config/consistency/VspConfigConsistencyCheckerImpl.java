...
public final class VspConfigConsistencyCheckerImpl implements ConfigConsistencyChecker {
  ...
  // VSP now regularly uses marg utls of travelling != null to fit distance distributions.  There
  // should be a switch to switch off the warnings.

  // VSP says that people < 18J should not use car, and implements that via car availability.  How
  // to handle that?
  ...
  public void checkConsistency(Config config) {
    ...
    if (config.qsim().getVehiclesSource() == VehiclesSource.fromVehiclesData
      ...
      log.log(
          lvl,
          "You can't use more than one main (=vehicular) mode while using the agent ID as missing vehicle ID ... "
    ...
    if (problem
        && config.vspExperimental().getVspDefaultsCheckingLevel()
            == VspDefaultsCheckingLevel.abort) {
  ...
  private static boolean checkStrategyConfigGroup(Config config, Level lvl, boolean problem) {
    ...
    // added may'16
    if (config.replanning().getFractionOfIterationsToDisableInnovation()
  ...
  private static boolean checkQsimConfigGroup(Config config, Level lvl, boolean problem) {
    ...
    if (config.qsim().getLinkDynamics() != QSimConfigGroup.LinkDynamics.PassingQ) {
      log.log(
          lvl,
          "found qsim.linkDynamics="
              + config.qsim().getLinkDynamics()
    ...
    //			log.log( lvl,  " found 'qsim.usingFastCapacityUpdate==false'; vsp should try out `true' and
    // report. ") ;
    //		}
    switch (config.qsim().getTrafficDynamics()) {
      ...
      default:
        log.log(
            lvl,
            " found 'qsim.trafficDynamics=="
                + config.qsim().getTrafficDynamics()
                + "'; vsp standard is`"
  ...
  private static boolean checkPlansConfigGroup(Config config, Level lvl, boolean problem) {
    ...
    if (actDurInterpr == PlansConfigGroup.ActivityDurationInterpretation.endTimeOnly) {
      ...
      if (config.transit().isUseTransit()) {
        ...
        log.error(
            "You are using "
                + config.plans().getActivityDurationInterpretation()
                + " as activityDurationInterpretation in "
    ...
    if (actDurInterpr == PlansConfigGroup.ActivityDurationInterpretation.minOfDurationAndEndTime) {
      ...
      log.log(
          lvl,
          "You are using ActivityDurationInterpretation "
              + config.plans().getActivityDurationInterpretation()
              + " ; vsp default is to use "
              + PlansConfigGroup.ActivityDurationInterpretation.tryEndTimeThenDuration
              + "This means you have to add the following lines into the vspExperimental section of your config file: ");
      log.log(
          lvl,
          "   <param name=\"activityDurationInterpretation\" value=\""
              + PlansConfigGroup.ActivityDurationInterpretation.tryEndTimeThenDuration
              + "\" />");
      log.log(
          lvl,
  ...
  private static boolean checkPlanCalcScoreConfigGroup(Config config, Level lvl, boolean problem) {
    ...
    for (ActivityParams params : config.scoring().getActivityParams()) {
      if (PtConstants.TRANSIT_ACTIVITY_TYPE.equals(params.getActivityType())) {
        // they have typicalDurationScoreComputation==relative, but are not scored anyways.
        // benjamin/kai, nov'15
        continue;
      }
      switch (params.getTypicalDurationScoreComputation()) {
        ...
        case uniform:
          ...
          log.log(
              lvl,
              "found `typicalDurationScoreComputation == uniform' for activity type "
    ...
    for (ModeParams params : config.scoring().getModes().values()) {
      if (params.getMonetaryDistanceRate() > 0.) {
        ...
        System.out.flush();
        log.error(
            "found monetary distance rate for mode "
                + params.getMode()
    ...
    if (config.scoring().getModes().get(TransportMode.car) != null
        && config.scoring().getModes().get(TransportMode.car).getMonetaryDistanceRate() > 0) {
      problem = true;
    }
    ...
    // added oct'17:
    if (config.scoring().getFractionOfIterationsToStartScoreMSA() == null
        || config.scoring().getFractionOfIterationsToStartScoreMSA() >= 1.) {
      problem = true;
      System.out.flush();
      log.log(
          lvl,
    ...
    // added apr'21:
    for (Map.Entry<String, ScoringConfigGroup.ScoringParameterSet> entry :
        config.scoring().getScoringParametersPerSubpopulation().entrySet()) {
      for (ActivityParams activityParam : entry.getValue().getActivityParams()) {
        if (activityParam.getMinimalDuration().isDefined()) {
          log.log(
              lvl,
              "Vsp default is to not define minimal duration.  Activity type="
                  + activityParam.getActivityType()
                  + "; subpopulation="
                  + entry.getKey());
        }
      }
    ...
    // added may'23
    for (ModeParams params : config.scoring().getModes().values()) {
      if (config.vspExperimental().getCheckingOfMarginalUtilityOfTravellng()
          == CheckingOfMarginalUtilityOfTravellng.allZero) {
        if (params.getMarginalUtilityOfTraveling() != 0.) {
          log.log(
              lvl,
              "You are setting the marginal utility of traveling with mode "
                  + params.getMode()
                  + " to "
                  + params.getMarginalUtilityOfTraveling()
      ...
      if (params.getMode().equals(TransportMode.walk) && params.getConstant() != 0.) {
        ...
        log.log(
            lvl,
            "You are setting the alternative-specific constant for the walk mode to "
                + params.getConstant()
  ...
  private static boolean checkLocationChoiceConfigGroup(Config config, boolean problem) {
    ...
    if (usingLocationChoice) {
      ...
      //				//			problem = true ;
      //				log.error("vsp will not accept location choice prob choice exponents other than 1 until
      // the corresponding warning in " +
