...
/**
 * A reader for config-files of MATSim. This reader recognizes the format of the config-file and
 * uses the correct reader for the specific config-version, without manual setting.
 *
 * @author mrieser
 */
public final class ConfigReader extends MatsimXmlParser {
   ...
   *     <p>As far as I can tell, pre-existing settings in config are preserved except if they are
   *     explicitly overwritten by entries in the config file. The MATSim4UrbanSim initialization
   *     sequence depends on this behavior. kn, mar'13
   */
  ...
  /**
   * Parses the specified config file, and uses the given dtd file as a local copy to use as dtd if
   * the one specified in the config file cannot be found.
   *
  ...
  public void readFile(final String filename, final String dtdFilename)
    ...
    // yyyyyy if this is a necessary/useful method, I would prefer it in the superclass.  kai,
    // jul'16
  ...
  // The following did override the inherited resolveEntity method.  But I have no idea why that may
  // have made sense.  kai, jul'16
  //	@Override
