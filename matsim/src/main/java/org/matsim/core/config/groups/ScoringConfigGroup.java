 ...
 *
 * <ul>
 *   <li>I have decided to modify those setters/getters that do not use SI units such that the units
 *       are attached. This means all the utility parameters which are "per hour" instead of "per
 *       second". kai, dec'10
 *   <li>Note that a similar thing is not necessary for money units since money units do not need to
 *       be specified (they are always implicit). kai, dec'10
 *   <li>The parameter names in the config file are <i>not</i> changed in this way since this would
 *       mean a public api change. kai, dec'10
 * </ul>
 *
 * @author nagel
 */
public final class ScoringConfigGroup extends ConfigGroup {
  ...
  public ScoringConfigGroup() {
    ...
    //                  <... modeParams ... >
    //                        < ... mode ... abc ... />
    //     (= fully hierarchical format), then the default modes will be removed before adding mode
    // abc.  The reason for this is that the second
    //     syntax clears the scoring params for the default subpopulation.

    //  Unfortunately, it continues:
    //  * Normally, we need a "clear defaults with first configured entry" (see
    // PlansCalcRouteConfigGroup).  Otherwise, we fail the write-read
    //  test: Assume we end up with a config that has _less_ material than the defaults.  Then we
    // write this to file, and read it back in.  If
    //  the defaults are not cleared, they would now be fully there.
    //  * The reason why this works here is that all the material is written out with the fully
    // hierarchical format.  I.e. it actually clears the
    //  defaults when being read it.

    // I am not sure if it can stay the way it is right now; took me several hours to understand it
    // (and fix a problem we had not by
    // trial-and-error but by understanding the root cause).  Considerations:
    // * Easiest would be to not have defaults.  However, defaults are helpful in particular to
    // avoid that everybody uses different parameters.
    // * We could also have the "manual addition triggers clearing" logic.  In
    // PlansCalcRouteConfigGroup I now have this with a warning, which
    // can be switched off with a switch.  I find this a good solution; I am, however, not 100%
    // certain that it is robust since that switch is a
    // "state" while "clearing the defaults" is an action, and I am not sure if they can be mapped
    ...
    // (this is there so that an empty config prints out at least one activity type, so that the
    // explanations of this
    // important concept show up e.g. in defaultConfig.xml, created from the GUI. kai, jul'17
    //			params.setScoringThisActivityAtAll(false); // no longer minimal when included here. kai,
    // jun'18
    ...
    this.addActivityParams(
        new ActivityParams(createStageActivityType(TransportMode.car))
            .setScoringThisActivityAtAll(false));
    this.addActivityParams(
        new ActivityParams(createStageActivityType(TransportMode.pt))
            .setScoringThisActivityAtAll(false));
    // (need this for self-programmed pseudo pt. kai, nov'16)
    this.addActivityParams(
        new ActivityParams(createStageActivityType(TransportMode.bike))
            .setScoringThisActivityAtAll(false));
    this.addActivityParams(
        new ActivityParams(createStageActivityType(TransportMode.drt))
            .setScoringThisActivityAtAll(false));
    this.addActivityParams(
        new ActivityParams(createStageActivityType(TransportMode.taxi))
            .setScoringThisActivityAtAll(false));
    this.addActivityParams(
        new ActivityParams(createStageActivityType(TransportMode.other))
            .setScoringThisActivityAtAll(false));
    this.addActivityParams(
        new ActivityParams(createStageActivityType(TransportMode.walk))
            .setScoringThisActivityAtAll(false));
  ...
  public void addParam(final String key, final String value) {
    ...
    // backward compatibility: underscored
      ...
      ModeParams modeParams =
          getOrCreateModeParams(key.substring("marginalUtlOfDistance_".length()));
      modeParams.setMarginalUtilityOfDistance(Double.parseDouble(value));
    } else if (key.startsWith("monetaryDistanceRate_")) {
      log.warn(key + msg);
      usesDeprecatedSyntax = true;
      ModeParams modeParams =
          getOrCreateModeParams(key.substring("monetaryDistanceRate_".length()));
    ...
    // backward compatibility: "typed" traveling
      ...
      this.getModes()
          ...
          .setMarginalUtilityOfTraveling(Double.parseDouble(value));
    ...
    // backward compatibility: "typed" util of distance
      ...
      this.getModes()
          .get(TransportMode.car)
          .setMarginalUtilityOfDistance(Double.parseDouble(value));
  ...
  public Collection<String> getActivityTypes() {
    if (getScoringParameters(null) != null)
      ...
      getScoringParametersPerSubpopulation()
          .values()
          .forEach(item -> activities.addAll(item.getActivityParamsPerType().keySet()));
      return activities;
    }
  }
  ...
  public Collection<String> getAllModes() {
    if (getScoringParameters(null) != null) {
      ...
      getScoringParametersPerSubpopulation()
          .values()
          .forEach(item -> modes.addAll(item.getModes().keySet()));
      return modes;
    }
  }

  public Collection<ActivityParams> getActivityParams() {
    if (getScoringParameters(null) != null) return getScoringParameters(null).getActivityParams();
    ...
    else throw new RuntimeException("Default subpopulation is not defined");
  }

  public Map<String, ModeParams> getModes() {
    if (getScoringParameters(null) != null) return getScoringParameters(null).getModes();
    else if (getScoringParameters(DEFAULT_SUBPOPULATION) != null)
  ...
  public Map<String, ScoringParameterSet> getScoringParametersPerSubpopulation() {
    ...
    final Collection<ScoringParameterSet> parameters =
        (Collection<ScoringParameterSet>) getParameterSets(ScoringParameterSet.SET_TYPE);
  ...
  protected final void checkConsistency(final Config config) {
    ...
    if (getScoringParametersPerSubpopulation().size() > 1) {
      if (!getScoringParametersPerSubpopulation()
          .containsKey(ScoringConfigGroup.DEFAULT_SUBPOPULATION)) {
        throw new RuntimeException(
            "Using several subpopulations in "
                + ScoringConfigGroup.GROUP_NAME
                + " requires defining a \""
                + ScoringConfigGroup.DEFAULT_SUBPOPULATION
                + " \" subpopulation."
                + " Otherwise, crashes can be expected.");
      }
    }
    //		if
    // (!config.plansCalcRoute().getAccessEgressType().equals(PlansCalcRouteConfigGroup.AccessEgressType.none)) {

    // there are modes such as pt or drt that need interaction params even without accessEgress
    // switched on.  The policy so far was that
    // they had to define them by themselves.  For drt, this needs to be done manually
    // (adjustDrtConfigGroup) since it is not a core
    // contrib.  At least from a user perspective, it will be easier if they are all generated here.
    //  Result of current variant is that they are now also generated
    // for situations where accessEgress routing is not switched on.  Since, in general, our data
    // model assumes that there is always
    // access/egress, I think that this is acceptable.  kai, jan'21

    // adding the interaction activities that result from access/egress routing. this is strictly
    // speaking not a consistency
    // check, but I don't know a better place where to add this. kai, jan'18

    for (ScoringParameterSet scoringParameterSet :
        this.getScoringParametersPerSubpopulation().values()) {

      for (String mode : config.routing().getNetworkModes()) {
        createAndAddInteractionActivity(scoringParameterSet, mode);
      }
      // (In principle, the for loop following next should be sufficient, i.e. taking the necessary
      // modes from scoring.
  ...
  private static void createAndAddInteractionActivity(
      ScoringParameterSet scoringParameterSet, String mode) {
    String interactionActivityType = createStageActivityType(mode);
    ActivityParams set =
        scoringParameterSet.getActivityParamsPerType().get(interactionActivityType);
    if (set == null) {
  ...
  public static class ActivityParams extends ReflectiveConfigGroup implements MatsimParameters {
    ...
    public static final String TYPICAL_DURATION_CMT =
    ...
    private String type;
    public static final String ACVITITY_TYPE_CMT =
  ...
  public static class ModeParams extends ReflectiveConfigGroup implements MatsimParameters {
    ...
    private static final String MONETARY_DISTANCE_RATE_CMT =
        "[unit_of_money/m] conversion of distance into money. Normally negative.";

    private static final String MARGINAL_UTILITY_OF_TRAVELING =
        "marginalUtilityOfTraveling_util_hr";

    private static final String CONSTANT = "constant";
    private static final String CONSTANT_CMT =
    ...
    private static final String DAILY_MONETARY_CONSTANT_CMT =
  ...
  public static class ScoringParameterSet extends ReflectiveConfigGroup {
    ...
    /**
     * This method is there to make the StringSetter/Getter automagic happy, but it is not meant to
     * be used.
     */
  ...
  private static class ReflectiveDelegate extends ReflectiveConfigGroup {
    ...
    public void setUsingOldScoringBelowZeroUtilityDuration(
      ...
      //				throw new RuntimeException( "using old scoringBelowZeroUtility duration is no longer
      // possible.  Use matsim version 14.0 " +
