...
/**
 * Stores all configuration settings specified in a configuration file and provides access to the
 * settings at runtime.
 *
...
public final class Config implements MatsimExtensionPoint {
  ...
  /**
   * Adds all the commonly used config-groups, also known as "core modules", to this
   * config-instance. This should be called before reading any configuration from file.
   */
  public void addCoreModules() {
    ...
    this.modules.put(
        TravelTimeCalculatorConfigGroup.GROUPNAME, new TravelTimeCalculatorConfigGroup());
    ...
    this.modules.put(
        TimeAllocationMutatorConfigGroup.GROUP_NAME, new TimeAllocationMutatorConfigGroup());
  ...
  /**
   * Checks each module for consistency, e.g. if the parameters that are currently set make sense in
   * their combination.
   */
  public void checkConsistency() {
    ...
    // has never been read." +
    //                        "This is probably an error: You may be expecting functionality which
    // is not available." +
   ...
   *
   * <p>This is the typical way to "materialize" material that, so far, exists only as Map, into a
   * specialized module.
   *
  ...
  public final void addModule(final ConfigGroup specializedConfigModule) {
    ...
    if (m != null) {
      ...
      if (m.getClass() == ConfigGroup.class
          && specializedConfigModule.getClass() != ConfigGroup.class) {
  ...
  /**
   * Returns the requested parameter. If the module or parameter is not known, an error is logged
  ...
  public final String getParam(final String moduleName, final String paramName) {
    ...
    if (str == null) {
      String message =
          "Parameter \"" + paramName + "\" of module \"" + moduleName + "\" is not known";
      log.error(message);
  ...
  // ////////////////////////////////////////////////////////////////////
  /**
   * Sets the parameter <code>paramName</code> in the module/config-group <code>moduleName</code> to
   * the specified value. If there is no config-group with the specified name, a new group will be
   * created.
   *
   * @param moduleName
   * @param paramName
  ...
  public TravelTimeCalculatorConfigGroup travelTimeCalculator() {
    return (TravelTimeCalculatorConfigGroup)
        this.getModule(TravelTimeCalculatorConfigGroup.GROUPNAME);
  }
  ...
  public TimeAllocationMutatorConfigGroup timeAllocationMutator() {
    return (TimeAllocationMutatorConfigGroup)
        this.getModule(TimeAllocationMutatorConfigGroup.GROUP_NAME);
  }
  ...
  public void addConfigConsistencyChecker(final ConfigConsistencyChecker checker) {
    ...
    if (!alreadyExists) {
      ...
      log.info(
          "ConfigConsistencyChecker with runtime type="
              + checker.getClass()
  ...
  public void removeConfigConsistencyChecker(final Class clazz) {
    // I am not saying that I like this.  But I would like to be able to check config consistency
    // before the iterator is created, but by then we still have
    // unmaterialized config groups, and so I need to remove that checker at that point.  Maybe we
    // can sort this in some different way ...
