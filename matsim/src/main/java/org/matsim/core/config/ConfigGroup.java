...
public class ConfigGroup implements MatsimExtensionPoint {
  ...
  private static final Logger log = LogManager.getLogger(ConfigGroup.class);
  ...
  /**
   * Little helper for subclasses (i.e. the ConfigGroups). This method adds the value of the
   * parameter to the given map only if the getValue() method of this Module doesn't return null
   * (Java null-type) or the String representation of null, i.e. "null" or "NULL". If the value is
   * null, the string "null" is added to the map to document the parameter.
   *
   * @param map
   * @param paramName
   */
  ...
  protected void checkConsistency(Config config) {
    // (I added Config as a parameter, since there are many occasions where the validity of a
    // ConfigGroup can only be checked when other
    // material is known.  Could put all of this in the "global" config consistency checker, but if
    // it conceptually belongs into the
    // ConfigGroup, I think it is easier to have it more local.  Wasn't a big problem, since this
    // method is _only_ called from the global config
  ...
  //		throw new IllegalArgumentException( "the \"name\" of parameter sets should correspond to their
  // type."+
  ...
  /**
   * Method called on parameter sets added by the add methods. Can be extended if there are
   * consistency checks to makes, for instance if parameter sets of a given type should be instances
  ...
  /**
   * Useful for instance if default values are provided but should be cleared if user provides
   * values.
   */
  ...
  public static URL getInputFileURL(URL context, String filename) {
    ...
    if (filename.startsWith("/")) {
      ...
      // Absolute filename on Windows, when obtained through URL.toURI().getPath() starts with `/`,
      // like on Unix.
      return IOUtils.getFileUrl(filename);
    }
