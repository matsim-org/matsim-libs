...
public final class QSimConfigGroup extends ReflectiveConfigGroup {
  ...
  private static final Logger log = LogManager.getLogger(QSimConfigGroup.class);
  ...
  /* package */ static final String REMOVE_STUCK_VEHICLES_STRING =
  ...
  private static final String USE_PERSON_ID_FOR_MISSING_VEHICLE_ID =
      "usePersonIdForMissingVehicleId";
  private static final String SIM_ENDTIME_INTERPRETATION = "simEndtimeInterpretation";
  /* package */ static final String STUCK_TIME_STRING =
  ...
  @Positive private double timeStepSize = 1.0;
  @PositiveOrZero private double snapshotPeriod = 0; // off, no snapshots
  @Positive private double flowCapFactor = 1.0;
  @Positive private double storageCapFactor = 1.0;
  @Positive private double stuckTime = 10;
  private boolean removeStuckVehicles = false;
  private boolean usePersonIdForMissingVehicleId = true;
  @Positive private int numberOfThreads = 1;
  //	private static final String CREATING_VEHICLES_FOR_ALL_NETWORK_MODES =
  // "creatingVehiclesForAllNetworkModes";
  ...
  private StarttimeInterpretation simStarttimeInterpretation =
      StarttimeInterpretation.maxOfStarttimeAndEarliestActivityEnd;

  // Vehicles of size (in PCU) smaller than or equal to this threshold will be allowed to enter the
  // buffer even
  // if flowcap_accumulate <= 0.
  // The default value is 0.0 meaning that all vehicles of non-zero sizes will be let into buffer
  // only
  // if flowcap_accumulate > 0.
  //
  // Flow capacity easing prevents buses from waiting long time for entering the buffer in
  // sub-sampled scenarios
  // For instance, for 10% scenario, a car (representing 10 cars) has a size of 1.0 PCU, a bus
  // (representing 1 bus)
  // has a size of 0.3 PCU, and link flow capacities are reduced to about 0.1 of the original
  // capacity.
  // (1) If pcuThresholdForFlowCapacityEasing == 0, all buses moving just behind private cars wait
  // long times before
  // entering the buffer (recovering the flow capacity accumulator in a 10% scenario takes approx.
  // 10 times longer than
  // in the 100% scenario).
  // (2) If pcuThresholdForFlowCapacityEasing == 0.3, buses at the front of the queue immediately
  // enter the buffer
  // (once they arrive at the end of a link). If they (one or more buses) have been queued behind a
  // private car,
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        NUMBER_OF_THREADS,
        "Number of threads used for the QSim.  "
    ...
    map.put(
        INSERTING_WAITING_VEHICLES_BEFORE_DRIVING_VEHICLES,
    ...
    map.put(
        USE_PERSON_ID_FOR_MISSING_VEHICLE_ID,
        "If a route does not reference a vehicle, agents will use the vehicle with the same id as their own.");
    map.put(
        FAST_CAPACITY_UPDATE,
        "If false, the qsim accumulates fractional flows up to one flow unit in every time step.  If true, "
    ...
    map.put(USE_LANES, "Set this parameter to true if lanes should be used, false if not.");
    {
      ...
      map.put(
          VEHICLES_SOURCE,
          "If vehicles should all be the same default vehicle, or come from the vehicles file, "
              + "or something else.  Possible values: "
              + stb);
    }
    map.put(
        SEEP_MODE,
        "If link dynamics is set as "
            ...
            + ", set a seep mode. Default is bike.");
    map.put(
        IS_SEEP_MODE_STORAGE_FREE,
        "If link dynamics is set as "
            + LinkDynamics.SeepageQ
            + ", set to true if seep mode do not consumes any space on the link. Default is false.");
    map.put(
        IS_RESTRICTING_SEEPAGE,
        "If link dynamics is set as "
            + LinkDynamics.SeepageQ
            + ", set to false if all seep modes should perform seepage. Default is true (better option).");
    map.put(
        FILTER_SNAPSHOTS,
        "If set to "
            + FilterSnapshots.withLinkAttributes
            + " snapshots will only be generated for links which include "
            + SnapshotWritersModule.GENERATE_SNAPSHOT_FOR_LINK_KEY
            + " as attribute key. Default is no filtering.");
    //		map.put(CREATING_VEHICLES_FOR_ALL_NETWORK_MODES, "If set to true, creates a vehicle for each
    // person corresponding to every network mode. However, " +
  ...
  /**
   * Sets the number of seconds the simulation should advance from one simulated time step to the
   * next.
   */
  ...
  public enum TrafficDynamics {
    queue,
    withHoles,
    kinematicWaves //  MATSim-630; previously, the switch was InflowConstraint.maxflowFromFdiag.
    // Amit Jan 2017.
  }

  /**
   * Defines how the qsim sets the inflow and/or how it reacts to link attributes which are
   * inconsistent with regard to the fundamental diagram. <br>
   * <li>Note that {@code MAX_CAP_FOR_ONE_LANE} is backwards-compatible but always sets the inflow
   *     capacity to the maximum according to the fundamental diagram for one lane, so it
   *     essentially sets the inflow capacity too low for multiple-lane-links.
   * <li>{@code INFLOW_FROM_FDIAG} sets the inflow capacity to maximum flow capacity according to
   *     the fundamental diagram, assuming the nr of lanes in the link attributes to be correct.
   * <li>{@code NR_OF_LANES_FROM_FDIAG} sets the number of lanes to minimum required according to
   *     the fundamental diagram, assuming the flow capacity in the link attributes to be correct.
   */
  ...
  public void setNumberOfThreads(final int numberOfThreads) {
    if (numberOfThreads < 1) {
      throw new IllegalArgumentException(
          "Number of threads must be strictly positive, got " + numberOfThreads);
    }
  ...
  // ---
  private static final String INSERTING_WAITING_VEHICLES_BEFORE_DRIVING_VEHICLES =
  ...
  // (yyyyyy switch this default to true; false has really weird consequences sometimes (vehicles
  // waiting for hours in driveway;
  ...
  public final void setVehiclesSource(VehiclesSource source) {
    // yyyy This setting triggers behavior in PrepareForSim, the result of which is also used by the
    // router.  A better place for this switch might be in the vehicles config group. kai, may'21
    testForLocked();
  ...
  //		// yyyy do we really need this switch?  Quite in general, please try to avoid boolean
  // switches.  kai, may'18
