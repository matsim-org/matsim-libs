...
public final class ControllerConfigGroup extends ReflectiveConfigGroup {
  ...
  public enum EventTypeToCreateScoringFunctions {
    IterationStarts,
    BeforeMobsim
  }
  ...
  private EventTypeToCreateScoringFunctions eventTypeToCreateScoringFunctions =
      EventTypeToCreateScoringFunctions.IterationStarts;
  ...
  private Set<EventsFileFormat> eventsFileFormats =
      Collections.unmodifiableSet(EnumSet.of(EventsFileFormat.xml));
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        ...
        "The type of routing (least cost path) algorithm used, may have the values: "
            + Arrays.toString(RoutingAlgorithmType.values()));
    map.put(
        RUNID,
    ...
    map.put(
        ...
        "Default="
            ...
            + "; Specifies the file format for writing events. Currently supported: "
            + Arrays.toString(EventsFileFormat.values())
            + IOUtils.NATIVE_NEWLINE
            + "\t\t"
    ...
    map.put(
        LINKTOLINK_ROUTING_ENABLED,
        "Default=false. If enabled, the router takes travel times needed for turning moves into account."
    ...
    map.put(
        COMPRESSION_TYPE,
        "Compression algorithm to use when writing out data to files. Possible values: "
            + Arrays.toString(CompressionType.values()));
    map.put(
        EVENT_TYPE_TO_CREATE_SCORING_FUNCTIONS,
        "Defines when the scoring functions for the population are created. Default=IterationStarts. Possible values: "
            + Arrays.toString(EventTypeToCreateScoringFunctions.values()));

    map.put(
        MOBSIM,
        "Defines which mobility simulation will be used. Currently supported: "
            + Arrays.toString(MobsimType.values())
            + IOUtils.NATIVE_NEWLINE
            + "\t\t"
  ...
  /**
   * Sets whether graphs showing some analyses should automatically be generated during the
   * simulation. The generation of graphs usually takes a small amount of time that does not have
   * any weight in big simulations, but add a significant overhead in smaller runs or in test cases
   * where the graphical output is not even requested.
   *
  ...
  public void setEventTypeToCreateScoringFunctions(
  ...
  protected void checkConsistency(Config config) {
    if (config.controller().getOverwriteFileSetting()
