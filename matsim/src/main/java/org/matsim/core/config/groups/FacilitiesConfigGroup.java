...
public final class FacilitiesConfigGroup extends ReflectiveConfigGroup {
  ...
  private static final String INSISTING_ON_USING_DEPRECATED_ATTRIBUTE_FILE =
  ...
  // following params are required only if activitiesFacilities are generated internally (e.g.,
  // FacilitiesSource.onePerActivityLocationInPlansFile). Amit Jan'18
  ...
  // ASSIGNING_LINKS_TO_FACILITIES_IF_MISSING="assigningLinksToFacilitiesIfMissing";

  public static final String FACILITIES_ATTRIBUTES_DEPRECATION_MESSAGE =
      "using the separate facilities attribute file is deprecated.  Add the information directly into each facility, using "
  ...
  public Map<String, String> getComments() {
...

    {
      ...
      comments.put(
          FACILITIES_SOURCE,
          "This defines how facilities should be created. Possible values: " + options.toString());
    }

    comments.put(ID_PREFIX, "A prefix to be used for auto-generated IDs.");

    //		comments.put(ONE_FACILITY_PER_LINK, "Sets whether all activities on a link should be
    // collected within one ActivityFacility." +
    //				" Default is 'true'. If set to 'false', for each coordinate found in the population's
    // activities a separate ActivityFacility will be created.");

    //		comments.put(REMOVING_LINKS_AND_COORDINATES, "If set to 'true' (which is the default), the
    // link and coordinate attributes " +
    //				"are replaced by null reference in the activities, as this information is now available
    // via the facility.");
    //
    //		comments.put(ADD_EMPTY_ACTIVITY_OPTIONS, "If set to 'true', empty activity option will be
    // created.");
    //
    //		comments.put(ASSIGNING_OPENING_TIME, "If set to 'true', opening time will be assigned to
    // activity facilities from ActivityParams. Default is false. This will NOT override option
    // 'addEmptyActivityOption'.");

    //		comments.put(ASSIGNING_LINKS_TO_FACILITIES_IF_MISSING, "Default is 'true'. In the case that
    // a facility has no link assigned, the ActivityFacility can be assigned to the closest link." +
    //				" If there should be only one ActivityFacility per link and if no link-assignment should
    // be done, set to 'false'." +
    //				"This will create a new ActivityFacility at the coordinate and the facility will not be
    // assigned to a link.");
    return comments;
  }
  ...
  @Deprecated // I think that this should be phased out; use Attributes inside each facility.  kai,
  // mar'19
