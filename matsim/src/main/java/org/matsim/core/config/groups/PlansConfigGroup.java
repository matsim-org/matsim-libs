...
public final class PlansConfigGroup extends ReflectiveConfigGroup {
  ...
  public enum TripDurationHandling {
    /**
     * When routing along a plan, the nominal end times of activities are using as departure times
     * of the following trips
     */
    ignoreDelays,
    /**
     * When routing along a plan, travel times are accumulated and activity end times may be
  ...
  public Map<String, String> getComments() {
    ...
    //				" (as freight, through traffic, etc.). The attribute must be of String type.  Change away
    // from default only in desperate situations." );

    StringBuilder str = new StringBuilder();
    for (PlansConfigGroup.ActivityDurationInterpretation itp :
        PlansConfigGroup.ActivityDurationInterpretation.values()) {
      str.append(" ").append(itp.toString());
    }
    comments.put(
        ACTIVITY_DURATION_INTERPRETATION,
        "String:"
            + str
            + ". Anything besides "
            + PlansConfigGroup.ActivityDurationInterpretation.minOfDurationAndEndTime
            + " will internally use a different "
            + "(simpler) version of the TimeAllocationMutator.");
    comments.put(
        TRIP_DURATION_HANDLING,
        "Defines how departure times are interpreted in rerouting applications. If set to '"
            + TripDurationHandling.ignoreDelays
            + "', "
            + "the departure time of a trip when routing along a plan will always be the nominal (plan-based) activity end time. If set to '"
    ...
    comments.put(
        REMOVING_UNNECESSARY_PLAN_ATTRIBUTES,
        "(not tested) will remove plan attributes that are presumably not used, such as "
  ...
  private static final String INSISTING_ON_USING_DEPRECATED_PERSON_ATTRIBUTE_FILE =
      "insistingOnUsingDeprecatedPersonAttributeFile";
  ...
  public static enum HandlingOfPlansWithoutRoutingMode {
    reject,
    useMainModeIdentifier
  }

  private HandlingOfPlansWithoutRoutingMode handlingOfPlansWithoutRoutingMode =
      HandlingOfPlansWithoutRoutingMode.reject;
  private static final String HANDLING_OF_PLANS_WITHOUT_ROUTING_MODE =
  ...
  public static final String PERSON_ATTRIBUTES_DEPRECATION_MESSAGE =
      "using the separate person attribute file is deprecated.  Add the information directly into each person, using "
  ...
  //	/**
  //	 * @deprecated -- use {@link
  // org.matsim.core.population.PopulationUtils#getSubpopulation(Person, Config)}
  //	 */
  //	@Deprecated
  ...
  public void setActivityDurationInterpretation(
      final PlansConfigGroup.ActivityDurationInterpretation actDurInterpret) {
  ...
  private static final String REMOVING_UNNECESSARY_PLAN_ATTRIBUTES =
  ...
  public void setRemovingUnneccessaryPlanAttributes(
      final boolean removingUnneccessaryPlanAttributes) {
  ...
  /**
   * @deprecated -- replace extraction of subpopulation by {@link
   *     PopulationUtils#getSubpopulation(HasPlansAndId)}
   */
  @Deprecated
  public String getSubpopulationAttributeName() {
