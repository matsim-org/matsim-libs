...
public final class ScenarioConfigGroup extends ConfigGroup {
  ...
  private static final String SIMULATION_PERIOD_DAYS =
      "simulationPeriodInDays"; // is not yet written to log-output so we can still rename it
  ...
  public Map<String, String> getComments() {
    ...
    map.put(
        USE_TRANSIT,
        "Deprecated, do not use.  See transit section of config file."); // since jul'15
  ...
  public final void addParam(final String paramName, final String value) {
    ...
    if (USE_LANES.equals(paramName)) {
      ...
      throw new IllegalArgumentException(
          "Parameter '" + paramName + "' is not supported by config group '" + GROUP_NAME + "'.");
    }
  }
  ...
  // Once the methods below are removed throughout the code, those exceptions can be moved into the
  // addParam method.
  ...
  public void setUseTransit(@SuppressWarnings("unused") final Boolean b) {
    throw new RuntimeException(
        "The " + USE_TRANSIT + " switch has moved to the transit section of the config file.");
  }
  ...
  private static String getMessage(String module) {
    return "The "
        + module
        + " switch is no longer operational.  The file is loaded if the file name"
