...
public final class ConfigAliases {
  ...
  private static final Logger LOG = LogManager.getLogger(ConfigAliases.class);
  ...
  public void addAlias(String oldName, String newName, String... path) {
    this.aliases
        .computeIfAbsent(oldName, k -> new ArrayList<>(2))
        .add(new ConfigAlias(oldName, newName, path));
  }
  ...
  public String resolveAlias(String oldName, Deque<String> pathStack) {
    ...
    for (ConfigAlias alias : definedAliases) {
      ...
      if (matches) {
        ...
        LOG.warn(
            "Config name '{}' is deprecated, please use '{}' instead. Config path: {}",
            oldName,
            alias.newName,
            stack);
