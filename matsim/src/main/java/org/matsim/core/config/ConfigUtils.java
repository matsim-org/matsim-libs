...
public class ConfigUtils implements MatsimExtensionPoint {
  ...
  public static Config loadConfig(final String filename, ConfigGroup... customModules)
      throws UncheckedIOException {
    return loadConfig(IOUtils.resolveFileOrResource(filename), customModules);
  }

  /**
   * This variant is meant such that one can have a command line call to MATSim that first provides
   * a config file, and then overrides some of it. Should be particularly useful for integration
   * tests for main methods, since, if those main methods are using this method here, it should be
   * possible to test them via
   *
   * <pre>
   *        ...main( <config.xml> --config:controler.outputDir=... )
   *  </pre>
   *
   * i.e. the current necessity to break runnable scripts into pieces to change things like output
   * directory or lastIteration should be gone with this.
   */
  ...
  /**
   * A standard version, where we ignore the 1st argument (which should be the config file), and
   * take everything afterwards. Standard usage should be something like
   *
   * <pre>
  ...
  public static Config loadConfig(
      Config config, final URL url, String[] typedArgs, ConfigGroup... customModules) {
  ...
  public static Config loadConfig(final URL url, ConfigGroup... customModules)
      throws UncheckedIOException {
  ...
  public static void loadConfig(final Config config, final URL url, ConfigGroup... customModules)
      throws UncheckedIOException {
    if (config.global() == null) {
      config.addCoreModules();
    }
  ...
  /**
   * Adds the <code>pathPrefix</code> in front of all supported file-paths in core-config groups in
   * the configuration, except when the paths are specified as absolute paths. This is useful e.g.
   * if the configuration contains file-paths relative to the location of the config-file. The
   * currently supported file-paths are:
   *
   * <ul>
   *   <li>global.localDtdBase
   *   <li>controler.outputDirectory
   *   <li>network.inputNetworkFile
   *   <li>plans.inputPlansFile
   *   <li>facilities.inputFacilitiesFile
   *   <li>counts.inputCountsFile
   *   <li>world.inputWorldFile
   *   <li>households.inputFile
   *   <li>roadpricing.tollLinksFile
   * </ul>
   *
   * @param config
   * @param pathPrefix
  ...
  public static void modifyFilePaths(final Config config, final String pathPrefix) {
    ...
    config
        .controller()
        .setOutputDirectory(getAbsolutePath(prefix, config.controller().getOutputDirectory()));
  ...
  @Deprecated // using this vs not using this can change results, presumably because strategies may
  // be used in a different sequence from the registry.
  ...
  /**
   * Convenience method to all addOrGetModule with only two arguments. <br>
   * Notes:
   *
   * <ul>
   *   <li>Seems to be really slow, so don't use in inner loop.
   * </ul>
   *
   * @param config
   * @param moduleClass
   ...
   * Instructions: If you want to remove e.g. Config.vspExperimental(), replace that method with
   * ConfigUtils.addOrGetModule(this, VspExperimentalConfigGroup.GROUP_NAME,
   * VspExperimentalConfigGroup.class) and then hit Refactor/Inline. <br>
   * Notes:
   *
   * <ul>
   *   <li>Seems to be really slow, so don't use in inner loop.
   * </ul>
   */
  public static <T extends ConfigGroup> T addOrGetModule(
