...
/**
 * Config Module for PlansCalcRoute class. Here you can specify the scale factors of freespeed
 * travel time which are used as travel time for not microsimulated modes.
 *
 * @author dgrether
 * @author mrieser
 */
public final class RoutingConfigGroup extends ConfigGroup {
  // yy There is a certain degree of messiness in this class because of retrofitting, e.g. making
  // beelineDistance mode-specific while
  // being backwards compatible.  This could eventually be cleaned up, maybe about a year after
  // introducing it.  kai, jun'15
  ...
  public enum AccessEgressType {
    ...
    /**
     * Euclidian distance from facility to nearest point on link; then teleported walk. In normal
     * cases, all activities that belong to the same link are mapped into the same facility; in
     * consequence, in that situation all will have the same walk time. kai, may'23
     */
    accessEgressModeToLink,

    /**
     * The walk time to the link is taken from a link attribute, and in consequence the same for all
     * agents walking to that link.
     */
    walkConstantTimeToLink,

    /**
     * The sum of the two above. I.e. walking the Euklidean distance PLUS a time taken from the link
     * attribute.
     */
  ...
  private static final String ACCESSEGRESSTYPE_CMT =
  ...
  private static final String CLEAR_MODE_ROUTING_PARAMS_CMT =
      "Some typical teleportation routing params are set by default, such as for walk and bike.  "
          + "Setting this switch to \"true\" will clear them.  Note that this will also clear "
          + "settings for helper modes such as for "
   ...
   */
  public static final class ModeRoutingParams extends TeleportedModeParams {
    ...
    @Override
    public ModeRoutingParams setBeelineDistanceFactor(Double beelineDistanceFactor) {
      super.setBeelineDistanceFactor(beelineDistanceFactor);
      return this;
    }
  }

  public static class TeleportedModeParams extends ReflectiveConfigGroup
    ...
    private static final String TELEPORTED_MODE_FREESPEED_FACTOR_CMT =
    ...
    private static final String TELEPORTED_MODE_FREESPEED_LIMIT_CMT =
     ...
     */
    public TeleportedModeParams setTeleportedModeFreespeedLimit(
    ...
    public TeleportedModeParams setTeleportedModeSpeed(Double teleportedModeSpeed) {
      ...
      if (getTeleportedModeFreespeedFactor() != null && teleportedModeSpeed != null) {
        throw new IllegalStateException(
            "cannot set both speed and freespeed factor for " + getMode());
      }
  ...
  public RoutingConfigGroup() {
    ...
    // the following two are deliberately different from "walk" since "walk" may become a network
    // routing mode, but these two
    // will not. kai, dec'15
    {
    ...
    // I'm not sure if anyone needs the "undefined" mode. In particular, it doesn't do anything for
    // modes which are
    // really unknown, it is just a mode called "undefined". michaz 02-2012
    //
    // The original design idea was that some upstream module would figure out expected travel times
    // and travel distances
    // for any modes, and the simulation would teleport all those modes it does not know anything
    // about.
    // With the travel times and travel distances given by the mode.  In practice, it seems that
    // people can live better
    // with the concept that mobsim figures it out by itself.  Although it is a much less flexible
    // design.  kai, jun'2012
    //		{
    ...
    //		}
    // one might add the above but it was not added in the original design.  Not sure about the
    // reason. kai, feb'15
  ...
  /**
   * @deprecated -- use {@link #clearTeleportedModeParams()} to be consistent with naming in xml
   *     config.
   */
  public void clearModeRoutingParams() {
    // This is essentially a config switch, except that it cannot be set back to false once it was
    // set to true.
    ...
    // memorize that we have cleared; this is what will be written into the output config (once we
    // have one):
  ...
  public void addParameterSet(final ConfigGroup set) {
    if (set.getName().equals(TeleportedModeParams.SET_TYPE)
      ...
      log.warn(
          "    warning, use clearTeleportedModeParams(true) in code, and \""
              + CLEAR_MODE_ROUTING_PARAMS
              + "\"=true in xml config.");

      //						  "This functionality was removed for " );
      //			log.warn( "    some weeks in the development head, after release 11.x, and before release
      // 12.x; it is now back.  " +

      // A bit more info:
      //
      // (1) I wanted to keep the default teleportation routers ... since for novice users I find it
      // better if they all use the same teleportation
      // speeds.  kai, nov'19
      //
      // (2) The result of "add" and "remove" in code evidently depends on the sequence. In
      // contrast, our "config" object
      // is a state ... as one notices when we write it out, since there we cannot play back
      // additions and removal.  So if
      // we add to and remove from the default entries, and write the final result to file, then
      // re-reading these entries
      // needs to trigger removal of the defaults since otherwise they will exist in addition. kai,
      // nov'19
    }
    TeleportedModeParams pars = (TeleportedModeParams) set;
    // for the time being pushing the "global" factor into the local ones if they are not
    // initialized by
  ...
  public void addParam(final String key, final String value) {
    ...
    if (PT_SPEED_FACTOR.equals(key)) {
      ...
      setTeleportedModeSpeed(
          key.substring(TELEPORTED_MODE_SPEEDS.length()), Double.parseDouble(value));
    } else if (key.startsWith(TELEPORTED_MODE_FREESPEED_FACTORS)) {
      setTeleportedModeFreespeedFactor(
          key.substring(TELEPORTED_MODE_FREESPEED_FACTORS.length()), Double.parseDouble(value));
  ...
  public final Map<String, String> getComments() {
    ...
    map.put(
        ...
        "factor with which beeline distances (and therefore times) "
    ...
    map.put(
        RANDOMNESS,
        "strength of the randomness for the utility of money in routing under toll.  "
  ...
  @Deprecated // rather use addModeRoutingParams(...), since that allows further params (e.g.
  // beeline distance factor). kai, nov'19
  ...
  public void setBeelineDistanceFactor(double val) {
    testForLocked();
    // yyyy thinking about this: this should in design maybe not be different from the other
    // teleportation factors (reset everything
