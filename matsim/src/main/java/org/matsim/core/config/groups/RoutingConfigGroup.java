...
/**
 * Config Module for PlansCalcRoute class. Here you can specify the scale factors of freespeed travel time which are used as travel time for not
 * microsimulated modes.
 *
 * @author dgrether
 * @author mrieser
 */
public final class RoutingConfigGroup extends ConfigGroup {
	...
	public enum AccessEgressType {
		...
		/**
		 * Euclidian distance from facility to nearest point on link; then teleported walk. In normal cases, all activities that belong to the same
		 * link are mapped into the same facility; in consequence, in that situation all will have the same walk time. kai, may'23
		 */
	...
	private static final String CLEAR_MODE_ROUTING_PARAMS_CMT =
			"Some typical teleportation routing params are set by default, such as for walk and bike.  "
					+ "Setting this switch to \"true\" will clear them.  Note that this will also clear " + "settings for helper modes such as for "
	...
	public static class TeleportedModeParams extends ReflectiveConfigGroup implements MatsimParameters {
		...
		private static final String PERSON_SPEED_ATTRIBUTE_CMT =
				"The name of a person attribute that indicates their individual travel speed. Falling back to teleportedModeSpeed.";
		...
		public Map<String, String> getComments() {
			...
			map.put("teleportedModeSpeed", "Speed for a teleported mode. "
		...
		public TeleportedModeParams setTeleportedModeFreespeedFactor(Double teleportedModeFreespeedFactor) {
			testForLocked();
	...
	public RoutingConfigGroup() {
...

		{
			...
			ride.setBeelineDistanceFactor(null);
...

		{
			...
			pt.setBeelineDistanceFactor(null);
			addParameterSet(pt);
		}
	...
	public void addParameterSet(final ConfigGroup set, ConfigWriter.Verbosity verbosity) {
		if (set.getName().equals(TeleportedModeParams.SET_TYPE) && !this.acceptModeParamsWithoutClearing) {
			...
			// A bit more info:
			//
			// (1) I wanted to keep the default teleportation routers ... since for novice users I find it better if they all use the same
			// teleportation
			// speeds. kai, nov'19
			//
	...
	public void addParam(final String key, final String value) {
		if (value.equals("null"))
			return; // old behavior of reader: keep defaults if null
	...
	protected void checkConsistency(Config config) {
		...
		for (String mode : modesRoutedAsTeleportation) {
			if (modesRoutedAsNetworkModes.contains(mode)) {
				throw new RuntimeException(
						"mode \"" + mode + "\" is defined both as teleportation (mode routing param) and for network routing.  You need to remove "
								+ "one or the other.");
			}
		}
