...
public final class MutateActivityTimeAllocation implements PlanAlgorithm {
  ...
  public MutateActivityTimeAllocation(
  ...
  public void run(final Plan plan) {
    for (Activity act :
      ...
      if (act.getEndTime().isDefined()) {
        ...
        double newEndTime =
            Math.min(mutateTime(endTime, mutationRange), this.latestActivityEndTime);
  ...
  private void setLegDepartureTimes(Plan plan) {
    // setting leg departure times can only be an estimate and might be useful for certain dynamic
    // modes.
