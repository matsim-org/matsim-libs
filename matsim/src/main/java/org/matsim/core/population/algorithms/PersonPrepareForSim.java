...
/**
 * Performs several checks that persons are ready for a mobility simulation. It is intended to run
 * only once after the initial plans are read from file, as we expect that no "damage" happens to
 * the plans during the iterations. <br>
 * Currently, this only checks that in all plans the act's have a link assigned and that all plans
 * have valid routes, calculating missing links and routes if required. Additionally, it will output
 * a warning to the log if a person has no plans at all.
 *
 * @author mrieser
 */
public final class PersonPrepareForSim extends AbstractPersonAlgorithm {
   ...
   */
  public PersonPrepareForSim(
      final PlanAlgorithm router, final Scenario scenario, final Network carOnlyNetwork) {
    super();
    this.router = router;
        ...
        if (legs.size() >= 1) {
          ...
          for (Leg leg : legs) {
            ...
            if (existingRoutingMode == null) {
              if (routingMode == null) {
                ...
                String errorMessage =
                    "Found a mixed trip, some legs with routingMode and others without. "
          ...
          if (routingMode == null) {
            if (legs.size() == 1) {
              ...
              String errorMessage =
                  "Found a trip whose legs have no routingMode. "
      ...
      for (PlanElement pe : plan.getPlanElements()) {
        if (pe instanceof Activity) {
          ...
          if (act.getLinkId() == null // neither activity nor facility has a link
              &&
              // this check is necessary here, else, XY2Links will put the link/coord back to
              // activity which is clear violation of
              // facilitiesConfigGroup.removingLinksAndCoordinates =true. Amit July'18
              (act.getFacilityId() == null
                  ...
                  || this.activityFacilities.getFacilities().get(act.getFacilityId()).getLinkId()
                      == null)) {
            needsXY2Links = true;
            needsReRoute = true;
            break;
          ...
          if (leg.getRoute() == null) {
            ...
            if (leg.getRoute() instanceof NetworkRoute) {
               ...
               * tt feb'16
               */
              double relativePositionStartLink =
                  scenario.getConfig().global().getRelativePositionOfEntryExitOnLink();
              double relativePositionEndLink =
                  scenario.getConfig().global().getRelativePositionOfEntryExitOnLink();
              //							dist = RouteUtils.calcDistance((NetworkRoute) leg.getRoute(),
              // relativePositionStartLink, relativePositionEndLink, this.network);
              dist =
                  RouteUtils.calcDistance(
                      (NetworkRoute) leg.getRoute(),
                      relativePositionStartLink,
                      relativePositionEndLink,
                      scenario.getNetwork());
              ...
              DefaultTransitPassengerRoute newRoute =
