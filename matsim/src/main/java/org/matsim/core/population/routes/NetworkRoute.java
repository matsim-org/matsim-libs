...
/**
 * A route that describes a path in a (road) network. Thus, the route is described usually as a
 * series of links or nodes.
 *
 * @author mrieser
 */
public interface NetworkRoute extends Route {
  ...
  /**
   * Returns the list of link ids that build the route. The links where the route starts and ends
   * (the links where the activities are on) are <b>not</b> included in the list (note that they are
   * in getStartLinkId() and getEndLinkId() of the Route super-interface).
   *
  ...
  // yyyy my intuition is that this should be removed from the API (and replaced by a static method
  // in RouteUtils). kai, oct'17
  ...
  // Does it really make sense to couple the vehicle to the route?  I would have coupled it to the
  // leg.  kai, aug'10
