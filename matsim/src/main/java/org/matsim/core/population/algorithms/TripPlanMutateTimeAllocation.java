...
/**
 * Copy/Paste of PlanMutateTimeAllocation, but with special handling for stage activities (eg
 * transit interaction activities, like line changes): they are just ignored and not changed at all.
 *
 * @author mrieser
 */
public final class TripPlanMutateTimeAllocation implements PlanAlgorithm {
  ...
  public TripPlanMutateTimeAllocation(
      final double mutationRange, final boolean affectingDuration, final Random random) {
  ...
  public TripPlanMutateTimeAllocation(
    ...
    this(
        ...
        subpopulationAffectingDuration,
  ...
  private void mutatePlan(final Plan plan) {
    ...
    Activity lastAct =
        (Activity) plan.getPlanElements().listIterator(plan.getPlanElements().size()).previous();
    ...
    for (PlanElement pe : plan.getPlanElements()) {
      ...
      if (pe instanceof Activity act) {
          ...
          if (!StageActivityTypeIdentifier.isStageActivity(act.getType())) {
            if (this.useActivityDurations) {
              if (act.getMaximumDuration().isDefined()) {
                ...
                // (may feel a bit disturbing since it was not mutated but it is just using the
                // "old" value which is perfectly ok. kai, jan'14)
              ...
              if (act.getEndTime().isUndefined()) {
                throw new IllegalStateException(
                    "Can not mutate activity end time because it is not set for Person: "
        ...
        // assume that there will be no delay between end time of previous activity and departure
        // time
  ...
  private double mutateTime(final OptionalTime time, final double mutationRange) {
    if (time.isDefined()) {
      ...
      if (t < 0) t = 0;
      if (t > this.latestEndTime) t = this.latestEndTime;
      return t;
    } else {
