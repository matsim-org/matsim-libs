...
public class ParallelPopulationWriterHandlerV6 implements PopulationWriterHandler {
    ...
    if (workerThreads == null) {
      ...
      for (int i = 0; i < computeThreads; i++) {
        ...
        PersonStringCreator worker =
            new PersonStringCreator(this.coordinateTransformation, this.inputQueue);
  ...
  private void tryInitWriterThread(BufferedWriter out) {
    if (this.writeThread == null) {
      ...
      writeThread.setUncaughtExceptionHandler(
          (thread, exception) -> {
            LOG.warn("Exception while writing population.", exception);
  ...
  public void writeHeaderAndStartElement(BufferedWriter out) throws IOException {
    ...
    out.write(
        "<!DOCTYPE population SYSTEM \""
            + MatsimXmlWriter.DEFAULT_DTD_LOCATION
            + "population_v6.dtd\">\n\n");
  }
  ...
  public static class ParallelPopulationWriterV6 implements Runnable {
    ...
    ParallelPopulationWriterV6(
        BlockingQueue<CompletableFuture<String>> outputQueue, BufferedWriter out) {
  ...
  static class PersonStringCreator implements Runnable {
    ...
    PersonStringCreator(
        ...
        BlockingQueue<ParallelPopulationWriterHandlerV6.PersonData> inputQueue) {
      this.coordinateTransformation = coordinateTransformation;
    ...
    private void writePerson(PersonData personData)
    ...
    private void startPlan(final Plan plan, final StringBuilder out) {
      ...
      if (PersonUtils.isSelected(plan)) out.append(" selected=\"yes\"");
      else out.append(" selected=\"no\"");
