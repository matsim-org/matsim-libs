...
/**
 * Changes the transportation mode of all legs in a plan to a randomly chosen different mode (but
 * the same mode for all legs in that plan) given a list of possible modes.
 *
 * @author mrieser
 */
public final class ChooseRandomLegMode implements PlanAlgorithm {
   ...
   * @see MatsimRandom
   */
  public ChooseRandomLegMode(
      final String[] possibleModes, final Random rng, boolean allowSwitchFromListedModesOnly) {
  ...
  private int chooseModeOtherThan(final String currentMode) {
    ...
    for (int i = 0; i <= newModeIdx; i++) {
      if (this.possibleModes[i].equals(currentMode)) {
        ...
        // (it's length - 1, but the upper bound of nextInt() is exclusive, thus it's essentially
        // -2)
        // This gives us exactly the number of possibilities of possibleModes with the current mode
        // excluded.
        // Instead of just accessing this.possibleModes[newModeIdx] we loop through the possible
        // modes
        // to figure out if currentMode is before or after the new mode. If it is before, we skip it
        // by increasing the newModeIdx. In other words: If newModeIndex < currentModeIndex, we
        // account for
