...
/**
 * Parallel implementation of the PopulationReaderMatsimV6. The main thread only reads the file and
 * creates empty person objects which are added to the population to ensure that their order is not
 * changed. Note that this approach is not compatible with population streaming. When this feature
 * is activated, the non-parallel reader is used. The parallel threads interpret the xml data for
 * each person.
 *
...
/* deliberately package */ class ParallelPopulationReaderMatsimV6 extends PopulationReaderMatsimV6 {
  ...
  private final BlockingQueue<CompletableFuture<Person>> personInsertionQueue =
      new LinkedBlockingQueue<>();
  private Thread personInsertionThread;
  private Throwable exception = null;

  public ParallelPopulationReaderMatsimV6(
      final String inputCRS, final String targetCRS, final Scenario scenario) {
    ...
    this.isPopulationStreaming =
        scenario.getPopulation() instanceof StreamingPopulationReader.StreamingPopulation;
  ...
  private static void initObjectAttributeConverters(
      ParallelPopulationReaderMatsimV6Runner runner, ObjectAttributesConverter converter) {
    Map<String, AttributeConverter<?>> targetConverter =
        runner.getObjectAttributesConverter().getConverters();
  ...
  private void initThreads() {
    ...
    if (this.scenario.getPopulation() instanceof StreamingPopulationReader.StreamingPopulation) {
      this.personInsertionThread =
          new Thread(new PersonInserter(this.scenario.getPopulation(), this.personInsertionQueue));
  ...
  public void startTag(String name, Attributes atts, Stack<String> context) {
    ...
    if (PERSON.equals(name)) {
      ...
      Person person =
          this.plans
              .getFactory()
              .createPerson(Id.create(atts.getValue(ATTR_PERSON_ID), Person.class));
      ...
      tag.atts =
          new AttributesImpl(
              atts); // We have to create copies of the attributes because the object is re-used by
      // the parser!
  ...
  public static final class PersonInserter implements Runnable {
    Population population;
    BlockingQueue<CompletableFuture<Person>> personInsertionQueue;

    PersonInserter(
        Population population, BlockingQueue<CompletableFuture<Person>> personInsertionQueue) {
