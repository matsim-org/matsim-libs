...
public final class PersonUtils {
  ...
  /**
   * convenience method for often used demographic attribute There is apparently no way to register
   * a Map(String, Double) at ObjectAttributesConverter since all Maps default to
   * StringStringMapConverter and there is no way to register a StringDoubleMapConverter. Therefore,
   * the personal scoring mode constants uses a Map(String, String). If this attribute is read often
   * an alternative similar to PersonVehicles can be considered.
   */
  public static Map<String, String> getModeConstants(Person person) {
    try {

      return (Map<String, String>)
          person.getAttributes().getAttribute(PERSONAL_SCORING_MODE_CONSTANTS_ATTRIBUTE_NAME);
    } catch (Exception e) {
      log.error(
          "Error retrieving personalScoringModeConstants from attribute "
              + PERSONAL_SCORING_MODE_CONSTANTS_ATTRIBUTE_NAME
  ...
  /**
   * Car is not available if Person.getLicense gives "no" or if PersonUtils.getCarAvail returns
   * "never".
   */
  ...
  public static void setIncome(Person person, final double income) {
    person
        .getCustomAttributes()
        .put(PERSONAL_INCOME_ATTRIBUTE_NAME, income); // deprecated, should not be necessary anymore
  ...
  public static void setModeConstants(Person person, Map<String, String> mode2scoringConstant) {
    person
        .getAttributes()
        .putAttribute(PERSONAL_SCORING_MODE_CONSTANTS_ATTRIBUTE_NAME, mode2scoringConstant);
  }
