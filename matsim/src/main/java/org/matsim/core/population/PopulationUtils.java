...
public final class PopulationUtils {
  ...
  private static final PopulationFactory populationFactory =
      createPopulation(new PlansConfigGroup(), null).getFactory();

  /**
   * @deprecated -- this is public only because it is needed in the also deprecated method {@link
   *     PlansConfigGroup#getSubpopulationAttributeName()}
   */
  @Deprecated public static final String SUBPOPULATION_ATTRIBUTE_NAME = "subpopulation";

  /** Is a namespace, so don't instantiate: */
  private PopulationUtils() {}

  /**
   * Creates a new Population container. Population instances need a Config, because they need to
   * know about the modes of transport.
   *
  ...
  /**
   * Creates a new Population container which, depending on configuration, may make use of the
   * specified Network instance to store routes more efficiently.
   *
   * @param config the configuration which is used to create the Population.
  ...
  public static Population createPopulation(PlansConfigGroup plansConfigGroup, Network network) {
    ...
    if (PlansConfigGroup.NetworkRouteType.LinkNetworkRoute.equals(networkRouteType)) {
    ...
    } else if (PlansConfigGroup.NetworkRouteType.MediumCompressedNetworkRoute.equals(
            networkRouteType)
        && network != null) {
      factory = new MediumCompressedNetworkRouteFactory();
    } else if (PlansConfigGroup.NetworkRouteType.HeavyCompressedNetworkRoute.equals(
            networkRouteType)
        && network != null) {
      ...
      throw new IllegalArgumentException(
          "The type \"" + networkRouteType + "\" is not a supported type for network routes.");
    }
  ...
  public static Id<Link> computeLinkIdFromActivity(
      Activity act, ActivityFacilities facs, Config config) {
    ...
    if (act.getFacilityId() == null) {
      ...
      // yy sorry about this mess, I am just trying to make explicit which seems to have been the
      // logic so far implicitly.  kai, feb'16
    }
  }

  @Deprecated // use "decideOnCoord..."
  public static Coord computeCoordFromActivity(
      Activity act, ActivityFacilities facs, Config config) {
  ...
  @Deprecated // use "decideOnCoord..."
  public static Coord computeCoordFromActivity(
      Activity act, ActivityFacilities facs, Network network, Config config) {
    // the following might eventually become configurable by config. kai, feb'16
    if (act.getFacilityId() == null) {
  ...
  public static List<Activity> getActivities(
      Plan plan, StageActivityHandling stageActivityHandling) {
  ...
  public static double calculateSimilarity(
      ...
      List<Leg> legs2,
      Network network,
      double sameModeReward,
      double sameRouteReward) {
    // yyyy should be made configurable somehow (i.e. possibly not a static method any more).  kai,
    // apr'15

    // yy kwa points to:
    // Schüssler, N. and K.W. Axhausen (2009b) Accounting for similarities in destination choice
    // modelling: A concept, paper presented at the 9th Swiss Transport Research Conference, Ascona,
    // October 2009.
    //		und
    //  Joh, Chang-Hyeon, Theo A. Arentze and Harry J. P. Timmermans (2001).
    // A Position-Sensitive Sequence Alignment Method Illustrated for Space-Time Activity-Diary
    // Data¹, Environment and Planning A 33(2): 313­338.
    ...
    for (; it1.hasNext() && it2.hasNext(); ) {
      ...
      simil +=
          sameRouteReward
              * (RouteUtils.calculateCoverage(nr1, nr2, network)
                  + RouteUtils.calculateCoverage(nr2, nr1, network))
              / 2;
    }
    return simil;
  }
  ...
  public static double calculateSimilarity(
      ...
      double sameActivityLocationPenalty,
  ...
  /**
   * Compares two Populations by serializing them to XML with the current writer and comparing their
   * XML form byte by byte. A bit like comparing checksums of files, but regression tests won't fail
   * just because the serialization changes. Limitation: If one of the PopulationWriters throws an
   * Exception, this will go unnoticed (this method will just return true or false, probably false,
   * except if both Writers have written the exact same text until the Exception happens).
   */
  public static boolean equalPopulation(final Population s1, final Population s2) {
    try {
      try (InputStream inputStream1 = openPopulationInputStream(s1);
          InputStream inputStream2 = openPopulationInputStream(s2)) {
  ...
  /**
   * The InputStream which comes from this method must be properly resource-managed, i.e. always be
   * closed.
   *
   * <p>Otherwise, the Thread which is opened here may stay alive.
   */
              ...
              new Runnable() {
                ...
                public void run() {
                  ...
                  try {
                    ...
                    // Writer will throw an IOException when pipe is closed from the other side
                    // (like "broken pipe" in UNIX)
  ...
  public static PopulationFactory getFactory() {
    ...
    // But I am not sure if this really makes a lot of sense, because this static default factory is
    // easier to change into
  ...
  public static Activity createActivityFromFacilityId(
      String type, Id<ActivityFacility> facilityId) {
    return getFactory().createActivityFromActivityFacilityId(type, facilityId);
  }

  public static Activity createInteractionActivityFromFacilityId(
      String type, Id<ActivityFacility> facilityId) {
  ...
  public static Activity createActivityFromCoordAndLinkId(
      String type, Coord coord, Id<Link> linkId) {
    Activity act = getFactory().createActivityFromCoord(type, coord);
    act.setLinkId(linkId);
    return act;
  }

  public static Activity createInteractionActivityFromCoordAndLinkId(
      String type, Coord coord, Id<Link> linkId) {
  ...
  public static Activity createAndAddActivityFromFacilityId(
      Plan plan, String type, Id<ActivityFacility> facilityId) {
  ...
  public static Activity createAndAddActivity(Plan plan, String type) {
    ...
    // (can't do this from the factory since factory method only exists with coord or with linkId.
    // kai, jun'16)
    plan.addActivity(act);
    return act;
  }

  public static Activity createStageActivityFromCoordLinkIdAndModePrefix(
      final Coord interactionCoord, final Id<Link> interactionLink, String modePrefix) {
    Activity act =
        createInteractionActivityFromCoordAndLinkId(
            ScoringConfigGroup.createStageActivityType(modePrefix),
            interactionCoord,
            interactionLink);
  ...
  public static void copyFromTo(
    ...
    for (PlanElement pe : in.getPlanElements()) {
      if (pe instanceof Activity) {
         ...
         */
        if (withInteractionActivities
            && (pe
                instanceof
                InteractionActivity /* || StageActivityTypeIdentifier.isStageActivity(((Activity) pe).getType())*/)) {
  ...
  public static void copyFromTo(Activity act, Activity newAct) {
    Coord coord =
        act.getCoord() == null ? null : new Coord(act.getCoord().getX(), act.getCoord().getY());
    ...
    act.getMaximumDuration()
        .ifDefinedOrElse(newAct::setMaximumDuration, newAct::setMaximumDurationUndefined);
  ...
  public static Activity createInteractionActivity(Activity act) {
    Activity newAct =
        getFactory().createInteractionActivityFromLinkId(act.getType(), act.getLinkId());
  ...
  /**
   * Makes a deep copy of this leg, however only when the Leg has a route which is instance of Route
   * or BasicRoute. Other route instances are not considered.
   *
   * <ul>
  ...
  /**
   * Removes the specified leg <b>and</b> the following act, too! If the following act is not the
   * last one, the following leg will be emptied to keep consistency (i.e. for the route)
   *
   * @param index
   */
   ...
   *
   * @param pos the position where to insert the leg-act-combo. acts and legs are both counted from
   *     the beginning starting at 0.
   * @param leg the leg to insert
   * @param act the act to insert, following the leg
   * @throws IllegalArgumentException If the leg and act cannot be inserted at the specified
   *     position without retaining the correct order of legs and acts.
   */
  ...
  public static Id<Link> decideOnLinkIdForActivity(Activity act, Scenario sc) {
    if (act.getFacilityId() != null) {
      final ActivityFacility facility =
          sc.getActivityFacilities().getFacilities().get(act.getFacilityId());
      if (facility == null) {
  ...
  public static Coord decideOnCoordForActivity(Activity act, Scenario sc) {
    ...
    return new Coord(
        fromCoord.getX() + rel * (toCoord.getX() - fromCoord.getX()),
        fromCoord.getY() + rel * (toCoord.getY() - fromCoord.getY()));
  }
  ...
  public static void readPopulation(Population population, String filename) {
    ...
    // (yyyy population reader uses network to retrofit some missing geo information such as route
    // lenth.
    // In my opinion, that should be done in prepareForSim, not in the parser.  It is commented as
    // such
  ...
  public static Object getPersonAttribute(HasPlansAndId<?, ?> person, String key) {
    ...
    // (This was originally "if person instanceof Attributable then ...".  Since HasPlansAndId now
    // implements Attributable, this is in
    // principle always fulfilled.  However, if person==null, then person instanceof Attributable
    // also fails.  kai, jul'22)
  ...
  /**
   * @deprecated -- this command is dangerous since it might clear some else's attributes. Better
   *     just remove specificially the attributes that you "own". kai, may'19
   */
