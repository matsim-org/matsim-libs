...
/**
 * Assigns each activity in a plan a link where the activity takes place based on the coordinates
...
public final class XY2Links extends AbstractPersonAlgorithm implements PlanAlgorithm {
  ...
  /**
   * When running XY2Links with given facilities, the linkIds of activities are taken from the
   * facilities where they are performed. This ensures that the activity is performed at the same
   * link where the facility is located.
   */
    ...
    for (PlanElement planElement : planElements) {
      if (planElement instanceof Activity) {
        ...
        if (this.activityFacilities != null) {
          // since the facilities in Scenario are now permanently enabled, this can only happen when
          // called through the
          ...
          //							// yy facility.getLinkId may be null, in particular since linkId is not even part
          // of the facilities DTD. kai, feb'16
          //							// right, just had such a situation in CarSharingIT test. Amit Jul'18
          //
          //							if (act.getLinkId() == null && act.getCoord()==null){
          //								// for FacilitiesSource.onePerActivityLocationInPlansFile, one can opt to keep
          // coords in facility only. Amit Jan'18
        ...
        if (act.getCoord() == null) {
          ...
          final ActivityFacility activityFacility =
              this.activityFacilities.getFacilities().get(act.getFacilityId());
        ...
        if (null == link) {
          throw new RuntimeException(
              "For person id="
                  ...
                  + ": getNearestLink returned Null! act="
                  + act);
        }
