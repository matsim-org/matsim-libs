...
public class HeavyCompressedNetworkRouteFactory implements RouteFactory {
  ...
  /**
   * Uses {@link SubsequentLinksAnalyzer} to get the map of subsequent links, used to compress the
  ...
  public NetworkRoute createRoute(final Id<Link> startLinkId, final Id<Link> endLinkId) {
    ...
    if (this.compressionData == null) {
      Link[] subsequentLinks =
          new SubsequentLinksAnalyzer(this.network, this.preferredMode).getSubsequentLinks();
