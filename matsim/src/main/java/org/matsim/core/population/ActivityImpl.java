 ...
 *
 * <ul>
 *   <li>When we developed the API, we were afraid of potentially inconsistent Coord, LinkId and
 *       FacilityId. As a result, one can set the Activity either from Coord or from LinkId, but not
 *       from both. The FacilityId cannot be set at all (in the API).
 * </ul>
 */
/* package */ final class ActivityImpl implements Activity {
  ...
  // Case (X): facilityId inconsistent with linkId, coord.  Idea: mobsim takes the facilityId and
  // (a) checks the other
  ...
  public void setCoord(final Coord coord) {
    //		testForLocked();
    // I currently think that rather than enforcing data consistency we should just walk them from
    // coordinate to link. kai, dec'15
    this.coord = coord;
  }
  ...
  public Attributes getAttributes() {
    ...
    return new LazyAllocationAttributes(
