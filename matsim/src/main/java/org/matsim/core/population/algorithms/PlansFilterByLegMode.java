 ...
 *
 * <p>Plans which do not fulfill the filter criteria are removed from a person, Persons with no
 * plans are removed from the population.
 */
    ...
    while (pid_it.hasNext()) {
      ...
      for (int i = person.getPlans().size() - 1; i >= 0; i--) {
        ...
        if (this.legModeIsCar && never.equals(PersonUtils.getCarAvail(person))) {
          // person cannot drive car if she has no car. this means, the person was given a lift by
          // someone else
          // --> do not include this person, as we're only interested in the driver
          if (cnt < 1) {
            cnt++;
            log.warn(
                "This method assumes that mode=car without car availability means `in car as passenger'.  Note that this is different "
                    + "from what is often done in transport planning, where `in car as passenger' is a separate mode.");
            log.warn(Gbl.ONLYONCE);
          }
          hasSearchedLegMode = false;
        }
        if (filterType == FilterType.keepAllPlansWithMode
            || filterType == FilterType.keepPlansWithOnlyThisMode) {
          if ((!hasSearchedLegMode)
              || (hasOtherLegMode && this.filterType == FilterType.keepPlansWithOnlyThisMode)) {
      ...
      if (person.getPlans().isEmpty()) {
        // the person has no plans left. remove the person afterwards (so we do not disrupt the
        // Iterator)
