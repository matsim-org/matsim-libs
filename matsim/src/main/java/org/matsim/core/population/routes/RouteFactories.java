...
public final class RouteFactories {
  ...
  public RouteFactories() {
    ...
    this.setRouteFactory(
        DefaultTransitPassengerRoute.class, new DefaultTransitPassengerRouteFactory());
  }
  ...
  public <R extends Route> R createRoute(
      final Class<R> routeClass, final Id<Link> startLinkId, final Id<Link> endLinkId) {
    return (R)
        this.routeFactories
            .getOrDefault(routeClass, defaultFactory)
            .createRoute(startLinkId, endLinkId);
  }

  /**
   * Registers a {@link RouteFactory} for the specified route type. If <code>factory</code> is
   * <code>null</code>, the existing entry for this <code>routeClass</code> will be deleted. If
   * <code>routeClass</code> is <code>null</code>, then the default factory is set that is used if
   * no specific RouteFactory for a routeType is set.
   *
   * @param routeClass
   * @param factory
   */
