...
/**
 * A population reader that reads the MATSim format. This reader recognizes the format of the
 * plans-file and uses the correct reader for the specific plans-version, without manual setting.
 *
 * @author mrieser
 */
public final class PopulationReader extends MatsimXmlParser {
  ...
  private static final String PLANS =
      "plans.dtd"; // a special, inofficial case, handle it like plans_v0
  ...
  /*deliberately package*/ PopulationReader(
      final String inputCRS, final String targetCRS, final Scenario scenario, boolean streaming) {
    super(ValidationType.DTD_ONLY);
    if (!streaming && scenario.getPopulation() instanceof StreamingPopulation) {
      throw new RuntimeException(
          "MatsimPopulationReader called directly with an instance of StreamingPopulation "
    ...
    switch (doctype) {
      case POPULATION_V6:
        if (FeatureFlags.useParallelIO()) {
          this.delegate = new ParallelPopulationReaderMatsimV6(inputCRS, targetCRS, this.scenario);
          ((ParallelPopulationReaderMatsimV6) delegate).putAttributeConverters(attributeConverters);
        } else {
          this.delegate = new PopulationReaderMatsimV6(inputCRS, targetCRS, this.scenario);
