...
/**
 * Implementation of {@link NetworkRoute} which internally stores the route as a series of {@link
 * Link}s.
 *
 * @author mrieser
 */
final class LinkNetworkRouteImpl extends AbstractRoute implements NetworkRoute {
  ...
  /*package*/ static final String ROUTE_TYPE = "links";
  ...
  LinkNetworkRouteImpl(
      final Id<Link> startLinkId, final Id<Link>[] linkIds, final Id<Link> endLinkId) {
    super(startLinkId, endLinkId);
  ...
  public NetworkRoute getSubRoute(Id<Link> fromLinkId, Id<Link> toLinkId) {
    /**
     * the index where the link after fromLinkId can be found in the route: fromIndex==0 -->
     * fromLinkId == startLinkId, fromIndex==1 --> fromLinkId == first link in the route, etc.
     */
    int fromIndex = -1;
