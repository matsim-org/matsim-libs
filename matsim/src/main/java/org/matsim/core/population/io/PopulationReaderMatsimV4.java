...
/* package */ class PopulationReaderMatsimV4 extends MatsimXmlParser implements MatsimReader {
  ...
  /* package */ static final String PLANS = "plans";
  ...
  private static final Logger log = LogManager.getLogger(PopulationReaderMatsimV4.class);
  ...
  private void startLocation(final Attributes atts) {
    ...
    ActivityFacility currfacility =
        this.facilities.getFacilities().get(Id.create(id, ActivityFacility.class));
    ...
    if (this.curractivity == null) {
      throw new RuntimeException(
          "facility id=" + id + ": Activity of type=" + this.curracttype + " does not exist!");
    }
  }
    ...
    if (atts.getValue("link") != null) {
      ...
      this.curract =
          PopulationUtils.createAndAddActivityFromLinkId(
              this.currplan, atts.getValue(ATTR_TYPE), linkId1);
      ...
      this.curract =
          PopulationUtils.createAndAddActivityFromCoord(
              this.currplan, atts.getValue(ATTR_TYPE), coord);
    } else {
