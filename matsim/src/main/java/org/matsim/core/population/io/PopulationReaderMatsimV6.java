 ...
 * @author balmermi
 */
/* deliberately package */ class PopulationReaderMatsimV6 extends MatsimXmlParser
    implements MatsimReader {
  private static final Logger log = LogManager.getLogger(PopulationReaderMatsimV6.class);

  /* package */ static final String POPULATION = "population";
  /* package */ static final String PERSON = "person";
  /* package */ static final String ATTRIBUTES = "attributes";
  ...
  /* package */ static final String ATTR_PERSON_ID = "id";
  ...
  PopulationReaderMatsimV6(final String inputCRS, final String targetCRS, final Scenario scenario) {
  ...
  private void startAct(final Attributes atts) {
    ...
    if (atts.getValue(ATTR_ACT_FACILITY) != null) {
      final Id<ActivityFacility> facilityId =
          Id.create(atts.getValue(ATTR_ACT_FACILITY), ActivityFacility.class);
      if (isStageActivity) {
    ...
    // Check whether the given times match the assumptions made in InteractionActivity. Otherwise,
    // convert it to a regular Activity.
    if (isStageActivity
        && (startTime.isDefined()
            || endTime.isDefined()
            || duration.isUndefined()
      ...
      duration.ifDefinedOrElse(
          this.curract::setMaximumDuration, this.curract::setMaximumDurationUndefined);
    ...
    if (Double.isNaN(this.currRoute.getDistance())) {
      if (this.currRoute instanceof NetworkRoute) {
        if (!this.scenario.getNetwork().getLinks().isEmpty()) {
          this.currRoute.setDistance(
              RouteUtils.calcDistanceExcludingStartEndLink(
                  (NetworkRoute) this.currRoute, this.scenario.getNetwork()));
        }
      } else {
        ...
        if (fromCoord != null && toCoord != null) {
          ...
          if (this.scenario
              ...
              .containsKey(this.currleg.getMode())) {
            double estimatedNetworkDistance =
                dist
                    * this.scenario
                        .getConfig()
                        .routing()
                        .getModeRoutingParams()
                        .get(this.currleg.getMode())
  ...
  private void startRoute(final Attributes atts) {
    ...
    if (vehicleRefId != null
        && !vehicleRefId.equals("null")
        && this.currRoute instanceof NetworkRoute) {
