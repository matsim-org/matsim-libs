...
/**
 * Parallel implementation of the PopulationReaderMatsimV4. The main thread only reads the file and
 ...
 * is activated, the non-parallel reader is used.
 *
 * <p>The parallel threads interpret the xml data for each person.
 *
 * @author cdobler
 */
/* deliberately package */ class ParallelPopulationReaderMatsimV4 extends PopulationReaderMatsimV4 {
  ...
  public ParallelPopulationReaderMatsimV4(
     ...
     */
    //		if (scenario.getPopulation() instanceof Population &&
    // ((Population)scenario.getPopulation()).isStreaming()) {
    if (scenario.getPopulation() instanceof StreamingPopulationReader.StreamingPopulation) {
      log.warn(
          "Population streaming is activated - cannot use "
              + ParallelPopulationReaderMatsimV4.class.getName()
              + "!");
    ...
    for (int i = 0; i < numThreads; i++) {
      ...
      ParallelPopulationReaderMatsimV4Runner runner =
          new ParallelPopulationReaderMatsimV4Runner(
              this.coordinateTransformation, this.collectorScenario, this.queue);
  ...
  public void startTag(String name, Attributes atts, Stack<String> context) {
    ...
    if (PLANS.equals(name)) {
      ...
      if (PERSON.equals(name)) {
        Person person =
            this.plans.getFactory().createPerson(Id.create(atts.getValue("id"), Person.class));
  ...
  private static class CollectorScenario implements Scenario {
    // yyyy Why is this necessary at all?  Could you please explain your design decisions?  The same
    // instance is passed to all threads, so
