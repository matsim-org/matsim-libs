...
/**
 * Implementation of {@link NetworkRoute} that tries to minimize the amount of data needed to be
 * stored for each route. This will give some memory savings, allowing for larger scenarios (=more
 * agents), especially on detailed networks, but is likely a bit slower due to the more complex
 * access of the route information internally.
 *
 ...
 * does not store the links in its route-information that are the same as the successor defined in
 * the subsequentLinks-map.<br>
 * Given a startLinkId, endLinkId and a list of linkIds to be stored, this implementation stores
 * first the startLinkId. Next, if the successor of the startLinkId is different from the first
 * linkId in the list, this linkId is stored, otherwise not. Then the successor of that linkId is
 * compared to the next linkId in the list. If the successor is different, the linkId is stored,
 * otherwise not. This procedure is repeated until the complete list of linkIds is processed.
 *
...
public class HeavyCompressedNetworkRoute extends AbstractNetworkRoute {
  ...
  public HeavyCompressedNetworkRoute(
      Id<Link> startLinkId,
      Id<Link> endLinkId,
      HeavyCompressedNetworkRouteFactory.CompressionData compressionData) {
  ...
  private void getLinksTillLink(
      final List<Id<Link>> links, final Id<Link> nextLinkId, final Id<Link> startLinkId) {
  ...
  public void setLinkIds(
    ...
    if (!multiplyVisitedNodes.isEmpty()) {
      ...
      for (Id<Link> linkId : srcRoute) {
        ...
        if (!subsequentLinks[previousLinkId.index()].getId().equals(linkId)
            || multiplyVisitedNodes.contains(fromNodeId)) {
          route[pos] = linkId.index();
          pos++;
        }
        previousLinkId = linkId;
