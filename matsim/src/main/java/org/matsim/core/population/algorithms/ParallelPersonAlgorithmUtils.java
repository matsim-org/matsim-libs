...
/**
 * An abstract/static helper class for running {@link AbstractPersonAlgorithm}s in parallel using
 * threads.
 *
 * @author mrieser
 */
public final class ParallelPersonAlgorithmUtils {
  ...
  private static final Logger log = LogManager.getLogger(ParallelPersonAlgorithmUtils.class);
  ...
  /**
   * Handles each person of the given <code>population</code> with the specified <code>algorithm
   * </code>, using up to <code>numberOfThreads</code> threads to speed things up. Use this method
   * only if the given algorithm is thread-safe! Otherwise, use {@link #run(Population, int,
   * PersonAlgorithmProvider)}.
   *
   * @param population
  ...
  public static void run(
      final Population population, final int numberOfThreads, final PersonAlgorithm algorithm) {
    run(
        population,
        numberOfThreads,
        new PersonAlgorithmProvider() {
          @Override
          public PersonAlgorithm getPersonAlgorithm() {
            return algorithm;
          }
