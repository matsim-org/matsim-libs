...
/**
 * Implementation of {@link NetworkRoute} that stores the links of the route using variable-length
 * integer encoding (VarInt) instead of a List of link-ids.
 *
 * <p>Each link-id of type Id uses 4 or 8 bytes, whereas the VarInt encoding will likely only take
 * at most 3 (for small to medium-sized scenarios) or 4 bytes (for very large scenarios). So
 * especially for large scenarios, where object pointers use 8 bytes, this saves at least half the
 * memory.
 *
 * <p>There is a small performance overhead, but it is rather small compared to {@link
 * org.matsim.core.population.routes.heavycompressed.HeavyCompressedNetworkRoute}.
 *
 * @author mrieser / Simunto
 */
public class MediumCompressedNetworkRoute extends AbstractNetworkRoute {

  private static final byte[] EMPTY_ROUTE = new byte[0];
