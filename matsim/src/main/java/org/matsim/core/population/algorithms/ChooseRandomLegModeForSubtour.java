 ...
 * different mode given a list of possible modes, considering that the means of transport follows
...
public final class ChooseRandomLegModeForSubtour implements PlanAlgorithm {
  ...
  public static class Candidate {
    ...
    public boolean equals(Object o) {
      ...
      return subtour.equals(candidate.subtour)
          && newTransportMode.equals(candidate.newTransportMode);
    }
  ...
  public ChooseRandomLegModeForSubtour(
    ...
    if (this.probaForChangeSingleTripMode > 0.) {
      ...
      if (possibleModes.length >= 2) {
        // nothing to choose if there is only one mode!  I also don't think that we can base this on
        // size, since
        ...
        // change single leg would not respect car availability, if car is not a chain based mode --
        // cr, aug'22
  ...
  /**
   * Some subtour consist of only a single trip, e.g. if a trip starts and ends on the same link or
   * facility. By default, for those modes, the normal chain based modes are available. But in
   * certain cases, not all the modes should be available for such trips (e.g. car-sharing does not
   * make much sense for such a trip), thus the list of modes available for single-trip subtours can
   * be specified independently. As mentioned, it is initialized by the constructor to the full list
   * of chain based modes.
   *
  ...
  public List<Candidate> determineChoiceSet(final Plan plan) {
    ...
    final Id<? extends BasicLocation> homeLocation =
        ((Activity) plan.getPlanElements().get(0)).getFacilityId() != null
            ? ((Activity) plan.getPlanElements().get(0)).getFacilityId()
            : ((Activity) plan.getPlanElements().get(0)).getLinkId();
    Collection<String> permissibleModesForThisPlan =
        permissibleModesCalculator.getPermissibleModes(plan);

    return determineChoiceSet(
        homeLocation, plan, TripStructureUtils.getTrips(plan), permissibleModesForThisPlan);
  }

  /**
   * Return whether a subtour is mass conserving according to configuration. Ignoring if subtours
   * are closed.
   */
  ...
  public void run(final Plan plan) {
    ...
    // if a plan consist of only chain based modes, change single trip won't be able to do anything,
    // in this case it will be ignored
    if (this.changeSingleLegMode != null
        && rng.nextDouble() < this.probaForChangeSingleTripMode
  ...
  /**
   * Checks if change single trip mode would have an option to choose from. That is the case, when
   * not all trips are chain based.
   */
  private boolean hasSingleTripChoice(List<Trip> trips) {
    return !trips.stream()
        .map(t -> mainModeIdentifier.identifyMainMode(t.getTripElements()))
  ...
  private List<Candidate> determineChoiceSet(
    ...
    if (behavior == SubtourModeChoice.Behavior.betweenAllAndFewerConstraints
        && trips.size() > 0
        && subtours.stream().noneMatch(s -> s.getTrips().equals(trips))) {
    ...
    for (Subtour subtour : subtours) {
      ...
      if (!subtour.isClosed()
          && behavior != SubtourModeChoice.Behavior.betweenAllAndFewerConstraints) {
        continue;
      }
      ...
      final Collection<String> testingModes =
          subtour.getTrips().size() == 1 ? singleTripSubtourModes : chainBasedModes;
      ...
      for (String mode : testingModes) {
        ...
        Activity lastDestination =
            findLastDestinationOfMode(
                trips.subList(0, trips.indexOf(subtour.getTrips().get(0))), mode);
      ...
      if (isMassConserving(subtour)
          || (behavior == SubtourModeChoice.Behavior.betweenAllAndFewerConstraints
              && subtour.getParent()
                  == null)) { // We can only replace a subtour if it doesn't itself move a vehicle
      ...
      for (String transportMode : usableModes) {
        choiceSet.add(new Candidate(trips, subtour, transportMode));
      }
    }
    return choiceSet;
  }
  ...
  private boolean isMassConserving(final Subtour subtour, final String mode) {
    final Activity firstOrigin = findFirstOriginOfMode(subtour.getTrips(), mode);

    if (firstOrigin == null) {
      return true;
    }

    final Activity lastDestination = findLastDestinationOfMode(subtour.getTrips(), mode);
  ...
  private boolean atSameLocation(Activity firstLegUsingMode, Activity lastLegUsingMode) {
    if (firstLegUsingMode.getFacilityId() != null)
      return firstLegUsingMode.getFacilityId().equals(lastLegUsingMode.getFacilityId());
        ...
        && firstLegUsingMode.getCoord() != null
        && lastLegUsingMode.getCoord() != null)
      return CoordUtils.calcEuclideanDistance(
              firstLegUsingMode.getCoord(), lastLegUsingMode.getCoord())
          <= coordDist;
    else return firstLegUsingMode.getLinkId().equals(lastLegUsingMode.getLinkId());
  }

  private Activity findLastDestinationOfMode(final List<Trip> tripsToSearch, final String mode) {
  ...
  private Activity findFirstOriginOfMode(final List<Trip> tripsToSearch, final String mode) {
  ...
  private String getTransportMode(final Subtour subtour) {
    return mainModeIdentifier.identifyMainMode(subtour.getTrips().get(0).getTripElements());
  }

  private void applyChange(final Candidate whatToDo, final Plan plan) {
    ...
    for (Trip trip : whatToDo.subtour.getTrips()) {
      ...
      if (childTrips.contains(trip)) originalModes.add(mainMode);
    ...
    if (behavior == SubtourModeChoice.Behavior.betweenAllAndFewerConstraints
        && containsChainBasedMode
        && originalModes.size() > 1
      ...
      logger.error(
          "Subtour of person {} contains a mix of chain- and non-chainbased modes: {}. "
              + "Subtour mode-choice won't be able to go back to this state. You need to adjust the input beforehand or disable 'betweenAllAndFewerConstraints'.",
