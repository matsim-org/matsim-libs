...
public abstract class AbstractRoute implements Route, Cloneable {
  // This has a public non-final non-empty method, which is "clone".  But in the end this is how it
  // is designed.
  // So we leave it as is; if we ever want to re-design it in the core, we will have to copy it and
  // start
  ...
  public AbstractRoute clone() {
    // "clone" is some automagic that, by itself, makes a copy of the "bit pattern" of the object.
    // That is:
    ...
    // * the references to objects can be changed on the copy without affecting the original.  For
    // example, a copy can
    //    point to other links or nodes or persons or IDs.
    // * One has to be careful with objects where the contents can be changed AND they are not
    // shared between the
    //    original and the copy.  For example, changing the income of the person is not a problem
    // since it is the same
    //    for two plans pointing to the same person.  In contrast (and potentially quite dangerous):
    // the contents of
    //    Customizable (currently not applicable for Route) would have to be explicitly deepcopied.
    // The method can only be called if a class implements "Cloneable"; otherwise, it leads to a
    // runtime exception (!).
