...
public class RouteUtils {
  ...
  public static List<Link> getLinksFromNodeIds(
      final Network network, final List<Id<Node>> nodeIds) {
  ...
  public static NetworkRoute getSubRoute(
      final NetworkRoute route, final Node fromNode, final Node toNode, final Network network) {
    ...
    for (Id<Link> linkId : linkIds) {
      ...
      if (link.getFromNode() == toNode) {
        toLinkId = link.getId();
        break; // we found the toLinkId, so we can stop searching, fromLinkId should be before after
        // all
      }
    }
  ...
  /**
   * Calculates the distance of the complete route, <b>excluding</b> the distance traveled on the
   * start- and end-link of the route.
   *
   * @param route
   * @param network
   * @return
   */
  public static double calcDistanceExcludingStartEndLink(
      final NetworkRoute route, final Network network) {
    double dist = 0;
  ...
  /**
   * Calculates the distance of the complete route, <b>including</b> the distance traveled on the
   * start- and end-link of the route.
   *
   * @param networkRoute
   * @param relPosOnDepartureLink relative position on the departure link where vehicles start
   *     traveling
  ...
  public static double calcDistance(
      ...
      final double relPosOnDepartureLink,
      final double relPosOnArrivalLink,
    ...
    routeDistance +=
        network.getLinks().get(networkRoute.getStartLinkId()).getLength()
            * (1.0 - relPosOnDepartureLink);
    if (!networkRoute.getStartLinkId().equals(networkRoute.getEndLinkId())) {
      // add relative distance of arrival link
      routeDistance +=
          network.getLinks().get(networkRoute.getEndLinkId()).getLength() * relPosOnArrivalLink;
    } else { // i.e. departure = arrival link
      // subtract relative distance of arrival link that is not traveled
      routeDistance -=
          network.getLinks().get(networkRoute.getEndLinkId()).getLength()
              * (1.0 - relPosOnArrivalLink);
    }
  ...
  public static NetworkRoute createNetworkRoute(List<Id<Link>> routeLinkIds) {
    ...
    List<Id<Link>> linksBetween =
        (routeLinkIds.size() > 2)
            ? routeLinkIds.subList(1, routeLinkIds.size() - 1)
            : new ArrayList<>(0);
  ...
  public static double calcDistance(
      TransitPassengerRoute route, TransitSchedule ts, Network network) {
  ...
  /**
   * How much of route is "covered" by the links of route2. Based on Ramming. Note that this is not
   * symmetric, i.e. route1 can be fully covered by route2, but not the other way around. kai,
   * nov'13
   *
   * @param route1
   * @param route2
  ...
  public static double calculateCoverage(
      NetworkRoute route1, NetworkRoute route2, Network network) {
    ...
    if (routeLength > 0.) {
      ...
      return 1.; // route has zero length = fully covered by any other route.  (but they are not
      // similar!?!?!?)
    }
  }
  ...
  public static NetworkRoute createLinkNetworkRouteImpl(
      Id<Link> startLinkId, List<Id<Link>> linkIds, Id<Link> endLinkId) {
    return new LinkNetworkRouteImpl(startLinkId, linkIds, endLinkId);
  }

  public static NetworkRoute createLinkNetworkRouteImpl(
      Id<Link> startLinkId, Id<Link>[] linkIds, Id<Link> endLinkId) {
    return new LinkNetworkRouteImpl(startLinkId, linkIds, endLinkId);
  }
}
