...
public class PolygonFeatureFactory {
  ...
  public SimpleFeature createPolygon(
      final Coordinate[] coordinates, final Map<String, Object> attributeValues, final String id) {
    LinearRing shell;
  ...
  public SimpleFeature createPolygon(
      final Polygon polygon, final Map<String, Object> attributeValues, final String id) {
    MultiPolygon mp = this.fac.createMultiPolygon(new Polygon[] {polygon});
    return this.createPolygon(mp, attributeValues, id);
  }

  public SimpleFeature createPolygon(
      final MultiPolygon polygon, final Map<String, Object> attributeValues, final String id) {
  ...
  public SimpleFeature createPolygon(
      final Coordinate[] coordinates, final Object[] attributeValues, final String id) {
    LinearRing shell;
    if (coordinates[0] == coordinates[coordinates.length - 1]) {
      shell = this.fac.createLinearRing(coordinates);
    } else {
  ...
  public SimpleFeature createPolygon(
      final Polygon polygon, final Object[] attributeValues, final String id) {
    MultiPolygon mp = this.fac.createMultiPolygon(new Polygon[] {polygon});
    return this.createPolygon(mp, attributeValues, id);
  }

  public SimpleFeature createPolygon(
      final MultiPolygon polygon, Object[] attributeValues, final String id) {
    this.builder.add(polygon);
