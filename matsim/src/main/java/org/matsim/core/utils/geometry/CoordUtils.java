...
public abstract class CoordUtils {
  private static final Logger LOG = LogManager.getLogger(CoordUtils.class);
  private static final EucledianDistanceCalculator eucledianDistanceCalculator =
      new EucledianDistanceCalculator();
  ...
  public static Coord plus(Coord coord1, Coord coord2) {
    if (!coord1.hasZ() && !coord2.hasZ()) {
      ...
      throw new RuntimeException(
          "Cannot 'plus' coordinates if one has elevation (z) and the other not; coord1.hasZ="
              + coord1.hasZ()
              + "; coord2.hasZ="
              + coord2.hasZ());
    }
  ...
  public static double length(Coord coord) {
    if (!coord.hasZ()) {
      ...
      return Math.sqrt(
          coord.getX() * coord.getX() + coord.getY() * coord.getY() + coord.getZ() * coord.getZ());
    }
  }

  /**
   * Note: If the given {@link Coord} has elevation, it's elevation will stay the same (jjoubert,
   * Sep '16).
   *
   * @param coord
   * @return
  ...
  /**
   * Round coordinate by using default precision. For coordinates that are assumed lat lon, it will
   * be 6 decimals. Otherwise, UTM is assumed and rounded to 2 decimals places. When these
   * assumptions hold, the resulting coordinate will be precise up to ~0.1 meter.
   */
  ...
  public static Coord round(Coord coord, int scale) {
    if (coord.hasZ()) {
      return new Coord(
          roundNumber(coord.getX(), scale), roundNumber(coord.getY(), scale), coord.getZ());
    }
  ...
  /**
   * Method to deal with distance calculation when only the x and y-components of the coordinates
   * are used. The elevation (z component) is ignored, whether it is available or not. (xy-plane)
   *
   * @param coord
   * @param other
   * @return
  ...
  /**
   * Method should only be used in within this class, and only by {@link
   * #distancePointLinesegment(Coord, Coord, Coord)}.
   *
   * @param coord1
   * @param coord2
   * @return
  ...
  /**
   * Calculates the shortest distance of a point to a line segment. The line segment is given by two
   * points, <code>lineFrom</code> and <code>lineTo</code>. Note that the line segment has finite
   * length, and thus the shortest distance cannot always be the distance on the tangent to the line
   * through <code>point</code>. <br>
   * <br>
   * The 3D version was adapted from the C++ implementation of <a
   * href="http://geomalgorithms.com/a02-_lines.html">Dan Sunday</a>.
   *
   ...
   * @return the distance of <code>point</code> to the line segment given by the two end points of
   *     the line segment, <code>lineFrom</code> and <code>lineTo</code>
   * @author mrieser, jwjoubert
   */
  private static boolean onlyOnceWarnGiven = false;

  public static double distancePointLinesegment(
      final Coord lineFrom, final Coord lineTo, final Coord point) {
    if (!lineFrom.hasZ() && !lineTo.hasZ() && !point.hasZ()) {
      ...
      double u =
          ((point.getX() - lineFrom.getX()) * lineDX + (point.getY() - lineFrom.getY()) * lineDY)
      ...
      return calcEuclideanDistance(
          new Coord(lineFrom.getX() + u * lineDX, lineFrom.getY() + u * lineDY), point);
      ...
      if (!onlyOnceWarnGiven) {
        LogManager.getLogger(CoordUtils.class)
            .warn("Mix of 2D / 3D coordinates. Assuming 2D only.\n" + Gbl.ONLYONCE);
        onlyOnceWarnGiven = true;
      }
      return distancePointLinesegment(
          new Coord(lineFrom.getX(), lineFrom.getY()),
          new Coord(lineTo.getX(), lineTo.getY()),
          new Coord(point.getX(), point.getY()));
    }
  }

  /**
   * Calculates the coordinate of the intersection point of the orthogonal projection of a given
   * point on a line segment with that line segment. The line segment is given by two points, <code>
   * lineFrom</code> and <code>lineTo</code>. If the projection point does not lie *on* the line
   * segment (but only somewhere on the extension of the line segment, i.e. the infinite line), the
   * end point of the line segment which is closest to the given point is returned. <br>
   * <br>
   * The 3D version was adapted from the documentation of <a
   * href="http://www.geometrictools.com/Documentation/DistancePointLine.pdf">David Eberly/a>.
   *
   * @param lineFrom The start point of the line segment
   * @param lineTo The end point of the line segment
   * @param point The point whose distance to the line segment should be calculated
   * @return the <code>coordinate</code> of the intersection point of the orthogonal projection of a
   *     given point on a line segment with that line segment
   * @author dziemke, jwjoubert
   */
  public static Coord orthogonalProjectionOnLineSegment(
      final Coord lineFrom, final Coord lineTo, final Coord point) {
    if (!lineFrom.hasZ() && !lineTo.hasZ() && !point.hasZ()) {
      ...
      return orthogonalProjectionOnLineSegment(
          new Coord(lineFrom.getX(), lineFrom.getY()),
          new Coord(lineTo.getX(), lineTo.getY()),
          new Coord(point.getX(), point.getY()));
      // throw new RuntimeException("All given coordinates must either be 2D, or 3D. A mix is not
      // allowed.");
    }
  }
    ...
    private double calculateDistance(Coord coord, Coord other) {
      ...
      if (!coord.hasZ() && !other.hasZ()) {
        ...
        if (warnCounter.incrementAndGet() <= maxWarnCount) {
          LOG.warn("Mixed use of elevation in coordinates: " + coord + "; " + other);
