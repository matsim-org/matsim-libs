...
public abstract class AbstractMatsimWriter {
  ...
  /**
   * Whether or not the output is gzip-compressed. If <code>null</code>, the usage of compression is
   * decided by the filename (whether it ends with .gz or not).
   */
  protected Boolean useCompression = null;

  /**
   * Sets whether the file should be gzip-compressed or not. Must be set before the file is opened
   * for writing. If not set explicitly, the usage of compression is defined by the ending of the
   * filename.
   *
  ...
  protected final void openOutputStream(OutputStream outputStream) {
    ...
    try {
      if (this.useCompression == null || this.useCompression) {
        ...
        this.writer =
            new BufferedWriter(
                new OutputStreamWriter(new GZIPOutputStream(outputStream), StandardCharsets.UTF_8));
      }
    } catch (IOException e) {
