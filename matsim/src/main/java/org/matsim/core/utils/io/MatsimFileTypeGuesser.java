...
public class MatsimFileTypeGuesser extends DefaultHandler {
  ...
  public static final String SYSTEMIDNOTFOUNDMESSAGE =
      "System Id of xml document couldn't be detected. "
  ...
  public MatsimFileTypeGuesser(final String fileName) throws UncheckedIOException {
    ...
    if (name.endsWith(".xml.gz") || name.endsWith(".xml")) {
      ...
      if (this.xmlSystemId != null) {
        shortSystemId =
            this.xmlSystemId.substring(this.xmlSystemId.replace('\\', '/').lastIndexOf('/') + 1);
      }
  ...
  /**
   * @return if the file is an XML file, this returns the public-id of the declared type, <code>null
   *     </code> if no type is declared or if it is not an XML file.
   */
  ...
  /**
   * @return if the file is an XML file, this returns the system-id of the declared type, <code>null
   *     </code> if no type is declared or if it is not an XML file.
   */
  ...
  private static final class XmlHandler extends DefaultHandler {
    ...
    public InputSource resolveEntity(final String publicId, final String systemId) {
      /**
       * As the xml schema of interest may be derived from other schema instances we are only
       * interested in the first entity resolved.
       */
  ...
  /**
   * Used to return the declared type encountered in the XML file, the root tag and to stop parsing
   * the file
   *
   * @author dgrether
   */
  private static final class XMLTypeDetectionException extends RuntimeException {
