...
/**
 * An abstract XML-Parser which can be easily extended for reading custom XML-formats. This class
 * handles all the low level functionality required to parse xml-files. Extending classes have only
 * to implement {@link #startTag} and {@link #endTag} to implement a custom parser.<br>
 * The parser implements a custom <code>EntityResolver</code> to look for DTDs in the MATSim world.
 *
 * <p>Notes:
 *
 * <ul>
 *   <li>If implementing classes want to override the final methods, the will have to resort to
 *       delegation.
 * </ul>
 *
 * @author mrieser
...
public abstract class MatsimXmlParser extends DefaultHandler implements MatsimReader {
  ...
  /**
   * As the mechanism implemented in InputSource is not really working for error handling the source
   * to be parsed is stored here for error handling.
   */
   ...
   *
   * @param validationType hint whether DTD or XSD is expected for validation, helps to optimize the
   ...
   *
   * @param name the name of the xml-tag.
   * @param content the character-content of the tag; any characters between <code>&lt;tag&gt;
   *     </code> and <code>&lt;/tag&gt;></code>, excluding other tags and their content.
   * @param context a stack containing the path/hierarchy to the current tag
   */
  public abstract void endTag(String name, String content, Stack<String> context);

  /**
   * Sets, if this parser should validate the read XML or not. Not validating is sometimes useful
   * during development or during some tests with format-extensions that are not yet part of the
   * DTD, but it is <b>strongly discouraged</b> not to validate during production use.
   *
  ...
  /**
   * Sets the directory where to look for DTD and XSD files if they are not found at the location
   ...
   * parsing. A gzip-compressed file must have the ending ".gz" to be correctly recognized. The
   * passed filename may or may not contain the ending ".gz". If no uncompressed file is found with
   * the specified name, the ending ".gz" will be added to the filename and a compressed file will
   * be searched for and read if found.
   *
   * @param filename The filename of the file to read, optionally ending with ".gz" to force reading
  ...
  public final void parse(final InputSource input) throws UncheckedIOException {
    try {
      ...
      if (useWstxParser) {
        // use Woodstox-library as XML parser when no validation or only DTD-validation is required,
        // as it is much faster than the default (xerces)
        ...
        factory.setFeature(
            ...
            false); // prevent XEE attack: https://en.wikipedia.org/wiki/XML_external_entity_attack

        if (validating) {
          factory.setFeature(
              "http://xml.org/sax/features/validation",
              true); // required to enable DTD validation in Woodstox
        ...
        // use the default (Xerces) SAX parser, it is slower than Woodstox, but supports XSD
        // validation
  ...
  protected void setDoctype(final String doctype) {
    // implementation of this method is what reacts to the different version of the file formats, so
    // we cannot make it final. kai, jul'16
  ...
  private static InputSource findDtdInRemoteLocation(final String fullSystemId) {
    ...
    try {
      ...
      // There was a problem getting the (remote) file, just show the error as information for the
      // user
  ...
  public void characters(final char[] ch, final int start, final int length) throws SAXException {
    // has to be non-final since otherwise the events parser does not work.  Probably ok (this here
    // is just a default implementation). kai, jul'16
  ...
  public final void startElement(
      final String uri, final String localName, final String qName, Attributes atts) {
  ...
  /**
   * Parses a String into a double, taking into account the special encoding for Infinity according
   * to the xsd-specifications for the xs:double data type
   */
