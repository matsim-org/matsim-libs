...
/**
 * Utility class for reading encrypted input file. This class is also integrated into {@link
 * IOUtils} to decrypt files ending with ".enc" automatically.
 *
 * <p>This class expects files to be encrypted in one specific way with certain parameters. This can
 * be achieved with {@code openssl} from the command line using:
 *
 * <pre>
 * openssl enc -aes256 -md sha512 -pbkdf2 -iter 10000 -in [some.secret] -out [some.secret.enc]
 * </pre>
 *
 * <p>Please note, that the arguments <emph>-aes256 -md sha512 -pbkdf2 -iter 10000</emph> are
 * strictly required and can not be changed.
 *
 * <p>The password for decryption has to be made available as environment variable or system
 * property to the program at runtime. Set {@code MATSIM_DECRYPTION_PASSWORD} to the password needed
 * for the files. This password has to be the same for all encrypted files.
 */
public class CipherUtils {
  ...
  private static Key getKeyForFile(byte[] salt)
    ...
    if (pw == null)
      throw new IllegalStateException(
          "No password specified for encrypted file. Please set "
  ...
  /**
   * Decrypts an input stream. For more information how to encrypt files and how supply the password
   * please see {@link CipherUtils}.
   *
   * @param is input stream to decrypt
   * @see CipherUtils
   */
  public static InputStream getDecryptedInput(InputStream is)
