 ...
 * Many operations (like {@link #get(Object)}) have a runtime of <code>O(n)</code>, so this map
 * implementation should only be used to store a small number of elements in it. But for small
 * number of elements, this implementation performs very well, especially because of its very low
 * memory overhead.
 *
 * <p>Internally, this implementation uses a single array which stores boths keys and values in
 * sequential order (data[i * 2] contains the keys, data[i * 2 + 1] contains the values).
 *
