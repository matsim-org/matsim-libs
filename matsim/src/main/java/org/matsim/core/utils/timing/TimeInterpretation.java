...
public class TimeInterpretation {
  ...
  public static TimeInterpretation create(Config config) {
  ...
  public static TimeInterpretation create(
      ActivityDurationInterpretation interpretation,
  ...
  public static TimeInterpretation create(
      ActivityDurationInterpretation interpretation, TripDurationHandling tripDurationHandling) {
  ...
  /**
   * This function takes a plan and finds the end time of the activity along this plan. Note that
   * this requires tracking end times and travel times along all plan elements. This is especially
   * inefficient when already looping over an agent plan. Whenever possible, make use of TimeTracker
   * which provides a simple step-by-step interface to track time along a plan.
   *
   * <p>TODO: Documentation TODO: Phase out shortcut.
   */
  public OptionalTime decideOnActivityEndTimeAlongPlan(Activity activity, Plan plan) {
    ...
    return decideOnElementsEndTime(
        plan.getPlanElements().subList(0, activityIndex + 1), simulationStartTime);
  }

  /**
   * Given the start time of an activity, the function returns the end time. The calculation depends
   * on whether an activity is defined by an end time or a duration and, if both are given, in which
   * order they are evaluated. This is controlled by the activityDurationInterpretation in the plans
   * config group.
   */
    ...
    switch (activityDurationInterpretation) {
      ...
      case minOfDurationAndEndTime:
        if (activity.getEndTime().isUndefined() && activity.getMaximumDuration().isUndefined()) {
          ...
          double durationBasedEndTime = startTime + activity.getMaximumDuration().seconds();
          return checkAdvance(
              activity.getEndTime().seconds() <= durationBasedEndTime
                  ? activity.getEndTime()
      ...
      default:
        throw new IllegalArgumentException(
            "Unsupported 'activityDurationInterpretation' enum type: "
                + activityDurationInterpretation);
    }
  }

  /**
   * Returns the travel time for a leg. If travel time is available in the leg's route, the route
   * travel time is used, otherwise the leg travel time.
   */
  ...
  /**
   * Obtains the end time of an element which is either and activity or a leg, according to the
   * logic of decideOnActivityEndTime and decideOnLegtravelTime.
   */
  ...
  /**
   * Traverses through a list of elements (activities and legs) and finds the end time of the chain
   * according to the logic of decideOnActivityEndTime and decideOnLegtravelTime.
   */
  public OptionalTime decideOnElementsEndTime(
      List<? extends PlanElement> elements, final double startTime) {
