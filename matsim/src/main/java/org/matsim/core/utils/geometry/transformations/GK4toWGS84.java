 ...
 * @see <a
 *     href="http://remotesensing.org/geotiff/proj_list/transverse_mercator.html">remotesensing.org,
 *     transverse mercator</a>
 * @see <a href="www.epsg.org/guides/docs/G7-2.pdf">OGP Surveying and Positioning Guidance Note
 *     number 7, part 2</a>
 */
public class GK4toWGS84 implements CoordinateTransformation {
  ...
  private static final double MU1_DIVISOR =
      a * (1 - e2 * (1.0 / 4.0 - e2 * (3.0 / 64.0) - e2 * (5.0 / 256.0)));
        ...
        phi1
            - (nu1 * tanphi1 / rho1)
                * (Math.pow(D, 2) / 2
                    ...
                    + (61 + 90 * T1 + 298 * C1 + 45 * T1 * T1 - 252 * e_2 - 3 * C1 * C1)
                        * Math.pow(D, 6)
                        / 720);
    ...
    double longitude =
        projectionLongitude
            + (D
                    ...
                    + (5 - 2 * C1 + 28 * T1 - 3 * C1 * C1 + 8 * e_2 + 24 * T1 * T1)
                        * Math.pow(D, 5)
                        / 120)
                / cosphi1;
