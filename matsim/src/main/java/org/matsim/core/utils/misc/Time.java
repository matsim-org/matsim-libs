...
public class Time {
  ...
  /**
   * Never change this to NaN, as a compare of any valid time to this should result to "greater" for
   * some algorithms to work still we found the name "UNDEFINED" more suitable than TIME_MIN_VALUE
   * <br>
   * <b><i>Note:</i></b> do not interpret the "UNDEFINED" as "time does not matter", as this has
   * implications for, example, routing. If start time is given as {@link Time#UNDEFINED_TIME} then
   * {@link Path#travelTime} will return {@link Double#NaN}, even though the {@link
   * TravelTime#getLinkTravelTime} is independent of the start time.
   */
  ...
  public static final double MIDNIGHT = 24 * 3600.0;
   ...
   */
  public static final String writeTime(
      final double seconds, final String timeformat, final char separator) {
  ...
  /**
   * Parses the given string for a textual representation for time and returns the time value in
   * seconds past midnight. It is the same as {@link #parseTime(String, char)} with the separator
   * set to ':'.
   *
  ...
  /**
   * Parses the given string for a textual representation for time and returns the time value in
   * seconds past midnight. The following formats are recognized: HH:mm:ss, HH:mm, ssss.
   *
   * @param time the string describing a time to parse.
