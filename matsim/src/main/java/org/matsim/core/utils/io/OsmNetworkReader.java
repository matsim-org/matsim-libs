...
/**
 * Reads in an OSM-File, exported from <a href="http://openstreetmap.org/"
 * target="_blank">OpenStreetMap</a>, and extracts information about roads to generate a
 * MATSim-Network.
 *
 * <p>OSM-Files can be obtained:
 *
 * <ul>
 *   <li>by <a href="http://openstreetmap.org/export" target="_blank">exporting</a> the data
 *       directly from OpenStreetMap. This works only for smaller regions.
 *   <li>by <a href="http://wiki.openstreetmap.org/wiki/Getting_Data"
 *       target="_blank">downloading</a> the requested data from OpenStreetMap.
 *   <li>by extracting the corresponding data from <a href="http://planet.openstreetmap.org/"
 *       target="_blank">Planet.osm</a>. Planet.osm contains the <em>complete</em> data from
 *       OpenStreetMap and is huge! Thus, you must extract a subset of data to be able to process
 *       it. For some countries, there are <a
 *       href="http://wiki.openstreetmap.org/wiki/Planet.osm#Extracts" target="_blank">Extracts</a>
 *       available containing only the data of a single country.
 * </ul>
 *
 * OpenStreetMap only contains limited information regarding traffic flow in the streets. The most
 * valuable attribute of OSM data is a <a
 * href="http://wiki.openstreetmap.org/wiki/Map_Features#Highway" target="_blank">categorization</a>
 * of "ways". This reader allows to set {@link #setHighwayDefaults(int, String, double, double,
 * double, double) defaults} how those categories should be interpreted to create a network with
 * suitable attributes for traffic simulation. For the most common highway-types, some basic
 * defaults can be loaded automatically (see code), but they can be overwritten if desired. If the
 * optional attributes <code>lanes</code> and <code>oneway</code> are set in the osm data, they
 * overwrite the default values. Using {@link #setHierarchyLayer(double, double, double, double,
 * int) hierarchy layers}, multiple overlapping areas can be specified, each with a different
 * denseness, e.g. one only containing motorways, a second one containing every link down to
 * footways.
 *
 * @deprecated The OsmNetworkReader has been moved to the osm-contrib. Use
 *     org.matsim.contribs.osm.networkReader.SupersonicOsmNetworkReader instead.
 * @author mrieser, aneumann
 */
@Deprecated
public class OsmNetworkReader implements MatsimSomeReader {

  private static final Logger log = LogManager.getLogger(OsmNetworkReader.class);
  ...
  protected static final String TAG_HIGHWAY = "highway";
  private static final String TAG_MAXSPEED = "maxspeed";
  protected static final String TAG_JUNCTION = "junction";
  protected static final String TAG_ONEWAY = "oneway";
  ...
  private boolean useVspAdjustments =
      false; // Adjustments discussed on 2018-04-30, kn,ik,dz. apr'18 (Might become default after
  // testing)
   ...
   * @param transformation A coordinate transformation to be used. OSM-data comes as WGS84, which is
   *     often not optimal for MATSim.
   */
   ...
   * Creates a new Reader to convert OSM data into a MATSim network.
   *
   * <p>After discussion, we (kn,ik,dz) introduced some adjustments to links regarding speeds and
   * capacities that aim to represent different travel times in urban vs. rural areas better, esp.
   * taking into account intersections/traffic lights and their implications on speeds and
   * capacities in urban areas.
   *
   * @param network An empty network where the converted OSM data will be stored.
   * @param transformation A coordinate transformation to be used. OSM-data comes as WGS84, which is
   *     often not optimal for MATSim.
  ...
  public OsmNetworkReader(
      ...
      final CoordinateTransformation transformation,
      final boolean useHighwayDefaults,
      final boolean useVspAdjustments) {
    ...
    if (useHighwayDefaults) {
      ...
      // Setting the following to considerably smaller values, since there are often traffic
      // signals/non-prio intersections.
      // If someone does a systematic study, please report.  kai, jul'16
      //
      // We revised the below street types (removed "minor" and put "living_street", "residential",
      // and "unclassified" into
      ...
      // changing the speed values failed the evacuation ScenarioGenerator test because of a
      // different network -- DESPITE
      // the fact that all the speed values are reset to some other value there.  No idea what
      // happens there. kai, jul'16
    }
  ...
  /**
   * Parses the given input stream and creates a MATSim network from the data. We need to pass a
   * supplier as in some cases, the stream could be parsed multiple times.
   *
   ...
   * @throws UncheckedIOException
   */
  private void parse(final String osmFilename, final Supplier<InputStream> streamSupplier)
      throws UncheckedIOException {
  ...
  /**
   * Set a parser for OSM XML data. If no parser is set, the default parser from this class
   * (OsmNetworkReader.OsmXmlParser) is used.
   */
  ...
  /**
   * Sets defaults for converting OSM highway paths into MATSim links, assuming it is no oneway
   * road.
   *
   ...
   * @see <a
   *     href="http://wiki.openstreetmap.org/wiki/Map_Features#Highway">http://wiki.openstreetmap.org/wiki/Map_Features#Highway</a>
   */
  public final void setHighwayDefaults(
      final int hierarchy,
   ...
   * Keeping the detailed paths results in a much higher number of nodes and links in the resulting
   * MATSim network. Not keeping the detailed paths removes all nodes where only one road passes
   * through, thus only real intersections or branchings are kept as nodes. This reduces the number
   * of nodes and links in the network, but can in some rare cases generate extremely long links
   * (e.g. for motorways with only a few ramps every few kilometers).
   *
   ...
   * e.g. by constrains of traffic lights not explicitly modeled, a kind of "average simulated
   * speed" can be used.
   *
   * <p>Defaults to <code>false</code>.
   *
   * @param scaleMaxSpeed <code>true</code> to scale the speed limit down by the value specified by
   *     the {@link #setHighwayDefaults(int, String, double, double, double, double) defaults}.
   */
  ...
  /**
   * Defines a new hierarchy layer by specifying a rectangle and the hierarchy level to which
   * highways will be converted.
   *
   ...
   */
  public final void setHierarchyLayer(
      final double coordNWNorthing,
      final double coordNWEasting,
      final double coordSENorthing,
      final double coordSEEasting,
      final int hierarchy) {
    this.hierarchyLayers.add(
        new OsmFilterImpl(
            this.transform.transform(new Coord(coordNWEasting, coordNWNorthing)),
            this.transform.transform(new Coord(coordSEEasting, coordSENorthing)),
            hierarchy));
  }
   ...
   * This can lead to OutOfMemoryExceptions when converting huge osm files. By enabling this memory
   * optimization, the converter tries to reduce its memory usage, but will run slower.
   *
   * @param memoryEnabled
   */
  ...
  /**
   * This method is called before the network (still based on osm data) gets simplified and before
   * the matsim network is created.
   *
   * <p>It cleans the OSM data (removes ways with nodes that does not exist) and prepares data for
   * simplification (collects information about all ways a node is located on; marks end nodes of
   * ways).
   *
   * <p>Override/Extend this method to add additional preprocessing of OSM data (see e.g. the
   * signals and lanes reader).
   */
  ...
  /**
   * This method marks node that should be kept based on the highway type, the number of ways they
   * are located at, whether they are endpoints of ways, and whether they belong to a certain group
   * of nodes that should be kept anyway (e.g. counting stations).
   *
   * <p>Override/Extend this method when further simplification of OSM data is necessary in your
   * reader.
   */
  protected void simplifyOsmData() {
    log.info("Mark OSM nodes that shoud be kept...");
    for (OsmNode node : this.nodes.values()) {
      // check whether the node belongs to a way that fulfills the hierarchyLayers (i.e. is 'big'
      // enough)
      wayLoop:
      ...
      // keep special nodes (e.g. nodes with counts data), also when they don't fulfill
      // hierarchyLayers (e.g. are located on smaller ways)
  ...
  /**
   * This method checks whether the node is an intersection node or an end node of a way and,
   * therefore, can not be removed/simplified. Override/extend this when you want to keep more or
   * other nodes, e.g. signalized nodes.
   *
  ...
  /**
   * This method creates nodes and links for a MATSim scenario. Override/extend this when additional
   * data, e.g. signals, should be created.
   */
  ...
  private void createLink(
    ...
    if (maxspeedTag != null) {
      try {
        if (maxspeedTag.endsWith("mph")) {
          freespeed =
              Double.parseDouble(maxspeedTag.replace("mph", "").trim())
                  * 1.609344
                  / 3.6; // convert mph to m/s
        } else {
          freespeed = Double.parseDouble(maxspeedTag) / 3.6; // convert km/h to m/s
        }
        if (useVspAdjustments) {
          // For links whose maxspeed is known, we assume that those with maxspeed lower than or
          // equl to 51km/h are 'urban' links
          // For these, we reduced speeds to 50% to account for traffic lights/intersections now,
          // kn,ik,dz, apr'18
          if (freespeed <= 51. / 3.6) {
      ...
      if (useVspAdjustments) {
        // For links whose maxspeed is unknown, we assume that links with a length (after removal of
        // purely geometric nodes) of more
        // than 300m are 'rural' others 'urban'. 'Rural' speed is 100km/h, 'urban' linearly
        // increasing from 10km/h at zero length to
        // the 'rural' speed for links with a length of 300m. kn,ik,dz, apr'18
        if (highway.equalsIgnoreCase("primary")
            || highway.equalsIgnoreCase("secondary")
            || highway.equalsIgnoreCase("tertiary")
            || highway.equalsIgnoreCase("primary_link")
            || highway.equalsIgnoreCase("secondary_link")
            || highway.equalsIgnoreCase("tertiary_link")) {
          if (length > 300.) {
            freespeed = 80. / 3.6; // Might be different (but also not too much different) in other
            // countries
          } else {
    ...
    if (useVspAdjustments) {
      // Adjustments that KN had been using for a while: For short links, often roundabouts or short
      // u-turns, etc.
      if (length < 100) {
    ...
    try {
      ...
      if (lanesForwardTag != null && lanesBackwardTag != null) {
        ...
        nofLanesBackward = 0;
        if (lanesForwardTag != null) nofLanesForward = Double.parseDouble(lanesForwardTag);
        else if (lanesTag != null) nofLanesForward = Double.parseDouble(lanesTag);
        // else keep default
      } else if (onewayReverse) {
        nofLanesForward = 0;
        if (lanesBackwardTag != null) nofLanesBackward = Double.parseDouble(lanesBackwardTag);
        else if (lanesTag != null) nofLanesBackward = Double.parseDouble(lanesTag);
        // else keep default
      } else if (lanesForwardTag != null) {
        // i.e. lanesBackwardTag is null
        nofLanesForward = Double.parseDouble(lanesForwardTag);
        if (lanesTag != null) nofLanesBackward = Double.parseDouble(lanesTag) - nofLanesForward;
        // else keep default
      } else if (lanesBackwardTag != null) {
        // i.e. lanesForwardTag is null
        nofLanesBackward = Double.parseDouble(lanesBackwardTag);
        if (lanesTag != null) nofLanesForward = Double.parseDouble(lanesTag) - nofLanesBackward;
        // else keep default
    ...
    if (this.scaleMaxSpeed) {
      ...
      if (useVspAdjustments) {
        throw new RuntimeException(
            "Max speed scaling and VSP adjustments used at the same time. Both reduce speeds. It is most likely "
  ...
  /**
   * Override this method if you want to add additional attributes to the node, e.g. elevation (z),
   * or modify parameter values.
   */
  protected void setOrModifyNodeAttributes(Node n, OsmNode node) {}

  /**
   * Override this method if you want to add additional attributes to the link, e.g. modes, or
   * modify parameter values.
   */
  protected void setOrModifyLinkAttributes(Link l, OsmWay way, boolean forwardDirection) {}

  /**
   * Override this method if you want to use another way to figure out if one can travel on this
  ...
  protected static final class OsmNode {
  ...
  protected static final class OsmWay {
    public final long id;
    ...
    public void endTag(final String name, final String content, final Stack<String> context) {
      if ("way".equals(name)) {
        if (!this.currentWay.nodes.isEmpty()) {
          boolean used = false;
          OsmHighwayDefaults osmHighwayDefaults =
              OsmNetworkReader.this.highwayDefaults.get(this.currentWay.tags.get(TAG_HIGHWAY));
  ...
  private static class StringCache {
    ...
    /**
     * Returns the cached version of the given String. If the strings was not yet in the cache, it
     * is added and returned as well.
     *
     * @param string
