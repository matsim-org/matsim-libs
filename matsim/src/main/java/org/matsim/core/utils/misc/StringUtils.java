...
public class StringUtils {
  ...
  /**
   * Splits a string at the specified delimiter into multiple substrings. This method is similar to
   * <code>String.split()</code>, but does only take a single char as a delimiter and not a regular
   * expression, making this method a lot faster than <code>String.split()</code> if no regular
   * expressions are required.
   *
   * <p>The <code>limit</code> parameter controls how many parts will be returned at most. If the
   * limit is zero, no limit is used and the string is splitted as many times as possible. Negative
   * values for <code>limit</code> are not supported. If there are more delimiters than <code>limit
   * </code>, only the first <em>limit-1</em> ones will be used for splitting the string, the
   * additional delimiters are contained in the last part returned. Trailing empty strings are not
   ...
   * @return Returns an array of strings, each being a substring of <code>str</code>, that were
   *     connected by <code>delimiter</code> in the original string <code>str</code>.
   * @author mrieser
   */
