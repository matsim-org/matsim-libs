...
/**
 * A simple re-implementation of a priority queue that offers a much better performance for {@link
 * #remove(Object) remove} operations than the standard {@link PriorityQueue}, but uses more memory
 * to manage the data.<br>
 * The actual implementation is backed by a standard {@link PriorityQueue}. Each added object is
 * encapsulated in a simple structure together with a valid-flag. If an element should be removed,
 * the valid-flag is invalidated but the element remains in the collection, foregoing the expensive
 * linear remove operation from the standard <tt>PriorityQueue</tt> implementation. Polling for
 * elements makes sure only valid elements are returned, all others are just ignored.
 *
