...
/**
 * An implementation of a QuadTree to store data assigned to geometric points. The expected bounds
 * of all added points must be given to the constructor for working properly. Correct functioning of
 * the QuadTree with elements being added outside of the given bounds cannot be guaranteed.<br>
 * At one location, several different objects can be put. An object can be put to the QuadTree at
 * different locations. But an object cannot be put more than once at the same location.
 *
 * @author mrieser
...
public class QuadTree<T> implements Serializable {
  ...
  /**
   * A cache to store all values of the QuadTree so it does not have to be computed for every call
   * to {@link #values()}. This is similar to TreeMap.java and AbstractMap.java
   */
  ...
  /**
   * Creates an empty QuadTree with the bounds minX/minY -- maxX/maxY. For optimal performance, all
   * points should be evenly distributed within this rectangle.
   *
   ...
   * @return true if insertion was successful and the data structure changed, false otherwise.
   */
  public boolean put(final double x, final double y, final T value) {
    if (!this.top.bounds.containsOrEquals(x, y)) {
      throw new IllegalArgumentException(
          "cannot add a point at x=" + x + ", y=" + y + " with bounds " + this.top.bounds);
    }
   ...
   * @return true if the specified value was found at the specified coordinates and was successfully
   *     removed (data structure changed), false otherwise.
   */
   ...
   *
   * <p>Note by JI (sept '15): This method can be significant faster than calling {@link
   * #getDisk(double, double, double)} and a manual check on the returned elements for >= r_min. For
   * randomly distributed points one can use the following rule-of-thumb: if r_min/r_max > 0.4 this
   * method is likely to be faster than retrieving all elements within r_max.
   *
   * @param x left-right location, longitude
   * @param y up-down location, latitude
   ...
   */
  public Collection<T> getRing(
      final double x, final double y, final double r_min, final double r_max) {
  ...
  public Collection<T> getElliptical(
      final double x1, final double y1, final double x2, final double y2, final double distance) {
    if (Math.pow(distance, 2) < Math.pow((x1 - x2), 2) + Math.pow((y1 - y2), 2)) {
      throw new IllegalArgumentException(
          "wrong ellipse specification: distance must be greater than distance between foci."
              + " x1="
              + x1
              + " y1="
              + y1
  ...
  /**
   * Gets all objects inside the specified boundary. Objects on the border of the boundary are not
   * included.
   *
  ...
  /**
   * Gets all objects inside the specified area. Objects on the border of the area are not included.
   *
   ...
   * @return The objects found within the area.
   */
  public Collection<T> getRectangle(
      final double minX,
      final double minY,
      final double maxX,
      final double maxY,
  ...
  /**
   * @return the minimum x coordinate (left-right, longitude, easting) of the bounds of the
   *     QuadTree.
   */
  ...
  /**
   * @return the maximum x coordinate (left-right, longitude, easting) of the bounds of the
   *     QuadTree.
   */
  ...
  /**
   * Returns a collection view of the values contained in this map. The collection's iterator will
   * return the values in the order that their corresponding keys appear in the tree. The collection
   * is backed by this <tt>QuadMap</tt> instance, so changes to this map are reflected in the
   * collection.
   *
              ...
              Iterator<T> iterator =
                  new Iterator<T>() {
                    ...
                    private void loadNext() {
                      ...
                      while (searching) {
                        int size =
                            this.currentLeaf.value != null ? 1 : this.currentLeaf.values.size();
                        if (this.nextIndex < size) {
                          this.nextIndex++;
                          this.next =
                              this.currentLeaf.value != null
                                  ? this.currentLeaf.value
                                  : this.currentLeaf.values.get(this.nextIndex - 1);
                          searching = false;
                        } else {
  ...
  /**
   * An internal class to hold variable parameters when calling methods. Here a double value is
   * packaged within an object so the value can be changed in a method and the changed value is
   * available outside of a method.
   */
  ...
  /**
   * An internal class to hold variable parameters when calling methods. Here a Leaf value is
   * packaged within an object so the value can be changed in a method and the changed value is
   * available outside of a method.
   *
  ...
  public static class Rect implements Serializable {
    ...
    /**
     * Calculates the (minimum) distance of a given point to the border of the rectangle. If the
     * point lies within the rectangle, the distance is zero.
     *
     ...
     * @return <code>true</code> if the specified coordinates are inside the boundary of this <code>
     *     Rect</code>; <code>false</code> otherwise.
     */
     ...
     * @return <code>true</code> if the specified rect is inside or on the boundary of this <code>
     *     Rect</code>; <code>false</code> otherwise.
     */
    ...
    /**
     * Tests if this <code>Rect</code> intersects another <code>Rect</code>. Intersection is either
     * interior or border of rect.
     *
     * @param other The rectangle that should be tested for intersection.
     * @return <code>true</code> if this <code>Rect</code> intersects the interior of the other
     *     <code>Rect</code>; <code>false</code> otherwise.
     */
    ...
    /**
     * Computes the intersection of this <code>Rect</code> with the specified <code>Rect</code>.
     * Returns a new <code>Rect</code> that represents the intersection of the two rectangles. If
     * the two rectangles do not intersect, the result will be null.
     *
     * @param r the specified <code>Rectangle</code>
     * @return the largest <code>Rectangle</code> contained in both the specified <code>Rectangle
     *     </code> and in this <code>Rectangle</code>; or if the rectangles do not intersect, an
     *     empty rectangle.
     */
    ...
    /**
     * Adds a <code>Rect</code> to this <code>Rect</code>. The resulting <code>Rect</code> is the
     * union of the two rectangles (i.e. the minimum rectangle that contains the two original
     * rectangles)
     *
     ...
     *
     * <p>(and "increase by" means: 1.0: increase it by 100% (scale it by 2.0) -0.5: decrease it by
     * 50% (scale it by 0.5) 0.0: do nothing) michaz
     */
    public Rect scale(double scaleX, double scaleY) {
      scaleY *= this.centerY - this.minY;
      scaleX *= this.centerX - this.minX;
      return new Rect(
          this.minX - scaleX, this.minY - scaleY, this.maxX + scaleX, this.maxY + scaleY);
    }
  ...
  protected static class Node<T> implements Serializable {
    ...
    /* default */ T get(final double x, final double y, final MutableDouble bestDistance) {
      if (this.hasChilds) {
        ...
        if (bestChild != this.northwest
            && this.northwest.bounds.calcDistance(x, y) < bestDistance.value) {
          T value = this.northwest.get(x, y, bestDistance);
          if (value != null) {
            closest = value;
          }
        }
        if (bestChild != this.northeast
            && this.northeast.bounds.calcDistance(x, y) < bestDistance.value) {
          T value = this.northeast.get(x, y, bestDistance);
          if (value != null) {
            closest = value;
          }
        }
        if (bestChild != this.southeast
            && this.southeast.bounds.calcDistance(x, y) < bestDistance.value) {
          T value = this.southeast.get(x, y, bestDistance);
          if (value != null) {
            closest = value;
          }
        }
        if (bestChild != this.southwest
            && this.southwest.bounds.calcDistance(x, y) < bestDistance.value) {
      ...
      if (this.leaves != null) {
        for (Leaf<T> leaf : this.leaves) {
          final double distance1 =
              Math.sqrt((leaf.x - x1) * (leaf.x - x1) + (leaf.y - y1) * (leaf.y - y1));
          // same trick as above, though it should not be useul in the vast
          // majority of cases
          if (distance1 <= maxDistance) {
            final double distance2 =
                Math.sqrt((leaf.x - x2) * (leaf.x - x2) + (leaf.y - y2) * (leaf.y - y2));
    ...
    /* default */ Collection<T> get(
        final double x, final double y, final double maxDistance, final Collection<T> values) {
      if (this.hasChilds) {
    ...
    private void stepInto(
        Node node, double x, double y, double r_min, double r_max, Collection<T> values) {
