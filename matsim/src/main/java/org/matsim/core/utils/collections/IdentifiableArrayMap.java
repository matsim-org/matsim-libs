...
/**
 * Memory-optimized map, backed by a simple array, for storing small number of {@link
 * Identifiable}s. Access using {@link #get(Object)} have a runtime of <code>O(n)</code>, so this
 * map implementation should only be used to store a small number of elements in it. But for small
...
public class IdentifiableArrayMap<S, T extends Identifiable<S>> implements Map<Id<S>, T> {
  ...
  /**
   * Array-backed read-only collection for high-performance, preventing unnecessary array copy
   * operations.
   *
   * @author mrieser / senozon
   * @param <A>
