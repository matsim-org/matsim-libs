...
public abstract class MatsimXmlWriter extends AbstractMatsimWriter {
  ...
  public static final String DEFAULTSCHEMANAMESPACELOCATION =
  ...
  /**
   * Set this flag to true for pretty print xml output, set it to false if no indentation,
   * whitespaces and newlines shall be printed.
   */
   ...
   * @throws UncheckedIOException
   */
  protected final void writeStartTag(String tagname, List<Tuple<String, String>> attributes)
      throws UncheckedIOException {
    this.writeStartTag(tagname, attributes, false, false);
  }

  protected final void writeStartTag(
      String tagname, List<Tuple<String, String>> attributes, boolean closeElement)
      throws UncheckedIOException {
    ...
    try {
      ...
      if (attributes != null) {
        for (Tuple<String, String> t : attributes) {
          this.writer.write(
              " " + t.getFirst() + "=\"" + encodeAttributeValue(t.getSecond()) + "\"");
        }
      }
      if (closeElement) {
        this.writer.write("/>");
  ...
  protected final void writeContent(String content, boolean allowWhitespaces)
      throws UncheckedIOException {
    try {
      if (doPrettyPrint) {
