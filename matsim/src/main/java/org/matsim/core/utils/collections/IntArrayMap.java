 ...
 * with integer keys. Many operations (like {@link #get(Object)}) have a runtime of <code>O(n)
 * </code>, so this map implementation should only be used to store a small number of elements in
 * it. But for small number of elements, this implementation performs very well, especially because
 * of its very low memory overhead.
 *
