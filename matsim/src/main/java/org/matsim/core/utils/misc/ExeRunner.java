...
public abstract class ExeRunner {
  ...
  /*package*/ static final Logger log = LogManager.getLogger(ExeRunner.class);

  /**
   * Runs an executable and waits until the executable finishes or until a certain amount of time
   * has passed (timeout).
   *
   ...
   * @param timeout A timeout in seconds. If the executable is still running after <code>timeout
   *     </code> seconds, the executable is stopped and this method returns.
  ...
  public static int run(
      ...
      final int timeout,
      final String workingDirectory) {
    final ExternalExecutor myExecutor =
        new ExternalExecutor(cmdArgs, stdoutFileName, workingDirectory);
    return waitForFinish(myExecutor, timeout);
  }
  ...
  private static class ExternalExecutor extends Thread {
    ...
    public ExternalExecutor(
        final String cmd, final String stdoutFileName, final String workingDirectory) {
      this.cmd = cmd;
      this.cmdArgs = null;
    ...
    public ExternalExecutor(
        final String[] cmdArgs, final String stdoutFileName, final String workingDirectory) {
    ...
    public void run() {
      try {
        ...
        log.info(
            "Starting external exe with command: "
                + (this.cmd != null ? this.cmd : Arrays.toString(this.cmdArgs)));
