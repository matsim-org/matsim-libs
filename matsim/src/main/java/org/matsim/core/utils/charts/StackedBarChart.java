...
/**
 * Creates two-dimensional, vertical bar charts. Supports multiple series, which are shown next to
...
public class StackedBarChart extends ChartUtil {
  ...
  /**
   * Creates a new BarChart with default category-labels (numbered from 1 to the number of
   * categories). The same as {@link #BarChart(String, String, String, String[]) BarChart(title,
   * xAxisLabel, yAxisLabel, null)}.
   *
   * @param title
   * @param xAxisLabel
   * @param yAxisLabel
  ...
  public void addSeries(final String title, final double[] values) {
    ...
    for (double value : values) {
      String category =
          (cnt > this.categories.length ? Integer.toString(cnt) : this.categories[cnt - 1]);
