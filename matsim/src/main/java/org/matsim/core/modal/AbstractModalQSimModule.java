...
public abstract class AbstractModalQSimModule<M extends Annotation> extends AbstractQSimModule {
   ...
   * AbstractModule.addMobsimListenerBinding() from the AbstractDvrpModeQSimModule.configureQSim()
   * method. (which has happened to me at least twice, michal.mac, feb'19)
   *
   * <p>Normally, if an AbstractQSimModule class is an inner class of an outer AbstractModule class,
   * addMobsimListenerBinding() will call AbstractModule.addMobsimListenerBinding(), which will have
   * no effect ( too late for adding a listener with the controller scope).
   *
   * <p>However, quite likely, the programmer intention is to add a listener with the mobsim/QSim
   * scope, as if addQSimComponentBinding() or addModalQSimComponentBinding() were called.
   *
   * <p>Although less likely, other methods of AbstractModule could also be accidentally called.
   * Therefore, one should consider prefixing calls with "this" (e.g.
   * this.addModalQSimComponentBinding()) when an AbstractQSimModule is inside an AbstractModule.
   *
  ...
  protected final <T extends QSimComponent> void addModalComponent(
      Class<T> componentClass, Key<? extends T> key) {
    bindModal(componentClass).to(key).asEagerSingleton();
    addModalQSimComponentBinding().to(modalKey(componentClass));
  }

  protected final <T extends QSimComponent> void addModalComponent(
      Class<T> componentClass, Provider<? extends T> componentProvider) {
    bindModal(componentClass).toProvider(componentProvider).asEagerSingleton();
    addModalQSimComponentBinding().to(modalKey(componentClass));
  }

  protected final <T extends QSimComponent> void addModalComponent(
      Class<T> componentClass, Class<? extends T> implementation) {
  ...
  protected final <T> Provider<T> modalProvider(
      Function<ModalProviders.InstanceGetter<M>, T> delegate) {
