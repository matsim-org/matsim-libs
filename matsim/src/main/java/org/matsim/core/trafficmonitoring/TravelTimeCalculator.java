...
/**
 * Calculates actual travel times on link from events and optionally also the link-to-link travel
 * times, e.g. if signaled nodes are used and thus turns in different directions at a node may take
 * a different amount of time. <br>
 * Travel times on links are collected and averaged in bins/slots with a specified size (<code>
 * binSize</code>, in seconds, default 900 seconds = 15 minutes).
 *
 * @author dgrether
 * @author mrieser
 */
public final class TravelTimeCalculator
  ...
  private static final String ERROR_STUCK_AND_LINKTOLINK =
  ...
  public static TravelTimeCalculator create(
      Network network, TravelTimeCalculatorConfigGroup group) {
    TravelTimeCalculator calculator = new TravelTimeCalculator(network, group);
    configure(calculator, group, network);
    return calculator;
  }

  private static TravelTimeCalculator configure(
      TravelTimeCalculator calculator, TravelTimeCalculatorConfigGroup config, Network network) {
    ...
    switch (config.getTravelTimeGetterType()) {
      ...
      case "linearinterpolation":
        calculator.travelTimeGetter =
            new LinearInterpolatingTravelTimeGetter(
                calculator.numSlots, calculator.timeSlice, calculator.aggregator);
        break;
      default:
  ...
  @Inject // yyyy why is this needed?  In general, this class is NOT injected, but explicitly
  // constructed in TravelTimeCalculatorModule.  kai, feb'19
  TravelTimeCalculator(
      TravelTimeCalculatorConfigGroup ttconfigGroup, EventsManager eventsManager, Network network) {
    // this injected constructor is not used when getSeparateModes is true
    this(
        network,
        ttconfigGroup.getTraveltimeBinSize(),
        ttconfigGroup.getMaxTime(),
        ttconfigGroup.isCalculateLinkTravelTimes(),
        ttconfigGroup.isCalculateLinkToLinkTravelTimes(),
        ttconfigGroup.isFilterModes(),
  ...
  public TravelTimeCalculator(
      final Network network, TravelTimeCalculatorConfigGroup ttconfigGroup) {
    // one tests needs this public
    // some tests currently use this. they are also quite happy without an events manager.  kai,
    // feb'19
  ...
  public static final class Builder {
    // The idea here is that the config group will NOT be passed into this object any more. kai,
    // feb'19
    ...
    public void configure(TravelTimeCalculatorConfigGroup ttcConfig) {
      ...
      // yyyyyy presumably would like to replace this with setters for {@link TravelTimeDataFactory}
      // and {@link TravelTimeGetter}.  But it ain't that easy because
      // they again depend on material that (currently) is only available _after_ construction of
      // {@link TravelTimeCalculator}.  kai, feb'19
  ...
  private TravelTimeCalculator(
    ...
    // if we just look at one mode, we need to ignore all vehicles with a different mode. However,
    // the info re the mode is only in
  ...
  public void handleEvent(VehicleAbortsEvent event) {
    ...
    if (e != null) {
      ...
      if (this.calculateLinkToLinkTravelTimes && event.getTime() < qsimConfig.getEndTime().seconds()
      // (we think that this only makes problems when the abort is not just because of mobsim end
      // time. kai & theresa, jan'17)
      ) {
  ...
  private TravelTimeDataArray getTravelTimeData(
      final Id<Link> linkId, final boolean createIfMissing) {
  ...
  private TravelTimeDataArray getLinkToLinkTravelTimeData(
      Tuple<Id<Link>, Id<Link>> fromLinkToLink) {
  ...
  private double getLinkToLinkTravelTime(
      final Id<Link> fromLinkId, final Id<Link> toLinkId, double time) {
    if (!this.calculateLinkToLinkTravelTimes) {
      throw new IllegalStateException(
   ...
   *
   * <p>Imagine short bin sizes (e.g. 5min), small links (e.g. 300 veh/hour) and small sample sizes
   * (e.g. 2%). This would mean that effectively in the simulation only 6 vehicles can pass the link
   * in one hour, one every 10min. So, the travel time in one time slot could be >= 10min if two
   * cars enter the link at the same time. If no car enters in the next time bin, the travel time in
   * that time bin should still be >=5 minutes (10min - binSize), and not freespeedTraveltime,
   * because actually every car entering the link in this bin will be behind the car entered before,
   * which still needs >=5min until it can leave.
   *
   * <p>This method ensures that the travel time in a time bin cannot be smaller than the travel
   * time in the bin before minus the bin size.
   */
  private void consolidateData(final TravelTimeData data) {
    synchronized (data) {
      if (data.isNeedingConsolidation()) {

        // initialize prevTravelTime with ttime from time bin 0 and time 0.  (The interface comment
        // already states that
        // having both as argument does not make sense.)
        double prevTravelTime = data.getTravelTime(0, 0.0);
        // changed (1, 0.0) to (0, 0.0) since Michal has convinced me (by a test) that using "1" is
        // wrong
        // because you get the wrong result for time slot number 1.  This change does not affect the
        // existing
        ...
        for (int i = 1; i < this.numSlots; i++) {
          ...
          // if the travel time in the previous time slice was X, then now it is X-S, where S is the
          // time slice:
          double minTravelTime = prevTravelTime - this.timeSlice;

          // if the travel time that has been measured so far is less than that minimum travel time,
          // then do something:
          if (travelTime < minTravelTime) {
            // (set the travel time to the smallest possible travel time that makes sense according
  ...
  public TravelTime getLinkTravelTimes() {
    return new TravelTime() {
      ...
      public double getLinkTravelTime(Link link, double time, Person person, Vehicle vehicle) {
        // right now, the link speed limit comes from the travel time calculator, and this here just
        // overrides it.  One might consider doing all of this here;
        ...
        double linkTTimeFromObservation =
            TravelTimeCalculator.this.getLinkTravelTime(link.getId(), time);
  ...
  public LinkToLinkTravelTime getLinkToLinkTravelTimes() {
    return new LinkToLinkTravelTime() {
      ...
      public double getLinkToLinkTravelTime(
        ...
        double linkTTimeFromObservation =
            TravelTimeCalculator.this.getLinkToLinkTravelTime(
                fromLink.getId(), toLink.getId(), time);
