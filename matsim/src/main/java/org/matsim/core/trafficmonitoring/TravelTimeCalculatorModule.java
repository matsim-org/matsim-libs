...
/**
 * The first module. Mind that it is retrofitted and I didn't touch the classes themselves. For new
 * code, we wouldn't need the extra layer of Providers.
 *
 * @author michaz
 */
public class TravelTimeCalculatorModule extends AbstractModule {
    ...
    if (getConfig().travelTimeCalculator().getSeparateModes()) {
      ...
      if (getConfig().travelTimeCalculator().isCalculateLinkToLinkTravelTimes()) {
        throw new RuntimeException(
            "separate modes together with link2link routing currently not implemented. doesn't look difficult, "
                + "but I cannot say if it would be picked up correctly by downstream modules.  kai, nov'16");
      }
      // go through all modes:
      //			for (final String mode :
      // CollectionUtils.stringToSet(getConfig().travelTimeCalculator().getAnalyzedModesAsString()
      // )) {
      for (final String mode : getConfig().routing().getNetworkModes()) {

        // generate and bind the observer:
        bind(TravelTimeCalculator.class)
            .annotatedWith(Names.named(mode))
            .toProvider(new SingleModeTravelTimeCalculatorProvider(mode))
            .in(Singleton.class);

        // bind the observer to travel time provider (for router):
        addTravelTimeBinding(mode)
            .toProvider(
                new Provider<TravelTime>() {
                  ...
                  public TravelTime get() {
                    return injector
                        .getInstance(Key.get(TravelTimeCalculator.class, Names.named(mode)))
                        .getLinkTravelTimes();
                  }

                  // the following is not there yet (leads to NPE).  Presumably, the collection into
                  // the underlying multi-binder is
                  // done later, and until then it is only available per annotation (as above)? kai,
                  // nov'19
                  //					@Inject Map<String,TravelTime> travelTimes ;
                  //					@Override public TravelTime get() { return travelTimes.get( mode ) ; }
                })
            .in(Singleton.class);
        // (This used to be without "Singleton".  I think that with Singleton it makes more sense,
        // but don't know ramifications. kai, nov'19)
      ...
      // bind the TravelTime objects.  In this case, this just passes on the same information from
      // TravelTimeCalculator to each individual mode:
  ...
  private static class SingleModeTravelTimeCalculatorProvider
    ...
    public TravelTimeCalculator get() {
      //			TravelTimeCalculator calculator = new TravelTimeCalculator(network,
      // config.getTraveltimeBinSize(), config.getMaxTime(),
      //					config.isCalculateLinkTravelTimes(), config.isCalculateLinkToLinkTravelTimes(), true,
      // CollectionUtils.stringToSet(mode));
      ...
      builder.setFilterModes(
          true); // no point asking the config since we are in "separateModes" anyways.
