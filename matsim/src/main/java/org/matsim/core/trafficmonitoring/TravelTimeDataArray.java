...
/**
 * Implementation of {@link TravelTimeData} that stores the data per time bin in simple arrays.
 * Useful if not too many empty time bins (time bins with no traffic on a link) exist, so no memory
 * is wasted.
 *
 * @author mrieser
 */
class TravelTimeDataArray extends TravelTimeData {
  ...
  public double getTravelTime(final int timeSlot, final double now) {
    ...
    // ttime can only be <0 if it never accumulated anything, i.e. if cnt == 0, so just use
    // freespeed
