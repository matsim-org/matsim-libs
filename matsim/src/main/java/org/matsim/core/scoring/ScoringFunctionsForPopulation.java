...
/**
 * This class helps EventsToScore by keeping ScoringFunctions for the entire Population - one per
 * Person -, and dispatching Activities and Legs to the ScoringFunctions. It also gives out the
 * ScoringFunctions, so they can be given other events by EventsToScore. It is not independently
 * useful. Please do not make public.
 *
 * @author michaz
 */
final class ScoringFunctionsForPopulation implements BasicEventHandler {
  ...
  ScoringFunctionsForPopulation(
      ...
      EventsToLegs eventsToLegs,
      Population population,
      ScoringFunctionFactory scoringFunctionFactory,
      Config config) {
    ControllerConfigGroup controllerConfigGroup = config.controller();

    if (controllerConfigGroup.getEventTypeToCreateScoringFunctions()
        == ControllerConfigGroup.EventTypeToCreateScoringFunctions.IterationStarts) {
      controlerListenerManager.addControlerListener((IterationStartsListener) event -> init());
    } else if (controllerConfigGroup.getEventTypeToCreateScoringFunctions()
        == ControllerConfigGroup.EventTypeToCreateScoringFunctions.BeforeMobsim) {
  ...
  private void init() {
    for (Person person : this.population.getPersons().values()) {
      this.agentScorers.put(
          person.getId(), this.scoringFunctionFactory.createNewScoringFunction(person));
  ...
  public void handleEvent(Event o) {
    // this is for the stuff that is directly based on events. note that this passes on _all_ person
    // events, even those which are
    // aggregated into legs and activities. for the time being, not all PersonEvents may "implement
    // HasPersonId". link enter/leave events
    ...
    if (o instanceof HasPersonId) {
      ...
      if (scoringFunction != null) {
        if (o instanceof PersonStuckEvent) {
          ...
          // yy looking at this, I am a bit skeptic if it truly makes sense to not pass this
          // additionally into the general events handling function below.
          // A use case might be different utilities of money by money transaction type (e.g. toll,
          // fare, reimbursement, ...).  kai, mar'17
    ...
    // Pass LinkEnterEvent to person scoring, required e.g. for bicycle where link attributes are
    // observed in scoring
    /*
    ...
    if (o instanceof LinkEnterEvent) {
      ...
      // (this will NOT do the scoring function lookup twice since LinkEnterEvent is not an instance
      // of HasPersonId.  kai, mar'17)
      if (scoringFunction != null) {
        scoringFunction.handleEvent(o);
      }
    }
    ...
    if (o instanceof TeleportationArrivalEvent)
      this.legsDelegate.handleEvent((TeleportationArrivalEvent) o);
    if (o instanceof TransitDriverStartsEvent)
      this.legsDelegate.handleEvent((TransitDriverStartsEvent) o);
    if (o instanceof PersonEntersVehicleEvent)
      this.legsDelegate.handleEvent((PersonEntersVehicleEvent) o);
    if (o instanceof VehicleArrivesAtFacilityEvent)
      this.legsDelegate.handleEvent((VehicleArrivesAtFacilityEvent) o);
    if (o instanceof VehicleEntersTrafficEvent)
      this.legsDelegate.handleEvent((VehicleEntersTrafficEvent) o);
    if (o instanceof VehicleLeavesTrafficEvent)
      this.legsDelegate.handleEvent((VehicleLeavesTrafficEvent) o);
  }
  ...
  private void callTripScoring(ActivityStartEvent event) {
    ...
    if (plan != null) {
      // we are at a real activity, which is not the first one we see for this agent.  output the
      // trip ...
      Activity activity =
      ...
      // yyyyyy should in principle only return one trip.  There are, however, situations where it
      // returns two trips, in particular
      // in conjunction with the minibus raptor.  Possibly something that has to do with not
      // alternating between acts and legs.
      // (To make matters worse, it passes on my local machine, but fails in jenkins.  Possibly, the
      // byte buffer memory management
      // in the minibus raptor implementation has issues--???) kai, sep'18

      ScoringFunction scoringFunction =
          ScoringFunctionsForPopulation.this.getScoringFunctionForAgent(event.getPersonId());
      for (Trip trip : trips) {
        if (trip != null) {
  ...
  void handleLeg(PersonExperiencedLeg o) {
    ...
    Leg leg = o.getLeg();
    ScoringFunction scoringFunction =
        ScoringFunctionsForPopulation.this.getScoringFunctionForAgent(agentId);
    if (scoringFunction != null) {
  ...
  /**
   * Returns the scoring function for the specified agent. If the agent already has a scoring
   * function, that one is returned. If the agent does not yet have a scoring function, a new one is
   * created and assigned to the agent and returned.
   *
