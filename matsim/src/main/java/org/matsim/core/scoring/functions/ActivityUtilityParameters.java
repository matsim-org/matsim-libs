...
/**
 * Class that converts the config parameters into parameters that are used by the scoring. Design
 * thoughts:
 *
 * <ul>
 *   <li>Class name feels a bit like a misnomer to me since the class seems to make sense only for
 *       the so-called Charypar-Nagel scoring, especially with the zeroUtilityDuration in the ctor.
 *       Yet maybe one might be able to rework the approach so that the most important params (such
 *       as typical duration, slope at typical duration, ...) become universal. kai, nov'12
 * </ul>
 *
 * @author nagel
 */
public final class ActivityUtilityParameters implements MatsimParameters {
  ...
  public static final class SameAbsoluteScore implements ZeroUtilityComputation {
    ...
    public double computeZeroUtilityDuration_s(double priority, double typicalDuration_s) {
      ...
      final double zeroUtilityDuration =
          typicalDuration_s1 * Math.exp(-10.0 / (typicalDuration_s1 / 3600.0) / priority1);
      // ( the 3600s are in there because the original formulation was in "hours".  So the values in
      // seconds are first
  ...
  /**
   * This is now deliberately an unmodifiable object which can only instantiated by a builder. If
   * you want/need to modify this design, please talk to kai nagel or michael zilske or marcel
   * rieser. kai, nov'12
   *
   * @author nagel
   */
  public static final class Builder {
    ...
    public Builder(ActivityParams ppp) {
      ...
      // seems to be somewhat overkill to set a computation method that is only used in the builder
      // ... but the builder has a method to
      // (re)set the
    }
    ...
    public ActivityUtilityParameters build() {
      ...
      this.typicalDuration_s.ifDefined(
          duration ->
              params.zeroUtilityDuration_h =
                  this.zeroUtilityComputation.computeZeroUtilityDuration_s(priority, duration)
                      / 3600.);
      // (I think that the only way in which the typical duration can be undefined is if the
      // activity is not scored at all.  Maybe change the condition. kai, may'22)
  ...
  /**
   * "duration at which the [performance] utility starts to be positive" (from Dave's paper,
   * ga-acts-iatbr03.tex, though he called it t_0) (In decimal number of hours.)
   */
  ...
  private void checkConsistency() {
    // if typical duration is <=48 seconds (and priority=1) then zeroUtilityDuration becomes 0.0
    // because of the double precision. This means it is not possible
    // to have activities with a typical duration <=48 seconds (GL/June2011)
    if (this.scoreAtAll && this.zeroUtilityDuration_h == 0.0) {
      throw new RuntimeException(
          "zeroUtilityDuration of type "
              + type
