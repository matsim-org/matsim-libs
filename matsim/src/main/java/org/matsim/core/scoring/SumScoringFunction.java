...
public final class SumScoringFunction implements ScoringFunction {
  ...
  /**
   * NOTE: Despite its somewhat misleading name, only Events that at the same time implement
   * HasPersonId are passed through this interface. This excludes, in particular, LinkEnterEvent and
   * LinkLeaveEvent. This was done for performance reasons, since passing those events to all
   * handlers imposes a significant additional burden. See comments in and implementation of the
   * handleEvent method in {@link EventsToScore}. (yyyyyy Those comments, and possibly also the
   * events filtering, have gone from EventsToScore in commit
   * e718809884cac6a86bdc35ea2a03c10195d37c69 . I don't know if the performance consequences were
   * tested.)
   *
   * @author nagel
   */
  ...
  public final void handleActivity(Activity activity) {
    if (activity.getStartTime().isUndefined() && activity.getEndTime().isDefined()) {
      ...
      throw new RuntimeException(
          "Trying to score an activity without start or end time. Should not happen. Activity="
              + activity);
    }
  }
  ...
  public double getScore() {
    ...
    for (BasicScoring basicScoringFunction : this.basicScoringFunctions) {
      ...
      if (log.isTraceEnabled()) {
        log.trace(
            "Contribution of scoring function: "
                + basicScoringFunction.getClass().getName()
                + " is: "
                + contribution);
      }
      if (Double.isNaN(contribution)) {
        // I consider this dangerous enough to justify a crash. If somebody has strong arguments for
        // NaN scores,
