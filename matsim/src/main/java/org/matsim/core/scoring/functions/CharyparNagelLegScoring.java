...
public class CharyparNagelLegScoring
    implements org.matsim.core.scoring.SumScoringFunction.LegScoring,
  ...
  public CharyparNagelLegScoring(
      final ScoringParameters params, Network network, Set<String> ptModes) {
    this.params = params;
  ...
  protected double calcLegScore(
    ...
    if (modeParams == null) {
      if (leg.getMode().equals(TransportMode.transit_walk)
        ...
        throw new RuntimeException(
            "just encountered mode for which no scoring parameters are defined: " + leg.getMode());
      }
    }
      ...
      if (Double.isNaN(dist)) {
        if (ccc < 10) {
          ...
          LogManager.getLogger(this.getClass())
              .warn(
                  "distance is NaN. Will make score of this plan NaN. Possible reason: Simulation does not report "
    ...
    // (yyyy once we have multiple legs without "real" activities in between, this will produce
    // wrong results.  kai, dec'12)
    // (yy NOTE: the constant is added for _every_ pt leg.  This is not how such models are
    // estimated.  kai, nov'12)
    ...
    if (!modesAlreadyConsideredForDailyConstants.contains(leg.getMode())) {
      tmpScore +=
          modeParams.dailyUtilityConstant
              + modeParams.dailyMoneyConstant * this.marginalUtilityOfMoney;
      modesAlreadyConsideredForDailyConstants.add(leg.getMode());
    }
    // yyyy the above will cause problems if we ever decide to differentiate pt mode into bus, tram,
    // train, ...
  ...
  public void handleEvent(Event event) {
    ...
    if (event instanceof PersonEntersVehicleEvent && currentLegIsPtLeg) {
      ...
      // add score of waiting, _minus_ score of travelling (since it is added in the legscoring
      // above):
      this.score +=
          (event.getTime() - this.lastActivityEndTime)
              * (this.params.marginalUtilityOfWaitingPt_s
                  - this.params.modeParams.get(TransportMode.pt).marginalUtilityOfTraveling_s);
    }
    ...
    if (event instanceof PersonDepartureEvent) {
      ...
      if (currentLegIsPtLeg) {
        if (!this.nextStartPtLegIsFirstOfTrip) {
          ...
          // (yyyy deducting this again, since is it wrongly added above.  should be consolidated;
          // this is so the code
