...
public final class CharyparNagelActivityScoring
    implements org.matsim.core.scoring.SumScoringFunction.ActivityScoring {
  ...
  public CharyparNagelActivityScoring(
      final ScoringParameters params, final OpeningIntervalCalculator openingIntervalCalculator) {
    this.params = params;
  ...
  protected double calcActScore(
    ...
    if (actParams.isScoreAtAll()) {
      ...
      if (latestStartTime.isDefined() && (activityStart > latestStartTime.seconds())) {
        tmpScore +=
            this.params.marginalUtilityOfLateArrival_s
                * (activityStart - latestStartTime.seconds());
      }
      ...
      if (this.params.usingOldScoringBelowZeroUtilityDuration) {
        if (duration > 0) {
          double utilPerf =
              this.params.marginalUtilityOfPerforming_s
                  * typicalDuration
        ...
        if (duration >= 3600. * actParams.getZeroUtilityDuration_h()) {
          ...
          //					if ( wrnCnt < 1 ) {
          //						wrnCnt++ ;
          //						log.warn("encountering duration < zeroUtilityDuration; the logic for this was
          // changed around mid-nov 2013.") ;
          //						log.warn( "your final score thus will be different from earlier runs; set
          // usingOldScoringBelowZeroUtilityDuration to true if you "
          //								+ "absolutely need the old version.  See
          // https://matsim.atlassian.net/browse/MATSIM-191." );
          //						log.warn( Gbl.ONLYONCE ) ;
          //					}

          // below zeroUtilityDuration, we linearly extend the slope ...:
          double slopeAtZeroUtility =
              this.params.marginalUtilityOfPerforming_s
                  * typicalDuration
                  / (3600. * actParams.getZeroUtilityDuration_h());
      ...
      if ((earliestEndTime.isDefined()) && (activityEnd < earliestEndTime.seconds())) {
        tmpScore +=
            this.params.marginalUtilityOfEarlyDeparture_s
                * (earliestEndTime.seconds() - activityEnd);
      }
      ...
      if ((minimalDuration.isDefined()) && (duration < minimalDuration.seconds())) {
        tmpScore +=
            this.params.marginalUtilityOfEarlyDeparture_s * (minimalDuration.seconds() - duration);
      }
    }
    return tmpScore;
  }
  ...
  private void handleOvernightActivity(Activity lastActivity) {
    ...
    if (lastActivity.getType().equals(this.firstActivity.getType())
        || this.firstActivity.getType().equals("not specified")) {
      // yyyy find better way to encode "not specified".  It is quite common for travel surveys that
      // the type of the
      // first activity is not encoded at all, and then we can as well assume that it is the same as
      // that of the last.  kai, sep'16
      ...
      if (firstLastActOpeningTimesWarning <= 10) {
        ...
        if (openInterval[0].isDefined() || openInterval[1].isDefined()) {
          log.warn(
              "There are opening or closing times defined for the first and last activity. The correctness of the scoring function can thus not be guaranteed.");
      ...
      double calcActScore =
          calcActScore(
              lastActivity.getStartTime().seconds(),
              this.firstActivity.getEndTime().seconds() + 24 * 3600,
              lastActivity);
      ...
      if (this.params.scoreActs) {
        ...
        if (firstLastActWarning <= last) {
          ...
          log.warn(
              "Will score the first activity from midnight to its end, and the last activity from its start to midnight.");
          log.warn(
              "Because of the nonlinear function, this is not the same as scoring from start to end.");
          log.warn("first activity: " + firstActivity);
          log.warn("last activity: " + lastActivity);
        ...
        this.score +=
            calcActScore(
                lastActivity.getStartTime().seconds(),
                this.params.simulationPeriodInDays * 24 * 3600,
                lastActivity);
      }
    }
  }
