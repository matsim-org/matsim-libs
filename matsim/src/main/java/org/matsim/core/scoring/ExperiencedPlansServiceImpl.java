...
class ExperiencedPlansServiceImpl
  ...
  private static final Logger log = LogManager.getLogger(ExperiencedPlansServiceImpl.class);
  ...
  ExperiencedPlansServiceImpl(
      EventsToActivities eventsToActivities, EventsToLegs eventsToLegs, Scenario scenario) {
  ...
  public synchronized void handleLeg(PersonExperiencedLeg o) {
    // Has to be synchronized because the thing which sends Legs and the thing which sends
    // Activities can run
    // on different threads. Will go away when/if we get a more Actor or Reactive Streams like event
    // infrastructure.
  ...
  public void writeExperiencedPlans(String iterationFilename) {
    ...
    // I removed the "V5" here in the assumption that it is better to move along with future format
    // changes.  If this is
    // undesired, please change back but could you then please also add a comment why you prefer
    // this.  Thanks.
    // kai, jan'16
  }
  ...
  public final void finishIteration() {
    // I separated this from "writeExperiencedPlans" so that it can be called separately even when
    // nothing is written.  Can't say
    // if the design might be better served by an iteration ends listener.  kai, feb'17
    for (Map.Entry<Id<Person>, Plan> entry : this.agentRecords.entrySet()) {
      Plan plan = entry.getValue();
      if (scoringFunctionsForPopulation != null) {
        plan.setScore(
            scoringFunctionsForPopulation.getScoringFunctionForAgent(entry.getKey()).getScore());
