...
class NewScoreAssignerImpl implements NewScoreAssigner {
	...
	private final Map<Plan, Integer> msaContributions = new HashMap<>();
	...
	@Override
	public void assignNewScore(int iteration, ScoringFunction scoringFunction, Person person) {
		...
		if (explainScores) {
			...
			scoringFunction.explainScore(explanation);
		...
		if (oldScore == null) {
			...
			if (plan.getScore().isNaN()) {
				log.warn("score is NaN; plan:" + plan);
			}
		} else {
			if (this.scoreMsaStartsAtIteration == null || iteration < this.scoreMsaStartsAtIteration) {
				...
				if (plan.getScore().isNaN()) {
					log.warn("score is NaN; plan:" + plan + " with lrn: " + this.learningRate + " oldScore: " + oldScore + " simScore: " + score + " newScore: " + newScore);
				}
			} else {
