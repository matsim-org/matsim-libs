...
/**
 * In MATSim, generally Cartesian Coordinates are used, with x increasing to the right, and y
 * increasing to the top: {@literal ^ y | | x (0/0) ----> }
 */
public final class Coord implements Serializable {
   ...
   *
   * <p>Coord coord = new Gson().fromJson("{\"x\":123.0,\"y\":456.0}", Coord.class); Without the
   * constructor, the default values are ignored and the non-existing z-coordinate is initialized
   * with the JAVA default of 0.0. {@linkplain Double}s would be initialized with {@code null}. This
   * could also better indicate that there is indeed no information on the z-coordinate. However,
   * memory consumption goes up. AN, 09/2016
   */
  public Coord() {}
