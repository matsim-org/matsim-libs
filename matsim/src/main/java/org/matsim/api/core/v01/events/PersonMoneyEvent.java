...
/**
 * This event specifies that an agent has gained (or paid) some money. Scoring functions should
 * handle these Events by adding the amount somehow to the score (this can include some kind of
 * normalization with the agent's income or something similar).
 *
 * @author mrieser
 */
public final class PersonMoneyEvent extends Event implements HasPersonId {
  ...
  /**
   * Creates a new event describing that the given <tt>agent</tt> has <em>gained</em> some money at
   * the specified <tt>time</tt>. Positive values for <tt>amount</tt> mean the agent has gained
   * money, negative values that the agent has paid money. <br>
   * There are three optional fields: <tt>purpose</tt> and <tt>transactionPartner</tt>. Those are
   * currently not read by any core MATSim code, but can be useful for analysis, e.g. calculate the
   * drt fare revenue (purpose = "drtFare") by drt operator (transactionPartner = "Greedy Shared
   * Taxis Inc."). Finally, there is a <tt>reference</tt>, for example to specify a link or any
   * other information one might find useful.
   *
   * @param time
   * @param agentId
   * @param amount
