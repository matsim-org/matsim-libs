...
/**
 * In the same way as {@link TransportMode} this is meant to somewhat standardize often used
 * activity types. Also as in the same way as {@link TransportMode}, the list here is not
 * exhaustive, and can even be completely ignored. <br>
 * There are loosely based on what is in the Victoria trip survey data ("VISTA"), modified by what
 * was useful in our own experience. <br>
 * I put the work01 etc in (the numbers referring to their typical duration) since this seems most
 * necessary for work. Feel free to put other ones with typical durations in if truly needed.
 * Outside users can (evidently) also program them, in the sense of
 *
 * <pre>
...
public final class DefaultActivityTypes {
  ...
  /**
   * Most trip diary surveys are trip oriented. In consequence, the start with the first trip, which
   * has a purpose, which is the activity it is going to. In consequence, the activity type it is
   * coming from is missing.
   */
