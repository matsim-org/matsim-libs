...
public interface Network extends MatsimToplevelContainer, Attributable {
  ...
  /**
   * Returns a set of this network's nodes. This set might be empty, but it should not be <code>null
   * </code>.
   *
  ...
  /**
   * Returns a set of this network's links. This set might be empty, but it should not be <code>null
   * </code>.
   *
  ...
  /**
   * Returns the time period over which the capacity of the given links has been measured. The
   * default is given in the dtd. Currently (may'11) it is 1h = 3600.0 sec.
   *
   * <p>Notes:
   *
   * <ul>
   *   <li>There is no setter for this value since API-based network generation code should not use
   *       anything else but the default. The default is in the network dtd, but it is an attribute
   *       under "links", not under "network".
   * </ul>
   *
  ...
  /**
   * Returns the lane width of the network's links. The default is given in the dtd; current
   * (may'11) it is 3.75m.
   *
   * <p>Notes:
   *
  ...
  /**
   * Removes the node with the specified Id from the network, along with all links connected to that
   * node. The return value corresponds to the behavior of {@link Map#remove(Object)}
   *
   ...
   * its to- and from-Node, but does not remove any node this link is connected to. The return value
   * corresponds to the behavior of {@link Map#remove(Object)}
   *
