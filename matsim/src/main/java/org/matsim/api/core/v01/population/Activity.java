...
public interface Activity extends PlanElement {
   ...
   *     <p>Note that there is deliberately no way to set the coordinate except at creation. We
   *     might consider something like moveActivityTo( linkid, coord ). kai, aug'10
   */
  ...
  /**
   * @return the if of the link to which the activity is attached. This may start as null, but is
   *     usually set automatically by the control(l)er before the zeroth iteration.
   *     <p>Note that there is deliberately no way to set the link id except at creation. We might
   *     consider something like moveActivityTo( linkid, coord ). kai, aug'10
   */
