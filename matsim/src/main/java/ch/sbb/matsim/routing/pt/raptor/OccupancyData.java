...
public class OccupancyData {
  ...
  private static final Logger LOG = LogManager.getLogger(OccupancyData.class);
  ...
  public DepartureData getNextAvailableDeparture(
      Id<TransitLine> transitLine,
      Id<TransitRoute> transitRoute,
      Id<TransitStopFacility> stopFacility,
      double time) {
  ...
  //	public Vehicle getVehicle(Id<TransitLine> transitLine, Id<TransitRoute> transitRoute,
  // Id<Departure> departure) {
  //		LineData line = this.lineData.get(transitLine);
  //		if (line == null) {
  //			return null;
  ...
  public int getNextAvailableDeparture(
      SwissRailRaptorData data, SwissRailRaptorData.RRouteStop routeStop, double time) {
    CacheData cache = getCache(data);

    int[] latestWaitStartTimes =
        cache.latestWaitingTimeStartPerRouteStopPerDeparture[routeStop.index];
    ...
    if (pos < 0) {
      // binarySearch returns (-(insertion point) - 1) if the element was not found, which will
  ...
  private synchronized CacheData buildCache(SwissRailRaptorData data) {
    ...
    for (int routeStopIdx = 0; routeStopIdx < routeStops.length; routeStopIdx++) {
      ...
      RouteData routeData =
          lineData == null ? null : lineData.routeData.get(routeStop.route.getId());
      StopData stopData =
          routeData == null
              ? null
              : routeData.stopData.get(routeStop.routeStop.getStopFacility().getId());
      ...
      for (int depIdx = 0; depIdx < origDepartures.size(); depIdx++) {
        ...
        if (dd == null) {
          ...
          latestWaitStart =
              Double.isFinite(dd.latestWaitStart) ? (int) dd.latestWaitStart : lastValue;
        }
