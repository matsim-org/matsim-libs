...
public class DefaultRaptorStopFinder implements RaptorStopFinder {
  ...
  @Inject
  public DefaultRaptorStopFinder(
      Config config,
      RaptorIntermodalAccessEgress intermodalAE,
    ...
    if (srrConfig.isUseIntermodalAccessEgress()) {
      for (IntermodalAccessEgressParameterSet params :
          srrConfig.getIntermodalAccessEgressParameterSets()) {
  ...
  public DefaultRaptorStopFinder(
      RaptorIntermodalAccessEgress intermodalAE, Map<String, RoutingModule> routingModules) {
    this.intermodalAE = intermodalAE;
  ...
  public List<InitialStop> findStops(
      ...
      RaptorStopFinder.Direction type) {
    if (type == Direction.ACCESS) {
      return findAccessStops(
          fromFacility, toFacility, person, departureTime, routingAttributes, parameters, data);
    }
    if (type == Direction.EGRESS) {
      return findEgressStops(
          fromFacility, toFacility, person, departureTime, routingAttributes, parameters, data);
    }
    return Collections.emptyList();
  }

  private List<InitialStop> findAccessStops(
      Facility fromFacility,
      Facility toFacility,
      Person person,
      double departureTime,
    ...
    if (srrCfg.isUseIntermodalAccessEgress()) {
      ...
      List<InitialStop> initialStops =
          stops.stream()
              .map(
                  stop -> {
                    double beelineDistance =
                        CoordUtils.calcEuclideanDistance(stop.getCoord(), fromFacility.getCoord());
                    double accessTime = TransitScheduleUtils.getStopAccessTime(stop);
                    double travelTime =
                        Math.ceil(beelineDistance / parameters.getBeelineWalkSpeed()) + accessTime;
                    double disutility =
                        travelTime
                            * -parameters.getMarginalUtilityOfTravelTime_utl_s(TransportMode.walk);
                    return new InitialStop(
                        stop,
                        disutility,
                        travelTime,
                        beelineDistance * distanceFactor,
                        TransportMode.walk);
                  })
  ...
  private List<InitialStop> findEgressStops(
      Facility fromFacility,
      Facility toFacility,
      Person person,
      double departureTime,
      ...
      List<InitialStop> initialStops =
          stops.stream()
              .map(
                  stop -> {
                    double beelineDistance =
                        CoordUtils.calcEuclideanDistance(stop.getCoord(), toFacility.getCoord());
                    double egressTime = TransitScheduleUtils.getStopEgressTime(stop);
                    double travelTime =
                        Math.ceil(beelineDistance / parameters.getBeelineWalkSpeed()) + egressTime;
                    double disutility =
                        travelTime
                            * -parameters.getMarginalUtilityOfTravelTime_utl_s(TransportMode.walk);
                    return new InitialStop(
  ...
  private List<InitialStop> findIntermodalStops(
      Facility fromFacility,
      Facility toFacility,
      Person person,
      double departureTime,
    ...
    switch (srrCfg.getIntermodalAccessEgressModeSelection()) {
      case CalcLeastCostModePerStop:
        for (IntermodalAccessEgressParameterSet parameterSet :
            srrCfg.getIntermodalAccessEgressParameterSets()) {
          addInitialStopsForParamSet(
              fromFacility,
              toFacility,
              person,
              departureTime,
      ...
      case RandomSelectOneModePerRoutingRequestAndDirection:
        ...
        do {
          ...
          addInitialStopsForParamSet(
              ...
              y,
              initialStops,
              srrCfg.getIntermodalAccessEgressParameterSets().get(rndSelector));
          counter++;
          // try again if no initial stop was found for the parameterset. Avoid infinite loop by
          // limiting number of tries.
        } while (initialStops.isEmpty()
            && counter < 2 * srrCfg.getIntermodalAccessEgressParameterSets().size());
        break;
      default:
        throw new RuntimeException(
            srrCfg.getIntermodalAccessEgressModeSelection() + " : not implemented!");
    }
    return initialStops;
  }

  private void addInitialStopsForParamSet(
      Facility fromFacility,
      Facility toFacility,
      Person person,
      double departureTime,
    ...
    if (personMatches) {
      ...
      if (stopFilterAttribute != null) {
        ...
        filteredStopsQT =
            data.stopFilterAttribute2Value2StopsQT.get(stopFilterAttribute).get(stopFilterValue);
      } else {
        filteredStopsQT = data.stopsQT;
      }

      double distance =
          CoordUtils.calcEuclideanDistance(fromFacility.getCoord(), toFacility.getCoord());
      ...
      if (stopFacilities.size() < 2) {
        ...
        double nearestDistance =
            CoordUtils.calcEuclideanDistance(facility.getCoord(), nearestStop.getCoord());
        searchRadius =
            Math.min(
                nearestDistance + paramset.getSearchExtensionRadius(), paramset.getMaxRadius());
      ...
      for (TransitStopFacility stop : stopFacilities) {
        ...
        if (direction == Direction.ACCESS) {
          ...
          routeParts =
              module.calcRoute(
                  DefaultRoutingRequest.of(
                      facility, stopFacility, departureTime, person, routingAttributes));
        } else { // it's Egress
          // We don't know the departure time for the egress trip, so just use the original
          // departureTime,
          ...
          routeParts =
              module.calcRoute(
                  DefaultRoutingRequest.of(
                      stopFacility, facility, departureTime, person, routingAttributes));
          if (routeParts == null) {
            // the router for the access/egress mode could not find a route, skip that access/egress
            // mode
            continue;
          }
        }
        ...
        if ((stopFacility != stop) || accessTime > 0.0 || egressTime > 0.0) {
          if (direction == Direction.ACCESS) {
            ...
            Route transferRoute =
                RouteUtils.createGenericRouteImpl(stopFacility.getLinkId(), stop.getLinkId());
            ...
            Leg transferLeg = PopulationUtils.createLeg(TransportMode.walk);
            Route transferRoute =
                RouteUtils.createGenericRouteImpl(stop.getLinkId(), stopFacility.getLinkId());
        ...
        RaptorIntermodalAccessEgress.RIntermodalAccessEgress accessEgress =
            this.intermodalAE.calcIntermodalAccessEgress(routeParts, parameters, person, direction);
        InitialStop iStop =
            new InitialStop(
                stop, accessEgress.disutility, accessEgress.travelTime, accessEgress.routeParts);
        initialStops.add(iStop);

        if (direction == Direction.EGRESS) {
          // clear the (wrong) departureTime so users don't get confused
          // do it only after passing it to RIntermodalAccessEgress, in case this wants to make some
          // use of it.
  ...
  private List<TransitStopFacility> findNearbyStops(
      Facility facility, RaptorParameters parameters, SwissRailRaptorData data) {
    double x = facility.getCoord().getX();
    double y = facility.getCoord().getY();
    Collection<TransitStopFacility> stopFacilities =
        data.stopsQT.getDisk(x, y, parameters.getSearchRadius());
    if (stopFacilities.size() < 2) {
      TransitStopFacility nearestStop = data.stopsQT.getClosest(x, y);
      double nearestDistance =
          CoordUtils.calcEuclideanDistance(facility.getCoord(), nearestStop.getCoord());
      stopFacilities =
          data.stopsQT.getDisk(x, y, nearestDistance + parameters.getExtensionRadius());
    }
