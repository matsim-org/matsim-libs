...
/**
 * This replicates the functionality of {@link org.matsim.core.router.TransitRouterWrapper}, but in
 * an adapted form suitable for the SwissRailRaptor.
 *
 * @author mrieser / SBB
 */
public class SwissRailRaptorRoutingModule implements RoutingModule {
  ...
  public SwissRailRaptorRoutingModule(
  ...
  public List<? extends PlanElement> calcRoute(RoutingRequest request) {
    ...
    return legs != null ? fillWithActivities(legs) : walkRouter.calcRoute(request);
  }
  ...
  private Coord findCoordinate(Leg prevLeg, Leg nextLeg) {
    if (prevLeg.getRoute() instanceof TransitPassengerRoute) {
      Id<TransitStopFacility> stopId =
          ((TransitPassengerRoute) prevLeg.getRoute()).getEgressStopId();
      return this.transitSchedule.getFacilities().get(stopId).getCoord();
    }
    if (nextLeg.getRoute() instanceof TransitPassengerRoute) {
      Id<TransitStopFacility> stopId =
          ((TransitPassengerRoute) nextLeg.getRoute()).getAccessStopId();
      return this.transitSchedule.getFacilities().get(stopId).getCoord();
    }
    // fallback: prevLeg and nextLeg are not pt routes, so we have to guess the coordinate based on
    // the link id
