...
/**
 * Collects various data related to public transport during the execution of the plans (=during
 * mobsim). For example, for each departure of a transit vehicle at a stop the latest time an agent
 * can arrive to catch a departure is recorder, as is the vehicle load upon departure at a stop.
 *
...
public class OccupancyTracker
  ...
  private static final Logger LOG = LogManager.getLogger(OccupancyTracker.class);
  ...
  private static final VehicleData DUMMY_VEHDATA = new VehicleData(null, null, null, null);
  ...
  public OccupancyTracker(
      OccupancyData data,
      Scenario scenario,
      RaptorInVehicleCostCalculator inVehCostCalculator,
      EventsManager events,
      ScoringParametersForPerson scoringParams) {
    this.data = data;
    this.scenario = scenario;
    this.inVehCostCalculator = inVehCostCalculator;
    // the default in vehicle cost calculator can be ignored
    this.useInVehCosts =
        !inVehCostCalculator.getClass().equals(DefaultRaptorInVehicleCostCalculator.class);
    this.events = events;
  ...
  public void handleEvent(PersonDepartureEvent event) {
    PassengerData pd =
        this.data.paxData.computeIfAbsent(
            event.getPersonId(),
            k -> {
              Person person = this.scenario.getPopulation().getPersons().get(k);
              Map<String, ModeUtilityParameters> modeParams =
                  this.scoringParams.getScoringParameters(person).modeParams;
  ...
  public void handleEvent(TransitDriverStartsEvent event) {
    ...
    this.data.vehicleData.put(
        event.getVehicleId(),
        new VehicleData(
            vehicle, event.getTransitLineId(), event.getTransitRouteId(), event.getDepartureId()));
    LineData line =
        this.data.lineData.computeIfAbsent(event.getTransitLineId(), id -> new LineData());
    RouteData route =
        line.routeData.computeIfAbsent(
  ...
  public void handleEvent(VehicleArrivesAtFacilityEvent event) {
    ...
    this.data.vehicleData.getOrDefault(event.getVehicleId(), DUMMY_VEHDATA).stopFacilityId =
        event.getFacilityId();
  }
  ...
  public void handleEvent(PersonLeavesVehicleEvent event) {
    ...
    if (vehData != null && !this.transitDrivers.contains(event.getPersonId())) {
      ...
      if (this.useInVehCosts) {
        calculateInVehicleCosts(
            event.getTime(), this.data.paxData.get(event.getPersonId()), vehData);
      }
    }
  }
  ...
  private void calculateInVehicleCosts(
      double time, PassengerData passengerData, VehicleData vehData) {
    ...
    double depTime =
        this.data
            ...
            .stopData
            .get(passengerData.boardingStopId)
            .depData
            .get(passengerData.departureId)
            .vehDepTime;
    ...
    EffectiveRouteSegmentIterator iter =
        new EffectiveRouteSegmentIterator(this.data, passengerData, time, vehData);
    double capacityScore =
        -this.inVehCostCalculator.getInVehicleCost(
            travelTime, margUtil_s, person, vehData.vehicle, null, iter);

    double scoreDiff = capacityScore - defaultScore;
    if (Double.isNaN(scoreDiff)) {
      LOG.warn(
          "Getting NaN as score: "
              + passengerData.person.getId()
              + " "
              + vehData.lineId
              + " "
              + vehData.routeId
      ...
      this.events.processEvent(
          new PersonScoreEvent(time, person.getId(), scoreDiff, "pt-occupancy"));
    }
  }
  ...
  public static class EffectiveRouteSegmentIterator implements RouteSegmentIterator {
    ...
    public EffectiveRouteSegmentIterator(
        OccupancyData data,
    ...
    public void next() {
      ...
      while (this.stopIter.hasNext()) {
        ...
        Id<TransitStopFacility> stopId =
            this.nextFromStop != null ? this.nextFromStop.getStopFacility().getId() : null;
        if (searchStart && this.nextFromStop != null && stopId.equals(boardingStopId)) {
          this.nextFromStopDepData =
              this.routeData.stopData.get(stopId).depData.get(this.passengerData.departureId);
          this.hasNext = true;
          break;
        }
        if (!searchStart) {
      ...
      if (this.hasNext) {
        ...
        this.nextToStopDepData =
            stopData == null ? null : stopData.depData.get(this.passengerData.departureId);
        if (this.nextToStopDepData == null) {
          // this can happen if it is on the first service in the morning, and an agent leaves at
          // this stop, where no vehicle yet has departed
