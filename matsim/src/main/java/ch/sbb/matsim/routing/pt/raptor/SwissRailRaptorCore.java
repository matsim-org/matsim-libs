 ...
 *
 * <p>This class is <b>NOT</b> thread-safe due to the use of internal state during the route
 * calculation.
 *
 * @author mrieser / SBB
 */
public class SwissRailRaptorCore {
  ...
  public RaptorRoute calcLeastCostRoute(
      double depTime,
    ...
    final int maxTransfers =
        20; // sensible defaults, could be made configurable if there is a need for it.
    ...
    // go through all egressStops; check if already in destinationStops; if so, check if current
    // cost is smaller; if so, then replace.  This can
    ...
    for (InitialStop egressStop : destinationStops.values()) {
      ...
      if (routeStopIndices != null) {
        for (int routeStopIndex : routeStopIndices) {
          ...
          this.egressCostsPerRouteStop[routeStopIndex] =
              egressStop.accessCost; // set egress costs from given stop
          // presumably, the routeStops are the stops for the different routes that stop at the same
          // stopFacility
        }
      }
    }
    ...
    for (InitialStop stop : initialStops.values()) {
      ...
      for (int routeStopIndex : routeStopIndices) {
        ...
        // (intermodal access is if there are planElements that describe the intermodal access,
        // which depends, I think, on which constructor was
        // called (since also non-intermodal access has a leg). kai, jul'19

        if (!isIntermodalAccess
            && routeStop.routeStop
                == routeStop.route.getStops().get(routeStop.route.getStops().size() - 1)) {
          // this is the last stop of a route, doesn't make sense to start here
          // if it's intermodal, we still start here, as we might transfer to another close-by but
          // non-intermodal stop.
          continue;
        }
        ...
        if (departureIndex >= 0) {
          ...
          // there is no more departure, but we start here by intermodal access, so still register
          // to allow transfers to other (non-)intermodal stops.
          RRouteStop toRouteStop = this.data.routeStops[routeStopIndex];
          PathElement pe =
              new PathElement(
                  null,
    ...
    for (int k = 0; k <= maxTransfers; k++) {
      // first stage (according to paper) is to set earliestArrivalTime_k(stop) =
      // earliestArrivalTime_k-1(stop)
  ...
  public List<RaptorRoute> calcRoutes(
    ...
    for (InitialStop accessStop : accessStops) {
      ...
      if (routeStopIndices != null) {
        for (int routeStopIndex : routeStopIndices) {
          ...
          RRoute route = this.data.routes[routeStop.transitRouteIndex];
          int depOffset = routeStop.departureOffset;
          for (int depIndex = route.indexFirstDeparture;
              depIndex < route.indexFirstDeparture + route.countDepartures;
              depIndex++) {
            ...
            if (depTimeAtStop >= earliestTimeAtStop && depTimeAtStop <= latestTimeAtStop) {
              double costOffset =
                  (depTimeAtStop - earliestTimeAtStop) * marginalUtilityOfWaitingPt_utl_s;
              departures.add(
                  new DepartureAtRouteStop(
                      routeStop, routeStopIndex, depIndex, depTimeAtStop, costOffset, accessStop));
            }
          }
        }
      }
    ...
    for (DepartureAtRouteStop depAtRouteStop : departures) {
      ...
      { // initialization for this departure Time
        ...
        PathElement pe =
            new PathElement(
                null,
                toRouteStop,
                depAtRouteStop.depTime,
                depAtRouteStop.depTime,
                arrivalTime,
                arrivalCost,
                0,
                depAtRouteStop.accessStop.distance,
                0,
                true,
                null,
                depAtRouteStop.accessStop);
        this.arrivalPathPerRouteStop[routeStopIndex] = pe;
        this.leastArrivalCostAtRouteStop[routeStopIndex] = arrivalCost;
        this.arrivalPathPerStop[toRouteStop.stopFacilityIndex] = pe;
        this.leastArrivalCostAtStop[toRouteStop.stopFacilityIndex] = arrivalCost;
      ...
      for (int k = 0; k <= maxTransfers; k++) {
        ...
        PathElement leastCostPath = findLeastCostArrival(destinationStops);
        if (leastCostPath != null
            && (lastFoundBestPath == null
                || leastCostPath.comingFrom != lastFoundBestPath.comingFrom)) {
          ...
          RaptorRoute raptorRoute =
              createRaptorRoute(fromFacility, toFacility, leastCostPath, depTime);
          leastCostPath.arrivalTravelCost += depAtRouteStop.costOffset;
  ...
  private double calculateOptimalDepartureTime(
      PathElement leastCostPath, Map<PathElement, InitialStop> initialStopsPerStartPath) {
    ...
    // for more realism, a (random) value from a distribution could be taken instead of a fixed
    // value
  ...
  public Map<Id<TransitStopFacility>, TravelInfo> calcLeastCostTree(
      double depTime,
      Collection<InitialStop> startStops,
      RaptorParameters parameters,
      Person person) {
    reset();
  ...
  private TravelInfo getTravelInfo(PathElement destination, RaptorParameters parameters) {
    ...
    if (destination.isTransfer && transferCount > 0) {
      transferCount--; // do not count this as transfer, as the router would merge it with the
      // egress walk
    }
    if (secondStage != null && secondStage.isTransfer && transferCount > 0) {
      transferCount--; // the first "leg" is a transfer, do not count it as such as the router would
      // merge it with the access walk
    }
    Id<TransitStopFacility> departureStopId =
        firstStage.toRouteStop.routeStop.getStopFacility().getId();
    return new TravelInfo(
        departureStopId,
        departureTimeAtFirstStop,
        arrivalTimeAtLastStop,
        travelCost,
        accessTime,
        accessCost,
        transferCount,
  ...
  private void exploreRoutes(RaptorParameters parameters, Person person) {
    ...
    int routeIndex = -1;
    for (int firstRouteStopIndex = this.improvedRouteStopIndices.nextSetBit(0);
        firstRouteStopIndex >= 0;
        firstRouteStopIndex = this.improvedRouteStopIndices.nextSetBit(firstRouteStopIndex + 1)) {
      ...
      int currentDepartureIndex =
          findNextDepartureIndex(route, firstRouteStop, agentFirstArrivalTime);
      if (currentDepartureIndex >= 0) {
        ...
        if ((currentTravelCostWhenBoarding + currentTransferCostWhenBoarding)
            > this.bestArrivalCost) {
          continue;
        }
        routeIndex = tmpRouteIndex;
        int firstDepartureTime =
            (boardingPE.firstDepartureTime == TIME_UNDEFINED)
                ? currentAgentBoardingTime
                : boardingPE.firstDepartureTime;

        double marginalUtilityOfTravelTime_utl_s =
            parameters.getMarginalUtilityOfTravelTime_utl_s(boardingPE.toRouteStop.mode);
        transferProvider.reset(boardingPE.transfer);

        for (int toRouteStopIndex = firstRouteStopIndex + 1;
            toRouteStopIndex < route.indexFirstRouteStop + route.countRouteStops;
            toRouteStopIndex++) {
          routeSegmentIterator.reset(
              currentDepartureIndex,
              currentAgentBoardingTime,
              currentBoardingRouteStopIndex,
              toRouteStopIndex);
          ...
          double inVehicleCost =
              this.inVehicleCostCalculator.getInVehicleCost(
                  inVehicleTime,
          ...
          double arrivalTransferCost =
              (boardingPE.firstDepartureTime != TIME_UNDEFINED)
                  ? (currentTransferCostWhenBoarding
                      + this.transferCostCalculator.calcTransferCost(
                          transferProvider,
                          parameters,
                          arrivalTime - firstDepartureTime,
                          boardingPE.transferCount,
                          boardingPE.arrivalTransferCost,
                          boardingPE.arrivalTime))
                  : 0;
          ...
          if (totalArrivalCost <= previousArrivalCost) {
            double distance =
                toRouteStop.distanceAlongRoute - boardingPE.toRouteStop.distanceAlongRoute;
            PathElement pe =
                new PathElement(
                    boardingPE,
                    toRouteStop,
            ...
            if (alternativeDepartureIndex >= 0) {
              ...
              int alternativeVehicleArrivalTime =
                  alternativeDepartureTime + toRouteStop.arrivalOffset;
              int alternativeAgentBoardingTime =
                  Math.max(alternativeAgentFirstArrivalTime, alternativeVehicleArrivalTime);
              int alternativeWaitingTime =
                  alternativeAgentBoardingTime - alternativeAgentFirstArrivalTime;
              double alternativeWaitingCost =
                  -marginalUtilityOfWaitingPt_utl_s * alternativeWaitingTime;
              double alternativeTravelCostWhenBoarding =
                  alternativeBoardingPE.arrivalTravelCost + alternativeWaitingCost;
              double alternativeTotalCostWhenBoarding =
                  alternativeTravelCostWhenBoarding + alternativeBoardingPE.arrivalTransferCost;
              if (alternativeTotalCostWhenBoarding < totalArrivalCost) {
                ...
                if (!alternativeBoardingPE.isTransfer) {
                  // we improve to a line/route we entered at some earlier stop, do not create a new
                  // transfer for this,
                  ...
                  alternativeVehicleArrivalTime =
                      alternativeDepartureTime + alternativeBoardingPE.toRouteStop.arrivalOffset;
                  alternativeAgentBoardingTime =
                      Math.max(alternativeAgentFirstArrivalTime, alternativeVehicleArrivalTime);

                  alternativeWaitingTime =
                      alternativeAgentBoardingTime - alternativeAgentFirstArrivalTime;
                  alternativeWaitingCost =
                      -marginalUtilityOfWaitingPt_utl_s * alternativeWaitingTime;
                  alternativeTravelCostWhenBoarding =
                      alternativeBoardingPE.arrivalTravelCost + alternativeWaitingCost;
                }
          ...
          firstRouteStopIndex =
              toRouteStopIndex; // we've handled this route stop, so we can skip it in the outer
          // loop
        }
      }
    }
  ...
  private void handleTransfers(boolean strict, RaptorParameters raptorParams) {
    ...
    double margUtilityTransitWalk =
        raptorParams.getMarginalUtilityOfTravelTime_utl_s(
            TransportMode.walk); // replaced TransportMode.transit_walk with walk
    CachingTransferProvider transferProvider = this.data.new CachingTransferProvider();

    for (int stopIndex = this.improvedStops.nextSetBit(0);
        stopIndex >= 0;
        stopIndex = this.improvedStops.nextSetBit(stopIndex + 1)) {
      ...
      RRouteStop fromRouteStop =
          fromPE.toRouteStop; // this is the route stop we arrive with least cost at stop
      ...
      for (int transferIndex = firstTransferIndex;
        ...
        double newArrivalTravelCost =
            arrivalTravelCost - transfer.transferTime * margUtilityTransitWalk;
        double newArrivalTransferCost =
            (fromPE.firstDepartureTime != TIME_UNDEFINED)
                ? (arrivalTransferCost
                    + this.transferCostCalculator.calcTransferCost(
                        transferProvider,
                        raptorParams,
                        newArrivalTime - fromPE.firstDepartureTime,
                        fromPE.transferCount + 1,
                        arrivalTransferCost,
                        arrivalTime))
                : 0;
        ...
        if (newTotalArrivalCost < prevLeastArrivalCost
            || (!strict && newTotalArrivalCost <= prevLeastArrivalCost)) {
          RRouteStop toRouteStop = this.data.routeStops[toRouteStopIndex];
          PathElement pe =
              new PathElement(
                  fromPE,
                  toRouteStop,
                  fromPE.firstDepartureTime,
                  TIME_UNDEFINED,
                  newArrivalTime,
                  newArrivalTravelCost,
                  newArrivalTransferCost,
                  transfer.transferDistance,
                  fromPE.transferCount + 1,
                  true,
                  transfer,
                  null);
          ...
          if (newTotalArrivalCost < prevLeastArrivalCost
              || (!strict && newTotalArrivalCost <= prevLeastArrivalCost)) {
            // store it in tmp only. We don't want that this PE is used by a stop processed later in
            // the same round. ("parallel update")
    ...
    for (int stopIndex = this.tmpImprovedStops.nextSetBit(0);
        stopIndex >= 0;
        stopIndex = this.tmpImprovedStops.nextSetBit(stopIndex + 1)) {
  ...
  private PathElement findLeastCostArrival(Map<TransitStopFacility, InitialStop> destinationStops) {
    ...
    SwissRailRaptorConfigGroup.IntermodalLegOnlyHandling intermodalLegOnlyHandling =
        data.config.getIntermodalLegOnlyHandling();
    boolean checkBothPtAndPurelyIntermodalRoutes =
        intermodalLegOnlyHandling.equals(SwissRailRaptorConfigGroup.IntermodalLegOnlyHandling.avoid)
            || intermodalLegOnlyHandling.equals(
    ...
    for (Map.Entry<TransitStopFacility, InitialStop> e : destinationStops.entrySet()) {
      ...
      if (pe != null) {
        ...
        if ((totalCost < leastCost)
            || (totalCost == leastCost && pe.transferCount < leastCostPath.transferCount)) {
          PathElement egressLegCandidate =
              new PathElement(
                  pe,
                  null,
                  pe.firstDepartureTime,
                  TIME_UNDEFINED,
                  arrivalTime,
                  arrivalTravelCost,
                  pe.arrivalTransferCost,
                  egressStop.distance,
                  pe.transferCount,
                  true,
                  null,
  ...
  private static RaptorRoute createRaptorRoute(
      Facility fromFacility,
      Facility toFacility,
      PathElement destinationPathElement,
      double departureTime) {
    LinkedList<PathElement> pes = new LinkedList<>();
    double arrivalCost = Double.POSITIVE_INFINITY;
    if (destinationPathElement != null) {
      arrivalCost =
          destinationPathElement.arrivalTravelCost + destinationPathElement.arrivalTransferCost;
    ...
    for (PathElement pe : pes) {
      ...
      TransitStopFacility toStop =
          pe.toRouteStop == null ? null : pe.toRouteStop.routeStop.getStopFacility();
      ...
      if (pe.initialStop != null && pe.initialStop.planElements != null) {
        ...
        // add (peCount > 2 || peCount == 2 && !pes.get(0).isTransfer) && to catch case of only
        // access and egress
        ...
        //
        // in case of peCount < 2 there should be no effect, because peCount-2 < 0 and i will be 0,
        // so i!=peCount - 2
        if ((peCount > 2 || peCount == 2 && !pes.get(0).isTransfer)
            && i == peCount - 2
            && pes.get(i + 1).isTransfer
  ...
  private static class DepartureAtRouteStop {
    ...
    DepartureAtRouteStop(
        RRouteStop routeStop,
        int routeStopIndex,
        int departureIndex,
        int depTime,
        double costOffset,
        InitialStop accessStop) {
  ...
  public static final class TravelInfo {
    ...
    /**
     * The cost for travelling from the first stop to the last stop. Not included are accessCost or
     * cost for waiting at the first stop.
     */
    public final double travelCost;

    /**
     * The time required to travel from the origin to the first stop. Not included in {@link
     * #ptTravelTime}.
     */
    ...
    /**
     * the costs an agent accumulates due to waiting at the first stop until the first pt vehicle
     * departs.
     */
    ...
    public RaptorRoute getRaptorRoute() {
      ...
      return createRaptorRoute(
          fromFacility, toFacility, this.destinationPath, firstPath.arrivalTime);
    }
  ...
  private static class RouteSegmentIteratorImpl implements RouteSegmentIterator {
    ...
    public void next() {
      ...
      DepartureData depData =
          this.data.occupancyData.getDepartureData(
              nextRouteStop.line.getId(),
              nextRouteStop.route.getId(),
              depRouteStop.routeStop.getStopFacility().getId(),
              this.currentDepartureId);
