...
/**
 * Provides public transport route search capabilities using an implementation of the RAPTOR
...
public class SwissRailRaptor implements TransitRouter {
  ...
  public List<? extends PlanElement> calcRoute(RoutingRequest request) {
    ...
    if (parameters.getConfig().isUseRangeQuery()) {
      return this.performRangeQuery(
          fromFacility, toFacility, departureTime, person, routingAttributes, parameters);
    }
    List<InitialStop> accessStops =
        findAccessStops(
            fromFacility, toFacility, person, departureTime, routingAttributes, parameters);
    List<InitialStop> egressStops =
        findEgressStops(
            fromFacility, toFacility, person, departureTime, routingAttributes, parameters);

    RaptorRoute foundRoute =
        this.raptor.calcLeastCostRoute(
            departureTime, fromFacility, toFacility, accessStops, egressStops, parameters, person);
    RaptorRoute directWalk =
        createDirectWalk(fromFacility, toFacility, departureTime, person, parameters);
     ...
     */
    if (directWalk.getTotalCosts() * parameters.getDirectWalkFactor()
        < foundRoute.getTotalCosts()) {
      foundRoute = directWalk;
    }
    boolean forbidPurelyIntermodalRoutes =
        data.config
                .getIntermodalLegOnlyHandling()
                .equals(SwissRailRaptorConfigGroup.IntermodalLegOnlyHandling.forbid)
            || data.config
                .getIntermodalLegOnlyHandling()
                .equals(SwissRailRaptorConfigGroup.IntermodalLegOnlyHandling.returnNull);
    if (foundRoute == null || (foundRoute.parts.size() == 0 && forbidPurelyIntermodalRoutes)) {
      if (person == null) {
        log.debug(
            "No route found for person null: trip from x="
                + fromFacility.getCoord().getX()
                + ",y="
                + fromFacility.getCoord().getY()
                + " departure at "
                + departureTime
                + " to x="
                + toFacility.getCoord().getX()
                + ",y="
                + toFacility.getCoord().getY());
      } else {
        log.debug(
            "No route found for person "
                + person.getId()
                + ": trip from x="
                + fromFacility.getCoord().getX()
                + ",y="
    ...
    List<? extends PlanElement> legs =
        RaptorUtils.convertRouteToLegs(foundRoute, this.data.config.getTransferWalkMargin());
    return legs;
  }
  ...
  private List<? extends PlanElement> performRangeQuery(
    ...
    SwissRailRaptorConfigGroup.RangeQuerySettingsParameterSet rangeSettings =
        srrConfig.getRangeQuerySettings(subpopulation);
    ...
    if (this.defaultRouteSelector instanceof ConfigurableRaptorRouteSelector selector) {
      ...
      SwissRailRaptorConfigGroup.RouteSelectorParameterSet params =
          srrConfig.getRouteSelector(subpopulation);
  ...
  public List<? extends PlanElement> calcRoute(
      ...
      Person person,
      Attributes routingAttributes,
      RaptorRouteSelector selector) {
    RaptorParameters parameters = this.parametersForPerson.getRaptorParameters(person);
    List<InitialStop> accessStops =
        findAccessStops(
            fromFacility, toFacility, person, desiredDepartureTime, routingAttributes, parameters);
    List<InitialStop> egressStops =
        findEgressStops(
            fromFacility, toFacility, person, desiredDepartureTime, routingAttributes, parameters);

    List<RaptorRoute> foundRoutes =
        this.raptor.calcRoutes(
            earliestDepartureTime,
            desiredDepartureTime,
            latestDepartureTime,
            fromFacility,
    ...
    RaptorRoute directWalk =
        createDirectWalk(fromFacility, toFacility, desiredDepartureTime, person, parameters);
  ...
  public List<RaptorRoute> calcRoutes(
      Facility fromFacility,
      Facility toFacility,
      double earliestDepartureTime,
      double desiredDepartureTime,
    ...
    if (foundRoutes.isEmpty()
        || directWalk.getTotalCosts() * parameters.getDirectWalkFactor()
            < foundRoutes.get(0).getTotalCosts()) {
      foundRoutes.add(directWalk); // add direct walk if it seems plausible
    }
    return foundRoutes;
  }

  public Map<Id<TransitStopFacility>, SwissRailRaptorCore.TravelInfo> calcTree(
  ...
  public Map<Id<TransitStopFacility>, SwissRailRaptorCore.TravelInfo> calcTree(
      ...
      RaptorParameters parameters,
      Person person) {
    if (this.data.config.getOptimization() != RaptorStaticConfig.RaptorOptimization.OneToAllRouting
        && !this.treeWarningShown) {
      log.warn(
  ...
  public Map<Id<TransitStopFacility>, SwissRailRaptorCore.TravelInfo> calcTree(
      Facility fromFacility, double departureTime, Person person, Attributes routingAttributes) {
    RaptorParameters parameters = this.parametersForPerson.getRaptorParameters(person);
    List<InitialStop> accessStops =
        findAccessStops(
            fromFacility, fromFacility, person, departureTime, routingAttributes, parameters);
    return this.calcLeastCostTree(accessStops, departureTime, parameters, person);
  }

  private Map<Id<TransitStopFacility>, SwissRailRaptorCore.TravelInfo> calcLeastCostTree(
      Collection<InitialStop> accessStops,
      double departureTime,
      RaptorParameters parameters,
      Person person) {
  ...
  private List<InitialStop> findAccessStops(
      ...
      Attributes routingAttributes,
      RaptorParameters parameters) {
    return this.stopFinder.findStops(
        fromFacility,
        toFacility,
        person,
        departureTime,
  ...
  private RaptorRoute createDirectWalk(
      Facility fromFacility,
      Facility toFacility,
      double departureTime,
      Person person,
    ...
    route.addNonPt(
        ...
        departureTime,
        walkTime,
        beelineDistance * beelineDistanceFactor,
        TransportMode.walk);
    return route;
  }
  ...
  public static class Builder {
    ...
    private RaptorStopFinder stopFinder =
        new DefaultRaptorStopFinder(new DefaultRaptorIntermodalAccessEgress(), null);
    private RaptorInVehicleCostCalculator inVehicleCostCalculator =
        new DefaultRaptorInVehicleCostCalculator();
    private RaptorTransferCostCalculator transferCostCalculator =
    ...
    public SwissRailRaptor build() {
      return new SwissRailRaptor(
          this.data,
          this.parametersForPerson,
          this.routeSelector,
          this.stopFinder,
          this.inVehicleCostCalculator,
          this.transferCostCalculator);
    }
  }
}
