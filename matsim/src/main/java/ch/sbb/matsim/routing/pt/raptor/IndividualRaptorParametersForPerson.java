...
/**
 * An implementation of {@link RaptorParametersForPerson} that returns an individual set of routing
 * parameters, based on {@link ScoringParametersForPerson}.
 *
...
public class IndividualRaptorParametersForPerson implements RaptorParametersForPerson {
  ...
  @Inject
  public IndividualRaptorParametersForPerson(
      Config config, ScoringParametersForPerson parametersForPerson) {
    this.config = config;
  ...
  public RaptorParameters getRaptorParameters(Person person) {
    ...
    for (Map.Entry<String, ScoringConfigGroup.ModeParams> e : pcsConfig.getModes().entrySet()) {
      ...
      if (modeParams != null) {
        raptorParameters.setMarginalUtilityOfTravelTime_utl_s(
            mode, modeParams.marginalUtilityOfTraveling_s - marginalUtilityOfPerforming);
      }
    }
    ...
    if (costPerHour == 0.0) {
      ...
      raptorParameters.setTransferPenaltyFixCostPerTransfer(
          this.raptorConfig.getTransferPenaltyBaseCost());
    }
