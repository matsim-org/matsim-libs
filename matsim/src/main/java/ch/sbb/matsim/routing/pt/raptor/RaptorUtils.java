...
public final class RaptorUtils {
  ...
  public static RaptorStaticConfig createStaticConfig(Config config) {
    ...
    RoutingConfigGroup.TeleportedModeParams walk =
        pcrConfig.getModeRoutingParams().get(TransportMode.walk);
    staticConfig.setBeelineWalkSpeed(
        walk.getTeleportedModeSpeed() / walk.getBeelineDistanceFactor());
    ...
    if (srrConfig.isUseModeMappingForPassengers()) {
      for (SwissRailRaptorConfigGroup.ModeMappingForPassengersParameterSet mapping :
          srrConfig.getModeMappingForPassengers()) {
        staticConfig.addModeMappingForPassengers(
            mapping.getRouteMode(), mapping.getPassengerMode());
      }
    }
  ...
  public static RaptorParameters createParameters(Config config) {
    SwissRailRaptorConfigGroup advancedConfig =
        ConfigUtils.addOrGetModule(config, SwissRailRaptorConfigGroup.class);
    ...
    raptorParams.setMarginalUtilityOfWaitingPt_utl_s(
        trConfig.getMarginalUtilityOfWaitingPt_utl_s());
    ...
    for (Map.Entry<String, ScoringConfigGroup.ModeParams> e : pcsConfig.getModes().entrySet()) {
      ...
      double marginalUtility_utl_s =
          modeParams.getMarginalUtilityOfTraveling() / 3600.0 - marginalUtilityPerforming;
    ...
    if (costPerHour == 0.0) {
      ...
      raptorParams.setTransferPenaltyFixCostPerTransfer(
          advancedConfig.getTransferPenaltyBaseCost());
    }
  ...
  public static List<? extends PlanElement> convertRouteToLegs(
      RaptorRoute route, double transferWalkMargin) {
    ...
    for (RaptorRoute.RoutePart part : route.parts) {
      if (part.planElements != null) {
        ...
        DefaultTransitPassengerRoute ptRoute =
            new DefaultTransitPassengerRoute(part.fromStop, part.line, part.route, part.toStop);
        ...
        if (previousTransferWalkleg != null) {
          ...
          double traveltime =
              Math.max(0, previousTransferWalkleg.getTravelTime().seconds() - transferWalkMargin);
        ...
        Id<Link> startLinkId =
            part.fromStop == null
                ? (route.fromFacility == null ? null : route.fromFacility.getLinkId())
                : part.fromStop.getLinkId();
        Id<Link> endLinkId =
            part.toStop == null
                ? (route.toFacility == null ? null : route.toFacility.getLinkId())
                : part.toStop.getLinkId();
