...
public class SwissRailRaptorConfigGroup extends ReflectiveConfigGroup {
  ...
  private static final String PARAM_INTERMODAL_ACCESS_EGRESS_MODE_SELECTION =
      "intermodalAccessEgressModeSelection";
  private static final String PARAM_INTERMODAL_ACCESS_EGRESS_MODE_SELECTION_DESC =
      "Sets whether intermodal access and egress modes are selected by "
  ...
  private static final String PARAM_TRANSFER_PENALTY_PERHOUR =
      "transferPenaltyCostPerTravelTimeHour";

  private static final String PARAM_USE_CAPACITY_CONSTRAINTS = "useCapacityConstraints";
  private static final String PARAM_USE_CAPACITY_CONSTRAINTS_DESC =
  ...
  private static final String PARAM_TRANSFER_WALK_MARGIN_DESC =
  ...
  private static final String PARAM_INTERMODAL_LEG_ONLYHANDLING_DESC =
      "Define how routes containing only intermodal legs are handled: Useful options: alllow, avoid, forbid";
  ...
  private final List<IntermodalAccessEgressParameterSet> intermodalAccessEgressSettings =
      new ArrayList<>();
  private final Map<String, ModeMappingForPassengersParameterSet>
      modeMappingForPassengersByRouteMode = new HashMap<>();
  ...
  public enum IntermodalLegOnlyHandling {
    /**
     * allows transit routes that only consist of intermodal feeder legs if these have the lowest
     * cost.
     */
    allow,
    /**
     * avoids transit routes that only consist of feeder routes, unless no route containing at least
     * one pt leg is found
     */
    avoid,
    /**
    ...
    forbid,
    /**
     * mimics the behaviour implemented between 2019 and 2023. Returns null if a purely intermodal
     * route has the lowest cost, does not check if a real pt route exists.
     */
    @Deprecated
    returnNull
  }
  ...
  public void setIntermodalAccessEgressModeSelection(
  ...
  public RangeQuerySettingsParameterSet removeRangeQuerySettings(String subpopulation) {
    RangeQuerySettingsParameterSet paramSet =
        this.rangeQuerySettingsPerSubpop.remove(subpopulation);
  ...
  public ModeMappingForPassengersParameterSet getModeMappingForPassengersParameterSet(
      String routeMode) {
  ...
  public static class IntermodalAccessEgressParameterSet extends ReflectiveConfigGroup {
    ...
    public IntermodalAccessEgressParameterSet setSearchExtensionRadius(
        double searchExtensionRadius) {
    ...
    public IntermodalAccessEgressParameterSet setPersonFilterAttribute(
    ...
    public IntermodalAccessEgressParameterSet setShareTripSearchRadius(
        double shareTripSearchRadius) {
    ...
    public Map<String, String> getComments() {
      ...
      map.put(
          PARAM_SHARE_TRIP_SEARCH_RADIUS,
  ...
  public Map<String, String> getComments() {
    ...
    comments.put(
        PARAM_INTERMODAL_ACCESS_EGRESS_MODE_SELECTION,
        PARAM_INTERMODAL_ACCESS_EGRESS_MODE_SELECTION_DESC);
    ...
    if (useIntermodality) {
      ...
      Verify.verify(
          config
              ...
              .equals(HandlingOfPlansWithoutRoutingMode.reject),
          "Using intermodal access and egress in "
              + "combination with plans without a routing mode is not supported.");
      Verify.verify(
          intermodalAccessEgressSettings.size() >= 1,
          "Using intermodal routing, but there are no access/egress "
      ...
      for (IntermodalAccessEgressParameterSet paramset : intermodalAccessEgressSettings) {
        Verify.verifyNotNull(
            paramset.mode,
            "mode of an IntermodalAccessEgressParameterSet "
                + "is undefined. Please set a value in the config.");
        Verify.verify(
            paramset.maxRadius > 0.0,
            "maxRadius of IntermodalAccessEgressParameterSet "
                + "for mode "
                + paramset.mode
                + " is negative or 0. Please set a positive value in the config.");
        Verify.verify(
            paramset.initialSearchRadius > 0.0,
            "initialSearchRadius of IntermodalAccessEgressParameterSet "
                + "for mode "
                + paramset.mode
                + " is negative or 0. Please set a positive value in the config.");
        Verify.verify(
            paramset.searchExtensionRadius > 0.0,
            "searchExtensionRadius of IntermodalAccessEgressParameterSet "
                + "for mode "
                + paramset.mode
                + " is negative or 0. Please set a positive value in the config.");

        Verify.verify(
            paramset.maxRadius >= paramset.initialSearchRadius,
            "maxRadius of IntermodalAccessEgressParameterSet "
                + "for mode "
                + paramset.mode
