...
//	private static Logger log = Logger.getLogger(CetinCompatibleLegTravelTimeEstimatorTest.class);
	...
	public void testGetLegTravelTimeEstimation() {
		...
		EventAgentDeparture depEvent = new EventAgentDeparture(
				departureTime, 
				TEST_PERSON_ID, 
				TEST_LEG_NR, 
				originAct.getLink().getId().toString());
		EventLinkLeave leaveEvent = new EventLinkLeave(departureTime + depDelay, testPerson.getId().toString(), 0, originAct.getLink().getId().toString());
