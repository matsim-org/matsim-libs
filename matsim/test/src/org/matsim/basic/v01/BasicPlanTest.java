...
public class BasicPlanTest extends MatsimTestCase {
	...
	public void testActLegIterator() throws Exception {
		...
		Leg leg1 = plan.createLeg(0, "car", 8.0*3600.0, 60, Time.UNDEFINED_TIME);
		...
		Leg leg2 = plan.createLeg(0, "car", 16.0*3600.0, 60, Time.UNDEFINED_TIME);
	...
	public void testGetIteratorAct() throws Exception {
		...
		// add leg + act, test again
		plan.createLeg(0, "car", 8.0*3600.0, 60, Time.UNDEFINED_TIME);
		Act act2 = plan.createAct("w", 100.0, 100.0, null, 8.0*3600.0, 16.0*3600.9, Time.UNDEFINED_TIME, false);
		iter = plan.getIteratorAct();
		assertEquals(act1, iter.next());
		assertTrue(iter.hasNext());
		...
		// add leg + act, test again. Now we have a generic plan with real first act, real last act, and real middle acts.
		plan.createLeg(0, "car", 16.0*3600.0, 60, Time.UNDEFINED_TIME);
		Act act3 = plan.createAct("h", 0.0, 0.0, null, 16.0*3600.0, Time.UNDEFINED_TIME, Time.UNDEFINED_TIME, false);
		iter = plan.getIteratorAct();
		assertEquals(act1, iter.next());
		assertTrue(iter.hasNext());
