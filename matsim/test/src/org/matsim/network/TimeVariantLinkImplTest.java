...
public class TimeVariantLinkImplTest extends MatsimTestCase {
	...
	public void testMultipleFreespeedChanges() {
		...
		// everything fine so long, now add two more changes that partly overlap

		// - a change event from 11am to 13am
		NetworkChangeEvent change4 = new NetworkChangeEvent(11*3600.0);
		change4.addLink(link);
		change4.setFreespeedChange(new ChangeValue(ChangeType.FACTOR, 0.5));
		change4.setEndTime(13*3600.0);
		link.applyEvent(change4);
		// - a change event starting at 12am, without end
		NetworkChangeEvent change5 = new NetworkChangeEvent(12*3600.0);
		change5.addLink(link);
		change5.setFreespeedChange(new ChangeValue(ChangeType.ABSOLUTE, 40));
		...
		assertEquals(20.0, link.getFreespeed(13*3600.0), EPSILON); // end of change4, apply inverse factor
		assertEquals(20.0, link.getFreespeed(14*3600.0), EPSILON); // the value should not change for later times
	}
