...
public class ActLinkFilterTest extends MatsimTestCase {
	...
	public void testActLinkFilter() throws Exception {
		...
		NodeImpl node1 = network.createNode(new IdImpl("1"), new CoordImpl(100.0, 100.0));
		NodeImpl node2 = network.createNode(new IdImpl("2"), new CoordImpl(200.0, 200.0));
		NodeImpl node3 = network.createNode(new IdImpl("3"), new CoordImpl(300.0, 300.0));
		...
		for (String personId : new String[]{"1", "2"}) {
			...
			plan.createActivity(actType, network.getLink(linkId));
