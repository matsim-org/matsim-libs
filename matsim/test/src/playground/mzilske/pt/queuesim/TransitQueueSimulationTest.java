...
public class TransitQueueSimulationTest extends TestCase {
	...
	public void testCreateAgentsForBackAndForthRoutes() {
		// setup: config
		ScenarioImpl scenario = new ScenarioImpl();
		scenario.getConfig().scenario().setUseVehicles(true);
		scenario.getConfig().scenario().setUseTransit(true);
		...
		{ // line 1, 2 routes, each 2 departure
			TransitLine line = builder.createTransitLine(scenario.createId("1"));
			{ // route 1
				TransitRoute tRoute = builder.createTransitRoute(scenario.createId("H"), route, stops, TransportMode.pt);
				Departure dep = builder.createDeparture(scenario.createId("depH1"), 8.0*3600);
				dep.setVehicleId(new IdImpl("veh1"));
				tRoute.addDeparture(dep);
				dep = builder.createDeparture(scenario.createId("depH2"), 10.0*3600);
				dep.setVehicleId(new IdImpl("veh1"));
				tRoute.addDeparture(dep);
				line.addRoute(tRoute);
			}
			{ // route 2
				TransitRoute tRoute = builder.createTransitRoute(scenario.createId("R"), route, stops, TransportMode.pt);
				Departure dep = builder.createDeparture(scenario.createId("depR1"), 9.0*3600);
				dep.setVehicleId(new IdImpl("veh1"));
				tRoute.addDeparture(dep);
				dep = builder.createDeparture(scenario.createId("depR2"), 11.0*3600);
				dep.setVehicleId(new IdImpl("veh1"));
				tRoute.addDeparture(dep);
				line.addRoute(tRoute);
			}
		...
		assertTrue(agents.get(0) instanceof TransitDriverAgent);
		assertEquals(8.0*3600, ((TransitDriverAgent) agents.get(0)).getDepartureTime(), MatsimTestCase.EPSILON);
	}
