...
public class MyFileSamplerTest extends MatsimTestCase{
	...
	/**
	 * This test checks that the right number of files of the right type are sampled if 
	 * and <i>copied</i> instead of moved to a given destination folder. If 
	 * <code>a</code> is the number of files of correct type in a group of <code>b</code> 
	 * files where <code>a < b</code>, then, given that <code>c <= a</code>, the right 
	 * number of files sampled should be <code>c</code>.
	 */
	public void testSmallSample(){
		...
		textSampler.sampleFiles(numberToSample, txtFilter);

		assertEquals("Incorrect number of files sampled.", numberToSample, textOutputFolder.listFiles().length);
	 ...
	 * <code>a</code> is the number of files of correct type in a group of <code>b</code> 
	 * files where <code>a < b</code>, then, given that <code>c > a</code>, the right 
	 * number of files sampled should be <code>a</code>.
	 */
	public void testLargeSample(){
		...
		assertEquals("Incorrect number of files sampled.", rightFiles, textOutputFolder.listFiles().length);
		for (File file : textOutputFolder.listFiles()) {
			String thisExtension = file.getName().substring(file.getName().indexOf("."), file.getName().length());
			assertEquals("Wrong file identified by the filter", true, thisExtension.equals(rightExtension));
		}
